...
## Active engagements
- Defense Unicorns / UDS meshing proposal for Andrew Greene — see `docs/proposals/defense-unicorns/`. Tuesday package (vision deck, architecture, meshing write-up, problem briefs, proof plan, evidence index, rendered .pptx/.pdf deck, email .docx, bundle zip) lives in `docs/proposals/defense-unicorns/tuesday/`; live view at a11oy `/uds`. Build script: `python3 scripts/build-tuesday-package.py`.
...
<!-- doctrine-scanner-exempt: this section enumerates live PostgreSQL table names (doctrine_pillpintu_config, khipuDoctrine.ts) and product-section names that retain the pre-rename token verbatim because schema/file renames are tracked as separate debt. Doctrine v6 scanner skips this file by marker. -->
...
- **Continuum Aerial Twin & Khipu Layer:** Doctrine hubs for digital-twin and defensive architecture.
- **Khipu Doctrine Governance (DB-backed):** All ~21 doctrine governance pages in A11oy are wired to real PostgreSQL tables via REST API endpoints at `/api/doctrine/*`. Schema in `lib/db/src/schema/doctrine.ts` (21 tables: `doctrine_constitutions`, `doctrine_behavioral_audits`, `doctrine_welfare_signals`, `doctrine_red_team_probes`, `doctrine_reward_hacking`, `doctrine_alignment_reviews`, `doctrine_code_behaviors`, `doctrine_covenant_lift`, `doctrine_risk_reports`, `doctrine_snapshots`, `doctrine_user_turn_signals`, `doctrine_capability_snapshots`, `doctrine_partners`, `doctrine_pillpintu_config`, `doctrine_cavd_records`, `doctrine_robustness_snapshots`, `doctrine_transparency_reports`, `doctrine_welfare_playbooks`, `doctrine_defender_credit_pool`, `doctrine_dsl_examples`, `doctrine_dsl_simulations`). CRUD routes in `artifacts/api-server/src/routes/doctrine-crud.ts`. Frontend hooks in `artifacts/a11oy/src/hooks/useDoctrine.tsx`. Seed endpoint: `POST /api/doctrine/seed`. Composite endpoints: `GET /api/doctrine/overview`, `GET /api/doctrine/system-card/:agentId`. The static data file `artifacts/a11oy/src/data/khipuDoctrine.ts` retains only types, constants, and helpers.
