...
- **PRISM Counsel:** Legal matter observability and governed execution vertical — full command center for plaintiff-side NY insurance litigation teams. **Extracted as a standalone artifact** at `artifacts/prism-counsel` (preview path `/prism-counsel/`, port 26500, Task #346). ~30-table DB schema, 3 demo matters (auto/FL, premises/NJ, coverage/NY), 80+ pages covering all PRISM sections. Uses 6-pillar PRISM scoring (Posture, Readiness, Integrity, Strategy, Money, Governance). NY insurance wedge includes Regulation 68 compliance clocks, insurer behavior profiling, venue intelligence, and no-fault claim tracking. API routes at `/api/prism-counsel/*`. SZL Holdings references this as a link, no longer co-hosting the frontend. Marketing page at `/solutions/prism-counsel`.
