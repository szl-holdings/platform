...
- **Mobile Shared Library (`lib/mobile-shared`, `@szl-holdings/mobile-shared`):** Shared React Native components and hooks used across all 7 mobile apps: `ErrorBoundary`, `SkeletonLoader`, `KeyboardAwareScrollViewCompat`, `useApiStatus`, `useWebSocket`, and `useRealtimeChannel`. Each mobile app's metro.config.js includes this package in its watchFolders.
