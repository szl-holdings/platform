...
- **Carlota Jo Drip Email Engine:** Lead nurturing drip sequences.
...
- **Historical Event Backfill (Signal Bus):** Endpoint for replaying historical signals.
...
- **Email Deliverability:** Centralized library for all outbound transactional email.
...
- **A11oy ARGO — Field Intelligence Forge (`/a11oy/argo`, v0.2.0-seed):** Distillation surface that fuses two tracks of external signal with the Sentra defensive executive doctrine and the Mythos public-research brief. Track 1 — Open-Source Primitives: 8 capability seeds studied with-knowledge-of from the public ei-grad/Andrew Grigorev repository index (kill-timeout, samogonka knowledge distillation, verify-oidc-token, check-expiring-certs, trinkup hardlink snapshots, python-conntrack, docker-freeradius-eap-tls, flask-shell-ipython). Track 2 — Frontier Methods (public research): 5 capability seeds reimplemented from first principles using only Anthropic public material (anthropics/skills standard, public system-card practice, chain-of-thought monitorability per Mikulik & Bowman, OpenMythos RDT/MoE reference reconstruction, Frontier Red Team gate). Each seed is typed with literal-union ids (DoctrinePillarId, SeedId) and tagged by source kind (open-source/public-research), status (observed/distilled/adoptable/piloted), constellation target (A11oy/Sentra/Psyche/Argo), evolved agent, defensive guardrail, doctrine link, and 24-month roadmap phase. Five doctrine pillars enforce the floor: no hack-back (CFAA §1030), human-in-the-loop (NIST AI RMF), evidence-is-evidence (NIST SP 800-61r3), MTTD<1h/MTTR<4h (SOC benchmark), and public-inputs-only (no leaks, no scraped weights). Includes doctrine-aligned capability matrix (11 capabilities mapped to seeds), 24-month roadmap insertion view, and a citations block linking to the public Anthropic research index, NIST standards, CFAA, and DARPA AIxCC. Persistent DEFENSIVE-ONLY banner. Backed by `artifacts/a11oy/src/data/argoForge.ts` (typed seeds, doctrine pillars, alignment matrix, citations). Route registered at App.tsx; lazy-loaded.
- **A11oy Aerial Twin (Site-Specific Wireless Digital-Twin Doctrine):** Doctrine hub at `/a11oy/aerial-twin` distilling the public NVIDIA Aerial Digital Twin overview into the SZL ecosystem, with 8 operational milestone surfaces delivered at `/a11oy/aerial-twin/<slug>` (sionna-eval, vessels-port, terra-building, sentra-anomaly, federated-ledger, ric-binding, ai-ran, attestation-soc2). Each milestone surface has its own KPIs, deliverables, OSS distillations (showing what was taken from each leader's public repo and how we reimplemented it), guardrails, related-surface cross-links, and citations. Dynamic page at `AerialTwinMilestone.tsx` with data from `aerialTwinMilestones.ts`. Post-merge drizzle-kit timeout fixed: made non-fatal (800+ table schema) with 5-min window. Static doctrine surface at `/a11oy/aerial-twin` distilling the public NVIDIA Aerial Digital Twin overview into the SZL ecosystem. Six primitives (differentiable ray tracing, scene mesh, channel impulse response, RU/DU/CU emulation, RIC closed-loop integration, AI-RAN inference) grounded in six open-source leaders (Sionna, Sionna RT, OpenAirInterface, srsRAN, O-RAN Software Community, GNU Radio) with license + URL + distillation per leader. Three vertical bindings — Vessels (port 5G coverage, vessel-to-shore link budget, defensive RF anomaly playback), Terra (in-building 5G/WiFi coverage, mmWave shadowing, pre-leasing connectivity attestation), Sentra (rogue base station detection, IMSI catcher fingerprinting, jamming-resilience scoring). Five innovation seeds A11oy adds on top of the upstream brief (federated RF episode ledger, evidence-bound xApp registry, A11oy planner over RAN policy graph, defensive RF red-team in twin, site-specific coverage attestation). Nine-milestone phased roadmap (0–24 months) plus a five-layer guardrails stack (inputs, emission, tenant scope, closed-loop approval, provenance) and a public-source citations block. Backed by `artifacts/a11oy/src/data/aerialTwin.ts` (literal-union typed, readonly arrays, no telemetry) and `artifacts/a11oy/src/pages/AerialTwin.tsx`. Route registered at App.tsx; lazy-loaded.
- **A11oy Mythos Layer (Defensive Architecture Doctrine):** Static doctrine surface at `/a11oy/mythos-layer` codifying the A11oy×Sentra orchestration architecture from the Mythos-class executive summary. Contains the 10-step orchestration loop, the 6-tool scanner toolchain catalogue (CodeQL, Semgrep, OSV-Scanner, Trivy, Gitleaks, Checkov) with sources/licenses/Sentra bindings/guardrails, the public-research RL pipeline formulation (state/action/reward/regime/RLHF/safety), the 6-layer guardrails stack, the 0–24 month phased roadmap mapped onto Sentra modules, and public model references. Backed by `artifacts/a11oy/src/data/mythosLayer.ts` (literal-union typed, readonly arrays, no telemetry) and `artifacts/a11oy/src/pages/MythosLayer.tsx`. Defensive-only, evidence-bound, public-input only.
- **A11oy × Sentra Glasswing Command Layer (Risk-as-Reward):** Premium executive command surface at `/a11oy/glasswing` synthesizing the 10-agent constellation (Daedalus/Argus/Ariadne/Hephaestus/Pallas/Hermes/Cerberus/Silver/Sentinel/Oracle), a research-anchored "Risk-as-Reward" novelty thesis (cited to Silver & Sutton's Era of Experience 2025, MITRE ATLAS, arxiv 2401.07031, Microsoft Agent Governance Toolkit, OpenAI Aardvark), the Sentra Control Plane (PolicyEngine, SecretsVault, ApprovalGate, AuditLedger), Silver's RL planner with calibrated reward table, the patch command center, the Engineering Loop, bidirectional compliance mapping, and Cerberus hard offensive-boundary list. Backed by `artifacts/a11oy/src/lib/glasswing-schemas.ts` (Zod schemas for Finding, PatchCandidate, Approval, AuditEvent, RLState, RLAction, RLReward) and `artifacts/api-server/src/data/glasswingDoctrine.ts` (research citations, agent registry, seed data).
