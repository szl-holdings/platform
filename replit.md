...
- **Alloy Unified Command Surface (Task #268):** Alloy transformed from orchestration dashboard into unified command platform. New pages and features:
  - **Global Command Bar** (`/alloy/components/command-bar.tsx`): Cmd+K / Ctrl+K persistent overlay with slash commands (/workflow, /approve, /decisions, /skills, /operators, /home, /connectors, /docs, /signals, /runs, /analytics), typeahead for workflows, recent command history, keyboard navigation (↑↓, Enter, Esc).
  - **Workspace Home** (`/alloy/home`): Priority dashboard showing pending approvals, active agent runs (with live model/cost/latency), critical signals by severity/source, pending decisions by confidence, recent artifacts, and quick access navigation.
  - **Decision Objects** (`/alloy/decisions`): First-class entity for consequential agent outputs — title, verdict, confidence bar, evidence list with source citations, approval status chips (propose_only/approval_required/approved_execute/blocked_by_policy), approve/reject actions. CRUD + approve/reject API endpoints at `/api/alloy/decisions`.
  - **Skill Registry** (`/alloy/skills`): Formal registry of agent capabilities — versioned, categorized, approval-class controlled (auto/review/admin_only), input/output contracts, dry-run support, enable/disable toggle, usage stats. API at `/api/alloy/skills`.
  - **Operator Control Center** (`/alloy/operator`): Agent health grid (SR, P50, queue, cost/24h, violations), policy violations feed (with severity), approval bottlenecks, AI provider health (latency, error rate, cost), execution queue summary.
  - **New DB Tables:** `alloy_decisions`, `alloy_skills`, `alloy_skill_runs` — all created and indexed.
  - **Sidebar**: New "Intelligence" section with Decision Objects, Skill Registry, Operator Control + "Workspace Home" at top of Automation section.
