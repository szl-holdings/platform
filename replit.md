...
- **SZL Holdings Innovation Layer (Task #586):** Cross-domain natural language query engine, ambient signal ranker, and autonomous signal chain engine. Three new API routes: `cross-domain-query.ts` (NL query resolver with domain identification + correlation scoring), `signal-chains.ts` (3 default chains: maritime→real-estate, security→legal, market→portfolio; trigger/evaluate/audit-log endpoints wired into `logActivity` audit trail), `correlation-map.ts` (entity graph nodes/edges). Four new Command Portal components: `FusionBar` (full-width NL search with domain-bucketed results), `AmbientSignalRanker` (priority-ranked cross-domain signals), `CorrelationMapViz` (SVG network graph with zoom/filter/hover), `SignalChainsPanel` (chain list with trigger + audit trail). Two new full-page views: `/command/correlation-map` and `/command/signal-chains`. Dashboard updated with all components and SectionNav. Mobile CORTEX app updated with a Morning Brief card featuring health badge, executive summary, stats, and per-signal rows.
