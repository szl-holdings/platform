...
- **Terra (`terra`, route: `/terra/`):** Full brokerage command platform — Distress Intelligence & Brokerage OS for NYC/NYS real estate (SZL). Dark, data-dense. Terra gold accent (#a07848), terra-bg (#0b1220), terra-surface (#111c2d). Pages: Command Center, Listings (grid/table, DOM, risk), Leads + CRM (real API — live CRUD from `terra_leads` table, pipeline scoring, convert-to-deal), Deal Pipeline (15-stage kanban/table, real API from `terra_deals` table, bottleneck detection), Distress Intelligence Engine (6 distress types, borough filters, map view, "Convert to Lead" live API call), Broker Overview (aggregate KPIs, funnel, top boroughs, score distribution), Ingestion Framework (CSV upload, NYC Open Data trigger, run history). Data: `src/data/brokerage.ts` (static/mock for listings/team). Components: `src/components/brokerage-ui.tsx`. DB tables: `terra_distress_properties`, `terra_distress_alerts`, `terra_ingestion_runs`, `terra_leads`, `terra_deals`, `terra_saved_opportunities`. API routes: `/terra/crm/leads`, `/terra/pipeline/deals`, `/terra/convert/distress-to-lead`, `/terra/convert/lead-to-deal`, `/terra/opportunities/save`, `/terra/distress/ai-score`, `/terra/broker/overview` (all in `api-server/src/routes/terra-crm.ts`). AI scoring: `api-server/src/lib/terra-ai-scoring.ts` — deterministic weighted formula for distress→opportunity scoring.
