...
function CampaignThumbnail({ category }: { category: string }) {
  ...
  return (
    <div className="w-full h-36 rounded-t-xl flex items-center justify-center relative overflow-hidden" style={{ background: gradient }}>
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <p className="text-slate-400 mt-1">Brand films, social campaigns, product launches, and event content — all in one execution view.</p>
        </motion.div>
      ...
      {!isLoading && (!campaigns || campaigns.length === 0) && (
        <div className="text-center py-12">
          ...
          <p className="text-slate-500 text-sm">No campaigns yet. Create your first campaign to get started.</p>
        </div>
      )}
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {campaigns?.map(campaign => (
          <motion.div key={campaign.id} variants={item}>
            <Link href={`/creative/campaigns/${campaign.id}`} className="block group">
              <div className="overflow-hidden border border-white/8 hover:border-cyan-400/30 hover:shadow-lg hover:shadow-cyan-400/5 transition-all duration-300 relative bg-[#0d1117] rounded-xl">
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`text-[10px] px-2 py-0.5 rounded border capitalize ${campaign.status === "review" ? "border-amber-500/30 text-amber-400" : campaign.status === "published" ? "border-emerald-500/30 text-emerald-400" : campaign.status === "post_production" ? "border-violet-500/30 text-violet-400" : campaign.status === "concept" ? "border-blue-500/30 text-blue-400" : "border-white/10 text-slate-400"}`}>
                  ...
                  {campaign.kpis && campaign.kpis.length > 0 && (
                    <div className="grid grid-cols-2 gap-1.5 my-2">
                      {campaign.kpis.slice(0, 2).map((kpi, i) => (
                        <div key={i} className="bg-white/3 rounded-lg px-2 py-1 border border-white/8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="lg:col-span-2">
          <div className="p-5 border border-white/8 bg-[#0d1117] rounded-xl">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/creative/content-calendar">
              ...
              {calendarEvents.map((event, i) => (
                ...
                >
                  <div className="text-center shrink-0 w-12">
                    ...
                    <div className="text-[10px] text-slate-500 uppercase">{format(event.date, "MMM")}</div>
                  </div>
