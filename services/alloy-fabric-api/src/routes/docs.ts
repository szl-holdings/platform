  ...
  endpoints: [
    {
      method: "POST",
      path: "/v1/embed",
      description: "Produce dense embeddings for one or more text strings. Requires X-Tenant-ID header.",
      auth: "Bearer token",
      body: {
    ...
    },
    {
      ...
      description: "Hybrid retrieval combining dense ANN search and keyword BM25 scoring, fused via RRF.",
      auth: "Bearer token",
      body: {
        requestId: "string (required)",
        tenantId: "string (required)",
    ...
    },
    {
      method: "POST",
      path: "/v1/evals/run",
      description: "Run a retrieval evaluation against a golden fixture set. Returns nDCG, recall, precision, MRR.",
      auth: "Bearer token",
    },
    {
      method: "POST",
      path: "/v1/openai/embeddings",
      description: "OpenAI-compatible embeddings endpoint. Drop-in replacement for openai.embeddings.create().",
      auth: "Bearer token",
      body: {
        input: "string | string[]",
