...
import { storageBundle, policyEngine, tenantEnforcer, defaultLedgerStore, profileRegistry } from "../context.js";
...
export function registerSearchRoute(router: Router): void {
  router.post("/v1/hybrid-search", async (req: Request, res: Response) => {
    ...
    const { requestId, profileId, query, topK, candidatePool, denseWeight, keywordWeight, metadataFilter: rawFilter, rerankEnabled } = parsed.data;
    ...
    const transformedQuery = profileId ? applyProfilePromptTransform(normalizedQuery, profileId) : normalizedQuery;
    stageMs["normalize_query"] = Date.now() - stageStart; stageStart = Date.now();
    ...
    if (profileId !== undefined) {
      try {
        ...
        res.status(400).json({ error: "unknown_profile", profileId, message: "Profile not found; register the profile or omit profileId to use the tenant default." });
        return;
      }
    }
    stageMs["load_profile"] = Date.now() - stageStart; stageStart = Date.now();
    ...
    stageMs["policy_check"] = Date.now() - stageStart; stageStart = Date.now();
    ...
    stageMs["query_shaping"] = Date.now() - stageStart; stageStart = Date.now();

    // 5. dense_ann — tenant-scoped ANN similarity search
    const denseRaw = await storageBundle.vectors.similaritySearch({ vector: queryVector, topK: candidatePool, tenantId, ...(profileId !== undefined ? { profileId } : {}) });
    const denseHits: DenseHit[] = denseRaw.map((h) => ({ chunkId: h.chunkId, sourceId: h.sourceId, score: h.score, metadata: h.metadata }));
    stageMs["dense_ann"] = Date.now() - stageStart; stageStart = Date.now();

    // 6. keyword_bm25 — tenant-scoped term match
    const keywordRaw = await storageBundle.metadataIndex.keywordSearch({ terms: transformedQuery, topK: candidatePool, tenantId });
    const keywordHits: KeywordHit[] = keywordRaw.map((h) => ({ chunkId: h.chunkId, sourceId: h.sourceId, score: h.score, highlights: h.highlights, metadata: h.metadata }));
    stageMs["keyword_bm25"] = Date.now() - stageStart; stageStart = Date.now();
    ...
    stageMs["exact_match_boost"] = Date.now() - stageStart; stageStart = Date.now();
    ...
    const fusedHits = reciprocalRankFusion(preBoostedDenseHits, keywordHits, { denseWeight, keywordWeight }).slice(0, candidatePool);
    const boostedHits = wrapAsBoosted(fusedHits);
    stageMs["rrf_fusion"] = Date.now() - stageStart; stageStart = Date.now();

    // 9. metadata_filter — apply caller-supplied scoping predicate
    const filteredHits = rawFilter ? applyMetadataFilter(boostedHits, rawFilter as MetadataFilter) : boostedHits;
    stageMs["metadata_filter"] = Date.now() - stageStart; stageStart = Date.now();
    ...
    stageMs["rerank"] = Date.now() - stageStart; stageStart = Date.now();
    ...
    stageMs["evidence_assemble"] = Date.now() - stageStart; stageStart = Date.now();

    // 13. ledger_write — governance record; UNCONDITIONAL; failure is surfaced loud
    let ledgerFailures = 0;
    const completedAt = new Date().toISOString();
    for (const citation of citations) {
      const scoreBreakdown: Record<string, number> = { fusedScore: citation.fusedScore, finalScore: citation.score };
      ...
      if (citation.rerankerScore !== undefined) scoreBreakdown["rerankerScore"] = citation.rerankerScore;
      ...
      try {
        defaultLedgerStore.append({
          ...
          ...(citation.rerankerScore !== undefined ? { rerankerScore: citation.rerankerScore } : {}),
        ...
        ledgerFailures++;
        logger.error("CRITICAL ledger write failed", { chunkId: citation.chunkId, reqId, traceId, err: String(err) });
      }
    }
    stageMs["ledger_write"] = Date.now() - stageStart; stageStart = Date.now();
    ...
    const enrichedHits = citations.map((c) => {
      ...
      if (!validation.success) {
        logger.warn("SearchHit contract violation", { chunkId: c.chunkId, issues: validation.error.issues });
      }
      ...
      return {
        ...
        sourceType: (c.metadata?.["contentType"] as string | undefined) ?? (c.metadata?.["sourceType"] as string | undefined) ?? "document",
        documentTitle: (c.metadata?.["title"] as string | undefined) ?? (c.metadata?.["documentTitle"] as string | undefined) ?? c.chunkId,
        ...(profileVersion !== undefined ? { profileVersion } : {}),
        exactMatchBoosts: c.boostApplied ? (c.metadata?.["boostRuleId"] ? [String(c.metadata["boostRuleId"])] : ["exact_match"]) : [],
        rationale: policyDecision.reasons.length > 0 ? policyDecision.reasons.join("; ") : "policy_allow",
        selectedRationale: policyDecision.reasons.length > 0 ? policyDecision.reasons.join("; ") : "policy_allow",
        evidenceId: `${traceId}-${c.chunkId}`,
        // Per-hit evidence traceability — evidence-first contract; 13-stage canonical pipeline
        traceId,
        retrievalPath: [
          "normalize_query", "load_profile", "policy_check", "query_shaping",
          "dense_ann", "keyword_bm25", "exact_match_boost", "rrf_fusion",
          "metadata_filter", "rerank", "evidence_assemble",
          "ledger_write", "response_normalization",
        ],
      };
    });

    logger.info("hybrid-search completed", { reqId, traceId, tenantId, profileId, hits: enrichedHits.length, processingMs: Date.now() - startMs });
    ...
    res.json({
      ...
      stageTimings: stageMs,
      policyDecision: { allow: policyDecision.allow, redactions: policyDecision.redactions, appliedRuleIds: policyDecision.appliedRuleIds },
      hits: enrichedHits,
