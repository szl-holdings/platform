# A11OY — 9 living agentic codex kernels
...
`A11OY` (16-node Khipu cord / router + orchestrator) runs **9 perpetual agentic kernels**: 7 universal (shared by every chakra) + 2 vertical (specific to this chakra). Each kernel is an OODA loop (**observe → decide → act → sign**) rooted in a SQLite-backed, hash-linked, DSSE-signed **codex**. Every tick appends a signed heartbeat receipt to the Khipu and emits an OpenTelemetry span `szl.kernel.a11oy.<name>.tick`.

## Lifecycle API

All endpoints are mounted **additively** under `/api/a11oy/v3/kernels` (preserving every pre-existing route). Deployed in flagship commit `e025e39`.
...
| GET  | `/api/a11oy/v3/kernels/{name}/heartbeat` | The kernel's most recent signed heartbeat |
| GET  | `/api/a11oy/v3/kernels/{name}/codex` | Hash-linked codex entries (one per tick) |
| POST | `/api/a11oy/v3/kernels/{name}/tick` | Force a single tick (returns the receipt) |
| POST | `/api/a11oy/v3/kernels/{name}/start` | Start a stopped kernel (admin-gated, fail-closed 403) |
| POST | `/api/a11oy/v3/kernels/{name}/stop` | Stop a kernel (admin-gated, fail-closed 403) |
| GET  | `/api/a11oy/v3/kernels/feed` | Server-Sent Events stream of heartbeats |
| POST | `/api/a11oy/v3/kernels/wake-receipt` | Sign a wake event (used by the warming cron) |
...
|---|---|---|---|
| `route` | 60s | `llm-router` | Observes LLM-router selections; tracks model/cost/latency. |
| `orchestrate` | 60s | `puriq-host-plan` | Observes PURIQ host orchestration plans; tracks step success. |
...
> **Heartbeat shape:** `{kernel, tick, ts, alive, did_work, summary, otel_span:"szl.kernel.a11oy.<name>.tick", doctrine:"v11", codex_head:"sha256:…", signed_payload:{payloadType, payload, signatures}}`. Signing uses the host Space's `szl_dsse` if present, otherwise an honest PLACEHOLDER DSSE envelope (never silently unsigned).
...
- Wired into the flagship via `import szl_kernels_organ as _kernels; _kernels.register(app, organ="a11oy")` (ADDITIVE).
