...
| **ASI07 Insecure Inter-Agent Communication** | **MODELED** | [`packages/gateway/src/gateway.ts`](../packages/gateway/src/gateway.ts) implements an `AgentGateway` request path that composes authentication, authorization, approval, audit, and execution modules; [`packages/gateway/test/gateway-integration.test.ts`](../packages/gateway/test/gateway-integration.test.ts) exercises that path. [`lib/ai-engine/src/a2a-registry.ts`](../lib/ai-engine/src/a2a-registry.ts) and [`lib/ai-engine/src/a2a-delegation.ts`](../lib/ai-engine/src/a2a-delegation.ts) model A2A discovery and task delegation. | The source evidence does not yet prove peer identity, message authentication, replay resistance, confidentiality, schema enforcement, or failure containment across every deployed inter-agent transport; retain runtime evidence for those properties before raising the claim. |
