...
## FINAL PUSH LOG (2026-06-01 ~09:23 UTC) — proof artifacts now published

- **HF** SZLHOLDINGS/uds-bundles-v1 (dataset): commit `ab5cc3ff7977b06083bc8114de0b79f419112d7c`.
  Now INCLUDES artifacts/ (5 signed PROOF .tar.zst + .uds.sig + cosign_signing_key.pub +
  PROOF_CAPTURE.txt + DEPLOY_DRYRUN.txt + SHA256SUMS_PROOF.txt + VERIFY_RECHECK.txt).
  Live 200 confirmed on resolve URL for SHA256SUMS_PROOF.txt.
- **GitHub** szl-holdings/uds-bundles (PRIVATE): commit `f54adaf` on main.
  uds-bundles/artifacts/ confirmed via gh api to contain all 5 packages + 5 sigs + 4 proof logs + pubkey.
- **AIRGAP_TEST_REPORT.md** updated: BLOCK → PARTIAL GREEN (5/5 signed PROOF packages built,
  verified, inspected, deploy-dry-run OK; image-bearing build + kind airgap test staged for docker host).
- Live re-verification 09:23 UTC: all 5 cosign verify-blob = "Verified OK" (artifacts/VERIFY_RECHECK.txt).
