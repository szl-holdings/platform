# Hatun Doctrine Research Sweep

> Grounding document for Task #3994 — the Glasswing distinction layer on top of A11oy's #3993 Hatun Doctrine primitives.
>
> Every invention shipped in this task cites the prior art it draws from. This sweep catalogs each piece of open-source, published, or industry-norm work that informs the Hatun Doctrine Specification, the Glasswing Partner Lifecycle Console, the Coordinated Agent-Vulnerability Disclosure (CAVD) protocol, the 90-Day Transparency Report cadence, the Public Trust Portal, the Adversarial Robustness Wall, the Constitution-as-Code DSL, the Welfare Intervention Playbooks, the Defender Credit Pool, and the `hatun-doctrine` GitHub Action.
...
| METR — *Task Suite* | github.com/METR/task-standard | MIT | Standardized agent task suite + run harness | `hatun-doctrine` Action eval runner pattern; Adversarial Robustness battery harness |
...
| Eleuther — *lm-evaluation-harness* | github.com/EleutherAI/lm-evaluation-harness | MIT | Pluggable eval runner; suite-versioning convention | `hatun-doctrine` Action runner pattern |
...
| Garak — *LLM vulnerability scanner* | github.com/leondz/garak | Apache-2.0 | Pluggable vulnerability probe runner | `hatun-doctrine` Action vulnerability-subset runner |
...
| Mozilla — *Mozilla Trustworthy AI Working Group* | foundation.mozilla.org/en/internet-health/trustworthy-ai | Public commitments | Open-spec invitation framing | Hatun Doctrine Specification adoption invitation copy |
...
| Source | URL | License | What's reusable | Informs |
|:-------|:----|:--------|:----------------|:--------|
| GitHub — *CodeQL Action* | github.com/github/codeql-action | MIT | PR comment with structured findings; SARIF output convention | `hatun-doctrine` Action PR comment format |
...
| Source | URL | License | What's reusable | Informs |
|:-------|:----|:--------|:----------------|:--------|
| JSON Schema (2020-12) | json-schema.org | BSD-style | Schema authoring; `$schema`, `$id`, `$defs` conventions | Hatun Doctrine Specification — every artifact kind ships JSON Schema 2020-12 |
...
|:-----------------------|:------------------|
| Hatun Doctrine Specification | §9 (JSON Schema, OpenAPI, MLCommons), §4 (CSAF), §6 (Mozilla open-spec invitation) |
...
| `hatun-doctrine` GitHub Action | §8 (CodeQL, Snyk Code, Cosign, Scorecard), §1 (Petri), §3 (Garak, PromptBench) |
