...
The **Mythos Doctrine Open Spec** is an open standard for the artifacts that govern enterprise agentic AI: constitutions, system cards, risk reports, behavioral-audit findings, welfare telemetry, adversarial-robustness scores, snapshot fingerprints, covenant-lift samples, partner attestations, and coordinated agent-vulnerability disclosures.
...
Grounded in the prior art catalogued in [`MYTHOS_RESEARCH_SWEEP.md`](../../MYTHOS_RESEARCH_SWEEP.md). See §9 of that sweep for spec-authoring conventions and §10 for the full citation map.
...
| `CovenantLiftSample` | One paired (governed vs. helpful-only shadow) brief outcome. | §1 (Mythos #3993) |
...
```ts
import type { Constitution, SystemCard } from "@mythos-doctrine/spec/types";
// or, fetch the JSON Schema directly
const schema = await fetch(
  "https://a11oy.io/spec/mythos-doctrine/0.1.0/schemas/constitution.json"
).then(r => r.json());
```

A11oy serves the schemas at `/spec/mythos-doctrine/<version>/schemas/<kind>.json`. The spec viewer at `/a11oy/mythos-spec` renders human-readable views of every kind with a copyable `$schema` URL, a `$ref` map, and live A11oy-emitted examples.
...
— A11oy, author and operator of the Mythos Doctrine Open Spec.
