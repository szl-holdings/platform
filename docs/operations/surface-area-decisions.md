...
| `formatDate()` / `LiveClock` (timestamp surfaces) | Aegis SOC dashboards, Command action center, Carlota Jo booking, RevenuePanel, SubscriptionManager, mobile articles/sessions all defined local copies that ignored the user's chosen time zone. | **Consolidated (Task #2197).** Web surfaces now import `formatDate` / `LiveClock` from `@szl-holdings/shared-ui/{utils,live-clock}`, which honor the saved user time zone via `resolveTimeZone()`. The shared `formatDate` was extended with an `intlOptions` passthrough so callers needing weekday/long-month variants can stay on the shared helper. Mobile surfaces (`@szl-holdings/mobile-shared/utils`) export an API-compatible `formatDate` plus `setMobileUserTimeZone()` so a future preferences sync layer can plumb the user's zone through without touching call sites. Local `Date.toLocaleString()` / `toLocaleDateString()` calls for visible timestamps in those files are removed. |
