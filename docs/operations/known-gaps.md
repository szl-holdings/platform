...
after seven days. Operators that require a recent committed observation run
`pnpm surfaces:freshness`, which adds the deliberate seven-day age audit to the same live checks.
The workflow uses the existing status context and does not weaken or replace a protected check.
