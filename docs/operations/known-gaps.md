...
| TD-006 | PRODUCT-SURFACES.md mobile section ambiguous about which mobile apps exist vs. are planned (aegis-mobile, vessels-mobile, terra-mobile, lyte-mobile, carlota-jo-mobile, and the APEX / `szl-holdings-mobile` flagship target) | Doc Accuracy | ✅ Resolved May-2026 (Task #5217) — `PRODUCT-SURFACES.md` § "Mobile Surfaces" rewritten with explicit "Shipped Mobile Surfaces" and "Roadmap" subsections. **Reconciled to ground truth:** no mobile artifacts are currently registered in the monorepo, so the Shipped list is empty and APEX (`artifacts/szl-holdings-mobile`) is now correctly classified as roadmap alongside the domain-specific mobile apps. Every roadmap row carries a 🗺️ status badge and cross-links back to this TD-006 entry. The top-level Surface Taxonomy table no longer implies APEX is live. Previous resolution (Apr-2026) had reclassified the domain-specific apps but still showed APEX as "Functional alpha" against a non-registered artifact — that inconsistency is what this revision corrects. |
