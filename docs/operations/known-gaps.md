...
**Last updated:** 2026-08-08 (rev 24 - measured public-surface truth)
...
The generated public-surface registry and deterministic
[`artifacts/PUBLIC_SURFACES.json`](../../artifacts/PUBLIC_SURFACES.json) distinguish source-tree
product inventory from measured public-route evidence. Revision 24 is bound to the artifact
observation at `2026-08-08T22:39:30.948Z`; it does not promote source presence into runtime
availability. Public quantitative claims remain governed by the canonical metrics registry and
generated [`docs/platform-facts.md`](../platform-facts.md), not by historical app directories or
marketing copy.

The measured A11oy.net static thesis, `robots.txt`, `sitemap.xml`, and `manifest.webmanifest`
returned HTTP 200. The manifest is therefore `REACHABLE` metadata with semantic JSON validation
and exact source ownership at `szl-holdings/a11oy-net/manifest.webmanifest`; it is not a product
runtime claim. `a11oy.net/chat` and `a11oy.net/code` returned HTTP 404 and remain explicit
`UNAVAILABLE` gaps. The A11oy application routes at `a-11-oy.com`, including the historical
`/lyte`, `/aegis`, `/vessels`, `/terra`, `/counsel`, `/carlota-jo`, `/command`, and `/pulse`
previews, returned HTTP 503 and remain `UNAVAILABLE` rather than being represented by the stale
404 snapshot.

Killinchu is also failed closed: its public console, source-binding endpoint, and readiness
endpoint each returned HTTP 503 while the Space was `PAUSED` by the measured cpu-basic quota
boundary (`requested=1`, `current=5`, `limit=3`). No redirect, readiness, or source-binding success
is claimed. A reachable documentation or metadata route is not an uptime, customer,
feature-completeness, or correctness claim; `LIVE`, `MIXED`, `DOCUMENTATION`, and `UNAVAILABLE`
modes retain that boundary.
