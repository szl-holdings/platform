# A11oy — FedRAMP Authorization Disclosure
...
This document discloses A11oy's current FedRAMP authorization posture in plain language so that procurement officers, prime contractors, and risk reviewers can make accurate sourcing decisions without inference.
...
| Item | Status |
|---|---|
| FedRAMP Marketplace listing | **Not listed.** A11oy is not currently a FedRAMP-authorized service. |
| FedRAMP Ready designation | Not yet pursued. |
| FedRAMP In-Process | Not yet pursued. |
| FedRAMP Authorized | Not authorized at any impact level. |
| Agency ATO inheritance | None at this time. |
| StateRAMP | Not listed. Pre-application research underway. |
| TX-RAMP | Not listed. |

A11oy is **not** marketed, sold, or represented as FedRAMP-authorized. Any reseller, partner, or prime that represents otherwise is doing so without SZL Holdings' authorization.

## 3. What this means for buyers

- **Federal high / FedRAMP-required workloads:** A11oy may not be procured for systems where FedRAMP authorization is a hard contractual requirement until the program is completed.
- **State and local workloads with no FedRAMP mandate:** A11oy may be evaluated on a case-by-case basis under the buyer's own risk framework. The compensating controls in §6 are the basis for that evaluation.
- **Hybrid procurement:** Some agencies allow a non-FedRAMP component if the system boundary excludes it from federal-data flows. SZL will execute a written boundary statement on request.

## 4. Authorization roadmap

The roadmap below is the authorization plan SZL Holdings is committing to in writing. Dates reflect SZL-owned milestones; FedRAMP PMO and 3PAO timelines are outside SZL's control and are stated as best-effort targets.
...
| Pre-readiness | Engage FedRAMP-recognized 3PAO for Readiness Assessment | Q4 2026 | SZL Compliance |
| FedRAMP Ready | Submit Readiness Assessment Report (RAR) | Q1 2027 | 3PAO + SZL |
| FedRAMP In-Process | Sponsor agency identified and PMO kickoff | Q2 2027 (sponsor-dependent) | Sponsor agency |
| FedRAMP Authorized (Moderate) | Final ATO issuance | Q4 2027 – Q2 2028 (best-effort) | Sponsor agency + PMO |
...
## 5. Hosting and FedRAMP-inheritable boundary
...
For pilot deployments where the buyer requires US-only FedRAMP-inheritable infrastructure, SZL will deploy A11oy into AWS GovCloud (US) under the buyer's own AWS account or a SZL-managed GovCloud account, with the boundary statement in §3 attached.

## 6. Compensating controls (today)

Until FedRAMP authorization is complete, A11oy's customers are protected by the following compensating controls. These are the controls a buyer's risk framework should evaluate against in lieu of an authorization letter:
...
For any active procurement, SZL Holdings will execute a written "FedRAMP Status Attestation" on request, signed by the founder, that contains:
...
Request: `inquiries@szlholdings.com`, subject line `FedRAMP Status Attestation request — [agency name]`.
