...
| **Finding**   | `node scripts/check-doctrine-v6.mjs` exits non-zero with 9 forbidden-token violations (legacy tokens `Mythos`, `Glasswing`, `AlloyScape`) across `dossier/payload-2026-05-25/*`, `packages/agi-forecast/data/*.json`, `scripts/release/uds-version-sync.json`, `artifacts/api-server/src/routes/foundry-deepseek-v4.ts:99`, `.agents/agent_assets_metadata.toml:631`. |
| **Risk**      | A CI gate failing on `main` signals "nobody is watching CI" during diligence. The most ironic offender is `uds-version-sync.json` — the **source of truth for UDS releases** itself contains `doctrine: "Mythos-V1"`. |
| **Resolution**| Per scanner contract: (a) frozen historical receipts → add `doctrine-scanner-exempt` marker; (b) live drift (foundry-deepseek-v4.ts) → rename token; (c) rename `Mythos-V1` doctrine string in `uds-version-sync.json` to current doctrine name. |
| **Owner**     | Platform Engineering                                                                                                                                       |
| **Target**    | Today                                                                                                                                                      |
