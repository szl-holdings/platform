...
| Bundle CI smoke test | `.github/workflows/uds-bundle-smoke.yml` | On every PR touching the Zarf packages or the UDS bundle: (1) `zarf package create` against all three components, (2) `uds-cli bundle create` against the local-build bundle, (3) `uds bundle deploy` into a fresh kind cluster and wait for all three component namespaces (a11oy, sentra, amaru) to reach Available. A green badge here is "the wires are set up *and the bundle deploys clean.*" |
| Bundle publish + sign | `.github/workflows/uds-bundle-publish.yml` | On `uds-v*.*.*` tag or release published: builds and publishes the three Zarf packages to `ghcr.io/szl-holdings/packages/{a11oy,amaru,sentra}` and the UDS bundle to `ghcr.io/szl-holdings/bundles/szl-mesh`, signs every OCI artifact with cosign keyless (sigstore OIDC), and cuts a GitHub Release in the SZL house style (matching `szl-holdings/a11oy uds-v0.1.0`): tarball + `.sha256` + `.sig` + `.cert` sidecars, the two bundle manifests, the operator docs (`ARCHITECTURE.md`, `OPERATOR-QUICKSTART.md`, `SECURITY.md`, `UDS-BUNDLE.md`), and an `AUDIT-LOG.md` captured live from the verify chain. |
| Bundle operator docs | `docs/proposals/defense-unicorns/szl-holdings/uds-mesh/{ARCHITECTURE,OPERATOR-QUICKSTART,SECURITY,UDS-BUNDLE}.md` | Source-controlled docs that ship attached to every release. The five-command quickstart, the signature-chain explanation, the bundle composition reference, and the architecture diagram — all version-pinned alongside the YAML they document. |
