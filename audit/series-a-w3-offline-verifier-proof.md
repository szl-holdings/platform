...
- **baseline:** Root `pnpm typecheck` exited 1 because this fresh worktree had no
  `node_modules`, so `turbo` was unavailable before TypeScript execution.
- **test_results:**
  - `pnpm --filter @szl/verify test` — exit 0, 21/21 tests passed.
