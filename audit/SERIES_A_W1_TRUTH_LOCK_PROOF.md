...
| Workspace package manifests | 197 | MEASURED |
| Static Express-style route registrations | 181 | MEASURED |
...
Database tables, per-test counts, live runtime API endpoints, Lean sorry count,
Lambda median, and receipt-chain depth are explicitly `UNAVAILABLE`; no stale
number is carried forward. The 181 source registrations are not represented as
reachable live endpoints.
