...
- `test_results`:
  ...
  - Platform `node scripts/qa/scan-secrets.js .`: exit `0`, `CLEAN — no
    secrets found`;
  - Platform PR #556 `Lint commit messages`: exit `1`; both otherwise-valid
    commit bodies had one line over the configured 100-character maximum.
    The PR was superseded through a fresh exact-main branch and newly signed
    commit rather than amend, rebase, or force-push;
