...
export const NOTIFICATION_SEND_TOOL_MANIFEST: ToolManifest = {
  ...
  inputSchema: {
    type: "object",
    properties: {
      channel: { type: "string", enum: ["email", "slack", "sms", "push", "webhook"], description: "Delivery channel for the notification" },
      recipients: { type: "array", items: { type: "string" }, description: "List of recipient identifiers (emails, user IDs, etc.)" },
      subject: { type: "string", description: "Message subject (used for email and similar channels)" },
      body: { type: "string", description: "Notification body content" },
      priority: { type: "string", enum: ["low", "normal", "high", "urgent"], description: "Delivery priority level" },
    },
...
export const EXTERNAL_WEBHOOK_TOOL_MANIFEST: ToolManifest = {
  ...
  inputSchema: {
    type: "object",
    properties: {
      endpointId: { type: "string", description: "Registered endpoint identifier to deliver the webhook to" },
...
export const INFRA_PROVISION_TOOL_MANIFEST: ToolManifest = {
  ...
  inputSchema: {
    type: "object",
    properties: {
      resourceType: { type: "string", enum: ["compute", "storage", "network", "database", "cache"], description: "Type of infrastructure resource to provision" },
      spec: { type: "object", description: "Resource specification (IaC-compatible config object)" },
