...
export const PORTFOLIO_SNAPSHOT_TOOL_MANIFEST: ToolManifest = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      asOf: { type: "string", description: "ISO 8601 datetime for point-in-time snapshot (defaults to now)" },
      includeBreakdown: { type: "boolean", description: "Whether to include full allocation breakdown" },
    },
...
export const BUDGET_FORECAST_TOOL_MANIFEST: ToolManifest = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      period: { type: "string", enum: ["monthly", "quarterly", "annual"], description: "Budget period granularity" },
      horizonMonths: { type: "integer", minimum: 1, maximum: 36, description: "Number of months to forecast ahead" },
    },
    required: ["orgId"],
  },
...
export const REGULATORY_FILING_TOOL_MANIFEST: ToolManifest = {
  ...
  inputSchema: {
    type: "object",
    properties: {
      filingType: { type: "string", enum: ["10-K", "10-Q", "8-K", "SAR", "FINCEN"], description: "Type of regulatory filing" },
      ...
      draftMode: { type: "boolean", description: "Whether to prepare a draft (true) or submit live (false)" },
    },
