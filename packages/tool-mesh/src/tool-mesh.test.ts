...
import { SECURITY_TOOL_MANIFESTS, threatScanHandler, complianceCheckHandler, vulnerabilityReportHandler } from "./tools/security-tools.js";
import { FINANCE_TOOL_MANIFESTS, portfolioSnapshotHandler, budgetForecastHandler } from "./tools/finance-tools.js";
import { OPERATIONS_TOOL_MANIFESTS, metricsQueryHandler, notificationSendHandler } from "./tools/operations-tools.js";
...
describe("Security tool manifests", () => {
  ...
  it("incident-containment requires approval and is human-approval-mandatory tier", () => {
    const containment = SECURITY_TOOL_MANIFESTS.find(m => m.id === "security.incident-containment");
  ...
  it("vulnerability-report handler returns expected shape", async () => {
    const result = await vulnerabilityReportHandler({ severity: "high" }, SECURITY_TOOL_MANIFESTS.find(m => m.id === "security.vulnerability-report")!);
...
describe("Operations tool manifests", () => {
  ...
  it("external-webhook is external-client-facing tier", () => {
    const webhook = OPERATIONS_TOOL_MANIFESTS.find(m => m.id === "communication.external-webhook");
...
describe("Gateway approval-required flow", () => {
  ...
  it("allows tool call when guardian has a matching allow rule", async () => {
    const registry = new InMemoryToolRegistry();
    registry.register(SECURITY_TOOL_MANIFESTS.find(m => m.id === "security.vulnerability-report")!);
...
describe("Total tool coverage", () => {
  ...
  it("all manifests pass schema validation", () => {
    ...
    for (const manifest of allManifests) {
      expect(() => ToolManifestSchema.parse(manifest), `Schema validation failed for: ${manifest.id}`).not.toThrow();
    }
  });
});
