...
describe("Rate limiting enforcement", () => {
  it("blocks invocation when per-minute rate limit is exceeded", async () => {
    ...
    const second = await gateway.invoke("rate-limited-tool", { query: "second" }, { requestId: "r-rate-2" });
  ...
  it("allows invocation when under rate limit", async () => {
    ...
    const result = await gateway.invoke("generous-rate-tool", { query: "test" }, { requestId: "r-rate-3" });
    expect(result.success).toBe(true);
  });
  ...
  it("blocks when concurrency limit is reached", async () => {
    ...
    const result = await gateway.invoke("concurrency-tool", { query: "test" }, { requestId: "r-conc-1" });
    expect(result.success).toBe(false);
...
describe("Timeout enforcement", () => {
  it("fails invocation when handler exceeds timeoutMs", async () => {
    ...
    const result = await gateway.invoke("timeout-tool", { query: "test" }, { requestId: "r-timeout-1" });
    expect(result.success).toBe(false);
  ...
  it("succeeds when handler completes within timeoutMs", async () => {
    ...
    const result = await gateway.invoke("fast-tool", { query: "fast" }, { requestId: "r-timeout-2" });
    expect(result.success).toBe(true);
  });
});
...
describe("Fallback chain execution", () => {
  it("invokes fallback tool when primary fails", async () => {
    ...
    const primaryManifest: ToolManifest = ToolManifestSchema.parse({
      ...GRAPH_QUERY_TOOL_MANIFEST,
      id: "primary-fails",
      failureModes: [{ type: "error", fallbackToolId: "fallback-doc", retryable: false, maxRetries: 0 }],
    });
    ...
    const result = await gateway.invoke("primary-fails", { query: "lease" }, { requestId: "r-fallback-1" });
    expect(result.success).toBe(true);
  ...
  it("returns failure when no fallback is configured", async () => {
    ...
    const result = await gateway.invoke("no-fallback-tool", { query: "test" }, { requestId: "r-fallback-2" });
    expect(result.success).toBe(false);
...
describe("Observability emission", () => {
  it("emits latency_ms metric on successful tool call", async () => {
    ...
    const latencyMetric = globalCollector.snapshot().find(
      (m) => m.name === "latency_ms" && m.labels["toolId"] === "obs-tool",
    );
  ...
  it("emits tool_error_rate=1 on tool failure", async () => {
    ...
    const errorMetric = globalCollector.snapshot().find(
      (m) => m.name === "tool_error_rate" && m.labels["toolId"] === "obs-fail-tool",
    );
  ...
  it("records trace with tool call detail on success", async () => {
    ...
    const result = await gateway.invoke("graph-query", { query: "nodes" }, { requestId: "r-trace-1" });
...
describe("Guardian approval path", () => {
  ...
  it("human-approval-mandatory tools are blocked without explicit guardian approval rule", async () => {
    ...
    const result = await gateway.invoke(
      ...
      { fromAccount: "a1", toAccount: "a2", amountUsd: 1000, currency: "USD", justification: "Test" },
...
describe("ToolMeshExecutor", () => {
  ...
  it("summary reports correct success rate", () => {
    const executor = new ToolMeshExecutor();
    const base = {
      toolId: "t1", toolName: "T1", callerId: "c1",
      input: {}, output: {}, dryRun: false, latencyMs: 10,
  ...
  it("getHistory filters by toolId", () => {
    const executor = new ToolMeshExecutor();
    const base = {
      callerId: "c1", input: {}, output: {}, success: true,
      dryRun: false, latencyMs: 5,
      startedAt: new Date().toISOString(), completedAt: new Date().toISOString(),
    };
  ...
  it("executeWithTimeout rejects when exceeded", async () => {
    ...
    await expect(
      executor.executeWithTimeout(slowHandler, {}, {
        ...GRAPH_QUERY_TOOL_MANIFEST,
        id: "slow",
        name: "Slow",
      } as never, 50),
...
describe("ToolMeshMcpBridge", () => {
  ...
  it("getServerInfo returns valid MCP server info", () => {
    const registry = new InMemoryToolRegistry();
    registry.register(GRAPH_QUERY_TOOL_MANIFEST);
    const gateway = new ToolMeshGateway(registry, makeAllowGuardian(), new TraceWriter(new InMemoryTraceStore()), new ToolRateLimiter());
...
describe("Gateway — schema-less tool hard rejection", () => {
  it("blocks invocation of a tool without inputSchema", async () => {
    ...
    const guardian = makeAllowGuardian();
    const gateway = new ToolMeshGateway(registry, guardian, new TraceWriter(new InMemoryTraceStore()), new ToolRateLimiter());
    gateway.registerHandler("no-schema-invokable", async () => ({ ok: true }));

    const result = await gateway.invoke("no-schema-invokable", { anything: true }, { requestId: "schema-less-1" });
    expect(result.success).toBe(false);
    expect(result.error).toMatch(/missing required inputSchema/);
...
describe("MCP bridge — external tool execution via call()", () => {
  ...
  it("surfaces handler error from external tool as isError response", async () => {
    ...
    bridge.registerExternalTool({
      ...
      inputSchema: {},
      requiresApproval: false,
      handler: async () => { throw new Error("upstream unavailable"); },
    });
...
describe("Fallback — routes through full gateway controls", () => {
  it("fallback tool is subject to guardian denial", async () => {
    const primaryManifest: ToolManifest = ToolManifestSchema.parse({
      ...GRAPH_QUERY_TOOL_MANIFEST,
      id: "gateway-primary",
      failureModes: [{ type: "error", fallbackToolId: "gateway-fallback", retryable: false, maxRetries: 0 }],
    });
    ...
    const result = await gateway.invoke("gateway-primary", { query: "test" }, { requestId: "r-fbc-1" });
  ...
  it("fallback tool succeeds when it passes guardian and other controls", async () => {
    ...
    const primaryManifest: ToolManifest = ToolManifestSchema.parse({
      ...GRAPH_QUERY_TOOL_MANIFEST,
      id: "ctrl-primary",
      failureModes: [{ type: "error", fallbackToolId: "ctrl-fallback", retryable: false, maxRetries: 0 }],
    });
    ...
    const result = await gateway.invoke("ctrl-primary", { query: "lease" }, { requestId: "r-fbc-2" });
    expect(result.success).toBe(true);
  ...
  it("fallback stops at MAX_FALLBACK_DEPTH to prevent infinite loops", async () => {
    const manifest: ToolManifest = ToolManifestSchema.parse({
      ...GRAPH_QUERY_TOOL_MANIFEST,
      id: "looping-tool",
      failureModes: [{ type: "error", fallbackToolId: "looping-tool", retryable: false, maxRetries: 0 }],
    });
    ...
    const result = await gateway.invoke("looping-tool", { query: "loop" }, { requestId: "r-loop-1" });
    expect(result.success).toBe(false);
  }, 10000);
});
