...
describe('End-to-end run trace — full waterfall', () => {
  ...
  it('full waterfall produces replay-ready trace with all required fields', () => {
    ...
    writer.appendRetrieval(traceId, {
      source: 'sanctions-db',
      query: 'MMSI 538009241',
      hitCount: 1,
      latencyMs: 30,
      qualityScore: 0.88,
    });
    ...
    writer.appendVerifierDecision(traceId, {
      verifierId: 'v1',
      step: 'output-check',
      outcome: 'pass',
      score: 0.95,
    ...
    writer.addRollbackPoint(traceId, {
      rollbackId: 'rb1',
      spanId: 's1',
      label: 'pre-action',
      createdAt: now,
...
describe('TraceSdk — convenience SDK', () => {
  ...
  it('wrapToolCall instruments the tool call on the session', async () => {
    ...
    const wrappedFetch = sdk.wrapToolCall(
      session,
      'tool-fetch-001',
      'ais-fetch',
      async (mmsi: string) => {
...
describe('TraceRecordSchema — all required spec fields present', () => {
  it('schema accepts full waterfall trace', () => {
    ...
    const result = TraceRecordSchema.safeParse({
      ...
      verifierDecisions: [
        { verifierId: 'v1', step: 'output-check', outcome: 'pass', timestamp: now },
      ],
      reflections: [
        { reflectionId: 'r1', trigger: 'low-confidence', content: 'Added context', timestamp: now },
      ],
      ...
      approvals: [
        { approvalId: 'a1', approver: 'compliance@szl.io', decision: 'approved', timestamp: now },
      ],
      totalTokens: 2000,
      promptTokens: 800,
...
describe('TraceWriter — operator comments and run grading', () => {
  it('addOperatorComment stores comment with operatorId and tags', () => {
    ...
    writer.addOperatorComment('t-comment', 'operator-007', 'Approved reroute recommendation', {
      tags: ['decision'],
    });
