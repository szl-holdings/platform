...
import { QueuedTraceStore, WriteQueue } from './queue.js';
...
describe('TraceRecordSchema', () => {
  ...
  it('captures all spec fields — run_id, objective, self-model, world-model ref, plan graph', () => {
      ...
      reflections: [
        {
          ...
          content: 'Retry with more context',
          actionTaken: 'added-context',
          timestamp: now,
        },
      ],
      rollbackPoints: [
    ...
    expect(trace.selfModelSnapshot).toEqual({
      role: 'analyst',
      capabilities: ['finance', 'logistics'],
    });
...
describe('TraceWriter — extended fields', () => {
  ...
  it('completes a trace and sets completedAt', () => {
    writer.startTrace({ traceId: 't-001' });
    const completed = writer.completeTrace('t-001', {
      status: 'completed',
      latencyMs: 250,
      totalTokens: 500,
    });
...
describe('TraceDiff — extended comparisons', () => {
  ...
  it('detects tool additions and removals', () => {
    ...
    writer.appendToolCall('candidate', {
      toolId: 't2',
      toolName: 'embed',
      success: true,
      retries: 0,
  ...
  it('detects verifier pass rate delta', () => {
    ...
    const now = new Date().toISOString();
    writer.startTrace({ traceId: 'base' });
    writer.appendVerifierDecision('base', {
      verifierId: 'v1',
      step: 's',
      outcome: 'pass',
      timestamp: now,
    ...
    writer.startTrace({ traceId: 'candidate' });
    writer.appendVerifierDecision('candidate', {
      verifierId: 'v1',
      step: 's',
      outcome: 'pass',
      timestamp: now,
...
describe('TraceSdk', () => {
  ...
  it('wrapToolCall records failure on rejected call', async () => {
    ...
    const sdk = new TraceSdk(new TraceWriter(store), new TraceQueryEngine(store));
    const session = sdk.startSession({});
    const wrapped = sdk.wrapToolCall(session, 't-fail', 'breaker', async () => {
