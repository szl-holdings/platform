...
describe("loginBodySchema", () => {
  it("accepts valid credentials", () => {
    expect(
      loginBodySchema.parse({ email: "u@e.com", password: "x" }),
    ).toBeTruthy();
  });
  it("rejects an invalid email", () => {
    expect(() =>
      loginBodySchema.parse({ email: "no-at", password: "x" }),
    ).toThrow();
  });
  it("rejects empty password", () => {
    expect(() =>
      loginBodySchema.parse({ email: "u@e.com", password: "" }),
    ).toThrow();
  });
...
describe("loginPasswordBodySchema", () => {
  ...
  it("rejects password shorter than 8 chars", () => {
    expect(() =>
      loginPasswordBodySchema.parse({ email: "u@e.com", password: "short" }),
    ).toThrow();
  });
});
...
describe("wsTicketBodySchema", () => {
  ...
  it("accepts a channel", () => {
    expect(wsTicketBodySchema.parse({ channel: "general" }).channel).toBe(
      "general",
    );
  });
  it("rejects channel > 128 chars", () => {
    expect(() =>
      wsTicketBodySchema.parse({ channel: "x".repeat(129) }),
    ).toThrow();
  });
});
...
describe("mfaVerifyBodySchema", () => {
  it("accepts a 6-digit token", () => {
    expect(mfaVerifyBodySchema.parse({ token: "123456" }).token).toBe(
      "123456",
    );
  });
