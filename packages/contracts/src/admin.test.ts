...
describe('userListQuerySchema', () => {
  ...
  it('rejects search > 256 chars', () => {
    expect(() => userListQuerySchema.parse({ search: 'x'.repeat(257) })).toThrow();
  });
});
...
describe('observabilityTimeRangeQuerySchema', () => {
  ...
  it.each(['1h', '6h', '24h', '7d'] as const)('accepts window=%s', (window) => {
    expect(observabilityTimeRangeQuerySchema.parse({ window }).window).toBe(window);
  });

  it('rejects unknown window value', () => {
    expect(() => observabilityTimeRangeQuerySchema.parse({ window: '30d' })).toThrow();
  });

  it('coerces orgId from string', () => {
    expect(observabilityTimeRangeQuerySchema.parse({ orgId: '12' }).orgId).toBe(12);
  });

  it('rejects orgId <= 0', () => {
    expect(() => observabilityTimeRangeQuerySchema.parse({ orgId: 0 })).toThrow();
  });
});
