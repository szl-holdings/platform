...
describe('chatBodySchema', () => {
  ...
  it('rejects message > 32768 chars', () => {
    expect(() => chatBodySchema.parse({ message: 'x'.repeat(32769) })).toThrow();
  });
  it('rejects systemPrompt > 8192 chars', () => {
    expect(() => chatBodySchema.parse({ message: 'x', systemPrompt: 'y'.repeat(8193) })).toThrow();
  });
  it('rejects unknown domain', () => {
    expect(() => chatBodySchema.parse({ message: 'x', domain: 'bogus' })).toThrow();
  });
});

describe('analyzeBodySchema', () => {
  it('accepts valid input', () => {
    expect(analyzeBodySchema.parse({ content: 'x', analysisType: 'sentiment' })).toBeTruthy();
  });
  it('rejects empty content', () => {
    expect(() => analyzeBodySchema.parse({ content: '', analysisType: 'x' })).toThrow();
  });
  ...
  it('rejects analysisType > 64 chars', () => {
    expect(() => analyzeBodySchema.parse({ content: 'x', analysisType: 'y'.repeat(65) })).toThrow();
  });
});

describe('recommendBodySchema', () => {
  it('accepts a string entityId', () => {
    expect(recommendBodySchema.parse({ entityType: 'vessel', entityId: 'abc' })).toBeTruthy();
  });
  it('accepts a number entityId', () => {
    expect(recommendBodySchema.parse({ entityType: 'vessel', entityId: 12 })).toBeTruthy();
  });
...
describe('traceListQuerySchema', () => {
  ...
  it('validates riskLevel enum', () => {
    expect(() => traceListQuerySchema.parse({ riskLevel: 'extreme' })).toThrow();
  });
  it('coerces orgId', () => {
...
describe('traceFeedbackBodySchema', () => {
  ...
  it('rejects unknown sentiment', () => {
    expect(() => traceFeedbackBodySchema.parse({ sentiment: 'neutral' })).toThrow();
  });
...
describe('reviewQueueListQuerySchema', () => {
  ...
  it('rejects unknown status', () => {
    expect(() => reviewQueueListQuerySchema.parse({ status: 'started' })).toThrow();
  });
  it('rejects unknown verdict', () => {
    expect(() => reviewQueueListQuerySchema.parse({ verdict: 'maybe' })).toThrow();
  });
});
...
describe('traceCapturBodySchema', () => {
  ...
  it('rejects negative tokens', () => {
    expect(() => traceCapturBodySchema.parse({ ...valid, promptTokens: -1 })).toThrow();
  });
  it('rejects non-integer tokens', () => {
    expect(() => traceCapturBodySchema.parse({ ...valid, completionTokens: 1.5 })).toThrow();
  });
  it('rejects negative latencyMs', () => {
    expect(() => traceCapturBodySchema.parse({ ...valid, latencyMs: -1 })).toThrow();
  });
  it('rejects confidence outside [0,1]', () => {
    expect(() => traceCapturBodySchema.parse({ ...valid, confidence: 1.1 })).toThrow();
    expect(() => traceCapturBodySchema.parse({ ...valid, confidence: -0.1 })).toThrow();
  });
  it('rejects negative cost', () => {
    expect(() => traceCapturBodySchema.parse({ ...valid, costEstimateUsd: -0.5 })).toThrow();
  });
});
