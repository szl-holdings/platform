...
describe("stripeWebhookBodySchema", () => {
  ...
  it("rejects when data.object is missing", () => {
    expect(() =>
      stripeWebhookBodySchema.parse({ ...valid, data: {} }),
    ).toThrow();
  });
  it("rejects when created is not a number", () => {
    expect(() =>
      stripeWebhookBodySchema.parse({ ...valid, created: "today" }),
    ).toThrow();
  });
...
describe("alloyEmailIngestBodySchema", () => {
  ...
  it("rejects an invalid from address", () => {
    expect(() =>
      alloyEmailIngestBodySchema.parse({ from: "no-at", to: "c@d.com" }),
    ).toThrow();
  });
...
describe("slackInteractionBodySchema", () => {
  it("accepts a minimal type-only payload", () => {
    expect(
      slackInteractionBodySchema.parse({ type: "url_verification" }),
    ).toBeTruthy();
  });
...
describe("docusignWebhookBodySchema", () => {
  it("accepts a minimal event", () => {
    expect(
      docusignWebhookBodySchema.parse({ event: "envelope-completed" }),
    ).toBeTruthy();
  });
  ...
  it("rejects non-integer retryCount", () => {
    expect(() =>
      docusignWebhookBodySchema.parse({ event: "x", retryCount: 1.5 }),
    ).toThrow();
  });
});
...
describe("genericWebhookBodySchema", () => {
  it("accepts an arbitrary record", () => {
    expect(
      genericWebhookBodySchema.parse({ a: 1, b: "two", c: { d: true } }),
    ).toBeTruthy();
  });
