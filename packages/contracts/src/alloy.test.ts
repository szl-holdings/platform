...
describe('createWorkflowBodySchema', () => {
  ...
  it('rejects empty title', () => {
    expect(() => createWorkflowBodySchema.parse({ title: '', type: 'x' })).toThrow();
  });

  it('rejects title > 256 chars', () => {
    expect(() => createWorkflowBodySchema.parse({ title: 'x'.repeat(257), type: 'x' })).toThrow();
  });

  it('rejects empty type', () => {
    expect(() => createWorkflowBodySchema.parse({ title: 'x', type: '' })).toThrow();
  });
  ...
  it('rejects assignedTo <= 0', () => {
    expect(() => createWorkflowBodySchema.parse({ ...base, assignedTo: 0 })).toThrow();
  });
});
...
describe('approveWorkflowBodySchema', () => {
  ...
  it('rejects comment > 1024 chars', () => {
    expect(() => approveWorkflowBodySchema.parse({ comment: 'x'.repeat(1025) })).toThrow();
  });
});
...
describe('rejectWorkflowBodySchema', () => {
  ...
  it('rejects reason > 1024 chars', () => {
    expect(() => rejectWorkflowBodySchema.parse({ reason: 'x'.repeat(1025) })).toThrow();
  });
});
