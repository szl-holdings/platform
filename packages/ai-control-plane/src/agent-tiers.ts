...
export const AGENT_TIER_DEFINITIONS: Record<AgentTierName, AgentTierDefinition> = {
  assistant: {
    ...
    label: 'Assistant',
    description:
      'Read-only conversational AI — answers questions, summarizes data, generates drafts. No write access, no action execution. Requires human approval for any output used in external communications.',
  ...
  analyst: {
    ...
    label: 'Analyst',
    description:
      'Research and analysis tier — reads all domain data, generates structured insights, proposes actions for human review. Can write analysis artifacts but cannot execute actions directly.',
  ...
  operator: {
    ...
    label: 'Operator',
    description:
      'Semi-autonomous operations tier — can execute pre-approved action types, trigger workflows, and write state. All high-risk actions require human approval. Full audit trail mandatory.',
  ...
  autonomous: {
    ...
    label: 'Autonomous',
    description:
      'Fully autonomous execution tier — can plan and execute multi-step workflows, write state, trigger external systems, and delegate to sub-agents. Requires pre-authorization and continuous audit. Reserved for trusted automation pipelines.',
    maxCostPerRequestUsd: 10.0,
