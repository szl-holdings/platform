...
export interface FallbackDecision {
  ...
  reason?: string;
  rule?: FallbackRule;
  fallbackEndpoint?: Omit<
    ModelEndpoint,
    'tags' | 'evalScore' | 'costPerInputToken' | 'costPerOutputToken' | 'maxTokens'
  >;
}
...
const DEFAULT_FALLBACK_RULES: FallbackRule[] = [
  ...
  {
    id: 'cloud-to-substrate',
    triggerCondition: 'circuit_open',
    toProvider: 'substrate',
    toModel: 'llama-3.3-70b-instruct',
    toBaseUrl: 'http://localhost:8070/v1',
    priority: 20,
    enabled: true,
  },
  {
  ...
  {
    id: 'local-to-substrate',
    triggerCondition: 'circuit_open',
    fromProvider: 'local',
    toProvider: 'substrate',
    toModel: 'qwen3-next-80b',
    toBaseUrl: 'http://localhost:8070/v1',
    priority: 25,
    enabled: true,
  },
  {
    id: 'budget-exceeded-to-substrate',
    triggerCondition: 'budget_exceeded',
    toProvider: 'substrate',
    toModel: 'llama-3.1-8b-instruct',
    toBaseUrl: 'http://localhost:8070/v1',
    priority: 6,
    enabled: true,
  },
...
class FallbackEngine {
  ...
  evaluate(ctx: FallbackContext): FallbackDecision {
    if (
      ...
      if (rule) {
        logger.warn(
          { usedUsd: ctx.budgetUsedUsd, limitUsd: ctx.budgetLimitUsd },
          'Budget exceeded — applying fallback',
        );
        return {
          shouldFallback: true,
    ...
    if (
      ctx.evalScore !== undefined &&
      ctx.evalThreshold !== undefined &&
      ctx.evalScore < ctx.evalThreshold
    ) {
      const rule = this.findRule('eval_below_threshold', ctx);
      if (rule) {
        logger.warn(
          { evalScore: ctx.evalScore, threshold: ctx.evalThreshold },
          'Eval below threshold — applying fallback',
        );
        return {
          shouldFallback: true,
    ...
    if (ctx.failedProvider) {
      ...
      if (rule) {
        logger.info(
          { failed: ctx.failedProvider, fallback: rule.toProvider },
          'Circuit open — applying fallback',
        );
        return {
          shouldFallback: true,
          reason: 'circuit_open',
  ...
  private findRule(
    condition: FallbackRule['triggerCondition'],
    ctx: FallbackContext,
