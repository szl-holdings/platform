...
import { AGENT_TIER_DEFINITIONS, isToolAllowedForTier, isRouteClassAllowedForTier, requiresApproval } from "./agent-tiers.js";
...
const BUILT_IN_RULES: PolicyRule[] = [
  ...
  },
  {
    ...
    condition: (req) => req.tier === "assistant" && req.tool !== undefined && ["execute_workflow", "trigger_alert", "update_record"].includes(req.tool),
    violation: {
...
function resolveApprovalLevel(req: PolicyRequest, violations: PolicyViolation[]): PolicyDecision["approvalLevel"] {
  ...
  evaluate(req: PolicyRequest): PolicyDecision {
    ...
    for (const rule of this.rules) {
      ...
      if (rule.condition(req)) {
        violations.push({ code: rule.violation.code, message: rule.violation.message, severity: rule.violation.severity });
    ...
    if (!allowed) {
      logger.warn({ tier: req.tier, tool: req.tool, blockers: blockers.map(b => b.code) }, "Policy blocked request");
    } else if (needsApproval) {
      logger.info({ tier: req.tier, approvalLevel: decision.approvalLevel }, "Policy: approval required");
    }
