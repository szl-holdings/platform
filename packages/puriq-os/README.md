...
PURIQ agentic loop OS: Yuyay-13 gate, Λ aggregator, Hukulla tripwires, Khipu emit, replay-hash.
...
## Honesty note
Λ is Conjecture 1 (NOT a theorem). Replay hash is deterministic SHA-256 over the ordered loop trace.

## Install (editable, from monorepo root)
```bash
pip install -e packages/puriq-os
```
