...
import { AEF_DOMAIN_PROFILE_DOMAINS, DomainProfileSchema, PrivacyLevelSchema } from './schema.js';
...
describe('profile schema validation', () => {
  it('every profile parses against its zod schema without errors', () => {
    for (const profile of ALL_DOMAIN_PROFILES) {
      ...
      expect(
        result.success,
        `Profile ${profile.profileId} failed schema parse: ${JSON.stringify(errorIssues)}`,
      ).toBe(true);
    }
  });
  ...
  it('every profile has at least one exactMatchBoostTerm', () => {
    for (const profile of ALL_DOMAIN_PROFILES) {
      expect(
        profile.exactMatchBoostTerms.length,
        `${profile.profileId} has no boost terms`,
      ).toBeGreaterThan(0);
    }
  });
...
describe('privacy and retention policy bounds', () => {
  ...
  it('aegis profile is set to restricted or higher privacy level', () => {
    const highPrivacy: Array<typeof aegisSecurityIncident.privacyLevel> = [
      'restricted',
      'privileged',
    ];
  ...
  it('all profiles have encryptAtRest and encryptInTransit enabled', () => {
    for (const profile of ALL_DOMAIN_PROFILES) {
      expect(
        profile.retentionRules.encryptAtRest,
        `${profile.profileId} missing encryptAtRest`,
      ).toBe(true);
      expect(
        profile.retentionRules.encryptInTransit,
        `${profile.profileId} missing encryptInTransit`,
      ).toBe(true);
    }
  });
  ...
  it('no profile allows cross-region replication', () => {
    for (const profile of ALL_DOMAIN_PROFILES) {
      expect(
        profile.retentionRules.allowCrossRegionReplication,
        `${profile.profileId} allows cross-region replication`,
      ).toBe(false);
    }
  });
  ...
  it('privileged profiles require deletion', () => {
    ...
    for (const profile of privileged) {
      expect(
        profile.retentionRules.deletionRequired,
        `${profile.profileId} should require deletion`,
      ).toBe(true);
    }
  });
...
describe('ProfileRegistry (legacy API — stageForTenants / activate / rollback)', () => {
  const makeProfile = (
    profileId: string,
    version: string,
    domain: 'compliance' | 'legal' | 'maritime' | 'advisory',
  ) => ({
    profileId,
    version,
    domain,
  ...
  it('activates a specific version', () => {
    ...
    registry.register(makeProfile('test-profile', '2.0.0', 'compliance'), {
  ...
  it('rolls back to previous version', () => {
    ...
    registry.register(makeProfile('rollback-profile', '2.0.0', 'maritime'), {
      allowOverwrite: false,
    });
