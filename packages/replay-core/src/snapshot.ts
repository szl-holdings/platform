...
export function createSnapshot(data: Omit<ReplaySnapshot, "createdAt" | "sanitized">): ReplaySnapshot {
...
export function listSnapshots(filter?: { domain?: string; snapshotType?: string; tags?: string[] }): ReplaySnapshot[] {
...
export function redactPII(snapshot: ReplaySnapshot, sensitiveKeys: string[] = ["email", "phone", "ssn", "name", "address", "ip"]): ReplaySnapshot {
  ...
  return {
    ...snapshot,
    historicalContext: deepRedact(snapshot.historicalContext, sensitiveKeys) as Record<string, unknown>,
    agentInputs: snapshot.agentInputs.map(i => deepRedact(i, sensitiveKeys) as Record<string, unknown>),
    groundTruth: snapshot.groundTruth ? deepRedact(snapshot.groundTruth, sensitiveKeys) as Record<string, unknown> : undefined,
    sanitized: true,
  };
}
