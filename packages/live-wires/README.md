...
Live-Wires: real-time 3D wires transport (SSE fan-out) across the flagship constellation.
...
## Honesty note
Visualization transport only; positions/labels are honest, no synthetic claims.

## Install (editable, from monorepo root)
```bash
pip install -e packages/live-wires
```
