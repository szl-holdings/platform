...
describe("Rollback and recovery", () => {
  it("run fails when a step returns success:false and captures error", async () => {
    ...
    const failStep: WorkflowStep = {
      ...
      name: "Will Fail",
      async execute(_ctx: StepContext): Promise<{ stepId: string; success: boolean; error: string; latencyMs: number }> {
        return { stepId: "rollback-test", success: false, error: "Simulated failure for rollback", latencyMs: 0 };
      },
    };
    const config = makeConfig();
    manager.createRun(config);
  ...
  it("steps after a failure are not executed", async () => {
    ...
    const neverStep: WorkflowStep = {
      id: "step-never",
      name: "Never Reached",
      async execute(_ctx: StepContext): Promise<{ stepId: string; success: boolean; latencyMs: number }> {
  ...
  it("thrown error in step handler causes failed state with checkpoint saved before failure", async () => {
    ...
    const goodStep: WorkflowStep = {
      id: "step-good",
      name: "Good Step",
      async execute(_ctx: StepContext): Promise<{ stepId: string; success: boolean; output: string; latencyMs: number }> {
