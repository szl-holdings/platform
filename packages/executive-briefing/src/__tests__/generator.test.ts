...
import type { WorldModelEntity, MemoryEntry, RecentReflection, Citation } from "../index.js";
...
describe("gateBrief", () => {
  it("passes a well-formed brief with evidence", () => {
    const ctx = buildBriefContext("vessels", mockEntities, mockMemories, mockReflections);
    const citations: Citation[] = buildCitations(ctx).map((c) => ({ ...c, verified: c.verified ?? false }));
    const provenance = extractEntityProvenance(mockEntities);
