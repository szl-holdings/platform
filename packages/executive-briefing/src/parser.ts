...
import { StructuredExecutiveBriefSchema, type StructuredExecutiveBrief, type Citation } from "./types.js";
...
export interface ParseResult {
  success: true;
  brief: Omit<StructuredExecutiveBrief, "id" | "generatedAt" | "verifierStatus" | "verifierFeedback" | "sourceTraceIds" | "briefingId">;
}
...
export function parseBriefResponse(
  ...
  if (jsonStart === -1 || jsonEnd === -1) {
    return { success: false, error: "No JSON object found in AI response", raw: aiContent.slice(0, 500) };
  }
  ...
  try {
    ...
    return { success: false, error: `JSON parse error: ${e instanceof Error ? e.message : String(e)}`, raw: raw.slice(0, 500) };
  }

  const headline = typeof parsed.headline === "string" ? parsed.headline.slice(0, 180) : `${domain} executive brief`;
  const situation = typeof parsed.situation === "string" ? parsed.situation : "Situation assessment pending.";
  const autonomyTier = isValidAutonomyTier(parsed.autonomyTier) ? parsed.autonomyTier : "human-in-the-loop";
  ...
  const result: Omit<StructuredExecutiveBrief, "id" | "generatedAt" | "verifierStatus" | "verifierFeedback" | "sourceTraceIds" | "briefingId"> = {
    domain,
    headline,
    situation,
    whatWeBelieve,
  ...
  if (!validated.success) {
    return { success: false, error: `Schema validation failed: ${validated.error.message}`, raw: raw.slice(0, 300) };
  }

  return { success: true, brief: validated.data };
}

function isValidAutonomyTier(v: unknown): v is "human-approval-mandatory" | "human-in-the-loop" | "supervised-autonomy" | "full-autonomy" {
  return typeof v === "string" && ["human-approval-mandatory", "human-in-the-loop", "supervised-autonomy", "full-autonomy"].includes(v);
}
...
function sanitizeBelief(b: unknown, idx: number) {
  ...
  return {
    ...
    citationIds: Array.isArray(obj["citationIds"]) ? (obj["citationIds"] as string[]).filter((s) => typeof s === "string") : [],
    supported: obj["supported"] !== false,
    caveats: Array.isArray(obj["caveats"]) ? (obj["caveats"] as string[]).filter((s) => typeof s === "string") : [],
  };
}
...
function sanitizeRecommendation(r: unknown, idx: number) {
  ...
  return {
    ...
    autonomyTier: isValidAutonomyTier(obj["autonomyTier"]) ? obj["autonomyTier"] : "human-in-the-loop",
    citationIds: Array.isArray(obj["citationIds"]) ? (obj["citationIds"] as string[]).filter((s) => typeof s === "string") : [],
  };
}
...
function sanitizeSection(s: unknown, idx: number, defaultDomain: string) {
  ...
  return {
    ...
    beliefs,
    gaps: Array.isArray(obj["gaps"]) ? (obj["gaps"] as string[]).filter((g) => typeof g === "string") : [],
    confidence: typeof obj["confidence"] === "number" ? Math.max(0, Math.min(1, obj["confidence"])) : 0.6,
