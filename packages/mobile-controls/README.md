...
Mobile-Controls: shared iOS/touch retrofit helpers (viewport-fit, touch targets, viz patches).
...
## Honesty note
Front-end touch ergonomics only; no behavioral claims.

## Install (editable, from monorepo root)
```bash
pip install -e packages/mobile-controls
```
