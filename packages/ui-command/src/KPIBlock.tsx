  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        {metric.trend === "up" && <TrendingUp style={{ width: 12, height: 12, color: "#22c55e" }} />}
        {metric.trend === "down" && <TrendingDown style={{ width: 12, height: 12, color: "#ef4444" }} />}
        {metric.trend === "flat" && <Minus style={{ width: 12, height: 12, color: "rgba(255,255,255,0.3)" }} />}
      </div>

      <div style={{ display: "flex", alignItems: "baseline", gap: "0.375rem" }}>
        <span style={{ fontSize: numSize, fontWeight: 800, color: sColor, letterSpacing: "-0.03em", lineHeight: 1 }}>
          {metric.value}
        </span>
        {metric.unit && (
          <span style={{ fontSize: "13px", fontWeight: 500, color: "rgba(255,255,255,0.4)" }}>{metric.unit}</span>
        )}
      </div>
      ...
      {metric.delta && (
        <span style={{
          fontSize: "11px",
          fontWeight: 600,
          color: metric.trend === "up" ? "#22c55e" : metric.trend === "down" ? "#ef4444" : "rgba(255,255,255,0.4)",
        }}>
          {metric.delta}
        </span>
      )}
      ...
      {metric.target !== undefined && (
        <div style={{ fontSize: "10px", color: "rgba(255,255,255,0.3)" }}>
          Target: <span style={{ color: "rgba(255,255,255,0.55)" }}>{metric.target}{metric.unit ?? ""}</span>
        </div>
      )}
      ...
      {showCause && metric.causalExplanation && (
        <div style={{
          ...
          <Info style={{ width: 11, height: 11, color: "rgba(255,255,255,0.25)", flexShrink: 0, marginTop: 1 }} />
