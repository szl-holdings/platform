      ...
      <div style={{ position: 'relative', paddingLeft: '1.5rem' }}>
        ...
        {visible.map((event, i) => {
          ...
          return (
            <div
              key={event.id}
              style={{ position: 'relative', marginBottom: i < visible.length - 1 ? '1rem' : 0 }}
            >
              <div
                style={{
                  position: 'absolute',
              ...
              <div style={{ cursor: 'pointer' }} onClick={() => toggle(event.id)}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <span
                    style={{
                      ...
                      padding: '1px 6px',
                      borderRadius: '3px',
                      background: `${event.domainColor}20`,
                ...
                <div style={{ marginTop: '0.25rem' }}>
                  <span
                    style={{ fontSize: '13px', fontWeight: 600, color: 'rgba(255,255,255,0.85)' }}
                  >
                    {event.title}
                  </span>
                  {event.delta && (
                    <span
                      style={{
                        ...
                        fontSize: '11px',
                        fontWeight: 600,
                        color: event.delta.startsWith('+') ? '#ef4444' : '#22c55e',
                      }}
                    >
                      {event.delta}
                    </span>
                ...
                >
                  {event.description && (
                    <p
                      style={{
                        ...
                        lineHeight: 1.6,
                        marginBottom:
                          event.causeOf?.length || event.causedBy?.length ? '0.5rem' : 0,
                      }}
                    >
                      {event.description}
                    </p>
                  ...
                  {event.owner && (
                    <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)' }}>
                      Owner: <span style={{ color: 'rgba(255,255,255,0.5)' }}>{event.owner}</span>
                    </p>
                  )}
                  {showCausalLinks && event.causedBy && event.causedBy.length > 0 && (
                    ...
                    >
                      <ArrowRight
                        style={{ width: 9, height: 9, display: 'inline', marginRight: '4px' }}
                      />
