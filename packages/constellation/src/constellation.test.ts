...
describe("Query helpers", () => {
  ...
  beforeEach(() => {
    ...
    store.upsertEdge(ConstellationEdgeSchema.parse({
      id: "e-ab", type: "relates-to", fromNodeId: "a", toNodeId: "b",
      provenance: { source: "test", ingestedAt: now }, createdAt: now, updatedAt: now,
    }));
    store.upsertEdge(ConstellationEdgeSchema.parse({
      id: "e-bc", type: "relates-to", fromNodeId: "b", toNodeId: "c",
      provenance: { source: "test", ingestedAt: now }, createdAt: now, updatedAt: now,
    }));
  });
