...
describe("Schema — extended node types", () => {
  it("accepts all required spec node types", async () => {
    ...
    const requiredTypes = [
      "organization", "user", "vendor", "account", "property", "parcel",
      "lender", "matter", "filing", "clause", "obligation", "vessel",
      "voyage", "port", "counterparty", "sanctions_entity", "cyber_asset",
      "identity", "control", "incident", "workflow", "approval", "agent",
      "prompt", "model", "tool", "trace", "memory", "recommendation",
...
describe("InMemoryGraphStore — alias index", () => {
  ...
  it("removes stale alias key when aliases change on update", () => {
    ...
    const updated = { ...node, aliases: [{ aliasType: "imo", aliasValue: "NEW_IMO", isPrimary: true }] };
...
describe("Entity Resolution", () => {
  ...
  it("mergeNodes rewires source edges to target, no dangling edges", () => {
    ...
    store.upsertEdge(makeEdge({ id: "src-edge", fromNodeId: "vessel-2-id", toNodeId: "third-party" }));
...
describe("Contradiction Detection", () => {
  ...
  it("detects supersession contradiction", () => {
    ...
    store.upsertEdge(makeEdge({ id: "sup", fromNodeId: "new", toNodeId: "old", type: "supersedes" }));
...
describe("Evidence Ranking", () => {
  ...
  it("prefers API source over manual source", () => {
    const now = new Date().toISOString();
    const items = [
      { id: "api", edgeId: "x", evidenceType: "doc", confidence: 0.7, recordedAt: now, sourceType: "api" },
      { id: "manual", edgeId: "x", evidenceType: "doc", confidence: 0.7, recordedAt: now, sourceType: "manual" },
    ];
    const ranked = rankEvidence(items);
...
describe("Cross-Domain Queries", () => {
  ...
  beforeEach(() => {
    ...
    store.upsertNode(makeNode({ id: "l1", domain: "prism", type: "matter", label: "Legal Matter" }));
    store.upsertNode(makeNode({ id: "a1", domain: "aegis", type: "incident", label: "Cyber Incident" }));
