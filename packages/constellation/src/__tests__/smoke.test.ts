...
describe('12-Step Foundation Smoke Test', () => {
  ...
  const deploymentHistory: Array<{
    version: string;
    status: 'active' | 'rolled-back' | 'inactive';
  }> = [];
  ...
  it('Step 5: grades agent output using evals-core', async () => {
    ...
    const suite: EvalSuiteDef = {
      suiteId: 'smoke-test-suite',
      name: 'Smoke Test Suite',
      domain: 'terra',
      cases,
    };
    const report = await runEvalSuite(suite, async (_input, caseId) => {
  ...
  it('Step 7: policy engine blocks/escalates high-value action requiring approval', () => {
    ...
    auditLog.push({
      ...
      ts: NOW,
      payload: { approvalId: 'approval-001', result },
    });
  ...
  it('Step 10: replays an agent run against a historical snapshot', async () => {
    const snapshot = createSnapshot({
      ...
      agentInputs: [{ entityId: 'terra-1', signal: 'title_dispute', occupancyDecline: -0.23 }],
      groundTruth: { recommendation: 'escalate', confidence: 0.91 },
      historicalContext: {
    ...
    const result = await replaySnapshot(snapshot, async (_input, _ctx, snapshotId) => ({
      snapshotId,
