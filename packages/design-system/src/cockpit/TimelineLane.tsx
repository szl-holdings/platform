  ...
  return (
    <div
      style={
        maxVisible
          ? {
              maxHeight: `${maxVisible * 72 + 24}px`,
              borderColor: v.borderDefault,
              backgroundColor: v.bgSurface,
            }
          : { borderColor: v.borderDefault, backgroundColor: v.bgSurface }
      }
      className={cn('rounded-lg border px-4 py-3', maxVisible && 'overflow-y-auto', className)}
    >
            ...
            >
              ...
              <div className="min-w-0 flex-1 pt-0">
                <div className="flex items-baseline gap-2 flex-wrap">
                  <span style={{ color: v.textPrimary }} className="text-xs font-medium">
                    {event.label}
                  </span>
                  {event.actor && (
                ...
                {event.description && (
                  <p style={{ color: v.textSecondary }} className="mt-0.5 text-[11px]">
                    {event.description}
                  </p>
                )}
                  ...
                  <dl className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5">
                    {Object.entries(event.meta).map(([k, val]) => (
                      <div key={k} className="flex gap-1">
                        <dt style={{ color: v.textMuted }} className="text-[10px]">
                          {k}:
                        </dt>
                        <dd style={{ color: labelColor }} className="text-[10px] font-medium">
                          {val}
                        </dd>
                      </div>
                    ))}
