...
export class SignalPipeline {
  ...
  async process(input: SignalInput): Promise<PipelineResult> {
    ...
    stages.push({ stage: "correlate", durationMs: Date.now() - t, notes: `${correlated.length} correlated` });

    t = Date.now();
    const scored = await this.stage_score(signal, correlated);
    signal = scored.signal;
    stages.push({ stage: "score", durationMs: Date.now() - t, notes: `score=${scored.score.toFixed(2)}` });
  ...
  private async stage_recommend(
    ...
    if (primaryEntity) {
      defaultEntityRegistry.linkRecommendation(primaryEntity.entityId, recommendation.recommendationId);
    }
  ...
  private async stage_telemetryWriteback(signal: Signal, recommendation: Recommendation | null): Promise<Signal> {
    const final: Signal = { ...signal, processedAt: new Date().toISOString(), stage: "telemetry-writeback" };
...
function buildRecommendationTitle(signal: Signal): string {
  const entityName = signal.entityRefs[0]?.displayName ?? signal.entityRefs[0]?.entityId ?? signal.domain;
...
function buildRecommendationSummary(signal: Signal, correlated: Signal[]): string {
  ...
  const extra = correlated.length > 0 ? ` Correlated with ${correlated.length} related signal(s).` : "";
