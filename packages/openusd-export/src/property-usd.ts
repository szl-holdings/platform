...
export interface PropertyUsdState {
  ...
  pricePerSqft?: number;
  simulationScenario?: "baseline" | "stress_test" | "vacancy_spike" | "cap_rate_compression" | "rate_shock";
...
export function exportPropertyTwin(state: PropertyUsdState): UsdExportResult {
  ...
  const locationPrim: UsdPrim = {
    ...
    attributes: state.coordinates
      ? [{ name: "xformOp:translate", type: "float3", value: [state.coordinates.lon, 0, state.coordinates.lat] }]
  ...
  const propertyPrim: UsdPrim = {
    ...
    attributes: [
      ...
      { name: "szl:dscr", type: "float", value: state.debtServiceCoverageRatio ?? 1.0, custom: true },
      ...
      { name: "szl:marketTrend", type: "token", value: state.marketTrend ?? "stable", custom: true },
      { name: "szl:tenantRisk", type: "float", value: state.tenantRiskScore ?? 0, custom: true },
      { name: "szl:buildingAreaSqM", type: "float", value: state.buildingAreaSqM ?? 0, custom: true },
      { name: "szl:yearBuilt", type: "int", value: state.yearBuilt ?? 0, custom: true },
      { name: "szl:scenario", type: "token", value: state.simulationScenario ?? "baseline", custom: true },
    ],
    children: [locationPrim],
  };

  if ((state.floodRiskScore ?? 0) > 70) warnings.push("High flood risk score — consider insurance and elevation data");
  if ((state.loanToValue ?? 0) > 0.8) warnings.push("High LTV — limited equity buffer in stress scenarios");
  if ((state.debtServiceCoverageRatio ?? 1.5) < 1.1) warnings.push("Low DSCR — close to coverage breach threshold");
...
export function exportPropertySimulation(params: PropertySimulationParams): UsdExportResult {
  ...
  switch (params.scenario) {
    case "stress_test":
      ...
      if (params.vacancyRateDelta !== undefined) base.occupancyRate = Math.max(0, (base.occupancyRate ?? 1) - params.vacancyRateDelta);
      ...
      break;
    case "vacancy_spike":
      base.occupancyRate = Math.max(0, (base.occupancyRate ?? 1) - (params.vacancyRateDelta ?? 0.2));
    ...
    case "cap_rate_compression":
      ...
      break;
    case "rate_shock":
      base.debtServiceCoverageRatio = Math.max(0.5, (base.debtServiceCoverageRatio ?? 1.3) - (params.interestRateDelta ?? 0) * 0.15);
