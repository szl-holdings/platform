...
export interface SecurityScenario {
  ...
  affectedSystems?: Array<{ id: string; name: string; criticality: "low" | "medium" | "high" | "critical" }>;
  phases?: Array<{
    id: string;
    name: string;
    description: string;
...
export function exportSecurityScenario(scenario: SecurityScenario): UsdExportResult {
  ...
  const scenarioPrim: UsdPrim = {
    ...
    attributes: [
      ...
      { name: "szl:postureScoreBefore", type: "float", value: scenario.postureScoreBefore ?? 0, custom: true },
      { name: "szl:postureScoreAfter", type: "float", value: scenario.postureScoreAfter ?? 0, custom: true },
      { name: "szl:mttdMinutes", type: "float", value: scenario.mttdEstimateMinutes ?? 0, custom: true },
      { name: "szl:mttrMinutes", type: "float", value: scenario.mttrEstimateMinutes ?? 0, custom: true },
      { name: "szl:blastRadiusPct", type: "float", value: scenario.blastRadiusPct ?? 0, custom: true },
      { name: "szl:classification", type: "token", value: scenario.classificationLevel ?? "unclassified", custom: true },
    ],
    children: [],
  };
  ...
  scenario.affectedSystems?.forEach((sys, i) => {
    ...
    if (sys.criticality === "critical") warnings.push(`Critical system '${sys.name}' in blast radius`);
  });
  ...
  scenario.phases?.forEach((phase, i) => {
    scenarioPrim.children!.push({
      ...
      attributes: [
        ...
        { name: "szl:detectionProbability", type: "float", value: phase.detectionProbability ?? 0.5, custom: true },
