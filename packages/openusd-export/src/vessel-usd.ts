...
function buildVesselPrim(
  state: VesselUsdState,
  allWaypoints: Array<{ lat: number; lon: number; name?: string }>,
): UsdPrim {
    ...
    attributes: [
      {
        ...
        type: 'float3',
        value: [state.position.lon, 0, state.position.lat],
      },
  ...
  const vesselPrim: UsdPrim = {
    ...
    attributes: [
      ...
      {
        name: 'szl:fuelLevelPct',
        type: 'float',
        value: state.fuelLevelPercent ?? 100,
        custom: true,
      },
      ...
      { name: 'szl:cargoStatus', type: 'string', value: state.cargoStatus ?? '', custom: true },
      {
        name: 'szl:scenario',
        type: 'token',
        value: state.simulationScenario ?? 'live_state',
        custom: true,
      },
    ],
    children: [
...
export function exportVesselTwin(state: VesselUsdState): UsdExportResult {
  ...
  if (state.routeRiskLevel === 'critical' || state.routeRiskLevel === 'high') {
    warnings.push(
      `High route risk level (${state.routeRiskLevel}) — simulation scenarios may show deviation`,
    );
  }
...
export function exportRouteSimulation(params: RouteSimulationParams): UsdExportResult {
  ...
  const allWaypoints: Array<{ lat: number; lon: number; name?: string }> = [
    ...(params.originPort
      ? [{ lat: params.originPort.lat, lon: params.originPort.lon, name: params.originPort.name }]
      : []),
    ...(params.waypoints ?? vessel.routeWaypoints ?? []),
    ...(params.destinationPort
      ? [
          {
            lat: params.destinationPort.lat,
            lon: params.destinationPort.lon,
            name: params.destinationPort.name,
          },
        ]
      : []),
  ];
  ...
  const vesselPrim = buildVesselPrim({ ...vessel, simulationScenario: scenario }, allWaypoints);
  ...
  const routePrim: UsdPrim = {
    ...
    attributes: [
      ...
      {
        name: 'szl:simulatedDurationHours',
        type: 'float',
        value: params.simulatedDurationHours ?? 0,
        custom: true,
      },
      {
        name: 'szl:windSpeedKnots',
        type: 'float',
        value: params.weatherConditions?.windSpeedKnots ?? 0,
        custom: true,
      },
      {
        name: 'szl:waveHeightM',
        type: 'float',
        value: params.weatherConditions?.waveHeightM ?? 0,
        custom: true,
      },
      {
        name: 'szl:currentKnots',
        type: 'float',
        value: params.weatherConditions?.currentKnots ?? 0,
        custom: true,
      },
      ...
      ...(params.destinationPort
        ? [
            {
              name: 'szl:destinationPort',
              type: 'string' as const,
              value: params.destinationPort.name,
              custom: true,
            },
          ]
        : []),
  ...
  if (scenario === 'storm_diversion')
    warnings.push('Storm diversion scenario: alternate waypoints may be automatically inserted');
  if (scenario === 'chokepoint_delay')
    warnings.push('Chokepoint delay scenario: ETA extended by simulated queue time');
