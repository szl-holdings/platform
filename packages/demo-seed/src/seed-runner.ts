...
export async function clearDemoData(): Promise<void> {
  ...
  if (demoWorkflowIds.length > 0) {
    ...
    await db.delete(alloyWorkflowRuns).where(inArray(alloyWorkflowRuns.workflowId, demoWorkflowIds));
  }
...
async function upsertOwner(externalId: string, name: string, type: "user" | "team" | "system" | "external", domain: string, email?: string) {
  const [row] = await db.insert(alloyOwners).values({ externalId, name, type, domain, email, metadata: { demo: true } })
    .onConflictDoNothing({ target: alloyOwners.externalId })
    .returning();
  if (row) return row;
  const [existing] = await db.select().from(alloyOwners).where(sql`external_id = ${externalId}`).limit(1);
  return existing!;
}

async function upsertSignal(values: typeof alloySignals.$inferInsert) {
  const [row] = await db.insert(alloySignals).values(values)
    .onConflictDoNothing({ target: alloySignals.externalId })
    .returning();
  if (row) return row;
  const [existing] = await db.select().from(alloySignals).where(sql`external_id = ${values.externalId}`).limit(1);
  return existing!;
}
...
async function seedBusinessNarrative() {
  ...
  const owner = await upsertOwner("demo-owner-lyte-cfo", "Marcus Holt (CFO Demo)", "user", "lyte", "m.holt@demo.szlholdings.com");
  ...
  const signal = await upsertSignal({
    ...
    summary: "Vantex Acquisition deal has not advanced in 47 days. Approval owner departed 2026-02-28 with no handoff. Q2 close probability: 31%.",
    category: "pipeline",
    severity: "high",
    score: 8.7,
    confidence: 0.91,
  ...
  const workflow = await (async () => {
    const [row] = await db.insert(alloyWorkflows).values({
      ...
      environment: "production",
      metadata: { demo: true, narrative: "business-revops", agent: "Beacon" },
      startedAt: hoursAgo(26),
      completedAt: hoursAgo(24),
    }).onConflictDoNothing({ target: alloyWorkflows.externalId }).returning();
    if (row) return row;
    const [existing] = await db.select().from(alloyWorkflows).where(sql`external_id = ${"demo-workflow-biz-001"}`).limit(1);
    return existing!;
  })();

  const [wfRun] = await db.insert(alloyWorkflowRuns).values({
  ...
  await db.insert(alloyActions).values([
    {
      ...
      description: "Deal ownership transferred via Alloy workflow — velocity monitoring reactivated",
      status: "completed",
      priority: "critical",
    ...
    },
    {
      ...
      description: "Beacon drafted buyer re-engagement email — pending Sophia's review before send",
      status: "completed",
      priority: "high",
      payload: { demo: true, recipient: "Buyer contact", draftedBy: "Beacon" },
      completedAt: hoursAgo(24),
      metadata: { demo: true, narrative: "business-revops" },
    },
    {
  ...
  await db.insert(alloyArtifacts).values({
    externalId: "demo-artifact-biz-001-summary",
    workflowId: workflow.id,
    signalId: signal.id,
    type: "summary",
    title: "Q2 Revenue Exposure Contained: $4.2M Deal Reactivated",
    content: "A 47-day pipeline stall was detected and resolved within one business day. Close probability restored from 31% to 74%. Total time from signal to resolution: 25 hours 36 minutes. Actions taken: 4. Approvals: 1. Audit record: complete.",
    domain: "lyte",
    metadata: { demo: true, narrative: "business-revops", closeProbabilityBefore: 0.31, closeProbabilityAfter: 0.74 },
  }).onConflictDoNothing({ target: alloyArtifacts.externalId });

  // Seed lyte-specific tables for Command Inbox visibility
  const [lyteSignal] = await db.insert(lyteSignalsTable).values({
  ...
  if (lyteSignal) {
    await db.insert(lyteCommandCardsTable).values({
      ...
      description: "Deal stalled 47 days. Approval owner departed without handoff. Revenue at risk: $4.2M. Beacon recommends emergency escalation to CFO with VP BD reassignment.",
      category: "risk",
      priority: "critical",
      status: "completed",
      assignee: "Marcus Holt",
      metadata: { demo: true, narrative: "business-revops", signalId: lyteSignal.id, confidence: 0.87 },
    }).onConflictDoNothing();
    ...
    await db.insert(lyteRecommendationsTable).values({
      ...
      description: "Escalate immediately. Reassign deal ownership to VP BD (Sarah Kim). CFO to sponsor buyer call. Deal has 78% historical close rate with this intervention pattern at this stage. Beacon confidence: 87%. Historical close rate with comparable intervention: 78%.",
...
async function seedSecurityNarrative() {
  ...
  const lyteWorkspaceId = await ensureDemoWorkspace();
  const cisoOwner = await upsertOwner("demo-owner-ciso", "Diana Reyes (CISO Demo)", "user", "aegis", "d.reyes@demo.szlholdings.com");
  const analystOwner = await upsertOwner("demo-owner-soc-analyst", "Priya Nair (SOC Analyst Demo)", "user", "aegis", "p.nair@demo.szlholdings.com");
  ...
  const signal = await upsertSignal({
    ...
    summary: "Auth Service Cluster under active credential stuffing attack. IP cluster matches CISA KEV-2026-0247 botnet. CVE-2025-31982 active (CVSS 9.1). MITRE T1110.004.",
  ...
  const workflow = await (async () => {
    ...
    if (row) return row;
    const [existing] = await db.select().from(alloyWorkflows).where(sql`external_id = ${"demo-workflow-sec-001"}`).limit(1);
    return existing!;
  })();
  ...
  await db.insert(alloyApprovals).values({
    ...
    runId: wfRun?.id,
    status: "approved",
    reason: "Execute containment immediately. Escalate patch to P0. Notify legal for regulatory reporting window.",
  ...
  await db.insert(alloyActions).values([
    {
      ...
      description: "All 14 identified botnet IPs blocked via Aegis Firewall Connector — confirmed at perimeter",
      status: "completed",
      priority: "critical",
    ...
    },
    {
      ...
      description: "Precautionary rotation of 3 privileged session tokens; affected users emailed",
      status: "completed",
      priority: "high",
  ...
  await db.insert(alloyArtifacts).values({
    ...
    content: "CISA KEV-2026-0247 botnet detected targeting Auth Service. CISO approved containment in 96 seconds. Analyst executed 4-step playbook. 0 accounts compromised. 0 data exfiltration detected. GDPR notification filed within 72-hour window. Evidence package ready for SOC 2 and ISO 27001 review.",
    domain: "aegis",
    metadata: { demo: true, narrative: "security-soc", minutesToContainment: 23, accountsCompromised: 0 },
  }).onConflictDoNothing({ target: alloyArtifacts.externalId });

  await db.insert(lyteIncidentsTable).values({
    workspaceId: lyteWorkspaceId,
    title: "Auth Service — Credential Stuffing Attack (Contained)",
    description: "2,400 failed auth attempts from 14 IPs matching CISA KEV botnet. MITRE T1110.004. CVE-2025-31982 (CVSS 9.1) active. Contained in 23 minutes. 0 accounts compromised.",
    severity: "critical",
    status: "resolved",
    assignee: "Priya Nair",
    impactArea: "Authentication / Identity",
    rootCause: "Distributed botnet credential stuffing exploiting CVE-2025-31982 in Auth Service v2.8.0",
    resolution: "IP block + rate limiting + session token rotation + patch scheduled. CISO-approved containment playbook executed.",
    metadata: { demo: true, narrative: "security-soc" },
...
async function seedMaritimeNarrative() {
  ...
  const fleetOwner = await upsertOwner("demo-owner-fleet-ops", "Captain James Wren (Demo)", "user", "vessels", "j.wren@demo.szlholdings.com");
  ...
  const signal = await upsertSignal({
    ...
    summary: "MV Soltana ceased AIS transmission for 134 minutes. Reappeared 34nm off declared route, within 18nm of Iranian waters. OFAC screening triggered. Revenue at risk: $840,000.",
    category: "compliance",
    severity: "critical",
    score: 9.1,
    confidence: 0.89,
  ...
  const workflow = await (async () => {
    const [row] = await db.insert(alloyWorkflows).values({
      ...
      environment: "production",
      metadata: { demo: true, narrative: "maritime", agent: "Helmsman", incidentRef: "INC-2026-0414-001" },
      startedAt: hoursAgo(8),
      completedAt: hoursAgo(3),
    }).onConflictDoNothing({ target: alloyWorkflows.externalId }).returning();
    if (row) return row;
    const [existing] = await db.select().from(alloyWorkflows).where(sql`external_id = ${"demo-workflow-mar-001"}`).limit(1);
    return existing!;
  })();
  ...
  await db.insert(alloyApprovals).values({
    ...
    runId: wfRun?.id,
    status: "approved",
    reason: "Agreed. Hold at anchorage. I am calling the P&I Club now. Notify the charterer — they need to know the ETA impact.",
  ...
  await db.insert(alloyActions).values([
    ...
    },
    {
      ...
      description: "Charterer (Meridian Energy Trading) informed of ETA revision pending OFAC clearance",
      status: "completed",
      priority: "medium",
  ...
  await db.insert(alloyArtifacts).values({
    ...
    content: "AIS dark event (134 min) detected near sanctions corridor. Fleet Ops Director approved anchorage hold in 5 minutes. OFAC clearance confirmed in 4.5 hours — no sanctions match. Voyage resumed. Demurrage $112,000 recoverable from charterer. Full audit trail preserved for flag state, port authority, and P&I review.",
    domain: "vessels",
...
async function seedLegalNarrative() {
  ...
  const legalOwner = await upsertOwner("demo-owner-attorney", "Sophia Marchetti (Demo)", "user", "prism-counsel", "s.marchetti@demo.szlholdings.com");
  ...
  const signal = await upsertSignal({
    ...
    summary: "Continental General Insurance has not acknowledged the Rivera demand within the 30-day window required under NY DFS Regulation 68 §216.6. Violation: 12 days. Demand readiness: 91%. Bad faith clock trigger available.",
    category: "compliance",
    severity: "high",
    score: 8.4,
    confidence: 0.97,
  ...
  const workflow = await (async () => {
    const [row] = await db.insert(alloyWorkflows).values({
      ...
      steps: [
        { step: 1, name: "Draft Reg 68 violation notice with insurer profile", status: "completed" },
        { step: 2, name: "Export demand packet to Word with citations", status: "completed" },
        { step: 3, name: "Transmit to Continental General (certified + email)", status: "completed" },
      ...
      environment: "production",
      metadata: { demo: true, narrative: "legal-compliance", ref: "RIV-2026-0130", agent: "PRISM Counsel AI" },
      startedAt: hoursAgo(72),
      completedAt: hoursAgo(70),
    }).onConflictDoNothing({ target: alloyWorkflows.externalId }).returning();
    if (row) return row;
    const [existing] = await db.select().from(alloyWorkflows).where(sql`external_id = ${"demo-workflow-leg-001"}`).limit(1);
    return existing!;
  })();
  ...
  await db.insert(alloyApprovals).values({
    ...
    runId: wfRun?.id,
    status: "approved",
    reason: "Agreed. Issue the notice today. CC our client. I want the bad faith letter to cite the prior violations — make sure the insurer profile is attached.",
  ...
  await db.insert(alloyActions).values([
    {
      ...
      description: "PRISM Counsel drafted violation notice citing 3 prior Continental General violations in this jurisdiction",
      status: "completed",
      priority: "high",
    ...
    },
    {
      ...
      description: "Demand letter exported with privilege controls, source citations, and metadata intact",
      status: "completed",
      priority: "high",
    ...
    },
    {
      ...
      description: "Client update sent in plain language explaining the violation notice and next steps",
      status: "completed",
      priority: "medium",
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  runner()
    .then(() => { console.log("[demo-seed] Done."); process.exit(0); })
