...
async function seedBusinessNarrative() {
  ...
  if (lyteSignal) {
    await db.insert(lyteCommandCardsTable).values({
      ...
      description: "Deal stalled 47 days. Approval owner departed without handoff. Revenue at risk: $4.2M. Lyte recommends emergency escalation to CFO with VP BD reassignment.",
      category: "risk",
      priority: "critical",
      status: "completed",
    ...
    await db.insert(lyteRecommendationsTable).values({
      ...
      description: "Escalate immediately. Reassign deal ownership to VP BD (Sarah Kim). CFO to sponsor buyer call. Deal has 78% historical close rate with this intervention pattern at this stage. Lyte confidence: 87%. Historical close rate with comparable intervention: 78%.",
