...
export const SZL_TREASURY_NARRATIVE = {
  ...
  entities: {
    org: { entityId: "org-szl-holdings", entityType: "organization" as const, displayName: "SZL Holdings", domain: "finance" as const },
    rcf: { entityId: "facility-rcf-001", entityType: "custom" as const, displayName: "Revolving Credit Facility #001", domain: "finance" as const },
    derivativeBook: { entityId: "book-commodity-derivatives", entityType: "custom" as const, displayName: "Commodity Derivative Book", domain: "finance" as const },
  },
  ...
  buildEvidenceItems(signals: Signal[]): EvidenceItem[] {
    return [
      createEvidenceItem({
        ...
        entityRefs: [this.entities.org],
        summary: "FX hedge ratio at 18.2% — 3.2 percentage points beyond 15% policy limit. $4.8M unhedged exposure.",
        confidence: 0.99,
        freshness: 0.9,
        weight: 0.35,
      ...
      createEvidenceItem({
        ...
        entityRefs: [this.entities.rcf],
        summary: "Liquidity covenant headroom at 2.5% — breaching at 1.2x ratio. $50M facility at risk.",
        confidence: 0.96,
        freshness: 0.92,
        weight: 0.4,
    ...
    return createRecommendation({
      ...
      actionPayload: {
        actions: [
          ...
          { action: "Draw $5M on RCF #001 to restore liquidity ratio to 1.28x", urgency: "today", approver: "cfo" },
          { action: "Unwind 40% of Brent Crude positions — crystallize $1.1M loss", urgency: "this-week", approver: "board" },
        ],
        aggregateExposureUsd: 14_200_000,
      ...
      confidence: 0.96,
      freshness: 0.92,
      projectedImpact: "Hedge rebalance and RCF draw restore liquidity ratio to 1.28x and prevent $14.2M covenant breach before board notification threshold.",
      projectedRisk: "Inaction triggers cross-default on $120M total debt and board notification requirement within 24 hours if covenant threshold is breached.",
