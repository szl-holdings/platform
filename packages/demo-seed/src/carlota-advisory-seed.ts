...
export const CARLOTA_ADVISORY_CLIENTS: ReadonlyArray<{ id: CarlotaClientId; name: string; industry: string }> = [
...
export interface RoiBenchmarks {
  avgRoi: number; avgPaybackMonths: number; avgRateRealisationPct: number;
  blendedMarginPct: number; clientRetentionPct: number; npsScore: number;
}
...
export const CARLOTA_RADAR_SIGNALS: Record<CarlotaClientId, RadarSignal[]> = {
  "luminary-brands": [
    { competitor: "Glossier", event: "DTC paid-acquisition costs up 23% across category — direct competitor pulled performance budget", impact: "high", direction: "opportunity", date: "Apr 16, 2026", detail: "Rising customer acquisition costs across the prestige DTC category create an opening for Luminary's owned-channel strategy. Glossier reportedly cut paid budget 30% in Q1." },
    { competitor: "Charlotte Tilbury", event: "Launched private label in two new EU markets — UK pricing held flat", impact: "medium", direction: "threat", date: "Apr 9, 2026", detail: "Charlotte Tilbury's EU expansion brings new shelf competition for the £40-£75 price tier where Luminary is repositioning. Watch UK Boots assortment changes." },
    { competitor: "Drunk Elephant", event: "Negative TikTok sentiment surge — formula reformulation backlash", impact: "medium", direction: "opportunity", date: "Apr 3, 2026", detail: "Sentiment dropped 18 points week-over-week. Opportunity for Luminary's editorial PR push to capture defectors searching for clean formulation alternatives." },
  ],
  "vertex-capital": [
    { competitor: "Bain & Company", event: "PE deal advisory fees down 12% — boutiques gaining mid-market share", impact: "high", direction: "opportunity", date: "Apr 12, 2026", detail: "Bain's mid-market PE advisory revenue declined for two consecutive quarters. Vertex's deal pipeline sits in the £50M–£200M range Bain is now de-prioritising." },
    { competitor: "EY-Parthenon", event: "Lost two senior MDs covering UK industrials — talent gap in core sector", impact: "high", direction: "opportunity", date: "Apr 6, 2026", detail: "EY-Parthenon's UK industrials practice lost both senior MDs within four weeks. Several portfolio targets in Vertex's pipeline previously bid by EY-P teams." },
    { competitor: "Lazard", event: "M&A volumes in UK lower-mid market down 18% YoY — pricing pressure on advisor fees", impact: "medium", direction: "threat", date: "Mar 28, 2026", detail: "Lazard's UK lower-mid market deal flow contraction is creating pricing pressure. Vertex should expect more aggressive fee proposals from competing sponsors." },
  ],
  "aurelius-pe": [
    { competitor: "Apollo Global", event: "Portfolio operating partner team expansion — UK ops focus", impact: "high", direction: "threat", date: "Apr 14, 2026", detail: "Apollo added 8 operating partners with UK industrials and consumer ops experience. Direct overlap with Aurelius portfolio uplift focus." },
    { competitor: "Permira", event: "Closed three portfolio-ops led value creation case studies — published thought leadership", impact: "medium", direction: "threat", date: "Apr 7, 2026", detail: "Permira's case studies establish them as the go-to PE firm for portfolio operations transformation, narrowing Aurelius' positioning advantage in UK PE LP conversations." },
    { competitor: "Triton Partners", event: "Lost two portfolio CEOs to competing GP buyouts in last 60 days", impact: "medium", direction: "opportunity", date: "Mar 30, 2026", detail: "Triton portfolio leadership turnover creates an opening for Aurelius to recruit experienced operators with mid-market portfolio uplift track records." },
  ],
  "oasis-wellness": [
    { competitor: "Holland & Barrett", event: "New private-label supplements line undercuts mid-tier wellness brands by ~30%", impact: "high", direction: "threat", date: "Apr 15, 2026", detail: "Holland & Barrett's expanded own-brand range targets the £18-£35 price tier where Oasis sits. Visible margin compression risk for Q3." },
    { competitor: "Heights", event: "Closed £15M Series B — accelerating UK retail rollout to Boots and Tesco", impact: "high", direction: "threat", date: "Apr 8, 2026", detail: "Heights' funding war chest enables aggressive shelf-space competition in UK pharmacy and grocery channels — Oasis must accelerate retail strategy." },
    { competitor: "AG1 (Athletic Greens)", event: "Subscription churn rising — sentiment shift toward simpler, single-ingredient products", impact: "medium", direction: "opportunity", date: "Apr 1, 2026", detail: "Consumer fatigue with all-in-one formulas creates opening for Oasis' targeted single-ingredient line. Earned media opportunity in wellness press." },
  ],
};

export const CARLOTA_COMPETITORS: Record<CarlotaClientId, { name: string; score: number; trend: "up" | "down" | "flat"; share: number }[]> = {
  "luminary-brands": [
...
export const CARLOTA_MARKET_TREND: Record<CarlotaClientId, { month: string; you: number; market: number }[]> = {
  "luminary-brands": [
...
export async function seedCarlotaAdvisoryData(): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    await tx.execute(sql`DELETE FROM carlota_client_market_trend WHERE client_external_id = ANY(${ids})`);
    await tx.execute(sql`DELETE FROM carlota_client_competitors WHERE client_external_id = ANY(${ids})`);
    await tx.execute(sql`DELETE FROM carlota_client_radar_signals WHERE client_external_id = ANY(${ids})`);
    await tx.execute(sql`DELETE FROM carlota_client_roi_trend WHERE client_external_id = ANY(${ids})`);
    await tx.execute(sql`DELETE FROM carlota_client_roi_benchmarks WHERE client_external_id = ANY(${ids})`);
    await tx.execute(sql`DELETE FROM carlota_client_margin_history WHERE client_external_id = ANY(${ids})`);
    ...
    for (let i = 0; i < CARLOTA_ADVISORY_CLIENTS.length; i++) {
      ...
      await tx.insert(carlotaAdvisoryClientsTable).values({
        externalId: c.id, name: c.name, industry: c.industry, sortOrder: i,
      });
    }

    for (const id of ids) {
      const rows = CARLOTA_MARGIN_HISTORY[id].map((r, i) => ({
        clientExternalId: id, month: r.month, margin: r.margin, sortOrder: i,
      }));
