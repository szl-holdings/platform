...
export async function seedConstellationDemo(): Promise<ConstellationSeedResult> {
  ...
  if (!terraNode) {
    const input: CreateCstNode = {
      ...
      description: "Mixed-use commercial tower in SF Financial District. Subject of active title dispute and occupancy revenue decline signal.",
  ...
  if (!vesselsNode) {
    const input: CreateCstNode = {
      ...
      description: "Bulk carrier vessel on charter supplying construction materials to Harbor View Tower renovation. Currently under sanctions screening review.",
  ...
  if (!prismNode) {
    const input: CreateCstNode = {
      ...
      description: "Active legal matter challenging chain of title for Harbor View Tower. Filed by Meridian Development LLC in SF Superior Court. PRISM tracking discovery deadlines and settlement pressure.",
  ...
  if (!aegisNode) {
    const input: CreateCstNode = {
      ...
      description: "Security breach: unauthorized personnel accessed server room on Floor 12 during the title dispute discovery period. Aegis flagged correlation with litigation timeline as a potential evidence-tampering risk.",
  ...
  if (!lyteNode) {
    const input: CreateCstNode = {
      ...
      description: "Lyte detected a 23% QoQ occupancy revenue decline at Harbor View Tower. Correlated with the ongoing title dispute uncertainty and the physical security incident. AI recommendation: engage distressed asset advisory.",
  ...
  async function ensureEdge(fromId: string, toId: string, relType: string, label: string, conf = 0.9) {
    try {
      ...
      console.log(`[constellation-seed] ✓ Edge: ${relType} (${fromId.slice(0, 8)}→${toId.slice(0, 8)})`);
    } catch (err) {
      console.warn(`[constellation-seed] ⚠ Edge ${relType} may already exist or failed:`, (err as Error).message);
    }
  }
