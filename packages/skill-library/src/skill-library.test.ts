...
describe("runSkill", () => {
  it("throws SkillNotFoundError for unknown skill", async () => {
    ...
    await expect(
      runSkill("does:not:exist", {}, { registry, runStore })
  ...
  it("throws SkillDisabledError for disabled skill", async () => {
    ...
    registry.registerSkill(sampleSkill({ id: "disabled", enabled: false }));
    await expect(
      runSkill("disabled", {}, { registry, runStore })
    ).rejects.toThrow(SkillDisabledError);
  });
...
describe("setBackend — write-through persistence", () => {
  it("forwards registerSkill to backend", async () => {
    ...
    registry.setBackend({
      persistSkill: async (s) => { persisted.push(s); },
  ...
  it("forwards updateSkill to backend", async () => {
    ...
    registry.setBackend({
      persistSkill: async () => {},
      persistSkillUpdate: async (skillId, patch) => { updates.push({ skillId, patch }); },
    });
    registry.registerSkill(sampleSkill());
  ...
  it("forwards saveRun to backend", async () => {
    ...
    store.setBackend({ persistRun: async (r) => { persisted.push(r); } });
    store.saveRun({
      runId: "r-backend",
      skillId: "s",
      skillName: "S",
