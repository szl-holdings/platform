...
import type { StageExecutorFn, AnyStage, StageExecutorContext } from "../types.js";
...
const syntheticExecutor: StageExecutorFn = async (stage: AnyStage, input: unknown, ctx: StageExecutorContext) => {
  switch (stage.type) {
    case "Reason":
    case "Decide":
      return {
    ...
    case "Verify":
      return {
        output: { passed: true, confidence: 0.85, reasoning: "Synthetic verification passed", verifiedAt: new Date().toISOString() },
        confidence: 0.85,
      };
    case "ApprovalGate":
...
function expectCompletedRun(run: { status: string; stageResults: Array<{ stageId: string; status: string }> }) {
...
describe("Phase 2: Cross-System Reconciliation", () => {
  it("runs in dry-run mode and returns stage results", async () => {
    ...
    const run = await rt.start(crossSystemReconciliationWorkflow, {
      systemAId: "erp", systemBId: "ledger", entityType: "invoice",
    }, { mode: "dry-run" });
  ...
  it("live mode pauses at approval gate (pending-approval)", async () => {
    ...
    const rt = makeSyntheticRuntime();
    const run = await rt.start(crossSystemReconciliationWorkflow, {
      systemAId: "erp", systemBId: "ledger", entityType: "contract",
    }, { mode: "live" });
...
describe("Phase 2: Executive Brief", () => {
  ...
  it("live run produces 4 stages with no approval gate (auto-publish policy)", async () => {
    const { executiveBriefWorkflow } = await import("./executive-brief.js");
    const rt = makeSyntheticRuntime();
    const run = await rt.start(executiveBriefWorkflow, { audienceLevel: "executive" }, { mode: "live" });
    expectCompletedRun(run);
    const stageIds = run.stageResults.map(r => r.stageId);
...
describe("Phase 2: Risk Escalation", () => {
  it("dry-run completes all stages", async () => {
    ...
    const rt = makeSyntheticRuntime();
    const run = await rt.start(riskEscalationWorkflow, {
      entityId: "ENTY-001", entityType: "vessel", domain: "maritime",
    }, { mode: "dry-run" });
    expectCompletedRun(run);
  });
  ...
  it("approval gate pauses pipeline in live mode", async () => {
    ...
    const rt = makeSyntheticRuntime();
    const run = await rt.start(riskEscalationWorkflow, {
      entityId: "ENTY-002", entityType: "counterparty", domain: "risk",
    }, { mode: "live" });
    expect(run.status).toBe("pending-approval");
...
describe("Phase 2: Evidence-Based Recommendation", () => {
  it("dry-run produces provenance graph stub", async () => {
    const { evidenceBasedRecommendationWorkflow } = await import("./evidence-based-recommendation.js");
    const rt = makeSyntheticRuntime();
    const run = await rt.start(evidenceBasedRecommendationWorkflow, {
      targetId: "ASSET-001",
...
describe("Vertical: Lyte — Operational Drift Review", () => {
  it("dry-run completes", async () => {
    ...
    const rt = makeSyntheticRuntime();
    const run = await rt.start(lyteOperationalDriftWorkflow, { services: ["lyte-api"] }, { mode: "dry-run" });
    expectCompletedRun(run);
  });
  ...
  it("counterfactual mode runs and outputs a run ID", async () => {
    ...
    const cf = await rt.start(lyteOperationalDriftWorkflow, {}, {
      mode: "counterfactual",
...
describe("Vertical: Aegis — Threat Triage", () => {
  it("dry-run produces triage stage results", async () => {
    ...
    const rt = makeSyntheticRuntime();
    const run = await rt.start(aegisThreatTriageWorkflow, { lookbackHours: 4 }, { mode: "dry-run" });
    expectCompletedRun(run);
  ...
  it("live mode pauses at approval gate (4 stages before gate + gate itself)", async () => {
    const { aegisThreatTriageWorkflow } = await import("./aegis-threat-triage.js");
    const rt = makeSyntheticRuntime();
    const run = await rt.start(aegisThreatTriageWorkflow, { minSeverity: "high" }, { mode: "live" });
    expect(run.status).toBe("pending-approval");
    expect(run.stageResults.map(r => r.stageId)).toContain("approval-gate");
  });
});
...
describe("Vertical: Vessels — Voyage Anomaly Review", () => {
  it("dry-run completes voyage anomaly pipeline", async () => {
    ...
    const rt = makeSyntheticRuntime();
    const run = await rt.start(vesselsVoyageAnomalyWorkflow, {
  ...
  it("replay mode skips completed stages", async () => {
    ...
    const replayed = await rt.start(vesselsVoyageAnomalyWorkflow, {}, {
      mode: "replay",
      sourceRunId: baseline.runId,
      replayDiffOnly: true,
    });
...
describe("Vertical: Terra — Portfolio Anomaly", () => {
  it("dry-run identifies portfolio anomaly stages", async () => {
    ...
    const rt = makeSyntheticRuntime();
    const run = await rt.start(terraPortfolioAnomalyWorkflow, { portfolioId: "CORE" }, { mode: "dry-run" });
    expectCompletedRun(run);
...
describe("Vertical: PRISM Counsel — Evidence Packaging", () => {
  it("dry-run completes matter evidence pipeline", async () => {
    const { prismCounselEvidencePackagingWorkflow } = await import("./prism-counsel-evidence-packaging.js");
    const rt = makeSyntheticRuntime();
    const run = await rt.start(prismCounselEvidencePackagingWorkflow, {
      matterIds: ["MTR-001"],
    }, { mode: "dry-run" });
    expectCompletedRun(run);
  });
...
describe("Vertical: Carlota Jo — Task Routing", () => {
  it("dry-run routes task and produces assignment decision", async () => {
    ...
    const rt = makeSyntheticRuntime();
    const run = await rt.start(carlotaJoTaskRoutingWorkflow, {
      clientId: "CLIENT-001",
  ...
  it("when stages return confidence below requireHumanBelow, engine escalates to pending-approval", async () => {
    const lowConfidenceExecutor: StageExecutorFn = async (stage, _input, _ctx) => {
      if (stage.type === "Verify") {
        return { output: { passed: false, confidence: 0.2, reasoning: "Low confidence synthetic" }, confidence: 0.2 };
      }
    ...
    const run = await rt.start(crossSystemReconciliationWorkflow, {
      systemAId: "a", systemBId: "b", entityType: "item",
    }, { mode: "dry-run" });
