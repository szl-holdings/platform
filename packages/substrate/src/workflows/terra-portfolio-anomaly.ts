...
export const terraPortfolioAnomalyWorkflow = defineWorkflow({
  ...
  stages: [
    ...
    Verify({
      ...
      description: "Validates anomaly classifications against market benchmarks and historical portfolio data.",
      minConfidence: 0.65,
      allowRevision: true,
...
export interface PortfolioAnomaly {
  propertyId: string;
  anomalyType: "distress-signal" | "valuation-outlier" | "tenant-risk" | "market-dislocation" | "covenant-breach-risk";
...
function buildPortfolioDecision(runId: string, anomalies: PortfolioAnomaly[], confidence: number): PortfolioAnomalyDecision {
  ...
  return {
    ...
    actions: anomalies.map((a, i) => ({
      ...
      action: a.anomalyType === "distress-signal"
