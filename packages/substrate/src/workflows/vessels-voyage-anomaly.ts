...
export const vesselsVoyageAnomalyWorkflow = defineWorkflow({
  ...
  stages: [
    ...
    Verify({
      ...
      description: "Validates anomaly classifications against historical patterns and regulatory thresholds.",
      minConfidence: 0.65,
      allowRevision: true,
    ...
    ApprovalGate({
      ...
      description: "Maritime operations analyst reviews the anomaly report before escalation is committed.",
...
export interface VoyageAnomaly {
  vesselId: string;
  voyageId: string;
  anomalyType: "ais-dark" | "unexpected-port" | "sts-transfer" | "route-deviation" | "sanctions-proximity";
...
function parseVoyageAnomalies(output: unknown): VoyageAnomaly[] {
  if (output && typeof output === "object" && Array.isArray((output as Record<string, unknown>)["anomalies"])) {
...
function buildAnomalyDecision(runId: string, anomalies: VoyageAnomaly[], confidence: number): VoyageAnomalyDecision {
  return {
    runId,
    anomalies,
    escalations: anomalies.map(a => ({
      vesselId: a.vesselId,
      action: a.anomalyType === "sts-transfer"
        ? "Create compliance case and notify sanctions team immediately"
        : "Flag for enhanced monitoring and counterparty due diligence",
      urgency: a.severity === "critical" ? "immediate" as const
        : a.severity === "high" ? "within-4h" as const : "next-business-day" as const,
