...
export const riskEscalationWorkflow = defineWorkflow({
  ...
  stages: [
    ...
    Verify({
      ...
      description: "Independent verifier confirms the risk score calculation and escalation rationale.",
      minConfidence: 0.65,
      allowRevision: true,
...
function parseSeedSignals(output: unknown, input: RiskEscalationInput): RiskSignalSummary[] {
  if (output && typeof output === "object" && Array.isArray((output as Record<string, unknown>)["signals"])) {
...
function buildDecision(
  ...
  const maxSeverity = signals.some(s => s.severity === "critical") ? "critical"
