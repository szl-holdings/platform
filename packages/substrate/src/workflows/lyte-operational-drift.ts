/**
 * @szl/substrate — Lyte Vertical Pack: Operational Drift Review
 *
...
export const lyteOperationalDriftWorkflow = defineWorkflow({
  ...
  stages: [
    ...
    Verify({
      ...
      description: "Confirms that drift signals are genuine trend deviations vs. measurement noise.",
      minConfidence: 0.65,
      allowRevision: true,
    ...
    ApprovalGate({
      ...
      description: "Operator reviews the drift packet and approves corrective actions before execution.",
...
function parseDriftItems(output: unknown): DriftItem[] {
  if (output && typeof output === "object" && Array.isArray((output as Record<string, unknown>)["driftItems"])) {
...
function buildDriftDecision(runId: string, driftItems: DriftItem[], confidence: number): OperationalDriftDecision {
  return {
    runId,
    driftItems,
    remediations: driftItems.map((d, i) => ({
      serviceId: d.serviceId,
      action: d.driftType === "slo-creep"
    ...
    overallDriftScore: driftItems.reduce((s, d) => s + d.driftScore, 0) / Math.max(driftItems.length, 1),
