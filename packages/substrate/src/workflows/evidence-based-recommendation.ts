...
export async function runEvidenceBasedRecommendation(
  ...
  const packet = run.status === "completed" && decideResult?.output
    ? buildDecisionPacket(run, input)
    : null;
...
function buildDecisionPacket(run: PipelineRun, input: EvidenceBasedRecommendationInput): RecommendationDecisionPacket {
  ...
  return {
    ...
    alternativesConsidered: ["Defer to next review cycle", "Partial implementation with monitoring"],
    expectedOutcome: "Positive impact on target metrics within 30-day window with moderate confidence",
