...
import { retrieverAdapterRegistry, type RetrieverAdapter } from "../adapters.js";
...
export const opportunityAuditWorkflow: WorkflowDefinition = {
  ...
  stages: [
    ...
    // before running in live mode.
    {
      ...
      timeoutMs: 45_000,
      maxRetries: 2,
      otelTags: { domain: "lyte", stage_category: "heavy-retrieval", python_worker_eligible: "true" },
      requiredEvidence: [],
      priority: "high",
    },
...
export async function runOpportunityAudit(
  ...
  const decision = run.status === "completed" && decideResult?.output
    ? parseRemediationDecision(run.runId, anomalies, decideResult.output, run.finalConfidence ?? 0)
    : null;
