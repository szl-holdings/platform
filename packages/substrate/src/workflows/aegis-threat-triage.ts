/**
 * @szl/substrate — PARAGON Vertical Pack: Threat Triage and Escalation Routing
 *
 * Ingests threat signals from PARAGON, performs AI-assisted triage to classify
...
export const aegisThreatTriageWorkflow = defineWorkflow({
  ...
  stages: [
    ...
    Verify({
      ...
      description:
        'Validates triage classifications for consistency with MITRE framework and internal policy.',
      minConfidence: 0.65,
      allowRevision: true,
  ...
  return [
    {
      ...
      description:
        'CVE-2024-XXXX active exploitation detected on EU-West-1 cluster — lateral movement indicators',
...
function buildThreatDecision(
  ...
  return {
    ...
    routingActions: threats.map((t) => ({
      ...
      action:
        t.triageTier === 'T1'
          ? 'Activate IR playbook immediately'
          : 'Assign to SOC queue for investigation',
      urgency:
        t.triageTier === 'T1'
          ? ('immediate' as const)
          : t.triageTier === 'T2'
