...
export const crossSystemReconciliationWorkflow = defineWorkflow({
  ...
  stages: [
    ...
    Retrieve({
      ...
      description:
        'Fetches authoritative state snapshot from System B for the same target entities.',
    ...
    Verify({
      ...
      description:
        'Independently verifies that identified discrepancies are genuine and not sampling artefacts.',
      minConfidence: 0.65,
      allowRevision: true,
    ...
    ApprovalGate({
      ...
      description:
        'Requires operator sign-off before any corrective writes are issued to either system.',
