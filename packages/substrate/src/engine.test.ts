...
const deterministicExecutor = async (
  ...
  switch (stage.type) {
    case "Retrieve":
      return { output: { documents: [{ id: "doc1", content: "test data", relevanceScore: 0.85 }] }, confidence: 0.85 };
    case "Reason":
      return { output: { analysis: "anomaly detected", findings: ["latency-spike"] }, confidence: 0.78 };
    case "Verify":
      return { output: { passed: true, confidence: 0.82, reasoning: "findings verified" }, confidence: 0.82 };
    case "Decide":
...
async function testCompilerRejectsMissingGate(): Promise<void> {
  ...
  if (!threw) throw new Error("Compiler must reject workflow with high-risk side effect and no gate");
...
async function testHookFiring(): Promise<void> {
  ...
  const runtime = new SubstrateRuntime({
    ...
    hooks: {
      before_pipeline: async () => { firedHooks.push("before_pipeline"); },
      after_pipeline: async () => { firedHooks.push("after_pipeline"); },
      before_stage: async (_run, stage) => { firedHooks.push(`before_stage:${stage.id}`); },
      after_stage: async (_run, stage) => { firedHooks.push(`after_stage:${stage.id}`); },
      before_finalize: async () => { firedHooks.push("before_finalize"); },
      after_finalize: async () => { firedHooks.push("after_finalize"); },
    },
  });
