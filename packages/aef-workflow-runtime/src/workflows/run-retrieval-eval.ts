...
export const runRetrievalEvalWorkflow: WorkflowDefinition = {
  ...
  steps: [
    ...
    },
    {
      ...
      description: "Execute the retrieval evaluation against golden fixtures, compute nDCG, recall, precision, MRR, and evidence-completeness metrics.",
      async execute(ctx: WorkflowContext, _prior: WorkflowStepResult[]) {
        const queries = ctx.input["queries"];
        const queryCount = Array.isArray(queries) ? queries.length : Number(ctx.input["queryCount"] ?? 0);
