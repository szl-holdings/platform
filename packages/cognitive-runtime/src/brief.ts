...
export function generateExecutiveBrief(run: CognitiveLoopRun): ExecutiveBrief {
  ...
  const execOut = execPhase?.output as
    ...
    | undefined;
  const verifyOut = verifyPhase?.output as { action?: string; overallScore?: number } | undefined;
  const reflectOut = reflectPhase?.output as
    | { lesson?: string; whatToTryNext?: string[] }
    | undefined;

  const completedSteps =
    execOut?.completedSteps ?? run.stepResults.filter((s) => s.status === 'completed').length;
  const failedSteps =
    execOut?.failedSteps ?? run.stepResults.filter((s) => s.status === 'failed').length;
  ...
  const executiveSummary = [
    ...
    verifyOut
      ? `Verifier verdict: ${verifyOut.action ?? 'N/A'} (score=${(verifyOut.overallScore ?? 0).toFixed(2)}).`
      : '',
  ...
  return {
    ...
    verifySummary: {
      verifyAttempts:
        (run.verifyRevisions ?? 0) + (run.phases.some((p) => p.phase === 'verify') ? 1 : 0),
