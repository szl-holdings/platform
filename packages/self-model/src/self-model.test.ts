...
import { updateAfterRun, requestHelpIfBelowThreshold } from "./update.js";
import {
...
import type { IdentityProfile, RunOutcome, EscalationThreshold } from "./types.js";
...
describe("SelfModelStore — creation", () => {
  ...
  it("allows custom capabilities and tool access", () => {
    ...
    const model = store.create({
      ...
      toolAccess: [{ toolId: "tool-1", name: "Search", permitted: true, riskTier: "advisory-only" }],
    });
...
describe("updateAfterRun — confidence adjustments", () => {
  ...
  it("resets consecutive failures on success", () => {
    ...
    const result = updateAfterRun("agent-001", makeOutcome({ runId: "run-3", status: "success" }), store);
  ...
  it("throws for unknown agent", () => {
    expect(() => updateAfterRun("no-such-agent", makeOutcome(), store)).toThrow(/No self-model found/);
  });
});
...
describe("updateAfterRun — drift tracking", () => {
  ...
  beforeEach(() => {
    store = new SelfModelStore();
    store.create({ agentId: "agent-001", identityProfile: makeIdentity(), escalationThresholds: [] });
  });
...
describe("requestHelpIfBelowThreshold — escalation", () => {
  ...
  it("triggers help when consecutiveFailures exceeds threshold", () => {
    ...
    store.create({
      ...
      identityProfile: makeIdentity(),
      escalationThresholds: [
        { metric: "consecutiveFailures", threshold: 3, action: "request-help", notifyRecipients: [] },
      ],
    });
...
describe("updateAfterRun — threshold-triggered help requests", () => {
  it("requests help after enough failures drop confidence below threshold", () => {
    ...
    store.create({
      ...
      identityProfile: makeIdentity(),
      escalationThresholds: [
        { metric: "confidence", threshold: 0.95, action: "request-help", notifyRecipients: ["supervisor"] },
      ],
    });
    const result = updateAfterRun(
      "agent-001",
...
describe("Zod schema validation", () => {
  ...
  it("rejects invalid status", () => {
    expect(() => RunOutcomeSchema.parse({ runId: "r1", agentId: "a1", status: "unknown-status" })).toThrow();
  });
