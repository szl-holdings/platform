# Frontier Mythos — Research Doctrine

> This document is the canonical doctrine layer for the `@szl-holdings/frontier-mythos` package.
> Every actor ID (`actor-*`) and idea ID (`idea-*`) defined in `src/index.ts` must appear here.
> Run `node packages/frontier-mythos/scripts/validate-sync.cjs` to verify synchronization.
...
Ideas are consumed by `MythosIndex` as the canonical graph nodes.
...
Doctrine citations referenced by `MYTHOS_CITATIONS` in `src/index.ts`.
...
Three ideas added to `MYTHOS_IDEAS` in May 2026 to align the Helios API enrichment keyspace with the
MythosIndex UI. These IDs (`idea-embodied-ai`, `idea-next-gen-gpu`, `idea-sovereign-ai`) are now part
...
> Every `actor-*` and `idea-*` ID exported by `packages/frontier-mythos/src/index.ts`
> must appear in this document.

Run `node packages/frontier-mythos/scripts/validate-sync.cjs` in CI or before any
release of the `@szl-holdings/frontier-mythos` package. If the script exits non-zero,
