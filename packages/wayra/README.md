...
Wayra: news/feed ingest + adapters + snapshot export (the wind that carries signal).
...
## Honesty note
Snapshots are timestamped static captures, labeled with data_freshness.

## Install (editable, from monorepo root)
```bash
pip install -e packages/wayra
```
