...
describe("recommendation rendering — maritime (Vessels)", () => {
  it("renders a complete vessel-reroute recommendation with all 8 proof-chain fields", () => {
    const rec = createRecommendation({
      ...
      summary: "AIS dark event correlated with OFAC-sanctioned port proximity. Reroute via Khor Fakkan.",
      rationale: "3-signal correlation: AIS dark event (74 min), OFAC match tier-2, port congestion spike. Confidence derived from Reuters sanctions feed and Lloyd's AIS feed convergence.",
      suggestedAction: "execute",
      actionPayload: { vesselId: "IMO-9234567", newRoute: "khor-fakkan-bypass" },
      confidence: 0.92,
      freshness: 0.89,
      projectedImpact: "Reroute via Khor Fakkan avoids $185K/day demurrage and clears OFAC gate within 48 hours.",
      projectedRisk: "Without reroute, vessel enters sanctioned corridor — $3.2M cargo exposure and potential port detention.",
  ...
  it("renders a vessel-hold recommendation with approval-required policy outcome", () => {
    const rec = createRecommendation({
      ...
      rationale: "Port congestion index at 82/100 with no berth ETA. Holding saves $185K/day demurrage.",
...
describe("recommendation rendering — legal (Counsel / PRISM)", () => {
  it("renders a complete filing-deadline recommendation with all 8 proof-chain fields", () => {
    const rec = createRecommendation({
      ...
      summary: "Discovery blockage + counsel lag detected 11 days before filing deadline. Parallel track required.",
      rationale: "Outside counsel performance index at 34/100 with discovery blockage. Court deadline in 11 days; single-track response has <20% completion probability.",
      suggestedAction: "execute",
      actionPayload: { matterId: "matter-lawsuit-001", track: "parallel" },
      confidence: 0.94,
      freshness: 0.92,
      projectedImpact: "Parallel filing track eliminates deadline breach risk and preserves $4.1M litigation position.",
      projectedRisk: "Without parallel track, single-track discovery yields <20% completion — $4.1M exposure crystallises on missed deadline.",
      policyEvaluation: POLICY_EVAL,
...
describe("recommendation rendering — security (Sentra)", () => {
  it("renders a complete ransomware-isolation recommendation with all 8 proof-chain fields", () => {
    const rec = createRecommendation({
      ...
      summary: "Ransomware lateral movement detected across 3 OT assets. Immediate isolation required.",
      rationale: "CrowdStrike telemetry shows ransomware footprint on SCADA, HMI, and PLC. Lateral movement velocity: 4 assets/hr. OT network exposure window: <4 hours.",
      suggestedAction: "execute",
      actionPayload: { segmentIds: ["scada-001", "hmi-001", "plc-001"] },
      confidence: 0.91,
      freshness: 0.96,
      projectedImpact: "Isolation contains ransomware spread, preserving $2.8M cost-avoidance and maintaining production continuity.",
      projectedRisk: "Without isolation, SCADA breach containment fails within 4 hours — $2.8M downtime and potential multi-site OT shutdown.",
      policyEvaluation: POLICY_EVAL,
...
describe("recommendation rendering — finance (SZL Treasury / Pulse)", () => {
  it("renders a treasury hedge recommendation with all 8 proof-chain fields", () => {
    const rec = createRecommendation({
      ...
      rationale: "GBP/USD volatility at 85th percentile. Unhedged position of $2.1M at risk of 12% adverse move over 30-day window. Alpaca macro signals confirm trend.",
      suggestedAction: "approve",
      actionPayload: { instrument: "USD/GBP-forward-30d", notional: 2_100_000 },
      confidence: 0.88,
      freshness: 0.91,
      projectedImpact: "30-day forward contract locks $2.1M at current rate, eliminating adverse FX exposure.",
      projectedRisk: "Without hedge, a 12% adverse GBP move crystallises $252K loss on unhedged position.",
      policyEvaluation: POLICY_EVAL,
...
describe("recommendation rendering — real-estate (Terra)", () => {
  it("renders a property acquisition recommendation with all 8 proof-chain fields", () => {
    const rec = createRecommendation({
      ...
      summary: "Competing bid detected. Accelerate acquisition timeline to secure asset before deadline.",
      rationale: "PropertyRadar data indicates competing LOI submitted 2 days ago. Cap rate at 6.8% — 40bps above comparable comps. LinkedIn signals confirm seller motivation.",
      suggestedAction: "approve",
      actionPayload: { propertyId: "prop-1430-industrial", targetCloseDate: "2026-05-01" },
      confidence: 0.85,
      freshness: 0.88,
      projectedImpact: "Accelerating acquisition locks 6.8% cap rate asset — $480K projected annual NOI on $7.1M purchase price.",
      projectedRisk: "Delay risks competing bid prevailing — $480K/yr NOI opportunity lost and replacement asset at lower cap rate.",
      policyEvaluation: POLICY_EVAL,
...
describe("recommendation rendering — cross-product schema invariants", () => {
  ...
  it.each(ALL_DOMAINS)("domain '%s' produces a recommendation with a unique recommendationId", (domain) => {
    const rec = createRecommendation({
      domain,
