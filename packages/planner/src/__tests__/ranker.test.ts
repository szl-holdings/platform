...
import { createPlan, getPlanFallbacks, InMemoryPlanStore, rankFallbacks } from '../index.js';
...
describe('rankFallbacks (decision-engine)', () => {
  it('assigns sequential rank starting at 1 and surfaces a fallbackPriority score', async () => {
    ...
    const primary = await createPlan('Cut maritime fuel cost', { fallbackCount: 3 }, { store });
