...
test('scans a bounded SVG prolog without backtracking', () => {
  const declared = Buffer.from(
    '\ufeff \n<?xml version="1.0"?>\n<!-- generated metadata -->\n<svg width="123" height="456"></svg>',
  );
  assert.deepEqual(imageSize(declared), { width: 123, height: 456, type: 'svg' });

  const repeatedDeclarations = Buffer.from(
    `${'<?xml version="1.0"?>'.repeat(128)}<svg width="123" height="456"></svg>`,
  );
  assert.throws(() => imageSize(repeatedDeclarations), /unsupported file type/);

  const craftedPrefix = Buffer.from(`${'<?xml'.repeat(700)}<svg width="123" height="456">`);
  assert.throws(() => imageSize(craftedPrefix), /unsupported file type/);

  const missingRoot = Buffer.from('<?xml version="1.0"?><!-- metadata --><not-svg/>');
  assert.throws(
    () => imageSize(missingRoot),
    (error) => error instanceof TypeError && error.message === 'unsupported file type (file: )',
  );
});

test('preserves upstream SVG absolute-unit compatibility without a viewBox', () => {
  const fixtures = [
    ['1.28125in', '4.75in'],
    ['7.6875em', '342pt'],
    ['3.254375cm', '120.65mm'],
  ...
  for (const [width, height] of fixtures) {
    assert.deepEqual(imageSize(Buffer.from(`<svg width="${width}" height="${height}"></svg>`)), {
      width: 123,
      height: 456,
      type: 'svg',
    });
