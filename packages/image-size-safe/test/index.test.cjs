...
test('parses bounded SVG, TIFF, and KTX metadata', () => {
  ...
  assert.deepEqual(
    imageSize(Buffer.from('<svg width="0" viewBox="0 0 100 50"></svg>')),
    { width: 100, height: 50, type: 'svg' },
  );
  assert.deepEqual(
    imageSize(Buffer.from('<svg height="0" viewBox="0 0 100 50"></svg>')),
    { width: 100, height: 50, type: 'svg' },
  );
  ...
  assert.deepEqual(imageSize(Buffer.from('<svg width="1m" height="2m"></svg>')), {
    width: 3780,
    height: 7559,
    type: 'svg',
  });
