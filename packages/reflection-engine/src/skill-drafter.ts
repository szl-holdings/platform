...
function inferDomainFromTrace(trace: TraceRecord): string {
  ...
  if (agentId.includes("maritime") || agentId.includes("vessel") || agentId.includes("helmsman")) return "maritime";
...
function inferCategory(
  trace: TraceRecord,
): CandidateSkill["category"] {
  const tools = trace.toolCalls.map((t) => t.toolName.toLowerCase()).join(" ");
  const agentId = (trace.agentId ?? "").toLowerCase();
  if (tools.includes("search") || tools.includes("retriev") || tools.includes("query")) return "research";
  if (tools.includes("analyz") || tools.includes("score") || tools.includes("detect")) return "analysis";
  if (tools.includes("generat") || tools.includes("draft") || tools.includes("write")) return "generation";
  if (tools.includes("extract") || tools.includes("parse")) return "extraction";
  if (tools.includes("validate") || tools.includes("check") || tools.includes("verify")) return "validation";
  if (tools.includes("monitor") || tools.includes("alert") || tools.includes("watch")) return "monitoring";
