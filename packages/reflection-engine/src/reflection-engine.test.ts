...
describe("scorer", () => {
  ...
  it("penalizes tool failures", () => {
    const trace = makeTrace({
      toolCalls: [
        { toolId: "t1", toolName: "search", success: false, errorCode: "TIMEOUT", approvalRequired: false, retries: 1 },
        { toolId: "t2", toolName: "summarize", success: true, approvalRequired: false, retries: 0 },
      ],
      status: "failed",
      errors: [{ code: "TOOL_FAILED", message: "search timed out", timestamp: new Date().toISOString() }],
    });
    const score = scoreTrace(trace);
...
describe("classifier", () => {
  ...
  it("classifies tool_failure when tools fail", () => {
    const trace = makeTrace({
      toolCalls: [
        { toolId: "t1", toolName: "fetch", success: false, errorCode: "ERR", approvalRequired: false, retries: 0 },
      ],
      status: "failed",
  ...
  it("classifies guardrail_block when a guardrail blocks", () => {
    const trace = makeTrace({
      guardrailResults: [
        { guardId: "g1", tier: "output", outcome: "block", reason: "blocked" },
      ],
      status: "failed",
    });
  ...
  it("classifies retrieval_miss when all retrievals miss", () => {
    const trace = makeTrace({
      retrieval: [
        { source: "kb", hitCount: 0, missCount: 3 },
      ],
      status: "failed",
    });
  ...
  it("classifies policy_violation for policy errors", () => {
    const trace = makeTrace({
      status: "failed",
      errors: [
        { code: "POLICY_BLOCKED", message: "policy denied this action", timestamp: new Date().toISOString() },
      ],
    });
...
describe("lesson-writer", () => {
  ...
  it("produces failure analysis for a failed run", () => {
    const trace = makeTrace({
      toolCalls: [
        { toolId: "t1", toolName: "fetch", success: false, errorCode: "TIMEOUT", approvalRequired: false, retries: 0 },
      ],
      status: "failed",
      errors: [{ code: "TOOL_ERR", message: "fetch timed out", timestamp: new Date().toISOString() }],
    });
    const score = scoreTrace(trace);
    const route = extractBestRoute(trace);
...
describe("reflect()", () => {
  ...
  it("produces a full reflection for a known trace", async () => {
    ...
    writer.completeTrace("reflect-trace-001", { status: "completed", latencyMs: 1200, totalTokens: 400, costUsd: 0.004 });
