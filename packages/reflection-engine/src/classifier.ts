...
export function classifyFailureMode(trace: TraceRecord): FailureMode {
  const hasGuardrailBlock = trace.guardrailResults.some(
    (g) => g.outcome === "block",
  );
  ...
  if (
    ...
    trace.latencyMs > HIGH_LATENCY_THRESHOLD_MS
  ) {
    return "timeout";
  }
  ...
  if (
    ...
    trace.costUsd > HIGH_COST_THRESHOLD_USD
  ) {
    return "high_cost";
  }
  ...
  const hasRetrievalMiss =
    ...
    trace.retrieval.every((r) => r.hitCount === 0 && r.missCount > 0);
