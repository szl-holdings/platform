...
describe('telemetry E2E — complete governed-autonomy workflow', () => {
  it('full lifecycle emits all required surface spans', () => {
    ...
    expect(
      emittedClasses.has('business.transaction.started'),
      'worker_jobs/agent_runs span missing',
    ).toBe(true);
    expect(
      emittedClasses.has('business.transaction.completed'),
      'transaction completion span missing',
    ).toBe(true);

    // model/tool call surface (recommendation.generated)
    expect(
      emittedClasses.has('recommendation.generated'),
      'recommendation.generated span missing',
    ).toBe(true);
    ...
    // policy gate surface
    expect(
      emittedClasses.has('policy.violation.detected'),
      'policy.violation.detected span missing',
    ).toBe(true);
  ...
  it('connector sync — ingestKPIBatch processes records and emits atlas events', async () => {
    ...
    for (const ev of buf) {
      expect(ev.eventClass).toMatch(
        /business\.risk\.detected|outcome\.realized|business\.opportunity\.created/,
      );
    }
  });
  ...
  it('action approval span carries human approval level and delay metadata', () => {
    ...
    const a = approvals[0] as Extract<
