...
function makeMinimalPolicyEvaluation(): PolicyEvaluation {
  return {
    ...
    evidenceChain: [
      { source: "db", summary: "Test evidence", confidence: 0.9, freshness: 0.95 },
    ],
    policyResult: {
      effect: "allow",
      allowed: true,
...
describe("executeWorkflow — PolicyEvaluation enforcement", () => {
  it("throws immediately when policyEvaluation is absent (live execution path)", async () => {
    ...
    await expect(
      executeWorkflow({ definition: def })
    ).rejects.toThrow(/policyEvaluation is required/i);
  });
  ...
  it("does NOT throw when isDryRun=true (dry-run bypass)", async () => {
    const def = makeDefinition({ requiresExplicitApproval: false });
    await expect(
      executeWorkflow({ definition: def, isDryRun: true })
    ).resolves.toBeDefined();
  });
  ...
  it("does NOT throw when isSimulation=true (simulation bypass)", async () => {
    const def = makeDefinition({ requiresExplicitApproval: false });
    await expect(
      executeWorkflow({ definition: def, isSimulation: true })
    ).resolves.toBeDefined();
  });
  ...
  it("rejects a partial policyEvaluation missing required evaluationId — no silent fallback", async () => {
    const def = makeDefinition({ requiresExplicitApproval: false });
    const partial = { mode: "approval-required", action: "test.action" } as unknown as PolicyEvaluation;
    await expect(
      executeWorkflow({ definition: def, policyEvaluation: partial })
    ).rejects.toThrow(/policyEvaluation.*failed schema validation/i);
  });
  ...
  it("accepts malformed policyEvaluation when isDryRun=true (schema validation bypassed for dry runs)", async () => {
    const def = makeDefinition({ requiresExplicitApproval: false });
    await expect(
      executeWorkflow({ definition: def, policyEvaluation: {} as unknown as PolicyEvaluation, isDryRun: true })
    ).resolves.toBeDefined();
  });
});
...
describe("executeWorkflow — simulation", () => {
  it("returns status=completed with simulationSummary", async () => {
    const def = makeDefinition({ requiresExplicitApproval: false });
    const { run, simulationSummary } = await executeWorkflow({ definition: def, isSimulation: true });
    expect(run.status).toBe("completed");
...
describe("executeWorkflow — live execution", () => {
  it("completes all steps and returns status=completed", async () => {
    const def = makeDefinition({
      requiresExplicitApproval: false,
      steps: [
        { id: "s1", name: "Step 1", handler: "noop-handler", executionMode: "manual", requiresApproval: false, retryCount: 0 },
        { id: "s2", name: "Step 2", handler: "echo-handler", executionMode: "manual", requiresApproval: false, retryCount: 0, parameters: { value: 42 } },
      ],
    });
    const { run } = await executeWorkflow({
      definition: def,
...
describe("executeWorkflow — failure and rollback", () => {
  it("marks run as failed when a step handler throws", async () => {
    const def = makeDefinition({
      requiresExplicitApproval: false,
      steps: [
        { id: "s1", name: "Fail Step", handler: "fail-handler", executionMode: "manual", requiresApproval: false, retryCount: 0 },
      ],
      rollbackPolicy: "none",
    });
    const { run } = await executeWorkflow({
  ...
  it("performs rollback when rollbackPolicy=step and rollback handler is registered", async () => {
    ...
    const def: WorkflowDefinition = {
      ...
      rollbackPolicy: "step",
      steps: [
        { id: "s1", name: "Succeed Step", handler: "success-then-fail-handler", rollbackHandler: "s1-rollback", executionMode: "manual", requiresApproval: false, retryCount: 0 },
        { id: "s2", name: "Fail Step", handler: "success-then-fail-handler", executionMode: "manual", requiresApproval: false, retryCount: 0 },
      ],
    };
