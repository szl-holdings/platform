...
function kernelExecutionRequestSnapshot(request: KernelExecutionRequest): KernelExecutionRequest {
  const authorization = request.authorization;
  const envelope = authorization.envelope;
  const decision = authorization.decision;
  const approval = authorization.approval;
  const inputCompatibility = request.inputCompatibility;
  const budget = request.budget;
  let parameters: Readonly<Record<string, unknown>>;
  try {
    parameters = Object.freeze(structuredClone(request.parameters));
  } catch (error) {
    throw new StateNativeError(
      'INVALID_INPUT',
      'Kernel parameters must be structured-cloneable.',
      undefined,
      { cause: error },
    );
  }
  ...
  return Object.freeze({
    authorization: Object.freeze({
      ...
      allowedSensitivities: Object.freeze([...authorization.allowedSensitivities]),
      approval: approval
        ? Object.freeze({
            approvalId: approval.approvalId,
    ...
    tenantId: request.tenantId,
    sessionId: request.sessionId,
    inputCapsuleIds: Object.freeze([...request.inputCapsuleIds]),
    inputCompatibility: Object.freeze({ ...inputCompatibility }),
    parameters,
    budget: Object.freeze({
      maxRuntimeMs: budget.maxRuntimeMs,
      maxInputBytes: budget.maxInputBytes,
      maxOutputBytes: budget.maxOutputBytes,
      maxStateWrites: budget.maxStateWrites,
    }),
    epochId: request.epochId,
    stateGrantId: request.stateGrantId,
...
export class AlloyKernelRuntime {
  ...
  public async execute(rawRequest: KernelExecutionRequest): Promise<KernelExecutionResult> {
