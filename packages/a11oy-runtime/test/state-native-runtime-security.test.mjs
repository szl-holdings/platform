...
test('governed requests are snapshotted before asynchronous execution', async () => {
  const stateKey = randomBytes(32);
  const bus = new AlloyStateBus({ masterKey: stateKey });
  const manager = new CognitiveEpochManager();
  ...
  try {
    ...
    const request = requestFor({
      ...
      kernelId: 'state.request-snapshot',
      compatibility: active.compatibility,
      epochId: 'epoch_request_snapshot',
    });
    const execution = runtime.execute(request);
    request.authorization.decision.effect = 'block';
    request.authorization.decision.reason = 'Caller mutated the admitted policy decision.';

    const result = await execution;
    assert.equal(result.receipt.outcome, 'success');
    assert.equal(result.receipt.policyEffect, 'allow');
    assert.equal(
      result.receipt.policyReason,
      'Bounded security conformance policy allows execution.',
    );
