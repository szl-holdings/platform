...
class NemoHooks {
  ...
  registerSuite(config: NemoEvalConfig): void {
    ...
    logger.info({ suiteId: config.suiteId, caseCount: config.cases.length, model: config.modelId }, "NeMo eval suite registered");
  }
  ...
  async runEval(
    ...
    for (const evalCase of suite.cases) {
      ...
      try {
        ...
        const { passed, score, failureReason } = this.scoreCase(output, evalCase, suite.scoringStrategy);
        results.push({ caseId: evalCase.id, passed, score, latencyMs, actualOutput: output.slice(0, 500), failureReason });
      } catch (err) {
        const latencyMs = Date.now() - start;
        results.push({ caseId: evalCase.id, passed: false, score: 0, latencyMs, failureReason: String(err) });
      }
    }
    ...
    const recommendation: NemoEvalReport["recommendation"] =
      passRate >= suite.passThreshold + 0.1 ? "promote" :
        passRate >= suite.passThreshold ? "hold" : "reject";
    ...
    logger.info({ suiteId, modelId: suite.modelId, passRate, recommendation }, "NeMo eval complete");
    return report;
  }
  ...
  emitEvent(params: Omit<NemoObservabilityEvent, "eventId" | "timestamp">): NemoObservabilityEvent {
    ...
    logger.debug({ type: event.type, severity: event.severity, modelId: event.modelId }, "NeMo observability event");
    return event;
  }

  getEvents(filters: { type?: NemoObservabilityEvent["type"]; severity?: string; modelId?: string; limit?: number } = {}): NemoObservabilityEvent[] {
  ...
  private scoreCase(
    ...
    switch (strategy) {
      case "keyword_match": {
        ...
        const found = evalCase.expectedKeywords.filter(k => output.toLowerCase().includes(k.toLowerCase()));
        const score = found.length / evalCase.expectedKeywords.length;
        const missing = evalCase.expectedKeywords.filter(k => !output.toLowerCase().includes(k.toLowerCase()));
        return { passed: score >= 0.8, score, failureReason: missing.length > 0 ? `Missing: ${missing.join(", ")}` : undefined };
      }
      case "exact_match": {
        const passed = output.trim() === (evalCase.expectedOutput ?? "").trim();
        return { passed, score: passed ? 1.0 : 0.0, failureReason: passed ? undefined : "Output does not exactly match expected" };
      }
      ...
      case "llm_judge": {
        ...
        return { passed: score >= 0.4, score, failureReason: score < 0.4 ? "Low semantic similarity to expected output" : undefined };
      }
      default:
