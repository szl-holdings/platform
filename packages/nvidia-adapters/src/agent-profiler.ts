...
function gradePerformance(entry: AgentProfileEntry): AgentProfileEntry['performanceGrade'] {
  ...
  if (durationMs < 5000 && entry.totalCostUsd < 0.1) return 'B';
  if (durationMs < 15000 && entry.totalCostUsd < 0.5) return 'C';
...
class AgentProfiler {
  ...
  startTrace(params: {
    ...
    const traceId =
      params.traceId ?? `trace-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    ...
    logger.debug(
      { traceId, agentId: params.agentId, tier: params.agentTier },
      'Agent trace started',
    );
    return traceId;
  }
  ...
  completeStep(
    traceId: string,
    stepId: string,
    outcome: { success: boolean; errorMessage?: string; durationMs?: number },
  ): void {
    const entry = this.entries.get(traceId);
    if (!entry) return;
  ...
  endTrace(
    traceId: string,
    status: AgentProfileEntry['status'],
    errorMessage?: string,
  ): AgentProfileEntry {
    const entry = this.entries.get(traceId);
    if (!entry) throw new Error(`Trace '${traceId}' not found`);
  ...
  getHistory(
    filters: { agentId?: string; tier?: string; status?: string; limit?: number } = {},
  ): AgentProfileEntry[] {
  ...
  getSummary(agentId: string): ProfileSummary | undefined {
    const records = this.completed.filter(
      (e) => e.agentId === agentId && e.durationMs !== undefined,
    );
    ...
    for (const r of records) {
      ...
      if (r.performanceGrade)
        gradeDist[r.performanceGrade] = (gradeDist[r.performanceGrade] ?? 0) + 1;
    }
