...
import type { EmailCalendarConnector, ConnectorMetadata, ConnectorStatus } from "../interfaces.js";
...
export class EmailCalendarDemoAdapter implements EmailCalendarConnector {
  ...
  getUnreadCount(): number { return 7; }

  getUpcomingMeetings() {
    return [
      { id: "mtg-001", title: "VIP Pre-Arrival Briefing — Castellano Estate", startAt: new Date(Date.now() + 2 * 3600_000).toISOString(), attendees: ["estate-mgr@carlota.jo", "housekeeper@castellano.estate"] },
      { id: "mtg-002", title: "Seasonal Deep Clean Sign-Off", startAt: new Date(Date.now() + 6 * 3600_000).toISOString(), attendees: ["ops@carlota.jo"] },
    ];
  }
  ...
  private async _emitInitialSignals(): Promise<void> {
    ...
    await this._emitSignal({
      ...
      rawPayload: {
        ...
        arrivalAt: new Date(Date.now() + 18 * 3600_000).toISOString(),
        openChecklistItems: 4,
        openItems: ["Wine cellar temperature calibration", "Master suite linen change", "Pool heating activation", "Security access codes reset"],
  ...
  private async _emitPeriodicSignals(): Promise<void> {
    ...
    await this._emitSignal({
      ...
      confidence: 0.99,
      severity: "info",
      entityRefs: [{ entityId: "property-castellano", entityType: "property", displayName: "Castellano Estate" }],
