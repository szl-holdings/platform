...
import type { CrmProjectConnector, ConnectorMetadata, ConnectorStatus } from "../interfaces.js";
...
export class CrmProjectDemoAdapter implements CrmProjectConnector {
  ...
  getOpenDeals() {
    return [
      { dealId: "deal-terra-001", name: "Eastside Commercial Park Acquisition", stage: "due-diligence", value: 12_500_000 },
      { dealId: "deal-terra-002", name: "Harbor View Residential Portfolio", stage: "loi-signed", value: 7_800_000 },
      { dealId: "deal-vessels-001", name: "MV Nordic Pioneer Charter Extension", stage: "negotiation", value: 2_100_000 },
    ];
  }

  getOverdueTasks() {
    return [
      { taskId: "task-001", title: "Eastside Park — Environmental Assessment Sign-Off", dueAt: new Date(Date.now() - 48 * 3600_000).toISOString(), assignee: "legal@szl.holdings" },
      { taskId: "task-002", title: "Harbor View — Title Search Completion", dueAt: new Date(Date.now() - 24 * 3600_000).toISOString(), assignee: "terra-ops@szl.holdings" },
    ];
  }
  ...
  private async _emitInitialSignals(): Promise<void> {
    ...
    await this._emitSignal({
      ...
      confidence: 0.99,
      severity: "medium",
      entityRefs: [{ entityId: "deal-terra-001", entityType: "deal", displayName: "Eastside Commercial Park Acquisition", domain: "real-estate" }],
      rawPayload: {
