...
import type { StorageDocsConnector, ConnectorMetadata, ConnectorStatus } from "../interfaces.js";
...
export class StorageDocsDemoAdapter implements StorageDocsConnector {
  ...
  getRecentDocuments() {
    return [
      { docId: "doc-001", title: "MV Soltana — Charter Party Amendment v3", updatedAt: new Date(Date.now() - 4 * 3600_000).toISOString(), author: "legal@arcturus.sh" },
      { docId: "doc-002", title: "Castellano Estate — VIP Arrival SLA Agreement", updatedAt: new Date(Date.now() - 1 * 3600_000).toISOString(), author: "carlota@carlota.jo" },
    ];
  }
  ...
  private async _emitInitialSignals(): Promise<void> {
    ...
    await this._emitSignal({
      ...
      confidence: 0.99,
      severity: "info",
      entityRefs: [{ entityId: "matter-001", entityType: "matter", displayName: "Soltana Charter Amendment", domain: "legal" }],
      rawPayload: {
