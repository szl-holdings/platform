...
export class MessagingDemoAdapter implements MessagingConnector {
  ...
  getActiveAlerts() {
    return [
      {
        channel: '#fleet-ops-critical',
        message: 'MV Soltana AIS dark — 134min — sanctions corridor proximity',
        sentAt: new Date(Date.now() - 120 * 60_000).toISOString(),
      },
      {
        channel: '#estate-readiness',
        message: 'Castellano Estate: 4 open checklist items — VIP arrival T-18h',
        sentAt: new Date(Date.now() - 30 * 60_000).toISOString(),
      },
      {
        channel: '#treasury-risk',
        message: 'FX hedge ratio drifted beyond 15% — review required',
