...
import type { PropertyOpsConnector, ConnectorMetadata, ConnectorStatus } from "../interfaces.js";
...
export class PropertyOpsDemoAdapter implements PropertyOpsConnector {
  ...
  getMaintenanceAlerts() {
    return [
      { propertyId: "property-castellano", unit: "Pool House", issue: "Pool heating system not responding", priority: "high", reportedAt: new Date(Date.now() - 2 * 3600_000).toISOString() },
      { propertyId: "property-castellano", unit: "Wine Cellar", issue: "Temperature above threshold (16°C vs 13°C target)", priority: "high", reportedAt: new Date(Date.now() - 1 * 3600_000).toISOString() },
      { propertyId: "property-harbor-view", unit: "Unit 4B", issue: "HVAC filter replacement overdue", priority: "medium", reportedAt: new Date(Date.now() - 5 * 3600_000).toISOString() },
    ];
  }
  ...
  private async _emitInitialSignals(): Promise<void> {
    ...
    await this._emitSignal({
      ...
      severity: "high",
      entityRefs: [
        { entityId: "property-castellano", entityType: "property", displayName: "Castellano Estate", domain: "real-estate" },
      ],
      rawPayload: {
    ...
    await this._emitSignal({
      ...
      confidence: 0.99,
      severity: "high",
      entityRefs: [{ entityId: "property-castellano", entityType: "property", displayName: "Castellano Estate", domain: "real-estate" }],
      rawPayload: {
