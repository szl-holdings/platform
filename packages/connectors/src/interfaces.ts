...
export interface WebhookConnector extends ConnectorAdapter {
  category: "webhooks";
  getReceivedEvents(): Array<{ eventId: string; source: string; payload: Record<string, unknown>; receivedAt: string }>;
}
...
export interface AISMaritimeConnector extends ConnectorAdapter {
  ...
  getDarkPeriods(): Array<{ mmsi: string; startedAt: string; durationMinutes: number; lat: number; lon: number }>;
}

export interface PropertyOpsConnector extends ConnectorAdapter {
  category: "property-ops";
  getMaintenanceAlerts(): Array<{ propertyId: string; unit: string; issue: string; priority: string; reportedAt: string }>;
...
export interface SecurityToolsConnector extends ConnectorAdapter {
  ...
  getActiveThreats(): Array<{ threatId: string; name: string; severity: string; affectedAssets: string[]; detectedAt: string }>;
...
export interface LegalMatterConnector extends ConnectorAdapter {
  ...
  getUpcomingDeadlines(): Array<{ matterId: string; matterName: string; deadline: string; type: string }>;
  getRetainerStatus(): Array<{ clientId: string; clientName: string; balanceUsd: number; threshold: number }>;
}
