...
test('runtime endpoint normalization safely accepts many trailing slashes', async () => {
  const root = await fixtureRoot();
  const keys = keyPair();
  ...
  try {
    const report = await runConformance({
      ...
      root,
      manifest: fixtureManifest(),
      baseUrl: `${server.baseUrl}${'/'.repeat(10_000)}`,
      expectedGitSha: FIXTURE_SHA,
      publicKeyPem: keys.publicKeyPem,
      expectedFingerprint: publicKeyFingerprint(keys.publicKeyPem),
      nowMs: FIXTURE_NOW,
    });
    assert.equal(report.conformant, true, JSON.stringify(report.checks));
  } finally {
    await server.close();
  }
});
