...
const AMBIGUOUS_INPUT =
  "the counterparty situation should be handled appropriately";

const CRISP_INPUT =
  "Block all payouts over $500,000 without two finance approvers.";
...
describe("compilePolicyWithLLM", () => {
  ...
  it("invokes the LLM only for rules below the threshold", async () => {
    ...
    const result = await compilePolicyWithLLM(
      `${CRISP_INPUT}\n${AMBIGUOUS_INPUT}.`,
      llm,
    );
    ...
    expect(result.ir.parseWarnings.some((w) => w.includes("LLM assistance"))).toBe(
      true,
    );
  });
  ...
  it("falls back to deterministic when the LLM returns null", async () => {
    ...
    expect(
      rule.warnings.some((w) =>
        w.toLowerCase().includes("llm fallback returned no result"),
      ),
    ).toBe(true);
  });
...
describe("mergeWithLLMResult", () => {
  ...
  it("never decreases confidence below the deterministic value", () => {
    const merged = mergeWithLLMResult(
      { ...base, confidence: 0.6 },
      { confidence: 0.2 },
    );
