...
import type { Policy, PolicyCondition, PolicyEffect, PolicyRule } from './types.js';
...
const ACTION_PATTERN =
  /\b(payout|payment|transfer|approval|action|transaction|request|change|modification|deletion|export|write|read|access|dispatch|execution|deployment)\b/gi;
...
function sentenceToRule(sentence: string, index: number): CompiledRuleIR {
  ...
  if (amount !== null) {
    ...
    if (isAbove) {
      ...
      warnings.push(
        `Ambiguous amount comparison — defaulted to ≥ $${amount.toLocaleString()}. Use "over" or "under" to be explicit.`,
      );
      confidence -= 0.1;
    }
  }
  ...
  const isBlock =
    /\b(block|deny|prohibit|forbidden|forbid|prevent|reject|disallow|no\s+\w+\s+(over|above|without))\b/.test(
      s,
    );
  ...
  const isRequireApproval =
    /\b(require[s]?\s+(approval|sign-off|sign\s*off|review|approver)|need[s]?\s+(approval|sign-off|sign\s*off)|without\s+(approval|sign-off))\b/.test(
      s,
    );
  const isAllow = /\b(allow[s]?|permit[s]?|approve[s]? automatically|auto-approve|allowed)\b/.test(
    s,
  );
  ...
  if (isBlock && !isRequireApproval) {
    ...
    if (roles.length > 0) {
      ...
      warnings.push(
        "No specific approver role identified — defaulted to 'approver'. Consider specifying a role (e.g. 'finance officer').",
      );
      confidence -= 0.05;
    }
    ...
    warnings.push(
      "Effect could not be determined with high confidence — defaulted to 'require_approval'.",
    );
    confidence -= 0.15;
  }
  ...
  if (conditions.length === 0 && effect !== 'allow' && effect !== 'audit_only') {
    warnings.push(
      'No specific conditions parsed — this rule will match ALL actions. Refine your rule for precision.',
    );
    confidence -= 0.1;
  }
...
function buildCompilerResult(
  ...
  const overallConfidence =
    ruleIRs.length === 0 ? 0 : ruleIRs.reduce((sum, r) => sum + r.confidence, 0) / ruleIRs.length;

  if (overallConfidence < 0.6) {
    parseWarnings.push(
      'Low parse confidence. Consider rewording rules using action-threshold-approver patterns.',
    );
  }
...
export function diffPolicies(prev: Policy | null, next: Policy): PolicyDiffLine[] {
