...
describe('evaluatePolicies — scope filtering', () => {
  it('tenant-scoped policy only matches correct tenant', () => {
    ...
    const matchingTenantResult = evaluatePolicies(
      [policy],
      makeRequest({ tenantId: 'tenant-xyz' }),
    );
...
describe('buildPolicyEvaluation — required metadata fields', () => {
  ...
  it('blockedReason is set when confidence is below auto-within-guardrails threshold', () => {
    ...
    const pe = buildPolicyEvaluation({
      ...
      projectedRisk: 'Low',
      evidenceChain: [{ source: 'db', summary: 'Cost data', confidence: 0.7, freshness: 0.9 }],
    });
  ...
  it('blockedReason is set in observe mode', () => {
    ...
    const pe = buildPolicyEvaluation({
      ...
      evidenceChain: [
        { source: 'stream', summary: 'Signal data', confidence: 0.99, freshness: 1.0 },
      ],
      confidence: 0.99,
      freshnessScore: 1.0,
  ...
  it('blockedReason is set when windowValid=false', () => {
    const pe = buildPolicyEvaluation({
      ...
      windowValid: false,
      evidenceChain: [
        { source: 'calendar', summary: 'Outside window', confidence: 1.0, freshness: 1.0 },
      ],
      confidence: 1.0,
      freshnessScore: 1.0,
