...
function evaluateCondition(condition: PolicyCondition, context: Record<string, unknown>): boolean {
  ...
  switch (condition.operator) {
    ...
    case 'matches':
      return typeof actualValue === 'string' && new RegExp(String(expectedValue)).test(actualValue);
    default:
      return false;
  }
}
...
function policyAppliesToRequest(policy: Policy, request: EvaluationRequest): boolean {
  ...
  if (policy.scope === 'domain' || policy.domain) {
    if (
      ...
      policy.domain !== request.resource.domain
    ) {
      return false;
    }
  }
  ...
  if (policy.scope === 'action' && policy.actionTypes) {
    if (
      !policy.actionTypes.includes(request.action) &&
      !policy.actionTypes.includes(request.actionClass ?? '')
    ) {
      return false;
    }
  }
...
function buildReasoning(
  ...
  const lines: string[] = [
    `Evaluated ${policiesEvaluated} applicable policies, matched ${matchCount} rules.`,
  ];
  if (effect === 'block') {
