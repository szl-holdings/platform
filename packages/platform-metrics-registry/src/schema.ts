...
export const StructuralFactsSchema = z.object({
  artifactCount: z.number().int().nonnegative().describe("Total registered artifacts (active + archived)"),
  activeArtifactCount: z.number().int().nonnegative().describe("Artifacts with a registered workflow"),
...
export const SchemaFactsSchema = z.object({
  dbTableCount: z.number().int().nonnegative().describe("Total database table definitions (Drizzle pgTable calls)"),
...
export const RuntimeFactsSchema = z.object({
  domainPackCount: z.number().int().nonnegative().describe("Domain pack count (Lyte/Vessels/Terra/Aegis/PRISM/Carlota)"),
  ...
  embeddingBackendCount: z.number().int().nonnegative().describe("Embedding backend implementations"),
  memoryTierCount: z.number().int().nonnegative().describe("Memory tier count (working/episodic/semantic/governance)"),
});
