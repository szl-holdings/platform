...
import { z } from 'zod';
import { aiOpsMetricSchema, aiTraceSchema, llmStructuredOutputSchema, toolCallSchema } from './ai';
...
describe('aiTraceSchema', () => {
  ...
  it('rejects negative tokens', () => {
    expect(() => aiTraceSchema.parse({ ...valid, promptTokens: -1 })).toThrow();
  });
  it('rejects negative latency / cost', () => {
    expect(() => aiTraceSchema.parse({ ...valid, latencyMs: -1 })).toThrow();
    expect(() => aiTraceSchema.parse({ ...valid, costEstimateUsd: -0.01 })).toThrow();
  });
  it('rejects confidence > 1', () => {
    expect(() => aiTraceSchema.parse({ ...valid, confidence: 1.1 })).toThrow();
  });
  it('rejects unknown routeClass', () => {
    expect(() => aiTraceSchema.parse({ ...valid, routeClass: 'premium' })).toThrow();
  });
  it('rejects unknown riskLevel', () => {
    expect(() => aiTraceSchema.parse({ ...valid, riskLevel: 'extreme' })).toThrow();
  });
...
describe('toolCallSchema', () => {
  ...
  it('rejects negative durationMs', () => {
    expect(() => toolCallSchema.parse({ ...valid, durationMs: -1 })).toThrow();
  });
...
describe('llmStructuredOutputSchema', () => {
  ...
  it('rejects confidence > 1', () => {
    expect(() => schema.parse({ content: { answer: 'x' }, confidence: 1.5 })).toThrow();
  });
...
describe('aiOpsMetricSchema', () => {
  ...
  it('rejects reviewRate > 1', () => {
    expect(() => aiOpsMetricSchema.parse({ ...valid, reviewRate: 1.1 })).toThrow();
  });
  it('rejects negative totals', () => {
    expect(() => aiOpsMetricSchema.parse({ ...valid, totalTraces: -1 })).toThrow();
    expect(() => aiOpsMetricSchema.parse({ ...valid, totalCostUsd: -0.01 })).toThrow();
  });
  it('rejects evalPassRate outside [0,1]', () => {
    expect(() => aiOpsMetricSchema.parse({ ...valid, evalPassRate: 1.5 })).toThrow();
  });
  it('rejects non-integer counts', () => {
    expect(() => aiOpsMetricSchema.parse({ ...valid, totalTraces: 1.5 })).toThrow();
  });
});
