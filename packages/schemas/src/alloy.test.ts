...
describe("workflowStepResultSchema", () => {
  ...
  it("rejects unknown status", () => {
    expect(() =>
      workflowStepResultSchema.parse({ ...valid, status: "done" }),
    ).toThrow();
  });
  it("rejects negative durationMs", () => {
    expect(() =>
      workflowStepResultSchema.parse({ ...valid, durationMs: -1 }),
    ).toThrow();
  });
  it("coerces dates", () => {
...
describe("agentDecisionSchema", () => {
  ...
  it("rejects confidence outside [0,1]", () => {
    expect(() =>
      agentDecisionSchema.parse({ ...valid, confidence: 1.5 }),
    ).toThrow();
  });
  it("rejects unknown riskLevel", () => {
    expect(() =>
      agentDecisionSchema.parse({ ...valid, riskLevel: "extreme" }),
    ).toThrow();
  });
...
describe("approvalSchema", () => {
  ...
  it("accepts a null approvedBy", () => {
    expect(
      approvalSchema.parse({ ...valid, approvedBy: null }).approvedBy,
    ).toBeNull();
  });
  it("rejects unknown status", () => {
...
describe("jobQueuePayloadSchema", () => {
  ...
  it("rejects negative attempts", () => {
    expect(() =>
      jobQueuePayloadSchema.parse({ ...valid, attempts: -1 }),
    ).toThrow();
  });
  it("rejects maxAttempts <= 0", () => {
    expect(() =>
      jobQueuePayloadSchema.parse({ ...valid, maxAttempts: 0 }),
    ).toThrow();
  });
  it("rejects unknown priority", () => {
    expect(() =>
      jobQueuePayloadSchema.parse({ ...valid, priority: "yesterday" }),
    ).toThrow();
  });
