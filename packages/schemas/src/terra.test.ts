...
describe('propertySchema', () => {
  ...
  it('rejects distressScore outside [0,100]', () => {
    expect(() => propertySchema.parse({ ...valid, distressScore: 101 })).toThrow();
    expect(() => propertySchema.parse({ ...valid, distressScore: -1 })).toThrow();
  });
...
describe('distressSignalSchema', () => {
  ...
  it('rejects negative liabilityAmount', () => {
    expect(() => distressSignalSchema.parse({ ...valid, liabilityAmount: -1 })).toThrow();
  });
  it('rejects notes > 2048 chars', () => {
    expect(() => distressSignalSchema.parse({ ...valid, notes: 'x'.repeat(2049) })).toThrow();
  });
});
...
describe('leadScoreSchema', () => {
  ...
  it('rejects confidence outside [0,1]', () => {
    expect(() => leadScoreSchema.parse({ ...valid, confidence: 1.1 })).toThrow();
  });
});
