...
describe("threatSchema", () => {
  ...
  it("accepts a null resolvedAt", () => {
    expect(
      threatSchema.parse({ ...valid, resolvedAt: null }).resolvedAt,
    ).toBeNull();
  });
});
...
describe("incidentSchema", () => {
  ...
  it("rejects unknown status", () => {
    expect(() =>
      incidentSchema.parse({ ...valid, status: "archived" }),
    ).toThrow();
  });
});
...
describe("vulnerabilitySchema", () => {
  ...
  it("accepts a valid CVE id", () => {
    expect(
      vulnerabilitySchema.parse({ ...valid, cveId: "CVE-2024-12345" }).cveId,
    ).toBe("CVE-2024-12345");
  });
  it("rejects malformed CVE id", () => {
    expect(() =>
      vulnerabilitySchema.parse({ ...valid, cveId: "CVE-24-1" }),
    ).toThrow();
  });
  it("rejects cvssScore outside [0,10]", () => {
    expect(() =>
      vulnerabilitySchema.parse({ ...valid, cvssScore: 11 }),
    ).toThrow();
    expect(() =>
      vulnerabilitySchema.parse({ ...valid, cvssScore: -1 }),
    ).toThrow();
  });
  it("rejects unknown status", () => {
    expect(() =>
      vulnerabilitySchema.parse({ ...valid, status: "deferred" }),
    ).toThrow();
  });
});
...
describe("playbookRunSchema", () => {
  ...
  it("accepts null completedAt", () => {
    expect(
      playbookRunSchema.parse({ ...valid, completedAt: null }).completedAt,
    ).toBeNull();
  });
  it("rejects unknown status", () => {
    expect(() =>
      playbookRunSchema.parse({ ...valid, status: "stopped" }),
    ).toThrow();
  });
});
