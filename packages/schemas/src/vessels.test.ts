...
describe('vesselSchema', () => {
  ...
  it('rejects buildYear < 1800', () => {
    expect(() => vesselSchema.parse({ ...valid, buildYear: 1799 })).toThrow();
  });
...
describe('vesselPositionSchema', () => {
  ...
  it('rejects lon < -180', () => {
    expect(() => vesselPositionSchema.parse({ ...valid, lon: -181 })).toThrow();
  });
  it('rejects heading > 360', () => {
    expect(() => vesselPositionSchema.parse({ ...valid, heading: 361 })).toThrow();
  });
  it('rejects negative speed', () => {
    expect(() => vesselPositionSchema.parse({ ...valid, speed: -1 })).toThrow();
  });
  it('rejects unknown source', () => {
    expect(() => vesselPositionSchema.parse({ ...valid, source: 'psychic' })).toThrow();
  });
});
...
describe('anomalySchema', () => {
  ...
  it('rejects unknown severity', () => {
    expect(() => anomalySchema.parse({ ...valid, severity: 'scary' })).toThrow();
  });
});
...
describe('sanctionsCheckSchema', () => {
  ...
  it('rejects unknown status', () => {
    expect(() => sanctionsCheckSchema.parse({ ...valid, status: 'yellow' })).toThrow();
  });
