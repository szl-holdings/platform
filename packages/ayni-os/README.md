...
Ayni-OS: reciprocity ledger, checkpoint/rewind, Tinkuy (Kuramoto) flow, replay API.
...
## Honesty note
Event-sourced rewind is replay of an append-only ledger — NOT time travel.

## Install (editable, from monorepo root)
```bash
pip install -e packages/ayni-os
```
