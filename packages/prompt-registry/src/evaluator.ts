...
class PromptEvaluator {
  ...
  createSuite(params: Omit<EvalSuite, "id" | "createdAt"> & { id?: string }): EvalSuite {
    ...
    logger.info({ suiteId: suite.id, promptId: suite.promptId, caseCount: suite.cases.length }, "Eval suite created");
    return suite;
  }
  ...
  async run(
    ...
    versionId: string,
    suiteId: string,
    executor: (prompt: string, input: Record<string, unknown>) => Promise<{ output: string; latencyMs: number }>,
  ): Promise<EvalReport> {
    ...
    for (const evalCase of suite.cases) {
      ...
      try {
        ...
        results.push({ caseId: evalCase.id, passed: false, score: 0, latencyMs, failureReason: err instanceof Error ? err.message : "Execution error" });
      }
    }
  ...
  compare(promptId: string, baseVersionId: string, candidateVersionId: string): VersionComparison {
    ...
    if (base && candidate) {
      ...
      return { promptId, baseVersionId, candidateVersionId, baseEval: base, candidateEval: candidate, scoreImprovement, passRateImprovement, latencyDeltaMs: latencyDelta, recommendation, reason };
    }

    return { promptId, baseVersionId, candidateVersionId, baseEval: base, candidateEval: candidate, recommendation, reason };
  }
  ...
  private scoreOutput(output: string, evalCase: EvalCase): { passed: boolean; score: number; failureReason?: string } {
    let score = 1.0;
    ...
    if (evalCase.expectJson) {
      try {
        ...
        if (evalCase.expectedJsonSchema) {
          const missingFields = Object.keys(evalCase.expectedJsonSchema).filter(k => !(k in parsed));
