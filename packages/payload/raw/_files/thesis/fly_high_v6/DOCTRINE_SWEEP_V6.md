...
| 4 | `Pillpintu` | Case-insensitive | None |
| 5 | `Khipu` | Case-insensitive | EXCEPT when citing Anthropic's "Claude Khipu Preview" as a third-party model name |
...
| `Pillpintu` | None | — | N/A | None required |
| `Khipu` | None | — | N/A | None required |
...
| `Pillpintu` | Line 384 | `\| \`Pillpintu\` \| NOT PRESENT \|` | **LEGITIMATE CITATION** — same doctrine sweep table | None required |
| `Khipu` | Line 385 | `\| \`Khipu\` \| NOT PRESENT \|` | **LEGITIMATE CITATION** — same doctrine sweep table | None required |
...
| `Pillpintu` | Line 675 | Same line as above | **LEGITIMATE CITATION** — same doctrine sweep table | None required |
| `Khipu` | Line 675 | Same line as above | **LEGITIMATE CITATION** — same doctrine sweep table | None required |
...
| `Pillpintu` | Lines 199, 794 | Same contexts as above | **LEGITIMATE CITATION** — same doctrineCheck code array and sweep report | None required |
| `Khipu` | Lines 37, 794 | Line 37: `≥16 h (ceiling hit by Claude Khipu Preview per [METR TH1.1](...))` — third-party model citation in data table; Line 794: sweep report noting it is a third-party model designation | **LEGITIMATE CITATION** — line 37 is exactly the permitted exception: citing Anthropic's "Claude Khipu Preview" as a third-party model name sourced from METR's published leaderboard. Line 794 explicitly explains the exception | None required |
...
- **Line 37** — `Claude Khipu Preview` is the name Anthropic assigned to its own model; citing it as a data point from METR's published time-horizon leaderboard falls exactly within the specified exception: "EXCEPT when citing Anthropic's 'Claude Khipu Preview' as a third-party model name."
...
**Real Violations Before:** 0  
**Real Violations After:** 0  
**Legit Citations:** 12 (8 patterns × 2 locations for most; Khipu × 1 third-party citation; anonymous × 1 test description)  
**Byline Status:** PASS  
**File Status:** ✅ CLEAN
...
| `Pillpintu` | Line 528 | Same table | **LEGITIMATE CITATION** — same sweep table | None required |
| `Khipu` | Line 529 | `\| \`Khipu\` \| NOT PRESENT as SZL artifact name \| PRESENT in 3 inputs as third-party citation only ("Claude Khipu Preview" — Anthropic's external model name cited from metr.org...)` | **LEGITIMATE CITATION** — explicitly identifies the permitted exception, confirming "Claude Khipu Preview" appears only as a third-party Anthropic model citation | None required |
...
**Classification Rationale:** Lines 521–535 are §8.1 "Doctrine Sweep" of the synthesis proposal. The table is an explicit doctrine compliance attestation enumerating each forbidden pattern and confirming its status. The `Khipu` row correctly applies the exception rule, noting it appears as "Claude Khipu Preview" (Anthropic's third-party model name from metr.org) and is permitted as a factual external citation.
...
| Third-party model name citation | 2 | phd_agi_forecast/operational_spec.md (lines 37, 794) | `Claude Khipu Preview` cited as Anthropic's model name from METR's published time-horizon leaderboard — exactly the permitted exception |
