...
This PR adds the three standard community health files that are currently absent from `szl-holdings/agi-forecast`, bringing it into parity with sibling repos (`ouroboros`, `sentra`, `amaru`).
...
- Closes the GitHub Community Standards checklist gaps for `agi-forecast`
...
Templates adapted from `szl-holdings/ouroboros` and `szl-holdings/sentra` equivalents, with `agi-forecast`-specific scope language in `SECURITY.md` (gauge provenance hashing, forecast receipt generation, Brier-score ledger integrity).
