...
async function verifyLivePublicSurface(
  ...
  try {
    ...
    const firstResponse = await requestSurface(approvedTarget.canonicalUrl, fetchSurface);
    ...
    if (approvedTarget.canonicalUrl !== approvedTarget.finalUrl) {
      ...
      response = await requestSurface(approvedTarget.finalUrl, fetchSurface);
