...
const VESSEL_SEEDS: VesselSeed[] = [
  {
    ...
    cargo: { cargoType: "Containerized goods (electronics, textiles)", quantity: "8200", unit: "TEU", origin: "Jebel Ali, AE", destination: "Rotterdam, NL", status: "in_transit" },
    route: { originPort: "Jebel Ali, AE", destinationPort: "Rotterdam, NL", distanceNm: "6420", status: "active",
      waypoints: [
  ...
  },
  {
    ...
    cargo: { cargoType: "Crude oil (WTI)", quantity: "1100000", unit: "BBL", origin: "Houston, US", destination: "Yokohama, JP", status: "in_transit" },
    route: { originPort: "Houston, US", destinationPort: "Yokohama, JP", distanceNm: "9870", status: "active",
      waypoints: [
  ...
  },
  {
    ...
    cargo: { cargoType: "Iron ore", quantity: "82000", unit: "MT", origin: "Port Hedland, AU", destination: "Qingdao, CN", status: "loading" },
    route: { originPort: "Port Hedland, AU", destinationPort: "Qingdao, CN", distanceNm: "3960", status: "planned",
      waypoints: [
  ...
  },
  {
    ...
    cargo: { cargoType: "Containerized goods (general cargo)", quantity: "11400", unit: "TEU", origin: "Algeciras, ES", destination: "New York, US", status: "in_transit" },
    route: { originPort: "Algeciras, ES", destinationPort: "New York, US", distanceNm: "3320", status: "active",
      waypoints: [
  ...
  },
  {
    ...
    cargo: { cargoType: "Forest products (sawn timber, pulp)", quantity: "9600", unit: "MT", origin: "Helsinki, FI", destination: "Lübeck, DE", status: "loading" },
    route: { originPort: "Helsinki, FI", destinationPort: "Lübeck, DE", distanceNm: "640", status: "planned",
      waypoints: [
  ...
  },
  {
    ...
    cargo: { cargoType: "Soybeans", quantity: "65000", unit: "MT", origin: "Santos, BR", destination: "Shanghai, CN", status: "in_transit" },
    route: { originPort: "Santos, BR", destinationPort: "Shanghai, CN", distanceNm: "11340", status: "active",
      waypoints: [
...
const PROPERTY_SEEDS: Array<Omit<InsertTerraProperty, "id">> = [
  {
    ...
    submarket: "Midtown South", propertyType: "office",
    sqft: 1240000, yearBuilt: 1969, floors: 40,
    latitude: "40.7536", longitude: "-73.9871",
    assessedValue: "612500000",
    zoning: "C6-4.5", ownerName: "Equity Office Properties REIT", ownerType: "reit",
    isActive: true, tags: ["trophy-asset", "transit-oriented", "class-a"],
  ...
  },
  {
    ...
    city: "Fort Worth", state: "TX", zipCode: "76137",
    submarket: "DFW North", propertyType: "industrial",
    sqft: 685000, yearBuilt: 2019, floors: 1,
    latitude: "32.8721", longitude: "-97.2856",
    assessedValue: "94800000",
    zoning: "I-2", ownerName: "Prologis Industrial REIT", ownerType: "reit",
    isActive: true, tags: ["bulk-distribution", "class-a", "ecommerce-grade"],
    rawData: { riskScore: 18, assetClass: "Bulk Distribution Industrial", capRate: 5.1, noi: 4834800 },
  },
  {
    externalId: "PROP-MIA-MFR-003",
    address: "888 Brickell Bay Dr",
    city: "Miami", state: "FL", zipCode: "33131",
    submarket: "Brickell", propertyType: "multifamily",
    sqft: 412000, yearBuilt: 2021, floors: 48, units: 360,
    latitude: "25.7617", longitude: "-80.1918",
    assessedValue: "248000000",
    zoning: "T6-48-O", ownerName: "Related Group Brickell Holdings LLC", ownerType: "llc",
    isActive: true, tags: ["luxury", "waterfront", "amenity-rich"],
    rawData: { riskScore: 34, assetClass: "Luxury High-Rise Multifamily", capRate: 4.7, noi: 11656000 },
  },
  {
    externalId: "PROP-BK-2026-0142",
    address: "842 Atlantic Ave",
    city: "Brooklyn", state: "NY", zipCode: "11238",
    submarket: "Crown Heights / Prospect Heights", propertyType: "mixed-use",
    sqft: 38400, yearBuilt: 1928, floors: 6, units: 24,
    latitude: "40.6816", longitude: "-73.9612",
    assessedValue: "1820000",
    zoning: "R7A/C2-4", ownerName: "Atlantic Avenue Holdings LLC", ownerType: "llc",
    isActive: true, tags: ["distress", "lis-pendens", "value-add"],
  ...
  },
  {
    ...
    submarket: "Downtown Denver", propertyType: "retail",
    sqft: 142000, yearBuilt: 2003, floors: 3,
    latitude: "39.7459", longitude: "-104.9959",
    assessedValue: "62400000",
    zoning: "D-C", ownerName: "Brookfield Properties Retail Trust", ownerType: "reit",
    isActive: true, tags: ["grocery-anchored", "transit-oriented"],
  ...
  },
  {
    ...
    city: "Phoenix", state: "AZ", zipCode: "85043",
    submarket: "Phoenix Goodyear Corridor", propertyType: "industrial",
    sqft: 285000, yearBuilt: 2023, floors: 2,
    latitude: "33.4319", longitude: "-112.1745",
    assessedValue: "1140000000",
    zoning: "A-1", ownerName: "Digital Realty Trust LP", ownerType: "reit",
    isActive: true, tags: ["data-center", "hyperscale", "power-rich"],
...
async function seedVessels(): Promise<{ inserted: number; skipped: number }> {
  ...
  const existing = imos.length
    ? await db.select({ id: vesselsTable.id, imo: vesselsTable.imo, orgId: vesselsTable.orgId }).from(vesselsTable).where(inArray(vesselsTable.imo, imos))
    : [];
  ...
  for (const seed of VESSEL_SEEDS) {
    ...
    if (prior) {
      ...
      if (prior.orgId !== SEED_ORG_ID) {
        await db.update(vesselsTable).set({ orgId: SEED_ORG_ID, updatedAt: new Date() }).where(eq(vesselsTable.id, vesselId));
      }
      skipped++;
      action = "backfill";
    } else {
      const [vessel] = await db.insert(vesselsTable).values({ ...seed.vessel, orgId: SEED_ORG_ID }).returning();
      vesselId = vessel.id;
      inserted++;
      action = "insert";
    }
    // Idempotently ensure each vessel has at least one position, cargo, and active route row
    const [pos] = await db.select({ id: vesselsPositionsTable.id }).from(vesselsPositionsTable).where(eq(vesselsPositionsTable.vesselId, vesselId)).limit(1);
    if (!pos) await db.insert(vesselsPositionsTable).values({ ...seed.position, vesselId });
    const [cargo] = await db.select({ id: vesselsCargoTable.id }).from(vesselsCargoTable).where(eq(vesselsCargoTable.vesselId, vesselId)).limit(1);
    if (!cargo) await db.insert(vesselsCargoTable).values({ ...seed.cargo, vesselId });
    const [route] = await db.select({ id: vesselsRoutesTable.id }).from(vesselsRoutesTable).where(eq(vesselsRoutesTable.vesselId, vesselId)).limit(1);
    if (!route) await db.insert(vesselsRoutesTable).values({ ...seed.route, vesselId });
    console.log(`  [${action}] vessel ${seed.vessel.imo} ${seed.vessel.name} (id: ${vesselId}, org: ${SEED_ORG_ID}) → ${seed.destination}`);
  }
...
async function seedProperties(): Promise<{ inserted: number; skipped: number }> {
  ...
  const existing = externals.length
    ? await db.select({ externalId: terraPropertiesTable.externalId }).from(terraPropertiesTable).where(inArray(terraPropertiesTable.externalId, externals))
    : [];
