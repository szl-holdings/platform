...
async function main(): Promise<void> {
  ...
  await step('Verify audit chain — confirm hybrid_verified > 0', async () => {
    ...
    const summary = (data as Record<string, unknown>).summary as { hybrid_verified: number; broken: number };
    if (!summary) throw new Error('No summary in verify response');
    if (summary.broken > 0) throw new Error(`Chain has ${summary.broken} broken entries`);
    ...
    console.log(`\n    hybrid_verified=${summary.hybrid_verified}, legacy_unsigned=${(data as Record<string, unknown>).summary && ((data as Record<string, unknown>).summary as Record<string, unknown>)['legacy_unsigned']}, broken=${summary.broken}`);
  });
