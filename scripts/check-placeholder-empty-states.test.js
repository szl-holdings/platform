...
import { findPlaceholderPhrase, runScan, scanSource } from './check-placeholder-empty-states.js';

describe('findPlaceholderPhrase', () => {
  it("flags JSX text node 'No data'", () => {
    expect(findPlaceholderPhrase('  <p className="x">No data</p>')).toBe('No data');
  });

  it("flags JSX text node 'No results found' with trailing copy", () => {
    expect(findPlaceholderPhrase('  <span>No results found — try again.</span>')).toBe(
      'No results',
    );
  });

  it("flags JSX text node 'No items found'", () => {
    expect(findPlaceholderPhrase('<div>No items found</div>')).toBe('No items found');
  });
  ...
  it('does NOT flag the phrase when it appears mid-sentence', () => {
    const line = 'summary: "Bot Manager 1.1 applied rate-limit. No data exfiltration risk.",';
...
describe('scanSource', () => {
  ...
  it('respects the inline `// empty-state-lint-allow:` marker', () => {
    ...
    expect(scanSource('artifacts/vessels/src/pages/fake.tsx', src)).toHaveLength(0);
  });

  it('flags multiple distinct violations', () => {
    const src = ['<p>No data</p>', '<span>No results found</span>', '<em>No items found</em>'].join(
      '\n',
    );
    const v = scanSource('artifacts/terra/src/pages/fake.tsx', src);
    expect(v.map((x) => x.match)).toEqual(['No data', 'No results', 'No items found']);
  });
});
