...
async function main(): Promise<void> {
  ...
  await runTest("POST /v1/embed returns vectors", async () => {
    ...
    const data = json as { vectors: Array<{ vector: number[]; index: number }>; dimensions: number; traceId: string };
  ...
  await runTest("POST /v1/hybrid-search returns hits with evidence", async () => {
    ...
    const data = json as {
      hits: Array<{ chunkId: string; finalScore: number; evidenceId: string; selectedRationale: string }>;
      traceId: string;
    };
    assert(status === 200, `Expected 200, got ${status}: ${JSON.stringify(json)}`);
