...
async function seedFirestorm() {
  ...
  const assets = await db.insert(firestormAssetsTable).values([
    {
      name: "Payment API v3",
      assetType: "service",
...
async function seedLyte() {
  ...
  const actions = await db.insert(lyteActionsTable).values([
    {
      title: "Northgate Contract — Legal Review Stalled",
      description: "Contract stuck in legal queue 48h past SLA. $840K ARR at risk if not executed by EOM.",
      ...
      metadata: { contractId: "NOR-2026-0312", dealStage: "legal_review", slaBreachedAt: "2026-03-28T09:00:00Z" },
    },
    {
      title: "TechCorp Onboarding — No Owner Assigned",
      description: "Critical onboarding step has been unassigned for 6 days. Customer escalation risk.",
    ...
    },
    {
      ...
      valueAtRisk: "180000",
      roleVisibility: { operations: true, delivery: true },
      metadata: { vendorIds: ["VEN-0045", "VEN-0078", "VEN-0092"], blockedStep: "compliance_review" },
    },
    {
      ...
      metadata: { customerId: "APEX-0156", handoffStep: "delivery_lead_confirmation", escalatedAt: "2026-03-29T14:00:00Z" },
    },
    {
      title: "Enterprise Deal Status Conflict — $1.2M",
      description: "CRM shows Closed Won but finance has not received PO. Revenue recognition at risk.",
      ...
      valueAtRisk: "1200000",
      roleVisibility: { executive: true, operations: true },
      metadata: { dealId: "ENT-2026-0889", crmStatus: "closed_won", financeStatus: "awaiting_po" },
    },
    {
      ...
      valueAtRisk: "450000",
      roleVisibility: { operations: true, delivery: true },
      metadata: { launchDate: "2026-04-15", gatesBlocked: ["security_review", "load_test", "legal_clearance"] },
    },
    {
      title: "Pipeline Hygiene — 47 Stale Opportunities",
      description: "Deals last touched >30 days consuming forecast capacity. Recommend close or disqualify.",
  ...
  await db.insert(lyteSavedViewsTable).values([
    { name: "Executive Dashboard", description: "High-value escalations for executive review", filters: { role: "executive", priority: "urgent" }, isDefault: true },
    { name: "Ops Queue — Today", description: "All open ops items due today", filters: { role: "operations", state: "new" }, isDefault: false },
    { name: "Delivery Blockers", description: "Readiness blockers and ownership gaps", filters: { role: "delivery", signalCategory: "readiness_blocker" }, isDefault: false },
  ]);
  ...
  await db.insert(lyteReadinessItemsTable).values([
    ...
    },
    {
      ...
      metadata: { complianceStandard: "PCI-DSS-v4", saqType: "SAQ-A", submissionUrl: "https://stripe.com/pci" },
    },
    {
  ...
  const events = await db.insert(vesselsEventsTable).values([
    {
      ...
      description: "Route delay due to Strait of Gibraltar congestion. Port slot at Rotterdam at risk.",
  ...
  await db.insert(vesselsCommandWorkflowsTable).values([
    {
      ...
      notes: "Escalated to director due to $420K margin impact. Alloy routing intervention triggered.",
    ...
    },
    {
      ...
      status: "in_progress",
      assignedTo: "Helmsman AI",
      notes: "Alternative route via north Atlantic corridor calculated. ETA penalty: 8h vs 34h typhoon delay.",
    ...
    },
    {
      ...
      notes: "Route deviation acknowledged. Monitoring fuel burn. No intervention required at this stage.",
