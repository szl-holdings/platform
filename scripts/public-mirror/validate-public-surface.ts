...
const MAX_SECRET_SCAN_FILES = 10_000;
...
function checkEnvFiles(): void {
  ...
  // Allow .env.example — it's the sanitized template intended for public consumption.
  // Block all other .env variants (secret material).
...
function findInternalDocsRecursive(rootDir: string): string[] {
  const found: string[] = [];
  const internalPattern = /(?:^|\/)docs\/internal(?:\/|$)/;
...
function checkInternalDocs(): void {
  ...
  const internalPaths = findInternalDocsRecursive(TARGET);
  if (internalPaths.length > 0) {
    addResult(
      'Internal docs (docs/internal/)',
      'ERROR',
      `Found at: ${internalPaths.slice(0, 3).join(', ')}`,
    );
  } else {
...
function checkReadmeContent(): void {
  ...
  const checks = [
    ...
    {
      label: 'Has architecture section',
      test: content.includes('Architecture') || content.includes('Stack'),
    },
    {
      label: 'Has contact information',
      test: content.includes('Contact') || content.includes('szlholdings.com'),
    },
    {
      label: 'Has trust/license notice',
      test: content.includes('License') || content.includes('Proprietary'),
    },
  ];
