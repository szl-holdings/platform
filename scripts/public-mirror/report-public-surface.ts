 ...
 *
 * NOTE: This script writes to docs/audit/public-surface-inventory.md.
 *       The validation report (docs/audit/public-mirror-report.md) is
 *       written exclusively by validate-public-surface.ts. The two files
 *       have distinct purposes and must not overwrite each other.
 *
 * Usage:
 ...
 *
 * Output:
 *   docs/audit/public-surface-inventory.md
 *   Console summary
 */
...
const REPORT_PATH = path.join(WORKSPACE_ROOT, 'docs/audit/public-surface-inventory.md');
...
const ROOT_DIR_CLASSIFICATIONS: Record<string, ContentClass> = {
  artifacts: {
    label: 'artifacts/',
    category: 'included',
    reason: 'Application source code — all 16 artifacts',
  },
  lib: { label: 'lib/', category: 'included', reason: 'Shared TypeScript libraries (8 packages)' },
  packages: {
    label: 'packages/',
    category: 'included',
    reason: 'Marketplace integration packages',
  },
  docs: {
    label: 'docs/',
    category: 'included',
    reason: 'Architecture, trust, investor, buyer, release docs',
  },
  infra: { label: 'infra/', category: 'included', reason: 'Azure Bicep IaC templates' },
  scripts: {
    label: 'scripts/',
    category: 'included',
    reason: 'Build, mirror, and automation scripts',
  },
  'profile-readme': {
    label: 'profile-readme/',
    category: 'included',
    reason: 'GitHub profile README content',
  },
  ops: { label: 'ops/', category: 'included', reason: 'GitHub operations and automation' },
  '.github': {
    label: '.github/',
    category: 'included',
    reason: 'PR templates, issue templates, CI workflows',
  },
  tests: { label: 'tests/', category: 'included', reason: 'Integration and E2E test suite' },
  '.archive': {
    label: '.archive/',
    category: 'excluded',
    reason: 'Historical/archived work — internal cleanup',
  },
  '.git-rewrite': {
    label: '.git-rewrite/',
    category: 'excluded',
    reason: 'Git history rewrite artifacts',
  },
  backups: {
    label: 'backups/',
    category: 'excluded',
    reason: 'CRITICAL — database backups with SQL dumps',
  },
  exports: { label: 'exports/', category: 'excluded', reason: 'Internal export artifacts' },
  'test-results': {
    label: 'test-results/',
    category: 'excluded',
    reason: 'CI/test output — operational noise',
  },
  attached_assets: {
    label: 'attached_assets/',
    category: 'excluded',
    reason: 'Raw user-uploaded files — unsorted',
  },
  'social-content': {
    label: 'social-content/',
    category: 'excluded',
    reason: 'Draft social media content',
  },
  ...
  node_modules: {
    label: 'node_modules/',
    category: 'excluded',
    reason: 'Dependencies — install via pnpm install',
  },
...
const ROOT_FILE_CLASSIFICATIONS: Record<string, ContentClass> = {
  'README.md': {
    label: 'README.md',
    category: 'included',
    reason: 'Primary repo entry point — investor-grade',
  },
  'CHANGELOG.md': {
    label: 'CHANGELOG.md',
    category: 'included',
    reason: 'Version history and release discipline',
  },
  'SECURITY.md': {
    label: 'SECURITY.md',
    category: 'included',
    reason: 'Responsible disclosure policy',
  },
  'CONTRIBUTING.md': {
    label: 'CONTRIBUTING.md',
    category: 'included',
    reason: 'Engineering standards and culture',
  },
  'LICENSE.md': { label: 'LICENSE.md', category: 'included', reason: 'Proprietary license notice' },
  CODEOWNERS: {
    label: 'CODEOWNERS',
    category: 'included',
    reason: 'Code ownership and governance',
  },
  'package.json': { label: 'package.json', category: 'included', reason: 'Workspace root config' },
  'pnpm-workspace.yaml': {
    label: 'pnpm-workspace.yaml',
    category: 'included',
    reason: 'Monorepo package declarations',
  },
  'tsconfig.json': {
    label: 'tsconfig.json',
    category: 'included',
    reason: 'TypeScript project config',
  },
  'tsconfig.base.json': {
    label: 'tsconfig.base.json',
    category: 'included',
    reason: 'TypeScript shared base config',
  },
  'eslint.config.js': {
    label: 'eslint.config.js',
    category: 'included',
    reason: 'Lint configuration',
  },
  '.env.example': {
    label: '.env.example',
    category: 'included',
    reason: 'Sanitized environment variable template',
  },
  '.gitignore': { label: '.gitignore', category: 'included', reason: 'Git exclusion policy' },
  'playwright.config.ts': {
    label: 'playwright.config.ts',
    category: 'included',
    reason: 'E2E test config',
  },
  'vitest.config.ts': {
  ...
  'vitest.components.config.ts': {
    label: 'vitest.components.config.ts',
    category: 'included',
    reason: 'Component test config',
  },
  '.prettierrc.cjs': {
    label: '.prettierrc.cjs',
    category: 'included',
    reason: 'Code formatting config',
  },
  '.lighthouserc.json': {
    label: '.lighthouserc.json',
    category: 'included',
    reason: 'Lighthouse CI config',
  },
  'ROADMAP.md': {
    label: 'ROADMAP.md',
    category: 'noisy',
    reason: 'Redundant with CHANGELOG + docs/releases/ — quarantine from root',
  },
  'ECOSYSTEM_ROADMAP.md': {
    label: 'ECOSYSTEM_ROADMAP.md',
    category: 'noisy',
    reason: 'Redundant with docs/architecture/platform-map.md — quarantine from root',
  },
  'PUBLIC_RELEASE_NOTES.md': {
    label: 'PUBLIC_RELEASE_NOTES.md',
    category: 'noisy',
    reason: 'Superseded by docs/releases/ — quarantine from root',
  },
  'PUBLIC_REPO_AUDIT_REPORT.md': {
    label: 'PUBLIC_REPO_AUDIT_REPORT.md',
    category: 'noisy',
    reason: 'Superseded by docs/audit/ — quarantine from root',
  },
  ...
  'replit.nix': {
    label: 'replit.nix',
    category: 'excluded',
    reason: 'Replit NixOS config — internal',
  },
  'replit.md': {
    label: 'replit.md',
    category: 'excluded',
    reason: 'Replit agent memory — internal',
  },
  '.watchmanconfig': {
    label: '.watchmanconfig',
    category: 'excluded',
    reason: 'Watchman file watcher config — internal',
  },
  '.npmrc': { label: '.npmrc', category: 'excluded', reason: 'npm config — Replit-specific' },
  'pnpm-lock.yaml': {
    label: 'pnpm-lock.yaml',
    category: 'included',
    reason: 'Lock file for reproducible installs',
  },
};
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      ...
      if (cls) {
        switch (cls.category) {
          case 'included':
            inv.includedDirs.push(entry.name);
            break;
          case 'excluded':
            inv.excludedDirs.push(entry.name);
            break;
          case 'noisy':
            inv.noisyDirs.push(entry.name);
            break;
          case 'flagged':
            inv.flaggedDirs.push(entry.name);
            break;
        }
      } else {
      ...
      if (cls) {
        switch (cls.category) {
          case 'included':
            inv.includedFiles.push(entry.name);
            break;
          case 'excluded':
            inv.excludedFiles.push(entry.name);
            break;
          case 'noisy':
            inv.noisyFiles.push(entry.name);
            break;
          case 'flagged':
            inv.flaggedDirs.push(entry.name);
            break;
...
function writeReport(inv: SurfaceInventory): void {
  ...
  lines.push('', '## Included Root Files', '', '| File | Purpose |', '|------|---------|');
  ...
  if (inv.noisyFiles.includes('ROADMAP.md'))
    actions.push('- Move `ROADMAP.md` to `.archive/root-cleanup/`');
  if (inv.noisyFiles.includes('ECOSYSTEM_ROADMAP.md'))
    actions.push('- Move `ECOSYSTEM_ROADMAP.md` to `.archive/root-cleanup/`');
  if (inv.noisyFiles.includes('PUBLIC_RELEASE_NOTES.md'))
    actions.push('- Move `PUBLIC_RELEASE_NOTES.md` to `.archive/root-cleanup/`');
  if (inv.noisyFiles.includes('PUBLIC_REPO_AUDIT_REPORT.md'))
    actions.push('- Move `PUBLIC_REPO_AUDIT_REPORT.md` to `.archive/root-cleanup/`');
  if (inv.noisyFiles.includes('LICENSE'))
    actions.push('- Remove duplicate `LICENSE` file (keep `LICENSE.md`)');
...
function main(): void {
  ...
  log(
    `Root directories: ${inv.totalDirs} (${inv.includedDirs.length} included, ${inv.excludedDirs.length} excluded, ${inv.noisyDirs.length} noisy, ${inv.unknownDirs.length} unknown)`,
  );
  log(
    `Root files: ${inv.totalFiles} (${inv.includedFiles.length} included, ${inv.excludedFiles.length} excluded, ${inv.noisyFiles.length} noisy)`,
  );
