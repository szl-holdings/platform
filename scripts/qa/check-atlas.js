...
async function main() {
  ...
  for (const expected of EXPECTED_SCENES) {
    ...
    if (row.domain !== expected.domain) {
      console.error(`  [FAIL] ${expected.slug}: domain mismatch. Expected ${expected.domain}, got ${row.domain}`);
      failed++;
      continue;
    }

    if (row.entity_type !== expected.entityType) {
      console.error(`  [FAIL] ${expected.slug}: entityType mismatch. Expected ${expected.entityType}, got ${row.entity_type}`);
      failed++;
      continue;
    }
    ...
    if (sections.length < expected.minSections) {
      console.error(`  [FAIL] ${expected.slug}: has ${sections.length} sections, expected at least ${expected.minSections}`);
      failed++;
      continue;
    }
    ...
    if (!metadata.demo) {
      console.error(`  [WARN] ${expected.slug}: metadata.demo is not set — may not be labelled as demo data`);
    }
