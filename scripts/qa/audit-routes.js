...
const APP_CONFIGS = [
  {
    ...
    pagesDir: "pages",
    knownRoutes: [
      "landing", "platform", "design-partners", "contact", "trust-center",
      "trust", "trust-security", "trust-governance", "trust-architecture",
      "trust-ai", "trust-approvals", "trust-operations", "ventures", "portfolio",
      "how-it-works", "solutions", "solutions-aegis", "solutions-vessels",
      "solutions-terra", "solutions-prism-counsel", "lyte-page", "alloy-page",
      "docs", "status", "legal-privacy", "legal-terms", "accessibility",
      "not-found", "founder", "company",
    ],
  },
  {
    ...
    pagesDir: "pages",
    knownRoutes: [
      "dashboard", "signals-page", "actions-page", "readiness-page",
      "approvals-center", "ownership-map-new", "escalation-center",
      "command-inbox", "action-queue", "operational-queue",
      "demo-dashboard", "demo-signals", "demo-priorities", "demo-workflows",
      "demo-recommendations", "demo-audit", "demo-exceptions", "demo-readiness",
      "demo-integrations", "demo-reports", "demo-settings", "demo-alerts",
    ],
    subDirRoutes: [
      { subDir: "admin", routes: ["overview", "users", "feature-flags", "audit-log", "jobs", "run-viewer", "approval-queue", "seeder", "export-history", "diagnostics", "billing-admin", "platform-health", "connectors", "workflows"] },
    ],
  },
  {
    name: "API Server",
    ...
    pagesDir: "routes",
    knownRoutes: [
      "health", "core", "admin", "auth", "alloy", "lyte", "vessels",
      "firestorm", "terra", "billing", "notifications", "connectors",
      "analytics", "jobs", "webhooks", "exports", "feedback",
    ],
  },
];
...
function main() {
  ...
  for (const config of APP_CONFIGS) {
    ...
    console.log(`    → ${config.knownRoutes.length + (config.subDirRoutes?.reduce((a, b) => a + b.routes.length, 0) ?? 0)} routes checked`);
  }
