...
const VALID_ADVISORY_SEVERITIES = new Set(['critical', 'high', 'moderate', 'low']);
...
export function validateAuditJson(auditJson) {
  ...
  if (
    !Number.isSafeInteger(auditJson.metadata.totalDependencies) ||
    auditJson.metadata.totalDependencies < 0
  ) {
  ...
  for (const [advisoryId, advisory] of Object.entries(auditJson.advisories)) {
    if (!isPlainRecord(advisory)) {
      throw new Error(`pnpm audit JSON advisory ${advisoryId} must be an object`);
    }
    if (
      typeof advisory.severity !== 'string' ||
      !VALID_ADVISORY_SEVERITIES.has(advisory.severity)
    ) {
      throw new Error(`pnpm audit JSON advisory ${advisoryId} has an invalid severity`);
    }
  }
}
