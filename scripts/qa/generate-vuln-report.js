...
async function main() {
  ...
  try {
    ...
    rawOutput = execSync("pnpm audit --json", { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
  } catch (err) {
  ...
  console.log(`[vuln-report] Scan complete: ${total} total findings (critical=${critical} high=${high} moderate=${moderate} low=${low})`);
  ...
  if (critHighAdvs.length === 0) {
    ...
    for (const adv of critHighAdvs) {
      ...
      const cve = adv.cves && adv.cves.length > 0
        ? adv.cves.join(", ")
        : (adv.url ? `[${adv.id}](${adv.url})` : String(adv.id ?? "N/A"));
      const sev = (adv.severity ?? "unknown").toUpperCase();
      const range = adv.vulnerable_versions ?? "unknown";
      const paths = (adv.findings ?? []).flatMap(f => f.paths ?? []).slice(0, 2).join("; ") || "direct";
  ...
  try {
    ...
    console.warn(`[vuln-report] WARN: Could not read pnpm.overrides from package.json: ${e.message}`);
  }

  const overrideEntries = Object.entries(overrides);
  if (overrideEntries.length > 0) {
    report += "## Overrides Active\n\n";
    report += "The following `pnpm.overrides` are pinned in `package.json` to resolve known transitive vulnerabilities or ensure version consistency:\n\n";
  ...
  report += "_Auto-generated by `scripts/qa/generate-vuln-report.js`. CI blocks on any Critical or High finding._\n";
