...
if (!JSON_MODE) {
  if (nonLaunchable > 0) {
    const detail = [broken.length > 0 ? `${broken.length} broken` : null, mocked.length > 0 ? `${mocked.length} mock/demo` : null].filter(Boolean).join(", ");
    console.log(`⚠️  ${nonLaunchable} non-launchable claim(s) found (${detail}). ${STRICT ? "Exiting 1 (--strict mode)." : "Run with --strict to fail CI."}`);
  } else {
