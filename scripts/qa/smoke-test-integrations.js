...
const REQUIRED = ["stripe", "sentry_server", "posthog_frontend", "amplitude_frontend", "google_maps"];
...
async function checkEnvIntegrations() {
  return {
    ...
    sentry_frontend: {
      ...
      detail: process.env.VITE_SENTRY_DSN ? "VITE_SENTRY_DSN configured" : "VITE_SENTRY_DSN not set",
      warnOnly: true,
    },
    posthog_server: {
      name: "PostHog (server)",
      ...
      detail: process.env.POSTHOG_API_KEY ? "POSTHOG_API_KEY configured" : "POSTHOG_API_KEY not set",
      warnOnly: true,
    },
    posthog_frontend: {
      ...
      detail: process.env.VITE_POSTHOG_KEY ? "VITE_POSTHOG_KEY configured" : "VITE_POSTHOG_KEY not set",
    },
    amplitude_frontend: {
      name: "Amplitude (frontend)",
      key: "amplitude_frontend",
      ok: !!process.env.VITE_AMPLITUDE_API_KEY,
      detail: process.env.VITE_AMPLITUDE_API_KEY ? "VITE_AMPLITUDE_API_KEY configured" : "VITE_AMPLITUDE_API_KEY not set",
    },
    google_maps: {
      name: "Google Maps",
      key: "google_maps",
    ...
    mapbox: {
      ...
      detail: process.env.VITE_MAPBOX_TOKEN ? "VITE_MAPBOX_TOKEN configured" : "VITE_MAPBOX_TOKEN not set",
      warnOnly: true,
    },
  };
}
...
async function probeGoogleMapsProxy(apiBase) {
  ...
  try {
    const gmRes = await fetch(
      `${apiBase}/api/maps/geocode?address=425+Park+Ave+New+York+NY`,
...
async function probeStripeCheckoutE2E() {
  ...
  try {
    ...
    const session = await client.checkout.sessions.create({
      ...
      mode: "payment",
      line_items: [{ price_data: { currency: "usd", product_data: { name: "Smoke Test Item" }, unit_amount: 100 }, quantity: 1 }],
      success_url: "https://example.com/success",
      cancel_url: "https://example.com/cancel",
    });
    if (session.id && session.url) {
      console.log(`✅ Stripe checkout session created — id: ${session.id.slice(0, 20)}...`);
      console.log(`   mode: ${process.env.STRIPE_SECRET_KEY.startsWith("sk_live") ? "LIVE" : "test"}, url: ${session.url.slice(0, 60)}...`);
    } else {
