  ...
  if (res.status === 200) {
    ...
    try {
      ...
      if (status === "ok") {
        ...
        recordSev1("health:status", `Health endpoint reports degraded status: ${JSON.stringify(healthData?.services ?? {})}`);
      } else {
...
try {
  ...
  if (res.status === 200) {
    ...
    info(`/api/health/ready — HTTP 404 (no dedicated readiness probe; using /api/health DB status)`);
    if (healthReachable && healthData?.services?.database) {
      ...
      recordSkip("health:ready", "/api/health/ready not implemented; DB status unavailable from /api/health");
    }
  } else {
...
try {
  ...
  if (res.status >= 500) {
    ...
    recordSev1("auth:user-missing", "/api/auth/user returned 404 — auth endpoint must be registered");
...
for (const { path, description } of PROTECTED_ROUTES_TO_PROBE) {
  try {
    ...
    if (res.status === 401 || res.status === 403) {
      pass(`${path} — HTTP ${res.status} (auth guard confirmed — unauthenticated request rejected)`);
      authGuardVerified = true;
      break;
    } else if (res.status === 200) {
      fail(`${path} — HTTP 200 without credentials (auth guard BYPASSED!)`);
      recordSev0("auth:guard-bypassed", `Protected route ${path} (${description}) returned 200 without credentials — auth middleware is broken`);
      break;
...
if (!authGuardVerified && results.sev0.filter(r => r.name === "auth:guard-bypassed").length === 0) {
  warn("Auth guard unverifiable — known protected routes all returned non-200/non-401 (API server may not be running)");
  recordSev2("auth:guard-unverifiable", "Could not confirm auth guard enforcement — API server may be offline");
}
...
try {
  ...
  if (res.status >= 500) {
    ...
    recordSev1("auth:login-missing", "/api/login returned 404 — OIDC login entry point must be registered");
...
const TRUST_ROUTES = [
  ...
  { path: "/api/health/integrations", description: "Integration health snapshot (public, from health-integrations.ts)" },
...
if (healthData?.services) {
  ...
  if (!db) {
    ...
    recordSev2("health:db-check-depth", "DB health check lacks latency measurement — verify it performs a real query");
  } else {
    pass(`Health endpoint reports database status: ${db.status} (latency: ${db.latencyMs ?? "N/A"}ms)`);
  }
...
if (totalFailed === 0 && totalWarnings === 0) {
  ...
  console.log(`${COLORS.yellow}${COLORS.bold}⚠ No blocking failures. ${totalWarnings} warning(s) to address.${COLORS.reset}`);
} else {
  console.log(`${COLORS.red}${COLORS.bold}✗ ${totalFailed} blocking failure(s). Platform not ready for release.${COLORS.reset}`);
  if (results.sev0.length > 0) {
    console.log(`${COLORS.red}  Sev 0 failures require immediate resolution before any deployment.${COLORS.reset}`);
  }
}
