...
function main() {
  ...
  for (const [dep, entries] of depVersionMap) {
    ...
    if (versions.size > 1) {
      ...
      const nonCatalog = [...versions].filter((v) => !v.startsWith("catalog:") && !v.startsWith("workspace:"));
  ...
  const SHOULD_USE_CATALOG = ["react", "react-dom", "@types/react", "@types/react-dom", "typescript", "tsx"];
  const catalogViolations = [];

  for (const pkg of packages) {
    for (const [dep, version] of Object.entries(pkg.deps)) {
      if (SHOULD_USE_CATALOG.includes(dep) && !version.startsWith("catalog:") && !version.startsWith("workspace:")) {
  ...
  if (totalIssues === 0) {
    ...
    console.log(`WARN — ${totalIssues} issue(s) found. Review above and harmonize via pnpm-workspace catalog.`);
