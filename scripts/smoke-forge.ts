...
async function main() {
  ...
  T(
    '2. aggregateDrift returns critical for high+critical',
  ...
  T(
    ...
    `score=${driftReport.driftScore} sev=${driftReport.severity}`,
  );
  ...
  const promoVal = await validatePromotion({
    agentId: matter.id,
    toVersionId: matter.activeVersionId!,
    fromEnv: 'sandbox',
    toEnv: 'staging',
  ...
  T(
    '4. promotion validator includes drift score in report',
    driftReported !== undefined && driftReported.score > 0,
    `drift=${JSON.stringify(driftReported)} blockers=${promoVal.blockers.map((b) => b.code).join(',')}`,
  );

  // ── 5. promotion validator covers all blocker codes individually
  // 5a) INVALID_TIER_TRANSITION
  const invalid = await validatePromotion({
    agentId: briefer.id,
    toVersionId: briefer.activeVersionId!,
    fromEnv: 'dev',
    toEnv: 'production',
    hasHumanApproval: true,
  });
  T(
    '5a. INVALID_TIER_TRANSITION emitted',
    invalid.blockers.some((b) => b.code === PROMOTION_BLOCKER_CODES.INVALID_TIER_TRANSITION),
  );
  // 5b) MISSING_HUMAN_APPROVAL for executive risk into production
  const noApproval = await validatePromotion({
    agentId: briefer.id,
    toVersionId: briefer.activeVersionId!,
    fromEnv: 'staging',
    toEnv: 'production',
  ...
  T(
    '5b. MISSING_HUMAN_APPROVAL emitted for executive→production',
    noApproval.blockers.some((b) => b.code === PROMOTION_BLOCKER_CODES.MISSING_HUMAN_APPROVAL),
  );
  ...
  const [unapproved] = await db
    ...
    .where(eq(forgeModelsTable.approved, false))
    .limit(1);
  if (unapproved) {
  ...
  const [bad] = await db
    ...
    .values({
      ...
      toolIds: [],
      evalsPassed: false,
      observabilityHookConfigured: false,
  ...
  T(
    '5d. EVALS_NOT_PASSED emitted',
    badRes.blockers.some((b) => b.code === PROMOTION_BLOCKER_CODES.EVALS_NOT_PASSED),
  );
  T(
    '5e. MISSING_OBSERVABILITY emitted',
    badRes.blockers.some((b) => b.code === PROMOTION_BLOCKER_CODES.MISSING_OBSERVABILITY),
  );
  T(
  ...
  T(
    '6. policy enforcement returns needs_approval for regulated executor',
    policy.outcome === 'needs_approval' || policy.outcome === 'deny',
  ...
  const run = await captureRuntime({
    agentId: briefer.id,
    versionId: briefer.activeVersionId!,
    envTier: 'production',
    status: 'success',
    latencyMs: 1234,
    inputTokens: 500,
  ...
  const versions = await db
    ...
    .where(eq(forgeAgentVersionsTable.agentId, briefer.id))
    .orderBy(forgeAgentVersionsTable.version);
  const v1 = versions[0]!;
  const before = (
    await db.select().from(forgeAgentsTable).where(eq(forgeAgentsTable.id, briefer.id)).limit(1)
  )[0]!;
  await rollbackAgent({
    agentId: briefer.id,
    toVersionId: v1.id,
  ...
  T(
    '8. rollback swaps activeVersionId atomically',
    after.activeVersionId === v1.id,
    `before=${before.activeVersionId?.slice(0, 8)} after=${after.activeVersionId?.slice(0, 8)}`,
  );
  // restore
  await db
    .update(forgeAgentsTable)
    .set({ activeVersionId: before.activeVersionId })
    .where(eq(forgeAgentsTable.id, briefer.id));

  // ── 9. audit trail recorded
  const audits = await db
    .select()
    .from(forgeAuditEventsTable)
    .where(eq(forgeAuditEventsTable.action, 'rollback'));
  T(
    '9. audit log records rollback action',
    audits.length > 0,
    `${audits.length} rollback audit rows`,
  );
  ...
  for (const a of agents) {
    const [v] = await db
      .select()
      .from(forgeAgentVersionsTable)
      .where(eq(forgeAgentVersionsTable.id, a.activeVersionId!))
      .limit(1);
