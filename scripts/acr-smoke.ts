...
const sources = [
  {
    sourceId: 'ais-feed-001',
    sourceType: 'sensor',
    retrievalScore: 0.92,
    summary: 'Live AIS data for 12 vessels',
  },
  {
    sourceId: 'charter-db-001',
    sourceType: 'document',
    retrievalScore: 0.87,
    summary: 'Q2 charter party clauses',
  },
  {
    sourceId: 'bunker-prices-001',
    sourceType: 'api',
    retrievalScore: 0.79,
    summary: 'Rotterdam bunker price index',
  },
];
...
const interruptSpec: ApprovalInterruptSpec = {
  ...
  policyReason:
    'Charter party mutations require dual-operator approval (policy: vessels-charter-gate-v1)',
  evidenceSummary:
    'Southern route reduces fuel by 11.8%. ETA deviation +2.1h within 8h window. Bunker savings: $14,280.',
...
const decisionResult = decideApproval({
  ...
  actor: 'elena.vasquez',
  reason:
    'Fuel savings well within risk tolerance. ETA deviation acceptable per charter party Article 14(b). Approving.',
});
...
LOG('RESUME', {
  runId,
  fromCheckpoint: approvalRequest.checkpointRef,
  withDecision: decisionResult.decision.decisionId,
});
