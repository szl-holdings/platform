...
from raw product screenshots. Outputs to archive/social-launch/LINKEDIN-LAUNCH/images/.

Run from the repository root:
    python scripts/media/build_carousel.py
"""
from PIL import Image, ImageDraw, ImageFont, ImageFilter
import os, glob, subprocess

W, H = 1080, 1350
# Vercel-black palette: pure black, pure white, single accent, hairline borders.
BG     = (0, 0, 0)
SURF   = (10, 10, 10)
PANEL  = (14, 14, 16)
INK    = (255, 255, 255)
DIM    = (161, 161, 170)        # zinc-400
MUTE   = (82, 82, 91)           # zinc-600
LINE   = (32, 32, 36)           # barely-perceptible
HAIR   = (22, 22, 26)
ACCENT = (255, 255, 255)        # pure-white accent (Vercel signal)
GOLD   = (250, 204, 21)         # one warm signal, used sparingly

OUT = "archive/social-launch/LINKEDIN-LAUNCH/images"
RAW = "archive/social-launch/LINKEDIN-LAUNCH/raw-screenshots"
...
def chrome(d, slide_n, total, kicker):
    # ultra-thin top rule
    d.line([(40, 56), (W-40, 56)], fill=LINE, width=1)
    # logo wordmark — pure type, no box
    d.text((40, 28), "SZL", font=F_LBL, fill=INK)
    d.text((86, 28), "HOLDINGS", font=F_LBL, fill=DIM)
    d.text((W-110, 28), f"{slide_n:02d} / {total:02d}", font=F_TINY, fill=MUTE)
    # ultra-thin bottom rule
    d.line([(40, H-56), (W-40, H-56)], fill=LINE, width=1)
    d.text((40, H-38), "szlholdings.com", font=F_TINY, fill=DIM)
    d.text((W-292, H-38), "github · medium · substack", font=F_TINY, fill=MUTE)
    if kicker:
        d.text((40, 96), kicker, font=F_TINY, fill=DIM)
...
def screenshot_panel(img, x, y, w, h, src_path, label=None):
    """Hairline-bordered panel, no chrome dots, no shadow. Vercel-style restraint."""
    d = ImageDraw.Draw(img)
    # panel
    d.rounded_rectangle([x,y,x+w,y+h], radius=10, fill=PANEL, outline=LINE, width=1)
    # title rule
    bar_h = 32
    d.line([(x+1, y+bar_h), (x+w-1, y+bar_h)], fill=LINE, width=1)
    if label:
        d.text((x+18, y+10), label, font=F_TINY, fill=DIM)
    # screenshot fitted into the panel body
    body_w, body_h = w-16, h-bar_h-16
    pic = fit(src_path, body_w, body_h)
    px = x + (w-pic.width)//2
    py = y + bar_h + 8 + (body_h-pic.height)//2
    img.paste(pic, (px, py))

# ---- SLIDE 1: COVER ----
def slide_cover(total):
    img, d = base_canvas()
    chrome(d, 1, total, None)
    d.text((40, 96), "SERIES A  ·  PUBLIC LAUNCH  ·  APRIL 2026", font=F_TINY, fill=DIM)

    # Headline — composed from three weighted lines
    d.text((40, 170), "Governed", font=F_HERO, fill=INK)
    d.text((40, 260), "decision", font=F_HERO, fill=INK)
    d.text((40, 350), "operating system.", font=F_HERO, fill=DIM)

    d.line([(40, 470), (200, 470)], fill=INK, width=2)

    d.text((40, 502), "Built for the enterprises that can't afford an AI", font=F_BODY, fill=DIM)
    d.text((40, 538), "hallucination to become a public incident.", font=F_BODY, fill=DIM)

    # 6-pack — minimal chip rows, just type
    d.text((40, 640), "DOMAIN PACKS", font=F_TINY, fill=MUTE)
    d.line([(40, 666), (W-40, 666)], fill=LINE, width=1)
    chips = [("Lyte / Pulse","execution"),("Vessels","maritime"),("Terra","real estate"),
             ("Carlota Jo / PRISM","advisory"),("Aegis","defense"),("Imperium","platform")]
    cy = 686
    for i,(n,k) in enumerate(chips):
        col = i%2; row = i//2
        x = 40 + col*500; y = cy + row*72
        d.text((x, y), n, font=F_H2, fill=INK)
        d.text((x, y+44), k.upper(), font=F_TINY, fill=DIM)

    # by-line
    d.line([(40, 1000), (W-40, 1000)], fill=LINE, width=1)
    d.text((40, 1018), "BY  STEPHEN LUTAR  ·  FOUNDER + ARCHITECT", font=F_TINY, fill=DIM)
    d.text((40, 1052), "30 days  ·  9 production surfaces  ·  17 artifacts  ·  120+ tables  ·  405 passing tests", font=F_BODY, fill=INK)
    img.save(f"{OUT}/01-cover.jpg", quality=92)

# ---- SLIDE 2: WHAT IS THIS ----
def slide_thesis(total):
    img, d = base_canvas()
    chrome(d, 2, total, "01  ·  THE THESIS")
    d.text((40, 138), "Enterprises don't need", font=F_H1, fill=DIM)
    d.text((40, 200), "another AI copilot.", font=F_H1, fill=DIM)
    d.text((40, 296), "They need an audit trail", font=F_H1, fill=INK)
    d.text((40, 358), "for every decision a model touches.", font=F_H1, fill=INK)

    # 8-step pipeline — minimal numbered grid
    d.text((40, 480), "THE GOVERNED DECISION PIPELINE", font=F_TINY, fill=MUTE)
    d.line([(40, 506), (W-40, 506)], fill=LINE, width=1)
    steps = ["Signal","Context","Recommend","Simulate","Policy","Execute","Proof","Outcome"]
    sx, sy = 40, 526; gap = 6; cw = (W-80-gap*7)//8
    for i,s in enumerate(steps):
        x = sx + i*(cw+gap)
        d.rounded_rectangle([x,sy,x+cw,sy+92], radius=6, fill=PANEL, outline=LINE, width=1)
        d.text((x+10, sy+12), f"{i+1:02d}", font=F_TINY, fill=DIM)
        d.text((x+10, sy+50), s, font=F_LBL, fill=INK)
    d.text((40, 644), "Every step → logged, attributed, replayable, policy-gated.", font=F_BODY, fill=DIM)

    # 3 columns
    cols = [
        ("GUARDIAN",    "8-tier policy engine. Pre-flight, in-flight, post-flight. Block, warn, escalate, or log on every model call."),
        ("TRACE GRAPH", "OpenTelemetry-native. Every signal → recommendation → action persisted as a queryable directed graph."),
        ("EVAL OS",     "Continuous eval gates promote prompts and tools. No silent regressions. No vibes-based shipping."),
    ]
    cx, cy = 40, 760; cw = (W-80-32)//3
    for i,(n,b) in enumerate(cols):
        x = cx + i*(cw+16)
        d.rounded_rectangle([x,cy,x+cw,cy+440], radius=10, fill=SURF, outline=LINE, width=1)
        d.text((x+24, cy+24), f"0{i+1}", font=F_TINY, fill=MUTE)
        d.text((x+24, cy+54), n, font=F_H2, fill=INK)
        d.line([(x+24, cy+108), (x+72, cy+108)], fill=INK, width=2)
        # wrap body
        words = b.split(); line=""; ly = cy+128
        for w in words:
            t = (line+" "+w).strip()
            if d.textlength(t, font=F_BODY) > cw-48:
                d.text((x+24, ly), line, font=F_BODY, fill=DIM); ly += 36; line = w
            else: line = t
        if line: d.text((x+24, ly), line, font=F_BODY, fill=DIM)
    img.save(f"{OUT}/02-thesis.jpg", quality=92)

# ---- SLIDE 3..N: PRODUCT SHOTS ----
PRODUCT_SLIDES = [
    ("00-szl-holdings-home-FRESH.jpg", "02  ·  PARENT PLATFORM", "SZL HOLDINGS  ·  GOVERNED DECISION OPERATING SYSTEM",
