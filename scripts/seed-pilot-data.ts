...
async function seedPilotSignals(orgId: number) {
  ...
  await db.insert(szlSignalsTable).values([
    {
      orgId,
      productSlug: "lyte",
      source: "ERP System",
      ...
      whyItMatters: "Delayed approval blocks 3 engineering hires. Each week of delay = $85K in deferred productivity.",
    ...
    },
    {
      ...
      whyItMatters: "Each blocked onboarding = $42K in delayed procurement savings. Total at risk: $84K.",
    ...
    },
    {
      ...
      whyItMatters: "SOC 2 certification required for 3 enterprise prospects ($2.8M combined ARR). Audit failure = lost pipeline.",
...
async function seedPilotActions(orgId: number) {
  ...
  await db.insert(szlActionsTable).values([
    {
      orgId,
      productSlug: "lyte",
...
async function seedPilotWorkflows(orgId: number) {
  ...
  await db.insert(szlWorkflowsTable).values([
    {
      orgId,
