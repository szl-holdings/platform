...
beforeAll(() => {
  ...
  w(
    "packages/cleanpkg/src/index.ts",
    `import { z } from "zod";\nexport const ok = z;\n`,
  );
...
describe("checkArtifactFiles (fixture)", () => {
  it("flags artifact-to-artifact imports", () => {
    ...
    expect(v.every((vi) => vi.rule.startsWith("ARTIFACT_CROSS_IMPORT"))).toBe(
      true,
    );
    expect(
      v.some((vi) => vi.file === "artifacts/dirty/src/index.ts"),
    ).toBe(true);
    // The flagged import should be the one that textually references "artifacts/".
    const dirtyViolation = v.find(
      (vi) => vi.file === "artifacts/dirty/src/index.ts",
    );
...
describe("runChecks (fixture)", () => {
  ...
  it("returns no violations when none of the rules are broken", () => {
    ...
    try {
      ...
      writeFileSync(
        join(cleanRoot, "artifacts/x/src/index.ts"),
        `export const a = 1;\n`,
      );
