...
const posts = [
  ...
  },
  {
    ...
    caption: 'KORA — live signal stream catching the Vantex Acquisition stall before the quarter closed.',
    story: 'kora-vantex',
  },
  {
    n: 3,
    ...
    caption: 'TENAX — cyber posture turned into a command surface; the same posture we hold ourselves to.',
  ...
  },
  {
    ...
    caption: 'DOMAINE — the operating surface for serious real estate, now in pilot.',
    story: 'domaine-live',
  },
  {
    n: 6,
...
const copy = {
  ...
  'audit-trails': {
    medium: {
      ...
      body: [
        ...
        'This week we shipped per-user decision history for every Quick Action surfaced through Cortex, our governed action layer. When an executive approves or rejects an AI-recommended action — on the dashboard, on mobile, anywhere — we now record who decided, when, against what evidence, and under which policy. That history is queryable per user, per organization, and per action type, ordered chronologically by the decision timestamp itself rather than the side effect of the action.',
        ...
        'On the user side, the new screen lives in the mobile shell at /quick-actions-history and on the web command surface inside Cortex. Each row shows the recommendation, the decision, the actor, the timestamp, the policy that allowed it, and a deep link to the full proof chain.',
  ...
  'kora-vantex': {
    medium: {
      title: 'How KORA Caught a $4.2M Stalled Deal Before the CFO Did',
      body: [
        'Inside the SZL portfolio there is a synthetic enterprise we use to demonstrate KORA, our decision intelligence platform. The synthetic CFO has a real problem: the Vantex Acquisition has been stalled for forty-seven days, and the approval chain is silently void because the original approver left the company without a recorded handoff.',
        ...
        'The pattern KORA caught — departed-approver chain failures — is a category, not a one-off. We have a backlog of customers describing the same incident in different industries: a procurement chain locks because the buyer left, a clinical trial site stalls because the medical monitor rotated off, a syndicated loan freezes because the lead bank changed deal teams. The chain is "silently void" until something forces an audit. KORA forces the audit on day three, not day forty-seven.',
    ...
    substack: {
      subject: 'KORA caught a $4.2M stalled deal before the CFO did',
      body: [
        ...
        'KORA, our decision intelligence platform, surfaces this morning with 91% confidence: $4.2M revenue exposure elevated, approval chain failure is the root cause. 47 active signals. 8 stalled approvals. Workflow health 62%, down 11 points. 94% evidence coverage on the decisions that have been made.',
    ...
    linkedin: {
      ...
      body: [
        ...
        '',
        'KORA, our decision intelligence platform, caught the Vantex stall on day three with 91% confidence. Root cause named, recommended action governed, audit trail attached. The CFO did not have to discover the problem from a quarterly review.',
        '',
    ...
    x: [
      ...
      'KORA caught the same pattern on day three: $4.2M exposure, 91% confidence, root cause named — approval chain failure, three automated escalations refused by policy.',
  ...
  'boring-security': {
    ...
    substack: {
      ...
      body: [
        ...
        'This is the structural posture. Deny by default, scope by org, record with attribution, rotate on a schedule, test before feature. We hold ourselves to the same posture our cyber resilience product (TENAX, on the Sentra surface) tells customers to hold.',
    ...
    linkedin: {
      ...
      body: [
        ...
        '',
        'The same posture we hold ourselves to is the posture our cyber resilience product (TENAX) tells customers to hold. The platform that runs the product is built the way the product says to build.',
      ],
    ...
    x: [
      ...
      'the platform that runs our cyber resilience product (TENAX) is built the way TENAX tells customers to build. that\'s the discipline that earns series A conviction.',
    ],
  },
  ...
  'counsel-ships': {
    medium: {
      ...
      body: [
        ...
        'Why ship Counsel inside SZL Holdings rather than as a standalone? Because the same governance primitives — covenant policy, proof chain, decision backlog, evidence coverage — are the spine for every artifact in our portfolio. Counsel reuses what KORA, TENAX, DOMAINE, and SEXTANT all sit on. The legal organization gets the same recorded decision loop as finance, security, real estate, and maritime. That loop is the company.',
  ...
  'domaine-live': {
    medium: {
      title: 'DOMAINE: The Operating Surface for Serious Real Estate',
      body: [
        'The serious real estate organization — the one that actually owns property, runs portfolios, and underwrites distressed acquisitions — does not need another listings tool. It needs an operating surface. That is what DOMAINE is.',
        'DOMAINE is the SZL artifact for investors, brokers, and portfolio teams who carry real-estate decisions on a balance sheet. It runs on a single intelligence layer that spans distressed property discovery, ownership analysis, pipeline management, and deal execution. Discovery flows into analysis flows into pipeline flows into execution; the same evidence travels with the asset from the first signal to the closed deal.',
        'The technical interesting bit is the spatial runtime underneath it. DOMAINE shares an ATLAS spatial layer with SEXTANT (our maritime intelligence artifact) and the Unified Command surface. That means a property in DOMAINE is the same kind of object as a vessel in SEXTANT — an entity in the worldline registry, with positions, ownership, jurisdiction, and policy attached. Cross-domain queries are real, not stitched.',
        'Why does that matter for a real estate operator? Because the messy real-world questions are cross-domain by nature. "Who owns the LLC that owns this distressed property, and have they had a vessel arrest in the last three years?" "Which of our portfolio properties are in jurisdictions where my buyer is currently subject to a regulatory action?" Those questions take a forensics team in most firms. In DOMAINE they are queries.',
        'What is shipping today is the public landing and the pilot request path, with the live operator surface behind sign-in for the design partner cohort. The pilot is gated because the seed data and the policy posture are calibrated to the partner — DOMAINE is not a self-serve product yet, by design.',
        'When we resume after the funding pause, DOMAINE expands its pilot cohort to a small number of additional partners. If you are running real-estate decisions on a balance sheet and you want to be evaluated, the request path is on the landing.',
      ],
    },
    substack: {
      subject: 'DOMAINE is live — the operating surface for serious real estate',
      body: [
        'DOMAINE is live on its public surface today. It is the SZL artifact for real-estate investors, brokers, and portfolio teams who carry decisions on a balance sheet.',
        ...
        'Underneath it is ATLAS, our spatial runtime, shared with SEXTANT (maritime intelligence) and Unified Command. A property in DOMAINE is the same kind of object as a vessel in SEXTANT — an entity in the worldline registry with positions, ownership, jurisdiction, policy. Cross-domain queries are real, not stitched.',
    ...
    linkedin: {
      ...
      body: [
        'DOMAINE is live today — the SZL artifact for investors, brokers, and portfolio teams who carry real-estate decisions on a balance sheet.',
        '',
        ...
        '',
        'Underneath sits ATLAS, our spatial runtime — shared with SEXTANT (maritime). A property and a vessel are the same kind of object: an entity with positions, ownership, jurisdiction, and policy. Cross-domain queries are queries, not forensic projects.',
      ],
      cta: 'Pilot cohort opens after a short funding pause. Request path is on the landing.',
    },
    x: [
      'DOMAINE is live. SZL\'s artifact for real-estate investors, brokers, and portfolio teams who carry decisions on a balance sheet.',
      ...
      'underneath: ATLAS, our spatial runtime. shared with SEXTANT (maritime intelligence) and Unified Command. a property in DOMAINE is the same kind of object as a vessel in SEXTANT — entity, positions, ownership, jurisdiction, policy.',
      'cross-domain question: "who owns the LLC that owns this distressed property, and have they had a vessel arrest in the last three years?" — that is a query in DOMAINE, not a forensics project.',
  ...
  'one-loop': {
    medium: {
      ...
      body: [
        ...
        'KORA is the loop applied to enterprise decision intelligence — signals from business systems, recommendations to operators, governed actions back into those systems. DOMAINE is the loop applied to real estate. SEXTANT is the loop applied to maritime. TENAX is the loop applied to cyber posture and incident response. FORGE is the execution fabric every other artifact runs on. Counsel is the loop applied to legal exposure. Carlota Jo is the loop applied to private advisory and household operations — yes, the same primitives apply, because the failure modes are the same: too many open threads, no record of who decided what, no continuity between advisor and principal.',
        'The Unified Command surface is the operator\'s view of all of it together. It is the cross-domain workspace where a CFO can look at a KORA signal next to a Counsel obligation and a TENAX exposure on the same screen, with the same evidence model behind each one. Two participants viewing live in the screenshot at the top of this post is not a stage prop; it is the actual collaborative session model the surface uses.',
    ...
    substack: {
      ...
      body: [
        'The honest answer to "is SZL one product or six?" is that the visible layer is six command surfaces — KORA, DOMAINE, SEXTANT, TENAX, FORGE, Carlota Jo — and the invisible layer is one decision loop. The thesis is the loop.',
        ...
        'Unified Command is the cross-domain workspace where a CFO can look at a KORA signal next to a Counsel obligation and a TENAX exposure on the same screen, same evidence model behind each.',
    ...
    linkedin: {
      ...
      body: [
        ...
        '• KORA — enterprise decision intelligence',
        '• DOMAINE — real estate',
        '• SEXTANT — maritime',
        '• TENAX — cyber posture and incident response',
        '• FORGE — the execution fabric every artifact runs on',
        '• Counsel — legal exposure',
        '• Carlota Jo — private advisory continuity',
        '',
        'Unified Command is the cross-domain workspace. A CFO looks at a KORA signal next to a Counsel obligation and a TENAX exposure on the same screen with the same evidence model behind each.',
      ],
    ...
    x: [
      ...
      'honest answer: visible layer is six command surfaces — KORA, DOMAINE, SEXTANT, TENAX, FORGE, Counsel, Carlota Jo. invisible layer is one decision loop. the thesis is the loop.',
      ...
      'Unified Command is the cross-domain workspace. a CFO can look at a KORA signal next to a Counsel obligation and a TENAX exposure on the same screen, same evidence model behind each.',
