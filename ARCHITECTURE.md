...
| Package | Detail |
|---------|--------|
| `@szl-holdings/shared-ui` | Component library: UI primitives, navigation, command palette, keyboard shortcuts. **One-of-One shell modules (Task #2308):** `DashboardShell` (collapsible sidebar chrome), `EcosystemNav` (cross-surface nav bar), `CommandPalette` (⌘K keyboard search, wired to Sentra, Counsel, Vessels, Terra, Command, and Lyte), `SentientLayer` (⌘J AI intelligence rail with Now/Next/Links tabs), `AgentRunCard` (compact trace card), `IncidentCommander` (full incident shell), `ScenarioBranchesPanel` (Monte Carlo branch comparison). All 13 surfaces share DashboardShell + EcosystemNav. Audit docs at `docs/audit/one-of-one/`. |
