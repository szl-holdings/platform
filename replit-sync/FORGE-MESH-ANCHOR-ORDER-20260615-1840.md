...
1. **Box on current main**: ensure a11oy.net redeploys to the latest main (incl. team D's hardened-pool resolver fix once merged). PROVE: GET /api/a11oy/v1/honest git_sha == GitHub HEAD; GET /compute-pool-hardened lists omen-betterwithage + chaski with resolver detail `via tailscale-*` at their LIVE IPs.
