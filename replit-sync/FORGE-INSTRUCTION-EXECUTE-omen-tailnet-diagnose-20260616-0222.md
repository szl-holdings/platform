...
## ON SUCCESS (box can now TCP-reach the OMEN): confirm omen-betterwithage flips reachable:true by REAL probe in GET https://a-11-oy.com/api/a11oy/v1/compute-pool-hardened (gpu_nodes_reachable -> 3). Then also do the COORDINATOR cutover (run szl-router mesh_coordinator.py on box port 11500, set A11OY_MODEL_BASE_URL=http://127.0.0.1:11500/v1) and report. cosign stays founder-key-gated.
