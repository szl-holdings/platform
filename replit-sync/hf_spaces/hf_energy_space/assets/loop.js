...
// Honor prefers-reduced-motion: render a calm static scene, no flow/drift.
let flowFactor = REDUCED ? 0 : 1.0;
window.__setLoopFlow = (soak) => { if (!REDUCED) flowFactor = soak ? 1.6 : 0.7; };
...
function animate() {
  ...
  // gentle camera drift (skipped under reduced-motion)
  if (!REDUCED) {
