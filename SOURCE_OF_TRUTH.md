...
| Top-level package directories (`packages/`) | **158** | Tracked top-level directories; excludes root file `packages/proxy-routes.ts`; not every directory is a workspace package |
...
| Workspace package manifests | **198** | Tracked `package.json` files included by `pnpm-workspace.yaml`; pnpm reports 199 projects when the root is included |
