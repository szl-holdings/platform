...
| # | Action | Where | Instruction | Acceptance Test |
|---|--------|--------|-------------|-----------------|
| 9 | Move `INTEGRATION_TEST_TOKEN` from source to env | GitHub → Settings → Secrets → Actions | Generate a new unique token value (e.g. `openssl rand -hex 32`); add as `INTEGRATION_TEST_TOKEN` in GitHub Secrets; update `tests/api/server-live.test.ts` to read from `process.env.INTEGRATION_TEST_TOKEN` instead of the hardcoded value | Integration tests pass in CI; token not visible in source |
