...
export function createRunsRouter(): Router {
  ...
  router.post("/runs", async (req: Request, res: Response) => {
    ...
    const definition = resolveWorkflowDefinition(workflowId, input as Record<string, unknown>, tenantId, profileId);
    if (!definition) {
  ...
  switch (workflowId) {
    ...
    case "verify_index_health":
      return buildVerifyIndexHealthWorkflow({
        tenantId,
        profileId,
        goldQueries: input["goldQueries"] as Array<{ query: string; expectedChunkIds: string[] }> | undefined,
    ...
    case "rotate_profile_version":
      return buildRotateProfileVersionWorkflow({
        ...
        shadowQueries: input["shadowQueries"] as Array<{ queryId: string; query: string; relevantChunkIds: string[] }> | undefined,
      });
    default:
      return null;
  }
