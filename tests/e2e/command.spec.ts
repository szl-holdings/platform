...
test.beforeAll(async ({ browser }) => {
  ...
  while (Date.now() < deadline) {
    try {
      const resp = await page.goto(COMMAND_PATH, { timeout: 10000, waitUntil: 'domcontentloaded' });
...
test.describe('Ecosystem Command Portal — Dashboard Content', () => {
  ...
  test('cross-domain navigation links to known portals', async ({ page }) => {
    ...
    const ecosystemLink = page
      .locator("a[href*='aegis'], a[href*='vessels'], a[href*='terra'], a[href*='lyte']")
      .first();
    const hasEcosystemLink = await ecosystemLink.isVisible({ timeout: 10000 }).catch(() => false);
