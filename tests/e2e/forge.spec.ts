...
const NURO_FORGE_PATH = (
  SZL_PATH.endsWith('/') ? `${SZL_PATH}nuro-forge` : `${SZL_PATH}/nuro-forge`
).replace('//', '/');
...
test.describe('Nuro Forge — Content Validation', () => {
  ...
  test('user navigates from forge home to arena via nav or links', async ({ page }) => {
    ...
    const arenaLink = page.locator("a[href*='arena'], a:has-text('Arena')").first();
...
test.describe('Forge — Submit Execution Mutation E2E', () => {
  ...
  test('submits valid payload, intercepts POST /api/forge/submit, and shows success state', async ({
    page,
  }) => {
    let captured: {
      method: string;
    ...
    await page.route('**/api/forge/submit', async (route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          success: true,
          data: { executionId: 'exec_e2e_001', status: 'queued' },
        }),
      });
    });
    ...
    await page
      .getByTestId('input-forge-input-json')
      .fill('{"matterId":"mat_001","task":"risk-scan"}');
  ...
  test('client-side validation blocks POST when agent slug is empty', async ({ page }) => {
    ...
    await expect(page.getByTestId('text-forge-submit-error')).toContainText(
