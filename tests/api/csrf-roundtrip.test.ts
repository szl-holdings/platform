...
vi.mock("../../artifacts/api-server/src/lib/api-response", () => {
  const sendError = (res: { status: (s: number) => { json: (b: unknown) => void } }, message: string, status = 403, code?: string) =>
...
function buildTestApp(extraExemptPaths: string[] = []): Express {
  ...
  for (const path of protectedPaths) {
    if (!extraExemptPaths.includes(path)) {
      ...
      app.patch(`${path}/:id`, (_req: Request, res: Response) => res.status(200).json({ ok: true }));
      app.delete(`${path}/:id`, (_req: Request, res: Response) => res.status(200).json({ ok: true }));
    }
  }
