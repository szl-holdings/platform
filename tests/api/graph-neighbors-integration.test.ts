...
describe("Integration — GET /graph/entities/:id/neighbors", () => {
  ...
  beforeAll(async () => {
    ...
    const insertedNodes = await db
      .insert(cstNodes)
      .values([
        { domain: "terra",   entityType: FIXTURE_ENTITY_TYPE, name: `${RUN_TAG}-HUB`,    provenanceSourceId: PROVENANCE_ID },
        { domain: "terra",   entityType: FIXTURE_ENTITY_TYPE, name: `${RUN_TAG}-OUT_1`,  provenanceSourceId: PROVENANCE_ID },
        { domain: "vessels", entityType: FIXTURE_ENTITY_TYPE, name: `${RUN_TAG}-OUT_2`,  provenanceSourceId: PROVENANCE_ID },
        { domain: "aegis",   entityType: FIXTURE_ENTITY_TYPE, name: `${RUN_TAG}-IN_1`,   provenanceSourceId: PROVENANCE_ID },
        { domain: "terra",   entityType: FIXTURE_ENTITY_TYPE, name: `${RUN_TAG}-IN_2`,   provenanceSourceId: PROVENANCE_ID },
        { domain: "terra",   entityType: FIXTURE_ENTITY_TYPE, name: `${RUN_TAG}-ISLAND`, provenanceSourceId: PROVENANCE_ID },
      ])
    ...
    const insertedEdges = await db
      .insert(cstEdges)
      .values([
        { fromNodeId: seeded.hub, toNodeId: seeded.out1, relationshipType: `${RUN_TAG}_hub_to_out1`, sourceId: PROVENANCE_ID },
        { fromNodeId: seeded.hub, toNodeId: seeded.out2, relationshipType: `${RUN_TAG}_hub_to_out2`, sourceId: PROVENANCE_ID },
        { fromNodeId: seeded.in1, toNodeId: seeded.hub,  relationshipType: `${RUN_TAG}_in1_to_hub`,  sourceId: PROVENANCE_ID },
        // Inactive edge — route must still surface it.
        { fromNodeId: seeded.in2, toNodeId: seeded.hub,  relationshipType: `${RUN_TAG}_in2_to_hub`,  sourceId: PROVENANCE_ID, active: false },
      ])
  ...
  afterAll(async () => {
    ...
    const allNodeIds = [
      seeded.hub, seeded.out1, seeded.out2, seeded.in1, seeded.in2, seeded.island,
    ].filter(Boolean);
  ...
  it("returns hydrated neighbor entities with domain/entityType/name fields", async () => {
    ...
    const neighbors = payload.neighbors as Array<{
      id: string; domain: string; entityType: string; name: string;
    }>;
