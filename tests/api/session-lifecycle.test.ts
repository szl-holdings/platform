    ...
    for (const row of matched) {
      for (const k of Object.keys(this.setVals)) {
        ...
        if (
          v &&
          typeof v === 'object' &&
          (v as { __sqlIncrementField?: string }).__sqlIncrementField
        ) {
...
vi.mock('drizzle-orm', () => ({
  ...
  sql: (strings: TemplateStringsArray, ...values: unknown[]) => {
    ...
    if (text.includes('+ 1')) {
      const field =
        (values[0] as { toString(): string } | string)?.toString?.() ?? String(values[0]);
...
function buildApp() {
  return import('../../artifacts/api-server/src/middlewares/auth').then(({ authMiddleware }) => {
    ...
    app.get('/protected', authMiddleware(), (req, res) => {
      res
        .status(200)
        .json({ ok: true, userId: (req as unknown as { user?: { id: number } }).user?.id });
    });
    return app;
  });
}
...
describe('Session lifecycle hardening (Task #1914)', () => {
  ...
  it('rejects concurrent refresh rotations atomically (only one wins)', async () => {
    seedUser();
    const { createSessionWithRefresh, rotateRefreshToken, RefreshTokenReplayError } = await import(
      '../../artifacts/api-server/src/middlewares/session-policy'
    );
