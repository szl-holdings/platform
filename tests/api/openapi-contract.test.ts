...
interface OpenAPISpec {
  ...
  paths: Record<string, Record<string, { tags?: string[]; operationId?: string; requestBody?: unknown; responses?: Record<string, unknown> }>>;
  components?: {
...
describe("OpenAPI contract — AI Engine (Alloy) domain", () => {
  ...
  it("AI decision POST requires recommendedAction, rationaleSummary, riskLevel", () => {
    ...
    const body = (postOp?.requestBody as { content?: { "application/json"?: { schema?: { required?: string[] } } } })
...
describe("OpenAPI contract — Coverage gap analysis", () => {
  it("identifies domains with routes but no OpenAPI spec coverage", () => {
    ...
    const implementedDomains = [
      "vessels", "firestorm", "lyte", "health", "ai", "observability",
      "auth", "billing", "projects", "connectors", "notifications",
      "audit", "feature-flags", "files", "assets", "stephen", "storage",
      "dreamscape", "readiness", "mobile-auth", "login", "callback", "logout",
    ];

    const undocumentedDomains = implementedDomains.filter(
      d => !coveredDomainPrefixes.has(d),
    );
  ...
  it("known coverage gaps exactly match uncovered domains (no regressions, no silent growth)", () => {
    ...
    const allImplementedDomains = [
      ...
      "audit", "feature-flags", "files", "assets", "stephen", "storage",
      "dreamscape", "readiness", "mobile-auth", "login", "callback", "logout",
      "terra", "prism", "holdings", "booking",
    ];

    const actualUncoveredDomains = allImplementedDomains.filter(
      d => !coveredPrefixes.has(d),
    );
...
describe("GraphQL schema — domain type coverage (static analysis)", () => {
  ...
  it("schema.ts imports all 7 major domain type definitions", () => {
    ...
    const expectedImports = ["vessels", "terra", "lyte", "firestorm", "holdings", "carlota-jo", "prism-counsel"];
    for (const domain of expectedImports) {
      expect(schemaSrc, `schema.ts missing import for domain: ${domain}`).toContain(`"./domains/${domain}`);
    }
  });
});
