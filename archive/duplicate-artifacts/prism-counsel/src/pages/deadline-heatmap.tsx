    ...
    for (const matter of matters) {
      for (const obl of matter.obligations) {
        ...
        const heat = Math.round(
          imm * 0.6 + matter.pressureScore * 0.25 + matter.complexityScore * 0.15,
        );
        cells.push({
          matterId: matter.id,
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-lg font-semibold font-display text-white/90">
              Deadline Pressure Heatmap
            </h1>
          </div>
          <p className="text-xs text-white/30">
            Imminence × complexity weighting across all open obligations
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: 'High Heat (70–89)', value: String(highCount), color: '#f97316' },
          {
            ...
            value: String(allCells.filter((c) => c.filingRequired).length),
            color: ACCENT,
          },
        ].map((s) => (
          <div
      ...
      <div className="flex items-center gap-2 flex-wrap text-[10px] text-white/30">
        ...
        <span className="ml-2 text-white/20">
          Score = imminence (60%) + pressure (25%) + complexity (15%)
        </span>
      </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/5">
                  <th className="text-left px-4 py-3 text-[10px] font-semibold text-white/30 uppercase tracking-wider w-48">
                    Matter
                  </th>
                  {['Imminence', 'Pressure', 'Complexity', 'Heat Score', 'Due', 'Filing'].map(
                    (h) => (
                      <th
                        key={h}
                        className="text-center px-3 py-3 text-[10px] font-semibold text-white/30 uppercase tracking-wider"
                      >
                        {h}
                      </th>
                    ),
                  ...
                  return (
                    <tr key={cell.obligationId} className="hover:bg-white/2 transition-colors">
                      <td className="px-4 py-3">
                        <p className="text-[11px] font-medium text-white/70 leading-snug line-clamp-2">
                          {cell.matterName.split(' — ')[0]}
                        </p>
                        <p className="text-[9px] text-white/25 mt-0.5 font-mono">{cell.matterId}</p>
                      </td>
                      <td className="px-3 py-3 text-center">
                        <div
                          className="inline-flex items-center justify-center w-10 h-6 rounded text-[11px] font-mono font-semibold"
                          style={{
                            background: `${heatColor(cell.imminence)}18`,
                            color: heatColor(cell.imminence),
                          }}
                        >
                          {cell.imminence}
                        </div>
                      ...
                      <td className="px-3 py-3 text-center">
                        <div
                          className="inline-flex items-center justify-center w-10 h-6 rounded text-[11px] font-mono font-semibold"
                          style={{
                            background: `${getPressureColor(cell.pressureScore)}18`,
                            color: getPressureColor(cell.pressureScore),
                          }}
                        >
                          {cell.pressureScore}
                        </div>
                      ...
                      <td className="px-3 py-3 text-center">
                        <span
                          ...
                          style={{
                            color:
                              daysUntil(cell.dueDate) <= 3
                                ? '#ef4444'
                                : daysUntil(cell.dueDate) <= 7
                                  ? '#f97316'
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-7 gap-2">
            {weekDays.map((day) => {
              ...
              const maxHeat =
                day.cells.length > 0 ? Math.max(...day.cells.map((c) => c.heatScore)) : 0;
              return (
                <div
                  key={day.label}
                  ...
                  style={{
                    ...
                    borderColor: isToday ? 'rgba(167,139,250,0.3)' : 'rgba(255,255,255,0.05)',
                  }}
                >
                  <div
                    className="p-2 border-b border-white/5 text-center"
                    style={isToday ? { background: 'rgba(167,139,250,0.08)' } : {}}
                  >
                    <p
                    ...
                    <p
                      ...
                      style={{
                        color: isToday ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.6)',
                      }}
                    >
                      {day.label.split(' ')[1]}
                    </p>
                    ...
                    {day.cells.length === 0 ? (
                      ...
                      day.cells.map((cell) => (
                        <div
                          ...
                          style={{
                            background: `${heatColor(cell.heatScore)}15`,
                            border: `1px solid ${heatColor(cell.heatScore)}25`,
                        ...
                        >
                          ...
                          <p
                            ...
                            style={{ color: heatColor(cell.heatScore) }}
                          >
                            {cell.heatScore}
                          </p>
                        </div>
                    ...
                    <div className="flex-1 flex items-center gap-1">
                      {cells.slice(0, 8).map((cell) => (
                        <div
                          ...
                          style={{
                            background: heatColor(cell.heatScore),
                            opacity: 0.7 + (cell.heatScore / 100) * 0.3,
                          }}
                          title={`${cell.obligationTitle} — Heat: ${cell.heatScore}`}
                        />
                      ))}
                      {cells.length > 8 && (
                        <span className="text-[10px] text-white/20 ml-1">+{cells.length - 8}</span>
                      )}
                    </div>
                    <span
                      className="text-[11px] font-mono w-8 text-right shrink-0"
                      style={{ color: heatColor(maxHeat) }}
                    >
                      {maxHeat || '—'}
                    </span>
                  </div>
