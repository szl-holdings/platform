...
Signed Zarf payload for the **ROSIE governed-decision-fabric runtime**:
policy admission, contradiction detection, governed-action emit,
...
zarf package pull oci://ghcr.io/szl-holdings/rosie-uds:0.2.0

# Latest dev (unsigned, tracks main)
zarf package pull oci://ghcr.io/szl-holdings/rosie-uds:dev
```
...
cosign verify \
  --certificate-identity-regexp 'https://github.com/szl-holdings/.+/\.github/workflows/rosie-uds-publish\.yml@.+' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  ghcr.io/szl-holdings/rosie-uds:0.2.0

# 2. Per-file integrity (run after unpack)
node artifacts/rosie-uds/scripts/verify-manifest.mjs /path/to/unpacked
```

## Install

```bash
zarf package deploy zarf-package-rosie-uds-*.tar.zst --confirm
# Components stage under /opt/rosie/ on the target node.
```
...
Output: `dist/rosie-uds/rosie-uds-<version>.tar.zst` (+ `.sha256`, + `.sig`
