...
const theorem = paper.theorems.find((t) => t.id === 'TH6')!;

export default function ThesisTH6() {
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-[#0a0a0a] text-text">
      ...
      <div className="absolute top-[6vh] left-0 w-[5vw] flex flex-col items-center gap-[1vh]">
        ...
        <div className="font-mono text-[0.7vw] tracking-[0.25em] text-muted uppercase [writing-mode:vertical-rl] mt-[2vh]">Theorem 6 of 8</div>
      </div>
      ...
      <div className="absolute left-[10vw] right-[10vw] top-[20vh]">
        <div className="font-mono text-[1vw] tracking-[0.25em] text-muted uppercase mb-[3vh]">Information bound via DPI</div>
        <h1 className="text-[4.6vw] leading-[1.02] font-light tracking-[-0.02em]" style={{ fontFamily: "'Cormorant Garamond', serif", textWrap: "balance" }}>
          {theorem.name}
        </h1>
        <p className="mt-[5vh] max-w-[72vw] text-[1.55vw] leading-[1.5] text-text/85" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
          The entropy of a receipt chain of length n is bounded by H(registry) — and that
          registry by 8 bits per byte. A clean physical ceiling on what any deterministic
          telemetry pipeline can observe.
        </p>
      </div>
      ...
      <div className="absolute bottom-[6vh] left-[10vw] right-[6vw] grid grid-cols-3 gap-[3vw]">
        ...
        <div>
          ...
          <div className="font-mono text-[1.1vw] text-text">Terra</div>
        </div>
      </div>
    </div>
  );
