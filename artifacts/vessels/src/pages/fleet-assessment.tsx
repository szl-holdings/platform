...
import {
  ChevronRight, ArrowLeft, Ship, AlertTriangle, ShieldAlert,
  Navigation, BarChart3, Eye, TrendingUp, CheckCircle2,
  Anchor, Globe, DollarSign, Lock, Activity, Users, Layers,
} from "lucide-react";
...
const EXCEPTION_SCENARIOS = [
  {
    ...
    response: [
      { step: "Signal", text: "AIS feed detects 47-minute blackout on MV Horizon at 08:14 UTC. Last known position: Gulf of Aden corridor." },
      { step: "Context", text: "Cross-domain enrichment: vessel history shows 2 prior dark events in OFAC-flagged waters. Cargo manifest flagged by counterparty risk model." },
      { step: "Recommendation", text: "AI recommends: initiate satellite position verification, flag to compliance officer, hold cargo release pending investigation." },
      { step: "Policy", text: "Covenant Policy routes to compliance officer for approval — automatic hold on cargo release enforced at platform layer." },
      { step: "Proof", text: "Proof Chain records full event: AIS loss timestamp, enrichment data, recommendation, policy trigger, and approval action." },
    ],
  },
  {
    id: "sanctions",
    ...
    response: [
      { step: "Signal", text: "Sanctions screening detects MT Pacific Star's new charterer appears on OFAC SDN list update — 3 hours before port entry." },
      { step: "Context", text: "PRISM Counsel cross-domain: existing charter party contains no sanctions clause — legal team alerted automatically." },
      { step: "Recommendation", text: "AI recommends: halt port clearance, initiate sanctions review workflow, engage legal counsel within 30 minutes." },
      { step: "Policy", text: "Covenant Policy enforces mandatory legal review — port clearance cannot proceed without legal sign-off." },
      { step: "Proof", text: "Proof Chain sealed: sanctions match timestamp, legal review record, decision rationale, and outcome attribution." },
    ],
  },
  {
    id: "voyage_deviation",
    ...
    response: [
      { step: "Signal", text: "Voyage telemetry: MV Atlas on weather-avoidance route — ETA deviation of +18 hours detected against SLA commitment." },
      { step: "Context", text: "Voyage P&L model: 18-hour delay triggers $42,000 demurrage clause. Freight rate benchmark shows spot alternative available." },
      { step: "Simulation", text: "Decision Simulation models 3 options: accept delay (P50 cost: $42K), reroute (P50 cost: $18K), spot charter (P50 cost: $67K)." },
      { step: "Policy", text: "Operator reviews simulation, selects reroute option — Covenant Policy routes to operations manager for approval." },
      { step: "Proof", text: "Decision recorded: simulation data, operator selection, approval, and revised ETA committed to Proof Chain." },
    ],
  },
];
...
const ASSESSMENT_QUESTIONS = [
  {
    ...
    label: "When a vessel goes dark or a sanctions flag fires, how does your team coordinate a response?",
    options: [
      { value: "manual", label: "Manually — calls, emails, and Slack threads across teams", score: 0 },
      { value: "alert", label: "We get an alert, but the response workflow is manual", score: 1 },
      { value: "partial", label: "We have documented playbooks, but execution is still manual", score: 2 },
      { value: "governed", label: "Governed workflow — the exception triggers a structured, approved response", score: 4 },
    ],
  },
  {
    id: "sanctions_audit",
    label: "If a regulator asks for your sanctions screening audit trail from the past 12 months, how quickly can you produce it?",
    options: [
      ...
      { value: "fast", label: "Under an hour — our screening tool has records but they're incomplete", score: 2 },
      { value: "instant", label: "Instantly — our audit trail is complete, attributed, and queryable", score: 4 },
    ],
  },
  {
    id: "voyage_pnl",
    label: "When an ETA deviation or demurrage risk emerges, do operators see the P&L impact before making a routing decision?",
    options: [
      ...
      { value: "estimate", label: "We have rough estimates, but not integrated into the decision workflow", score: 2 },
      { value: "integrated", label: "Yes — voyage P&L is integrated into every exception workflow", score: 4 },
    ],
  },
  {
    id: "cross_domain",
    label: "When a vessel is involved in both an AIS anomaly and a legal/sanctions issue, do your operations and compliance teams see the same correlated picture?",
    options: [
      ...
      { value: "meetings", label: "We have cross-team meetings but no shared intelligence layer", score: 2 },
      { value: "yes", label: "Yes — signals are correlated and both teams see the same governed picture", score: 4 },
    ],
  },
];

const RESULTS = [
  { min: 0, max: 4, label: "High Exposure", severity: "critical" as const, desc: "Your maritime operations have significant gaps in exception governance, sanctions audit trail, and cross-domain intelligence. A dark vessel or sanctions flag could trigger a regulatory investigation with no defensible record.", cta: "Request a Risk Briefing" },
  { min: 5, max: 9, label: "Partial Coverage", severity: "moderate" as const, desc: "You have some tools in place but significant workflow gaps remain. Exception responses are manual, audit trails are incomplete, and cross-domain correlation depends on individual coordination.", cta: "See a Vessels Demo" },
  { min: 10, max: 13, label: "Operationally Sound", severity: "ready" as const, desc: "Your operations have good coverage but are still relying on manual integration across exception response, compliance, and voyage economics. The opportunity is systematic governance and P&L integration.", cta: "Talk to a Maritime Expert" },
  { min: 14, max: 16, label: "Governance-Grade", severity: "ready" as const, desc: "Your fleet operations are at or near governance-grade. The conversation is about cross-domain intelligence, outcome tracking at scale, and Proof Chain-backed regulatory readiness.", cta: "See Advanced Features" },
];
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      const interval = setInterval(() => {
        if (i >= scenario.response.length) { clearInterval(interval); return; }
        setPlayStep(i);
        i++;
      }, 600);
      ...
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-base max-w-2xl mx-auto" style={{ color: TEXT_SEC }}>
            Not just an alert. A recommendation, a risk simulation, an approved response, and an immutable record.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-8">
          {EXCEPTION_SCENARIOS.map((s, i) => (
            ...
            >
              <span className="text-xs font-bold px-1.5 py-0.5 rounded" style={{ background: i === active ? `${BG}30` : `${s.color}20`, color: i === active ? BG : s.color }}>
                {s.severity}
              </span>
              {s.label}
            </button>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="p-5 border-b flex items-center gap-3" style={{ borderColor: BORDER }}>
              ...
              <span className="text-sm font-semibold" style={{ color: TEXT }}>{scenario.vessel}</span>
              <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ background: `${scenario.color}15`, color: scenario.color }}>
                {scenario.severity}
              </span>
              <span className="text-xs ml-auto" style={{ color: TEXT_SEC }}>{scenario.event}</span>
            </div>
            ...
            <div className="p-6 flex flex-col gap-3">
              {scenario.response.map((r, i) => (
                ...
                >
                  ...
                  <p className="text-sm leading-relaxed" style={{ color: playStep >= i ? TEXT : TEXT_SEC }}>
                    {r.text}
                  </p>
                  {playStep >= i && i === scenario.response.length - 1 && (
                    <CheckCircle2 className="w-4 h-4 shrink-0 mt-0.5" style={{ color: "hsl(152,70%,50%)" }} />
                  )}
                </motion.div>
              ))}
            </div>
            ...
            <div className="px-6 pb-5">
              <button
                onClick={() => { setPlayStep(-1); setTimeout(() => { let i = 0; const iv = setInterval(() => { if (i >= scenario.response.length) { clearInterval(iv); return; } setPlayStep(i); i++; }, 500); }, 50); }}
            ...
            <motion.div key={tab.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.22 }}
              ...
              <div className="flex flex-col md:flex-row gap-8">
                <div className="flex-1">
                  ...
                  <h3 className="text-xl md:text-2xl font-bold mb-4" style={{ color: TEXT }}>{tab.headline}</h3>
...
function FleetAssessment() {
  ...
  const handleAnswer = async (qId: string, value: string, score: number) => {
    ...
    if (nextStep >= ASSESSMENT_QUESTIONS.length) {
      ...
      setStep("result");
      analytics.formSubmit({ form_name: "fleet-governance-assessment", form_type: "diagnostic_assessment" });
      try {
        await fetch("/api/contact/submit", {
          ...
          body: JSON.stringify({
            ...
            metadata: { assessment: "fleet-governance-readiness", score: finalScore, result: finalResult?.label, answers: newAnswers },
          }),
        });
      } catch (_) {}
    } else {
  ...
  return (
    <section style={{ background: BG }} className="py-24 px-4" id="assessment">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-10">
          <p className="text-xs font-semibold tracking-widest uppercase mb-3" style={{ color: ACCENT }}>Fleet Governance Assessment</p>
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight mb-4" style={{ color: TEXT }}>
          ...
          {step === "intro" && (
            <motion.div key="intro" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -14 }} transition={{ duration: 0.25 }}
              ...
              <div className="flex justify-center gap-2 mb-6">
                {ASSESSMENT_QUESTIONS.map((_, i) => (<div key={i} className="h-1 rounded-full" style={{ background: BORDER, width: 40 }} />))}
              </div>
              <button
                onClick={() => setStep(0)}
...
export default function FleetAssessmentPage() {
  useEffect(() => { analytics.pageView({ path: "/fleet-assessment" }); }, []);
  return (
      ...
      <section className="pt-28 pb-16 px-4 text-center" style={{ background: BG }}>
        <div className="max-w-3xl mx-auto">
          ...
          <motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, delay: 0.18 }}
            ...
            When a vessel goes dark, a sanctions flag fires, or an ETA deviation threatens your SLA — you need a recommendation, a risk simulation, and an approved response. Not just an alert.
          </motion.p>
