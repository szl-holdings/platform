...
const DEMO_ROUTE_MEMORY = [
  {
    date: 'Apr 8–10',
    segment: 'Persian Gulf → Indian Ocean',
    avgSpeed: 13.8,
    fuelBurn: 72.4,
    incidents: 0,
    aisGap: false,
  ...
  {
    date: 'Apr 12–14',
    segment: 'Bab-el-Mandeb crossing',
    avgSpeed: 11.2,
    fuelBurn: 61.0,
    incidents: 0,
    aisGap: true,
...
export default function VesselsAtlasRuntimePage() {
  ...
  const { data: driftData } = useStandardQuery<{
    ...
    queryFn: () =>
      fetch('/api/atlas/spatial/drift?twinCategory=vessel')
  ...
  const { data: branchData } = useStandardQuery<{ count: number }>({
    ...
    queryFn: () =>
      fetch('/api/atlas/spatial/branches?twinCategory=vessel')
        .then((r) => (r.ok ? r.json() : Promise.reject(r.status)))
        .then((r) => r.data ?? r),
    staleTime: 60000,
    retry: 1,
  ...
  const {
    data: liveVesselsData,
    isError: vesselsError,
    isLoading: vesselsLoading,
  } = useStandardQuery<{ data: ApiVessel[] }>({
    queryKey: ['vessels-list'],
    queryFn: () => fetch('/api/vessels').then((r) => (r.ok ? r.json() : Promise.reject(r.status))),
    staleTime: 120000,
    retry: 1,
  });
  const { data: liveRouteData } = useStandardQuery<{
    data: {
      vessel: ApiVessel;
      position: { latitude: string; longitude: string; heading: string; speed: string } | null;
      waypoints: Waypoint[];
    };
  }>({
    queryKey: ['vessels-route', selectedVesselId],
    queryFn: () =>
      fetch(`/api/vessels/${selectedVesselId}/route`).then((r) =>
  ...
  const displayVessels: DisplayVessel[] =
    apiVessels && apiVessels.length > 0 ? apiVessels.slice(0, 6).map(mapApiVessel) : DEMO_VESSELS;
  ...
  const waypoints: Waypoint[] =
    liveWaypoints && liveWaypoints.length > 0 ? liveWaypoints : DEMO_ROUTE_WAYPOINTS;
  const isLiveWaypoints = liveWaypoints && liveWaypoints.length > 0;

  const livePosition = liveRouteData?.data?.position;
  const displayHeading = livePosition?.heading
    ? Number(livePosition.heading)
        ...
        <div className="flex items-start gap-2 text-[11px] px-3 py-2.5 rounded-lg border border-amber-500/30 bg-amber-500/8 text-amber-200">
          ...
          <div>
            ...
            <p className="text-amber-300/70 text-[10px] mt-0.5">
              The vessels API responded successfully but returned no records. The page is showing
        ...
        <div className="flex items-start gap-2 text-[11px] px-3 py-2.5 rounded-lg border border-red-500/30 bg-red-500/8 text-red-200">
          ...
          <div>
            ...
            <p className="text-red-300/70 text-[10px] mt-0.5">
              The vessels API request failed. Showing demo content while the connection is restored.
            </p>
          </div>
        </div>
      )}
