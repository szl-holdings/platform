...
export default function CO2EmissionsPage() {
  const { data: vessels = [] } = useStandardQuery({
    queryKey: ['co2-vessels'],
    queryFn: () => dataProvider.getVessels(),
  });
  const { data: emissions = [] } = useStandardQuery({
    queryKey: ['emissions'],
    queryFn: () => dataProvider.getEmissionRecords(),
  });
  const [selectedVessel, setSelectedVessel] = useState<number | null>(null);

  const filteredEmissions = selectedVessel
    ? emissions.filter((e) => e.vesselId === selectedVessel)
    : emissions;
  ...
  filteredEmissions.forEach((e) => {
    if (!monthlyTotals[e.month ?? ''])
      monthlyTotals[e.month ?? ''] = { co2: 0, fuel: 0, distance: 0 };
  ...
  vessels.forEach((v) => {
    ciiDistribution[v.ciiRating ?? ''] = (ciiDistribution[v.ciiRating ?? ''] || 0) + 1;
  });
  const ciiPieData = Object.entries(ciiDistribution).map(([rating, count]) => ({
    name: rating,
    value: count,
  }));

  const vesselEmissions = vessels
    .map((v) => ({
      name: v.name,
    ...
    .filter((v) => v.co2Daily > 0)
    .sort((a, b) => b.co2Daily - a.co2Daily);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between animate-fade-in-up">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1 tracking-wider font-mono">
            MARPOL ANNEX VI REG. 28 // IMO DCS // EU MRV 2015/757 // EU ETS MARITIME
          </p>
        </div>
        <select
                  ...
                  <p className="text-2xl font-bold font-display">
                    {vessels.length > 0
                      ? (() => {
                          ...
                          const avg =
                            vessels.reduce((s, v) => s + (ciiMap[v.ciiRating ?? ''] || 3), 0) /
                            vessels.length;
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold font-display mt-1 text-emerald-400">
                  {(ciiDistribution['A'] || 0) + (ciiDistribution['B'] || 0)}/{vessels.length}
                </p>
              </div>
              ...
              <ResponsiveContainer width="50%" height={220}>
                <PieChart>
                  <Pie
                    ...
                    dataKey="value"
                  >
                    {ciiPieData.map((entry) => (
                      <Cell key={entry.name} fill={ciiColors[entry.name] || '#666'} />
                    ))}
                  </Pie>
                  <Tooltip
                    contentStyle={{
                  ...
                  .map((rating) => (
                    <div key={rating} className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-bold">
                        {ciiDistribution[rating]} vessel{ciiDistribution[rating] > 1 ? 's' : ''}
                      </span>
                    </div>
                  ))}
              </div>
      ...
      <Card className="bg-card border-border animate-fade-in-up stagger-4">
        <CardHeader className="pb-2">
          <CardTitle className="font-display text-base flex items-center gap-2">
            <TrendingDown className="w-5 h-5 text-primary" /> IMO Compliance Pathway (CII Reference
            Line Index)
          </CardTitle>
        </CardHeader>
        <CardContent>
        ...
        <CardContent>
          <div className="space-y-2">
            {vesselEmissions.map((v, i) => (
              <div
                key={i}
                className="flex items-center justify-between p-3 rounded-lg border border-border bg-background/50 hover:border-primary/20 transition-all"
              >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <Badge
                    variant="outline"
                    className={`text-xs ${ciiBadgeColors[v.ciiRating ?? '']}`}
                  >
                    CII {v.ciiRating}
                  </Badge>
