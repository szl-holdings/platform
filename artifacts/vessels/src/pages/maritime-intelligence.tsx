...
function VesselMapCanvas({
  ...
  useEffect(() => {
    ...
    chokepoints.forEach((cp) => {
      ...
      grad.addColorStop(
        0,
        color.replace('0.6', '0.15').replace('0.5', '0.12').replace('0.4', '0.1'),
      );
...
export default function MaritimeIntelligence() {
  const { data: aisVessels = [] } = useStandardQuery({
    queryKey: ['intel-maritime-vessels'],
    queryFn: () => apiFetch<any[]>('/intelligence/maritime/vessels'),
    refetchInterval: 30000,
  });
  const { data: chokepoints = [] } = useStandardQuery({
    queryKey: ['intel-chokepoints'],
    queryFn: () => apiFetch<any[]>('/intelligence/maritime/chokepoints'),
    refetchInterval: 60000,
  });
  const { data: weather = [] } = useStandardQuery({
    queryKey: ['intel-marine-weather'],
    queryFn: () => apiFetch<any[]>('/intelligence/maritime/weather'),
    refetchInterval: 300000,
  });
  const { data: sanctions = [] } = useStandardQuery({
    queryKey: ['intel-sanctions'],
      ...
      <div className="flex items-center justify-between animate-fade-in-up">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1 tracking-wider font-mono">
            AIS VESSEL TRACKING // CHOKEPOINT MONITORING // THREAT ASSESSMENT // SANCTIONS
            COMPLIANCE
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Badge
            variant="outline"
            className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 animate-pulse font-mono text-[10px]"
          >
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold font-display mt-1">
                  <AnimatedCounter value={chokepoints.length} />
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold font-display mt-1 text-orange-400">
                  <AnimatedCounter value={alertChokepoints} />
                </p>
              </div>
              <div
                className={`w-10 h-10 rounded-lg bg-orange-500/10 flex items-center justify-center group-hover:scale-110 transition-transform ${alertChokepoints > 0 ? 'animate-pulse' : ''}`}
              >
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold font-display mt-1 text-red-400">
                  <AnimatedCounter value={sanctions.length} />
                </p>
              </div>
      ...
      <Card className="bg-card border-border animate-fade-in-up stagger-2">
        <CardHeader className="pb-2">
          <CardTitle className="font-display flex items-center gap-2">
            <Navigation className="w-5 h-5 text-primary" /> Live Vessel Tracking & Chokepoint
            Monitor
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          <CardContent>
            <div className="space-y-3">
              {chokepoints.map((cp) => (
                <div
                  key={cp.name}
                  className={`p-3 rounded-lg border transition-all ${cp.riskLevel === 'critical' ? 'border-red-500/20 bg-red-500/5' : cp.riskLevel === 'warning' ? 'border-orange-500/20 bg-orange-500/5' : 'border-border bg-background/50'}`}
                >
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="font-display font-semibold text-sm">{cp.name}</h4>
                    <Badge
                      variant="outline"
                      className={`text-xs ${riskColors[cp.riskLevel] || ''}`}
                    >
                      {cp.riskLevel === 'critical' && (
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs text-muted-foreground">
                    ...
                    <div>
                      ...
                      <span className="font-semibold text-foreground">{cp.dailyTransits}</span>
                    </div>
                    <div>
                      <span className="block text-muted-foreground/60 font-mono text-[9px]">
                        OIL FLOW
                      </span>
                      <span className="font-semibold text-foreground">{cp.oilFlowMbpd} Mb/d</span>
                    </div>
                  </div>
            ...
            <CardContent>
              <div className="space-y-2">
                {(weather as WeatherRow[]).map((w) => (
                  <div
                    key={w.region}
                    className={`p-3 rounded-lg border ${w.warning ? 'border-amber-500/20 bg-amber-500/5' : 'border-border bg-background/50'}`}
                  >
                    ...
                    <div className="grid grid-cols-4 gap-2 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <Wind className="w-3 h-3" /> {w.windSpeed}kt {w.windDirection}
                      </span>
                      <span className="flex items-center gap-1">
                        <Waves className="w-3 h-3" /> {w.waveHeight}m
                      </span>
                      <span className="flex items-center gap-1">
                        <Thermometer className="w-3 h-3" /> {w.seaTemp}°C
                      </span>
                      <span className="flex items-center gap-1">
                        <Eye className="w-3 h-3" /> {w.visibility}
                      </span>
                    </div>
                  </div>
                ))}
                  ...
                  <div key={v.imo} className="p-3 rounded-lg border border-red-500/10 bg-red-500/5">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-[10px] text-muted-foreground/60">
                      ...
                      >
                        ...
                        translateMutation.variables?.imo === v.imo ? (
                    ...
                    {v.entities && v.entities.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {v.entities.map((e, idx) => (
                          <span
                            key={idx}
                            className="text-[10px] px-1.5 py-0.5 rounded bg-red-500/10 text-red-300 border border-red-500/15"
                          >
                            {e.entity}: {e.word}
                          </span>
                        ))}
