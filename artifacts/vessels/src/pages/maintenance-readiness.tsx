          ...
          <div className="mt-3 grid grid-cols-3 gap-2">
            ...
            <div className="bg-sky-500/5 rounded p-2 border border-sky-500/10">
              ...
              <p
                className={cn(
                  'text-[10px] font-mono mt-0.5',
                  item.riskOfServiceIssue >= 80
                    ? 'text-red-400'
                    : item.riskOfServiceIssue >= 60
                      ? 'text-orange-400'
                      : 'text-amber-400',
                )}
              >
...
export default function MaintenanceReadinessPage() {
  ...
  const allItems: DisplayItem[] = useMemo(() => {
    return liveItems.map((item) =>
      adaptMaintenanceItem(item, vesselNameMap.get(item.vesselId) ?? `Vessel ${item.vesselId}`),
    );
  ...
  const backlogCost = allItems
    .filter((m) => m.status !== 'completed')
    .reduce((a, m) => a + m.estimatedCost, 0);
    ...
    return Array.from(map.values())
      .map((v) => {
        ...
        const score = Math.max(
          0,
          health - v.items.filter((i) => i.status === 'overdue').length * 10,
        );
        const state =
          score >= 80 ? 'ready' : score >= 60 ? 'watch' : score >= 40 ? 'limited' : 'unavailable';
                  ...
                  <div className="flex items-center gap-2 text-[9px] text-sky-400/40">
                    ...
                    {v.itemCount > 0 && (
                      <span className="ml-auto text-amber-400/60">
                        {v.itemCount} open item{v.itemCount > 1 ? 's' : ''}
                      </span>
                    )}
                  </div>
                </div>
