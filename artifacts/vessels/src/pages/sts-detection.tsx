...
const STS_EVENTS: StsEvent[] = [
  ...
  },
  {
    ...
    ownerChains: [
      'Atlantic Ridge Corp. — partial opacity',
      'Calabar Maritime Ltd. — Benin registry',
    ],
  },
  {
    id: 'STS-003',
    ...
    <div className="flex items-center gap-2">
      <div className="flex-1 h-1.5 bg-sky-500/10 rounded-full overflow-hidden">
        <div
          className={cn(
            'h-full rounded-full',
            score >= 90 ? 'bg-red-400' : score >= 75 ? 'bg-orange-400' : 'bg-amber-400',
          )}
          style={{ width: `${score}%` }}
        />
      </div>
...
export default function StsDetectionPage() {
  ...
  const filtered =
    statusFilter === 'all' ? STS_EVENTS : STS_EVENTS.filter((e) => e.status === statusFilter);
  ...
  const avgConfidence = Math.round(
    STS_EVENTS.reduce((a, e) => a + e.confidence, 0) / STS_EVENTS.length,
  );
          ...
          <div className="space-y-4">
            <div className="bg-[#0a1628]/80 border border-sky-500/20 rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <Badge
                  variant="outline"
                  className={cn('text-[9px]', statusConfig[selected.status]?.color)}
                >
