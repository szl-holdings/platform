...
const ACTION_TYPES = ["", "login", "logout", "data_access", "config_change", "export", "alert", "voyage_update"];
...
function RiskBadge({ level }: { level: string }) {
  const s = RISK_STYLES[level] ?? { cls: "text-zinc-400 bg-zinc-500/10 border-zinc-500/20", icon: Info };
  return (
    <span className={cn("text-[10px] font-medium px-2 py-0.5 rounded-full border capitalize", s.cls)}>
      {level}
    </span>
  );
}
...
function DEMO_EVENTS(): AuditEvent[] {
  const now = Date.now();
  return [
    { id: 1, actorLabel: "Marcus Wentworth",    action: "Viewed vessel detail: MV Harbour Star",  actionType: "data_access",   domain: "vessels", riskLevel: "low",      outcome: "success", eventHash: "a1b2c3d4", prevHash: "genesis", createdAt: new Date(now - 4 * 60_000).toISOString() },
    { id: 2, actorLabel: "Priya Chandrasekaran", action: "Exported voyage P&L report (Q1 2026)",  actionType: "export",        domain: "vessels", riskLevel: "medium",   outcome: "success", eventHash: "b2c3d4e5", prevHash: "a1b2c3d4", createdAt: new Date(now - 17 * 60_000).toISOString() },
    { id: 3, actorLabel: "system",               action: "AIS feed reconnected after 4m blackout", actionType: "alert",         domain: "vessels", riskLevel: "high",     outcome: "success", eventHash: "c3d4e5f6", prevHash: "b2c3d4e5", createdAt: new Date(now - 38 * 60_000).toISOString() },
    { id: 4, actorLabel: "Nolan Ashford",        action: "Updated sanctions watchlist configuration", actionType: "config_change", domain: "aegis", riskLevel: "high",  outcome: "success", eventHash: "d4e5f6g7", prevHash: "c3d4e5f6", createdAt: new Date(now - 1.5 * 3600_000).toISOString() },
    { id: 5, actorLabel: "Marcus Wentworth",     action: "User login",                             actionType: "login",         domain: "auth",    riskLevel: "low",      outcome: "success", eventHash: "e5f6g7h8", prevHash: "d4e5f6g7", createdAt: new Date(now - 2 * 3600_000).toISOString() },
    { id: 6, actorLabel: "Serena Kowalski",      action: "Generated intelligence brief: Red Sea Corridor", actionType: "data_access", domain: "vessels", riskLevel: "low", outcome: "success", eventHash: "f6g7h8i9", prevHash: "e5f6g7h8", createdAt: new Date(now - 4 * 3600_000).toISOString() },
    { id: 7, actorLabel: "Rafael Oduya",         action: "Submitted maintenance readiness report (MV Tempest)", actionType: "voyage_update", domain: "vessels", riskLevel: "medium", outcome: "success", eventHash: "g7h8i9j0", prevHash: "f6g7h8i9", createdAt: new Date(now - 6 * 3600_000).toISOString() },
    { id: 8, actorLabel: "system",               action: "Invoice payment confirmed: INV-2026-003", actionType: "data_access",  domain: "billing", riskLevel: "low",      outcome: "success", eventHash: "h8i9j0k1", prevHash: "g7h8i9j0", createdAt: new Date(now - 8 * 3600_000).toISOString() },
    { id: 9, actorLabel: "Ingrid Halvorsen",     action: "Failed login attempt — 2FA required",   actionType: "login",         domain: "auth",    riskLevel: "critical", outcome: "failure", eventHash: "i9j0k1l2", prevHash: "h8i9j0k1", createdAt: new Date(now - 12 * 3600_000).toISOString() },
    { id: 10,actorLabel: "Marcus Wentworth",     action: "Invited new team member: i.halvorsen@szlholdings.com", actionType: "config_change", domain: "auth", riskLevel: "medium", outcome: "success", eventHash: "j0k1l2m3", prevHash: "i9j0k1l2", createdAt: new Date(now - 24 * 3600_000).toISOString() },
  ];
}
...
export default function AuditLogPanelPage() {
  ...
  const usingFallback = rawEvents.length === 0 && page === 0 && !search && !actionTypeFilter && !riskFilter;
  ...
  const hasNextPage = usingFallback ? false : (total > 0 ? (page + 1) * pageSize < total : events.length >= pageSize);
  ...
  return (
    <div className="p-6 space-y-5 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-sky-400/50 mt-0.5">
            Hash-chained compliance ledger · {displayTotal > 0 ? `${displayTotal} events` : "immutable record of all user and system actions"}
          </p>
        </div>
        <button
          onClick={() => refetch()}
      ...
      <div className="flex items-center gap-2 bg-emerald-500/5 border border-emerald-500/15 rounded-lg px-4 py-2.5">
        ...
        <p className="text-xs text-emerald-300">Chain integrity verified — SHA-256 hash-linked, tamper-evident ledger</p>
      </div>
      ...
      <div className="bg-[#0a1628]/80 border border-sky-500/10 rounded-xl overflow-hidden">
        ...
        {isLoading ? (
          <div className="p-5 space-y-2.5 animate-pulse">
            {[1, 2, 3, 4, 5].map(i => <div key={i} className="h-12 bg-sky-500/5 rounded-lg" />)}
          </div>
        ) : events.length === 0 ? (
          ...
          events.map(ev => (
            ...
            >
              ...
              <span className={cn("text-[11px] font-medium capitalize", DOMAIN_COLORS[ev.domain] ?? "text-sky-400/60")}>
                {ev.domain}
              </span>
              <RiskBadge level={ev.riskLevel} />
              <span className="text-[11px] text-sky-400/50 whitespace-nowrap">{relTime(ev.createdAt)}</span>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-sky-500/10">
          ...
          <span className="text-[10px] text-sky-400/40">
            Page {page + 1}{displayTotal > 0 ? ` · ${displayTotal} total events` : ""}
          </span>
          <button
