      ...
      <div className="flex items-center gap-2 mt-0.5 pl-3.5 text-[9px] text-sky-400/40">
        ...
        {(vessel.etaDelta ?? 0) !== 0 && (
          <span
            className={cn(
              'ml-auto',
              (vessel.etaDelta ?? 0) < 0 ? 'text-emerald-400/60' : 'text-orange-400/60',
            )}
          >
          ...
          return (
            <g key={v.id}>
              ...
              <circle
                ...
                fill={color}
                filter={isSelected ? 'url(#cmd-glow)' : undefined}
              />
...
export default function CommandModePage() {
  ...
  const vesselExceptions = activeVessel
    ? fleetExceptions.filter((e) => e.vesselId === activeVessel.id && e.status === 'active')
    : [];
  const vesselMaint = activeVessel
    ? maintenanceItems.filter((m) => m.vesselId === activeVessel.id && m.status !== 'completed')
    : [];
  const criticalCount = fleetExceptions.filter(
      ...
      <div className="px-4 py-2 border-b border-sky-500/10 flex items-center gap-3 shrink-0">
        ...
        <div className="ml-auto flex items-center gap-4">
          {[
            ...
            {
              label: 'At Sea',
              value: vessels.filter((v) => v.status === 'at_sea').length,
              color: 'text-emerald-400',
            },
            {
              label: 'Delayed',
              value: vessels.filter((v) => ['delayed', 'exception_active'].includes(v.status))
                .length,
        ...
        <div className="flex-1 flex flex-col overflow-hidden p-3 gap-3">
          ...
          <div className="grid grid-cols-4 gap-2 shrink-0">
            {[
              {
                label: 'At Sea',
                count: vessels.filter((v) => v.status === 'at_sea').length,
                color: '#22c55e',
              },
              {
              ...
              },
              {
                ...
                count: vessels.filter((v) => ['delayed', 'exception_active'].includes(v.status))
                  .length,
                color: '#f97316',
              },
              {
                label: 'Maintenance',
                count: vessels.filter((v) => v.status === 'maintenance').length,
                color: '#ef4444',
              },
            ].map((s) => (
              <div
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  {[
                    {
                      label: 'Status',
                      value: statusLabels[activeVessel.status] ?? activeVessel.status,
                    },
