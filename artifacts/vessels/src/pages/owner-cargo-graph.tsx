...
const TYPE_CONFIG: Record<string, { color: string; bg: string; border: string; icon: React.FC<React.SVGProps<SVGSVGElement>>; label: string }> = {
  vessel:    { color: "#38bdf8", bg: "rgba(56,189,248,0.08)",  border: "rgba(56,189,248,0.25)",  icon: Anchor,     label: "Vessel" },
  owner:     { color: "#a78bfa", bg: "rgba(167,139,250,0.08)", border: "rgba(167,139,250,0.25)", icon: Building2,  label: "Owner" },
  charterer: { color: "#34d399", bg: "rgba(52,211,153,0.08)",  border: "rgba(52,211,153,0.25)",  icon: Building2,  label: "Charterer" },
  port:      { color: "#fbbf24", bg: "rgba(251,191,36,0.08)",  border: "rgba(251,191,36,0.25)",  icon: MapPin,     label: "Port" },
  cargo:     { color: "#fb923c", bg: "rgba(251,146,60,0.08)",  border: "rgba(251,146,60,0.25)",  icon: Package,    label: "Cargo" },
};
...
function NodeCard({ node, selected, onClick }: { node: GraphNode; selected: boolean; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className={cn("text-left rounded-xl p-3 transition-all border", selected ? "ring-2 ring-sky-400/50" : "")}
      style={{ background: selected ? cfg.bg : "rgba(15,31,56,0.7)", borderColor: isSanctioned ? "rgba(239,68,68,0.4)" : selected ? cfg.border : "rgba(255,255,255,0.06)" }}
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            ...
            {node.cargoCategory && <span className="text-[9px] text-orange-400/60 capitalize">{node.cargoCategory.replace(/_/g, " ")}</span>}
          </div>
        </div>
        {isSanctioned && <AlertTriangle className="w-3.5 h-3.5 text-red-400 mt-0.5 flex-shrink-0" />}
        {node.hazardClass && !isSanctioned && <AlertTriangle className="w-3.5 h-3.5 text-amber-400 mt-0.5 flex-shrink-0" />}
        {node.riskTier && !isSanctioned && !node.hazardClass && (
          <div className="w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0" style={{ background: RISK_COLORS[node.riskTier] ?? "#888" }} />
        )}
      </div>
      {node.provenance && (
        <div className="mt-1.5 flex items-center gap-1">
          ...
          <span className="text-[9px] text-sky-400/40">{Math.round(node.provenance.confidence * 100)}% conf.</span>
        </div>
      )}
    </button>
  );
...
function EdgeList({ edges, nodes, sourceId }: { edges: GraphEdge[]; nodes: GraphNode[]; sourceId: string }) {
...
export default function OwnerCargoGraphPage() {
  ...
  async function load() {
    ...
    } catch { /* non-fatal — API may require auth */ }
    setLoading(false);
  }
    ...
    <div style={{ padding: "28px 28px 48px", maxWidth: 1400, margin: "0 auto" }}>
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <Badge variant="outline" className="text-[9px] border-sky-500/30 text-sky-400/70">CONSTELLATION</Badge>
          </div>
          <p className="text-xs text-sky-400/60">
            Vessel ownership chains, beneficial owners, charterers, cargo manifests, and port call relationships — rendered via CONSTELLATION with full provenance.
          </p>
        </div>
      ...
      {data && (
        <div className="grid grid-cols-7 gap-3 mb-5">
          {[
            ...
            { label: "Hazardous Cargo", value: data.stats.hazardousCargoVessels ?? 0, color: "#fbbf24" },
            { label: "Sanction Exposure", value: data.stats.sanctionExposureVessels, color: "#f87171" },
            { label: "High-Risk Owners", value: data.stats.highRiskOwners, color: "#c084fc" },
            { label: "Total Edges", value: data.stats.totalEdges ?? edges.length, color: "#94a3b8" },
          ].map(s => (
            <div key={s.label} className="rounded-xl p-3 border border-sky-500/10" style={{ background: "rgba(10,22,40,0.8)" }}>
              <div className="text-[9px] text-sky-400/50 uppercase tracking-wider mb-1">{s.label}</div>
      ...
      <div className="flex gap-2 mb-5">
        {(["all", "vessel", "owner", "charterer", "cargo", "port"] as const).map(t => (
          ...
          >{t === "all" ? "All Entity Types" : t}</button>
        ))}
      </div>
            ...
            {selected ? (
              <>
                ...
                <div className="space-y-3 text-[11px]">
                  {selected.flag && <div className="flex justify-between"><span className="text-sky-400/50">Flag State</span><span className="text-sky-200">{selected.flag}</span></div>}
                  {selected.country && <div className="flex justify-between"><span className="text-sky-400/50">Country</span><span className="text-sky-200">{selected.country}</span></div>}
                  {selected.imo && <div className="flex justify-between"><span className="text-sky-400/50">IMO</span><span className="text-sky-200 font-mono">{selected.imo}</span></div>}
                  {selected.subtype && <div className="flex justify-between"><span className="text-sky-400/50">Vessel Type</span><span className="text-sky-200">{selected.subtype}</span></div>}
                  {selected.riskTier && (
                    <div className="flex justify-between">
                      <span className="text-sky-400/50">Risk Tier</span>
                      <span style={{ color: RISK_COLORS[selected.riskTier] }} className="font-medium capitalize">{selected.riskTier}</span>
                    </div>
                  )}
                  {selected.creditRating && <div className="flex justify-between"><span className="text-sky-400/50">Credit Rating</span><span className="text-sky-200">{selected.creditRating}</span></div>}
                  {selected.congestionLevel && (
                    <div className="flex justify-between">
                      <span className="text-sky-400/50">Port Congestion</span>
                      <span className="capitalize" style={{ color: selected.congestionLevel === "high" ? "#f87171" : selected.congestionLevel === "medium" ? "#fbbf24" : "#34d399" }}>{selected.congestionLevel}</span>
                    </div>
                  )}
                  {selected.cargoCategory && (
                    <div className="flex justify-between">
                      <span className="text-sky-400/50">Cargo Category</span>
                      <span className="text-orange-300 capitalize">{selected.cargoCategory.replace(/_/g, " ")}</span>
                    </div>
                  )}
                  {selected.cargoTonnes !== undefined && (
                    <div className="flex justify-between">
                      <span className="text-sky-400/50">Cargo Tonnes</span>
                      <span className="text-sky-200">{selected.cargoTonnes.toLocaleString()} mt</span>
                    </div>
                  )}
                  ...
                  {selected.provenance && (
                    <div className="pt-2 border-t border-sky-500/10">
                      ...
                      <div className="flex items-center gap-1.5">
                        {selected.provenance.verifierApproved ? <CheckCircle2 className="w-3 h-3 text-emerald-400" /> : <Shield className="w-3 h-3 text-amber-400" />}
                        <span className="text-[10px] text-sky-300/60">{selected.provenance.attestation}</span>
                      </div>
                      <div className="text-[10px] text-sky-400/40 mt-0.5">{Math.round(selected.provenance.confidence * 100)}% confidence</div>
                      <div className="text-[10px] text-sky-400/30 mt-0.5">{new Date(selected.provenance.freshness.fetchedAt).toLocaleString()}</div>
                    </div>
                  )}
                  <div className="pt-2 border-t border-sky-500/10">
              ...
              <div className="flex flex-col items-center justify-center h-48 text-center">
                ...
                <p className="text-sky-400/25 text-xs mt-1">Owners, vessels, charterers, cargo, ports</p>
              </div>
            )}
          </div>

          <div className="rounded-xl border border-sky-500/10 p-4" style={{ background: "rgba(10,22,40,0.8)" }}>
            <div className="text-[10px] text-sky-400/50 uppercase tracking-wider mb-3">Entity Legend</div>
