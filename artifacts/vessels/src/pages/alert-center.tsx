...
export default function AlertCenterPage() {
  ...
  const {
    ...
    isError: alertsError,
  } = useStandardQuery({ queryKey: ['alerts'], queryFn: api.alerts.list });
  const { data: alertRulesRaw = [] } = useStandardQuery({
    queryKey: ['alertRules'],
    queryFn: api.alertRules.list,
  });
  const { data: vesselsRaw = [] } = useStandardQuery({
    queryKey: ['vessels'],
    ...
    if (activeAlerts.length > 0) {
      ...
      doctrineEventBus.emit({
        ...
        context: {
          ...
          operationalImpact:
            'Active alert monitoring underway; fleet movements may require rerouting',
          layer: 'OBSERVE',
          timestamp: Date.now(),
        },
    ...
    <div className="p-6 space-y-6">
      <div className="animate-fade-in-up">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Active alert stream, rule configuration, and escalation thresholds across fleet operations
        </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card
          className={`bg-card border-border animate-fade-in-up stagger-1 hover:border-chart-5/20 transition-all duration-300 group ${activeAlerts.length > 0 ? 'ring-1 ring-chart-5/10' : ''}`}
        >
          <CardContent className="p-4 flex items-center justify-between">
            ...
            <div
              className={`w-10 h-10 rounded-lg bg-chart-5/10 flex items-center justify-center group-hover:scale-110 transition-transform ${activeAlerts.length > 0 ? 'animate-pulse' : ''}`}
            >
        ...
        <Card className="bg-card border-border animate-fade-in-up stagger-3 hover:border-chart-2/20 transition-all duration-300 group">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold font-display mt-1">
                <AnimatedCounter value={alertRules.length} />
              </p>
            </div>
          ...
          {loadingAlerts ? (
            ...
            alerts.map((alert, i: number) => {
              ...
              return (
                <Card
                  key={alert.id}
                  className={`bg-card border-border transition-all duration-300 animate-fade-in-up stagger-${Math.min(i + 1, 8)} ${isCritical && isActive ? 'ring-1 ring-red-500/20' : 'hover:border-primary/20'}`}
                >
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-3">
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center ${alert.severity === 'critical' ? 'bg-red-500/10' : alert.severity === 'high' ? 'bg-orange-500/10' : 'bg-amber-500/10'} ${isCritical && isActive ? 'animate-pulse' : ''}`}
                        >
                          <AlertTriangle
          ...
          <div className="flex justify-end">
            <Dialog open={ruleOpen} onOpenChange={setRuleOpen}>
              ...
              <DialogContent className="bg-card border-border">
                <DialogHeader>
                  <DialogTitle className="font-display">Create Alert Rule</DialogTitle>
                </DialogHeader>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setRuleForm((p) => ({ ...p, ruleType: v }))}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setRuleForm((p) => ({ ...p, severity: v }))}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {rule.description || `${rule.ruleType} based alert rule`}
                      </p>
                      <div className="flex items-center gap-2 mt-2">
