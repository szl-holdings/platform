    ...
    >
      ...
      <div className="absolute top-3 right-3 z-10 flex items-center gap-3">
        ...
        <span className="flex items-center gap-1 text-[9px] text-red-400">
          <span className="w-5 h-0.5 border-t-2 border-dashed border-red-400 inline-block" />{' '}
          Anomaly
        </span>
      </div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%" style={{ display: 'block' }}>
        ...
        {[-60, -30, 0, 30, 60].map((lat) => (
          <line
            ...
            y2={latToY(lat, H)}
            stroke="#1e3a5f"
            strokeWidth={lat === 0 ? 0.8 : 0.3}
            strokeDasharray={lat === 0 ? 'none' : '3,6'}
          />
        ))}
          ...
          return (
            <g key={i}>
              ...
              <circle
                ...
                cy={cy}
                r={2.5}
                fill={p.congested ? '#f97316' : '#38bdf8'}
                fillOpacity={0.9}
              />
              <text
                x={cx + 4}
...
const COMMODITIES = [
  {
    ...
    anomalyFlag: true,
    anomaly:
      'Unusual fleet repositioning: 23 VLCCs shifted from Persian Gulf to Atlantic basin — possible demand signal ahead of summer driving season',
  },
  {
    id: 'lng',
    name: 'LNG',
    ...
    anomalyFlag: true,
    anomaly:
      'Volume decline 4.8% YoY. European terminal bookings down 18% — spot LNG prices contracting. Potential overstock in NW Europe',
  },
  {
    id: 'grain',
    name: 'Grain / Dry Bulk',
    ...
    anomalyFlag: true,
    anomaly:
      'Volume spike +8.1%: Black Sea grain corridor re-opened for 30-day window. Rapid charter activity observed. Price arbitrage opportunity',
  },
  {
    id: 'containers',
    name: 'Containers',
...
const FLOW_ROUTES = [
  {
    ...
    anomaly: true,
    anomalyText:
      'Fleet concentration unusually high — 89 VLCCs vs 72 historical average. Possible demand pull from Chinese SPR build',
  },
  {
    id: 'us-europe',
  ...
  },
  {
    ...
    anomaly: true,
    anomalyText:
      'Volume surge +14.2% in 60 days. Route uses ship-to-ship transfers near Kalamata and Lakshadweep. Sanctions exposure for cargo buyers',
  },
  {
    id: 'australia-china',
  ...
  },
  {
    ...
    anomaly: true,
    anomalyText:
      'Corridor opened for 30-day window. Bulk carrier bookings up 82%. Short-window arbitrage — expect rapid reversal if corridor closes. Monitor political developments',
  },
  {
    id: 'red-sea',
    ...
    anomaly: true,
    anomalyText:
      'Volume down 23.4% — active Houthi threat diverting 420+ ships via Cape of Good Hope since Jan 2024. Average voyage time +11 days',
  },
];
...
export default function TradeFlowHeatmap() {
  ...
  const anomalyCount =
    COMMODITIES.filter((c) => c.anomalyFlag).length + FLOW_ROUTES.filter((r) => r.anomaly).length;
                  ...
                  <div className="text-right shrink-0">
                    <p
                      className={cn(
                        ...
                        route.change >= 0 ? 'text-emerald-400' : 'text-red-400',
                      )}
                    >
                      {route.change >= 0 ? '+' : ''}
                      {route.change}%
