  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>
        ...
        <span>·</span>
        <span
          style={{ color: COMPLIANCE_COLOR[vessel.complianceStatus] || 'rgba(255,255,255,0.4)' }}
        >
        ...
        <div className="mt-2 flex flex-wrap gap-1">
          {vessel.anomalyFlags.map((f) => (
            <span
              ...
              style={{ background: '#c04a2a15', color: '#c04a2a' }}
            >
              {f.replace('_', ' ')}
            </span>
          ))}
...
export default function VoyageDesk() {
  ...
  const activeVoyage = voyages.find(
    (v) => v.status === 'active' || v.status === 'exception' || v.status === 'deviating',
  );

  return (
    <div className="flex h-full" style={{ background: 'hsl(210 15% 7%)' }}>
      <aside
        className="w-72 flex-shrink-0 border-r overflow-y-auto p-4 space-y-3"
      ...
      >
        ...
        {vesselTwins.map((v) => (
          <VesselCard
            ...
            onSelect={() => setSelectedVesselId(v.id)}
          />
        ))}
      </aside>
      ...
      {vessel ? (
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              ...
              { label: 'Crew', value: `${vessel.crewCount}`, color: 'rgba(255,255,255,0.8)' },
              {
                ...
                value: `${vessel.pscDeficiencies}`,
                color: vessel.pscDeficiencies > 0 ? '#c08a2c' : '#40856a',
              },
              {
                label: 'Compliance',
                value: vessel.complianceStatus.replace('_', ' '),
                color: COMPLIANCE_COLOR[vessel.complianceStatus],
              },
            ].map((m) => (
              <div
                key={m.label}
              ...
              >
                ...
                <div className="text-lg font-bold capitalize" style={{ color: m.color }}>
                  {m.value}
                </div>
              </div>
            ))}
          ...
          {activeVoyage && (
            <div
              ...
              style={{
                background:
                  activeVoyage.status === 'exception' ? '#9b1c1c08' : 'rgba(255,255,255,0.02)',
                borderColor:
                  activeVoyage.status === 'exception' ? '#9b1c1c30' : 'rgba(255,255,255,0.06)',
              }}
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  <BalticPill
                    voyageTce={activeVoyage.economics.tcEquivalent}
                    cargo={activeVoyage.cargo}
                  />
                  <span
                    className="text-xs px-2 py-0.5 rounded-full"
                    style={{
              ...
              <div className="grid grid-cols-4 gap-3 mb-4">
                {[
                  ...
                  {
                    label: 'TC Equiv',
                    value: `$${activeVoyage.economics.tcEquivalent.toLocaleString()}/d`,
                  },
                  {
                    label: 'Margin',
                    value: `${activeVoyage.economics.profitMarginPct.toFixed(1)}%`,
                    color: activeVoyage.economics.profitMarginPct > 20 ? '#40856a' : '#c08a2c',
                  },
                ].map((m) => (
                  <div
                    key={m.label}
                  ...
                  >
                    ...
                    <div
                      className="text-sm font-semibold"
                      style={{ color: m.color ?? 'rgba(255,255,255,0.8)' }}
                    >
                      {m.value}
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  {activeVoyage.deviations.map((d, i) => (
                    <div
                      ...
                      style={{
                        background: d.resolved ? '#40856a10' : '#c04a2a10',
                        border: `1px solid ${d.resolved ? '#40856a25' : '#c04a2a25'}`,
                      }}
                    >
                      <AlertTriangle
                        size={14}
                        style={{
                          color: d.resolved ? '#40856a' : '#c04a2a',
                          flexShrink: 0,
                          marginTop: 1,
                        }}
                      />
                      <div className="flex-1 text-xs">
                        <span
                          ...
                          style={{ color: d.resolved ? '#40856a' : '#c04a2a' }}
                        >
                          {d.type.replace('_', ' ')}:
                        </span>
        ...
        <div className="flex-1 flex items-center justify-center">
          <EmptyState
            icon={Anchor}
            headline="Select a vessel"
            description="Choose a vessel from the fleet to view its twin."
            accentColor={ACCENT}
          />
        </div>
      )}
