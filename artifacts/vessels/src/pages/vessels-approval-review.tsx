...
} from '@szl-holdings/shared-ui/decision-receipt-card';
...
import { type VoyageApproval, voyageTwins } from '@/data/fleet-twin';
...
function buildReceiptPayload(approval: FlatApproval, action: string): ReceiptPayload {
  ...
  return {
    ...
    aiRecommendation: {
      ...
      confidence:
        approval.priority === 'critical' ? 0.72 : approval.priority === 'high' ? 0.81 : 0.91,
      riskLevel:
        approval.priority === 'critical' ? 'P0' : approval.priority === 'high' ? 'P1' : 'P2',
      ...
      evidenceRefs: [
        ...
        },
        {
          ...
          content:
            'Port of call and counterparty screened against OFAC, EU, UN sanctions lists — no matches.',
          relevanceScore: 0.94,
        },
        {
    ...
    alternativesConsidered: [
      {
        label:
          action === 'approve' ? 'Reject and request re-documentation' : 'Approve with conditions',
        description:
          action === 'approve'
...
function ApprovalCard({
  ...
  async function handleAction(action: string) {
    ...
    try {
      ...
      alert(
        `This decision cannot be completed — the receipt could not be recorded: ${message}. Governed actions require a persisted audit record.`,
      );
    } finally {
      setSubmitting(false);
    }
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <span
                ...
                style={{ background: 'hsl(205 70% 38% / 0.15)', color: ACCENT }}
              >
            ...
            <div className="text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.4)' }}>
              {approval.vesselName} · {approval.voyageNumber} · {approval.requestedBy} ·{' '}
                ...
                decision={{
                  ...
                  whatNeedsToChange: [
                    ...
                    approval.priority === 'critical'
                      ...
                      : 'Review voyage documentation before deciding',
                  ],
                }}
              />
            </div>
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            {receiptCount > 0 && (
              <span
                ...
                style={{ background: '#40856a20', color: '#40856a' }}
              >
                <Receipt size={10} />
          ...
          <p className="text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>
            Voyage approval requests — route deviations, port clearances, exception escalations, and
            export packets
          </p>
        </div>
      </div>
        ...
        {['all', 'pending', 'approved', 'rejected'].map((s) => {
          ...
          return (
            <button
              ...
              style={{
                background:
                  statusFilter === s ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.01)',
                borderColor:
                  statusFilter === s ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,0.04)',
              }}
            >
              <div className="text-lg font-bold" style={{ color: 'rgba(255,255,255,0.75)' }}>
                {count}
              </div>
              <div
      ...
      {receiptCount > 0 && (
        <div
          ...
          style={{ background: 'rgba(64,133,106,0.04)', borderColor: '#40856a20' }}
        >
          <div className="flex items-center gap-2">
            <Shield size={13} style={{ color: '#40856a' }} />
            <p className="text-xs" style={{ color: 'rgba(255,255,255,0.55)' }}>
              <span className="font-medium" style={{ color: '#40856a' }}>
                {receiptCount} decision receipt{receiptCount !== 1 ? 's' : ''}
              </span>{' '}
              generated for this session. Each receipt is cryptographically hashed and stored in the
              audit trail. Receipts are downloadable from the card below.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4">
        {displayed.length === 0 ? (
          <EmptyState
            ...
            description="No approvals match this filter."
            accentColor={ACCENT}
          />
        ) : (
          displayed.map((a) => (
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="text-sm font-medium" style={{ color: 'rgba(255,255,255,0.5)' }}>
            Voyage Export Packet / Audit Trail
          </h3>
        </div>
        <p className="text-xs mb-3" style={{ color: 'rgba(255,255,255,0.35)' }}>
          Generate a voyage export packet — voyage summary, approval log, deviation record, and
          audit chain. Immutable once generated.
          {receiptCount > 0 &&
            ` Includes ${receiptCount} decision receipt${receiptCount !== 1 ? 's' : ''} from this session.`}
        </p>
        <button
