...
export default function InfrastructurePage() {
  const { data: vessels = [] } = useStandardQuery({
    queryKey: ['infra-vessels'],
    queryFn: () => dataProvider.getVessels(),
  });
  const { data: maintenanceLogs = [] } = useStandardQuery({
    queryKey: ['maintenance-logs'],
    queryFn: () => dataProvider.getMaintenanceLogs(),
  });
  ...
  const avgEngine =
    vessels.length > 0
      ? Math.round(vessels.reduce((s, v) => s + (v.engineHealth ?? 0), 0) / vessels.length)
      : 0;
  const avgHull =
    vessels.length > 0
    ...
    <div className="p-6 space-y-6">
      <div className="animate-fade-in-up">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Engine telemetry, hull fouling index, dry-dock schedules, and predictive maintenance
          alerts
        </p>
      </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p
                  className={`text-2xl font-bold font-display mt-1 ${avgEngine >= 85 ? 'text-emerald-400' : avgEngine >= 70 ? 'text-amber-400' : 'text-red-400'}`}
                >
                  {avgEngine}%
                </p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p
                  className={`text-2xl font-bold font-display mt-1 ${avgHull >= 85 ? 'text-emerald-400' : avgHull >= 70 ? 'text-amber-400' : 'text-red-400'}`}
                >
                  {avgHull}%
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold font-display mt-1">
                  {inProgressCount + scheduledCount + overdueCount}
                </p>
                {overdueCount > 0 && (
              ...
              <div
                className={`w-10 h-10 rounded-lg bg-orange-500/10 flex items-center justify-center group-hover:scale-110 transition-transform ${overdueCount > 0 ? 'animate-pulse' : ''}`}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in-up stagger-2">
        {vessels
          .filter(
            (v) =>
              (v.engineHealth ?? 0) < 85 ||
              (v.hullCondition ?? 0) < 80 ||
              (v.maintenanceScore ?? 0) < 75,
          )
          .map((v) => (
            <Card
              key={v.id}
              className={`bg-card border-border hover:border-primary/20 transition-all ${(v.engineHealth ?? 0) < 70 || (v.hullCondition ?? 0) < 70 ? 'ring-1 ring-red-500/20' : ''}`}
            >
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  ...
                  <Badge
                    variant="outline"
                    className={`text-xs ${v.status === 'maintenance' ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'}`}
                  >
                    {v.status.replace('_', ' ')}
                  </Badge>
                </div>
              ...
              <CardContent className="space-y-3">
                ...
                <HealthGauge
                  value={v.maintenanceScore ?? 0}
                  label="Maintenance Score"
                  color="chart-3"
                />
                <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground pt-2 border-t border-border">
                  <div>
                    Type: <span className="text-foreground capitalize">{v.vesselType}</span>
                  </div>
                  <div>
                    Built: <span className="text-foreground">{v.yearBuilt}</span>
                  </div>
                  <div>
                    GT:{' '}
                    <span className="text-foreground">
                      {(v.grossTonnage ?? 0).toLocaleString()}
                    </span>
                  </div>
                  <div>
                    EEXI: <span className="text-foreground">{v.eexi}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            {maintenanceLogs
              ...
              .map((log) => {
                ...
                return (
                  <div
                    key={log.id}
                    className={`p-4 rounded-lg border transition-all ${log.status === 'Overdue' ? 'border-red-500/20 bg-red-500/5' : log.status === 'In Progress' ? 'border-amber-500/20 bg-amber-500/5' : 'border-border bg-background/50'}`}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-3">
                        <StatusIcon
                          className={`w-4 h-4 mt-0.5 ${log.status === 'Completed' ? 'text-emerald-400' : log.status === 'Overdue' ? 'text-red-400' : log.status === 'In Progress' ? 'text-amber-400' : 'text-muted-foreground'}`}
                        />
                        <div>
                          <p className="text-sm font-semibold">{log.component}</p>
                          <p className="text-xs text-muted-foreground mt-0.5">
                            {log.vesselName} · {log.type}
                          </p>
                      ...
                      <div className="flex items-center gap-2">
                        <Badge
                          variant="outline"
                          className={`text-xs ${log.severity ? (severityColors[log.severity] ?? '') : ''}`}
                        >
                          {log.severity}
                        </Badge>
                        <Badge
                          variant="outline"
                          className={`text-xs ${log.status === 'Completed' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : log.status === 'Overdue' ? 'bg-red-500/10 text-red-400 border-red-500/20' : log.status === 'In Progress' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-muted text-muted-foreground'}`}
                        >
                          {log.status}
                        </Badge>
                      </div>
                    ...
                    <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground ml-7">
                      <span>
                        Scheduled: {log.date ? new Date(log.date).toLocaleDateString() : '—'}
                      </span>
