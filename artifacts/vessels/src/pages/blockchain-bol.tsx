    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={() => refetch()}
            className="text-sky-400/40 hover:text-sky-300 transition-colors"
          >
      ...
      {showCreate && (
        <div className="bg-[#0a1628]/80 border border-sky-500/20 rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-sky-200 flex items-center gap-1.5">
              <PlusCircle className="w-3.5 h-3.5 text-sky-400" />
              Create New Bill of Lading
            </p>
            <button onClick={() => setShowCreate(false)}>
              <X className="w-4 h-4 text-sky-400/40 hover:text-sky-300" />
            </button>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {[
              ...
              <div key={key}>
                <label className="text-[10px] text-sky-400/50 uppercase tracking-wider block mb-1">
                  {label}
                </label>
                <input
      ...
      {verifyResult && (
        ...
        >
          ...
          <button
            ...
            className="text-sky-400/30 hover:text-sky-300"
          >
            ...
            {docs.map((doc) => (
              <button
                ...
                className={cn(
                  ...
                  selectedId === doc.id ? 'border-sky-500/30' : 'border-sky-500/10',
                )}
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="grid grid-cols-2 gap-1 text-[10px]">
                      <div>
                        <span className="text-sky-400/40">Cargo:</span>{' '}
                        <span className="text-sky-300">{doc.cargo}</span>
                      </div>
                      <div>
                        <span className="text-sky-400/40">Qty:</span>{' '}
                        <span className="text-sky-300">{doc.quantity}</span>
                      </div>
                      <div>
                        <span className="text-sky-400/40">From:</span>{' '}
                        <span className="text-sky-300">{doc.originPort}</span>
                      </div>
                      <div>
                        <span className="text-sky-400/40">To:</span>{' '}
                        <span className="text-sky-300">{doc.destinationPort}</span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="space-y-4">
                ...
                <div className="bg-[#0a1628]/80 border border-sky-500/20 rounded-xl p-4">
                  ...
                  <div className="space-y-2 text-[10px]">
                    {[
                      ...
                      <div
                        key={r.label}
                        className="flex justify-between py-1 border-b border-sky-500/5 last:border-0"
                      >
                        <span className="text-sky-400/40">{r.label}</span>
                        <span className="text-sky-200 font-mono text-right max-w-[60%] truncate">
                          {r.val}
                        </span>
                      </div>
                    ))}
                    ...
                    signActionLabel && (
                      <div className="mt-4 pt-3 border-t border-sky-500/10 space-y-2">
                        ...
                        <p className="text-[10px] text-sky-400/40">
                          ...
                          <span className="text-sky-300 font-mono">{user.name}</span> ({user.role}).
                        </p>
                        <button
                          onClick={() => {
                            setSignError(null);
            ...
            {!selectedId && (
              <div className="bg-[#0a1628]/40 border border-sky-500/10 rounded-xl p-6 text-center">
                ...
                <p className="text-xs text-sky-400/30">
      ...
      {!isLoading && !isError && tab === 'chain' && (
        <div className="bg-[#0a1628]/80 border border-sky-500/10 rounded-xl p-4">
          <p className="text-xs font-semibold text-sky-200 mb-4 flex items-center gap-1.5">
            ...
            {selectedDoc && (
              <span className="text-sky-400/30 font-normal ml-2">· {selectedDoc.id}</span>
            )}
          </p>
          {!selectedId ? (
            <p className="text-xs text-sky-400/30 text-center py-8">
                ...
                <div key={i} className="flex gap-3">
                  ...
                  <div className="pb-3">
                    ...
                    <div className="flex items-center gap-3 mt-1 flex-wrap">
                      ...
                      <span className="text-[9px] font-mono text-sky-400/20">
