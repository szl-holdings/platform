...
function getVesselType(type: string, typeCode?: number): VesselType {
  ...
  if (t.includes('container') || t.includes('ship') || (code >= 70 && code <= 79))
    return 'container';
  ...
  return (
    <div className="relative w-full h-full overflow-hidden" style={{ background: '#061018' }}>
        ...
        {vessels.map((vessel) => {
          ...
          return (
            <g
              ...
              onMouseEnter={() => setHovered(vessel.id)}
            ...
            >
              ...
              {vessel.alerts.length > 0 && !vessel.aisDark && !vessel.sanctionsFlag && (
                <circle
                  ...
                  cy={y - 5}
                  r={2.5}
                  fill={SEVERITY_CONFIG[vessel.alerts[0]].color}
                />
              )}
      ...
      <div className="absolute bottom-3 right-3 text-[9px] font-mono" style={{ color: TEXT.muted }}>
        {vessels.some((v) => v.source !== 'cached') ? 'AIS LIVE' : 'AIS CACHED'} · {vessels.length}{' '}
        vessels · click marker to select
      </div>
    </div>
  );
}
...
function DetailRow({ label, value, color }: DetailRowProps) {
  return (
    ...
    >
      ...
      <span className="text-[11px] font-mono" style={{ color: color ?? TEXT.secondary }}>
        {value}
      </span>
    </div>
  );
...
function VesselDetail({ vessel }: { vessel: LiveVessel }) {
  ...
  return (
    <div
      ...
      style={{ background: BG.surface, borderLeft: `1px solid ${BORDER.subtle}` }}
    >
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          <div className="text-right shrink-0">
            <p
              ...
              style={{
                color:
                  vessel.riskScore > 70 ? '#ef4444' : vessel.riskScore > 40 ? '#f97316' : '#22c55e',
              }}
            >
              {vessel.riskScore}
            </p>
        ...
        {vessel.riskScore > 50 && (
          ...
          >
            ...
            <p className="text-[11px]" style={{ color: '#f97316' }}>
              Risk score {vessel.riskScore} — elevated monitoring active
            </p>
          </div>
        )}
      </div>
      <div className="px-5 py-4">
        ...
        <DetailRow
          label="AIS Status"
          value={vessel.aisDark ? 'DARK — signal lost' : 'Normal'}
          color={vessel.aisDark ? '#ef4444' : '#22c55e'}
        />
        <DetailRow
          label="Sanctions"
          value={vessel.sanctionsFlag ? 'FLAGGED — review required' : 'Clear'}
          color={vessel.sanctionsFlag ? '#ef4444' : '#22c55e'}
        />
            ...
            {vessel.alerts.map((sev, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[11px] capitalize" style={{ color: s.color }}>
                    {sev} severity alert
                  </span>
                </div>
              );
        ...
        <div className="flex items-center gap-3">
          ...
          {apiMeta ? (
            <div
              ...
              style={{
                background: apiMeta.liveData ? 'rgba(34,197,94,0.12)' : 'rgba(251,191,36,0.12)',
                color: apiMeta.liveData ? '#22c55e' : '#fbbf24',
              }}
            >
        ...
        <div className="flex items-center gap-2">
            ...
            {(['map', 'list'] as const).map((v) => (
              <button
                ...
                style={{
                  ...
                  color: view === v ? ACCENT : TEXT.muted,
                }}
              >
                {v === 'map' ? 'Map' : 'List'}
              </button>
          ...
          >
            <RefreshCw
              ...
              style={{ color: loading ? ACCENT : TEXT.tertiary }}
            />
          </button>
      ...
      <div className="grid grid-cols-5 gap-px shrink-0" style={{ background: BORDER.subtle }}>
        {[
          ...
          { label: 'Underway', value: String(stats.underway), color: '#22c55e' },
          {
            ...
            color: stats.alerts > 0 ? '#f97316' : TEXT.tertiary,
          },
          {
            label: 'AIS dark',
            value: String(stats.dark),
            color: stats.dark > 0 ? '#ef4444' : TEXT.tertiary,
          },
          {
            label: 'Sanctions flags',
            value: String(stats.sanctions),
            color: stats.sanctions > 0 ? '#ef4444' : TEXT.tertiary,
          },
        ].map((s) => (
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px]" style={{ color: TEXT.tertiary }}>
                          MMSI {vessel.mmsi} · {status.label} · {vessel.lastSignal} ·{' '}
                          {vessel.source}
                        </p>
                      </div>
                    </div>
      ...
      {apiMeta?.fetchedAt && (
        ...
        >
          ...
          <span className="text-[9px] font-mono" style={{ color: TEXT.muted }}>
            Sources: Digitraffic (FTIA) + BarentsWatch (NCA) + AISStream.io + USCG NAIS · Last sync:{' '}
            {new Date(apiMeta.fetchedAt).toLocaleTimeString()} · Refreshes every 60s
          </span>
        </div>
      )}
    </div>
