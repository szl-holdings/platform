...
function VesselSidePanel({
  ...
  onClose: () => void;
  exceptions: ReturnType<typeof useFleetExceptions>['fleetExceptions'];
}) {
  const vesselExceptions = exceptions.filter(
    (e) => e.vesselId === vessel.id && e.status === 'active',
  );
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {vesselExceptions.length > 0 && (
            <Badge
              variant="outline"
              className="text-[9px] text-red-400 border-red-500/20 bg-red-500/10"
            >
        ...
        {vessel.lat != null && vessel.lon != null && (
          <div className="rounded-lg overflow-hidden border border-white/[0.06] relative">
            ...
            />
            <div
              className="absolute bottom-2 left-2 flex items-center gap-1 rounded-md px-2 py-1 text-[10px]"
        ...
        <div className="bg-[#c9b787]/8 rounded-lg p-3 border border-white/[0.06] space-y-2">
          ...
          <div className="flex items-center justify-between text-[10px]">
            ...
            >
              {(vessel.etaDelta ?? 0) < 0
                ? `${Math.abs(vessel.etaDelta ?? 0)}h ahead`
                : (vessel.etaDelta ?? 0) > 0
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="bg-[#c9b787]/8 rounded-lg p-2.5 border border-white/[0.06]">
            ...
            <p
              className={cn(
                'text-xs font-mono font-bold mt-0.5',
                (vessel.readinessScore ?? 0) >= 80
                  ? 'text-emerald-400'
                  : (vessel.readinessScore ?? 0) >= 60
                    ? 'text-amber-400'
                    : 'text-red-400',
              )}
            >
...
function navStatusColor(navStatus: number): string {
  ...
  if (navStatus === 5) return 'var(--gi-accent-blue)';
...
interface TrackHistoryResponse {
  data: { vesselId: number; track: TrackPoint[] };
}
...
function MapboxFleetMap({
  ...
  useEffect(() => {
    ...
    const routeFeatures = VESSEL_ROUTES.filter((r) =>
      filteredVessels.some((v) => v.id === r.vesselId),
    ).map((r) => ({
  ...
  useEffect(() => {
    ...
    import('mapbox-gl').then((module) => {
      ...
      aisVessels.forEach((v) => {
        ...
        const marker = new mbgl.Marker({ element: el }).setLngLat([v.lon, v.lat]).addTo(map);
      ...
      {!token && mapLoaded && (
        <div className="absolute top-10 left-1/2 -translate-x-1/2 bg-amber-500/10 border border-amber-500/20 rounded-lg px-3 py-1.5 z-10">
          <p className="text-[10px] text-amber-400">
            Add MAPBOX_ACCESS_TOKEN to enable full map tiles
          </p>
        </div>
      )}
...
export default function FleetMapPage() {
  ...
  const types: string[] = [
    'all',
    ...Array.from(new Set(vessels.map((v) => v.type).filter((t): t is string => t != null))),
  ];
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          >
            ...
            {showAis && aisVessels.length > 0 && (
              <span className="ml-1 text-[9px] font-mono">{aisVessels.length}</span>
            )}
          </button>
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
            <div className="pointer-events-auto rounded-2xl bg-[#060e1a]/85 backdrop-blur-md border border-white/[0.08] px-6 py-2 max-w-md shadow-2xl">
              <EmptyState
                icon={Ship}
                headline={
                  vessels.length === 0 ? 'No vessels in fleet' : 'No vessels matching filters'
                }
                description={
                  vessels.length === 0
                    ? 'No vessels have been onboarded yet. Once vessels are added to the fleet they will plot here in real time via AIS.'
                    : 'Adjust status or vessel-type filters to expand results. Currently 0 of ' +
                      vessels.length +
                      ' vessels match the active filter set.'
                }
                action={
                  vessels.length > 0
                    ? {
