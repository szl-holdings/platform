          ...
          <div className="border-t border-white/[0.06] grid grid-cols-1 md:grid-cols-3 gap-0 divide-y md:divide-y-0 md:divide-x divide-sky-500/10">
            ...
            <div className="p-4 space-y-2">
              ...
              {vessel.priorSanctionedPorts.length > 0 && (
                <div className="mt-2 p-2 bg-red-500/5 border border-red-500/10 rounded-lg">
                  <p className="text-[9px] text-red-400/70 uppercase tracking-wider mb-1">
                    Prior Sanctioned Port Calls
                  </p>
                  {vessel.priorSanctionedPorts.map((p) => (
                    <p key={p} className="text-[10px] text-red-400/60">
                      • {p}
                    </p>
                  ))}
                </div>
              ...
              {vessel.insuranceExposure.sanctionsVoidRisk && (
                <div className="p-2 bg-red-500/5 border border-red-500/10 rounded-lg">
                  <p className="text-[10px] text-red-400 flex items-center gap-1">
                    <AlertTriangle className="w-3 h-3" /> Policy may be void — sanctions exposure
                    detected
                  </p>
                </div>
              )}
            ...
            <div className="p-4 space-y-2">
              ...
              {vessel.sanctionsPenalty.criminalExposure && (
                <div className="p-2 bg-red-500/5 border border-red-500/10 rounded-lg">
                  <p className="text-[10px] text-red-400 flex items-center gap-1">
                    <Shield className="w-3 h-3" /> Criminal prosecution risk — refer to legal
                    counsel immediately
                  </p>
                </div>
              )}
...
export default function DarkFleetEconomics() {
  ...
  const totalInsurance = DARK_FLEET_VESSELS.reduce(
    (s, v) => s + v.insuranceExposure.totalExposure,
    0,
  );
  const totalFines = DARK_FLEET_VESSELS.reduce(
    (s, v) => s + v.sanctionsPenalty.totalPotentialFine,
    0,
  );
    ...
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <Badge
            variant="outline"
            className="text-[9px] text-orange-400 border-orange-500/20 bg-orange-500/5"
          >
        ...
        {[
          {
            ...
            value: fmtMoney(totalCargo),
            sub: 'at risk across dark vessels',
            icon: DollarSign,
            color: 'text-orange-400',
          },
          {
            label: 'Total Insurance Exposure',
            value: fmtMoney(totalInsurance),
            sub: 'hull + cargo + P&I',
            icon: Building,
            color: 'text-red-400',
          },
          {
            label: 'Sanctions Penalty Pool',
            value: fmtMoney(totalFines),
            sub: 'potential regulatory fines',
            icon: Scale,
      ...
      <div className="bg-amber-500/5 border border-amber-500/10 rounded-xl p-4">
        ...
        <p className="text-xs text-amber-300/50">
          Cargo values are estimated using vessel DWT utilisation (90% assumption), current spot
          commodity prices, and known trade routes. Insurance exposure includes maximum policy
          limits. Sanctions penalties reflect published US OFAC, EU, and UK OFSI civil penalty
          schedules. This analysis is indicative and does not constitute legal or insurance advice.
        </p>
      </div>
    </div>
  );
