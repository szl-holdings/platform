...
function ProbabilityRing({ prob }: { prob: number }) {
  ...
  const color =
    prob > 0.75 ? '#f87171' : prob > 0.5 ? '#fb923c' : prob > 0.3 ? '#fbbf24' : '#38bdf8';
  return (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-3">
                      <div className="flex items-center gap-1 text-[10px] text-red-400">
                        ...
                        {p.daysToFailure} days to failure
                      </div>
                      <div className="flex items-center gap-1 text-[10px] text-emerald-400">
                        <CheckCircle2 className="w-3 h-3" />
                        Saves ${(p.avoidedCost / 1000).toFixed(0)}K if actioned
                      </div>
                    </div>
          ...
          <div className="divide-y divide-sky-500/5">
            {SCHEDULE.map((s, i) => (
              <div key={i} className="px-4 py-3 flex items-center gap-4">
                ...
                <span className={cn('text-[10px] font-medium', priorityColor[s.priority])}>
                  {s.priority}
                </span>
              </div>
            ))}
