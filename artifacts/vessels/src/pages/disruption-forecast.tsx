...
import {
  AlertTriangle, Clock, Globe, TrendingUp, Wind, Anchor, Shield,
  Activity, ChevronDown, ChevronRight, Zap, Navigation, MapPin,
  BarChart3, RefreshCw, Filter
} from "lucide-react";
import { AreaChart, Area, ResponsiveContainer, Tooltip, XAxis, YAxis, CartesianGrid, RadarChart, Radar, PolarGrid, PolarAngleAxis } from "recharts";
...
function DisruptionGlobe({ zones, selectedId, onSelect }: {
  ...
  return (
    <div className="bg-[#060e1a] border border-sky-500/15 rounded-xl overflow-hidden relative" style={{ height: 310 }}>
      <div className="absolute top-3 left-3 z-10 flex items-center gap-1.5">
        <Globe className="w-3.5 h-3.5 text-sky-400/50" />
        <span className="text-[10px] text-sky-400/40 uppercase tracking-widest">72-Hour Disruption Globe</span>
      </div>
      <div className="absolute top-3 right-3 z-10 flex items-center gap-2">
        {[["Critical", "#ef4444"], ["High", "#f97316"], ["Medium", "#f59e0b"]].map(([label, color]) => (
          <span key={label} className="flex items-center gap-1 text-[9px]" style={{ color }}>
            <span className="w-2 h-2 rounded-full" style={{ background: color, opacity: 0.8 }} /> {label}
          </span>
        ))}
      </div>
      <svg
      ...
      >
        ...
        {[-60, -30, 0, 30, 60].map(lat => (
          <line
            ...
            stroke="#1e3a5f" strokeWidth={lat === 0 ? 0.8 : 0.4} strokeDasharray={lat === 0 ? "none" : "4,6"}
          />
        ))}
        {[-150, -120, -90, -60, -30, 0, 30, 60, 90, 120, 150].map(lon => (
          <line
            key={lon}
            x1={lonToX(lon, W)} y1={0}
            x2={lonToX(lon, W)} y2={H}
            stroke="#1e3a5f" strokeWidth={0.4} strokeDasharray="4,6"
          />
        ))}
        ...
        {[
          ...
          <line key={i} x1={r.x1} y1={r.y1} x2={r.x2} y2={r.y2} stroke="#38bdf8" strokeWidth={0.5} strokeOpacity={0.2} strokeDasharray="4,4" />
        ))}
        ...
        {zones.map(zone => {
          ...
          return (
            <g key={zone.id} style={{ cursor: "pointer" }} onClick={() => onSelect(zone.id)}>
              {/* Pulse ring */}
              <circle cx={cx} cy={cy} r={r + 6} fill="none" stroke={color} strokeWidth={0.8} strokeOpacity={0.3}>
              ...
              {/* Main circle */}
              <circle cx={cx} cy={cy} r={r} fill={color} fillOpacity={0.15} stroke={color} strokeWidth={isSelected ? 2 : 1} />
              {/* Center dot */}
              <circle cx={cx} cy={cy} r={3} fill={color} fillOpacity={0.9} />
              {/* Label */}
              <text x={cx} y={cy - r - 4} textAnchor="middle" fontSize={7} fill={color} fillOpacity={0.85} fontFamily="monospace">
                {zone.probability72h}%
              </text>
              {isSelected && (
                <text x={cx} y={cy + r + 10} textAnchor="middle" fontSize={7} fill={color} fillOpacity={0.9} fontFamily="sans-serif" fontWeight="bold">
                  {zone.name}
                </text>
              )}
            </g>
...
const CHOKEPOINT_HISTORY = [
  { hour: "-72h", risk: 42 }, { hour: "-60h", risk: 45 }, { hour: "-48h", risk: 51 },
  { hour: "-36h", risk: 58 }, { hour: "-24h", risk: 64 }, { hour: "-12h", risk: 71 },
  { hour: "Now", risk: 78 }, { hour: "+12h", risk: 82 }, { hour: "+24h", risk: 84 },
  { hour: "+36h", risk: 80 }, { hour: "+48h", risk: 76 }, { hour: "+72h", risk: 71 },
];
...
function ProbabilityArc({ value }: { value: number }) {
  const color = value >= 75 ? "#ef4444" : value >= 50 ? "#f97316" : value >= 30 ? "#f59e0b" : "#22c55e";
  return (
    <div className="relative flex items-center justify-center">
      ...
      <span className="absolute text-xs font-mono font-bold" style={{ color }}>{value}%</span>
    </div>
  );
}

function ZoneCard({ zone, expanded, onToggle }: { zone: typeof DISRUPTION_ZONES[0]; expanded: boolean; onToggle: () => void }) {
    ...
    >
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              <Badge variant="outline" className={cn("text-[9px]", severityColors[zone.severity])}>{zone.severity}</Badge>
              <Badge variant="outline" className="text-[9px] text-sky-400/50 border-sky-500/20">{zone.type}</Badge>
            </div>
            <p className="text-[11px] text-sky-400/50 mb-2">{zone.region}</p>
            <div className="flex items-center gap-4 text-[10px]">
              <span className="flex items-center gap-1 text-sky-400/60"><Clock className="w-3 h-3" /> Peak in <CountdownTimer etaString={zone.eta} /></span>
              <span className="flex items-center gap-1 text-sky-400/60"><Navigation className="w-3 h-3" /> {zone.affectedVessels} vessels</span>
              <span className="flex items-center gap-1 text-sky-400/60"><BarChart3 className="w-3 h-3" /> {zone.cargoValue} at risk</span>
            </div>
          </div>
      ...
      {expanded && (
        <div className="border-t border-sky-500/10 px-4 py-4 space-y-3 bg-sky-500/2">
          <div>
            <p className="text-[10px] uppercase tracking-widest text-sky-400/40 mb-2">Signal Drivers</p>
          ...
          <div className="bg-sky-500/5 border border-sky-500/10 rounded-lg p-3">
            <p className="text-[10px] uppercase tracking-widest text-sky-400/40 mb-1">Recommended Action</p>
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-xl font-bold text-sky-50 font-display">Disruption Forecast Engine</h1>
            <Badge variant="outline" className="text-[9px] text-emerald-400 border-emerald-500/20 bg-emerald-500/5">LIVE · 72H</Badge>
          </div>
          <p className="text-xs text-sky-400/50">Predictive disruption zones with probability scores and time-to-impact countdowns</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Active Disruption Zones", value: DISRUPTION_ZONES.length, sub: "across 6 regions", icon: AlertTriangle, color: "text-orange-400" },
          { label: "Vessels at Risk", value: totalAtRisk, sub: "in 72h window", icon: Navigation, color: "text-sky-400" },
          { label: "Critical Zones", value: criticalCount, sub: "require immediate action", icon: Zap, color: "text-red-400" },
          { label: "High-Risk Zones", value: highCount, sub: "monitor closely", icon: Shield, color: "text-amber-400" },
        ].map(kpi => (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="xl:col-span-2 bg-[#0a1628]/80 border border-sky-500/10 rounded-xl p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Badge variant="outline" className="text-[9px] text-red-400 border-red-500/20">↑ 18% vs 24h ago</Badge>
          </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <span className="text-[10px] text-sky-400/40 uppercase tracking-wider">Filter by severity:</span>
      ...
      <div className="space-y-3">
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12 text-sky-400/30 text-sm">No disruption zones match current filters</div>
        )}
      </div>
    </div>
  );
