...
const BRANCHES: Branch[] = [
  {
    ...
    icon: RotateCcw,
    color: "#4a90b8",
    description: "Full diversion via Cape of Good Hope to avoid Suez Canal closure or conflict escalation in the Red Sea corridor.",
    ...
    risks: ["Charter party penalties for late delivery", "Additional insurance premium", "Cargo quality risk on extended voyage"],
    mitigations: ["Force majeure clause invocation", "Charterer notification within 12h", "Bunker topping at Las Palmas"],
  },
  {
    id: "speed-reduction",
    ...
    icon: TrendingDown,
    color: "#6b8f71",
    description: "Reduce service speed from 13.4 to 10.0 knots to optimize fuel consumption and CII score, accepting ETA delay.",
    ...
    risks: ["Potential charter party late delivery", "Cargo temperature risk (reefer vessels only)"],
    mitigations: ["Charterer pre-approval for ETA revision", "Dynamic routing optimization enabled"],
  },
  {
    id: "embargo",
    ...
    icon: Shield,
    color: "#c45a4a",
    description: "Rotterdam or intermediate port subject to emergency sanctions. Vessel must divert to alternate discharge port.",
    ...
    risks: ["Cargo ownership disputes", "Vessel blacklisting risk", "Insurance invalidation if sanctions breach"],
    mitigations: ["Legal review within 2h of notification", "P&I Club emergency line activated", "OFAC license application if applicable"],
  },
  {
    id: "detention",
    ...
    icon: AlertTriangle,
    color: "#c8953c",
    description: "PSC inspection at Suez Canal or Rotterdam results in detention order. Deficiency rectification required before departure.",
    ...
    risks: ["Cargo demurrage accrual from day 1", "Crew overtime costs", "Charterer claim for delay"],
    mitigations: ["Pre-arrival PSC checklist completed", "DNV survey scheduled at next port", "Deficiency action plan prepared"],
  },
];
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <Badge variant="outline" className="text-[9px] text-sky-400 border-sky-500/30 bg-sky-500/5">ATLAS RUNTIME</Badge>
          </div>
      ...
      <div className="bg-[#0a1628]/80 border border-sky-500/10 rounded-xl p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[11px] text-sky-300 font-semibold uppercase tracking-wider">Baseline Worldline — Pacific Navigator</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
        <div className="space-y-2">
          ...
          {BRANCHES.map(b => {
            ...
            return (
              <div key={b.id} className={cn("p-3 rounded-xl border cursor-pointer transition-all", isSelected ? "border-sky-500/30 bg-sky-500/8" : "border-sky-500/10 bg-[#0a1628]/60 hover:border-sky-500/20")}>
                ...
                <button onClick={() => setComparing(isCompare ? null : b.id)} className={cn("mt-2 w-full text-[9px] py-1 rounded border transition-colors", isCompare ? "border-violet-500/30 text-violet-400 bg-violet-500/8" : "border-sky-500/10 text-sky-400/40 hover:text-sky-300")} disabled={isSelected}>
...
function BranchDetail({ branch, color, label }: { branch: Branch; color: string; label: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-[#0a1628]/80 border border-sky-500/10 rounded-xl p-4" style={{ borderColor: `${color}25` }}>
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-[10px] px-2 py-0.5 rounded-full border font-mono" style={{ color, borderColor: `${color}30`, background: `${color}10` }}>{branch.probability}% probable</span>
        </div>
        <p className="text-[11px] text-sky-400/50 mb-1">{branch.description}</p>
        <p className="text-[10px] text-sky-400/30"><span className="text-sky-400/50">Trigger:</span> {branch.trigger}</p>
      </div>
      ...
      <div className="bg-[#0a1628]/80 border border-sky-500/10 rounded-xl p-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: "ETA Change", value: branch.deltaEta, positive: branch.deltaEta.startsWith("−") },
            { label: "Fuel Δ", value: branch.deltaFuel, positive: branch.deltaFuel.startsWith("−") },
            { label: "Cost Δ", value: branch.deltaCost, positive: branch.deltaCost.startsWith("−") },
          ].map(d => (
            <div key={d.label} className={cn("p-2.5 rounded-lg border text-center", d.positive ? "border-emerald-500/20 bg-emerald-500/5" : "border-amber-500/20 bg-amber-500/5")}>
              <p className="text-[9px] text-sky-400/40 mb-1">{d.label}</p>
              <p className={cn("text-[11px] font-mono font-bold", d.positive ? "text-emerald-400" : "text-amber-400")}>{d.value}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div className="bg-[#0a1628]/80 border border-red-500/10 rounded-xl p-4">
          <p className="text-[11px] font-semibold text-red-400/80 mb-2 flex items-center gap-1.5"><AlertTriangle className="w-3 h-3" />Key Risks</p>
          <ul className="space-y-1.5">
            {branch.risks.map((r, i) => <li key={i} className="text-[10px] text-sky-400/50 flex gap-1.5"><span className="text-red-400/40 mt-0.5">•</span>{r}</li>)}
          </ul>
        </div>
        <div className="bg-[#0a1628]/80 border border-emerald-500/10 rounded-xl p-4">
          <p className="text-[11px] font-semibold text-emerald-400/80 mb-2 flex items-center gap-1.5"><CheckCircle2 className="w-3 h-3" />Mitigations</p>
          <ul className="space-y-1.5">
            {branch.mitigations.map((m, i) => <li key={i} className="text-[10px] text-sky-400/50 flex gap-1.5"><span className="text-emerald-400/40 mt-0.5">•</span>{m}</li>)}
          </ul>
        </div>
      </div>
    </div>
