...
import {
  ...
  TrendingUp, Activity, Loader2, Sparkles
} from "lucide-react";
...
const BRIEFS = [
  {
    ...
    situation: "Iranian Revolutionary Guard Corps (IRGC) has announced naval exercises covering transit corridor in the Strait of Hormuz (26.0°N–26.8°N, 55.5°E–56.5°E). 147 commercial vessels currently within or approaching the affected zone, including 89 crude oil tankers with aggregate cargo value of ~$1.9B. Three VLCCs have deviated from planned routes in last 6 hours.",
    impact: "Potential disruption to ~18.4 MB/day of crude throughput. Insurance war risk premiums have risen 2.3 percentage points in last 24h. Earliest viable alternative routing via Strait of Malacca adds 11 days to voyage time for India-bound traffic, with estimated cost uplift of $280K–$480K per voyage.",
    recommendations: [
    ...
    sources: ["Helmsman AI (AIS deviation analysis)", "Disruption Forecast Engine", "Lloyd's Market Association Joint War Committee"],
  },
  {
    id: "IB-0046",
    ...
    sources: ["Trade Flow Heatmap AI", "Baltic Exchange", "Ukraine Ministry of Infrastructure advisory"],
  },
  {
    id: "IB-0045",
    ...
    triggerSignal: "Dark Fleet Economics — GHOST MERIDIAN AIS gap analysis",
    affectedVessels: 2,
    dollarImpact: "$151M cargo + $1.2B insurance exposure",
    timeframe: "Transfer completed — post-event analysis",
    affected: [
      "Insurance underwriters with GHOST MERIDIAN H&M exposure",
    ...
    situation: "GHOST MERIDIAN (IMO 9821045, VLCC, 300k DWT) completed a 14h 22m AIS blackout near 26.1°N 55.8°E in the Persian Gulf. Satellite imagery analysis suggests ship-to-ship transfer with KAZAN SPIRIT (sanctioned entity, OFAC SDN list). Estimated 285,000 MT crude oil transferred, cargo value ~$151M. Both vessels now showing false flag of convenience.",
    impact: "Any downstream buyer of cargo from KAZAN SPIRIT faces OFAC civil penalty exposure of $1.28M per violation plus potential criminal prosecution. P&I club coverage for GHOST MERIDIAN may be void under war risk / sanctions exclusion clause. Hull underwriters should consider policy suspension pending investigation. Cargo title chain must be traced immediately.",
    recommendations: [
      "Insurance underwriters: issue vessel alert and initiate policy review for GHOST MERIDIAN",
    ...
    sources: ["Dark Fleet Economics AI", "Helmsman AI (AIS analysis)", "OFAC SDN List cross-reference"],
  },
  {
    id: "IB-0044",
    ...
    sources: ["Trade Flow Heatmap AI", "Voyage P&L Predictor", "Baltic Exchange BLNG3", "S&P Global Platts"],
  },
];
  ...
  return (
    <div
      className={cn(
        ...
        expanded ? "border-sky-500/30 bg-[#0a1628]/90" : "border-sky-500/10 bg-[#0a1628]/80 hover:border-sky-500/20"
      )}
    >
        ...
        <div className="px-5 py-4">
          <div className="flex items-start gap-3">
            <div className={cn("w-9 h-9 rounded-lg flex items-center justify-center shrink-0 border",
              brief.severity === "Critical" ? "bg-red-500/5 border-red-500/15" : brief.severity === "High" ? "bg-orange-500/5 border-orange-500/15" : "bg-amber-500/5 border-amber-500/15"
            )}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap mb-1">
                ...
                <Badge variant="outline" className={cn("text-[9px]", severityColors[brief.severity])}>{brief.severity}</Badge>
                <Badge variant="outline" className="text-[9px] text-sky-400/40 border-sky-500/15">{brief.type}</Badge>
              </div>
              ...
              <div className="flex items-center gap-4 text-[10px] text-sky-400/50 flex-wrap">
                <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {brief.generatedAt}</span>
                <span className="flex items-center gap-1"><Ship className="w-3 h-3" /> {brief.affectedVessels} vessels</span>
                <span className="flex items-center gap-1"><DollarSign className="w-3 h-3" />{brief.dollarImpact}</span>
                <span className="flex items-center gap-1"><Zap className="w-3 h-3 text-amber-400" />{brief.triggerSignal}</span>
              </div>
            </div>
            <ChevronRight className={cn("w-4 h-4 text-sky-400/30 shrink-0 transition-transform mt-1", expanded && "rotate-90")} />
          </div>
        </div>
      </button>
      ...
      {expanded && (
        <div className="border-t border-sky-500/10 divide-y divide-sky-500/10">
          ...
          <div className="px-5 py-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <div className="p-3 bg-sky-500/3 border border-sky-500/10 rounded-lg">
                <p className="text-[9px] text-sky-400/40 uppercase tracking-wider mb-1">Timeframe</p>
                <p className="text-xs text-sky-200">{brief.timeframe}</p>
              </div>
              <div className="p-3 bg-sky-500/3 border border-sky-500/10 rounded-lg">
                <p className="text-[9px] text-sky-400/40 uppercase tracking-wider mb-1">Dollar Impact Estimate</p>
          ...
          {/* Situation */}
          <div className="px-5 py-4">
            <p className="text-[10px] uppercase tracking-widest text-sky-400/40 mb-2">Situation Summary</p>
            <p className="text-xs text-sky-300/70 leading-relaxed">{brief.situation}</p>
          </div>

          {/* Impact */}
          <div className="px-5 py-4">
            <p className="text-[10px] uppercase tracking-widest text-sky-400/40 mb-2">Financial & Operational Impact</p>
            ...
            <ul className="space-y-2">
              {brief.recommendations.map((r, i) => (
                <li key={i} className="flex items-start gap-2 text-xs text-sky-200/70">
                  <span className="w-4 h-4 rounded-full bg-sky-500/10 border border-sky-500/20 flex items-center justify-center shrink-0 text-[8px] font-bold text-sky-400">{i + 1}</span>
                  {r}
                </li>
              ))}
            </ul>
          ...
          <div className="px-5 py-3 bg-sky-500/2">
            <p className="text-[10px] uppercase tracking-widest text-sky-400/30 mb-1">Intelligence Sources</p>
...
export default function IntelligenceBriefs() {
  ...
  const [selectedTopic, setSelectedTopic] = useState("Strait of Hormuz disruption assessment for crude traders");
    ...
    if (signal) {
      ...
      fetch("/api/intelligence/ai/maritime-intelligence", {
        ...
        .then((data: { response?: string; result?: string }) => {
          setGeneratedBrief({ loading: false, content: data.response || data.result || "No content returned" });
        })
        .catch(() => {
          setGeneratedBrief({ loading: false, content: "", error: "Failed to generate brief — AI service unavailable" });
        });
    }
  }, []);
  ...
  const generateBrief = async () => {
    ...
    try {
      ...
      setGeneratedBrief({ loading: false, content: data.response || data.result || "No content generated" });
    } catch {
      setGeneratedBrief({ loading: false, content: "", error: "Failed to generate brief — AI service unavailable" });
    }
  };
    ...
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <Badge variant="outline" className="text-[9px] text-emerald-400 border-emerald-500/20 bg-emerald-500/5">AUTO-GENERATED</Badge>
        </div>
        <p className="text-xs text-sky-400/50">Structured disruption reports auto-generated when significant signals are detected — affected parties, dollar impact, and recommended actions</p>
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Briefs Generated", value: BRIEFS.length, sub: "last 48 hours", icon: FileText, color: "text-sky-400" },
          { label: "Critical Alerts", value: criticalCount, sub: "require immediate action", icon: AlertTriangle, color: "text-red-400" },
          { label: "High Priority", value: highCount, sub: "monitor closely", icon: Zap, color: "text-orange-400" },
          { label: "Trigger Sources", value: 4, sub: "AI signal engines", icon: Activity, color: "text-purple-400" },
        ].map(kpi => (
      ...
      {autoAlert && (
        <div className="flex items-start gap-3 bg-emerald-500/5 border border-emerald-500/25 rounded-xl px-4 py-3 animate-in fade-in duration-500">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-emerald-300">Auto-trigger fired — Intelligence Brief generating</p>
            <p className="text-[11px] text-emerald-400/70 mt-0.5">
              Source: <span className="text-emerald-300">{autoAlert.source}</span>
            </p>
          </div>
          <button className="text-[10px] text-emerald-400/50 hover:text-emerald-300 shrink-0" onClick={() => setAutoAlert(null)}>Dismiss</button>
        </div>
      )}
      ...
      <div className="bg-[#0a1628]/80 border border-sky-500/15 rounded-xl p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-semibold text-sky-200">Generate Intelligence Brief On-Demand</p>
        </div>
        <p className="text-[11px] text-sky-400/50">Helmsman AI will produce a structured brief for any maritime intelligence topic</p>
        <div className="flex gap-2">
          ...
          >
            {generatedBrief?.loading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
            {generatedBrief?.loading ? "Generating..." : "Generate Brief"}
          </button>
        </div>
        {generatedBrief && !generatedBrief.loading && (
          <div className={cn("rounded-lg p-4 text-xs leading-relaxed", generatedBrief.error ? "bg-red-500/5 border border-red-500/10 text-red-400/70" : "bg-sky-500/3 border border-sky-500/10 text-sky-200/80")}>
