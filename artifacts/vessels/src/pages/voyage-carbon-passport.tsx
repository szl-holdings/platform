...
export default function VoyageCarbonPassport() {
  ...
  const { data, isLoading, isError, refetch } = useStandardQuery<EmissionsApiResponse>({
    ...
    queryFn: () =>
      fetch(`${API_BASE}/vessels/modules/voyages-emissions`, { credentials: 'include' }).then(
        (r) => {
    ...
    mutationFn: (payload: Record<string, unknown>) =>
      fetch(`${API_BASE}/vessels/modules/voyages-emissions`, {
        ...
        if (!r.ok) {
          ...
          throw new Error((err as { error?: string }).error ?? 'Failed to compute passport');
        }
        return r.json().then((d) => d.data ?? d);
      }),
    onSuccess: () => {
  ...
  return (
    <div className="min-h-screen bg-[#060810] text-white p-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-emerald-200 flex items-center gap-1.5">
                <PlusCircle className="w-3.5 h-3.5 text-emerald-400" /> Compute Carbon Passport from
                AIS
              </p>
              <p className="text-[10px] text-white/30 mt-0.5">
                Enter MMSI to auto-fill vessel name/IMO from live AIS · provide voyage + bunker data
                to compute CII rating
              </p>
            </div>
            <button onClick={() => setShowCreate(false)}>
              <X className="w-4 h-4 text-white/20 hover:text-white/50" />
            </button>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {(
              [
                {
                  key: 'mmsi',
                  label: 'MMSI (AIS track)',
                  hint: 'Optional — derives distance from AIS + fills vessel name/IMO',
                },
                {
                  key: 'vesselName',
                  label: 'Vessel Name',
                  hint: 'Overridden by live AIS when MMSI given',
                },
                ...
                { key: 'destination', label: 'Destination Port *', hint: 'e.g. Singapore' },
                {
                  key: 'distanceNm',
                  label: 'Distance (nm)',
                  hint: 'Derived from AIS track when MMSI given; required otherwise',
                },
                {
                  key: 'fuelConsumedMt',
                  label: 'Fuel Consumed (MT) *',
                  hint: 'From bunker / fuel log',
                },
                {
                  key: 'departedAt',
                  label: 'Departure (UTC) *',
                  hint: 'ISO: 2026-04-18T00:00:00Z',
                },
              ] as { key: keyof ComputeFormData; label: string; hint: string }[]
            ).map(({ key, label, hint }) => (
              <div key={key}>
                <label className="text-[10px] text-emerald-400/50 uppercase tracking-wider block mb-1">
                  {label}
                </label>
                <input
                  value={form[key]}
            ...
            <div>
              ...
              >
                {['VLSFO', 'HFO', 'MGO', 'LNG', 'METHANOL'].map((ft) => (
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              disabled={
                ...
                !form.fuelConsumedMt ||
                !form.departedAt ||
                (!form.mmsi && (!form.vesselName || !form.distanceNm))
              }
            ...
            {computeMutation.isError && (
              <span className="text-xs text-red-400">
                {computeMutation.error?.message ?? 'Error'}
              </span>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            label: 'Fleet CO₂ Emissions',
            value: `${formatNum(totals.totalCo2Mt)} MT`,
            color: '#ef4444',
          },
          {
            label: 'Total Carbon Cost',
            value: `$${formatNum(totals.totalCarbonCostUsd)}`,
            color: '#f59e0b',
          },
          {
            label: 'Avg Efficiency Score',
            value: `${totals.avgEfficiencyScore}/100`,
            color: '#10b981',
          },
          {
            label: 'EU ETS Liability',
            value: `$${formatNum(totals.totalEuEtsUsd)}`,
            color: '#8b5cf6',
          },
        ].map((kpi) => (
              ...
              <div className="flex items-center gap-6">
                <div className="text-right">
                  ...
                  <div
                    className={cn(
                      ...
                      voyage.co2PerNm < voyage.fleetAvgCo2PerNm
                        ? 'text-emerald-400'
                        : 'text-red-400',
                    )}
                  >
                ...
                <div className="text-right">
                  ...
                  <div
                    ...
                    style={{
                      color:
                        voyage.efficiencyScore >= 85
                          ? '#10b981'
                          : voyage.efficiencyScore >= 70
                            ? '#f59e0b'
                            : '#ef4444',
                    }}
                  >
                ...
                <div className="text-right">
                  ...
                  <div className="text-sm font-mono font-medium text-amber-400">
                    ${formatNum(voyage.carbonCostUsd)}
                  </div>
                </div>
              </div>
            </div>
            ...
            {selectedId === voyage.id && (
              <div className="mt-4 pt-4 border-t border-white/5 space-y-3">
                <div className="grid grid-cols-3 gap-4">
                  <div className="rounded-lg bg-white/5 p-3">
                    ...
                    <div className="space-y-1.5 text-xs">
                      <div className="flex justify-between">
                        ...
                        <span className="text-white/70 font-mono">{voyage.fuelType}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-white/50">Fuel Consumed</span>
                        <span className="text-white/70 font-mono">{voyage.fuelConsumedMt} MT</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-white/50">CO₂ Emissions</span>
                        <span className="text-white/70 font-mono">{voyage.co2EmissionsMt} MT</span>
                      </div>
                  ...
                  <div className="rounded-lg bg-white/5 p-3">
                    <div className="text-[10px] text-white/30 uppercase mb-2">
                      Performance vs Fleet
                    </div>
                    <div className="space-y-1.5 text-xs">
                      <div className="flex justify-between">
                        <span className="text-white/50">Your CO₂/nm</span>
                        <span
                          className="font-mono"
                          style={{
                            color:
                              voyage.co2PerNm < voyage.fleetAvgCo2PerNm ? '#10b981' : '#ef4444',
                          }}
                        >
                          {voyage.co2PerNm.toFixed(4)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-white/50">Fleet Avg</span>
                        <span className="text-white/70 font-mono">
                          {voyage.fleetAvgCo2PerNm.toFixed(4)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-white/50">AER</span>
                        <span className="text-white/70 font-mono">{voyage.aer.toFixed(6)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-white/50">Weather-Adj Score</span>
                        <span className="text-white/70 font-mono">
                          {voyage.weatherAdjustedScore}%
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className="rounded-lg bg-white/5 p-3">
                    ...
                    <div className="space-y-1.5 text-xs">
                      <div className="flex justify-between">
                        <span className="text-white/50">Carbon Cost</span>
                        <span className="text-amber-400 font-mono">
                          ${formatNum(voyage.carbonCostUsd)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-white/50">EU ETS Liability</span>
                        <span className="text-white/70 font-mono">
                          ${formatNum(voyage.euEtsLiability)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-white/50">Port Waste</span>
                        <span className="text-white/70 font-mono">
                          {voyage.portCongestionWasteHours}h
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="rounded-lg bg-emerald-500/5 border border-emerald-500/20 p-3 flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-mono text-emerald-400/60 break-all">
                    {voyage.passportHash}
                  </span>
                </div>
              </div>
      ...
      <div className="rounded-xl bg-white/[0.02] border border-white/5 p-4 text-[11px] text-white/30 flex items-center gap-2">
        ...
        Emissions computed using IMO 2023 fuel emission factors (VLSFO 3.151, HFO 3.114, MGO 3.206,
        LNG 2.75 MT CO₂/MT fuel). CII ratings derived from Annual Efficiency Ratio (AER = CO₂ /
        GT·nm). Carbon cost at $72/MT, EU ETS at €65/MT.
      </div>
    </div>
  );
}
