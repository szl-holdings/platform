...
const severityConfig: Record<
  string,
  { label: string; color: string; dot: string; badgeColor: string }
> = {
  critical: {
    label: 'Critical',
    color: 'text-red-400',
...
function useAcknowledgeException() {
  ...
  return useStandardMutation({
    ...
    onMutate: async ({ id }) => {
      ...
      qc.setQueriesData(
        { queryKey: ['vesselExceptions'] },
        (old: VesselsException[] | undefined) =>
        ...
        <div className="flex items-center gap-2 mt-0.5 shrink-0">
          <span
            className={cn(
              ...
              sev.dot,
              exc.status === 'active' && exc.severity === 'critical' && 'animate-pulse',
            )}
          />
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-1.5 flex-wrap ml-auto">
              ...
              <Badge variant="outline" className={cn('text-[9px] shrink-0', stat.color)}>
                {stat.label}
              </Badge>
            </div>
          </div>
          <div className="flex items-center gap-3 mt-1 text-[10px] text-sky-400/50">
            ...
            {Math.abs(impact) > 0 && (
              ...
              >
                {isPositive ? '+' : ''}${(Math.abs(impact) / 1000).toFixed(0)}K{' '}
                {isPositive ? 'opportunity' : 'exposure'}
              </span>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: 'Critical', value: isLoading ? '—' : criticalCount, color: 'text-red-400' },
          {
            ...
            value: isLoading ? '—' : `$${(totalExposure / 1000).toFixed(0)}K`,
            color: 'text-orange-400',
          },
        ].map((s) => (
          <div
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((exc) => (
              <ExceptionCard
                ...
                onEscalate={(id) =>
                  escalateExc.mutate({ id, notes: 'Escalated via command center' })
                }
