...
const severityConfig = {
  ...
  Warning: { color: "bg-orange-500/10 text-orange-400 border-orange-500/20", icon: AlertCircle, dotColor: "bg-orange-400", lineColor: "border-l-orange-400" },
  Info: { color: "bg-blue-500/10 text-blue-400 border-blue-500/20", icon: Info, dotColor: "bg-blue-400", lineColor: "border-l-blue-400" },
  Debug: { color: "bg-zinc-500/10 text-zinc-400 border-zinc-500/20", icon: Bug, dotColor: "bg-zinc-400", lineColor: "border-l-zinc-400" },
};
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="animate-fade-in-up">
        ...
        <p className="text-sm text-muted-foreground mt-1">Fleet-wide event stream with severity filtering and search</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-4 animate-fade-in-up stagger-1">
        ...
        <div className="flex items-center gap-3 text-xs">
          ...
          {criticalCount > 0 && <Badge variant="outline" className="bg-red-500/10 text-red-400 border-red-500/20">{criticalCount} Critical</Badge>}
          {warningCount > 0 && <Badge variant="outline" className="bg-orange-500/10 text-orange-400 border-orange-500/20">{warningCount} Warning</Badge>}
        </div>
      </div>
        ...
        <CardContent className="p-0">
          <div className="divide-y divide-border">
            {logs.map(log => {
              ...
              return (
                <div key={log.id} className={`border-l-2 ${config.lineColor} transition-all hover:bg-muted/30`}>
                  <button
                  ...
                  >
                    <div className="flex items-center gap-2 mt-0.5 shrink-0">
                      ...
                      <SeverityIcon className={`w-4 h-4 ${log.severity === "Critical" ? "text-red-400" : log.severity === "Warning" ? "text-orange-400" : log.severity === "Info" ? "text-blue-400" : "text-zinc-400"}`} />
                    </div>
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4 ml-9 space-y-2 animate-fade-in-up">
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground">
                        <span>Source: <span className="text-foreground">{log.source}</span></span>
                        <span>Vessel ID: <span className="text-foreground">{log.vesselId}</span></span>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {logs.length === 0 && !isLoading && (
              <div className="p-12 text-center">
                ...
                <p className="text-xs text-muted-foreground/60 mt-1">Try adjusting the severity filter or search terms</p>
              </div>
            )}
          </div>
        </CardContent>
