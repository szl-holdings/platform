...
import { AlertTriangle, Navigation, Radio, Eye, RefreshCw, CheckCircle2, Clock, Filter, TrendingUp, MapPin, Zap } from "lucide-react";
...
interface AnomalyData {
  alerts: Alert[];
  summary: { total: number; open: number; acknowledged: number; uniqueVesselsAffected: number; bySeverity: Record<string, number> };
  provenance: { confidence: number; verifierApproved: boolean; attestation: string; freshness: { fetchedAt: string } };
}
...
function AlertRow({ alert, selected, onClick }: { alert: Alert; selected: boolean; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className={cn("w-full text-left rounded-xl p-3.5 border transition-all", selected ? "ring-1 ring-sky-400/40" : "hover:border-sky-500/20")} style={{ background: selected ? sev.bg : "rgba(10,22,40,0.7)", borderColor: selected ? sev.border : "rgba(255,255,255,0.06)" }}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <span className="ml-auto text-[9px] px-1.5 py-0.5 rounded-full border font-medium" style={{ color: sev.color, borderColor: sev.border, background: sev.bg }}>{sev.label}</span>
          </div>
          ...
          <div className="flex items-center gap-3 mt-1.5 text-[9px] text-sky-400/40">
            ...
            <span className="ml-auto flex items-center gap-1">
              {alert.status === "acknowledged" ? <CheckCircle2 className="w-2.5 h-2.5 text-emerald-400/50" /> : <Clock className="w-2.5 h-2.5 text-amber-400/60" />}
    ...
    <div style={{ padding: "28px 28px 48px", maxWidth: 1400, margin: "0 auto" }}>
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-sky-400/60">Detects deviations from expected voyage corridors, AIS gaps, and behavioral anomalies — ranked by risk.</p>
        </div>
      ...
      {data && (
        <div className="grid grid-cols-5 gap-3 mb-5">
          {[
            ...
            { label: "Vessels Affected", value: data.summary.uniqueVesselsAffected, color: "#a78bfa" },
          ].map(s => (
        ...
        <div className="flex gap-1.5">
          {["all", "critical", "high", "medium"].map(s => (
            <button key={s} onClick={() => setSeverityFilter(s)}
              className={cn("px-2.5 py-1 rounded-lg text-[10px] border transition-colors capitalize", severityFilter === s ? "bg-sky-500/15 border-sky-500/30 text-sky-300" : "border-sky-500/10 text-sky-400/50 hover:text-sky-300/70")}
        ...
        <div className="flex gap-1.5 ml-2">
          {["all", "open", "acknowledged"].map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={cn("px-2.5 py-1 rounded-lg text-[10px] border transition-colors capitalize", statusFilter === s ? "bg-sky-500/15 border-sky-500/30 text-sky-300" : "border-sky-500/10 text-sky-400/50 hover:text-sky-300/70")}
      ...
      <div className="grid grid-cols-12 gap-4">
        <div className="col-span-7 space-y-2">
          {loading ? (
            <div className="flex items-center justify-center h-48 text-sky-400/40 text-sm">Loading anomaly alerts…</div>
          ) : filtered.length === 0 ? (
            ...
            filtered.map(alert => (
              <AlertRow key={alert.id} alert={alert} selected={selected?.id === alert.id} onClick={() => setSelected(selected?.id === alert.id ? null : alert)} />
            ))
          )}
        </div>
        ...
        <div className="col-span-5">
          {selected ? (
            <div className="rounded-xl border border-sky-500/10 p-4 sticky top-4" style={{ background: "rgba(10,22,40,0.9)" }}>
              <div className="flex items-center gap-2 mb-4">
                {(() => { const sev = SEVERITY_CONFIG[selected.severity]; const Icon = ANOMALY_ICONS[selected.anomalyType] ?? AlertTriangle; return <div className="p-2 rounded-lg" style={{ background: sev.bg }}><Icon className="w-4 h-4" style={{ color: sev.color }} /></div>; })()}
                <div>
                  <div className="text-sm font-semibold text-sky-100">{selected.anomalyLabel}</div>
                  <div className="text-[10px] text-sky-400/50">{selected.vesselName} · {selected.id}</div>
                </div>
              </div>
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  <div><span className="text-sky-400/50">Corridor</span><div className="text-sky-200 mt-0.5">{selected.corridor}</div></div>
                  <div><span className="text-sky-400/50">Drift</span><div className="text-sky-200 mt-0.5">{selected.driftNm} nm</div></div>
                  <div><span className="text-sky-400/50">Position</span><div className="text-sky-200 mt-0.5">{selected.lastKnownPosition.lat.toFixed(2)}°, {selected.lastKnownPosition.lon.toFixed(2)}°</div></div>
                  <div><span className="text-sky-400/50">Confidence</span><div className="text-sky-200 mt-0.5">{Math.round(selected.confidence * 100)}%</div></div>
                </div>
                <div className="p-2.5 rounded-lg border" style={{ background: SEVERITY_CONFIG[selected.severity]?.bg, borderColor: SEVERITY_CONFIG[selected.severity]?.border }}>
                  <div className="text-[9px] uppercase tracking-wider mb-1" style={{ color: SEVERITY_CONFIG[selected.severity]?.color }}>Recommended Action</div>
                ...
                <div className="pt-2 border-t border-sky-500/10">
                  ...
                  <div className="text-[10px] text-sky-400/40 mt-0.5">Detected {new Date(selected.detectedAt).toLocaleString()}</div>
                </div>
              </div>
            </div>
          ) : (
            <div className="rounded-xl border border-sky-500/10 p-4" style={{ background: "rgba(10,22,40,0.8)" }}>
              <div className="flex flex-col items-center justify-center h-48 text-center">
                ...
                <p className="text-sky-400/25 text-xs mt-1">View details, position, recommended action</p>
              </div>
            </div>
          )}
        </div>
