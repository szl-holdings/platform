...
export default function PerformanceAnalyticsPage() {
  ...
  const sortedByOTA = [...performanceMetrics].sort(
    (a, b) => b.onTimeArrivalRate - a.onTimeArrivalRate,
  );
  ...
  const fleetOnTime =
    activeMetrics.length > 0
      ? activeMetrics.reduce((a, m) => a + m.onTimeArrivalRate, 0) / activeMetrics.length
      : 0;
  const fleetUtil =
    activeMetrics.length > 0
      ? activeMetrics.reduce((a, m) => a + m.utilization, 0) / activeMetrics.length
      : 0;
  const tceMetrics = performanceMetrics.filter((m) => m.tce > 0);
  const fleetTCE =
    tceMetrics.length > 0 ? tceMetrics.reduce((a, m) => a + m.tce, 0) / tceMetrics.length : 0;
  const totalDelayHours = performanceMetrics.reduce((a, m) => a + m.avgDelayHours, 0);

  const corridorWeatherRisk: Record<string, string> = {
    low: 'text-emerald-400',
    moderate: 'text-amber-400',
    high: 'text-orange-400',
    severe: 'text-red-400',
  };
  const corridorCongestion: Record<string, string> = {
    low: 'text-emerald-400',
    moderate: 'text-amber-400',
    high: 'text-orange-400',
  };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          {
            label: 'Avg Fleet TCE',
            value: `$${(fleetTCE / 1000).toFixed(1)}K`,
          ...
          },
          {
            ...
            value: `${totalDelayHours.toFixed(0)}h`,
            color: 'text-orange-400',
            icon: Clock,
            sub: 'fleet this month',
            trend: 'down' as const,
          },
        ].map((s) => (
              ...
              >
                ...
                <div className="flex items-center gap-4 mt-2 text-[10px] text-sky-400/40">
                  ...
                  <span className="ml-auto">
                    Fuel eff.: {m.fuelEfficiency > 0 ? `${m.fuelEfficiency}` : '—'} gCO2/MT-nm
                  </span>
                </div>
              </div>
            ))}
                ...
                >
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span
                      className={cn(
                        'text-sm font-bold font-mono',
                        m.onTimeArrivalRate >= 90
                          ? 'text-emerald-400'
                          : m.onTimeArrivalRate >= 75
                            ? 'text-amber-400'
                            : 'text-red-400',
                      )}
                    >
                  ...
                  <div className="flex items-center gap-4 mt-2 text-[10px] text-sky-400/40">
                    ...
                    <span className="ml-auto">
                      Route margin:{' '}
                      {m.routeProfitability > 0 ? `${m.routeProfitability.toFixed(1)}%` : 'N/A'}
                    </span>
                  </div>
                </div>
              ))}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="text-right shrink-0">
                    <p
                      className={cn(
                        'text-sm font-bold font-mono',
                        Number(corridor.profitabilityIndex) >= 70
                          ? 'text-emerald-400'
                          : Number(corridor.profitabilityIndex) >= 50
                            ? 'text-amber-400'
                            : 'text-red-400',
                      )}
                    >
                ...
                <div className="mt-3 grid grid-cols-4 gap-2">
                  ...
                  <div className="bg-sky-500/5 rounded p-2 border border-sky-500/10">
                    ...
                    <p
                      className={cn(
                        'text-[10px] font-mono',
                        Number(corridor.delayRate) >= 30
                          ? 'text-red-400'
                          : Number(corridor.delayRate) >= 15
                            ? 'text-amber-400'
                            : 'text-emerald-400',
                      )}
                    >
                  ...
                  <div className="bg-sky-500/5 rounded p-2 border border-sky-500/10">
                    ...
                    <p
                      className={cn(
                        ...
                        corridorWeatherRisk[corridor.weatherRisk],
                      )}
                    >
                  ...
                  <div className="bg-sky-500/5 rounded p-2 border border-sky-500/10">
                    ...
                    <p
                      className={cn(
                        'text-[10px] font-mono capitalize',
                        corridorCongestion[corridor.portCongestionRisk],
                      )}
                    >
