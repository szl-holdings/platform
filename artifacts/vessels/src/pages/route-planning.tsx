...
function RoutePathIndicator({ status }: { status: string }) {
  ...
  return (
    <div className="flex items-center gap-1 my-2">
      <div
        className={`w-2.5 h-2.5 rounded-full border-2 ${isActive ? 'border-emerald-400 bg-emerald-400/30' : 'border-muted-foreground/40 bg-transparent'}`}
      />
      <div className="flex-1 relative h-0.5">
        <div
          className={`absolute inset-0 rounded-full ${isActive ? 'bg-emerald-400/30' : 'bg-muted-foreground/20'}`}
        />
        {isActive && (
      ...
      <div
        className={`w-2.5 h-2.5 rounded-full border-2 ${status === 'completed' ? 'border-blue-400 bg-blue-400/30' : 'border-muted-foreground/40 bg-transparent'}`}
      />
    </div>
  );
}
...
export default function RoutePlanningPage() {
  ...
  const { data: routes = [], isLoading } = useStandardQuery({
    queryKey: ['routes'],
  ...
  const handleCreate = () => {
    if (!form.vesselId || !form.originPort || !form.destinationPort) {
      ...
      <div className="flex items-center justify-between animate-fade-in-up">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Waypoint optimization, ETA forecasting, and weather-adjusted routing
          </p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="bg-card border-border">
            <DialogHeader>
              <DialogTitle className="font-display">Create Route</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setForm((p) => ({ ...p, originPort: e.target.value }))}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setForm((p) => ({ ...p, distanceNm: e.target.value }))}
        ...
        <Card className="bg-card border-border border-dashed animate-fade-in-up stagger-2">
          <CardContent className="p-16 text-center">
            ...
            <p className="text-xs text-muted-foreground/60 mt-1">
              Create your first route to begin tracking vessel journeys
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-3">
          {routes.map((route: any, i: number) => {
            ...
            return (
              <Card
                key={route.id}
                className={`bg-card border-border hover:border-primary/20 transition-all duration-300 animate-fade-in-up stagger-${Math.min(i + 1, 8)} ${isActive ? 'ring-1 ring-emerald-500/10' : ''}`}
              >
                <CardContent className="p-5">
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-4 flex-1">
                      <div
                        className={`w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center mt-0.5 ${isActive ? 'ring-1 ring-emerald-500/20' : ''}`}
                      >
                        <Navigation
                          className={`w-5 h-5 text-primary ${isActive ? 'animate-pulse' : ''}`}
                        />
                      </div>
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {(vessel?.name as string) || 'Unassigned vessel'}
                        </p>
                    ...
                    <div className="flex items-center gap-2">
                      <Badge
                        variant="outline"
                        className={`${statusColors[route.status] || ''} ${isActive ? 'animate-pulse' : ''}`}
                      >
                        {isActive && (
