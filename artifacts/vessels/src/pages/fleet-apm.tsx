...
export default function FleetAPMPage() {
  ...
  const avgTCE =
    activeVessels.length > 0
      ? Math.round(activeVessels.reduce((s, v) => s + (v.tce ?? 0), 0) / activeVessels.length)
      : 0;
  const avgUtil =
    vessels.length > 0
      ? Math.round((vessels.reduce((s, v) => s + (v.utilization ?? 0), 0) / vessels.length) * 10) /
        10
      : 0;
  const topPerformers = [...vessels].sort((a, b) => (b.tce ?? 0) - (a.tce ?? 0)).slice(0, 5);
  const bottomPerformers = [...vessels]
    .filter((v) => (v.tce ?? 0) > 0)
    .sort((a, b) => (a.tce ?? 0) - (b.tce ?? 0))
    .slice(0, 5);

  const utilizationData = vessels
    .filter((v) => (v.utilization ?? 0) > 0)
    .map((v) => ({
      name: v.name,
  ...
  vessels.forEach((v) => {
    if (!tceByType[v.vesselType ?? 'unknown'])
      tceByType[v.vesselType ?? 'unknown'] = { count: 0, total: 0 };
    if ((v.tce ?? 0) > 0) {
      tceByType[v.vesselType ?? 'unknown'].count++;
      tceByType[v.vesselType ?? 'unknown'].total += v.tce ?? 0;
    }
  });
  const marketTrendData = Object.entries(tceByType)
    .filter(([, d]) => d.count > 0)
    .map(([type, d]) => ({
      type: type.charAt(0).toUpperCase() + type.slice(1),
      avgTCE: Math.round(d.total / d.count),
      benchmark:
        type === 'container' ? 22000 : type === 'tanker' ? 28000 : type === 'bulk' ? 14000 : 16000,
    }));
    ...
    <div className="p-6 space-y-6">
      <div className="animate-fade-in-up">
        ...
        <p className="text-xs text-muted-foreground mt-1 tracking-wider font-mono">
          TCE ANALYTICS // UTILIZATION TRACKING // MARKET BENCHMARKS // VOYAGE P&L
        </p>
      </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-1 mt-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {vessels.filter((v) => v.status === 'at_sea').length} at sea
                  </span>
                </div>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold font-display mt-1">
                  {vessels.length > 0
                    ? (vessels.reduce((s, v) => s + (v.eexi ?? 0), 0) / vessels.length).toFixed(1)
                    : 0}
                </p>
                <div className="flex items-center gap-1 mt-1">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <AreaChart data={monthlyTrend}>
                ...
                <Tooltip
                  ...
                  formatter={(v: number) => [`$${v.toLocaleString()}/day`, '']}
                />
                <Legend />
                <Area
                  type="monotone"
          ...
          <CardContent>
            <div className="space-y-3">
              {topPerformers.map((v, i) => (
                <div
                  key={v.id}
                  className="flex items-center justify-between p-3 rounded-lg border border-border bg-background/50 hover:border-emerald-500/20 transition-all"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground capitalize">
                        {v.vesselType ?? 'unknown'} · {v.flag}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-bold text-emerald-400">
                      ${(v.tce ?? 0).toLocaleString()}/day
                    </p>
          ...
          <CardContent>
            <div className="space-y-3">
              {bottomPerformers.map((v, i) => (
                <div
                  key={v.id}
                  className="flex items-center justify-between p-3 rounded-lg border border-border bg-background/50 hover:border-amber-500/20 transition-all"
                >
                  <div className="flex items-center gap-3">
                  ...
                  <div className="text-right">
                    ...
                    <Badge
                      variant="outline"
                      className={`text-xs ${(v.utilization ?? 0) < 85 ? 'bg-red-500/10 text-red-400' : 'bg-amber-500/10 text-amber-400'}`}
                    >
                      {v.utilization}%
                    </Badge>
                  </div>
