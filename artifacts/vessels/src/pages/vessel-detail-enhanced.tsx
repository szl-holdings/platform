...
export default function VesselDetailEnhancedPage() {
  ...
  const auditEntries: AuditHistoryEntry[] = (() => {
    ...
    entries.push({
      id: 'vessel-added',
      action: 'Vessel in fleet registry',
      actor: 'System',
      actorType: 'system',
      newState: vessel.status,
      timestamp: vesselCreatedAt,
    });
    exceptions.forEach((exc: any, i: number) => {
      entries.push({
        id: `exc-${i}`,
        action: `Exception raised: ${exc.title}`,
        actor: exc.ownerFunction ?? 'Fleet Ops',
        actorType: 'user',
        newState: 'exception_open',
        notes: exc.description?.slice(0, 80),
        timestamp: exc.raisedAt ?? exc.detectedAt ?? vesselCreatedAt,
      });
    });
    portCalls.slice(0, 5).forEach((pc: any, i: number) => {
      entries.push({
        id: `pc-${i}`,
        action: `Port call: ${pc.portName}`,
        actor: 'AIS System',
        actorType: 'system',
        newState: 'in_port',
        notes: pc.purpose ? `Purpose: ${pc.purpose}` : undefined,
        timestamp: pc.arrivalAt ?? pc.arrivedAt ?? vesselCreatedAt,
      });
    });
    if (activeVoyage) {
      entries.push({
        id: 'voyage-active',
        action: `Active voyage: ${activeVoyage.originPort} → ${activeVoyage.destinationPort}`,
        actor: 'Voyage System',
        actorType: 'system',
        newState: 'at_sea',
        notes: activeVoyage.voyageRef,
        timestamp: activeVoyage.scheduledDepartureAt ?? vesselCreatedAt,
      });
    }
    maintenance.slice(0, 3).forEach((m: any, i: number) => {
      if (m.status === 'in_progress') {
        entries.push({
          id: `maint-${i}`,
          action: `Maintenance in progress: ${m.component}`,
          actor: m.technician ?? 'Maintenance Ops',
          actorType: 'user',
          notes: m.description?.slice(0, 60),
          timestamp: vesselUpdatedAt,
        });
      }
    });
    return entries.sort(
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {exceptions.length > 0 && (
              <Badge
                variant="outline"
                className="text-xs text-red-400 bg-red-500/10 border-red-500/20"
              >
          ...
          <div className="bg-[#0a1628]/80 border border-sky-500/10 rounded-xl p-4 col-span-2">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                {
                  label: 'Speed',
                  value: position.speed ? `${parseFloat(position.speed).toFixed(1)} kn` : '—',
                },
                {
                  label: 'Heading',
                  value: position.heading ? `${parseFloat(position.heading).toFixed(0)}°` : '—',
                },
                ...
                <div className="flex items-center justify-between">
                  ...
                  <p
                    className={cn(
                      ...
                      parseFloat(sanctions.complianceScore ?? '0') >= 75
                        ? 'text-emerald-400'
                        : 'text-amber-400',
                    )}
                  >
                    {fmtNum(sanctions.complianceScore, 0)}/100
                  </p>
                </div>
              </div>
            ) : (
                  ...
                  <div className="flex items-start gap-2">
                    <AlertTriangle
                      className={cn(
                        ...
                        exc.severity === 'critical' ? 'text-red-400' : 'text-orange-400',
                      )}
                    />
                    <div>
              ...
              <div className="bg-[#0a1628]/80 border border-sky-500/10 rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    {
                      label: 'Gross Revenue',
                      value: fmtUsd(activeVoyage.grossRevenue),
                      color: 'text-emerald-400',
                    },
                    {
                      label: 'Total Costs',
                      value: fmtUsd(activeVoyage.totalCostsUsd),
                    ...
                    {
                      label: 'Margin %',
                      value: activeVoyage.marginPct
                        ? `${parseFloat(activeVoyage.marginPct) * 100 > 1 ? parseFloat(activeVoyage.marginPct).toFixed(1) : (parseFloat(activeVoyage.marginPct) * 100).toFixed(1)}%`
                        : '—',
                      color: 'text-violet-400',
                    },
                    {
                      label: 'TCE/day',
                      value: activeVoyage.tcePerDay
                    ...
                    {
                      label: 'Fuel Cost',
                      value: fmtUsd(activeVoyage.fuelCostUsd),
                      color: 'text-amber-400',
                    },
                  ].map((item) => (
                    <div
                ...
                <div className="bg-[#0a1628]/80 border border-sky-500/10 rounded-xl p-4">
                  ...
                  <p
                    className={cn(
                      ...
                      parseFloat(activeVoyage.delayHours ?? '0') > 0
                        ? 'text-orange-400'
                        : 'text-emerald-400',
                    )}
                  >
                    {parseFloat(activeVoyage.delayHours ?? '0') > 0
                      ? `${parseFloat(activeVoyage.delayHours!).toFixed(0)}h delayed`
                      : 'On schedule'}
                  </p>
                  {parseFloat(activeVoyage.delayCostUsd ?? '0') > 0 && (
                    <p className="text-[10px] text-orange-400/70">
                      {fmtUsd(activeVoyage.delayCostUsd)} delay cost
                    </p>
                  )}
                </div>
              </div>
                ...
                {[
                  {
                    label: 'Fuel Cost',
                    value: fmtUsd(activeVoyage.fuelCostUsd),
                    sub: activeVoyage.fuelConsumedMt
                      ? `${parseFloat(activeVoyage.fuelConsumedMt).toFixed(0)}t consumed`
                      : '—',
                    icon: Fuel,
                    color: 'text-amber-400',
                  },
                  ...
                  },
                  {
                    ...
                    value:
                      parseFloat(activeVoyage.delayCostUsd ?? '0') > 0
                        ? fmtUsd(activeVoyage.delayCostUsd)
                        : 'None',
                    sub:
                      parseFloat(activeVoyage.delayHours ?? '0') > 0
                        ? `${parseFloat(activeVoyage.delayHours!).toFixed(0)}h delay`
                        : 'On schedule',
                    icon: Clock,
                    color:
                      parseFloat(activeVoyage.delayCostUsd ?? '0') > 0
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-3">
                      ...
                      <div>
                        ...
                        <p
                          className={cn(
                            'text-[10px] font-mono',
                            parseFloat(m.assetHealth ?? '75') >= 80
                              ? 'text-emerald-400'
                              : parseFloat(m.assetHealth ?? '75') >= 60
                                ? 'text-amber-400'
                                : 'text-red-400',
                          )}
                        >
                          {fmtNum(m.assetHealth, 0)}/100
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="flex-1">
                    ...
                    {pc.canalTransit && pc.canalName && (
                      <p className="text-[9px] text-amber-400/60 mt-2">
                        Canal transit: {pc.canalName} · {fmtUsd(pc.canalFeeUsd)}
                      </p>
                    )}
                    {pc.agentName && (
          ...
          <div className="flex items-center justify-between">
            ...
            {sanctions?.knownOwner && (
              <div className="text-right">
                ...
                <OperationalOwnerChip
                  owner={{
                    name: sanctions.knownOwner,
                    role: sanctions.knownManager ? `Mgr: ${sanctions.knownManager}` : 'Owner',
                  }}
                  size="xs"
                />
              </div>
