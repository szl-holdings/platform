...
import {
  ...
  Play, FileText, Activity, Layers, XCircle, Info, ChevronDown, ChevronUp, Anchor
} from "lucide-react";
...
const SIGNAL_TYPES = ["voyage-anomaly", "sanctions-match", "ais-gap", "cargo-risk", "port-risk", "weather-hazard", "dark-vessel"];
...
export default function VesselsAtlasExecute() {
  ...
  const [newSignal, setNewSignal] = useState({ signalType: SIGNAL_TYPES[0], severity: "high" as Severity, title: "", description: "", source: "operator", confidence: 0.85 });
  ...
  const ingestSignal = async () => {
    ...
    try {
      ...
      setNewSignal({ signalType: SIGNAL_TYPES[0], severity: "high", title: "", description: "", source: "operator", confidence: 0.85 });
      await load();
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>
            Ingest voyage signals, run governed workflows, review execution evidence, and approve pending operations.
          </p>
        </div>
      ...
      {pendingApproval && (
        <div className="rounded-xl border p-4 flex items-start gap-4" style={{ borderColor: `${ACCENT}40`, background: `${ACCENT}10` }}>
          ...
          <div className="flex-1">
            <div className="text-[11px] font-bold mb-0.5" style={{ color: ACCENT }}>Operator Approval Required</div>
            <div className="text-[11px] text-white mb-1">{pendingApproval.reason ?? "This voyage workflow requires operator approval before execution continues."}</div>
        ...
        <div className="flex flex-col gap-3">
          <div className="rounded-xl border overflow-hidden" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
            ...
            <div className="divide-y divide-white/[0.04] max-h-80 overflow-y-auto">
              ...
              {!loading && signals.length === 0 && <div className="p-4 text-center text-[11px]" style={{ color: "rgba(255,255,255,0.3)" }}>No signals. Ingest one to begin.</div>}
              {signals.map(sig => {
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="rounded-xl border p-3.5 flex items-start gap-2.5" style={{ borderColor: `${ACCENT}20`, background: `${ACCENT}06` }}>
            ...
            <div className="text-[9px] leading-relaxed" style={{ color: "rgba(255,255,255,0.35)" }}>
              Select voyage signals, then <strong className="text-white">Run Voyage Workflow</strong> to trigger the sanctions check, risk estimate, and operator action plan pipeline. Sanctions matches trigger an automatic hard block — operator approval is required for high-risk voyages.
            </div>
          </div>
        </div>
      </div>
