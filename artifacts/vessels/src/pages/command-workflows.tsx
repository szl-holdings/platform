...
const EVENT_TYPE_CONFIG: Record<string, { label: string; icon: React.ElementType; color: string }> = {
  ...
  alert_classification: { label: "Alert Classification", icon: AlertTriangle, color: "text-rose-400" },
...
const FALLBACK_EVENTS = [
  {
    ...
    description: "Route delay due to Strait of Gibraltar congestion. Port slot at Rotterdam at risk. Delay impact: $420K.",
    consequenceData: { delayHours: 34, marginImpact: 420000, routePressure: "high", fuelImpact: 18000, portSlotRisk: true },
    occurredAt: new Date(Date.now() - 2 * 3600000).toISOString(),
  },
  {
    id: 2, vesselId: 2, eventType: "route_deviation", severity: "warning", status: "acknowledged",
    title: "Route Deviation — Pacific Guardian off optimal lane",
    description: "Vessel deviated 42nm from planned route due to weather system. Monitoring fuel burn impact.",
  ...
  {
    id: 3, vesselId: 3, eventType: "maintenance_watch", severity: "warning", status: "open",
    title: "Engine Maintenance Watch — Nordic Crest",
    description: "Predictive maintenance flag: Main engine bearing wear at 78% threshold. Recommend inspection at next port call.",
    consequenceData: { componentRisk: "main_engine", wearPct: 78, inspectionDue: "Rotterdam", operationalRisk: "medium" },
    occurredAt: new Date(Date.now() - 18 * 3600000).toISOString(),
  },
  {
    id: 4, vesselId: 4, eventType: "port_congestion", severity: "watch", status: "assigned",
    title: "Port Congestion — Singapore Anchorage",
    description: "Average wait time at Singapore increased to 4.2 days. Southern Cross scheduled arrival may face anchor queue.",
  ...
  {
    id: 5, vesselId: 1, eventType: "weather_pressure", severity: "critical", status: "open",
    title: "Severe Weather — Typhoon Track Intersecting Route",
    description: "Tropical storm tracking to intercept planned route. Rerouting required within 6h window.",
    consequenceData: { windKnots: 55, waveHeightM: 8.5, routePressure: "severe", rerouting: "recommended" },
...
export default function CommandWorkflowsPage() {
  ...
  const criticalOpen = events.filter((e: any) => e.severity === "critical" && e.status === "open").length;
  const totalDelayImpact = events.reduce((s: number, e: any) => s + (e.consequenceData?.marginImpact || e.consequenceData?.revenueImpact || 0), 0);
    ...
    <div className="flex flex-col gap-6 p-6 min-h-full">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-0.5">Exception queue, owner assignment, route interventions — powered by Alloy</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className="bg-orange-500/5 border-orange-500/20">
          <CardContent className="p-4">
            ...
            <div className="text-3xl font-bold text-orange-400">{formatCurrency(totalDelayImpact)}</div>
          </CardContent>
        </Card>
        ...
        {filtered.map((event: any) => {
          ...
          return (
            <Card key={event.id} className={cn(
              ...
              event.severity === "critical" ? "border-red-500/30 bg-red-500/3" :
              event.severity === "warning" ? "border-amber-500/20 bg-amber-500/3" :
                ...
                <div className="flex items-start gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1.5">
                      ...
                      <div className={`flex items-center gap-1 text-[10px] font-medium ${typeInfo.color}`}>
                    ...
                    {Object.keys(cd).length > 0 && (
                      <div className="flex flex-wrap gap-3 mb-2">
                        {cd.delayHours && <span className="text-[10px] text-orange-400">⏱ {cd.delayHours}h delay</span>}
                        {cd.marginImpact && <span className="text-[10px] text-red-400">💰 {formatCurrency(cd.marginImpact)} margin impact</span>}
                        {cd.revenueImpact && <span className="text-[10px] text-red-400">💰 {formatCurrency(cd.revenueImpact)} revenue impact</span>}
                        {cd.fuelImpact && <span className="text-[10px] text-amber-400">⛽ {formatCurrency(cd.fuelImpact)} fuel impact</span>}
                        {cd.deviationNm && <span className="text-[10px] text-sky-400">📍 {cd.deviationNm}nm deviation</span>}
                        {cd.wearPct && <span className="text-[10px] text-yellow-400">🔧 {cd.wearPct}% wear</span>}
                        {cd.waitDays && <span className="text-[10px] text-purple-400">⚓ {cd.waitDays}d wait</span>}
                        {cd.windKnots && <span className="text-[10px] text-sky-400">💨 {cd.windKnots}kts wind</span>}
                      </div>
                    )}

                    <div className="flex items-center gap-3 text-[10px] text-slate-500">
                      {event.assignedTo && <span className="flex items-center gap-1"><User className="w-3 h-3" />{event.assignedTo}</span>}
                  ...
                  <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                    ...
                    <Button
                      ...
                      onClick={() => createWorkflow.mutate({
                        ...
                        workflowType: event.severity === "critical" ? "escalation" : "owner_assignment",
                    ...
                    {event.eventType === "route_deviation" || event.eventType === "weather_pressure" ? (
                      <Button
                        size="sm"
                        variant="outline"
