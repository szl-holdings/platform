...
export default function WeatherPage() {
  const { data: rawSnapshots = [], isLoading } = useStandardQuery({
    queryKey: ['weather'],
    queryFn: () => api.weather.snapshots(),
  });
  const snapshots = rawSnapshots as unknown as WeatherSnapshot[];
  const { data: rawRoutes = [] } = useStandardQuery({
    queryKey: ['routes'],
  ...
  const severeCount = snapshots.filter(
    (s) => s.riskLevel === 'severe' || s.riskLevel === 'high',
  ).length;
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="animate-fade-in-up flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Sea state forecasts, wind and wave conditions, and storm avoidance routing
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold font-display mt-1">
                  <AnimatedCounter value={snapshots.length} />
                </p>
              </div>
        ...
        <Card
          className={`bg-card border-border animate-fade-in-up stagger-2 hover:border-chart-5/20 transition-all duration-300 group ${severeCount > 0 ? 'ring-1 ring-chart-5/10' : ''}`}
        >
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold font-display mt-1 text-chart-5">
                  <AnimatedCounter value={severeCount} />
                </p>
              </div>
              <div
                className={`w-10 h-10 rounded-lg bg-chart-5/10 flex items-center justify-center group-hover:scale-110 transition-transform ${severeCount > 0 ? 'animate-pulse' : ''}`}
              >
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold font-display mt-1">
                  <AnimatedCounter
                    value={new Set(snapshots.map((s) => s.routeId).filter(Boolean)).size}
                  />
                </p>
              </div>
          ...
          {marineLoading ? (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-4">
                <div className="p-3 rounded-lg bg-blue-500/5 border border-blue-500/10">
                  ...
                  <p className="text-2xl font-bold text-blue-400">
                    {marineWeather.current.waveHeight != null
                      ? `${marineWeather.current.waveHeight.toFixed(1)}m`
                      : '—'}
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-cyan-500/5 border border-cyan-500/10">
                  ...
                  <p className="text-2xl font-bold text-cyan-400">
                    {marineWeather.current.swellWaveHeight != null
                      ? `${marineWeather.current.swellWaveHeight.toFixed(1)}m`
                      : '—'}
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/10">
                  ...
                  <p className="text-2xl font-bold text-emerald-400">
                    {marineWeather.current.windWaveHeight != null
                      ? `${marineWeather.current.windWaveHeight.toFixed(1)}m`
                      : '—'}
                  </p>
                </div>
              </div>
              {(marineWeather.forecastHours?.length ?? 0) > 0 && (
                <div>
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-1">
                    {(marineWeather.forecastHours ?? [])
                      .slice(0, 12)
                      .map((h: ForecastHour, i: number) => (
                        <div
                          key={i}
                          className="shrink-0 text-center p-2 rounded bg-muted/30 min-w-[50px]"
                        >
              ...
              <p className="text-[10px] text-muted-foreground">
                Source: Open-Meteo Marine API · Updates hourly
              </p>
            </div>
          ) : (
        ...
        <Card className="bg-card border-border border-dashed animate-fade-in-up stagger-4">
          <CardContent className="p-16 text-center">
            ...
            <p className="text-xs text-muted-foreground/60 mt-1">
              Weather snapshots will appear once routes are being monitored
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {snapshots.map((snap, i: number) => {
            ...
            return (
              <Card
                key={snap.id}
                className={`bg-card border-border hover:border-primary/20 transition-all duration-300 relative overflow-hidden animate-fade-in-up stagger-${Math.min((i % 6) + 1, 8)} ${riskGlow[snap.riskLevel] || ''}`}
              >
                ...
                <CardHeader className="pb-3 relative">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      variant="outline"
                      className={`${riskColors[snap.riskLevel] || ''} ${isSevere ? 'animate-pulse' : ''}`}
                    >
                      {isSevere && (
                        <span
                          className={`w-1.5 h-1.5 rounded-full mr-1.5 animate-pulse-dot ${snap.riskLevel === 'severe' ? 'bg-red-400' : 'bg-orange-400'}`}
                        />
                      )}
                      {snap.riskLevel} risk
                    </Badge>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="flex items-center gap-2 text-xs bg-muted/30 rounded-lg px-2.5 py-1.5">
                      ...
                      <span className="font-medium">
                        {snap.temperature ? `${Number(snap.temperature)}°C` : 'N/A'}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-xs bg-muted/30 rounded-lg px-2.5 py-1.5">
                      ...
                      <span className="font-medium">
                        {snap.windSpeed
                          ? `${Number(snap.windSpeed)} km/h ${snap.windDirection || ''}`
                          : 'N/A'}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-xs bg-muted/30 rounded-lg px-2.5 py-1.5">
                      <Waves
                        className={`w-3.5 h-3.5 ${riskIcons[snap.riskLevel]} ${isSevere ? 'animate-wave-float' : ''}`}
                      />
                      <span className="text-muted-foreground">Waves:</span>
                      <span className="font-medium">
                        {snap.waveHeight ? `${Number(snap.waveHeight)}m` : 'N/A'}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-xs bg-muted/30 rounded-lg px-2.5 py-1.5">
                      ...
                      <span className="font-medium">
                        {snap.visibility ? `${Number(snap.visibility)} km` : 'N/A'}
                      </span>
                    </div>
                  </div>
