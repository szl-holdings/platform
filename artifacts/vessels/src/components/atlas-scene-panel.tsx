...
import { Anchor, MapPin, Package, ShieldAlert, Activity, TrendingDown, Clock } from "lucide-react";
...
const DEMO_CARGO = [
  { id: "BL-001", commodity: "Iron Ore", quantity: "44,200 MT", origin: "Fujairah", shipper: "Emirates Mining Corp", sanctionFlag: false },
  { id: "BL-002", commodity: "Coal (Thermal)", quantity: "21,800 MT", origin: "Ruwais", shipper: "Gulf Energy Ltd", sanctionFlag: false },
  { id: "BL-003", commodity: "Steel Coils", quantity: "8,400 MT", origin: "Jebel Ali", shipper: "Nile Steel Group", sanctionFlag: true },
];

const DEMO_RISKS = [
  { label: "Sanctions Exposure", score: 0.61, level: "HIGH", detail: "1 B/L shipper (Nile Steel Group) matches OFAC secondary list cluster", color: "#f59e0b" },
  { label: "War Risk Premium", score: 0.44, level: "MEDIUM", detail: "Red Sea corridor proximity — war risk up 18% since Apr 12", color: "#fbbf24" },
  { label: "Port State Control", score: 0.22, level: "LOW", detail: "Last PSC inspection clear — Rotterdam Port Authority, Mar 2026", color: "#4ade80" },
  { label: "Weather Routing", score: 0.31, level: "LOW", detail: "Sea state 3–4 forecast on current heading — within operating envelope", color: "#4ade80" },
];
    ...
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1rem 1.25rem 0" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
          ...
          {isDemo && (
            <span style={{ fontSize: "0.6rem", fontFamily: "monospace", padding: "0.1rem 0.4rem", borderRadius: "2rem", background: "rgba(14,165,233,0.1)", color: ACCENT, border: "1px solid rgba(14,165,233,0.25)" }}>
              DEMO
            </span>
          )}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
          ...
          <span style={{ fontSize: "0.6rem", fontFamily: "monospace", padding: "0.15rem 0.5rem", borderRadius: "2rem", background: "rgba(34,197,94,0.1)", color: "#4ade80", border: "1px solid rgba(34,197,94,0.2)" }}>
            {v.status}
          </span>
        </div>
      </div>
      ...
      <div style={{ padding: "1rem 1.25rem 1.25rem" }}>
        {tab === "position" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.5rem 0.625rem", background: "rgba(14,165,233,0.05)", borderRadius: "0.375rem", border: "1px solid rgba(14,165,233,0.12)" }}>
              ...
              <span style={{ fontSize: "0.6875rem", color: "rgba(255,255,255,0.55)", fontFamily: "monospace" }}>
                AIS signal last received <strong style={{ color: "rgba(255,255,255,0.8)" }}>4 min ago</strong> · MMSI 357234810
              </span>
            </div>
          </div>
        )}
            ...
            {DEMO_CARGO.map((bl) => (
              <div
                ...
                style={{
                  ...
                  border: bl.sanctionFlag ? "1px solid rgba(239,68,68,0.2)" : "1px solid rgba(255,255,255,0.07)",
                  borderRadius: "0.5rem",
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "0.5rem" }}>
                  <div>
                    <p style={{ fontSize: "0.8125rem", color: "rgba(255,255,255,0.85)", fontWeight: 500, margin: "0 0 0.25rem" }}>{bl.commodity}</p>
                    <p style={{ fontSize: "0.6875rem", color: "rgba(255,255,255,0.4)", fontFamily: "monospace", margin: 0 }}>
                      {bl.quantity} · {bl.origin} · {bl.shipper}
                    </p>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "0.25rem" }}>
                    <span style={{ fontSize: "0.6rem", fontFamily: "monospace", color: "rgba(255,255,255,0.3)", letterSpacing: "0.06em" }}>{bl.id}</span>
                    {bl.sanctionFlag && (
                      <span style={{ fontSize: "0.6rem", fontFamily: "monospace", fontWeight: 600, padding: "0.15rem 0.4rem", borderRadius: "2rem", background: "rgba(239,68,68,0.14)", color: "#f87171", border: "1px solid rgba(239,68,68,0.3)" }}>
                        OFAC FLAG
                      </span>
                    )}
                  </div>
        ...
        {tab === "risk" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.5rem 0.625rem", background: "rgba(245,158,11,0.06)", borderRadius: "0.375rem", border: "1px solid rgba(245,158,11,0.15)" }}>
              ...
              <span style={{ fontSize: "0.6875rem", color: "rgba(255,255,255,0.55)", fontFamily: "monospace" }}>
                Composite risk score <strong style={{ color: "#fbbf24" }}>0.44 / MEDIUM</strong> — awaiting compliance review for B/L BL-003
              </span>
            </div>
          </div>
        )}
