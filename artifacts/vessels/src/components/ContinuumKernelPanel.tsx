...
import { Anchor, BookOpen, Brain, Sparkles } from 'lucide-react';
import { useMemo, useState } from 'react';

export function AlloyKernelPanel() {
  const kernel = useMemo(() => getKernel('maritime-risk-brief'), []);
  const [showExample, setShowExample] = useState(false);
  const example = kernel.codex.examples[0];

  return (
    <div className="rounded-xl border border-emerald-500/15 bg-slate-900/60 p-5 mt-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-sm font-bold text-slate-100">Voyage Risk Brief</div>
      ...
      {showExample && example && (
        <div className="mt-3 rounded-lg bg-slate-950/80 border border-emerald-500/10 p-3">
