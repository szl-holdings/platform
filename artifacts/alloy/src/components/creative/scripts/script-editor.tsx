      ...
      <div className="flex-1 flex flex-col bg-[#0d1117] rounded-xl border border-white/8 overflow-hidden">
        <div className="h-14 border-b border-white/8 flex items-center justify-between px-4 bg-white/3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] px-2 py-0.5 rounded border border-white/10 text-slate-400">v{activeScript.version}</span>
            <span className={`text-[10px] px-2 py-0.5 rounded capitalize ${activeScript.status === "approved" ? "bg-emerald-500/10 text-emerald-400" : "bg-white/5 text-slate-400"}`}>
              {activeScript.status}
            </span>
          </div>
          <button
        ...
        <div className="bg-[#0d1117] rounded-xl border border-white/8 p-4">
          ...
          {activeScript.status !== "approved" && (
            <button
              ...
              onClick={() => updateScript.mutate({ id: activeScript.id, campaignId: parseInt(campaignId, 10), status: "approved" })}
            >
