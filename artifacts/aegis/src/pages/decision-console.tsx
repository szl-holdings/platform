...
const DECISIONS = [
  {
    ...
    alternatives: [
      { hypothesis: "Insider threat — privileged misuse", confidence: 22, ruling: "unlikely", rationale: "C2 beacon to known APT29 IP makes insider-only scenario implausible" },
      { hypothesis: "Automated attack tool — no human operator", confidence: 31, ruling: "possible", rationale: "TTPs are consistent with automation but lateral movement is precise" },
    ],
    recommendedOutcome: "Escalate to active response. Isolate DC-PROD-03. Revoke SVC-ACCNT-04. Block 103.45.18.22.",
    auditChain: [
      { action: "Decision object created", user: "J. Chen", at: "15:42 UTC", hash: "sha256:a3f1..." },
      { action: "Evidence linked (3 items)", user: "J. Chen", at: "15:44 UTC", hash: "sha256:b9c2..." },
      { action: "Submitted for approval", user: "J. Chen", at: "15:49 UTC", hash: "sha256:c7d4..." },
    ],
  },
  {
    id: "DEC-040",
    ...
    envLabel: "PRODUCTION",
    evidence: [
      { id: "EV-10", name: "auth-fail-log-1204.csv", trustLevel: "verified" },
    ],
    keyAssumptions: [
    ...
    alternatives: [
      { hypothesis: "Targeted credential attack by known threat actor", confidence: 8, ruling: "unlikely", rationale: "Pattern, timing, and source ASN do not match known targeted campaigns" },
    ],
    recommendedOutcome: "Block source ASN range. Close as automated threat. No escalation required.",
    auditChain: [
      { action: "Decision object created", user: "L. Kim", at: "12:11 UTC", hash: "sha256:f2a8..." },
      { action: "Approved by SOC Manager", user: "M. Walsh", at: "12:24 UTC", hash: "sha256:g5b1..." },
    ],
  },
];

const APPROVAL_COLORS: Record<string, { bg: string; text: string; border: string; label: string }> = {
  pending:  { bg: "bg-amber-500/10", text: "text-amber-300", border: "border-amber-500/25", label: "Awaiting Approval" },
  approved: { bg: "bg-emerald-500/10", text: "text-emerald-300", border: "border-emerald-500/25", label: "Approved" },
  rejected: { bg: "bg-red-500/10", text: "text-red-300", border: "border-red-500/25", label: "Rejected" },
      ...
      <div className="w-72 shrink-0 border-r border-white/5 flex flex-col">
        <div className="px-4 py-3 border-b border-white/5">
          ...
          <p className="text-[10px] mt-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>
            {sensitivityLabel ? <><Lock className="w-2.5 h-2.5 inline mr-1" />{sensitivityLabel} · </> : null}
            Structured analytical decisions
            {liveDecisions.length > 0 && <span className="text-blue-400 ml-1">· {liveDecisions.length} live</span>}
          </p>
        </div>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isActive ? "bg-blue-500/10 border-l-2 border-blue-400" : "hover:bg-white/[0.02] border-l-2 border-transparent"
                    )}
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[9px] font-mono text-white/40">FIND-{String(dec.id).padStart(4, "0")}</span>
                      <span className={cn("text-[8px] font-mono px-1 py-0.5 rounded border", st.bg, st.text, st.border)}>{st.label}</span>
                    </div>
                    <p className="text-[11px] font-medium text-white/85 leading-snug mb-1 line-clamp-2">{dec.title}</p>
            ...
            : DECISIONS.map(dec => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "w-full text-left px-4 py-3 transition-all",
                      selectedMockId === dec.id ? "bg-blue-500/10 border-l-2 border-blue-400" : "hover:bg-white/[0.02] border-l-2 border-transparent"
                    )}
                  >
                    <div className="flex items-center justify-between mb-1">
                    ...
                    <p className="text-[11px] font-medium text-white/85 leading-snug mb-1">{dec.title}</p>
                    <div className="flex items-center gap-2 text-[10px]" style={{ color: "rgba(255,255,255,0.35)" }}>
      ...
      <div className="flex-1 overflow-y-auto">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1.5">
                {usingLive && selectedLive ? (
                  <span className="text-[9px] font-mono text-white/40">FIND-{String(selectedLive.id).padStart(4, "0")}</span>
                ) : (
                  <span className="text-[9px] font-mono text-white/40">{selectedMock.id}</span>
                )}
                <span className="text-[8px] font-mono text-white/30">→</span>
                <span className="text-[9px] font-mono text-white/40">{usingLive && selectedLive ? selectedLive.affectedAsset ?? "—" : selectedMock.caseRef}</span>
                <span className="text-[8px] font-mono px-1.5 py-0.5 rounded border border-emerald-500/30 bg-emerald-500/5 text-emerald-400/70">{envLabel ?? (usingLive ? "PRODUCTION" : selectedMock.envLabel)}</span>
                {!usingLive && <span className="text-[8px] font-mono px-1.5 py-0.5 rounded border border-blue-500/30 bg-blue-500/5 text-blue-400/70">{selectedMock.tenantLabel}</span>}
                <span className="text-[8px] font-mono px-1.5 py-0.5 rounded border border-red-500/20 bg-red-500/5 text-red-400/70 flex items-center gap-0.5">
                  <Lock className="w-2 h-2" />{sensitivityLabel ?? (usingLive ? "RESTRICTED" : selectedMock.sensitivityLabel)}
                </span>
              </div>
              <h1 className="text-sm font-bold text-white leading-snug">{usingLive && selectedLive ? selectedLive.title : selectedMock.title}</h1>
              <div className="flex items-center gap-3 mt-1 text-[10px]" style={{ color: "rgba(255,255,255,0.4)" }}>
                ...
                <span>·</span>
                <span className={cn("flex items-center gap-1 font-mono px-1.5 py-0.5 rounded border", approvalStyle.bg, approvalStyle.text, approvalStyle.border)}>
                  {approvalState === "approved" ? <CheckCircle2 className="w-3 h-3" /> : approvalState === "rejected" ? <XCircle className="w-3 h-3" /> : <Clock className="w-3 h-3" />}
                  {approvalStyle.label}
                </span>
              </div>
            </div>
            {(usingLive && selectedLive && (selectedLive.status === "open" || selectedLive.status === "confirmed")) ||
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {usingLive && selectedLive && selectedLive.recommendation && (
            <div className="bg-white/[0.025] border border-white/5 rounded-xl p-5">
              <div className="text-[10px] font-mono uppercase tracking-[0.15em] mb-2 text-white/40">Recommendation</div>
              <p className="text-[11px] text-white/75 leading-relaxed">{selectedLive.recommendation}</p>
            </div>
          )}

          {usingLive && selectedLive && selectedLive.affectedAsset && (
            <div className="bg-white/[0.025] border border-white/5 rounded-xl p-5">
              <div className="text-[10px] font-mono uppercase tracking-[0.15em] mb-2 text-white/40">Affected Asset</div>
          ...
          {!usingLive && (
            <>
              ...
              <div className="bg-white/[0.025] border border-white/5 rounded-xl p-5">
                <div className="text-[10px] font-mono uppercase tracking-[0.15em] mb-3 text-white/40">Evidence ({selectedMock.evidence.length})</div>
                <div className="space-y-2">
                  {selectedMock.evidence.map(ev => (
                    <div key={ev.id} className="flex items-center gap-3 bg-white/[0.025] rounded-lg px-3 py-2">
              ...
              <div className="bg-white/[0.025] border border-white/5 rounded-xl p-5">
                <div className="text-[10px] font-mono uppercase tracking-[0.15em] mb-3 text-white/40">Key Assumptions</div>
              ...
              <div className="bg-white/[0.025] border border-white/5 rounded-xl p-5">
                <div className="text-[10px] font-mono uppercase tracking-[0.15em] mb-3 text-white/40">Alternative Hypotheses</div>
                <div className="space-y-3">
                  {selectedMock.alternatives.map((alt, i) => (
                    <div key={i} className="bg-white/[0.02] rounded-lg p-3">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-[11px] font-medium text-white/85">{alt.hypothesis}</span>
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-mono text-white/40 tabular-nums">{alt.confidence}%</span>
                          <span className={cn("text-[8px] font-mono px-1.5 py-0.5 rounded border uppercase",
                            alt.ruling === "unlikely" ? "border-emerald-500/20 bg-emerald-500/5 text-emerald-400/70" :
                            alt.ruling === "possible" ? "border-yellow-500/20 bg-yellow-500/5 text-yellow-400/70" :
          ...
          {/* Approval State */}
          <div className="bg-white/[0.025] border border-white/5 rounded-xl p-5">
            <div className="text-[10px] font-mono uppercase tracking-[0.15em] mb-3 text-white/40">Approval Chain</div>
            <div className="flex items-center gap-3 mb-3">
              <div className={cn("flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-semibold", approvalStyle.bg, approvalStyle.text, approvalStyle.border)}>
                {approvalState === "approved" ? <CheckCircle2 className="w-3.5 h-3.5" /> : <Clock className="w-3.5 h-3.5" />}
                {approvalStyle.label}
              </div>
            ...
            {usingLive && selectedLive && selectedLive.remediationOwner && (
              <p className="text-[10px] text-white/40">Remediation owner: {selectedLive.remediationOwner}</p>
            )}
          </div>
          ...
          <div className="bg-white/[0.025] border border-white/5 rounded-xl overflow-hidden">
            {(() => {
              const auditEntries = usingLive && selectedLive
                ...
                <>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-mono uppercase tracking-[0.15em] text-white/40">Audit Chain ({auditEntries.length} entries)</span>
                    </div>
                    {expandedAudit ? <ChevronDown className="w-3.5 h-3.5 text-white/30" /> : <ChevronRight className="w-3.5 h-3.5 text-white/30" />}
                  </button>
                  {expandedAudit && (
                      ...
                      {auditEntries.map((entry, i) => (
                        <div key={i} className="px-5 py-3 flex items-center justify-between gap-4">
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-0.5 text-[10px]" style={{ color: "rgba(255,255,255,0.35)" }}>
                              <span>{entry.user}</span>
                              <span>·</span>
                              <span>{entry.at}</span>
                            </div>
                          </div>
                          {entry.hash && <span className="text-[9px] font-mono text-white/20">{entry.hash}</span>}
                        </div>
                      ))}
                    </div>
                  )}
