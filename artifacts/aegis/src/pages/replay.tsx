...
interface ReplayEvent {
  id: string;
  t: number;
  type: "initial_access" | "execution" | "persistence" | "lateral_move" | "exfil" | "c2" | "discovery" | "defense_evasion";
...
const SEV_COLOR: Record<EventSeverity, string> = {
  critical: "#ef4444", high: "#f59e0b", medium: "#8b7ac8", low: "#6b7280",
};

const TYPE_COLOR: Record<string, string> = {
  initial_access: "#ef4444", execution: "#f97316", persistence: "#f59e0b",
  lateral_move: "#8b7ac8", exfil: "#ec4899", c2: "#06b6d4",
  discovery: "#6b7280", defense_evasion: "#84cc16",
};

const TYPE_LABEL: Record<string, string> = {
  initial_access: "Initial Access", execution: "Execution", persistence: "Persistence",
  lateral_move: "Lateral Movement", exfil: "Data Exfiltration", c2: "C2 Beacon",
...
const REPLAY_EVENTS: ReplayEvent[] = [
  ...
  { id: "e04", t: 12, type: "discovery", severity: "medium", actor: "APT29", target: "CORP-NET-10.0.0/16", description: "LDAP queries enumerate domain admin group membership and GPO objects", mitre: "T1087.002", evidence: "Network capture: 847 LDAP queries in 90s" },
  { id: "e05", t: 18, type: "c2", severity: "high", actor: "APT29", target: "185.220.101.45", description: "Cobalt Strike Beacon establishes HTTPS C2 channel over port 443 with traffic blending", mitre: "T1071.001", evidence: "JA3 fingerprint match, beacon timing jitter analysis" },
  { id: "e06", t: 26, type: "lateral_move", severity: "critical", actor: "APT29", target: "PROD-DC-01", description: "Pass-the-ticket with Golden Ticket using harvested krbtgt NTLM hash", mitre: "T1550.003", evidence: "Kerberos ticket anomaly: 10y lifetime, event 4769" },
  { id: "e07", t: 33, type: "lateral_move", severity: "high", actor: "APT29", target: "FILESERVER-01", description: "SMB lateral movement via hijacked domain admin session", mitre: "T1021.002", evidence: "Windows event 4624 type 3 anomaly, source: DC01" },
  { id: "e08", t: 41, type: "defense_evasion", severity: "high", actor: "APT29", target: "FILESERVER-01", description: "Security event log cleared via wevtutil, AV tampered via COM interface", mitre: "T1070.001", evidence: "Event 1102 on FILESERVER, AV service stopped" },
  { id: "e09", t: 48, type: "exfil", severity: "critical", actor: "APT29", target: "185.220.101.45:8443", description: "Staged archive of 47GB exfiltrated over encrypted HTTPS in 2h window", mitre: "T1048.003", evidence: "NetFlow anomaly, DLP: 47.2GB egress to unfamiliar IP" },
];
...
function AegisReplayContent() {
  ...
  useEffect(() => {
    if (playing) {
      intervalRef.current = setInterval(() => {
        setPlayhead(p => {
          if (p >= TOTAL_DURATION) { setPlaying(false); return TOTAL_DURATION; }
          return p + 1;
        });
      }, 300);
    } else {
  ...
  function reset() { setPlaying(false); setPlayhead(0); setSelectedEvent(null); }
  ...
  if (safeMode) {
    return (
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        <div className="rounded-2xl border flex flex-col items-center justify-center gap-4 py-16 px-8 text-center" style={{ borderColor: "rgba(139,122,200,0.2)", background: "rgba(139,122,200,0.04)" }}>
          <div className="p-4 rounded-2xl" style={{ background: "rgba(139,122,200,0.1)", border: "1px solid rgba(139,122,200,0.2)" }}>
    ...
    <div className="max-w-7xl mx-auto space-y-5">
      ...
      <div className="rounded-xl border p-5" style={{ borderColor: "rgba(139,122,200,0.15)", background: "rgba(139,122,200,0.03)" }}>
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="ml-auto text-[11px] font-mono" style={{ color: "#8b7ac8" }}>{formatTime(playhead)}</span>
        </div>
        ...
        <div className="relative mb-2 h-8 flex items-center">
          ...
          <div className="absolute left-0 h-1 rounded-full transition-all" style={{ width: `${(playhead / TOTAL_DURATION) * 100}%`, background: "linear-gradient(to right, #8b7ac8, #ef4444)" }} />
          {REPLAY_EVENTS.map(ev => (
            <EventDot key={ev.id} event={ev} playhead={playhead} onClick={() => setSelectedEvent(ev)} />
          ))}
          <input
            type="range" min={0} max={TOTAL_DURATION} value={playhead}
            onChange={handleScrub}
            className="absolute inset-0 w-full opacity-0 cursor-pointer h-8"
          />
        </div>

        <div className="flex items-center gap-3 justify-between text-[9px] font-mono mb-4" style={{ color: "rgba(255,255,255,0.25)" }}>
          <span>T+00h00m</span>
          <span>T+00h30m</span>
          <span>T+01h00m</span>
        </div>
        ...
        <div className="flex items-center gap-2 justify-center">
          <button onClick={reset} className="p-2 rounded-lg transition-colors hover:bg-white/8" style={{ color: "rgba(255,255,255,0.5)", border: "1px solid rgba(255,255,255,0.08)" }}>
            <SkipBack className="w-3.5 h-3.5" />
          </button>
          <button
            onClick={() => setPlaying(p => !p)}
            className="px-6 py-2 rounded-lg font-bold text-sm flex items-center gap-2 transition-all"
            style={{ background: playing ? "rgba(139,122,200,0.15)" : "rgba(139,122,200,0.2)", color: "#8b7ac8", border: "1px solid rgba(139,122,200,0.3)" }}
          >
            {playing ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
            {playing ? "Pause" : "Play"}
          </button>
          <button onClick={() => setPlayhead(TOTAL_DURATION)} className="p-2 rounded-lg transition-colors hover:bg-white/8" style={{ color: "rgba(255,255,255,0.5)", border: "1px solid rgba(255,255,255,0.08)" }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="divide-y divide-white/[0.04] max-h-80 overflow-y-auto">
            {REPLAY_EVENTS.map((ev, i) => {
              ...
              return (
                ...
                >
                  <div className="text-[9px] font-mono mt-0.5 w-16 shrink-0" style={{ color: "rgba(255,255,255,0.3)" }}>{formatTime(ev.t)}</div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <span className="text-[9px] font-bold px-1.5 py-0.5 rounded" style={{ color, background: `${color}15` }}>{TYPE_LABEL[ev.type]}</span>
                      <span className="text-[9px] font-mono" style={{ color: "rgba(139,122,200,0.6)" }}>{ev.mitre}</span>
                    </div>
                    <div className="text-[10px] line-clamp-1" style={{ color: isVisible ? "rgba(255,255,255,0.7)" : "rgba(255,255,255,0.3)" }}>{ev.description}</div>
                  </div>
                  <span className="text-[8px] font-bold uppercase px-1 py-0.5 rounded shrink-0" style={{ color: SEV_COLOR[ev.severity], background: `${SEV_COLOR[ev.severity]}15` }}>{ev.severity}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="rounded-xl border p-4" style={{ borderColor: "rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.01)" }}>
          ...
          {displayEvent ? (
            <div className="space-y-3">
              <div className="rounded-lg p-3" style={{ background: `${TYPE_COLOR[displayEvent.type]}08`, border: `1px solid ${TYPE_COLOR[displayEvent.type]}20` }}>
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="text-[9px] font-bold px-1.5 py-0.5 rounded" style={{ color: TYPE_COLOR[displayEvent.type], background: `${TYPE_COLOR[displayEvent.type]}15` }}>{TYPE_LABEL[displayEvent.type]}</span>
                  <span className="text-[9px] font-bold px-1.5 py-0.5 rounded" style={{ color: SEV_COLOR[displayEvent.severity], background: `${SEV_COLOR[displayEvent.severity]}15` }}>{displayEvent.severity}</span>
                </div>
                <div className="text-sm font-semibold text-white leading-snug">{displayEvent.description}</div>
              </div>
              {[
                ...
                <div key={r.label} className="flex items-center gap-2 text-[10px]">
                  <r.icon className="w-3 h-3 shrink-0" style={{ color: "rgba(255,255,255,0.25)" }} />
                  <span className="font-medium w-20 shrink-0" style={{ color: "rgba(255,255,255,0.35)" }}>{r.label}</span>
              ...
              <div className="rounded-lg p-3" style={{ background: "rgba(16,185,129,0.05)", border: "1px solid rgba(16,185,129,0.15)" }}>
                <div className="text-[9px] font-bold uppercase tracking-widest mb-1" style={{ color: "#10b981" }}>Evidence</div>
                <div className="text-[10px]" style={{ color: "rgba(255,255,255,0.6)" }}>{displayEvent.evidence}</div>
              </div>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <Play className="w-8 h-8 mb-3" style={{ color: "rgba(255,255,255,0.1)" }} />
              <div className="text-[11px]" style={{ color: "rgba(255,255,255,0.25)" }}>Play the timeline or click an event to inspect</div>
            </div>
          )}
        </div>
      </div>
