...
): { text: string; urgency: 'breached' | 'critical' | 'warning' | 'ok' } {
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          {ticket.clientName && (
            <span className="text-xs text-muted-foreground">{ticket.clientName}</span>
          )}
        </div>
        <div className="flex items-center gap-3">
          {ticket.assigneeName && (
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            SLA-tracked ticket queue with priority escalation and technician assignment
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-3">
          {useApi
            ? filteredApi.map((ticket, i) => (
                <ApiTicketCard key={ticket.id} ticket={ticket} index={i} />
              ))
            : filteredFallback.map((ticket, i) => (
                <FallbackTicketCard key={ticket.id} ticket={ticket} index={i} />
              ))}
