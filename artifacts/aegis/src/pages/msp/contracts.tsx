...
export default function ContractsPage() {
  ...
  const { data, isLoading, refetch } = useStandardQuery<ContractsResponse>({
    ...
    queryFn: () =>
      apiFetch<ContractsResponse>(
        `/msp/contracts${statusFilter !== 'all' ? `?status=${statusFilter}` : ''}`,
      ),
    staleTime: 60_000,
  });
  ...
  const avgCompliance =
    activeContracts.length > 0
      ? Math.round(
          (activeContracts.reduce((s, c) => s + (c.slaActual || 0), 0) / activeContracts.length) *
            10,
        ) / 10
      : 0;
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            MSAs, SLA attainment, renewal timelines, and contract value by tier
          </p>
        </div>
        <button
          onClick={() => refetch()}
