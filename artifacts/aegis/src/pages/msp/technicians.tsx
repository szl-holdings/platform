...
export default function TechniciansPage() {
  ...
  const technicians: (Technician | ApiTechnician)[] =
    data?.technicians && data.technicians.length > 0 ? data.technicians : fallbackTechnicians;

  const available = technicians.filter((t) => t.status === 'available').length;
  const busy = technicians.filter(
    (t) => t.status === 'busy' || t.status === 'on-site' || t.status === 'traveling',
  ).length;
    ...
    <div className="p-6 space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Utilization, active ticket load, certifications, and dispatch availability
        </p>
      </div>
      ...
      <div className="glass-card rounded-xl p-5">
        ...
        <div className="space-y-3">
          {technicians
            .filter((t) => t.status !== 'off' && t.status !== 'off-duty')
            .map((tech) => (
              <div key={tech.id} className="flex items-center gap-4">
                ...
                <div className="flex-1 h-3 rounded-full bg-muted overflow-hidden flex">
                  ...
                  <div
                    ...
                    style={{
                      width: `${Math.max(0, (tech.utilization ?? 0) - (tech.activeTickets ?? 0) * 8 - (tech.resolvedToday ?? 0) * 5)}%`,
                    }}
                    title="Other work"
                  />
                </div>
