...
import {
  ...
  Download, ToggleLeft, ToggleRight, Play, X, ShieldCheck, Info,
  BarChart3, Layers, ArrowRight, Terminal, Flag,
} from "lucide-react";
    ...
    steps: [
      ...
      },
      {
        ...
        iocs: [
          ...
          "Named pipe: \\.\pipe\mojo.5688.8052",
        ],
        observables: [
          {
            id: "obs-004",
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        coverage: "inferred",
        timestamp: "12:35:10",
        iocs: ["ALPHV ransom note template match", "C2 via Tor hidden service (partial)", "Golang binary structure"],
        observables: [
          {
            id: "obs-r-007",
            type: "alert",
    ...
    steps: [
      {
        ...
        observables: [
          {
            ...
            excerpt: "CRITICAL | react-utils-pro@3.2.1 | Obfuscated postinstall script | Malware score: 97/100 | Build: #2041",
            timestamp: "10:14:07",
            confidence: 97,
          },
        ],
        recommendedAction: {
          ...
          riskLevel: "high",
          requiresApproval: true,
          details: "Mark all artifacts from the compromised build window as untrusted. Rotate all secrets stored in CI/CD environment variables including AWS credentials and API tokens.",
        },
      },
      {
        seq: 2,
        ...
        coverage: "evidenced",
        timestamp: "10:14:22",
        iocs: ["node .scripts/util.js", "base64-encoded payload (1.4 KB)", "Process: node PID 19283 spawned by npm"],
        observables: [
          {
            id: "obs-sc-002",
            type: "process",
            ...
            excerpt: "npm install → node .scripts/util.js | base64 decode exec | env dump → /tmp/.env.cache | PID: 19283",
            timestamp: "10:14:22",
            confidence: 91,
          },
        ],
      ...
      },
      {
        ...
        observables: [
          {
            ...
            excerpt: "ANOMALY | 289 TXT queries in 4m | dst: *.d1x.cdn-worker[.]io | entropy: high | direction: outbound",
            timestamp: "10:22:41",
            confidence: 78,
          },
        ],
        recommendedAction: {
          ...
          riskLevel: "medium",
          requiresApproval: false,
          details: "Add DNS sinkhole rule for *.cdn-worker.io and capture last 15 minutes of DNS traffic from BUILD-SRV-02 for forensic analysis.",
        },
      },
      {
        seq: 4,
        ...
        recommendedAction: {
          ...
          riskLevel: "critical",
          requiresApproval: true,
          details: "Rotate all AWS IAM credentials stored in CI/CD environment. Audit AWS CloudTrail for any API calls using the compromised keys in the last 2 hours.",
        },
      },
    ],
  },
    ...
    steps: [
      {
        ...
        iocs: ["szlho1dings.com (registered 3d prior)", "MX: mail.szlho1dings.com", "SPF record configured"],
        observables: [
          {
            id: "obs-bec-001",
            type: "alert",
            ...
            excerpt: "TYPOSQUAT DETECTED | szlho1dings.com | Registered: 2026-04-16 | Similar to: szlholdings.com | Registrar: NameCheap",
      ...
      },
      {
        ...
        coverage: "evidenced",
        timestamp: "08:47:14",
        iocs: ["From: ceo@szlho1dings.com", "Subject: URGENT — Acquisition wire (confidential)", "No attachment — social engineering only"],
        observables: [
          {
            id: "obs-bec-002",
            type: "alert",
            ...
            excerpt: "HIGH | Lookalike sender domain | ceo@szlho1dings[.]com → cfo@szlholdings.com | No malicious payload | Social engineering flagged",
            timestamp: "08:47:14",
            confidence: 95,
          },
        ],
      ...
      },
      {
        ...
        coverage: "evidenced",
        timestamp: "09:12:33",
        iocs: ["Requested amount: $240,000 USD", "Destination account: Cayman Islands intermediary"],
        observables: [
          {
            id: "obs-bec-003",
            type: "log",
            ...
            excerpt: "TRANSFER BLOCKED | Requested by: CFO (forwarded from suspicious email) | Amount: $240,000 | Blocked by: K. Rivera (analyst review) | Reason: Sender domain mismatch",
            timestamp: "09:12:33",
            confidence: 100,
          },
        ],
        recommendedAction: {
          ...
          riskLevel: "medium",
          requiresApproval: false,
          details: "Update DMARC record from 'quarantine' to 'reject' policy to prevent future spoofing. Also enroll executive email accounts in advanced anti-impersonation scanning.",
        },
      },
    ],
  },
    ...
    steps: [
      {
        ...
        observables: [
          {
            ...
            excerpt: "AssumeRole | User: svc-terraform | RoleArn: ProdAdminRole | ResponseStatus: 200 | SourceIP: 10.0.14.22 | UserAgent: aws-cli/2.x",
            timestamp: "22:14:05",
            confidence: 98,
          },
        ],
        recommendedAction: {
          ...
          riskLevel: "high",
          requiresApproval: true,
          details: "Immediately restrict the trust policy on ProdAdminRole to only allow assumption from specific service principals with MFA. Audit all AssumeRole events in the last 30 days.",
        },
      },
      {
        seq: 2,
        ...
        coverage: "evidenced",
        timestamp: "22:14:09",
        iocs: ["DescribeInstances: 847 instances across 3 regions", "ListBuckets: 67 S3 buckets enumerated"],
        observables: [
          {
            id: "obs-iam-002",
            type: "log",
            ...
            excerpt: "DescribeInstances | 3 regions | 847 instances | ListBuckets | 67 buckets | DescribeSecurityGroups | Duration: 4s | Automated pattern",
            timestamp: "22:14:09",
            confidence: 88,
          },
        ],
      ...
      },
      {
        ...
        coverage: "inferred",
        timestamp: "22:14:15",
        iocs: ["s3://szl-prod-configs (contains API keys)", "s3://szl-terraform-state (contains infrastructure map)"],
        observables: [],
        recommendedAction: {
          ...
          riskLevel: "medium",
          requiresApproval: false,
          details: "Enable CloudTrail S3 data event logging for all production buckets. Review access logs for `szl-prod-configs` and `szl-terraform-state` immediately.",
        },
      },
    ],
  },
...
const COVERAGE_CONFIG: Record<MitreStageCoverage, { label: string; color: string; bg: string; border: string; icon: typeof CheckCircle }> = {
  evidenced: { label: "Evidenced", color: "#10b981", bg: "rgba(16,185,129,0.08)", border: "rgba(16,185,129,0.25)", icon: CheckCircle },
  inferred: { label: "Inferred", color: "#f59e0b", bg: "rgba(245,158,11,0.08)", border: "rgba(245,158,11,0.25)", icon: Brain },
  missing: { label: "Missing", color: "#64748b", bg: "rgba(100,116,139,0.05)", border: "rgba(100,116,139,0.15)", icon: Info },
};
...
function DemoLabel() {
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-bold tracking-widest uppercase border"
...
function ConfidenceBadge({ value, size = "sm" }: { value: number; size?: "sm" | "xs" }) {
  const color = value >= 90 ? "#10b981" : value >= 75 ? "#f59e0b" : value >= 50 ? "#f97316" : "#64748b";
  return (
    <span className={cn("font-bold font-mono", size === "xs" ? "text-[9px]" : "text-[11px]")} style={{ color }}>
      {value > 0 ? `${value}%` : "—"}
    </span>
  );
}
...
function ApprovalModal({ action, stepTitle, incidentId, onApprove, onDeny, onClose }: ApprovalModalProps) {
  ...
  const submitMutation = useStandardMutation({
    ...
    onSuccess: ({ approvalId, decision }) => {
      const ref = approvalId ? `APRV-${approvalId}` : `APRV-${action.id.toUpperCase()}-${Date.now().toString().slice(-6)}`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)" }}>
      <div className="w-full max-w-lg rounded-2xl border p-6 space-y-5" style={{ background: "#070f1c", borderColor: "rgba(239,68,68,0.2)" }}>
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-[11px] text-white/40">This action requires human authorization before execution</p>
          </div>
          <button onClick={onClose} disabled={isLoading} className="text-white/30 hover:text-white/60 transition-colors disabled:opacity-40">
            <X className="w-4 h-4" />
          </button>
        </div>

        <div className="rounded-xl p-4 space-y-3 border" style={{ borderColor: `${risk.color}25`, background: `${risk.color}06` }}>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold px-2 py-0.5 rounded border" style={{ color: risk.color, borderColor: `${risk.color}30`, background: `${risk.color}12` }}>
              {risk.label}
            </span>
        ...
        <div className="rounded-lg p-3 border text-[10px] text-white/40 space-y-1" style={{ borderColor: "rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.02)" }}>
          <p className="flex items-center gap-1.5"><Lock className="w-3 h-3" /> Decision will be written to the policy engine audit trail via <span className="font-mono text-white/50">POST /approvals</span></p>
          <p className="flex items-center gap-1.5"><ShieldCheck className="w-3 h-3" /> Policy: <span className="font-mono text-white/60">response-actions.{action.riskLevel}.requires-human</span></p>
          <p className="flex items-center gap-1.5"><Activity className="w-3 h-3" /> Approval record bound to incident <span className="font-mono text-white/60">{incidentId}</span></p>
        </div>
...
export default function AdversaryNarrativeEngine() {
  const [selectedIncidentId, setSelectedIncidentId] = useState<string>(() => {
    ...
    return param && NARRATIVE_INCIDENTS.some(i => i.id === param) ? param : NARRATIVE_INCIDENTS[0].id;
  });
  ...
  const [pendingAction, setPendingAction] = useState<{ action: NonNullable<NarrativeStep["recommendedAction"]>; stepTitle: string } | null>(null);
  ...
  const incident = NARRATIVE_INCIDENTS.find(i => i.id === selectedIncidentId) ?? NARRATIVE_INCIDENTS[0];
  ...
  const handleApprove = (approvalRef: string) => {
    ...
    toast.success(`Action approved — policy record ${approvalRef} written`, { description: pendingAction.action.label });
  };
  ...
  const handleDeny = (approvalRef: string) => {
    ...
    setPendingAction(null);
    toast.error(`Action denied — policy record ${approvalRef} written`, { description: pendingAction.action.label });
  };

  const handleExportExecutiveSummary = () => {
    toast.success("Executive summary exported", { description: "PDF generation queued — available in Reports" });
  };

  // Build MITRE stage map from incident steps
  const mitreStageMap = new Map<string, { coverage: MitreStageCoverage; confidence: number; stepSeq: number }>();
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden" style={{ background: "#050d18" }}>
      ...
      {/* Header */}
      <div className="px-6 py-4 border-b shrink-0 flex items-center justify-between" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: "rgba(139,92,246,0.12)", border: "1px solid rgba(139,92,246,0.25)" }}>
          ...
          <div>
            ...
            <p className="text-[10px] text-white/30">Evidence-backed attack storyline · MITRE ATT&CK mapped · Analyst & Executive modes</p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          {/* Mode Toggle */}
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl border" style={{ borderColor: "rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.02)" }}>
            <button
              onClick={() => setMode("analyst")}
              className={cn("flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all", mode === "analyst" ? "text-violet-300" : "text-white/30 hover:text-white/50")}
              style={mode === "analyst" ? { background: "rgba(139,92,246,0.15)", border: "1px solid rgba(139,92,246,0.25)" } : {}}>
              <Terminal className="w-3 h-3" /> Analyst
            </button>
            <button
              onClick={() => setMode("executive")}
              className={cn("flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all", mode === "executive" ? "text-blue-300" : "text-white/30 hover:text-white/50")}
              style={mode === "executive" ? { background: "rgba(59,130,246,0.15)", border: "1px solid rgba(59,130,246,0.25)" } : {}}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-64 shrink-0 border-r flex flex-col overflow-hidden" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
          <div className="p-3 border-b shrink-0" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
            <p className="text-[9px] uppercase tracking-widest font-bold text-white/25 mb-2">Active Incidents</p>
          </div>
          <div className="flex-1 overflow-y-auto">
            {NARRATIVE_INCIDENTS.map(inc => {
              ...
              return (
                <button
                  key={inc.id}
                  onClick={() => { setSelectedIncidentId(inc.id); setExpandedStep(1); setActiveTab("narrative"); }}
                  ...
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <span className="text-[9px] font-mono" style={{ color: s.color }}>{inc.id}</span>
                    <span className="text-[9px] px-1.5 py-0.5 rounded font-bold border" style={{ color: s.color, borderColor: s.border, background: s.bg }}>
                      {inc.severity.toUpperCase()}
                    </span>
                  </div>
                  <p className="text-[11px] text-white/75 font-medium leading-tight mb-1.5 line-clamp-2">{inc.title}</p>
                  <div className="flex items-center gap-2 text-[9px] text-white/30">
                    <span className="flex items-center gap-0.5"><CheckCircle className="w-2.5 h-2.5 text-emerald-500" /> {inc.stepsEvidenced}</span>
                    <span className="flex items-center gap-0.5"><Brain className="w-2.5 h-2.5 text-amber-500" /> {inc.stepsInferred}</span>
                    <span className="flex items-center gap-0.5"><Info className="w-2.5 h-2.5 text-slate-500" /> {inc.stepsMissing}</span>
                  </div>
                </button>
              );
            })}
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          {/* Incident Header */}
          <div className="p-5 border-b shrink-0 space-y-4" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                      ...
                      <span className="px-2 py-0.5 rounded text-[9px] font-bold border" style={{ color: sev.color, borderColor: sev.border, background: sev.bg }}>
                        {incident.severity.toUpperCase()}
                      </span>
                      <span className="text-[9px] text-white/30">{incident.actor}</span>
                      <span className="text-[9px] text-white/30">Confidence: <ConfidenceBadge value={incident.confidence} size="xs" /></span>
                    </div>
                    <h2 className="text-base font-bold text-white mb-2">{incident.title}</h2>
                  </div>
                </div>
                <div className="rounded-xl p-4 border" style={{ borderColor: "rgba(59,130,246,0.15)", background: "rgba(59,130,246,0.04)" }}>
                  <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-2">Executive Summary</p>
                  <p className="text-[12px] text-white/70 leading-relaxed">{incident.executiveSummary}</p>
                </div>
                <div className="rounded-xl p-3 border" style={{ borderColor: `${sev.color}25`, background: sev.bg }}>
                  <p className="text-[10px] font-bold uppercase tracking-widest mb-1" style={{ color: sev.color }}>Business Impact</p>
                  <p className="text-[12px] text-white/70 leading-relaxed">{incident.businessImpact}</p>
                </div>
              </div>
            ) : (
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-[11px] text-white/40 leading-relaxed">{incident.executiveSummary.slice(0, 180)}…</p>
                </div>
                <div className="grid grid-cols-4 gap-3 shrink-0 text-center">
                  {[
                    ...
                    <div key={label} className="rounded-xl p-2.5 border" style={{ borderColor: "rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.02)" }}>
            ...
            {mode === "analyst" && (
              <div className="flex items-center gap-1">
                ...
                ] as { id: typeof activeTab; label: string; icon: typeof BookOpen }[]).map(tab => (
                  <button
                    key={tab.id}
                    onClick={() => setActiveTab(tab.id)}
                    className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium transition-all border", activeTab === tab.id ? "text-violet-300" : "text-white/35 hover:text-white/55 border-transparent")}
                    style={activeTab === tab.id ? { background: "rgba(139,92,246,0.1)", borderColor: "rgba(139,92,246,0.25)" } : {}}>
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {mode === "executive" && (
              <div className="p-5 space-y-3">
                <p className="text-[10px] text-white/30 uppercase tracking-widest font-bold mb-3">Attack Storyline — Plain Language</p>
                  ...
                  return (
                    <div key={step.seq} className="flex gap-4 items-start">
                      <div className="flex flex-col items-center shrink-0">
                        <div className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: cov.bg, border: `2px solid ${cov.color}` }}>
                          <CovIcon className="w-3.5 h-3.5" style={{ color: cov.color }} />
                        </div>
                        {step.seq < incident.steps.length && (
                          <div className="w-px flex-1 my-1 min-h-[20px]" style={{ background: "rgba(255,255,255,0.06)" }} />
                        )}
                      </div>
                      <div className="flex-1 pb-3">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-[10px] px-1.5 py-0.5 rounded border font-medium" style={{ color: cov.color, borderColor: cov.border, background: cov.bg }}>
                            {cov.label}
                          </span>
                          <span className="text-[10px] text-white/30">{step.mitreStage}</span>
                          {step.confidence > 0 && <span className="text-[9px] text-white/25">· conf <ConfidenceBadge value={step.confidence} size="xs" /></span>}
                        </div>
                        <p className="text-[13px] font-semibold text-white/85 mb-1">{step.title}</p>
                        <p className="text-[11px] text-white/45 leading-relaxed">{step.description}</p>
                        {step.recommendedAction && (
                          <div className="mt-2 flex items-start gap-2 rounded-lg p-2.5 border" style={{ borderColor: `${RISK_CONFIG[step.recommendedAction.riskLevel].color}25`, background: `${RISK_CONFIG[step.recommendedAction.riskLevel].color}06` }}>
                            <Zap className="w-3 h-3 mt-0.5 shrink-0" style={{ color: RISK_CONFIG[step.recommendedAction.riskLevel].color }} />
                            <div className="flex-1">
                              <p className="text-[10px] font-semibold text-white/70">{step.recommendedAction.label}</p>
                            </div>
                            {step.recommendedAction.requiresApproval && (
                              <button
                                onClick={() => {
                                  ...
                                  setPendingAction({ action: step.recommendedAction!, stepTitle: step.title });
                                }}
                                ...
                                style={
                                  approvedActions.has(step.recommendedAction.id)
                                    ? { color: "#10b981", borderColor: "rgba(16,185,129,0.3)", background: "rgba(16,185,129,0.1)" }
                                    : deniedActions.has(step.recommendedAction.id)
                                    ? { color: "#64748b", borderColor: "rgba(100,116,139,0.2)", background: "rgba(100,116,139,0.06)" }
                                    : { color: "#f59e0b", borderColor: "rgba(245,158,11,0.3)", background: "rgba(245,158,11,0.08)" }
                                }>
                                {approvedActions.has(step.recommendedAction.id) ? "✓ Approved" : deniedActions.has(step.recommendedAction.id) ? "✗ Denied" : "Approve →"}
                              </button>
                            )}
                          </div>
                        )}
                  ...
                  return (
                    <div key={step.seq} className="flex gap-4 items-start">
                      ...
                      <div className="flex flex-col items-center shrink-0 pt-4">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center z-10 shrink-0 border-2" style={{ background: cov.bg, borderColor: cov.color }}>
                          <CovIcon className="w-3.5 h-3.5" style={{ color: cov.color }} />
                        </div>
                        {!isLast && <div className="w-px flex-1 my-1 min-h-[24px]" style={{ background: `${cov.color}30` }} />}
                      </div>
                      ...
                      <div className="flex-1 pb-3">
                        <button
                          ...
                          style={{ borderColor: isExpanded ? `${cov.color}30` : "rgba(255,255,255,0.06)", background: isExpanded ? cov.bg : "rgba(255,255,255,0.01)" }}>
                          <div className="flex items-start gap-3">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1 flex-wrap">
                                <span className="text-[10px] px-1.5 py-0.5 rounded border" style={{ color: cov.color, borderColor: cov.border, background: cov.bg }}>
                                  {cov.label}
                                </span>
                                <span className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ color: "#a78bfa", background: "rgba(139,92,246,0.1)" }}>
                                  {step.mitreTechniqueId}
                                </span>
                                <span className="text-[9px] text-white/25">{step.mitreStage}</span>
                                {step.timestamp !== "—" && <span className="text-[9px] font-mono text-white/25">{step.timestamp}</span>}
                              </div>
                              <p className="text-[13px] font-semibold text-white/90 mb-1">{step.title}</p>
                              <p className="text-[11px] text-white/45 leading-relaxed">{step.description}</p>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                              ...
                              {step.observables.length > 0 && (
                                <span className="text-[9px] px-1.5 py-0.5 rounded border" style={{ color: "#60a5fa", borderColor: "rgba(96,165,250,0.2)", background: "rgba(96,165,250,0.06)" }}>
                                  {step.observables.length} obs
                                </span>
                              )}
                              <ChevronDown className={cn("w-3.5 h-3.5 text-white/25 transition-transform", isExpanded && "rotate-180")} />
                            </div>
                          </div>
                        </button>
                        ...
                        {/* Expanded Detail */}
                        {isExpanded && (
                          <div className="mt-2 rounded-xl border p-4 space-y-4" style={{ borderColor: `${cov.color}15`, background: "rgba(255,255,255,0.01)" }}>
                            {/* Technique detail */}
                            <div>
                              <p className="text-[9px] uppercase tracking-widest text-white/25 mb-1.5">Technique</p>
                              <p className="text-[12px] text-white/70">{step.mitreTechnique} <span className="font-mono text-violet-400 text-[10px]">({step.mitreTechniqueId})</span></p>
                            </div>

                            {/* IOCs */}
                            {step.iocs.length > 0 && (
                              <div>
                                <p className="text-[9px] uppercase tracking-widest text-white/25 mb-1.5">Indicators of Compromise</p>
                                <div className="flex flex-col gap-1">
                                  {step.iocs.map((ioc, i) => (
                                    <div key={i} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[10px] font-mono" style={{ background: "rgba(239,68,68,0.05)", border: "1px solid rgba(239,68,68,0.12)" }}>
                            ...
                            {step.observables.length > 0 && (
                              <div>
                                <p className="text-[9px] uppercase tracking-widest text-white/25 mb-1.5">Evidence — Triggering Observables</p>
                                <div className="space-y-2">
                                  {step.observables.map(obs => {
                                    const ot = OBSERVABLE_TYPE_CONFIG[obs.type] ?? OBSERVABLE_TYPE_CONFIG.log;
                                    const OtIcon = ot.icon;
                                    return (
                                      <div key={obs.id} className="rounded-lg border p-3 space-y-2" style={{ borderColor: `${ot.color}18`, background: `${ot.color}05` }}>
                                        <div className="flex items-center justify-between gap-2">
                                          <div className="flex items-center gap-1.5">
                                            <OtIcon className="w-3 h-3 shrink-0" style={{ color: ot.color }} />
                                            <span className="text-[10px] font-semibold" style={{ color: ot.color }}>{obs.type.toUpperCase()}</span>
                                            <span className="text-[10px] text-white/40">{obs.source}</span>
                                          </div>
                                          <div className="flex items-center gap-2">
                                            <span className="text-[9px] font-mono text-white/25">{obs.rawRef}</span>
                                            <span className="text-[9px] font-mono text-white/25">{obs.timestamp}</span>
                                            <ConfidenceBadge value={obs.confidence} size="xs" />
                                          </div>
                                        </div>
                                        <pre className="text-[10px] text-white/55 font-mono leading-relaxed whitespace-pre-wrap break-all p-2 rounded" style={{ background: "rgba(0,0,0,0.3)" }}>
                                          {obs.excerpt}
                                        </pre>
                                      </div>
                                    );
                            ...
                            {step.coverage === "missing" && (
                              <div className="flex items-start gap-2.5 rounded-lg p-3 border" style={{ borderColor: "rgba(100,116,139,0.2)", background: "rgba(100,116,139,0.05)" }}>
                                <Info className="w-4 h-4 text-slate-400 shrink-0 mt-0.5" />
                                <p className="text-[11px] text-slate-400 leading-relaxed">
                                  No observables available for this stage. Active monitoring is in place. The absence of evidence is not evidence of absence — this stage may not have occurred, or detection coverage may be insufficient.
                                </p>
                              </div>
                            )}
                            ...
                            {step.coverage === "inferred" && (
                              <div className="flex items-start gap-2.5 rounded-lg p-3 border" style={{ borderColor: "rgba(245,158,11,0.2)", background: "rgba(245,158,11,0.05)" }}>
                                <Brain className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
                                <p className="text-[11px] text-amber-300/70 leading-relaxed">
                                  This step is <strong>inferred</strong> from behavioral patterns and correlated signals — not yet confirmed by direct observable evidence. Confidence is lower and should be treated as a working hypothesis.
                                </p>
                              </div>
                            )}
                            ...
                            {action && (
                              <div>
                                <p className="text-[9px] uppercase tracking-widest text-white/25 mb-1.5">Recommended Response Action</p>
                                <div className="rounded-lg border p-3 space-y-2.5" style={{ borderColor: `${RISK_CONFIG[action.riskLevel].color}25`, background: `${RISK_CONFIG[action.riskLevel].color}06` }}>
                                  <div className="flex items-start justify-between gap-3">
                                    <div className="flex-1">
                                      <div className="flex items-center gap-2 mb-1">
                                        <span className="text-[9px] px-1.5 py-0.5 rounded border font-bold" style={{ color: RISK_CONFIG[action.riskLevel].color, borderColor: `${RISK_CONFIG[action.riskLevel].color}30`, background: `${RISK_CONFIG[action.riskLevel].color}12` }}>
                                          {RISK_CONFIG[action.riskLevel].label}
                                        </span>
                                        {action.requiresApproval && (
                                          <span className="text-[9px] px-1.5 py-0.5 rounded border" style={{ color: "#a78bfa", borderColor: "rgba(139,92,246,0.25)", background: "rgba(139,92,246,0.08)" }}>
                                            Requires Approval
                                          </span>
                                        )}
                                      </div>
                                      <p className="text-[12px] font-semibold text-white/80 mb-1">{action.label}</p>
                                      <p className="text-[10px] text-white/40 leading-relaxed">{action.details}</p>
                                    </div>
                                    <button
                                      onClick={() => {
                                        if (approvedActions.has(action.id)) { toast.info("Already approved"); return; }
                                        if (deniedActions.has(action.id)) { toast.info("Action denied — re-open to reconsider"); return; }
                                        if (action.requiresApproval) {
                                          setPendingAction({ action, stepTitle: step.title });
                                        } else {
                                          setApprovedActions(prev => new Map(prev).set(action.id, `DIRECT-${action.id.toUpperCase()}`));
                                  ...
                                  {(approvedActions.has(action.id) || deniedActions.has(action.id)) && (
                                    <div className="flex items-center gap-1.5 text-[9px] pt-1 border-t" style={{ borderColor: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.3)" }}>
                                      <Lock className="w-3 h-3" />
                                      Decision written to policy engine · Approval record: <span className="font-mono">{approvedActions.get(action.id) ?? deniedActions.get(action.id)}</span>
                                    </div>
                                  )}
                                </div>
                              </div>
            ...
            {mode === "analyst" && activeTab === "mitre" && (
              <div className="p-5 space-y-4">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <p className="text-xs font-bold text-white/80 mb-0.5">MITRE ATT&CK — Stage Coverage</p>
                    <p className="text-[10px] text-white/30">Which stages are evidenced, inferred, or not yet observed for {incident.id}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    {(["evidenced", "inferred", "missing"] as MitreStageCoverage[]).map(c => {
                      ...
                      return (
                        <div key={c} className="flex items-center gap-1.5 text-[10px]" style={{ color: cfg.color }}>
                  ...
                  {MITRE_STAGE_ORDER.map(stageName => {
                    ...
                    return (
                      <div
                        ...
                        style={{ borderColor: stageData ? cfg.border : "rgba(255,255,255,0.06)", background: stageData ? cfg.bg : "rgba(255,255,255,0.01)", opacity: stageData ? 1 : 0.5 }}>
                        <div className="flex items-start justify-between gap-2 mb-2">
                          <CfgIcon className="w-4 h-4 shrink-0 mt-0.5" style={{ color: cfg.color }} />
                          <span className="text-[9px] font-bold uppercase tracking-widest px-1.5 py-0.5 rounded border shrink-0" style={{ color: cfg.color, borderColor: cfg.border, background: cfg.bg }}>
                            {cfg.label}
                          </span>
                        </div>
                        ...
                        {incidentStep ? (
                          <>
                            <p className="text-[10px] text-white/35 leading-tight mb-2 line-clamp-2">{incidentStep.mitreTechnique}</p>
                            <div className="flex items-center justify-between">
                              <span className="text-[9px] font-mono text-violet-400/70">{incidentStep.mitreTechniqueId}</span>
                            ...
                            {incidentStep.confidence > 0 && (
                              <div className="mt-2 h-1 rounded-full" style={{ background: "rgba(255,255,255,0.06)" }}>
                                <div className="h-full rounded-full transition-all" style={{ width: `${incidentStep.confidence}%`, background: cfg.color, opacity: 0.7 }} />
                              </div>
                            )}
                          </>
                        ) : (
                          <p className="text-[10px] text-white/20 leading-tight">No activity observed in this incident</p>
                        )}
                      </div>
                    );
                  })}
                ...
                {/* Coverage summary */}
                <div className="rounded-xl border p-4 grid grid-cols-3 gap-4 text-center mt-2" style={{ borderColor: "rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.01)" }}>
                  {(["evidenced", "inferred", "missing"] as MitreStageCoverage[]).map(c => {
                    const cfg = COVERAGE_CONFIG[c];
            ...
            {mode === "analyst" && activeTab === "evidence" && (
              <div className="p-5 space-y-4">
                <div>
                  <p className="text-xs font-bold text-white/80 mb-0.5">All Observables — {incident.id}</p>
                  <p className="text-[10px] text-white/30">Complete list of evidence artifacts. Click any record to view the raw excerpt and trace back to the narrative step.</p>
                </div>

                {incident.steps.filter(s => s.observables.length > 0).map(step => (
                  <div key={step.seq} className="space-y-2">
                    <div className="flex items-center gap-2 py-1">
                      <ArrowRight className="w-3 h-3 text-white/25" />
                      <span className="text-[10px] font-bold text-white/50">Step {step.seq}: {step.title}</span>
                      <span className="text-[9px] font-mono text-violet-400/60">{step.mitreTechniqueId}</span>
                    </div>
                    {step.observables.map(obs => {
                      const ot = OBSERVABLE_TYPE_CONFIG[obs.type] ?? OBSERVABLE_TYPE_CONFIG.log;
                      const OtIcon = ot.icon;
                      return (
                        <div key={obs.id} className="rounded-xl border p-4 space-y-2" style={{ borderColor: `${ot.color}15`, background: "rgba(255,255,255,0.01)" }}>
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-center gap-2">
                              <div className="w-6 h-6 rounded-lg flex items-center justify-center shrink-0" style={{ background: `${ot.color}12`, border: `1px solid ${ot.color}25` }}>
                                <OtIcon className="w-3 h-3" style={{ color: ot.color }} />
                              </div>
                              <div>
                                <div className="flex items-center gap-1.5">
                                  <span className="text-[10px] font-bold" style={{ color: ot.color }}>{obs.type.toUpperCase()}</span>
                                  <span className="text-[10px] text-white/40">{obs.source}</span>
                                </div>
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              ...
                              <span className="text-[8px] px-1.5 py-0.5 rounded border" style={{ color: "#60a5fa", borderColor: "rgba(96,165,250,0.2)", background: "rgba(96,165,250,0.06)" }}>
                                {obs.id}
                              </span>
                            </div>
                          </div>
                          <pre className="text-[10px] text-white/55 font-mono leading-relaxed whitespace-pre-wrap break-all p-2.5 rounded" style={{ background: "rgba(0,0,0,0.35)" }}>
                            {obs.excerpt}
                          </pre>
                          <div className="flex items-center gap-1.5 text-[9px] text-white/25">
                            <Link2 className="w-3 h-3" />
                            <span>Linked to narrative step {step.seq}: <span className="text-white/40">{step.title}</span></span>
                          </div>
                        </div>
                      );
                    })}
