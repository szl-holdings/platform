...
function _tsToEpochMs(ts: string): number {
  ...
  return Date.parse(
    `${SESSION_DATE}T${String(hh).padStart(2, '0')}:${String(mm).padStart(2, '0')}:${String(ss).padStart(2, '0')}.${String(millis).padStart(3, '0')}Z`,
  );
}
...
export default function OtIcsDashboard() {
  ...
  const scoresByAsset = useMemo(() => {
    ...
    for (const list of map.values())
      list.sort((a, b) => new Date(a.bucketAt).getTime() - new Date(b.bucketAt).getTime());
    return map;
  }, [scores]);

  const selectedAssetData = useMemo(
    () => assets.find((a) => a.assetId === selectedAsset) ?? null,
    [assets, selectedAsset],
  );
  ...
  const safeReplayIndex = Math.max(
    0,
    Math.min(replayIndex, Math.max(filteredConversation.length - 1, 0)),
  );
  ...
  const handleDownloadCapture = async (format: 'pcap' | 'pcapng') => {
    ...
    try {
      ...
      const startTs =
        framesPayload.length > 0 ? Math.min(...framesPayload.map((f) => f.ts)) : undefined;
      const endTs =
        framesPayload.length > 0 ? Math.max(...framesPayload.map((f) => f.ts)) : undefined;
      ...
      const res = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          filter: {
            protocol:
              replayProtocolFilter === 'all'
                ? 'all'
                : (replayProtocolFilter.toLowerCase() as 'modbus' | 'dnp3' | 's7'),
            startTs,
            endTs,
          },
        }),
  ...
  const activeAnomalies = scores.filter(
    (s) => Number(s.score) >= Number(s.baselineSnapshot ?? 10) * 2,
  ).length;
  const meanScore =
    scores.length > 0
      ? Math.round(scores.reduce((a, s) => a + Number(s.score), 0) / scores.length)
      : 0;
  const meanBaseline =
    assets.length > 0 ? assets.reduce((a, b) => a + Number(b.baseline), 0) / assets.length : 0;
  const driftPct = meanBaseline > 0 ? Math.round((meanScore / meanBaseline - 1) * 100) : 0;
  const protoCounts = frames.reduce<Record<ProtocolName, number>>(
    (acc, f) => {
  ...
  const tickPlayback = () =>
    setReplayIndex((i) => Math.min(Math.max(0, filteredConversation.length - 1), i + 1));
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Protocol-aware monitoring for Modbus, DNP3, and S7. Live decoder, conversation replay,
            and per-asset anomaly scoring.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            <RefreshCw
              className={`w-3.5 h-3.5 ${recomputeMutation.isPending ? 'animate-spin' : ''}`}
            />
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span
                          className={`text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded border ${protocolColor[frame.protocol]}`}
                        >
                          {frame.protocol}
                        </span>
                        <span className="font-mono text-[10px] text-muted-foreground">
                          {formatTime(frame.observedAt)}
                        </span>
                      </div>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-[10px] uppercase tracking-wider opacity-80">
                        {selectedFrame.protocol} · {selectedFrame.functionLabel}
                      </p>
                      <p className="text-sm font-medium mt-1">{selectedFrame.summary}</p>
                    </div>
                    <span className="text-[10px] font-mono opacity-80">
                      {selectedFrame.frameId} · {formatTime(selectedFrame.observedAt)}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-3 text-[11px] mt-3 opacity-90">
                    <div>
                      <span className="opacity-70">Source:</span>{' '}
                      <span className="font-mono">{selectedFrame.src}</span>
                    </div>
                    <div>
                      <span className="opacity-70">Destination:</span>{' '}
                      <span className="font-mono">{selectedFrame.dst}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
                  ...
                  <div className="space-y-1">
                    {(selectedFrame.fields ?? []).map((f, i) => (
                      <div
                        key={i}
                        className="grid grid-cols-12 gap-2 px-2 py-2 rounded hover:bg-white/[0.03] text-[12px]"
                      >
                        <div className="col-span-4">
                          <p className="font-medium">{f.name}</p>
                          {f.note && (
                            <p
                              className={`text-[10px] mt-0.5 ${f.flag ? fieldFlagColor[f.flag] : 'text-muted-foreground'}`}
                            >
                              {f.note}
                            </p>
                          )}
                        ...
                        <div
                          className={`col-span-5 font-mono ${f.flag ? fieldFlagColor[f.flag] : 'text-slate-200'}`}
                        >
                          {f.value}
                        </div>
                        <div className="col-span-3 font-mono text-[10px] text-muted-foreground tracking-wider text-right">
                          {f.bytes}
                        </div>
                      </div>
                    ))}
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
              <div className="flex items-center justify-between gap-4 flex-wrap">
                <div>
                  <p className="text-xs uppercase tracking-wider text-muted-foreground">
                    Session Replay — {ACTIVE_SESSION_ID}
                  </p>
                  <p className="text-sm font-medium">
                    Frame {filteredConversation.length === 0 ? 0 : safeReplayIndex + 1} of{' '}
                    {filteredConversation.length}
                    {replayFrame ? ` · ${formatTime(replayFrame.observedAt)}` : ''}
                  </p>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <button
                    ...
                    disabled={
                      pcapDownloading || pcapngDownloading || filteredConversation.length === 0
                    }
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3 text-[11px]">
                <div>
                  <p className="uppercase tracking-wider text-muted-foreground mb-1.5">
                    Protocol Filter
                  </p>
                  ...
                  >
                    {conversation.map((f, i) => (
                      <option key={f.id} value={i}>
                        {formatTime(f.observedAt)} · #{f.seq} {f.protocol}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div className="mt-4 h-1.5 rounded-full bg-white/5 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${filteredConversation.length === 0 ? 0 : ((safeReplayIndex + 1) / filteredConversation.length) * 100}%`,
                  }}
                />
              </div>
              ...
              {replayFrame && (
                ...
                >
                  <div className="flex items-center gap-2 text-[11px] font-mono">
                    <span
                      className={`px-1.5 py-0.5 rounded border ${protocolColor[replayFrame.protocol]}`}
                    >
                      {replayFrame.protocol}
                    </span>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-1.5 font-mono text-muted-foreground">
                          {formatTime(f.observedAt)}
                        </td>
                        <td className="px-3 py-1.5 font-mono">{f.src}</td>
                        <td className="px-3 py-1.5 font-mono">{f.dst}</td>
                        <td className="px-3 py-1.5">
                          <span
                            className={`px-1.5 py-0.5 rounded border text-[10px] ${protocolColor[f.protocol]}`}
                          >
                            {f.protocol}
                          </span>
                        </td>
                ...
                <thead>
                  <tr className="text-muted-foreground">
                    ...
                    {Array.from({ length: HEATMAP_HOURS }).map((_, h) => (
                      <th
                        key={h}
                        className="px-1 py-1 font-normal text-center"
                      >{`${HEATMAP_HOURS - 1 - h}h`}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    if (series.length > 0) {
                      ...
                      for (const s of series) {
                        const offset = Math.round(
                          (lastBucket - new Date(s.bucketAt).getTime()) / (60 * 60 * 1000),
                        );
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="pr-3 py-1.5">
                          <span
                            className={`px-1.5 py-0.5 rounded border text-[10px] ${protocolColor[a.protocol]}`}
                          >
                            {a.protocol}
                          </span>
                        </td>
                        {cells.map((s, i) => (
                          <td key={i} className="px-0.5 py-1">
                            <div
                              title={
                                s ? `Score ${s.score} (baseline ${baseline})` : 'Score unavailable'
                              }
            ...
            <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4 space-y-3">
              ...
              {selectedAssetData ? (
                <>
                  <div>
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      {selectedAssetData.zone} · {selectedAssetData.protocol}
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-[11px]">
                    <Stat
                      label="Baseline"
                      value={String(Math.round(Number(selectedAssetData.baseline)))}
                    />
                    <Stat
                      label="Current"
                      value={
                        selectedAssetScores.length > 0
                          ? String(
                              Math.round(
                                Number(selectedAssetScores[selectedAssetScores.length - 1].score),
                              ),
                            )
                          : '—'
                      }
                    ...
                    <Stat
                      ...
                      value={
                        selectedAssetScores.length > 0
                          ? String(
                              Math.round(
                                Math.max(...selectedAssetScores.map((s) => Number(s.score))),
                              ),
                            )
                          : '—'
                      }
                  ...
                  <div className="pt-2 border-t border-white/10 space-y-2">
                    ...
                    {selectedAssetScores
                      ...
                      .map((s) => (
                        <DeviationRow
                          key={s.id}
                          label={formatTime(s.bucketAt)}
                          value={s.reason ?? ''}
                        />
                      ))}
                    {selectedAssetScores.filter((s) => s.reason).length === 0 && (
                      <p className="text-[11px] text-muted-foreground">
                        No deviation reasons recorded for this window.
                      </p>
                    )}
                  </div>
                  {selectedAssetData.baselineLastComputedAt && (
                    <p className="text-[10px] text-muted-foreground">
                      Baseline last computed{' '}
                ...
                <p className="text-xs text-muted-foreground">
                  Select an asset to inspect its anomaly profile.
                </p>
              )}
            </div>
          </div>
      ...
      <div className="rounded-xl border border-white/5 bg-white/[0.02] p-3 text-[11px] text-muted-foreground flex items-center gap-2">
        ...
        Decoder profiles: Modbus/TCP (RFC), DNP3 (IEEE 1815), S7Comm (PROFINET). Baselines learned
        over rolling 30-day window via trimmed-mean of hourly anomaly scores.
      </div>
    </div>
  );
}
