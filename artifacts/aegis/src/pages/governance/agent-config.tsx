...
const DEFAULT_AGENTS: AgentDef[] = [
  ...
  },
  {
    ...
    systemPrompt:
      "You are Sentinel, the cybersecurity intelligence agent within the Alloy. You specialize in threat analysis, CVE assessment, incident response, and security posture evaluation. You also serve as the maker-checker validator for other agents' high-stakes recommendations.",
  ...
  },
  {
    ...
    preferredProvider: 'anthropic',
    systemPrompt:
      'You are Helmsman, the maritime intelligence agent within the Alloy. You specialize in fleet operations, AIS tracking, maritime security, route risk assessment, and sanctions compliance.',
  ...
  },
  {
    ...
    preferredProvider: 'openai',
    systemPrompt:
      'You are Zeus, the infrastructure intelligence agent within the Alloy. You specialize in cloud infrastructure, system health, resource optimization, and incident detection.',
    tools: ['system_health', 'admin_overview'],
  ...
  },
  {
    ...
    preferredProvider: 'gemini',
    systemPrompt:
      'You are Muse, the creative intelligence agent within the Alloy. You specialize in content strategy, campaign ideation, creative briefs, and brand voice.',
...
function AgentEditor({
  ...
  const toggleTool = (tool: string) =>
    set(
      'tools',
      agent.tools.includes(tool) ? agent.tools.filter((t) => t !== tool) : [...agent.tools, tool],
    );
  const toggleHighStakes = (domain: string) =>
    set(
      'highStakesDomains',
      agent.highStakesDomains.includes(domain)
        ? agent.highStakesDomains.filter((d) => d !== domain)
        : [...agent.highStakesDomains, domain],
    );
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-[#0d1117] border border-[#1e2a3a] rounded-2xl w-full max-w-2xl my-8 shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-[#1e2a3a]">
          <div className="flex items-center gap-3">
            <div
              ...
              style={{ backgroundColor: `${DOMAIN_COLORS[agent.domain] ?? '#6b7280'}18` }}
            >
          ...
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-[#1e2a3a] transition-colors">
            <X size={16} className="text-[#8b9ab0]" />
          </button>
        </div>
        <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-[#8b9ab0] mb-1.5 block uppercase tracking-wide">
                Provider
              </label>
              <select
                value={agent.preferredProvider}
                onChange={(e) => {
                  set('preferredProvider', e.target.value);
                  set('preferredModel', MODELS[e.target.value]?.[0] ?? '');
                }}
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-[#1e2a3a] border border-[#2e3a4a] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-amber-400"
              >
                {(MODELS[agent.preferredProvider] ?? []).map((m) => (
          ...
          <div>
            <label className="text-xs text-[#8b9ab0] mb-1.5 block uppercase tracking-wide">
              Confidence Threshold: {Math.round((agent.confidenceThreshold ?? 0.7) * 100)}%
            </label>
            <input
              type="range"
              min={0}
            ...
            />
            <div className="flex justify-between text-[10px] text-[#8b9ab0] mt-1">
              <span>0% (always output)</span>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs text-[#8b9ab0] uppercase tracking-wide">
                System Prompt
              </label>
              <button
          ...
          <div>
            <label className="text-xs text-[#8b9ab0] mb-2 block uppercase tracking-wide">
              Tool Permissions
            </label>
          ...
          <div>
            <label className="text-xs text-[#8b9ab0] mb-2 block uppercase tracking-wide">
              High-Stakes Domains (require maker-checker)
            </label>
            <div className="flex flex-wrap gap-2">
              {[
        ...
        <div className="flex gap-3 p-6 pt-0">
          <button
            onClick={onClose}
            className="flex-1 py-2 rounded-lg border border-[#1e2a3a] text-sm text-[#8b9ab0] hover:bg-[#1e2a3a] transition-colors"
          >
            Cancel
          </button>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {agents.map((agent) => {
            ...
            return (
              <div
                key={agent.id}
                className={`bg-[#0d1117] border rounded-xl p-5 transition-colors ${isActive ? 'border-[#1e2a3a]' : 'border-[#1e2a3a] opacity-60'}`}
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#8b9ab0] capitalize">
                        {agent.domain} · {agent.preferredProvider}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="space-y-3 text-xs">
                  <div className="flex items-center justify-between">
                    ...
                    <code className="text-white font-mono text-[10px] bg-[#1e2a3a] px-2 py-0.5 rounded">
                  ...
                  <div>
                    <span className="text-[#8b9ab0] font-mono block mb-1.5">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {agent.tools.slice(0, 4).map((t) => (
                        <span
                          key={t}
                          className="text-[9px] bg-[#1e2a3a] text-[#8b9ab0] px-1.5 py-0.5 rounded font-mono"
                        >
                          {t}
                        </span>
                      ))}
                      {agent.tools.length > 4 && (
                        <span className="text-[9px] text-[#8b9ab0]">
                  ...
                  {agent.highStakesDomains.length > 0 && (
                    <div>
                      <span className="text-[#8b9ab0] font-mono block mb-1.5">
                        High-Stakes ({agent.highStakesDomains.length})
                      </span>
                      <div className="flex flex-wrap gap-1">
                        ...
                        {agent.highStakesDomains.length > 3 && (
                          <span className="text-[9px] text-[#8b9ab0]">
                            +{agent.highStakesDomains.length - 3}
                          </span>
                        )}
