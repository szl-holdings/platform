import {
  Shield, CheckCircle2, Clock, AlertTriangle, Info, Lock,
  Eye, FileText, TrendingUp, ExternalLink
} from "lucide-react";

const ENVIRONMENT_LABELS = [
  { env: "Demo", color: "#f59e0b", bg: "bg-amber-500/10 text-amber-400 border-amber-500/30", description: "Seeded data, scripted scenario, representative capabilities. Not connected to live systems unless explicitly stated.", when: "Used for: sales demos, prospect walkthroughs, conference presentations" },
  { env: "Pilot", color: "#3b82f6", bg: "bg-blue-500/10 text-blue-400 border-blue-500/30", description: "Real customer environment, real data, limited blast radius. Some integrations may be in hook-ready state pending customer config.", when: "Used for: enterprise pilots, proof-of-value engagements, early adopter deployments" },
  { env: "Production", color: "#22c55e", bg: "bg-green-500/10 text-green-400 border-green-500/30", description: "Full deployment. All integrations configured, SSO active, SCIM provisioned, SLAs in effect.", when: "Used for: contracted production deployments with signed agreements and support SLAs" },
];
...
const SECURITY_POSTURE_ITEMS = [
  { control: "Data encryption in transit", status: "implemented", evidence: "TLS 1.3 enforced on all API endpoints" },
  { control: "Data encryption at rest", status: "implemented", evidence: "AES-256 for database and evidence store" },
  { control: "RBAC enforcement", status: "implemented", evidence: "Server-side, not client-side; all routes guarded" },
  { control: "Tenant isolation", status: "implemented", evidence: "All DB queries are tenant-scoped; cross-tenant blocked at query level" },
  { control: "Audit logging", status: "implemented", evidence: "Immutable audit trail for every action; retention enforced" },
  { control: "Secrets management", status: "implemented", evidence: "No secrets in source code; secrets managed via platform environment vault (not hardcoded)" },
  { control: "Dependency audit", status: "pilot", evidence: "Dependency scanning available; automated SAST pipeline not yet configured in CI" },
  { control: "Penetration testing", status: "planned", evidence: "Scheduled for pre-production — not yet completed" },
  { control: "SOC 2 Type II", status: "planned", evidence: "In planning — no audit completed or claims made" },
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-xl font-bold text-white font-mono tracking-tight">Trust & Security Posture</h1>
          </div>
          <p className="text-xs text-[#8b9ab0] font-mono">Honest current-state assessment · No overstated capabilities · Demo / Pilot / Production labeled</p>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-amber-200/80 font-mono leading-relaxed">
                Aegis is in <strong>production-ready pilot</strong> stage. Everything on this page reflects honest current state. We separate current capabilities from roadmap clearly. We publish our AI trust metrics — schema validity rate, retrieval miss rate, unsupported claim rate — because hiding them would undermine the trust we're building.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <h2 className="text-sm font-semibold text-white font-mono uppercase tracking-widest">Environment Labels</h2>
        ...
        <div className="space-y-3">
          <h2 className="text-sm font-semibold text-white font-mono uppercase tracking-widest flex items-center gap-2">
            <CheckCircle2 size={14} className="text-green-400" /> Current Capabilities (Built & Demonstrable)
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {CURRENT_CAPABILITIES.map(c => (
              <div key={c} className="flex items-start gap-2 p-2.5 bg-[#0d1117] border border-green-500/20 rounded-lg">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {FUTURE_ROADMAP.map(r => (
              <div key={r} className="flex items-start gap-2 p-2.5 bg-[#0d1117] border border-amber-500/20 rounded-lg">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {TRUST_PROHIBITIONS.map(p => (
              <div key={p} className="flex items-start gap-2 p-2.5 bg-[#0d1117] border border-red-500/20 rounded-lg">
                  ...
                  <tr key={item.control} className="hover:bg-[#0d1117] transition-colors">
                    ...
                    <td className="py-2.5 pr-4">
                      <span className={`px-2 py-0.5 rounded text-xs font-bold ${
                        ...
                        item.status === "pilot" ? "bg-blue-500/20 text-blue-400" :
                        "bg-amber-500/20 text-amber-400"
                      }`}>
                        {item.status === "implemented" ? "Implemented" : item.status === "pilot" ? "Pilot" : "Planned"}
                      </span>
                    </td>
        ...
        <div className="flex gap-4">
          <a href="/gov/governance" className="flex items-center gap-1.5 text-xs text-amber-400 font-mono hover:text-amber-300 transition-colors">
            <FileText size={12} /> Identity & Access Management <ExternalLink size={10} />
          </a>
          <a href="/gov/governance" className="flex items-center gap-1.5 text-xs text-amber-400 font-mono hover:text-amber-300 transition-colors">
            <FileText size={12} /> Governance Summary <ExternalLink size={10} />
          </a>
          <a href="/gov/trust-analytics" className="flex items-center gap-1.5 text-xs text-amber-400 font-mono hover:text-amber-300 transition-colors">
