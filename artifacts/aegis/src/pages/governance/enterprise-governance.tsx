...
const RBAC_ROLES = [
  { role: 'Super Admin', scope: 'platform', permissions: ['*'], tenantIsolated: false },
  {
    role: 'Tenant Admin',
    scope: 'tenant',
    permissions: ['manage_users', 'manage_integrations', 'view_audit_logs', 'configure_policies'],
    tenantIsolated: true,
  },
  {
    role: 'SOC Analyst',
    scope: 'tenant',
    permissions: ['view_incidents', 'update_incidents', 'create_cases', 'view_findings'],
    tenantIsolated: true,
  },
  {
    role: 'SOC Lead',
    scope: 'tenant',
    permissions: ['approve_actions', 'close_incidents', 'manage_cases', 'view_all_reports'],
    tenantIsolated: true,
  },
  {
    role: 'Executive Viewer',
    scope: 'tenant',
    permissions: ['view_executive_reports', 'view_risk_posture'],
    tenantIsolated: true,
  },
  {
    role: 'Integration Manager',
    scope: 'tenant',
    permissions: ['manage_integrations', 'view_audit_logs'],
    tenantIsolated: true,
  },
  {
    role: 'Read Only',
    scope: 'tenant',
    permissions: ['view_incidents', 'view_findings', 'view_reports'],
    tenantIsolated: true,
  },
];
...
const AUDIT_LOG_ENTRIES = [
  {
    ...
    actor: 'M. Walsh (SOC Lead)',
    target: 'APR-041 — network isolation DC-PROD-03',
    tenant: 'Acme Corp',
    at: '2m ago',
    risk: 'high',
  },
  {
    ...
    action: 'policy_block',
    actor: 'system',
    target: 'cross-tenant query attempt by agent-07',
    tenant: 'Acme Corp',
    at: '18m ago',
    risk: 'critical',
  },
  {
    id: 'AL-8819',
    action: 'case_closed',
    actor: 'L. Kim (SOC Analyst)',
    target: 'CASE-0038 — credential spray',
    tenant: 'Acme Corp',
    at: '1h 42m ago',
    risk: 'low',
  },
  {
    ...
    actor: 'R. Patel (Integration Mgr)',
    target: 'Slack webhook — #soc-alerts',
    tenant: 'Acme Corp',
    at: '3h 10m ago',
    risk: 'low',
  },
  {
    ...
    actor: 'S. Torres (Executive Viewer)',
    target: 'Board Summary Q1 2025',
    tenant: 'Acme Corp',
    at: '1d ago',
    risk: 'low',
  },
...
const RETENTION_POLICIES = [
  {
    type: 'Audit logs',
    retention: '2 years',
    enforcement: 'Hard delete after retention period',
    status: 'active',
  },
  {
    type: 'Incident records',
    retention: '7 years',
    enforcement: 'Archive to cold storage after 1 year',
    status: 'active',
  },
  {
    type: 'Evidence artifacts',
    retention: '3 years',
    enforcement: 'Encrypted archive, tenant-keyed',
    status: 'active',
  },
  {
    type: 'Model call logs',
    retention: '90 days',
    enforcement: 'Rolling delete',
    status: 'active',
  },
  {
    type: 'Session tokens',
...
const POLICY_TEMPLATES = [
  {
    ...
    scope: 'global',
    description:
      'All actions classified as high-risk require explicit human approval before execution.',
  },
  {
    id: 'TPL-002',
    name: 'Cross-Tenant Isolation Block',
    type: 'data_access',
    scope: 'global',
    description:
      'Agent queries are strictly bounded to the originating tenant. Cross-tenant reads or writes are blocked and logged.',
  },
  {
    id: 'TPL-003',
    name: 'Model Cost Ceiling — $500/month',
    type: 'cost_ceiling',
    scope: 'tenant',
    description:
      'Hard stop at $500/month per tenant. Warn at 80%. Requires admin override to exceed.',
  },
  {
    id: 'TPL-004',
    name: 'Observe-Only Demo Mode',
    type: 'agent_permission',
    scope: 'tenant',
    description:
      'All agent actions limited to observe_only. No writes, no notifications, no approvals triggered.',
  },
  {
    id: 'TPL-005',
    name: 'NIST CSF Compliance Controls Baseline',
    type: 'compliance_template',
    scope: 'tenant',
    description:
      'Enforce required controls mapped to NIST CSF Identify, Protect, Detect, Respond, Recover.',
  },
];
...
const INTEGRATIONS_PERMISSION_STATUS = [
  {
    name: 'Slack',
    permission: 'send_notifications, read_channels',
    tenant: 'per-tenant key',
    auditLog: true,
    status: 'connected',
  },
  {
    name: 'Microsoft Teams',
    permission: 'send_adaptive_cards',
    tenant: 'per-tenant OAuth',
    auditLog: true,
    status: 'hook_ready',
  },
  {
    ...
    tenant: 'per-tenant SMTP creds',
    auditLog: true,
    status: 'connected',
  },
  {
    name: 'SIEM / Splunk',
    permission: 'ingest_events (read)',
    tenant: 'per-tenant API key',
    auditLog: true,
    status: 'hook_ready',
  },
  {
    name: 'Ticketing / Jira',
    permission: 'create_ticket, update_ticket',
    tenant: 'per-tenant OAuth',
    auditLog: true,
    status: 'hook_ready',
  },
  {
    name: 'Evidence Store / S3',
    permission: 'put_object, get_object',
    tenant: 'per-tenant bucket',
    auditLog: true,
    status: 'connected',
  },
  {
    name: 'Identity Provider',
    permission: 'read_user, read_group',
    tenant: 'per-tenant SCIM/OIDC',
    auditLog: true,
    status: 'hook_ready',
  },
  {
  ...
  return (
    <div className="bg-[#0d1117] border border-[#1e2a3a] rounded-xl overflow-hidden">
      ...
      >
        ...
        {open ? (
          <ChevronDown size={14} className="text-[#8b9ab0]" />
        ) : (
          <ChevronRight size={14} className="text-[#8b9ab0]" />
        )}
      </button>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-xs text-[#8b9ab0] font-mono">
            SSO/SCIM hooks · RBAC · Tenant isolation · Audit logs · Retention · Export controls ·
            Admin console · Policy templates
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div
              key={m.label}
              className="bg-[#0d1117] border border-[#1e2a3a] rounded-xl p-4 flex flex-col gap-2"
            >
              ...
              <span className="text-lg font-bold font-mono" style={{ color: m.color }}>
                {m.value}
              </span>
            </div>
          ))}
              ...
              <thead>
                <tr className="border-b border-[#1e2a3a]">
                  {['Role', 'Scope', 'Tenant Isolated', 'Key Permissions'].map((h) => (
                    <th
                      key={h}
                      className="text-left text-[#8b9ab0] pb-2 pr-4 font-normal uppercase tracking-widest text-xs"
                    >
                      {h}
                    </th>
                  ))}
                  ...
                  <tr key={r.role} className="hover:bg-[#0a0f16] transition-colors">
                    ...
                    <td className="py-2.5 text-[#8b9ab0] max-w-xs truncate">
                      ...
                      {r.permissions.length > 3 && (
                        <span className="text-[#8b9ab0]"> +{r.permissions.length - 3}</span>
                      )}
                    </td>
                  </tr>
                ))}
          ...
          <div className="space-y-2 mt-2">
            {AUDIT_LOG_ENTRIES.map((e) => (
              ...
              >
                ...
                <span className="text-xs text-[#8b9ab0] font-mono whitespace-nowrap ml-4">
                  {e.at}
                </span>
              </div>
            ))}
