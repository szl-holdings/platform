...
const SEED_TWINS: DigitalTwin[] = [
  ...
  { id: "tw-004", name: "OT-SCADA-CONTROL", domain: "OT/ICS", type: "ot", health: "awaiting_approval", driftScore: 18, lastSync: "11m ago", proofState: "pending", incidents: 1, evidence: ["PLC firmware delta", "Protocol anomaly flagged", "Awaiting CISO sign-off"], worldline: "WL-GAMMA" },
  { id: "tw-005", name: "IDENTITY-FABRIC-AD", domain: "Identity", type: "identity", health: "stable", driftScore: 6, lastSync: "45s ago", proofState: "verified", incidents: 0, evidence: ["Group policy snapshot", "Privileged accounts: 12", "MFA coverage: 98.4%"], worldline: "WL-ALPHA" },
  { id: "tw-006", name: "APP-TIER-K8S", domain: "Application", type: "app", health: "degraded", driftScore: 24, lastSync: "7m ago", proofState: "unverified", incidents: 3, evidence: ["Pod drift: 6 containers", "CVE-2024-3890 exposed", "Security context missing"], worldline: "WL-BETA" },
  { id: "tw-007", name: "DB-CLUSTER-PG", domain: "Database", type: "endpoint", health: "stable", driftScore: 1, lastSync: "18s ago", proofState: "verified", incidents: 0, evidence: ["Schema hash verified", "Replication lag: 0ms", "Encryption: AES-256"], worldline: "WL-ALPHA" },
...
const HEALTH_CONFIG: Record<TwinHealth, { color: string; label: string; bg: string; border: string; dot?: boolean }> = {
  ...
  degraded: { color: "#f59e0b", label: "Degraded", bg: "rgba(245,158,11,0.08)", border: "rgba(245,158,11,0.2)", dot: true },
  awaiting_approval: { color: "#8b7ac8", label: "Awaiting Approval", bg: "rgba(139,122,200,0.08)", border: "rgba(139,122,200,0.2)", dot: true },
  offline: { color: "#ef4444", label: "Offline", bg: "rgba(239,68,68,0.08)", border: "rgba(239,68,68,0.2)", dot: true },
};

const SEV_COLOR: Record<string, string> = { critical: "#ef4444", high: "#f59e0b", medium: "#6b7280" };
const TYPE_ICON: Record<string, typeof Server> = { endpoint: Server, network: Network, cloud: Globe, ot: Zap, identity: Shield, app: Layers, database: Database };
...
function DriftBadge({ score }: { score: number }) {
  ...
  return (
    <span className="text-[8px] font-bold font-mono px-1.5 py-0.5 rounded" style={{ color, background: `${color}15`, border: `1px solid ${color}30` }}>
      Δ {score}%
    </span>
  );
}
  ...
  return (
    <div className="flex gap-1 flex-wrap">
      {twins.map(tw => {
        ...
        return (
          <div key={tw.id} className="flex items-center gap-1 px-2 py-1 rounded-lg" style={{ background: h.bg, border: `1px solid ${h.border}` }}>
            {h.dot && <span className="w-1.5 h-1.5 rounded-full animate-pulse shrink-0" style={{ background: h.color }} />}
            <span className="text-[9px] font-mono" style={{ color: h.color }}>{tw.name}</span>
          </div>
        );
      })}
    </div>
...
function EvidenceOverlay({ twin, onClose }: { twin: DigitalTwin; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-end p-4" onClick={onClose}>
      <div className="w-full max-w-sm rounded-2xl border flex flex-col gap-0 overflow-hidden" style={{ background: "#0c1420", borderColor: "rgba(255,255,255,0.1)" }} onClick={e => e.stopPropagation()}>
        <div className="p-4 border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
          <div className="flex items-center justify-between mb-2">
            <span className="text-[9px] font-bold uppercase tracking-widest font-mono" style={{ color: "rgba(139,122,200,0.8)" }}>ATLAS · Evidence Overlay</span>
            <button onClick={onClose} className="text-[10px] text-slate-500 hover:text-white px-1.5 py-0.5 rounded hover:bg-white/5 transition-colors">✕</button>
          </div>
          <div className="text-sm font-bold text-white">{twin.name}</div>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-[9px] px-1.5 py-0.5 rounded border" style={{ color: h.color, background: h.bg, borderColor: h.border }}>{h.label}</span>
            <span className="text-[9px] px-1.5 py-0.5 rounded" style={{ color: proof.color, background: `${proof.color}15` }}>{proof.label}</span>
            <DriftBadge score={twin.driftScore} />
          </div>
        </div>
        <div className="p-4 space-y-2">
          <div className="text-[9px] font-bold uppercase tracking-widest mb-2" style={{ color: "rgba(255,255,255,0.3)" }}>Evidence Chain</div>
        ...
        <div className="p-4 border-t flex items-center gap-2" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
          <span className="text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.3)" }}>Worldline: {twin.worldline}</span>
          <span className="ml-auto text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.3)" }}>Sync: {twin.lastSync}</span>
        </div>
      </div>
    </div>
  );
    ...
    <div className="max-w-7xl mx-auto space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: "Degraded", value: degraded.length, color: "#f59e0b", pulse: degraded.length > 0 },
          { label: "Awaiting Approval", value: awaiting.length, color: "#8b7ac8", pulse: awaiting.length > 0 },
        ].map(c => (
      ...
      {safeMode && (
        <div className="rounded-xl border p-3 flex items-center gap-3" style={{ borderColor: "rgba(139,122,200,0.25)", background: "rgba(139,122,200,0.06)" }}>
          ...
          <div>
            ...
            <div className="text-[9px]" style={{ color: "rgba(255,255,255,0.4)" }}>Only verified/stable twins shown. {TWINS.length - visibleTwins.length} degraded or unreviewed twin{TWINS.length - visibleTwins.length !== 1 ? "s" : ""} hidden. Incident theater suppressed pending review.</div>
          </div>
        </div>
      )}
      ...
      <div className="rounded-xl border p-4" style={{ borderColor: "rgba(139,122,200,0.12)", background: "rgba(139,122,200,0.02)" }}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest font-mono" style={{ color: "#8b7ac8" }}>Twin Status Rail</span>
          <span className="ml-auto text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.25)" }}>Last sync: {lastRefresh.toLocaleTimeString()}</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="rounded-xl border overflow-hidden" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
          <div className="p-4 border-b flex items-center gap-2" style={{ borderColor: "rgba(255,255,255,0.05)", background: "rgba(255,255,255,0.01)" }}>
            ...
            <span className="ml-auto text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.3)" }}>{visibleTwins.length}{safeMode ? ` / ${TWINS.length} (filtered)` : ""} twins</span>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="p-1.5 rounded-lg shrink-0" style={{ background: `${h.color}15`, border: `1px solid ${h.color}25` }}>
                      <TIcon className="w-3 h-3" style={{ color: h.color }} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <span className="text-[11px] font-semibold text-white truncate">{tw.name}</span>
                        <DriftBadge score={tw.driftScore} />
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-[9px]" style={{ color: "rgba(255,255,255,0.3)" }}>{tw.domain}</span>
                        <span className="text-[9px]" style={{ color: proof.color }}>· {proof.label}</span>
                      </div>
                    </div>
                    <div className="shrink-0 flex items-center gap-2">
                      <span className="text-[9px] px-1.5 py-0.5 rounded border flex items-center gap-1" style={{ color: h.color, background: h.bg, borderColor: h.border }}>
                        {h.dot && <span className="w-1 h-1 rounded-full animate-pulse" style={{ background: h.color }} />}
                        {h.label}
                      </span>
                      {tw.incidents > 0 && (
                        <span className="text-[9px] font-bold px-1.5 py-0.5 rounded" style={{ color: "#ef4444", background: "rgba(239,68,68,0.1)" }}>{tw.incidents}i</span>
                      )}
                      <Eye className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: "rgba(255,255,255,0.35)" }} />
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="divide-y divide-white/[0.04]">
            {safeMode && visibleEvents.length === 0 && (
              <div className="p-6 flex flex-col items-center gap-2 text-center">
                ...
                <div className="text-[10px] font-medium" style={{ color: "rgba(139,122,200,0.6)" }}>Incident Theater Suppressed</div>
                <div className="text-[9px]" style={{ color: "rgba(255,255,255,0.25)" }}>Active incident data is hidden in executive safe mode until reviewed and approved by the SOC team.</div>
              </div>
            )}
              ...
              return (
                <div key={ev.id} className="p-3.5">
                  <div className="flex items-start gap-3">
                    <div className="p-1.5 rounded-lg shrink-0 mt-0.5" style={{ background: `${SEV_COLOR[ev.severity]}15`, border: `1px solid ${SEV_COLOR[ev.severity]}25` }}>
                      <AlertTriangle className="w-3 h-3" style={{ color: SEV_COLOR[ev.severity] }} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                        ...
                        <span className="text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.25)" }}>{ev.timestamp}</span>
                        <span className="text-[9px] font-mono px-1 py-0.5 rounded" style={{ color: "rgba(139,122,200,0.7)", background: "rgba(139,122,200,0.08)" }}>{ev.mitre}</span>
                      </div>
                      <div className="text-[11px] text-white mb-0.5 leading-snug">{ev.description}</div>
                      {twin && (
      ...
      {selectedTwin && <EvidenceOverlay twin={selectedTwin} onClose={() => setSelectedTwin(null)} />}
    </div>
  );
}
