...
const threatFeed = [
  ...
  { type: "High", title: "Anomalous credential dumping via LSASS process injection", source: "Identity Threat · AD", time: "8 min ago", color: "text-orange-400 bg-orange-500/10 border-orange-500/20" },
  { type: "Medium", title: "Suspicious outbound C2 beacon pattern on port 443", source: "NDR · Network", time: "23 min ago", color: "text-amber-400 bg-amber-500/10 border-amber-500/20" },
  { type: "High", title: "Privilege escalation via service account misuse", source: "Identity Threat · Entra ID", time: "41 min ago", color: "text-orange-400 bg-orange-500/10 border-orange-500/20" },
];
...
function ThreatParticles() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 60; i++) {
      pts.push({ x: Math.random() * canvas.width, y: Math.random() * canvas.height, vx: (Math.random() - 0.5) * 0.3, vy: (Math.random() - 0.5) * 0.3, r: Math.random() * 1.5 + 0.4, o: Math.random() * 0.3 + 0.05 });
    }
    const draw = () => {
      ...
      for (let i = 0; i < pts.length; i++) {
        for (let j = i + 1; j < pts.length; j++) {
          ...
          if (d < 120) { ctx.beginPath(); ctx.strokeStyle = `rgba(239,68,68,${0.06 * (1 - d / 120)})`; ctx.lineWidth = 0.5; ctx.moveTo(pts[i].x, pts[i].y); ctx.lineTo(pts[j].x, pts[j].y); ctx.stroke(); }
        }
        ctx.beginPath(); ctx.arc(pts[i].x, pts[i].y, pts[i].r, 0, Math.PI * 2); ctx.fillStyle = `rgba(239,68,68,${pts[i].o})`; ctx.fill();
      }
    ...
    <div className="min-h-screen bg-[#0a0608] text-gray-100 overflow-x-hidden">
      <nav className="fixed top-0 inset-x-0 z-50 border-b border-red-500/10 bg-[#0a0608]/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/demo" className="px-4 py-2 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 text-white text-[13px] font-semibold rounded-lg transition-all">
      ...
      <section className="relative flex items-center justify-center pt-20 pb-12 sm:pt-24 sm:pb-16 lg:pt-28 lg:pb-20 overflow-hidden" style={{ minHeight: "min(85vh, 760px)" }}>
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 text-center">
          <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-red-500/20 bg-red-500/5 mb-8">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75" />
              <span className="relative inline-flex rounded-full h-2 w-2 bg-red-500" />
            </span>
            <span className="text-[11px] font-semibold text-red-400/80 tracking-[0.1em] uppercase">Cyber Defense Platform</span>
          </m.div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10 grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8">
          {stats.map((s, i) => (
            <m.div key={s.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + i * 0.1 }} className="text-center">
              <p className="font-bold text-2xl sm:text-3xl bg-gradient-to-r from-red-400 to-orange-400 bg-clip-text text-transparent mb-1">{s.value}</p>
      ...
      <section id="platform" className="py-16 sm:py-24 border-b border-white/5">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          ...
          <div className="space-y-3">
            {threatFeed.map((t, i) => (
              <m.div key={i} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + i * 0.12 }} className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 bg-[#110e0e]/80 border border-white/5 rounded-xl p-4">
                <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border whitespace-nowrap w-fit ${t.color}`}>{t.type}</span>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
            {capabilities.map((c, i) => (
              <m.div key={c.title} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="group bg-[#110e0e]/60 border border-white/5 hover:border-red-500/20 rounded-xl p-5 sm:p-6 transition-all duration-300">
      ...
      <section id="use-cases" className="py-16 sm:py-24 border-t border-white/5 bg-[#0c0a0a]/40">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12">
            <p className="text-[11px] font-semibold text-red-400/60 tracking-[0.15em] uppercase mb-3">Built For</p>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white">Every role in the SOC</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-5 sm:gap-6">
            {useCases.map((u, i) => (
              <m.div key={u.role} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="border border-white/5 hover:border-red-500/15 rounded-xl p-6 sm:p-7 transition-all duration-300 bg-[#110e0e]/40">
                <p className="text-[10px] font-bold text-red-400/70 uppercase tracking-[0.15em] mb-3">{u.role}</p>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-2 sm:gap-3">
            {["Initial Access", "Execution", "Persistence", "Privilege Escalation", "Defense Evasion", "Credential Access", "Discovery", "Lateral Movement", "Collection", "Exfiltration", "Command & Control", "Impact", "Reconnaissance", "Resource Development"].map((tactic, i) => (
              <m.div key={tactic} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.04 }} className="bg-gradient-to-b from-red-500/8 to-transparent border border-red-500/10 rounded-lg p-3 text-center">
                <p className="text-[10px] font-semibold text-red-400/80 leading-tight">{tactic}</p>
                <p className="text-[18px] font-bold text-white mt-1">{Math.floor(85 + Math.random() * 14)}%</p>
              </m.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 sm:py-24 border-t border-white/5 bg-[#0c0a0a]/40">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-[11px] font-semibold text-red-400/60 tracking-[0.15em] uppercase mb-4">Enterprise Grade</p>
              <h2 className="text-2xl sm:text-3xl font-bold text-white mb-6">Security that secures itself</h2>
              <div className="space-y-4">
                {[
                  { label: "SOC 2 Type II compliant", desc: "Annual audits with continuous monitoring controls." },
                  { label: "Multi-tenant isolation", desc: "Complete data and compute separation between environments." },
                  { label: "Zero-trust architecture", desc: "Every request authenticated, authorized, and encrypted." },
                  { label: "Role-based access control", desc: "Granular permissions across every module and data set." },
                ].map(item => (
            ...
            <div className="bg-[#110e0e] border border-white/5 rounded-2xl p-6 sm:p-8">
              <div className="text-center mb-6">
                <p className="text-[11px] font-semibold text-gray-500 tracking-[0.1em] uppercase mb-2">Platform Telemetry</p>
              </div>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          <m.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="text-gray-500 text-[15px] mb-10 max-w-xl mx-auto leading-relaxed">
              Schedule a private threat briefing with the Aegis team. We'll assess your current posture and show you what you're missing.
            </p>
      ...
      <footer className="border-t border-white/5 py-12 sm:py-16">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-8 sm:gap-10 mb-12">
            ...
            <div>
              <p className="text-[11px] font-semibold text-gray-400 tracking-[0.1em] uppercase mb-4">Company</p>
              <div className="space-y-2.5">
