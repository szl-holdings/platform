...
import {
  AlertTriangle, Shield, Target, BarChart3, FileText,
  RefreshCw, ArrowUpRight, Activity
} from "lucide-react";
import { aegisWhatChanged, type AegisWhatChangedEvent } from "@/data/threat-twin";
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-0.5">
            ...
            <span className="text-xs capitalize" style={{ color: "rgba(255,255,255,0.2)" }}>· {event.entityType} · {event.source}</span>
          </div>
          <div className="text-sm font-medium" style={{
            color: event.severity === "critical" ? "#f87171" : event.severity === "warning" ? "#c08a2c" : "rgba(255,255,255,0.75)"
          }}>
            {event.summary}
          </div>
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="rounded-xl border p-4" style={{ background: "rgba(255,255,255,0.02)", borderColor: "rgba(255,255,255,0.06)" }}>
          <div className="text-xs mb-1" style={{ color: "rgba(255,255,255,0.4)" }}>Events (rolling)</div>
          <div className="text-2xl font-bold" style={{ color: "rgba(255,255,255,0.9)" }}>{aegisWhatChanged.length}</div>
        </div>
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        {["all", "threat", "exposure", "governance"].map(f => (
          <button key={f} onClick={() => setEntityFilter(f)} className="text-xs px-3 py-1 rounded-lg capitalize transition-colors"
            style={{
      ...
      <div className="mt-4 space-y-3">
        {filtered.length === 0 ? (
          <div className="py-16 text-center text-sm" style={{ color: "rgba(255,255,255,0.3)" }}>No events match filter.</div>
        ) : (
