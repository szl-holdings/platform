...
const FALLBACK_PLAYBOOKS = [
  {
    ...
    steps: [
      { id: "S01", name: "Alert triage and severity confirmation", status: "executed", gate: "approved_execute", executedBy: "J. Chen", executedAt: "35m ago", assignedTo: null, requestedAt: null, notes: "Severity confirmed: CRITICAL" },
      { id: "S02", name: "Network isolation proposal — DC-PROD-03", status: "pending", gate: "approval_required", executedBy: null, executedAt: null, assignedTo: "SOC Manager", requestedAt: "28m ago", notes: "Awaiting manager approval APR-001" },
      { id: "S03", name: "Credential rotation — SVC-ACCNT-04", status: "pending", gate: "approval_required", executedBy: null, executedAt: null, assignedTo: "SOC Manager", requestedAt: "28m ago", notes: "Blocked on approval of S02" },
      { id: "S04", name: "Perimeter block — 103.45.18.22", status: "pending", gate: "approval_required", executedBy: null, executedAt: null, assignedTo: "Network Ops", requestedAt: "15m ago", notes: "" },
      { id: "S05", name: "Memory forensics — DC-PROD-03", status: "pending", gate: "propose_only", executedBy: null, executedAt: null, assignedTo: "IR Team", requestedAt: null, notes: "" },
      { id: "S06", name: "Post-containment sweep — adjacent segment", status: "pending", gate: "propose_only", executedBy: null, executedAt: null, assignedTo: "IR Team", requestedAt: null, notes: "" },
      { id: "S07", name: "Executive notification and SLA update", status: "pending", gate: "approved_execute", executedBy: null, executedAt: null, assignedTo: "SOC Manager", requestedAt: null, notes: "" },
    ],
  },
  {
    id: "PB-002",
    ...
    steps: [
      { id: "S01", name: "Source IP block — ASN range", status: "executed", gate: "approved_execute", executedBy: "System", executedAt: "2h ago", assignedTo: null, requestedAt: null, notes: "" },
      { id: "S02", name: "Auth failure audit — affected accounts", status: "executed", gate: "approved_execute", executedBy: "L. Kim", executedAt: "1h 48m ago", assignedTo: null, requestedAt: null, notes: "No successful auth events confirmed" },
      { id: "S03", name: "Alert closed — automated threat", status: "executed", gate: "approved_execute", executedBy: "L. Kim", executedAt: "1h 42m ago", assignedTo: null, requestedAt: null, notes: "" },
    ],
  },
];

const ROLLBACK_ACTIONS = [
  { id: "RB-01", name: "Re-enable DC-PROD-03 network connectivity", associatedStep: "S02", gate: "approval_required", status: "available" },
  { id: "RB-02", name: "Restore SVC-ACCNT-04 prior credential state", associatedStep: "S03", gate: "approval_required", status: "available" },
];

const CONTAINMENT_ACTIONS = [
  { id: "CT-01", name: "Emergency segment isolation — PROD-EAST", actionType: "network_isolation", targetAsset: "PROD-EAST", gate: "approval_required", severity: "critical", status: "available" },
  { id: "CT-02", name: "Force session termination — all active sessions", actionType: "session_termination", targetAsset: "all-active-sessions", gate: "approval_required", severity: "high", status: "available" },
  { id: "CT-03", name: "Kill switch — svc-integration OAuth tokens", actionType: "credential_revocation", targetAsset: "svc-integration-oauth", gate: "approval_required", severity: "high", status: "available" },
];

const GATE_STYLES: Record<string, { bg: string; text: string; border: string; label: string }> = {
  approval_required: { bg: "bg-amber-500/10", text: "text-amber-300", border: "border-amber-500/20", label: "Approval Required" },
  approved_execute:  { bg: "bg-emerald-500/10", text: "text-emerald-300", border: "border-emerald-500/20", label: "Approved Execute" },
  propose_only:      { bg: "bg-blue-500/10", text: "text-blue-300", border: "border-blue-500/20", label: "Propose Only" },
  blocked_by_policy: { bg: "bg-red-500/10", text: "text-red-300", border: "border-red-500/20", label: "Blocked by Policy" },
};
...
function StepRow({ step }: { step: StepShape }) {
  ...
  return (
    <div className={cn("flex items-start gap-3 px-4 py-3 rounded-lg border border-white/[0.04]", s.bg)}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-1">
          <span className={cn("text-[8px] font-mono px-1.5 py-0.5 rounded border uppercase tracking-wide", gate.bg, gate.text, gate.border)}>{gate.label}</span>
          {step.assignedTo && <span className="text-[10px] text-white/35">→ {step.assignedTo}</span>}
          {step.executedAt && <span className="text-[10px] text-white/30">executed {step.executedAt}</span>}
          {step.requestedAt && <span className="text-[10px] text-white/30">requested {step.requestedAt}</span>}
        </div>
        {step.notes && <p className="text-[10px] text-white/45 mt-1 leading-relaxed">{step.notes}</p>}
      </div>
      {step.status === "pending" && step.gate === "approval_required" && (
        <div className="flex gap-1.5 shrink-0">
          <button className="px-2 py-1 rounded text-[9px] font-semibold bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/20">Approve</button>
          <button className="px-2 py-1 rounded text-[9px] font-semibold bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20">Block</button>
        </div>
      )}
...
function deriveGate(action: LiveWorkflowAction): string {
  if (action.actionType === "escalate" || action.actionType === "remediate") return "approval_required";
...
function groupLiveActions(actions: LiveWorkflowAction[]): PlaybookGroup[] {
  ...
  return Array.from(byEntity.entries()).map(([key, group], idx) => {
    ...
    const pbStatus = anyFailed ? "failed" : allCompleted ? "completed" : anyInProgress ? "in_progress" : "pending";
...
export default function ResponseOrchestration() {
  ...
  const containMutation = useStandardMutation({
    mutationFn: async (payload: { containmentType: string; assetId: string; justification: string }) => {
      ...
      return api.command.contain(payload.containmentType, payload.assetId, payload.justification, token);
    },
  });
  ...
  const executed = pb?.steps.filter(s => s.status === "executed" || s.status === "completed").length ?? 0;
  const pending = pb?.steps.filter(s => s.status === "pending").length ?? 0;
  const blocked = pb?.steps.filter(s => s.status === "blocked" || s.status === "failed").length ?? 0;

  return (
    <div className="flex h-full min-h-screen" style={{ backgroundColor: "#070A10", color: "#e2e8f0" }}>
      <div className="w-72 shrink-0 border-r border-white/5 flex flex-col">
        <div className="px-4 py-3 border-b border-white/5">
          ...
          <p className="text-[10px] mt-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>
            {sensitivityLabel && <><Lock className="w-2.5 h-2.5 inline mr-1" />{sensitivityLabel} · </>}
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-white/[0.04]">
          {displayPlaybooks.map(pb2 => (
            <button
              ...
              className={cn("w-full text-left px-4 py-3 transition-all border-l-2", selectedId === pb2.id ? "bg-blue-500/10 border-blue-400" : "hover:bg-white/[0.02] border-transparent")}
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-[9px] font-mono text-white/40">{pb2.id}</span>
                <span className={cn("text-[8px] font-mono px-1 py-0.5 rounded border uppercase", PB_STATUS_STYLES[pb2.status] ?? "border-white/10 text-white/40")}>{pb2.status.replace("_", " ")}</span>
              </div>
              <p className="text-[11px] font-medium text-white/85 mb-0.5">{pb2.name}</p>
              <p className="text-[10px] text-white/35">{pb2.caseRef} · {pb2.startedAt}</p>
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {pb && (
          <>
            <div className="px-6 py-4 border-b border-white/5 sticky top-0 z-10" style={{ backgroundColor: "#070A10" }}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className={cn("text-[8px] font-mono px-1.5 py-0.5 rounded border uppercase", PB_STATUS_STYLES[pb.status] ?? "")}>{pb.status.replace("_", " ")}</span>
                    {pb.gate && (
                      <span className={cn("text-[8px] font-mono px-1.5 py-0.5 rounded border uppercase tracking-wide", GATE_STYLES[pb.gate]?.bg, GATE_STYLES[pb.gate]?.text, GATE_STYLES[pb.gate]?.border)}>
              ...
              <div className="mt-3">
                <div className="flex justify-between text-[10px] mb-1" style={{ color: "rgba(255,255,255,0.35)" }}>
                  <span>{executed}/{pb.steps.length} steps</span>
                ...
                <div className="h-1.5 rounded-full bg-white/5 overflow-hidden">
                  <div className="h-full rounded-full bg-blue-500 transition-all" style={{ width: `${pb.steps.length > 0 ? (executed / pb.steps.length) * 100 : 0}%` }} />
                </div>
              </div>
            </div>
              ...
              {(["playbook", "rollback", "containment"] as const).map(tab => (
                <button
                  ...
                  className={cn(
                    ...
                    activeTab === tab ? "bg-blue-500/15 text-blue-300 border border-blue-500/20" : "text-white/50 hover:text-white/80 hover:bg-white/5"
                  )}
                >
                  {tab === "rollback" ? "Rollback Actions" : tab === "containment" ? "Containment" : "Playbook Steps"}
                </button>
              ))}
            </div>
                  ...
                  {pb.steps.map((step, i) => (
                    <div key={step.id} className="flex gap-3 items-stretch">
                      <div className="flex flex-col items-center">
                        <div className="w-6 h-6 rounded-full bg-white/5 flex items-center justify-center text-[9px] font-mono text-white/30 shrink-0">{i + 1}</div>
                ...
                <div className="space-y-3">
                  <div className="bg-amber-500/[0.06] border border-amber-500/15 rounded-xl p-4 flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-amber-200/60">Rollback actions reverse executed playbook steps. All rollbacks require approval_required gate minimum. Actions are irreversible once confirmed.</p>
                    </div>
                  </div>
                  {ROLLBACK_ACTIONS.map(rb => (
                    <div key={rb.id} className="bg-white/[0.025] border border-white/5 rounded-xl px-4 py-3 flex items-center justify-between gap-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-[9px] font-mono px-1.5 py-0.5 rounded border border-amber-500/20 bg-amber-500/10 text-amber-400">approval_required</span>
                          <span className="text-[10px] text-white/35">reverses step {rb.associatedStep}</span>
                        </div>
                      </div>
              ...
              {activeTab === "containment" && (
                <div className="space-y-3">
                  <div className="bg-red-500/[0.06] border border-red-500/15 rounded-xl p-4 flex gap-3">
                    ...
                    <div>
                      <p className="text-xs font-semibold text-red-300 mb-1">Emergency Containment</p>
                      <p className="text-[11px] text-red-200/60">Containment actions have broad impact. All require explicit approval. Automated blocking is gated at approval_required minimum — no containment action executes without named authorization.</p>
                    </div>
                  </div>
                  {CONTAINMENT_ACTIONS.map(ct => (
                    <div key={ct.id} className="bg-white/[0.025] border border-white/5 rounded-xl px-4 py-3 flex items-center justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                        ...
                        <div className="flex items-center gap-2">
                          <span className={cn("text-[8px] font-mono px-1.5 py-0.5 rounded border uppercase", GATE_STYLES[ct.gate]?.bg, GATE_STYLES[ct.gate]?.text, GATE_STYLES[ct.gate]?.border)}>
                            {ct.gate}
                          </span>
                          <span className={cn("text-[8px] font-mono px-1.5 py-0.5 rounded border uppercase",
                            ct.severity === "critical" ? "bg-red-500/10 text-red-400 border-red-500/20" : "bg-orange-500/10 text-orange-400 border-orange-500/20"
                          )}>{ct.severity}</span>
                        </div>
                      </div>
                      <button
                        onClick={() => containMutation.mutate({ containmentType: ct.actionType, assetId: ct.targetAsset, justification: ct.name })}
