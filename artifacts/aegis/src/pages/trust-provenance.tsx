  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: "Proof Records", value: proofResult.loading ? "…" : proofs.length, icon: FileSearch, color: "text-orange-400", bg: "bg-orange-500/10" },
          { label: "Policies Active", value: policyResult.loading ? "…" : policyDecisions.length, icon: Shield, color: "text-purple-400", bg: "bg-purple-500/10" },
          { label: "Pending Reviews", value: proofResult.loading ? "…" : proofs.filter(p => p.reviewState === "unreviewed").length, icon: Clock, color: "text-amber-400", bg: "bg-amber-500/10" },
          { label: "Audit Events", value: auditResult.loading ? "…" : auditEntries.length, icon: CheckCircle, color: "text-green-400", bg: "bg-green-500/10" },
          ...
          {proofResult.loading ? (
            ...
            <>
              <div className="text-xs text-orange-400/50 px-1">
                {proofs.length} proof records · Showing AI-generated content provenance, review states, and export safety indicators
              </div>
              {proofs.map(proof => (
