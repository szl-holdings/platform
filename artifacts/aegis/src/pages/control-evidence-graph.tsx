...
function FreshnessBar({ score }: { score: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[10px] font-mono w-7 shrink-0" style={{ color }}>{score}</span>
    </div>
  );
}
...
export default function ControlEvidenceGraph() {
  ...
  const allControls: Array<{
    controlId: number; name: string; framework: string; category: string;
    status: "effective" | "gap"; freshnessScore: number; lastVerified: string;
    nextReviewDue: string;
    evidenceItems: Array<{ id: string; type: string; description: string; collectedAt: string; collectedBy: string; freshnessStatus: string; verifiedBy: string; traceRef: string }>;
    provenance: { source: string; traceId: string };
  }> = result.controls ?? [];
  const summary = result.summary ?? {};
  const provenance = result.provenance ?? {};

  const controls = filterStatus === "all" ? allControls : allControls.filter(c => c.status === filterStatus);
    ...
    <div className="p-6 space-y-6" style={{ maxWidth: 1280, margin: "0 auto" }}>
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: DS.text.secondary }}>
            Each control linked to evidence proving effectiveness, with freshness scoring and verification chains.
          </p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          { label: "Avg Freshness", value: summary.avgFreshnessScore != null ? `${summary.avgFreshnessScore}%` : "—", color: "text-yellow-400" },
        ].map(({ label, value, color }) => (
          <div key={label} className="rounded-xl p-4" style={{ background: DS.surface, border: `1px solid ${DS.border}` }}>
            <p className="text-[11px] mb-1" style={{ color: DS.text.muted }}>{label}</p>
            {isLoading ? <div className="h-7 w-14 rounded animate-pulse" style={{ background: DS.border }}/> : <p className={cn("text-2xl font-bold", color)}>{value ?? "—"}</p>}
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center gap-2">
        {(["all", "effective", "gap"] as const).map(f => (
          <button key={f} onClick={() => setFilterStatus(f)} className="px-3 py-1.5 rounded-lg text-xs font-medium transition-colors capitalize" style={filterStatus === f ? { background: "rgba(16,185,129,0.1)", color: "#10b981", border: "1px solid rgba(16,185,129,0.25)" } : { background: DS.surface, color: DS.text.muted, border: `1px solid ${DS.border}` }}>
            {f} {f !== "all" && <span className="ml-1 opacity-60">{f === "effective" ? summary.effectiveControls ?? "" : summary.gapControls ?? ""}</span>}
          </button>
        ))}
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {controls.map((ctrl) => {
            ...
            const staleCount = ctrl.evidenceItems.filter(e => e.freshnessStatus === "stale-90d").length;
            return (
              <div key={ctrl.controlId} className="rounded-xl overflow-hidden transition-all" style={{ background: DS.surface, border: `1px solid ${ctrl.status === "gap" ? "rgba(239,68,68,0.2)" : DS.border}` }}>
                <button className="w-full flex items-center gap-3 p-4 text-left hover:bg-white/[0.02] transition-colors" onClick={() => setExpandedControl(isExpanded ? null : ctrl.controlId)}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <p className="text-sm font-medium truncate" style={{ color: DS.text.primary }}>{ctrl.name}</p>
                      <Badge className={cn("text-[9px] shrink-0", ctrl.status === "effective" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" : "bg-red-500/10 text-red-400 border-red-500/20")}>
                        {ctrl.status}
                      </Badge>
                      {staleCount > 0 && <Badge className="text-[9px] shrink-0 bg-red-500/10 text-red-400 border-red-500/20">{staleCount} stale</Badge>}
                    </div>
                  ...
                  <div className="w-32 shrink-0">
                    <p className="text-[9px] mb-1" style={{ color: DS.text.muted }}>Freshness</p>
                    <FreshnessBar score={ctrl.freshnessScore}/>
                  </div>
                  <div className="text-[10px] text-right shrink-0 mr-2" style={{ color: DS.text.muted }}>
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 space-y-3" style={{ borderTop: `1px solid ${DS.border}` }}>
                    <h4 className="text-[10px] font-semibold uppercase tracking-widest pt-3" style={{ color: DS.text.muted }}>Evidence Items</h4>
                    <div className="grid grid-cols-2 gap-2">
                      {ctrl.evidenceItems.map(ev => {
                        ...
                        return (
                          <div key={ev.id} className="p-3 rounded-lg space-y-1.5" style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${DS.border}` }}>
                            <div className="flex items-center gap-2">
                              <FileText className="w-3 h-3 shrink-0" style={{ color: typeColor }}/>
                              <span className="text-[10px] font-medium" style={{ color: typeColor }}>{ev.type.replace(/-/g, " ")}</span>
                              <span className="ml-auto text-[9px] px-1.5 py-0.5 rounded" style={{ background: fc.bg, color: fc.color }}>{fc.label}</span>
                            </div>
                            ...
                            <div className="flex items-center justify-between text-[9px]" style={{ color: DS.text.muted }}>
                              <span>{ev.collectedBy === "automated" ? "⚡ automated" : "👤 analyst"}</span>
                    ...
                    <div className="flex items-center gap-2 text-[10px] pt-1" style={{ color: DS.text.muted }}>
