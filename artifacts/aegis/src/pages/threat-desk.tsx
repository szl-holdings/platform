  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-2">
        ...
        <span
          className="text-xs px-1.5 py-0.5 rounded-full capitalize ml-2"
      ...
      <div className="flex items-center gap-3 text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>
        <span>
          {threat.affectedAssetCount} asset{threat.affectedAssetCount !== 1 ? 's' : ''}
        </span>
        <span>·</span>
...
export default function ThreatDesk() {
  ...
  const affectedAssets = threat
    ? assetTwins.filter((a) => threat.affectedAssets.includes(a.id))
    : [];
    ...
    <div className="flex h-full" style={{ background: 'hsl(220 10% 7%)' }}>
      ...
      {threat ? (
            ...
            <div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-xs" style={{ color: 'rgba(255,255,255,0.35)' }}>
                  Detected {relTime(threat.detectedAt)} · Last activity{' '}
                  {relTime(threat.lastActivityAt)}
                </span>
              </div>
            </div>
            ...
            {[
              {
                ...
                color: threat.confidence >= 80 ? '#c04a2a' : '#c08a2c',
              },
              { label: 'Affected Assets', value: `${threat.affectedAssetCount}` },
              {
                label: 'Kill Chain',
                value: threat.killChainStage.replace('_', ' '),
                color: '#c04a2a',
              },
              ...
              >
                ...
                <div
                  ...
                  style={{ color: m.color ?? 'rgba(255,255,255,0.85)' }}
                >
                  {m.value}
                </div>
              </div>
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="col-span-2 space-y-5">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {threat.mitreTechniques.map((t) => (
                          <span
                            ...
                            className="text-xs px-2 py-0.5 rounded-full"
                            style={{
                              background: 'hsl(220 72% 56% / 0.08)',
                              color: ACCENT,
                              border: 'hsl(220 72% 56% / 0.2) 1px solid',
                            }}
                          >
                            {t}
                          </span>
              ...
              {pendingApprovals.length > 0 && (
                <div
                  className="rounded-xl border p-5"
                  style={{ background: '#c08a2c05', borderColor: '#c08a2c20' }}
                >
                  <h3
                    className="text-sm font-semibold mb-3"
                    style={{ color: 'rgba(255,255,255,0.75)' }}
        ...
        <div className="flex-1 flex items-center justify-center">
          <EmptyState
            icon={Shield}
            headline="Select a threat"
            description="Choose a threat to view its twin."
            accentColor={ACCENT}
          />
        </div>
      )}
