...
import { FileText, Download, Shield, AlertTriangle, Target, Eye, Crosshair, Clock, Users, Building2, Cpu, CheckCircle, Lock, Globe } from "lucide-react";
...
const REPORT_CONFIGS: Record<ReportType, { label: string; code: string; color: string; icon: typeof FileText; classification: string }> = {
  "phantom-campaign": { label: "PHANTOM Campaign Assessment", code: "PHT-2025-0412", color: PHANTOM_ACCENT, icon: Crosshair, classification: "CONFIDENTIAL // FOUO" },
  "sentinel-findings": { label: "SENTINEL Insider Threat Report", code: "SNT-2025-0412", color: SENTINEL_ACCENT, icon: Eye, classification: "CONFIDENTIAL // PERSONNEL" },
  "purple-team": { label: "Purple Team Exercise Report", code: "PTE-2025-0411", color: "#3b82f6", icon: Shield, classification: "CONFIDENTIAL // SEC CLEARANCE" },
  "executive-summary": { label: "Executive Intelligence Summary", code: "EIS-2025-Q2", color: "#f59e0b", icon: Building2, classification: "CONFIDENTIAL // BOARD ONLY" },
};
...
const REPORT_CONTENT: Record<ReportType, { title: string; subtitle: string; summary: string; sections: ReportSection[]; findings: { id: string; finding: string; severity: "critical" | "high" | "medium"; recommendation: string }[]; stats: { label: string; value: string; color: string }[] }> = {
  "phantom-campaign": {
    title: "PHANTOM Adversary Simulation — Campaign Assessment",
    subtitle: "Operation Shadow Lattice: APT29 Nation-State Simulation",
    summary: "PHANTOM adversary simulation engine conducted a full-spectrum APT29-profile attack campaign against the production environment of [TARGET ORGANIZATION]. This classified assessment documents all simulated attack stages, MITRE ATT&CK mappings, detection coverage gaps identified, and recommended hardening actions to close material security control deficiencies.",
    stats: [
    ...
    sections: [
      {
        id: "s1", title: "Executive Assessment", classification: "CONFIDENTIAL",
        content: [
      ...
      {
        id: "s2", title: "Kill Chain Analysis", classification: "CONFIDENTIAL // FOUO", severity: "critical",
        content: [
      ...
      {
        id: "s3", title: "MITRE ATT&CK Coverage Assessment", classification: "CONFIDENTIAL",
        content: [
    ...
    findings: [
      { id: "f1", finding: "Cloud exfiltration undetected — DLP in audit-only mode, CASB gap on OneDrive volume anomalies", severity: "critical", recommendation: "Enable DLP blocking policy for >500MB uploads to non-approved cloud storage. Deploy CASB for Microsoft 365 with anomaly detection." },
      { id: "f2", finding: "Pass-the-Hash lateral movement uncontrolled — SMB signing not enforced enterprise-wide", severity: "critical", recommendation: "Enforce SMB signing via GPO RequireSecuritySignature=1. Implement LAPS to prevent credential reuse." },
      { id: "f3", finding: "Email gateway allows PDF with JavaScript — partial spearphishing protection", severity: "high", recommendation: "Enable Microsoft Defender Safe Attachments in block mode for all file types with active content capability." },
      { id: "f4", finding: "LSASS detection has 11-minute analyst review delay — no automated containment", severity: "high", recommendation: "Configure CrowdStrike to automatically isolate host on LSASS process access alerts with confidence >90." },
    ],
  },
  "sentinel-findings": {
    title: "SENTINEL Insider Threat Intelligence Report",
    subtitle: "Behavioral Analytics: High-Risk Identities Q2 FY2025",
    summary: "This SENTINEL behavioral analytics assessment documents anomalous identity activity detected across monitored user populations during the Q2 FY2025 assessment period. Two users have been classified as active insider threat investigations based on behavioral deviation patterns, data access anomalies, and exfiltration indicators.",
    stats: [
    ...
    sections: [
      {
        id: "s1", title: "High-Priority Investigation: M. Rodriguez (VP Finance)", classification: "CONFIDENTIAL // PERSONNEL", severity: "critical",
        content: [
      ...
      {
        id: "s2", title: "Elevated Risk: K. Tanaka (Senior DevOps Engineer)", classification: "CONFIDENTIAL", severity: "high",
        content: [
    ...
    findings: [
      { id: "f1", finding: "M. Rodriguez — 12.4GB data exfiltration via personal cloud storage (confirmed DLP event)", severity: "critical", recommendation: "Immediately escalate to Legal and HR for covert forensic investigation. Revoke access to M&A and Strategy SharePoint libraries pending review. Preserve forensic evidence chain." },
      { id: "f2", finding: "K. Tanaka — Production database root access outside change window from anomalous geographic origin", severity: "high", recommendation: "Freeze K. Tanaka production access pending review. Force password reset. Audit all PROD-DB-03 queries from Saturday session. Escalate to IT security and CISO." },
      { id: "f3", finding: "DLP policy in alert-only mode — exfiltration not blocked at point of transfer", severity: "high", recommendation: "Switch DLP policy from audit to block for cloud sync >500MB to personal OneDrive/Google Drive destinations." },
    ],
  },
  "purple-team": {
    title: "Purple Team Exercise — After Action Report",
    subtitle: "APT29 Campaign Simulation: Red vs. Blue Team Assessment",
    summary: "This Purple Team Exercise After Action Report documents the results of a coordinated red team / blue team exercise conducted against production-representative infrastructure. The exercise simulated a full APT29-profile intrusion campaign across 7 attack stages, with simultaneous blue team response and real-time gap analysis.",
    stats: [
    ...
    sections: [
      {
        id: "s1", title: "Overall Assessment", classification: "CONFIDENTIAL // SEC CLEARANCE",
        content: [
      ...
      {
        id: "s2", title: "Red Team Achievements", classification: "CONFIDENTIAL", severity: "critical",
        content: [
    ...
    findings: [
      { id: "f1", finding: "Credential Guard not deployed — enables LSASS credential extraction on all endpoints", severity: "critical", recommendation: "Deploy Credential Guard via Group Policy to all Windows 10/11 and Server 2016+ systems. Estimated deployment: 2 weeks." },
      { id: "f2", finding: "SMB signing not enforced — enables Pass-the-Hash lateral movement across domain", severity: "critical", recommendation: "Enable SMB signing via GPO RequireSecuritySignature. Test compatibility with legacy systems. Deploy within 30 days." },
      { id: "f3", finding: "DLP in alert-only mode — cloud exfiltration not blocked", severity: "high", recommendation: "Switch DLP from audit to active block for cloud storage destinations >500MB." },
      { id: "f4", finding: "Email gateway delivered 1 of 3 spearphishing payloads — partial protection only", severity: "high", recommendation: "Enable Safe Attachments in block mode for all file types with macro/script capability." },
      { id: "f5", finding: "Sysmon registry alerts not threshold-tuned — persistence mechanism undetected", severity: "medium", recommendation: "Review Sysmon EventID 13 (registry modification) alert thresholds. Add detection rule for HKCU Run key modifications." },
    ],
  },
  "executive-summary": {
    title: "Executive Intelligence Summary — Q2 FY2025",
    subtitle: "Consolidated PHANTOM & SENTINEL Security Posture Assessment",
    summary: "This Executive Intelligence Summary consolidates findings from PHANTOM adversary simulation exercises, SENTINEL insider threat behavioral analytics, and purple team exercises conducted in Q2 FY2025. The assessment is prepared for Board of Directors and C-suite review and represents the most current picture of organizational security posture against advanced threats.",
    stats: [
      ...
      { label: "Detection Rate (Nation-State)", value: "37.5% — below 85% target", color: "#ef4444" },
      { label: "Simulation Exercises", value: "3 completed Q2", color: PHANTOM_ACCENT },
    ],
    sections: [
      {
        id: "s1", title: "Board-Level Risk Summary", classification: "CONFIDENTIAL // BOARD ONLY",
        content: [
      ...
      {
        id: "s2", title: "Financial Exposure Summary", classification: "CONFIDENTIAL // BOARD ONLY", severity: "critical",
        content: [
    ...
    findings: [
      { id: "f1", finding: "Nation-state detection rate 37.5% — critical gap vs. 85% organizational target", severity: "critical", recommendation: "Approve $2.1M security controls remediation budget. Priority: Credential Guard, DLP active blocking, CASB deployment." },
      { id: "f2", finding: "Active insider threat investigation — VP Finance data exfiltration risk", severity: "critical", recommendation: "Board risk committee to be briefed. Ensure legal privilege protection over investigation. Forensic evidence chain verified." },
      { id: "f3", finding: "Cyber insurance coverage insufficient vs. simulated exposure ($10M policy vs. $78.4M exposure)", severity: "high", recommendation: "Engage insurance broker to review cyber policy adequacy. Target $25-50M coverage increase." },
    ],
  },
};
...
export default function ThreatSimReport() {
  ...
  function exportPDF() {
    ...
    const statsHtml = `<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:16px;">${report.stats.map(s =>
    ...
    const footerHtml = `<div style="display:flex;justify-content:space-between;padding-top:8px;border-top:1px solid rgba(255,255,255,0.08);font-size:8px;font-family:'Courier New',monospace;">
      <span style="color:rgba(255,255,255,0.3);">GENERATED BY PHANTOM/SENTINEL AEGIS ENGINE · ${reportDate}</span>
    ...
    const body = `
      ...
      <div style="padding:20px 24px;background:#060810;">
        <div style="margin-bottom:14px;padding-bottom:12px;border-bottom:1px solid ${accent}20;">
          ...
          <div style="font-size:8px;font-family:'Courier New',monospace;color:rgba(255,255,255,0.35);margin-bottom:6px;">Prepared by: PHANTOM/SENTINEL Engine · Aegis Defense Platform</div>
    ...
    <div className="min-h-screen p-5 space-y-5" style={{ background: "#080B12" }}>
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: DS.text.muted }}>
            Export classified-style intelligence assessments from PHANTOM simulations and SENTINEL behavioral findings
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {(Object.entries(REPORT_CONFIGS) as [ReportType, typeof REPORT_CONFIGS[ReportType]][]).map(([type, cfg]) => {
          const Icon = cfg.icon;
          return (
            <button key={type} onClick={() => { setSelectedType(type); setGenerated(false); }}
          ...
          {!generated && !generating && (
            <div className="h-full flex items-center justify-center rounded-xl border" style={{ borderColor: DS.border, background: DS.surface, minHeight: 400 }}>
              <div className="text-center">
                ...
                <p className="text-[10px] mt-1" style={{ color: DS.text.muted }}>Classified intelligence assessment format</p>
              </div>
            </div>
          )}
          ...
          {generating && (
            <div className="h-full flex items-center justify-center rounded-xl border" style={{ borderColor: `${config.color}20`, background: `${config.color}04`, minHeight: 400 }}>
              <div className="text-center space-y-3">
                ...
                <p className="text-[12px] font-mono" style={{ color: config.color }}>Compiling intelligence assessment…</p>
                <div className="space-y-1 text-left max-w-xs mx-auto">
                  {["Loading PHANTOM campaign results…", "Querying SENTINEL behavioral data…", "Applying classification markings…", "Generating executive findings…", "Formatting classified brief…"].map((s, i) => (
            ...
            <div ref={reportRef} className="rounded-xl border overflow-hidden" style={{ borderColor: `${config.color}30` }}>
              ...
              <div className="p-5 space-y-5" style={{ background: "#060810" }}>
                <div className="border-b pb-4" style={{ borderColor: `${config.color}20` }}>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                      ...
                      <div className="text-[8px] font-mono" style={{ color: DS.text.muted }}>Prepared by: PHANTOM/SENTINEL Engine · Aegis Defense Platform</div>
                    </div>
                  </div>
                  <h2 className="text-base font-bold text-white">{report.title}</h2>
                  <p className="text-[11px] mt-0.5" style={{ color: config.color }}>{report.subtitle}</p>
                </div>
                ...
                <div className="rounded-xl p-4" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${config.color}15` }}>
                  ...
                  <p className="text-[11px] leading-relaxed" style={{ color: "rgba(255,255,255,0.75)" }}>{report.summary}</p>
                </div>
                ...
                <div className="border-t pt-3 flex items-center justify-between" style={{ borderColor: `${config.color}15` }}>
                  <span className="text-[8px] font-mono" style={{ color: DS.text.muted }}>GENERATED BY PHANTOM/SENTINEL AEGIS ENGINE · {reportDate}</span>
