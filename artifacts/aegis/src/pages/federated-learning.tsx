      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-xl font-bold text-red-100">Federated Learning</h1>
              <p className="text-xs text-red-400/60 mt-0.5">Privacy-preserving model training · Gradient-only aggregation · Raw data never leaves tenant boundary</p>
            </div>
          </div>

          <div className="mt-4 p-3 rounded-lg bg-green-500/5 border border-green-500/20 flex items-start gap-2">
            <ShieldCheck className="w-4 h-4 text-green-400 flex-shrink-0 mt-0.5" />
            <div>
              <p className="text-xs text-green-300 font-semibold">Zero data egress guarantee</p>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { label: 'Active Tenants', value: `${activeCount}/${TENANTS.length}`, icon: Users, color: '#4ade80' },
                { label: 'Gradients Aggregated', value: totalGradients.toLocaleString(), icon: Network, color: '#60a5fa' },
                { label: 'Avg Accuracy Gain/Round', value: `+${avgAccuracyDelta.toFixed(2)}%`, icon: TrendingUp, color: '#a78bfa' },
                { label: 'Privacy Budget Remaining', value: '78%', icon: Shield, color: '#f59e0b' },
              ].map((kpi) => (
                <div key={kpi.label} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
                  ...
                  <p className="text-xl font-bold text-red-100">{kpi.value}</p>
                </div>
              ))}
            </div>
            ...
            <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5">
              <h3 className="text-xs font-mono uppercase tracking-wider text-red-400/60 mb-4">Aggregation Architecture</h3>
              <div className="flex items-center justify-around py-4 gap-4 overflow-x-auto">
                {TENANTS.filter((t) => t.status === 'active').map((tenant, i) => (
                  <div key={tenant.id} className="flex flex-col items-center gap-2 min-w-[80px]">
                    ...
                    <p className="text-[9px] text-center text-red-300/70 leading-tight">{tenant.name.split(' ')[0]}</p>
                    <div className="flex flex-col items-center gap-1">
                      <EyeOff className="w-2.5 h-2.5 text-red-400/40" />
                      <p className="text-[8px] text-red-400/30">raw data stays</p>
                    </div>
                ...
                <div className="flex flex-col items-center">
                  <ChevronRight className="w-4 h-4 text-green-400/40" />
                </div>
                <div className="flex flex-col items-center gap-2">
                  <div className="w-12 h-12 rounded-xl bg-red-500/10 border border-red-500/30 flex items-center justify-center">
                    <Shield className="w-5 h-5 text-red-400" />
                  </div>
                  <p className="text-[9px] text-center text-red-300/70">Secure<br />Aggregator</p>
                  <p className="text-[8px] text-green-400/50">gradients only</p>
                </div>
                <div className="flex flex-col items-center">
                  <ChevronRight className="w-4 h-4 text-green-400/40" />
                </div>
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-[9px] text-center text-red-300/70">Global<br />Model</p>
                  <p className="text-[8px] text-green-400/50">distributed</p>
                </div>
              </div>
              <div className="mt-3 p-3 rounded-lg bg-white/[0.02] border border-white/[0.04]">
                <div className="grid grid-cols-3 gap-3 text-[10px]">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-red-400/60">Differential Privacy (ε={MODEL_CONFIG.epsilon})</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <Fingerprint className="w-3 h-3 text-blue-400/60" />
                    <span className="text-red-400/60">Secure Multi-Party Aggregation</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <CheckCircle className="w-3 h-3 text-purple-400/60" />
                    <span className="text-red-400/60">Gradient Clipping (threshold={MODEL_CONFIG.clippingThreshold})</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-5">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-xs font-semibold text-green-300">Round #{currentRound.roundId} — In Progress</h3>
                  </div>
                  <span className="text-[10px] text-green-400/60">{currentRound.participantCount} tenants participating</span>
                </div>
                <div className="grid grid-cols-3 gap-3 text-[11px]">
                  <div><p className="text-green-400/50">Gradients collected</p><p className="text-green-300 font-bold mt-0.5">{currentRound.aggregatedGradients.toLocaleString()}</p></div>
                  <div><p className="text-green-400/50">Privacy guarantee</p><p className="text-green-300 font-bold mt-0.5">{currentRound.privacyGuarantee}</p></div>
                  <div><p className="text-green-400/50">Started at</p><p className="text-green-300 font-bold mt-0.5">{currentRound.startedAt.split(' ')[1]}</p></div>
                </div>
              </div>
            )}
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold transition-all ${
                isAggregating || currentRound
                  ? 'bg-white/[0.04] text-red-400/30 cursor-not-allowed'
              ...
              <div key={tenant.id} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-semibold text-red-100">{tenant.name}</p>
                        <span
                      ...
                      <div className="flex items-center gap-3 mt-0.5">
                        <span className="text-[10px] text-red-400/50">{tenant.region}</span>
                        <span className="text-[10px] text-red-400/50">Local dataset: {tenant.dataSize}</span>
                        <span className="text-[10px] text-red-400/50">Last gradient: {tenant.lastGradientAt}</span>
                      </div>
                    </div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-lg font-bold" style={{ color: tenant.contributionScore >= 80 ? '#4ade80' : tenant.contributionScore >= 50 ? '#f59e0b' : '#94a3b8' }}>
                      {tenant.contributionScore}
                    </p>
                    <p className="text-[9px] text-red-400/40">contribution</p>
                  </div>
                </div>
                {tenant.status !== 'opted_out' && (
                  <div className="mt-3 space-y-2">
                    <div className="flex items-center justify-between text-[10px]">
                      <span className="text-red-400/50">Privacy budget used</span>
                      <span className="text-red-300/70">{tenant.privacyBudgetUsed} / {tenant.privacyBudgetTotal} ε</span>
                    </div>
                    <div className="w-full h-1 bg-white/[0.06] rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          ...
                          background: tenant.privacyBudgetUsed / tenant.privacyBudgetTotal > 0.7 ? '#ef4444' : '#4ade80',
                        }}
                      />
                    </div>
                    <p className="text-[10px] text-red-400/40">{tenant.roundsParticipated} rounds participated</p>
                  </div>
                )}
              </div>
            ))}
            ...
            {ROUNDS.sort((a, b) => b.roundId - a.roundId).map((round) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    {round.status === 'running' && <div className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse" />}
                    <span className="text-sm font-bold text-red-100">Round #{round.roundId}</span>
                    <span className={`text-[9px] font-mono uppercase px-1.5 py-0.5 rounded ${
                      round.status === 'complete' ? 'bg-green-500/10 text-green-400' :
                      round.status === 'running' ? 'bg-blue-500/10 text-blue-400' :
                      'bg-red-500/10 text-red-400'
                    }`}>{round.status}</span>
                  </div>
                  {round.globalAccuracyDelta > 0 && (
                    <span className="text-[11px] text-green-400 font-bold">+{round.globalAccuracyDelta.toFixed(2)}% accuracy</span>
                  )}
                </div>
                <div className="grid grid-cols-4 gap-3 text-[10px]">
                  <div><p className="text-red-400/40">Participants</p><p className="text-red-200 mt-0.5">{round.participantCount}</p></div>
                  <div><p className="text-red-400/40">Gradients</p><p className="text-red-200 mt-0.5">{round.aggregatedGradients.toLocaleString()}</p></div>
                  <div><p className="text-red-400/40">Privacy</p><p className="text-red-200 mt-0.5">{round.privacyGuarantee}</p></div>
                  <div><p className="text-red-400/40">Started</p><p className="text-red-200 mt-0.5">{round.startedAt.split(' ')[1]}</p></div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-5">
            <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5">
              <div className="flex items-center gap-2 mb-4">
                <Settings className="w-4 h-4 text-red-400/60" />
                <h3 className="text-xs font-mono uppercase tracking-wider text-red-400/60">Model Configuration</h3>
              </div>
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  <div key={item.label} className="flex flex-col gap-0.5">
                    <p className="text-[10px] text-red-400/40">{item.label}</p>
                    <p className="text-[12px] text-red-200 font-mono">{item.value}</p>
                  </div>
                ))}
              </div>
            </div>
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 flex items-start gap-2">
              <Info className="w-4 h-4 text-amber-400 flex-shrink-0 mt-0.5" />
              <div>
                <p className="text-xs text-amber-300 font-semibold">Configuration is read-only in demo mode</p>
