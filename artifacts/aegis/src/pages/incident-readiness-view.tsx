...
function ReadinessCard({ area }: { area: (typeof incidentReadiness)[0] }) {
  ...
  return (
    <div
      ...
      style={{
        background:
          area.status === 'degraded' || area.status === 'not_ready'
            ? '#c04a2a05'
...
export default function IncidentReadinessView() {
  ...
  const criticalExposures = exposureTwins.filter(
    (e) =>
      (e.severity === 'critical' && e.remediationStatus === 'open') ||
      e.remediationStatus === 'in_progress',
  ).length;
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>
            Domain-by-domain readiness scoring across detection, response, recovery, communication,
            and governance
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div
          className="col-span-1 rounded-xl border p-5 flex flex-col items-center justify-center"
        ...
        >
          ...
          <div className="text-2xl font-bold" style={{ color: '#c08a2c' }}>
            {
              incidentReadiness.filter((a) => a.status === 'degraded' || a.status === 'not_ready')
                .length
            }
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        {incidentReadiness.map((a) => (
          <ReadinessCard key={a.id} area={a} />
        ))}
      </div>
      ...
      >
        ...
        <p className="text-xs mb-4" style={{ color: 'rgba(255,255,255,0.35)' }}>
          Connecting these tools would significantly improve detection and readiness scores. Connect
          credentials to enable live data.
        </p>
