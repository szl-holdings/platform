...
interface GraphNode {
  ...
  x: number; y: number; compromised: boolean;
...
function AttackGraphCanvas({ nodes, edges, onNodeClick }: { nodes: GraphNode[]; edges: GraphEdge[]; onNodeClick?: (n: GraphNode) => void }) {
  ...
  return (
    <div className="relative w-full" style={{ height: 440, background: "rgba(10,13,20,0.95)", border: `1px solid ${DS.border}`, borderRadius: 12, overflow: "hidden" }}>
      <svg width="100%" height="100%" viewBox="0 0 800 440" preserveAspectRatio="xMidYMid meet">
        <defs>
          <filter id="glow-red"><feGaussianBlur stdDeviation="3" result="c"/><feMerge><feMergeNode in="c"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
          <filter id="glow-blue"><feGaussianBlur stdDeviation="2" result="c"/><feMerge><feMergeNode in="c"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
          <marker id="arrow-red" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#ef4444" opacity="0.7"/></marker>
          <marker id="arrow-green" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#10b981" opacity="0.7"/></marker>
          <marker id="arrow-blue" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L0,6 L8,3 z" fill="#3b82f6" opacity="0.5"/></marker>
        </defs>
        ...
        {edges.map((e, i) => {
          ...
          const sx = src.x * scale.x; const sy = src.y * scale.y;
          const dx = dst.x * scale.x; const dy = dst.y * scale.y;
          const color = e.blocked ? "#10b981" : e.weight > 0.8 ? "#ef4444" : "#3b82f6";
          const marker = e.blocked ? "url(#arrow-green)" : e.weight > 0.8 ? "url(#arrow-red)" : "url(#arrow-blue)";
          return (
            <g key={i}>
              <line x1={sx} y1={sy} x2={dx} y2={dy}
                stroke={color} strokeWidth={e.weight * 1.5} opacity={0.5}
                strokeDasharray={e.blocked ? "4 3" : undefined}
                markerEnd={marker}
              />
              <text x={(sx + dx) / 2} y={(sy + dy) / 2 - 4} fontSize="7" fill={color} opacity={0.7} textAnchor="middle">{e.label}</text>
            </g>
          );
        })}
        ...
        {nodes.map((n) => {
          ...
          return (
            <g key={n.id} style={{ cursor: "pointer" }} onMouseEnter={() => setHovered(n.id)} onMouseLeave={() => setHovered(null)} onClick={() => onNodeClick?.(n)}>
              ...
              <circle cx={cx} cy={cy} r={r} fill={`${color}22`} stroke={color} strokeWidth={isHovered ? 2.5 : 1.5} filter={n.compromised ? "url(#glow-red)" : undefined} opacity={0.95}/>
              {!n.compromised && <circle cx={cx} cy={cy} r={r} fill="none" stroke="#10b981" strokeWidth="1" strokeDasharray="3 2" opacity="0.4"/>}
              <text x={cx} y={cy + 4} fontSize={n.type === "actor" ? "9" : "8"} fill={color} textAnchor="middle" fontWeight="600">{n.type === "actor" ? "APT" : n.type.slice(0, 3).toUpperCase()}</text>
              <text x={cx} y={cy + r + 12} fontSize="7" fill={DS.text.secondary} textAnchor="middle" style={{ maxWidth: 70 }}>{n.label.slice(0, 14)}{n.label.length > 14 ? "…" : ""}</text>
              {n.techniqueId && <text x={cx} y={cy + r + 21} fontSize="6" fill={color} textAnchor="middle" opacity={0.7}>{n.techniqueId}</text>}
            </g>
          );
        })}
      </svg>

      <div className="absolute bottom-3 right-3 flex items-center gap-3 text-[10px]" style={{ color: DS.text.muted }}>
  ...
  return (
    <div className="p-6 space-y-6" style={{ maxWidth: 1280, margin: "0 auto" }}>
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: DS.text.secondary }}>
            CONSTELLATION-rendered live attack path — cyber assets, identities, controls, and incidents with evidence provenance.
          </p>
        </div>
        <div className="flex items-center gap-3">
          {provenance.traceId && (
            <span className="text-[10px] font-mono px-2 py-1 rounded" style={{ background: "rgba(239,68,68,0.08)", color: "#ef4444", border: "1px solid rgba(239,68,68,0.2)" }}>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: "Total Nodes", value: summary.totalNodes ?? nodes.length, color: "text-blue-400", icon: Network },
          { label: "Compromised", value: summary.compromisedNodes ?? nodes.filter(n => n.compromised).length, color: "text-red-400", icon: AlertTriangle },
          { label: "Critical Paths", value: summary.criticalPaths ?? 0, color: "text-orange-400", icon: ChevronRight },
          { label: "Blocked Paths", value: summary.blockedPaths ?? 0, color: "text-emerald-400", icon: CheckCircle },
        ].map(({ label, value, color, icon: Icon }) => (
          <div key={label} className="rounded-xl p-4" style={{ background: DS.surface, border: `1px solid ${DS.border}` }}>
            <div className="flex items-center gap-2 mb-2">
              <Icon className={cn("w-3.5 h-3.5", color)}/>
              <span className="text-[11px]" style={{ color: DS.text.muted }}>{label}</span>
            </div>
            {isLoading ? <div className="h-7 w-12 rounded animate-pulse" style={{ background: DS.border }}/> : <p className={cn("text-2xl font-bold", color)}>{value}</p>}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-xl p-4 space-y-3" style={{ background: DS.surface, border: `1px solid ${DS.border}` }}>
          <h3 className="text-xs font-semibold uppercase tracking-widest" style={{ color: DS.text.muted }}>High-Risk Nodes</h3>
          <div className="space-y-2">
            {nodes.filter(n => n.compromised).slice(0, 5).map(node => {
              ...
              return (
                <div key={node.id} className="flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition-colors" style={{ background: "rgba(239,68,68,0.05)", border: "1px solid rgba(239,68,68,0.12)" }} onClick={() => drillNode(node)}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium truncate" style={{ color: DS.text.primary }}>{node.label}</p>
                    <p className="text-[10px]" style={{ color: DS.text.muted }}>{node.type} · {node.techniqueId ?? "unknown"}</p>
                  </div>
                  <Badge className="text-[9px] bg-red-500/10 text-red-400 border-red-500/20 shrink-0">COMPROMISED</Badge>
                </div>
              );
            })}
          </div>
        ...
        <div className="rounded-xl p-4 space-y-3" style={{ background: DS.surface, border: `1px solid ${DS.border}` }}>
          <h3 className="text-xs font-semibold uppercase tracking-widest" style={{ color: DS.text.muted }}>Active Critical Paths</h3>
          <div className="space-y-2">
            {edges.filter(e => !e.blocked && e.weight > 0.7).slice(0, 5).map((edge, i) => {
              const src = nodes.find(n => n.id === edge.from);
              const dst = nodes.find(n => n.id === edge.to);
              return (
                <div key={i} className="flex items-center gap-2 p-2.5 rounded-lg" style={{ background: "rgba(239,68,68,0.04)", border: "1px solid rgba(239,68,68,0.1)" }}>
                  <AlertTriangle className="w-3 h-3 text-red-400 shrink-0"/>
                  <span className="text-[10px] flex-1 truncate" style={{ color: DS.text.secondary }}>
                    {src?.label?.slice(0, 16) ?? edge.from} → {dst?.label?.slice(0, 16) ?? edge.to}
                  </span>
                  <span className="text-[10px] font-mono" style={{ color: "#f97316" }}>{(edge.weight * 100).toFixed(0)}%</span>
                </div>
              );
            })}
            {edges.filter(e => e.blocked).slice(0, 3).map((edge, i) => {
              const src = nodes.find(n => n.id === edge.from);
              const dst = nodes.find(n => n.id === edge.to);
              return (
                <div key={`blocked-${i}`} className="flex items-center gap-2 p-2.5 rounded-lg" style={{ background: "rgba(16,185,129,0.04)", border: "1px solid rgba(16,185,129,0.1)" }}>
      ...
      {selectedNode && (
        <div className="rounded-xl p-4 space-y-3" style={{ background: DS.surface, border: `1px solid ${DS.border}` }}>
          ...
          <div className="grid grid-cols-3 gap-4 text-[11px]">
            ...
            <div><span style={{ color: DS.text.muted }}>Severity: </span><span style={{ color: "#ef4444" }}>{selectedNode.severity}</span></div>
            <div><span style={{ color: DS.text.muted }}>Technique: </span><span style={{ color: DS.text.primary }}>{selectedNode.techniqueId ?? "—"}</span></div>
          </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {selectedNode.evidence.map((ev, i) => (
                  <span key={i} className="text-[10px] px-2 py-0.5 rounded font-mono" style={{ background: "rgba(59,130,246,0.08)", color: "#3b82f6", border: "1px solid rgba(59,130,246,0.2)" }}>{ev}</span>
                ))}
              </div>
            </div>
          )}
          <div className="text-[10px] pt-1" style={{ color: DS.text.muted }}>
            ...
            Verified by: {selectedNode.provenance.verifiedBy} · Trace: {selectedNode.provenance.traceId}
          </div>
        </div>
      )}
