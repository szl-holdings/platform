...
import {
  Shield, AlertTriangle, Crosshair, Eye, Lock, Radar, Bug, Fingerprint, Activity, BarChart3, Target, Skull
} from "lucide-react";
...
const EVENT_TYPES = [
  { type: "threat_detected", messages: ["C2 beacon pattern identified", "Lateral movement attempt blocked", "Phishing payload quarantined"] },
  { type: "ioc_matched", messages: ["IOC hash matched: SHA256 db4f…", "IP blacklist hit: 185.x.x.x", "Domain reputation: malicious"] },
  { type: "compliance_scan", messages: ["SOC2 control verified", "NIST CSF gap identified", "PCI-DSS scan passed"] },
  { type: "vulnerability_found", messages: ["CVE-2026-0142 patched", "Critical vuln: RCE in edge proxy", "Zero-day advisory processed"] },
  { type: "incident_created", messages: ["P1 incident: unauthorized access", "P2 alert: anomalous egress", "P3 notice: failed login burst"] },
  { type: "mitre_coverage", messages: ["T1566 coverage: 94%", "ATT&CK matrix updated", "Detection rule deployed for T1059"] },
];

function MitreHeatmap() {
  const tactics = ["Recon", "Resource Dev", "Init Access", "Execution", "Persist", "Priv Esc", "Defense Evasion", "Cred Access", "Discovery", "Lat Move", "Collection", "Exfil", "C2", "Impact"];
  const [coverage, setCoverage] = useState(() => tactics.map(() => Math.floor(Math.random() * 40 + 60)));
  useEffect(() => {
    const t = setInterval(() => setCoverage(prev => prev.map(v => Math.max(50, Math.min(100, v + Math.floor((Math.random() - 0.4) * 5))))), 4000);
  ...
  return (
    <div className="grid grid-cols-7 gap-1">
      {tactics.map((t, i) => {
        ...
        return (
          <m.div key={t} className="rounded px-1 py-2 text-center" whileHover={{ scale: 1.05 }}
            style={{ background: `rgba(${r},${g},${b},0.2)`, border: `1px solid rgba(${r},${g},${b},0.3)` }}>
            <div className="text-[8px] font-bold truncate" style={{ color: `rgb(${r},${g},${b})` }}>{t}</div>
            <div className="text-[11px] font-bold tabular-nums mt-1" style={{ color: `rgb(${r},${g},${b})` }}>{v}%</div>
          </m.div>
        );
      })}
    </div>
...
function ThreatRadar() {
  const [threats, setThreats] = useState<{ angle: number; dist: number; severity: string; id: string }[]>([]);
  useEffect(() => {
    const t = setInterval(() => {
      setThreats(prev => {
        ...
        if (next.length < 6) {
          next.push({
            angle: Math.random() * 360, dist: Math.random() * 0.8 + 0.1,
  ...
  return (
    <svg viewBox="0 0 200 200" className="w-full max-w-[200px] mx-auto">
      {[30, 60, 90].map(r => <circle key={r} cx="100" cy="100" r={r} fill="none" stroke="rgba(59,130,246,0.08)" strokeWidth="1" />)}
      ...
      {threats.map(t => {
        ...
        const color = t.severity === "high" ? "#ef4444" : t.severity === "medium" ? "#f59e0b" : "#3b82f6";
        return (
          <g key={t.id}>
      ...
      <circle cx="100" cy="100" r="6" fill="rgba(59,130,246,0.2)" stroke="#3b82f6" strokeWidth="1" />
    </svg>
  );
}
...
export default function AegisPulse() {
  ...
  useEffect(() => { const t = setInterval(() => setTotalThreats(p => p + Math.floor(Math.random() * 3)), 3000); return () => clearInterval(t); }, []);
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-medium uppercase tracking-wider" style={{ color: "rgba(255,255,255,0.3)" }}>Threat Ingestion Rate</span>
            </div>
            <span className="text-[10px] tabular-nums font-medium" style={{ color: "#3b82f6" }}>{totalThreats.toLocaleString()} / hr</span>
          </div>
          <PulseThroughputChart color="#3b82f6" />
        </m.div>

        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
          <PulseMetricCard label="Active Threats" value={23} icon={Skull} color="#ef4444" trend="−3 from baseline" delay={0} />
          <PulseMetricCard label="Incidents Open" value={7} icon={AlertTriangle} color="#f97316" trend="2 P1, 5 P2" delay={80} />
          <PulseMetricCard label="IOCs Tracked" value={14892} icon={Fingerprint} color="#3b82f6" trend="+127 today" delay={160} />
          <PulseMetricCard label="MITRE Coverage" value={91} suffix="%" icon={Target} color="#10b981" trend="+2% this month" delay={240} />
          <PulseMetricCard label="Mean TTR" value={14} suffix="min" icon={Activity} color="#06b6d4" trend="−4 min improvement" delay={320} />
          <PulseMetricCard label="Assets Protected" value={3847} icon={Shield} color="#8b5cf6" trend="All monitored" delay={400} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
          <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
            ...
            <ThreatRadar />
            <p className="text-[9px] text-center mt-2" style={{ color: "rgba(255,255,255,0.2)" }}>Live threat proximity map</p>
          </m.div>
          ...
          <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
            ...
            <h2 className="text-[13px] font-semibold mb-3 flex items-center gap-2" style={{ color: "rgba(255,255,255,0.65)" }}>
              ...
              <span className="ml-auto text-[9px] px-2 py-0.5 rounded-full" style={{ background: "rgba(59,130,246,0.1)", color: "#3b82f6" }}>6 agents</span>
            </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
          ...
          <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}
            ...
            <PulseFlowDiagram flows={[
              ...
              { from: "Threat", to: "Hunt", type: "IOC Enrichment", color: "#ef4444", intensity: 3 },
              { from: "Vuln", to: "Patch", type: "Remediation Queue", color: "#f59e0b", intensity: 2 },
              { from: "GRC", to: "Exec", type: "Risk Reporting", color: "#10b981", intensity: 5 },
              { from: "MSP", to: "NOC", type: "Client Monitoring", color: "#8b5cf6", intensity: 3 },
            ]} />
          </m.div>
        </div>
