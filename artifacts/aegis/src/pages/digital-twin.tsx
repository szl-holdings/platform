...
interface AttackSurfaceItem { area: string; risk: number }
...
export default function DigitalTwin() {
  ...
  type TopologyResponse = { data?: { nodes?: TwinNode[]; attackSurface?: AttackSurfaceItem[]; syncedCount?: number; totalVulns?: number; fidelity?: string } };
  ...
  const scenarios: TwinScenario[] = (scenariosQuery.data as ScenariosResponse | null)?.data?.scenarios ?? [];

  const syncedCount = topology?.syncedCount ?? nodes.filter(n => n.syncState === "synced").length;
  const totalVulns = topology?.totalVulns ?? nodes.reduce((s, n) => s + (n.vulnerabilities ?? 0), 0);
  ...
  return (
    <div className="p-6 space-y-6 max-w-full">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Assets Modeled", value: nodes.length || "—", sub: `${syncedCount} synced, ${nodes.length - syncedCount} drifted`, color: "#06b6d4", icon: Layers },
          { label: "Twin Fidelity", value: topology?.fidelity ?? "99.1%", sub: "config accuracy vs live", color: "#10b981", icon: CheckCircle },
          ...
          return (
            <div key={m.label} className="rounded-xl border border-white/8 bg-white/3 p-4">
              ...
              <div className="text-2xl font-bold text-white">{topologyQuery.isLoading ? <span className="text-zinc-500 text-lg">loading…</span> : m.value}</div>
                ...
                return (
                  <div key={node.id} className="rounded-xl border border-white/8 bg-white/3 p-3">
                    <div className="flex items-center gap-3">
                      <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style={{ background: `${typeColor[node.type] ?? "#888"}20` }}>
                        <Icon className="w-3.5 h-3.5" style={{ color: typeColor[node.type] ?? "#888" }} />
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-0.5 text-[10px]">
                          <span className="text-zinc-500">Sync: {node.lastSyncLabel ?? node.lastSync}</span>
                          {node.vulnerabilities > 0 && <span className="text-red-400">{node.vulnerabilities} vulns</span>}
                        </div>
                      </div>
                      <span className={cn("text-[10px] px-1.5 py-0.5 rounded border capitalize shrink-0", syncColor[node.syncState] ?? "text-zinc-400")}>
                        {node.syncState}
                      </span>
                    </div>
                  </div>
                ...
                <div key={scenario.id} className={cn("rounded-xl border p-3", scenario.status === "running" ? "border-amber-500/30 bg-amber-500/5" : "border-white/8 bg-white/3")}>
                  <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span className={cn("text-[10px] font-medium capitalize", scenarioStatusColor[scenario.status] ?? "text-zinc-400")}>
            ...
            <div className="rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-4 mt-3">
              ...
              <div className="space-y-2">
                {attackSurface.map((item: AttackSurfaceItem) => {
                  const color = item.risk >= 70 ? "#ef4444" : item.risk >= 55 ? "#f97316" : "#f59e0b";
                  return (
                    <div key={item.area}>
                      ...
                      <div className="h-1.5 rounded-full bg-white/5">
                        <div className="h-full rounded-full" style={{ width: `${item.risk}%`, background: color + "80" }} />
                      </div>
                    </div>
                  );
                })}
