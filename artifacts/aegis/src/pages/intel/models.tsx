...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span className={cn(
      ...
      {active ? <CheckCircle2 className="w-2.5 h-2.5" /> : <AlertTriangle className="w-2.5 h-2.5" />}
      {status}
    </span>
  );
}
...
function PlatformBadge({ platform }: { platform: string }) {
  ...
  return (
    <span className={cn("px-2 py-0.5 rounded text-[10px] font-medium", colors[platform] ?? colors.Internal)}>
      {platform}
    </span>
  );
}
...
function ModelRegistryTable({ entries, isLoading }: { entries: RegistryEntry[]; isLoading: boolean }) {
  ...
  const avgConfidence = (() => {
    ...
    return withBaseline.reduce((acc, e) => acc + (e.confidenceBaseline ?? 0), 0) / withBaseline.length;
  })();
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-card/60 border border-border rounded-xl p-4">
          ...
          <p className="text-2xl font-bold text-cyan-400">{avgConfidence != null ? `${Math.round(avgConfidence * 100)}%` : "—"}</p>
        </div>
      </div>
            ...
            <div className="flex gap-1 flex-wrap">
              {domains.slice(0, 7).map(d => (
                <button key={d} onClick={() => setDomainFilter(d)}
                  className={cn("px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-colors",
                    domainFilter === d ? "bg-primary/15 text-primary" : "text-muted-foreground hover:bg-muted/50"
                  )}>{d}</button>
              ))}
            </div>
          )}
      ...
      <div className="bg-card/60 border border-border rounded-xl overflow-x-auto">
        {isLoading ? (
          ...
          <EmptyState icon={Brain} title="No models registered" detail="Populate alloy_runtime_agents to see live entries" />
        ) : (
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left px-4 py-3 text-muted-foreground font-medium">Model / Agent</th>
                ...
                <th className="text-left px-4 py-3 text-muted-foreground font-medium">Policy Tier</th>
                <th className="text-center px-4 py-3 text-muted-foreground font-medium">Confidence</th>
            ...
            <tbody>
              {filtered.map((entry) => (
                <tr key={entry.id} className="border-b border-border/50 hover:bg-muted/10 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-muted-foreground font-mono">{entry.model || entry.id}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-muted-foreground">{entry.provider}</td>
                  <td className="px-4 py-3">
                    <span className="font-mono bg-muted px-1.5 py-0.5 rounded text-foreground">v{entry.version}</span>
                  </td>
                  <td className="px-4 py-3 capitalize text-muted-foreground">{entry.domain}</td>
                  <td className="px-4 py-3">
                    <span className="text-muted-foreground text-[10px] font-mono">{entry.policyTier}</span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right font-mono text-[10px] text-muted-foreground">{fmt(entry.updatedAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div className="bg-card/60 border border-border rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-display font-semibold text-foreground">Inference Governance Log</h3>
          <span className="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded">{entries.length} events</span>
        </div>
        {entries.length > 15 && (
      ...
      {isLoading ? (
        ...
        <EmptyState icon={InboxIcon} title="No inference events yet" detail="Events will appear here as Alloy agents run inference calls" />
      ) : (
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border bg-muted/20">
                ...
                <th className="text-left px-4 py-2.5 text-muted-foreground font-medium">Platform</th>
                <th className="text-center px-4 py-2.5 text-muted-foreground font-medium">Confidence</th>
                <th className="text-right px-4 py-2.5 text-muted-foreground font-medium">Timestamp</th>
              </tr>
            </thead>
            <tbody>
              {visible.map((e) => (
                <tr key={e.id} className="border-b border-border/40 hover:bg-muted/10 transition-colors">
                  <td className="px-4 py-2.5 font-mono text-foreground max-w-[180px] truncate">{e.model}</td>
                  <td className="px-4 py-2.5 text-muted-foreground capitalize">{e.action.replace(/-/g, " ")}</td>
                  <td className="px-4 py-2.5">
                    <span className="text-muted-foreground">{e.entityType}</span>
                    {e.entityId && <span className="text-[10px] font-mono text-muted-foreground/60 ml-1">{e.entityId}</span>}
                  </td>
                  <td className="px-4 py-2.5 font-mono text-muted-foreground">{e.actor}</td>
                  <td className="px-4 py-2.5"><PlatformBadge platform={e.platform} /></td>
                  <td className="px-4 py-2.5 text-center"><ConfidenceBadge value={e.confidence} /></td>
                  <td className="px-4 py-2.5 text-right font-mono text-[10px] text-muted-foreground">{fmt(e.timestamp)}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div className="p-6 lg:p-8 space-y-6 max-w-[1300px]">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Live registry of active AI models with version tracking, confidence baselines, and a full inference audit log
          </p>
        </div>
        <button
          onClick={() => { void refetchReg(); void refetchLog(); }}
      ...
      <section>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-sm font-semibold text-foreground uppercase tracking-wide">Model Registry</h2>
        </div>
        {regError ? (
      ...
      <section>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-sm font-semibold text-foreground uppercase tracking-wide">Inference Governance Log</h2>
        </div>
        {logError ? (
