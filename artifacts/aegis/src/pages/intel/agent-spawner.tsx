...
const DOMAIN_TEMPLATES = [
  ...
  },
  {
    ...
    defaultInstruction:
      'Monitor client portfolio performance, prepare pre-meeting intelligence briefings, and track competitive movements.',
    domain: 'Carlota Jo',
    systemType: 'deep' as const,
    capabilities: [
  ...
  },
  {
    ...
    defaultInstruction:
      'Monitor social, news, and industry forums for brand mentions, reputation risks, and speaking opportunities.',
    domain: 'Stephen',
    systemType: 'fast' as const,
    capabilities: [
    ...
    <div className="p-6 lg:p-8 space-y-6 max-w-[1600px]">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Create, configure, and deploy domain-specific intelligence agents across the SZL ecosystem
          ...
          {selectedTemplate ? (
            <div className="bg-card/60 border border-border rounded-xl p-5">
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {selectedTemplate.capabilities.map((cap) => (
                    <span
                      key={cap}
                      className="px-2 py-0.5 rounded text-[10px] font-mono bg-muted/40 text-muted-foreground border border-border/50"
                    >
                      {cap}
                    </span>
                  ))}
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground/50 mt-1 flex items-center gap-1">
                      ...
                      The AI Engine interprets this instruction and configures the agent's mission
                      parameters.
                    </p>
                  </div>
            ...
            <div className="bg-card/30 border border-border/50 rounded-xl p-8 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-sm text-muted-foreground">
                Select a domain template to configure and deploy an agent
              </p>
              <p className="text-xs text-muted-foreground/50 mt-1">
                Each agent becomes a node in the network
              </p>
            </div>
          )}
        </div>
      ...
      <div className="bg-card/60 border border-border rounded-xl overflow-hidden">
        <div className="px-5 py-3 border-b border-border flex items-center justify-between">
          ...
          <span className="text-[10px] text-muted-foreground font-mono">
            {LIVE_AGENTS.length} agents running
          </span>
        </div>
          ...
          <thead>
            <tr className="border-b border-border bg-muted/10">
              {['Agent', 'Domain', 'Status', 'Uptime', 'Actions', 'Last Action'].map((col) => (
                <th
                  key={col}
                  className="text-left px-4 py-2 font-mono text-[10px] text-muted-foreground uppercase tracking-wider"
                >
                  {col}
                </th>
              ))}
