    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Layer-by-layer inspection, parameter counts, and topology comparison across model families
        </p>
      </div>
        ...
        {architectures.map((arch) => (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${statusColors[arch.status]}`}
              >
                {arch.status}
              </span>
            </div>
            ...
            <p className="text-xs text-muted-foreground mb-3">
              {arch.type} · {arch.params} parameters
            </p>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                <span className="text-muted-foreground">Layers:</span>{' '}
                <span className="font-medium">{arch.layers}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Heads:</span>{' '}
                <span className="font-medium">{arch.heads}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className="font-medium">{arch.latency}</span>
              </div>
            </div>
          </div>
        ))}
