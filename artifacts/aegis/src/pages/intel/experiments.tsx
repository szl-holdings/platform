      ...
      >
        ...
        {lastMetric && (
          <div className="hidden md:flex items-center gap-4 text-xs text-muted-foreground">
            <span>
              Acc: <span className="text-foreground">{lastMetric.accuracy.toFixed(1)}%</span>
            </span>
            <span>
              Loss: <span className="text-foreground">{lastMetric.loss.toFixed(3)}</span>
            </span>
          </div>
        )}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(experiment.hyperparameters).map(([key, val]) => (
                <span
                  key={key}
                  className="text-[10px] font-mono px-2 py-1 rounded bg-muted/40 text-muted-foreground"
                >
          ...
          {completedExps.slice(0, 10).map((exp) => {
            ...
            return (
              <tr key={exp.id} className="border-b border-border/50 hover:bg-muted/10">
                ...
                <td className="py-2 text-right font-mono text-foreground">
                  {last.valAccuracy != null ? last.valAccuracy.toFixed(1) : '—'}%
                </td>
        ...
        <ExportButton
          data={experiments.map((e) => {
            ...
            return {
              ...
              'Val Accuracy':
                last && last.valAccuracy != null ? `${last.valAccuracy.toFixed(1)}%` : '',
