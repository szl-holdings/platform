...
const MODELS = [
  ...
  { id: "code-analyst", name: "Code Analyst 13B", type: "Technical Analysis", latency: "380ms", cost: "$0.0015/req", requests: 445, accuracy: 95.9, status: "degraded" },
];
...
function generateEvent(): RelayEvent {
  const sources = ["Maritime Analyst", "IT Sentinel", "Deal Scout", "Creative Director", "Advisory Agent", "Portfolio Analyst", "Brand Monitor"];
  const types = ["inference", "analysis", "prediction", "generation", "classification"];
  const privacies: RelayEvent["privacy"][] = ["public", "internal", "internal", "confidential", "restricted"];
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-[1600px]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Requests (live window)", value: totalRequests, color: "text-foreground", bg: "" },
          ...
          { label: "Blocked (policy)", value: blocked, color: blocked > 0 ? "text-red-400" : "text-emerald-400", bg: "" },
        ].map(s => (
            ...
            <div key={model.id} className={cn(
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={cn("text-[10px] font-mono px-1.5 py-0.5 rounded",
                  model.status === "optimal" ? "text-emerald-400 bg-emerald-400/10" : "text-amber-400 bg-amber-400/10"
                )}>{model.status}</span>
              </div>
              <p className="text-[10px] text-muted-foreground font-mono mb-3">{model.type}</p>
              <div className="grid grid-cols-2 gap-2 text-[10px]">
                <div><span className="text-muted-foreground">Latency: </span><span className="font-mono text-foreground">{model.latency}</span></div>
                <div><span className="text-muted-foreground">Cost: </span><span className="font-mono text-foreground">{model.cost}</span></div>
                <div><span className="text-muted-foreground">Requests: </span><span className="font-mono text-cyan-400">{model.requests.toLocaleString()}</span></div>
                <div><span className="text-muted-foreground">Accuracy: </span><span className="font-mono text-emerald-400">{model.accuracy}%</span></div>
              </div>
              <div className="mt-2">
                <div className="h-1 bg-border rounded-full overflow-hidden">
                  <div className="h-full bg-cyan-400/60 rounded-full" style={{ width: `${(model.requests / 5000) * 100}%` }} />
                </div>
              </div>
            </div>
          ))}
          ...
          {ROUTING_RULES.map((rule, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-4 px-5 py-3 hover:bg-muted/10 transition-colors">
                ...
                <span className={cn("text-[10px] font-mono px-2 py-0.5 rounded-full", privacy.bg, privacy.color)}>
                  {privacy.label}
                </span>
              </div>
            );
      ...
      <div className="bg-card/60 border border-border rounded-xl overflow-hidden">
        <div className="px-5 py-3 border-b border-border flex items-center justify-between">
          ...
          <button onClick={() => setPaused(p => !p)}
            className={cn("text-[10px] font-mono px-3 py-1 rounded border transition-colors",
              paused
        ...
        <div className="overflow-auto max-h-72">
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-card/90 backdrop-blur-sm border-b border-border">
              <tr>
                {["Time", "Source Agent", "Target Model", "Type", "Privacy", "Latency", "Tokens", "Status"].map(col => (
                  <th key={col} className="text-left px-3 py-2 font-mono text-[10px] text-muted-foreground uppercase tracking-wider">{col}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {events.map((event, i) => {
                ...
                return (
                  <tr key={event.id}
                    ...
                    <td className="px-3 py-1.5 text-cyan-400 font-mono text-[10px]">{event.targetModel}</td>
                    <td className="px-3 py-1.5 text-muted-foreground">{event.requestType}</td>
                    <td className="px-3 py-1.5">
                      <span className={cn("text-[10px] px-1.5 py-0.5 rounded font-mono", privacy.bg, privacy.color)}>
                        {privacy.label}
                      </span>
                    </td>
                    <td className="px-3 py-1.5 font-mono text-muted-foreground">{event.latency}ms</td>
