export default function S11Market() {
  const markets = [
    ...
    { domain: "Cloud Governance", size: "$22B", growth: "+24% CAGR", note: "Multi-cloud policy and infrastructure compliance", color: "#fb923c" },
  ];
  return (
    <div
    ...
    >
      ...
      <div style={{ position: "absolute", inset: 0, padding: "5vh 7vw" }}>
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: "2vw", marginBottom: "3.5vh" }}>
          <h2 style={{ fontSize: "3.5vw", fontWeight: 800, letterSpacing: "-0.025em", color: "#f0ece6", lineHeight: 1.05 }}>
            Six High-Value Markets
          </h2>
          <div style={{ fontFamily: "'Inter', sans-serif", fontSize: "2vw", color: "#f5a623", fontWeight: 700 }}>
            $134B+ Combined TAM
          </div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: "1.6vh", height: "65vh", justifyContent: "flex-start" }}>
          {markets.map((m) => (
            <div
              key={m.domain}
              style={{
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: "1.2vw" }}>
                <div style={{ width: "0.4vw", height: "3vh", background: m.color, borderRadius: "2px", flexShrink: 0 }} />
                <div style={{ fontSize: "1.5vw", fontWeight: 600, color: "#f0ece6" }}>{m.domain}</div>
              </div>
              <div style={{ fontSize: "2vw", fontWeight: 800, color: "#f5a623", letterSpacing: "-0.02em" }}>{m.size}</div>
              <div style={{ fontFamily: "'Inter', sans-serif", fontSize: "1.2vw", color: "#0cc8d9", fontWeight: 500 }}>{m.growth}</div>
              <div style={{ fontFamily: "'Inter', sans-serif", fontSize: "1.15vw", color: "rgba(240,236,230,0.4)" }}>{m.note}</div>
            </div>
          ))}
        </div>
      </div>
