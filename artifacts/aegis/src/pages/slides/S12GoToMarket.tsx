export default function S12GoToMarket() {
  ...
  return (
    ...
    >
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "0.15vh", background: "linear-gradient(90deg, transparent, #8b5cf6, transparent)" }} />
      <div style={{ position: "absolute", inset: 0, padding: "5vh 7vw" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "3vw", height: "62vh" }}>
          {phases.map((p) => (
            <div
              key={p.phase}
              style={{
            ...
            >
              <div>
                <div style={{ fontFamily: "'Inter', sans-serif", fontSize: "0.9vw", fontWeight: 600, letterSpacing: "0.15em", textTransform: "uppercase", color: p.color, marginBottom: "0.6vh" }}>{p.phase}</div>
                <div style={{ fontSize: "2vw", fontWeight: 800, color: "#f0ece6", lineHeight: 1.1, marginBottom: "0.5vh" }}>{p.label}</div>
                <div style={{ fontFamily: "'Inter', sans-serif", fontSize: "1.1vw", color: "rgba(240,236,230,0.35)" }}>{p.timing}</div>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1.2vh", flex: 1 }}>
                {p.items.map((item, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "1vw" }}>
                    <div style={{ width: "0.4vw", height: "0.4vw", borderRadius: "50%", background: p.color, flexShrink: 0, marginTop: "0.8vh" }} />
                    <div style={{ fontFamily: "'Inter', sans-serif", fontSize: "1.25vw", color: "rgba(240,236,230,0.6)", lineHeight: 1.4 }}>{item}</div>
                  </div>
                ))}
              </div>
            </div>
