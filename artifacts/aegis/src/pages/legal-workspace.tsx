...
import { Scale, Clock, DollarSign, ShieldCheck, FileText, ChevronRight, WifiOff } from "lucide-react";
...
const SECTION_META: Record<SectionKey, { title: string; subtitle: string }> = {
  overview: { title: "Matter Overview", subtitle: "Cross-matter health, exposure, and forecast at a glance" },
  matters: { title: "Active Matters", subtitle: "Detailed readiness across all open matters" },
  deadlines: { title: "Deadline Risk Queue", subtitle: "Time-sensitive filings and discovery cutoffs" },
...
function DeadlineRow({ title, date, matter }: { title: string; date: string; priority: string; matter: string }) {
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <div
            ...
            style={{
              background: matter.healthScore >= 70 ? "#4a90b820" : matter.healthScore >= 50 ? "#d4a05420" : "#c45a4a20",
              color: matter.healthScore >= 70 ? "#4a90b8" : matter.healthScore >= 50 ? "#d4a054" : "#c45a4a",
            }}
          >
            {matter.healthScore}
          </div>
          <div>
            ...
            <div className="text-[10px] text-slate-500">{matter.caseNumber} · {matter.jurisdiction}</div>
          </div>
        </div>
        <span className={`px-2 py-0.5 rounded text-[10px] font-medium ${
          matter.status === "discovery" ? "bg-[#4a90b8]/10 text-[#4a90b8]" : "bg-[#d4a054]/10 text-[#d4a054]"
        }`}>
          {matter.status.replace("_", " ").toUpperCase()}
        </span>
      </div>
      <div className="space-y-1 mt-2">
        {Object.entries(matter.readinessScores).slice(0, 4).map(([k, v]) => (
    ...
    <div className="p-6 space-y-6 max-w-[1400px] mx-auto">
          ...
          <div className="rounded-lg border border-white/[0.06] p-4" style={{ background: "#0c1220" }}>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <div key={item.label} className="flex items-center justify-between rounded border border-white/[0.04] px-3 py-2" style={{ background: "#080c14" }}>
      ...
      {section === "deadlines" && (
        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-2 rounded-lg border border-white/[0.06] p-4" style={{ background: "#0c1220" }}>
          ...
          <div className="space-y-3">
            ...
            <KpiCard
              label="Critical / High"
              value={String(allDeadlines.filter(d => d.priority === "critical" || d.priority === "high").length)}
