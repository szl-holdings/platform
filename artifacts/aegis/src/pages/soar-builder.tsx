...
const nodeTypeConfig: Record<NodeType, { color: string; bg: string; border: string; label: string; icon: React.ElementType }> = {
...
const DEFAULT_CANVAS_NODES: PlaybookNode[] = [
  { id: "n1", type: "trigger", label: "SIEM Alert: Critical", config: "Severity = Critical, Source = Any SIEM", auto: true },
  { id: "n2", type: "enrich", label: "Fetch IOC Reputation", config: "Query VirusTotal, AbuseIPDB, MISP", auto: true },
  { id: "n3", type: "condition", label: "Asset Tier 1?", config: "IF asset.criticality == 'tier-1'", auto: true },
  { id: "n4", type: "approve", label: "CISO Approval Gate", config: "Notify: CISO + Security Lead. Timeout: 15min", auto: false },
  { id: "n5", type: "action", label: "Auto-Isolate Endpoint", config: "EDR API: isolate host, block network", auto: true },
  { id: "n6", type: "action", label: "Block IOCs in Firewall", config: "Push to firewall blocklist, DNS sinkhole", auto: true },
  { id: "n7", type: "notify", label: "Create JIRA Ticket", config: "Create P1 ticket, assign to SOC oncall", auto: true },
  { id: "n8", type: "notify", label: "Slack Notification", config: "#soc-incidents channel, tag oncall", auto: true },
];
...
export default function SOARBuilder() {
  ...
  useEffect(() => {
    const detail = (playbookDetailQuery.data as { data?: { playbook?: PlaybookDetail } } | null)?.data?.playbook;
  ...
  const saveMutation = useStandardMutation({
    mutationFn: () => {
      ...
      if (selectedPlaybookId && selectedPlaybook) {
        return api.soarBuilder.updatePlaybook(selectedPlaybookId, { nodes: canvasNodes, status: "active" });
      }
  ...
  const playbooks: PlaybookSummary[] = (playbooksQuery.data as { data?: { playbooks?: PlaybookSummary[] } } | null)?.data?.playbooks ?? [];
  const runs: SoarRun[] = (runsQuery.data as { data?: { runs?: SoarRun[] } } | null)?.data?.runs ?? [];
  const selectedPlaybook: PlaybookSummary | null = playbooks.find(p => p.id === selectedPlaybookId) ?? playbooks[0] ?? null;
    ...
    <div className="p-6 space-y-6 max-w-full">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-zinc-500">Build, persist, and execute response playbooks. All runs are recorded with full audit history.</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => { setSelectedPlaybookId(null); setCanvasNodes(DEFAULT_CANVAS_NODES); setSelectedNode(null); }}
          ...
          >
            {saveMutation.isPending ? <><RefreshCw className="w-3.5 h-3.5 animate-spin" /> Saving...</> : <><CheckCircle className="w-3.5 h-3.5" /> Save & Deploy</>}
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 border-b border-white/10">
        <button onClick={() => setTab("builder")} className={cn("flex items-center gap-1.5 text-xs px-3 py-2 border-b-2 transition-colors", tab === "builder" ? "border-cyan-400 text-cyan-300" : "border-transparent text-zinc-500 hover:text-zinc-300")}>
          <Zap className="w-3 h-3" /> Builder
        </button>
        <button onClick={() => setTab("runs")} className={cn("flex items-center gap-1.5 text-xs px-3 py-2 border-b-2 transition-colors", tab === "runs" ? "border-cyan-400 text-cyan-300" : "border-transparent text-zinc-500 hover:text-zinc-300")}>
          <History className="w-3 h-3" /> Run History {runs.length > 0 && <span className="text-[10px] px-1 py-0.5 rounded-full bg-cyan-500/20 text-cyan-400">{runs.length}</span>}
        </button>
      </div>
                ...
                {playbooks.map((pb: PlaybookSummary) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 text-[9px] text-zinc-500">
                      ...
                      {pb.runCount > 0 && <span className="text-emerald-400">{pb.successRate}% success</span>}
          ...
          <div>
            ...
            {selectedPlaybook ? (
              <div className="space-y-3">
                <div className="rounded-xl border border-white/8 bg-white/3 p-3 space-y-2">
                  ...
                  {[
                    ...
                    { label: "Status", value: selectedPlaybook.status, color: selectedPlaybook.status === "active" ? "#10b981" : "#f59e0b" },
                  ].map(stat => (
                    <div key={stat.label} className="flex items-center justify-between text-[11px]">
                      <span className="text-zinc-500">{stat.label}</span>
                      <span style={stat.color ? { color: stat.color } : {}} className={!stat.color ? "text-white capitalize" : "capitalize"}>{stat.value}</span>
                    </div>
                  ))}
                </div>
                ...
                >
                  {executeMutation.isPending ? <><RefreshCw className="w-3.5 h-3.5 animate-spin" /> Executing…</> : <><Play className="w-3.5 h-3.5" /> Execute Playbook</>}
                </button>
                ...
                <div className="rounded-xl border border-white/8 bg-white/3 p-3">
                  <div className="text-[11px] font-semibold text-zinc-300 mb-2">Recent Activity</div>
                  <div className="space-y-1.5">
                    {runs.filter(r => r.playbookId === selectedPlaybook.id).slice(0, 3).map((run: SoarRun) => (
                      <div key={run.id} className="text-[10px] text-zinc-500">
                        <span className={STATUS_COLOR[run.status] ? `text-[10px] px-1 py-0.5 rounded border capitalize ${STATUS_COLOR[run.status]}` : "text-zinc-500"}>{run.status.replace(/_/g, " ")}</span>
              ...
              <div className="rounded-xl border border-white/8 bg-white/3 p-4 text-center">
                <div className="text-xs text-zinc-500">Select a playbook to view details and run history, or build a new one on the canvas.</div>
              </div>
            )}
          </div>
        </div>
      ...
      {tab === "runs" && (
        <div className="space-y-3">
          <div className="flex items-center justify-between mb-2">
            ...
            {selectedPlaybookId && (
              <button onClick={() => setSelectedPlaybookId(null)} className="text-[10px] text-zinc-500 hover:text-zinc-300">
                Show all runs →
              </button>
            )}
          </div>
          ...
          {runsQuery.isLoading ? (
            ...
            <div className="rounded-xl border border-white/8 bg-white/3 p-8 text-center">
              ...
              <div className="text-xs text-zinc-500">No runs yet. Execute a playbook to see history here.</div>
            </div>
          ) : (
              ...
              <div key={run.id} className="rounded-xl border border-white/8 bg-white/3 p-4">
                <div className="flex items-start justify-between gap-3 mb-2">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={cn("text-[10px] px-1.5 py-0.5 rounded border capitalize", STATUS_COLOR[run.status] ?? "text-zinc-400 border-zinc-500/30 bg-zinc-500/10")}>
                      {run.status.replace(/_/g, " ")}
                    </span>
                  </div>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-[10px] mb-2">
                  <div><span className="text-zinc-500">Duration: </span><span className="text-white">{run.duration}</span></div>
                  <div><span className="text-zinc-500">Steps done: </span><span className="text-white">{run.stepsCompleted}</span></div>
                  <div><span className="text-zinc-500">Failed: </span><span className={run.stepsFailed > 0 ? "text-red-400" : "text-emerald-400"}>{run.stepsFailed}</span></div>
                  <div><span className="text-zinc-500">Started: </span><span className="text-white">{relTime(run.startedAt)}</span></div>
                </div>
                {run.outcome && (
                  <div className="text-[10px] text-zinc-400 bg-white/3 rounded p-2">{run.outcome}</div>
                )}
                {run.incidentId && (
                ...
                {run.status === "awaiting_approval" && (
                  <div className="flex items-center gap-1.5 text-[10px] text-amber-400 mt-2 px-2 py-1.5 rounded bg-amber-500/10 border border-amber-500/20">
                    <Clock className="w-3 h-3" /> Paused at approval gate — awaiting CISO authorization
                  </div>
                )}
              </div>
            ))
