...
const threatTypeIcons: Record<string, string> = {
  malware: "M", ransomware: "R", phishing: "P", ddos: "D", apt: "A", vulnerability: "V", insider: "I", supply_chain: "S",
};
...
function ThreatMapCanvas({ threats }: { threats: any[] }) {
  ...
  useEffect(() => {
    ...
    threats.forEach((t, i) => {
      ...
      const color = isCritical ? "rgba(239, 68, 68, 0.8)" : t.severity === "high" ? "rgba(249, 115, 22, 0.7)" : "rgba(234, 179, 8, 0.6)";
      ...
      gradient.addColorStop(0, color.replace("0.8", "0.3").replace("0.7", "0.2").replace("0.6", "0.15"));
...
export default function ThreatIntelligence() {
  ...
  const filteredCves = cves.filter((c: any) => {
    const matchesSearch = !cveFilter || c.id.toLowerCase().includes(cveFilter.toLowerCase()) || c.description.toLowerCase().includes(cveFilter.toLowerCase());
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 animate-fade-in-up stagger-1">
        <Card className="bg-card border-border hover:border-red-500/20 transition-all group">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-muted-foreground uppercase tracking-wider">Active Threats</p>
                <p className="text-2xl font-bold font-display mt-1 text-red-400"><AnimatedCounter value={threats.length} /></p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold font-display mt-1 text-orange-400"><AnimatedCounter value={criticalThreats} /></p>
              </div>
              <div className={`w-10 h-10 rounded-lg bg-orange-500/10 flex items-center justify-center group-hover:scale-110 transition-transform ${criticalThreats > 0 ? "animate-pulse" : ""}`}>
        ...
        <Card className="bg-card border-border hover:border-amber-500/20 transition-all group">
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-muted-foreground uppercase tracking-wider">CVE Advisories</p>
                <p className="text-2xl font-bold font-display mt-1"><AnimatedCounter value={cves.length} /></p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold font-display mt-1"><AnimatedCounter value={geoEvents.length} /></p>
              </div>
            ...
            {briefing ? (
              <>
                <div className="bg-purple-500/5 rounded-lg p-3 border border-purple-500/10">
                  ...
                  <p className="text-sm text-muted-foreground">{briefing.analysis?.summary?.summary || "Generating analysis..."}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-2">Sentiment Analysis</p>
                  <Badge variant="outline" className={briefing.analysis?.sentiment?.label === "NEGATIVE" ? "bg-red-500/10 text-red-400" : "bg-emerald-500/10 text-emerald-400"}>
                    {briefing.analysis?.sentiment?.label} ({((briefing.analysis?.sentiment?.score || 0) * 100).toFixed(0)}%)
                  </Badge>
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {briefing.analysis.entities.entities.slice(0, 8).map((e: any, i: number) => (
                        <Badge key={i} variant="outline" className="text-xs">{e.word} <span className="text-muted-foreground ml-1">({e.entity})</span></Badge>
                      ))}
                    </div>
                  </div>
                )}
                <p className="text-[10px] text-muted-foreground/50">Generated {new Date(briefing.generatedAt).toLocaleTimeString()}</p>
              </>
            ) : (
        ...
        <Card className="bg-card border-border animate-fade-in-up stagger-4">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="outline" className="text-xs">NVD</Badge>
            </div>
            <div className="flex gap-2 mt-2">
              <div className="relative flex-1">
                <Search className="absolute left-2.5 top-2.5 w-3.5 h-3.5 text-muted-foreground" />
                <Input placeholder="Search CVEs..." value={cveFilter} onChange={e => setCveFilter(e.target.value)} className="pl-8 h-8 text-sm" />
              </div>
          ...
          <CardContent>
            <div className="space-y-2 max-h-[400px] overflow-y-auto pr-1">
              {filteredCves.map((cve: any) => (
                <div key={cve.id} className="p-3 rounded-lg bg-background/50 border border-border hover:border-primary/20 transition-all cursor-pointer group">
                  <div className="flex items-start justify-between mb-1">
                    <span className="font-mono text-sm font-semibold text-primary group-hover:text-primary/80">{cve.id}</span>
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className={`text-xs ${severityColors[cve.severity] || ""}`}>{cve.severity}</Badge>
                  ...
                  <div className="flex items-center gap-3 mt-2 text-[10px] text-muted-foreground/60">
                    ...
                    <span className="flex items-center gap-1"><Clock className="w-2.5 h-2.5" /> {new Date(cve.published).toLocaleDateString()}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <CardContent>
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
              {geoEvents.map((event: any) => (
                <div key={event.id} className="p-3 rounded-lg bg-background/50 border border-border hover:border-primary/20 transition-all">
                  <div className="flex items-start justify-between mb-1">
                    <Badge variant="outline" className={`text-xs ${severityColors[event.severity] || ""}`}>{event.severity}</Badge>
                  ...
                  <div className="flex items-center justify-between mt-2 text-[10px] text-muted-foreground/60">
                    <span className="flex items-center gap-1"><FileText className="w-2.5 h-2.5" /> {event.source}</span>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
            {threats.map((threat: any) => (
              <div key={threat.id} className={`p-3 rounded-lg border transition-all hover:-translate-y-0.5 hover:shadow-lg cursor-pointer ${threat.severity === "critical" ? "border-red-500/20 bg-red-500/5 hover:shadow-red-500/5" : "border-border bg-background/50 hover:shadow-primary/5"}`}>
                <div className="flex items-center justify-between mb-2">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${threat.severity === "critical" ? "bg-red-500/20 text-red-400" : "bg-orange-500/20 text-orange-400"}`}>
                    {threatTypeIcons[threat.type] || "?"}
                  </div>
                  <Badge variant="outline" className={`text-xs ${severityColors[threat.severity] || ""}`}>
                    {threat.severity === "critical" && <span className="w-1.5 h-1.5 rounded-full bg-red-400 mr-1 animate-pulse" />}
                    {threat.severity}
                  </Badge>
                </div>
                <h4 className="text-sm font-semibold font-display">{threat.name}</h4>
                <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{threat.description}</p>
                <div className="flex items-center justify-between mt-2 text-[10px] text-muted-foreground/60">
                  <span className="flex items-center gap-1"><MapPin className="w-2.5 h-2.5" /> {threat.country}</span>
