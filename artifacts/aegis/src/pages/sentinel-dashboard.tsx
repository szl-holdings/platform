    ...
    <div className="p-4 md:p-6 space-y-5 max-w-[1400px]">
        ...
        {monitoringZones.map((zone) => (
          <div key={zone.zone} className="rounded-xl border border-border bg-card p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span
                className={`w-2 h-2 rounded-full ${zone.threatLevel === 'high' ? 'bg-red-400 animate-pulse' : zone.threatLevel === 'medium' ? 'bg-amber-400' : 'bg-emerald-400'}`}
              />
            </div>
              ...
              <div key={alert.id} className="p-4 hover:bg-muted/30 transition-colors">
                <div className="flex items-center justify-between mb-1 flex-wrap gap-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-xs font-mono ${alert.time === 'LIVE' ? 'text-red-400 animate-pulse' : 'text-muted-foreground'}`}
                    >
                      {alert.time}
                    </span>
                  </div>
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-sm flex items-center gap-2">
            <Target className="w-4 h-4 text-violet-400" /> MITRE ATT&CK Detection Coverage —
            Enterprise v14
          </h2>
          <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-violet-500/60 inline-block" /> Covered
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-emerald-500/60 inline-block" /> Detected in
              Wild
            </span>
          </div>
        </div>
