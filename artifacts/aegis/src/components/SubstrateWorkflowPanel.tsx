    ...
    try {
      const run = await apiFetch<Record<string, unknown>>("/control-tower/substrate/run", {
        ...
        body: JSON.stringify({
          ...
          input: { alertIds: ["aegis-alert-001", "aegis-alert-002"], minSeverity: "high", lookbackHours: 48 },
          mode,
        }),
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="text-xs font-semibold text-slate-100">Threat Triage and Escalation Routing</p>
              <p className="text-[10px] text-cyan-400/50 mt-0.5 font-mono">Substrate · aegis-threat-triage · Phase 2</p>
            </div>
          ...
          <p className="text-[10px] text-slate-400 mt-1.5 leading-relaxed">
            AI-assisted triage of threat signals across MITRE ATT&CK, CVE feeds, and alert clusters. Operator approval required before escalation.
          </p>

          {status !== "idle" && (
            <div className="mt-2 flex items-center gap-2">
              {status === "running" && <span className="text-[9px] font-mono text-cyan-400 animate-pulse">● RUNNING</span>}
        ...
        <div className="border-t border-cyan-500/10 pt-3 space-y-3">
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={m.label} className="rounded border border-slate-700 bg-slate-800/60 p-2">
                ...
                <p className={`text-sm font-mono font-bold truncate ${STATUS_COLOR[String(m.value)] ?? "text-cyan-300"}`}>{m.value}</p>
              </div>
            ))}
          </div>
          {result.stages.length > 0 && (
            <div className="space-y-1">
              ...
              {result.stages.map(s => (
                <div key={s.stageId} className="rounded border border-slate-700 bg-slate-800/40 px-2.5 py-1.5 flex items-center justify-between">
                  <div>
                  ...
                  <div className="text-right">
                    ...
                    {s.confidence !== undefined && (
                      <p className="text-[9px] font-mono text-cyan-400/60">{(s.confidence * 100).toFixed(0)}%</p>
                    )}
                  </div>
                </div>
              ))}
          ...
          <div className="flex items-center justify-between">
            <span className="text-[9px] font-mono text-slate-600">mode:{result.mode} · inbox:aegis-threat-triage · policy:aegis-threat-triage-policy</span>
          ...
          {mode === "dry-run" && (
            <div className="rounded border border-sky-500/15 bg-sky-500/5 p-2">
              <p className="text-[9px] font-mono text-sky-400">DRY-RUN — escalation and notification side effects suppressed.</p>
            </div>
          )}
