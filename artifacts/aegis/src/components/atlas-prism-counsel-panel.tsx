...
import { GitBranch, Activity, Scale, ChevronDown, ChevronRight, FileText, Lock, RefreshCw, AlertTriangle } from "lucide-react";
...
const ACCENT = { purple: "#8b7ac8", amber: "#c8953c", green: "#6b8f71", red: "#c45a4a", blue: "#4a90b8" };
  ...
  branches: [
    {
      ...
      outcomeProjections: [
        { label: "Settlement accepted at $4.2M", probability: 0.71, impact: "medium — avoids $8.4M trial exposure", metrics: { settlementUsd: 4200000, totalCostUsd: 4200000, trialRiskAvoided: true } },
        { label: "Counterparty rejects, trial proceeds", probability: 0.29, impact: "high — full trial, outcome uncertain", metrics: { settlementUsd: 0, totalCostUsd: 8400000, trialRiskAvoided: false } },
      ],
    },
    {
      branchId: "prism-discovery-completion",
      branchLabel: "Complete Discovery Then Negotiate",
      hypothesis: "Complete discovery in 34 days, strengthen position, then counter-propose at $3.8M.",
      deltaState: {
      ...
      outcomeProjections: [
        { label: "Improved position, settle at $3.8M", probability: 0.52, impact: "medium — saves $400K vs. current offer", metrics: { settlementUsd: 3800000, totalCostUsd: 4080000, trialRiskAvoided: true } },
        { label: "Counterparty hardens, trial proceeds", probability: 0.48, impact: "high — full trial exposure remains", metrics: { settlementUsd: 0, totalCostUsd: 8680000, trialRiskAvoided: false } },
      ],
    },
  ],
  resolutionRecommendations: [
    { title: "Accept $4.2M settlement within 5 business days", priority: "critical", rationale: "Client pressure score 78/100 and 34-day deadline create settlement urgency." },
    { title: "Demand privilege log from opposing counsel", priority: "high", rationale: "14 documents flagged for privilege review — potential smoking gun protection." },
    { title: "Prepare emergency motion for continuance", priority: "high", rationale: "Contingency if settlement collapses: buys 60-90 days at nominal cost." },
  ],
};

function DriftBar({ score }: { score: number }) {
  const color = score >= 0.75 ? ACCENT.red : score >= 0.5 ? ACCENT.amber : ACCENT.green;
  const label = score >= 0.75 ? "Critical Pressure" : score >= 0.5 ? "Elevated Pressure" : "Nominal";
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-[9px] font-mono uppercase tracking-widest" style={{ color: TEXT.tertiary }}>Pressure Drift</span>
...
function BranchCard({ branch, defaultOpen = false }: { branch: typeof PRISM_DEMO_SCENE.branches[0]; defaultOpen?: boolean }) {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] font-mono px-1.5 py-0.5 rounded" style={{ background: "rgba(139,122,200,0.12)", color: ACCENT.purple, border: "1px solid rgba(139,122,200,0.2)" }}>
            {scene.entityId}
          </span>
        </div>
        <div className="flex items-center gap-1.5">
      ...
      <div className="rounded-lg p-3 space-y-2" style={{ background: BG.card, border: `1px solid ${BORDER.muted}` }}>
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-1.5">
          <div className="col-span-2">
            <span className="text-[9px]" style={{ color: TEXT.tertiary }}>Matter</span>
            <p className="text-[11px] font-semibold" style={{ color: TEXT.primary }}>{scene.state.matterTitle}</p>
          </div>
          <div>
            <span className="text-[9px]" style={{ color: TEXT.tertiary }}>Total Exposure</span>
            <p className="text-[11px] font-bold" style={{ color: ACCENT.red }}>${(scene.state.totalExposureUsd / 1e6).toFixed(1)}M</p>
          </div>
          <div>
            <span className="text-[9px]" style={{ color: TEXT.tertiary }}>Settlement Offer</span>
            <p className="text-[11px] font-semibold" style={{ color: TEXT.primary }}>${(scene.state.settlementOfferUsd / 1e6).toFixed(1)}M</p>
          </div>
          <div>
            <span className="text-[9px]" style={{ color: TEXT.tertiary }}>Key Deadline</span>
            <p className="text-[11px] font-semibold" style={{ color: ACCENT.amber }}>{scene.state.keyDeadlineDays} days</p>
          </div>
          <div>
            <span className="text-[9px]" style={{ color: TEXT.tertiary }}>Client Pressure</span>
            <p className="text-[11px] font-semibold" style={{ color: scene.state.clientPressureScore >= 75 ? ACCENT.red : ACCENT.amber }}>{scene.state.clientPressureScore}/100</p>
          </div>
          <div>
            <span className="text-[9px]" style={{ color: TEXT.tertiary }}>Discovery</span>
            <p className="text-[11px] font-semibold capitalize" style={{ color: TEXT.primary }}>{scene.state.discoveryStatus}</p>
          </div>
          <div>
            <span className="text-[9px]" style={{ color: TEXT.tertiary }}>Type</span>
            <p className="text-[11px] font-semibold capitalize" style={{ color: TEXT.primary }}>{scene.state.matterType.replace("_", " ")}</p>
          </div>
        </div>
      </div>

      <div>
        <p className="text-[9px] uppercase tracking-wider font-semibold mb-2" style={{ color: TEXT.tertiary }}>Resolution Branches</p>
      ...
      <div className="rounded-lg p-3" style={{ background: BG.card, border: `1px solid ${BORDER.muted}` }}>
        <div className="flex items-center gap-1.5 mb-2.5">
          ...
          <p className="text-[9px] uppercase tracking-wider font-semibold" style={{ color: TEXT.tertiary }}>Resolution Recommendations</p>
        </div>
        <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-1.5 rounded px-2.5 py-1.5" style={{ background: "rgba(255,255,255,0.025)", border: `1px solid ${BORDER.muted}` }}>
        ...
        <p className="text-[9px]" style={{ color: TEXT.tertiary }}>Branch execution requires <span style={{ color: TEXT.secondary }}>lead_counsel</span> approval — governed via Alloy proof chain</p>
      </div>
    </div>
  );
}
