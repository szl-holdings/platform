      ...
      <div className="relative w-[80vw] h-[60vh] mt-[10vh]">
        ...
        {nodes.map((node, i) => (
          <motion.div
            ...
            transition={{ type: 'spring', stiffness: 300, damping: 20, delay: phase >= 3 ? i * 0.2 : 0 }}
          >
            ...
            <div className="bg-[var(--color-surface)] border border-[var(--color-border)] px-[1vw] py-[0.5vh] rounded-md backdrop-blur-md text-center shadow-xl">
              <div className="font-display text-[1.2vw] text-[var(--color-text-primary)] whitespace-nowrap">{node.name}</div>
              <div className="font-mono text-[0.6vw] text-[var(--color-text-muted)] mt-[0.2vh] whitespace-nowrap">{node.desc}</div>
            </div>
          </motion.div>
        ))}
      </div>
