...
The video walks through the SZL Holdings platform thesis in ~70 seconds: the governance problem, a tour of all 10 product surfaces, the Alloy Fabric decision architecture, APEX mobile command, and the "governed autonomy" closing brand moment.
...
| `cortex` | APEX Mobile | Cross-domain alert correlation on mobile command (the `cortex` scene slug is preserved as a stable URL identifier per the originality audit) |
