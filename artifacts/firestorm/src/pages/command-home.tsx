...
export default function CommandHome() {
  ...
  const { data: incidents = [] } = useQuery<FirestormIncident[]>({ queryKey: ["firestorm-incidents"], queryFn: () => api.incidents.list() });
  const { data: alerts = [] } = useQuery<FirestormAlert[]>({ queryKey: ["firestorm-alerts"], queryFn: () => api.alerts.list() });
