...
export default function AegisPricingPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: BG, color: "rgba(255,255,255,0.88)" }}>
      <ContactModal isOpen={contactOpen} onClose={closeContact} type="demo" app="aegis" title="Request Enterprise Access" subtitle="Vetted access only. Tell us about your organization." />

      <div className="border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <a href="/firestorm/" className="flex items-center gap-2.5">
          ...
          <a href="/firestorm/" className="text-[12px]" style={{ color: "rgba(255,255,255,0.4)" }}>← Back to Platform</a>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-20 text-center">
        ...
        <p className="text-[15px] max-w-xl mx-auto mb-10" style={{ color: "rgba(255,255,255,0.5)" }}>
          From security teams to government agencies. Every tier includes full SOC command capability.
        </p>
          ...
          {tiers.map((tier) => {
            ...
            return (
              <div
                ...
                style={{
                  background: tier.highlight ? `linear-gradient(135deg, rgba(239,68,68,0.10), rgba(239,68,68,0.05))` : "rgba(255,255,255,0.03)",
                  border: tier.highlight ? `1px solid ${ACCENT}35` : "1px solid rgba(255,255,255,0.07)",
                }}
              >
                {tier.highlight && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider" style={{ background: ACCENT, color: "#fff" }}>
                    Most Popular
                  </div>
                )}

                <div className="flex items-center gap-3 mb-4">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: tier.highlight ? `${ACCENT}20` : "rgba(255,255,255,0.06)" }}>
                    <Icon className="w-4 h-4" style={{ color: tier.highlight ? ACCENT : "rgba(255,255,255,0.5)" }} />
                  </div>
        ...
        <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-8 text-center max-w-3xl mx-auto">
          {[
            ...
            { label: "Air-gap ready", desc: "On-premises deployment available for classified environments." },
