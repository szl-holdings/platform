    ...
    <div className="min-h-screen bg-[#070A10] text-white p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-mono text-amber-400 uppercase tracking-widest">SEC/FINRA Compliance Command</span>
            </div>
            <h1 className="text-2xl font-bold text-white">Financial Regulatory Compliance</h1>
            <p className="text-slate-400 text-sm mt-1">Regulation Best Interest · Rule 17a-4 Archival · Supervision Workflow · Compliance Calendar</p>
          </div>
            ...
            {postureLoading ? (
              ...
              <>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    { label: "Overall Compliance Score", score: posture.overallRiskScore, icon: Shield, color: "amber" },
                    { label: "Reg BI Suitability", score: posture.regBiScore, icon: Scale, color: "blue" },
                    { label: "Rule 17a-4 Archival", score: posture.archivalScore, icon: Archive, color: "purple" },
                    { label: "Supervision Workflow", score: posture.supervisionScore, icon: Eye, color: "green" },
                  ].map((item) => (
                    <motion.div
                      key={item.label}
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    { label: "Open Supervision Items", value: posture.openSupervisionItems, icon: AlertTriangle, urgent: posture.openSupervisionItems > 0 },
                    { label: "Critical Violations", value: posture.criticalItems, icon: AlertCircle, urgent: posture.criticalItems > 0 },
                    { label: "Overdue Calendar Events", value: posture.overdueCalendarItems, icon: Clock, urgent: posture.overdueCalendarItems > 0 },
                    { label: "Pending Reg BI Reviews", value: posture.pendingSuitabilityReviews, icon: FileText, urgent: posture.pendingSuitabilityReviews > 0 },
                  ].map((item) => (
                    <div key={item.label} className={`rounded-xl border p-4 ${item.urgent ? "bg-red-950/20 border-red-500/20" : "bg-white/5 border-white/10"}`}>
                      <div className="flex items-center gap-2 mb-2">
                        <item.icon className={`w-4 h-4 ${item.urgent ? "text-red-400" : "text-slate-400"}`} />
                        <span className="text-xs text-slate-400">{item.label}</span>
                      </div>
                      <div className={`text-2xl font-bold ${item.urgent ? "text-red-400" : "text-white"}`}>{item.value}</div>
                    </div>
                  ))}
                </div>

                {supervision && supervision.items.filter(i => i.priority === "critical" || i.priority === "high").length > 0 && (
                  <div className="bg-red-950/20 border border-red-500/20 rounded-xl p-4">
                    <h3 className="text-sm font-semibold text-red-400 mb-3 flex items-center gap-2">
                      <AlertCircle className="w-4 h-4" /> Critical Supervision Items Requiring Immediate Action
                    </h3>
                    <div className="space-y-2">
                      {supervision.items.filter(i => i.priority === "critical" || i.priority === "high").slice(0, 3).map(item => (
                        <div key={item.id} className="flex items-start justify-between p-3 bg-black/20 rounded-lg">
                          <div>
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border mr-2 ${PRIORITY_COLORS[item.priority]}`}>{item.priority}</span>
                            <span className="text-sm text-white">{item.title}</span>
                            <p className="text-xs text-slate-400 mt-1">{item.assignedToName ?? "Unassigned"}</p>
                          </div>
                          <button
                ...
                {calendar && calendar.events.filter(e => e.status === "overdue" || new Date(e.dueAt) < new Date(Date.now() + 7 * 86400000)).length > 0 && (
                      ...
                      {calendar.events.slice(0, 3).map(event => (
                        <div key={event.id} className="flex items-center justify-between p-3 bg-black/20 rounded-lg">
                          <div>
                            ...
                            <p className="text-xs text-slate-400 mt-0.5">{event.regulatoryBody} · {EVENT_TYPE_LABELS[event.eventType] ?? event.eventType}</p>
                          </div>
                          <span className={`text-xs font-medium ${event.status === "overdue" ? "text-red-400" : "text-amber-400"}`}>
              ...
              <div className="space-y-3">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${PRIORITY_COLORS[item.priority]}`}>
                            {item.priority}
                          </span>
                          <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs ${STATUS_COLORS[item.status]}`}>
                            {item.status.replace("_", " ")}
                          </span>
                          {item.riskScore !== undefined && (
                            <span className="text-xs text-slate-400">Risk: <span className={item.riskScore >= 80 ? "text-red-400 font-bold" : "text-white"}>{item.riskScore}</span>/100</span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-4 mt-2 text-xs text-slate-500">
                          ...
                          {item.dueAt && <span className={new Date(item.dueAt) < new Date() ? "text-red-400" : ""}>{formatDue(item.dueAt)}</span>}
                        </div>
                      </div>
                      <div className="flex flex-col gap-2 shrink-0">
                        {item.status === "open" && (
                          <>
                            <button
                              onClick={() => actionMutation.mutate({ itemId: item.id, action: "assign" })}
                              className="px-3 py-1.5 text-xs bg-blue-500/20 text-blue-400 rounded-lg hover:bg-blue-500/30 transition-colors"
                            >
                              Take Review
                            </button>
                            <button
                              onClick={() => actionMutation.mutate({ itemId: item.id, action: "escalate", notes: "Escalated via compliance dashboard" })}
                        ...
                        {item.status === "in_review" && (
                          <button
                            onClick={() => actionMutation.mutate({ itemId: item.id, action: "resolve", notes: "Resolved after review" })}
                ...
                {(!supervision?.items || supervision.items.length === 0) && (
                  <div className="flex items-center justify-center h-48 text-slate-400 border border-white/10 rounded-xl">
                    <CheckCircle2 className="w-5 h-5 text-green-400 mr-2" /> No open supervision items
                  </div>
                )}
              </div>
            )}
        ...
        {activeTab === "calendar" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-slate-400">Form ADV · Form CRS · SEC/FINRA Exams · Annual Reviews</span>
            </div>
            {calendarLoading ? (
              <div className="flex items-center justify-center h-48 text-slate-400">
              ...
              <div className="space-y-3">
                {(calendar?.events ?? []).map(event => (
                  <div key={event.id} className={`border-l-4 rounded-r-xl p-4 bg-white/5 border border-white/10 ${event.status === "overdue" ? "border-l-red-500 bg-red-950/10" : event.status === "in_progress" ? "border-l-amber-500 bg-amber-950/10" : "border-l-blue-500"}`}>
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-xs font-mono text-slate-400 uppercase">{EVENT_TYPE_LABELS[event.eventType] ?? event.eventType}</span>
                          {event.regulatoryBody && <span className="text-xs text-amber-400">· {event.regulatoryBody}</span>}
                        </div>
                        <h3 className="text-sm font-semibold text-white">{event.title}</h3>
                        {event.description && <p className="text-xs text-slate-400 mt-1">{event.description}</p>}
                        {event.assignedToName && <p className="text-xs text-slate-500 mt-1">Assigned: {event.assignedToName}</p>}
                      </div>
                      <div className="text-right shrink-0">
                        <div className={`text-sm font-semibold ${event.status === "overdue" ? "text-red-400" : event.status === "in_progress" ? "text-amber-400" : "text-slate-300"}`}>
                          {formatDue(event.dueAt)}
                        </div>
                        <div className="text-xs text-slate-500">{new Date(event.dueAt).toLocaleDateString()}</div>
                        <span className={`mt-1 inline-block px-2 py-0.5 rounded text-xs ${event.status === "overdue" ? "bg-red-500/20 text-red-400" : event.status === "in_progress" ? "bg-amber-500/20 text-amber-400" : event.status === "completed" ? "bg-green-500/20 text-green-400" : "bg-blue-500/20 text-blue-400"}`}>
        ...
        {activeTab === "archival" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-white">SEC Rule 17a-4 Communication Archival</h2>
            </div>
            <div className="bg-amber-950/20 border border-amber-500/20 rounded-xl p-4 text-sm">
              ...
              <p className="text-slate-400">All communications are stored with SHA-256 hash chains per SEC Rule 17a-4(f). Records are retained for 3 years (general) or 6 years (account-level). Content is immutable once archived.</p>
            </div>
            <ArchivalPanel />
          </div>
        )}
        ...
        {activeTab === "fusion" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-400">Market conditions × CRM pipeline × Compliance posture</span>
            </div>
            {fusionLoading ? (
              <div className="flex items-center justify-center h-48 text-slate-400">
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-xs font-mono uppercase ${insight.severity === "high" ? "text-red-400" : insight.severity === "medium" ? "text-amber-400" : "text-blue-400"}`}>
                            {insight.type.replace("_", " ")}
                          </span>
                          {insight.crmAccountName && (
                            <span className="text-xs text-slate-400">· {insight.crmAccountName}</span>
                          )}
                        </div>
                        ...
                        {insight.marketData && (
                          <div className="mt-2 flex flex-wrap gap-2">
                            {Object.entries(insight.marketData).map(([k, v]) => (
                              <span key={k} className="text-xs bg-white/10 px-2 py-0.5 rounded font-mono text-slate-300">
                                {k}: {String(v)}
                              </span>
                            ))}
                          </div>
...
function ArchivalPanel() {
  ...
  if (isLoading) return <div className="flex items-center justify-center h-32 text-slate-400"><RefreshCw className="w-4 h-4 animate-spin mr-2" /> Loading archival records...</div>;
      ...
      {items.map(item => (
        <div key={item.entryId} className="bg-white/5 border border-white/10 rounded-xl p-4">
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-mono text-purple-400 uppercase">{item.communicationType.replace("_", " ")}</span>
                <span className="text-xs text-slate-500">· Archived {new Date(item.archivedAt).toLocaleDateString()}</span>
              </div>
              {item.subject && <h3 className="text-sm text-white">{item.subject}</h3>}
              <div className="flex items-center gap-2 mt-1">
                <Archive className="w-3 h-3 text-slate-500" />
                <span className="text-xs font-mono text-slate-500 truncate max-w-xs">{item.contentHash.slice(0, 20)}...</span>
              </div>
              <p className="text-xs text-slate-500 mt-1">Retention expires: {new Date(item.retentionExpiresAt).toLocaleDateString()}</p>
            </div>
