...
const priorityConfig: Record<string, { label: string; color: string; dot: string }> = {
  p1_critical: { label: "P1 Critical", color: "bg-red-500/15 text-red-400 border-red-500/30", dot: "bg-red-400 animate-pulse" },
  p2_high: { label: "P2 High", color: "bg-orange-500/15 text-orange-400 border-orange-500/30", dot: "bg-orange-400" },
  p3_medium: { label: "P3 Medium", color: "bg-amber-500/15 text-amber-400 border-amber-500/30", dot: "bg-amber-400" },
  p4_low: { label: "P4 Low", color: "bg-blue-500/15 text-blue-400 border-blue-500/30", dot: "bg-blue-400" },
};

const statusConfig: Record<string, { label: string; color: string; icon: any }> = {
  open: { label: "Open", color: "bg-red-500/10 text-red-400 border-red-500/20", icon: AlertTriangle },
  in_progress: { label: "In Progress", color: "bg-amber-500/10 text-amber-400 border-amber-500/20", icon: Activity },
  pending_review: { label: "Pending Review", color: "bg-blue-500/10 text-blue-400 border-blue-500/20", icon: Clock },
...
function SLAIndicator({ case: c }: { case: Case }) {
  ...
  const resolvePct = c.resolvedAt || c.status === "closed" ? 100 : Math.min((elapsedMinutes / c.slaResolve) * 100, 100);

  return (
    <div className="space-y-2">
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-[9px] font-mono text-muted-foreground uppercase tracking-wider">Triage SLA</span>
          <span className={cn("text-[9px] font-mono", c.triagedAt ? "text-emerald-400" : triageBreached ? "text-red-400" : "text-amber-400")}>
            {c.triagedAt ? "Met" : triageBreached ? `Breached +${Math.round(elapsedMinutes - c.slaTriage)}m` : `${Math.round(c.slaTriage - elapsedMinutes)}m left`}
          </span>
        </div>
        <div className="h-1.5 bg-border rounded-full overflow-hidden">
          <div
            className={cn("h-full rounded-full transition-all", c.triagedAt ? "bg-emerald-400" : triageBreached ? "bg-red-400" : "bg-amber-400")}
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-[9px] font-mono text-muted-foreground uppercase tracking-wider">Resolve SLA</span>
          <span className={cn("text-[9px] font-mono", (c.resolvedAt || c.status === "closed") ? "text-emerald-400" : resolveBreached ? "text-red-400" : "text-amber-400")}>
            {(c.resolvedAt || c.status === "closed") ? "Met" : resolveBreached ? `Breached +${Math.round(elapsedMinutes - c.slaResolve)}m` : `${Math.round(c.slaResolve - elapsedMinutes)}m left`}
          </span>
        </div>
        <div className="h-1.5 bg-border rounded-full overflow-hidden">
          <div
            className={cn("h-full rounded-full transition-all", (c.resolvedAt || c.status === "closed") ? "bg-emerald-400" : resolveBreached ? "bg-red-400" : "bg-amber-400")}
...
function CaseDetailPanel({ caseItem, onClose, onUpdate }: { caseItem: Case; onClose: () => void; onUpdate: () => void }) {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Case updated"); qc.invalidateQueries({ queryKey: ["firestorm-cases"] }); onUpdate(); },
      ...
      >
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[10px] font-mono text-muted-foreground/60">{caseItem.caseNumber}</span>
                <Badge variant="outline" className={cn("text-[9px]", pc.color)}>{pc.label}</Badge>
                <Badge variant="outline" className={cn("text-[9px]", sc.color)}>
                  <StatusIcon className="w-2.5 h-2.5 mr-1" />{sc.label}
                </Badge>
              </div>
              <h2 className="font-display text-base font-bold text-foreground leading-snug">{caseItem.title}</h2>
            </div>
            <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-white/10 text-muted-foreground hover:text-foreground transition-colors shrink-0">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-white/5 rounded-xl p-4 space-y-3">
            <div className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider">SLA Status</div>
          ...
          {(caseItem.relatedIncidentIds?.length ?? 0) > 0 && (
            <div>
              <div className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider mb-2">Linked Incidents</div>
              <div className="flex flex-wrap gap-1.5">
                {caseItem.relatedIncidentIds?.map((id) => (
                  <Badge key={id} variant="outline" className="text-[9px] bg-red-500/5 text-red-400/70 border-red-500/20">INC-{id}</Badge>
                ))}
              </div>
            </div>
          )}
          ...
          <div>
            <div className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider mb-2">Update Status</div>
            <div className="flex gap-2">
              ...
              <Button size="sm" variant="outline" onClick={handleStatusUpdate} disabled={newStatus === caseItem.status || updateMutation.isPending}>
                Update
              </Button>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2 mb-3">
              {(caseItem.notes ?? []).map((note, i) => (
                <div key={i} className="bg-white/5 rounded-lg p-3">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[10px] font-mono text-muted-foreground/70">{note.author}</span>
                    <span className="text-[9px] text-muted-foreground/50">{new Date(note.at).toLocaleString()}</span>
                  </div>
            ...
            <div className="flex gap-2">
              ...
              />
              <Button size="sm" variant="outline" onClick={handleAddNote} disabled={!noteText.trim() || updateMutation.isPending}>
                Add
              </Button>
            </div>
          </div>
...
export default function CasesPage() {
  ...
  const stats = useMemo(() => ({
    ...
    p1: cases.filter((c) => c.priority === "p1_critical" && !["resolved", "closed"].includes(c.status)).length,
  ...
  const handleCaseRefresh = () => {
    qc.invalidateQueries({ queryKey: ["firestorm-cases"] });
    if (selectedCase) {
  ...
  return (
    <div className="p-5 space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[9px] font-mono text-red-400/50 uppercase tracking-widest">Firestorm / Cases</span>
            <span className="text-red-500/20">·</span>
            <span className="text-[9px] font-mono text-muted-foreground/40 uppercase tracking-widest">Case Management</span>
          </div>
          <h1 className="font-display text-xl font-bold flex items-center gap-2.5">
            <Briefcase className="w-5 h-5 text-red-400" />
            Case Management
          </h1>
          <p className="text-[11px] text-muted-foreground mt-1">Security cases with SLA tracking, analyst assignment, and evidence chain-of-custody</p>
        </div>
        <Button size="sm" variant="outline" className="border-red-500/20 text-red-400 hover:bg-red-500/10 hover:text-red-300 shrink-0">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {[
          ...
          { label: "SLA Breached", value: stats.slaBreached, color: stats.slaBreached > 0 ? "text-red-400" : "text-emerald-400" },
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-36 h-8 text-xs bg-background border-border">
            <Filter className="w-3 h-3 mr-1.5" /><SelectValue placeholder="Status" />
          </SelectTrigger>
          <SelectContent>
        ...
        <Select value={priorityFilter} onValueChange={setPriorityFilter}>
          <SelectTrigger className="w-36 h-8 text-xs bg-background border-border">
            <Filter className="w-3 h-3 mr-1.5" /><SelectValue placeholder="Priority" />
          </SelectTrigger>
          <SelectContent>
        ...
        <div className="divide-y divide-border/50">
          {isLoading ? (
            ...
            filtered.map((c) => {
              ...
              return (
                <div key={c.id} className={cn("transition-colors", slaBreached && "border-l-2 border-red-500")}>
                  <div
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                          <span className="text-[10px] font-mono text-muted-foreground/70">{c.caseNumber}</span>
                          <Badge variant="outline" className={cn("text-[9px]", pc.color)}>{pc.label}</Badge>
                          <Badge variant="outline" className={cn("text-[9px]", sc.color)}>
                            <StatusIcon className="w-2.5 h-2.5 mr-1" />{sc.label}
                          </Badge>
                          {slaBreached && (
                            <Badge variant="outline" className="text-[9px] bg-red-500/15 text-red-400 border-red-500/30 animate-pulse">SLA Breached</Badge>
                          )}
                        </div>
                    ...
                    <div className="px-5 pb-4 border-t border-border/40 bg-muted/5">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3">
                        <div className="space-y-2">
                          ...
                          <div className="flex gap-2">
                            <Button size="sm" variant="outline" className="text-[10px] h-7" onClick={() => setSelectedCase(c)}>
