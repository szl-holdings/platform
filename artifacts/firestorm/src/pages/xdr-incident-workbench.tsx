...
const SEVERITY_CONFIG = {
  ...
  medium: { label: "Medium", color: "#eab308", bg: "rgba(234,179,8,0.1)", border: "rgba(234,179,8,0.25)" },
...
const XDR_INCIDENTS = [
  {
    ...
    confidence: 94,
    description: "Suspected APT41 actor deployed credential harvesting tooling on WS-PROD-012, followed by lateral movement to DC-EAST-01. LSASS memory dump detected, Kerberoasting activity observed across 6 service accounts.",
    timeline: [
      { time: "14:22", event: "Initial detection — LSASS dump on WS-PROD-012", type: "detection" },
      { time: "14:24", event: "Lateral movement to DC-EAST-01 via Pass-the-Hash", type: "activity" },
    ...
    linkedAlerts: 23,
    notes: [
      { author: "M. Chen", time: "14:40", content: "Confirmed LSASS dump via ProcDump. Hash extracted. Pivoting to check AD for golden ticket artifacts." },
      { author: "K. Singh", time: "14:43", content: "AD logs show unusual LDAP query volume from DC-EAST-01. Possible domain recon." },
    ],
  },
  {
    id: "INC-2039",
    ...
    confidence: 98,
    description: "Ransomware staging behavior detected across file server cluster. Volume shadow copies deleted, encrypted volume mounted, batch file execution observed. Containment applied to 12 endpoints.",
    timeline: [
      ...
      { time: "12:14", event: "Lateral spread to 11 additional endpoints detected", type: "activity" },
      { time: "12:19", event: "Mass containment applied across cluster", type: "response" },
      { time: "12:35", event: "Ransomware family identified: BlackCat/ALPHV variant", type: "intel" },
    ],
    entities: [
    ...
    linkedAlerts: 47,
    notes: [
      { author: "K. Singh", time: "12:21", content: "ALPHV variant confirmed. Using intermittent encryption pattern — some files skipped. Recovery feasibility high." },
    ],
  },
  {
    id: "INC-2038",
    ...
    confidence: 81,
    description: "Malicious npm package detected in CI/CD pipeline. Package `react-utils-pro` (v3.2.1) contains obfuscated payload executing on build server. Potential secrets exfiltration via DNS tunnel.",
    timeline: [
  ...
  },
  {
    ...
    confidence: 89,
    description: "Business email compromise attempt targeting CFO. Attacker spoofed CEO email (similar domain typo) and requested $240K wire transfer. Finance team flagged anomaly before execution.",
    timeline: [],
    entities: [
  ...
  },
  {
    ...
    confidence: 76,
    description: "Service account assumed overprivileged IAM role in AWS production environment. CloudTrail shows unauthorized DescribeInstances and ListBuckets calls across 3 regions.",
    timeline: [],
    entities: [
      { type: "cloud", name: "aws-prod-west", risk: "medium", action: "remediated" },
    ],
    linkedAlerts: 6,
    notes: [],
  },
...
export default function XDRIncidentWorkbench() {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden" style={{ background: "var(--color-aegis-surface, #060e1a)" }}>
      {/* Header */}
      <div className="px-6 py-4 border-b flex items-center justify-between shrink-0" style={{ borderColor: "rgba(239,68,68,0.1)" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-white/30">Incident-centric analyst workspace — cross-domain entity linking</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" className="h-7 text-[11px] gap-1.5" style={{ background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.3)", color: "#ef4444" }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-[300px] shrink-0 border-r flex flex-col overflow-hidden" style={{ borderColor: "rgba(239,68,68,0.08)" }}>
          {/* Filters */}
          <div className="p-3 space-y-2 border-b shrink-0" style={{ borderColor: "rgba(239,68,68,0.08)" }}>
            <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border" style={{ borderColor: "rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.03)" }}>
            ...
            <div className="flex items-center gap-1.5">
              {["all", "critical", "high", "medium"].map(s => (
                <button
                  ...
                  style={{
                    borderColor: filterSeverity === s && s !== "all" ? (SEVERITY_CONFIG[s as keyof typeof SEVERITY_CONFIG]?.border ?? "rgba(255,255,255,0.15)") : "rgba(255,255,255,0.08)",
                    color: filterSeverity === s ? (s === "all" ? "#fff" : SEVERITY_CONFIG[s as keyof typeof SEVERITY_CONFIG]?.color) : "rgba(255,255,255,0.3)",
                    background: filterSeverity === s ? (s === "all" ? "rgba(255,255,255,0.06)" : SEVERITY_CONFIG[s as keyof typeof SEVERITY_CONFIG]?.bg) : "transparent",
                  }}
                >
            ...
            {filtered.map(inc => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[11px] text-white/80 font-medium leading-tight mb-2 line-clamp-2">{inc.title}</p>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-5 space-y-5">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[9px] border" style={{ color: stat.color, borderColor: `${stat.color}30`, background: `${stat.color}10` }}>
                    {stat.label}
                  </span>
                  <span className="text-[9px] text-white/30">Confidence: <span style={{ color: sev.color }}>{selected.confidence}%</span></span>
                </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <Button size="sm" className="h-7 text-[11px] gap-1.5" style={{ background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.2)", color: "#22c55e" }}>
            ...
            <div className="grid grid-cols-5 gap-3">
              {[
                { label: "Linked Alerts", value: selected.alertCount, icon: Bell, color: "#f97316" },
                { label: "Affected Assets", value: selected.affectedAssets, icon: Server, color: "#a78bfa" },
                { label: "Domains", value: selected.domains.length, icon: Network, color: "#60a5fa" },
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  {selected.tactics.map(t => (
                    <span key={t} className="px-2 py-0.5 rounded text-[9px] font-medium border" style={{ color: "#a78bfa", borderColor: "rgba(167,139,250,0.2)", background: "rgba(167,139,250,0.08)" }}>
                      {t}
                    </span>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  {selected.domains.map(d => (
                    <span key={d} className="px-2 py-0.5 rounded text-[9px] font-medium border" style={{ color: "#60a5fa", borderColor: "rgba(96,165,250,0.2)", background: "rgba(96,165,250,0.08)" }}>
                      {DOMAIN_LABELS[d] ?? d}
                    </span>
                  ))}
                </div>
            ...
            {selected.entities.length > 0 && (
              <div className="rounded-xl border p-4" style={{ borderColor: "rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.01)" }}>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-xs font-bold text-white flex items-center gap-2">
                    <Network className="w-3.5 h-3.5 text-blue-400" /> Entity Graph — Linked Assets
                  </h3>
                  <span className="text-[9px] text-white/30">{selected.entities.length} entities</span>
                </div>
                <div className="flex items-center gap-3 flex-wrap">
                  {selected.entities.map((entity, idx) => {
                    const Icon = ENTITY_ICONS[entity.type as keyof typeof ENTITY_ICONS] ?? Server;
                    const color = ENTITY_COLORS[entity.type as keyof typeof ENTITY_COLORS] ?? "#fff";
                    const actionBadge = ACTION_BADGES[entity.action] ?? { label: entity.action, color: "#64748b" };
                    const riskSev = SEVERITY_CONFIG[entity.risk as keyof typeof SEVERITY_CONFIG];
                    return (
                      <div key={idx} className="flex flex-col items-center gap-1.5">
                        <div className="relative">
                          <div className="w-12 h-12 rounded-xl flex items-center justify-center border" style={{ borderColor: `${color}25`, background: `${color}10` }}>
                            <Icon className="w-5 h-5" style={{ color }} />
                          </div>
                          <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full border border-[#060e1a]" style={{ background: riskSev?.color ?? "#64748b" }} />
                        </div>
                        <div className="text-center">
                          <p className="text-[9px] font-mono text-white/70">{entity.name}</p>
                          <p className="text-[8px]" style={{ color: actionBadge.color }}>{actionBadge.label}</p>
                        </div>
                        {idx < selected.entities.length - 1 && (
                          <div className="hidden md:block absolute translate-x-16 w-6 h-px" style={{ background: "rgba(255,255,255,0.1)" }} />
                        )}
                      </div>
                    );
                  })}
                ...
                <div className="rounded-xl border p-4" style={{ borderColor: "rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.01)" }}>
                  ...
                  <div className="space-y-3">
                    {selected.timeline.map((event, idx) => {
                      const Icon = TIMELINE_ICONS[event.type as keyof typeof TIMELINE_ICONS] ?? Activity;
                      const color = TIMELINE_COLORS[event.type as keyof typeof TIMELINE_COLORS] ?? "#fff";
                      return (
                        <div key={idx} className="flex items-start gap-3">
                          <div className="flex flex-col items-center">
                            ...
                            {idx < selected.timeline.length - 1 && (
                              <div className="w-px h-4 mt-1" style={{ background: "rgba(255,255,255,0.06)" }} />
                            )}
                          </div>
                          <div className="flex-1 min-w-0 pb-1">
                            <span className="text-[9px] font-mono text-white/30 mr-2">{event.time}</span>
                            <span className="text-[10px] text-white/60 leading-tight">{event.event}</span>
                          </div>
                        </div>
                      );
                    })}
              ...
              <div className="rounded-xl border p-4" style={{ borderColor: "rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.01)" }}>
                ...
                <div className="space-y-3 mb-3">
                  {selected.notes.length === 0 && (
                    <p className="text-[10px] text-white/20">No notes yet — add your investigation findings below.</p>
                  )}
                  {selected.notes.map((note, idx) => (
                    <div key={idx} className="rounded-lg p-3 border" style={{ borderColor: "rgba(255,255,255,0.05)", background: "rgba(255,255,255,0.02)" }}>
                      <div className="flex items-center gap-2 mb-1">
                        <div className="w-4 h-4 rounded-full bg-blue-500/20 flex items-center justify-center">
                          <span className="text-[7px] text-blue-400 font-bold">{note.author[0]}</span>
                        </div>
                        <span className="text-[9px] text-white/50 font-medium">{note.author}</span>
                        <span className="text-[8px] text-white/20">{note.time}</span>
                      </div>
                      <p className="text-[10px] text-white/60 leading-relaxed">{note.content}</p>
                    </div>
                  ))}
                </div>
            ...
            <div className="rounded-xl border p-4" style={{ borderColor: "rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.01)" }}>
              <h3 className="text-xs font-bold text-white mb-3 flex items-center gap-2">
                <Zap className="w-3.5 h-3.5 text-amber-400" /> Unified Response Console — One-Click Actions
              </h3>
              <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
                {[
                  ...
                  >
                    ...
                    <span className="text-[9px] font-medium text-center leading-tight" style={{ color: `${color}cc` }}>{label}</span>
                  </button>
                ))}
              </div>
            </div>
