...
import { Bug, AlertTriangle, CheckCircle, Clock, User, TrendingUp, Filter, RefreshCw, Shield, Zap, ChevronDown, ChevronRight, BarChart3, Activity } from "lucide-react";
...
function EPSSBadge({ score }: { score: number }) {
  const color = score > 0.8 ? "#ef4444" : score > 0.5 ? "#f97316" : score > 0.2 ? "#f59e0b" : "#10b981";
  return (
    <div className="flex items-center gap-1">
      ...
      <span className="text-[10px]" style={{ color }}>{(score * 100).toFixed(0)}%</span>
    </div>
  );
}
...
export default function VulnLifecycle() {
  ...
  const filtered = filter === "all" ? VULNS : filter === "kev" ? VULNS.filter(v => v.kev) : filter === "exploited" ? VULNS.filter(v => v.activelyExploited) : VULNS.filter(v => v.status === filter);
    ...
    <div className="p-6 space-y-6 max-w-full">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-zinc-500">Risk-based prioritization using EPSS, asset criticality, and KEV intelligence. End-to-end remediation tracking from discovery to verification.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Critical Open", value: criticalOpen, sub: "requiring immediate action", color: "#ef4444", icon: AlertTriangle },
          { label: "KEV Exposures", value: kevCount, sub: "CISA known exploited vulns", color: "#ef4444", icon: Zap },
          { label: "In Remediation", value: VULNS.filter(v => v.status === "in-remediation").length, sub: "actively being patched", color: "#f59e0b", icon: RefreshCw },
          { label: "Avg Risk Score", value: avgRiskScore, sub: "composite EPSS + criticality", color: "#8b5cf6", icon: BarChart3 },
        ].map(m => {
          const Icon = m.icon;
          return (
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {[
          ...
          <button
            ...
            className={cn("px-2.5 py-1 rounded-lg text-[11px] border transition-colors", filter === f.id ? "bg-orange-500/15 border-orange-500/30 text-orange-400" : "border-white/8 bg-white/3 text-zinc-400 hover:text-zinc-300")}
          >
            {f.label}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
          ...
          <div className="space-y-1.5">
            {filtered.map(vuln => (
              <button
                ...
                className={cn("w-full rounded-xl border p-3 text-left transition-all", selectedVuln?.id === vuln.id ? "border-orange-500/30 bg-orange-500/5" : "border-white/8 bg-white/3 hover:bg-white/5")}
              >
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      {vuln.kev && <span className="text-[10px] px-1 py-0.5 rounded bg-red-500/15 border border-red-500/30 text-red-400">KEV</span>}
                      {vuln.activelyExploited && <span className="text-[10px] px-1 py-0.5 rounded bg-orange-500/15 border border-orange-500/30 text-orange-400">⚡ Active</span>}
                    </div>
                    <div className="flex items-center gap-3 text-[10px] text-zinc-500 flex-wrap">
                      <span>{vuln.asset}</span>
                      <span>·</span>
                      <span>CVSS: <span className="text-white">{vuln.cvss}</span></span>
                      <span>EPSS:</span>
                  ...
                  <div className="flex flex-col items-end gap-1.5 shrink-0">
                    <span className={cn("text-[10px] px-1.5 py-0.5 rounded border", severityColor[vuln.severity])}>{vuln.severity}</span>
                    <span className={cn("text-[10px] px-1.5 py-0.5 rounded border", statusConfig[vuln.status])}>{vuln.status}</span>
                  </div>
                </div>
                <div className="mt-2">
                  ...
                  <div className="h-1 rounded-full bg-white/8">
                    <div className="h-full rounded-full transition-all" style={{ width: `${vuln.riskScore}%`, background: vuln.riskScore > 80 ? "#ef4444" : vuln.riskScore > 60 ? "#f97316" : "#f59e0b" }} />
                  </div>
                </div>
              </button>
            ))}
        ...
        {selectedVuln && (
          <div>
            <h2 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-3">Vuln Detail</h2>
            <div className="rounded-xl border border-orange-500/20 bg-white/2 p-4 space-y-4">
              <div>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {selectedVuln.kev && <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-500/15 border border-red-500/30 text-red-400">CISA KEV</span>}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "CVSS Score", value: selectedVuln.cvss.toString(), color: selectedVuln.cvss >= 9 ? "#ef4444" : "#f97316" },
                  ...
                  <div key={item.label}>
                    <div className="text-[10px] text-zinc-500 uppercase tracking-wide mb-0.5">{item.label}</div>
                    <div className="text-xs text-white capitalize" style={item.color ? { color: item.color } : {}}>{item.value}</div>
                  </div>
                ))}
              </div>
              <div>
                <div className="text-[10px] text-zinc-500 uppercase tracking-wide mb-1">EPSS Score (Exploit Prediction)</div>
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-2 rounded-full bg-white/8">
                    <div className="h-full rounded-full" style={{ width: `${selectedVuln.epss * 100}%`, background: selectedVuln.epss > 0.8 ? "#ef4444" : "#f97316" }} />
                  </div>
                  <span className="text-xs font-bold" style={{ color: selectedVuln.epss > 0.8 ? "#ef4444" : "#f97316" }}>{(selectedVuln.epss * 100).toFixed(1)}%</span>
                </div>
                <div className="text-[10px] text-zinc-500 mt-0.5">Probability of exploitation in next 30 days</div>
              </div>
              <div>
                <div className="text-[10px] text-zinc-500 uppercase tracking-wide mb-1">Composite Risk Score</div>
                <div className="text-2xl font-bold" style={{ color: selectedVuln.riskScore > 80 ? "#ef4444" : "#f97316" }}>{selectedVuln.riskScore}<span className="text-sm text-zinc-500 font-normal">/100</span></div>
                <div className="text-[10px] text-zinc-500">CVSS + EPSS + Asset Criticality + Exploit Activity</div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => toast.success("Remediation task created and assigned")} className="flex-1 py-1.5 rounded-lg bg-orange-500/10 border border-orange-500/20 text-orange-400 text-xs hover:bg-orange-500/20 transition-colors">Assign Remediation</button>
                <button onClick={() => toast.success("Vulnerability accepted with risk acknowledgment")} className="flex-1 py-1.5 rounded-lg bg-white/5 border border-white/10 text-zinc-300 text-xs hover:bg-white/8 transition-colors">Accept Risk</button>
              </div>
            </div>
          </div>
        )}
