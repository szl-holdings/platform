...
const BG = { page: "#0A0D14", surface: "#0f1320", elevated: "#131825" };
      ...
      <div style={{ marginBottom: "24px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "4px" }}>
          ...
          <span style={{
            ...
            background: `${AEGIS_ACCENT}15`, color: AEGIS_ACCENT, border: `1px solid ${AEGIS_ACCENT}30`,
            borderRadius: "4px", padding: "2px 8px",
          }}>Aegis</span>
        </div>
        <p style={{ fontSize: "13px", color: TEXT.secondary, margin: 0 }}>
          AI-powered threat document analysis, entity extraction, and automated defense actions.
        </p>
      </div>
        ...
        {(["actions", "document"] as const).map(tab => (
          <button
            ...
            style={{
              ...
              background: activeTab === tab ? `${AEGIS_ACCENT}20` : "transparent",
              border: `1px solid ${activeTab === tab ? `${AEGIS_ACCENT}40` : "transparent"}`,
              color: activeTab === tab ? AEGIS_ACCENT : TEXT.muted,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "16px" }}>
        {activeTab === "actions" ? (
          <div style={{ position: "relative" }}>
            <AIActionsPanel
              domain="defense"
              actions={DOMAIN_ACTIONS.defense ?? []}
        ...
        <div style={{
          ...
          {[
            { name: "Threat Brief Generator", status: "active", runs: 12, color: "#6b8f71" },
            { name: "IOC Auto-Extraction", status: "active", runs: 48, color: "#6b8f71" },
            { name: "MITRE ATT&CK Mapper", status: "active", runs: 7, color: "#6b8f71" },
            { name: "Board Brief Distributor", status: "paused", runs: 2, color: "#d4a054" },
          ].map((wf, i) => (
            <div key={i} style={{
          ...
          <div style={{ marginTop: "16px", padding: "12px", background: `${AEGIS_ACCENT}08`, border: `1px solid ${AEGIS_ACCENT}15`, borderRadius: "8px" }}>
            <div style={{ fontSize: "10px", color: TEXT.muted, marginBottom: "4px" }}>Try in command palette (⌘K):</div>
            <div style={{ fontSize: "11px", color: AEGIS_ACCENT, fontStyle: "italic" }}>
              "Generate a threat brief for Q2 adversary activity"
            </div>
          </div>
        </div>
      </div>
