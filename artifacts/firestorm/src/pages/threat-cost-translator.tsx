...
export default function ThreatCostTranslator() {
  ...
  const totalExposure = useMemo(() => DEMO_INCIDENTS.reduce((sum, i) => sum + i.totalExposure, 0), []);
  const avgReputationDamage = useMemo(() => Math.round(DEMO_INCIDENTS.reduce((s, i) => s + i.reputationDamageScore, 0) / DEMO_INCIDENTS.length), []);

  const selectedIncident = DEMO_INCIDENTS.find((i) => i.id === selected);

  const ambientSignals: AmbientSignal[] = [
    { id: "sig-1", domain: "firestorm", title: "APT-41 Activity Spike", summary: "Threat actor APT-41 activity spike detected across 3 subsidiaries", severity: "high", score: 0.92, timestamp: Date.now() },
    { id: "sig-2", domain: "lyte", title: "Self-Healing Active", summary: "94% of P1 incidents resolved without human intervention", severity: "info", score: 0.38, timestamp: Date.now() },
  ];
  const energyMetrics: EnergyMetrics = { apiCallsPerMinute: 127, wsMessagesPerMinute: 340, chartRendersPerMinute: 24, dataRefreshesPerMinute: 18, activeSubscriptions: 42, deferredUpdates: 3, totalBudget: 120, usedBudget: 78 };
  ...
  return (
    <div className="min-h-screen bg-[#060810] text-white p-6 space-y-6">
      <AmbientBar signals={ambientSignals} appDomain="firestorm" accentColor="#ef4444" compact />
      <div>
        <h1 className="text-2xl font-bold text-white/90">Threat Cost Translator</h1>
        <p className="text-sm text-white/40 mt-1">Every security incident translated into financial impact — real-time breach cost modeling</p>
      </div>

      <div className="grid grid-cols-4 gap-4">
        {[
          { label: "Total Financial Exposure", value: formatDollars(totalExposure), color: "#ef4444" },
          ...
          { label: "Insurance Premium Impact", value: formatDollars(DEMO_INCIDENTS.reduce((s, i) => s + i.insurancePremiumImpact, 0)), color: "#3b82f6" },
        ].map((kpi) => (
          ...
          {DEMO_INCIDENTS.map((incident) => (
            <div
              ...
              className={cn(
                ...
                selected === incident.id ? "bg-white/[0.06] border-white/15" : "bg-white/[0.02] border-white/5 hover:border-white/10",
              )}
              onClick={() => setSelected(selected === incident.id ? null : incident.id)}
            >
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-3">
                  <div className="w-2 h-2 rounded-full mt-1.5" style={{ background: SEV_COLORS[incident.severity] }} />
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1 text-[11px] text-white/40">
                      ...
                      <span>•</span>
                      <span className="uppercase" style={{ color: SEV_COLORS[incident.severity] }}>{incident.severity}</span>
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-lg font-bold text-red-400">{formatDollars(incident.totalExposure)}</div>
              ...
              <div className="mt-3 grid grid-cols-4 gap-3">
                {[
                  ...
                  { label: "Downtime", value: `${incident.estimatedDowntimeHours}h × ${formatDollars(incident.downtimeCostPerHour)}/h` },
          ...
          {selectedIncident ? (
            <>
              <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
                ...
                {[
                  { label: "Direct Breach Cost", value: selectedIncident.breachCostEstimate, pct: selectedIncident.breachCostEstimate / selectedIncident.totalExposure },
                  { label: "Downtime Cost", value: selectedIncident.downtimeCostPerHour * selectedIncident.estimatedDowntimeHours, pct: (selectedIncident.downtimeCostPerHour * selectedIncident.estimatedDowntimeHours) / selectedIncident.totalExposure },
                  { label: "Remediation Cost", value: selectedIncident.remediationCost, pct: selectedIncident.remediationCost / selectedIncident.totalExposure },
                  { label: "Insurance Premium Impact", value: selectedIncident.insurancePremiumImpact, pct: selectedIncident.insurancePremiumImpact / selectedIncident.totalExposure },
                ].map((item) => (
                  <div key={item.label} className="mb-3">
                    ...
                    <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                      <div className="h-full rounded-full bg-red-500/60" style={{ width: `${item.pct * 100}%` }} />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
                <h3 className="text-sm font-semibold text-white/70 mb-3">Regulatory Penalty Assessment</h3>
                <div className="space-y-2">
                  <div className="flex justify-between text-xs">
                    <span className="text-white/50">Penalty Range</span>
                    <span className="text-white/70 font-mono">
                      {formatDollars(selectedIncident.regulatoryPenaltyRange[0])} – {formatDollars(selectedIncident.regulatoryPenaltyRange[1])}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-white/50">Probability of Penalty</span>
                    <span className="text-amber-400 font-mono">{Math.round(selectedIncident.regulatoryPenaltyProbability * 100)}%</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-white/50">Expected Penalty</span>
                    <span className="text-red-400 font-mono font-semibold">
                      {formatDollars(
                        ((selectedIncident.regulatoryPenaltyRange[0] + selectedIncident.regulatoryPenaltyRange[1]) / 2) *
            ...
            <div className="rounded-xl border border-white/5 bg-white/[0.02] p-8 text-center">
              <p className="text-sm text-white/30">Select an incident to view detailed financial breakdown</p>
            </div>
          )}
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-white/5">
        <div className="md:col-span-2">
          <CorrelationFeed correlations={correlations} currentDomain="firestorm" accentColor="#ef4444" />
        </div>
        <div className="flex items-start justify-center">
          <EnergyPulse metrics={energyMetrics} utilization={energyMetrics.usedBudget / energyMetrics.totalBudget} accentColor="#ef4444" />
        </div>
      </div>
    </div>
  );
