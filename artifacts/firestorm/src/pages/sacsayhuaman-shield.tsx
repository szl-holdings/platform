...
const SEED_DECISIONS: PolicyDecision[] = [
  ...
  { id: "pd2", timestamp: new Date(Date.now() - 18000), agent: "IT Sentinel", domain: "Rosie", action: "Write auto-remediation script to production server", actionType: "external_api", decision: "escalated", policyRule: "PROD-WRITE-REQUIRE-HUMAN", riskScore: 67, details: "Production write operations require human operator approval per SZL-SEC-022." },
  ...
  { id: "pd4", timestamp: new Date(Date.now() - 52000), agent: "Deal Scout", domain: "Beacon", action: "Initiate API call to CoStar property database", actionType: "external_api", decision: "permitted", policyRule: "EXT-API-COSTAR-APPROVED", riskScore: 22, details: "CoStar API is pre-approved for Beacon domain agents." },
  ...
  { id: "pd6", timestamp: new Date(Date.now() - 102000), agent: "Quipu Engine", domain: "INCA Cortex", action: "Spawn sub-agent with cross-domain write permissions", actionType: "agent_spawn", decision: "blocked", policyRule: "AGENT-SPAWN-WRITE-PROHIBITED", riskScore: 94, details: "Sub-agents cannot be spawned with elevated permissions without explicit authorization chain." },
  { id: "pd7", timestamp: new Date(Date.now() - 130000), agent: "Creative Director", domain: "Nimbus", action: "Post campaign content to LinkedIn API", actionType: "external_api", decision: "permitted", policyRule: "SOCIAL-POST-APPROVED-PLATFORMS", riskScore: 18, details: "LinkedIn publishing is pre-approved for Nimbus agents." },
  ...
  { id: "pd10", timestamp: new Date(Date.now() - 275000), agent: "IT Sentinel", domain: "Rosie", action: "Read all client device inventories across Rosie accounts", actionType: "data_access", decision: "permitted", policyRule: "MSP-AGENT-FULL-READ", riskScore: 35, details: "IT Sentinel has full read access to Rosie domain data within its scope." },
];

function generateDecision(): PolicyDecision {
  const agents = ["Maritime Analyst", "IT Sentinel", "Deal Scout", "Creative Director", "Portfolio Analyst", "Brand Monitor", "Advisory Agent"];
  const domains = ["Vessels", "Rosie", "Beacon", "Nimbus", "SZL Holdings", "Stephen", "Carlota Jo"];
  const actions = [
    ...
    <div className="p-6 lg:p-8 space-y-6 max-w-[1600px]">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <h1 className="text-xl font-display font-bold text-orange-50 tracking-tight">Sacsayhuamán Shield</h1>
        ...
        <p className="text-xs text-muted-foreground mt-2">
          Named for <span className="text-orange-400/80">Sacsayhuamán</span> — the massive fortress protecting Cusco. Every agent action passes through this policy engine before execution.
          Governs data access, external API calls, financial transactions, and agent spawning across the entire Quipu network.
        </p>
      </div>
