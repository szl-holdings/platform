...
const SEED_DECISIONS: PolicyDecision[] = [
  ...
  { id: "pd2", timestamp: new Date(Date.now() - 18000), agent: "IT Sentinel", domain: "MSP", action: "Write auto-remediation script to production server", actionType: "external_api", decision: "escalated", policyRule: "PROD-WRITE-REQUIRE-HUMAN", riskScore: 67, details: "Production write operations require human operator approval per SZL-SEC-022." },
  ...
  { id: "pd7", timestamp: new Date(Date.now() - 130000), agent: "Creative Director", domain: "Dreamscape", action: "Post campaign content to LinkedIn API", actionType: "external_api", decision: "permitted", policyRule: "SOCIAL-POST-APPROVED-PLATFORMS", riskScore: 18, details: "LinkedIn publishing is pre-approved for Dreamscape agents." },
  ...
  { id: "pd10", timestamp: new Date(Date.now() - 275000), agent: "IT Sentinel", domain: "MSP", action: "Read all client device inventories across MSP accounts", actionType: "data_access", decision: "permitted", policyRule: "MSP-AGENT-FULL-READ", riskScore: 35, details: "IT Sentinel has full read access to MSP domain data within its scope." },
];
...
function generateDecision(): PolicyDecision {
  ...
  const domains = ["Vessels", "MSP", "Terra", "Dreamscape", "SZL Holdings", "Stephen", "Carlota Jo"];
  const actions = [
