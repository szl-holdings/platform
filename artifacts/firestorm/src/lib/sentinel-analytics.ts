...
function detectVolumeAnomaly(weekly: number, b: BehavioralBaseline, ts: number): DetectedAnomaly | null {
  ...
  const sev: DetectedAnomaly["severity"] = Math.abs(z) >= 4 ? "critical" : Math.abs(z) >= 3 ? "high" : "medium";
  return {
    id: `vol-${ts}`, type: "volume-spike", severity: sev, zScore: z,
    ...
    timestamp: ts, mitreTechnique: "T1005",
  };
}

function detectOffHoursAnomaly(events: AccessEvent[], b: BehavioralBaseline, ts: number): DetectedAnomaly | null {
  if (events.length === 0) return null;
  const rate = events.filter((e) => e.offHours).length / events.length;
  if (rate < 0.3 || rate <= b.offHoursRate * 1.5) return null;
  const sev: DetectedAnomaly["severity"] = rate >= 0.6 ? "critical" : rate >= 0.4 ? "high" : "medium";
  const z = zScore(rate, b.offHoursRate, Math.max(b.offHoursRate * 0.3, 0.05));
  return {
    id: `offhours-${ts}`, type: "off-hours-pattern", severity: sev, zScore: z,
    ...
    timestamp: ts, mitreTechnique: "T1078",
  };
}

function detectGeoAnomaly(events: AccessEvent[], b: BehavioralBaseline, ts: number): DetectedAnomaly | null {
  ...
  const sev: DetectedAnomaly["severity"] = unknownEvents.length >= 5 ? "critical" : unknownEvents.length >= 2 ? "high" : "medium";
  return {
    id: `geo-${ts}`, type: "geo-anomaly", severity: sev, zScore: 3.5,
    ...
    timestamp: ts, mitreTechnique: "T1078.004",
  };
}

function detectSensitivityAnomaly(events: AccessEvent[], b: BehavioralBaseline, ts: number): DetectedAnomaly | null {
  if (events.length === 0) return null;
  const sensitive = events.filter((e) => ["confidential", "restricted", "top-secret"].includes(e.sensitivity));
  ...
  return {
    id: `sens-${ts}`, type: "sensitivity-escalation", severity: sev, zScore: z,
...
function detectBytesAnomaly(events: AccessEvent[], b: BehavioralBaseline, ts: number): DetectedAnomaly | null {
  const total = events.reduce((s, e) => s + (e.bytesTransferred ?? 0), 0);
  if (total === 0) return null;
  const z = zScore(total, b.meanBytesPerSession * events.length, Math.max(b.stdBytesPerSession * Math.sqrt(events.length), 1));
  if (Math.abs(z) < 2.5) return null;
  const gb = (total / 1_073_741_824).toFixed(2);
  const sev: DetectedAnomaly["severity"] = Math.abs(z) >= 4 ? "critical" : Math.abs(z) >= 3 ? "high" : "medium";
  return {
    id: `bytes-${ts}`, type: "bytes-anomaly", severity: sev, zScore: z,
    ...
    timestamp: ts, mitreTechnique: "T1567.002",
  };
}
...
export function computeRiskAssessment(
  ...
  if (unmanaged.length > 0 && baseline.managedDeviceRate >= 0.9) {
    anomalies.push({
      id: `device-${now}`, type: "unmanaged-device",
      severity: unmanaged.length >= 3 ? "high" : "medium", zScore: 2.8,
  ...
  // Factor scores
  const volumeRisk = zToRisk(zScore(weeklyCount, baseline.meanWeeklyAccesses, Math.max(baseline.stdWeeklyAccesses, 1)));
  const offRate = events.length > 0 ? events.filter((e) => e.offHours).length / events.length : 0;
  const temporalRisk = zToRisk(zScore(offRate, baseline.offHoursRate, Math.max(baseline.offHoursRate * 0.4, 0.05)));
  const unknownGeoFrac = events.length > 0 ? events.filter((e) => !baseline.knownGeoKeys.includes(e.geoKey)).length / events.length : 0;
  const geographicRisk = clamp(unknownGeoFrac * 100, 0, 100);
  const currentSensRate = events.length > 0 ? events.filter((e) => ["confidential", "restricted", "top-secret"].includes(e.sensitivity)).length / events.length : 0;
  const sensitivityRisk = zToRisk(zScore(currentSensRate, baseline.sensitiveAccessRate, Math.max(baseline.sensitiveAccessRate * 0.4, 0.05)));
  const unmanagedFrac = events.length > 0 ? events.filter((e) => !e.deviceManaged).length / events.length : 0;
  ...
  const bytesZ = zScore(totalBytes, baseline.meanBytesPerSession * events.length, Math.max(baseline.stdBytesPerSession * Math.sqrt(Math.max(events.length, 1)), 1_000_000));
  ...
  const weights = { volumeRisk: 0.20, temporalRisk: 0.15, geographicRisk: 0.20, sensitivityRisk: 0.20, deviceNetworkRisk: 0.10, dataTransferRisk: 0.15 };
  let raw = (Object.entries(weights) as [keyof typeof factorScores, number][]).reduce(
    (sum, [k, w]) => sum + factorScores[k] * w, 0,
  );
  ...
  const baselineDeviation = baseline.meanWeeklyAccesses > 0
    ? Math.round(((weeklyCount - baseline.meanWeeklyAccesses) / baseline.meanWeeklyAccesses) * 100)
    : 0;
  const summary = anomalies.length === 0
    ? "No significant behavioral anomalies detected."
    : `${anomalies.length} anomaly${anomalies.length !== 1 ? "s" : ""}: ${anomalies.slice(0, 2).map((a) => a.description).join("; ")}.`;

  return { userId, riskScore, riskLevel: riskLevelFromScore(riskScore), baselineDeviation, anomalies, summary, factorScores, assessedAt: now };
}

export function buildBaseline(userId: string, historicalEvents: AccessEvent[], windowDays = 90): BehavioralBaseline {
  ...
  const mean = weekCounts.length > 0 ? weekCounts.reduce((s, v) => s + v, 0) / weekCounts.length : historicalEvents.length / weekCount;
  const std = weekCounts.length > 1 ? Math.sqrt(weekCounts.reduce((s, v) => s + Math.pow(v - mean, 2), 0) / (weekCounts.length - 1)) : mean * 0.3;
  const offRate = historicalEvents.length > 0 ? historicalEvents.filter((e) => e.offHours).length / historicalEvents.length : 0.05;
  const sensRate = historicalEvents.length > 0 ? historicalEvents.filter((e) => ["confidential", "restricted", "top-secret"].includes(e.sensitivity)).length / historicalEvents.length : 0.1;
  ...
  const stdBytes = bytes.length > 1 ? Math.sqrt(bytes.reduce((s, b) => s + Math.pow(b - meanBytes, 2), 0) / (bytes.length - 1)) : meanBytes;
  return {
    userId, meanWeeklyAccesses: mean, stdWeeklyAccesses: std, offHoursRate: offRate,
    sensitiveAccessRate: sensRate, meanBytesPerSession: meanBytes, stdBytesPerSession: stdBytes,
    knownGeoKeys: [...new Set(historicalEvents.map((e) => e.geoKey))],
    managedDeviceRate: historicalEvents.length > 0 ? historicalEvents.filter((e) => e.deviceManaged).length / historicalEvents.length : 1.0,
  };
}
