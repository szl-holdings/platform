...
import {
  ...
  Brain, Target, Users, BarChart3, FileCheck, Lightbulb, Layers,
...
const ASSESSMENT_QUESTIONS = [
  {
    ...
    label: "If a board member asks who made a specific consequential decision six months ago, and why, can your team produce that record in under an hour?",
    options: [
      { value: "no", label: "No — we would need to reconstruct it from emails and meetings", score: 0 },
      { value: "partial", label: "Partially — some decisions are documented, most aren't", score: 1 },
  ...
  {
    id: "ai_governance",
    label: "When AI tools generate recommendations that your team acts on, is there a governance structure ensuring human approval before execution?",
    options: [
      { value: "no", label: "No — AI recommendations are often acted on without explicit approval gates", score: 0 },
      { value: "informal", label: "Informally — experienced team members review, but there's no structure", score: 1 },
      { value: "some", label: "For some decisions — but not consistently across the organization", score: 2 },
      { value: "yes", label: "Yes — all AI-assisted decisions have mandatory approval gates", score: 4 },
    ],
  },
  {
    id: "cross_function",
    label: "When a decision in one department has downstream effects on another (legal, finance, operations), are those teams automatically alerted and involved?",
    options: [
      ...
      { value: "yes", label: "Yes — cross-function signals are automatically correlated and routed", score: 4 },
    ],
  },
  {
    id: "outcome_tracking",
    label: "Does your organization systematically track whether consequential decisions achieved their intended outcomes?",
    options: [
      { value: "no", label: "No — we measure business results but not decision outcomes", score: 0 },
      { value: "some", label: "For major initiatives only — most decisions aren't tracked", score: 1 },
      { value: "partial", label: "We track outcomes but don't connect them back to specific decisions", score: 2 },
      { value: "yes", label: "Yes — every consequential decision is tracked to its measured outcome", score: 4 },
    ],
  },
];

const RESULTS = [
  { min: 0, max: 4, label: "Critical Governance Gap", severity: "critical" as const, desc: "Your organization has significant accountability exposure. Without governance infrastructure, AI tools are operating without approval gates, decisions are undocumented, and outcome tracking is absent. A regulatory inquiry or litigation event would reveal this gap immediately.", cta: "Request a Governance Briefing" },
  { min: 5, max: 9, label: "Structural Gaps", severity: "moderate" as const, desc: "You have some governance structure but significant gaps remain. Decision accountability is inconsistent, AI governance is informal, and cross-function coordination is manual. A structured governance program would close these gaps systematically.", cta: "Book a Discovery Session" },
  { min: 10, max: 13, label: "Governance-Aware", severity: "ready" as const, desc: "Your organization has awareness of governance requirements and some implementation. The opportunity is making governance systematic — replacing manual oversight with structured decision infrastructure.", cta: "Explore an Engagement" },
  { min: 14, max: 16, label: "Governance-Mature", severity: "ready" as const, desc: "Your organization has strong governance foundations. The conversation is about scaling governance infrastructure across new domains and integrating outcome tracking at the portfolio level.", cta: "Talk to Carlota Jo" },
];
    ...
    <section ref={ref} style={{ background: BG }} className="py-24 px-4">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-12">
          <p className="text-xs font-semibold tracking-widest uppercase mb-3" style={{ color: ACCENT }}>Engagement Model</p>
          ...
          <p className="text-base max-w-2xl mx-auto" style={{ color: TEXT_SEC }}>
            Not consulting in the traditional sense. Governance architecture installed, with accountability and outcome tracking built in from day one.
          </p>
        </div>
            ...
            {SERVICE_ORCHESTRATION_STAGES.map((s, i) => {
              ...
              return (
                <button
                  ...
                  style={{ background: isA ? `${s.color}15` : "transparent", border: `1px solid ${isA ? `${s.color}40` : "transparent"}` }}
                >
                  <span className="text-[10px] font-mono font-bold shrink-0" style={{ color: isA ? s.color : TEXT_SEC }}>{s.phase}</span>
                ...
                <>
                  <div className="flex items-center justify-between mb-5">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: `${stage.color}15`, border: `1px solid ${stage.color}30` }}>
                        <stage.icon className="w-5 h-5" style={{ color: stage.color }} />
                      </div>
                      <div>
                        <span className="text-[10px] font-mono font-bold block" style={{ color: stage.color }}>Phase {stage.phase}</span>
                        <span className="text-xl font-bold" style={{ color: TEXT }}>{stage.label}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5 px-3 py-1 rounded-full" style={{ background: `${stage.color}10`, border: `1px solid ${stage.color}20` }}>
                      <Clock className="w-3 h-3" style={{ color: stage.color }} />
                      <span className="text-xs font-medium" style={{ color: stage.color }}>{stage.duration}</span>
                    </div>
                  </div>

                  <div className="mb-5">
                    <p className="text-xs font-semibold tracking-widest uppercase mb-3" style={{ color: TEXT_SEC }}>Activities</p>
                    <div className="flex flex-col gap-2">
                      {stage.activities.map((a, i) => (
                        <div key={i} className="flex items-start gap-2">
                          <div className="w-4 h-4 rounded-full flex items-center justify-center shrink-0 mt-0.5" style={{ background: `${stage.color}20` }}>
                            <div className="w-1.5 h-1.5 rounded-full" style={{ background: stage.color }} />
                          </div>
                          <p className="text-sm" style={{ color: TEXT }}>{a}</p>
                        </div>
                      ))}
                    </div>
                  </div>

                  <div className="p-4 rounded-xl" style={{ background: `${stage.color}08`, border: `1px solid ${stage.color}20` }}>
                    <p className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: stage.color }}>Deliverable</p>
                    <p className="text-sm" style={{ color: TEXT }}>{stage.deliverable}</p>
                  </div>

                  <div className="flex gap-1 mt-6">
                    {SERVICE_ORCHESTRATION_STAGES.map((_, i) => (
                      <button key={i} onClick={() => setActive(i)} className="h-1 rounded-full flex-1 transition-all duration-200" style={{ background: i === active ? stage.color : BORDER }} />
                    ))}
                  </div>
                </>
              )}
    ...
    <section style={{ background: BG }} className="py-24 px-4">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight mb-4" style={{ color: TEXT }}>Who we work with.</h2>
          <p className="text-base max-w-xl mx-auto" style={{ color: TEXT_SEC }}>Strategic governance for leaders who need decisions to scale with accountability.</p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {tab && (
            <motion.div key={tab.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.22 }}
              ...
              <div className="flex flex-col md:flex-row gap-8">
                <div className="flex-1">
                  ...
                  <Link href="/engage">
                    <button className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-semibold hover:opacity-90" style={{ background: ACCENT, color: BG }}>
...
function ReadinessFunnel() {
  ...
  const handleAnswer = async (qId: string, value: string, score: number) => {
    ...
    if (nextStep >= ASSESSMENT_QUESTIONS.length) {
      ...
      setStep("result");
      analytics.formSubmit({ form_name: "strategic-readiness-assessment", form_type: "diagnostic_assessment" });
      try {
        await fetch("/api/contact/submit", {
          ...
          body: JSON.stringify({
            ...
            metadata: { assessment: "strategic-governance-readiness", score: finalScore, result: finalResult?.label, answers: newAnswers },
          }),
        });
      } catch (_) {}
    } else {
  ...
  return (
    <section style={{ background: BG }} className="py-24 px-4" id="assessment">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-10">
          <p className="text-xs font-semibold tracking-widest uppercase mb-3" style={{ color: ACCENT }}>Governance Readiness</p>
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight mb-4" style={{ color: TEXT }}>
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key={`step-${step}`} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <div className="flex gap-1.5 flex-1">
                  {ASSESSMENT_QUESTIONS.map((_, i) => (<div key={i} className="h-1 rounded-full flex-1 transition-all duration-300" style={{ background: i <= step ? ACCENT : BORDER }} />))}
                </div>
          ...
          {step === "result" && result && (
            <motion.div key="result" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.28 }}
              className="rounded-2xl p-8" style={{ background: SURFACE, border: `1px solid ${BORDER}` }}>
              <button onClick={() => setStep((ASSESSMENT_QUESTIONS.length - 1) as number)} className="flex items-center gap-2 mb-6 text-xs hover:opacity-80" style={{ color: TEXT_SEC }}>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <button onClick={() => { setAnswers({}); setScores({}); setStep("intro"); }}
                  className="px-5 py-3 rounded-lg text-sm font-medium border hover:bg-white/5" style={{ borderColor: BORDER, color: TEXT_SEC }}>
                  Retake
                </button>
              </div>
            </motion.div>
...
export default function StrategicReadinessPage() {
  useEffect(() => { analytics.pageView({ path: "/readiness" }); }, []);
  return (
    <div style={{ background: BG, minHeight: "100dvh" }}>
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-4" style={{ background: `${BG}ee`, borderBottom: `1px solid ${BORDER}`, backdropFilter: "blur(12px)" }}>
        <Link href="/">
          <span className="text-sm font-bold" style={{ color: TEXT }}>Carlota Jo</span>
        </Link>
        <div className="flex items-center gap-4">
          <Link href="/" className="text-sm" style={{ color: TEXT_SEC }}>Home</Link>
          <Link href="/services" className="text-sm" style={{ color: TEXT_SEC }}>Services</Link>
          <Link href="/engage">
            <button className="px-4 py-2 rounded-lg text-sm font-semibold hover:opacity-90" style={{ background: ACCENT, color: BG }}>
              Engage
            </button>
          </Link>
        </div>
      ...
      <section className="pt-28 pb-16 px-4 text-center" style={{ background: BG }}>
        <div className="max-w-3xl mx-auto">
          ...
          <motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, delay: 0.18 }}
            ...
            Decision accountability, AI governance, and cross-function intelligence — installed as structural architecture, not consulting deliverables that age on a shelf.
          </motion.p>
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.24 }}
            ...
            <Link href="/engage">
              <button className="inline-flex items-center gap-2 px-6 py-3 rounded-lg text-sm font-semibold border hover:bg-white/5" style={{ borderColor: `${ACCENT}30`, color: TEXT }}>
                Book a Discovery Session <ArrowRight className="w-4 h-4" style={{ color: ACCENT }} />
              </button>
            </Link>
          </motion.div>
        </div>
      ...
      <RoleSection />
      <div id="assessment"><ReadinessFunnel /></div>

      <footer className="py-12 px-4 text-center border-t" style={{ borderColor: BORDER }}>
        <p className="text-sm" style={{ color: TEXT_SEC }}>Carlota Jo Consulting · Strategic Governance Architecture</p>
