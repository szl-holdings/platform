...
import { ArrowRight, Calculator, Loader2, TrendingUp } from 'lucide-react';
...
export default function ROICalculator() {
  usePageMeta({
    ...
    description:
      'Track and calculate the return on advisory investment. See how Carlota Jo engagements translate to real revenue, margin, and value creation.',
        ...
        if (res.ok) {
          ...
          if (json.data) {
            setMetrics({
              ...
              portfolioBenchmarks:
                json.data.portfolioBenchmarks ?? STATIC_METRICS.portfolioBenchmarks,
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <h1 className="text-2xl" style={{ fontFamily: 'var(--font-serif)' }}>
            {activeClient
              ? `${activeClient.name} — Engagement ROI`
              : 'Engagement Returns & Value Creation'}
          </h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        {[
          {
            label: 'Average Portfolio ROI',
            value: `${metrics.portfolioBenchmarks.avgRoi}%`,
            sub: 'Across all closed engagements',
          },
          {
            label: 'Avg Payback Period',
            value: `${metrics.portfolioBenchmarks.avgPaybackMonths} months`,
            sub: 'Time to positive ROI',
          },
          {
            label: 'Client Retention Rate',
            value: `${metrics.portfolioBenchmarks.clientRetentionPct}%`,
            sub: '12-month retention',
          },
          {
            label: 'Blended Margin',
            value: `${metrics.portfolioBenchmarks.blendedMarginPct}%`,
            sub: 'Across portfolio',
          },
          {
            label: 'Rate Realisation',
            value: `${metrics.portfolioBenchmarks.avgRateRealisationPct}%`,
            sub: 'vs target billing rate',
          },
          {
            label: 'Net Promoter Score',
            value: `${metrics.portfolioBenchmarks.npsScore}`,
        ...
        <div>
          <Card>
            ...
            <CardContent className="space-y-5">
              ...
              />
              <RangeInput
                label="Expected Engagement Lift"
                value={engagementLift}
                min={20}
                max={200}
                suffix="%"
                onChange={setEngagementLift}
              />
              <RangeInput
              ...
              <div className="pt-3 border-t border-border space-y-2">
                {[
                  { label: 'Annual Investment', val: `£${annualInvestment.toLocaleString()}` },
                  {
                    ...
                    val: `£${Math.round(totalReturn).toLocaleString()}`,
                    highlight: true,
                  },
                  ...
                  <div key={row.label} className="flex justify-between text-sm">
                    ...
                    <span className={`font-medium ${row.highlight ? 'text-emerald-600' : ''}`}>
                      {row.val}
                    </span>
                  </div>
                ))}
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            ...
            <Card>
              <CardContent className="p-4 text-center">
                ...
                >
                  £{(annualRevLift / 1000).toFixed(0)}K
                </p>
            ...
            <Card>
              <CardContent className="p-4 text-center">
                ...
                <p className="text-3xl font-bold mt-1" style={{ fontFamily: 'var(--font-serif)' }}>
                  £{(brandValue / 1000).toFixed(0)}K
                </p>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="flex gap-4 mb-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-emerald-600">{cs.returnValue}</p>
                    </div>
                    <div>
