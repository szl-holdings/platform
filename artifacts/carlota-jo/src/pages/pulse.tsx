...
const EVENT_TYPES = [
  { type: "client_engagement", messages: ["UHNW client session completed: 4.9/5 rating", "Quarterly review prepared for 3 accounts", "Brand audit report delivered"] },
  { type: "brand_insight", messages: ["Competitor positioning shift detected", "Social sentiment: +12% positive", "Thought leadership opportunity flagged"] },
  { type: "pipeline_update", messages: ["New referral from existing client", "Proposal sent: $85K engagement", "Contract renewal: 18-month extension"] },
  { type: "roi_analysis", messages: ["Client ROI: 340% on brand strategy", "Revenue attribution: $2.1M influenced", "NPS score: 92 — exceptional"] },
  { type: "strategy_note", messages: ["Market positioning brief finalized", "Competitive landscape updated", "Advisory framework v3 published"] },
];
      ...
      {metrics.map(metric => (
        <div key={metric.label}>
          <div className="flex justify-between mb-1">
            <span className="text-[10px] font-medium" style={{ color: "rgba(255,255,255,0.4)" }}>{metric.label}</span>
            <span className="text-[10px] font-bold tabular-nums" style={{ color: metric.color }}>{metric.label === "Avg Rating" ? "4.9/5" : `${metric.value}%`}</span>
          </div>
...
export default function CarlotaJoPulse() {
  ...
  useEffect(() => { const t = setInterval(() => setEngagements(p => p + (Math.random() > 0.7 ? 1 : 0)), 8000); return () => clearInterval(t); }, []);
  return (
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-lg overflow-hidden mb-5 p-3"
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2"><BarChart3 className="w-3.5 h-3.5" style={{ color: "#c4a265" }} />
              <span className="text-[10px] font-medium uppercase tracking-wider" style={{ color: "rgba(255,255,255,0.3)" }}>Engagement Activity</span></div>
            <span className="text-[10px] tabular-nums font-medium" style={{ color: "#c4a265" }}>{engagements} active</span>
          </div>
          <PulseThroughputChart color="#c4a265" />
        </m.div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
          <PulseMetricCard label="Clients" value={engagements} icon={Users} color="#c4a265" trend="3 new this month" delay={0} />
          <PulseMetricCard label="Revenue" value={2} suffix=".1M" icon={TrendingUp} color="#10b981" trend="+18% YoY" delay={80} />
          <PulseMetricCard label="NPS Score" value={92} icon={Heart} color="#ec4899" trend="Best in class" delay={160} />
          <PulseMetricCard label="Sessions" value={187} icon={Calendar} color="#3b82f6" trend="42 this month" delay={240} />
          <PulseMetricCard label="Referrals" value={14} icon={Star} color="#f59e0b" trend="67% referral rate" delay={320} />
          <PulseMetricCard label="Deliverables" value={89} icon={Award} color="#8b5cf6" trend="All on schedule" delay={400} />
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
          <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
            ...
            <h2 className="text-[13px] font-semibold mb-3 flex items-center gap-2" style={{ color: "rgba(255,255,255,0.65)" }}>
              <Heart className="w-4 h-4" style={{ color: "#c4a265" }} /> Client Satisfaction</h2>
            <ClientSatisfaction />
          </m.div>
          <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
            ...
            <h2 className="text-[13px] font-semibold mb-3 flex items-center gap-2" style={{ color: "rgba(255,255,255,0.65)" }}>
              <Sparkles className="w-4 h-4" style={{ color: "#c4a265" }} /> Advisory Intelligence</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
          <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
            className="rounded-lg p-4" style={{ background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.04)" }}>
            <h2 className="text-[13px] font-semibold mb-3 flex items-center gap-2" style={{ color: "rgba(255,255,255,0.65)" }}>
              <Target className="w-4 h-4" style={{ color: "#c4a265" }} /> Client Journey Flow</h2>
            <PulseFlowDiagram flows={[
              { from: "Referral", to: "Intake", type: "Client Onboarding", color: "#c4a265", intensity: 4 },
              { from: "Brand", to: "Strategy", type: "Audit → Plan", color: "#8b5cf6", intensity: 3 },
              { from: "Execute", to: "Measure", type: "Deliverable → ROI", color: "#10b981", intensity: 5 },
              { from: "Review", to: "Renew", type: "QBR → Retention", color: "#3b82f6", intensity: 4 },
              { from: "Client", to: "Referral", type: "Advocacy Loop", color: "#f59e0b", intensity: 3 },
            ]} />
          </m.div>
          <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45 }}
            className="rounded-lg p-4" style={{ background: "rgba(255,255,255,0.015)", border: "1px solid rgba(255,255,255,0.04)" }}>
            <h2 className="text-[13px] font-semibold mb-3 flex items-center gap-2" style={{ color: "rgba(255,255,255,0.65)" }}>
              <Activity className="w-4 h-4" style={{ color: "#10b981" }} /> Service Health</h2>
        ...
        ]} title="Advisory Operations" /></div>
        <div className="text-center py-4 mt-4">
          <p className="text-[9px] uppercase tracking-[0.25em]" style={{ color: "rgba(255,255,255,0.08)" }}>Carlota Jo Consulting — Advisory Pulse</p>
        </div>
      </div>
    </div>
  );
