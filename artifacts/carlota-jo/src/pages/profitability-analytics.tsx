...
const calcMargin = (rev: number, cost: number) =>
  rev > 0 ? Math.round(((rev - cost) / rev) * 100) : 0;

export default function ProfitabilityAnalytics() {
  usePageMeta({
    title: 'Engagement Profitability Analytics | Carlota Jo',
    description:
      'Real-time margin tracking per engagement — rate realization analysis, scope creep detection, write-off tracking, and true P&L visibility.',
  ...
  const [marginHistory, setMarginHistory] =
    useState<{ month: string; margin: number }[]>(STATIC_MARGIN_HISTORY);
  ...
  const WRITE_OFF_DATA = engagements.map((e) => ({
    name: e.client.split(' ')[0],
    writeOffs: e.writeOffs,
    scopeCreep: e.scopeCreepHours * 300,
  }));
      ...
      >
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            >
              True Margins. Zero Surprises.
              <br />
              <em style={{ color: '#34D399' }}>P&L Visibility Per Engagement.</em>
            </h1>
            <p
              style={{
                fontSize: 15,
            ...
            >
              Real-time margin tracking with rate realization analysis, scope creep detection, and
              write-off monitoring — so every engagement delivers the margin it should.
            </p>
            <div
              style={{
                display: 'grid',
              ...
              {[
                {
                  ...
                  value: fmtGBP(totalContracted),
                  sub: `${engagements.length} engagements`,
                },
                {
                  label: 'Revenue Collected',
                  value: fmtGBP(totalCollected),
                  sub: 'Cash received YTD',
                },
                { label: 'Blended Margin', value: `${overallMargin}%`, sub: 'vs 38% target' },
                {
                  label: 'Write-offs YTD',
                  value: fmtGBP(totalWriteOffs),
                  sub: `${totalScopeHours}h uncompensated`,
                },
                {
                  label: 'Avg Rate Realisation',
                  value: `${Math.round(engagements.reduce((s, e) => s + e.rateRealisationPct, 0) / Math.max(1, engagements.length))}%`,
                  sub: 'vs 100% target',
                },
              ].map((kpi) => (
                <div
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
              ...
              <h2 style={{ fontSize: 14, fontWeight: 600, color: '#1A1A14' }}>
                Portfolio Margin — Monthly Trend
              </h2>
            </div>
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={marginHistory}>
                ...
                <ReferenceLine
                  ...
                  stroke={GOLD}
                  strokeDasharray="4 2"
                  label={{ value: 'Target 38%', fontSize: 10, fill: GOLD }}
                />
                <Line
                  type="monotone"
                  dataKey="margin"
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
              ...
              <h2 style={{ fontSize: 14, fontWeight: 600, color: '#1A1A14' }}>
                Write-offs & Uncompensated Scope
              </h2>
            </div>
            <ResponsiveContainer width="100%" height={180}>
            ...
            {engagements.map((eng, i) => {
              ...
              return (
                <motion.div
                  ...
                  style={{
                    background: '#fff',
                    border: `1px solid ${eng.alerts.length > 0 ? '#FCA5A530' : '#E8E2D6'}`,
                    borderRadius: 16,
                    overflow: 'hidden',
                  }}
                >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        <span style={{ fontSize: 10, color: '#A89878' }}>
                          {eng.feeType === 'time-and-materials'
                            ? 'T&M'
                            : eng.feeType === 'fixed'
                              ? 'Fixed Fee'
                              : 'Retainer'}
                      ...
                      <div style={{ fontSize: 12, color: '#6B5E47', marginBottom: 8 }}>
                        {eng.engagement} · {eng.phase}
                      </div>
                      <div style={{ marginBottom: 8 }}>
                        ...
                        >
                          ...
                          <span style={{ fontSize: 11, color: '#6B5E47' }}>
                            {fmtGBP(eng.invoiced)} of {fmtGBP(eng.contractedValue)}
                          </span>
                        </div>
                        <div
                          style={{
                        ...
                        >
                          <div
                            style={{
                              height: '100%',
                              width: `${Math.min((eng.invoiced / eng.contractedValue) * 100, 100)}%`,
                              background: GOLD,
                              borderRadius: 3,
                            }}
                          />
                      ...
                      <div style={{ textAlign: 'center' }}>
                        <div
                          style={{
                            fontSize: 20,
                            fontWeight: 700,
                            color: currentMargin >= eng.marginTarget ? '#059669' : '#DC2626',
                            fontFamily: "'Cormorant Garamond', serif",
                          }}
                        >
                          {currentMargin}%
                      ...
                      <div style={{ textAlign: 'center' }}>
                        <div
                          style={{
                            fontSize: 20,
                            fontWeight: 700,
                            color: forecastMargin >= eng.marginTarget ? '#059669' : '#DC2626',
                            fontFamily: "'Cormorant Garamond', serif",
                          }}
                        >
                          {forecastMargin}%
                      ...
                      >
                        {[
                          ...
                          { label: 'Cost to Date', value: fmtGBP(eng.costToDate) },
                          {
                            ...
                            value: fmtGBP(eng.forecastedCost),
                            highlight: eng.forecastedCost > eng.contractedValue,
                          },
                          {
                            label: 'Write-offs YTD',
                            value: fmtGBP(eng.writeOffs),
                            highlight: eng.writeOffs > 0,
                          },
                          {
                            label: 'Rate Realisation',
                            value: `${eng.rateRealisationPct}%`,
                            highlight: eng.rateRealisationPct < 90,
                          },
                          {
                            label: 'Scope Creep Hours',
                            value: `${eng.scopeCreepHours}h`,
                            highlight: eng.scopeCreepHours > 10,
                          },
                        ].map((metric) => (
                          <div
                            key={metric.label}
                            style={{
                              padding: '12px 14px',
                              background: metric.highlight ? '#FEF2F2' : '#fff',
                              border: `1px solid ${metric.highlight ? '#FCA5A530' : '#E8E2D6'}`,
                              borderRadius: 10,
                            }}
                          >
                            ...
                            <div
                              style={{
                                fontSize: 16,
                                fontWeight: 700,
                                color: metric.highlight ? '#DC2626' : '#1A1A14',
                                fontFamily: "'Cormorant Garamond', serif",
                              }}
                            >
                              {metric.value}
                      ...
                      {eng.alerts.length === 0 && (
                        ...
                        >
                          ...
                          <span style={{ fontSize: 12, color: '#065F46' }}>
                            On track — no margin or scope alerts
                          </span>
                        </div>
                      )}
                    </motion.div>
