...
export default function KnowledgeVault() {
  ...
  const matchItemsFromText = (text: string): typeof KNOWLEDGE_VAULT_ITEMS => {
    ...
    return KNOWLEDGE_VAULT_ITEMS.filter(
      (k) =>
  ...
  const runAISearch = async () => {
    ...
    try {
      ...
      const fallbackItems = KNOWLEDGE_VAULT_ITEMS.filter(
        (k) =>
          k.tags.some((t) => t.toLowerCase().includes(searchQuery.toLowerCase())) ||
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              ...
              >
                Knowledge Vault & Methodology Library
              </span>
            </div>
            <h1
              style={{
            ...
            >
              ...
              <br />
              <em style={{ color: '#A78BFA' }}>Always at Hand.</em>
            </h1>
            <p
              style={{
                fontSize: 15,
            ...
            >
              Every framework, playbook, template, and case study — searchable, living, and growing
              with every engagement. AI finds the most relevant asset in seconds.
            </p>
            <div
              style={{
                display: 'grid',
            ...
            >
              {[
                ...
                {
                  label: 'Frameworks',
                  value: KNOWLEDGE_VAULT_ITEMS.filter(
                    (k) => k.type === 'framework',
                  ).length.toString(),
                },
                {
                  ...
                  value:
                    (
                      KNOWLEDGE_VAULT_ITEMS.reduce((s, k) => s + k.rating, 0) /
                      KNOWLEDGE_VAULT_ITEMS.length
                    ).toFixed(1) + '/5',
                },
              ].map((kpi) => (
                <div
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px' }}>
        ...
        <div style={{ padding: '32px 0 0', marginBottom: 24 }}>
          <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
            ...
            <button
              onClick={runAISearch}
              disabled={!searchQuery || aiSearching}
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: 8,
                ...
                fontWeight: 600,
                cursor: 'pointer',
                opacity: !searchQuery || aiSearching ? 0.5 : 1,
              }}
            >
              {aiSearching ? (
            ...
            {['all', 'framework', 'playbook', 'template', 'case-study', 'research'].map((type) => (
              <button
                ...
                style={{
                  ...
                  padding: '5px 14px',
                  borderRadius: 100,
                  border: `1px solid ${typeFilter === type ? GOLD : '#E8E2D6'}`,
                  background: typeFilter === type ? `${GOLD}15` : 'transparent',
                  color: typeFilter === type ? '#6B5E47' : '#A89878',
                  cursor: 'pointer',
          ...
          {aiResult && (
            ...
            >
              <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                ...
                <div>
                  <div style={{ fontSize: 11, fontWeight: 600, color: '#6B5E47', marginBottom: 6 }}>
                    AI KNOWLEDGE SEARCH
                  </div>
                  <p style={{ fontSize: 13, color: '#1A1A14', lineHeight: 1.7, margin: 0 }}>
                    {aiResult.reasoning}
                  </p>
                </div>
              </div>
          ...
          {(aiResult ? aiResult.items : filteredItems).map((item, i) => {
            ...
            return (
              <motion.div
                ...
                onMouseEnter={(e) => {
                  (e.currentTarget as HTMLElement).style.boxShadow =
                    `0 4px 20px ${typeMeta.color}15`;
                  (e.currentTarget as HTMLElement).style.borderColor = `${typeMeta.color}40`;
                }}
                onMouseLeave={(e) => {
              ...
              >
                ...
                <p style={{ fontSize: 12, color: '#6B5E47', lineHeight: 1.6, marginBottom: 12 }}>
                  {item.description}
                </p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 12 }}>
                  {item.tags.slice(0, 3).map((tag) => (
                    <span
                      ...
                      style={{
                        ...
                        padding: '2px 6px',
                        borderRadius: 4,
                        background: `${typeMeta.color}08`,
                        color: typeMeta.color,
                        border: `1px solid ${typeMeta.color}15`,
                      }}
                    >
                      {tag}
                    </span>
                  ...
                  {item.tags.length > 3 && (
                    <span style={{ fontSize: 10, color: '#A89878' }}>+{item.tags.length - 3}</span>
                  )}
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  <ChevronRight
                    ...
                    color="#A89878"
                    style={{
                      transform: selectedItem?.id === item.id ? 'rotate(90deg)' : 'none',
                  ...
                  {selectedItem?.id === item.id && (
                    ...
                    >
                      <div style={{ fontSize: 11, color: '#6B5E47', marginBottom: 8 }}>
                        <strong>Author:</strong> {item.author} · <strong>Industries:</strong>{' '}
