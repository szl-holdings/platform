      ...
      setTwin({
        ...
        recommendations: [
          ...CASTELLANO_ESTATE_TWIN.recommendations,
          {
            ...
            description:
              'BlueWave Pool Services is offline. Dispatch AquaTech Services for immediate pool heating restoration before VIP arrival.',
            confidence: 0.92,
            freshness: 'live',
            policyState: 'cleared',
    ...
    <div className="p-8 space-y-8 bg-[#fdfcf9] min-h-screen text-[#1a1a1a]">
        ...
        <div>
          ...
          <div className="flex items-center gap-4 text-sm text-[#666]">
            ...
            <div className="flex items-center gap-1.5">
              <Users size={16} />
              <span>
                {twin.expectedArrival.guestName} arrival in {twin.expectedArrival.countdown}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 bg-white border border-[#9a7d52]/10 rounded-xl shadow-sm">
          ...
          <div className="flex items-baseline gap-2">
            <span
              ...
              style={{ color: twin.readinessScore < 60 ? '#ef4444' : 'var(--color-gold)' }}
            >
              {twin.readinessScore}%
            </span>
          </div>
        ...
        <div className="p-6 bg-white border border-[#9a7d52]/10 rounded-xl shadow-sm">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-4xl font-serif font-bold text-red-500">
              {
                twin.systemChecks.filter(
                  (c: SystemCheck) => c.criticality === 'critical' && c.status !== 'online',
                ).length
              }
            </span>
          </div>
        ...
        <div className="p-6 bg-white border border-[#9a7d52]/10 rounded-xl shadow-sm">
          ...
          <div className="flex items-baseline gap-2">
            <span
              ...
              style={{ color: 'var(--color-ink-900)' }}
            >
              {twin.vendors.filter((v: VendorTwin) => v.status === 'online').length}/
              {twin.vendors.length}
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          <h2 className="text-xl font-serif font-medium border-b border-[#9a7d52]/10 pb-2">
            System Readiness
          </h2>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#fdfcf9] border-b border-[#9a7d52]/10">
                  <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-[#9a7d52]">
                    System
                  </th>
                  <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-[#9a7d52]">
                    Status
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {twin.recommendations.map((rec: EstateRecommendation) => (
              <ProofEnvelope
                ...
                policyState={
                  rec.policyState === 'cleared'
                    ? 'allowed'
                    : rec.policyState === 'flagged'
                      ? 'blocked'
                      : 'requires-approval'
                }
              ...
              >
                <div className="space-y-4">
                  ...
                  <div className="p-3 bg-[#fdfcf9] border border-[#9a7d52]/10 rounded-lg">
                    <div className="text-[10px] font-bold uppercase text-[#9a7d52] mb-1">
                      Evidence Cluster
                    </div>
          ...
          <div className="bg-white border border-[#9a7d52]/10 rounded-xl p-6 shadow-sm">
            <h3 className="text-sm font-medium text-[#9a7d52] uppercase tracking-wider mb-4">
              Vendor Status
            </h3>
            <div className="space-y-3">
              {twin.vendors.map((vendor: VendorTwin) => (
                <div
                  key={vendor.id}
                  className="flex items-center justify-between p-3 bg-[#fdfcf9] rounded-lg border border-[#9a7d52]/5"
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-2 h-2 rounded-full ${vendor.status === 'online' ? 'bg-green-500' : 'bg-red-500 animate-pulse'}`}
                    />
