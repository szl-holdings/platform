...
const clients = [
  {
    ...
    openItems: 1,
    radarData: [
      { axis: 'Brand Clarity', value: 92 },
      { axis: 'Market Fit', value: 88 },
      { axis: 'Content Perf.', value: 76 },
      { axis: 'Team Align', value: 95 },
      { axis: 'ROI Delivery', value: 89 },
    ],
  },
  {
    name: 'Velas Agency',
    ...
    openItems: 4,
    radarData: [
      { axis: 'Brand Clarity', value: 74 },
      { axis: 'Market Fit', value: 61 },
      { axis: 'Content Perf.', value: 82 },
      { axis: 'Team Align', value: 58 },
      { axis: 'ROI Delivery', value: 71 },
    ],
  },
  {
    name: 'Oasis Wellness',
    ...
    openItems: 0,
    radarData: [
      { axis: 'Brand Clarity', value: 97 },
      { axis: 'Market Fit', value: 94 },
      { axis: 'Content Perf.', value: 91 },
      { axis: 'Team Align', value: 99 },
      { axis: 'ROI Delivery', value: 96 },
    ],
  },
  {
    name: 'Kova Spirits',
    ...
    openItems: 7,
    radarData: [
      { axis: 'Brand Clarity', value: 55 },
      { axis: 'Market Fit', value: 42 },
      { axis: 'Content Perf.', value: 61 },
      { axis: 'Team Align', value: 38 },
      { axis: 'ROI Delivery', value: 57 },
    ],
  },
];
...
export default function ClientIntel() {
  ...
  usePageMeta({
    ...
    description:
      '360-degree client intelligence for Carlota Jo engagements. Track NPS scores, churn risk, ARR by client, and engagement health across the consulting portfolio.',
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            NPS tracking, churn risk, engagement scores, and health signals across your client
            portfolio.
          </p>
        </div>
            ...
            {clients.map((c) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline" className={`text-[10px] ${churnColor[c.churnRisk]}`}>
                    {c.churnRisk} Risk
                  </Badge>
                </div>
                <div className="flex items-center gap-3 mt-2">
                  <span className="text-[11px] text-muted-foreground">
                    NPS <span className="text-foreground font-medium">{c.nps}</span>
                  </span>
                  <span className="text-[11px] text-muted-foreground">
                    ARR <span className="text-foreground font-medium">{c.arr}</span>
                  </span>
                </div>
              </button>
            ))}
            ...
            <Card>
              <CardHeader className="pb-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {selected.phase} phase · Last contact: {selected.lastContact}
                </p>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="bg-muted/30 rounded-lg p-3 text-center">
                    ...
                    <p
                      className={`text-xl font-bold ${selected.openItems > 3 ? 'text-red-400' : selected.openItems > 0 ? 'text-amber-400' : 'text-emerald-400'}`}
                    >
                      {selected.openItems}
                    </p>
                  </div>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-primary" /> Portfolio NPS Trend (6 months)
            </CardTitle>
          </CardHeader>
          <CardContent>
...
const mentions = [
  {
    brand: 'Luminary Cosmetics',
    platform: 'Instagram',
    sentiment: 'positive',
    volume: 2847,
    change: '+18%',
    topTopic: 'Summer Campaign',
    timestamp: 'Live',
  },
  {
    brand: 'Oasis Wellness',
    platform: 'Twitter/X',
    sentiment: 'positive',
    volume: 1204,
    change: '+31%',
    topTopic: 'Mindfulness Series',
    timestamp: 'Live',
  },
  {
    brand: 'Kova Spirits',
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Globe className="w-4 h-4 text-primary" /> Social Listening Dashboard
          </CardTitle>
          <Badge
            variant="outline"
            className="text-[10px] bg-primary/5 text-primary border-primary/20"
          >
            Simulated Brandwatch Feed
          </Badge>
        </div>
      </CardHeader>
      <CardContent>
            ...
            <div key={m.brand} className="p-3 rounded-xl border border-border bg-muted/20">
              <div className="flex items-center justify-between mb-1.5">
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {m.platform} · {m.timestamp}
                  </p>
                </div>
                <Badge variant="outline" className={`text-[9px] ${sentimentColor[m.sentiment]}`}>
                  {m.sentiment}
                </Badge>
              </div>
              <div className="flex items-center gap-3 text-[11px]">
                <span className="text-muted-foreground">
                  Mentions:{' '}
                  <span className="text-foreground font-bold">{m.volume.toLocaleString()}</span>
                </span>
                <span className={m.change.startsWith('+') ? 'text-emerald-400' : 'text-red-400'}>
                  {m.change}
                </span>
              </div>
              <p className="text-[10px] text-muted-foreground mt-1">
                Top topic: <span className="text-foreground">{m.topTopic}</span>
              </p>
            </div>
          ))}
        </div>
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Target className="w-4 h-4 text-primary" /> Competitive Benchmarking
          </CardTitle>
            ...
            <div key={b.metric}>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <div className="flex items-center gap-3 text-[10px]">
                  <span className="flex items-center gap-1">
                    ...
                    Carlota Jo: <strong className="text-primary">{b.carlotaJo}</strong>
                  </span>
                  <span className="flex items-center gap-1">
              ...
              <div className="h-2 bg-muted rounded-full overflow-hidden relative">
                <div
                  ...
                  style={{
                    width: `${Math.min((b.industry / (b.carlotaJo > b.industry ? b.carlotaJo * 1.2 : b.industry * 1.2)) * 100, 100)}%`,
                  }}
                />
                <div
                  className="absolute top-0 h-full rounded-full bg-amber-400/50"
                  style={{
                    width: `${Math.min((b.topQuartile / (b.carlotaJo > b.topQuartile ? b.carlotaJo * 1.2 : b.topQuartile * 1.2)) * 100, 100)}%`,
                  }}
                />
                <div
                  className="absolute top-0 h-full rounded-full bg-primary"
                  style={{ width: `${Math.min(100, 83)}%` }}
                />
              </div>
            </div>
          ))}
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm flex items-center gap-2">
            <Award className="w-4 h-4 text-primary" /> Campaign ROI Attribution
          </CardTitle>
          <Badge variant="outline" className="text-[10px] bg-muted text-muted-foreground">
            Simulated
          </Badge>
