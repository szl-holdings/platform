...
import {
  ChevronDown, ChevronUp, Clock, Shield, CheckCircle2,
  AlertCircle, ArrowRight, Layers, Star,
} from "lucide-react";
import {
  SERVICE_PLAYBOOKS, getCategoryLabel, DEMO_NOTE,
  type ServicePlaybook, type PlaybookTask,
...
function TaskRow({ task, index, totalTasks }: { task: PlaybookTask; index: number; totalTasks: number }) {
    ...
    <div style={{ display: "flex", gap: 14, position: "relative" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0, width: 28 }}>
        <div style={{
          ...
          height: 28,
          borderRadius: "50%",
          background: task.connectorStatus === "stub" ? "rgba(183,134,46,0.12)" : "rgba(154,125,82,0.12)",
      ...
      >
        <div style={{ display: "flex", gap: 14, flex: 1 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: 12, color: MUTED, margin: 0, lineHeight: 1.5 }}>{playbook.description}</p>
          </div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8, flexShrink: 0 }}>
          ...
          <div style={{
            ...
            <span style={{
              ...
              color: GOLD, border: `1px solid ${GOLD}40`, background: `${GOLD}08`,
        ...
        {expanded && (
          ...
          >
            <div style={{ padding: "16px 24px 24px", borderTop: `1px solid ${BORDER}` }}>
              <div style={{ marginBottom: 16, fontSize: 11, color: MUTED, display: "flex", gap: 20 }}>
                <span>Last deployed: {playbook.lastUsed}</span>
                <span>·</span>
                <span style={{ color: AMBER }}>
                  {playbook.tasks.filter(t => t.connectorStatus === "stub").length} integration(s) pending credentials
                </span>
              </div>
              ...
              {playbook.tasks.some(t => t.connectorStatus === "stub") && (
                <div style={{
                  ...
                  <div style={{ fontSize: 11, color: INK, lineHeight: 1.55 }}>
                    <strong>Integration stubs:</strong> Steps marked "Integration pending" connect to third-party providers (VistaJet, SevenRooms, DHL). These stubs are ready for credential connection — no workflow logic changes are required when activated.
                  </div>
                </div>
              )}
      ...
      <div style={{
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          ...
          <p style={{ fontSize: 14, color: MUTED, margin: "0 0 16px" }}>
            Named sequences that orchestrate every task, specialist, and SLA for recurring service types.
          </p>
          <div style={{
            display: "inline-flex",
            alignItems: "center",
      ...
      <div style={{ padding: "32px 48px", maxWidth: 960 }}>
        ...
        <div style={{
          ...
          {[
            ...
            { label: "Total Deployments", value: SERVICE_PLAYBOOKS.reduce((s, p) => s + p.usageCount, 0) },
            { label: "Avg. Completion", value: `${SERVICE_PLAYBOOKS.reduce((s, p) => s + p.estimatedDays, 0) / SERVICE_PLAYBOOKS.length}d` },
          ].map(stat => (
            <div key={stat.label} style={{
              ...
              <div style={{ fontSize: 28, fontWeight: 700, color: INK, marginBottom: 4 }}>{stat.value}</div>
              <div style={{ fontSize: 11, color: MUTED, textTransform: "uppercase", letterSpacing: "0.12em" }}>{stat.label}</div>
            </div>
          ))}
        </div>
