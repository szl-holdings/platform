...
import {
  Users, ChevronDown, ChevronUp, Shield, Eye, EyeOff,
  MapPin, Plane, Gift, Phone, Home, Star, Lock,
  AlertTriangle, CheckCircle2, Clock,
} from "lucide-react";
import {
  CLIENT_DOSSIERS, getSLALabel, getTierBadgeColor, DEMO_NOTE,
  type ClientDossier, type ClientTier, type SLAStatus,
...
function RedactedField({ label, redacted }: { label: string; redacted: boolean }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ fontFamily: show ? "inherit" : "monospace", letterSpacing: show ? "normal" : "0.15em", color: show ? INK : MUTED }}>
        {show ? label : "••••••••"}
      </span>
    </button>
  );
      ...
      >
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: MUTED, display: "flex", gap: 14, flexWrap: "wrap" }}>
              ...
              <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
                ...
                <span style={{ color: slaColor(client.slaRisk) }}>{getSLALabel(client.slaRisk)}</span>
              </span>
            </div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontSize: 10, color: MUTED, textTransform: "uppercase", letterSpacing: "0.12em" }}>Open</div>
          </div>
          <div style={{
            width: 32,
            height: 32,
            ...
            <div style={{ padding: "0 24px 28px", borderTop: `1px solid ${BORDER}` }}>
              ...
              <div style={{
                ...
                <span style={{ fontSize: 11, color: MUTED }}>
                  Access scoped to: <strong style={{ color: INK }}>{client.accessScope.join(", ")}</strong>
                  &nbsp;·&nbsp;Tier 4 — Restricted
                </span>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 32px" }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 13, color: INK, margin: "0 0 4px" }}>{client.primaryContact}</p>
                  ...
                  {client.household.map(h => (
                    <div key={h.id} style={{
                      ...
                      {h.note && <p style={{ fontSize: 11, color: MUTED, margin: "4px 0 0 19px" }}>{h.note}</p>}
                    </div>
                  ))}
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap" }}>
                    {client.brandAffinities.map(b => <Pill key={b}>{b}</Pill>)}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div style={{
                    ...
                    {[
                      ...
                      <div key={row.label} style={{ display: "flex", gap: 8, marginBottom: 8, fontSize: 12 }}>
                    ...
                    {client.travelPreferences.allergies.length > 0 && (
                      <div style={{ display: "flex", gap: 8, marginBottom: 8, fontSize: 12, alignItems: "flex-start" }}>
                        <span style={{ color: RED, fontWeight: 600, minWidth: 64 }}>Allergies</span>
                        <span style={{ color: RED, fontWeight: 500 }}>{client.travelPreferences.allergies.join(", ")}</span>
                      </div>
                    )}
                    {client.travelPreferences.dietaryRequirements.length > 0 && (
                      <div style={{ display: "flex", gap: 8, marginBottom: 0, fontSize: 12 }}>
                        ...
                        <span style={{ color: INK }}>{client.travelPreferences.dietaryRequirements.join(", ")}</span>
                      </div>
                    )}
                    {client.travelPreferences.notes && (
                      <p style={{ fontSize: 11, color: MUTED, marginTop: 10, paddingTop: 10, borderTop: `1px solid ${BORDER}`, marginBottom: 0 }}>
                        {client.travelPreferences.notes}
                      </p>
                    )}
                  </div>
                  ...
                  {client.residences.map((res, i) => (
                    <div key={i} style={{
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                        ...
                        <span style={{ fontSize: 13, fontWeight: 500, color: INK }}>{res.name}</span>
                      </div>
                      <p style={{ fontSize: 11, color: MUTED, margin: "0 0 4px 20px" }}>{res.address}</p>
                      {res.houseManager && (
                      ...
                      {res.notes && (
                        <p style={{ fontSize: 11, color: INK, margin: "4px 0 0 20px", fontStyle: "italic" }}>"{res.notes}"</p>
                      )}
                    </div>
                  ))}
                  ...
                  {client.giftHistory.length === 0 ? (
                    <p style={{ fontSize: 12, color: MUTED, fontStyle: "italic" }}>No gift history recorded.</p>
                  ) : (
                    client.giftHistory.map(g => (
                      <div key={g.id} style={{
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                          ...
                          <span style={{ fontSize: 13, fontWeight: 500, color: INK }}>{g.description}</span>
                        </div>
                        ...
                        {g.note && (
                          <p style={{ fontSize: 11, color: MUTED, margin: "4px 0 0 19px", fontStyle: "italic" }}>{g.note}</p>
                        )}
                      </div>
                    ))
                  )}
