...
function AuditEntry({
  ...
  entry: { timestamp: string; actor: string; action: string; redacted?: boolean };
}) {
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        border:
          request.priority === 'vip-exception' ? `1px solid ${AMBER}50` : `1px solid ${BORDER}`,
        borderRadius: 14,
        overflow: 'hidden',
        borderLeft:
    ...
    >
      ...
      >
        <div style={{ flex: 1 }}>
          ...
          <h3 style={{ fontSize: 14, fontWeight: 500, color: INK, margin: '0 0 4px' }}>
            {request.title}
          </h3>
        ...
        >
          ...
          <div
            style={{ fontSize: 11, color: MUTED, display: 'flex', alignItems: 'center', gap: 4 }}
          >
...
export default function ConciergeRequests() {
  ...
  const atRiskCount = SERVICE_REQUESTS.filter(
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
            ...
            {atRiskCount > 0 && (
              ...
              >
                <AlertTriangle size={12} /> {atRiskCount} request{atRiskCount > 1 ? 's' : ''} at SLA
                risk
              </div>
            )}
          </div>
