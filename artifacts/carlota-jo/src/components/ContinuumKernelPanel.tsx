...
export function AlloyKernelPanel() {
  const kernel = useMemo(() => getKernel('conversational-crm'), []);
    ...
    <div className="rounded-xl border border-border bg-card p-5 mt-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-sm font-bold">Conversational CRM</div>
            <div className="text-[10px] text-muted-foreground font-mono">
              Continuum Codex · {kernel.name} · v{kernel.version}
            </div>
          </div>
        </div>
        <span className="px-2 py-0.5 rounded-full bg-primary/10 border border-primary/20 text-[10px] text-primary font-mono uppercase tracking-wider">
          {kernel.pattern}
        </span>
      </div>
      ...
      <div className="space-y-3 mb-4">
        <div className="rounded-lg bg-muted/30 border border-border p-3">
          <div className="text-[10px] text-muted-foreground font-mono uppercase tracking-wider mb-1.5 flex items-center gap-1">
            <MessageSquare className="w-3 h-3" /> Role
          </div>
      ...
      <div className="flex items-center justify-between">
        ...
        {example && (
          <button
            ...
            className="flex items-center gap-1 text-[10px] text-primary hover:text-primary/80 transition-colors font-mono"
          >
