...
interface ClientLite { id: ClientId; name: string; industry: string }

interface MarginRow { month: string; margin: number }
interface RoiTrendRow { month: string; avgRoi: number }
interface MarketTrendRow { month: string; you: number; market: number }
interface CompetitorRow { name: string; score: number; trend: string; share: number }
interface RadarRow extends Record<string, unknown> {
  competitor: string; event: string; impact: string; direction: string;
  date?: string; signalDate?: string; detail: string;
}
...
const sectionTitleStyle: React.CSSProperties = {
  fontSize: "12px", fontWeight: 700, color: "rgba(255,255,255,0.75)",
  marginBottom: "8px", textTransform: "uppercase", letterSpacing: "0.5px",
};

interface SaveBarProps { accent: string; onSave: () => void; saving: boolean; status: string | null }
function SaveBar({ accent, onSave, saving, status }: SaveBarProps) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: "10px", marginTop: "10px" }}>
      {status && <span style={{ fontSize: "11px", color: status.startsWith("Saved") ? "#22c55e" : "#ef4444" }}>{status}</span>}
      <button
        onClick={onSave}
        disabled={saving}
        style={{
...
export default function AdvisoryDataEditor({ accent }: { accent: string }) {
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        if (!res.ok) return;
        const json = await res.json() as { data?: { clients?: ClientLite[] }; clients?: ClientLite[] };
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch(`${API}/carlota/admin/clients/${encodeURIComponent(clientId)}/advisory-data`, { credentials: "include" });
        if (!res.ok) {
        ...
        const payload: AdvisoryData = ("data" in (json as object) && (json as { data?: AdvisoryData }).data)
          ? (json as { data: AdvisoryData }).data
          : (json as AdvisoryData);
        setData({
          ...payload,
          radarSignals: payload.radarSignals.map(r => ({ ...r, date: r.signalDate ?? r.date ?? "" })),
        });
      } catch (e) {
  ...
  async function saveSection(section: string, body: object) {
    ...
    try {
      const res = await fetch(`${API}/carlota/admin/clients/${encodeURIComponent(clientId)}/${section}`, {
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "12px", padding: "16px" }}>
      ...
      {loading && <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.5)" }}>Loading client data...</div>}
          ...
          <div>
            ...
            <ListEditor
              ...
              columns={[
                ...
                { key: "impact", label: "Impact", width: "0.6fr", select: ["high", "medium", "low"] },
                { key: "direction", label: "Direction", width: "0.7fr", select: ["threat", "opportunity", "neutral"] },
                { key: "date", label: "Date", width: "0.8fr" },
                { key: "detail", label: "Detail", width: "2fr" },
              ]}
              newRow={() => ({ competitor: "", event: "", impact: "medium", direction: "neutral", date: "", detail: "" })}
            />
            <SaveBar accent={accent}
...
function ListEditor<T extends Record<string, unknown>>({ rows, onChange, columns, newRow }: {
  rows: T[];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
      <div style={{ display: "grid", gridTemplateColumns: grid, gap: "8px", padding: "0 4px" }}>
        {columns.map(c => <div key={c.key} style={labelStyle}>{c.label}</div>)}
        <div />
      </div>
      {rows.map((row, idx) => (
        <div key={idx} style={{ display: "grid", gridTemplateColumns: grid, gap: "8px", alignItems: "center" }}>
          {columns.map(c => {
            const val = row[c.key];
            if (c.select) {
              return (
                <select key={c.key} value={String(val ?? "")} style={inputStyle}
                  onChange={(e) => {
                    const next = [...rows];
                    next[idx] = { ...row, [c.key]: e.target.value } as T;
                    onChange(next);
                  }}>
                  {c.select.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                </select>
              );
            }
            return (
          ...
          <button
            ...
            style={{
              ...
              fontSize: "11px", cursor: "pointer", fontFamily: "system-ui, sans-serif",
            }}
          >Remove</button>
        </div>
      ))}
