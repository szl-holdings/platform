...
export function SubstrateWorkflowPanel({ clientId, taskTitle }: { clientId?: string; taskTitle?: string }) {
      ...
      const run = await apiFetch<Record<string, unknown>>("/control-tower/substrate/run", {
        ...
        body: JSON.stringify({
          ...
          input: {
            ...
            taskDescription: "Full market positioning diagnostic with competitive benchmarking and capability gap analysis for Q3 strategic roadmap.",
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <p className="text-[10px] text-amber-400/50 mt-0.5 font-mono">Substrate · carlota-jo-task-routing · Phase 2</p>
            </div>
          ...
          <p className="text-[10px] text-amber-100/40 mt-1.5 leading-relaxed">
            Matches incoming client tasks to advisors by expertise and availability. Practice lead approval before assignment confirmation.
          </p>
          ...
          {status !== "idle" && (
            <div className="mt-2 flex items-center gap-2">
              ...
              {result && <span className="text-[9px] font-mono text-amber-400/30">{result.runId}</span>}
              {(result || error) && (
                <button onClick={() => setExpanded(v => !v)} className="ml-auto text-amber-400/30 hover:text-amber-400">
        ...
        <div className="border-t border-amber-500/10 pt-3 space-y-3">
          ...
          {result.stages.length > 0 && (
            <div className="space-y-1">
              ...
              {result.stages.map(s => (
                <div key={s.stageId} className="flex items-center justify-between rounded border border-amber-500/10 bg-amber-950/20 px-2.5 py-1.5">
                  <div>
          ...
          {result.retriever && (
            <div className="flex items-center gap-2">
              ...
              {result.retriever.adapterId && (
                <span className="text-[9px] font-mono text-amber-400/40">adapter:{result.retriever.adapterId}</span>
              )}
            </div>
          )}
          <div className="flex items-center justify-between">
            <span className="text-[9px] font-mono text-amber-400/20">mode:{result.mode} · inbox:carlota-jo-task-routing</span>
          ...
          {mode === "dry-run" && (
            <div className="rounded border border-sky-500/15 bg-sky-500/5 p-2">
              <p className="text-[9px] font-mono text-sky-400">DRY-RUN — assignment and client notification suppressed.</p>
            </div>
          )}
          {status === "pending-approval" && (
            <div className="rounded border border-amber-500/20 bg-amber-500/5 p-2">
              <p className="text-[9px] font-mono text-amber-400">PENDING APPROVAL — paused at approval gate. Practice lead must review before routing commits.</p>
            </div>
          )}
        </div>
      )}
