...
const qualities = [
  {
    label: 'Principal-led',
    description: 'Every engagement led by the founding advisor — not delegated to junior staff.',
  },
  {
    label: 'Confidential',
    description: 'Strict confidentiality across all engagements, clients, and sensitive matters.',
  },
  {
    label: 'Selective',
    description:
      'We work with a small number of clients at any time to preserve depth of engagement.',
  },
  {
    label: 'Independent',
    description: "No conflicts of interest. Advice that serves only the client's interests.",
  },
];
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-[#f5f0e8]/6 mb-20">
          {socialProofMetrics.map((m, i) => (
            ...
            >
              <p
                className="text-3xl font-light text-[#c8a96a] mb-2"
                style={{ fontFamily: 'Georgia, serif' }}
              >
                {m.value}
              </p>
              <p className="text-[11px] tracking-[0.12em] uppercase text-[#f5f0e8]/35 font-medium">
                {m.label}
              </p>
            </motion.div>
          ))}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-px bg-[#f5f0e8]/5">
          {qualities.map((q, i) => (
            ...
            >
              ...
              <p className="text-[#f5f0e8]/40 text-[13px] font-light leading-relaxed">
                {q.description}
              </p>
            </motion.div>
          ))}
