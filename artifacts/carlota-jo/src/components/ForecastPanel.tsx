...
const CJ_FORECAST_DATA: ForecastHead[] = [
  {
    headName: 'carlota-jo:demand-spikes',
    label: 'Demand Spike Forecast',
    intervals: [
      { horizon: '3d', point: 0.51, lower: 0.36, upper: 0.68, confidence: 0.83, unit: 'score' },
      { horizon: '7d', point: 0.63, lower: 0.46, upper: 0.80, confidence: 0.77, unit: 'score' },
      { horizon: '14d', point: 0.72, lower: 0.54, upper: 0.88, confidence: 0.70, unit: 'score' },
    ],
    provenance: { modelId: 'safe-default-carlota-jo:demand-spikes', modelVersion: '0.1.0', adapterId: 'safe-default', generatedAt: new Date().toISOString() },
    alertThreshold: 0.75,
    thresholdBreached: true,
  },
  {
    headName: 'carlota-jo:vendor-failure',
    label: 'Vendor Failure Risk',
    intervals: [
      { horizon: '7d', point: 0.28, lower: 0.15, upper: 0.43, confidence: 0.87, unit: 'score' },
      { horizon: '30d', point: 0.41, lower: 0.26, upper: 0.58, confidence: 0.80, unit: 'score' },
    ],
    provenance: { modelId: 'safe-default-carlota-jo:vendor-failure', modelVersion: '0.1.0', adapterId: 'safe-default', generatedAt: new Date().toISOString() },
    alertThreshold: 0.6,
  },
  {
    headName: 'carlota-jo:booking-conflict',
    label: 'Booking Conflict Probability',
    intervals: [
      { horizon: '1d', point: 0.23, lower: 0.11, upper: 0.38, confidence: 0.90, unit: 'score' },
      { horizon: '7d', point: 0.37, lower: 0.22, upper: 0.54, confidence: 0.84, unit: 'score' },
      { horizon: '14d', point: 0.49, lower: 0.33, upper: 0.66, confidence: 0.77, unit: 'score' },
    ],
    provenance: { modelId: 'safe-default-carlota-jo:booking-conflict', modelVersion: '0.1.0', adapterId: 'safe-default', generatedAt: new Date().toISOString() },
    alertThreshold: 0.5,
  },
];
...
export function ForecastPanel() {
  const breachedCount = CJ_FORECAST_DATA.filter((h) => h.thresholdBreached).length;
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <TrendingUp className="w-4 h-4 text-primary" />
          <h2 className="text-sm font-semibold text-foreground">Forecast Fabric — Carlota Jo Heads</h2>
        </div>
        <div className="flex items-center gap-2">
          {breachedCount > 0 && (
          ...
          <span className="text-[10px] font-mono text-muted-foreground">{CJ_FORECAST_DATA.length} heads · safe-default adapter</span>
        </div>
      </div>
      <p className="text-xs text-muted-foreground">Calibrated forecast intervals for all Carlota Jo consulting heads. Click any card to expand all horizons and view provenance.</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {CJ_FORECAST_DATA.map((head) => <HeadCard key={head.headName} head={head} />)}
      </div>
