const CACHE_VERSION = 'carlota-v1';
...
self.addEventListener('push', (event) => {
  ...
  const title = payload.title || 'Carlota Jo Consulting';
  const options = {
    body: payload.body || '',
    icon: payload.icon || '/favicon.svg',
    badge: payload.badge || '/favicon.svg',
    tag: payload.tag || 'carlota-alert',
    data: payload.data || {},
...
self.addEventListener('notificationclick', (event) => {
  ...
  const actionUrl = event.notification.data?.actionUrl || '/carlota-jo/';
  event.waitUntil(
