...
import { Brain, Zap, Radio, Server } from "lucide-react";
...
function GlobalFeedStats() {
  const { data } = useQuery<{ stats: { knowledge: { byDomain?: Record<string, number> }; eventBus: { totalPublished?: number } }; globalFeed: { correlations: unknown[] } }>({
    queryKey: ["agent-os-global-feed-msp"],
    queryFn: async () => {
  ...
  return (
    <div className="grid grid-cols-3 gap-4 mb-6">
      {[
        { label: "Service Findings", value: knowledge?.byDomain?.["msp"] ?? 0, icon: Server, color: "text-emerald-400", bg: "bg-emerald-500/10" },
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h1 className="text-xl font-bold">Autonomous Service Intelligence</h1>
          <p className="text-xs text-muted-foreground">MSP insights gathered autonomously while you were away</p>
        </div>
      </div>
      ...
      <div className="text-[10px] text-muted-foreground/40 text-center pt-2">
        Powered by SZL Agent OS — autonomous service monitoring running 24/7
      </div>
    </div>
  );
}
