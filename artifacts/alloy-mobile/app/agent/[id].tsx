...
export default function AgentDetailScreen() {
  ...
  const cfg = agent ? (STATUS_CONFIG[agent.status] ?? { color: "#6b7280", label: agent.status }) : null;
  ...
  if (agentError || !agent) {
    return (
      <View style={[styles.container, styles.center, { backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.emptyTitle, { color: colors.creamDim, marginTop: 12 }]}>Agent not found</Text>
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={[styles.agentHeader, { backgroundColor: colors.card, borderColor: colors.borderSubtle }]}>
        ...
        <View style={styles.statsGrid}>
          {[
            ...
            { label: "Success Rate", value: agent.successRate != null ? `${Math.round(agent.successRate * 100)}%` : "—", color: "#10b981" },
            { label: "Errors", value: String(agent.errorCount ?? "—"), color: agent.errorCount ? "#ef4444" : colors.mutedForeground },
            { label: "Avg Duration", value: formatMs(agent.avgDurationMs ?? null), color: colors.violet },
          ].map((s) => (
        ...
        {runsLoading ? (
          ...
          <View style={[styles.runList, { borderColor: colors.borderSubtle }]}>
            {runs.map((run, i) => {
              ...
              return (
                <View key={run.id} style={[styles.runRow, { borderTopWidth: i > 0 ? 1 : 0, borderColor: colors.borderSubtle }]}>
                  <View>
                    ...
                    <View style={styles.runMetaRow}>
                      <Text style={[styles.runMeta, { color: colors.mutedForeground }]}>{formatRelative(run.startedAt ?? run.createdAt)}</Text>
                      <Text style={[styles.runMeta, { color: colors.mutedForeground }]}>·</Text>
                      <Text style={[styles.runMeta, { color: colors.mutedForeground }]}>{formatMs(run.durationMs)}</Text>
                    </View>
                    {run.errorMessage && (
                      <Text style={[styles.runError, { color: "#ef4444" }]} numberOfLines={2}>{run.errorMessage}</Text>
                    )}
                  </View>
                </View>
              );
...
const styles = StyleSheet.create({
  ...
  agentHeader: { flexDirection: "row", alignItems: "center", gap: 12, padding: 16, borderRadius: 12, borderWidth: 1, marginBottom: 12 },
  agentIconLarge: { width: 52, height: 52, borderRadius: 12, alignItems: "center", justifyContent: "center" },
  ...
  sectionTitle: { fontSize: 9, fontFamily: "Inter_600SemiBold", letterSpacing: 2, marginBottom: 10 },
  ...
  emptyState: { alignItems: "center", paddingVertical: 32, borderRadius: 10, borderWidth: 1, borderStyle: "dashed" },
