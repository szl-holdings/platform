...
const BIOMETRIC_ENABLED_KEY = "alloy_biometric_enabled";
...
export function BiometricProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    LocalAuthentication.hasHardwareAsync().then((has) => setIsAvailable(has));
    (async () => {
      const enabled = await secureGet(BIOMETRIC_ENABLED_KEY);
      if (enabled === "true") { setIsEnabled(true); setIsLocked(true); }
    })();
  }, []);
  ...
  useEffect(() => {
    ...
    const handleChange = (nextState: AppStateStatus) => {
      if (nextState === "active") {
        ...
        if (isEnabled && elapsed > LOCK_TIMEOUT_MS) setIsLocked(true);
  ...
  const unlock = useCallback(async (): Promise<boolean> => {
    ...
    const result = await LocalAuthentication.authenticateAsync({
      promptMessage: "Authenticate to access Alloy",
      fallbackLabel: "Use passcode",
      cancelLabel: "Cancel",
    });
    if (result.success) { setIsLocked(false); lastActiveRef.current = Date.now(); return true; }
    return false;
  }, []);
