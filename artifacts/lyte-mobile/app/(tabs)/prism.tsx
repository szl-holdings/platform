      ...
      <View style={[styles.dimCard, { borderColor: expanded ? `${dim.color}30` : LYTE_COLORS.border }]}>
        <View style={styles.dimHeader}>
          ...
          <View style={styles.dimInfo}>
            ...
            <Text style={styles.dimDesc} numberOfLines={expanded ? undefined : 1}>{dim.description}</Text>
          </View>
...
export default function PrismScreen() {
  ...
  const riskScore = Math.max(0, 100 - criticalCount * 20 - activeSignals.filter(s => s.severity === "high").length * 10);
  const intelScore = Math.min(100, 60 + signals.length * 2);
  const signalsScore = activeSignals.length === 0 ? 100 : Math.max(0, 100 - activeSignals.length * 5);
  ...
  const dimensions: PrismDimension[] = [
    {
      ...
      metrics: [
        { label: "Avg Uptime", value: `${platforms.length > 0 ? (platforms.reduce((s, p) => s + p.uptime, 0) / platforms.length).toFixed(1) : "—"}%`, color: LYTE_COLORS.neonGreen },
        { label: "Healthy Platforms", value: `${healthyPlatforms}/${platforms.length}` },
        { label: "SLA Compliance", value: `${avgSla.toFixed(1)}%` },
        { label: "Degraded", value: `${platforms.filter(p => p.status === "degraded").length}`, color: platforms.some(p => p.status === "degraded") ? LYTE_COLORS.high : LYTE_COLORS.neonGreen },
      ],
    },
    {
      key: "risk",
      ...
      trendIcon: (criticalCount > 2 ? "trending-down" : criticalCount === 0 ? "trending-up" : "minus") satisfies FeatherName,
      ...
      metrics: [
        { label: "Critical", value: `${criticalCount}`, color: criticalCount > 0 ? LYTE_COLORS.critical : LYTE_COLORS.neonGreen },
        { label: "High", value: `${activeSignals.filter(s => s.severity === "high").length}`, color: activeSignals.some(s => s.severity === "high") ? LYTE_COLORS.high : LYTE_COLORS.neonGreen },
        { label: "Active Signals", value: `${activeSignals.length}` },
        { label: "Risk Score", value: `${riskScore}/100`, color: riskScore > 70 ? LYTE_COLORS.neonGreen : LYTE_COLORS.high },
      ],
    },
    {
      key: "intel",
      ...
      metrics: [
        ...
        { label: "Resolution Rate", value: `${signals.length > 0 ? Math.round((signals.filter(s => s.status === "resolved").length / signals.length) * 100) : 0}%`, color: LYTE_COLORS.neonGreen },
    ...
    },
    {
      ...
      metrics: [
        { label: "Active", value: `${activeSignals.length}`, color: activeSignals.length > 10 ? LYTE_COLORS.high : LYTE_COLORS.neonGreen },
        { label: "Unacknowledged", value: `${signals.filter(s => s.status === "new").length}`, color: signals.filter(s => s.status === "new").length > 0 ? LYTE_COLORS.critical : LYTE_COLORS.neonGreen },
    ...
    },
    {
      ...
      trendIcon: (openActions.length < 5 ? "trending-up" : openActions.length > 10 ? "trending-down" : "minus") satisfies FeatherName,
      color: "#a78bfa",
      score: motionScore,
      ...
      metrics: [
        { label: "Open Actions", value: `${openActions.length}`, color: openActions.length > 0 ? LYTE_COLORS.medium : LYTE_COLORS.neonGreen },
        { label: "Critical Actions", value: `${openActions.filter(a => a.priority === "critical" || a.urgency === "critical").length}`, color: LYTE_COLORS.critical },
        ...
        <View style={styles.overallCard}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.overallDesc}>
              {overallScore >= 80 ? "All systems operating within normal parameters." :
               overallScore >= 60 ? "Some attention areas detected — review signals." :
               "Elevated risk state — immediate action recommended."}
            </Text>
            <View style={[styles.statusBadge, {
              backgroundColor: overallScore >= 80 ? LYTE_COLORS.neonGreenDim : overallScore >= 60 ? LYTE_COLORS.mediumDim : LYTE_COLORS.criticalDim,
              borderColor: overallScore >= 80 ? LYTE_COLORS.neonGreenLight : overallScore >= 60 ? LYTE_COLORS.mediumLight : LYTE_COLORS.criticalLight,
            }]}>
              <Text style={[styles.statusText, {
                color: overallScore >= 80 ? LYTE_COLORS.neonGreen : overallScore >= 60 ? LYTE_COLORS.medium : LYTE_COLORS.critical
              }]}>
