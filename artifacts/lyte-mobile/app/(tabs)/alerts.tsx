  ...
  return (
    <View style={[styles.alertCard, { borderLeftColor: color, borderLeftWidth: 3 }]}>
      <View style={styles.alertTop}>
        <View style={[styles.priorityBadge, { backgroundColor: `${color}15`, borderColor: `${color}25` }]}>
          <Text style={[styles.priorityText, { color }]}>{(action.priority ?? action.urgency ?? "medium").toUpperCase()}</Text>
        </View>
        <Text style={styles.alertTime}>{timeAgo(action.dueAt)}</Text>
      </View>
      <Text style={styles.alertTitle}>{action.title}</Text>
      {action.description != null && <Text style={styles.alertDesc} numberOfLines={2}>{action.description}</Text>}
        ...
        <View style={styles.timelineSection}>
          ...
          {action.escalationTimeline.map((ev, i) => (
            <View key={i} style={styles.timelineRow}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.timelineActor}>{ev.actor} · {ev.action}</Text>
                {ev.notes != null && <Text style={styles.timelineNotes}>{ev.notes}</Text>}
                <Text style={styles.timelineTime}>{new Date(ev.timestamp).toLocaleTimeString()}</Text>
              </View>
            </View>
          ))}
        </View>
      ...
      <View style={styles.alertMeta}>
        ...
        <View style={[styles.stateBadge, { backgroundColor: action.state === "resolved" ? LYTE_COLORS.neonGreenDim : LYTE_COLORS.electricBlueDim }]}>
          <Text style={[styles.stateText, { color: action.state === "resolved" ? LYTE_COLORS.neonGreen : LYTE_COLORS.electricBlue }]}>{action.state}</Text>
        </View>
      </View>
      {!["resolved", "dismissed"].includes(action.state) && (
        <View style={styles.alertActions}>
          ...
          <Pressable
            style={[styles.actionBtn, { borderColor: "rgba(255,210,61,0.3)", backgroundColor: LYTE_COLORS.mediumDim }]}
...
export default function AlertsScreen() {
  ...
  const mergedActions = actions.map(a => localOverrides[a.id] ? { ...a, state: localOverrides[a.id] } : a);
  ...
  const activeActions = mergedActions
    ...
    .sort((a, b) => {
      ...
      return (order[a.priority ?? a.urgency ?? ""] ?? 3) - (order[b.priority ?? b.urgency ?? ""] ?? 3);
    });
  ...
  return (
    <View style={[styles.container, { backgroundColor: LYTE_COLORS.background }]}>
      ...
      <ScrollView
        ...
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={LYTE_COLORS.critical} />}
      >
        ...
        <View style={styles.summaryRow}>
          <View style={styles.summaryCard}>
            <Text style={[styles.summaryValue, { color: LYTE_COLORS.critical }]}>{activeActions.filter(a => (a.priority ?? a.urgency) === "critical" || (a.priority ?? a.urgency) === "urgent").length}</Text>
            <Text style={styles.summaryLabel}>Critical</Text>
          </View>
          <View style={styles.summaryCard}>
            <Text style={[styles.summaryValue, { color: LYTE_COLORS.high }]}>{activeActions.filter(a => (a.priority ?? a.urgency) === "high").length}</Text>
          ...
          <View style={styles.summaryCard}>
            <Text style={[styles.summaryValue, { color: LYTE_COLORS.neonGreen }]}>{actions.filter(a => a.state === "resolved").length}</Text>
