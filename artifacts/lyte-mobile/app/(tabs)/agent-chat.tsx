...
const TEXT = "#e2f4ff";
const TEXT_DIM = "rgba(226,244,255,0.4)";

const AGENTS = [
  { id: "beacon", name: "Terra Analytics", role: "AIOps Analytics", icon: "bar-chart-2", color: "#00d4ff" },
...
export default function AgentChatScreen() {
  ...
  const [messages, setMessages] = useState<Message[]>([{
    ...
    content: "AIOps intelligence active. I analyze signals, detect anomalies, and surface operational insights across your infrastructure. What do you need?",
    timestamp: Date.now() - 60000,
  }]);
  const [isLoading, setIsLoading] = useState(false);
  const [showPicker, setShowPicker] = useState(false);
  const { search, buildContextString } = useEmbeddingSearch({ domain: "operations", limit: 3 });
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      const fallbacks: Record<string, string> = {
        beacon: "Signal analysis complete. SLO burn rate normal. Error budget: 98.3% remaining. No anomalies in last 4h.",
        zeus: "Infrastructure stable. 42 services healthy, 0 degraded. Memory nominal across all nodes.",
        alloy: "Coordination active. All AIOps pipelines operational.",
        sentinel: "Security posture clean. No active incidents in monitoring window.",
      };
  ...
  return (
    <KeyboardAvoidingView style={[styles.root, { paddingTop: insets.top }]} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      <View style={styles.header}>
        ...
        <View style={styles.pill}><View style={styles.pillDot} /><Text style={styles.pillText}>AIOPS</Text></View>
      </View>
      {showPicker && (
      ...
      <View style={[styles.bar, { paddingBottom: insets.bottom + 8 }]}>
        <TextInput style={styles.input} value={input} onChangeText={setInput} placeholder="Query your AIOps agents…" placeholderTextColor={TEXT_DIM} multiline maxLength={2000} />
...
const styles = StyleSheet.create({
  ...
  pill: { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: `${ACCENT}12`, borderRadius: 20, paddingHorizontal: 8, paddingVertical: 3 },
  ...
  sendOff: { backgroundColor: `${ACCENT}25` },
});
