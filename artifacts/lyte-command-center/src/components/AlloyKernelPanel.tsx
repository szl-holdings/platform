...
export function AlloyKernelPanel() {
  const kernel = useMemo(() => getKernel('decision-intelligence'), []);
  const [showExample, setShowExample] = useState(false);
  const example = kernel.codex.examples[0];

  return (
    <div className="rounded-xl border border-sky-500/15 bg-slate-900/60 p-5 mt-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="px-2 py-0.5 rounded-full bg-sky-500/10 border border-sky-500/20 text-[10px] text-sky-400 font-mono uppercase tracking-wider">
          {kernel.pattern}
        </span>
      </div>
      ...
      {showExample && example && (
        <div className="mt-3 rounded-lg bg-slate-950/80 border border-sky-500/10 p-3">
