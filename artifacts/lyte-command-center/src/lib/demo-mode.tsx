...
export interface DemoScenario {
  ...
  recommendation: Omit<LiveRecommendation, "id" | "approvalState" | "executionLog"> & { signalId?: string };
}
...
export const DEMO_SCENARIOS: DemoScenario[] = [
  {
    ...
    signals: [
      {
        id: "LIVE-AEG-001",
        domain: "aegis",
        title: "Unauthorized credential sweep: 3 admin accounts targeted — anomalous auth pattern detected",
        summary: "Behavioral analytics detected a structured credential sweep across admin accounts at 02:14 UTC. 847 failed attempts over 4 minutes from rotating IPs. Pattern consistent with credential-stuffing attack.",
        severity: "critical",
      ...
      },
      {
        id: "LIVE-AEG-002",
        domain: "aegis",
        title: "Lateral movement detected: internal service-to-service calls outside normal baseline",
        summary: "Post-authentication, internal API calls show a 340% spike from one compromised service account, querying unauthorized data endpoints.",
        severity: "high",
    ...
    recommendation: {
      ...
      reasoning: "Pattern analysis confirms a credential-stuffing attack targeting admin accounts. The lateral movement signal indicates at least one account may have been partially compromised. Blast radius includes 2 internal APIs with elevated data access.",
      evidence: [
      ...
      suggestedAction: "1. Force logout and suspend 3 admin accounts\n2. Rotate all credentials and API keys\n3. Enable MFA enforcement globally\n4. Block identified IP ranges at perimeter\n5. Notify CISO and initiate incident record",
      impactEstimate: "Containment within 8 minutes. Prevents estimated $2.4M exposure from data breach risk.",
    },
  },
  {
    key: "vessels",
    ...
    color: "#38bdf8",
    icon: "⚓",
    description: "Cargo vessel deviating from approved route — fuel anomaly and ETA impact detected",
    tagline: "Vessel anomaly → route analysis → risk scoring → approval → course correction",
    signals: [
      {
        id: "LIVE-VES-001",
        domain: "vessels",
        title: "MV Meridian Star: unauthorized course deviation — 47nm off approved route, ETA impact +14h",
        summary: "AIS telemetry shows MV Meridian Star deviating 47 nautical miles south of approved route for past 6.3 hours. Fuel consumption anomaly of +34% detected. Cargo: high-value electronics, Port of Rotterdam ETA now missed.",
        severity: "critical",
      ...
      },
      {
        id: "LIVE-VES-002",
        domain: "vessels",
        title: "Fuel consumption anomaly: MV Meridian Star — 34% above baseline, engine fault code ECU-0147",
        summary: "Engine telemetry reports fault code ECU-0147 (injector irregularity) alongside elevated fuel draw. Chief Engineer has not acknowledged 3 automated system alerts.",
        severity: "high",
    ...
    recommendation: {
      ...
      reasoning: "MV Meridian Star deviation is caused by engine fault ECU-0147 combined with adverse current. Current trajectory misses Rotterdam port window, triggering demurrage penalties of $84K/day. Immediate corrective action minimizes losses.",
      evidence: [
      ...
      suggestedAction: "1. Authorize heading correction to 047° NE immediately\n2. Notify Chief Engineer — escalate if no response in 15 minutes\n3. Alert freight client (Nexus Electronics) — revised ETA +4h\n4. Schedule port berth rebook at Rotterdam\n5. Open engine maintenance ticket for injector service at arrival",
      impactEstimate: "Avoids $84K/day demurrage. Saves $340K freight client penalty clause. Net loss: $12K fuel overage vs $424K+ exposure.",
    },
  },
  {
    key: "terra",
    ...
    color: "#a07848",
    icon: "🏢",
    description: "Distressed commercial property flagged — acquisition window opening within 72 hours",
    tagline: "Market signal → valuation analysis → risk scoring → approval → offer execution",
    signals: [
      {
        id: "LIVE-TER-001",
        domain: "terra",
        title: "Distress signal: 847 Commerce Blvd — owner listed after failed refinancing, 22% below market",
        summary: "Proptech intelligence identified a distressed C-class office building at 847 Commerce Blvd. Owner filed for voluntary sale after failed Q1 refinancing. Listed at $4.1M — 22% below comparable sales. 72-hour exclusive window before public listing.",
        severity: "high",
      ...
      },
      {
        id: "LIVE-TER-002",
        domain: "terra",
        title: "Cap rate opportunity: 847 Commerce Blvd at 9.2% — portfolio target is 7.5% threshold",
        summary: "Pro-forma cap rate analysis shows 9.2% at ask price, assuming 85% occupancy target. Current occupancy: 71%. Lease-up runway estimated 14 months based on comparable absorption.",
        severity: "medium",
    ...
    workflowSteps: [
      ...
      { id: "s3", label: "Decide: Recommendation with offer price, risk factors, and return model" },
    ...
    recommendation: {
      ...
      reasoning: "847 Commerce Blvd represents a high-conviction acquisition aligned with portfolio strategy. Distressed owner, exclusive window, and above-target cap rate create a rare entry point. Risk-adjusted IRR of 18.4% at $3.95M exceeds the 15% hurdle rate.",
      evidence: [
...
export function DemoModeProvider({ children }: { children: React.ReactNode }) {
  ...
  const runPhaseSequence = useCallback((
    ...
    setState(prev => {
      ...
      const newSteps = prev.workflowRun?.steps.map((s, i) => ({
        ...s,
        status: i < phaseIndex ? "done" : i === phaseIndex ? "running" : "pending",
      } as WorkflowStep)) ?? wfRun.steps;

      return {
        ...prev,
        phase,
        running: true,
        workflowRun: prev.workflowRun ? { ...prev.workflowRun, phase, steps: newSteps } : { ...wfRun, phase, steps: newSteps },
        recommendation: phase === "decide" ? { ...rec, approvalState: "pending" } : prev.recommendation,
      };
    });
    ...
    addTimer(() => {
      ...
      setState(prev => {
        if (sessionTokenRef.current !== sessionToken) return prev;
        const completedSteps = prev.workflowRun?.steps.map((s, i) => ({
          ...s,
          status: i <= phaseIndex ? "done" : "pending",
          durationMs: i <= phaseIndex ? duration + Math.floor(Math.random() * 800) : undefined,
        } as WorkflowStep)) ?? [];
  ...
  const activate = useCallback((scenarioKey: DemoScenarioKey = "aegis") => {
    clearAllTimers();
    sessionTokenRef.current += 1;
    ...
    if (scenario.signals[1]) {
      addTimer(() => {
        ...
        setState(prev => {
          ...
          return {
            ...prev,
            signals: [...prev.signals, { ...scenario.signals[1], detectedAt: new Date().toISOString(), phase: "detect" as PrismPhase }],
  ...
  const approveRecommendation = useCallback(() => {
    ...
    setState(prev => {
      ...
      return {
        ...prev,
        recommendation: prev.recommendation ? { ...prev.recommendation, approvalState: "approved" } : null,
      };
    });

    const scenario = DEMO_SCENARIOS.find(s => s.key === stateRef.current.currentScenario) ?? DEMO_SCENARIOS[0];
  ...
  const rejectRecommendation = useCallback(() => {
    ...
    setState(prev => ({
      ...prev,
      recommendation: prev.recommendation ? { ...prev.recommendation, approvalState: "rejected" } : null,
      running: false,
      workflowRun: prev.workflowRun ? {
        ...prev.workflowRun,
        steps: prev.workflowRun.steps.map(s => ({ ...s, status: s.status === "running" ? "failed" : s.status })),
  ...
  return (
    <Ctx.Provider value={{ state, activate, deactivate, reset, selectScenario, approveRecommendation, rejectRecommendation }}>
      {children}
    </Ctx.Provider>
  );
}
