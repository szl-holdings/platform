...
import { api, type LyteReadinessItem } from "@/lib/api";
...
function ScoreRing({ score }: { score: number }) {
  ...
  return (
    <svg width={100} height={100} className="rotate-[-90deg]">
      ...
      />
      <text x={50} y={56} textAnchor="middle" fill={color} fontSize={18} fontWeight="bold" className="rotate-90 origin-[50px_50px] font-mono" style={{ rotate: "90deg", transformBox: "fill-box", transformOrigin: "center" }}>
        {score}
      </text>
    </svg>
  );
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            <span className={cn("text-[10px] px-1.5 py-0.5 rounded border", statConf.color, statConf.bg, statConf.border)}>
              {statConf.label}
            </span>
          </div>
          <div className="flex items-center gap-3 text-[10px] text-slate-500">
            ...
            {item.readinessScore != null && <span>· Score: <span className="text-slate-400">{item.readinessScore}%</span></span>}
          </div>
        </div>
        ...
        <div className="px-4 pb-4 border-t border-white/5 pt-3 space-y-3">
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            {item.status !== "in_progress" && item.status !== "complete" && item.status !== "waived" && (
              ...
              >
                {loading === "in_progress" ? <RefreshCw className="w-3 h-3 animate-spin" /> : <Clock className="w-3 h-3" />}
                Start
              </button>
            )}
  ...
  return (
    <div className="max-w-[900px] space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="font-display font-bold text-2xl text-white tracking-tight">Lyte Readiness</h1>
          <p className="text-sm text-slate-400 mt-1">Operational readiness scoring across all business dimensions</p>
        </div>
        <button onClick={() => refetch()} className="p-2 rounded-lg border border-white/10 text-slate-500 hover:text-white hover:border-white/20 transition-all">
        ...
        <div className="md:col-span-3 grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            <div key={stat.label} className="rounded-xl p-4 border border-white/5 bg-white/[0.02]">
              ...
              <div className={cn("font-display font-bold text-xl", stat.color)}>{isLoading ? "—" : stat.value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1,2,3,4].map(i => <div key={i} className="h-14 rounded-xl bg-white/[0.02] border border-white/5 animate-pulse" />)}
        </div>
      ) : categoryFilter === "all" ? (
        <div className="space-y-6">
          {Object.entries(groupedByType).filter(([, its]) => its.length > 0).map(([t, typeItems]) => {
            ...
            return (
              <div key={t}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[10px] text-slate-600">{typeItems.length} items · {tScore}% complete</span>
                </div>
        ...
        <div className="space-y-2">
          {filtered.map(item => <ReadinessItemCard key={item.id} item={item} onUpdate={onUpdate} />)}
          {filtered.length === 0 && (
            <div className="py-12 text-center text-slate-500 text-sm">No items in this category.</div>
          )}
        </div>
      )}
    </div>
