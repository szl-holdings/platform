...
function MetricCard({ metric }: { metric: BoardMetric }) {
  ...
  const trendColor = metric.trend === metric.good
    ? "text-emerald-400"
    : metric.trend === "flat"
    ? "text-amber-400/50"
    : "text-red-400";
...
function RiskCard({ risk }: { risk: BoardRisk }) {
  ...
  return (
    <div className={`cockpit-panel p-5 border ${cfg.border} ${risk.severity === "critical" ? "border-l-2" : ""}`}>
      <div className="flex items-start gap-3">
        <div className={`w-8 h-8 rounded flex items-center justify-center shrink-0 ${cfg.bg} border ${cfg.border} mt-0.5`}>
          <AlertTriangle className={`w-4 h-4 ${cfg.color}`} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            <p className="text-sm font-semibold text-amber-100">{risk.title}</p>
            <span className={`text-[9px] font-mono px-1.5 py-0.5 rounded border shrink-0 ${cfg.color} ${cfg.bg} ${cfg.border}`}>
              {cfg.label}
            </span>
          </div>
          ...
          <div className="rounded bg-emerald-500/5 border border-emerald-500/15 p-2.5 mb-3">
            <p className="text-[9px] font-mono text-emerald-400/50 mb-1">RECOMMENDED INTERVENTION</p>
    ...
    <div className="p-6 space-y-6 max-w-6xl">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-amber-100/50">Evidence-backed executive brief — risks, interventions, and operational health.</p>
        </div>
      ...
      <div className="cockpit-panel p-5 border-amber-500/20">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-mono text-amber-400/45">Generated by Alloy · Apr 18, 2026 · Proof: ALLOY-BRIEF-0418</p>
          </div>
        </div>
        <div className="space-y-3 text-sm text-amber-100/65 leading-relaxed">
          <p>
            The organisation is carrying <span className="text-amber-300 font-semibold">6 active ownership drift items</span> with a combined Action Debt score of <span className="text-amber-300 font-semibold">43</span> — both metrics at their highest point in 7 weeks. 
            Three items require board-level awareness today: the CloudPlatform vendor contract (expiry in 15 days with no ownership resolution), 
            the Q2 revenue forecast impasse (blocking board deck assembly), and the Meridian Corp renewal ($180K ARR at risk with NPS proxy in decline).
          </p>
          <p>
            The primary risk driver is <span className="text-amber-300 font-semibold">cross-functional ownership gaps</span> — work where multiple parties are named but none are acting. 
            Alloy has detected 3 circular dependency patterns in the last 14 days. The Customer Success escalation management workflow 
            carries the <span className="text-amber-300 font-semibold">highest pressure score in the organisation at 88/100</span>, driven by 8 escalated items and a team approaching capacity.
          </p>
          <p>
            Total at-risk ARR attributable to drift-path items is <span className="text-red-400 font-semibold">$600K</span> — $420K from the Q1 enterprise launch delay and $180K from the Meridian renewal.
            Series B due diligence is also at risk with the investor call on April 20 and the data room incomplete.
          </p>
          <p className="text-amber-100/45 text-[11px]">
            Lyte recommendation: COO to convene a 30-minute triage session today to assign single decision authority on CloudPlatform and Meridian. 
            All other items are addressed in the intervention schedule below.
          </p>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[9px] font-mono px-1.5 py-0.5 rounded border border-red-500/25 bg-red-500/8 text-red-400">{critical.length} CRITICAL</span>
            <span className="text-[9px] font-mono px-1.5 py-0.5 rounded border border-orange-500/25 bg-orange-500/8 text-orange-400">{high.length} HIGH</span>
            <span className="text-[9px] font-mono px-1.5 py-0.5 rounded border border-amber-500/25 bg-amber-500/8 text-amber-400">{medium.length} MEDIUM</span>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          {critical.length > 0 && (
            <div>
              <p className="text-[9px] font-mono text-red-400/60 uppercase tracking-widest mb-2">Critical — Act Today</p>
              <div className="space-y-2">
                {critical.map(r => <RiskCard key={r.id} risk={r} />)}
              </div>
            </div>
          )}
          ...
          {high.length > 0 && (
            <div>
              <p className="text-[9px] font-mono text-orange-400/60 uppercase tracking-widest mb-2 mt-4">High — Act This Week</p>
              <div className="space-y-2">
          ...
          {medium.length > 0 && (
            <div>
              <p className="text-[9px] font-mono text-amber-400/50 uppercase tracking-widest mb-2 mt-4">Medium — Monitor</p>
              <div className="space-y-2">
      ...
      {/* Navigation links */}
      <div className="cockpit-panel p-4">
        <p className="text-[10px] font-mono text-amber-400/40 uppercase mb-3">Explore Detail Surfaces</p>
          ...
          {[
            ...
            <Link key={l.href} href={l.href}>
              <div className="flex items-center justify-between p-2.5 rounded border border-amber-500/12 hover:border-amber-500/25 hover:bg-amber-500/4 transition-all cursor-pointer group">
                <div>
                  <p className="text-[11px] font-medium text-amber-100/80 group-hover:text-amber-200">{l.label}</p>
      ...
      <div className="cockpit-panel p-5 border-amber-500/20 bg-amber-500/4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-xs font-semibold text-amber-100 mb-1">Ready to deploy Lyte in your organisation?</p>
            <p className="text-[11px] text-amber-100/55 mb-3">
              Lyte onboards in 2 weeks with a read-only Alloy connector. See your first Ownership Drift report within 48 hours.
            </p>
            <a
