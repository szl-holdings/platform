...
function StageProgress({ stage, maxStage, path }: { stage: number; maxStage: number; path: Array<{ stage: number; owner: string; label: string }> | null }) {
  return (
      ...
      {Array.from({ length: maxStage }).map((_, i) => {
        ...
        return (
          <div key={s} className="flex items-center gap-1">
            ...
            {s < maxStage && <ChevronRight className="w-3 h-3" style={{ color: "rgba(255,255,255,0.1)" }} />}
          </div>
        );
      })}
    </div>
    ...
    >
      <div className="flex items-start gap-3">
        <div className="shrink-0 flex flex-col gap-1.5 pt-0.5">
          <span className="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded border" style={{ color: sc.color, background: sc.bg, borderColor: sc.border }}>{sc.label}</span>
          <span className="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded" style={{ color: sevColor, background: `${sevColor}12` }}>{esc.severity}</span>
        </div>
        <div className="flex-1 min-w-0">
          ...
          {esc.description && <p className="text-[10px] mb-2 leading-relaxed" style={{ color: "rgba(255,255,255,0.4)" }}>{esc.description}</p>}
    ...
    <div className="fixed inset-0 z-50 flex" onClick={onClose}>
      ...
      <div className="w-full max-w-lg border-l flex flex-col h-full overflow-y-auto" style={{ background: "#0c1626", borderColor: "rgba(255,255,255,0.08)" }} onClick={e => e.stopPropagation()}>
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded border" style={{ color: sc.color, background: sc.bg, borderColor: sc.border }}>{sc.label}</span>
                <span className="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded" style={{ color: SEV_COLORS[esc.severity], background: `${SEV_COLORS[esc.severity]}12` }}>{esc.severity}</span>
              </div>
              <h2 className="text-sm font-bold text-white">{esc.title}</h2>
              {esc.description && <p className="text-[11px] mt-1" style={{ color: "rgba(255,255,255,0.4)" }}>{esc.description}</p>}
            </div>
        ...
        <div className="p-5 space-y-5">
          <div>
            <div className="text-[10px] uppercase tracking-wider mb-2" style={{ color: "rgba(255,255,255,0.3)" }}>Escalation Progress</div>
            <StageProgress stage={esc.stage} maxStage={esc.maxStage} path={esc.escalationPath} />
          </div>

          {esc.escalationPath && esc.escalationPath.length > 0 && (
            <div>
              <div className="text-[10px] uppercase tracking-wider mb-2" style={{ color: "rgba(255,255,255,0.3)" }}>Escalation Path</div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { label: "SLA Deadline", value: esc.slaDeadlineAt ? new Date(esc.slaDeadlineAt).toLocaleString() : "None" },
            ].map(c => (
...
export default function EscalationWorkflow() {
  ...
  const escalations: Escalation[] = statusFilter === "active"
    ? rawData.filter((e: Escalation) => ["open", "in_progress", "escalated"].includes(e.status))
    : rawData;

  const openCount = rawData.filter((e: Escalation) => ["open", "in_progress", "escalated"].includes(e.status)).length;
  const criticalCount = rawData.filter((e: Escalation) => e.severity === "critical" && ["open", "in_progress", "escalated"].includes(e.status)).length;
  const overdueCount = rawData.filter((e: Escalation) => e.slaDeadlineAt && new Date(e.slaDeadlineAt) < new Date() && !["resolved", "closed"].includes(e.status)).length;
