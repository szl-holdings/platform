...
import { type RunItem, type RunStatus, runItems } from '@/data/seed';
    ...
    <div className={`cockpit-panel border ${cfg.border}`}>
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <span className={`text-[10px] font-mono ${POLICY_COLORS[run.policyState]}`}>
                  {run.policyState}
                </span>
                <span className="text-[10px] font-mono text-amber-400/30">·</span>
      ...
      {expanded && run.outcome && (
        <div className="px-4 pb-4 border-t border-amber-500/10 pt-3 space-y-3">
          <div
            className={`rounded p-3 border ${run.status === 'failed' ? 'bg-red-500/5 border-red-500/15' : 'bg-amber-500/4 border-amber-500/12'}`}
          >
          ...
          <div className="flex items-center justify-between">
            <div className="grid grid-cols-3 gap-4 text-[10px]">
              <div>
                <span className="text-amber-400/40">Agent: </span>
                <span className="text-amber-200/70 font-mono">{run.agentId}</span>
              </div>
              {run.durationMs !== undefined && (
                <div>
                  <span className="text-amber-400/40">Duration: </span>
              ...
              {run.tokensUsed !== undefined && (
                <div>
                  <span className="text-amber-400/40">Tokens: </span>
                  <span className="text-amber-200/70 font-mono">
        ...
        <div className="space-y-1 max-h-72 overflow-y-auto font-mono text-[10px] leading-relaxed">
          {data.entries.map((e) => (
            <div key={`${e.stepId}-${e.timestamp}`} className="flex gap-2">
              <span className="text-amber-400/30 shrink-0">
      ...
      {items.length > 0 && (
        <div className="space-y-2">
          {items.map((p) => (
            <div
              key={p.id}
              className="rounded p-3 bg-amber-500/4 border border-amber-500/15 space-y-1"
            >
              <div className="flex items-center justify-between">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-amber-400/50 mt-0.5">
            Agent execution history — {totalRuns} runs recorded
          </p>
        </div>
      </div>
