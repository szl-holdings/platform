...
function DebtCard({ item }: { item: DebtItem }) {
  ...
  return (
    <div className={`cockpit-panel transition-all ${
      addressed ? "border-emerald-500/25 opacity-80" :
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            {addressed && (
              <span className="text-[9px] font-mono px-1.5 py-0.5 rounded border border-emerald-500/30 bg-emerald-500/10 text-emerald-300">ADDRESSED</span>
            )}
          </div>
        </div>
      </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-0 border-t border-amber-500/8 space-y-3">
          ...
          {(reassigned || addressed) && (
            <div className={`rounded p-3 border ${addressed ? "bg-emerald-500/5 border-emerald-500/20" : "bg-sky-500/5 border-sky-500/20"}`}>
              <p className={`text-[10px] font-mono uppercase mb-1 ${addressed ? "text-emerald-400/60" : "text-sky-400/60"}`}>
              ...
              {reassigned && (
                <p className="text-[11px] text-amber-100/70">
                  ...
                  <span className="text-sky-300 font-semibold">{intervention?.reassignedTo}</span> · {formatTimestamp(intervention?.reassignedAt ?? "")}
                  {intervention?.reassignProofRef && (
                    <span className="ml-2 proof-badge text-[9px]"><Shield className="w-2 h-2" />{intervention.reassignProofRef}</span>
                  )}
                </p>
              )}
              {addressed && (
                <p className="text-[11px] text-amber-100/70 mt-1">
                  <span className="text-amber-200">{intervention?.addressedBy}</span> flagged as addressed · {formatTimestamp(intervention?.addressedAt ?? "")}
                  {intervention?.addressedProofRef && (
                    <span className="ml-2 proof-badge text-[9px]"><Shield className="w-2 h-2" />{intervention.addressedProofRef}</span>
                  )}
                </p>
              )}
              {addressed && intervention?.addressedNote && (
                <p className="text-[11px] text-amber-100/55 mt-1.5 italic">"{intervention.addressedNote}"</p>
              )}
            </div>
          )}
          ...
          {!addressed && (
            <div className="flex flex-wrap items-center gap-2" onClick={e => e.stopPropagation()}>
              ...
              {!addressOpen && (
                <button
                  type="button"
                  onClick={(e) => { e.stopPropagation(); setAddressOpen(true); }}
          ...
          {reassignOpen && !addressed && (
            <div className="rounded border border-sky-500/20 bg-sky-500/4 p-3 space-y-2" onClick={e => e.stopPropagation()}>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => { setReassignOpen(false); setNewOwner(""); }}
          ...
          {addressOpen && !addressed && (
            <div className="rounded border border-emerald-500/20 bg-emerald-500/4 p-3 space-y-2" onClick={e => e.stopPropagation()}>
              <p className="text-[10px] font-mono text-emerald-400/60 uppercase">Evidence of resolution</p>
              <textarea
                value={evidence}
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => { setAddressOpen(false); setEvidence(""); }}
                  className="px-3 py-1 rounded text-[11px] text-amber-400/60 hover:text-amber-300 transition-colors"
                >
                  Cancel
                </button>
