...
import {
  Network, CheckCircle, AlertTriangle, WifiOff, Clock,
} from "lucide-react";
...
const INTEGRATIONS: Integration[] = [
  {
    id: "slack", name: "Slack", category: "Notifications", icon: "💬",
    status: "degraded", latency: "504ms", successRate: 82.3, lastCheck: "32s ago",
    workspace: "@acme", eventsToday: 31, errorsToday: 4,
  ...
  },
  {
    ...
    status: "healthy", latency: "214ms", successRate: 99.8, lastCheck: "12s ago",
    workspace: "acme.atlassian.net", eventsToday: 88, errorsToday: 0,
    config: { project: "OPS", issueType: "Task", defaultAssignee: "unassigned" },
  },
  {
    id: "github", name: "GitHub", category: "Code & CI", icon: "🐙",
    status: "healthy", latency: "128ms", successRate: 100, lastCheck: "8s ago",
    workspace: "acme-org", eventsToday: 142, errorsToday: 0,
    config: { repos: ["platform", "backend", "frontend"], webhookEvents: ["push", "pr", "review"] },
  },
  {
    id: "postgres", name: "PostgreSQL", category: "Data Source", icon: "🐘",
    status: "healthy", latency: "4ms", successRate: 100, lastCheck: "4s ago",
    workspace: "prod-db.acme.io", eventsToday: 18400, errorsToday: 0,
    config: { pool: 10, queryTimeout: "30s", readOnly: false },
  },
  {
    id: "azure-ad", name: "Azure AD / SCIM", category: "Identity", icon: "🔒",
    status: "healthy", latency: "88ms", successRate: 99.6, lastCheck: "1m ago",
    workspace: "acme.onmicrosoft.com", eventsToday: 24, errorsToday: 0,
    config: { tenantId: "xxx-yyy-zzz", scimEndpoint: "/scim/v2", syncInterval: "15m" },
  },
  {
    id: "email", name: "Email (SMTP)", category: "Notifications", icon: "📧",
    status: "healthy", latency: "312ms", successRate: 98.2, lastCheck: "45s ago",
    workspace: "alerts@acme.io", eventsToday: 14, errorsToday: 0,
    config: { host: "smtp.sendgrid.net", from: "alerts@acme.io", tls: true },
  },
  {
    id: "webhook-generic", name: "Generic Webhooks", category: "Outbound", icon: "🔗",
    status: "healthy", latency: "188ms", successRate: 97.4, lastCheck: "18s ago",
    workspace: "5 configured", eventsToday: 63, errorsToday: 1,
    config: { endpoints: 5, signatureSecret: "configured", retries: 3 },
  },
  {
    id: "openai", name: "OpenAI (via Alloy)", category: "AI Inference", icon: "🧠",
    status: "healthy", latency: "842ms", successRate: 99.1, lastCheck: "12s ago",
    workspace: "org:acme-ai", eventsToday: 184, errorsToday: 1,
    config: { models: ["gpt-4o", "gpt-4o-mini"], maxTokens: 128000, budgetAlert: true },
  },
];

interface StatusStyle { color: string; bg: string; label: string; icon: React.ElementType }
...
const STATUS_CONFIG: Record<IntegrationStatus, StatusStyle> = {
  ...
  degraded: { color: "#f97316", bg: "rgba(249,115,22,0.1)", label: "Degraded", icon: AlertTriangle },
...
function IntegrationCard({ integration, selected, onClick }: { integration: Integration; selected: boolean; onClick: () => void }) {
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-base shrink-0" style={{ background: BG.elevated }}>
            {integration.icon}
          </div>
          <div className="min-w-0">
            <p className="text-[12px] font-semibold truncate" style={{ color: TEXT.primary }}>{integration.name}</p>
            <p className="text-[10px]" style={{ color: TEXT.tertiary }}>{integration.category} · {integration.workspace}</p>
          </div>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          <div className="text-right">
            <p className="text-[11px] font-mono" style={{ color: integration.successRate > 95 ? "#22c55e" : "#f97316" }}>{integration.successRate}%</p>
            <p className="text-[9px]" style={{ color: TEXT.muted }}>success</p>
          </div>
          <div className="text-right">
            <p className="text-[11px] font-mono" style={{ color: TEXT.secondary }}>{integration.latency}</p>
            <p className="text-[9px]" style={{ color: TEXT.muted }}>latency</p>
          </div>
          <div className="rounded-full px-2 py-0.5 text-[10px] font-medium flex items-center gap-1" style={{ background: s.bg, color: s.color }}>
...
interface DetailMetric { label: string; value: string; color: string }
...
function IntegrationDetail({ integration }: { integration: Integration }) {
  ...
  const detailMetrics: DetailMetric[] = [
    { label: "Success rate", value: `${integration.successRate}%`, color: integration.successRate > 95 ? "#22c55e" : "#f97316" },
    ...
    { label: "Errors today", value: String(integration.errorsToday), color: integration.errorsToday > 0 ? "#f97316" : "#22c55e" },
    ...
    <div className="h-full overflow-y-auto" style={{ background: BG.surface }}>
      <div className="px-5 py-4" style={{ borderBottom: `1px solid ${BORDER.subtle}` }}>
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ background: BG.elevated }}>
              {integration.icon}
            </div>
            <div>
              <p className="text-[13px] font-semibold" style={{ color: TEXT.primary }}>{integration.name}</p>
              <p className="text-[11px]" style={{ color: TEXT.tertiary }}>{integration.category} · {integration.workspace}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 rounded-full px-3 py-1.5 text-[11px] font-medium" style={{ background: s.bg, color: s.color }}>
        ...
        <div className="grid grid-cols-3 gap-3">
          {detailMetrics.map((m) => (
            <div key={m.label} className="rounded-md px-3 py-2.5" style={{ background: BG.elevated }}>
              ...
              <p className="text-[12px] font-mono font-semibold" style={{ color: m.color }}>{m.value}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {integration.alert && (
        <div className="mx-4 mt-4 rounded-lg px-4 py-3 flex items-start gap-2.5" style={{ background: "rgba(249,115,22,0.08)", border: "1px solid rgba(249,115,22,0.2)" }}>
          ...
          <div>
            <p className="text-[11px] font-medium mb-1" style={{ color: "#f97316" }}>Integration alert</p>
            <p className="text-[11px] leading-relaxed" style={{ color: "rgba(249,115,22,0.8)" }}>{integration.alert}</p>
          </div>
        </div>
      )}

      <div className="px-4 py-4">
        <p className="text-[10px] uppercase tracking-widest mb-3" style={{ color: TEXT.muted }}>Configuration</p>
        <div className="rounded-md overflow-hidden" style={{ background: BG.elevated, border: `1px solid ${BORDER.subtle}` }}>
          {Object.entries(integration.config).map(([key, value], i, arr) => (
            <div
              ...
              style={{ borderBottom: i < arr.length - 1 ? `1px solid ${BORDER.subtle}` : undefined }}
            >
              <span className="text-[11px]" style={{ color: TEXT.tertiary }}>{key}</span>
        ...
        <p className="text-[10px] uppercase tracking-widest mb-3 mt-4" style={{ color: TEXT.muted }}>Uptime (last 30d)</p>
    ...
    <div className="flex flex-col h-full overflow-hidden" style={{ background: BG.page }}>
      <div className="flex items-center justify-between px-4 py-3 shrink-0" style={{ borderBottom: `1px solid ${BORDER.subtle}` }}>
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[13px] font-semibold" style={{ color: TEXT.primary }}>Integration Health</span>
        ...
        <div className="flex gap-2">
          {filterOptions.map((f) => (
            ...
            >
              {f}{f !== "all" ? ` (${counts[f as IntegrationStatus]})` : ""}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 grid grid-cols-[320px_1fr] overflow-hidden">
        <div className="overflow-y-auto" style={{ borderRight: `1px solid ${BORDER.subtle}` }}>
          <div className="px-4 py-2.5 flex items-center gap-4 text-[11px]" style={{ borderBottom: `1px solid ${BORDER.subtle}`, background: BG.elevated }}>
            <span style={{ color: "#22c55e" }}>{counts.healthy} healthy</span>
            {counts.degraded > 0 && <span style={{ color: "#f97316" }}>{counts.degraded} degraded</span>}
