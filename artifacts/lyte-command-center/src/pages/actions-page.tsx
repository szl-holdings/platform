...
import { useActions, useUpdateAction } from "@/hooks/use-lyte";
import { Zap, CheckCircle, Clock, User, RefreshCw, ChevronRight } from "lucide-react";
      ...
      <div className="w-full max-w-lg bg-[#0c1626] border-l border-white/10 flex flex-col h-full overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="p-5 border-b border-white/5">
          <div className="flex items-start justify-between gap-3 mb-2">
            <span className={cn("text-[10px] px-1.5 py-0.5 rounded border uppercase tracking-wide", stateColors[action.state] ?? stateColors.open)}>{action.state}</span>
          ...
          {action.description && <p className="text-[11px] text-slate-400 mt-1">{action.description}</p>}
        </div>
        <div className="p-5 border-b border-white/5 space-y-2">
          ...
          {action.notes && (
            <div className="text-[11px] text-slate-300 bg-white/[0.03] rounded p-2 border border-white/5 mt-2">{action.notes}</div>
          )}
        </div>
        {history.length > 0 && (
          <div className="p-5 border-b border-white/5">
            ...
            <div className="space-y-1.5">
              {history.map((h, i) => (
                <div key={i} className="flex items-center gap-2 text-[10px]">
                  ...
                  <span className="ml-auto text-slate-600">{new Date(h.at).toLocaleString("en-US", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="p-5">
          ...
          <div className="flex flex-wrap gap-2">
            {transitions.map(t => (
              <button
                key={t.state}
                onClick={() => { onTransition(action.id, t.state); onClose(); }}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    {action.urgency && (
                      <span className={cn("text-[9px] px-1.5 py-0.5 rounded border uppercase tracking-wide font-mono", uc)}>{action.urgency.replace("_", " ")}</span>
                    )}
                  </div>
                  <p className="text-xs font-medium text-white/90 leading-tight mb-1">{action.title}</p>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <span className={cn("text-[9px] px-1.5 py-0.5 rounded border uppercase tracking-wide", sc)}>{action.state}</span>
