...
import { Shield, Search, Database, User, Bot, FileText, AlertTriangle, BookOpen, ExternalLink, ChevronDown, ChevronUp } from "lucide-react";
import { evidenceItems, type EvidenceItem } from "@/data/seed";

const TYPE_CONFIG: Record<string, { label: string; icon: React.ReactNode; color: string; bg: string; border: string }> = {
  system: { label: "System", icon: <Database className="w-3.5 h-3.5" />, color: "text-sky-400", bg: "bg-sky-500/8", border: "border-sky-500/20" },
  human: { label: "Human", icon: <User className="w-3.5 h-3.5" />, color: "text-purple-400", bg: "bg-purple-500/8", border: "border-purple-500/20" },
  alloy: { label: "Alloy / AI", icon: <Bot className="w-3.5 h-3.5" />, color: "text-amber-400", bg: "bg-amber-500/8", border: "border-amber-500/20" },
  signal: { label: "Signal", icon: <AlertTriangle className="w-3.5 h-3.5" />, color: "text-orange-400", bg: "bg-orange-500/8", border: "border-orange-500/20" },
  document: { label: "Document", icon: <FileText className="w-3.5 h-3.5" />, color: "text-teal-400", bg: "bg-teal-500/8", border: "border-teal-500/20" },
  audit_log: { label: "Audit Log", icon: <BookOpen className="w-3.5 h-3.5" />, color: "text-red-400", bg: "bg-red-500/8", border: "border-red-500/20" },
  external: { label: "External", icon: <ExternalLink className="w-3.5 h-3.5" />, color: "text-emerald-400", bg: "bg-emerald-500/8", border: "border-emerald-500/20" },
};
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className={`text-[9px] font-mono px-1.5 py-0.5 rounded border ${cfg.color} ${cfg.bg} ${cfg.border}`}>{cfg.label.toUpperCase()}</span>
              {expanded ? <ChevronUp className="w-3.5 h-3.5 text-amber-400/40" /> : <ChevronDown className="w-3.5 h-3.5 text-amber-400/40" />}
            </div>
          </div>
          <div className="flex items-center gap-3 mt-1">
            <span className={`text-[10px] font-mono ${FRESHNESS_COLORS[ev.freshness]}`}>{ev.freshness.toUpperCase()}</span>
            <span className="text-[10px] font-mono text-amber-400/30">{Math.round(ev.confidence * 100)}% confidence</span>
            {ev.chainRef && (
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-amber-500/10 pt-3 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {ev.linkedEntityLabel && (
              <div className="rounded bg-amber-500/4 border border-amber-500/12 p-2.5">
                ...
                {ev.linkedEntityType && <p className="text-[9px] font-mono text-amber-400/30">{ev.linkedEntityType}</p>}
              </div>
            )}
            {ev.linkedRecommendationId && (
              <div className="rounded bg-amber-500/4 border border-amber-500/12 p-2.5">
                <p className="text-[9px] font-mono text-amber-400/40 mb-1">SUPPORTS RECOMMENDATION</p>
          ...
          <div className="flex items-center justify-between text-[10px]">
            <span className="text-amber-400/40 font-mono">Captured: {new Date(ev.capturedAt).toLocaleString("en-US", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</span>
...
export default function EvidenceExplorerPage() {
  ...
  const filtered = useMemo(() => {
    return evidenceItems.filter(ev => {
      ...
      if (search && !ev.label.toLowerCase().includes(search.toLowerCase()) && !ev.value.toLowerCase().includes(search.toLowerCase())) return false;
      return true;
    });
  }, [search, filterType]);
  ...
  const typeCounts = useMemo(() => {
    ...
    evidenceItems.forEach(ev => { counts[ev.type] = (counts[ev.type] ?? 0) + 1; });
    return counts;
  }, []);

  const totalConfidence = Math.round(evidenceItems.reduce((sum, ev) => sum + ev.confidence, 0) / evidenceItems.length * 100);
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-amber-400/50 mt-0.5">Full evidence chain for all signals, recommendations, and decisions — {evidenceItems.length} evidence items</p>
        </div>
      ...
      <div className="space-y-3">
        ...
        {filtered.map(ev => <EvidenceCard key={ev.id} ev={ev} />)}
      </div>
    </div>
  );
}
