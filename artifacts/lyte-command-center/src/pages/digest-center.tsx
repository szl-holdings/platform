...
import { FileText, ChevronDown, ChevronRight, Clock, Download, Zap, RefreshCw, CheckCircle2 } from "lucide-react";
...
const DIGESTS: Digest[] = [
  ...
  },
  {
    ...
    status: "ready",
    summary: "Moderate portfolio health across Q1 close period. Vessels completed 4 successful port calls. PRISM Q1 reporting cycle closed. Terra completed 3 asset reviews. Aegis initiated security vendor onboarding process.",
    highlights: [
  ...
  },
  {
    ...
    status: "ready",
    summary: "Stable week. Portfolio health at 82% aggregate. No critical items. Vessels initiated Q2 charter contract cycle. PRISM ownership review completed.",
    highlights: [
...
function StatusPill({ status }: { status: Digest["status"] }) {
  ...
  return (
    <span className="text-[8px] font-medium px-2 py-0.5 rounded-full" style={{ color: cfg.color, background: cfg.bg }}>
      {cfg.label}
    </span>
  );
}
...
function DigestCard({ digest }: { digest: Digest }) {
  ...
  return (
    <div className="rounded-md overflow-hidden" style={{ background: BG.surface, border: `1px solid ${BORDER.subtle}`, opacity: isPending ? 0.7 : 1 }}>
      <button
      ...
      >
        <div className="w-8 h-8 rounded flex items-center justify-center shrink-0 mt-0.5" style={{ background: isPending ? "rgba(255,255,255,0.04)" : "rgba(45,212,191,0.08)", border: `1px solid ${isPending ? BORDER.subtle : "rgba(45,212,191,0.14)"}` }}>
          <FileText className="w-3.5 h-3.5" style={{ color: isPending ? TEXT.muted : ELECTRIC }} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <StatusPill status={digest.status} />
            <span className="text-[8px] font-mono" style={{ color: TEXT.muted }}>{digest.id}</span>
          </div>
          <h3 className="text-[12px] font-medium leading-snug" style={{ color: TEXT.primary }}>{digest.title}</h3>
          <p className="text-[9px] mt-0.5" style={{ color: TEXT.tertiary }}>{digest.period}</p>
        </div>
      ...
      {expanded && !isPending && (
        <div className="px-4 pb-4" style={{ borderTop: `1px solid ${BORDER.subtle}` }}>
          <div className="pt-3 space-y-3">
            <div>
              <span className="text-[8px] uppercase tracking-widest font-medium mb-1.5 block" style={{ color: TEXT.muted }}>Executive Summary</span>
              <p className="text-[11px] leading-relaxed" style={{ color: TEXT.secondary }}>{digest.summary}</p>
            </div>

            <div>
              <span className="text-[8px] uppercase tracking-widest font-medium mb-1.5 block" style={{ color: TEXT.muted }}>Key Highlights</span>
              <ul className="space-y-1.5">
                {digest.highlights.map((h, i) => (
                  <li key={i} className="flex items-start gap-2 text-[10px]" style={{ color: TEXT.secondary }}>
                    <CheckCircle2 className="w-2.5 h-2.5 mt-0.5 shrink-0" style={{ color: ELECTRIC }} />
                    {h}
                  </li>
                ))}
              </ul>
            ...
            <div className="flex items-center justify-between pt-1">
              <div className="flex items-center gap-3 text-[9px]">
                <span style={{ color: TEXT.muted }}>Packs: <span style={{ color: TEXT.tertiary }}>{digest.packsCovered.join(", ")}</span></span>
                <span style={{ color: TEXT.muted }}>{digest.wordCount} words</span>
              </div>
              <button className="flex items-center gap-1.5 px-2.5 py-1 rounded text-[9px] font-medium transition-all hover:opacity-80" style={{ background: "rgba(45,212,191,0.1)", border: "1px solid rgba(45,212,191,0.18)", color: ELECTRIC }}>
    ...
    <div className="p-4 md:p-5 space-y-5" style={{ background: BG.page }}>
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[9px] font-mono uppercase tracking-widest" style={{ color: ELECTRIC }}>Digest Center</span>
          </div>
          <h1 className="text-lg font-bold tracking-tight" style={{ color: TEXT.primary }}>Executive Summary Generation</h1>
          <p className="text-[11px] mt-0.5" style={{ color: TEXT.secondary }}>Portfolio-wide executive digests generated weekly and on-demand</p>
        </div>
        <button className="flex items-center gap-1.5 px-3 py-1.5 rounded text-[9px] font-medium transition-all hover:opacity-80 shrink-0" style={{ background: "rgba(45,212,191,0.1)", border: "1px solid rgba(45,212,191,0.18)", color: ELECTRIC }}>
      ...
      <div className="rounded-md p-3 flex items-center gap-3" style={{ background: BG.elevated, border: `1px solid ${BORDER.subtle}` }}>
        ...
        <p className="text-[10px]" style={{ color: TEXT.secondary }}>
          Digests are generated automatically every <strong style={{ color: TEXT.primary }}>Sunday at 7:00 AM</strong> covering the prior week's portfolio signals across all packs. On-demand generation is available above.
        </p>
      </div>
