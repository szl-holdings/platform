...
import { Shield, Info, GitBranch, Users, Package, Layers, CheckCircle2, AlertTriangle, Clock, ZapOff } from "lucide-react";
import { entityNodes, entityEdges, type EntityNode, type EntityEdge } from "@/data/seed";

const NODE_TYPE_CONFIG: Record<string, { icon: React.ReactNode; color: string; bg: string; border: string; label: string }> = {
  opportunity: { icon: <Package className="w-3.5 h-3.5" />, color: "text-amber-400", bg: "bg-amber-500/10", border: "border-amber-500/30", label: "Opportunity" },
  approval_chain: { icon: <GitBranch className="w-3.5 h-3.5" />, color: "text-orange-400", bg: "bg-orange-500/10", border: "border-orange-500/30", label: "Approval Chain" },
  project: { icon: <Layers className="w-3.5 h-3.5" />, color: "text-sky-400", bg: "bg-sky-500/10", border: "border-sky-500/30", label: "Project" },
  stakeholder: { icon: <Users className="w-3.5 h-3.5" />, color: "text-purple-400", bg: "bg-purple-500/10", border: "border-purple-500/30", label: "Stakeholder" },
  deliverable: { icon: <CheckCircle2 className="w-3.5 h-3.5" />, color: "text-teal-400", bg: "bg-teal-500/10", border: "border-teal-500/30", label: "Deliverable" },
};
...
function NodeCard({ node, selected, onClick }: { node: EntityNode; selected: boolean; onClick: () => void }) {
    ...
    >
      <div className="flex items-center gap-2 px-2.5 py-2">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[11px] font-semibold text-amber-100 truncate leading-tight">{node.label}</p>
          {node.sublabel && <p className={`text-[9px] font-mono truncate ${sts?.color ?? "text-amber-400/40"}`}>{node.sublabel}</p>}
        </div>
      </div>
    </div>
  );
...
function EdgeLine({ edge, nodes }: { edge: EntityEdge; nodes: EntityNode[] }) {
  ...
  return (
    <g>
      <line x1={x1} y1={y1} x2={x2} y2={y2} stroke={color} strokeWidth={edge.strength === "strong" ? 1.5 : 1} strokeDasharray={dashArray} opacity={0.6} markerEnd="url(#arrow)" />
      <text x={mx} y={my - 4} textAnchor="middle" fontSize="8" fill={color} opacity={0.7}>{edge.label}</text>
    </g>
  );
}
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex items-start justify-between gap-4 px-6 pt-6 pb-4 shrink-0">
        <div>
          ...
          <p className="text-xs text-amber-400/50 mt-0.5">Vantex Acquisition — Stalled Approval Chain scenario · {entityNodes.length} entities · {entityEdges.length} relationships</p>
        </div>
      ...
      <div className="flex flex-wrap gap-2 px-6 pb-3 shrink-0">
        {Object.entries(NODE_TYPE_CONFIG).filter(([k]) => nodeTypeFilter.has(k as never)).map(([k, v]) => (
          <div key={k} className={`flex items-center gap-1.5 px-2 py-1 rounded border text-[10px] font-mono ${v.color} ${v.bg} ${v.border}`}>
            {v.icon}
            {v.label}
          </div>
        ))}
        <div className="flex items-center gap-3 px-2 py-1 rounded border border-amber-500/15 bg-amber-500/5 text-[9px] font-mono text-amber-400/50">
          <span className="flex items-center gap-1"><span className="inline-block w-6 h-0.5 bg-emerald-400 opacity-60" /> active</span>
          <span className="flex items-center gap-1"><span className="inline-block w-6 border-t-2 border-dashed border-orange-400 opacity-60" /> stalled</span>
          <span className="flex items-center gap-1"><span className="inline-block w-6 border-t-2 border-dashed border-red-400 opacity-60" /> broken</span>
        </div>
      </div>
        ...
        <div className="flex-1 relative overflow-auto">
          <div className="relative" style={{ width: 800, height: 620 }}>
            <svg width="800" height="620" className="absolute inset-0 pointer-events-none">
              ...
              {entityEdges.map(edge => <EdgeLine key={edge.id} edge={edge} nodes={entityNodes} />)}
            </svg>
          ...
          <div className="w-72 shrink-0 border-l border-amber-500/10 bg-[hsl(220_30%_4%)] overflow-y-auto">
            <div className="p-4 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setSelected(null)} className="text-amber-400/40 hover:text-amber-300 text-xs">✕</button>
              </div>
                  ...
                  <>
                    <div className={`flex items-center gap-2 p-3 rounded-lg ${cfg.bg} border ${cfg.border}`}>
                      ...
                      <div>
                        ...
                        <p className={`text-[10px] font-mono ${sts?.color ?? "text-amber-400/40"}`}>{selected.status}</p>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      {entityEdges.filter(e => e.sourceId === selected.id || e.targetId === selected.id).map(e => {
                        const other = entityNodes.find(n => n.id === (e.sourceId === selected.id ? e.targetId : e.sourceId));
                        return other ? (
                          <div key={e.id} className="flex items-center gap-2 py-1.5">
                            <span className="text-[9px] font-mono text-amber-400/30">{e.sourceId === selected.id ? "→" : "←"}</span>
                            <div>
