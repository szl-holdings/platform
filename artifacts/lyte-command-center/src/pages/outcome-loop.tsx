...
import {
  ...
  ChevronRight, Activity, Target, Filter,
  CheckCircle2,
} from "lucide-react";
...
const OUTCOME_COLORS: Record<OutcomeType, { fg: string; bg: string; label: string; icon: React.ElementType }> = {
...
function OutcomeBadge({ outcome }: { outcome: OutcomeType }) {
  ...
  const Icon = c.icon;
  return (
    <span className="flex items-center gap-1 text-[8px] font-mono px-1.5 py-0.5 rounded uppercase tracking-wider" style={{ color: c.fg, background: c.bg }}>
...
function OutcomeCard({ rec }: { rec: RecommendationOutcome }) {
  ...
  const accColor = !rec.accuracy ? TEXT.muted : rec.accuracy >= 90 ? "#6b8f71" : rec.accuracy >= 75 ? "#c8953c" : "#c45a4a";
      ...
      <button onClick={() => setExpanded(e => !e)} className="w-full flex items-start gap-3 p-3.5 text-left hover:bg-white/[0.01] transition-colors">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-[8px] font-bold tracking-wider px-1.5 py-0.5 rounded uppercase" style={{ color: rec.packColor, background: `${rec.packColor}14` }}>{rec.pack}</span>
            <span className="text-[8px] font-mono" style={{ color: TEXT.muted }}>{rec.id}</span>
          </div>
          <p className="text-[10px] font-medium leading-snug" style={{ color: TEXT.primary }}>{rec.title}</p>
          <p className="text-[8px] mt-0.5" style={{ color: TEXT.tertiary }}>Decided: {rec.decidedAt} by {rec.decidedBy}</p>
        </div>
        <div className="shrink-0 flex flex-col items-end gap-1">
          {rec.accuracy && (
            <span className="text-[10px] font-mono" style={{ color: accColor }}>{rec.accuracy}%</span>
          )}
        ...
        <div className="px-3.5 pb-4 space-y-3" style={{ borderTop: `1px solid ${BORDER.subtle}` }}>
          <div className="pt-3 grid grid-cols-2 gap-3">
            <div className="rounded p-2.5" style={{ background: BG.elevated }}>
              <div className="text-[7px] uppercase tracking-wider mb-1" style={{ color: TEXT.muted }}>Predicted Impact</div>
              <p className="text-[9px] font-medium" style={{ color: TEXT.secondary }}>{rec.predictedImpact}</p>
            </div>
            <div className="rounded p-2.5" style={{ background: BG.elevated }}>
              <div className="text-[7px] uppercase tracking-wider mb-1" style={{ color: TEXT.muted }}>Actual Impact</div>
              <p className="text-[9px] font-medium" style={{ color: rec.actualImpact ? TEXT.primary : TEXT.muted }}>{rec.actualImpact ?? "Pending"}</p>
            </div>
          </div>
          {rec.modifiedTo && (
            <div className="rounded p-2.5" style={{ background: "rgba(200,149,60,0.06)", border: "1px solid rgba(200,149,60,0.14)" }}>
              <div className="text-[7px] uppercase tracking-wider mb-1" style={{ color: "rgba(200,149,60,0.6)" }}>Modified To</div>
              <p className="text-[9px]" style={{ color: "#c8953c" }}>{rec.modifiedTo}</p>
            </div>
          )}
        </div>
      )}
...
export default function OutcomeLoopPage() {
  ...
  const avgAccuracy = Math.round(OUTCOMES.filter(r => r.accuracy).reduce((s, r) => s + (r.accuracy ?? 0), 0) / OUTCOMES.filter(r => r.accuracy).length);
    ...
    <div className="p-4 md:p-5 space-y-5" style={{ background: BG.page }}>
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-[9px] font-mono uppercase tracking-widest" style={{ color: "#4a90b8" }}>Outcome Loop</span>
        </div>
        <h1 className="text-lg font-bold tracking-tight" style={{ color: TEXT.primary }}>Recommendation Accuracy Tracker</h1>
        <p className="text-[11px] mt-0.5" style={{ color: TEXT.secondary }}>Accepted vs rejected recommendations over time — feeds back into ranking quality</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "Acceptance Rate", value: `${Math.round((accepted / OUTCOMES.length) * 100)}%`, sub: `${accepted} of ${OUTCOMES.length}`, color: "#6b8f71", Icon: CheckCircle2 },
          { label: "Avg Accuracy", value: `${avgAccuracy}%`, sub: "Predicted vs actual", color: "#4a90b8", Icon: Target },
          { label: "Modified", value: String(modified), sub: "Human-adjusted", color: "#c8953c", Icon: RotateCcw },
          { label: "Rejected", value: String(rejected), sub: "Not accepted", color: "#c45a4a", Icon: X },
        ].map(m => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="rounded-md p-4" style={{ background: BG.surface, border: `1px solid ${BORDER.subtle}` }}>
          <div className="text-[9px] uppercase tracking-widest font-medium mb-3" style={{ color: TEXT.muted }}>Accuracy Trend (8 Weeks)</div>
          <div className="h-28">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={ACCURACY_TREND}>
                ...
                <XAxis dataKey="week" tick={{ fontSize: 7, fill: TEXT.muted }} axisLine={false} tickLine={false} />
                <YAxis domain={[60, 100]} tick={{ fontSize: 7, fill: TEXT.muted }} axisLine={false} tickLine={false} width={25} />
                <Tooltip contentStyle={{ background: BG.elevated, border: `1px solid ${BORDER.muted}`, borderRadius: 4, fontSize: 9 }} labelStyle={{ color: TEXT.tertiary }} formatter={(v: number) => [`${v}%`, "Accuracy"]} />
                <Area type="monotone" dataKey="accuracy" stroke="#4a90b8" strokeWidth={1.5} fill="url(#accGrad)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="rounded-md p-4" style={{ background: BG.surface, border: `1px solid ${BORDER.subtle}` }}>
          <div className="text-[9px] uppercase tracking-widest font-medium mb-3" style={{ color: TEXT.muted }}>Accuracy by Category</div>
              ...
              <div key={c.category}>
                <div className="flex items-center justify-between text-[8px] mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono" style={{ color: c.accuracy >= 90 ? "#6b8f71" : c.accuracy >= 80 ? "#c8953c" : "#c45a4a" }}>{c.accuracy}%</span>
                  </div>
                </div>
                <div className="h-1 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.05)" }}>
                  <div className="h-full rounded-full" style={{ width: `${c.accuracy}%`, background: c.accuracy >= 90 ? "#6b8f71" : c.accuracy >= 80 ? "#c8953c" : "#c45a4a" }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {["all", "accepted", "rejected", "modified"].map(o => (
          <button key={o} onClick={() => setFilterOutcome(o)} className="px-2.5 py-1 rounded text-[9px] font-medium capitalize transition-all" style={{ background: filterOutcome === o ? "rgba(74,144,184,0.12)" : "rgba(255,255,255,0.04)", border: `1px solid ${filterOutcome === o ? "rgba(74,144,184,0.25)" : BORDER.subtle}`, color: filterOutcome === o ? "#4a90b8" : TEXT.secondary }}>
            {o === "all" ? "All outcomes" : o}
          </button>
        ))}
        <div className="w-px h-5 self-center" style={{ background: BORDER.subtle }} />
        {["all", ...packs].map(p => (
          <button key={p} onClick={() => setFilterPack(p)} className="px-2.5 py-1 rounded text-[9px] font-medium capitalize transition-all" style={{ background: filterPack === p ? "rgba(74,144,184,0.12)" : "rgba(255,255,255,0.04)", border: `1px solid ${filterPack === p ? "rgba(74,144,184,0.25)" : BORDER.subtle}`, color: filterPack === p ? "#4a90b8" : TEXT.secondary }}>
            {p === "all" ? "All packs" : p}
          </button>
        ))}
      </div>
