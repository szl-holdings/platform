...
import { Shield, AlertTriangle, Clock, CheckCircle2, TrendingDown, Zap, Users, ChevronDown, ChevronUp } from "lucide-react";
import { workflowItems, type WorkflowItem } from "@/data/seed";

const STATUS_CONFIG = {
  on_track: { label: "ON TRACK", color: "text-emerald-400", bg: "bg-emerald-500/8", border: "border-emerald-500/20", bar: "#34d399" },
  at_risk: { label: "AT RISK", color: "text-orange-400", bg: "bg-orange-500/8", border: "border-orange-500/20", bar: "#fb923c" },
  stalled: { label: "STALLED", color: "text-red-400", bg: "bg-red-500/8", border: "border-red-500/25", bar: "#f87171" },
  blocked: { label: "BLOCKED", color: "text-red-500", bg: "bg-red-500/10", border: "border-red-500/30", bar: "#ef4444" },
  complete: { label: "COMPLETE", color: "text-emerald-400", bg: "bg-emerald-500/5", border: "border-emerald-500/10", bar: "#6ee7b7" },
};
...
function WorkflowCard({ wf }: { wf: WorkflowItem }) {
  ...
  return (
    <div className={`cockpit-panel border ${cfg.border} ${wf.status === "blocked" || wf.status === "stalled" ? "border-l-2" : ""}`}>
      <div className="flex items-start gap-3 p-4 cursor-pointer hover:bg-amber-500/3 transition-colors" onClick={() => setExpanded(v => !v)}>
        <div className={`w-8 h-8 rounded flex items-center justify-center shrink-0 mt-0.5 ${cfg.bg} border ${cfg.border}`}>
          {wf.status === "on_track" ? <CheckCircle2 className={`w-4 h-4 ${cfg.color}`} /> :
           wf.status === "blocked" ? <AlertTriangle className={`w-4 h-4 ${cfg.color}`} /> :
           wf.status === "stalled" ? <Clock className={`w-4 h-4 ${cfg.color}`} /> :
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <div className="flex items-center gap-3 mt-0.5">
                <span className="text-[10px] text-amber-400/40 font-mono">{TYPE_LABELS[wf.type]} · {wf.owner}</span>
              </div>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              {wf.slaBreach && <span className="text-[9px] font-mono px-1.5 py-0.5 rounded border text-red-400 bg-red-500/8 border-red-500/20">SLA BREACH</span>}
        ...
        <div className="px-4 pb-4 space-y-3 border-t border-amber-500/10 pt-3">
          {wf.bottleneckStep && (
            <div className="rounded bg-red-500/5 border border-red-500/15 p-3">
              ...
              {wf.bottleneckOwner && <p className="text-[10px] text-amber-100/50 mt-0.5">Owner: {wf.bottleneckOwner}</p>}
            </div>
          )}
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-5">
      ...
      <div>
        ...
        <p className="text-xs text-amber-400/50 mt-0.5">Bottleneck detection, value-at-risk, and SLA breach status across all tracked workflows</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="cockpit-panel p-4">
          ...
          <p className={`text-2xl font-mono font-bold ${healthScore >= 70 ? "text-emerald-400" : healthScore >= 50 ? "text-amber-400" : "text-red-400"}`}>{healthScore}%</p>
          <p className="text-[10px] text-amber-400/40 mt-1">{healthy.length} of {workflowItems.length} on track</p>
        </div>
        <div className="cockpit-panel p-4 border border-orange-500/20">
          ...
          <p className="text-2xl font-mono font-bold text-orange-400">${(totalAtRisk / 1e6).toFixed(1)}M</p>
      ...
      {critical.length > 0 && (
        <div className="space-y-3">
          <p className="text-[9px] font-mono text-red-400/50 uppercase tracking-widest">Blocked / Stalled ({critical.length})</p>
          {critical.map(wf => <WorkflowCard key={wf.id} wf={wf} />)}
        </div>
      )}
      ...
      {atRisk.length > 0 && (
        <div className="space-y-3">
          <p className="text-[9px] font-mono text-orange-400/50 uppercase tracking-widest">At Risk ({atRisk.length})</p>
          {atRisk.map(wf => <WorkflowCard key={wf.id} wf={wf} />)}
        </div>
      )}
      ...
      {healthy.length > 0 && (
        <div className="space-y-3">
          <p className="text-[9px] font-mono text-emerald-400/30 uppercase tracking-widest">On Track ({healthy.length})</p>
          {healthy.map(wf => <WorkflowCard key={wf.id} wf={wf} />)}
        </div>
      )}
    </div>
  );
