...
function AutonomyBadge({ mode }: { mode: AutonomyMode }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-medium ${cfg.color} ${cfg.bg}`}
    >
      <span>{cfg.icon}</span>
      {cfg.label}
    </span>
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        {/* Severity indicator */}
        <div
          className={`w-1 h-full min-h-[40px] rounded-full shrink-0 mt-0.5 ${SEVERITY_CFG[card.severity].color.replace('text-', 'bg-')}`}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 flex-wrap mb-1.5">
            <SeverityBadge severity={card.severity} />
            <span className={`text-[9px] font-medium ${domainCfg.color}`}>
          ...
          {/* Title */}
          <p className="text-sm font-semibold text-amber-100 leading-snug line-clamp-2 mb-1.5">
            {card.title}
          </p>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span
              className={`text-[9px] font-medium ${POLICY_CFG[card.policyState]?.color ?? 'text-slate-400'}`}
            >
          ...
          {Array.isArray(card.entityScope) && card.entityScope.length > 0 && (
            <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
              {(card.entityScope as string[]).slice(0, 4).map((e, i) => (
                <span
                  key={i}
                  className="px-1.5 py-0.5 rounded bg-amber-500/6 border border-amber-500/15 text-[9px] text-amber-300/60"
                >
                  {e}
                </span>
              ))}
            ...
            <div className="flex items-start justify-between gap-2 mb-1.5">
              <div className="flex items-center gap-1.5">
                <span className="text-amber-400/50">
                  {SOURCE_TYPE_ICONS[ev.sourceType] ?? <Activity className="w-3 h-3" />}
                </span>
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <span className="text-[9px] text-amber-400/50">
                  Conf:{' '}
                  <span className="text-amber-400/80">{(ev.confidence * 100).toFixed(0)}%</span>
                </span>
              </div>
            </div>
            <p className="text-[11px] text-amber-100/80 mb-1">{ev.value}</p>
            {ev.excerpt && (
              <p className="text-[10px] text-amber-400/50 italic border-l border-amber-500/20 pl-2 mb-1">
                {ev.excerpt}
              </p>
            )}
...
const CHECK_LABELS: Record<string, { label: string; desc: string }> = {
  contradiction: {
    ...
    desc: 'ACH — does any evidence contradict the recommendation?',
  },
  'stale-data': {
    label: 'Stale-Data Check',
    desc: 'I&W freshness — is any critical evidence expired?',
  },
  'missing-evidence': {
    label: 'Missing Evidence',
    desc: 'Key assumptions — are required evidence types absent?',
  },
  policy: {
    label: 'Policy Check',
    desc: 'Constitution — does the action fall within workspace policy?',
  },
  'confidence-floor': {
    label: 'Confidence Floor',
    desc: 'Uncertainty — is composite confidence above workspace minimum?',
  },
  falsification: {
    label: 'Falsification Prompt',
    desc: "Devil's advocacy — what would invalidate this recommendation?",
  },
};
  ...
  return (
    <div className="space-y-3">
      <p className="text-[10px] text-amber-400/40 leading-relaxed">
        Six structured adversarial checks run before every card is promoted to "ready for review".
        Based on intelligence tradecraft: Analysis of Competing Hypotheses (ACH), Indicators &
        Warnings (I&W), Key Assumptions Check, and Devil's Advocacy.
      </p>
      {validations.map((v) => {
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              ...
              <span
                className={`text-[10px] font-semibold ${!v.passed && v.severity === 'blocking' ? 'text-red-300' : v.severity === 'warning' ? 'text-amber-300' : 'text-emerald-300'}`}
              >
                {info.label}
              </span>
              <span
...
function RunTraceTab({
  ...
  run: NonNullable<import('@/data/decisions-api').DecisionDetail['runTrace']>;
}) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        {[
          {
            label: 'Total Latency',
            value: run.totalLatencyMs ? `${run.totalLatencyMs.toLocaleString()}ms` : '—',
          },
          ...
          {
            label: 'Est. Cost',
            value: run.estimatedCostUsd ? `$${run.estimatedCostUsd.toFixed(4)}` : '—',
          },
        ].map(({ label, value }) => (
      ...
      <div className="flex items-center gap-3 flex-wrap">
        {(run.modelsCalled as string[]).map((m) => (
          <span
            key={m}
            className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] bg-violet-500/10 text-violet-300 border border-violet-500/20"
          >
        ...
        {(run.toolsCalled as string[]).map((t) => (
          <span
            key={t}
            className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] bg-sky-500/10 text-sky-300 border border-sky-500/20"
          >
      ...
      <div className="space-y-2">
        <p className="text-[9px] font-mono text-amber-400/30 uppercase tracking-wider">
          Step Trace
        </p>
        {steps.map((step, idx) => (
          <div
            key={idx}
            className="flex items-start gap-2.5 p-2.5 rounded-lg bg-white/2 border border-amber-500/8"
          >
            <div
              className={`w-5 h-5 rounded flex items-center justify-center shrink-0 mt-0.5 ${
                step.stepType === 'model-call'
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2">
              <span className="text-[9px] text-amber-400/60">
                {evt.actorDisplay ?? evt.actorId}
              </span>
              {evt.reason && (
                <span className="text-[9px] text-amber-400/40 italic">"{evt.reason}"</span>
              )}
            </div>
            {evt.previousStatus && (
...
function DetailDrawer({ cardId, onClose }: { cardId: string; onClose: () => void }) {
  ...
  const canAct =
    ...
    (card.status === 'ready-for-review' ||
      card.status === 'validation-pending' ||
      card.status === 'draft');
      ...
      >
        ...
        <div className="px-5 py-4 border-b border-amber-500/10 shrink-0">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              {card && (
                <div className="flex items-center gap-2 flex-wrap mb-1.5">
                  ...
                  <AutonomyBadge mode={card.autonomyMode} />
                  <span
                    className={`text-[9px] ${DOMAIN_CFG[card.domain]?.color ?? 'text-slate-400'}`}
                  >
            ...
            <button
              onClick={onClose}
              className="text-amber-400/40 hover:text-amber-300 transition-colors text-lg leading-none shrink-0"
            >
              ✕
            </button>
          </div>
          ...
          <div className="px-5 py-3 border-b border-amber-500/8 shrink-0">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div>
                ...
                <p
                  className={`text-[10px] font-medium ${POLICY_CFG[card.policyState]?.color ?? 'text-slate-400'}`}
                >
              ...
              <div className="mt-2 p-2 rounded bg-amber-500/5 border border-amber-500/10">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[9px] text-amber-400/50">
                    Policy:{' '}
                    <span
                      className={`font-medium ${card.policyEvaluation.decision === 'allow' ? 'text-emerald-400' : card.policyEvaluation.decision === 'block' ? 'text-red-400' : 'text-amber-400'}`}
                    >
                      {card.policyEvaluation.decision.toUpperCase()}
                    </span>
                    {card.policyEvaluation.requiredApproverRoles &&
                      card.policyEvaluation.requiredApproverRoles.length > 0 && (
            ...
            <>
              ...
              {tab === 'run-trace' &&
                (detail.runTrace ? (
                  ...
                  <p className="text-[11px] text-amber-400/40 text-center py-8">
                    No run trace recorded
                  </p>
                ))}
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-[11px] text-amber-400/50 max-w-xl">
              Every card carries a full proof-chain — evidence, adversarial validation, run trace,
              and audit trail — backed by a real Postgres runtime with workspace constitution
              governance.
            </p>
          </div>
          <button
      ...
      <div className="divide-y divide-amber-500/5">
        ...
        {!isLoading && !error && cards.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-[10px] text-amber-400/25 mt-1">
              Try changing the filters or wait for the seed to complete
            </p>
          </div>
        )}
        {cards.map((card) => (
