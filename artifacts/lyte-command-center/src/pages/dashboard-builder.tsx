...
import {
  ...
  TrendingUp, Shield, Edit3, Layers, Zap, Share2, Check, Link
} from "lucide-react";
...
const WIDGET_CATALOG: Array<{
  ...
  { type: "metric_card", label: "Metric Card", icon: BarChart3, description: "Display a single KPI with trend indicator", defaultSize: "sm", defaultConfig: { metric: "signals_today", label: "Signals Today", format: "number" } },
  { type: "time_series", label: "Time Series Chart", icon: TrendingUp, description: "Line or area chart over time", defaultSize: "lg", defaultConfig: { metric: "signal_volume", window: "24h", chartType: "area" } },
  { type: "table", label: "Data Table", icon: Table, description: "Tabular view of signals, actions, or workflows", defaultSize: "lg", defaultConfig: { source: "signals", columns: ["title", "severity", "status", "createdAt"], limit: 10 } },
  { type: "alert_feed", label: "Alert Feed", icon: AlertTriangle, description: "Live feed of critical alerts and signals", defaultSize: "md", defaultConfig: { severities: ["critical", "high"], limit: 8 } },
  { type: "topology_map", label: "Topology Mini-Map", icon: Network, description: "Service dependency and ownership map", defaultSize: "xl", defaultConfig: { depth: 2, focus: "platform" } },
  { type: "status_indicator", label: "Status Indicator", icon: Circle, description: "Single-metric health status with threshold bands", defaultSize: "sm", defaultConfig: { metric: "platform_health", thresholds: { warning: 70, critical: 50 } } },
];
...
const TEMPLATES: Array<{
...
}> = [
  {
    ...
    widgets: [
      ...
      { type: "metric_card", title: "Open Actions", config: { metric: "actions_open", label: "Open Actions", format: "number" }, gridSize: "sm", col: 2, row: 0 },
      { type: "status_indicator", title: "Platform Health", config: { metric: "platform_health", thresholds: { warning: 70, critical: 50 } }, gridSize: "sm", col: 3, row: 0 },
      { type: "time_series", title: "Signal Volume (24h)", config: { metric: "signal_volume", window: "24h", chartType: "area" }, gridSize: "xl", col: 0, row: 1 },
      { type: "alert_feed", title: "Critical Alerts", config: { severities: ["critical", "high"], limit: 6 }, gridSize: "md", col: 0, row: 2 },
      { type: "table", title: "Top Actions", config: { source: "actions", columns: ["title", "urgency", "owner", "valueProtected"], limit: 5 }, gridSize: "md", col: 1, row: 2 },
    ],
  },
  {
    id: "engineering_ops",
    ...
    widgets: [
      ...
      { type: "metric_card", title: "p99 Latency", config: { metric: "api_p99_ms", label: "API p99", format: "ms" }, gridSize: "sm", col: 2, row: 0 },
      { type: "status_indicator", title: "API Health", config: { metric: "api_health", thresholds: { warning: 85, critical: 70 } }, gridSize: "sm", col: 3, row: 0 },
      { type: "topology_map", title: "Service Topology", config: { depth: 2, focus: "platform" }, gridSize: "xl", col: 0, row: 1 },
      { type: "time_series", title: "Workflow Run Volume", config: { metric: "run_volume", window: "12h", chartType: "bar" }, gridSize: "lg", col: 0, row: 2 },
      { type: "table", title: "Recent Failed Runs", config: { source: "runs", filter: "failed", columns: ["id", "workflowName", "startedAt", "errorMessage"], limit: 8 }, gridSize: "lg", col: 1, row: 2 },
    ],
  },
  {
    id: "security_posture",
    ...
    widgets: [
      ...
      { type: "metric_card", title: "Escalations", config: { metric: "escalations_open", label: "Escalations", format: "number" }, gridSize: "sm", col: 2, row: 0 },
      { type: "status_indicator", title: "Security Posture", config: { metric: "security_posture", thresholds: { warning: 75, critical: 60 } }, gridSize: "sm", col: 3, row: 0 },
      { type: "alert_feed", title: "Security Alerts", config: { severities: ["critical", "high"], filter: "security", limit: 10 }, gridSize: "lg", col: 0, row: 1 },
      { type: "time_series", title: "Threat Volume (7d)", config: { metric: "threat_volume", window: "7d", chartType: "area" }, gridSize: "lg", col: 1, row: 1 },
      { type: "table", title: "Open Escalations", config: { source: "escalations", columns: ["title", "severity", "assignee", "dueAt"], limit: 8 }, gridSize: "xl", col: 0, row: 2 },
    ],
  },
];
...
function TimeSeriesWidget({ widget }: { widget: Widget }) {
  ...
  return (
    <div className="h-full flex flex-col p-1">
      <div className="text-[10px] mb-1" style={{ color: "rgba(255,255,255,0.35)" }}>Last {cfg.window ?? "24h"}</div>
...
function TableWidget({ widget }: { widget: Widget }) {
  ...
  return (
    <div className="h-full flex flex-col p-1 overflow-hidden">
      <div className="grid gap-1 text-[9px] font-medium uppercase tracking-wider pb-1 border-b" style={{ gridTemplateColumns: `repeat(${cols.length}, 1fr)`, borderColor: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.3)" }}>
...
function AlertFeedWidget({ widget }: { widget: Widget }) {
  const alerts = [
    { id: 1, title: "Salesforce API timeout — CRM sync interrupted", severity: "critical", time: "2m ago" },
    { id: 2, title: "Approval SLA breach — contract renewal 72h", severity: "high", time: "15m ago" },
    ...
    <div className="h-full p-1 relative">
      <svg viewBox="0 0 100 100" className="w-full h-full">
        {edges.map(([a, b], i) => {
          ...
          return <line key={i} x1={from.x} y1={from.y} x2={to.x} y2={to.y} stroke="rgba(255,255,255,0.1)" strokeWidth="0.5" />;
        })}
        {nodes.map(n => (
          <g key={n.id}>
            <circle cx={n.x} cy={n.y} r="4" fill={`${n.color}20`} stroke={n.color} strokeWidth="0.8" />
            <text x={n.x} y={n.y + 8} textAnchor="middle" fontSize="3.5" fill={`${n.color}90`}>{n.label}</text>
          </g>
        ))}
      </svg>
    </div>
...
function StatusIndicatorWidget({ widget }: { widget: Widget }) {
  const cfg = widget.config as { metric: string; thresholds: { warning: number; critical: number } };
    ...
    <div className="h-full flex flex-col items-center justify-center p-1">
      <div className="relative">
        <svg viewBox="-40 -40 80 80" className="w-20 h-20">
          <circle cx="0" cy="0" r="28" fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="4" />
          <circle cx="0" cy="0" r="28" fill="none" stroke={color} strokeWidth="4"
            strokeDasharray={`${pct} 280`} strokeDashoffset="70" strokeLinecap="round" style={{ transition: "stroke-dasharray 1s ease" }} />
        </svg>
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="font-bold text-sm font-mono" style={{ color }}>{value}</span>
          <span className="text-[8px] uppercase tracking-wider" style={{ color: "rgba(255,255,255,0.3)" }}>{label}</span>
        </div>
      </div>
    </div>
  );
...
function WidgetRenderer({ widget }: { widget: Widget }) {
  switch (widget.type) {
    case "metric_card": return <MetricCardWidget widget={widget} />;
    case "time_series": return <TimeSeriesWidget widget={widget} />;
    case "table": return <TableWidget widget={widget} />;
    case "alert_feed": return <AlertFeedWidget widget={widget} />;
    case "topology_map": return <TopologyWidget />;
    case "status_indicator": return <StatusIndicatorWidget widget={widget} />;
    default: return <div className="h-full flex items-center justify-center text-[11px]" style={{ color: "rgba(255,255,255,0.3)" }}>Unknown widget</div>;
  }
}

function SortableWidget({
  widget, editing, onRemove, onResize, dragOverlay = false,
}: {
  widget: Widget;
  editing: boolean;
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={cn("rounded-xl border relative flex flex-col overflow-hidden transition-colors", GRID_SIZE_CLASSES[widget.gridSize], dragOverlay && "shadow-2xl shadow-black/60")}
      {...attributes}
    >
      <div
        ...
        {editing && (
          <div className="flex items-center gap-1">
            <div className="relative">
              ...
              {showResize && (
                <div className="absolute right-0 top-full mt-1 z-50 rounded-lg border p-1" style={{ background: "#0c1420", borderColor: "rgba(255,255,255,0.1)" }}>
                  {SIZES.map(size => (
                    <button
                      key={size}
      ...
      <div
        ...
        style={{
          ...
          minHeight: widget.type === "topology_map" ? 200 : widget.type === "time_series" ? 160 : widget.type === "table" ? 180 : 120,
...
export default function DashboardBuilder() {
  ...
  const handleSave = () => { if (activeDashboard) saveMutation.mutate(activeDashboard); };
  ...
  const handleAddWidget = (type: WidgetType) => {
    ...
    setActiveDashboard(prev => prev ? { ...prev, widgets: [...prev.widgets, newWidget] } : prev);
    setShowCatalog(false);
  };

  const handleRemoveWidget = useCallback((widgetId: string) => {
    setActiveDashboard(prev => prev ? { ...prev, widgets: prev.widgets.filter(w => w.id !== widgetId) } : prev);
  }, []);
  ...
  const handleCopyShareLink = () => {
    ...
    if (!token) return;
    const url = `${window.location.origin}/lyte-command-center/shared/${token}`;
  ...
  const activeWidget = dragActiveId ? activeDashboard?.widgets.find(w => w.id === dragActiveId) : null;
  ...
  if (showTemplates) {
    return (
      <div className="max-w-5xl mx-auto space-y-8 p-6">
        ...
        {dashboards.length > 0 && (
          <div>
            <div className="text-[11px] uppercase tracking-widest font-medium mb-3" style={{ color: "rgba(255,255,255,0.3)" }}>Your Dashboards</div>
        ...
        <div>
          <div className="text-[11px] uppercase tracking-widest font-medium mb-3" style={{ color: "rgba(255,255,255,0.3)" }}>Pre-Built Templates</div>
            ...
            {TEMPLATES.map(t => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[11px] leading-relaxed mb-3" style={{ color: "rgba(255,255,255,0.4)" }}>{t.description}</p>
                  <div className="text-[10px]" style={{ color: `${t.accentColor}80` }}>{t.widgets.length} pre-configured widgets</div>
                </button>
              );
            })}
          </div>
        ...
        <div>
          <div className="text-[11px] uppercase tracking-widest font-medium mb-3" style={{ color: "rgba(255,255,255,0.3)" }}>Start from Scratch</div>
          {showNewDashboard ? (
            <div className="flex items-center gap-3 p-4 rounded-xl border" style={{ borderColor: "rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.02)" }}>
              <input
                type="text"
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="shrink-0 flex items-center gap-3 px-6 py-3 border-b" style={{ borderColor: "rgba(255,255,255,0.06)", background: "rgba(8,12,20,0.8)" }}>
        <button
      ...
      <div className="flex-1 overflow-auto p-6">
        {activeDashboard && activeDashboard.widgets.length === 0 && (
          <div className="flex flex-col items-center justify-center h-64 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-[11px]" style={{ color: "rgba(255,255,255,0.35)" }}>Click "Edit" then "Add Widget" to start building</p>
            </div>
            <button
        ...
        {activeDashboard && activeDashboard.widgets.length > 0 && (
          ...
          >
            <SortableContext items={activeDashboard.widgets.map(w => w.id)} strategy={rectSortingStrategy}>
      ...
      {showCatalog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.7)" }} onClick={() => setShowCatalog(false)}>
          <div className="w-full max-w-lg rounded-2xl border p-6" style={{ background: "#0c1420", borderColor: "rgba(255,255,255,0.1)" }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-bold text-white">Widget Catalog</h2>
              <button onClick={() => setShowCatalog(false)} className="p-1 rounded hover:bg-white/5 transition-colors" style={{ color: "rgba(255,255,255,0.4)" }}>
                <X className="w-4 h-4" />
              </button>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-[10px]" style={{ color: "rgba(255,255,255,0.4)" }}>{item.description}</div>
                    </div>
                  </button>
                );
              })}
