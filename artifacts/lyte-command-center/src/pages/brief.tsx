...
import { FileText, ArrowLeft, Shield, Target, Zap, Brain, Eye, Lock } from "lucide-react";
...
const ROADMAP = [
  { phase: "Days 1–30", items: ["Decision Center GA (Postgres-backed, 12-card seed)", "Policy DSL v1 with domain-scoped rules", "Adversarial validation across all six check types", "Audit event stream per workspace"] },
  { phase: "Days 31–60", items: ["Real-time policy evaluation endpoint", "Entity Graph integration (cards anchor to ontology objects)", "Autonomy mode enforcement at API layer", "Webhook fan-out for approved decisions"] },
  { phase: "Days 61–90", items: ["Multi-workspace constitution editor", "AI-assisted card drafting with evidence pre-fill", "Investor-grade audit report export", "SOC 2 evidence collection hooks"] },
];
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Link href="/" className="flex items-center gap-1.5 text-[10px] text-amber-400/50 hover:text-amber-300 transition-colors">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-semibold text-amber-100 font-display">Lyte Differentiation Brief</h1>
              <p className="text-[11px] text-amber-400/50 mt-0.5">v1.0 · April 19, 2026 · Strategy — investor diligence, enterprise sales, engineering alignment</p>
            </div>
          </div>
        </div>
      ...
      <div className="cockpit-panel p-5 border border-amber-500/15">
        <p className="text-[10px] font-mono text-amber-400/40 uppercase tracking-wider mb-3">Executive Summary</p>
        <p className="text-sm text-amber-300/80 leading-relaxed">
          Every "AI ops" vendor ships observability or automation. None ship governed decision infrastructure.
          Lyte's moat is the combination that none of them have packaged cleanly: a <strong className="text-amber-200">proof-chain on every card</strong>,{" "}
          ...
          <strong className="text-amber-200">autonomy modes</strong> that let operators dial between observe-only and auto-execute.
          This brief maps the competitive landscape, names the five moves only Lyte makes, and sets a 90-day roadmap for deepening the moat.
        </p>
      </div>
        ...
        <div className="cockpit-panel divide-y divide-amber-500/5">
          {COMPETITORS.map((c, i) => (
            <div key={i} className="px-4 py-3 grid grid-cols-[100px_1fr_1fr] gap-4">
              ...
              <div>
                <p className="text-[10px] font-mono text-amber-400/40 uppercase mb-1">Lyte's Delta</p>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          {ROADMAP.map((phase, i) => (
            <div key={i} className="cockpit-panel p-4 border border-amber-500/10">
              <p className="text-[10px] font-mono text-amber-400/40 uppercase tracking-wider mb-3">{phase.phase}</p>
      ...
      <div className="cockpit-panel p-5 border border-amber-500/20 bg-amber-500/5 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-amber-400/50 mt-0.5">Every claim in this brief is backed by live decision cards in the Decision Center.</p>
        </div>
        <Link href="/decisions" className="px-4 py-2 rounded border border-amber-500/30 bg-amber-500/10 hover:bg-amber-500/15 text-xs font-semibold text-amber-300 transition-colors">
