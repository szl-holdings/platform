import { Puzzle, Box, Database, Bell, Layers } from "lucide-react";
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-text-secondary">Extend the SZL Holdings platform with custom widgets, data sources, and alert rules. The plugin architecture provides a defined interface for third-party integrations.</p>
      </div>

      <div className="grid md:grid-cols-3 gap-4">
        {[
          { icon: Box, title: "Widgets", desc: "Custom dashboard widgets that render in any app's dashboard", color: "text-info" },
          { icon: Database, title: "Data Sources", desc: "Add new data feeds and external data integrations", color: "text-success" },
          { icon: Bell, title: "Alert Rules", desc: "Custom alerting conditions and automated response actions", color: "text-warning" },
        ].map((cap) => (
      ...
      <div className="bg-surface rounded-xl border border-border p-6 space-y-4">
        ...
        <p className="text-text-secondary text-sm">Every plugin declares a manifest describing its identity and capabilities:</p>
        <pre><code>{`import { definePlugin } from "@szl-holdings/sdk/plugins";
  ...
  async destroy() {
    console.log("Plugin cleaned up");
      ...
      <div className="bg-surface rounded-xl border border-border p-6 space-y-4">
        ...
        <p className="text-text-secondary text-sm">Register and manage plugins using the central registry:</p>
        <pre><code>{`import { pluginRegistry } from "@szl-holdings/sdk/plugins";
...
await pluginRegistry.unregister("my-org.custom-monitor");`}</code></pre>
      </div>
      ...
      <div className="bg-surface rounded-xl border border-border p-6 space-y-4">
        ...
        <div className="space-y-4">
          <div className="p-4 bg-surface-elevated rounded-lg">
            ...
            <pre><code>{`interface WidgetDefinition {
  ...
  domains: string[];       // Applicable domains ("*" for all)
          ...
          <div className="p-4 bg-surface-elevated rounded-lg">
            ...
            <pre><code>{`interface DataSourceDefinition {
  ...
  schema?: object;         // JSON Schema for returned data
          ...
          <div className="p-4 bg-surface-elevated rounded-lg">
            ...
            <pre><code>{`interface AlertRuleDefinition {
  ...
  actions: [{              // Actions to take
    ...
    config: Record<string, unknown>;
      ...
      <div className="bg-surface rounded-xl border border-accent/20 p-6">
        ...
        <p className="text-text-secondary text-sm mb-4">Plugins can target one or more domains. Use <code className="text-accent">"*"</code> to apply to all domains.</p>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {["security", "analytics", "maritime", "finance", "ai", "platform", "infrastructure", "observability"].map((domain) => (
