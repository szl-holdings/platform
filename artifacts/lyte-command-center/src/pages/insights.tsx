...
import { narrativeInsights, severityColors } from "@/lib/business-data";
...
export default function InsightsPage() {
  return (
    <div className="max-w-[860px] space-y-6">
      <div>
        <h1 className="font-display font-bold text-2xl text-white tracking-tight">Narrative Intelligence</h1>
        <p className="text-sm text-slate-400 mt-1">Raw signals translated into operating intelligence — decision-ready, not data-dump</p>
      </div>
            ...
            <article key={ins.id} className={cn("rounded-xl border p-5", c.border, c.bg)}>
              <div className="flex items-start justify-between gap-4 mb-3">
                <div className="flex items-start gap-3">
                  <div className={cn("w-1 self-stretch rounded-full shrink-0 mt-0.5", c.dot)} style={{ minHeight: 20 }} />
                  <div>
                    ...
                    <h2 className="font-display font-semibold text-base text-white/95 leading-snug">{ins.title}</h2>
                  </div>
                </div>
                <div className="shrink-0 text-right">
                  <div className={cn("font-mono font-bold text-lg", c.text)}>{formatCurrency(ins.valueAtRisk)}</div>
                  <div className={cn("text-[10px] flex items-center gap-1 justify-end mt-0.5", trendColor)}>
              ...
              <div className="flex items-center gap-3 mt-4 ml-4">
                <div className="flex gap-1">
                  {ins.signalIds.map(id => (
                    <span key={id} className="text-[9px] font-mono px-1.5 py-0.5 rounded border border-white/10 text-slate-500 bg-white/[0.02]">{id}</span>
                  ))}
                </div>
              </div>
            </article>
