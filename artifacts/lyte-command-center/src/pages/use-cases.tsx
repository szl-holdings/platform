...
import { Clock, AlertTriangle, TrendingDown, Users, BarChart3, CheckCircle2 } from "lucide-react";
...
const USE_CASES = [
  ...
  },
  {
    ...
    bg: "bg-violet-500/5",
    dot: "bg-violet-500",
    description: "Sales-to-CS, CS-to-delivery, SE-to-account — every handoff is a churn risk. Lyte tracks all of them.",
...
function ApprovalLatencyPanel() {
  ...
  const workflows = workflowLatencies.filter(w => w.name.includes("Approval") || w.name.includes("Discount"));
  return (
        ...
        return (
          <div key={sig.id} className={cn("p-4 rounded-xl border", c.border, c.bg)}>
            <div className="flex items-center gap-2 mb-2">
              <div className={cn("w-1.5 h-1.5 rounded-full", c.dot, sig.severity === "critical" && "animate-pulse")} />
...
function ForecastDriftPanel() {
  ...
  return (
    <div className="space-y-4">
      ...
      {forecastInsight && (
        <div className={cn("p-4 rounded-xl border", severityColors[forecastInsight.severity].border, severityColors[forecastInsight.severity].bg)}>
          <div className="text-[10px] text-slate-500 uppercase tracking-wide mb-2">Narrative Intelligence</div>
  ...
  return (
    <div className="max-w-[1000px] space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Five business observability scenarios — each with its own signals, insights, and evidence</p>
      </div>
