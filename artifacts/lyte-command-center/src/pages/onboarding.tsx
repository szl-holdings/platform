...
const USE_CASES = [
  {
    ...
    subtitle: 'Surface the signals my team should act on today',
  },
  {
    id: 'risk',
    title: 'Risk & compliance',
    subtitle: 'Govern decisions with proof, policy, and audit trail',
  },
  {
    id: 'exec',
    title: 'Executive briefing',
    subtitle: 'Roll up cross-domain status into one command view',
  },
  {
    id: 'advisory',
    title: 'Advisory / pilot',
    subtitle: 'Evaluate the platform with a guided demo',
  },
];
...
const LOOP_STEPS: { label: string; icon: React.ReactNode; blurb: string }[] = [
  {
    ...
    blurb: 'A business event is detected and surfaced in the queue.',
  },
  {
    label: 'Context',
    icon: <Compass className="w-3.5 h-3.5" />,
    blurb: 'Cross-domain intelligence enriches the signal with prior art.',
  },
  {
    label: 'Recommendation',
    icon: <Brain className="w-3.5 h-3.5" />,
    blurb: 'AI proposes an action with evidence and confidence score.',
  },
  {
    label: 'Simulation',
    icon: <Sparkles className="w-3.5 h-3.5" />,
    blurb: 'Monte Carlo models the risk of acting vs. waiting.',
  },
  {
    label: 'Policy',
    icon: <Shield className="w-3.5 h-3.5" />,
    blurb: 'Covenant Policy verifies the action is authorized.',
  },
  {
    label: 'Approval',
    icon: <CheckCircle2 className="w-3.5 h-3.5" />,
    blurb: 'Required human approvers are notified and confirm.',
  },
  {
    label: 'Execution',
    icon: <GitBranch className="w-3.5 h-3.5" />,
    blurb: 'The action runs as a tracked, durable workflow.',
  },
  {
    label: 'Proof',
    icon: <ScrollText className="w-3.5 h-3.5" />,
    blurb: 'An immutable Proof Chain record is sealed.',
  },
  {
    label: 'Outcome',
    icon: <CheckCircle2 className="w-3.5 h-3.5" />,
    blurb: 'Real-world result is captured and compared to the prediction.',
  },
];
...
function Panel({ children }: { children: React.ReactNode }) {
  return <div className="cockpit-panel border border-amber-500/20 p-6 rounded-md">{children}</div>;
}
...
export default function OnboardingPage() {
  ...
  const markComplete = (step: number, partial?: Partial<WizardState>) => {
    setState((prev) => {
      const completedSteps = Array.from(new Set([...prev.completedSteps, step])).sort(
        (a, b) => a - b,
      );
  ...
  return (
    <div className="max-w-4xl mx-auto py-6" data-testid="onboarding-wizard">
      ...
      {current === 1 && (
        <Panel>
          <h2 className="text-lg font-mono text-amber-200 mb-1">
            Step 01 — Tell us about your org
          </h2>
          ...
          <div className="grid gap-4">
            <label className="block">
              ...
              <input
                ...
                onChange={(e) =>
                  setState((p) => ({ ...p, org: { ...p.org, name: e.target.value } }))
                }
            ...
            <label className="block">
              ...
              <select
                ...
                onChange={(e) =>
                  setState((p) => ({ ...p, org: { ...p.org, industry: e.target.value } }))
                }
                ...
                {USE_CASES.map((u) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        setState((p) => ({ ...p, org: { ...p.org, primaryUseCase: u.id } }))
                      }
      ...
      {current === 2 && (
        <Panel>
          <h2 className="text-lg font-mono text-amber-200 mb-1">
            Step 02 — Seed your demo workspace
          </h2>
          <p className="text-xs text-amber-300/60 mb-5">
            One click provisions realistic signals, recommendations, policies, and a worked governed
            decision so you can explore the platform with real-feeling data. Nothing leaves your
            tenant.
          </p>
          <div className="grid sm:grid-cols-3 gap-3 mb-5">
            {[
              ...
              <div key={card.label} className="border border-amber-500/15 rounded p-3">
                <p className="text-[10px] font-mono text-amber-400/40 uppercase tracking-wider">
                  {card.label}
                </p>
          ...
          {state.demoSeeded ? (
            ...
            >
              ...
              <span className="text-sm text-emerald-300">
                Demo workspace ready. You can re-seed at any time from Settings.
              </span>
            </div>
          ) : (
      ...
      {current === 3 && (
        <Panel>
          <h2 className="text-lg font-mono text-amber-200 mb-1">
            Step 03 — Land in your first view
          </h2>
          <p className="text-xs text-amber-300/60 mb-5">
            ...
            {[
              {
                ...
                title: 'Overview',
                blurb: 'Cockpit metrics',
              },
              {
                href: '/signals',
                icon: <Radio className="w-4 h-4" />,
                title: 'Signals Console',
                blurb: 'Live ranked feed',
              },
              {
                href: '/decisions',
                icon: <Brain className="w-4 h-4" />,
                title: 'Decision Center',
                blurb: 'Recommendations queue',
              },
            ].map((c) => (
              <Link
                key={c.href}
      ...
      {current === 4 && (
        <Panel>
          <h2 className="text-lg font-mono text-amber-200 mb-1">
            Step 04 — Walk a governed decision loop
          </h2>
          <p className="text-xs text-amber-300/60 mb-5">
            Every action on the platform follows the same nine-step loop. Step through it once so
            the sequence is familiar before you take a real decision.
          </p>
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-[11px] text-emerald-300/70 mt-1">
              You can re-run this wizard any time from{' '}
