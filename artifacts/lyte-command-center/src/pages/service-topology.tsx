...
const PLATFORM_NODES = [
  ...
  { id: "escalation-mgr",  label: "Escalation Mgr",   icon: AlertTriangle, connects: ["notification-svc", "action-router"], platform: "Core" },
  ...
  { id: "notification-svc", label: "Notification Svc", icon: Globe,    connects: [], platform: "Core" },
  { id: "workflow-exec",    label: "Workflow Exec",    icon: Layers,   connects: ["metrics-store"], platform: "Alloy" },
  { id: "ml-inference",     label: "ML Inference",     icon: Database, connects: ["metrics-store"], platform: "Alloy" },
];
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: `${pColor}15`, border: `1px solid ${pColor}20` }}>
          ...
          <div>
            ...
            <div className="text-[9px]" style={{ color: "rgba(255,255,255,0.25)" }}>{node.platform}</div>
          </div>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <span className="w-1.5 h-1.5 rounded-full" style={{ background: hs.dot, boxShadow: health !== "healthy" ? `0 0 4px ${hs.dot}` : "none" }} />
          <span className="text-[9px] capitalize" style={{ color: hs.dot }}>{health}</span>
        </div>
      </div>
        ...
        <div className="grid grid-cols-3 gap-1 mt-1">
          {[
            ...
            <div key={c.label} className="text-center">
              ...
              <div className="text-[8px]" style={{ color: "rgba(255,255,255,0.2)" }}>{c.label}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {node.connects.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1">
          {node.connects.slice(0, 3).map(c => (
            <span key={c} className="text-[8px] px-1.5 py-0.5 rounded" style={{ background: "rgba(255,255,255,0.04)", color: "rgba(255,255,255,0.25)", border: "1px solid rgba(255,255,255,0.06)" }}>→ {c}</span>
          ))}
          {node.connects.length > 3 && <span className="text-[8px]" style={{ color: "rgba(255,255,255,0.2)" }}>+{node.connects.length - 3}</span>}
        </div>
      )}
    </div>
  );
...
export default function ServiceTopology() {
  ...
  for (const n of nodes) { metricsMap[n.service] = n; }

  const healthyPlatforms = PLATFORM_NODES.filter(n => {
    const m = metricsMap[n.id] ?? metricsMap[n.label] ?? metricsMap[n.label.toLowerCase().replace(/ /g, "-")];
  ...
  const selectedMetrics = selectedNode ? (metricsMap[selectedNode.id] ?? metricsMap[selectedNode.label.toLowerCase().replace(/ /g, "-")]) : null;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5">
      ...
      <div className="flex gap-5">
            ...
            return (
              <div key={platform}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: pColor }}>{platform}</span>
                  <div className="flex-1 h-px" style={{ background: `linear-gradient(to right, ${pColor}30, transparent)` }} />
                </div>
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                  {pNodes.map(node => (
                    <ServiceCard
                      key={node.id}
                      node={node}
                      metricsNode={metricsMap[node.id] ?? metricsMap[node.label.toLowerCase().replace(/ /g, "-")]}
        ...
        {selectedNode && (
          <div className="w-72 shrink-0 rounded-xl border p-4 h-fit sticky top-6" style={{ borderColor: `${PLATFORM_COLORS[selectedNode.platform]}20`, background: "rgba(255,255,255,0.015)" }}>
            ...
            {selectedMetrics ? (
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2 p-2 rounded-lg" style={{ background: "rgba(255,255,255,0.03)" }}>
                  <span className="w-2 h-2 rounded-full" style={{ background: HEALTH_STYLES[selectedMetrics.health].dot }} />
                  <span className="text-[11px] capitalize" style={{ color: HEALTH_STYLES[selectedMetrics.health].dot }}>{selectedMetrics.health}</span>
                </div>
              </div>
            ) : (
              <p className="text-[11px] text-slate-500">No live metrics for this node yet. Seed data to populate.</p>
            )}

            <div className="mt-3">
              <div className="text-[9px] uppercase tracking-wider mb-2" style={{ color: "rgba(255,255,255,0.25)" }}>Downstream Connections</div>
