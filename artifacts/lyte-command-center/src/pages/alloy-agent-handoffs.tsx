...
const STATUS_CFG: Record<HandoffStatus, { color: string; bg: string; icon: LucideIcon; label: string }> = {
  ...
  in_progress: { color: "#d4a054", bg: "rgba(212,160,84,0.1)",   icon: Activity,     label: "In Progress" },
  completed:   { color: "#6b8f71", bg: "rgba(107,143,113,0.1)",  icon: CheckCircle,  label: "Completed" },
...
const HANDOFFS: AgentHandoff[] = [
  {
    ...
    pack: "Vessels",
    packColor: "#38bdf8",
    evidence: ["3 vessels within storm radius", "2 viable alternate routes identified", "ETA delta: +14h average"],
    result: "Routes calculated: Northern bypass (14h delay) and Southern deviation (18h delay). Recommended: Northern bypass. Fleet Ops approval gate inserted.",
  },
  {
    id: "A2A-0040",
    fromAgent: "Alloy Orchestrator",
    ...
    context: "Assets: TA-007, TA-012 · Conflict type: overlapping ownership claims · Priority: HIGH",
    status: "completed",
    ...
    pack: "Terra",
    packColor: "#a07848",
    evidence: ["TA-007: NAV $14.2M, 3 conflicting owners", "TA-012: NAV $8.7M, 2 conflicting owners", "Resolution path: single-owner assignment"],
    result: "Valuations complete. Conflict resolution recommendations issued. Finance approval required for ownership re-assignment.",
  },
  {
    id: "A2A-0039",
    fromAgent: "Alloy Orchestrator",
  ...
  },
  {
    ...
    pack: "Vessels",
    packColor: "#38bdf8",
    evidence: ["Storm system detected 38.2N 151.4E", "Wind speed: 62 knots peak", "Forecast valid through 2026-04-04"],
    result: "72h forecast delivered in A2A structured format. Storm advisory in effect for Pacific corridor.",
  },
  {
    id: "A2A-0036",
    fromAgent: "Alloy Orchestrator",
...
function HandoffRow({ handoff }: { handoff: AgentHandoff }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="w-1.5 shrink-0 mt-1.5 h-10 rounded-full" style={{ background: STATUS_CFG[handoff.status].color + "50" }} />
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span className="text-[8px] font-bold px-1.5 py-0.5 rounded uppercase tracking-widest" style={{ color: handoff.packColor, background: `${handoff.packColor}14` }}>{handoff.pack}</span>
            <StatusBadge status={handoff.status} />
            <span className={`text-[7px] font-mono px-1.5 py-0.5 rounded`} style={{ color: handoff.toAgentType === "remote" ? "#c8953c" : "#4a90b8", background: handoff.toAgentType === "remote" ? "rgba(200,149,60,0.08)" : "rgba(74,144,184,0.08)" }}>
              {handoff.toAgentType === "remote" ? "REMOTE" : "INTERNAL"}
            </span>
            <span className="text-[7px] font-mono" style={{ color: TEXT.muted }}>{handoff.id}</span>
          </div>
          <div className="flex items-center gap-2 text-[9px] mb-0.5">
            <span className="font-mono" style={{ color: TEXT.tertiary }}>{handoff.fromAgent}</span>
            <ArrowRight className="w-2.5 h-2.5 shrink-0" style={{ color: TEXT.muted }} />
            <span className="font-mono" style={{ color: TEXT.secondary }}>{handoff.toAgent}</span>
          </div>
          <p className="text-[10px]" style={{ color: TEXT.primary }}>{handoff.subtask}</p>
          <div className="flex items-center gap-3 mt-1 text-[8px]" style={{ color: TEXT.muted }}>
            ...
            {handoff.confidence && <span>· {Math.round(handoff.confidence * 100)}% confidence</span>}
            {handoff.receiptId && (
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-3" style={{ borderTop: `1px solid ${BORDER.subtle}` }}>
          <div className="pt-3 text-[9px] space-y-1">
            <div className="text-[8px] uppercase tracking-widest mb-1.5" style={{ color: TEXT.muted }}>Context Passed</div>
          ...
          {handoff.evidence && (
            <div>
              <div className="text-[8px] uppercase tracking-widest mb-1.5" style={{ color: TEXT.muted }}>Evidence</div>
          ...
          {handoff.result && (
            <div className="rounded p-2.5" style={{ background: "rgba(107,143,113,0.04)", border: "1px solid rgba(107,143,113,0.12)" }}>
              <div className="text-[8px] uppercase tracking-widest mb-1" style={{ color: "#6b8f71" }}>Result</div>
              <p className="text-[9px]" style={{ color: TEXT.secondary }}>{handoff.result}</p>
            </div>
          )}

          {handoff.failureReason && (
            <div className="rounded p-2.5" style={{ background: "rgba(196,90,74,0.04)", border: "1px solid rgba(196,90,74,0.12)" }}>
              <div className="text-[8px] uppercase tracking-widest mb-1" style={{ color: "#c45a4a" }}>Failure Reason</div>
              <p className="text-[9px]" style={{ color: "#c45a4a" }}>{handoff.failureReason}</p>
            </div>
          )}

          {handoff.receiptId && (
            <div className="rounded p-2.5 flex items-center justify-between" style={{ background: "rgba(107,143,113,0.04)", border: "1px solid rgba(107,143,113,0.12)" }}>
              <div className="flex items-center gap-2">
                <FileText className="w-3 h-3" style={{ color: "#6b8f71" }} />
                <span className="text-[9px] font-mono" style={{ color: "#6b8f71" }}>Delegation Receipt: {handoff.receiptId}</span>
              </div>
              <span className="text-[8px] font-mono" style={{ color: TEXT.muted }}>Signed · Immutable</span>
            </div>
          )}
        </div>
      )}
...
const VALID_HANDOFF_STATUSES: HandoffStatus[] = ["pending", "accepted", "in_progress", "completed", "failed", "rejected"];
...
function mapApiHandoff(h: Record<string, unknown>): AgentHandoff {
  ...
  return {
    ...
    pack: String(h.sourcePackId ?? "Alloy"),
    packColor: "#d4a054",
    ...
    <div className="p-4 md:p-5 space-y-5" style={{ background: BG.page, minHeight: "100vh" }}>
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-[9px] font-mono uppercase tracking-widest" style={{ color: ACCENT }}>Alloy · A2A Protocol</span>
        </div>
        <h1 className="text-lg font-bold tracking-tight" style={{ color: TEXT.primary }}>Agent Handoffs</h1>
        <p className="text-[11px] mt-0.5" style={{ color: TEXT.secondary }}>
          A2A-compatible delegation protocol for subtask handoffs to specialized agents. Each delegation is tracked with a structured receipt.
        </p>
      </div>
      ...
      <div className="rounded-md p-3 flex items-start gap-2" style={{ background: "rgba(74,144,184,0.04)", border: "1px solid rgba(74,144,184,0.12)" }}>
        ...
        <p className="text-[9px] leading-relaxed" style={{ color: TEXT.secondary }}>
          Alloy implements the A2A (Agent-to-Agent) delegation protocol. When the orchestrator delegates a subtask, it sends a structured handoff with full context, receives a delegation receipt, and tracks status until completion. Internal agents and remote APIs are treated uniformly.
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        ] as { key: HandoffStatus; label: string; color: string }[]).map(m => (
          <button
            key={m.key}
            onClick={() => setFilter(m.key)}
            className="rounded-md p-3 text-center transition-all hover:opacity-80"
            style={{ background: filter === m.key ? `${m.color}10` : BG.surface, border: `1px solid ${filter === m.key ? m.color + "30" : BORDER.subtle}` }}
          >
            <div className="text-base font-bold font-mono" style={{ color: m.color }}>{counts[m.key as keyof typeof counts] ?? 0}</div>
            <div className="text-[8px] uppercase tracking-widest mt-0.5" style={{ color: filter === m.key ? m.color : TEXT.muted }}>{m.label}</div>
          </button>
        ))}
      </div>
      ...
      <div className="flex gap-0.5" style={{ borderBottom: `1px solid ${BORDER.subtle}` }}>
        ...
        <div className="ml-auto flex items-center px-3 text-[8px] gap-1.5" style={{ color: TEXT.muted }}>
          {isLoading
            ? <><RefreshCw className="w-2.5 h-2.5 animate-spin" /> Loading</>
            : apiHandoffs.length > 0
              ? <><span className="w-1.5 h-1.5 rounded-full bg-green-500/60" />{apiHandoffs.length} live</>
      ...
      <div className="space-y-2">
        {filtered.map(h => <HandoffRow key={h.id} handoff={h} />)}
        {filtered.length === 0 && (
          <div className="rounded-md py-12 flex flex-col items-center gap-3" style={{ background: BG.surface, border: `1px solid ${BORDER.subtle}` }}>
            ...
            <p className="text-[11px]" style={{ color: TEXT.tertiary }}>No handoffs in this state</p>
          </div>
        )}
      </div>

      <div className="rounded-md p-3.5" style={{ background: BG.surface, border: `1px solid ${BORDER.subtle}` }}>
        <div className="flex items-center gap-2 mb-3">
          <FileText className="w-3.5 h-3.5" style={{ color: "#6b8f71" }} />
          <span className="text-[11px] font-semibold" style={{ color: TEXT.primary }}>A2A Delegation Receipt Schema</span>
        </div>
        <div className="rounded p-3 font-mono text-[8px] leading-relaxed" style={{ background: BG.elevated, color: "#6b8f71" }}>
{`{
