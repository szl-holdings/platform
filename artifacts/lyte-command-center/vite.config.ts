...
const port = Number(process.env.PORT) || Number(process.env.VITE_PORT) || 19291;
...
const GATEWAY_ROUTES: Array<{ prefix: string; port: number }> = [
  ...
  { prefix: "/lyte-command-center/", port },
  ...
  return {
    ...
    async configureServer() {
      ...
      proxyServer.listen({ port: 9090, host: "0.0.0.0", reusePort: true }, () => {
        console.log("[lyte/gateway] Port 9090 (reusePort) joined — serving /lyte-command-center/");
      });
      proxyServer.on("error", (err: NodeJS.ErrnoException) => {
        console.warn("[lyte/gateway] Port 9090 bind:", err.code);
      });
    },
  };
}
