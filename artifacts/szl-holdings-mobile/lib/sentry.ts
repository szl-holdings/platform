...
export function captureMessage(message: string, level: "error" | "warning" | "info" = "info", extra?: Record<string, unknown>): void {
  if (!isSentryEnabled()) return;
  const parsed = parseDsn(DSN);
  if (!parsed) return;
...
export function initSentryGlobalHandlers(): void {
  ...
  if (typeof globalThis !== "undefined" && "addEventListener" in globalThis) {
    (globalThis as typeof globalThis & { addEventListener?: (type: string, fn: (e: PromiseRejectionEvent) => void) => void }).addEventListener?.(
      "unhandledrejection",
  ...
  if (typeof global !== "undefined" && typeof (global as Record<string, unknown>).HermesInternal !== "undefined") {
    const origTracked = (global as Record<string, unknown>).__trackAsyncError as ((fn: unknown, err: unknown) => void) | undefined;
  ...
  console.debug("[sentry] Mobile Sentry HTTP reporter initialized — global errors + promise rejections captured");
}
