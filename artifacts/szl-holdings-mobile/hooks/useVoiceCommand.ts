...
function generateResponse(
  ...
  const domainResponses: Record<WorkspaceDomain, { response: string; cards: VoiceResultCard[] }> = {
    command: {
      response:
        'Cross-domain status nominal. 2 high-priority signals require attention across Defense and Fleet.',
      cards: [
        {
          id: '1',
    ...
    defense: {
      response:
        'Defense posture elevated. 1 active incident, 3 critical CVEs pending patch validation.',
      cards: [
    ...
    properties: {
      response:
        'Portfolio performing well. 3 new distress signals detected in Miami-Dade corridor.',
      cards: [
    ...
    advisory: {
      response:
        '3 client sessions scheduled today. Pending document review for Blackstone engagement.',
      cards: [
