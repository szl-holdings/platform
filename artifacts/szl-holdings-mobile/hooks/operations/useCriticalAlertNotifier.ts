...
import type { LyteSignal, Severity } from '@/context/LyteContext';
import type { NotificationPreferences } from '@/context/NotificationContext';
...
export function useCriticalAlertNotifier(
  ...
  useEffect(() => {
    ...
    signals.forEach((signal) => {
      ...
      const notifBody =
        signal.title + (signal.recommendedAction ? ` — ${signal.recommendedAction}` : '');
