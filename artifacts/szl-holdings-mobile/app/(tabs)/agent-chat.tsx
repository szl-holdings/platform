...
const AGENTS = [
  ...
  { id: "beacon", name: "Terra Analytics", role: "Analytics", icon: "bar-chart-2", color: "#22c55e" },
...
export default function AgentChatScreen() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      role: "assistant",
      content: "Alloy orchestration active. I coordinate all domain agents across the SZL Holdings platform. Ask me anything about your portfolio, operations, or intelligence.",
      agentId: "alloy",
      agentName: "Alloy",
  ...
  const sendMessage = useCallback(async () => {
    ...
    const userMsg: Message = { id: `user_${Date.now()}`, role: "user", content: input.trim(), timestamp: Date.now() };
    setMessages(prev => [...prev, userMsg]);
    setInput("");
    setIsLoading(true);
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    const assistantId = `asst_${Date.now()}`;
    setMessages(prev => [...prev, { id: assistantId, role: "assistant", content: "", agentId: selectedAgent.id, agentName: selectedAgent.name, timestamp: Date.now(), streaming: true }]);
    try {
      const base = process.env.EXPO_PUBLIC_API_BASE_URL ?? "";
      const res = await fetch(`${base}/api/nuro-mesh/chat`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ agentId: selectedAgent.id, message: userMsg.content, stream: false }),
      });
      if (res.ok) {
        const data = await res.json();
        setMessages(prev => prev.map(m => m.id === assistantId ? { ...m, content: data.response ?? data.content ?? "Done.", streaming: false } : m));
      } else {
        throw new Error("API error");
      }
    } catch {
      const fallbacks: Record<string, string> = {
        alloy: "Portfolio metrics stable. Orchestrating across 7 domain agents. No critical escalations.",
      ...
      setMessages(prev => prev.map(m => m.id === assistantId ? { ...m, content: fallbacks[selectedAgent.id] ?? "Processing...", streaming: false } : m));
    } finally {
      setIsLoading(false);
    ...
    <KeyboardAvoidingView style={[styles.root, { paddingTop: insets.top }]} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      ...
      {showAgentPicker && (
        <Animated.View style={[styles.agentPicker, pickerStyle]}>
          {AGENTS.map(agent => (
            <TouchableOpacity key={agent.id} style={[styles.agentPickerRow, selectedAgent.id === agent.id && styles.agentPickerRowSelected]} onPress={() => switchAgent(agent)} activeOpacity={0.7}>
      ...
      <ScrollView ref={scrollRef} style={styles.messages} contentContainerStyle={styles.messagesContent} keyboardDismissMode="interactive" showsVerticalScrollIndicator={false}>
        {messages.map(msg => <MessageBubble key={msg.id} msg={msg} />)}
        {isLoading && <View style={styles.typingRow}><ActivityIndicator size="small" color={ACCENT} /><Text style={styles.typingText}>{selectedAgent.name} thinking…</Text></View>}
      </ScrollView>
      <View style={[styles.inputBar, { paddingBottom: insets.bottom + 8 }]}>
        <TextInput style={styles.input} value={input} onChangeText={setInput} placeholder="Ask your agents…" placeholderTextColor={TEXT_DIM} multiline maxLength={2000} />
        <TouchableOpacity style={[styles.sendBtn, (!input.trim() || isLoading) && styles.sendBtnDisabled]} onPress={sendMessage} disabled={!input.trim() || isLoading} activeOpacity={0.7}>
...
const styles = StyleSheet.create({
  ...
  statusPill: { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: `${ACCENT}15`, borderRadius: 20, paddingHorizontal: 8, paddingVertical: 3 },
  ...
  agentPicker: { backgroundColor: CARD, borderWidth: 1, borderColor: BORDER, marginHorizontal: 12, borderRadius: 12, overflow: "hidden", zIndex: 10 },
  ...
  agentAvatar: { width: 28, height: 28, borderRadius: 14, borderWidth: 1, alignItems: "center", justifyContent: "center" },
