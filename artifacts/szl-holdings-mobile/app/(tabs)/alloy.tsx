...
function useRuns() {
  return useQuery<{ data: WorkflowRun[] } | WorkflowRun[]>({
    ...
    queryFn: () => sharedApiFetch<{ data: WorkflowRun[] } | WorkflowRun[]>("/api/alloy/runs?limit=20"),
...
function useApprovals() {
  return useQuery<{ data: Approval[] } | Approval[]>({
    ...
    queryFn: () =>
      sharedApiFetch<{ data: Approval[] } | Approval[]>("/api/alloy/approvals?status=pending&limit=20"),
    refetchInterval: 60000,
    retry: 1,
  });
}
    ...
    <View style={[
      ...
      <View style={styles.runContent}>
        <View style={styles.runTopRow}>
          <Text style={[styles.runId, { color: colors.cream }]}>
            Run #{run.id}
          </Text>
          {run.workflowId && (
        ...
        <View style={styles.runBottomRow}>
          <Text style={[styles.runState, { color: cfg.color }]}>
            {cfg.label}
          </Text>
...
function ApprovalCard({
  ...
  return (
    <View style={[styles.approvalCard, { backgroundColor: colors.card, borderColor: "rgba(245,158,11,0.2)" }]}>
      ...
      <Text style={[styles.approvalRole, { color: colors.creamDim }]}>
        Requested from:{" "}
        <Text style={{ color: "#8b5cf6" }}>{approval.requestedFrom}</Text>
      </Text>
      {expires && (
      ...
      >
        ...
        <View style={styles.statsRow}>
          {[
            ...
            <View
              key={s.label}
              style={[styles.statCard, { backgroundColor: colors.card, borderColor: colors.borderSubtle }]}
            >
        ...
        <View style={styles.tabs}>
          <Pressable
            ...
            onPress={() => setTab("runs")}
          >
            <Text style={[styles.tabBtnText, { color: tab === "runs" ? "#8b5cf6" : colors.mutedForeground }]}>
              Recent Runs
            </Text>
          </Pressable>
          <Pressable
          ...
          >
            <Text style={[styles.tabBtnText, { color: tab === "approvals" ? "#f59e0b" : colors.mutedForeground }]}>
              Approvals
              ...
              <View style={[styles.runList, { borderColor: colors.borderSubtle }]}>
                {rawRuns.slice(0, 12).map((run) => (
                  <RunRow
                    key={run.id}
                    run={run}
                    isNew={recentlyUpdatedIds.has(run.id)}
                  />
                ))}
              </View>
            )}
