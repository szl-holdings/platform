...
import { VARIANT_RECOMMENDATIONS, VARIANT_SOURCE_HEALTH } from "@szl-holdings/shared-ui/os-demo-data";
        ...
        <View style={[styles.modalSheet, { backgroundColor: "#0e1117", borderColor: "#ffffff14" }]}>
          ...
          <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
            ...
            {[
              ...
              ...(rec.valueAtRisk != null ? [{ label: "Value at Risk", value: currency(rec.valueAtRisk), color: "#ef4444" }] : []),
              ...(rec.opportunityValue != null ? [{ label: "Opportunity", value: currency(rec.opportunityValue), color: "#4ade80" }] : []),
              ...
              <View key={row.label} style={styles.modalRow}>
                ...
                <Text style={[styles.rowValue, { color: "color" in row && row.color ? row.color : colors.text }]}>{row.value}</Text>
              </View>
            ))}
            ...
            <View style={styles.modalRow}>
              ...
              <View style={[styles.pill, { backgroundColor: vColor + "22", borderColor: vColor + "55" }]}>
            ...
            {/* Proposed action */}
            <View style={styles.section}>
              <Text style={[styles.sectionLabel, { color: colors.textMuted }]}>PROPOSED ACTION</Text>
              <Text style={[styles.bodyText, { color: colors.textSecondary }]}>{rec.proposedAction}</Text>
            </View>

            {/* Policy justification notice */}
            {mandatesJustification && !isHardBlocked && (
              <View style={[styles.notice, { borderColor: "#f59e0b33", backgroundColor: "#f59e0b0a" }]}>
            ...
            {pendingAction != null && (
              <View style={styles.section}>
                <Text style={[styles.sectionLabel, { color: colors.textMuted }]}>JUSTIFICATION FOR {pendingAction.toUpperCase()}</Text>
                <TextInput
                  value={justText}
                  ...
                  multiline
                  numberOfLines={3}
                  style={[styles.justInput, { backgroundColor: "#ffffff08", borderColor: "#ffffff18", color: colors.text }]}
                />
                <View style={{ flexDirection: "row", gap: 8, marginTop: 8 }}>
                  <TouchableOpacity
                    onPress={() => { setPendingAction(null); setJustText(""); }}
                  ...
                  <TouchableOpacity
                    ...
                    style={[styles.actionBtn, { flex: 1, borderColor: "#f59e0b44", backgroundColor: "#f59e0b14", opacity: justText.trim() ? 1 : 0.4 }]}
                  >
              ...
              <View style={[styles.section, { marginBottom: 32 }]}>
                {isExecuted ? (
                  <TouchableOpacity
                    ...
                    style={[styles.actionBtn, { borderColor: ACTION_COLORS.rollback.border, backgroundColor: ACTION_COLORS.rollback.bg }]}
                  >
                    <Text style={{ color: ACTION_COLORS.rollback.color, fontSize: 13, fontWeight: "600" }}>Rollback</Text>
                  </TouchableOpacity>
                ) : isHardBlocked ? (
                  <View style={[styles.notice, { borderColor: "#ef444433", backgroundColor: "#ef44440a" }]}>
                    ...
                    {(["approve", "reject", "escalate"] as RecommendationAction[]).map(action => {
                      ...
                      return (
                        <TouchableOpacity
                          ...
                          style={[styles.actionBtn, { flex: 1, borderColor: cfg.border, backgroundColor: cfg.bg }]}
                        >
            ...
            {auditLog.length > 0 && (
              <View style={[styles.section, { marginBottom: 32 }]}>
                <Text style={[styles.sectionLabel, { color: colors.textMuted }]}>SESSION AUDIT</Text>
                {auditLog.map((entry, i) => (
                  <View key={i} style={{ flexDirection: "row", gap: 8, paddingVertical: 4 }}>
                    <Text style={{ color: ACTION_COLORS[entry.action]?.color ?? colors.textMuted, fontSize: 10, fontWeight: "600" }}>
                      {entry.action}
                    </Text>
                    ...
                    {entry.justification && (
                      <Text style={{ color: colors.textSecondary, fontSize: 10, flex: 1 }} numberOfLines={2}>
                        "{entry.justification}"
                      </Text>
                    )}
                  </View>
...
function RecCard({ rec, onPress, colors }: RecCardProps) {
  ...
  return (
    <TouchableOpacity
      style={[styles.card, { backgroundColor: "#0e1117", borderColor: isHardBlocked ? "#ef444435" : "#ffffff12" }]}
      onPress={onPress}
      activeOpacity={0.85}
    >
      <View style={styles.cardHeader}>
        <View style={[styles.priorityBadge, { backgroundColor: vColor + "18", borderColor: vColor + "35" }]}>
      ...
      <Text style={[styles.cardTitle, { color: colors.text }]} numberOfLines={2}>{rec.title}</Text>
      <Text style={[styles.cardSummary, { color: colors.textSecondary }]} numberOfLines={2}>{rec.summary}</Text>
      ...
      <View style={styles.cardFooter}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", marginLeft: "auto" }}>
          ...
          <Text style={{ color: colors.textMuted, fontSize: 9, marginLeft: 3 }}>{rec.evidenceCount}</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
...
export default function DecisionsScreen() {
  ...
  function handleAction(action: RecommendationAction, _justification?: string) {
    ...
    setRecs(prev => prev.map(r =>
      r.id === selectedRec.id ? { ...r, status: newStatus } : r
    ));
  }

  const pendingRecs = recs.filter(r => r.status === "pending");
  const flaggedRecs = recs.filter(r => r.policyVerdict.verdict === "red" || r.policyVerdict.verdict === "blocked" || r.policyVerdict.verdict === "yellow");
    ...
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
      ...
      >
        {sources.map(s => (
          <View key={s.sourceId} style={styles.sourcePill}>
            <View style={[styles.sourceDot, { backgroundColor: s.status === "healthy" ? "#4ade80" : s.status === "degraded" ? "#f97316" : "#ef4444" }]} />
      ...
      <View style={styles.pageHeader}>
        <View>
          ...
          <Text style={[styles.pageSubtitle, { color: colors.textMuted }]}>Decisions, with receipts</Text>
        </View>
        <TouchableOpacity
      ...
      >
        ...
        {filtered.length === 0 && (
          <View style={styles.empty}>
            ...
            <Text style={{ color: colors.textMuted, marginTop: 12, fontSize: 14 }}>No recommendations</Text>
          </View>
        )}
      </ScrollView>
