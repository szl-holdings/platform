    ...
    >
      ...
      <View style={{ flex: 1, marginLeft: 12 }}>
        ...
        <View style={styles.templateChips}>
          <View style={[styles.chip, { backgroundColor: prioColor + "18", borderColor: prioColor + "35" }]}>
...
function ActionRecordRow({ record, colors }: { record: ActionRecord; colors: ReturnType<typeof useColors> }) {
  const prioColor = PRIORITY_COLORS[record.priority] ?? "#6b7280";
  const statusColor = record.status === "pending" ? "#f59e0b" : record.status === "approved" ? "#22c55e" : record.status === "rejected" ? "#ef4444" : "#6b7280";
  ...
  return (
    <View style={[styles.recordRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      <View style={styles.recordBadges}>
        <View style={[styles.chip, { backgroundColor: statusColor + "18", borderColor: statusColor + "35" }]}>
          <Text style={[styles.chipText, { color: statusColor }]}>{record.status}</Text>
        </View>
        <View style={[styles.chip, { backgroundColor: prioColor + "12", borderColor: prioColor + "25" }]}>
...
function ActionModal({
  ...
  const handleSubmit = () => {
    if (!resourceId.trim()) {
      Alert.alert("Missing field", "Resource ID is required to scope this action through Guardian.");
      return;
    }
        ...
        <View style={[styles.modalSheet, { backgroundColor: "#0d1220" }]}>
          ...
          {template && (
            <>
              ...
              {template.requiresBiometric && (
                <View style={[styles.bioNotice, { borderColor: "#6366f135", backgroundColor: "#6366f110" }]}>
                  <Feather name="lock" size={12} color="#6366f1" />
                  <Text style={[styles.bioNoticeText, { color: "#6366f1" }]}>
                    Biometric confirmation required. This action will be recorded in the guardian audit ledger.
                  </Text>
                </View>
              )}
              ...
              <View style={[styles.rollbackNotice, { borderColor: "#f59e0b25", backgroundColor: "#f59e0b08" }]}>
              ...
              <TouchableOpacity
                ...
                disabled={isPending}
                style={[styles.submitBtn, { backgroundColor: template.accentColor + "18", borderColor: template.accentColor + "50" }]}
              >
                {isPending ? (
                  ...
                  <>
                    ...
                    <Text style={[styles.submitBtnText, { color: template.accentColor }]}>
                      {template.requiresBiometric ? "Authenticate & Execute" : "Execute via Guardian"}
                    </Text>
                  </>
                )}
              </TouchableOpacity>
...
export default function SecureQuickActionsScreen() {
  ...
  const normalizeRecords = (raw: { data: ActionRecord[] } | ActionRecord[] | undefined): ActionRecord[] => {
    if (!raw) return [];
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: insets.top + 12, borderBottomColor: colors.border }]}>
        ...
        <View style={styles.headerCenter}>
          <Text style={[styles.headerTitle, { color: colors.foreground }]}>Secure Quick Actions</Text>
      ...
      >
        <View style={[styles.guardianBanner, { borderColor: ACCENT + "35", backgroundColor: ACCENT + "08" }]}>
          <Feather name="shield" size={14} color={ACCENT} />
          <View style={{ flex: 1, marginLeft: 8 }}>
            <Text style={[styles.guardianBannerTitle, { color: ACCENT }]}>Guardian Policy Layer Active</Text>
        ...
        <Text style={[styles.sectionLabel, { color: colors.mutedForeground }]}>AVAILABLE ACTIONS</Text>
        {ACTION_TEMPLATES.map((template) => (
          <ActionTemplateCard key={template.id} template={template} onPress={handleTemplatePress} />
        ))}

        <Text style={[styles.sectionLabel, { color: colors.mutedForeground, marginTop: 6 }]}>RECENT ACTIVITY</Text>
        {recentActionsQuery.isLoading ? (
          ...
          <View style={[styles.emptyRecent, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Text style={[styles.emptyRecentText, { color: colors.mutedForeground }]}>No recent actions</Text>
          </View>
        ) : (
        ...
        <TouchableOpacity
          ...
          style={[styles.viewInboxBtn, { borderColor: colors.border, backgroundColor: colors.card }]}
        >
      ...
      <ActionModal
        ...
        onClose={() => { setModalVisible(false); setActiveTemplate(null); }}
...
const styles = StyleSheet.create({
  ...
  emptyRecent: {
    padding: 16, borderRadius: 8, borderWidth: 1, alignItems: "center",
  },
  emptyRecentText: { fontSize: 12 },
  viewInboxBtn: {
    flexDirection: "row", alignItems: "center", justifyContent: "center",
    gap: 8, padding: 12, borderRadius: 8, borderWidth: 1, marginTop: 4,
  },
  viewInboxText: { fontSize: 13, fontWeight: "600", flex: 1, textAlign: "center" },
  modalOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.7)", justifyContent: "flex-end" },
  modalSheet: {
    borderTopLeftRadius: 20, borderTopRightRadius: 20,
    padding: 20, paddingBottom: 36, borderWidth: 1, borderColor: "#1e2433",
  },
  modalHandle: { width: 40, height: 4, borderRadius: 2, backgroundColor: "#2d3748", alignSelf: "center", marginBottom: 16 },
  ...
  bioNotice: {
    ...
    padding: 10, borderRadius: 8, borderWidth: 1, marginBottom: 14,
  },
  bioNoticeText: { fontSize: 11, lineHeight: 16, flex: 1 },
  fieldLabel: { fontSize: 10, fontWeight: "700", color: "#6b7280", letterSpacing: 0.8, marginBottom: 6 },
  input: {
    borderWidth: 1, borderColor: "#1e2433", backgroundColor: "rgba(0,0,0,0.3)",
    borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10,
    color: "#e8edf8", fontSize: 13, marginBottom: 14,
  },
  rollbackNotice: {
    flexDirection: "row", alignItems: "center", gap: 7,
    padding: 10, borderRadius: 8, borderWidth: 1, marginBottom: 16,
  },
  rollbackText: { fontSize: 11, lineHeight: 16, flex: 1 },
  submitBtn: {
    flexDirection: "row", alignItems: "center", justifyContent: "center",
    gap: 7, paddingVertical: 13, borderRadius: 10, borderWidth: 1,
  },
