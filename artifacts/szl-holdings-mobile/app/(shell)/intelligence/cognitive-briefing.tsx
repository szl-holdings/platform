...
function levelColor(level: string): string {
  return URGENCY_COLOR[level] ?? URGENCY_COLOR[level === "high" ? "urgent" : level === "medium" ? "moderate" : "low"] ?? "#6b7280";
}
...
function BottleneckOwnerCard({
  ...
  return (
    <View
      style={[
        styles.card,
        { backgroundColor: colors.card, borderColor: colors.border, borderLeftColor: lColor, borderLeftWidth: 3 },
      ]}
    >
      <View style={styles.cardHeader}>
        <View style={styles.rankBadge}>
        ...
        <View style={[styles.urgencyChip, { backgroundColor: lColor + "18", borderColor: lColor + "35" }]}>
          <Text style={[styles.urgencyChipText, { color: lColor }]}>{item.level.toUpperCase()}</Text>
        </View>
      </View>
      ...
      <View style={styles.cardStatsRow}>
        ...
        <View style={styles.statBlock}>
          <Text style={[styles.statValue, { color: colors.foreground }]}>{formatAge(item.ageHours)}</Text>
          <Text style={[styles.statLabel, { color: colors.mutedForeground }]}>OLDEST AGE</Text>
        </View>
        <View style={styles.statBlock}>
          <Text style={[styles.statValue, { color: colors.foreground }]}>{item.escalationCount}</Text>
...
function AccountabilityOwnerCard({
  ...
  return (
    <View
      style={[
        styles.card,
        { backgroundColor: colors.card, borderColor: colors.border, borderLeftColor: cColor, borderLeftWidth: 3 },
      ]}
    >
      <View style={styles.cardHeader}>
        <View style={styles.rankBadge}>
        ...
        <View style={[styles.urgencyChip, { backgroundColor: cColor + "18", borderColor: cColor + "35" }]}>
      ...
      <View style={styles.cardStatsRow}>
        ...
        <View style={styles.statBlock}>
          <Text style={[styles.statValue, { color: colors.foreground }]}>{item.interventions.length}</Text>
          <Text style={[styles.statLabel, { color: colors.mutedForeground }]}>ACTIONS</Text>
        </View>
        <View style={styles.statBlock}>
          <Text style={[styles.statValue, { color: colors.foreground }]}>{item.bottlenecks.length}</Text>
          <Text style={[styles.statLabel, { color: colors.mutedForeground }]}>BLOCKERS</Text>
        </View>
        <View style={styles.statBlock}>
          <Text style={[styles.statValue, { color: colors.foreground }]}>{item.incidents.length}</Text>
      ...
      {(urgentInterventions > 0 || criticalIncidents > 0) && (
        <View style={styles.alertChipRow}>
          {urgentInterventions > 0 && (
            <View style={[styles.alertChip, { backgroundColor: "#f9731618", borderColor: "#f9731635" }]}>
          ...
          {criticalIncidents > 0 && (
            <View style={[styles.alertChip, { backgroundColor: "#ef444418", borderColor: "#ef444435" }]}>
      ...
      {item.escalationPath.length > 0 && (
        <View style={styles.itemsList}>
          <Text style={[styles.itemSectionLabel, { color: colors.mutedForeground }]}>ESCALATION PATH</Text>
          {item.escalationPath.slice(0, 3).map((esc) => {
            ...
            return (
              <View key={esc.escalationId} style={styles.escalationRow}>
                ...
                <Text style={[styles.itemLine, { color: colors.foreground, flex: 1 }]} numberOfLines={1}>
                  {esc.title}
                </Text>
          ...
          bottlenecksLoading ? (
            ...
            <>
              ...
              {bottlenecks.rankedByOwner.length === 0 ? (
                ...
                bottlenecks.rankedByOwner.slice(0, 8).map((owner, idx) => (
                  <BottleneckOwnerCard
                    key={owner.owner}
                    item={owner}
                    rank={idx + 1}
                ...
                <>
                  ...
                  {rankedDomainBottlenecks.map(({ domain, data }) => {
                    ...
                    return (
                      <View
                        key={domain}
                        style={[styles.varRow, { backgroundColor: colors.card, borderColor: colors.border }]}
                      >
                        <View style={styles.varRowHeader}>
                          <View
                            style={[
          ...
          accountabilityLoading ? (
            ...
            <>
              ...
              {accountability.accountabilityMap.length === 0 ? (
                ...
                accountability.accountabilityMap.slice(0, 10).map((owner, idx) => (
