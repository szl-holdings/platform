...
function AuditTrailSection({
  ...
  const trailQuery = useQuery<AuditEntry[]>({
    ...
    queryFn: async () => {
      const raw = await apiFetch<{ data: AuditEntry[] } | AuditEntry[]>(auditTrailPath(approvalId));
  ...
  return (
    <View
      style={[styles.threadBox, { backgroundColor: colors.background, borderColor: colors.border }]}
    >
            ...
            <View key={entry.id} style={styles.auditRow}>
              ...
              <View style={{ flex: 1 }}>
                <View style={styles.auditMetaRow}>
                  <Text style={[styles.auditAction, { color: c }]}>
...
function ApprovalCard({
  ...
  const iconName = (ACTION_CLASS_ICONS[approval.actionClass] ??
    'check-square') as React.ComponentProps<typeof Feather>['name'];
  const expiring =
    approval.expiresAt && new Date(approval.expiresAt).getTime() - Date.now() < 2 * 3600 * 1000;
      ...
      <View style={styles.cardHeader}>
        ...
        <View style={{ flex: 1, marginLeft: 10 }}>
          <View style={styles.cardMeta}>
            ...
            {expiring && (
              <View
                style={[styles.pill, { backgroundColor: '#ef444418', borderColor: '#ef444440' }]}
              >
        ...
        <View style={styles.cardBody}>
          ...
          <View style={styles.metaGrid}>
            ...
            <View style={styles.metaItem}>
              ...
              <Text style={[styles.metaValue, { color: colors.foreground }]}>
                {approval.requiredApproverRole ?? 'any'}
              </Text>
            </View>
            <View style={styles.metaItem}>
    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={reset}>
      <View style={styles.modalOverlay}>
        <View style={[styles.modalSheet, { backgroundColor: '#0d1220' }]}>
          ...
          {isOffline && (
            <View style={styles.offlineNotice}>
              ...
              <Text style={styles.offlineNoticeText}>
                You're offline — this escalation will be queued and synced when connectivity
                returns.
              </Text>
            </View>
          )}
          ...
          {approval && (
            <>
              <Text style={styles.modalApprovalTitle} numberOfLines={2}>
                {approval.title}
              </Text>
  ...
  return (
    <View style={[styles.queuedCard, { backgroundColor: colors.card, borderColor: '#f59e0b40' }]}>
      <View style={styles.queuedHeader}>
        <View
          style={[styles.queuedBadge, { backgroundColor: '#f59e0b15', borderColor: '#f59e0b40' }]}
        >
        ...
        <View style={[styles.modalSheet, { backgroundColor: '#0d1220' }]}>
          ...
          {approval && (
            <>
              ...
              <Text style={styles.modalApprovalSub}>
                {approval.resourceType} · {approval.actionClass} ·{' '}
                {PRIORITY_COLORS[approval.priority]
                  ? approval.priority.toUpperCase()
                  : approval.priority}
              </Text>
                ...
                {DECISIONS.map((d) => (
                  ...
                  >
                    <Feather
                      name={d.icon}
                      size={14}
                      color={decision === d.key ? d.color : '#6b7280'}
                    />
                    <Text
                      style={[
...
export default function ApprovalInboxScreen() {
  ...
  useEffect(() => {
    Promise.all([loadQueuedDecisions(), loadQueuedComments(), loadQueuedEscalations()]).then(
      ([decisions, comments, escalations]) => {
  ...
  const enqueueEscalationOffline = useCallback(async (approval: Approval, reason: string) => {
  ...
  const discardQueuedItem = useCallback((approvalId: number) => {
    Alert.alert('Discard Decision', 'Remove this queued decision? It will not be submitted.', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Discard',
        style: 'destructive',
        onPress: () => setOfflineQueue((prev) => prev.filter((q) => q.approvalId !== approvalId)),
      },
    ]);
  }, []);
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={[styles.headerSub, { color: colors.mutedForeground }]}>
            ...
            {(() => {
              const total =
                offlineQueue.length + offlineComments.length + offlineEscalations.length;
      ...
      {offlineQueue.length > 0 && (
        <View
          style={[
            styles.queueBanner,
            { backgroundColor: '#f59e0b10', borderBottomColor: '#f59e0b25' },
          ]}
        >
