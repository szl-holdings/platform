...
function SignalAlertCard({ signal, colors }: { signal: FusionSignal; colors: ReturnType<typeof useColors> }) {
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        styles.card,
        { backgroundColor: colors.card, borderColor: colors.border, borderLeftColor: sevColor, borderLeftWidth: 3 },
      ]}
      activeOpacity={0.85}
    >
      <View style={styles.cardHeader}>
        ...
        <View style={{ flex: 1, marginLeft: 8 }}>
          <View style={styles.cardMeta}>
            <View style={[styles.pill, { backgroundColor: sevColor + "18", borderColor: sevColor + "35" }]}>
              <Text style={[styles.pillText, { color: sevColor }]}>{signal.severity.toUpperCase()}</Text>
            </View>
            <Text style={[styles.cardTime, { color: colors.mutedForeground }]}>{formatRelative(signal.timestamp)}</Text>
          </View>
          ...
          <View style={styles.domainRow}>
            {signal.affectedDomains.slice(0, 3).map((d) => {
              ...
              return (
                <View key={d} style={[styles.domainPill, { backgroundColor: meta.color + "15", borderColor: meta.color + "30" }]}>
      ...
      {expanded && (
        <View style={styles.cardBody}>
          ...
          <View style={styles.metaRow}>
            ...
            <Text style={[styles.metaLabel, { color: colors.mutedForeground, marginLeft: 16 }]}>CONFIDENCE</Text>
            <Text style={[styles.metaValue, { color: colors.foreground }]}>{Math.round(signal.confidence * 100)}%</Text>
          </View>
          {signal.recommendedActions && signal.recommendedActions.length > 0 && (
            <View style={[styles.actionsBox, { backgroundColor: colors.background, borderColor: colors.border }]}>
              <Text style={[styles.actionsLabel, { color: colors.mutedForeground }]}>RECOMMENDED ACTIONS</Text>
...
function EscalationCard({ approval, colors }: { approval: Approval; colors: ReturnType<typeof useColors> }) {
  const prioColors: Record<string, string> = {
    critical: "#ef4444", high: "#f97316", medium: "#f59e0b", low: "#6b7280",
  };
  const prioColor = prioColors[approval.priority] ?? "#6b7280";
  const expiringMs = approval.expiresAt ? new Date(approval.expiresAt).getTime() - Date.now() : null;
      ...
      <View style={styles.cardHeader}>
        ...
        <View style={{ flex: 1, marginLeft: 8 }}>
          <View style={styles.cardMeta}>
            ...
            {expiringSoon && (
              <View style={[styles.pill, { backgroundColor: "#ef444415", borderColor: "#ef444435" }]}>
...
function WorldModelAlert({
  ...
  return (
    <View style={[styles.wmAlert, { backgroundColor: sevColor + "10", borderColor: sevColor + "30", borderLeftColor: sevColor, borderLeftWidth: 3 }]}>
      <View style={styles.wmAlertHeader}>
        <Text style={[styles.wmDomain, { color: domMeta.color }]}>{domMeta.icon} {domMeta.label}</Text>
        <View style={[styles.pill, { backgroundColor: sevColor + "18", borderColor: sevColor + "35" }]}>
...
export default function AlertCenterScreen() {
  ...
  const isRefetching = signalsQuery.isRefetching || escalationsQuery.isRefetching || briefingQuery.isRefetching;
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      {briefing && (
        <View style={[styles.healthBar, { borderBottomColor: colors.border, backgroundColor: colors.card }]}>
          <View style={styles.healthItem}>
            <Text style={[styles.healthLabel, { color: colors.mutedForeground }]}>HEALTH</Text>
            <Text style={[
              styles.healthValue,
              { color: briefing.overallHealthScore >= 0.8 ? "#22c55e" : briefing.overallHealthScore >= 0.5 ? "#f59e0b" : "#ef4444" }
            ]}>
          ...
          <View style={styles.healthItem}>
            ...
            <Text style={[styles.healthValue, { color: criticalSignals.length > 0 ? "#ef4444" : "#22c55e" }]}>
          ...
          <View style={styles.healthItem}>
            ...
            <Text style={[styles.healthValue, { color: escalations.length > 0 ? "#f59e0b" : colors.foreground }]}>
          ...
          <View style={styles.healthItem}>
            ...
            <Text style={[styles.healthValue, { color: colors.foreground }]}>{briefing.domains.length}</Text>
          </View>
        </View>
      )}
        ...
        {TABS.map((tab) => (
          ...
          >
            <View style={styles.tabLabelRow}>
              <Text style={[styles.tabLabel, { color: activeTab === tab.key ? ACCENT : colors.mutedForeground }]}>
                {tab.label}
              </Text>
              {(tab.badge ?? 0) > 0 && (
        ...
        {isLoading ? (
          ...
          criticalSignals.length === 0 ? (
            <View style={styles.empty}>
              ...
              <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>No critical or high signals</Text>
              <Text style={[styles.emptySub, { color: colors.mutedForeground }]}>All systems nominal</Text>
            </View>
          ) : (
          ...
          escalations.length === 0 ? (
            <View style={styles.empty}>
              ...
              <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>No escalated approvals</Text>
              <Text style={[styles.emptySub, { color: colors.mutedForeground }]}>All clear — verifier queue is empty</Text>
            </View>
          ) : (
          ...
          worldModelAlerts.length === 0 ? (
            <View style={styles.empty}>
              ...
              <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>No world-model alerts</Text>
              <Text style={[styles.emptySub, { color: colors.mutedForeground }]}>Constellation is healthy across all domains</Text>
            </View>
          ) : (
            <>
              {briefing && (
...
const styles = StyleSheet.create({
  ...
  tabBadge: { minWidth: 16, height: 16, borderRadius: 8, paddingHorizontal: 3, alignItems: "center", justifyContent: "center" },
  ...
  wmAlertHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 6 },
