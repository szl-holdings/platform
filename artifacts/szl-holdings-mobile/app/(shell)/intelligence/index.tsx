    ...
    >
        ...
        <View style={styles.signalInfo}>
          <View style={styles.signalMeta}>
            ...
            {signal.hasActionDrafts && (
              <View style={[styles.draftsBadge, { borderColor: ACCENT + "50", backgroundColor: ACCENT + "15" }]}>
          ...
          <Text style={[styles.signalTitle, { color: colors.foreground }]} numberOfLines={expanded ? undefined : 2}>
            {signal.title}
          </Text>
          {!expanded && (
            <Text style={[styles.signalSummary, { color: colors.mutedForeground }]} numberOfLines={1}>
              {signal.summary}
            </Text>
          )}
          <View style={styles.domainPills}>
            {signal.affectedDomains.slice(0, 3).map((d) => {
              ...
              return (
                <View key={d} style={[styles.domainPill, { backgroundColor: meta.color + "18", borderColor: meta.color + "30" }]}>
                  <Text style={[styles.domainPillText, { color: meta.color }]}>{meta.icon} {meta.label}</Text>
                </View>
              );
            })}
          </View>
      ...
      {expanded && (
        <View style={styles.signalExpanded}>
          <Text style={[styles.signalSummaryFull, { color: colors.foreground }]}>{signal.summary}</Text>

          {signal.recommendedActions && signal.recommendedActions.length > 0 && (
            <View style={styles.actionsBlock}>
              <Text style={[styles.blockLabel, { color: colors.mutedForeground }]}>RECOMMENDED ACTIONS</Text>
              {signal.recommendedActions.map((a, i) => (
                <Text key={i} style={[styles.actionItem, { color: colors.foreground }]}>• {a}</Text>
              ))}
            </View>
          )}
          ...
          {!signal.hasActionDrafts && signal.status === "active" && (
            <TouchableOpacity
              ...
              style={[styles.generateBtn, { borderColor: ACCENT + "50", backgroundColor: ACCENT + "15" }]}
            >
              <Text style={[styles.generateBtnText, { color: ACCENT }]}>Generate Autonomous Action Drafts</Text>
            </TouchableOpacity>
          )}
        </View>
      )}
...
function DraftCard({
  ...
  const [expanded, setExpanded] = useState(false);
  const prioColor = draft.priority === "urgent" ? "#ef4444" : draft.priority === "high" ? "#f97316" : "#6b7280";
  const statusColor = draft.status === "approved" ? "#22c55e" : draft.status === "dismissed" ? "#6b7280" : prioColor;
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        styles.draftCard,
        { backgroundColor: colors.card, borderColor: colors.border, borderLeftColor: statusColor, borderLeftWidth: 3 },
    ...
    >
      <View style={styles.draftHeader}>
        <View style={styles.draftInfo}>
          <View style={styles.draftMeta}>
            <Text style={[styles.draftPriority, { color: prioColor }]}>{draft.priority.toUpperCase()}</Text>
            {draft.status !== "pending" && (
              <Text style={[styles.draftStatus, { color: draft.status === "approved" ? "#22c55e" : "#6b7280" }]}>
          ...
          <Text style={[styles.draftDomain, { color: colors.mutedForeground }]}>
            {DOMAIN_META[draft.domain]?.icon ?? "◆"} {DOMAIN_META[draft.domain]?.label ?? draft.domain}
      ...
      {expanded && (
        <View style={styles.draftExpanded}>
          <Text style={[styles.draftContent, { color: colors.foreground, backgroundColor: colors.background }]}>
            {draft.content}
          </Text>
          {draft.status === "pending" && (
...
function CascadeCard({ cascade, colors }: { cascade: WhatIfCascade; colors: ReturnType<typeof useColors> }) {
  const [expanded, setExpanded] = useState(false);
  const color = IMPACT_COLORS[cascade.impact] ?? "#6b7280";
  const domMeta = DOMAIN_META[cascade.domain] ?? { label: cascade.domain, icon: "◆", color: "#6b7280" };

  return (
    <TouchableOpacity
      onPress={() => setExpanded((v) => !v)}
      style={[styles.cascadeCard, { backgroundColor: colors.card, borderColor: colors.border, borderLeftColor: color, borderLeftWidth: 3 }]}
      activeOpacity={0.85}
    >
      <View style={styles.cascadeHeader}>
        <View style={[styles.cascadeIcon, { backgroundColor: domMeta.color + "20", borderColor: domMeta.color + "40" }]}>
        ...
        <View style={styles.cascadeInfo}>
          <View style={styles.cascadeMeta}>
            ...
            <View style={[styles.impactBadge, { backgroundColor: color + "20", borderColor: color + "40" }]}>
              <Text style={[styles.impactBadgeText, { color }]}>{cascade.impact.toUpperCase()}</Text>
            </View>
          </View>
          <Text style={[styles.cascadeDesc, { color: colors.foreground }]} numberOfLines={expanded ? undefined : 2}>
            {cascade.description}
          </Text>
      ...
      {expanded && cascade.mitigationOptions.length > 0 && (
        <View style={styles.mitigationBlock}>
          <Text style={[styles.blockLabel, { color: colors.mutedForeground }]}>MITIGATION OPTIONS</Text>
          {cascade.mitigationOptions.map((opt, i) => (
            <Text key={i} style={[styles.actionItem, { color: colors.foreground }]}>• {opt}</Text>
          ))}
        </View>
      )}
    </TouchableOpacity>
...
export default function CortexIntelligenceScreen() {
  ...
  const feedQuery = useQuery<{ signals: FusionSignal[]; stats: FeedStats }>({
    ...
    queryFn: () => apiFetch<{ signals: FusionSignal[]; stats: FeedStats }>("/api/cortex/intelligence-feed"),
  ...
  const draftsQuery = useQuery<{ drafts: ActionDraft[]; pendingCount: number }>({
    ...
    queryFn: () => apiFetch<{ drafts: ActionDraft[]; pendingCount: number }>("/api/cortex/action-drafts?status=pending"),
  ...
  const generateDraftsMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      Alert.alert("Drafts Generated", "Autonomous action drafts have been created and queued for your review in the Drafts tab.");
      setActiveTab("drafts");
    },
    onError: () => {
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <View style={styles.headerCenter}>
          <Text style={[styles.headerTitle, { color: colors.foreground }]}>CORTEX Intelligence</Text>
        ...
        {TABS.map((tab) => (
          ...
          >
            <View style={styles.tabLabelRow}>
              ...
              {(tab.badge ?? 0) > 0 && (
                <View style={[styles.tabBadge, { backgroundColor: activeTab === tab.key ? ACCENT : colors.mutedForeground + "80" }]}>
        ...
        >
          ...
          <View style={styles.runtimeSection}>
            ...
            <View style={styles.runtimeGrid}>
              {[
                { route: "/(shell)/intelligence/cognitive-briefing", label: "Cognitive Briefing", icon: "cpu" as const, color: "#8b7ac8", sub: "Top interventions · VaR by domain" },
      ...
      {activeTab === "drafts" && (
        ...
        >
          {pendingCount > 0 && (
            <View style={[styles.pendingBanner, { backgroundColor: "#ef444415", borderColor: "#ef444440" }]}>
      ...
      {activeTab === "whatif" && (
        ...
        >
          <View style={[styles.scenarioInput, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <TextInput
              ...
              multiline
              numberOfLines={3}
              style={[styles.scenarioTextInput, { color: colors.foreground, borderColor: colors.border }]}
            />
            <TouchableOpacity
            ...
            >
              <Text style={[styles.scenarioBtnText, { color: whatIfLoading || !whatIfQuery.trim() ? colors.mutedForeground : "#000000" }]}>
                {whatIfLoading ? "Simulating…" : "Run Simulation ▶"}
              </Text>
            </TouchableOpacity>

            <Text style={[styles.starterLabel, { color: colors.mutedForeground }]}>Quick scenarios:</Text>
            <View style={styles.starters}>
              {STARTER_QUERIES.map((q) => (
                ...
                >
                  <Text style={[styles.starterChipText, { color: colors.mutedForeground }]} numberOfLines={2}>
                    {q}
                  </Text>
                </TouchableOpacity>
              ))}
          ...
          {whatIfResult && !whatIfLoading && (
            <View>
                ...
                <View style={styles.resultMeta}>
                  <View
                    style={[
                      styles.riskBadge,
                      {
                        backgroundColor: (IMPACT_COLORS[whatIfResult.overallRisk] ?? "#6b7280") + "20",
                        borderColor: (IMPACT_COLORS[whatIfResult.overallRisk] ?? "#6b7280") + "40",
                      },
                    ]}
                  >
                    <Text style={[styles.riskBadgeText, { color: IMPACT_COLORS[whatIfResult.overallRisk] ?? "#6b7280" }]}>
                      Overall Risk: {whatIfResult.overallRisk.toUpperCase()}
                    </Text>
                  </View>
                  <Text style={[styles.resultMeta2, { color: colors.mutedForeground }]}>
                    Horizon: {whatIfResult.timeHorizon} · Confidence: {Math.round(whatIfResult.confidence * 100)}%
                  </Text>
                </View>
                <Text style={[styles.resultSummary, { color: colors.foreground }]}>{whatIfResult.summary}</Text>
              ...
              <Text style={[styles.sectionTitle, { color: colors.mutedForeground }]}>IMPACT CASCADES</Text>
...
const styles = StyleSheet.create({
  ...
  signalInfo: { flex: 1 },
  signalMeta: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 3, flexWrap: "wrap" },
  ...
  cascadeInfo: { flex: 1 },
  cascadeMeta: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 2, flexWrap: "wrap" },
  ...
  mitigationBlock: { marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: "#ffffff10" },
  ...
  sectionTitle: { fontSize: 11, fontWeight: "600", letterSpacing: 0.5, marginBottom: 8, marginTop: 4 },
});
