...
const MODELS: ProFormaModel[] = [
  {
    ...
    capRate: 6.0,
    scenarios: [
      { id: "s1", label: "Bear", irr: 8.4, equityMultiple: 1.6, cashOnCash: 5.2, holdPeriod: 5, exitCapRate: 6.75, color: "#ef4444" },
      { id: "s2", label: "Base", irr: 14.2, equityMultiple: 2.1, cashOnCash: 7.8, holdPeriod: 5, exitCapRate: 6.0, color: "#34d399" },
      { id: "s3", label: "Bull", irr: 19.8, equityMultiple: 2.8, cashOnCash: 10.1, holdPeriod: 5, exitCapRate: 5.25, color: "#60a5fa" },
    ],
    yearlyNoi: [
  ...
  },
  {
    ...
    capRate: 7.0,
    scenarios: [
      { id: "s4", label: "Bear", irr: 9.1, equityMultiple: 1.5, cashOnCash: 6.0, holdPeriod: 7, exitCapRate: 7.5, color: "#ef4444" },
      { id: "s5", label: "Base", irr: 15.6, equityMultiple: 2.4, cashOnCash: 8.5, holdPeriod: 7, exitCapRate: 6.75, color: "#34d399" },
      { id: "s6", label: "Bull", irr: 21.3, equityMultiple: 3.1, cashOnCash: 11.2, holdPeriod: 7, exitCapRate: 6.0, color: "#60a5fa" },
    ],
    yearlyNoi: [
...
function Bar({ value, max, color }: { value: number; max: number; color: string }) {
  ...
  return (
    <View style={barStyles.track}>
      <View style={[barStyles.fill, { width: `${pct}%` as DimensionValue, backgroundColor: color }]} />
    </View>
  );
}
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.sectionHead, { borderBottomColor: colors.border }]}>
          <Text style={[styles.sectionHeadText, { color: colors.mutedForeground }]}>SCENARIO ANALYSIS</Text>
        </View>

        <View style={{ paddingHorizontal: 16, gap: 8, paddingTop: 12, marginBottom: 16 }}>
          {model.scenarios.map(scenario => (
            <View key={scenario.id} style={[styles.scenarioCard, { backgroundColor: colors.surface, borderColor: scenario.color + "30", borderLeftColor: scenario.color, borderLeftWidth: 3 }]}>
              <View style={styles.scenarioTop}>
                <Text style={[styles.scenarioLabel, { color: scenario.color }]}>{scenario.label} Case</Text>
                <Text style={[styles.scenarioHold, { color: colors.mutedForeground }]}>{scenario.holdPeriod}yr hold · {scenario.exitCapRate}% exit cap</Text>
              </View>
              <View style={styles.scenarioMetrics}>
                <View style={styles.scenMet}>
                  <Text style={[styles.scenMetVal, { color: scenario.color }]}>{scenario.irr}%</Text>
                  <Text style={[styles.scenMetLbl, { color: colors.mutedForeground }]}>IRR</Text>
                </View>
                <View style={styles.scenMet}>
                  <Text style={[styles.scenMetVal, { color: scenario.color }]}>{scenario.equityMultiple}x</Text>
                  <Text style={[styles.scenMetLbl, { color: colors.mutedForeground }]}>EM</Text>
                </View>
                <View style={styles.scenMet}>
                  <Text style={[styles.scenMetVal, { color: scenario.color }]}>{scenario.cashOnCash}%</Text>
                  <Text style={[styles.scenMetLbl, { color: colors.mutedForeground }]}>CoC Yr1</Text>
                </View>
              </View>
            </View>
          ))}
        ...
        <View style={[styles.sectionHead, { borderBottomColor: colors.border }]}>
          <Text style={[styles.sectionHeadText, { color: colors.mutedForeground }]}>NOI PROJECTION · BASE CASE</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  scenarioTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 10 },
