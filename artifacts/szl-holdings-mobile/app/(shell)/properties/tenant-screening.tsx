...
function ApplicationCard({
  ...
  const creditColor = app.creditScore >= 720 ? "#34d399" : app.creditScore >= 660 ? "#60a5fa" : app.creditScore >= 580 ? "#fbbf24" : "#ef4444";
  const rtiColor = app.rentToIncome <= 30 ? "#34d399" : app.rentToIncome <= 40 ? "#fbbf24" : "#ef4444";

  return (
    <Pressable
      onPress={() => { Haptics.selectionAsync(); onToggle(); }}
      style={[styles.appCard, { backgroundColor: colors.surface, borderColor: expanded ? statusColor + "40" : colors.border }]}
    >
      <View style={styles.appCardTop}>
        <View style={[styles.statusIconWrap, { backgroundColor: statusColor + "15", borderColor: statusColor + "30" }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.appSub, { color: colors.mutedForeground }]}>{app.property} · Unit {app.unit}</Text>
        </View>
        <View style={{ alignItems: "flex-end", gap: 4 }}>
          ...
          <Text style={[styles.appDate, { color: colors.mutedForeground }]}>{app.submittedDate}</Text>
        </View>
      </View>
      ...
      {expanded && (
        <View style={[styles.expandedSection, { borderTopColor: colors.border }]}>
          <View style={styles.checkRow}>
            ...
            {app.evictionHistory && (
              <View style={[styles.checkBadge, { backgroundColor: "#ef4444" + "12", borderColor: "#ef4444" + "30" }]}>
          ...
          {app.notes && (
            <View style={[styles.notesBox, { backgroundColor: "#fbbf24" + "08", borderColor: "#fbbf24" + "20" }]}>
...
function mapApiToApplications(raw: unknown): ScreeningApplication[] | null {
  ...
  return list.map((a: Record<string, unknown>, idx: number) => ({
    ...
    status: (["applied", "in-review", "approved", "denied", "withdrawn"].includes(String(a.status)) ? a.status : "applied") as ScreeningApplication["status"],
...
export default function TenantScreeningScreen() {
  ...
  const counts: Record<string, number> = displayApplications.reduce((acc, a) => {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: topPad + 12 }]}>
        ...
        <View style={[styles.pendingBadge, { backgroundColor: "#60a5fa" + "15", borderColor: "#60a5fa" + "40" }]}>
          <Text style={[styles.pendingText, { color: "#60a5fa" }]}>{counts["pending"] ?? 0} pending</Text>
        </View>
      </View>
      ...
      <View style={styles.summaryStrip}>
        {[
          ...
          { label: "In Review", value: (counts["in-review"] ?? 0) + (counts["pending"] ?? 0), color: "#60a5fa" },
          { label: "Action Needed", value: (counts["more-info"] ?? 0) + (counts["denied"] ?? 0), color: "#ef4444" },
        ].map((stat, i) => (
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40, paddingHorizontal: 16 }} showsVerticalScrollIndicator={false}>
        {filtered.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>No applications in this category</Text>
          </View>
        ) : (
          filtered.map(app => (
            <ApplicationCard
...
const styles = StyleSheet.create({
  ...
  stripLabel: { fontSize: 9, fontFamily: "Inter_300Light", letterSpacing: 0.5, textAlign: "center" },
