    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.topBar, { paddingTop: topPad + 12 }]}>
        ...
        <View style={[styles.locStatus, { backgroundColor: locStatusConfig.color + "15" }]}>
          ...
          <Text style={[styles.locText, { color: locStatusConfig.color }]}>{locStatusConfig.label}</Text>
        </View>
      </View>
      ...
      >
        ...
        <View style={styles.photoSection}>
          {photos.length === 0 ? (
            <Pressable
              ...
              style={[styles.cameraPlaceholder, { backgroundColor: colors.surface, borderColor: colors.border }]}
            >
              <View style={[styles.cameraIconBg, { backgroundColor: colors.goldDim, borderColor: colors.goldBorder }]}>
              ...
              <Text style={[styles.cameraSubhint, { color: colors.mutedForeground }]}>Automatically GPS-tagged</Text>
            </Pressable>
          ) : (
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.photoRow}>
              {photos.map((photo, i) => (
                <View key={i} style={styles.photoThumbWrapper}>
                  ...
                  <Pressable
                    onPress={() => { Haptics.selectionAsync(); setPhotos(prev => prev.filter((_, j) => j !== i)); }}
                  ...
                  <Text style={[styles.photoTime, { color: colors.mutedForeground }]}>{photo.timestamp}</Text>
                </View>
              ))}
              <Pressable
                onPress={handleTakePhoto}
        ...
        <View style={[styles.notesBlock, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          />
          <Text style={[styles.charCount, { color: colors.mutedForeground }]}>{notes.length} characters</Text>
        </View>

        {photos.length > 0 && (
          <View style={[styles.captureStats, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <View style={styles.captureStatRow}>
              <Feather name="camera" size={12} color={colors.gold} />
              <Text style={[styles.captureStatText, { color: colors.cream }]}>{photos.length} photo{photos.length !== 1 ? "s" : ""} captured</Text>
            </View>
            {photos.some(p => p.lat) && (
              <View style={styles.captureStatRow}>
        ...
        <View style={styles.actions}>
          ...
          <Pressable
            ...
            style={[styles.saveBtn, { backgroundColor: colors.goldDim, borderColor: colors.goldBorder, opacity: saveCapture.isPending ? 0.6 : 1 }]}
          >
            <Feather name={saveCapture.isPending ? "loader" : "save"} size={16} color={colors.gold} />
            <Text style={[styles.saveText, { color: colors.gold }]}>{saveCapture.isPending ? "Saving..." : "Save Capture"}</Text>
          </Pressable>
        </View>
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  backBtn: { width: 36, height: 36, borderRadius: 18, borderWidth: 1, alignItems: "center", justifyContent: "center", marginTop: 8 },
  eyebrow: { fontSize: 8, fontFamily: "Inter_500Medium", letterSpacing: 3, marginBottom: 2 },
  title: { fontSize: 18, fontFamily: "Inter_600SemiBold" },
  locStatus: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 8, paddingVertical: 5, borderRadius: 8, marginTop: 8 },
  locText: { fontSize: 9, fontFamily: "Inter_500Medium" },
  scroll: { flex: 1 },
  inputBlock: { flexDirection: "row", alignItems: "center", gap: 10, marginHorizontal: 20, paddingHorizontal: 14, paddingVertical: 12, borderRadius: 10, borderWidth: 1, marginBottom: 14 },
  textInput: { flex: 1, fontSize: 13, fontFamily: "Inter_300Light" },
  photoSection: { paddingHorizontal: 20, marginBottom: 14 },
  cameraPlaceholder: { borderRadius: 12, borderWidth: 1, borderStyle: "dashed", height: 180, alignItems: "center", justifyContent: "center", gap: 10 },
  cameraIconBg: { width: 60, height: 60, borderRadius: 30, borderWidth: 1, alignItems: "center", justifyContent: "center" },
  ...
  photoDelete: { position: "absolute", top: 6, right: 6, width: 18, height: 18, borderRadius: 9, alignItems: "center", justifyContent: "center" },
  gpsTag: { position: "absolute", bottom: 24, left: 6, width: 18, height: 18, borderRadius: 9, alignItems: "center", justifyContent: "center" },
  photoTime: { fontSize: 9, fontFamily: "Inter_300Light", textAlign: "center", marginTop: 4 },
  addPhotoBtn: { width: 120, height: 120, borderRadius: 10, borderWidth: 1, alignItems: "center", justifyContent: "center", gap: 6 },
  addPhotoText: { fontSize: 11, fontFamily: "Inter_300Light" },
  notesBlock: { marginHorizontal: 20, borderRadius: 12, borderWidth: 1, padding: 14, marginBottom: 14 },
  ...
  captureStats: { marginHorizontal: 20, borderRadius: 10, borderWidth: 1, padding: 12, marginBottom: 14, gap: 8 },
  ...
  saveBtn: { flex: 2, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 14, borderRadius: 12, borderWidth: 1 },
