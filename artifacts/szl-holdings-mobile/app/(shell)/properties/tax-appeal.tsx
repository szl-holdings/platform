  ...
  return (
    <View style={{ gap: 4 }}>
      <View style={gaugeStyles.track}>
        <View style={[gaugeStyles.fill, { width: `${pct}%` as DimensionValue, backgroundColor: "#34d399" }]} />
        <View style={[gaugeStyles.over, { width: `${overPct}%` as DimensionValue, backgroundColor: ACCENT + "60" }]} />
      </View>
      <View style={gaugeStyles.legend}>
        <View style={gaugeStyles.legendItem}>
          <View style={[gaugeStyles.dot, { backgroundColor: "#34d399" }]} />
          <Text style={[gaugeStyles.legendText, { color: colors.mutedForeground }]}>Market {fmt(market)}</Text>
        </View>
        <View style={gaugeStyles.legendItem}>
          <View style={[gaugeStyles.dot, { backgroundColor: ACCENT }]} />
          <Text style={[gaugeStyles.legendText, { color: colors.mutedForeground }]}>Over-assessed {fmt(assessed - market)}</Text>
        </View>
      </View>
    </View>
  );
...
const gaugeStyles = StyleSheet.create({
  track: { height: 8, backgroundColor: "rgba(255,255,255,0.07)", borderRadius: 4, flexDirection: "row", overflow: "hidden" },
  fill: { height: 8 },
  over: { height: 8 },
...
export default function TaxAppealScreen() {
  ...
  const activeAppeals = APPEALS.filter(a => a.appealStatus === "in-hearing" || a.appealStatus === "filed").length;
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40, paddingHorizontal: 16 }} showsVerticalScrollIndicator={false}>
        {APPEALS.map(appeal => {
          ...
          return (
            <View key={appeal.id} style={[styles.appealCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <View style={styles.appealTop}>
                <View style={{ flex: 1 }}>
                  <Text style={[styles.appealProp, { color: colors.cream }]}>{appeal.property}</Text>
                  <Text style={[styles.appealAddr, { color: colors.mutedForeground }]}>{appeal.address}</Text>
                </View>
                <View style={[styles.statusPill, { backgroundColor: statusColor + "15", borderColor: statusColor + "30" }]}>
                  <Text style={[styles.statusPillText, { color: statusColor }]}>{appeal.appealStatus}</Text>
                </View>
              </View>

              <View style={styles.appealMetrics}>
                <View style={styles.appealmMet}>
                  <Text style={[styles.appealMetVal, { color: ACCENT }]}>{fmt(appeal.assessedValue)}</Text>
                  <Text style={[styles.appealMetLbl, { color: colors.mutedForeground }]}>Assessed</Text>
                </View>
                <View style={styles.appealmMet}>
                  <Text style={[styles.appealMetVal, { color: "#34d399" }]}>{fmt(appeal.marketValue)}</Text>
                  <Text style={[styles.appealMetLbl, { color: colors.mutedForeground }]}>Market</Text>
                </View>
                <View style={styles.appealmMet}>
                  <Text style={[styles.appealMetVal, { color: "#34d399" }]}>{fmt(appeal.potentialSavings)}</Text>
                  <Text style={[styles.appealMetLbl, { color: colors.mutedForeground }]}>Savings/yr</Text>
                </View>
              </View>
              ...
              {appeal.hearingDate && (
                <View style={[styles.dateRow, { borderTopColor: colors.border }]}>
                  ...
                  <Text style={[styles.dateRowText, { color: "#fbbf24" }]}>Hearing: {appeal.hearingDate}</Text>
                </View>
              )}
              {appeal.filingDeadline && (
                <View style={[styles.dateRow, { borderTopColor: colors.border }]}>
                  <Feather name="clock" size={11} color="#60a5fa" />
                  <Text style={[styles.dateRowText, { color: "#60a5fa" }]}>Filing deadline: {appeal.filingDeadline}</Text>
                </View>
              )}

              <View style={[styles.compsSection, { borderTopColor: colors.border }]}>
                <Text style={[styles.compsLabel, { color: colors.mutedForeground }]}>COMPARABLES · {appeal.comparables.length}</Text>
                {appeal.comparables.map((comp, i) => (
                  <View key={i} style={styles.compRow}>
                    <Feather name="home" size={10} color={colors.mutedForeground} />
                    <Text style={[styles.compAddress, { color: colors.mutedForeground }]} numberOfLines={1}>{comp.address}</Text>
                    <Text style={[styles.compVal, { color: colors.cream }]}>{fmt(comp.assessedValue)}</Text>
                  </View>
                ))}
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  dateRow: { flexDirection: "row", alignItems: "center", gap: 6, borderTopWidth: 1, paddingTop: 10, marginTop: 2 },
