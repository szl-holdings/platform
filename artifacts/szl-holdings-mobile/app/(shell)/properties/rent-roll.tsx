...
const PROPERTIES: PropertyRentRoll[] = [
  {
    ...
    vacancyLoss: 28500,
    leases: [
      { id: "l1", tenant: "Meridian Technologies", suite: "100", sqft: 12500, monthlyRent: 31250, leaseEnd: "Dec 2027", status: "active", creditScore: "A", paymentHistory: "excellent", markToMarketGap: 14 },
      { id: "l2", tenant: "Apex Financial Group", suite: "200", sqft: 8200, monthlyRent: 22960, leaseEnd: "May 2026", status: "active", creditScore: "A", paymentHistory: "excellent", markToMarketGap: 2 },
      { id: "l3", tenant: "Vanguard Legal LLP", suite: "300", sqft: 6800, monthlyRent: 18360, leaseEnd: "Feb 2026", status: "expiring", creditScore: "B", paymentHistory: "good", markToMarketGap: 6 },
      { id: "l4", tenant: "BrightPath Health", suite: "400", sqft: 10500, monthlyRent: 23100, leaseEnd: "Jun 2026", status: "expiring", creditScore: "B", paymentHistory: "good", markToMarketGap: 30 },
      { id: "l5", tenant: "Cascade Marketing", suite: "500", sqft: 4200, monthlyRent: 10500, leaseEnd: "Dec 2026", status: "active", creditScore: "B", paymentHistory: "excellent", markToMarketGap: 14 },
      { id: "l6", tenant: "DataVault Systems", suite: "600", sqft: 9800, monthlyRent: 27440, leaseEnd: "Aug 2028", status: "active", creditScore: "A", paymentHistory: "excellent", markToMarketGap: 2 },
      { id: "l7", tenant: "Pinnacle Consulting", suite: "700", sqft: 3800, monthlyRent: 7600, leaseEnd: "Dec 2025", status: "expiring", creditScore: "C", paymentHistory: "fair", markToMarketGap: 43 },
      { id: "l8", tenant: "Vertex Engineering", suite: "800", sqft: 7200, monthlyRent: 19440, leaseEnd: "Feb 2029", status: "active", creditScore: "A", paymentHistory: "excellent", markToMarketGap: 6 },
    ],
  },
  {
    id: "rr-2",
    ...
    vacancyLoss: 22000,
    leases: [
      { id: "l9", tenant: "Summit Logistics", suite: "A", sqft: 22000, monthlyRent: 44000, leaseEnd: "Mar 2028", status: "active", creditScore: "A", paymentHistory: "excellent", markToMarketGap: 5 },
      { id: "l10", tenant: "Ironclad Freight", suite: "B", sqft: 18500, monthlyRent: 37000, leaseEnd: "Sep 2026", status: "active", creditScore: "B", paymentHistory: "good", markToMarketGap: 8 },
      { id: "l11", tenant: "NorthStar Distribution", suite: "C", sqft: 14000, monthlyRent: 21000, leaseEnd: "Jun 2026", status: "expiring", creditScore: "C", paymentHistory: "fair", markToMarketGap: 20 },
    ],
  },
];
    ...
    <View style={[styles.summaryCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
      <View style={styles.summaryTop}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.propAddress, { color: colors.mutedForeground }]}>{property.address}</Text>
        </View>
        <View style={[styles.occupancyBadge, { backgroundColor: "#34d399" + "15", borderColor: "#34d399" + "30" }]}>
      ...
      <View style={styles.summaryMetrics}>
        <View style={styles.metricBlock}>
          <Text style={[styles.metricVal, { color: colors.cream }]}>{fmt(property.effectiveGrossIncome)}</Text>
        ...
        <View style={[styles.metricDivider, { backgroundColor: colors.border }]} />
        <View style={styles.metricBlock}>
          <Text style={[styles.metricVal, { color: colors.cream }]}>{property.occupiedUnits}/{property.totalUnits}</Text>
...
function LeaseRow({ lease }: { lease: Lease }) {
  ...
  return (
    <View style={[styles.leaseRow, { borderBottomColor: colors.border, backgroundColor: isExpiring ? "#fbbf24" + "05" : "transparent" }]}>
      <View style={[styles.leaseDot, { backgroundColor: statusColor }]} />
      <View style={styles.leaseTenantBlock}>
        <Text style={[styles.leaseTenant, { color: colors.cream }]} numberOfLines={1}>{lease.tenant}</Text>
        <Text style={[styles.leaseSuite, { color: colors.mutedForeground }]}>Suite {lease.suite} · {(lease.sqft / 1000).toFixed(1)}K sqft</Text>
      </View>
      <View style={{ alignItems: "flex-end", gap: 3 }}>
        <Text style={[styles.leaseRent, { color: colors.cream }]}>{fmt(lease.monthlyRent)}</Text>
        <Text style={[styles.leaseEnd, { color: isExpiring ? "#fbbf24" : colors.mutedForeground }]}>thru {lease.leaseEnd}</Text>
      </View>
      <View style={styles.creditBadge}>
        <Text style={[styles.creditText, { color: CREDIT_COLORS[lease.creditScore] }]}>{lease.creditScore}</Text>
      </View>
    </View>
  );
}
...
function mapApiToProperties(raw: unknown): PropertyRentRoll[] | null {
  ...
  return list.map((p: Record<string, unknown>, idx: number) => ({
    ...
    leases: Array.isArray(p.leases) ? (p.leases as Record<string, unknown>[]).map((l, li) => ({
      id: String(l.id ?? li),
      ...
      status: (["active", "expiring", "month-to-month", "vacant"].includes(String(l.status)) ? l.status : "active") as Lease["status"],
      creditScore: (["A", "B", "C", "D"].includes(String(l.creditScore)) ? l.creditScore : "B") as Lease["creditScore"],
      paymentHistory: (["excellent", "good", "fair", "poor"].includes(String(l.paymentHistory)) ? l.paymentHistory : "good") as Lease["paymentHistory"],
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: topPad + 12 }]}>
        ...
        {expiringCount > 0 && (
          <View style={[styles.alertBadge, { backgroundColor: "#fbbf24" + "15", borderColor: "#fbbf24" + "40" }]}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.propertyTabs}>
        {displayProperties.map(p => (
          <Pressable
            key={p.id}
            onPress={() => { Haptics.selectionAsync(); setSelectedProperty(p.id); }}
            style={[
              styles.propertyTab,
              {
          ...
          >
            <Text style={[styles.propertyTabText, { color: selectedProperty === p.id ? ACCENT : colors.mutedForeground }]}>
              {p.name}
            </Text>
          </Pressable>
        ))}
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.leaseHeader, { borderBottomColor: colors.border, borderTopColor: colors.border }]}>
          <Text style={[styles.leaseHeaderText, { color: colors.mutedForeground }]}>TENANT</Text>
          <Text style={[styles.leaseHeaderText, { color: colors.mutedForeground }]}>RENT / EXPIRY</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  creditBadge: { width: 22, height: 22, borderRadius: 4, alignItems: "center", justifyContent: "center", backgroundColor: "rgba(255,255,255,0.05)" },
