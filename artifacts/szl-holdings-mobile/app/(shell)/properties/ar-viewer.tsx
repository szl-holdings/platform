...
function ScanLine() {
  ...
  useEffect(() => {
    const anim = Animated.loop(
      Animated.sequence([
        Animated.timing(position, { toValue: 1, duration: 2000, useNativeDriver: true, easing: Easing.inOut(Easing.ease) }),
        Animated.timing(position, { toValue: 0, duration: 2000, useNativeDriver: true, easing: Easing.inOut(Easing.ease) }),
      ])
    );
    anim.start();
    return () => anim.stop();
...
function ARDataCard({ property, visible }: { property: ARProperty; visible: boolean }) {
  ...
  const oppColor =
    property.opportunity === "buy" ? "#22c55e" : property.opportunity === "watch" ? "#f59e0b" : "#ef4444";
  const oppLabel =
    property.opportunity === "buy" ? "BUY SIGNAL" : property.opportunity === "watch" ? "WATCH" : "AVOID";

  return (
    <Animated.View style={[styles.arCard, { opacity, transform: [{ translateY }] }]}>
      <View style={styles.arCardHeader}>
        <Text style={styles.arAddress} numberOfLines={1}>{property.address}</Text>
        <View style={[styles.oppBadge, { backgroundColor: `${oppColor}20`, borderColor: `${oppColor}40` }]}>
      ...
      <View style={styles.arMetricRow}>
        <View style={styles.arMetric}>
          ...
          <Text style={[styles.arMetricSub, { color: property.valueChange.startsWith("+") ? "#22c55e" : "#ef4444" }]}>
        ...
        <View style={styles.arMetric}>
          ...
          <Text style={[styles.arMetricValue, { color: property.distressScore > 50 ? "#f59e0b" : "#22c55e" }]}>
...
const styles = StyleSheet.create({
  ...
  arMetricLabel: { fontSize: 9, fontFamily: "Inter_400Regular", color: "rgba(201,168,76,0.5)", letterSpacing: 0.5 },
