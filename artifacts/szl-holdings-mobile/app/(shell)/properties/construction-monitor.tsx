...
import {
  ...
  type Milestone,
  PROJECTS,
} from './construction-monitor.data';
  ...
  return (
    <View style={styles.progressTrack}>
      <View
        style={[
          styles.progressFill,
          { width: `${Math.min(pct, 100)}%` as DimensionValue, backgroundColor: color },
        ]}
      />
    </View>
  );
      ...
      <View style={{ flex: 1 }}>
        <Text
          style={[
            ...
            { color: isComplete ? colors.mutedForeground : colors.cream },
          ]}
        >
          {milestone.label}
        </Text>
        <Text
          style={[styles.milestoneDue, { color: isDelayed ? '#ef4444' : colors.mutedForeground }]}
        >
          ...
          <View key={i} style={[styles.budgetRow, { borderBottomColor: colors.border }]}>
            <View style={{ flex: 1 }}>
              <View style={styles.budgetTop}>
                <Text style={[styles.budgetCategory, { color: colors.cream }]}>
                  {line.category}
                </Text>
                <Text
                  style={[
                    styles.budgetPct,
                    { color: pct > 100 ? '#ef4444' : colors.mutedForeground },
                  ]}
                >
                  {pct}%
                </Text>
              ...
              <ProgressBar
                pct={pct}
                color={pct > 100 ? '#ef4444' : pct > 85 ? '#fbbf24' : '#34d399'}
              />
              <View style={styles.budgetSubRow}>
                <Text style={[styles.budgetSub, { color: colors.mutedForeground }]}>
                  Budget {fmt(line.budgeted)}
                </Text>
                <Text style={[styles.budgetSub, { color: colors.mutedForeground }]}>
                  Spent {fmt(line.spent)}
                </Text>
                {line.committed > 0 && (
                  <Text style={[styles.budgetSub, { color: '#60a5fa' }]}>
                    +{fmt(line.committed)} committed
                  </Text>
                )}
              </View>
            </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
          ...
          >
            <Text
              style={[
                ...
                { color: selectedProject === p.id ? ACCENT : colors.mutedForeground },
              ]}
            >
              {p.name}
            </Text>
      ...
      >
          ...
          >
            <View style={styles.projectCardTop}>
              ...
              >
                ...
                <Text style={[styles.inspText, { color: inspColor }]}>
            ...
            <View style={styles.progressSection}>
              <View style={styles.progressLabels}>
                ...
                <Text style={[styles.progressPct, { color: colors.cream }]}>
            ...
            <View style={styles.progressSection}>
              <View style={styles.progressLabels}>
                ...
                <Text
                  style={[styles.progressPct, { color: budgetPct > 80 ? '#fbbf24' : colors.cream }]}
                >
                  {budgetPct}%
                </Text>
              </View>
              <ProgressBar
                pct={budgetPct}
                color={budgetPct > 90 ? '#ef4444' : budgetPct > 75 ? '#fbbf24' : '#34d399'}
              />
            </View>

            <View style={styles.cardMetrics}>
              <View style={styles.cardMetric}>
                <Text style={[styles.cardMetricValue, { color: colors.cream }]}>
                  {fmt(project.spentToDate)}
                </Text>
                <Text style={[styles.cardMetricLabel, { color: colors.mutedForeground }]}>
                  Spent
                </Text>
              </View>
              ...
              <View style={[styles.metricDivider, { backgroundColor: colors.border }]} />
              <View style={styles.cardMetric}>
                <Text style={[styles.cardMetricValue, { color: ACCENT }]}>
            ...
            {project.flags.length > 0 && (
              <View
                style={[
                  styles.flagsBox,
                  { backgroundColor: '#f59e0b' + '08', borderColor: '#f59e0b' + '25' },
                ]}
              >
            ...
            <Text style={[styles.gcLabel, { color: colors.mutedForeground }]}>
              GC: {project.gcName} · {project.startDate} → {project.targetCompletion}
            </Text>
          </View>
        </View>
          ...
          {(['milestones', 'budget'] as const).map((tab) => (
            ...
            >
              <Text
                style={[
                  styles.tabText,
                  { color: activeTab === tab ? ACCENT : colors.mutedForeground },
                ]}
              >
        ...
        {activeTab === 'budget' && <BudgetLines lines={project.budgetLines} />}
      </ScrollView>
    </View>
  );
}
