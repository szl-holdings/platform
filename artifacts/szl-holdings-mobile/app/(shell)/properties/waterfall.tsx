...
const MODELS: WaterfallModel[] = [
  {
    ...
    gpPromote: 20,
    tiers: [
      { id: "t1", label: "Return of Capital", preferredReturn: 0, lpSplit: 90, gpSplit: 10, hurdle: "Capital return", lpAmount: 4_995_000, gpAmount: 555_000, color: "#60a5fa" },
      { id: "t2", label: "Preferred Return (8%)", preferredReturn: 8, lpSplit: 90, gpSplit: 10, hurdle: "8% pref", lpAmount: 1_796_400, gpAmount: 199_600, color: "#34d399" },
      { id: "t3", label: "GP Catch-Up (50/50)", preferredReturn: 0, lpSplit: 50, gpSplit: 50, hurdle: "Catch-up to 20%", lpAmount: 398_000, gpAmount: 398_000, color: "#fbbf24" },
      { id: "t4", label: "Carried Interest (80/20)", preferredReturn: 0, lpSplit: 80, gpSplit: 20, hurdle: "Above pref", lpAmount: 9_648_000, gpAmount: 2_412_000, color: ACCENT },
    ],
  },
  {
    id: "wf-2",
    ...
    gpPromote: 25,
    tiers: [
      { id: "t5", label: "Return of Capital", preferredReturn: 0, lpSplit: 90, gpSplit: 10, hurdle: "Capital return", lpAmount: 2_484_000, gpAmount: 276_000, color: "#60a5fa" },
      { id: "t6", label: "Preferred Return (7%)", preferredReturn: 7, lpSplit: 90, gpSplit: 10, hurdle: "7% pref", lpAmount: 870_400, gpAmount: 96_700, color: "#34d399" },
      { id: "t7", label: "Promote (75/25)", preferredReturn: 0, lpSplit: 75, gpSplit: 25, hurdle: "Above pref", lpAmount: 7_782_000, gpAmount: 2_594_000, color: ACCENT },
    ],
  },
];
...
export default function WaterfallScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.modelTabs}>
        {MODELS.map(m => (
          <Pressable
            key={m.id}
            onPress={() => { Haptics.selectionAsync(); setSelectedModel(m.id); }}
            style={[
              styles.modelTab,
              {
          ...
          >
            <Text style={[styles.modelTabText, { color: selectedModel === m.id ? ACCENT : colors.mutedForeground }]}>
              {m.name}
            </Text>
          </Pressable>
        ))}
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        <View style={{ paddingHorizontal: 16, marginBottom: 16 }}>
          <View style={[styles.modelCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <Text style={[styles.modelProp, { color: colors.mutedForeground }]}>{model.property}</Text>
            <View style={styles.modelMetrics}>
              {[
                ...
                <View key={i} style={styles.modelMet}>
                  ...
                  <Text style={[styles.modelMetLbl, { color: colors.mutedForeground }]}>{m.label}</Text>
                </View>
              ))}
            </View>
            <View style={styles.splitBar}>
              <View style={[styles.splitSegment, { flex: 100 - gpPct, backgroundColor: "#60a5fa" }]} />
            ...
            <View style={styles.splitLegend}>
              <View style={styles.splitLegendItem}>
                ...
                <Text style={[styles.splitLegendText, { color: colors.mutedForeground }]}>LP {100 - gpPct}%</Text>
              </View>
              <View style={styles.splitLegendItem}>
                <View style={[styles.splitDot, { backgroundColor: ACCENT }]} />
                <Text style={[styles.splitLegendText, { color: colors.mutedForeground }]}>GP {gpPct}%</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        <View style={[styles.sectionHead, { borderBottomColor: colors.border }]}>
          <Text style={[styles.sectionHeadText, { color: colors.mutedForeground }]}>DISTRIBUTION TIERS</Text>
        </View>
        ...
        <View style={{ paddingHorizontal: 16, gap: 10, paddingTop: 12 }}>
          {model.tiers.map((tier, idx) => {
            ...
            return (
              <View key={tier.id} style={[styles.tierCard, { backgroundColor: colors.surface, borderColor: tier.color + "25", borderLeftColor: tier.color, borderLeftWidth: 3 }]}>
                <View style={styles.tierTop}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={[styles.tierHurdle, { color: colors.mutedForeground }]}>{tier.hurdle} · {tier.lpSplit}/{tier.gpSplit} split</Text>
                  </View>
                  <Text style={[styles.tierTotal, { color: colors.cream }]}>{fmt(tierTotal)}</Text>
                </View>
                <View style={tierBar}>
                  <View style={[{ width: `${lpPct}%` as DimensionValue, backgroundColor: "#60a5fa", height: 6 }]} />
                  <View style={[{ width: `${100 - lpPct}%` as DimensionValue, backgroundColor: tier.color, height: 6 }]} />
                </View>
                <View style={styles.tierAmounts}>
                  <Text style={[styles.tierAmt, { color: "#60a5fa" }]}>LP {fmt(tier.lpAmount)}</Text>
                  <Text style={[styles.tierAmt, { color: tier.color }]}>GP {fmt(tier.gpAmount)}</Text>
                </View>
              </View>
            );
          })}
...
const styles = StyleSheet.create({
  ...
  splitBar: { flexDirection: "row", height: 8, borderRadius: 4, overflow: "hidden", marginBottom: 6 },
  ...
  tierNum: { width: 24, height: 24, borderRadius: 6, alignItems: "center", justifyContent: "center", flexShrink: 0 },
