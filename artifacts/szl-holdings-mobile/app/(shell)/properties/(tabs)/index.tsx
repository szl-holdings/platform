...
type Marker = {
  id: string;
  address: string;
  status: 'distress' | 'opportunity' | 'watchlist' | 'portfolio';
  score: number;
  price: string;
  lat: number;
  lng: number;
    ...
    >
      <View style={[styles.markerCard, { borderColor: `${statusColor}30` }]}>
        ...
        <View style={styles.markerInfo}>
          ...
          <Text style={[styles.markerSub, { color: colors.mutedForeground }]}>
...
export default function MapTab() {
    ...
    queryFn: async () => {
      try {
        ...
        if (props.length > 0) {
          ...
          const mapped = props.slice(0, 20).map(
            (
              p: {
                id: string;
                distressType: string;
  ...
  const displayMarkers = baseMarkers.filter((m) => !selectedStatus || m.status === selectedStatus);
    ...
    try {
      ...
      try {
        const res = await fetch(
          API_BASE +
            '/terra/distress/nearby?lat=' +
            latitude +
            '&lng=' +
            longitude +
            '&radiusMiles=2&limit=10',
        );
        if (res.ok) {
          const json = await res.json();
          const props = json.data?.properties ?? json.properties ?? [];
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: topPad + 12 }]}>
        ...
        <Pressable
          ...
          style={[
            styles.nearMeBtn,
            {
              borderColor: nearMeActive ? colors.gold : colors.border,
              backgroundColor: nearMeActive ? colors.goldDim : 'transparent',
            },
          ]}
        >
          <Feather
            name={locationLoading ? 'loader' : 'navigation'}
            size={14}
            color={nearMeActive ? colors.gold : colors.mutedForeground}
          />
          <Text
            style={[
              styles.nearMeText,
        ...
        >
          <Pressable
            ...
            style={[
              styles.filterChip,
              {
                borderColor: !selectedStatus ? colors.gold : colors.border,
                backgroundColor: !selectedStatus ? colors.goldDim : 'transparent',
              },
            ]}
          >
            <Text
              style={[
                styles.filterText,
                { color: !selectedStatus ? colors.gold : colors.mutedForeground },
              ]}
            >
              All
            </Text>
          ...
          {statuses.map((s) => (
            ...
            >
              ...
              <Text
                style={[
                  styles.filterText,
                  { color: selectedStatus === s.key ? s.color : colors.mutedForeground },
                ]}
              >
                {s.label}
              </Text>
      ...
      >
        <View style={styles.mapGrid}>
          {displayMarkers.map((m, idx) => (
            <Pressable
              ...
              onPress={() => {
                Haptics.selectionAsync();
                setSelectedMarker(m === selectedMarker ? null : m);
              }}
              style={[
                styles.mapPin,
                {
        ...
        <View style={styles.mapOverlay}>
          ...
          <Text
            style={[styles.mapNote, { color: mapApiError ? colors.rose : colors.mutedForeground }]}
          >
            {mapApiError
      ...
      {selectedMarker && (
        ...
        >
          <View style={styles.markerDetailRow}>
            <View
              style={[styles.statusDot, { backgroundColor: STATUS_COLORS[selectedMarker.status] }]}
            />
            <View style={{ flex: 1 }}>
              <Text style={[styles.markerDetailAddress, { color: colors.cream }]}>
                {selectedMarker.address}
              </Text>
              <Text style={[styles.markerDetailSub, { color: colors.mutedForeground }]}>
                {STATUS_LABELS[selectedMarker.status]} · Score {selectedMarker.score} ·{' '}
          ...
          <View style={[styles.markerActions, { borderTopColor: colors.border }]}>
            ...
            <Pressable
              style={[
                styles.actionBtn,
                { backgroundColor: 'rgba(192,80,58,0.1)', borderColor: 'rgba(192,80,58,0.2)' },
              ]}
            >
      ...
      >
        {displayMarkers.map((m) => (
          <MarkerCard
            key={m.id}
            marker={m}
            onPress={() => setSelectedMarker(m === selectedMarker ? null : m)}
          />
        ))}
      </ScrollView>
    </View>
