...
type ClauseCategory = "rent" | "term" | "option" | "insurance" | "use" | "exclusivity" | "termination";
...
const LEASES: AbstractedLease[] = [
  {
    ...
    aiConfidence: 97,
    clauses: [
      { id: "c1", category: "rent", title: "Base Rent & Escalation", summary: "Base rent $31,250/mo with 3% annual escalation on Jan 1 each year. First escalation Jan 2023.", riskLevel: "low" },
      { id: "c2", category: "option", title: "Renewal Option", summary: "Two 3-year renewal options at 95% of prevailing market rate. Tenant must exercise 9 months prior to expiry.", riskLevel: "low", criticalDate: "Mar 31, 2027" },
      { id: "c3", category: "termination", title: "Early Termination", summary: "Tenant may terminate after Year 4 with 6 months notice and a termination fee equal to 4 months unamortized TI allowance.", riskLevel: "medium", criticalDate: "Jan 1, 2026" },
      { id: "c4", category: "insurance", title: "Insurance Requirements", summary: "General liability $2M per occurrence / $4M aggregate. Landlord named as additional insured. Certificates due annually.", riskLevel: "low" },
      { id: "c5", category: "exclusivity", title: "Exclusivity Clause", summary: "Landlord may not lease to direct competitors in SaaS infrastructure sector within the building for lease term.", riskLevel: "medium" },
      { id: "c6", category: "use", title: "Permitted Use", summary: "Technology company operations, software development, and ancillary office use only.", riskLevel: "low" },
    ],
  },
  {
    id: "la-2",
    ...
    aiConfidence: 91,
    clauses: [
      { id: "c7", category: "rent", title: "Base Rent", summary: "Base rent $23,100/mo escalating 2% annually. Current rent below market — 30% mark-to-market gap identified.", riskLevel: "high" },
      { id: "c8", category: "term", title: "Lease Expiry", summary: "Lease expires Jun 30, 2026. No renewal option. Tenant has indicated intent to downsize.", riskLevel: "high", criticalDate: "Jun 30, 2026" },
      { id: "c9", category: "option", title: "ROFO", summary: "Tenant has right of first offer on any adjacent space that becomes available during lease term.", riskLevel: "medium" },
    ],
  },
];
...
function ClauseCard({ clause }: { clause: LeaseClause }) {
  ...
  return (
    <View style={[styles.clauseCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
      <View style={styles.clauseTop}>
        <View style={[styles.clauseIcon, { backgroundColor: ACCENT + "15", borderColor: ACCENT + "25" }]}>
        ...
        {clause.criticalDate && (
          <View style={[styles.datePill, { backgroundColor: "#fbbf24" + "15", borderColor: "#fbbf24" + "30" }]}>
            <Feather name="calendar" size={9} color="#fbbf24" />
            <Text style={[styles.dateText, { color: "#fbbf24" }]}>{clause.criticalDate}</Text>
          </View>
        )}
      </View>
      <Text style={[styles.clauseSummary, { color: colors.mutedForeground }]}>{clause.summary}</Text>
    </View>
  );
}
...
export default function LeaseAbstractionScreen() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.leaseTabs}>
        {LEASES.map(l => (
          <Pressable
            key={l.id}
            onPress={() => { Haptics.selectionAsync(); setSelectedId(l.id); }}
            style={[
              styles.leaseTab,
              {
          ...
          >
            <Text style={[styles.leaseTabText, { color: selectedId === l.id ? ACCENT : colors.mutedForeground }]}>
              {l.tenant}
            </Text>
          </Pressable>
        ))}
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        <View style={{ paddingHorizontal: 16, marginBottom: 14 }}>
          <View style={[styles.leaseCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <View style={styles.leaseCardTop}>
              <View style={{ flex: 1 }}>
                <Text style={[styles.leaseTenant, { color: colors.cream }]}>{lease.tenant}</Text>
                <Text style={[styles.leaseProp, { color: colors.mutedForeground }]}>{lease.property} · {lease.suite}</Text>
              </View>
              <View style={[styles.confidenceBadge, { backgroundColor: "#34d399" + "15", borderColor: "#34d399" + "30" }]}>
                <Feather name="cpu" size={10} color="#34d399" />
                <Text style={[styles.confidenceText, { color: "#34d399" }]}>{lease.aiConfidence}% AI</Text>
              </View>
            </View>
            <View style={styles.leaseMetrics}>
              <View style={styles.leaseMet}>
                <Text style={[styles.leaseMetVal, { color: colors.cream }]}>{fmt(lease.monthlyRent)}</Text>
              ...
              <View style={[styles.metDivider, { backgroundColor: colors.border }]} />
              <View style={styles.leaseMet}>
                <Text style={[styles.leaseMetVal, { color: colors.cream }]}>{lease.escalation}</Text>
                <Text style={[styles.leaseMetLbl, { color: colors.mutedForeground }]}>Escalation</Text>
              </View>
            </View>
            {criticalDates.length > 0 && (
              <View style={[styles.criticalBox, { backgroundColor: "#fbbf24" + "08", borderColor: "#fbbf24" + "20" }]}>
                <Feather name="clock" size={11} color="#fbbf24" />
                <Text style={[styles.criticalText, { color: "#fbbf24" }]}>
                  {criticalDates.length} critical date{criticalDates.length !== 1 ? "s" : ""} require action
                </Text>
              </View>
            )}
          </View>
        ...
        <View style={[styles.sectionLabel, { borderBottomColor: colors.border }]}>
          <Text style={[styles.sectionLabelText, { color: colors.mutedForeground }]}>EXTRACTED CLAUSES · {lease.clauses.length}</Text>
        </View>

        <View style={{ paddingHorizontal: 16, gap: 8, paddingTop: 12 }}>
          {lease.clauses.map(clause => <ClauseCard key={clause.id} clause={clause} />)}
        </View>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  alertBadge: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 8, paddingVertical: 5, borderRadius: 8, borderWidth: 1, marginTop: 14 },
  ...
  confidenceBadge: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 8, paddingVertical: 5, borderRadius: 8, borderWidth: 1 },
  ...
  criticalBox: { flexDirection: "row", alignItems: "center", gap: 7, padding: 10, borderRadius: 8, borderWidth: 1 },
  ...
  clauseIcon: { width: 30, height: 30, borderRadius: 7, borderWidth: 1, alignItems: "center", justifyContent: "center", flexShrink: 0 },
  ...
  datePill: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 7, paddingVertical: 4, borderRadius: 6, borderWidth: 1 },
