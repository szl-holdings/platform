...
const founderCommands: SpotlightCommand[] = [
  { id: "nav-home", label: "Home", description: "Founder personal command", icon: "◊", group: "Navigate", action: () => router.push("/(shell)/founder/(tabs)") },
  { id: "nav-articles", label: "Articles", description: "Published writing", icon: "📝", group: "Navigate", action: () => router.push("/(shell)/founder/(tabs)/articles") },
  { id: "nav-ventures", label: "Ventures", description: "Platform ventures", icon: "🚀", group: "Navigate", action: () => router.push("/(shell)/founder/(tabs)/ventures") },
  { id: "nav-tools", label: "Tools", description: "Command tools", icon: "🔧", group: "Navigate", action: () => router.push("/(shell)/founder/(tabs)/tools") },
];

function NativeTabLayout() {
  const { setActiveWorkspace } = useWorkspace();
  useEffect(() => { setActiveWorkspace("founder"); }, [setActiveWorkspace]);
  return (
    <NativeTabs>
    ...
    <Tabs screenOptions={tabBarScreenOptions}>
      ...
      <Tabs.Screen
        ...
        options={{
          title: "Tools",
          tabBarIcon: ({ color }) =>
            isIOS ? <SymbolView name="terminal" tintColor={color} size={22} /> : <Feather name="cpu" size={20} color={color} />,
        }}
      />
      <Tabs.Screen
        name="profile"
        options={{
          title: "Profile",
          tabBarIcon: ({ color }) =>
            isIOS ? <SymbolView name="person.fill" tintColor={color} size={22} /> : <Feather name="user" size={20} color={color} />,
        }}
      />
    </Tabs>
  );
