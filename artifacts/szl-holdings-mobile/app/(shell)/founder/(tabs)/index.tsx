...
const MILESTONES = [
  ...
  {
    year: '2024 Q1',
    event: 'Shipped Counsel — Execution Fabric',
    role: 'Lead Architect',
    company: 'SZL Holdings',
    logoInitials: 'AL',
...
const CASE_STUDIES_FALLBACK: CaseStudy[] = [
  ...
  },
  {
    ...
    outcome:
      '500K+ properties indexed. First-mover advantage on distress pipeline delivered to institutional clients.',
  },
];
...
export default function HomeScreen() {
  ...
  const ventures = VENTURES.map((v) => {
    ...
    if (v.slug === 'terra' && liveDistress != null) {
      return {
        ...v,
        metrics: v.metrics.map((m) =>
          m.label === 'Properties' ? { ...m, value: liveDistress.toLocaleString() } : m,
        ),
      };
    }
    return v;
  ...
  const handleSectionLayout = useCallback(
    (sectionId: string) => (event: { nativeEvent: { layout: { y: number } } }) => {
  ...
  const handleShare = async () => {
    ...
    try {
      if (Platform.OS === 'web') {
        Alert.alert('Stephen Lutar — Founder Card', shareText, [
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <Animated.View
        style={[styles.navFixed, navStyle, { top: topPad, borderBottomColor: colors.border }]}
      >
        {isIOS ? (
      ...
      >
        ...
        <View onLayout={handleSectionLayout('portfolio')} style={styles.section}>
          <SectionHeader
            label="Portfolio"
            subtitle="Six platforms. One architecture."
            colors={colors}
          />
          <ScrollView
            horizontal
            ...
            onMomentumScrollEnd={(e) => {
              const idx = Math.round(e.nativeEvent.contentOffset.x / (VENTURE_CARD_WIDTH + 16));
        ...
        <View onLayout={handleSectionLayout('case-studies')} style={styles.section}>
          ...
          {csLoading ? (
            ...
            <ScrollView
              ...
              nestedScrollEnabled
            >
              {(caseStudies && caseStudies.length > 0 ? caseStudies : CASE_STUDIES_FALLBACK).map(
                (cs) => (
                  <CaseStudyCard
                    key={cs.id}
                    study={cs}
        ...
        <View onLayout={handleSectionLayout('thesis')} style={styles.section}>
          ...
          <ScrollView
            ...
            snapToAlignment="start"
            onMomentumScrollEnd={(e) => {
              const idx = Math.round(e.nativeEvent.contentOffset.x / (THESIS_CARD_WIDTH + 16));
        ...
        <View onLayout={handleSectionLayout('writing')} style={styles.section}>
          ...
          {artLoading ? (
            ...
            <View style={{ paddingHorizontal: 20, gap: 12 }}>
              {(articles && articles.length > 0 ? articles : ARTICLES_FALLBACK)
                .slice(0, 5)
                .map((article) => (
                  <ArticleRow
                    key={article.id}
...
function HeroPulseRings({ colors }: { colors: ReturnType<typeof useColors> }) {
  ...
  const ring2Style = useAnimatedStyle(() => ({
    opacity: interpolate(pulse2.value, [0.5, 1], [0.04, 0.1], Extrapolation.CLAMP),
  ...
  return (
    <Animated.View style={animStyle}>
      ...
      >
        ...
        />
        <Text
          style={[styles.actionBtnText, { color: primary ? colors.background : colors.foreground }]}
        >
          {label}
        </Text>
      </TouchableOpacity>
    ...
    <Animated.View entering={FadeInDown.delay(index * 80).springify()} style={styles.milestoneRow}>
      ...
      <View style={styles.milestoneContent}>
        <View style={styles.milestoneYearRow}>
          ...
          <View
            style={[
              styles.milestoneRoleBadge,
              { backgroundColor: colors.card, borderColor: colors.border },
            ]}
          >
            <Text
              style={[styles.milestoneRoleText, { color: colors.mutedForeground }]}
              numberOfLines={1}
            >
              {milestone.role}
            </Text>
