...
type UsageSummary = {
  ...
  summary: {
    totalMembers: number; activeUsers: number;
    apiCalls: number; storageBytes: number; storageMB: number;
  };
...
export default function UsageScreen() {
  ...
  const STAT_CARDS = [
    { label: "Active Users", value: summary ? fmt(summary.activeUsers) : "—", icon: "users" as const, color: "#6366f1", sub: `of ${summary ? fmt(summary.totalMembers) : "—"}` },
    { label: "API Calls", value: summary ? fmt(summary.apiCalls) : "—", icon: "activity" as const, color: ACCENT, sub: `last ${days} days` },
    { label: "Storage", value: summary ? fmtBytes(summary.storageBytes) : "—", icon: "hard-drive" as const, color: "#10b981", sub: "used" },
    { label: "Features", value: String(features.length), icon: "zap" as const, color: "#f59e0b", sub: "active" },
  ];
      ...
      <View style={[styles.header, { paddingTop: insets.top + 12, borderBottomColor: colors.border }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {usageQuery.data?.org?.name && (
            <Text style={[styles.orgName, { color: colors.muted }]}>{usageQuery.data.org.name}</Text>
          )}
        </View>
        <View style={styles.periodPicker}>
          {PERIOD_OPTIONS.map((p) => (
            ...
            >
              <Text style={[styles.periodBtnText, { color: days === p.days ? "#000" : colors.muted }]}>
                {p.label}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      >
        {usageQuery.isLoading || defaultOrgQuery.isLoading ? (
          ...
          <>
            ...
            {features.length > 0 && (
              <>
                <Text style={[styles.sectionTitle, { color: colors.muted }]}>FEATURE UTILIZATION</Text>
                {features.slice(0, 10).map((f) => {
                  const max = Math.max(...features.map((x) => x.quantity), 1);
                  const pct = (f.quantity / max) * 100;
                  return (
                    <View key={f.feature} style={[styles.featureRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
                      <View style={styles.featureInfo}>
                        <Text style={[styles.featureName, { color: colors.foreground }]} numberOfLines={1}>
                          {f.feature}
                        </Text>
                        <View style={styles.featureBar}>
                          <View style={[styles.featureBarFill, { width: `${pct}%`, backgroundColor: ACCENT }]} />
                        </View>
                      </View>
                      <Text style={[styles.featureCount, { color: ACCENT }]}>{fmt(f.quantity)}</Text>
                    </View>
                  );
                })}
              </>
...
const styles = StyleSheet.create({
  ...
  statCard: {
    width: "47%", padding: 14, borderRadius: 16, borderWidth: StyleSheet.hairlineWidth,
  },
  ...
  sectionTitle: { fontSize: 11, fontWeight: "700", letterSpacing: 0.5, marginTop: 4, marginBottom: 4 },
  featureRow: {
    flexDirection: "row", alignItems: "center", gap: 12, padding: 14,
    borderRadius: 12, borderWidth: StyleSheet.hairlineWidth, marginBottom: 6,
  },
  featureInfo: { flex: 1 },
  featureName: { fontSize: 12, fontWeight: "600", marginBottom: 6 },
  featureBar: { height: 4, backgroundColor: "rgba(255,255,255,0.08)", borderRadius: 2, overflow: "hidden" },
