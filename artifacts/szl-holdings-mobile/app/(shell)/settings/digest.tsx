      ...
      >
        ...
        >
          <View style={styles.masterToggleLeft}>
            ...
            <View>
              <Text style={[styles.masterToggleName, { color: colors.foreground }]}>
                Morning Briefing
              </Text>
              <Text style={[styles.masterToggleSub, { color: colors.mutedForeground }]}>
                {config.enabled
                  ? `Delivers at ${formatTime(config.deliveryHour, config.deliveryMinute)}`
                  : 'Currently disabled'}
              </Text>
            </View>
          </View>
        ...
        {config.enabled && (
          <>
            ...
            >
              <TouchableOpacity style={styles.timeRow} onPress={() => setShowTimePicker((v) => !v)}>
            ...
            >
              {(Object.entries(config.sections) as [keyof DigestConfig['sections'], boolean][]).map(
                ([key, val]) => {
                  ...
                  return (
                    ...
                    >
                      <Text style={[styles.sectionRowText, { color: colors.foreground }]}>
                        {sectionLabels[key]}
                      </Text>
                      <Switch
                        value={val}
              ...
              {(['concise', 'detailed'] as const).map((fmt) => (
                ...
                >
                  <Text
                    style={[
                      styles.formatLabel,
                      { color: config.digestFormat === fmt ? ACCENT : colors.foreground },
                    ]}
                  >
                  ...
                  <Text style={[styles.formatSub, { color: colors.mutedForeground }]}>
                    {fmt === 'concise'
                      ? 'Top 5 signals, 30-second read'
                      : 'Full domain rundown, 2-minute read'}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
