...
const SECURITY_LEVELS: SecurityLevelRow[] = [
  ...
  { level: "Sensitive Screens", requirement: "Biometric or PIN", icon: "eye-off", color: "#f59e0b" },
  { level: "Financial Actions", requirement: "Biometric required", icon: "dollar-sign", color: "#ef4444" },
];
...
export default function SecuritySettingsScreen() {
  ...
  const { isEnabled: biometricEnabled, enableBiometric, disableBiometric, isAvailable } = useBiometric();
  ...
  useEffect(() => {
    ...
    hasPINSet().then(setPinSet).catch(() => {});
  }, []);
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: insets.top + 12, borderBottomColor: colors.border }]}>
        ...
        <View style={styles.headerRight}>
          <View style={[styles.shieldBadge, { backgroundColor: `${ACCENT}10`, borderColor: `${ACCENT}30` }]}>
      ...
      >
        <View style={[styles.infoCard, { backgroundColor: `${ACCENT}08`, borderColor: `${ACCENT}20` }]}>
          ...
          <Text style={[styles.infoText, { color: colors.mutedForeground }]}>
            Configure layered security for sensitive executive workspaces. Enterprise-grade protection for classified intelligence.
          </Text>
        </View>
        ...
        <SecuritySection title="BIOMETRIC AUTHENTICATION">
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <View style={[styles.settingRow, { borderBottomColor: colors.border }]}>
              <View style={styles.settingLeft}>
                ...
                <View>
                  <Text style={[styles.settingName, { color: colors.foreground }]}>Face ID / Touch ID</Text>
            ...
            <View style={[styles.settingRow, { borderBottomColor: colors.border }]}>
              <View style={styles.settingLeft}>
                ...
                <View>
                  <Text style={[styles.settingName, { color: colors.foreground }]}>Financial Re-auth</Text>
                  <Text style={[styles.settingSub, { color: colors.mutedForeground }]}>
                    {financialReauth
          ...
          <View style={[styles.levelCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Text style={[styles.levelTitle, { color: colors.foreground }]}>Active Security Layers</Text>
            {SECURITY_LEVELS.map((item) => {
              const active =
                item.level === "Financial Actions"
                  ...
                  : biometricEnabled;
              return (
                <View key={item.level} style={[styles.levelRow, { borderBottomColor: colors.border }]}>
                  <View style={[styles.levelIcon, { backgroundColor: `${item.color}15` }]}>
                    <Feather name={item.icon} size={13} color={item.color} />
                  </View>
                  <View style={styles.levelText}>
                    <Text style={[styles.levelName, { color: colors.foreground }]}>{item.level}</Text>
                    <Text style={[styles.levelReq, { color: colors.mutedForeground }]}>{item.requirement}</Text>
                  </View>
                  <View style={[styles.levelActive, { backgroundColor: active ? `${item.color}20` : "#33333330" }]}>
                    <Text style={[styles.levelActiveText, { color: active ? item.color : colors.mutedForeground }]}>
                      {active ? "ON" : "OFF"}
                    </Text>
                  </View>
                </View>
        ...
        <SecuritySection title="SCREENSHOT PREVENTION">
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Text style={[styles.cardNote, { color: colors.mutedForeground }]}>
              Block screenshots and screen recording per workspace. Defense, Advisory, and Portfolio are protected by default.
            </Text>
          </View>
            ...
            {WORKSPACES.map((ws, index) => {
              ...
              return (
                ...
                >
                  ...
                  {enabled && (
                    <View style={[styles.protectedChip, { backgroundColor: `${ws.accent}10`, borderColor: `${ws.accent}30` }]}>
                      <Feather name="shield" size={9} color={ws.accent} />
                      <Text style={[styles.protectedChipText, { color: ws.accent }]}>Protected</Text>
                    </View>
                  )}
                  <Switch
                    value={enabled}
          ...
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            {ENTERPRISE_POLICIES.map((item, i, arr) => (
              ...
              >
                ...
                <Text style={[styles.policyValue, { color: colors.mutedForeground }]}>{item.value}</Text>
              </View>
            ))}
          </View>
        </SecuritySection>
