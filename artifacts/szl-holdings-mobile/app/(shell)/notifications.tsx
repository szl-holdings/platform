...
export default function NotificationsScreen() {
  ...
  const updatePrefsMutation = useMutation({
    mutationFn: (data: Partial<NotifPrefs>) =>
      apiPut("/api/user/notification-preferences", data),
    onSuccess: () => {
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      {tab === "inbox" && (
        <ScrollView
          ...
          refreshControl={
            <RefreshControl refreshing={notifQuery.isFetching} onRefresh={onRefresh} tintColor={ACCENT} />
          }
        >
              ...
              >
                ...
                <View style={styles.cardBody}>
                  <View style={styles.cardHeader}>
                    ...
                    <Text style={[styles.cardTime, { color: colors.muted }]}>{timeAgo(n.createdAt)}</Text>
                  </View>
        ...
        <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
          {prefsQuery.isLoading ? (
            ...
            <>
              ...
              {[
                { key: "inAppEnabled", label: "In-App", icon: "bell" as const, desc: "Platform alerts" },
                { key: "emailEnabled", label: "Email", icon: "mail" as const, desc: "Digest and critical alerts" },
                { key: "smsEnabled", label: "SMS", icon: "smartphone" as const, desc: "Text message alerts" },
                { key: "slackEnabled", label: "Slack", icon: "message-square" as const, desc: "Workspace notifications" },
              ].map(({ key, label, icon, desc }) => {
                const current = prefsForm[key as keyof NotifPrefs] ?? prefsQuery.data?.[key as keyof NotifPrefs] ?? false;
                return (
              ...
              <TouchableOpacity
                style={[
                  styles.saveBtn,
                  !Object.keys(prefsForm).length && styles.saveBtnDisabled,
                ]}
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 11, fontWeight: "700", letterSpacing: 0.5, marginBottom: 8, marginTop: 4 },
  prefRow: {
  ...
  saveBtn: {
    marginTop: 8, backgroundColor: ACCENT, borderRadius: 12,
