...
export default function AgentChatScreen() {
  ...
  const [messages, setMessages] = useState<Message[]>([{
    id: "welcome", role: "assistant", agentId: "muse", agentName: "Muse",
    content: "Muse creative intelligence ready. I help with content strategy, campaign ideation, brand voice, and creative briefs. What are we building together?",
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      const fallbacks: Record<string, string> = {
        ...
        alloy: "Strategy coordination active. Synthesizing brand intelligence across all touchpoints.",
  ...
  return (
    <KeyboardAvoidingView style={[styles.root, { paddingTop: insets.top }]} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      <View style={styles.header}>
        ...
        <View style={styles.pill}><View style={styles.pillDot} /><Text style={styles.pillText}>CREATIVE AI</Text></View>
      </View>
      {showPicker && (
      ...
      <ScrollView ref={scrollRef} style={styles.msgs} contentContainerStyle={styles.msgsContent} keyboardDismissMode="interactive" showsVerticalScrollIndicator={false}>
        ...
        {isLoading && <View style={styles.typing}><ActivityIndicator size="small" color={ACCENT} /><Text style={styles.typingText}>{selectedAgent.name} crafting…</Text></View>}
      </ScrollView>
      <View style={[styles.bar, { paddingBottom: insets.bottom + 8 }]}>
        <TextInput style={styles.input} value={input} onChangeText={setInput} placeholder="Ask about strategy, content…" placeholderTextColor={TEXT_DIM} multiline maxLength={2000} />
