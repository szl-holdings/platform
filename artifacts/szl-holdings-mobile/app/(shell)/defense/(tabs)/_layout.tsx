...
const defenseCommands: SpotlightCommand[] = [
  { id: "nav-dashboard", label: "SOC Dashboard", description: "Security operations center overview", icon: "🛡", group: "Navigate", action: () => router.push("/(shell)/defense/(tabs)") },
  { id: "nav-incidents", label: "Incidents", description: "Active security incidents", icon: "⚠", group: "Navigate", action: () => router.push("/(shell)/defense/(tabs)/incidents") },
  { id: "nav-ai", label: "Sentinel AI", description: "SOC intelligence assistant", icon: "🤖", group: "Navigate", action: () => router.push("/(shell)/defense/(tabs)/agent-chat") },
  { id: "nav-digest", label: "Digest", description: "Security briefings", icon: "📋", group: "Navigate", action: () => router.push("/(shell)/defense/(tabs)/digest") },
  { id: "nav-findings", label: "Findings", description: "Vulnerability findings", icon: "🔍", group: "Navigate", action: () => router.push("/(shell)/defense/findings") },
  { id: "nav-mitre", label: "MITRE ATT&CK", description: "Attack framework reference", icon: "🎯", group: "Navigate", action: () => router.push("/(shell)/defense/mitre") },
];

function NativeTabLayout() {
  const { setActiveWorkspace } = useWorkspace();
  useEffect(() => { setActiveWorkspace("defense"); }, [setActiveWorkspace]);
  return (
    <NativeTabs>
    ...
    <Tabs screenOptions={tabBarScreenOptions}>
      <Tabs.Screen
        ...
        options={{
          ...
          tabBarIcon: ({ color }) =>
            isIOS ? <SymbolView name="shield.fill" tintColor={color} size={22} /> : <Ionicons name="shield" size={20} color={color} />,
        }}
      />
      <Tabs.Screen
        name="incidents"
        options={{
          title: "Incidents",
          tabBarIcon: ({ color }) =>
            isIOS ? <SymbolView name="exclamationmark.triangle.fill" tintColor={color} size={22} /> : <Ionicons name="warning" size={20} color={color} />,
        }}
      />
      <Tabs.Screen
        name="agent-chat"
