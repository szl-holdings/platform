...
function AgentCard({ agent, live }: { agent: typeof AGENT_DEFS[0]; live?: { status: string; lastRunMs?: number; confidence?: number; delegations?: number } }) {
  const status = live?.status ?? "idle";
  return (
    <View style={styles.agentCard}>
      <View style={styles.agentCardRow}>
        <View style={[styles.agentIcon, { backgroundColor: `${agent.color}18`, borderColor: `${agent.color}30` }]}>
      ...
      <View style={styles.agentStats}>
        <View style={styles.agentStat}>
          <Text style={styles.statVal}>{live?.delegations ?? Math.floor(Math.random() * 40 + 5)}</Text>
          <Text style={styles.statLabel}>delegations</Text>
        </View>
        <View style={styles.agentStat}>
          <Text style={styles.statVal}>{live?.confidence != null ? `${Math.round(live.confidence * 100)}%` : `${Math.floor(Math.random() * 10 + 88)}%`}</Text>
          <Text style={styles.statLabel}>confidence</Text>
        </View>
        <View style={styles.agentStat}>
          <Text style={styles.statVal}>{live?.lastRunMs != null ? `${live.lastRunMs}ms` : `${Math.floor(Math.random() * 800 + 200)}ms`}</Text>
...
export default function AgentsScreen() {
  ...
  const { data: agentStats, isLoading, refetch } = useQuery({
    ...
    queryFn: () => apiGet<{ agents?: Array<{ id: string; status: string; lastRunMs?: number; confidence?: number; delegations?: number }> }>("/api/nuro-mesh/agents"),
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={[styles.scrollContent, { paddingBottom: insets.bottom + 80 }]} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={ACCENT} />} showsVerticalScrollIndicator={false}>
        {activeTab === "agents" && (
          <>
            ...
            {AGENT_DEFS.map(agent => <AgentCard key={agent.id} agent={agent} live={getLiveData(agent.id)} />)}
          </>
        )}
          ...
          <View>
            ...
            {CONNECTORS.map(c => (
              <View key={c.name} style={styles.connectorRow}>
                <View style={[styles.connectorDot, { backgroundColor: c.status === "live" ? GREEN : AMBER }]} />
                <View style={{ flex: 1 }}>
                  <Text style={styles.connectorName}>{c.name}</Text>
                  <Text style={styles.connectorStatus}>{c.status === "live" ? "Live configured" : "Demo mode"}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  statCard: { flex: 1, backgroundColor: CARD, borderRadius: 12, borderWidth: 1, borderColor: BORDER, padding: 12, alignItems: "center" },
  ...
  tab: { flex: 1, paddingVertical: 8, borderRadius: 8, alignItems: "center", backgroundColor: CARD, borderWidth: 1, borderColor: BORDER },
  ...
  agentCard: { backgroundColor: CARD, borderRadius: 14, borderWidth: 1, borderColor: BORDER, padding: 14, marginBottom: 8 },
  agentCardRow: { flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 12 },
  agentIcon: { width: 36, height: 36, borderRadius: 10, borderWidth: 1, alignItems: "center", justifyContent: "center" },
  agentInfo: { flex: 1 },
  agentName: { fontSize: 14, fontWeight: "600", color: TEXT },
  agentDomain: { fontSize: 11, color: TEXT_DIM, marginTop: 1 },
  badge: { flexDirection: "row", alignItems: "center", gap: 5, borderWidth: 1, borderRadius: 20, paddingHorizontal: 8, paddingVertical: 3 },
  badgeDot: { width: 6, height: 6, borderRadius: 3 },
  badgeText: { fontSize: 9, fontWeight: "700", letterSpacing: 0.5 },
  agentStats: { flexDirection: "row", gap: 0 },
  agentStat: { flex: 1, alignItems: "center", borderTopWidth: 1, borderTopColor: BORDER, paddingTop: 10 },
  statVal: { fontSize: 16, fontWeight: "700", color: TEXT },
  statLabel: { fontSize: 10, color: TEXT_DIM, marginTop: 2 },
  chainContainer: { gap: 0 },
  sectionTitle: { fontSize: 13, fontWeight: "600", color: TEXT_DIM, letterSpacing: 0.5, marginBottom: 12, textTransform: "uppercase" },
  ...
  chainContent: { flex: 1, backgroundColor: CARD, borderRadius: 10, borderWidth: 1, borderColor: BORDER, padding: 10, marginBottom: 4 },
  ...
  connectorRow: { flexDirection: "row", alignItems: "center", gap: 12, backgroundColor: CARD, borderRadius: 10, borderWidth: 1, borderColor: BORDER, padding: 12, marginBottom: 8 },
