...
export default function DigestTab() {
  ...
  const closedIncidents = Array.isArray(incidents) ? incidents.filter(i => i.status === "closed") : [];
  const criticalFindings = Array.isArray(findings) ? findings.filter(f => f.severity === "critical" && f.status !== "mitigated").length : 0;
  const pendingDecisions = Array.isArray(decisions) ? decisions.filter((d: Decision) => decisionNeedsReview(d)) : [];
  ...
  const overallPosture = activeIncidents.some(i => i.severity === "critical") ? "critical"
    : activeIncidents.some(i => i.severity === "high") ? "elevated"
    : "guarded";
  const postureColor = overallPosture === "critical" ? "#ef4444" : overallPosture === "elevated" ? "#f59e0b" : "#22c55e";
  ...
  const keyMetrics = [
    ...
    { label: "Security Score", value: hardeningSummary ? `${hardeningSummary.overallScore}%` : "—", live: !!hardeningSummary },
    { label: "Critical Gaps", value: hardeningSummary ? String(hardeningSummary.criticalGaps) : "—", live: !!hardeningSummary },
  ];
    ...
    <View style={[styles.container, { backgroundColor: colors.navy, paddingTop: insets.top }]}>
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        <View>
          ...
          <Text style={[styles.headerSub, { color: colors.muted }]}>Live data · {new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</Text>
        </View>
        <View style={[styles.postureBadge, { backgroundColor: postureColor + "20", borderColor: postureColor + "40" }]}>
          <Text style={[styles.postureText, { color: postureColor }]}>{overallPosture.toUpperCase()}</Text>
        </View>
      </View>
      ...
      {isLoading ? (
        ...
        <ScrollView
          ...
          refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={handleRefresh} tintColor={colors.amber} />}
        >
          <View style={[styles.section, { backgroundColor: colors.navyLight, borderColor: colors.border }]}>
              ...
              {keyMetrics.map(m => (
                <View key={m.label} style={styles.metricItem}>
                  ...
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 3 }}>
                    <Ionicons name={m.live ? "radio-button-on" : "radio-button-off"} size={8} color={m.live ? "#22c55e" : "#6b7280"} />
                    <Text style={{ fontSize: 8, fontFamily: "Inter_400Regular", color: m.live ? "#22c55e" : "#6b7280" }}>{m.live ? "live" : "pilot"}</Text>
                  </View>
                </View>
              ))}
            </View>
          ...
          <View style={[styles.section, { backgroundColor: colors.navyLight, borderColor: colors.border }]}>
            ...
            <View style={{ gap: 8 }}>
              {activeIncidents.length === 0 && (
                <Text style={[styles.incidentMeta, { color: colors.muted }]}>No active incidents</Text>
              )}
              {activeIncidents.slice(0, 5).map(inc => (
                <View key={inc.id} style={[styles.incidentRow, { borderColor: (SEVERITY_COLORS[inc.severity] ?? "#6b7280") + "30" }]}>
                  <View style={[styles.severityDot, { backgroundColor: SEVERITY_COLORS[inc.severity] ?? "#6b7280" }]} />
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.incidentTitle, { color: colors.foreground }]}>{inc.title}</Text>
                    <Text style={[styles.incidentMeta, { color: colors.muted }]}>{inc.status} {inc.assignedAnalyst ? `· ${inc.assignedAnalyst}` : ""}</Text>
                  </View>
                </View>
              ))}
            </View>
            ...
            <View style={[styles.section, { backgroundColor: colors.navyLight, borderColor: colors.border }]}>
              ...
              <View style={{ gap: 8 }}>
                {pendingDecisions.slice(0, 4).map((d: Decision) => (
                  <View key={d.id ?? d.objectId} style={[styles.incidentRow, { borderColor: "#f59e0b30" }]}>
                    <View style={[styles.severityDot, { backgroundColor: "#f59e0b" }]} />
                    <View style={{ flex: 1 }}>
                      <Text style={[styles.incidentTitle, { color: colors.foreground }]}>{d.recommendedAction ?? d.summary ?? d.decisionType ?? "Decision pending review"}</Text>
                      <Text style={[styles.incidentMeta, { color: colors.muted }]}>{d.id ?? d.objectId}</Text>
                    </View>
          ...
          <View style={[styles.section, { backgroundColor: colors.navyLight, borderColor: colors.border }]}>
            <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 4 }}>
              ...
              <View style={{ paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, backgroundColor: "#f59e0b20" }}>
                <Text style={{ fontSize: 8, fontFamily: "Inter_600SemiBold", color: "#f59e0b" }}>PILOT DATA</Text>
              </View>
            </View>
            <View style={{ gap: 8 }}>
              {PILOT_TRUST_METRICS.map(r => (
                <View key={r.label} style={styles.trustRow}>
                  ...
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                    <Text style={[styles.trustValue, { color: r.status === "good" ? "#22c55e" : "#f59e0b" }]}>{r.value}</Text>
                    <Ionicons name={r.status === "good" ? "checkmark-circle" : "warning"} size={12} color={r.status === "good" ? "#22c55e" : "#f59e0b"} />
                  </View>
                </View>
              ))}
            </View>
            ...
            <View style={[styles.section, { backgroundColor: colors.navyLight, borderColor: colors.border }]}>
              ...
              <View style={{ gap: 4, marginBottom: 8 }}>
                <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                  ...
                  <Text style={[styles.trustValue, { color: hardeningSummary.overallScore >= 80 ? "#22c55e" : hardeningSummary.overallScore >= 60 ? "#f59e0b" : "#ef4444" }]}>{hardeningSummary.overallScore}%</Text>
                </View>
                <View style={{ height: 4, backgroundColor: colors.border, borderRadius: 2 }}>
                  <View style={{ height: 4, borderRadius: 2, width: `${hardeningSummary.overallScore}%`, backgroundColor: hardeningSummary.overallScore >= 80 ? "#22c55e" : hardeningSummary.overallScore >= 60 ? "#f59e0b" : "#ef4444" }} />
                </View>
              </View>
              <Text style={[styles.incidentMeta, { color: "#ef4444" }]}>{hardeningSummary.criticalGaps} critical control gaps</Text>
              <Text style={[styles.incidentMeta, { color: colors.muted }]}>{hardeningSummary.implemented} of {hardeningSummary.total} controls implemented</Text>
            </View>
          )}
          ...
          <View style={[styles.section, { backgroundColor: "#1e3a1e", borderColor: "#22c55e30" }]}>
            ...
            <Text style={[styles.incidentMeta, { color: "#86efac" }]}>· Incidents, findings, decisions: live API</Text>
            <Text style={[styles.incidentMeta, { color: "#86efac" }]}>· Security score: live (hardening-summary endpoint)</Text>
            <Text style={[styles.incidentMeta, { color: "#86efac80" }]}>· Trust quality metrics: seeded pilot data</Text>
            <Text style={[styles.incidentMeta, { color: "#86efac80" }]}>· MTTD/MTTR: not yet instrumented in production</Text>
            <Text style={[styles.incidentMeta, { color: "#86efac80", marginTop: 6 }]}>Updated: {new Date().toLocaleString()}</Text>
          </View>
        </ScrollView>
      )}
    </View>
...
const styles = StyleSheet.create({
  ...
  incidentRow: { flexDirection: "row", alignItems: "center", gap: 10, padding: 8, borderRadius: 8, borderWidth: 1 },
