...
const DEMO_INCIDENTS: Incident[] = [
  {
    id: 1001,
    title: 'Ransomware detected on OT gateway',
    severity: 'critical',
    status: 'triage',
    assignedAnalyst: 'Martinez',
    attackTechnique: 'T1486',
    affectedAsset: 'ICS Controller 3',
    detectedAt: new Date(Date.now() - 3600000).toISOString(),
  },
  {
    id: 1002,
    title: 'Unauthorized SSH login attempt — Vessel VSAT',
    severity: 'high',
    status: 'investigation',
    assignedAnalyst: 'Chen',
    attackTechnique: 'T1110',
    affectedAsset: 'Bridge Terminal',
    detectedAt: new Date(Date.now() - 7200000).toISOString(),
  },
  {
    id: 1003,
    title: 'Lateral movement within cargo ops subnet',
    severity: 'high',
    status: 'containment',
    assignedAnalyst: 'Okonkwo',
    attackTechnique: 'T1021',
    affectedAsset: 'Cargo Ops LAN',
    detectedAt: new Date(Date.now() - 14400000).toISOString(),
  },
  {
    id: 1004,
    title: 'Suspicious DNS exfiltration pattern',
    severity: 'medium',
    status: 'detection',
    attackTechnique: 'T1048',
    affectedAsset: 'DNS Gateway',
    detectedAt: new Date(Date.now() - 86400000).toISOString(),
  },
  {
    id: 1005,
    title: 'AIS spoofing signal detected — Port Approach',
    severity: 'medium',
    status: 'triage',
    assignedAnalyst: 'Nakamura',
    affectedAsset: 'AIS Receiver',
  ...
  return (
    <View style={{ marginBottom: 8, overflow: 'hidden' }}>
      <View style={[styles.swipeActions, { backgroundColor: colors.navySurface }]}>
        <TouchableOpacity
          style={[styles.swipeActionBtn, { backgroundColor: colors.emeraldDim }]}
          onPress={handleAck}
        >
        ...
        <TouchableOpacity
          style={[styles.swipeActionBtn, { backgroundColor: colors.amberDim }]}
          onPress={() => onAssign(incident.id)}
        >
          <Ionicons name="person-add" size={18} color={colors.amber} />
          <Text style={[styles.swipeActionText, { color: colors.amber }]}>ASSIGN</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.swipeActionBtn, { backgroundColor: colors.redDim }]}
          onPress={handleEsc}
        >
        ...
        <TouchableOpacity
          style={[styles.swipeActionBtn, { backgroundColor: colors.border }]}
          onPress={() => onDismiss(incident.id)}
        >
              ...
              <View style={styles.cardMeta}>
                ...
                {incident.attackTechnique && (
                  <View style={[styles.techBadge, { backgroundColor: colors.accent }]}>
                    <Text style={[styles.techText, { color: colors.amber }]}>
                      {incident.attackTechnique}
                    </Text>
                  </View>
                )}
...
export default function IncidentsScreen() {
  ...
  const handleAssign = (id: number) => {
    Alert.alert('Assign Incident', 'Select analyst to assign:', [
  ...
  const bulkAssign = () => {
    Alert.alert('Assign Selected', `Assign ${selected.size} incident(s) to:`, [
      ...ANALYSTS.map((analyst) => ({
        text: analyst,
        onPress: () => {
  ...
  const fuzzyIncidents = useFuzzySearch(incidents, search, (i) => [
    i.title ?? '',
    i.severity ?? '',
    i.status ?? '',
  ]);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        {bulkMode ? (
          <>
            ...
            <Text
              style={[
                styles.bulkCount,
                { color: colors.amber, fontFamily: 'SpaceGrotesk_700Bold' },
              ]}
            >
      ...
      {bulkMode && selected.size > 0 && (
        ...
        >
          <TouchableOpacity
            style={[styles.bulkBtn, { backgroundColor: colors.emeraldDim }]}
            onPress={bulkAcknowledge}
          >
            <Ionicons name="checkmark-circle" size={16} color={colors.emerald} />
            <Text style={[styles.bulkBtnText, { color: colors.emerald }]}>ACK All</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.bulkBtn, { backgroundColor: colors.amberDim }]}
            onPress={bulkAssign}
          >
          ...
          <TouchableOpacity
            style={[styles.bulkBtn, { backgroundColor: colors.redDim }]}
            onPress={bulkDismiss}
          >
          ...
          return (
            <TouchableOpacity
              ...
              style={[
                styles.filterChip,
                {
                  backgroundColor: active ? `${sevColor}20` : colors.navyLight,
                  borderColor: active ? `${sevColor}50` : colors.border,
                },
              ]}
            ...
            >
              <Text
                style={[
                  styles.filterText,
                  {
                    color: active ? sevColor : colors.mutedForeground,
