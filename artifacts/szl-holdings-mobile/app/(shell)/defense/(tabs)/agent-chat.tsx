  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: 'assistant',
      content:
        'Sentinel online. I monitor threats, analyze CVEs, and coordinate incident response. What do you need?',
      agentId: 'sentinel',
      agentName: 'Sentinel',
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === assistantId
              ? { ...m, content: 'Unable to reach agent — API unavailable.', streaming: false }
              : m,
          ),
        );
      }
      ...
      const fallbackReplies: Record<string, string> = {
        sentinel:
          'Threat analysis running. All SIEM feeds nominal. No active critical incidents in the last 24h.',
        alloy:
          'Orchestrating across all domain agents. Query received and routing to best specialist.',
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === assistantId
            ? {
                ...m,
                content: fallbackReplies[selectedAgent.id] ?? 'Processing your request...',
                streaming: false,
              }
            : m,
        ),
          ...
          {AGENTS.map((agent) => (
            ...
            >
              ...
              {selectedAgent.id === agent.id && (
                <Feather name="check" size={14} color={ACCENT} style={{ marginLeft: 'auto' }} />
              )}
            </TouchableOpacity>
          ))}
        </Animated.View>
