...
export default function ProfileScreen() {
  ...
  const handleToggleBiometric = async (value: boolean) => {
    if (value) {
      ...
      Alert.alert("Disable Biometric Lock", "Are you sure? The app will no longer require authentication on resume.", [
  ...
  return (
    ...
    >
      ...
      <View style={styles.avatarSection}>
        <View style={[styles.avatar, { backgroundColor: colors.amberDim, borderColor: colors.amberBorder }]}>
          <Text style={[styles.avatarText, { color: colors.amber, fontFamily: "SpaceGrotesk_700Bold" }]}>{initials}</Text>
        </View>
        <Text style={[styles.userName, { color: colors.foreground, fontFamily: "SpaceGrotesk_600SemiBold" }]}>
          {user?.displayName ?? "Analyst"}
        </Text>
        <Text style={[styles.userEmail, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>
          {user?.email ?? "—"}
        </Text>
        <View style={[styles.roleBadge, { backgroundColor: colors.amberDim, borderColor: colors.amberBorder }]}>
      ...
      <View style={styles.section}>
        <Text style={[styles.sectionLabel, { color: colors.mutedForeground, fontFamily: "Inter_500Medium" }]}>SECURITY</Text>

        {(isAvailable || Platform.OS === "web") && (
          <View style={[styles.settingRow, { backgroundColor: colors.navyLight, borderColor: colors.border }]}>
            <View style={styles.settingLeft}>
              <View style={[styles.settingIcon, { backgroundColor: colors.amberDim }]}>
                <Ionicons name="finger-print" size={18} color={colors.amber} />
              </View>
              <View>
                <Text style={[styles.settingTitle, { color: colors.foreground, fontFamily: "Inter_500Medium" }]}>
                  Biometric Lock
                </Text>
                <Text style={[styles.settingDesc, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>
                  {isEnabled ? "App locks after 5 min background" : "Face ID / Fingerprint on resume"}
                </Text>
              </View>
            </View>
            <Switch
      ...
      <View style={styles.section}>
        <Text style={[styles.sectionLabel, { color: colors.mutedForeground, fontFamily: "Inter_500Medium" }]}>SOC STATUS</Text>

        {(
          [
            { icon: "shield-checkmark", label: "Aegis Platform", value: "Connected", color: colors.emerald },
          ...
          ] as Array<{ icon: ComponentProps<typeof Ionicons>["name"]; label: string; value: string; color: string }>
        ).map((item) => (
          <View key={item.label} style={[styles.statusRow, { backgroundColor: colors.navyLight, borderColor: colors.border }]}>
            <View style={styles.settingLeft}>
            ...
            <View style={[styles.valueChip, { backgroundColor: colors.emeraldDim }]}>
              <Text style={[styles.valueText, { color: colors.emerald, fontFamily: "Inter_500Medium" }]}>
                {item.value}
              </Text>
            </View>
          </View>
      ...
      <View style={styles.section}>
        <Text style={[styles.sectionLabel, { color: colors.mutedForeground, fontFamily: "Inter_500Medium" }]}>ABOUT</Text>
        <View style={[styles.aboutCard, { backgroundColor: colors.navyLight, borderColor: colors.border }]}>
          <View style={styles.aboutRow}>
            <Text style={[styles.aboutKey, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>App</Text>
            <Text style={[styles.aboutValue, { color: colors.foreground, fontFamily: "Inter_500Medium" }]}>Aegis Mobile v1.0</Text>
          </View>
          <View style={styles.aboutRow}>
            <Text style={[styles.aboutKey, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>Platform</Text>
            <Text style={[styles.aboutValue, { color: colors.foreground, fontFamily: "Inter_500Medium" }]}>{Platform.OS === "ios" ? "iOS" : Platform.OS === "android" ? "Android" : "Web"}</Text>
          </View>
          <View style={styles.aboutRow}>
            <Text style={[styles.aboutKey, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>Backend</Text>
            <Text style={[styles.aboutValue, { color: colors.foreground, fontFamily: "Inter_500Medium" }]}>Aegis API</Text>
          </View>
        </View>
      </View>
      ...
      <View style={[styles.logoutSection, { paddingBottom: 16 }]}>
        <TouchableOpacity
          style={[styles.logoutBtn, { backgroundColor: colors.redDim, borderColor: colors.redBorder }]}
