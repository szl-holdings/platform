...
const PORTFOLIO_PROPERTIES: PropertyPin[] = [
  { id: "p1", name: "425 Park Ave Tower", address: "425 Park Ave, New York, NY", lat: 40.758, lng: -73.972, type: "commercial", value: "$2.4B" },
  { id: "p2", name: "SZL Miami Hub", address: "1111 Brickell Ave, Miami, FL", lat: 25.761, lng: -80.192, type: "commercial", value: "$380M" },
  { id: "p3", name: "London Gateway", address: "30 St Mary Axe, London, UK", lat: 51.514, lng: -0.083, type: "commercial", value: "$620M" },
  { id: "p4", name: "Singapore Nexus", address: "1 Raffles Place, Singapore", lat: 1.284, lng: 103.851, type: "commercial", value: "$510M" },
];
...
export default function PortfolioMapScreen() {
  ...
  useEffect(() => {
    ...
    trackEvent("feature_used", { feature: "portfolio_map", product: "szl-holdings-mobile", source: "spotlight" });
  }, []);

  function handleSelectProperty(prop: PropertyPin) {
    setSelected(selected?.id === prop.id ? null : prop);
    setImgLoaded(false);
    trackEvent("feature_used", { feature: "portfolio_map_property_selected", property_id: prop.id, product: "szl-holdings-mobile" });
  }
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: insets.top + 12, borderBottomColor: colors.border }]}>
        ...
        {selected && (
          <TouchableOpacity onPress={() => { setSelected(null); setImgLoaded(false); }} style={styles.clearBtn}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        <View style={[styles.mapContainer, { borderColor: colors.border }]}>
          {!imgLoaded && (
            <View style={styles.mapPlaceholder}>
              ...
              <Text style={[styles.loadingText, { color: colors.mutedForeground }]}>Loading satellite map…</Text>
            </View>
          )}
          <Image
            source={{ uri: mapUrl }}
        ...
        <Text style={[styles.sectionLabel, { color: colors.mutedForeground }]}>PORTFOLIO PROPERTIES</Text>
        {PORTFOLIO_PROPERTIES.map((prop) => (
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <View style={[styles.pinDot, { backgroundColor: selected?.id === prop.id ? ACCENT : `${ACCENT}40` }]} />
            <View style={styles.propText}>
              <Text style={[styles.propName, { color: colors.foreground }]}>{prop.name}</Text>
              <Text style={[styles.propAddress, { color: colors.mutedForeground }]}>{prop.address}</Text>
            </View>
