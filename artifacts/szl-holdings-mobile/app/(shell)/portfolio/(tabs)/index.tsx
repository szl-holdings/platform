...
import { SkeletonLoader, useApiStatus, useSyncEngine } from '@szl-holdings/mobile-shared';
...
function SwipeApprovalCard({
  ...
  return (
    <View style={approvalStyles.wrapper}>
      <RNAnimated.View
        style={[
          approvalStyles.actionBg,
          { opacity: approveOpacity, left: 0, backgroundColor: '#10b981' },
        ]}
      >
      ...
      <RNAnimated.View
        style={[
          approvalStyles.actionBg,
          { opacity: deferOpacity, right: 0, backgroundColor: '#64748b' },
        ]}
      >
      ...
      >
        <View style={approvalStyles.topRow}>
          ...
          <Text style={[approvalStyles.platform, { color: colors.mutedForeground }]}>
            {approval.platform}
          </Text>
          <Text style={[approvalStyles.time, { color: colors.mutedForeground }]}>{relTime}</Text>
        </View>
        <Text style={[approvalStyles.title, { color: colors.cream }]} numberOfLines={2}>
          {approval.title}
        </Text>
        <View style={approvalStyles.bottomRow}>
          <Text style={[approvalStyles.requestedBy, { color: colors.mutedForeground }]}>
            req. {approval.requestedBy}
          </Text>
          {approval.amount && (
            <Text style={[approvalStyles.amount, { color: colors.gold }]}>{approval.amount}</Text>
          )}
        </View>
        <View style={approvalStyles.swipeHint}>
          <Feather name="chevron-right" size={10} color={`${cfg.color}60`} />
          <Text style={[approvalStyles.swipeHintText, { color: colors.mutedForeground }]}>
            swipe to approve · swipe left to defer
          </Text>
...
const SEVERITY_CONFIG: Record<
  HoldingAlert['severity'],
  { color: string; icon: FeatherIconName; label: string }
> = {
    ...
    <View style={[styles.alertRow, { borderColor: colors.borderSubtle }]}>
      ...
      <View style={styles.alertContent}>
        <View style={styles.alertTopRow}>
          <Text style={[styles.alertPlatform, { color: colors.cream }]}>{alert.platform}</Text>
          <Text style={[styles.alertTime, { color: colors.mutedForeground }]}>{relTime}</Text>
        </View>
...
function PlatformStatusCard({ platform }: { platform: EcosystemHealth['platforms'][0] }) {
  const colors = useColors();
...
export default function CommandScreen() {
  ...
  const pendingApprovals: PendingApproval[] = (approvalsData as PendingApproval[]).filter(
    (a) => !locallyDismissed.has(a.id),
  );
  ...
  const approveMutation = useMutation({
    mutationFn: async ({ id, action }: { id: string; action: 'approve' | 'defer' }) => {
      ...
      const body = {
        ...
        note: action === 'defer' ? 'Deferred via mobile' : undefined,
      };
      ...
      if (!syncEngine.isOnline) {
        await syncEngine.enqueue({
          domain: 'szl-holdings',
          method: 'POST',
          url: process.env.EXPO_PUBLIC_DOMAIN
            ? `https://${process.env.EXPO_PUBLIC_DOMAIN}${url}`
            : url,
          body,
          idempotencyKey,
        });
  ...
  const allOnline = health ? health.summary.online === health.summary.total : null;
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      {(isOffline || isDegraded) && (
        ...
        >
          <Text style={{ color: '#fca5a5', fontSize: 11, fontWeight: '600' }}>
            {isOffline
              ? 'Offline — portfolio data may be stale'
      ...
      >
        <View style={styles.header}>
          <View>
            <Text style={[styles.eyebrow, { color: colors.goldSubtle }]}>COMMAND CENTER</Text>
            <Text style={[styles.greeting, { color: colors.cream }]}>{greeting}</Text>
          </View>
          <View
            style={[
              styles.statusBadge,
          ...
          >
            <View
              style={[
                ...
                {
                  backgroundColor:
                    allOnline === null ? colors.mutedForeground : allOnline ? '#10b981' : '#ef4444',
                },
              ]}
            />
            <Text
        ...
        <View style={[styles.section, { borderTopColor: colors.borderSubtle }]}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionLabel, { color: colors.goldSubtle }]}>CRITICAL ALERTS</Text>
        ...
        <View style={[styles.section, { borderTopColor: colors.borderSubtle }]}>
          <Text style={[styles.sectionLabel, { color: colors.goldSubtle }]}>ECOSYSTEM HEALTH</Text>
          {healthLoading ? (
        ...
        <View style={[styles.section, { borderTopColor: colors.borderSubtle }]}>
          <Text style={[styles.sectionLabel, { color: colors.goldSubtle }]}>AGGREGATE KPIs</Text>
          {kpisLoading ? (
            <View style={{ gap: 8 }}>
              {[1, 2, 3, 4].map((i) => (
            ...
            <View style={[styles.kpiList, { borderColor: colors.borderSubtle }]}>
              <KpiRow
                label="Workflow Runs"
                value={kpis.aggregate.totalWorkflowRuns}
                icon="git-merge"
                color={colors.violet}
              />
              <KpiRow
                label="Active Incidents"
                value={kpis.aggregate.activeIncidents}
                icon="alert-triangle"
                color={colors.amber}
              />
              <KpiRow
                label="Distress Properties"
                value={kpis.aggregate.distressProperties}
                icon="home"
                color="#4d7c0f"
              />
              <KpiRow
              ...
              <KpiRow
                label="Active Deals"
                value={kpis.aggregate.activeDeals}
                icon="briefcase"
                color={colors.green}
              />
              <KpiRow
                label="Security Findings"
                value={kpis.aggregate.securityFindings}
                icon="shield"
                color={colors.red}
              />
            </View>
            ...
            <View style={styles.summaryGrid}>
              {[
                { label: 'Portfolio ARR', value: '$35M+', color: colors.gold },
                {
                  ...
                  value: `${health.summary.online}/${health.summary.total}`,
                  color: '#10b981',
                },
