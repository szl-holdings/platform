...
import { useContinuumWebSocket } from '@/hooks/useContinuumWebSocket';
...
function useRuns() {
  return useQuery<{ data: WorkflowRun[] } | WorkflowRun[]>({
    queryKey: ['szl-continuum-runs'],
    queryFn: () =>
      sharedApiFetch<{ data: WorkflowRun[] } | WorkflowRun[]>('/api/continuum/runs?limit=20'),
...
function useApprovals() {
  return useQuery<{ data: Approval[] } | Approval[]>({
    queryKey: ['szl-continuum-approvals-pending'],
    queryFn: () =>
      sharedApiFetch<{ data: Approval[] } | Approval[]>(
        '/api/continuum/approvals?status=pending&limit=20',
      ),
    refetchInterval: 60000,
    retry: 1,
  });
...
function useDecideApproval() {
  ...
  return useOptimisticMutation<unknown, Error, { id: number; decision: string }>({
    ...
    mutationFn: async ({ id, decision }) => {
      return sharedApiFetch(`/api/continuum/approvals/${id}/decide`, {
        method: 'POST',
        body: JSON.stringify({ decision }),
      });
    },
    onSettled: () => {
      qc.invalidateQueries({ queryKey: ['szl-continuum-runs'] });
    },
  });
}
...
export default function ContinuumScreen() {
  const colors = useColors();
  ...
  const { status: wsStatus, lastEvent } = useContinuumWebSocket(true);
