...
const AGENTS = [
  { id: "helmsman", name: "Helmsman", role: "Maritime Intelligence", icon: "anchor", color: "#0ea5e9" },
...
export default function AgentChatScreen() {
  ...
  const [messages, setMessages] = useState<Message[]>([{
    id: "welcome", role: "assistant", agentId: "helmsman", agentName: "Helmsman",
    content: "Helmsman online. AIS feeds active. I monitor your fleet, assess route risk, and track maritime threats. What's your query?",
    timestamp: Date.now() - 60000,
  }]);
  const [isLoading, setIsLoading] = useState(false);
  const [showPicker, setShowPicker] = useState(false);
  const { search, buildContextString } = useEmbeddingSearch({ domain: "maritime", limit: 3 });
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      setMessages(prev => prev.map(m => m.id === aid ? { ...m, content: data.response ?? data.content ?? "Done.", streaming: false } : m));
    } catch {
      const fallbacks: Record<string, string> = {
        helmsman: "Fleet status: 12 vessels active, 2 in port. No collision risks detected. Weather window clear for Atlantic routes.",
        alloy: "Cross-domain coordination active. All vessel data streams nominal.",
        sentinel: "Maritime security: No sanctioned vessel contacts. Piracy risk map updated.",
      };
  ...
  return (
    <KeyboardAvoidingView style={[styles.root, { paddingTop: insets.top }]} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      <View style={styles.header}>
        <TouchableOpacity style={styles.agentSelector} onPress={() => setShowPicker(p => !p)} activeOpacity={0.7}>
        ...
        <View style={styles.livePill}><View style={styles.liveDot} /><Text style={styles.liveText}>AIS LIVE</Text></View>
      </View>
      {showPicker && (
      ...
      <ScrollView ref={scrollRef} style={styles.messages} contentContainerStyle={styles.msgsContent} keyboardDismissMode="interactive" showsVerticalScrollIndicator={false}>
        {messages.map(msg => {
          const isUser = msg.role === "user";
          const agent = AGENTS.find(a => a.id === msg.agentId);
          return (
            <View key={msg.id} style={[styles.row, isUser && styles.rowUser]}>
              {!isUser && <View style={[styles.avatar, { backgroundColor: `${agent?.color ?? ACCENT}18`, borderColor: `${agent?.color ?? ACCENT}30` }]}><Feather name={(agent?.icon ?? "cpu") as any} size={12} color={agent?.color ?? ACCENT} /></View>}
              <View style={[styles.bubble, isUser ? styles.bubbleUser : styles.bubbleAssist]}>
                {!isUser && msg.agentName && <Text style={[styles.agentLabel, { color: agent?.color ?? ACCENT }]}>{msg.agentName}</Text>}
                <Text style={[styles.bubbleText, isUser && { color: BG }]}>{msg.streaming ? msg.content + "▍" : msg.content}</Text>
              </View>
            </View>
          );
        })}
        {isLoading && <View style={styles.typing}><ActivityIndicator size="small" color={ACCENT} /><Text style={styles.typingText}>{selectedAgent.name} scanning…</Text></View>}
      </ScrollView>
      <View style={[styles.inputBar, { paddingBottom: insets.bottom + 8 }]}>
        <TextInput style={styles.input} value={input} onChangeText={setInput} placeholder="Ask Helmsman…" placeholderTextColor={TEXT_DIM} multiline maxLength={2000} />
        <TouchableOpacity style={[styles.sendBtn, (!input.trim() || isLoading) && styles.sendOff]} onPress={sendMessage} disabled={!input.trim() || isLoading} activeOpacity={0.7}>
...
const styles = StyleSheet.create({
  ...
  livePill: { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: `${ACCENT}15`, borderRadius: 20, paddingHorizontal: 8, paddingVertical: 3 },
  liveDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: ACCENT },
  liveText: { fontSize: 9, fontWeight: "700", color: ACCENT, letterSpacing: 1 },
