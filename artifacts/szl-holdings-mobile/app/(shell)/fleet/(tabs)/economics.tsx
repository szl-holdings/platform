...
import { api, type VoyageEconomics, CACHE_KEYS, cacheSet, cacheGetStale } from "@/lib/fleet/api";
      ...
      {data.map((v, i) => (
        <View key={i} style={styles.miniBarWrap}>
          <View
            style={[
              styles.miniBar,
              { height: maxVal > 0 ? Math.max((v / maxVal) * 60, 2) : 2, backgroundColor: colors.primary },
            ]}
          />
        </View>
      ))}
    ...
    >
      ...
      <View style={styles.voyMetrics}>
        {[
          ...
          { l: "Margin", v: marginPct !== 0 ? `${marginPct.toFixed(1)}%` : "—", c: isPositive ? colors.green : colors.red },
          { l: "Net", v: margin !== 0 ? fmt(Math.abs(margin)) : "—", c: isPositive ? colors.green : colors.red },
        ...
        <View style={[styles.expanded, { borderTopColor: colors.border }]}>
          ...
          {[
            ...
            <View key={item.l} style={styles.costRow}>
              ...
              <View style={styles.costBarWrap}>
                <View
                  style={[styles.costBar, { width: `${costs > 0 ? Math.max((item.v / costs) * 100, 2) : 0}%`, backgroundColor: item.c }]}
                />
              </View>
          ...
          {marginPct !== 0 && (
            <View style={styles.marginRow}>
              ...
              <Text style={[styles.marginPctText, { color: isPositive ? colors.green : colors.red }]}>
        ...
        ListHeaderComponent={
          <>
            <View style={styles.kpiGrid}>
              <KpiCard label="Fleet Revenue" value={fmt(totalRevenue)} sub={`${voyages.length} voyages`} color={colors.green} icon="dollar-sign" />
              <KpiCard label="Fleet Margin" value={fmt(totalMargin)} sub={totalRevenue > 0 ? `${(totalMargin / totalRevenue * 100).toFixed(1)}% avg` : "—"} color={colors.primary} icon="trending-up" />
              <KpiCard label="Avg Fleet TCE" value={avgTce > 0 ? fmt(avgTce) : "—"} sub="per day" color={colors.violet} icon="bar-chart-2" />
              <KpiCard label="Fuel Spend" value={fmt(totalFuel)} sub="total fleet" color={colors.amber} icon="zap" />
            </View>

            {revenueData.length > 0 && (
              <View style={[styles.chartCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <Text style={[styles.chartTitle, { color: colors.textFaint }]}>MONTHLY REVENUE (8-MONTH)</Text>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterBar} contentContainerStyle={styles.filterContent}>
              {[
                ...
                <TouchableOpacity
                  ...
                  style={[styles.filterBtn, { backgroundColor: statusFilter === f.id ? colors.primaryDim : "transparent", borderColor: statusFilter === f.id ? colors.primaryBorder : colors.border }]}
                >
                  <Text style={[styles.filterText, { color: statusFilter === f.id ? colors.primary : colors.textDim }]}>{f.label}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>

            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={[styles.filterBar, { marginTop: 6 }]} contentContainerStyle={styles.filterContent}>
              <Text style={[styles.sortLabel, { color: colors.textFaint }]}>Sort: </Text>
              {[{ id: "margin", l: "Margin" }, { id: "tce", l: "TCE" }, { id: "revenue", l: "Revenue" }].map(s => (
                <TouchableOpacity
                  key={s.id}
                  onPress={() => setSortBy(s.id as typeof sortBy)}
                  style={[styles.filterBtn, { backgroundColor: sortBy === s.id ? colors.primaryDim : "transparent", borderColor: sortBy === s.id ? colors.primaryBorder : colors.border }]}
                >
                  <Text style={[styles.filterText, { color: sortBy === s.id ? colors.primary : colors.textDim }]}>{s.l}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>

            {isLoading && <ActivityIndicator size="small" color={colors.primary} style={{ marginTop: 20 }} />}
          </>
        }
...
const styles = StyleSheet.create({
  ...
  chartCard: {
    marginHorizontal: 16, marginTop: 12, padding: 14, borderRadius: 12, borderWidth: 1,
  },
  ...
  chartLegend: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginTop: 8 },
  ...
  voyRevenue: { fontSize: 14, fontWeight: "700" as const, fontFamily: "Inter_700Bold", marginTop: 4 },
  ...
  metricValue: { fontSize: 12, fontWeight: "700" as const, fontFamily: "Inter_700Bold", marginTop: 2 },
  ...
  costValue: { width: 48, textAlign: "right", fontSize: 10, fontWeight: "600" as const, fontFamily: "Inter_600SemiBold" },
