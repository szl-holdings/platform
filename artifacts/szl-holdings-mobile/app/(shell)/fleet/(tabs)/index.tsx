...
function WorldMapCanvas({
  ...
  const mappable = vessels.filter((v) => {
    const pos = livePositions.get(v.id);
    const lat = pos
      ? parseFloat(pos.latitude ?? '0')
      : v.latitude
        ? parseFloat(v.latitude ?? '0')
        : null;
    const lon = pos
      ? parseFloat(pos.longitude ?? '0')
      : v.longitude
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      ...
      >
          ...
          {Object.entries(STATUS_COLORS)
            ...
            .map(([k, c]) => (
              <View key={k} style={styles.legendItem}>
                ...
                <Text style={[styles.legendLabel, { color: colors.textFaint }]}>
                  {STATUS_LABELS[k]?.split(' ')[0] || k}
                </Text>
              </View>
            ))}
        </View>
        ...
        <View
          style={[styles.mapBadge, { backgroundColor: colors.card, borderColor: colors.border }]}
        >
    ...
    >
      <View style={styles.calloutHeader}>
        ...
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={[styles.calloutMeta, { color: colors.textDim }]}>
      ...
      <View style={styles.calloutFooter}>
        ...
        {livePosition && <View style={[styles.liveDot, { backgroundColor: colors.green }]} />}
...
export default function FleetMapScreen() {
  ...
  const filtered =
    statusFilter === 'all' ? vessels : vessels.filter((v) => v.status === statusFilter);
      ...
      <View style={styles.mapFullScreen}>
        ...
        <View style={[styles.mapHeaderOverlay]} pointerEvents="box-none">
          <View
            style={[
              styles.header,
              { borderBottomColor: 'transparent', backgroundColor: `${colors.bg}cc` },
            ]}
          >
            <View>
            ...
            <View style={styles.headerRight}>
              {wsConnected && (
                <View style={[styles.wsIndicator, { backgroundColor: colors.green }]} />
              )}
              <TouchableOpacity
                style={[
                  styles.headerBtn,
              ...
              return (
                ...
                >
                  ...
                  <View style={{ flex: 1, marginLeft: 10 }}>
                    ...
                    <Text style={[styles.listCardMeta, { color: colors.textDim }]}>
                      {v.vesselType} · {v.flag}
                    </Text>
                  </View>
                  <View style={{ alignItems: 'flex-end' }}>
                    ...
                    {spd !== null && (
                      <Text style={[styles.listCardSpeed, { color: colors.textFaint }]}>
                        {spd.toFixed(1)} kn
                      </Text>
                    )}
