...
const fleetCommands: SpotlightCommand[] = [
  { id: "nav-map", label: "Fleet Map", description: "Live vessel positions", icon: "🗺", group: "Navigate", action: () => router.push("/(shell)/fleet/(tabs)") },
  { id: "nav-fleet", label: "Fleet List", description: "All vessels", icon: "⚓", group: "Navigate", action: () => router.push("/(shell)/fleet/(tabs)/fleet") },
  { id: "nav-alerts", label: "Alerts", description: "Fleet alerts & exceptions", icon: "🔔", group: "Navigate", action: () => router.push("/(shell)/fleet/(tabs)/alerts") },
  { id: "nav-ai", label: "Helmsman AI", description: "Maritime intelligence assistant", icon: "🤖", group: "Navigate", action: () => router.push("/(shell)/fleet/(tabs)/agent-chat") },
];

function NativeTabLayout() {
  const { setActiveWorkspace } = useWorkspace();
  useEffect(() => { setActiveWorkspace("fleet"); }, [setActiveWorkspace]);
  return (
    <NativeTabs>
    ...
    <Tabs screenOptions={tabBarScreenOptions}>
      ...
      <Tabs.Screen
        ...
        options={{
          title: "Fleet",
          tabBarIcon: ({ color }) =>
            isIOS ? <SymbolView name="ferry" tintColor={color} size={22} /> : <Feather name="anchor" size={20} color={color} />,
        }}
      />
      <Tabs.Screen
        name="alerts"
