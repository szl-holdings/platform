...
function ImageCarousel({
  ...
  return (
    <View
      style={[styles.carouselWrap, { backgroundColor: colors.card, borderColor: colors.border }]}
    >
      <FlatList
        data={images}
        horizontal
          ...
          <View style={{ width: CARD_W, height: IMG_H }}>
            ...
            {item.caption && (
              <View style={[styles.imgCaption, { backgroundColor: `${colors.bg}cc` }]}>
                <Text style={[styles.imgCaptionText, { color: colors.textDim }]} numberOfLines={1}>
                  {item.caption}
                </Text>
              </View>
            )}
...
export default function VesselDetailScreen() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'voyage' | 'exceptions' | 'compliance'>(
    'overview',
  );
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: colors.bg }]} edges={['top']}>
        <View style={styles.loadingState}>
          ...
          <Text style={[styles.loadingText, { color: colors.textFaint }]}>
            Loading vessel data...
          </Text>
        </View>
      </SafeAreaView>
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: colors.bg }]} edges={['top']}>
        <View style={styles.loadingState}>
          ...
          <TouchableOpacity
            onPress={() => refetch()}
            style={[styles.retryBtn, { backgroundColor: colors.primaryDim }]}
          >
  ...
  const complianceChecks: ComplianceCheck[] = [
    ...
    },
    {
      ...
      detail: euClear
        ? 'Cleared against EU consolidated list'
        : 'FLAGGED — EU restriction in effect',
      icon: 'globe',
    },
    {
    ...
    },
    {
      ...
      detail:
        maintenance.length > 5
          ? 'Review due — multiple open maintenance items'
          : 'No deficiencies on file',
      icon: 'anchor',
    },
  ];
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.bg }]} edges={['top']}>
      <View style={[styles.navBar, { borderBottomColor: colors.border }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.navSub, { color: colors.textFaint }]}>
            {vessel.vesselType} · IMO {vessel.imo || '—'}
          </Text>
        </View>
      ...
      {(critExc > 0 || highExc > 0) && (
        <View
          style={[
            styles.alertBanner,
            { backgroundColor: colors.redDim, borderBottomColor: `${colors.red}20` },
          ]}
        >
          <VesselIcon name="alert-triangle" size={13} color={colors.red} />
          <Text style={[styles.alertBannerText, { color: colors.red }]}>
            {critExc > 0 && `${critExc} critical`}
            {critExc > 0 && highExc > 0 && ' · '}
            {highExc > 0 && `${highExc} high`} severity alerts active
          </Text>
        </View>
      )}
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.body}>
        {activeTab === 'overview' && (
          <>
            {data.images && data.images.length > 0 && <ImageCarousel images={data.images} />}
            ...
            >
              ...
              <View style={styles.grid}>
                ...
                <MetricPair
                  label="Built"
                  value={vessel.yearBuilt ? String(vessel.yearBuilt) : '—'}
                />
                <MetricPair
                  label="Gross Tonnage"
                  value={
                    vessel.grossTonnage ? `${Number(vessel.grossTonnage).toLocaleString()} GT` : '—'
                  }
                />
              </View>
            </View>
            ...
            {position && (
              ...
              >
                ...
                <View style={styles.grid}>
                  ...
                  <MetricPair
                    label="Speed"
                    value={`${parseFloat(position.speed).toFixed(1)} kn`}
                    color={colors.green}
                  />
                ...
                {vessel.destination && (
                  <View style={[styles.destRow, { borderTopColor: colors.border }]}>
                    ...
                    <Text style={[styles.destText, { color: colors.textDim }]}>
              ...
              >
                ...
                <View style={styles.grid}>
                  ...
                  <MetricPair
                    label="ETA"
                    value={
                      activeVoyage.estimatedArrivalAt
                        ? new Date(activeVoyage.estimatedArrivalAt).toLocaleDateString()
                        : '—'
                    }
                  />
                  <MetricPair
                  ...
                  <MetricPair
                    label="Net Margin"
                    value={margin !== 0 ? fmt(Math.abs(margin)) : '—'}
                    color={margin >= 0 ? colors.green : colors.red}
                  />
                  <MetricPair
                    label="Total Costs"
                    value={
                      activeVoyage.totalCostsUsd ? fmt(parseFloat(activeVoyage.totalCostsUsd)) : '—'
                    }
                    color={colors.amber}
                  />
                </View>
          ...
          exceptions.length === 0 ? (
              ...
              exceptions.map((exc) => {
                ...
                return (
                  <View
                    key={exc.id}
                    style={[styles.card, { backgroundColor: colors.card, borderColor: `${ec}25` }]}
                  >
                    <View style={styles.excHeader}>
                      ...
                      <View
                        style={[
                          ...
                          { backgroundColor: `${ec}15`, borderColor: `${ec}30` },
                        ]}
                      >
                        <Text style={[styles.severityText, { color: ec }]}>
                          {exc.severity.toUpperCase()}
                        </Text>
                      </View>
                    </View>
                    <Text style={[styles.excDesc, { color: colors.textDim }]}>
                      {exc.description}
                    </Text>
              ...
              <View style={styles.complianceSummaryRow}>
                {[
                  ...
                  <View
                    key={s.label}
                    style={[
                      styles.complianceSummaryItem,
                      { backgroundColor: s.bg, borderColor: `${s.color}25` },
                    ]}
                  >
                    <Text style={[styles.complianceSummaryCount, { color: s.color }]}>
                      {s.count}
                    </Text>
                    <Text style={[styles.complianceSummaryLabel, { color: s.color }]}>
                      {s.label}
                    </Text>
                  </View>
                ))}
              ...
              >
                ...
                <View
                  style={[
                    ...
                    {
                      backgroundColor:
                        sanctions.ofacStatus === 'clear' ? colors.greenDim : colors.redDim,
                ...
                >
                  ...
                  >
                    {sanctions.ofacStatus === 'clear'
                      ? 'Vessel cleared for all major jurisdictions'
                      : 'SANCTIONS FLAG — Immediate review required'}
                  </Text>
                </View>
              </View>
            )}
