...
async function buildAll() {
  ...
  await esbuild({
    ...
    alias: {
      ...
      'prom-client': path.join(
        workspaceRoot,
        'apps/alloy-embedding-api/src/middleware/prom-client-stub.ts',
      ),
    },
...
async function createFastStartWrapper(distDir) {
  ...
  try {
    await rename(serverBundleMap, serverDestMap);
  } catch {
    /* no map */
  }
...
async function copyPdfkitData() {
  let pdfkitDataSrc;
  const searchPaths = [artifactDir, path.join(artifactDir, 'node_modules'), workspaceRoot];
  try {
