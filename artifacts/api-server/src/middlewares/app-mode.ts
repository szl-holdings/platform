...
const PRODUCTION_DESTRUCTIVE_PREFIXES = [
  "/api/admin",
];
...
export function appModeMiddleware(req: Request, res: Response, next: NextFunction): void {
  ...
  if (mode === "demo" && isApiPath) {
    if (req.method === "GET" && !isReadBypassed(req.path)) {
      ...
      logger.debug({ path: req.path, mode: "demo" }, "[app-mode] Demo GET: no fixture, returning empty stub");
    ...
    if (MUTATING_METHODS.has(req.method) && !isWriteWhitelisted(req.path)) {
      logger.info({ method: req.method, path: req.path, mode: "demo" }, "[app-mode] Demo intercept: write blocked");
      res.status(200).json({
        ok: true,
        demo: true,
