...
function deriveKey(masterKey: string, context: string): Buffer {
  return Buffer.from(
    createHmac('sha256', Buffer.from(masterKey, 'hex')).update(context).digest(),
  ).slice(0, KEY_LENGTH);
}
...
function getMasterKey(): string {
  ...
  if (!key || key.length < 64) {
    ...
    logger.warn(
      '[field-encryption] FIELD_ENCRYPTION_KEY not set — using dev fallback. DO NOT USE IN PRODUCTION.',
    );
  ...
  for (const field of fields) {
    ...
    if (typeof value === 'string' && isEncryptedField(value)) {
      try {
        ...
        logger.error(
          { err, field },
          '[field-encryption] Failed to decrypt field — returning masked value',
        );
