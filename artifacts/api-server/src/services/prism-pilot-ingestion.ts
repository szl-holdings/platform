...
export class PilotIngestionService {
  async ingestEmail(
    ...
    await db
      .update(pcIngestionJobsTable)
      .set({
        status: 'completed',
        processedCount: 1 + (payload.attachments?.length ?? 0),
  ...
  async getJobStats(orgId: number) {
    const jobs = await db
      .select()
      .from(pcIngestionJobsTable)
      .where(eq(pcIngestionJobsTable.orgId, orgId));
