...
async function courtListenerFetch(endpoint: string, params: Record<string, string> = {}): Promise<Record<string, unknown>> {
...
function asArr(v: unknown): CLRaw[] { return Array.isArray(v) ? (v as CLRaw[]) : []; }
function str(v: unknown, fallback = ""): string { return typeof v === "string" ? v : fallback; }
function strN(v: unknown): string | null { return typeof v === "string" ? v : null; }
function num(v: unknown, fallback = 0): number { return typeof v === "number" ? v : fallback; }

class CourtListenerService {
  async searchDockets(query: string, court?: string, limit = 20): Promise<{
    count: number;
    ...
    try {
      ...
      return {
        ...
        results: asArr(data.results).map((r): CourtListenerDocket => ({
      ...
      logger.warn({ err: err instanceof Error ? err.message : String(err) }, "CourtListener docket search failed — returning empty");
  ...
  async getDocket(docketId: number): Promise<{
    ...
    try {
      ...
      return {
        ...
        entries: asArr(entriesData.results).map((e): DocketEntry => ({
          id: num(e.id),
      ...
      logger.warn({ err: err instanceof Error ? err.message : String(err), docketId }, "CourtListener getDocket failed");
      return { docket: null, entries: [], source: "error" };
    }
  }

  async searchOpinions(query: string, court?: string, limit = 10): Promise<{
    count: number;
    ...
    try {
      ...
      return {
        count: num(data.count),
        results: asArr(data.results).map((r): CourtListenerOpinion => ({
          id: num(r.id),
          absolute_url: `https://www.courtlistener.com${str(r.absolute_url)}`,
      ...
      logger.warn({ err: err instanceof Error ? err.message : String(err) }, "CourtListener opinion search failed");
      return { count: 0, results: [], source: "error" };
    }
  }

  async searchJudges(name: string, court?: string, limit = 10): Promise<{
    count: number;
    ...
    try {
      ...
      return {
        count: num(data.count),
        results: asArr(data.results).map((r): CourtListenerJudge => ({
          id: num(r.id),
      ...
      logger.warn({ err: err instanceof Error ? err.message : String(err) }, "CourtListener judge search failed");
      return { count: 0, results: [], source: "error" };
    }
  }

  async getRecentFilings(court?: string, limit = 15): Promise<{
    results: CourtListenerDocket[];
    source: "live" | "error";
  }> {
    try {
      ...
      logger.warn({ err: err instanceof Error ? err.message : String(err) }, "CourtListener recent filings failed");
