...
class ForecastExpandedEngine {
  async computeAllPilotOneForecasts(orgId: number, matterId: number): Promise<ForecastResult[]> {
    ...
    const [communications, liens, deadlines, approvals, pressureSnap, frictionSnap, quietRisk] =
      await Promise.all([
        db
          .select()
        ...
        db
          ...
          .where(
            and(
              eq(pcInsurerPressureSnapshotsTable.orgId, orgId),
              eq(pcInsurerPressureSnapshotsTable.matterId, matterId),
            ),
          )
    ...
    const lastInboundCarrierComm = communications.find(
      (c) =>
        c.direction === 'inbound' &&
        (c.fromParty?.toLowerCase().includes('carrier') ||
          c.fromParty?.toLowerCase().includes('adjuster')),
    );
    const daysSinceCarrierResponse = lastInboundCarrierComm
      ? Math.floor((Date.now() - new Date(lastInboundCarrierComm.sentAt!).getTime()) / 86400000)
      : 30;
    ...
    results.push(
      this.buildForecast('insurer_response_latency', {
        currentScore: Math.min(1, daysSinceCarrierResponse / 30),
        drivers: [
          `${daysSinceCarrierResponse} days since last carrier response`,
          pressureScore > 0.6 ? 'Elevated insurer pressure index' : 'Normal pressure range',
        ],
        internalDrivers: ['Communication log analysis'],
        worldlineDrivers: pressureScore > 0.5 ? ['Insurer pressure pattern matching'] : [],
        recommendedActions:
          daysSinceCarrierResponse > 14
            ? [
                'Send formal follow-up with response deadline',
                'Document silence window for potential bad faith record',
              ]
            : ['Continue monitoring communication cadence'],
        whatChanged:
          daysSinceCarrierResponse > 7
            ? 'Response lag has exceeded normal threshold'
    ...
    results.push(
      this.buildForecast('offer_movement_likelihood', {
        currentScore: Math.max(0, 0.7 - frictionScore * 0.5),
        drivers: [
          `Settlement friction at ${Math.round(frictionScore * 100)}%`,
          pressureScore > 0.5
            ? 'Carrier pressure may accelerate offer'
            : 'Moderate carrier engagement',
        ],
        ...
        recommendedActions:
          frictionScore > 0.6
            ? [
                'Reduce top friction blockers before expecting offer movement',
                'Prepare counter-offer strategy',
              ]
            : ['Position for offer movement by strengthening readiness posture'],
        whatChanged:
          frictionScore > 0.5
            ? 'High friction is suppressing offer movement probability'
            : 'Reasonable conditions for offer movement',
        confidence: 0.72,
      }),
    );

    results.push(
      this.buildForecast('settlement_friction', {
        currentScore: frictionScore,
        drivers: frictionSnap[0]
          ? [
              frictionSnap[0].smallestAction ?? 'Review friction blockers',
              `Friction class: ${frictionSnap[0].frictionClass}`,
            ]
          : ['No friction data computed yet'],
        internalDrivers: ['Settlement friction engine', 'Blocker analysis'],
        worldlineDrivers: [],
        recommendedActions: [
          frictionSnap[0]?.smallestAction ??
            'Run friction computation to identify smallest blocking action',
        ],
        whatChanged:
          frictionSnap[0]?.direction === 'rising'
            ? 'Friction has increased since last snapshot'
            : frictionSnap[0]?.direction === 'falling'
              ? 'Friction is decreasing — positive trajectory'
              : 'Friction stable',
        confidence: 0.75,
      }),
    );
    ...
    results.push(
      this.buildForecast('review_bottleneck', {
        ...
        recommendedActions:
          approvals.length > 0
            ? [
                'Prioritize oldest pending approval',
                'Assign dedicated review time in next work session',
              ]
            : ['Review queue is clear — no bottleneck detected'],
        whatChanged:
          approvals.length > 2
            ? 'Review backlog is building — immediate attention needed'
            : 'Review load within acceptable range',
        confidence: 0.85,
      }),
    );

    results.push(
      this.buildForecast('approval_lag_risk', {
        currentScore: Math.min(1, approvals.length * 0.2 + overdueDeadlines.length * 0.15),
        drivers: [
          `${approvals.length} pending approval(s)`,
          `${overdueDeadlines.length} overdue deadline(s)`,
          'Approval lag increases with each pending item',
        ],
        ...
        recommendedActions:
          approvals.length > 0
            ? [
                'Review all pending approvals — each unresolved approval adds estimated 1-3 days lag',
              ]
            : ['No approval lag detected'],
        whatChanged:
          approvals.length > 1
            ? 'Multiple pending approvals are compounding lag risk'
            : 'Approval lag is low',
        confidence: 0.8,
      }),
    );
    ...
    results.push(
      this.buildForecast('recovery_lien_drag_risk', {
        currentScore: Math.min(1, activeLiens.length * 0.25),
        drivers: [
          `${activeLiens.length} active lien(s)`,
          activeLiens.length > 2
            ? 'Multiple active liens — significant recovery timeline risk'
            : 'Manageable lien exposure',
        ],
        ...
        recommendedActions:
          activeLiens.length > 0
            ? [
                'Initiate lien negotiation conversations immediately',
                'Assess Medicare/Medicaid conditional payment amounts',
                'Consider requesting lien waiver where applicable',
              ]
            : ['No active lien drag detected'],
        whatChanged:
          activeLiens.length > 0
            ? `${activeLiens.length} active lien(s) creating settlement drag`
            : 'No active lien drag',
        confidence: 0.78,
      }),
    );
    ...
    results.push(
      this.buildForecast('quiet_risk_deterioration', {
        ...
        recommendedActions:
          quietRiskScore > 0.5
            ? [
                'Schedule immediate matter review',
                'Identify and address root cause of activity drought',
              ]
            : ['Matter is within normal activity range'],
        whatChanged: quietRisk[0]
          ? 'Quiet risk signals detected'
          : 'No quiet risk baseline computed',
        confidence: 0.75,
      }),
    );
  ...
  private buildForecast(
    ...
    return {
      ...
      priorScore: null,
      trend:
        data.currentScore > 0.65 ? 'declining' : data.currentScore < 0.35 ? 'improving' : 'stable',
      confidence: data.confidence,
      topDrivers: data.drivers,
      sourceClassesUsed: [
        'internal_firm',
        ...(data.worldlineDrivers.length > 0 ? ['worldline'] : []),
      ],
      whatChanged: data.whatChanged,
      internalDrivers: data.internalDrivers,
      worldlineDrivers: data.worldlineDrivers,
      recommendedActions: data.recommendedActions,
      requiresReview: data.currentScore >= 0.7,
      modelVersion: 'pilot_one_v1',
    };
  }

  async saveForecastDiffs(
    orgId: number,
    matterId: number,
    ...
    for (const f of forecasts) {
      const prior = await db
        ...
        .where(
          and(
            eq(pcForecastDiffsTable.orgId, orgId),
            eq(pcForecastDiffsTable.matterId, matterId),
            eq(pcForecastDiffsTable.forecastType, f.forecastType),
          ),
        )
      ...
      const trend: 'improving' | 'declining' | 'stable' | 'volatile' =
        priorScore === null
          ? 'stable'
          : f.currentScore < priorScore - 0.05
            ? 'improving'
            : f.currentScore > priorScore + 0.05
              ? 'declining'
              : 'stable';
    ...
    logger.info(
      { orgId, matterId, forecastCount: forecasts.length },
      'Pilot One forecast diffs saved',
    );
  }
  ...
  async getForecastDiffView(orgId: number, matterId: number) {
    const diffs = await db
      ...
      .from(pcForecastDiffsTable)
      .where(
        and(eq(pcForecastDiffsTable.orgId, orgId), eq(pcForecastDiffsTable.matterId, matterId)),
      )
      .orderBy(desc(pcForecastDiffsTable.createdAt))
      .limit(20);
    ...
    const pilotOneTypes: PilotOneForecastType[] = [
      'insurer_response_latency',
      'offer_movement_likelihood',
      'settlement_friction',
      'review_bottleneck',
    ...
    return {
      ...
      summary: {
        highRisk: Object.values(latestByType).filter((d) => d.currentScore >= 0.7).length,
