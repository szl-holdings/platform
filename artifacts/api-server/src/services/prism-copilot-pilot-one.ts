...
export class CopilotPilotOneService {
  ...
  private async whyHarderThanItLooks(
    orgId: number,
    matterId: number,
    title: string,
    ref: string,
    ...
    const highRiskForecasts = forecasts.forecasts.filter(
      (f) => f.data && f.data.currentScore >= 0.65,
    );
...
${
  pressure?.drivers
    .slice(0, 3)
    .map((d) => `- **${d.driverName}**: ${d.explanation}`)
...
${
  friction?.drivers
    .slice(0, 3)
    .map((d) => `- **${d.driverName}** (${d.blockerCategory}): ${d.explanation}`)
...
*Source: Insurer Pressure Engine, Settlement Friction Engine, Forecast System | Confidence: 0.78 | Ref: ${ref}*`;
    ...
    return {
      cardId: 'why_harder_than_looks',
      title,
      response,
      confidence: 0.78,
      drivers: pressure?.drivers.map((d) => d.driverName) ?? [],
      sources: ['insurer_pressure_engine', 'settlement_friction_engine', 'forecast_system'],
      requiresApproval: false,
      proofChainRef: ref,
    };
  }

  private async whatBlockingSettlement(
    orgId: number,
    matterId: number,
    title: string,
    ref: string,
    ...
    if (!friction) {
      return {
        cardId: 'what_blocking_settlement',
        title,
        response: `## What Is Blocking Settlement?\n\nNo friction analysis has been run for this matter. Run the friction computation to identify settlement blockers.\n\n*Source: Settlement Friction Engine | Ref: ${ref}*`,
        confidence: 0.5,
        drivers: [],
        sources: ['settlement_friction_engine'],
        requiresApproval: false,
        proofChainRef: ref,
      };
    }
    ...
    for (const d of drivers) {
      if (!grouped[d.blockerCategory]) grouped[d.blockerCategory] = [];
      grouped[d.blockerCategory].push(d);
    }
...
${drivers
  ...
  .map(
    (d, i) =>
      `${i + 1}. **${d.driverName}** (${categoryLabels[d.blockerCategory] ?? d.blockerCategory})\n   ${d.explanation}\n   *Estimated drag: ${d.dragEstimateDays ?? 'unknown'} days*`,
  )
  .join('\n\n')}
...
${Object.entries(grouped)
  .map(
    ([cat, items]) =>
      `**${categoryLabels[cat] ?? cat}**: ${items.map((d) => d.driverName).join(', ')}`,
  )
  .join('\n')}
...
*Source: Settlement Friction Engine | Confidence: ${snapshot.confidence} | Ref: ${ref}*`;
    ...
    return {
      ...
      title,
      response,
      confidence: snapshot.confidence ?? 0.75,
  ...
  private async explainPressureScore(
    orgId: number,
    matterId: number,
    title: string,
    ref: string,
    ...
    if (!pressure) {
      return {
        cardId: 'explain_pressure_score',
        title,
        response: `## Insurer Pressure Score\n\nNo pressure analysis has been computed for this matter. Run the pressure engine to generate a score.\n\n*Source: Insurer Pressure Engine | Ref: ${ref}*`,
        confidence: 0.5,
        drivers: [],
        sources: ['insurer_pressure_engine'],
        requiresApproval: false,
        proofChainRef: ref,
      };
    }
    ...
    const directionLabel =
      { rising: 'Rising', falling: 'Falling', stable: 'Stable', new: 'New' }[snapshot.direction] ??
      snapshot.direction;
...
${drivers
  .slice(0, 4)
  .map(
    (d) => `- **${d.driverName}** (weight: ${Math.round(d.weight * 100)}%)\n  *${d.explanation}*`,
  )
  .join('\n\n')}
  ...
  private async whatChangedSinceMonday(
    ...
    const [recentComms, matter] = await Promise.all([
      db
        ...
        .where(
          and(
            eq(pcCommunicationsTable.matterId, matterId),
            gte(pcCommunicationsTable.sentAt, monday),
          ),
        )
...
${
  inbound.length > 0
    ? `**Recent Inbound Activity**:\n${inbound
        ...
        .map(
          (c) =>
            `- From ${c.fromParty ?? 'unknown'} via ${c.channel}: ${c.summary ?? 'No summary'}`,
        )
        .join('\n')}`
...
*Source: Communication Log, Matter Timeline | Confidence: 0.82 | Ref: ${ref}*`;
    ...
    return {
      cardId: 'what_changed_since_monday',
      title,
      response,
      confidence: 0.82,
      drivers: [],
      sources: ['communication_log', 'matter_timeline'],
      requiresApproval: false,
      proofChainRef: ref,
    };
  }

  private async smallestActionReadiness(
    orgId: number,
    matterId: number,
    title: string,
    ref: string,
    ...
    const smallestAction =
      friction?.snapshot.smallestAction ??
      topRec?.explanation ??
      'Run friction computation to identify smallest action';
...
${
  recommendations
    .slice(0, 3)
    .map((r, i) => `${i + 1}. ${r.title} (~${r.estimatedMinutes ?? 30} min)`)
...
*Source: Settlement Friction Engine, Movement Recommendations | Confidence: 0.76 | Ref: ${ref}*`;
    ...
    return {
      cardId: 'smallest_action_readiness',
      title,
      response,
      confidence: 0.76,
      drivers: recommendations.map((r) => r.title),
      sources: ['settlement_friction_engine', 'movement_recommendations'],
      requiresApproval: false,
      proofChainRef: ref,
    };
  }

  private async draftPartnerBriefing(
    orgId: number,
    matterId: number,
    title: string,
    ref: string,
...
${
  friction?.drivers
    .slice(0, 2)
    .map((d) => `- ${d.driverName} (${d.blockerCategory})`)
    .join('\n') ?? ''
}
...
*Source: Insurer Pressure Engine, Settlement Friction Engine | Ref: ${ref}*`;
    ...
    return {
      cardId: 'draft_partner_briefing',
      title,
      response,
      confidence: 0.8,
      drivers: [],
      sources: ['insurer_pressure_engine', 'settlement_friction_engine'],
      requiresApproval: true,
      proofChainRef: ref,
    };
  }

  private async draftEscalationNote(
    orgId: number,
    matterId: number,
    title: string,
    ref: string,
...
*Source: Insurer Pressure Engine, Communication Log | Ref: ${ref}*`;
    ...
    return {
      cardId: 'draft_escalation_note',
      title,
      response,
      confidence: 0.78,
      drivers: [],
      sources: ['insurer_pressure_engine', 'communication_log'],
      requiresApproval: true,
      proofChainRef: ref,
    };
  }

  private async settlementFrictionMemo(
    orgId: number,
    matterId: number,
    title: string,
    ref: string,
...
${
  friction
    ? Object.entries({
        ...
        'Carrier / Insurer': friction.drivers.filter(
          (d) => d.blockerCategory === 'carrier_insurer',
        ),
        'Internal Process': friction.drivers.filter(
          (d) => d.blockerCategory === 'internal_process',
        ),
        'Medical & Damages': friction.drivers.filter(
          (d) => d.blockerCategory === 'medical_damages',
        ),
      })
...
*Source: Settlement Friction Engine | Confidence: ${friction?.snapshot.confidence ?? 'N/A'} | Ref: ${ref}*`;
    ...
    return {
      cardId: 'settlement_friction_memo',
      title,
      response,
      confidence: friction?.snapshot.confidence ?? 0.7,
      drivers: friction?.drivers.map((d) => d.driverName) ?? [],
      sources: ['settlement_friction_engine'],
      requiresApproval: true,
      proofChainRef: ref,
    };
  ...
  private async matterMovementSummary(
    orgId: number,
    matterId: number,
    title: string,
    ref: string,
...
*Source: Forecast System, Pressure Engine, Friction Engine | Ref: ${ref}*`;
    ...
    return {
      cardId: 'matter_movement_summary',
      title,
      response,
      confidence: 0.77,
      drivers: [],
      sources: ['forecast_system', 'insurer_pressure_engine', 'settlement_friction_engine'],
      requiresApproval: false,
      proofChainRef: ref,
    };
  }

  private async carrierWatchSummary(
    orgId: number,
    matterId: number,
    title: string,
    ref: string,
...
${
  pressure?.drivers
    .filter((d) =>
      ['silence_window', 'partial_response', 'denial_frequency', 'adjuster_handoff'].includes(
        d.driverCategory,
      ),
    )
    .slice(0, 3)
...
*Source: Insurer Pressure Engine, Silence Window Tracker | Ref: ${ref}*`;
    ...
    return {
      cardId: 'carrier_watch_summary',
      title,
      response,
      confidence: 0.8,
      drivers: [],
      sources: ['insurer_pressure_engine', 'silence_window_tracker'],
      requiresApproval: false,
      proofChainRef: ref,
    };
  }

  private async pressureTrendNarrative(
    orgId: number,
    matterId: number,
    title: string,
    ref: string,
...
${
  pressure?.drivers
    ...
    .map(
      (d) =>
        `**${d.driverName}** (${Math.round(d.weight * 100)}% weight, ${d.impact} impact):\n${d.explanation}`,
    )
...
*Source: Insurer Pressure Engine | Confidence: ${pressure?.snapshot.confidence ?? 'N/A'} | Ref: ${ref}*`;
    ...
    return {
      cardId: 'pressure_trend_narrative',
      title,
      response,
      confidence: pressure?.snapshot.confidence ?? 0.75,
      drivers: pressure?.drivers.map((d) => d.driverName) ?? [],
      sources: ['insurer_pressure_engine'],
      requiresApproval: false,
      proofChainRef: ref,
  ...
  private async movementBoardItem(
    orgId: number,
    matterId: number,
    title: string,
    ref: string,
    ...
    const trendingToward =
      movementScore > 0.5
        ...
        : (friction?.snapshot.overallScore ?? 0 < 0.4)
          ? 'demand readiness'
          : 'stalled';
...
${
  friction?.drivers
    .slice(0, 3)
    .map((d) => `- ${d.driverName} must be addressed (${d.blockerCategory})`)
...
*Source: Movement Recommendations, Settlement Friction Engine, Forecast System | Ref: ${ref}*`;
    ...
    return {
      cardId: 'movement_board_item',
      title,
      response,
      confidence: 0.74,
      drivers: [],
      sources: ['movement_recommendations', 'settlement_friction_engine', 'forecast_system'],
      requiresApproval: false,
      proofChainRef: ref,
    };
  }
  ...
  getAvailableCards(): Array<{
    ...
    return [
      {
        id: 'why_harder_than_looks',
        title: CARD_TITLES.why_harder_than_looks,
        mode: 'strategy',
        requiresApproval: false,
      },
      {
        id: 'what_blocking_settlement',
        title: CARD_TITLES.what_blocking_settlement,
        mode: 'strategy',
        requiresApproval: false,
      },
      {
        id: 'explain_pressure_score',
        title: CARD_TITLES.explain_pressure_score,
        mode: 'matter',
        requiresApproval: false,
      },
      {
        id: 'what_changed_since_monday',
        title: CARD_TITLES.what_changed_since_monday,
        mode: 'matter',
        requiresApproval: false,
      },
      {
        id: 'smallest_action_readiness',
        title: CARD_TITLES.smallest_action_readiness,
        mode: 'strategy',
        requiresApproval: false,
      },
      {
        id: 'draft_partner_briefing',
        title: CARD_TITLES.draft_partner_briefing,
        mode: 'strategy',
        requiresApproval: true,
      },
      {
        id: 'draft_escalation_note',
        title: CARD_TITLES.draft_escalation_note,
        mode: 'communications',
        requiresApproval: true,
      },
      {
        id: 'settlement_friction_memo',
        title: CARD_TITLES.settlement_friction_memo,
        mode: 'strategy',
        requiresApproval: true,
      },
      {
        id: 'matter_movement_summary',
        title: CARD_TITLES.matter_movement_summary,
        mode: 'matter',
        requiresApproval: false,
      },
      {
        id: 'carrier_watch_summary',
        title: CARD_TITLES.carrier_watch_summary,
        mode: 'communications',
        requiresApproval: false,
      },
      {
        id: 'pressure_trend_narrative',
        title: CARD_TITLES.pressure_trend_narrative,
        mode: 'matter',
        requiresApproval: false,
      },
      {
        id: 'movement_board_item',
        title: CARD_TITLES.movement_board_item,
        mode: 'strategy',
        requiresApproval: false,
      },
    ];
