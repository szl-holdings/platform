...
import { pcCostTrackingTable, pcHfEndpointsTable } from '@szl-holdings/db/schema';
  ...
  async execute(req: HfTaskRequest): Promise<HfTaskResult> {
    const endpoint = req.options?.endpointId
      ? (
          await db
            .select()
            .from(pcHfEndpointsTable)
            .where(
              and(
                eq(pcHfEndpointsTable.id, req.options.endpointId),
                eq(pcHfEndpointsTable.orgId, req.orgId),
              ),
            )
        )[0]
      : (
          await db
            ...
            .where(
              and(
                eq(pcHfEndpointsTable.orgId, req.orgId),
                eq(pcHfEndpointsTable.task, req.task as any),
                eq(pcHfEndpointsTable.status, 'healthy'),
              ),
            )
            .limit(1)
        )[0];
    ...
    if (endpoint.circuitState === 'open') {
      const elapsed = endpoint.circuitOpenedAt
        ? Date.now() - new Date(endpoint.circuitOpenedAt).getTime()
        : Infinity;
    ...
    try {
      const output = await this.callEndpoint(
        endpoint,
        req.input,
        req.options?.timeout ?? endpoint.timeoutMs ?? 15000,
      );
      const latencyMs = Date.now() - start;

      await db
        .update(pcHfEndpointsTable)
        .set({
          totalCalls: (endpoint.totalCalls ?? 0) + 1,
          avgLatencyMs:
            ((endpoint.avgLatencyMs ?? 0) * (endpoint.totalCalls ?? 0) + latencyMs) /
            ((endpoint.totalCalls ?? 0) + 1),
      ...
      if (endpoint.costPerCall) {
        await this.trackCost(
          req.orgId,
          'hf_endpoint',
          parseFloat(endpoint.costPerCall),
          endpoint.name,
        );
      }
      ...
      return {
        ...
        output,
        latencyMs,
        modelId: endpoint.modelId ?? endpoint.name,
      };
    } catch (error: any) {
