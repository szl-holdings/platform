...
export interface IRmmProvider {
  ...
  runScript(deviceId: string, script: string, scriptType?: "powershell" | "bash"): Promise<RemoteActionResult>;
...
export class NinjaOneProvider implements IRmmProvider {
  ...
  async getDevices(): Promise<NormalizedDevice[]> {
    try {
      const devices = await this.fetch<Array<{
        id: number; systemName: string; ipAddresses: string[]; os?: { name: string; platform: string };
        nodeClass: string; online: boolean; lastContact?: string;
        approval?: string; agentVersion?: string;
  ...
  async getDeviceById(id: string): Promise<NormalizedDevice | null> {
    try {
      const d = await this.fetch<{
        id: number; systemName: string; ipAddresses: string[]; os?: { name: string };
        online: boolean; lastContact?: string; agentVersion?: string;
        cpuUsage?: number; memoryUsage?: number; diskUsage?: number; nodeClass: string;
      }>(`/device/${id}`);
      return {
        providerDeviceId: String(d.id),
  ...
  async restartService(deviceId: string, serviceName: string): Promise<RemoteActionResult> {
    try {
      const result = await this.fetch<{ jobId: string }>(`/device/${deviceId}/windows-service/${encodeURIComponent(serviceName)}/restart`, { method: "POST" });
  ...
  async rebootDevice(deviceId: string, forced = false): Promise<RemoteActionResult> {
    try {
      await this.fetch(`/device/${deviceId}/reboot/${forced ? "forced" : "normal"}`, { method: "POST" });
      return { success: true };
    } catch (err) {
      return { success: false, errorMessage: String(err) };
    }
  ...
  async runScript(deviceId: string, script: string, scriptType: "powershell" | "bash" = "powershell"): Promise<RemoteActionResult> {
    try {
...
export class ConnectWiseAutomateProvider implements IRmmProvider {
  ...
  private async fetch<T>(path: string, options: RequestInit = {}): Promise<T> {
    const auth = Buffer.from(`${this.config.username ?? ""}:${this.config.password ?? this.config.apiKey ?? ""}`).toString("base64");
  ...
  async getDevices(): Promise<NormalizedDevice[]> {
    try {
      const devices = await this.fetch<Array<{
        Id: number; ComputerName: string; LastIPAddress: string;
        OperatingSystemName: string; ComputerClass: string;
        Status: number; LastContact: string;
        Memory?: number; CPUFrequency?: number;
  ...
  async getDeviceById(id: string): Promise<NormalizedDevice | null> {
    try {
      const d = await this.fetch<{ Id: number; ComputerName: string; LastIPAddress: string; OperatingSystemName: string; ComputerClass: string; Status: number; LastContact: string }>(`/computers/${id}`);
      return {
        providerDeviceId: String(d.Id),
        hostname: d.ComputerName,
        clientName: "",
  ...
  async restartService(deviceId: string, serviceName: string): Promise<RemoteActionResult> {
    try {
      await this.fetch(`/computers/${deviceId}/services/${encodeURIComponent(serviceName)}/restart`, { method: "POST" });
      return { success: true };
    } catch (err) {
      return { success: false, errorMessage: String(err) };
    }
  ...
  async runScript(deviceId: string, script: string, _scriptType?: string): Promise<RemoteActionResult> {
    try {
...
export class ConnectWiseManageProvider implements IRmmProvider {
  ...
  async restartService(_deviceId: string, _serviceName: string): Promise<RemoteActionResult> {
    return { success: false, errorMessage: "ConnectWise Manage is a PSA — use RMM for remote actions" };
  }
  ...
  async runScript(_deviceId: string, _script: string, _scriptType?: string): Promise<RemoteActionResult> {
    return { success: false, errorMessage: "ConnectWise Manage is a PSA — use RMM for remote actions" };
  }
  ...
  async createTicket(data: Partial<NormalizedTicket>): Promise<NormalizedTicket | null> {
    try {
      const ticket = await this.fetch<{ id: number; summary: string; status: { name: string }; priority: { name: string }; dateEntered: string; _info: { lastUpdated: string } }>("/service/tickets", {
        method: "POST",
        body: JSON.stringify({
...
export class HaloPsaProvider implements IRmmProvider {
  ...
  async createTicket(data: Partial<NormalizedTicket>): Promise<NormalizedTicket | null> {
    try {
      const ticket = await this.fetch<{ id: number; summary: string; dateoccurred: string; lastmodified: string }>("/tickets", {
        method: "POST",
        body: JSON.stringify({
          summary: data.subject,
...
export class DattoRmmProvider implements IRmmProvider {
  ...
  constructor(config: RmmProviderConfig) { this.config = config; }
  ...
  private async getToken(): Promise<string> {
    ...
    const creds = Buffer.from(`${this.config.apiKey ?? ""}:${this.config.clientSecret ?? ""}`).toString("base64");
  ...
  async getDevices(): Promise<NormalizedDevice[]> {
    try {
      const data = await this.fetch<{ devices?: Array<Record<string, unknown>> }>("/account/devices?max=200");
  ...
  async runScript(deviceId: string, script: string, _scriptType?: "powershell" | "bash"): Promise<RemoteActionResult> {
    try {
  ...
  async createTicket(): Promise<NormalizedTicket | null> { return null; }
  async closeTicket(): Promise<boolean> { return false; }
  ...
  private inferType(deviceType: string): NormalizedDevice["type"] {
    ...
    if (lower.includes("network") || lower.includes("switch") || lower.includes("router")) return "network";
...
export class AutotaskPsaProvider implements IRmmProvider {
  ...
  async getDevices(): Promise<NormalizedDevice[]> {
    try {
      const data = await this.fetch<{ items?: Array<Record<string, unknown>> }>("/V1.0/ConfigurationItems/query", {
        method: "POST",
        body: JSON.stringify({ Filter: [{ field: "isActive", op: "eq", value: true }], MaxRecords: 200 }),
      });
  ...
  async getDeviceById(id: string): Promise<NormalizedDevice | null> {
    try {
      const d = await this.fetch<{ item?: Record<string, unknown> }>(`/V1.0/ConfigurationItems/${id}`);
      const item = d.item ?? {};
      return {
        ...
        cpu: 0, memory: 0, disk: 0,
        patchesPending: 0, lastSeen: new Date(), alerts: 0, threats: 0,
      };
    } catch { return null; }
  }

  async restartService(): Promise<RemoteActionResult> {
    return { success: false, errorMessage: "Autotask PSA does not support remote service management directly. Use the integrated RMM (Datto) for remote actions." };
  }

  async rebootDevice(): Promise<RemoteActionResult> {
    return { success: false, errorMessage: "Autotask PSA does not support remote device management directly. Use the integrated RMM (Datto) for remote actions." };
  }

  async runScript(): Promise<RemoteActionResult> {
    return { success: false, errorMessage: "Autotask PSA does not support remote script execution. Use the integrated RMM (Datto) for remote actions." };
  }

  async killProcess(): Promise<RemoteActionResult> {
    return { success: false, errorMessage: "Autotask PSA does not support remote process management. Use the integrated RMM (Datto) for remote actions." };
  }
  ...
  async testConnection(): Promise<{ ok: boolean; latencyMs: number; error?: string }> {
    ...
    try {
      await this.fetch("/V1.0/Companies/query", {
        method: "POST",
        body: JSON.stringify({ Filter: [{ field: "isActive", op: "eq", value: true }], MaxRecords: 1 }),
      });
      return { ok: true, latencyMs: Date.now() - start };
    } catch (err) {
      return { ok: false, latencyMs: Date.now() - start, error: String(err) };
...
export function createRmmProvider(config: RmmProviderConfig): IRmmProvider | null {
  switch (config.provider) {
    case "ninjaone": return new NinjaOneProvider(config);
    case "connectwise_automate": return new ConnectWiseAutomateProvider(config);
    case "connectwise_manage": return new ConnectWiseManageProvider(config);
    case "halopsa": return new HaloPsaProvider(config);
    case "datto_rmm": return new DattoRmmProvider(config);
    case "autotask_psa": return new AutotaskPsaProvider(config);
    default:
...
export function setCachedProvider(connectorId: number, config: RmmProviderConfig): IRmmProvider | null {
