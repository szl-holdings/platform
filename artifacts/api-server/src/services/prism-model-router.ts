...
import { pcModelLanesTable, pcModelRequestsTable, pcCostTrackingTable } from "@szl-holdings/db/schema";
import { eq, and, desc } from "drizzle-orm";
import { logger } from "../lib/logger";

export type ModelLane = "embedding" | "retrieval" | "classification" | "extraction" | "reasoning" | "forecast" | "policy_guardrail";
...
class ModelRouter {
  private circuitStates: Map<number, { state: "closed" | "open" | "half_open"; failures: number; openedAt?: Date }> = new Map();
  ...
  async route(req: RouteRequest): Promise<RouteResult> {
    ...
    try {
      ...
      const output = await this.executeLane(req.lane, req.taskType, req.input, provider, model, lane?.endpoint ?? undefined);
      const latencyMs = Date.now() - start;
      const cost = lane?.costPerRequest ? parseFloat(lane.costPerRequest) : 0;

      await this.logRequest(req.orgId, lane?.id ?? null, req.lane, provider, model, req.taskType, req.matterId, latencyMs, cost, "success");
      ...
      await this.logRequest(req.orgId, lane?.id ?? null, req.lane, provider, model, req.taskType, req.matterId, latencyMs, 0, "failure", error.message);
  ...
  private async selectLane(orgId: number, lane: ModelLane) {
    const lanes = await db.select().from(pcModelLanesTable)
      .where(and(eq(pcModelLanesTable.orgId, orgId), eq(pcModelLanesTable.lane, lane), eq(pcModelLanesTable.status, "active")))
      .orderBy(pcModelLanesTable.priority)
      .limit(1);
    return lanes[0] ?? null;
  }

  private async executeLane(lane: ModelLane, taskType: string, input: any, provider: string, model: string, endpoint?: string): Promise<any> {
    switch (lane) {
      case "embedding":
  ...
  private async executeClassification(input: any, provider: string, model: string, endpoint?: string) {
    const text = typeof input === "string" ? input : input.text;
    const labels = input.labels ?? ["privilege", "work_product", "standard", "spam"];
    return {
      label: labels[0],
      confidence: 0.85,
      scores: labels.reduce((acc: any, l: string, i: number) => ({ ...acc, [l]: Math.max(0.1, 0.85 - i * 0.2) }), {}),
      model,
      provider,
    };
  }
  ...
  private async tryFallback(req: RouteRequest, excludeLaneId?: number | null): Promise<RouteResult | null> {
    const lanes = await db.select().from(pcModelLanesTable)
    ...
    for (const lane of lanes) {
      ...
      try {
        const start = Date.now();
        const output = await this.executeLane(req.lane, req.taskType, req.input, lane.provider, lane.modelName, lane.endpoint ?? undefined);
        const latencyMs = Date.now() - start;
        logger.info({ lane: req.lane, fallbackProvider: lane.provider }, "Model mesh failover succeeded");
        return { laneId: lane.id, provider: lane.provider, model: lane.modelName, output, latencyMs };
      } catch {
        continue;
      }
    }
  ...
  private async logRequest(orgId: number, laneId: number | null, lane: string, provider: string, model: string, taskType: string, matterId: number | undefined, latencyMs: number, cost: number, status: string, error?: string) {
    try {
      await db.insert(pcModelRequestsTable).values({
        orgId, laneId, lane, provider, model, taskType, matterId,
        latencyMs, cost: cost.toString(), status: status as any,
  ...
  async getLaneHealth(orgId: number) {
    const lanes = await db.select().from(pcModelLanesTable).where(eq(pcModelLanesTable.orgId, orgId));
    return lanes.map(l => ({
      id: l.id,
      lane: l.lane,
      provider: l.provider,
  ...
  async getRequestStats(orgId: number, hours = 24) {
    ...
    const requests = await db.select().from(pcModelRequestsTable)
      .where(and(eq(pcModelRequestsTable.orgId, orgId)))
      .orderBy(desc(pcModelRequestsTable.createdAt))
      .limit(1000);

    const stats: Record<string, { total: number; success: number; failed: number; avgLatency: number; totalCost: number }> = {};
    for (const r of requests) {
      if (!stats[r.lane]) stats[r.lane] = { total: 0, success: 0, failed: 0, avgLatency: 0, totalCost: 0 };
