...
export type PrivilegeType = "attorney_client" | "work_product" | "joint_defense" | "common_interest" | "none";
...
async function getClawbackStatusForFlag(flagId: number): Promise<"none" | "requested" | "completed"> {
  try {
    const [latest] = await db
...
async function toLogEntry(flag: typeof pcPrivilegeFlagsTable.$inferSelect): Promise<PrivilegeLogEntry> {
...
class PrivilegeEngine {
  async tagEntity(opts: {
    ...
    const [inserted] = await db
      ...
      .values({
        ...
        flagType: opts.flagType as "attorney_client" | "work_product" | "joint_defense" | "common_interest",
    ...
    await db.insert(pcReviewAuditEventsTable).values({
      ...
      toState: "unreviewed",
      details: { flagId: inserted.id, flagType: opts.flagType, entityType: opts.entityType, entityId: opts.entityId },
  ...
  async resolveReview(
    ...
    if (row.matterOrgId !== orgId) {
      throw Object.assign(new Error("Access denied — privilege flag belongs to a different organization"), { statusCode: 403 });
    }
  ...
  async generatePrivilegeLogForProduction(matterId: number, orgId: number): Promise<{
    matterId: number;
    ...
    const flags = await db
      ...
      .where(
        and(
          ...
          inArray(pcPrivilegeFlagsTable.entityId, entityIds),
        ),
      );
  ...
  async classifyContent(
    ...
    const isAttorneyInvolved = authorRole.includes("attorney") || authorRole.includes("counsel") || authorRole.includes("lawyer")
      || recipientRoles.some(r => r.includes("attorney") || r.includes("counsel") || r.includes("lawyer"));

    const signals: string[] = [];
    let type: PrivilegeType = "none";
    let confidence = 0.3;

    if (lower.includes("privileged") || lower.includes("attorney-client") || lower.includes("work product")) {
      signals.push("Explicit privilege marking detected");
      type = "attorney_client"; confidence = 0.92;
    } else if (isAttorneyInvolved && (lower.includes("legal advice") || lower.includes("confidential") || lower.includes("strategy"))) {
      signals.push("Attorney involvement with legal advice content");
      type = "attorney_client"; confidence = 0.85;
    } else if (lower.includes("trial") || lower.includes("litigation") || lower.includes("prepared in anticipation")) {
      signals.push("Work product indicators present");
      type = "work_product"; confidence = 0.78;
    } else if (lower.includes("joint defense") || lower.includes("common interest")) {
      signals.push("Joint defense indicators detected");
      type = "joint_defense"; confidence = 0.82;
    } else if (isAttorneyInvolved) {
      signals.push("Attorney in communication chain");
      type = "attorney_client"; confidence = 0.55;
    }
