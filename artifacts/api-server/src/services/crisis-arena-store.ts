...
import { runScenarioSimulation, type ScenarioDefinition } from '@szl-holdings/monte-carlo';
import {
    ...
    inputs: [
      {
        ...
        distribution: { type: 'log_normal', mean: revenueM, stdDev: revenueM * 0.3 },
        format: 'currency',
      },
      {
        id: 'rtoH',
        label: 'RTO Breach (hours)',
        distribution: { type: 'normal', mean: rtoH, stdDev: Math.max(0.1, rtoH * 0.2) },
        format: 'number',
      },
      {
        id: 'rpoH',
        label: 'RPO Breach (hours)',
        distribution: { type: 'normal', mean: rpoH, stdDev: Math.max(0.1, rpoH * 0.2) },
        format: 'number',
      },
      {
        id: 'regM',
...
function rowToProfile(
  row: typeof crisisArenaArchitectProfilesTable.$inferSelect,
): ArchitectProfile {
  return {
    id: row.id,
    handle: row.handle,
...
export async function updateSubmission(
  ...
  const values: Record<string, unknown> = { updatedAt: new Date() };
  if (fields.status !== undefined) values['status'] = fields.status;
  if (fields.reputationAwarded !== undefined)
    values['reputationAwarded'] = fields.reputationAwarded;
  if (fields.payoutAwarded !== undefined) values['payoutAwarded'] = fields.payoutAwarded;
  if (fields.triageJustification !== undefined)
    values['triageJustification'] = fields.triageJustification;
  if (fields.graduatedIncidentId !== undefined)
    values['graduatedIncidentId'] = fields.graduatedIncidentId;
  await db
...
export async function getArchitectProfileByHandle(
  handle: string,
...
export async function insertReputationEvent(evt: Omit<ReputationEvent, 'id'>): Promise<void> {
...
export async function updateArchitectStats(architectId: string): Promise<void> {
  ...
  for (const s of accepted) {
    const badge: ArchetypeBadge =
      s.archetype === 'black_swan'
        ? 'Black Swan'
        : s.archetype === 'cascade'
          ? 'Cascade'
          : s.archetype === 'insider'
            ? 'Insider'
            : s.archetype === 'regulatory'
              ? 'Regulator'
              : s.archetype === 'supply_chain'
                ? 'Cascade'
                : 'Black Swan';
    badgeSet.add(badge);
    if (!archetypeMap[s.archetype]) {
      archetypeMap[s.archetype] = {
        archetype: s.archetype as ThreatArchetype,
        badge,
        count: 0,
        totalScore: 0,
      };
...
export async function seedCrisisArenaData(): Promise<void> {
  ...
  const architects: ArchitectProfile[] = [
    ...
    {
      id: 'arch-002',
      handle: 'CascadeEngine',
      displayName: 'M. Alvarado',
      ...
      reputationScore: 3615,
      acceptedCount: 5,
      submissionCount: 7,
      totalImpactUsd: 31_500_000,
    ...
    {
      id: 'arch-005',
      handle: 'SupplyChain_Red',
      displayName: 'T. Nakamura',
  ...
  await insertEngagement({
    ...
    description:
      'Model the 72-hour cash, customer, and regulatory blast radius of a ransomware event on the billing cluster during quarter-close. Focus on RTO/RPO breach, revenue recognition delay, and regulatory notification timelines.',
    ...
    payoutPool: 25000,
    deadline: daysFromNow(14).toISOString(),
    status: 'accepting',
  ...
  await insertEngagement({
    ...
    description:
      'Simulate a supply chain compromise targeting OT/ICS firmware update channels. Model operational downtime, environmental liability, and insurance policy clause triggers.',
    scopedAssets: ['ot-segment', 'plc-controllers', 'scada-server'],
    scopedDomains: ['Sentra', 'Vessels'],
    archetypeFilter: ['supply_chain', 'cascade'],
    payoutPool: 18000,
    deadline: daysFromNow(21).toISOString(),
    status: 'accepting',
  ...
  await insertEngagement({
    ...
    description:
      'Identify scenarios that simultaneously trigger DORA ICT incident notification, GDPR breach disclosure, and NIS2 reporting obligations. Model compound regulatory penalty exposure.',
  ...
  await insertSubmission({
    ...
    narrative:
      'At 17:00 on a quarter-close Friday, a LockBit-adjacent payload encrypts the billing cluster. Revenue recognition for $14M in Q4 contracts is blocked. Finance cannot close books, triggering a material weakness disclosure risk. The 72-hour window overlaps with three customer SLA breach points and a SEC reporting deadline.',
    killChain: [
      {
        phase: 'Initial Access',
        technique: 'T1566.001 Spearphishing',
        description: 'CFO assistant email compromise via weaponized invoice PDF',
      },
      {
        phase: 'Privilege Escalation',
        technique: 'T1078 Valid Accounts',
        description: 'Lateral movement to billing admin via stolen SAML token',
      },
      {
        phase: 'Impact',
        technique: 'T1486 Data Encryption',
        description: 'Ransomware payload deployed to billing cluster and ERP backup shadow copies',
      },
    ],
    impactEstimate: {
      revenueAtRiskUsd: 14_000_000,
      rtoBreach: 4320,
      rpoBreach: 2880,
      regulatoryExposureUsd: 2_500_000,
      blastRadiusDomains: ['Sentra', 'Counsel', 'Terra'],
    },
    evidenceNotes:
      'Based on real Q4 close ransomware incidents at comparably sized SaaS firms. RTO breach assumes 72h TTR from forensics.',
    archetype: 'ransomware',
    status: 'accepted',
    businessImpactScore: 98,
    reputationAwarded: 147,
    payoutAwarded: 8000,
    triageJustification:
      'Highest-scoring submission. Scenario is immediately playable in Incident Commander. Business impact model validated against Monte Carlo run.',
  ...
  await insertSubmission({
    ...
    narrative:
      'Secondary cascade from the billing cluster ransomware into the ERP system causes accounts payable to freeze. Supplier contracts contain 48-hour payment clauses with penalty exposure.',
    killChain: [
      {
        phase: 'Initial Access',
        technique: 'T1486 Ransomware Spread',
        description: 'Ransomware propagates from billing cluster to ERP via shared network segment',
      },
      {
        phase: 'Impact',
        technique: 'T1657 Financial Theft',
        description: 'A/P system unavailability triggers 14 supplier penalty clauses',
      },
    ],
    impactEstimate: {
      revenueAtRiskUsd: 4_200_000,
      rtoBreach: 2880,
      rpoBreach: 1440,
      regulatoryExposureUsd: 800_000,
      blastRadiusDomains: ['Sentra', 'Counsel'],
    },
    evidenceNotes:
      'Supplier penalty exposure derived from standard NET-30 clause with 2% penalty at 48h.',
    archetype: 'cascade',
    status: 'pending',
  ...
  await insertSubmission({
    id: 'sub-003',
    engagementId: 'eng-001',
    architectId: 'arch-003',
    title: 'Regulatory Double-Bind: SOX + GDPR Simultaneous Trigger',
    narrative:
      'The billing cluster ransomware triggers simultaneous SOX material weakness and GDPR Article 33 notification obligations. The 72-hour GDPR clock conflicts with the SEC 4-day material disclosure window, creating a regulatory collision scenario.',
    killChain: [
      {
        phase: 'Initial Access',
        technique: 'T1486 Ransomware',
        description: 'Billing cluster encryption exposes PII in payment records',
      },
      {
        phase: 'Regulatory',
        technique: 'Dual Notification',
        description:
          'GDPR 72h and SEC 4-day clocks run simultaneously with conflicting disclosure obligations',
      },
    ],
    impactEstimate: {
      revenueAtRiskUsd: 2_100_000,
      rtoBreach: 1440,
      rpoBreach: 720,
      regulatoryExposureUsd: 4_800_000,
      blastRadiusDomains: ['Sentra', 'Counsel'],
    },
    evidenceNotes:
      'GDPR Article 83 penalties modeled at 2% of global annual turnover. SOX material weakness modeled at restatement cost.',
    archetype: 'regulatory',
    status: 'pending',
    businessImpactScore: 83,
    reputationAwarded: 0,
    payoutAwarded: 0,
  ...
  await insertSubmission({
    ...
    architectId: 'arch-005',
    title: 'Firmware Supply Chain Compromise — OT Segment Operational Halt',
    narrative:
      'A compromised firmware update from a trusted ICS vendor pushes a dormant backdoor to PLC controllers. Triggered during a maintenance window, it causes a 4-day operational halt. Insurance clause 8.3 requires verified backup cadence — which is already breached.',
    killChain: [
      {
        phase: 'Supply Chain',
        technique: 'T1195.002 Compromise Hardware Supply Chain',
        description: 'ICS vendor firmware update poisoned with persistent backdoor',
      },
      {
        phase: 'Execution',
        technique: 'T1059 Command and Scripting',
        description: 'Backdoor triggered during maintenance window, causing PLC controller halt',
      },
      {
        phase: 'Impact',
        technique: 'T1498 Operational Disruption',
        description:
          '4-day plant operational halt, triggering insurance and environmental liability clauses',
      },
    ],
    impactEstimate: {
      revenueAtRiskUsd: 8_400_000,
      rtoBreach: 5760,
      rpoBreach: 2880,
      regulatoryExposureUsd: 1_800_000,
      blastRadiusDomains: ['Sentra', 'Vessels'],
    },
    evidenceNotes:
      'Operational downtime cost modeled at $2.1M/day for manufacturing context. Insurance clause breach adds $1.8M exposure.',
    archetype: 'supply_chain',
    status: 'pending',
    businessImpactScore: 79,
    reputationAwarded: 0,
    payoutAwarded: 0,
    submittedAt: hoursAgo(18).toISOString(),
    updatedAt: hoursAgo(18).toISOString(),
  });
  ...
  await insertReputationEvent({
    ...
    submissionId: 'sub-001',
    delta: 147,
    reason: 'Accepted submission — BIS 98',
