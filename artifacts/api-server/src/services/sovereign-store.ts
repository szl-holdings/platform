 ...
 *
 * Provides the runtime data layer for A11oy's sovereign execution APIs.
 * All exported constants and helpers are consumed by `a11oy-sovereign-api.ts`.
 *
 * Architecture note: this module is the data/service boundary. When the
 * persistence task (scheduled after Phase 3) adds DB tables for models, evals,
 * connectors, twins, skills, replay reports, and board packets, only this file
 * needs to change — routes stay stable.
 */
...
export const minus = (minutes: number) => new Date(Date.now() - minutes * 60_000).toISOString();
export const minusH = (hours: number) => new Date(Date.now() - hours * 3_600_000).toISOString();
export const minusD = (days: number) => new Date(Date.now() - days * 86_400_000).toISOString();
...
export const SEED_EVALS = [
  ...
  makeEval({ id: 'eval-e005', targetId: 'wc-tg-ember-response', disposition: 'pass', tenantId: 'atlas', scores: { groundedness: 0.99, evidence_coverage: 0.98, action_safety: 1.0, hallucination_risk: 0.99, policy_compliance: 1.0, tool_risk: 0.99, stale_context: 0.95, verification_readiness: 0.98, counterfactual_strength: 0.91, causal_validity: 0.99, approval_alignment: 1.0, scope_adherence: 1.0, output_fidelity: 0.97, proof_completeness: 0.97 }, composite: 0.977, flags: [] }),
  ...
  makeEval({ id: 'eval-e007', targetId: 'wc-sanctions-alert', disposition: 'blocked', tenantId: 'meridian', scores: { groundedness: 0.19, evidence_coverage: 0.15, action_safety: 0.88, hallucination_risk: 0.31, policy_compliance: 0.24, tool_risk: 0.44, stale_context: 0.18, verification_readiness: 0.22, counterfactual_strength: 0.12, causal_validity: 0.28, approval_alignment: 0.19, scope_adherence: 0.35, output_fidelity: 0.24, proof_completeness: 0.12 }, composite: 0.274, flags: ['critical_policy_violation', 'insufficient_evidence', 'hallucination_detected'], gatingBlocked: true }),
  makeEval({ id: 'eval-e008', targetId: 'wc-revenue-friction', disposition: 'pass', tenantId: 'northstar', scores: { groundedness: 0.91, evidence_coverage: 0.86, action_safety: 0.97, hallucination_risk: 0.92, policy_compliance: 0.96, tool_risk: 0.89, stale_context: 0.82, verification_readiness: 0.88, counterfactual_strength: 0.79, causal_validity: 0.91, approval_alignment: 0.97, scope_adherence: 0.99, output_fidelity: 0.87, proof_completeness: 0.84 }, composite: 0.898, flags: [] }),
  makeEval({ id: 'eval-e009', targetId: 'wc-sow-aging', disposition: 'pass_with_warning', tenantId: 'hudson', scores: { groundedness: 0.76, evidence_coverage: 0.69, action_safety: 0.88, hallucination_risk: 0.81, policy_compliance: 0.82, tool_risk: 0.74, stale_context: 0.61, verification_readiness: 0.74, counterfactual_strength: 0.66, causal_validity: 0.79, approval_alignment: 0.88, scope_adherence: 0.92, output_fidelity: 0.77, proof_completeness: 0.68 }, composite: 0.762, flags: ['stale_context'] }),
  makeEval({ id: 'eval-e010', targetId: 'wc-voyage-risk', disposition: 'pass', tenantId: 'meridian', scores: { groundedness: 0.95, evidence_coverage: 0.91, action_safety: 0.99, hallucination_risk: 0.97, policy_compliance: 0.98, tool_risk: 0.94, stale_context: 0.88, verification_readiness: 0.93, counterfactual_strength: 0.86, causal_validity: 0.95, approval_alignment: 0.99, scope_adherence: 1.0, output_fidelity: 0.92, proof_completeness: 0.91 }, composite: 0.941, flags: [] }),
...
export const SKILLS_REGISTRY = [
  { id: 'skill-revenue-friction', name: 'Revenue Friction Detector', category: 'Revenue Intelligence', domain: 'Revenue', version: '2.1.0', status: 'LIVE', calls: 214, successRate: 0.97, avgLatencyMs: 380, description: 'Detects revenue friction in CRM pipeline — stall patterns, velocity decay, at-risk deals. Outputs: Action Brief with deal list, risk scores, and recommended executive touchpoints.', allowedTools: ['read_opportunity', 'read_account', 'create_task'], blockedTools: ['delete_opportunity', 'bulk_export', 'admin_api'], requiredPolicies: ['pol-revenue-001', 'pol-executive-approval'], evalRequired: true, sampleInput: { pipeline_snapshot_id: 'snap-q2-2026', threshold_days_stalled: 14 }, sampleOutput: { at_risk_deals: 3, total_arr_at_risk: '$2.1M', recommended_action: 'VP outreach — 3 deals', workcell_id: 'wc-q2-pipeline' } },
  { id: 'skill-sow-aging', name: 'SOW Aging Analyzer', category: 'Advisory Intelligence', domain: 'Advisory', version: '1.4.0', status: 'LIVE', calls: 88, successRate: 0.99, avgLatencyMs: 220, description: 'Scores SOW delivery aging risk — days to deadline, completion percentage, milestone drift. Outputs: risk score, recommended escalation action, and Proof Packet.', allowedTools: ['read_sow', 'read_proposal'], blockedTools: ['submit_proposal', 'approve_sow'], requiredPolicies: ['pol-advisory-001'], evalRequired: true, sampleInput: { sow_id: 'sow-meridian-sire', tenant_id: 'hudson' }, sampleOutput: { risk_score: 0.82, risk_label: 'HIGH', days_to_deadline: 8, completion_pct: 42, recommended_action: 'Escalate timeline risk' } },
  { id: 'skill-duplicate-scopestack', name: 'Duplicate ScopeStack Detector', category: 'Advisory Intelligence', domain: 'Advisory', version: '1.0.1', status: 'LIVE', calls: 41, successRate: 0.95, avgLatencyMs: 140, description: 'Identifies duplicate or near-duplicate SOW proposals in ScopeStack. Flags cost inflation and redundant scope. Outputs: duplicate cluster report.', allowedTools: ['read_sow', 'search_library'], blockedTools: ['delete_sow', 'admin_api'], requiredPolicies: ['pol-advisory-001'], evalRequired: false, sampleInput: { tenant_id: 'hudson', similarity_threshold: 0.85 }, sampleOutput: { duplicate_clusters: 2, total_duplicates: 5, estimated_savings: '$42,000' } },
  { id: 'skill-board-packet', name: 'Executive Board Packet Generator', category: 'Boardroom Intelligence', domain: 'Cross-domain', version: '3.0.0', status: 'LIVE', calls: 29, successRate: 0.93, avgLatencyMs: 2100, description: 'Synthesizes a board-quality executive briefing from live twin states, eval results, and proof packets. Produces a BoardPacket linked to a MirrorEvalResult and ProofPacket.', allowedTools: ['read_opportunity', 'read_matter', 'get_eta', 'read_incident'], blockedTools: ['*:write', '*:delete', 'admin_api'], requiredPolicies: ['pol-executive-approval', 'pol-board-001'], evalRequired: true, sampleInput: { tenant_id: 'northstar', period: 'Q2-2026', include_verticals: ['revenue', 'advisory'] }, sampleOutput: { packet_id: 'bp-northstar-q2-2026', disposition: 'pass', executive_summary: 'Q2 pipeline tracking $128M vs $142M target. 3 critical actions pending.' } },
  { id: 'skill-proof-reconstruction', name: 'Proof Reconstruction Engine', category: 'Governance', domain: 'A11oy Core', version: '1.2.0', status: 'LIVE', calls: 156, successRate: 1.0, avgLatencyMs: 890, description: 'Reconstructs a complete Proof-Carrying Execution chain from trace spans, eval results, and approval records. Produces a cryptographically-linked ProofPacket.', allowedTools: ['read_trace', 'read_eval', 'read_proof'], blockedTools: ['write_proof', 'delete_proof', 'admin_api'], requiredPolicies: ['pol-governance-001'], evalRequired: false, sampleInput: { workcell_id: 'wc-cascade-standby', include_telemetry: true }, sampleOutput: { proof_packet_id: 'pce-abc123', hash: 'sha256:a1b2c3d4', completeness: 0.97 } },
  { id: 'skill-legal-deadline', name: 'Legal Deadline Risk Scorer', category: 'Legal Intelligence', domain: 'Legal', version: '2.0.0', status: 'LIVE', calls: 88, successRate: 0.98, avgLatencyMs: 280, description: 'Scores breach risk for legal deadlines — outstanding docs, days remaining, historical completion rates, opposing counsel patterns.', allowedTools: ['read_matter', 'read_docket', 'search_documents'], blockedTools: ['create_matter', 'delete_document', 'admin_api'], requiredPolicies: ['pol-legal-001'], evalRequired: true, sampleInput: { matter_id: 'matter-talbot', tenant_id: 'prism' }, sampleOutput: { risk_score: 0.91, risk_label: 'HIGH', days_to_deadline: 2, outstanding_docs: 3, recommended_action: 'Escalate to lead counsel' } },
  { id: 'skill-voyage-risk', name: 'Voyage Risk Assessor', category: 'Maritime Intelligence', domain: 'Maritime', version: '1.8.0', status: 'LIVE', calls: 142, successRate: 0.96, avgLatencyMs: 420, description: 'Computes voyage risk from AIS data, weather models, port congestion, and charter party terms. Outputs: risk score, demurrage exposure, recommended action.', allowedTools: ['read_vessel_position', 'read_port_status', 'get_eta'], blockedTools: ['write_ais', 'admin_api'], requiredPolicies: ['pol-maritime-001'], evalRequired: true, sampleInput: { vessel_id: 'mv-cascade', voyage_id: 'voyage-houston-2026' }, sampleOutput: { risk_score: 0.74, demurrage_exposure: '$540,400', delay_hours: 38, recommended_action: 'Authorize 48h port standby' } },
  { id: 'skill-sanctions-watch', name: 'Sanctions Watch Monitor', category: 'Maritime Intelligence', domain: 'Maritime', version: '1.1.0', status: 'LIVE', calls: 312, successRate: 1.0, avgLatencyMs: 85, description: 'Screens vessels, counterparties, and cargo against OFAC/EU/UN sanctions lists. Outputs: match result, risk classification, recommended action.', allowedTools: ['read_vessel_position', 'read_account'], blockedTools: ['*:write', 'admin_api'], requiredPolicies: ['pol-compliance-001', 'pol-sanctions'], evalRequired: true, sampleInput: { vessel_id: 'mv-resolution', counterparty_id: 'cp-delta-shipping' }, sampleOutput: { sanctions_match: false, risk_classification: 'low', recommended_action: 'Clear for voyage' } },
  { id: 'skill-security-incident', name: 'Security Incident Classifier', category: 'Defense Intelligence', domain: 'Defense', version: '2.4.0', status: 'LIVE', calls: 289, successRate: 0.99, avgLatencyMs: 155, description: 'Classifies security incidents by threat tier, TTP cluster, and MITRE ATT&CK mapping. Outputs: threat classification, posture recommendation, escalation action.', allowedTools: ['read_alert', 'read_device', 'get_threat_report'], blockedTools: ['isolate_device', 'remediate', 'admin_api'], requiredPolicies: ['pol-security-007', 'pol-executive-approval'], evalRequired: true, sampleInput: { alert_id: 'alert-tg-ember-2026', tenant_id: 'atlas' }, sampleOutput: { threat_tier: 'ORANGE', ttps_matched: 14, posture_recommendation: 'Maintain containment', escalation: 'CISO notification required' } },
  { id: 'skill-vendor-sla', name: 'Vendor SLA Performance Monitor', category: 'Procurement Intelligence', domain: 'Cross-domain', version: '1.3.0', status: 'LIVE', calls: 56, successRate: 0.94, avgLatencyMs: 190, description: 'Monitors vendor SLA compliance across all contractual KPIs. Flags underperformance and recommends corrective action.', allowedTools: ['read_vendor', 'read_invoice'], blockedTools: ['approve_invoice', 'create_vendor', 'admin_api'], requiredPolicies: ['pol-procurement-001'], evalRequired: false, sampleInput: { vendor_id: 'vendor-meridian-shipyard', period: 'Q1-2026' }, sampleOutput: { sla_compliance: 0.81, failing_kpis: ['delivery_time', 'quality_score'], recommended_action: 'SLA performance review meeting' } },
  { id: 'skill-property-capex', name: 'Property Capex Variance Detector', category: 'Real Estate Intelligence', domain: 'Real Estate', version: '1.5.0', status: 'LIVE', calls: 34, successRate: 0.97, avgLatencyMs: 310, description: 'Detects capex budget variance and cap rate compression across the property portfolio. Outputs: variance report, covenant risk flag, recommended action.', allowedTools: ['read_vendor', 'get_property_status', 'read_invoice'], blockedTools: ['approve_invoice', 'admin_api'], requiredPolicies: ['pol-realestate-001'], evalRequired: true, sampleInput: { portfolio_id: 'plano-industrial', tenant_id: 'terranova' }, sampleOutput: { cap_rate_delta: '+18bps', covenant_risk: 'elevated', recommended_action: 'Initiate capex review' } },
  { id: 'skill-codebase-audit', name: 'Codebase Audit Scanner', category: 'Engineering Intelligence', domain: 'Engineering', version: '1.0.0', status: 'DEMO', calls: 12, successRate: 0.92, avgLatencyMs: 4200, description: 'Scans codebase for security vulnerabilities, outdated dependencies, and code quality issues. Outputs: audit report with severity classification.', allowedTools: ['read_issue', 'search_library'], blockedTools: ['admin_api', 'bulk_export'], requiredPolicies: ['pol-governance-001'], evalRequired: true, sampleInput: { repo_url: 'internal://codebase', scan_depth: 'full' }, sampleOutput: { vulnerabilities: 3, severity: 'medium', top_issue: 'Outdated auth library', recommended_action: 'Patch and review' } },
  { id: 'skill-data-quality', name: 'Data Quality Evaluator', category: 'Data Intelligence', domain: 'A11oy Core', version: '1.1.0', status: 'LIVE', calls: 842, successRate: 0.99, avgLatencyMs: 55, description: 'Evaluates signal data quality, freshness, and completeness before feeding into Workcells. Outputs: quality score, stale fields, recommended refresh.', allowedTools: ['read_issue'], blockedTools: ['admin_api'], requiredPolicies: ['pol-governance-001'], evalRequired: false, sampleInput: { signal_batch_id: 'batch-2026-04-25', tenant_id: 'meridian' }, sampleOutput: { quality_score: 0.91, stale_fields: ['port_status'], recommended_action: 'Refresh port_status from AIS connector' } },
  { id: 'skill-connector-risk', name: 'Connector Risk Assessor', category: 'Governance', domain: 'A11oy Core', version: '1.2.0', status: 'LIVE', calls: 67, successRate: 1.0, avgLatencyMs: 95, description: 'Assesses connector trust score, injection risk, and schema validation status. Flags high-risk connectors for review.', allowedTools: ['read_incident'], blockedTools: ['admin_api'], requiredPolicies: ['pol-governance-001', 'pol-compliance-001'], evalRequired: false, sampleInput: { connector_id: 'conn-generic-mcp' }, sampleOutput: { trust_score: 40, risk_level: 'high', injection_attempts: 0, recommended_action: 'Pending review — no tools approved' } },
  { id: 'skill-decision-latency', name: 'Decision Latency Monitor', category: 'Governance', domain: 'A11oy Core', version: '1.0.0', status: 'LIVE', calls: 1840, successRate: 1.0, avgLatencyMs: 12, description: 'Measures time from signal ingestion to human decision. Flags approval bottlenecks and computes decision velocity KPIs.', allowedTools: [], blockedTools: ['admin_api'], requiredPolicies: ['pol-governance-001'], evalRequired: false, sampleInput: { tenant_id: 'northstar', period_hours: 24 }, sampleOutput: { avg_decision_latency_min: 23, p95_latency_min: 87, bottleneck: 'executive_approval', recommended_action: 'Delegate tier-2 approvals' } },
];

// ─── Replay Reports ────────────────────────────────────────────────────────────

export const REPLAY_REPORTS = [
  {
...
export const BOARD_PACKETS: Array<Record<string, unknown>> = [
  {
...
// ─── Tenant Board Metadata ─────────────────────────────────────────────────────

export const TENANT_BOARD_META: Record<string, { name: string; domain: string; approvedBy: string }> = {
...
export function toBoardPacket(p: Record<string, unknown>) {
...
export const BOARDROOM_CAPABILITIES = [
