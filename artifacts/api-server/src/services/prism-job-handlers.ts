...
import { createExtractionJob, completeExtraction, failExtraction } from "./prism-document-pipeline";
...
export function registerAllPrismJobHandlers(): void {
  registerPrismJobHandler(PRISM_JOB_TYPES.DEADLINE_EVALUATE, async (job) => {
    const now = new Date();
    const overdueDeadlines = await db.select().from(pcDeadlinesTable)
      .where(and(
        eq(pcDeadlinesTable.status, "pending"),
        lte(pcDeadlinesTable.dueDate, now)
      ));

    let updated = 0;
    for (const d of overdueDeadlines) {
      await db.update(pcDeadlinesTable)
        .set({ status: "overdue" })
    ...
    for (const d of upcomingDeadlines) {
      const daysUntil = Math.ceil((new Date(d.dueDate!).getTime() - now.getTime()) / (24 * 60 * 60 * 1000));
      if (daysUntil <= 3) {
        await db.insert(pcNotificationsTable).values({
          orgId: job.orgId,
          matterId: d.matterId,
    ...
    logger.info({ updated, warnings: upcomingDeadlines.length }, "[prism-jobs] Deadline evaluation complete");
  ...
  registerPrismJobHandler(PRISM_JOB_TYPES.DOCUMENT_EXTRACT, async (job) => {
    ...
    if (azureDocIntelEndpoint && azureDocIntelKey) {
      try {
        ...
        const response = await fetch(`${azureDocIntelEndpoint}/formrecognizer/documentModels/prebuilt-layout:analyze?api-version=2023-07-31`, {
          method: "POST",
          headers: {
        ...
        await completeExtraction(extractionJobId, {
      ...
      logger.info({ documentId: payload.documentId }, "[prism-jobs] Doc Intel not configured, placeholder extraction stored");
  ...
  registerPrismJobHandler(PRISM_JOB_TYPES.NOTIFICATION_SEND, async (job) => {
    ...
    logger.info({ notificationType: payload.type, orgId: job.orgId }, "[prism-jobs] Notification dispatched");
    return { dispatched: true, type: payload.type };
  });

  registerPrismJobHandler(PRISM_JOB_TYPES.EXPORT_GENERATE, async (job) => {
    const payload = job.payload as { exportId: number; exportType: string; format: string; matterId?: number };

    await db.update(pcExportsTable)
  ...
  registerPrismJobHandler(PRISM_JOB_TYPES.CONNECTOR_SYNC, async (job) => {
    const payload = job.payload as { accountId: number; syncRunId: number; connectorType: string; config: unknown };
    ...
    try {
      ...
      if (payload.connectorType === "microsoft_365") {
        ...
        if (graphToken) {
          const response = await fetch("https://graph.microsoft.com/v1.0/me/messages?$top=10&$select=id,subject,from,receivedDateTime,hasAttachments", {
  ...
  registerPrismJobHandler(PRISM_JOB_TYPES.MANUAL_REVIEW, async (job) => {
    const payload = job.payload as { extractionJobId: number; documentId: number; confidence: number };
    logger.info({ documentId: payload.documentId, confidence: payload.confidence }, "[prism-jobs] Manual review queued");
    return { queued: true };
  });
