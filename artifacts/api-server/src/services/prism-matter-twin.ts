...
class MatterTwinService {
  async buildSnapshot(orgId: number, matterId: number, snapshotType: "daily" | "weekly" | "on_change" | "manual" = "on_change") {
    ...
    const domains = {
      ...
      communications: { total: comms.length, recent: comms.slice(0, 5).map(c => ({ type: c.channel, date: c.createdAt })) },
    ...
    const [snapshot] = await db.insert(pcMatterTwinSnapshotsTable).values({
      ...
      communicationsSummary: { total: comms.length, recentTypes: comms.slice(0, 5).map(c => c.channel) },
