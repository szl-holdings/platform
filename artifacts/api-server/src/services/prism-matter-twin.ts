...
class MatterTwinService {
  async buildSnapshot(
    orgId: number,
    matterId: number,
    snapshotType: 'daily' | 'weekly' | 'on_change' | 'manual' = 'on_change',
  ) {
    ...
    const [parties, claims, deadlines, comms] = await Promise.all([
      ...
      db
        ...
        .orderBy(desc(pcCommunicationsTable.createdAt))
        .limit(20),
    ]);
    ...
    const worldlineFeatures = await db
      .select()
      ...
      .orderBy(desc(pcWorldlineFeaturesTable.createdAt))
      .limit(20);
    ...
    const priorSnapshot = await db
      .select()
      .from(pcMatterTwinSnapshotsTable)
      .where(
        and(
          eq(pcMatterTwinSnapshotsTable.orgId, orgId),
          eq(pcMatterTwinSnapshotsTable.matterId, matterId),
        ),
      )
      .orderBy(desc(pcMatterTwinSnapshotsTable.createdAt))
      .limit(1);

    const changes = this.computeChanges(priorSnapshot[0] ?? null, latestPressure, forecasts);
    const missingArtifacts = this.identifyMissing(
      matter,
      parties,
      claims,
      deadlines,
    ...
    const domains = {
      ...
      deadlines: {
        ...
        upcoming: deadlines.filter((d: any) => d.status === 'pending').length,
      },
      documents: {
        proofChainEntries: proofChain.length,
        reviewPending: proofChain.filter((p) => p.reviewState === 'pending_review').length,
      },
      communications: {
        total: comms.length,
        recent: comms.slice(0, 5).map((c) => ({ type: c.channel, date: c.createdAt })),
      },
      forecasts: {
        total: forecasts.length,
        types: [...new Set(forecasts.map((f) => f.forecastType))],
      },
      worldline: {
        features: worldlineFeatures.length,
        sourceClasses: [...new Set(worldlineFeatures.map((f) => f.sourceClass))],
      },
      dataProducts: latestProducts,
    };

    const [snapshot] = await db
      .insert(pcMatterTwinSnapshotsTable)
      .values({
        orgId,
        matterId,
        snapshotType,
        domains,
        pressureScores: latestPressure,
        forecastSnapshot: forecasts.slice(0, 10),
        worldlineOverlays: worldlineFeatures.slice(0, 10),
        communicationsSummary: {
          total: comms.length,
          recentTypes: comms.slice(0, 5).map((c) => c.channel),
        },
        documentsSummary: {
          proofChainEntries: proofChain.length,
          pendingReview: proofChain.filter((p) => p.reviewState === 'pending_review').length,
        },
        approvalsSummary: {
          pending: proofChain.filter((p) => p.approvalState === 'pending').length,
        },
    ...
    logger.info(
      { orgId, matterId, snapshotId: snapshot.id, snapshotType },
      'Matter Twin snapshot created',
    );
    return snapshot;
  }
  ...
  private computeChanges(
    ...
    for (const [dim, current] of Object.entries(currentPressure)) {
      ...
      if (!prior) {
        changes.push(`New pressure dimension: ${dim}`);
        continue;
      }
      const diff = (current as any).score - (prior.score ?? 0);
      if (Math.abs(diff) > 0.05) {
        changes.push(
          `${dim} pressure ${diff > 0 ? 'increased' : 'decreased'} by ${Math.abs(Math.round(diff * 100))}%`,
        );
      }
    }
  ...
  private identifyMissing(
    ...
    if (!matter.settlementLow && !matter.settlementHigh)
      missing.push('No settlement range estimated');
  ...
  private identifyRisks(
    ...
    for (const [dim, p] of Object.entries(pressure)) {
      if ((p as any).score > 0.7)
        risks.push({
          level: 'high',
          description: `${dim} pressure elevated at ${Math.round((p as any).score * 100)}%`,
          dimension: dim,
        });
      else if ((p as any).score > 0.5)
        risks.push({
          level: 'medium',
          description: `${dim} pressure moderate at ${Math.round((p as any).score * 100)}%`,
          dimension: dim,
        });
    }
    ...
    if (overdue.length > 0)
      risks.push({
        level: 'critical',
        description: `${overdue.length} overdue deadline(s)`,
        dimension: 'deadline',
      });
  ...
  private recommendActions(risks: any[], missing: string[], _pressure: Record<string, any>) {
    ...
    for (const r of risks
      .filter((r: any) => r.level === 'critical' || r.level === 'high')
      .slice(0, 3)) {
