...
export class PilotExportService {
  async generateExport(
    ...
    if (m.privilegeFlag && !m.exportSafe) {
      throw Object.assign(
        new Error(
          'Export blocked: this matter has unresolved privilege flags. Resolve all pending privilege reviews before exporting.',
        ),
        { statusCode: 403 },
      );
    }
    ...
    if (unreviewedPrivilegedDocs.length > 0) {
      throw Object.assign(
        new Error(
          'Export blocked: matter contains privileged documents with unreviewed status. Complete privilege review for all flagged documents before exporting.',
        ),
        { statusCode: 403 },
      );
    }
    ...
    if (pendingPrivilegeFlags.length > 0 && !m.exportSafe) {
      throw Object.assign(
        new Error(
          'Export blocked: matter has outstanding privilege flags that require review. Resolve all privilege assertions before exporting.',
        ),
        { statusCode: 403 },
      );
    }
    ...
    if (data.reviewItemId) {
      const review = await db
        ...
        .where(
          and(eq(pcReviewItemsTable.id, data.reviewItemId), eq(pcReviewItemsTable.orgId, orgId)),
        )
        .limit(1);
    ...
    const result = await db
      ...
      .values({
        ...
        accessLog: [
          { action: 'created', userId: data.generatedBy, timestamp: new Date().toISOString() },
        ],
      })
      .returning();
  ...
  async buildDocxContent(orgId: number, exportId: number) {
    ...
    return {
      content,
      matterTitle,
      proofChainRef: exp.proofChainRef,
