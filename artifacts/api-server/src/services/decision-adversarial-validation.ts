...
function checkContradiction(input: AdversarialValidationInput): ValidationCheckResult {
  ...
  const textToScan = [input.summary, input.reasoning ?? '', ...input.evidence.map((e) => e.value)]
    .join(' ')
    .toLowerCase();
  ...
  return {
    ...
    passed: true,
    explanation:
      'No direct contradictions detected between evidence items and the recommendation. Evidence chain is internally consistent.',
    severity: 'info',
  };
}
...
function checkMissingEvidence(input: AdversarialValidationInput): ValidationCheckResult {
  if (input.evidence.length === 0) {
    return {
      ...
      passed: false,
      explanation:
        'No evidence items attached to this decision card. Every recommendation must carry at least one evidence item with source attribution. Cannot promote without evidence.',
      severity: 'blocking',
    };
  }
...
function checkConfidenceFloor(input: AdversarialValidationInput): ValidationCheckResult {
  ...
  return {
    ...
    metadata: {
      compositeConfidence,
      cardConfidence: input.confidence,
