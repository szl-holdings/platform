...
class InsurerPressureEngine {
  async computePressure(orgId: number, matterId: number): Promise<PressureAnalysis> {
    ...
    const carrierComms = communications.filter(
      (c) =>
        c.fromParty?.toLowerCase().includes('carrier') ||
        c.fromParty?.toLowerCase().includes('insurer') ||
    ...
    if (daysSilent >= 14) {
      ...
      drivers.push({
        name: 'Carrier Silence Window',
        category: 'silence_window',
        weight: w,
        impact: 'negative',
        explanation: `Carrier has been silent for ${daysSilent} days — silence windows exceeding 14 days typically indicate posture hardening or internal review.`,
      });
      ...
      drivers.push({
        name: 'Response Lag',
        category: 'response_latency',
        weight: 0.5,
        impact: 'negative',
        explanation: `${daysSilent}-day response lag — monitor for extension into a silence window.`,
      });
    ...
    if (denialComms.length > 0) {
      ...
      drivers.push({
        name: 'Denial Activity',
        category: 'denial_frequency',
        weight: w,
        impact: 'negative',
        explanation: `${denialComms.length} denial communication(s) detected. Repeated denials indicate carrier resistance pattern.`,
      });
    ...
    if (claims.length === 0) {
      drivers.push({
        ...
        category: 'matter_stage_sensitivity',
        weight: 0.6,
        impact: 'negative',
        explanation:
          'No carrier claims have been linked. Pressure cannot be accurately scored without claim context.',
      });
      rawScore += 0.1;
    }

    const partialComms = communications.filter(
      (c) =>
        c.summary?.toLowerCase().includes('partial') ||
        c.summary?.toLowerCase().includes('incomplete'),
    );
    if (partialComms.length > 0) {
      drivers.push({
        ...
        weight: 0.65,
        impact: 'negative',
        explanation: `${partialComms.length} partial or incomplete carrier response(s) detected. Partial responses indicate delay tactics.`,
      });
      rawScore += 0.1;
      signals.push('Partial response pattern detected');
    }

    const overallScore = Math.min(1, Math.max(0, rawScore + 0.3));
    ...
    const operationalMeaning = this.generateOperationalMeaning(
      overallScore,
      daysSilent,
      denialComms.length,
      direction,
    ...
    if (drivers.length === 0) {
      drivers.push({
        name: 'Standard Response Cadence',
        category: 'response_latency',
        weight: 0.3,
        impact: 'neutral',
        explanation:
          'Carrier communications are within expected response windows. No elevated pressure signals detected.',
      });
    }
  ...
  private generateOperationalMeaning(
    score: number,
    daysSilent: number,
    denials: number,
    direction: string,
    ...
    if (score >= 0.75)
      return `High insurer pressure — ${daysSilent > 14 ? 'prolonged silence window' : 'multiple resistance signals'} detected. ${direction === 'rising' ? 'Pressure is escalating and requires immediate action.' : 'Sustained high pressure requires strategic response.'}`;
    if (score >= 0.55)
      return `Moderate-to-high insurer pressure. ${denials > 0 ? 'Denial pattern is active.' : 'Communication cadence is lagging.'} ${direction === 'rising' ? 'Pressure is increasing — act before it escalates.' : 'Monitor closely for changes.'}`;
    if (score >= 0.35)
      return `Moderate insurer pressure. Carrier is engaged but showing ${daysSilent > 7 ? 'response delays' : 'some resistance signals'}. Maintain monitoring cadence.`;
  ...
  private generateRecommendedAction(score: number, daysSilent: number, denials: number): string {
    if (score >= 0.75 && daysSilent > 21)
      return 'Send formal escalation letter to carrier supervisor, document the silence window, and consider filing a bad faith preservation notice.';
    if (score >= 0.75)
      return 'Issue written follow-up demand with response deadline. Prepare bad faith documentation if silence continues.';
    if (score >= 0.55 && denials > 0)
      return 'Request written basis for all denials. Prepare counter-argument packet and consider formal dispute letter.';
    if (score >= 0.55)
      return 'Send structured follow-up communication with specific asks and a reasonable response deadline. Track all carrier responses.';
    if (score >= 0.35)
      return 'Maintain regular communication cadence. Log all carrier responses and flag any silence windows immediately.';
  ...
  async saveSnapshot(orgId: number, matterId: number, analysis: PressureAnalysis): Promise<number> {
    const [snapshot] = await db
      .insert(pcInsurerPressureSnapshotsTable)
      .values({
        orgId,
        matterId,
    ...
    logger.info(
      ...
      'Insurer pressure snapshot saved',
    );
    return snapshot.id;
  }
  ...
  async compute(
    orgId: number,
    matterId: number,
  ): Promise<{ snapshotId: number; analysis: PressureAnalysis }> {
  ...
  async generateSilenceWindow(
    ...
    const existing = await db
      ...
      .where(
        and(
          eq(pcCarrierSilenceWindowsTable.orgId, orgId),
          eq(pcCarrierSilenceWindowsTable.matterId, matterId),
    ...
    if (existing.length === 0) {
      await db.insert(pcCarrierSilenceWindowsTable).values({
        ...
        escalationTemplateKey:
          daysSilent >= 21 ? 'carrier_silence_formal_escalation' : 'carrier_silence_follow_up',
        outstandingItems: ['Response to last demand letter', 'Claim status update'],
      });
    } else {
      await db
        .update(pcCarrierSilenceWindowsTable)
        .set({
          daysSilent,
          updatedAt: new Date(),
          silenceRisk: daysSilent >= 21 ? 'critical' : daysSilent >= 14 ? 'high' : 'medium',
    ...
    for (const m of matters) {
      const latest = await db
        ...
        .where(
          and(
            eq(pcInsurerPressureSnapshotsTable.orgId, orgId),
            eq(pcInsurerPressureSnapshotsTable.matterId, m.id),
          ),
        )
        .orderBy(desc(pcInsurerPressureSnapshotsTable.computedAt))
        .limit(1);
  ...
  async getCarrierPatterns(orgId: number, carrierName?: string) {
    const conditions = carrierName
      ? and(
          ...
          eq(pcCarrierBehaviorPatternsTable.carrierName, carrierName),
        )
      : eq(pcCarrierBehaviorPatternsTable.orgId, orgId);
    return db
      .select()
      .from(pcCarrierBehaviorPatternsTable)
      .where(conditions)
      .orderBy(desc(pcCarrierBehaviorPatternsTable.updatedAt))
      .limit(50);
  }
  ...
  async getSilenceWindows(orgId: number, matterId?: number) {
    ...
    return db
      ...
      .from(pcCarrierSilenceWindowsTable)
      .where(conditions)
      .orderBy(desc(pcCarrierSilenceWindowsTable.daysSilent))
      .limit(50);
  }
}
