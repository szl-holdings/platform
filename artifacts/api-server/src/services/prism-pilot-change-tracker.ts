...
export class PilotChangeTracker {
  ...
  async generateMorningBrief(orgId: number, userId: number) {
    ...
    const recentChanges = await db
      ...
      .from(pcChangeEventsTable)
      .where(
        and(eq(pcChangeEventsTable.orgId, orgId), gte(pcChangeEventsTable.createdAt, yesterday)),
      )
      .orderBy(desc(pcChangeEventsTable.createdAt));
    ...
    const mattersChanged = matterIds.map((mid) => {
      ...
      return {
        ...
        types: [...new Set(changes.map((c) => c.changeType))],
      };
    });
    ...
    const brief = await db
      ...
      .values({
        ...
        silenceWindows: quietRisks
          .filter((r) => r.riskType === 'no_carrier_response')
          .map((r) => ({
            matterId: r.matterId,
            daysSilent: r.daysSilent,
            explanation: r.explanation,
          })),
        newFilesReceived: recentChanges
          .filter((c) => c.changeType === 'new_file')
          .map((c) => ({
            matterId: c.matterId,
            title: c.title,
  ...
  async detectQuietRisks(orgId: number) {
    const matters = await db
      .select()
      .from(pcMattersTable)
      .where(and(eq(pcMattersTable.orgId, orgId), eq(pcMattersTable.status, 'active' as any)));
    ...
    for (const matter of matters) {
      const recentComms = await db
        ...
        .where(
          and(
            eq(pcCommunicationsTable.matterId, matter.id),
  ...
  async getNextActions(orgId: number, matterId?: number) {
    const conditions = [
      eq(pcNextActionsTable.orgId, orgId),
