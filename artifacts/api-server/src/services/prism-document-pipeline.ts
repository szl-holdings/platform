...
export async function ingestDocument(
  ...
  const existing = await db.select({ id: pcDocumentsTable.id })
    .from(pcDocumentsTable)
    .where(and(
      eq(pcDocumentsTable.orgId, orgId),
      eq(pcDocumentsTable.checksum, checksum)
    ))
    .limit(1);

  if (existing.length > 0) {
    logger.info({ documentId: existing[0].id, checksum }, "[doc-pipeline] Duplicate document detected");
  ...
  const [doc] = await db.insert(pcDocumentsTable).values({
    orgId,
  ...
  await enqueuePrismJob(orgId, PRISM_JOB_TYPES.DOCUMENT_EXTRACT, {
    documentId: doc.id,
    mimeType,
    storageUri,
  }, {
...
export async function createExtractionJob(
  ...
  const [job] = await db.insert(pcExtractionJobsTable).values({
    orgId,
    documentId,
  ...
  logger.info({ extractionJobId: job.id, documentId, provider }, "[doc-pipeline] Extraction job created");
  return job.id;
}
...
export async function completeExtraction(
  ...
  await db.update(pcExtractionJobsTable).set({
    status,
  ...
  if (status === "review_required") {
    const [job] = await db.select().from(pcExtractionJobsTable)
      .where(eq(pcExtractionJobsTable.id, extractionJobId));
...
export async function getDocumentsForMatter(matterId: number, orgId: number) {
  return db.select().from(pcDocumentsTable)
    .where(and(
      eq(pcDocumentsTable.matterId, matterId),
      eq(pcDocumentsTable.orgId, orgId)
    ))
...
export async function getExtractionJobsForDocument(documentId: number) {
  return db.select().from(pcExtractionJobsTable)
