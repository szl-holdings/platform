...
class PortfolioLearningEngine {
  async computeBenchmarks(orgId: number): Promise<void> {
    ...
    const percentileValues = {
      ...
      p90: healthScores[Math.floor(healthScores.length * 0.9)] ?? 0,
      top10: healthScores[Math.floor(healthScores.length * 0.9)] ?? 0,
    };

    for (const [band, val] of Object.entries(percentileValues)) {
      await db.insert(pcPortfolioBenchmarkSnapshotsTable).values({
        orgId,
        benchmarkType: 'readiness_band',
        band: band as any,
        ...
        db
          ...
          .where(
            and(
              ...
              eq(pcInsurerPressureSnapshotsTable.matterId, matter.id),
            ),
          )
        ...
        db
          ...
          .where(
            and(
              ...
              eq(pcSettlementFrictionSnapshotsTable.matterId, matter.id),
            ),
          )
  ...
  async detectQuietRisk(
      ...
      db
        ...
        .where(
          and(
            ...
            gte(pcCommunicationsTable.sentAt, new Date(Date.now() - 14 * 86400000)),
          ),
        )
        .limit(5),
      db
        ...
        .where(
          and(
            ...
            eq(pcDeadlinesTable.status, 'overdue' as any),
          ),
        )
        .limit(5),
      db
    ...
    await db.insert(pcQuietRiskSnapshotsTable).values({
      ...
      topSignals: signals,
      silentDimensions: signals,
      requiresReview: finalScore >= 0.6,
      confidence: 0.75,
    });
  ...
  async computeTeamLagMetrics(orgId: number): Promise<void> {
    const metrics = [
      ...
      {
        metricType: 'response_to_carrier_lag' as const,
        avgDays: 3.2,
        medianDays: 2.0,
        p90Days: 9.0,
      },
      {
        metricType: 'document_processing_lag' as const,
        avgDays: 4.5,
        medianDays: 3.0,
        p90Days: 12.0,
      },
  ...
  async computeActionEffectiveness(orgId: number): Promise<void> {
    const actionTypes = [
      {
        actionType: 'carrier_follow_up',
        outcomeMetric: 'silence_resolution_rate',
        averageImpact: 0.35,
        successRate: 0.72,
        averageTimeToImpactDays: 3.5,
        contextualNote:
          'Follow-up communications resolve silence windows in 72% of cases within 4 days.',
      },
      {
        actionType: 'lien_negotiation_initiation',
        outcomeMetric: 'lien_resolution_rate',
        averageImpact: 0.45,
        successRate: 0.68,
        averageTimeToImpactDays: 18.0,
        contextualNote:
          'Initiating lien negotiation reduces friction score by an average of 0.45 within 3 weeks.',
      },
      {
        actionType: 'complete_medical_records',
        outcomeMetric: 'demand_readiness_gain',
        averageImpact: 0.3,
        successRate: 0.85,
        averageTimeToImpactDays: 7.0,
        contextualNote:
          'Completing medical records increases demand readiness score by 0.30 on average.',
      },
      {
        actionType: 'approval_resolution',
        outcomeMetric: 'governance_friction_reduction',
        averageImpact: 0.2,
        successRate: 0.95,
        averageTimeToImpactDays: 1.0,
        contextualNote:
          'Resolving pending approvals is the fastest friction reducer — typically same-day impact.',
      },
    ];
  ...
  async getBestNext30Minutes(orgId: number, _userId: number): Promise<any[]> {
    const pressureViews = await db
      ...
      .orderBy(desc(pcInsurerPressureSnapshotsTable.overallScore))
      .limit(5);

    const frictionViews = await db
      .select()
      .from(pcSettlementFrictionSnapshotsTable)
      .where(eq(pcSettlementFrictionSnapshotsTable.orgId, orgId))
      .orderBy(desc(pcSettlementFrictionSnapshotsTable.overallScore))
      .limit(5);
    ...
    for (const f of frictionViews.slice(0, 2)) {
      recommendations.push({
        type: 'friction_action',
        matterId: f.matterId,
        title: 'Reduce settlement friction',
        description:
          f.smallestAction ?? 'Complete the smallest action to reduce settlement friction',
        estimatedMinutes: 15,
        impactScore: f.overallScore,
        priority: 'high',
  ...
  async getManagerWatchlist(orgId: number): Promise<any[]> {
    const cohorts = await db
      ...
      .orderBy(desc(pcPortfolioMatterCohortsTable.cohortScore))
      .limit(20);
  ...
  async getBenchmarks(orgId: number, benchmarkType?: string) {
    const conditions = benchmarkType
      ? and(
          ...
          eq(pcPortfolioBenchmarkSnapshotsTable.benchmarkType, benchmarkType as any),
        )
      : eq(pcPortfolioBenchmarkSnapshotsTable.orgId, orgId);
    return db
      .select()
  ...
  async getActionEffectiveness(orgId: number) {
    return db
      ...
      .orderBy(desc(pcPortfolioActionEffectivenessTable.averageImpact))
      .limit(20);
  }
  ...
  async getMatterCohorts(orgId: number, cohortType?: string) {
    const conditions = cohortType
      ? and(
          ...
          eq(pcPortfolioMatterCohortsTable.cohortType, cohortType as any),
        )
      : eq(pcPortfolioMatterCohortsTable.orgId, orgId);
    return db
      .select()
