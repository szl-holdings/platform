  ...
  async submitForSignoff(orgId: number, reviewId: number, userId: number) {
    ...
    const signoff = await db
      ...
      .values({
        ...
        supportSummary: review.sourceSupport
          ? `${(review.sourceSupport as any[]).length} sources supporting this output`
          : 'No source support attached',
        riskSummary: review.contradictionWarnings
          ? `${(review.contradictionWarnings as any[]).length} contradiction warning(s) detected`
