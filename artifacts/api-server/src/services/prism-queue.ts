...
export async function enqueuePrismJob(
  ...
  if (options.idempotencyKey) {
    const existing = await db
      .select({ id: pcBackgroundJobsTable.id })
    ...
    if (existing.length > 0) {
      logger.info(
        ...
        '[prism-queue] Duplicate job skipped',
      );
  ...
  const correlationId =
    options.correlationId ?? `prism-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

  const [job] = await db
    .insert(pcBackgroundJobsTable)
    .values({
      orgId,
      jobType,
      status: 'pending',
...
export async function replayDeadLetterEvent(
  ...
  const newJobId = await enqueuePrismJob(dle.orgId, dle.jobType as PrismJobType, dle.payload, {
    actorId,
    correlationId: `replay-${dle.id}-${Date.now()}`,
  });

  await db
    .update(pcDeadLetterEventsTable)
