...
export async function createConnectorAccount(
  ...
  logger.info(
    { accountId: account.id, connectorType, orgId },
    '[prism-connectors] Account created',
  );
  return account.id;
}
...
export async function activateConnector(accountId: number): Promise<void> {
  await db
    .update(pcConnectorAccountsTable)
...
export async function triggerSync(
  ...
  if (account.status !== 'active')
    throw new Error(`Connector ${accountId} is not active (status: ${account.status})`);

  const [syncRun] = await db
    .insert(pcConnectorSyncRunsTable)
    .values({
  ...
  await enqueuePrismJob(
    orgId,
    PRISM_JOB_TYPES.CONNECTOR_SYNC,
    {
      accountId,
...
export async function completeSyncRun(syncRunId: number, result: SyncResult): Promise<void> {
  await db
    .update(pcConnectorSyncRunsTable)
    .set({
...
export async function registerGraphSubscription(
  ...
  logger.info(
    { orgId, subscriptionId, resourcePath },
    '[prism-connectors] Graph subscription registered',
  );
}
...
export async function getExpiredSubscriptions() {
  return db
    .select()
    .from(pcGraphSubscriptionStateTable)
    .where(
      and(
        eq(pcGraphSubscriptionStateTable.status, 'active'),
        lte(
          pcGraphSubscriptionStateTable.expirationDateTime,
