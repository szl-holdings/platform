...
const SEVERITY_WEIGHT: Record<DriftFinding["severity"], number> = {
  none: 0, low: 10, medium: 35, high: 65, critical: 95,
};

export function aggregateDrift(findings: DriftFinding[]): { score: number; severity: DriftReport["severity"] } {
...
export async function evaluateDrift(agentId: string, envId: string): Promise<DriftReport> {
  ...
  const [env] = await db.select().from(forgeEnvironmentProfilesTable).where(eq(forgeEnvironmentProfilesTable.id, envId)).limit(1);
  ...
  if (!snapshot) {
    ...
    const inv = (snapshot.agentInventory as Record<string, { modelId?: string; promptVersionId?: string; toolIds?: string[] }>) ?? {};
    const observed = inv[agent.id];

    if (version && observed) {
      if (version.modelId && observed.modelId && version.modelId !== observed.modelId) {
        findings.push({ dimension: "model", expected: version.modelId, observed: observed.modelId, severity: "high", detail: "Active model differs from snapshot" });
      }
      if (version.promptVersionId && observed.promptVersionId && version.promptVersionId !== observed.promptVersionId) {
        findings.push({ dimension: "prompt", expected: version.promptVersionId, observed: observed.promptVersionId, severity: "medium", detail: "Prompt version drift detected" });
      }
      ...
      findings.push({ dimension: "config", expected: "deployed", observed: "absent", severity: "critical", detail: "Active version not found in environment snapshot" });
    }
  }

  const { score, severity } = aggregateDrift(findings);
  const recommendation =
    severity === "critical" ? "Block promotions and freeze production traffic until drift is reconciled."
    : severity === "high" ? "Re-snapshot the environment and re-evaluate active version inventory."
    : severity === "medium" ? "Schedule a controlled redeploy at the next maintenance window."
    : severity === "low" ? "Monitor for additional drift over the next 24h."
    : "No action required.";
...
export async function validatePromotion(params: {
  ...
  const [agent] = await db.select().from(forgeAgentsTable).where(eq(forgeAgentsTable.id, params.agentId)).limit(1);
  if (!agent) {
    blockers.push({ code: "AGENT_NOT_FOUND", message: `Agent ${params.agentId} not found` });
    return { ok: false, blockers, report };
  }

  const [version] = await db.select().from(forgeAgentVersionsTable).where(eq(forgeAgentVersionsTable.id, params.toVersionId)).limit(1);
  if (!version) {
    blockers.push({ code: "VERSION_NOT_FOUND", message: `Version ${params.toVersionId} not found` });
    return { ok: false, blockers, report };
  }
  ...
  if (!version.evalsPassed) {
    blockers.push({ code: PROMOTION_BLOCKER_CODES.EVALS_NOT_PASSED, message: "Required evaluation suite has not passed for this version" });
  }
  // 2. Observability hook must be configured
  if (!version.observabilityHookConfigured) {
    blockers.push({ code: PROMOTION_BLOCKER_CODES.MISSING_OBSERVABILITY, message: "Observability hook not configured for this version" });
  }
  // 3. Provenance must be complete
  if (!version.provenanceComplete) {
    blockers.push({ code: PROMOTION_BLOCKER_CODES.MISSING_PROVENANCE, message: "Model + prompt + tool + data-source provenance is incomplete" });
  }

  // 4. Model must be approved
  if (version.modelId) {
    const [model] = await db.select().from(forgeModelsTable).where(eq(forgeModelsTable.id, version.modelId)).limit(1);
    if (!model || !model.approved) {
      blockers.push({ code: PROMOTION_BLOCKER_CODES.UNAPPROVED_MODEL, message: `Model ${model?.slug ?? version.modelId} is not on the approved list` });
    }
    report.model = model?.slug ?? null;
  } else {
    blockers.push({ code: PROMOTION_BLOCKER_CODES.MISSING_PROVENANCE, message: "No model assigned to this version" });
  }

  // 5. All tools must be approved
  const toolIds = (version.toolIds ?? []) as string[];
  if (toolIds.length) {
    const tools = await db.select().from(forgeToolsTable).where(inArray(forgeToolsTable.id, toolIds));
    const unapproved = tools.filter(t => !t.approved).map(t => t.slug);
    if (unapproved.length) {
      blockers.push({ code: PROMOTION_BLOCKER_CODES.UNAPPROVED_TOOL, message: `Unapproved tools assigned: ${unapproved.join(", ")}` });
    }
    report.tools = tools.map(t => ({ slug: t.slug, approved: t.approved }));
  }

  // 6. Required secrets present in target env
  const [targetEnv] = await db.select().from(forgeEnvironmentProfilesTable).where(eq(forgeEnvironmentProfilesTable.tier, params.toEnv)).limit(1);
  if (targetEnv?.targetId) {
    const target = await db.execute(sql`SELECT required_secrets FROM forge_deployment_targets WHERE id = ${targetEnv.targetId}`);
    const required = (target.rows[0]?.required_secrets as string[] | undefined) ?? [];
    const missing = required.filter(s => !process.env[s]);
    if (missing.length) {
      blockers.push({ code: PROMOTION_BLOCKER_CODES.MISSING_SECRETS, message: `Missing secrets in ${params.toEnv}: ${missing.join(", ")}` });
    }
  ...
  const requiresApproval = params.toEnv === "production" || agent.riskTier === "regulated" || agent.riskTier === "executive";
...
export async function enforcePolicy(params: {
  ...
  if (assignment) {
    const [p] = await db.select().from(forgePolicyPacksTable).where(eq(forgePolicyPacksTable.id, assignment.policyPackId)).limit(1);
    pack = p ?? null;
  }
  if (!pack) {
    return { outcome: "allow", policyPackId: null, policyPackName: null, reasons: ["No policy pack assigned — default allow (low-risk internal)"] };
  }
...
export async function captureRuntime(input: RuntimeCaptureInput): Promise<ForgeExecutionRun> {
  ...
  if (input.output !== undefined) artifacts.push({ kind: "output", content: { value: input.output } });
  if (input.trace !== undefined) artifacts.push({ kind: "trace", content: { value: input.trace } });
  if (artifacts.length) {
    await db.insert(forgeExecutionArtifactsTable).values(
      artifacts.map(a => ({ executionId: run!.id, kind: a.kind, content: a.content })),
    );
  }
...
export async function rollbackAgent(params: {
  ...
  const [target] = await db.select().from(forgeAgentVersionsTable).where(eq(forgeAgentVersionsTable.id, params.toVersionId)).limit(1);
  if (!target) throw new Error(`Target version ${params.toVersionId} not found`);
  if (target.agentId !== params.agentId) throw new Error("Target version does not belong to this agent");
...
export async function executePromotion(params: {
  ...
  const [promo] = await db.select().from(forgePromotionsTable).where(eq(forgePromotionsTable.id, params.promotionId)).limit(1);
  ...
  const approvals = await db.select().from(forgePromotionApprovalsTable)
    .where(and(
  ...
  if (!fresh.ok) {
    await db.update(forgePromotionsTable)
      .set({ status: "blocked", blockers: fresh.blockers, validationReport: fresh.report, updatedAt: new Date() })
      .where(eq(forgePromotionsTable.id, params.promotionId));
    throw new Error(`Cannot execute a promotion with blockers: ${fresh.blockers.map(b => b.code).join(", ")}`);
  }

  // Atomic env + active version swap
  await db.update(forgeAgentsTable)
...
export async function recordPromotionApproval(params: {
  ...
  const [row] = await db.insert(forgePromotionApprovalsTable).values({
  ...
  if (params.decision === "approved") {
    ...
    const [promo] = await db.select().from(forgePromotionsTable).where(eq(forgePromotionsTable.id, params.promotionId)).limit(1);
    const fresh = promo ? await validatePromotion({
      agentId: promo.agentId,
      toVersionId: promo.toVersionId,
      fromEnv: promo.fromEnv as EnvTier,
      toEnv: promo.toEnv as EnvTier,
...
export type { ForgeAgent, ForgeAgentVersion, ForgePromotion, ForgeDriftEvent, ForgeExecutionRun, ForgePolicyPack };
