  ...
  try {
    ...
    if (sfAdapter?.getOpportunities) {
      ...
      if (data?.records?.length) {
        return sendSuccess(res, data.records.map((r) => ({
          id: r["Id"],
          name: r["Name"],
          accountName: (r["Account"] as Record<string, unknown>)?.["Name"] ?? r["AccountName"] ?? null,
  ...
  try {
    ...
    if (hubSpotAdapter?.getDeals) {
      ...
      if (data?.results?.length) {
        return sendSuccess(res, data.results.map((d) => ({
          id: d["id"],
          ...
          closeDate: (d["properties"] as Record<string, unknown>)?.["closedate"] ?? new Date().toISOString(),
        })));
      }
    }
...
router.get("/hubspot/contacts", authMiddleware({ required: false }), async (_req, res) => {
  try {
    ...
    sendSuccess(res, { count: 0, contacts: [], available: false, note: "HubSpot connector not configured" });
  } catch (err) {
  ...
  try {
    ...
    if (dynAdapter?.getOpportunities) {
      ...
      if (data?.value?.length) {
        return sendSuccess(res, data.value.map((o) => ({
    ...
    sendSuccess(res, { items: [], available: false, note: "Dynamics 365 connector not configured" });
  } catch (err) {
    ...
    for (const type of typesToSync) {
      try {
        ...
        if (conn) {
          ...
          results[type] = { status: "not_configured", message: `${type} connector not configured`, timestamp: new Date().toISOString() };
        }
      } catch {
