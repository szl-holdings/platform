...
import { demoSeedGovernedScenariosBodySchema, validateBody } from '../lib/validation';
  ...
  async (_req: Request, res: Response) => {
    try {
      ...
      const vesselsModeConfig = PolicyModeConfigSchema.parse({
        ...
        reason:
          'Vessel rerouting carries significant fuel cost, contractual, and safety implications; requires operator sign-off.',
        createdBy: 'demo-seed',
        createdAt: Date.now(),
        updatedAt: Date.now(),
      });
      ...
      const carlotaModeConfig = PolicyModeConfigSchema.parse({
        ...
        environment: 'production',
        reason:
          'Vendor reschedule within cost guardrails can be handled autonomously to reduce operator load.',
        createdBy: 'demo-seed',
        createdAt: Date.now(),
        updatedAt: Date.now(),
      });
      ...
      const vesselEval = buildPolicyEvaluation({
        ...
        projectedImpact:
          'MV Albatross rerouted via Cape Finisterre to avoid Storm Nadia. +18h transit. Estimated fuel delta: +$34,200.',
        projectedRisk: 'Weather risk eliminated. Minor schedule delay for 2 downstream port calls.',
        evidenceChain: [
          {
            source: 'weather-intelligence',
            summary: 'Storm Nadia Force-9 winds along original route for next 36h',
            confidence: 0.96,
            freshness: 0.99,
          },
          {
            source: 'port-authority-feed',
            summary: 'Finisterre waypoint clear; berth availability confirmed for revised ETA',
            confidence: 0.88,
            freshness: 0.91,
          },
          {
            source: 'fuel-ledger',
            summary: 'Current fuel margin adequate for +18% transit extension',
            confidence: 0.83,
            freshness: 0.85,
          },
        ],
      ...
      const carlotaEval = buildPolicyEvaluation({
        ...
        projectedImpact:
          'FloorWorks Co. installation rescheduled from Friday 2pm to Monday 9am. Client notified automatically.',
        projectedRisk:
          'Minimal. Client confirmed availability for Monday slot. No penalty clause triggered.',
        evidenceChain: [
          {
            source: 'calendar-integration',
            summary: 'Client calendar shows Monday 9am as preferred alternative',
            confidence: 0.95,
            freshness: 0.99,
          },
          {
            source: 'vendor-crm',
            summary: 'FloorWorks confirms Monday slot available at no additional cost',
            confidence: 0.91,
            freshness: 0.97,
          },
        ],
      ...
      results.evaluations = {
        vessels: {
          ...
          mode: vesselEval.mode,
          action: vesselEval.action,
        },
        carlota: {
          evaluationId: carlotaEval.evaluationId,
          mode: carlotaEval.mode,
          action: carlotaEval.action,
        },
      };
      ...
      const vesselEntityEntry = buildMemoryEntry({
        ...
        summary:
          'MV Albatross reroute decision pending operator approval — Storm Nadia avoidance via Cape Finisterre',
        confidence: 0.91,
      ...
      const lessonEntry = buildMemoryEntry({
        ...
        value:
          'Vendor reschedule requests with cost < $2000 and confirmed client availability can be auto-executed without operator review.',
      ...
      results.memoryEntries = {
        seeded: [
          vesselEntityEntry.id,
          vesselWorkingEntry.id,
          carlotaEntityEntry.id,
          lessonEntry.id,
        ],
      };
