...
router.delete(
  ...
  validateBody(scimTokenRevokeSchema),
  tenantRateLimit,
  authMiddleware(),
  requireRole('admin'),
  async (req: Request, res: Response) => {
    try {
      ...
      await db
        ...
        .where(eq(scimTokensTable.id, tokenId));
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const brandingData: Partial<InsertTenantBranding> = {
        ...
        accentColor:
          body.accentColor !== undefined
            ? String(body.accentColor || '').trim() || null
            : undefined,
        sidebarHeaderText:
          body.sidebarHeaderText !== undefined
            ? String(body.sidebarHeaderText || '').trim() || null
            : undefined,
        customDomainLabel:
          body.customDomainLabel !== undefined
            ? String(body.customDomainLabel || '').trim() || null
            : undefined,
        emailFromName:
          body.emailFromName !== undefined
            ? String(body.emailFromName || '').trim() || null
            : undefined,
        emailFooterText:
          body.emailFooterText !== undefined
            ? String(body.emailFooterText || '').trim() || null
            : undefined,
      };
      ...
      const [existing] = await db
        .select({ id: tenantBrandingTable.id })
        .from(tenantBrandingTable)
        .where(eq(tenantBrandingTable.tenantId, id))
        .limit(1);
...
router.delete(
  '/admin/tenants/:id/branding',
  validateBody(tenantBrandingResetSchema),
  tenantRateLimit,
  authMiddleware(),
  requireRole('admin'),
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const [tenant] = await db
        .select({
          ...
          displayName: azureTenantsTable.displayName,
