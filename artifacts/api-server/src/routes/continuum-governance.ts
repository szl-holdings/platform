...
// Defense-in-depth: this router is mounted under /continuum and /governance in
// routes/groups/continuum.ts which already apply tenantScope({ required: true }),
...
router.use('/continuum/policies', tenantScope({ required: true }));
...
router.get('/continuum/policies/:id', authMiddleware(), async (req, res) => {
  try {
...
router.get(
  '/continuum/governance/incidents',
  authMiddleware(),
  validateQuery(listQuerySchema),
  async (req, res) => {
    try {
...
router.patch(
  '/continuum/governance/incidents/:id/resolve',
  authMiddleware(),
  validateBody(
    bodyShape({
