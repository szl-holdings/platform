...
router.use('/continuum/digest', tenantScope({ required: true }));
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        logger.warn({ err: dbErr }, 'continuum-digest: conflict on upsert, fetching existing');
      }
      ...
      if (deliveryChannels.includes('slack')) {
        ...
        if (!slackToken || !slackChannel) {
          ...
          logger.warn(
            'continuum-digest: Slack delivery requested but SLACK_BOT_TOKEN/ALLOY_DIGEST_SLACK_CHANNEL not set',
          );
        } else {
          try {
            const slackText = `*Continuum Daily Digest — ${date}*\n*Role: ${roleScope}*\n\n${markdown.slice(0, 2800)}${markdown.length > 2800 ? '\n\n_[digest truncated — view full version in app]_' : ''}`;
            ...
            logger.warn({ err: slackErr }, 'continuum-digest: Slack delivery failed');
          }
        }
      }
...
router.get('/continuum/digest/config', authMiddleware(), async (req: Request, res: Response) => {
  try {
    if (!req.user?.id) {
...
router.get('/continuum/digest/:id', authMiddleware(), async (req: Request, res: Response) => {
  try {
...
export async function generateAndStoreDigest(roleScope: string = 'executive'): Promise<string> {
  ...
  try {
    ...
    logger.warn({ err }, 'continuum-digest: scheduled generation conflict ignored');
  }
