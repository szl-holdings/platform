...
import {
  ...
  broadcastWs, pubsub, TERRA_EVENTS,
  CreateDealSchema, UpdateDealStageSchema, auditLog, nowStr,
  ...
  router.patch("/terra/pipeline/deals/:id/stage", authMiddleware({ required: true }), async (req, res) => {
    try {
      ...
      const newTimeline = [
        ...
        { date: nowDate, event: `Stage changed: ${prevStage} → ${stage}`, type: "stage_change", ...(notes ? { notes } : {}) },
      ];
