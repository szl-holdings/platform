  ...
  router.get(
    ...
    async (req, res) => {
      try {
        ...
        if (distressType)
          conditions.push(eq(terraDistressPropertiesTable.distressType, str(distressType) as any));
        if (minScore)
          conditions.push(
            sql`${terraDistressPropertiesTable.opportunityScore} >= ${Number(minScore)}`,
          );
        if (q) {
          const qStr = str(q)!;
          conditions.push(
        ...
        res.setHeader(
          'Content-Disposition',
          `attachment; filename="terra-distress-export-${new Date().toISOString().slice(0, 10)}.csv"`,
        );
        res.send(csv);
      } catch (err) {
        handleRouteError(res, err, 'Failed to export CSV');
      }
    },
  );
}
