...
export function register(router: IRouter): void {
  router.post(
    '/terra/convert/distress-to-lead',
      ...
      try {
        ...
        const lead: InsertTerraLead = {
          ...
          conversionProbability:
            prop.opportunityScore >= 80 ? '0.75' : prop.opportunityScore >= 60 ? '0.50' : '0.25',
          ...
          notes:
            notes ?? `Converted from distress engine — ${prop.distressType} at ${prop.address}`,
          tags: [prop.distressType, prop.borough, 'distress-converted'],
          timeline: [
            { date: nowStr(), event: 'Converted from Distress Engine', type: 'conversion' },
          ],
        ...
        handleRouteError(res, err, 'Failed to convert distress property to lead');
      }
    },
  );

  router.post(
    '/terra/convert/lead-to-deal',
    authMiddleware({ required: true }),
    validateBody(bodyShape({})),
    async (req, res) => {
      try {
        ...
        sendSuccess(res, {
          ...
          deal: dealRow,
          lead: {
            id: lead.externalId ?? String(lead.id),
            name: `${lead.firstName} ${lead.lastName}`,
          },
          workflowRun: { trigger: 'lead-to-deal', status: 'enqueued', dealId: externalId },
        });
      } catch (err) {
        handleRouteError(res, err, 'Failed to convert lead to deal');
      }
    },
  );
}
