...
export function register(router: IRouter): void {
  router.get(
    ...
    async (req, res) => {
      try {
        ...
        handleRouteError(res, err, 'Failed to fetch leads');
      }
    },
  );
  ...
  router.get('/terra/crm/leads/:id', authMiddleware({ required: false }), async (req, res) => {
    try {
      ...
      if (r.linkedDealId) {
        const dealCheck = await db
          .select({ id: terraDealsTable.id })
      ...
      handleRouteError(res, err, 'Failed to fetch lead');
    }
  });
  ...
  router.post(
    ...
    async (req, res) => {
      try {
        ...
        handleRouteError(res, err, 'Failed to create lead');
      }
    },
  );
    ...
    async (req, res) => {
      try {
        ...
        if (body.addNote || body.timelineEvent) {
          ...
          updates.timeline = [
            ...prevTimeline,
            { date: nowDate, event, type: body.timelineType ?? 'note' },
          ] as any;
        }
        ...
        handleRouteError(res, err, 'Failed to update lead');
      }
    },
  );
}
