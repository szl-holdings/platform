...
router.post(
  ...
  validateBody(connectorCreateSchema),
  async (req, res) => {
    try {
...
router.patch(
  ...
  authMiddleware(),
  requireRole('ops', 'super_admin'),
  validateBody(connectorUpdateSchema),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const logs = await db
        .select()
        .from(connectorLogsTable)
        .where(eq(connectorLogsTable.connectorId, id))
        .orderBy(desc(connectorLogsTable.createdAt));
