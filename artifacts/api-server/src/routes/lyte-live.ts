...
async function getDbTableSizes(): Promise<
  { tableName: string; rowCount: number; sizeKb: number }[]
> {
  try {
  ...
  try {
    ...
    const signals = [
      ...
      },
      {
        ...
        status:
          requestMetrics.totalRequests > 0 &&
          requestMetrics.errorCount / requestMetrics.totalRequests > 0.05
            ? 'degraded'
            : 'healthy',
        category: 'quality',
        source: 'internal-metrics',
      ...
      },
      {
        ...
        unit: 'bytes',
        status:
          memUsage.heapUsed / v8.getHeapStatistics().heap_size_limit > 0.9
            ? 'critical'
            : memUsage.heapUsed / v8.getHeapStatistics().heap_size_limit > 0.75
              ? 'elevated'
              : 'healthy',
        category: 'infrastructure',
        source: 'process',
      ...
      },
      {
        ...
        unit: 'bytes',
        status:
          (totalMem - freeMem) / totalMem > 0.9
            ? 'critical'
            : (totalMem - freeMem) / totalMem > 0.75
              ? 'elevated'
              : 'healthy',
        category: 'infrastructure',
        source: 'os',
...
router.get(
  ...
  async (_req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to fetch database telemetry');
    }
  },
);

router.get(
  '/lyte/live/github-activity',
  authMiddleware({ required: false }),
  ...
  async (req, res) => {
    try {
      ...
      const result = await getLyteCached<any>(
        `lyte-github-${owner}-${repo}`,
        5 * 60 * 1000,
        async () => {
          try {
            if (!repo) {
      ...
      handleRouteError(res, err, 'Failed to fetch GitHub activity');
    }
  },
);
    ...
    try {
      ...
      sendSuccess(res, {
        ...
        memory: {
          ...
          heapTotalMb: +(v8.getHeapStatistics().heap_size_limit / 1024 / 1024).toFixed(1),
          heapUtilizationPct: +(
            (memUsage.heapUsed / v8.getHeapStatistics().heap_size_limit) *
            100
          ).toFixed(1),
          rssMb: +(memUsage.rss / 1024 / 1024).toFixed(1),
      ...
      handleRouteError(res, err, 'Failed to fetch Lyte operations summary');
    }
  },
);
