...
function verifySlackSignature(req: Request): boolean {
  if (!SLACK_SIGNING_SECRET) {
    logger.warn(
      'alloy-channels: SLACK_SIGNING_SECRET not configured — rejecting inbound Slack request',
    );
    return false;
  }
          ...
          if (command) {
            ...
            if (command === '/alloy') {
              ...
              if (requiresApproval) {
                ...
                await postToSlack(
                  channelId,
                  `:hourglass: Alloy approval required for \`${skillRequested}\` skill`,
                  approvalBlocks,
                );
                await logChannelAction({
                  channelType: 'slack',
              ...
              try {
                const aiResult = await services.ai.chatCompletion(
                  [
                    {
                      role: 'system',
                      content: `You are Alloy, an AI operations assistant. Process this ${skillRequested} request from a trusted Slack admin channel and provide a concise, actionable response. Keep it under 500 words.`,
                    },
...
router.post(
  '/alloy/channels/slack/interactive',
  validateBody(
    bodyShape({
            ...
            try {
              const aiResult = await services.ai.chatCompletion(
                [
                  {
                    role: 'system',
                    content: `You are Alloy, an AI operations assistant. Process this approved ${skill} request and provide a concise, actionable response. Keep it under 500 words.`,
                  },
...
router.patch(
  '/alloy/channels/config/:channelId',
  authMiddleware(),
  requireRole('admin', 'super_admin'),
  validateBody(
    bodyShape({
...
router.post(
  '/alloy/channels/approvals/:id/decide',
  authMiddleware(),
  requireRole('ops', 'admin', 'super_admin'),
  validateBody(
    bodyShape({
