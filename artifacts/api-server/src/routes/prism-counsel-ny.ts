...
import { type ForecastType, runAllForecasts, runSingleForecast } from '../lib/ny-forecast-engine';
...
async function assertMatterAccess(
  ...
  if (matter.orgId !== orgId) {
    sendForbidden(res, 'Access denied to this matter');
    return false;
  }
  return true;
}
...
router.get('/prism-counsel/ny/clock-rules', authMiddleware(), async (_req, res) => {
  try {
    const rules = await db
      .select()
      .from(pcClockRulesTable)
      .where(eq(pcClockRulesTable.isActive, true))
      .orderBy(pcClockRulesTable.clockType);
...
router.get('/prism-counsel/ny/rule-profiles', authMiddleware(), async (_req, res) => {
  try {
    const profiles = await db
      .select()
      .from(pcNyRuleProfilesTable)
      .where(eq(pcNyRuleProfilesTable.isActive, true))
      .orderBy(pcNyRuleProfilesTable.category);
...
router.get('/prism-counsel/ny/matters/:matterId/clocks', authMiddleware(), async (req, res) => {
  try {
    ...
    const clocks = await db
      ...
      .orderBy(pcMatterClocksTable.deadlineAt);
...
router.post(
  '/prism-counsel/ny/matters/:matterId/clocks',
  authMiddleware(),
  validateBody(clockCreateSchema),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      if (!orgId) return sendForbidden(res, 'Authentication required');
      const matterId = parseIdParam(req.params.matterId);
      if (!(await assertMatterAccess(matterId, orgId, res))) return;
      const { clockType, startedAt, deadlineAt, ruleRef, notes } = req.body as z.infer<
        typeof clockCreateSchema
      >;
...
router.get('/prism-counsel/ny/clocks/:clockId/events', authMiddleware(), async (req, res) => {
  try {
    ...
    const [clock] = await db
      .select()
      .from(pcMatterClocksTable)
      .where(eq(pcMatterClocksTable.id, clockId));
    if (!clock || clock.orgId !== orgId) return sendForbidden(res, 'Access denied');
    const events = await db
      .select()
      .from(pcClockEventsTable)
      .where(eq(pcClockEventsTable.clockId, clockId))
      .orderBy(desc(pcClockEventsTable.occurredAt));
...
/* ── No-Fault Claims ── */
router.get(
  '/prism-counsel/ny/matters/:matterId/no-fault-claims',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      ...
      const claims = await db
        ...
        .orderBy(desc(pcNoFaultClaimsTable.createdAt));
...
router.post(
  '/prism-counsel/ny/matters/:matterId/no-fault-claims',
  authMiddleware(),
  validateBody(noFaultClaimSchema),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      if (!orgId) return sendForbidden(res, 'Authentication required');
...
router.get(
  '/prism-counsel/ny/matters/:matterId/verifications',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      ...
      const reqs = await db
        ...
        .orderBy(desc(pcVerificationRequestsTable.requestedAt));
...
router.get('/prism-counsel/ny/matters/:matterId/denials', authMiddleware(), async (req, res) => {
  try {
    ...
    const denials = await db
      ...
      .orderBy(desc(pcDenialsTable.deniedAt));
...
router.get('/prism-counsel/ny/matters/:matterId/appeals', authMiddleware(), async (req, res) => {
  try {
    ...
    const appeals = await db
      .select()
      .from(pcAppealsTable)
      .where(eq(pcAppealsTable.matterId, matterId))
      .orderBy(desc(pcAppealsTable.createdAt));
...
router.post(
  '/prism-counsel/ny/matters/:matterId/appeals',
  authMiddleware(),
...
/* ── External Appeals ── */
router.get(
  '/prism-counsel/ny/matters/:matterId/external-appeals',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      ...
      const appeals = await db
        .select()
        .from(pcExternalAppealsTable)
        .where(eq(pcExternalAppealsTable.matterId, matterId))
        .orderBy(desc(pcExternalAppealsTable.createdAt));
      sendSuccess(res, appeals);
    } catch (err) {
...
/* ── Disclaimers ── */
router.get(
  '/prism-counsel/ny/matters/:matterId/disclaimers',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      ...
      const disclaimers = await db
        ...
        .orderBy(desc(pcDisclaimersTable.issuedAt));
...
/* ── Coverage Positions ── */
router.get(
  '/prism-counsel/ny/matters/:matterId/coverage-positions',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      ...
      const positions = await db
        ...
        .orderBy(desc(pcCoveragePositionsTable.positionDate));
...
/* ── Medical Bill Cycles ── */
router.get(
  '/prism-counsel/ny/matters/:matterId/medical-bills',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      ...
      const bills = await db
        .select()
        .from(pcMedicalBillCyclesTable)
        .where(eq(pcMedicalBillCyclesTable.matterId, matterId))
        .orderBy(desc(pcMedicalBillCyclesTable.serviceDate));
...
/* ── Offer Movements ── */
router.get(
  '/prism-counsel/ny/matters/:matterId/offer-movements',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
...
router.post(
  '/prism-counsel/ny/matters/:matterId/offer-movements',
  authMiddleware(),
  validateBody(offerMovementSchema),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      if (!orgId) return sendForbidden(res, 'Authentication required');
      const matterId = parseIdParam(req.params.matterId);
      if (!(await assertMatterAccess(matterId, orgId, res))) return;
      const { offerType, amount, offeredAt, offeringParty, notes } = req.body as z.infer<
        typeof offerMovementSchema
      >;
...
/* ── Reserve Movements ── */
router.get(
  '/prism-counsel/ny/matters/:matterId/reserve-movements',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
...
/* ── Mediation Events ── */
router.get(
  '/prism-counsel/ny/matters/:matterId/mediation-events',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      ...
      const events = await db
        ...
        .orderBy(desc(pcMediationEventsTable.createdAt));
      sendSuccess(res, events);
    } catch (err) {
...
router.get(
  '/prism-counsel/ny/matters/:matterId/communication-windows',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      ...
      const windows = await db
        ...
        .orderBy(desc(pcCommunicationWindowsTable.updatedAt));
...
/* ── Demand Packets ── */
router.get(
  '/prism-counsel/ny/matters/:matterId/demand-packets',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
...
router.post(
  '/prism-counsel/ny/matters/:matterId/demand-packets',
  authMiddleware(),
  validateBody(demandPacketSchema),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      if (!orgId) return sendForbidden(res, 'Authentication required');
...
router.get(
  '/prism-counsel/ny/matters/:matterId/readiness-snapshots',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
...
router.get('/prism-counsel/ny/venue-profiles', authMiddleware(), async (_req, res) => {
  try {
    const venues = await db
      .select()
      .from(pcVenueProfilesTable)
      .orderBy(pcVenueProfilesTable.county);
...
router.get('/prism-counsel/ny/venue-profiles/:id', authMiddleware(), async (req, res) => {
  try {
    ...
    const [venue] = await db
      .select()
      .from(pcVenueProfilesTable)
      .where(eq(pcVenueProfilesTable.id, id));
    if (!venue) return sendNotFound(res, 'Venue profile not found');
    const parts = await db
      .select()
      .from(pcPartProfilesTable)
      .where(eq(pcPartProfilesTable.venueId, id));
...
router.get('/prism-counsel/ny/insurer-profiles', authMiddleware(), async (req, res) => {
  try {
    ...
    const insurers = await db
      ...
      .orderBy(pcInsurerProfilesTable.carrierName);
...
router.get('/prism-counsel/ny/insurer-profiles/:id', authMiddleware(), async (req, res) => {
  try {
    ...
    const [insurer] = await db
      .select()
      .from(pcInsurerProfilesTable)
      .where(and(eq(pcInsurerProfilesTable.id, id), eq(pcInsurerProfilesTable.orgId, orgId)));
    if (!insurer) return sendNotFound(res, 'Insurer profile not found');
    const adjusters = await db
      .select()
      .from(pcAdjusterProfilesTable)
      .where(eq(pcAdjusterProfilesTable.insurerProfileId, id));
...
/* ── Forecast Engine ── */
router.get(
  '/prism-counsel/ny/matters/:matterId/forecasts',
  authMiddleware(),
    ...
    try {
      ...
      const conditions = type
        ? and(
            ...
            eq(pcForecastRunsTable.forecastType, type as ForecastType),
          )
        : eq(pcForecastRunsTable.matterId, matterId);
...
router.post(
  '/prism-counsel/ny/matters/:matterId/forecasts/compute',
  authMiddleware(),
  validateBody(forecastComputeSchema),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      if (!orgId) return sendForbidden(res, 'Authentication required');
...
router.get(
  '/prism-counsel/ny/matters/:matterId/forecasts/latest',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
        ...
        FORECAST_TYPES.map((ft) =>
          db
            ...
            .where(
              and(
                eq(pcForecastRunsTable.matterId, matterId),
                eq(pcForecastRunsTable.forecastType, ft),
              ),
            )
            .orderBy(desc(pcForecastRunsTable.runAt))
            .limit(1)
...
router.get('/prism-counsel/ny/matters/:matterId/ai-reviews', authMiddleware(), async (req, res) => {
  try {
    ...
    const reviews = await db
      .select()
      .from(pcAiReviewPacketsTable)
      .where(
        and(eq(pcAiReviewPacketsTable.matterId, matterId), eq(pcAiReviewPacketsTable.orgId, orgId)),
      )
      .orderBy(desc(pcAiReviewPacketsTable.createdAt));
...
router.patch(
  '/prism-counsel/ny/ai-reviews/:id/approve',
  authMiddleware(),
  ...
  async (req, res) => {
    try {
      ...
      const [review] = await db
        .select()
        .from(pcAiReviewPacketsTable)
        .where(and(eq(pcAiReviewPacketsTable.id, id), eq(pcAiReviewPacketsTable.orgId, orgId)));
      if (!review) return sendNotFound(res, 'AI review packet not found');
      const [updated] = await db
        .update(pcAiReviewPacketsTable)
        .set({ status: 'approved', approvedBy: req.user?.id, approvedAt: new Date() })
        .where(eq(pcAiReviewPacketsTable.id, id))
        .returning();
...
router.get(
  '/prism-counsel/ny/matters/:matterId/defensibility',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      ...
      const [score] = await db
        .select()
        .from(pcDefensibilityScoresTable)
        .where(
          and(
            eq(pcDefensibilityScoresTable.matterId, matterId),
            eq(pcDefensibilityScoresTable.orgId, orgId),
          ),
        )
        .orderBy(desc(pcDefensibilityScoresTable.computedAt))
        .limit(1);
...
router.get(
  '/prism-counsel/ny/matters/:matterId/demand-readiness',
  authMiddleware(),
  async (req, res) => {
    try {
      const orgId = getAuthOrgId(req);
      ...
      const [snap] = await db
        ...
        .where(
          and(
            eq(pcDemandReadinessSnapshotsTable.matterId, matterId),
            eq(pcDemandReadinessSnapshotsTable.orgId, orgId),
          ),
        )
        .orderBy(desc(pcDemandReadinessSnapshotsTable.computedAt))
        .limit(1);
...
router.get('/prism-counsel/ny/matters/:matterId/offers', authMiddleware(), async (req, res) => {
  try {
    ...
    const offers = await db
      ...
      .from(pcOfferMovementsTable)
      .where(
        and(eq(pcOfferMovementsTable.matterId, matterId), eq(pcOfferMovementsTable.orgId, orgId)),
      )
      .orderBy(pcOfferMovementsTable.offeredAt);
    sendSuccess(res, offers);
  } catch (err) {
...
router.get('/prism-counsel/ny/matters/:matterId/mediations', authMiddleware(), async (req, res) => {
  try {
    ...
    const evts = await db
      ...
      .from(pcMediationEventsTable)
      .where(
        and(eq(pcMediationEventsTable.matterId, matterId), eq(pcMediationEventsTable.orgId, orgId)),
      )
      .orderBy(pcMediationEventsTable.scheduledAt);
...
router.get('/prism-counsel/ny/dashboard', authMiddleware(), async (req, res) => {
  try {
    const orgId = getAuthOrgId(req);
    if (!orgId) return sendForbidden(res, 'Authentication required');
    const [[{ activeMatters }], [{ criticalClocks }], [{ breachedClocks }], [{ pendingAppeals }]] =
      await Promise.all([
        db
          .select({ activeMatters: sql<number>`count(*)::int` })
          .from(pcMattersTable)
          .where(eq(pcMattersTable.orgId, orgId)),
        db
          .select({ criticalClocks: sql<number>`count(*)::int` })
          .from(pcMatterClocksTable)
          .where(
            and(eq(pcMatterClocksTable.orgId, orgId), eq(pcMatterClocksTable.status, 'running')),
          ),
        db
          .select({ breachedClocks: sql<number>`count(*)::int` })
          .from(pcMatterClocksTable)
          .where(
            and(eq(pcMatterClocksTable.orgId, orgId), eq(pcMatterClocksTable.isBreached, true)),
          ),
        db
          .select({ pendingAppeals: sql<number>`count(*)::int` })
          .from(pcAppealsTable)
          .where(and(eq(pcAppealsTable.orgId, orgId), eq(pcAppealsTable.status, 'pending'))),
      ]);
