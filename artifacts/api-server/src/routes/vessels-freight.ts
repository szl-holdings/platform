...
const ROUTE_BASELINES = [
  { route: "C5 — W. Australia → China (Iron Ore)", classKey: "capesize", baseRate: 11.2, unit: "$/MT", volatility: 0.08 },
  { route: "C3 — Tubarão → Qingdao (Iron Ore)", classKey: "capesize", baseRate: 28.1, unit: "$/MT", volatility: 0.07 },
  { route: "P1A — Transatlantic Round", classKey: "panamax", baseRate: 16280, unit: "$/day", volatility: 0.06 },
  { route: "P2A — Fronthaul (AMS → FEast)", classKey: "panamax", baseRate: 22400, unit: "$/day", volatility: 0.06 },
  { route: "S1C — USG → Far East", classKey: "supramax", baseRate: 24800, unit: "$/day", volatility: 0.05 },
  { route: "S5 — USG → Cont/Med", classKey: "supramax", baseRate: 18200, unit: "$/day", volatility: 0.05 },
  { route: "HS1 — Cont → Far East (MV)", classKey: "handysize", baseRate: 13800, unit: "$/day", volatility: 0.05 },
  { route: "HS3 — USG → Far East", classKey: "handysize", baseRate: 14900, unit: "$/day", volatility: 0.05 },
];
...
function computeSnapshot(now: Date) {
  ...
  const tHours = Math.floor(now.getTime() / REFRESH_INTERVAL_MS) * (REFRESH_INTERVAL_MS / 3_600_000);
  ...
  const benchmarks = CLASS_BASELINES.map((c, idx) => {
    ...
    const fleetAvg = Math.round(tce * (0.94 + Math.abs(deterministicWiggle(seed + 11, tHours)) * 0.04));
    const topQuartile = Math.round(tce * (1.13 + Math.abs(deterministicWiggle(seed + 23, tHours)) * 0.03));
    const bottomQuartile = Math.round(tce * (0.81 + Math.abs(deterministicWiggle(seed + 41, tHours)) * 0.03));
  ...
  const routes = ROUTE_BASELINES.map((r, idx) => {
    ...
    const ratePrev = +(r.baseRate * (1 + driftPrev * r.volatility)).toFixed(r.unit === "$/MT" ? 1 : 0);
...
function buildPayload(): SnapshotPayload {
  ...
  const payload: SnapshotPayload = {
    ...
    methodology: "Time-banded TCE drift seeded from Baltic Exchange published baselines; refreshed on a fixed cadence so all surfaces share the same snapshot.",
