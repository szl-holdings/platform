...
router.post("/continuum/ingest/signal", authMiddleware(), validateBody(alloyIngestSignalSchema), async (req, res) => {
  try {
...
router.post("/continuum/ingest/batch", authMiddleware(), requireRole("super_admin", "ops", "analyst"), validateBody(alloyIngestBatchSchema), async (req, res) => {
  try {
...
router.get("/continuum/workflows", authMiddleware(), validateQuery(listQuerySchema), async (req, res) => {
  try {
...
router.get("/continuum/workflows/:id", authMiddleware(), async (req, res) => {
  try {
...
router.post("/continuum/workflows", authMiddleware(), requireRole("super_admin", "ops", "analyst"), validateBody(alloyWorkflowMutationSchema), async (req, res) => {
  try {
...
router.patch("/continuum/workflows/:id", authMiddleware(), requireRole("super_admin", "ops", "analyst"), validateBody(patchWorkflowSchema), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    const orgIds = callerOrgFilter(req.user);
    if (orgIds !== null && orgIds.length === 0) { sendForbidden(res, "No organization membership"); return; }
...
router.delete("/continuum/workflows/:id", validateBody(alloyWorkflowDeleteSchema), authMiddleware(), requireRole("super_admin", "ops"), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    const orgIds = callerOrgFilter(req.user);
    if (orgIds !== null && orgIds.length === 0) { sendForbidden(res, "No organization membership"); return; }
...
router.post("/continuum/workflows/:id/run", authMiddleware(), validateBody(workflowRunSchema), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    const orgIds = callerOrgFilter(req.user);
    if (orgIds !== null && orgIds.length === 0) { sendForbidden(res, "No organization membership"); return; }
...
router.get("/continuum/runs/:id", authMiddleware(), requireRole("super_admin", "admin", "ops"), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
...
router.post("/continuum/runs/:id/retry", authMiddleware(), requireRole("super_admin", "ops"), validateBody(alloyRunActionSchema), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
...
router.post("/continuum/runs/:id/cancel", authMiddleware(), requireRole("super_admin", "ops"), validateBody(alloyRunActionSchema), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    const [run] = await db.select().from(alloyWorkflowRunsTable).where(eq(alloyWorkflowRunsTable.id, id));
    if (!run) { sendNotFound(res, "Run"); return; }
...
router.get("/continuum/artifacts", authMiddleware(), validateQuery(listQuerySchema), async (req, res) => {
  try {
    const { page, limit, offset } = parsePagination(req.query as Record<string, unknown>);
    const orgIds = getUserOrgIds(req.user);
...
router.get("/continuum/artifacts/:id", authMiddleware(), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    const orgIds = callerOrgFilter(req.user);
    if (orgIds !== null && orgIds.length === 0) { sendForbidden(res, "No organization membership"); return; }
...
router.post("/continuum/artifacts/:id/approve", authMiddleware(), requireRole("super_admin", "ops", "compliance"), validateBody(artifactApproveSchema), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    const orgIds = callerOrgFilter(req.user);
    if (orgIds !== null && orgIds.length === 0) { sendForbidden(res, "No organization membership"); return; }
...
router.post("/continuum/artifacts/:id/reject", authMiddleware(), requireRole("super_admin", "ops", "compliance"), validateBody(artifactRejectSchema), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    const orgIds = callerOrgFilter(req.user);
    if (orgIds !== null && orgIds.length === 0) { sendForbidden(res, "No organization membership"); return; }
...
router.get("/continuum/admin/flags", authMiddleware(), requireRole("super_admin", "ops"), async (_req, res) => {
  try {
...
router.post("/continuum/admin/flags", authMiddleware(), requireRole("super_admin", "ops"), validateBody(upsertFeatureFlagSchema), async (req, res) => {
  try {
...
router.patch("/continuum/admin/flags/:key", authMiddleware(), requireRole("super_admin", "ops"), validateBody(patchFeatureFlagSchema), async (req, res) => {
  try {
...
router.get("/continuum/audit", authMiddleware(), requireRole("super_admin", "ops", "compliance"), validateQuery(listQuerySchema), async (req, res) => {
  try {
    const { page, limit, offset } = parsePagination(req.query as Record<string, unknown>);
    const orgIds = getUserOrgIds(req.user);
    const isAdmin = isGlobalAdmin(req.user);
...
router.get("/continuum/factory-floor", authMiddleware(), async (req, res) => {
  try {
    const orgIds = getUserOrgIds(req.user);
    const isAdmin = isGlobalAdmin(req.user);
    if (!isAdmin && orgIds.length === 0) {
...
router.get("/continuum/signals", authMiddleware(), validateQuery(listQuerySchema), async (req, res) => {
  try {
    const { page, limit, offset } = parsePagination(req.query as Record<string, unknown>);
...
router.get("/continuum/runs/:id/steps", authMiddleware(), requireRole("super_admin", "admin", "ops"), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    const [run] = await db.select().from(alloyWorkflowRunsTable).where(eq(alloyWorkflowRunsTable.id, id));
    if (!run) { sendNotFound(res, "Run"); return; }
...
router.get("/continuum/runs", authMiddleware(), requireRole("super_admin", "admin", "ops"), validateQuery(listQuerySchema), async (req, res) => {
  try {
    const { page, limit, offset } = parsePagination(req.query as Record<string, unknown>);
    ...
    const runtimeSessions = await import("@szl/continuum")
      .then(m => {
...
router.get("/continuum/approvals", authMiddleware(), validateQuery(listQuerySchema), async (req, res) => {
  try {
    const { page, limit, offset } = parsePagination(req.query as Record<string, unknown>);
    const orgIds = getUserOrgIds(req.user);
    const isAdmin = isGlobalAdmin(req.user);
...
router.post("/continuum/approvals/:id/decide", authMiddleware(), requireRole("super_admin", "admin", "ops", "compliance"), validateBody(approvalDecisionSchema), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
...
router.get("/continuum/dashboard", authMiddleware(), async (req, res) => {
  try {
    const orgIds = getUserOrgIds(req.user);
    const isAdmin = isGlobalAdmin(req.user);
    if (!isAdmin && orgIds.length === 0) {
    ...
    handleRouteError(res, err, "Failed to build Continuum dashboard");
  }
});
...
router.get("/continuum/evidence", authMiddleware(), validateQuery(listQuerySchema), async (req: Request, res: Response) => {
  try {
    ...
    const { listEvidence } = await import("@szl/continuum/evidence");
...
router.post("/continuum/evidence", authMiddleware(), validateBody(evidenceCreateSchema), async (req: Request, res: Response) => {
  try {
    const tenantOrgId = requireAlloyTenant(req, res);
    if (tenantOrgId === null) return;
    const { createEvidence } = await import("@szl/continuum/evidence");
    ...
    logger.info({ evidenceId: ev.id, kind: ev.kind, tenantOrgId }, "Evidence created via Continuum");
...
router.get("/continuum/evidence/:id", authMiddleware(), async (req: Request, res: Response) => {
  try {
    const tenantOrgId = requireAlloyTenant(req, res);
    if (tenantOrgId === null) return;
    const { getEvidence } = await import("@szl/continuum/evidence");
...
router.post("/continuum/policy/simulate", authMiddleware(), validateBody(policySimulateSchema), async (req: Request, res: Response) => {
  try {
    const tenantOrgId = requireAlloyTenant(req, res);
    if (tenantOrgId === null) return;
    const { checkAction } = await import("@szl/continuum");
...
router.post("/continuum/recommend", authMiddleware(), validateBody(recommendBodySchema), async (req: Request, res: Response) => {
  try {
    const tenantOrgId = requireAlloyTenant(req, res);
    if (tenantOrgId === null) return;
    ...
    // PATCH /continuum/autonomy-mode wins over the per-call autonomyMode hint
    ...
    if (decision.disposition === "block") {
      logger.info(
        ...
        "continuum.recommend.blocked-by-autonomy-mode",
      );
    ...
    const { recommend: alloyRecommend } = await import("@szl/continuum");
    ...
    // Without this, /continuum/recommend would silently return a recommendation
      ...
      try {
        ...
        if (decision.disposition === "draft") {
          try {
            ...
            logger.warn({ err, recommendationId: result.id }, "continuum.recommend.draft-artifact-insert-failed");
          }
        }
        ...
        approval = await createApprovalRequest({
          ...
          serviceAttribution: "continuum.recommend",
          payload: {
            recommendationId: result.id,
            runId: result.runId,
            traceId: result.traceId,
        ...
        logger.info(
          ...
          "continuum.recommend.approval-queued",
        );
      } catch (err) {
        logger.warn(
          { err, recommendationId: result.id, tenantOrgId },
          "continuum.recommend.approval-create-failed",
        );
      }
    }

    logger.info({ recommendationId: result.id, domain: result.domain, confidence: result.confidence, tenantOrgId }, "Recommendation generated via Continuum");
...
router.get("/continuum/autonomy-mode", authMiddleware(), validateQuery(autonomyModeQuerySchema), async (req: Request, res: Response) => {
  try {
    const tenantOrgId = resolveAlloyTenant(req);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        ...
        "continuum.autonomy-mode.updated",
      );
 ...
 */
router.post(
  "/continuum/autonomy-mode/evaluate",
  authMiddleware(),
  validateBody(alloyResourceBodySchema),
  async (req: Request, res: Response) => {
    try {
