...
import { db, organizationsTable, orgMembersTable, usageEventsTable, usersTable } from "@szl-holdings/db";
  ...
  router.get("/admin/usage", readLimiter, validateQuery(listQuerySchema), async (req: Request, res: Response) => {
    try {
      ...
      if (plan && ["free", "starter", "professional", "enterprise"].includes(plan)) {
        orgFilters.push(eq(organizationsTable.plan, plan as "free" | "starter" | "professional" | "enterprise"));
      }
      ...
      const overCount = rows.filter((r) => Object.values(r.overages).some((v) => v === "over")).length;
      const warnCount = rows.filter((r) => Object.values(r.overages).some((v) => v === "warn")).length;
