...
export const integrationActivityLog: IntegrationActivity[] = [
  ...
  },
  {
    ...
    app: 'readiness',
    status: 'success',
    message: 'Fetched 12 assessment pages',
  ...
  },
  {
    ...
    app: 'control-plane',
    status: 'success',
    message: 'All monitoring endpoints healthy',
  ...
  },
  {
    ...
    app: 'dreamscape',
    status: 'success',
    message: 'Generated audio narration',
  ...
  },
  {
    ...
    app: 'stephen-site',
    status: 'success',
    message: 'push event on main branch',
  ...
  },
  {
    ...
    app: 'firestorm',
    status: 'error',
    message: 'SMS delivery failed - invalid number',
...
const PROVISIONING_DOCS: Record<string, { signup: string; docsUrl: string; notes?: string }> = {
  'marine-traffic': {
    signup: 'https://www.marinetraffic.com/en/p/api-services',
    docsUrl: 'https://www.marinetraffic.com/en/ais-api-services',
    notes: 'Requires commercial API plan',
  },
  'vessel-finder': {
    signup: 'https://api.vesselfinder.com/',
    docsUrl: 'https://api.vesselfinder.com/docs',
    notes: 'Free tier available for development',
  },
  zillow: {
    signup: 'https://www.zillow.com/howzillow/zillowgroup/tech/',
    docsUrl: 'https://bridgeinteractive.github.io/zillow-swagger/',
    notes: 'Contact Zillow Bridge API team for access',
  },
  corelogic: {
    signup: 'https://developer.corelogic.com/',
    docsUrl: 'https://developer.corelogic.com/documentation',
    notes: 'Enterprise only — requires sales approval',
  },
  redfin: {
    signup: 'https://www.redfin.com/news/data-center/',
    docsUrl: 'https://redfin.com/stingray/api/gis-csv',
    notes: 'Limited public API; contact for commercial use',
  },
  pacer: {
    signup: 'https://pacer.uscourts.gov/register-account',
    docsUrl: 'https://pacer.uscourts.gov/file-case/developer-resources',
    notes: 'US federal court account required',
  },
  'court-listener': {
    signup: 'https://www.courtlistener.com/sign-in/?next=/',
    docsUrl: 'https://www.courtlistener.com/help/api/rest/',
    notes: 'Free non-commercial tier available',
  },
  virustotal: {
    signup: 'https://www.virustotal.com/gui/join-us',
    docsUrl: 'https://developers.virustotal.com/reference',
    notes: 'Free tier: 4 lookups/min, 500/day',
  },
  shodan: {
    signup: 'https://account.shodan.io/register',
    docsUrl: 'https://developer.shodan.io/api',
    notes: 'Membership required for full scanning',
  },
  'alien-vault-otx': {
    signup: 'https://otx.alienvault.com/',
    docsUrl: 'https://otx.alienvault.com/api',
    notes: 'Free; requires OTX account',
  },
  'alpha-vantage': {
  ...
  'dun-bradstreet': {
    signup: 'https://developer.dnb.com/',
    docsUrl: 'https://developer.dnb.com/docs',
    notes: 'Requires commercial contract for production',
  },
  crunchbase: {
    signup: 'https://data.crunchbase.com/docs/getting-started',
    docsUrl: 'https://data.crunchbase.com/reference',
    notes: 'Free trial available; basic/pro/enterprise tiers',
  },
};
...
export function register(router: IRouter): void {
  ...
  router.get('/admin/integration-health', (_req, res) => {
    const matrix = services.getHealthMatrix();
    const appHealth: Record<
      string,
      {
        slug: string;
        name: string;
        connectors: string[];
        health: ReturnType<typeof services.getHealthMatrix>;
      }
    > = {};
