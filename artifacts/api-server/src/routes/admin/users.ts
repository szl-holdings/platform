...
import { z } from 'zod';
import { sendBadRequest, sendError, sendForbidden, sendNotFound } from '../../lib/api-response.js';
...
export function register(router: IRouter): void {
  ...
  router.patch('/admin/users/:id/role', validateBody(roleAssignSchema), async (req, res) => {
    try {
      ...
      await db.insert(auditEventsTable).values({
        userId: req.user?.id ?? null,
        action: action === 'add' ? 'user.role.assigned' : 'user.role.removed',
        entityType: 'user',
        entityId: String(userId),
        newValues: { roleName: role.name, roleId, targetUserEmail: targetUser.email, action },
        ipAddress: hashIp(req.ip ?? null),
        userAgent: req.headers['user-agent'] ?? null,
      });
  ...
  router.post(
    ...
    requireRole('admin'),
    validateBody(impersonateEndSchema),
    async (req, res) => {
      try {
    ...
    async (req, res) => {
      try {
        ...
        const roleRows = await db
          .select()
          .from(rolesTable)
          .where(inArray(rolesTable.name, roles as RoleName[]));
        ...
        await db.transaction(async (tx) => {
          ...
          for (const role of roleRows) {
            await tx
              ...
              .values({ userId: targetUserId, roleId: role.id })
              .onConflictDoNothing();
          }
