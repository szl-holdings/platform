...
function getOrgId(req: Request): number {
  ...
  if (process.env.NODE_ENV === 'production') {
    throw Object.assign(new Error('Organization membership required to access this resource'), {
      statusCode: 403,
    });
  }
...
router.get(
  '/court/filings/recent',
  authMiddleware(),
  courtListenerLimiter,
...
router.post(
  '/court/matters/:matterId/link-docket',
  authMiddleware(),
  validateBody(docketLinkSchema),
  async (req, res) => {
    try {
      ...
      try {
        const [matterRow] = await db
          .select({ tags: pcMattersTable.tags })
          .from(pcMattersTable)
          .where(eq(pcMattersTable.id, matterId))
          .limit(1);
        interface LinkedDocketEntry {
          docketId: string;
          courtId?: string | null;
        ...
        const existingDockets = Array.isArray(existingTags.linkedDockets)
          ? (existingTags.linkedDockets as LinkedDocketEntry[])
          : [];
        const alreadyLinked = existingDockets.some((d) => d.docketId === String(docketId));
        if (!alreadyLinked) {
          existingDockets.push({
          ...
          await db
            ...
            .set({
              tags: { ...existingTags, linkedDockets: existingDockets },
              updatedAt: new Date(),
            })
            .where(eq(pcMattersTable.id, matterId));
        }
...
function hasPartnerRole(req: Request): boolean {
  const roles = req.user?.roles ?? [];
  return (
    roles.some((r) => ['partner', 'super_admin', 'admin'].includes(r)) ||
    (roles.length === 0 && process.env.NODE_ENV !== 'production')
  );
}
...
router.post(
  '/privilege/classify',
  authMiddleware(),
  validateBody(privilegeClassifySchema),
  async (req, res) => {
    try {
      if (!hasAttorneyRole(req)) return sendForbidden(res, 'Attorney role required');
      const { content, authorRole, recipientRoles, subject } = req.body as z.infer<
        typeof privilegeClassifySchema
      >;
      ...
      const engineResult = await privilegeEngine.classifyContent(content, {
        authorRole,
        recipientRoles,
        subject,
      });
...
router.post(
  '/privilege/tag',
  authMiddleware(),
  validateBody(privilegeTagSchema),
  async (req, res) => {
    try {
      if (!hasAttorneyRole(req)) return sendForbidden(res, 'Attorney role required');
      ...
      if (privilegeType === 'none') {
        return sendBadRequest(
          res,
          "Cannot tag with privilege type 'none' — use classify to assess",
        );
      }
...
router.post(
  '/privilege/review/:tagId/resolve',
  authMiddleware(),
  validateBody(privilegeResolveSchema),
  async (req, res) => {
    try {
      if (!hasAttorneyRole(req)) return sendForbidden(res, 'Attorney role required');
...
router.post(
  '/privilege/clawback/:tagId',
  authMiddleware(),
  validateBody(privilegeClawbackSchema),
  async (req, res) => {
    try {
      if (!hasAttorneyRole(req)) return sendForbidden(res, 'Attorney role required');
...
router.post(
  '/privilege/export-check',
  authMiddleware(),
  validateBody(privilegeExportCheckSchema),
  async (req, res) => {
    try {
      if (!hasAttorneyRole(req)) return sendForbidden(res, 'Attorney role required');
...
router.post(
  ...
  authMiddleware(),
  validateBody(draftGenerateSchema),
  async (req, res) => {
    try {
      if (!hasAttorneyRole(req)) return sendForbidden(res, 'Attorney role required');

      const { draftType, matterId, groundingContext } = req.body as z.infer<
        typeof draftGenerateSchema
      >;
      ...
      const draftContent =
        modelResult?.output?.response ?? buildFallbackDraft(draftType, groundingContext ?? {});
...
router.post(
  '/copilot/drafts/:draftId/advance',
  authMiddleware(),
  validateBody(draftAdvanceSchema),
  async (req, res) => {
    try {
      if (!hasAttorneyRole(req)) return sendForbidden(res, 'Attorney role required');
      ...
      const [draftRecord] = await db
        .select({
          id: pcApprovalRequestsTable.id,
          sourceBasis: pcApprovalRequestsTable.sourceBasis,
        })
      ...
      if (!expectedNext) {
        return sendBadRequest(
          res,
          `Draft is in terminal state '${storedState}' and cannot be advanced`,
        );
      }
      if (toState !== expectedNext) {
        return sendBadRequest(
          res,
          `Invalid transition from stored state '${storedState}': expected '${expectedNext}', got '${toState}'`,
        );
      }
      ...
      await db
        ...
        .set({
          status: dbStatus,
          approvedBy: actorId,
          resolvedAt: advancedAt,
          sourceBasis: updatedBasis,
        })
      ...
      await emitCopilotAudit({
        ...
        details: {
          ...
          versionSnapshot: {
            ...
            by: actorId ?? null,
            ...(toState === 'final'
              ? { signedOffBy: actorId ?? null, signedOffAt: advancedAt.toISOString() }
              : {}),
          },
        },
      });
