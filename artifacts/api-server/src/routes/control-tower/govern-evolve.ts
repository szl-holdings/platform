  ...
  (req: Request, res: Response) => {
    try {
      ...
      sendSuccess(res, {
        layer: 'govern',
        entries,
        total: getKernelAuditTrail().length,
        filteredCount: entries.length,
        integrity: verifyAuditChainIntegrity(),
      });
    } catch (err) {
...
router.get(
  '/control-tower/govern/certificates',
    ...
    try {
      ...
      try {
        ...
        ragResults = ragHits.map((hit) => ({
          ...
          metadata: {
            ...
            sensitivity: hit.sensitivityLevel,
      ...
      try {
        ...
        decisionResults = decisions
          .filter((d) => {
            const text =
              `${d.recommendedAction} ${d.rationaleSummary} ${d.rawInput ?? ''}`.toLowerCase();
...
router.get('/control-tower/evolve/metrics', async (_req: Request, res: Response) => {
  try {
    ...
    try {
      const { total: totalDb } = await listDecisions({
        limit: 1,
        offset: 0,
        orgId: null,
        isAdmin: true,
      });
      const { total: approvedDb } = await listDecisions({
        limit: 1,
        offset: 0,
        status: 'approved',
        orgId: null,
      ...
      const { total: proposedDb } = await listDecisions({
        limit: 1,
        offset: 0,
        status: 'proposed',
        orgId: null,
        ...
        metrics: {
          ...
          successRate:
            auditEntries.length > 0
              ? parseFloat(
                  (
                    auditEntries.filter((e) => e.executionResult === 'success').length /
                    auditEntries.length
                  ).toFixed(3),
                )
              : null,
    ...
    sendSuccess(res, {
      ...
      systemMetrics: {
        ...
        avgLatencyMs:
          providerStats.reduce((s, p) => s + p.avgLatencyMs, 0) / Math.max(1, providerStats.length),
  ...
  (req: Request, res: Response) => {
    try {
      ...
      sendSuccess(res, {
        ...
        proposal,
        agentId,
        totalProposals: perf.proposedOptimizations.length,
      });
    } catch (err) {
...
router.patch(
  '/control-tower/evolve/propose/:proposalId',
  requireRole('super_admin', 'ops', 'exec'),
  validateBody(
    bodyShape({
...
router.get('/control-tower/status', async (_req: Request, res: Response) => {
  try {
    ...
    const agentHealthSummary = buildAgentRegistryWithHealth().reduce(
      (acc, a) => {
        acc[a.health.status] = (acc[a.health.status] ?? 0) + 1;
        return acc;
      },
