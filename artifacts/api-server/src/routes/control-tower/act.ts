...
import {
  executeComposedPipeline,
  executePipeline,
  getPipelineConfig,
  listPipelines,
} from '../../lib/intelligence-pipelines';
import { logger } from '../../lib/logger';
import { validateBody } from '../../lib/validation';
import { requireRole } from '../../middlewares/auth';
import { evaluatePolicies, makeEvidenceRef, toRiskLevel } from './shared';
...
router.post(
  '/control-tower/act/pipelines/:id/run',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        { pipelineId: id, inputLength: input.length },
        'Control Tower — pipeline execution',
      );
      ...
      sendSuccess(res, {
        ...
        result,
        decisionId,
        governanceCheck: { requiresApproval: govCheck.requiresApproval },
      });
    } catch (err) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (invalidStage) {
        sendBadRequest(
          res,
          `Unknown stage type: ${invalidStage.type}. Valid types: ${validStageTypes.join(', ')}`,
        );
        return;
      }
      ...
      try {
        const confidence =
          result.status === 'completed' ? 0.85 : result.status === 'partial' ? 0.5 : 0.2;
        await insertDecision({
          decisionId,
        ...
        logger.warn(
          { err: dbErr },
          'control-tower: failed to persist composed pipeline decision to DB',
        );
      }
