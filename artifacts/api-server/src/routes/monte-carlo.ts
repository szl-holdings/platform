...
function parsePositiveInt(value: unknown, defaultVal: number | undefined, min: number, max: number): number | null {
  ...
  try {
    ...
    sendSuccess(res, {
      ...
      inputs: scenario.inputs.map((i) => ({
        id: i.id, label: i.label, description: i.description, unit: i.unit, format: i.format, distribution: i.distribution,
      })),
...
router.post("/monte-carlo/simulate/custom", simulationLimiter, authMiddleware(), (req, res) => {
  try {
    ...
    if (parsedIterations === null) { sendBadRequest(res, "iterations must be a positive integer between 1,000 and 50,000"); return; }
    const parsedBatchSize = parsePositiveInt(batchSize, 500, 100, 2_000);
    if (parsedBatchSize === null) { sendBadRequest(res, "batchSize must be a positive integer between 100 and 2,000"); return; }
      ...
      result: job.result
        ? {
            ...
            results: Object.fromEntries(
              Object.entries(job.result.results).map(([k, v]) => [k, {
                metric: v.metric,
                stats: v.stats,
...
router.get("/monte-carlo/jobs/:id/stream", authMiddleware(), (req, res: Response) => {
  ...
  const isAdmin = req.user?.roles?.some((r: string) => r === "founder_admin" || r === "platform_admin" || r === "super_admin" || r === "admin") ?? false;
  ...
  return {
    ...
    result: job.result
      ? {
          ...
          durationMs: job.result.durationMs,
          results: Object.fromEntries(
            Object.entries(job.result.results).map(([k, v]) => [k, { stats: v.stats, histogram: v.histogram }])
          ),
        }
      : null,
...
router.post("/monte-carlo/calibrate", authMiddleware(), (req, res) => {
  try {
    ...
    if (!scenarioId) { sendBadRequest(res, "scenarioId is required"); return; }
    if (!simulationJobId) { sendBadRequest(res, "simulationJobId is required"); return; }

    const job = getJob(simulationJobId);
    if (!job) { sendNotFound(res, "Simulation job"); return; }
    if (!canAccessJob(job, req)) { res.status(403).json({ error: "Forbidden" }); return; }
    if (job.status !== "complete" || !job.result) { sendBadRequest(res, "Simulation job is not complete"); return; }
...
router.post("/monte-carlo/backtest", authMiddleware(), (req, res) => {
  try {
    ...
    if (!scenarioId) { sendBadRequest(res, "scenarioId is required"); return; }
    if (!outputId) { sendBadRequest(res, "outputId is required"); return; }
    if (!simulationJobId) { sendBadRequest(res, "simulationJobId is required"); return; }
  ...
  sendSuccess(res, {
    executionModel: {
      builtinScenarios: {
        ...
        description: "Built-in scenario simulations run in-process using an async batching loop. The event loop is yielded between each batch (setTimeout 0) to avoid blocking the server. Sensitivity OAT sweeps run synchronously after the main simulation.",
      ...
      customDslScenarios: {
        ...
        description: "Custom DSL scenario simulations distribute work across up to 4 Node.js Worker threads. Each worker receives an equal chunk of iterations and evaluates the DSL expression tree. Workers emit progress messages at 10% intervals.",
        recommendedIterationLimit: 100_000,
        hardLimit: 100_000,
        parallelism: "up to 4 worker threads (scales with CPU, capped at min(4, ceil(iterations/2000)))",
    ...
    sensitivityAnalysis: {
      ...
      samplesPerVariable: "default 200, configurable via sensitivitySamples config param (clamped 50–1000)",
      description: "For each input, runs samplesPerVariable independent evaluations sampling only that input while holding all others at their expected value. P10/P90 of the resulting output distribution provides tornado low/high bars. Pearson correlation from the main simulation run provides impact percentages.",
    },
  });
});
...
router.get("/monte-carlo/schema", authMiddleware({ required: false }), (_req, res) => {
  sendSuccess(res, {
    ...
    description: "Monte Carlo DSL scenario definition — submittable to POST /monte-carlo/simulate/custom",
    type: "object",
    ...
    properties: {
      ...
      domain:      { type: "string", minLength: 1, description: "One of: vessels, terra, szl, prism, aegis, nexus, lyte" },
      tags:        { type: "array", items: { type: "string" } },
      inputs: {
        type: "array", minItems: 1,
        items: {
          type: "object", required: ["id", "distribution"],
          properties: {
            ...
            distribution: {
              ...
              properties: {
                type: { type: "string", enum: ["normal","log_normal","uniform","triangular","beta","poisson","constant","custom"] },
                mean:   { type: "number" }, stdDev: { type: "number" },
                min:    { type: "number" }, max:    { type: "number" },
                mode:   { type: "number" }, alpha:  { type: "number" },
                beta:   { type: "number" }, lambda: { type: "number" },
                value:  { type: "number" },
      ...
      constraints: {
        type: "array",
        description: "Optional constraints evaluated after outputs are computed. Iterations violating any constraint are counted as violations and excluded from valid results.",
        items: {
          type: "object", required: ["id", "description", "expr"],
          properties: {
            ...
            expr:        { "$ref": "#/definitions/BoolExpr", description: "Boolean expression tree evaluated against input, intermediate, and output variables" },
          },
        },
      },
    },
    definitions: {
      Expr: {
        ...
        oneOf: [
          { type: "object", required: ["type","value"],        title: "NumberExpr",   properties: { type: { const: "number" }, value: { type: "number" } } },
          { type: "object", required: ["type","id"],           title: "VariableExpr", properties: { type: { const: "variable" }, id: { type: "string", minLength: 1 } } },
          { type: "object", required: ["type","op","left","right"], title: "BinaryExpr",
            properties: { type: { const: "binary" }, op: { type: "string", enum: ["+","-","*","/","**","%","min","max"] }, left: { "$ref": "#/definitions/Expr" }, right: { "$ref": "#/definitions/Expr" } } },
          { type: "object", required: ["type","op","operand"], title: "UnaryExpr",
            properties: { type: { const: "unary" }, op: { type: "string", enum: ["-","abs","sqrt","log","log10","exp","floor","ceil","round"] }, operand: { "$ref": "#/definitions/Expr" } } },
          { type: "object", required: ["type","fn","args"],    title: "CallExpr",
            properties: { type: { const: "call" }, fn: { type: "string", enum: ["max","min","pow","clamp"] }, args: { type: "array", items: { "$ref": "#/definitions/Expr" } } } },
          { type: "object", required: ["type","condition","then","else"], title: "ConditionalExpr",
            properties: { type: { const: "conditional" }, condition: { "$ref": "#/definitions/BoolExpr" }, then: { "$ref": "#/definitions/Expr" }, else: { "$ref": "#/definitions/Expr" } } },
        ],
      },
      BoolExpr: {
        oneOf: [
          { type: "object", required: ["type","op","left","right"], title: "CompareExpr",
            properties: { type: { const: "compare" }, op: { type: "string", enum: ["<","<=",">",">=","==","!="] }, left: { "$ref": "#/definitions/Expr" }, right: { "$ref": "#/definitions/Expr" } } },
          { type: "object", required: ["type","op","operands"], title: "LogicalExpr",
            properties: { type: { const: "logical" }, op: { type: "string", enum: ["and","or","not"] }, operands: { type: "array", items: { "$ref": "#/definitions/BoolExpr" }, minItems: 1 } } },
        ],
      },
    },
  });
...
router.get("/monte-carlo/ws-info", authMiddleware({ required: false }), (_req, res) => {
  sendSuccess(res, {
    wsChannel: WS_CHANNELS.MONTE_CARLO_PROGRESS,
    events: [
      { event: "progress", description: "Emitted every iteration batch — contains jobId, iteration, totalIterations, percentComplete, elapsedMs, estimatedRemainingMs" },
      { event: "interim-snapshot", description: "Emitted every snapshotInterval valid iterations (built-in scenarios via POST /simulate; pass snapshotInterval in request body, e.g. 500). Contains jobId, validIterations, totalIterations, percentComplete, and snapshots[] with per-output mean/p25/p50/p75/min/max. Enables live distribution formation UX." },
      { event: "complete", description: "Emitted when simulation finishes — contains jobId, status, result summary" },
      { event: "error", description: "Emitted on simulation failure — contains jobId and error message" },
    ],
    auth: {
      required: true,
      ...
      tokenFormats: [
        {
          type: "ws-ticket",
          description: "Recommended for tenant-scoped subscriptions. Obtain a short-lived ticket via POST /auth/ws-ticket (requires session auth). The ticket embeds your userId, platformRole, and tenantId. Clients subscribing with a ticket receive ONLY events scoped to their tenantId.",
          howToSubscribe: "Connect to /ws, then send: { type: 'subscribe', channel: 'monte-carlo:progress', token: '<ws-ticket>' }",
        },
        {
          type: "session-token",
          description: "Session token from the Authorization header or session cookie. Resolves without tenantId context. Clients using session tokens will NOT receive tenant-scoped job progress events (i.e., events published with a creatorTenantId). Use the ws-ticket flow to receive tenant-scoped Monte Carlo events.",
          howToSubscribe: "Connect to /ws, then send: { type: 'subscribe', channel: 'monte-carlo:progress', token: '<session-token>' }",
        },
      ],
    },
    tenantScoping: {
      description: "Monte Carlo job progress events are scoped to the tenant of the job creator. Only clients whose WS session tenantId matches the job's creatorTenantId will receive those events. Platform/founder admins with no tenantId context receive all events for cross-tenant monitoring.",
      recommendation: "Always use a WS ticket (which carries tenantId) for tenant-aware applications. Session-token subscribers with tenantId=null are excluded from tenant-scoped events.",
    },
    parallelExecution: "Custom DSL scenarios (POST /simulate/custom) use parallel worker threads; built-in scenarios execute in-process.",
  });
});
