...
const MATURITY_SCORES: Record<MaturityLevel, number> = {
  "missing": 0, "stub": 1, "functional": 2, "polished": 3, "best-in-class": 4,
};
...
autopilotRouter.get("/autopilot/drift-alerts", (_req: Request, res: Response) => {
  const alerts: Array<{
    id: string; severity: "critical" | "warning" | "info"; app: string; metric: string;
    detail: string; recommendation: string; timestamp: string;
  }> = [];
  ...
  try {
    ...
    if (stats && Array.isArray(stats)) {
      recentStats = stats.filter(s => s.p95Ms > LATENCY_THRESHOLD_MS).map(s => ({
        route: s.route,
  ...
  for (const stat of recentStats.slice(0, 3)) {
    alerts.push({
      ...
      recommendation: "Profile this route for N+1 queries or missing DB indexes. Add query result caching for read-heavy endpoints.",
  ...
  const structuralAlerts: typeof alerts = [
    {
      ...
      recommendation: "Reconnect the CRM sync pipeline and add a freshness watchdog to alert at >30m stale.",
    ...
    ...(GENOME_CONFIG.terra?.["Real-Time Data"] !== "best-in-class" ? [{
      ...
      recommendation: "Add a DB index on distress_score + borough. Current query performs a sequential scan.",
    ...
    ...(GENOME_CONFIG.prism?.["Webhooks"] === "stub" ? [{
      ...
      recommendation: "Implement PRISM outbound webhook dispatch using the existing webhook-engine library.",
    ...
    ...(GENOME_CONFIG.carlota?.["Webhooks"] === "missing" ? [{
      ...
      app: "Carlota Jo",
      metric: "Webhooks",
      detail: "Webhooks capability rated Missing — no implementation found in route manifest.",
      recommendation: "Use the shared webhook-engine to add outbound hooks for client milestone events.",
    ...
    }] : []),
    {
      ...
      recommendation: "Code-split the MITRE ATT&CK matrix — it is 280KB and loaded eagerly on all routes.",
...
const FEATURE_USAGE_DATA = [
  { app: "Lyte", feature: "AI Signal Summarizer", uses: 1420, trend: "rising" as const, delta: "+34%" },
  { app: "Aegis", feature: "Threat Feed Dashboard", uses: 980, trend: "rising" as const, delta: "+18%" },
  { app: "Vessels", feature: "Voyage Economics", uses: 762, trend: "stable" as const, delta: "+2%" },
  ...
  { app: "Aegis", feature: "MITRE ATT&CK Map", uses: 290, trend: "declining" as const, delta: "-14%" },
  { app: "PRISM", feature: "Matter Timeline", uses: 210, trend: "stable" as const, delta: "+5%" },
  { app: "Terra", feature: "Ownership Graph", uses: 180, trend: "declining" as const, delta: "-22%" },
  { app: "Carlota Jo", feature: "Estate Report PDF", uses: 94, trend: "stable" as const, delta: "+3%" },
  { app: "Aegis", feature: "Adversary Emulation Wizard", uses: 28, trend: "dead" as const, delta: "-61%" },
  { app: "Lyte", feature: "SCIM Provisioning UI", uses: 12, trend: "dead" as const, delta: "-40%" },
  { app: "Vessels", feature: "Charter Rate Benchmarks", uses: 8, trend: "dead" as const, delta: "-73%" },
];
...
const PERF_BUDGETS = [
  { app: "Lyte", bundleBudgetKB: 800, bundleActualKB: 734, latencyBudgetMs: 1500, latencyActualMs: 820, ttiMs: 1100 },
  { app: "Aegis", bundleBudgetKB: 900, bundleActualKB: 1340, latencyBudgetMs: 2000, latencyActualMs: 1740, ttiMs: 1800 },
  { app: "Vessels", bundleBudgetKB: 700, bundleActualKB: 610, latencyBudgetMs: 1800, latencyActualMs: 1020, ttiMs: 1300 },
  { app: "Terra", bundleBudgetKB: 850, bundleActualKB: 790, latencyBudgetMs: 2000, latencyActualMs: 2420, ttiMs: 2600 },
  { app: "PRISM", bundleBudgetKB: 650, bundleActualKB: 490, latencyBudgetMs: 1500, latencyActualMs: 1100, ttiMs: 1400 },
  ...
  try {
    ...
    if (stats && Array.isArray(stats)) {
      apmEnriched = PERF_BUDGETS.map(budget => {
        const match = stats.find(s => s.route?.includes(budget.app.toLowerCase().replace(/ /g, "-")));
        if (match) {
...
const FEEDBACK_SEED = [
  { app: "Lyte", feature: "AI Signal Summarizer", thumbsUp: 142, thumbsDown: 9, topComment: "Saves me 20 minutes every morning. Keep it." },
  { app: "Vessels", feature: "Voyage Economics", thumbsUp: 89, thumbsDown: 4, topComment: "The fuel cost estimator is accurate now." },
  { app: "Terra", feature: "Distress Engine", thumbsUp: 76, thumbsDown: 12, topComment: "Would love filtering by borough on the map." },
  { app: "Aegis", feature: "Threat Feed Dashboard", thumbsUp: 61, thumbsDown: 3, topComment: "Clean. The priority scoring is spot on." },
  { app: "PRISM", feature: "Matter Timeline", thumbsUp: 44, thumbsDown: 7, topComment: "Timeline needs event grouping by date." },
  { app: "Carlota Jo", feature: "Estate Report PDF", thumbsUp: 38, thumbsDown: 2, topComment: "Client loved the layout. Very professional." },
  { app: "Aegis", feature: "Adversary Emulation Wizard", thumbsUp: 6, thumbsDown: 18, topComment: "Confusing UX — 3 steps to get to the actual config." },
];
  ...
  try {
    ...
    const rows = await db
      .select({
        ...
        positiveCount: sql<number>`count(*) filter (where ${feedbackTable.score} >= 4)`.as("positive_count"),
        negativeCount: sql<number>`count(*) filter (where ${feedbackTable.score} <= 2)`.as("negative_count"),
...
const PLAYBOOKS_DATA = [
  { id: "p1", domain: "Carlota Jo", name: "Client Onboarding", steps: 8, completionRate: 84, avgDurationMin: 22, status: "complete", lastRun: "2 days ago", bottleneck: "Step 4: Contract sign-off (avg 6 min)" },
  { id: "p2", domain: "Aegis", name: "Incident Response", steps: 12, completionRate: 91, avgDurationMin: 41, status: "complete", lastRun: "14 hr ago", bottleneck: "Step 7: Escalation approval (avg 11 min)" },
  { id: "p3", domain: "Vessels", name: "Vessel Inspection", steps: 6, completionRate: 77, avgDurationMin: 18, status: "in-progress", lastRun: "4 hr ago", bottleneck: "Step 5: Photo upload (avg 4 min)" },
  { id: "p4", domain: "Terra", name: "Property Due Diligence", steps: 10, completionRate: 68, avgDurationMin: 55, status: "in-progress", lastRun: "1 day ago", bottleneck: "Step 6: Ownership verification (avg 18 min)" },
  { id: "p5", domain: "PRISM", name: "Matter Intake", steps: 7, completionRate: 88, avgDurationMin: 14, status: "complete", lastRun: "6 hr ago", bottleneck: "Step 3: Conflict check (avg 3 min)" },
  { id: "p6", domain: "SZL Holdings", name: "LP Quarterly Update", steps: 9, completionRate: 55, avgDurationMin: 90, status: "not-started", lastRun: "Never", bottleneck: "Not yet run — template ready" },
];
...
autopilotRouter.get("/autopilot/playbooks", (_req: Request, res: Response) => {
  ...
  const playbooksWithSignals = PLAYBOOKS_DATA.map(p => ({
    ...p,
    scheduledJobsRelated: registry.filter(j => j.name.toLowerCase().includes(p.domain.toLowerCase())).length,
  }));
...
const COMPETITIVE_RADAR_DATA = [
  {
    ...
    weHave: ["Unified SOC+MSP+AI in one surface", "Agentic cortex with model registry", "Client management for MSPs"],
  ...
  },
  {
    ...
    weHave: ["Multi-factor distress scoring", "Ownership graph traversal", "AI-driven market signals"],
...
function computeNextBestActions(
  ...
  rank: number; title: string; app: string; accent: string; why: string;
  effort: string; impact: string; signals: string[];
}> {
  const actions = [
    {
      rank: 1,
