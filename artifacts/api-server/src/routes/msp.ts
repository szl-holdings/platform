...
router.get("/msp/dashboard", auth, async (_req, res) => {
  try {
    ...
    const mrrGrowth = prevMrr > 0 ? Math.round(((clients.totalMrr - prevMrr) / prevMrr) * 100 * 10) / 10 : 0;
...
router.get("/msp/clients", auth, async (req, res) => {
  try {
    ...
    let query = db.select().from(mspClientsTable).limit(limit).offset(offset).orderBy(desc(mspClientsTable.mrr));

    const conditions = [];
    if (status && status !== "all") conditions.push(eq(mspClientsTable.status, status as "active" | "inactive" | "at-risk" | "churned"));
    if (search) {
      conditions.push(or(
    ...
    const data = conditions.length > 0
      ? await db.select().from(mspClientsTable).where(and(...conditions)).limit(limit).offset(offset).orderBy(desc(mspClientsTable.mrr))
      : await query;
...
router.get("/msp/tickets", auth, async (req, res) => {
  try {
    ...
    const conditions = [];
    if (status && status !== "all") conditions.push(eq(mspTicketsTable.status, status as "open" | "in-progress" | "waiting" | "resolved" | "closed"));
    if (priority && priority !== "all") conditions.push(eq(mspTicketsTable.priority, priority as "critical" | "high" | "medium" | "low"));
    ...
    const data = conditions.length > 0
      ? await db.select().from(mspTicketsTable).where(and(...conditions)).limit(limit).offset(offset).orderBy(desc(mspTicketsTable.createdAt))
      : await db.select().from(mspTicketsTable).limit(limit).offset(offset).orderBy(desc(mspTicketsTable.createdAt));
...
router.post("/msp/tickets", auth, async (req, res) => {
  try {
    ...
    const now = new Date();
    const slaHours = priority === "critical" ? 1 : priority === "high" ? 4 : priority === "medium" ? 8 : 24;
    const slaDeadline = new Date(now.getTime() + slaHours * 60 * 60 * 1000);

    const [ticket] = await db.insert(mspTicketsTable).values({
      ticketNumber,
      subject,
...
router.get("/msp/devices", auth, async (req, res) => {
  try {
    ...
    const conditions = [];
    if (type && type !== "all") conditions.push(eq(mspDevicesTable.type, type as "server" | "workstation" | "network" | "printer" | "mobile" | "firewall"));
    if (status && status !== "all") conditions.push(eq(mspDevicesTable.status, status as "online" | "warning" | "critical" | "offline"));
    ...
    const data = conditions.length > 0
      ? await db.select().from(mspDevicesTable).where(and(...conditions)).limit(limit).offset(offset).orderBy(desc(mspDevicesTable.updatedAt))
      : await db.select().from(mspDevicesTable).limit(limit).offset(offset).orderBy(desc(mspDevicesTable.updatedAt));
...
router.get("/msp/contracts", auth, async (req, res) => {
  try {
    ...
    const conditions = [];
    if (status && status !== "all") conditions.push(eq(mspContractsTable.status, status as "active" | "expiring" | "expired" | "pending-renewal"));
    if (search) {
      conditions.push(or(
    ...
    const data = conditions.length > 0
      ? await db.select().from(mspContractsTable).where(and(...conditions)).limit(limit).offset(offset).orderBy(desc(mspContractsTable.value))
      : await db.select().from(mspContractsTable).limit(limit).offset(offset).orderBy(desc(mspContractsTable.value));
...
router.get("/msp/technicians", auth, async (req, res) => {
  try {
    const status = req.query.status as string | undefined;
    const conditions = [];
    if (status && status !== "all") conditions.push(eq(mspTechniciansTable.status, status as "available" | "on-site" | "traveling" | "off-duty"));

    const data = conditions.length > 0
      ? await db.select().from(mspTechniciansTable).where(and(...conditions)).orderBy(mspTechniciansTable.name)
...
router.get("/msp/revenue", auth, async (_req, res) => {
  try {
    const [clients, contractsResult] = await Promise.all([
      ...
      }).from(mspClientsTable).orderBy(desc(mspClientsTable.mrr)),
      db.select({
        totalValue: sql<number>`coalesce(sum(value), 0)::int`,
        avgSla: sql<number>`coalesce(avg(sla_actual), 0)::int`,
      }).from(mspContractsTable).where(eq(mspContractsTable.status, "active")),
    ]);
    ...
    const avgContractValue = activeClients.length > 0 ? Math.round(totalMrr / activeClients.length) : 0;
