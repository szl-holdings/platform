...
import {
  ...
  type ConstellationSavedView,
} from "@szl-holdings/db";
import { and, eq, asc, or, inArray } from "drizzle-orm";
import {
  sendSuccess,
  sendBadRequest,
  sendNotFound,
...
function annotateView(
  row: ConstellationSavedView,
  user: AuthenticatedUser,
): SavedViewResponse {
  const isOwner = row.userId === user.id;
  const orgMembership =
    row.orgId != null ? findOrgMembership(user, row.orgId) : undefined;
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      sendSuccess(res, rows.map((r) => annotateView(r, user)));
    } catch (err) {
    ...
    try {
      ...
      if (visibility === "org") {
        ...
        if (requested == null) {
          sendBadRequest(
            res,
            "Cannot share view: user is not a member of any organization",
          );
          return;
        }
        if (!findOrgMembership(user, requested)) {
          sendForbidden(
            res,
            "Cannot share view with an organization you don't belong to",
          );
          return;
        }
        orgId = requested;
...
async function loadEditableView(
  ...
  | { kind: "ok"; row: ConstellationSavedView }
  | { kind: "not_found" }
  | { kind: "forbidden" }
> {
  const [row] = await db
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (access.kind === "forbidden") {
        sendForbidden(
          res,
          "Only the owner or an org admin can modify this shared view",
        );
        return;
      }
      ...
      if ((wantsVisibilityChange || wantsOrgChange) && existing.userId !== user.id) {
        sendForbidden(
          res,
          "Only the owner can change visibility or organization of a saved view",
        );
        return;
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (access.kind === "forbidden") {
        sendForbidden(
          res,
          "Only the owner or an org admin can delete this shared view",
        );
        return;
      }
      const [row] = await db
