/**
 * Continuum Cognitive Learning Routes
 *
 * Endpoints for:
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        sendBadRequest(
          res,
          'Missing required fields: decisionId, agentId, outcome, originalAction, originalConfidence, topic',
        );
        return;
      }
      ...
      const topicKeywords = `${topic} ${originalAction}`
        .toLowerCase()
        .split(/\W+/)
      ...
      logger.error({ err }, 'POST /continuum/cognitive/outcomes failed');
...
router.get(
  '/continuum/cognitive/calibration/:agentId',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
      if (!req.user) {
      ...
      const avgConfidence =
        rows.reduce((s, r) => s + r.originalConfidence, 0) / (totalDecisions || 1);
      ...
      logger.error({ err }, 'GET /continuum/cognitive/calibration failed');
...
router.get(
  '/continuum/cognitive/evals/latest',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error({ err }, 'GET /continuum/cognitive/evals/latest failed');
...
router.get(
  '/continuum/cognitive/evals/history',
  authMiddleware(),
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error({ err }, 'GET /continuum/cognitive/evals/history failed');
...
router.get(
  '/continuum/cognitive/evals/calibrations',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
      if (!req.user || !isElevatedUser(req.user)) {
      ...
      logger.error({ err }, 'GET /continuum/cognitive/evals/calibrations failed');
    ...
    try {
      ...
      const executor = async (input: string, _category: string) => {
        ...
        const evalResult = await callModel({
          provider: 'openai', model: 'gpt-4o-mini', surface: 'continuum-cognitive-evals',
          fn: async () => {
      ...
      logger.error({ err }, 'POST /continuum/cognitive/evals/run failed');
...
router.get(
  '/continuum/cognitive/corrections/:agentId',
  authMiddleware(),
  validateQuery(listQuerySchema),
  async (req: Request, res: Response) => {
    try {
      ...
      const orgFilter = elevated
        ...
        : userOrgIds.length > 0
          ? or(
              ...
              inArray(alloyAgentCorrections.orgId, userOrgIds),
            )
          : isNull(alloyAgentCorrections.orgId);
      ...
      logger.error({ err }, 'GET /continuum/cognitive/corrections failed');
...
router.get(
  '/continuum/cognitive/memory-stats',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
      if (!req.user || !isElevatedUser(req.user)) {
      ...
      logger.error({ err }, 'GET /continuum/cognitive/memory-stats failed');
