...
import {
  policyModesDeleteBodySchema,
  policyModesResolveQuerySchema,
  validateBody,
  validateQuery,
} from '../lib/validation';
...
router.get(
  '/policy-modes/resolve',
  authMiddleware(),
  validateQuery(policyModesResolveQuerySchema),
...
router.get('/policy-modes/:id', authMiddleware(), async (req: Request, res: Response) => {
  try {
    ...
    if (!config) {
      sendNotFound(res, 'PolicyModeConfig');
      return;
    }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const parsed = PolicyModeConfigSchema.safeParse({
        ...body,
        id,
        createdAt: now,
        updatedAt: now,
...
router.delete(
  ...
  validateBody(policyModesDeleteBodySchema),
  async (req: Request, res: Response) => {
    try {
      const { id } = req.params as { id: string };
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const evaluation = buildPolicyEvaluation({
        ...
        projectedImpact:
          body.projectedImpact ??
          'Not specified — caller must supply projectedImpact for production use.',
        projectedRisk:
          body.projectedRisk ??
          'Not specified — caller must supply projectedRisk for production use.',
      ...
      logger.info(
        {
          ...
          effect: evaluation.policyResult.effect,
        },
