...
function validateBody<T extends z.ZodTypeAny>(schema: T) {
  return (req: Request, res: Response, next: () => void) => {
    ...
    if (!result.success) {
      res.status(400).json({ ok: false, error: 'validation_error', details: result.error.flatten() });
      return;
    }
...
function computeEventHash(
  prevHash: string,
  payload: { action: string; actor: string; domain: string; actionType: string; entityId?: string | null; createdAt: string },
): string {
  const data = [prevHash, payload.action, payload.actor, payload.domain, payload.actionType, payload.entityId ?? '', payload.createdAt].join('|');
...
async function evaluatePromotionGate(candidateId: string, targetState: string): Promise<{
  eligible: boolean;
  blockers: string[];
  reasons: string[];
  rewardScore: number;
  ...
  const [lastRun] = await db
    ...
    .where(and(eq(perEvaluationRunsTable.candidateId, candidateId), eq(perEvaluationRunsTable.status, 'completed')))
  ...
  if (!lastRun) {
    ...
    const passRate = lastRun.passRate ?? (lastRun.totalCases > 0 ? lastRun.passed / lastRun.totalCases : 0);
  ...
  if (lastDrift) {
    ...
    if (DRIFT_GUARD && lastDrift.status === 'critical') {
      blockers.push(`Drift status is critical (score: ${driftScore.toFixed(3)}) — drift guard active`);
    } else if (lastDrift.status === 'degraded') {
      reasons.push(`Drift score ${driftScore.toFixed(3)} (degraded — within tolerance for review)`);
    } else {
      reasons.push(`Drift within healthy bounds (${lastDrift.status}, score: ${driftScore.toFixed(3)})`);
    }
  } else {
...
router.get('/evolution/diagnostics', async (_req: Request, res: Response) => {
  try {
    ...
    if (!IS_SIMULATION) {
      ...
      await db.insert(perRuntimeHealthSnapshotsTable).values({
        snapshotId,
...
router.post('/evolution/candidates', authMiddleware(), validateBody(RegisterCandidateSchema), async (req: Request, res: Response) => {
  try {
    if (IS_SIMULATION) {
      const candidateId = `cand-${randomUUID()}`;
      const now = new Date().toISOString();
      return res.status(201).json({ ok: true, data: { candidateId, ...req.body, state: 'draft', simulated: true, createdAt: now, updatedAt: now }, simulated: true });
    }
    const candidateId = `cand-${randomUUID()}`;
    const [row] = await db
...
router.get('/evolution/candidates', async (_req: Request, res: Response) => {
  try {
    if (IS_SIMULATION) {
      const { buildSimulatedState } = await import('@szl-holdings/evolution-core/simulation');
      const state = buildSimulatedState();
      return res.json({ ok: true, data: state.candidates, total: state.candidates.length, simulated: true });
    }
    const candidates = await db.select().from(perCandidatePoliciesTable).orderBy(desc(perCandidatePoliciesTable.createdAt));
...
router.get('/evolution/candidates/:id', async (req: Request, res: Response) => {
  try {
    ...
    const [candidate] = await db.select().from(perCandidatePoliciesTable).where(eq(perCandidatePoliciesTable.candidateId, req.params.id));
    if (!candidate) return res.status(404).json({ ok: false, error: 'not_found' });
...
router.post('/evolution/candidates/:id/evaluate', authMiddleware(), validateBody(LaunchEvaluationSchema), async (req: Request, res: Response) => {
  try {
    const runId = `eval-${randomUUID()}`;
    if (IS_SIMULATION) {
      return res.status(202).json({
        ok: true,
        data: { runId, candidateId: req.params.id, status: 'queued', ...req.body, simulated: true, createdAt: new Date().toISOString() },
        message: 'Evaluation run queued (simulation)', simulated: true,
      });
    }
    const [row] = await db
      .insert(perEvaluationRunsTable)
      .values({ runId, candidateId: req.params.id, suiteId: req.body.suiteId, suiteName: req.body.suiteName, triggeredBy: req.body.triggeredBy, status: 'queued', simulated: false })
      .returning();
...
router.post('/evolution/candidates/:id/calibrate', authMiddleware(), validateBody(LaunchCalibrationSchema), async (req: Request, res: Response) => {
  try {
    const { launchCalibrationRun } = await import('@szl-holdings/evolution-core/calibration');
    const result = await launchCalibrationRun({ candidateId: req.params.id, runType: req.body.runType, datasetId: req.body.datasetId, datasetName: req.body.datasetName });
...
router.get('/evolution/scorecards/:runId', async (req: Request, res: Response) => {
  try {
    ...
    const [run] = await db.select().from(perEvaluationRunsTable).where(eq(perEvaluationRunsTable.runId, req.params.runId));
    if (!run) return res.status(404).json({ ok: false, error: 'run_not_found', message: `No evaluation run found for runId: ${req.params.runId}` });
    res.json({
      ok: true,
      data: {
        runId: run.runId,
...
router.get('/evolution/drift/:candidateId', async (req: Request, res: Response) => {
  try {
    ...
    const reports = await db.select().from(perDriftReportsTable).where(eq(perDriftReportsTable.candidateId, req.params.candidateId)).orderBy(desc(perDriftReportsTable.measuredAt));
...
router.post('/evolution/candidates/:id/promote', authMiddleware(), requireRole('ops', 'admin'), validateBody(PromotionRequestSchema), async (req: Request, res: Response) => {
  try {
    const decisionId = `promo-${randomUUID()}`;
    if (IS_SIMULATION) {
      return res.status(202).json({
        ok: true,
        data: { decisionId, candidateId: req.params.id, outcome: 'pending_review', targetState: req.body.targetState, humanApprovalRequired: true, simulated: true, createdAt: new Date().toISOString() },
    ...
    if (!validFromStates.includes(candidate.state)) {
      return res.status(409).json({ ok: false, error: 'invalid_state', message: `Cannot promote a candidate in state '${candidate.state}'` });
    }
    ...
    await db.update(perCandidatePoliciesTable).set({ state: 'review', updatedAt: new Date() }).where(eq(perCandidatePoliciesTable.candidateId, req.params.id));
    ...
    const auditId = await appendAuditChainEvent({
      ...
      outcome: 'pending_review',
      metadata: { candidateId: req.params.id, targetState: req.body.targetState, gateResult: gate },
    });

    if (auditId) {
      await db.update(perPromotionDecisionsTable).set({ auditChainEventId: auditId }).where(eq(perPromotionDecisionsTable.decisionId, decisionId));
    }

    res.status(202).json({ ok: true, data: decision, message: 'Promotion gate passed — awaiting human approval', gateResult: gate });
  } catch (err) {
    res.status(500).json({ ok: false, error: 'promotion_request_failed', message: String(err) });
  }
});

router.post('/evolution/candidates/:id/approve', authMiddleware(), requireRole('ops', 'admin'), validateBody(ApproveRejectSchema), async (req: Request, res: Response) => {
  try {
    if (IS_SIMULATION) {
      return res.json({ ok: true, data: { candidateId: req.params.id, outcome: 'approved', newState: 'active', approvedAt: new Date().toISOString(), simulated: true } });
    }
    const [decision] = await db
      .select()
      .from(perPromotionDecisionsTable)
      .where(and(eq(perPromotionDecisionsTable.candidateId, req.params.id), eq(perPromotionDecisionsTable.outcome, 'pending_review')))
      .orderBy(desc(perPromotionDecisionsTable.createdAt));
    if (!decision) return res.status(404).json({ ok: false, error: 'no_pending_decision', message: 'No pending promotion decision found.' });

    if (req.body.decision === 'approved') {
      await db.update(perPromotionDecisionsTable).set({ outcome: 'approved', approvedAt: new Date(), updatedAt: new Date() }).where(eq(perPromotionDecisionsTable.decisionId, decision.decisionId));
      await appendAuditChainEvent({
        ...
        outcome: 'success',
        metadata: { candidateId: req.params.id, decisionId: decision.decisionId, reason: req.body.reason },
      });
      return res.json({ ok: true, data: { candidateId: req.params.id, decisionId: decision.decisionId, outcome: 'approved', newState: decision.toState } });
    } else {
      await db.update(perPromotionDecisionsTable).set({ outcome: 'rejected', rejectionReason: req.body.reason, updatedAt: new Date() }).where(eq(perPromotionDecisionsTable.decisionId, decision.decisionId));
      await db.update(perCandidatePoliciesTable).set({ state: 'draft', updatedAt: new Date() }).where(eq(perCandidatePoliciesTable.candidateId, req.params.id));
      await appendAuditChainEvent({
      ...
      return res.json({ ok: true, data: { candidateId: req.params.id, decisionId: decision.decisionId, outcome: 'rejected' } });
    }
  } catch (err) {
...
router.post('/evolution/candidates/:id/reject', authMiddleware(), requireRole('ops', 'admin'), validateBody(ApproveRejectSchema), async (req: Request, res: Response) => {
  try {
    if (IS_SIMULATION) {
      return res.json({ ok: true, data: { candidateId: req.params.id, outcome: 'rejected', reason: req.body.reason, rejectedAt: new Date().toISOString(), simulated: true } });
    }
    const [decision] = await db
      .select()
      .from(perPromotionDecisionsTable)
    ...
    res.json({ ok: true, data: { candidateId: req.params.id, decisionId: decision.decisionId, outcome: 'rejected', reason: req.body.reason } });
  } catch (err) {
    res.status(500).json({ ok: false, error: 'rejection_failed', message: String(err) });
  }
});

router.post('/evolution/candidates/:id/activate', authMiddleware(), requireRole('ops', 'admin'), async (req: Request, res: Response) => {
  try {
    if (IS_SIMULATION) {
      return res.json({ ok: true, data: { candidateId: req.params.id, state: 'active', activatedAt: new Date().toISOString(), simulated: true } });
    }
    const [candidate] = await db.select().from(perCandidatePoliciesTable).where(eq(perCandidatePoliciesTable.candidateId, req.params.id));
    if (!candidate) return res.status(404).json({ ok: false, error: 'candidate_not_found' });
    ...
    const [approvedDecision] = await db
      .select()
      .from(perPromotionDecisionsTable)
      .where(and(eq(perPromotionDecisionsTable.candidateId, req.params.id), eq(perPromotionDecisionsTable.outcome, 'approved')))
      .orderBy(desc(perPromotionDecisionsTable.createdAt));
    if (!approvedDecision) {
      return res.status(409).json({ ok: false, error: 'activation_blocked', message: 'Activation requires an approved promotion decision. Submit /promote and approve it first.' });
    }

    const { validateStateTransition } = await import('@szl-holdings/evolution-core/control-plane');
    const transition = validateStateTransition(candidate.state as Parameters<typeof validateStateTransition>[0], 'active');
    if (!transition.valid) {
      return res.status(400).json({ ok: false, error: 'invalid_transition', message: transition.reason });
    }
    ...
    await appendAuditChainEvent({
      ...
      outcome: 'success',
      metadata: { candidateId: req.params.id, decisionId: approvedDecision.decisionId, precisionProfile: candidate.precisionProfile },
    });
...
router.post('/evolution/candidates/:id/rollback', authMiddleware(), requireRole('ops', 'admin'), validateBody(RollbackSchema), async (req: Request, res: Response) => {
  try {
    if (IS_SIMULATION) {
      return res.json({ ok: true, data: { candidateId: req.params.id, state: 'rolled_back', rollbackReason: req.body.reason, rolledBackAt: new Date().toISOString(), simulated: true } });
    }
    const [candidate] = await db.select().from(perCandidatePoliciesTable).where(eq(perCandidatePoliciesTable.candidateId, req.params.id));
    if (!candidate) return res.status(404).json({ ok: false, error: 'candidate_not_found' });
    if (candidate.state !== 'active') {
      return res.status(409).json({ ok: false, error: 'not_active', message: 'Rollback only applies to active policies.' });
    }

    const [updated] = await db
      .update(perCandidatePoliciesTable)
      .set({ state: 'rolled_back', rolledBackAt: new Date(), rollbackReason: req.body.reason, updatedAt: new Date() })
      .where(eq(perCandidatePoliciesTable.candidateId, req.params.id))
      .returning();
  ...
  try {
    if (IS_SIMULATION) {
      ...
      const events = state.candidates.map((c, i) => ({
        id: i + 1,
        type: c.state === 'active' ? 'policy_activated' : c.state === 'review' ? 'promotion_requested' : 'candidate_registered',
    ...
    const [candidates, decisions, snapshots] = await Promise.all([
      db.select().from(perCandidatePoliciesTable).orderBy(desc(perCandidatePoliciesTable.updatedAt)).limit(50),
      db.select().from(perPromotionDecisionsTable).orderBy(desc(perPromotionDecisionsTable.createdAt)).limit(50),
      db.select().from(perRuntimeHealthSnapshotsTable).orderBy(desc(perRuntimeHealthSnapshotsTable.measuredAt)).limit(20),
    ]);

    const events = [
      ...candidates.map((c) => ({ type: `candidate_${c.state}`, candidateId: c.candidateId, detail: c.displayName, timestamp: c.updatedAt?.toISOString(), domain: 'per:candidates', outcome: 'success', riskLevel: 'low', simulated: false })),
      ...decisions.map((d) => ({ type: `promotion_${d.outcome}`, candidateId: d.candidateId, decisionId: d.decisionId, detail: `${d.fromState}→${d.toState}`, timestamp: d.createdAt?.toISOString(), domain: 'per:promotions', outcome: d.outcome, riskLevel: 'high', simulated: false })),
      ...snapshots.map((s) => ({ type: 'health_snapshot', snapshotId: s.snapshotId, detail: s.precisionProfile, timestamp: s.measuredAt?.toISOString(), domain: 'per:diagnostics', outcome: 'success', riskLevel: 'low', simulated: false })),
...
router.post('/evolution/candidates/:id/rollout', authMiddleware(), validateBody(z.object({ batchSize: z.number().int().optional(), deterministicReplay: z.boolean().optional() })), async (req: Request, res: Response) => {
  try {
    const jobId = `job-${randomUUID()}`;
    if (IS_SIMULATION) {
      return res.status(202).json({ ok: true, data: { jobId, candidateId: req.params.id, status: 'queued', simulated: true, createdAt: new Date().toISOString() }, simulated: true });
    }
    const [candidate] = await db.select().from(perCandidatePoliciesTable).where(eq(perCandidatePoliciesTable.candidateId, req.params.id));
    if (!candidate) return res.status(404).json({ ok: false, error: 'candidate_not_found' });
    const [job] = await db.insert(perRolloutJobsTable).values({ jobId, candidateId: req.params.id, status: 'queued', totalBatches: 10, simulated: false }).returning();
...
router.get('/evolution/candidates/:id/rollout', async (req: Request, res: Response) => {
  try {
    ...
    const jobs = await db.select().from(perRolloutJobsTable).where(eq(perRolloutJobsTable.candidateId, req.params.id)).orderBy(desc(perRolloutJobsTable.createdAt));
...
router.get('/evolution/evaluations', async (_req: Request, res: Response) => {
  try {
    if (IS_SIMULATION) {
      const { buildSimulatedState } = await import('@szl-holdings/evolution-core/simulation');
      const state = buildSimulatedState();
      return res.json({ ok: true, data: state.evaluationRuns, total: state.evaluationRuns.length, simulated: true });
    }
    const runs = await db.select().from(perEvaluationRunsTable).orderBy(desc(perEvaluationRunsTable.createdAt)).limit(100);
...
router.get('/evolution/promotions', async (_req: Request, res: Response) => {
  try {
    if (IS_SIMULATION) {
      const { buildSimulatedState } = await import('@szl-holdings/evolution-core/simulation');
      const state = buildSimulatedState();
      return res.json({ ok: true, data: state.promotionQueue ?? [], total: (state.promotionQueue ?? []).length, simulated: true });
    }
    const decisions = await db.select().from(perPromotionDecisionsTable).orderBy(desc(perPromotionDecisionsTable.createdAt)).limit(50);
