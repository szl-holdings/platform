...
async function loadLp(lpId: number, scope: LpScope): Promise<FundAccreditedInvestor | null> {
  const [lp] = await db.select().from(fundAccreditedInvestorsTable).where(eq(fundAccreditedInvestorsTable.id, lpId));
  if (!lp) return null;
...
router.get("/lp-portal/lps", optionalAuth, async (req, res) => {
  try {
    ...
    if (scope.isGp) {
      rows = await db.select().from(fundAccreditedInvestorsTable).orderBy(asc(fundAccreditedInvestorsTable.lpName));
    } else if (scope.selfLpId) {
      rows = await db.select().from(fundAccreditedInvestorsTable).where(eq(fundAccreditedInvestorsTable.id, scope.selfLpId));
    } else {
      // demo only
      rows = await db.select().from(fundAccreditedInvestorsTable).orderBy(asc(fundAccreditedInvestorsTable.lpName));
    ...
    })), 200, { scope: { isGp: scope.isGp, isDemoOnly: scope.isDemoOnly, selfLpId: scope.selfLpId } });
  } catch (err) {
...
router.get("/lp-portal/lps/:id/capital-account", optionalAuth, async (req, res) => {
  try {
    ...
    if (!lp) { sendForbidden(res, "Not authorized for this LP's capital account"); return; }
    ...
    if (!acct) { sendNotFound(res, "Capital account"); return; }
...
router.get("/lp-portal/lps/:id/documents", optionalAuth, validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    const scope = await resolveScope(req);
    const lp = await loadLp(lpId, scope);
    if (!lp) { sendForbidden(res, "Not authorized for this LP's documents"); return; }
    ...
    const totalDocsConditions = scope.isDemoOnly ? [eq(fundLpDataRoomDocsTable.isDemo, true)] : [];
    ...
    })), 200, { totalAvailable: total, visibleTiers: tiers, accessTier: tierFor(lp) });
  } catch (err) {
...
router.get("/lp-portal/lps/:id/reports", optionalAuth, async (req, res) => {
  try {
    ...
    const scope = await resolveScope(req);
    const lp = await loadLp(lpId, scope);
    if (!lp) { sendForbidden(res, "Not authorized for this LP's reports"); return; }
    ...
    sendSuccess(res, rows.map(r => {
      ...
      return {
        ...
        generated: r.distributedAt ? new Date(r.distributedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : new Date(r.updatedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }),
        navPerUnit: meta.nav_per_unit ?? null,
...
function relativeTime(d: Date): string {
  ...
  if (day < 7) return d.toLocaleString("en-US", { weekday: "short", hour: "numeric", minute: "2-digit" });
...
router.get("/lp-portal/lps/:id/activity", optionalAuth, async (req, res) => {
  try {
    ...
    const scope = await resolveScope(req);
    const lp = await loadLp(lpId, scope);
    if (!lp) { sendForbidden(res, "Not authorized for this LP's activity"); return; }
...
router.post("/lp-portal/lps/:id/activity", optionalAuth, validateBody(bodyShape({
  ...
  try {
    ...
    const body = req.body as { action?: string; target?: string; documentId?: number; reportId?: number } | undefined;
    const actionRaw = (body?.action ?? "").toLowerCase();
    const action = (["viewed", "downloaded", "messaged_gp", "logged_in"].includes(actionRaw)
      ? actionRaw
      : null) as "viewed" | "downloaded" | "messaged_gp" | "logged_in" | null;
    if (!action || !body?.target) { sendBadRequest(res, "action and target are required"); return; }

    const isDemo = scope.isDemoOnly || (lp.metadata as { is_demo?: boolean } | null)?.is_demo === true;

    const [inserted] = await db.insert(fundLpActivityEventsTable).values({
      lpId: lp.id,
      action,
...
router.get("/lp-portal/lps/:id/messages", optionalAuth, async (req, res) => {
  try {
    ...
    const scope = await resolveScope(req);
    const lp = await loadLp(lpId, scope);
    if (!lp) { sendForbidden(res, "Not authorized for this LP's messages"); return; }
...
router.post("/lp-portal/lps/:id/messages", optionalAuth, validateBody(bodyShape({})), async (req, res) => {
  try {
    ...
    if (!text) { sendBadRequest(res, "body is required"); return; }
    ...
    const [inserted] = await db.insert(fundLpMessagesTable).values({
      lpId: lp.id,
      fromRole,
      authorName,
