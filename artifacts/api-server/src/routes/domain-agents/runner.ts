...
async function persistEvalAsync(
  trace: AITrace,
  ctx: DomainEvalContext,
): Promise<void> {
  try {
...
function computeConfidenceProxy(responseText: string, isFallback: boolean): number {
  ...
  return Math.min(0.92, 0.72 + (Math.min(len, 2000) / 2000) * 0.20);
}
...
export async function runDomainAgentChat(
  ...
  function emitTrace(responseText: string, completion: ChatCompletionResult, isFallback: boolean): void {
