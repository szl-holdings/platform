...
export async function dispatchToExternalChannels(params: {
  ...
  for (const channel of channels) {
    void durableJobQueue
      ...
      .catch((err: unknown) => {
        logger.error(
          { err, channel, notificationId: params.notificationId },
          '[notifications] Failed to enqueue dispatch job',
        );
      });
  }
}
...
router.post(
  ...
  async (req, res) => {
    try {
      const { userId, type, channel, title, message, actionUrl } = req.body as z.infer<
