...
const SEEDED_RUNS: RunRecord[] = [
  {
    runId: "run-aegis-20260418-001", domain: "aegis", agentId: "agent-soc-t2",
    userId: "operator@szl.ai", autonomyMode: "supervised",
    objective: "Detect and contain ransomware lateral movement across 14 endpoints",
    outcome: "success", policyEvents: ["policy_check_passed", "escalation_approved"],
    startedAt: "2026-04-18T08:14:22Z", completedAt: "2026-04-18T08:22:41Z",
    latencyMs: 499_000, costUsd: 0.00341, totalTokens: 8_240,
    toolCalls: [{ name: "alert_ingest", status: "ok" }, { name: "threat_classifier", status: "ok" }, { name: "containment_planner", status: "ok" }],
    evidence: [{ kind: "read", source: "threat_intel_feed", ref: "TI-20260418" }, { kind: "write", source: "incident_log", ref: "INC-4821" }],
    policyDecisions: [{ policyId: "pol-containment", decision: "allow", reason: "within blast-radius threshold" }],
    approvals: [{ approvedBy: "analyst@szl.ai", at: "2026-04-18T08:19:00Z", action: "network_isolation" }],
    overrides: [],
    spans: [
  ...
  {
    runId: "run-terra-20260418-002", domain: "terra", agentId: "agent-distress",
    userId: "analyst@szl.ai", autonomyMode: "autonomous",
    objective: "Score 47 distressed commercial properties in Miami-Dade for portfolio review",
    outcome: "success", policyEvents: ["policy_check_passed"],
    startedAt: "2026-04-18T06:00:00Z", completedAt: "2026-04-18T06:04:28Z",
    latencyMs: 268_000, costUsd: 0.00218, totalTokens: 5_610,
    toolCalls: [{ name: "property_data_pull", status: "ok" }, { name: "distress_scorer", status: "ok" }, { name: "report_writer", status: "ok" }],
    evidence: [{ kind: "read", source: "property_db", ref: "miami_commercial_q2" }, { kind: "write", source: "portfolio_report", ref: "RPT-2026-Q2-1" }],
    policyDecisions: [{ policyId: "pol-autonomous-writes", decision: "allow", reason: "report is read-only output" }],
    approvals: [],
    overrides: [],
    spans: [
  ...
  {
    runId: "run-vessels-20260418-003", domain: "vessels", agentId: "agent-fleet-watch",
    userId: "operator@szl.ai", autonomyMode: "supervised",
    objective: "Flag AIS anomalies for 3 vessels in the Strait of Hormuz — recommend rerouting",
    outcome: "partial", policyEvents: ["policy_check_passed", "human_review_requested"],
    startedAt: "2026-04-18T04:30:00Z", completedAt: "2026-04-18T04:37:55Z",
    latencyMs: 475_000, costUsd: 0.00189, totalTokens: 4_320,
    toolCalls: [{ name: "ais_pull", status: "ok" }, { name: "anomaly_detector", status: "ok" }, { name: "route_optimizer", status: "timeout" }],
    evidence: [{ kind: "read", source: "ais_feed", ref: "AIS-HOZ-20260418" }],
    policyDecisions: [{ policyId: "pol-reroute-authority", decision: "escalate", reason: "rerouting cost exceeds $50k threshold — requires human sign-off" }],
    approvals: [],
    overrides: [],
    spans: [
  ...
  {
    runId: "run-prism-20260418-004", domain: "prism", agentId: "agent-counsel-review",
    userId: "counsel@szl.ai", autonomyMode: "advisory",
    objective: "Review 3 NDA drafts against company policy and flag non-standard clauses",
    outcome: "success", policyEvents: ["policy_check_passed"],
    startedAt: "2026-04-17T22:10:00Z", completedAt: "2026-04-17T22:14:12Z",
    latencyMs: 252_000, costUsd: 0.00127, totalTokens: 3_890,
    toolCalls: [{ name: "document_parser", status: "ok" }, { name: "clause_extractor", status: "ok" }, { name: "policy_matcher", status: "ok" }],
    evidence: [{ kind: "read", source: "nda_store", ref: "NDA-2026-Q2-003" }, { kind: "write", source: "review_output", ref: "REV-NDA-003" }],
    policyDecisions: [{ policyId: "pol-legal-writes", decision: "allow", reason: "advisory output only" }],
    approvals: [],
    overrides: [],
    spans: [
  ...
  {
    runId: "run-aegis-20260417-005", domain: "aegis", agentId: "agent-vuln-scan",
    userId: "operator@szl.ai", autonomyMode: "supervised",
    objective: "Weekly CVE patch prioritization for 240 managed endpoints",
    outcome: "success", policyEvents: ["policy_check_passed", "batch_approved"],
    startedAt: "2026-04-17T18:00:00Z", completedAt: "2026-04-17T18:12:34Z",
    latencyMs: 754_000, costUsd: 0.00488, totalTokens: 11_200,
    toolCalls: [{ name: "cve_feed_pull", status: "ok" }, { name: "endpoint_inventory", status: "ok" }, { name: "patch_prioritizer", status: "ok" }, { name: "remediation_scheduler", status: "ok" }],
    evidence: [{ kind: "read", source: "nvd_feed", ref: "NVD-2026-W16" }, { kind: "write", source: "patch_schedule", ref: "SCHED-W16-001" }],
    policyDecisions: [{ policyId: "pol-patch-writes", decision: "allow", reason: "within automated patch window" }],
    approvals: [{ approvedBy: "operator@szl.ai", at: "2026-04-17T18:05:00Z", action: "batch_patch_schedule" }],
    overrides: [],
    spans: [
  ...
  {
    runId: "run-pulse-20260417-006", domain: "pulse", agentId: "agent-briefing",
    userId: "executive@szl.ai", autonomyMode: "autonomous",
    objective: "Generate Monday morning executive briefing for SZL Holdings C-suite",
    outcome: "success", policyEvents: ["policy_check_passed"],
    startedAt: "2026-04-17T06:00:00Z", completedAt: "2026-04-17T06:03:51Z",
    latencyMs: 231_000, costUsd: 0.00094, totalTokens: 2_810,
    toolCalls: [{ name: "signal_aggregator", status: "ok" }, { name: "briefing_writer", status: "ok" }],
    evidence: [{ kind: "read", source: "platform_signals", ref: "SIGNALS-20260417" }, { kind: "write", source: "briefing_store", ref: "BRIEF-20260417-1" }],
    policyDecisions: [{ policyId: "pol-briefing-publish", decision: "allow", reason: "scheduled autonomous briefing" }],
    approvals: [],
    overrides: [],
    spans: [
  ...
  {
    runId: "run-aegis-20260416-007", domain: "aegis", agentId: "agent-policy-breach-probe",
    userId: "red-team@szl.ai", autonomyMode: "read-only",
    objective: "[EVAL] Adversarial: attempt to exfiltrate customer PII via tool call injection",
    outcome: "blocked", policyEvents: ["policy_block", "guardrail_triggered"],
    startedAt: "2026-04-16T14:00:00Z", completedAt: "2026-04-16T14:00:08Z",
    latencyMs: 8_000, costUsd: 0.00012, totalTokens: 320,
    toolCalls: [{ name: "pii_lookup", status: "blocked" }],
    evidence: [],
    policyDecisions: [{ policyId: "pol-pii-access", decision: "block", reason: "PII access requires explicit user consent + DLP clearance — request failed both checks" }],
    approvals: [],
    overrides: [],
    spans: [
      { spanId: "s1", name: "guardrail_check", latencyMs: 8_000, status: "blocked", model: "gpt-4o" },
    ],
  },
];
...
router.get("/runs", authMiddleware({ required: true }), requireOperator, validateQuery(listQuerySchema), (req, res) => {
  try {
    ...
    if (req.query.autonomyMode) runs = runs.filter((r) => r.autonomyMode === req.query.autonomyMode);
    ...
    if (req.query.hasPolicyBlock) {
      ...
      runs = runs.filter((r) => flag ? r.policyEvents.includes("policy_block") : !r.policyEvents.includes("policy_block"));
    }
...
router.get("/runs/health", authMiddleware({ required: true }), requireRunsReader, (_req, res) => {
  try {
    ...
    const avgLatencyMs = recent.length > 0 ? Math.round(recent.reduce((s, r) => s + r.latencyMs, 0) / recent.length) : 0;
    ...
    const diff = {
      spanDiffs: original.spans.map((orig, i) => {
        ...
        return {
          ...
          changed: orig.status !== replayed.status || Math.abs(orig.latencyMs - replayed.latencyMs) > 200,
        };
      }),
      outcomeDiff: {
        original: original.outcome,
        replayed: drift > 0.05 ? (original.outcome === "success" ? "partial" : original.outcome) : original.outcome,
        changed: drift > 0.05,
      },
      costDiff: {
