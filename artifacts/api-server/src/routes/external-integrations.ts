...
function verifySiemToken(req: Request, res: Response, next: NextFunction): void {
  ...
  if (!bearer || bearer !== siemToken) {
    res.status(401).json({
      error: 'SIEM ingest token required — set Authorization: Bearer <SIEM_INGEST_TOKEN>',
    });
    return;
  }
  next();
...
router.get('/integrations/status', async (_req: Request, res: Response) => {
  try {
    ...
    const overallHealth =
      summary.degraded > 0 ? 'degraded' : summary.live > 0 ? 'operational' : 'demo_mode';
...
router.get('/integrations/status/:adapter', async (req: Request, res: Response) => {
  try {
    ...
    if (!TRACKED_INTEGRATIONS.includes(adapter)) {
      sendError(
        res,
        `Unknown integration: ${adapter}. Available: ${TRACKED_INTEGRATIONS.join(', ')}`,
        404,
      );
      return;
    }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (webhookSecret && !signature) {
        res
          .status(401)
          .json({ error: 'Webhook signature required: X-Hub-Signature-256 header missing' });
        return;
      }

      const payload = req.body as Record<string, unknown>;

      const event = await services.jira.handleWebhookEvent(
        payload,
        rawBody,
        signature || undefined,
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (webhookSecret && !signature) {
        res
          .status(401)
          .json({ error: 'Webhook signature required: X-PagerDuty-Signature header missing' });
        return;
      }

      const payload = req.body as Record<string, unknown>;

      const event = await services.pagerduty.handleWebhookEvent(
        payload,
        rawBody,
        signature || undefined,
      );
      ...
      if (event) {
        logger.info(
          { eventType: event.eventType, incidentId: event.incident.id, source: 'pagerduty' },
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (signingSecret && (!signature || !timestamp)) {
        res.status(401).json({
          error:
            'Slack signature headers required: X-Slack-Signature and X-Slack-Request-Timestamp',
        });
        return;
      }
...
router.post(
  '/webhooks/inbound/slack/interactions',
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const interactionPayload = payloadStr
        ? (JSON.parse(
            payloadStr,
          ) as import('@szl-holdings/services').SlackInteractiveMessagePayload)
...
router.post(
  '/webhooks/inbound/slack/commands',
  validateBody(bodyShape({})),
  async (req: Request, res: Response) => {
    try {
      const rawBody =
        (req as RequestWithRawBody).rawBody?.toString('utf-8') ??
...
router.post(
  '/webhooks/inbound/salesforce/cdc',
  validateBody(bodyShape({})),
  async (req: Request, res: Response) => {
    try {
      const rawBody = extractRawBody(req);
      ...
      const signature =
        (req.headers['x-salesforce-signature'] as string | undefined) ??
      ...
      if (webhookSecret && !signature) {
        res
          .status(401)
          .json({ error: 'Webhook signature required: X-Salesforce-Signature header missing' });
        return;
      }

      const payload = req.body as Record<string, unknown>;

      const event = await services.salesforce.processCdcEvent(
        payload,
        rawBody,
        signature || undefined,
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        ...
        'SIEM Splunk HEC ingestion complete',
      );
...
router.post(
  '/webhooks/inbound/siem/sentinel',
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        { accepted: result.accepted, correlatedAlerts: result.correlatedAlerts.length },
        'SIEM Sentinel ingestion complete',
      );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const incident = await services.pagerduty.createIncident({
        title,
        serviceId,
        fromEmail,
        urgency,
  ...
  sendSuccess(res, {
    inboundWebhooks: {
      ...
      siem: {
        ...
        authentication:
          'Bearer token via Authorization header — set SIEM_INGEST_TOKEN env var to enforce',
    ...
    signatureVerification: {
      ...
      gracefulDegradation:
        'Signature verification is skipped when the webhook secret env var is not configured',
    },
    credentialEnvVars: {
      jira: {
