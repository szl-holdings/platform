...
async function resolveOrgMembership(orgSlug: string, userId: number, minRole: "owner" | "admin" | "member" | "viewer" = "viewer") {
  const [org] = await db
    .select()
...
router.get("/orgs/:orgSlug/profile", readLimiter, authMiddleware(), async (req: Request, res: Response) => {
  try {
...
router.get("/orgs/:orgSlug/members", readLimiter, authMiddleware(), async (req: Request, res: Response) => {
  try {
    const orgSlug = req.params["orgSlug"] as string;
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!isElevated(req)) {
        ...
        if (!requesterMembership || (ORG_ROLE_HIERARCHY[requesterMembership.role] ?? 0) < ORG_ROLE_HIERARCHY["admin"]) {
          sendForbidden(res, "Admin access required");
          return;
        }
      }
...
router.get("/orgs/:orgSlug/notification-prefs", readLimiter, authMiddleware(), async (req: Request, res: Response) => {
  try {
    const orgSlug = req.params["orgSlug"] as string;
    const [org] = await db
      .select({ id: organizationsTable.id })
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { rows: existingRows } = await pool.query<{
        ...
        slack_enabled: boolean; in_app_enabled: boolean;
      }>(`SELECT email_enabled, sms_enabled, slack_enabled, in_app_enabled FROM org_notification_settings WHERE org_id = $1`, [org.id]);
...
router.post("/user/password-reset/confirm", writeLimiter, validateBody(bodyShape({
  ...
  try {
    ...
    await db.insert(notificationsTable).values({
      ...
      message: "Your password has been reset successfully. All other sessions have been signed out.",
    });

    logger.info({ userId: user.id, email: user.email }, "[user-lifecycle] Password reset confirmed and sessions invalidated");
...
router.get("/user/notification-preferences", readLimiter, authMiddleware(), async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
