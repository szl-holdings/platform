...
Respond in valid JSON with this exact structure:
{
  ...
  try {
      ...
      [
        {
          role: 'system',
          content:
            'You are a professional meeting analyst. Always respond with valid JSON only. No extra text.',
        },
    ...
    logger.warn({ err }, 'continuum-meetings: AI extraction failed, using defaults');
  }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!transcript && !recordingUrl) {
        sendBadRequest(res, 'transcript or recordingUrl is required');
        return;
      }
      ...
      res.status(202).json({
        success: true,
        data: { id, status: 'processing', message: 'Meeting capture initiated' },
      });

      setImmediate(async () => {
        try {
          const sourceText =
            transcript ?? `[Recording URL provided: ${recordingUrl}. Transcript not available.]`;
          ...
          for (const item of extracted.actionItems) {
            ...
            try {
              const sigResult = await pool.query(
               ...
               VALUES ('meeting_capture', 'automated', $1, $2, 'meetings', $3, 'raw', $4, NOW(), NOW()) RETURNING id`,
                [
                  item.title,
                  item.description ??
                    `Action item from meeting: ${title}. Assignee: ${item.assignee ?? 'unassigned'}.`,
              ...
              logger.warn(
                { err: sigErr },
                'continuum-meetings: failed to create continuum signal for action item',
              );
            }
          ...
          logger.info(
            ...
            'continuum-meetings: capture completed',
          );
        } catch (asyncErr) {
          logger.error({ err: asyncErr, meetingId: id }, 'continuum-meetings: async extraction failed');
          await pool.query(
            `UPDATE alloy_meetings SET status = 'failed', updated_at = NOW() WHERE id = $1`,
            [id],
          );
        }
      });
...
router.get('/continuum/meetings/:id', authMiddleware(), async (req: Request, res: Response) => {
  try {
    const [meetingResult, decisionsResult, actionItemsResult] = await Promise.all([
      ...
      pool.query(
        `SELECT * FROM alloy_meeting_decisions WHERE meeting_id = $1 ORDER BY created_at ASC`,
        [req.params.id],
      ),
      pool.query(
        `SELECT * FROM alloy_meeting_action_items WHERE meeting_id = $1 ORDER BY created_at ASC`,
        [req.params.id],
      ),
    ]);
...
router.get(
  '/continuum/meetings/:id/follow-up',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
      const result = await pool.query(
        `SELECT follow_up_draft, title, summary, attendees FROM alloy_meetings WHERE id = $1`,
        [req.params.id],
      );
      if (!result.rows[0]) {
        sendError(res, 'Meeting not found', 404);
      ...
      sendSuccess(res, {
        id: req.params.id,
        followUpDraft: result.rows[0].follow_up_draft,
...
Be concise and actionable.`;
        ...
        [
          {
            role: 'system',
            content:
              'You are an expert executive briefing assistant. Provide practical, concise meeting preparation guidance.',
          },
      ...
      sendSuccess(res, {
        ...
        recentContext: recentMeetings.rows.map((m) => ({
          id: m.id,
          title: m.title,
          summary: m.summary,
        })),
...
router.patch(
  '/continuum/meetings/:id/action-items/:itemId',
  authMiddleware(),
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      const { status, assignee, dueDate } = req.body as {
        status?: string;
        assignee?: string;
        dueDate?: string;
      };
...
router.get(
  '/continuum/meetings/action-items/open',
  authMiddleware(),
