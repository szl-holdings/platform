...
import { validateBody, jobEnqueueSchema, durableJobEnqueueSchema, jobScheduleEnableSchema, validateQuery, listQuerySchema } from "../lib/validation";
...
const VALID_JOB_STATUSES = new Set<DurableJobStatus>([
  "pending", "running", "completed", "failed", "dead_letter", "cancelled", "waiting",
]);
...
router.get("/jobs/status", authMiddleware(), requireRole("ops", "admin"), async (_req, res) => {
  try {
    ...
    const byTypeAccum: Record<string, { completed: number; failed: number; totalDurationMs: number; durationCount: number }> = {};
    for (const job of recent) {
      if (!byTypeAccum[job.type]) byTypeAccum[job.type] = { completed: 0, failed: 0, totalDurationMs: 0, durationCount: 0 };
...
router.post("/jobs/trigger/:type", authMiddleware(), requireRole("ops"), validateBody(bodyShape({})), async (req, res) => {
...
router.get("/jobs/durable/stats", authMiddleware(), requireRole("ops", "admin"), async (_req, res) => {
  try {
    const stats = await durableJobQueue.getStats();
    sendSuccess(res, { timestamp: new Date().toISOString(), ...stats });
  } catch (err) {
...
router.post("/jobs/durable/dead-letter/:jobId/replay", authMiddleware(), requireRole("ops", "admin"), validateBody(bodyShape({})), async (req, res) => {
  try {
    const jobId = String(req.params["jobId"] ?? "");
    const job = await durableJobQueue.replayDeadLetterJob(jobId);
    await logActivity(req, "replay_dead_letter_job", "job", jobId, `Replayed dead letter job: ${jobId}`);
    sendSuccess(res, { id: job.id, type: job.type, status: job.status }, 202);
  } catch (err) {
...
router.post("/jobs/durable/:jobId/cancel", authMiddleware(), requireRole("ops", "admin"), validateBody(bodyShape({})), async (req, res) => {
  try {
    const jobId = String(req.params["jobId"] ?? "");
    const cancelled = await durableJobQueue.cancelJob(jobId);
    if (!cancelled) {
      sendBadRequest(res, `Job ${jobId} is not in a cancellable state (must be pending or waiting)`);
      return;
    }
...
router.post("/jobs/schedules/:name/trigger", authMiddleware(), requireRole("ops", "admin"), validateBody(bodyShape({})), async (req, res) => {
  try {
    const name = String(req.params["name"] ?? "");
    await durableScheduler.triggerNow(name);
    await logActivity(req, "trigger_schedule", "schedule", name, `Manually triggered schedule: ${name}`);
...
router.get("/jobs/durable/dashboard", authMiddleware(), requireRole("ops", "admin"), async (_req, res) => {
  try {
    ...
    if (stats.failureRate > 10) alertsTriggered.push(`High failure rate: ${stats.failureRate.toFixed(1)}%`);
    if (stats.deadLetter > 50) alertsTriggered.push(`Dead letter queue depth: ${stats.deadLetter} entries`);
    if (stats.pending > 500) alertsTriggered.push(`Queue depth critical: ${stats.pending} pending jobs`);
