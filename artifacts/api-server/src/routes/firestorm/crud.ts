...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const [scenario] = await db
        .update(firestormScenariosTable)
...
router.delete(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id as string);
      const [scenario] = await db
        .delete(firestormScenariosTable)
        .where(eq(firestormScenariosTable.id, id))
        .returning();
      if (!scenario) {
        sendNotFound(res, 'Scenario');
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const [assessment] = await db
        .update(firestormAssessmentsTable)
        .set({ ...data, updatedAt: new Date() })
        .where(eq(firestormAssessmentsTable.id, id))
        .returning();
      if (!assessment) {
...
router.delete(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id as string);
      const [assessment] = await db
        .delete(firestormAssessmentsTable)
        .where(eq(firestormAssessmentsTable.id, id))
        .returning();
      if (!assessment) {
        sendNotFound(res, 'Assessment');
...
router.get('/firestorm/simulations/:id', authMiddleware(), async (req, res) => {
  try {
    ...
    const [run] = await db
      .select()
      .from(firestormSimulationRunsTable)
      .where(eq(firestormSimulationRunsTable.id, id));
    if (!run) {
      sendNotFound(res, 'Simulation Run');
      return;
    }
    sendSuccess(res, run);
  } catch (err) {
    ...
    try {
      ...
      const query = assessmentId
        ? db
            ...
            .where(eq(firestormFindingsTable.assessmentId, assessmentId))
    ...
    try {
      ...
      if ((data.severity === 'critical' || data.severity === 'high') && confirmedOrOpen) {
        if (!data.remediationOwner) {
          sendError(
            res,
            'Remediation owner is required when creating a critical/high finding in confirmed or open status.',
            422,
        ...
        if (!data.dueDate) {
          sendError(
            res,
            'Due date is required when creating a critical/high finding in confirmed or open status.',
            422,
            'UNPROCESSABLE_ENTITY',
          );
          return;
        ...
        if (!data.recommendation) {
          sendError(
            res,
            'Recommended action is required when creating a critical/high finding in confirmed or open status.',
            422,
            'UNPROCESSABLE_ENTITY',
          );
          return;
      ...
      const [finding] = await db
        ...
        .values({ ...data, auditTrail: [initAuditEntry] })
        .returning();
      broadcastWs('aegis-incidents', 'finding-created', {
        id: finding.id,
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      broadcastWs('aegis-incidents', 'finding-updated', {
        id: finding.id,
        severity: finding.severity,
        status: finding.status,
      });
    ...
    try {
      ...
      const query = assessmentId
        ? db
            ...
            .where(eq(firestormRiskScoresTable.assessmentId, assessmentId))
    ...
    const reports = await Promise.all(
      assessments.map(async (assessment) => {
        const findings = await db
          .select()
          .from(firestormFindingsTable)
          .where(eq(firestormFindingsTable.assessmentId, assessment.id));
        ...
        return {
          ...
          findingsBySeverity: {
            critical: bySeverity.critical.map((f) => ({
              id: f.id,
              title: f.title,
              status: f.status,
            ...
            medium: bySeverity.medium.map((f) => ({
              id: f.id,
              title: f.title,
              status: f.status,
              affectedAsset: f.affectedAsset,
...
router.get('/firestorm/reports/:assessmentId', authMiddleware(), async (req, res) => {
  try {
    ...
    const [assessment] = await db
      .select()
      .from(firestormAssessmentsTable)
      .where(eq(firestormAssessmentsTable.id, assessmentId));
    if (!assessment) {
      sendNotFound(res, 'Assessment');
      return;
    }
    ...
    const simulations = await db
      .select()
      .from(firestormSimulationRunsTable)
      .where(eq(firestormSimulationRunsTable.assessmentId, assessmentId));
