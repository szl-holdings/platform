...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      broadcastWs('aegis-incidents', 'incident-created', {
        id: incident.id,
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        activeStatuses.includes(effectiveStatus)
      ) {
        if (!effectiveAnalyst) {
          sendError(
            res,
            'Assigned analyst is required for critical/high severity incidents in active status.',
            422,
      ...
      if (
        ...
        await db.insert(firestormWorkflowActionsTable).values({
          ...
          entityId: id,
          actionType:
            data.status === 'containment' || data.status === 'remediation'
              ? 'route_to_response'
              : 'escalate',
      ...
      broadcastWs('aegis-incidents', 'incident-updated', {
        id: incident.id,
        severity: incident.severity,
        status: incident.status,
        title: incident.title,
...
router.delete(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id as string);
      const [incident] = await db
        .delete(firestormIncidentsTable)
        .where(eq(firestormIncidentsTable.id, id))
        .returning();
      if (!incident) {
        sendNotFound(res, 'Incident');
...
router.get('/firestorm/vulnerabilities/:id', authMiddleware(), async (req, res) => {
  try {
    ...
    if (!finding) {
      sendNotFound(res, 'Vulnerability');
      return;
    }
    sendSuccess(res, {
      ...finding,
...
router.put(
  '/firestorm/vulnerabilities/:id',
  authMiddleware({ required: true }),
  validateBody(bodyShape({})),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id as string);
      const { status, remediationOwner, dueDate, recommendedAction, recommendation } =
        updateVulnerabilitySchema.parse(req.body);
      const [current] = await db
        .select()
        .from(firestormFindingsTable)
        .where(eq(firestormFindingsTable.id, id));
      ...
      if (
        (effectiveStatus === 'confirmed' || effectiveStatus === 'open') &&
        (current.severity === 'critical' || current.severity === 'high')
      ) {
        if (!effectiveOwner) {
          sendError(
            res,
            'Remediation owner is required for critical/high severity findings when status is confirmed or open.',
            422,
            'UNPROCESSABLE_ENTITY',
          );
          return;
        ...
        if (!effectiveDueDate) {
          sendError(
            res,
            'Due date is required for critical/high severity findings when status is confirmed or open.',
            422,
            'UNPROCESSABLE_ENTITY',
          );
          return;
        ...
        if (!effectiveRecommendation) {
          sendError(
            res,
            'Recommended action is required for critical/high severity findings when status is confirmed or open.',
            422,
            'UNPROCESSABLE_ENTITY',
          );
          return;
      ...
      const updates: Partial<typeof firestormFindingsTable.$inferInsert> & { updatedAt: Date } = {
        updatedAt: new Date(),
      };
...
async function ensureComplianceControlsSeeded(): Promise<void> {
  const existing = await db
    .select({ id: firestormComplianceControlsTable.id })
  ...
  const rows = REFERENCE_COMPLIANCE_CONTROLS.map((c) => ({
    ...
    status: (c.status === 'compliant'
      ...
      : c.status === 'partial'
        ? 'partial'
        : 'not_implemented') as 'implemented' | 'partial' | 'not_implemented',
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const framework: ComplianceFramework = VALID_COMPLIANCE_FRAMEWORKS.includes(
      ...
      const controls = await db
        .select()
        .from(firestormComplianceControlsTable)
        .where(eq(firestormComplianceControlsTable.framework, framework))
        .orderBy(firestormComplianceControlsTable.category);
...
router.put(
  '/firestorm/compliance/:controlId',
  authMiddleware({ required: true }),
  validateBody(bodyShape({})),
  async (req, res) => {
    try {
      ...
      const [existing] = await db
        .select()
        .from(firestormComplianceControlsTable)
        .where(eq(firestormComplianceControlsTable.controlId, controlId));
      if (!existing) {
        sendNotFound(res, 'Compliance Control');
        return;
      }
      ...
      if (
        (effectiveStatus === 'not_implemented' || effectiveStatus === 'partial') &&
        !effectiveOwner
      ) {
        sendError(
          res,
          'Owner assignment is required for non-compliant compliance controls to enable gap routing.',
          422,
          'UNPROCESSABLE_ENTITY',
        );
        return;
      ...
      const auditEntry = {
        at: new Date().toISOString(),
        action: `Updated: ${[status && 'status', owner && 'owner', dueDate && 'dueDate'].filter(Boolean).join(', ')}`,
      };
      const updates: Partial<typeof firestormComplianceControlsTable.$inferInsert> & {
        updatedAt: Date;
      } = { updatedAt: new Date(), auditTrail: [...prevAudit, auditEntry] };
      ...
      const [updated] = await db
        .update(firestormComplianceControlsTable)
        .set(updates)
        .where(eq(firestormComplianceControlsTable.controlId, controlId))
        .returning();
...
router.get(
  '/firestorm/vulnerability-inventory',
  authMiddleware(),
  validateQuery(listQuerySchema),
  async (req, res) => {
    try {
      ...
      const stats = {
        ...
        unowned: filtered.filter(
          (f) => !f.remediationOwner && (f.severity === 'critical' || f.severity === 'high'),
        ).length,
      ...
      sendSuccess(res, {
        ...
        description:
          'Platform-wide vulnerability inventory. firestorm_findings is the canonical vulnerability store; assessment_id links findings to the originating assessment but is not required for platform-level tracking queries.',
        stats,
    ...
    try {
      ...
      const query = status
        ? db
            ...
            .where(
              eq(
                firestormAlertsTable.status,
                status as 'new' | 'acknowledged' | 'investigating' | 'resolved' | 'dismissed',
              ),
            )
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      broadcastWs('aegis-incidents', 'alert-created', {
        id: alert.id,
  ...
  async (req, res) => {
    try {
      ...
      const [alert] = await db
        .update(firestormAlertsTable)
        .set(updates as any)
        .where(eq(firestormAlertsTable.id, id))
        .returning();
      if (!alert) {
        sendNotFound(res, 'Alert');
        return;
      }
      broadcastWs('aegis-incidents', 'alert-updated', {
        id: alert.id,
        severity: alert.severity,
        status: alert.status,
        title: alert.title,
...
router.get('/firestorm/soc-dashboard', authMiddleware(), async (_req, res) => {
  try {
    ...
    const closedIncidents = incidents.filter(
      (i) => i.status === 'closed' && i.resolvedAt && i.detectedAt,
    );
    const mttd =
    ...
    sendSuccess(res, {
      ...
      criticalFindings: findings.filter((f) => f.severity === 'critical' && f.status === 'open')
        .length,
    });
  } catch (err) {
    ...
    try {
      ...
      const cves = (data.vulnerabilities || []).map((v: NvdVulnerability) => {
        ...
        return {
          id: cve.id,
          description:
            cve.descriptions?.find((d: NvdCveDescription) => d.lang === 'en')?.value || '',
          score,
          severity,
          published: cve.published,
          lastModified: cve.lastModified,
          vendor:
            cve.configurations?.[0]?.nodes?.[0]?.cpeMatch?.[0]?.criteria?.split(':')[3] || 'N/A',
          product:
            cve.configurations?.[0]?.nodes?.[0]?.cpeMatch?.[0]?.criteria?.split(':')[4] || 'N/A',
        };
      });
