...
import {
  alloyRuntimeResourceDeleteSchema,
  alloyRuntimeResourceSchema,
  listQuerySchema,
  validateBody,
  validateQuery,
} from '../lib/validation';
...
router.post(
  '/workflows',
  authMiddleware(),
  validateBody(alloyRuntimeResourceSchema),
...
router.delete(
  '/workflows/:workflowId',
  validateBody(alloyRuntimeResourceDeleteSchema),
  authMiddleware(),
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        { newRunId, originalRunId: runId, status: replayState.status },
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const effectiveDomain = (domain ?? 'continuum').trim() || 'continuum';
...
router.post(
  ...
  (req: Request, res: Response) => {
    try {
      ...
      const selected = modelRouter.selectModel({
        task,
        latencyBudgetMs,
        maxCostUsd,
        preferredModel,
...
router.get('/prompts/:promptId/versions', authMiddleware(), (req: Request, res: Response) => {
  try {
    const { promptId } = req.params as { promptId: string };
    if (!inMemoryPrompts.has(promptId)) {
      sendNotFound(res, 'Prompt');
      return;
    }
...
router.get(
  ...
  (req: Request, res: Response) => {
    try {
      const { page, limit, offset } = parsePagination(req.query as Record<string, unknown>);
      const { severity, status, domain } = req.query as {
        severity?: string;
        status?: string;
        domain?: string;
      };
