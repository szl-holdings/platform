...
function requireScope(req: Request, res: Response): { tenantId: string | undefined } | null {
  ...
  if (orgId == null) {
    res
      .status(403)
      .json({ error: 'Forbidden', message: 'No tenant context: user has no org membership' });
    return null;
  }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (includeGraph) {
        ...
        const result = await unifiedSemanticSearch({
          query,
          domains,
          limit,
          includeGraph: true,
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const result = await buildVectorRAGContext(query, {
        domain,
        limit,
        minScore,
        includeGraph,
    ...
    try {
      ...
      if (tenantId) {
        ...
        if (ownedByCallerTenant.length === 0) {
          ...
          const isAdmin = callerRoles.some((role) =>
...
router.get('/centrality', validateQuery(listQuerySchema), async (req, res) => {
  ...
  try {
    ...
    const scores = await computeCentralityScores({
      domain,
      limit: parseIntParam(limit, 50, 1, 200),
      tenantId,
    });
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const entities = await findSimilarEntities(query, {
        entityTypes,
        domains,
        topK,
        minScore,
    ...
    try {
      ...
      if (!entityId || !entityName || !domain || !entityType || !targetDomains?.length) {
        return sendBadRequest(
          res,
          'entityId, entityName, domain, entityType, and targetDomains are required',
        );
      }
...
router.post(
  ...
  async (req, res) => {
    try {
      const { targetTable, targetId, contentColumn, targetColumn, modelId, priority } =
        req.body as {
          targetTable: string;
          targetId: string;
      ...
      if (!targetTable || !targetId)
        return sendBadRequest(res, 'targetTable and targetId are required');
      const allowedTables = getAllowedEmbedTableNames();
      if (!allowedTables.includes(targetTable)) {
        return sendBadRequest(
          res,
          `targetTable "${targetTable}" is not in the embedding allowlist. Allowed: ${allowedTables.join(', ')}`,
        );
      }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const ALLOWED_TABLES = ['rag_knowledge_chunks', 'kg_entities'];

      if (!ALLOWED_TABLES.includes(tableName)) {
        return sendBadRequest(res, `tableName must be one of: ${ALLOWED_TABLES.join(', ')}`);
      }

      const result = await batchEmbedTable(
        tableName,
        idColumn,
        contentColumn,
        embeddingColumn,
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const anchors = await findSimilarEntities(query, {
        domains,
        topK: limit,
        minScore,
        tenantId,
