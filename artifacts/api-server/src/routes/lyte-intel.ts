...
const FUSION_SIGNALS_SEED: FusionSignalSeed[] = [
  { id: "sf1", domain: "Aegis", severity: "critical", title: "KEV CVE-2025-1337 active exploitation — 3 internal hosts confirmed", sourceType: "threat_intelligence", confidence: 0.94, correlatedWith: ["sf4", "sf6"], correlationType: "causal", correlationStrength: 0.87, timestamp: "T-04m", metadata: { "Attack vector": "RCE via log4j derivative", "Affected systems": "3 (auth-svc, api-gw, reporting)", "SLA": "T-2h" } },
  { id: "sf2", domain: "Vessels", severity: "high", title: "MV Adriatic Star — AIS dark gap 6h20m, last fix Strait of Messina", sourceType: "ais_telemetry", confidence: 0.91, correlatedWith: ["sf5"], correlationType: "temporal", correlationStrength: 0.72, timestamp: "T-11m", metadata: { "Gap duration": "6h20m", "Last AIS fix": "37.42N, 15.61E", "OFAC status": "Pending" } },
  { id: "sf3", domain: "PRISM Counsel", severity: "high", title: "Motion HC-2025-0487 — deadline T-38h, no filing draft, no owner", sourceType: "deadline_monitor", confidence: 0.98, correlatedWith: [], timestamp: "T-22m", metadata: { "Matter ID": "HC-2025-0487", "Deadline": "38h remaining", "Owner": "Unassigned" } },
  { id: "sf4", domain: "Alloy", severity: "high", title: "Approval queue depth 14 workflows — 6 exceed 72h threshold", sourceType: "workflow_monitor", confidence: 0.99, correlatedWith: ["sf1"], correlationType: "causal", correlationStrength: 0.63, timestamp: "T-02h", metadata: { "Queue depth": "14 pending", "Over SLA": "6 workflows", "Oldest": "89h 14m" } },
  { id: "sf5", domain: "Terra", severity: "medium", title: "NYC portfolio distress threshold — 12 properties, $340M exposure", sourceType: "market_intelligence", confidence: 0.83, correlatedWith: ["sf2"], correlationType: "semantic", correlationStrength: 0.54, timestamp: "T-01h", metadata: { "Properties in scope": "12", "Estimated exposure": "$340M", "Diligence": "34% complete" } },
  { id: "sf6", domain: "IMPERIUM", severity: "medium", title: "Configuration drift detected — AWS us-east-1 unrestricted egress rule", sourceType: "cloud_policy", confidence: 0.96, correlatedWith: ["sf1"], correlationType: "causal", correlationStrength: 0.51, timestamp: "T-04h", metadata: { "Asset": "sg-0xf823b1a", "Policy": "unrestricted egress", "Region": "us-east-1" } },
  { id: "sf7", domain: "Carlota Jo", severity: "info", title: "Engagement milestone 3 — delivery confirmed, awaiting client sign-off", sourceType: "engagement_tracker", confidence: 0.88, correlatedWith: [], timestamp: "T-03h", metadata: { "Client": "Archipelago Capital", "Milestone": "3 of 6", "Response SLA": "48h" } },
];
...
const SOURCE_TO_DOMAIN: Record<string, string> = {
  aegis: "Aegis", vessels: "Vessels", terra: "Terra",
  prism: "PRISM Counsel", "prism-counsel": "PRISM Counsel",
  alloy: "Alloy", carlota: "Carlota Jo", "carlota-jo": "Carlota Jo",
  imperium: "IMPERIUM",
};
  ...
  try {
    ...
    const liveFusion: FusionSignalSeed[] = liveSignals
      ...
      .map((s, idx) => {
        ...
        const ageMin = Math.max(1, Math.floor((Date.now() - new Date(s.receivedAt).getTime()) / 60000));
        const timestamp = ageMin < 60 ? `T-${String(ageMin).padStart(2, "0")}m` : `T-${Math.floor(ageMin / 60)}h`;
        return {
          id: `live-sf${s.id}`,
          ...
          confidence: typeof meta.confidence === "number" ? meta.confidence : 0.85 + (idx % 3) * 0.04,
          correlatedWith: [],
          timestamp,
          metadata: metaStr,
        };
...
const DOMAIN_HEALTH_SEED: DomainHealthSeed[] = [
  { domain: "Aegis", color: "hsl(222,60%,60%)", iconKey: "Shield", policyCount: 24, activePolicies: 22, pendingApprovals: 3, approvalThroughputPct: 94, overrideRate: 4.2, proofCoverage: 99.1, slaBreaches: 1, maturityScore: 91, trend: "rising", lastReviewed: "2d ago" },
  { domain: "Vessels", color: "hsl(206,72%,54%)", iconKey: "Ship", policyCount: 18, activePolicies: 16, pendingApprovals: 2, approvalThroughputPct: 88, overrideRate: 7.8, proofCoverage: 97.4, slaBreaches: 2, maturityScore: 82, trend: "stable", lastReviewed: "5d ago" },
  { domain: "Terra", color: "hsl(142,52%,48%)", iconKey: "Building2", policyCount: 14, activePolicies: 14, pendingApprovals: 4, approvalThroughputPct: 79, overrideRate: 12.3, proofCoverage: 95.2, slaBreaches: 0, maturityScore: 74, trend: "declining", lastReviewed: "7d ago" },
  { domain: "PRISM Counsel", color: "hsl(260,60%,65%)", iconKey: "Briefcase", policyCount: 21, activePolicies: 21, pendingApprovals: 5, approvalThroughputPct: 97, overrideRate: 2.1, proofCoverage: 99.8, slaBreaches: 0, maturityScore: 97, trend: "rising", lastReviewed: "1d ago" },
  { domain: "Carlota Jo", color: "hsl(340,52%,60%)", iconKey: "Users", policyCount: 11, activePolicies: 11, pendingApprovals: 0, approvalThroughputPct: 100, overrideRate: 0, proofCoverage: 98.7, slaBreaches: 0, maturityScore: 99, trend: "stable", lastReviewed: "2d ago" },
  { domain: "IMPERIUM", color: "hsl(25,72%,54%)", iconKey: "Layers", policyCount: 31, activePolicies: 28, pendingApprovals: 0, approvalThroughputPct: 91, overrideRate: 5.6, proofCoverage: 96.8, slaBreaches: 1, maturityScore: 86, trend: "rising", lastReviewed: "1d ago" },
];
...
router.get("/lyte/governance-domains", noAuth, async (_req, res) => {
  try {
    ...
    const [actionAgg, signalAgg, incidentAgg, actions7d, queueRows, violationRows] = await Promise.all([
      db.select({ pending: sql<number>`count(*) filter (where state in ('new','assigned'))::int`, total: sql<number>`count(*)::int` }).from(lyteActionsTable).catch(() => [{ pending: 0, total: 0 }]),
      db.select({ total: sql<number>`count(*)::int`, sla: sql<number>`count(*) filter (where status = 'new' and received_at <= now() - interval '24 hours')::int` }).from(lyteSignalsTable).catch(() => [{ total: 0, sla: 0 }]),
      db.select({ total: sql<number>`count(*)::int`, open: sql<number>`count(*) filter (where status not in ('resolved','closed'))::int` }).from(lyteIncidentsTable).catch(() => [{ total: 0, open: 0 }]),
      db.select({ total: sql<number>`count(*)::int`, dismissed: sql<number>`count(*) filter (where state = 'dismissed')::int` }).from(lyteActionsTable).where(sql`created_at >= ${sevenDaysAgo}`).catch(() => [{ total: 0, dismissed: 0 }]),
      db.select().from(lyteActionsTable).where(sql`state in ('new','assigned','escalated')`).orderBy(desc(lyteActionsTable.createdAt)).limit(8).catch((): Array<typeof lyteActionsTable.$inferSelect> => []),
      db.select().from(lyteIncidentsTable).orderBy(desc(lyteIncidentsTable.createdAt)).limit(8).catch((): Array<typeof lyteIncidentsTable.$inferSelect> => []),
    ]);
    ...
    const liveOverrideRate = act7d.total > 0 ? parseFloat(((act7d.dismissed / act7d.total) * 100).toFixed(1)) : null;
    const liveProofCoverage = inc.total > 0 ? parseFloat((((inc.total - inc.open) / inc.total) * 100).toFixed(1)) : null;
    ...
    const domains = DOMAIN_HEALTH_SEED.map((d) => {
      const pendingApprovals = livePending > 0 && totalSeedPending > 0
        ? Math.round((d.pendingApprovals / totalSeedPending) * livePending)
        : d.pendingApprovals;
      return {
        ...d,
        pendingApprovals,
        overrideRate: liveOverrideRate != null ? parseFloat((d.overrideRate * 0.5 + liveOverrideRate * 0.5).toFixed(1)) : d.overrideRate,
        proofCoverage: liveProofCoverage != null ? parseFloat(((d.proofCoverage + liveProofCoverage) / 2).toFixed(1)) : d.proofCoverage,
      };
    });
    ...
    const platformMetrics = {
      ...
      avgApprovalThroughput: domains.reduce((a, d) => a + d.approvalThroughputPct, 0) / domains.length,
...
interface SchemaStepSeed { stage: string; action: string; owner: string; gate?: string; timeout?: string; }
...
const SCHEMAS_SEED: DecisionSchemaSeed[] = [
  { id: "sch1", name: "Cyber Incident Response", category: "Security Operations", domain: "Aegis", color: "hsl(222,60%,60%)", iconKey: "Shield",
    ...
    triggerPatterns: ["KEV exploitation confirmed on production asset", "MITRE ATT&CK technique detected (T1071, T1566)", "Identity anomaly score exceeds threshold 0.82", "Critical vulnerability + active threat actor correlation"],
    actionSequence: [
      ...
      { stage: "Execution", action: "Alloy orchestrates isolation workflow with checkpoint recovery", owner: "SOC Lead", gate: "Approver: security_lead" },
    ...
    avgDurationH: 4.2, timesUsed: 34, successRate: 0.94, lastUsed: "3 days ago", tags: ["incident-response", "containment", "mitre", "soc"], complexity: "high" },
  { id: "sch2", name: "Maritime Anomaly Investigation", category: "Fleet Operations", domain: "Vessels", color: "hsl(206,72%,54%)", iconKey: "Ship",
    ...
    triggerPatterns: ["AIS signal gap exceeds 4 hours for flagged route", "Vessel proximity to OFAC-designated entity", "Dark vessel detection from satellite imagery correlation", "Sanctions list match score exceeds 0.75"],
    actionSequence: [
    ...
    avgDurationH: 8.5, timesUsed: 22, successRate: 0.91, lastUsed: "11 days ago", tags: ["maritime", "sanctions", "ofac", "ais", "compliance"], complexity: "medium" },
  { id: "sch3", name: "Distressed Asset Acquisition Review", category: "Investment Operations", domain: "Terra", color: "hsl(142,52%,48%)", iconKey: "Building2",
    ...
    triggerPatterns: ["Distress signal threshold breached (≥3 indicators)", "Ownership transfer filing detected on target property", "Tax lien accumulation above $500K on monitored asset", "Broker network signal: off-market interest confirmed"],
    actionSequence: [
      ...
      { stage: "Execution", action: "Route to LP approval workflow via Alloy with deal memorandum", owner: "Investment Lead", gate: "Approver: investment_committee" },
    ...
    avgDurationH: 168, timesUsed: 12, successRate: 0.88, lastUsed: "21 days ago", tags: ["real-estate", "acquisition", "underwriting", "lp-approval"], complexity: "high" },
  { id: "sch4", name: "Legal Deadline Response Protocol", category: "Legal Operations", domain: "PRISM Counsel", color: "hsl(260,60%,65%)", iconKey: "Briefcase",
    ...
    triggerPatterns: ["Motion deadline within 72h, no filing draft confirmed", "Discovery deadline approach without production status", "Court order deadline detected without acknowledgment", "Settlement demand deadline approaching without response status"],
    actionSequence: [
      { stage: "Signal", action: "Deadline proximity signal from PRISM Counsel matter twin", owner: "System" },
    ...
    avgDurationH: 36, timesUsed: 18, successRate: 0.97, lastUsed: "5 days ago", tags: ["legal", "filing", "deadline", "matter-management"], complexity: "medium" },
  { id: "sch5", name: "Client Engagement Delivery Protocol", category: "Advisory Operations", domain: "Carlota Jo", color: "hsl(340,52%,60%)", iconKey: "Users",
    ...
    triggerPatterns: ["Engagement milestone completion signal from delivery tracker", "Client communication gap exceeds SLA threshold", "Deliverable review deadline approaching without sign-off", "Service agreement renewal window within 30 days"],
    actionSequence: [
    ...
    avgDurationH: 48, timesUsed: 28, successRate: 0.96, lastUsed: "2 days ago", tags: ["advisory", "delivery", "client-management", "engagement"], complexity: "low" },
  { id: "sch6", name: "Cloud Policy Violation Remediation", category: "Infrastructure Governance", domain: "IMPERIUM", color: "hsl(25,72%,54%)", iconKey: "Layers",
    ...
    triggerPatterns: ["Security group policy violation detected (unrestricted ingress/egress)", "Configuration drift from approved baseline", "Unauthorized IAM permission escalation", "Compliance benchmark failure (CIS, SOC 2, ISO 27001)"],
    actionSequence: [
      ...
      { stage: "Execution", action: "Apply remediation via Alloy with automatic rollback gate", owner: "Cloud Ops", gate: "Approver: cloud_ops_lead" },
    ...
    avgDurationH: 2.5, timesUsed: 47, successRate: 0.99, lastUsed: "1 day ago", tags: ["cloud", "compliance", "remediation", "infrastructure"], complexity: "medium" },
];

router.get("/lyte/decision-schemas", noAuth, async (_req, res) => {
  try {
    const counts = await db
      .select({ category: lyteActionsTable.signalCategory, total: sql<number>`count(*)::int`, completed: sql<number>`count(*) filter (where state = 'completed')::int` })
    ...
    for (const row of counts) {
      if (row.category) categoryToStats.set(row.category, { total: row.total, completed: row.completed });
    }
    ...
    const schemas = SCHEMAS_SEED.map((s) => {
      ...
      const successRate = liveStats.completed > 0 ? Math.min(0.99, liveStats.completed / liveStats.total) : s.successRate;
      return { ...s, timesUsed: s.timesUsed + liveStats.total, successRate: parseFloat(successRate.toFixed(2)) };
    });
