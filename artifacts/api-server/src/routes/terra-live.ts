...
function getCached<T>(key: string, ttlMs: number, fetcher: () => Promise<{ data: T; source: string }>): Promise<{ data: T; source: string; cacheAgeSeconds: number; isStale: boolean }> {
  ...
  if (cached && cached.expiry > now) {
    return Promise.resolve({ data: cached.data as T, source: cached.source, cacheAgeSeconds: Math.floor((now - cached.fetchedAt) / 1000), isStale: false });
  }
  return fetcher().then(({ data, source }) => {
    ...
    if (stale) return { data: stale.data as T, source: "stale-" + stale.source, cacheAgeSeconds: Math.floor((now - stale.fetchedAt) / 1000), isStale: true };
...
router.get("/terra/live/census-housing", terraLiveLimit, authMiddleware({ required: false }), async (req, res) => {
  try {
    ...
    const filtered = msaCode ? result.data.stats.filter((s: any) => s.msaCode === msaCode) : result.data.stats;
    sendSuccess(res, {
  ...
  try {
    ...
    const result = await getCached<any>(cacheKey, 6 * 3600000, async () => {
      try {
        ...
        const avgYear = withYear.length > 0 ? Math.round(withYear.reduce((s, p) => s + +p.yearbuilt, 0) / withYear.length) : null;
        const avgAssessed = raw.filter(p => +p.assesstot > 0).length > 0
          ? Math.round(raw.filter(p => +p.assesstot > 0).reduce((s, p) => s + +p.assesstot, 0) / raw.filter(p => +p.assesstot > 0).length)
          : null;

        const landUseMap: Record<string, string> = { "01": "One & Two Family", "02": "Multi-Family Walkup", "03": "Multi-Family Elevator", "04": "Mixed Residential/Commercial", "05": "Commercial/Office", "06": "Industrial/Manufacturing", "07": "Transportation/Utility", "08": "Public Facilities", "09": "Open Space/Recreation", "10": "Parking", "11": "Vacant Land" };
        ...
        return {
          data: {
            ...
            landUseBreakdown: { "Multi-Family Elevator": 22, "One & Two Family": 8, "Mixed Residential/Commercial": 10, "Commercial/Office": 6, "Vacant Land": 2 },
    ...
    const result = await getCached<any>(`terra-nyc-311-${neighborhood}`, 60 * 60 * 1000, async () => {
      try {
        ...
        const avgResponseHours = raw
          ...
          .slice(0, 20)
          .map(c => (new Date(c.closed_date).getTime() - new Date(c.created_date).getTime()) / 3600000)
  ...
  try {
    ...
    const result = await getCached<any>(`terra-census-acs-${state}-${county}`, 86400000 * 7, async () => {
      try {
        ...
        const num = (field: string) => { const v = +get(field); return isNaN(v) || v < 0 ? null : v; };
        ...
        return {
          data: {
            ...
            unemploymentRate: laborForce && unemployed ? +((unemployed / laborForce) * 100).toFixed(1) : null,
            bachelorsDegreeRate: bachTotal && bachPlus ? +((bachPlus / bachTotal) * 100).toFixed(1) : null,
            ownerOccupancyRate: totalOcc && owners ? +((owners / totalOcc) * 100).toFixed(1) : null,
            povertyRate: povertyTotal && povertyCount ? +((povertyCount / povertyTotal) * 100).toFixed(1) : null,
          },
