...
const HANDOFF_DEFINITIONS: Record<HandoffContractType, Omit<HandoffContract, "id" | "payload" | "correlationId" | "tenantId" | "timestamp" | "status">> = {
  lyte_priority_to_forge: {
  ...
  aegis_threat_to_covenant: {
    ...
    governedAction: "COVENANT enforces policy-gated response: containment, escalation, or mitigation",
    severity: "critical",
  },
  vessels_voyage_to_forge: {
    ...
    governedAction: "FORGE creates command workflow for rerouting decision with compliance audit trail",
    severity: "high",
  },
  terra_blocker_to_carlota: {
  ...
  holdings_investor_to_atlas: {
    ...
    governedAction: "ATLAS generates investor-appropriate artifact package (deck, data room, proof chain)",
    severity: "medium",
  },
};
...
router.get("/cross-app/handoffs/stats", authMiddleware(), (_req: Request, res: Response) => {
  try {
    const byType: Record<string, { total: number; routed: number; executed: number; failed: number }> = {};
    ...
    const successRate = totalHandoffs > 0
      ? Math.round((handoffHistory.filter(h => h.status === "executed").length / totalHandoffs) * 100)
      : 0;
    ...
    sendSuccess(res, {
      ...
      familyHealth: {
        lyteForge: byType["lyte_priority_to_forge"] ?? { total: 0, executed: 0, routed: 0, failed: 0 },
        aegisCovenant: byType["aegis_threat_to_covenant"] ?? { total: 0, executed: 0, routed: 0, failed: 0 },
        vesselsForge: byType["vessels_voyage_to_forge"] ?? { total: 0, executed: 0, routed: 0, failed: 0 },
        terraCarlota: byType["terra_blocker_to_carlota"] ?? { total: 0, executed: 0, routed: 0, failed: 0 },
        holdingsAtlas: byType["holdings_investor_to_atlas"] ?? { total: 0, executed: 0, routed: 0, failed: 0 },
      },
    });
  } catch (err) {
...
router.post("/cross-app/handoffs/trigger", authMiddleware(), async (req: Request, res: Response) => {
  try {
    ...
    if (!definition) {
      sendBadRequest(res, `Unknown handoff type: ${type}. Valid types: ${Object.keys(HANDOFF_DEFINITIONS).join(", ")}`);
      return;
    }
...
router.get("/cross-app/family/health", authMiddleware(), (_req: Request, res: Response) => {
  try {
    ...
    const totalHandoffs = handoffHistory.length;
    const recentHandoffs = handoffHistory.filter(h => h.timestamp > Date.now() - 24 * 60 * 60 * 1000);
