...
analyticsEngineRouter.post(
  '/analytics-engine/events/batch',
...
analyticsEngineRouter.get('/analytics-engine/metrics', async (req: Request, res: Response) => {
  try {
    ...
    const metrics = await db
      ...
      .orderBy(
        asc(analyticsMetricDefinitionsTable.domain),
        asc(analyticsMetricDefinitionsTable.name),
      );
...
analyticsEngineRouter.post('/analytics-engine/metrics', async (req: Request, res: Response) => {
  try {
    ...
    const [created] = await db
      .insert(analyticsMetricDefinitionsTable)
      .values({
        ...body,
        ...
        name: body.name,
        calculationType:
          body.calculationType as (typeof analyticsMetricDefinitionsTable.$inferInsert)['calculationType'],
        filterConditions: (req.body.filterConditions ??
          []) as (typeof analyticsMetricDefinitionsTable.$inferInsert)['filterConditions'],
...
analyticsEngineRouter.get(
  '/analytics-engine/metrics/:metricId/query',
  async (req: Request, res: Response) => {
    try {
      ...
      const granularity =
        (granularityParam as 'minute' | 'hour' | 'day' | 'week' | 'month' | undefined) ??
        ...
        .where(
          and(
            ...
            eq(
              ...
              granularity as (typeof analyticsMetricSnapshotsTable.$inferSelect)['granularity'],
            ),
...
analyticsEngineRouter.get('/analytics-engine/funnels', async (req: Request, res: Response) => {
  try {
    ...
    const funnels = await db
      .select()
      .from(analyticsFunnelDefinitionsTable)
      .where(conditions.length > 0 ? and(...conditions) : undefined);
    res.json({ funnels });
  } catch (err) {
...
analyticsEngineRouter.post('/analytics-engine/funnels', async (req: Request, res: Response) => {
  try {
    ...
    const [created] = await db
      .insert(analyticsFunnelDefinitionsTable)
      .values({
...
analyticsEngineRouter.get(
  '/analytics-engine/funnels/:funnelId',
  async (req: Request, res: Response) => {
    try {
...
analyticsEngineRouter.get('/analytics-engine/cohorts', async (req: Request, res: Response) => {
  try {
    ...
    const cohorts = await db
      .select()
      .from(analyticsCohortDefinitionsTable)
      .where(conditions.length > 0 ? and(...conditions) : undefined);
    res.json({ cohorts });
  } catch (err) {
...
analyticsEngineRouter.post('/analytics-engine/cohorts', async (req: Request, res: Response) => {
  try {
    ...
    const [created] = await db
      .insert(analyticsCohortDefinitionsTable)
      .values({
        ...
        entryConditions: (req.body.entryConditions ??
          []) as (typeof analyticsCohortDefinitionsTable.$inferInsert)['entryConditions'],
...
analyticsEngineRouter.get('/analytics-engine/anomalies', async (req: Request, res: Response) => {
  try {
    ...
    if (severity)
      conditions.push(
        eq(
          analyticsAnomaliesTable.severity,
          severity as (typeof analyticsAnomaliesTable.$inferSelect)['severity'],
        ),
      );
    if (resolved !== undefined)
      conditions.push(eq(analyticsAnomaliesTable.isResolved, resolved === 'true'));
...
analyticsEngineRouter.patch(
  '/analytics-engine/anomalies/:anomalyId/resolve',
  async (req: Request, res: Response) => {
    try {
      const { anomalyId } = req.params as Record<string, string>;
      await db
        .update(analyticsAnomaliesTable)
...
analyticsEngineRouter.patch(
  '/analytics-engine/anomalies/:anomalyId/suppress',
  async (req: Request, res: Response) => {
    try {
      const { anomalyId } = req.params as Record<string, string>;
      await db
...
analyticsEngineRouter.post('/analytics-engine/dashboards', async (req: Request, res: Response) => {
  try {
    ...
    const [created] = await db
      .insert(analyticsDashboardsTable)
      .values({
...
analyticsEngineRouter.get(
  '/analytics-engine/dashboards/:dashboardId',
  async (req: Request, res: Response) => {
    try {
  ...
  try {
    ...
    const [_exportJob] = await db
      ...
      .values({
        ...
        scheduleFrequency:
          scheduleFrequency as (typeof analyticsExportJobsTable.$inferInsert)['scheduleFrequency'],
    ...
    jobQueue
      .enqueue(JOB_TYPES.ANALYTICS_EXPORT, {
        exportId,
        domain,
        exportType,
        format,
...
analyticsEngineRouter.get(
  '/analytics-engine/export/:exportId',
  async (req: Request, res: Response) => {
    try {
...
analyticsEngineRouter.get(
  '/analytics-engine/export/:exportId/download',
  async (req: Request, res: Response) => {
    try {
      const { exportId } = req.params as Record<string, string>;
      ...
      const { from, to } = {
        from: new Date(
          (job.filterParams as Record<string, string>)?.from ?? Date.now() - 30 * 24 * 3600 * 1000,
        ),
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="analytics_export_${exportId}${ext}"`,
      );
      res.send(content);
    } catch (err) {
...
analyticsEngineRouter.get('/analytics-engine/attribution', async (req: Request, res: Response) => {
  try {
    ...
    const channelCounts = new Map<
      string,
      { firstTouch: number; lastTouch: number; total: number }
    >();
    ...
    const channelSummary = Array.from(channelCounts.entries())
      .map(([channel, counts]) => ({
        channel,
        ...counts,
      }))
