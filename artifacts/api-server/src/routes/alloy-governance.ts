...
function canWriteForOrg(user: AuthenticatedUser | undefined, orgId: number | null | undefined): boolean {
  if (!user) return false;
  ...
  try {
    ...
    if (!isGlobalAdmin(user)) {
      if (orgIds.length > 0) {
        ...
        whereClauses.push(
          sql`("platform_alloy_policies"."org_id" IN (${sql.join(orgIds.map(id => sql`${id}`), sql`, `)}) OR ("platform_alloy_policies"."org_id" IS NULL AND "platform_alloy_policies"."kind" = 'compliance_template'))`
        );
      } else {
        // No org memberships: only global compliance templates visible
        whereClauses.push(and(isNull(alloyPoliciesTable.orgId), eq(alloyPoliciesTable.kind, "compliance_template")));
      }
    }
    if (kindFilter) whereClauses.push(eq(alloyPoliciesTable.kind, kindFilter as "approval_matrix" | "model_routing" | "cost_control" | "agent_permission" | "compliance_template"));
    if (statusFilter) whereClauses.push(eq(alloyPoliciesTable.status, statusFilter as "active" | "draft" | "archived"));
...
router.post("/alloy/policies", authMiddleware(), requireRole("admin"), validateBody(bodyShape({
...
router.delete("/alloy/policies/:id", validateBody(bodyShape({})), authMiddleware(), requireRole("admin"), async (req, res) => {
  try {
    const id = parseInt(String(req.params.id as string));
    if (isNaN(id)) return sendBadRequest(res, "Invalid policy ID");
    const user = req.user as AuthenticatedUser | undefined;
    ...
    await db.update(alloyPoliciesTable)
...
router.post("/alloy/policies/:id/apply", authMiddleware(), requireRole("admin"), validateBody(bodyShape({
      "orgId": z.unknown().optional(),
    })), async (req, res) => {
  try {
    const id = parseInt(String(req.params.id as string));
    ...
    const [template] = await db
      .select()
      .from(alloyPoliciesTable)
      .where(and(eq(alloyPoliciesTable.id, id), eq(alloyPoliciesTable.kind, "compliance_template")))
      .limit(1);

    if (!template) return sendNotFound(res, "Compliance template not found");

    const [applied] = await db.insert(alloyPoliciesTable).values({
      orgId,
      name: template.name,
...
router.get("/alloy/governance/incidents", authMiddleware(), validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    if (!isGlobalAdmin(user)) {
      if (orgIds.length > 0) {
        incidentWhere.push(sql`"platform_governance_incidents"."org_id" IN (${sql.join(orgIds.map(id => sql`${id}`), sql`, `)})`);
      } else {
...
router.post("/alloy/governance/incidents", authMiddleware(), validateBody(bodyShape({
      "orgId": z.unknown().optional(),
    })), async (req, res) => {
  try {
    const user = req.user as AuthenticatedUser | undefined;
...
router.patch("/alloy/governance/incidents/:id/resolve", authMiddleware(), validateBody(bodyShape({
...
router.post("/alloy/governance/enforce", authMiddleware(), validateBody(bodyShape({
  ...
  try {
    ...
    const subjectRoles: string[] = req.isInternalAgent && Array.isArray(callerRoles)
    ...
    for (const policy of activePolicies) {
      ...
      if (policy.kind === "cost_control") {
        ...
        if (pct >= hardStopPct) {
          ...
          violations.push({ policyId: policy.id, policyName: policy.name, reason: hardBlockReason });
          await db.insert(alloyGovernanceIncidentsTable).values({
            orgId,
            policyId: policy.id,
      ...
      if (policy.kind === "model_routing" && model) {
        ...
        if (blocked.includes(model) || blockedByAllowlist) {
          ...
          await db.insert(alloyGovernanceIncidentsTable).values({
            ...
            metadata: { action, model, agentId, blockedByAllowlist, allowedModels: allowed, blockedModels: blocked },
          }).catch(() => {});
        }
        const ceiling = rules.costCeilingPerCall as number | undefined;
        if (ceiling && estimatedCostCents && estimatedCostCents / 100 > ceiling) {
          violations.push({ policyId: policy.id, policyName: policy.name, reason: `Estimated cost $${(estimatedCostCents / 100).toFixed(2)} exceeds ceiling $${ceiling}` });
        }
      }
      ...
      if (policy.kind === "agent_permission" && agentId) {
        ...
        if (restricted.includes(agentId)) {
          ...
          if (!hasAccess) {
            violations.push({ policyId: policy.id, policyName: policy.name, reason: `Agent ${agentId} requires roles: ${allowedRoles.join(", ")}` });
            await db.insert(alloyGovernanceIncidentsTable).values({
              orgId,
              policyId: policy.id,
              workflowRunId: workflowRunId ?? null,
...
router.post("/alloy/usage/events", authMiddleware(), validateBody(bodyShape({
      "orgId": z.unknown().optional(),
    })), async (req, res) => {
  try {
    const user = req.user as AuthenticatedUser | undefined;
  ...
  try {
    ...
    if (!isGlobalAdmin(user)) {
      ...
      if (scopedOrgIds.length > 0) {
        usageWhere.push(sql`"platform_usage_events"."org_id" IN (${sql.join(scopedOrgIds.map(id => sql`${id}`), sql`, `)})`);
      } else {
        return sendSuccess(res, []);
      }
...
router.get("/alloy/admin/analytics", authMiddleware(), requireRole("super_admin"), async (req, res) => {
  try {
    const now = new Date();
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    ...
    const avgApprovalLatencyMin = resolvedIncidents7d.length > 0
      ? Math.round(
