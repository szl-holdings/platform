...
async function hasPartnerOrgAccess(userId: number, orgId: number, writeAccess = false): Promise<boolean> {
...
router.get("/partner/accounts", authMiddleware(), requireRole("admin"), async (_req: Request, res: Response) => {
  try {
...
router.post("/partner/accounts", authMiddleware(), validateBody(createPartnerSchema), async (req: Request, res: Response) => {
  try {
    ...
    const [partner] = await db.insert(partnerAccountsTable).values({
      name: body.name,
      slug: body.slug,
    ...
    } as InsertPartnerAccount).returning();
...
router.get("/partner/accounts/:id", authMiddleware(), async (req: Request, res: Response) => {
  try {
    ...
    if (!partner) { sendNotFound(res, "Partner account"); return; }
...
router.patch("/partner/accounts/:id", authMiddleware(), async (req: Request, res: Response) => {
  try {
    const id = parseIdParam(req.params.id);
    if (isNaN(id)) { sendBadRequest(res, "Invalid partner ID"); return; }
    ...
    if (!isAdmin(req)) {
      const membership = await getPartnerForUser(req.user!.id);
      if (!membership || membership.partnerId !== id || !["owner", "admin"].includes(membership.role)) {
        sendForbidden(res);
        return;
      }
    }
    ...
    if (isAdmin(req)) {
      ...
      if (body.maxManagedTenants !== undefined) updates.maxManagedTenants = Number(body.maxManagedTenants);
    ...
    if (!updated) { sendNotFound(res, "Partner account"); return; }
...
router.post("/partner/accounts/:id/tenants", authMiddleware(), validateBody(provisionTenantSchema), async (req: Request, res: Response) => {
  try {
    ...
    if (!isAdmin(req)) {
      const membership = await getPartnerForUser(req.user!.id);
      if (!membership || membership.partnerId !== partnerId || !["owner", "admin"].includes(membership.role)) {
        sendForbidden(res);
        return;
      }
    }
    ...
    const [partner] = await db
      .select({ id: partnerAccountsTable.id, status: partnerAccountsTable.status, maxManagedTenants: partnerAccountsTable.maxManagedTenants })
      .from(partnerAccountsTable)
...
router.post("/partner/accounts/:id/tenants/assign", authMiddleware(), async (req: Request, res: Response) => {
  try {
    const partnerId = parseIdParam(req.params.id);
    if (isNaN(partnerId)) { sendBadRequest(res, "Invalid partner ID"); return; }
    ...
    if (!orgId) { sendBadRequest(res, "orgId is required"); return; }

    // Enforce partner active status and quota before assigning
    const [partnerRecord] = await db
      .select({ status: partnerAccountsTable.status, maxManagedTenants: partnerAccountsTable.maxManagedTenants })
      .from(partnerAccountsTable)
      .where(eq(partnerAccountsTable.id, partnerId))
      .limit(1);

    if (!partnerRecord) { sendNotFound(res, "Partner account"); return; }
    if (partnerRecord.status !== "active") {
      sendBadRequest(res, "Partner account is not active. Only active partners can manage tenants.");
      return;
    }
    ...
    if (currentCount >= partnerRecord.maxManagedTenants) {
      sendBadRequest(res, `Partner has reached the maximum of ${partnerRecord.maxManagedTenants} managed tenants.`);
      return;
    }

    const [org] = await db.select({ id: organizationsTable.id }).from(organizationsTable).where(eq(organizationsTable.id, Number(orgId))).limit(1);
    if (!org) { sendNotFound(res, "Organization"); return; }

    const [assignment] = await db.insert(partnerOrgAssignmentsTable).values({
      partnerId,
      orgId: org.id,
      accessLevel: accessLevel ?? "manage",
      provisionedByUserId: req.user!.id,
    } as InsertPartnerOrgAssignment).onConflictDoUpdate({
...
router.delete("/partner/accounts/:id/tenants/:orgId", authMiddleware(), async (req: Request, res: Response) => {
  try {
    ...
    if (isNaN(partnerId) || isNaN(orgId)) { sendBadRequest(res, "Invalid ID"); return; }
    ...
    await db.delete(partnerOrgAssignmentsTable).where(
      and(eq(partnerOrgAssignmentsTable.partnerId, partnerId), eq(partnerOrgAssignmentsTable.orgId, orgId))
    );
...
router.get("/orgs/:orgId/branding", authMiddleware(), async (req: Request, res: Response) => {
  try {
    ...
    if (!isAdmin(req)) {
      ...
      if (!isMember && !isPartner) { sendForbidden(res); return; }
    }
...
router.put("/orgs/:orgId/branding", authMiddleware(), validateBody(brandingSchema), async (req: Request, res: Response) => {
  try {
    const orgId = parseIdParam(req.params.orgId);
    if (isNaN(orgId)) { sendBadRequest(res, "Invalid org ID"); return; }
    ...
    if (!isAdmin(req)) {
      ...
      const hasOrgAccess = (orgMembership && ["owner", "admin"].includes(orgMembership.role))
    ...
    const upsertData: InsertOrgBranding = {
      ...
      ...(body.emailFooterText !== undefined ? { emailFooterText: body.emailFooterText || null } : {}),
    ...
    const [branding] = await db.insert(orgBrandingTable)
      .values(upsertData)
      .onConflictDoUpdate({
...
router.post("/orgs/:orgId/custom-domains", authMiddleware(), async (req: Request, res: Response) => {
  try {
    const orgId = parseIdParam(req.params.orgId);
    if (isNaN(orgId)) { sendBadRequest(res, "Invalid org ID"); return; }
    ...
    if (!domain || typeof domain !== "string") { sendBadRequest(res, "domain is required"); return; }
    ...
    const [domainRecord] = await db.insert(orgCustomDomainsTable).values({
      orgId,
      domain: normalized,
    ...
    } as InsertOrgCustomDomain).returning();
...
router.post("/orgs/:orgId/custom-domains/:domainId/verify", authMiddleware(), async (req: Request, res: Response) => {
  try {
    const orgId = parseIdParam(req.params.orgId);
    const domainId = parseIdParam(req.params.domainId);
    if (isNaN(orgId) || isNaN(domainId)) { sendBadRequest(res, "Invalid ID"); return; }
    ...
    if (!domainRecord) { sendNotFound(res, "Custom domain"); return; }
    ...
    if (method === "dns_txt") {
      ...
      if (!safeUrl) {
        ...
        await db.update(orgCustomDomainsTable)
...
router.post("/orgs/:orgId/custom-domains/:domainId/activate", authMiddleware(), async (req: Request, res: Response) => {
  try {
    const orgId = parseIdParam(req.params.orgId);
    const domainId = parseIdParam(req.params.domainId);
    ...
    if (domainRecord.status !== "verified") {
      sendBadRequest(res, `Domain must be in 'verified' status before activation (current: ${domainRecord.status}). Call /verify first.`);
      return;
    }
...
router.patch("/orgs/:orgId/custom-domains/:domainId", authMiddleware(), async (req: Request, res: Response) => {
  try {
    const orgId = parseIdParam(req.params.orgId);
    const domainId = parseIdParam(req.params.domainId);
    if (isNaN(orgId) || isNaN(domainId)) { sendBadRequest(res, "Invalid ID"); return; }
    ...
    if (!updated) { sendNotFound(res, "Custom domain"); return; }
...
router.delete("/orgs/:orgId/custom-domains/:domainId", authMiddleware(), async (req: Request, res: Response) => {
  try {
    ...
    await db.delete(orgCustomDomainsTable).where(
      and(eq(orgCustomDomainsTable.id, domainId), eq(orgCustomDomainsTable.orgId, orgId))
    );
...
router.get("/resolve-domain", async (req: Request, res: Response) => {
  try {
    ...
    if (!domain) { sendBadRequest(res, "domain query param required"); return; }
    ...
    const [domainRecord] = await db
      ...
      .from(orgCustomDomainsTable)
      .where(and(eq(orgCustomDomainsTable.domain, domain), eq(orgCustomDomainsTable.status, "active")))
      .limit(1);
    ...
    const [org] = await db
      .select({ id: organizationsTable.id, slug: organizationsTable.slug, name: organizationsTable.name })
      .from(organizationsTable)
    ...
    const [branding] = await db
      .select()
      .from(orgBrandingTable)
      .where(and(eq(orgBrandingTable.orgId, domainRecord.orgId), eq(orgBrandingTable.isActive, true)))
      .limit(1);
