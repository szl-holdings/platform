  ...
  try {
    ...
    const sourceTypeList = sourceTypes
      ? (String(sourceTypes)
          .split(',')
          .map((s) => s.trim())
          .filter(Boolean) as RagSourceType[])
      : undefined;
...
ragKnowledgeRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        { docId, chunksCreated: result.chunksCreated },
        'Document ingested into RAG knowledge base',
      );
...
ragKnowledgeRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { id, title, description, severity, status, attackTechnique, notes, detectedAt } =
        req.body;
...
ragKnowledgeRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { entryId, type, domain, sourceAgent, title, summary, confidence, tags, timestamp } =
        req.body;
...
ragKnowledgeRouter.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const deleted = await deleteChunksByObjectId(
        objectId,
        sourceType as RagSourceType | undefined,
      );
