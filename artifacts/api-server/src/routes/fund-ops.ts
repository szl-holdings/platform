...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (companySlug)
        query = query.where(
          eq(fundPortfolioFinancialsTable.companySlug, companySlug),
        ) as typeof query;
...
router.get('/fund-ops/portfolio-financials/:id', ...auth, async (req, res) => {
  try {
    ...
    if (!row) {
      sendNotFound(res, 'Portfolio financial record');
      return;
    }
    sendSuccess(res, row);
  } catch (err) {
...
router.patch(
  '/fund-ops/portfolio-financials/:id',
  ...auth,
  validateBody(bodyShape({})),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(fundPortfolioFinancialsTable)
...
router.delete(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .delete(fundPortfolioFinancialsTable)
        .where(eq(fundPortfolioFinancialsTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Portfolio financial record');
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (companySlug)
        query = query.where(eq(fundPortfolioKpisTable.companySlug, companySlug)) as typeof query;
      const rows = await query
        .orderBy(desc(fundPortfolioKpisTable.periodStart))
        .limit(limit)
        .offset(offset);
      const [{ total }] = await db
        .select({ total: sql<number>`count(*)::int` })
...
router.patch(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(fundPortfolioKpisTable)
        .set({ ...req.body, updatedAt: new Date() })
        .where(eq(fundPortfolioKpisTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Portfolio KPI record');
        return;
      }
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const rows = await db
        .select()
        .from(fundFormDFilingsTable)
        .orderBy(desc(fundFormDFilingsTable.createdAt))
        .limit(limit)
        .offset(offset);
      const [{ total }] = await db
        .select({ total: sql<number>`count(*)::int` })
...
router.patch(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(fundFormDFilingsTable)
        .set({ ...req.body, updatedAt: new Date() })
        .where(eq(fundFormDFilingsTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Form D filing');
        return;
      }
...
router.get('/fund-ops/accredited-investors/:id', ...auth, async (req, res) => {
  try {
    ...
    const [row] = await db
      .select()
      .from(fundAccreditedInvestorsTable)
      .where(eq(fundAccreditedInvestorsTable.id, id));
    if (!row) {
      sendNotFound(res, 'Accredited investor');
      return;
    }
    const account = await db
      .select()
      .from(fundLpCapitalAccountsTable)
      .where(eq(fundLpCapitalAccountsTable.lpId, id));
...
router.patch(
  '/fund-ops/accredited-investors/:id',
  ...auth,
  validateBody(bodyShape({})),
  async (req, res) => {
    try {
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (req.body.status === 'distributed' && !req.body.distributedAt)
        update.distributedAt = new Date();
      const [row] = await db
        .update(fundLpReportsTable)
        .set(update)
        .where(eq(fundLpReportsTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'LP report');
        return;
...
router.get('/fund-ops/share-classes', ...auth, async (req, res) => {
  try {
    const rows = await db
      .select()
      .from(fundShareClassesTable)
      .orderBy(asc(fundShareClassesTable.seniority));
...
router.patch(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(fundShareClassesTable)
        .set({ ...req.body, updatedAt: new Date() })
        .where(eq(fundShareClassesTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Share class');
        return;
      }
...
router.get('/fund-ops/cap-table-holders', ...auth, async (req, res) => {
  try {
    const rows = await db
      .select()
      .from(fundCapTableHoldersTable)
      .orderBy(asc(fundCapTableHoldersTable.name));
    sendSuccess(res, rows);
  } catch (err) {
...
router.patch(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(fundCapTableHoldersTable)
        .set({ ...req.body, updatedAt: new Date() })
        .where(eq(fundCapTableHoldersTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Cap table holder');
        return;
      }
...
router.get(
  '/fund-ops/cap-table-transactions',
  ...auth,
  validateQuery(listQuerySchema),
  async (req, res) => {
    try {
      ...
      if (holderId)
        query = query.where(eq(fundCapTableTransactionsTable.holderId, holderId)) as typeof query;
      const rows = await query
        .orderBy(desc(fundCapTableTransactionsTable.transactionDate))
        .limit(limit)
        .offset(offset);
      const [{ total }] = await db
        .select({ total: sql<number>`count(*)::int` })
        .from(fundCapTableTransactionsTable);
      sendSuccess(res, rows, 200, { page, limit, total });
    } catch (err) {
...
router.get('/fund-ops/cap-table-summary', ...auth, async (req, res) => {
  try {
    const holders = await db
      .select()
      .from(fundCapTableHoldersTable)
      .where(eq(fundCapTableHoldersTable.isActive, true));
    const shareClasses = await db
      .select()
      .from(fundShareClassesTable)
      .where(eq(fundShareClassesTable.isActive, true));
    const transactions = await db
      .select()
      .from(fundCapTableTransactionsTable)
      .orderBy(asc(fundCapTableTransactionsTable.transactionDate));
    ...
    for (const txn of transactions) {
      ...
      if (fromId && isDecrease) {
        holderShareMap[fromId][txn.shareClassId] =
          (holderShareMap[fromId][txn.shareClassId] ?? 0) - shares;
      } else {
        holderShareMap[txn.holderId][txn.shareClassId] =
          (holderShareMap[txn.holderId][txn.shareClassId] ?? 0) + shares;
      }
    }
...
router.get('/fund-ops/vesting-schedules', ...auth, async (req, res) => {
  try {
    const rows = await db
      .select()
      .from(fundVestingSchedulesTable)
      .orderBy(desc(fundVestingSchedulesTable.grantDate));
    sendSuccess(res, rows);
  } catch (err) {
...
router.patch(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(fundVestingSchedulesTable)
        .set({ ...req.body, updatedAt: new Date() })
        .where(eq(fundVestingSchedulesTable.id, id))
        .returning();
      if (!row) {
...
router.get('/fund-ops/capital-calls', ...auth, validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(fundCapitalCallsTable)
      .orderBy(desc(fundCapitalCallsTable.callDate))
      .limit(limit)
      .offset(offset);
    const [{ total }] = await db
      .select({ total: sql<number>`count(*)::int` })
...
router.get('/fund-ops/capital-calls/:id', ...auth, async (req, res) => {
  try {
    ...
    const [call] = await db
      .select()
      .from(fundCapitalCallsTable)
      .where(eq(fundCapitalCallsTable.id, id));
    if (!call) {
      sendNotFound(res, 'Capital call');
      return;
    }
    const lines = await db
      .select()
      .from(fundCapitalCallLinesTable)
      .where(eq(fundCapitalCallLinesTable.capitalCallId, id));
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (req.body.status === 'notices_sent' && !req.body.noticesSentAt)
        update.noticesSentAt = new Date();
      const [row] = await db
        .update(fundCapitalCallsTable)
        .set(update)
        .where(eq(fundCapitalCallsTable.id, id))
        .returning();
      if (!row) {
...
router.patch(
  '/fund-ops/capital-call-lines/:id',
  ...auth,
  validateBody(
    bodyShape({
  ...
  async (req, res) => {
    try {
      ...
      const [row] = await db
        .update(fundCapitalCallLinesTable)
        .set(update)
        .where(eq(fundCapitalCallLinesTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Capital call line');
        return;
      }
      ...
      const allLines = await db
        .select()
        .from(fundCapitalCallLinesTable)
        .where(eq(fundCapitalCallLinesTable.capitalCallId, row.capitalCallId));
      const funded = allLines.reduce((s, l) => s + (l.receivedCents ?? 0), 0);
      await db
        .update(fundCapitalCallsTable)
        .set({ fundedAmountCents: funded, updatedAt: new Date() })
        .where(eq(fundCapitalCallsTable.id, row.capitalCallId));
...
router.get('/fund-ops/lp-capital-accounts', ...auth, async (req, res) => {
  try {
    const rows = await db
      ...
      .leftJoin(
        fundAccreditedInvestorsTable,
        eq(fundLpCapitalAccountsTable.lpId, fundAccreditedInvestorsTable.id),
      )
...
router.patch(
  '/fund-ops/lp-capital-accounts/:id',
  ...auth,
  validateBody(bodyShape({})),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(fundLpCapitalAccountsTable)
        .set({ ...req.body, updatedAt: new Date() })
        .where(eq(fundLpCapitalAccountsTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'LP capital account');
        return;
      }
...
router.get('/fund-ops/distributions', ...auth, validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(fundDistributionsTable)
      .orderBy(desc(fundDistributionsTable.distributionDate))
      .limit(limit)
      .offset(offset);
    const [{ total }] = await db
      .select({ total: sql<number>`count(*)::int` })
...
router.get('/fund-ops/distributions/:id', ...auth, async (req, res) => {
  try {
    ...
    const [dist] = await db
      .select()
      .from(fundDistributionsTable)
      .where(eq(fundDistributionsTable.id, id));
    if (!dist) {
      sendNotFound(res, 'Distribution');
      return;
    }
    const lines = await db
      .select()
      .from(fundDistributionLinesTable)
      .where(eq(fundDistributionLinesTable.distributionId, id));
...
router.get('/fund-ops/audit-log', ...auth, validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(fundCapTableAuditLogTable)
      .orderBy(desc(fundCapTableAuditLogTable.occurredAt))
      .limit(limit)
      .offset(offset);
    const [{ total }] = await db
      .select({ total: sql<number>`count(*)::int` })
  ...
  try {
    const [
      ...
      db
        ...
        .from(fundAccreditedInvestorsTable)
        .where(eq(fundAccreditedInvestorsTable.verificationStatus, 'verified')),
      ...
      db
        .select()
        .from(fundCapitalCallsTable)
        .where(eq(fundCapitalCallsTable.status, 'notices_sent'))
        .orderBy(asc(fundCapitalCallsTable.dueDate))
        .limit(5),
    ]);
