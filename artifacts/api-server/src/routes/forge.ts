    ...
    try {
      ...
      const [agent] = await db
        ...
        .values({
          ...
          orgId: req.user?.orgs?.[0]?.orgId ?? null,
        })
        .returning();
      await writeAudit({
        agentId: agent?.id,
...
router.get('/forge/agents/:id', async (req, res) => {
  try {
    const [agent] = await db
      .select()
      .from(forgeAgentsTable)
      .where(eq(forgeAgentsTable.id, req.params.id as string))
      .limit(1);
    if (!agent) return sendNotFound(res, 'Agent not found');
    const versions = await db
      .select()
      .from(forgeAgentVersionsTable)
      .where(eq(forgeAgentVersionsTable.agentId, agent.id))
      .orderBy(desc(forgeAgentVersionsTable.version));
    const promotions = await db
      .select()
      .from(forgePromotionsTable)
      .where(eq(forgePromotionsTable.agentId, agent.id))
      .orderBy(desc(forgePromotionsTable.createdAt))
      .limit(20);
    const drift = await db
      .select()
      .from(forgeDriftEventsTable)
      .where(eq(forgeDriftEventsTable.agentId, agent.id))
      .orderBy(desc(forgeDriftEventsTable.detectedAt))
      .limit(20);
    const rollbacks = await db
      .select()
      .from(forgeRollbackEventsTable)
      .where(eq(forgeRollbackEventsTable.agentId, agent.id))
      .orderBy(desc(forgeRollbackEventsTable.createdAt))
      .limit(10);
    sendSuccess(res, { agent, versions, promotions, drift, rollbacks });
  } catch (err) {
    handleRouteError(res, err, 'Failed to fetch agent');
  }
});
...
router.get('/forge/agents/:id/versions', async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list versions');
  }
});
...
router.post('/forge/agents/:id/versions', validateBody(createVersionSchema), async (req, res) => {
  try {
    ...
    const [last] = await db
      .select({ v: sql<number>`coalesce(max(${forgeAgentVersionsTable.version}), 0)` })
      .from(forgeAgentVersionsTable)
      .where(eq(forgeAgentVersionsTable.agentId, agentId));
    ...
    const [v] = await db
      ...
      .values({
        ...
        evalsPassed: data.evalsPassed,
        observabilityHookConfigured: data.observabilityHookConfigured,
        provenanceComplete: data.provenanceComplete,
        notes: data.notes,
    ...
    handleRouteError(res, err, 'Failed to create version');
  }
});
...
router.post('/forge/agents/:id/promote', validateBody(promoteSchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to request promotion');
  }
});
...
router.post('/forge/agents/:id/rollback', validateBody(rollbackSchema), async (req, res) => {
  try {
    ...
    const [target] = await db
      .select()
      .from(forgeAgentVersionsTable)
      .where(eq(forgeAgentVersionsTable.id, data.toVersionId))
      .limit(1);
    ...
    if (policy.outcome === 'needs_approval') {
      return sendBadRequest(
        res,
        `Rollback in ${data.envTier} requires human approval (policy: ${policy.reasons.join('; ')})`,
      );
    }
    const result = await rollbackAgent({
      agentId,
      toVersionId: data.toVersionId,
      envTier: data.envTier,
      reason: data.reason,
      triggeredBy: req.user?.id,
    });
    sendSuccess(res, result);
  } catch (err) {
    handleRouteError(res, err, 'Failed to rollback');
  }
});
...
router.post('/forge/agents/:id/execute', validateBody(executeSchema), async (req, res) => {
  try {
    ...
    if (policy.outcome === 'needs_approval' && !data.humanOverride) {
      return sendBadRequest(
        res,
        `Execution requires human approval (policy: ${policy.reasons.join('; ')})`,
      );
    }
    const run = await captureRuntime({
      ...
      valueAtRiskUsd: data.valueAtRiskUsd,
      input: data.input,
      output: data.output,
      trace: data.trace,
    });
    ...
    handleRouteError(res, err, 'Failed to execute agent');
  }
});
...
router.get('/forge/promotions', validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list promotions');
  }
});
...
router.post('/forge/promotions/:id/approve', validateBody(approveSchema), async (req, res) => {
  try {
    ...
    const result = await recordPromotionApproval({
      promotionId: req.params.id as string,
      approverUserId: req.user?.id,
      approverRole: req.user?.roles?.[0],
    ...
    handleRouteError(res, err, 'Failed to approve promotion');
  }
});
...
router.get('/forge/drift/events', validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list drift events');
  }
});
...
router.get('/forge/drift/summary', async (_req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to compute drift summary');
  }
});

router.post(
  '/forge/drift/evaluate',
  validateBody(
    bodyShape({
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to evaluate drift');
    }
  },
);
...
router.get('/forge/executions', validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(forgeExecutionRunsTable)
      .where(where)
      .orderBy(desc(forgeExecutionRunsTable.startedAt))
      .limit(limit)
      .offset(offset);
    sendSuccess(res, rows);
  } catch (err) {
    handleRouteError(res, err, 'Failed to list executions');
  }
});
...
router.get('/forge/executions/:id', async (req, res) => {
  try {
    const [run] = await db
      .select()
      .from(forgeExecutionRunsTable)
      .where(eq(forgeExecutionRunsTable.id, req.params.id as string))
      .limit(1);
    if (!run) return sendNotFound(res, 'Execution not found');
    sendSuccess(res, run);
  } catch (err) {
    handleRouteError(res, err, 'Failed to fetch execution');
  }
});
...
router.get('/forge/telemetry/summary', async (_req, res) => {
  try {
    ...
    const sortedLat = recent
      ...
      .sort((a, b) => a - b);
    const p = (q: number) =>
      sortedLat.length ? sortedLat[Math.floor((sortedLat.length - 1) * q)] : 0;
    sendSuccess(res, {
      total,
      successes,
      failures,
    ...
    handleRouteError(res, err, 'Failed to compute telemetry summary');
  }
});
...
router.get('/forge/policies', async (_req, res) => {
  try {
    sendSuccess(
      res,
      await db.select().from(forgePolicyPacksTable).orderBy(forgePolicyPacksTable.name),
    );
  } catch (err) {
...
router.get('/forge/targets', async (_req, res) => {
  try {
    sendSuccess(
      res,
      await db.select().from(forgeDeploymentTargetsTable).orderBy(forgeDeploymentTargetsTable.name),
    );
  } catch (err) {
    handleRouteError(res, err, 'Failed to list targets');
  }
});
router.get('/forge/models', async (_req, res) => {
  try {
    sendSuccess(res, await db.select().from(forgeModelsTable).orderBy(forgeModelsTable.name));
  } catch (err) {
...
router.get('/forge/tools', async (_req, res) => {
  try {
    sendSuccess(res, await db.select().from(forgeToolsTable).orderBy(forgeToolsTable.name));
  } catch (err) {
...
router.get('/forge/prompts', async (_req, res) => {
  try {
    ...
    for (const v of versions) {
      (byPrompt[v.promptId] ||= []).push(v);
    }
    sendSuccess(
      res,
      prompts.map((p) => ({ ...p, versions: byPrompt[p.id] ?? [] })),
    );
  } catch (err) {
  ...
  try {
    sendSuccess(
      ...
      await db
        ...
        .orderBy(forgeEnvironmentProfilesTable.tier),
    );
  } catch (err) {
    handleRouteError(res, err, 'Failed to list environments');
  }
});
...
router.get('/forge/overview', async (_req, res) => {
  try {
    ...
    const promotionQueue = promos.filter(
      (p) =>
        p.status === 'validated' ||
        p.status === 'blocked' ||
        p.status === 'requested' ||
