...
function lookupMatchedRule(matchedRuleId: unknown): MatchedRuleDetails | null {
  ...
  const rule = getGuardianEngine()
    .getRules()
    .find((r: GuardianRule) => r.id === matchedRuleId);
  if (!rule) return null;
  return {
    id: rule.id,
    name: rule.name,
...
router.get(
  '/approvals',
  authMiddleware(),
  requireRole('super_admin', 'admin', 'ops', 'compliance', 'analyst'),
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { reviewApproval, getApprovalById, ApprovalAccessDeniedError } = await import(
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { escalateApproval, ApprovalAccessDeniedError } = await import(
        '@szl-holdings/covenant-policy'
      );
      let updated;
      try {
      ...
      const persistedTarget = (updated as { escalatedToId?: number | null } | null | undefined)
        ?.escalatedToId;
...
router.get(
  '/approvals/by-resource/:resourceType/:resourceId',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
