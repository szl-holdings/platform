...
function detectContradictions(claims: string[]): { hasContradiction: boolean; positions?: { position: string; source: string }[] } {
...
router.post("/alloy/research/spaces", authMiddleware({ required: false }), validateBody(bodyShape({
  ...
  } catch (err) { handleRouteError(res, err, "Failed to create research space"); }
});

router.get("/alloy/research/spaces", authMiddleware({ required: false }), async (_req: Request, res: Response) => {
  try {
    const result = await pool.query(`SELECT * FROM alloy_research_spaces ORDER BY created_at DESC LIMIT 50`);
    sendSuccess(res, { spaces: result.rows });
  } catch (err) { handleRouteError(res, err, "Failed to list research spaces"); }
});

router.post("/alloy/research/spaces/:id/run", authMiddleware({ required: false }), validateBody(bodyShape({
      "gov": z.unknown().optional(),
    })), async (req: Request, res: Response) => {
  const { id } = req.params as { id: string };
  try {
    const spaceResult = await pool.query(`SELECT * FROM alloy_research_spaces WHERE id = $1`, [id]);
    if (spaceResult.rows.length === 0) { sendError(res, "Research space not found", 404); return; }
    ...
    const sources = [
      { type: "web", url: "https://search.example.com/result-1", title: `${space.name} — Web Analysis`, trustScore: classifySourceTrust("web") },
      { type: "academic", url: "https://papers.example.com/study-2025", title: `Research Paper: ${space.query.slice(0, 40)}`, trustScore: classifySourceTrust("academic") },
      { type: "government", url: "https://data.gov.example/dataset-123", title: `Official Dataset: ${space.name}`, trustScore: classifySourceTrust("government") },
    ];
...
Return ONLY valid JSON.`;
      ...
      if (jsonMatch) {
        const parsed = JSON.parse(jsonMatch[0]) as Array<{
          claim: string;
          confidence: number;
          citations: Array<{ title: string; url: string; sourceType: string; snippet: string; relevanceScore: number }>;
        }>;
    ...
    await pool.query(`UPDATE alloy_research_spaces SET status = 'idle' WHERE id = $1`, [id]).catch(() => {});
    handleRouteError(res, err, "Failed to run research");
  }
});

router.get("/alloy/research/spaces/:id", authMiddleware({ required: false }), async (req: Request, res: Response) => {
  try {
    const { id } = req.params as { id: string };
    const result = await pool.query(`SELECT * FROM alloy_research_spaces WHERE id = $1`, [id]);
    if (result.rows.length === 0) { sendError(res, "Not found", 404); return; }
    sendSuccess(res, result.rows[0]);
  } catch (err) { handleRouteError(res, err, "Failed to get research space"); }
});

router.delete("/alloy/research/spaces/:id", validateBody(bodyShape({})), authMiddleware({ required: false }), async (req: Request, res: Response) => {
  try {
    const { id } = req.params as { id: string };
...
router.post("/alloy/browser/tasks", authMiddleware({ required: false }), validateBody(bodyShape({
  ...
  try {
    ...
    const plannedActions = [
      { step: 1, type: "navigate", description: `Navigate to ${startUrl}`, target: startUrl, requiresApproval: false },
      { step: 2, type: "screenshot", description: "Capture initial page state", requiresApproval: false },
      { step: 3, type: "extract", description: "Extract page structure and content", target: "body", requiresApproval: false },
      { step: 4, type: "navigate", description: "Follow relevant internal links", requiresApproval: false },
      { step: 5, type: "extract", description: `Extract: ${objective.slice(0, 60)}`, requiresApproval: false },
      { step: 6, type: "screenshot", description: "Capture final state as evidence", requiresApproval: false },
    ];
    ...
    sendSuccess(res, { id, name: taskName, startUrl: startUrl.trim(), objective: objective.trim(), status, plannedActions });
  } catch (err) { handleRouteError(res, err, "Failed to create browser task"); }
});

router.get("/alloy/browser/tasks", authMiddleware({ required: false }), async (_req: Request, res: Response) => {
  try {
    const result = await pool.query(`SELECT * FROM alloy_browser_tasks ORDER BY started_at DESC LIMIT 50`);
    sendSuccess(res, { tasks: result.rows });
  } catch (err) { handleRouteError(res, err, "Failed to list browser tasks"); }
});

router.post("/alloy/browser/tasks/:id/execute", authMiddleware({ required: false }), validateBody(bodyShape({})), async (req: Request, res: Response) => {
  try {
    const { id } = req.params as { id: string };
    const taskResult = await pool.query(`SELECT * FROM alloy_browser_tasks WHERE id = $1`, [id]);
    if (taskResult.rows.length === 0) { sendError(res, "Task not found", 404); return; }

    const task = taskResult.rows[0] as {
      id: string; start_url: string; objective: string; planned_actions: unknown[];
    };

    const startMs = Date.now();
    const actions = (task.planned_actions as Array<{ step: number; type: string; description: string; target?: string }>).map((p, i) => ({
      id: `a-${id}-${i}`,
      type: p.type,
      target: p.target,
      ...
      extractedData: p.type === "extract" ? { result: `Extracted content for: ${p.target || "page"}`, from: task.start_url } : undefined,
    }));
...
router.post("/alloy/browser/tasks/:id/pause", authMiddleware({ required: false }), validateBody(bodyShape({})), async (req: Request, res: Response) => {
  try {
    const { id } = req.params as { id: string };
...
router.post("/alloy/browser/tasks/:id/resume", authMiddleware({ required: false }), validateBody(bodyShape({})), async (req: Request, res: Response) => {
  try {
    const { id } = req.params as { id: string };
  ...
  } catch (err) { handleRouteError(res, err, "Failed to resume task"); }
});

router.get("/alloy/browser/allowlist", authMiddleware({ required: false }), async (_req: Request, res: Response) => {
  try {
...
router.post("/alloy/browser/allowlist", authMiddleware({ required: false }), validateBody(bodyShape({
...
router.delete("/alloy/browser/allowlist/:id", validateBody(bodyShape({})), authMiddleware({ required: false }), async (req: Request, res: Response) => {
  try {
    const { id } = req.params as { id: string };
    await pool.query(`DELETE FROM alloy_url_allowlist WHERE id = $1`, [id]);
    sendSuccess(res, { deleted: true });
  } catch (err) { handleRouteError(res, err, "Failed to delete allowlist entry"); }
});
