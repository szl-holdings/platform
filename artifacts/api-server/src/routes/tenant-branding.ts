...
import { azureTenantsTable, tenantBrandingTable, type InsertTenantBranding } from "@szl-holdings/db";
import { eq } from "drizzle-orm";
import { tenantRateLimit } from "./tenant-core";

const router: IRouter = Router();

router.get("/admin/tenants/:id/branding", tenantRateLimit, authMiddleware(), requireRole("admin"), async (req: Request, res: Response) => {
  try {
  ...
  } catch (err) { handleRouteError(res, err, "Failed to get tenant branding"); }
});

router.put("/admin/tenants/:id/branding", tenantRateLimit, authMiddleware(), requireRole("admin"), async (req: Request, res: Response) => {
  try {
    const id = parseInt(String(req.params.id), 10);
    if (isNaN(id)) { sendBadRequest(res, "Invalid tenant ID"); return; }
    const [tenant] = await db.select().from(azureTenantsTable).where(eq(azureTenantsTable.id, id)).limit(1);
    ...
    Object.keys(brandingData).forEach((k) => { const key = k as keyof typeof brandingData; if (brandingData[key] === undefined) delete brandingData[key]; });
    ...
    let branding;
    if (existing) {
      [branding] = await db.update(tenantBrandingTable).set({ ...brandingData, updatedAt: new Date() }).where(eq(tenantBrandingTable.tenantId, id)).returning();
    } else {
      [branding] = await db.insert(tenantBrandingTable).values({ tenantId: id, ...brandingData }).returning();
    }
    sendSuccess(res, { branding, message: "Tenant branding saved" });
  } catch (err) { handleRouteError(res, err, "Failed to save tenant branding"); }
});

router.delete("/admin/tenants/:id/branding", tenantRateLimit, authMiddleware(), requireRole("admin"), async (req: Request, res: Response) => {
  try {
    const id = parseInt(String(req.params.id), 10);
    if (isNaN(id)) { sendBadRequest(res, "Invalid tenant ID"); return; }
  ...
  } catch (err) { handleRouteError(res, err, "Failed to reset tenant branding"); }
});

router.get("/tenant-branding/:azureTenantId", tenantRateLimit, async (req: Request, res: Response) => {
  try {
    const azureTenantId = String(req.params.azureTenantId);
    if (!azureTenantId) { sendBadRequest(res, "azureTenantId is required"); return; }
    const [tenant] = await db.select({ id: azureTenantsTable.id, displayName: azureTenantsTable.displayName, status: azureTenantsTable.status }).from(azureTenantsTable).where(eq(azureTenantsTable.azureTenantId, azureTenantId)).limit(1);
    if (!tenant || tenant.status !== "active") { sendSuccess(res, { branding: null }); return; }
