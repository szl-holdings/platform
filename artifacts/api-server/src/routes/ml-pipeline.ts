...
router.post(
  '/ml/features/compute',
  authMiddleware(),
  validateBody(featureComputeSchema),
  (req, res) => {
    try {
      const { featureId, domain, entityId, entityType, value } = req.body as z.infer<
        typeof featureComputeSchema
      >;
      const result = featureStoreService.computeFeature(
        featureId,
        domain,
        entityId,
        entityType,
...
router.post(
  '/ml/features/vector',
  authMiddleware(),
  validateBody(featureVectorSchema),
  (req, res) => {
    try {
...
router.post('/ml/datasets', authMiddleware(), validateBody(fullDatasetSchema), async (req, res) => {
  try {
    const { name, domain, featureIds, labelColumn, splitStrategy, temporalRange, description } =
      req.body as z.infer<typeof fullDatasetSchema>;
    const ds = await datasetService.create({
      ...
      labelColumn,
      splitStrategy,
      temporalRange: temporalRange as any,
      description,
    });
...
router.post(
  '/ml/training/runs',
  authMiddleware(),
  validateBody(startTrainingRunSchema),
  async (req, res) => {
    try {
      const {
        domain,
        ...
        hyperparameters,
        triggeredBy,
      } = req.body as z.infer<typeof startTrainingRunSchema>;
      const run = await trainingService.startRun({
        domain,
        modelType,
        algorithmFamily,
        datasetId,
...
router.post(
  '/ml/registry/models/:modelVersionId/promote',
  authMiddleware(),
  validateBody(
    bodyShape({
    ...
    try {
      ...
      if (!lifecycle)
        return sendBadRequest(
          res,
          'lifecycle is required (experimental | staging | production | deprecated)',
        );
      const result = modelRegistryService.promote(
        req.params.modelVersionId as string,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await inferenceService.batchPredict({
        domain,
        modelType,
        entities,
        includeExplanation,
...
router.post(
  '/ml/monitoring/run/:modelVersionId',
  authMiddleware(),
  validateBody(bodyShape({})),
  async (req, res) => {
    try {
  ...
  (req, res) => {
    try {
      ...
      if (
        ...
        return sendBadRequest(
          res,
          'domain, modelType, prediction, featureImportance, and featureValues are required',
        );
      }
      res.json(
