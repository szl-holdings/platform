  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const leads = conditions.length
      ? await db
          .select({
            id: dosLeadsTable.id,
            email: dosLeadsTable.email,
            source: dosLeadsTable.source,
            stage: dosLeadsTable.stage,
            createdAt: dosLeadsTable.createdAt,
          })
          .from(dosLeadsTable)
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const [lead] = await db
      ...
      .values({ email, source: source || 'api', stage: 'new', score: 10, tags: [] })
      .returning();
