...
import { adminPinVerifySchema, validateBody } from '../lib/validation';
...
router.post(
  '/config/verify-admin-pin',
  pinLimit,
  validateBody(adminPinVerifySchema),
...
router.get('/config/mapbox-token', configLimit, (_req: Request, res: Response) => {
  ...
  if (!token.startsWith('pk.')) {
    res.json({
      configured: false,
      token: null,
      message: 'A public Mapbox token (pk.*) is required. The current token is not a public token.',
    });
    return;
  }
