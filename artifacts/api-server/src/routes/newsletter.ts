...
router.post("/newsletter/subscribe", validateBody(bodyShape({})), async (req, res) => {
  ...
  try {
    ...
    logger.warn({ email: maskEmail(email), utm_source, status: resp.status, body: text }, "[newsletter] substack returned non-ok status");

    if (resp.status === 400) {
      return res.status(400).json({ message: "This email address could not be subscribed. Please check it and try again." });
    }
    ...
    return res.status(502).json({ message: "Unable to subscribe right now. Please try again later." });
  } catch (err) {
    logger.error({ err, utm_source }, "[newsletter] subscribe fetch error");
    return res.status(500).json({ message: "Unable to subscribe right now. Please try again later." });
  }
});
