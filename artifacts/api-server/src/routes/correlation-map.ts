...
async function fetchLiveEntityData(): Promise<LiveEntityData> {
  ...
  return {
    ...
    vesselHighAlertCount: vesselAlerts.filter((a) => a.severity === "high" || a.severity === "critical").length,
...
function buildGraph(live?: LiveEntityData) {
  ...
  const liveEntitiesCount = live
    ? (live.criticalIncident ? 1 : 0) + (live.vesselHighAlertCount > 0 ? 1 : 0) + (live.distressCount > 0 ? 1 : 0) + live.kgEntityNodes.length
    : 0;
  ...
  const nodes: GraphNode[] = [
    ...
    ...(live?.criticalIncident
      ...
      : [{ id: "entity-incident-0412", label: "INC-2026-0412", type: "entity" as const, domain: "aegis", severity: "critical" as const, description: "Critical cyber incident — 47 assets affected" }]
    ),
    ...
    ...(live?.vesselHighAlertCount
      ? [{
          ...
          description: live.vesselDelayTitle ?? `${live.vesselHighAlertCount} high/critical vessel alerts active`,
          live: true,
        }]
      : [{ id: "entity-vessel-pacific", label: "MV Pacific Star", type: "entity" as const, domain: "vessels", severity: "high" as const, description: "IMO 9876543 — delayed 32h at Shanghai" }]
    ),
    ...
    ...(live?.vesselHighAlertCount
      ...
      : [{ id: "signal-port-delay", label: "Port Delay +32h", type: "signal" as const, domain: "vessels", severity: "high" as const, description: "Threshold exceeded — signal chain active" }]
    ),
    ...(live?.criticalIncident
      ? [{
      ...
      : [{ id: "signal-apt-detected", label: "APT-41 Detected", type: "signal" as const, domain: "aegis", severity: "critical" as const, description: "Critical threshold crossed — legal cascade triggered" }]
    ),
    ...
    ...(live?.distressCount
      ...
      : [{ id: "signal-distress-spike", label: "18 Properties Distressed", type: "signal" as const, domain: "terra", severity: "high" as const, description: "Distress score spike from rate sensitivity" }]
    ),
  ...
  const vesselEntityId = live?.vesselHighAlertCount ? "entity-live-vessel-alert" : "entity-vessel-pacific";
  ...
  const staticEdges: GraphEdge[] = [
    { id: "e1", source: vesselEntityId, target: "entity-port-shanghai", label: "delayed at", strength: 0.95, type: "dependency", description: "High-severity vessel activity correlates with port congestion events", lastActive: Date.now() - 3600000 },
    { id: "e2", source: "entity-port-shanghai", target: distressEntityId, label: "supply chain risk →", strength: 0.87, type: "causal", description: "Port congestion delays construction materials for port-adjacent properties", lastActive: Date.now() - 3600000 },
    { id: "e3", source: "entity-port-shanghai", target: "entity-contracts-mm", label: "triggers review →", strength: 0.82, type: "causal", description: "Delay above threshold triggers force-majeure review in maritime contracts", lastActive: Date.now() - 3600000 },
    { id: "e4", source: vesselSignalId, target: "domain-terra", label: "cascades to", strength: live?.vesselHighAlertCount ? 0.88 : 0.84, type: "escalation", description: "Maritime delay signal propagates to Terra domain via signal chain", lastActive: Date.now() - 3600000 },
    { id: "e5", source: vesselSignalId, target: "domain-prism", label: "cascades to", strength: 0.80, type: "escalation", description: "Maritime delay signal triggers legal review chain in PRISM", lastActive: Date.now() - 3600000 },

    { id: "e6", source: "entity-apt41", target: incidentEntityId, label: "caused", strength: 0.99, type: "causal", description: "APT-41 is the attributed threat actor for the active incident", lastActive: Date.now() - 1800000 },
    { id: "e7", source: incidentEntityId, target: "entity-legal-hold", label: "triggered", strength: 0.95, type: "escalation", description: "Cyber incident triggered legal hold on all forensic artifacts", lastActive: Date.now() - 7200000 },
    { id: "e8", source: incidentSignalId, target: "domain-prism", label: "legal cascade →", strength: live?.criticalIncident ? 0.94 : 0.91, type: "escalation", description: "Security incident detection triggered security→legal signal chain", lastActive: Date.now() - 1800000 },
    { id: "e9", source: incidentSignalId, target: "entity-nav", label: "risk impact →", strength: 0.73, type: "correlative", description: "Cyber incident elevated portfolio risk score", lastActive: Date.now() - 1800000 },
    ...
    { id: "e12", source: distressSignalId, target: "entity-fund3-lps", label: "sentiment risk →", strength: 0.6, type: "correlative", description: "Property distress levels may affect LP confidence in real estate thesis", lastActive: Date.now() - 7200000 },
...
router.get(
  ...
  async (_req, res) => {
    ...
    try {
      ...
      logger.info(
        { incidents: live.openIncidentCount, vesselAlerts: live.vesselHighAlertCount, distress: live.distressCount, kgEntities: live.kgEntityNodes.length, kgEdges: live.kgEdges.length },
