...
function verifyEmailSignature(req: Request): boolean {
  const signature = req.headers['x-continuum-email-signature'] as string | undefined;
  if (!signature) return false;
  if (!EMAIL_INGEST_SECRET) {
    logger.warn('continuum-email: ALLOY_EMAIL_INGEST_SECRET not configured — rejecting inbound email');
    return false;
  }
  const rawBody =
...
function scoreEmailPriority(subject: string, body: string, sender: string): number {
  ...
  if (combined.includes('urgent') || combined.includes('asap') || combined.includes('critical'))
    score += 30;
  if (combined.includes('ceo') || combined.includes('executive') || combined.includes('investor'))
    score += 20;
  if (combined.includes('payment') || combined.includes('invoice') || combined.includes('contract'))
    score += 15;
  if (
    ...
    score += 15;
  if (combined.includes('support') || combined.includes('help') || combined.includes('issue'))
    score += 10;
  if (
    ...
    score -= 30;
  if (combined.includes('spam') || combined.includes('click here') || combined.includes('buy now'))
    score -= 40;
...
function detectCategory(subject: string, body: string): string {
  ...
  if (
    ...
    combined.includes('joint venture')
  )
    return 'partnership';
  if (
  ...
  async (req: Request, res: Response) => {
    try {
      if (!verifyEmailSignature(req)) {
        res.status(401).json({ error: 'Missing or invalid x-continuum-email-signature' });
        return;
      }

      const { subject, senderEmail, senderName, recipientEmail, bodyText, bodyHtml, metadata } =
        req.body as {
          subject: string;
          senderEmail: string;
          senderName?: string;
          ...
          [
            {
              role: 'system',
              content:
                'You are an email triage assistant. Analyze this email and respond in JSON with: { summary: string (1 sentence), intent: string (what the sender wants), requires_response: boolean, suggested_action: string }. Be concise.',
            },
            {
              role: 'user',
      ...
      if (priorityScore >= 65) {
        try {
          ...
          logger.warn(
            { err: sigErr },
            'continuum-email: failed to create signal for high-priority email',
          );
        }
      }
...
router.get('/continuum/email/triage/:id', authMiddleware(), async (req: Request, res: Response) => {
  try {
    const result = await pool.query(`SELECT * FROM alloy_email_triage WHERE id = $1`, [
      req.params.id,
    ]);
    if (!result.rows[0]) {
...
router.post(
  '/continuum/email/triage/:id/draft',
  authMiddleware(),
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      const emailResult = await pool.query(`SELECT * FROM alloy_email_triage WHERE id = $1`, [
        req.params.id,
      ]);
      ...
      const { tone = 'professional', includeContext } = req.body as {
        tone?: string;
      ...
      await pool.query(
        `UPDATE alloy_email_triage SET auto_draft = $1, updated_at = NOW() WHERE id = $2`,
        [draft.content, req.params.id],
      );
...
router.post(
  '/continuum/email/triage/:id/route',
  authMiddleware(),
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!targetWorkflow) {
        sendBadRequest(res, 'targetWorkflow is required');
        return;
      }
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      const { status, priority, labels } = req.body as {
        status?: string;
        priority?: string;
        labels?: string[];
      };
      ...
      await pool.query(
        `UPDATE alloy_email_triage SET ${updates.join(', ')} WHERE id = $${idx}`,
        params,
      );
...
router.get('/continuum/email/stats', authMiddleware(), async (_req: Request, res: Response) => {
  try {
    const [totalResult, byStatus, byCategory] = await Promise.all([
      pool.query(
        `SELECT COUNT(*) as total, AVG(ai_priority_score) as avg_score FROM alloy_email_triage`,
      ),
      pool.query(`SELECT status, COUNT(*) as count FROM alloy_email_triage GROUP BY status`),
      pool.query(
        `SELECT category, COUNT(*) as count FROM alloy_email_triage GROUP BY category ORDER BY count DESC LIMIT 5`,
      ),
    ]);
    sendSuccess(res, {
