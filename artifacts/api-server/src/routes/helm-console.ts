  ...
  try {
    ...
    const [
      ...
      db
        .select({
          total: count(),
          avgLatency: avg(agentUsageStats.latencyMs),
      ...
      db
        .select({ status: atlasArtifactsTable.status, total: count() })
        .from(atlasArtifactsTable)
        .where(gte(atlasArtifactsTable.createdAt, since7d))
        .groupBy(atlasArtifactsTable.status),

      db
        .select({ status: atlasExportJobsTable.status, total: count() })
      ...
      db
        .select({
          status: outcomeGraphTable.status,
          total: count(),
      ...
      db
        .select({ status: worldlineSourceRegistryTable.status, total: count() })
        .from(worldlineSourceRegistryTable)
        .groupBy(worldlineSourceRegistryTable.status),

      db
        .select({ reviewState: proofChainTable.reviewState, total: count() })
  ...
  try {
    ...
    const [byTemplate, exportsByFormat, recentJobs] = await Promise.all([
      db
        ...
        .from(atlasArtifactsTable)
        .where(
          and(gte(atlasArtifactsTable.createdAt, since), eq(atlasArtifactsTable.isLatest, true)),
        )
    ...
    return void res.json({
      success: true,
      data: { byTemplate, exportsByFormat, failedExports: recentJobs },
    });
  } catch (err) {
...
helmRouter.get('/helm/proof-chain', async (req: Request, res: Response) => {
  try {
    ...
    const anomalies = await db
      .select()
      .from(proofChainTable)
      .where(and(gte(proofChainTable.createdAt, since), eq(proofChainTable.reviewState, 'flagged')))
