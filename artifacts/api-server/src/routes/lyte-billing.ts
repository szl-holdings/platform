...
async function logRevenueEvent(
  ...
  try {
    ...
    logger.warn(
      { err, eventType },
      'Failed to log revenue event (idempotency conflict or DB error)',
    );
  }
}
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!plan) {
        sendBadRequest(
          res,
          `Unknown Lyte plan "${planId}". Valid: ${Object.keys(LYTE_PLANS).join(', ')}`,
        );
        return;
      }
    ...
    try {
      ...
      if (!resolvedCustomerId && email) {
        ...
        if (existing) {
          ...
          const customer = await services.stripe.createCustomer(email, companyName ?? email, {
            product: 'lyte',
          });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.warn(
        ...
        'Lyte payment failed — subscription marked past_due',
      );
