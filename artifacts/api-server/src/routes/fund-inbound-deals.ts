...
import { type DealAttachment, db, fundInboundDealsTable } from '@szl-holdings/db';
...
const attachmentSchema = z.object({
  ...
  contentType: z
    ...
    .min(1)
    .max(120)
    .refine((t) => ALLOWED_UPLOAD_TYPES.has(t), {
...
async function verifyAndCanonicalizeAttachments(
  ...
  for (const a of attachments) {
    ...
    if (actualSize > MAX_FILE_BYTES) {
      return {
        ok: false,
        error: `Attachment "${a.name}" exceeds the 25MB per-file limit (actual ${(actualSize / MB).toFixed(1)}MB)`,
      };
    }
    if (!ALLOWED_UPLOAD_TYPES.has(actualContentType)) {
      return {
        ok: false,
        error: `Attachment "${a.name}" has an unsupported content type (${actualContentType})`,
      };
    }
    verified.push({
      kind: a.kind,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!parsed.success) {
        sendBadRequest(
          res,
          `Invalid submission: ${parsed.error.issues.map((i) => `${i.path.join('.')}: ${i.message}`).join('; ')}`,
        );
        return;
      }
      const v = parsed.data;
  ...
  try {
    ...
    const mapped = rows.map((r) => ({
      ...
      founder:
        r.founderName +
        (r.founderBackground
          ? ` (${r.founderBackground.slice(0, 60)}${r.founderBackground.length > 60 ? '…' : ''})`
          : ''),
