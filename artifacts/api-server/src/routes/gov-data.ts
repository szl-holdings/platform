...
async function postJson(url: string, body: unknown, headers?: Record<string, string>, timeoutMs = 12000): Promise<unknown> {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const res = await fetch(url, {
      method: "POST",
      signal: controller.signal,
      headers: { "User-Agent": "SZL-GovData/1.0", "Content-Type": "application/json", Accept: "application/json", ...(headers || {}) },
...
type CisaKevEntry = { cveID: string; vendorProject: string; product: string; vulnerabilityName: string; dateAdded: string; shortDescription: string; requiredAction: string; dueDate: string; knownRansomwareCampaignUse: string; notes: string };
type ArxivEntry = { id: string; title: string; authors: string[]; abstract: string; categories: string[]; published: string; updated: string; pdfUrl: string };
...
router.get("/gov/mitre-attack", govRateLimit, authMiddleware(), async (req, res) => {
  try {
    ...
    const data = await getCached("mitre-attack", 86400000, async () => {
      const raw = await fetchJson("https://raw.githubusercontent.com/mitre-attack/attack-stix-data/master/enterprise-attack/enterprise-attack.json", {}, 20000) as any;
      const objects = raw?.objects as any[] ?? [];
      const techniques = objects.filter((o: any) => o.type === "attack-pattern" && !o.revoked && !o.x_mitre_deprecated).slice(0, 50).map((o: any) => ({
        id: o.external_references?.find((r: any) => r.source_name === "mitre-attack")?.external_id ?? o.id,
        name: o.name ?? "Unknown",
        tactic: o.kill_chain_phases?.[0]?.phase_name?.replace(/-/g, " ").replace(/\b\w/g, (c: string) => c.toUpperCase()) ?? "Unknown",
...
router.get("/gov/fedramp", govRateLimit, authMiddleware(), async (req, res) => {
  try {
    ...
    const products = await getCached("fedramp-products", 86400000, async () => {
      const res2 = await fetch("https://raw.githubusercontent.com/GSA/data/master/IT/fedramp-authorized-products.csv", {
      ...
      const get = (cells: string[], name: string) => cells[headers.indexOf(name)]?.replace(/"/g, "").trim() ?? "";
      return rows.slice(1).filter(r => r.trim()).slice(0, 20).map(row => {
        const cells = row.split(",");
        return {
          packageId: get(cells, "Package ID") || (cells[0]?.replace(/"/g, "").trim() ?? ""),
          productName: get(cells, "Cloud Service Provider - Package Name") || (cells[1]?.replace(/"/g, "").trim() ?? ""),
    ...
    }).catch(() => [] as { packageId: string; productName: string; cspName: string; packageStatus: string; authorizationType: string; impactLevel: string; authorizationDate: string; expirationDate: null; serviceDescription: string }[]);
    let filtered = products;
    if (impactLevel) filtered = filtered.filter(p => p.impactLevel.toLowerCase() === impactLevel.toLowerCase());
...
router.get("/gov/census", govRateLimit, authMiddleware(), async (_req, res) => {
  try {
    ...
    }).catch(() => ({ population: null, medianHouseholdIncome: null, medianHomeValue: null, source: "U.S. Census Bureau ACS 5-Year Estimates", note: "Census API temporarily unavailable." }));
    sendSuccess(res, {
...
router.get("/gov/bls-employment", govRateLimit, authMiddleware(), async (_req, res) => {
  try {
    ...
    }).catch(() => ({ nationalUnemploymentRate: null, period: null, source: "U.S. Bureau of Labor Statistics", note: "BLS API temporarily unavailable." }));
    sendSuccess(res, {
      source: "U.S. Bureau of Labor Statistics",
...
router.get("/gov/usaspending", govRateLimit, authMiddleware(), async (req, res) => {
  try {
    ...
    const contracts = await getCached(`usaspending-${agency || "all"}-${minAmount}`, 3600000, async () => {
      const body = {
        filters: {
      ...
      const raw = await postJson("https://api.usaspending.gov/api/v2/search/spending_by_award/", body, {}, 12000) as any;
...
type NoaaStationReading = { buoyId: string; name: string; lat: number; lon: number; windSpeed: number | null; windDir: string | null; airTemp: number | null; waterTemp: number | null; timestamp: string; source: string };

async function fetchNoaaStation(station: typeof NOAA_COOPS_STATIONS[0]): Promise<NoaaStationReading> {
  const controller = new AbortController();
  ...
  try {
    ...
    clearTimeout(timer);
    return { buoyId: station.id, name: station.name, lat: station.lat, lon: station.lon, airTemp: null, waterTemp: null, windSpeed: null, windDir: null, timestamp: new Date().toISOString(), source: "unavailable" };
  }
}
...
router.get("/gov/pubmed", govRateLimit, authMiddleware(), async (req, res) => {
  try {
    ...
    const papers = await getCached(`pubmed-${query}`, 3600000, async () => {
      const searchRaw = await fetchJson(`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${encodeURIComponent(query)}&retmax=5&retmode=json&sort=pub+date`, {}, 10000) as any;
      const ids: string[] = searchRaw?.esearchresult?.idlist ?? [];
      if (ids.length === 0) throw new Error("No PubMed IDs");
      const summaryRaw = await fetchJson(`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id=${ids.join(",")}&retmode=json`, {}, 10000) as any;
    ...
    }).catch(() => [] as { pmid: string; title: string; authors: string[]; journal: string; year: number | null; doi: string | null; meshTerms: string[] }[]);
    sendSuccess(res, {
