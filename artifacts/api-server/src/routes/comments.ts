...
import { commentsTable, db } from '@szl-holdings/db';
      ...
      if (req.user) {
        if (existing.authorId === null) {
          res.status(403).json({
            success: false,
            error:
              'Forbidden: this comment was created anonymously and cannot be edited by an authenticated user',
          });
          return;
        }
        if (existing.authorId !== req.user.id) {
          res
            .status(403)
            .json({ success: false, error: 'Forbidden: you can only edit your own comments' });
          return;
        }
      } else {
      ...
      if (req.user) {
        if (existing.authorId === null) {
          res.status(403).json({
            success: false,
            error:
              'Forbidden: this comment was created anonymously and cannot be deleted by an authenticated user',
          });
          return;
        }
        if (existing.authorId !== req.user.id) {
          res
            .status(403)
            .json({ success: false, error: 'Forbidden: you can only delete your own comments' });
          return;
        }
      } else {
