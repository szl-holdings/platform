  ...
  async (req, res) => {
    try {
      ...
      if (signal.status !== 'new') {
        sendBadRequest(res, 'Only new signals can be acknowledged');
        return;
      }
      const [row] = await db
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!assignee) {
        sendBadRequest(res, 'assignee is required');
        return;
      }
      const [signal] = await db.select().from(lyteSignalsTable).where(eq(lyteSignalsTable.id, id));
      if (!signal) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const [row] = await db
        .update(lyteSignalsTable)
        .set({ severity: newSeverity as typeof signal.severity })
        .where(eq(lyteSignalsTable.id, id))
        .returning();
      await recordSignalTimeline({
        signalId: id,
        ...
        actorName: req.user?.displayName,
        actorId: req.user?.id,
        notes: `Escalated to ${newSeverity}`,
      });
      sendSuccess(res, row);
    } catch (err) {
  ...
  async (req, res) => {
    try {
      ...
      if (signal.status === 'resolved') {
        sendBadRequest(res, 'Signal is already resolved');
        return;
      }
      const [row] = await db
        .update(lyteSignalsTable)
  ...
  async (req, res) => {
    try {
      ...
      await recordSignalTimeline({
        ...
        actorName: req.user?.displayName,
        actorId: req.user?.id,
        notes: `Manual override: ${JSON.stringify(req.body)}`,
      });
      sendSuccess(res, row);
    } catch (err) {
...
router.get('/signals/:id/timeline', authMiddleware(), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(lyteSignalTimelineTable)
      .where(eq(lyteSignalTimelineTable.signalId, id))
      .orderBy(desc(lyteSignalTimelineTable.createdAt));
...
router.get('/signals/:id/comments', authMiddleware(), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(lyteSignalCommentsTable)
      .where(eq(lyteSignalCommentsTable.signalId, id))
      .orderBy(desc(lyteSignalCommentsTable.createdAt));
    sendSuccess(res, rows);
  } catch (err) {
...
router.post(
  ...
  validateBody(
    bodyShape({
      ...
      commentType: z.unknown().optional(),
    }),
  ),
  async (req, res) => {
    try {
      ...
      const [row] = await db
        .insert(lyteSignalCommentsTable)
        .values({
          signalId: id,
      ...
      await recordSignalTimeline({
        ...
        actorName: req.user?.displayName,
        actorId: req.user?.id,
        notes: req.body.body.slice(0, 100),
      });
      sendCreated(res, row);
    } catch (err) {
...
router.post('/actions', authMiddleware(), validateBody(bodyShape({})), async (req, res) => {
  try {
...
router.patch('/actions/:id', authMiddleware(), validateBody(bodyShape({})), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    const [row] = await db
...
router.delete('/actions/:id', validateBody(bodyShape({})), authMiddleware(), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
...
router.post('/saved-views', authMiddleware(), validateBody(bodyShape({})), async (req, res) => {
  try {
    const data = insertLyteSavedViewSchema.parse(req.body);
    const [row] = await db
      .insert(lyteSavedViewsTable)
      .values({ ...data, userId: req.user?.id ? String(req.user.id) : null })
      .returning();
    sendCreated(res, row);
  } catch (err) {
...
router.get(
  '/readiness',
  requireFeatureFlag('lyte_readiness_enabled'),
  authMiddleware(),
...
router.get(
  ...
  async (_req, res) => {
    try {
      ...
      const breakdown = items.reduce<Record<string, { total: number; complete: number }>>(
        (acc, item) => {
  ...
  try {
    const [
      ...
      db
        .select({ count: sql<number>`count(*)::int` })
        .from(lyteSignalsTable)
        .where(and(eq(lyteSignalsTable.severity, 'critical'), eq(lyteSignalsTable.status, 'new')))
        .then((r) => r[0]?.count ?? 0),
      db
        .select({ count: sql<number>`count(*)::int` })
        .from(lyteIncidentsTable)
        .where(ne(lyteIncidentsTable.status, 'closed'))
        .then((r) => r[0]?.count ?? 0),
      db
        .select({ count: sql<number>`count(*)::int` })
        .from(lyteActionsTable)
      ...
      db
        ...
        .where(eq(lyteRecommendationsTable.status, 'suggested'))
        .then((r) => r[0]?.count ?? 0),
  ...
  try {
    ...
    try {
      signals = await db
        .select()
        .from(lyteSignalsTable)
        .where(ne(lyteSignalsTable.status, 'resolved'))
    ...
    if (criticalCount > 0) {
      narratives.push({
        type: 'alert',
        priority: 'critical',
        headline: `${criticalCount} critical signal${criticalCount > 1 ? 's' : ''} require immediate attention`,
        detail: 'Critical signals are unacknowledged and may have downstream impact.',
      });
    }
    if (incidents.length > 0) {
      ...
      narratives.push({
        type: 'incident',
        priority: 'high',
        headline: `${incidents.length} active incident${incidents.length > 1 ? 's' : ''} in progress`,
        detail: `Most recent: ${firstTitle}`,
      });
    }
    ...
    if (topSource) {
      narratives.push({
        type: 'pattern',
        priority: 'medium',
        headline: `${topSource[0]} is the leading signal source (${topSource[1]} signal${topSource[1] > 1 ? 's' : ''})`,
        detail: 'Consider reviewing integration health or thresholds for this source.',
      });
    }
    if (narratives.length === 0) {
      narratives.push({
        ...
        detail: 'No active signals or open incidents detected.',
      });
    }
