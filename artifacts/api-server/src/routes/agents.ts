...
import { getPendingApprovalRequests, resolvePendingApprovalRequest } from "@workspace/approvals-inbox";
import { createToolEvalSuite, createPromptEvalSuite } from "@workspace/agents-evals";
import {
  threatScanTool,
  alertEscalationTool,
  complianceCheckTool,
...
const _asGenericTool = (t: unknown): TypedTool<unknown, unknown> => t as TypedTool<unknown, unknown>;
 ...
 */
router.post("/agents/approvals/:runId/:stepId/resolve", authMiddleware(), (req: Request, res: Response) => {
  try {
    const runId = String(req.params["runId"] ?? "");
    ...
    if (!verdict || !["approved", "rejected", "escalated"].includes(verdict)) {
      res.status(400).json({ success: false, error: "verdict must be one of approved|rejected|escalated" });
      return;
    }
  ...
  try {
    const toolSuites = [
      createToolEvalSuite(_asGenericTool(threatScanTool), [
        { label: "scan baseline asset",  input: { assetId: "asset-001" }, groundTruth: { findingsCount: 0 } },
        { label: "scan compromised asset", input: { assetId: "asset-pwn" }, groundTruth: { findingsCount: 1 } },
      ]),
      createToolEvalSuite(_asGenericTool(alertEscalationTool), [
        { label: "p1 escalation", input: { alertId: "alert-1", severity: "critical" }, groundTruth: { escalated: true } },
      ]),
      ...
      createToolEvalSuite(_asGenericTool(fundTransferTool), [
        { label: "intra-fund transfer", input: { fromAccount: "A", toAccount: "B", amount: 1000 }, groundTruth: { settled: true } },
      ]),
      createToolEvalSuite(_asGenericTool(portfolioSnapshotTool), [
        { label: "snapshot pulse fund", input: { fundId: "pulse-1" }, groundTruth: { positions: 12 } },
      ]),
    ...
    const promptSuites = [
      createPromptEvalSuite({ id: "agents.plan" }, [
        { label: "decompose security incident", variables: { objective: "contain ransomware spread" }, groundTruth: { stepCount: 5 } },
      ]),
      createPromptEvalSuite({ id: "agents.verify" }, [
        { label: "verify clean tool output", variables: { stepOutput: "ok" }, groundTruth: { passed: true } },
      ]),
    ];
    ...
    sendSuccess(res, {
      toolSuites,
      promptSuites,
      totals: { tools: toolSuites.length, prompts: promptSuites.length, cases: [...toolSuites, ...promptSuites].reduce((n, s) => n + s.cases.length, 0) },
    });
  } catch (err) {
