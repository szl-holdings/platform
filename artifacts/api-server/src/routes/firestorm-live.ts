...
router.get('/firestorm/live/threats', authMiddleware(), async (_req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch live threats');
  }
});
...
router.get('/firestorm/live/incidents', authMiddleware(), async (_req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch live incidents');
  }
});
...
router.get('/firestorm/live/compliance-summary', authMiddleware(), async (_req, res) => {
  try {
    ...
    const passing = controls.filter(
      (c) => (c.status as any) === 'pass' || (c.status as any) === 'compliant',
    ).length;
    const failing = controls.filter(
      (c) => (c.status as any) === 'fail' || (c.status as any) === 'non_compliant',
    ).length;
    ...
    sendSuccess(res, {
      frameworks: [
        ...
        { name: 'StateRAMP Moderate', score: 74, status: 'Assessment', controls: 323, passing: 239 },
    ...
    handleRouteError(res, err, 'Failed to fetch compliance summary');
  }
});
...
router.get('/firestorm/live/asset-risk', authMiddleware(), async (_req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch asset risk');
  }
});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      logger.info(
        { playbookId, alertId, executionId, userId: req.user?.id },
        'SOAR playbook execution triggered',
      );
      ...
      handleRouteError(res, err, 'Failed to execute SOAR playbook');
    }
  },
);
  ...
  try {
    const stixObjects = [
      ...
      },
      {
        ...
        description:
          'Sophisticated backdoor used by APT29 in supply chain attacks. SolarWinds-related variant detected.',
        tlp: 'TLP:AMBER',
        confidence: 97,
        created: '2024-02-01',
        modified: '2024-03-20',
      ...
      },
      {
        ...
        description:
          'Russian SVR-linked advanced persistent threat actor. Sophisticated nation-state capability targeting government, defense, and finance sectors.',
        tlp: 'TLP:AMBER',
        confidence: 98,
        created: '2023-01-01',
        modified: '2024-03-29',
      ...
      },
      {
        ...
        description:
          'Active APT29 campaign targeting SZL Holdings financial systems. Multi-stage intrusion: spearphishing → credential harvest → lateral movement → data exfil attempt.',
        tlp: 'TLP:RED',
        confidence: 97,
    ...
    handleRouteError(res, err, 'Failed to fetch STIX objects');
  }
});

router.post(
  '/firestorm/stix/export',
  authMiddleware({ required: true }),
  requireRole('analyst', 'operator', 'admin'),
  validateBody(
    bodyShape({
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to export STIX bundle');
    }
  },
);
...
router.get('/firestorm/taxii/feeds', authMiddleware(), async (_req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch TAXII feeds');
  }
});
  ...
  try {
    ...
    sendSuccess(res, {
      ...
      summary: {
        ...
        totalCovered,
        overallCoverage,
        gaps: totalTechniques - totalCovered,
      },
      framework: 'MITRE ATT&CK Enterprise v14',
      fetchedAt: new Date().toISOString(),
    });
  } catch (err) {
    handleRouteError(res, err, 'Failed to fetch MITRE coverage');
  }
});
