...
function seedRegistry() {
  ...
  promptRegistry.create({
    ...
    variables: [{ name: "document", type: "string", description: "Contract text to analyse", required: true }],
  ...
  promptRegistry.addVersion("legal-risk-extraction", {
    ...
    systemPrompt: "You are a senior legal analyst with expertise in risk extraction and contract law.",
    variables: [
      { name: "document", type: "string", description: "Contract text to analyse", required: true },
      { name: "context", type: "string", description: "Additional context about the counterparty", required: false },
    ],
    modelHints: { preferredModel: "gpt-4o", temperature: 0.1 },
  ...
  promptRegistry.create({
    ...
    variables: [{ name: "vesselData", type: "string", description: "Raw AIS and port data", required: true }],
  ...
  promptRegistry.addVersion("maritime-intel-brief", {
    ...
    variables: [
      ...
      { name: "timeRange", type: "string", description: "Date range for the analysis", required: false },
    ],
    modelHints: { preferredModel: "gpt-4o-mini", temperature: 0.2 },
    changelog: "Added time range variable; expanded output to include sanctions exposure and recommended actions.",
    createdBy: "vessels-team",
  ...
  promptRegistry.addVersion("maritime-intel-brief", {
    ...
    variables: [
      { name: "vesselData", type: "string", description: "Raw AIS and port data", required: true },
      { name: "timeRange", type: "string", description: "Date range for the analysis", required: false },
      { name: "jurisdiction", type: "string", description: "Regulatory jurisdiction", required: false },
    ],
    modelHints: { preferredModel: "gpt-4o", temperature: 0.15 },
    changelog: "Added jurisdiction variable and geopolitical risk section; switched output to structured JSON.",
    createdBy: "vessels-team",
  ...
  promptRegistry.create({
    ...
    variables: [{ name: "indicators", type: "string", description: "Threat indicators (IOCs, TTPs)", required: true }],
    modelHints: { preferredModel: "gpt-4o", temperature: 0.1 },
  ...
  promptRegistry.addVersion("threat-assessment-report", {
    ...
    variables: [
      { name: "indicators", type: "string", description: "Threat indicators (IOCs, TTPs)", required: true },
      { name: "affectedSystems", type: "string", description: "List of affected systems and services", required: false },
    ],
    modelHints: { preferredModel: "gpt-4o", temperature: 0.05 },
    changelog: "Added MITRE ATT&CK mapping, confidence scoring, blast radius estimate, and affected systems variable.",
    createdBy: "security-team",
  ...
  promptRegistry.create({
    ...
    variables: [{ name: "content", type: "string", description: "Source intelligence content", required: true }],
  ...
  promptRegistry.create({
    ...
    variables: [
      { name: "propertyDetails", type: "string", description: "Property attributes and condition", required: true },
      { name: "comparables", type: "string", description: "Recent comparable sales data", required: false },
    ],
    modelHints: { preferredModel: "gpt-4o-mini", temperature: 0.2 },
  ...
  promptRegistry.create({
    ...
    variables: [
      { name: "dealDetails", type: "string", description: "Investment opportunity details", required: true },
      { name: "fundMandate", type: "string", description: "Fund investment mandate", required: false },
    ],
  ...
  promptRegistry.create({
    ...
    variables: [
      { name: "currentPolicies", type: "string", description: "Current policy documentation", required: true },
      { name: "targetFramework", type: "string", description: "Target compliance framework (e.g. SOC2, ISO27001)", required: true },
    ],
    modelHints: { preferredModel: "gpt-4o", temperature: 0.1 },
  ...
  promptEvaluator.createSuite({
    ...
    cases: [
      { input: { document: "Supplier may terminate with 30 days notice." }, expectedKeywords: ["termination", "risk"] },
      { input: { document: "Liability capped at $10,000 per incident." }, expectedKeywords: ["liability", "cap"] },
      { input: { document: "Force majeure clause excludes cyber events." }, expectedKeywords: ["force majeure", "cyber"] },
    ],
  ...
  try {
    ...
    if (evalledVersions.length >= 2) {
      ...
      comparison = {
        ...
        recommendation: scoreDiff > 5 && latencyDiff < 1000 ? "promote" : scoreDiff < -5 ? "reject" : "hold",
      };
    }
 ...
 */
router.post("/ai/prompts/:id/promote", authMiddleware, (req, res) => {
  try {
 ...
 */
router.post("/ai/prompts/:id/versions/:versionId/eval", authMiddleware, async (req, res) => {
  try {
    if (!requireRegistryWrite(req, res)) return;
    ...
    if (suites.length === 0) {
      ...
      const keywords = version.variables
        ...
        .map(v => v.name)
        .slice(0, 3);
      suites = [promptEvaluator.createSuite({
        promptId: id,
        ...
        cases: [
          {
            id: `${id}-auto-1`,
            promptId: id,
            input: Object.fromEntries(version.variables.map(v => [v.name, `sample ${v.name}`])),
            expectedKeywords: keywords.length > 0 ? keywords : ["result"],
          },
          {
            id: `${id}-auto-2`,
            promptId: id,
            input: Object.fromEntries(version.variables.map(v => [v.name, `test ${v.name} value`])),
            expectJson: false,
          },
        ],
      })];
    ...
    const report = await promptEvaluator.run(id, versionId, suite.id, async (rendered, input) => {
      ...
      const words = rendered.split(/\W+/).filter(w => w.length > 4).slice(0, 8);
