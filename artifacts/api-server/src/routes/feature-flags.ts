...
router.post(
  ...
  authMiddleware(),
  validateBody(featureFlagEvaluateSchema),
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      ...
      await logActivity(
        ...
        'feature_flag',
        String(flag.id),
        `Updated flag: ${flag.key} (enabled=${flag.isEnabled})`,
      );
...
router.post(
  ...
  authMiddleware(),
  requireRole('ops', 'admin'),
  validateBody(featureFlagOverrideSchema),
  async (req, res) => {
    try {
...
router.delete(
  '/feature-flags/:id/overrides/:overrideId',
  validateBody(bodyShape({})),
  authMiddleware(),
  requireRole('ops', 'admin'),
  async (req, res) => {
