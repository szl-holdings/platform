...
router.get('/rmm/actions', auth, validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list remote actions');
  }
});
...
router.post(
  ...
  authWrite,
  roleOperator,
  validateBody(rmmActionCreateSchema),
  async (req, res) => {
    try {
    ...
    `);
      ...
      sendCreated(res, {
        action,
        message: requiresApproval ? 'Action queued for approval' : 'Action executing',
      });
    } catch (err) {
      handleRouteError(res, err, 'Failed to create remote action');
    }
  },
);

router.post(
  '/rmm/actions/:id/approve',
  authWrite,
  roleOperator,
  validateBody(
    bodyShape({
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to cancel action');
    }
  },
);
...
export async function executeRemoteAction(
  ...
  try {
    await db.execute(
      sql`UPDATE msp_remote_actions SET status = 'executing', executed_at = NOW() WHERE id = ${actionId}`,
    );
    ...
    if (!connRow) {
      await db.execute(
        sql`UPDATE msp_remote_actions SET status = 'failed', error_message = 'Connector not found', completed_at = NOW() WHERE id = ${actionId}`,
      );
      return;
    }
    ...
    if (!provider) {
      await db.execute(
        sql`UPDATE msp_remote_actions SET status = 'failed', error_message = 'Provider not supported', completed_at = NOW() WHERE id = ${actionId}`,
      );
      return;
    }

    const deviceRows = await db.execute<{ deviceId: string }>(
      sql`SELECT device_id as "deviceId" FROM msp_devices WHERE id = ${deviceId}`,
    );
    const providerDeviceId =
      (deviceRows.rows[0] as { deviceId: string } | undefined)?.deviceId ?? String(deviceId);

    let result: { success: boolean; jobId?: string; output?: string; errorMessage?: string };
    const normalizedAction = actionType
      .replace('restart_service', 'service_restart')
      .replace('clear_disk', 'clear_temp');
    switch (normalizedAction) {
      case 'service_restart':
        result = await provider.restartService(providerDeviceId, target ?? '');
        break;
      case 'service_start':
        result = await provider.runScript(
          providerDeviceId,
          `Start-Service -Name '${(target ?? '').replace(/'/g, "''")}'`,
          'powershell',
        );
        break;
      case 'service_stop':
        result = await provider.runScript(
          providerDeviceId,
          `Stop-Service -Name '${(target ?? '').replace(/'/g, "''")}'`,
          'powershell',
        );
        break;
      case 'reboot':
      ...
      case 'run_script':
        result = await provider.runScript(
          ...
          (parameters.scriptType as 'powershell' | 'bash') ?? 'powershell',
        );
        break;
      case 'kill_process': {
      ...
      case 'clear_temp':
        result = await provider.runScript(
          providerDeviceId,
          "Remove-Item -Path $env:TEMP\\* -Recurse -Force -ErrorAction SilentlyContinue; Write-Output 'Temp cleared'",
          'powershell',
        );
        break;
      default:
    ...
    await db
      .execute(
        sql`UPDATE msp_remote_actions SET status = 'failed', error_message = ${String(err)}, completed_at = NOW() WHERE id = ${actionId}`,
      )
      .catch(() => undefined);
  }
}
