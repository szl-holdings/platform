...
const REFERENCE_CMMC_MATURITY = {
  level1: {
    ...
    contractRequirement: 'All Federal contractors handling FCI',
    selfAttestation: true,
  },
  level2: {
    controls: 110,
    description: 'Advanced Cyber Hygiene — NIST SP 800-171',
    contractRequirement: 'Contractors handling CUI',
    selfAttestation: false,
    thirdPartyAssessment: 'C3PAO required',
  },
  level3: {
    controls: 134,
    description: 'Expert — NIST SP 800-172',
    contractRequirement: 'High priority DoD programs',
    selfAttestation: false,
    governmentLed: 'DIBCAC assessment required',
  },
};
  ...
  async (req, res) => {
    try {
      ...
      type UsaSpendingResult = {
        ...
        Place_of_Performance_Location_Code?: string;
      };
      ...
      handleRouteError(res, err, 'Failed to fetch federal contracts');
    }
  },
);
  ...
  async (req, res) => {
    try {
          ...
          const parsed = rows
            ...
            .map((row) => {
              ...
              return {
                ...
                productName:
                  (get('Cloud Service Provider - Package Name') ||
                    get('productName') ||
                    cells[1]) ??
                  '',
      ...
      const filtered = impact
        ? products.filter(
            (p: { impactLevel?: string }) => p.impactLevel?.toLowerCase() === impact.toLowerCase(),
          )
        : products;
      sendSuccess(res, {
...
router.get('/msp/live/pipeline', mspLiveRateLimit, authMiddleware(), async (_req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch MSP contract pipeline');
  }
});
...
router.get('/msp/live/health-metrics', mspLiveRateLimit, authMiddleware(), async (_req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch MSP health metrics');
  }
});
...
router.get('/msp/live/system-metrics', mspLiveRateLimit, authMiddleware(), async (_req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to collect system metrics');
  }
});
