...
async function logCapitalAudit(action: string, entity: string, entityId: string | number, payload?: unknown) {
  try {
...
router.get("/capital/artifacts", ...auth, validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    const [{ count }] = await db.select({ count: sql<number>`count(*)::int` }).from(capitalArtifactsTable);
...
router.patch("/capital/artifacts/:id", ...auth, validateBody(bodyShape({})), async (req, res) => {
  try {
    ...
    if (!row) { sendNotFound(res, "Capital artifact"); return; }
...
router.get("/capital/lender-packets", ...auth, validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    const [{ count }] = await db.select({ count: sql<number>`count(*)::int` }).from(lenderPacketsTable);
    sendSuccess(res, rows, 200, { page, limit, total: count });
  } catch (err) {
...
router.get("/capital/lender-packets/:id", ...auth, async (req, res) => {
  try {
    ...
    const deliverables = await db.select().from(lenderPacketDeliverables).where(eq(lenderPacketDeliverables.packetId, id)).orderBy(lenderPacketDeliverables.sortOrder);
...
router.patch("/capital/lender-packets/:id", ...auth, validateBody(bodyShape({})), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    const [row] = await db.update(lenderPacketsTable).set({ ...req.body, updatedAt: new Date() }).where(eq(lenderPacketsTable.id, id)).returning();
    if (!row) { sendNotFound(res, "Lender packet"); return; }
...
router.delete("/capital/lender-packets/:id", validateBody(bodyShape({})), ...auth, async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
...
router.patch("/capital/lender-deliverables/:id", ...auth, validateBody(bodyShape({
  ...
  try {
    ...
    const [row] = await db.update(lenderPacketDeliverables).set(update).where(eq(lenderPacketDeliverables.id, id)).returning();
    if (!row) { sendNotFound(res, "Deliverable"); return; }
    await logCapitalAudit("update", "lender_packet_deliverable", id, req.body);
    const all = await db.select().from(lenderPacketDeliverables).where(eq(lenderPacketDeliverables.packetId, row.packetId));
    ...
    await db.update(lenderPacketsTable).set({ completionPct: pct, updatedAt: new Date() }).where(eq(lenderPacketsTable.id, row.packetId));
    sendSuccess(res, row);
  } catch (err) {
...
router.delete("/capital/lender-deliverables/:id", validateBody(bodyShape({})), ...auth, async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
...
router.get("/capital/investor-packets", ...auth, validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    const [{ count }] = await db.select({ count: sql<number>`count(*)::int` }).from(investorPacketsTable);
    sendSuccess(res, rows, 200, { page, limit, total: count });
  } catch (err) {
...
router.get("/capital/investor-packets/:id", ...auth, async (req, res) => {
  try {
    ...
    const deliverables = await db.select().from(investorPacketDeliverables).where(eq(investorPacketDeliverables.packetId, id)).orderBy(investorPacketDeliverables.sortOrder);
    sendSuccess(res, { ...packet, deliverables });
  } catch (err) {
...
router.patch("/capital/investor-packets/:id", ...auth, validateBody(bodyShape({})), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    ...
    if (!row) { sendNotFound(res, "Investor packet"); return; }
...
router.delete("/capital/investor-packets/:id", validateBody(bodyShape({})), ...auth, async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
...
router.patch("/capital/investor-deliverables/:id", ...auth, validateBody(bodyShape({
      "completedAt": z.unknown().optional(),
      "status": z.unknown().optional(),
    })), async (req, res) => {
  try {
    ...
    const [row] = await db.update(investorPacketDeliverables).set(update).where(eq(investorPacketDeliverables.id, id)).returning();
    if (!row) { sendNotFound(res, "Deliverable"); return; }
    await logCapitalAudit("update", "investor_packet_deliverable", id, req.body);
    const all = await db.select().from(investorPacketDeliverables).where(eq(investorPacketDeliverables.packetId, row.packetId));
    const completedCount = all.filter(d => ["reviewed", "final"].includes(d.status)).length;
    const pct = all.length ? Math.round((completedCount / all.length) * 100) : 0;
    await db.update(investorPacketsTable).set({ completionPct: pct, updatedAt: new Date() }).where(eq(investorPacketsTable.id, row.packetId));
    sendSuccess(res, row);
  } catch (err) {
...
router.delete("/capital/investor-deliverables/:id", validateBody(bodyShape({})), ...auth, async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
...
router.get("/capital/milestones", ...auth, async (req, res) => {
  try {
    const rows = await db.select().from(fundraisingMilestonesTable).orderBy(fundraisingMilestonesTable.sortOrder, fundraisingMilestonesTable.targetDate);
...
router.patch("/capital/milestones/:id", ...auth, validateBody(bodyShape({
      "completedAt": z.unknown().optional(),
      "status": z.unknown().optional(),
    })), async (req, res) => {
  try {
    ...
    const [row] = await db.update(fundraisingMilestonesTable).set(update).where(eq(fundraisingMilestonesTable.id, id)).returning();
...
router.get("/capital/financial-models", ...auth, async (req, res) => {
  try {
    const rows = await db.select().from(financialModelsTable).orderBy(desc(financialModelsTable.createdAt));
    sendSuccess(res, rows);
  } catch (err) {
...
router.patch("/capital/financial-models/:id", ...auth, validateBody(bodyShape({})), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    ...
    if (!row) { sendNotFound(res, "Financial model"); return; }
...
router.delete("/capital/financial-models/:id", validateBody(bodyShape({})), ...auth, async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
...
router.get("/capital/use-of-funds", ...auth, async (req, res) => {
  try {
    const rows = await db.select().from(useOfFundsVersionsTable).orderBy(desc(useOfFundsVersionsTable.createdAt));
    sendSuccess(res, rows);
  } catch (err) {
...
router.patch("/capital/use-of-funds/:id", ...auth, validateBody(bodyShape({})), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    ...
    if (!row) { sendNotFound(res, "Use-of-funds version"); return; }
...
router.delete("/capital/use-of-funds/:id", validateBody(bodyShape({})), ...auth, async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
...
router.get("/capital/diligence-checklists", ...auth, async (req, res) => {
  try {
    const rows = await db.select().from(diligenceChecklistsTable).orderBy(desc(diligenceChecklistsTable.createdAt));
    sendSuccess(res, rows);
  } catch (err) {
...
router.get("/capital/diligence-checklists/:id", ...auth, async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    const [checklist] = await db.select().from(diligenceChecklistsTable).where(eq(diligenceChecklistsTable.id, id));
    if (!checklist) { sendNotFound(res, "Checklist"); return; }
    const items = await db.select().from(diligenceChecklistItemsTable).where(eq(diligenceChecklistItemsTable.checklistId, id)).orderBy(diligenceChecklistItemsTable.sortOrder);
...
router.post("/capital/diligence-checklists", ...auth, validateBody(bodyShape({})), async (req, res) => {
  try {
...
router.patch("/capital/diligence-checklists/:id", ...auth, validateBody(bodyShape({})), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    ...
    if (!row) { sendNotFound(res, "Checklist"); return; }
...
router.delete("/capital/diligence-checklists/:id", validateBody(bodyShape({})), ...auth, async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
...
router.patch("/capital/diligence-checklist-items/:id", ...auth, validateBody(bodyShape({
      "completedAt": z.unknown().optional(),
      "status": z.unknown().optional(),
    })), async (req, res) => {
  try {
    ...
    const [row] = await db.update(diligenceChecklistItemsTable).set(update).where(eq(diligenceChecklistItemsTable.id, id)).returning();
    if (!row) { sendNotFound(res, "Checklist item"); return; }
    await logCapitalAudit("update", "diligence_checklist_item", id, req.body);
    const all = await db.select().from(diligenceChecklistItemsTable).where(eq(diligenceChecklistItemsTable.checklistId, row.checklistId));
    const completedCount = all.filter(i => ["complete", "waived", "na"].includes(i.status)).length;
    const pct = all.length ? Math.round((completedCount / all.length) * 100) : 0;
    await db.update(diligenceChecklistsTable).set({ completionPct: pct, updatedAt: new Date() }).where(eq(diligenceChecklistsTable.id, row.checklistId));
    sendSuccess(res, row);
  } catch (err) {
...
router.delete("/capital/diligence-checklist-items/:id", validateBody(bodyShape({})), ...auth, async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
...
router.get("/capital/cap-table", ...auth, async (req, res) => {
  try {
    const rows = await db.select().from(capTablePlaceholdersTable).where(eq(capTablePlaceholdersTable.isActive, true)).orderBy(capTablePlaceholdersTable.sortOrder);
    sendSuccess(res, rows);
  } catch (err) {
...
router.patch("/capital/cap-table/:id", ...auth, validateBody(bodyShape({})), async (req, res) => {
  try {
    ...
    if (!row) { sendNotFound(res, "Cap table entry"); return; }
...
router.post("/capital/generate-lender-packet/:id", ...auth, validateBody(bodyShape({})), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    const [packet] = await db.select().from(lenderPacketsTable).where(eq(lenderPacketsTable.id, id));
    if (!packet) { sendNotFound(res, "Lender packet"); return; }
    ...
    await logCapitalAudit("generate", "lender_packet", id, { triggeredBy: "on_demand", jobId: job.id });
    sendSuccess(res, { queued: true, packetId: id, jobId: job.id, message: "Lender packet generation queued" }, 202);
  } catch (err) {
    handleRouteError(res, err, "Failed to queue lender packet generation");
  }
});

router.post("/capital/generate-investor-packet/:id", ...auth, validateBody(bodyShape({})), async (req, res) => {
  try {
    const id = parseIdParam(req.params.id);
    const [packet] = await db.select().from(investorPacketsTable).where(eq(investorPacketsTable.id, id));
    if (!packet) { sendNotFound(res, "Investor packet"); return; }
    ...
    await logCapitalAudit("generate", "investor_packet", id, { triggeredBy: "on_demand", jobId: job.id });
    sendSuccess(res, { queued: true, packetId: id, jobId: job.id, message: "Investor packet generation queued" }, 202);
  } catch (err) {
...
router.get("/capital/dashboard", ...auth, async (req, res) => {
  try {
    ...
    const bankReadiness = lenderPackets.length
      ? Math.round(lenderPackets.reduce((sum, p) => sum + p.completionPct, 0) / lenderPackets.length)
      : 0;
    const angelReadiness = investorPackets.length
      ? Math.round(investorPackets.reduce((sum, p) => sum + p.completionPct, 0) / investorPackets.length)
      : 0;
