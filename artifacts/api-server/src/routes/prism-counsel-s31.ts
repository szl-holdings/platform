...
import {
  ...
  pcM365DeltaCursorsTable,
  pcM365SubscriptionsTable,
  pcModelLanesTable,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [lane] = await db
        .insert(pcModelLanesTable)
        .values({ ...parsed.data, orgId: getOrgId(req) })
        .returning();
      res.json(lane);
    } catch (e: any) {
      res.status(500).json({ error: e.message });
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await hfGateway.execute({
        orgId: getOrgId(req),
        task,
        input,
        options: { endpointId },
...
router.post(
  '/pressure-graph/:matterId/compute',
  validateBody(bodyShape({})),
  async (req: Request, res: Response) => {
    try {
...
router.post(
  '/proof-chain/:id/review',
  requireRole('super_admin', 'admin', 'compliance'),
  validateBody(
    bodyShape({
...
router.post(
  '/copilot/sessions/:sessionId/message',
  validateBody(
    bodyShape({
...
router.get('/m365/subscriptions', async (req: Request, res: Response) => {
  try {
    const subs = await db
      .select()
      .from(pcM365SubscriptionsTable)
      .where(eq(pcM365SubscriptionsTable.orgId, getOrgId(req)));
...
router.get('/m365/delta-cursors', async (req: Request, res: Response) => {
  try {
    const cursors = await db
      .select()
      .from(pcM365DeltaCursorsTable)
      .where(eq(pcM365DeltaCursorsTable.orgId, getOrgId(req)));
    res.json({ cursors });
  } catch (e: any) {
    res.status(500).json({ error: e.message });
  }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const costs = await db
        .select()
        .from(pcCostTrackingTable)
        .where(
          and(
            eq(pcCostTrackingTable.orgId, getOrgId(req)),
            sql`${pcCostTrackingTable.createdAt} >= ${since}`,
          ),
        )
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        modelMesh: {
          lanes: lanes.length,
          activeLanes: lanes.filter((l: any) => l.status === 'active').length,
        },
        hfGateway: {
          endpoints: hfEndpoints.length,
          healthy: hfEndpoints.filter((e) => e.status === 'healthy').length,
        },
        worldline: {
          sources: wlSources.length,
          active: wlSources.filter((s) => s.status === 'active').length,
        },
        m365: {
          subscriptions: m365Subs.length,
          active: m365Subs.filter((s) => s.status === 'active').length,
        },
        subsystems: [
