...
async function sodaFetch(url: string): Promise<unknown[]> {
  ...
  try {
    ...
    if (!r.ok) {
      throw new SodaProviderError(
        `NYC Open Data returned HTTP ${r.status} for ${new URL(url).pathname}`,
        r.status,
      );
    }
...
const SEED_PROPERTIES: Record<string, PropertyInfo> = {
  ...
  'dp-002': {
    id: 'dp-002',
    address: '422 East 138th St',
    borough: 'Bronx',
    zipCode: '10454',
    distressType: 'tax-lien',
    estimatedValue: 1250000,
    debtAmount: 890000,
    ownerName: 'Riverside Properties Inc',
    ownerType: 'corporate',
    opportunityScore: 74,
    yearBuilt: 1948,
    sqft: 3200,
  ...
  'dp-003': {
    id: 'dp-003',
    address: '1124 Boston Rd',
    borough: 'Bronx',
    zipCode: '10456',
    distressType: 'foreclosure',
    estimatedValue: 1650000,
    debtAmount: 1480000,
    ownerName: 'J. Morrow (Individual)',
  ...
  'dp-004': {
    id: 'dp-004',
    address: '88-12 Jamaica Ave',
    borough: 'Queens',
    zipCode: '11421',
    distressType: 'auction',
    estimatedValue: 3200000,
    debtAmount: 2600000,
    ownerName: 'Kingsway Commercial LLC',
    ownerType: 'llc',
    opportunityScore: 96,
    yearBuilt: 1971,
    sqft: 8400,
  ...
  'dp-005': {
    id: 'dp-005',
    address: '531 West 159th St',
    borough: 'Manhattan',
    zipCode: '10032',
    distressType: 'pre-foreclosure',
    estimatedValue: 4800000,
    debtAmount: 3600000,
    ownerName: 'Uptown Holdings Trust',
    ownerType: 'trust',
    opportunityScore: 79,
    yearBuilt: 1929,
    sqft: 9600,
...
async function fetchAcrisDeedHistory(
  ...
  if (bbl) {
    ...
    const streetFilter = streetPrimary
      ? ` AND street_name LIKE '%${soqlEscape(streetPrimary)}%'`
      : '';
  ...
  return {
    records: deeds.slice(0, 5),
    mortgages: mortgages.slice(0, 5),
    transferScore,
    ownershipEdges,
  };
}
...
async function fetchDofAssessment(
  ...
  let summary =
    'No DOF sales record found for this parcel — parcel may be exempt or non-arms-length';
  ...
  if (records.length > 0) {
    ...
    if (salePrice && salePrice > 10000) {
      ...
      score +=
        ltv !== null && ltv > 0.85 ? 10 : ltv !== null && ltv > 0.7 ? 6 : ltv !== null ? 2 : 0;
    }
...
function buildOwnershipChainFromProperty(
  ...
  const nodes: OwnerNode[] = [
    ...
    },
    {
      ...
      source:
        acrisEdges.length > 0
          ? `ACRIS — ${acrisEdges[0]?.traceRef ?? ''}`
          : (prop.connectorSource ?? 'Terra Distress Registry'),
...
function buildFinancingStress(
  ...
  const ltvEst =
    prop.estimatedValue && prop.debtAmount ? prop.debtAmount / prop.estimatedValue : null;

  const recentMortgage = mortgages[0];
  const maturityDate = recentMortgage
    ? null
    : mortgageAge > 5
      ? new Date(new Date().setFullYear(new Date().getFullYear() + Math.max(1, 7 - mortgageAge)))
          .toISOString()
          .slice(0, 10)
...
function buildDistressDecomposition(
  ...
  const factors: DistressFactorResult[] = [
    ...
    },
    {
      ...
      summary:
        acris.transferScore >= 10
          ? 'Multiple recent deed transfers — ownership instability signal'
          : acris.transferScore >= 5
            ? 'Recent transfer activity detected'
            : 'Stable ownership — no recent distress transfers',
      records: [],
    },
    {
  ...
  const tier = total >= 72 ? 'critical' : total >= 55 ? 'high' : total >= 38 ? 'medium' : 'low';
...
function buildHeadline(
  ...
  if (tier === 'critical')
    return `Distress score ${total}/100 — ${topFactor?.factor ?? 'multiple signals'} driving critical risk for ${ownerName}`;
  if (tier === 'high')
    return `Distress score ${total}/100 — high-conviction opportunity driven by ${topFactor?.factor ?? 'compounding signals'}`;
  if (tier === 'medium')
    return `Distress score ${total}/100 — early-stage distress signals across ${factors.filter((f) => f.score > 0).length} data sources`;
...
function buildMemoTemplate(
  ...
  return {
    ...
    disclaimer:
      'This memo is generated from NYC public data sources via Terra intelligence engine. Verify all facts before transacting. Not investment advice.',
  };
}
...
router.get('/terra/why-this-property/:propertyId', limiter, auth, async (req, res) => {
  try {
    ...
    const result = await getCached(cacheKey, 60 * 60 * 1000, async () => {
      ...
      const neighborhoodMotion = await fetchNeighborhoodMotion(
        prop.borough,
        prop.zipCode,
        dobData,
      ).catch(() => ({
