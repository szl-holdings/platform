...
seedScenariosIfEmpty(SEED_SCENARIOS).then(seeded => {
  if (seeded) {
    return Promise.all(SEED_RUNS.map(run =>
      persistRun(run).catch(err => console.warn("[replay-seed] Failed to seed run", run.runId, err))
    ));
  }
