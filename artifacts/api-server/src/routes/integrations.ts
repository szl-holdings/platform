...
router.get(
  '/integrations/salesforce/query',
  authMiddleware(),
  requireRole('ops', 'analyst', 'super_admin', 'admin'),
    ...
    try {
      ...
      switch (object?.toLowerCase()) {
        ...
        default:
          sendBadRequest(
            res,
            'object parameter required: account, opportunity, case, lead, pipeline_health, signals — or provide soql parameter',
          );
      }
    } catch (err) {
...
router.post(
  '/integrations/salesforce/push/task',
  authMiddleware(),
...
router.post(
  '/integrations/salesforce/push/case',
  authMiddleware(),
  requireRole('ops', 'super_admin', 'admin'),
  validateBody(
    bodyShape({
...
router.post(
  '/integrations/salesforce/webhook',
  validateBody(
    bodyShape({
      description: z.unknown().optional(),
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!secretValid) {
        logger.warn(
          'integrations: Salesforce webhook rejected — invalid or missing x-szl-webhook-secret',
        );
    ...
    try {
      ...
      switch (type?.toLowerCase()) {
        ...
        case 'issues': {
          const resolvedJql =
            jql ??
            'project in (LYTE, VESSEL, ALLOY, OPS) AND statusCategory != Done ORDER BY updated DESC';
        ...
        default:
          sendBadRequest(
            res,
            'type parameter required: projects, issues, sprints, sprint_health, signals — or provide jql parameter for issue search',
          );
      }
    } catch (err) {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { projectKey, summary, description, issueType, priority, labels, source } =
        req.body as {
          projectKey?: string;
          summary?: string;
          description?: string;
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!flagEnabled) {
        res
          .status(200)
          .json({ received: true, processed: false, reason: 'jira_webhook_enabled flag is off' });
        return;
      }
...
router.get('/integrations/health', authMiddleware(), async (_req, res) => {
  try {
    ...
    const [sfFlags, jiraFlags] = await Promise.all([
      ...
      Promise.all([isFlagEnabled('jira_sync_enabled'), isFlagEnabled('jira_webhook_enabled')]),
    ]);
...
router.post(
  '/integrations/salesforce/ingest-signals',
  validateBody(bodyShape({})),
  authMiddleware(),
  requireRole('ops', 'super_admin', 'admin'),
  async (_req, res) => {
...
router.post(
  '/integrations/jira/ingest-signals',
  validateBody(bodyShape({})),
  authMiddleware(),
  requireRole('ops', 'super_admin', 'admin'),
  async (_req, res) => {
...
function generateOAuthState(provider: 'salesforce' | 'jira'): string {
  if (!OAUTH_STATE_SECRET)
    throw new Error('OAUTH_STATE_SECRET env var is required to initiate OAuth flows');
...
function validateOAuthState(
  ...
  expectedProvider: 'salesforce' | 'jira',
): boolean {
    ...
    try {
      ...
      if (error) {
        logger.warn(
          ...
          'integrations: Salesforce OAuth callback — authorization denied',
        );
        res.redirect(
          `${PLATFORM_UI_URL}/integrations/salesforce?error=${encodeURIComponent(error_description ?? error)}`,
        );
        return;
      }
      ...
      if (!validateOAuthState(state, 'salesforce')) {
        logger.warn(
          { statePresent: !!state },
          'integrations: Salesforce OAuth callback — invalid or expired state',
        );
      ...
      logger.info(
        { codePresent: true },
        'integrations: Salesforce OAuth callback received — exchange pending',
      );
      ...
      if (!tokenRes.ok) {
        ...
        logger.error(
          ...
          'integrations: Salesforce token exchange failed',
        );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (error) {
        logger.warn(
          { error, error_description },
          'integrations: Jira OAuth callback — authorization denied',
        );
        res.redirect(
          `${PLATFORM_UI_URL}/integrations/jira?error=${encodeURIComponent(error_description ?? error)}`,
        );
        return;
      }
      ...
      if (!validateOAuthState(state, 'jira')) {
        logger.warn(
          { statePresent: !!state },
          'integrations: Jira OAuth callback — invalid or expired state',
        );
      ...
      logger.info(
        { codePresent: true },
        'integrations: Jira OAuth callback received — exchange pending',
      );
      ...
      logger.info(
        ...
        'integrations: Jira OAuth token exchange successful — persisting connector',
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [existing] = await db
        ...
        .where(
          and(
            eq(connectorsTable.type, 'custom'),
            eq(connectorsTable.name, `atlassian:${tenant.clientKey}`),
          ),
        );
...
router.get(
  '/integrations/atlassian/tenant/:clientKey',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
      ...
      const [connector] = await db
        ...
        .where(
          and(
            eq(connectorsTable.type, 'custom'),
            eq(connectorsTable.name, `atlassian:${clientKey}`),
          ),
        );
