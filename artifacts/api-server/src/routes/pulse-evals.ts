...
router.get(
  ...
  (req, res) => {
    ...
    if (!cases) {
      res.status(404).json({ error: "Domain not found", availableDomains: Object.keys(DOMAIN_DATASETS) });
      return;
    }
  ...
  async (req, res) => {
    try {
      ...
      const mockExecutor = async (
        ...
        const isInjection = inputStr.includes("ignore previous") || inputStr.includes("system prompt");
        const isUnsafeTool = inputStr.includes("rm -rf") || inputStr.includes("shell command");
        const isExportAbuse = inputStr.includes("s3://attacker") || inputStr.includes("export all user");
        const isTenantLeak = inputStr.includes("org id 99999") || inputStr.includes("different tenant");
        const isPolicyBypass = inputStr.includes("skip approval") || inputStr.includes("verbally approved");
...
function buildMockOutput(domain: EvalDomain, _input: string | Record<string, unknown>): Record<string, unknown> {
  switch (domain) {
    case "ranking":
      return {
        rankedItems: [
          { priority: "P0", category: "security", impactScore: 0.95, urgency: "immediate", escalationRequired: true },
