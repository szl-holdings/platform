...
router.get("/terra/properties/:id/climate-risk", authOptional, (req: Request, res: Response) => {
  try {
    ...
    const overallGrade = overallRiskScore < 35 ? "A" : overallRiskScore < 55 ? "B" : overallRiskScore < 72 ? "C" : "D";
    const riskLevels: Array<"Negligible" | "Low" | "Medium" | "High" | "Critical"> = ["Negligible","Low","Medium","High","Critical"];
    ...
    const data = {
      ...
      regulatoryFlags: [
        ...
        ...(rng(6, 0, 1) ? ["SB 54 — Climate Disclosure (CA)"] : ["HB 1557 — Mandatory Flood Disclosure (FL)"]),
      ],
      hazards: [
        { type: "flood", current: toRisk(rng(7, 0, 100)), projected2030: toRisk(rng(8, 0, 100)), projected2050: toRisk(Math.min(100, rng(9, 0, 100) + 10)), trend: rng(10, 0, 1) ? "increasing" : "stable", detail: "FEMA flood zone assessment based on current SFHA mapping and 2050 SLR projections." },
        { type: "wildfire", current: toRisk(rng(11, 0, 100)), projected2030: toRisk(Math.min(100, rng(12, 0, 100) + 8)), projected2050: toRisk(Math.min(100, rng(13, 0, 100) + 18)), trend: "increasing", detail: "Cal Fire WUI hazard zone — smoke exposure and direct ignition risk modeled via EMBR platform." },
        { type: "heat", current: toRisk(rng(14, 0, 60)), projected2030: toRisk(rng(15, 0, 75)), projected2050: toRisk(Math.min(100, rng(16, 0, 80) + 15)), trend: rng(17, 0, 1) ? "increasing" : "stable", detail: "Urban heat island effect combined with 2050 IPCC temperature anomalies applied to local NOAA data." },
        { type: "seismic", current: toRisk(rng(18, 0, 100)), projected2030: toRisk(rng(19, 0, 100)), projected2050: toRisk(rng(20, 0, 100)), trend: "stable", detail: `Site class ${["B","C","D"][rng(21, 0, 2)]} soils. Estimated PML: ${rng(22, 6, 28)}%.` },
        { type: "storm", current: toRisk(rng(23, 0, 60)), projected2030: toRisk(rng(24, 0, 75)), projected2050: toRisk(Math.min(100, rng(25, 0, 80) + 10)), trend: rng(26, 0, 1) ? "increasing" : "stable", detail: "Atmospheric river intensification projected. 100-year storm return period shortening under RCP 8.5." },
        { type: "sea-level", current: toRisk(rng(27, 0, 80)), projected2030: toRisk(Math.min(100, rng(28, 0, 80) + 10)), projected2050: toRisk(Math.min(100, rng(29, 0, 80) + 20)), trend: "increasing", detail: `${(rng(30, 14, 36) / 10).toFixed(1)}ft SLR projected by 2060 per NOAA Intermediate High scenario.` },
      ],
      dataSource: "FIRST STREET FOUNDATION / NOAA / FEMA (seeded)",
      generatedAt: new Date().toISOString(),
    };
    sendSuccess(res, { data, propertyId, dataMode: "seeded" });
  } catch (err) { handleRouteError(res, err, "Failed to generate climate risk data"); }
});
...
router.get("/terra/properties/:id/zoning", authOptional, (req: Request, res: Response) => {
  try {
    ...
    const zones = ["Commercial Mixed-Use","Medium-Density Residential","General Commercial","Mixed Use Medium","Transit-Oriented Development","Light Industrial","Business District","Residential High Density"];
    ...
    const scenarios = [
      { id: "s1", name: "As-of-Right Development", type: "Mixed Use", units: Math.floor(maxUnits * 0.68), grossSqft: Math.floor(lotSqft * maxFar * 0.88), far: parseFloat((maxFar * 0.88).toFixed(1)), stories: rng(11, 4, 7), parkingSpaces: Math.floor(maxUnits * 0.68 * 0.5), estimatedRevenue: rng(12, 12_000_000, 38_000_000), constructionCost: rng(13, 9_000_000, 28_000_000), landValue: rng(14, 2_000_000, 8_000_000), requiresVariance: false, varianceProbability: 100, timelineMonths: rng(16, 18, 28) },
      { id: "s2", name: "Maximum Density Residential", type: "Multifamily", units: maxUnits, grossSqft: Math.floor(lotSqft * maxFar), far: maxFar, stories: rng(17, 6, 10), parkingSpaces: Math.floor(maxUnits * 0.5), estimatedRevenue: rng(18, 20_000_000, 52_000_000), constructionCost: rng(19, 14_000_000, 38_000_000), landValue: rng(20, 2_000_000, 8_000_000), requiresVariance: true, varianceProbability: rng(22, 45, 85), timelineMonths: rng(23, 24, 36) },
      { id: "s3", name: "Office + Retail Conversion", type: "Commercial", units: 0, grossSqft: Math.floor(lotSqft * maxFar * 0.75), far: parseFloat((maxFar * 0.75).toFixed(1)), stories: rng(24, 3, 6), parkingSpaces: rng(25, 30, 80), estimatedRevenue: rng(26, 10_000_000, 32_000_000), constructionCost: rng(27, 8_000_000, 24_000_000), landValue: rng(28, 2_000_000, 8_000_000), requiresVariance: false, varianceProbability: 100, timelineMonths: rng(30, 16, 26) },
    ];
    const bestScenario = scenarios[0];
    const bestProfit = bestScenario.estimatedRevenue - bestScenario.constructionCost - bestScenario.landValue;
    const data = {
      propertyId,
      ...
      overlayDistricts: [
        ["Transit-Oriented Development","Opportunity Zone","Arts & Cultural District","Affordable Housing Overlay","Inclusionary Housing"][rng(9, 0, 4)],
        ...(rng(10, 0, 1) ? [["Historic Preservation","Urban Renewal","Design Review"][rng(11, 0, 2)]] : []),
      ],
      scenarios,
    ...
    sendSuccess(res, { data, propertyId, dataMode: "seeded" });
  } catch (err) { handleRouteError(res, err, "Failed to generate zoning data"); }
});

// ---------------------------------------------------------------------------
// Neighborhood Momentum
// ---------------------------------------------------------------------------
router.get("/terra/properties/:id/neighborhood-momentum", authOptional, (req: Request, res: Response) => {
  try {
    const propertyId = req.params.id as string;
    const rng = seedRng(propertyId);
    const trajectories = ["accelerating","gentrifying","stable","declining","distressed"] as const;
    const trajectory = trajectories[rng(0, 0, trajectories.length - 1)];
    const momentumScore = trajectory === "accelerating" ? rng(1, 72, 98) :
      trajectory === "gentrifying" ? rng(2, 55, 80) :
      trajectory === "stable" ? rng(3, 40, 65) :
      trajectory === "declining" ? rng(4, 20, 45) : rng(5, 5, 28);
    const data = {
      propertyId,
      trajectory,
      momentumScore,
      ...
      microMarkets: [
        { name: "Core Submarket", score: momentumScore, trajectory, deltaQoQ: parseFloat((rng(22, -15, 25) / 10).toFixed(1)) },
        { name: "Adjacent Corridor", score: Math.min(100, momentumScore + rng(23, -15, 20)), trajectory: trajectories[rng(24, 0, 4)], deltaQoQ: parseFloat((rng(25, -10, 20) / 10).toFixed(1)) },
        { name: "Broader Metro", score: rng(26, 35, 70), trajectory: trajectories[rng(27, 0, 4)], deltaQoQ: parseFloat((rng(28, -8, 12) / 10).toFixed(1)) },
      ],
      dataSource: "COSTAR / REDFIN / YELP FUSION / WALK SCORE (seeded)",
      generatedAt: new Date().toISOString(),
    };
    sendSuccess(res, { data, propertyId, dataMode: "seeded" });
  } catch (err) { handleRouteError(res, err, "Failed to generate neighborhood momentum data"); }
});

// ---------------------------------------------------------------------------
// Seller Motivation Predictor
// ---------------------------------------------------------------------------
router.get("/terra/properties/:id/seller-motivation", authOptional, (req: Request, res: Response) => {
  try {
    const propertyId = req.params.id as string;
    const rng = seedRng(propertyId);
    const acceptanceScore = rng(0, 15, 95);
    const acceptanceCategory = acceptanceScore >= 75 ? "very-likely" :
      acceptanceScore >= 55 ? "likely" :
      acceptanceScore >= 35 ? "possible" : "unlikely";
    const daysOnMarket = rng(6, 12, 220);
    const motivationFactors = [
      { factor: "Tax delinquency severity", weight: 0.20 },
      ...
      { factor: "Absentee owner flag", weight: 0.10 },
      { factor: "Permit lapse / code violations", weight: 0.10 },
    ].map((f, i) => ({ ...f, weight: parseFloat((f.weight * (0.8 + rng(i + 7, 0, 40) / 100)).toFixed(2)) }));
    const data = {
      propertyId,
      acceptanceScore,
      acceptanceCategory,
      ...
      outreachScript: acceptanceCategory === "very-likely"
    ...
    sendSuccess(res, { data, propertyId, dataMode: "seeded" });
  } catch (err) { handleRouteError(res, err, "Failed to generate seller motivation data"); }
});

// ---------------------------------------------------------------------------
// Spatial Walkthrough
// ---------------------------------------------------------------------------
router.get("/terra/properties/:id/spatial-walkthrough", authOptional, (req: Request, res: Response) => {
  try {
    const propertyId = req.params.id as string;
    const rng = seedRng(propertyId);
    const roomTypes = ["Living Room","Kitchen","Primary Suite","Office","Bedroom 2","Dining Room","Garage","Bathroom"] as const;
    ...
    const rooms = Array.from({ length: numRooms + 2 }, (_, i) => {
      ...
      return {
        id: `room-${i}`,
        name: i === 0 ? "Living Room" : i === 1 ? "Kitchen" : roomTypes[rng(1 + i, 0, roomTypes.length - 1)],
        sqft: rng(2 + i, 100, 680),
        condition: conditions[rng(3 + i, 0, conditions.length - 1)],
        ceiling: parseFloat((rng(5 + i, 8, 12) + 0.5).toFixed(1)),
        renovationOptions: [
          { name: "Full Renovation", cost: renovCost, valueAdd, timelineWeeks: rng(i * 5 + 22, 4, 12) },
          { name: "Cosmetic Update", cost: Math.round(renovCost * 0.35), valueAdd: Math.round(valueAdd * 0.4), timelineWeeks: rng(i * 5 + 23, 1, 4) },
        ],
      };
    });
    ...
    const data = {
      ...
      rooms,
      stagingOptions: [
        { name: "Modern Minimalist", description: "Clean lines, neutral palette, contemporary furniture", estimatedValue: rng(30, 15_000, 65_000) },
        { name: "Traditional Classic", description: "Warm tones, classic furniture, layered textures", estimatedValue: rng(31, 12_000, 50_000) },
        { name: "Luxury Premium", description: "High-end finishes, designer pieces, curated art", estimatedValue: rng(32, 25_000, 95_000) },
      ],
      recentInspection: {
        date: `20${rng(16, 22, 25)}-${String(rng(17, 1, 12)).padStart(2,"0")}-${String(rng(18, 1, 28)).padStart(2,"0")}`,
        inspector: ["Pacific Inspection Group","National Property Inspectors","Metro Verify","SafeHouse Inspections"][rng(19, 0, 3)],
    ...
    sendSuccess(res, { data, propertyId, dataMode: "seeded" });
  } catch (err) { handleRouteError(res, err, "Failed to generate spatial walkthrough data"); }
});
...
router.get("/terra/properties/:id/waterfall", authOptional, (req: Request, res: Response) => {
  try {
    ...
    const catchUpTarget = remainingAfterPref > 0
      ? (catchUpPct / 100) * (prefReturnAmount / (1 - catchUpPct / 100))
      : 0;
    ...
    const gpTotal = gpEquity + (prefReturnAmount * gpContributionPct / 100) + catchUpAmount + gpPromote;
    ...
    const data = {
      ...
      tiers: [
        ...
        { description: `Preferred Return — ${preferredReturn}% × ${holdMonths}mo`, gpAmount: prefReturnAmount * gpContributionPct / 100, lpAmount: prefReturnAmount * (100 - gpContributionPct) / 100 },
        { description: `GP Catch-Up — ${catchUpPct}%`, gpAmount: catchUpAmount, lpAmount: 0 },
        { description: `Promoted Interest — ${promotePct}% GP promote`, gpAmount: gpPromote, lpAmount: lpRemainder },
      ],
      dataSource: "COMPUTED FROM PROPERTY SEED (seeded)",
      generatedAt: new Date().toISOString(),
    };
    sendSuccess(res, { data, propertyId, dataMode: "seeded" });
  } catch (err) { handleRouteError(res, err, "Failed to compute waterfall data"); }
});
