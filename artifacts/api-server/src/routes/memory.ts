...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const { tier, key, scopeId, tags, includeStale, minConfidence, search, sortBy } =
        req.query as {
          tier?: string;
          key?: string;
          scopeId?: string;
      ...
      if (validatedTier === 'invalid') {
        sendBadRequest(
          res,
          `Invalid tier '${tier}'. Must be one of: ${ALL_MEMORY_TYPES.join(', ')}`,
        );
        return;
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!result) {
        sendSuccess(res, {
          distilled: false,
          reason: 'Not enough high-quality feedback to distill',
        });
        return;
      }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { halfLifeDays, staleThreshold } = req.body as {
        halfLifeDays?: number;
...
router.get('/memory/tiers/overview', authMiddleware(), async (_req: Request, res: Response) => {
  try {
    const TIER_DEFS: Array<{
    ...
    }> = [
      {
        tier: 'working',
        label: 'Ephemeral Run',
        description: 'In-flight data for a single agent run; discarded when the run completes.',
        retentionPolicy: 'ephemeral',
      },
      {
        tier: 'session',
        label: 'Session',
        description: 'Context shared across steps within a user or agent session.',
        retentionPolicy: 'session-scoped',
      },
      {
        tier: 'episodic',
        label: 'Workspace Episodes',
        description:
          'Event records and observations scoped to a workspace; compacted into semantic memory.',
        retentionPolicy: 'workflow-scoped',
      },
      {
        tier: 'entity',
        label: 'Entity',
        description:
          'Durable facts about specific entities (vessels, properties, clients, assets).',
      ...
      },
      {
        ...
        description:
          'Distilled lessons and playbooks derived from high-confidence episodic evidence.',
        retentionPolicy: 'archival',
      },
      {
        tier: 'semantic',
        label: 'Semantic',
        description: 'Summarised knowledge extracted from episodes.',
        retentionPolicy: 'persistent',
      },
      {
        tier: 'workflow',
        label: 'Workflow',
        description: 'Workflow execution state and cross-step context.',
        retentionPolicy: 'workflow-scoped',
      },
      {
        tier: 'artifact',
        label: 'Artifact',
        description: 'Generated artifacts (documents, drafts) linked to actions.',
        retentionPolicy: 'persistent',
      },
      {
        tier: 'operator-feedback',
        label: 'Operator Feedback',
        description: 'Structured feedback from operators used to distill lessons.',
        retentionPolicy: 'persistent',
      },
      {
        tier: 'executive',
        label: 'Executive',
        description: 'High-level summaries surfaced in executive briefings.',
        retentionPolicy: 'persistent',
      },
    ];
    ...
    const overview = TIER_DEFS.map((def) => ({
      ...
      pinnedCount: memoryStore
        .list({ tier: def.tier, includeStale: true })
        .filter((e) => e.retention.pinned).length,
      staleCount: memoryStore
        .list({ tier: def.tier, includeStale: true })
        .filter((e) => e.freshness.isStale).length,
    }));
