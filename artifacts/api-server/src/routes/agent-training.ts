...
trainingRouter.post(
  '/agent-training/pairs',
  validateBody(trainingPairSchema),
  async (req: Request, res: Response) => {
    try {
      const { agentId, question, answer, category } = req.body as z.infer<
        typeof trainingPairSchema
      >;
      const [pair] = await db
      ...
      if (existing.length > 0) {
        ...
        const [created] = await db
          ...
          .values({
            ...
            domainJargon: domainJargon ?? true,
...
trainingRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { agentId, rating, messageContent, responseContent, feedbackNote } =
        req.body as z.infer<typeof feedbackSchema>;
      const [fb] = await db
...
trainingRouter.post(
  '/agent-training/advisory-audit',
  validateBody(advisoryAuditSchema),
  async (req: Request, res: Response) => {
    try {
      const { agentId, recommendationType, riskLevel, title, description, runbook } =
        req.body as z.infer<typeof advisoryAuditSchema>;
...
trainingRouter.patch(
  '/agent-training/advisory-audit/:id/action',
