...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { signers } = req.body as {
        signers?: Array<{ email: string; name: string; order?: number }>;
      };
      if (!signers || signers.length === 0)
        return sendBadRequest(res, 'At least one signer is required');
...
router.get('/documents/sign/:token', async (req, res) => {
  try {
    ...
    const [sig] = await db
      ...
      .where(eq(signaturesTable.signingToken, token));
    ...
    const priorUnsigned = allSigs.filter(
      (s) => s.signingOrder < sig.signingOrder && s.status !== 'signed',
    );

    if (priorUnsigned.length > 0) {
      return sendBadRequest(
        res,
        `Signing order enforced: ${priorUnsigned.length} prior signer(s) must sign first`,
      );
    }
    ...
    sendSuccess(res, {
      signature: { ...sig, status: sig.status === 'pending' ? 'viewed' : sig.status },
      document: { id: doc.id, title: doc.title, type: doc.type, appSource: doc.appSource },
    });
  } catch (err) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (sig.status === 'signed')
        return sendBadRequest(res, 'Document already signed by this signer');
      if (sig.status === 'declined')
        return sendBadRequest(res, 'This signing request was declined');
      ...
      if (priorUnsigned.length > 0) {
        return sendBadRequest(
          res,
          `Signing order enforced: ${priorUnsigned.length} prior signer(s) must complete signing first`,
        );
      }
      ...
      if (!signatureData || !signatureType)
        return sendBadRequest(res, 'Signature data and type are required');
      if (!['typed', 'drawn', 'uploaded'].includes(signatureType))
      ...
      const fpHash = browserFingerprint
        ? createHash('sha256').update(browserFingerprint).digest('hex')
        : 'no-fp';
      const auditHash = createHash('sha256')
        .update(
          `${sig.documentId}:${sig.signerEmail}:${signatureType}:${signatureContentHash}:${fpHash}:${Date.now()}`,
        )
        .digest('hex');
      ...
      sendSuccess(res, {
        ...
        remainingSigners: refreshedSigs.filter(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!isPrivileged && callerEmail !== sig.signerEmail) {
        return res
          .status(403)
          .json({ error: 'You are not authorised to sign on behalf of this signer' });
      }
      ...
      if (sig.expiresAt && sig.expiresAt < new Date())
        return sendBadRequest(res, 'Signing request has expired');

      const { signatureImage, browserFingerprint } = req.body as {
      ...
      const auditHash = createHash('sha256')
        .update(
          `${docId}:${sig.signerEmail}:embedded:${signatureContentHash}:${documentContentHash}:${fpHash}:${Date.now()}`,
        )
        .digest('hex');
...
router.post(
  '/documents/:id/signatures/:sigId/decline',
  authMiddleware(),
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!isPrivileged && callerEmail !== sig.signerEmail) {
        return res
          .status(403)
          .json({ error: 'You are not authorised to decline on behalf of this signer' });
      }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (sig.status === 'signed')
        return sendBadRequest(res, 'Cannot decline: document already signed');
...
router.post(
  '/documents/:id/signatures/:sigId/remind',
  authMiddleware(),
  validateBody(bodyShape({})),
  async (req: Request, res: Response) => {
    try {
