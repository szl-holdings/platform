  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!canMutateDocument(userId, role, existing)) {
        return res
          .status(403)
          .json({ error: 'Access denied: you do not have permission to modify this document' });
      }
      ...
      if (contentJson !== undefined) {
        const [{ maxVer }] = await db
          .select({ maxVer: sql<number>`max(version)` })
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [ver] = await db
        ...
        .from(documentVersionsTable)
        .where(
          and(eq(documentVersionsTable.documentId, id), eq(documentVersionsTable.version, version)),
        );
      ...
      await db
        .update(documentsTable)
        .set({ contentJson: ver.contentJson, updatedAt: new Date() })
        .where(eq(documentsTable.id, id));
      await db.insert(documentVersionsTable).values({
        documentId: id,
        version: (maxVer || 0) + 1,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { content, sectionRef, authorName } = req.body as {
        content?: string;
        sectionRef?: string;
        authorName?: string;
      };
...
router.patch(
  '/documents/comments/:commentId/resolve',
  authMiddleware(),
  validateBody(bodyShape({})),
  async (req: Request, res: Response) => {
    try {
      ...
      if (role !== 'admin' && role !== 'editor' && comment.authorId !== userId) {
        return res.status(403).json({
          error: 'Access denied: only the comment author, editors, or admins can resolve comments',
        });
      }
...
router.get('/documents/templates', validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    if (appSource)
      conditions.push(
        eq(
          documentTemplatesTable.appSource,
          appSource as typeof documentTemplatesTable.appSource._.data,
        ),
      );
  ...
  try {
    ...
    const [template] = bySlug
      ? await db
          .select()
          .from(documentTemplatesTable)
          .where(eq(documentTemplatesTable.slug, req.params.id))
...
router.get('/documents/content-library', validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    if (category)
      conditions.push(
        eq(
          contentLibraryBlocksTable.category,
          category as typeof contentLibraryBlocksTable.category._.data,
        ),
      );
    ...
    const blocks = await db
      .select()
      .from(contentLibraryBlocksTable)
      .where(and(...conditions))
      .orderBy(contentLibraryBlocksTable.category, contentLibraryBlocksTable.title);
