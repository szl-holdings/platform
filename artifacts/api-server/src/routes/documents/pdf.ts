...
router.get('/documents/batch-pdf/:batchId', authMiddleware(), async (req, res) => {
  try {
    ...
    const jobs = await db
      .select()
      .from(pdfJobsTable)
      .where(eq(pdfJobsTable.batchId, batchId))
      .orderBy(pdfJobsTable.createdAt);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (appSource)
        conditions.push(
          eq(pdfBatchesTable.appSource, appSource as typeof pdfBatchesTable.appSource._.data),
        );
...
router.post(
  '/documents/batch-pdf/:batchId/cancel',
  authMiddleware(),
  ...
  async (req, res) => {
    try {
      ...
      await db
        ...
        .where(and(eq(pdfJobsTable.batchId, batchId), eq(pdfJobsTable.status, 'pending')));
      await db
        .update(pdfBatchesTable)
...
router.post(
  '/documents/pdf-jobs/:jobId/retry',
  authMiddleware(),
  validateBody(bodyShape({})),
  async (req, res) => {
    try {
      ...
      await db
        ...
        .where(eq(pdfJobsTable.id, jobId));
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (appSource)
        docConditions.push(
          eq(documentsTable.appSource, appSource as typeof documentsTable.appSource._.data),
        );
...
async function processJobAsync(job: typeof pdfJobsTable.$inferSelect) {
  ...
  const [batch] = await db
    .select()
    .from(pdfBatchesTable)
    .where(eq(pdfBatchesTable.batchId, job.batchId));
  ...
  try {
    ...
    try {
      ...
      logger.warn(
        { err: storageErr },
        '[pdf-batch] Object storage upload failed, falling back to base64',
      );
    }
...
router.get('/documents/pdf-output/:filename', authMiddleware(), async (req, res) => {
  try {
    ...
    const [job] = await db
      .select()
      .from(pdfJobsTable)
      .where(eq(pdfJobsTable.outputFilename, filename));
    ...
    if (storageKey) {
      ...
      try {
        ...
        if (err instanceof ObjectNotFoundError)
          return sendNotFound(res, 'PDF output not found in storage');
        throw err;
      }
    }
...
router.get('/documents/:id/pdf', authMiddleware(), async (req, res) => {
  try {
    ...
    const content = (doc.contentJson || { blocks: [], version: 1 }) as {
      blocks: BlockNode[];
      version: number;
    };
...
router.get(
  '/documents/:id/versions/:versionA/diff/:versionB',
  authMiddleware(),
  async (req, res) => {
    try {
      const docId = parseInt(req.params.id as string, 10);
      ...
      const contentA: EditorContent | null =
        versionA === 0 ? latestContent : ((snapA?.contentJson as EditorContent | null) ?? null);
      const contentB: EditorContent | null =
        versionB === 0 ? latestContent : ((snapB?.contentJson as EditorContent | null) ?? null);
...
async function getDocuSignAccessToken(): Promise<string> {
  ...
  if (!DOCUSIGN_CLIENT_ID || !DOCUSIGN_USER_ID || !DOCUSIGN_PRIVATE_KEY) {
    throw new Error(
      'DocuSign credentials not configured (DOCUSIGN_CLIENT_ID, DOCUSIGN_USER_ID, DOCUSIGN_PRIVATE_KEY)',
    );
  }
...
router.post(
  '/documents/:id/docusign/embed/:sigId',
  authMiddleware(),
  validateBody(bodyShape({})),
  async (req: Request, res: Response) => {
    try {
