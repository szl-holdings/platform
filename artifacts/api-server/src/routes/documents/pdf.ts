...
router.post("/documents/batch-pdf", authMiddleware(), validateBody(bodyShape({
...
router.post("/documents/batch-pdf/:batchId/cancel", authMiddleware(), validateBody(bodyShape({})), async (req, res) => {
  try {
    const { batchId } = req.params as Record<string, string>;
    const [batch] = await db.select().from(pdfBatchesTable).where(eq(pdfBatchesTable.batchId, batchId));
    if (!batch) return sendNotFound(res, "Batch");
...
router.post("/documents/pdf-jobs/:jobId/retry", authMiddleware(), validateBody(bodyShape({})), async (req, res) => {
  try {
...
router.post("/documents/:id/docusign/send", authMiddleware(), validateBody(bodyShape({})), async (req: Request, res: Response) => {
  try {
    const docId = parseInt(req.params.id as string, 10);
    const [doc] = await db.select().from(documentsTable).where(eq(documentsTable.id, docId));
    if (!doc) return sendNotFound(res, "Document");
...
router.post("/documents/docusign/webhook", validateBody(bodyShape({})), async (req: Request, res: Response) => {
  try {
...
router.post("/documents/:id/docusign/embed/:sigId", authMiddleware(), validateBody(bodyShape({})), async (req: Request, res: Response) => {
  try {
    const docId = parseInt(req.params.id as string, 10);
