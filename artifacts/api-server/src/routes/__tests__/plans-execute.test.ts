...
describe('POST /api/plans/:id/execute', () => {
  ...
  it('runs the plan to completion and returns run + persisted plan', async () => {
    ...
    const res = await request(app).post(`/api/plans/${plan.planId}/execute`).send({}).expect(200);
  ...
  it('returns awaiting-approval when a gated step blocks the run, then resumes after step approval', async () => {
    ...
    const first = await request(app).post(`/api/plans/${plan.planId}/execute`).send({}).expect(200);
  ...
  it('returns 404 for unknown plans', async () => {
    const app = buildApp();
    await request(app).post('/api/plans/does-not-exist/execute').send({}).expect(404);
  });
