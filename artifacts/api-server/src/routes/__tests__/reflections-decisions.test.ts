...
describe('reflections decision routes', () => {
  ...
  it('rejects a skill and keeps the most recent decision per skill', async () => {
    const app = buildApp();
    await request(app).post('/api/reflections/ref-001/skills/sk1/adopt').send({}).expect(200);
    const res = await request(app)
  ...
  it('rejects invalid strategy indices', async () => {
    const app = buildApp();
    await request(app).post('/api/reflections/ref-002/strategy/-1/apply').send({}).expect(400);
    ...
    await request(app).post('/api/reflections/ref-002/strategy/9999/apply').send({}).expect(400);
  });
  ...
  it("rejects empty skill names via the route (returns 404 since route won't match)", async () => {
    ...
    await request(app).post('/api/reflections/ref-001/skills//adopt').send({}).expect(404);
  });
});
