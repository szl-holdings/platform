...
vi.mock('@szl-holdings/db', () => {
  ...
  function flattenWhere(
    node: unknown,
    out: Array<{ col?: string; val?: unknown; op: string }> = [],
  ): typeof out {
  ...
  return {
      ...
      select() {
        ...
        const chain: Record<string, unknown> = {
          ...
          where: (predicate: unknown) => {
            state.filters.push(...flattenWhere(predicate));
            return chain;
          },
          innerJoin: () => chain,
          orderBy: () => chain,
    ...
    alloyWorkflowsTable: {
      id: col('id'),
      orgId: col('org_id'),
      runCount: col('run_count'),
      lastRunAt: col('last_run_at'),
    ...
    alloyApprovalsTable: {},
    alloyAuditLogTable: Object.assign(
      { _name: 'alloy_audit_log' },
      {
        id: col('id'),
        orgId: col('org_id'),
        userId: col('user_id'),
    ...
    alloySkills: {},
    alloySkillRuns: {},
    alloyAutonomyModesTable: Object.assign(
      { _name: 'alloy_autonomy_modes' },
      {
        id: col('id'),
...
describe('Autonomy mode → /alloy/recommend (end-to-end)', () => {
  ...
  it('PATCH observe → /alloy/recommend returns 409 AUTONOMY_BLOCKED, audit row written', async () => {
    ...
    const recRes = await request(app).post('/alloy/recommend').send({
