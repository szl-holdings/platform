...
let authUser: {
  ...
  roles: ["super_admin"],
  orgs: [
    { orgId: TEST_ORG_ID, orgSlug: "tool-mesh-test", orgName: "Tool Mesh Test", role: "super_admin" },
  ],
};
...
async function cleanup(): Promise<void> {
  ...
  await db
    .delete(toolMeshToolsTable)
    .where(like(toolMeshToolsTable.toolId, `${TOOL_PREFIX}%`));
...
describe("Guardian routes — engine resync after route mutation", () => {
  it("POST /policies adds the new rule to the in-process decision engine", async () => {
    ...
    const ids = getGuardianEngine().getRules().map((r) => r.id);
