...
function wizardStateRows(overrides: Partial<{
  current_step: string;
...
describe("POST /onboarding/org — create organization", () => {
  ...
  it("returns 201 and the new org when slug is unique", async () => {
    _selectQueue = [[]];
    _insertReturnQueue = [
      [{ ...ORG, id: 99, name: "New Co", slug: "new-co" }],
      [],
      [],
      [],
    ];
  ...
  it("accepts optional domain and orgType fields", async () => {
    ...
    const res = await request(app)
      .post("/onboarding/org")
      .send({ name: "Enterprise Co", slug: "enterprise-co", domain: "enterprise.co", orgType: "enterprise" });
...
describe("PUT /onboarding/wizard/:orgSlug — wizard step progression", () => {
  ...
  it("completes the notifications step and advances to integrations", async () => {
    _selectQueue = [[ORG], [OWNER_MEMBERSHIP]];
    _poolQueryQueue = [
      { rows: wizardStateRows({ completed_steps: ["profile", "team"], current_step: "notifications" }) },
      { rows: [] },
    ];
  ...
  it("completes the integrations step and sets currentStep to complete", async () => {
    _selectQueue = [[ORG], [OWNER_MEMBERSHIP]];
    _poolQueryQueue = [
      { rows: wizardStateRows({ completed_steps: ["profile", "team", "notifications"], current_step: "integrations" }) },
      { rows: [] },
    ];
...
describe("POST /onboarding/wizard/:orgSlug/complete — mark onboarding complete", () => {
  ...
  it("returns 200 and marks onboarding complete", async () => {
    _selectQueue = [[ORG], [OWNER_MEMBERSHIP]];
    _poolQueryQueue = [
      { rows: wizardStateRows({ completed_steps: ["profile", "team", "notifications", "integrations"] }) },
      { rows: [] },
    ];
    _insertReturnQueue = [[], []];
  ...
  it("returns 404 for a nonexistent org", async () => {
    ...
    const app = await getApp();
    const res = await request(app)
      .post("/onboarding/wizard/ghost-org/complete")
      .send({});
...
describe("POST /onboarding/resend-invite/:orgSlug — send/resend invite", () => {
  ...
  it("creates and returns a pending invitation", async () => {
    _selectQueue = [[ORG], [OWNER_MEMBERSHIP], [{ displayName: "Alice", email: "alice@test.example" }]];
    _updateReturnQueue = [[]];
    _insertReturnQueue = [[{
      id: 77,
