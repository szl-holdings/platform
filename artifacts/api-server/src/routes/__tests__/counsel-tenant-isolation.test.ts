  ...
  db: {
    select() {
      ...
      const chain: Record<string, unknown> = {
        from: () => chain,
        where: (w: unknown) => { _lastSelectWhere = w; return chain; },
    ...
    delete() {
      const result = (_selectQueue.shift() ?? []) as unknown[];
      const chain: Record<string, unknown> = {
        where: () => { _deleteWhereCalls++; return chain; },
        returning: () => Promise.resolve(result),
      };
      return chain;
    },
...
describe("PRISM Counsel tenant isolation — real router with DB-level org scoping", () => {
  ...
  describe("GET /counsel/matters/:id — cross-tenant matter access blocked at DB layer", () => {
    ...
    it("returns 200 when Org A asks for one of their own matters", async () => {
      const orgAMatter = {
        id: "M-2024-001", orgId: "1", name: "Apex M&A", clientName: "Apex",
        matterNumber: "2024-MA-001", type: "transaction", status: "active",
        privilegeLevel: "restricted", pressureScore: 80, complexityScore: 70,
        openedDate: "2024-01-01", trialDate: null, closingDate: null,
        nextDeadline: "2024-02-01", nextDeadlineLabel: "X", leadCounsel: "M",
        jurisdiction: "DE", estimatedExposure: null, summary: "", tags: [],
        parties: [], wall: {},
      };
  ...
  describe("POST /counsel/matters — orgId stamped from session", () => {
    it("stamps orgId='1' for Org A users when creating a matter", async () => {
      ...
      const res = await request(app)
        ...
        .send({
          name: "New Matter", clientName: "Client", matterNumber: "2024-NEW-001",
          type: "litigation", status: "active", privilegeLevel: "confidential",
          pressureScore: 50, complexityScore: 50, openedDate: "2024-01-01",
          nextDeadline: "2024-02-01", nextDeadlineLabel: "Filing",
          leadCounsel: "X", jurisdiction: "NY", summary: "...", tags: [],
    ...
    it("stamps orgId='2' for Org B users when creating a matter", async () => {
      ...
      await request(app)
        .post("/counsel/matters")
        .send({
          name: "Org B Matter", clientName: "Client B", matterNumber: "2024-NEW-002",
          type: "litigation", status: "active", privilegeLevel: "confidential",
          pressureScore: 50, complexityScore: 50, openedDate: "2024-01-01",
          nextDeadline: "2024-02-01", nextDeadlineLabel: "Filing",
          leadCounsel: "X", jurisdiction: "NY", summary: "...", tags: [],
  ...
  describe("PATCH /counsel/matters/:id — cross-tenant updates blocked", () => {
    it("returns 404 when Org A tries to PATCH an Org B matter (org-scoped UPDATE returns empty)", async () => {
      ...
      const app = await buildCounselApp(makeOrgAUser);
      const res = await request(app).patch("/counsel/matters/M-OTHER-ORG").send({ status: "closed" });
      expect(res.status).toBe(404);
    });
    ...
    it("returns 404 when Org A appends to an audit trail for an Org B matter", async () => {
      ...
      const res = await request(app)
        ...
        .send({
          ...
          user: "alice", role: "Partner", action: "viewed",
          detail: "trying to inject", ip: "10.0.0.1",
        });
      expect(res.status).toBe(404);
  ...
  describe("POST /counsel/proof-chain — cross-tenant proof append blocked", () => {
    it("returns 404 when Org A appends a proof entry for an Org B matter", async () => {
      ...
      const app = await buildCounselApp(makeOrgAUser);
      const res = await request(app)
        .post("/counsel/proof-chain")
        .send({
          matterId: "M-OTHER-ORG", eventType: "filing", title: "x", summary: "y",
          privilegeLevel: "confidential", author: "alice", parties: [],
        });
      expect(res.status).toBe(404);
      expect(_insertValues.length).toBe(0);
    });
