...
const listApprovalsByResourceMock = vi.fn(async (_resourceType: string, _resourceId: string) => [] as Array<{
  orgId: number | null;
  status: string;
}>);
...
function makeTrace(opts: {
  ...
  return {
    ...
    guardrailResults: opts.guardrailOutcome
      ? [{ guardId: `g-${opts.traceId}`, tier: "action", outcome: opts.guardrailOutcome, reason: "test" }]
      : [],
    verifierDecisions: [],
    errors: [],
    events: [],
...
describe("cross-platform correlations: inbox + approvals emission", () => {
  ...
  it(
    "high-strength entity-overlap correlation reaches the Inbox; escalated breach reaches Approvals; second poll does not duplicate",
    async () => {
      ...
      const highTraces: TraceRecord[] = [
        ...
        makeTrace({ traceId: "tr-high-vessels", domain: "vessels", startedAt: "2026-04-01T08:01:00Z" }),
      ...
      const approvalCalls = createApprovalRequestMock.mock.calls.map((c) => c[0] as Record<string, unknown>);
      const breachApprovals = approvalCalls.filter(
        (p) => p["resourceType"] === "cross-platform-correlation" && p["resourceId"] === corrBreach.correlationId,
      );
      const highApprovals = approvalCalls.filter(
        (p) => p["resourceType"] === "cross-platform-correlation" && p["resourceId"] === corrHigh.correlationId,
      );
      ...
      listApprovalsByResourceMock.mockImplementation(async (resourceType, resourceId) => {
        if (resourceType === "cross-platform-correlation" && resourceId === corrBreach.correlationId) {
