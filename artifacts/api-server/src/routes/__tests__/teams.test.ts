...
vi.mock('@szl-holdings/db', () => {
  ...
  function rowsFor(table: unknown): Record<string, unknown>[] {
    ...
    if (n === 'notification_preferences')
      return store.prefs as unknown as Record<string, unknown>[];
...
let authUser: { id: number; email: string; displayName: string; isReadOnly?: boolean } | null =
  null;
