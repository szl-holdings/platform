...
describe('GET /api/admin/observability — response shape', () => {
  it.each([
    ...
    '24h',
    '7d',
  ] as const)('returns the expected envelope for window=%s', async (window) => {
    const res = await request(buildApp()).get(`/api/admin/observability?window=${window}`);
  ...
  it('rejects an unsupported window value with 400', async () => {
    const res = await request(buildApp()).get('/api/admin/observability?window=30d');
    expect(res.status).toBe(400);
    expect(res.body.error).toBeTruthy();
  });

  it('rejects a non-positive orgId with 400', async () => {
    const res = await request(buildApp()).get('/api/admin/observability?orgId=-1');
    expect(res.status).toBe(400);
  });
...
describe('GET /api/admin/observability — aggregation logic', () => {
  ...
  it('aggregates agent-tool stats and computes a success rate', async () => {
    ...
    expect(body.agentTools.topErrors).toEqual([{ tool: 'search', errorCount: 2 }]);
  });
