...
describe("plans step approval routes", () => {
  ...
  it("rejects approval on a step that is not gated", async () => {
    ...
    await request(app)
      .post("/api/plans/plan-test-1/steps/s2/approve")
      .send({})
      .expect(400);
  });
  ...
  it("returns 404 for unknown plans and steps", async () => {
    ...
    await request(app)
      .post("/api/plans/missing/steps/s1/approve")
      .send({})
      .expect(404);

    await request(app)
      .post("/api/plans/plan-test-1/steps/missing/approve")
      .send({})
      .expect(404);
  });

  it("lists fallbacks for a plan", async () => {
    const fb = makePlan({ planId: "plan-test-fb", rank: 1, title: "Fallback A", fallbackOf: "plan-test-1" });
    await defaultPlanStore.put(fb);
    await defaultPlanStore.put(makePlan({ fallbacks: ["plan-test-fb"] }));
    const app = buildApp();

    const res = await request(app)
      .get("/api/plans/plan-test-1/fallbacks")
      .expect(200);
