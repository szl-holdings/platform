...
feedbackRouter.post(
  '/feedback/nps',
  validateBody(feedbackNpsSchema),
  async (req: Request, res: Response) => {
    try {
      const { score, comment, appName, pageUrl, userRole } = req.body as z.infer<
        typeof feedbackNpsSchema
      >;
...
feedbackRouter.post(
  '/feedback/contextual',
  validateBody(feedbackContextualSchema),
  async (req: Request, res: Response) => {
    try {
      const { sentiment, comment, appName, pageUrl, userRole } = req.body as z.infer<
        typeof feedbackContextualSchema
      >;
    ...
    const npsOverTime = await db
      ...
      .where(
        and(
          ...
          gte(feedbackTable.createdAt, new Date(Date.now() - 365 * 24 * 60 * 60 * 1000)),
        ),
      )
  ...
  try {
    ...
    const [entries, [countResult]] = await Promise.all([
      db
        ...
        .limit(limit)
        .offset(offset),
      db.select({ count: sql<number>`COUNT(*)::int` }).from(feedbackTable).where(whereClause),
    ]);
