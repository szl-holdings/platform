...
outcomeGraphRouter.post(
  '/outcome-graph/recommendations/:id/decision',
  async (req: Request, res: Response) => {
    try {
      ...
      if (err?.code === 'NOT_FOUND')
        return void res.status(404).json({ error: 'Outcome record not found' });
...
outcomeGraphRouter.post(
  '/outcome-graph/recommendations/:id/outcome',
  async (req: Request, res: Response) => {
    try {
      const id = parseInt((req.params.id as string) ?? '0', 10);
      if (!id) return void res.status(400).json({ error: 'Invalid id' });
...
outcomeGraphRouter.get(
  '/outcome-graph/recommendations/:id',
  async (req: Request, res: Response) => {
    try {
      const id = parseInt((req.params.id as string) ?? '0', 10);
      if (!id) return void res.status(400).json({ error: 'Invalid id' });
...
outcomeGraphRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (err?.code === 'NOT_FOUND')
        return void res.status(404).json({ error: 'Learning job not found' });
