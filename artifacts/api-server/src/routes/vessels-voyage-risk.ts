...
export interface VoyageRiskScore {
  ...
  sanctionsRefresh: {
    sources: { name: string; region: string; entities: number; lastRefreshedAt: string; dataLabel: string }[];
  };
  provenance: {
    dataLabel: "live" | "sampled" | "demo";
    confidence: number;
...
const SANCTIONS_WATCHLIST = [
  { name: "Sovcomflot", aliases: ["sovcomflot", "scf"], lists: ["US SDN", "EU Consolidated", "UK OFSI"], jurisdiction: "Russia" },
  { name: "IRISL", aliases: ["irisl", "islamic republic of iran shipping"], lists: ["US SDN", "EU Consolidated"], jurisdiction: "Iran" },
  { name: "National Iranian Tanker", aliases: ["nitc", "national iranian tanker"], lists: ["US SDN"], jurisdiction: "Iran" },
  { name: "Black Sea Tanker Holdings", aliases: ["black sea tanker", "bsth"], lists: ["US SDN"], jurisdiction: "Cyprus" },
  { name: "Palmali", aliases: ["palmali"], lists: ["US SDN"], jurisdiction: "Russia" },
];

const SHADOW_FLEET_PORTS = ["Novorossiysk", "Primorsk", "Ust-Luga", "Bandar Abbas", "Kharg Island", "Sokhna"];
const HIGH_RISK_FLAGS = ["Cameroon", "Palau", "Togo", "Gabon", "Comoros", "Belize", "Tanzania"];
const CHOKEPOINT_STS_ZONES = ["Bab-el-Mandeb", "Strait of Hormuz", "Turkish Straits", "Ceuta Strait"];
...
function scoreSanctions(params: VoyageRiskRequest, rng: () => number): RiskDimension {
  ...
  const ownerMatch = SANCTIONS_WATCHLIST.find(e => e.aliases.some(a => chartererLower.includes(a)));
  if (ownerMatch) {
    base += 40;
    evidence.push({
  ...
  return { score, level: scoreLevel(score), summary: buildSanctionsSummary(score, params, isShadowOrigin), evidence };
}

function buildSanctionsSummary(score: number, params: VoyageRiskRequest, isShadowOrigin: boolean): string {
  if (score >= 80) return `CRITICAL: Route originates from ${params.origin}, a sanctioned cargo export terminal. High probability of embargo-restricted cargo. Beneficial control likely linked to sanctioned state entity.`;
  if (score >= 60) return `Elevated sanctions exposure detected. Charterer entity or ownership chain has partial match to monitored watchlists. Enhanced due diligence required before fixture.`;
  if (score >= 35) return `Moderate sanctions risk. Route passes through elevated-risk jurisdiction. Charterer identity requires verification before fixture.`;
...
function scoreDarkActivity(params: VoyageRiskRequest, rng: () => number): RiskDimension {
  const isShadowOrigin = SHADOW_FLEET_PORTS.some(p => params.origin.toLowerCase().includes(p.toLowerCase()));
  const isHighRiskFlag = HIGH_RISK_FLAGS.some(f => (params.vesselName ?? "").toLowerCase().includes(f.toLowerCase()));
...
function scoreSTS(params: VoyageRiskRequest, rng: () => number): RiskDimension {
  ...
  const passesSTSZone = CHOKEPOINT_STS_ZONES.some(z => {
    ...
    return z.toLowerCase().split(" ").some(w => r.includes(w));
  });
  ...
  if (passesSTSZone) {
    base += 18;
    const zoneName = CHOKEPOINT_STS_ZONES.find(z => params.routeVariant.toLowerCase().includes(z.toLowerCase().split(" ")[0])) ?? "route chokepoint";
    evidence.push({
...
function buildEconomics(params: VoyageRiskRequest, distanceNm: number, rng: () => number): VoyageEconomics {
  const isTanker = (params.cargoType ?? "crude").toLowerCase().includes("crude") ||
  ...
  const canalFeesUsd = hasSuez ? 380_000 + Math.floor(rng() * 60_000) : hasPanama ? 250_000 + Math.floor(rng() * 40_000) : 0;

  const revenueUsd = (isTanker ? 42_000 : 18_000) * transitDays + Math.floor(rng() * 500_000);
  const totalCostsUsd = fuelCostUsd + portDisbursementsUsd + canalFeesUsd + Math.floor(rng() * 200_000);
  ...
  return {
    fuelMt, bunkerPriceUsd: bunkerPrice, fuelCostUsd,
    transitDays: Math.ceil(transitDays),
    distanceNm, revenueUsd, tce,
    profitUsd, portDisbursementsUsd, canalFeesUsd,
    totalCostsUsd, marginPct: Math.round(marginPct * 10) / 10,
    dataLabel: "demo",
  };
}
...
function buildCounterparty(params: VoyageRiskRequest, sanctionsScore: number): CounterpartyProfile {
  const isShadowOrigin = SHADOW_FLEET_PORTS.some(p => params.origin.toLowerCase().includes(p.toLowerCase()));
  const sanctionRisk = sanctionsScore >= 80 ? "critical" : sanctionsScore >= 60 ? "elevated" : sanctionsScore >= 30 ? "watch" : "none";

  const chain: OwnerNode[] = [];
  if (isShadowOrigin) {
    chain.push({ name: params.chartererName ?? "Unknown Charterer", jurisdiction: "UAE", entityType: "company", sanctioned: false, opacity: "opaque" });
    chain.push({ name: "Black Sea Tanker Holdings", jurisdiction: "Cyprus", entityType: "company", sanctioned: true, opacity: "opaque", notes: "SDN-adjacent via Sovcomflot" });
    chain.push({ name: "Sovcomflot JSC", jurisdiction: "Russia", entityType: "state", sanctioned: true, opacity: "partial", notes: "US/EU/UK sanctioned state entity" });
  } else if (params.chartererName?.toLowerCase().includes("shell")) {
    chain.push({ name: params.chartererName ?? "Shell Entity", jurisdiction: "Netherlands", entityType: "company", sanctioned: false, opacity: "transparent" });
    chain.push({ name: "Shell plc", jurisdiction: "United Kingdom", entityType: "company", sanctioned: false, opacity: "transparent", notes: "LSE listed — full ownership transparency" });
  } else {
    chain.push({ name: params.chartererName ?? "Charterer Entity", jurisdiction: "UAE", entityType: "company", sanctioned: false, opacity: "partial" });
    chain.push({ name: "Holding Company (UBO unknown)", jurisdiction: "Cayman Islands", entityType: "company", sanctioned: false, opacity: "opaque", notes: "Beneficial owner not publicly disclosed" });
  }

  return {
    charterer: params.chartererName ?? "Unknown",
    chartererCountry: isShadowOrigin ? "UAE" : params.chartererName?.toLowerCase().includes("shell") ? "Netherlands" : "Unknown",
    sanctionRisk: sanctionRisk as "none" | "watch" | "elevated" | "critical",
    creditRating: isShadowOrigin ? "Unrated" : params.chartererName?.toLowerCase().includes("shell") ? "A+" : "Unrated",
    beneficialControlChain: chain,
    keyRisk: sanctionsScore >= 80
...
function routeDistance(origin: string, destination: string, variant: string): number {
  ...
  const fromRasTanura = origin.toLowerCase().includes("tanura") || origin.toLowerCase().includes("ras");
...
function chokepoints(origin: string, destination: string, variant: string): string[] {
  ...
  if (origin.toLowerCase().includes("houston") || destination.toLowerCase().includes("houston")) result.push("Gulf of Mexico");
  ...
  async (req, res) => {
    try {
      ...
      const recommendation =
        composite >= 80 ? "HOLD — Compliance block recommended before fixture" :
        composite >= 60 ? "CAUTION — Escalate to compliance team before committing" :
        composite >= 35 ? "MONITOR — Proceed with enhanced due diligence" :
    ...
    } catch (err) { handleRouteError(res, err, "Failed to score voyage risk"); }
  }
);
...
const PRESET_SCENARIOS = [
  {
    ...
    params: {
      vesselName: "Pacific Guardian", vesselImo: "9821045",
      origin: "Ras Tanura, SA", destination: "Rotterdam, NL",
      routeVariant: "Suez Canal", cargoType: "Crude Oil",
  ...
  },
  {
    ...
    params: {
      vesselName: "Nordic Carrier", vesselImo: "9445566",
      origin: "Houston, USA", destination: "Rotterdam, NL",
      routeVariant: "Cape of Good Hope", cargoType: "Refined Products",
  ...
  },
  {
    ...
    params: {
      vesselName: "Orient Meridian", vesselImo: "9654789",
      origin: "Novorossiysk, RU", destination: "Fujairah, UAE",
      routeVariant: "Turkish Straits Suez", cargoType: "Russian Crude (URALS)",
...
router.post(
  ...
  (req, res) => {
    ...
    } catch (err) { handleRouteError(res, err, "Failed to run sanctions refresh"); }
  }
);
...
const pdfStyles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 9, fontWeight: "bold", textTransform: "uppercase", letterSpacing: 1, color: "#0ea5e9", marginTop: 12, marginBottom: 4, borderBottomWidth: 1, borderBottomColor: "#e5e7eb", paddingBottom: 2 },
  row: { flexDirection: "row", justifyContent: "space-between", marginBottom: 2 },
  label: { color: "#6b7280", width: "40%" },
  value: { color: "#111827", width: "58%", textAlign: "right" },
  evidenceItem: { marginBottom: 4, paddingLeft: 8, paddingTop: 3, paddingBottom: 3, backgroundColor: "#f9fafb", borderLeftWidth: 2, borderLeftColor: "#e5e7eb" },
  ...
  disclaimer: { marginTop: 16, padding: 8, backgroundColor: "#fef3c7", borderWidth: 1, borderColor: "#fbbf24" },
  disclaimerText: { fontSize: 7.5, color: "#92400e" },
  watermark: { position: "absolute", top: 200, left: 50, fontSize: 72, color: "#e5e7eb", opacity: 0.3, transform: "rotate(-30deg)" },
  footer: { position: "absolute", bottom: 30, left: 40, right: 40, borderTopWidth: 1, borderTopColor: "#e5e7eb", paddingTop: 6 },
...
function ComplianceMemoDoc({ score }: { score: VoyageRiskScore }) {
  ...
  const levelColor = (level: string) => {
    const m: Record<string, string> = { critical: "#ef4444", high: "#f87171", moderate: "#fbbf24", low: "#34d399", none: "#34d399" };
  ...
  return React.createElement(Document,
    ...
    React.createElement(Page, { size: "A4", style: pdfStyles.page },
      ...
      React.createElement(View, { style: { marginBottom: 16 } },
        ...
        React.createElement(Text, { style: { fontSize: 10, color: "#6b7280", marginBottom: 2 } }, "Vessels Maritime Intelligence — SZL Holdings"),
        React.createElement(Text, { style: { fontSize: 8, color: "#9ca3af" } }, `Generated: ${now} · Ref: ${score.scenarioId}`),
      ),
      ...
      React.createElement(View, { style: pdfStyles.row },
        ...
        React.createElement(Text, { style: pdfStyles.value }, `${score.vessel.name} (IMO ${score.vessel.imo})`),
      ),
      React.createElement(View, { style: pdfStyles.row },
        React.createElement(Text, { style: pdfStyles.label }, "Type / Flag / DWT"),
        React.createElement(Text, { style: pdfStyles.value }, `${score.vessel.type} · ${score.vessel.flag} · ${score.vessel.dwt.toLocaleString()} DWT`),
      ),
      React.createElement(View, { style: pdfStyles.row },
        React.createElement(Text, { style: pdfStyles.label }, "Route"),
        React.createElement(Text, { style: pdfStyles.value }, `${score.route.origin} → ${score.route.destination}`),
      ),
      React.createElement(View, { style: pdfStyles.row },
      ...
      React.createElement(View, { style: pdfStyles.row },
        ...
        React.createElement(Text, { style: pdfStyles.value }, `${score.route.distanceNm.toLocaleString()} nm`),
      ),
      React.createElement(View, { style: pdfStyles.row },
        React.createElement(Text, { style: pdfStyles.label }, "Chokepoints"),
        React.createElement(Text, { style: pdfStyles.value }, score.route.chokepoints.join(", ") || "None"),
      ),

      React.createElement(Text, { style: pdfStyles.sectionTitle }, "Composite Risk Summary"),
      React.createElement(View, { style: { ...pdfStyles.row, marginBottom: 6 } },
        React.createElement(Text, { style: pdfStyles.label }, "Composite Score"),
        React.createElement(Text, { style: { ...pdfStyles.value, color: levelColor(score.risk.compositeLevel), fontWeight: "bold" } },
      ...
      React.createElement(View, { style: { ...pdfStyles.row, marginBottom: 6 } },
        ...
        React.createElement(Text, { style: { ...pdfStyles.value, color: levelColor(score.risk.compositeLevel) } }, score.risk.recommendation),
      ),

      React.createElement(Text, { style: pdfStyles.sectionTitle }, "Risk Dimensions — Evidence"),
      ...dims.map(({ key, dim }) =>
        React.createElement(View, { key, style: pdfStyles.riskBlock },
          React.createElement(View, { style: { flexDirection: "row", alignItems: "center", marginBottom: 2 } },
            ...
            React.createElement(Text, { style: { fontSize: 8, color: levelColor(dim.level), fontWeight: "bold" } },
              `${dim.score}/100 — ${dim.level.toUpperCase()}`),
          ),
          React.createElement(Text, { style: pdfStyles.riskSummary }, dim.summary),
          ...dim.evidence.map((ev, i) =>
            React.createElement(View, { key: i, style: pdfStyles.evidenceItem },
              React.createElement(Text, { style: pdfStyles.evidenceSignal }, `• ${ev.signal}`),
              React.createElement(Text, { style: pdfStyles.evidenceSource }, `Source: ${ev.source} · Confidence: ${ev.confidence}%`),
            )
          ),
        )
      ),
      ...
      React.createElement(View, { style: pdfStyles.row },
        ...
        React.createElement(Text, { style: pdfStyles.value }, `${score.counterparty.charterer} (${score.counterparty.chartererCountry})`),
      ),
      React.createElement(View, { style: pdfStyles.row },
        React.createElement(Text, { style: pdfStyles.label }, "Sanction Risk"),
        React.createElement(Text, { style: { ...pdfStyles.value, color: levelColor(score.counterparty.sanctionRisk), fontWeight: "bold" } },
      ...
      React.createElement(Text, { style: { color: "#374151", marginBottom: 6, marginTop: 2 } }, score.counterparty.keyRisk),
      React.createElement(Text, { style: { color: "#6b7280", fontSize: 8, marginBottom: 4, fontWeight: "bold" } }, "Beneficial Control Chain:"),
      ...score.counterparty.beneficialControlChain.map((node, i) =>
        React.createElement(View, { key: i, style: pdfStyles.ownerNode },
          React.createElement(View, { style: { ...pdfStyles.ownerDot, backgroundColor: node.sanctioned ? "#ef4444" : "#34d399" } }),
          React.createElement(View, { style: pdfStyles.ownerContent },
            React.createElement(Text, { style: pdfStyles.ownerName }, `${node.name}${node.sanctioned ? " ⚠ SANCTIONED" : ""}`),
            React.createElement(Text, { style: pdfStyles.ownerMeta },
      ...
      ...[
        ...
        ["Bunker / Fuel", `${score.economics.fuelMt.toLocaleString()} MT @ $${score.economics.bunkerPriceUsd}/MT = $${(score.economics.fuelCostUsd / 1000).toFixed(0)}K`],
        ...
        React.createElement(View, { key: label, style: pdfStyles.row },
          React.createElement(Text, { style: pdfStyles.label }, label),
          React.createElement(Text, { style: pdfStyles.value }, value),
        )
      ),

      React.createElement(View, { style: pdfStyles.disclaimer },
        React.createElement(Text, { style: pdfStyles.disclaimerText },
      ...
      React.createElement(View, { style: pdfStyles.footer },
        React.createElement(Text, { style: pdfStyles.footerText },
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.setHeader("Content-Disposition", `attachment; filename="compliance-memo-${score.scenarioId.toLowerCase()}.pdf"`);
