...
router.get(
  '/atlas/spatial/snapshots/:twinId',
  authMiddleware(),
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { getSnapshotHistory } = await import(
        '@szl-holdings/ai-engine/digital-twins/twin-engine-spatial'
      );
      const user = req.user;
...
router.post(
  '/atlas/spatial/snapshots/compare',
  authMiddleware(),
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!twinId || !entityId || !twinCategory || !currentState || currentConfidence == null) {
        sendBadRequest(
          res,
          'twinId, entityId, twinCategory, currentState, and currentConfidence are required',
        );
        return;
      }
...
router.get(
  '/atlas/spatial/twins/sync-status',
  authMiddleware(),
  validateQuery(listQuerySchema),
  async (req: Request, res: Response) => {
    try {
      ...
      const statuses = await Promise.all(
        twinIds.map(async (twinId) => {
          try {
            ...
            const driftScore =
              rawDrift != null ? Math.round(Math.max(0, Math.min(1, rawDrift)) * 100) : null;
...
router.get(
  '/atlas/spatial/drift/:twinId/latest',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
      const { twinId } = req.params as { twinId: string };
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        sendBadRequest(
          res,
          'twinId, entityId, twinCategory, baselineSnapshotId, branchName, and parameters are required',
        );
        return;
      }
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { listBranches, getLatestDriftAssessment } = await import(
        '@szl-holdings/atlas-spatial-runtime'
      );
      const user = req.user;
      const orgId = user?.orgs?.[0]?.orgId ?? undefined;
      ...
      const branches = await listBranches({
        ...
        status: status as
          | import('@szl-holdings/atlas-spatial-runtime').ScenarioBranch['status']
          | undefined,
        orgId,
...
router.get(
  '/atlas/spatial/branches/compare',
  authMiddleware(),
  validateQuery(listQuerySchema),
  async (req: Request, res: Response) => {
    try {
...
router.get(
  '/atlas/spatial/branches/:branchId',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
...
router.get(
  '/atlas/spatial/replay/:twinId/timeline',
  authMiddleware(),
  validateQuery(listQuerySchema),
  async (req: Request, res: Response) => {
    try {
...
router.get(
  '/atlas/spatial/replay/:twinId/frame/:frameIndex',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
...
router.get(
  '/atlas/spatial/proof-bundle/:contentType/:contentId',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
...
router.post(
  '/atlas/spatial/proof-bundle/tag',
  authMiddleware(),
  validateBody(
    bodyShape({
...
router.get(
  '/atlas/spatial/worldline/overlays',
  authMiddleware(),
  validateQuery(listQuerySchema),
  async (req: Request, res: Response) => {
    try {
      const { signalType, sourceTrustClass, severity, isActive, entityId, twinCategory, limit } =
        req.query as {
          signalType?: string;
      ...
      const overlays = await querySignalOverlays({
        ...
        sourceTrustClass: sourceTrustClass as
          | import('@szl-holdings/db').SourceTrustClass
          | undefined,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const overlay = await createSignalOverlay({
        ...
        boundingRegion,
        affectedEntityIds,
        affectedTwinCategories: affectedTwinCategories as
          | import('@szl-holdings/db').SpatialTwinCategory[]
          | undefined,
        payload,
        confidenceScore,
        causalLinkage,
...
router.get(
  '/atlas/spatial/worldline/overlays/:overlayId',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
...
router.patch(
  '/atlas/spatial/worldline/overlays/:overlayId/expire',
  authMiddleware(),
  requireRole('super_admin', 'admin', 'ops'),
  validateBody(bodyShape({})),
  async (req: Request, res: Response) => {
...
router.get('/atlas/spatial/model-lanes', authMiddleware(), async (_req: Request, res: Response) => {
  try {
    const { allLaneMetadata, LANE_TO_ROUTE_CLASS } = await import(
      '@szl-holdings/atlas-spatial-runtime'
    );
...
router.post(
  '/atlas/spatial/model-lanes/invoke',
  authMiddleware(),
  validateBody(
    bodyShape({
      correlationId: z.unknown().optional(),
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const routeClass = mapLaneToRouteClass(
        laneType as import('@szl-holdings/atlas-spatial-runtime').ModelLaneType,
      );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { entityId, state } = req.body as {
        entityId?: string;
      ...
      const twin = matterTwin.createTwin(
        entityId,
        state as unknown as Parameters<typeof matterTwin.createTwin>[1],
      );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const twin = portfolioTwin.createTwin(
        entityId,
        state as unknown as Parameters<typeof portfolioTwin.createTwin>[1],
      );
      sendCreated(res, twin);
    } catch (err) {
      handleRouteError(res, err, 'Internal server error');
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const twin = incidentTwin.createTwin(
        entityId,
        state as unknown as Parameters<typeof incidentTwin.createTwin>[1],
      );
      sendCreated(res, twin);
    } catch (err) {
      handleRouteError(res, err, 'Internal server error');
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const twin = portTwin.createTwin(
        entityId,
        state as unknown as Parameters<typeof portTwin.createTwin>[1],
      );
      sendCreated(res, twin);
    } catch (err) {
      handleRouteError(res, err, 'Internal server error');
...
router.get(
  '/atlas/spatial/cross-domain/summary',
  authMiddleware(),
  async (_req: Request, res: Response) => {
    try {
      ...
      const totals = {
        ...
        avgDriftScore: parseFloat(
          (summary.reduce((s, d) => s + d.avgDriftScore, 0) / summary.length).toFixed(3),
        ),
      };
