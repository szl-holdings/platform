...
const HAZARD_CLASS: Record<string, { factor: number; description: string }> = {
  'Non-hazardous': { factor: 1.0, description: 'Standard general cargo or dry bulk' },
  'Class 1 - Explosives': { factor: 2.5, description: 'Extremely hazardous; war risk required' },
  'Class 2 - Gases': { factor: 1.8, description: 'Flammable/compressed gases (LNG, LPG)' },
  'Class 3 - Flammable Liquids': { factor: 1.6, description: 'Crude oil, petroleum products' },
  'Class 4 - Flammable Solids': { factor: 1.3, description: 'Coal, sulphur, iron ore' },
  'Class 5 - Oxidizers': { factor: 1.4, description: 'Ammonium nitrate, fertilizers' },
  'Class 6 - Toxic': { factor: 1.7, description: 'Chemicals, pesticides' },
  'Class 7 - Radioactive': { factor: 3.0, description: 'Restricted; specialist underwriting only' },
  'Class 8 - Corrosive': { factor: 1.45, description: 'Acids, caustic soda' },
  'Perishable Goods': { factor: 1.2, description: 'Temperature-sensitive commodities' },
};
...
const ROUTE_CHOKEPOINTS: Record<string, { riskFactor: number; description: string }> = {
  ...
  'Suez Canal': { riskFactor: 1.2, description: 'Traffic congestion risk; political instability' },
  'Panama Canal': { riskFactor: 1.1, description: 'Drought-related draft restrictions' },
  'Danish Straits': { riskFactor: 1.05, description: 'Winter ice conditions possible' },
  'Cape of Good Hope': {
    riskFactor: 1.08,
    description: 'Heavy weather; avoided as Suez alternative',
  },
  'South China Sea': { riskFactor: 1.35, description: 'Territorial disputes; piracy risk' },
  'Gulf of Guinea': { riskFactor: 1.5, description: 'High piracy activity area' },
  'Arctic Route': { riskFactor: 1.4, description: 'Ice risk; limited SAR coverage' },
};

const COVERAGE_RATES: Record<string, number> = {
  marine_cargo: 0.0012,
  hull_machinery: 0.0045,
  protection_indemnity: 0.008,
  freight_demurrage: 0.0008,
  war_risk: 0.0025,
  pollution_liability: 0.006,
};

const BASE_DEDUCTIBLES: Record<string, number> = {
  marine_cargo: 5000,
  hull_machinery: 75000,
  protection_indemnity: 25000,
  freight_demurrage: 10000,
  war_risk: 15000,
  pollution_liability: 50000,
};
...
function computeRiskScore(params: {
  ...
  const hazardFactor = HAZARD_CLASS[cargoHazardClass]?.factor ?? 1.0;
  const routeFactors = routeChokepoints.map((cp) => ROUTE_CHOKEPOINTS[cp]?.riskFactor ?? 1.0);
  const routeFactor = routeFactors.length > 0 ? Math.max(...routeFactors) : 1.0;
  const tonnageFactor =
    vesselGrossTonnage > 200000
      ? 1.12
      : vesselGrossTonnage > 100000
        ? 1.06
        : vesselGrossTonnage > 50000
          ? 1.02
          : 1.0;
  const coverageFactor =
    coverageType === 'protection_indemnity' ? 1.15 : coverageType === 'war_risk' ? 1.3 : 1.0;

  const composite =
    ageFactor * flagFactor * hazardFactor * routeFactor * tonnageFactor * coverageFactor;
  ...
  return {
    ...
    factors: {
      ageFactor: {
        value: ageFactor,
        description: `Vessel age ${vesselAge} years`,
        impact: ageFactor > 1.1 ? 'High' : ageFactor > 1.0 ? 'Medium' : 'Low',
      },
      flagStateFactor: {
        value: flagFactor,
        flag: flagState,
        description: `Flag state risk for ${flagState}`,
        impact: flagFactor > 1.1 ? 'High' : flagFactor > 1.0 ? 'Medium' : 'Low',
      },
      cargoHazardFactor: {
        value: hazardFactor,
        cargoClass: cargoHazardClass,
        description: HAZARD_CLASS[cargoHazardClass]?.description ?? 'Standard cargo',
        impact:
          hazardFactor > 1.4
            ? 'Critical'
            : hazardFactor > 1.2
      ...
      routeFactor: {
        ...
        description:
          routeChokepoints.length > 0
            ? `Route through ${routeChokepoints.join(', ')}`
            : 'Standard route',
        impact:
          routeFactor > 1.3
            ? 'Critical'
      ...
      tonnageFactor: {
        ...
        impact: tonnageFactor > 1.1 ? 'High' : tonnageFactor > 1.0 ? 'Medium' : 'Low',
      },
      coverageFactor: {
        value: coverageFactor,
        coverageType,
        impact: coverageFactor > 1.2 ? 'High' : coverageFactor > 1.0 ? 'Medium' : 'Low',
      },
    },
  };
}
...
function computePremium(params: {
  ...
  const broker = Math.round(periodPremium * 0.1 * 100) / 100;
  return {
    annualRate,
    periodRate,
    annualPremium,
...
const demoQuotes: any[] = [
  {
    id: 1,
    quoteRef: 'QT-MAR-0001',
    vesselMmsi: '636092587',
    vesselImo: '9654321',
    vesselName: 'PACIFIC GUARDIAN',
    vesselType: 'Tanker',
    vesselAge: 8,
    vesselGrossTonnage: 158000,
    vesselFlag: 'LR',
    cargoType: 'Crude Oil',
    ...
    routeChokepoints: ['Strait of Hormuz', 'Suez Canal'],
    coverageType: 'marine_cargo',
    coverageLimitUsd: 45000000,
    deductibleUsd: 500000,
    coveragePeriodDays: 45,
    riskRating: 'high',
    riskScore: 68.4,
    premiumUsd: 89250,
    annualPremiumUsd: 724500,
    status: 'quote',
    createdAt: new Date(Date.now() - 86400000 * 3).toISOString(),
  },
  {
    id: 2,
    quoteRef: 'QT-MAR-0002',
    vesselMmsi: '352456789',
    vesselImo: '9456789',
    vesselName: 'LIBERTY WAVE',
    vesselType: 'Container',
    vesselAge: 4,
    vesselGrossTonnage: 210000,
    vesselFlag: 'PA',
    cargoType: 'General Cargo',
    cargoValueUsd: 18000000,
    cargoHazardClass: 'Non-hazardous',
    voyageOrigin: 'Shanghai, China',
    voyageDestination: 'Los Angeles, USA',
    routeChokepoints: ['South China Sea'],
    coverageType: 'hull_machinery',
    coverageLimitUsd: 180000000,
    deductibleUsd: 1500000,
    coveragePeriodDays: 365,
    riskRating: 'moderate',
    riskScore: 41.2,
    premiumUsd: 378000,
    annualPremiumUsd: 378000,
    status: 'bound',
...
const demoPolicies: any[] = [
  {
    id: 1,
    quoteId: 2,
    policyNumber: 'POL-MAR-20260214-001',
    vesselMmsi: '352456789',
    vesselImo: '9456789',
    vesselName: 'LIBERTY WAVE',
    coverageType: 'hull_machinery',
    ...
    premiumUsd: 378000,
    status: 'active',
    effectiveAt: new Date(Date.now() - 86400000 * 5).toISOString(),
    expiresAt: new Date(Date.now() + 86400000 * 360).toISOString(),
    carrier: "Lloyd's of London Syndicate 1234",
    syndicateCode: 'SYN-1234-VMI',
    claimsCount: 0,
    totalClaimsUsd: 0,
    boundAt: new Date(Date.now() - 86400000 * 5).toISOString(),
...
const demoClaims: any[] = [
  {
    id: 1,
    policyId: 1,
    claimRef: 'CLM-MAR-0001',
    vesselMmsi: '352456789',
    vesselName: 'LIBERTY WAVE',
    incidentType: 'Machinery Breakdown',
    incidentDescription: 'Main engine failure at sea — emergency tow required',
    incidentAt: new Date(Date.now() - 86400000 * 2).toISOString(),
    incidentLocation: 'North Pacific, 38.5°N 165.2°E',
    claimedAmountUsd: 2800000,
    approvedAmountUsd: null,
    settledAmountUsd: null,
    deductibleApplied: 1500000,
    status: 'under_review',
    reserveAmountUsd: 2800000,
    adjustorNotes:
      'Initial survey confirms main engine failure. Tow to Yokohama completed. Parts assessment ongoing.',
    filedAt: new Date(Date.now() - 86400000 * 2).toISOString(),
...
router.get(
  ...
  async (_req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to fetch quotes');
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (!vesselName || !coverageType || !coverageLimitUsd) {
        res
          .status(400)
          .json({ error: 'vesselName, coverageType, and coverageLimitUsd are required' });
        return;
      }
...
router.post(
  '/vessels/insurance/quotes/:id/bind',
  insLimit,
  authMiddleware({ required: false }),
  ...
  async (req, res) => {
    try {
      ...
      if (quote.status === 'bound') {
        res.status(400).json({ error: 'Quote already bound' });
        return;
      }
      if (quote.riskRating === 'uninsurable') {
        res.status(400).json({ error: 'This risk is uninsurable; cannot bind' });
        return;
      }
      ...
      const policy: any = {
        ...
        policyTerms: {
          coverageType: quote.coverageType,
          institute: 'Institute Cargo Clauses (A)',
          law: 'English Law and Jurisdiction',
        },
        exclusions: [
          'Nuclear Risk',
          'War & Strikes (separate cover available)',
          'Wilful Misconduct',
        ],
        createdAt: new Date().toISOString(),
      };
      ...
      handleRouteError(res, err, 'Failed to bind policy');
    }
  },
);
...
router.get(
  ...
  async (_req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to fetch claims');
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (!policyId || !incidentType || !claimedAmountUsd) {
        res
          .status(400)
          .json({ error: 'policyId, incidentType, and claimedAmountUsd are required' });
        return;
      }
      ...
      const reserve =
        Math.round(Math.min(claimed, parseFloat(policy.coverageLimitUsd ?? '1000000')) * 100) / 100;
      ...
      handleRouteError(res, err, 'Failed to file claim');
    }
  },
);

router.put(
  '/vessels/insurance/claims/:id/status',
  insLimit,
  authMiddleware({ required: false }),
  validateBody(
    bodyShape({
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to update claim status');
    }
  },
);

router.get(
  '/vessels/insurance/risk-score',
  insLimit,
  authMiddleware({ required: false }),
  ...
  (req, res) => {
    try {
      ...
      const chokepoints = routeChokepoints
      ...
      handleRouteError(res, err, 'Failed to compute risk score');
    }
  },
);

router.get(
  '/vessels/insurance/reference/chokepoints',
  insLimit,
  authMiddleware({ required: false }),
  (_req, res) => {
    try {
      const chokepoints = Object.entries(ROUTE_CHOKEPOINTS).map(([name, data]) => ({
        name,
        ...data,
      }));
      sendSuccess(res, { chokepoints, count: chokepoints.length });
    } catch (err) {
      handleRouteError(res, err, 'Failed to fetch chokepoints');
    }
  },
);

router.get(
  '/vessels/insurance/reference/hazard-classes',
  insLimit,
  authMiddleware({ required: false }),
  (_req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to fetch hazard classes');
    }
  },
);
  ...
  async (_req, res) => {
    try {
      ...
      const totalExposure = activePolicies.reduce(
        (s, p) => s + parseFloat(p.coverageLimitUsd ?? '0'),
        0,
      );
      const openClaims = allClaims.filter(
        (c) => !['settled', 'closed', 'rejected'].includes(c.status),
      );
      const totalReserve = openClaims.reduce(
        (s, c) => s + parseFloat(c.reserveAmountUsd ?? '0'),
        0,
      );
      const settledClaims = allClaims.filter((c) => c.status === 'settled');
      const totalPaid = settledClaims.reduce(
        (s, c) => s + parseFloat(c.settledAmountUsd ?? '0'),
        0,
      );
      ...
      handleRouteError(res, err, 'Failed to fetch portfolio summary');
    }
  },
);
