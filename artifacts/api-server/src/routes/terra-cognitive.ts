import { queryEdges, queryNodes } from '@szl-holdings/constellation';
...
router.get(
  '/terra/cognitive/ownership-graph',
  cogLimit,
  auth,
  ...
  async (req, res) => {
    try {
      ...
      //     lien-source nodes)
      const [propertyNodes, ownerNodes, lenderNodes, dbProperties, dbCanonicalProps] =
        await Promise.all([
      ...
      const allNodes = [...propertyNodes.nodes, ...ownerNodes.nodes, ...lenderNodes.nodes];
      ...
      for (const p of dbProperties) {
        ...
        const confLevel =
          p.confidenceLevel === 'high' ? 0.9 : p.confidenceLevel === 'low' ? 0.55 : 0.75;
      ...
      if (allNodes.length > 0 || dbNodes.length > 0) {
        ...
        const filteredNodes =
          propertyId && nodes.some((n) => n.id === propertyId)
        ...
        riskFlags = nodes
          ...
          .map((n) => ({
            ...
            flag: n.riskFlag,
            severity:
              n.riskFlag === 'offshore' || n.riskFlag === 'high_distress' ? 'medium' : 'high',
          }));
        ...
        const dbLenderDebt = dbProperties.reduce((s, p) => {
          return (
            s +
            (p.debtAmount !== null ? Number(p.debtAmount) : 0) +
            (p.lienAmount !== null ? Number(p.lienAmount) : 0)
          );
        }, 0);
        const totalDebt =
          lenderMetas.reduce((s, ln) => {
            const m = ((ln as any).meta as Record<string, number> | null) ?? {};
            return s + (m.loanAmount ?? 0);
          }, 0) + dbLenderDebt;
        const propNodes = nodes.filter((n) => n.type === 'property');
        const totalValue = propNodes.reduce(
          (s, n) => s + (((n as any).meta as Record<string, number> | null)?.value ?? 0),
          0,
        );
        ...
        summary = {
          ...
          source:
            dbProperties.length > 0 && allNodes.length > 0
        ...
        nodes = [
          {
            id: 'ent_001',
            label: 'Meridian Capital Holdings LLC',
            type: 'entity',
            entityType: 'llc',
          ...
          {
            id: 'ent_002',
            label: 'Apex Real Estate Partners LP',
            type: 'entity',
            entityType: 'lp',
          ...
          {
            id: 'prop_001',
            label: propertyId ?? '245 Park Avenue South',
            type: 'property',
            role: 'asset',
            value: 48500000,
            riskFlag: null,
          ...
          {
            id: 'lender_002',
            label: 'Meridian Mezzanine Partners',
            type: 'lender',
            role: 'mezz_lender',
            loanAmount: 7000000,
            ltv: 0.722,
        ...
        edges = [
          ...
          { id: 'e2', from: 'ent_002', to: 'ent_001', label: 'controls (GP)', weight: 0.3 },
          { id: 'e3', from: 'ent_003', to: 'ent_001', label: 'invested (LP)', weight: 0.7 },
          { id: 'e4', from: 'per_001', to: 'ent_002', label: 'managing_member', weight: 0.6 },
          { id: 'e5', from: 'per_002', to: 'ent_002', label: 'managing_member', weight: 0.4 },
        ...
        riskFlags = [
          {
            entityId: 'ent_003',
            entity: 'Sovereign Yield Fund I',
            flag: 'offshore',
            severity: 'medium',
          },
          {
            entityId: 'lender_002',
            entity: 'Meridian Mezzanine Partners',
            flag: 'high_ltv',
            severity: 'high',
          },
        ];
      ...
      handleRouteError(res, err, 'Failed to build ownership graph');
    }
  },
);
...
router.get('/terra/cognitive/lender-exposure', cogLimit, auth, async (req, res) => {
  try {
    ...
    const [lenderGraphResult, propGraphResult, distressProperties, recentTransactions] =
      await Promise.all([
    ...
    const dataSource = dataSourceParts.length > 0 ? dataSourceParts.join('+') : 'illustrative';
    ...
    if (graphLenders.length > 0 || properties.length > 0 || recentTransactions.length > 0) {
      ...
      if (txPropertyIds.length > 0) {
        const propRows = await db
          ...
          .from(terraPropertiesTable)
          .where(inArray(terraPropertiesTable.id, txPropertyIds));
      ...
      for (const p of properties) {
        ...
        if (p.auctionDate) {
          const daysToAuction = Math.ceil(
            (new Date(p.auctionDate).getTime() - Date.now()) / 86400000,
          );
      ...
      const typeToLenderMeta: Record<string, { lenderType: string; avgRate: number }> = {
        foreclosure: { lenderType: 'senior_mortgage', avgRate: 7.1 },
        'tax-lien': { lenderType: 'tax-lien', avgRate: 5.0 },
        ...
        reo: { lenderType: 'life_co', avgRate: 6.5 },
        auction: { lenderType: 'senior_mortgage', avgRate: 7.5 },
      ...
      for (const gl of graphLenderEntries) {
        ...
        const matchedMeta = typeToLenderMeta[lenderType] ?? {
      ...
      for (const agg of Object.values(bySource)) {
        ...
        const syntheticEstimate = totalEncumbrance === 0 ? Math.round(agg.totalValue * 0.65) : 0;
        const avgLtv =
          agg.totalValue > 0 && totalEncumbrance > 0
            ? Math.min(0.95, totalEncumbrance / agg.totalValue)
            : 0;
        const riskScore = Math.round(
          30 + avgLtv * 50 + (agg.highDistress / Math.max(agg.count, 1)) * 20,
        );
        riskIdx++;
        lenders.push({
      ...
      maturityLadder = [
        {
          period: '0–90d',
          amount: lenders.reduce((s, l) => {
            const m = l.maturities as Record<string, number>;
            return (
              s + m.within90d * ((l.totalExposure as number) / Math.max(l.loanCount as number, 1))
            );
          }, 0),
        },
        {
          ...
          amount: lenders.reduce((s, l) => {
            ...
            return (
              s +
              (m.within180d - m.within90d) *
                ((l.totalExposure as number) / Math.max(l.loanCount as number, 1))
            );
          }, 0),
        },
        {
      ...
      summary = {
        ...
        highestSingleExposure:
          lenders.length > 0 ? Math.max(...lenders.map((l) => l.totalExposure as number)) : 0,
        byType: byTypeSummary,
        nearTermMaturities: lenders.reduce(
          (s, l) => s + ((l as any).maturities?.within90d ?? 0),
          0,
        ),
      ...
      lenders = [
        {
          ...
          type: 'bridge',
          totalExposure: 112400000,
          loanCount: 7,
          avgLtv: 0.64,
          avgRate: 8.25,
          maturities: { within90d: 1, within180d: 2, within365d: 4 },
          covenantBreaches: 0,
        ...
        {
          id: 'ldr_002',
          name: 'Meridian Mezzanine Partners',
          type: 'mezzanine',
          totalExposure: 38700000,
          loanCount: 4,
          avgLtv: 0.81,
          avgRate: 12.5,
          maturities: { within90d: 1, within180d: 1, within365d: 2 },
          covenantBreaches: 1,
          watchlistProperties: 2,
          riskScore: 71,
          riskLabel: 'High',
        },
        {
          id: 'ldr_003',
          name: 'Sovereign Life Co',
          type: 'life_co',
          totalExposure: 88000000,
          loanCount: 3,
          avgLtv: 0.55,
          avgRate: 6.8,
          maturities: { within90d: 0, within180d: 0, within365d: 1 },
          covenantBreaches: 0,
          watchlistProperties: 0,
          riskScore: 28,
          riskLabel: 'Low',
        ...
        {
          id: 'ldr_004',
          name: 'Atlas CMBS 2024-NE1',
          type: 'cmbs',
          totalExposure: 65200000,
          loanCount: 2,
          avgLtv: 0.72,
          avgRate: 7.4,
          maturities: { within90d: 0, within180d: 1, within365d: 1 },
          covenantBreaches: 1,
          watchlistProperties: 1,
          riskScore: 58,
          riskLabel: 'Medium',
      ...
      const totalExposure = lenders.reduce((s, l) => s + (l.totalExposure as number), 0);
      const byTypeFb = lenders.reduce((acc: Record<string, number>, l) => {
        acc[l.type as string] = (acc[l.type as string] ?? 0) + (l.totalExposure as number);
        return acc;
      }, {});
      maturityLadder = [
    ...
    if (recentTransactions.length > 0)
      sourceParts.push('Terra-Transactions(financingType+salePrice)');
    ...
    const realSourceCount =
      (graphLenders.length > 0 ? 1 : 0) + (recentTransactions.length > 0 ? 1 : 0);
    ...
    handleRouteError(res, err, 'Failed to build lender exposure map');
  }
});
  ...
  try {
    ...
    if (usedCovenantTable) {
      ...
      for (const m of measurements) {
        ...
        const remedyDate =
          m.status === 'breach'
            ? new Date(Date.now() + (m.covenant.remedyPeriodDays ?? 60) * 86400000)
                .toISOString()
                .split('T')[0]
            : null;
      ...
      if (properties.length > 0) {
        const highDistress = properties.filter((p) => (p.opportunityScore ?? 0) >= 50).slice(0, 8);
        ...
        for (const prop of highDistress) {
          ...
          const isBreach = score >= 70 || impliedDscr < 1.2 || impliedLtv > 0.8;
          const isWatch = !isBreach && (score >= 50 || impliedLtv > 0.65);

          const covenantType =
            prop.distressType === 'tax-lien' ? 'ltv' : impliedDscr < 1.2 ? 'dscr' : 'occupancy';
          ...
          covenants.push({
            ...
            threshold: covenantType === 'dscr' ? 1.2 : covenantType === 'ltv' ? 0.75 : 0.85,
            current:
              covenantType === 'dscr'
                ? +impliedDscr.toFixed(2)
                : covenantType === 'ltv'
                  ? +impliedLtv.toFixed(3)
                  : +Math.min(0.99, 0.75 + (1 - score / 100) * 0.2).toFixed(2),
            ...
            evidence: [
              {
                source: `Distress Registry — ${prop.connectorSource ?? 'Terra DB'}`,
                value: `${prop.distressType} distress, score ${score}/100, ${prop.daysInDistress ?? 0} days in distress`,
                confidence: 0.88,
              },
              ...(prop.debtAmount
                ? [
                    {
                      ...
                      value: `Debt $${(prop.debtAmount / 1e6).toFixed(1)}M on $${(value / 1e6).toFixed(1)}M value (${(impliedLtv * 100).toFixed(1)}% LTV)`,
                      confidence: 0.82,
                    },
                  ]
                : []),
            ],
            remedyDeadline: isBreach
    ...
    if (covenants.length === 0) {
      ...
      covenants = [
        {
          id: 'cov_001',
          property: '245 Park Avenue South',
          lender: 'Meridian Mezzanine Partners',
          type: 'dscr',
          ...
          evidence: [
            {
              source: 'NOI Report Q1 2026',
              value: '$2.4M NOI vs $2.22M debt service',
              confidence: 0.92,
            },
          ],
          remedyDeadline: '2026-05-15',
          guardianActionId: null,
          pendingApproval: false,
        },
        {
          id: 'cov_002',
          property: '180 Water Street, Manhattan',
          lender: 'Atlas CMBS 2024-NE1',
          type: 'occupancy',
          label: 'Minimum Occupancy Covenant',
          threshold: 0.85,
          current: 0.81,
          status: 'breach',
          severity: 'high',
          ...
          evidence: [
            {
              source: 'Certificate of Occupancy — April 2026',
              value: '81% occupancy vs 85% covenant floor',
              confidence: 0.95,
            },
          ],
          remedyDeadline: '2026-06-01',
          guardianActionId: null,
          pendingApproval: false,
        },
        {
          id: 'cov_003',
          property: 'Liberty Industrial Park',
          lender: 'Pacific Bridge Capital',
          type: 'ltv',
          label: 'Loan-to-Value Maintenance',
          threshold: 0.7,
          current: 0.67,
          status: 'watch',
          severity: 'medium',
          ...
          evidence: [
            {
              source: 'Q4 2025 Appraisal',
              value: 'LTV at 67%, approaching 70% trigger',
              confidence: 0.91,
            },
          ],
          remedyDeadline: null,
        ...
        {
          id: 'cov_004',
          property: 'Sovereign Core Office Portfolio',
          lender: 'Sovereign Life Co',
          type: 'dscr',
          label: 'Debt Service Coverage Ratio',
          threshold: 1.35,
          current: 1.52,
          ...
          evidence: [
            {
              ...
              value: 'DSCR at 1.52x above 1.35x threshold',
              confidence: 0.96,
            },
          ],
          remedyDeadline: null,
    ...
    const summary = {
      ...
      source: usedCovenantTable
        ? 'terra-covenants-table'
        : covenants.length > 0
          ? 'distress-db'
          : 'illustrative',
    };
    ...
    sendSuccess(res, {
      ...
      provenance: provenance(
        usedCovenantTable
          ? 'terra_covenants/Live-Financials/Guardian-Actions'
          : 'Distress-DB/Guardian-Actions/Loan-Agreements',
        usedCovenantTable ? 0.93 : 0.78,
        trace,
      ),
    });
  } catch (err) {
    handleRouteError(res, err, 'Failed to fetch covenant status');
  }
});
...
router.post(
  '/terra/cognitive/covenants/scan',
  cogLimit,
  auth,
...
router.post(
  '/terra/cognitive/covenants/seed',
  cogLimit,
  auth,
  validateBody(bodyShape({})),
  async (req, res) => {
...
router.post(
  '/terra/cognitive/covenants/submit-review',
  cogLimit,
  validateBody(
    bodyShape({
  ...
  async (req, res) => {
    try {
      ...
      const { propertyId, covenantType, score, address, distressType, debtAmount, estimatedValue } =
        req.body ?? {};
...
router.get(
  '/terra/cognitive/distress-forecast',
  cogLimit,
  auth,
  validateQuery(listQuerySchema),
  async (req, res) => {
    try {
      ...
      if (properties.length > 0) {
        ranked = properties.map((p, idx) => {
          ...
          const confidence =
            confidenceLevel === 'high' ? 0.91 : confidenceLevel === 'medium' ? 0.78 : 0.62;

          const signals: Array<Record<string, unknown>> = [];
          if (p.distressType === 'foreclosure' || p.distressType === 'pre-foreclosure') {
            signals.push({
              type: 'foreclosure_risk',
              label: `${p.distressType} filing — ${daysInDistress} days in process`,
          ...
          if (p.distressType === 'tax-lien') {
            signals.push({
              type: 'tax-lien',
              label: `Tax lien delinquency — ${daysInDistress} days outstanding`,
              severity: 'high',
              confidence: 0.88,
            });
          }
          if (impliedLtv > 0.75) {
            signals.push({
              type: 'high_ltv',
              label: `LTV at ${(impliedLtv * 100).toFixed(0)}% — refinancing pressure elevated`,
              severity: impliedLtv > 0.9 ? 'critical' : 'high',
              confidence: 0.82,
            });
          }
          if (p.auctionDate) {
            ...
            if (daysToAuction >= 0 && daysToAuction <= 120) {
              signals.push({
                type: 'auction_proximity',
                label: `Foreclosure auction in ${daysToAuction} days (${p.auctionDate})`,
                severity: daysToAuction <= 30 ? 'critical' : 'high',
                confidence: 0.95,
              });
            }
          }
          if (signals.length === 0) {
            signals.push({
              ...
              label: `${p.distressType ?? 'general'} distress, score ${score}/100`,
          ...
          const evidence: Array<Record<string, unknown>> = [
            {
              source: `Terra Distress Registry — ${p.connectorSource ?? 'Terra DB'}`,
              excerpt: `${p.distressType} distress, ${daysInDistress} days in process, stage: ${p.stage ?? 'N/A'}`,
              freshness: p.lastActivityDate
                ? `${Math.ceil((Date.now() - new Date(p.lastActivityDate).getTime()) / 86400000)}d`
                : 'N/A',
            },
            ...(debt > 0
              ? [
                  {
                    source: 'Loan Register',
                    excerpt: `Debt $${(debt / 1e6).toFixed(1)}M on $${(value / 1e6).toFixed(1)}M value = ${(impliedLtv * 100).toFixed(0)}% LTV`,
                    freshness: 'live',
                  },
                ]
              : []),
            ...(p.timeline
              ?.slice(0, 2)
              .map((t: { date?: string; type?: string; description?: string }) => ({
                source: `Timeline — ${t.type ?? 'event'}`,
                excerpt: t.description ?? t.type ?? 'Timeline event',
                freshness: t.date
                  ? `${Math.ceil((Date.now() - new Date(t.date).getTime()) / 86400000)}d`
                  : 'N/A',
              })) ?? []),
          ];
          ...
          return {
            ...
            suggestedAction:
              score >= 75
                ? 'Escalate to Investment Committee; initiate direct outreach to owner'
                : score >= 55
                  ? 'Monitor weekly; prepare term sheet for distressed acquisition'
                  : 'Watch list — track quarterly for developing distress',
            traceRef: trace,
          };
        });
      } else {
        ...
        ranked = [
          {
            rank: 1,
            propertyId: 'prop_f001',
            address: '245 Park Avenue South, Manhattan',
            distressScore: 88,
            confidence: 0.91,
            horizon: '60-90 days',
            signals: [
              {
                ...
                label: 'DSCR below threshold (1.08 vs 1.20)',
                severity: 'critical',
                confidence: 0.92,
              },
            ],
            evidence: [
              { source: 'NOI Report Q1 2026', excerpt: 'NOI declined 14% YoY', freshness: '7d' },
            ],
            suggestedAction: 'Initiate lender conversation for extension',
            traceRef: trace,
          },
          {
            rank: 2,
            propertyId: 'prop_f002',
            address: '180 Water Street, Manhattan',
            distressScore: 74,
            confidence: 0.84,
            horizon: '90-120 days',
            signals: [
              {
                type: 'occupancy_covenant',
                label: 'Occupancy at 81% vs 85% CMBS floor',
                severity: 'high',
                confidence: 0.95,
              },
            ],
            evidence: [
              {
                source: 'CMBS Monitor April 2026',
                excerpt: 'Transferred to watchlist',
                freshness: '5d',
              },
            ],
            suggestedAction: 'Accelerate lease-up; evaluate conversion feasibility',
            traceRef: trace,
          },
        ];
      }
      ...
      const watchCount = ranked.filter(
        (r) => (r.distressScore as number) >= 50 && (r.distressScore as number) < 70,
      ).length;
      sendSuccess(res, {
        ...
        provenance: provenance(
          'Terra-Distress-DB/ACRIS/Tax-Records',
          properties.length > 0 ? 0.88 : 0.72,
          trace,
        ),
      });
    } catch (err) {
      handleRouteError(res, err, 'Failed to generate distress forecast');
    }
  },
);
...
router.get(
  '/terra/cognitive/underwriting-copilot',
  cogLimit,
  auth,
  validateQuery(listQuerySchema),
  async (req, res) => {
    try {
      ...
      const monthlyDebtService =
        (debtAmount * (monthlyRate * (1 + monthlyRate) ** n)) / ((1 + monthlyRate) ** n - 1);
      ...
      let compsContext = {
        comparableDistressCount: 0,
        boroughMedianValue: 0,
        distressedInBorough: 0,
      };
      try {
        ...
        const medianValue =
          sorted.length > 0 ? (sorted[Math.floor(sorted.length / 2)]?.estimatedValue ?? 0) : 0;
        compsContext = {
      ...
      const steps = [
        {
          ...
          citations: [
            {
              source: 'CoStar Market Report Q1 2026',
              excerpt: `Submarket avg cap rate ${cap.toFixed(2)}%, 12-mo trend +18bps`,
              confidence: 0.88,
            },
            ...(compsContext.distressedInBorough > 0
              ? [
                  {
                    source: 'Terra Distress Registry',
                    excerpt: `${compsContext.distressedInBorough} distressed properties in market, median value $${(compsContext.boroughMedianValue / 1e6).toFixed(1)}M`,
                    confidence: 0.91,
                  },
                ]
              : []),
        ...
        },
        {
          ...
          citations: [
            {
              source: 'Pacific Bridge Capital Term Sheet April 2026',
              excerpt: `Bridge loan at ${assumedRate}% for 24mo I/O`,
              confidence: 0.91,
            },
          ],
        ...
        },
        {
          ...
          citations: [
            {
              source: 'Internal IC Return Criteria 2026',
              excerpt: 'Min cash-on-cash 7% for core-plus; 9% for value-add',
              confidence: 0.95,
            },
          ],
        ...
        {
          step: 4,
          label: 'Covenant Compliance Pre-Check',
          finding: `Pre-close covenant review: DSCR test at ${dscr.toFixed(2)}x vs 1.20x covenant; LTV at ${(ltvRatio * 100).toFixed(0)}% vs 75% covenant; occupancy assumption 87%. ${dscr >= 1.2 && ltvRatio <= 0.75 ? 'No projected breaches at underwritten NOI.' : 'Warning: one or more covenants at risk.'}`,
          citations: [
            {
              source: 'Draft Loan Agreement v3 — April 2026',
              excerpt: 'DSCR covenant 1.20x; LTV covenant 75%; occupancy covenant 82%',
              confidence: 0.87,
            },
          ],
          flag:
            dscr < 1.2 || ltvRatio > 0.75
              ? 'Covenant breach risk at underwritten parameters'
              : null,
          passed: dscr >= 1.2 && ltvRatio <= 0.75,
        },
        {
          step: 5,
          ...
          citations: [
            {
              source: 'Historical NOI Variance — Subject Submarket 2022-2026',
              excerpt: 'Worst-case NOI decline was -15% in 2023 for office',
              confidence: 0.79,
            },
            ...(compsContext.comparableDistressCount > 0
              ? [
                  {
                    source: 'Terra Distress Registry',
                    excerpt: `${compsContext.comparableDistressCount} properties in distress — supply headwind to NOI`,
                    confidence: 0.85,
                  },
                ]
              : []),
          ...
          flag:
            dscr * 0.8 < 1.0
              ? 'High sensitivity to NOI decline — consider higher debt service reserve'
              : null,
          passed: dscr * 0.8 >= 1.0,
        },
      ];
      ...
      sendSuccess(
        res,
        {
          ...
          input: {
            propertyId: propertyId ?? 'prop_subject',
          ...
          recommendation: {
            ...
            flags: steps
              .filter((s) => s.flag)
              .map((s) => ({ step: s.step, label: s.label, flag: s.flag })),
          },
          steps,
          verdict,
          verdictRationale: `${passCount}/${steps.length} underwriting checks passed.`,
      ...
      handleRouteError(res, err, 'Underwriting copilot failed');
    }
  },
);
...
function serializeMatter(
  matter: typeof terraDiligenceMattersTable.$inferSelect,
  evidence: Array<typeof terraDiligenceEvidenceTable.$inferSelect>,
) {
  return {
    id: matter.id,
    title: matter.title,
...
router.post(
  '/terra/cognitive/diligence-room/matters',
  cogLimit,
  authWrite,
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to create diligence matter');
    }
  },
);
...
router.post(
  '/terra/cognitive/diligence-room/matters/:matterId/evidence',
  cogLimit,
  authWrite,
  diligenceUpload.single('file'),
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (typeof body.citations === 'string') {
        try {
          body.citations = JSON.parse(body.citations);
        } catch {
          body.citations = [];
        }
      }
      ...
      if (req.file) {
        ...
        try {
          ...
          const safeName = (req.file.originalname || `upload-${Date.now()}`).replace(
            /[^a-zA-Z0-9._-]/g,
            '_',
          );
          ...
          logger.warn(
            { err: (storageErr as Error).message },
            'Object storage unavailable for diligence upload; storing metadata only',
          );
      ...
      handleRouteError(res, err, 'Failed to attach evidence');
    }
  },
);
...
router.patch(
  '/terra/cognitive/diligence-room/evidence/:evidenceId',
  cogLimit,
  authWrite,
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const updates: Partial<typeof terraDiligenceEvidenceTable.$inferInsert> = {
        updatedAt: new Date(),
      };
...
router.get(
  '/terra/cognitive/diligence-room',
  cogLimit,
  auth,
  validateQuery(listQuerySchema),
  async (req, res) => {
    try {
      ...
      if (dbMatters.length > 0) {
        ...
        const avgConfidence =
          chain.length > 0 ? chain.reduce((s, e) => s + e.confidence, 0) / chain.length : 0;
      ...
      if (matterNodes.length > 0) {
          ...
          evidenceChain: [
            {
              ...
              source: 'CONSTELLATION — Terra Entity Record',
              date: new Date(n.freshness).toISOString().split('T')[0],
              ...
              citations: [
                {
                  ref: `CST-NODE-${n.id}`,
                  page: 1,
                  excerpt: `CONSTELLATION entity confirmed with ${((n.confidence ?? 0.85) * 100).toFixed(0)}% confidence`,
                },
              ],
            },
          ],
        ...
        matters = distressCandidates.slice(0, 5).map((p) => {
          const score = p.opportunityScore ?? 50;
          const timeline = (p.timeline ?? []) as Array<{
            date?: string;
            type?: string;
            description?: string;
          }>;
          const evidenceChain: Array<Record<string, unknown>> = [
              ...
              citations: [
                {
                  ref: `TERRA-${p.id}`,
                  page: 1,
                  excerpt: `${p.distressType} distress confirmed, ${p.daysInDistress ?? 0} days in process`,
                },
              ],
            },
            ...(p.debtAmount
              ? [
                  {
                    id: `ev_${p.id}_financial`,
                    category: 'financial',
                    label: 'Debt & Lien Analysis',
                    ...
                    citations: [
                      {
                        ref: `LOAN-${p.id}`,
                        page: 1,
                        excerpt: `Total encumbrances: $${(((p.debtAmount ?? 0) + (p.lienAmount ?? 0)) / 1e6).toFixed(1)}M`,
                      },
                    ],
                  },
                ]
            ...
            ...timeline.slice(0, 3).map((t, tidx) => ({
              id: `ev_${p.id}_tl${tidx}`,
              category: 'legal',
              label: t.type ?? 'Legal Event',
              ...
              citations: [
                { ref: `TL-${p.id}-${tidx}`, page: 1, excerpt: t.description ?? t.type ?? '' },
              ],
            })),
          ];
            ...
            evidenceChain: [
              {
                id: 'ev_001',
                category: 'title',
                label: 'Title Commitment — Chicago Title',
                source: 'Chicago Title Insurance Company',
                date: '2026-04-02',
                freshness: '15d',
                status: 'verified',
                confidence: 0.96,
                summary: 'Clear title with two open permits (DOB ECB), both waived by counsel.',
                citations: [
                  {
                    ref: 'Title Commit #CT-2026-4421',
                    page: 3,
                    excerpt:
                      'Schedule B exceptions: Items 4 and 7 — waived per counsel letter 2026-04-05',
                  },
                ],
              },
              {
                ...
                confidence: 0.91,
                summary:
                  'No RECs identified. Historical dry cleaner 2 blocks — no vapor intrusion pathway.',
                citations: [
                  {
                    ref: 'Terracon Report #TER-2026-NY-0092',
                    page: 14,
                    excerpt:
                      'No recognized environmental conditions identified at the subject property',
                  },
                ],
              },
              {
                id: 'ev_003',
                category: 'financial',
                label: '3-Year Historical P&L — CPA Certified',
                ...
                confidence: 0.88,
                summary:
                  'NOI trend: $3.1M → $2.8M → $2.4M (2023-25). Declining, driven by 3 vacancies.',
                citations: [
                  {
                    ref: 'E&Y Audit 2025 — Property 245PS',
                    page: 7,
                    excerpt:
                      'Net operating income $2,402,184 for FY2025, decline of 12.5% vs FY2024',
                  },
                ],
              },
              {
                ...
                source: 'Property Management — Meridian Capital',
                date: '2026-04-14',
                freshness: '3d',
                status: 'verified',
                confidence: 0.94,
                summary:
                  '18 tenants, 84% occupied, $41.50/sqft avg NNN. Weighted avg lease term 3.2 years.',
                citations: [
                  {
                    ref: 'Certified Rent Roll 2026-04-14',
                    page: 1,
                    excerpt: 'Physical occupancy 84.3%, economic occupancy 82.1%',
                  },
                ],
              },
              {
                id: 'ev_005',
                category: 'structural',
                label: 'PCA / Property Condition Assessment',
                source: 'EMG Engineering',
                date: '2026-03-30',
                freshness: '17d',
                status: 'verified',
                confidence: 0.89,
                summary:
                  'Immediate repairs $320K. 10-yr capital plan $2.1M. No deferred maintenance concerns.',
                citations: [
                  {
                    ref: 'EMG PCA Report #EMG-NY-2026-4401',
                    page: 22,
                    excerpt: 'Immediate capital needs $319,500; 10-year plan $2,085,000',
                  },
                ],
              },
              {
                id: 'ev_006',
                category: 'legal',
                label: 'Litigation Search — PACER + NYSCEF',
                ...
                confidence: 0.75,
                summary:
                  'One open action (tenant claim, $85K). Counsel: immaterial, likely to settle.',
                citations: [
                  {
                    ref: 'SRZ Litigation Memo 2026-04-12',
                    page: 2,
                    excerpt: 'NYSCEF Index 2024-102847: SRZ recommends $85K reserve',
                  },
                ],
              },
            ],
      ...
      const avgConfidence =
        chain.length > 0
          ? chain.reduce((s, e) => s + (e.confidence as number), 0) / chain.length
          : 0;
      ...
      sendSuccess(res, {
        ...
        provenance: provenance(
          matterNodes.length > 0
            ? 'CONSTELLATION/Terra-DB'
            : distressCandidates.length > 0
              ? 'Terra-Distress-DB/ACRIS'
              : 'Diligence-Vault/PACER/NYSCEF',
