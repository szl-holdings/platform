...
function detectIntent(transcription: string): IntentType {
  ...
  if (
    ...
    return 'research';
  if (lower.includes('meeting') || lower.includes('notes from') || lower.includes('discussed'))
    return 'meeting_note';
  ...
  if (lower.includes('do') || lower.includes('complete') || lower.includes('action'))
    return 'action';
  return 'unknown';
}
      ...
      setImmediate(async () => {
        try {
          ...
          try {
            ...
            logger.error(
              { err: transcribeErr, voiceNoteId: id },
              'continuum-voice: transcription failed — marking as failed, no fallback',
            );
            await pool.query(
            ...
            [
              {
                role: 'system',
                content:
                  "You are a voice note processor. Summarize the transcription in one sentence and identify the primary action needed. Respond in JSON: { summary: string, action: string, priority: 'high'|'medium'|'low' }",
              },
          ...
          if (
            convertTo === 'task' ||
            (convertTo !== 'note' && intent === 'task') ||
            (convertTo !== 'note' && intent === 'research')
          ) {
            ...
            try {
              ...
              logger.warn('continuum-voice: failed to create signal from voice note');
            }
          }
          ...
          logger.info(
            { voiceNoteId: id, intent, convertedTo },
            'continuum-voice: transcription completed',
          );
        } catch (asyncErr) {
          logger.error({ err: asyncErr, voiceNoteId: id }, 'continuum-voice: async processing failed');
          await pool.query(
            `UPDATE alloy_voice_notes SET status='failed', updated_at=NOW() WHERE id=$1`,
            [id],
          );
...
router.post(
  '/continuum/voice/transcribe-text',
  authMiddleware(),
  validateBody(
    bodyShape({
      convertTo: z.unknown().optional(),
  ...
  async (req: Request, res: Response) => {
    try {
      const { text, convertTo, title } = req.body as {
        text: string;
        convertTo?: string;
        title?: string;
      };
      if (!text) {
        sendBadRequest(res, 'text is required for text-based voice note');
        return;
      }
        ...
        [
          {
            role: 'system',
            content:
              "Process this voice note text. Respond in JSON: { summary: string, suggestedTitle: string, priority: 'high'|'medium'|'low', actionItems: string[] }",
          },
      ...
      await pool.query(
       ...
       VALUES ($1,$2,$3,$4,$5,'completed',$6,$7,NOW(),NOW())`,
        [
          ...
          summary,
          intent,
          JSON.stringify({ actionItems, source: 'text_input' }),
          req.user?.id ?? null,
        ],
      );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      let idx = 1;
      if (userId) {
        q += ` AND (created_by = $${idx++} OR created_by IS NULL)`;
        params.push(userId);
      }
      if (status) {
        q += ` AND status = $${idx++}`;
...
router.get('/continuum/voice/notes/:id', authMiddleware(), async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id ?? null;
    ...
    if (!result.rows[0]) {
      sendError(res, 'Voice note not found', 404);
      return;
    }
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!result.rows[0]) {
        sendError(res, 'Voice note not found or access denied', 404);
        return;
      }
