...
router.get(
  '/alloy/integrations/connections',
  authMiddleware(),
...
router.patch(
  '/alloy/integrations/connections/:id',
  authMiddleware(),
  requireRole('admin', 'super_admin'),
  validateBody(
    bodyShape({
...
router.post(
  '/alloy/integrations/webhooks/endpoints',
  authMiddleware(),
  requireRole('admin', 'super_admin'),
  validateBody(
    bodyShape({
...
router.post(
  '/alloy/integrations/webhooks/receive/:endpointId',
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!signature) {
        res
          .status(401)
          .json({ error: 'Missing webhook signature — x-alloy-signature header is required' });
        return;
      }
      const rawBody =
      ...
      setImmediate(async () => {
        try {
            ...
            try {
              await services.ai.chatCompletion(
                [
                  {
                    role: 'system',
                    content: `You are an Alloy signal processor. Process this incoming ${eventType} webhook event for the ${endpoint.target_skill} skill and summarize what action should be taken.`,
                  },
          ...
          logger.error(
            ...
            'alloy-integrations: webhook processing failed',
          );
        }
      });
    } catch (err) {
...
router.post(
  '/alloy/integrations/connections/:id/test',
  authMiddleware(),
  requireRole('ops', 'admin', 'super_admin'),
  validateBody(bodyShape({})),
  async (req: Request, res: Response) => {
