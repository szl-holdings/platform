...
export async function getCached<T>(
  ...
  try {
    const [row] = await db
      ...
      .where(eq(intelligenceCacheTable.key, key))
      .limit(1);
    if (row && new Date(row.expiresAt).getTime() > now) {
      ...
      if (
        !refreshing.has(key) &&
        refreshing.size < MAX_CONCURRENT_REFRESHES &&
        new Date(row.expiresAt).getTime() - now < ttlMs * 0.25
      ) {
        refreshing.add(key);
        fetcher()
          .then(async (fresh) => {
...
export async function upsertCache<T>(key: string, data: T, ttlMs: number): Promise<void> {
  ...
  try {
    await db
      ...
      .values({ key, data, expiresAt, fetchedAt: new Date() })
      .onConflictDoUpdate({
  ...
  const prune = async () => {
    try {
      ...
      const countResult = await db.execute(
        sql`SELECT COUNT(*)::int AS cnt FROM intelligence_cache`,
      );
      const rows =
        (countResult as unknown as { rows?: { cnt: number }[] }).rows ??
        (countResult as unknown as { cnt: number }[]);
...
export async function computeIntelligenceBriefing(): Promise<Record<string, unknown>> {
  ...
  return {
    ...
    topCves: c.slice(0, 3),
    riskLevel: t.some((x: Record<string, unknown>) => x.severity === 'critical')
      ? 'critical'
      : t.length > 10
        ? 'high'
        : 'medium',
...
export function scheduleIntelligenceRefresh(): NodeJS.Timeout {
  return setInterval(async () => {
    ...
    for (const [key, ttlMs, fetcher] of jobs) {
      ...
      refreshing.add(key);
      fetcher()
        .then((data) => upsertCache(key, data, ttlMs))
        .catch(() => {})
        .finally(() => refreshing.delete(key));
    }
  }, 60000);
...
export async function fetchNvdCves(): Promise<CveItem[]> {
  ...
  return items.map((v, idx: number) => {
    ...
    return {
      ...
      score,
      vendor:
        cve?.configurations?.[0]?.nodes?.[0]?.cpeMatch?.[0]?.criteria?.split(':')[3] ?? 'Various',
      product:
...
export async function fetchRssNews(): Promise<NewsItem[]> {
  ...
  const rssData = raw as {
    items?: { title?: string; author?: string; link?: string; pubDate?: string }[];
  };
...
async function fetchOfacSdnVessels(): Promise<
  {
    name: string;
    imo: string;
    flag: string;
