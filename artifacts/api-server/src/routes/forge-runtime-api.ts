...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const tenantPolicy = callerTenantId
        ? forgeRuntime.getTenantPolicy(callerTenantId)
        : undefined;

      const isSuperAdmin =
        req.user?.roles?.includes('super_admin') || req.user?.roles?.includes('admin');
      const requestedClass = (approvalClass ??
        tenantPolicy?.defaultApprovalClass ??
        'propose_only') as ApprovalClass;
      ...
      const effectiveApprovalClass: ApprovalClass = isSuperAdmin
        ? requestedClass
      ...
      const task: ForgeTask = {
        ...
        correlationId,
        sandboxPolicy: sandboxPolicy as Partial<
          import('@szl-holdings/forge-runtime').ForgeSandboxPolicy
        >,
      };
...
router.post(
  '/forge/executions/:executionId/approve',
  authMiddleware(),
  requireRole('admin', 'super_admin', 'exec', 'compliance', 'ops'),
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        !isSuperAdmin &&
        pending.task.tenantId != null &&
        pending.task.tenantId !== callerTenantId
      ) {
        sendNotFound(res, `Execution ${executionId} not found`);
        return;
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const history = forgeRuntime
        ...
        .filter(
          (e) => isSuperAdmin || e.task.tenantId == null || e.task.tenantId === callerTenantId,
        );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!isSuperAdmin && executionId) {
        ...
        if (
          !execution ||
          (execution.task.tenantId != null && execution.task.tenantId !== callerTenantId)
        ) {
          sendNotFound(res, `Execution ${executionId} not found`);
          return;
        }
