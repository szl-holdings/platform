...
const createStoryboardSchema = z
  .object({
    title: z.string().min(1).max(200),
    campaignId: z.number().int().positive(),
...
const createVoiceAssetSchema = z
  .object({
    name: z.string().min(1).max(200),
    campaignId: z.number().int().positive(),
...
const createCampaignAssetSchema = z
  .object({
    campaignId: z.number().int().positive(),
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const rows = await db
        .select()
        .from(dreamscapeCampaignsTable)
        .orderBy(desc(dreamscapeCampaignsTable.createdAt))
        .limit(limit)
        .offset(offset);
...
router.get('/dreamscape/campaigns/:id', authMiddleware(), async (req, res) => {
  try {
    ...
    const [row] = await db
      .select()
      .from(dreamscapeCampaignsTable)
      .where(eq(dreamscapeCampaignsTable.id, id));
    if (!row) {
      sendNotFound(res, 'Campaign');
      return;
    }
    sendSuccess(res, row);
  } catch (err) {
...
router.patch(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(dreamscapeCampaignsTable)
...
router.delete(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .delete(dreamscapeCampaignsTable)
        .where(eq(dreamscapeCampaignsTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Campaign');
...
router.get('/dreamscape/campaigns/:id/scripts', authMiddleware(), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(dreamscapeScriptsTable)
      .where(eq(dreamscapeScriptsTable.campaignId, id))
      .orderBy(desc(dreamscapeScriptsTable.updatedAt));
...
router.get('/dreamscape/scripts/:id', authMiddleware(), async (req, res) => {
  try {
    ...
    const [row] = await db
      .select()
      .from(dreamscapeScriptsTable)
      .where(eq(dreamscapeScriptsTable.id, id));
    if (!row) {
      sendNotFound(res, 'Script');
      return;
    }
    sendSuccess(res, row);
  } catch (err) {
...
router.get('/dreamscape/campaigns/:id/storyboards', authMiddleware(), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(dreamscapeStoryboardsTable)
      .where(eq(dreamscapeStoryboardsTable.campaignId, id))
      .orderBy(dreamscapeStoryboardsTable.sceneNumber);
    sendSuccess(res, rows);
  } catch (err) {
...
router.post(
  '/dreamscape/storyboards',
  authMiddleware(),
  validateBody(createStoryboardSchema),
  async (req, res) => {
    try {
...
router.patch(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(dreamscapeStoryboardsTable)
        .set({ ...req.body, updatedAt: new Date() })
        .where(eq(dreamscapeStoryboardsTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Storyboard');
        return;
      }
      sendSuccess(res, row);
    } catch (err) {
...
router.delete(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .delete(dreamscapeStoryboardsTable)
        .where(eq(dreamscapeStoryboardsTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Storyboard');
...
router.get('/dreamscape/campaigns/:id/voice-assets', authMiddleware(), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(dreamscapeVoiceAssetsTable)
      .where(eq(dreamscapeVoiceAssetsTable.campaignId, id))
      .orderBy(desc(dreamscapeVoiceAssetsTable.createdAt));
    sendSuccess(res, rows);
  } catch (err) {
...
router.post(
  '/dreamscape/voice-assets',
  authMiddleware(),
  validateBody(createVoiceAssetSchema),
  async (req, res) => {
    try {
...
router.patch(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(dreamscapeVoiceAssetsTable)
        .set(req.body)
        .where(eq(dreamscapeVoiceAssetsTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Voice asset');
        return;
      }
      sendSuccess(res, row);
    } catch (err) {
...
router.delete(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .delete(dreamscapeVoiceAssetsTable)
        .where(eq(dreamscapeVoiceAssetsTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Voice asset');
...
router.get('/dreamscape/campaigns/:id/assets', authMiddleware(), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(dreamscapeCampaignAssetsTable)
      .where(eq(dreamscapeCampaignAssetsTable.campaignId, id))
      .orderBy(desc(dreamscapeCampaignAssetsTable.createdAt));
    sendSuccess(res, rows);
  } catch (err) {
...
router.post(
  '/dreamscape/campaign-assets',
  authMiddleware(),
  validateBody(createCampaignAssetSchema),
  async (req, res) => {
    try {
...
router.delete(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .delete(dreamscapeCampaignAssetsTable)
        .where(eq(dreamscapeCampaignAssetsTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Campaign asset');
        return;
      }
      sendSuccess(res, { deleted: true });
    } catch (err) {
...
router.get('/dreamscape/campaigns/:id/reviews', authMiddleware(), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(dreamscapeReviewsTable)
      .where(eq(dreamscapeReviewsTable.campaignId, id))
      .orderBy(desc(dreamscapeReviewsTable.createdAt));
    sendSuccess(res, rows);
  } catch (err) {
...
router.patch(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(dreamscapeReviewsTable)
        .set(req.body)
        .where(eq(dreamscapeReviewsTable.id, id))
        .returning();
      if (!row) {
...
const FALLBACK_CREATIVE_TRENDS = [
  {
    trend: 'AI-Generated Short Films',
  ...
  {
    trend: 'Immersive Brand Storytelling',
    platform: 'Meta + TikTok',
    growth: '+128%',
    audienceReach: '1.8B',
    sentiment: 0.68,
    contentType: 'interactive',
    tools: ['Unreal Engine', 'Unity', 'SparkAR'],
  },
  {
    trend: 'AI-Narrated Podcast Campaigns',
    platform: 'Spotify + Apple',
    growth: '+92%',
    audienceReach: '680M',
    sentiment: 0.64,
    contentType: 'audio',
    tools: ['ElevenLabs', 'Murf AI', 'LOVO'],
  },
  {
    trend: 'Dynamic Hyper-Personalization',
    platform: 'Email + Programmatic',
    growth: '+215%',
    audienceReach: '4.2B',
    sentiment: 0.77,
    contentType: 'multi-channel',
    tools: ['Jasper', 'Copy.ai', 'Persado'],
  },
  {
    trend: 'Creator Economy Partnerships',
    platform: 'TikTok + YouTube',
    growth: '+167%',
    audienceReach: '3.1B',
    sentiment: 0.73,
    contentType: 'influencer',
    tools: ['Grin', 'AspireIQ', 'Upfluence'],
  },
];
...
const FALLBACK_MEDIA_SIGNALS = [
  {
    ...
    benchmark: 3.8,
    trend: '+10.5%',
    insight: 'Long-form content outperforming 2025 baseline',
  },
  {
    platform: 'Instagram',
    metric: 'reelEngagementRate',
    value: 8.4,
    unit: '%',
    benchmark: 6.2,
    trend: '+35.5%',
    insight: 'Reel carousels driving 2x engagement vs single images',
  },
  {
    platform: 'LinkedIn',
    metric: 'thoughtLeadershipCTR',
    value: 3.1,
    unit: '%',
    benchmark: 2.4,
    trend: '+29.2%',
    insight: 'Founder-authored content outperforming brand pages',
  },
  {
    platform: 'TikTok',
    ...
    benchmark: 38.1,
    trend: '+11.0%',
    insight: 'Hook optimization in first 2 seconds critical for completion',
  },
];

router.get(
  '/dreamscape/live/creative-trends',
  dreamLiveLimit,
  ...
  async (_req, res) => {
    try {
        ...
        try {
          ...
          for (const match of xml.matchAll(/<item>([\s\S]*?)<\/item>/g)) {
            ...
            items.push({
              ...
              category: isAI ? 'ai-content' : 'creative',
              source: 'Marketing Land RSS',
              liveSource: true,
            });
      ...
      handleRouteError(res, err, 'Failed to fetch creative trends');
    }
  },
);

router.get(
  '/dreamscape/live/media-signals',
  dreamLiveLimit,
  authMiddleware({ required: false }),
  async (_req, res) => {
    try {
      sendSuccess(res, {
        ...
        insightSummary:
          'AI-assisted content production shows +47% engagement uplift across measured platforms.',
        fetchedAt: new Date().toISOString(),
      });
    } catch (err) {
      handleRouteError(res, err, 'Failed to fetch media signals');
    }
  },
);

router.get(
  '/dreamscape/live/ai-creative-tools',
  dreamLiveLimit,
  authMiddleware({ required: false }),
  async (_req, res) => {
    try {
          ...
          return [
            {
              ...
              likes: 2341,
              downloads: 890000,
              tags: ['diffusion', 'video', 'image2video'],
              openSource: true,
              source: 'demo',
            },
            {
              ...
              likes: 1876,
              downloads: 423000,
              tags: ['video', 'diffusion', 'text2video'],
              openSource: true,
              source: 'demo',
            },
            {
              ...
              likes: 1234,
              downloads: 287000,
              tags: ['video', 'cogvideo', 'generation'],
              openSource: true,
              source: 'demo',
            },
          ];
      ...
      handleRouteError(res, err, 'Failed to fetch AI creative tools');
    }
  },
);
