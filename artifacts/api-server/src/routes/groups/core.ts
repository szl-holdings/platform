...
export function register(router: IRouter): void {
  ...
  router.use(lazyMatch(["/health", "/integrations"], () => import("../health-integrations"), "health-integrations"));
  ...
  router.use(lazyMatch(["/integrations", "/webhooks"], () => import("../external-integrations"), "external-integrations"));
  router.use(lazyMatch("/auth", () => import("../auth"), "auth"));
  router.use(lazyMatch(["/auth", "/login", "/callback", "/logout", "/mobile-auth", "/azure-ad"], () => import("../oidc-auth"), "oidc-auth"));
  ...
  router.use(lazyMatch(["/feedback", "/admin"], () => import("../feedback").then(m => ({ default: m.feedbackRouter })), "feedback"));

  router.use(lazyMatch("/config", () => import("../config"), "config"));
  router.use(lazyMatch("/apm", () => import("../apm"), "apm"));

  router.use("/public", lazyMount(() => import("../public-status"), "public-status"));
  router.use("/admin/status", _writeLimiter);
  router.use("/admin/status", lazyMount(() => import("../public-status"), "public-status"));
