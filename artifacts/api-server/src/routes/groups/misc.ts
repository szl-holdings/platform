...
import { lazyMatch, lazyMount, lazyRegister } from '../../lib/lazy-router';
import {
...
export function register(router: IRouter): void {
  ...
  router.use(
    lazyMatch(['/booking', '/portal', '/carlota'], () => import('../carlota-jo'), 'carlota-jo'),
  );
  ...
  router.use(
    lazyMatch(
      '/certification',
      () => import('../certification-readiness'),
  ...
  router.use(
    lazyMatch(['/salesforce', '/hubspot', '/dynamics', '/crm'], () => import('../crm'), 'crm'),
  );
  ...
  router.use(
    '/innovation-engine',
    lazyMount(() => import('../innovation-engine'), 'innovation-engine'),
  );
  ...
  router.use(
    lazyMatch(
      '/autopilot',
      () => import('../autopilot').then((m) => ({ default: m.autopilotRouter })),
      'autopilot',
    ),
  );
  ...
  router.use(
    lazyMatch('/cross-domain-query', () => import('../cross-domain-query'), 'cross-domain-query'),
  );
  ...
  router.use(
    lazyMatch('/sessions', () => import('../multiplayer-sessions'), 'multiplayer-sessions'),
  );
  ...
  router.use(
    '/distribution-os',
    lazyRegister(() => import('../distribution-os'), 'distribution-os'),
  );
  // /v1 — public API gated by dosApiKeyAuth (API key), not user session. Exempt from tenantScope by design.
  router.use(
    '/v1',
    lazyMount(() => import('../dos-public-api'), 'dos-public-api'),
  );

  router.use('/integrations', _readLimiter);
  router.use(
    lazyMatch('/integrations', () => import('../microsoft-integrations'), 'microsoft-integrations'),
  );
  ...
  router.use(
    lazyMatch('/push-preferences', () => import('../push-preferences'), 'push-preferences'),
  );
  ...
  router.use(
    lazyMatch('/web-push', () => import('../web-push-subscriptions'), 'web-push-subscriptions'),
  );
  ...
  router.use(
    lazyMatch('/investor-analytics', () => import('../investor-analytics'), 'investor-analytics'),
  );
