...
router.get("/firestorm/live/threat-indicators", firestormLiveLimit, authMiddleware({ required: false }), async (req, res) => {
  try {
    ...
    sendSuccess(res, { source: "AlienVault OTX + CISA KEV Composite", note: "Live OTX integration requires OTX API key. Showing enriched demo indicators with CISA KEV risk context.", count: indicators.length, indicators, cisaContext: { mandatoryPatchCount: 1554, ransomwareCampaignLinked: 312 }, fetchedAt: new Date().toISOString() });
