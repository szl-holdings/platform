...
router.get("/alloy/skills", authMiddleware(), async (req, res) => {
  try {
    ...
    const mergedSkills = [
      ...
      ...inMemorySkills.filter(s => !dbSkillIds.has(s.skillId)).map(s => ({ ...s, source: "builtin" })),
    ];
...
router.post("/alloy/skills", authMiddleware(), requireRole("admin"), async (req, res) => {
  try {
    const user = req.user as AuthenticatedUser | undefined;
    if (!user) return sendForbidden(res, "Authentication required");

    const {
      name, version, capability, domain, description,
      triggerConditions, requiredInputs, optionalInputs, outputSchema,
      outputDecisionType, chainMetadata, analyticMode, policyClass,
      estimatedLatencyMs, tags,
    } = req.body;

    if (!name || !capability || !domain || !description || !outputDecisionType || !analyticMode || !policyClass) {
      return sendBadRequest(res, "Missing required fields: name, capability, domain, description, outputDecisionType, analyticMode, policyClass");
    }
    ...
    const registered = skillRegistry.register({
      ...
      outputDecisionType,
      chainMetadata: chainMetadata ?? { canChainTo: [], canChainFrom: [], requiredPreconditions: [], outputsFedToNext: [], maxChainDepth: 4, parallelizable: false },
      analyticMode,
      policyClass,
    ...
    const [saved] = await db.insert(alloySkillRegistryTable).values({
...
router.patch("/alloy/skills/:skillId", authMiddleware(), requireRole("admin"), async (req, res) => {
  try {
    const skillId = req.params.skillId as string;
    const user = req.user as AuthenticatedUser | undefined;
    if (!user) return sendForbidden(res, "Authentication required");
    ...
    const allowed = ["name", "description", "isActive", "triggerConditions", "requiredInputs", "optionalInputs", "chainMetadata", "tags", "estimatedLatencyMs"];
    for (const field of allowed) {
      if (req.body[field] !== undefined) updates[field] = req.body[field];
    }

    if (Object.keys(updates).length === 0) return sendBadRequest(res, "No valid fields to update");

    if (inMemory) {
      skillRegistry.update(skillId, updates as Parameters<typeof skillRegistry.update>[1]);
    }

    const [existing] = await db.select().from(alloySkillRegistryTable).where(eq(alloySkillRegistryTable.skillId, skillId)).limit(1);
    if (existing) {
      const [updated] = await db.update(alloySkillRegistryTable)
        .set({ ...updates as Partial<typeof alloySkillRegistryTable.$inferInsert>, updatedAt: new Date() })
        .where(eq(alloySkillRegistryTable.skillId, skillId))
        .returning();
...
router.delete("/alloy/skills/:skillId", authMiddleware(), requireRole("admin"), async (req, res) => {
  try {
    ...
    await db.update(alloySkillRegistryTable)
...
router.post("/alloy/skills/chains/compose", authMiddleware(), async (req, res) => {
  try {
...
router.get("/alloy/skills/chains/prebuilt/:scenario", authMiddleware(), async (req, res) => {
  try {
    const scenario = req.params.scenario as "full_incident" | "quick_triage" | "compliance_review" | "executive_brief";
    const validScenarios = ["full_incident", "quick_triage", "compliance_review", "executive_brief"];
...
router.post("/alloy/skills/chains/:chainId/plan", authMiddleware(), async (req, res) => {
  try {
...
router.post("/alloy/skills/discover", authMiddleware(), async (req, res) => {
  try {
...
router.post("/alloy/skills/select", authMiddleware(), async (req, res) => {
  try {
...
router.post("/alloy/decisions/:decisionId/outcome", authMiddleware(), async (req, res) => {
  try {
    const decisionId = req.params.decisionId as string;
    const user = req.user as AuthenticatedUser | undefined;
    const {
      agentId, tenantId, skillId, capability,
      actualOutcome, wasActedOn, wasOverridden, overrideReason,
      actualImpactLevel, finalAction, executionResult,
    ...
    if (existing) {
      const [updated] = await db.update(alloyDecisionOutcomes)
        .set({
          actualOutcome,
    ...
    const {
      predictedConfidence, predictedImpactLevel, recommendedAction,
      humanReviewRequired, decisionType,
    } = req.body;

    if (predictedConfidence == null || !predictedImpactLevel || !recommendedAction || !decisionType) {
      return sendBadRequest(res, "For new outcomes: predictedConfidence, predictedImpactLevel, recommendedAction, decisionType are required");
    }

    const [created] = await db.insert(alloyDecisionOutcomes).values({
      decisionId,
      agentId,
      tenantId,
...
router.get("/alloy/decisions/outcomes", authMiddleware(), async (req, res) => {
  try {
    const { limit, offset } = parsePagination(req.query as Record<string, unknown>);
...
router.get("/alloy/agents/:agentId/performance", authMiddleware(), async (req, res) => {
  try {
...
router.get("/alloy/agents/:agentId/accuracy", authMiddleware(), async (req, res) => {
  try {
    const agentId = req.params.agentId as string;
    const windowDays = parseInt(String(req.query.windowDays ?? "30"));
...
router.get("/alloy/agents/:agentId/calibration", authMiddleware(), async (req, res) => {
  try {
    const agentId = req.params.agentId as string;
    const windowDays = parseInt(String(req.query.windowDays ?? "30"));
...
router.get("/alloy/agents/:agentId/trend", authMiddleware(), async (req, res) => {
  try {
    const agentId = req.params.agentId as string;
...
router.get("/alloy/agents/:agentId/skill-effectiveness", authMiddleware(), async (req, res) => {
  try {
    const agentId = req.params.agentId as string;
    const windowDays = parseInt(String(req.query.windowDays ?? "30"));
...
router.post("/alloy/agents/:agentId/performance/snapshot", authMiddleware(), requireRole("admin"), async (req, res) => {
  try {
    const agentId = req.params.agentId as string;
    const user = req.user as AuthenticatedUser | undefined;
    ...
    const [snapshot] = await db.insert(alloyAgentPerformanceSnapshots).values({
      agentId,
      tenantId,
      windowDays,
...
router.get("/alloy/agents/:agentId/performance/history", authMiddleware(), async (req, res) => {
  try {
    const agentId = req.params.agentId as string;
    const { limit, offset } = parsePagination(req.query as Record<string, unknown>);
...
router.get("/alloy/agents/:agentId/self-reflection", authMiddleware(), async (req, res) => {
  try {
    const agentId = req.params.agentId as string;
    const tenantId = req.query.tenantId as string | undefined;
    const user = req.user as AuthenticatedUser | undefined;
    ...
    if (tenantId) {
      ...
      if (tenantConfig && tenantConfig.selfReflectionEnabled === false) {
        return sendSuccess(res, { agentId, hasData: false, contextBlock: "Self-reflection is disabled for this tenant.", reasoningAdjustments: [], confidenceAdjustment: 0, urgentFlags: [], generatedAt: new Date().toISOString() });
      }
    }
...
router.get("/alloy/agents/:agentId/reflections", authMiddleware(), async (req, res) => {
  try {
    const agentId = req.params.agentId as string;
    const { limit, offset } = parsePagination(req.query as Record<string, unknown>);
...
router.get("/alloy/performance/alerts", authMiddleware(), async (req, res) => {
  try {
    const agentId = req.query.agentId as string | undefined;
    const tenantId = req.query.tenantId as string | undefined;
    const user = req.user as AuthenticatedUser | undefined;
...
router.post("/alloy/performance/alerts/evaluate", authMiddleware(), requireRole("admin"), async (req, res) => {
  try {
    ...
    if (tenantConfig && tenantConfig.alertsEnabled === false) {
      return sendSuccess(res, { evaluated: false, reason: "Alerts are disabled for this tenant", alertsGenerated: 0, alerts: [] });
    }
...
router.patch("/alloy/performance/alerts/:alertId/resolve", authMiddleware(), async (req, res) => {
  try {
    ...
    const [resolved] = await db.update(alloyConfidenceAlerts)
...
router.get("/alloy/self-improvement/config", authMiddleware(), async (req, res) => {
  try {
    const tenantId = req.query.tenantId as string | undefined;
    const agentId = req.query.agentId as string | undefined;
    const user = req.user as AuthenticatedUser | undefined;
...
router.put("/alloy/self-improvement/config", authMiddleware(), requireRole("admin"), async (req, res) => {
  try {
    const user = req.user as AuthenticatedUser | undefined;
    if (!user) return sendForbidden(res, "Authentication required");
    ...
    const allowedFields = [
      "shortWindowDays", "longWindowDays", "minSampleSize",
      "accuracyDeclineThreshold", "overrideRateThreshold",
      "lowAcceptanceThreshold", "calibrationDriftThreshold",
      "selfReflectionEnabled", "alertsEnabled",
      "autoEscalateOnCritical", "alertCooldownHours",
    ];
    ...
    let result;
    if (existing) {
      [result] = await db.update(alloySelfImprovementConfig)
        .set({ ...updates as Partial<typeof alloySelfImprovementConfig.$inferInsert>, updatedAt: new Date() })
        .where(eq(alloySelfImprovementConfig.id, existing.id))
        .returning();
    } else {
      [result] = await db.insert(alloySelfImprovementConfig).values({
        tenantId,
    ...
    if (Object.keys(scoringPatch).length > 0) scoringEngine.updateConfig(scoringPatch as Parameters<typeof scoringEngine.updateConfig>[0]);

    const monitorPatch: Record<string, unknown> = {};
    if (configFields.accuracyDeclineThreshold) monitorPatch.accuracyDeclineThreshold = configFields.accuracyDeclineThreshold;
    if (configFields.overrideRateThreshold) monitorPatch.overrideRateThreshold = configFields.overrideRateThreshold;
    if (configFields.lowAcceptanceThreshold) monitorPatch.lowAcceptanceThreshold = configFields.lowAcceptanceThreshold;
    if (configFields.calibrationDriftThreshold) monitorPatch.calibrationDriftThreshold = configFields.calibrationDriftThreshold;
    if (configFields.alertCooldownHours) monitorPatch.alertCooldownMs = configFields.alertCooldownHours * 60 * 60 * 1000;
    if (Object.keys(monitorPatch).length > 0) confidenceMonitor.updateConfig(monitorPatch as Parameters<typeof confidenceMonitor.updateConfig>[0]);
