...
import { dataRetentionAuditLogTable, dataRetentionPoliciesTable, db, pool } from '@szl-holdings/db';
...
const PURGEABLE_TABLES: Array<{
  name: string;
  label: string;
  description: string;
  defaultRetentionDays: number;
  hasTenantColumn: boolean;
  piiColumns: PiiColumnMap;
}> = [
  {
    name: 'audit_events',
    label: 'Audit Events',
  ...
  },
  {
    name: 'usage_events',
    label: 'Usage Events',
    description: 'Feature usage telemetry',
  ...
  {
    name: 'connector_logs',
    label: 'Connector Logs',
    description: 'Integration connector logs',
  ...
  },
  {
    ...
    description: 'Outbound webhook delivery records',
    defaultRetentionDays: 90,
    hasTenantColumn: true,
    piiColumns: null,
  },
  ...
  {
    name: 'platform_contact_requests',
    label: 'Contact Requests',
    description: 'Public contact form submissions',
    defaultRetentionDays: 730,
    hasTenantColumn: false,
    piiColumns: { email: 'redacted@purged.invalid', name: 'Purged User' },
  },
  {
    ...
    hasTenantColumn: true,
    piiColumns: { submitter_email: 'redacted@purged.invalid', submitter_name: 'Purged User' },
  },
];
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      const user = req.user as AuthenticatedUser;
      const { orgId, tableName, retentionDays, purgeStrategy, isActive, description } =
        req.body as z.infer<typeof upsertPolicySchema>;
      ...
      const whereClause =
        orgId != null
          ? and(
              eq(dataRetentionPoliciesTable.tableName, tableName),
              eq(dataRetentionPoliciesTable.orgId, orgId),
            )
          : and(
              eq(dataRetentionPoliciesTable.tableName, tableName),
              isNull(dataRetentionPoliciesTable.orgId),
            );
...
router.post(
  '/data-retention/policies/:policyId/run',
    ...
    try {
      ...
      if (policy.orgId != null) {
        if (!isSuperAdmin(user) && !user.orgs.some((o) => o.orgId === policy.orgId)) {
          res
            .status(403)
            .json({ error: 'Access denied: policy belongs to a different organization' });
          return;
        }
      } else if (!isSuperAdmin(user)) {
      ...
      try {
        ...
        if (policy.purgeStrategy === 'delete') {
          ...
          if (!piiColumns || Object.keys(piiColumns).length === 0) {
            ...
            logger.warn(
              { tableName: policy.tableName, policyId: policy.id },
              'Anonymize strategy: no PII columns defined for this table — skipping data modification',
            );
          } else {
            const setClauses = Object.entries(piiColumns)
              .map(([col, val]) => `"${col}" = '${val}'`)
              .join(', ');
            const whereExtra = Object.keys(piiColumns)
          ...
          affectedRows = countResult.rows[0]?.cnt ?? 0;
          logger.info(
            { tableName: policy.tableName, policyId: policy.id, affectedRows },
            'Archive strategy: rows identified for archival. External archival pipeline required before deletion.',
          );
        }
        ...
        status = 'failure';
        logger.error(
          { err: purgeErr, tableName: policy.tableName, policyId: policy.id },
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const {
        ...
        orgId,
        limit = '50',
      } = req.query as { tableName?: string; orgId?: string; limit?: string };
...
router.get('/data-retention/sweep-status', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const sweepSchedule = schedules.find(
        (s: { name: string }) => s.name === 'data_retention_sweep_weekly',
      ) as any;
      if (sweepSchedule) {
        scheduleInfo = {
    ...
    const policyFilter = superAdmin
      ...
      : (() => {
          ...
          return orgIds.length > 0
            ? and(
                eq(dataRetentionPoliciesTable.isActive, true),
                inArray(dataRetentionPoliciesTable.orgId, orgIds),
              )
            : eq(dataRetentionPoliciesTable.isActive, false);
        })();

    const activePolicies = await db.select().from(dataRetentionPoliciesTable).where(policyFilter);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!user.roles.includes('super_admin')) {
        res
          .status(403)
          .json({ error: 'Only super admins can trigger the global data retention sweep' });
        return;
      }
      ...
      logger.info(
        { jobId: job.id, actorId: user.id, actorName: user.displayName ?? user.email },
        'Manual data retention sweep triggered',
      );

      res.json({
        success: true,
        jobId: job.id,
        message: 'Retention sweep enqueued — all active policies will be processed.',
      });
    } catch (err) {
