...
router.get('/health/integrations', async (_req, res) => {
  try {
    ...
    const overallStatus = summary.degraded > 0 || summary.unavailable > 0 ? 'degraded' : 'healthy';
...
const feedStatusCache = new LRUCache<string, { status: ExternalFeedStatus; checkedAt: number }>({
  max: 100,
});
...
const EXTERNAL_FEEDS: Array<{
  ...
  },
  {
    ...
    requiresKey: false,
    method: 'POST',
    body: 'query=get_recent&selector=5',
...
async function checkExternalFeed(
  feed: (typeof EXTERNAL_FEEDS)[number],
): Promise<ExternalFeedStatus> {
...
router.get('/health/external-feeds', async (_req, res) => {
  try {
    ...
    const overallStatus =
      unavailableCount > 3 ? 'degraded' : degradedCount > 2 ? 'degraded' : 'operational';
...
router.get('/health/billing', async (_req, res) => {
  try {
    ...
    const { result, latencyMs, error } = await checkWithTimeout(() =>
      services.stripe.testConnection(),
    );
...
router.get(
  '/integrations/misp-taxii/indicators',
  authMiddleware(),
  validateQuery(listQuerySchema),
  async (req, res) => {
    try {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const result = await services.nvd.searchCves({
        keyword,
        severity,
