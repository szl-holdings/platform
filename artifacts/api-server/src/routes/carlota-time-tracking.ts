...
import { carlotaInvoicesTable, carlotaTimeEntriesTable, db } from '@szl-holdings/db';
    ...
    try {
      ...
      const [row] = await db
        ...
        .values({
          ...
          rate: typeof b.rate === 'number' ? b.rate : parseInt(String(b.rate ?? 0), 10) || 0,
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (b.rate !== undefined)
        update.rate = typeof b.rate === 'number' ? b.rate : parseInt(String(b.rate), 10) || 0;
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        sendBadRequest(res, 'id, client, engagement, amount, dueDate, issuedDate are required');
        return;
      }
      const [row] = await db
        ...
        .values({
          ...
          items: typeof b.items === 'number' ? b.items : parseInt(String(b.items ?? 0), 10) || 0,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const existing = await db.select({ id: carlotaInvoicesTable.id }).from(carlotaInvoicesTable);
      let maxSeq = 0;
