...
async function scimBearerAuth(req: Request, res: Response, next: NextFunction) {
  ...
  const [tokenRow] = await db
    ...
    .where(and(eq(scimTokensTable.tokenHash, tokenHash), eq(scimTokensTable.isActive, true)))
    .limit(1);
...
function scimResponse(res: Response, status: number, body: unknown) {
  return res.status(status).set('Content-Type', SCIM_CONTENT_TYPE).json(body);
}
    ...
    Resources: [
      {
        ...
        meta: {
          resourceType: 'Schema',
          location: `${baseUrl}/scim/v2/Schemas/${SCIM_USER_SCHEMA}`,
        },
      },
      {
        ...
        meta: {
          resourceType: 'Schema',
          location: `${baseUrl}/scim/v2/Schemas/${SCIM_GROUP_SCHEMA}`,
        },
      },
    ],
  });
  ...
  async (req: Request, res: Response) => {
    try {
        ...
        if (parsed) {
          if (parsed.field === 'userName' || parsed.field === 'email') {
            userQuery = userQuery.where(
              ilike(usersTable.email, parsed.op === 'sw' ? `${parsed.value}%` : parsed.value),
            ) as typeof userQuery;
          } else if (parsed.field === 'displayName') {
            userQuery = userQuery.where(
              ilike(usersTable.displayName, `%${parsed.value}%`),
            ) as typeof userQuery;
      ...
      return scimResponse(res, 200, {
        ...
        startIndex,
        itemsPerPage: count_,
        Resources: page.map((u) =>
          buildUserScimResource(u, provisionedMap.get(u.id) ?? null, baseUrl),
        ),
      });
    } catch (err) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const existing = await db
        ...
        .from(usersTable)
        .where(
          emailToLookup.includes('@')
            ? eq(usersTable.email, emailToLookup)
            : eq(usersTable.email, emailToLookup),
        )
        .limit(1);
      ...
      const [provisionedRow] = await db
        .insert(scimProvisionedUsersTable)
        .values({
      ...
      if (ctx.organizationId) {
        const existing = await db
          ...
          .where(
            and(eq(orgMembersTable.orgId, ctx.organizationId), eq(orgMembersTable.userId, user.id)),
          )
          .limit(1);
        if (!existing[0]) {
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [updatedProvisioned] = await db
        .update(scimProvisionedUsersTable)
        .set({
        ...
        .where(eq(scimProvisionedUsersTable.id, provisioned.id))
        .returning();
      ...
      return scimResponse(
        res,
        200,
        buildUserScimResource(updatedUser, updatedProvisioned ?? null, baseUrl),
      );
    } catch (err) {
    ...
    try {
      ...
      if (userUpdates.isActive === false && ctx.organizationId) {
        await db
          ...
          .where(
            and(eq(orgMembersTable.orgId, ctx.organizationId), eq(orgMembersTable.userId, userId)),
          );
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return scimResponse(res, 200, {
        ...
        startIndex,
        itemsPerPage: count_,
        Resources: page.map((g) =>
          buildGroupScimResource(
            g,
            (membersByGroup.get(g.id) ?? []).map((m) => ({
              userId: m.userId,
              userName: m.userName ?? '',
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (memberUserIds.length > 0 && group) {
        ...
        if (validIds.length > 0) {
          await db
            ...
            .values(validIds.map((uid) => ({ groupId: group.id, userId: uid })))
            .onConflictDoNothing();
        }
      }
      ...
      return scimResponse(res, 201, buildGroupScimResource(group!, [], baseUrl));
    } catch (err) {
    ...
    try {
      ...
      if (newMemberIds.length > 0) {
        const existingUsers = await db
          .select({ id: usersTable.id })
          .from(usersTable)
          .where(inArray(usersTable.id, newMemberIds));
        const validIds = existingUsers.map((u) => u.id);
        if (validIds.length > 0) {
          await db
            .insert(scimGroupMembersTable)
            .values(validIds.map((uid) => ({ groupId, userId: uid })))
            .onConflictDoNothing();
        }
      }
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const operations: { op: string; path?: string; value?: unknown }[] = body.Operations ?? [];
      const groupUpdates: Partial<{ displayName: string; platformRole: string; updatedAt: Date }> =
        { updatedAt: new Date() };
        ...
        if (op.path === 'members' && Array.isArray(op.value)) {
          ...
          if (opName === 'add') {
            if (memberIds.length > 0) {
              ...
              await db
                .insert(scimGroupMembersTable)
                .values(existingUsers.map((u) => ({ groupId, userId: u.id })))
                .onConflictDoNothing();
              for (const u of existingUsers) {
                await revokeUserSessionsOnRoleChange({
                  userId: u.id,
                  changedByUserId: null,
                  reason: 'scim_group_member_add',
                }).catch((err) => {
                  logger.error(
                    { err, userId: u.id },
                    '[scim] session revocation failed on member add',
                  );
                });
              }
            }
            ...
            if (memberIds.length > 0) {
              await db
                ...
                .where(
                  and(
                    eq(scimGroupMembersTable.groupId, groupId),
                    inArray(scimGroupMembersTable.userId, memberIds),
                  ),
                );
              for (const id of memberIds) {
                await revokeUserSessionsOnRoleChange({
                  userId: id,
                  changedByUserId: null,
                  reason: 'scim_group_member_remove',
                }).catch((err) => {
                  logger.error(
                    { err, userId: id },
                    '[scim] session revocation failed on member remove',
                  );
                });
              }
            }
            ...
            const previousMembers = await db
              .select({ userId: scimGroupMembersTable.userId })
              .from(scimGroupMembersTable)
              .where(eq(scimGroupMembersTable.groupId, groupId));
            await db
              .delete(scimGroupMembersTable)
            ...
            for (const uid of allAffected) {
              await revokeUserSessionsOnRoleChange({
                userId: uid,
                changedByUserId: null,
                reason: 'scim_group_replace',
              }).catch((err) => {
                logger.error(
                  { err, userId: uid },
                  '[scim] session revocation failed on group replace',
                );
              });
            }
          }
      ...
      await logScimOperation(ctx.tenantId, 'patch_group', 'Group', 'success', {
        requestBody: body,
      });
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await logScimOperation(ctx.tenantId, 'delete_group', 'Group', 'success', {
