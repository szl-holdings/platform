...
import { hasEmailProviderConfigured, sendEmail } from '../lib/email';
import {
  GUARDIAN_LEDGER_SSE_CHANNEL,
  getGuardianEngine,
...
const GUARDIAN_ALERT_EMAIL =
  process.env.FOUNDER_ALERT_EMAIL ?? process.env.SZL_INTERNAL_EMAIL ?? 'team@szlholdings.com';
const GUARDIAN_SLACK_WEBHOOK = process.env.SLACK_WEBHOOK_URL;
const APPROVALS_URL = process.env.APP_URL
  ? `${process.env.APP_URL}/command/operations/guardian/approvals`
  : 'https://szlholdings.com/command/operations/guardian/approvals';
...
function buildLiveApprovalPolicyEvaluation(args: {
  ...
  const approvalRequired =
    args.outcome === 'require-approval' ||
    args.outcome === 'require-dual-approval' ||
    args.outcome === 'block';
  ...
  memoryRefs.push({
    tier: 'semantic',
    key: args.matchedRuleId ? `guardian.rule:${args.matchedRuleId}` : `guardian.tier:${tier}`,
    freshness: 0.95,
...
async function recordPolicyDecisionAudit(params: {
  ...
  try {
    ...
    const resolvedMode =
      (ev?.resolvedMode as string | undefined) ?? (ev?.mode as string | undefined) ?? null;
...
async function notifyApprovalQueueFilled(params: {
  ...
  if (GUARDIAN_SLACK_WEBHOOK) {
    ...
    fetch(GUARDIAN_SLACK_WEBHOOK, {
      ...
      logger.warn({ err }, '[guardian] Failed to post approval Slack notification'),
    );
  }
}
...
function approvalRowToApi(row: ToolMeshActionApproval, actors?: ActorMap) {
  ...
  return {
    ...
    approvedAt:
      row.approvedAt instanceof Date ? row.approvedAt.toISOString() : (row.approvedAt ?? undefined),
    rejectedById: row.rejectedById ?? undefined,
    rejectedBy: actorOrUndefined(row.rejectedById, map),
    rejectedAt:
      row.rejectedAt instanceof Date ? row.rejectedAt.toISOString() : (row.rejectedAt ?? undefined),
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (enabled !== undefined)
        conditions.push(eq(guardianPoliciesTable.enabled, enabled === 'true'));
      ...
      sendSuccess(res, rows.map(policyRowToApi), 200, {
        page,
        limit,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [existing] = await db
        ...
        .where(
          and(
            eq(guardianTiersTable.tier, tier),
            orgId !== null ? eq(guardianTiersTable.orgId, orgId) : isNull(guardianTiersTable.orgId),
          ),
        )
        .limit(1);
      ...
      const [updated] = await db
        .update(guardianTiersTable)
        .set(u)
        .where(eq(guardianTiersTable.id, existing.id))
        .returning();
      invalidateEffectiveTierCache(orgId);
...
router.get('/policies/:id', authMiddleware(), async (req: Request, res: Response) => {
  try {
    ...
    const [row] = await db
      ...
      .limit(1);
    if (!row) {
      sendNotFound(res, 'Policy not found');
      return;
    }
    ...
    try {
      ...
      const [inserted] = await db
        ...
        .values({
          ...
          tags: rule.tags,
          allowedModels: rule.allowedModels ?? null,
          allowedTools: rule.allowedTools ?? null,
          createdById: user?.id ?? null,
        })
        .returning();

      if (!inserted) {
        handleRouteError(res, new Error('insert returned no row'), 'Failed to create policy');
        return;
      }
      ...
      logger.info(
        { policyId: inserted.id, tier: inserted.tier, action: inserted.action },
        'Policy created',
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [policy] = await db
        .select({ id: guardianPoliciesTable.id })
        .from(guardianPoliciesTable)
        .where(eq(guardianPoliciesTable.id, policyId))
        .limit(1);
      if (!policy) {
        sendNotFound(res, 'Policy not found');
        return;
      ...
      if (!inserted) {
        const [ex] = await db
          ...
          .limit(1);
        if (ex) {
          sendSuccess(res, assignmentRowToApi(ex));
          return;
        }
      ...
      logger.info(
        { policyId, subject: `${inserted.subjectType}:${inserted.subjectId}` },
...
router.delete(
  '/policies/:id/assignments/:assignmentId',
  validateBody(bodyShape({})),
  authMiddleware(),
  requireRole('super_admin', 'admin', 'ops'),
  async (req: Request, res: Response) => {
    try {
      ...
      await logToolAuditEvent({
        ...
        oldValues: existing
          ? (assignmentRowToApi(existing) as unknown as Record<string, unknown>)
          : null,
        req,
      });
      sendSuccess(res, { deleted: true });
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (policyTier)
        conditions.push(
          eq(toolMeshToolsTable.policyTier, policyTier as ToolMeshTool['policyTier']),
        );
      if (enabled !== undefined)
        conditions.push(eq(toolMeshToolsTable.enabled, enabled === 'true'));
      ...
      if (domainTag)
        manifests = manifests.filter((m) =>
          m.domainTags.includes(domainTag as ToolManifest['domainTags'][number]),
        );

      const offset = (page - 1) * limit;
      sendSuccess(res, manifests.slice(offset, offset + limit), 200, {
        page,
        limit,
...
router.get('/tools/:toolId', authMiddleware(), async (req: Request, res: Response) => {
  try {
    ...
    const [row] = await db
      .select()
      .from(toolMeshToolsTable)
      .where(eq(toolMeshToolsTable.toolId, toolId))
      .limit(1);
    if (!row) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const ex = await db
        ...
        .from(toolMeshToolsTable)
        .where(eq(toolMeshToolsTable.toolId, parsed.data.id))
        .limit(1);
      if (ex.length > 0) {
      ...
      const [inserted] = await db
        ...
        .values({
          ...
          policyTier: m.policyTier as any,
      ...
      if (!inserted) {
        handleRouteError(res, new Error('insert returned no row'), 'Failed to register tool');
        return;
      }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!body.version) {
        sendBadRequest(res, 'version is required');
        return;
      }
      const [tool] = await db
        .select({ id: toolMeshToolsTable.id })
      ...
      await logToolAuditEvent({
        action: 'tool.version.add',
        entityType: 'tool_version',
        entityId: toolId,
        newValues: { version: body.version },
        req,
      });
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const rows = await db
        .select()
        .from(toolMeshToolPermissionsTable)
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [inserted] = await db
        .insert(toolMeshToolPermissionsTable)
        .values({
          toolDbId: tool.id,
          subjectType: body.subjectType,
          subjectId: body.subjectId,
      ...
      if (!inserted) {
        const [ex] = await db
          ...
          .limit(1);
        if (ex) {
          sendSuccess(res, permissionRowToApi(ex));
          return;
        }
      ...
      await logToolAuditEvent({
        action: 'tool.permission.grant',
        entityType: 'tool_permission',
        entityId: toolId,
        newValues: { subjectType: body.subjectType, subjectId: body.subjectId, permission },
        req,
      });
...
router.delete(
  '/tools/:toolId/permissions/:permissionId',
  validateBody(bodyShape({})),
  authMiddleware(),
  requireRole('super_admin', 'admin'),
  async (req: Request, res: Response) => {
    try {
      ...
      const deleted = await db
        .delete(toolMeshToolPermissionsTable)
        .where(
          and(
      ...
      await logToolAuditEvent({
        action: 'tool.permission.revoke',
        entityType: 'tool_permission',
        entityId: toolId,
        newValues: { permissionId },
        req,
      });
      sendSuccess(res, { deleted: true });
    } catch (err) {
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      sendSuccess(
        res,
        rows.map((r) => approvalRowToApi(r, actors)),
        200,
        { page, limit, total: totalRow[0]?.count ?? 0 },
      );
    } catch (err) {
    ...
    try {
      ...
      const domainRows = await db
        ...
        .where(
          (!isAdminUser(user)
            ? and(
                eq(guardianActionsTable.orgId, userOrgId(user) ?? -1),
                sql`${tracesTable.domain} IS NOT NULL`,
              )
...
router.get('/actions/:id', authMiddleware(), async (req: Request, res: Response) => {
  try {
    ...
    const [action] = await db
      .select()
      .from(toolMeshActionApprovalsTable)
      .where(eq(toolMeshActionApprovalsTable.id, id))
      .limit(1);
    if (!action) {
      sendNotFound(res, 'Guardian action not found');
      return;
    }
    if (!isAdminUser(req.user)) {
      const orgId = userOrgId(req.user);
    ...
    const actors = await resolveActorMap([
      action.approvedById,
      action.rejectedById,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const toolManifestForEval = toolRow
        ...
        : registryHit
          ? {
              ...
              policyTier: registryHit.policyTier,
              description: registryHit.description,
            }
          : null;
      const tierForEval =
        (toolManifestForEval?.policyTier as string | undefined) ?? 'operator-approved';
      ...
      const [inserted] = await db
        .insert(toolMeshActionApprovalsTable)
        .values({
          requestId,
          toolId,
          action,
      ...
      if (!inserted) {
        handleRouteError(
          ...
          'Failed to create action approval',
        );
        return;
      }
...
const approveActionHandler = async (req: Request, res: Response) => {
  try {
    ...
    const [existing] = await db
      ...
      .limit(1);
    if (!existing) {
      sendNotFound(res, 'Action approval not found');
      return;
    }
    ...
    const user = req.user;
    const [updated] = await db
      .update(toolMeshActionApprovalsTable)
      .set({
        status: 'approved',
    ...
    try {
      ...
      const [rows, totalRow] = await Promise.all([
        db
          ...
          .from(guardianApprovalRequestsTable)
          .where(where as ReturnType<typeof and>)
          .orderBy(desc(guardianApprovalRequestsTable.createdAt))
          .limit(limit)
          .offset(offset),
        db
          .select({ count: sql<number>`count(*)::int` })
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [approval] = await db
        .select()
        .from(guardianApprovalRequestsTable)
        .where(eq(guardianApprovalRequestsTable.requestId, requestId))
        .limit(1);
      if (!approval) {
        sendNotFound(res, 'Guardian approval not found');
        return;
      }
      if (!isAdminUser(req.user)) {
        const orgId = userOrgId(req.user);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!decision || !['approved', 'rejected'].includes(decision)) {
        sendBadRequest(res, 'decision must be one of: approved, rejected');
        return;
      }
      ...
      if (existing.status !== 'pending') {
        sendBadRequest(res, `Cannot review approval in status: ${existing.status}`);
        return;
      }
      ...
      if (!matchingRole && !requiredApprovers.includes('approver')) {
        sendBadRequest(
          res,
          `None of your roles [${userRoles.join(', ')}] are in the required approvers set: [${requiredApprovers.join(', ')}]`,
        );
        return;
      }
      ...
      const [updated] = await db
        ...
        .set({ approvals: updatedApprovals, status: newStatus, updatedAt: new Date() })
        .where(eq(guardianApprovalRequestsTable.requestId, requestId))
        .returning();
      logger.info(
        { requestId, decision, approverId: resolvedApproverId, newStatus },
        'Guardian approval reviewed',
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [rows, totalRow] = await Promise.all([
        db
          .select()
          .from(rollbackEventsTable)
          .where(where as ReturnType<typeof and>)
          .orderBy(desc(rollbackEventsTable.createdAt))
          .limit(limit)
          .offset(offset),
        db
          .select({ count: sql<number>`count(*)::int` })
...
router.get('/rollback-events/:id', authMiddleware(), async (req: Request, res: Response) => {
  try {
    ...
    const [event] = await db
      .select()
      .from(rollbackEventsTable)
      .where(eq(rollbackEventsTable.id, id))
      .limit(1);
    if (!event) {
      sendNotFound(res, 'Rollback event not found');
      return;
    }
    if (!isAdminUser(req.user)) {
      const orgId = userOrgId(req.user);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { actionId, requestId, agentId, tier, triggeredBy, reason, metadata } = req.body as {
        actionId?: string;
        requestId?: string;
        agentId?: string;
        tier?: string;
      ...
      if (!actionId || !requestId || !tier || !triggeredBy || !reason) {
        sendBadRequest(res, 'actionId, requestId, tier, triggeredBy, and reason are required');
        return;
      }

      const tierParsed = PolicyTierSchema.safeParse(tier);
      if (!tierParsed.success) {
        sendBadRequest(res, `Invalid tier: ${tier}`);
        return;
      }
      ...
      logger.info(
        { rollbackEventId: event?.id, actionId, requestId, tier },
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!status || !['pending', 'in-progress', 'completed', 'failed'].includes(status)) {
        sendBadRequest(res, 'status must be one of: pending, in-progress, completed, failed');
        return;
      }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { requestId, agentId, sessionId, workflowId, action, domain, tier, context } =
        req.body as Partial<DecisionRequest>;
      if (!requestId || !action) {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const user = req.user;
      const orgId = userOrgId(user);
      const isFailClosedOutcome =
        result.outcome === 'require-approval' ||
        result.outcome === 'require-dual-approval' ||
      ...
      try {
        ...
        const livePolicyEvaluation = buildLiveApprovalPolicyEvaluation({
          ...
          blockedReason:
            result.outcome === 'block' ? (result.reason ?? 'Blocked by policy') : undefined,
          toolId: toolId ?? null,
          agentId: agentId ?? null,
          sessionId: sessionId ?? null,
          workflowId: workflowId ?? null,
        ...
        const [actionRecord] = await db
          ...
          .values({
            ...
            rollbackRequired: result.rollbackRequired,
            redactApplied: result.redactApplied || redactedFields.length > 0,
            controlViolations: result.controlViolations,
            payload: enrichedPayload,
        ...
        if (result.outcome === 'require-approval' || result.outcome === 'require-dual-approval') {
          ...
          const inserted = await db
            ...
            .onConflictDoNothing()
            .returning({ requestId: guardianApprovalRequestsTable.requestId });
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!isAdminUser(user)) {
        ...
        conditions.push(
          or(eq(guardrailConfigsTable.orgId, orgId), isNull(guardrailConfigsTable.orgId))!,
        );
      }
      if (guardrailType)
        conditions.push(
          eq(
            guardrailConfigsTable.guardrailType,
            guardrailType as GuardrailConfig['guardrailType'],
          ),
        );
      if (enabled !== undefined)
        conditions.push(eq(guardrailConfigsTable.enabled, enabled === 'true'));
      ...
      const [rows, totalRow] = await Promise.all([
        db
          .select()
          .from(guardrailConfigsTable)
          .where(where as ReturnType<typeof and>)
          .orderBy(desc(guardrailConfigsTable.id))
          .limit(limit)
          .offset(offset),
        db
          .select({ count: sql<number>`count(*)::int` })
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [row] = await db
        .select()
        .from(guardrailConfigsTable)
        .where(eq(guardrailConfigsTable.id, id))
        .limit(1);
      if (!row) {
  ...
  async (req: Request, res: Response) => {
    try {
      const body = req.body as {
        ...
        name?: string;
        description?: string;
        guardrailType?: GuardrailConfig['guardrailType'];
        config?: Record<string, unknown>;
        appliesToTier?: GuardrailConfig['appliesToTier'];
        enforcement?: GuardrailConfig['enforcement'];
        enabled?: boolean;
      };
      if (!body.guardrailId || !body.name || !body.guardrailType) {
        sendBadRequest(res, 'guardrailId, name, and guardrailType are required');
        return;
      }
      const orgId = userOrgId(req.user);
      const [inserted] = await db
        .insert(guardrailConfigsTable)
        .values({
          orgId,
      ...
      if (!inserted) {
        handleRouteError(
          res,
          new Error('insert returned no row'),
          'Failed to create guardrail config',
        );
        return;
      }
      logger.info(
