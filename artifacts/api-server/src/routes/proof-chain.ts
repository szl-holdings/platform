...
router.get(
  '/proof-chain/by-content/:contentType/:contentId',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        !reviewState ||
        !['unreviewed', 'reviewed', 'approved', 'flagged', 'retracted'].includes(reviewState)
      ) {
        sendBadRequest(
          res,
          'reviewState must be one of: unreviewed, reviewed, approved, flagged, retracted',
        );
        return;
      }
      ...
      const updated = await reviewProof({
        ...
        reviewNote,
        exportSafetyState: exportSafetyState as
