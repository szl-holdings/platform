...
const INTEGRATION_REGISTRY = [
  {
    ...
    setupGuide:
      'Create a Google Cloud OAuth2 app with the required scopes and provide client_id and client_secret.',
  ...
  },
  {
    ...
    setupGuide:
      'Create a Slack app with Bot Token Scopes. Set SLACK_BOT_TOKEN and SLACK_SIGNING_SECRET.',
  ...
  },
  {
    ...
    description: 'Custom inbound webhook — any system can push events to Continuum',
    authType: 'secret',
    scopes: ['*'],
    approvalClass: 'standard',
...
router.get(
  '/continuum/integrations/registry',
  authMiddleware(),
  async (_req: Request, res: Response) => {
    try {
      const connections = await pool.query(
        `SELECT integration_type, status, is_enabled FROM alloy_integration_connections`,
      );
      const connectedTypes = new Set(
        connections.rows.filter((r) => r.status === 'active').map((r) => r.integration_type),
      );
      const legacyConnectors = await db
        .select({
          type: connectorsTable.type,
      ...
      const registry = INTEGRATION_REGISTRY.map((integration) => ({
        ...integration,
        connected:
          connectedTypes.has(integration.type) ||
          legacyConnectors.some(
            (c) => c.name.toLowerCase() === integration.name.toLowerCase() && c.status === 'active',
          ),
      }));
...
router.get(
  '/continuum/integrations/connections',
  authMiddleware(),
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!integrationType) {
        sendBadRequest(res, 'integrationType is required');
        return;
      }

      const integrationDef = INTEGRATION_REGISTRY.find((i) => i.type === integrationType);
      if (!integrationDef) {
        sendBadRequest(res, `Unknown integration type: ${integrationType}`);
        return;
      }
      ...
      await pool.query(
       ...
       VALUES ($1,$2,$3,$4,$5,$6,'pending',$7,$8,$9,$10,$11,NOW(),NOW())`,
        [
          ...
          integrationType,
          integrationDef.name,
          displayName ?? integrationDef.name,
          tenantId ?? null,
          integrationDef.authType,
          JSON.stringify(scope ?? integrationDef.scopes),
          aprClass,
          JSON.stringify(config ?? {}),
          integrationDef.rateLimit.rpm,
          integrationDef.rateLimit.rph,
        ],
      );
      ...
      sendCreated(res, {
        id,
        integrationType,
        displayName: displayName ?? integrationDef.name,
        status: 'pending',
...
router.patch(
  '/continuum/integrations/connections/:id',
  authMiddleware(),
  requireRole('admin', 'super_admin'),
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await pool.query(
        `UPDATE alloy_integration_connections SET ${updates.join(', ')} WHERE id = $${idx}`,
        params,
      );
...
router.post(
  '/continuum/integrations/webhooks/endpoints',
  authMiddleware(),
  requireRole('admin', 'super_admin'),
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      sendCreated(res, {
        ...
        name,
        secret,
        webhookUrl: `/api/continuum/integrations/webhooks/receive/${id}`,
        allowedEvents,
        targetSkill,
        targetWorkflowType,
  ...
  async (_req: Request, res: Response) => {
    try {
      const result = await pool.query(
        `SELECT id, name, description, is_enabled, allowed_events, target_skill, event_count, last_received_at, created_at FROM alloy_webhook_endpoints ORDER BY created_at DESC`,
      );
...
router.post(
  '/continuum/integrations/webhooks/receive/:endpointId',
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const endpointResult = await pool.query(
        `SELECT * FROM alloy_webhook_endpoints WHERE id = $1 AND is_enabled = TRUE`,
        [endpointId],
      );
      ...
      const signature = req.headers['x-continuum-signature'] as string | undefined;
      if (!signature) {
        res
          .status(401)
          .json({ error: 'Missing webhook signature — x-continuum-signature header is required' });
        return;
      }
      const rawBody =
      ...
      setImmediate(async () => {
        try {
            ...
            try {
              await services.ai.chatCompletion(
                [
                  {
                    role: 'system',
                    content: `You are an Continuum signal processor. Process this incoming ${eventType} webhook event for the ${endpoint.target_skill} skill and summarize what action should be taken.`,
                  },
          ...
          logger.error(
            { err: asyncErr, endpointId },
            'continuum-integrations: webhook processing failed',
          );
        }
      });
    } catch (err) {
...
router.post(
  '/continuum/integrations/connections/:id/test',
  authMiddleware(),
  requireRole('ops', 'admin', 'super_admin'),
  validateBody(bodyShape({})),
  async (req: Request, res: Response) => {
    try {
      const result = await pool.query(`SELECT * FROM alloy_integration_connections WHERE id = $1`, [
        req.params.id,
      ]);
      if (!result.rows[0]) {
      ...
      if (connection.integration_type === 'salesforce') {
        ...
        testResult = {
          connected: sfStatus.connected,
          message: sfStatus.connected ? 'Connected successfully' : 'Connection failed',
        };
        ...
        testResult = {
          ...
          message: jiraStatus.connected ? 'Connected successfully' : 'Connection failed',
        };
      } else if (connection.integration_type === 'slack') {
        const hasToken = !!process.env.SLACK_BOT_TOKEN;
        testResult = {
          connected: hasToken,
          message: hasToken ? 'Slack token configured' : 'SLACK_BOT_TOKEN not set',
        };
      }

      if (testResult.connected) {
        await pool.query(
          `UPDATE alloy_integration_connections SET status='active', last_success_at=NOW(), failure_count=0, updated_at=NOW() WHERE id=$1`,
          [req.params.id],
        );
      } else {
        await pool.query(
          `UPDATE alloy_integration_connections SET failure_count=failure_count+1, last_failure_at=NOW(), updated_at=NOW() WHERE id=$1`,
          [req.params.id],
        );
      }
...
router.get('/continuum/integrations/health', authMiddleware(), async (_req: Request, res: Response) => {
  try {
    const [connections, events] = await Promise.all([
      pool.query(
        `SELECT integration_type, status, is_enabled, failure_count, last_success_at FROM alloy_integration_connections`,
      ),
      pool.query(
        `SELECT integration_type, COUNT(*) as total, SUM(CASE WHEN status='failed' THEN 1 ELSE 0 END) as failed FROM alloy_integration_events WHERE created_at > NOW() - INTERVAL '24 hours' GROUP BY integration_type`,
      ),
    ]);
