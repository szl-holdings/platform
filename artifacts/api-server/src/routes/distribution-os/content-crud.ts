...
import {
  ...
  insertDosAutomationRunSchema,
  insertDosCampaignSchema,
  insertDosCarouselProjectSchema,
  insertDosCarouselSlideSchema,
  insertDosContentCalendarItemSchema,
  insertDosCtaBlockSchema,
  insertDosDistributionTargetSchema,
  insertDosEditorialPillarSchema,
  insertDosIntegrationStatusSchema,
  insertDosLeadNoteSchema,
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    const [article] = await db
      .select()
      .from(dosArticlesTable)
      .where(eq(dosArticlesTable.id, Number(req.params.id)));
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    const [article] = await db
      .select()
      .from(dosArticlesTable)
      .where(eq(dosArticlesTable.slug, req.params.slug as string));
    if (!article) return void sendNotFound(res, 'Article');
    res.json(article);
  },
);
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!parsed.success)
      return void sendBadRequest(res, 'Validation failed', parsed.error.flatten());
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    const versions = await db
      .select()
      .from(dosArticleVersionsTable)
      .where(eq(dosArticleVersionsTable.articleId, Number(req.params.id)))
      .orderBy(desc(dosArticleVersionsTable.createdAt));
    res.json(versions);
  },
);
...
router.get(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    const newsletters = await db
      ...
      .orderBy(desc(dosNewslettersTable.createdAt))
      .limit(100);
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    const [nl] = await db
      .select()
      .from(dosNewslettersTable)
      .where(eq(dosNewslettersTable.id, Number(req.params.id)));
...
router.get(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    const carousels = await db
      .select()
      .from(dosCarouselProjectsTable)
      .orderBy(desc(dosCarouselProjectsTable.createdAt))
      .limit(100);
    res.json(carousels);
  },
);
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    const [c] = await db
      .select()
      .from(dosCarouselProjectsTable)
      .where(eq(dosCarouselProjectsTable.id, Number(req.params.id)));
    if (!c) return void sendNotFound(res, 'Carousel');
    const slides = await db
      .select()
      .from(dosCarouselSlidesTable)
      .where(eq(dosCarouselSlidesTable.projectId, c.id))
      .orderBy(asc(dosCarouselSlidesTable.slideNumber));
    res.json({ ...c, slides });
  },
);

const carouselSlideInputSchema = insertDosCarouselSlideSchema.omit({
  projectId: true,
  slideNumber: true,
});
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (slides !== undefined && slides !== null) {
      ...
      for (let i = 0; i < slides.length; i++) {
        ...
        if (!sr.success)
          return void sendBadRequest(res, `Invalid slide at index ${i}`, sr.error.flatten());
      }
    }
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!parsed.success)
      return void sendBadRequest(res, 'Validation failed', parsed.error.flatten());
    const [c] = await db
      .update(dosCarouselProjectsTable)
      .set({ ...parsed.data, updatedAt: new Date() })
      .where(eq(dosCarouselProjectsTable.id, Number(req.params.id)))
      .returning();
    if (!c) return void sendNotFound(res, 'Carousel');
...
router.get(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    const posts = await db
      .select()
      .from(dosXPostsTable)
      .orderBy(desc(dosXPostsTable.createdAt))
      .limit(100);
    res.json(posts);
  },
);
...
router.get(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    const campaigns = await db
      .select()
      .from(dosCampaignsTable)
      .orderBy(desc(dosCampaignsTable.createdAt))
      .limit(100);
    res.json(campaigns);
  },
);
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!parsed.success)
      return void sendBadRequest(res, 'Validation failed', parsed.error.flatten());
    const [c] = await db
      .update(dosCampaignsTable)
      .set({ ...parsed.data, updatedAt: new Date() })
      .where(eq(dosCampaignsTable.id, Number(req.params.id)))
      .returning();
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    const links = await db
      .select()
      .from(dosCampaignLinksTable)
      .where(eq(dosCampaignLinksTable.campaignId, Number(req.params.id)));
    res.json(links);
  },
);
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (period === 'weekly')
      conditions.push(gte(dosLeadsTable.createdAt, new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)));
    else if (period === 'monthly')
      conditions.push(
        gte(dosLeadsTable.createdAt, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)),
      );
    ...
    const leads =
      conditions.length > 0
        ? await db
            ...
            .orderBy(desc(dosLeadsTable.createdAt))
            .limit(200)
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const [lead] = await db
      ...
      .values({ ...data, score } as typeof dosLeadsTable.$inferInsert)
      .returning();
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const [existing] = await db
      .select()
      .from(dosLeadsTable)
      .where(eq(dosLeadsTable.id, Number(req.params.id)));
    ...
    const [lead] = await db
      .update(dosLeadsTable)
      .set({ ...parsed.data, score, updatedAt: new Date() })
      .where(eq(dosLeadsTable.id, Number(req.params.id)))
      .returning();
    res.json(lead);
  },
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    const notes = await db
      .select()
      .from(dosLeadNotesTable)
      .where(eq(dosLeadNotesTable.leadId, Number(req.params.id)))
      .orderBy(desc(dosLeadNotesTable.createdAt));
    res.json(notes);
  },
);
...
router.post(
  ...
  validateBody(bodyShape({})),
  async (req: Request, res: Response): Promise<void> => {
    const parsed = insertDosLeadNoteSchema.safeParse({
      ...req.body,
...
router.get(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    const pillars = await db
      ...
      .orderBy(asc(dosEditorialPillarsTable.sortOrder));
    res.json(pillars);
  },
);
...
router.get(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    const blocks = await db
      .select()
      .from(dosCtaBlocksTable)
      .orderBy(desc(dosCtaBlocksTable.createdAt));
    res.json(blocks);
  },
);
...
router.get(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    const items = await db
      ...
      .orderBy(asc(dosContentCalendarItemsTable.scheduledDate))
      .limit(200);
    res.json(items);
  },
);
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!parsed.success)
      return void sendBadRequest(res, 'Validation failed', parsed.error.flatten());
    const [item] = await db
      .update(dosContentCalendarItemsTable)
      .set({ ...parsed.data, updatedAt: new Date() })
      .where(eq(dosContentCalendarItemsTable.id, Number(req.params.id)))
      .returning();
...
router.get(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    const targets = await db
      .select()
      .from(dosDistributionTargetsTable)
      .orderBy(desc(dosDistributionTargetsTable.createdAt))
      .limit(200);
    res.json(targets);
  },
);
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!parsed.success)
      return void sendBadRequest(res, 'Validation failed', parsed.error.flatten());
    const [t] = await db
      .update(dosDistributionTargetsTable)
      .set({ ...parsed.data, updatedAt: new Date() })
      .where(eq(dosDistributionTargetsTable.id, Number(req.params.id)))
      .returning();
...
router.get(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    const settings = await db
      ...
      .orderBy(asc(dosSiteSettingsTable.category));
    res.json(settings);
  },
);
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const [s] = await db
      ...
      .values(parsed.data as typeof dosSiteSettingsTable.$inferInsert)
      .returning();
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    const [s] = await db
      .update(dosSiteSettingsTable)
      .set({ value: req.body.value, updatedAt: new Date() })
      .where(eq(dosSiteSettingsTable.key, req.params.key as string))
      .returning();
...
router.post(
  '/integrations/retry/:provider',
  requireAuth,
  validateBody(bodyShape({})),
  async (req: Request, res: Response): Promise<void> => {
    const [i] = await db
      .update(dosIntegrationStatusTable)
      .set({ status: 'disconnected', lastError: null, updatedAt: new Date() })
      .where(eq(dosIntegrationStatusTable.provider, req.params.provider as string))
      .returning();
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const [i] = await db
      .update(dosIntegrationStatusTable)
      .set({ ...parsed.data, updatedAt: new Date() })
      .where(eq(dosIntegrationStatusTable.id, Number(req.params.id)))
      .returning();
    if (!i) return void sendNotFound(res, 'Integration');
    res.json(i);
  },
...
router.get(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    ...
    const items = await db
      .select()
      .from(dosLinktreeConfigTable)
      .where(eq(dosLinktreeConfigTable.isActive, true))
      .orderBy(asc(dosLinktreeConfigTable.sortOrder));
    res.json(items);
  },
);
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!parsed.success)
      return void sendBadRequest(res, 'Validation failed', parsed.error.flatten());
    const [item] = await db
      .update(dosLinktreeConfigTable)
      .set({ ...parsed.data, updatedAt: new Date() })
      .where(eq(dosLinktreeConfigTable.id, Number(req.params.id)))
      .returning();
...
router.get(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    const runs = await db
      ...
      .orderBy(desc(dosAutomationRunsTable.createdAt))
      .limit(50);
    res.json(runs);
  },
);
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!parsed.success)
      return void sendBadRequest(res, 'Validation failed', parsed.error.flatten());
    const [run] = await db
      .update(dosAutomationRunsTable)
      .set(parsed.data)
      .where(eq(dosAutomationRunsTable.id, Number(req.params.id)))
      .returning();
...
router.post(
  '/automation-runs/trigger/:jobType',
  requireAuth,
  validateBody(bodyShape({})),
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      if (jobType === 'carousel-ideas') {
        ...
        const pillars = await db
          ...
          .from(dosEditorialPillarsTable)
          .orderBy(
            desc(dosEditorialPillarsTable.isFavorite),
            asc(dosEditorialPillarsTable.sortOrder),
          );
        ...
        const pillars = await db
          .select()
          .from(dosEditorialPillarsTable)
          .where(eq(dosEditorialPillarsTable.isFavorite, true))
          .orderBy(asc(dosEditorialPillarsTable.sortOrder));
        const pillar = pillars[0];
        const week = now.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
        const items = [
          {
            title: `[Hero Carousel] ${pillar?.name || 'Leadership'} — This Week's Insight (${week})`,
            contentType: 'carousel' as const,
            notes: 'Hero carousel for thought-leadership pack. Generate via Carousel Lab.',
          },
          {
            title: `[Text Post] ${pillar?.name || 'Leadership'} — Founder Perspective (${week})`,
            contentType: 'x-post' as const,
            notes: 'Supporting text post. Adapt from hero carousel key insight.',
          },
          {
            title: `[Email Draft] CTA Newsletter — ${pillar?.name || 'Leadership'} Focus (${week})`,
            contentType: 'newsletter' as const,
            notes: 'CTA email draft. Lead with carousel teaser, CTA to offer page.',
          },
          {
            title: `[Offer Rec] Review offer page alignment for: ${pillar?.name || 'Leadership'} (${week})`,
            contentType: 'other' as const,
            notes: "Recommendation: ensure current offer page CTA matches this week's theme.",
          },
        ];
        ...
        const [pvToday] = await db
          ...
          .where(gte(dosPageViewsTable.createdAt, todayStart));
        const [leadsToday] = await db
          .select({ count: count() })
          .from(dosLeadsTable)
          .where(gte(dosLeadsTable.createdAt, todayStart));
        const [openCalItems] = await db
          .select({ count: count() })
          .from(dosContentCalendarItemsTable)
          .where(eq(dosContentCalendarItemsTable.status, 'idea'));
        const [schedItems] = await db
          .select({ count: count() })
          .from(dosContentCalendarItemsTable)
          .where(eq(dosContentCalendarItemsTable.status, 'ready'));
        const [xQueued] = await db
          .select({ count: count() })
          .from(dosXPostsTable)
          .where(eq(dosXPostsTable.status, 'queued'));
        ...
        const [pvCount] = await db
          .select({ count: count() })
          .from(dosPageViewsTable)
          .where(gte(dosPageViewsTable.createdAt, weekAgo));
        const [leadCount] = await db
          .select({ count: count() })
          .from(dosLeadsTable)
          .where(gte(dosLeadsTable.createdAt, weekAgo));
        const [articleCount] = await db
          .select({ count: count() })
          .from(dosArticlesTable)
          .where(
            and(eq(dosArticlesTable.status, 'published'), gte(dosArticlesTable.createdAt, weekAgo)),
          );
        const [carouselCount] = await db
          .select({ count: count() })
          .from(dosCarouselProjectsTable)
          .where(gte(dosCarouselProjectsTable.createdAt, weekAgo));
        const [xSent] = await db
          .select({ count: count() })
          .from(dosXPostsTable)
          .where(and(eq(dosXPostsTable.status, 'sent'), gte(dosXPostsTable.sentAt, weekAgo)));
        const [nlCount] = await db
          .select({ count: count() })
          .from(dosNewslettersTable)
          .where(
            and(
              eq(dosNewslettersTable.status, 'published'),
              gte(dosNewslettersTable.createdAt, weekAgo),
            ),
          );
        const [autoRuns] = await db
          .select({ count: count() })
          .from(dosAutomationRunsTable)
          .where(
            and(
              eq(dosAutomationRunsTable.status, 'completed'),
              gte(dosAutomationRunsTable.createdAt, weekAgo),
            ),
          );
        const [followUpQueue] = await db
          .select({ count: count() })
          .from(dosLeadsTable)
          .where(eq(dosLeadsTable.stage, 'needs-followup'));
        const campaigns = await db
          .select()
          .from(dosCampaignsTable)
          .where(eq(dosCampaignsTable.status, 'active'))
          .orderBy(desc(dosCampaignsTable.totalConversions))
          .limit(1);
        ...
        if (carouselsGenerated === 0)
          recommendations.push(
            "No carousels were generated this week. Use Carousel Lab to create 2-3 pieces for next week's queue.",
          );
        else
          recommendations.push(
            `Carousels generated: ${carouselsGenerated}. Ensure all ready carousels are exported to aiCarousels and scheduled on LinkedIn.`,
          );
        if (totalLeads === 0)
          recommendations.push(
            'No new leads this week. Review CTA placement on high-traffic pages and update Linktree priority links.',
          );
        else
          recommendations.push(
            `${totalLeads} new lead(s) this week via ${topLeadSource}. Follow up within 48 hours on any warm leads.`,
          );
        if (followUp > 0)
          recommendations.push(
            `${followUp} lead(s) in the follow-up queue. Clear this queue before end of week.`,
          );
        if (publishedArticles === 0)
          recommendations.push(
            'No articles published this week. Consider publishing or scheduling a flagship essay for authority building.',
          );
        recommendations.push(
          "Review the content calendar and ensure next week has at least 3 items in 'Scheduled' status before Friday.",
        );
        output = {
          totalVisits,
          totalLeads,
