...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (existing.status === 'sent')
      return void res.status(400).json({ error: 'Already published' });
    try {
      ...
      await db
        ...
        .where(eq(dosXPostsTable.id, existing.id));
...
router.post(
  '/articles/:id/publish-medium',
  validateBody(
    bodyShape({
      publishStatus: z.unknown().optional(),
    }),
  ),
  requireAuth,
  async (req: Request, res: Response): Promise<void> => {
      ...
      if (!result.ok) {
        const status =
          result.error === 'Article not found'
            ? 404
            : result.error === 'Article has no body content'
              ? 400
              : 502;
...
router.post(
  '/newsletters/:id/publish-substack',
  validateBody(bodyShape({})),
  requireAuth,
  async (_req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (!result.ok) {
        const status =
          result.error === 'Newsletter not found'
            ? 404
            : result.error === 'Newsletter has no body content'
              ? 400
              : 502;
        return void res.status(status).json({ error: result.error });
      }
      const [newsletter] = await db
        ...
        .where(eq(dosNewslettersTable.id, id));
  ...
  async (_req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      const [carousel] = await db
        ...
        .where(eq(dosCarouselProjectsTable.id, id));
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const slides = await db
      ...
      .where(eq(dosCarouselSlidesTable.projectId, carousel.id))
      ...
      for (let i = 0; i < slides.length; i++) {
        ...
        if (slide.slideType === 'intro') {
          if (slide.tagline) {
            ...
            doc.text(slide.tagline.toUpperCase(), 80, 300, {
              width: 920,
              align: 'center',
              characterSpacing: 4,
            });
          ...
          doc.text(slide.title || carousel.title, 80, 380, {
            width: 920,
            align: 'center',
            lineGap: 8,
          });
          ...
          doc.font('Helvetica').fontSize(14).fillColor(accentColor);
          doc.text((slide.tagline || 'SZL HOLDINGS').toUpperCase(), 80, 280, {
            width: 920,
            align: 'center',
            characterSpacing: 4,
          });
          doc.font('Helvetica-Bold').fontSize(40).fillColor(textColor);
          doc.text(slide.title || 'Thank You', 80, 360, {
            width: 920,
            align: 'center',
            lineGap: 8,
          });
          ...
          doc.text(`${slideNum} / ${String(slides.length).padStart(2, '0')}`, 80, 980, {
            width: 920,
            align: 'right',
          });
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const [pvCount] = await db
      ...
      .where(gte(dosPageViewsTable.createdAt, windowStart));
    const [leadCount] = await db
      .select({ count: count() })
      .from(dosLeadsTable)
      .where(gte(dosLeadsTable.createdAt, windowStart));
    const [articleCount] = await db
      .select({ count: count() })
      .from(dosArticlesTable)
      .where(eq(dosArticlesTable.status, 'published'));
    ...
    const [xSent] = await db
      .select({ count: count() })
      .from(dosXPostsTable)
      .where(eq(dosXPostsTable.status, 'sent'));
    const [xFailed] = await db
      .select({ count: count() })
      .from(dosXPostsTable)
      .where(eq(dosXPostsTable.status, 'failed'));
    const [nlReady] = await db
      .select({ count: count() })
      .from(dosNewslettersTable)
      .where(eq(dosNewslettersTable.status, 'approved'));
    const [autoRuns] = await db
      .select({ count: count() })
      .from(dosAutomationRunsTable)
      .where(
        and(
          gte(dosAutomationRunsTable.createdAt, windowStart),
    ...
    const campaigns = await db
      ...
      .orderBy(desc(dosCampaignsTable.totalClicks))
      .limit(1);
    ...
    // Automations health
    const failedAutos = await db
      .select({ count: count() })
      .from(dosAutomationRunsTable)
      .where(
        and(
          gte(dosAutomationRunsTable.createdAt, windowStart),
          eq(dosAutomationRunsTable.status, 'failed'),
        ),
      );
...
router.post(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    ...
    const seedCampaigns = [
      {
        name: 'Founder LinkedIn Spring 2025',
        slug: 'founder-linkedin-spring-2025',
        description:
          'Founder-led content campaign on LinkedIn targeting enterprise operators and MSP buyers.',
      ...
      {
        name: 'Newsletter — Weekly Brief Launch',
        slug: 'newsletter-weekly-brief-launch',
        description:
          'Campaign to grow newsletter subscriber base via bio links, X, and Substack cross-posts.',
        status: 'active' as const,
        owner: 'Stephen',
        totalClicks: 87,
        totalConversions: 23,
      ...
      {
        name: 'Lyte Product Launch Q2',
        slug: 'lyte-product-launch-q2',
        description: 'Campaign for Lyte product awareness targeting SMB IT teams and MSPs.',
    ...
    if (linkedinCamId) {
      const existingLinks = await db
        ...
        .where(eq(dosCampaignLinksTable.campaignId, linkedinCamId));
      if (existingLinks.length === 0) {
        await db.insert(dosCampaignLinksTable).values([
          {
            campaignId: linkedinCamId,
            name: 'LinkedIn Bio Link',
            source: 'linkedin',
            medium: 'bio-link',
            campaign: 'founder-linkedin-spring-2025',
            destination: 'https://szlholdings.com',
            fullUrl:
              'https://szlholdings.com?utm_source=linkedin&utm_medium=bio-link&utm_campaign=founder-linkedin-spring-2025',
            clicks: 89,
            conversions: 4,
          },
          {
            campaignId: linkedinCamId,
            name: 'Carousel CTA — Lyte',
            source: 'linkedin',
            medium: 'social',
            campaign: 'founder-linkedin-spring-2025',
            content: 'carousel-lyte-ops',
            destination: 'https://szlholdings.com/lyte',
            fullUrl:
              'https://szlholdings.com/lyte?utm_source=linkedin&utm_medium=social&utm_campaign=founder-linkedin-spring-2025&utm_content=carousel-lyte-ops',
            clicks: 53,
            conversions: 4,
          },
        ]);
    ...
    const seedLeads = [
      {
        ...
        budget: '$2k–5k/mo',
        message:
          "We've been struggling to scale our NOC operations with our current tooling. Your Lyte platform looks like exactly what we need. Would love to set up a call to discuss pricing and onboarding.",
        source: 'linkedin',
        medium: 'social',
        campaign: 'founder-linkedin-spring-2025',
        landingPage: '/lyte',
      ...
      },
      {
        ...
        budget: '$5k–15k/mo',
        message:
          "Reached out from your newsletter. We're building internal automation tooling and evaluating partners. Enterprise email, proper budget, and we're serious buyers.",
        source: 'newsletter',
        medium: 'email',
