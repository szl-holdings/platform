...
export async function runSignalFusion(
  ...
  const [signals, alerts, escalations, metrics] = await Promise.all([
    ...
    db
      ...
      .where(
        or(eq(lyteEscalationsTable.status, 'open'), eq(lyteEscalationsTable.status, 'in_progress')),
      )
      .limit(50),
    db
      .select()
      ...
      .where(
        and(
          ...
          gte(lyteMetricsTable.recordedAt, new Date(Date.now() - 24 * 3600 * 1000)),
        ),
      )
      .limit(50),
  ]);

  const fusedNodes: Array<{
    id: string;
    entityType: string;
    name: string;
    source: string;
    ...
    try {
      const node = await lyteAdapter.upsertEntity({
        ...
        provenance: {
          ...
          sourceType: 'api',
          sourceLabel: `lyte:${sig.sourceType}`,
        },
  ...
  for (const metric of metrics) {
    try {
      const node = await lyteAdapter.upsertEntity({
        ...
        provenance: {
          sourceId: String(metric.id),
          sourceType: 'system',
          sourceLabel: 'lyte:metric-anomaly',
        },
        confidence: 0.75,
...
router.post(
  '/lyte/cognitive/signal-fusion/run',
  authMiddleware(),
...
router.get('/lyte/cognitive/signal-fusion', authMiddleware(), async (req, res) => {
  try {
    const [signals, alerts, escalations] = await Promise.all([
      ...
      db
        .select({
          id: lyteAlertsTable.id,
          name: lyteAlertsTable.name,
          service: lyteAlertsTable.service,
          severity: lyteAlertsTable.severity,
          status: lyteAlertsTable.status,
        })
        .from(lyteAlertsTable)
        .limit(50),
      db
        .select({
          id: lyteEscalationsTable.id,
          title: lyteEscalationsTable.title,
          severity: lyteEscalationsTable.severity,
          status: lyteEscalationsTable.status,
        })
        .from(lyteEscalationsTable)
        .limit(50),
    ]);
    ...
    for (const s of signals) {
      if (!bySource[s.source])
        bySource[s.source] = { count: 0, severities: {}, latestAt: s.receivedAt.toISOString() };
      bySource[s.source].count++;
      bySource[s.source].severities[s.severity] =
        (bySource[s.source].severities[s.severity] ?? 0) + 1;
      if (s.receivedAt > new Date(bySource[s.source].latestAt))
        bySource[s.source].latestAt = s.receivedAt.toISOString();
    }
    ...
    const sourceDomains = [
      ...
      },
      {
        type: 'alerts',
        count: alerts.length,
        sources: [...new Set(alerts.map((a) => a.service ?? 'unknown'))],
      },
  ...
  try {
    ...
    const [blockedItems, stalledActions, openEscalations, criticalSignals] = await Promise.all([
      db
        ...
        .where(eq(lyteReadinessItemsTable.status, 'blocked'))
        .limit(50),
      db
        .select()
      ...
      db
        ...
        .from(lyteEscalationsTable)
        .where(
          or(eq(lyteEscalationsTable.status, 'open'), eq(lyteEscalationsTable.status, 'escalated')),
        )
      ...
      db
        ...
        .from(lyteSignalsTable)
        .where(
          and(eq(lyteSignalsTable.severity, 'critical'), ne(lyteSignalsTable.status, 'resolved')),
        )
        .limit(20),
    ]);
    ...
    for (const item of blockedItems) {
      ...
      if (!ownerMap[owner])
        ownerMap[owner] = { bottlenecks: 0, var: 0, ageHours: 0, items: [], escalationCount: 0 };
    ...
    for (const action of stalledActions) {
      ...
      if (!domainBottlenecks[domain])
        domainBottlenecks[domain] = { count: 0, var: 0, level: 'low' };
...
router.get(
  '/lyte/cognitive/interventions',
  authMiddleware(),
  validateQuery(lyteCognitiveInterventionsQuerySchema),
  async (req, res) => {
    try {
      ...
      const [activeSignals, stalledActions, blockedItems, openEscalations] = await Promise.all([
        db
          ...
          .from(lyteSignalsTable)
          .where(
            and(ne(lyteSignalsTable.status, 'resolved'), ne(lyteSignalsTable.status, 'dismissed')),
          )
          .orderBy(desc(lyteSignalsTable.receivedAt))
          .limit(100),
        db
          .select()
          .from(lyteActionsTable)
          .where(ne(lyteActionsTable.state, 'resolved'))
        ...
        db
          ...
          .where(
            or(
              eq(lyteReadinessItemsTable.status, 'blocked'),
              eq(lyteReadinessItemsTable.status, 'not_started'),
            ),
          )
          .limit(30),
        db
          ...
          .where(
            or(
              eq(lyteEscalationsTable.status, 'open'),
              eq(lyteEscalationsTable.status, 'escalated'),
            ),
          )
          .limit(20),
      ]);
      ...
      const groups: SignalGroup[] = Object.entries(signalsByCategory).map(([cat, sigs]) => {
        ...
        return {
          ...
          suggestedOwner: sigs[0]
            ? (((sigs[0].metadata as any | null)?.assignee as string) ?? undefined)
            : undefined,
          ...
          evidence: [
            ...
            {
              label: 'Max Severity',
              value: criticalCount > 0 ? 'critical' : highCount > 0 ? 'high' : 'medium',
              source: 'lyte:signals',
            },
            {
              label: 'Value at Risk',
              value: `$${(totalVar / 1000).toFixed(0)}K`,
              source: 'lyte:actions',
            },
            ...sources
              .slice(0, 2)
              .map((src) => ({ label: 'Source', value: src, source: 'lyte:connector' })),
          ],
          ...
          rec.urgency === 'critical'
            ...
            : rec.urgency === 'urgent'
              ...
              : rec.urgency === 'moderate'
                ...
                : 0.15;
        const varRisk =
          varValue >= 1_000_000 ? 0.3 : varValue >= 250_000 ? 0.2 : varValue >= 50_000 ? 0.1 : 0;
      ...
      sendSuccess(res, {
        ...
        stalledActionsSummary: {
          ...
          byCategory: stalledActions.reduce<Record<string, number>>((a, act) => {
            a[act.signalCategory] = (a[act.signalCategory] ?? 0) + 1;
            return a;
          }, {}),
        },
  ...
  try {
    const [actions, readinessItems, escalations, incidents] = await Promise.all([
      ...
      db
        ...
        .where(
          or(
            eq(lyteReadinessItemsTable.status, 'blocked'),
            eq(lyteReadinessItemsTable.status, 'not_started'),
            eq(lyteReadinessItemsTable.status, 'in_progress'),
          ),
        )
        .limit(50),
      db
        ...
        .where(
          or(
            eq(lyteEscalationsTable.status, 'open'),
            eq(lyteEscalationsTable.status, 'escalated'),
            eq(lyteEscalationsTable.status, 'in_progress'),
          ),
        )
        .orderBy(desc(lyteEscalationsTable.createdAt))
        .limit(50),
      db
        ...
        .where(
          or(eq(lyteIncidentsTable.status, 'open'), eq(lyteIncidentsTable.status, 'investigating')),
        )
        .limit(30),
    ]);

    const accountabilityMap: Record<
      string,
      {
        owner: string;
    ...
    for (const action of actions) {
      ...
      entry.interventions.push({
        id: action.id,
        title: action.title,
        category: action.signalCategory,
    ...
    const sortedMap = Object.values(accountabilityMap).sort(
    ...
    try {
      ...
      const [actions, signals, escalations] = await Promise.all([
        db
          ...
          .from(lyteActionsTable)
          .where(
            and(ne(lyteActionsTable.state, 'resolved'), gte(lyteActionsTable.createdAt, cutoff)),
          )
          .limit(200),
        db
          .select()
          .from(lyteSignalsTable)
          .where(
            and(ne(lyteSignalsTable.status, 'resolved'), gte(lyteSignalsTable.createdAt, cutoff)),
          )
          .limit(200),
        db
          .select()
          ...
          .where(
            and(
              ...
              gte(lyteEscalationsTable.createdAt, cutoff),
            ),
          )
          .limit(100),
      ]);
      ...
      const totalVaR =
        actions.reduce((s, a) => s + parseFloat(a.valueAtRisk ?? '0'), 0) +
        signals.reduce((s, sig) => s + estimateVarFromSignal(sig), 0);

      const totalSignalVaR = signals.reduce(
        (s, sig) => {
          const v = estimateVarFromSignal(sig);
          return {
            critical: s.critical + (sig.severity === 'critical' ? v : 0),
            high: s.high + (sig.severity === 'high' ? v : 0),
            total: s.total + v,
          };
        },
...
router.get(
  '/lyte/cognitive/executive-narrative',
  authMiddleware(),
  validateQuery(listQuerySchema),
  async (req, res) => {
    try {
      ...
      const [signals, actions, incidents, escalations, readinessItems] = await Promise.all([
        db
          ...
          .where(
            and(
              gte(lyteSignalsTable.receivedAt, fromDate),
              lte(lyteSignalsTable.receivedAt, toDate),
            ),
          )
          .orderBy(desc(lyteSignalsTable.receivedAt))
          .limit(200),
        db
          ...
          .from(lyteActionsTable)
          .where(
            and(gte(lyteActionsTable.createdAt, fromDate), lte(lyteActionsTable.createdAt, toDate)),
          )
          .orderBy(desc(lyteActionsTable.createdAt))
          .limit(100),
        db
          ...
          .where(
            and(
              gte(lyteIncidentsTable.createdAt, fromDate),
              lte(lyteIncidentsTable.createdAt, toDate),
            ),
          )
        ...
        db
          ...
          .where(
            and(
              gte(lyteEscalationsTable.createdAt, fromDate),
              lte(lyteEscalationsTable.createdAt, toDate),
            ),
          )
          .orderBy(desc(lyteEscalationsTable.createdAt))
          .limit(50),
        db
          ...
          .where(
            and(
              gte(lyteReadinessItemsTable.createdAt, fromDate),
              lte(lyteReadinessItemsTable.createdAt, toDate),
            ),
          )
          .limit(50),
      ]);
      ...
      for (const cs of criticalSignals.slice(0, 3)) {
        citations.push({
          ref: `SIG-${cs.id}`,
          source: cs.source,
          value: `Critical: ${cs.title}`,
          at: cs.receivedAt.toISOString(),
        });
      }
      for (const inc of openIncidents.slice(0, 2)) {
        citations.push({
          ref: `INC-${inc.id}`,
          source: 'lyte:incidents',
          value: inc.title,
      ...
      const operationalStatus =
        criticalSignals.length > 3 || openIncidents.length > 2
          ? 'elevated-risk'
      ...
      if (criticalSignals.length > 0) {
        paragraphs.push(
          `During ${windowLabel}, Lyte detected ${criticalSignals.length} critical signal${criticalSignals.length > 1 ? 's' : ''} ` +
            `requiring immediate executive attention [${criticalSignals
              .slice(0, 3)
              .map((s) => `SIG-${s.id}`)
              .join(', ')}]. ` +
      ...
      if (criticalSignals.length > 0)
        recommendations.push(
          `Immediately review and resolve ${criticalSignals.length} critical signal(s) — estimated $${(criticalSignals.reduce((s, sig) => s + estimateVarFromSignal(sig), 0) / 1000).toFixed(0)}K VaR.`,
        );
      if (overdueActions.length > 0)
        recommendations.push(
          `Reassign or escalate ${overdueActions.length} overdue action(s) to prevent SLA breach.`,
        );
      if (blockedItems.length > 0)
        recommendations.push(
          `Unblock ${blockedItems.length} readiness item(s) to restore delivery cadence.`,
        );
      if (openIncidents.length > 0)
        recommendations.push(
          `Drive resolution of ${openIncidents.length} open incident(s) — target closure within 24h for critical severity.`,
        );
      if (recommendations.length === 0)
        recommendations.push('Maintain current monitoring cadence. No critical actions required.');
