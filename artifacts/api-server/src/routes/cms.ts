...
const patchPageSchema = createPageSchema
  .partial()
...
const patchArticleSchema = createArticleSchema
  .partial()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one field is required' });
...
const patchCmsPostSchema = createCmsPostSchema
  .partial()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one field is required' });
...
const patchSectionSchema = createSectionSchema
  .partial()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one field is required' });
...
const patchVentureSchema = createVentureSchema
  .partial()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one field is required' });
...
const patchCaseStudySchema = createCaseStudySchema
  .partial()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one field is required' });
...
const patchNavigationItemSchema = createNavigationItemSchema
  .partial()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one field is required' });
...
const patchTestimonialSchema = createTestimonialSchema
  .partial()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one field is required' });
...
const patchFaqSchema = createFaqSchema
  .partial()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one field is required' });
...
const patchCtaSchema = createCtaSchema
  .partial()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one field is required' });
...
const patchRoadmapItemSchema = createRoadmapItemSchema
  .partial()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one field is required' });
...
const patchServiceItemSchema = createServiceItemSchema
  .partial()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one field is required' });
...
const patchUpdateSchema = createUpdateSchema
  .partial()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one field is required' });
...
const patchLeadStatusSchema = createLeadStatusSchema
  .partial()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one field is required' });
...
const patchDownloadSchema = createDownloadSchema
  .partial()
  .refine((d) => Object.keys(d).length > 0, { message: 'At least one field is required' });
...
const createRedirectSchema = z.object({
  ...
  statusCode: z
    ...
    .refine((v) => [301, 302, 307, 308].includes(v), {
      message: 'statusCode must be 301, 302, 307, or 308',
    })
    .optional(),
...
router.get('/cms/sites', async (_req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list sites');
  }
});
...
router.get('/cms/sites/:slug', async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to get site');
  }
});
...
router.patch('/cms/sites/:id', requireCmsWrite, validateBody(patchSiteSchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to update site');
  }
});
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to list pages');
    }
  },
);
  ...
  try {
    ...
    const [row] = await db
      ...
      .where(
        isEditor
          ? eq(pagesTable.id, id)
          : and(eq(pagesTable.id, id), eq(pagesTable.status, 'published')),
      );
    if (!row) {
      sendNotFound(res, 'Page');
      return;
    }
    sendSuccess(res, row);
  } catch (err) {
    handleRouteError(res, err, 'Failed to get page');
  }
});
...
router.post('/cms/pages', requireCmsWrite, validateBody(createPageSchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to create page');
  }
});
...
router.patch('/cms/pages/:id', requireCmsWrite, validateBody(patchPageSchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to update page');
  }
});
...
router.delete('/cms/pages/:id', validateBody(bodyShape({})), requireCmsWrite, async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to delete page');
  }
});
...
router.get('/cms/sections', validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list sections');
  }
});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to create section');
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      const [row] = await db
        ...
        .returning();
      if (!row) {
        sendNotFound(res, 'Section');
        return;
      }
      sendSuccess(res, row);
    } catch (err) {
      handleRouteError(res, err, 'Failed to update section');
    }
  },
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to delete section');
    }
  },
);
...
router.get('/cms/ventures', async (_req, res) => {
  try {
    const rows = await db
      .select()
      .from(venturesTable)
      .orderBy(asc(venturesTable.sortOrder), asc(venturesTable.name));
    sendSuccess(res, rows);
  } catch (err) {
...
router.get('/cms/ventures/:slug', async (req, res) => {
  try {
    const [row] = await db
      .select()
      .from(venturesTable)
      .where(eq(venturesTable.slug, String(req.params.slug)));
    if (!row) {
      sendNotFound(res, 'Venture');
      return;
    }
    sendSuccess(res, row);
  } catch (err) {
...
router.patch(
  '/cms/ventures/:id',
  requireCmsWrite,
  validateBody(patchVentureSchema),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to list articles');
    }
  },
);
...
router.get('/cms/articles/:slug', authMiddleware({ required: false }), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to get article');
  }
});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to create article');
    }
  },
);
...
router.patch(
  '/cms/articles/:id',
  requireCmsWrite,
  validateBody(patchArticleSchema),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
      ...
      handleRouteError(res, err, 'Failed to update article');
    }
  },
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to delete article');
    }
  },
);
...
router.get('/cms/case-studies/:slug', authMiddleware({ required: false }), async (req, res) => {
  try {
    ...
    const [row] = await db
      ...
      .where(
        isEditor
          ? eq(caseStudiesTable.slug, String(req.params.slug))
          : and(
              eq(caseStudiesTable.slug, String(req.params.slug)),
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to create case study');
    }
  },
);
...
router.patch(
  '/cms/case-studies/:id',
  requireCmsWrite,
  validateBody(patchCaseStudySchema),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
      ...
      handleRouteError(res, err, 'Failed to update case study');
    }
  },
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to delete case study');
    }
  },
);
...
router.get('/cms/navigation-items', validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list navigation items');
  }
});
...
router.post(
  ...
  requireCmsWrite,
  validateBody(createNavigationItemSchema),
  async (req, res) => {
    try {
      const [row] = await db.insert(navigationItemsTable).values(req.body).returning();
      sendSuccess(res, row, 201);
    } catch (err) {
      handleRouteError(res, err, 'Failed to create navigation item');
    }
  },
);

router.patch(
  '/cms/navigation-items/:id',
  requireCmsWrite,
  validateBody(patchNavigationItemSchema),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        ...
        .returning();
      if (!row) {
        sendNotFound(res, 'Navigation item');
        return;
      }
      sendSuccess(res, row);
    } catch (err) {
      handleRouteError(res, err, 'Failed to update navigation item');
    }
  },
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to delete navigation item');
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to create testimonial');
    }
  },
);
...
router.patch(
  '/cms/testimonials/:id',
  requireCmsWrite,
  validateBody(patchTestimonialSchema),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        ...
        .returning();
      if (!row) {
        sendNotFound(res, 'Testimonial');
        return;
      }
      sendSuccess(res, row);
    } catch (err) {
      handleRouteError(res, err, 'Failed to update testimonial');
    }
  },
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to delete testimonial');
    }
  },
);
...
router.get('/cms/faqs', validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list FAQs');
  }
});
...
router.post('/cms/faqs', requireCmsWrite, validateBody(createFaqSchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to create FAQ');
  }
});
...
router.patch('/cms/faqs/:id', requireCmsWrite, validateBody(patchFaqSchema), async (req, res) => {
  try {
    ...
    const [row] = await db
      ...
      .returning();
    if (!row) {
      sendNotFound(res, 'FAQ');
      return;
    }
    sendSuccess(res, row);
  } catch (err) {
    handleRouteError(res, err, 'Failed to update FAQ');
  }
});
...
router.delete('/cms/faqs/:id', validateBody(bodyShape({})), requireCmsWrite, async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to delete FAQ');
  }
});
...
router.get('/cms/ctas', validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list CTAs');
  }
});
...
router.post('/cms/ctas', requireCmsWrite, validateBody(createCtaSchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to create CTA');
  }
});
...
router.patch('/cms/ctas/:id', requireCmsWrite, validateBody(patchCtaSchema), async (req, res) => {
  try {
    ...
    const [row] = await db
      .update(ctasTable)
      .set({ ...req.body, updatedAt: new Date() })
      .where(eq(ctasTable.id, id))
      .returning();
    if (!row) {
      sendNotFound(res, 'CTA');
      return;
    }
    sendSuccess(res, row);
  } catch (err) {
    handleRouteError(res, err, 'Failed to update CTA');
  }
});
...
router.delete('/cms/ctas/:id', validateBody(bodyShape({})), requireCmsWrite, async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to delete CTA');
  }
});
...
router.get('/cms/roadmap-items', validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list roadmap items');
  }
});
...
router.post(
  '/cms/roadmap-items',
  requireCmsWrite,
  validateBody(createRoadmapItemSchema),
  async (req, res) => {
    try {
      const [row] = await db.insert(roadmapItemsTable).values(req.body).returning();
      sendSuccess(res, row, 201);
    } catch (err) {
      handleRouteError(res, err, 'Failed to create roadmap item');
    }
  },
);

router.patch(
  '/cms/roadmap-items/:id',
  requireCmsWrite,
  validateBody(patchRoadmapItemSchema),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        ...
        .returning();
      if (!row) {
        sendNotFound(res, 'Roadmap item');
        return;
      }
      sendSuccess(res, row);
    } catch (err) {
      handleRouteError(res, err, 'Failed to update roadmap item');
    }
  },
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to delete roadmap item');
    }
  },
);
...
router.patch(
  '/cms/services-items/:id',
  requireCmsWrite,
  validateBody(patchServiceItemSchema),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
...
router.get('/cms/features-items', validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list features');
  }
});
...
router.post(
  '/cms/features-items',
  requireCmsWrite,
  validateBody(createFeatureItemSchema),
  async (req, res) => {
    try {
      const [row] = await db.insert(featuresTable).values(req.body).returning();
      sendSuccess(res, row, 201);
    } catch (err) {
      handleRouteError(res, err, 'Failed to create feature');
    }
  },
);
...
router.get('/cms/use-cases', validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list use cases');
  }
});
...
router.post(
  '/cms/use-cases',
  requireCmsWrite,
  validateBody(createUseCaseSchema),
  async (req, res) => {
    try {
      const [row] = await db.insert(useCasesTable).values(req.body).returning();
      sendSuccess(res, row, 201);
    } catch (err) {
      handleRouteError(res, err, 'Failed to create use case');
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      const rows = await db
        ...
        .from(updatesTable)
        .where(conditions.length ? and(...conditions) : undefined)
        .orderBy(desc(updatesTable.publishedAt))
        .limit(limit)
        .offset(offset);
      const [{ count }] = await db
        .select({ count: sql<number>`count(*)::int` })
      ...
      handleRouteError(res, err, 'Failed to list updates');
    }
  },
);
...
router.post('/cms/updates', requireCmsWrite, validateBody(createUpdateSchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to create update');
  }
});
...
router.patch(
  '/cms/updates/:id',
  requireCmsWrite,
  validateBody(patchUpdateSchema),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        ...
        .returning();
      if (!row) {
        sendNotFound(res, 'Update');
        return;
      }
      sendSuccess(res, row);
    } catch (err) {
      handleRouteError(res, err, 'Failed to update update record');
    }
  },
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to list contact submissions');
    }
  },
);
...
router.post(
  '/cms/lead-status',
  requireCmsWrite,
  validateBody(createLeadStatusSchema),
  async (req, res) => {
    try {
      const [row] = await db.insert(leadStatusTable).values(req.body).returning();
      sendSuccess(res, row, 201);
    } catch (err) {
      handleRouteError(res, err, 'Failed to create lead status');
    }
  },
);
...
router.patch(
  '/cms/lead-status/:id',
  requireCmsWrite,
  validateBody(patchLeadStatusSchema),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        ...
        .set({ ...req.body, updatedAt: new Date() })
        .where(eq(leadStatusTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Lead status');
        return;
      }
      sendSuccess(res, row);
    } catch (err) {
      handleRouteError(res, err, 'Failed to update lead status');
    }
  },
);
...
router.get('/cms/site-settings', validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list site settings');
  }
});
...
router.get('/cms/site-settings/:siteId', async (req, res) => {
  try {
    ...
    const rows = await db
      ...
      .where(eq(siteSettingsTable.siteId, siteId));
    sendSuccess(res, rows);
  } catch (err) {
    handleRouteError(res, err, 'Failed to get site settings');
  }
});
...
router.post(
  '/cms/site-settings',
  requireCmsWrite,
  validateBody(siteSettingSchema),
  async (req, res) => {
    try {
      ...
      if (existing) {
        ...
        const [row] = await db
          ...
          .values({ siteId, key, valueJson })
          .returning();
        sendSuccess(res, row, 201);
      }
    } catch (err) {
      handleRouteError(res, err, 'Failed to set site setting');
    }
  },
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to delete site setting');
    }
  },
);
...
router.get('/cms/media-assets', validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list media assets');
  }
});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to create media asset');
    }
  },
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to delete media asset');
    }
  },
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to list downloads');
    }
  },
);
...
router.post(
  '/cms/downloads',
  requireCmsWrite,
  validateBody(createDownloadSchema),
  async (req, res) => {
    try {
      const [row] = await db.insert(downloadsTable).values(req.body).returning();
      sendSuccess(res, row, 201);
    } catch (err) {
      handleRouteError(res, err, 'Failed to create download');
    }
  },
);
...
router.patch(
  '/cms/downloads/:id',
  requireCmsWrite,
  validateBody(patchDownloadSchema),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        ...
        .returning();
      if (!row) {
        sendNotFound(res, 'Download');
        return;
      }
      sendSuccess(res, row);
    } catch (err) {
      handleRouteError(res, err, 'Failed to update download');
    }
  },
);
...
router.get('/cms/redirects', authMiddleware({ required: false }), async (_req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to list redirects');
  }
});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to create redirect');
    }
  },
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to delete redirect');
    }
  },
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to list CMS posts');
    }
  },
);
...
router.get('/cms/posts/:slug', authMiddleware({ required: false }), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to get CMS post');
  }
});
...
router.post(
  ...
  validateBody(createCmsPostSchema),
  async (req, res) => {
    try {
      const [row] = await db.insert(cmsPostsTable).values(req.body).returning();
      sendSuccess(res, row, 201);
    } catch (err) {
      handleRouteError(res, err, 'Failed to create CMS post');
    }
  },
);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to update CMS post');
    }
  },
);
...
router.patch(
  ...
  authMiddleware(),
  requireRole('admin', 'editor'),
  validateBody(patchCmsPostSchema),
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const updateData = { ...req.body, updatedAt: new Date() };
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to delete CMS post');
    }
  },
);

router.post(
  '/cms/posts/upload-image',
  authMiddleware(),
  requireRole('admin', 'editor'),
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to upload image');
    }
  },
);
