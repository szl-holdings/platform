...
import {
  ...
  triggerSync,
} from '../services/prism-connectors';
import {
  getDocumentPipelineStats,
  getDocumentsForMatter,
} from '../services/prism-document-pipeline';
import {
  enqueuePrismJob,
  getJobStats,
  PRISM_JOB_TYPES,
  replayDeadLetterEvent,
} from '../services/prism-queue';

const PC_MATTER_TYPES = [
  'auto_injury',
  'premises_liability',
  'insurance_coverage',
  'medical_malpractice',
...
const PC_APPROVAL_REQUEST_TYPES = [
  'demand_send',
...
function deriveMatterPrivilegeLevel(
  matterType: string | null | undefined,
): 'public' | 'confidential' | 'privileged' | 'restricted' {
  switch (matterType) {
...
router.get('/matters', authMiddleware(), validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch matters');
  }
});
...
router.get('/matters/:matterId', authMiddleware(), async (req, res) => {
  try {
    ...
    const parties = await db
      .select()
      .from(pcPartiesTable)
      .where(eq(pcPartiesTable.matterId, matterId));
    const claims = await db
      .select()
      .from(pcClaimsTable)
      .where(eq(pcClaimsTable.matterId, matterId));
    ...
    const offers = await db
      .select()
      .from(pcOffersTable)
      .where(eq(pcOffersTable.matterId, matterId))
      .orderBy(desc(pcOffersTable.offerDate));
    const damages = await db
      .select()
      .from(pcDamagesTable)
      .where(eq(pcDamagesTable.matterId, matterId));
    ...
    const tasks = await db
      .select()
      .from(pcTasksTable)
      .where(eq(pcTasksTable.matterId, matterId))
      .orderBy(desc(pcTasksTable.createdAt));
    ...
    handleRouteError(res, err, 'Failed to fetch matter detail');
  }
});
...
router.post('/matters', authMiddleware(), validateBody(createMatterSchema), async (req, res) => {
  try {
    const orgId = getOrgId(req);
    const { title, matterType, status, jurisdiction, caseNumber, courtName, filingDate } =
      req.body as z.infer<typeof createMatterSchema>;
    ...
    handleRouteError(res, err, 'Failed to create matter');
  }
});
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to update matter');
    }
  },
);
...
router.get('/matters/:matterId/parties', authMiddleware(), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch parties');
  }
});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to create party');
    }
  },
);
...
router.get('/matters/:matterId/deadlines', authMiddleware(), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch deadlines');
  }
});

router.post(
  '/matters/:matterId/deadlines',
  authMiddleware(),
  validateBody(addDeadlineSchema),
  async (req, res) => {
    try {
      const matterId = parseIdParam(req.params.matterId as string);
      if (!matterId) return sendBadRequest(res, 'Invalid matter ID');
      const { title, dueDate, priority, deadlineType } = req.body as z.infer<
        typeof addDeadlineSchema
      >;
      ...
      await enqueuePrismJob(getOrgId(req), PRISM_JOB_TYPES.DEADLINE_EVALUATE, {
        matterId,
      ...
      handleRouteError(res, err, 'Failed to create deadline');
    }
  },
);
...
router.get('/matters/:matterId/communications', authMiddleware(), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch communications');
  }
});
...
router.get('/matters/:matterId/documents', authMiddleware(), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch documents');
  }
});
...
router.get('/matters/:matterId/discovery', authMiddleware(), async (req, res) => {
  try {
    ...
    const discovery = await db
      .select()
      .from(pcDiscoveryTable)
      .where(eq(pcDiscoveryTable.matterId, matterId))
      .orderBy(desc(pcDiscoveryTable.createdAt));
    sendSuccess(res, { discovery });
  } catch (err) {
    handleRouteError(res, err, 'Failed to fetch discovery');
  }
});
...
router.get('/matters/:matterId/witnesses', authMiddleware(), async (req, res) => {
  try {
    ...
    const witnesses = await db
      .select()
      .from(pcWitnessesTable)
      .where(eq(pcWitnessesTable.matterId, matterId));
    sendSuccess(res, { witnesses });
  } catch (err) {
    handleRouteError(res, err, 'Failed to fetch witnesses');
  }
});
...
router.get('/approvals', authMiddleware(), validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch approvals');
  }
});
...
router.post(
  ...
  async (req, res) => {
    try {
      const orgId = getOrgId(req);
      const { title, description, matterId, requestType, sourceBasis } = req.body as z.infer<
        typeof createApprovalSchema
      >;
      if (!matterId || !requestType || !title)
        return sendBadRequest(res, 'matterId, requestType, and title required');
      ...
      handleRouteError(res, err, 'Failed to create approval');
    }
  },
);
...
router.post('/exports', authMiddleware(), validateBody(createExportSchema), async (req, res) => {
  try {
    const orgId = getOrgId(req);
    const { matterId, exportType, format, externalMatterId } = req.body as z.infer<
    ...
    let derivedPrivilegeLevel: 'public' | 'confidential' | 'privileged' | 'restricted' =
      'privileged';
    ...
    if (policyResult.effect === 'require_approval') {
      if (!resolvedMatter) {
        return sendForbidden(
          res,
          `Export requires approval but no DB-resolved matter is available`,
        );
      }
      ...
      void db
        ...
        .values({
          ...
          matterId: resolvedMatter.id,
          actorId: req.user?.id ?? null,
          action: 'export_approval_required',
          entityType: 'export',
    ...
    await enqueuePrismJob(
      orgId,
      PRISM_JOB_TYPES.EXPORT_GENERATE,
      {
        exportId: exp.id,
        matterId: resolvedMatterId,
        exportType,
    ...
    handleRouteError(res, err, 'Failed to create export');
  }
});
...
router.get('/connectors', authMiddleware(), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch connectors');
  }
});

router.post(
  '/connectors/:accountId/sync',
  authMiddleware(),
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to trigger sync');
    }
  },
);
...
router.get('/connectors/:accountId/history', authMiddleware(), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch sync history');
  }
});
...
router.get('/jobs', authMiddleware(), async (req, res) => {
  try {
    ...
    const dlqCount = await db
      .select({ count: count() })
      .from(pcDeadLetterEventsTable)
      .where(
        and(
          eq(pcDeadLetterEventsTable.orgId, orgId),
          sql`${pcDeadLetterEventsTable.resolvedAt} IS NULL`,
        ),
      );
    sendSuccess(res, { stats, deadLetterCount: dlqCount[0]?.count ?? 0 });
  } catch (err) {
    handleRouteError(res, err, 'Failed to fetch job stats');
  }
});
...
router.get('/jobs/dead-letter', authMiddleware(), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch dead letter events');
  }
});

router.post(
  '/jobs/dead-letter/:eventId/replay',
  authMiddleware(),
  validateBody(bodyShape({})),
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to replay dead letter event');
    }
  },
);
...
router.get('/pipeline/stats', authMiddleware(), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch pipeline stats');
  }
});
...
router.get('/notifications', authMiddleware(), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch notifications');
  }
});
...
router.get('/audit', authMiddleware(), validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch audit events');
  }
});
