...
async function buildDomainSnapshot(domain: string): Promise<DomainSnapshot> {
  ...
  const [entityCount, activeCount, avgRow, typeRows, staleRow, edgeRow] = await Promise.all([
    ...
    db.select({ count: sql<number>`count(*)::int` }).from(cstNodes).where(and(eq(cstNodes.domain, domain), eq(cstNodes.isActive, true))),
    db.select({ avg: sql<number>`coalesce(avg(confidence), 1)::float` }).from(cstNodes).where(eq(cstNodes.domain, domain)),
    db.select({ entityType: cstNodes.entityType, count: sql<number>`count(*)::int` }).from(cstNodes).where(eq(cstNodes.domain, domain)).groupBy(cstNodes.entityType).orderBy(sql`count(*) desc`).limit(5),
    db.select({ count: sql<number>`count(*)::int` }).from(cstNodes).where(and(eq(cstNodes.domain, domain), sql`freshness < ${staleThreshold}`)),
    db.select({ count: sql<number>`count(*)::int` }).from(cstEdges).where(sql`(select domain from cst_nodes where id = from_node_id limit 1) = ${domain}`),
  ]);
...
async function generateBrief(domains: string[]): Promise<ExecutiveBrief> {
  ...
  const [totalEdges, crossDomainEdges] = await Promise.all([
    ...
    db.select({ count: sql<number>`count(*)::int` }).from(cstEdges).where(
  ...
  const overallHealth = snapshots.length > 0
    ? Math.round(snapshots.reduce((s, d) => s + d.healthScore, 0) / snapshots.length * 100) / 100
    : 1.0;
  ...
  for (const snap of snapshots) {
    if (snap.entityCount === 0) {
      alerts.push({ domain: snap.domain, message: `No entities found for domain ${snap.domain}`, severity: "info" });
    } else if (snap.staleFraction > 0.5) {
      alerts.push({ domain: snap.domain, message: `${Math.round(snap.staleFraction * 100)}% of ${snap.domain} entities are stale (>24h)`, severity: "warning" });
    }
    if (snap.entityCount > 0) {
