...
const DOMAIN_TOOLS: McpTool[] = [
  {
    ...
    description: "Query current fleet positions, vessel details, and active voyage status for the Vessels maritime fleet",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        region: { type: "string", description: "Maritime region to check (e.g. 'South China Sea', 'Gulf of Aden')" },
      },
    },
  },
  {
    name: "firestorm_threat_scan",
    description: "Query active cybersecurity threats, recent CVEs, and incident status from the Firestorm SOC platform",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        framework: { type: "string", description: "Compliance framework (e.g. 'NIST', 'ISO 27001', 'CIS')" },
      },
    },
  },
  {
  ...
  },
  {
    ...
    description: "Query recent real estate market activity, distress signals, and economic indicators",
    inputSchema: {
      type: "object",
      properties: {
        domain: { type: "string", description: "Market domain filter (e.g. 'commercial', 'residential', 'industrial')" },
      },
    },
  },
  {
    name: "lyte_health_check",
    description: "Get current platform health metrics, active monitoring alerts, and system status across the SZL ecosystem",
    inputSchema: {
      type: "object",
      properties: {},
    },
  },
  {
    ...
    description: "Generate an executive summary of the current ecosystem state including health, signals, and key metrics",
    inputSchema: {
      type: "object",
      properties: {
        timeRange: { type: "string", description: "Time range for the summary (e.g. '1h', '24h', '7d')" },
      },
    },
  },
  {
...
const PLATFORM_TOOLS: McpTool[] = [
  {
    ...
    description: "Start a named Alloy workflow with parameters. Returns a run ID for status tracking. Workflows that require approval return pending_approval status.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Generate a document, brief, or report as an Alloy artifact. Returns the created artifact with content.",
    inputSchema: {
      type: "object",
      properties: {
        title: { type: "string", description: "Title of the artifact" },
        content: { type: "object", description: "Content payload for the artifact" },
        artifactType: { type: "string", enum: ["report", "brief", "plan", "analysis"], description: "Type of artifact to create" },
  ...
  {
    name: "alloy_research",
    description: "Launch a multi-source research query through Alloy's research mode. Synthesizes intelligence from domain agents.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        domains: {
          type: "array",
          items: { type: "string" },
          description: "Domain agents to query: maritime, security, research, creative, analytics, infrastructure, readiness",
        },
      },
      required: ["query"],
    },
  ...
  },
  {
    ...
    description: "Approve or reject a pending decision in the Alloy decision pipeline. Requires ops or admin role.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "List available Alloy skills from the skill registry, including their approval class and capability metadata",
    inputSchema: {
      type: "object",
      properties: {
        category: { type: "string", description: "Filter by skill category" },
        approvalClass: { type: "string", enum: ["auto", "review", "admin_only"], description: "Filter by approval class" },
      },
    },
  },
  {
    name: "alloy_skill_invoke",
    description: "Invoke a registered skill by slug with parameters. Skills with review/admin_only approval class will queue for approval instead of executing immediately.",
    inputSchema: {
      type: "object",
      properties: {
...
const MCP_RESOURCES: McpResource[] = [
  ...
  },
  {
    ...
    description: "System prompts for all 12 domain agents — describes each agent's expertise and capabilities",
    mimeType: "application/json",
  },
  {
    uri: "alloy://skills/catalog",
    name: "Skill Registry Catalog",
    description: "Available skills with their input/output contracts and approval class requirements",
    mimeType: "application/json",
  },
  {
...
const MCP_PROMPTS: McpPrompt[] = [
  {
    ...
    arguments: [
      { name: "topic", description: "The research topic or question to investigate", required: true },
      { name: "domains", description: "Comma-separated domain agents to consult (maritime, security, research, creative, analytics, infrastructure, readiness)", required: false },
  ...
  },
  {
    ...
    arguments: [
      { name: "target", description: "The asset, system, or organization to assess", required: true },
      { name: "threat_type", description: "Type of threat to focus on (e.g. ransomware, APT, insider, vulnerability)", required: false },
      { name: "framework", description: "Security framework for assessment (MITRE ATT&CK, NIST, CIS)", required: false },
    ],
  },
  {
    ...
    arguments: [
      { name: "location", description: "Geographic region or specific address to analyze", required: true },
      { name: "property_type", description: "Property type: commercial, residential, industrial, mixed-use", required: false },
      { name: "analysis_type", description: "Type of analysis: distress, investment, market, comparative", required: false },
    ],
  },
  {
    name: "fleet_report",
    ...
    arguments: [
      { name: "fleet_scope", description: "Fleet or vessel group to report on (all, region, vessel ID)", required: false },
      { name: "focus_areas", description: "Areas to focus on: weather, chokepoints, compliance, performance", required: false },
      { name: "report_period", description: "Time period for the report (e.g. 24h, 7d, 30d)", required: false },
    ],
  },
  {
    ...
    arguments: [
      { name: "time_range", description: "Time range for the digest (default: 24h)", required: false },
      { name: "include_domains", description: "Domains to include: maritime, security, real_estate, operations, research, all", required: false },
      { name: "format", description: "Output format: brief, detailed, or slide-ready", required: false },
    ],
  },
];

function buildPromptMessages(promptName: string, args: Record<string, string>): Array<{ role: string; content: { type: string; text: string } }> {
  switch (promptName) {
    case "research_brief": {
...
async function executeTool(
  ...
  switch (toolName) {
    ...
    case "vessels_weather_risk": {
      ...
      const path = region ? `/api/intelligence/maritime/weather?region=${encodeURIComponent(region)}` : "/api/intelligence/maritime/weather";
      const data = await internalGet(path, token);
      return { tool: toolName, domain: "vessels", result: data };
    }
    case "firestorm_threat_scan": {
      const severity = toolArgs["severity"] as string | undefined;
      const path = severity ? `/api/intelligence/threats?severity=${encodeURIComponent(severity)}` : "/api/intelligence/threats";
      const data = await internalGet(path, token);
    ...
    case "terra_property_search": {
      const region = toolArgs["region"] as string | undefined;
      const path = region ? `/api/terra/distress?region=${encodeURIComponent(region)}` : "/api/terra/distress";
      const data = await internalGet(path, token);
    ...
    case "alloy_launch_workflow": {
      ...
      const [workflow] = await db.select().from(alloyWorkflowsTable).where(eq(alloyWorkflowsTable.id, workflowId));
      if (!workflow) throw new Error(`Workflow ${workflowId} not found`);
      if (!isGlobalAdmin(user)) {
        const orgIds = getUserOrgIds(user);
        if (!orgIds.includes(workflow.orgId ?? -1)) throw new Error("Access denied to this workflow");
      }
      const runResult = await internalPost(`/api/alloy/workflows/${workflowId}/run`, {
    ...
    case "alloy_workflow_status": {
      ...
      const [run] = await db.select().from(alloyWorkflowRunsTable).where(eq(alloyWorkflowRunsTable.id, runId));
      ...
      return { tool: toolName, runId, state: run.state, startedAt: run.startedAt, completedAt: run.completedAt, durationMs: run.durationMs, output: run.output };
    }
    case "alloy_create_artifact": {
      const { title, content, artifactType, orgId } = toolArgs as { title: string; content: Record<string, unknown>; artifactType?: string; orgId?: number };
      if (!title || !content) throw new Error("title and content are required");
      const userOrgIds = getUserOrgIds(user);
      const resolvedOrgId = isGlobalAdmin(user) ? (orgId ?? userOrgIds[0] ?? null) : userOrgIds[0] ?? null;
      const result = await internalPost("/api/alloy/artifacts", {
        title,
    ...
    case "alloy_research": {
      ...
      const result = await internalPost("/api/nuro-mesh/orchestrate", {
        query,
    ...
    case "alloy_decision_status": {
      ...
      const rows = await db.select().from(alloyDecisions)
    ...
    case "alloy_approve_decision": {
      ...
      if (!isGlobalAdmin(user) && !user.roles.includes("ops")) throw new Error("Insufficient permissions — requires ops or admin role");
      ...
      const [decision] = await db.select().from(alloyDecisions).where(eq(alloyDecisions.id, decisionId));
      if (!decision) throw new Error(`Decision ${decisionId} not found`);
      const reviewer = user.displayName ?? "MCP Operator";
      const path = action === "approve" ? `/api/decisions/${decisionId}/approve` : `/api/decisions/${decisionId}/reject`;
      const result = await internalPost(path, { notes: toolArgs["notes"] ?? `${action} via MCP`, reviewer }, token);
    ...
    case "alloy_skill_invoke": {
      ...
      if (skill.approvalClass === "admin_only" && !isGlobalAdmin(user)) {
        const runRecord = await db.insert(alloySkillRuns).values({
          skillId: skill.id,
          agentId: "mcp",
          input,
          status: "pending",
      ...
      if (skill.approvalClass === "review") {
        ...
        return { tool: toolName, status: "dry_run", skill: { name: skill.name, description: skill.description }, inputReceived: input };
      }
      ...
      try {
        const output = { executed: true, skillSlug, input, executedAt: new Date().toISOString(), note: "Skill executed via MCP — actual domain execution depends on skill implementation" };
        await db.update(alloySkillRuns).set({
          status: "success",
          output,
          durationMs: Date.now() - start,
          completedAt: new Date(),
        }).where(eq(alloySkillRuns.id, runId!));
        await db.update(alloySkills).set({ usageCount: (skill.usageCount ?? 0) + 1, lastUsedAt: new Date() }).where(eq(alloySkills.id, skill.id));
        return { tool: toolName, status: "success", skillRunId: runId, output };
      } catch (err) {
        await db.update(alloySkillRuns).set({ status: "failed", errorMessage: String(err), completedAt: new Date() }).where(eq(alloySkillRuns.id, runId!));
        throw err;
      }
    }
    ...
    case "query_audit_log": {
      if (!user) throw new Error("Authentication required");
      if (!isGlobalAdmin(user) && !user.roles.includes("ops") && !user.roles.includes("compliance")) {
      ...
      const rows = await db.select().from(alloyAuditLogTable)
  ...
  switch (uri) {
    case "alloy://schema/platform": {
      return {
        uri,
        entities: [
          { name: "Workflow", table: "alloy_workflows", fields: ["id", "name", "type", "domain", "status", "priority", "requiresApproval", "steps", "inputs", "outputs", "orgId", "createdAt"] },
          { name: "WorkflowRun", table: "alloy_workflow_runs", fields: ["id", "workflowId", "state", "input", "output", "startedAt", "completedAt", "durationMs", "errorMessage"] },
          { name: "Artifact", table: "alloy_artifacts", fields: ["id", "workflowRunId", "workflowId", "orgId", "title", "artifactType", "content", "status", "approvalStatus"] },
          { name: "Approval", table: "alloy_approvals", fields: ["id", "workflowRunId", "artifactId", "status", "requestedFrom", "expiresAt"] },
          { name: "Decision", table: "alloy_decisions", fields: ["id", "title", "summary", "verdict", "confidence", "approvalStatus", "evidence", "agentId", "agentName"] },
          { name: "Skill", table: "alloy_skills", fields: ["id", "name", "slug", "version", "category", "description", "approvalClass", "isEnabled", "inputSchema", "outputSchema", "usageCount"] },
          { name: "SkillRun", table: "alloy_skill_runs", fields: ["id", "skillId", "workflowRunId", "agentId", "input", "output", "status", "durationMs", "errorMessage"] },
          { name: "AuditLog", table: "alloy_audit_log", fields: ["id", "orgId", "userId", "action", "resourceType", "resourceId", "before", "after", "correlationId", "createdAt"] },
        ],
        mimeType: "application/json",
      };
    }
  ...
  switch (method) {
    case "initialize": {
      return {
        ...
        instructions: "Alloy MCP Server — exposes the full SZL Holdings AI capability stack as MCP tools. Use alloy_skill_list to discover available skills, alloy_launch_workflow to start workflows, and domain-specific tools (vessels_*, firestorm_*, terra_*, lyte_*, inca_*) for domain intelligence.",
      };
    }
    ...
    case "tools/call": {
      ...
      if (!toolName) throw Object.assign(new Error("Tool name is required"), { code: JSON_RPC_ERRORS.INVALID_PARAMS });
      const tool = ALL_TOOLS.find(t => t.name === toolName);
      if (!tool) throw Object.assign(new Error(`Tool '${toolName}' not found`), { code: JSON_RPC_ERRORS.INVALID_PARAMS });
      const start = Date.now();
      try {
        const result = await executeTool(toolName, toolArgs, user);
        const latencyMs = Date.now() - start;
        await writeAuditLog({ userId: user?.id, toolName, args: toolArgs, result: JSON.stringify(result).slice(0, 500), latencyMs });
        return {
          content: [
            {
              type: "text",
        ...
        await writeAuditLog({ userId: user?.id, toolName, args: toolArgs, result: `ERROR: ${errorMsg}`, latencyMs });
        return {
          content: [
            {
              type: "text",
    ...
    case "resources/read": {
      ...
      if (!uri) throw Object.assign(new Error("URI is required"), { code: JSON_RPC_ERRORS.INVALID_PARAMS });
    ...
    case "prompts/get": {
      ...
      if (!promptName) throw Object.assign(new Error("Prompt name is required"), { code: JSON_RPC_ERRORS.INVALID_PARAMS });
      const prompt = MCP_PROMPTS.find(p => p.name === promptName);
      if (!prompt) throw Object.assign(new Error(`Prompt '${promptName}' not found`), { code: JSON_RPC_ERRORS.INVALID_PARAMS });
...
router.get("/mcp/resources", authMiddleware({ required: false }), (_req: Request, res: Response) => {
