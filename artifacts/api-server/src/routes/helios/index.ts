...
const proposals: CapabilityProposal[] = [...PROPOSALS];
const scanners: Scanner[] = [...SCANNERS];
...
router.get('/mythos/search', (req, res) => {
  ...
  const scored = MYTHOS_NODES.map((n) => {
...
router.get('/mythos/nodes/:id', (req, res) => {
  ...
  const neighborIds = new Set(
    neighborEdges.flatMap((e) => [e.source, e.target]).filter((id) => id !== node.id),
  );
...
router.post('/scanners/:id/run', (req, res) => {
  ...
  res.json({
    message: `Scanner "${scanners[idx].name}" triggered successfully. Results will appear in the feed within minutes.`,
  });
});
...
router.post('/mcp', (req, res) => {
  ...
  if (entity) {
    ...
    if (node) {
      ...
      res.json({
        ...
        node,
        edges,
        signals: SIGNALS.filter((s) => s.entities.includes(entity)),
      });
      return;
    }
  }
  ...
  if (query) {
    ...
    const relevantNodes = MYTHOS_NODES.filter(
