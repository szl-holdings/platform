...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const rows = await db
        .select()
        .from(readinessProgramsTable)
        .orderBy(desc(readinessProgramsTable.createdAt))
        .limit(limit)
        .offset(offset);
...
router.get('/readiness/programs/:id', authMiddleware(), async (req, res) => {
  try {
    ...
    const [row] = await db
      .select()
      .from(readinessProgramsTable)
      .where(eq(readinessProgramsTable.id, id));
    if (!row) {
      sendNotFound(res, 'Program');
      return;
    }
    sendSuccess(res, row);
  } catch (err) {
...
router.get('/readiness/programs/:id/dimensions', authMiddleware(), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(readinessDimensionsTable)
      .where(eq(readinessDimensionsTable.programId, id))
      .orderBy(readinessDimensionsTable.category);
...
router.patch(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(readinessDimensionsTable)
        .set({ ...req.body, updatedAt: new Date() })
        .where(eq(readinessDimensionsTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Dimension');
        return;
      }
      sendSuccess(res, row);
    } catch (err) {
...
router.delete(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .delete(readinessDimensionsTable)
        .where(eq(readinessDimensionsTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Dimension');
...
router.get('/readiness/dimensions/:id/scores', authMiddleware(), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(readinessScoreHistoryTable)
      .where(eq(readinessScoreHistoryTable.dimensionId, id))
      .orderBy(desc(readinessScoreHistoryTable.recordedAt));
    sendSuccess(res, rows);
  } catch (err) {
...
router.get('/readiness/programs/:id/milestones', authMiddleware(), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(readinessMilestonesTable)
      .where(eq(readinessMilestonesTable.programId, id))
      .orderBy(readinessMilestonesTable.dueDate);
    sendSuccess(res, rows);
  } catch (err) {
...
router.patch(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(readinessMilestonesTable)
        .set({ ...req.body, updatedAt: new Date() })
        .where(eq(readinessMilestonesTable.id, id))
        .returning();
      if (!row) {
...
router.delete(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .delete(readinessMilestonesTable)
        .where(eq(readinessMilestonesTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Milestone');
...
router.get('/readiness/programs/:id/risks', authMiddleware(), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(readinessRisksTable)
      .where(eq(readinessRisksTable.programId, id))
      .orderBy(desc(readinessRisksTable.createdAt));
    sendSuccess(res, rows);
  } catch (err) {
...
router.patch(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(readinessRisksTable)
        .set({ ...req.body, updatedAt: new Date() })
        .where(eq(readinessRisksTable.id, id))
        .returning();
      if (!row) {
        sendNotFound(res, 'Risk');
        return;
      }
      sendSuccess(res, row);
    } catch (err) {
...
router.get('/readiness/programs/:id/alerts', authMiddleware(), async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(readinessAlertsTable)
      .where(eq(readinessAlertsTable.programId, id))
      .orderBy(desc(readinessAlertsTable.createdAt));
    sendSuccess(res, rows);
  } catch (err) {
...
router.patch(
  ...
  async (req, res) => {
    try {
      const id = parseIdParam(req.params.id);
      const [row] = await db
        .update(readinessAlertsTable)
        .set(req.body)
        .where(eq(readinessAlertsTable.id, id))
        .returning();
      if (!row) {
...
router.get('/readiness/executive-rollup', authMiddleware(), async (_req, res) => {
  try {
    ...
    const alerts = await db
      .select()
      .from(readinessAlertsTable)
      .where(eq(readinessAlertsTable.isRead, false));
...
const NIST_CSF_FRAMEWORK = {
  ...
  functions: [
    ...
    },
    {
      ...
      name: 'IDENTIFY',
      description:
        'Understand cybersecurity risks to systems, people, assets, data, and capabilities',
      categories: 5,
      subcategories: 21,
    ...
    },
    {
      id: 'DE',
      name: 'DETECT',
      description: 'Find and analyze possible cybersecurity incidents',
      categories: 2,
      subcategories: 7,
    ...
    },
    {
      id: 'RS',
      name: 'RESPOND',
      description: 'Actions regarding a detected cybersecurity incident',
      categories: 4,
      subcategories: 17,
      keyOutcomes: [
        'Incident Management',
...
export const REFERENCE_COMPLIANCE_CONTROLS = [
  {
    ...
    owner: 'CISO',
    evidence: 'Documented risk management strategy reviewed Q1 2026',
    priority: 'high',
  },
  {
    controlId: 'ID.AM-01',
    ...
    owner: 'IT Operations',
    evidence: 'Asset inventory updated, 98% coverage',
    priority: 'high',
  },
  {
    controlId: 'ID.RA-01',
    ...
    owner: 'Risk Team',
    evidence: 'Annual risk assessment completed, quarterly updates missing',
    priority: 'medium',
  },
  {
    controlId: 'PR.AA-01',
    ...
    owner: 'IAM Team',
    evidence: 'MFA enforced, PAM deployed, 100% critical accounts covered',
    priority: 'critical',
  },
  {
    controlId: 'PR.DS-01',
    ...
    owner: 'Data Team',
    evidence: 'Data classification complete, encryption at rest 89% coverage',
    priority: 'high',
  },
  {
    controlId: 'DE.CM-01',
    ...
    status: 'compliant',
    score: 91,
    lastAssessed: '2026-03-25',
    owner: 'SOC',
    evidence: '24/7 SIEM monitoring, automated alerting, 99.9% uptime',
    priority: 'critical',
  },
  {
    controlId: 'RS.MA-01',
    ...
    owner: 'IR Team',
    evidence: 'IRP tested quarterly, avg MTTR 4.2 hours',
    priority: 'high',
  },
  {
    controlId: 'RC.RP-01',
    function: 'RECOVER',
    category: 'Incident Recovery Plan',
    status: 'partial',
    score: 74,
    lastAssessed: '2026-02-28',
    owner: 'BCM Team',
    evidence: 'BCP documented, DR test completed annually (last: Jan 2026)',
    priority: 'medium',
  },
];
...
const REFERENCE_AUDIT_FINDINGS = [
  {
    id: 'AF-2026-001',
    severity: 'high',
    finding: 'Multi-factor authentication not enforced on 3 legacy VPN endpoints',
    standard: 'NIST CSF PR.AA-02',
    status: 'remediation_in_progress',
    dueDate: '2026-04-15',
    assignee: 'Network Operations',
    riskScore: 78,
    ciaCritical: ['confidentiality', 'access_control'],
  },
  {
    id: 'AF-2026-002',
    severity: 'medium',
    finding: 'Security awareness training completion rate at 84% — below 95% target',
    standard: 'NIST CSF PR.AT-01',
    status: 'open',
    dueDate: '2026-04-30',
  ...
  },
  {
    id: 'AF-2026-003',
    severity: 'critical',
    finding: '3rd-party vendor with access to CUI has not completed annual security review',
    standard: 'NIST CSF GV.SC-05',
    status: 'open',
    dueDate: '2026-04-01',
    assignee: 'Vendor Management',
    riskScore: 91,
    ciaCritical: ['confidentiality', 'integrity'],
  },
  {
    id: 'AF-2026-004',
    severity: 'low',
    finding: 'Backup restoration testing documentation not current (last test: July 2025)',
    standard: 'NIST CSF RC.RP-02',
    status: 'open',
    dueDate: '2026-05-31',
    assignee: 'IT Operations',
    riskScore: 34,
    ciaCritical: ['availability'],
  },
  {
    id: 'AF-2026-005',
    severity: 'medium',
    finding: 'Privileged access review not completed within 90-day window for 12 accounts',
    standard: 'NIST CSF PR.AA-05',
    status: 'remediation_in_progress',
    dueDate: '2026-04-10',
    assignee: 'IAM Team',
    riskScore: 61,
...
router.get(
  '/readiness/live/nist-framework',
  readinessLiveRateLimit,
  authMiddleware(),
  async (_req, res) => {
    try {
      sendSuccess(res, {
      ...
      handleRouteError(res, err, 'Failed to fetch NIST framework');
    }
  },
);

router.get(
  '/readiness/live/controls',
  readinessLiveRateLimit,
  authMiddleware(),
  validateQuery(listQuerySchema),
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to fetch compliance controls');
    }
  },
);

router.get(
  '/readiness/live/audit-findings',
  readinessLiveRateLimit,
  authMiddleware(),
  validateQuery(listQuerySchema),
  async (req, res) => {
    try {
      ...
      sendSuccess(res, {
        ...
        overallRiskRating:
          critical > 0 ? 'CRITICAL' : high > 1 ? 'HIGH' : high > 0 ? 'MEDIUM' : 'LOW',
        fetchedAt: new Date().toISOString(),
      });
    } catch (err) {
      handleRouteError(res, err, 'Failed to fetch audit findings');
    }
  },
);

router.get(
  '/readiness/live/framework-mappings',
  readinessLiveRateLimit,
  authMiddleware(),
  async (_req, res) => {
    try {
      sendSuccess(res, {
        ...
        overallCompliance: Math.round(
          Object.values(REFERENCE_FRAMEWORK_MAPPINGS).reduce((s, f) => s + f.complianceScore, 0) /
            Object.keys(REFERENCE_FRAMEWORK_MAPPINGS).length,
        ),
        fetchedAt: new Date().toISOString(),
      });
    } catch (err) {
      handleRouteError(res, err, 'Failed to fetch framework mappings');
    }
  },
);

router.get(
  '/readiness/live/risk-posture',
  readinessLiveRateLimit,
  authMiddleware(),
  async (_req, res) => {
    try {
      const openFindings = REFERENCE_AUDIT_FINDINGS.filter((f) => f.status === 'open').length;
      const criticalFindings = REFERENCE_AUDIT_FINDINGS.filter(
        (f) => f.severity === 'critical',
      ).length;
      const avgControlScore =
        REFERENCE_COMPLIANCE_CONTROLS.reduce((s, c) => s + c.score, 0) /
        REFERENCE_COMPLIANCE_CONTROLS.length;
      const compositeRisk =
        100 -
        Math.round(
          avgControlScore * 0.5 +
      ...
      handleRouteError(res, err, 'Failed to fetch risk posture');
    }
  },
);
