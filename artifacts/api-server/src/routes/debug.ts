...
router.get("/debug/sentry-test", authMiddleware(), (req: Request, res: Response) => {
  ...
  if (isProduction) {
    res.status(403).json({ error: "Test error endpoint is only available in non-production environments" });
    return;
  }

  const testError = new Error(`[Sentry Test] Deliberate test error triggered from /api/debug/sentry-test at ${new Date().toISOString()}`);
  testError.name = "SentryTestError";

  if (Sentry.isInitialized()) {
    Sentry.withScope(scope => {
      scope.setTag("test", "true");
      scope.setTag("triggered_by", (req as Request & { user?: { id?: string } }).user?.id ?? "anonymous");
    ...
    res.status(200).json({
      ...
      instructions: "Check your Sentry project dashboard — the error should appear within ~60 seconds.",
    });
  } else {
...
router.get("/debug/integrations", authMiddleware(), (_req: Request, res: Response) => {
  const isProduction = process.env.NODE_ENV === "production";
  if (isProduction) {
    res.status(403).json({ error: "Integration status endpoint is only available in non-production environments" });
    return;
  }
  ...
  res.json({
    ...
    integrations: {
      stripe: {
        ...
        mode: process.env.STRIPE_SECRET_KEY?.startsWith("sk_live") ? "live" : process.env.STRIPE_SECRET_KEY ? "test" : "not_configured",
      ...
      google_maps: {
        ...
        usage: "satellite map proxy (/api/maps/static), geocoding (/api/maps/geocode), server-side only (key never exposed to browser)",
        static_map_endpoint: "/api/maps/static?center=LAT,LNG&zoom=15&size=600x400&maptype=satellite",
        geocode_endpoint: "/api/maps/geocode?address=ADDRESS",
        frontend_integration: "terra property walkthrough shows satellite imagery via /api/maps/static proxy",
      },
      mapbox: {
