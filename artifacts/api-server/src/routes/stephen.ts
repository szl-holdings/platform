...
const ListStephenContentBlocksQueryParams = z.object({
  type: z.enum(CONTENT_BLOCK_TYPES).optional(),
});
...
import {
  listQuerySchema,
  stephenBookingRequestsQuerySchema,
  validateBody,
  validateQuery,
} from '../lib/validation';
...
router.get('/stephen/contacts', authMiddleware(), requireRole('ops'), async (_req, res) => {
  try {
    const contacts = await db
      .select()
      .from(stephenSiteContactsTable)
      .orderBy(desc(stephenSiteContactsTable.createdAt));
  ...
  async (req, res) => {
    try {
      ...
      const [contact] = await db
        .insert(stephenSiteContactsTable)
        .values({
          name: name.trim(),
          email: email.trim(),
...
router.get('/stephen/testimonials', authMiddleware(), async (_req, res) => {
  try {
    const testimonials = await db
      .select()
      .from(stephenSiteTestimonialsTable)
      .orderBy(desc(stephenSiteTestimonialsTable.createdAt));
...
router.get('/stephen/case-studies', authMiddleware(), async (_req, res) => {
  try {
    const studies = await db
      .select()
      .from(stephenSiteCaseStudiesTable)
      .orderBy(desc(stephenSiteCaseStudiesTable.createdAt));
...
router.get('/stephen/case-studies/:slug', authMiddleware(), async (req, res) => {
  try {
    ...
    const [study] = await db
      .select()
      .from(stephenSiteCaseStudiesTable)
      .where(eq(stephenSiteCaseStudiesTable.slug, slug));
    if (!study) {
...
router.post(
  '/stephen/portfolio-case-studies',
  validateBody(
    bodyShape({
...
router.get('/stephen/portfolio-case-studies/:slug', async (req, res) => {
  try {
    const { slug } = GetStephenPortfolioCaseStudyParams.parse({
      slug: String(String(req.params.slug)),
    });
    const [study] = await db
      .select()
      .from(stephenCaseStudiesTable)
...
router.patch(
  '/stephen/portfolio-case-studies/:slug',
  validateBody(
    bodyShape({
      client: z.unknown().optional(),
      content: z.unknown().optional(),
  ...
  async (req, res) => {
    try {
      const { slug } = UpdateStephenPortfolioCaseStudyParams.parse({
        slug: String(String(req.params.slug)),
      });
...
router.delete(
  ...
  async (req, res) => {
    try {
      const { slug } = DeleteStephenPortfolioCaseStudyParams.parse({
        slug: String(String(req.params.slug)),
      });
      const [deleted] = await db
...
router.get(
  '/stephen/booking-requests',
  authMiddleware(),
  requireRole('ops'),
  validateQuery(stephenBookingRequestsQuerySchema),
  async (req, res) => {
    try {
          ...
          .then((result) => {
            if (!result.success) {
              logger.warn(
                ...
                '[email] Stephen status update delivery failed',
              );
            }
          })
          .catch((err) => {
        ...
        .then((results) => {
          for (const r of results) {
            if (r.status === 'rejected') {
              ...
              logger.warn(
                { error: r.value.error },
                '[email] Stephen booking email delivery failed',
              );
            }
          }
        })
...
router.get('/stephen/ecosystem-status', async (_req, res) => {
  ...
  const platformDefs = [
    ...
    { name: 'Counsel', slug: 'alloy', description: 'Execution Fabric' },
    ...
    try {
      ...
      Promise.allSettled([
        db.insert(alloySignals).values({
          ...
          status: 'raw',
          dedupeKey: `design-partner-${body.email}-${body.company}`
            .toLowerCase()
            .replace(/[^a-z0-9-]/g, '-'),
        }),
        sendEmail({
