...
router.get(
  '/alloy/cognitive/calibration/:agentId',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
      if (!req.user) {
...
router.get(
  '/alloy/cognitive/evals/calibrations',
  authMiddleware(),
  async (req: Request, res: Response) => {
    try {
      if (!req.user || !isElevatedUser(req.user)) {
...
router.get(
  '/alloy/cognitive/corrections/:agentId',
  authMiddleware(),
  validateQuery(listQuerySchema),
  async (req: Request, res: Response) => {
    try {
