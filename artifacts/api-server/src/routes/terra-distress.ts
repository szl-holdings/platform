...
router.get("/terra/distress/search", authMiddleware({ required: false }), validateQuery(listQuerySchema), async (req, res) => {
  try {
    ...
    const str = (v: unknown): string | undefined => typeof v === "string" ? v : Array.isArray(v) ? String(v[0]) : undefined;
...
router.get("/terra/distress/property/:id", authMiddleware({ required: false }), async (req, res) => {
  try {
...
router.post(
  ...
  async (req, res) => {
    try {
      if (!req.file) {
        res.status(400).json({ error: "No CSV file uploaded. Use multipart/form-data with field name 'file'." });
        return;
      }
      ...
      await auditLog("csv_upload_started", "terra_distress", undefined, {
        sourceLabel,
      ...
      await auditLog("csv_upload_completed", "terra_distress", String(result.runId), {
        sourceLabel,
    ...
    } catch (err) { handleRouteError(res, err, "CSV ingestion failed"); }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      if (rawSources !== undefined) {
        const invalid = rawSources.filter(s => !VALID_SOURCE_KEYS.has(s as typeof ALL_SOURCES[number]));
        if (invalid.length > 0) {
...
router.get(
  ...
  async (req, res) => {
    ...
    } catch (err) { handleRouteError(res, err, "Failed to fetch ingestion stats"); }
  }
);
