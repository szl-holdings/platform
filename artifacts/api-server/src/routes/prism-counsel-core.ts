...
router.post(
  ...
  async (req, res) => {
    try {
      const orgId = requireAuth(req, res);
      if (!orgId) return;
      const { title, matterType, status, caseNumber, jurisdiction, courtName, notes } =
        req.body as z.infer<typeof matterCreateSchema>;
      const [matter] = await db
      ...
      setImmediate(() => {
        void ingestPrismMatter(
          ...
          logger.error({ err: e }, '[prism-counsel] ingestPrismMatter failed'),
        );
      });
...
router.get('/prism-counsel/matters/:id/twin', authMiddleware(), async (req, res) => {
  try {
    ...
    const recs = await db
      ...
      .where(
        and(
          eq(pcAiRecommendationsTable.matterId, matterId),
          eq(pcAiRecommendationsTable.status, 'pending'),
        ),
      )
      .limit(10);
...
router.get('/prism-counsel/matters/:id/copilot-drafts', authMiddleware(), async (req, res) => {
  try {
    ...
    try {
      ...
      drafts = await db
        .select()
        .from(pcCopilotDraftsTable)
        .where(eq(pcCopilotDraftsTable.matterId, matterId))
        .orderBy(desc(pcCopilotDraftsTable.createdAt))
        .limit(20);
    } catch {
      drafts = [];
    }
    ...
    try {
      ...
      if (status) {
        items = await db
          ...
          .from(pcApprovalRequestsTable)
          .where(
            sql`matter_id IN (SELECT id FROM pc_matters WHERE org_id = ${orgId}) AND status = ${status}`,
          )
...
router.post(
  '/prism-counsel/approvals/:id/approve',
  authMiddleware(),
...
router.post(
  '/prism-counsel/approvals/:id/reject',
  authMiddleware(),
  validateBody(bodyShape({})),
  async (req, res) => {
    try {
  ...
  try {
    ...
    try {
      ...
      const [latest] = await db
        .select()
        .from(pcDashboardSnapshotsTable)
        .where(
          and(
            eq(pcDashboardSnapshotsTable.orgId, orgId),
            eq(pcDashboardSnapshotsTable.dashboardType, dashType as never),
          ),
        )
        .orderBy(desc(pcDashboardSnapshotsTable.computedAt))
        .limit(1);
...
router.get('/prism-counsel/admin/tenant-config', authMiddleware(), async (req, res) => {
  try {
    ...
    try {
      ...
      const [cfg] = await db
        .select()
        .from(pcTenantConfigTable)
        .where(eq(pcTenantConfigTable.orgId, orgId));
      config = cfg ?? null;
    } catch {
      config = null;
    }
...
router.get('/prism-counsel/admin/incidents', authMiddleware(), async (req, res) => {
  try {
    ...
    try {
      ...
      incidents = await db
        .select()
        .from(pcIncidentsTable)
        .where(eq(pcIncidentsTable.orgId, orgId))
        .orderBy(desc(pcIncidentsTable.detectedAt))
        .limit(50);
    } catch {
      incidents = [];
    }
...
router.get('/prism-counsel/admin/onboarding', authMiddleware(), async (req, res) => {
  try {
    ...
    try {
      ...
      checklist = await db
        .select()
        .from(pcOnboardingChecklistTable)
...
router.get('/prism-counsel/matters/:id/contradictions', authMiddleware(), async (req, res) => {
  try {
    ...
    try {
      ...
      contradictions = await db
        .select()
        .from(pcContradictionPanelTable)
        .where(eq(pcContradictionPanelTable.matterId, matterId))
        .orderBy(desc(pcContradictionPanelTable.createdAt))
        .limit(50);
    } catch {
      contradictions = [];
    }
...
router.get('/prism-counsel/operational-flows', authMiddleware(), async (req, res) => {
  try {
    ...
    try {
      ...
      flows = await db
        .select()
        .from(pcOperationalFlowRunsTable)
        .where(eq(pcOperationalFlowRunsTable.orgId, orgId))
        .orderBy(desc(pcOperationalFlowRunsTable.startedAt))
        .limit(50);
    } catch {
      flows = [];
    }
...
router.get('/prism-counsel/signal-forge/runs', authMiddleware(), async (req, res) => {
  try {
    ...
    try {
      ...
      runs = await db
        .select()
        .from(pcSignalForgeRunsTable)
        .where(eq(pcSignalForgeRunsTable.orgId, orgId))
        .orderBy(desc(pcSignalForgeRunsTable.startedAt))
        .limit(30);
    } catch {
      runs = [];
    }
...
router.get('/prism-counsel/matters/:id/audit-packets', authMiddleware(), async (req, res) => {
  try {
    ...
    try {
      ...
      packets = await db
        .select()
        .from(pcAuditPacketsTable)
        .where(eq(pcAuditPacketsTable.matterId, matterId))
        .orderBy(desc(pcAuditPacketsTable.generatedAt))
        .limit(20);
    } catch {
      packets = [];
    }
