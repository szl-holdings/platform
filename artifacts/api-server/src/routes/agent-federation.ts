...
federationRouter.post('/federation/agents/:agentId/chat', authMiddleware({ required: false }), async (req, res) => {
  ...
  if (!parseResult.success) {
    res
      .status(400)
      .json({ error: 'Validation failed', details: parseResult.error.flatten().fieldErrors });
    return;
  }
...
federationRouter.post(
  ...
  async (req, res) => {
    ...
    const {
      ...
      requesterUrl,
      priority = 'normal',
    } = req.body as z.infer<typeof federationDelegateSchema>;

    const agent = AGENT_REGISTRY.find((a) => a.id === targetAgentId);
    if (!agent) {
      res
        .status(404)
        .json({ error: 'Target agent not found', available: AGENT_REGISTRY.map((a) => a.id) });
      return;
    }

    const delegationId = `del_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    logger.info(
      { delegationId, targetAgentId, requesterName, priority },
      '[federation] A2A delegation received',
    );
