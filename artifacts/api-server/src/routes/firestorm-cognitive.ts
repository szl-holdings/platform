...
function makeProvenance(source: string, verifiedBy = 'CONSTELLATION Engine', traceRef?: string) {
  const id =
    traceRef ??
    `trace-${Date.now().toString(36)}-${deterministicHash(source + Date.now())
      .toString(36)
      .slice(0, 8)}`;
  return {
    source,
  ...
  async (_req, res) => {
    try {
      ...
      const nodes: Array<{
        id: string;
        label: string;
        type: 'asset' | 'identity' | 'control' | 'incident' | 'actor';
        severity: string;
        x: number;
        y: number;
        compromised: boolean;
      ...
      const IDENTITY_NODES = [
        {
          id: 'identity-jsmith',
          label: 'j.smith@corp.com',
          severity: 'critical',
          x: 600,
          y: 120,
          technique: 'T1078',
          evidence: ['AD:jsmith', 'anomaly:impossible-travel'],
        },
        {
          id: 'identity-svc-admin',
          label: 'admin.svc@corp.com',
          severity: 'high',
          x: 600,
          y: 240,
          technique: 'T1078.002',
          evidence: ['AD:admin.svc', 'anomaly:credential-stuffing'],
        },
        {
          id: 'identity-devops',
          label: 'devops.svc@corp.com',
          severity: 'medium',
          x: 600,
          y: 360,
          technique: 'T1078.004',
          evidence: ['AD:devops.svc'],
        },
      ];
      IDENTITY_NODES.forEach((identity) => {
        nodes.push({
          ...identity,
          type: 'identity',
          compromised: identity.severity === 'critical' || identity.severity === 'high',
          provenance: makeProvenance('Identity Provider / UEBA'),
        });
      ...
      controls.slice(0, 4).forEach((ctrl, i) => {
        ...
        const isEffective =
          (ctrl.status as string) === 'pass' || (ctrl.status as string) === 'compliant';
        nodes.push({
          id,
          label: ctrl.controlName ?? `CTRL-${ctrl.id}`,
          type: 'control',
          severity: isEffective ? 'low' : 'high',
          x: 760 + i * 40,
          y: 160 + i * 80,
      ...
      const crownNode = {
        id: 'crown-jewel-findb',
        label: 'SQL-PROD-01 (Crown Jewel)',
        type: 'asset' as const,
        severity: 'critical',
        x: 900,
        y: 280,
      ...
      nodes
        .filter((n) => n.type === 'identity' && n.severity === 'critical')
        .forEach((n) => {
          edges.push({
            from: n.id,
            to: crownNode.id,
      ...
      logger.info({
        msg: 'Cognitive attack-path-graph served',
        nodes: nodes.length,
        edges: edges.length,
      });
    } catch (err) {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const IDENTITY_PROFILES: Record<
        string,
        {
          displayName: string;
          role: string;
      ...
      > = {
        'j.smith@corp.com': {
          ...
          riskScore: 97,
          groups: ['Finance-RW', 'ERP-Access', 'VPN-Users', 'SharePoint-Finance'],
          mfaEnabled: true,
          lastLogin: '2h ago (Moscow IP)',
          anomalyCount: 3,
        },
        'admin.svc@corp.com': {
        ...
        'devops.svc@corp.com': {
          ...
          role: 'CI/CD Account',
          riskScore: 68,
          groups: ['Azure-Contributor', 'Build-Agents', 'K8s-Deploy'],
          mfaEnabled: false,
          lastLogin: '1h ago',
          anomalyCount: 1,
        },
      ...
      const permissionGraph = {
        directPermissions: profile.groups.map((g) => ({
          group: g,
          permissions: ['read', 'write'].slice(0, g.includes('Admin') ? 2 : 1),
      ...
      const evidenceCitations = [
        {
          id: 'EV-001',
          type: 'AD-group-membership',
          description: `${identityId} is member of ${profile.groups[0]}`,
        ...
        },
        {
          ...
          description: `${profile.anomalyCount} UEBA anomalies in last 48h`,
          collectedAt: new Date(Date.now() - 7200_000).toISOString(),
          source: 'Sentinel UEBA',
        },
        {
          id: 'EV-003',
          type: 'login-event',
          description: `Last login: ${profile.lastLogin}`,
          collectedAt: new Date().toISOString(),
          source: 'SIEM / Azure AD Logs',
        },
      ...
      logger.info({
        msg: 'Cognitive identity-blast-radius served',
        identityId,
        reachableAssets: reachableAssets.length,
      });
    } catch (err) {
    ...
    try {
      ...
      const controlsWithEvidence = controls.map((ctrl, i) => {
        ...
        const evidenceItems = Array.from({ length: 2 + (i % 3) }, (_, j) => ({
          ...
          freshnessStatus:
            daysSinceCheck < 7 ? 'fresh' : daysSinceCheck < 30 ? 'aging' : 'stale-90d',
      ...
      const summary = {
        ...
        avgFreshnessScore: Math.round(
          controlsWithEvidence.reduce((a, c) => a + c.freshnessScore, 0) /
            (controlsWithEvidence.length || 1),
        ),
        staleEvidence: controlsWithEvidence.filter((c) =>
          c.evidenceItems.some((e) => e.freshnessStatus === 'stale-90d'),
        ).length,
      ...
      logger.info({
        msg: 'Cognitive control-evidence-graph served',
        controls: controlsWithEvidence.length,
      });
    } catch (err) {
    ...
    try {
      ...
      if (!incident) {
        sendSuccess(res, {
          incident: null,
          chain: [],
          provenance: makeProvenance('Cognitive Runtime'),
          fetchedAt: new Date().toISOString(),
        });
        return;
      }
      ...
      const chainEvents = [
          ...
          citations: [
            {
              ...
              ref: 'log:AzureAD/SignInLogs/2025-03',
              confidence: 98,
            },
            {
              id: 'CIT-002',
              source: 'Sentinel Analytics Rule',
              ref: 'rule:R-1042-impossible-travel',
              confidence: 95,
            },
          ],
          mitreTag: 'T1078',
        ...
        },
        {
          ...
          citations: [
            {
              id: 'CIT-003',
              source: 'TAXII 2.1 / MISP Feed',
              ref: 'indicator--e8098b1e-4f3c-4d7f-a54a-1c97d41fa432',
              confidence: 97,
            },
            {
              id: 'CIT-004',
              source: 'Mandiant Threat Intel',
              ref: 'report:APT29-Q1-2025',
              confidence: 92,
            },
            ...(alerts[0]
              ? [
          ...
          citations: [
            {
              ...
              ref: 'edr:WORKSTATION-142:proc-2847',
              confidence: 99,
            },
            {
              id: 'CIT-007',
              source: 'Network Flow Analysis',
              ref: 'netflow:192.168.1.45→10.0.0.3:445',
              confidence: 96,
            },
          ],
          mitreTag: 'T1021.002',
        ...
        },
        {
          ...
          citations: [
            {
              id: 'CIT-008',
              source: 'EDR Memory Forensics',
              ref: 'edr:DC-PROD-03:lsass-dump-evidence',
              confidence: 99,
            },
            {
              id: 'CIT-009',
              source: 'Sysmon Event 10',
              ref: 'sysmon:DC-PROD-03:evt10:lsass',
              confidence: 97,
            },
          ],
          mitreTag: 'T1003.001',
        ...
        },
        {
          ...
          citations: [
            {
              id: 'CIT-010',
              source: 'SOAR Execution Log',
              ref: `soar:EXEC-${incident.id}-PB002`,
              confidence: 100,
            },
            {
              id: 'CIT-011',
              source: 'Firewall ACL Audit',
              ref: 'fw:rule-BLOCK-WKS142-2025',
              confidence: 100,
            },
          ],
          mitreTag: null,
      ...
      sendSuccess(res, {
        ...
        summary: {
          ...
          avgConfidence: Math.round(
            chainEvents.flatMap((e) => e.citations).reduce((a, c) => a + c.confidence, 0) /
              chainEvents.flatMap((e) => e.citations).length,
          ),
          verifiedBy: 'CONSTELLATION Trace Engine',
      ...
      logger.info({
        msg: 'Cognitive incident-proof-chain served',
  ...
  async (_req, res) => {
    try {
      ...
      const BUSINESS_ENTITIES = [
        ...
        {
          id: 'biz-customer-data',
          name: 'Customer Data Assets',
          type: 'data',
          owner: 'CDO',
          annualRevenue: null,
          atRisk: 18_500_000,
      ...
      const impactMappings = incidents.slice(0, 6).map((inc, i) => {
        ...
        return {
          ...
          estimatedFinancialImpact:
            baseImpact + (deterministicHash(`${inc.id}-impact`) % Math.floor(baseImpact * 0.5)),
          estimatedDowntimeHours:
            severity === 'critical' ? 18 + i * 2 : severity === 'high' ? 6 + i : 1,
      ...
      const execNarrative = {
        ...
        totalEstimatedExposure:
          impactMappings.reduce((a, m) => a + m.estimatedFinancialImpact, 0) +
          findingImpacts.reduce((a, f) => a + f.estimatedFineExposure, 0),
      ...
      logger.info({
        msg: 'Cognitive business-impact-map served',
        incidents: impactMappings.length,
        findings: findingImpacts.length,
      });
    } catch (err) {
