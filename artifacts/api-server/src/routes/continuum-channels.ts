...
function verifySlackSignature(req: Request): boolean {
  if (!SLACK_SIGNING_SECRET) {
    logger.warn(
      'continuum-channels: SLACK_SIGNING_SECRET not configured — rejecting inbound Slack request',
    );
    return false;
  }
...
const TRUST_LEVELS: Record<string, { skills: string[]; approvalClass: string }> = {
  ...
  elevated: {
    skills: ['research', 'analysis', 'report', 'notify', 'escalate'],
...
async function postToSlack(channel: string, text: string, blocks?: unknown[]): Promise<void> {
  if (!SLACK_BOT_TOKEN) {
    logger.warn('continuum-channels: SLACK_BOT_TOKEN not set — cannot post to Slack');
    return;
  }
  ...
  if (!res.ok) logger.warn({ status: res.status }, 'continuum-channels: Slack postMessage failed');
}

function detectSkillFromMessage(text: string): string | null {
  const lower = text.toLowerCase();
  if (lower.includes('research') || lower.includes('investigate') || lower.includes('find out'))
    return 'research';
  if (lower.includes('analyze') || lower.includes('analyse') || lower.includes('review'))
    return 'analysis';
  if (lower.includes('report') || lower.includes('summarize') || lower.includes('summary'))
    return 'report';
  if (lower.includes('notify') || lower.includes('alert') || lower.includes('send'))
    return 'notify';
...
router.post(
  '/continuum/channels/slack/webhook',
  validateBody(
    bodyShape({
      ...
      setImmediate(async () => {
        try {
          ...
          if (command) {
            ...
            if (command === '/continuum') {
              const skillRequested = detectSkillFromMessage(text) ?? 'research';

              const channelAllowedSkills: string[] = Array.isArray(config?.allowed_skills)
                ? config.allowed_skills
                : trustConfig.skills;
              const allowed =
                channelAllowedSkills.includes('*') || channelAllowedSkills.includes(skillRequested);
              if (!allowed) {
                await postToSlack(
                  channelId,
                  `This channel's trust level (*${trustLevel}*) does not allow the *${skillRequested}* skill. Contact your admin to adjust channel permissions.`,
                );
                await logChannelAction({
                  ...
                  outcome: 'blocked',
                  outcomeDetail: `Trust level ${trustLevel} insufficient — allowed: ${channelAllowedSkills.join(', ')}`,
                  trustLevel,
                });
                return;
              }
              ...
              if (requiresApproval) {
                    ...
                    elements: [
                      {
                        ...
                        style: 'primary',
                        value: `approve:${pendingId}`,
                        action_id: 'alloy_approve',
                      },
                      {
                        type: 'button',
                        text: { type: 'plain_text', text: ':x: Reject' },
                        style: 'danger',
                        value: `reject:${pendingId}`,
                        action_id: 'alloy_reject',
                      },
                    ],
                  },
                ];
                await postToSlack(
                  channelId,
                  `:hourglass: Continuum approval required for \`${skillRequested}\` skill`,
                  approvalBlocks,
                );
                await logChannelAction({
                  channelType: 'slack',
                  ...
                  outcomeDetail: `pendingId=${pendingId}, approvalClass=${approvalClass}`,
                  trustLevel,
                });
                return;
              }
              ...
              try {
                const aiResult = await services.ai.chatCompletion(
                  [
                    {
                      role: 'system',
                      content: `You are Continuum, an AI operations assistant. Process this ${skillRequested} request from a trusted Slack admin channel and provide a concise, actionable response. Keep it under 500 words.`,
                    },
              ...
              await postToSlack(channelId, `*Continuum Result — ${skillRequested}*\n${result}`);
              await logChannelAction({
                channelType: 'slack',
                channelId,
                userId,
                ...
                outcome: 'completed',
                outcomeDetail: result.slice(0, 200),
                trustLevel,
              });
            }
            return;
          ...
          if (body.type === 'event_callback' && event?.type === 'message') {
            ...
            if (botId) return;
            if (!text.toLowerCase().includes('continuum')) return;
            ...
            await logChannelAction({
              ...
              userId,
              message: text,
              skillInvoked: skillDetected ?? undefined,
              outcome: 'received',
              trustLevel,
            });
          }
        } catch (asyncErr) {
          logger.error({ err: asyncErr }, 'continuum-channels: async Slack processing failed');
        }
      });
    } catch (err) {
...
router.post(
  '/continuum/channels/slack/interactive',
  validateBody(
    bodyShape({
      ...
      setImmediate(async () => {
        try {
          ...
          const existing = await pool.query(
            `SELECT * FROM alloy_pending_approvals_chat WHERE id = $1 AND status = 'pending'`,
            [approvalId],
          );
          ...
          if (channelId) {
            await postToSlack(
              channelId,
              `${emoji} *Approval ${decision.toUpperCase()}* by ${reviewerName}\n> ${approval.action_description}`,
            );
          }
          ...
          await logChannelAction({
            channelType: 'slack',
            channelId: channelId ?? approval.channel_id,
            userId: reviewerId,
            userName: reviewerName,
            workflowId: approval.workflow_id,
            approvalStatus: decision,
            outcome: 'interactive_approval_decided',
            outcomeDetail: `approvalId=${approvalId}`,
          });
            ...
            try {
              const aiResult = await services.ai.chatCompletion(
                [
                  {
                    role: 'system',
                    content: `You are Continuum, an AI operations assistant. Process this approved ${skill} request and provide a concise, actionable response. Keep it under 500 words.`,
                  },
              ...
              if (channelId) {
                await postToSlack(channelId, `*Continuum Result — ${skill}*\n${aiResult.content}`);
              }
            } catch (execErr) {
              logger.warn({ err: execErr }, 'continuum-channels: post-approval execution failed');
            }
          }
        } catch (asyncErr) {
          logger.error({ err: asyncErr }, 'continuum-channels: interactive callback processing failed');
        }
      });
    } catch (err) {
...
router.get(
  '/continuum/channels/config',
  authMiddleware(),
  requireRole('ops', 'admin', 'super_admin'),
  async (_req: Request, res: Response) => {
    try {
      const result = await pool.query(
        `SELECT * FROM alloy_channel_configs ORDER BY created_at DESC`,
      );
...
router.patch(
  '/continuum/channels/config/:channelId',
  authMiddleware(),
  requireRole('admin', 'super_admin'),
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (trustLevel !== undefined) {
        updates.push(`trust_level = $${idx++}`);
        params.push(trustLevel);
      }
      if (allowedSkills !== undefined) {
        updates.push(`allowed_skills = $${idx++}`);
        params.push(JSON.stringify(allowedSkills));
      }
      ...
      if (approvalClass !== undefined) {
        updates.push(`approval_class = $${idx++}`);
        params.push(approvalClass);
      }
      params.push(channelId);
      await pool.query(
        `UPDATE alloy_channel_configs SET ${updates.join(', ')} WHERE channel_id = $${idx}`,
        params,
      );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      let idx = 1;
      if (channelType) {
        q += ` AND channel_type = $${idx++}`;
        params.push(channelType);
      }
      if (channelId) {
        q += ` AND channel_id = $${idx++}`;
        params.push(channelId);
      }
...
router.post(
  '/continuum/channels/approvals/:id/decide',
  authMiddleware(),
  requireRole('ops', 'admin', 'super_admin'),
  validateBody(
    bodyShape({
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const existing = await pool.query(
        `SELECT * FROM alloy_pending_approvals_chat WHERE id = $1`,
        [id],
      );
      if (!existing.rows[0]) {
      ...
      if (approval.channel_id && approval.channel_type === 'slack') {
        ...
        await postToSlack(
          approval.channel_id,
          `${emoji} *Approval ${decision.toUpperCase()}* by ${req.user?.displayName ?? 'admin'}\n${approval.action_description}${note ? `\n_Note: ${note}_` : ''}`,
        );
      }
      await logChannelAction({
        channelType: approval.channel_type,
...
router.get(
  '/continuum/channels/trust-levels',
  authMiddleware(),
  async (_req: Request, res: Response) => {
    try {
      sendSuccess(res, {
        trustLevels: Object.entries(TRUST_LEVELS).map(([key, val]) => ({
          ...
          description:
            key === 'admin'
              ...
              : key === 'elevated'
                ? 'Extended skills with elevated approval'
                : key === 'standard'
                  ? 'Research and analysis skills'
                  : 'Read-only, no skill execution',
        })),
      });
    } catch (err) {
