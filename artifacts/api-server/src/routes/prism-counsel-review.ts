...
import { and, desc, eq, gte, isNull, not, or, sql } from 'drizzle-orm';
  ...
  try {
    const [all, byState, slaBreaches] = await Promise.all([
      ...
      db
        ...
        .groupBy(
          pcManagedReviewItemsTable.lifecycleState,
          pcManagedReviewItemsTable.reviewWorkType,
        ),
      db
        .select()
        .from(pcManagedReviewItemsTable)
    ...
    const highPriority = all.filter((i) => i.priorityScore >= 0.7).length;
    const avgAge =
      all.length > 0
        ? all.reduce((sum, i) => sum + (Date.now() - new Date(i.createdAt).getTime()), 0) /
          all.length /
          3600000
        : 0;
    ...
    res.json({
      ...
      stateBreakdown,
      typeBreakdown,
      topItems: all.sort((a, b) => b.priorityScore - a.priorityScore).slice(0, 10),
    });
  } catch (err: any) {
  ...
  try {
    ...
    const [item, notes, assignments] = await Promise.all([
      db
        ...
        .where(
          and(
            eq(pcManagedReviewItemsTable.id, id),
            eq(pcManagedReviewItemsTable.orgId, getOrgId(req)),
          ),
        )
        .limit(1),
...
router.post(
  '/review-desk/items',
  validateBody(ReviewItemCreateSchema),
  async (req: Request, res: Response) => {
    try {
      const {
        matterId,
        ...
        whatItUnblocks,
        deadlineRiskScore,
        settlementFrictionScore,
        insurerPressureScore,
        contradictionSeverityScore,
        lowConfidenceScore,
        exportSendDependencyScore,
        workUnblockedScore,
        partnerUrgencyScore,
        clientFacingImpactScore,
        recoveryLienDependencyScore,
        slaHours,
        dueBy,
      ...
      const [item] = await db
        .insert(pcManagedReviewItemsTable)
        .values({
          orgId: getOrgId(req),
          matterId,
          reviewWorkType,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!allowed.includes(toState)) {
        return void res
          .status(400)
          .json({ error: `Invalid transition from ${item.lifecycleState} to ${toState}`, allowed });
      }
      ...
      const [updated] = await db
        .update(pcManagedReviewItemsTable)
        .set(updateData)
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!allowed.includes('approved')) {
        return void res
          .status(400)
          .json({ error: `Cannot approve from state: ${item.lifecycleState}` });
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await db.insert(pcManagedReviewNotesTable).values({
        ...
        reviewItemId: id,
        noteType: 'escalation',
        content: reason ?? `Escalated to ${escalateTo}`,
        authorId: actorId,
      });
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await db
        .update(pcManagedReviewAssignmentsTable)
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await db.insert(pcManagedReviewNotesTable).values({
        orgId: getOrgId(req),
        reviewItemId: id,
        noteType: 'missing_support_request',
        content: request,
        authorId: actorId,
      });
      ...
      await emitReviewAudit({
        ...
        reviewItemId: id,
        actorId,
        action: 'review_support_requested',
        fromState: item.lifecycleState,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const avgAge =
        all.length > 0
          ? all.reduce((s, i) => s + (Date.now() - new Date(i.createdAt).getTime()), 0) /
            all.length /
            3600000
          : 0;
      ...
      const contradictionItems = closed.filter(
        (i) => i.reviewWorkType === 'contradiction_review' && i.reviewedAt,
      );
      const avgContradictionHours =
        contradictionItems.length > 0
          ? contradictionItems.reduce(
              (s, i) => s + (new Date(i.reviewedAt!).getTime() - new Date(i.createdAt).getTime()),
              0,
            ) /
      ...
      const lowConfItems = closed.filter(
        (i) => i.reviewWorkType === 'low_confidence_extraction_review' && i.reviewedAt,
      );
      const avgLowConfHours =
      ...
      const avgExportTurnaroundHours =
        exportItems.length > 0
          ? exportItems.reduce(
              (s, i) => s + (new Date(i.exportedAt!).getTime() - new Date(i.approvedAt!).getTime()),
              0,
            ) /
            exportItems.length /
            3600000
      ...
      const avgApprovalWaitHours =
        approvalItems.length > 0
          ? approvalItems.reduce(
              (s, i) => s + (new Date(i.approvedAt!).getTime() - new Date(i.assignedAt!).getTime()),
              0,
            ) /
      ...
      res.json({
        ...
        avgContradictionResolutionHours: avgContradictionHours
          ? Math.round(avgContradictionHours * 10) / 10
          : null,
        avgLowConfidenceResolutionHours: avgLowConfHours
          ? Math.round(avgLowConfHours * 10) / 10
          : null,
        avgExportReadyTurnaroundHours: avgExportTurnaroundHours
          ? Math.round(avgExportTurnaroundHours * 10) / 10
          : null,
        avgApprovalWaitTimeHours: avgApprovalWaitHours
          ? Math.round(avgApprovalWaitHours * 10) / 10
          : null,
      });
    } catch (err: any) {
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      all
        .filter((i) => i.lifecycleState !== 'closed')
        .forEach((i) => {
          backlogByType[i.reviewWorkType] = (backlogByType[i.reviewWorkType] ?? 0) + 1;
          backlogByState[i.lifecycleState] = (backlogByState[i.lifecycleState] ?? 0) + 1;
        });

      const contradictionBacklog = all.filter(
        (i) => i.reviewWorkType === 'contradiction_review' && i.lifecycleState !== 'closed',
      ).length;
      const lowConfBacklog = all.filter(
        (i) =>
          i.reviewWorkType === 'low_confidence_extraction_review' && i.lifecycleState !== 'closed',
      ).length;
      const failedPackets = all.filter(
        (i) => i.auditPacketRef !== null && i.lifecycleState === 'blocked',
      ).length;
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const needsAction = items.filter((i) =>
        ['new', 'triaged', 'assigned', 'in_review'].includes(i.lifecycleState),
      );
      const risky = items.filter((i) => i.priorityScore >= 0.7);
