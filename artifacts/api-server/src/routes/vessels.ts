...
import { type IRouter, type Request, type RequestHandler, Router } from 'express';
...
import {
  ...
  validateBody,
  validateQuery,
  vesselsResourceDeleteSchema,
  vesselsResourceMutationSchema,
} from '../lib/validation';
  ...
  try {
    ...
    const fleets = where
      ? await db
          ...
          .where(where)
          .orderBy(desc(vesselsFleetsTable.createdAt))
...
router.get(
  ...
  async (req: Request, res) => {
    try {
      ...
      if (
        vesselIdFilter !== undefined &&
        orgVesselIds !== null &&
        !orgVesselIds.includes(vesselIdFilter)
      ) {
      ...
      const events =
        effectiveVesselIds !== null
          ? await db
              .select()
              .from(vesselsEventsTable)
              .where(inArray(vesselsEventsTable.vesselId, effectiveVesselIds))
      ...
      handleRouteError(res, err, 'Failed to list vessel events');
    }
  },
);
    ...
    try {
      ...
      const workflows =
        effectiveVesselIds !== null
          ? await db
              .select()
              .from(vesselsCommandWorkflowsTable)
              .where(inArray(vesselsCommandWorkflowsTable.vesselId, effectiveVesselIds))
              .orderBy(desc(vesselsCommandWorkflowsTable.createdAt))
          : await db
              .select()
              .from(vesselsCommandWorkflowsTable)
              .orderBy(desc(vesselsCommandWorkflowsTable.createdAt));
      ...
      handleRouteError(res, err, 'Failed to list command workflows');
    }
  },
);
...
router.post(
  ...
  async (req: Request, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to create command workflow');
    }
  },
);
  ...
  async (req: Request, res) => {
    try {
      ...
      const [existing] = await db
        .select()
        .from(vesselsRoutesTable)
        .where(eq(vesselsRoutesTable.id, id));
      if (!existing) {
  ...
  async (req: Request, res) => {
    try {
      ...
      const rules = where
        ? await db
            .select()
            .from(vesselsAlertRulesTable)
            .where(where)
            .orderBy(desc(vesselsAlertRulesTable.createdAt))
        : await db
            .select()
            .from(vesselsAlertRulesTable)
            .orderBy(desc(vesselsAlertRulesTable.createdAt));
...
router.put(
  ...
  async (req: Request, res) => {
    try {
      const id = parseIdParam(req.params.id);
      // Strip orgId — tenant context is authoritative; clients must not reassign tenancy
      const { orgId: _discardOrgId, ...data } = insertVesselAlertRuleSchema
        .partial()
        .parse(req.body);
      const condition =
        req.tenantOrgId !== undefined
    ...
    const alerts =
      orgVesselIds !== null
        ? await db
            ...
            .where(inArray(vesselsAlertsTable.vesselId, orgVesselIds))
  ...
  async (req: Request, res) => {
    try {
      const routeIdParam = req.query.routeId
        ? parseInt(req.query.routeId as string, 10)
        : undefined;
      ...
      if (routeIdParam !== undefined) {
        const [route] = await db
          .select()
          .from(vesselsRoutesTable)
          .where(eq(vesselsRoutesTable.id, routeIdParam));
        if (!route) {
          sendSuccess(res, []);
          return;
        }
        ...
        const snapshots = await db
          .select()
          .from(vesselsWeatherSnapshotsTable)
          .where(eq(vesselsWeatherSnapshotsTable.routeId, routeIdParam))
          .orderBy(desc(vesselsWeatherSnapshotsTable.recordedAt));
      ...
      if (orgVesselIds !== null) {
        ...
        const snapshots = await db
          .select()
          .from(vesselsWeatherSnapshotsTable)
          .where(inArray(vesselsWeatherSnapshotsTable.routeId, orgRouteIds))
          .orderBy(desc(vesselsWeatherSnapshotsTable.recordedAt));
        sendSuccess(res, snapshots);
      } else {
        const snapshots = await db
    ...
    try {
      ...
      const simulations =
        orgVesselIds !== null
          ? await db
              .select()
              .from(vesselsSimulationsTable)
              .where(inArray(vesselsSimulationsTable.vesselId, orgVesselIds))
              .orderBy(desc(vesselsSimulationsTable.createdAt))
          : await db
              .select()
              .from(vesselsSimulationsTable)
              .orderBy(desc(vesselsSimulationsTable.createdAt));
...
router.post(
  ...
  authMiddleware(),
  tenantScope(),
  requireRole('ops', 'exec', 'admin', 'analyst'),
  validateBody(vesselsResourceMutationSchema),
  async (req: Request, res) => {
    try {
      ...
      const [simulation] = await db
        .insert(vesselsSimulationsTable)
        .values({
          ...data,
          status: 'running',
          startedAt: new Date(),
      ...
      setTimeout(async () => {
        try {
          ...
          await db
            .update(vesselsSimulationsTable)
            .set({
              status: 'completed',
  ...
  async (req: Request, res) => {
    try {
      ...
      const [simulation] = await db
        .select()
        .from(vesselsSimulationsTable)
        .where(eq(vesselsSimulationsTable.id, id));
      if (!simulation) {
        sendNotFound(res, 'Simulation');
        return;
      }
...
router.get(
  '/vessels/live/geopolitical-events',
  vesselsLiveLimit,
  authMiddleware(),
  tenantScope(),
  async (_req, res) => {
...
router.get(
  ...
  async (_req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to fetch port congestion data');
    }
  },
);

router.get(
  '/vessels/live/weather-marine',
  vesselsLiveLimit,
  authMiddleware(),
  tenantScope(),
  validateQuery(listQuerySchema),
...
router.get('/vessels/:id/events', authMiddleware(), tenantScope(), async (req: Request, res) => {
  try {
    ...
    const events = await db
      .select()
      .from(vesselsEventsTable)
      .where(eq(vesselsEventsTable.vesselId, id))
      .orderBy(desc(vesselsEventsTable.occurredAt));
  ...
  async (req: Request, res) => {
    try {
      ...
      if (!parseResult.success) {
        ...
        res
          .status(400)
          .json({ error: `Validation error: ${errors}`, issues: parseResult.error.issues });
        return;
      }
      ...
      const [existing] = await db
        .select()
        .from(vesselsEventsTable)
        .where(eq(vesselsEventsTable.id, id));
      if (!existing) {
        sendNotFound(res, 'Vessel event');
        return;
      }
      const vessel = await getVesselInOrg(existing.vesselId, req.tenantOrgId);
      if (!vessel) {
      ...
      handleRouteError(res, err, 'Failed to update vessel event');
    }
  },
);
  ...
  async (req: Request, res) => {
    try {
      ...
      const [existing] = await db
        .select()
        .from(vesselsCommandWorkflowsTable)
        .where(eq(vesselsCommandWorkflowsTable.id, id));
      if (!existing) {
        sendNotFound(res, 'Command workflow');
        return;
      }
      if (existing.vesselId) {
        const vessel = await getVesselInOrg(existing.vesselId, req.tenantOrgId);
      ...
      if (patch.notes !== undefined) updateData.notes = patch.notes;
      if (patch.assignedTo !== undefined) updateData.assignedTo = patch.assignedTo;
      if (patch.consequenceImpact !== undefined)
        updateData.consequenceImpact = patch.consequenceImpact;
      if (patch.status === 'completed') updateData.completedAt = new Date();
      const [workflow] = await db
        .update(vesselsCommandWorkflowsTable)
        .set(updateData)
        .where(eq(vesselsCommandWorkflowsTable.id, id))
        .returning();
      if (!workflow) {
      ...
      handleRouteError(res, err, 'Failed to update command workflow');
    }
  },
);
