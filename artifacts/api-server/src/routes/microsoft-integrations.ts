...
router.get(
  '/integrations/dynamics/entities',
  authMiddleware(),
    ...
    try {
      ...
      if (!enabled) {
        sendSuccess(res, {
          ...
          message:
            'Dynamics 365 sync is not enabled. Enable the dynamics365_sync_enabled feature flag.',
        });
        return;
      }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const mapped = signalMap[key] ?? {
        title: `Dynamics — ${entityName} ${messageName}`,
    ...
    try {
      ...
      if (!enabled) {
        res.status(200).json({
          success: false,
          reason:
            'Power Automate webhook integration is not enabled. Enable the power_automate_webhook_enabled feature flag.',
        });
        return;
      }
      ...
      const { action, workflowId, signalPayload, orgId } = req.body as {
  ...
  try {
    ...
    if (!enabled) {
      sendSuccess(res, {
        featureDisabled: true,
        message:
          'SharePoint SPFx integration is not enabled. Enable the sharepoint_spfx_enabled feature flag.',
        webParts: [],
      });
      return;
    }
...
router.get(
  '/integrations/sharepoint/health',
  authMiddleware(),
  requireRole('super_admin', 'ops'),
  async (_req, res) => {
    try {
