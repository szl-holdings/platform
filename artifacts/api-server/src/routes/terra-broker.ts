  ...
  async (req, res) => {
    try {
      ...
      if (q) {
        const q_ = `%${q}%`;
        conditions.push(
          or(ilike(terraPropertiesTable.address, q_), ilike(terraPropertiesTable.submarket, q_))!,
        );
      }
      ...
      const orderCol = (() => {
        switch (sort) {
          case 'price_asc':
            return asc(terraListingsTable.listPrice);
          case 'price_desc':
            return desc(terraListingsTable.listPrice);
          case 'dom_asc':
            return asc(terraListingsTable.daysOnMarket);
          case 'dom_desc':
            return desc(terraListingsTable.daysOnMarket);
          default:
            return desc(terraListingsTable.opportunityScore);
        }
      })();
      ...
      handleRouteError(res, err, 'Failed to fetch listings');
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!parsed.success) {
        res.status(400).json({ error: 'Invalid listing data', issues: parsed.error.issues });
        return;
      }
      ...
      handleRouteError(res, err, 'Failed to create listing');
    }
  },
);
...
router.get('/terra/broker/listings/:id', terraRateLimit, auth, async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch listing detail');
  }
});
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to update listing');
    }
  },
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      handleRouteError(res, err, 'Failed to delete listing');
    }
  },
);
    ...
    try {
      ...
      sendSuccess(res, {
        ...
        inquiries: inquiries.map((i) => ({
          ...
          propertyAddress:
            i.propertyAddress && i.propertyCity ? `${i.propertyAddress}, ${i.propertyCity}` : null,
        })),
        routingRules: [
          {
            rule: 'cash-buyer-priority',
            description: 'Cash buyers automatically scored +15 and escalated',
            active: true,
          },
          {
            rule: 'qualification-threshold',
            description: 'Score < 60 held for manual review before agent assignment',
            active: true,
          },
          {
            rule: 'specialty-match',
            description: 'Inquiries matched to agents by property type specialty',
            active: true,
          },
          {
            rule: 'workload-balance',
            description: 'Routing respects agent active inquiry cap of 20',
            active: true,
          },
        ],
        fetchedAt: new Date().toISOString(),
      ...
      handleRouteError(res, err, 'Failed to fetch inquiries');
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!parsed.success) {
        res.status(400).json({ error: 'Invalid inquiry data', issues: parsed.error.issues });
        return;
      }
  ...
  async (req, res) => {
    try {
      ...
      const orderCol = (() => {
        switch (sort) {
          case 'close_rate':
            return desc(terraAgentsTable.closeRatePct);
          case 'closed_ltm':
            return desc(terraAgentsTable.closedDealsLtm);
          case 'active_listings':
            return desc(terraAgentsTable.activeListings);
          default:
            return desc(terraAgentsTable.closeRatePct);
        }
      })();
      ...
      handleRouteError(res, err, 'Failed to fetch agent workload');
    }
  },
);
...
router.get('/terra/broker/brokerage', terraRateLimit, auth, async (req, res) => {
  try {
    ...
    handleRouteError(res, err, 'Failed to fetch brokerage visibility');
  }
});
...
router.get('/terra/broker/map', terraRateLimit, auth, async (req, res) => {
  try {
    ...
    const submarketGroups = listings.reduce<
      Record<string, { listings: number; totalPrice: number; totalInquiries: number }>
    >((acc, l) => {
    ...
    handleRouteError(res, err, 'Failed to fetch map payload');
  }
});
  ...
  async (req, res) => {
    try {
      ...
      const transactions = await db
        ...
        .leftJoin(
          terraPropertiesTable,
          eq(terraTransactionsTable.propertyId, terraPropertiesTable.id),
        )
      ...
      sendSuccess(res, {
        ...
        transactions: transactions.map((t) => ({
          ...
          address:
            t.propertyAddress && t.propertyCity ? `${t.propertyAddress}, ${t.propertyCity}` : null,
          type: t.propertyType,
      ...
      handleRouteError(res, err, 'Failed to fetch transactions');
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (minPrice !== undefined)
        conditions.push(gte(sql`${terraListingsTable.listPrice}::numeric`, sql`${minPrice}`));
      if (maxPrice !== undefined)
        conditions.push(lte(sql`${terraListingsTable.listPrice}::numeric`, sql`${maxPrice}`));
      if (minScore !== undefined)
        conditions.push(gte(terraListingsTable.opportunityScore, minScore));
      ...
      handleRouteError(res, err, 'Failed to execute search');
    }
  },
);
