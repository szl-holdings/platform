...
type HookRow = { id: string; domain: string; workflowId: string; workflowName: string; replayable: boolean; tenantId?: string; signalSnapshot: unknown[]; runSnapshot: Record<string, unknown>; snapshotAt: string; benchmarkMetrics?: Record<string, unknown> };
...
const HOOKS: HookRow[] = [
  { id: "h-a", domain: "aegis", workflowId: "wf-a", workflowName: "A", replayable: true, tenantId: "10", signalSnapshot: [], runSnapshot: {}, snapshotAt: new Date().toISOString() },
  { id: "h-b", domain: "aegis", workflowId: "wf-b", workflowName: "B", replayable: true, tenantId: "20", signalSnapshot: [], runSnapshot: {}, snapshotAt: new Date().toISOString() },
];
...
vi.mock("../lib/atlas-execution-engine.js", () => ({
  ...
  DOMAIN_WORKFLOWS: {
    "aegis-incident-response": { id: "aegis-incident-response", name: "Aegis", domain: "aegis", steps: [] },
  },
  ingestSignal: vi.fn().mockImplementation(async (r: Record<string, unknown>) => ({ ...r, id: "new", createdAt: "", updatedAt: "" })),
  getSignals: vi.fn().mockImplementation(async (domain: string, limit?: number, tenantId?: string) => {
    calls.getSignals.push([domain, limit, tenantId]);
    return SIGNALS.filter(s => s.domain === domain && (tenantId == null || s.tenantId === tenantId));
  }),
  ...
  getEvidence: vi.fn().mockImplementation(async (domain: string, workflowId?: string, tenantId?: string) => {
  ...
  getOutcomes: vi.fn().mockImplementation(async (domain: string, limit?: number, tenantId?: string) => {
    calls.getOutcomes.push([domain, limit, tenantId]);
    return OUTCOMES.filter(o => o.domain === domain && (tenantId == null || o.tenantId === tenantId));
  }),
  getEvaluationHooks: vi.fn().mockImplementation(async (domain: string, tenantId?: string) => {
    ...
    return HOOKS.filter(h => h.domain === domain && (tenantId == null || h.tenantId === tenantId));
  }),
  ...
  executedomainWorkflow: vi.fn().mockResolvedValue({ run: { runId: "r", status: "completed", steps: [] }, requiresApproval: false }),
}));
...
let currentUser: { id: number; roles: string[]; orgs: Array<{ orgId: number; orgSlug: string }> } | null = null;
let isInternalAgent = false;

vi.mock("../middlewares/auth.js", () => ({
  authMiddleware: (options: { required?: boolean } = {}) => (req: Request, res: Response, next: NextFunction): void => {
    if (isInternalAgent) {
      (req as Request & { user: typeof currentUser; isInternalAgent: boolean }).user = {
        id: 0, roles: ["super_admin"], orgs: [],
...
describe("ATLAS row-level tenant isolation — read endpoints", () => {
  ...
  it("internal agent (server-to-server) bypasses tenant scoping with undefined tenantId", async () => {
    ...
    expect(res.body.signals.map((s: SignalRow) => s.id).sort()).toEqual([
      "s-tenant-a", "s-tenant-b", "s-tenant-default",
    ]);
