...
describe("competitive-intel notifications", () => {
  beforeEach(async () => {
    ...
    const dir = path.join(os.tmpdir(), `szl-comp-intel-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  function makeAlert(overrides: Partial<import("../jobs/competitive-intel-monitor").IntelAlert> = {}) {
    return {
      id: "test-alert-1",
      laneId: "cyber",
  ...
  it("skips dismissed alerts (no re-notify after dismiss)", async () => {
    const { notifyNewAlerts, shouldNotify } = await import("../lib/competitive-intel-notifications");
