/**
 * Integration tests for POST /continuum/recommend autonomy → approval wiring
 * (Task #2196).
 *
 * Locks in the contract that /continuum/recommend honors the persisted autonomy
...
const CONTINUUM_ARTIFACTS_TABLE = { __tableName: 'continuum_artifacts' };

// Mutable autonomy mode per test — drives the mocked autonomy-store.
const autonomyState = {
  mode: 'ask-to-act' as 'observe' | 'recommend' | 'draft' | 'ask-to-act' | 'approved-act',
};
...
vi.mock('@szl-holdings/db', () => {
  ...
  const db = {
    insert: (table: unknown) => ({
      values: (vals: Record<string, unknown>) => {
        ...
        return {
          returning: () => Promise.resolve([{ id: insertCalls.length, ...vals }]),
          onConflictDoNothing: () => ({
  ...
  return new Proxy({ db, continuumArtifactsTable: CONTINUUM_ARTIFACTS_TABLE } as Record<string, unknown>, {
    get(target, prop) {
...
vi.mock('../lib/pubsub-bridge.js', () => ({
  ...
  CONTINUUM_EVENTS: {
    WORKFLOW_RUN_UPDATED: 'CONTINUUM_WORKFLOW_RUN_UPDATED',
    SIGNAL_CREATED: 'CONTINUUM_SIGNAL_CREATED',
    APPROVAL_REQUIRED: 'CONTINUUM_APPROVAL_REQUIRED',
    WORKFLOW_STATUS_CHANGED: 'CONTINUUM_WORKFLOW_STATUS_CHANGED',
  },
}));

vi.mock('../lib/autonomy-store.js', () => {
  const AUTONOMY_MODES = ['observe', 'recommend', 'draft', 'ask-to-act', 'approved-act'];
...
vi.mock('@szl/continuum', () => ({
  recommend: recommendMock,
  checkAction: vi.fn(),
}));

vi.mock('@szl/continuum/evidence', () => ({
...
const { default: continuumRouter } = await import('../routes/continuum.js');

function buildApp() {
  const app = express();
  app.use(express.json());
  app.use('/api', continuumRouter as unknown as ExpressRouter);
  return app;
}
...
const baseRequestBody = {
  ...
  summary: 'Routine remediation suggested by continuum',
...
describe('POST /continuum/recommend autonomy → approval wiring', () => {
  it('ask-to-act: creates exactly one pending approval row carrying recommendation id + policy reason, no draft artifact', async () => {
    autonomyState.mode = 'ask-to-act';
    const res = await request(buildApp()).post('/api/continuum/recommend').send(baseRequestBody);
    ...
    expect(call.resourceType).toBe('continuum_recommendation');
    expect(call.resourceId).toBe('rec-uuid-1');
    expect(call.description).toMatch(/ASK-TO-ACT/);
    expect(call.serviceAttribution).toBe('continuum.recommend');
    ...
    const artifactInserts = insertCalls.filter((c) => c.table === CONTINUUM_ARTIFACTS_TABLE);
