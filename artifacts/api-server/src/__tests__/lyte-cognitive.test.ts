...
describe('parseTimeWindow', () => {
  ...
  it('throws on invalid from date', () => {
    expect(() => parseTimeWindow('not-a-date', '2026-01-01')).toThrow(
      'Invalid from/to date format',
    );
  });

  it('throws on invalid to date', () => {
    expect(() => parseTimeWindow('2026-01-01', 'also-not-a-date')).toThrow(
      'Invalid from/to date format',
    );
  });
});
...
describe('computeBottleneckUrgency', () => {
  ...
  it('rates higher age as higher urgency', () => {
    const low = computeBottleneckUrgency({
      var: 0,
      bottlenecks: 1,
      ageHours: 1,
      escalationCount: 0,
    });
    const high = computeBottleneckUrgency({
      var: 0,
      bottlenecks: 1,
      ageHours: 30,
      escalationCount: 0,
