...
vi.mock("@szl-holdings/services", () => ({
  services: {
    stripe: {
      createCheckoutSession: vi.fn(async () => ({ id: "cs_live_test", url: "https://checkout.stripe.com/live_test" })),
      createCustomer: vi.fn(async () => ({ id: "cus_test", email: "test@example.com" })),
      createCustomerPortalSession: vi.fn(async () => ({ url: "https://billing.stripe.com/portal/test" })),
      getCustomerByEmail: vi.fn(async () => null),
      createInvoice: vi.fn(async () => ({ id: "in_test", status: "open", hostedInvoiceUrl: "https://invoice.stripe.com/test", invoicePdf: null })),
      cancelSubscription: vi.fn(async () => ({ status: "canceled", cancelAtPeriodEnd: false, currentPeriodEnd: 0 })),
      updateSubscriptionPlan: vi.fn(async () => ({ status: "active", cancelAtPeriodEnd: false, currentPeriodEnd: 0 })),
...
vi.mock("../middlewares/auth", async () => {
  ...
  return {
    ...base,
    authMiddleware: (opts?: { required?: boolean }) => (req: Request, res: Response, next: NextFunction) => {
...
describe("Scenario A: live_stripe_billing_enabled flag is OFF", () => {
  ...
  it("POST /lyte/billing/create-invoice → 200 demo response", async () => {
    const res = await request(lyteApp).post("/lyte/billing/create-invoice").send({
      email: "user@example.com",
...
describe("Scenario D: unauthenticated requests to privileged billing routes", () => {
  ...
  it("POST /billing/aegis/invoice → 401 when unauthenticated (flag OFF)", async () => {
    const res = await request(billingApp).post("/billing/aegis/invoice").send({
      customerId: "cus_test",
