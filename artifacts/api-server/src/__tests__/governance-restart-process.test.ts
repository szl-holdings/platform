...
async function bootChild(port: number, sessionToken: string): Promise<ChildProcess> {
  const child = spawn(
    "node",
    ["--max-old-space-size=1024", "--enable-source-maps", DIST_BIN],
    {
      cwd: API_DIR,
      env: {
        ...process.env,
...
d("Governance state survives a real OS-level process restart (#1929)", () => {
  ...
  beforeAll(async () => {
    ...
    // yet on a first run.
    let [adminRole] = await db.select().from(rolesTable).where(eq(rolesTable.name, "super_admin")).limit(1);
    if (!adminRole) {
      const inserted = await db.insert(rolesTable).values({ name: "super_admin", description: "Test bootstrap" }).onConflictDoNothing().returning();
      adminRole = inserted[0];
      if (!adminRole) {
        // Conflict: re-read.
        const [reread] = await db.select().from(rolesTable).where(eq(rolesTable.name, "super_admin")).limit(1);
        adminRole = reread!;
      }
    }
    ...
    await db.insert(userRolesTable).values({ userId: userId!, roleId: adminRole.id }).onConflictDoNothing();
  ...
  it("writes governance state, restarts the OS process, and reads it back identically", async () => {
    ...
    try {
      ...
      if (evalRes.status !== 200) {
        ...
        console.error("[restart-test] /api/guardian/evaluate failed:", evalRes.status, await evalRes.text());
      }
      ...
      const { db: dbAfterWrite, guardianApprovalRequestsTable: gtableW } =
        await import("@szl-holdings/db");
      const { eq: eqW } = await import("drizzle-orm");
      const [persisted] = await dbAfterWrite.select().from(gtableW)
        .where(eqW(gtableW.requestId, PINNED.approval.requestId)).limit(1);
    ...
    try {
      ...
      const { db: dbVerify, guardianApprovalRequestsTable: gtable } = await import("@szl-holdings/db");
      const { eq: eqVerify } = await import("drizzle-orm");
      const [approval] = await dbVerify.select().from(gtable)
        .where(eqVerify(gtable.requestId, PINNED.approval.requestId)).limit(1);
