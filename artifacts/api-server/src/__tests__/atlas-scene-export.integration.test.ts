    ...
    insert: (_table: unknown) => ({
      values: (vals: Record<string, unknown>) => {
        if (auditInsertShouldFail) {
          ...
          const rejection = Promise.reject(new Error('simulated audit_events insert failure'));
          ...
          return {
            ...
            catch: (onRejected: unknown) => rejection.catch(onRejected as never),
          };
        }
...
vi.mock('../middlewares/auth', () => ({
  authMiddleware:
    ...
    (req: Request, _res: Response, next: NextFunction): void => {
      if (!authUser) {
        _res.status(401).json({ error: 'Unauthorized', code: 'UNAUTHORIZED' });
        return;
      }
...
describe('ATLAS Scene Export Routes', () => {
  ...
  describe('Audit trail', () => {
    ...
    it('FAILS the export with 503 AUDIT_LOG_UNAVAILABLE when the audit write fails (proof bundle)', async () => {
      ...
      const res = await request(app).post('/atlas/proof-bundle/export').send({
  ...
  describe('CSRF middleware compatibility', () => {
    ...
    it('POST with cookie-auth (no bearer) is blocked by CSRF when CSRF token is absent', async () => {
      ...
      const res = await request(csrfApp).post('/atlas/branch/export').send(validBranch);
