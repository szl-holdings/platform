...
d("ATLAS execution data persists across an in-process server restart (#1838)", { timeout: 30_000 }, () => {
  ...
  afterAll(async () => {
    ...
    if (evidenceId) await db.delete(atlasEvidenceTable).where(eq(atlasEvidenceTable.id, evidenceId));
    if (outcomeId) await db.delete(atlasOutcomesTable).where(eq(atlasOutcomesTable.id, outcomeId));
  ...
  it("Phase 2 — fresh app reads every record back identically over HTTP", async () => {
    ...
    const signals = (sigList.body?.signals ?? []) as Array<{ id: string; title: string; payload: Record<string, unknown>; tenantId: string; severity: string; source: string }>;
    ...
    const evidence = (evList.body?.evidence ?? []) as Array<{ id: string; label: string; value: string; workflowId: string; immutable: boolean }>;
    ...
    const outcomes = (ocList.body?.outcomes ?? []) as Array<{ id: string; title: string; summary: string; status: string; businessImpact?: { financialImpactUsd?: number; operationalSeverity?: string; entitiesAffected?: number }; metadata?: Record<string, unknown> }>;
    ...
    expect(ourOutcome!.businessImpact?.financialImpactUsd).toBe(PINNED.outcome.businessImpact.financialImpactUsd);
    expect(ourOutcome!.businessImpact?.operationalSeverity).toBe(PINNED.outcome.businessImpact.operationalSeverity);
    expect(ourOutcome!.businessImpact?.entitiesAffected).toBe(PINNED.outcome.businessImpact.entitiesAffected);
    ...
    const hooks = (hooksList.body?.hooks ?? []) as Array<{ id: string; workflowId: string; workflowName: string; replayable: boolean; signalSnapshotCount: number; benchmarkMetrics?: { latencyMs?: number } }>;
    ...
    // a domain list scan.
    const { db, atlasSignalsTable, atlasEvidenceTable, atlasOutcomesTable } = await import("@szl-holdings/db");
    const { eq } = await import("drizzle-orm");

    const [sigRow] = await db.select().from(atlasSignalsTable).where(eq(atlasSignalsTable.id, signalId)).limit(1);
    ...
    const [evRow] = await db.select().from(atlasEvidenceTable).where(eq(atlasEvidenceTable.id, evidenceId)).limit(1);
    expect(evRow).toBeDefined();
    expect(evRow!.label).toBe(PINNED.evidence.label);
    expect(evRow!.workflowId).toBe(WORKFLOW_ID);

    const [ocRow] = await db.select().from(atlasOutcomesTable).where(eq(atlasOutcomesTable.id, outcomeId)).limit(1);
  ...
  it("Phase 3 — replay endpoint rehydrates from the persisted hook", async () => {
    ...
    expect(replayRes.body?.benchmarkComparison?.originalLatencyMs).toBe(PINNED.hook.benchmarkMetrics.latencyMs);
    ...
    const hooksList = await request(app).get(`/${DOMAIN}/atlas/evaluation-hooks`);
    expect(hooksList.status).toBe(200);
    const hooks = (hooksList.body?.hooks ?? []) as Array<{ id: string; workflowName: string; replayable: boolean }>;
