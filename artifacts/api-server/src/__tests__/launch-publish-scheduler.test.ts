...
function makeSelectChain() {
  const chain = {
    ...
    orderBy() { return resolveSelect(); },
    limit() { return resolveSelect(); },
...
async function resolveSelect(): Promise<unknown[]> {
  ...
  if (table instanceof Map) {
    ...
    switch (w.scan) {
      ...
      case "xposts":
        return all.filter((p) =>
          ["approved-for-auto-send", "queued", "scheduled", "approved"].includes(p.status as string) &&
          !p.sentAt &&
...
function makeUpdateChain(table: unknown) {
  ...
  return {
    ...
    returning() { return runUpdate().then((r) => (r ? [r] : [])); },
  };
...
function makeInsertChain(table: unknown) {
  ...
  return {
    ...
    returning() { return runInsert().then((r) => (r ? [r] : [])); },
  };
...
const adapterFailures: { medium: boolean; substack: boolean; linkedin: boolean; x: boolean } = {
  medium: false, substack: false, linkedin: false, x: false,
};

vi.mock("@szl-holdings/services", () => ({
  MediumAdapter: class {
    async publishArticle() {
      if (adapterFailures.medium) { adapterCounters.medium.fail++; return { published: false, error: "medium-down", mock: false }; }
  ...
  SubstackAdapter: class {
    async publishNewsletter() {
      if (adapterFailures.substack) { adapterCounters.substack.fail++; return { published: false, error: "substack-down", mock: false }; }
  ...
  LinkedInAdapter: class {
    async sharePost() {
      if (adapterFailures.linkedin) { adapterCounters.linkedin.fail++; return { posted: false, error: "linkedin-down", mock: false }; }
  ...
  XTwitterAdapter: class {
    async postTweet() {
      if (adapterFailures.x) { adapterCounters.x.fail++; return { posted: false, error: "x-down", mock: false }; }
      adapterCounters.x.ok++;
      return { posted: true, externalPostId: "xid", externalPostUrl: "https://x.com/szl/status/1", mock: false };
    }
    async postThread() {
      if (adapterFailures.x) { adapterCounters.x.fail++; return [{ posted: false, error: "x-down", mock: false }]; }
      adapterCounters.x.ok++;
...
describe("runLaunchPublishScheduler", () => {
  beforeEach(async () => {
    ...
    adapterFailures.medium = adapterFailures.substack = adapterFailures.linkedin = adapterFailures.x = false;
  ...
  it("does NOT publish a calendar slot whose status is planned or in-progress", async () => {
    ...
    state.calendar.set(950, {
      id: 950, title: "Planned slot", contentType: "article", contentId: 401,
      status: "planned", scheduledDate: new Date("2026-04-20T08:00:00Z"),
    });
    state.calendar.set(951, {
      id: 951, title: "In progress slot", contentType: "article", contentId: 401,
      status: "in-progress", scheduledDate: new Date("2026-04-20T08:00:00Z"),
    });
  ...
  it("does NOT auto-publish an article that is still in-review (only approved)", async () => {
    ...
    state.calendar.set(970, {
      id: 970, title: "Slot for in-review article", contentType: "article", contentId: 610,
      status: "ready", scheduledDate: new Date("2026-04-20T11:00:00Z"),
    });
    const { runLaunchPublishScheduler } = await import("../jobs/launch-publish-scheduler");
    const result = await runLaunchPublishScheduler({ now: nowOverride.value });
    expect(adapterCounters.medium.ok).toBe(0);
  ...
  it("does NOT auto-publish a draft article even when a ready calendar slot points at it", async () => {
    ...
    state.calendar.set(960, {
      id: 960, title: "Ready slot pointing at draft", contentType: "article", contentId: 501,
      status: "ready", scheduledDate: new Date("2026-04-20T08:00:00Z"),
    });
  ...
  it("dispatches a calendar carousel item to LinkedIn and a calendar x-post to X", async () => {
    state.carousels.set(50, { id: 50, title: "Carousel", linkedinShortCaption: "hi", status: "ready", ctaUrl: "https://szl/" });
    state.xposts.set(60, { id: 60, body: "tweet body", postType: "single", status: "approved", scheduledFor: new Date("2026-04-20T11:30:00Z"), retryCount: 0 });
    state.calendar.set(901, {
      id: 901, title: "Carousel slot", contentType: "carousel", contentId: 50,
      status: "ready", scheduledDate: new Date("2026-04-20T11:00:00Z"),
    });
    state.calendar.set(902, {
      id: 902, title: "X-post slot", contentType: "x-post", contentId: 60,
      status: "ready", scheduledDate: new Date("2026-04-20T11:30:00Z"),
    });
