...
d('Vessels BoL persistence (Task #1847)', () => {
  ...
  it('persists a new BoL across a simulated server restart and rebuilds chain hashes from events', {
    timeout: 30_000,
  }, async () => {
    ...
    const verify = await request(app2).get(
      `/api/vessels/modules/bills-of-lading/${createdId}/verify`,
    );
  ...
  it('seeds demo BoLs only on first run (idempotent across restarts)', {
    timeout: 30_000,
  }, async () => {
    const { db, vesselsBillsOfLadingTable } = await import('@szl-holdings/db');
