...
describe("internal-tokens registry (GAP-016)", () => {
  ...
  it("matches a scoped token and returns its declared scopes", () => {
    setEnv(
      JSON.stringify([
        { name: "alloy-runner", token: "secret-aaa", scopes: ["alloy:write", "agent:write"], pathPrefixes: ["/api/alloy/agent/"] },
      ]),
      undefined,
    );
  ...
  it("ignores tokens with unknown scopes (allowlist)", () => {
    setEnv(
      JSON.stringify([
        { name: "x", token: "secret-aaa", scopes: ["alloy:write", "super_admin"] },
      ]),
      undefined,
    );
    const match = matchInternalToken("secret-aaa");
  ...
  it("verifyInternalHeader enforces pathPrefixes", () => {
    setEnv(
      JSON.stringify([
        { name: "alloy-runner", token: "secret-aaa", scopes: ["alloy:write"], pathPrefixes: ["/api/alloy/agent/"] },
      ]),
      undefined,
    );
