...
describe('Guardian enforcement smoke (#1214)', () => {
  beforeAll(async () => {
    ...
    guardianPolicyCheck = middlewareMod.guardianPolicyCheck as unknown as GuardianMiddlewareFactory;
    const { getGuardianEngine, initGuardianEngine } = await import('../lib/guardian-engine.js');
  ...
  for (const domain of AGENT_DOMAINS) {
    describe(`domain=${domain}`, () => {
      ...
      it('require-approval → 202, routes through Alloy approval gate, surfaces approvalRequestId', async () => {
        ...
        expect(res.headers['x-guardian-approval-id']).toBe(String(res.body.approvalRequestId));
