...
const WORKFLOW_TYPES = [
  {
    ...
    description:
      'Ingests, transforms, and loads daily operational data across all platform connectors.',
  ...
  },
  {
    ...
    steps: [
      ...
      { id: 'setup_billing', label: 'Setup Billing', deps: ['validate_crm'] },
      {
        ...
        deps: ['provision_access', 'setup_billing'],
      },
  ...
  },
  {
    ...
    description:
      'Processes and normalizes cross-platform intelligence signals from all Alloy connectors.',
    trigger: 'webhook' as const,
    outputType: 'action' as const,
    steps: [
...
function generateStepHistory(
  steps: Array<{ id: string; label: string; deps: string[] }>,
  status: string,
  durationMs: number,
) {
  const now = Date.now();
  ...
  return steps.map((step, i) => {
    ...
    return {
      ...
      logs: [
        ...
        stepStatus === 'failed'
          ? `[${new Date(stepEnd).toISOString()}] ERROR: Connection timeout after 30s`
          : `[${new Date(stepEnd).toISOString()}] ${step.label} completed (${(perStep / 1000).toFixed(1)}s)`,
      ],
      input: { step: step.id, iteration: i + 1 },
      output:
        stepStatus === 'failed' ? null : { processed: randBetween(100, 10000), success: true },
    };
  });
}
...
async function seedAlloyWorkflows() {
  ...
  const workflows = WORKFLOW_TYPES.map((wt, i) => ({
    ...
    trigger: wt.trigger,
    triggerConfig:
      wt.trigger === 'schedule' ? { cron: i % 2 === 0 ? '0 6 * * *' : '0 */4 * * *' } : null,
    steps: wt.steps,
...
async function seedAlloySignals(count = 500) {
  ...
  const signals = Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      receivedAt,
      processedAt:
        Math.random() > 0.3 ? new Date(receivedAt.getTime() + randBetween(1000, 60000)) : null,
    };
  });
...
async function seedAlloyRuns(
  ...
  for (const wf of workflows) {
    ...
    for (let i = 0; i < runsPerWorkflow; i++) {
      ...
      const stepsExecuted =
        ['completed', 'failed'].includes(state) && steps.length > 0
      ...
      const stateHistory = [
        ...
        ...(['completed', 'failed', 'running', 'waiting_approval'].includes(state)
          ? [{ state: 'running', at: startedAt.toISOString(), by: 'system' }]
          : []),
        ...(state === 'waiting_approval'
          ? [
              {
                state: 'waiting_approval',
                at: new Date(startedAt.getTime() + durationMs * 0.8).toISOString(),
                by: 'system',
              },
            ]
          : []),
        ...(['completed', 'failed'].includes(state) && completedAt
          ? [{ state, at: completedAt.toISOString(), by: 'system' }]
          : []),
      ];
        ...
        durationMs:
          state === 'completed'
            ...
            : state === 'failed'
              ...
              : null,
        queuedAt,
        startedAt: ['completed', 'failed', 'running', 'waiting_approval'].includes(state)
          ? startedAt
          : null,
        completedAt,
      });
...
async function seedAlloyApprovals(_workflows: Array<{ id: number; requiresApproval: boolean }>) {
  ...
  const waitingRuns = await db
    .select({ id: alloyWorkflowRunsTable.id, workflowId: alloyWorkflowRunsTable.workflowId })
    .from(alloyWorkflowRunsTable)
  ...
  const decisions = [
    null,
    'Reviewed and approved — all controls verified',
    'Approved pending minor revisions',
    'Rejected — non-compliant with SOC 2 CC6.1',
    'Expired — no reviewer response within 48h',
  ];
  ...
  const approvalsToInsert = waitingRuns.slice(0, 60).map((run, i) => {
    ...
    const decisionAt =
      status !== 'pending' ? new Date(createdAt.getTime() + randBetween(1, 48) * 3600000) : null;
    ...
    return {
      ...
      status,
      decision:
        status !== 'pending' ? decisions[Math.floor(Math.random() * decisions.length)] : null,
      decisionAt,
