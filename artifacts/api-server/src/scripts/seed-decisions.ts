...
const WORKFLOWS = [
  ...
  {
    id: 'wf-terra-outreach-002',
    name: 'Distress Property Outreach — 234 W 145th St',
    domain: 'terra',
  },
  ...
  {
    id: 'wf-terra-model-006',
    name: 'Valuation Model Refresh — Brooklyn Multifamily',
    domain: 'terra',
  },
  {
    id: 'wf-vessels-reroute-007',
    name: 'Red Sea Re-routing Authorization — MV Atlantic Voyager',
    domain: 'vessels',
  },
  {
    id: 'wf-platform-backup-008',
    name: 'Disaster Recovery Drill — Backup Validation',
    domain: 'platform',
  },
...
export async function seedDecisions() {
  ...
  const aiDecisions: (typeof alloyAiDecisions.$inferInsert)[] = [
    {
      ...
      recommendedAction:
        'Deploy emergency security patch to payment-api-v3 — approve in next 2 hours',
      rationaleSummary:
        'CVSS 9.3 IDOR vulnerability in payment API. Active exploitation window exists. Patch tested in staging — no regressions. Risk of non-action: $4.2M payment data exposure.',
      evidenceRefs: [
        {
          type: 'finding',
          id: 'aegis-asset-payment-api-v3',
          label: 'IDOR finding — payment-api-v3',
        },
        {
          type: 'incident',
          id: 'inc-aegis-2026-001',
          label: 'Active threat monitoring — confirmed exploitation attempt',
        },
      ],
      confidence: 0.96,
      ...
      riskLevel: 'critical',
      fallbackPlan:
        'Take payment-api-v3 offline and redirect to v2 failover endpoint if patch deployment fails',
      modelRoute: 'gpt-4o',
      schemaVersion: '2.1',
      status: 'approved',
      ...
      rawInput: JSON.stringify({
        finding: 'IDOR in payment API',
        cvss: 9.3,
        affected_asset: 'payment-api-v3',
      }),
    ...
    },
    {
      ...
      recommendedAction:
        'Authorize letter of intent submission for 234 W 145th St — auction window closes April 10',
      rationaleSummary:
        'Opportunity score 92/100. 283 days in distress. Auction April 10. Comparable analysis confirms 8% below-market entry. Projected IRR 18.4% at hold-to-sell exit.',
      evidenceRefs: [
        { type: 'property', id: 'dp-seed-002', label: '234 W 145th St — distress property' },
        {
          type: 'signal',
          id: 'terra-auction-signal-002',
          label: 'Auction deadline alert — 11 days',
        },
      ],
      confidence: 0.89,
      ...
      status: 'proposed',
      rawInput: JSON.stringify({
        property: 'dp-seed-002',
        opportunity_score: 92,
        days_in_distress: 283,
      }),
    ...
    },
    {
      ...
      recommendedAction:
        'Clear IMO 9876543 — secondary OFAC screening complete, no match, resume normal tracking',
      rationaleSummary:
        '18-hour AIS dark period resolved. OFAC API re-query: no match on vessel, operator, or last 3 port calls.',
      evidenceRefs: [
      ...
      riskLevel: 'medium',
      fallbackPlan:
        'If additional anomalies detected within 72 hours, escalate to Tier-1 compliance review',
      modelRoute: 'gpt-4o',
      schemaVersion: '2.1',
      status: 'executed',
    ...
    },
    {
      ...
      recommendedAction:
        'Escalate PRISM document review latency spike to infrastructure team — SLA at risk',
      rationaleSummary:
        'P95 latency up 340% over 4-hour window. Pattern consistent with DB connection pool contention. 14 active matters at risk of SLA breach.',
      evidenceRefs: [
        {
          type: 'signal',
          id: 'lyte-signal-prism-latency',
          label: 'Lyte latency spike — PRISM pipeline',
        },
      ],
      confidence: 0.84,
      ...
      riskLevel: 'medium',
      fallbackPlan:
        'Route PRISM review pipeline to dedicated DB replica if primary pool remains saturated',
      modelRoute: 'claude-3-5-sonnet',
      schemaVersion: '2.1',
      status: 'executed',
      executedAt: daysAgo(2),
      executionOutcome:
        'Escalated. Platform ops routed pipeline to replica — latency resolved within 20 minutes.',
      rawInput: JSON.stringify({
        service: 'prism-review-pipeline',
        p95_latency_ms: 8400,
        baseline_ms: 2100,
      }),
    ...
    },
    {
      ...
      recommendedAction:
        'Enforce MFA on all Azure AD admin accounts — estimated 4-hour rollout, 0 business disruption',
      rationaleSummary:
        '4 admin accounts without MFA enforcement. CVSS 9.0. Rollout plan tested — push notification method compatible with all enrolled devices.',
      evidenceRefs: [
      ...
      riskLevel: 'critical',
      fallbackPlan:
        'If MFA enforcement causes lockout, break-glass procedure documented in Vault-prod',
      modelRoute: 'gpt-4o',
      schemaVersion: '2.1',
      status: 'approved',
      approvedBy: 'stephen@szlholdings.com',
    ...
    ...Array.from({ length: 25 }, (_, i) => ({
      ...
      evidenceRefs: [
        {
          type: 'signal',
          id: `signal-auto-${i}`,
          label: `Automated signal — ${DOMAINS[i % DOMAINS.length]}`,
        },
      ],
      ...
      fallbackPlan:
        'Escalate to human operator if automated resolution fails within defined timeout.',
  ...
  const decisioningRuns: (typeof szlDecisioningRunsTable.$inferInsert)[] = WORKFLOWS.flatMap(
    (wf, wfIdx) =>
      Array.from({ length: 3 }, (_, runIdx) => {
        const daysBack = wfIdx * 2 + runIdx * 5;
        const status: 'completed' | 'dry_run' | 'simulated' =
          runIdx === 0 ? 'completed' : runIdx === 1 ? 'dry_run' : 'simulated';
        ...
        return {
          ...
          steps: [
            ...
            },
            {
              ...
              status: outcome === 'success' ? 'completed' : 'skipped',
          ...
          auditTrail: [
            ...
            },
            {
              ...
              actor: 'policy-engine',
              result: 'pass',
              timestamp: new Date(daysAgo(daysBack).getTime() + durationMs * 0.4).toISOString(),
            },
            {
              action: 'workflow_completed',
              actor: 'atlas-scheduler',
              timestamp: new Date(daysAgo(daysBack).getTime() + durationMs).toISOString(),
            },
          ],
          policyEvaluation: { passed: true, policiesChecked: 3 + (wfIdx % 4), violationsFound: 0 },
          cost: {
            modelTokens: 1200 + Math.floor(Math.random() * 3000),
            estimatedUsd: parseFloat((0.01 + Math.random() * 0.4).toFixed(4)),
          },
          outcome,
          outcomeSummary:
            outcome === 'success'
              ? `${wf.name} completed successfully in ${durationMs}ms — all policy gates passed`
              : undefined,
          outcomeImpact:
            outcome === 'success'
              ? { riskReduction: 'moderate', entitiesAffected: 1 + (wfIdx % 3) }
              : {},
  ...
  const recommendations: (typeof szlDecisioningRecommendationsTable.$inferInsert)[] = [
    {
      ...
      description:
        "IDOR vulnerability in payment API allows unauthorized access to other users' transaction data. Emergency patch available and tested.",
      domain: 'aegis',
      ...
      urgency: 'critical',
      businessImpact: {
        riskExposureUsd: 4_200_000,
        slaImpact: 'none',
        userImpact: 'payment_data_exposure',
      },
      signals: [
        { id: 'aegis-asset-payment-api-v3', type: 'security_finding', severity: 'critical' },
      ],
      evidence: [
        {
          type: 'finding',
      ...
      reasoning:
        'CVSS 9.3 confirmed by red team. Active monitoring shows 3 probe attempts in past 6 hours. Patch tested in staging with 0 regression failures.',
    ...
    },
    {
      ...
      description:
        'Auction April 10. Opportunity score 92. Below-market debt load. Recommend LOI submission within 48 hours.',
      domain: 'terra',
      action:
        'Submit non-binding letter of intent to 145th Holdings LLC — acquisition team to lead',
      priorityScore: 0.89,
      confidence: 0.89,
      urgency: 'urgent',
      businessImpact: {
        estimatedMarginUsd: 1_400_000,
        acquisitionPriceUsd: 4_200_000,
        projectedIrr: 0.184,
      },
      signals: [{ id: 'terra-distress-dp-seed-002', type: 'distress_signal', severity: 'high' }],
      evidence: [
        { type: 'property', id: 'dp-seed-002', label: '234 W 145th St', opportunityScore: 92 },
      ],
      reasoning:
        '283 days in distress. No cure plan on file. Auction scheduled in 11 days. Model projects 18.4% IRR at hold-to-sell 5-year exit.',
      policyState: 'requires_approval',
    ...
    },
    {
      ...
      description:
        '18-hour AIS dark period resolved. OFAC re-query: no match. Route consistent with declared manifest.',
      domain: 'vessels',
      ...
      evidence: [
        {
          type: 'screening',
          id: 'ofac-screen-2026-0148',
          label: 'OFAC API screening',
          result: 'no_match',
        },
      ],
      reasoning:
        'Secondary OFAC screening complete. Port call history verified. Analyst dissent reviewed and addressed.',
      policyState: 'allowed',
    ...
    ...Array.from({ length: 20 }, (_, i) => ({
      ...
      priorityScore: parseFloat((0.4 + Math.random() * 0.5).toFixed(2)),
      confidence: parseFloat((0.65 + Math.random() * 0.3).toFixed(2)),
      urgency: (['routine', 'routine', 'moderate', 'urgent', 'critical'] as const)[i % 5]!,
      businessImpact: {},
      signals: [
        { id: `signal-auto-${i}`, type: 'platform_signal', domain: DOMAINS[i % DOMAINS.length] },
      ],
      evidence: [],
      reasoning: `Routine recommendation from automated platform monitoring.`,
      policyState: (['allowed', 'allowed', 'requires_approval', 'allowed', 'blocked'] as const)[
        i % 5
      ]!,
  ...
  const policyViolations: (typeof szlPolicyViolationsTable.$inferInsert)[] = [
    {
      ...
      reason:
        'Agent run exceeded per-call cost ceiling of $0.50 — actual cost $0.78 on PRISM escalation workflow',
    ...
    },
    {
      ...
      reason:
        'Automated playbook triggered without required sign-off — confidence 67% below PSP-2024-07 threshold of 80%',
      confidence: 0.67,
      runId: 'run-wf-aegis-redteam-009-1',
      tenantId: 'org-1',
      metadata: {
        suggestedRemediation: 'Raise auto-trigger threshold to 80% confidence in PSP-2024-07.',
      },
      occurredAt: daysAgo(2),
      createdAt: daysAgo(2),
    },
    {
      ...
      reason:
        'LOI submission for $4.2M acquisition requires senior operator approval per binding-decision policy',
      estimatedCostUsd: 4_200_000,
      confidence: 0.89,
  ...
  const knowledgeEntries: (typeof agentKnowledgeTable.$inferInsert)[] = [
    {
      ...
      summary:
        'IDOR in payment-api-v3 /api/payments/transactions/{id} allows sequential ID enumeration. 3 probe attempts detected in past 6h. Patch pre-staged.',
      confidence: 0.96,
    ...
    },
    {
      ...
      summary:
        'Production payment API. 47 endpoints. $2.4M daily transaction volume. Last scanned 30 minutes ago. 4 critical, 9 high findings open.',
      confidence: 0.99,
      tags: ['payment-api', 'production', 'critical-asset'],
      relatedEntryIds: ['k-aegis-001'],
      data: {
        assetId: 'payment-api-v3',
        criticalFindings: 4,
        highFindings: 9,
    ...
    },
    {
      ...
      summary:
        'Q1 2026 comparables show 8-12% discount to 2025 peak valuations. Auction properties clearing at 94-96% of appraised value.',
      confidence: 0.87,
    ...
    },
    {
      ...
      summary:
        'Opportunity score 92. 283 days distress. Auction Apr 10. Model: 18.4% IRR at 5-year hold-to-sell.',
      confidence: 0.89,
      tags: ['145th-st', 'manhattan', 'auction', 'high-opportunity'],
      relatedEntryIds: ['k-terra-001'],
      data: {
        propertyId: 'dp-seed-002',
        opportunityScore: 92,
        projectedIrr: 0.184,
    ...
    },
    {
      ...
      summary:
        'MV Horizon Star. OFAC API re-query on 2026-04-13. No match on vessel, operator, or last 3 port calls. AIS dark period route-consistent.',
      confidence: 0.93,
    ...
    },
    {
      ...
      summary:
        'Vessels Maritime ARR tracking $200K above Q2 plan. NRR TTM 119%. Two enterprise expansion events in Q1.',
      confidence: 0.91,
    ...
    ...Array.from({ length: 30 }, (_, i) => ({
      ...
      summary: `Signal aggregated from ${DOMAINS[i % DOMAINS.length]} domain pipeline. Confidence: ${(0.65 + Math.random() * 0.3).toFixed(2)}. Routine monitoring.`,
      confidence: parseFloat((0.65 + Math.random() * 0.3).toFixed(2)),
