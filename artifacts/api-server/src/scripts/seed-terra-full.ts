...
const BOROUGH_DATA: Record<
  string,
  { county: string; zips: string[]; lat: [number, number]; lng: [number, number] }
> = {
  Manhattan: {
    county: 'New York',
    zips: [
...
const COUNTY_DATA: Record<
  string,
  { borough: string; zips: string[]; lat: [number, number]; lng: [number, number] }
> = {
  Westchester: {
    borough: 'Manhattan',
    zips: [
...
const OWNER_NAMES_LLC = [
  'Blue Rock Capital LLC',
  'SunBridge Properties LLC',
  'NYC Equity Holdings LLC',
  'Empire State Realty Partners LLC',
  'Tri-Borough Ventures LLC',
  'Westside Capital Group LLC',
  'Gotham Asset Management LLC',
  'Hudson Valley Realty LLC',
  'Crown Heights Properties LLC',
  'BedStuy Partners LLC',
  'Astoria Capital LLC',
  'Flatbush Holdings LLC',
  'Williamsburg Ventures LLC',
  'DUMBO Asset Group LLC',
  'Midtown South Partners LLC',
  'Harlem Renaissance Holdings LLC',
  'Washington Heights Realty LLC',
  'Mott Haven Capital LLC',
  'Soundview Properties LLC',
  'East New York Holdings LLC',
  'Canarsie Partners LLC',
  'Bay Ridge Realty LLC',
  'Sunset Park Holdings LLC',
  'Woodside Capital LLC',
  'Jamaica Ave Properties LLC',
  'Forest Hills Holdings LLC',
  'Kew Gardens Partners LLC',
  'Flushing Capital LLC',
  'Long Island City Holdings LLC',
  'Ridgewood Properties LLC',
  'Rockaway Partners LLC',
  'Howard Beach Holdings LLC',
  'College Point Capital LLC',
  'Whitestone Realty LLC',
  'Bayside Properties LLC',
  'Little Neck Holdings LLC',
  'Great Neck Partners LLC',
  'Manhasset Capital LLC',
  'Garden City Holdings LLC',
  'Valley Stream Properties LLC',
  'Hempstead Capital LLC',
  'Mineola Holdings LLC',
  'Rockville Centre Partners LLC',
  'Lynbrook Realty LLC',
  'Far Rockaway Capital LLC',
  'Ossining Realty LLC',
  'Yonkers Capital Holdings LLC',
  'White Plains Partners LLC',
  'New Rochelle Properties LLC',
  'Mamaroneck Holdings LLC',
];
...
const OWNER_NAMES_TRUST = [
  ...
  "O'Brien Revocable Trust",
  'Kim Living Trust',
  'Patel Family Trust',
  'Rosenberg Irrevocable Trust',
  'Nguyen Family Trust',
  'Cohen Revocable Trust',
  'Washington Living Trust',
  'Murphy Family Trust',
  'Schwartz Trust',
  'Fernandez Family Trust',
  "O'Sullivan Irrevocable Trust",
];

const OWNER_NAMES_CORP = [
  'Chase Bank REO',
  'First Republic Bank (FDIC)',
  'Wells Fargo Asset Recovery',
  'Signature Bank REO Portfolio',
  'NYCB REO Division',
  'Citibank REO',
  'Investors Bank Asset Recovery',
  'Metropolitan Savings REO',
  'Apple Bank REO Portfolio',
  'Sterling Bancorp Asset Recovery',
];
...
function generateOwner(ownerType: (typeof OWNER_TYPES)[number]): string {
  switch (ownerType) {
    case 'llc':
      return pick(OWNER_NAMES_LLC);
    case 'individual':
      return pick(OWNER_NAMES_INDIVIDUAL);
    case 'trust':
      return pick(OWNER_NAMES_TRUST);
    case 'corporate':
      return pick(OWNER_NAMES_CORP);
  }
}

function generateAddressForBorough(borough: string): {
  address: string;
  zipCode: string;
  lat: string;
  lng: string;
...
function generateAddressForCounty(county: string): {
  address: string;
  zipCode: string;
  lat: string;
  lng: string;
...
function scoreRationale(
  ...
  if (distressType === 'auction') parts.push(`auction approaching — immediate action window`);
    ...
    parts.push(`early-stage filing — ideal for direct off-market outreach`);
  ...
  else if (distressType === 'tax-lien')
    parts.push(`growing tax liability, owner likely under financial pressure`);
  else parts.push(`expired listing signals seller frustration and motivation`);

  parts.push(`${daysInDistress}d in distress`);
  if (['Manhattan', 'Brooklyn'].includes(borough))
    parts.push(`${borough} premium location commands strong resale demand`);

  if (ownerType === 'llc') parts.push(`LLC owner may respond to off-market acquisition approach`);
  else if (ownerType === 'individual')
    parts.push(`individual owner — direct personal outreach recommended`);
  else if (ownerType === 'trust')
    parts.push(`trust-managed — potential estate/succession motivation`);
...
function generateTimeline(
  ...
  timeline.push({
    ...
    description:
      distressType === 'pre-foreclosure'
        ? 'Pre-foreclosure notice filed with county'
        : distressType === 'foreclosure'
          ? 'Foreclosure proceeding initiated by lender'
          : distressType === 'tax-lien'
            ? `Tax lien filed — $${(rand(50, 400) * 1000).toLocaleString()}`
            : distressType === 'reo'
              ? 'Property transferred to bank REO portfolio'
              : distressType === 'expired-listing'
                ? `Listing expired after ${rand(6, 18)} months on market`
  ...
  if (auctionDate) {
    timeline.push({
      date: auctionDate,
      type: 'Auction',
      description: 'Foreclosure auction scheduled',
    });
  }
...
function generateLeads(
  propertyIds: number[],
  propertyMap: Map<number, InsertTerraDistressProperty>,
): InsertTerraLead[] {
  ...
  for (let i = 1; i <= 120; i++) {
    ...
    const distressPropertyId =
      i <= 40 && propertyIds.length > 0 ? (propertyIds[i - 1] ?? null) : null;
    ...
    leads.push({
      ...
      notes:
        i <= 40 && distressProp
          ? `Converted from distress engine — ${distressProp.distressType} at ${distressProp.address}`
          : rand(0, 2) === 0
            ? pick([
                'Looking for off-market multifamily in Brooklyn',
                'Investor seeking distressed commercial properties',
                'Owner-occupant — needs quick close',
                'Wholesale buyer, cash available within 2 weeks',
                '1031 exchange buyer — needs replacement property by Q4',
              ])
            : null,
      tags:
        source === 'distress-engine'
          ? [distressProp?.distressType ?? 'distress', 'distress-converted']
          : [type, source],
      timeline: [
        { date: dateStr(daysAgo), event: 'Lead created', type: 'created' },
        ...(lastContact
          ? [{ date: lastContact, event: 'Follow-up call completed', type: 'contact' }]
          : []),
      ],
      nextAction: pick([
        'Initial outreach call',
        'Send property overview',
        'Schedule site visit',
        'Follow up on offer',
        'Qualify budget range',
        'Send market comps',
      ...
      isActive: true,
      desiredAreas: [
        pick(['Brooklyn', 'Manhattan', 'Queens', 'Bronx', 'Staten Island', 'Westchester']),
      ],
...
function generateDeals(
  ...
  for (let i = 1; i <= 60; i++) {
    ...
    const price =
      stage === 'closed'
        ? Math.round((askingPrice * rand(88, 105)) / 100)
        : ['offer', 'negotiation', 'accepted'].includes(stage)
          ? Math.round((askingPrice * rand(85, 100)) / 100)
          : null;
    const ownerName = pick(AGENT_NAMES);
    ...
    deals.push({
      ...
      address:
        prop?.address ??
        `${rand(100, 999) * 2} ${pick(['Broadway', 'Atlantic Ave', 'Flatbush Ave', 'Grand Concourse', 'Jamaica Ave'])}`,
      ...
      clientName: pick([
        ...
        'Greenpoint Capital',
        'Meridian Capital',
        'Battery Park Ventures',
        'Crown Heights Investors',
        'Bed-Stuy Partners',
        'Flatbush Group',
        'Jamaica Capital',
        'Pelham Bay Partners',
      ...
      timeline: [
        ...
        {
          date: dateStr(stageEnteredDaysAgo),
          event: `Entered ${stage} stage`,
          type: 'stage_change',
        },
      ],
      documents: [
        ...
        },
        {
          ...
          status: ['clear-to-close', 'closed'].includes(stage) ? 'complete' : 'not_started',
        },
      ],
      isActive: true,
    });
