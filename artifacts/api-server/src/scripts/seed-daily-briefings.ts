...
import { dailyBriefingsTable } from "@szl-holdings/db";
...
const HEALTH_STATES: Array<"nominal" | "elevated" | "degraded" | "critical"> = [
  "nominal", "nominal", "nominal", "nominal", "elevated", "nominal", "nominal",
  "nominal", "elevated", "nominal", "nominal", "nominal", "elevated", "nominal",
  "nominal", "nominal", "nominal", "degraded", "nominal", "nominal", "critical",
  "nominal", "nominal", "nominal", "elevated", "nominal", "nominal", "nominal",
  "nominal", "elevated",
];
  ...
  return [
    {
      ...
      title: day === 20 ? "Ransomware precursor detected — playbook triggered" : "Aegis daily scan completed",
      count: 2 + (day % 6),
...
export async function seedDailyBriefings() {
  ...
  const existing = await db.select({ id: dailyBriefingsTable.id })
  ...
  if (existing.length > 0) {
    console.log(`[seed-daily-briefings] Partial seed detected (${existing.length}/30 rows) — rebuilding.`);
