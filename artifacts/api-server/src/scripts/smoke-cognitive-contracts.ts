...
const VALID_FRESHNESS = new Set(['fresh', 'aging', 'stale-90d']);
{
  ...
  assert(
    'provenance has traceRef',
    typeof p.traceRef === 'string' && p.traceRef.startsWith('trace-'),
  );
...
}
{
  ...
  for (const { days, expected } of cases) {
    ...
    assert(`"${result}" is a valid freshness enum value`, VALID_FRESHNESS.has(result));
  }
  assert(
...
}
{
  ...
  const impactMapping = {
    ...
    estimatedFinancialImpact:
      8_000_000 + (deterministicHash(`${mockIncidentId}-impact`) % 4_000_000),
    affectedEntities: [],
    citations: [],
  ...
  assert(
    'incidentImpact.provenance.traceRef exists',
    typeof impactMapping.provenance.traceRef === 'string',
  );
  assert(
    'incidentImpact.provenance.traceRef matches incident',
    impactMapping.provenance.traceRef === `trace-bim-${mockIncidentId}`,
  );
  assert(
    'incidentImpact.provenance.traceId matches traceRef',
    impactMapping.provenance.traceId === impactMapping.provenance.traceRef,
  );
...
}
{
  ...
  for (const node of mockNodes) {
    assert(
      `node[${node.type}] provenance.traceId present`,
      typeof node.provenance.traceId === 'string',
    );
    assert(
      `node[${node.type}] provenance.traceRef present`,
      typeof node.provenance.traceRef === 'string',
    );
...
}
{
  ...
  const mockEvent = {
    ...
    citations: [{ id: 'CIT-001', source: 'SIEM', ref: 'siem:alert-001', confidence: 95 }],
    mitreTag: 'T1078',
  ...
  assert(
    'event citations have confidence',
    mockEvent.citations.every(
      (c) => typeof c.confidence === 'number' && c.confidence >= 0 && c.confidence <= 100,
    ),
  );
...
}
{
  ...
  const unknown = resolveIdentity('unknown@external.com', mockAssets) as {
    displayName: string;
    role: string;
    riskScore: number;
    groups: string[];
  ...
  assert(
    'unknown identity displayName is the identityId',
    unknown.displayName === 'unknown@external.com',
  );
  assert(
    "unknown identity role is 'Unknown / Unclassified'",
    unknown.role === 'Unknown / Unclassified',
  );
  assert(
    'unknown identity riskScore is in [40, 79]',
    unknown.riskScore >= 40 && unknown.riskScore <= 79,
  );
  assert(
    'unknown identity groups include asset-based groups',
    unknown.groups.some((g) => g.startsWith('Asset-Group-')),
  );
  assert(
    'unknown identity is deterministic (same input = same riskScore)',
...
}
{
  ...
  assert(
    "no evidence items use bare 'stale'",
