...
async function ensureFirestormIncidents() {
  ...
  const rows = await db.insert(firestormIncidentsTable).values([
    {
      ...
      notes: "Likely tied to overnight breach disclosure at SaaS competitor; replay attempts using leaked corpus.",
    ...
    },
    {
      ...
      timeline: [
        { at: hoursAgo(3).toISOString(), event: "Detection — EDR tamper signal" },
      ],
...
async function ensureVesselsAlerts() {
  ...
  if (active >= 5 && critical >= 2) {
    console.log(`[seed-live-signals] vessels_alerts: ${active} active / ${critical} high+critical, skipping`);
    return 0;
  }

  const vessels = await db.select({ id: vesselsTable.id, name: vesselsTable.name }).from(vesselsTable).limit(5);
  if (vessels.length === 0) {
    console.warn("[seed-live-signals] vessels_alerts: no vessels rows — run seed:demo first; skipping");
    return 0;
  }
  const v = (i: number) => vessels[i % vessels.length].id;
  const vName = (i: number) => vessels[i % vessels.length].name;

  const rows = await db.insert(vesselsAlertsTable).values([
    {
      vesselId: v(0), title: `Speed deviation — ${vName(0)}`,
    ...
    {
      vesselId: v(1), title: `Geofence breach — ${vName(1)} entered restricted zone`,
    ...
    {
      vesselId: v(2), title: `Weather window — ${vName(2)} entering Force 9`,
      message: "Forecast shows Beaufort 9 winds within 6h on current heading; reroute advisory issued.",
      severity: "high", status: "acknowledged",
      metadata: { beaufort: 9, etaToWeather: 6 },
      triggeredAt: hoursAgo(3),
    },
    {
      vesselId: v(3), title: `Schedule risk — ${vName(3)} berth slot at risk`,
      message: "Cumulative delay (+11h) projected to miss Rotterdam berth window; queue position +2.",
    ...
    {
      vesselId: v(4), title: `Maintenance watch — ${vName(4)} main engine vibration`,
      message: "ISO 10816 vibration band crossed 'alarm' threshold on cylinder 4 over last 4 hours.",
      severity: "medium", status: "active",
      metadata: { component: "main_engine", iso10816Band: "alarm", cylinder: 4 },
      triggeredAt: hoursAgo(5),
    },
    {
      vesselId: v(0), title: `Cargo temperature excursion — ${vName(0)} reefer bay 3`,
      message: "Reefer bay 3 temperature 4.2°C above setpoint for 38 minutes; QA notification sent.",
      severity: "high", status: "active",
...
async function ensureVesselsDelayEvents() {
  ...
  const vessels = await db.select({ id: vesselsTable.id, name: vesselsTable.name }).from(vesselsTable).limit(3);
  if (vessels.length === 0) {
    console.warn("[seed-live-signals] vessels_events: no vessels rows; skipping");
    return 0;
  }

  const rows = await db.insert(vesselsEventsTable).values([
    {
      ...
      description: "Anchorage queue extended; berth allocation shifted by 32 hours due to congestion.",
      consequenceData: { delayHours: 32, port: "Shanghai", queuePosition: 14, marginImpact: 380000 },
    ...
    },
    {
      ...
      description: "Rail handover at LA/LB delayed 14h; 2 customer SLAs at risk on automotive parts shipment.",
...
async function ensureTerraDistress() {
  ...
  console.log("[seed-live-signals] terra_distress_properties below threshold — invoking seedTerraDistress()…");
  try {
    ...
    const after = await db
      ...
      .from(terraDistressPropertiesTable)
      .where(eq(terraDistressPropertiesTable.isActive, true));
    console.log(`[seed-live-signals] terra_distress_properties: ${after[0]?.count ?? 0} active after seed`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  seedLiveSignals()
    .then(() => process.exit(0))
    .catch((err) => { console.error("[seed-live-signals] failed:", err); process.exit(1); });
}
