import {
  continuumRuntimeAgentsTable,
  continuumRuntimeAgentVersionsTable,
  db,
  organizationsTable,
...
export async function seedAlloyRuntimeAgents() {
  ...
  for (const agent of CONTINUUM_AEGIS_AGENTS) {
    const { version, changelog, ...agentFields } = agent;

    const result = await db
      .insert(continuumRuntimeAgentsTable)
      .values({ ...agentFields, orgId: ORG_ID })
      .onConflictDoNothing({ target: continuumRuntimeAgentsTable.agentId })
      .returning({ agentId: continuumRuntimeAgentsTable.agentId });

    if (result.length > 0) {
      inserted++;
      await db
        .insert(continuumRuntimeAgentVersionsTable)
        .values({
