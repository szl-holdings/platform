...
function assertProvenance(prov: Provenance | undefined, label: string): void {
  ...
  if (prov.verifierApproved !== true) throw new Error(`${label} — verifierApproved is not true (got ${prov.verifierApproved})`);
  if (!prov.freshness?.fetchedAt) throw new Error(`${label} — freshness.fetchedAt missing`);
  const ts = new Date(prov.freshness.fetchedAt);
  if (isNaN(ts.getTime())) throw new Error(`${label} — freshness.fetchedAt is not a valid ISO timestamp (got "${prov.freshness.fetchedAt}")`);
  if (!prov.attestation.includes("NEXUS-VERIFIER")) throw new Error(`${label} — attestation missing NEXUS-VERIFIER (got "${prov.attestation}")`);
  ...
  if (!Array.isArray(prov.sources) || prov.sources.length === 0) throw new Error(`${label} — sources array empty`);
}
...
function mockRes(): MockResponse {
  const res: MockResponse = {
    ...
    send(data) { if (typeof data === "object") this.body = data as Record<string, unknown>; return this; },
  };
  return res;
}
...
const OWNERS = [
  { id: "OWN-001", name: "Pacific Maritime Holdings", country: "GR", riskTier: "low" as const, sanctionExposure: false },
  { id: "OWN-004", name: "Apex Maritime Ltd", country: "MH", riskTier: "high" as const, sanctionExposure: true },
  { id: "OWN-006", name: "Eastbound Logistics SA", country: "LR", riskTier: "high" as const, sanctionExposure: true },
];
...
const CARGOES = [
  { id: "CGO-001", label: "Crude Oil", category: "crude_oil", tonnesRange: [80000, 280000] as [number, number], hazardClass: "IMO Class 3" },
  { id: "CGO-002", label: "Containers", category: "general_cargo", tonnesRange: [20000, 80000] as [number, number] },
];

const VESSELS = [
  { imo: "9234567", name: "Pacific Guardian", type: "VLCC", flag: "LR", ownerId: "OWN-004", chartererId: "CHR-001", cargoId: "CGO-001", ports: ["Rotterdam", "Fujairah"] },
  { imo: "9456789", name: "Liberty Wave", type: "Container", flag: "PA", ownerId: "OWN-001", chartererId: "CHR-002", cargoId: "CGO-002", ports: ["Shanghai"] },
];

function buildOwnerGraph(): { nodes: GraphNode[]; edges: GraphEdge[]; stats: Record<string, unknown>; provenance: Provenance } {
  ...
  return {
    nodes,
    edges,
    stats: { totalNodes: nodes.length, totalEdges: edges.length, byType, uniqueCargoTypes: byType.cargo },
...
const OWNERSHIP_CHAINS: Record<string, OwnershipHop[]> = {
  "9234567": [
    ...
    { hopIndex: 2, sanctioned: true, sanctionListMatches: ["OFAC SDN", "EU Consolidated List"], confidence: 0.91 },
    { hopIndex: 3, sanctioned: true, sanctionListMatches: ["OFAC SDN", "UN Security Council"], confidence: 0.84 },
  ],
};

const DEFAULT_CHAIN: OwnershipHop[] = [
  { hopIndex: 0, sanctioned: false, confidence: 0.70 },
  { hopIndex: 1, sanctioned: false, confidence: 0.82 },
];

function buildSanctionsChain(vesselImo: string): { chain: OwnershipHop[]; analysis: Record<string, unknown>; provenance: Provenance } {
  ...
  return {
    ...
    provenance: provenance(["OFAC SDN List", "EU Consolidated List", "UN Security Council"], 0.92, true),
  };
}

const COUNTERPARTIES = [
  { id: "CP-001", name: "Vitol SA", riskTier: "low", riskScore: 20, totalExposureUsd: 48_000_000, overdueAmount: 0 },
  { id: "CP-006", name: "Pacific Coal Resources", riskTier: "critical", riskScore: 85, totalExposureUsd: 8_200_000, overdueAmount: 820_000 },
];

function buildCounterpartyRisk(): { counterparties: typeof COUNTERPARTIES; portfolio: Record<string, unknown>; provenance: Provenance } {
  ...
  return {
    ...
    provenance: provenance(["Baltic Exchange", "S&P Global CreditWatch"], 0.90, true),
  };
}

const VOYAGE_SNAPSHOTS = [
  { snapshotId: "S001", timestamp: "2026-03-01T06:00:00Z", speed: 14.2, fuelConsumed: 0, cargoIntact: true, etaOriginal: "2026-03-18T14:00:00Z", etaCurrent: "2026-03-18T14:00:00Z" },
  { snapshotId: "S008", timestamp: "2026-03-19T06:30:00Z", speed: 0, fuelConsumed: 3248, cargoIntact: true, status: "moored", etaOriginal: "2026-03-18T14:00:00Z", etaCurrent: "2026-03-19T06:30:00Z" },
];
...
function buildVoyageTwin(): { snapshots: typeof VOYAGE_SNAPSHOTS; whatIfScenarios: typeof WHAT_IF_SCENARIOS; timeline: Record<string, unknown>; provenance: Provenance } {
  return {
    snapshots: VOYAGE_SNAPSHOTS,
    whatIfScenarios: WHAT_IF_SCENARIOS,
    timeline: { voyageDurationDays: 18.0 },
    provenance: provenance(["Voyage Data Recorder", "AIS Track Archive", "NEXUS Voyage Twin"], 0.93, true),
  };
}
...
async function run(): Promise<void> {
  ...
  await check("route-anomalies: provenance/verifier/freshness", () => {
    const p = provenance(["AIS Digitraffic", "Voyage Plan Registry", "GMDSS", "NEXUS Route Memory"], 0.91, true);
  ...
  await check("sanctions-chain/9234567: sanctionedHops > 0", () => {
    const result = buildSanctionsChain("9234567");
    if ((result.analysis.sanctionedHops as number) === 0) throw new Error("Expected sanctioned hops > 0");
  });
  ...
  await check("sanctions-chain: averageConfidence in [0,1]", () => {
    ...
    if (typeof avg !== "number" || avg < 0 || avg > 1) throw new Error(`averageConfidence out of range: ${avg}`);
  });
  ...
  await check("counterparty-risk: sorted by riskScore descending", () => {
    ...
    for (let i = 0; i < result.counterparties.length - 1; i++) {
      if (result.counterparties[i].riskScore < result.counterparties[i + 1].riskScore) {
        throw new Error(`Not sorted: index ${i} (score ${result.counterparties[i].riskScore}) < index ${i + 1} (${result.counterparties[i + 1].riskScore})`);
      }
    }
  });
  ...
  await check("voyage-twin: snapshots non-empty", () => {
    ...
    if (!result.snapshots || result.snapshots.length === 0) throw new Error("snapshots array empty");
  });

  await check("voyage-twin: whatIfScenarios non-empty", () => {
    const result = buildVoyageTwin();
    if (!result.whatIfScenarios || result.whatIfScenarios.length === 0) throw new Error("whatIfScenarios array empty");
  });

  await check("voyage-twin: timeline.voyageDurationDays is a number", () => {
    const result = buildVoyageTwin();
    if (typeof result.timeline.voyageDurationDays !== "number") throw new Error("voyageDurationDays missing or not a number");
  });
