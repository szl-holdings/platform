...
function runSeedScript(name: string, scriptName: string): SeedResult {
  ...
  try {
    ...
    const skipped = /already (exists|seeded|populated|present)/i.test(output) && !output.includes("Inserted");
...
function printResult(r: SeedResult) {
  ...
  const suffix = r.status === "seeded" && r.details && Object.keys(r.details).length > 0
    ? ` (${Object.entries(r.details).map(([k, v]) => `${k}: ${v}`).join(", ")})`
    : r.status === "skipped" ? " [already seeded]"
