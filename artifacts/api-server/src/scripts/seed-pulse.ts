...
const RISK_WEIGHTS = [0.05, 0.12, 0.28, 0.40, 0.15];
...
function conf(base: number, variance: number): string {
  return String(Math.min(0.99, Math.max(0.50, base + (Math.random() - 0.5) * variance)).toFixed(2));
}
  ...
  return [
    {
      ...
      keyFindings: [
        { id: `f-${day}-1`, title: "Payment API IDOR finding — remediation owner assigned", severity: "critical", sourceId: `firestorm-finding-${day}` },
        { id: `f-${day}-2`, title: "Azure AD conditional access gap — MFA enforcement pending", severity: "high", sourceId: "finding-azure-ad-001" },
        { id: `f-${day}-3`, title: "Jenkins CI pipeline secret exposure — secrets rotated", severity: "high", sourceId: "finding-jenkins-001" },
      ],
      signals: [
    ...
    },
    {
      ...
      keyFindings: [
        { id: `f-v-${day}-1`, title: "AIS dark period resolved — IMO 9876543", severity: day % 7 === 0 ? "high" : "info", sourceId: "vessel-imo-9876543" },
        { id: `f-v-${day}-2`, title: "Red Sea alternate route cost tracking active", severity: "medium", sourceId: "vessel-routing-q2" },
      ],
    ...
    },
    {
      ...
      keyFindings: [
        { id: `f-t-${day}-1`, title: "234 W 145th St auction — acquisition window", severity: "high", sourceId: "dp-seed-002" },
        { id: `f-t-${day}-2`, title: "Tax-lien cluster — Flushing/Queens corridor", severity: "medium", sourceId: "dp-seed-006" },
      ],
    ...
    },
    {
      ...
      keyFindings: [
        { id: `f-l-${day}-1`, title: "Vessels ARR $200K above plan — Q2 tracking", severity: "info", sourceId: "lyte-signal-vessels-arr" },
        { id: `f-l-${day}-2`, title: "PRISM document review latency spike — monitoring", severity: "medium", sourceId: "lyte-signal-prism-latency" },
      ],
...
function makeRecommendedActions(day: number) {
  return [
    ...
    },
    {
      ...
      priority: "medium",
      domain: "vessels",
      action: "Confirm Red Sea re-routing cost authorization for MV Atlantic Voyager — $0.8M impact",
  ...
  for (let day = 30; day >= 0; day--) {
    ...
    for (const domain of (["aegis", "vessels", "terra", "lyte"] as const)) {
      ...
      execBriefs.push({
        ...
        whatWeBelieve: [
          { claim: `${domain} operations nominal — no systemic anomalies detected`, confidence: 0.91 },
          { claim: "Cross-domain entity graph consistent with last verified snapshot", confidence: 0.87 },
        ],
        whyCitations: [
          { sourceId: `cst-node-${domain}-primary`, sourceLabel: `Constellation — ${domain} primary node`, type: "constellation_entity" },
          { sourceId: `signal-${domain}-${day}`, sourceLabel: "Platform signal bus", type: "platform_signal" },
        ],
        whatWeRecommend: [
          { action: `Continue monitoring ${domain} — no immediate escalation required`, priority: "routine", autonomy: "autonomous_with_log" },
        ],
        sourceTraceIds: [`trace-${domain}-${day}-001`],
        entityProvenance: [{ entityId: `cst-${domain}-001`, domain, resolvedAt: daysAgo(day).toISOString() }],
        sections: [],
        scheduled: true,
        generatedAt: daysAgo(day),
        createdAt: daysAgo(day),
  ...
  const dissents: (typeof pulseDissentsTable.$inferInsert)[] = [
    {
      ...
      dissentingView: "The 18-hour AIS dark period for IMO 9876543 should remain open for secondary screening. Route consistency alone is insufficient — vessel was in proximity to sanctioned entity at waypoint 3.",
      basis: "OFAC Consolidated Sanctions List cross-reference against last known port call. Historical pattern analysis shows 3 similar events in 24 months.",
      impactIfCorrect: "Potential OFAC compliance breach. Cargo manifest would require independent verification before port clearance.",
      filedBy: "Vessels Intelligence Analyst — M. Rodriguez",
      filedAt: daysAgo(5),
      status: "acknowledged",
      resolution: "Secondary review completed — no sanctions match confirmed. IMO cleared after OFAC API re-query.",
    ...
    },
    {
      ...
      dissentingView: "Opportunity score of 92 for 234 W 145th St is overstated. Comparable Harlem transactions in Q4 2025 show 8-12% discounts to estimated value — model inputs may be stale.",
      basis: "REBNY comparables database pulled 2026-04-05. Cap rate compression in upper Manhattan has reversed since Q3 2025.",
      impactIfCorrect: "Acquisition model overpays by $400-600K. Projected IRR drops from 18% to 12%.",
      filedBy: "Terra Acquisition Analyst — K. Wilson",
      filedAt: daysAgo(12),
      status: "resolved",
      resolution: "Model refreshed with Q1 2026 comps. Estimated value adjusted to $3.95M. Opportunity score revised to 88. Acquisition thesis intact.",
    ...
    },
    {
      ...
      dissentingView: "Ransomware precursor classification is premature. T1059 pattern match rate was 67% — below our 80% threshold for automatic playbook trigger.",
      basis: "Internal threshold policy: PSP-2024-07 requires ≥80% confidence for autonomous playbook activation. Current run was at 67%.",
      impactIfCorrect: "Playbook triggered 14 minutes of unnecessary service interruption on staging cluster. False positive rate increasing — model needs retraining.",
    ...
    },
    {
      ...
      dissentingView: "Vessels NRR reported at 127% is based on trailing 6 months — TTM shows 119%. Executive deck should show TTM to avoid overstating expansion trajectory.",
      basis: "NRR calculation methodology not standardized across portfolio. Trailing 6-month vs TTM diverge when a large renewal occurred in month 8.",
      impactIfCorrect: "Investor deck NRR overstated by 8pp — material misrepresentation risk in upcoming roadshow materials.",
      filedBy: "Lyte Finance Controller — L. Nguyen",
      filedAt: daysAgo(18),
      status: "resolved",
      resolution: "Metric corrected to TTM basis. All investor materials updated. NRR reported as 119% TTM.",
    ...
    },
    {
      ...
      sectionTitle: "Security & Threat Intelligence",
      dissentingView: "Payment API IDOR remediation timeline of 7 days is too long given CVSS 9.3. SZL security policy PSP-2024-03 mandates critical findings patched within 48 hours.",
      basis: "PSP-2024-03: Critical (CVSS ≥9.0) findings must be remediated and verified within 48 hours of confirmation.",
      impactIfCorrect: "Non-compliance with internal security SLA. Potential breach window remains open 5 additional days beyond policy mandate.",
  ...
  const customBriefs: (typeof pulseCustomBriefsTable.$inferInsert)[] = [
    {
      ...
      scenario: "Ongoing Houthi attacks in Red Sea forcing Atlantic re-routing — cost impact and timeline analysis",
    ...
    },
    {
      ...
      scenario: "Rising interest rates and Q2 auction calendar — identify top 10 acquisition targets by IRR",
      domains: ["terra", "lyte"],
    ...
    },
    {
      ...
      scenario: "If Azure AD admin account compromised — what lateral movement paths exist to critical systems?",
      domains: ["aegis"],
    ...
    },
    {
      ...
      scenario: "Is demand letter generation pace consistent with Q2 settlement targets? What are the bottlenecks?",
      domains: ["lyte"],
