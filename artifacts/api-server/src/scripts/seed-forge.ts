...
export async function seedForge() {
  ...
  const policies = await db
    .insert(forgePolicyPacksTable)
    .values([
      {
        slug: 'low-risk-internal',
        name: 'Low Risk — Internal',
        description: 'Default pack for internal-only low-risk agents',
        riskTier: 'low',
        rules: { requireApprovalActions: [], denyEnvs: [] },
      },
      {
        slug: 'standard-prod-gate',
        name: 'Standard — Production Gate',
        description: 'Standard production promotions require human approval',
        riskTier: 'standard',
        rules: { requireApprovalActions: ['promote'], denyEnvs: [] },
      },
      {
        slug: 'regulated-finance',
        name: 'Regulated — Finance',
        description: 'Finance/insurance agents — strict approval and execution gates',
        riskTier: 'regulated',
        rules: { requireApprovalActions: ['promote', 'modify', 'execute'], denyEnvs: [] },
      },
      {
        slug: 'executive-briefings',
        name: 'Executive — Briefings',
        description: 'Executive-tier agents — promotion + rollback approval required',
        riskTier: 'executive',
        rules: { requireApprovalActions: ['promote', 'rollback', 'modify'], denyEnvs: [] },
      },
    ])
    .returning();
  ...
  const targets = await db
    .insert(forgeDeploymentTargetsTable)
    .values([
      {
        slug: 'replit-edge',
        name: 'Replit Edge',
        kind: 'replit',
        region: 'us-east',
        computeProfile: { cpu: 'shared', memMb: 512 },
        packageLock: {},
        requiredSecrets: [],
        storageDeps: ['postgres'],
      ...
      },
      {
        ...
        kind: 'vercel',
        region: 'iad1',
        computeProfile: { runtime: 'edge', memMb: 256 },
        packageLock: {},
        requiredSecrets: [],
        storageDeps: ['postgres', 'redis'],
        allowedIntegrations: ['openai'],
      },
      {
        slug: 'fargate-regulated',
        name: 'AWS Fargate (Regulated)',
        kind: 'aws-fargate',
        region: 'us-east-1',
        computeProfile: { vcpu: 1, memMb: 2048 },
        packageLock: {},
        requiredSecrets: [],
        storageDeps: ['postgres', 'kms'],
        complianceNotes: 'SOC2 + HIPAA + PCI',
        allowedIntegrations: [],
      },
    ])
    .returning();
  ...
  const envs = await db
    .insert(forgeEnvironmentProfilesTable)
    .values([
      {
        slug: 'dev',
        name: 'Development',
  ...
  const models = await db
    ...
    .values([
      ...
      {
        slug: 'experimental-llama-405b',
        name: 'Llama 3.1 405B (experimental)',
        provider: 'meta',
        family: 'llama',
        contextWindow: 128000,
  ...
  const tools = await db
    .insert(forgeToolsTable)
    .values([
      {
        slug: 'search-knowledge',
        name: 'Knowledge Search',
        description: 'Reads from RAG knowledge base',
        category: 'read',
        riskLevel: 'low',
        approved: true,
        schema: {},
      },
      {
        slug: 'draft-document',
        name: 'Draft Document',
        description: 'Generates draft documents (read-only)',
        category: 'write',
        riskLevel: 'low',
        approved: true,
        schema: {},
      },
      {
        slug: 'execute-trade',
        name: 'Execute Trade',
        description: 'Submits trade orders to broker',
        category: 'execute',
        riskLevel: 'high',
        approved: true,
        schema: {},
      ...
      {
        slug: 'experimental-fetch',
        name: 'Experimental Web Fetch',
        description: 'Untrusted browsing — pending review',
        category: 'read',
        riskLevel: 'high',
        approved: false,
        schema: {},
  ...
  const prompts = await db
    .insert(forgePromptsTable)
    .values([
      {
        slug: 'executive-briefer-system',
        name: 'Executive Briefer — System',
        purpose: 'Daily exec briefing',
        tags: ['executive'],
      },
      {
      ...
      {
        slug: 'vessels-ops-router',
        name: 'Vessels — Ops Router',
        purpose: 'Maritime ops routing',
        tags: ['maritime'],
      },
      {
        slug: 'carlota-concierge',
        name: 'Carlota — Concierge',
  ...
  for (const p of prompts) {
    const [v1] = await db
      .insert(forgePromptVersionsTable)
      .values({
        promptId: p.id,
        version: 1,
        body: `# ${p.name}\nv1 baseline prompt.`,
  ...
  const agentSpecs = [
    {
      slug: 'executive-briefer',
      name: 'Executive Briefer',
      domain: 'executive',
      riskTier: 'executive',
      policy: 'executive-briefings',
      env: 'production',
      model: 'claude-3-opus',
      prompt: 'executive-briefer-system',
      tools: ['search-knowledge', 'draft-document'],
    },
    {
      slug: 'matter-twin-analyst',
      name: 'Matter Twin Analyst',
      domain: 'legal',
      riskTier: 'regulated',
      policy: 'regulated-finance',
      env: 'staging',
      model: 'claude-3-5-sonnet',
      prompt: 'matter-twin-analyst',
      tools: ['search-knowledge', 'draft-document'],
    },
    {
      slug: 'vessels-ops-router',
      ...
      domain: 'maritime',
      riskTier: 'standard',
      policy: 'standard-prod-gate',
      env: 'production',
      model: 'gpt-4o',
      prompt: 'vessels-ops-router',
    ...
    },
    {
      ...
      prompt: 'carlota-concierge',
      tools: ['search-knowledge', 'draft-document'],
    },
    {
      slug: 'trade-executor-bot',
      name: 'Trade Executor (sandbox)',
      domain: 'finance',
      riskTier: 'regulated',
      policy: 'regulated-finance',
      env: 'dev',
      model: 'claude-3-5-sonnet',
      prompt: 'matter-twin-analyst',
      tools: ['execute-trade', 'search-knowledge'],
    },
  ] as const;
  ...
  for (const s of agentSpecs) {
    const [agent] = await db
      .insert(forgeAgentsTable)
      .values({
        slug: s.slug,
        name: s.name,
        description: `${s.name} (seeded demo agent)`,
        domain: s.domain,
        riskTier: s.riskTier,
        currentEnv: s.env,
        status: 'active',
    ...
    const [v1] = await db
      ...
      .values({
        ...
        systemConfig: {},
        evalsPassed: true,
        observabilityHookConfigured: true,
  ...
  for (const env of envs) {
    ...
    for (const c of created) {
      ...
      const useV1 =
        (env.tier === 'staging' && c.spec.env === 'staging') ||
  ...
  const driftSeeds: Array<{
    agentSlug: string;
    envTier: string;
    severity: 'low' | 'medium' | 'high' | 'critical';
    dimension: 'model' | 'prompt' | 'tool' | 'data' | 'config';
    score: number;
    remediation: string;
  }> = [
    {
      agentSlug: 'matter-twin-analyst',
      envTier: 'staging',
      severity: 'medium',
      dimension: 'prompt',
      score: 42,
      remediation: 'Re-deploy v2 prompt to staging',
    },
    {
      agentSlug: 'executive-briefer',
      envTier: 'production',
      severity: 'low',
      dimension: 'config',
      score: 18,
      remediation: 'Refresh env snapshot',
    },
    {
      agentSlug: 'trade-executor-bot',
      envTier: 'sandbox',
      severity: 'high',
      dimension: 'tool',
      score: 72,
      remediation: 'Restore execute-trade tool permission in sandbox',
    },
    {
      agentSlug: 'vessels-ops-router',
      envTier: 'production',
      severity: 'critical',
      dimension: 'model',
      score: 88,
      remediation: 'Freeze production traffic; investigate model regression',
    },
  ];
  ...
  for (const c of created) {
    ...
    await db.insert(forgePromotionsTable).values({
      agentId: c.agent.id,
      fromVersionId: c.v1.id,
      toVersionId: c.v2.id,
      fromEnv: 'dev',
      toEnv: 'sandbox',
      status: 'promoted',
      promotedAt: daysAgo(7),
      blockers: [],
  ...
  await db.insert(forgePromotionsTable).values([
    ...
    },
    {
      ...
      blockers: [
        {
          code: 'DRIFT_OVER_THRESHOLD',
          message: 'Drift score 88 exceeds threshold 60 for production',
        },
      ],
  ...
  for (const c of created) {
    for (let i = 0; i < 6; i++) {
      ...
      execRows.push({
        ...
        versionId: c.v2.id,
        envTier: c.spec.env,
        modelId: c.v2.modelId,
        promptVersionId: c.v2.promptVersionId,
        ...
        valueAtRiskUsd:
          c.spec.domain === 'finance' ? String(50000 + Math.floor(Math.random() * 200000)) : null,
        provenance: {
          modelId: c.v2.modelId,
          promptVersionId: c.v2.promptVersionId,
          capturedAt: daysAgo(i).toISOString(),
        },
        isSeed: true,
        startedAt: daysAgo(i),
