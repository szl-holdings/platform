...
async function post(path: string, payload: unknown, withAuth = true): Promise<{ status: number; body: unknown }> {
...
async function testOwnershipGraph() {
  ...
  assert("graph.nodes is array", Array.isArray((b?.graph as Record<string, unknown> | null)?.nodes));
  assert("graph.edges is array", Array.isArray((b?.graph as Record<string, unknown> | null)?.edges));
  ...
  assert("summary.ultimateBeneficialOwners is array", Array.isArray(summary?.ultimateBeneficialOwners));
  assert("summary.combinedLtv is number or null", summary?.combinedLtv === null || typeof summary?.combinedLtv === "number");
...
async function testLenderExposure() {
  ...
  assert("graphStats present", typeof b?.graphStats === "object", "CONSTELLATION integration block missing");
...
async function testCovenants() {
  ...
  assert("summary present", typeof b?.summary === "object");
  assert("scheduledSkill present", typeof b?.scheduledSkill === "object", "scheduled skill block missing");
  const sk = b?.scheduledSkill as Record<string, unknown> | null;
  assert("scheduledSkill.agentId = terra-covenant-monitor", sk?.agentId === "terra-covenant-monitor");
...
async function testCovenantSubmitReview() {
  ...
  const noAuth = await post("/api/terra/cognitive/covenants/submit-review", {
    propertyId: "smoke-prop-001",
    covenantType: "dscr",
  }, false);
  assert("no-auth rejected (401 or 403)", noAuth.status === 401 || noAuth.status === 403, `got ${noAuth.status}`);

  if (!TOKEN) {
    console.warn("[terra-cog] ⚠  No ALLOY_INTERNAL_TOKEN — skipping authenticated submit-review test");
    return;
  }
  ...
  assert("re-submit alreadyExisted=true", r2?.alreadyExisted === true, `alreadyExisted=${r2?.alreadyExisted}`);
...
async function testUnderwritingCopilot() {
  ...
  const { status, body } = await get("/api/terra/cognitive/underwriting-copilot?propertyId=test-001");
  const b = body as Record<string, unknown>;
  assert("status 200", status === 200, `got ${status}`);
