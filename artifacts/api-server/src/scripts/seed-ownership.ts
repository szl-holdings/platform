...
export async function seedOwnership() {
  ...
  const existingAllocCount = fullySeeded
    ? (await db.select({ id: ownershipAllocationsTable.id }).from(ownershipAllocationsTable).limit(1)).length
    : 0;
  ...
  if (existingScenarios.length > 0) {
    ...
    console.log(`[seed-ownership] Partial seed detected (${existingScenarios.length} scenarios) — rebuilding.`);
    await db.delete(ownershipAllocationsTable).where(inArray(ownershipAllocationsTable.scenarioId, ids));
  ...
  const scenarios = await db.insert(ownershipScenariosTable).values([
    {
      name: "Baseline — Founding Structure (Pre-Seed)",
      description: "Original founding equity split before any external capital. Used as reference point for all dilution modeling.",
      isTemplate: false,
      isActive: false,
      isPreferred: false,
      status: "approved",
      certificationFitSummary: "Simple two-founder structure. Sole citizenship confirmed for both founders. Meets SBA size standards — no foreign ownership concerns.",
      fundraisingFitScore: 62,
      bankFitScore: 78,
      investorClarityScore: 84,
      notes: "Pre-seed baseline. Not the current operative structure — retained for cap table history.",
      metadata: {
        version: 1,
    ...
    },
    {
      ...
      description: "Current capitalization post-Seed round. Reflects $2.1M raised at $8M pre-money valuation. Primary scenario used for all regulatory filings and bank applications.",
      isTemplate: false,
      isActive: true,
      isPreferred: true,
      status: "approved",
      certificationFitSummary: "Majority ownership (51%) held by U.S. citizen and SBA-disadvantaged individual. Minority investors hold 22% combined, all with passive rights only. Structure meets SBA 8(a) and WOSB certification eligibility thresholds.",
      fundraisingFitScore: 85,
      bankFitScore: 91,
      investorClarityScore: 88,
      notes: "This is the operative structure. All bank applications, LP decks, and SBA certification filings use Scenario A.",
      metadata: {
        version: 4,
    ...
    },
    {
      ...
      description: "Pro forma cap table assuming $8M Series A at $32M pre-money. Models dilution impact and post-money ownership for LP pitch materials.",
      isTemplate: false,
      isActive: false,
      isPreferred: false,
      status: "under_review",
      certificationFitSummary: "Post-Series A majority ownership drops to 47.2% — falls below SBA 8(a) 51% threshold. WOSB certification would be at risk. Recommend structuring investor rights to preserve certification eligibility.",
      fundraisingFitScore: 92,
      bankFitScore: 73,
      investorClarityScore: 95,
      notes: "Series A model. Currently under review by outside counsel. Do not use for external distribution until approved.",
      metadata: {
        version: 2,
    ...
    },
    {
      ...
      description: "Variant of Scenario A with expanded employee option pool from 12% to 18% for Series A recruiting competitiveness. Models double-trigger acceleration and 4-year vesting.",
      isTemplate: false,
      isActive: false,
      isPreferred: false,
      status: "draft",
      certificationFitSummary: "ESOP expansion does not affect certification — unissued options excluded from certification ownership calculation. Certification eligibility maintained.",
      fundraisingFitScore: 80,
      bankFitScore: 88,
      investorClarityScore: 82,
      notes: "Created for Series A negotiations. Contingent on Series A closing. Not yet board-approved.",
      metadata: {
        version: 1,
    ...
    },
    {
      ...
      description: "Reusable template demonstrating 8(a) certification-compliant ownership structure. Reference document for structuring future capital raises.",
      isTemplate: true,
      isActive: false,
      isPreferred: false,
      status: "approved",
      certificationFitSummary: "Template meets all SBA 8(a) requirements: ≥51% ownership by socially and economically disadvantaged individual(s), U.S. citizenship, unconditional ownership, management control.",
      fundraisingFitScore: 70,
      bankFitScore: 90,
      investorClarityScore: 75,
      notes: "Use as reference when structuring new capital raises to preserve certification eligibility.",
  ...
  const allocations: (typeof ownershipAllocationsTable.$inferInsert)[] = [
    // Scenario A — Current
    {
      ...
      notes: "Founder and CEO. SBA-disadvantaged individual certification filed with SBA June 2024. Unconditional ownership — no put/call provisions.",
    },
    {
      scenarioId: scenarioA.id,
      ...
      citizenshipConfirmed: true,
      notes: "Co-founder and CTO. U.S. citizen. Standard co-founder provisions — no disproportionate rights.",
    },
    {
      scenarioId: scenarioA.id,
      ...
      notes: "Lead seed investor. Standard protective provisions: liquidation preference 1x non-participating, pro-rata rights, information rights. No board seat at seed stage.",
    },
    {
      scenarioId: scenarioA.id,
    ...
    // Scenario B — Series A
    {
      ...
      isMajorityOwner: false,
      citizenshipConfirmed: true,
      notes: "Post Series A — diluted from 51%. Certification risk flagged. Review board control structure.",
    },
    {
      scenarioId: scenarioB.id,
      personName: "Co-Founder",
    ...
    // Scenario C
    {
      ...
      isMajorityOwner: false,
      citizenshipConfirmed: true,
      notes: "ESOP expansion variant — option pool increases dilute founder slightly. Certification maintained.",
    },
    {
      scenarioId: scenarioC.id,
      personName: "Co-Founder",
  ...
  const controlRoles: (typeof controlRolesTable.$inferInsert)[] = [
    {
      ...
      controlDescription: "CEO and Managing Director. Exercises full day-to-day operational control. Board chairman. Strategic decisions require CEO signature. Hiring/firing authority up to VP-level.",
    },
    {
      scenarioId: scenarioA.id,
      personName: "Co-Founder",
      ...
      controlDescription: "CTO — exercises day-to-day control over engineering and product only. No strategic or financial veto rights. Reports to CEO.",
    },
    {
      scenarioId: scenarioB.id,
      personName: "Stephen Lewis",
      ...
      hasHiringFiringAuthority: true,
      hasStrategicVeto: false,
      controlDescription: "Post Series A — CEO authority maintained for day-to-day. Board composition changes: 2 founders + 1 Series A investor + 1 independent. Strategic decisions now require board majority.",
    },
    {
      scenarioId: scenarioB.id,
      personName: "Series A Investor",
      ...
      hasHiringFiringAuthority: false,
      hasStrategicVeto: true,
      controlDescription: "Board seat holder. Standard protective provisions: veto on M&A above threshold, sale of substantially all assets, additional equity issuance.",
    },
  ];
