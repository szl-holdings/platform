...
export async function publishArticleToMedium(
  ...
  const [article] = await db
    ...
    .where(eq(dosArticlesTable.id, articleId));
  if (!article) return { ok: false, error: 'Article not found' };
  if (article.externalUrlMedium) {
    return {
      ok: true,
      externalUrl: article.externalUrlMedium,
      skipped: true,
  ...
  if (opts.enforceReady) {
    ...
    if (article.mediumStatus !== 'ready') {
      return {
        ok: false,
        skipped: true,
        reason: `medium-not-ready (mediumStatus=${article.mediumStatus})`,
      };
    }
  }
...
export async function publishNewsletterToSubstack(
  ...
  const [nl] = await db
    ...
    .where(eq(dosNewslettersTable.id, newsletterId));
  ...
  if (opts.enforceReady) {
    ...
    if (nl.substackStatus !== 'ready') {
      return {
        ok: false,
        skipped: true,
        reason: `substack-not-ready (substackStatus=${nl.substackStatus})`,
      };
    }
  }
...
export async function publishCarouselToLinkedIn(
  ...
  const [carousel] = await db
    ...
    .where(eq(dosCarouselProjectsTable.id, carouselId));
  ...
  const caption =
    ...
    carousel.linkedinLongCaption ||
    `${carousel.title} — by SZL Holdings`;
 ...
 */
export async function findDuePublishCandidates(
  now: Date = new Date(),
  ...
  const dueXPosts = await db
    .select({ id: dosXPostsTable.id })
    .from(dosXPostsTable)
    .where(
      and(
        or(
    ...
    switch (item.contentType) {
      case 'article':
        candidates.push({
          kind: 'article-medium',
          id: item.contentId,
          source: 'calendar-item',
          calendarItemId: item.id,
        });
        break;
      case 'newsletter':
        candidates.push({
          kind: 'newsletter-substack',
          id: item.contentId,
          source: 'calendar-item',
          calendarItemId: item.id,
        });
 ...
 */
export async function runLaunchPublishScheduler(
  opts: { now?: Date } = {},
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      logger.warn(
        { candidate, err, attempts: entry.attempts },
        '[launch-publish-scheduler] publish threw',
      );
      continue;
    }
    ...
    if (outcome.skipped) {
      ...
      logger.warn(
        { candidate, error: outcome.error, attempts: entry.attempts },
        '[launch-publish-scheduler] publish failed',
      );
    }
    ...
    if (candidate.calendarItemId != null) {
      try {
        ...
        logger.warn(
          { err, calendarItemId: candidate.calendarItemId },
          '[launch-publish-scheduler] failed to update calendar item',
        );
      }
    }
    ...
    if (!outcome.ok && !outcome.skipped) {
      ...
      if (entry && entry.attempts >= MAX_PERMANENT_FAIL_ATTEMPTS) {
        try {
          ...
          logger.warn(
            { err, candidate },
            '[launch-publish-scheduler] failed to flip terminal status',
          );
        }
      }
    }
  ...
  try {
    await db.insert(dosAutomationRunsTable).values({
      ...
      errorLog: result.failures.length
        ? result.failures
            .map((f) => `${f.kind}#${f.id} (attempt ${f.attempts}): ${f.error}`)
            .join('\n')
        : null,
    });
  } catch (err) {
