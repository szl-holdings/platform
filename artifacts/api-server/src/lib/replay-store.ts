...
function mapScenario(r: Record<string, unknown>): ScenarioRow {
  return {
    ...
    groundTruthMatchRate: r["ground_truth_match_rate"] != null ? Number(r["ground_truth_match_rate"]) : null,
...
export async function getScenario(scenarioId: string): Promise<ScenarioRow | null> {
  ...
  try {
    const result = await pool.query("SELECT * FROM replay_scenarios WHERE scenario_id = $1", [scenarioId]);
    ...
    const result = await pool.query(
      `INSERT INTO replay_scenarios (scenario_id, name, domain, description, tags, snapshot_count, created_at, updated_at)
       ...
       ON CONFLICT (scenario_id) DO UPDATE SET
         ...
         updated_at = NOW()
       RETURNING *`,
      [scenario.scenarioId, scenario.name, scenario.domain, scenario.description, scenario.tags, scenario.snapshotCount],
    );
    return mapScenario(result.rows[0] as Record<string, unknown>);
  } catch (err) {
    logger.warn({ err, scenarioId: scenario.scenarioId }, "[replay-store] Failed to upsert scenario");
    return null;
  }
}

export async function seedScenariosIfEmpty(scenarios: Array<{
  scenarioId: string;
  name: string;
  domain: string;
  description: string;
    ...
    try {
      ...
      for (const s of scenarios) {
        await client.query(
          ...
          [s.scenarioId, s.name, s.domain, s.description, s.tags, s.snapshotCount, s.lastReplayed ?? null, s.lastOutcome ?? null, s.groundTruthMatchRate ?? null],
        );
      }
...
export async function persistRun(run: {
  ...
  const insertResult = await pool.query(
    ...
    [run.runId, run.scenarioId, run.scenarioName, run.startedAt, run.completedAt, run.totalSnapshots, run.successful, run.failed, run.avgLatencyMs, run.groundTruthMatchRate, run.totalCostUsd],
  );
  ...
  if (insertResult.rowCount && insertResult.rowCount > 0) {
    await pool.query(
      ...
      [
        run.startedAt,
        run.successful === run.totalSnapshots ? "pass" : run.failed === run.totalSnapshots ? "fail" : "partial",
...
export async function persistEvalBaseline(baseline: {
  ...
  try {
    await pool.query(
      ...
      [baseline.suiteId, baseline.model, baseline.passRate, baseline.avgScore, baseline.avgLatencyMs, baseline.totalCostUsd, baseline.version, baseline.recordedAt],
    );
  } catch (err) {
    logger.warn({ err, suiteId: baseline.suiteId }, "[replay-store] Failed to persist eval baseline");
  }
}
