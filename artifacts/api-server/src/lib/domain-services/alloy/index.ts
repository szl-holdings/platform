...
export const VALID_WORKFLOW_TYPES = new Set<WorkflowType>([
  'investigation',
  'remediation',
  'escalation',
  'review',
...
export const VALID_WORKFLOW_STATUSES = new Set<WorkflowStatus>([
  'draft',
  'pending',
  'running',
  'waiting_approval',
...
export async function listAlloySignals(
  storage: AlloyStoragePort,
  args: { severity?: string; status?: string; domain?: string; limit?: number; offset?: number },
) {
  return storage.listSignals({
...
export async function listAlloyWorkflows(
  storage: AlloyStoragePort,
  args: { status?: string; priority?: string; domain?: string; limit?: number; offset?: number },
) {
  const statusFilter =
    args.status && VALID_WORKFLOW_STATUSES.has(args.status as WorkflowStatus)
      ? args.status
      : undefined;
  const priorityFilter =
    args.priority && VALID_PRIORITIES.has(args.priority as WorkflowPriority)
      ? args.priority
      : undefined;
  const rows = await storage.listWorkflows({
    status: statusFilter,
...
export async function getAlloyWorkflowStateTransitions(
  storage: AlloyStoragePort,
  workflowId: number,
) {
...
export async function listAlloyWorkflowRuns(
  storage: AlloyStoragePort,
  args: { workflowId?: number; status?: string; limit?: number; offset?: number },
) {
  return storage.listWorkflowRuns({
...
export async function listAlloyApprovals(
  storage: AlloyStoragePort,
  args: { workflowId?: number; status?: string; limit?: number; offset?: number },
) {
  return storage.listApprovals({
    workflowId: args.workflowId,
    status: args.status,
    limit: args.limit ?? 50,
    offset: args.offset ?? 0,
...
export async function listAlloyActions(
  storage: AlloyStoragePort,
  args: { workflowId?: number; limit?: number; offset?: number },
) {
  return storage.listActions({
    workflowId: args.workflowId,
    limit: args.limit ?? 50,
    offset: args.offset ?? 0,
  });
...
export async function listAlloyArtifacts(
  storage: AlloyStoragePort,
  args: { workflowId?: number; domain?: string; limit?: number; offset?: number },
) {
  return storage.listArtifacts({
    workflowId: args.workflowId,
    domain: args.domain,
    limit: args.limit ?? 50,
    offset: args.offset ?? 0,
...
export async function listAlloyAuditLog(
  storage: AlloyStoragePort,
  args: { entityType?: string; entityId?: number; limit?: number; offset?: number },
) {
  const rows = await storage.listAuditLog({
