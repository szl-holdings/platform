...
export function classifyMitreTactic(finding: {
  ...
  if (
    description.includes('exfiltrat') ||
    description.includes('data theft') ||
    description.includes('exfil')
  ) {
  ...
  if (
    ...
    description.includes('scheduled task')
  ) {
...
export async function listFirestormAssessments(
...
export async function listFirestormFindings(
  storage: FirestormStoragePort,
  args: { assessmentId?: number; severity?: string; limit?: number; offset?: number },
) {
  return storage.listFindings({
...
export async function listFirestormIncidents(
  storage: FirestormStoragePort,
  args: { status?: string; severity?: string; limit?: number; offset?: number },
) {
  return storage.listIncidents({
    status: args.status,
    severity: args.severity,
    limit: args.limit ?? 50,
    offset: args.offset ?? 0,
...
export async function updateFirestormIncident(
  ...
  if (
    status === 'escalated' ||
    (updated.severity === 'critical' && previousStatus !== 'critical')
  ) {
...
export async function listFirestormAssets(
  storage: FirestormStoragePort,
  args: { limit?: number; offset?: number },
) {
