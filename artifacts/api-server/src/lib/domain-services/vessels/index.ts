...
export interface VesselsStoragePort {
  ...
  listRoutes(args: { vesselId?: number; status?: string; limit: number; offset: number }): Promise<unknown[]>;
  listEvents(args: { vesselId?: number; severity?: string; limit: number; offset: number }): Promise<unknown[]>;
}
...
export function classifyVesselBehavior(vessel: { vesselType?: string | null; status?: string | null }, events: Array<{ eventType?: string | null; severity?: string | null }>): string {
...
export async function listVessels(
  ...
  return storage.listVessels({ status: args.status, limit: args.limit ?? 50, offset: args.offset ?? 0 });
}
...
export async function listVesselRoutes(
  ...
  return storage.listRoutes({ vesselId: args.vesselId, status: args.status, limit: args.limit ?? 50, offset: args.offset ?? 0 });
}
...
export async function listVesselEvents(
  ...
  return storage.listEvents({ vesselId: args.vesselId, severity: args.severity, limit: args.limit ?? 50, offset: args.offset ?? 0 });
}
