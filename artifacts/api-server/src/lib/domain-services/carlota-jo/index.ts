...
export interface CarlotaJoStoragePort {
  ...
  createInquiry(data: { name: string; email: string; service: string; message: string; status: string }): Promise<unknown>;
}
...
export async function listCarlotaServices(
  ...
  return storage.listServices({ category: args.category, isActive: args.isActive, limit: args.limit ?? 50 });
}
...
export async function listCarlotaReservations(
  ...
  return storage.listReservations({ status: args.status, limit: args.limit ?? 50, offset: args.offset ?? 0 });
}

export async function getCarlotaReservation(
  storage: CarlotaJoStoragePort,
