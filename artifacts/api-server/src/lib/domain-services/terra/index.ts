  ...
  'prop-007': [
    {
      ...
      dueDate: '2026-04-15',
      status: 'in_progress',
      recommendedAction: 'Activate leasing incentive program; engage Compass multifamily team',
    },
    {
      externalId: 'act-002',
      ...
      dueDate: '2026-04-07',
      status: 'open',
      recommendedAction: 'Demand letter sent; escalate to eviction counsel if unpaid by Apr 7',
    },
    {
      externalId: 'act-003',
      ...
      dueDate: '2026-05-01',
      status: 'open',
      recommendedAction: 'Engage lender for maturity extension; simultaneously market for refi',
    },
    {
      externalId: 'act-004',
      ...
      dueDate: '2026-04-30',
      status: 'open',
      recommendedAction: 'Complete scope + bid by Apr 30; include in lender remediation plan',
    },
  ],
  'prop-005': [
    {
      ...
      dueDate: '2026-04-20',
      status: 'open',
      recommendedAction: 'Tenant incentive program — 3 months free rent for 5+ year leases',
    },
    {
      externalId: 'act-006',
      ...
      dueDate: '2026-05-01',
      status: 'open',
      recommendedAction: 'Send renewal proposal with updated market terms',
    },
  ],
  'prop-001': [
    {
      ...
      dueDate: '2026-04-05',
      status: 'in_progress',
      recommendedAction: 'Vendor contracted; work order #WO-2026-0847 active',
    },
    {
      externalId: 'act-008',
      ...
      dueDate: '2026-04-15',
      status: 'open',
      recommendedAction: 'Schedule renewal conversation; assess market rate delta',
    },
  ],
};
...
export async function listTerraProperties(
...
export async function listTerraListings(
  storage: TerraStoragePort,
...
export async function listTerraDistressProperties(
  storage: TerraStoragePort,
  args: { borough?: string; distressType?: string; limit?: number; offset?: number },
) {
  return storage.listDistressProperties({
    borough: args.borough,
    distressType: args.distressType,
    limit: args.limit ?? 50,
    offset: args.offset ?? 0,
  });
}

export async function listTerraDeals(
  storage: TerraStoragePort,
  args: { stage?: string; limit?: number; offset?: number },
) {
  return storage.listDeals({
    stage: args.stage,
    limit: args.limit ?? 50,
    offset: args.offset ?? 0,
  });
...
export async function listTerraLeads(
  storage: TerraStoragePort,
  args: { stage?: string; limit?: number; offset?: number },
) {
...
export async function createTerraLead(
  storage: TerraStoragePort,
  data: { firstName: string; lastName: string; type?: string },
) {
...
export async function listTerraActionItems(
  storage: TerraStoragePort,
  args: { propertyId?: string; status?: string; limit?: number; offset?: number },
) {
  return storage.listActionItems({
