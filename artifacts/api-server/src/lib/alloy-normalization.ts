...
function classifyCategory(domain: string, title: string): string {
  ...
  if (titleLower.includes('threat') || titleLower.includes('attack') || titleLower.includes('vuln'))
    return 'threat';
  if (titleLower.includes('health') || titleLower.includes('slo') || titleLower.includes('latency'))
    return 'health';
