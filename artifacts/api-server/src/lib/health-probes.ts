...
async function probeAuth(): Promise<ProbeResult> {
  ...
  try {
    ...
    const latencyMs = Date.now() - start;
    const status: ProbeStatus = latencyMs > SLOW_THRESHOLD_MS ? "degraded" : "ok";
    return { status, latencyMs, details: "session store query succeeded (probe token not found, expected)" };
  } catch (err) {
    const latencyMs = Date.now() - start;
    const msg = err instanceof Error ? err.message : String(err);
...
async function probeAI(): Promise<ProbeResult> {
  ...
  if (!probeUrl) {
    return { status: "not_configured", details: "API key present but no probeable endpoint (Anthropic/Gemini key-only mode)" };
  }
...
async function probeQueue(): Promise<QueueProbeResult> {
  ...
  try {
    ...
    const status: ProbeStatus =
      depth > 50 ? "degraded" :
      latencyMs > SLOW_THRESHOLD_MS ? "degraded" :
      "ok";
    return {
      status,
      latencyMs,
