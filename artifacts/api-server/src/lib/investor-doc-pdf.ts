...
function renderH1(doc: PDFKit.PDFDocument, text: string): void {
  ...
  doc.moveDown(0.2);
  doc.font("Helvetica-Bold").fontSize(18).fillColor(BRAND.text).text(stripInline(text), MARGIN, doc.y, { width: CONTENT_WIDTH });
  doc.moveDown(0.5);
}
...
function renderH2(doc: PDFKit.PDFDocument, text: string): void {
  ensureSpace(doc, 40);
  doc.moveDown(0.6);
  doc.font("Helvetica-Bold").fontSize(13).fillColor(BRAND.primaryLight).text(stripInline(text), MARGIN, doc.y, { width: CONTENT_WIDTH });
...
function renderH3(doc: PDFKit.PDFDocument, text: string): void {
  ensureSpace(doc, 30);
  doc.moveDown(0.4);
  doc.font("Helvetica-Bold").fontSize(11).fillColor(BRAND.text).text(stripInline(text), MARGIN, doc.y, { width: CONTENT_WIDTH });
  doc.moveDown(0.25);
}
...
function renderH4(doc: PDFKit.PDFDocument, text: string): void {
  ensureSpace(doc, 25);
  doc.moveDown(0.3);
  doc.font("Helvetica").fontSize(9).fillColor(BRAND.muted).text(stripInline(text).toUpperCase(), MARGIN, doc.y, { width: CONTENT_WIDTH, characterSpacing: 1 });
  doc.moveDown(0.2);
}
...
function renderParagraph(doc: PDFKit.PDFDocument, text: string): void {
  ...
  if (!clean) return;
  ensureSpace(doc, 20);
  doc.font("Helvetica").fontSize(10).fillColor(BRAND.text).text(clean, MARGIN, doc.y, { width: CONTENT_WIDTH, lineGap: 2 });
  doc.moveDown(0.35);
}
...
function renderBullet(doc: PDFKit.PDFDocument, text: string, indent = 0): void {
  ...
  const before = doc.y;
  doc.font("Helvetica").fontSize(10).fillColor(BRAND.text).text(clean, textX, before, { width: CONTENT_WIDTH - textX + MARGIN, lineGap: 2 });
  doc.moveDown(0.2);
}
...
function renderNumbered(doc: PDFKit.PDFDocument, text: string, num: number): void {
  ...
  const before = doc.y;
  doc.font("Helvetica").fontSize(9).fillColor(BRAND.muted).text(`${num}.`, MARGIN, before, { width: 18 });
  doc.font("Helvetica").fontSize(10).fillColor(BRAND.text).text(clean, textX, before, { width: CONTENT_WIDTH - 20, lineGap: 2 });
  doc.moveDown(0.2);
}

function renderCodeLine(doc: PDFKit.PDFDocument, line: string): void {
  doc.font("Courier").fontSize(8).fillColor(BRAND.code).text(line || " ", MARGIN + 8, doc.y, { width: CONTENT_WIDTH - 16, lineGap: 1 });
}
...
function renderCodeBlock(doc: PDFKit.PDFDocument, lines: string[]): void {
  ...
  const blockY = doc.y;
  doc.rect(MARGIN, blockY - 2, CONTENT_WIDTH, Math.min(blockHeight, doc.page.height - blockY - 80)).fill(BRAND.surface);
  doc.moveDown(0.15);
  for (const line of lines.slice(0, 40)) {
    renderCodeLine(doc, line);
  }
  if (lines.length > 40) {
    doc.font("Helvetica").fontSize(8).fillColor(BRAND.muted).text(`… (${lines.length - 40} more lines)`, MARGIN + 8, doc.y);
  }
  doc.moveDown(0.4);
}
...
function renderTableRow(doc: PDFKit.PDFDocument, cells: string[], isHeader: boolean): void {
  ...
  cells.forEach((cell, i) => {
    ...
    if (isHeader) {
      doc.font("Helvetica-Bold").fontSize(8).fillColor(BRAND.muted).text(clean.toUpperCase(), x + 4, rowY + 2, { width: cellW - 8, lineGap: 1 });
    } else {
      doc.font("Helvetica").fontSize(9).fillColor(BRAND.text).text(clean, x + 4, rowY, { width: cellW - 8, lineGap: 1 });
    }
  });
...
function drawPageHeader(doc: PDFKit.PDFDocument): void {
  ...
  doc.font("Helvetica").fontSize(7.5).fillColor(BRAND.muted).text("SZL HOLDINGS — CONFIDENTIAL", MARGIN, MARGIN + 8, { characterSpacing: 1.5, width: CONTENT_WIDTH });
  doc.moveDown(0.5);
}
...
export function generateInvestorDocPdf(
  ...
  doc.font("Helvetica-Bold").fontSize(24).fillColor(BRAND.text).text(docTitle, MARGIN, doc.y + 8, { width: CONTENT_WIDTH });
  doc.moveDown(0.4);
  doc.font("Helvetica").fontSize(11).fillColor(BRAND.muted).text(subtitle, MARGIN, doc.y, { width: CONTENT_WIDTH });
  doc.moveDown(0.3);
  doc.font("Helvetica").fontSize(9).fillColor(BRAND.muted).text(`Prepared: ${new Date().toLocaleDateString("en-US", { year: "numeric", month: "long" })}  ·  Confidential — For qualified investors only`, MARGIN, doc.y, { width: CONTENT_WIDTH });
