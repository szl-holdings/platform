...
export async function seedPlatformData(): Promise<void> {
  ...
  await db.insert(featureFlagsTable).values([
    ...
    { key: "lyte.dashboard", name: "Lyte Dashboard", description: "Enable role-aware dashboard", isEnabled: true, scope: "product", product: "lyte", rolloutPercentage: 100 },
    { key: "lyte.signal_lifecycle", name: "Lyte Signal Lifecycle", description: "Enable full signal lifecycle (ack/assign/escalate/resolve/override)", isEnabled: true, scope: "product", product: "lyte", rolloutPercentage: 100 },
  ...
  await db.insert(platformSignalsTable).values([
    {
      ...
      whyItMatters: "Customer SLA breach imminent — 23% of requests degraded, revenue at risk",
      nextAction: "Escalate to on-call engineering, initiate incident runbook",
      valueAtRisk: "450000",
      valueAtRiskCurrency: "USD",
      category: "performance",
      tags: ["api", "latency", "sla", "production"] as any,
    ...
    },
    {
      ...
      whyItMatters: "Revenue loss in progress — each failed transaction averages $2,400",
      nextAction: "Investigate Stripe webhook endpoint, check for cert rotation or auth issues",
      valueAtRisk: "352800",
      valueAtRiskCurrency: "USD",
      category: "revenue",
      tags: ["payments", "stripe", "webhooks"] as any,
      detectedAt: hourAgo,
    },
    {
      orgId: seedOrgId,
      ...
      severity: "high",
      status: "escalated",
      whatHappened: "Credential stuffing attempt detected — 47 attempts targeting 12 admin accounts",
      whyItMatters: "Potential account takeover of privileged accounts — data breach risk",
      nextAction: "Block IP range, force MFA re-enrollment for targeted accounts, review access logs",
      valueAtRisk: "2000000",
      valueAtRiskCurrency: "USD",
      category: "security",
      tags: ["security", "login", "threat"] as any,
    ...
    },
    {
      ...
      severity: "critical",
      status: "new",
      whatHappened: "Database connection pool at 98% capacity — new connections queueing",
      whyItMatters: "Application will become unavailable within minutes if not addressed",
      nextAction: "Immediate: scale read replicas, review connection pooling configuration",
      valueAtRisk: "800000",
      category: "infrastructure",
      tags: ["database", "rds", "connections"] as any,
      detectedAt: new Date(now.getTime() - 900000),
    },
    {
      orgId: seedOrgId,
      product: "alloy",
      ...
      severity: "medium",
      status: "resolved",
      whatHappened: "CI pipeline failing on main branch — root cause: dependency update broke test suite",
      whyItMatters: "Release blocked — sprint deliverables at risk, team velocity impacted",
      nextAction: "Revert dependency update, pin versions, add regression tests",
      valueAtRisk: "25000",
      category: "engineering",
      tags: ["ci", "deployment", "github"] as any,
      detectedAt: dayAgo,
      resolvedAt: new Date(dayAgo.getTime() + 14400000),
    },
  ]).onConflictDoNothing();
  ...
  await db.insert(platformSignalsTable).values([
    {
      ...
      severity: "critical",
      status: "assigned",
      whatHappened: "Acme Corp's prod environment is unresponsive — 3 core services down",
      whyItMatters: "SLA breach at T+60min — contract penalty of $50K/hr kicks in at T+90min",
      nextAction: "Dispatch senior MSP engineer, initiate war room, notify customer exec",
      valueAtRisk: "150000",
      category: "incident",
      tags: ["customer", "sla", "incident", "acme"] as any,
      detectedAt: new Date(now.getTime() - 2820000),
    },
    {
      orgId: seedLyteOrgId,
      product: "lyte",
      ...
      severity: "high",
      status: "new",
      whatHappened: "DR backup failed 3 consecutive nights — GlobalTech exposed to data loss risk",
      whyItMatters: "72-hour RPO gap creates compliance violation and data loss exposure",
      nextAction: "Investigate backup agent health, verify storage target availability",
      valueAtRisk: "500000",
      category: "backup",
      tags: ["backup", "dr", "compliance", "globaltech"] as any,
      detectedAt: hourAgo,
    },
    {
      orgId: seedLyteOrgId,
      ...
      severity: "high",
      status: "acknowledged",
      whatHappened: "Suspicious process flagged by EDR on retail client — possible ransomware precursor",
      whyItMatters: "Early detection window — ransomware spread risk to 140 endpoints on same segment",
      nextAction: "Isolate endpoint, forensic analysis, check lateral movement indicators",
      valueAtRisk: "2500000",
      category: "security",
      tags: ["malware", "edr", "endpoint", "retail"] as any,
      detectedAt: new Date(now.getTime() - 5400000),
      acknowledgedAt: new Date(now.getTime() - 5100000),
    },
  ]).onConflictDoNothing();
  ...
  await db.insert(actionsTable).values([
    {
      ...
      description: "Deep dive into API gateway logs, database query patterns, and infra metrics to identify root cause of P99 spike.",
    ...
    },
    {
      ...
      description: "Deploy updated connection pool configuration to staging, validate, promote to production.",
    ...
    },
    {
      ...
      description: "Execute service restoration runbook for Acme Corp. Steps: 1) Check VM health 2) Restart services in order 3) Validate connectivity",
      actionType: "remediation",
      status: "in_progress",
      priority: "critical",
    },
    {
      ...
      description: "Network isolation of affected endpoint, forensic image capture, submit to threat intel platform.",
      actionType: "investigation",
      status: "pending",
      priority: "high",
    },
  ...
  const [wf1] = await db.insert(workflowsTable).values({
    ...
    description: "Auto-triggered workflow for critical severity signals — assessment, notification, escalation, remediation",
    triggerType: "signal",
    triggerConfig: { severity: "critical", autoTrigger: true } as any,
    steps: [
      { id: "assess", name: "Auto-Assessment", type: "auto", config: { timeout: 60 } },
      { id: "notify_pager", name: "PagerDuty Alert", type: "notification", config: { service: "pagerduty", urgency: "high" } },
      { id: "notify_slack", name: "Slack War Room", type: "notification", config: { channel: "#incidents" } },
      { id: "investigate", name: "Engineer Investigation", type: "manual", config: { sla: 1800 } },
      { id: "remediate", name: "Remediation Steps", type: "manual", config: { playbook: "critical-response-v2" } },
  ...
  await db.insert(workflowsTable).values([
    {
      ...
      description: "Generates and distributes weekly compliance status report to stakeholders",
      triggerType: "schedule",
      triggerConfig: { cron: "0 9 * * 1" } as any,
      steps: [
        { id: "gather", name: "Gather Signals", type: "auto", config: {} },
        { id: "generate", name: "Generate Report", type: "auto", config: { template: "compliance-weekly" } },
        { id: "review", name: "Human Review", type: "manual", config: { sla: 86400 } },
        { id: "distribute", name: "Distribute", type: "notification", config: { recipients: "exec-team" } },
      ] as any,
      status: "active",
      runCount: 8,
    },
    {
      ...
      steps: [
        ...
        { id: "document", name: "Post-Incident Report", type: "manual", config: { sla: 86400 } },
      ] as any,
      status: "active",
      runCount: 47,
    },
    ...
    await db.insert(workflowRunsTable).values([
      {
        ...
        stepResults: [
          { stepId: "assess", status: "completed", startedAt: new Date().toISOString(), completedAt: new Date().toISOString() },
          { stepId: "notify_pager", status: "completed", startedAt: new Date().toISOString(), completedAt: new Date().toISOString() },
          { stepId: "notify_slack", status: "completed", startedAt: new Date().toISOString(), completedAt: new Date().toISOString() },
          { stepId: "investigate", status: "completed", startedAt: new Date().toISOString(), completedAt: new Date().toISOString() },
          { stepId: "remediate", status: "completed", startedAt: new Date().toISOString(), completedAt: new Date().toISOString() },
          { stepId: "verify", status: "completed", startedAt: new Date().toISOString(), completedAt: new Date().toISOString() },
        ] as any,
        retryCount: 0,
        maxRetries: 3,
      },
  ...
  const portValues = [
    ...
    { name: "Port of Rotterdam", locode: "NLRTM", country: "Netherlands", region: "Europe", latitude: "51.9225", longitude: "4.4792", portType: "multipurpose" as const, status: "open" as const, avgCongestionDays: "0.8", weeklyCapacityTeu: 58000 },
    { name: "Jebel Ali", locode: "AEJEA", country: "UAE", region: "Middle East", latitude: "24.9858", longitude: "55.0555", portType: "container" as const, status: "open" as const, avgCongestionDays: "1.4", weeklyCapacityTeu: 52000 },
    { name: "Port of Los Angeles", locode: "USLAX", country: "USA", region: "North America", latitude: "33.7278", longitude: "-118.2606", portType: "container" as const, status: "open" as const, avgCongestionDays: "2.1", weeklyCapacityTeu: 43000 },
  ...
  const corridorValues = [
    ...
    { name: "Suez Canal Corridor", description: "Critical Europe-Asia route through Egypt", riskLevel: "high" as const, distanceNm: "193", avgTransitDays: "0.5", geopoliticalRisk: 67, pirateRisk: 15, weatherRisk: 25, activeConflicts: ["Houthi missile attacks on Red Sea shipping"] as any },
    { name: "Bab el-Mandeb", description: "Narrowest chokepoint connecting Red Sea to Gulf of Aden", riskLevel: "critical" as const, distanceNm: "20", avgTransitDays: "0.1", geopoliticalRisk: 91, pirateRisk: 60, weatherRisk: 30, activeConflicts: ["Active Houthi drone and missile attacks", "Iranian proxy activity"] as any },
    { name: "Strait of Hormuz", description: "Critical Persian Gulf oil transit chokepoint", riskLevel: "high" as const, distanceNm: "35", avgTransitDays: "0.1", geopoliticalRisk: 82, pirateRisk: 20, weatherRisk: 25, activeConflicts: ["Iranian naval presence", "Mine threat"] as any },
    { name: "Panama Canal Corridor", description: "Americas' primary Pacific-Atlantic link", riskLevel: "low" as const, distanceNm: "80", avgTransitDays: "0.5", geopoliticalRisk: 28, pirateRisk: 5, weatherRisk: 35, activeConflicts: [] as any },
    ...
    await db.insert(maritimeExceptionsTable).values([
      {
        ...
        description: "Vessel deviating south of planned route to avoid Houthi attack zone. ETA impact +12 hours.",
        status: "acknowledged",
        ...
        metadata: { alternateRoute: "Cape of Good Hope detour segment", riskZone: "Bab el-Mandeb" } as any,
      },
      {
        orgId: seedVesselsOrgId,
        vesselId: vessel2.id,
        ...
        description: "Tropical cyclone developing 200nm north of vessel track. Wind gusts forecast 85 knots.",
        status: "new",
        valueAtRiskUsd: "76500000",
        etaImpactHours: "6",
        costImpactUsd: "48000",
        detectedAt: new Date(now.getTime() - 3600000),
        metadata: { cycloneCategory: 2, forecastPath: "Northern diversion recommended" } as any,
      },
      {
        orgId: seedVesselsOrgId,
        vesselId: vessel3.id,
        ...
        description: "Port of Rotterdam anchorage queue increased to 31 vessels. Expected delay 18-24 hours for berth allocation.",
        status: "resolved",
      ...
      },
      {
        ...
        description: "Unknown small vessel approaching at high speed off coast of Gulf of Aden. Master requesting naval escort.",
        status: "escalated",
  ...
  await db.insert(readinessItemsTable).values([
    ...
    },
    {
      ...
      description: "BMP5 procedures must be followed and documented for all vessels transiting high-risk areas",
      status: "completed",
      priority: "critical",
      score: "100",
      targetScore: "100",
...
if (process.argv[1]?.includes("seed-platform")) {
  seedPlatformData().then(() => process.exit(0)).catch((err) => {
    console.error(err);
    process.exit(1);
  });
}
