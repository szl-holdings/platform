...
const DOMAIN_CONTEXT: Record<string, string> = {
  szl_holdings:
    'SZL Holdings is a private operating company building command-layer infrastructure across six domain-specific enterprise intelligence platforms: Carlota Jo (advisory), Terra (real estate), Aegis (cybersecurity), Vessels (maritime), Lyte (AIOps), and PRISM (legal). The company operates on a compounding architecture thesis — shared infrastructure that reduces marginal build cost per platform.',
  carlota_jo:
    'Carlota Jo Consulting is a strategic advisory firm delivering high-value consulting engagements to enterprise clients. Advisory services include strategic planning, market positioning, revenue architecture, and operational transformation. Engagements are delivered via retainer model with AI-augmented advisory sessions.',
  aegis:
    'Aegis is a unified defense and intelligence command platform converging SOC operations, threat intelligence, penetration testing, and MSP management. Clients are enterprise organizations and managed security service providers requiring multi-domain security coverage.',
  terra:
    'Terra is a distress-first real estate intelligence platform covering NYC markets. The platform combines multi-factor distress scoring, NYC Open Data integration, deal pipeline management, and market context. Target users are real estate investors and brokers seeking off-market opportunities.',
  vessels:
...
const TONE_INSTRUCTIONS: Record<string, string> = {
  executive:
    'Write in a crisp, executive briefing style. Lead with the most important insight. Use specific numbers. Avoid jargon. Max 3 sentences per section.',
  technical:
    'Write with technical precision. Include specific metrics, system references, and operational details. Structured and factual.',
  investor:
    'Write for sophisticated investors. Focus on market dynamics, competitive positioning, and financial implications. Reference benchmarks and industry context.',
  advisory:
    'Write in the voice of a trusted strategic advisor. Identify the key tension, offer a clear perspective, and provide actionable direction.',
};
...
function parseNarrativeResponse(text: string, _sections: string[]): Partial<NarrativeResult> {
  ...
  for (const [marker, key] of Object.entries(sectionMap)) {
    const regex = new RegExp(
      `${marker}:\\s*([\\s\\S]*?)(?=(?:EXECUTIVE_SUMMARY|TREND_ANALYSIS|RECOMMENDATIONS|RISK_FACTORS|OUTLOOK):|$)`,
      'i',
    );
...
export async function generateReportNarrative(request: NarrativeRequest): Promise<NarrativeResult> {
  ...
  try {
    ...
    try {
      aiModule = (await import('@szl-holdings/ai-engine/providers/anthropic')) as {
        default?: unknown;
      };
    } catch {
      try {
        aiModule = (await import('@szl-holdings/ai-engine/providers/openai')) as {
          default?: unknown;
        };
      } catch {
    ...
    logger.error(
      { err, domain: request.domain, reportType: request.reportType },
