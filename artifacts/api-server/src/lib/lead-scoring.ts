...
export function computeLeadScore(data: LeadScoringInput): number {
  ...
  if (data.email) {
    const freeProviders = ["gmail", "yahoo", "hotmail", "outlook", "icloud", "proton", "aol", "mail.com"];
    const domain = data.email.split("@")[1]?.toLowerCase() ?? "";
    if (!freeProviders.some(p => domain.includes(p))) score += 20;
    if (/^[a-z0-9]{12,}@/.test(data.email) || domain.includes("mailinator") || domain.includes("guerrilla") || domain.includes("temp") || data.email.includes("test") || data.email.includes("spam") || data.email.includes("fake")) score -= 10;
  }
  ...
  if (data.source === "linkedin" || data.source?.includes("founder") || data.medium === "founder-content") score += 10;
