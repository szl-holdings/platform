...
export type BrandTheme = "szl" | "carlota" | "aegis" | "terra" | "vessels" | "lyte" | "prism" | "neutral";
...
export const BRAND_THEMES: Record<BrandTheme, BrandColors> = {
  szl: {
    ...
    primary: "#c2a55a", primaryLight: "#d4bc82", text: "#e8e0d0",
    muted: "#7a8099", accent: "#3b82f6", success: "#10b981",
    warning: "#f59e0b", danger: "#f43f5e", headerTag: "SZL HOLDINGS",
  },
  carlota: {
    bg: "#0d0a12", surface: "#13101a", border: "#2a1f3d",
    primary: "#a855f7", primaryLight: "#c084fc", text: "#f0e8ff",
    muted: "#8b7aaa", accent: "#ec4899", success: "#22c55e",
    warning: "#f59e0b", danger: "#ef4444", headerTag: "CARLOTA JO CONSULTING",
  },
  aegis: {
    bg: "#080d0f", surface: "#0d1518", border: "#1a2e35",
    primary: "#06b6d4", primaryLight: "#22d3ee", text: "#e0f2f4",
    muted: "#5f8a92", accent: "#f43f5e", success: "#10b981",
    warning: "#f59e0b", danger: "#ef4444", headerTag: "AEGIS — UNIFIED DEFENSE & INTELLIGENCE",
  },
  terra: {
    bg: "#0a0c08", surface: "#111410", border: "#1e2a18",
    primary: "#22c55e", primaryLight: "#4ade80", text: "#e8f0e0",
    muted: "#6b8060", accent: "#f59e0b", success: "#10b981",
    warning: "#f97316", danger: "#f43f5e", headerTag: "TERRA REAL ESTATE INTELLIGENCE",
  },
  vessels: {
    bg: "#080c10", surface: "#0d1318", border: "#1a2535",
    primary: "#3b82f6", primaryLight: "#60a5fa", text: "#e0ecf8",
    muted: "#5a7a99", accent: "#06b6d4", success: "#10b981",
    warning: "#f59e0b", danger: "#ef4444", headerTag: "VESSELS MARITIME INTELLIGENCE",
  },
  lyte: {
    bg: "#090a0f", surface: "#10111a", border: "#1c1e30",
    primary: "#8b5cf6", primaryLight: "#a78bfa", text: "#e8e0f8",
    muted: "#6a6a99", accent: "#06b6d4", success: "#10b981",
    warning: "#f59e0b", danger: "#ef4444", headerTag: "LYTE COMMAND CENTER",
  },
  prism: {
    bg: "#0a080c", surface: "#130f18", border: "#281e35",
    primary: "#e879f9", primaryLight: "#f0abfc", text: "#f0e8ff",
    muted: "#8a6899", accent: "#3b82f6", success: "#10b981",
    warning: "#f59e0b", danger: "#ef4444", headerTag: "PRISM COUNSEL",
  },
  neutral: {
    bg: "#f8fafc", surface: "#ffffff", border: "#e2e8f0",
    primary: "#1e3a5f", primaryLight: "#2563eb", text: "#0f172a",
    muted: "#64748b", accent: "#3b82f6", success: "#10b981",
    warning: "#f59e0b", danger: "#ef4444", headerTag: "SZL HOLDINGS",
  },
};
...
function renderCover(ctx: RenderCtx, block: ReportBlock): void {
  ...
  const date = resolveValue((d.date as string) || new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }), ctx.data);
  ...
  if (preparedFor || preparedBy) {
    ctx.doc
      ...
      .fontSize(8.5)
      .fillColor(rgb(brand.muted))
      .text(`Prepared for: ${preparedFor || "—"}  ·  Prepared by: ${preparedBy}`, ctx.margin, pageH - 80, { width: ctx.contentW });
  }
...
function renderExecutiveSummary(ctx: RenderCtx, block: ReportBlock): void {
  ...
  ctx.doc
    .rect(ctx.margin, ctx.y, ctx.contentW, 3)
...
function renderMetricsRow(ctx: RenderCtx, block: ReportBlock): void {
  const d = block.data || {};
  const brand = ctx.brand;
  const metricsRaw = (d.metrics as Array<{ label: string; value: string; trend?: string; trendDir?: "up" | "down" | "flat" }>) || [];
  ...
  metrics.slice(0, cols).forEach((m, i) => {
    ...
    if (m.trend) {
      const trendColor = m.trendDir === "up" ? brand.success : m.trendDir === "down" ? brand.danger : brand.muted;
      ctx.doc
        .font("Helvetica")
        .fontSize(8)
        .fillColor(rgb(trendColor))
        .text(`${m.trendDir === "up" ? "▲" : m.trendDir === "down" ? "▼" : "→"} ${m.trend}`, x + 10, ctx.y + 42, { width: colW - 20 });
    }
  });
...
function renderBulletList(ctx: RenderCtx, block: ReportBlock): void {
  const d = block.data || {};
  const brand = ctx.brand;
  const items = (d.items as Array<{ text: string; color?: string }>) ||
...
function renderDataTable(ctx: RenderCtx, block: ReportBlock): void {
  ...
  allHeaders.forEach((h, i) => {
    ctx.doc
      ...
      .fontSize(7.5)
      .fillColor(rgb(brand.muted))
      .text(String(h).toUpperCase(), ctx.margin + i * colW + 8, ctx.y + 7, { width: colW - 12, characterSpacing: 0.5 });
  });
  ctx.y += 22;
  ...
  for (let ri = 0; ri < allRows.length; ri++) {
    ...
    if (ri % 2 === 1) {
      ctx.doc.rect(ctx.margin, ctx.y, ctx.contentW, rowHeight).fillOpacity(0.3).fill(rgb(brand.surface));
      ctx.doc.fillOpacity(1);
    }
...
function renderStatusGrid(ctx: RenderCtx, block: ReportBlock): void {
  const d = block.data || {};
  const brand = ctx.brand;
  const items = (d.items as Array<{ name: string; status: "active" | "warning" | "critical" | "inactive"; detail?: string }>) || [];
  ...
  for (const item of items) {
    ...
    ctx.doc
      ...
      .text(item.status.toUpperCase(), ctx.contentW + ctx.margin - 80, ctx.y + 10, { width: 80, align: "right" });
  ...
  points.forEach((p, i) => {
    if (i % Math.ceil(points.length / 6) === 0) {
      ctx.doc
        ...
        .fontSize(7)
        .fillColor(rgb(brand.muted))
        .text(p.label.slice(0, 8), ctx.margin + i * stepX - 16, ctx.y + chartH - 16, { width: 32, align: "center" });
    }
  });
...
function renderDistressIndicator(ctx: RenderCtx, block: ReportBlock): void {
  ...
  if (description) {
    ctx.doc
      ...
      .fontSize(9)
      .fillColor(rgb(brand.muted))
      .text(resolveValue(description, ctx.data), ctx.margin + 130, ctx.y + 18, { width: ctx.contentW - 150, lineGap: 2 });
  }
...
function renderTimeline(ctx: RenderCtx, block: ReportBlock): void {
  const d = block.data || {};
  const brand = ctx.brand;
  const events = (d.events as Array<{ date: string; title: string; description?: string; status?: string }>) || [];
  ...
  ctx.doc.rect(ctx.margin + 14, ctx.y, 1, events.length * 46).fillOpacity(0.4).fill(rgb(brand.border));
  ctx.doc.fillOpacity(1);
  ...
  for (const event of events) {
    ...
    ctx.doc
      ...
      .fontSize(10)
      .fillColor(rgb(brand.text))
      .text(resolveValue(event.title, ctx.data), ctx.margin + 28, ctx.y + 12, { width: ctx.contentW - 44 });
    ...
    if (event.description) {
      ctx.doc
        ...
        .fontSize(9)
        .fillColor(rgb(brand.muted))
        .text(resolveValue(event.description, ctx.data), ctx.margin + 28, ctx.y + 26, { width: ctx.contentW - 44 });
    }
...
function renderAppendixHeader(ctx: RenderCtx, block: ReportBlock): void {
  ...
  ctx.doc
    ...
    .fontSize(20)
    .fillColor(rgb(brand.text))
    .text(`${letter ? `Appendix ${letter}: ` : ""}${text}`, ctx.margin + 14, ctx.y + 8, { width: ctx.contentW - 14 });
...
function renderRiskMatrix(ctx: RenderCtx, block: ReportBlock): void {
  const d = block.data || {};
  const brand = ctx.brand;
  const items = (d.risks as Array<{ title: string; likelihood: "low" | "medium" | "high"; impact: "low" | "medium" | "high"; description?: string }>) || [];
  if (items.length === 0) return;

  const LEVEL_COLORS: Record<string, string> = { low: brand.success, medium: brand.warning, high: brand.danger };
  ...
  for (const item of items) {
    ...
    ctx.doc
      .rect(ctx.margin, ctx.y, 3, 48)
      .fill(rgb(LEVEL_COLORS[item.impact] || brand.muted));

    ctx.doc
      .font("Helvetica-Bold")
      .fontSize(10)
      .fillColor(rgb(brand.text))
      .text(resolveValue(item.title, ctx.data), ctx.margin + 12, ctx.y + 8, { width: ctx.contentW - 180 });

    ctx.doc
      .font("Helvetica")
      .fontSize(7.5)
      .fillColor(rgb(lColor))
      .text(`Likelihood: ${item.likelihood.toUpperCase()}`, ctx.contentW - 100 + ctx.margin, ctx.y + 8, { width: 100 });

    ctx.doc
      .font("Helvetica")
      .fontSize(7.5)
      .fillColor(rgb(iColor))
      .text(`Impact: ${item.impact.toUpperCase()}`, ctx.contentW - 100 + ctx.margin, ctx.y + 22, { width: 100 });
    ...
    if (item.description) {
      ctx.doc
        ...
        .fontSize(9)
        .fillColor(rgb(brand.muted))
        .text(resolveValue(item.description, ctx.data), ctx.margin + 12, ctx.y + 24, { width: ctx.contentW - 180, lineGap: 1 });
    }
...
function evaluateCondition(condition: ReportBlock["condition"], data: Record<string, unknown>): boolean {
...
function renderBlock(ctx: RenderCtx, block: ReportBlock): void {
  ...
  switch (block.type) {
    case "cover": return renderCover(ctx, block);
    case "section_header": return renderSectionHeader(ctx, block);
    case "body_text": return renderBodyText(ctx, block);
    case "executive_summary": return renderExecutiveSummary(ctx, block);
    case "metrics_row": return renderMetricsRow(ctx, block);
    case "bullet_list": return renderBulletList(ctx, block);
    case "data_table": return renderDataTable(ctx, block);
    case "status_grid": return renderStatusGrid(ctx, block);
    case "key_value_pairs": return renderKeyValuePairs(ctx, block);
    case "chart_bar": return renderChartBar(ctx, block);
    case "chart_line": return renderChartLine(ctx, block);
    case "chart_gauge": return renderChartGauge(ctx, block);
    case "distress_indicator": return renderDistressIndicator(ctx, block);
    case "timeline": return renderTimeline(ctx, block);
    case "signature_block": return renderSignatureBlock(ctx, block);
    case "appendix_header": return renderAppendixHeader(ctx, block);
    case "risk_matrix": return renderRiskMatrix(ctx, block);
    case "page_break":
      ctx.doc.addPage();
      drawPageBackground(ctx);
      ctx.y = ctx.margin;
...
function drawFooter(ctx: RenderCtx, entityName: string, classification: string = "CONFIDENTIAL"): void {
  const pages = ctx.doc.bufferedPageRange();
  for (let i = 0; i < pages.count; i++) {
    ctx.doc.switchToPage(pages.start + i);
    const y = ctx.doc.page.height - 48;
    ctx.doc
      .rect(ctx.margin, y, ctx.contentW, 0.5)
      .fill(rgb(ctx.brand.border));
    ctx.doc
      .font("Helvetica")
      .fontSize(7.5)
...
export const DOMAIN_TEMPLATES: Record<string, ReportTemplate> = {
  ...
  szl_quarterly_investor: {
    ...
    blocks: [
      {
        id: "cover", type: "cover",
        data: { title: "Quarterly Investor Letter — {{quarter}}", subtitle: "SZL Holdings — Strategic Update & Portfolio Review", classification: "CONFIDENTIAL — INVESTOR USE ONLY" }
      },
      { id: "exec", type: "executive_summary", data: { text: "{{_executiveSummary}}" } },
      {
        id: "kpis", type: "metrics_row",
        data: {
          metrics: [
            { label: "Active Platforms", value: "{{activePlatforms}}", trend: "stable", trendDir: "flat" },
            { label: "Revenue Tracks", value: "{{revenueTracks}}", trend: "growing", trendDir: "up" },
      ...
      {
        id: "platforms", type: "status_grid",
        data: {
          items: [
            { name: "Carlota Jo Consulting", status: "active", detail: "High-margin advisory retainers. Cash flow positive." },
            { name: "Terra Real Estate Intelligence", status: "active", detail: "NYC distress market coverage. Subscription SaaS." },
            { name: "Aegis — Unified Defense & Intelligence", status: "active", detail: "SOC + MSP + AI intelligence. Enterprise contracts." },
            { name: "Vessels Maritime Intelligence", status: "active", detail: "Fleet command. AIS tracking. Voyage economics." },
            { name: "Lyte Command Center", status: "active", detail: "Business observability. Multi-model AI routing." },
            { name: "PRISM Counsel", status: "active", detail: "Legal matter command. Compliance tracking." },
          ]
        }
      },
      ...
      { id: "sig", type: "signature_block", data: { name: "Stephen Lutar", title: "Founder & CEO, SZL Holdings" } },
    ],
  },
  ...
  szl_portfolio: {
    ...
    blocks: [
      {
        id: "cover", type: "cover",
        data: { title: "Portfolio Overview — {{quarter}}", subtitle: "SZL Holdings Platform Intelligence" }
      },
      { id: "exec", type: "executive_summary", data: { text: "{{_executiveSummary}}" } },
      { id: "s1", type: "section_header", data: { text: "Portfolio at a Glance" } },
      {
        id: "metrics", type: "metrics_row",
        data: {
          metrics: [
      ...
      { id: "thesis", type: "body_text", data: { text: "Six platforms on one backbone means every platform gets smarter as the others grow. Shared components and infrastructure reduce marginal build cost per platform and create structural cross-domain intelligence advantages." } },
    ],
  },
  ...
  carlota_engagement_summary: {
    ...
    blocks: [
      {
        id: "cover", type: "cover",
        data: { title: "Engagement Summary — {{client}}", subtitle: "Carlota Jo Consulting — Strategic Advisory  |  {{period}}", classification: "CLIENT CONFIDENTIAL" }
      },
      { id: "exec", type: "executive_summary", data: { text: "{{_executiveSummary}}" } },
      ...
      { id: "insights_table", type: "data_table", data: { headers: ["Insight", "Type", "Confidence"], rows: [] } },
      ...
      { id: "next_steps_table", type: "data_table", data: { headers: ["Action", "Owner", "Timeline"], rows: [] } },
      { id: "s5", type: "section_header", data: { text: "Outlook" } },
      { id: "outlook", type: "body_text", data: { text: "{{_outlook}}" } },
      { id: "sig", type: "signature_block", data: { name: "Carlota Jo", title: "Managing Principal, Carlota Jo Consulting" } },
    ],
  },
  ...
  aegis_security_assessment: {
    ...
    blocks: [
      {
        id: "cover", type: "cover",
        data: { title: "Security Assessment Report", subtitle: "Aegis — Unified Defense & Intelligence Command  |  {{assessmentType}}", classification: "CONFIDENTIAL — RESTRICTED DISTRIBUTION" }
      },
      { id: "exec", type: "executive_summary", data: { text: "{{_executiveSummary}}" } },
      { id: "s1", type: "section_header", data: { text: "Assessment Details" } },
      {
        id: "details", type: "key_value_pairs",
        data: {
          cols: 2,
          pairs: [
      ...
      {
        id: "finding_metrics", type: "metrics_row",
        data: {
          metrics: [
      ...
      { id: "findings_table", type: "data_table", data: { headers: ["Finding", "Severity", "System", "Status"], rows: [] } },
      ...
      { id: "class_text", type: "body_text", data: { text: "This report is CONFIDENTIAL and intended solely for the named client organization. Distribution is restricted to authorized personnel. Contents describe actual or potential security vulnerabilities and must be handled accordingly." } },
    ],
  },
  ...
  terra_property_analysis: {
    ...
    blocks: [
      {
        id: "cover", type: "cover",
        data: { title: "{{propertyName}}", subtitle: "Terra Real Estate Intelligence — Property Analysis  |  {{address}}" }
      },
      { id: "exec", type: "executive_summary", data: { text: "{{_executiveSummary}}" } },
      ...
      {
        id: "distress", type: "distress_indicator",
        data: { score: 0, description: "Multi-factor distress scoring combining ownership stress, tax delinquency, permit violations, and market context." }
      },
      { id: "s3", type: "section_header", data: { text: "Contributing Factors" } },
      {
        id: "factors", type: "bullet_list",
        data: {
          items: [
      ...
      { id: "comps", type: "data_table", data: { headers: ["Address", "Price", "Cap Rate", "Date"], rows: [] } },
  ...
  vessels_voyage: {
    ...
    blocks: [
      {
        id: "cover", type: "cover",
        data: { title: "Voyage Report — {{vesselName}}", subtitle: "Vessels Maritime Intelligence  |  {{voyageId}}" }
      },
      { id: "exec", type: "executive_summary", data: { text: "{{_executiveSummary}}" } },
      ...
      {
        id: "compliance", type: "key_value_pairs",
        data: {
          cols: 2,
          pairs: [
  ...
  lyte_weekly_briefing: {
    ...
    blocks: [
      {
        id: "cover", type: "cover",
        data: { title: "Weekly Operations Briefing", subtitle: "Lyte Command Center — Week of {{weekOf}}" }
      },
      { id: "exec", type: "executive_summary", data: { text: "{{_executiveSummary}}" } },
      { id: "s1", type: "section_header", data: { text: "Signal Summary" } },
      {
        id: "signal_metrics", type: "metrics_row",
        data: {
          metrics: [
            { label: "Total Signals", value: "{{totalSignals}}", trend: "vs last week", trendDir: "up" },
      ...
      {
        id: "model_metrics", type: "key_value_pairs",
        data: {
          cols: 3,
          pairs: [
  ...
  prism_legal_memo: {
    ...
    blocks: [
      {
        id: "cover", type: "cover",
        data: { title: "Legal Memorandum — {{matterNumber}}", subtitle: "PRISM Counsel  |  {{matterTitle}}", classification: "ATTORNEY-CLIENT PRIVILEGED" }
      },
      { id: "exec", type: "executive_summary", data: { text: "{{_executiveSummary}}" } },
      {
        id: "header_info", type: "key_value_pairs",
        data: {
          cols: 2,
          pairs: [
      ...
      { id: "privilege", type: "body_text", data: { text: "This memorandum is protected by attorney-client privilege and work product doctrine. It is intended solely for the named client and authorized legal personnel. Disclosure to any third party without explicit written consent of PRISM Counsel may constitute a waiver of privilege." } },
      { id: "sig", type: "signature_block", data: { name: "PRISM Counsel", title: "Legal Matter Command" } },
    ],
  },
};

export function getTemplateForDomain(domain: string, reportType?: string): ReportTemplate | null {
  const key = reportType ? `${domain}_${reportType}` : Object.keys(DOMAIN_TEMPLATES).find(k => k.startsWith(domain));
  return key ? (DOMAIN_TEMPLATES[key] || null) : null;
}

export function listAvailableTemplates(): Array<{ key: string; name: string; domain: string; reportType: string; brandTheme: string }> {
