...
import { type DetailedHealthSnapshot, getDetailedHealth, type ProbeStatus } from './health-probes';
  ...
  if (channel === 'notification' || channel === 'both') {
    ...
    jobs.push(
      queueExternalAlert({
        ...
        logger.warn(
          { err, probe },
          '[health-watcher] queueExternalAlert failed — falling back to inline dispatch',
        );
        try {
          ...
          logger.warn(
            { err: innerErr, probe },
            '[health-watcher] inline notification dispatch fallback failed',
          );
        }
      }),
    );
    ...
    for (const probe of probes) {
      ...
      if (!isUnhealthy(status)) {
        if (probeState.has(probe)) {
          logger.info(
            { probe, status },
            '[health-watcher] probe recovered — clearing degradation state',
          );
...
export function startHealthDegradationWatcher(): void {
  ...
  watcherInterval = setInterval(() => {
    evaluateOnce().catch((err) => logger.warn({ err }, '[health-watcher] evaluation cycle failed'));
  }, intervalMs);
