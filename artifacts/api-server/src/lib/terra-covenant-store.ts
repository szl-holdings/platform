import { db, type TerraCovenant, terraCovenantsTable } from '@szl-holdings/db';
...
export function measureCovenant(
  ...
  const impliedDebtYield = debt > 0 ? Math.max(0.04, 0.1 - (score / 100) * 0.05) : 0;
  ...
  const evidence: Array<{ source: string; value: string; confidence: number }> = [
    ...
    {
      source: `Live financials — ${prop.distressType ?? 'registry'}`,
      value: evidenceValue,
      confidence: 0.85,
    },
  ];
        ...
        evidence: [
          {
            ...
            value: 'Property data unavailable; using last persisted measurement',
            confidence: 0.5,
          },
        ],
        propertyData: null,
