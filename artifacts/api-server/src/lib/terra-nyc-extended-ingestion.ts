    ...
    if (!res.ok) {
      if (res.status === 400 || res.status === 403) {
        logger.warn(
          ...
          'NYC Open Data extended source unavailable (400/403) — skipping source, continuing with others',
        );
      } else {
        logger.warn(
          { status: res.status, url },
          'NYC Open Data extended fetch failed — skipping source',
        );
      }
      return [];
    }
...
function calcExtendedOpportunityScore(data: {
  ...
  if (data.distressType === 'tax-lien') {
    score += 5;
    rationale.push('Tax lien on record');
  } else if (data.distressType === 'pre-foreclosure') {
    score += 15;
    rationale.push('Pre-foreclosure signal');
  } else if (data.distressType === 'foreclosure') {
    score += 12;
    rationale.push('Active foreclosure');
  } else if (data.distressType === 'auction') {
    score += 20;
    rationale.push('Auction pending');
  } else if (data.distressType === 'reo') {
    score += 8;
    rationale.push('Bank-owned property');
  }

  if (data.daysInDistress > 200) {
    score += 10;
    rationale.push('advanced distress');
  } else if (data.daysInDistress > 90) {
    score += 5;
    rationale.push('sustained distress');
  } else {
    score += 2;
    rationale.push('fresh signal');
  }
  ...
  if (data.debtAmount && data.estimatedValue > 0) {
    ...
    if (equityPct >= 40) {
      score += 12;
      rationale.push(`${Math.round(equityPct)}% equity`);
    } else if (equityPct >= 20) {
      score += 7;
      rationale.push(`${Math.round(equityPct)}% equity`);
    } else if (equityPct <= 5) {
      score -= 5;
      rationale.push('near-underwater');
    }
  }

  if (['Manhattan', 'Brooklyn'].includes(data.borough)) {
    score += 8;
    rationale.push(`${data.borough} premium`);
  } else if (['Queens', 'Bronx'].includes(data.borough)) {
    score += 4;
    rationale.push(`${data.borough} demand`);
  }
...
export async function ingestRollingPropertySales(
  runId: number,
): Promise<{ inserted: number; skipped: number; alerts: number }> {
  ...
  const url = buildSodaUrl('usep-8jbt', {
    ...
    $select:
      'borough,neighborhood,building_class_category,tax_class_at_present,block,lot,address,zip_code,residential_units,commercial_units,land_square_feet,gross_square_feet,year_built,sale_price,sale_date',
  });
  ...
  for (const raw of records) {
    ...
    const saleDate = rec.sale_date
      ? new Date(rec.sale_date).toISOString().split('T')[0]!
    ...
    const scoring = calcExtendedOpportunityScore({
      distressType: 'reo',
      daysInDistress,
      estimatedValue: salePrice,
      borough,
    });
    ...
    const property: InsertTerraDistressProperty = {
      ...
      tags: [
        ...
        'dof',
        borough.toLowerCase(),
        ...(rec.neighborhood ? [rec.neighborhood.toLowerCase()] : []),
      ],
      timeline: [
        {
          date: saleDate,
          type: 'Property Sale Recorded',
          description: `NYC DOF Rolling Sale — $${salePrice.toLocaleString()}`,
        },
      ],
    ...
    try {
      ...
      if (isNew) {
        inserted++;
        const ac = await generateAlertsForProperty(property as any, dbId, externalId);
        alerts += ac;
      } else skipped++;
    } catch (err) {
      logger.warn({ err, externalId }, 'Failed to upsert rolling sale');
      skipped++;
    }
  }
...
export async function ingestTaxLienSaleList(
  runId: number,
): Promise<{ inserted: number; skipped: number; alerts: number }> {
  ...
  for (const raw of records) {
    ...
    const property: InsertTerraDistressProperty = {
      ...
      timeline: [
        {
          date: filingDate,
          type: 'Tax Lien Sale Flagged',
          description: `NYC DOF Tax Lien Sale List — $${lienAmount.toLocaleString()} total debt`,
        },
      ],
    ...
    try {
      ...
      logger.warn({ err, externalId }, 'Failed to upsert tax lien sale');
      skipped++;
    }
  }
...
export async function ingestHpdComplaints(
  runId: number,
): Promise<{ inserted: number; skipped: number; alerts: number }> {
  ...
  const url = buildSodaUrl('uwyv-629c', {
    ...
    $order: 'opendate DESC',
    $select:
      'buildingid,boroughid,borough,block,lot,buildingnumber,streetname,apartment,zip,unittype,unitid,spacetypeid,type,majorcategoryid,minorcategoryid,codeid,statusid,statusdate,stateindicator,status,duedate,closedate,closereason,opendate,assigneddate',
  });
  ...
  for (const [, recs] of grouped) {
    ...
    const address = await normalizeAddress(
      `${rec.buildingnumber ?? ''} ${rec.streetname ?? ''}`.trim(),
    );
    if (!address || address.length < 5) {
      skipped++;
      continue;
    ...
    const filingDate = rec.opendate
      ? new Date(rec.opendate).toISOString().split('T')[0]!
      : new Date().toISOString().split('T')[0]!;
    ...
    const property: InsertTerraDistressProperty = {
      ...
      timeline: [
        {
          date: filingDate,
          type: 'HPD Complaint Filed',
          description: `${recs.length} open complaint(s) on record`,
        },
      ],
    ...
    try {
      ...
      logger.warn({ err, externalId }, 'Failed to upsert HPD complaint');
      skipped++;
    }
  }
  ...
  for (const [, recs] of grouped) {
    ...
    const property: InsertTerraDistressProperty = {
      ...
      timeline: [
        {
          date: filingDate,
          type: 'DOB Violation Issued',
          description: `${recs.length} DOB violation(s) on record`,
        },
      ],
    ...
    try {
      ...
      logger.warn({ err, externalId }, 'Failed to upsert DOB violation');
      skipped++;
    }
  }
...
export async function ingestNyc311PropertyComplaints(
  runId: number,
): Promise<{ inserted: number; skipped: number; alerts: number }> {
  ...
  const propertyComplaintTypes =
    "complaint_type IN ('HEAT/HOT WATER','PLUMBING','PAINT/PLASTER','WATER LEAK','DOOR/WINDOW','ELEVATOR','GENERAL CONSTRUCTION')";
  const url = buildSodaUrl('erm2-nwe9', {
    ...
    $select:
      'unique_key,created_date,closed_date,complaint_type,descriptor,incident_address,incident_zip,bbl,borough,x_coordinate_state_plane,y_coordinate_state_plane,latitude,longitude',
  });
  ...
  for (const [, recs] of grouped) {
    ...
    const filingDate = rec.created_date
      ? new Date(rec.created_date).toISOString().split('T')[0]!
      : new Date().toISOString().split('T')[0]!;
    const daysInDistress = Math.ceil((Date.now() - new Date(filingDate).getTime()) / 86400000);
    ...
    const property: InsertTerraDistressProperty = {
      ...
      tags: [
        ...
        'neighborhood-signal',
        borough.toLowerCase(),
        ...(rec.incident_zip ? [rec.incident_zip] : []),
      ],
      timeline: [
        {
          date: filingDate,
          type: '311 Complaints Filed',
          description: `${recs.length} property complaint(s): ${recs
            .map((r) => r.complaint_type)
    ...
    try {
      ...
      logger.warn({ err, externalId }, 'Failed to upsert 311 complaint cluster');
      skipped++;
    }
  }
...
export async function ingestAcrisParties(runId: number): Promise<{ enriched: number }> {
  ...
  for (const raw of records) {
    ...
    enriched++;
    logger.debug(
      { documentId: rec.document_id, name: rec.name },
      'ACRIS party identified as entity — LLC ownership tracing candidate',
    );
  }
...
export async function ingestMapPluto(
  runId: number,
): Promise<{ inserted: number; skipped: number; alerts: number }> {
  logger.info(
    { runId },
    'Ingesting MapPLUTO property data (64uk-42ks) — land use, zoning, assessed values',
  );
  const url = buildSodaUrl('64uk-42ks', {
    ...
    $select:
      'bbl,block,lot,cd,ct2010,borocode,address,zipcode,bldgclass,landuse,ownername,lotarea,bldgarea,numfloors,yearbuilt,assessland,assesstot,exemptland,exempttot,borough',
  });
  ...
  for (const raw of records) {
    const rec = raw as Record<string, string>;
    if (!rec.bbl || !rec.ownername || !rec.address) {
      skipped++;
      continue;
    }
    ...
    const distressIndicator =
      exemptTotal > assessedValue * 0.5
        ? 'tax-lien'
        : buildingAge > 80 && buildingClass.startsWith('D')
          ? 'pre-foreclosure'
          : assessedValue > 0 && landValue > assessedValue * 0.8
            ? 'tax-lien'
            : null;

    if (!distressIndicator) {
      skipped++;
      continue;
    }
      ...
      timeline: [
        {
          ...
          type: 'PLUTO Assessment',
          description: `Assessed value $${assessedValue.toLocaleString()}, land $${landValue.toLocaleString()}, building class ${buildingClass}`,
        },
      ],
    ...
    try {
      ...
      logger.warn({ err, bbl }, 'Failed to upsert PLUTO record');
      skipped++;
    }
  }
...
durableJobQueue.register<NycExtendedIngestionJobPayload>(
  ...
  async (job) => {
    ...
    await writeAuditLog('terra_extended_ingestion_started', 'terra_ingestion_run', String(runId), {
      sources,
      jobId: job.id,
    ...
    try {
      for (const source of sources) {
        ...
        try {
          let result:
            | { inserted: number; skipped: number; alerts: number }
            | { enriched: number }
            | null = null;
          ...
          logger.info(
            { source, result, durationMs: Date.now() - sourceStart },
            'Extended source ingestion complete',
          );
        } catch (err) {
