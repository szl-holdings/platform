...
async function writeAuditLog(
  ...
  try {
    ...
    logger.warn(
      ...
      'Failed to write terra ingestion audit log',
    );
  }
}
...
async function fetchLegalsByDocIds(
  docIds: string[],
...
function calcOpportunityScore(data: {
  ...
  if (data.distressType === 'auction') {
    ...
    if (data.auctionDate) {
      ...
      if (daysUntil <= 14) {
        score += 10;
        rationale.push(`auction in ${daysUntil}d`);
      }
    }
  ...
  } else if (data.daysInDistress <= 30) {
    score += 3;
    rationale.push('fresh filing');
  }
  ...
  if (data.debtAmount && data.estimatedValue > 0) {
    ...
    if (equityPct >= 40) {
      score += 12;
      rationale.push(`${Math.round(equityPct)}% equity cushion`);
...
async function ingestAcrisForeclosures(
  runId: number,
): Promise<{ inserted: number; skipped: number; alerts: number }> {
  logger.info(
    { runId },
    'Ingesting ACRIS foreclosure data (JUDG + TLS judgments via ACRIS Master + Legals)',
  );
  ...
  const allDocIds = records
    .map((r: unknown) => (r as Record<string, string>).document_id)
    .filter(Boolean) as string[];
  const legalsByDocId = await fetchLegalsByDocIds(allDocIds);
  logger.info(
    { runId, masterCount: records.length, legalsResolved: Object.keys(legalsByDocId).length },
    'ACRIS foreclosure: legals resolved',
  );
  ...
  for (const raw of records) {
    ...
    if (!borough) {
      skipped++;
      logger.debug({ boroughCode, address }, 'ACRIS foreclosure: unresolvable borough, skipping');
      continue;
    }
      ...
      timeline: [
        {
          ...
          description: 'Foreclosure proceeding filed in ACRIS',
        },
      ],
...
async function ingestDofTaxLiens(
  runId: number,
): Promise<{ inserted: number; skipped: number; alerts: number }> {
  logger.info(
    { runId },
    'Ingesting DOF Tax Lien data (via ACRIS Master — DTL/TL&R coded doc_types)',
  );
  ...
  if (records.length === 0) {
    logger.info(
      { runId },
      'DOF tax lien: no records returned (API may be unavailable or no matching doc types)',
    );
    return { inserted, skipped, alerts };
  }

  const allDocIdsDof = records
    .map((r: unknown) => (r as Record<string, string>).document_id)
    .filter(Boolean) as string[];
  const legalsByDocId = await fetchLegalsByDocIds(allDocIdsDof);
  logger.info(
    { runId, masterCount: records.length, legalsResolved: Object.keys(legalsByDocId).length },
    'DOF lien: legals resolved',
  );
  ...
  for (const raw of records) {
    ...
    if (!borough) {
      skipped++;
      logger.debug({ boroughCode, address }, 'DOF lien: unresolvable borough, skipping');
      continue;
    }
    ...
    const daysInDistress = Math.ceil((Date.now() - new Date(filingDate).getTime()) / 86400000);
    const docAmount = rec.document_amt ? parseFloat(rec.document_amt) : null;
    const distressType = (rec.doc_type ?? '').toLowerCase().includes('deed in lieu')
      ? 'foreclosure'
      : 'tax-lien';
    ...
    const property: InsertTerraDistressProperty = {
      ...
      timeline: [
        {
          date: filingDate,
          type: distressType === 'tax-lien' ? 'Tax Lien Filed' : 'Deed In Lieu',
          description: `ACRIS ${rec.doc_type ?? 'distress'} document recorded`,
        },
      ],
...
async function ingestHpdViolations(
  runId: number,
): Promise<{ inserted: number; skipped: number; alerts: number }> {
  ...
  for (const [, recs] of grouped) {
    ...
    const address = await normalizeAddress(
      `${rec.housenumber ?? ''} ${rec.streetname ?? ''}`.trim(),
    );
    if (!address || address.length < 5) {
      skipped++;
      continue;
    ...
    if (!borough) {
      skipped++;
      logger.debug({ boroughRaw, address }, 'HPD violation: unresolvable borough, skipping');
      continue;
    }
    ...
    const property: InsertTerraDistressProperty = {
      ...
      timeline: [
        {
          date: filingDate,
          type: 'HPD Violation Issued',
          description: `Class C violation(s): ${recs.length} on record`,
        },
      ],
...
async function ingestAcrisRealPropertyMaster(
  runId: number,
): Promise<{ inserted: number; skipped: number; alerts: number }> {
  ...
  const allDocIdsMaster = masterRecords
    .map((r: unknown) => (r as Record<string, string>).document_id)
    .filter(Boolean) as string[];
  const legalsByDocId = await fetchLegalsByDocIds(allDocIdsMaster);
  logger.info(
    { runId, masterCount: masterRecords.length, legalsResolved: Object.keys(legalsByDocId).length },
    'ACRIS master: legals resolved',
  );
  ...
  for (const rawMaster of masterRecords) {
    ...
    const resolvedBoroughMaster =
      boroughMap[boroughCode.toLowerCase()] ?? mapBoroughFromCounty(boroughCode);
    if (!resolvedBoroughMaster) {
      skipped++;
      logger.debug({ boroughCode, address }, 'ACRIS master: unresolvable borough, skipping');
      continue;
    }
    ...
    const property: InsertTerraDistressProperty = {
      ...
      scoreRationale: salePrice
        ? scoring.rationale
        : `${scoring.rationale} — no sale price available`,
      daysInDistress,
      ...
      timeline: [
        {
          date: filingDate,
          type: 'Deed Recorded',
          description: `ACRIS deed transfer: ${rec.doc_type ?? 'DEED'}`,
        },
      ],
...
async function ingestForeclosureFilings(
  runId: number,
): Promise<{ inserted: number; skipped: number; alerts: number }> {
  logger.info(
    { runId },
    'Ingesting ACRIS Foreclosure Filings (JUDG coded doc_type via Master dataset)',
  );
  ...
  if (records.length === 0) {
    logger.info(
      { runId },
      'ACRIS foreclosure filings: no records returned (API may be unavailable)',
    );
    return { inserted, skipped, alerts };
  }

  const allDocIdsForeclosure = records
    .map((r: unknown) => (r as Record<string, string>).document_id)
    .filter(Boolean) as string[];
  const legalsByDocId = await fetchLegalsByDocIds(allDocIdsForeclosure);
  logger.info(
    { runId, masterCount: records.length, legalsResolved: Object.keys(legalsByDocId).length },
    'ACRIS foreclosure filings: legals resolved',
  );
  ...
  for (const raw of records) {
    ...
    if (!borough) {
      ...
      logger.debug(
        { boroughCode, address },
        'ACRIS foreclosure filing: unresolvable borough, skipping',
      );
      continue;
    }
    ...
    const property: InsertTerraDistressProperty = {
      ...
      timeline: [
        {
          date: filingDate,
          type: isJudgment ? 'Foreclosure Judgment' : 'Foreclosure Filing',
          description: `ACRIS ${rec.doc_type ?? 'foreclosure'} recorded`,
        },
      ],
...
durableJobQueue.register<NycIngestionJobPayload>(NYC_INGESTION_JOB_TYPE, async (job) => {
  ...
  await writeAuditLog('terra_ingestion_started', 'terra_ingestion_run', String(runId), {
    sources,
    jobId: job.id,
  ...
  try {
    for (const source of sources) {
      ...
      try {
        ...
        await writeAuditLog('terra_source_ingested', 'terra_ingestion_run', String(runId), {
          source,
        ...
        totalFailed++;
        await writeAuditLog('terra_source_failed', 'terra_ingestion_run', String(runId), {
          source,
          error: msg,
          durationMs: Date.now() - sourceStart,
        });
    ...
    await writeAuditLog('terra_ingestion_completed', 'terra_ingestion_run', String(runId), {
      status: finalStatus,
      totalFetched,
      totalInserted,
      totalSkipped,
    ...
    await writeAuditLog('terra_ingestion_failed', 'terra_ingestion_run', String(runId), {
      error: msg,
      totalFetched,
      totalInserted,
