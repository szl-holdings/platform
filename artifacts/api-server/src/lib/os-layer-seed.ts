...
const SZL_RECOMMENDATIONS = [
  {
    ...
    evidenceCount: 4,
    evidence: [
      ev('e-szl-1', 'Pipeline CRM', 'database', 'Three anchor deals (PRAXIS Group, Mira Ventures, Atlas Capital) have slipped 14 days each with no updated close probability. Combined ARR impact: $4.1M.', 180, 0.93, ['CRM sync', 'delta-extract', 'pipeline-model']),
      ...
      ev('e-szl-3', 'Deal Desk Activity Log', 'database', 'No inbound deal-desk activity logged for PRAXIS Group in 9 days. Last contact: April 9, status: term-sheet review.', 300, 0.95, ['deal-desk-log']),
  ...
  },
  {
    ...
    summary: 'The PRAXIS Group opportunity has been in internal approval for 18 days — 12 days past the 6-day SLA. No blocking conditions detected in compliance or legal reviews.',
    rationale: "Approval SLA breach increases deal loss probability by 23% per 7-day increment (internal benchmark). Competitor activity detected in PRAXIS Group's domain from AIS signal cluster.",
    ...
    evidenceCount: 3,
    evidence: [
      ev('e-szl-5', 'Approval Workflow Engine', 'database', 'PRAXIS Group opportunity entered approval queue on April 1. SLA: 6 days. Days elapsed: 18. No approver action logged since April 4.', 60, 0.99, ['workflow-engine']),
      ev('e-szl-6', 'Competitive Intelligence Feed', 'external_api', "3 competitor RFP signals detected in PRAXIS Group's domain over past 10 days. Two signals correlate with known competitor onboarding patterns.", 1800, 0.74, ['competitive-feed', 'signal-correlation']),
...
const PULSE_RECOMMENDATIONS = [
  {
    id: 'pls-001',
    variant: 'pulse',
    priority: 'P0',
    title: 'Cross-variant alert: 3 P0 recommendations require executive attention',
    summary: 'Three P0 recommendations are open across the platform: PARAGON (active threat containment), SEXTANT (OFAC sanctions proximity), and SZL Holdings (Q2 pipeline at risk). Combined value at risk: $50.6M.',
    ...
    proposedAction: 'Review each P0 and assign approver or escalate to the appropriate decision owner. PARAGON requires SOC lead action within the next 30 minutes.',
    confidence: 0.98,
    ...
    evidenceCount: 3,
    evidence: [
      ev('e-pls-1', 'Decision Center — PARAGON', 'derived', 'aeg-001: Active threat lateral movement, 14 endpoints without EDR. Open 25 minutes. Approver: unassigned.', 1500, 0.99),
      ev('e-pls-2', 'Decision Center — SEXTANT', 'derived', 'ves-001: OFAC sanctions proximity alert, MV Meridian Star. Open 15 minutes. Approver: unassigned.', 900, 0.99),
...
const PULSE_SOURCE_HEALTH = [
  ...
  { sourceId: 'pls-aegis', sourceName: 'PARAGON Feed', connector: 'internal', status: 'healthy', lastSeenAt: new Date(Date.now() - 30000).toISOString(), freshnessSeconds: 30, latencyMs: 18 },
  { sourceId: 'pls-vessels', sourceName: 'SEXTANT Feed', connector: 'internal', status: 'healthy', lastSeenAt: new Date(Date.now() - 60000).toISOString(), freshnessSeconds: 60, latencyMs: 22 },
];

const COMMAND_RECOMMENDATIONS = [
  { ...SZL_RECOMMENDATIONS[0], id: 'cmd-szl-001', variant: 'command' },
  { ...VESSELS_RECOMMENDATIONS[0], id: 'cmd-ves-001', variant: 'command' },
  { ...AEGIS_RECOMMENDATIONS[0], id: 'cmd-aeg-001', variant: 'command' },
];

const COMMAND_SOURCE_HEALTH = [
  ...
  { sourceId: 'cmd-vessels', sourceName: 'SEXTANT', connector: 'internal', status: 'healthy', lastSeenAt: new Date(Date.now() - 60000).toISOString(), freshnessSeconds: 60, latencyMs: 18 },
  { sourceId: 'cmd-aegis', sourceName: 'PARAGON', connector: 'internal', status: 'healthy', lastSeenAt: new Date(Date.now() - 30000).toISOString(), freshnessSeconds: 30, latencyMs: 12 },
  { sourceId: 'cmd-terra', sourceName: 'DOMAINE', connector: 'internal', status: 'healthy', lastSeenAt: new Date(Date.now() - 120000).toISOString(), freshnessSeconds: 120, latencyMs: 21 },
];

function baseRuns(variant: string) {
  return [
    {
      id: `run-${variant}-001`,
      variant,
...
const VARIANT_DATA: Record<string, { recommendations: Record<string, unknown>[]; sourceHealth: Record<string, unknown>[] }> = {
  'szl-holdings': { recommendations: SZL_RECOMMENDATIONS, sourceHealth: SZL_SOURCE_HEALTH },
  vessels: { recommendations: VESSELS_RECOMMENDATIONS, sourceHealth: VESSELS_SOURCE_HEALTH },
  'carlota-jo': { recommendations: CARLOTA_RECOMMENDATIONS, sourceHealth: CARLOTA_SOURCE_HEALTH },
  aegis: { recommendations: AEGIS_RECOMMENDATIONS, sourceHealth: AEGIS_SOURCE_HEALTH },
  terra: { recommendations: TERRA_RECOMMENDATIONS, sourceHealth: TERRA_SOURCE_HEALTH },
  pulse: { recommendations: PULSE_RECOMMENDATIONS, sourceHealth: PULSE_SOURCE_HEALTH },
  command: { recommendations: COMMAND_RECOMMENDATIONS, sourceHealth: COMMAND_SOURCE_HEALTH },
};

export function getSeedData(variant: string) {
  ...
  return {
    ...
    runs: baseRuns(variant),
    evalResults: variant === 'szl-holdings' ? SHARED_EVAL_RESULTS : null,
  };
}
