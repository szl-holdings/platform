...
DO $$ BEGIN
  ALTER TABLE alloy_ai_decisions ADD COLUMN IF NOT EXISTS org_id INTEGER;
...
DO $$ BEGIN
  ALTER TABLE alloy_ai_audit_log ADD COLUMN IF NOT EXISTS org_id INTEGER;
EXCEPTION WHEN OTHERS THEN NULL;
END $$;
...
export async function ensureDecisionTables(): Promise<void> {
  if (bootstrapped) return;
  try {
    await pool.query(BOOTSTRAP_SQL);
    bootstrapped = true;
    logger.info("Alloy decision and audit tables ensured");
  } catch (err) {
    logger.warn({ err }, "Alloy decision table bootstrap failed (non-fatal)");
  }
}
