...
const MODEL_CARDS_META: Record<string, { purpose: string; capabilities: string[] }> = {
  vessels: {
    purpose: 'Maritime intelligence — fleet tracking, sanctions screening, route analysis',
    capabilities: ['fleet-tracking', 'sanctions-risk', 'route-economics', 'ais-analysis'],
  },
  firestorm: {
    purpose: 'Cybersecurity threat detection, vulnerability assessment, pen-test analysis',
    capabilities: [
  ...
  inca: {
    purpose: 'AI research synthesis — experiment tracking, paper analysis, benchmark comparison',
    capabilities: [
      'research-synthesis',
      'experiment-analysis',
      'model-benchmarking',
  ...
  lyte: {
    purpose: 'SRE observability intelligence — health monitoring, SLO analysis, incident diagnosis',
    capabilities: [
      'health-monitoring',
      'slo-compliance',
      'capacity-planning',
  ...
  dreamscape: {
    purpose: 'Creative AI — campaign generation, content scoring, brand analysis',
    capabilities: [
      'content-generation',
      'campaign-scoring',
      'brand-monitoring',
  ...
  'szl-holdings': {
    purpose: 'Portfolio intelligence — investment analysis, risk assessment, market monitoring',
    capabilities: ['portfolio-analysis', 'risk-scoring', 'market-intelligence', 'due-diligence'],
  },
  'carlota-jo': {
    purpose: 'Consulting intelligence — engagement analysis, strategy recommendation',
    capabilities: [
      'engagement-tracking',
      'strategy-analysis',
      'client-health',
  ...
  'readiness-report': {
    purpose: 'Operational readiness and maturity assessment',
    capabilities: [
      'maturity-scoring',
      'gap-analysis',
  ...
  msp: {
    purpose: 'Managed services — SLA monitoring, ticket analysis, client health',
    capabilities: [
      'sla-monitoring',
  ...
  terra: {
    purpose:
      'Real estate market intelligence — property scoring, distress detection, investment analysis',
    capabilities: [
      'property-scoring',
      'market-analysis',
      'distress-detection',
  ...
  admin: {
    purpose: 'Platform administration decision support and system diagnostics',
    capabilities: [
      'system-diagnostics',
  ...
  stephen: {
    purpose: 'Personal brand and portfolio command agent',
    capabilities: ['portfolio-presentation', 'case-study-generation', 'career-narrative'],
  },
};
...
export function getModelCard(agentId: string): ModelCard {
  ...
  const meta = MODEL_CARDS_META[agentId] ?? {
    purpose: `AI agent for ${agentId}`,
    capabilities: [],
  };
  const provider = config.model.includes('gpt')
    ? 'OpenAI'
  ...
  return {
    ...
    performance: {
      ...
      successRate:
        records.length > 0 ? parseFloat((successRecords.length / records.length).toFixed(4)) : 1,
    },
    lastDeployed,
    lastReviewed: LAST_REVIEWED,
  };
