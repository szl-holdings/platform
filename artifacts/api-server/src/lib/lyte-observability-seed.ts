...
export async function seedLyteObservability() {
  ...
  for (const service of SERVICES) {
    ...
    const goldenSeries = _metricSim.generateGoldenSignalsHistory(
      service,
      GOLDEN_POINTS,
      GOLDEN_INTERVAL_MS,
      now,
      ...
      metricBatches.push(
        {
          ...
          metricName: 'latency',
          metricType: 'latency',
          value: parseFloat(pt.latencyP99.toFixed(2)),
          unit: 'ms',
          tags: { env: 'production', percentile: 'p99' },
          anomaly: isAnomalyLatency,
          anomalyScore: isAnomalyLatency ? parseFloat(_rng.range(0.7, 1.0).toFixed(3)) : null,
        ...
        },
        {
          ...
          value: parseFloat(pt.errorRate.toFixed(3)),
          unit: '%',
          tags: { env: 'production' },
          anomaly: isAnomalyError,
          anomalyScore: isAnomalyError ? parseFloat(_rng.range(0.7, 1.0).toFixed(3)) : null,
          recordedAt: new Date(pt.timestamp),
        },
        {
          service,
          ...
          value: parseFloat(pt.throughput.toFixed(1)),
          unit: 'req/s',
          tags: { env: 'production' },
          anomaly: false,
          anomalyScore: null,
        ...
        },
        {
          ...
          metricName: 'cpu',
          metricType: 'cpu',
          value: parseFloat(pt.saturation.toFixed(1)),
          unit: '%',
          tags: { env: 'production' },
          anomaly: pt.saturation > 90,
          anomalyScore: pt.saturation > 90 ? parseFloat(_rng.range(0.6, 1.0).toFixed(3)) : null,
          recordedAt: new Date(pt.timestamp),
        },
        {
          service,
          metricName: 'apdex',
          metricType: 'apdex',
          value: parseFloat(pt.apdex.toFixed(3)),
          unit: 'score',
          tags: { env: 'production' },
          anomaly: pt.apdex < 0.7,
          anomalyScore: pt.apdex < 0.7 ? parseFloat(_rng.range(0.6, 0.9).toFixed(3)) : null,
          recordedAt: new Date(pt.timestamp),
        },
      );
    }
    ...
    for (const mt of METRIC_TYPES.filter(
      (m) => !['latency', 'error_rate', 'throughput', 'cpu'].includes(m.type),
    )) {
  ...
  for (const alert of insertedAlerts) {
    ...
    for (let i = 0; i < eventCount; i++) {
      ...
      alertEventBatches.push({
        alertId: alert.id,
        eventType,
        triggerValue: alert.threshold
          ? parseFloat((alert.threshold * rng(1.0, 1.5)).toFixed(2))
          : null,
...
function getTopSignalsForLens(
  lens: string,
): Array<{ title: string; severity: string; source: string }> {
  ...
  return [
    {
      ...
      description: 'P95 latency exceeds SLA threshold',
      ...
      firingCount: 7,
      lastFiredAt: new Date(Date.now() - 900000),
    },
    {
      ...
      severity: 'critical' as const,
      status: 'firing' as const,
      notificationChannels: ['pagerduty', 'email'],
      firingCount: 3,
      lastFiredAt: new Date(Date.now() - 1800000),
    },
    {
      name: 'ML Inference CPU Saturation',
      description: 'CPU usage approaching capacity limit',
      alertType: 'threshold' as const,
      ...
      firingCount: 12,
      lastFiredAt: new Date(Date.now() - 3600000),
    },
    {
      name: 'Signal Bus Queue Depth',
      description: 'Queue depth exceeds processing capacity',
      alertType: 'threshold' as const,
      service: 'signal-bus',
      ...
      firingCount: 5,
      lastFiredAt: new Date(Date.now() - 600000),
    },
    {
      name: 'Alert Engine Anomaly',
      description: 'Anomalous behavior detected in alert engine',
      alertType: 'anomaly' as const,
      service: 'alert-engine',
      metricName: 'latency',
      condition: 'anomaly' as const,
      threshold: null,
    ...
    {
      name: 'Alloy Engine Throughput Drop',
      description: 'Request throughput fell below minimum',
      alertType: 'threshold' as const,
      ...
      notificationChannels: ['slack'],
      firingCount: 4,
      lastFiredAt: new Date(Date.now() - 86400000),
    },
    {
      name: 'Vessels Intel Memory',
      description: 'Memory usage above safe operating threshold',
      alertType: 'threshold' as const,
      service: 'vessels-intel',
      metricName: 'memory',
      condition: 'gt' as const,
      threshold: 80,
      severity: 'medium' as const,
      ...
      firingCount: 8,
      lastFiredAt: new Date(Date.now() - 43200000),
    },
    {
      name: 'PRISM Engine Availability',
      description: 'Availability dropped below SLA commitment',
      alertType: 'threshold' as const,
      ...
      firingCount: 1,
      lastFiredAt: new Date(Date.now() - 172800000),
    },
    {
      name: 'Terra Beacon Latency Warn',
      description: 'API latency exceeding warning threshold',
      alertType: 'threshold' as const,
      service: 'terra-beacon',
      metricName: 'latency',
      condition: 'gt' as const,
      threshold: 200,
      severity: 'medium' as const,
    ...
    },
    {
      ...
      firingCount: 6,
      lastFiredAt: new Date(Date.now() - 10800000),
    },
    {
      name: 'Notification SVC Queue',
      description: 'Notification delivery queue backed up',
      alertType: 'threshold' as const,
      ...
      notificationChannels: ['slack'],
      firingCount: 9,
      lastFiredAt: new Date(Date.now() - 14400000),
    },
    {
      name: 'Revenue Anomaly Detection',
      description: 'Unexpected revenue pattern detected',
      alertType: 'anomaly' as const,
      service: 'lyte-core',
      metricName: 'revenue',
      condition: 'anomaly' as const,
      ...
      firingCount: 1,
      lastFiredAt: new Date(Date.now() - 259200000),
    },
    {
      name: 'Action Router CPU',
      description: 'CPU spike in action routing service',
      alertType: 'threshold' as const,
    ...
    {
      name: 'Workflow Exec Throughput',
      description: 'Workflow execution rate below SLA',
      alertType: 'threshold' as const,
      service: 'workflow-exec',
      metricName: 'throughput',
      condition: 'lt' as const,
      threshold: 30,
      severity: 'high' as const,
      ...
      notificationChannels: ['pagerduty'],
      firingCount: 2,
      lastFiredAt: new Date(Date.now() - 18000000),
    },
    {
      name: 'Metrics Store Memory',
      description: 'Metrics database memory pressure',
      alertType: 'threshold' as const,
      ...
      notificationChannels: ['slack', 'pagerduty'],
      firingCount: 4,
      lastFiredAt: new Date(Date.now() - 1200000),
    },
    {
      name: 'Geo Index Latency',
      description: 'Geospatial index query latency spiking',
      alertType: 'threshold' as const,
      service: 'terra-beacon',
      metricName: 'latency',
      condition: 'gt' as const,
      ...
      notificationChannels: ['slack'],
      firingCount: 2,
      lastFiredAt: new Date(Date.now() - 432000000),
    },
    {
      name: 'Composite: Core Platform Degraded',
      description: 'Multiple core services showing degradation simultaneously',
      alertType: 'composite' as const,
      service: 'api-gateway',
      metricName: 'availability',
      condition: 'lt' as const,
      threshold: 98,
    ...
    {
      name: 'Churn Rate Warning',
      description: 'Customer churn rate trending above threshold',
      alertType: 'threshold' as const,
      service: 'lyte-core',
      ...
      notificationChannels: ['email'],
      firingCount: 1,
      lastFiredAt: new Date(Date.now() - 604800000),
    },
    {
      name: 'NPS Below Target',
      description: 'Net Promoter Score fell below company target',
      alertType: 'threshold' as const,
      service: 'lyte-core',
      metricName: 'nps',
      condition: 'lt' as const,
    ...
    {
      name: 'AIS Stream Throughput',
      description: 'AIS data stream throughput degraded',
      alertType: 'threshold' as const,
      service: 'vessels-intel',
      metricName: 'throughput',
      condition: 'lt' as const,
      ...
      notificationChannels: ['email'],
      firingCount: 5,
      lastFiredAt: new Date(Date.now() - 345600000),
    },
    {
      name: 'Threat DB Query Latency',
      description: 'Threat intelligence database slow',
      alertType: 'threshold' as const,
      service: 'firestorm-soc',
      metricName: 'latency',
      condition: 'gt' as const,
      ...
      notificationChannels: ['slack'],
      firingCount: 7,
      lastFiredAt: new Date(Date.now() - 28800000),
    },
    {
      name: 'PRISM Engine Error Rate Anomaly',
      description: 'Anomalous error patterns in PRISM scoring',
      alertType: 'anomaly' as const,
      service: 'prism-engine',
      metricName: 'error_rate',
      condition: 'anomaly' as const,
    ...
    {
      name: 'Escalation Manager Overload',
      description: 'Escalation processing capacity exceeded',
      alertType: 'threshold' as const,
      service: 'escalation-mgr',
      metricName: 'queue_depth',
      condition: 'gt' as const,
      threshold: 80,
      severity: 'critical' as const,
      ...
      firingCount: 2,
      lastFiredAt: new Date(Date.now() - 300000),
    },
    {
      name: 'Revenue Delta > 20%',
      description: 'Revenue delta exceeds ±20% from 7-day avg',
      alertType: 'threshold' as const,
      service: 'alloy-engine',
      metricName: 'revenue',
      condition: 'gt' as const,
    ...
    },
    {
      ...
      description: 'ML serving availability below SLA',
      alertType: 'threshold' as const,
      service: 'ml-inference',
      metricName: 'availability',
      condition: 'lt' as const,
...
function buildEscalationDefs(alerts: any[]) {
  ...
  const PATHS_3 = [
    [
      { stage: 1, owner: 'on-call-eng', label: 'On-Call Engineer' },
      { stage: 2, owner: 'team-lead', label: 'Team Lead' },
      { stage: 3, owner: 'vp-engineering', label: 'VP Engineering' },
    ],
    [
      { stage: 1, owner: 'sre-primary', label: 'SRE Primary' },
      { stage: 2, owner: 'platform-lead', label: 'Platform Lead' },
      { stage: 3, owner: 'cto-office', label: 'CTO Office' },
    ],
  ];
  const PATHS_4 = [
    [
      { stage: 1, owner: 'l1-support', label: 'L1 Support' },
      { stage: 2, owner: 'l2-engineering', label: 'L2 Engineering' },
      { stage: 3, owner: 'team-lead', label: 'Team Lead' },
      { stage: 4, owner: 'vp-ops', label: 'VP Operations' },
    ],
  ];
  ...
  defs.push({
    ...
    description:
      'API gateway latency exceeding 500ms P95 — multi-tenant impact across all SZL platforms',
...
function getSignalTitle(type: string, service: string): string {
  const titles: Record<string, string[]> = {
    anomaly: [
      `Anomalous metric pattern in ${service}`,
      `Unusual activity detected: ${service}`,
      `Behavioral deviation flagged for ${service}`,
    ],
    alert_fired: [
      `Alert triggered on ${service}`,
      `Threshold breach on ${service}`,
      `Alert condition met: ${service}`,
    ],
    sla_breach: [
      `SLA breach: ${service} response time`,
      `SLO violation detected on ${service}`,
      `Uptime commitment missed: ${service}`,
    ],
    owner_gap: [
      `Ownership gap identified in ${service}`,
      `No active owner for ${service}`,
      `${service} missing on-call coverage`,
    ],
    compliance_flag: [
      `Compliance gap: ${service} audit trail`,
      `Policy violation detected in ${service}`,
      `Regulatory exposure: ${service}`,
    ],
    cost_spike: [
      `Cost anomaly detected: ${service}`,
      `Unexpected resource spend on ${service}`,
      `Budget overrun signal: ${service}`,
    ],
    churn_risk: [
      `Churn risk signal from ${service}`,
      `Customer health score declining via ${service}`,
      `Retention risk detected: ${service}`,
    ],
    deployment: [
      `Deployment event on ${service}`,
      `Configuration change: ${service}`,
      `Version rollout completed: ${service}`,
    ],
  };
