...
export async function runMetricsAggregation(payload: {
  ...
  const definitions = await db.select().from(analyticsMetricDefinitionsTable)
  ...
  for (const def of definitions) {
    ...
    for (const granularity of granularities as Array<"minute" | "hour" | "day" | "week" | "month">) {
      ...
      for (const bucket of buckets) {
        ...
        try {
          ...
          logger.warn({ err, metricId: def.metricId, granularity }, "[analytics-agg] Failed to upsert snapshot");
        }
      }
    }
  }

  logger.info({ totalSnapshots, definitions: definitions.length }, "[analytics-agg] Aggregation complete");
}
...
export async function runAnomalyScan(payload: {
  ...
  const snapshots = await db.select().from(analyticsMetricSnapshotsTable)
    .where(and(...snapshotConditions))
    .orderBy(asc(analyticsMetricSnapshotsTable.metricId), asc(analyticsMetricSnapshotsTable.periodStart));
  ...
  for (const [mId, snaps] of byMetric) {
    ...
    const candidates = [
      ...
      ...detectTrendChange(dataPoints),
    ];

    for (const candidate of candidates) {
      const anomalyId = `anm_${mId.replace(/\./g, "_")}_${candidate.timestamp.getTime()}`;

      const existing = await db.select({ id: analyticsAnomaliesTable.id })
  ...
  logger.info({ totalAnomalies, metrics: byMetric.size }, "[analytics-anomaly] Anomaly scan complete");
}
...
export async function runAnalyticsExport(payload: {
  ...
  try {
    await db.update(analyticsExportJobsTable)
    ...
    const [job] = await db.select({ webhookUrl: analyticsExportJobsTable.webhookUrl })
...
export function registerAnalyticsJobHandlers(): void {
  jobQueue.register(JOB_TYPES.METRICS_AGGREGATION, async (job) => {
    await runMetricsAggregation(job.payload as {
      domain?: string;
      metricId?: string;
      lookbackHours?: number;
    });
  ...
  jobQueue.register(JOB_TYPES.ANOMALY_SCAN, async (job) => {
    await runAnomalyScan(job.payload as {
      domain?: string;
      metricId?: string;
      lookbackDays?: number;
    });
  ...
  jobQueue.register(JOB_TYPES.ANALYTICS_EXPORT, async (job) => {
    await runAnalyticsExport(job.payload as {
      exportId: string;
      domain: string;
      exportType: string;
      format: string;
  ...
  logger.info("[analytics-jobs] Registered analytics job handlers: METRICS_AGGREGATION, ANOMALY_SCAN, ANALYTICS_EXPORT");
}
