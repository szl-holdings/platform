...
export interface VesselPositionUpdatedEvent {
  ...
  payload: { vesselId: number; latitude: number | null; longitude: number | null; speed: number | null; recordedAt: string | null };
}
...
export interface FirestormIncidentUpdatedEvent {
  ...
  payload: { incidentId: number; previousStatus: string; newStatus: string; severity: string | null };
}
...
export interface PrismCounselApprovalResolvedEvent {
  ...
  payload: { requestId: number; matterId: number; decision: "approved" | "rejected"; actorId: number };
}

export interface PrismCounselDeadlineApproachingEvent {
  type: "prism-counsel.deadline-approaching";
  payload: { deadlineId: number; matterId: number; title: string; dueDate: string; priority: string };
}

export interface PrismCounselRecommendationActedEvent {
  type: "prism-counsel.recommendation-acted";
  payload: { recommendationId: number; matterId: number; action: "accepted" | "dismissed"; actorId: number };
}
...
export interface LyteIncidentEscalatedEvent {
  ...
  payload: { incidentId: number; severity: string | null; targetRole: string; reason: string | null };
}
...
export interface AlloySignalIngestedEvent {
  ...
  payload: { signalId: number; severity: string; domain: string | null; source: string; title: string };
}
...
type DomainEventHandler<T extends DomainEventType> = (payload: DomainEventPayload<T>) => void | Promise<void>;
