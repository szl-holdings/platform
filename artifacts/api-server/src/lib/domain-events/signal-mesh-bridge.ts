...
import { type DomainEvent, type DomainEventType, domainEventBus } from './index.js';
...
function publish(args: {
  ...
  try {
    const sig = createSignal({
      ...
      entityRefs: [
        { entityId: args.entityId, entityType: args.entityType, displayName: args.title },
      ],
...
export function initSignalMeshBridge(): void {
  ...
  bridge('alloy.signal-ingested', (p) => {
    ...
    const domain: SignalDomain = (
      validDomains.has(d as SignalDomain) ? d : 'platform'
    ) as SignalDomain;
    return {
      type: 'anomaly',
      domain,
