...
import type { BlockNode, TextNode, DocumentEditorContent } from "./pdf-renderer-types";

// ─── Styles ──────────────────────────────────────────────────────────────────

const styles = StyleSheet.create({
  page: { padding: 48, fontFamily: "Helvetica", fontSize: 10, color: "#1f2937", backgroundColor: "#ffffff" },
  h1: { fontSize: 20, fontFamily: "Helvetica-Bold", marginBottom: 6, marginTop: 16, color: "#111827" },
  h2: { fontSize: 15, fontFamily: "Helvetica-Bold", marginBottom: 4, marginTop: 12, color: "#1e3a5f" },
  h3: { fontSize: 12, fontFamily: "Helvetica-Bold", marginBottom: 3, marginTop: 8, color: "#374151" },
  ...
  blockquote: { marginBottom: 6, paddingLeft: 12, borderLeftWidth: 3, borderLeftColor: "#6366f1", borderLeftStyle: "solid", color: "#6b7280", fontStyle: "italic" },
  codeBlock: { backgroundColor: "#1e293b", color: "#86efac", padding: 10, borderRadius: 4, fontFamily: "Courier", fontSize: 9, marginBottom: 8 },
  hr: { borderBottomWidth: 1, borderBottomColor: "#e5e7eb", borderBottomStyle: "solid", marginVertical: 10 },
  ...
  header: { marginBottom: 24, paddingBottom: 12, borderBottomWidth: 2, borderBottomColor: "#1e3a5f", borderBottomStyle: "solid" },
  ...
  switch (block.type) {
    case "heading1":
      return React.createElement(View, { key: idx }, [
        React.createElement(Text, { key: 0, style: { ...styles.h1, textAlign } }, block.children.map(c => c.text).join("")),
      ]);

    case "heading2":
      return React.createElement(View, { key: idx }, [
        React.createElement(Text, { key: 0, style: { ...styles.h2, textAlign } }, block.children.map(c => c.text).join("")),
      ]);

    case "heading3":
      return React.createElement(View, { key: idx }, [
        React.createElement(Text, { key: 0, style: { ...styles.h3, textAlign } }, block.children.map(c => c.text).join("")),
      ]);
    ...
    case "table":
      if (!block.tableData || block.tableData.length === 0) return React.createElement(View, { key: idx });
      return React.createElement(View, { key: idx }, [
    ...
    default:
      return React.createElement(View, { key: idx }, [
        React.createElement(Text, { key: 0, style: { ...styles.paragraph, textAlign } },
...
function SzlDocument({ title, documentType, appSource, blocks, generatedAt, entityData }: {
  title: string;
  documentType?: string;
  appSource?: string;
  blocks: BlockNode[];
  ...
  return React.createElement(Document, {},
    React.createElement(Page, { size: "A4", style: styles.page },
      React.createElement(View, { style: styles.header },
        ...
        React.createElement(Text, { style: styles.headerSub }, `${documentType || "Document"} · ${appSource || ""} · ${generatedAt}`)
      ),
      ...elements,
      React.createElement(Text, { style: styles.footer, render: ({ pageNumber, totalPages }: { pageNumber: number; totalPages: number }) =>
...
export async function renderEntityDataToPdfBuffer(params: {
  ...
  const blocks: BlockNode[] = [
    ...
    { id: "meta", type: "paragraph", children: [{ text: `Template: ${templateId} · Entity: ${entityType}/${entityId} · Source: ${appSource || "general"}` }] },
  ];
