...
export async function seedDreamscapeData(): Promise<void> {
  ...
  const campaigns = await db.insert(dreamscapeCampaignsTable).values([
    {
      name: "Horizon Brand Film",
      description: "60-second flagship brand film showcasing SZL Holdings' platform ecosystem — intelligence, execution, and precision at scale.",
    ...
    },
    {
      ...
      description: "Cinematic documentary-style campaign covering global maritime operations — real footage from the Atlantic and Pacific corridors.",
    ...
    },
    {
      ...
      description: "High-tension social media series demonstrating live-fire cybersecurity simulations — targeting CISO decision makers across LinkedIn and YouTube.",
    ...
    },
    {
      ...
      description: "Luxury lifestyle campaign for Carlota Jo Consulting seasonal rebrand — editorial video, brand photography, and Instagram carousel series.",
      clientName: "Carlota Jo Consulting",
      status: "published",
      category: "commercial",
      targetAudience: "High-net-worth individuals, C-suite executives, and luxury brand enthusiasts",
      deadline: daysAgo(14),
      metadata: {
        progress: 100,
        budget: "$75,000",
  ...
  const scripts = await db.insert(dreamscapeScriptsTable).values([
    {
...
END CARD: alloy.szlholdings.com — Request Access`,
  ...
  const storyboards = await db.insert(dreamscapeStoryboardsTable).values([
    ...
    },
    {
      ...
      sceneNumber: 3,
      visualDescription: "4-panel split of Alloy, Vessels, Firestorm, INCA dashboards — animated data flows",
    ...
    },
    {
      ...
      sceneNumber: 1,
      visualDescription: "Screen recording of Alloy signal ingest — animated signals populating in real-time",
    ...
    },
    {
      ...
      sceneNumber: 1,
      visualDescription: "4K drone footage — container vessel steaming at dawn, Atlantic light, cinematic grade",
  ...
  await db.insert(dreamscapeCampaignAssetsTable).values([
    {
      ...
      metadata: { resolution: "3840×2160", codec: "ProRes 4444", frameRate: "24fps", colorSpace: "Rec. 2020" },
    },
    {
      campaignId: horizon.id,
  ...
  await db.insert(dreamscapeReviewsTable).values([
    {
      ...
      comment: "The opening server hall sequence is outstanding — the pace and score work perfectly together. Minor note: the maritime shot transition at 0:08 could be smoother. Let's discuss in tomorrow's call.",
      status: "approved",
    },
    {
      campaignId: horizon.id,
      reviewerName: "Priya Sharma",
      comment: "Legal cleared the final VO script. Brand guidelines compliance confirmed. End card duration needs extending from 6s to 8s per accessibility standards.",
      status: "changes_requested",
    },
    {
      campaignId: horizon.id,
      reviewerName: "Marcus Chen",
      comment: "Client loved V3. The platform montage is exactly what we asked for. Requesting one additional B-roll shot of the INCA interface for the 0:32 slot.",
      status: "pending",
    },
    {
      campaignId: vessels.id,
      reviewerName: "Elena Vasquez",
      comment: "Drone footage from the Atlantic is stunning. Color grade is approved. Audio mix needs revision — wind noise in scene 1 is too prominent.",
      status: "changes_requested",
    },
  ]);
