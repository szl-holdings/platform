...
export async function ingestSignal(
  record: Omit<AtlasSignalRecord, 'id' | 'createdAt' | 'updatedAt'>,
): Promise<AtlasSignalRecord> {
  const now = new Date();
  const id = randomUUID();
  ...
  const signal: AtlasSignalRecord = {
    ...record,
    id,
  ...
  logger.info(
    { domain: record.domain, signalId: signal.id, signalType: record.signalType },
    'atlas:signal ingested',
  );
  return signal;
}
...
export async function updateSignalStatus(
  domain: string,
  signalId: string,
  status: AtlasSignalRecord['status'],
): Promise<boolean> {
...
export async function captureEvidence(
  record: Omit<AtlasEvidenceRecord, 'id' | 'capturedAt'>,
): Promise<AtlasEvidenceRecord> {
  const now = new Date();
  const id = randomUUID();
  ...
  logger.info(
    { domain: record.domain, workflowId: record.workflowId, evidenceId: ev.id },
    'atlas:evidence captured',
  );
  return ev;
}
...
export async function recordOutcome(
  record: Omit<AtlasOutcomeRecord, 'id' | 'recordedAt'>,
): Promise<AtlasOutcomeRecord> {
  const now = new Date();
  const id = randomUUID();
...
export async function getOutcomes(
  ...
  return rows.map((row) => ({
    ...
    status: row.status,
    businessImpact:
      row.financialImpactUsd != null ||
      row.operationalSeverity != null ||
      row.entitiesAffected != null
        ? {
...
export async function registerEvaluationHook(
  hook: Omit<EvaluationHookRecord, 'id' | 'snapshotAt'>,
): Promise<EvaluationHookRecord> {
  const now = new Date();
  const rows = await db
    .insert(atlasRunsTable)
    .values({
      id: randomUUID(),
      domain: hook.domain,
  ...
  logger.info(
    { domain: hook.domain, hookId: record.id, workflowId: hook.workflowId },
    'atlas:evaluation-hook registered',
  );
  return record;
}

export async function getEvaluationHooks(
  domain: string,
  tenantId?: string,
): Promise<EvaluationHookRecord[]> {
  const where = tenantId
...
export async function getEvaluationHookById(
  hookId: string,
  tenantId?: string,
): Promise<EvaluationHookRecord | undefined> {
  ...
  const rows = await db.select().from(atlasRunsTable).where(where).limit(1);
...
function rowToHookRecord(row: typeof atlasRunsTable.$inferSelect): EvaluationHookRecord {
  return {
    ...
    benchmarkMetrics:
      row.latencyMs != null || row.stepsCompleted != null || row.stepsFailed != null
        ? {
...
export async function evaluateSignalsForDomain(
  ...
  const groups: SignalGroup[] = signals.map((record, idx) => ({
    ...
    suggestedAction:
      (record.payload.suggestedAction as string | undefined) ??
      `Investigate ${record.signalType} signal`,
...
const DOMAIN_POLICIES: Policy[] = [
  {
    ...
    description:
      'All critical or high severity security incidents require exec approval before response execution.',
    scope: 'domain',
    domain: 'aegis',
    ...
    rules: [
      {
        ...
        conditions: [
          { field: 'resource.attributes.dealValueUsd', operator: 'gte', value: 5000000 },
        ],
        effect: 'require_approval',
        requiredApproverRole: 'admin',
    ...
    rules: [
      {
        ...
        conditions: [
          { field: 'resource.attributes.requiresCounselReview', operator: 'eq', value: true },
        ],
        effect: 'require_approval',
        requiredApproverRole: 'admin',
    ...
    rules: [
      {
        ...
        conditions: [
          { field: 'resource.attributes.commitmentValueUsd', operator: 'gte', value: 25000 },
        ],
        effect: 'require_approval',
        requiredApproverRole: 'admin',
  ...
  },
  {
    ...
    description:
      'Any change to sovereign classification or cross-boundary data movement requires executive approval.',
    scope: 'domain',
    domain: 'imperium',
    ...
    rules: [
      {
        ...
        conditions: [
          { field: 'resource.attributes.affectsSovereignty', operator: 'eq', value: true },
        ],
        effect: 'require_approval',
        requiredApproverRole: 'exec',
      ...
      },
      {
        ...
        conditions: [
          { field: 'resource.attributes.estimatedSavingsUsd', operator: 'gte', value: 100000 },
        ],
        effect: 'require_approval',
        requiredApproverRole: 'admin',
...
export const DOMAIN_WORKFLOWS: Record<string, WorkflowDefinition> = {
  'aegis-incident-response': {
    ...
    description:
      'End-to-end security incident response: triage → containment → eradication → recovery → post-incident review.',
    domain: 'aegis',
    ...
    steps: [
      {
        ...
        handler: 'aegis.triage',
        executionMode: 'manual',
        requiresApproval: false,
        description: 'Assess severity, scope, and impact of the security incident.',
      },
      {
        id: 'enrich',
        name: 'Signal Enrichment',
        handler: 'aegis.enrich',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Correlate with threat intel feeds and MITRE ATT&CK mappings.',
      },
      {
        id: 'recommend',
        name: 'Response Recommendation',
        handler: 'aegis.recommend',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Generate ranked response options with confidence scores.',
      },
      {
        id: 'approve',
        name: 'Approval Gate',
        handler: 'aegis.approve',
        executionMode: 'manual',
        requiresApproval: true,
        approverRole: 'admin',
        description: 'Human approval gate before containment execution.',
      },
      {
        id: 'contain',
        name: 'Containment',
        handler: 'aegis.contain',
        executionMode: 'semi_auto',
        requiresApproval: false,
        rollbackHandler: 'aegis.rollback-contain',
        description: 'Execute containment actions to isolate the threat.',
      },
      {
        id: 'eradicate',
        name: 'Eradication',
        handler: 'aegis.eradicate',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Remove the threat from affected systems.',
      },
      {
        id: 'recover',
        name: 'Recovery',
        handler: 'aegis.recover',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Restore affected systems to normal operations.',
      },
      {
        id: 'evidence',
        name: 'Evidence Capture',
        handler: 'aegis.capture-evidence',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Collect forensic artifacts and audit chain entries.',
      },
      {
        id: 'outcome',
        name: 'Outcome Recording',
        handler: 'aegis.record-outcome',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Record incident closure, MTTR, and business impact.',
      },
    ],
  ...
  'vessels-voyage-risk': {
    ...
    description:
      'Voyage risk assessment through to execution: signal → risk estimate → operator plan → escalation → execution record.',
    domain: 'vessels',
    executionMode: 'semi_auto',
    requiresExplicitApproval: true,
    isDryRunCapable: true,
    ...
    steps: [
      {
        ...
        handler: 'vessels.ingest-signal',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Normalize voyage/sanctions/anomaly signal data.',
      },
      {
        id: 'risk-estimate',
        name: 'Business Risk Estimation',
        handler: 'vessels.estimate-risk',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Compute financial and operational risk for the voyage.',
      },
      {
        id: 'sanctions-check',
        name: 'Sanctions & Compliance Screening',
        handler: 'vessels.sanctions-check',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Screen vessel, cargo, and counterparties against sanctions lists.',
      },
      {
        id: 'operator-plan',
        name: 'Operator Action Plan',
        handler: 'vessels.build-action-plan',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Generate operator-ready action plan with risk mitigations.',
      },
      {
        id: 'approve',
        name: 'Approval Gate',
        handler: 'vessels.approve',
        executionMode: 'manual',
        requiresApproval: true,
        approverRole: 'operator',
        description: 'Operator approval of voyage execution plan.',
      },
      {
        id: 'execute-voyage',
        name: 'Voyage Execution',
        handler: 'vessels.execute-voyage',
        executionMode: 'semi_auto',
        requiresApproval: false,
        rollbackHandler: 'vessels.rollback-voyage',
        description: 'Record voyage commencement and update AIS tracking.',
      },
      {
        id: 'evidence',
        name: 'Execution Record',
        handler: 'vessels.capture-evidence',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Capture bill of lading, charter party, and cargo manifest.',
      },
      {
        id: 'outcome',
        name: 'Outcome Recording',
        handler: 'vessels.record-outcome',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Record voyage P&L, compliance status, and incident log.',
      },
    ],
    ...
    steps: [
      {
        id: 'signal-ingest',
        name: 'Signal Ingestion',
        handler: 'terra.ingest-signal',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Normalize property/ownership/distress signal.',
      },
      {
        id: 'deal-thesis',
        name: 'Deal Thesis Generation',
        handler: 'terra.generate-thesis',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Generate investment thesis with pro forma and IRR projections.',
      },
      {
        id: 'underwriting',
        name: 'Underwriting Workflow',
        handler: 'terra.underwrite',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Run automated underwriting checks and stress tests.',
      },
      {
        id: 'legal-review',
        name: 'Legal Review Gate',
        handler: 'terra.legal-review',
        executionMode: 'manual',
        requiresApproval: true,
        approverRole: 'admin',
        description: 'Legal and compliance review of the deal thesis.',
      },
      {
        id: 'approve-deal',
        name: 'Deal Approval Gate',
        handler: 'terra.approve-deal',
        executionMode: 'manual',
        requiresApproval: true,
        approverRole: 'exec',
        description: 'Executive approval of deal commitment.',
      },
      {
        id: 'execute-action',
        name: 'Deal Action Execution',
        handler: 'terra.execute-action',
        executionMode: 'semi_auto',
        requiresApproval: false,
        rollbackHandler: 'terra.rollback-action',
        description: 'Execute approved deal action (offer, LOI, or close).',
      },
      {
        id: 'evidence',
        name: 'Deal Record Capture',
        handler: 'terra.capture-evidence',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Capture executed documents, title reports, and appraisals.',
      },
      {
        id: 'outcome',
        name: 'Outcome Recording',
        handler: 'terra.record-outcome',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Record deal outcome, IRR, and capital deployment.',
      },
    ],
  ...
  'prism-matter-execution': {
    ...
    steps: [
      {
        id: 'ingest-event',
        name: 'Matter Event Ingestion',
        handler: 'prism.ingest-event',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Normalize legal matter event (filing deadline, discovery, hearing).',
      },
      {
        id: 'enrich',
        name: 'Matter Context Enrichment',
        handler: 'prism.enrich',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Enrich with docket data, precedent search, and exposure analysis.',
      },
      {
        id: 'recommend',
        name: 'Recommendation Generation',
        handler: 'prism.generate-recommendation',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Generate legal strategy recommendations with precedent analysis.',
      },
      {
        id: 'legal-review',
        name: 'Legal Review Gate',
        handler: 'prism.legal-review',
        executionMode: 'manual',
        requiresApproval: true,
        approverRole: 'admin',
        description: 'Counsel reviews and approves recommended legal strategy.',
      },
      {
        id: 'client-approval',
        name: 'Client Approval Gate',
        handler: 'prism.client-approval',
        executionMode: 'manual',
        requiresApproval: true,
        approverRole: 'client',
        description: 'Client approval of legal strategy before execution.',
      },
      {
        id: 'execute-filing',
        name: 'Execution & Filing',
        handler: 'prism.execute-filing',
        executionMode: 'manual',
        requiresApproval: false,
        description: 'Execute the approved legal action (filing, motion, settlement).',
      },
      {
        id: 'evidence',
        name: 'Execution Trail Capture',
        handler: 'prism.capture-trail',
        executionMode: 'manual',
        requiresApproval: false,
        description: 'Capture filing confirmations, docket entries, and communications.',
      },
      {
        id: 'outcome',
        name: 'Recovery/Risk Outcome',
        handler: 'prism.record-outcome',
        executionMode: 'manual',
        requiresApproval: false,
        description: 'Record matter outcome, recovery amount, and risk disposition.',
      },
    ],
  ...
  'carlota-concierge-workflow': {
    ...
    description:
      'Client/property ops/vendor signal → concierge workflow → approval → service completion proof.',
    domain: 'carlota-jo',
    executionMode: 'semi_auto',
    requiresExplicitApproval: true,
    isDryRunCapable: true,
    ...
    steps: [
      {
        id: 'signal-ingest',
        name: 'Request Signal Ingestion',
        handler: 'carlota.ingest-request',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Capture and normalize client/property/vendor service request.',
      },
      {
        id: 'enrichment',
        name: 'Request Enrichment',
        handler: 'carlota.enrich-request',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Enrich with client preferences, property context, and vendor data.',
      },
      {
        id: 'concierge-plan',
        name: 'Concierge Workflow Planning',
        handler: 'carlota.plan-workflow',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Generate service execution plan with vendor options and timeline.',
      },
      {
        id: 'approve',
        name: 'Approval Gate',
        handler: 'carlota.approve',
        executionMode: 'manual',
        requiresApproval: true,
        approverRole: 'admin',
        description: 'Principal approval for high-value or complex service requests.',
      },
      {
        id: 'vendor-commit',
        name: 'Vendor Commitment',
        handler: 'carlota.vendor-commit',
        executionMode: 'semi_auto',
        requiresApproval: false,
        rollbackHandler: 'carlota.rollback-vendor',
        description: 'Commit vendor booking and generate service order.',
      },
      {
        id: 'service-execution',
        name: 'Service Execution',
        handler: 'carlota.execute-service',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Coordinate and track service delivery.',
      },
      {
        id: 'evidence',
        name: 'Service Completion Proof',
        handler: 'carlota.capture-proof',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Capture service completion confirmation, receipts, and photos.',
      },
      {
        id: 'outcome',
        name: 'Outcome Recording',
        handler: 'carlota.record-outcome',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Record client satisfaction, service cost, and completion status.',
      },
    ],
  ...
  'imperium-remediation': {
    ...
    description:
      'Infra/cost/policy/sovereignty signal → remediation plan → approval → evidence and savings log.',
    domain: 'imperium',
    executionMode: 'semi_auto',
    requiresExplicitApproval: true,
    isDryRunCapable: true,
    ...
    steps: [
      {
        id: 'signal-ingest',
        name: 'Infrastructure Signal Ingestion',
        handler: 'imperium.ingest-signal',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Normalize infra/cost/policy/sovereignty signal.',
      },
      {
        id: 'enrich',
        name: 'Signal Enrichment',
        handler: 'imperium.enrich',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Correlate with cloud cost data, policy inventory, and sovereignty map.',
      },
      {
        id: 'remediation-plan',
        name: 'Remediation Plan Generation',
        handler: 'imperium.plan-remediation',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Generate ranked remediation options with cost/savings projections.',
      },
      {
        id: 'approve',
        name: 'Approval Gate',
        handler: 'imperium.approve',
        executionMode: 'manual',
        requiresApproval: true,
        approverRole: 'admin',
        description: 'Admin approval of remediation plan before automation.',
      },
      {
        id: 'automate',
        name: 'Automated Remediation',
        handler: 'imperium.automate',
        executionMode: 'semi_auto',
        requiresApproval: false,
        rollbackHandler: 'imperium.rollback',
        description: 'Execute approved remediation via IaC or cloud API.',
      },
      {
        id: 'verify',
        name: 'Verification & Validation',
        handler: 'imperium.verify',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Verify remediation effectiveness and policy compliance.',
      },
      {
        id: 'evidence',
        name: 'Evidence & Savings Log',
        handler: 'imperium.capture-evidence',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Log before/after state, actual savings, and compliance attestation.',
      },
      {
        id: 'outcome',
        name: 'Outcome Recording',
        handler: 'imperium.record-outcome',
        executionMode: 'semi_auto',
        requiresApproval: false,
        description: 'Record remediation outcome, actual vs projected savings, and SLA impact.',
      },
    ],
...
export async function executedomainWorkflow(
  ...
  try {
    ...
    logger.warn(
      { err, runId: result.run.runId, domain: req.domain },
      'atlas:run direct persistence failed',
    );
  }
  ...
  if (!req.isDryRun && !req.isSimulation) {
    ...
    await registerEvaluationHook({
      ...
      benchmarkMetrics: {
        latencyMs: result.run.completedAt
          ? result.run.completedAt - result.run.startedAt
          : undefined,
        stepsCompleted: (result.run.steps as Array<{ status: string }>).filter(
          (s) => s.status === 'completed',
        ).length,
        stepsFailed: (result.run.steps as Array<{ status: string }>).filter(
          (s) => s.status === 'failed',
        ).length,
      },
    });
