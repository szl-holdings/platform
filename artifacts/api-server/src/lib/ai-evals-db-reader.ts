...
import type { AITrace, TraceStatus, TraceDomain, TraceAggregate, ReviewQueueItem, ReviewVerdict, ReviewPriority, ReviewQueueStats } from "@szl-holdings/ai-engine";
...
export async function dbListTraces(options: {
  ...
  try {
    const [dataResult, countResult] = await Promise.all([
      ...
      client.query(
        `SELECT COUNT(*)::int AS total FROM ai_traces ${where}`,
        params,
      ),
    ]);
...
export async function dbGetTrace(traceId: string): Promise<AITrace | null> {
  ...
  try {
    const result = await client.query(
      `SELECT * FROM ai_traces WHERE trace_id = $1 LIMIT 1`,
      [traceId],
    );
...
export async function dbListReviewQueue(options: {
  ...
  try {
    const [dataResult, countResult] = await Promise.all([
      ...
      client.query(
        `SELECT COUNT(*)::int AS total FROM ai_review_queue ${where}`,
        params,
      ),
    ]);
...
export async function dbRecordReviewDecision(opts: {
  ...
  try {
    ...
    logger.error({ err, reviewId: opts.reviewId }, "[ai-evals-reader] dbRecordReviewDecision failed");
    return null;
  } finally {
    client.release();
  }
