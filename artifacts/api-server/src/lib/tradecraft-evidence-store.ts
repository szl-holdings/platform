  ...
  try {
    ...
    const [
      ...
      db
        ...
        .from(workflowsTable)
        .where(eq(workflowsTable.product, 'firestorm'))
        .limit(100),
    ]);
    ...
    for (const c of cases) {
      const notesText = Array.isArray(c.notes)
        ? c.notes
            .map((n: { content: string; author: string }) => `[${n.author}]: ${n.content}`)
            .join('\n')
        : '';
      const evidenceText = Array.isArray(c.evidence)
        ? c.evidence.map((e: { name: string; type: string }) => `${e.type}: ${e.name}`).join(', ')
        : '';
    ...
    const distinctOwners = [
      ...new Map(assets.filter((a) => a.owner).map((a) => [a.owner, a])).values(),
    ];
      ...
      pipeline.ingestRetentionPolicy({
        ...
        content: [
          ...
          notCompliant.length > 0
            ? `Non-Compliant Controls: ${notCompliant
                .map((c) => `${c.controlId} ${c.controlName}`)
                .slice(0, 10)
                .join('; ')}`
            : 'All controls compliant.',
          controls[0]?.category
            ? `Categories: ${[...new Set(controls.map((c) => c.category))].join(', ')}`
            : '',
        ]
          .filter(Boolean)
          .join('\n'),
    ...
    lastRefreshAt = now;
    logger.info(
      { indexed: pipeline.totalIndexed },
      '[tradecraft-evidence-store] Evidence index refreshed from DB',
    );
  } catch (err) {
    logger.warn(
      { err },
      '[tradecraft-evidence-store] Failed to refresh evidence index from DB — using stale index',
    );
  }
}
