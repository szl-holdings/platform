...
const VESSEL_NAMES = [
  "MV ATLAS PIONEER", "MT PACIFIC GLORY", "MV NORDIC STAR", "MT OCEAN FORTUNE", "MV SILVER HORIZON",
  "MT GOLDEN GATE", "MV TITAN VOYAGER", "MT BLUE ODYSSEY", "MV SEA MERCURY", "MT CORAL KING",
  "MV ARCTIC BRIDGE", "MT EMERALD WAVE", "MV STORM RUNNER", "MT IRON GIANT", "MV JADE NAVIGATOR",
  "MT SAPPHIRE COAST", "MV DELTA PRIDE", "MT AURORA BOREALIS", "MV HORIZON HAWK", "MT CRIMSON TIDE",
  "MV MONSOON SPIRIT", "MT PACIFIC EAGLE", "MV GOLD PIONEER", "MT EAST STAR", "MV BERING QUEST",
  "MT CAPE HOPE", "MV OCEAN KING", "MT STELLAR DAWN", "MV TROPICAL BREEZE", "MT VIKING SPIRIT",
  "MV MARATHON RUNNER", "MT ARCTIC WOLF", "MV CAPE VERDE", "MT MORNING STAR", "MV SOUTH WIND",
  "MT BLACK PEARL", "MV CORAL SEA", "MT DEEP OCEAN", "MV EAGLE POINT", "MT FRONTIER SPIRIT",
  "MV GLOBAL REACH", "MT HARBOR MASTER", "MV INDIA OCEAN", "MT JADE EMPRESS", "MV KESTREL",
  "MT LIBERTY BELL", "MV MARINE EAGLE", "MT NAUTILUS PRIME", "MV OCEAN GUARDIAN", "MT POLAR STAR",
  "MV QUANTUM LEAP", "MT RED SEA GLORY", "MV SILK ROAD", "MT THUNDER BAY", "MV UNITED SEA",
] as const;

const FLAGS = [
  "Panama", "Liberia", "Marshall Islands", "Bahamas", "Malta", "Cyprus", "Cayman Islands",
  "Singapore", "Greece", "Norway", "Denmark", "United Kingdom", "China", "Japan",
  "Hong Kong", "Antigua and Barbuda", "St Kitts and Nevis", "Palau", "Cook Islands",
];
...
const VESSEL_STATUSES: Array<typeof vesselsTable.$inferInsert["status"]> = [
  "at_sea", "at_sea", "at_sea", "at_sea", "at_sea", "at_sea", "at_sea", "at_sea", "at_sea",
  "at_sea", "at_sea", "at_sea", "at_sea", "at_sea", "at_sea", "at_sea", "at_sea", "at_sea",
  "in_port", "in_port", "in_port", "in_port", "in_port", "in_port", "in_port", "in_port",
...
const DESTINATIONS = [
  "Rotterdam", "Singapore", "Shanghai", "Houston", "Yokohama", "Dubai", "Jebel Ali",
  "Los Angeles", "Colombo", "Cape Town", "Mumbai", "Hamburg", "Le Havre", "Antwerp",
  "Busan", "Qingdao", "Ningbo", "Port Said", "Piraeus", "Barcelona",
];

const OWNERS = [
  "Oceanic Shipping Corp", "Pacific Maritime Holdings", "Nordic Tankers AS", "Golden Gate Shipping",
  "Atlantic Bulk Carriers", "Mediterranean Lines SA", "Eastern Pacific Navigation",
  "Global Maritime Partners", "Titan Ocean Freight", "Blue Ocean Ventures Ltd",
  "Northern Star Maritime", "Coral Bay Shipping", "Viking Ocean Transport",
  "Emerald Seas Corp", "SZL Maritime Holdings", "Apex Fleet Management",
];

const MANAGERS = [
  "V.Group", "Anglo-Eastern", "Stolt-Nielsen", "MOL Ship Management", "Bernhard Schulte",
  "Thome Group", "Fleet Management Ltd", "Columbia Shipmanagement", "Wilhelmsen Ship Management",
  "Pacific Basin Shipping", "Wallem Group", "Döhle Seafront Crewing", "OSMC Maritime",
];
...
const CARGO_TYPES = [
  "Crude Oil", "Refined Products", "LNG", "LPG", "Iron Ore", "Coal", "Grain", "Bauxite",
  "Containers", "General Cargo", "Steel Products", "Fertilizers", "Chemical Products",
  "Phosphate", "Vehicles", "Timber",
];

const ORIGIN_PORTS = [
  "Rotterdam", "Singapore", "Shanghai", "Houston", "Yokohama", "Dubai", "Jebel Ali",
  "Los Angeles", "Colombo", "Cape Town", "Mumbai", "Ras Tanura", "Sullom Voe",
  "Port Hedland", "Gladstone", "Dampier",
];

const CHARTER_TYPES: Array<typeof vesselVoyageEconomicsTable.$inferInsert["charterType"]> = [
  "time_charter", "voyage_charter", "spot",
];

const VOYAGE_STATUSES: Array<typeof vesselVoyageEconomicsTable.$inferInsert["status"]> = [
  "at_sea", "completed", "completed", "planned",
];

const PORT_PURPOSES: Array<typeof vesselPortCallsTable.$inferInsert["purpose"]> = [
  "loading", "discharging", "bunkering", "crew_change", "repair", "inspection", "transit",
];

const CANAL_NAMES = ["Suez Canal", "Panama Canal", "Kiel Canal"] as const;

const PORT_AGENTS = [
  "Inchcape Shipping", "GAC Group", "Wilhelmsen Ships Service", "Swire Shipping", "Mediterranean Shipping Agency",
];

const MAINTENANCE_COMPONENTS = [
  "Main Engine — Turbocharger", "Main Engine — Fuel Injection System", "Main Engine — Cylinder Liner",
  "Auxiliary Engine 1", "Auxiliary Engine 2", "Boiler System",
  "Propeller — Blades Inspection", "Propeller Shaft Seals", "Bow Thruster",
  "Cargo Pump System", "Ballast Water Treatment System", "Bilge System",
  "Deckpipes and Manifold", "Fire Safety Equipment", "Lifesaving Appliances",
  "Navigation Equipment — ECDIS", "AIS Transponder", "GMDSS Radio",
  "Hull — Underwater Inspection", "Hull — Antifouling Coating", "Hull — Structural Survey",
  "Electrical System — Switchboard", "Air Conditioning System", "Fresh Water Generator",
  "Crane and Lifting Gear", "Anchoring Equipment", "Mooring Equipment",
];

const MAINTENANCE_TYPES: Array<typeof vesselMaintenanceTable.$inferInsert["maintenanceType"]> = [
  "preventive", "corrective", "scheduled", "predictive",
];
...
const MAINTENANCE_STATUSES: Array<typeof vesselMaintenanceTable.$inferInsert["status"]> = [
  "overdue", "due_soon", "scheduled", "in_progress", "completed",
];

const MAINTENANCE_TECHNICIANS = [
  "Chief Engineer", "2nd Engineer", "3rd Engineer", "Electrical Officer", "Contractor",
];
...
const EXCEPTION_TEMPLATES: Array<{
...
}> = [
  {
    ...
    descFn: (name) => `Vessel ${name} last transmitted AIS position 14 hours ago at Strait of Hormuz. No satellite or terrestrial AIS reception.`,
    whyItMatters: "Dark vessel events in high-risk zones suggest possible identity manipulation, sanctions evasion activity, or ship-to-ship transfer.",
    recommendedResponse: "Cross-reference with satellite imagery, contact charterer for vessel welfare, initiate behavioral risk review.",
  ...
  },
  {
    ...
    descFn: (name) => `${name} is 47nm off planned course, heading toward restricted waters near Iranian territorial limit.`,
    whyItMatters: "Unauthorized route deviation near sanctioned territory creates compliance exposure and insurance voidance risk.",
    recommendedResponse: "Contact master for explanation. If unresponsive, initiate emergency contact protocol and alert P&I club.",
  ...
  },
  {
    ...
    descFn: (name) => `Beneficial ownership screening for ${name} returned 87% confidence match against OFAC SDN list entity "Kish Shipping Ltd."`,
    whyItMatters: "Direct OFAC sanctions breach could result in civil penalties of $1M+ per violation and criminal prosecution.",
    recommendedResponse: "Suspend commercial engagement. Escalate to legal and compliance immediately. Do not make or receive payments.",
  ...
  },
  {
    ...
    descFn: (name) => `${name} is 28 hours overdue at Rotterdam. Last confirmed position 19h ago near Dogger Bank.`,
    whyItMatters: "Overdue arrival triggers demurrage claims at $38K/day and may indicate mechanical or weather emergency.",
    recommendedResponse: "Attempt contact via Inmarsat C and satellite phone. Alert port agent and terminal of delay. Check weather NAVTEX.",
  ...
  },
  {
    ...
    descFn: (name) => `${name} has been at anchor outside Shanghai waiting for berth for 6.3 days, exceeding 4-day demurrage-free period.`,
    whyItMatters: "Each additional day costs $52K in demurrage and fuel at anchor. Delays downstream cargo commitments.",
    recommendedResponse: "Negotiate alternative berth or transship to feeder. Apply force majeure clause if eligible.",
  ...
  },
  {
    ...
    descFn: (name) => `${name} has hove-to for weather at 40°S, 20°E due to Sea State 8 (Beaufort). ETA drifting +36h.`,
    whyItMatters: "Extended weather delay in Southern Ocean adds $78K in costs and risks cargo condition compliance.",
    recommendedResponse: "Monitor GRIB weather files. Advise charterer of force majeure. Assess deviation to sheltered anchorage.",
  ...
  },
  {
    ...
    descFn: (name) => `Engine room data shows ${name} main engine running at 108% MCR for 14h. Turbocharger surge detected twice.`,
    whyItMatters: "Continued overload risks catastrophic engine failure at sea, requiring costly tow and total loss of hire.",
    recommendedResponse: "Reduce speed to 85% MCR immediately. Schedule contingency engineering port call at nearest major port.",
  ...
  },
  {
    ...
    descFn: (name) => `${name} daily fuel consumption is 42 MT/day vs 31.2 MT/day budgeted. Anomaly persists for 8 days.`,
    whyItMatters: "At $680/MT bunker prices, 10.8MT daily excess costs $7,344/day, adding $52K to voyage P&L.",
    recommendedResponse: "Check hull fouling, propeller pitch, trim optimization. Review engine performance against sea trial data.",
  ...
  },
  {
    ...
    descFn: (name) => `${name} detained by Rotterdam PSC on 12 deficiencies including fire safety, MARPOL violations, and expired SOLAS certs.`,
    whyItMatters: "Detention prevents vessel from departing. Every day in detention costs $35K hire loss plus legal and rectification fees.",
    recommendedResponse: "Engage port agent and classification society immediately. Prioritize SOLAS fire safety items for release.",
  ...
  },
  {
    ...
    descFn: (name) => `IMB Piracy Reporting Center issued BMP5 security alert for vessel ${name} in Gulf of Guinea — 20nm NW of Lagos roads.`,
    whyItMatters: "Armed robbery and crew kidnapping risk in Gulf of Guinea is among highest in world. IMO security protocols required.",
    recommendedResponse: "Implement BMP5 procedures immediately. Contact MSCHOA. Increase watch routine. Notify flag state.",
  ...
  },
  {
    ...
    descFn: (name) => `Accumulated delays from weather and port congestion have pushed ${name} ETA by 22 hours beyond contracted window.`,
    whyItMatters: "ETA breach triggers laycan renegotiation and potential cargo rejection if spread exceeds tolerance.",
    recommendedResponse: "Notify charterer via CP email. Request laycan extension. If refused, proceed at master's discretion.",
...
export async function seedVesselsData(): Promise<void> {
  ...
  // Create fleets
  const fleetData = [
    { name: "SZL Tanker Fleet", description: "VLCC and Suezmax crude tankers", region: "Global", status: "active" as const },
    { name: "SZL Container Fleet", description: "Container and box ship operations", region: "Asia-Pacific", status: "active" as const },
    { name: "SZL Bulk Carrier Fleet", description: "Dry bulk commodities fleet", region: "Atlantic", status: "active" as const },
    { name: "SZL Specialty Fleet", description: "LNG, LPG, and chemical tankers", region: "Middle East", status: "active" as const },
  ];

  const insertedFleets = await db.insert(vesselsFleetsTable).values(fleetData).onConflictDoNothing().returning();
  ...
  const vesselRows = VESSEL_NAMES.map((name, i) => {
    ...
    const grossTonnage = vtype === "tanker" ? 45000 + seeded(i, 8, 80000) :
                         vtype === "container" ? 30000 + seeded(i, 8, 120000) :
                         vtype === "bulk" ? 20000 + seeded(i, 8, 60000) :
  ...
  for (let i = 0; i < positionRows.length; i += 50) {
    await db.insert(vesselsPositionsTable).values(positionRows.slice(i, i + 50)).onConflictDoNothing();
  }
  ...
  for (let v = 0; v < insertedVessels.length; v++) {
    ...
    for (let j = 0; j < 4; j++) {
      ...
      const totalCostsUsd = fuelCostUsd + portCostsUsd + canalFeesUsd + crewCostsUsd + maintenanceCostsUsd + otherCostsUsd;
      ...
      voyageRows.push({
        ...
        actualDepartureAt: status !== "planned" ? new Date(baseDate.getTime() + delayHours * 3600000) : null,
        scheduledArrivalAt: new Date(baseDate.getTime() + durationDays * 86400000),
        estimatedArrivalAt: new Date(baseDate.getTime() + (durationDays + delayHours / 24) * 86400000),
        actualArrivalAt: status === "completed" ? new Date(baseDate.getTime() + (durationDays + delayHours / 24) * 86400000 + 3600000) : null,
      });
    }
  }

  for (let i = 0; i < voyageRows.length; i += 50) {
    await db.insert(vesselVoyageEconomicsTable).values(voyageRows.slice(i, i + 50)).onConflictDoNothing();
  }
  ...
  for (let v = 0; v < insertedVessels.length; v++) {
    ...
    for (let j = 0; j < callCount; j++) {
      ...
      portCallRows.push({
        ...
        cargoDischarged: purpose === "discharging" ? (10000 + seeded(idx, 5, 50000)).toFixed(2) : "0",
  ...
  for (let i = 0; i < portCallRows.length; i += 50) {
    await db.insert(vesselPortCallsTable).values(portCallRows.slice(i, i + 50)).onConflictDoNothing();
  }
  ...
  for (let i = 0; i < exceptionRows.length; i += 50) {
    await db.insert(fleetExceptionsTable).values(exceptionRows.slice(i, i + 50)).onConflictDoNothing();
  }
  ...
  for (let v = 0; v < insertedVessels.length; v++) {
    ...
    for (let j = 0; j < itemCount; j++) {
      ...
      const dueDays = status === "overdue" ? -seeded(idx, 5, 30) :
                      status === "due_soon" ? seeded(idx, 6, 14) :
                      status === "completed" ? -seeded(idx, 7, 90) :
                      seeded(idx, 8, 180);
      const estimatedCost = 15000 + seeded(idx, 9, 285000);
      const riskScore = status === "overdue" ? 60 + seeded(idx, 10, 35) :
                        status === "due_soon" ? 40 + seeded(idx, 11, 30) :
                        10 + seeded(idx, 12, 30);
      const assetHealth = status === "overdue" ? 40 + seeded(idx, 13, 30) :
                          status === "completed" ? 75 + seeded(idx, 14, 20) :
                          55 + seeded(idx, 15, 35);
      ...
      maintenanceRows.push({
        ...
        completedAt: status === "completed" ? new Date(now.getTime() - seeded(idx, 16, 30) * 86400000) : null,
        estimatedCost: estimatedCost.toFixed(2),
        riskOfServiceIssue: riskScore.toFixed(2),
        impactsVoyageAvailability: priority === "critical" || (priority === "high" && status === "overdue"),
        assetHealth: assetHealth.toFixed(2),
        technician: pick(MAINTENANCE_TECHNICIANS, idx, 17),
        notes: status === "overdue" ? "Delayed due to port availability constraints. Urgent scheduling required." : null,
      });
    }
  }

  for (let i = 0; i < maintenanceRows.length; i += 50) {
    await db.insert(vesselMaintenanceTable).values(maintenanceRows.slice(i, i + 50)).onConflictDoNothing();
  }
  ...
  for (let v = 0; v < insertedVessels.length; v++) {
    ...
    const pscResult: PscResult = hasDeficiencies ? (seeded(v, 62, 1) > 0.7 ? "detained" : "deficiency") : "passed";
    ...
    const complianceScore = isHighRisk ? 15 + seeded(v, 63, 25) :
                            isMediumRisk ? 40 + seeded(v, 64, 30) :
                            70 + seeded(v, 65, 28);
    ...
    screeningRows.push({
      ...
      ukStatus,
      matchedLists,
      matchConfidence: isHighRisk ? (82 + seeded(v, 67, 16)).toFixed(2) :
                       isMediumRisk ? (55 + seeded(v, 68, 30)).toFixed(2) : null,
      flagRegistryValid: !isHighRisk,
      flagRegistryNotes: isHighRisk ? "Flag state does not provide vessel ownership transparency." : null,
      pscInspectionDate: daysAgo(Math.floor(seeded(v, 69, 180))),
      pscResult,
      pscDeficiencies: pscResult === "detained" ? Math.floor(5 + seeded(v, 70, 10)) :
                       pscResult === "deficiency" ? Math.floor(1 + seeded(v, 71, 5)) : 0,
      ...
      notes: isHighRisk ? "Vessel flagged for enhanced due diligence. Commercial engagement suspended pending legal review." :
             isMediumRisk ? "Beneficial ownership chain includes multiple SPV layers. Monitoring required." : null,
    });
  }

  for (let i = 0; i < screeningRows.length; i += 50) {
    await db.insert(vesselSanctionsScreeningTable).values(screeningRows.slice(i, i + 50)).onConflictDoNothing();
  }
