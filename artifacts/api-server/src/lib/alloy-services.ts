...
const SEVERITY_KEYWORDS: Array<{ keywords: string[]; severity: NonNullable<InsertAlloySignal["severity"]> }> = [
...
export const signalNormalizer = {
  ...
  _computeScore(severity: NonNullable<InsertAlloySignal["severity"]>, confidence: number): number {
    const weights: Record<string, number> = { critical: 1.0, high: 0.8, medium: 0.5, low: 0.25, info: 0.1 };
    return Math.min(1.0, (weights[severity] ?? 0.5) * confidence);
  },

  _assignTags(domain: string, category: string, severity: NonNullable<InsertAlloySignal["severity"]>, existing: string[]): string[] {
...
export const severityAssigner = {
  assess(signal: Partial<InsertAlloySignal>): SeverityAssessment {
    ...
    const severityScores: Record<string, number> = { critical: 1.0, high: 0.8, medium: 0.5, low: 0.25, info: 0.1 };
    const score = Math.min(1.0, (severityScores[sev] ?? 0.5) * conf);

    const varMap: Record<string, number> = { critical: 100000, high: 50000, medium: 10000, low: 1000, info: 0 };
    ...
    const workflowType: NonNullable<InsertAlloyWorkflow["type"]> =
...
export const ownerResolver = {
  async resolve(signal: Partial<AlloySignal>, options: { actorUserId?: number; domain?: string } = {}): Promise<OwnerResolution> {
...
export const valueAtRiskEstimator = {
  estimate(severity: NonNullable<InsertAlloySignal["severity"]>, confidence: number, domain?: string): {
    valueAtRisk: number;
    adjustedValueAtRisk: number;
    currency: string;
  } {
    const baseMap: Record<string, number> = { critical: 100000, high: 50000, medium: 10000, low: 1000, info: 0 };
    const domainMultiplier: Record<string, number> = {
      vessels: 2.5, firestorm: 2.0, lyte: 1.5, msp: 1.5, terra: 1.8, inca: 1.2, alloy: 1.0,
    };
...
export const runLogger = {
  async log(params: {
    ...
    try {
      ...
      _logger.error({ err, params: { entityType: params.entityType, entityId: params.entityId, action: params.action } }, "runLogger: audit write failed");
    }
  },
  ...
  async getAuditLog(filters: {
    ...
    if (filters.actorUserId != null) conditions.push(eq(alloyAuditLog.actorUserId, filters.actorUserId));
...
export const retryHandler = {
  ...
  async cancelRun(runId: number, actorUserId?: number): Promise<typeof alloyWorkflowRuns.$inferSelect | null> {
