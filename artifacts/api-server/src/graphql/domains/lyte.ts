...
async function buildLyteStorage(): Promise<LyteStoragePort> {
  ...
  const { lyteWorkspacesTable, lyteSignalsTable, lyteActionsTable, lyteIncidentsTable } = await import("@szl-holdings/db/schema");
  const { desc, eq, and } = await import("drizzle-orm");

  return {
    async listWorkspaces(args) {
      try { return await db.select().from(lyteWorkspacesTable).orderBy(desc(lyteWorkspacesTable.createdAt)).limit(args.limit).offset(args.offset); } catch { return []; }
    },
    async listSignals(args) {
      try {
        const conditions: ReturnType<typeof eq>[] = [];
        if (args.severity && VALID_SIGNAL_SEVERITIES.has(args.severity as SignalSeverity)) conditions.push(eq(lyteSignalsTable.severity, args.severity as SignalSeverity));
        if (args.status && VALID_SIGNAL_STATUSES.has(args.status as SignalStatus)) conditions.push(eq(lyteSignalsTable.status, args.status as SignalStatus));
        if (args.source) conditions.push(eq(lyteSignalsTable.source, args.source));
        const q = db.select().from(lyteSignalsTable).orderBy(desc(lyteSignalsTable.createdAt)).limit(args.limit).offset(args.offset);
        return conditions.length > 0 ? await q.where(and(...conditions)) : await q;
      } catch { return []; }
    },
    async getSignal(id) {
      try { const rows = await db.select().from(lyteSignalsTable).where(eq(lyteSignalsTable.id, id)).limit(1); return rows[0] ?? null; } catch { return null; }
    },
    async updateSignal(id, data) {
      const rows = await db.update(lyteSignalsTable).set(data).where(eq(lyteSignalsTable.id, id)).returning();
      return rows[0];
    },
    async listActions(args) {
      try {
        const conditions: ReturnType<typeof eq>[] = [];
        if (args.state && VALID_ACTION_STATES.has(args.state as ActionState)) conditions.push(eq(lyteActionsTable.state, args.state as ActionState));
        if (args.priority && VALID_ACTION_PRIORITIES.has(args.priority as ActionPriority)) conditions.push(eq(lyteActionsTable.priority, args.priority as ActionPriority));
        if (args.assignee) conditions.push(eq(lyteActionsTable.assignedTo, args.assignee));
        const q = db.select().from(lyteActionsTable).orderBy(desc(lyteActionsTable.createdAt)).limit(args.limit).offset(args.offset);
        return conditions.length > 0 ? await q.where(and(...conditions)) : await q;
      } catch { return []; }
    },
    async getAction(id) {
      try { const rows = await db.select().from(lyteActionsTable).where(eq(lyteActionsTable.id, id)).limit(1); return rows[0] ?? null; } catch { return null; }
    },
    async updateAction(id, data) {
      const rows = await db.update(lyteActionsTable).set(data).where(eq(lyteActionsTable.id, id)).returning();
      return rows[0];
    },
    async listIncidents(args) {
      try {
        const conditions: ReturnType<typeof eq>[] = [];
        if (args.status && VALID_INCIDENT_STATUSES.has(args.status as IncidentStatus)) conditions.push(eq(lyteIncidentsTable.status, args.status as IncidentStatus));
        if (args.severity && VALID_INCIDENT_SEVERITIES.has(args.severity as IncidentSeverity)) conditions.push(eq(lyteIncidentsTable.severity, args.severity as IncidentSeverity));
        if (args.assignee) conditions.push(eq(lyteIncidentsTable.assignee, args.assignee));
        const q = db.select().from(lyteIncidentsTable).orderBy(desc(lyteIncidentsTable.createdAt)).limit(args.limit).offset(args.offset);
        return conditions.length > 0 ? await q.where(and(...conditions)) : await q;
      } catch { return []; }
    },
    async getIncident(id) {
      try { const rows = await db.select().from(lyteIncidentsTable).where(eq(lyteIncidentsTable.id, id)).limit(1); return rows[0] ?? null; } catch { return null; }
    },
    async updateIncident(id, data) {
      const rows = await db.update(lyteIncidentsTable).set(data).where(eq(lyteIncidentsTable.id, id)).returning();
      return rows[0];
    },
    async countSignals() { return {}; },
    async countIncidents() { return {}; },
    async countActions() { return 0; },
  };
}
    ...
    lyteQueue: async (_: unknown, args: {
      ...
      try {
        ...
        if (!filter.entityType || filter.entityType === "signal") {
          ...
          if (filter.severity && VALID_SIGNAL_SEVERITIES.has(filter.severity as SignalSeverity)) sigConditions.push(eq(lyteSignalsTable.severity, filter.severity as SignalSeverity));
          if (filter.status && VALID_SIGNAL_STATUSES.has(filter.status as SignalStatus)) sigConditions.push(eq(lyteSignalsTable.status, filter.status as SignalStatus));
          if (filter.domain) sigConditions.push(eq(lyteSignalsTable.source, filter.domain));
          const sigQ = db.select().from(lyteSignalsTable).orderBy(desc(lyteSignalsTable.createdAt)).limit(limit + offset);
        ...
        if (!filter.entityType || filter.entityType === "incident") {
          ...
          if (filter.severity && VALID_INCIDENT_SEVERITIES.has(filter.severity as IncidentSeverity)) incConditions.push(eq(lyteIncidentsTable.severity, filter.severity as IncidentSeverity));
          if (filter.status && VALID_INCIDENT_STATUSES.has(filter.status as IncidentStatus)) incConditions.push(eq(lyteIncidentsTable.status, filter.status as IncidentStatus));
          ...
          const incQ = db.select().from(lyteIncidentsTable).orderBy(desc(lyteIncidentsTable.createdAt)).limit(limit + offset);
        ...
        if (!filter.entityType || filter.entityType === "action") {
          ...
          if (filter.status && VALID_ACTION_STATES.has(filter.status as ActionState)) actConditions.push(eq(lyteActionsTable.state, filter.status as ActionState));
          if (filter.priority && VALID_ACTION_PRIORITIES.has(filter.priority as ActionPriority)) actConditions.push(eq(lyteActionsTable.priority, filter.priority as ActionPriority));
          if (filter.assignee) actConditions.push(eq(lyteActionsTable.assignedTo, filter.assignee));
          const actQ = db.select().from(lyteActionsTable).orderBy(desc(lyteActionsTable.createdAt)).limit(limit + offset);
    ...
    lyteExecutiveSummary: async () => {
      try {
        ...
        return {
          ...
          totalSignals: signals.length, criticalSignals, highSignals,
          openIncidents, criticalIncidents, pendingActions,
  ...
  Mutation: {
    triageLyteSignal: async (_: unknown, args: { id: string; status: string; rationale?: string; nextAction?: string }) => {
      try {
        ...
        pubsub.publish(LYTE_EVENTS.QUEUE_CHANGED, { lyteQueueChanged: buildSignalQueueItem(result as Record<string, unknown>) });
        return result;
    ...
    assignLyteSignalOwner: async (_: unknown, args: { id: string; assignee: string }) => {
      try {
        const result = await assignLyteSignalOwner(await buildLyteStorage(), parseIntId(args.id), args.assignee);
        pubsub.publish(LYTE_EVENTS.SIGNAL_UPDATED, { lyteSignalUpdated: result });
        pubsub.publish(LYTE_EVENTS.QUEUE_CHANGED, { lyteQueueChanged: buildSignalQueueItem(result as Record<string, unknown>) });
        return result;
    ...
    escalateLyteSignal: async (_: unknown, args: { id: string; reason?: string; targetRole: string }) => {
      try {
        const result = await escalateLyteSignal(await buildLyteStorage(), parseIntId(args.id), args);
        pubsub.publish(LYTE_EVENTS.SIGNAL_UPDATED, { lyteSignalUpdated: result });
        pubsub.publish(LYTE_EVENTS.QUEUE_CHANGED, { lyteQueueChanged: buildSignalQueueItem(result as Record<string, unknown>) });
        return result;
    ...
    updateLyteIncident: async (_: unknown, args: { id: string; status: string }) => {
      try {
        const result = await updateLyteIncident(await buildLyteStorage(), parseIntId(args.id), args.status);
        pubsub.publish(LYTE_EVENTS.INCIDENT_UPDATED, { lyteIncidentUpdated: result });
        pubsub.publish(LYTE_EVENTS.QUEUE_CHANGED, { lyteQueueChanged: buildIncidentQueueItem(result as Record<string, unknown>) });
        return result;
    ...
    assignLyteIncidentOwner: async (_: unknown, args: { id: string; assignee: string }) => {
      try {
        const result = await assignLyteIncidentOwner(await buildLyteStorage(), parseIntId(args.id), args.assignee);
        pubsub.publish(LYTE_EVENTS.INCIDENT_UPDATED, { lyteIncidentUpdated: result });
        pubsub.publish(LYTE_EVENTS.QUEUE_CHANGED, { lyteQueueChanged: buildIncidentQueueItem(result as Record<string, unknown>) });
        return result;
    ...
    escalateLyteIncident: async (_: unknown, args: { id: string; reason?: string; targetRole: string }) => {
      try {
        const result = await escalateLyteIncident(await buildLyteStorage(), parseIntId(args.id), args);
        pubsub.publish(LYTE_EVENTS.INCIDENT_UPDATED, { lyteIncidentUpdated: result });
        pubsub.publish(LYTE_EVENTS.QUEUE_CHANGED, { lyteQueueChanged: buildIncidentQueueItem(result as Record<string, unknown>) });
        return result;
    ...
    resolveLyteIncident: async (_: unknown, args: { id: string; resolution: string; rootCause?: string }) => {
      try {
        const result = await resolveLyteIncident(await buildLyteStorage(), parseIntId(args.id), args);
        pubsub.publish(LYTE_EVENTS.INCIDENT_UPDATED, { lyteIncidentUpdated: result });
        pubsub.publish(LYTE_EVENTS.QUEUE_CHANGED, { lyteQueueChanged: buildIncidentQueueItem(result as Record<string, unknown>) });
        return result;
    ...
    updateLyteActionState: async (_: unknown, args: { id: string; state: string; rationale?: string }) => {
      try {
        const result = await updateLyteActionState(await buildLyteStorage(), parseIntId(args.id), args);
        pubsub.publish(LYTE_EVENTS.QUEUE_CHANGED, { lyteQueueChanged: buildActionQueueItem(result as Record<string, unknown>) });
        return result;
    ...
    assignLyteActionOwner: async (_: unknown, args: { id: string; assignedTo: string }) => {
      try {
        const result = await assignLyteActionOwner(await buildLyteStorage(), parseIntId(args.id), args.assignedTo);
        pubsub.publish(LYTE_EVENTS.QUEUE_CHANGED, { lyteQueueChanged: buildActionQueueItem(result as Record<string, unknown>) });
        return result;
    ...
    escalateLyteAction: async (_: unknown, args: { id: string; reason?: string; targetRole: string }) => {
      try {
        const result = await escalateLyteAction(await buildLyteStorage(), parseIntId(args.id), args);
        pubsub.publish(LYTE_EVENTS.QUEUE_CHANGED, { lyteQueueChanged: buildActionQueueItem(result as Record<string, unknown>) });
        return result;
