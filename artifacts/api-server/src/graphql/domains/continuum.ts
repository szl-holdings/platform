...
//
// Lifecycle:
//   draft           → pending (submit via submitContinuumWorkflow) | cancelled
...
// Valid DB enum values for continuum_workflow_runs.status
...
// Valid DB enum values for continuum_workflows.type
type WorkflowType =
  | 'investigation'
  | 'remediation'
  | 'escalation'
...
// Valid DB enum values for continuum_workflows.priority
type WorkflowPriority = 'low' | 'medium' | 'high' | 'critical';
const VALID_PRIORITIES = new Set<WorkflowPriority>(['low', 'medium', 'high', 'critical']);

// Valid DB enum values for continuum_workflows.status (now includes "draft" as initial state)
type WorkflowStatus =
  | 'draft'
  | 'pending'
  | 'running'
...
// Valid DB enum values for continuum_actions.type
type ActionType =
  | 'alert'
  | 'notify'
  | 'escalate'
...
export const continuumTypeDefs = `#graphql
  # ── Workflow State Machine ────────────────────────────────────────────────────
  ...
  type ContinuumWorkflow {
    id: ID!
    name: String!
    description: String
    type: String
    domain: String
    status: ContinuumWorkflowStatus!
    priority: ContinuumPriority
    requiresApproval: Boolean!
    approvalState: ContinuumApprovalState!
    confidenceScore: Float
    triggerId: ID
    triggerType: String
    environment: String
    steps: [ContinuumWorkflowStep!]
    currentStep: Int
    retryCount: Int
    ownerUserId: ID
    startedAt: String
  ...
  type ContinuumWorkflowRun {
    ...
    stepsExecuted: [ContinuumWorkflowStep!]
    startedAt: String
    completedAt: String
  }

  type ContinuumApproval {
    id: ID!
    workflowId: ID!
    status: String!
    reason: String
  ...
  type ContinuumDashboardStats {
    ...
    successRate: Float!
    avgDurationMs: Float
    workflowsByStatus: [ContinuumStatusCount!]!
    recentActivity: [ContinuumAuditEntry!]!
  }

  type ContinuumStatusCount {
    status: String!
    count: Int!
  }
  ...
  extend type Query {
    alloySignals(limit: Int, offset: Int, severity: String, status: String, domain: String): [ContinuumSignal!]!
    alloySignal(id: ID!): ContinuumSignal

    alloyWorkflows(limit: Int, offset: Int, status: String, priority: String, domain: String): [ContinuumWorkflow!]!
    alloyWorkflow(id: ID!): ContinuumWorkflow
    alloyWorkflowStateTransitions(workflowId: ID!): [String!]!

    alloyWorkflowRuns(workflowId: ID, limit: Int, offset: Int, status: String): [ContinuumWorkflowRun!]!
    alloyWorkflowRun(id: ID!): ContinuumWorkflowRun

    alloyApprovals(workflowId: ID, status: String, limit: Int, offset: Int): [ContinuumApproval!]!
    alloyApproval(id: ID!): ContinuumApproval

    alloyActions(workflowId: ID, limit: Int, offset: Int): [ContinuumAction!]!
    alloyArtifacts(workflowId: ID, domain: String, limit: Int, offset: Int): [ContinuumArtifact!]!

    alloyAuditLog(entityType: String, entityId: ID, limit: Int, offset: Int): [ContinuumAuditEntry!]!
    alloyDashboard: ContinuumDashboardStats!
  }
  ...
  extend type Mutation {
    ...
    createContinuumWorkflow(name: String!, type: String, priority: String, description: String, domain: String, requiresApproval: Boolean): ContinuumWorkflow!
    submitContinuumWorkflow(id: ID!): ContinuumWorkflow!
    cancelContinuumWorkflow(id: ID!, reason: String): ContinuumWorkflow!
    retryContinuumWorkflow(id: ID!): ContinuumWorkflow!

    # Approval flow
    requestContinuumApproval(workflowId: ID!, reason: String, reviewerUserId: ID, expiresInHours: Int): ContinuumApproval!
    reviewContinuumApproval(approvalId: ID!, decision: String!, reviewNote: String, reviewerUserId: ID!): ContinuumApproval!

    # Execution
    runContinuumWorkflow(workflowId: ID!, overrideApproval: Boolean): ContinuumWorkflowRun!
    advanceContinuumWorkflowStep(runId: ID!, stepNumber: Int!, result: String!, error: String): ContinuumWorkflowRun!

    # Signal processing
    createContinuumSignalWorkflow(signalId: ID!, workflowType: String, priority: String): ContinuumWorkflow

    # Actions — type must be a valid continuum_actions.type enum value
    recordContinuumAction(workflowId: ID!, type: String!, description: String, outcome: String): ContinuumAction!
  }

  # ── Subscriptions ─────────────────────────────────────────────────────────────

  extend type Subscription {
    alloyWorkflowRunUpdated(workflowId: ID): ContinuumWorkflowRun!
    alloyApprovalRequired(reviewerUserId: ID): ContinuumApproval!
    alloyWorkflowStatusChanged: ContinuumWorkflow!
  }
`;
...
export const continuumResolvers = {
  Query: {
    alloySignals: async (
      _: unknown,
      args: {
  ...
  Mutation: {
    ...
    retryContinuumWorkflow: async (_: unknown, args: { id: string }) => {
      try {
        const { db } = await import('@szl-holdings/db');
        const { alloyWorkflows } = await import('@szl-holdings/db/schema');
        const { eq } = await import('drizzle-orm');
    ...
    requestContinuumApproval: async (
      _: unknown,
      args: {
        workflowId: string;
        reason?: string;
      ...
      try {
        ...
        if (isAlreadyWaiting) {
          ...
          if (pendingApproval)
            throw new Error(
              'Workflow already has a pending approval — use reviewContinuumApproval to resolve it',
            );
        }
    ...
    createContinuumSignalWorkflow: async (
      ...
      try {
        ...
        const [signal] = await db
          ...
          .limit(1);
        if (signal) {
          domainEventBus.publish('continuum.signal-ingested', {
            signalId: signal.id,
        ...
        if (workflow) {
          domainEventBus.publish('continuum.workflow-created', {
