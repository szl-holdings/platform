...
const VALID_RUN_STATUSES = new Set<WorkflowRunStatus>(["started", "completed", "failed", "cancelled"]);

// Valid DB enum values for alloy_workflows.type
type WorkflowType = "investigation" | "remediation" | "escalation" | "review" | "notification" | "report" | "custom";
const VALID_WORKFLOW_TYPES = new Set<WorkflowType>(["investigation", "remediation", "escalation", "review", "notification", "report", "custom"]);
...
type WorkflowStatus = "draft" | "pending" | "running" | "waiting_approval" | "approved" | "rejected" | "completed" | "failed" | "cancelled";
const VALID_WORKFLOW_STATUSES = new Set<WorkflowStatus>(["draft", "pending", "running", "waiting_approval", "approved", "rejected", "completed", "failed", "cancelled"]);

// Valid DB enum values for alloy_actions.type
type ActionType = "alert" | "notify" | "escalate" | "assign" | "resolve" | "suppress" | "review" | "remediate" | "report" | "custom";
const VALID_ACTION_TYPES = new Set<ActionType>(["alert", "notify", "escalate", "assign", "resolve", "suppress", "review", "remediate", "report", "custom"]);
  ...
  Query: {
    alloySignals: async (_: unknown, args: { limit?: number; offset?: number; severity?: string; status?: string; domain?: string }) => {
      try {
        ...
        if (args.status) conditions.push(eq(alloySignals.status, args.status as typeof alloySignals.status._.data));
        if (args.domain) conditions.push(eq(alloySignals.domain, args.domain));
        const q = db.select().from(alloySignals).orderBy(desc(alloySignals.createdAt))
    ...
    alloyWorkflows: async (_: unknown, args: { limit?: number; offset?: number; status?: string; priority?: string; domain?: string }) => {
      try {
        ...
        const q = db.select().from(alloyWorkflows).orderBy(desc(alloyWorkflows.createdAt))
          .limit(args.limit ?? 50).offset(args.offset ?? 0);
    ...
    alloyWorkflowRuns: async (_: unknown, args: { workflowId?: string; limit?: number; offset?: number; status?: string }) => {
      try {
        ...
        const q = db.select().from(alloyWorkflowRuns).orderBy(desc(alloyWorkflowRuns.startedAt))
          .limit(args.limit ?? 50).offset(args.offset ?? 0);
        return conditions.length > 0 ? await q.where(and(...conditions)) : await q;
      } catch { return []; }
    },
    ...
    alloyApprovals: async (_: unknown, args: { workflowId?: string; status?: string; limit?: number; offset?: number }) => {
      try {
        ...
        const conditions: ReturnType<typeof eq>[] = [];
        if (args.workflowId) conditions.push(eq(alloyApprovals.workflowId, parseInt(args.workflowId, 10)));
        ...
        const q = db.select().from(alloyApprovals).orderBy(desc(alloyApprovals.createdAt))
          .limit(args.limit ?? 50).offset(args.offset ?? 0);
        return conditions.length > 0 ? await q.where(and(...conditions)) : await q;
      } catch { return []; }
    },
    ...
    alloyActions: async (_: unknown, args: { workflowId?: string; limit?: number; offset?: number }) => {
      try {
        ...
        const q = db.select().from(alloyActions).orderBy(desc(alloyActions.createdAt))
          .limit(args.limit ?? 50).offset(args.offset ?? 0);
        if (args.workflowId) return await q.where(eq(alloyActions.workflowId, parseInt(args.workflowId, 10)));
        return await q;
      } catch { return []; }
    },
    ...
    alloyArtifacts: async (_: unknown, args: { workflowId?: string; domain?: string; limit?: number; offset?: number }) => {
      try {
        ...
        const q = db.select().from(alloyArtifacts).orderBy(desc(alloyArtifacts.createdAt))
          .limit(args.limit ?? 50).offset(args.offset ?? 0);
        return conditions.length > 0 ? await q.where(and(...conditions)) : await q;
      } catch { return []; }
    },
    ...
    alloyAuditLog: async (_: unknown, args: { entityType?: string; entityId?: string; limit?: number; offset?: number }) => {
      try {
        ...
        const q = db.select().from(alloyAuditLog).orderBy(desc(alloyAuditLog.createdAt))
    ...
    alloyDashboard: async () => {
      try {
        ...
        const avgDurationMs = completedWithDuration.length > 0
          ? completedWithDuration.reduce((s, r) => s + (r.durationMs ?? 0), 0) / completedWithDuration.length
          : null;
        ...
        return {
          ...
          recentActivity: recentAudit.map(r => auditEntrySerialize(r as unknown as Record<string, unknown>)),
        };
      } catch {
        return { totalWorkflows: 0, totalRuns: 0, runningRuns: 0, pendingApprovals: 0, failedRuns: 0, successRate: 100, avgDurationMs: null, workflowsByStatus: [], recentActivity: [] };
      }
    },
  },
  ...
  Mutation: {
    createAlloyWorkflow: async (_: unknown, args: { name: string; type?: string; priority?: string; description?: string; domain?: string; requiresApproval?: boolean }) => {
      try {
        ...
        const wf = rows[0];
        await writeAuditLog({ entityType: "workflow", entityId: wf.id, action: "created", actorType: "user", newState: { status: "draft", name: args.name } });
        return enrichWorkflow(wf as unknown as Record<string, unknown>);
      } catch (err) { throw new Error(`Failed to create workflow: ${err}`); }
    },
    ...
    submitAlloyWorkflow: async (_: unknown, args: { id: string }) => {
      try {
        ...
        if (existing.status !== "draft") {
          throw new Error(`Cannot submit workflow in state '${existing.status}' — only draft workflows can be submitted`);
        }
        if (!canTransition(existing.status, "pending")) {
          throw new Error(`State machine error: draft → pending transition not allowed`);
        }
        const rows = await db.update(alloyWorkflows)
          .set({ status: "pending", updatedAt: new Date() })
          .where(eq(alloyWorkflows.id, id)).returning();
        const wf = rows[0];
        await writeAuditLog({ entityType: "workflow", entityId: id, action: "submitted", actorType: "user", previousState: { status: "draft" }, newState: { status: "pending" } });
        pubsub.publish(ALLOY_EVENTS.WORKFLOW_STATUS_CHANGED, { alloyWorkflowStatusChanged: enrichWorkflow(wf as unknown as Record<string, unknown>) });
        return enrichWorkflow(wf as unknown as Record<string, unknown>);
      } catch (err) { throw new Error(`Failed to submit workflow: ${err}`); }
    },
    ...
    cancelAlloyWorkflow: async (_: unknown, args: { id: string; reason?: string }) => {
      try {
        ...
        const rows = await db.update(alloyWorkflows)
          ...
          .where(eq(alloyWorkflows.id, id)).returning();
        const wf = rows[0];
        await writeAuditLog({ entityType: "workflow", entityId: id, action: "cancelled", actorType: "user", previousState: { status: existing.status }, newState: { status: "cancelled" }, notes: args.reason });
        pubsub.publish(ALLOY_EVENTS.WORKFLOW_STATUS_CHANGED, { alloyWorkflowStatusChanged: enrichWorkflow(wf as unknown as Record<string, unknown>) });
        return enrichWorkflow(wf as unknown as Record<string, unknown>);
      } catch (err) { throw new Error(`Failed to cancel workflow: ${err}`); }
    },
    ...
    retryAlloyWorkflow: async (_: unknown, args: { id: string }) => {
      try {
        ...
        if (existing.status !== "failed") {
          throw new Error(`Cannot retry workflow in state '${existing.status}' — only failed workflows can be retried`);
        }
        if (!canTransition(existing.status, "pending")) {
        ...
        const rows = await db.update(alloyWorkflows)
          ...
          .where(eq(alloyWorkflows.id, id)).returning();
        const wf = rows[0];
        await writeAuditLog({ entityType: "workflow", entityId: id, action: "retry_requested", actorType: "user", previousState: { status: existing.status }, newState: { status: "pending" } });
        return enrichWorkflow(wf as unknown as Record<string, unknown>);
      } catch (err) { throw new Error(`Failed to retry workflow: ${err}`); }
    },
    ...
    requestAlloyApproval: async (_: unknown, args: { workflowId: string; reason?: string; reviewerUserId?: string; expiresInHours?: number }) => {
      try {
        ...
        if (!isPending && !isAlreadyWaiting) {
          throw new Error(`Cannot request approval for workflow in state '${existing.status}' — workflow must be in 'pending' or 'waiting_approval' state`);
        }
        if (isAlreadyWaiting) {
          // Idempotent: only re-create approval if no pending approval exists
          const [pendingApproval] = await db.select({ id: alloyApprovals.id })
            .from(alloyApprovals)
            .where(and(eq(alloyApprovals.workflowId, workflowId), eq(alloyApprovals.status, "pending")))
            .limit(1);
          if (pendingApproval) throw new Error("Workflow already has a pending approval — use reviewAlloyApproval to resolve it");
        }
        ...
        const rows = await db.select().from(alloyApprovals)
          .where(eq(alloyApprovals.workflowId, workflowId))
          .orderBy(desc(alloyApprovals.createdAt)).limit(1);
        const approval = rows[0];
        pubsub.publish(ALLOY_EVENTS.APPROVAL_REQUIRED, { alloyApprovalRequired: approval });
        return approval;
      } catch (err) { throw new Error(`Failed to request approval: ${err}`); }
    },

    reviewAlloyApproval: async (_: unknown, args: { approvalId: string; decision: string; reviewNote?: string; reviewerUserId: string }) => {
      try {
        ...
        if (approval.status !== "pending") throw new Error(`Approval is already '${approval.status}' — cannot review again`);
        const [workflow] = await db.select({ status: alloyWorkflows.status }).from(alloyWorkflows).where(eq(alloyWorkflows.id, approval.workflowId)).limit(1);
        if (workflow && !canTransition(workflow.status, args.decision as "approved" | "rejected")) {
          throw new Error(`Cannot ${args.decision} approval for workflow in state '${workflow.status}' — must be in 'waiting_approval'`);
        }
        await reviewApproval(approvalId, args.decision as "approved" | "rejected", {
          reviewerUserId,
          reviewNote: args.reviewNote,
        });
        const rows = await db.select().from(alloyApprovals).where(eq(alloyApprovals.id, approvalId)).limit(1);
        return rows[0];
      } catch (err) { throw new Error(`Failed to review approval: ${err}`); }
    },
    ...
    runAlloyWorkflow: async (_: unknown, args: { workflowId: string; overrideApproval?: boolean }) => {
      try {
        ...
        if (!canTransition(existing.status, "running")) {
          throw new Error(`Cannot start workflow in state '${existing.status}' — must be in pending or approved state`);
        }
        ...
        publish(WS_CHANNELS.WORKFLOW_RUNS, "workflow-run-started", { id: run.id, workflowId: run.workflowId, status: run.status });
        return run;
      } catch (err) { throw new Error(`Failed to run workflow: ${err}`); }
    },

    advanceAlloyWorkflowStep: async (_: unknown, args: { runId: string; stepNumber: number; result: string; error?: string }) => {
      try {
        ...
        pubsub.publish(ALLOY_EVENTS.WORKFLOW_RUN_UPDATED, { alloyWorkflowRunUpdated: run });
        return run;
      } catch (err) { throw new Error(`Failed to advance step: ${err}`); }
    },
    ...
    recordAlloyAction: async (_: unknown, args: { workflowId: string; type: string; description?: string; outcome?: string }) => {
      try {
        ...
        const action = rows[0];
        await writeAuditLog({ entityType: "action", entityId: action.id, action: "recorded", actorType: "user", newState: { type: actionType } });
        return action;
      } catch (err) { throw new Error(`Failed to record action: ${err}`); }
    },
  },
  ...
  Subscription: {
    ...
    alloyApprovalRequired: {
      subscribe: withFilter(
        ...
        (payload: { alloyApprovalRequired: { reviewerUserId?: number } }, variables: { reviewerUserId?: string }) => {
