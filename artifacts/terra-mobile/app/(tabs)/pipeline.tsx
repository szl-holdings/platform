...
function LeadCard({ lead, onEdit }: { lead: Lead; onEdit: (lead: Lead) => void }) {
  ...
  const scoreColor = lead.score >= 80 ? colors.emerald : lead.score >= 60 ? colors.amber : colors.rose;

  return (
    <Pressable onPress={() => { Haptics.selectionAsync(); onEdit(lead); }} style={[styles.leadCard, { borderColor: colors.border, backgroundColor: "rgba(255,255,255,0.02)" }]}>
      <View style={styles.leadTop}>
        <View style={{ flex: 1 }}>
          <Text style={[styles.leadName, { color: colors.cream }]}>{lead.name}</Text>
          <Text style={[styles.leadAddress, { color: colors.mutedForeground }]} numberOfLines={1}>{lead.address}</Text>
        </View>
        <View style={[styles.leadScore, { borderColor: scoreColor + "40", backgroundColor: scoreColor + "10" }]}>
          <Text style={[styles.leadScoreText, { color: scoreColor }]}>{lead.score}</Text>
        </View>
      </View>
      <View style={styles.leadMeta}>
        <View style={[styles.stageChip, { backgroundColor: stageColor + "15", borderColor: stageColor + "30" }]}>
          <Text style={[styles.stageText, { color: stageColor }]}>{STAGE_LABELS[lead.stage] ?? lead.stage}</Text>
        </View>
      ...
      <View style={[styles.leadFooter, { borderTopColor: colors.border }]}>
        <View style={styles.leadFooterItem}>
          ...
          <Text style={[styles.leadFooterText, { color: colors.mutedForeground }]}>{lead.lastContact}</Text>
        </View>
        <View style={styles.leadFooterItem}>
          <Feather name="arrow-right" size={10} color={colors.gold} />
          <Text style={[styles.leadFooterText, { color: colors.goldDim }]} numberOfLines={1}>{lead.nextAction}</Text>
        </View>
      </View>
    </Pressable>
  );
...
export default function PipelineTab() {
  ...
  const apiLeads: Lead[] = rawApiLeads.map((r: {
    id?: string; externalId?: string; firstName?: string; lastName?: string; ownerName?: string;
    stage?: string; score?: number; source?: string; priority?: string; notes?: string;
    propertyAddress?: string; nextAction?: string; lastContact?: string; estimatedValue?: number;
  }) => ({
    id: r.id ?? r.externalId ?? String(Math.random()),
    name: (r.ownerName ?? ((r.firstName ?? "") + " " + (r.lastName ?? "")).trim()) || "Unknown Owner",
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.statsRow}>
        {[
          ...
          { label: "ACTIVE LEADS", value: String(allLeads.filter(l => l.stage !== "closed").length), color: colors.cream },
          { label: "HOT LEADS", value: String(allLeads.filter(l => l.priority === "high").length), color: colors.rose },
        ].map(stat => (
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.stageScroll}>
        {STAGES.map(s => {
          ...
          return (
            <Pressable
              key={s}
              onPress={() => { Haptics.selectionAsync(); setSelectedStage(s); }}
              style={[styles.stageChip, { borderColor: isSelected ? stageColor : colors.border, backgroundColor: isSelected ? stageColor + "15" : "transparent" }]}
            >
              <Text style={[styles.stageText, { color: isSelected ? stageColor : colors.mutedForeground }]}>
      ...
      >
        <Pressable style={styles.modalOverlay} onPress={() => setShowEditModal(false)}>
          <Pressable style={[styles.modalSheet, { backgroundColor: colors.surfaceElevated, borderColor: colors.border }]} onPress={() => {}}>
            ...
            <Text style={[styles.modalAddress, { color: colors.mutedForeground }]}>{selectedLead?.address}</Text>
            ...
            <View style={styles.stageOptions}>
              {["new", "contacted", "analyzing", "negotiating", "closed"].map(s => {
                ...
                return (
                  <Pressable
                    key={s}
                    onPress={() => { Haptics.selectionAsync(); setEditStage(s); }}
            ...
            <TextInput
              ...
              style={[styles.noteInput, { color: colors.cream, borderColor: colors.border, backgroundColor: colors.surface }]}
            />
            ...
            <Pressable
              ...
              style={[styles.saveLeadBtn, { backgroundColor: colors.goldDim, borderColor: colors.goldBorder, opacity: updateLead.isPending ? 0.6 : 1 }]}
            >
              <Text style={[styles.saveLeadText, { color: colors.gold }]}>{updateLead.isPending ? "Saving..." : "Save Update"}</Text>
            </Pressable>
          </Pressable>
        </Pressable>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  header: { paddingHorizontal: 20, paddingBottom: 14, flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between" },
  ...
  addBtn: { width: 36, height: 36, borderRadius: 18, borderWidth: 1, alignItems: "center", justifyContent: "center", marginTop: 2 },
  ...
  statLabel: { fontSize: 7, fontFamily: "Inter_500Medium", letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 },
  ...
  leadTop: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", padding: 14, paddingBottom: 8 },
  leadName: { fontSize: 13, fontFamily: "Inter_500Medium", marginBottom: 3 },
  leadAddress: { fontSize: 10, fontFamily: "Inter_300Light" },
  leadScore: { width: 36, height: 36, borderRadius: 18, borderWidth: 1, alignItems: "center", justifyContent: "center" },
  leadScoreText: { fontSize: 12, fontFamily: "Inter_600SemiBold" },
  leadMeta: { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 14, paddingBottom: 10 },
  priorityDot: { width: 6, height: 6, borderRadius: 3 },
  leadValue: { fontSize: 12, fontFamily: "Inter_600SemiBold" },
  leadFooter: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", borderTopWidth: 1, paddingHorizontal: 14, paddingVertical: 8 },
  ...
  modalSheet: { borderRadius: 20, borderWidth: 1, borderBottomLeftRadius: 0, borderBottomRightRadius: 0, padding: 20, paddingBottom: 40 },
  ...
  noteInput: { borderWidth: 1, borderRadius: 10, padding: 12, minHeight: 80, fontSize: 13, fontFamily: "Inter_300Light", lineHeight: 20, marginBottom: 16 },
