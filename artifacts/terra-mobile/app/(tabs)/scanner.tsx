...
function SwipeCard({
  ...
  const scoreColor = card.score >= 80 ? colors.emerald : card.score >= 60 ? colors.amber : colors.rose;
  ...
  const panResponder = PanResponder.create({
    ...
    onPanResponderRelease: (_, gs) => {
      if (gs.dx > 80) {
        ...
        Animated.spring(pan, { toValue: { x: 400, y: 0 }, useNativeDriver: true }).start(onSwipeRight);
      } else if (gs.dx < -80) {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning);
        Animated.spring(pan, { toValue: { x: -400, y: 0 }, useNativeDriver: true }).start(onSwipeLeft);
      } else {
        setAction(null);
  ...
  return (
    <Animated.View
      ...
      style={[
        styles.swipeCard,
        {
          ...
          borderColor: action === "right" ? colors.emerald + "60" : action === "left" ? colors.rose + "60" : colors.border,
          transform: [
    ...
    >
      {action && (
        <View style={[styles.swipeLabel, { backgroundColor: action === "right" ? colors.emerald + "20" : colors.rose + "20", borderColor: action === "right" ? colors.emerald + "40" : colors.rose + "40" }]}>
          <Text style={[styles.swipeLabelText, { color: action === "right" ? colors.emerald : colors.rose }]}>
      ...
      <View style={styles.cardTop}>
        <View style={[styles.distressChip, { backgroundColor: typeColor + "15", borderColor: typeColor + "30" }]}>
          <Text style={[styles.distressText, { color: typeColor }]}>{card.distressType}</Text>
        </View>
        <View style={[styles.scoreBox, { borderColor: scoreColor + "40", backgroundColor: scoreColor + "10" }]}>
      ...
      <View style={[styles.cardStats, { borderTopColor: colors.border, borderBottomColor: colors.border }]}>
        {[
          { label: "ASKING", value: card.price },
          { label: "DAYS", value: String(card.daysListed) },
          { label: "OWNER", value: card.ownerName.split(" ").slice(-1)[0] },
        ].map((stat, i) => (
          <View key={stat.label} style={[styles.stat, { borderLeftWidth: i > 0 ? 1 : 0, borderLeftColor: colors.border }]}>
            <Text style={[styles.statLabel, { color: colors.mutedForeground }]}>{stat.label}</Text>
            <Text style={[styles.statValue, { color: colors.cream }]} numberOfLines={1}>{stat.value}</Text>
          </View>
        ))}
      </View>
      ...
      <View style={styles.cardActions}>
        <Pressable
          onPress={() => { Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning); onSwipeLeft(); }}
          style={[styles.cardAction, { backgroundColor: colors.rose + "10", borderColor: colors.rose + "30" }]}
        >
          <Feather name="x" size={22} color={colors.rose} />
        </Pressable>
        <Pressable
          onPress={() => { Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); onSwipeRight(); }}
          style={[styles.cardAction, { backgroundColor: colors.emerald + "10", borderColor: colors.emerald + "30" }]}
        >
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.instructions}>
        <View style={styles.instrItem}>
          ...
          <Text style={[styles.instrText, { color: colors.mutedForeground }]}>Swipe right to add to pipeline</Text>
        </View>
        <View style={styles.instrItem}>
          <Feather name="arrow-left" size={12} color={colors.rose} />
          <Text style={[styles.instrText, { color: colors.mutedForeground }]}>Swipe left to dismiss</Text>
        </View>
      </View>
      ...
      <View style={styles.deck}>
        {isLoadingCards ? (
          <View style={[styles.emptyDeck, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <Text style={[styles.emptySubtitle, { color: colors.mutedForeground }]}>Fetching top distress properties</Text>
          </View>
        ) : cards.length === 0 ? (
          <View style={[styles.emptyDeck, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <Pressable
              onPress={resetCards}
              style={[styles.resetBtn, { backgroundColor: colors.goldDim, borderColor: colors.goldBorder }]}
            >
...
const styles = StyleSheet.create({
  ...
  header: { paddingHorizontal: 20, paddingBottom: 8, flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between" },
  ...
  counter: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 },
  ...
  swipeLabel: { position: "absolute", top: 16, right: 16, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8, borderWidth: 1 },
  ...
  scoreBox: { width: 52, height: 52, borderRadius: 26, borderWidth: 1, alignItems: "center", justifyContent: "center" },
  ...
  cardStats: { flexDirection: "row", borderTopWidth: 1, borderBottomWidth: 1, paddingVertical: 10, marginBottom: 14 },
  stat: { flex: 1, alignItems: "center", paddingHorizontal: 8 },
  statLabel: { fontSize: 8, fontFamily: "Inter_500Medium", letterSpacing: 1, textTransform: "uppercase", marginBottom: 3 },
  ...
  cardAction: { width: 56, height: 56, borderRadius: 28, borderWidth: 1, alignItems: "center", justifyContent: "center" },
  emptyDeck: { width: "100%", borderRadius: 16, borderWidth: 1, padding: 32, alignItems: "center", gap: 12 },
  emptyTitle: { fontSize: 20, fontFamily: "Inter_600SemiBold" },
  emptySubtitle: { fontSize: 13, fontFamily: "Inter_300Light", textAlign: "center" },
  resetBtn: { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 16, paddingVertical: 10, borderRadius: 10, borderWidth: 1, marginTop: 8 },
  resetText: { fontSize: 13, fontFamily: "Inter_500Medium" },
  remaining: { textAlign: "center", fontSize: 11, fontFamily: "Inter_300Light", paddingBottom: 100 },
  dataSourceBadge: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 20, paddingBottom: 4 },
