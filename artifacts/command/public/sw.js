const CACHE_VERSION = 'command-v1';
...
self.addEventListener('push', (event) => {
  ...
  const title = payload.title || 'Ecosystem Command Portal';
  const options = {
    body: payload.body || '',
    icon: payload.icon || '/favicon.svg',
    badge: payload.badge || '/favicon.svg',
    tag: payload.tag || 'command-alert',
    data: payload.data || {},
...
self.addEventListener('notificationclick', (event) => {
  ...
  const actionUrl = event.notification.data?.actionUrl || '/command/';
  event.waitUntil(
