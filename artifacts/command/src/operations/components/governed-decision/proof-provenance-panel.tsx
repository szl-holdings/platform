...
const REVIEW_STATE_CFG = {
  ...
  retracted: { color: "#c45a4a", bg: "rgba(196,90,74,0.1)", icon: AlertTriangle, label: "Retracted" },
};
    ...
    return (
      <div className="rounded-xl p-4" style={{ background: BG.surface, border: `1px solid ${BORDER.muted}` }}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "#d4a054" }}>Proof Chain</span>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <span className="text-[9px] px-1.5 py-0.5 rounded font-bold" style={{ color: reviewCfg.color, background: reviewCfg.bg }}>
            <ReviewIcon className="w-2.5 h-2.5 inline mr-1" />{reviewCfg.label}
          </span>
          <span className="text-[10px] font-mono" style={{ color: TEXT.tertiary }}>{SOURCE_CLASS_LABELS[entry.sourceClass]}</span>
        ...
        {entries.map((entry) => {
          ...
          return (
            <div key={entry.id}>
              ...
              >
                <div className="w-6 h-6 rounded flex items-center justify-center shrink-0" style={{ background: reviewCfg.bg, border: `1px solid ${reviewCfg.color}30` }}>
                  <ReviewIcon className="w-3 h-3" style={{ color: reviewCfg.color }} />
                </div>
                <div className="flex-1 min-w-0 text-left">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-semibold" style={{ color: TEXT.primary }}>{entry.contentType}</span>
                    <span className="text-[9px] font-mono" style={{ color: TEXT.tertiary }}>#{entry.id}</span>
                  </div>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[9px]" style={{ color: TEXT.tertiary }}>{SOURCE_CLASS_LABELS[entry.sourceClass]}</span>
                    {entry.modelId && (
                      <>
                        <span style={{ color: TEXT.muted }}>·</span>
                        <span className="text-[9px] font-mono" style={{ color: TEXT.tertiary }}>{entry.modelId}</span>
                      </>
                    )}
                  </div>
                </div>
                <div className="w-24 shrink-0"><ConfidenceBar score={entry.confidenceScore} /></div>
                {isExpanded ? <ChevronDown className="w-3 h-3 shrink-0" style={{ color: TEXT.tertiary }} /> : <ChevronRight className="w-3 h-3 shrink-0" style={{ color: TEXT.tertiary }} />}
              </button>
                ...
                <div className="px-5 pb-4 ml-9">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                    {[
                      ...
                      { label: "Source", value: SOURCE_CLASS_LABELS[entry.sourceClass], color: TEXT.secondary },
                      { label: "Generated", value: entry.generatedAt, color: TEXT.secondary },
                    ].map(f => (
                      <div key={f.label}>
                        <div className="text-[8px] font-mono uppercase tracking-wider mb-0.5" style={{ color: TEXT.muted }}>{f.label}</div>
                        <div className="text-[10px] font-semibold" style={{ color: f.color }}>{f.value}</div>
                      </div>
                    ))}
                  </div>
                  ...
                  {entry.modelProvider && (
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-[10px]" style={{ color: TEXT.tertiary }}>Provider: {entry.modelProvider} · Model: {entry.modelId}</span>
                    </div>
                  )}

                  {entry.inputSources.length > 0 && (
                    <div className="mb-2">
                      <div className="text-[8px] font-mono uppercase tracking-wider mb-1" style={{ color: TEXT.muted }}>Input Sources</div>
                      <div className="flex flex-wrap gap-1">
                        {entry.inputSources.map((src, i) => (
                          <span key={i} className="flex items-center gap-1 text-[9px] px-1.5 py-0.5 rounded" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${BORDER.muted}`, color: TEXT.secondary }}>
                            <Link2 className="w-2.5 h-2.5" />{src.label}
                          </span>
                        ))}
                      </div>
                    </div>
                  ...
                  {entry.reviewedBy && (
                    <div className="flex items-center gap-2 mt-2 pt-2" style={{ borderTop: `1px solid ${BORDER.subtle}` }}>
                      <Eye className="w-3 h-3" style={{ color: TEXT.tertiary }} />
                      <span className="text-[10px]" style={{ color: TEXT.tertiary }}>Reviewed by {entry.reviewedBy} on {entry.reviewedAt}</span>
                      {entry.reviewNote && <span className="text-[10px] italic" style={{ color: TEXT.tertiary }}>— "{entry.reviewNote}"</span>}
                    </div>
                  )}
                  ...
                  {entry.parentProofId && (
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-[10px]" style={{ color: TEXT.tertiary }}>Derived from proof #{entry.parentProofId}</span>
                    </div>
                  )}
                </div>
              )}
