import { type AppObservabilityState, LENSES, type LensId } from '@szl-holdings/observability';
...
function statusBg(status: string): string {
  switch (status) {
    case 'healthy':
      return 'bg-[#6b8f71]/10 border-[#6b8f71]/20';
    case 'degraded':
      return 'bg-[#d4a054]/10 border-[#d4a054]/20';
    case 'critical':
      return 'bg-[#c45a4a]/10 border-[#c45a4a]/20';
    default:
...
function AppLensCard({ state }: { state: AppObservabilityState }) {
  ...
  return (
    <div
      className={`rounded-xl border p-4 ${statusBg(state.overallStatus)} transition-all hover:scale-[1.01]`}
    >
      <div className="flex items-center justify-between mb-1">
        <h4 className="text-sm font-semibold text-white truncate pr-2">{name}</h4>
        <span className={`text-lg font-bold ${statusColor(state.overallStatus)}`}>
          {postureScore}
        </span>
      </div>
      {state.topSignal && (
        ...
        {lenses.map((lens) => {
          ...
          return (
            <div key={lensId} className="flex flex-col items-center py-1">
              <span className={`text-xs font-mono font-bold ${statusColor(lens.status)}`}>
                {lens.score}
              </span>
              <span className="text-[8px] text-slate-500">
                {LENS_ICONS[lensId] || lensId.slice(0, 3)}
              </span>
            </div>
          );
        })}
...
function LensAggregateRow({ lensId, apps }: { lensId: LensId; apps: AppObservabilityState[] }) {
  ...
  const scores = apps.map((a) => {
    ...
    return (
      lenses.find((l) => l.lensId === lensId || (l as { pillarId?: string }).pillarId === lensId)
        ?.score ?? 0
    );
  });
    ...
    <div className="flex items-center gap-4 py-3 px-4 rounded-lg hover:bg-white/5 transition-colors">
      ...
      <div className="flex items-center gap-1">
        {scores.map((s, i) => (
          <div
            key={i}
            className={`w-6 h-6 rounded text-[9px] font-bold flex items-center justify-center ${s >= 80 ? 'bg-[#6b8f71]/10 text-[#6b8f71]' : s >= 50 ? 'bg-[#d4a054]/10 text-[#d4a054]' : 'bg-[#c45a4a]/10 text-[#c45a4a]'}`}
          >
            {s}
          </div>
        ))}
      </div>
      <span className={`text-lg font-bold min-w-[40px] text-right ${statusColor(status)}`}>
        {avg}
      </span>
    </div>
  );
  ...
  const fetchFromApi = useCallback(async () => {
    try {
      ...
      const states = results
        .filter(
          (r): r is PromiseFulfilledResult<AppObservabilityState | null> =>
            r.status === 'fulfilled',
        )
        .map((r) => r.value)
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            The 6 Lenses of Business Observability across {appStates.length} portfolio applications
          </p>
        </div>
        <div
          className={`flex items-center gap-2 px-4 py-2 rounded-full border text-sm font-medium ${statusBg(portfolioStatus)} ${statusColor(portfolioStatus)}`}
        >
          <span
            className={`w-2 h-2 rounded-full ${portfolioStatus === 'healthy' ? 'bg-[#6b8f71]' : portfolioStatus === 'degraded' ? 'bg-[#d4a054]' : 'bg-[#c45a4a]'}`}
          />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="text-center">
            ...
            <div className={`text-sm font-semibold capitalize ${statusColor(portfolioStatus)}`}>
              {portfolioStatus}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3 w-full">
            ...
            <div className="rounded-lg bg-white/[0.03] border border-white/5 p-3 text-center">
              <div className="text-lg font-bold text-white">
                {appStates.reduce((s, a) => s + a.metrics.length, 0)}
              </div>
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.02]">
        <div className="p-4 border-b border-white/10">
          ...
          <p className="text-xs text-slate-400">
            Average score per lens across all {appStates.length} applications (each box = one app)
          </p>
        </div>
        ...
        <div className="divide-y divide-white/5">
          {recentEvents.map((e) => {
            ...
            return (
              <div
                key={e.id}
                className="flex items-start gap-3 py-2 px-4 hover:bg-white/5 transition-colors"
              >
                <span
                  className={`inline-flex items-center justify-center w-5 h-5 rounded-full text-xs flex-shrink-0 mt-0.5 ${e.severity === 'critical' ? 'text-[#c45a4a] bg-[#c45a4a]/10' : e.severity === 'warning' ? 'text-[#d4a054] bg-[#d4a054]/10' : 'text-[#4a90b8] bg-[#4a90b8]/10'}`}
                >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-slate-500 flex gap-2 mt-0.5">
                    <span className="text-cyan-400">
                      {ALL_CONFIGS.find((c) => c.appSlug === e.appSlug)?.appName || e.appSlug}
                    </span>
                    <span>·</span>
                    <span className="capitalize text-white/30">
                      {LENS_ICONS[lensId] || '◆'} {lensId} lens
                    </span>
                  </div>
                </div>
              </div>
