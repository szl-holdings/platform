      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Power BI embedded report — SLA performance, escalation trends, PRISM health scores, and
            workflow analytics.
          </p>
        </div>
        <button
        ...
        <>
          <div className="rounded-xl border border-[#d4a054]/20 bg-[#d4a054]/5 p-4 flex items-start gap-3">
            ...
            <div className="text-xs text-slate-400 leading-relaxed">
              <strong className="text-white">Sample data shown below.</strong> Click "Load Live
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {OPS_METRICS.map((m) => {
              ...
              return (
                <div
                  key={m.label}
                  className="bg-slate-900 border border-slate-700/60 rounded-xl p-4"
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span
                      className={cn(
                        ...
                        m.up ? 'text-[#6b8f71]' : 'text-[#c45a4a]/80',
                      )}
                    >
                      {m.delta}
                    </span>
                  ...
                  return (
                    <div key={d.week} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-5 bg-slate-800 rounded-sm overflow-hidden flex">
                        {d.critical > 0 && (
                          <div
                            className="bg-[#c45a4a]/70 h-full"
                            style={{ width: `${(d.critical / (maxEscalation || 1)) * 100}%` }}
                          />
                        )}
                        {d.high > 0 && (
                          <div
                            className="bg-[#d4a054]/60 h-full"
                            style={{ width: `${(d.high / (maxEscalation || 1)) * 100}%` }}
                          />
                        )}
                      </div>
                      ...
                      <span className="text-[10px] text-[#6b8f71] w-16 text-right">
                        {d.resolved} resolved
                      </span>
                    </div>
                  );
                  ...
                  <div key={l.name}>
                    ...
                    <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${l.score}%`, background: l.color }}
                      />
                    </div>
                  </div>
                ))}
