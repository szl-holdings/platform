...
const POLICIES: Policy[] = [
  {
    id: 'pol-001',
    name: 'Human-in-the-Loop Enforcement',
    category: 'Action Control',
    status: 'active',
    scope: 'All agents',
    appliedAt: '14d ago',
    description:
      'All consequential actions require explicit human approval before execution. No autonomous action in production.',
    violations: 0,
    checks: 4280,
  },
  {
    id: 'pol-002',
    name: 'PII Non-Transmission Policy',
    category: 'Data Governance',
    status: 'active',
    scope: 'All agents',
    appliedAt: '14d ago',
    description:
      'No personally identifiable information may be included in agent payloads transmitted to external systems.',
    violations: 0,
    checks: 12480,
  },
  {
    ...
    status: 'active',
    scope: 'All LLM agents',
    appliedAt: '14d ago',
    description:
      'Daily token budgets enforced per agent. Agents exceeding 80% of budget trigger an alert.',
    violations: 1,
    checks: 384,
  ...
  {
    id: 'pol-004',
    name: 'Approval Delegation Limits',
    category: 'Action Control',
    status: 'active',
    scope: 'Action Router',
    appliedAt: '10d ago',
    description:
      'Approval delegation permitted for actions under $500 impact. Actions above threshold require senior approval.',
    violations: 0,
    checks: 124,
  },
  {
    ...
    appliedAt: '14d ago',
    description:
      'Every generated finding must include a source signal chain. Findings without traceable evidence are rejected.',
    violations: 0,
    checks: 2184,
  },
  {
    ...
    appliedAt: '14d ago',
    description:
      'Every agent run must produce a complete, immutable trace entry. Missing audit records trigger immediate alert.',
    violations: 0,
    checks: 18240,
  },
];
...
const RECENT_APPROVALS: ApprovalEntry[] = [
  ...
  {
    id: 'apr-002',
    action: 'Escalate: Blocker unresolved 72h',
    agent: 'Action Router',
    requester: 'System',
    status: 'approved',
    approver: 'M. Patel',
  ...
  {
    id: 'apr-003',
    action: 'Route: High-risk signal to CISO',
  ...
  {
    id: 'apr-004',
    action: 'Archive: Stale workflow (45d)',
    ...
    at: '1d ago',
    impact: 'Medium',
    reason: 'Workflow still referenced in Q2 planning',
  },
  {
    id: 'apr-005',
    action: 'Notify: Token budget alert',
    agent: 'Action Router',
    requester: 'System',
    status: 'approved',
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden" style={{ background: BG.page }}>
      ...
      <div className="flex-1 overflow-y-auto">
        <div
          className="grid grid-cols-6 gap-px"
          style={{ borderBottom: `1px solid ${BORDER.subtle}`, background: BORDER.subtle }}
        >
                ...
                >
                  <div className="flex items-start justify-between px-4 py-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] mb-2" style={{ color: TEXT.secondary }}>
                        {policy.description}
                      </p>
                      <div
                      ...
                      >
                        <span>
                          Category: <span style={{ color: TEXT.tertiary }}>{policy.category}</span>
                        </span>
                        <span>
                          Scope: <span style={{ color: TEXT.tertiary }}>{policy.scope}</span>
                        </span>
                        <span>
                          Since: <span style={{ color: TEXT.tertiary }}>{policy.appliedAt}</span>
                        </span>
                      </div>
                    </div>
                    <div className="shrink-0 text-right ml-4">
                      <p
                        className="text-[14px] font-bold font-mono mb-0.5"
                        style={{ color: policy.violations === 0 ? '#22c55e' : '#ef4444' }}
                      >
                        {policy.violations}
                      </p>
                      ...
                      <p className="text-[11px] font-mono mt-1" style={{ color: TEXT.tertiary }}>
                        {policy.checks.toLocaleString()} checks
                      </p>
                    </div>
                  </div>
                  {policy.alert && (
          ...
          <div className="p-4 space-y-2">
            {RECENT_APPROVALS.map((approval) => {
              ...
              return (
                <div
                  key={approval.id}
                  className="rounded-md px-4 py-3 flex items-center gap-4"
                  style={{ background: BG.surface, border: `1px solid ${BORDER.subtle}` }}
                >
                  <div className="flex-1 min-w-0">
                    <p
        ...
        {activeTab === 'audit' && (
          <div className="p-4">
            ...
            >
              ...
              <p className="text-[12px] font-medium mb-1" style={{ color: TEXT.secondary }}>
                Proof Chain Audit Log
              </p>
              <p className="text-[11px] mb-4" style={{ color: TEXT.muted }}>
                Full immutable audit trail is available in the Proof Chain viewer. Every agent
                action, approval, and inference is recorded and retrievable.
              </p>
              <button
                className="inline-flex items-center gap-2 rounded-lg px-4 py-2 text-[11px] font-medium"
