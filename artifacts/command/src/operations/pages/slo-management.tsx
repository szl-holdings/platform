...
const SEED: SLO[] = [
  {
    ...
    errorBudgetConsumed: 34.6,
    burnRate1h: 14.2,
    burnRate6h: 8.7,
    burnRate24h: 4.1,
    burnRate72h: 2.3,
    status: 'at_risk',
    exhaustionForecast: 3.2,
    alertFiring: true,
    alertType: 'fast-burn',
    history: Array.from({ length: 24 }, (_, i) => ({
      t: `${i}h`,
      budget: Math.max(0, 43.2 - i * 1.45 + (Math.random() - 0.5) * 1.2),
      br: 4 + Math.random() * 10 + (i > 18 ? 8 : 0),
    })),
  },
  {
    id: 's2',
    name: 'Alloy Workflow Completion Rate',
    service: 'szl-alloy-workflow',
    metric: 'completion_rate',
    target: 99.5,
    current: 99.62,
    window: '30d',
    errorBudgetTotal: 216,
    errorBudgetConsumed: 89.4,
    burnRate1h: 1.8,
    burnRate6h: 1.4,
    burnRate24h: 1.2,
    burnRate72h: 1.1,
    status: 'healthy',
    exhaustionForecast: null,
    alertFiring: false,
    alertType: null,
    history: Array.from({ length: 24 }, (_, i) => ({
      t: `${i}h`,
      budget: Math.max(0, 216 - i * 3.7 + (Math.random() - 0.5) * 4),
      br: 1 + Math.random() * 0.8,
    })),
  },
  {
    id: 's3',
    ...
    current: 98.71,
    window: '30d',
    errorBudgetTotal: 432,
    errorBudgetConsumed: 428.1,
    burnRate1h: 3.2,
    burnRate6h: 2.8,
    burnRate24h: 1.9,
    burnRate72h: 1.5,
    status: 'breached',
    exhaustionForecast: 0,
    alertFiring: true,
    alertType: 'slow-burn',
    history: Array.from({ length: 24 }, (_, i) => ({
      t: `${i}h`,
      budget: Math.max(0, 432 - i * 17.8 + (Math.random() - 0.5) * 5),
      br: 2 + Math.random() * 2,
    })),
  },
  {
    ...
    current: 99.88,
    window: '7d',
    errorBudgetTotal: 50.4,
    errorBudgetConsumed: 6.2,
    burnRate1h: 0.4,
    burnRate6h: 0.6,
    burnRate24h: 0.5,
    burnRate72h: 0.5,
    ...
    history: Array.from({ length: 24 }, (_, i) => ({
      t: `${i}h`,
      budget: Math.max(0, 50.4 - i * 0.26 + (Math.random() - 0.5) * 0.5),
      br: 0.4 + Math.random() * 0.3,
    })),
  },
  {
    id: 's5',
    name: 'AI Inference Latency P95',
    service: 'szl-ai-engine',
    metric: 'latency_p95',
    target: 99.95,
    current: 99.91,
    window: '30d',
    errorBudgetTotal: 21.6,
    errorBudgetConsumed: 18.9,
    burnRate1h: 2.1,
    burnRate6h: 1.8,
    burnRate24h: 1.4,
    burnRate72h: 1.3,
    status: 'at_risk',
    exhaustionForecast: 8.4,
    alertFiring: false,
    alertType: null,
    history: Array.from({ length: 24 }, (_, i) => ({
      t: `${i}h`,
      budget: Math.max(0, 21.6 - i * 0.79 + (Math.random() - 0.5) * 0.4),
      br: 1.3 + Math.random() * 0.8,
    })),
  },
  {
    id: 's6',
    name: 'Signal-to-Action P95',
    service: 'szl-signal-ingestion',
    metric: 'signal_to_action',
    target: 99.0,
    current: 99.4,
    window: '30d',
    errorBudgetTotal: 432,
    errorBudgetConsumed: 172.8,
    burnRate1h: 0.7,
    burnRate6h: 0.9,
    burnRate24h: 0.8,
    burnRate72h: 0.8,
    ...
    history: Array.from({ length: 24 }, (_, i) => ({
      t: `${i}h`,
      budget: Math.max(0, 432 - i * 7.2 + (Math.random() - 0.5) * 5),
      br: 0.8 + Math.random() * 0.3,
    })),
  },
];
    ...
    <div>
      <div className="flex items-center justify-between mb-0.5">
        ...
        <div className="flex items-center gap-1">
          <span
            ...
            style={{ color: firing ? '#ef4444' : DS.text.secondary }}
          >
            {rate.toFixed(1)}x
          </span>
      ...
      >
        <div
          ...
          style={{ width: `${pct}%`, background: firing ? '#ef4444' : color }}
        />
      </div>
...
function BudgetGauge({ pct, color }: { pct: number; color: string }) {
  ...
  return (
    <svg viewBox="0 0 72 72" className="w-16 h-16">
      ...
      <text x={cx} y={cx + 10} textAnchor="middle" fontSize="5" fill="rgba(255,255,255,0.3)">
        left
      </text>
    </svg>
  );
...
function SLODetail({ slo }: { slo: SLO }) {
  ...
  return (
    <div
      ...
      style={{ borderColor: `${sc.color}25`, background: sc.bg }}
    >
      <div className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              {slo.alertFiring && (
                <span
                  className="text-[8px] px-1.5 py-0.5 rounded font-mono animate-pulse"
            ...
            <div className="text-[9px] font-mono mb-2" style={{ color: DS.text.muted }}>
              {slo.service} · {slo.window} window
            </div>
            <div className="flex items-center gap-4 text-[10px]">
              <div>
                <span style={{ color: DS.text.muted }}>Target </span>
                <span className="font-mono font-bold" style={{ color: DS.text.secondary }}>
                  {slo.target}%
                </span>
              </div>
              <div>
                ...
                <span
                  ...
                  style={{ color: slo.current >= slo.target ? '#10b981' : '#ef4444' }}
                >
                  {slo.current}%
                </span>
              </div>
              {slo.exhaustionForecast !== null && slo.exhaustionForecast > 0 && (
                <div className="text-[9px]" style={{ color: '#ef4444' }}>
                  Budget exhausts in{' '}
                  <span className="font-mono font-bold">{slo.exhaustionForecast.toFixed(1)}d</span>
                </div>
              )}
              {slo.status === 'breached' && (
                <div className="text-[9px]" style={{ color: '#ef4444' }}>
        ...
        <div className="mt-3">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[9px]" style={{ color: DS.text.muted }}>
              Error Budget: {slo.errorBudgetConsumed.toFixed(1)}m consumed of{' '}
              {slo.errorBudgetTotal.toFixed(1)}m
            </span>
...
function LatencyCell({
  ...
  return (
    <div
      ...
      style={{ background: DS.surface, border: `1px solid ${color}22` }}
    >
      <div className="text-[9px] uppercase tracking-wider mb-1.5" style={{ color: DS.text.muted }}>
        {label}
      </div>
...
function ActiveAlertBanner({
  ...
  alerts: Array<{ rule_name: string; metric_name: string; message: string; created_at: string }>;
  tier: 'P0' | 'P1';
}) {
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[11px] font-bold uppercase tracking-widest" style={{ color }}>
          {tier} Active — {alerts.length} alert{alerts.length > 1 ? 's' : ''} firing
        </span>
        <span
          className="text-[8px] px-1.5 py-0.5 rounded font-mono animate-pulse"
      ...
      <div className="flex flex-col gap-1.5">
        {alerts.map((a, i) => (
          <div key={i} className="flex items-start gap-2 text-[10px]">
            ...
            <div>
              <span className="font-semibold" style={{ color: DS.text.primary }}>
                {a.rule_name.replace(/^\[P\d\] /, '')}
              </span>
...
function ApprovalLatencyRow({
  ...
  return (
    ...
    >
      <div
        className="w-20 text-[10px] font-semibold capitalize"
      ...
      <div className="flex-1 h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
        <div
          ...
          style={{ width: `${Math.min(100, (maxMinutes / slaTarget) * 100)}%`, background: color }}
        />
      </div>
...
export default function SLOManagement() {
  ...
  const counts = {
    healthy: SEED.filter((s) => s.status === 'healthy').length,
    at_risk: SEED.filter((s) => s.status === 'at_risk').length,
    breached: SEED.filter((s) => s.status === 'breached').length,
  };
  ...
  const budgetColor = budget
    ? budget.remainingPct > 50
      ? '#10b981'
      : budget.remainingPct > 25
        ? GOLD
        : '#ef4444'
    : '#10b981';

  const availabilityColor =
    slo?.availability?.current != null
      ? slo.availability.current >= slo.availability.target
        ? '#10b981'
        : '#ef4444'
      : DS.text.muted;
    ...
    <div className="p-4 md:p-6 max-w-7xl space-y-6" style={{ background: '#080c14' }}>
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            {(alertFiring > 0 || totalFiring > 0) && (
              ...
              >
                {Math.max(alertFiring, totalFiring)} ALERT
                {Math.max(alertFiring, totalFiring) > 1 ? 'S' : ''} FIRING
              </span>
            )}
          </div>
          <p className="text-[11px]" style={{ color: DS.text.muted }}>
            Platform SLOs with live error budget tracking, multi-window burn rate alerting, and
            exhaustion forecasting.
            {slo && (
              <span className="ml-1 opacity-60">
                Updated {new Date(slo.generatedAt).toLocaleTimeString()} · {slo.dataSource}
              </span>
            )}
          </p>
        </div>
      ...
      {slo && (
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {!isLoading && (
              <span
                ...
                style={{ color: slo.dataSource === 'live' ? '#10b981' : DS.text.muted }}
              >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div
              ...
              style={{
                ...
                border: `1px solid ${availabilityColor}22`,
              }}
            >
              <div
            ...
            <div
              ...
              style={{
                background: 'rgba(255,255,255,0.02)',
                border: `1px solid ${slo.errorRate.sloMet ? '#10b981' : '#ef4444'}22`,
              }}
            >
              <div
                className="text-[9px] uppercase tracking-wider mb-1"
              ...
              <div
                ...
                style={{ color: slo.errorRate.sloMet ? '#10b981' : '#ef4444' }}
              >
            ...
            <div
              className="rounded-lg p-3"
              style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${budgetColor}22` }}
            >
              <div
                className="text-[9px] uppercase tracking-wider mb-1"
                style={{ color: DS.text.muted }}
              ...
              <div className="text-[8px] mt-1" style={{ color: DS.text.muted }}>
                {budget?.remainingMinutes}m remaining · {budget?.policy}
              </div>
            </div>
            ...
            <div
              ...
              style={{
                background: 'rgba(255,255,255,0.02)',
                border: `1px solid ${slo.workflowEngine.completionRate24h >= 99 ? '#10b981' : GOLD}22`,
              }}
            >
              <div
                className="text-[9px] uppercase tracking-wider mb-1"
              ...
              <div
                className="text-[18px] font-bold font-mono"
                style={{ color: slo.workflowEngine.completionRate24h >= 99 ? '#10b981' : GOLD }}
              >
                {slo.workflowEngine.completionRate24h.toFixed(1)}%
              </div>
              <div className="text-[8px] mt-1" style={{ color: DS.text.muted }}>
                queue: {slo.workflowEngine.queueDepth} · target:{' '}
                {slo.workflowEngine.completionRateTarget}%
              </div>
            </div>
          </div>
          ...
          {latency && (
            <div>
              <div
                className="text-[9px] uppercase tracking-wider mb-2 flex items-center gap-1.5"
                style={{ color: DS.text.muted }}
              >
              ...
              <div className="grid grid-cols-3 gap-3">
                <LatencyCell
                  label="P50 Latency"
                  current={latency.p50Current}
                  target={latency.p50Target}
                />
                <LatencyCell
                  label="P95 Latency"
                  current={latency.p95Current}
                  target={latency.p95Target}
                />
                <LatencyCell
                  label="P99 Latency"
                  current={latency.p99Current}
                  target={latency.p99Target}
                />
              </div>
            </div>
          )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          >
            ...
            <div className="text-[22px] font-bold font-mono" style={{ color: k.color }}>
              {k.value}
            </div>
          </div>
        ))}
      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-[10px]">
          {[
            {
              window: 'Fast Burn (1h + 5m)',
              threshold: '14.4×',
              rationale: '2% budget in 1h',
              color: '#ef4444',
            },
            {
              window: 'Fast Burn (6h + 30m)',
              threshold: '6×',
              rationale: '5% budget in 6h',
              color: '#f97316',
            },
            {
              window: 'Slow Burn (24h + 2h)',
              threshold: '3×',
              rationale: '10% budget in 24h',
              color: GOLD,
            },
            {
              window: 'Slow Burn (3d + 6h)',
              threshold: '1×',
              rationale: '10% budget in 3d',
              color: '#3b82f6',
            },
          ].map((r) => (
            <div
              key={r.window}
              className="rounded-lg p-3"
              style={{ background: `${r.color}06`, border: `1px solid ${r.color}18` }}
            >
              <div className="font-semibold mb-1" style={{ color: r.color }}>
                {r.window}
              </div>
              <div
        ...
        {(['all', 'healthy', 'at_risk', 'breached'] as const).map((f) => (
          <button
            ...
            style={{
              background:
                filter === f
                  ? `${f === 'all' ? GOLD : f === 'healthy' ? '#10b981' : f === 'at_risk' ? GOLD : '#ef4444'}15`
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {SEED.filter((s) => s.exhaustionForecast !== null).map((s) => {
            ...
            return (
              ...
              >
                <div className="text-[10px] font-semibold mb-1" style={{ color: DS.text.primary }}>
                  {s.name}
                </div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span
                    ...
                    style={{
                      color:
                        days === 0 ? '#ef4444' : days < 3 ? '#ef4444' : days < 7 ? GOLD : '#10b981',
                    }}
                  >
      ...
      >
        <div
          className="text-[9px] uppercase tracking-widest mb-2 flex items-center gap-1.5"
          style={{ color: DS.text.muted }}
        >
        ...
        <div className="text-[10px] leading-relaxed" style={{ color: DS.text.secondary }}>
          ...
          end-to-end, propagated across service boundaries from API gateway through Alloy workflow
          steps and audit log entries. Alloy workflow spans are instrumented with{' '}
          <code
            className="font-mono text-[9px] px-1 py-0.5 rounded"
            style={{ background: 'rgba(255,255,255,0.06)' }}
          >
