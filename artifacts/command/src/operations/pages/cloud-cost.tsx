...
import { AlertTriangle, Cloud, DollarSign, TrendingDown, TrendingUp, Zap } from 'lucide-react';
import {
  Bar,
  BarChart,
  CartesianGrid,
...
const monthlyTrend = [
  { month: 'Oct', cost: 98400 },
  { month: 'Nov', cost: 102100 },
  { month: 'Dec', cost: 108300 },
  { month: 'Jan', cost: 112800 },
  { month: 'Feb', cost: 109200 },
  { month: 'Mar', cost: 117950 },
];

const recommendations = [
  {
    action: 'Right-size 23 over-provisioned EC2 instances',
    saving: '$12,800/mo',
    effort: 'Low',
    confidence: 94,
  },
  {
    action: 'Switch 8 RDS instances to Aurora Serverless v2',
    saving: '$6,200/mo',
    effort: 'Medium',
    confidence: 87,
  },
  {
    action: 'Enable S3 Intelligent-Tiering on 3 buckets',
    saving: '$2,100/mo',
    effort: 'Low',
    confidence: 99,
  },
  {
    action: 'Consolidate 4 underutilized GKE node pools',
    saving: '$8,400/mo',
    effort: 'Medium',
    confidence: 82,
  },
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Cloud spend tracking, waste identification, and optimization recommendations
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            value: `$${(totalWaste / 1000).toFixed(0)}k`,
            color: 'text-[#c45a4a]',
          },
          {
            label: 'Savings Opportunity',
            value: `$${(recommendations.reduce((a, r) => a + parseFloat(r.saving.replace(/[$,/mo]/g, '')), 0) / 1000).toFixed(1)}k/mo`,
            color: 'text-[#6b8f71]',
          },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <ResponsiveContainer width="100%" height={180}>
                <BarChart data={monthlyTrend} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(v: number | string) => [`$${Number(v).toLocaleString()}`, 'Cost']}
                  />
        ...
        <div className="space-y-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">Spend by Service</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={180}>
                <PieChart>
              ...
              {costByService.map((s, i) => (
                <div key={s.service} className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-[10px] ${s.trend.startsWith('+') ? 'text-[#c45a4a]' : 'text-[#6b8f71]'}`}
                    >
                      {s.trend}
                    </span>
                  </div>
