  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-xl font-display font-bold flex items-center gap-2"><Play className="w-5 h-5 text-primary" />Workflow Runs</h1>
          <p className="text-xs text-muted-foreground mt-1">Alloy workflow execution history and state</p>
        </div>
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <span>{total} total runs</span>
          <button onClick={() => qc.invalidateQueries({ queryKey: ["admin-runs"] })} className="p-1.5 rounded-lg hover:bg-white/5 transition-colors">
        ...
        {Object.entries(filterLabels).map(([key, label]) => (
          <button
            ...
            className={cn(
              ...
              stateFilter === key ? "bg-primary/15 text-primary border border-primary/30" : "text-muted-foreground hover:text-foreground hover:bg-white/5 border border-transparent"
            )}
          >
            {label}
          </button>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left px-4 py-3 text-muted-foreground font-medium hidden sm:table-cell">Workflow</th>
                <th className="text-left px-4 py-3 text-muted-foreground font-medium hidden md:table-cell">Queued</th>
                <th className="text-left px-4 py-3 text-muted-foreground font-medium hidden md:table-cell">Duration</th>
            ...
            <tbody>
              {runs.map((run) => {
                ...
                return (
                  <tr key={run.id} className="border-b border-border/50 hover:bg-white/[0.02] transition-colors cursor-pointer" onClick={() => setSelected(run)}>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setSelected(null)}>
          <div className="bg-card border border-border rounded-xl p-6 max-w-lg w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-xs text-muted-foreground mb-4">Queued {timeAgo(selected.queuedAt)}</p>
            <dl className="grid grid-cols-2 gap-3 text-xs mb-4">
              <div>
                <dt className="text-muted-foreground mb-0.5">State</dt>
                <dd>
                  <span className={cn("inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium border", stateStyles[selected.state]?.color)}>
              ...
              <div>
                ...
                <dd className="font-mono">{selected.startedAt ? timeAgo(selected.startedAt) : "—"}</dd>
              </div>
              <div>
                <dt className="text-muted-foreground mb-0.5">Retries</dt>
                <dd>{selected.retryCount} / {selected.maxRetries ?? 3}</dd>
              </div>
            </dl>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={() => setSelected(null)} className="px-3 py-1.5 rounded-lg text-xs text-muted-foreground hover:text-foreground hover:bg-white/5 transition-colors">Close</button>
            </div>
          </div>
        </div>
      )}
