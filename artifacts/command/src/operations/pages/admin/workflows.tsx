import { Workflow, Play, Pause, Clock, CheckCircle, AlertTriangle, Plus, Zap, GitBranch } from "lucide-react";

const workflows = [
  { id: "WF-001", name: "Deploy Pipeline", trigger: "Git Push", status: "active" as const, lastRun: "5m ago", nextRun: "On trigger", runs24h: 12, successRate: 98, steps: 6, category: "Deployment" },
  { id: "WF-002", name: "Data Sync — All Apps", trigger: "Schedule (1h)", status: "active" as const, lastRun: "22m ago", nextRun: "38m", runs24h: 24, successRate: 100, steps: 4, category: "Data" },
  { id: "WF-003", name: "Health Check Sweep", trigger: "Schedule (5m)", status: "active" as const, lastRun: "2m ago", nextRun: "3m", runs24h: 288, successRate: 99.6, steps: 3, category: "Monitoring" },
  { id: "WF-004", name: "User Onboarding", trigger: "API Webhook", status: "active" as const, lastRun: "1h ago", nextRun: "On trigger", runs24h: 3, successRate: 100, steps: 8, category: "Users" },
  { id: "WF-005", name: "Log Rotation & Archive", trigger: "Schedule (daily)", status: "paused" as const, lastRun: "1d ago", nextRun: "Paused", runs24h: 0, successRate: 95, steps: 5, category: "Maintenance" },
  { id: "WF-006", name: "Compliance Report Gen", trigger: "Schedule (weekly)", status: "error" as const, lastRun: "2d ago", nextRun: "Failed", runs24h: 0, successRate: 80, steps: 7, category: "Compliance" },
];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-display font-bold flex items-center gap-2"><Workflow className="w-5 h-5 text-primary" />Workflow Automation</h1>
          <p className="text-xs text-muted-foreground mt-1">Automated workflows, change management, and system orchestration</p>
        </div>
      ...
      <div className="rounded-xl border border-border bg-card">
        <div className="p-4 border-b border-border"><h2 className="font-display font-semibold text-sm">Workflow Registry</h2></div>
            ...
            <div key={wf.id} className="p-4 hover:bg-muted/30 transition-colors">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${statusStyles[wf.status]}`}>{wf.status}</span>
                  <span className="text-[10px] bg-muted text-muted-foreground px-1.5 py-0.5 rounded">{wf.category}</span>
                </div>
            ...
            <div key={ch.id} className="px-4 py-3 hover:bg-muted/30 transition-colors">
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-1.5 py-0.5 rounded text-[10px] font-medium bg-muted text-muted-foreground uppercase">{ch.type}</span>
                  <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold uppercase ${impactColors[ch.impact as keyof typeof impactColors]}`}>{ch.impact} impact</span>
                </div>
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <Clock className="w-3 h-3" />{ch.timestamp} · {ch.status}
                </div>
              </div>
