...
function StatusBadge({ status }: { status: ExportRecord['status'] }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${cfg.cls}`}
    >
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Audit trail of all data exports — who triggered them, what data, and when. Downloads
          ...
          {isLoading ? (
            ...
            <div className="py-10 text-center text-sm text-muted-foreground">
              ...
              <p className="text-xs mt-1">
                Exports from audit logs and data tables will appear here
              </p>
            </div>
          ) : (
      ...
      <div className="rounded-xl bg-muted/30 border border-border p-4 text-xs text-muted-foreground space-y-1">
        ...
        <p>
          • Download tokens expire after 24 hours — files are generated on-demand and not stored
          server-side
        </p>
        <p>
          • GDPR data portability requests should use the bulk export endpoints with appropriate
          date filters
        </p>
        <p>
          • Exports over 10,000 rows are limited to the first 10,000 records — contact support for
          full dumps
        </p>
        <p>
          • Only users with admin or compliance role can access export history and trigger audit
          exports
        </p>
      </div>
    </div>
