...
import { AlertTriangle, CheckCircle, Play, RefreshCw, Wifi, WifiOff } from 'lucide-react';
...
const statusIcon = (status: string) => {
  ...
  if (status === 'MOCKED_DEMO_MODE')
    return <AlertTriangle className="w-3.5 h-3.5 text-[#d4a054]" />;
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Integration health matrix and connector management
        </p>
      </div>
      ...
      {error ? (
        <div className="bg-card border border-border rounded-xl p-8 text-center text-muted-foreground">
          ...
          <p>Connector data requires API connection</p>
        </div>
      ) : (
        <>
              ...
              services.map((svc) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {svc.mode || svc.status.toLowerCase().replace(/_/g, ' ')}
                    </span>
                    <button
                      onClick={() => {
                        setTesting(svc.name);
