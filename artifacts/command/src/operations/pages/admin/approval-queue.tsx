...
export default function ApprovalQueue() {
  ...
  const { data, isLoading, error } = useStandardQuery<ApprovalsResponse>({
    ...
    queryFn: () =>
      apiFetch(`/admin/artifact-approvals${filter !== 'all' ? `?status=${filter}` : ''}`),
  ...
  const rejectMutation = useStandardMutation({
    mutationFn: ({ approvalId, reason: r }: { approvalId: string; reason?: string }) =>
      apiFetch(`/admin/artifact-approvals/${approvalId}/reject`, {
        method: 'POST',
        ...
        <div className="space-y-4">
          ...
          <div className="bg-card border border-border rounded-xl p-8 text-center">
            ...
            <p className="text-muted-foreground text-sm mb-1">
              Artifact Approvals feature is disabled
            </p>
            <p className="text-xs text-muted-foreground/60">
              Enable the <code className="text-[#d4a054]">alloy_artifact_approvals_enabled</code>{' '}
              feature flag to activate this queue.
            </p>
          </div>
        </div>
      );
      ...
      <div className="space-y-4">
        ...
        <div className="bg-card border border-border rounded-xl p-8 text-center">
          ...
          <p className="text-muted-foreground text-sm">
            Failed to load approvals — API unavailable
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Alloy artifact approvals requiring human review
          </p>
        </div>
        {pending > 0 && (
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={() => qc.invalidateQueries({ queryKey: ['admin-approvals'] })}
          className="ml-auto p-1.5 rounded-lg hover:bg-white/5 text-muted-foreground"
        >
        ...
        >
          ...
          >
            ...
            <p className="text-xs text-muted-foreground mb-1">
              Type: <span className="text-foreground">{selected.artifactType}</span> · Domain:{' '}
              <span className="text-foreground">{selected.domain}</span>
            </p>
            {selected.requestedByLabel && (
              <p className="text-xs text-muted-foreground mb-4">
                Requested by <span className="text-foreground">{selected.requestedByLabel}</span> ·{' '}
                {timeAgo(selected.requestedAt)}
              </p>
            )}
            <div className="bg-black/20 border border-border/50 rounded-lg p-2.5 text-[10px] font-mono text-muted-foreground mb-4">
              <span className="text-muted-foreground/50">approval_id: </span>
              {selected.approvalId}
              <br />
              <span className="text-muted-foreground/50">artifact_id: </span>
              {selected.artifactId}
              {selected.workflowRunId && (
                <>
                  <br />
                  <span className="text-muted-foreground/50">run_id: </span>
                  {selected.workflowRunId}
                </>
              )}
            </div>
