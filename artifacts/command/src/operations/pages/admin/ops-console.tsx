...
function StatusIcon({ status }: { status: string }) {
  if (status === 'healthy' || status === 'ok' || status === 'active')
    return <CheckCircle style={{ width: 14, height: 14, color: '#6b8f71' }} />;
  if (status === 'degraded')
    return <AlertTriangle style={{ width: 14, height: 14, color: '#d4a054' }} />;
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        ...
        <div>
          ...
          {subtitle && (
            <div style={{ fontSize: '10px', color: TEXT.tertiary, marginTop: '1px' }}>
              {subtitle}
            </div>
          )}
        </div>
...
function MetricCard({
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: '20px', fontWeight: 700, color, letterSpacing: '-0.02em' }}>
        {value}
      </div>
...
export default function OpsConsole() {
  ...
  const overview = useStandardQuery<AdminOverview>({
    queryKey: ['ops-overview', refreshKey],
    queryFn: () => apiFetch('/admin/overview'),
    refetchInterval: 30000,
  });
  const systemHealth = useStandardQuery<SystemHealth>({
    queryKey: ['ops-system-health', refreshKey],
    queryFn: () => apiFetch('/admin/system-health'),
    refetchInterval: 60000,
  });
  const jobsData = useStandardQuery<JobStats>({
    queryKey: ['ops-jobs', refreshKey],
    queryFn: () => apiFetch('/admin/jobs/stats'),
    refetchInterval: 30000,
  });
  const connectorsData = useStandardQuery<ConnectorSummary>({
    queryKey: ['ops-connectors', refreshKey],
    queryFn: () => apiFetch('/admin/connectors'),
    refetchInterval: 60000,
  });
  const seedData = useStandardQuery<SeedValidation>({
    queryKey: ['ops-seed', refreshKey],
    queryFn: () => apiFetch('/admin/seed/validate'),
    staleTime: 5 * 60 * 1000,
  });
  const feedHealth = useStandardQuery<FeedHealth>({
    queryKey: ['ops-feed-health', refreshKey],
    queryFn: () => apiFetch('/admin/feed-health'),
    refetchInterval: 30000,
  });
  const rmmHealth = useStandardQuery<{
    overallStatus: string;
    providers: {
      total: number;
      active: number;
  ...
  const TABS: { key: TabKey; label: string; icon: any; badge?: number | string }[] = [
    { key: 'overview', label: 'Overview', icon: BarChart3 },
    {
      ...
      label: 'System Health',
      icon: Shield,
      badge: sh ? sh.summary.degraded + sh.summary.down : undefined,
    },
    { key: 'jobs', label: 'Jobs & Queue', icon: Activity, badge: jd?.stats?.running },
    {
      ...
      label: 'Connectors',
      icon: Globe,
      badge: cd ? cd.summary.manualRequired : undefined,
    },
    {
      key: 'seed',
      label: 'Seed Data',
      icon: Database,
      badge: sd ? sd.summary.failed + sd.summary.errors : undefined,
    },
    {
      key: 'feeds',
      label: 'Intel Feeds',
      icon: Rss,
      badge: feedHealth.data
        ? feedHealth.data.summary.degraded + feedHealth.data.summary.down
        : undefined,
    },
    { key: 'errors', label: 'Error Summary', icon: AlertCircle },
    {
      ...
      icon: Server,
      badge: rmmHealth.data?.devices.critical ?? undefined,
    },
  ];
    ...
    <div style={{ padding: '1.25rem 1.5rem', maxWidth: '1200px' }}>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: '11px', color: TEXT.tertiary, marginTop: '2px' }}>
            Operational visibility: service health, deployment info, queue status, connector sync,
            and diagnostics
          </p>
        </div>
      ...
      {ov && (
        ...
        >
          ...
          Updated {formatTime(ov.timestamp)} · Uptime {formatUptime(ov.system.uptime)} · Node{' '}
          {ov.system.nodeVersion}
        </div>
      )}
      ...
      {ov && (
        ...
        >
          <MetricCard
            icon={Server}
            label="Apps"
            value={`${ov.counts.activeApps}/${ov.counts.apps}`}
            sub="active"
            color="#d4a054"
          />
          <MetricCard
            icon={Zap}
            label="Connectors"
            value={`${ov.counts.liveConnectors}/${ov.counts.connectors}`}
            sub="live"
            color="#6b8f71"
          />
          <MetricCard
            icon={Users}
            label="Users"
            value={`${ov.counts.activeUsers}/${ov.counts.users}`}
            sub="active"
            color="#4a90b8"
          />
          <MetricCard
            icon={Cpu}
            label="Heap"
            value={`${Math.round((ov.system.memoryUsage.heapUsed / ov.system.memoryUsage.heapTotal) * 100)}%`}
            sub={formatBytes(ov.system.memoryUsage.heapUsed)}
            color={
              ov.system.memoryUsage.heapUsed / ov.system.memoryUsage.heapTotal > 0.8
                ? '#c45a4a'
                : '#d4a054'
            }
          />
          <MetricCard
            icon={Database}
            label="DB Latency"
            value={`${ov.database.latency}ms`}
            sub={ov.database.status}
            color={ov.database.status === 'healthy' ? '#6b8f71' : '#c45a4a'}
          />
        </div>
      )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          ...
          >
            <SectionHeader
              ...
              subtitle="Node.js process and resource utilization"
            />
            {ov ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                ...
                <div>
                  ...
                  >
                    ...
                    <span>
                      {Math.round(
                        (ov.system.memoryUsage.heapUsed / ov.system.memoryUsage.heapTotal) * 100,
                      )}
                      %
                    </span>
                  </div>
                  <ProgressBar
                    pct={(ov.system.memoryUsage.heapUsed / ov.system.memoryUsage.heapTotal) * 100}
                    color={
                      ov.system.memoryUsage.heapUsed / ov.system.memoryUsage.heapTotal > 0.8
                        ? '#c45a4a'
                        : '#6b8f71'
          ...
          >
            <SectionHeader
              icon={Database}
              title="Database"
              subtitle="PostgreSQL connection pool and query health"
            />
            {ov ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <div
                ...
                {[
                  ...
                  {
                    label: 'Connections',
                    value: `${ov.database.connections}/${ov.database.maxConnections}`,
                  },
                ...
                <div style={{ paddingTop: '8px', borderTop: `1px solid ${BORDER.subtle}` }}>
                  ...
                  >
                    ...
                    <StatusBadge
                      status={ov.storage.status === 'healthy' ? 'healthy' : 'degraded'}
                    />
                  </div>
                  <div
                    style={{
                  ...
                  >
                    ...
                    >
                      {formatBytes(ov.storage.usedBytes)} / {formatBytes(ov.storage.totalBytes)}
                    </span>
                  </div>
          ...
          >
            <SectionHeader
              ...
              subtitle="Integration adapter health and sync status"
            />
            {cd ? (
              <div>
                <div
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                  {cd.connectors.slice(0, 6).map((c) => (
                    ...
                    >
                      ...
                      <StatusBadge
                        status={
                          c.status === 'LIVE_CONFIGURED'
                            ? 'healthy'
                            : c.status === 'MOCKED_DEMO_MODE'
                              ? 'degraded'
                              : 'down'
                        }
                      />
                  ...
                  {cd.connectors.length > 6 && (
                    <div style={{ fontSize: '10px', color: TEXT.muted, paddingTop: '4px' }}>
                      +{cd.connectors.length - 6} more connectors
                    </div>
                  )}
                </div>
              </div>
          ...
          >
            <SectionHeader
              icon={Radio}
              title="Model Lane & Worldline"
              subtitle="AI inference layer and event fabric freshness"
            />
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {[
                {
                  lane: 'Fast Lane (GPT-4o)',
                  latency: '~800ms',
                  status: 'healthy',
                  model: 'gpt-4o',
                },
                {
                  lane: 'Deep Lane (Claude 3.5)',
                  latency: '~2.1s',
                  status: 'healthy',
                  model: 'claude-3-5-sonnet',
                },
                {
                  lane: 'Economy Lane (Gemini)',
                  latency: '~1.2s',
                  status: 'healthy',
                ...
                >
                  <div>
                    <div style={{ fontSize: '11px', fontWeight: 500, color: TEXT.primary }}>
                      {lane.lane}
                    </div>
                    <div
                      style={{
              ...
              <div style={{ paddingTop: '8px', borderTop: `1px solid ${BORDER.subtle}` }}>
                ...
                {[
                  ...
                  >
                    ...
                    <span
                      style={{
                        color: s.status === 'healthy' ? '#6b8f71' : '#d4a054',
                        fontFamily: 'var(--font-mono)',
                        fontSize: '10px',
                      }}
                    >
            ...
            >
              <div
                style={{
                  width: 20,
                  height: 20,
                  border: '2px solid rgba(212,160,84,0.2)',
                  borderTopColor: '#d4a054',
                  borderRadius: '50%',
            ...
            <>
              ...
              >
                <MetricCard
                  icon={Activity}
                  label="Total Checks"
                  value={sh.summary.total}
                  color="#d4a054"
                />
                <MetricCard
                  icon={CheckCircle}
                  label="Healthy"
                  value={sh.summary.healthy}
                  color="#6b8f71"
                />
                <MetricCard
                  icon={AlertTriangle}
                  label="Degraded"
                  value={sh.summary.degraded}
                  color="#d4a054"
                />
                      ...
                      {checks.map((check) => (
                        ...
                        >
                          ...
                          >
                            ...
                            <div>
                              <div
                                style={{ fontSize: '12px', fontWeight: 500, color: TEXT.primary }}
                              >
                                {check.name}
                              </div>
                              <div
                                style={{ fontSize: '10px', color: TEXT.tertiary, marginTop: '1px' }}
                              >
                                {check.details}
                              </div>
                            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          >
            <SectionHeader
              ...
              subtitle="Latest platform job executions and status"
            />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                {jd.jobs.slice(0, 20).map((job) => (
                  ...
                  >
                    ...
                    >
                      <span style={{ color: TEXT.muted, fontSize: '10px' }}>
          ...
          >
            <SectionHeader
              icon={Lock}
              title="Blocked Exports & Approvals"
              subtitle="Exports or approvals awaiting action"
            />
            <div style={{ color: TEXT.muted, fontSize: '12px', padding: '0.5rem 0' }}>
              Connect to API admin endpoints for live export queue data. Visit{' '}
              <span style={{ color: '#d4a054', fontFamily: 'var(--font-mono)', fontSize: '11px' }}>
                /admin/exports
              </span>{' '}
              or{' '}
              <span style={{ color: '#d4a054', fontFamily: 'var(--font-mono)', fontSize: '11px' }}>
            ...
            <>
              ...
              >
                ...
                <MetricCard
                  icon={CheckCircle}
                  label="Live"
                  value={cd.summary.liveConfigured}
                  color="#6b8f71"
                />
                <MetricCard
                  icon={AlertTriangle}
                  label="Demo Mode"
                  value={cd.summary.mockedDemoMode}
                  color="#d4a054"
                />
                <MetricCard
                  icon={WifiOff}
                  label="Needs Config"
                  value={cd.summary.manualRequired}
                  color="#c45a4a"
                />
              </div>
              ...
              >
                <SectionHeader
                  ...
                  subtitle="Integration adapter status and sync health"
                />
            ...
            <>
              ...
              >
                <MetricCard
                  icon={Database}
                  label="Total Tables"
                  value={sd.summary.total}
                  color="#d4a054"
                />
                <MetricCard
                  icon={CheckCircle}
                  label="Seeded"
                  value={sd.summary.passed}
                  color="#6b8f71"
                />
                <MetricCard
                  icon={AlertTriangle}
                  label="Insufficient"
                  value={sd.summary.failed}
                  color="#c45a4a"
                />
              ...
              >
                <SectionHeader
                  ...
                  subtitle="Expected row counts vs actual for all seeded tables"
                />
                <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                  {sd.results.map((row) => (
                    <div
          ...
          {feedHealth.data && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '0.75rem' }}>
              <MetricCard
                icon={Rss}
                label="Feeds"
                value={feedHealth.data.summary.totalFeeds}
                color="#d4a054"
              />
              <MetricCard
                icon={CheckCircle}
                label="Healthy"
                value={feedHealth.data.summary.healthy}
                color="#6b8f71"
              />
              <MetricCard
                icon={AlertTriangle}
                label="Degraded"
                value={feedHealth.data.summary.degraded}
                color="#d4a054"
              />
              <MetricCard
                icon={X}
                label="Down"
                value={feedHealth.data.summary.down}
                color="#c45a4a"
              />
              <MetricCard
                icon={TrendingUp}
                label="Created / Merged"
                value={`${feedHealth.data.summary.totalEntitiesCreated} / ${feedHealth.data.summary.totalEntitiesMerged}`}
                sub="since startup"
                color="#4a90b8"
              />
            </div>
          ...
          >
            <SectionHeader
              ...
              action={
                feedHealth.data && (
                  <span
                    style={{ fontSize: '10px', color: TEXT.muted, fontFamily: 'var(--font-mono)' }}
                  >
            ...
            {feedHealth.data && feedHealth.data.feeds.length === 0 && (
              ...
              >
                No intelligence feeds registered. Enable feeds via env flags (e.g.{' '}
                <span style={{ fontFamily: 'var(--font-mono)' }}>SANCTIONS_FEED_ENABLED</span>) and
            ...
            {feedHealth.data && feedHealth.data.feeds.length > 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <div
                  style={{
                    display: 'grid',
                    gridTemplateColumns: '1.6fr 0.7fr 0.9fr 0.9fr 1.1fr 1.1fr',
                    gap: '10px',
                    fontSize: '9px',
                    textTransform: 'uppercase',
                    letterSpacing: '0.08em',
                ...
                {feedHealth.data.feeds.map((f) => {
                  const sparkSeries = f.recentPolls.map(
                    (p) => p.entitiesCreated + p.entitiesMerged,
                  );
                  const stalled =
                    f.recentPolls.length === 0 ||
                    (f.lastIngestedAt !== null &&
                      Date.now() - new Date(f.lastIngestedAt).getTime() > 30 * 60 * 1000);
                  return (
                    <div
                      key={f.feedId}
                      style={{
                        ...
                        borderRadius: '6px',
                        background: BG.section,
                        border: `1px solid ${stalled ? 'rgba(212,160,84,0.18)' : BORDER.muted}`,
                      }}
                    >
                      <div style={{ minWidth: 0 }}>
                        ...
                        >
                          {f.feedId} · avg {f.avgPollDurationMs}ms
                          {f.consecutiveFailures > 0 ? ` · ${f.consecutiveFailures} fails` : ''}
                        </div>
                      </div>
                      <StatusBadge
                        status={
                      ...
                      >
                        {f.totalEntitiesMerged}
                      </span>
                      <span
                        style={{
                          fontSize: '11px',
                          color: stalled ? '#d4a054' : TEXT.secondary,
                          fontFamily: 'var(--font-mono)',
                        }}
                      >
                        {relativeTime(f.lastIngestedAt)}
                      </span>
                      <div
                        style={{
                          display: 'flex',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          >
            <SectionHeader
              ...
              subtitle="Aggregated errors from API server and services"
            />
            ...
            {sh && (sh.summary.degraded > 0 || sh.summary.down > 0) ? (
                  ...
                  .map((c) => (
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: '6px',
                        background:
                          c.status === 'down' ? 'rgba(196,90,74,0.06)' : 'rgba(212,160,84,0.04)',
                        border: `1px solid ${c.status === 'down' ? 'rgba(196,90,74,0.2)' : 'rgba(212,160,84,0.2)'}`,
                      }}
                    >
                      <StatusIcon status={c.status} />
                      <div>
                        <div style={{ fontSize: '12px', fontWeight: 500, color: TEXT.primary }}>
                          {c.category}: {c.name}
                        </div>
                        <div style={{ fontSize: '11px', color: TEXT.secondary, marginTop: '2px' }}>
                          {c.details}
                        </div>
                      </div>
              ...
              >
                ...
                <span style={{ fontSize: '12px', color: '#6b8f71' }}>
                  No errors detected across monitored services.
                </span>
              </div>
            )}
          </div>
          ...
          >
            <SectionHeader
              ...
              subtitle="Deployment version and build context"
            />
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {[
          ...
          >
            <SectionHeader
              ...
              subtitle="Active tenants and provisioning status"
            />
            <div style={{ color: TEXT.muted, fontSize: '12px', padding: '0.5rem 0' }}>
              Tenant management is available at{' '}
              <span style={{ color: '#d4a054', fontFamily: 'var(--font-mono)', fontSize: '11px' }}>
                /api/admin/tenants
              </span>{' '}
              (admin auth required). Azure AD tenant onboarding is handled via the Azure Tenant
              Dashboard in SZL Holdings.
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'infrastructure' &&
        (() => {
          ...
          const providerLabels: Record<string, string> = {
            ninjaone: 'NinjaOne',
            connectwise_automate: 'CW Automate',
            connectwise_manage: 'CW Manage',
            halopsa: 'HaloPSA',
            datto_rmm: 'Datto RMM',
            autotask_psa: 'Autotask PSA',
          };
          ...
          return (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                  ...
                  {[
                    {
                      ...
                      value: rmm?.devices.total ?? 0,
                      color: '#7aafcf',
                      icon: Server,
                    },
                    {
                      label: 'Online',
                      value: rmm?.devices.online ?? 0,
                      color: '#6b8f71',
                      icon: CheckCircle,
                    },
                    {
                      label: 'Critical Alerts',
                      value: rmm?.devices.critical ?? 0,
                      color: '#c45a4a',
                      icon: AlertTriangle,
                    },
                    {
                      label: 'Pending Approvals',
                      value: rmm?.healing.pendingApprovals ?? 0,
                      color: '#d4a054',
                      icon: Clock,
                    },
              ...
              >
                <SectionHeader
                  icon={Server}
                  title="RMM / PSA Providers"
                  subtitle="Connected provider status and device counts"
                />
                {rmmHealth.isLoading ? (
                  <div style={{ color: TEXT.muted, fontSize: '12px' }}>
                    Loading provider status…
                  </div>
                ) : !rmm || rmm.providers.total === 0 ? (
                  <div style={{ color: TEXT.muted, fontSize: '12px', padding: '0.75rem 0' }}>
                    No providers configured. Add one at{' '}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                    {rmm.providers.list.map((p) => (
                      ...
                      >
                        <div>
                          <div style={{ fontSize: '12px', fontWeight: 600, color: TEXT.primary }}>
                            {p.name}
                          </div>
                          <div
                            style={{
                          ...
                          >
                            {providerLabels[p.provider] ?? p.provider} · {p.deviceCount ?? 0}{' '}
                            devices
                          </div>
                        </div>
              ...
              >
                <SectionHeader
                  ...
                  subtitle="Aggregated metrics across all managed endpoints"
                />
                {rmm ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    {[
              ...
              >
                <SectionHeader
                  icon={Zap}
                  title="Auto-Healing Status"
                  subtitle="Healing playbook execution statistics"
                />
                {rmm ? (
