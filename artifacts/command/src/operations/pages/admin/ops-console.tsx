  ...
  return (
    <div style={{ padding: "1.25rem 1.5rem", maxWidth: "1200px" }}>
      ...
      {activeTab === "infrastructure" && (() => {
        ...
        const providerLabels: Record<string, string> = { ninjaone: "NinjaOne", connectwise_automate: "CW Automate", connectwise_manage: "CW Manage", halopsa: "HaloPSA", datto_rmm: "Datto RMM", autotask_psa: "Autotask PSA" };
                  ...
                  {rmm.providers.list.map(p => (
                    <div key={p.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.625rem 0.75rem", borderRadius: "0.5rem", background: BG.section, border: `1px solid ${BORDER.muted}` }}>
                      <div>
                        ...
                        <div style={{ fontSize: "10px", color: TEXT.tertiary, fontFamily: "var(--font-mono)" }}>{providerLabels[p.provider] ?? p.provider} · {p.deviceCount ?? 0} devices</div>
                      </div>
              ...
              {rmm ? (
                <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
                  ...
                  {Object.keys(rmm.healing.stats).length === 0 && (
                    <div style={{ fontSize: "12px", color: TEXT.muted }}>No healing executions recorded yet. Configure playbooks in the Aegis Ops Console.</div>
                  )}
                </div>
              ) : (
