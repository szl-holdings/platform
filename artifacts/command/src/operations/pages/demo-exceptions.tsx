    ...
    <div className="p-4 max-w-[1100px] space-y-4">
      <div>
        ...
        <p className="text-[11px] mt-0.5" style={{ color: TEXT.secondary }}>
          Categorized exceptions requiring review — SLA breaches, ownership gaps, compliance, and
          process violations
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <div className="flex items-center gap-1">
          ...
          {['all', 'sla_breach', 'ownership_gap', 'data_quality', 'compliance', 'capacity'].map(
            (f) => {
              const cc = CAT[f];
              return (
                <button
                  ...
                  style={{
                    color: catFilter === f ? (cc?.color ?? '#d4a054') : TEXT.muted,
                    background: catFilter === f ? `${cc?.color ?? '#d4a054'}12` : 'transparent',
                    borderColor: catFilter === f ? `${cc?.color ?? '#d4a054'}25` : BORDER.subtle,
                  }}
                >
                  {f === 'all' ? 'All' : cc.label}
                </button>
          ...
          return (
            <div
              ...
              style={{
                ...
                border: `1px solid ${isHighRisk && exc.severity === 'critical' ? 'rgba(196,90,74,0.15)' : BORDER.subtle}`,
              }}
            >
                ...
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      <span
                        className="text-[8px] px-1.5 py-px rounded uppercase font-mono"
                        style={{
                          color: sc.text,
                          background: sc.bg,
                      ...
                      <span
                        ...
                        style={{
                          color: sta.color,
                          background: `${sta.color}10`,
                          border: `1px solid ${sta.color}20`,
                        }}
                      >
                        {sta.label}
                      </span>
                      {exc.slaBreachAt && (
                        <span
                          ...
                          style={{ color: '#c45a4a' }}
                        >
                          <Clock className="w-2.5 h-2.5" /> SLA breached {timeAgo(exc.slaBreachAt)}
                        </span>
                      )}
                    </div>
                    <p
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      <span className="text-[9px]" style={{ color: TEXT.muted }}>
                        System: {exc.affectedSystem}
                      </span>
                      <span className="text-[9px]" style={{ color: TEXT.muted }}>
                        Owner: {exc.owner ?? 'Unassigned'}
                      </span>
                      <span className="text-[9px]" style={{ color: TEXT.muted }}>
                        Detected: {timeAgo(exc.detectedAt)}
                      </span>
                    </div>
                  </div>
