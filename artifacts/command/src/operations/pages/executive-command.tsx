...
import {
  ...
  Clock, Eye, Heart, Radio, Shield, TrendingDown,
  TrendingUp, Zap, BarChart3, Users, FileText, ArrowUpRight,
  Target, Layers, Server, GitBranch, CheckCheck, Play,
  User, Calendar, Hash, CircleDot, ArrowRight, Cpu, Database
} from "lucide-react";
...
const PACK_SIGNALS: PackSignal[] = [
  { pack: "PRISM", label: "Portfolio Intelligence", color: "#d4a054", status: "warning", health: 71, openItems: 14, criticalCount: 3, lastSignal: "4m ago", riskValue: "$5.03M", trend: "down" },
  { pack: "Terra", label: "Real Estate Intelligence", color: "#a07848", status: "healthy", health: 88, openItems: 6, criticalCount: 0, lastSignal: "11m ago", riskValue: "$1.2M", trend: "stable" },
  { pack: "Vessels", label: "Fleet Command", color: "#38bdf8", status: "degraded", health: 52, openItems: 21, criticalCount: 5, lastSignal: "2m ago", riskValue: "$8.7M", trend: "down" },
  { pack: "Aegis", label: "Defense & Intelligence", color: "#4f6ef7", status: "healthy", health: 94, openItems: 3, criticalCount: 0, lastSignal: "28m ago", riskValue: "$0.4M", trend: "up" },
];

const PRESSURE_ITEMS = [
  { pack: "Vessels", title: "Fleet ETA compliance gap — 3 vessels outside SLA", severity: "critical", age: "6h", impact: "$2.1M", owner: "Fleet Ops", stage: "Escalated", dueDate: "Today", risk: "SLA Breach" },
  { pack: "PRISM", title: "Ownership conflict detected in accounts receivable", severity: "high", age: "14h", impact: "$890K", owner: "Finance", stage: "Review", dueDate: "Apr 17", risk: "Ownership Gap" },
  { pack: "Vessels", title: "Fuel surcharge approval chain stalled", severity: "high", age: "22h", impact: "$450K", owner: "Fleet Ops", stage: "Approval Pending", dueDate: "Overdue", risk: "Process Stall" },
  { pack: "PRISM", title: "Executive approval pending — Q2 pricing revision", severity: "high", age: "31h", impact: "$1.2M", owner: "Operations", stage: "Approval Pending", dueDate: "Overdue", risk: "Revenue Risk" },
  { pack: "Terra", title: "Lease renewal document missing signature block", severity: "medium", age: "2d", impact: "$320K", owner: "Legal", stage: "In Progress", dueDate: "Apr 18", risk: "Compliance" },
];
...
const PENDING_APPROVALS = [
  {
    ...
    confidence: 88,
    approvalChain: [
      { actor: "Sarah Chen", role: "Director", action: "submitted", time: "31h ago", status: "done" },
      { actor: "Michael Torres", role: "VP Finance", action: "approved", time: "28h ago", status: "done" },
  ...
  },
  {
    ...
    confidence: 76,
    approvalChain: [
      { actor: "Terra Finance", role: "Finance", action: "submitted", time: "4d ago", status: "done" },
...
const OPERATING_LOOP = [
  { id: "observe", label: "Observe", icon: Eye, count: 44, active: true, description: "44 active signals" },
  { id: "evaluate", label: "Evaluate", icon: BarChart3, count: 18, active: true, description: "18 under analysis" },
  { id: "decide", label: "Decide", icon: Target, count: 8, active: true, description: "8 decision items" },
  { id: "approve", label: "Approve", icon: CheckCircle2, count: 4, active: true, description: "4 pending approval" },
  { id: "act", label: "Act", icon: Play, count: 12, active: true, description: "12 in motion" },
  { id: "prove", label: "Prove", icon: CheckCheck, count: 31, active: true, description: "31 evidenced" },
];
...
function OperatingLoopRail() {
  return (
    <div className="rounded-md px-4 py-3" style={{ background: BG.surface, border: `1px solid ${BORDER.subtle}` }}>
      <div className="flex items-center gap-2 mb-2.5">
        <CircleDot className="w-3 h-3" style={{ color: ELECTRIC }} />
        <span className="text-[9px] font-medium uppercase tracking-widest" style={{ color: TEXT.muted }}>Operating Loop — Observe → Evaluate → Decide → Approve → Act → Prove</span>
      </div>
            ...
            <div key={step.id} className="flex items-center flex-1 min-w-0">
              <div className="flex-1 min-w-0 group cursor-default">
                <div
                  ...
                  style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${step.active ? ELECTRIC + "20" : BORDER.subtle}` }}
                  title={step.description}
                >
                  <div className="flex items-center gap-1">
                    <Icon className="w-3 h-3" style={{ color: step.active ? ELECTRIC : TEXT.muted }} />
                    <span className="text-[9px] font-medium" style={{ color: step.active ? TEXT.secondary : TEXT.muted }}>{step.label}</span>
                  </div>
                  <span
                    className="text-[11px] font-mono font-bold"
                    style={{ color: step.active ? (step.count > 0 ? ELECTRIC : "#22c55e") : TEXT.muted }}
                  >
                    {step.count}
                  </span>
                </div>
...
function ServiceHealthStrip() {
  return (
    <div className="rounded-md overflow-hidden" style={{ background: BG.surface, border: `1px solid ${BORDER.subtle}` }}>
      <div className="flex items-center justify-between px-4 py-2.5" style={{ borderBottom: `1px solid ${BORDER.subtle}` }}>
        <div className="flex items-center gap-2">
          <Server className="w-3 h-3" style={{ color: TEXT.tertiary }} />
          <span className="text-[10px] font-medium" style={{ color: TEXT.secondary }}>Service Health</span>
          <span className="text-[8px] px-1.5 py-0.5 rounded font-mono" style={{ background: "rgba(34,197,94,0.08)", color: "#22c55e" }}>
            {SERVICE_HEALTH.filter(s => s.status === "healthy").length}/{SERVICE_HEALTH.length} Healthy
          </span>
        </div>
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 divide-x" style={{ borderColor: BORDER.subtle }}>
        {SERVICE_HEALTH.map(svc => {
          const sc = serviceStatusColor(svc.status);
          return (
            <div key={svc.name} className="px-3 py-2.5 text-center" style={{ borderColor: BORDER.subtle }}>
              <div className="flex items-center justify-center gap-1 mb-1">
                <span className="w-1.5 h-1.5 rounded-full" style={{ background: sc }} />
                <span className="text-[9px] font-medium truncate" style={{ color: TEXT.secondary }}>{svc.name}</span>
              </div>
              <div className="text-[10px] font-mono" style={{ color: sc }}>{svc.latency}</div>
              <div className="text-[7px] font-mono" style={{ color: TEXT.muted }}>{svc.uptime}</div>
            </div>
          );
        })}
      </div>
        ...
        <div key={i} className="flex items-center gap-0.5">
          <div className="flex items-center gap-1 text-[8px]">
            <span
              ...
              style={{
                color: step.status === "done" ? "#22c55e" : step.status === "pending" ? TEXT.muted : "#d4a054",
                background: step.status === "done" ? "rgba(34,197,94,0.07)" : step.status === "pending" ? "rgba(255,255,255,0.03)" : "rgba(212,160,84,0.07)",
              }}
            >
              {step.actor}
            </span>
            {step.status === "done" && <CheckCircle2 className="w-2.5 h-2.5" style={{ color: "#22c55e" }} />}
            {step.status === "pending" && <Clock className="w-2.5 h-2.5" style={{ color: TEXT.muted }} />}
            {step.status === "review" && <Eye className="w-2.5 h-2.5" style={{ color: "#d4a054" }} />}
          </div>
          {i < chain.length - 1 && <ArrowRight className="w-2.5 h-2.5" style={{ color: TEXT.muted }} />}
        </div>
      ))}
    </div>
  );
...
function DemoModeBanner() {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2 rounded-md" style={{ background: "rgba(212,160,84,0.07)", border: `1px solid rgba(212,160,84,0.2)` }}>
      <div className="flex items-center gap-1.5">
        <span className="relative flex h-2 w-2">
          <span className="animate-ping absolute inline-flex h-full w-full rounded-full opacity-50" style={{ background: "#d4a054" }} />
          <span className="relative inline-flex rounded-full h-2 w-2" style={{ background: "#d4a054" }} />
        </span>
        <span className="text-[10px] font-mono font-bold uppercase tracking-widest" style={{ color: "#d4a054" }}>Demo Mode Active</span>
      </div>
      <span className="text-[9px]" style={{ color: "rgba(212,160,84,0.6)" }}>
        All data is synthetic — no live systems connected. Toggle demo mode off to connect real signals.
      </span>
      <div className="ml-auto shrink-0">
        <span className="text-[8px] px-1.5 py-0.5 rounded font-mono uppercase" style={{ background: "rgba(212,160,84,0.12)", color: "#d4a054", border: "1px solid rgba(212,160,84,0.2)" }}>
          SEEDED
        </span>
      </div>
    </div>
  ...
  return (
    <div className="p-4 md:p-5 space-y-4" style={{ background: BG.page }}>
            ...
            ? PRESSURE_ITEMS.map((item, i) => (
                <div key={i} className="flex items-start gap-3 px-4 py-3 hover:bg-white/[0.02] transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px]" style={{ color: TEXT.primary }}>{item.title}</p>
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      ...
                      <span className="text-[8px] flex items-center gap-1" style={{ color: TEXT.muted }}>
                        ...
                        <span style={{ color: item.stage.includes("Pending") ? "#c8953c" : item.stage === "Escalated" ? "#c45a4a" : TEXT.tertiary }}>
                          {item.stage}
                        </span>
                      </span>
                      <span className="text-[8px] flex items-center gap-1" style={{ color: item.dueDate === "Overdue" ? "#c45a4a" : item.dueDate === "Today" ? "#d4a054" : TEXT.muted }}>
                        <Calendar className="w-2.5 h-2.5" /> {item.dueDate}
                      </span>
                      <span className="text-[8px] px-1.5 py-0.5 rounded" style={{ color: TEXT.tertiary, background: "rgba(255,255,255,0.03)" }}>
                        {item.risk}
                      </span>
                    </div>
                  </div>
      ...
      <div className="rounded-md overflow-hidden" style={{ background: BG.surface, border: `1px solid ${BORDER.subtle}` }}>
        ...
        <div className="divide-y" style={{ "--tw-divide-opacity": 1 } as React.CSSProperties}>
          {PENDING_APPROVALS.map(a => {
            ...
            return (
              <div key={a.id} className="transition-colors" style={{ background: isExpanded ? "rgba(255,255,255,0.01)" : "transparent" }}>
                <button
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      ...
                      <span className="text-[7px] px-1 py-px rounded font-mono" style={{ background: "rgba(255,255,255,0.04)", color: TEXT.muted }}>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <ChevronRight className="w-3 h-3 transition-transform" style={{ color: TEXT.muted, transform: isExpanded ? "rotate(90deg)" : "none" }} />
                  </div>
                </button>

                {isExpanded && (
                  <div className="px-4 pb-3 pt-1" style={{ borderTop: `1px solid ${BORDER.subtle}` }}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        <div className="text-[8px] uppercase tracking-widest mb-1.5 font-medium" style={{ color: TEXT.muted }}>Evidence / Provenance</div>
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <FileText className="w-2.5 h-2.5" style={{ color: TEXT.tertiary }} />
                          <span className="text-[9px]" style={{ color: TEXT.secondary }}>{a.evidence}</span>
                        </div>
                      </div>
                      <div>
                        <div className="text-[8px] uppercase tracking-widest mb-1.5 font-medium" style={{ color: TEXT.muted }}>Approval Chain</div>
                        <ApprovalAuditChain chain={a.approvalChain} />
                      </div>
                    </div>
                    <div className="flex items-center gap-2 mt-3">
                      <button className="text-[9px] px-2.5 py-1.5 rounded font-medium transition-all hover:opacity-80" style={{ color: "#22c55e", background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)" }}>
                        Approve
                      </button>
                      <button className="text-[9px] px-2.5 py-1.5 rounded font-medium transition-all hover:opacity-80" style={{ color: "#c45a4a", background: "rgba(196,90,74,0.08)", border: "1px solid rgba(196,90,74,0.2)" }}>
                        Reject
                      </button>
                      <button className="text-[9px] px-2.5 py-1.5 rounded font-medium transition-all hover:opacity-80" style={{ color: "#8b7ac8", background: "rgba(139,92,246,0.08)", border: "1px solid rgba(139,92,246,0.2)" }}>
                        Escalate
                      </button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {[
          { href: "/operations/blocker-board", label: "Blocker Board", icon: AlertTriangle, color: "#c45a4a" },
          { href: "/operations/digest", label: "Digest Center", icon: FileText, color: "#d4a054" },
          { href: "/operations/approvals", label: "Approvals", icon: CheckCircle2, color: ELECTRIC },
          { href: "/operations/trust-audit", label: "Trust & Audit", icon: Shield, color: "#8b7ac8" },
        ].map(link => (
