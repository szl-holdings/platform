...
const BLOCKERS: Blocker[] = [
  {
    ...
    description: "Three vessels awaiting fuel surcharge approval. SLA breach imminent if not resolved within 4h. Fleet operations cannot re-route until pricing is confirmed.",
    category: "approval",
    severity: "critical",
    age: "22h",
    ageHours: 22,
  ...
  },
  {
    ...
    description: "Pricing revision submitted 31h ago. Board deadline is T+48h. No response from approver chain. Downstream go-live blocked.",
    category: "approval",
    severity: "critical",
    age: "31h",
    ageHours: 31,
  ...
  },
  {
    ...
    description: "Vessel departure clearance pending port authority countersignature. Legal hold in place. Operations team has not been notified.",
    category: "compliance",
    severity: "high",
    age: "14h",
    ageHours: 14,
  ...
  },
  {
    ...
    description: "Two owners claimed on same AR account. Payments withheld pending resolution. Finance reconciliation blocked.",
    category: "ownership",
    severity: "high",
    age: "18h",
    ageHours: 18,
  ...
  },
  {
    ...
    description: "Tenant has signed but document is missing required exhibit B attachment. Property manager unresponsive. Renewal deadline in 3 days.",
    category: "compliance",
    severity: "high",
    age: "2d",
    ageHours: 48,
  ...
  },
  {
    ...
    description: "New physical security vendor cannot commence until background checks clear. HR vendor has 5-day processing backlog.",
    category: "dependency",
    severity: "medium",
    age: "6d",
    ageHours: 144,
  ...
  },
  {
    ...
    description: "Building 7A refinancing on hold. Third-party appraiser has not delivered final report. Closing window closes in 8 days.",
    category: "resource",
    severity: "medium",
    age: "4d",
    ageHours: 96,
    ...
    <div className="flex gap-0.5 items-center">
      {["critical", "high", "medium"].map((s, i) => (
        <div
          ...
          style={{
            background: i <= (sev === "critical" ? 2 : sev === "high" ? 1 : 0)
              ? (sev === "critical" ? "#c45a4a" : sev === "high" ? "#c8953c" : "#d4a054")
  ...
  return (
    <div className="p-4 md:p-5 space-y-5" style={{ background: BG.page }}>
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[9px] font-mono uppercase tracking-widest" style={{ color: "#c45a4a" }}>Blocker Board</span>
          </div>
          <h1 className="text-lg font-bold tracking-tight" style={{ color: TEXT.primary }}>Stuck Items Requiring Attention</h1>
          <p className="text-[11px] mt-0.5" style={{ color: TEXT.secondary }}>Items that are preventing progress across the portfolio</p>
        </div>
        {critical > 0 && (
          <div className="rounded px-3 py-2 text-center shrink-0" style={{ background: "rgba(196,90,74,0.08)", border: "1px solid rgba(196,90,74,0.18)" }}>
            <div className="text-base font-mono font-bold" style={{ color: "#c45a4a" }}>{critical}</div>
            <div className="text-[7px] uppercase tracking-wider" style={{ color: "rgba(196,90,74,0.55)" }}>Critical</div>
          </div>
        )}
      </div>
      ...
      <div className="space-y-2.5">
        {filtered.map(b => (
          <div key={b.id} className="rounded-md overflow-hidden" style={{ background: BG.surface, border: `1px solid ${b.severity === "critical" ? "rgba(196,90,74,0.18)" : BORDER.subtle}` }}>
            <div className="flex items-start gap-3 p-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                  <span className="text-[8px] font-bold tracking-widest px-1.5 py-0.5 rounded uppercase" style={{ color: b.packColor, background: `${b.packColor}14` }}>{b.pack}</span>
                  <span className="text-[8px] px-1.5 py-0.5 rounded uppercase tracking-wider" style={{ color: CATEGORY_COLORS[b.category], background: `${CATEGORY_COLORS[b.category]}12` }}>{CATEGORY_LABELS[b.category]}</span>
                  <SevBar sev={b.severity} />
                  <span className="text-[8px] font-mono capitalize" style={{ color: b.severity === "critical" ? "#c45a4a" : b.severity === "high" ? "#c8953c" : TEXT.secondary }}>{b.severity}</span>
                </div>

                <h3 className="text-[12px] font-medium leading-snug mb-1" style={{ color: TEXT.primary }}>{b.title}</h3>
                <p className="text-[10px] leading-relaxed mb-2.5" style={{ color: TEXT.secondary }}>{b.description}</p>
                ...
                <div className="flex flex-wrap items-center gap-3 text-[9px]">
                  ...
                  <span className="font-mono" style={{ color: "#c8953c" }}>Impact: {b.impact}</span>
                </div>
              </div>

              <div className="shrink-0 flex flex-col items-end gap-2">
                <span className="text-[8px] font-mono" style={{ color: TEXT.muted }}>{b.id}</span>
                {b.escalatable && (
                  <button className="flex items-center gap-1 px-2 py-1 rounded text-[8px] font-medium transition-all hover:opacity-80" style={{ background: "rgba(196,90,74,0.1)", border: "1px solid rgba(196,90,74,0.2)", color: "#c45a4a" }}>
