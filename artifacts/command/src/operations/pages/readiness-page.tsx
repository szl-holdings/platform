    ...
    <div className="space-y-6 max-w-[1200px]">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Operational readiness items with scores and status
          </p>
        </div>
        <button
          onClick={() => refetch()}
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="col-span-1 rounded-xl border border-[#d4a054]/20 bg-[#d4a054]/5 p-5 flex flex-col items-center justify-center">
          ...
          <div className="text-[11px] text-[#d4a054]/70 uppercase tracking-wide">
            Readiness Score
          </div>
        </div>
        {[
            ...
            {catItems.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className={cn(
                    'rounded-xl border bg-white/[0.02] p-4 flex items-start gap-3',
                    config.border,
                  )}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.status !== 'complete' && (
                      <div className="flex gap-2 mt-2">
                        ...
                        <button
                          onClick={() =>
                            updateItem.mutate({ id: item.id, status: 'complete', score: 100 })
                          }
