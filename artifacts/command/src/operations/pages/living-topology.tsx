...
function initNodes(w: number, h: number): NodeState[] {
  ...
  return PLATFORM_NODES.map((n) => ({
    ...
    health: (['healthy', 'healthy', 'healthy', 'degraded', 'critical'] as const)[
          ...
          const healthScore = m
            ? m.health === 'healthy'
              ...
              : m.health === 'degraded'
                ? 45 + Math.random() * 25
                : 15 + Math.random() * 25
            : n.healthScore + (Math.random() - 0.5) * 5;
      ...
      {path.map((n, i) => {
        ...
        return (
          <div key={n.id} className="flex items-center gap-2 text-[10px]">
            <div className="w-28 text-right truncate font-mono" style={{ color: colors[i] }}>
              {n.label}
            </div>
            ...
            <div className="w-12 text-right font-mono" style={{ color: colors[i] }}>
              {duration}ms
            </div>
          </div>
        );
...
export default function LivingTopology() {
  ...
  const draw = useCallback(() => {
    ...
    for (const node of nodeArr) {
      ...
      const pulseFactor =
        0.5 + 0.5 * Math.sin(t * (node.health === 'critical' ? 4 : 1.5) + node.pulse);
      ...
      if (node.ripple > 0) {
        for (let i = 1; i <= 3; i++) {
          ...
          ctx.strokeStyle = `${hc}${Math.round(alpha * 255)
            .toString(16)
            .padStart(2, '0')}`;
          ctx.lineWidth = 1;
          ctx.stroke();
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span
              ...
              style={{ background: 'rgba(212,160,84,0.15)', color: '#d4a054' }}
            >
              LIVE
            </span>
          </div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>
            Animated force-directed mesh — services pulse with health, traffic flows as particles,
            anomalies ripple outward.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            style={{
              color: showTrace ? '#d4a054' : 'rgba(255,255,255,0.4)',
              borderColor: showTrace ? 'rgba(212,160,84,0.3)' : 'rgba(255,255,255,0.1)',
              background: showTrace ? 'rgba(212,160,84,0.08)' : 'transparent',
            }}
          >
      ...
      <div className="flex gap-4">
        ...
        {selectedNode && (
          <div
            className="w-64 shrink-0 rounded-xl border p-4 h-fit space-y-4"
            style={{
              borderColor: `${PLATFORM_COLORS[selectedNode.platform] ?? '#d4a054'}20`,
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <div
                  ...
                  style={{ color: PLATFORM_COLORS[selectedNode.platform] ?? '#d4a054' }}
                >
            ...
            <div className="space-y-1">
              {[
                {
                  label: 'Health Score',
                  value: `${selectedNode.healthScore.toFixed(0)}`,
                  color: HEALTH_COLORS[selectedNode.health],
                },
                {
                  label: 'Latency',
                  value: `${selectedNode.latencyMs.toFixed(0)}ms`,
                  color: selectedNode.latencyMs > 150 ? '#d4a054' : '#6b8f71',
                },
                {
                  label: 'Error Rate',
                  value: `${selectedNode.errorRate.toFixed(1)}%`,
                  color: selectedNode.errorRate > 2 ? '#c45a4a' : '#6b8f71',
                },
                {
                  label: 'Throughput',
                  value: `${selectedNode.throughput.toFixed(0)}/s`,
                  color: '#4B8BDB',
                },
              ].map((m) => (
                <div
                ...
                >
                  ...
                  <span className="text-[11px] font-mono font-bold" style={{ color: m.color }}>
                    {m.value}
                  </span>
                </div>
              ))}
            ...
            {selectedNode.anomaly && (
              ...
              >
                ...
                <span className="text-[10px] text-[#c45a4a]">
                  Anomaly detected — ripple propagating
                </span>
              </div>
            )}
                ...
                selectedNode.connects.map((id) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full flex items-center gap-2 text-[10px] py-1 hover:opacity-80 text-left"
                    >
                      <span
                        className="w-1.5 h-1.5 rounded-full"
                        style={{ background: HEALTH_COLORS[cn.health] }}
                      />
                      <span style={{ color: PLATFORM_COLORS[cn.platform] ?? '#d4a054' }}>
                        {cn.label}
                      </span>
                      <span
      ...
      <div className="grid grid-cols-3 gap-3">
        {nodes
          ...
          .concat(nodes.filter((n) => n.anomaly))
          .slice(0, 6)
          .map((n) => (
            <div
              key={n.id}
              ...
              style={{
                borderColor: `${HEALTH_COLORS[n.health]}20`,
                background: `${HEALTH_COLORS[n.health]}08`,
              }}
            >
              <span
                className="w-2 h-2 rounded-full mt-1 animate-pulse shrink-0"
                style={{ background: HEALTH_COLORS[n.health] }}
              />
              <div className="min-w-0">
                ...
                <div
                  ...
                  style={{ color: 'rgba(255,255,255,0.3)' }}
                >
                  {n.latencyMs.toFixed(0)}ms · {n.errorRate.toFixed(1)}% err
                </div>
              </div>
            </div>
          ))}
