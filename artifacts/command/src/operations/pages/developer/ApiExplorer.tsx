...
const ENDPOINTS: Endpoint[] = [
  { method: "GET", path: "/api/health", summary: "Basic API health check", tag: "Platform", auth: false },
  ...
  { method: "GET", path: "/api/vessels/health", summary: "Vessels maritime platform health", tag: "Maritime", auth: false },
  ...
  { method: "DELETE", path: "/api/developer/api-keys/:id", summary: "Revoke an API key", tag: "Developer", auth: true, params: [{ name: "id", in: "path", type: "integer", required: true }] },
  { method: "GET", path: "/api/developer/webhooks", summary: "List registered webhooks", tag: "Developer", auth: true },
  { method: "POST", path: "/api/developer/webhooks", summary: "Register a webhook endpoint", tag: "Developer", auth: true, body: '{\n  "url": "https://example.com/webhook",\n  "events": ["aegis.threat.detected"],\n  "description": "Security alerts"\n}' },
  { method: "GET", path: "/api/developer/webhook-events", summary: "List available webhook event types", tag: "Developer", auth: false },
  { method: "GET", path: "/api/developer/scopes", summary: "List API scopes and permissions", tag: "Developer", auth: false },
];
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-text-secondary">Interactive documentation for all SZL Holdings API endpoints.</p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {tags.map((tag) => (
          <button
            key={tag}
            onClick={() => { setFilterTag(tag); setExpandedTags(new Set([tag])); }}
        ...
        <div className="lg:w-1/2 space-y-2">
          {Object.entries(grouped).map(([tag, endpoints]) => (
            <div key={tag} className="bg-surface rounded-xl border border-border overflow-hidden">
              ...
              >
                <span className={`text-xs font-medium px-2 py-0.5 rounded ${TAG_COLORS[tag]}`}>{tag}</span>
                {expandedTags.has(tag) ? <ChevronDown className="w-4 h-4 text-text-muted" /> : <ChevronRight className="w-4 h-4 text-text-muted" />}
              </button>
              {expandedTags.has(tag) && (
                <div className="border-t border-border">
                  {endpoints.map((ep, i) => (
                    ...
                    >
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${METHOD_COLORS[ep.method]} min-w-[52px] text-center`}>
                        {ep.method}
                      </span>
          ...
          {selectedEndpoint ? (
            <div className="bg-surface rounded-xl border border-border p-5 space-y-4 sticky top-4">
              <div className="flex items-center gap-3">
                <span className={`text-xs font-bold px-2 py-1 rounded ${METHOD_COLORS[selectedEndpoint.method]}`}>
              ...
              <div className="flex items-center gap-2">
                <span className={`text-xs px-2 py-0.5 rounded ${TAG_COLORS[selectedEndpoint.tag]}`}>{selectedEndpoint.tag}</span>
                {selectedEndpoint.auth && (
                  <span className="text-xs px-2 py-0.5 rounded bg-accent/10 text-accent">Requires Auth</span>
                )}
              </div>
              ...
              {selectedEndpoint.body && (
                <div>
                  ...
                  <pre className="text-xs"><code>{selectedEndpoint.body}</code></pre>
                </div>
              )}
              ...
              {tryItResponse && (
                <div>
                  ...
                  <pre className="text-xs max-h-96 overflow-auto"><code>{tryItResponse}</code></pre>
                </div>
              )}
            </div>
          ) : (
            <div className="bg-surface rounded-xl border border-border p-8 text-center">
              ...
              <p className="text-text-secondary">Select an endpoint to view details and try it live</p>
            </div>
          )}
        </div>
      </div>
...
function Code2Icon(props: { className?: string }) {
  return <svg className={props.className} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M16 18l6-6-6-6M8 6l-6 6 6 6"/></svg>;
}
