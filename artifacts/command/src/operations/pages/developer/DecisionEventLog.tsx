import { Activity, CheckCircle2, Clock, Filter, RefreshCw, Webhook, XCircle } from 'lucide-react';
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [deliveryData, endpointData] = await Promise.all([
        apiFetch<{ data: WebhookDelivery[] }>(
          '/webhooks/deliveries?eventType=decision.*&limit=100',
        ),
      ...
      <div className="grid grid-cols-5 gap-2">
        {DECISION_EVENTS.map((ev) => (
          <div key={ev} className="bg-surface border border-border rounded-lg p-3 text-center">
            <p
              className={`text-xs font-mono px-2 py-0.5 rounded inline-block mb-1 ${EVENT_COLORS[ev] ?? 'text-text-muted'}`}
            >
              {ev.split('.')[1]}
            </p>
            ...
            {filtered.map((delivery) => {
              ...
              return (
                <div key={delivery.id} className="py-3 flex items-center gap-3 text-sm">
                  <StatusIcon
                    className={`w-4 h-4 flex-shrink-0 ${STATUS_COLORS[delivery.status] ?? 'text-text-muted'}`}
                  />
                  <span
                    className={`px-2 py-0.5 rounded text-xs font-mono flex-shrink-0 ${EVENT_COLORS[delivery.eventType] ?? 'text-text-muted bg-surface-elevated'}`}
                  >
                    {delivery.eventType}
                  </span>
        ...
        <div className="bg-surface border border-border rounded-xl p-6 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-text-secondary mb-3">
              ...
              <code className="text-accent font-mono text-xs">decision.*</code> to capture all
              lifecycle stages.
            </p>
          ...
          {[
            {
              ...
              desc: 'Fired when signals are evaluated and recommendations are generated.',
              endpoint: 'POST /api/decisioning/evaluate',
            },
            {
              event: 'decision.approved',
              desc: 'Fired when a decision is approved, either by policy or an explicit approver.',
              endpoint: 'POST /api/decisioning/execute',
            },
            {
              event: 'decision.executed',
              desc: 'Fired when a workflow run is recorded after execution.',
              endpoint: 'POST /api/decisioning/execute',
            },
            {
              event: 'decision.proved',
              desc: 'Fired when cryptographic or human proof is recorded for a run.',
              endpoint: 'POST /api/decisioning/runs/:runId/prove',
            },
            {
              event: 'decision.outcome_recorded',
              desc: 'Fired when the outcome of a decision run is logged.',
              endpoint: 'POST /api/decisioning/runs/:runId/outcome',
            },
          ].map((item) => (
            <div
              key={item.event}
            ...
            >
              <span
                className={`px-2 py-0.5 rounded text-xs font-mono flex-shrink-0 mt-0.5 ${EVENT_COLORS[item.event] ?? ''}`}
              >
                {item.event}
              </span>
