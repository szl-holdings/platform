...
function apdexToLabel(score: number): { label: string; color: string } {
  ...
  if (score >= 0.7) return { label: 'Fair', color: 'text-amber-400' };
  if (score >= 0.5) return { label: 'Poor', color: 'text-orange-400' };
...
function TraceWaterfall({ trace }: { trace: ServiceApmTrace }) {
  ...
  return (
    <div
      className={`rounded-lg border ${isError ? 'border-red-500/20' : 'border-border'} p-3 space-y-2`}
    >
      <div className="flex items-center gap-3 cursor-pointer" onClick={() => setOpen((o) => !o)}>
        <span
          className={`text-xs font-mono font-bold px-2 py-0.5 rounded ${isError ? 'bg-red-500/10 text-red-400' : 'bg-emerald-500/10 text-emerald-400'}`}
        >
          {trace.statusCode}
        </span>
        ...
        <span
          className={`text-xs font-mono font-bold ${trace.totalMs > 500 ? 'text-orange-400' : trace.totalMs > 200 ? 'text-amber-400' : 'text-emerald-400'}`}
        >
      ...
      {open && (
        <div className="pt-2 space-y-1.5 border-t border-border">
          ...
          <div className="space-y-1">
            {trace.spans.map((span, i) => (
              <div key={i} className="flex items-center gap-3 text-[10px]">
                <div
                  className={`w-2 h-2 rounded-sm shrink-0 ${SPAN_COLORS[span.type] ?? 'bg-slate-500'}`}
                />
                ...
                <span
                  className={`ml-auto font-mono ${span.durationMs > 100 ? 'text-amber-400' : 'text-muted-foreground'}`}
                >
          ...
          <div className="flex gap-2 pt-1">
            ...
            {trace.userId && (
              <span className="text-[9px] text-muted-foreground">User: {trace.userId}</span>
            )}
...
export default function ObservabilityPage() {
  ...
  const [selectedTab, setSelectedTab] = useState<'golden' | 'traces' | 'heatmap' | 'deployments'>(
    'golden',
  );
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {latestSnapshot
          ? [
              {
                label: 'Latency P95',
                value: formatMs(latestSnapshot.latencyP95),
                color: latestSnapshot.latencyP95 > 500 ? 'text-orange-400' : 'text-emerald-400',
              },
              {
                label: 'Throughput',
                value: `${latestSnapshot.throughput.toFixed(0)} req/s`,
                color: 'text-cyan-400',
              },
              {
                label: 'Error Rate',
                value: `${latestSnapshot.errorRate.toFixed(2)}%`,
                color: latestSnapshot.errorRate > 2 ? 'text-red-400' : 'text-emerald-400',
              },
              {
                label: 'Saturation',
                value: `${latestSnapshot.saturation.toFixed(0)}%`,
                color: latestSnapshot.saturation > 85 ? 'text-orange-400' : 'text-slate-300',
              },
      ...
      {selectedTab === 'golden' && (
        <div className="space-y-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">
                Latency Percentiles — {selectedService} (1h)
              </CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <LineChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                  ...
                  {deployMarkerTimes.map((t, i) => (
                    <ReferenceLine
                      ...
                      stroke="#6366f1"
                      strokeDasharray="4 2"
                      label={{ value: 'deploy', fill: '#6366f1', fontSize: 9 }}
                    />
                  ))}
                  <Line
                    type="monotone"
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">Apdex History — {selectedService}</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={100}>
                <AreaChart data={chartData} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(v: number) => [v.toFixed(3), 'Apdex']}
                  />
                  <ReferenceLine
                    y={0.94}
                    stroke="#10b981"
      ...
      {selectedTab === 'traces' && (
        <div className="space-y-3">
          <div className="flex items-center gap-3 text-xs text-muted-foreground">
            <span>
              {serviceTraces.length} traces — {selectedService}
            </span>
            <span>·</span>
          ...
          <div className="flex flex-wrap gap-3 mb-2">
            {[
              ...
              {
                label: 'Errors',
                value: serviceTraces.filter((t) => t.statusCode >= 400).length,
                color: 'text-red-400',
              },
              {
                label: 'Slow (>500ms)',
              ...
              },
              {
                label: 'Avg Latency',
                value: formatMs(
                  serviceTraces.reduce((s, t) => s + t.totalMs, 0) / (serviceTraces.length || 1),
                ),
                color: 'text-cyan-400',
              },
            ].map(({ label, value, color }) => (
                ...
                <div key={svc} className="bg-card border border-border rounded-xl p-3">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {cells.length} endpoints monitored
                    </span>
                  </div>
      ...
      {selectedTab === 'deployments' && (
        <div className="space-y-4">
          <div className="space-y-3">
            {SERVICES.flatMap(
              (svc) => deploymentMarkers[svc]?.map((d) => ({ ...d, _service: svc })) ?? [],
            )
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Deployed by: <span className="text-foreground">{d.triggeredBy}</span>
                      </p>
                      <div className="flex gap-4 mt-2 text-xs">
                        <span
                          className={
                            d.metricShift.latencyDelta < 0 ? 'text-emerald-400' : 'text-red-400'
                          }
                        >
                          {d.metricShift.latencyDelta > 0 ? '+' : ''}
                          {d.metricShift.latencyDelta.toFixed(0)}ms latency
                        </span>
                        <span
                          className={
                            d.metricShift.errorRateDelta < 0 ? 'text-emerald-400' : 'text-red-400'
                          }
                        >
                          {d.metricShift.errorRateDelta > 0 ? '+' : ''}
                          {d.metricShift.errorRateDelta.toFixed(2)}% error rate
                        </span>
                        <span
                          className={
                            d.metricShift.throughputDelta > 0
                              ? 'text-emerald-400'
                              : 'text-amber-400'
                          }
                        >
                          {d.metricShift.throughputDelta > 0 ? '+' : ''}
                          {d.metricShift.throughputDelta.toFixed(0)} req/s throughput
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-3 gap-2 text-[10px] text-muted-foreground">
                <div>
                  <p className="font-medium text-foreground">
                    {slo.errorBudgetConsumedPct.toFixed(1)}%
                  </p>
                  <p>Budget used</p>
                </div>
                <div>
                  <p
                    className={`font-medium ${slo.burnRate1h > 1.5 ? 'text-red-400' : 'text-foreground'}`}
                  >
