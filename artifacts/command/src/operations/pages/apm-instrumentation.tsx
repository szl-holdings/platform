...
function RouteRow({ r }: { r: RouteBreakdown }) {
  const p99Color =
    r.p99 > 500 ? 'text-[#c45a4a]' : r.p99 > 200 ? 'text-[#d4a054]' : 'text-[#6b8f71]';
  ...
  return (
    <tr className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
      <td className="py-2 pr-3 text-[11px] font-mono text-slate-300 truncate max-w-[180px]">
        {r.route}
      </td>
      <td className="py-2 pr-3 text-center text-[11px] font-mono text-slate-400">{r.count}</td>
      <td className="py-2 pr-3 text-center text-[11px] font-mono text-slate-400">
        {r.avgTotal.toFixed(0)}ms
      </td>
      ...
      <td className={cn('py-2 text-center text-[11px] font-mono font-semibold', p99Color)}>
        {r.p99.toFixed(0)}ms
      </td>
    </tr>
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Real latency breakdowns from OpenTelemetry spans — DB query time, external API time, and
            application time per route
          </p>
        </div>
      ...
      {!loading && bd && (
        <>
          <div className="grid grid-cols-4 gap-3">
            ...
            <MetricCard
              label="p95 Latency"
              value={bd.overallP95.toFixed(0)}
              unit="ms"
              icon={TrendingUp}
              color={bd.overallP95 > 300 ? 'text-[#d4a054]' : 'text-[#6b8f71]'}
            />
            <MetricCard
              label="p99 Latency"
              value={bd.overallP99.toFixed(0)}
              unit="ms"
              icon={TrendingUp}
              color={
                bd.overallP99 > 500
          ...
          <div className="rounded-xl border border-white/5 bg-white/[0.02] p-5 space-y-4">
            <div className="text-[11px] text-slate-400 uppercase tracking-wide font-semibold">
              Time Allocation Breakdown (avg across all instrumented routes)
            </div>
            <div className="space-y-3">
              <FractionBar label="DB Query Time" pct={bd.avgDbFraction} color="bg-[#4a90b8]" />
              <FractionBar
                label="External API Time"
                pct={bd.avgExternalFraction}
                color="bg-[#8b7ac8]"
              />
              <FractionBar
                label="Application + Serialization"
                pct={appFraction}
                color="bg-[#6b8f71]"
              />
            </div>
            <div className="grid grid-cols-3 gap-3 pt-2 border-t border-white/5 text-center">
              <div>
                ...
                <div className="font-mono text-sm text-[#4a90b8]">
                  {bd.avgDbFraction.toFixed(1)}%
                </div>
              </div>
              <div>
                <div className="flex items-center justify-center gap-1.5 text-[10px] text-slate-500 uppercase tracking-wide mb-1">
                ...
                <div className="font-mono text-sm text-[#8b7ac8]">
                  {bd.avgExternalFraction.toFixed(1)}%
                </div>
              </div>
              <div>
                <div className="flex items-center justify-center gap-1.5 text-[10px] text-slate-500 uppercase tracking-wide mb-1">
          ...
          {routes.length > 0 && (
            <div className="rounded-xl border border-white/5 bg-white/[0.02] p-5">
              <div className="text-[11px] text-slate-400 uppercase tracking-wide font-semibold mb-4">
                Route Latency Breakdown — Top {Math.min(routes.length, 20)} by avg total
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/10">
                      <th className="pb-2 text-left text-[10px] text-slate-500 uppercase tracking-wide pr-3">
                        Route
                      </th>
                      <th className="pb-2 text-center text-[10px] text-slate-500 uppercase tracking-wide pr-3">
                        Reqs
                      </th>
                      <th className="pb-2 text-center text-[10px] text-slate-500 uppercase tracking-wide pr-3">
                        Avg
                      </th>
                      <th className="pb-2 text-center text-[10px] text-[#4a90b8]/70 uppercase tracking-wide pr-3">
                        DB%
                      </th>
                      <th className="pb-2 text-center text-[10px] text-[#8b7ac8]/70 uppercase tracking-wide pr-3">
                        Ext%
                      </th>
                      <th className="pb-2 text-center text-[10px] text-slate-500 uppercase tracking-wide">
                        p99
                      </th>
                    </tr>
                  </thead>
          ...
          {routes.length === 0 && extCalls.length === 0 && (
            <div className="rounded-xl border border-white/5 bg-white/[0.02] p-8 text-center">
              ...
              <div className="text-sm text-slate-400">
                No APM spans recorded in the 5-minute window.
              </div>
              <div className="text-[11px] text-slate-600 mt-1">
                Navigate other pages to generate traces, then refresh to see live data.
              </div>
            </div>
          )}
        </>
