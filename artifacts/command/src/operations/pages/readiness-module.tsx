...
const DEMO_READINESS = {
  items: [
    { id: 1, title: "Security Review Sign-off", itemType: "launch_gate", status: "blocked", owner: "Riley Torres", description: "Awaiting infosec review of payment integration. CVE-2024-3891 flagged.", dueAt: new Date(Date.now() + 3 * 86400000).toISOString() },
    { id: 2, title: "Load Testing — 10K concurrent users", itemType: "launch_gate", status: "in_progress", owner: "Sam Park", description: "Testing in progress on staging cluster. Results expected in 48h.", dueAt: new Date(Date.now() + 2 * 86400000).toISOString() },
    { id: 3, title: "Legal Clearance — Data Processing Agreement", itemType: "launch_gate", status: "not_started", owner: "Morgan Lee", description: "DPA with three EU vendors not yet signed.", dueAt: new Date(Date.now() + 7 * 86400000).toISOString() },
    { id: 4, title: "Customer Success Handoff Checklist", itemType: "milestone", status: "complete", owner: "Alex Chen", description: "All CS onboarding materials reviewed and approved.", dueAt: null },
    { id: 5, title: "Stripe Integration — PCI DSS Attestation", itemType: "blocker", status: "blocked", owner: "Jordan Alvarez", description: "PCI DSS SAQ-A must be submitted before processing live payments.", dueAt: new Date(Date.now() + 1 * 86400000).toISOString() },
    { id: 6, title: "Feature Flag Rollout Plan", itemType: "dependency", status: "complete", owner: "Sam Park", description: "Progressive rollout plan confirmed. Feature flags configured in admin panel.", dueAt: null },
    { id: 7, title: "Executive Sponsor Sign-off", itemType: "owner_check", status: "not_started", owner: "Sarah Kim", description: "Executive review of launch readiness deck pending.", dueAt: new Date(Date.now() + 4 * 86400000).toISOString() },
    { id: 8, title: "Runbook — Incident Response During Launch", itemType: "dependency", status: "in_progress", owner: "Riley Torres", description: "On-call runbook for launch window being finalized.", dueAt: new Date(Date.now() + 2 * 86400000).toISOString() },
  ],
  ...
  return (
    <div className="flex flex-col gap-6 p-6 min-h-full">
      ...
      <div className="space-y-3">
        {items.map((item: any) => {
          const typeInfo = ITEM_TYPE_CONFIG[item.itemType] || { label: item.itemType, icon: Package, color: "text-zinc-400" };
          const statusInfo = STATUS_CONFIG[item.status] || { label: item.status, color: "text-zinc-400", bg: "bg-zinc-800" };
          const TypeIcon = typeInfo.icon;
          const dueText = formatDate(item.dueAt);
          const isOverdue = item.dueAt && new Date(item.dueAt) < new Date() && item.status !== "complete";
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <Badge variant="outline" className={statusInfo.bg + " " + statusInfo.color + " text-[10px]"}>
                        {statusInfo.label}
                      </Badge>
