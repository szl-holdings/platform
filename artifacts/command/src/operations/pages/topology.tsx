...
const services: ServiceNode[] = [
  {
    ...
    latency: '12ms',
    load: 45,
    connections: ['svc-auth', 'svc-core', 'svc-data'],
  },
  {
    id: 'svc-auth',
    name: 'Auth Service',
    ...
    latency: '8ms',
    load: 32,
    connections: ['db-users', 'cache-sessions'],
  },
  {
    id: 'svc-core',
    name: 'Core Engine',
    ...
    latency: '45ms',
    load: 89,
    connections: ['db-main', 'queue-events', 'svc-data'],
  },
  {
    id: 'svc-data',
    name: 'Data Pipeline',
    ...
    latency: '22ms',
    load: 61,
    connections: ['db-analytics', 'cache-results'],
  },
  {
    id: 'db-main',
    name: 'Primary DB',
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          System dependency mapping and cross-service navigation
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <div className="rounded-xl border border-border bg-card p-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-xl font-bold">{degradedCount}</div>
        ...
        <div className="rounded-xl border border-border bg-card p-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-xl font-bold">{regions.length}</div>
        ...
        <div key={region} className="rounded-xl border border-border bg-card">
          <div className="p-4 border-b border-border flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">
              ({services.filter((s) => s.region === region).length} services)
            </span>
          </div>
            ...
            {services
              ...
              .map((svc) => {
                ...
                return (
                  <div
                    key={svc.id}
                    className={`rounded-lg border p-4 ${st.bg} hover:bg-muted/30 transition-colors cursor-pointer`}
                  >
                    ...
                    <div className="grid grid-cols-3 gap-2 text-xs mb-2">
                      <div>
                        ...
                        <span className="font-medium">{svc.latency}</span>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Load:</span>{' '}
                        <span
                          className={`font-medium ${svc.load > 80 ? 'text-[#c45a4a]' : svc.load > 60 ? 'text-[#d4a054]' : 'text-[#6b8f71]'}`}
                        >
                          {svc.load}%
                        </span>
                      </div>
                      <div>
                        ...
                        <span className="font-medium capitalize">{svc.type}</span>
                      </div>
                    </div>
                    {svc.connections.length > 0 && (
                      <div className="flex items-center gap-1 text-[10px] text-muted-foreground flex-wrap">
                        ...
                        {svc.connections.map((c) => (
                          <span key={c} className="px-1.5 py-0.5 rounded bg-muted">
                            {c}
                          </span>
                        ))}
                      </div>
