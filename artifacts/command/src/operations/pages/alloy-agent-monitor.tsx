...
const REGISTERED_AGENTS: Agent[] = [
  ...
  {
    id: 'ownership-validator',
    name: 'Ownership Validator',
    status: 'idle',
    type: 'Ownership Intelligence',
    uptime: '14d 6h',
    runsToday: 24,
    avgDuration: '2.1s',
    lastRun: '5m ago',
        ...
        <div className="flex items-center gap-3 min-w-0">
          <div
            ...
            style={{
              ...
              animationPlayState: agent.status === 'running' ? 'running' : 'paused',
            }}
          />
...
function AgentDetail({ agent }: { agent: Agent }) {
  ...
  const metrics: MetricRow[] = [
    ...
    { label: 'Last run', value: agent.lastRun, color: TEXT.secondary },
    {
      ...
      color: agent.successRate > 95 ? '#22c55e' : '#f97316',
    },
    {
      label: 'Active findings',
      value: String(agent.findings),
      color: agent.findings > 0 ? ACCENT : TEXT.secondary,
    },
    {
      label: 'Pending actions',
      value: String(agent.pendingActions),
      color: agent.pendingActions > 0 ? '#f97316' : TEXT.secondary,
    },
  ];
  ...
  const modelConfig: Array<{ label: string; value: string }> = [
    ...
    {
      label: 'Tokens today',
      value: agent.tokens > 0 ? agent.tokens.toLocaleString() : 'N/A (internal)',
    },
    ...
    <div className="h-full overflow-y-auto" style={{ background: BG.surface }}>
      <div className="px-5 py-4" style={{ borderBottom: `1px solid ${BORDER.subtle}` }}>
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[13px] font-semibold mb-0.5" style={{ color: TEXT.primary }}>
              {agent.name}
            </p>
      ...
      <div className="px-4 py-4 grid grid-cols-2 gap-3">
        {metrics.map((m) => (
          ...
          >
            ...
            <p className="text-[12px] font-semibold font-mono" style={{ color: m.color }}>
              {m.value}
            </p>
          </div>
        ))}
            ...
            >
              <div className="flex items-center gap-2">
                <div
                  ...
                  style={{ background: run.outcome === 'ok' ? '#22c55e' : '#f97316' }}
                />
              ...
              <div className="flex items-center gap-4">
                ...
                <span
                  className="text-[11px]"
                  style={{ color: run.findings > 0 ? ACCENT : TEXT.muted }}
                >
...
export default function AlloyAgentMonitorPage() {
  ...
  const filtered =
    filter === 'all' ? REGISTERED_AGENTS : REGISTERED_AGENTS.filter((a) => a.status === filter);
      ...
      >
        ...
        <div className="flex gap-2">
          {filterOptions.map((f) => (
            ...
            >
              {f}
              {f !== 'all' && ` (${statusCounts[f as AgentStatus]})`}
            </button>
          ))}
        </div>
      </div>
          ...
          >
            <div className="flex gap-4 text-[11px]">
              ...
              {statusCounts.degraded > 0 && (
                <span style={{ color: '#f97316' }}>{statusCounts.degraded} degraded</span>
              )}
            </div>
          </div>
          {filtered.map((agent) => (
