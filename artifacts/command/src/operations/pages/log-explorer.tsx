...
export default function LogExplorer() {
  ...
  const filtered = logs.filter((log) => {
    ...
    const matchSearch =
      ...
      log.service.includes(search.toLowerCase()) ||
      (log.traceId ?? '').includes(search.toLowerCase());
    ...
    <div className="h-full flex flex-col overflow-hidden" style={{ background: '#080f1c' }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] text-white/30">
              Structured log search · faceted filtering · live tail
            </p>
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          <span className="text-[9px] text-white/15 font-mono border-l border-white/10 pl-2 ml-1">
            ⌘K
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Facets sidebar */}
        <div
          className="w-[200px] shrink-0 border-r overflow-y-auto p-3 space-y-4"
          style={{ borderColor: 'rgba(212,160,84,0.08)' }}
        >
          {/* Level facets */}
          <div>
            <p className="text-[9px] font-medium text-white/25 uppercase tracking-wider mb-2">
              Level
            </p>
          ...
          {filtered.map((log, _idx) => {
            ...
            return (
              <div key={log.id}>
                ...
                >
                  <span className="text-white/20 shrink-0 w-24 text-[10px]">
                    {timeStr}.{msStr}
                  </span>
                  <span className="shrink-0 w-12 text-[9px] font-bold" style={{ color: cfg.color }}>
                    {cfg.label}
                  </span>
                  <span className="shrink-0 w-24 text-[10px] truncate" style={{ color: svcColor }}>
                    {log.service}
                  </span>
                  ...
                  {log.statusCode && log.statusCode >= 500 && (
                    <span className="shrink-0 text-[9px] text-red-400 font-bold">
                      {log.statusCode}
                    </span>
                  )}
                </div>
                {isSelected && (
                  ...
                  >
                    ...
                    >
                      <p className="text-[10px] text-white/60 leading-relaxed whitespace-pre-wrap font-mono">
                        {log.message}
                      </p>
                    </div>
                    <div className="flex items-center gap-3 mt-2">
                      <button className="text-[9px] text-white/30 hover:text-white/60 transition-colors">
                        Copy trace ID
                      </button>
                      <button className="text-[9px] text-white/30 hover:text-white/60 transition-colors">
                        Filter by service
