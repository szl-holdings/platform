import { demoIntegrations } from '@lyte/lib/demo-seed';
import { AlertTriangle, CheckCircle, Clock, Network, RefreshCw, XCircle, Zap } from 'lucide-react';
...
export default function DemoIntegrationsPage() {
  ...
  const filtered =
    statusFilter === 'all'
      ? demoIntegrations
      : demoIntegrations.filter((i) => i.status === statusFilter);
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: TEXT.secondary }}>
            All connected data sources with sync health and signal generation status
          </p>
        </div>
        <button
          ...
          return (
            <div
              ...
              style={{
                ...
                border: `1px solid ${intg.status === 'disconnected' ? 'rgba(196,90,74,0.12)' : intg.status === 'degraded' ? 'rgba(200,149,60,0.12)' : BORDER.subtle}`,
              }}
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  >
                    {TYPE_LABELS[intg.type]}
                  </span>
                </div>
                <p className="text-[9px] leading-snug" style={{ color: TEXT.muted }}>
                  {intg.description}
                </p>
              </div>
              <div className="grid grid-cols-4 gap-4 shrink-0">
                <div className="text-center">
                  <div
                    ...
                    style={{ color: intg.status === 'connected' ? TEXT.primary : TEXT.muted }}
                  >
                    {fmt(intg.recordsIngested)}
                  </div>
                  <div
                ...
                <div className="text-center">
                  <div
                    ...
                    style={{ color: hasErrors ? '#c45a4a' : '#6b8f71' }}
                  >
                    {hasErrors ? `${intg.errorsLast24h} errors` : 'Clean'}
                  </div>
                  <div
                    className="text-[7px] uppercase tracking-wider"
                    style={{ color: TEXT.muted }}
