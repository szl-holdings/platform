...
function StepRow({ step }: { step: RemediationStep }) {
  ...
  return (
    <div className="flex items-center gap-2 text-[10px]">
      ...
      <span style={{ color: step.status === 'pending' ? DS.text.muted : DS.text.secondary }}>
        {step.action}
      </span>
      {step.durationMs && (
      ...
      {step.status === 'running' && (
        <span className="ml-auto text-[8px] animate-pulse" style={{ color: '#f59e0b' }}>
          RUNNING
        </span>
      )}
    </div>
...
function RunCard({ run }: { run: RemediationRun }) {
  const [expanded, setExpanded] = useState(
    run.status === 'executing' || run.status === 'pending_approval',
  );
    ...
    <div className="rounded-xl border" style={{ borderColor: `${tc}20`, background: `${tc}03` }}>
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[10px] mb-1" style={{ color: DS.text.muted }}>
              {run.triggerSignal}
            </div>
            <div className="flex items-center gap-3 text-[9px]" style={{ color: DS.text.muted }}>
              ...
              <span>·</span>
              <span>
                MTTR saved: <span style={{ color: '#10b981' }}>~{run.mttrSavedMins}m</span>
              </span>
            </div>
          </div>
          <button
          ...
          <div className="mt-3">
            ...
            >
              <div
                ...
                style={{ width: `${progress}%`, background: tc }}
              />
            </div>
          </div>
        )}
        ...
        {run.status === 'pending_approval' && (
          ...
          >
            ...
            <span className="text-[10px]" style={{ color: '#8b5cf6' }}>
              Awaiting approval from <strong>{run.approver}</strong> — Counsel governance gate active
            </span>
          </div>
        )}
      </div>
      ...
      {expanded && (
        <div className="border-t px-4 py-3 space-y-1.5" style={{ borderColor: DS.border }}>
          ...
          <div
            className="pt-2 flex items-center justify-between text-[8px] font-mono"
            style={{ color: DS.text.muted, borderTop: `1px solid ${DS.border}` }}
          >
            <span>Audit ref: {run.auditRef}</span>
            <span>Counsel Governance · Immutable log</span>
          </div>
        </div>
      )}
    </div>
...
function PatternRow({
  ...
  onEdit: (p: FailurePattern) => void;
  onDelete: (p: FailurePattern) => void;
  onHistory: (p: FailurePattern) => void;
}) {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <div className="text-[11px] font-medium truncate" style={{ color: DS.text.primary }}>
            {p.name}
          </div>
          <span
            className="text-[8px] px-1 py-0.5 rounded uppercase font-mono shrink-0"
            style={{ background: `${tc}15`, color: tc }}
          >
            {p.type}
          </span>
        ...
        {p.lastEditedAt && (
          <div className="text-[9px] mt-0.5 truncate" style={{ color: DS.text.muted }}>
            <span style={{ color: DS.text.secondary }}>
              {p.lastEditedAction === 'create'
                ? 'Created'
                : p.lastEditedAction === 'delete'
                  ? 'Deleted'
                  : 'Edited'}
            </span>{' '}
            by <span style={{ color: DS.text.secondary }}>{p.lastEditedBy ?? 'system'}</span> ·{' '}
            {fmtAgo(p.lastEditedAt)}
          </div>
        )}
      </div>
      ...
      <div className="text-right shrink-0 hidden md:block">
        <div className="text-[10px] font-mono" style={{ color: GOLD }}>
          ~{p.avgMttrSavedMins}m
        </div>
        <div className="text-[8px]" style={{ color: DS.text.muted }}>
          avg MTTR saved
        </div>
...
function PatternEditorModal({
  ...
  const canSubmit =
    form.name.trim().length > 0 && form.trigger.trim().length > 0 && form.runbook.trim().length > 0;
  ...
  for (const k of keys) {
    if (
      ...
      out.push({
        ...
        to: (after as Record<string, unknown>)[k],
      });
    }
  }
  return out;
...
function PatternHistoryPanel({
  ...
  const historyQuery = useStandardQuery<{ entries: PatternHistoryEntry[] }>({
    ...
    queryFn: () =>
      apiFetch<{ entries: PatternHistoryEntry[] }>(`/self-healing/policies/${pattern.id}/history`),
  });
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-start justify-between mb-1">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="text-[13px] font-semibold truncate" style={{ color: DS.text.primary }}>
                Change history
              </h2>
            </div>
            <div className="text-[10px] truncate" style={{ color: DS.text.muted }}>
              {pattern.name}
            </div>
        ...
        <div
          className="text-[9px] uppercase tracking-widest mt-4 mb-2"
          style={{ color: DS.text.muted }}
        >
        ...
        {historyQuery.isLoading && (
          <div className="flex items-center justify-center h-24" style={{ color: DS.text.muted }}>
            <div
              ...
              style={{ borderColor: 'rgba(212,160,84,0.25)', borderTopColor: GOLD }}
            />
        ...
        {!historyQuery.isLoading && !historyQuery.isError && entries.length === 0 && (
          <div
            className="text-[10px] px-3 py-4 rounded-lg text-center"
            style={{
              background: DS.surface,
              color: DS.text.muted,
              border: `1px solid ${DS.border}`,
            ...
            return (
              ...
              >
                ...
                {e.actorEmail && e.actorEmail !== e.actor && (
                  <div className="text-[9px] mb-1 truncate" style={{ color: DS.text.muted }}>
                    {e.actorEmail}
                  </div>
                )}
                  ...
                  <div className="mt-2 space-y-1">
                    {diffs.map((d) => (
                      <div key={d.key} className="text-[9px]" style={{ color: DS.text.secondary }}>
                        ...
                        <span style={{ color: '#ef4444' }} className="line-through">
                          {String(d.from ?? '—')}
                        </span>
                        {' → '}
...
export default function SelfHealingPage() {
  ...
  const [editor, setEditor] = useState<
    { mode: 'create' } | { mode: 'edit'; pattern: FailurePattern } | null
  >(null);
  ...
  const handleDeletePattern = (p: FailurePattern) => {
    if (
      ...
      !window.confirm(`Delete pattern "${p.name}"? This cannot be undone.`)
    )
      return;
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span
              className="text-[8px] px-1.5 py-0.5 rounded font-mono uppercase tracking-wider animate-pulse"
              style={{
          ...
          <p className="text-[11px]" style={{ color: DS.text.muted }}>
            Pattern-matched auto-remediation with Counsel approval gates and immutable audit trails.
            MTTR: hours → seconds.
          </p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              {
                label: 'MTTR Saved Today',
                value: `${totalMttrSaved}m`,
                color: '#10b981',
                icon: Clock,
              },
              {
          ...
          <div className="flex gap-1 border-b" style={{ borderColor: DS.border }}>
            {(['runs', 'patterns'] as const).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className="text-[11px] px-4 py-2 capitalize transition-all"
                style={{
                  color: tab === t ? GOLD : DS.text.muted,
                  borderBottom: `2px solid ${tab === t ? GOLD : 'transparent'}`,
                }}
              >
                {t === 'runs' ? 'Remediation Runs' : 'Pattern Library'}
              </button>
            ))}
          </div>
              ...
              {!policiesQuery.isLoading && (
                <div className="flex items-center justify-between mb-3">
                  <div className="text-[10px]" style={{ color: DS.text.muted }}>
                    {policies.filter((p) => p.enabled).length} patterns active ·{' '}
                    {policies.filter((p) => !p.enabled).length} disabled
                  </div>
                  <button
                    onClick={() => {
                      setEditorError(null);
      ...
      {historyPattern && (
        <PatternHistoryPanel pattern={historyPattern} onClose={() => setHistoryPattern(null)} />
      )}
    </div>
  );
}
