    ...
    <div className="space-y-1">
      <div className="flex items-center justify-between text-[11px]">
        ...
        <div className="flex items-center gap-3">
          {stagnant > 0 && <span className="text-[#c45a4a] font-mono">{stagnant} stagnant</span>}
      ...
      <div className="relative h-2 bg-white/5 rounded-full overflow-hidden">
        ...
        />
        <div
          className="absolute inset-y-0 border-r border-white/20"
          style={{ left: `${Math.min((expected / (expected * 6)) * 100, 100)}%` }}
        />
      </div>
  ...
  return (
    <div
      className={cn(
        ...
        c.border,
        workflow.severity === 'critical' ? 'bg-[#c45a4a]/[0.03]' : 'bg-white/[0.02]',
      )}
    >
        ...
        <div className="flex items-start gap-3">
          <div
            className={cn(
              ...
              c.dot,
              workflow.severity === 'critical' && 'animate-pulse',
            )}
          />
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-3 mb-2">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span className={cn('font-mono font-bold text-sm', c.text)}>
                  {workflow.totalDwellDays}d
                </span>
            ...
            <div className="relative h-2 bg-white/5 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${Math.min((workflow.totalDwellDays / (workflow.expectedDays * 5)) * 100, 100)}%`,
                  opacity: 0.8,
                }}
              />
              <div
                ...
                style={{
                  left: `${Math.min((workflow.expectedDays / (workflow.expectedDays * 5)) * 100, 100)}%`,
                }}
              />
            </div>
      ...
      {expanded && (
        <div className="px-6 pb-5 border-t border-white/5 pt-4 space-y-4">
          <div>
            <div className="text-[10px] text-slate-500 uppercase tracking-wide mb-3">
              Stage Breakdown
            </div>
          ...
          <div className="p-3 rounded-lg border border-[#d4a054]/20 bg-[#d4a054]/5">
            <div className="text-[10px] font-mono text-[#d4a054] uppercase tracking-wide mb-1.5">
              Slowdown Analysis
            </div>
            <p className="text-[11px] text-slate-300 leading-relaxed">
              This workflow is running{' '}
              <strong className="text-white">{excessDays.toFixed(1)} days</strong> beyond target.
              The biggest bottleneck is{' '}
              <strong className="text-white">
                {
                  workflow.stages.reduce(
                    (max, s) =>
                      s.avgDwellDays - s.expectedDays > max.avgDwellDays - max.expectedDays
                        ? s
                        : max,
                    workflow.stages[0],
                  ).name
              ...
              . Total stagnant items:{' '}
              <strong className="text-white">
                {workflow.stages.reduce((sum, s) => sum + s.stagnatCount, 0)}
              </strong>
              .
            </p>
          </div>
...
export default function WorkflowLatencyPage() {
  ...
  const totalStagnant = workflowLatencies.reduce(
    (sum, wf) => sum + wf.stages.reduce((s2, st) => s2 + st.stagnatCount, 0),
    0,
  );
  const totalVaR = workflowLatencies.reduce((sum, wf) => sum + wf.valueAtRisk, 0);
  const avgLatencyRatio =
    workflowLatencies.reduce((sum, wf) => sum + wf.totalDwellDays / wf.expectedDays, 0) /
    workflowLatencies.length;
    ...
    <div className="max-w-[960px] space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">
          Approval delay, stage dwell time, task aging, and queue congestion
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          {
            label: 'Avg Latency Ratio',
            value: `${avgLatencyRatio.toFixed(1)}x`,
            color: 'text-[#c8953c]',
          },
          {
            label: 'Total Value at Risk',
            value: formatCurrency(totalVaR),
            color: 'text-[#d4a054]',
          },
        ].map((stat) => (
