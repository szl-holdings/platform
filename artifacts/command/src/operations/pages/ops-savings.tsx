...
const ACTIONS: AutomationAction[] = [
  ...
  },
  {
    ...
    description: 'Counsel auto-classifies and routes alerts to correct owner',
    count: 2840,
...
const MONTHLY_REPORTS: MonthlyReport[] = [
  {
    month: "Nov '24",
    laborSaved: 142800,
    incidentCostSaved: 88200,
    downtimeSaved: 34100,
    totalValue: 265100,
    automationActions: 14820,
  },
  {
    month: "Dec '24",
    laborSaved: 156400,
    incidentCostSaved: 92400,
    downtimeSaved: 38800,
    totalValue: 287600,
    automationActions: 16240,
  },
  {
    month: "Jan '25",
    laborSaved: 138200,
    incidentCostSaved: 78800,
    downtimeSaved: 29400,
    totalValue: 246400,
    automationActions: 13980,
  },
  {
    month: "Feb '25",
    laborSaved: 168400,
    incidentCostSaved: 104200,
    downtimeSaved: 44800,
    totalValue: 317400,
    automationActions: 17640,
  },
  {
    month: "Mar '25",
    laborSaved: 182800,
    incidentCostSaved: 118400,
    downtimeSaved: 52200,
    totalValue: 353400,
    automationActions: 19280,
  },
  {
    month: "Apr '25",
    laborSaved: 197400,
    incidentCostSaved: 132800,
    downtimeSaved: 58400,
    totalValue: 388600,
    automationActions: 21140,
  },
];
...
export default function OpsSavingsPage() {
  const [selectedCategory, setSelectedCategory] = useState<AutomationAction['category'] | 'all'>(
    'all',
  );
  const [activeReport, setActiveReport] = useState<MonthlyReport>(
    MONTHLY_REPORTS[MONTHLY_REPORTS.length - 1]!,
  );
  ...
  const avgMttrReduction = Math.round(
    ACTIONS.filter((a) => a.mttrReductionMins > 0).reduce((s, a) => s + a.mttrReductionMins, 0) /
      ACTIONS.filter((a) => a.mttrReductionMins > 0).length,
  );
    ...
    <div className="p-4 md:p-6 max-w-7xl space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-[15px] font-bold" style={{ color: DS.text.primary }}>
              Ops Savings Calculator
            </h1>
          </div>
          <p className="text-[11px]" style={{ color: DS.text.muted }}>
            Continuously tracking automation value — labor hours, incident costs, and downtime
            prevented. Your monthly ROI proof.
          </p>
        </div>
        <button
          className="flex items-center gap-1.5 text-[10px] px-3 py-1.5 rounded-lg transition-all hover:opacity-80"
          style={{
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          {
            label: 'Dollar Value Delivered',
            value: fmt$(totalDollars),
            color: '#10b981',
            icon: DollarSign,
          },
          {
            label: 'Automation Actions (YTD)',
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div className="text-[10px] font-medium" style={{ color: DS.text.secondary }}>
            Monthly Value Delivered — Last 6 Months
          </div>
          <div className="flex gap-3 text-[8px]" style={{ color: DS.text.muted }}>
            <span className="flex items-center gap-1">
              <span className="w-2 h-1 rounded inline-block" style={{ background: GOLD }} />
              Labor
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-1 rounded inline-block" style={{ background: '#10b981' }} />
              Incident cost
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-1 rounded inline-block" style={{ background: '#8b5cf6' }} />
              Downtime
            </span>
          </div>
        </div>
          ...
          {MONTHLY_REPORTS.map((r) => {
            ...
            return (
              ...
              >
                ...
                <div
                  style={{
                    height: `${(incidentPct / totalPct) * 100}%`,
                    background: '#10b98160',
                    minHeight: 4,
                  }}
                />
                <div
                  style={{
                    height: `${(downtimePct / totalPct) * 100}%`,
                    background: '#8b5cf660',
                    minHeight: 4,
                  }}
                />
        ...
        <div className="flex gap-2 mt-1">
          {MONTHLY_REPORTS.map((r) => (
            <div
              ...
              style={{ color: activeReport.month === r.month ? GOLD : DS.text.muted }}
            >
              {r.month}
            </div>
          ))}
        ...
        >
          <div>
            <div
              ...
              style={{ color: DS.text.muted }}
            >
              Labor Saved — {activeReport.month}
            </div>
            <div className="text-[16px] font-bold font-mono" style={{ color: GOLD }}>
              {fmt$(activeReport.laborSaved)}
            </div>
          </div>
          <div>
            <div
            ...
            <div className="text-[16px] font-bold font-mono" style={{ color: '#10b981' }}>
              {fmt$(activeReport.incidentCostSaved)}
            </div>
          </div>
          <div>
            <div
            ...
            <div className="text-[16px] font-bold font-mono" style={{ color: '#8b5cf6' }}>
              {fmt$(activeReport.downtimeSaved)}
            </div>
          </div>
        </div>
        <div
          className="mt-3 p-3 rounded-lg flex items-center justify-between"
          style={{ background: 'rgba(212,160,84,0.05)', border: '1px solid rgba(212,160,84,0.12)' }}
        >
          <span className="text-[10px]" style={{ color: DS.text.secondary }}>
            Total Value Delivered — {activeReport.month}
          </span>
          <span className="text-[16px] font-bold font-mono" style={{ color: GOLD }}>
            {fmt$(activeReport.totalValue)}
          </span>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <div className="text-[11px] font-semibold" style={{ color: DS.text.primary }}>
            Automation Breakdown by Action Type
          </div>
          <div className="flex gap-1">
            {(
              ['all', 'remediation', 'detection', 'routing', 'reporting', 'ticketing'] as const
            ).map((cat) => (
              <button
                key={cat}
        ...
        <div className="rounded-xl border overflow-hidden" style={{ borderColor: DS.border }}>
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: `1px solid ${DS.border}`, background: DS.surface }}>
                {[
                  ...
                  <th
                    key={h}
                    className="text-[8px] uppercase tracking-widest font-medium px-4 py-2.5 text-right first:text-left"
                    style={{ color: DS.text.muted }}
                  >
                    {h}
                  </th>
                ...
                return (
                  <tr key={a.id} style={{ borderBottom: `1px solid rgba(255,255,255,0.03)` }}>
                    ...
                    <td className="py-3 px-4 text-right">
                      <span className="text-[11px] font-mono" style={{ color: DS.text.secondary }}>
                        {a.count.toLocaleString()}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-right">
                      <span className="text-[11px] font-mono" style={{ color: DS.text.muted }}>
                        {a.avgMinsPerManual}m
                      </span>
                    </td>
                    <td className="py-3 px-4 text-right">
                      <span className="text-[11px] font-mono font-semibold" style={{ color: GOLD }}>
                        {fmtHrs(a.totalHoursSaved)}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-right">
                      <span
                        ...
                        style={{ color: '#10b981' }}
                      >
                        {fmt$(a.totalDollarsSaved)}
                      </span>
                    </td>
                    <td className="py-3 px-4 text-right">
                      <span
                        className="text-[11px] font-mono"
                        style={{ color: a.mttrReductionMins > 0 ? '#10b981' : DS.text.muted }}
                      >
            ...
            <tfoot>
              <tr
                style={{
                  borderTop: `1px solid rgba(212,160,84,0.15)`,
                  background: 'rgba(212,160,84,0.02)',
                }}
              >
                <td
                  className="py-2.5 px-4 text-[9px] font-medium uppercase tracking-widest"
                  style={{ color: DS.text.muted }}
                >
                  Total (YTD)
                </td>
                <td
                  className="py-2.5 px-4 text-right text-[11px] font-mono"
                ...
                <td
                  className="py-2.5 px-4 text-right text-[12px] font-bold font-mono"
                  style={{ color: GOLD }}
                >
                  {fmtHrs(totalHours)}
                </td>
