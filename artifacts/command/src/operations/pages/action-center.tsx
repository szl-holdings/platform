    ...
    <div
      className={cn(
        ...
        action.urgency === 'immediate'
          ? 'border-[#c45a4a]/20 bg-[#c45a4a]/[0.03]'
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 text-[11px] flex-wrap">
              ...
              {action.valueProtected > 0 && (
                <>
                  <span className="text-slate-600">·</span>
                  <span className="text-[#6b8f71] font-mono flex items-center gap-1">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {action.dependencies.map((dep) => (
                  <span
                    key={dep}
                    className="text-[10px] font-mono px-2 py-1 rounded border border-[#d4a054]/20 text-[#d4a054] bg-[#d4a054]/5"
                  >
                    {dep}
                  </span>
                ))}
  ...
  const updateMutation = useStandardMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) =>
      api.actions.update(id, {
        state: status === 'done' ? 'resolved' : status === 'in_progress' ? 'assigned' : status,
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['lyte-actions'] });
    },
  ...
  const sections = [
    {
      ...
      icon: Zap,
      color: 'text-[#c45a4a]',
      badgeColor: 'text-[#c45a4a] bg-[#c45a4a]/10 border-[#c45a4a]/20',
    },
    {
      key: 'today',
      items: today,
      ...
      icon: Clock,
      color: 'text-[#c8953c]',
      badgeColor: 'text-[#c8953c] bg-[#c8953c]/10 border-[#c8953c]/20',
    },
    {
      key: 'this_week',
      items: thisWeek,
      ...
      icon: Clock,
      color: 'text-[#d4a054]',
      badgeColor: 'text-[#d4a054] bg-[#d4a054]/10 border-[#d4a054]/15',
    },
    {
      key: 'next_week',
      items: nextWeek,
      ...
      icon: Clock,
      color: 'text-[#4a90b8]',
      badgeColor: 'text-[#4a90b8] bg-[#4a90b8]/10 border-[#4a90b8]/10',
    },
  ];
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-0.5">
            ...
            <span className="text-[9px] px-1.5 py-0.5 rounded border border-[#6b8f71]/30 bg-[#6b8f71]/10 text-[#6b8f71] font-mono uppercase tracking-wide">
              Live
            </span>
          </div>
          <p className="text-sm text-slate-400 mt-1">
            Prioritized actions
            {totalProtected > 0 ? ` · ${formatCurrency(totalProtected)} value protected` : ''}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          },
          {
            ...
            value: isLoading ? '—' : formatCurrency(totalProtected),
            color: 'text-[#6b8f71]',
          ...
          >
            <div
              style={{
                ...
                right: 0,
                height: '2px',
                background: `linear-gradient(90deg, ${stat.accent.replace('0.1', '0.6')}, transparent)`,
              }}
            />
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-16 rounded-xl bg-white/[0.02] border border-white/5 animate-pulse"
            />
          ))}
        </div>
      ) : (
        <>
          {sections.map(
            ({ key, items, label, icon: Icon, color, badgeColor }) =>
              items.length > 0 && (
                <div key={key}>
                  <div className="flex items-center gap-2 mb-3">
                    <Icon className={cn('w-4 h-4', color)} />
                    <h2 className={cn('font-display font-semibold text-sm', color)}>{label}</h2>
                    <span className={cn('text-[10px] px-1.5 py-0.5 rounded border', badgeColor)}>
                      {items.length}
                    </span>
                  </div>
