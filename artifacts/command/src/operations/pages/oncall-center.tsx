...
import { Bell, Phone, AlertCircle, ChevronRight, RefreshCw } from "lucide-react";
...
function avatarColor(id: number): string {
  const palette = ["#60a5fa", "#34d399", "#a78bfa", GOLD, "#f97316", "#38bdf8", "#ef4444", "#10b981", "#ec4899"];
...
export default function OnCallCenter() {
  ...
  const overrideCount = useMemo(
    () => teams.reduce((acc, t) => acc + t.overrides.filter((o) => new Date(o.startAt).getTime() <= now && new Date(o.endAt).getTime() > now).length, 0),
    [teams, now],
  );
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl space-y-5" style={{ background: "#080c14" }}>
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-[15px] font-bold" style={{ color: DS.text.primary }}>On-Call Center</h1>
          </div>
          <p className="text-[11px]" style={{ color: DS.text.muted }}>
            Who's on-call across every team right now, and who's up next. Click a team to edit the schedule or page the on-call.
          </p>
        </div>
        <button
      ...
      {uncoveredCount > 0 && (
        ...
        >
          ...
          <div className="text-[11px]" style={{ color: DS.text.primary }}>
            <span className="font-semibold" style={{ color: "#ef4444" }}>{uncoveredCount}</span>{" "}
            team{uncoveredCount === 1 ? "" : "s"} have no on-call coverage right now — no active members or rotation configured.
          </div>
        </div>
      )}
      ...
      {query.isLoading ? (
        <div className="text-center py-12 text-[11px]" style={{ color: DS.text.muted }}>Loading on-call schedules…</div>
      ) : query.error ? (
        ...
        <div className="text-center py-12 text-[11px]" style={{ color: DS.text.muted }}>
          No teams configured yet. Assign team owners to apps or members to teams to populate this view.
        </div>
      ) : tab === "current" ? (
          ...
          {teams.map((t) => {
            ...
            return (
              <button
                ...
                style={{ borderColor: oncall ? `${color}25` : DS.border, background: oncall ? `${color}05` : DS.surface }}
              >
                <div className="flex items-start justify-between mb-3 gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-[11px] font-semibold truncate" style={{ color: DS.text.primary }}>{t.team}</span>
                      <SourceBadge source={t.currentOnCallSource} />
                    </div>
                    <div className="text-[9px] font-mono" style={{ color: DS.text.muted }}>
                      {t.memberCount} member{t.memberCount === 1 ? "" : "s"}
                      {t.ownedApps.length > 0 && ` · ${t.ownedApps.length} app${t.ownedApps.length === 1 ? "" : "s"}`}
                    </div>
                  </div>
                  <ChevronRight className="w-3.5 h-3.5 shrink-0 mt-0.5" style={{ color: DS.text.muted }} />
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-2 rounded-lg" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="text-[10px] font-medium truncate" style={{ color: DS.text.primary }}>
                          {oncall?.displayName ?? "No coverage"}
                        </div>
                        <div className="text-[8px]" style={{ color: DS.text.muted }}>Primary on-call</div>
                      </div>
                    </div>
                  ...
                  {escalation && escalation.id !== oncall?.id && (
                    <div className="flex items-center gap-2 px-2 py-1 rounded text-[9px]" style={{ color: DS.text.muted }}>
                      <Bell className="w-2.5 h-2.5 shrink-0" />
                      Escalation → <span style={{ color: DS.text.secondary }}>{escalation.displayName}</span>
                    </div>
                  )}
                </div>

                <div className="mt-3 pt-3 grid grid-cols-2 gap-2" style={{ borderTop: `1px solid ${DS.border}` }}>
                  <div>
                    <div className="text-[8px] uppercase tracking-wider mb-0.5" style={{ color: DS.text.muted }}>
                      {activeOverride ? "Override ends" : "Next handoff"}
                    </div>
                    <div className="text-[10px] font-mono" style={{ color: activeOverride ? "#f97316" : nextHandoff ? "#3b82f6" : DS.text.muted }}>
                      {activeOverride
                  ...
                  <div>
                    <div className="text-[8px] uppercase tracking-wider mb-0.5" style={{ color: DS.text.muted }}>Then</div>
        ...
        <div className="rounded-xl border overflow-hidden" style={{ borderColor: DS.border, background: DS.surface }}>
          <div className="p-3 border-b flex items-center justify-between" style={{ borderColor: DS.border }}>
            ...
            <span className="text-[9px] font-mono" style={{ color: DS.text.muted }}>Next 7 days · {timeline.length} event{timeline.length === 1 ? "" : "s"}</span>
          </div>
          {timeline.length === 0 ? (
            <div className="text-center py-12 text-[11px]" style={{ color: DS.text.muted }}>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-0.5">
                        <span className="text-[11px] font-semibold" style={{ color: DS.text.primary }}>{e.team}</span>
                        <span
                        ...
                        >
                          {isActiveOverride ? "ACTIVE OVERRIDE" : isOverride ? "OVERRIDE" : "HANDOFF"}
                        </span>
                      </div>
