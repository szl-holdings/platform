...
const CHANGES: Change[] = [
  {
    ...
    postChangeValidations: [
      'Smoke test checkout flow',
      'Verify queue consumer lag < 500ms',
      'Confirm payment webhook delivery',
    ],
    regressions: [],
    aiRiskFactors: [
      'Consumer performance regression in v2.14.1 not fully resolved',
      'High traffic window — 2PM EST',
      'No canary deploy configured',
    ],
  ...
  },
  {
    ...
    postChangeValidations: [
      'Run pg_upgrade validation suite',
      'Verify replication lag < 1s',
      'Confirm connection pool health',
    ],
    regressions: [],
    aiRiskFactors: [
      'Conflict with CHG-4439 in same maintenance window',
      'Upgrade requires 2-minute downtime window',
    ],
  ...
  },
  {
    ...
    postChangeValidations: [
      'Rate limit test from synthetic client',
      'Verify legitimate traffic unblocked',
    ],
    regressions: [
      'External clients receiving 429 on valid API calls',
      'P99 error rate spiked to 12%',
    ],
    aiRiskFactors: [
      'Rate limit threshold set too aggressively (100 req/min vs baseline 850 req/min)',
    ],
      ...
      <div className="max-w-[1400px] mx-auto p-4 space-y-4">
        <div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: DS.text.muted }}>
            AI risk scoring · blast radius prediction · maintenance window optimization · regression
            detection
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { label: 'Failed Changes', value: failed, color: '#ef4444' },
            {
              ...
              value: Math.round(CHANGES.reduce((a, c) => a + c.riskScore, 0) / CHANGES.length),
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-4">
          ...
          <div className="space-y-2">
            ...
            {CHANGES.map((c) => {
              ...
              const timeLabel =
                msFromNow > 0
                  ? `in ${Math.round(msFromNow / 60000)}m`
                  : `${Math.round(-msFromNow / 60000)}m ago`;
              return (
                <button
                  key={c.id}
                ...
                >
                  ...
                  <div className="pl-4 text-[9px]" style={{ color: DS.text.muted }}>
                    {c.type} · {c.owner}
                  </div>
                </button>
              );
          ...
          >
            <div className="p-4 border-b" style={{ borderColor: DS.border }}>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      className="text-[8px] px-1.5 py-0.5 rounded font-mono capitalize"
                      style={{
                        background: `${RISK_COLOR[selected.riskLevel]}15`,
              ...
              <div className="grid grid-cols-3 gap-3 text-[10px]">
                <div>
                  <span style={{ color: DS.text.muted }}>Owner: </span>
                  <span style={{ color: DS.text.secondary }}>{selected.owner}</span>
                </div>
                <div>
                  <span style={{ color: DS.text.muted }}>Window: </span>
                  <span style={{ color: DS.text.secondary }}>{selected.maintenanceWindow}</span>
                </div>
                <div>
                  <span style={{ color: DS.text.muted }}>Duration: </span>
                  <span style={{ color: DS.text.secondary }}>{selected.estimatedDuration}m</span>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div
                    ...
                    style={{ background: 'rgba(255,255,255,0.03)', color: DS.text.secondary }}
                  >
