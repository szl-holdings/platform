...
const HIGHLIGHTS = [
  {
    type: 'achievement',
    text: 'P99 latency improved 34% week-over-week following API Gateway optimization deployment.',
  },
  {
    type: 'achievement',
    text: 'Successful zero-downtime migration of auth service to new infrastructure. Zero customer impact.',
  },
  {
    type: 'issue',
    text: 'Payment service experienced 2 brief degradations (avg 4.2 min each) due to DB connection pool exhaustion. Remediation in progress.',
  },
  {
    type: 'issue',
    text: 'Worker pool memory pressure detected Thursday, triggering auto-scaling. Root cause: memory leak in queue consumer (patch deployed Fri).',
  },
  {
    type: 'forecast',
    text: 'Projected 99.97% availability next 7 days based on current incident rate and planned maintenance windows.',
  },
  {
    type: 'recommendation',
    text: 'Recommend pre-emptive capacity upgrade to payment service DB pool ahead of Q2 reporting period (projected +40% load).',
  },
];
    ...
    <div className="h-full overflow-y-auto" style={{ background: '#080f1c' }}>
      <div className="max-w-4xl mx-auto p-6 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/30">
                Business-readable operational health digest for leadership
              </p>
            </div>
          </div>
        ...
        <div
          ...
          style={{ borderColor: 'rgba(212,160,84,0.15)', background: 'rgba(212,160,84,0.04)' }}
        >
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-bold text-white">
              Platform Operational Summary — Last 7 Days
            </h2>
            <div
          ...
          <div className="space-y-4 text-[12px] leading-relaxed text-white/60">
            <p>
              Platform availability for the trailing 7-day period was{' '}
              <span className="font-bold text-white">99.41%</span> — slightly below our 99.9% SLO
              target, driven by two payment service degradations on Wednesday and Thursday. All
              other services maintained full availability throughout the period.
            </p>
            <p>
              The week's primary operational highlight was the successful zero-downtime API Gateway
              migration, which improved P99 request latency by{' '}
              <span className="font-bold text-white">34%</span> (from 72ms to 48ms). This is
              expected to have measurable impact on checkout completion rates in Q2 reporting.
            </p>
            <p>
              Worker pool instability on Thursday was traced to a memory leak in the queue consumer
              introduced in the v2.14.1 release. A patch was deployed Friday without incident;
              monitoring shows stable memory utilization since. No data loss occurred.
            </p>
            <p>
              Our current trajectory projects{' '}
              <span className="font-bold text-white">99.97% availability</span> for the upcoming
              7-day window assuming no new incidents. One planned maintenance window is scheduled
              for Tuesday 02:00–03:00 UTC (infrastructure upgrade, zero customer impact expected).
            </p>
          </div>
        </div>
        ...
        >
          <h3 className="text-[11px] font-bold text-white mb-3 flex items-center gap-2">
            <Activity className="w-3.5 h-3.5" style={{ color: '#d4a054' }} /> Key Highlights &
            Recommendations
          </h3>
          <div className="space-y-2">
            {HIGHLIGHTS.map((h, i) => {
        ...
        <div className="text-center text-[9px] text-white/15 pb-4">
          Generated by Command AIOps · All metrics derived from live telemetry · For internal
          distribution only
        </div>
      </div>
    </div>
