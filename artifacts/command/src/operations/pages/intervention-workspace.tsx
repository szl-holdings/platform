  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4 h-[600px]">
        <div className="rounded-xl border overflow-hidden flex flex-col" style={{ borderColor: "rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)" }}>
          <div className="px-4 py-3 border-b text-xs font-semibold text-white" style={{ borderColor: "rgba(255,255,255,0.05)" }}>Active Threads</div>
        ...
        <div className="col-span-2 rounded-xl border overflow-hidden flex flex-col" style={{ borderColor: "rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.01)" }}>
          {selectedThread ? (
            <>
              <div className="px-5 py-4 border-b" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
                <div className="text-sm font-semibold text-white mb-0.5">{selectedThread.workflow}</div>
                <div className="text-[9px]" style={{ color: "rgba(255,255,255,0.35)" }}>Correlation: {selectedThread.correlation_id}</div>
              </div>
                  ...
                  <div key={i} className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[9px]" style={{ color: "rgba(255,255,255,0.3)" }}>{c.time}</span>
                      </div>
                      <div className="text-[11px] leading-relaxed" style={{ color: "rgba(255,255,255,0.6)" }}>{c.text}</div>
                    </div>
                  </div>
                ))}
              </div>

              <div className="px-5 py-4 border-t space-y-3" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
                <div className="flex gap-2 flex-wrap">
                  {selectedThread.actions_available.map(action => {
                    const s = ACTION_STYLES[action];
                    if (!s) return null;
                    return (
                      <button key={action} className="text-[10px] px-3 py-1.5 rounded-lg font-medium transition-all hover:opacity-80" style={{ color: s.color, background: s.bg, border: `1px solid ${s.border}` }}>
                        {s.label}
                      </button>
                    );
                  })}
            ...
            <div className="flex-1 flex items-center justify-center text-sm" style={{ color: "rgba(255,255,255,0.3)" }}>Select a thread to review</div>
          )}
        </div>
      </div>
    </div>
