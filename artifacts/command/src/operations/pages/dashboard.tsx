  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: sev === 'critical' && pulse ? `0 0 6px ${colors.critical}60` : 'none',
      }}
    />
  );
}
...
function ImpactBadge({ impact }: { impact: string }) {
  ...
  return (
    <span
      ...
      style={{
        color: isHigh ? '#c45a4a' : '#c8953c',
        background: isHigh ? 'rgba(196,90,74,0.06)' : 'rgba(200,149,60,0.06)',
        border: `1px solid ${isHigh ? 'rgba(196,90,74,0.15)' : 'rgba(200,149,60,0.15)'}`,
      }}
    >
      {impact}
    </span>
...
function PrismCompositeRing({
  composite,
  prismData,
}: {
  composite: number;
  prismData: typeof PRISM_DATA;
}) {
  const r = 40;
  const c = 2 * Math.PI * r;
                ...
                <div style={{ width: 24, height: 24, position: 'relative' }}>
                  <svg width="24" height="24" className="rotate-[-90deg]">
                    ...
                    <circle
                      ...
                      stroke={p.color}
                      fill="none"
                      strokeDasharray={`${(p.score / 100) * 2 * Math.PI * 9} ${2 * Math.PI * 9}`}
                      strokeLinecap="round"
                    />
                  </svg>
                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="text-[8px] font-black font-mono" style={{ color: p.color }}>
                      {p.key}
                    </span>
                  </div>
                </div>
                <span className="text-[7px] font-mono" style={{ color: scoreColor }}>
                  {p.score}
                </span>
              </div>
            </Link>
...
export default function Dashboard() {
  ...
  const signalTrendData = (() => {
    const buckets: Record<
      string,
      { critical: number; high: number; medium: number; low: number; date: string }
    > = {};
    const now = Date.now();
    for (let i = 6; i >= 0; i--) {
  ...
  const metrics = [
    {
      ...
      value: isLoading ? '—' : (summary?.criticalUnresolved ?? criticalSignals.length),
      color: '#c45a4a',
      pulse: true,
      sub: 'live',
    },
    {
      ...
      value: isLoading ? '—' : summary?.agedApprovals != null ? String(summary.agedApprovals) : '—',
      color: '#c8953c',
      sub: '>48h',
    },
    {
      label: 'Ownership Gaps',
      value: isLoading ? '—' : summary?.ownershipGaps != null ? String(summary.ownershipGaps) : '—',
      color: '#d4a054',
      sub: 'unassigned',
    },
    {
    ...
    {
      label: 'Value at Risk',
      value: isLoading ? '—' : (summary?.valueAtRisk ?? '—'),
      color: '#c45a4a',
      sub: 'estimated',
    },
    {
      label: 'Decision Latency',
      value: isLoading ? '—' : (summary?.decisionLatency ?? '—'),
      color: '#c8953c',
      sub: 'avg',
    },
  ];
  ...
  const queue = [
    ...criticalSignals.slice(0, 3).map((s) => ({
      title: s.title,
      severity: s.severity,
      owner: (s.metadata?.affectedFunction as string | undefined) ?? 'Unassigned',
      time: timeAgo(s.receivedAt ?? s.createdAt),
  ...
  const platforms = [
    {
      ...
      count: signals.filter(
        (s) => s.source.toLowerCase().includes('api') && s.status !== 'resolved',
      ).length,
    },
    {
      label: 'SEXTANT',
      ...
      count: signals.filter(
        (s) =>
          (s.source.toLowerCase().includes('firestorm') ||
            s.source.toLowerCase().includes('aegis')) &&
          s.status !== 'resolved',
      ).length,
    },
    {
    ...
    <div className="p-3 lg:p-4 space-y-3" style={{ maxWidth: 1440 }}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => refetch()}
            className="flex items-center gap-1 px-2 py-1 rounded text-[9px] font-mono transition-all hover:bg-white/[0.03]"
            style={{ border: `1px solid ${BORDER.subtle}`, color: TEXT.tertiary }}
          >
      ...
      <div
        className="grid grid-cols-3 lg:grid-cols-6 rounded-lg overflow-hidden"
        style={{ background: BORDER.subtle, gap: '1px' }}
      >
        {metrics.map((m) => (
          <div key={m.label} className="px-3 py-2.5 relative" style={{ background: BG.surface }}>
            <div
              style={{
                ...
                right: 0,
                height: 2,
                background: `linear-gradient(90deg, ${m.color}50, transparent)`,
              }}
            />
            <div
            ...
            <div className="flex items-baseline gap-1.5">
              ...
              {m.pulse && typeof m.value === 'number' && m.value > 0 && (
                <span
      ...
      <div className="grid grid-cols-5 gap-1.5">
        {PRISM_DATA.map((p) => {
          ...
          return (
            <Link key={p.key} href={p.href}>
              <div
                className="rounded-lg px-3 py-2 cursor-pointer transition-all group relative overflow-hidden hover:bg-white/[0.02]"
                style={{ background: BG.surface, border: `1px solid ${p.color}14` }}
              >
                <div
                ...
                <div className="flex items-center gap-2 mb-2">
                  <div style={{ width: 20, height: 20, position: 'relative', flexShrink: 0 }}>
                    <svg width="20" height="20" className="rotate-[-90deg]">
                      ...
                      <circle
                        ...
                        stroke={p.color}
                        fill="none"
                        strokeDasharray={`${pfilled} ${pc - pfilled}`}
                        strokeLinecap="round"
                      />
                    </svg>
                    <div className="absolute inset-0 flex items-center justify-center">
                  ...
                  <p.icon
                    size={9}
                    className="ml-auto opacity-30 group-hover:opacity-60 transition-opacity"
                    style={{ color: p.color }}
                  />
                </div>
                <div className="flex items-center gap-2">
      ...
      {activeRole && (
        <div
          ...
          style={{
            ...
            borderLeft: `2px solid ${activeRole === 'analyst' ? '#8b7ac8' : activeRole === 'buyer' ? '#4a90b8' : '#d4a054'}`,
          }}
        >
          <span
            className="text-[8px] uppercase tracking-widest font-mono shrink-0"
            style={{ color: TEXT.muted }}
          >
            {activeRole === 'buyer' ? 'demo' : activeRole}
          </span>
          <span className="text-[10px]" style={{ color: TEXT.secondary }}>
            {activeRole === 'executive' &&
              `${criticalSignals.length} critical. ${highSignals.length} high. PRISM composite ${prismComposite}/100. Decision latency 34h.`}
            {activeRole === 'operator' &&
              `${criticalSignals.length} critical in triage. ${summary?.openActions ?? 0} open actions. ${summary?.openIncidents ?? 0} incidents. 14 approvals aged >48h.`}
            {activeRole === 'analyst' &&
          ...
          <Link
            ...
            className="shrink-0 ml-auto text-[8px] px-2 py-px rounded font-mono hover:opacity-80"
            style={{
              color: '#c8953c',
        ...
        <div className="col-span-12 lg:col-span-5 space-y-3">
            ...
            <PanelHead
              ...
              right={
                <div className="flex items-center gap-2">
                  ...
                  <Link
                    href="/operations/approvals"
                    className="text-[9px] font-mono flex items-center gap-0.5 hover:opacity-80"
            ...
            <div className="divide-y" style={{ borderColor: BORDER.subtle }}>
              {queue.length > 0 ? (
                queue.slice(0, 5).map((a, i) => (
                  <div
                    key={i}
                    className="px-3 py-2.5 hover:bg-white/[0.015] transition-colors cursor-pointer"
                    onClick={() => setAuditSignal(a.signal)}
                  >
                    ...
                    <div
                      ...
                      style={{
                        borderLeft:
                          a.severity === 'critical'
                            ? '2px solid rgba(196,90,74,0.5)'
                            : '2px solid transparent',
                        paddingLeft: a.severity === 'critical' ? '0.5rem' : '0',
                      }}
                    >
                      <Dot sev={a.severity} pulse={a.severity === 'critical'} />
                      <div className="flex-1 min-w-0">
                        <div
                          className="text-[10px] font-medium leading-snug line-clamp-1 mb-1"
                ...
                <div className="flex items-center gap-2 px-3 py-3">
                  ...
                  <span className="text-[10px]" style={{ color: TEXT.secondary }}>
                    No priority actions — system nominal
                  </span>
                </div>
              )}
            </div>
          ...
          <Panel accent="#8b7ac8">
            <PanelHead
              ...
              accent="#8b7ac8"
              right={
                <span className="text-[8px] font-mono" style={{ color: 'rgba(139,122,200,0.5)' }}>
                  AI-scored
                </span>
              }
            />
            <div className="divide-y" style={{ borderColor: BORDER.subtle }}>
              {recommendationsData.length === 0 ? (
                <div className="px-3 py-4 flex items-center gap-2">
                  <CheckCircle2 className="w-3 h-3" style={{ color: '#4a90b8' }} />
                  <span className="text-[10px]" style={{ color: TEXT.secondary }}>
                    No recommendations — connect integrations to generate actions
                  </span>
                </div>
              ) : (
                recommendationsData.slice(0, 4).map((a) => (
                  <div
                    key={a.id}
                    className="px-3 py-2 hover:bg-white/[0.015] transition-colors cursor-pointer"
                  >
                    <div className="flex items-start gap-2">
                      <div className="mt-0.5">
                        <Dot
                          sev={a.category === 'critical' ? 'critical' : 'high'}
                          pulse={a.category === 'critical'}
                        />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div
                          className="text-[10px] font-medium line-clamp-1 mb-1"
                          style={{ color: TEXT.primary }}
                        >
                          {a.title}
                        </div>
                      ...
                      <button
                        className="shrink-0 text-[8px] px-2 py-px rounded font-mono hover:opacity-80"
                        style={{
                          color: '#8b7ac8',
        ...
        <div className="col-span-12 lg:col-span-4 space-y-3">
          ...
          <Panel accent="#d4a054">
            <PanelHead
              ...
              accent="#d4a054"
              right={
                <span className="text-[8px] font-mono tabular-nums" style={{ color: TEXT.muted }}>
                  {prismComposite}/100
                </span>
              }
            />
            <div className="px-3 py-3 flex items-center gap-4">
              ...
              <div className="flex-1 space-y-1.5">
                {PRISM_DATA.map((p) => {
                  const scoreColor =
                    p.score >= 70 ? '#6b8f71' : p.score >= 50 ? '#c8953c' : '#c45a4a';
                  return (
                    <div key={p.key} className="flex items-center gap-1.5">
                      <p.icon size={8} style={{ color: p.color, flexShrink: 0 }} />
                      <span className="text-[8px]" style={{ color: TEXT.tertiary, width: 64 }}>
                        {p.name}
                      </span>
                      <div
                        className="flex-1 h-1 rounded-full overflow-hidden"
                ...
                <AreaChart data={signalTrendData}>
                  <defs>
                    ...
                    <linearGradient id="hG" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#c8953c" stopOpacity={0.2} />
            ...
            <PanelHead
              ...
              right={
                <Link
                  ...
                  style={{ color: TEXT.tertiary }}
                >
                  Feed <ArrowUpRight className="w-2.5 h-2.5" />
                </Link>
              }
            />
            <div className="px-3">
              {recentSignals.length === 0 ? (
                <div className="flex items-center gap-2 py-4 justify-center">
                  ...
                  <span className="text-[9px] font-mono" style={{ color: TEXT.muted }}>
                    No recent signals — system quiet
                  </span>
                </div>
              ) : (
                recentSignals.slice(0, 7).map((s) => (
                  <div
                    key={s.id}
                    className="flex items-center gap-2 py-1.5 cursor-pointer hover:bg-white/[0.01] transition-colors"
                    style={{ borderBottom: `1px solid ${BORDER.subtle}` }}
                    onClick={() => setAuditSignal(s)}
                  >
                    <Dot sev={s.severity} pulse={s.severity === 'critical'} />
                    <div className="flex-1 min-w-0">
                      <span className="text-[9px] truncate block" style={{ color: TEXT.secondary }}>
                        {s.title}
                      </span>
                    ...
                    <span className="text-[8px] font-mono shrink-0" style={{ color: TEXT.muted }}>
                      {timeAgo(s.receivedAt ?? s.createdAt)}
                    </span>
                    <span
                      className="text-[7px] font-mono px-1 py-px rounded uppercase shrink-0"
                      style={{
                        color:
                          s.status === 'resolved'
                            ? 'rgba(74,144,184,0.5)'
                            : s.status === 'acknowledged'
                              ? 'rgba(139,122,200,0.4)'
                              : TEXT.muted,
          ...
          <Panel accent="#8b7ac8">
            ...
            <div className="px-3 py-1">
              {platforms.map((p) => (
                <div
                  key={p.label}
                  className="flex items-center justify-between py-1.5 text-[9px]"
                  style={{ borderBottom: `1px solid ${BORDER.subtle}` }}
                >
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    >
                      <div
                        style={{
                          height: '100%',
                          width: signalsLoading
                            ? '0%'
                            : p.count === 0
                              ? '100%'
                              : `${Math.min(p.count * 15, 100)}%`,
                          background: signalsLoading
                            ? TEXT.muted
                            : p.count === 0
                              ? '#6b8f71'
                              : p.count <= 2
          ...
          <Panel accent="rgba(212,160,84,0.4)">
            ...
            <div className="px-3 py-1">
              {[
                ...
                {
                  k: 'Last sync',
                  v: dashboardData?.fetchedAt ? timeAgo(dashboardData.fetchedAt) : '—',
                  c: TEXT.tertiary,
                },
                { k: 'Confidence', v: 'High', c: 'rgba(212,160,84,0.45)' },
                {
                  k: 'Sources',
                  v: signalsLoading ? '—' : `${new Set(signals.map((s) => s.source)).size}`,
                  c: TEXT.tertiary,
                },
      ...
      <ActionLoop
        ...
        actions={[
          {
            id: '1',
            label: 'Investigate API heap memory alert',
            severity: 'critical',
            type: 'investigate',
          },
          {
            id: '2',
            label: 'Resolve aged procurement approvals (14)',
            severity: 'high',
            type: 'approve',
          },
          {
            id: '3',
            label: 'Assign owner to 8 orphaned processes',
            severity: 'high',
            type: 'assign',
          },
          {
            id: '4',
            label: 'Review Salesforce pipeline stall ($400K)',
            severity: 'critical',
            type: 'investigate',
          },
          {
            id: '5',
            label: 'Escalate vendor renewal gap to COO',
            severity: 'medium',
            type: 'escalate',
          },
        ]}
          ...
          >
            ...
            >
              <div>
                ...
                <p className="text-[10px] mt-1" style={{ color: TEXT.tertiary }}>
                  {auditSignal.source} · {auditSignal.sourceType}
                </p>
              </div>
              <button
                onClick={() => setAuditSignal(null)}
                className="text-slate-500 hover:text-white text-sm shrink-0 ml-4"
              >
                ✕
              </button>
            </div>
                  ...
                  style={{
                    color:
                      auditSignal.status === 'resolved'
                        ? '#6b8f71'
                        : auditSignal.status === 'acknowledged'
                          ? '#8b7ac8'
                          : '#c45a4a',
                    background: 'rgba(255,255,255,0.04)',
                      ...
                      [
                        {
                          ...
                          detail: `Source: ${auditSignal.source}`,
                          time: auditSignal.receivedAt ?? auditSignal.createdAt,
                          done: true,
                        },
                        {
                          icon: Eye,
                          ...
                          detail:
                            'Cross-referenced with 3 related signals · PRISM Risk lens scored',
                          time: auditSignal.receivedAt ?? auditSignal.createdAt,
                          done: true,
                        },
                        {
                          ...
                          detail: 'Action recommended with high confidence',
                          time: auditSignal.receivedAt ?? auditSignal.createdAt,
                          done: true,
                        },
                        {
                          ...
                          detail: 'HITL gate triggered — awaiting operator sign-off',
                          time: auditSignal.receivedAt ?? auditSignal.createdAt,
                          done: auditSignal.status !== 'new',
                        },
                        {
                          icon: CheckCircle2,
                          color: '#6b8f71',
                          ...
                          detail:
                            auditSignal.status === 'resolved'
                              ...
                              : 'Pending approval',
                          time: auditSignal.status === 'resolved' ? auditSignal.createdAt : null,
                          done: auditSignal.status === 'resolved',
                        },
                        {
                          icon: FileText,
                          color: TEXT.tertiary,
                          label: 'Result Logged',
                          detail: 'Outcome recorded in immutable audit trail',
                          time: null,
                          done: auditSignal.status === 'resolved',
                        },
                      ] as Array<{
                      ...
                      return (
                        <div key={si} className="flex items-start gap-3">
                          ...
                          >
                            <Icon
                              className="w-2 h-2"
                              style={{ color: step.done ? step.color : TEXT.muted }}
                            />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div
                              className="text-[10px] font-medium"
                              style={{ color: step.done ? TEXT.primary : TEXT.muted }}
                            >
                              {step.label}
                            </div>
              ...
              {auditSignal.status !== 'resolved' && (
                <div className="pt-2" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <button
                      className="text-[10px] px-3 py-1.5 rounded font-medium hover:opacity-80"
                      style={{
                        color: '#6b8f71',
