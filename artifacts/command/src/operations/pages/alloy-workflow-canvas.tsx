...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; icon: React.ElementType }> = {
  ...
  awaiting_approval: { label: "Awaiting Approval", color: "#8b7ac8", bg: "rgba(139,122,200,0.12)", icon: UserCheck },
};

const NODE_TYPE_CONFIG: Record<NodeType, { color: string; bg: string; icon: React.ElementType; shape: string }> = {
  ...
  approval: { color: "#8b7ac8", bg: "rgba(139,122,200,0.12)", icon: UserCheck, shape: "rounded-lg" },
...
const STATIC_WORKFLOWS: AlloyWorkflow[] = [
  {
    id: 1, name: "Incident Triage Pipeline", description: "Ingest alert → classify → route → page on-call",
    status: "active", triggerType: "event", stepCount: 7, lastRunAt: "2026-04-01T03:22:41Z",
    dag: {
      nodes: [
        ...
        { id: "ap1", type: "approval", label: "Ops Approval", status: "awaiting", actor: "J. Martinez" },
      ...
      edges: [
        { from: "t1", to: "c1" }, { from: "c1", to: "a1", label: "yes" },
        { from: "a1", to: "ap1" }, { from: "ap1", to: "a2" },
        { from: "a2", to: "n1" }, { from: "n1", to: "e1" },
      ],
    },
  },
  {
    id: 2, name: "SLO Burn-Rate Response", description: "Monitor SLO budget → auto-scale → notify SRE",
    status: "active", triggerType: "metric", stepCount: 5, lastRunAt: "2026-04-01T02:55:18Z",
    dag: {
      nodes: [
      ...
      edges: [
        ...
        { from: "c1", to: "a2", label: "yes" }, { from: "a2", to: "n1" },
      ],
    },
  },
  {
    id: 3, name: "Compliance Evidence Collector", description: "Pull audit trails → compile artifacts → push to GRC",
    status: "active", triggerType: "schedule", stepCount: 6, lastRunAt: "2026-03-31T23:00:00Z",
  },
  {
    id: 4, name: "Entity Resolution Engine", description: "Cross-ref vendor data → deduplicate → enrich CRM",
    status: "active", triggerType: "api", stepCount: 8, lastRunAt: "2026-04-01T01:04:22Z",
  },
  {
    id: 5, name: "Capital Readiness Briefing", description: "Aggregate KPIs → format exec brief → route for approval",
    status: "draft", triggerType: "schedule", stepCount: 6, lastRunAt: undefined,
  },
];

const STATIC_RUNS: AlloyRun[] = [
  { id: 101, workflowId: 1, status: "awaiting_approval", startedAt: "2026-04-01T03:22:41Z", actor: "system" },
  { id: 102, workflowId: 2, status: "running", startedAt: "2026-04-01T03:21:00Z", actor: "system" },
  { id: 103, workflowId: 3, status: "completed", startedAt: "2026-03-31T23:00:00Z", completedAt: "2026-03-31T23:01:42Z", actor: "scheduler" },
  { id: 104, workflowId: 1, status: "failed", startedAt: "2026-04-01T02:10:00Z", completedAt: "2026-04-01T02:10:04Z", errorMessage: "Step 4: API timeout — retrying", actor: "system" },
  { id: 105, workflowId: 4, status: "completed", startedAt: "2026-04-01T01:04:22Z", completedAt: "2026-04-01T01:04:38Z", actor: "api" },
  { id: 106, workflowId: 2, status: "completed", startedAt: "2026-04-01T00:15:00Z", completedAt: "2026-04-01T00:15:11Z", actor: "scheduler" },
];

const CONNECTORS = [
  { id: "sf", name: "Salesforce", category: "CRM", status: "connected", events: 142, lastSync: "2m ago", icon: "🔷" },
  { id: "sn", name: "ServiceNow", category: "ITSM", status: "connected", events: 87, lastSync: "5m ago", icon: "🟢" },
  { id: "jira", name: "Jira", category: "Projects", status: "connected", events: 63, lastSync: "1m ago", icon: "🔵" },
  { id: "gh", name: "GitHub", category: "Engineering", status: "connected", events: 211, lastSync: "30s ago", icon: "⚫" },
  { id: "slack", name: "Slack", category: "Comms", status: "connected", events: 39, lastSync: "10s ago", icon: "🟣" },
  { id: "wd", name: "Workday", category: "HR/Finance", status: "connected", events: 22, lastSync: "15m ago", icon: "🟠" },
  { id: "pagerduty", name: "PagerDuty", category: "Alerting", status: "connected", events: 18, lastSync: "2m ago", icon: "🔴" },
  { id: "okta", name: "Okta", category: "Identity", status: "degraded", events: 5, lastSync: "45m ago", icon: "🔵" },
  { id: "stripe", name: "Stripe", category: "Payments", status: "connected", events: 91, lastSync: "1m ago", icon: "🟣" },
  { id: "grc", name: "GRC Platform", category: "Compliance", status: "pending", events: 0, lastSync: "Never", icon: "🟡" },
];

const POLICY_RULES = [
  { id: "p1", name: "High-severity signals → require human approval", scope: "All workflows", active: true, condition: "severity >= high", action: "Insert approval gate", priority: 1 },
  { id: "p2", name: "Financial actions > $50K → Finance review", scope: "Finance workflows", active: true, condition: "impact_estimate > 50000", action: "Route to Finance approver", priority: 2 },
  { id: "p3", name: "Compliance events → immutable audit log", scope: "Compliance workflows", active: true, condition: "category = compliance", action: "Append to audit trail", priority: 3 },
  { id: "p4", name: "Auto-remediation for known patterns", scope: "Ops workflows", active: false, condition: "pattern_match = known", action: "Execute remediation playbook", priority: 4 },
  { id: "p5", name: "Off-hours actions → defer or escalate", scope: "All workflows", active: true, condition: "time NOT BETWEEN 08:00-18:00", action: "Escalate or defer", priority: 5 },
];

const AUDIT_LOG = [
  { id: "al1", ts: "2026-04-01T03:22:48Z", level: "info", actor: "alloy-engine", event: "Run #101 started", workflow: "Incident Triage Pipeline", runId: 101 },
  { id: "al2", ts: "2026-04-01T03:22:49Z", level: "info", actor: "alloy-engine", event: "Step 1 completed — Alert Ingested", workflow: "Incident Triage Pipeline", runId: 101 },
  { id: "al3", ts: "2026-04-01T03:22:50Z", level: "info", actor: "alloy-engine", event: "Step 3 completed — Context Enriched", workflow: "Incident Triage Pipeline", runId: 101 },
  { id: "al4", ts: "2026-04-01T03:22:51Z", level: "warn", actor: "alloy-engine", event: "Approval gate triggered — awaiting J. Martinez", workflow: "Incident Triage Pipeline", runId: 101 },
  { id: "al5", ts: "2026-04-01T03:21:00Z", level: "info", actor: "scheduler", event: "Run #102 started — SLO threshold crossed", workflow: "SLO Burn-Rate Response", runId: 102 },
  { id: "al6", ts: "2026-04-01T02:10:04Z", level: "error", actor: "alloy-engine", event: "Run #104 failed — API timeout at Step 4", workflow: "Incident Triage Pipeline", runId: 104 },
  { id: "al7", ts: "2026-04-01T01:04:38Z", level: "info", actor: "api", event: "Run #105 completed — 16s duration", workflow: "Entity Resolution Engine", runId: 105 },
  { id: "al8", ts: "2026-03-31T23:01:42Z", level: "info", actor: "scheduler", event: "Run #103 completed — 102s duration", workflow: "Compliance Evidence Collector", runId: 103 },
];
  ...
  return (
    <div className="flex flex-col items-center min-w-[80px]">
      <div
        ...
        style={{
          background: node.status === "done" ? `${statusColor}15` : node.status === "active" ? `${statusColor}20` : node.status === "awaiting" ? `${statusColor}18` : "rgba(255,255,255,0.02)",
          border: `2px solid ${node.status ? statusColor : "rgba(255,255,255,0.08)"}`,
          opacity: node.status === "pending" ? 0.4 : 1,
        }}
      >
        {node.status === "active" && <div className="absolute inset-0 rounded-lg animate-pulse" style={{ background: `${statusColor}10` }} />}
        <Icon className="w-5 h-5" style={{ color: node.status ? statusColor : TEXT.muted }} />
        {isApproval && node.status === "awaiting" && (
          <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full animate-pulse" style={{ background: "#8b7ac8" }} />
        )}
      </div>
      <div className="mt-1.5 text-center px-1">
        <div className="text-[8px] font-medium leading-tight" style={{ color: node.status === "pending" ? TEXT.muted : TEXT.secondary, maxWidth: 80 }}>{node.label}</div>
        {node.actor && node.status === "awaiting" && (
          <div className="text-[7px] mt-0.5 font-mono" style={{ color: "#8b7ac8" }}>{node.actor}</div>
        )}
        {node.status === "active" && (
          <div className="text-[7px] mt-0.5 font-mono" style={{ color: "#d4a054" }}>running</div>
        )}
      </div>
    </div>
  );
...
function DAGEdge({ label }: { label?: string }) {
  return (
    <div className="flex flex-col items-center justify-center px-1" style={{ minWidth: 24 }}>
      ...
      {label && <div className="text-[7px] font-mono mt-0.5" style={{ color: TEXT.muted }}>{label}</div>}
    </div>
  );
}
      ...
      <div className="mt-3 flex items-center gap-1 overflow-x-auto pb-1">
        {Array.from({ length: wf.stepCount ?? 5 }).map((_, si) => (
          <div key={si} className="flex items-center gap-1 shrink-0">
            ...
            >{si + 1}</div>
            {si < (wf.stepCount ?? 5) - 1 && <ArrowRight className="w-2 h-2 shrink-0" style={{ color: TEXT.muted }} />}
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div className="mt-3 overflow-x-auto pb-2">
      <div className="flex items-start gap-0 min-w-max">
        {ordered.map((node, i) => {
          const incomingEdge = dag.edges.find(e => e.to === node.id && ordered[i - 1]?.id === e.from);
          return (
...
export default function AlloyWorkflowCanvas() {
  ...
  return (
    <div className="p-4 space-y-4" style={{ background: BG.page, minHeight: "100vh", color: TEXT.primary }}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] mt-0.5" style={{ color: TEXT.tertiary }}>Governed orchestration engine — signal → decision → action → audit</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          {awaiting > 0 && (
            <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded" style={{ background: "rgba(139,122,200,0.1)", border: "1px solid rgba(139,122,200,0.2)" }}>
              <UserCheck className="w-3 h-3" style={{ color: "#8b7ac8" }} />
              <span className="text-[10px] font-mono" style={{ color: "#8b7ac8" }}>{awaiting} awaiting approval</span>
            </div>
          )}
          {isLive ? (
      ...
      {/* KPI Strip */}
      <div className="grid grid-cols-6 gap-px rounded overflow-hidden" style={{ background: BORDER.subtle }}>
        {(
          [
            { label: "Workflows", value: dash?.totalWorkflows ?? workflows.length, color: TEXT.primary, sub: "total" },
            { label: "Total Runs", value: dash?.totalRuns ?? runs.length, color: TEXT.primary, sub: "all time" },
            { label: "Running", value: dash?.runningRuns ?? running, color: running > 0 ? ACCENT : TEXT.secondary, sub: "active" },
            { label: "Awaiting Approval", value: awaiting, color: awaiting > 0 ? "#8b7ac8" : TEXT.secondary, sub: "HITL gates", pulse: awaiting > 0 },
            { label: "Failed", value: failed, color: failed > 0 ? "#c45a4a" : "#6b8f71", sub: "recent" },
            { label: "Success Rate", value: `${successRate}%`, color: successRate >= 95 ? "#6b8f71" : successRate >= 80 ? "#c8953c" : "#c45a4a", sub: "7d avg" },
          ] as Array<{ label: string; value: number | string; color: string; sub: string; pulse?: boolean }>
        ).map(kpi => (
          <div key={kpi.label} className="px-3 py-2.5 relative" style={{ background: BG.surface }}>
            <p className="text-[8px] uppercase tracking-widest font-medium mb-1" style={{ color: TEXT.muted }}>{kpi.label}</p>
            <div className="flex items-baseline gap-1">
              <p className="text-xl font-bold font-mono leading-none" style={{ color: kpi.color }}>{kpi.value}</p>
              {kpi.pulse && <span className="w-1.5 h-1.5 rounded-full animate-pulse shrink-0" style={{ background: "#8b7ac8" }} />}
            </div>
            <p className="text-[8px] font-mono mt-0.5" style={{ color: TEXT.muted }}>{kpi.sub}</p>
          </div>
        ))}
      </div>

      {/* Tab navigation */}
      <div className="flex items-center gap-0.5 p-1 rounded w-fit" style={{ background: BG.surface, border: `1px solid ${BORDER.muted}` }}>
        {TABS.map(t => {
          const Icon = t.icon;
          return (
            <button
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {filteredWorkflows.map(wf => {
              ...
              return (
                <div key={wf.id} className="rounded-lg overflow-hidden" style={{ background: BG.surface, border: `1px solid ${BORDER.muted}` }}>
                  <div className="p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {wf.description && (
                            <p className="text-[10px] truncate" style={{ color: TEXT.secondary }}>{wf.description}</p>
                          )}
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <button
                          ...
                          style={{ background: "rgba(255,255,255,0.04)", color: TEXT.secondary, border: `1px solid ${BORDER.subtle}` }}
                        >
            ...
            {filteredWorkflows.length === 0 && (
              <div className="text-center py-10 rounded-lg border" style={{ color: TEXT.tertiary, borderColor: BORDER.muted, background: "rgba(255,255,255,0.01)" }}>
            ...
            <table className="w-full text-[11px]">
              <thead>
                <tr style={{ background: BG.elevated, borderBottom: `1px solid ${BORDER.muted}` }}>
                  {["Run ID", "Workflow", "Status", "Actor", "Started", "Duration", "Actions"].map(h => (
                    <th key={h} className="text-left px-3 py-2 text-[9px] uppercase tracking-wider font-semibold" style={{ color: TEXT.tertiary }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {runs.map((run, idx) => {
                  ...
                  return (
                    <tr key={run.id} style={{ borderBottom: idx < runs.length - 1 ? `1px solid ${BORDER.subtle}` : undefined, background: isApproving ? "rgba(139,122,200,0.04)" : idx % 2 === 0 ? BG.surface : BG.elevated }}>
                      ...
                      <td className="px-3 py-2 font-mono text-[10px]" style={{ color: TEXT.secondary }}>{run.actor ?? "system"}</td>
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-2">
                          {run.status === "awaiting_approval" && (
                            <button className="flex items-center gap-1 px-2 py-0.5 rounded text-[9px]"
            ...
            <div className="space-y-2">
              ...
              {runs.filter(r => r.errorMessage).map(run => (
                <div key={run.id} className="rounded px-3 py-2.5 flex items-start gap-2" style={{ background: "rgba(196,90,74,0.06)", border: "1px solid rgba(196,90,74,0.15)" }}>
                  ...
                  <button className="ml-auto flex items-center gap-1 px-2 py-0.5 rounded text-[9px]"
                    style={{ color: ACCENT, background: "rgba(212,160,84,0.08)", border: "1px solid rgba(212,160,84,0.15)" }}>
                    <RotateCcw className="w-2.5 h-2.5" />
                    Retry
                  </button>
                </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] mt-0.5" style={{ color: TEXT.tertiary }}>Active integrations feeding signals into the Alloy orchestration layer</p>
            </div>
            <button className="flex items-center gap-1.5 px-3 py-1.5 rounded text-[11px] font-medium"
              style={{ background: "rgba(212,160,84,0.1)", border: "1px solid rgba(212,160,84,0.2)", color: ACCENT }}>
              <Plus className="w-3 h-3" />
              Add Connector
            </button>
          ...
          <div className="grid grid-cols-2 gap-px rounded overflow-hidden" style={{ background: BORDER.subtle }}>
            {CONNECTORS.map(c => (
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <div className="text-[8px] font-mono uppercase tracking-wider mt-0.5" style={{ color: TEXT.muted }}>{c.category}</div>
                    </div>
                  </div>
                  <span className="text-[8px] font-mono px-1.5 py-px rounded shrink-0" style={{
                    color: c.status === "connected" ? "#6b8f71" : c.status === "degraded" ? "#c8953c" : "#7c85a0",
                    background: c.status === "connected" ? "rgba(107,143,113,0.08)" : c.status === "degraded" ? "rgba(200,149,60,0.08)" : "rgba(124,133,160,0.08)",
                    border: `1px solid ${c.status === "connected" ? "rgba(107,143,113,0.2)" : c.status === "degraded" ? "rgba(200,149,60,0.2)" : "rgba(124,133,160,0.15)"}`,
                  }}>{c.status}</span>
                </div>
                <div className="mt-2 flex items-center gap-3 text-[8px]" style={{ color: TEXT.muted }}>
          ...
          <div className="rounded-lg p-4" style={{ background: "rgba(212,160,84,0.04)", border: "1px solid rgba(212,160,84,0.1)" }}>
            <div className="flex items-center gap-2 mb-2">
              <Network className="w-3.5 h-3.5" style={{ color: ACCENT }} />
              <span className="text-[10px] font-semibold" style={{ color: ACCENT }}>Signal Routing</span>
            </div>
            <p className="text-[10px] leading-relaxed" style={{ color: TEXT.secondary }}>
              All connector signals are ingested → normalized → scored by PRISM → routed to relevant workflows via policy rules. High-severity signals trigger HITL approval gates before any action is taken.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] mt-0.5" style={{ color: TEXT.tertiary }}>Routing and governance rules that control how signals flow through Alloy</p>
            </div>
            <button className="flex items-center gap-1.5 px-3 py-1.5 rounded text-[11px] font-medium"
              style={{ background: "rgba(212,160,84,0.1)", border: "1px solid rgba(212,160,84,0.2)", color: ACCENT }}>
              <Plus className="w-3 h-3" />
          ...
          <div className="space-y-2">
            {POLICY_RULES.map((rule, i) => (
              <div key={rule.id} className="rounded-lg p-3" style={{ background: BG.surface, border: `1px solid ${rule.active ? BORDER.muted : BORDER.subtle}`, opacity: rule.active ? 1 : 0.6 }}>
                <div className="flex items-start gap-3">
                  <div className="flex items-center justify-center w-5 h-5 rounded font-mono text-[9px] font-bold shrink-0 mt-0.5"
                    style={{ background: "rgba(212,160,84,0.08)", color: ACCENT, border: "1px solid rgba(212,160,84,0.15)" }}>
                    {rule.priority}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className="text-[11px] font-medium text-white">{rule.name}</span>
                      <span className="text-[8px] px-1.5 py-px rounded font-mono shrink-0" style={{
                        color: rule.active ? "#6b8f71" : TEXT.muted,
                        background: rule.active ? "rgba(107,143,113,0.08)" : "rgba(255,255,255,0.03)",
                    ...
                    <div className="grid grid-cols-3 gap-3 text-[9px]">
                      <div>
                        <div className="font-mono uppercase tracking-wider mb-0.5" style={{ color: TEXT.muted }}>Scope</div>
                        <div style={{ color: TEXT.secondary }}>{rule.scope}</div>
                      </div>
                      <div>
                        <div className="font-mono uppercase tracking-wider mb-0.5" style={{ color: TEXT.muted }}>Condition</div>
                        <div className="font-mono" style={{ color: "#d4a054" }}>{rule.condition}</div>
                      </div>
                      <div>
                        <div className="font-mono uppercase tracking-wider mb-0.5" style={{ color: TEXT.muted }}>Action</div>
                        <div style={{ color: TEXT.secondary }}>{rule.action}</div>
                      </div>
                    </div>
                  </div>
                  <button className="shrink-0 text-[9px] px-2.5 py-1.5 rounded font-medium hover:opacity-80"
                    style={{ color: TEXT.tertiary, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
                    Edit
                  </button>
                </div>
              </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] mt-0.5" style={{ color: TEXT.tertiary }}>Append-only audit trail of all orchestration events — tamper-evident</p>
            </div>
            <div className="flex items-center gap-1.5 text-[9px] font-mono px-2.5 py-1.5 rounded" style={{ color: "#6b8f71", background: "rgba(107,143,113,0.06)", border: "1px solid rgba(107,143,113,0.15)" }}>
          ...
          <div className="rounded-lg overflow-hidden" style={{ border: `1px solid ${BORDER.muted}` }}>
            <div className="px-3 py-2" style={{ background: BG.elevated, borderBottom: `1px solid ${BORDER.muted}` }}>
              <div className="grid grid-cols-5 gap-3 text-[9px] uppercase tracking-wider font-semibold" style={{ color: TEXT.tertiary }}>
                <span>Timestamp</span>
                <span>Level</span>
                <span>Actor</span>
            ...
            <div className="divide-y" style={{ borderColor: BORDER.subtle }}>
              {AUDIT_LOG.map((entry, i) => {
                const levelColor = entry.level === "error" ? "#c45a4a" : entry.level === "warn" ? "#c8953c" : TEXT.tertiary;
                return (
                  <div key={entry.id} className="px-3 py-2 hover:bg-white/[0.01] transition-colors">
                    <div className="grid grid-cols-5 gap-3 text-[10px]">
                      ...
                      <span className="font-mono uppercase text-[8px] font-bold" style={{ color: levelColor }}>{entry.level}</span>
                      <span className="font-mono" style={{ color: TEXT.tertiary }}>{entry.actor}</span>
                      <div className="col-span-2">
                        <span style={{ color: TEXT.secondary }}>{entry.event}</span>
                        <div className="text-[8px] mt-0.5 font-mono" style={{ color: TEXT.muted }}>{entry.workflow} · Run #{entry.runId}</div>
                      </div>
                    </div>
                  </div>
                );
          ...
          <div className="text-[9px] font-mono text-center py-2" style={{ color: TEXT.muted }}>
            {AUDIT_LOG.length} entries · Last updated {fmtAgo(AUDIT_LOG[0]?.ts)} · Append-only · Cryptographically signed
          </div>
        </div>
      )}
    </div>
