...
const ROLE_CARDS = [
  {
    ...
    description:
      'Value at risk by function. Forecast drift trends. Top actions required. Coverage ratios. Signal summaries in plain language — no dashboards to decode.',
  ...
  },
  {
    ...
    description:
      'Every stalled workflow. Every unassigned item. Every approval bottleneck. Prioritized action queue by urgency, owner, and value. Signal filters by function.',
  ...
  },
  {
    ...
    description:
      "Deal stage health. Implementation handoff status. SOW signing age. Renewal calendar. All surfaced through the lens of what's moving and what's stuck.",
...
const USE_CASES = [
  {
    ...
    icon: Clock,
    desc: 'Detect when deals stall in legal, finance, or executive sign-off queues — before close dates slip.',
  },
  {
    label: 'Stalled Workflows',
    icon: AlertTriangle,
    desc: 'Surface implementation projects, SOWs, and onboarding queues with no movement and no owner.',
  },
  {
    label: 'Forecast Drift',
    icon: TrendingUp,
    desc: 'Track when committed pipeline reclassifies — before your board call reveals a miss.',
  },
  {
    label: 'Handoff Failures',
    icon: Users,
    desc: 'Catch the moment sales-to-CS, CS-to-delivery, or SE handoffs break — before SLAs are breached.',
  },
  {
    label: 'Pipeline Hygiene',
    icon: BarChart3,
    desc: 'Identify ghost opportunities that inflate reported pipeline and corrupt forecast models.',
  },
  {
    label: 'Revenue Leakage',
    icon: DollarSign,
    desc: 'Surface exposed revenue across approval delays, ownership gaps, and unmanaged exceptions.',
  },
];
...
const SIGNALS_PREVIEW = [
  {
    severity: 'critical',
    type: 'Approval Latency',
    title: 'Enterprise deal approvals averaging 14.2 days — 340% above target',
    var: '$2.1M',
    func: 'Enterprise Sales',
  },
  {
    severity: 'critical',
    type: 'Forecast Drift',
    title: 'Q1 forecast dropped $3.8M in 10 days — commit at risk',
    var: '$3.8M',
    func: 'Revenue',
  },
  {
    ...
    title: '14 implementation projects pending handoff for 8+ days — no assignee',
    var: '$890K',
    func: 'Customer Success',
  },
  {
    severity: 'high',
    type: 'Ownership Gap',
    title: '7 enterprise renewals ($2.3M) have no named owner',
    var: '$2.3M',
    func: 'Account Management',
  },
];
    ...
    <div className="max-w-[1100px] space-y-20 pb-20">
      <section className="pt-6 pb-8 relative">
        ...
        <div className="mt-12 space-y-2">
          <div className="text-[11px] text-slate-600 text-center mb-3 uppercase tracking-wide">
            Live signal preview
          </div>
          {SIGNALS_PREVIEW.map((s, i) => (
            <div
              key={i}
              className={cn(
                'flex items-start gap-3 p-3.5 rounded-xl border text-[12px]',
            ...
            >
              <div
                className={cn(
                  ...
                  s.severity === 'critical' ? 'bg-[#c45a4a] animate-pulse' : 'bg-[#c8953c]',
                )}
              />
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            },
            {
              ...
              accent: 'text-violet-400',
              icon: Eye,
              desc: "Correlated signals become Narrative Intelligence — human-readable paragraphs explaining what's happening, why it matters, and the business consequence. No decoding required.",
            },
            {
              step: '03',
              label: 'Actions',
            ...
            return (
              <div key={card.step} className={cn('rounded-xl p-5 border', card.color)}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div
                    className={cn('font-mono text-[10px] uppercase tracking-widest', card.accent)}
                  >
      ...
      <section>
        <div className="text-center mb-10">
          ...
          <p className="text-slate-400 max-w-xl mx-auto text-sm">
            Every step has a clear function. Nothing is black-box. Every output is explainable.
          </p>
        </div>
        ...
        <div className="space-y-3">
          {HOW_COMMAND_WORKS.map((step) => (
            ...
            >
              <div
                className={cn(
                  'font-mono text-[10px] uppercase tracking-widest shrink-0 pt-0.5 w-16',
                  step.accent,
                )}
              >
      ...
      <section>
        <div className="text-center mb-10">
          ...
          <h2 className="font-display font-bold text-3xl text-white mb-3">
            One platform. Three command surfaces.
          </h2>
          <p className="text-slate-400 max-w-xl mx-auto text-sm">
            KPIs, insights, and actions that change based on who's looking — executive, operations,
            or delivery.
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {ROLE_CARDS.map((r) => (
            <div key={r.role} className={cn('rounded-xl p-5 border', r.color)}>
              <div
                className={cn('text-[10px] font-mono uppercase tracking-widest mb-3', r.labelColor)}
              >
                {r.role}
              </div>
      ...
      <section>
        <div className="text-center mb-10">
          ...
          <h2 className="font-display font-bold text-3xl text-white mb-3">
            Six signal categories that matter.
          </h2>
          <p className="text-slate-400 max-w-xl mx-auto text-sm">
            Each signal type has its own feed, narrative, workflow visualization, and action queue.
          </p>
        </div>
            ...
            return (
              <Link key={u.label} href="/use-cases">
                <div className="rounded-xl p-5 border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] hover:border-white/10 transition-all cursor-pointer h-full">
                  ...
                  <h3 className="font-display font-semibold text-[13px] text-white mb-1.5">
                    {u.label}
                  </h3>
        ...
        <div className="grid grid-cols-4 gap-4 max-w-2xl mx-auto mb-8">
          {[
            ...
            <div key={item.label} className="rounded-lg p-3 border border-white/5 bg-white/[0.02]">
              <div className="font-display font-semibold text-sm text-cyan-400 mb-1">
                {item.label}
              </div>
      ...
      <section className="rounded-2xl border border-white/5 bg-white/[0.02] p-10">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          {[
            {
              metric: '4 min',
              label: 'Detection-to-action time',
              detail: 'Approval queue stall identified and routed to owner in under 4 minutes',
              accent: '#4a90b8',
            },
            {
              metric: '$340K',
              label: 'Revenue recovered / quarter',
              detail: 'Critical approval stall surfaced 8 days before the weekly review would have',
              accent: '#0891b2',
            },
            {
              metric: '8 days',
            ...
            {
              metric: '14',
              label: 'Approval workflows monitored',
              detail:
                'Continuous — across procurement, legal, and finance; ownership gaps auto-detected',
              accent: '#155e75',
            },
            {
              metric: '100%',
              ...
              detail:
                'All 5-day legal review windows maintained; escalation routing prevented breaches',
              accent: '#164e63',
            },
            {
              metric: '3',
              label: 'Ownership gaps found in 48h',
              detail:
        ...
        <a
          ...
          className="text-[11px] text-cyan-500/50 hover:text-cyan-400/70 transition-colors inline-flex items-center gap-1.5"
        >
      ...
      <section className="rounded-2xl border border-cyan-500/20 bg-gradient-to-br from-cyan-500/5 to-blue-500/5 p-10 text-center">
        ...
        <p className="text-slate-400 max-w-xl mx-auto mb-6 text-sm">
          Explore the full platform with real business signals, narrative intelligence, and a
