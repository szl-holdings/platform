...
const ROLE_KPI_CONFIG: Record<
  Role,
  { kpis: string[]; defaultSort: string; label: string; accent: string }
> = {
  executive: {
...
export default function ActionQueuePage() {
  ...
  const actions: any[] =
    rawActions && rawActions.length > 0 ? rawActions : IS_DEMO ? DEMO_ACTIONS : [];

  const transition = useStandardMutation({
    mutationFn: ({ id, state, assignedTo }: { id: number; state: string; assignedTo?: string }) =>
      apiFetch(`/lyte/actions/${id}`, {
        method: 'PATCH',
        body: JSON.stringify({ state, assignedTo }),
      }),
  ...
  const slaBreached = filtered.filter(
    (a: any) => getAgeHours(a.createdAt) > 48 && !['resolved', 'dismissed'].includes(a.state),
  ).length;
    ...
    <div className="flex flex-col gap-6 p-6 min-h-full">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-0.5">
            Business signals → state transitions → resolution
          </p>
        </div>
        ...
        {[
          {
            ...
            color: 'text-[#c45a4a]',
            bg: 'bg-[#c45a4a]/5 border-[#c45a4a]/20',
          },
          {
            label: 'Escalated',
            value: escalated,
            color: 'text-[#c8953c]',
            bg: 'bg-[#c8953c]/5 border-[#c8953c]/20',
          },
          {
            label: 'SLA Breached',
            value: slaBreached,
            color: 'text-[#d4a054]',
            bg: 'bg-[#d4a054]/5 border-[#d4a054]/20',
          },
          {
            label: 'Resolved',
            value: resolved,
            color: 'text-[#6b8f71]',
            bg: 'bg-[#6b8f71]/5 border-[#6b8f71]/20',
          },
      ...
      <div className="space-y-3">
        ...
        {filtered.map((action: any) => {
          const catInfo = SIGNAL_CATEGORY_LABELS[action.signalCategory] || {
            label: action.signalCategory,
            icon: Zap,
            color: 'text-zinc-400',
          };
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <Badge
                        variant="outline"
                        className={STATE_COLORS[action.state as ActionState] || ''}
                      >
                        {action.state}
                      </Badge>
                      <div
                        className={`flex items-center gap-1 text-[10px] font-medium ${catInfo.color}`}
                      >
                    ...
                    <div className="flex items-center flex-wrap gap-3 text-[10px] text-zinc-500">
                      ...
                      {action.assignedTo && (
                        <span className="flex items-center gap-1">
                          <Users className="w-3 h-3" />→ {action.assignedTo}
                        </span>
                      )}
                      {action.valueAtRisk && (
                        <span className="flex items-center gap-1 text-[#d4a054]">
                          <BarChart3 className="w-3 h-3" />
                          {formatCurrency(action.valueAtRisk)} VAR
                        </span>
                      )}
                      {action.workflowStage && (
                        <span
                          className="flex items-center gap-1 px-1.5 py-px rounded font-mono text-[9px]"
                          style={{
                            color: '#8b7ac8',
                            background: 'rgba(139,122,200,0.08)',
                            border: '1px solid rgba(139,122,200,0.18)',
                          }}
                        >
                          {action.workflowStage}
                        </span>
                      ...
                      {action.dueDate &&
                        (() => {
                          ...
                          return (
                            <span
                              className={`flex items-center gap-1 ${isPast ? 'text-[#c45a4a]' : hoursUntil < 12 ? 'text-[#c8953c]' : 'text-zinc-500'}`}
                            >
                    ...
                    {nextStates.map((nextState) => (
                      <Button
                        ...
                        className={cn(
                          ...
                          nextState === 'escalated'
                            ? 'border-[#c45a4a]/30 text-[#c45a4a] hover:bg-[#c45a4a]/10'
                            : nextState === 'resolved'
                              ? 'border-[#6b8f71]/30 text-[#6b8f71] hover:bg-[#6b8f71]/10'
                ...
                {isExpanded && (
                  <div
                    ...
                    style={{
                      ...
                      gridTemplateColumns:
                        evidence.length > 0 && auditHistory.length > 0 ? '1fr 1fr' : '1fr',
                    }}
                  >
                    {evidence.length > 0 && (
                      <div>
