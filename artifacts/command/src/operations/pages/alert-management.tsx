...
const ALERT_RULES: AlertRule[] = [
  {
    ...
    name: 'P99 Latency > 2s',
    service: 'api-gateway',
    condition: 'p99_latency > 2000ms for 5m',
    severity: 'critical',
    enabled: true,
    channels: ['pagerduty', 'slack'],
    lastFired: '12m ago',
    fireCount: 3,
    escalationPolicy: 'EP-001',
  },
  {
    id: 'AR-002',
    name: 'Error Rate > 5%',
  ...
  },
  {
    ...
    name: 'Memory > 90%',
    service: 'worker-pool',
    condition: 'heap_used_pct > 90% for 10m',
    severity: 'high',
    enabled: true,
    channels: ['slack', 'email'],
    lastFired: '4h ago',
    fireCount: 7,
    escalationPolicy: 'EP-002',
  },
  {
    id: 'AR-004',
    name: 'Cache Hit Rate < 50%',
    service: 'cache-layer',
    condition: 'cache_hit_rate < 0.5 for 15m',
    severity: 'high',
    enabled: true,
    channels: ['slack'],
    lastFired: '1d ago',
  ...
  {
    id: 'AR-005',
    name: 'DB Connection Pool < 10%',
    service: 'db-primary',
    condition: 'pool_available_pct < 10% for 3m',
    severity: 'critical',
    enabled: true,
    channels: ['pagerduty', 'slack', 'sms'],
    lastFired: '3d ago',
  ...
  {
    id: 'AR-006',
    name: 'Slow Queries > 10/min',
    service: 'db-primary',
    condition: 'slow_query_rate > 10 per minute',
    severity: 'medium',
    enabled: true,
    channels: ['slack', 'email'],
    lastFired: '2d ago',
    fireCount: 5,
    escalationPolicy: 'EP-003',
  },
  {
    id: 'AR-007',
    name: 'ML Inference Latency',
    service: 'ml-engine',
    condition: 'inference_p95 > 800ms for 10m',
    severity: 'medium',
    enabled: false,
    channels: ['slack'],
    fireCount: 0,
  ...
  },
  {
    ...
    condition: 'auth_failures > 100/min for 3m',
    severity: 'high',
    enabled: true,
...
const ONCALL_SCHEDULES: OnCallSchedule[] = [
  {
    ...
    currentOncall: 'Maria Chen',
    nextOncall: 'James Park',
    rotationType: 'weekly',
    members: ['Maria Chen', 'James Park', 'Kevin Singh', 'Anna Reyes'],
  },
  {
    id: 'SC-002',
    name: 'ML & Data',
    team: 'AI/ML Team',
    currentOncall: 'David Liu',
    nextOncall: 'Priya Patel',
    rotationType: 'weekly',
    members: ['David Liu', 'Priya Patel', 'Carlos Mendez'],
  },
  {
    id: 'SC-003',
    name: 'Security On-Call',
    team: 'Security Ops',
    currentOncall: 'Emma Torres',
    nextOncall: 'Noah Kim',
    rotationType: 'daily',
    members: ['Emma Torres', 'Noah Kim', 'Sara Johnson', 'Mike Zhang'],
  },
];
...
const ACTIVE_ALERTS: ActiveAlert[] = [
  {
    id: 'ALE-001',
    ruleId: 'AR-001',
    ruleName: 'P99 Latency > 2s',
    service: 'api-gateway',
    severity: 'critical',
    message: 'p99 latency reached 3,842ms — threshold: 2,000ms. Affecting 8.4% of requests.',
    firedAt: '12m ago',
    acknowledged: false,
    escalationLevel: 2,
    totalEscalations: 2,
  },
  {
    ...
    service: 'worker-pool',
    severity: 'high',
    message: 'Heap utilization at 92.4% on worker-pool-03. OOM risk if not addressed.',
    firedAt: '1h ago',
    acknowledged: true,
    acknowledgedBy: 'K. Singh',
    escalationLevel: 1,
    totalEscalations: 1,
  },
];
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden" style={{ background: '#080f1c' }}>
      ...
      <div
        ...
        style={{ borderColor: 'rgba(212,160,84,0.12)' }}
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px] text-white/30">
              Multi-channel alerting · escalation policies · on-call schedules
            </p>
          </div>
        </div>
      ...
      {/* Tabs */}
      <div
        className="px-5 border-b flex items-center gap-0 shrink-0"
        style={{ borderColor: 'rgba(212,160,84,0.08)' }}
      >
      ...
      <div className="flex-1 overflow-y-auto p-5">
        ...
        {tab === 'active' && (
          <div className="space-y-4">
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-4 mb-3">
                      ...
                      <div className="shrink-0 flex items-center gap-2">
                        {alert.acknowledged ? (
                          ...
                          <button
                            className="px-2.5 py-1 rounded text-[9px] font-medium border transition-all"
                            style={{
                              borderColor: sev.border,
                              color: sev.color,
                    ...
                    <div className="flex items-center gap-4 text-[9px] text-white/30">
                      <span>
                        Escalation Level {alert.escalationLevel}/{alert.totalEscalations}
                      </span>
            ...
            {/* Recent resolved */}
            <div>
              <h3 className="text-[10px] text-white/30 font-medium mb-3 uppercase tracking-wider">
                Recently Resolved
              </h3>
              <div className="space-y-2">
                {[
                  'Error Rate Spike — payment-svc',
                  'Memory Alert — ml-engine',
            ...
            {ALERT_RULES.map((rule) => {
              ...
              return (
                <div
                  ...
                  style={{
                    borderColor: rule.enabled ? sev.border : 'rgba(255,255,255,0.06)',
                    background: rule.enabled ? sev.bg : 'rgba(255,255,255,0.02)',
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <div
                      className={cn(
                        ...
                        rule.enabled ? 'bg-emerald-500/40' : 'bg-white/10',
                      )}
                    >
                      <div
                        className={cn(
                          'absolute top-0.5 w-3 h-3 rounded-full transition-all',
                          rule.enabled ? 'left-4 bg-emerald-400' : 'left-0.5 bg-white/30',
                        )}
                      />
                    </div>
                  </div>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <p className="text-[10px] text-white/40">
                      {schedule.team} · {schedule.rotationType.replace(/_/g, ' ')} rotation
                    </p>
                  </div>
                  <span
