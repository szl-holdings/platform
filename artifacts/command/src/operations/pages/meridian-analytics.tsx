...
const heatmapData = [
  { hour: '00:00', cpu: 32, mem: 45, net: 18 },
  { hour: '04:00', cpu: 28, mem: 42, net: 12 },
  { hour: '08:00', cpu: 65, mem: 68, net: 45 },
  { hour: '12:00', cpu: 82, mem: 78, net: 67 },
  { hour: '16:00', cpu: 94, mem: 85, net: 78 },
  { hour: '20:00', cpu: 58, mem: 62, net: 42 },
];
...
function HeatCell({ value }: { value: number }) {
  ...
  return (
    <div
      className={`w-full h-8 rounded ${bg} flex items-center justify-center text-[10px] font-mono text-white`}
      style={{ opacity }}
    >
      {value}%
    </div>
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Infrastructure performance metrics and resource utilization
        </p>
      </div>
          ...
          <div key={m.name} className="rounded-xl border border-border bg-card p-4">
            ...
            <div className="flex items-end justify-between">
              <div className="text-2xl font-display font-bold">
                {typeof m.current === 'number' && m.current > 100
                  ? m.current.toLocaleString()
                  : m.current}
                {m.unit}
              </div>
              <span
                className={`flex items-center gap-1 text-xs ${m.trend === 'up' ? 'text-[#d4a054]' : 'text-[#6b8f71]'}`}
              >
                {m.trend === 'up' ? (
            ...
            <div className="mt-2 w-full bg-muted rounded-full h-2">
              <div
                className={`h-2 rounded-full ${typeof m.current === 'number' && m.current > 80 ? 'bg-[#c45a4a]' : m.current > 60 ? 'bg-[#d4a054]' : 'bg-[#6b8f71]'}`}
                style={{
                  width: `${Math.min(typeof m.current === 'number' ? m.current : 50, 100)}%`,
                }}
              />
            </div>
            <div className="flex justify-between mt-1 text-[10px] text-muted-foreground">
              <span>
                Avg: {typeof m.avg === 'number' && m.avg > 100 ? m.avg.toLocaleString() : m.avg}
                {m.unit}
              </span>
              <span>
                Peak:{' '}
                {typeof m.peak === 'number' && m.peak > 100 ? m.peak.toLocaleString() : m.peak}
                {m.unit}
              </span>
            </div>
          </div>
          ...
          <table className="w-full min-w-[500px]">
            <thead>
              <tr className="text-xs text-muted-foreground">
                ...
                {heatmapData.map((d) => (
                  <th key={d.hour} className="text-center py-2">
                    {d.hour}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              <tr>
                ...
                {heatmapData.map((d) => (
                  <td key={d.hour} className="px-1 py-1">
                    <HeatCell value={d.cpu} />
                  </td>
                ))}
              </tr>
              <tr>
          ...
          <tbody className="divide-y divide-border">
            {serviceLatency.map((s) => (
              <tr key={s.service} className="hover:bg-muted/30">
                ...
                <td className="p-4 text-center text-sm font-mono text-muted-foreground">
                  {s.p99}ms
                </td>
                <td className="p-4 text-center">
                  <span
                    className={`px-2 py-0.5 rounded text-xs font-semibold ${s.status === 'healthy' ? 'bg-[#6b8f71]/10 text-[#6b8f71]' : 'bg-[#d4a054]/10 text-[#d4a054]'}`}
                  >
                    {s.status}
                  </span>
                </td>
