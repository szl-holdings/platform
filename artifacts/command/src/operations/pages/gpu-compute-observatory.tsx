...
function GpuCard({ gpu, animate }: { gpu: GpuState; animate: boolean }) {
  const utilColor =
    gpu.status === 'critical' ? '#c45a4a' : gpu.status === 'high' ? '#d4a054' : '#6b8f71';
  const memPct = (gpu.memUsedGb / gpu.memTotalGb) * 100;

  return (
    <div
      className="rounded-xl border p-4 space-y-3"
      style={{ borderColor: `${utilColor}25`, background: `${utilColor}06` }}
    >
      <div className="flex items-start justify-between">
        ...
        <div
          ...
          style={{ background: `${utilColor}15`, border: `1px solid ${utilColor}30` }}
        >
          <span
            className="w-1.5 h-1.5 rounded-full"
            style={{ background: utilColor, boxShadow: animate ? `0 0 6px ${utilColor}` : 'none' }}
          />
          <span className="text-[10px] font-bold font-mono" style={{ color: utilColor }}>
            {gpu.status}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-[10px] font-bold font-mono" style={{ color: utilColor }}>
              {gpu.utilizationPct.toFixed(0)}%
            </span>
          </div>
          <div className="h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
            <div
              className="h-full rounded-full transition-all duration-700"
              style={{ width: `${gpu.utilizationPct}%`, background: utilColor }}
            />
          </div>
        </div>
        <div>
          <div className="flex items-center justify-between mb-1">
            ...
            <span
              ...
              style={{ color: memPct > 85 ? '#c45a4a' : GOLD }}
            >
              {memPct.toFixed(0)}%
            </span>
          </div>
          <div className="h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
            <div
              className="h-full rounded-full transition-all duration-700"
              style={{ width: `${memPct}%`, background: memPct > 85 ? '#c45a4a' : '#4B8BDB' }}
            />
          </div>
        </div>
      </div>
        ...
        {[
          {
            ...
            color: gpu.temperatureC > 80 ? '#c45a4a' : GOLD,
          },
          { label: 'Power', value: `${gpu.powerWatts.toFixed(0)}W`, color: GOLD },
          {
            label: 'Mem',
            value: `${gpu.memUsedGb.toFixed(0)}/${gpu.memTotalGb}G`,
            color: '#4B8BDB',
          },
        ].map((m) => (
          <div
...
function ModelCard({ model }: { model: ModelState }) {
  const driftColor =
    model.driftScore > 25 ? '#c45a4a' : model.driftScore > 15 ? '#d4a054' : '#6b8f71';
  const trendIcon =
  ...
  const trendColor =
    model.trend === 'improving' ? '#6b8f71' : model.trend === 'degrading' ? '#c45a4a' : GOLD;
    ...
    >
        ...
        <div className="flex items-center gap-1">
          ...
          {model.driftAlert && (
            <div
              ...
              style={{ background: 'rgba(196,90,74,0.15)', color: '#c45a4a' }}
            >
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          {
            label: 'Latency P50',
            value: `${model.latencyMs.toFixed(0)}ms`,
            color: model.latencyMs > 800 ? '#c45a4a' : model.latencyMs > 500 ? GOLD : '#6b8f71',
          },
          ...
          {
            label: 'Error Rate',
            value: `${model.errorRate.toFixed(1)}%`,
            color: model.errorRate > 3 ? '#c45a4a' : '#6b8f71',
          },
        ].map((m) => (
          <div
            key={m.label}
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-[10px] font-bold font-mono" style={{ color: driftColor }}>
        ...
        <div className="h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
          <div
            className="h-full rounded-full transition-all duration-700"
            style={{ width: `${Math.min(100, model.driftScore * 3.3)}%`, background: driftColor }}
          />
        </div>
      </div>
...
const MODELS_SEED = [
  ...
  {
    id: 'szl-classify',
    name: 'SZL Classifier v2',
    provider: 'SZL Fine-tuned',
    baseLatency: 180,
    baseDrift: 12,
  },
  {
...
const GPU_SEED = [
  { id: 'gpu-0', name: 'GPU-0 · NVIDIA A100', workload: 'Counsel GPT-4o Inference' },
...
export default function GpuComputeObservatory() {
  const [gpus, setGpus] = useState<GpuState[]>(() => {
    ...
    return GPU_SEED.map((g, i) => {
      ...
      return {
        ...
        history:
          node?.thermalCurve.slice(-20).map((p) => p.celsius) ??
          Array.from({ length: 20 }, () => rng.range(45, 95)),
        status:
          util > 90 ? ('critical' as const) : util > 75 ? ('high' as const) : ('optimal' as const),
      };
    });
  });
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setGpus((prev) =>
        prev.map((g) => {
          const newUtil = Math.max(
            10,
            Math.min(100, g.utilizationPct + (_liveRng.next() - 0.5) * 8),
          );
          const newMem = Math.max(10, Math.min(78, g.memUsedGb + (_liveRng.next() - 0.5) * 3));
          const status: GpuState['status'] =
            newUtil > 90 ? 'critical' : newUtil > 75 ? 'high' : 'optimal';
          return {
            ...g,
      ...
      setModels((prev) =>
        prev.map((m) => {
          const newLatency = Math.max(
            20,
            m.latencyMs + (_liveRng.next() - 0.5) * m.latencyMs * 0.1,
          );
          ...
          return {
            ...
            trend:
              newLatency < m.latencyMs * 0.98
                ? 'improving'
                : newLatency > m.latencyMs * 1.02
                  ? 'degrading'
                  : 'stable',
          };
        }),
    ...
    <div className="max-w-7xl mx-auto space-y-5">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span
              ...
              style={{ background: 'rgba(212,160,84,0.15)', color: GOLD }}
            >
              NVIDIA-CLASS
            </span>
          </div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>
            Real-time GPU utilization, model inference monitoring, drift detection, and compute
            allocation optimization across all AI workloads.
          </p>
        </div>
        <button
        ...
        {[
          {
            ...
            color: totalUtil > 85 ? '#c45a4a' : totalUtil > 70 ? GOLD : '#6b8f71',
            icon: Cpu,
          },
          {
          ...
          {
            label: 'Drift Alerts',
            value: driftAlerts.toString(),
            color: driftAlerts > 0 ? '#c45a4a' : '#6b8f71',
            icon: AlertTriangle,
            pulse: driftAlerts > 0,
          },
          {
            label: 'Compute Cost (est.)',
      ...
      {tab === 'gpu' && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {gpus.map((gpu) => (
            <GpuCard key={gpu.id} gpu={gpu} animate={animate} />
          ))}
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <div className="space-y-3">
              {models
                .sort((a, b) => b.costPerInference - a.costPerInference)
                .map((m) => {
                  ...
                  return (
                    <div key={m.id} className="flex items-center gap-3">
                      ...
                      <div
                        ...
                        style={{ color: GOLD }}
                      >
                        ${m.costPerInference.toFixed(4)}
                      </div>
                      <div
                        className="w-24 text-right font-mono text-[10px]"
                        style={{ color: '#4B8BDB' }}
                      >
                        ${totalModelCost.toFixed(2)} total
                      </div>
                    </div>
                  );
                })}
            ...
            {[
              {
                ...
                desc: 'Counsel GPT-4o handles 40% of requests that could be routed to smaller models. Estimated savings: $120/day',
                color: '#6b8f71',
              },
              {
                label: 'Drift Alert Action',
                desc: 'SZL Classifier v2 drift score 18.4 — re-evaluation recommended. Model performance degrading on financial domain queries.',
                color: '#d4a054',
              },
              {
                label: 'Capacity Forecast',
                desc: 'GPU-0 projected to exceed 95% utilization in 3.2 hours at current inference rate. Recommend spinning up GPU-4.',
                color: '#c45a4a',
              },
            ].map((r) => (
              <div
