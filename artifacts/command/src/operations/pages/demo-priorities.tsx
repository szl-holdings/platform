...
function ScoreBreakdown({ factors }: { factors: (typeof demoPriorities)[0]['scoreFactors'] }) {
  return (
    <div
      className="mt-3 space-y-1.5 pt-3 border-t"
      style={{ borderColor: 'rgba(255,255,255,0.04)' }}
    >
      <div className="text-[8px] uppercase tracking-widest mb-2" style={{ color: TEXT.muted }}>
        Scoring Factors
      </div>
      {factors.map((f) => (
        <div key={f.label} className="grid grid-cols-[1fr_auto_auto] gap-2 items-center">
          ...
          >
            <div
              ...
              style={{
                width: `${f.score}%`,
                background: f.score >= 80 ? '#c45a4a' : f.score >= 60 ? '#d4a054' : '#4a90b8',
              }}
            />
          </div>
          <div className="text-[9px] font-mono text-right" style={{ color: TEXT.tertiary }}>
            {f.score}
          </div>
        </div>
      ))}
...
export default function DemoPrioritiesPage() {
  ...
  const filtered =
    roleFilter === 'all' ? demoPriorities : demoPriorities.filter((p) => p.role === roleFilter);
  ...
  function PriorityCard({ pri }: { pri: (typeof demoPriorities)[0] }) {
    ...
    return (
      <div
        ...
        style={{ background: BG.surface, border: `1px solid rgba(255,255,255,0.05)` }}
      >
        <div
          className="px-4 py-3 cursor-pointer"
          onClick={() => setExpanded(isOpen ? null : pri.id)}
        >
          <div className="flex items-center gap-3">
            <div
              className="w-7 h-7 rounded flex items-center justify-center text-[11px] font-black shrink-0"
              style={{
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <span
                  ...
                  style={{
                    color: sta.color,
                    background: `${sta.color}14`,
                    border: `1px solid ${sta.color}25`,
                  }}
                >
                  {sta.label}
                </span>
            ...
            <div className="text-right shrink-0 mr-2">
              <div className="text-[10px] font-mono font-bold" style={{ color: '#6b8f71' }}>
            ...
            <div className="shrink-0 w-14">
              <div className="text-[7px] text-right mb-0.5 font-mono" style={{ color: TEXT.muted }}>
                Score
              </div>
              <div
              ...
              >
                <div
                  ...
                  style={{ width: `${scoreBar}%`, background: barColor }}
                />
              </div>
              <div className="text-[9px] font-mono text-right mt-0.5" style={{ color: barColor }}>
                {scoreBar}
              </div>
            </div>
            <ChevronDown
              className="w-3.5 h-3.5 shrink-0 transition-transform"
              style={{ color: TEXT.muted, transform: isOpen ? 'rotate(180deg)' : 'none' }}
            />
          </div>
        </div>
        {isOpen && (
          <div className="px-4 pb-4 border-t" style={{ borderColor: 'rgba(255,255,255,0.04)' }}>
            <div className="grid grid-cols-3 gap-3 mt-3">
              ...
              <div>
                ...
                <div className="text-[10px] font-medium capitalize" style={{ color: TEXT.primary }}>
                  {pri.role}
                </div>
              </div>
              <div>
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: TEXT.secondary }}>
            Ranked by composite impact score — value at risk, urgency, and probability decay
          </p>
        </div>
        <div
          ...
          {ROLES.map((r) => (
            <button
              ...
              style={{
                ...
                border:
                  roleFilter === r.key ? '1px solid rgba(212,160,84,0.2)' : '1px solid transparent',
              }}
            >
              {r.label}
            </button>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: 'Due Today', value: today.length, color: '#c8953c' },
          {
            ...
            value: `$${(filtered.reduce((a, p) => a + p.valueProtected, 0) / 1_000_000).toFixed(1)}M`,
            color: '#6b8f71',
          },
        ].map((c) => (
          <div
