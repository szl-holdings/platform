...
import { Shield, AlertTriangle, Lock, Network, Activity, Eye, ChevronRight, Wifi, Server, RefreshCw } from "lucide-react";
import { IMPERIUM_DATA, INTELLIGENCE_BRIEFS, getThreatColor, type ThreatLevel } from "@imp/lib/imperium-data";
  ...
  return (
    <div className="imperial-card rounded-xl p-6">
      <div className="flex items-center gap-2 mb-5">
        ...
        <span className="font-display text-xs tracking-[0.2em] gold-text uppercase">Threat Condition</span>
      </div>
          ...
          {THREAT_LEVELS.map((level, i) => {
            ...
            return (
              <div key={level} className="flex-1">
                <div
                  className={cn("h-3 rounded transition-all duration-500", isActive && "animate-pulse")}
                  style={{
  ...
  return (
    <div className="flex items-start justify-between py-2.5 border-b border-white/5 last:border-0">
      <div className="flex items-center gap-2">
        <div className={cn("w-2 h-2 rounded-full flex-shrink-0 mt-0.5", ok ? "bg-green-400" : "bg-orange-400")} />
        <div>
          <div className="text-xs text-slate-300">{label}</div>
          {detail && <div className="text-[10px] text-slate-600 mt-0.5">{detail}</div>}
        </div>
      </div>
      <span className={cn("text-xs font-mono tracking-wide", ok ? "text-green-400" : "text-orange-400")}>{value}</span>
    </div>
  );
}

function ThreatFeedEntry({ indicator }: { indicator: typeof INTELLIGENCE_BRIEFS.threat.indicators[0] }) {
  ...
  return (
    <div className="flex items-start gap-3 py-2.5 border-b border-white/5 last:border-0 animate-data-stream">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[10px] text-slate-500 mt-0.5 leading-relaxed">{indicator.description}</div>
      </div>
      <div className="font-mono text-xs text-slate-500 flex-shrink-0">{indicator.count.toLocaleString()}</div>
    </div>
  );
}
  ...
  return (
    <div className="imperial-card rounded-lg p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="font-display text-xs tracking-[0.15em] gold-text uppercase">Auto-Hardening Recommendations</span>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="imperial-card rounded-lg p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-display text-xs tracking-[0.12em] text-green-400 uppercase">WAF Status</span>
          </div>
          <StatusRow label="Prevention Mode" value="ACTIVE" ok={true} detail="DRS 2.1 + Bot Manager 1.1" />
          ...
          <StatusRow label="Requests Blocked (24h)" value="847" ok={true} detail="WAF successfully intercepted" />
          <StatusRow label="Rate Limit Threshold" value="WARNING" ok={false} detail="Recommend: reduce to 500/min" />
        </div>

        <div className="imperial-card rounded-lg p-4">
          <div className="flex items-center gap-2 mb-3">
            <Lock className="w-4 h-4 text-green-400" />
            <span className="font-display text-xs tracking-[0.12em] text-green-400 uppercase">VNet Isolation</span>
          </div>
          <StatusRow label="PostgreSQL" value="PRIVATE" ok={true} detail="Delegated subnet 10.0.2.0/24" />
          <StatusRow label="Redis Cache" value="PRIVATE" ok={true} detail="Private endpoint 10.0.3.x" />
          <StatusRow label="Key Vault" value="PRIVATE" ok={true} detail="Private endpoint + RBAC" />
          <StatusRow label="Blob Storage" value="PRIVATE" ok={true} detail="Private endpoint 10.0.3.x" />
          <StatusRow label="Service Bus" value="PUBLIC" ok={false} detail="Senate proposal #005 pending" />
        </div>

        <div className="imperial-card rounded-lg p-4">
          <div className="flex items-center gap-2 mb-3">
            <Eye className="w-4 h-4 text-green-400" />
            <span className="font-display text-xs tracking-[0.12em] text-green-400 uppercase">Front Door Health</span>
          </div>
          ...
          <StatusRow label="Origin Health" value="100%" ok={true} detail="API container app healthy" />
          <StatusRow label="Custom Domain" value="VERIFIED" ok={true} detail="szlholdings.com managed cert" />
        </div>
      </div>
      ...
      <div className="imperial-card rounded-lg p-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-display text-xs tracking-[0.15em] gold-text uppercase">Threat Intelligence Feed</span>
      ...
      <div className="imperial-card rounded-lg p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-display text-xs tracking-[0.15em] gold-text uppercase">Sovereign Key Vault</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            <div key={label} className="rounded p-3 text-center" style={{ background: "rgba(74,222,128,0.06)", border: "1px solid rgba(74,222,128,0.15)" }}>
