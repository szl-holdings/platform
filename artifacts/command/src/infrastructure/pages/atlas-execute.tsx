...
import {
  ...
  Play, FileText, Activity, Layers, XCircle, Info, ChevronDown, ChevronUp, Server
} from "lucide-react";

const DOMAIN = "imperium";
const ACCENT = "#dc2626";
const DOMAIN_LABEL = "IMPERIUM — Infrastructure Governance";
const WORKFLOW_KEY = "imperium-remediation";

const SIGNAL_TYPES = ["cost-anomaly", "policy-drift", "sovereignty-risk", "capacity-breach", "security-misconfiguration", "compliance-gap"];
...
export default function ImperiumAtlasExecute() {
  ...
  const [newSignal, setNewSignal] = useState({ signalType: SIGNAL_TYPES[0], severity: "high" as Severity, title: "", description: "", source: "ops-controller", confidence: 0.85 });
  ...
  const ingestSignal = async () => {
    ...
    try {
      ...
      setNewSignal({ signalType: SIGNAL_TYPES[0], severity: "high", title: "", description: "", source: "ops-controller", confidence: 0.85 });
      await load();
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>
            Ingest infrastructure signals, run governed remediation workflows, review execution evidence, and approve sovereign actions.
          </p>
        </div>
      ...
      {pendingApproval && (
        <div className="rounded-xl border p-4 flex items-start gap-4" style={{ borderColor: `${ACCENT}40`, background: `${ACCENT}10` }}>
          ...
          <div className="flex-1">
            <div className="text-[11px] font-bold mb-0.5" style={{ color: ACCENT }}>Governor Approval Required</div>
            <div className="text-[11px] text-white mb-1">{pendingApproval.reason ?? "This infrastructure action requires governor approval before execution continues."}</div>
          ...
          <div className="rounded-xl border p-4 space-y-3" style={{ borderColor: "rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.02)" }}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-bold uppercase tracking-widest font-mono text-white">Infrastructure Signals</span>
            ...
            {showIngestForm && (
              <div className="rounded-lg border p-3 space-y-3" style={{ borderColor: `${ACCENT}30`, background: `${ACCENT}08` }}>
                ...
                <input value={newSignal.title} onChange={e => setNewSignal(s => ({ ...s, title: e.target.value }))}
                  placeholder="Infrastructure anomaly or governance event…"
                  className="w-full text-[11px] rounded-lg px-2 py-1.5 outline-none"
                  style={{ background: "rgba(255,255,255,0.05)", color: "white", border: "1px solid rgba(255,255,255,0.1)" }} />
                <textarea value={newSignal.description} onChange={e => setNewSignal(s => ({ ...s, description: e.target.value }))}
                  placeholder="System, region, policy context, impact estimate…"
                  rows={2}
            ...
            {loading ? (
              ...
              <div className="flex items-center gap-2 py-4" style={{ color: "rgba(255,255,255,0.3)" }}>
                <Info className="w-3.5 h-3.5" /><span className="text-[11px]">No signals yet. Ingest an infrastructure event to begin.</span>
              </div>
            ) : (
