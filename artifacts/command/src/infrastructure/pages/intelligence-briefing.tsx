...
import { Radio, DollarSign, TrendingUp, Shield, Activity, ChevronRight, AlertTriangle, Clock } from "lucide-react";
import { INTELLIGENCE_BRIEFS, getClassificationColor, getThreatColor } from "@imp/lib/imperium-data";
...
function BriefSection({ title, icon: Icon, classification, children }: {
  title: string; icon: React.ElementType; classification: any; children: React.ReactNode;
}) {
  ...
  return (
    <div className="imperial-card rounded-lg overflow-hidden">
      ...
      >
        ...
        <span className="font-display text-xs tracking-[0.15em] gold-text font-semibold uppercase flex-1">{title}</span>
...
export default function IntelligenceBriefing() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg px-4 py-2 flex items-center justify-between border"
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-display text-xs tracking-[0.15em] text-green-400 uppercase">Operational Summary</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { label: "SLA Compliance", value: `${operational.apiSlaCompliance}%`, color: "#4ade80" },
      ...
      <BriefSection title={cost.title} icon={DollarSign} classification={cost.classification}>
        <div className="space-y-4">
          <div className="flex items-start justify-between">
            <div>
              <div className="font-mono text-3xl font-bold gold-text">${cost.totalMonthly.toLocaleString()}</div>
              <div className="text-xs text-slate-500 mt-0.5">Monthly spend · Trend: {cost.trend}</div>
            </div>
            <div className="text-right">
              <div className="font-mono text-sm text-slate-400">${cost.forecast90Days.toLocaleString()}</div>
              <div className="text-[10px] text-slate-600">90-day forecast</div>
            </div>
          </div>

          <div>
            <div className="text-[10px] font-mono text-slate-500 uppercase tracking-wider mb-2">Top Cost Consumers</div>
            <div className="space-y-2">
              {cost.topConsumers.map((item) => (
                <div key={item.resource} className="flex items-center gap-3">
                  ...
                  <div className="font-mono text-xs text-gold-dim w-16 text-right">${item.cost}/mo</div>
                  <div className="font-mono text-[10px] w-10 text-right" style={{ color: item.trend === "+0%" ? "#4ade80" : "#fb923c" }}>
                    {item.trend}
                  </div>
                </div>
              ))}
      ...
      <BriefSection title={capacity.title} icon={TrendingUp} classification={capacity.classification}>
        <div className="space-y-4">
          ...
          <div>
            <div className="text-[10px] font-mono text-slate-500 uppercase tracking-wider mb-2">Capacity Bottlenecks</div>
                ...
                return (
                  <div key={b.resource}>
                    <div className="flex justify-between text-xs mb-1.5">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-mono" style={{ color }}>{b.current}{b.unit}</span>
                        {b.daysToCapacity && (
                          <span className="text-slate-600 text-[10px]">~{b.daysToCapacity}d to capacity</span>
                        )}
                      </div>
                    </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-400">
              <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: getThreatColor(threat.currentLevel) }} />
          ...
          <div className="grid grid-cols-1 gap-2">
            {threat.indicators.map((ind, i) => {
              ...
              return (
                <div key={i} className="flex items-start gap-3 rounded p-2.5 bg-white/3 border border-white/5">
                  <div className="px-1.5 py-0.5 rounded font-mono text-[9px] tracking-wider border flex-shrink-0"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded p-2.5 text-center bg-white/3">
              <div className="font-mono text-xs font-bold text-green-400">{threat.wafStatus.split(" — ")[0]}</div>
