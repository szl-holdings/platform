...
import {
  Users, Plus, Edit2, Check, X, AlertTriangle, Clock,
  Shield, Zap, Scale, Anchor
} from "lucide-react";
import {
  INITIAL_COALITION, type CoalitionPartner, type CoalitionStatus,
  type Classification
...
const STATUS_CONFIG: Record<CoalitionStatus, { label: string; color: string; bg: string; border: string }> = {
  ACTIVE: { label: "ACTIVE", color: "#4ade80", bg: "rgba(74,222,128,0.08)", border: "rgba(74,222,128,0.25)" },
  OBSERVING: { label: "OBSERVING", color: "#60a5fa", bg: "rgba(96,165,250,0.08)", border: "rgba(96,165,250,0.25)" },
  SUSPENDED: { label: "SUSPENDED", color: "#facc15", bg: "rgba(250,204,21,0.08)", border: "rgba(250,204,21,0.25)" },
  TERMINATED: { label: "TERMINATED", color: "#475569", bg: "rgba(71,85,105,0.05)", border: "rgba(71,85,105,0.2)" },
};
...
function TrustBar({ score }: { score: number }) {
  const color = score >= 90 ? "#4ade80" : score >= 75 ? "#facc15" : score >= 60 ? "#fb923c" : "#ef4444";
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="font-mono text-xs w-7 text-right" style={{ color }}>{score}</span>
    </div>
  );
}
      ...
      <div className="space-y-2 mb-3">
        ...
        <div className="flex items-center justify-between">
          ...
          {editing ? (
            ...
            >
              {(["ACTIVE", "OBSERVING", "SUSPENDED", "TERMINATED"] as CoalitionStatus[]).map((s) => (
...
function AddPartnerModal({ onClose, onSubmit }: { onClose: () => void; onSubmit: (p: CoalitionPartner) => void }) {
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            ...
            <span className="font-display text-sm tracking-[0.15em] gold-text uppercase">Add Coalition Partner</span>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-[10px] font-mono tracking-wider text-slate-500 mb-1.5">PARTNER NAME *</label>
            <input
              type="text"
              value={form.name}
          ...
          <div>
            <label className="block text-[10px] font-mono tracking-wider text-slate-500 mb-1.5">ROLE / FUNCTION *</label>
            <input
              type="text"
              value={form.role}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] font-mono tracking-wider text-slate-500 mb-1.5">DOMAIN</label>
              <select
                value={form.domain}
              ...
              >
                {["Security", "Finance", "Legal", "Operations", "Engineering", "Compliance"].map((d) => (
          ...
          <div>
            <label className="block text-[10px] font-mono tracking-wider text-slate-500 mb-1.5">NOTES</label>
            <textarea
              value={form.notes}
...
export default function Coalition() {
  ...
  const filtered = filterDomain === "ALL" ? partners : partners.filter((p) => p.domain === filterDomain);
  ...
  return (
    <div className="space-y-6">
      ...
      {showModal && (
        <AddPartnerModal onClose={() => setShowModal(false)} onSubmit={handleAdd} />
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: "Active", value: partners.filter((p) => p.status === "ACTIVE").length, color: "#4ade80" },
          { label: "Alerts", value: partners.reduce((s, p) => s + p.alerts, 0), color: "#ef4444" },
          { label: "Avg Trust", value: `${avgTrust}/100`, color: avgTrust >= 90 ? "#4ade80" : avgTrust >= 75 ? "#facc15" : "#fb923c" },
