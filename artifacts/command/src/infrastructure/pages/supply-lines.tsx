...
import { Network, Zap, AlertTriangle, TrendingUp, Activity, ArrowRight, Wifi } from "lucide-react";
import { SUPPLY_ROUTES, getClassificationColor, type Classification } from "@imp/lib/imperium-data";
...
function LatencyBar({ ms, max = 1500 }: { ms: number; max?: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="font-mono text-[10px] w-12 text-right" style={{ color }}>{ms}ms</span>
    </div>
  );
}
...
function ThroughputBar({ rps, max = 3500 }: { rps: number; max?: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="font-mono text-[10px] w-14 text-right text-blue-400">{rps.toLocaleString()} RPS</span>
    </div>
  );
}
...
function ErrorBadge({ rate }: { rate: number }) {
  ...
  const color = rate === 0 ? "#4ade80" : rate < 0.01 ? "#a3e635" : rate < 0.05 ? "#facc15" : "#ef4444";
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-3">
        <div
          ...
          style={{ backgroundColor: isSevered ? "#ef4444" : classColor, boxShadow: `0 0 6px ${isSevered ? "#ef4444" : classColor}80` }}
        />
        ...
        {isSevered && <AlertTriangle className="w-4 h-4 text-red-400 flex-shrink-0 animate-pulse" />}
      </div>

      {/* Protocol */}
      <div className="flex items-center justify-between mb-3">
        <span className="font-mono text-[10px] text-slate-500 bg-white/3 px-2 py-0.5 rounded">{route.protocol}</span>
...
function TopologyDiagram() {
  ...
  const edges = [
    { from: "fd", to: "api" }, { from: "fd", to: "swa" },
    { from: "api", to: "pg" }, { from: "api", to: "redis" },
    { from: "api", to: "kv" }, { from: "api", to: "sb" },
  ];
  ...
  return (
    <div className="imperial-card rounded-lg p-4 mb-4">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="font-display text-xs tracking-[0.15em] gold-text uppercase">Supply Route Topology</span>
      </div>
      <div className="relative w-full" style={{ paddingBottom: "50%", minHeight: "200px" }}>
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 75">
          ...
          {nodes.map((node) => (
            <g key={node.id}>
              <circle cx={node.x} cy={node.y} r="4" fill={`${node.color}25`} stroke={node.color} strokeWidth="0.5" />
              <text x={node.x} y={node.y + 0.5} textAnchor="middle" dominantBaseline="middle"
                fontSize="2" fill={node.color} fontFamily="monospace" fontWeight="bold">
                {node.label.slice(0, 4).toUpperCase()}
              </text>
              <text x={node.x} y={node.y + 6} textAnchor="middle" fontSize="1.8" fill="rgba(148,163,184,0.6)" fontFamily="monospace">
                {node.label}
              </text>
            </g>
          ))}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: "Severed Lines", value: severed.length, color: severed.length > 0 ? "#ef4444" : "#4ade80", icon: AlertTriangle },
          { label: "Total Throughput", value: `${(SUPPLY_ROUTES.reduce((a, r) => a + r.throughputRps, 0) / 1000).toFixed(1)}K RPS`, color: "#60a5fa", icon: TrendingUp },
          { label: "Avg Latency", value: `${Math.round(SUPPLY_ROUTES.filter(r => r.latencyMs < 500).reduce((a, r) => a + r.latencyMs, 0) / active.length)}ms`, color: "#c9a227", icon: Activity },
      ...
      {severed.length > 0 && (
        <div className="rounded-lg p-3 border border-red-900/50 bg-red-950/20">
          ...
          {severed.map((r) => (
            <div key={r.id} className="text-xs text-red-300">{r.from} → {r.to}</div>
          ))}
        </div>
      )}
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="font-display text-xs tracking-[0.15em] gold-text uppercase">All Supply Routes ({SUPPLY_ROUTES.length})</span>
        </div>
