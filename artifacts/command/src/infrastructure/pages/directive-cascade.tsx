...
import {
  Zap, Plus, ChevronDown, ChevronUp, Send, Pause, Archive,
  RefreshCw, X
} from "lucide-react";
import {
  INITIAL_DIRECTIVES, type Directive, type DirectiveStatus, type DirectivePriority,
  type Classification
...
const COHORT_OPTIONS = ["GROUP — SECURITY", "GROUP — COMPUTE", "GROUP — DATA", "GROUP — FRONTEND", "REGION I — PRIMARY", "REGION II — STANDBY"];
...
function DirectiveCard({
  ...
  const [expanded, setExpanded] = useState(directive.status === "ACTIVE" || directive.status === "CASCADING");
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 flex-wrap">
            <span className="font-semibold text-sm text-slate-200 leading-tight">{directive.title}</span>
          ...
          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
            ...
            <span className="text-[10px] text-slate-500">{timeAgo}h ago · {directive.issuedBy}</span>
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-white/5 pt-3 space-y-3">
          ...
          {directive.cascadedTo.length > 0 && (
            <div>
              <div className="text-[10px] font-mono text-slate-500 tracking-wider mb-1.5">CASCADED TO</div>
              <div className="flex flex-wrap gap-1.5">
                {directive.cascadedTo.map((g) => (
                  <span key={g} className="px-2 py-0.5 rounded text-[10px] font-mono bg-blue-950/40 border border-blue-900/30 text-blue-400">
                    {g}
                  </span>
                ))}
              </div>
          ...
          {directive.tags.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {directive.tags.map((t) => (
                <span key={t} className="px-2 py-0.5 rounded text-[10px] font-mono bg-white/3 border border-white/5 text-slate-400">
                  #{t}
                </span>
              ))}
            </div>
...
function NewDirectiveModal({ onClose, onSubmit }: { onClose: () => void; onSubmit: (d: Directive) => void }) {
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            ...
            <span className="font-display text-sm tracking-[0.15em] gold-text uppercase">Issue New Directive</span>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-[10px] font-mono tracking-wider text-slate-500 mb-1.5">DIRECTIVE TITLE *</label>
            <input
              type="text"
              value={form.title}
          ...
          <div>
            <label className="block text-[10px] font-mono tracking-wider text-slate-500 mb-1.5">DIRECTIVE BODY *</label>
            <textarea
              value={form.body}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] font-mono tracking-wider text-slate-500 mb-1.5">PRIORITY</label>
              <select
                value={form.priority}
                onChange={(e) => setForm((f) => ({ ...f, priority: e.target.value as DirectivePriority }))}
            ...
            <div>
              <label className="block text-[10px] font-mono tracking-wider text-slate-500 mb-1.5">CLASSIFICATION</label>
              <select
          ...
          <div>
            <label className="block text-[10px] font-mono tracking-wider text-slate-500 mb-1.5">CASCADE TO (select groups)</label>
          ...
          <div>
            <label className="block text-[10px] font-mono tracking-wider text-slate-500 mb-1.5">TAGS (comma-separated)</label>
            <input
              type="text"
              value={form.tags}
...
export default function DirectiveCascade() {
  ...
  function handleCascade(id: string) {
    setDirectives((prev) =>
      prev.map((d) =>
        d.id === id
          ? { ...d, status: "CASCADING", cascadeCount: d.cascadeCount + 1 }
          : d
      )
    );
  ...
  return (
    <div className="space-y-6">
      ...
      {showModal && (
        <NewDirectiveModal onClose={() => setShowModal(false)} onSubmit={handleAdd} />
      )}
        ...
        {(["ACTIVE", "CASCADING", "SUSPENDED", "ARCHIVED"] as DirectiveStatus[]).map((s) => {
          ...
          return (
            <div key={s} className="imperial-card rounded-lg p-3 text-center">
              <div className="font-mono text-2xl font-bold" style={{ color: cfg.color }}>{counts[s]}</div>
