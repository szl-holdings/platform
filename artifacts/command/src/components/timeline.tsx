...
import type { TimelineEvent, DomainId, SeverityLevel } from "../types";
...
const DOMAIN_OPTIONS: DomainId[] = ["aegis", "vessels", "szl", "lyte", "prism", "terra", "carlota", "stephen"];
const PAGE_SIZE = 8;
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-wrap gap-1.5">
          {domainsPresent.map((d) => {
            ...
            return (
              <button
                key={d}
                onClick={() => { setDomainFilter(active ? null : d); setPage(1); }}
        ...
        <div className="flex flex-wrap gap-1.5">
          {SEVERITY_OPTIONS.map((s) => {
            ...
            return (
              <button
                key={s}
                onClick={() => { setSeverityFilter(active ? null : s); setPage(1); }}
                className="text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 rounded transition-colors"
                style={{
                  color: active ? "var(--color-bg-primary)" : color,
                  backgroundColor: active ? color : `color-mix(in srgb, ${color} 12%, transparent)`,
      ...
      <div className="overflow-y-auto p-4 flex-1 flex flex-col gap-4 max-h-[600px]">
        {visible.length === 0 ? (
          <div className="flex-1 flex items-center justify-center text-xs" style={{ color: "var(--color-fg-muted)" }}>
