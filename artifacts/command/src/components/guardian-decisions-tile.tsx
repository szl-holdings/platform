...
import { ShieldCheck, ShieldAlert, ShieldX, CheckCircle2, Clock, ExternalLink, AlertTriangle } from "lucide-react";
...
const DECISION_STYLE: Record<Decision, { fg: string; bg: string; border: string }> = {
  ...
  "require-approval": { fg: "#d4a054", bg: "rgba(212,160,84,0.10)", border: "rgba(212,160,84,0.30)" },
  "require-dual-approval": { fg: "#f97316", bg: "rgba(249,115,22,0.10)", border: "rgba(249,115,22,0.30)" },
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col min-w-0">
        <span className="text-[18px] font-bold leading-tight" style={{ color: "var(--color-fg-primary)" }}>
...
export function GuardianDecisionsTile() {
  ...
  const counts = data?.counts ?? { allow: 0, requireApproval: 0, requireDualApproval: 0, block: 0, total: 0 };
  ...
  return (
    ...
    >
      ...
      {q.isLoading ? (
        <div className="text-[11px] font-mono py-6 text-center" style={{ color: "var(--color-fg-muted)" }}>
          Loading…
        </div>
      ) : q.error ? (
        <div
