...
import { Play, X, Clock, Sparkles, ArrowRight } from "lucide-react";
import { useDemoMode, DEMO_SCENARIOS, type DemoScenarioKey } from "@lyte/lib/demo-mode";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "#d4a054" }}>
          Demo Launchpad
        </span>
      </div>
          ...
          return (
            <button
              ...
              style={{
                ...
                padding: "12px 14px",
                borderRadius: 10,
                background: isActive ? "color-mix(in srgb, #d4a054 12%, transparent)" : "var(--color-bg-primary)",
                border: `1px solid ${isActive ? "color-mix(in srgb, #d4a054 40%, transparent)" : "var(--color-surface-border)"}`,
                cursor: "pointer",
              }}
            >
              <div className="flex items-center gap-2 mb-1">
                <Clock className="w-3 h-3" style={{ color: isActive ? "#d4a054" : "var(--color-fg-muted)" }} />
                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: isActive ? "#d4a054" : "var(--color-fg-primary)" }}>
                  {r.label}
                </span>
              </div>
              <p className="text-[11px]" style={{ color: "var(--color-fg-muted)" }}>{r.description}</p>
      ...
      <div className="mb-4">
        <p className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: "var(--color-fg-muted)" }}>
          ...
          {DEMO_SCENARIOS.map((s) => {
            ...
            return (
              <button
                ...
                style={{
                  backgroundColor: isActive ? `color-mix(in srgb, ${s.color} 15%, transparent)` : "var(--color-bg-primary)",
