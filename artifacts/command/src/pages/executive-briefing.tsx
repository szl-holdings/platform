...
import { ArrowLeft, AlertTriangle, Clock, CheckCircle2, TrendingDown, Shield } from "lucide-react";
import { useEcosystemData } from "../hooks/use-ecosystem-data";
import { getSeverityColor, getDomainColor } from "../lib/utils";
...
export function ExecutiveBriefing() {
  ...
  const criticalEvents = data.timeline.filter((e) => e.severity === "critical" || e.severity === "high").slice(0, 5);
  const pendingApprovals = data.actions.filter((a) => a.priority === "critical" || a.priority === "high");
  const criticalIntel = data.intelligence.filter((c) => c.severity === "critical" || c.severity === "high");

  const scoreColor = (score: number) =>
    score >= 80 ? "var(--color-low)" :
    score >= 65 ? "var(--color-medium)" :
    "var(--color-critical)";
    ...
    >
      ...
      <main className="flex-1 p-6 lg:p-8 max-w-[1200px] mx-auto w-full flex flex-col gap-8">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "var(--color-fg-muted)" }}>{dateStr}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            ...
            <div className="text-4xl font-bold font-mono" style={{ color: scoreColor(data.compositeScore) }}>
              {data.compositeScore}
            </div>
            <div
          ...
          >
            ...
            <div className="text-4xl font-bold font-mono" style={{ color: "var(--color-critical)" }}>
              {criticalDomains.length}
            </div>
            <div className="text-xs" style={{ color: "var(--color-fg-muted)" }}>
              {criticalDomains.length === 0 ? "All domains nominal" : `Domain${criticalDomains.length > 1 ? "s" : ""} below threshold`}
            </div>
          </div>
          ...
          >
            ...
            <div className="text-4xl font-bold font-mono" style={{ color: pendingApprovals.length > 0 ? "var(--color-high)" : "var(--color-low)" }}>
              ...
              {[...criticalDomains, ...warningDomains, ...nominalDomains].map((domain) => (
                ...
                >
                  <div className="flex-1 flex flex-col gap-0.5">
                    ...
                    <span className="text-xs" style={{ color: "var(--color-fg-muted)" }}>{domain.status}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-lg font-bold font-mono w-10 text-right" style={{ color: scoreColor(domain.score) }}>
                      {domain.score}
                    </div>
                  </div>
                </a>
                ...
                >
                  {criticalEvents.map((event, i) => (
                    <div
                      ...
                      style={{
                        borderBottom: i < criticalEvents.length - 1 ? "1px solid var(--color-surface-border)" : "none",
                      }}
                    >
                      <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-wider">
                        ...
                        <span style={{ color: "var(--color-fg-muted)", marginLeft: "auto" }}>{event.time}</span>
                      </div>
                      <p className="text-sm font-semibold" style={{ color: "var(--color-fg-primary)" }}>{event.title}</p>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  {pendingApprovals.map((action, i) => {
                    ...
                    return (
                      <div
                        ...
                        style={{ borderBottom: i < pendingApprovals.length - 1 ? "1px solid var(--color-surface-border)" : "none" }}
                      >
                        <div className="w-1 h-8 rounded-full shrink-0" style={{ backgroundColor: sColor }} />
                        <div className="flex-1 flex flex-col gap-0.5">
                          <span className="text-[10px] font-mono uppercase tracking-wider" style={{ color: getDomainColor(action.domain) }}>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <h3 className="font-bold text-sm leading-tight" style={{ color: "var(--color-fg-primary)" }}>{card.title}</h3>
                        <span
                          className="text-[10px] font-bold uppercase px-1.5 py-0.5 rounded shrink-0"
                          style={{ color: sColor, backgroundColor: `color-mix(in srgb, ${sColor} 12%, transparent)` }}
                        >
                          {card.severity}
                        </span>
                      </div>
                      <p className="text-xs leading-relaxed" style={{ color: "var(--color-fg-muted)" }}>{card.description}</p>
                      <div className="text-xs font-medium" style={{ color: sColor }}>→ {card.action}</div>
                    </div>
                  );
                })}
              </div>
