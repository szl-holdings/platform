...
function buildDigest(role: Role, compositeScore: number): DigestSection[] {
  const base: DigestSection[] = [
    ...
    },
    {
      ...
      detail:
        'AI Model Governance Policy v1.0 submitted for CISO review. Legal has approved. 2 approvals remaining before activation. Also: Q1 data retention audit complete, no violations.',
      actions: [
        { label: 'Review Policy', href: '/governance' },
        { label: 'Audit Trail', href: '/governance' },
      ],
  ...
  return base.filter((s) => s.relevantFor.includes(role)).sort((a, b) => a.priority - b.priority);
}
        ...
        >
          <div>
            ...
            <p className="text-xs" style={{ color: 'var(--color-fg-muted)' }}>
              {timestamp} · Personalized for: {ROLE_LABELS[role]}
            </p>
          </div>
        ...
        >
          ...
          <div className="flex-1">
            <div className="text-xs" style={{ color: 'var(--color-fg-muted)' }}>
              Digest is personalized based on your role, recent activity, and ecosystem signal
              priority weights. {sections.length} sections relevant to {ROLE_LABELS[role]} view.
            </div>
          </div>
          <a href="/alerts" className="text-xs font-semibold" style={{ color: '#8b7ac8' }}>
            Full Alert Inbox →
          </a>
        </div>
      </div>
