...
export default function EvalLab() {
  ...
  async function handleRunEval(suite: EvalSuiteConfig) {
    ...
    try {
      const result = await fetchJson<{ runId: string; suiteId: string; suiteName: string; domain: string; passRate: number; avgScore: number; totalCases: number; passed: number; failed: number; hasRegression: boolean; regressionSeverity: "none" | "minor" | "major" | "critical"; regressionNotes: string[]; improvementNotes: string[]; runAt: string }>(
        apiUrl("/evals/run"),
        {
          method: "POST",
          ...
          <>
            ...
            {activeTab === "suites" && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 20 }}>
                <div>
                  {suites.length === 0 && (
                    <p style={{ textAlign: "center", color: "#475569", fontSize: 13, padding: "40px 0" }}>No eval suites registered</p>
                  )}
                  {suites.map(suite => (
                    <div
                      key={suite.suiteId}
                      ...
                      style={{
                        background: selectedSuite?.suiteId === suite.suiteId ? `${ACCENT}10` : "rgba(255,255,255,0.03)",
                        border: selectedSuite?.suiteId === suite.suiteId ? `1px solid ${ACCENT}50` : "1px solid rgba(255,255,255,0.07)",
                        borderRadius: 10,
                        padding: "16px 18px",
                        marginBottom: 10,
                        cursor: "pointer",
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", fontSize: 11, color: "#94a3b8" }}>
                            ...
                            {suite.graderTypes.slice(0, 2).map(g => (
                              <span key={g} style={{ color: "#475569" }}>{g}</span>
                            ))}
                          </div>
                        </div>
                    ...
                    {selectedSuite ? (
                      <div>
                        ...
                        <button
                          ...
                          style={{
                            width: "100%",
                            background: runningId === selectedSuite.suiteId ? `${ACCENT}40` : ACCENT,
                            border: "none",
                            borderRadius: 7,
                            padding: "10px 14px",
                            color: "#fff",
            ...
            {activeTab === "results" && (
              <div>
                <div style={{ display: "grid", gridTemplateColumns: "1.8fr 80px 80px 80px 100px 120px", gap: 8, padding: "8px 12px", borderBottom: "1px solid rgba(255,255,255,0.08)", marginBottom: 6 }}>
                ...
                {runs.map((r) => (
                  <div key={r.runId} style={{ display: "grid", gridTemplateColumns: "1.8fr 80px 80px 80px 100px 120px", gap: 8, padding: "12px 12px", borderBottom: "1px solid rgba(255,255,255,0.04)", alignItems: "center" }}>
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 500, color: "#e2e8f0" }}>{r.suiteName ?? r.suiteId}</div>
                      <div style={{ fontSize: 11, color: "#475569" }}>{r.domain ?? "—"} · {new Date(r.runAt).toLocaleDateString()}</div>
                    </div>
                    <div style={{ fontSize: 12, color: r.passRate >= 0.85 ? "#22c55e" : r.passRate >= 0.7 ? "#f59e0b" : "#ef4444", fontWeight: 600 }}>
