...
const TRUST_METRICS: TrustMetric[] = [
  {
    ...
    color: '#22c55e',
    description:
      '% of agent recommendations accepted without operator override in the last 30 days',
  },
  {
    id: 'override-rate',
    ...
    color: '#f59e0b',
    description: '% of agent decisions manually overridden by human operators',
  },
  {
    ...
    isGoodWhenHigh: true,
    color: '#22c55e',
    description: '% of autonomous agent executions that completed without system error or rollback',
  },
  {
    id: 'triage-time',
  ...
  },
  {
    ...
    color: '#a855f7',
    description:
      'Estimated financial value protected by autonomous agent actions this quarter (avoided losses + recoveries)',
  },
  {
    ...
    color: '#22c55e',
    description:
      '% of agent-flagged incidents that were subsequently cleared as non-issues by analysts',
  },
  {
    id: 'policy-violation',
    ...
    color: '#f97316',
    description:
      '% of agent actions that triggered a policy guardrail or were flagged for compliance review',
  },
  {
    id: 'regression-rate',
    label: 'Agent Regression Rate',
    value: '2.8%',
    delta: -0.6,
    trend: 'down',
    isGoodWhenHigh: false,
    color: '#22c55e',
    description:
      '% of eval suite re-runs that showed a measurable performance regression vs baseline',
  },
  {
    id: 'token-cost',
    ...
    color: '#22c55e',
    description:
      'Average token spend per successful autonomous outcome (resolved incident, completed action)',
  },
];
...
const RECENT_INCIDENTS: IncidentRecord[] = [
  {
    ...
    type: 'Threat Triage',
    severity: 'critical',
    agentDecision: 'Escalate to SOC-T2 + isolate endpoints',
    outcome: 'accepted',
    timeToTriage: '1m 32s',
    policyViolation: false,
    falsePositive: false,
    timestamp: '2025-04-16T08:14:00Z',
  },
  {
    id: 'INC-2025-19833',
    domain: 'vessels',
    ...
    severity: 'high',
    agentDecision: 'Divert via Cape of Good Hope',
    outcome: 'overridden',
    timeToTriage: '4m 18s',
    policyViolation: false,
    falsePositive: false,
    timestamp: '2025-04-16T07:48:00Z',
  },
  {
    id: 'INC-2025-19821',
    domain: 'prism',
    ...
    severity: 'medium',
    agentDecision: 'File SAR within 72h window',
    outcome: 'escalated',
    timeToTriage: '3m 05s',
    policyViolation: false,
    falsePositive: false,
    timestamp: '2025-04-16T06:22:00Z',
  },
  {
    id: 'INC-2025-19814',
    domain: 'aegis',
    ...
    severity: 'low',
    agentDecision: 'Flag for analyst review',
    outcome: 'accepted',
    timeToTriage: '2m 44s',
    policyViolation: false,
    falsePositive: true,
    timestamp: '2025-04-16T05:11:00Z',
  },
  {
    id: 'INC-2025-19798',
    domain: 'terra',
    type: 'Valuation Alert',
    severity: 'high',
    agentDecision: 'Trigger re-appraisal workflow',
    outcome: 'accepted',
    timeToTriage: '5m 58s',
    policyViolation: false,
    falsePositive: false,
  ...
  },
  {
    ...
    type: 'Policy Check',
    severity: 'critical',
    agentDecision: 'Export blocked — policy violation',
    outcome: 'failed',
    timeToTriage: '0m 48s',
    policyViolation: true,
    falsePositive: false,
    timestamp: '2025-04-15T20:14:00Z',
  },
];
    ...
    >
      ...
      >
        ...
        {metric.delta !== undefined && (
          ...
          >
            {metric.trend === 'up' ? '▲' : '▼'} {Math.abs(metric.delta)}
            {metric.unit ?? '%'}
          </span>
        )}
      </div>
...
export default function TrustConsole() {
  ...
  const filteredIncidents =
    selectedDomain === 'all'
      ? RECENT_INCIDENTS
      : RECENT_INCIDENTS.filter((i) => i.domain === selectedDomain);

  const overallHealth = {
    acceptanceRate: DOMAIN_ROWS.reduce((s, d) => s + d.acceptanceRate, 0) / DOMAIN_ROWS.length,
    executionSuccess:
      DOMAIN_ROWS.reduce((s, d) => s + d.executionSuccessRate, 0) / DOMAIN_ROWS.length,
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
            ...
            <span
              style={{
                fontSize: 11,
                color: overallHealth.degradingDomains > 0 ? '#f59e0b' : '#22c55e',
                background: overallHealth.degradingDomains > 0 ? '#f59e0b18' : '#22c55e18',
                padding: '2px 10px',
                borderRadius: 20,
                border: `1px solid ${overallHealth.degradingDomains > 0 ? '#f59e0b40' : '#22c55e40'}`,
                fontWeight: 600,
              }}
            >
              {overallHealth.degradingDomains > 0
                ? `⚠ ${overallHealth.degradingDomains} DOMAIN${overallHealth.degradingDomains > 1 ? 'S' : ''} DEGRADING`
                : '✓ SYSTEMS HEALTHY'}
            </span>
          </div>
          ...
          {[
            {
              ...
              value: `${(overallHealth.acceptanceRate * 100).toFixed(1)}%`,
              color: '#22c55e',
              sub: 'Cross-domain avg',
            },
            {
              label: 'Execution Success',
              value: `${(overallHealth.executionSuccess * 100).toFixed(1)}%`,
              color: '#22c55e',
              sub: 'Last 30 days',
            },
            {
              label: 'False Positive Rate',
              value: `${(overallHealth.falsePositive * 100).toFixed(1)}%`,
              color: '#f59e0b',
              sub: 'Cross-domain avg',
            },
            {
              label: 'Domains Degrading',
              value: overallHealth.degradingDomains,
              color: overallHealth.degradingDomains > 0 ? '#ef4444' : '#22c55e',
              sub: 'Trend vs prior period',
            },
          ].map((m) => (
            <div
              key={m.label}
            ...
            {DOMAIN_ROWS.map((row) => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                  ...
                  <div
                    style={{ marginLeft: 'auto', fontSize: 13, color: '#22c55e', fontWeight: 700 }}
                  >
                    {row.businessValueProtected} protected
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 12 }}>
                  {[
                    {
                      label: 'Acceptance',
                      value: `${(row.acceptanceRate * 100).toFixed(0)}%`,
                      good: row.acceptanceRate >= 0.8,
                    },
                    {
                      label: 'Override',
                      value: `${(row.overrideRate * 100).toFixed(0)}%`,
                      good: row.overrideRate <= 0.15,
                    },
                    {
                      label: 'Exec Success',
                      value: `${(row.executionSuccessRate * 100).toFixed(1)}%`,
                      good: row.executionSuccessRate >= 0.95,
                    },
                    {
                      label: 'False Pos',
                      value: `${(row.falsePositiveRate * 100).toFixed(1)}%`,
                      good: row.falsePositiveRate <= 0.04,
                    },
                    {
                      label: 'Policy Viol',
                      value: `${(row.policyViolationRate * 100).toFixed(1)}%`,
                      good: row.policyViolationRate <= 0.005,
                    },
                    {
                      label: 'Regression',
                      value: `${(row.regressionRate * 100).toFixed(1)}%`,
                      good: row.regressionRate <= 0.03,
                    },
                  ].map((m) => (
                    <div key={m.label}>
                      <div
        ...
        {activeView === 'incidents' && (
          <div>
            ...
            <div
              style={{
                display: 'grid',
                gridTemplateColumns: '1fr 90px 80px 100px 1fr 80px 80px 80px 140px',
                gap: 8,
                padding: '8px 12px',
              ...
              >
                ...
                <div
                  style={{
                    fontSize: 10,
                    fontWeight: 700,
                    color: OUTCOME_COLORS[inc.outcome],
                    textTransform: 'uppercase',
                  }}
                >
                  {inc.outcome}
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                  ...
                  {!inc.policyViolation && !inc.falsePositive && (
