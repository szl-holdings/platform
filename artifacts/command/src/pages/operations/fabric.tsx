...
interface SystemHealth {
  signalMesh: SysService;
  runEngine: SysService;
  evidenceGraph: SysService;
  policyEngine: SysService;
  connectorHub: SysService;
  database: SysService;
}
interface Entity {
  id: string;
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: pinned
          ? '0 0 0 1px rgba(139,122,200,0.15), 0 12px 40px rgba(0,0,0,0.4)'
          : undefined,
      ...
      {layout.collapsed ? null : (
        <div className="flex-1 overflow-y-auto p-3 grid grid-cols-2 gap-2">
          {products.map((p) => (
            <div
              key={p.id}
              className="rounded-lg p-3 flex flex-col gap-1.5 cursor-pointer hover:bg-white/5 transition-colors"
              style={{
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-medium text-white leading-snug truncate">
                      {s.title}
                    </p>
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <span className="text-[9px] font-semibold" style={{ color: pc }}>
                        {s.product}
                      </span>
...
function syntheticSpans(run: Run): RunSpan[] {
  ...
  if (run.policyEvents > 0) {
    ...
    base.push({
      spanId: `${run.runId}-tool`,
      name: 'Execute action',
      kind: 'tool',
      status: run.status === 'completed' ? 'ok' : 'pending',
      latencyMs: run.status === 'completed' ? 95 : 0,
      startOffsetMs: 640,
    });
  }
  return base;
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        ...
                        {r.policyEvents > 0 && (
                          <Badge label={`${r.policyEvents} policy evt`} color="#f59e0b" />
                        )}
                      </div>
                    </div>
  ...
  return (
    <Panel layout={layout}>
      <PanelHeader
        ...
        count={
          recs.filter((r) => r.status === 'pending' || r.status === 'awaiting_approval').length
        }
        accent="#22c55e"
        layout={layout}
        right={
          <Link href="/operations/recommendations">
            <a
              className="text-[10px] flex items-center gap-1 hover:opacity-70 transition-opacity"
              style={{ color: 'rgba(255,255,255,0.3)' }}
            >
  ...
  return (
    <Panel layout={layout}>
      <PanelHeader
        ...
        right={
          degraded.length > 0 ? (
            <Badge label={`${degraded.length} degraded`} color="#f97316" />
          ) : (
            <Badge label="All healthy" color="#22c55e" />
          )
        }
      />
      {layout.collapsed ? null : (
          ...
          {connectors.map((c) => {
            ...
            return (
              <div key={c.connectorId} className="px-4 py-2 flex items-center gap-3">
                {c.status === 'healthy' ? (
                  <Wifi className="w-3 h-3 shrink-0" style={{ color: sc }} />
                ) : (
                  <WifiOff className="w-3 h-3 shrink-0" style={{ color: sc }} />
                )}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className="text-[11px] font-medium text-white">{c.label}</span>
                    <span className="text-[9px]" style={{ color: pc }}>
                      ({c.product})
                    </span>
                  </div>
                  <div className="flex items-center gap-2 mt-0.5 flex-wrap">
  ...
  return (
    <Panel layout={layout}>
      <PanelHeader
        ...
        right={
          degraded > 0 ? (
            <Badge label={`${degraded} issue${degraded > 1 ? 's' : ''}`} color="#f97316" />
          ) : (
            <Badge label="All systems go" color="#22c55e" />
          )
        }
      />
      {layout.collapsed ? null : (
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span
              ...
              style={{ background: 'rgba(139,122,200,0.12)', color: '#8b7ac8' }}
            >
        ...
        {corr.entities.map((e) => {
          ...
          return (
            <div
              ...
              style={{ background: `${pc}10`, border: `1px solid ${pc}25` }}
            >
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {corr.runs.map((rid) => (
          <Link key={rid} href="/operations/runs">
            <a className="text-[9px] font-mono hover:underline" style={{ color: '#d4a054' }}>
              {rid}
            </a>
          </Link>
        ))}
...
function loadStoredLayout(): FabricLayoutState {
  ...
  try {
    ...
    const pinned =
      parsed.pinned && PANEL_IDS.includes(parsed.pinned as PanelId)
...
function useFabricLayout() {
  ...
  const ctlFor = useCallback(
    (id: PanelId): PanelLayoutCtl => ({
...
export function GlobalFabricPage() {
  ...
  useEffect(() => {
    try {
      const es = new EventSource(apiUrl('/fabric/stream'), {
      ...
      es.addEventListener('products', (e: MessageEvent) => {
        ...
        setSnap((prev) => (prev ? { ...prev, products } : prev));
      });
      es.addEventListener('runs', (e: MessageEvent) => {
        const { runs } = JSON.parse(e.data);
        setSnap((prev) => (prev ? { ...prev, runs } : prev));
      });
      es.addEventListener('alerts', (e: MessageEvent) => {
        const { alerts } = JSON.parse(e.data);
        setSnap((prev) => (prev ? { ...prev, alerts } : prev));
      });
      es.addEventListener('recommendations', (e: MessageEvent) => {
        const { recommendations } = JSON.parse(e.data);
        setSnap((prev) => (prev ? { ...prev, recommendations } : prev));
      });
      es.addEventListener('approvals', (e: MessageEvent) => {
        const { approvals } = JSON.parse(e.data);
        setSnap((prev) => (prev ? { ...prev, approvals: filterActed(approvals) } : prev));
      });
      es.addEventListener('system_health', (e: MessageEvent) => {
        const { systemHealth } = JSON.parse(e.data);
        setSnap((prev) => (prev ? { ...prev, systemHealth } : prev));
      });
      es.addEventListener('connectors', (e: MessageEvent) => {
        const { connectors } = JSON.parse(e.data);
        setSnap((prev) => (prev ? { ...prev, connectors } : prev));
      });
    ...
    async (approval: Approval, action: 'approve' | 'dismiss') => {
      ...
      setSnap((prev) =>
        prev
          ? {
              ...prev,
              approvals: prev.approvals.filter((a) => a.approvalId !== approval.approvalId),
            }
          : prev,
      );
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              {products.length} products · {signals.length} signals ·{' '}
              {runs.filter((r) => r.status !== 'completed').length} active runs
            </span>
          </div>
          <div className="flex items-center gap-3">
            {criticalCount > 0 && (
            ...
            <div
              ...
              style={{ color: streamStatus === 'live' ? '#22c55e' : '#f59e0b' }}
            >
              <Dot
                color={streamStatus === 'live' ? '#22c55e' : '#f59e0b'}
                pulse={streamStatus === 'connecting'}
              />
              {streamStatus === 'live'
                ? 'Live'
        ...
        {layoutState.pinned ? (
          <div className="h-full flex flex-col gap-3">
            ...
            >
              ...
              <span>
                Focused on{' '}
                <span className="text-white font-semibold">{PANEL_LABELS[layoutState.pinned]}</span>{' '}
                — other panels are hidden.
              </span>
            </div>
            <div className="flex-1 min-h-0">
              {layoutState.pinned === 'products' && (
                <ProductsPanel products={products} layout={ctlFor('products')} />
              )}
              {layoutState.pinned === 'signals' && (
                <SignalsPanel signals={signals} onDrill={drillSignal} layout={ctlFor('signals')} />
              )}
              {layoutState.pinned === 'runs' && (
                <RunsPanel runs={runs} onDrill={drillRun} layout={ctlFor('runs')} />
              )}
              {layoutState.pinned === 'alerts' && (
                <AlertsPanel alerts={alerts} onDrill={drillAlert} layout={ctlFor('alerts')} />
              )}
              {layoutState.pinned === 'recommendations' && (
                <RecsPanel
                  recs={recommendations}
                  onDrill={drillRec}
                  layout={ctlFor('recommendations')}
                />
              )}
              {layoutState.pinned === 'approvals' && (
                <ApprovalsPanel
                  approvals={approvals}
                  onDrill={drillApproval}
                  onAction={handleApprovalAction}
                  layout={ctlFor('approvals')}
                />
              )}
              {layoutState.pinned === 'connectors' && (
                <ConnectorsPanel connectors={connectors} layout={ctlFor('connectors')} />
              )}
              {layoutState.pinned === 'systemHealth' && (
