...
const RETRIEVER_SOURCE_STYLE: Record<
  RetrieverSource,
...
function RetrievalBadge({ retriever }: { retriever: RetrieverSourceMeta }) {
  ...
  return (
    ...
    >
      RETRIEVAL · {s.label}
      {retriever.adapterId && <span className="ml-1 opacity-70">· {retriever.adapterId}</span>}
    </span>
  );
}
        ...
        return (
          <div key={stage.id} className="flex gap-3">
            ...
            <div className="flex flex-col items-center">
              ...
              >
                <Icon
                  className={`w-3 h-3 ${stage.status === 'running' ? 'animate-spin' : ''}`}
                  style={{ color: isSelected ? '#000' : cfg.color }}
                />
              </button>
              {!isLast && (
                <div
                  className="w-px flex-1 mt-1"
                  style={{ background: 'hsla(0,0%,100%,0.08)', minHeight: '24px' }}
                />
              )}
            </div>
            {/* Content */}
                ...
                {isSelected && (
                  <div className="mt-2 space-y-2">
                    {(() => {
                      ...
                      return (
                        <RetrievalBadge
                          retriever={{ source: src, adapterId: out.retrieverAdapterId ?? null }}
                        />
                      );
                    })()}
                    {stage.policyResult && (
                      <div
                        ...
                        style={{
                          background:
                            stage.policyResult.result === 'pass' ? '#22c55e10' : '#ef444410',
                          border: `1px solid ${stage.policyResult.result === 'pass' ? '#22c55e30' : '#ef444430'}`,
                        }}
                      >
                        <p
                          className="font-medium mb-0.5"
                          style={{
                            color: stage.policyResult.result === 'pass' ? '#22c55e' : '#ef4444',
                          }}
                        >
                          Policy: {stage.policyResult.policyName} —{' '}
                          {stage.policyResult.result.toUpperCase()}
                        </p>
                        {stage.policyResult.blockedReason && (
                        ...
                        {stage.policyResult.violatedPolicies.length > 0 && (
                          <p className="mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>
                            Violated: {stage.policyResult.violatedPolicies.join(', ')}
                          </p>
                        )}
                      </div>
                    )}
                    ...
                    {stage.evidenceRefs.length > 0 && <EvidenceDrawer refs={stage.evidenceRefs} />}
                    {stage.traceSpanId && (
                      <p className="text-[9px] font-mono" style={{ color: 'hsl(214,7%,35%)' }}>
                        OTel span: {stage.traceSpanId}
                      </p>
                    )}
                  </div>
                )}
      ...
      {open && (
        <div className="mt-2 space-y-2">
          {refs.map((ref) => (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-[10px] font-semibold" style={{ color: 'hsl(38,8%,92%)' }}>
                  {ref.source}
                </span>
                <div className="flex items-center gap-2">
                  <span
                  ...
                  <span
                    ...
                    style={{ color: ref.relevanceScore >= 0.9 ? '#22c55e' : '#f59e0b' }}
                  >
  ...
  if (spans.length === 0)
    return (
      <p className="text-xs" style={{ color: 'hsl(214,7%,35%)' }}>
        No trace spans captured for this run.
      </p>
    );
          ...
          >
            <div className="flex items-center justify-between mb-1">
              <div>
                ...
                <span className="ml-2 text-[9px]" style={{ color: 'hsl(214,7%,35%)' }}>
                  {span.service}
                </span>
              </div>
              <div className="flex items-center gap-2">
            ...
            >
              <div
                ...
                style={{
                  width: `${pct}%`,
                  background:
                    span.status === 'ok' ? ACCENT : span.status === 'error' ? '#ef4444' : '#f59e0b',
                }}
              />
            </div>
          </div>
...
function ApprovalHistory({ events }: { events: ApprovalEvent[] }) {
  if (events.length === 0)
    return (
      <p className="text-xs" style={{ color: 'hsl(214,7%,35%)' }}>
        No approval events recorded for this run.
      </p>
    );
  return (
    <div className="space-y-2">
      {events.map((ev) => (
        ...
        >
          ...
          <p className="text-[9px] font-mono mt-1" style={{ color: 'hsl(214,7%,35%)' }}>
            Actor: {ev.actor} · Proof: {ev.proofRef}
          </p>
        </div>
      ))}
    </div>
...
function Checkpoints({ checkpoints }: { checkpoints: RollbackCheckpoint[] }) {
  if (checkpoints.length === 0)
    return (
      <p className="text-xs" style={{ color: 'hsl(214,7%,35%)' }}>
        No rollback checkpoints captured.
      </p>
    );
  return (
    <div className="space-y-2">
      {checkpoints.map((ck) => (
        <div
          key={ck.id}
          className="rounded p-2.5 border flex items-start justify-between gap-4"
          style={{ background: 'rgba(255,255,255,0.02)', borderColor: 'rgba(255,255,255,0.06)' }}
        >
          <div>
            ...
            <p className="text-[9px] font-mono mt-0.5" style={{ color: 'hsl(214,7%,35%)' }}>
              hash: {ck.worldStateHash} · stage: {ck.stageId}
            </p>
            <p className="text-[9px] font-mono" style={{ color: 'hsl(214,7%,35%)' }}>
              {new Date(ck.capturedAt).toLocaleString()}
            </p>
          </div>
          {ck.restorable ? (
            <button
              className="flex-shrink-0 text-[10px] px-2.5 py-1 rounded border transition-colors hover:bg-white/5"
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-4">
          ...
          <div className="text-center">
            ...
            <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.3)' }}>
              Run ID {params.id} does not exist or has been archived
            </p>
          </div>
          <Link href={SUB}>
            <a
  ...
  return (
    <div className="p-6 max-w-5xl">
      ...
      <div className="flex items-center gap-2 mb-5 text-xs" style={{ color: 'hsl(214,7%,35%)' }}>
        <Link href={SUB}>
          <a className="hover:text-white/60 transition-colors">Trajectory Map</a>
        </Link>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-xs font-mono mt-0.5" style={{ color: 'hsl(214,7%,35%)' }}>
            {run.id} · {run.tenant}
          </p>
          {run.retriever && (
            <div className="mt-2">
              <RetrievalBadge retriever={run.retriever} />
            </div>
          )}
          <p className="text-sm mt-2 max-w-2xl" style={{ color: 'hsl(214,7%,55%)' }}>
            {run.objectiveText}
          </p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {[
          ...
          {
            label: 'Confidence',
            value: `${Math.round(run.confidence * 100)}%`,
            color:
              run.confidence >= 0.85 ? '#22c55e' : run.confidence >= 0.7 ? '#f59e0b' : '#ef4444',
          },
          {
            label: 'Risk Level',
            value: run.riskLevel,
          ...
          >
            <p className="text-[10px] mb-1" style={{ color: 'hsl(214,7%,55%)' }}>
              {m.label}
            </p>
            <p className="text-base font-semibold capitalize" style={{ color: m.color }}>
              {m.value}
            </p>
          </div>
        ))}
      ...
      {/* Tabs */}
      <div
        className="flex items-center gap-1 border-b mb-5"
