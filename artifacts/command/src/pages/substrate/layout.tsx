...
const PERSPECTIVE_OPTIONS: {
  ...
  { value: 'executive', label: 'Executive', icon: Zap, description: 'Posture, KPIs, risk summary' },
  {
    ...
    label: 'Operator',
    icon: Activity,
    description: 'In-flight runs, live trajectory',
  },
  {
    value: 'analyst',
    label: 'Analyst',
    icon: FlaskConical,
    description: 'Evidence, traces, counterfactuals',
  },
  {
    value: 'approver',
    label: 'Approver',
...
export function SubstrateLayout({ children, pendingCount = 0 }: SubstrateLayoutProps) {
  ...
  const [perspective, setPerspective] = useState<Perspective>(() => {
    try {
      return (localStorage.getItem('substrate-perspective') as Perspective) || 'operator';
    } catch {
      return 'operator';
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem('substrate-perspective', perspective);
    } catch {}
  }, [perspective]);
  ...
  return (
    <div
      ...
      style={{ background: 'hsl(214,16%,4%)', color: 'hsl(38,8%,92%)' }}
    >
      {/* Sidebar */}
      <aside
      ...
      >
        ...
        <div className="px-3 py-3 border-b" style={{ borderColor: 'hsla(0,0%,100%,0.08)' }}>
          ...
          >
            ...
            <ChevronRight
              className={`w-3 h-3 transition-transform ${perspectiveOpen ? 'rotate-90' : ''}`}
              style={{ color: 'hsl(214,7%,35%)' }}
            />
          </button>
          {perspectiveOpen && (
            <div
              className="mt-1 rounded-md overflow-hidden border"
              style={{ borderColor: 'hsla(0,0%,100%,0.1)', background: 'hsl(214,10%,11%)' }}
            >
                ...
                return (
                  ...
                  >
                    <Icon
                      ...
                      style={{ color: opt.value === perspective ? ACCENT : 'hsl(214,7%,55%)' }}
                    />
                    <div>
                      <p
                        className="text-[11px] font-medium"
                        style={{ color: opt.value === perspective ? ACCENT : 'hsl(38,8%,92%)' }}
                      >
                        {opt.label}
                      </p>
                      <p className="text-[9px]" style={{ color: 'hsl(214,7%,35%)' }}>
                        {opt.description}
                      </p>
                    </div>
                  </button>
        ...
        <nav className="flex-1 px-3 py-3 space-y-4">
          {NAV_SECTIONS.map((section, si) => (
            <div key={si}>
              {section.heading && (
                <p
                  className="text-[9px] font-mono uppercase tracking-widest mb-1.5 px-2"
                  style={{ color: 'hsl(214,7%,35%)' }}
                >
                  {section.heading}
                </p>
        ...
        >
          ...
          <Link href="/strategy">
            <a
              className="flex items-center gap-2 px-2 py-1 rounded text-[9px] transition-colors hover:bg-white/5"
              style={{ color: 'hsl(214,7%,35%)' }}
            >
...
function PerspectiveBanner({ perspective }: { perspective: Perspective }) {
  const config: Record<
    Perspective,
    { color: string; label: string; icon: React.ElementType; hint: string }
  > = {
    executive: {
      color: '#f59e0b',
      label: 'Executive View',
      icon: Zap,
      hint: 'Risk posture & KPI summary',
    },
    operator: {
      color: ACCENT,
      label: 'Operator View',
      icon: Activity,
      hint: 'Live trajectory & run management',
    },
    analyst: {
      color: '#8b5cf6',
      label: 'Analyst View',
      icon: Search,
      hint: 'Evidence, traces & counterfactuals',
    },
    approver: {
      color: '#22c55e',
      label: 'Approver View',
      icon: Shield,
      hint: 'Pending decisions requiring your action',
    },
  };
