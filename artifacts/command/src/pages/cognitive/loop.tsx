...
import {
  Brain, Play, RefreshCw, Activity, CheckCircle2, AlertTriangle,
  Clock, Eye, Lightbulb, GitBranch, Layers, Target,
  X, ChevronRight, History, Repeat,
} from "lucide-react";
import { useStandardQuery, useStandardMutation } from "@szl-holdings/api-client-react";
import { apiFetch, ApiError } from "@szl-holdings/shared-ui/api-fetch";
...
function statusColor(status: string): string {
  switch (status) {
    ...
    case "blocked":
    case "guardian_blocked": return "#f97316";
    case "pending_approval":
    case "checkpointed": return "#f59e0b";
    case "skipped": return FG_DIM;
    default: return FG_MUT;
  }
}
...
function PhaseTrack({ run }: { run: RunResult }) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: `repeat(${LOOP_PHASES.length}, 1fr)`, gap: 6 }}>
      {LOOP_PHASES.map((p) => {
        ...
        return (
          <div key={p.id} style={{
            ...
            <div style={{ fontSize: 9, fontWeight: 700, color: FG, letterSpacing: "0.04em", textTransform: "uppercase" }}>
              {p.label}
            </div>
            <div style={{ fontSize: 9, color: FG_MUT, marginTop: 2, fontVariantNumeric: "tabular-nums" }}>
              {ran ? fmtMs(r.durationMs) : "—"}
            </div>
            {ran && (
              <div style={{
                marginTop: 4, fontSize: 8, fontWeight: 700, color: c,
...
function RunForm({ onRun, busy }: { onRun: (objective: string, agentTier: string, dryRun: boolean) => void; busy: boolean }) {
  const [objective, setObjective] = useState("");
  const [agentTier, setAgentTier] = useState<"assistant" | "analyst" | "operator" | "autonomous">("analyst");
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 700, color: FG, letterSpacing: "0.06em", textTransform: "uppercase" }}>
      ...
      <textarea
        ...
        style={{
          ...
          background: "rgba(0,0,0,0.3)", color: FG, border: `1px solid ${BORDER}`,
          borderRadius: 6, fontSize: 12, fontFamily: "inherit", resize: "vertical",
          marginBottom: 10,
        }}
      />
      <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        <label style={{ fontSize: 11, color: FG_MUT, display: "flex", alignItems: "center", gap: 6 }}>
          Agent tier
          <select
            value={agentTier}
        ...
        >
          {busy ? <RefreshCw style={{ width: 12, height: 12 }} className="animate-spin" /> : <Play style={{ width: 12, height: 12 }} />}
    ...
    <div style={{
      ...
      <div style={{
        ...
        {[
          ...
          <div key={s.label} style={{
            ...
            <div style={{ fontSize: 9, color: FG_DIM, textTransform: "uppercase", letterSpacing: "0.06em" }}>{s.label}</div>
      ...
      {run.summary && (
        <div style={{
          ...
          <div style={{ fontSize: 9, color: FG_DIM, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>
            Reflection summary
          </div>
      ...
      {run.worldModelUpdate && (
        <div style={{
          ...
          <span style={{ fontSize: 10, color: FG_MUT }}>
            Novelty <strong style={{ color: FG }}>{((run.worldModelUpdate.noveltyScore ?? 0) * 100).toFixed(0)}%</strong>
          </span>
          <span style={{ fontSize: 10, color: FG_MUT }}>
            Risk <strong style={{ color: FG }}>{((run.worldModelUpdate.riskScore ?? 0) * 100).toFixed(0)}%</strong>
          </span>
          <span style={{ fontSize: 10, color: FG_MUT }}>
            Uncertainty <strong style={{ color: FG }}>{((run.worldModelUpdate.uncertaintyScore ?? 0) * 100).toFixed(0)}%</strong>
          </span>
          {(run.worldModelUpdate.detectedAnomalies?.length ?? 0) > 0 && (
            <span style={{ fontSize: 10, color: "#f97316" }}>
              {run.worldModelUpdate.detectedAnomalies!.length} anomaly{run.worldModelUpdate.detectedAnomalies!.length > 1 ? "ies" : ""}
            </span>
          )}
        </div>
      )}
      ...
      {run.error && (
        <div style={{
          ...
          borderRadius: 6, color: "#fca5a5", fontSize: 11, display: "flex", gap: 6, alignItems: "flex-start",
        }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
        ...
        <button
          ...
          style={{
            ...
            background: "transparent", color: FG_MUT, border: `1px solid ${BORDER}`,
            borderRadius: 4, padding: "3px 8px", fontSize: 10, cursor: "pointer",
          }}
        >
          <RefreshCw style={{ width: 10, height: 10 }} className={loading ? "animate-spin" : undefined} />
          Refresh
        </button>
      </div>
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 9, color: FG_MUT, marginTop: 2 }}>
                  step {cp.stepIndex} · run {cp.runId.slice(0, 8)}… · agent {cp.agentId} · {fmtTime(cp.createdAt)}
                </div>
              </div>
...
function CheckpointDrawer({
  ...
  return (
    <div
      ...
      aria-modal="true"
      style={{
        position: "fixed", inset: 0, background: "rgba(2,4,10,0.6)",
    ...
    >
      <div
        ...
        style={{
          ...
          background: "#0a0e18", borderLeft: `1px solid ${BORDER}`,
      ...
      >
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              color: FG_MUT, border: "none", cursor: "pointer",
            }}
            aria-label="Close"
          >
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 18 }}>
          ...
          {detail && (
            <>
              <div style={{ fontSize: 10, color: FG_DIM, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                Checkpoint ref
              </div>
              <div style={{ fontSize: 12, fontFamily: "monospace", color: FG, marginBottom: 12, wordBreak: "break-all" }}>
                {detail.ref}
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
                <div>
                  <div style={{ fontSize: 9, color: FG_DIM, textTransform: "uppercase", letterSpacing: "0.06em" }}>Run</div>
                  <div style={{ fontSize: 11, fontFamily: "monospace", color: FG }}>{detail.runId}</div>
                </div>
                <div>
                  <div style={{ fontSize: 9, color: FG_DIM, textTransform: "uppercase", letterSpacing: "0.06em" }}>Agent</div>
                  <div style={{ fontSize: 11, fontFamily: "monospace", color: FG }}>{detail.agentId}</div>
                </div>
                <div>
                  <div style={{ fontSize: 9, color: FG_DIM, textTransform: "uppercase", letterSpacing: "0.06em" }}>Phase</div>
                  <StatusBadge status={detail.phase} />
                </div>
                <div>
                  <div style={{ fontSize: 9, color: FG_DIM, textTransform: "uppercase", letterSpacing: "0.06em" }}>Step</div>
                  <div style={{ fontSize: 11, color: FG }}>{detail.stepIndex}</div>
                </div>
                <div>
                  <div style={{ fontSize: 9, color: FG_DIM, textTransform: "uppercase", letterSpacing: "0.06em" }}>Created</div>
                  <div style={{ fontSize: 11, color: FG }}>{fmtTime(detail.createdAt)}</div>
                </div>
                {detail.expiresAt && (
                  <div>
                    <div style={{ fontSize: 9, color: FG_DIM, textTransform: "uppercase", letterSpacing: "0.06em" }}>Expires</div>
                    <div style={{ fontSize: 11, color: FG }}>{fmtTime(detail.expiresAt)}</div>
                  </div>
                )}
              </div>

              <div style={{ fontSize: 9, color: FG_DIM, textTransform: "uppercase", letterSpacing: "0.06em" }}>Objective</div>
              <div style={{
                fontSize: 12, color: FG, marginBottom: 14,
                padding: "8px 10px", background: "rgba(0,0,0,0.25)",
                border: `1px solid ${BORDER}`, borderRadius: 6, lineHeight: 1.5,
              }}>{detail.objective}</div>

              <div style={{ fontSize: 9, color: FG_DIM, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 4, marginBottom: 16 }}>
                {detail.snapshotPhases.length === 0 ? (
                  ...
                  <div key={i} style={{
                    display: "flex", alignItems: "center", gap: 8,
                    padding: "6px 10px",
                    background: "rgba(0,0,0,0.18)", border: `1px solid ${BORDER}`,
                    borderRadius: 4,
                  }}>
                    <span style={{ fontSize: 9, color: FG_DIM, fontFamily: "monospace", width: 18 }}>{i + 1}.</span>
              ...
              <button
                ...
                style={{
                  display: "inline-flex", alignItems: "center", gap: 6,
                  padding: "8px 14px", background: ACCENT, color: "#0b0e16",
                  border: "none", borderRadius: 6, fontSize: 11, fontWeight: 700,
              ...
              >
                {resuming ? <RefreshCw style={{ width: 12, height: 12 }} className="animate-spin" /> : <Repeat style={{ width: 12, height: 12 }} />}
...
export default function CognitiveLoopPage() {
  ...
  const checkpointsQuery = useStandardQuery({
    ...
    queryFn: () => apiFetch<{ data: { checkpoints: CheckpointEntry[]; total: number } }>(
    ...
    >
      ...
      {selectedRef && (
        <CheckpointDrawer
          ...
          onClose={() => { setSelectedRef(null); setDrawerError(null); }}
