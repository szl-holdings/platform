    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
      {[plan, fallback].map((p, idx) => (
        ...
        >
          ...
          >
            <div>
              <span style={{ color: '#94a3b8' }}>{p.steps.length}</span> steps
            </div>
            <div>
              <span style={{ color: '#94a3b8' }}>{fmtMoney(p.estimatedCostUsd)}</span> cost
            </div>
            <div>
              <span style={{ color: RISK_COLORS[p.riskLevel] }}>{p.riskLevel}</span> risk
            </div>
            <div>
      ...
      {step.requiredEvidence.length > 0 && (
        <div style={{ marginBottom: 12 }}>
          ...
          {step.requiredEvidence.map((e) => (
            <div key={e} style={{ fontSize: 11, color: '#94a3b8' }}>
              • {e}
            </div>
          ))}
        </div>
...
export default function PlannerStudio() {
  ...
  >({
    ...
    onSuccess: (_data, variables) => {
      ...
      planQuery.refetch();
      plansQuery.refetch();
      toast.success(variables.decision === 'approve' ? 'Step approved' : 'Step rejected');
    },
    onError: (err, variables) => {
      setDecisionError(err.message);
      toast.error(`Could not ${variables.decision} step`, { description: err.message });
    },
  });
