...
const SEEDED_TRACES: TraceRun[] = [
  {
    ...
    domain: "aegis",
    taskDescription: "Detect and contain ransomware lateral movement across 14 endpoints detected at 08:14 UTC. Classify threat, escalate to SOC-T2, propose containment.",
    ...
    phases: [
      {
        ...
        costUsd: 0.00062,
        latencyMs: 820,
        summary: "Ingested 14 endpoint alerts, retrieved lateral movement telemetry (SMB traffic anomaly, credential spray across 3 subnets). Matched against 2 active CVEs in threat intel feed.",
      ...
      },
      {
        ...
        costUsd: 0.00091,
        latencyMs: 1100,
        summary: "Classified as CRITICAL — ransomware precursor pattern (Conti TTP match 87%). Estimated blast radius: 14 endpoints, 3 shared drives, 1 domain controller. Confidence: high.",
      ...
      },
      {
        ...
        costUsd: 0.00073,
        latencyMs: 890,
        summary: "Selected SOC-T2 escalation playbook. Planned network segment isolation for affected subnets. Queued credential reset for 14 accounts. Routed approval request to on-call analyst.",
      ...
      },
      {
        ...
        costUsd: 0.00048,
        latencyMs: 1240,
        summary: "Executed network isolation on 2 subnets (pending analyst approval for 3rd). Created SOC-T2 escalation ticket INC-2025-19841. Notified on-call analyst via PagerDuty.",
      ...
      },
      {
        ...
        costUsd: 0.00067,
        latencyMs: 770,
        summary: "Verified isolation applied successfully. Ground truth match: 91% vs operator decision. One discrepancy: operator applied immediate domain controller lockout (not recommended by agent). Flagged for calibration.",
  ...
  },
  {
    ...
    domain: "vessels",
    taskDescription: "MV Pacific Horizon approaching Typhoon Mawar track. Optimize routing via Cape of Good Hope or Malacca with fuel and schedule P&L tradeoffs.",
    ...
    phases: [
      {
        ...
        costUsd: 0.00042,
        latencyMs: 640,
        summary: "Retrieved typhoon track data (NHC+JMA), vessel position (lat 18.4°N, lng 121.2°E), cargo manifest (bulk grain, 48,200 MT), charter party constraints.",
      ...
      },
      {
        ...
        costUsd: 0.00058,
        latencyMs: 810,
        summary: "Typhoon avoidance: Cape route adds 4.2 days, $312K fuel premium. Malacca route: 1.8 days delay, piracy risk LOW (current MMC advisory). Recommended Malacca with 48hr early departure.",
      ...
      },
      {
        ...
        costUsd: 0.00044,
        latencyMs: 720,
        summary: "Generated revised voyage plan: depart 36hr early, Malacca Strait, ETA Singapore revised to Apr 21. Notified port agent. Queued charterer amendment for captain approval.",
      ...
      },
      {
        ...
        costUsd: 0.00028,
        latencyMs: 480,
        summary: "Committed voyage plan update. Captain override: selected Cape route despite agent recommendation (concerns about Malacca congestion). Outcome marked PARTIAL — override logged.",
      ...
      },
      {
        ...
        costUsd: 0.00026,
        latencyMs: 560,
        summary: "Override documented. Cape route selected adds est. $298K vs agent plan. Reason flagged: agent did not weight port congestion risk sufficiently. Calibration note created.",
  ...
  },
  {
    ...
    domain: "terra",
    taskDescription: "200bps interest rate shock scenario — compute cap rate impact, NAV delta, and reallocation recommendations for 12-asset office/industrial portfolio.",
    ...
    phases: [
      {
        ...
        costUsd: 0.00058,
        latencyMs: 940,
        summary: "Loaded 12 assets (8 office, 4 industrial), current cap rates (avg 5.8%), NOI data, debt structure. Retrieved Fed curve shift scenario data.",
        regressionFlag: false,
      },
      {
        phase: "reason",
        ...
        costUsd: 0.00092,
        latencyMs: 1420,
        summary: "200bps shock → avg cap rate 7.8% (+200bps). Total NAV decline: -$142M (-18.4%). Office assets worst impacted (-23%), industrial resilient (-9%). 3 assets cross LTV covenant threshold.",
        regressionFlag: false,
      },
      {
        phase: "plan",
        ...
        costUsd: 0.00071,
        latencyMs: 1100,
        summary: "Recommended disposition of 2 office assets (CBD Portland, suburban Dallas) to reduce LTV exposure. Industrial reallocation proposal: acquire 2 logistics assets in Sun Belt. IRR-neutral at 6.2% cap.",
        regressionFlag: false,
      },
      {
        phase: "act",
        ...
        costUsd: 0.00032,
        latencyMs: 980,
        summary: "Generated executive stress test report (PDF, 14 pages). Drafted lender notification for 3 covenant-breach assets. Queued for CFO approval before send.",
        regressionFlag: false,
      },
      {
        phase: "reflect",
        ...
        costUsd: 0.00024,
        latencyMs: 700,
        summary: "Report validated against prior appraisal data. NAV calculation cross-checked: within $2M of analyst estimate. Output accepted without modification.",
        regressionFlag: false,
      },
    ],
  },
...
function apiTraceToTraceRun(t: ApiTrace): TraceRun {
  ...
  const phases: PhaseSnapshot[] = PHASES.map((phase, i) => {
    ...
    return {
      ...
      summary: reflection?.summary ?? `${phase} phase executed for objective: ${t.objective ?? "unknown"}`,
...
function PhaseStep({
  ...
  return (
    ...
    >
      <div style={{ width: 32, height: 32, borderRadius: 8, background: `${c}20`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>
        {PHASE_ICONS[snapshot.phase]}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: isActive ? c : "#94a3b8", textTransform: "capitalize" }}>{snapshot.phase}</span>
          <span style={{ fontSize: 9, color: "#475569" }}>#{index + 1}</span>
          {snapshot.regressionFlag && (
            <span style={{ fontSize: 9, color: "#ef4444", background: "#ef444420", padding: "1px 5px", borderRadius: 3, fontWeight: 700 }}>REGRESSION</span>
          )}
        </div>
        <div style={{ fontSize: 10, color: "#475569", marginTop: 1 }}>{snapshot.latencyMs}ms · ${snapshot.costUsd.toFixed(5)}</div>
      </div>
    </button>
  );
}
...
function PhaseDetail({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${c}40`, borderRadius: 10, padding: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
          <span style={{ fontSize: 18 }}>{PHASE_ICONS[snapshot.phase]}</span>
          <span style={{ fontSize: 14, fontWeight: 700, color: c, textTransform: "capitalize" }}>{snapshot.phase}</span>
          {snapshot.regressionFlag && (
            <span style={{ fontSize: 10, color: "#ef4444", background: "#ef444418", border: "1px solid #ef444440", padding: "2px 8px", borderRadius: 4, fontWeight: 700, marginLeft: "auto" }}>⚠ REGRESSION MARKER</span>
          )}
        </div>

        <p style={{ fontSize: 12, color: "#94a3b8", lineHeight: 1.6, marginBottom: 14 }}>{snapshot.summary}</p>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, padding: "12px 0", borderTop: "1px solid rgba(255,255,255,0.06)", borderBottom: "1px solid rgba(255,255,255,0.06)", marginBottom: 12 }}>
          <StatPill label="Model" value={snapshot.model.replace("gpt-4o-", "4o-").replace("claude-3-5-sonnet-", "cs-")} />
          <StatPill label="Latency" value={`${snapshot.latencyMs}ms`} color="#0ea5e9" />
          <StatPill label="Tokens" value={`${snapshot.inputTokens + snapshot.outputTokens}`} color="#a855f7" />
          <StatPill label="Cost" value={`$${snapshot.costUsd.toFixed(5)}`} color="#f59e0b" />
        </div>

        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 10, color: "#475569", marginBottom: 6, fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5 }}>Tools Called</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {snapshot.tools.map(t => (
              <span key={t} style={{ fontSize: 10, color: "#94a3b8", background: "rgba(255,255,255,0.06)", padding: "2px 8px", borderRadius: 4, fontFamily: "monospace" }}>{t}</span>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "70px 1fr", gap: 6 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{ flex: 1, height: 4, background: "rgba(255,255,255,0.06)", borderRadius: 2 }}>
              <div style={{ width: `${Math.min(100, snapshot.inputTokens / 30)}%`, height: "100%", background: "#0ea5e9", borderRadius: 2 }} />
            </div>
            <span style={{ fontSize: 10, color: "#64748b", width: 34 }}>{snapshot.inputTokens}</span>
          </div>
          <span style={{ fontSize: 10, color: "#475569" }}>Out tokens</span>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{ flex: 1, height: 4, background: "rgba(255,255,255,0.06)", borderRadius: 2 }}>
              <div style={{ width: `${Math.min(100, snapshot.outputTokens / 10)}%`, height: "100%", background: "#a855f7", borderRadius: 2 }} />
            </div>
            <span style={{ fontSize: 10, color: "#64748b", width: 34 }}>{snapshot.outputTokens}</span>
          </div>
        </div>
      </div>

      {snapshot.delta && (
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, padding: "10px 14px" }}>
          <div style={{ fontSize: 10, color: "#475569", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 4 }}>Snapshot Delta vs Prior Run</div>
          <div style={{ fontSize: 12, color: snapshot.regressionFlag ? "#f59e0b" : "#94a3b8" }}>{snapshot.delta}</div>
        </div>
      )}
        ...
        {savedComment ? (
          <div style={{ marginBottom: 8 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <button onClick={() => setSavedComment(null)} style={{ fontSize: 10, color: "#475569", background: "none", border: "none", cursor: "pointer" }}>Edit</button>
              {commentMutation.isSuccess && <span style={{ fontSize: 10, color: "#22c55e" }}>✓ Saved to trace</span>}
              {commentMutation.isError && <span style={{ fontSize: 10, color: "#f59e0b" }}>⚠ Saved locally (API error)</span>}
            </div>
          </div>
        ) : (
          <>
...
export default function CognitiveTraces() {
  const initialTraceId = typeof window !== "undefined"
      ...
      <div style={{ maxWidth: 1300, margin: "0 auto", padding: "24px 20px" }}>
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
            ...
            {isLiveData && <span style={{ fontSize: 10, color: "#22c55e", background: "#22c55e15", padding: "2px 8px", borderRadius: 4 }}>● LIVE</span>}
            {tracesQuery.isError && <span style={{ fontSize: 10, color: "#f59e0b" }}>⚠ Showing sample traces</span>}
          </div>
          <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>
            Step through agent execution phases — perceive, reason, plan, act, reflect — with snapshot deltas, model telemetry, and regression markers vs prior runs.
          </p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 24 }}>
          {[
            ...
            { label: "Phases Logged", value: traces.reduce((s, t) => s + t.phases.length, 0), color: "#0ea5e9" },
            { label: "Regression Flags", value: traces.reduce((s, t) => s + t.phases.filter(p => p.regressionFlag).length, 0), color: "#ef4444" },
            { label: "Avg Cost / Trace", value: `$${(traces.reduce((s, t) => s + t.totalCostUsd, 0) / (traces.length || 1)).toFixed(5)}`, color: "#f59e0b" },
          ].map(m => (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {filtered.map(trace => {
                ...
                return (
                  <button
                    key={trace.id}
                    onClick={() => { setSelectedTraceId(trace.id); setActivePhaseIndex(0); }}
                    style={{
                      display: "block",
                      width: "100%",
                      textAlign: "left",
                  ...
                  >
                    <div style={{ fontSize: 11, fontWeight: 600, color: isSelected ? "#e2e8f0" : "#94a3b8", marginBottom: 3, lineHeight: 1.4 }}>{trace.label}</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                      <span style={{ fontSize: 9, color: tc, background: `${tc}15`, padding: "1px 6px", borderRadius: 3, fontWeight: 700 }}>{trace.domain.toUpperCase()}</span>
                      <span style={{ fontSize: 9, color: OUTCOME_STYLE[trace.outcome]?.color ?? "#94a3b8", fontWeight: 700 }}>{OUTCOME_STYLE[trace.outcome]?.label ?? "UNKNOWN"}</span>
                      {regFlags > 0 && <span style={{ fontSize: 9, color: "#ef4444" }}>⚠ {regFlags} regression{regFlags > 1 ? "s" : ""}</span>}
                    </div>
                    <div style={{ fontSize: 10, color: "#475569", marginTop: 4 }}>{new Date(trace.startedAt).toLocaleString()}</div>
                  </button>
                );
              })}
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "14px 18px" }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 12 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 15, fontWeight: 700, color: "#e2e8f0", marginBottom: 4 }}>{selectedTrace.label}</div>
                  <p style={{ fontSize: 12, color: "#64748b", margin: 0, lineHeight: 1.5 }}>{selectedTrace.taskDescription}</p>
                </div>
                <div style={{ display: "flex", gap: 16, flexShrink: 0 }}>
                  <StatPill label="Total Latency" value={`${(selectedTrace.totalLatencyMs / 1000).toFixed(2)}s`} color="#0ea5e9" />
                  <StatPill label="Total Cost" value={`$${selectedTrace.totalCostUsd.toFixed(5)}`} color="#f59e0b" />
                  <StatPill label="Outcome" value={outcome.label} color={outcome.color} />
                  {regressionCount > 0 && <StatPill label="Regressions" value={regressionCount} color="#ef4444" />}
                </div>
              </div>
                  ...
                  return (
                    <React.Fragment key={i}>
                      ...
                      >
                        ...
                        <span style={{ fontSize: 10, fontWeight: 600, color: activePhaseIndex === i ? c : "#64748b", textTransform: "capitalize" }}>{phase.phase}</span>
                        {phase.regressionFlag && <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#ef4444", flexShrink: 0 }} />}
                      </button>
