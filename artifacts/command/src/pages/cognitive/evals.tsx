...
const SEEDED_SUITES: EvalSuite[] = [
  {
    ...
    domain: "aegis",
    description: "Precision/recall on threat classification and escalation routing against labeled SOC ground truths. Includes 5 red-team adversarial prompts.",
    totalCases: 28,
    redTeamCases: 5,
  ...
  },
  {
    ...
    domain: "vessels",
    description: "Evaluates route decision quality under cyclone, piracy, and fuel constraint scenarios. Tests operator override prediction accuracy.",
    totalCases: 22,
    redTeamCases: 3,
  ...
  },
  {
    ...
    domain: "cross-domain",
    description: "Red-team battery: prompt injection, unsafe tool execution, tenant isolation, policy bypass, and data export abuse across all domains.",
    totalCases: 55,
    redTeamCases: 20,
    models: ["gpt-4o-2024-11-20", "gpt-4o-finetuned"],
  },
  {
    ...
    domain: "cross-domain",
    description: "Evaluates completeness, accuracy, and usefulness scores for executive reports, runbooks, stress tests, and risk briefs.",
    totalCases: 35,
    redTeamCases: 0,
    models: ["gpt-4o-2024-11-20", "gpt-4o-finetuned", "claude-3-5-sonnet-20241022"],
  },
  {
    ...
    domain: "cross-domain",
    description: "Tests agent refusal accuracy and confidence calibration when insufficient data is available or queries are ambiguous.",
    totalCases: 30,
    redTeamCases: 0,
    models: ["gpt-4o-2024-11-20", "gpt-4o-finetuned"],
  },
  {
    ...
    domain: "terra",
    description: "NAV calculation accuracy under rate shock, cap rate estimation, and reallocation recommendation quality.",
    totalCases: 18,
    redTeamCases: 0,
...
const SEEDED_RUNS: EvalRun[] = [
  ...
  },
  {
    ...
    regressionDetail: "Recall dropped 9.1pp vs prior baseline — threat classification missed 2 lateral movement cases",
  },
  {
    id: "run-policy-001",
  ...
  },
  {
    ...
    avgCostUsd: 0.00058,
    regressionSeverity: "minor",
    regressionDetail: "Piracy risk assessment degraded after new MMC advisory feed — 2 Malacca Strait cases incorrectly rated LOW",
    linkedTraceIds: ["trace-vessels-20250416-003"],
  },
  {
    id: "run-artifact-001",
...
const METRICS_9 = [
  { key: "passRate" as const, label: "Pass Rate", fmt: (v: number) => `${(v * 100).toFixed(1)}%`, good: (v: number) => v >= 0.85 },
  { key: "f1Score" as const, label: "F1", fmt: (v: number) => `${(v * 100).toFixed(1)}%`, good: (v: number) => v >= 0.85 },
  { key: "precision" as const, label: "Precision", fmt: (v: number) => `${(v * 100).toFixed(1)}%`, good: (v: number) => v >= 0.85 },
  { key: "recall" as const, label: "Recall", fmt: (v: number) => `${(v * 100).toFixed(1)}%`, good: (v: number) => v >= 0.85 },
  { key: "avgScore" as const, label: "Avg Score", fmt: (v: number) => `${(v * 100).toFixed(1)}%`, good: (v: number) => v >= 0.80 },
  { key: "policyCompliance" as const, label: "Policy Compliance", fmt: (v: number) => `${(v * 100).toFixed(1)}%`, good: (v: number) => v >= 0.95 },
  { key: "overrideRate" as const, label: "Override Rate", fmt: (v: number) => `${(v * 100).toFixed(1)}%`, good: (v: number) => v <= 0.15 },
  { key: "avgLatencyMs" as const, label: "Avg Latency", fmt: (v: number) => `${v}ms`, good: (v: number) => v <= 400 },
  { key: "avgCostUsd" as const, label: "Avg Cost", fmt: (v: number) => `$${v.toFixed(5)}`, good: (v: number) => v <= 0.001 },
];
      ...
      <div style={{ maxWidth: 1300, margin: "0 auto", padding: "24px 20px" }}>
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
            ...
            {evalsQuery.isLoading && <span style={{ fontSize: 10, color: "#475569" }}>Loading…</span>}
            {isLiveData && <span style={{ fontSize: 10, color: "#22c55e", background: "#22c55e15", padding: "2px 8px", borderRadius: 4 }}>● LIVE</span>}
            {evalsQuery.isError && <span style={{ fontSize: 10, color: "#f59e0b" }}>⚠ Showing sample data</span>}
          </div>
          <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>
            Eval suites, recent runs, and all 9 quality metrics — with regression highlights and links to underlying execution traces.
          </p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 24 }}>
          {[
            ...
            { label: "Total Cases", value: suites.reduce((s, x) => s + x.totalCases, 0), color: "#0ea5e9" },
            { label: "Red-Team Cases", value: suites.reduce((s, x) => s + x.redTeamCases, 0), color: "#ef4444" },
            { label: "Avg Pass Rate", value: `${(avgPassRate * 100).toFixed(1)}%`, color: avgPassRate >= 0.85 ? "#22c55e" : "#f59e0b" },
            { label: "Avg F1 Score", value: `${(avgF1 * 100).toFixed(1)}%`, color: avgF1 >= 0.85 ? "#22c55e" : "#f59e0b" },
          ].map(m => (
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: 20, background: "rgba(255,255,255,0.03)", borderRadius: 8, padding: 4, width: "fit-content" }}>
          {(["suites", "runs", "compare"] as const).map(t => (
            ...
            >
              {t === "suites" ? "Suites" : t === "runs" ? `Runs (${runs.length})` : "9-Metric Compare"}
            </button>
          ))}
        </div>
        ...
        {activeTab === "suites" && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 20 }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                          ...
                          {hasRegression && <span style={{ fontSize: 9, color: "#f59e0b", background: "#f59e0b18", padding: "1px 6px", borderRadius: 3, fontWeight: 700 }}>REGRESSION</span>}
                        </div>
                        ...
                        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", fontSize: 11, color: "#94a3b8" }}>
                          <span style={{ color: dc, background: `${dc}15`, padding: "1px 6px", borderRadius: 3, fontSize: 10, fontWeight: 700 }}>{suite.domain.toUpperCase()}</span>
                          ...
                          {suite.linkedTraceIds && (
                            <Link href="/cognitive/traces" style={{ color: ACCENT, textDecoration: "none" }}>↗ {suite.linkedTraceIds.length} trace{suite.linkedTraceIds.length > 1 ? "s" : ""}</Link>
                          )}
                        </div>
                      </div>
                      {latestRun && (
                        <div style={{ textAlign: "right", flexShrink: 0 }}>
                          <div style={{ fontSize: 18, fontWeight: 700, color: latestRun.passRate >= 0.85 ? "#22c55e" : "#f59e0b" }}>{(latestRun.passRate * 100).toFixed(1)}%</div>
            ...
            <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: 18 }}>
              ...
              {selectedSuite ? (
                <div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: "#e2e8f0", marginBottom: 4 }}>{selectedSuite.name}</div>
                  <p style={{ fontSize: 12, color: "#64748b", lineHeight: 1.5, marginBottom: 14 }}>{selectedSuite.description}</p>
                  <div style={{ fontSize: 12, color: "#475569", marginBottom: 10 }}>Select strategy to evaluate:</div>
                    ...
                    {selectedSuite.models.map(model => {
                      ...
                      return (
                        <button
                          ...
                          style={{
                            background: runningId === key ? `${ACCENT}40` : "rgba(255,255,255,0.06)",
                  ...
                  <div style={{ fontSize: 12, fontWeight: 600, color: "#475569", marginBottom: 8 }}>Past Runs</div>
                  {runs.filter(r => r.suiteId === selectedSuite.id).map(run => {
                    const sev = SEVERITY_MAP[run.regressionSeverity] ?? SEVERITY_MAP.none;
                    return (
                      <div key={run.id} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 8, padding: "10px 12px", marginBottom: 8 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                          <span style={{ fontSize: 11, color: "#e2e8f0", fontWeight: 500 }}>{run.model.replace("gpt-4o-", "4o-").replace("claude-3-5-sonnet-", "claude-")}</span>
                          <span style={{ fontSize: 10, fontWeight: 600, color: sev.color }}>{sev.label}</span>
                        </div>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                          {[
                            { label: "Pass", value: `${(run.passRate * 100).toFixed(1)}%`, good: run.passRate >= 0.85 },
                            { label: "F1", value: `${(run.f1Score * 100).toFixed(1)}%`, good: run.f1Score >= 0.85 },
                            { label: "Override", value: `${(run.overrideRate * 100).toFixed(1)}%`, good: run.overrideRate <= 0.15 },
                          ].map(m => (
                            <div key={m.label}>
                              <div style={{ fontSize: 10, color: "#475569" }}>{m.label}</div>
                              <div style={{ fontSize: 13, fontWeight: 700, color: m.good ? "#22c55e" : "#f59e0b" }}>{m.value}</div>
                            </div>
                          ))}
                        </div>
                        {run.regressionDetail && (
                          <div style={{ fontSize: 10, color: "#f59e0b", background: "#f59e0b10", borderRadius: 4, padding: "4px 8px", marginTop: 6 }}>{run.regressionDetail}</div>
                        )}
                        {run.linkedTraceIds && (
                          <div style={{ marginTop: 6, display: "flex", gap: 4, flexWrap: "wrap" }}>
                            {run.linkedTraceIds.map(tid => (
                              <Link key={tid} href={`/cognitive/traces?trace=${encodeURIComponent(tid)}`} style={{ fontSize: 9, color: ACCENT, background: `${ACCENT}15`, padding: "1px 6px", borderRadius: 3, fontFamily: "monospace", textDecoration: "none" }}>↗ {tid.slice(0, 22)}…</Link>
                            ))}
                          </div>
                        )}
                        <div style={{ fontSize: 10, color: "#334155", marginTop: 4 }}>{new Date(run.runAt).toLocaleString()}</div>
                      </div>
                    );
                  })}
                </div>
              ) : (
                <p style={{ fontSize: 12, color: "#475569", textAlign: "center", padding: "40px 0" }}>Select an eval suite to view runs or trigger a new evaluation</p>
              )}
            </div>
          </div>
        )}
        ...
        {activeTab === "runs" && (
          <div>
            <div style={{ display: "flex", gap: 10, marginBottom: 14, alignItems: "center" }}>
              ...
              {selectedSuite && (
                <button onClick={() => setSelectedSuite(null)} style={{ background: "none", border: "none", color: "#475569", cursor: "pointer", fontSize: 11 }}>
                  × Clear suite filter
                </button>
              )}
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "1.8fr 1fr 70px 60px 60px 60px 70px 60px 70px 100px", gap: 8, padding: "8px 12px", borderBottom: "1px solid rgba(255,255,255,0.08)", marginBottom: 4 }}>
              {["Suite / Model", "Run At", "Pass%", "F1", "Prec", "Rec", "Policy%", "OR%", "Latency", "Status"].map(h => (
            ...
            {filteredRuns.map(run => {
              ...
              return (
                ...
                >
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 500, color: "#e2e8f0" }}>{run.suiteName}</div>
                    <div style={{ fontSize: 10, color: "#475569" }}>{run.model.replace("gpt-4o-", "4o-").replace("claude-3-5-sonnet-", "claude-")}</div>
                  </div>
                  <div style={{ fontSize: 10, color: "#64748b" }}>{new Date(run.runAt).toLocaleDateString()}</div>
                  {[
                    ...
                    <div key={i} style={{ fontSize: 12, fontWeight: 600, color: m.good ? "#22c55e" : "#f59e0b" }}>
            ...
            {selectedRun && (
              <div style={{ background: `${ACCENT}08`, border: `1px solid ${ACCENT}30`, borderRadius: 10, padding: 16, marginTop: 16 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: "#e2e8f0", marginBottom: 10 }}>{selectedRun.suiteName} — All 9 Metrics</div>
                {selectedRun.regressionDetail && (
                  <div style={{ fontSize: 12, color: "#f59e0b", background: "#f59e0b10", borderRadius: 6, padding: "8px 12px", marginBottom: 10 }}>
                    ⚠ {selectedRun.regressionDetail}
                  </div>
                )}
                {selectedRun.linkedTraceIds && (
                  <div style={{ marginBottom: 10 }}>
                    <div style={{ fontSize: 10, color: "#475569", marginBottom: 4 }}>Linked Traces</div>
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {selectedRun.linkedTraceIds.map(tid => (
                        <Link key={tid} href={`/cognitive/traces?trace=${encodeURIComponent(tid)}`} style={{ fontSize: 10, color: ACCENT, background: `${ACCENT}15`, padding: "2px 8px", borderRadius: 4, fontFamily: "monospace", textDecoration: "none" }}>↗ {tid}</Link>
                      ))}
                    </div>
                  </div>
                )}
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
                  {METRICS_9.map(m => {
                    ...
                    return (
                      <div key={m.key} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 6, padding: "8px 12px" }}>
                        <div style={{ fontSize: 10, color: "#475569", marginBottom: 2 }}>{m.label}</div>
                        <div style={{ fontSize: 16, fontWeight: 700, color: isGood ? "#22c55e" : "#f59e0b" }}>{m.fmt(val as number)}</div>
        ...
        {activeTab === "compare" && (
          <div>
            ...
            {suites.map(suite => {
              ...
              const dc = DOMAIN_COLORS[suite.domain] ?? DOMAIN_COLORS.default;
              return (
                <div key={suite.id} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "16px 18px", marginBottom: 14 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: "#e2e8f0" }}>{suite.name}</span>
                    <span style={{ fontSize: 9, color: dc, background: `${dc}15`, padding: "1px 6px", borderRadius: 3, fontWeight: 700 }}>{suite.domain.toUpperCase()}</span>
                  </div>
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                      <thead>
                        <tr>
                          <th style={{ textAlign: "left", color: "#475569", padding: "4px 8px", fontWeight: 600, fontSize: 9, textTransform: "uppercase" }}>Model</th>
                          {METRICS_9.map(m => (
                            <th key={m.key} style={{ textAlign: "right", color: "#475569", padding: "4px 8px", fontWeight: 600, fontSize: 9, textTransform: "uppercase", whiteSpace: "nowrap" }}>{m.label}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {suiteRuns.map(run => (
                          <tr key={run.id} style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                            ...
                            {METRICS_9.map(m => {
                              ...
                              return (
                                <td key={m.key} style={{ padding: "8px 8px", textAlign: "right", fontWeight: 600, color: isGood ? "#22c55e" : "#f59e0b" }}>
                                  {m.fmt(val)}
                                </td>
                              );
                            })}
