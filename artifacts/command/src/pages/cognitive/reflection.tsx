...
const SEEDED_REFLECTIONS: Reflection[] = [
  {
    ...
    context:
      'Incident #1182: LockBit 3.0 lateral movement across 14 endpoints. Agent successfully classified threat and coordinated containment but did not pre-identify OT gateway dependency before issuing isolation order.',
    whatWorked: [
    ...
    lesson:
      'Before bulk endpoint isolation actions, the agent must query the CMDB/OT asset register to identify OT-adjacent hosts and route those to a separate, slower approval flow. The isolation step should be decomposed into OT-safe and OT-risk subgroups.',
    candidateSkills: [
      {
        name: 'ot-dependency-preflight',
        description:
          'Query the OT asset register and CMDB before any bulk isolation or network policy action to identify OT-adjacent endpoints and apply a slower approval path.',
        status: 'under-review',
      ...
      },
      {
        ...
        description:
          'Split endpoint isolation plans into OT-safe and OT-risk subgroups with different approval gates and latency budgets.',
        status: 'proposed',
        estimatedImpact: 'high',
      },
    ],
    strategyUpdates: [
      {
        ...
        proposed:
          'Two-track isolation: OT-safe hosts isolated immediately; OT-risk hosts routed to manual approval.',
        rationale:
          'Prevents unacceptable OT operational disruption while maintaining fast response on non-OT hosts.',
      },
    ],
  ...
  },
  {
    ...
    context:
      'Voyage reroute for MV Aurora Constellation around Cyclone Halia. Overall outcome successful: vessel safe, cargo delivered 28h late. Reflects on what can be improved for similar future events.',
    whatWorked: [
    ...
    lesson:
      'Operator approval latency dominates voyage planning plan time. The agent should initiate approval requests earlier (pre-emptively, before data is 100% complete) and provide the full economic comparison of all alternatives at first presentation to reduce round-trips.',
    candidateSkills: [
      {
        name: 'pre-emptive-approval-initiation',
        description:
          'Send a preliminary approval request with high-confidence partial data when confidence >85%, rather than waiting for full data collection to complete.',
        status: 'proposed',
      ...
      },
      {
        ...
        description:
          'Always compute and present full economic comparison for all viable alternatives (cost, ETA delta, CII impact, berth availability) in a single output to reduce operator round-trips.',
        status: 'adopted',
        estimatedImpact: 'medium',
      },
    ],
    strategyUpdates: [
      {
        ...
        proposed:
          'Preliminary approval request sent when confidence >85% with clear uncertainty markers; updated automatically when data is complete.',
        rationale:
          'Reduces operator idle wait time by an estimated 40–60% while preserving operator autonomy.',
      },
    ],
  ...
  },
  {
    ...
    context:
      'Interest rate stress analysis for NYC mid-market portfolio. Verifier check COMP-002 failed because industrial asset cap rate model used 2024 benchmark data instead of current 2026 comps. Plan paused pending remediation.',
    whatWorked: [
    ...
    lesson:
      'The agent must perform a data staleness check on all comparable sets before invoking the rate shock model. Any comp set older than 12 months for a primary asset class should block execution and surface a data quality alert rather than silently proceeding.',
    candidateSkills: [
      {
        name: 'comp-set-staleness-gate',
        description:
          'Check the age of all comparable transaction datasets before model invocation. Block execution and raise a data quality alert if any primary asset class comp set is older than 12 months.',
        status: 'under-review',
        estimatedImpact: 'high',
      },
    ],
    strategyUpdates: [
      {
        ...
        proposed:
          'Pre-model staleness gate: fail fast if any primary comp set >12 months old. Surface as explicit blocker requiring analyst resolution.',
        rationale:
          'Prevents silent use of stale data that passes all downstream checks except a dedicated verifier rule.',
      },
      {
        area: 'Risk Flagging',
        current: 'Data quality risks surfaced only if verifier detects them.',
        proposed:
          'Agent proactively flags data quality risks in its own output before verifier runs.',
        rationale:
          'Reduces reliance on verifier as the primary data quality control — defence in depth.',
      },
    ],
  ...
  },
  {
    ...
    context:
      'GDPR breach notification workflow for a data subject access incident. Agent discovered a re-usable contact resolution pattern for regional DPA offices not in the standard lookup table, now proposed as a candidate skill.',
    whatWorked: [
    ...
    lesson:
      'The 2-step DPA contact resolution fallback (web research → legal DB verification) proved robust for non-standard jurisdictions. This pattern should be codified as a reusable skill and made available to all compliance-domain agents.',
    candidateSkills: [
      {
        name: 'dpa-contact-resolution-v2',
        description:
          'Two-step fallback for DPA contact resolution: (1) structured lookup in legal DB; (2) web research with source verification. Handles 50+ EU/EEA jurisdictions including non-standard cases (Liechtenstein, San Marino, etc.).',
        status: 'adopted',
        estimatedImpact: 'medium',
      },
    ],
  ...
  },
  {
    ...
    context:
      'Routine quarterly portfolio review across 42 NYC mid-market assets. Revealed a pattern of confidence overstatement when the model operates outside its historical training distribution.',
    whatWorked: [
    ...
    lesson:
      "The valuation model's confidence estimator was trained on 2021–2024 market conditions. Post-2025 rate environment is outside distribution; confidence scores must be down-calibrated by an estimated 0.15–0.20 for affected asset classes until recalibration data is available.",
    candidateSkills: [
      {
        name: 'ood-confidence-calibration',
        description:
          'Detect when model inputs fall outside training distribution and apply automatic confidence down-calibration, with explicit OOD flag surfaced in model output.',
        status: 'under-review',
        estimatedImpact: 'high',
      },
    ],
    strategyUpdates: [
      {
        ...
        proposed:
          'Apply OOD detection gate; down-calibrate confidence for out-of-distribution inputs and add explicit OOD flag to output.',
        rationale:
          'Prevents operators from over-relying on confidence scores that do not reflect true model uncertainty in novel market conditions.',
      },
    ],
...
export default function ReflectionConsole() {
  ...
  const [strategyDecisions, setStrategyDecisions] = useState<Record<string, StrategyDecision>>({});
  ...
  const decideSkill = useCallback(
    async (reflectionId: string, skillName: string, action: 'adopt' | 'reject') => {
      ...
      const optimistic: CandidateSkillStatus =
        action === 'adopt' ? (previous === 'proposed' ? 'under-review' : 'adopted') : 'rejected';
      setPending(key, true);
  ...
  const decideStrategy = useCallback(
    async (reflectionId: string, index: number, action: 'apply' | 'defer') => {
      ...
      try {
        await fetchJson(
          apiUrl(`/reflections/${encodeURIComponent(reflectionId)}/strategy/${index}/${action}`),
          { method: 'POST', body: JSON.stringify({}) },
        );
        toast.success(action === 'apply' ? 'Strategy update applied' : 'Strategy update deferred');
      } catch (err) {
  ...
  const types: Array<ReflectionType | 'all'> = [
    'all',
    'post-incident',
    'performance-review',
    'strategy-update',
  ...
  const proposedSkills = allCandidateSkills.filter(
    (s) => s.status === 'proposed' || s.status === 'under-review',
  ).length;

  return (
    <CognitiveLayout
      title="Reflection Console"
      subtitle="Agent reflections: what worked, what failed, lessons learned, candidate skills, and strategy updates — each linked to a source trace."
    >
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <select
          value={outcomeFilter}
          onChange={(e) => setOutcomeFilter(e.target.value as OutcomeClass | 'all')}
          style={{
            background: 'rgba(255,255,255,0.04)',
            border: '1px solid rgba(255,255,255,0.1)',
            borderRadius: 7,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 20 }}>
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              >
                <div style={{ display: 'flex', gap: 5, marginBottom: 6, flexWrap: 'wrap' }}>
                  <span
                    style={{
                      fontSize: 9,
                      fontWeight: 700,
                      color: DOMAIN_COLORS[ref.domain] ?? DOMAIN_COLORS.default,
                      background: `${DOMAIN_COLORS[ref.domain] ?? DOMAIN_COLORS.default}15`,
                      padding: '1px 6px',
                      borderRadius: 3,
                    }}
                  >
                  ...
                  <span
                    style={{
                      ...
                      background: `${TYPE_COLORS[ref.type]}15`,
                      padding: '1px 6px',
                      borderRadius: 3,
                    }}
                  >
                  ...
                  <span
                    style={{
                      fontSize: 9,
                      fontWeight: 700,
                      color: OUTCOME_COLORS[ref.outcomeClass],
                      background: `${OUTCOME_COLORS[ref.outcomeClass]}15`,
                      padding: '1px 6px',
                      borderRadius: 3,
                      marginLeft: 'auto',
                    }}
                ...
                <div style={{ display: 'flex', gap: 8, fontSize: 10, color: '#334155' }}>
                  ...
                  <span style={{ marginLeft: 'auto' }}>
                    {new Date(ref.timestamp).toLocaleDateString(undefined, {
                      month: 'short',
                      day: 'numeric',
                    })}
                  </span>
                    ...
                    {ref.candidateSkills.map((skill) => (
                      <span
                        ...
                        style={{
                          ...
                          background: `${SKILL_STATUS_COLORS[skill.status]}15`,
                          padding: '1px 6px',
                          borderRadius: 3,
                          border: `1px solid ${SKILL_STATUS_COLORS[skill.status]}30`,
                        }}
                      >
                        ⊕ {skill.name}
                      </span>
            ...
            {filtered.length === 0 && (
              <div
                style={{ textAlign: 'center', color: '#475569', fontSize: 13, padding: '40px 0' }}
              >
        ...
        <div>
          <div
            style={{
              ...
              border: `1px solid ${OUTCOME_COLORS[selected.outcomeClass]}30`,
              borderRadius: 12,
              padding: 20,
              marginBottom: 16,
            }}
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
              <span
                style={{
                  fontSize: 10,
                  fontWeight: 700,
                  color: DOMAIN_COLORS[selected.domain] ?? DOMAIN_COLORS.default,
                  background: `${DOMAIN_COLORS[selected.domain] ?? DOMAIN_COLORS.default}15`,
                  padding: '2px 8px',
                  borderRadius: 4,
                }}
              >
              ...
              <span
                style={{
                  fontSize: 10,
                  fontWeight: 700,
                  color: OUTCOME_COLORS[selected.outcomeClass],
                  background: `${OUTCOME_COLORS[selected.outcomeClass]}15`,
                  padding: '2px 8px',
                  borderRadius: 4,
                  textTransform: 'capitalize',
                }}
          ...
          >
            {(['overview', 'skills', 'strategy'] as const).map((tab) => (
              ...
              >
                {tab === 'skills'
                  ? `Skills (${selected.candidateSkills.length})`
                  : tab === 'strategy'
                    ? `Strategy (${selected.strategyUpdates.length})`
                    : 'Overview'}
              </button>
            ))}
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {selected.candidateSkills.length === 0 && (
                <div
                  style={{ textAlign: 'center', color: '#475569', fontSize: 13, padding: '40px 0' }}
                >
                  No candidate skills for this reflection
                </div>
              )}
                      ...
                      <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                        <span
                          style={{
                            ...
                            fontWeight: 700,
                            color: SKILL_STATUS_COLORS[skill.status],
                            background: `${SKILL_STATUS_COLORS[skill.status]}18`,
                            padding: '2px 8px',
                            borderRadius: 4,
                            textTransform: 'capitalize',
                          }}
                        ...
                        <span
                          style={{
                            fontSize: 10,
                            fontWeight: 600,
                            color: IMPACT_COLORS[skill.estimatedImpact],
                            background: `${IMPACT_COLORS[skill.estimatedImpact]}15`,
                            padding: '2px 8px',
                            borderRadius: 4,
                          }}
                        >
                          Impact: {skill.estimatedImpact}
                        </span>
                      </div>
                      <div
                        style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {selected.strategyUpdates.length === 0 && (
                <div
                  style={{ textAlign: 'center', color: '#475569', fontSize: 13, padding: '40px 0' }}
                >
                  No strategy updates for this reflection
                </div>
              )}
                    ...
                    {decision && (
                      <div
                        style={{
                          ...
                          fontSize: 10,
                          fontWeight: 700,
                          color: decision === 'applied' ? '#22c55e' : '#f59e0b',
                          textTransform: 'uppercase',
                          letterSpacing: 0.5,
                        }}
                      >
