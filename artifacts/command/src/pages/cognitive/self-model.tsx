...
const DEMO_SELF_MODEL: SelfModel = {
  ...
  capabilities: [
    {
      id: 'cap-1',
      name: 'Cross-Domain Risk Synthesis',
      category: 'Analysis',
      confidenceScore: 0.88,
      lastCalibrated: '2026-04-16',
      evidenceCount: 142,
      trend: 'improving',
      notes:
        'Strong performance on multi-domain correlation tasks; slight drop when Carlota data stale',
    },
    {
      id: 'cap-2',
      ...
      evidenceCount: 87,
      trend: 'stable',
      notes: 'Consistently rated high relevance by Stephen L. over trailing 30 briefings',
    },
    {
      id: 'cap-3',
      ...
      evidenceCount: 63,
      trend: 'degrading',
      notes: 'False positive rate elevated on VYG-class voyages. Recalibration in progress.',
    },
    {
      id: 'cap-4',
      name: 'Policy Compliance Reasoning',
      category: 'Governance',
      confidenceScore: 0.84,
      lastCalibrated: '2026-04-14',
      evidenceCount: 218,
      trend: 'stable',
      notes: 'Reliable across 8 policy domains; edge case in TIER-3 escalation path identified',
    },
    {
      id: 'cap-5',
      ...
      evidenceCount: 55,
      trend: 'improving',
      notes: 'Reprioritization after Carlota outage validated by Ops Lead; model improving',
    },
    {
      id: 'cap-6',
      name: 'LP Portfolio Summarization',
      category: 'Finance',
      confidenceScore: 0.94,
      lastCalibrated: '2026-04-12',
      evidenceCount: 34,
      trend: 'stable',
      notes: 'CFO validates outputs quarterly; no material errors in last 3 cycles',
    },
  ],
  confidenceProfiles: [
    {
    ...
    {
      domain: 'Carlota',
      calibrationAccuracy: 0.79,
      overconfidenceRate: 0.08,
      underconfidenceRate: 0.13,
      sampleCount: 41,
      lastUpdated: '2026-04-16',
    },
    {
...
const DEMO_HISTORY: SelfModelHistory = {
  snapshots: [
    ...
    {
      snapshotAt: '2026-04-16T09:00:00Z',
      version: 'v3.4.0',
      overallTrustScore: 0.83,
      autonomyTier: 'TIER-2',
      capabilityCount: 6,
      activeObjectives: 3,
      driftSummary:
        'Stable. LP portfolio summarization confidence improved after CFO validation of Q1 draft.',
      changeType: 'minor',
    },
    {
      snapshotAt: '2026-04-14T16:30:00Z',
      version: 'v3.3.9',
      ...
      activeObjectives: 5,
      driftSummary:
        'Upgraded from v3.3.8. New policy edge case in TIER-3 escalation path identified and logged. Trust slightly reduced pending fix.',
      changeType: 'moderate',
    },
    {
      snapshotAt: '2026-04-10T10:00:00Z',
      version: 'v3.3.8',
      ...
      activeObjectives: 4,
      driftSummary:
        'Major update: cross-domain risk synthesis capability added after successful 2-week pilot. Trust score improved significantly.',
      changeType: 'major',
    },
    {
      snapshotAt: '2026-04-05T08:00:00Z',
      version: 'v3.3.7',
    ...
    {
      snapshotAt: '2026-03-28T12:00:00Z',
      version: 'v3.3.6',
      ...
      activeObjectives: 4,
      driftSummary:
        'Objective prioritization capability confidence improved after Ops Lead validated reprioritization sequence.',
      changeType: 'minor',
    },
  ],
};
    ...
    <SectionCard
      ...
      headerRight={
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          ...
          <span style={{ fontSize: '8px', color: FG_MUT }}>
            snap {new Date(model.snapshotAt).toLocaleTimeString()}
          </span>
        </div>
      }
    >
      <div style={{ padding: '1rem' }}>
        <div style={{ marginBottom: '0.875rem' }}>
          ...
          <p style={{ fontSize: '11px', color: FG, lineHeight: 1.65, margin: 0 }}>
            {identity.purpose}
          </p>
        </div>
        <div style={{ marginBottom: '0.875rem' }}>
        ...
        <div style={{ marginBottom: '0.875rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
            {identity.boundaryConstraints.map((c, i) => (
              ...
              >
                <Shield
                  style={{ width: 9, height: 9, color: '#ef4444', flexShrink: 0, marginTop: 2 }}
                />
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
            {identity.uncertaintyAcknowledgements.map((u, i) => (
              ...
              >
                <Info
                  style={{ width: 9, height: 9, color: '#f59e0b', flexShrink: 0, marginTop: 2 }}
                />
...
function CapabilitiesSection({ capabilities }: { capabilities: Capability[] }) {
  ...
  const trendColor = (t: string) =>
    t === 'improving' ? '#22c55e' : t === 'degrading' ? '#ef4444' : FG_MUT;
  ...
  return (
    ...
    >
      {capabilities.map((cap, i) => (
        <div
          key={cap.id}
          style={{ borderBottom: i < capabilities.length - 1 ? `1px solid ${BORDER}` : undefined }}
        >
          <div
            style={{
              display: 'flex',
          ...
          >
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <span style={{ fontSize: '8px', color: trendColor(cap.trend), fontWeight: 600 }}>
                  {cap.trend}
                </span>
              </div>
            </div>
          ...
          {expanded === cap.id && (
            <div style={{ padding: '0 1rem 0.75rem 1rem' }}>
              <p style={{ fontSize: '10px', color: FG_MUT, margin: '0 0 0.5rem', lineHeight: 1.6 }}>
                {cap.notes}
              </p>
              <div style={{ display: 'flex', gap: '1rem', fontSize: '9px', color: FG_MUT }}>
                <span>
                  Evidence: <strong style={{ color: FG }}>{cap.evidenceCount} samples</strong>
                </span>
                <span>
                  Calibrated: <strong style={{ color: FG }}>{cap.lastCalibrated}</strong>
                </span>
              </div>
            </div>
          )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.625rem' }}>
          {profiles.map((p) => {
            ...
            return (
              ...
              >
                <div
                  style={{ fontSize: '10px', fontWeight: 700, color: FG, marginBottom: '0.5rem' }}
                >
                  {p.domain}
                </div>
                <div style={{ marginBottom: '0.375rem' }}>
                  <div style={{ fontSize: '8px', color: FG_MUT, marginBottom: '2px' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', fontSize: '8px', color: FG_MUT }}>
                  <span style={{ color: '#f97316' }}>
                    Over: {Math.round(p.overconfidenceRate * 100)}%
                  </span>
                  <span style={{ color: '#3b82f6' }}>
                    Under: {Math.round(p.underconfidenceRate * 100)}%
                  </span>
                </div>
                <div style={{ fontSize: '8px', color: FG_MUT, marginTop: '2px' }}>
...
function SnapshotTimeline({ snapshots }: { snapshots: SnapshotEntry[] }) {
  ...
  const changeBg: Record<string, string> = {
    minor: 'rgba(107,114,128,0.1)',
    moderate: 'rgba(245,158,11,0.1)',
    major: `rgba(139,122,200,0.1)`,
  };
  ...
  return (
    <SectionCard
      ...
      icon={GitBranch}
      headerRight={<span style={{ fontSize: '9px', color: FG_MUT }}>Drift history</span>}
    >
            ...
            return (
              <div
                ...
                style={{
                  display: 'flex',
                  gap: '0.75rem',
                  paddingBottom: i < snapshots.length - 1 ? '0.875rem' : 0,
                  position: 'relative',
                }}
              >
                <div
                    ...
                    >
                      <span
                        style={{
                          ...
                          fontWeight: 700,
                          color:
                            tscore >= 0.85 ? '#22c55e' : tscore >= 0.75 ? '#f59e0b' : '#ef4444',
                        }}
                      >
      ...
      <div style={{ padding: '1.5rem 2rem', maxWidth: 1400, margin: '0 auto' }}>
        <div style={{ marginBottom: '1.25rem' }}>
          ...
          <p style={{ fontSize: '11px', color: FG_MUT, margin: 0 }}>
            What this system believes about itself — identity, capabilities, calibration, and drift
            over time.
          </p>
        </div>
        ...
        >
          {[
            {
              label: 'Trust Score',
              value: `${Math.round(model.overallTrustScore * 100)}%`,
              color:
                model.overallTrustScore >= 0.85
                  ? '#22c55e'
                  : model.overallTrustScore >= 0.75
                    ? '#f59e0b'
                    : '#ef4444',
            },
            ...
            {
              label: 'Snapshot',
              value: new Date(model.snapshotAt).toLocaleTimeString([], {
                hour: '2-digit',
                minute: '2-digit',
              }),
              color: FG_MUT,
