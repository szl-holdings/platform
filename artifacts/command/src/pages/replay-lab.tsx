...
function OutcomeBadge({ outcome }: { outcome?: string | null }) {
  ...
  return (
    <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1, color, background: `${color}18`, borderRadius: 4, padding: "2px 8px", border: `1px solid ${color}40` }}>
...
function CompareOutcomePanel({ runA, runB, onClose }: ComparePanel & { onClose: () => void }) {
  const metrics: { label: string; format: (v: ReplayRun) => string; delta: number; higherBetter: boolean; suffix?: string }[] = [
    {
      label: "GT Match Rate",
...
export default function ReplayLab() {
    ...
    async function load() {
      try {
        ...
        const [scenariosResp, runsResp] = await Promise.all([
          fetchJson<{ scenarios: Scenario[]; total: number; domains: string[] }>(apiUrl("/replay/scenarios")),
  ...
  const filtered = filterDomain === "all" ? scenarios : scenarios.filter(s => s.domain === filterDomain);
    ...
    try {
      ...
      setScenarios(prev => prev.map(s =>
        s.scenarioId === scenario.scenarioId
          ? { ...s, lastReplayed: run.startedAt, lastOutcome: run.successful === run.totalSnapshots ? "pass" : run.failed === run.totalSnapshots ? "fail" : "partial", groundTruthMatchRate: run.groundTruthMatchRate }
          : s
      ));
    } catch (err) {
  ...
  const avgGtMatch = runs.length > 0 ? runs.reduce((s, r) => s + r.groundTruthMatchRate, 0) / runs.length : 0;
          ...
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 28 }}>
              {[
                ...
                { label: "Avg GT Match", value: `${(avgGtMatch * 100).toFixed(0)}%`, color: "#f59e0b" },
              ].map(m => (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 20 }}>
              <div>
                ...
                {filtered.length === 0 && (
                  <p style={{ textAlign: "center", color: "#475569", fontSize: 13, padding: "40px 0" }}>No scenarios found</p>
                )}
                ...
                {filtered.map(s => (
                  <div
                    ...
                    style={{
                      background: selected?.scenarioId === s.scenarioId ? `${ACCENT}12` : "rgba(255,255,255,0.03)",
                      border: selected?.scenarioId === s.scenarioId ? `1px solid ${ACCENT}60` : "1px solid rgba(255,255,255,0.07)",
                      borderRadius: 10,
                      padding: "16px 18px",
                      cursor: "pointer",
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                      ...
                      <button
                        ...
                        style={{ background: replayingId === s.scenarioId ? "#334155" : ACCENT, color: "#fff", border: "none", borderRadius: 6, padding: "6px 14px", fontSize: 11, fontWeight: 600, cursor: replayingId === s.scenarioId ? "not-allowed" : "pointer", flexShrink: 0 }}
                      >
              ...
              <div>
                    ...
                    <div>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        {[
                          ...
                          { label: "GT Match Rate", value: selected.groundTruthMatchRate != null ? `${(selected.groundTruthMatchRate * 100).toFixed(0)}%` : "—" },
                        ].map(row => (
                      ...
                      {runError && (
                        <div style={{ marginTop: 8, padding: "6px 10px", background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 6, fontSize: 11, color: "#fca5a5" }}>
                          {runError}
                        </div>
                      )}
                    </div>
                ...
                <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: 18 }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                    ...
                    {!compareMode ? (
                      <button
                        ...
                        style={{ fontSize: 11, color: ACCENT, background: `${ACCENT}15`, border: "none", borderRadius: 4, padding: "3px 10px", cursor: "pointer", fontWeight: 600 }}
                      >
                        Compare
                      </button>
                    ) : (
                      <div style={{ display: "flex", gap: 6 }}>
                        {compareIds.length === 2 && (
                          <button
                            ...
                            style={{ fontSize: 11, color: "#fff", background: ACCENT, border: "none", borderRadius: 4, padding: "3px 10px", cursor: "pointer", fontWeight: 600 }}
                          >
                        ...
                        <button
                          ...
                          style={{ fontSize: 11, color: "#475569", background: "rgba(255,255,255,0.05)", border: "none", borderRadius: 4, padding: "3px 10px", cursor: "pointer" }}
                        >
                          Cancel
                        </button>
                      </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: compareMode ? "20px 1fr 70px 70px 80px 70px" : "1fr 70px 70px 80px 70px 80px", gap: 8, padding: "6px 0", borderBottom: "1px solid rgba(255,255,255,0.08)", marginBottom: 4 }}>
                    {compareMode && <div />}
                    {["Scenario", "Pass", "GT%", "Latency", "Cost", compareMode ? "" : "Date"].map(h => (
                  ...
                  {runs.length === 0 && (
                    <p style={{ fontSize: 12, color: "#475569", textAlign: "center", padding: "20px 0" }}>No runs yet — replay a scenario</p>
                  )}
                </div>
              </div>
            </div>
