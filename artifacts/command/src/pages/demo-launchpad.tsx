...
import {
  Play, RotateCcw, Clock, Users, Zap, Shield, Map, Ship, Crown,
  ChevronRight, CheckCircle2, Circle, ArrowRight, AlertTriangle,
  BarChart3, Brain, Lock, GitBranch, Database, Star, Layers,
  Activity, Eye, BookOpen, Target, X, Info,
} from "lucide-react";
...
const ALL_STOPS: DemoStop[] = [
  ...
  },
  {
    ...
    domain: "Lyte",
    domainColor: ACCENT,
    description: "Simulate approve / delay / reroute / escalate with downstream impact and confidence bands.",
  ...
  },
  {
    ...
    domain: "Alloy",
    domainColor: "#d4a054",
    description: "Plain-English → validated, versioned, rollback-able policy objects with workflow coverage.",
  ...
  },
  {
    ...
    domain: "Terra",
    domainColor: "#4ade80",
    description: "Ranked acquisition thesis combining liens, violations, ownership complexity, and neighborhood trend.",
  ...
  },
  {
    ...
    domain: "Aegis",
    domainColor: "#f87171",
    description: "Readable incident storyline: attack chain, MITRE mapping, affected assets, confidence.",
  ...
  },
  {
    ...
    domain: "Vessels",
    domainColor: "#60a5fa",
    description: "Route-level risk: AIS gaps, sanctions, weather, voyage economics → explained risk + alternatives.",
  ...
  },
  {
    ...
    domain: "Command",
    domainColor: "#8b7ac8",
    description: "Cross-domain evidence registry — provenance, freshness, confidence on every claim.",
  ...
  },
  {
    ...
    domain: "Command",
    domainColor: "#8b7ac8",
    description: "Pressure map, ownership drift, blocker board, and action debt across all domains.",
...
const PERSONA_CONFIGS: Record<Persona, {
  label: string;
  ...
  investor: {
    ...
    focusIds: ["command-overview", "lyte-decision-twin", "alloy-policy-compiler", "terra-why-now", "aegis-adversary", "vessels-voyage-risk", "carlota-concierge"],
  },
  ceo: {
    label: "CEO",
    ...
    focusIds: ["command-overview", "lyte-signals", "lyte-decision-twin", "terra-why-now", "carlota-concierge", "alloy-audit"],
  },
  coo: {
    label: "COO",
    ...
    focusIds: ["lyte-signals", "lyte-decision-twin", "alloy-policy-compiler", "prism-dashboard", "alloy-audit", "command-evidence"],
  },
  ciso: {
    label: "CISO",
    ...
    focusIds: ["aegis-adversary", "alloy-audit", "command-evidence", "alloy-policy-compiler", "prism-dashboard"],
  },
  analyst: {
    label: "Analyst",
    ...
    focusIds: ["lyte-signals", "terra-why-now", "terra-map", "vessels-voyage-risk", "command-evidence", "aegis-adversary"],
  },
};
...
const DEMO_PATHS: Record<DemoLength, { label: string; description: string; stopIds: string[] }> = {
  ...
  20: {
    ...
    stopIds: ["command-overview", "lyte-signals", "lyte-decision-twin", "alloy-policy-compiler", "terra-why-now", "alloy-audit"],
  },
  45: {
    label: "45-Minute",
    description: "Complete platform tour. All six domain packs, six innovations, full governance proof.",
    stopIds: ["command-overview", "lyte-signals", "lyte-decision-twin", "alloy-policy-compiler", "terra-why-now", "terra-map", "aegis-adversary", "vessels-voyage-risk", "carlota-concierge", "command-evidence", "alloy-audit"],
  },
};
...
function StopCard({
  ...
  return (
    <div
      ...
      style={{
        background: active ? `${stop.domainColor}10` : completed ? `rgba(255,255,255,0.02)` : SURFACE,
    ...
    >
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
        <div style={{
          ...
          height: 28,
          borderRadius: 6,
          background: completed ? `${stop.domainColor}20` : active ? `${stop.domainColor}25` : "rgba(255,255,255,0.05)",
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            ...
            {stop.tag && <Tag label={stop.tag} color={stop.tag === "SIGNATURE" ? stop.domainColor : "#c9a227"} />}
          </div>
          <p style={{ fontSize: 11, color: TEXT_SECONDARY, marginTop: 4, lineHeight: 1.5 }}>{stop.description}</p>
...
export function DemoLaunchpad() {
  ...
  const [resetResult, setResetResult] = useState<null | { ops: Array<{ operation: string; status: string; detail?: string }> }>(null);
  ...
  const effectiveStopIds = selectedPersona !== "investor"
    ? demoPath.stopIds.filter(id => personaConfig.focusIds.includes(id))
    : demoPath.stopIds;

  const activeStops = effectiveStopIds.map(id => ALL_STOPS.find(s => s.id === id)!).filter(Boolean);
  ...
  const handleReset = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setResetResult({ ops: [{ operation: "api_error", status: "skipped", detail: `Server returned ${res.status} — UI state cleared locally` }] });
      } else {
        const data = await res.json();
        if (data?.operations) {
          setResetResult({ ops: data.operations });
        } else {
          setResetResult({ ops: [{ operation: "reset_complete", status: "done", detail: "Demo state reset — all stops, persona, and track cleared" }] });
        }
      }
    } catch {
      setResetResult({ ops: [{ operation: "server_unreachable", status: "skipped", detail: "API server not reachable — UI state cleared locally" }] });
    }
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, color: TEXT_PRIMARY, fontFamily: "Inter, system-ui, sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 32 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
              ...
              <div>
                <h1 style={{ fontSize: 20, fontWeight: 700, color: TEXT_PRIMARY, margin: 0 }}>Demo Launchpad</h1>
                <p style={{ fontSize: 11, color: TEXT_MUTED, margin: 0, fontFamily: "monospace" }}>
                  SZL HOLDINGS — GOVERNED INTELLIGENCE PLATFORM
                </p>
              </div>
            </div>
            <p style={{ fontSize: 13, color: TEXT_SECONDARY, maxWidth: 580, lineHeight: 1.6, margin: 0 }}>
              A single presenter control surface for investor demos. Select your audience, choose a time track, and launch each stop in sequence. One-click reset returns to clean state.
            </p>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            {showReset && (
              <div style={{
                ...
                <button
                  onClick={handleReset}
                  style={{ fontSize: 11, fontWeight: 600, color: "#f87171", background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 6, padding: "4px 10px", cursor: "pointer" }}
                >
                  Reset
                </button>
                <button
          ...
          <div style={{
            ...
            {resetting ? (
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <RotateCcw size={13} color={ACCENT} style={{ animation: "spin 1s linear infinite" }} />
                <span style={{ fontSize: 12, color: TEXT_SECONDARY, fontFamily: "monospace" }}>Resetting demo state…</span>
              </div>
            ) : resetResult && (
              <>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <CheckCircle2 size={13} color="#34d399" />
                  <span style={{ fontSize: 12, fontWeight: 600, color: "#34d399", fontFamily: "monospace" }}>RESET COMPLETE — Vantex scenario fresh</span>
                </div>
                {resetResult.ops.map((op, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 8, paddingLeft: 20 }}>
                    <span style={{ fontSize: 10, color: op.status === "done" ? "#34d399" : TEXT_MUTED, fontFamily: "monospace", minWidth: 40, marginTop: 1 }}>{op.status.toUpperCase()}</span>
                    <span style={{ fontSize: 10, color: TEXT_MUTED, fontFamily: "monospace" }}>{op.detail ?? op.operation}</span>
                  </div>
                ))}
              </>
            )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 20 }}>
              <p style={{ fontSize: 10, fontFamily: "monospace", color: TEXT_MUTED, letterSpacing: "0.12em", marginBottom: 12 }}>
                AUDIENCE PERSONA
              </p>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {(Object.entries(PERSONA_CONFIGS) as [Persona, typeof PERSONA_CONFIGS[Persona]][]).map(([key, cfg]) => (
                  <button
                    key={key}
              ...
              {selectedPersona !== "investor" && (
                <p style={{ fontSize: 11, color: TEXT_SECONDARY, marginTop: 10 }}>{personaConfig.description}</p>
              )}
            </div>
                ...
                {([10, 20, 45] as DemoLength[]).map(len => (
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                      ...
                      <span style={{ fontSize: 14, fontWeight: 700, color: selectedLength === len ? ACCENT : TEXT_PRIMARY }}>
            ...
            <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 20 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                <div>
                  <p style={{ fontSize: 10, fontFamily: "monospace", color: TEXT_MUTED, letterSpacing: "0.12em", margin: 0 }}>
                    STOP SEQUENCE
                  </p>
                  <p style={{ fontSize: 11, color: TEXT_SECONDARY, margin: "4px 0 0" }}>
                    {activeStops.length} stops · ~{totalTime} minutes total
              ...
              {activeStops.length === 0 && (
                <div style={{ textAlign: "center", padding: "40px 0", color: TEXT_MUTED }}>
                  ...
                  <p style={{ fontSize: 11 }}>Try switching to Investor persona for the full path.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 18 }}>
              <p style={{ fontSize: 10, fontFamily: "monospace", color: TEXT_MUTED, letterSpacing: "0.12em", marginBottom: 14 }}>
                PLATFORM STATUS
              </p>
              {[
                { label: "Lyte — Decision Intelligence", color: "#4ade80", href: "/lyte/", status: "LIVE" },
                { label: "Alloy — Policy Compiler", color: "#4ade80", href: `${BASE}/operations/alloy/policy-compiler`, status: "LIVE" },
                { label: "Terra — Real Estate Intelligence", color: "#4ade80", href: "/terra/", status: "LIVE" },
                { label: "Aegis — Cyber Resilience", color: "#4ade80", href: "/aegis/", status: "LIVE" },
                { label: "Vessels — Maritime Intelligence", color: "#4ade80", href: "/vessels/", status: "LIVE" },
                { label: "Carlota Jo — Concierge", color: "#4ade80", href: "/carlota-jo/", status: "LIVE" },
              ].map(item => (
                <div
                  key={item.label}
                ...
                >
                  ...
                  <span style={{ flex: 1, fontSize: 11, color: TEXT_SECONDARY, lineHeight: 1.3 }}>{item.label}</span>
                  <span style={{ fontSize: 9, fontFamily: "monospace", color: item.color }}>{item.status}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 18 }}>
              ...
              {[
                { label: "Decision Twin", domain: "Lyte", color: ACCENT, href: "/lyte/decision-twin" },
                { label: "Policy Compiler", domain: "Alloy", color: ACCENT, href: `${BASE}/operations/alloy/policy-compiler` },
                { label: "Why This Property Now", domain: "Terra", color: "#4ade80", href: "/terra/why-this-property-now" },
                { label: "Adversary Narrative Engine", domain: "Aegis", color: "#f87171", href: "/aegis/adversary-narrative-engine" },
                { label: "Voyage Risk Twin", domain: "Vessels", color: "#60a5fa", href: "/vessels/voyage-risk-twin" },
                { label: "White-Glove Command", domain: "Carlota Jo", color: "#9A7D52", href: "/carlota-jo/concierge" },
              ].map(item => (
                <div
                  key={item.label}
                  onClick={() => {
            ...
            <div style={{
              ...
              <p style={{ fontSize: 10, fontFamily: "monospace", color: ACCENT, letterSpacing: "0.12em", marginBottom: 10 }}>
              ...
              <p style={{ fontSize: 11, color: TEXT_SECONDARY, lineHeight: 1.6, margin: 0 }}>
                ...
                <strong style={{ color: TEXT_PRIMARY }}>Vantex Acquisition</strong> — a $4.2M / 47-day stalled approval chain used to demonstrate the governed decision loop end-to-end.
              </p>
              <div style={{
                marginTop: 12,
                padding: "8px 12px",
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[
                  ...
                  <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                    <span style={{ fontSize: 10, fontFamily: "monospace", color: TEXT_MUTED, flexShrink: 0, marginTop: 1 }}>
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <p style={{ fontSize: 11, color: TEXT_SECONDARY, margin: 0, lineHeight: 1.5 }}>{note}</p>
                  </div>
                ))}
              </div>
            </div>
