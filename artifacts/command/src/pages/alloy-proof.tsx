...
const AUTONOMY_OPTIONS: AutonomyMode[] = [
  'observe',
  'recommend',
  'draft',
  'ask-to-act',
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            ...
            <Chip
              label={`approval: ${result.approvalMode}`}
              color={APPROVAL_COLOR[result.approvalMode]}
            />
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 16, fontSize: 11, color: '#475569' }}>
          ...
          {result.freshness.validUntil && (
            <span>valid until {new Date(result.freshness.validUntil).toLocaleString()}</span>
          )}
      ...
      {result.policyDecision && (
        ...
        >
          ...
          <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 8 }}>
            {result.policyDecision.reasoning}
          </div>
...
const DEMO_PARAMS = {
  ...
  summary:
    'Cross-domain signals indicate a 23% probability of supply chain stress in July. Pre-positioning 15% buffer inventory at key depots reduces expected stockout exposure by $4.2M.',
  reasoning:
    'Historical Q3 patterns (2021–2024), current supplier lead-time degradation (+18 days), and demand forecast confidence from the logistics model all converge on a shared risk window. Policy allows procurement at this scale without escalation.',
  domain: 'vessels',
  ...
  inlineEvidence: [
    {
      ...
      label: 'Q3 Historical Stockout Rate',
      value: '14.2% average across 4 years',
      source: 'vessels.analytics',
      confidence: 0.95,
    },
    {
      kind: 'signal' as const,
      label: 'Supplier Lead-Time Alert',
      value: '+18 days average vs. baseline (from 6 of 9 key suppliers)',
      source: 'vessels.supplier-monitor',
      confidence: 0.88,
    },
    {
      kind: 'observation' as const,
      label: 'Demand Forecast Delta',
      value: '+23% above baseline for July–August period',
      source: 'vessels.demand-model',
      confidence: 0.72,
    },
    {
      kind: 'document' as const,
      label: 'Q2 Board Directive',
      value: 'Minimize stockout exposure to <5% for Tier-1 routes by end of year',
      source: 'szl.governance',
      confidence: 1.0,
    },
  ],
};
    ...
    >
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 24px' }}>
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setParams((p) => ({ ...p, autonomyMode: e.target.value as AutonomyMode }))
                }
                style={{
                  width: '100%',
                  background: '#1e293b',
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setParams((p) => ({ ...p, baseConfidence: parseFloat(e.target.value) }))
                }
          ...
          >
            <span style={{ color: '#8b7ac8', fontWeight: 600 }}>{params.autonomyMode}</span> —{' '}
            {AUTONOMY_DESCRIPTIONS[params.autonomyMode]}
          </div>
        </div>
