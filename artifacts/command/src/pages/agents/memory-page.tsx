    ...
    <div style={{ background: '#080c14', minHeight: '100vh', color: '#e2e8f0', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ borderBottom: '1px solid #1e293b', padding: '20px 32px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: '#64748b' }}>5 memory tiers — freshness monitoring · retention policy · compaction status</div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1, borderBottom: '1px solid #1e293b' }}>
        {[
          ...
          { label: 'Redacted Entries', value: totalRedacted.toLocaleString(), color: '#64748b', icon: Archive },
        ].map((s) => (
      ...
      <div style={{ padding: '20px 32px' }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ fontSize: 11, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>Memory Tiers</div>
          <div style={{ display: 'grid', gap: 10 }}>
            {MEMORY_TIERS.map((tier) => (
              <div key={tier.tier} style={{ background: '#0f172a', borderRadius: 10, border: `1px solid ${tier.color}18`, padding: '16px 18px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: '#64748b' }}>{tier.description}</div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: 20, fontWeight: 800, color: tier.color }}>{Math.round(tier.freshnessScore * 100)}%</div>
                    <div style={{ fontSize: 10, color: '#64748b' }}>freshness</div>
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, marginBottom: 12 }}>
                  {[
                    ...
                    { label: 'Stale', value: tier.staleEntries.toLocaleString(), color: tier.staleEntries > 100 ? '#d4a054' : '#64748b' },
                    { label: 'Redacted', value: tier.redactedEntries.toLocaleString(), color: '#64748b' },
                    { label: 'Policy Adherence', value: `${Math.round(tier.policyAdherence * 100)}%`, color: tier.policyAdherence >= 0.95 ? '#22c55e' : '#d4a054' },
                  ].map((m) => (
                    <div key={m.label}>
                      <div style={{ fontSize: 9, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 3 }}>{m.label}</div>
                ...
                <div>
                  ...
                  <div style={{ height: 6, background: '#1e293b', borderRadius: 3, overflow: 'hidden', display: 'flex' }}>
                    ...
                    <div style={{ width: `${(tier.compactedEntries / tier.totalEntries) * 100}%`, background: '#1e293b' }} />
                  </div>
                  <div style={{ display: 'flex', gap: 12, marginTop: 5 }}>
                    {[{ label: 'Fresh', color: '#22c55e' }, { label: 'Stale', color: '#d4a054' }, { label: 'Redacted', color: '#475569' }, { label: 'Compacted', color: '#1e293b' }].map((l) => (
        ...
        {/* Retention Policy */}
        <div>
          <div style={{ fontSize: 11, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>Data Retention Policy</div>
          <div style={{ background: '#0f172a', borderRadius: 10, border: '1px solid #1e293b', overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1.2fr 1.5fr auto', padding: '10px 16px', borderBottom: '1px solid #1e293b' }}>
            ...
            {RETENTION_POLICY.map((r, i) => (
              <div key={r.tier} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1.2fr 1.5fr auto', padding: '12px 16px', borderBottom: i < RETENTION_POLICY.length - 1 ? '1px solid #0f172a' : 'none' }}>
                <div style={{ fontSize: 12, color: '#e2e8f0', fontWeight: 500 }}>{r.tier}</div>
