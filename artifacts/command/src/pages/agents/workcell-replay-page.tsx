...
import { WORKCELL_MAP, type TraceStep } from '@szl/a11oy-runtime';
    ...
    >
      ...
      <div style={{ height: 3, background: '#0f172a' }}>
        <div
          style={{
            height: '100%',
            width: `${progress}%`,
            background: 'linear-gradient(90deg, #8b7ac8, #a78bfa)',
      ...
      >
        ...
        <button
          ...
          style={{
            background: playing ? 'rgba(212,160,84,0.15)' : 'rgba(139,122,200,0.15)',
            border: `1px solid ${playing ? 'rgba(212,160,84,0.3)' : 'rgba(139,122,200,0.3)'}`,
            borderRadius: 6,
            padding: '6px 16px',
            cursor: 'pointer',
            color: playing ? '#d4a054' : '#8b7ac8',
            display: 'flex',
            alignItems: 'center',
            gap: 6,
            fontSize: 12,
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {([1, 2, 4] as const).map((s) => (
            <button
              ...
              onClick={() => setSpeed(s)}
              style={{
                background: speed === s ? 'rgba(139,122,200,0.15)' : 'transparent',
                border: `1px solid ${speed === s ? 'rgba(139,122,200,0.3)' : '#1e293b'}`,
                borderRadius: 4,
                padding: '4px 10px',
                cursor: 'pointer',
                color: speed === s ? '#8b7ac8' : '#64748b',
                fontSize: 11,
              }}
            >
              {s}×
        ...
        <div
          style={{ marginLeft: 'auto', display: 'flex', gap: 16, fontSize: 11, color: '#64748b' }}
        >
      ...
      <div
        style={{ display: 'grid', gridTemplateColumns: '1fr 400px', height: 'calc(100vh - 130px)' }}
      >
        {/* Timeline */}
          ...
          {currentStep === -1 && (
            ...
            >
              ...
              <div style={{ fontSize: 14, color: '#64748b' }}>
                Press Play to replay the execution trace
              </div>
              <div style={{ fontSize: 12, color: '#475569' }}>
                {steps.length} steps · {wc.title}
              </div>
            </div>
          )}
          ...
          {visibleSteps.map((step, i) => {
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <div
                    style={{
                      ...
                      height: 32,
                      borderRadius: 8,
                      background: isActive ? `${color}22` : `${color}10`,
                ...
                <div style={{ fontSize: 10, color: '#334155', paddingTop: 3 }}>
            ...
            <>
              <div style={{ marginBottom: 20 }}>
                ...
                <div
                  style={{
                    background: '#0f172a',
                    borderRadius: 8,
                    border: `1px solid ${STATUS_COLOR[activeStep.status]}28`,
                    padding: 14,
                  }}
                >
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: '#94a3b8' }}>
                        {activeStep.type.replace(/_/g, ' ')}
                      </div>
                    </div>
                    <div>
                      <div style={{ fontSize: 10, color: '#64748b' }}>Status</div>
                      <div style={{ fontSize: 12, color: STATUS_COLOR[activeStep.status] }}>
                        {activeStep.status}
                      </div>
                    </div>
                    {activeStep.tokens && (
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: '#94a3b8' }}>
                          {activeStep.tokens.toLocaleString()}
                        </div>
                      </div>
                    )}
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: '#94a3b8' }}>
                        {(activeStep.latencyMs / 1000).toFixed(1)}s
                      </div>
                    </div>
                    {activeStep.operatorId && (
                      <div>
                        <div style={{ fontSize: 10, color: '#64748b' }}>Operator</div>
                        <div style={{ fontSize: 12, color: '#8b7ac8' }}>
                          {activeStep.operatorId}
                        </div>
                      </div>
                    )}
                    {activeStep.toolId && (
                      <div>
                        <div style={{ fontSize: 10, color: '#64748b' }}>Tool</div>
                        <div style={{ fontSize: 12, color: '#d4a054' }}>{activeStep.toolId}</div>
                      </div>
                    )}
                  </div>
                </div>
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                  ...
                  <span style={{ fontSize: 12, color: '#8b7ac8', fontWeight: 600 }}>
                    {Math.round(progress)}%
                  </span>
                </div>
                ...
                >
                  <div>
                    <div style={{ fontSize: 10, color: '#64748b' }}>Steps Done</div>
                    <div style={{ fontSize: 14, color: '#22c55e', fontWeight: 700 }}>
                      {visibleSteps.filter((s) => s.status === 'success').length}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: '#64748b' }}>Warnings</div>
                    <div style={{ fontSize: 14, color: '#d4a054', fontWeight: 700 }}>
                      {visibleSteps.filter((s) => s.status === 'warning').length}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: '#64748b' }}>Tokens</div>
                    <div style={{ fontSize: 14, color: '#8b7ac8', fontWeight: 700 }}>
                      {visibleSteps.reduce((s, t) => s + (t.tokens ?? 0), 0).toLocaleString()}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: '#64748b' }}>Cost</div>
                    <div style={{ fontSize: 14, color: '#94a3b8', fontWeight: 700 }}>
                      ${visibleSteps.reduce((s, t) => s + (t.costUsd ?? 0), 0).toFixed(4)}
                    </div>
                  </div>
                </div>
              </div>
