...
import { WORKCELL_MAP, TOOL_MAP, type TraceStep, type MirrorEvalScore } from '@szl/a11oy-runtime';
...
function MirrorEvalDisplay({ eval: ev }: { eval: MirrorEvalScore }) {
  ...
  return (
    <div
      style={{ background: '#0a0f1a', borderRadius: 10, border: '1px solid #1e293b', padding: 16 }}
    >
      <div
        style={{
          display: 'flex',
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ fontSize: 20, fontWeight: 800, color: '#8b7ac8' }}>
            {Math.round(ev.overallScore * 100)}%
          </div>
          <div
            style={{
              background: `${dispositionColor[ev.disposition]}18`,
              border: `1px solid ${dispositionColor[ev.disposition]}38`,
              borderRadius: 20,
              padding: '3px 10px',
            }}
          >
            <span
              style={{ fontSize: 10, color: dispositionColor[ev.disposition], fontWeight: 600 }}
            >
          ...
          <div key={d.label}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
              ...
              <span
                style={{
                  fontSize: 10,
                  color: d.value >= 0.85 ? '#22c55e' : d.value >= 0.7 ? '#d4a054' : '#ef4444',
                }}
              >
            ...
            <div style={{ height: 3, background: '#1e293b', borderRadius: 2 }}>
              <div
                style={{
                  height: '100%',
                  width: `${d.value * 100}%`,
                  background: d.value >= 0.85 ? '#22c55e' : d.value >= 0.7 ? '#d4a054' : '#ef4444',
                  borderRadius: 2,
                }}
              />
            </div>
...
function TraceStepRow({ step }: { step: TraceStep }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 12, padding: '10px 0', borderBottom: '1px solid #0f172a' }}>
      ...
      <div style={{ flex: 1, paddingBottom: 4 }}>
        ...
        {step.tokens && (
          <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
            ...
            {step.costUsd && (
              <span style={{ fontSize: 10, color: '#475569' }}>${step.costUsd.toFixed(4)}</span>
            )}
            <span style={{ fontSize: 10, color: '#475569' }}>
      ...
      <div style={{ fontSize: 10, color: color, fontWeight: 600, paddingTop: 4 }}>
        {step.status}
      </div>
    </div>
  );
...
export function WorkcellDetailPage() {
  ...
  const statusColor: Record<string, string> = {
    ...
    approved: '#22c55e',
    planning: '#8b7ac8',
    action_brief_created: '#8b7ac8',
    risk_review: '#d4a054',
    verifying: '#8b7ac8',
    rejected: '#ef4444',
    intake: '#64748b',
    context_building: '#4d8fcc',
    archived: '#475569',
  };
  ...
  return (
    ...
    >
      ...
      >
        <button
          onClick={() => navigate('/agents/workcells')}
          style={{
            background: 'none',
            border: 'none',
            cursor: 'pointer',
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          <div
            style={{
              background: `${statusColor[wc.status] ?? '#64748b'}12`,
              border: `1px solid ${statusColor[wc.status] ?? '#64748b'}28`,
              borderRadius: 20,
              padding: '4px 12px',
            }}
          >
            <span
              style={{ fontSize: 11, color: statusColor[wc.status] ?? '#64748b', fontWeight: 600 }}
            >
      ...
      <div
        style={{ display: 'grid', gridTemplateColumns: '1fr 380px', height: 'calc(100vh - 65px)' }}
      >
        {/* Main Content */}
        <div style={{ overflow: 'auto', padding: '20px 24px', borderRight: '1px solid #1e293b' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {wc.signals.map((s) => (
                <div
                  ...
                  style={{
                    background: '#0f172a',
                    border: `1px solid ${s.severity === 'critical' ? 'rgba(239,68,68,0.2)' : s.severity === 'warning' ? 'rgba(212,160,84,0.2)' : '#1e293b'}`,
                    borderRadius: 8,
                    padding: '10px 14px',
                    display: 'flex',
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div
                      style={{ fontSize: 12, fontWeight: 600, color: '#e2e8f0', marginBottom: 2 }}
                    >
                      {s.label}
                    </div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            >
              <div style={{ display: 'flex', gap: 16, marginBottom: 12 }}>
                <div>
                  <div style={{ fontSize: 10, color: '#64748b' }}>Mode</div>
                  <div style={{ fontSize: 12, color: '#8b7ac8', fontWeight: 600 }}>
                    {wc.contextPack.mode}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: '#64748b' }}>Citations</div>
                  <div style={{ fontSize: 12, color: '#94a3b8' }}>
                    {wc.contextPack.citations.length}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: '#64748b' }}>Tokens Used</div>
                  <div style={{ fontSize: 12, color: '#94a3b8' }}>
                    {wc.contextPack.tokensUsed.toLocaleString()} /{' '}
                    {wc.contextPack.tokenBudget.toLocaleString()}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: '#64748b' }}>Stale</div>
                  <div
                    style={{
                      fontSize: 12,
                      color: wc.contextPack.staleCount > 0 ? '#d4a054' : '#22c55e',
                    }}
                  >
                    {wc.contextPack.staleCount}
                  </div>
                </div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {wc.contextPack.citations.map((c, i) => (
                  <div
                    ...
                    style={{
                      background: '#080c14',
                      borderRadius: 6,
                      border: `1px solid ${c.isStale ? 'rgba(212,160,84,0.2)' : '#0f172a'}`,
                      padding: '8px 12px',
                    }}
                  >
                    <div
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        <span style={{ fontSize: 10, color: '#475569' }}>
                          fresh {Math.round(c.freshnessScore * 100)}%
                        </span>
                      </div>
                    </div>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              >
                ...
                <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 12 }}>
                  {wc.actionBrief.objective}
                </div>

                <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
                  <div>
                    <div style={{ fontSize: 10, color: '#64748b' }}>Risk Category</div>
                    <div style={{ fontSize: 11, color: '#d4a054', fontWeight: 600 }}>
                      {wc.actionBrief.riskCategory.replace(/_/g, ' ')}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: '#64748b' }}>Approver Role</div>
                    <div style={{ fontSize: 11, color: '#94a3b8' }}>
                      {wc.actionBrief.requiredApproverRole}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: '#64748b' }}>Impact</div>
                    <div style={{ fontSize: 11, color: '#94a3b8' }}>
                      {wc.actionBrief.estimatedImpact.slice(0, 50)}…
                    </div>
                  </div>
                </div>
          ...
          {wc.executionResult && (
            <div style={{ marginBottom: 20 }}>
              ...
              <div
                style={{
                  background: wc.executionResult.success
                    ? 'rgba(34,197,94,0.05)'
                    : 'rgba(239,68,68,0.05)',
                  border: `1px solid ${wc.executionResult.success ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)'}`,
                  borderRadius: 8,
                  padding: 14,
                }}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                  <CheckCircle2
                    size={14}
                    color={wc.executionResult.success ? '#22c55e' : '#ef4444'}
                  />
                  <span
                    style={{
                      fontSize: 12,
          ...
          {wc.failureReason && (
            ...
            >
              ...
              {wc.retryRecommendation && (
                <div style={{ fontSize: 11, color: '#94a3b8' }}>
                  <span style={{ color: '#d4a054' }}>Retry: </span>
                  {wc.retryRecommendation}
                </div>
              )}
            </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
              {wc.operatorSequence.map((op, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  {i < wc.operatorSequence.length - 1 && (
                    <ChevronRight size={10} color="#334155" style={{ margin: '0 2px' }} />
                  )}
                </div>
              ))}
            </div>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                <span style={{ fontSize: 10, color: '#475569' }}>
                  {wc.executionTrace.totalTokens.toLocaleString()} tokens
                </span>
                <span style={{ fontSize: 10, color: '#475569' }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              >
                ...
                <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 10 }}>
                  {wc.proofPacket.executionSummary}
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  <div>
                    <div style={{ fontSize: 10, color: '#64748b' }}>Evidence Coverage</div>
                    <div style={{ fontSize: 12, color: '#22c55e', fontWeight: 600 }}>
                      {Math.round(wc.proofPacket.evidenceCoverage * 100)}%
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: '#64748b' }}>Agent Calls</div>
                    <div style={{ fontSize: 12, color: '#94a3b8' }}>
                      {wc.proofPacket.agentTrace.length}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: '#64748b' }}>Tool Calls</div>
                    <div style={{ fontSize: 12, color: '#94a3b8' }}>
                      {wc.proofPacket.toolCalls.length}
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: '#64748b' }}>Approvals</div>
                    <div style={{ fontSize: 12, color: '#94a3b8' }}>
                      {wc.proofPacket.approvalChain.length}
                    </div>
                  </div>
                </div>
                <div
