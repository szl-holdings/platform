    ...
    <div style={{ background: 'var(--gi-bg-base)', minHeight: '100vh', color: 'var(--gi-text-primary)', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ borderBottom: '1px solid var(--gi-border-subtle)', padding: '20px 32px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--gi-text-muted)' }}>9 A11oy skills — domain-specific operator instructions · eval criteria · policy bindings</div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', height: 'calc(100vh - 73px)' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '16px 24px' }}>
          <div style={{ display: 'grid', gap: 10 }}>
            {SKILLS.map((skill) => {
              const domainColor = DOMAIN_COLORS[skill.domain] ?? '#4a6070';
              ...
              return (
                <div
                  ...
                  style={{
                    background: isSelected ? 'rgba(236,72,153,0.05)' : 'var(--gi-bg-base)',
                    border: `1px solid ${isSelected ? 'rgba(236,72,153,0.25)' : 'var(--gi-border-subtle)'}`,
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, flexShrink: 0, marginLeft: 12 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        <Clock size={10} color="var(--gi-text-muted)" />
                        <span style={{ fontSize: 10, color: 'var(--gi-text-muted)' }}>{(skill.estimatedDurationMs / 1000).toFixed(0)}s</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                      <span style={{ fontSize: 10, color: 'var(--gi-text-muted)' }}>Tools:</span>
                      ...
                      {skill.allowedTools.length > 4 && <span style={{ fontSize: 9, color: 'var(--gi-text-muted)' }}>+{skill.allowedTools.length - 4}</span>}
                    </div>
                    <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                      <span style={{ fontSize: 10, color: 'var(--gi-text-muted)' }}>Blocked:</span>
        ...
        {/* Detail Panel */}
        {selectedSkill && (
          <div style={{ width: 400, borderLeft: '1px solid var(--gi-border-subtle)', overflow: 'auto', padding: 20, background: 'var(--gi-bg-base)' }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: '#f1f5f9', marginBottom: 4 }}>{selectedSkill.name}</div>
            <div style={{ fontSize: 11, color: 'var(--gi-text-muted)', marginBottom: 16 }}>{selectedSkill.objective}</div>

            {/* Required Inputs */}
            <div style={{ background: 'var(--gi-bg-base)', borderRadius: 8, border: '1px solid var(--gi-border-subtle)', padding: 14, marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: 'var(--gi-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>Required Inputs</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {selectedSkill.requiredInputs.map((inp) => (
                  <span key={inp} style={{ fontSize: 10, color: 'var(--gi-accent-blue)', background: 'rgba(77,143,204,0.08)', border: '1px solid rgba(77,143,204,0.2)', borderRadius: 4, padding: '2px 8px' }}>{inp}</span>
                ))}
              </div>
            </div>
            ...
            {/* Policies */}
            <div style={{ background: 'var(--gi-bg-base)', borderRadius: 8, border: '1px solid var(--gi-border-subtle)', padding: 14, marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: 'var(--gi-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>Policy Bindings</div>
            ...
            {/* Eval Criteria */}
            <div style={{ background: 'var(--gi-bg-base)', borderRadius: 8, border: '1px solid var(--gi-border-subtle)', padding: 14, marginBottom: 12 }}>
              <div style={{ fontSize: 11, color: 'var(--gi-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>MirrorEval Criteria</div>
            ...
            <div style={{ background: 'var(--gi-bg-base)', borderRadius: 8, border: '1px solid var(--gi-border-subtle)', padding: 14 }}>
              <div style={{ fontSize: 11, color: 'var(--gi-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>Expected Outputs</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
