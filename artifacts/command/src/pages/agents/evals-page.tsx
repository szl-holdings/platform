...
import { DEMO_WORKCELLS, type MirrorEvalScore } from '@szl/a11oy-runtime';
...
export function EvalsPage() {
  ...
  const evalRows: EvalRow[] = DEMO_WORKCELLS.filter((w) => w.actionBrief).map((w) => ({
    workcellId: w.id,
    title: w.title,
    domain: w.domain,
  ...
  const avgScore =
    evalRows.length > 0
      ? evalRows.reduce((s, r) => s + r.eval.overallScore, 0) / evalRows.length
      : 0;
  ...
  const blockCount = evalRows.filter(
    (r) => r.eval.disposition === 'blocked' || r.eval.disposition === 'requires_human_review',
  ).length;
      ...
      >
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: '#64748b' }}>
            10-dimension evaluation across {evalRows.length} Action Briefs
          </div>
        </div>
      </div>
        ...
        {[
          {
            ...
            color: avgScore >= 0.85 ? '#22c55e' : '#d4a054',
            icon: BarChart3,
          },
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ fontSize: 18, fontWeight: 800, color: '#8b7ac8' }}>
                        {Math.round(row.eval.overallScore * 100)}%
                      </div>
                      <div
                        style={{
                      ...
                      >
                        <span style={{ fontSize: 10, color: dColor, fontWeight: 600 }}>
                          {row.eval.disposition.replace(/_/g, ' ')}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  {row.eval.warnings.length > 0 && (
                    <div
                      style={{ marginTop: 8, display: 'flex', alignItems: 'flex-start', gap: 5 }}
                    >
                      <AlertTriangle
                        size={10}
                        color="#d4a054"
                      ...
                      />
                      <div style={{ fontSize: 10, color: '#d4a054' }}>
                        {row.eval.warnings.join(' · ')}
                      </div>
                    </div>
                  )}
                </div>
          ...
          >
            ...
            <div style={{ display: 'grid', gap: 7 }}>
              {[
                ...
                {
                  label: 'Unsafe Autonomy Risk',
                  value: 1 - selected.eval.unsafeAutonomyRisk,
                  inverted: true,
                },
                {
                  label: 'Hallucination Risk',
                  value: 1 - selected.eval.hallucinationRisk,
                  inverted: true,
                },
                ...
                {
                  label: 'Stale Context Risk',
                  value: 1 - selected.eval.staleContextRisk,
                  inverted: true,
                },
            ...
            >
              <div style={{ fontSize: 11, color: '#64748b', marginBottom: 6 }}>Run At</div>
              <div style={{ fontSize: 11, color: '#94a3b8' }}>
                {new Date(selected.eval.runAt).toLocaleString()}
              </div>
            </div>
          </div>
        )}
