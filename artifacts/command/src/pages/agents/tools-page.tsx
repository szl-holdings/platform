...
import { TOOLS } from '@szl/a11oy-runtime';
import type { ToolRisk } from '@szl/a11oy-runtime';
  ...
  return (
    <div style={{ background: '#080c14', minHeight: '100vh', color: '#e2e8f0', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: 8, padding: '12px 32px', borderBottom: '1px solid #1e293b', overflowX: 'auto' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 5, padding: '4px 10px', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 20 }}>
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8 }}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: isBlocked ? '#ef4444' : '#f1f5f9' }}>{tool.name}</div>
                      <div style={{ fontSize: 10, color: '#475569', marginTop: 2 }}>{tool.id}</div>
                    </div>
                    <div style={{ display: 'flex', gap: 4 }}>
                      {isBlocked && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 3, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 4, padding: '2px 7px' }}>
                          <Lock size={9} color="#ef4444" />
                          <span style={{ fontSize: 9, color: '#ef4444', fontWeight: 700 }}>BLOCKED</span>
                        </div>
                      )}
                      <div style={{ background: `${riskCfg.color}12`, border: `1px solid ${riskCfg.color}28`, borderRadius: 4, padding: '2px 8px' }}>
                        <span style={{ fontSize: 10, color: riskCfg.color }}>{riskCfg.label}</span>
                      </div>
                    </div>
                  </div>

                  <div style={{ fontSize: 11, color: '#94a3b8', lineHeight: 1.5, marginBottom: 10 }}>{tool.description.slice(0, 90)}…</div>

                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      <span style={{ fontSize: 10, color: catColor, background: `${catColor}10`, border: `1px solid ${catColor}25`, borderRadius: 10, padding: '2px 8px' }}>{tool.category}</span>
              ...
              {selectedTool.allowedOperators.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                  {selectedTool.allowedOperators.map((o) => (
                    <span key={o} style={{ fontSize: 10, background: 'rgba(139,122,200,0.08)', border: '1px solid rgba(139,122,200,0.2)', color: '#8b7ac8', borderRadius: 4, padding: '2px 8px' }}>{o}</span>
                  ))}
                </div>
              )}
            </div>
