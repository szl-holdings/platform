...
const SIM_VARIABLES: SimVariable[] = [
  { id: "oil-price", label: "Oil Price Change", unit: "%", min: -60, max: 100, step: 5, default: 0, description: "WTI crude shock (%)", icon: "🛢" },
  { id: "interest-rate", label: "Interest Rate Change", unit: "bps", min: -200, max: 300, step: 25, default: 0, description: "Central bank policy shift (basis points)", icon: "🏦" },
  { id: "threat-level", label: "Global Threat Level", unit: "", min: 1, max: 5, step: 1, default: 2, description: "Geopolitical threat multiplier (1–5)", icon: "⚠" },
  { id: "market-conditions", label: "Market Conditions", unit: "%", min: -40, max: 40, step: 5, default: 0, description: "Equity market index movement (%)", icon: "📈" },
  { id: "fx-usd", label: "USD Strength", unit: "%", min: -20, max: 20, step: 2, default: 0, description: "USD index change vs DXY (%)", icon: "💵" },
];
...
function computeImpact(vars: Record<string, number>): DomainImpact[] {
  ...
  return [
    ...
    },
    {
      ...
      impactValue: rate !== 0 || market !== 0 ? `${rate < 0 || market > 0 ? "+" : ""}${((-rate * 2.1) + (market * 0.14)).toFixed(1)}%` : "Neutral",
      ...
      affectedEntities: rate !== 0 ? ["Miami Beach Commercial", "Austin Industrial", "NYC Mixed-Use"] : [],
    },
    {
      domain: "SZL Holdings",
      icon: "◆",
    ...
    },
    {
      ...
      impactValue: oil > 10 ? "7 clauses triggered" : threat > 3 ? "4 force majeure alerts" : "Minimal exposure",
      ...
      affectedEntities: oil > 10 ? ["MV Poseidon Charter", "Q3 Cargo Agreement", "Force Majeure Review"] : [],
    },
    {
      domain: "Carlota Jo",
      icon: "◈",
      ...
      impactValue: market !== 0 ? `${Math.abs(market) > 15 ? "3" : "1"} alert${Math.abs(market) > 15 ? "s" : ""}` : "Stable",
      trend: Math.abs(market) > 15 ? "down" : "flat",
      confidence: 0.71 + Math.random() * 0.1,
      details: Math.abs(market) > 15
      ...
      affectedEntities: Math.abs(market) > 15 ? ["Emerging Markets Client", "PE Strategy Engagement"] : [],
    },
    {
      domain: "Aegis",
      icon: "🛡",
      ...
      impactValue: threat > 3 ? `+${((threat - 2) * 18).toFixed(0)}% attack probability` : threat < 2 ? "Reduced" : "Stable",
      ...
      affectedEntities: threat > 3 ? ["Critical Infrastructure", "Maritime OT Systems", "Legal Case Files"] : [],
    },
  ];
}
...
export default function SimulationPage() {
  ...
  const loadScenario = useCallback((scenario: (typeof savedScenarios)[0]) => {
    setValues((prev) => ({ ...prev, ...(scenario.variables as unknown as Record<string, number>) }));
    setHasRun(false);
  }, []);
      ...
      <main style={{ maxWidth: "1400px", margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ marginBottom: "32px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "8px" }}>
            ...
            <div>
              <h1 style={{ fontSize: "22px", fontWeight: 700, margin: 0, color: "rgba(255,255,255,0.95)" }}>What-If Simulation Engine</h1>
              <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)", margin: 0, marginTop: "2px" }}>
                Monte Carlo scenario modeling across all domains · {iterations.toLocaleString()} iterations per run
              </p>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "360px 1fr", gap: "24px", alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "14px", overflow: "hidden" }}>
              <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span style={{ fontSize: "11px", fontWeight: 700, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: "1px" }}>Saved Scenarios</span>
              </div>
              <div style={{ padding: "8px" }}>
                {savedScenarios.map((s) => (
                  <button
                    ...
                    onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = `${ACCENT}12`; (e.currentTarget as HTMLButtonElement).style.borderColor = `${ACCENT}30`; }}
                  ...
                  >
                    <span>{s.label}</span>
                    <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.3)" }}>{s.date}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "14px", overflow: "hidden" }}>
              <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                <span style={{ fontSize: "11px", fontWeight: 700, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: "1px" }}>Scenario Variables</span>
              </div>
                    ...
                    <div key={v.id}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "7px" }}>
                          ...
                          <span style={{ fontSize: "12px", fontWeight: 600, color: changed ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.6)" }}>{v.label}</span>
                        </div>
                        <span style={{ fontSize: "13px", fontWeight: 700, color: changed ? ACCENT : "rgba(255,255,255,0.4)", minWidth: "60px", textAlign: "right" }}>
                          {val > 0 ? "+" : ""}{val}{v.unit}
                        </span>
                      </div>
                      <input
                        ...
                        step={v.step}
                        value={val}
                        onChange={(e) => { setValues((prev) => ({ ...prev, [v.id]: Number(e.target.value) })); setHasRun(false); }}
                        style={{ width: "100%", accentColor: ACCENT, cursor: "pointer" }}
                      />
                      <div style={{ display: "flex", justifyContent: "space-between", marginTop: "3px" }}>
                        <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.25)" }}>{v.min}{v.unit}</span>
                        <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.25)" }}>{v.description}</span>
                        <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.25)" }}>{v.max}{v.unit}</span>
                      </div>
                    </div>
                  );
                })}
              ...
              <div style={{ padding: "12px 16px", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", gap: "8px" }}>
                ...
                >
                  {running ? (
                    <>
                      <div style={{ width: "12px", height: "12px", border: `2px solid rgba(255,255,255,0.3)`, borderTop: "2px solid #fff", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
                      Running Monte Carlo...
                    </>
                  ) : "▶ Run Simulation"}
                </button>
                {hasChanges && (
                  <button
                    onClick={resetAll}
                    style={{ padding: "10px 14px", borderRadius: "10px", border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: "rgba(255,255,255,0.4)", fontSize: "12px", cursor: "pointer", fontFamily: "system-ui, sans-serif" }}
                  >
                    Reset
                  </button>
                )}
          ...
          <div>
            {!hasRun && !running && (
              <div style={{ height: "400px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "rgba(255,255,255,0.02)", border: "1px dashed rgba(255,255,255,0.1)", borderRadius: "16px", gap: "12px" }}>
                <div style={{ fontSize: "40px", opacity: 0.3 }}>🌊</div>
                <div style={{ fontSize: "14px", color: "rgba(255,255,255,0.3)", fontWeight: 500 }}>Adjust variables and run the simulation</div>
                <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.2)" }}>Cross-domain impact will appear here</div>
              </div>
            )}

            {running && (
              <div style={{ height: "400px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "16px" }}>
                <div style={{ width: "40px", height: "40px", border: `3px solid ${ACCENT}30`, borderTop: `3px solid ${ACCENT}`, borderRadius: "50%", animation: "spin 0.9s linear infinite" }} />
                <div style={{ fontSize: "14px", color: "rgba(255,255,255,0.5)" }}>Running {iterations.toLocaleString()} Monte Carlo iterations...</div>
                <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.3)" }}>Tracing entity relationships across all domains</div>
              </div>
            )}
                ...
                {impacts.map((impact) => (
                  ...
                  >
                    <div style={{ padding: "14px 18px", display: "flex", alignItems: "flex-start", gap: "14px" }}>
                      <div style={{ width: "36px", height: "36px", borderRadius: "10px", background: `${impact.color}15`, border: `1px solid ${impact.color}30`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px", flexShrink: 0 }}>
                        {impact.icon}
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                          <span style={{ fontSize: "13px", fontWeight: 700, color: "rgba(255,255,255,0.9)" }}>{impact.domain}</span>
                          <span style={{ fontSize: "10px", color: impact.color, background: `${impact.color}15`, border: `1px solid ${impact.color}30`, borderRadius: "5px", padding: "2px 7px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.5px" }}>
                            {impact.impactLabel}
                          </span>
                        </div>
                        <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.5)", lineHeight: 1.6 }}>{impact.details}</div>
                        {impact.affectedEntities.length > 0 && (
                          <div style={{ display: "flex", gap: "5px", marginTop: "8px", flexWrap: "wrap" }}>
                            {impact.affectedEntities.map((e) => (
                              <span key={e} style={{ fontSize: "10px", color: "rgba(255,255,255,0.4)", background: "rgba(255,255,255,0.05)", borderRadius: "4px", padding: "2px 7px", border: "1px solid rgba(255,255,255,0.08)" }}>
                                {e}
                              </span>
                            ))}
                          </div>
                      ...
                      <div style={{ textAlign: "right", flexShrink: 0 }}>
                        <div style={{ fontSize: "18px", fontWeight: 800, color: impact.trend === "up" ? "#22c55e" : impact.trend === "down" ? "#ef4444" : "rgba(255,255,255,0.4)" }}>
                          {impact.impactValue}
                        </div>
                        ...
                        {/* Confidence bar */}
                        <div style={{ marginTop: "6px", width: "80px", height: "3px", background: "rgba(255,255,255,0.08)", borderRadius: "2px" }}>
                          <div style={{ height: "100%", width: `${impact.confidence * 100}%`, background: impact.color, borderRadius: "2px" }} />
                        </div>
                      </div>
                    </div>
                  </div>
