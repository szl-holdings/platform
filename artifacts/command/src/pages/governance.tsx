...
const FALLBACK_POLICIES: Policy[] = [
  {
    ...
    description:
      'Defines mandatory data retention periods by classification, automated disposal schedules, and legal hold procedures for all SZL ecosystem applications.',
    enforcement: 'auto',
    approvalChain: [
      {
        role: 'Legal Counsel',
        ...
        status: 'approved',
        date: 'Dec 28',
        comment: 'Compliant with GDPR & CCPA requirements.',
      },
  ...
  {
    id: 'p2',
    title: 'Cross-Domain Access Control Framework',
    category: 'access',
    status: 'active',
    domains: ['PARAGON', 'SEXTANT', 'DOMAINE', 'KORA', 'PRAXIS'],
    version: 'v1.4',
    owner: 'James Okafor',
    lastUpdated: 'Apr 8',
    effectiveDate: 'Mar 15, 2026',
    description:
      'Defines role-based access control (RBAC) requirements, privilege escalation procedures, and audit requirements for cross-domain data access.',
    enforcement: 'auto',
    approvalChain: [
  ...
  {
    id: 'p3',
    title: 'AI Model Governance & Output Validation',
    category: 'compliance',
    status: 'pending-approval',
    domains: ['Command', 'PARAGON', 'PRAXIS', 'DOMAINE'],
    version: 'v1.0-draft',
    owner: 'Stephen Lutar',
    lastUpdated: 'Apr 14',
    effectiveDate: 'TBD',
    ...
    approvalChain: [
      {
        ...
        status: 'approved',
        date: 'Apr 13',
        comment: 'Legal framework sound. Recommend adding liability clauses.',
      },
  ...
  {
    id: 'p4',
    title: 'Maritime Cybersecurity Incident Response',
    category: 'security',
    status: 'active',
    ...
    description:
      'Prescribes response procedures for OT/IT cyber incidents affecting vessel systems, including isolation protocols, crew notification, and port authority reporting obligations.',
    enforcement: 'auto',
    approvalChain: [
    ...
    auditLog: [
      {
        date: 'Apr 5',
        action: 'Updated port authority contacts — Singapore MPA',
        actor: 'Marcus Chen',
      },
  ...
  {
    id: 'p5',
    title: 'Real Estate Deal Approval Thresholds',
    ...
    auditLog: [{ date: 'Apr 15', action: 'Draft policy created', actor: 'Sofia Reyes' }],
  },
];
...
export default function GovernancePage() {
  ...
  const allAuditEvents = POLICIES.flatMap((p) =>
    p.auditLog.map((e) => ({ ...e, policy: p.title, domain: p.domains[0] })),
      ...
      <div className="flex flex-col gap-6">
        ...
        {pendingPolicies.length > 0 && (
          <div
            ...
            style={{
              backgroundColor: 'color-mix(in srgb, var(--color-medium) 8%, transparent)',
              border: '1px solid color-mix(in srgb, var(--color-medium) 25%, transparent)',
            }}
          >
            ...
            <div className="flex-1">
              <span className="text-sm font-semibold" style={{ color: 'var(--color-fg-primary)' }}>
                {pendingPolicies.length} polic
                {pendingPolicies.length > 1 ? 'ies require' : 'y requires'} your review and approval
              </span>
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {(
                    ['data', 'access', 'compliance', 'security', 'operational'] as PolicyCategory[]
                  ).map((c) => (
              ...
              {filtered.map((policy) => {
                ...
                const pendingSteps = policy.approvalChain.filter(
                  (s) => s.status === 'pending',
                ).length;
                return (
                  <div
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <span
                            className="text-[9px] font-bold uppercase tracking-widest px-1.5 py-0.5 rounded capitalize"
                            style={{
                              backgroundColor: `color-mix(in srgb, ${CATEGORY_COLORS[policy.category]} 12%, transparent)`,
                      ...
                      <span
                        className="text-[9px] px-2 py-0.5 rounded-full font-bold capitalize shrink-0"
                        style={{
                          backgroundColor: `color-mix(in srgb, ${STATUS_COLORS[policy.status]} 12%, transparent)`,
                    ...
                    >
                      ...
                      {pendingSteps > 0 && (
                        ...
                        >
                          {pendingSteps} approval{pendingSteps > 1 ? 's' : ''} pending
                        </span>
                      )}
                    </div>
                  </div>
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span
                            className="text-[9px] font-bold uppercase tracking-widest px-1.5 py-0.5 rounded capitalize"
                            style={{
                              backgroundColor: `color-mix(in srgb, ${CATEGORY_COLORS[selectedPolicy.category]} 12%, transparent)`,
                              color: CATEGORY_COLORS[selectedPolicy.category],
                            }}
                          >
                          ...
                          <span
                            ...
                            style={{
                              backgroundColor: `color-mix(in srgb, ${STATUS_COLORS[selectedPolicy.status]} 12%, transparent)`,
                              color: STATUS_COLORS[selectedPolicy.status],
                            }}
                          >
                    ...
                    >
                      {[
                        ...
                        { label: 'Effective', value: selectedPolicy.effectiveDate },
                        {
                          ...
                          color:
                            selectedPolicy.enforcement === 'auto'
                              ? 'var(--color-low)'
                              : 'var(--color-medium)',
                        },
                        ...
                        <div key={label}>
                          ...
                          <div
                            ...
                            style={{ color: color ?? 'var(--color-fg-secondary)' }}
                          >
                            {value}
                          </div>
                        </div>
                          ...
                          <div key={i} className="flex gap-3">
                            <div className="flex flex-col items-center">
                              <div
                                ...
                                style={{
                                  backgroundColor: `color-mix(in srgb, ${colors[step.status]} 15%, transparent)`,
                            ...
                            <div className="flex-1 pb-3">
                              ...
                              {step.status === 'pending' &&
                                selectedPolicy.approvalChain[i - 1]?.status === 'approved' && (
                                  <div className="flex gap-2 mt-2">
                                    <button
                                      className="px-3 py-1.5 rounded-lg text-xs font-bold"
                                      style={{ backgroundColor: 'var(--color-low)', color: '#fff' }}
                                    >
                                      Approve
                                    </button>
                                    <button
        ...
        {tab === 'audit' && (
          ...
          >
            <div
              className="text-[10px] font-mono uppercase tracking-widest px-5 py-3"
              style={{
                color: 'var(--color-fg-muted)',
            ...
            {allAuditEvents.map((event, i) => (
              ...
              >
                ...
                <span className="text-xs shrink-0" style={{ color: '#8b7ac8' }}>
                  {event.actor}
                </span>
              </div>
            ))}
