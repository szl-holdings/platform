...
import { ACCENT, apiUrl, fetchJson, BASE_URL } from "./cognitive/shared";
...
interface EvalForgeMetrics {
  correctness: { passRate: number; avgScore: number; passed: number; failed: number; total: number };
  evidenceQuality: { citationCoverage: number; citationAccuracy: number; sourceVerified: number; totalCitations: number; score: number };
  confidenceCalibration: { avgConfidence: number; calibrationError: number; overconfidenceRate: number; underconfidenceRate: number; brierScore: number; score: number };
  latency: { avgLatencyMs: number; p50LatencyMs: number; p95LatencyMs: number; p99LatencyMs: number; maxLatencyMs: number };
  cost: { totalCostUsd: number; avgCostUsd: number; costPerOutcome: number; totalTokensUsed: number; avgTokensUsed: number; p95CostUsd: number };
  interventionValue: { interventions: number; totalDecisions: number; interventionRate: number; avgImprovementFromIntervention: number; estimatedValueSaved: number };
  humanOverrideRate: { overrides: number; totalDecisions: number; overrideRate: number; acceptedRate: number; overrideReasons: Record<string, number> };
  rollbackRate: { rollbacks: number; totalActions: number; rollbackRate: number; rollbackReasons: Record<string, number>; avgRollbackLatencyMs: number };
  policyViolations: { totalChecks: number; violations: number; violationRate: number; criticalViolations: number; violationsByType: Record<string, number>; complianceRate: number };
}
...
function RegressionBadge({ severity, hasRegression }: { severity?: string; hasRegression?: boolean }) {
...
function EvalForgeDashboard() {
  ...
  const handleRun = useCallback(
    async (suite: EvalSuite) => {
      ...
      try {
        ...
        const result = await fetchJson<EvalRunSummary & { regressionSeverity?: EvalRunSummary["regressionSeverity"] }>(
          apiUrl("/evals/run"),
          {
            method: "POST",
            headers: {
  ...
  const avgPassRate = runs.length > 0 ? runs.reduce((sum, r) => sum + (r.passRate ?? 0), 0) / runs.length : 0;
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "24px 20px" }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>
            Browse Eval Forge suites by type, trigger runs, and track regressions across all nine metric categories.
          </p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 24 }}>
          {[
            ...
            { label: "Avg Pass Rate", value: runs.length ? `${(avgPassRate * 100).toFixed(1)}%` : "—", color: "#22c55e" },
          ...
          <div>
            ...
            {domains.length > 0 && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 18 }}>
                <FilterPill active={domainFilter === "all"} onClick={() => setDomainFilter("all")} label="All Domains" subtle />
                {domains.map((d) => (
                  <FilterPill
                    key={d}
            ...
            {!loading && visibleSuites.length === 0 && (
              <div style={{ ...cardStyle, padding: 32, textAlign: "center", color: "#64748b", fontSize: 13 }}>
                ...
                return (
                  <div key={type} style={{ marginBottom: 28 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                      <span style={{ fontSize: 13, fontWeight: 700, color: "#e2e8f0" }}>{EVAL_TYPE_LABEL[type]}</span>
                      <span style={{ fontSize: 11, color: "#64748b" }}>{ofType.length} suite{ofType.length === 1 ? "" : "s"}</span>
                    </div>
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                                  <span style={{ fontSize: 13, fontWeight: 600, color: "#e2e8f0" }}>{suite.name}</span>
                                  <span
                                    style={{
                                      fontSize: 10,
                                      color: "#0ea5e9",
                                  ...
                                  <span style={{ fontSize: 10, color: "#64748b" }}>v{suite.version}</span>
                                </div>
                                ...
                                <div style={{ display: "flex", gap: 12, fontSize: 11, color: "#64748b", flexWrap: "wrap" }}>
                                  ...
                                  {suite.redTeamCount > 0 && <span style={{ color: "#ef4444" }}>⚔ {suite.redTeamCount} red-team</span>}
                                  ...
                                  {lastRun && (
                                    <>
                                      ...
                                      {lastRun.hasRegression && <RegressionBadge hasRegression severity={lastRun.regressionSeverity} />}
                                    </>
                                  )}
                                </div>
                              </div>
                              <div style={{ display: "flex", flexDirection: "column", gap: 6, flexShrink: 0 }}>
                                <button
                                  ...
                                  style={{
                                    background: runningSuiteId === suite.suiteId ? `${ACCENT}40` : ACCENT,
                                    border: "none",
                                    borderRadius: 6,
                                    padding: "8px 14px",
                                    color: "#fff",
                ...
                {runs.slice(0, 30).map((r) => (
                  ...
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                      <span style={{ fontSize: 12, fontWeight: 600 }}>{r.suiteName ?? r.suiteId}</span>
                      <span style={{ fontSize: 12, fontWeight: 700, color: passRateColor(r.passRate) }}>
...
function MetricCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ ...cardStyle, padding: 16 }}>
      <div style={{ fontSize: 12, fontWeight: 600, color: "#e2e8f0", marginBottom: 10 }}>{title}</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>{children}</div>
    </div>
  );
}

function MetricRow({ label, value, color }: { label: string; value: React.ReactNode; color?: string }) {
  return (
...
function EvalForgeRunDetail({ runId }: { runId: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        const res = await fetchJson<EvalRunReport>(apiUrl(`/evals/runs/${encodeURIComponent(runId)}`));
        if (!cancelled) {
          setReport(res);
          setError(null);
        }
          ...
          <>
            <div style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8, flexWrap: "wrap" }}>
                <span style={{ fontSize: 22, fontWeight: 700 }}>{report.suiteName ?? report.suiteId}</span>
                <RegressionBadge hasRegression={report.hasRegression} severity={report.regressionSeverity} />
                {report.evalType && (
                ...
                {report.domain && (
                  <span style={{ fontSize: 11, color: "#0ea5e9", background: "#0ea5e915", padding: "2px 8px", borderRadius: 4 }}>
                    {report.domain.toUpperCase()}
                  </span>
                )}
              </div>
              <div style={{ display: "flex", gap: 16, fontSize: 12, color: "#64748b", flexWrap: "wrap" }}>
                <span>Run ID: <code style={{ color: "#94a3b8" }}>{report.runId}</code></span>
            ...
            {(report.regressionNotes?.length || report.improvementNotes?.length) ? (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
                {report.regressionNotes && report.regressionNotes.length > 0 && (
                  <div style={{ background: "#ef444412", border: "1px solid #ef444430", borderRadius: 8, padding: "12px 14px" }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: "#fca5a5", marginBottom: 6 }}>Regressions vs baseline</div>
                ...
                {report.improvementNotes && report.improvementNotes.length > 0 && (
                  <div style={{ background: "#22c55e12", border: "1px solid #22c55e30", borderRadius: 8, padding: "12px 14px" }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: "#86efac", marginBottom: 6 }}>Improvements vs baseline</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 24 }}>
              <div style={{ ...cardStyle, padding: "14px 18px" }}>
                <div style={{ fontSize: 22, fontWeight: 700, color: passRateColor(report.passRate) }}>{fmtPct(report.passRate)}</div>
                <div style={{ fontSize: 11, color: "#64748b", marginTop: 2 }}>Pass Rate</div>
              </div>
              <div style={{ ...cardStyle, padding: "14px 18px" }}>
                <div style={{ fontSize: 22, fontWeight: 700, color: "#e2e8f0" }}>{fmtPct(report.avgScore)}</div>
              ...
              <div style={{ ...cardStyle, padding: "14px 18px" }}>
                <div style={{ fontSize: 22, fontWeight: 700, color: "#e2e8f0" }}>{fmtMs(report.avgLatencyMs ?? 0)}</div>
                <div style={{ fontSize: 11, color: "#64748b", marginTop: 2 }}>Avg Latency</div>
              </div>
              <div style={{ ...cardStyle, padding: "14px 18px" }}>
                <div style={{ fontSize: 22, fontWeight: 700, color: "#e2e8f0" }}>{fmtUsd(report.totalCostUsd ?? 0)}</div>
                <div style={{ fontSize: 11, color: "#64748b", marginTop: 2 }}>Total Cost</div>
              </div>
            </div>

            <div style={{ marginBottom: 24 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#e2e8f0", marginBottom: 12 }}>Metric Categories</div>
              <div
                style={{
                  display: "grid",
              ...
              >
                <MetricCard title="Correctness">
                  <MetricRow label="Pass rate" value={fmtPct(report.metrics.correctness.passRate)} color={passRateColor(report.metrics.correctness.passRate)} />
                  <MetricRow label="Avg score" value={fmtPct(report.metrics.correctness.avgScore)} />
                  <MetricRow label="Passed / total" value={`${report.metrics.correctness.passed} / ${report.metrics.correctness.total}`} />
                  <MetricRow label="Failed" value={report.metrics.correctness.failed} color={report.metrics.correctness.failed > 0 ? "#ef4444" : undefined} />
                </MetricCard>

                <MetricCard title="Evidence Quality">
                  <MetricRow label="Score" value={fmtPct(report.metrics.evidenceQuality.score)} />
                  <MetricRow label="Citation coverage" value={fmtPct(report.metrics.evidenceQuality.citationCoverage)} />
                  <MetricRow label="Citation accuracy" value={fmtPct(report.metrics.evidenceQuality.citationAccuracy)} />
                  <MetricRow label="Sources verified" value={`${report.metrics.evidenceQuality.sourceVerified} / ${report.metrics.evidenceQuality.totalCitations}`} />
                </MetricCard>

                <MetricCard title="Confidence Calibration">
                  <MetricRow label="Score" value={fmtPct(report.metrics.confidenceCalibration.score)} />
                  <MetricRow label="Avg confidence" value={fmtPct(report.metrics.confidenceCalibration.avgConfidence)} />
                  <MetricRow label="Calibration error" value={fmtPct(report.metrics.confidenceCalibration.calibrationError)} />
                  <MetricRow label="Brier score" value={fmtNum(report.metrics.confidenceCalibration.brierScore, 3)} />
                  <MetricRow label="Overconfidence" value={fmtPct(report.metrics.confidenceCalibration.overconfidenceRate)} />
                  <MetricRow label="Underconfidence" value={fmtPct(report.metrics.confidenceCalibration.underconfidenceRate)} />
                </MetricCard>
                ...
                <MetricCard title="Cost">
                  ...
                  <MetricRow label="Cost per outcome" value={fmtUsd(report.metrics.cost.costPerOutcome)} />
                  <MetricRow label="p95 cost" value={fmtUsd(report.metrics.cost.p95CostUsd)} />
                  <MetricRow label="Tokens (total)" value={fmtNum(report.metrics.cost.totalTokensUsed)} />
                  <MetricRow label="Tokens (avg)" value={fmtNum(report.metrics.cost.avgTokensUsed, 1)} />
                </MetricCard>

                <MetricCard title="Intervention Value">
                  <MetricRow label="Interventions" value={`${report.metrics.interventionValue.interventions} / ${report.metrics.interventionValue.totalDecisions}`} />
                  <MetricRow label="Intervention rate" value={fmtPct(report.metrics.interventionValue.interventionRate)} />
                  <MetricRow label="Avg improvement" value={fmtPct(report.metrics.interventionValue.avgImprovementFromIntervention)} />
                  <MetricRow label="Value saved" value={fmtUsd(report.metrics.interventionValue.estimatedValueSaved)} />
                </MetricCard>

                <MetricCard title="Human Override Rate">
                  <MetricRow label="Overrides" value={`${report.metrics.humanOverrideRate.overrides} / ${report.metrics.humanOverrideRate.totalDecisions}`} />
                  <MetricRow label="Override rate" value={fmtPct(report.metrics.humanOverrideRate.overrideRate)} color={report.metrics.humanOverrideRate.overrideRate > 0.1 ? "#f59e0b" : undefined} />
                  <MetricRow label="Accepted rate" value={fmtPct(report.metrics.humanOverrideRate.acceptedRate)} />
                  {Object.entries(report.metrics.humanOverrideRate.overrideReasons).slice(0, 3).map(([k, v]) => (
                    <MetricRow key={k} label={`· ${k}`} value={v} />
                  ))}
                </MetricCard>

                <MetricCard title="Rollback Rate">
                  <MetricRow label="Rollbacks" value={`${report.metrics.rollbackRate.rollbacks} / ${report.metrics.rollbackRate.totalActions}`} />
                  <MetricRow label="Rate" value={fmtPct(report.metrics.rollbackRate.rollbackRate)} color={report.metrics.rollbackRate.rollbackRate > 0.05 ? "#f59e0b" : undefined} />
                  <MetricRow label="Avg rollback latency" value={fmtMs(report.metrics.rollbackRate.avgRollbackLatencyMs)} />
                  {Object.entries(report.metrics.rollbackRate.rollbackReasons).slice(0, 3).map(([k, v]) => (
                    <MetricRow key={k} label={`· ${k}`} value={v} />
                  ))}
                </MetricCard>

                <MetricCard title="Policy Violations">
                  <MetricRow label="Compliance" value={fmtPct(report.metrics.policyViolations.complianceRate)} color={passRateColor(report.metrics.policyViolations.complianceRate)} />
                  <MetricRow label="Violations" value={`${report.metrics.policyViolations.violations} / ${report.metrics.policyViolations.totalChecks}`} color={report.metrics.policyViolations.violations > 0 ? "#ef4444" : undefined} />
                  <MetricRow label="Violation rate" value={fmtPct(report.metrics.policyViolations.violationRate)} />
                  <MetricRow label="Critical" value={report.metrics.policyViolations.criticalViolations} color={report.metrics.policyViolations.criticalViolations > 0 ? "#ef4444" : undefined} />
                  {Object.entries(report.metrics.policyViolations.violationsByType).slice(0, 3).map(([k, v]) => (
                    <MetricRow key={k} label={`· ${k}`} value={v} />
                  ))}
                </MetricCard>
              </div>
            ...
            <div style={{ marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: "#e2e8f0" }}>
                Per-Case Results <span style={{ color: "#64748b", fontWeight: 400, fontSize: 12 }}>({visibleCases.length} of {report.caseResults.length})</span>
              </div>
                ...
                return (
                  <div key={c.caseId} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    >
                      ...
                      <div style={{ fontSize: 12, color: passRateColor(c.score), fontWeight: 600 }}>{fmtPct(c.score)}</div>
                    ...
                    {expanded && (
                      <div style={{ padding: "0 14px 14px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                        ...
                        {c.failureReason && (
                          <div style={{ gridColumn: "1 / -1", background: "#ef444412", border: "1px solid #ef444430", borderRadius: 6, padding: "8px 12px", fontSize: 12, color: "#fca5a5" }}>
...
function CaseSection({ title, data }: { title: string; data: Record<string, unknown> }) {
  return (
    <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 6, padding: 10 }}>
