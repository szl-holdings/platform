...
import {
  Shield, Activity, Anchor, Building2, Gavel, Briefcase,
  CheckCircle2, ChevronRight, Server, PlayCircle, Users, Bell,
  Command as CmdIcon, UserCircle, ArrowRight,
} from "lucide-react";
...
const PLATFORMS = [
  { id: "aegis", name: "Aegis", icon: Shield, desc: "Defense & Intelligence Command", color: "#3b82f6" },
  { id: "vessels", name: "Vessels", icon: Anchor, desc: "Maritime Fleet Management", color: "#0ea5e9" },
  { id: "terra", name: "Terra", icon: Building2, desc: "Real Estate Intelligence", color: "#40856a" },
  { id: "lyte", name: "Lyte", icon: Activity, desc: "AIOps & Autonomous Ops", color: "#d4a054" },
  { id: "prism", name: "PRISM Counsel", icon: Gavel, desc: "Legal Matter Intelligence", color: "#f59e0b" },
  { id: "szl", name: "SZL Holdings", icon: Briefcase, desc: "Executive Portfolio Command", color: "#b8bfcb" },
  { id: "carlota-jo", name: "Carlota Jo", icon: UserCircle, desc: "Consulting Platform", color: "#e879f9" },
  { id: "stephen", name: "Stephen", icon: CmdIcon, desc: "Personal Command Center", color: "#818cf8" },
  { id: "command", name: "Command Portal", icon: Server, desc: "Ecosystem Orchestration", color: "#8b7ac8" },
];
          ...
          {STEP_TITLES.map((label, i) => (
            <div key={i} className="flex-1 flex items-center">
              <div className="flex flex-col items-center flex-1">
                <div className={`text-[10px] font-medium transition-colors ${i + 1 === step ? "text-blue-400" : i + 1 < step ? "text-emerald-400" : "text-white/25"}`}>
                  {i + 1 < step ? <CheckCircle2 className="w-3 h-3" /> : label}
                </div>
              </div>
              {i < STEP_TITLES.length - 1 && (
                <div className={`h-px flex-1 transition-colors ${i + 1 < step ? "bg-emerald-500/40" : "bg-white/10"}`} />
              )}
            </div>
          ))}
        </div>
            ...
            {step === 1 && (
              ...
              >
                <div className="text-center mb-8">
                  <h1 className="text-3xl font-bold tracking-tight mb-2">Configure Your Organization</h1>
            ...
            {step === 2 && (
              ...
              >
                <div className="text-center mb-8">
                  ...
                  <p className="text-white/50 text-sm">Choose the intelligence modules to enable for your deployment.</p>
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={`p-4 rounded-xl border cursor-pointer transition-all flex items-start gap-3 text-left ${
                          isSelected ? "border-opacity-60 bg-opacity-10" : "bg-white/[0.02] border-white/[0.07] hover:bg-white/[0.04]"
                        }`}
                        style={isSelected ? { borderColor: app.color + "70", backgroundColor: app.color + "12" } : {}}
                        data-testid={`app-toggle-${app.id}`}
                      >
                        <div
                          className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                          style={isSelected ? { backgroundColor: app.color + "25" } : { backgroundColor: "rgba(255,255,255,0.06)" }}
                        >
                          <app.icon className="w-4 h-4" style={isSelected ? { color: app.color } : { color: "rgba(255,255,255,0.5)" }} />
                        </div>
              ...
              >
                <div className="text-center mb-8">
                  ...
                  <p className="text-white/50 text-sm">
                    Integrate your existing infrastructure. You can also do this later from Settings.
                  </p>
                </div>
                <div className="space-y-3">
                  {DATA_SOURCES.map((src, i) => {
                    ...
                    return (
                      <div key={i} className="p-4 rounded-xl border border-white/[0.07] bg-white/[0.02] flex items-center justify-between">
                        <div>
                          <div className="font-medium text-sm text-white/85">{src}</div>
                          {isConnected && <div className="text-xs text-emerald-400 mt-0.5">Connected</div>}
                        </div>
                        <Button
            ...
            {step === 4 && (
              ...
              >
                <div className="text-center mb-8">
                  ...
                  <p className="text-white/50 text-sm">A 3-minute overview of navigating the Command interface.</p>
                </div>
                <div
                ...
                >
                  ...
                  {tourWatched ? (
                    ...
                    <>
                      ...
                      <div className="text-sm text-white/60 z-10">Click to watch the guided tour</div>
                    </>
                  )}
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { icon: Bell, label: "Alert Configuration", desc: "Set thresholds and routing rules" },
                    { icon: Users, label: "Team Management", desc: "Roles, permissions, and invites" },
                    { icon: ArrowRight, label: "Keyboard Shortcuts", desc: "Navigate at command speed" },
                  ].map(({ icon: Icon, label, desc }, i) => (
                    <div key={i} className="p-4 rounded-xl border border-white/[0.06] bg-white/[0.01] text-center">
              ...
              >
                <div className="text-center mb-8">
                  ...
                  <p className="text-white/50 text-sm">
                    ...
                    {selectedApps.length > 0 && ` ${selectedApps.length} platform${selectedApps.length !== 1 ? "s" : ""} activated.`}
                  </p>
                </div>
                ...
                <div className="bg-white/[0.02] border border-white/[0.07] rounded-2xl p-6 mb-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-xs text-white/40">{checklistCompleted}/{checklistTotal} complete</span>
                  </div>
                  ...
                  <ul className="space-y-3">
                    {([
                      { key: "configure_org", label: "Configure organization", done: true, toggle: false },
                      { key: "activate_platforms", label: `Activate platforms (${selectedApps.length} selected)`, done: selectedApps.length > 0, toggle: false },
                      ...CHECKLIST_ITEMS.map(item => ({ key: item.key, label: item.label, done: checklist[item.key] ?? false, toggle: true })),
                    ] as Array<{ key: string; label: string; done: boolean; toggle: boolean }>).map(({ key, label, done, toggle }) => (
                      <li
                        key={key}
                      ...
                      >
                        <div className={`w-5 h-5 rounded-full border flex items-center justify-center shrink-0 transition-colors ${done ? "bg-emerald-500 border-emerald-500" : "border-white/30 group-hover:border-white/50"}`}>
                          {done && <CheckCircle2 className="w-3 h-3 text-white" />}
                        </div>
                        <span className={done ? "text-white/40 line-through" : "text-white/75"}>{label}</span>
                      </li>
                    ))}
                  </ul>
                </div>
