...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="border-b border-white/10">
      ...
      >
        <span className="font-medium text-white/90 group-hover:text-white transition-colors pr-8">{q}</span>
        <ChevronDown
...
function ContactSalesModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      ...
      >
        <button onClick={onClose} className="absolute top-4 right-4 text-white/40 hover:text-white" data-testid="modal-close">
          <X className="w-5 h-5" />
        </button>
        <h3 className="text-2xl font-bold mb-2">Contact Sales</h3>
        <p className="text-white/50 text-sm mb-6">Tell us about your organization and we'll be in touch within 1 business day.</p>
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <Input required className="bg-white/5 border-white/10 text-white h-10" placeholder="Jane" data-testid="input-first-name" />
              </div>
              <div className="space-y-1">
                <Label className="text-white/70 text-xs">Last Name</Label>
                <Input required className="bg-white/5 border-white/10 text-white h-10" placeholder="Smith" data-testid="input-last-name" />
              </div>
            </div>
            <div className="space-y-1">
              <Label className="text-white/70 text-xs">Work Email</Label>
              <Input required type="email" className="bg-white/5 border-white/10 text-white h-10" placeholder="jane@organization.com" data-testid="input-email" />
            </div>
            <div className="space-y-1">
              <Label className="text-white/70 text-xs">Organization</Label>
              <Input required className="bg-white/5 border-white/10 text-white h-10" placeholder="ACME Defense Corp" data-testid="input-org" />
            </div>
            <div className="space-y-1">
              <Label className="text-white/70 text-xs">Team Size</Label>
              <select className="w-full h-10 rounded-md bg-white/5 border border-white/10 text-white px-3 text-sm" data-testid="select-team-size">
            ...
            <Button type="submit" className="w-full h-11 bg-white text-black hover:bg-white/90 mt-2" data-testid="button-submit-sales">
              Send Inquiry
            </Button>
          </form>
        )}
...
export function MarketingPricing() {
  ...
  const handleCheckout = async (planId: string, tier: string) => {
    ...
    try {
      ...
      if (res.status === 503) {
        toast.error("Pro subscriptions are not yet configured. Please contact sales@szlholdings.com for Pro access.");
      ...
      if (url && url.startsWith("http")) {
        ...
        setTimeout(() => {
          window.location.href = "/command/marketing/signup?success=true";
        }, 1000);
      }
    } catch {
        ...
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <ul className="space-y-3 mb-8 flex-1 text-sm text-white/70">
              <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-white/30 shrink-0" /> 1 User</li>
              <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-white/30 shrink-0" /> Access to 1 Platform</li>
              <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-white/30 shrink-0" /> 7-day audit log</li>
              <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-white/30 shrink-0" /> Community support</li>
            </ul>
            <Button
              variant="outline"
          ...
          >
            ...
            <div className="text-5xl font-bold mb-2">
              ${isAnnual ? "82" : "99"}<span className="text-lg text-white/40 font-normal">/mo</span>
            </div>
            ...
            <ul className="space-y-3 mb-8 flex-1 text-sm text-blue-100/80">
              <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-blue-400 shrink-0" /> Up to 10 Users</li>
            ...
            <Button
              ...
              onClick={() => handleCheckout(isAnnual ? "command-pro-annual" : "command-pro-monthly", "pro")}
          ...
          >
            ...
            <ul className="space-y-3 mb-8 flex-1 text-sm text-white/70">
              ...
              <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-white/30 shrink-0" /> 99.99% SLA</li>
              <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-white/30 shrink-0" /> 24/7 Dedicated Support</li>
            </ul>
            <Button
              variant="outline"
              className="w-full h-11 border-white/20 text-white hover:bg-white/5"
