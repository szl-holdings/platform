...
const EVAL_TYPE_CONFIG: Record<string, { icon: React.ReactNode; color: string; label: string }> = {
  ...
  "recommendation-quality":  { icon: <CheckCircle className="h-3.5 w-3.5" />, color: "#22c55e", label: "Rec Quality" },
  "hallucination":           { icon: <AlertTriangle className="h-3.5 w-3.5" />, color: "#ef4444", label: "Hallucination" },
  "latency-cost":            { icon: <DollarSign className="h-3.5 w-3.5" />, color: "#f59e0b", label: "Latency/Cost" },
  "policy-compliance":       { icon: <Shield className="h-3.5 w-3.5" />,    color: "#f97316", label: "Policy Compliance" },
  "operator-acceptance":     { icon: <Cpu className="h-3.5 w-3.5" />,      color: "#a855f7", label: "Operator Acceptance" },
};

function getTypeConfig(evalType: string) {
  return EVAL_TYPE_CONFIG[evalType] ?? { icon: <FlaskConical className="h-3.5 w-3.5" />, color: "#475569", label: evalType };
}
...
function RegressionBadge({ severity }: { severity: EvalRunSummary["regressionSeverity"] }) {
  ...
  return (
    <span style={{ fontSize: 10, fontWeight: 600, color, background: `${color}18`, padding: "2px 8px", borderRadius: 4, border: `1px solid ${color}40` }}>
      {label}
    </span>
  );
}
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 10, justifyContent: "space-between" }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
            ...
            {suite.domain && (
              <span style={{ fontSize: 10, color: "#475569", background: "rgba(255,255,255,0.05)", padding: "2px 7px", borderRadius: 3 }}>
                {suite.domain}
              </span>
            )}
          </div>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#e2e8f0", marginBottom: 4 }}>{suite.name}</div>
          <p style={{ fontSize: 12, color: "#64748b", margin: "0 0 8px", lineHeight: 1.5 }}>{suite.description}</p>
        ...
        >
          {running ? (
            <>
              <div style={{ width: 10, height: 10, border: "2px solid #fff4", borderTop: "2px solid #fff8", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
              Running…
            </>
          ) : (
            <>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 20 }}>
      <div>
        ...
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, overflow: "hidden", marginBottom: 20 }}>
          <div style={{ display: "grid", gridTemplateColumns: "24px 1fr 70px 70px 100px 120px", gap: 8, padding: "8px 14px", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
            {["", "Suite", "Pass%", "Score", "Latency", "Regression"].map((h) => (
              <div key={h} style={{ fontSize: 9, fontWeight: 700, color: "#334155", textTransform: "uppercase", letterSpacing: 0.5 }}>{h}</div>
            ))}
          </div>
            ...
            return (
              ...
              >
                <div style={{
                  ...
                  {isBaseline && <span style={{ color: "#fff", fontSize: 7, fontWeight: 700 }}>●</span>}
                </div>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 500, color: "#e2e8f0" }}>{run.suiteName}</div>
                  <div style={{ fontSize: 10, color: "#475569" }}>{run.domain} · {new Date(run.runAt).toLocaleDateString()}</div>
                </div>
                <div style={{ fontSize: 12, fontWeight: 600, color: run.passRate >= 0.85 ? "#22c55e" : run.passRate >= 0.7 ? "#f59e0b" : "#ef4444" }}>
                  {(run.passRate * 100).toFixed(1)}%
                </div>
                <div style={{ fontSize: 12, color: "#94a3b8" }}>{(run.avgScore * 100).toFixed(1)}%</div>
                <div style={{ fontSize: 12, color: "#94a3b8" }}>{run.avgLatencyMs > 0 ? `${run.avgLatencyMs.toFixed(0)}ms` : "—"}</div>
          ...
          {runs.length === 0 && (
            <div style={{ textAlign: "center", padding: "40px 0", color: "#475569", fontSize: 13 }}>No runs available — run an eval suite first</div>
          )}
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: 18 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 4 }}>
            2. Configure Variant
          </div>
          <div style={{ fontSize: 11, color: "#334155", marginBottom: 14 }}>
            A new eval run is dispatched against the baseline suite using the selected model, strategy, and prompt. Results are compared in real time.
          </div>

          {baseline && (
            <div style={{ background: `${ACCENT}08`, border: `1px solid ${ACCENT}30`, borderRadius: 6, padding: "8px 10px", marginBottom: 14, fontSize: 11 }}>
              <span style={{ color: "#475569" }}>Baseline: </span>
              <span style={{ color: "#e2e8f0", fontWeight: 500 }}>{baseline.suiteName}</span>
              <span style={{ color: "#475569" }}> · {(baseline.passRate * 100).toFixed(1)}% pass</span>
            </div>
          )}
          ...
          {error && (
            <div style={{ background: "#ef444418", border: "1px solid #ef444440", borderRadius: 6, padding: "8px 10px", marginBottom: 10, color: "#ef4444", fontSize: 11 }}>
              {error}
            </div>
          )}
        ...
        {compareResult && (
          <div style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${ACCENT}40`, borderRadius: 10, padding: 18 }}>
            ...
            <div style={{ fontSize: 11, color: "#475569", marginBottom: 14 }}>
              {compareResult.variant.suiteName} · {variant.model} · {new Date(compareResult.variant.runAt).toLocaleDateString()}
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "80px 80px 80px 70px", gap: 4, padding: "4px 0 8px", borderBottom: "1px solid rgba(255,255,255,0.06)", marginBottom: 4 }}>
            ...
            {[
              { label: "Pass Rate", a: compareResult.baseline.passRate, b: compareResult.variant.passRate, format: (v: number) => `${(v * 100).toFixed(1)}%`, higherBetter: true },
              { label: "Avg Score", a: compareResult.baseline.avgScore, b: compareResult.variant.avgScore, format: (v: number) => `${(v * 100).toFixed(1)}%`, higherBetter: true },
              { label: "Latency", a: compareResult.baseline.avgLatencyMs, b: compareResult.variant.avgLatencyMs, format: (v: number) => v > 0 ? `${v.toFixed(0)}ms` : "—", higherBetter: false },
              { label: "Cost", a: compareResult.baseline.totalCostUsd, b: compareResult.variant.totalCostUsd, format: (v: number) => `$${v.toFixed(5)}`, higherBetter: false },
              ...
              const deltaLabel = Math.abs(delta) < 0.001 ? "—" : `${delta > 0 ? "▲" : "▼"} ${higherBetter ? `${(Math.abs(delta) * 100).toFixed(1)}pp` : label === "Latency" ? `${Math.abs(delta).toFixed(0)}ms` : `$${Math.abs(delta).toFixed(5)}`}`;
              return (
                <div key={label} style={{ display: "grid", gridTemplateColumns: "80px 80px 80px 70px", gap: 4, padding: "6px 0", borderBottom: "1px solid rgba(255,255,255,0.04)", fontSize: 11, alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12, paddingTop: 10, borderTop: "1px solid rgba(255,255,255,0.05)" }}>
              {[
                ...
                <div key={label} style={{ flex: 1, background: "rgba(255,255,255,0.03)", borderRadius: 6, padding: "8px 10px" }}>
...
export default function EvalStudio() {
  ...
  const filteredSuites = filterType === "all" ? suites : suites.filter((s) => s.evalType === filterType);
  ...
  async function handleRunSuite(suite: EvalSuiteConfig) {
    ...
    try {
      const csrfToken = getCsrfToken();
      const result = await fetchJson<{ runId?: string; passRate?: number; avgScore?: number; hasRegression?: boolean; regressionSeverity?: string }>(apiUrl("/evals/run"), {
        method: "POST",
  ...
  const auditEvents: AuditEvent[] = runs.slice(0, 20).map((r) => ({
    ...
    detail: r.hasRegression ? `Regression detected — ${r.regressionSeverity}` : `Pass rate ${(r.passRate * 100).toFixed(1)}%`,
    timestamp: r.runAt,
    outcome: r.hasRegression ? (r.regressionSeverity === "critical" ? "failure" : "blocked") : "success",
  }));
      ...
      <div style={{ maxWidth: 1440, margin: "0 auto", padding: "24px 20px" }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>
            Run and compare evaluations across rule, simulation, recommendation quality, hallucination, latency/cost, policy compliance, and operator acceptance suites. Replay runs against variant models, prompts, and orchestration strategies.
          </p>
        </div>
        ...
        {!isLoading && !error && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 24 }}>
            {[
              ...
              { label: "Avg Pass Rate", value: `${(avgPassRate * 100).toFixed(1)}%`, color: "#22c55e" },
              { label: "Regressions", value: regressionCount, color: criticalRegressionCount > 0 ? "#ef4444" : "#f59e0b" },
            ].map((m) => (
        ...
        {error && (
          <div style={{ background: "#ef444418", border: "1px solid #ef444440", borderRadius: 8, padding: "12px 16px", marginBottom: 20, color: "#ef4444", fontSize: 13 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
              ...
              {tab === "suites" && (
                <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                  {evalTypes.map((et) => {
                    const cfg = et === "all" ? { color: "#475569", label: "All" } : getTypeConfig(et);
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 20, alignItems: "start" }}>
                <div>
                  {filteredSuites.length === 0 && (
                    <div style={{ textAlign: "center", padding: "40px 0", color: "#475569", fontSize: 13 }}>No eval suites found</div>
                  )}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  {selectedSuite && (
                    <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: 18 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#e2e8f0", marginBottom: 10 }}>{selectedSuite.name}</div>
            ...
            {tab === "results" && (
              <div>
                <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, overflow: "hidden" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 80px 80px 80px 100px 120px", gap: 8, padding: "8px 16px", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
                  ...
                  {runs.length === 0 && (
                    <div style={{ textAlign: "center", padding: "40px 0", color: "#475569", fontSize: 13 }}>No eval results yet — run an evaluation suite</div>
                  )}
                      ...
                      >
                        <div>
                          <div style={{ fontSize: 12, fontWeight: 500, color: "#e2e8f0", display: "flex", alignItems: "center", gap: 6 }}>
                            {expandedRun === r.runId ? <ChevronDown className="h-3 w-3 text-[#475569]" /> : <ChevronRight className="h-3 w-3 text-[#475569]" />}
                            {r.suiteName ?? r.suiteId}
                          </div>
                          <div style={{ fontSize: 10, color: "#475569", marginLeft: 15 }}>{r.domain ?? "—"} · {new Date(r.runAt).toLocaleDateString()}</div>
                        </div>
                        <div style={{ fontSize: 12, fontWeight: 600, color: r.passRate >= 0.85 ? "#22c55e" : r.passRate >= 0.7 ? "#f59e0b" : "#ef4444" }}>
                          {(r.passRate * 100).toFixed(1)}%
                        </div>
                        <div style={{ fontSize: 12, color: "#94a3b8" }}>{(r.avgScore * 100).toFixed(1)}%</div>
                        <div style={{ fontSize: 12, color: "#94a3b8" }}>{r.passed}/{r.totalCases}</div>
                        <div style={{ fontSize: 12, color: "#94a3b8" }}>{r.avgLatencyMs > 0 ? `${r.avgLatencyMs.toFixed(0)}ms` : "—"}</div>
                        <RegressionBadge severity={r.regressionSeverity ?? "none"} />
                      </div>

                      {expandedRun === r.runId && (
                        <div style={{ padding: "12px 16px 16px", background: "rgba(255,255,255,0.01)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                            {r.regressionNotes.length > 0 && (
                              <div style={{ background: "#ef444408", border: "1px solid #ef444425", borderRadius: 7, padding: 10 }}>
                                <div style={{ fontSize: 10, fontWeight: 700, color: "#ef4444", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 }}>Regressions</div>
                                {r.regressionNotes.map((n, i) => (
                                  <div key={i} style={{ fontSize: 11, color: "#f87171", lineHeight: 1.5 }}>{n}</div>
                                ))}
                              </div>
                            )}
                            {r.improvementNotes.length > 0 && (
                              <div style={{ background: "#22c55e08", border: "1px solid #22c55e25", borderRadius: 7, padding: 10 }}>
                                <div style={{ fontSize: 10, fontWeight: 700, color: "#22c55e", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 }}>Improvements</div>
                                {r.improvementNotes.map((n, i) => (
                                  <div key={i} style={{ fontSize: 11, color: "#86efac", lineHeight: 1.5 }}>{n}</div>
                                ))}
                              </div>
                            )}
                          </div>
                          <div style={{ display: "flex", gap: 12, fontSize: 11, color: "#475569" }}>
                            ...
                            <span>Run ID: <span style={{ fontFamily: "monospace" }}>{r.runId.slice(0, 16)}…</span></span>
                          </div>
                        </div>
                      )}
                    </div>
