  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "#080c14", color: "rgba(255,255,255,0.85)" }}>
      <div className="px-6 py-4 border-b flex items-center justify-between" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        ...
        <div className="text-[9px] font-mono px-2 py-1 rounded" style={{ background: "rgba(34,197,94,0.08)", color: "#22c55e", border: "1px solid rgba(34,197,94,0.15)" }}>
          LIVE · trace-graph
        </div>
      </div>
        ...
        <div className="px-6 py-3 border-b" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
          <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
            {[
              ...
              { label: "Total Runs",    value: pipeline.totalRuns.toLocaleString(),   color: "rgba(255,255,255,0.6)" },
              { label: "7-Day Runs",    value: pipeline.weeklyRuns.toLocaleString(),  color: "#0ea5e9" },
            ].map((m) => (
              <div key={m.label} className="text-center">
                <div className="text-base font-bold font-mono" style={{ color: m.color }}>{m.value}</div>
                <div className="text-[8px] uppercase tracking-wide" style={{ color: "rgba(255,255,255,0.25)" }}>{m.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="px-6 py-2 border-b flex items-center gap-2" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
      ...
      <div className="flex-1 overflow-auto px-6 py-4">
        ...
        {error && <div className="text-center py-12 text-sm" style={{ color: "#ef4444" }}>Failed to load portfolio intelligence</div>}
        {!isLoading && !error && accounts.length === 0 && (
          <div className="text-center py-12 text-sm" style={{ color: "rgba(255,255,255,0.3)" }}>No products match this filter</div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {accounts.map((acct) => {
            ...
            const passColor  = acct.passRate === null  ? "#64748b"
                             : acct.passRate >= 90     ? "#22c55e"
                             : acct.passRate >= 70     ? "#f59e0b"
                             :                           "#ef4444";
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-sm font-semibold" style={{ color: acct.color }}>{acct.name}</div>
                      <div className="text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.25)" }}>{acct.domain}</div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded p-2" style={{ background: "rgba(255,255,255,0.03)" }}>
                    <div className="text-[8px] uppercase tracking-wide mb-0.5" style={{ color: "rgba(255,255,255,0.2)" }}>30-Day Runs</div>
                    <div className="text-sm font-mono font-bold" style={{ color: acct.totalRuns > 0 ? "rgba(255,255,255,0.75)" : "rgba(255,255,255,0.25)" }}>
                      {acct.totalRuns.toLocaleString()}
                    </div>
                  </div>
                  <div className="rounded p-2" style={{ background: "rgba(255,255,255,0.03)" }}>
                    <div className="text-[8px] uppercase tracking-wide mb-0.5" style={{ color: "rgba(255,255,255,0.2)" }}>7-Day Runs</div>
                    <div className="text-sm font-mono font-bold" style={{ color: acct.weeklyRuns > 0 ? "#0ea5e9" : "rgba(255,255,255,0.25)" }}>
                      {acct.weeklyRuns.toLocaleString()}
                    </div>
                  </div>
                  <div className="rounded p-2" style={{ background: "rgba(255,255,255,0.03)" }}>
                    <div className="text-[8px] uppercase tracking-wide mb-0.5" style={{ color: "rgba(255,255,255,0.2)" }}>Pass Rate</div>
                    <div className="text-sm font-mono font-bold" style={{ color: passColor }}>
                      {acct.passRate !== null ? `${acct.passRate}%` : "—"}
                    </div>
                  </div>
                  <div className="rounded p-2" style={{ background: "rgba(255,255,255,0.03)" }}>
                    <div className="text-[8px] uppercase tracking-wide mb-0.5" style={{ color: "rgba(255,255,255,0.2)" }}>Errors</div>
                    <div className="text-sm font-mono font-bold" style={{ color: acct.errorCount > 0 ? "#ef4444" : "rgba(255,255,255,0.25)" }}>
                ...
                {acct.totalRuns > 0 && acct.passRate !== null && (
                  <div>
                    ...
                    <div className="text-[8px] font-mono mt-0.5" style={{ color: "rgba(255,255,255,0.2)" }}>
