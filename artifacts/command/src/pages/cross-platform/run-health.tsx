...
function MiniSparkline({ trend }: { trend: { date?: string; passRate: number | null; runs: number }[] }) {
  const filled = trend.filter((t) => t.passRate !== null && t.runs > 0);
  if (!trend || trend.length === 0 || filled.length < 2) return (
    <div className="w-[60px] h-[28px] flex items-center justify-center">
      <span className="text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.15)" }}>no runs</span>
    </div>
  );
  const h = 28;
  const w = 60;
    ...
    <div className="w-full">
      <div className="flex rounded-full overflow-hidden h-1.5 gap-px">
        {bars.map((b) => b.value > 0 && (
          <div key={b.label} style={{ width: `${b.value}%`, background: b.color }} title={`${b.label}: ${b.value}%`} />
        ))}
      </div>
      <div className="mt-1 flex gap-2 flex-wrap">
        {bars.map((b) => b.value > 0 && (
          <span key={b.label} className="text-[9px] font-mono flex items-center gap-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: "#080c14", color: "rgba(255,255,255,0.85)" }}>
      <div className="px-6 py-4 border-b flex items-center justify-between" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        ...
        {agg && (
          <div className="hidden md:flex items-center gap-4 text-[10px] font-mono">
            <span style={{ color: "rgba(255,255,255,0.4)" }}>{agg.totalRuns.toLocaleString()} runs total</span>
      ...
      <div className="flex-1 overflow-auto px-6 py-4">
        ...
        {error && <div className="text-center py-12 text-sm" style={{ color: "#ef4444" }}>Failed to load run health</div>}

        {agg && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            {[
              { label: "Total Runs", value: agg.totalRuns.toLocaleString(), color: "#8b7ac8" },
              { label: "Pass Rate", value: agg.totalRuns > 0 ? `${Math.round((agg.totalPass / agg.totalRuns) * 100)}%` : "—", color: "#22c55e" },
              ...
              <div key={m.label} className="rounded-lg p-4" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
                <div className="text-[9px] uppercase tracking-widest font-mono mb-1" style={{ color: "rgba(255,255,255,0.25)" }}>{m.label}</div>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-[11px] font-semibold" style={{ color: p.color }}>{p.label}</div>
                      <div className="text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.25)" }}>{p.product}</div>
                    </div>
                  </div>
                  <span
                    className="text-[9px] font-mono px-2 py-0.5 rounded"
                    style={{ color: statusMeta.color, background: `${statusMeta.color}12`, border: `1px solid ${statusMeta.color}25` }}
                  >
                    {statusMeta.label}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 mb-3">
                  <div className="text-center">
                    <div className="text-base font-bold font-mono" style={{ color: "#22c55e" }}>{p.passRate}%</div>
                    <div className="text-[8px] uppercase tracking-wide" style={{ color: "rgba(255,255,255,0.25)" }}>Pass Rate</div>
                  </div>
                  <div className="text-center">
                    <div className="text-base font-bold font-mono" style={{ color: "rgba(255,255,255,0.8)" }}>{p.totalRuns}</div>
                    <div className="text-[8px] uppercase tracking-wide" style={{ color: "rgba(255,255,255,0.25)" }}>Total Runs</div>
                  </div>
                  <div className="text-center">
                    <div className="text-base font-bold font-mono" style={{ color: "#f59e0b" }}>{p.policyBreachCount}</div>
                    <div className="text-[8px] uppercase tracking-wide" style={{ color: "rgba(255,255,255,0.25)" }}>Breaches</div>
                  </div>
                </div>

                <div className="flex items-center justify-between mb-3">
                  <MiniSparkline trend={p.p7dTrend} />
                  <div className="flex items-center gap-1 text-[10px] font-mono" style={{ color: deltaColor }}>
                ...
                <div className="mb-2">
                  <div className="text-[9px] uppercase tracking-widest font-mono mb-1.5" style={{ color: "rgba(255,255,255,0.2)" }}>Autonomy Mix</div>
                  <AutonomyBar mix={p.autonomyMix} />
                </div>

                <div className="text-[9px] font-mono mt-2" style={{ color: "rgba(255,255,255,0.2)" }}>
                  {p.lastRunAt ? `Last run ${new Date(p.lastRunAt).toLocaleString()}` : "No runs recorded"}
                </div>
              </div>
            );
          })}
