...
const DOMAIN_CONFIG: Record<Domain, { label: string; color: string; icon: typeof Globe; appPath: string }> = {
...
const DEMO_WORLDLINES: Worldline[] = [
  {
    ...
    twins: [
      { id: "tw-terra-001", name: "Terra Property Fabric", domain: "terra", state: "stable", driftScore: 4, lastSync: "45s ago", proofState: "verified", pendingActions: 0, summary: "All property twins synced, portfolio valuation current" },
      { id: "tw-vessels-001", name: "Vessels Fleet Twin", domain: "vessels", state: "stable", driftScore: 7, lastSync: "2m ago", proofState: "verified", pendingActions: 0, summary: "14 vessels tracked, AIS data nominal" },
      { id: "tw-alloy-001", name: "Alloy Execution Fabric", domain: "alloy", state: "stable", driftScore: 2, lastSync: "30s ago", proofState: "verified", pendingActions: 0, summary: "247 workflows active, proof chain intact" },
      { id: "tw-prism-001", name: "Prism Counsel Twin", domain: "prism", state: "stable", driftScore: 3, lastSync: "1m ago", proofState: "verified", pendingActions: 0, summary: "Legal entity registry current, 2 NDAs in flight" },
    ],
  },
  {
    id: "WL-BETA",
    ...
    twins: [
      { id: "tw-aeg-001", name: "Aegis Posture Twin", domain: "aegis", state: "degraded", driftScore: 28, lastSync: "3m ago", proofState: "pending", pendingActions: 3, summary: "3 active incidents in AWS VPC, K8s app tier drifted" },
      { id: "tw-lyte-001", name: "Lyte AIOps Twin", domain: "lyte", state: "degraded", driftScore: 21, lastSync: "6m ago", proofState: "pending", pendingActions: 4, summary: "4 SLO breaches active, 1 requires human escalation" },
    ],
  },
  {
    id: "WL-GAMMA",
    ...
    twins: [
      { id: "tw-aeg-002", name: "Aegis OT/ICS Twin", domain: "aegis", state: "awaiting_approval", driftScore: 18, lastSync: "11m ago", proofState: "pending", pendingActions: 1, summary: "PLC firmware delta awaiting CISO approval" },
    ],
  },
  {
    id: "WL-DELTA",
    ...
    twins: [
      { id: "tw-vessels-002", name: "Vessels Cargo Twin", domain: "vessels", state: "awaiting_approval", driftScore: 12, lastSync: "8m ago", proofState: "pending", pendingActions: 2, summary: "Cargo manifest variance on VES-MV-047" },
    ],
  },
  {
    id: "WL-SIM-001",
    ...
    twins: [
      { id: "tw-terra-sim-001", name: "Terra Stress Scenario", domain: "terra", state: "stable", driftScore: 0, lastSync: "2d ago", proofState: "verified", pendingActions: 0, summary: "Scenario result: 12% cap rate expansion, 2 properties enter distress" },
    ],
  },
];
...
function WorldlineStatusBadge({ status }: { status: Worldline["status"] }) {
  const cfg = {
    ...
    simulation: { color: "#8b7ac8", label: "Simulation", bg: "rgba(139,122,200,0.08)", border: "rgba(139,122,200,0.2)" },
    archived: { color: "#6b7280", label: "Archived", bg: "rgba(107,114,128,0.08)", border: "rgba(107,114,128,0.2)" },
  }[status];
  return (
...
function DriftBadge({ score }: { score: number }) {
  ...
  return (
    <span className="text-[8px] font-bold font-mono px-1 py-0.5 rounded" style={{ color, background: `${color}15`, border: `1px solid ${color}25` }}>
      Δ{score}%
    </span>
  );
}
...
export function WorldlineRegistryPage() {
  ...
  const degradedCount = DEMO_WORLDLINES.reduce((s, wl) => s + wl.twins.filter(t => t.state === "degraded" || t.state === "awaiting_approval").length, 0);
    ...
    <div className="max-w-7xl mx-auto space-y-6 p-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest font-mono" style={{ color: "#8b7ac8" }}>Command · ATLAS</span>
          </div>
          <h1 className="text-xl font-bold text-white tracking-tight">Worldline Registry</h1>
          <p className="text-[11px] mt-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>Causal branch navigation — all active, simulation, and archived worldlines across domains.</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: "Need Attention", value: degradedCount, color: degradedCount > 0 ? "#f59e0b" : "#10b981", pulse: degradedCount > 0 },
      ...
      {apiBranches.length > 0 && (
        <div className="rounded-xl border p-4" style={{ borderColor: "rgba(139,122,200,0.12)", background: "rgba(139,122,200,0.02)" }}>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest font-mono" style={{ color: "#8b7ac8" }}>Live Scenario Branches from API</span>
            <span className="ml-auto text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.25)" }}>{apiBranches.length} branch{apiBranches.length !== 1 ? "es" : ""}</span>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
            {apiBranches.slice(0, 6).map(br => (
              <div key={br.id} className="rounded-lg border p-3" style={{ borderColor: "rgba(139,122,200,0.15)", background: "rgba(139,122,200,0.03)" }}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] font-bold text-white truncate">{br.branchName}</span>
                  <span className="text-[8px] font-mono px-1.5 py-0.5 rounded" style={{ color: br.status === "active" ? "#10b981" : "#8b7ac8", background: br.status === "active" ? "rgba(16,185,129,0.1)" : "rgba(139,122,200,0.1)" }}>{br.status}</span>
                </div>
                {br.branchDescription && <div className="text-[9px]" style={{ color: "rgba(255,255,255,0.4)" }}>{br.branchDescription}</div>}
                <div className="flex items-center gap-2 mt-2">
                  {br.twinCategory && <span className="text-[8px] font-mono" style={{ color: "rgba(255,255,255,0.25)" }}>{br.twinCategory}</span>}
                  <button onClick={() => copyId(br.id)} className="ml-auto flex items-center gap-1 text-[8px] px-1.5 py-0.5 rounded hover:bg-white/5 transition-colors" style={{ color: "rgba(255,255,255,0.3)" }}>
      ...
      {loadingBranches && !branchData && (
        <div className="rounded-xl border p-4 flex items-center gap-3" style={{ borderColor: "rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.01)" }}>
          <RefreshCw className="w-3.5 h-3.5 animate-spin" style={{ color: "rgba(139,122,200,0.5)" }} />
          <span className="text-[11px]" style={{ color: "rgba(255,255,255,0.35)" }}>Loading live branch data from API…</span>
        </div>
      )}
      ...
      <div className="space-y-4">
        {visible.map(wl => {
          ...
          const lineColor = allStable ? "#10b981" : hasUrgent ? "#f59e0b" : hasPending ? "#8b7ac8" : "#6b7280";
              ...
              <div className="p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      ...
                      <span className="ml-auto text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.25)" }}>Created {wl.createdAt}</span>
                    </div>
                    <div className="text-[11px] text-white mb-1">{wl.description}</div>
                    {wl.branchReason && (
                      <div className="text-[10px] italic" style={{ color: "rgba(255,255,255,0.3)" }}>Branch reason: {wl.branchReason}</div>
                    )}
                  </div>
                  <div className="shrink-0 flex items-center gap-2">
                    <div className="text-right">
                      ...
                      <div className="text-[9px]" style={{ color: allStable ? "#10b981" : lineColor }}>{allStable ? "All stable" : hasUrgent ? "Degraded" : "Pending"}</div>
                    </div>
                ...
                {isSelected && (
                  <div className="mt-4 pt-4 border-t space-y-3" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
                    <div className="text-[9px] font-bold uppercase tracking-widest mb-2" style={{ color: "rgba(255,255,255,0.25)" }}>Twin Detail</div>
                        ...
                        return (
                          <div key={tw.id} className="rounded-xl border p-3" style={{ borderColor: `${s.color}20`, background: s.bg }}>
                            <div className="flex items-start gap-2 mb-2">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="text-[9px] font-bold mb-0.5" style={{ color: d.color }}>{d.label}</div>
                                <div className="text-[10px] font-semibold text-white truncate">{tw.name}</div>
                              </div>
                              <DriftBadge score={tw.driftScore} />
                            </div>
                            <div className="text-[9px] mb-2" style={{ color: "rgba(255,255,255,0.45)" }}>{tw.summary}</div>
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-1">
                                <span className="text-[8px] font-mono" style={{ color: tw.proofState === "verified" ? "#10b981" : tw.proofState === "pending" ? "#f59e0b" : "#ef4444" }}>
                                  {tw.proofState === "verified" ? <CheckCircle className="w-2.5 h-2.5 inline" /> : tw.proofState === "pending" ? <Clock className="w-2.5 h-2.5 inline" /> : <AlertTriangle className="w-2.5 h-2.5 inline" />}
                                  {" "}{tw.proofState}
                                </span>
                    ...
                    <div className="flex items-center gap-2 pt-1 flex-wrap">
                      <a
                        ...
                        style={{ color: lineColor, borderColor: `${lineColor}30`, background: `${lineColor}06` }}
