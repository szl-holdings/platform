...
export function VoiceoverManager({ campaignId }: { campaignId: string }) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-6">
      <div className="bg-cyan-500/5 border border-cyan-500/15 rounded-xl p-6 mb-8 flex items-start gap-4">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${isCreating ? "border border-white/10 text-slate-400 hover:border-white/20" : "bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 hover:bg-cyan-500/20"}`}
        >
      ...
      {isCreating && (
        <div className="p-6 mb-8 border border-cyan-500/20 bg-cyan-500/3 rounded-xl">
          ...
          <textarea
            ...
            className="w-full h-24 bg-white/5 border border-white/10 rounded-lg p-3 text-sm text-slate-200 resize-none outline-none focus:border-cyan-500/30 placeholder:text-slate-600 mb-4"
          />
          <div className="flex justify-end">
            <button
              onClick={handleCreate}
              disabled={!newText.trim() || createVoice.isPending}
              className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 hover:bg-cyan-500/20 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
      ...
      <div className="space-y-4">
        {voices?.map(voice => (
          <div key={voice.id} className="p-4 flex items-center gap-4 bg-[#0d1117] border border-white/8 hover:border-white/12 rounded-xl transition-colors">
            <button className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center hover:bg-cyan-500/10 hover:text-cyan-400 transition-colors shrink-0 text-slate-400">
