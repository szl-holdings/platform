    ...
    <div className="min-h-screen text-white px-6 py-16 max-w-6xl mx-auto">
      ...
      <div className="mb-12">
        ...
        <p className="text-white/50 max-w-2xl leading-relaxed">
          Alloy's governance layer wraps every workflow in accountability controls. Human approval
      ...
      {/* Principle Statement */}
      <div
        className="mb-14 p-8 rounded-2xl border"
        style={{
        ...
        <div className="md:flex md:gap-12 md:items-center">
          <div className="md:flex-1 mb-6 md:mb-0">
            ...
            <p className="text-white/60 leading-relaxed">
              Automation accelerates work. Governance keeps it accountable. Alloy is designed on the
            ...
            <p className="text-white/60 leading-relaxed mt-3">
              Confidence signals flag uncertainty. Escalation rules surface exceptions. Role-based
              controls ensure the right people make the right decisions. And every outcome —
              approved or rejected — becomes a permanent record.
            </p>
          </div>
      ...
      <div className="mb-14">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {GOVERNANCE_CONTROLS.map((control) => {
            const accent = CATEGORY_ACCENT[control.category] ?? '#4B8BDB';
            return (
              <div
                key={control.id}
              ...
              >
                ...
                >
                  <div className="text-[10px] uppercase tracking-widest text-white/25 mb-1.5">
                    Mechanism
                  </div>
      ...
      <div className="mb-14">
        ...
        >
          <div className="flex flex-col md:flex-row items-center gap-3 flex-wrap justify-center">
            {[
              { label: 'Alloy Output', sub: 'Generated with confidence score', color: '#4B8BDB' },
              { label: '→', sub: '', color: 'rgba(255,255,255,0.2)' },
              { label: 'Confidence Gate', sub: 'Auto-route if above threshold', color: '#3b82f6' },
              { label: '→', sub: '', color: 'rgba(255,255,255,0.2)' },
              {
                label: 'Human Review',
                sub: 'Approval agent provides full context',
                color: '#f59e0b',
              },
              { label: '↓', sub: '', color: 'rgba(255,255,255,0.2)' },
              {
                label: 'Decision Captured',
                sub: 'Approved or rejected with record',
                color: '#10b981',
              },
              { label: '→', sub: '', color: 'rgba(255,255,255,0.2)' },
              ...
              node.label === '→' || node.label === '↓' ? (
                <div key={i} className="text-xl font-bold" style={{ color: node.color }}>
                  {node.label}
                </div>
              ) : (
                <div
                  key={i}
                  className="text-center p-4 rounded-lg border min-w-[120px]"
                  style={{ borderColor: `${node.color}30`, background: `${node.color}08` }}
                >
                  <div className="text-xs font-bold mb-1" style={{ color: node.color }}>
                    {node.label}
                  </div>
      ...
      >
        ...
        <p className="text-white/50 mb-6">
          Explore how approval flows integrate with the Workflow Coordinator and Approval Agent.
        </p>
