...
import {
  ...
  Lock, Zap, RefreshCw, Copy, Settings2, Eye, Trash2
} from "lucide-react";
...
interface Policy {
  ...
  slug: string;
  orgId: number | null;
  kind: "approval_matrix" | "model_routing" | "cost_control" | "agent_permission" | "compliance_template";
...
const DEMO_POLICIES: Policy[] = [
  {
    id: 1, name: "Browser Form Submission Approval", slug: "browser-form-approval",
    orgId: 1, kind: "approval_matrix", status: "active",
  ...
  {
    id: 2, name: "Artifact Publishing Compliance Review", slug: "artifact-publish-compliance",
    orgId: 1, kind: "approval_matrix", status: "active",
    description: "All artifacts must pass compliance review before publishing to external channels.",
    rules: {
  ...
  {
    id: 3, name: "Claude for Analysis — $0.50 Ceiling", slug: "model-routing-claude-analysis",
    orgId: 1, kind: "model_routing", status: "active",
    description: "Route analysis tasks to Claude with a $0.50 per-invocation cost ceiling. GPT fallback for general ops. Gemini blocked in production.",
    rules: {
      allowedModels: ["claude-sonnet-4-6", "gpt-5.2"],
      blockedModels: ["gemini-3.1-pro-preview"],
      costCeilingPerCall: 0.50,
  ...
  {
    id: 4, name: "Monthly Budget Control — $500", slug: "monthly-budget-500",
    orgId: 1, kind: "cost_control", status: "active",
  ...
  {
    id: 5, name: "SOC 2 Compliance Template", slug: "soc2-template",
    orgId: null, kind: "compliance_template", status: "active",
    description: "Pre-built governance template for SOC 2 Type II compliance frameworks. One-click apply to any tenant.",
    rules: {
  ...
  {
    id: 6, name: "HIPAA Data Handling Template", slug: "hipaa-template",
    orgId: null, kind: "compliance_template", status: "draft",
  ...
  {
    id: 7, name: "Admin-Only Agent Access", slug: "admin-agent-access",
    orgId: 1, kind: "agent_permission", status: "active",
...
const DEMO_INCIDENTS: GovernanceIncident[] = [
  {
    id: 1, policyId: 1, policyName: "Browser Form Submission Approval",
    severity: "high", type: "policy_violation",
    description: "Agent attempted browser form submission without queuing for manager approval. Request blocked by policy middleware.",
  ...
  {
    id: 2, policyId: 4, policyName: "Monthly Budget Control — $500",
    severity: "medium", type: "cost_threshold",
    description: "Tenant #1 reached 80% of monthly budget ($400/$500). Alert sent to admin and ops roles.",
    resolution: "Monitoring. No action required yet.",
    resolvedAt: null,
    createdAt: new Date(Date.now() - 6 * 3600000).toISOString(),
  },
  {
    id: 3, policyId: 7, policyName: "Admin-Only Agent Access",
    severity: "low", type: "user_override",
    description: "User with analyst role attempted to invoke Sentinel agent. Access denied, incident logged.",
...
const KIND_CONFIG: Record<string, { label: string; color: string; bg: string; border: string; icon: React.ReactNode }> = {
  approval_matrix: {
    label: "Approval Matrix", color: "#8b5cf6", bg: "rgba(139,92,246,0.06)", border: "rgba(139,92,246,0.2)",
    icon: <CheckCircle className="w-3.5 h-3.5" />,
  },
  model_routing: {
    label: "Model Routing", color: "#0ea5e9", bg: "rgba(14,165,233,0.06)", border: "rgba(14,165,233,0.2)",
    icon: <Zap className="w-3.5 h-3.5" />,
  },
  cost_control: {
    label: "Cost Control", color: "#f59e0b", bg: "rgba(245,158,11,0.06)", border: "rgba(245,158,11,0.2)",
    icon: <AlertTriangle className="w-3.5 h-3.5" />,
  },
  agent_permission: {
    label: "Agent Permission", color: "#ef4444", bg: "rgba(239,68,68,0.06)", border: "rgba(239,68,68,0.2)",
    icon: <Lock className="w-3.5 h-3.5" />,
  },
  compliance_template: {
    label: "Compliance Template", color: "#10b981", bg: "rgba(16,185,129,0.06)", border: "rgba(16,185,129,0.2)",
...
function PolicyCard({ policy, onView }: { policy: Policy; onView: (p: Policy) => void }) {
  ...
  return (
    <div className="rounded-xl border overflow-hidden" style={{ borderColor: "rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.015)" }}>
      <div
        className="flex items-start gap-3 p-4 cursor-pointer"
        onClick={() => setExpanded(e => !e)}
      >
        <div className="mt-0.5 p-1.5 rounded-lg shrink-0" style={{ background: cfg.bg, border: `1px solid ${cfg.border}`, color: cfg.color }}>
          {cfg.icon}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span className="text-sm font-semibold text-white">{policy.name}</span>
            <span className="text-[9px] uppercase tracking-widest px-1.5 py-0.5 rounded border font-bold shrink-0"
              style={{ color: cfg.color, borderColor: cfg.border, background: cfg.bg }}>
              {cfg.label}
            </span>
            <span className="text-[9px] uppercase tracking-widest px-1.5 py-0.5 rounded font-semibold shrink-0"
              style={{ color: statusColor, background: `${statusColor}15` }}>
              {policy.status}
            </span>
          </div>
          {policy.description && (
            <p className="text-[11px] mb-1" style={{ color: "rgba(255,255,255,0.45)" }}>{policy.description}</p>
          )}
          <div className="text-[10px]" style={{ color: "rgba(255,255,255,0.25)" }}>
            Updated {formatRelative(policy.updatedAt)} · {policy.orgId ? `Tenant #${policy.orgId}` : "Global template"}
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={e => { e.stopPropagation(); onView(policy); }}
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t space-y-3" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
          <div className="pt-3" />
          <div className="text-[10px] font-mono rounded-lg p-3" style={{ background: "rgba(0,0,0,0.4)", color: "rgba(255,255,255,0.5)", border: "1px solid rgba(255,255,255,0.06)" }}>
            <pre style={{ whiteSpace: "pre-wrap", wordBreak: "break-word" }}>{JSON.stringify(policy.rules, null, 2)}</pre>
          </div>
          <div className="flex gap-2">
            {policy.kind === "compliance_template" && (
              <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all"
            ...
            <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all"
              style={{ borderColor: "rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.03)", color: "rgba(255,255,255,0.4)" }}>
...
function IncidentRow({ incident }: { incident: GovernanceIncident }) {
  ...
  return (
    <div className="flex items-start gap-3 p-3 rounded-xl border" style={{
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          <span className="text-xs font-semibold text-white">{typeLabels[incident.type] ?? incident.type}</span>
        ...
        <p className="text-[11px] mb-1" style={{ color: "rgba(255,255,255,0.5)" }}>{incident.description}</p>
        {incident.resolution && (
          <p className="text-[10px]" style={{ color: "rgba(255,255,255,0.3)" }}>Resolution: {incident.resolution}</p>
        )}
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const defaultRules: Record<Policy["kind"], Record<string, unknown>> = {
        approval_matrix: { trigger: "agent_run", requiredApprovalLevel: "manager", expiresAfterHours: 24 },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.75)" }}
      ...
      <div className="w-full max-w-md rounded-2xl border p-6 space-y-4"
        ...
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-[11px]" style={{ color: "rgba(255,255,255,0.3)" }}>✕</button>
        </div>
        <div className="space-y-3">
          <div>
            <label className="text-[10px] font-medium mb-1 block" style={{ color: "rgba(255,255,255,0.4)" }}>Policy Name *</label>
          ...
          <div>
            <label className="text-[10px] font-medium mb-1 block" style={{ color: "rgba(255,255,255,0.4)" }}>Policy Type *</label>
            <select value={kind} onChange={e => setKind(e.target.value as Policy["kind"])}
          ...
          <div>
            <label className="text-[10px] font-medium mb-1 block" style={{ color: "rgba(255,255,255,0.4)" }}>Org ID</label>
          ...
          <div>
            <label className="text-[10px] font-medium mb-1 block" style={{ color: "rgba(255,255,255,0.4)" }}>Description</label>
        ...
        {error && (
          <div className="text-[10px] px-3 py-2 rounded-lg border" style={{ color: "#ef4444", borderColor: "rgba(239,68,68,0.2)", background: "rgba(239,68,68,0.04)" }}>
            {error}
          </div>
        )}
...
export default function PolicyManager() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5 p-1">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest font-mono" style={{ color: "#8b5cf6" }}>
              Alloy · Policy Manager
            </span>
          </div>
          <h1 className="text-xl font-bold text-white tracking-tight">Policy Manager</h1>
          <p className="text-[11px] mt-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>
            Approval matrices, model routing rules, cost controls, and compliance templates — per tenant.
          </p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
      ...
      {isDemo && (
        <div className="flex items-center gap-2 px-4 py-2 rounded-lg text-[10px] font-medium border"
          style={{ background: "rgba(75,139,219,0.04)", borderColor: "rgba(75,139,219,0.1)", color: "rgba(75,139,219,0.6)" }}>
          <Shield className="w-3 h-3 shrink-0" />
          Demo Mode — Illustrative policies shown. Connect the Alloy API for live tenant policy data.
        </div>
      )}
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: "Open Incidents", value: policyStats.incidents, color: policyStats.incidents > 0 ? "#f59e0b" : "#6b7280" },
        ].map(c => (
      ...
      <div className="flex items-center gap-2">
        {(["policies", "incidents"] as const).map(t => (
          ...
          >
            {t === "incidents" ? `Incidents${openIncidentCount > 0 ? ` (${openIncidentCount})` : ""}` : "Policies"}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-1.5 flex-wrap">
            <button
              ...
              style={{
                borderColor: kindFilter === "all" ? "rgba(139,92,246,0.3)" : "rgba(255,255,255,0.06)",
        ...
        <div className="space-y-2">
          {incidentsAreDemo && (
            <div className="flex items-center gap-2 px-4 py-2 rounded-lg text-[10px] font-medium border"
              ...
              Demo Mode — Illustrative incidents shown. Live incidents populate as governance enforce runs execute.
            </div>
          )}
          {incidents.length === 0 && !incidentsAreDemo && (
            <div className="text-center py-12 text-[11px]" style={{ color: "rgba(255,255,255,0.3)" }}>
              No incidents recorded yet. Incidents appear when governance policies block or flag agent actions.
            </div>
          )}
          {incidents.map(i => (
