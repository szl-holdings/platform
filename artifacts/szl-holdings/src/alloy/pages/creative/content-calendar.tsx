...
import { Calendar, Linkedin, Twitter, Briefcase, Loader2 } from "lucide-react";
...
const STATUSES: CalendarPost["status"][] = ["published", "published", "scheduled", "scheduled", "draft", "draft", "draft", "draft"];

function buildCalendarFromCampaigns(campaigns: { name: string; status: string; deadline?: string; category?: string }[]): CalendarWeek[] {
  ...
  const fallbackThemes = [
    "Launch Announcement", "Product Deep Dives", "Technical Leadership", "Community & Hiring",
    "Customer Stories", "Innovation Spotlight", "Behind the Scenes", "Growth & Vision",
  ];

  return Array.from({ length: 8 }, (_, i) => {
    const theme = activeThemes[i] ?? fallbackThemes[i] ?? `Week ${i + 1} Campaign`;
    const platformSet = PLATFORM_THEMES[i % PLATFORM_THEMES.length] ?? ["linkedin", "x", "linkedin"] as Platform[];
    ...
    return {
      ...
      posts: DAYS.map((day, j): CalendarPost => {
        ...
        return {
          ...
          status: j === 0 && status === "published" ? "published" : j < 2 && ["published", "scheduled"].includes(status) ? "scheduled" : "draft",
...
export default function ContentCalendar() {
  ...
  const weeks: CalendarWeek[] = campaigns && campaigns.length > 0
    ? buildCalendarFromCampaigns(campaigns)
    : [];
  ...
  const scheduled = weeks.reduce((s, w) => s + w.posts.filter(p => p.status === "scheduled").length, 0);
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="space-y-3">
        {weeks.map(w => {
          ...
          return (
            <div key={w.week} className="bg-[#0d1117] border border-white/8 rounded-xl overflow-hidden">
              <button
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  {w.posts.map((p, i) => {
                    ...
                    const colorClass = platformColors[p.platform]?.split(" ")[0] ?? "text-slate-500";
              ...
              {isExpanded && (
                <div className="border-t border-white/8 p-4 space-y-2">
                  {w.posts.map((post, i) => {
                    ...
                    const colorClasses = platformColors[post.platform] ?? "text-slate-500 bg-white/5";
                    return (
