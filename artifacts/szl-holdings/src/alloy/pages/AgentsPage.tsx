...
export default function AgentsPage() {
  ...
  const filtered = filter === "All" ? ALLOY_AGENTS : ALLOY_AGENTS.filter(a => a.category === filter);
          ...
          <div className="grid md:grid-cols-2 gap-3 mb-6">
            {filtered.map(agent => {
              ...
              return (
                <button
                  key={agent.id}
                  onClick={() => { if (!isSelected) trackEvent("agent_card_open", { agent_id: agent.id, agent_name: agent.name, category: agent.category }); setSelected(isSelected ? null : agent.id); }}
                ...
                >
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] px-1.5 py-0.5 rounded font-medium" style={{ background: `${accent}15`, color: accent }}>{agent.category}</span>
                        <div
                          className="flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded"
                          style={{ background: `${ESCALATION_COLORS[agent.escalationMode]}10`, color: ESCALATION_COLORS[agent.escalationMode] }}
                        >
                          <span className="w-1 h-1 rounded-full" style={{ background: ESCALATION_COLORS[agent.escalationMode] }} />
                  ...
                  <div className="flex items-center gap-3 mt-3">
                    <span className={`text-[10px] px-1.5 py-0.5 rounded ${agent.status === "active" ? "text-emerald-400 bg-emerald-400/10" : "text-white/30 bg-white/5"}`}>
                      {agent.status}
                    </span>
    ...
    <div className="sticky top-6 rounded-xl border p-6" style={{ borderColor: `${accent}25`, background: "rgba(255,255,255,0.02)" }}>
      <div className="flex items-start gap-3 mb-5">
        ...
        <div>
          ...
          <span className="text-[11px] px-1.5 py-0.5 rounded font-medium" style={{ background: `${accent}15`, color: accent }}>{agent.category}</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          <div className="text-[10px] uppercase tracking-widest text-white/30 mb-2">Connected Systems</div>
          <div className="flex flex-wrap gap-1.5">
            {agent.systemsConnected.map(s => (
              <span key={s} className="text-[11px] px-2 py-0.5 rounded border" style={{ borderColor: "rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.04)", color: "rgba(255,255,255,0.5)" }}>{s}</span>
            ))}
          </div>
        </div>
        ...
        <div className="pt-4 border-t space-y-2" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
          <div className="flex items-center justify-between text-xs">
            ...
            <span style={{ color: ESCALATION_COLORS[agent.escalationMode] }}>{ESCALATION_LABELS[agent.escalationMode]}</span>
          </div>
          <div className="flex items-center justify-between text-xs">
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <span className={agent.status === "active" ? "text-emerald-400" : "text-white/40"}>{agent.status}</span>
          </div>
