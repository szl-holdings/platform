...
const agentRoles = [
  ...
  { name: "Orchestrator", role: "Sequences multi-step workflows and routes tasks to the right system or person", color: "#005e73" },
  { name: "Output Generator", role: "Produces structured outputs: reports, briefs, API calls, and audit records", color: "#004d60" },
  { name: "Governance Reviewer", role: "Intercepts high-stakes decisions for human approval before execution", color: "#003d4d" },
];

const powers = [
  { name: "Lyte", desc: "Decision intelligence and command. Alloy drives signal ingestion, anomaly classification, and workflow routing.", accent: "#f59e0b" },
  ...
  { name: "Creative Workflows", desc: "Governed creative orchestration. Alloy sequences campaign production — scripts, storyboards, voice assets, and approvals — from brief to final delivery.", accent: "#4B8BDB" },
];

const pipelineSteps = [
  { step: "01", label: "Inputs", desc: "Structured and unstructured data from APIs, databases, operational feeds", icon: Activity },
  { step: "02", label: "Normalization", desc: "Standardized schema — unified regardless of source format or protocol", icon: Layers },
  { step: "03", label: "Reasoning", desc: "Domain-specific classification, consequence scoring, and context enrichment", icon: Network },
  { step: "04", label: "Orchestration", desc: "Multi-step workflow sequencing with conditional logic and dependency graphs", icon: GitBranch },
  { step: "05", label: "Outputs", desc: "Reports, API triggers, task assignments, and automated workflow executions", icon: FileText },
  { step: "06", label: "Governance", desc: "Human checkpoints, approval gates, and full audit trail for every decision", icon: Shield },
];
    ...
    <div style={{ minHeight: "100vh", background: "#080c14", color: "#e2e8f0", fontFamily: "'Space Grotesk', 'Inter', system-ui, sans-serif" }}>
      ...
      <section style={{ paddingTop: "120px", paddingBottom: "80px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: "900px", margin: "0 auto", padding: "0 1.5rem", textAlign: "center", position: "relative", zIndex: 1 }}>
          ...
          <h1 style={{ fontSize: "clamp(2.4rem, 5vw, 3.75rem)", fontWeight: 700, letterSpacing: "-0.032em", lineHeight: 1.05, color: "#f1f5f9", marginBottom: "1.25rem" }}>
            The intelligence layer behind<br />premium command systems.
          </h1>
          <p style={{ fontSize: "1.0625rem", lineHeight: 1.72, color: "#64748b", maxWidth: "36rem", margin: "0 auto 2.5rem" }}>
            Alloy is not a chatbot. It is the orchestration engine that acquires signals, applies reasoning, sequences workflows, and routes actions — across every SZL platform.
          </p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", alignItems: "start" }} className="alloy-grid">
            <div>
              ...
              <p style={{ fontSize: "0.9375rem", lineHeight: 1.72, color: "#64748b" }}>
                Every platform in the SZL ecosystem — Lyte for decision intelligence, Vessels for maritime command, and Creative Workflows for governed campaign production — is powered by Alloy's orchestration layer. The platforms are the interface. Alloy is the engine.
              </p>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              {[
                { label: "Signal Acquisition", desc: "Pulls from operational data sources, APIs, and event streams across every connected platform.", rgb: "0,212,255" },
                { label: "Reasoning Layer", desc: "Applies domain-specific classification and consequence scoring to every signal.", rgb: "0,212,255" },
                { label: "Workflow Orchestration", desc: "Sequences multi-step processes with conditional logic and automatic dependency resolution.", rgb: "0,212,255" },
                { label: "Governance Gates", desc: "Intercepts high-stakes decisions with built-in human approval checkpoints.", rgb: "0,212,255" },
              ].map((item) => (
                <div
                  key={item.label}
                  style={{
                    display: "flex", gap: "12px", padding: "14px 16px", borderRadius: "6px",
      ...
      <section style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "5rem 0", background: "rgba(75,139,219,0.015)" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 1.5rem" }}>
          ...
          <p style={{ fontSize: "0.9375rem", lineHeight: 1.72, color: "#64748b", marginBottom: "2.5rem", maxWidth: "32rem" }}>
            Alloy underpins every intelligent operation within the SZL ecosystem.
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }} className="powers-grid">
            {powers.map((p) => (
              <div
                key={p.name}
                style={{
                  padding: "1.75rem", borderRadius: "6px",
                  background: `${p.accent}06`, border: `1px solid ${p.accent}18`,
              ...
              >
                <p style={{ fontSize: "10px", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: p.accent, marginBottom: "0.75rem" }}>{p.name}</p>
                <p style={{ fontSize: "0.9375rem", lineHeight: 1.65, color: "#94a3b8" }}>{p.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="pipeline" style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "5rem 0" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 1.5rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "12px" }} className="pipeline-grid">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {agentRoles.map((agent, i) => (
              <div
                key={agent.name}
                style={{
                  display: "flex", alignItems: "center", gap: "16px", padding: "14px 18px", borderRadius: "4px",
              ...
              >
                <span style={{ fontSize: "10px", fontFamily: "monospace", color: "#334155", fontWeight: 600, letterSpacing: "0.06em", minWidth: "20px" }}>0{i + 1}</span>
                <div style={{ width: "3px", height: "32px", borderRadius: "2px", background: agent.color, opacity: 0.6, flexShrink: 0 }} />
                <div>
                  <p style={{ fontSize: "13px", fontWeight: 600, color: "#cbd5e1", marginBottom: "2px" }}>{agent.name}</p>
      ...
      <section style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "5rem 0" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 1.5rem" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", alignItems: "start" }} className="outputs-grid">
            <div>
              <p style={{ fontSize: "10px", fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "#475569", marginBottom: "0.75rem" }}>Outputs</p>
              <h2 style={{ fontSize: "clamp(1.75rem, 3vw, 2.25rem)", fontWeight: 700, letterSpacing: "-0.026em", color: "#f1f5f9", lineHeight: 1.08, marginBottom: "1rem" }}>
                Structured decisions.<br />Not raw data.
              </h2>
              <p style={{ fontSize: "0.9375rem", lineHeight: 1.72, color: "#64748b" }}>
                Every Alloy workflow produces a clean, traceable output — structured enough to act on, explainable enough to audit, and precise enough to route automatically.
              </p>
            </div>
      ...
      <section id="governance" style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "5rem 0", background: "rgba(255,255,255,0.01)" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 1.5rem" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", alignItems: "center" }} className="gov-grid">
            <div>
              <p style={{ fontSize: "10px", fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "#475569", marginBottom: "0.75rem" }}>Governance</p>
              <h2 style={{ fontSize: "clamp(1.75rem, 3vw, 2.25rem)", fontWeight: 700, letterSpacing: "-0.026em", color: "#f1f5f9", lineHeight: 1.08, marginBottom: "1rem" }}>
                Humans stay in the loop.
              </h2>
              <p style={{ fontSize: "0.9375rem", lineHeight: 1.72, color: "#64748b" }}>
                Alloy's governance layer is not a bolt-on. It's built into every workflow. High-consequence decisions pause for human review. Every approval is logged. Every action is attributable. The audit trail is immutable.
              </p>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {[
                { label: "Approval checkpoints", desc: "Configurable governance gates on any workflow step" },
                { label: "Rationale recording", desc: "Every decision includes the reasoning chain that produced it" },
                { label: "Immutable audit trail", desc: "Full history of actions, approvals, and overrides" },
      ...
      <section style={{ borderTop: "1px solid rgba(255,255,255,0.05)", padding: "5rem 0" }}>
        <div style={{ maxWidth: "600px", margin: "0 auto", padding: "0 1.5rem", textAlign: "center" }}>
          ...
          <p style={{ fontSize: "0.9375rem", lineHeight: 1.65, color: "#64748b", marginBottom: "2rem" }}>
            Alloy is available to authenticated SZL Holdings operators. Sign in to access the platform, or request a demonstration.
          </p>
          <div style={{ display: "flex", gap: "12px", justifyContent: "center", flexWrap: "wrap" }}>
            <button
              onClick={onSignIn}
