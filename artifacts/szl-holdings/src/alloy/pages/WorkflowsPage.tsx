          ...
          <div className="space-y-2">
            {ALLOY_WORKFLOWS.map(workflow => {
              ...
              return (
                <button
                  key={workflow.id}
                  onClick={() => { if (!isSelected) trackEvent("workflow_click", { workflow_id: workflow.id, workflow_name: workflow.name, category: workflow.category }); setSelected(isSelected ? null : workflow.id); }}
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-white/40 leading-relaxed line-clamp-2">{workflow.description}</div>
                      <div className="flex items-center gap-3 mt-2">
                        <span className="text-[10px] font-medium px-1.5 py-0.5 rounded" style={{ background: `${accent}15`, color: accent }}>{workflow.category}</span>
                        <span className="text-[10px] text-white/30">{workflow.steps.length} steps</span>
                        {workflow.approvalPoints > 0 && (
                          <span className="text-[10px] text-white/30">{workflow.approvalPoints} approval{workflow.approvalPoints > 1 ? "s" : ""}</span>
                        )}
                      </div>
                    </div>
                  </div>
        ...
        <div className="flex-1">
          {selectedWorkflow ? (
            ...
            <div className="rounded-xl border p-10 text-center" style={{ borderColor: "rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.02)" }}>
              ...
              <div className="text-white/50 text-sm">Select a workflow to see its steps, agents, and outputs in detail.</div>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="rounded-xl border p-6 md:p-8" style={{ borderColor: `${accent}25`, background: "rgba(255,255,255,0.02)" }}>
      <div className="flex items-start gap-4 mb-6">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            <span className="text-xs px-2 py-0.5 rounded font-medium" style={{ background: `${accent}15`, color: accent }}>{workflow.category}</span>
            <span className="text-xs px-2 py-0.5 rounded border text-white/40" style={{ borderColor: "rgba(255,255,255,0.1)" }}>~{workflow.estimatedDuration}</span>
            {workflow.approvalPoints > 0 && (
              <span className="text-xs px-2 py-0.5 rounded border text-amber-400" style={{ borderColor: "rgba(245,158,11,0.3)", background: "rgba(245,158,11,0.08)" }}>
                {workflow.approvalPoints} human approval point{workflow.approvalPoints > 1 ? "s" : ""}
              </span>
            )}
          </div>
        </div>
      ...
      <div className="mb-6">
        ...
        <div className="text-sm text-white/65 p-3 rounded-lg border" style={{ borderColor: "rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.03)" }}>
          {workflow.trigger}
        </div>
      </div>
            ...
            <div key={step.step} className="flex items-start gap-3">
              ...
              <div className="flex-1 pb-2">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  {step.approvalPoint && (
                    <span className="text-[10px] font-medium px-1.5 py-0.5 rounded" style={{ background: "rgba(245,158,11,0.12)", color: "#f59e0b" }}>Approval Required</span>
                  )}
                </div>
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          {workflow.outputs.map(o => (
            <span key={o} className="text-xs px-2.5 py-1 rounded-lg border" style={{ borderColor: "rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.04)", color: "rgba(255,255,255,0.65)" }}>{o}</span>
          ))}
        </div>
      </div>
      ...
      <div className="border-t pt-5 mt-5" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => onNavigate("agents")} className="inline-flex items-center gap-1.5 text-xs font-medium" style={{ color: accent }}>
