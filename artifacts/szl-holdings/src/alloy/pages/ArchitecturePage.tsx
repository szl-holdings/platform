...
const LAYERS = [
  ...
  },
  {
    ...
    description: "Transforms raw inputs into consistent, structured data objects. Applies classification tags, extracts entities, and prepares data for downstream reasoning.",
    components: ["Schema normalisation", "Classification engine", "Entity extraction", "Priority scoring"],
  ...
  },
  {
    ...
    description: "Applies AI reasoning, anomaly detection, and knowledge base retrieval to produce structured insights with confidence scores and explainable rationale.",
    components: ["AI reasoning engine", "Knowledge base retrieval", "Anomaly detection", "Confidence scoring"],
  ...
  },
  {
    ...
    description: "Coordinates multi-agent workflows, manages sequencing and handoffs between agents, and ensures correct execution of complex compound tasks.",
    components: ["Workflow coordinator", "Agent sequencing", "Handoff management", "State tracking"],
  ...
  },
  {
    ...
    number: 5,
    name: "Output Layer",
    description: "Produces structured, human-readable outputs — summaries, alerts, documents, proposals, and action queues — from fully reasoned workflow results.",
    components: ["Document generation", "Summary synthesis", "Alert formatting", "Action queue management"],
  ...
  },
  {
    ...
    description: "Wraps the entire pipeline in accountability controls — human approval flows, audit trails, confidence gates, escalation logic, and role-based access.",
...
const PRODUCT_INTEGRATIONS = [
  ...
  },
  {
    ...
    icon: "🚢",
    accent: "#3b82f6",
    description: "Vessels provides AIS signal data, fleet status, and voyage information. Alloy interprets signals, detects deviations, and generates fleet intelligence briefings.",
    dataIn: ["AIS position signals", "Vessel status", "Voyage data", "Weather conditions"],
    dataOut: ["Fleet intelligence briefs", "Deviation alerts", "Voyage summaries", "Operator notifications"],
  },
  {
    name: "Carlota Jo",
    icon: "✨",
    accent: "#f472b6",
    description: "Carlota Jo passes client requests, engagement data, and document requirements to Alloy. Alloy generates documents, manages approval flows, and routes operational tasks.",
              ...
              <div className="p-5 md:p-7">
                <div className="md:flex md:items-start md:gap-8">
                  <div className="flex items-center gap-4 mb-4 md:mb-0 md:w-72 shrink-0">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-0.5">
                        <span className="text-xs font-mono" style={{ color: layer.accent }}>L{layer.number}</span>
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-white/55 leading-relaxed mb-4">{layer.description}</p>
                    <div className="flex flex-wrap gap-4">
                      <div>
                        <div className="text-[10px] uppercase tracking-widest text-white/30 mb-2">Components</div>
                        <div className="flex flex-wrap gap-1.5">
                          {layer.components.map(c => (
                            <span key={c} className="text-[11px] px-2 py-0.5 rounded border" style={{ borderColor: `${layer.accent}25`, background: `${layer.accent}08`, color: `${layer.accent}99` }}>{c}</span>
                          ))}
                        </div>
                      </div>
                      <div>
                        <div className="text-[10px] uppercase tracking-widest text-white/30 mb-2">Connected To</div>
                        <div className="flex flex-wrap gap-1.5">
                          {layer.products.map(p => (
                            <span key={p} className="text-[11px] px-2 py-0.5 rounded border" style={{ borderColor: "rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.04)", color: "rgba(255,255,255,0.5)" }}>{p}</span>
                          ))}
                        </div>
                      </div>
                    </div>
      ...
      <div className="mb-20">
        ...
        <div className="space-y-4">
          {PRODUCT_INTEGRATIONS.map(prod => (
            <div key={prod.name} className="rounded-xl border p-6 md:p-8" style={{ borderColor: "rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.02)" }}>
              <div className="md:flex md:gap-8">
                <div className="md:w-64 shrink-0 mb-5 md:mb-0">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 rounded-lg flex items-center justify-center text-xl" style={{ background: `${prod.accent}15`, border: `1px solid ${prod.accent}30` }}>{prod.icon}</div>
                    <div>
                      <div className="text-sm font-bold">{prod.name}</div>
                      <div className="text-[11px] px-1.5 py-0.5 rounded inline-flex mt-0.5 font-medium" style={{ background: `${prod.accent}15`, color: prod.accent }}>Powered by Alloy</div>
                    </div>
                  </div>
                ...
                <div className="flex-1 grid md:grid-cols-2 gap-5">
                  <div>
                    <div className="text-[10px] uppercase tracking-widest text-white/30 mb-2.5">Data Into Alloy</div>
                    <div className="space-y-1.5">
                      {prod.dataIn.map(d => (
                        <div key={d} className="flex items-center gap-2 text-xs text-white/55">
                          <div className="w-1 h-1 rounded-full shrink-0" style={{ background: prod.accent }} />
                          {d}
                        </div>
                      ))}
                    </div>
                  ...
                  <div>
                    <div className="text-[10px] uppercase tracking-widest text-white/30 mb-2.5">Alloy Outputs Back</div>
                    <div className="space-y-1.5">
                      {prod.dataOut.map(d => (
                        <div key={d} className="flex items-center gap-2 text-xs text-white/55">
