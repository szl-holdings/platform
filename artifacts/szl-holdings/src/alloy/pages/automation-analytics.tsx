      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          <div key={c.label} className="rounded-xl border p-4" style={{ borderColor: "rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)" }}>
            ...
            <div className="text-[9px]" style={{ color: "#10b981" }}>{c.trend} vs last week</div>
          </div>
        ))}
      </div>
        ...
        <div className="rounded-xl border p-5" style={{ borderColor: "rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)" }}>
          ...
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={VOLUME_DATA} margin={{ top: 0, right: 0, left: -20, bottom: 0 }} barSize={10}>
              ...
              <div key={i}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-[10px]" style={{ color: "rgba(255,255,255,0.4)" }}>{wf.runs} runs</span>
                    <span className="text-[10px] font-medium" style={{ color: wf.success_rate >= 90 ? "#10b981" : "#f59e0b" }}>{wf.success_rate}%</span>
                  </div>
                </div>
                <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
                  <div className="h-1.5 rounded-full" style={{ width: `${wf.success_rate}%`, background: wf.success_rate >= 90 ? "#10b981" : "#f59e0b" }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="rounded-xl border p-5" style={{ borderColor: "rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.02)" }}>
        ...
        <div className="grid grid-cols-4 gap-3">
          {CONNECTORS_UI.slice(0, 8).map(c => (
            <div key={c.id} className="rounded-lg p-3" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-medium text-white truncate max-w-[80px]">{c.name.split(" ")[0]}</span>
                <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: c.health === "healthy" ? "#10b981" : c.health === "degraded" ? "#f59e0b" : "#ef4444" }} />
              </div>
              <div className="grid grid-cols-3 gap-1 text-center">
                ...
                <div>
                  <div className="text-xs font-bold" style={{ color: c.error_rate > 5 ? "#ef4444" : "#10b981" }}>{c.error_rate}%</div>
                  <div className="text-[8px]" style={{ color: "rgba(255,255,255,0.3)" }}>err</div>
                </div>
                <div>
                  <div className="text-xs font-bold" style={{ color: c.latency_ms > 500 ? "#f59e0b" : "#10b981" }}>{c.latency_ms}ms</div>
                  <div className="text-[8px]" style={{ color: "rgba(255,255,255,0.3)" }}>lat</div>
                </div>
              </div>
            </div>
          ))}
