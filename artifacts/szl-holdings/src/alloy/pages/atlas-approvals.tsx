...
import {
  ATLAS_APPROVALS_BADGE_QUERY_KEY,
  useAtlasApprovalsBadge,
} from '@/alloy/hooks/use-atlas-approvals-badge';
...
const DEMO_APPROVALS: ScenarioRecommendation[] = [
  {
    ...
    numericId: -1,
    scenarioId: 'SIM-0847',
    scenarioName: 'APT29 Ransomware Campaign',
    branch: 'Branch A — Immediate Isolation',
    domain: 'aegis',
    recommendation:
      'Approve immediate host isolation and Kerberos ticket revocation for WKSTN-FIN-042',
    rationale:
      'Scenario Forge modeling indicates 34% probability of containment with blast radius limited to 3 assets if action is taken within 15 minutes of this approval.',
    riskLevel: 'critical',
    blastRadius: 12,
    costImpact: '$240K',
    mttr: '1h 22m',
    actions: [
      'Isolate WKSTN-FIN-042 via EDR API',
      'Revoke krbtgt — Kerberos forest-wide reissue',
      'Page SOC Tier-2 (SLA: 5m)',
      'Enable enhanced DC logging',
    ],
    requestedBy: 'ATLAS Spatial Runtime',
    requestedAt: '2m ago',
    approvalStatus: 'pending',
    requiredApprover: 'CISO / SOC Lead',
    evidenceLinks: [
      'INC-2024-0847 incident thread',
      'Posture twin drift report',
      'Replay Engine: lateral movement path',
    ],
  },
  {
    id: 'apr-002',
    numericId: -2,
    scenarioId: 'SIM-0831',
    scenarioName: 'Cloud Privilege Escalation',
    branch: 'Branch C — Targeted Containment',
    domain: 'aegis',
    recommendation: 'Authorize AWS IAM policy rollback and temporary cross-account role suspension',
    rationale:
      'IAM drift detected across 3 SGs in AWS VPC. Approved rollback will reduce exposure surface by 87% with minimal service disruption.',
    riskLevel: 'high',
    blastRadius: 19,
    costImpact: '$180K',
    mttr: '0h 55m',
    actions: [
      'Roll back IAM policy to version 14',
      'Suspend temp cross-account role chain',
    ...
    evidenceLinks: ['AWS VPC twin drift report', 'IAM policy delta log'],
  },
  {
    id: 'apr-003',
    numericId: -3,
    scenarioId: 'SIM-0819',
    scenarioName: 'OT/ICS PLC Anomaly',
    branch: 'Branch B — Supervised Patch',
    domain: 'aegis',
    recommendation: 'Approve supervised PLC firmware reconciliation during next maintenance window',
    rationale:
      'PLC ladder logic delta requires firmware patch. Supervised patch during maintenance minimizes production impact. Dwell risk accepted pending approval.',
    riskLevel: 'high',
    blastRadius: 8,
    costImpact: '$95K',
    mttr: '2h 30m',
    actions: [
      'Schedule maintenance window T+4h',
      'Patch PLC FW under engineer supervision',
      'Validate post-patch telemetry',
      'Re-baseline OT twin',
    ],
    requestedBy: 'ATLAS OT Runtime',
    requestedAt: '15m ago',
    approvalStatus: 'escalated',
    requiredApprover: 'OT Security Lead + CISO',
    evidenceLinks: ['OT twin anomaly report', 'ICS protocol deviation log'],
  },
  {
    id: 'apr-004',
    numericId: -4,
    scenarioId: 'SIM-0801',
    scenarioName: 'K8s App Tier Drift',
    branch: 'Branch A — Immediate Remediation',
    domain: 'aegis',
    recommendation: 'Approve pod security context enforcement and CVE-2024-3890 emergency patch',
    rationale:
      '6 containers drifted from approved security baseline. CVE-2024-3890 is remotely exploitable. Immediate patch reduces CVSS 9.1 exposure.',
    riskLevel: 'critical',
    blastRadius: 24,
    costImpact: '$320K',
    mttr: '1h 45m',
    actions: [
      'Enforce pod security context via admission controller',
      'Emergency patch CVE-2024-3890',
    ...
    evidenceLinks: ['K8s twin drift analysis', 'CVE advisory CVE-2024-3890'],
  },
];
...
function ApprovalCard({
  ...
  return (
    <div
      ...
      style={{
        borderColor: rec.approvalStatus === 'pending' ? 'rgba(245,158,11,0.15)' : s.border,
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[10px]" style={{ color: 'rgba(255,255,255,0.4)' }}>
              <span style={{ color: 'rgba(75,139,219,0.8)' }}>{rec.scenarioName}</span> ·{' '}
              {rec.branch}
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-1.5 text-right">
              ...
              >
                ...
                <div
                  ...
                  style={{ color: rec.blastRadius > 20 ? '#f59e0b' : '#10b981' }}
                >
                  {rec.blastRadius}%
                </div>
              </div>
        ...
        <div className="border-t px-4 pb-4" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
          <div className="pt-3 space-y-3">
            ...
            >
              ...
              <div className="text-[10px] leading-snug" style={{ color: 'rgba(255,255,255,0.65)' }}>
                {rec.rationale}
              </div>
            </div>
            <div>
              ...
              {rec.actions.map((a, i) => (
                <div
                  key={i}
                  className="flex items-start gap-1.5 text-[10px] mb-1"
                  style={{ color: 'rgba(255,255,255,0.6)' }}
                >
                  <ArrowRight
            ...
            {rec.approvalStatus !== 'pending' && (
              ...
              >
                {rec.approvalStatus === 'approved' ? (
                  ...
                  <AlertTriangle className="w-3 h-3" style={{ color: '#8b7ac8' }} />
                )}
...
export default function AlloyAtlasApprovals() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useStandardQuery<ApiApproval[]>({
    queryKey: ['atlas-approvals'],
    queryFn: async () => {
      const result = await apiRequest<ApiApproval[] | { data: ApiApproval[] }>(
        'GET',
    ...
    try {
      if (rec.numericId > 0) {
        await escalateMutation.mutateAsync({
          ...
          reason: 'Escalated from ATLAS Approvals queue',
        });
      } else {
  ...
  const resolved = approvals.filter((a) =>
    ['approved', 'rejected', 'revised', 'expired', 'withdrawn'].includes(a.approvalStatus),
  );
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>
            Review and approve ATLAS Scenario Forge recommendations before they enter the Alloy
      ...
      {isLiveData && (
        ...
        >
          ...
          <span className="text-[10px]" style={{ color: 'rgba(255,255,255,0.5)' }}>
            Showing{' '}
            <span className="font-bold font-mono" style={{ color: '#4B8BDB' }}>
              {approvals.length}
            </span>{' '}
            live approval{approvals.length !== 1 ? 's' : ''} from the governance workflow queue —
            approve, reject, or escalate to trigger Alloy execution handoff
          </span>
        </div>
      )}
