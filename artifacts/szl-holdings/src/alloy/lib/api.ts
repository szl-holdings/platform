...
export const alloyApi = {
  ...
  workflows: {
    list: () =>
      apiFetch<{ data: AlloyWorkflow[] } | AlloyWorkflow[]>('/alloy/workflows').then(unwrapList),
    get: (id: number) => apiFetch<AlloyWorkflow>(`/alloy/workflows/${id}`),
    create: (data: Partial<AlloyWorkflow>) =>
      apiFetch<AlloyWorkflow>('/alloy/workflows', { method: 'POST', body: JSON.stringify(data) }),
    update: (id: number, data: Partial<AlloyWorkflow>) =>
      apiFetch<AlloyWorkflow>(`/alloy/workflows/${id}`, {
        method: 'PATCH',
        body: JSON.stringify(data),
      }),
    delete: (id: number) =>
      apiFetch<{ deleted: boolean }>(`/alloy/workflows/${id}`, { method: 'DELETE' }),
    ...
    runs: (id: number) =>
      apiFetch<{ data: AlloyWorkflowRun[] } | AlloyWorkflowRun[]>(
        `/alloy/runs?workflowId=${id}`,
      ).then(unwrapList),
  },
  ...
  signals: {
    list: (params?: { status?: string; severity?: string }) => {
      ...
      const qs = q.toString();
      return apiFetch<{ data: AlloySignal[] } | AlloySignal[]>(
        `/alloy/signals${qs ? `?${qs}` : ''}`,
      ).then(unwrapList);
    },
    ingest: (signal: {
      ...
      apiFetch<{ signal: AlloySignal; triggeredWorkflows: number }>('/alloy/ingest/signal', {
        method: 'POST',
        body: JSON.stringify(signal),
      }),
    batchIngest: (signals: unknown[]) =>
      apiFetch<{ processed: number; failed: number; signals: AlloySignal[] }>(
        '/alloy/ingest/batch',
  ...
  runs: {
    list: (params?: { state?: string; workflowId?: number }) => {
      ...
      const qs = q.toString();
      return apiFetch<{ data: AlloyWorkflowRun[] } | AlloyWorkflowRun[]>(
        `/alloy/runs${qs ? `?${qs}` : ''}`,
      ).then(unwrapList);
    },
    get: (id: number) => apiFetch<AlloyWorkflowRun>(`/alloy/runs/${id}`),
    retry: (id: number) =>
      apiFetch<AlloyWorkflowRun>(`/alloy/runs/${id}/retry`, { method: 'POST' }),
    cancel: (id: number) =>
      apiFetch<AlloyWorkflowRun>(`/alloy/runs/${id}/cancel`, { method: 'POST' }),
  },
  ...
  artifacts: {
    list: (params?: { status?: string }) => {
      ...
      if (params?.status) q.set('status', params.status);
      const qs = q.toString();
      return apiFetch<{ data: AlloyArtifact[] } | AlloyArtifact[]>(
        `/alloy/artifacts${qs ? `?${qs}` : ''}`,
      ).then(unwrapList);
    },
    get: (id: number) => apiFetch<AlloyArtifact>(`/alloy/artifacts/${id}`),
    approve: (id: number, notes?: string) =>
      apiFetch<AlloyArtifact>(`/alloy/artifacts/${id}/approve`, {
        method: 'POST',
        body: JSON.stringify({ notes }),
      }),
    reject: (id: number, notes: string) =>
      apiFetch<AlloyArtifact>(`/alloy/artifacts/${id}/reject`, {
        method: 'POST',
        body: JSON.stringify({ notes }),
      }),
  },
  ...
  featureFlags: {
    ...
    update: (key: string, data: { isEnabled?: boolean; rolloutPercentage?: number }) =>
      apiFetch<FeatureFlag>(`/alloy/admin/flags/${key}`, {
        method: 'PATCH',
        body: JSON.stringify(data),
      }),
  },
  ...
  audit: {
    list: (params?: { resourceType?: string; action?: string; limit?: number }) => {
      ...
      const qs = q.toString();
      return apiFetch<{ data: unknown[] } | unknown[]>(`/alloy/audit${qs ? `?${qs}` : ''}`).then(
        unwrapList,
      );
    },
  },
