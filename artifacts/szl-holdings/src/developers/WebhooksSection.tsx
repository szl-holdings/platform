  ...
  return (
    <section>
      ...
      <p style={{ color: 'hsl(214,8%,64%)', lineHeight: '1.7', marginBottom: '1rem' }}>
        Webhooks are configured per-integration through <InlineCode>POST /api/webhooks</InlineCode>.
        Each webhook target is associated with a list of event types, a secret for signature
        verification, and an active/inactive status.
      </p>
...
# Response
{
  ...
  "createdAt": "2026-04-01T00:00:00.000Z"
      ...
      <p style={{ color: 'hsl(214,8%,64%)', lineHeight: '1.7', marginBottom: '1rem' }}>
        Every webhook delivery includes an <InlineCode>X-SZL-Signature-256</InlineCode> header
        containing an HMAC-SHA256 signature of the raw request body, keyed with your webhook secret.
          ...
          <tbody>
            {WEBHOOK_EVENTS.map(({ event, description }, i) => (
              <tr
                ...
                style={{
                  borderBottom:
                    i < WEBHOOK_EVENTS.length - 1 ? '1px solid hsla(0,0%,100%,0.04)' : 'none',
              ...
              >
                ...
                <td className="px-4 py-3" style={{ color: 'hsl(214,8%,64%)' }}>
                  {description}
                </td>
              </tr>
            ))}
