...
export function OverviewSection({ scrollTo }: { scrollTo: (id: string) => void }) {
  return (
    <section>
      ...
      <Callout type="info">
        All API endpoints are mounted under the <InlineCode>/api</InlineCode> prefix. The base path
        for the Replit-hosted environment is <InlineCode>/api</InlineCode>. Self-hosted deployments
        should configure <InlineCode>CORS_ORIGINS</InlineCode> to allow your integration's origin.
      </Callout>
    </section>
  );
}
