  ...
  return (
    <section>
      ...
      <p style={{ color: 'hsl(214,8%,64%)', lineHeight: '1.7', marginBottom: '1rem' }}>
        The GraphQL endpoint at <InlineCode>/api/graphql</InlineCode> exposes the same data as the
        REST API through a typed schema. Authentication works identically — include a Bearer token
        in the
      ...
      <p style={{ color: 'hsl(214,8%,64%)', lineHeight: '1.7', marginBottom: '1.25rem' }}>
        Use the embedded playground to explore the schema, run queries, and test mutations
        interactively. Set your Bearer token in the HTTP Headers panel as{' '}
      ...
      >
        ...
        <div className="px-5 py-5" style={{ background: 'hsla(214,14%,5%,0.95)' }}>
          ...
          >
            The GraphQL playground provides schema documentation, query autocompletion, and live
            execution. Set your Bearer token in the HTTP Headers panel to access protected types and
            fields.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
            {[
              ...
              >
                ...
                <p style={{ fontSize: '0.75rem', color: 'hsl(214,8%,54%)', lineHeight: '1.55' }}>
                  {desc}
                </p>
              </div>
            ))}
      ...
      <CodeBlock filename="ingest-signal.graphql" language="graphql" code={GQL_MUTATION_SIGNAL} />
    </section>
  );
}
