...
import { fmt } from "./api";
import { PORTFOLIO_COMPANIES, derivePeriodDates } from "./api";
...
export function MetricCard({ label, value, sub, icon: Icon, color, trend }: {
  label: string; value: string; sub?: string; icon: ElementType; color: string; trend?: "up" | "down" | "neutral";
}) {
  return (
...
export function DataEntryModal({ open, onClose, onSubmit, title }: {
            ...
            <select value={form.periodValue} onChange={e => setForm(f => ({ ...f, periodValue: e.target.value }))} className={inputCls}>
              {form.periodType === "monthly"
                ? Array.from({ length: 12 }, (_, i) => {
                    ...
                    return <option key={i + 1} value={String(i + 1)}>{mo}</option>;
                  })
