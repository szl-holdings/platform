...
const DOMAIN_CONFIG: Record<string, { color: string; icon: typeof Shield; shortName: string }> = {
  ...
  Prism: { color: 'hsl(260,60%,65%)', icon: Briefcase, shortName: 'Prism' },
  ...
  return Object.entries(DOMAIN_CONFIG)
    .map(([domain, cfg]) => {
      ...
      return {
        ...
        avgConfidence:
          inDomain.reduce((a, s) => a + s.confidence, 0) / Math.max(1, inDomain.length),
      };
    })
...
function CorrelationTypeBadge({ type }: { type?: string }) {
  const colors: Record<string, string> = {
    causal: 'hsl(0,72%,54%)',
    temporal: 'hsl(48,90%,52%)',
    semantic: 'hsl(260,60%,65%)',
  };
...
const SIG_PROOF_RECORDS: Record<string, ProofRecord> = {
  sf1: {
    ...
    chainLinks: [
      {
        id: 'c1',
        event: 'Signal ingested — KEV CVE-2025-1337 active exploitation',
        actor: 'System / PRAXIS Bus',
        timestamp: '16 Apr 2026 08:14:22',
        hash: 'sha256:a3f7b2c1d...',
      },
      {
        id: 'c2',
        event: 'Correlated with IMPERIUM drift event (sf6) and Vessels anomaly (sf4)',
      ...
      {
        id: 'c3',
        event: 'AI recommendation generated — isolate affected hosts',
  ...
  sf2: {
    ...
    policyChecks: [
      ...
      {
        label: 'OFAC screening required before execution',
        passed: false,
        note: 'Screening in progress',
      },
    ],
    chainLinks: [
      {
        id: 'c1',
        event: 'AIS dark gap detected — MV Adriatic Star (6h20m)',
        actor: 'System / Vessels Feed',
        timestamp: '16 Apr 2026 07:07:44',
        hash: 'sha256:d1e3f5b7a...',
      },
      {
        id: 'c2',
        event: 'Temporal correlation with Counsel filing event (sf5)',
        actor: 'System / Signal Fusion',
        timestamp: '16 Apr 2026 08:03:11',
        hash: 'sha256:f2a8c4e6b...',
      },
    ],
    metadata: {
      'Signal ID': 'SIG-20260416-002',
      'Last AIS fix': '37.42N, 15.61E',
      'Gap duration': '6h20m',
      'OFAC status': 'Pending',
    },
...
export default function SignalFusionPage() {
  const __pageMeta = usePageMeta({
    ...
    description:
      'Cross-domain signal aggregation showing where signals originate, how they correlate, and confidence levels. Inspired by Five Eyes STONE GHOST and NSA signal fusion architectures.',
  ...
  const fusionQuery = useStandardQuery<{
  ...
  const CORRELATION_PAIRS: CorrelationPair[] =
    fusionQuery.data?.correlations ?? CORRELATION_PAIRS_FALLBACK;
  ...
  const filteredSignals = FUSION_SIGNALS.filter(
    (s) =>
  ...
  const correlatingTo = FUSION_SIGNALS.filter(
    (s) => s.correlatedWith.includes(sig.id) && !sig.correlatedWith.includes(s.id),
  );
  ...
  const avgConfidence =
    FUSION_SIGNALS.length > 0
      ? FUSION_SIGNALS.reduce((a, s) => a + s.confidence, 0) / FUSION_SIGNALS.length
      : 0;
      ...
      <div style={{ minHeight: '100vh', background: BG, color: TEXT }}>
        ...
        <main id="main-content">
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  Cross-domain signal aggregation showing where signals originate, how they
                  correlate across domains, and what confidence the system places in each one.
                  Inspired by Five Eyes STONE GHOST and NSA's multi-source correlation architecture.
                </p>
              </m.div>
              ...
              >
                {[
                  ...
                  {
                    label: 'Correlated',
                    value: `${correlatedCount}/${FUSION_SIGNALS.length}`,
                    color: LYTE,
                  },
                  {
                  ...
                  },
                  {
                    ...
                    value: DOMAIN_STATS.length.toString(),
                    color: 'hsl(260,60%,65%)',
                  },
                ].map((stat, i) => (
                  <div
          ...
          <section style={{ borderBottom: `1px solid ${BORDER}`, padding: '2rem 0' }}>
            <div
              style={{ maxWidth: '1440px', margin: '0 auto', padding: '0 var(--space-content-x)' }}
            >
              {/* Filters */}
              <div
                style={{
                    ...
                    {filteredSignals
                      ...
                      .map((s) => (
                        <SignalCard
                          ...
                          highlight={
                            sig.correlatedWith.includes(s.id) || s.correlatedWith.includes(sig.id)
                          }
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    <div style={{ background: BG, padding: '1.125rem' }}>
                      ...
                      >
                        Signal ID: {sig.id.toUpperCase()} · {sig.sourceType.replace(/_/g, ' ')} ·{' '}
                        {sig.timestamp}
                      </p>
                    </div>
                        ...
                        <div style={{ background: BG, padding: '1rem 1.125rem' }}>
                          ...
                          {Object.entries(sig.metadata).map(([k, v]) => (
                            ...
                            >
                              ...
                              <span
                                style={{ fontSize: '0.6875rem', color: TEXT_SEC, fontWeight: 600 }}
                              >
                                {v}
                              </span>
                            </div>
                          ...
                          <div style={{ background: BG, padding: '1rem 1.125rem' }}>
                            ...
                            >
                              {allCorrelated.map((cs) => {
                                ...
                                const pair = CORRELATION_PAIRS.find(
                                  (p) =>
                                    (p.from === sig.id && p.to === cs.id) ||
                                    (p.from === cs.id && p.to === sig.id),
                                );
                                return (
                                  <button
                                    key={cs.id}
                                  ...
                                  >
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                      ...
                                      {pair?.label && (
                                        ...
                                        >
                                          {pair.label} · {(pair.strength * 100).toFixed(0)}%
                                          strength
                                        </p>
                                      )}
                                    </div>
                    ...
                    {detailTab === 'proof' && (
                      <div
                        style={{ background: BG, padding: '1rem 1.125rem' }}
                        data-testid={`proof-tab-content-${sig.id}`}
                      >
                        <ProofDrawer
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                          ...
                          <button
                            ...
                            style={{
                              ...
                              borderRadius: 5,
                              background: acknowledged.has(sig.id)
                                ? 'hsla(142,60%,50%,0.12)'
                                : 'transparent',
                              border: `1px solid ${acknowledged.has(sig.id) ? 'hsla(142,60%,50%,0.3)' : BORDER}`,
                              cursor: 'pointer',
                              fontSize: '0.75rem',
                              fontWeight: 600,
                              color: acknowledged.has(sig.id) ? 'hsl(142,60%,50%)' : TEXT_SEC,
                              textAlign: 'left',
                              display: 'flex',
                              alignItems: 'center',
                              gap: '0.5rem',
                          ...
                          <button
                            ...
                            style={{
                              ...
                              padding: '0.5rem 0.875rem',
                              borderRadius: 5,
                              background: escalated.has(sig.id)
                                ? 'hsla(30,90%,52%,0.12)'
                                : 'transparent',
                              border: `1px solid ${escalated.has(sig.id) ? 'hsla(30,90%,52%,0.3)' : BORDER}`,
                              cursor: 'pointer',
                              fontSize: '0.75rem',
                              fontWeight: 600,
                              color: escalated.has(sig.id) ? 'hsl(30,90%,52%)' : TEXT_SEC,
                              textAlign: 'left',
                              display: 'flex',
                              alignItems: 'center',
                              gap: '0.5rem',
          ...
          {/* Domain coverage */}
          <section
            style={{ borderBottom: `1px solid ${BORDER}`, padding: 'clamp(3rem,6vw,4rem) 0' }}
          >
            <div
              style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 var(--space-content-x)' }}
            >
            ...
            >
              ...
              >
                ...
                <p style={{ fontSize: '0.875rem', color: TEXT_SEC, margin: 0 }}>
                  Signals correlated across domain boundaries — with correlation type and strength
                  score.
                </p>
              </m.div>
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                        ...
                        <div>
                          ...
                          >
                            {fromSig.title.slice(0, 45)}
                            {fromSig.title.length > 45 ? '…' : ''}
                          </p>
                        </div>
                      </div>
                      ...
                      >
                        <span
                          style={{
                            ...
                            padding: '2px 6px',
                            borderRadius: 3,
                            background: `${pairColor}15`,
                            border: `1px solid ${pairColor}25`,
                          }}
                        >
                          {pair.type}
                        </span>
                      ...
                      >
                        <div style={{ textAlign: 'right' }}>
                          ...
                          >
                            {toSig.title.slice(0, 45)}
                            {toSig.title.length > 45 ? '…' : ''}
                          </p>
                        </div>
                        <div
                          style={{
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    The Signal Fusion Panel draws from STONE GHOST's multinational fusion database,
                    NSA's signals correlation architecture, and Russia's SORM-generation ingestion
                    completeness model — and adapts these patterns for governed enterprise signal
                    management.
                  </p>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                  {[
                    {
                      src: 'Five Eyes STONE GHOST',
                      map: 'Multinational fusion DB → Cross-domain PRAXIS Bus signal correlation',
                    },
                    {
                      src: 'NSA SIGINT architecture',
                      map: 'Multi-source correlation → Confidence-weighted signal scoring',
                    },
                    {
                      src: 'Russia SORM-III',
                      map: 'Signal coverage breadth by source → Domain coverage completeness per pack',
                    },
                    {
                      src: 'Anduril Lattice ingestion',
                      map: 'Sensor fusion layer → Normalized domain signal schema',
                    },
                  ].map((item, i) => (
                    <m.div
                      key={i}
