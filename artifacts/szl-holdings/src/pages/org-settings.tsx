...
import {
  ...
  Loader2, AlertCircle, CheckCircle2, ArrowLeft, Shield, User,
  Mail, Phone, MessageSquare, Globe, Settings,
} from "lucide-react";
...
type OrgProfile = {
  ...
  logoUrl: string | null; orgType: string | null; plan: string; status: string;
};

type Member = {
  memberId: number; userId: number; role: string; joinedAt: string;
  displayName: string; email: string | null; avatarUrl: string | null;
  isActive: boolean; lastLoginAt: string | null;
};

type NotifPrefs = {
  emailEnabled: boolean; smsEnabled: boolean; slackEnabled: boolean; inAppEnabled: boolean;
};
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      <div className="border-b border-white/10 px-6 py-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-white/40">{org?.plan ? `${org.plan.charAt(0).toUpperCase() + org.plan.slice(1)} plan` : ""}</p>
        </div>
      </div>
      ...
      <div className="flex max-w-5xl mx-auto gap-8 px-6 py-8">
        ...
        <div className="flex-1 space-y-6">
          {activeTab === "profile" && (
            <m.div key="profile" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              <div className="bg-white/4 border border-white/10 rounded-2xl p-6">
                ...
                {orgQuery.isLoading ? (
                  <div className="flex items-center gap-2 text-white/40"><Loader2 size={14} className="animate-spin" /> Loading...</div>
                ) : (
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="text-xs font-medium text-white/60 mb-1.5 block">Domain</label>
                      <input
                        className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-[#c9a84c]/60 transition-colors"
                        defaultValue={org?.domain ?? ""}
                        placeholder="yourdomain.com"
                        onChange={(e) => setOrgForm((f) => ({ ...f, domain: e.target.value || null }))}
                      />
                    </div>
                    <div>
                      <label className="text-xs font-medium text-white/60 mb-1.5 block">Logo URL</label>
                      <input
                        className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-[#c9a84c]/60 transition-colors"
                        defaultValue={org?.logoUrl ?? ""}
                        placeholder="https://..."
                        onChange={(e) => setOrgForm((f) => ({ ...f, logoUrl: e.target.value || null }))}
                      />
                    </div>
                    <button
          ...
          {activeTab === "team" && (
            <m.div key="team" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              <div className="bg-white/4 border border-white/10 rounded-2xl p-6">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {inviteMutation.isPending ? <Loader2 size={13} className="animate-spin" /> : <Plus size={13} />}
                    Invite
                  </button>
                </div>
              </div>
              ...
              <div className="bg-white/4 border border-white/10 rounded-2xl p-6">
                ...
                {membersQuery.isLoading ? (
                  ...
                  <div className="space-y-2">
                    {members.map((m) => (
                      <div key={m.userId} className="flex items-center gap-3 p-3 bg-white/3 border border-white/8 rounded-xl">
                    ...
                    {members.length === 0 && (
                      <p className="text-sm text-white/30 text-center py-4">No members found. Invite someone above.</p>
                    )}
                  </div>
                )}
              </div>
          ...
          {activeTab === "notifications" && (
            <m.div key="notifications" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              <div className="bg-white/4 border border-white/10 rounded-2xl p-6">
                <h2 className="text-base font-semibold mb-1">Notification Channels</h2>
                <p className="text-xs text-white/40 mb-5">Configure how your organization receives alerts and updates.</p>
                {notifQuery.isLoading ? (
                  <div className="flex items-center gap-2 text-white/40"><Loader2 size={14} className="animate-spin" /> Loading...</div>
                ) : (
                  <div className="space-y-3">
                    {[
                      { key: "inAppEnabled", label: "In-App Notifications", icon: Bell, desc: "Real-time alerts within the platform" },
                      { key: "emailEnabled", label: "Email Notifications", icon: Mail, desc: "Alerts and digests via email" },
                      { key: "smsEnabled", label: "SMS Notifications", icon: Phone, desc: "Critical alerts via text message" },
                      { key: "slackEnabled", label: "Slack Notifications", icon: MessageSquare, desc: "Alerts in your connected Slack workspace" },
                    ].map(({ key, label, icon: Icon, desc }) => {
                      const current = notifForm[key as keyof NotifPrefs] ?? notifQuery.data?.[key as keyof NotifPrefs] ?? false;
                      return (
                        <div key={key} className="flex items-center justify-between p-4 bg-white/3 border border-white/8 rounded-xl">
                          ...
                          >
                            <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${current ? "translate-x-[22px]" : "translate-x-0.5"}`} />
                          </button>
                        </div>
                      );
                    })}
                    ...
                    >
                      {updateNotifMutation.isPending ? <Loader2 size={13} className="animate-spin" /> : <Save size={13} />}
                      Save Preferences
                    </button>
                  </div>
                )}
          ...
          {activeTab === "account" && (
            <m.div key="account" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              <div className="bg-white/4 border border-white/10 rounded-2xl p-6">
                ...
                {userProfileQuery.isLoading ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-3 bg-white/3 rounded-lg">
                      <p className="text-xs text-white/40">Email: {userProfileQuery.data?.email ?? "—"}</p>
                    </div>
                    <button
                      className="flex items-center gap-2 px-4 py-2 bg-[#c9a84c]/20 hover:bg-[#c9a84c]/30 border border-[#c9a84c]/30 rounded-lg text-sm text-[#c9a84c] font-medium transition-colors disabled:opacity-50"
                    ...
                    >
                      {updateUserMutation.isPending ? <Loader2 size={13} className="animate-spin" /> : <Save size={13} />}
                      Save Profile
                    </button>
                  </div>
                )}
              ...
              <div className="bg-white/4 border border-white/10 rounded-2xl p-6">
                ...
                <p className="text-xs text-white/40 mb-4">Personal preferences override org-level defaults.</p>
                {userNotifQuery.isLoading ? (
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      const current = userNotifForm[key as keyof NotifPrefs] ?? userNotifQuery.data?.[key as keyof NotifPrefs] ?? false;
                      return (
                    ...
                    <button
                      ...
                      disabled={updateUserNotifMutation.isPending || !Object.keys(userNotifForm).length}
                    >
                      {updateUserNotifMutation.isPending ? <Loader2 size={13} className="animate-spin" /> : <Save size={13} />}
                      Save
                    </button>
                  </div>
                )}
              ...
              <div className="bg-white/4 border border-red-500/20 rounded-2xl p-6">
                ...
                <p className="text-xs text-white/40 mb-4">Actions here are irreversible. Proceed with caution.</p>
                <div className="space-y-2">
                  <a
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (!email) { showError("No email address on file"); return; }
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (confirm("Are you sure you want to deactivate your account? This will log you out.")) {
