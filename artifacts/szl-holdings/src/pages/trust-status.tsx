...
function StatusLabel({ status }: { status: LiveStatus }) {
  ...
  return (
    <span style={{ fontSize: '12px', fontWeight: 700, color: colors[status] }}>{map[status]}</span>
  );
}
...
export default function TrustStatusPage() {
  const __pageMeta = usePageMeta({
    ...
    description:
      'Live platform status: uptime, security posture, dependency health, data residency, and SOC 2 progress.',
  ...
  const cards: StatusCard[] = [
    ...
    },
    {
      ...
      detail: loading
        ...
        : dbLatency !== undefined
          ...
          : 'Connected — latency not reported',
    ...
    },
    {
      ...
      icon: Shield,
      status: 'live',
      detail: `${provenClaims.length} proven capabilities have live or working-demo proof in the manifest. RBAC, OIDC, tenant isolation, and safety gates are documented; production certification remains separate.`,
      source:
        'artifacts/szl-holdings/src/data/capability-manifest.json; docs/security/access-control-matrix.md',
      lastChecked: '2026-04-26',
    },
    {
      id: 'ci',
      label: 'CI / Build Pipeline',
      description: 'GitHub Actions: lint, typecheck, test, build, security gates',
      icon: Activity,
      status: 'live',
      detail:
        'Source-of-truth records 24 workflows covering CI, CodeQL, dependency review, security scans, build, E2E, Lighthouse, README QA, and one archived manual-only workflow.',
      source: 'audit/source-of-truth.json (ci.workflows.count)',
    ...
    },
    {
      ...
      status: 'degraded',
      detail:
        'Not yet initiated. Controls alignment underway. Audit engagement targeted post-Series A funding.',
    ...
    },
    {
      ...
      status: 'live',
      detail:
        'All production data stored in Replit-managed PostgreSQL. No third-party data replication. Tenant data scoped by org_id at all query paths.',
    ...
    },
    {
      ...
      status: 'live',
      detail:
        'Automated dependency audit integrated in CI pipeline. Critical findings trigger immediate review block.',
      source: '.github/workflows/dependency-review.yml; .github/workflows/security.yml',
      lastChecked: '2026-04-25',
    },
  ];

  const operationalCount = cards.filter((c) => c.status === 'live').length;
  const overallStatus: LiveStatus = cards.some((c) => c.status === 'down')
    ? 'down'
    : cards.some((c) => c.status === 'degraded')
      ? 'degraded'
      : cards.every((c) => c.status === 'live')
        ? 'live'
        : 'unknown';
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  Live status for platform uptime, database, security posture, and compliance
                  progress. API probe data refreshes every 60 seconds. Manifest data reflects last
                  audit.
                </p>
                <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' as const }}>
                  <div>
                    <span style={{ fontSize: '1.75rem', fontWeight: 700, color: '#10b981' }}>
                      {operationalCount}/{cards.length}
                    </span>
                    <span style={{ fontSize: '12px', color: 'hsl(210,5%,46%)', display: 'block' }}>
                      systems operational
                    </span>
                  </div>
                  <div>
                  ...
                  <div>
                    <span
                      style={{ fontSize: '1.75rem', fontWeight: 700, color: 'hsl(38,12%,88%)' }}
                    >
                      {totals.live}
                    </span>
                    <span style={{ fontSize: '12px', color: 'hsl(210,5%,46%)', display: 'block' }}>
          ...
          >
            ...
            >
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                          <span
                            style={{ fontSize: '14px', fontWeight: 700, color: 'hsl(38,12%,88%)' }}
                          >
                            {card.label}
                          </span>
                        ...
                        <p style={{ fontSize: '12px', color: 'hsl(210,5%,44%)', lineHeight: 1.55 }}>
                          {card.detail}
                        </p>
                        <p
                          style={{
                        ...
                        >
                          Source:{' '}
                          <code style={{ fontFamily: 'monospace', color: 'hsl(210,55%,55%)' }}>
                            {card.source}
                          </code>
                          {card.lastChecked && (
                            <>
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '0.4rem' }}>
                  {[
                    ...
                    [
                      'Capability manifest',
                      'artifacts/szl-holdings/src/data/capability-manifest.json - audited 2026-04-26',
                    ],
                    [
                      'Architecture docs',
                      'architecture.md, docs/DEPLOYMENT_MODEL.md, audit/source-of-truth.json',
                    ],
              ...
              >
                <Link
                  href="/trust-center"
                  style={{ fontSize: '13px', color: 'hsl(210,55%,58%)', textDecoration: 'none' }}
                >
                ...
                <span style={{ color: 'hsl(210,5%,30%)' }}>·</span>
                <a
                  href="mailto:security@szlholdings.com"
                  style={{ fontSize: '13px', color: 'hsl(210,5%,40%)', textDecoration: 'none' }}
                >
