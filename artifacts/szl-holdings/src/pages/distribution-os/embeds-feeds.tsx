...
export default function EmbeedsFeedsPage() {
  ...
  const [widgetType, setWidgetType] = useState<
    'content-card' | 'newsletter-signup' | 'content-feed'
  >('newsletter-signup');
      ...
      <m.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <SectionCard
            ...
            subtitle="Auto-generated feeds for every content type — syndicate anywhere"
            color="#d4a054"
          >
            <div>
              <FeedRow
                icon={Rss}
                label="All Articles (RSS)"
                url={`${BASE}/feeds/articles.rss`}
                color="#d4a054"
                desc="All published flagship essays and articles"
              />
              <FeedRow
                icon={Rss}
                label="Newsletters (RSS)"
                url={`${BASE}/feeds/newsletters.rss`}
                color="#d4a054"
                desc="Every published newsletter issue"
              />
              <FeedRow
                icon={BookOpen}
                label="X Posts (Atom)"
                url={`${BASE}/feeds/posts.atom`}
                color="#1a8cd8"
                desc="Published X posts and threads"
              />
              <FeedRow
                icon={Rss}
                label="All Content (Combined)"
                url={`${BASE}/feeds/all.rss`}
                color="#5a9c5a"
                desc="Combined feed of all published content types"
              />
              <div
                style={{
                  paddingTop: '0.875rem',
              ...
              >
                ...
                <span style={{ fontSize: '0.75rem', color: '#6b6560' }}>
                  Feeds auto-update within 2 minutes of publishing. Add to Feedly, Inoreader, or any
                  RSS aggregator.
                </span>
              </div>
            </div>
          ...
          <SectionCard
            ...
            subtitle="Paste a content URL anywhere — WordPress, Ghost, Notion, Slack auto-embed it"
            color="#8b7ac8"
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div
                style={{
                  ...
                  background: 'hsla(270,40%,50%,0.06)',
              ...
              >
                ...
                <p style={{ fontSize: '0.8125rem', color: '#6b6560', lineHeight: 1.55 }}>
                  Any platform that supports oEmbed (WordPress, Ghost, Notion, Slack, Discord) will
                  automatically show a rich embed when someone pastes a SZL Holdings content URL. No
                  plugin required.
                </p>
              </div>
              ...
              <CopyBlock
                code={oembedProviderConfig}
                label="oEmbed provider registration (for CMS platforms)"
              />
            </div>
          </SectionCard>
          ...
          <SectionCard
            ...
            subtitle="Drop a <script> tag on any site to show live content from Distribution OS"
            color="#5a9c5a"
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                ...
                {(['newsletter-signup', 'content-card', 'content-feed'] as const).map((type) => (
                  <button
                    ...
                    style={{
                      padding: '0.75rem',
                      background:
                        widgetType === type ? 'hsla(120,30%,40%,0.12)' : 'hsla(0,0%,100%,0.03)',
                      border: `1px solid ${widgetType === type ? 'hsla(120,30%,40%,0.3)' : 'hsla(0,0%,100%,0.08)'}`,
                      borderRadius: '8px',
                      color: widgetType === type ? '#5a9c5a' : '#6b6560',
                      fontSize: '0.75rem',
                      fontWeight: widgetType === type ? 700 : 400,
                      cursor: 'pointer',
                      textAlign: 'center',
                    }}
                  >
                ...
                {(['dark', 'light'] as const).map((theme) => (
                  <button
                    ...
                    style={{
                      ...
                      background: widgetTheme === theme ? 'hsla(0,0%,100%,0.08)' : 'none',
                      border: `1px solid ${widgetTheme === theme ? 'hsla(0,0%,100%,0.15)' : 'hsla(0,0%,100%,0.06)'}`,
                      borderRadius: '6px',
                      color: widgetTheme === theme ? '#e8e4de' : '#4a4540',
                      fontSize: '0.75rem',
                      cursor: 'pointer',
                      textTransform: 'capitalize',
                      fontWeight: widgetTheme === theme ? 600 : 400,
                    }}
                  >
                    {theme}
                  </button>
              ...
              >
                ...
                <div
                  style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.25rem 1.5rem' }}
                >
                  {[
          ...
          <SectionCard
            ...
            subtitle="Auto-generated meta tags for every piece of published content"
            color="#4a90b8"
          >
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div
                style={{
                  padding: '0.875rem 1rem',
                  background: 'hsla(210,50%,50%,0.06)',
                  border: '1px solid hsla(210,50%,50%,0.15)',
                  borderRadius: '8px',
                }}
              >
                <div style={{ fontSize: '0.75rem', color: '#6b6560', lineHeight: 1.55 }}>
                  Every article, newsletter, and X post published through Distribution OS
                  automatically gets Open Graph and Twitter Card meta tags. No manual configuration
                  needed.
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                {[
                  ...
                  >
                    <div
                      style={{ fontSize: '0.6875rem', color: '#4a4540', marginBottom: '0.125rem' }}
                    >
                      {item.label}
                    </div>
