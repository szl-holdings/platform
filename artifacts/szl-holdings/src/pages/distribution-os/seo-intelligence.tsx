...
export default function SeoIntelligencePage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/distribution-os/seo/overview`, { credentials: 'include' }).then((r) =>
        r.json(),
      ),
      fetch(`${API}/api/distribution-os/seo/keywords`, { credentials: 'include' }).then((r) =>
        r.json(),
      ),
    ])
      .then(([o, k]) => {
  ...
  const severityColor = (s: string) =>
    s === 'high' ? DS.red : s === 'medium' ? DS.accent : DS.text.tertiary;
        ...
        >
          <div>
            ...
            <p style={{ fontSize: '0.75rem', color: DS.text.tertiary, marginTop: '0.25rem' }}>
              Competitive gap analysis, keyword prediction, and technical SEO monitoring
            </p>
          </div>
        ...
        >
          {[
            ...
            },
            {
              ...
              color: DS.green,
              icon: TrendingUp,
              sub: `+${data?.trafficGrowth ?? 0}%`,
            },
            {
              label: 'Indexed Pages',
              value: data?.indexedPages ?? '—',
              color: DS.blue,
              icon: BarChart2,
              sub: 'in search index',
            },
            {
              ...
              value: data?.technicalIssues?.reduce((s, i) => s + i.count, 0) ?? '—',
              color: DS.red,
              icon: AlertCircle,
              sub: `${data?.technicalIssues?.filter((i) => i.severity === 'high').length ?? 0} high priority`,
            },
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {data?.topOpportunities.map((op, i) => (
                  ...
                  >
                    ...
                    >
                      <span
                        style={{ fontSize: '0.8125rem', fontWeight: 600, color: DS.text.primary }}
                      >
                        {op.keyword}
                      </span>
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '0.5rem' }}>
                      <span style={{ fontSize: '0.6875rem', color: DS.text.muted }}>
                        Vol: {op.volume.toLocaleString()}
                      </span>
                      ...
                      <div style={{ display: 'flex', gap: '0.625rem', alignItems: 'center' }}>
                        ...
                        <span
                          style={{
                            ...
                            borderRadius: '4px',
                            background: `${severityColor(issue.severity)}14`,
            ...
            {keywords?.keywords.map((kw) => (
              ...
              >
                ...
                <span
                  style={{
                    fontSize: '0.75rem',
                    color: kw.currentRank && kw.currentRank <= 10 ? DS.green : DS.text.secondary,
                    fontVariantNumeric: 'tabular-nums',
                  }}
                >
                  {kw.currentRank ?? '—'}
                ...
                <span
                  style={{
                    fontSize: '0.8125rem',
                    fontWeight: 700,
                    color: kw.opportunityScore >= 85 ? DS.green : DS.text.primary,
                  }}
                >
                  {kw.opportunityScore}
                </span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {data?.contentGaps.map((gap, i) => (
              ...
              >
                ...
                >
                  <span style={{ fontSize: '0.875rem', fontWeight: 600, color: DS.text.primary }}>
                    {gap.topic}
                  </span>
                  <span
                    style={{
                ...
                <div style={{ display: 'flex', gap: '1.5rem' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.875rem', fontWeight: 700, color: DS.text.primary }}>
                      {gap.competitors}
                    </div>
                  </div>
                  <div>
                    ...
                    <div style={{ fontSize: '0.875rem', fontWeight: 700, color: DS.accent }}>
                      {gap.avgRank}
                    </div>
                  </div>
                </div>
