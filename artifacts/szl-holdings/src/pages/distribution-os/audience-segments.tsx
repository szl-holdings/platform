...
export default function AudienceSegmentsPage() {
  ...
  useEffect(() => {
    fetch(`${API}/api/distribution-os/audience/segments`, { credentials: 'include' })
      ...
      .then((d) => {
        setData(d);
        if (d.segments?.length) setSelected(d.segments[0]);
      })
      .catch(() => {});
  }, []);
      ...
      >
        ...
        >
          <div>
            ...
            >
              <Users size={18} style={{ color: DS.accent }} /> Audience Segmentation &
              Personalization
            </h1>
            <p style={{ fontSize: '0.75rem', color: DS.text.tertiary, marginTop: '0.25rem' }}>
              Dynamic segments with personalized content recommendations and AI-generated sequences
            </p>
          </div>
          ...
          {[
            {
              ...
              value: data?.totalAudience?.toLocaleString() ?? '—',
              color: DS.accent,
              icon: Users,
            },
            {
            ...
            },
            {
              ...
              value: data
                ? `${Math.round(data.segments.reduce((s, seg) => s + seg.engagementScore, 0) / data.segments.length)}/100`
                : '—',
              color: DS.green,
              icon: TrendingUp,
            },
            {
              label: 'AI Sequences Active',
              value: data?.segments.length ?? '—',
              color: '#9b7fd4',
              icon: Sparkles,
            },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '1rem' }}>
                ...
                >
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                      ...
                      />
                      <span
                        style={{ fontSize: '0.9375rem', fontWeight: 700, color: DS.text.primary }}
                      >
                        {seg.name}
                      </span>
                    </div>
                  ...
                  >
                    {[
                      ...
                      {
                        label: 'Engagement',
                        value: `${seg.engagementScore}/100`,
                        icon: TrendingUp,
                      },
                      {
                        label: 'Revenue Share',
                        value: `${seg.revenueContribution}%`,
                        icon: DollarSign,
                      },
          ...
          {selected &&
            (() => {
              const color =
                SEGMENT_COLORS[
                  data?.segments.findIndex((s) => s.id === selected.id) ?? 0 % SEGMENT_COLORS.length
                ];
              return (
                <div
                  style={{
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ fontSize: '0.875rem', fontWeight: 700, color: DS.text.primary }}>
                      {selected.name}
                    </span>
                  </div>
                  ...
                  <div style={{ marginBottom: '1.125rem' }}>
                    ...
                    >
                      <Sparkles size={11} style={{ color: DS.accent }} /> Personalized Content
                      Tracks
                    </div>
                    ...
                    >
                      {selected.recommendedSequence.split(' → ').map((step, i, arr) => (
                        ...
                        >
                          ...
                          {i < arr.length - 1 && (
                            <ArrowRight size={11} style={{ color: DS.text.muted }} />
                          )}
                        </div>
                      ))}
                    </div>
