...
const STAGE_META: Record<string, { color: string; bg: string; label: string }> = {
  ...
  'proposal-candidate': {
    color: '#8b7ac8',
    bg: 'hsla(270,40%,50%,0.12)',
    label: 'Proposal Candidate',
  },
      ...
      >
        ...
        >
          <div>
            ...
            <p style={{ fontSize: '0.75rem', color: '#6b6560', marginTop: '0.1rem' }}>
              {lead.email} · Added {new Date(lead.createdAt).toLocaleDateString()}
            </p>
          </div>
        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
          {/* CRM Quick Fields */}
          <div
            style={{ padding: '1.125rem 1.5rem', borderBottom: '1px solid hsla(0,0%,100%,0.05)' }}
          >
            <div
              style={{
                fontSize: '0.6875rem',
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      border:
                        stage === s ? `1px solid ${sm.color}40` : '1px solid hsla(0,0%,100%,0.06)',
                    }}
                  >
                    {sm.label}
                  </button>
          ...
          <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid hsla(0,0%,100%,0.05)' }}>
            ...
            <div style={{ marginTop: '0.625rem' }}>
              ...
              <textarea
                ...
                onChange={(e) => {
                  setProfile((p) => ({ ...p, message: e.target.value }));
                  setDirty(true);
                }}
                rows={3}
                style={{
          ...
          {dirty && (
            <div
              style={{
                ...
                background: 'hsla(40,60%,50%,0.07)',
                borderBottom: '1px solid hsla(40,60%,50%,0.15)',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'space-between',
              }}
          ...
          <div style={{ padding: '1rem 1.5rem' }}>
            ...
            >
              {notes.length === 0 && (
                <p style={{ fontSize: '0.75rem', color: '#4a4540' }}>No notes yet.</p>
              )}
              {notes.map((n) => (
                <div
                  key={n.id}
                ...
                >
                  <p style={{ fontSize: '0.8125rem', color: '#e8e4de', marginBottom: '0.25rem' }}>
                    {n.note}
                  </p>
        ...
        >
          <div>
            ...
            <p style={{ fontSize: '0.75rem', color: '#6b6560' }}>
              Map columns to lead fields. Email is required.
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          {done ? (
            ...
            <div>
              ...
              >
                ...
                <p style={{ color: '#4a4540', fontSize: '0.75rem', marginTop: '0.25rem' }}>
                  Supports Linktree and standard lead exports
                </p>
              </div>
              <input
                ref={fileRef}
            ...
            <div>
              ...
              >
                <p style={{ fontSize: '0.75rem', color: '#5a9c5a' }}>
              ...
              >
                {headers.map((h) => (
                  <div key={h} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <span style={{ fontSize: '0.8125rem', color: '#e8e4de', minWidth: '160px' }}>
                      {h}
                    </span>
                    <select
                      value={mapping[h] || ''}
      ...
      >
        ...
        >
          <div>
            ...
            <p style={{ fontSize: '0.75rem', color: '#6b6560' }}>
              Manually enter a contact into the pipeline
            </p>
          </div>
          <button
            onClick={onClose}
...
export default function LeadsPage() {
  ...
  const sources = [
    'all',
    ...Array.from(new Set(leads.filter((l) => l.source).map((l) => l.source!))),
  ];
  const campaigns = [
    'all',
    ...Array.from(new Set(leads.filter((l) => l.campaign).map((l) => l.campaign!))),
  ];
        ...
        >
          <div>
            ...
            <p style={{ fontSize: '0.8125rem', color: '#6b6560', marginTop: '0.25rem' }}>
              ...
              {needsFollowup > 0 && (
                <span style={{ color: '#c45a4a' }}>{needsFollowup} need follow-up</span>
              )}
            </p>
          </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: active
                    ? `1px solid ${meta?.color || '#e8e4de'}40`
                    : '1px solid hsla(0,0%,100%,0.06)',
                  textTransform: 'uppercase',
                  letterSpacing: '0.05em',
                }}
            ...
            {filtered.map((lead) => (
              ...
              >
                ...
                <div>
                  ...
                  {lead.source && (
                    <div style={{ fontSize: '0.6875rem', color: '#4a4540' }}>{lead.source}</div>
                  )}
                </div>
                <div onClick={(e) => e.stopPropagation()}>
                  <select
                    value={lead.stage}
                    onChange={(e) =>
                      quickStageUpdate(lead.id, e.target.value, e as unknown as React.MouseEvent)
                    }
                    style={{
                      padding: '0.25rem 0.375rem',
                      background: 'transparent',
                      border: 'none',
                      color: STAGE_META[lead.stage]?.color || '#e8e4de',
                      fontSize: '0.75rem',
                      fontWeight: 700,
                      cursor: 'pointer',
                    }}
