...
import { Twitter, Plus, Send, Clock, CheckCircle, AlertCircle, Trash2, MessageSquare, Zap, ExternalLink, Loader2 } from "lucide-react";
...
export default function XStudioPage() {
  ...
  async function deletePost(id: number) {
    await fetch(`${API}/api/distribution-os/x-posts/${id}`, { method: "DELETE", credentials: "include", headers: { "x-csrf-token": getCsrfToken() } });
      ...
      <m.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "2rem" }}>
          <div>
            ...
            <p style={{ fontSize: "0.8125rem", color: "#6b6560", marginTop: "0.25rem" }}>Compose, queue, and publish posts to X (Twitter)</p>
          </div>
              ...
              <div key={post.id} style={{ padding: "1rem 1.25rem", background: "hsla(0,0%,100%,0.02)", border: "1px solid hsla(0,0%,100%,0.05)", borderRadius: "8px" }}>
                <div style={{ display: "flex", alignItems: "flex-start", gap: "0.75rem" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginTop: "0.5rem", flexWrap: "wrap" }}>
                      ...
                      {post.externalPostUrl && (
                        <a href={post.externalPostUrl} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: "0.25rem", fontSize: "0.6875rem", color: "#4a90b8", textDecoration: "none" }}>
                  ...
                  <div style={{ display: "flex", gap: "0.375rem" }}>
                    {post.status === "draft" && (
                      <button onClick={() => queuePost(post.id)} title="Queue for sending" style={{ padding: "0.375rem", background: "none", border: "1px solid hsla(0,0%,100%,0.08)", borderRadius: "4px", color: "#d4a054", cursor: "pointer" }}>
                        <Send size={14} />
                      </button>
                    )}
                    {(post.status === "queued" || post.status === "draft" || post.status === "failed") && (
                      <button
                        onClick={() => publishPost(post.id)}
                        disabled={isPublishing}
                        title="Publish to X now"
                        style={{ padding: "0.375rem 0.625rem", background: isPublishing ? "hsla(0,0%,100%,0.04)" : "hsla(210,50%,50%,0.12)", border: "1px solid hsla(210,50%,50%,0.2)", borderRadius: "4px", color: "#4a90b8", cursor: isPublishing ? "default" : "pointer", display: "flex", alignItems: "center", gap: "0.25rem", fontSize: "0.6875rem", fontWeight: 600 }}
                      >
                        {isPublishing ? <Loader2 size={12} style={{ animation: "spin 1s linear infinite" }} /> : <Zap size={12} />}
                        {isPublishing ? "Sending..." : "Publish"}
                      </button>
                    )}
                    <button onClick={() => deletePost(post.id)} title="Delete" style={{ padding: "0.375rem", background: "none", border: "1px solid hsla(0,0%,100%,0.08)", borderRadius: "4px", color: "#c45a4a", cursor: "pointer" }}>
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
