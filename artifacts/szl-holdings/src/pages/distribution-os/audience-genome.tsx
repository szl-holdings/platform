...
export default function AudienceGenomePage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/distribution-os/audience/genome`, { credentials: 'include' }).then((r) =>
        r.json(),
      ),
      fetch(`${API}/api/distribution-os/audience/migration`, { credentials: 'include' }).then((r) =>
        r.json(),
      ),
    ])
      .then(([g, m]) => {
        ...
        >
          <div>
            ...
            <p style={{ fontSize: '0.75rem', color: DS.text.tertiary, marginTop: '0.25rem' }}>
              Deep psychographic profiling, preference mapping, and audience migration tracking
            </p>
          </div>
          ...
          {[
            {
              ...
              value: genome?.totalAudience?.toLocaleString() ?? '—',
              icon: Users,
              color: DS.accent,
            },
            {
              label: 'Active Segments',
              value: genome?.segments?.length ?? '—',
              icon: BarChart2,
              color: DS.blue,
            },
            {
              label: 'Fastest Growing',
              value: genome?.fastestGrowing ?? '—',
              icon: TrendingUp,
              color: DS.green,
            },
            {
              label: 'Audience Migrations / Mo',
              value: migration?.totalMigrations?.toLocaleString() ?? '—',
              icon: ArrowRight,
              color: '#9b7fd4',
            },
            ...
            >
              ...
              <div
                style={{
                  fontSize: typeof value === 'number' ? '1.75rem' : '0.9375rem',
                  fontWeight: 700,
                  color: DS.text.primary,
                  ...
                  >
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                        ...
                        />
                        <span
                          style={{ fontSize: '0.875rem', fontWeight: 600, color: DS.text.primary }}
                        >
                          {seg.name}
                        </span>
                      </div>
            ...
            {migration && (
              <div style={{ marginTop: '1.25rem' }}>
                ...
                >
                  ...
                  <div style={{ fontSize: '0.875rem', fontWeight: 600, color: DS.accent }}>
                    {migration.topPath}
                  </div>
                  <div
                    style={{ fontSize: '0.6875rem', color: DS.text.tertiary, marginTop: '0.25rem' }}
                  >
                    Avg {migration.avgConversionDays} days from first touch to conversion
                  </div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {migration.flows.map((f, i) => (
                    ...
                    >
                      <span
                        style={{ fontSize: '0.75rem', color: DS.text.secondary, minWidth: 100 }}
                      >
                        {f.from}
                      </span>
                      <ArrowRight size={13} style={{ color: DS.text.muted, flexShrink: 0 }} />
                      <span style={{ fontSize: '0.75rem', color: DS.text.secondary, flex: 1 }}>
                        {f.to}
                      </span>
                      <span
                        style={{
            ...
            >
              ...
              >
                {[
                  ...
                  {
                    label: 'Revenue Share',
                    value: `${selected.revenueContribution}%`,
                    icon: ArrowUpRight,
                  },
