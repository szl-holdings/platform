...
import { Activity, Archive, RefreshCw, TrendingUp, AlertCircle, CheckCircle2, Clock, Flame, BarChart2, ArrowUpRight } from "lucide-react";
...
interface LifecycleData {
  ...
  summary: { evergreen: number; needsAction: number; totalContent: number; avgHealthScore: number; redistributionCandidates: number };
}
...
function HealthBar({ score }: { score: number }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
      ...
      <span style={{ fontSize: "0.6875rem", fontWeight: 600, color, fontVariantNumeric: "tabular-nums", minWidth: 24 }}>{score}</span>
    </div>
  );
}
...
export default function LifecycleIntelligencePage() {
  ...
  const [filter, setFilter] = useState<"all" | "evergreen" | "needs-action" | "redistribute">("all");
      ...
      <m.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "1.25rem", flexWrap: "wrap", gap: "0.75rem" }}>
          <div>
            ...
            <p style={{ fontSize: "0.75rem", color: DS.text.tertiary, marginTop: "0.25rem" }}>Health tracking, evergreen identification, and auto-redistribution recommendations</p>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "0.75rem", marginBottom: "1.25rem" }}>
          {[
            { label: "Total Content", value: data?.summary.totalContent ?? "—", color: DS.blue, icon: BarChart2 },
            { label: "Evergreen Assets", value: data?.summary.evergreen ?? "—", color: "#9b7fd4", icon: Flame },
            { label: "Avg Health Score", value: data ? `${data.summary.avgHealthScore}/100` : "—", color: DS.green, icon: Activity },
            { label: "Needs Action", value: data?.summary.needsAction ?? "—", color: DS.accent, icon: AlertCircle },
            { label: "Redistribution Queue", value: data?.summary.redistributionCandidates ?? "—", color: DS.green, icon: RefreshCw },
            ...
            <div key={label} style={{ padding: "1rem 1.25rem", background: DS.surface, border: `1px solid ${DS.border}`, borderRadius: "10px", position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ fontSize: "0.625rem", color: DS.text.tertiary, marginTop: "0.25rem" }}>{label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1rem" }}>
          {([
            ...
            <button key={f.key} onClick={() => setFilter(f.key)} style={{ padding: "0.3rem 0.75rem", borderRadius: "6px", border: `1px solid ${filter === f.key ? DS.accent + "40" : DS.border}`, background: filter === f.key ? `${DS.accent}10` : "transparent", color: filter === f.key ? DS.accent : DS.text.tertiary, fontSize: "0.75rem", fontWeight: filter === f.key ? 600 : 400, cursor: "pointer" }}>
              {f.label}
            </button>
          ))}
        </div>
        ...
        <div style={{ background: DS.surface, border: `1px solid ${DS.border}`, borderRadius: "10px", overflow: "hidden" }}>
          <div style={{ display: "grid", gridTemplateColumns: "3fr 100px 90px 80px 80px 100px 120px", padding: "0.625rem 1rem", borderBottom: `1px solid ${DS.border}`, fontSize: "0.625rem", fontWeight: 700, color: DS.text.muted, textTransform: "uppercase", letterSpacing: "0.08em", gap: "0.5rem" }}>
            <span>Content</span><span>Stage</span><span>Health</span><span>Views/Mo</span><span>Total $</span><span>Distrib.</span><span>Action</span>
          </div>
          {filtered.map(c => {
            ...
            return (
              <m.div key={c.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: "grid", gridTemplateColumns: "3fr 100px 90px 80px 80px 100px 120px", padding: "0.75rem 1rem", borderBottom: `1px solid ${DS.border}`, gap: "0.5rem", alignItems: "center" }}>
                <div>
                  <div style={{ fontSize: "0.75rem", color: DS.text.secondary, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginBottom: "0.125rem" }}>{c.title}</div>
                  {c.isEvergreen && <span style={{ fontSize: "0.5625rem", padding: "0.1rem 0.375rem", borderRadius: "3px", background: "#9b7fd414", color: "#9b7fd4", fontWeight: 600 }}>EVERGREEN</span>}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: "0.25rem" }}>
                  <StageIcon size={11} style={{ color: stageCfg.color, flexShrink: 0 }} />
                  <span style={{ fontSize: "0.6875rem", color: stageCfg.color }}>{stageCfg.label}</span>
                </div>
                <HealthBar score={c.contentHealthScore} />
                <span style={{ fontSize: "0.75rem", color: DS.text.secondary, fontVariantNumeric: "tabular-nums" }}>{c.monthlyViews.toLocaleString()}</span>
                <span style={{ fontSize: "0.75rem", color: c.revenueGenerated > 0 ? DS.green : DS.text.muted, fontVariantNumeric: "tabular-nums" }}>{c.revenueGenerated > 0 ? `$${(c.revenueGenerated / 1000).toFixed(1)}k` : "—"}</span>
                <span style={{ fontSize: "0.75rem", color: DS.text.muted, fontVariantNumeric: "tabular-nums" }}>{c.redistributionCount}×</span>
                <div style={{ display: "flex", alignItems: "center", gap: "0.25rem" }}>
                  <ActionIcon size={11} style={{ color: actionCfg.color, flexShrink: 0 }} />
                  <span style={{ fontSize: "0.6875rem", color: actionCfg.color }}>{actionCfg.label}</span>
                </div>
              </m.div>
            );
          })}
