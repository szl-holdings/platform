...
const PLATFORMS: Platform[] = [
  {
    id: 'x',
    name: 'X (Twitter)',
    description: 'Single posts and auto-split threads',
    authType: 'oauth2',
    color: '#1a8cd8',
    category: 'social',
    capabilities: [
  ...
  },
  {
    id: 'linkedin',
    name: 'LinkedIn',
    description: 'Posts and carousel PDF publishing',
    authType: 'oauth2',
    color: '#0a66c2',
    category: 'social',
  ...
  },
  {
    id: 'threads',
    name: 'Threads',
    description: "Publish to Meta's Threads platform",
    authType: 'oauth2',
    color: '#e8e4de',
    category: 'social',
  ...
  },
  {
    id: 'mastodon',
    name: 'Mastodon',
    description: 'ActivityPub fediverse publishing',
    authType: 'oauth2',
    color: '#6364ff',
    category: 'social',
  ...
  },
  {
    id: 'instagram',
    name: 'Instagram',
    description: 'Posts via Meta Business API',
    authType: 'oauth2',
    color: '#e1306c',
    category: 'social',
  ...
  },
  {
    id: 'devto',
    name: 'Dev.to',
    description: 'Publish developer-focused articles',
    authType: 'api-key',
    color: '#3b49df',
    category: 'blog',
  ...
  },
  {
    id: 'hashnode',
    name: 'Hashnode',
    description: 'GraphQL-powered developer blog network',
    authType: 'api-key',
    color: '#2962ff',
    category: 'blog',
  ...
  },
  {
    id: 'wordpress',
    name: 'WordPress',
    description: 'REST API publishing to any WP site',
    authType: 'username-password',
    color: '#21759b',
    category: 'cms',
  ...
  },
  {
    id: 'ghost',
    name: 'Ghost',
    description: 'Headless CMS with Ghost Admin API',
    authType: 'api-key',
    color: '#15171a',
    category: 'cms',
  ...
  },
  {
    id: 'reddit',
    name: 'Reddit',
    description: 'OAuth2 posts to subreddits',
    authType: 'oauth2',
    color: '#ff4500',
    category: 'social',
...
const CATEGORY_LABELS: Record<string, string> = {
  social: 'Social Media',
  blog: 'Blog Platforms',
  newsletter: 'Newsletter Platforms',
  cms: 'CMS & Publishing',
};
const AUTH_LABELS: Record<string, string> = {
  oauth2: 'OAuth 2.0',
  'api-key': 'API Key',
  'username-password': 'Username + App Password',
  bearer: 'Bearer Token',
};
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <p style={{ fontSize: '0.75rem', color: '#6b6560' }}>
              via {AUTH_LABELS[platform.authType]}
            </p>
          </div>
        </div>
          ...
          <>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <span style={{ fontSize: '0.75rem', color: '#8b8579' }}>
                  Auth method: {AUTH_LABELS[platform.authType]}. Credentials stored encrypted at
                  rest.
                </span>
              </div>
            </div>
        ...
        {step === 'success' && (
          <div style={{ textAlign: 'center', padding: '1rem 0' }}>
            ...
            <p style={{ fontSize: '0.8125rem', color: '#6b6560', marginBottom: '1.5rem' }}>
              Content can now be published to this platform from the Distribution OS.
            </p>
            <button
              onClick={onClose}
              style={{
  ...
  return (
    ...
    >
      <div style={{ padding: '1.25rem' }}>
        ...
        <div
          style={{ display: 'flex', gap: '0.25rem', flexWrap: 'wrap', marginBottom: '0.875rem' }}
        >
          <span
            style={{
              fontSize: '0.6rem',
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            <div
              style={{ padding: '0 1.25rem 1.25rem', borderTop: '1px solid hsla(0,0%,100%,0.04)' }}
            >
...
export default function PlatformConnectionsPage() {
  ...
  const filtered =
    filterCategory === 'all' ? platforms : platforms.filter((p) => p.category === filterCategory);
  const connectedCount = platforms.filter(
    (p) => p.status === 'connected' || p.status === 'mock',
  ).length;
      ...
      <m.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <span style={{ fontSize: '0.75rem', color: '#5a9c5a', fontWeight: 600 }}>
                {connectedCount} platforms active
              </span>
            </div>
            <div style={{ fontSize: '0.6875rem', color: '#4a4540' }}>
              of {PLATFORMS.length} supported
            </div>
          </div>
        </div>
        ...
        >
          {[
            ...
            {
              label: 'Disconnected',
              value: PLATFORMS.filter((p) => p.status === 'disconnected').length,
              color: '#6b6560',
              icon: WifiOff,
            },
          ...
          {categories.map((cat) => (
            <button
              ...
              style={{
                padding: '0.375rem 0.875rem',
                background:
                  filterCategory === cat ? 'hsla(40,60%,50%,0.12)' : 'hsla(0,0%,100%,0.04)',
                color: filterCategory === cat ? '#d4a054' : '#6b6560',
                border: `1px solid ${filterCategory === cat ? 'hsla(40,60%,50%,0.2)' : 'hsla(0,0%,100%,0.06)'}`,
                borderRadius: '6px',
                fontSize: '0.75rem',
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <div>
              ...
              >
                Need a platform we don't support yet?
              </div>
              <div style={{ fontSize: '0.75rem', color: '#6b6560' }}>
                Use the Public API to build a custom connector, or request a new platform
                integration from the Developer API section.
              </div>
            </div>
            <a
              href="/admin/distribution/developer-api"
              style={{
                flexShrink: 0,
                display: 'flex',
                alignItems: 'center',
                ...
                background: 'hsla(0,0%,100%,0.05)',
                color: '#8b7ac8',
                border: '1px solid hsla(270,40%,50%,0.2)',
                borderRadius: '6px',
                fontSize: '0.75rem',
                fontWeight: 600,
                textDecoration: 'none',
