...
const INTEGRATION_META: Record<
  string,
  { name: string; description: string; icon: typeof Globe; color: string }
> = {
  x: {
    name: 'X (Twitter)',
    description: 'Auto-post approved content to X',
    icon: Twitter,
    color: '#1a8cd8',
  },
  substack: {
  ...
  medium: {
    name: 'Medium',
    description: 'Cross-post articles to Medium',
    icon: Newspaper,
    color: '#e8e4de',
  },
  linkedin: {
    name: 'LinkedIn',
    description: 'Share updates to LinkedIn',
    icon: Globe,
    color: '#0a66c2',
  },
  linktree: {
    name: 'Linktree',
    description: 'Sync link-in-bio configuration',
    icon: Link2,
    color: '#5ecc7b',
  },
  email: {
    name: 'Email Provider',
    description: 'Send newsletters via email',
    icon: Mail,
    color: '#d4a054',
  },
};
...
const SETTING_GROUPS = [
  {
    ...
    fields: [
      { key: 'company_name', label: 'Company Name', placeholder: 'SZL Holdings' },
      {
        key: 'company_tagline',
        label: 'Tagline',
        placeholder: 'The Operating System for AI-Native Businesses',
      },
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: 'inquiry_email',
        label: 'Inquiry Destination',
        placeholder: 'Where form submissions are forwarded',
      },
      { key: 'newsletter_from_name', label: 'Newsletter From Name', placeholder: 'Stephen @ SZL' },
      {
        key: 'newsletter_reply_to',
        label: 'Newsletter Reply-To',
        placeholder: 'hello@szlholdings.com',
      },
    ],
  },
];
...
const RECOMMENDED_LINKTREE = [
  {
    label: 'Newsletter — Get the Weekly Brief',
    destination: 'https://szlholdings.com/newsletter',
    tag: 'newsletter',
  },
  {
    label: 'SZL Holdings — Platform Overview',
    destination: 'https://szlholdings.com',
    tag: 'homepage',
  },
  {
    label: 'Work With Me — Private Inquiry',
...
export default function SettingsPage() {
  ...
  const [editingLinkData, setEditingLinkData] = useState<{
    ...
    campaignTag: string;
    contentTag: string;
  }>({ label: '', destination: '', campaignTag: '', contentTag: '' });
    ...
    if (existing) {
      ...
      const res = await fetch(`${API}/api/distribution-os/settings`, {
        ...
        body: JSON.stringify({
          ...
          value,
          category:
            SETTING_GROUPS.find((g) => g.fields.find((f) => f.key === key))?.category || 'company',
          label: SETTING_GROUPS.flatMap((g) => g.fields).find((f) => f.key === key)?.label || key,
        }),
      });
  ...
  async function retryIntegration(provider: string) {
    const res = await fetch(`${API}/api/distribution-os/integrations/retry/${provider}`, {
      method: 'POST',
      credentials: 'include',
      headers: writeHeaders(),
    });
  ...
  async function deleteLink(id: number) {
    ...
    await fetch(`${API}/api/distribution-os/linktree/${id}`, {
      method: 'DELETE',
      credentials: 'include',
  ...
  async function moveLink(idx: number, dir: -1 | 1) {
    ...
    await Promise.all([
      fetch(`${API}/api/distribution-os/linktree/${a.id}`, {
        method: 'PATCH',
        credentials: 'include',
        headers: writeHeaders(),
        body: JSON.stringify({ sortOrder: b.sortOrder }),
      }),
      fetch(`${API}/api/distribution-os/linktree/${b.id}`, {
        method: 'PATCH',
        credentials: 'include',
        headers: writeHeaders(),
      ...
      <m.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          <p style={{ fontSize: '0.8125rem', color: '#6b6560', marginTop: '0.25rem' }}>
            Brand configuration, integrations, and distribution setup
          </p>
        </div>
                    ...
                    return (
                      ...
                      >
                        <div style={{ minWidth: '180px' }}>
                          <div style={{ fontSize: '0.8125rem', fontWeight: 600, color: '#e8e4de' }}>
                            {field.label}
                          </div>
                        ...
                        {isEditing ? (
                          <div style={{ display: 'flex', gap: '0.5rem', flex: 1 }}>
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') saveSetting(field.key, editValue);
                            ...
                            />
                            <button
                              onClick={() => saveSetting(field.key, editValue)}
                              disabled={saving}
                              style={{
                                display: 'flex',
                                alignItems: 'center',
                          ...
                          <>
                            <div
                              style={{
                                flex: 1,
                                fontSize: '0.8125rem',
                                color: currentValue ? '#e8e4de' : '#4a4540',
                              }}
                            >
                              {currentValue || (
                                <span style={{ fontStyle: 'italic' }}>{field.placeholder}</span>
                              )}
                            </div>
                            <button
                              onClick={() => {
              ...
              return (
                <div
                  ...
                  style={{
                    padding: '1.25rem',
                    background: 'hsla(0,0%,100%,0.02)',
                    border: `1px solid ${status === 'connected' ? 'hsla(120,30%,40%,0.25)' : status === 'error' ? 'hsla(0,60%,50%,0.2)' : 'hsla(0,0%,100%,0.06)'}`,
                    borderRadius: '12px',
                  }}
                >
                  <div
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: '0.6875rem', color: '#6b6560' }}>
                          {meta.description}
                        </p>
                      </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                      <div
                        style={{ width: 7, height: 7, borderRadius: '50%', background: sm.dot }}
                      />
                      <span style={{ fontSize: '0.6875rem', fontWeight: 700, color: sm.color }}>
                        {sm.label}
                      </span>
                    </div>
                  </div>
                  ...
                  {integration?.lastError && (
                    ...
                    >
                      <p style={{ fontSize: '0.6875rem', color: '#c45a4a' }}>
                        {integration.lastError}
                      </p>
                    </div>
                  )}
                  ...
                  <div style={{ display: 'flex', gap: '0.375rem', flexWrap: 'wrap' }}>
                    <button
                      style={{
                        padding: '0.375rem 0.75rem',
                        background:
                          status === 'connected' ? 'hsla(0,0%,100%,0.05)' : 'hsla(40,60%,50%,0.12)',
                        color: status === 'connected' ? '#8b8579' : '#d4a054',
                        border: '1px solid hsla(0,0%,100%,0.08)',
                        borderRadius: '6px',
                        fontSize: '0.6875rem',
                        fontWeight: 600,
                    ...
                    {integration?.lastError && (
                      <button
                        onClick={() => retryIntegration(provider)}
                        style={{
                          display: 'flex',
                          alignItems: 'center',
                          gap: '0.25rem',
                    ...
                    {integration && (
                      <button
                        onClick={() => toggleMockMode(integration)}
                        style={{
                          display: 'flex',
                          alignItems: 'center',
                          gap: '0.25rem',
                          padding: '0.375rem 0.75rem',
                          background: status === 'mock' ? 'hsla(270,40%,50%,0.12)' : 'none',
                          color: status === 'mock' ? '#8b7ac8' : '#4a4540',
                          border:
                            status === 'mock'
                              ? '1px solid hsla(270,40%,50%,0.25)'
                      ...
                      >
                        {status === 'mock' ? <ToggleRight size={12} /> : <ToggleLeft size={12} />}{' '}
                        Dev Mode
                      </button>
                    )}
                  </div>
        ...
        {activeTab === 'linktree' && (
          ...
          >
            <div>
              ...
              >
                <div>
                  <h2 style={{ fontSize: '0.875rem', fontWeight: 600, color: '#e8e4de' }}>
                    Active Linktree Buttons
                  </h2>
                  <p style={{ fontSize: '0.75rem', color: '#6b6560', marginTop: '0.125rem' }}>
                    Configure the ordered link destinations shown on your Linktree profile
                  </p>
                </div>
                <button
                      ...
                      >
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setNewLinkForm((p) => ({ ...p, label: e.target.value }))
                            }
                            placeholder="Newsletter — Get the Brief"
                            style={{
                              width: '100%',
                              padding: '0.5rem 0.625rem',
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setNewLinkForm((p) => ({ ...p, destination: e.target.value }))
                            }
                            placeholder="https://szlholdings.com/newsletter"
                            style={{
                              width: '100%',
                              padding: '0.5rem 0.625rem',
                              background: 'hsla(0,0%,100%,0.04)',
                        ...
                        <div>
                          ...
                          <input
                            value={newLinkForm.campaignTag}
                            onChange={(e) =>
                              setNewLinkForm((p) => ({ ...p, campaignTag: e.target.value }))
                            }
                        ...
                        <div>
                          ...
                          <input
                            value={newLinkForm.contentTag}
                            onChange={(e) =>
                              setNewLinkForm((p) => ({ ...p, contentTag: e.target.value }))
                            }
              ...
              {linktreeItems.length === 0 ? (
                ...
                >
                  ...
                  <button
                    onClick={seedRecommendedLinktree}
                    style={{
                      ...
                      {editingLinkId === item.id ? (
                          ...
                          >
                            <div>
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setEditingLinkData((p) => ({ ...p, label: e.target.value }))
                                }
                                style={{
                                  width: '100%',
                            ...
                            <div>
                              ...
                              <input
                                value={editingLinkData.destination}
                                onChange={(e) =>
                                  setEditingLinkData((p) => ({ ...p, destination: e.target.value }))
                                }
                                style={{
                                  width: '100%',
                                  padding: '0.35rem 0.5rem',
                            ...
                            <div>
                              ...
                              <input
                                value={editingLinkData.campaignTag}
                                onChange={(e) =>
                                  setEditingLinkData((p) => ({ ...p, campaignTag: e.target.value }))
                                }
                                style={{
                                  width: '100%',
                                  padding: '0.35rem 0.5rem',
                            ...
                            <div>
                              ...
                              <input
                                value={editingLinkData.contentTag}
                                onChange={(e) =>
                                  setEditingLinkData((p) => ({ ...p, contentTag: e.target.value }))
                                }
                                style={{
                                  width: '100%',
                                  padding: '0.35rem 0.5rem',
                        ...
                        >
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.1rem' }}>
                            ...
                            <button
                              onClick={() => moveLink(idx, 1)}
                              disabled={idx === linktreeItems.length - 1}
                              style={{
                                padding: '0.1rem',
                                background: 'none',
                                border: 'none',
                                color: idx === linktreeItems.length - 1 ? '#2a2520' : '#6b6560',
                                cursor: idx === linktreeItems.length - 1 ? 'default' : 'pointer',
                                lineHeight: 1,
                              }}
                            >
                              <ChevronDown size={11} />
                          ...
                          <div style={{ display: 'flex', gap: '0.25rem', flexShrink: 0 }}>
                            <button
                              onClick={() => updateLink(item.id, { isActive: !item.isActive })}
                              ...
                              style={{
                                ...
                                background: item.isActive
                                  ? 'hsla(120,30%,40%,0.12)'
                                  : 'hsla(0,0%,100%,0.04)',
                                border: `1px solid ${item.isActive ? 'hsla(120,30%,40%,0.2)' : 'hsla(0,0%,100%,0.08)'}`,
                                borderRadius: '4px',
                                color: item.isActive ? '#5a9c5a' : '#4a4540',
                                fontSize: '0.6875rem',
                                fontWeight: 700,
                                cursor: 'pointer',
                              }}
                            ...
                            <button
                              ...
                              style={{
                                padding: '0.25rem',
                                background: 'none',
                                border: '1px solid hsla(0,60%,55%,0.15)',
                                borderRadius: '4px',
                                color: '#c45a4a',
                                cursor: 'pointer',
                                display: 'flex',
            ...
            <div>
              ...
              >
                <div
                  style={{
                    ...
                    height: 56,
                    borderRadius: '50%',
                    background: 'linear-gradient(135deg, #d4a054, #8b7ac8)',
                ...
                <div style={{ fontSize: '0.75rem', color: '#6b6560', marginBottom: '1.25rem' }}>
                ...
                <p style={{ fontSize: '0.625rem', color: '#4a4540', marginTop: '1rem' }}>
                  Recommended: 6 buttons
                </p>
              </div>
              ...
              >
                ...
                {RECOMMENDED_LINKTREE.map((r, i) => (
                  ...
                  >
                    <span style={{ fontSize: '0.6875rem', color: '#4a4540', minWidth: '14px' }}>
                      {i + 1}.
                    </span>
