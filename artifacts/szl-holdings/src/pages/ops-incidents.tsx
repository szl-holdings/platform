...
const severityColor = (s: string) =>
  s === 'critical' ? '#ef4444' : s === 'major' ? '#f59e0b' : '#6b7280';
...
function IncidentDetailModal({
  ...
  const validNextStatuses: IncidentStatus[] = [
    incident.status,
    ...(VALID_NEXT[incident.status] ?? []),
  ];

  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch(`${BASE}/ops/incidents/${incident.id}`, { credentials: 'include' }).then((r) =>
        r.json(),
      ),
      fetch(
        `${BASE}/ops/runbooks?severity=${incident.severity}&services=${encodeURIComponent(incident.affected_services.join(','))}`,
        { credentials: 'include' },
      )
        .then((r) => r.json())
        .catch(() => ({ runbooks: [] })),
    ...
    >
      ...
      >
        ...
        <div style={card}>
          ...
          <p style={{ fontSize: '0.9rem', color: 'hsl(38,12%,80%)', lineHeight: 1.6, margin: 0 }}>
            {incident.description}
          </p>
          {incident.affected_services.length > 0 && (
            <div style={{ marginTop: 12 }}>
              <span style={{ fontSize: 11, color: 'hsl(210,5%,46%)', marginRight: 8 }}>
                Affected:
              </span>
            ...
            [
              {
                id: 'timeline',
                label: 'Timeline',
                icon: <MessageSquare size={12} style={{ display: 'inline', marginRight: 5 }} />,
              },
              {
                id: 'postmortem',
                label: 'Postmortem',
                icon: <FileText size={12} style={{ display: 'inline', marginRight: 5 }} />,
              },
              {
                id: 'runbooks',
                label: `Runbooks${runbooks.length ? ` (${runbooks.length})` : ''}`,
                icon: <BookOpen size={12} style={{ display: 'inline', marginRight: 5 }} />,
              },
            ] as const
          ).map((tab) => (
            <button
              ...
              style={{
                ...
                borderColor:
                  activeTab === tab.id ? 'hsla(210,55%,52%,0.35)' : 'hsla(0,0%,100%,0.08)',
            ...
            {loading ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
                {updates.map((u) => (
                  <div
                    key={u.id}
                    style={{ paddingLeft: '1rem', borderLeft: '2px solid hsla(0,0%,100%,0.07)' }}
                  >
                    <div
                      style={{
                        fontSize: '0.875rem',
        ...
        {activeTab === 'postmortem' && (
          <div style={card}>
            <div style={{ fontSize: 12, color: 'hsl(210,5%,46%)', marginBottom: '0.75rem' }}>
              Postmortem Notes
            </div>
            <textarea
              value={postmortem}
        ...
        {activeTab === 'runbooks' && (
          <div style={card}>
            <div style={{ fontSize: 12, color: 'hsl(210,5%,46%)', marginBottom: '0.875rem' }}>
              Runbooks matched by severity (
              <strong style={{ color: 'hsl(38,12%,74%)' }}>{incident.severity}</strong>) and
              affected services
            </div>
            {loading ? (
              <div style={{ color: 'hsl(210,5%,48%)', fontSize: 13 }}>Loading runbooks...</div>
            ) : runbooks.length === 0 ? (
              <div style={{ color: 'hsl(210,5%,48%)', fontSize: 13 }}>
                No runbooks matched this incident's severity or affected services. Browse all
                runbooks in the Runbooks section.
              </div>
            ) : (
                ...
                {runbooks.map((rb) => (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      ...
                      <span
                        style={{ fontSize: '0.875rem', fontWeight: 600, color: 'hsl(38,12%,88%)' }}
                      >
                        {rb.title}
                      </span>
                      <span
                        ...
                        {rb.tags.map((tag) => (
                          <span
                            ...
                            style={{
                              ...
                              padding: '1px 6px',
                              borderRadius: 4,
                              background: 'hsla(210,55%,52%,0.08)',
                              border: '1px solid hsla(210,55%,52%,0.16)',
                              color: 'hsl(210,55%,62%)',
                            }}
                          >
                            #{tag}
            ...
            <div>
              ...
              >
                ...
                {validNextStatuses.length === 1 && (
                  <span style={{ marginLeft: 6, color: 'hsl(210,5%,36%)' }}>(terminal)</span>
                )}
              </label>
              <select
                value={nextStatus}
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            ...
            <button
              ...
              style={{
                ...
                fontSize: 13,
                fontWeight: 600,
                background: 'hsla(210,55%,52%,0.18)',
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div>
            <label
              style={{ fontSize: 12, color: 'hsl(210,5%,48%)', display: 'block', marginBottom: 4 }}
            >
              Title *
            </label>
            <input
...
export default function OpsIncidentsPage() {
  ...
  const activeCnt = incidents.filter(
  ...
  return (
    ...
    >
      ...
      {showCreate && (
        <CreateIncidentModal onClose={() => setShowCreate(false)} onCreated={() => load()} />
      )}
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        >
          <div>
            ...
            <p style={{ fontSize: '0.875rem', color: 'hsl(210,5%,50%)' }}>
              Create, triage, and resolve platform incidents with full audit trail.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '4rem 0' }}>
            <CheckCircle
              size={32}
              style={{ color: '#10b981', margin: '0 auto 1rem', display: 'block' }}
            />
            <div
              style={{
                fontSize: '1rem',
            ...
            <div style={{ fontSize: 13, color: 'hsl(210,5%,46%)' }}>
            ...
            {filtered.map((inc) => (
              ...
              >
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      ...
                      {inc.affected_services.length > 0 && (
                        <span style={{ fontSize: 11, color: 'hsl(210,5%,44%)' }}>
                          Affects: {inc.affected_services.slice(0, 3).join(', ')}
                  ...
                  <ChevronDown
                    size={14}
                    style={{ color: 'hsl(210,5%,42%)', flexShrink: 0, marginTop: 4 }}
                  />
                </div>
              </div>
            ))}
