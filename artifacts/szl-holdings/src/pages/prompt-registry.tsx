  ...
  return (
    <div className="space-y-1">
      ...
      <div
        ...
        style={{ height: 6, background: 'rgba(239,68,68,0.2)' }}
      >
        <m.div
          initial={{ width: 0 }}
      ...
      <div className="grid grid-cols-3 gap-3 mb-3">
        ...
        >
          ...
          <div
            ...
            style={{ color: scoreDiff > 0 ? '#10b981' : scoreDiff < 0 ? '#ef4444' : '#f59e0b' }}
          >
            {scoreDiff > 0 ? '+' : ''}
            {scoreDiff.toFixed(1)}
          </div>
        ...
        >
          ...
          >
            {latencyDiffMs > 0 ? '+' : ''}
            {latencyDiffMs.toLocaleString()} ms
          </div>
        </div>
        <div
          className="rounded-md p-2.5 flex flex-col justify-between"
          style={{ background: `${rec.color}10`, border: `1px solid ${rec.color}20` }}
        >
          <rec.icon className="w-3.5 h-3.5" style={{ color: rec.color }} />
          <div className="text-[9px] font-semibold mt-1" style={{ color: rec.color }}>
            {rec.label}
          </div>
        </div>
      </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        background: isActive ? `${color}04` : 'rgba(255,255,255,0.01)',
      }}
    >
      <div
      ...
      >
        ...
        <div className="flex items-center gap-3 shrink-0">
          {version.evalMetadata?.score != null && (
            <div className="text-right">
              <div className="text-[11px] font-semibold tabular-nums" style={{ color }}>
                {version.evalMetadata.score.toFixed(1)}
              </div>
          ...
          {version.evalMetadata?.passRate != null && (
            <div className="text-right">
              <div className="text-[11px] font-semibold tabular-nums" style={{ color: '#10b981' }}>
                {(version.evalMetadata.passRate * 100).toFixed(0)}%
              </div>
              <div className="text-[9px]" style={{ color: 'rgba(255,255,255,0.2)' }}>
                pass
              </div>
          ...
          <div className="flex items-center gap-1.5">
            <button
              onClick={(e) => {
                e.stopPropagation();
                evalMutation.mutate(version.versionId);
              }}
            ...
            {!isActive && version.status !== 'archived' && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  promoteMutation.mutate(version.versionId);
                }}
            ...
            >
              {version.evalMetadata && (
                <div className="mt-3 space-y-3">
                  ...
                  <PassFailBar
                    passed={version.evalMetadata.passedCases}
                    failed={version.evalMetadata.failedCases}
                  />
                </div>
              )}
              {version.modelHints.preferredModel && (
                <div className="flex items-center gap-2 mt-2">
                  ...
                  >
                    {version.modelHints.preferredModel}
                    {version.modelHints.temperature != null &&
                      ` (temp ${version.modelHints.temperature})`}
                  </code>
                </div>
              )}
              <div
                className="rounded-md p-3 font-mono text-[10px] leading-relaxed whitespace-pre-wrap"
                style={{
              ...
              >
                ...
                {version.evalMetadata?.lastEvalAt && (
                  <span>Eval: {fmtDate(version.evalMetadata.lastEvalAt)}</span>
                )}
              </div>
            </div>
          </m.div>
...
function PromptDetailPanel({ id }: { id: string }) {
  ...
  if (error || !prompt) {
    return (
      <div
        className="flex items-center justify-center h-48 gap-2"
        style={{ color: 'rgba(255,255,255,0.3)' }}
      >
  ...
  return (
    ...
    >
      <div>
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <span
              className="text-[9px] px-2 py-1 rounded-full capitalize font-medium"
      ...
      {prompt.comparison && <ComparisonPanel comparison={prompt.comparison} color={color} />}
  ...
  return (
    <div className="min-h-screen" style={{ background: '#070a10' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        >
          <div
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg flex-1 max-w-xs"
            style={{
            ...
            <div className="flex gap-1">
              ...
              {domains.map((d) => (
                <button
                  ...
                  style={{
                    background:
                      domainFilter === d ? `${domainColor(d)}12` : 'rgba(255,255,255,0.02)',
        ...
        {isLoading ? (
          ...
          <div className="flex gap-4">
            {/* Left: prompt table */}
            <div
              className={`${selectedId ? 'w-[440px] shrink-0' : 'flex-1'} transition-all duration-200`}
            >
              {filtered.length === 0 ? (
                <div
                  className="flex items-center justify-center h-40 text-[12px]"
                  style={{ color: 'rgba(255,255,255,0.2)' }}
                >
                  No prompts found
                </div>
                ...
                <div className="space-y-1">
                  ...
                  {!selectedId && (
                    <div
                      className="grid grid-cols-12 px-4 py-1.5 mb-1 text-[9px] uppercase tracking-widest"
                      style={{ color: 'rgba(255,255,255,0.2)' }}
                    >
                        ...
                        {selectedId ? (
                          ...
                          <div className="flex items-center gap-3 px-3 py-2.5">
                            ...
                            <ChevronRight
                              ...
                              style={{ color: isSelected ? color : 'rgba(255,255,255,0.15)' }}
                            />
                          </div>
                        ) : (
                          // Full table row
          ...
          >
            {[
              { label: 'Total prompts', value: prompts.length, icon: Layers },
              {
                ...
                value: prompts.filter((p) => p.status === 'active').length,
                icon: CheckCircle2,
              },
              {
                label: 'Drafts',
              ...
              },
              {
                ...
                value: (() => {
                  ...
                  return (scored.reduce((s, p) => s + p.lastEvalScore!, 0) / scored.length).toFixed(
                    1,
                  );
                })(),
                icon: BarChart3,
