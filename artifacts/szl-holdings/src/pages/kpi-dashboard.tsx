...
function Sparkline({ bars = 8, accent }: { bars?: number; accent: string }) {
  const vals = Array.from(
    { length: bars },
    (_, i) => 30 + Math.abs(Math.sin(i * 1.8 + 0.7) * 55 + Math.cos(i * 0.9) * 20),
  );
    ...
    >
      ...
      >
        ...
        {href && (
          <a
            href={href}
            style={{ color: 'hsl(210,5%,38%)', transition: 'color 0.18s', textDecoration: 'none' }}
            onMouseEnter={(e) => {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        onMouseEnter={(e) => {
          if (!refreshing)
            (e.currentTarget as HTMLElement).style.background = 'hsla(0,0%,100%,0.07)';
        }}
        onMouseLeave={(e) => {
        ...
        <div style={{ textAlign: 'center', maxWidth: '360px' }}>
          <div
            style={{
              ...
              borderRadius: '50%',
              background:
                'radial-gradient(ellipse, hsla(214,80%,65%,0.1) 0%, hsla(0,0%,100%,0.03) 100%)',
              border: '1px solid hsla(214,80%,65%,0.15)',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
          ...
          <button
            ...
            style={{
              ...
              background: 'hsla(214,80%,55%,0.15)',
              color: 'hsl(214,80%,72%)',
              border: '1px solid hsla(214,80%,55%,0.25)',
              borderRadius: '8px',
              cursor: 'pointer',
              fontSize: '13px',
              fontWeight: 600,
            ...
            onMouseEnter={(e) => {
              (e.currentTarget as HTMLElement).style.background = 'hsla(214,80%,55%,0.22)';
            }}
            onMouseLeave={(e) => {
              (e.currentTarget as HTMLElement).style.background = 'hsla(214,80%,55%,0.15)';
            }}
          >
            Sign In
          </button>
  ...
  return (
    <m.div
      ...
      style={{
        borderRadius: '14px',
        background:
          'radial-gradient(ellipse at top left, hsla(139,62%,48%,0.06) 0%, hsla(0,0%,100%,0.018) 60%)',
    ...
    >
      <div
        style={{
          ...
          right: 0,
          height: '2px',
          background: 'linear-gradient(90deg, hsl(139,62%,48%)70, transparent)',
        }}
      />
      <div
        style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
          <div
            style={{
              ...
              alignItems: 'center',
              justifyContent: 'center',
              background: 'hsla(139,62%,48%,0.12)',
              border: '1px solid hsla(139,62%,48%,0.2)',
            }}
          >
        ...
        <div>
          ...
          >
            {[
              ...
              {
                label: 'Policy Blocks',
                value: health.policyBlockCount,
                icon: XCircle,
                color: 'hsl(0,72%,62%)',
              },
              {
                label: 'Approvals',
                value: health.approvalCount,
                icon: AlertTriangle,
                color: 'hsl(38,72%,58%)',
              },
              ...
              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                {Object.entries(health.domainBreakdown).map(([domain, count]) => (
                  <span
                    ...
                    style={{
                      fontSize: '10px',
                      color: DOMAIN_ACCENT[domain] ?? 'hsl(139,62%,55%)',
                      background: `${DOMAIN_ACCENT[domain] ?? 'hsl(139,62%,48%)'}12`,
                      padding: '2px 8px',
                      borderRadius: '4px',
                      border: `1px solid ${DOMAIN_ACCENT[domain] ?? 'hsl(139,62%,48%)'}28`,
                      fontWeight: 600,
                    }}
                  >
                    {domain} {count}
          ...
          <div style={{ marginTop: '0.75rem', fontSize: '10px', color: 'hsl(210,5%,36%)' }}>
            Avg latency{' '}
            {health.avgLatencyMs >= 60_000
              ? `${(health.avgLatencyMs / 60_000).toFixed(1)}m`
              : `${(health.avgLatencyMs / 1000).toFixed(1)}s`}{' '}
            · {health.recentRuns} run{health.recentRuns !== 1 ? 's' : ''} in last 24h
          </div>
        </div>
      )}

      {!loading && !health && (
        <p style={{ fontSize: '12px', color: 'hsl(210,5%,40%)', margin: 0 }}>
          Run health unavailable — agent runtime not connected.
        </p>
      )}
    </m.div>
  );
      ...
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 0 }}>
        <div
          style={{
            ...
            height: '60vw',
            borderRadius: '50%',
            background: 'radial-gradient(ellipse, hsla(214,80%,55%,0.055) 0%, transparent 70%)',
          }}
        />
        <div
          style={{
            ...
            height: '50vw',
            borderRadius: '50%',
            background: 'radial-gradient(ellipse, hsla(232,68%,50%,0.04) 0%, transparent 70%)',
          }}
        />
      </div>
      <SiteNav />
              ...
              >
                ...
                >
                  <div
                    style={{
                      ...
                      borderRadius: '8px',
                      background:
                        'linear-gradient(135deg, hsla(214,80%,55%,0.2), hsla(232,68%,50%,0.12))',
                      border: '1px solid hsla(214,80%,55%,0.2)',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                    }}
                ...
                <p style={{ fontSize: '13px', color: 'hsl(210,5%,50%)' }}>
                  {data
                    ...
                    : loading
                      ? 'Loading…'
                      : 'Real-time aggregate metrics across all platforms.'}
                </p>
              </m.div>
