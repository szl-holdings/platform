...
function StatusBadge({ status }: { status: string }) {
  ...
  const config = map[status] ?? { label: status, cls: "bg-muted text-muted-foreground border-border" };
  return (
    <span className={cn("text-[10px] font-semibold px-2 py-0.5 rounded-full border uppercase tracking-wider", config.cls)}>
      {config.label}
    </span>
  );
}
...
function TenantDetailPanel({
  ...
  const fetchConsentUrl = async () => {
    ...
    try {
      const data = await apiFetch<{ adminConsentUrl: string }>(`/admin/tenants/${tenant.id}/admin-consent-url`);
    ...
    >
      <div className="flex items-center justify-between px-5 py-4 border-b border-border/50">
        <div>
          ...
          <div className="text-[10px] text-muted-foreground font-mono mt-0.5">{tenant.azureTenantId}</div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="rounded-xl border border-border/50 bg-muted/20 p-3">
            <div className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1.5">Admin Consent</div>
            <StatusBadge status={tenant.adminConsentGranted} />
          </div>
          <div className="rounded-xl border border-border/50 bg-muted/20 p-3">
            <div className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1.5">Domain</div>
            <div className="text-xs font-medium text-foreground">{tenant.domain ?? "—"}</div>
          </div>
          <div className="rounded-xl border border-border/50 bg-muted/20 p-3">
            <div className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1.5">Provisioned</div>
            <div className="text-xs font-medium text-foreground">
              {tenant.provisionedAt ? new Date(tenant.provisionedAt).toLocaleDateString() : "Pending"}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/20 px-3 py-2">
            <code className="text-[11px] text-muted-foreground font-mono flex-1">{tenant.azureTenantId}</code>
            <button onClick={() => handleCopy(tenant.azureTenantId)} className="text-muted-foreground hover:text-foreground transition-colors">
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            {tenant.status === "pending" && (
              <button
                onClick={() => updateStatus.mutate({ status: "active", adminConsentGranted: "granted" })}
            ...
            >
              {loadingConsent ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Shield className="w-3.5 h-3.5" />}
              Consent URL
            </button>
        ...
        {consentUrl && (
          <div className="space-y-2">
            ...
            <div className="rounded-lg border border-border bg-muted/20 p-2.5 flex items-start gap-2">
              ...
              <button onClick={() => handleCopy(consentUrl)} className="text-muted-foreground hover:text-foreground">
        ...
        <div className="space-y-2">
          ...
          {[
            {
              ...
              detail: tenant.adminConsentGranted === "granted" ? "Consent granted" : "Pending consent",
            },
            {
              label: "Admin Consent",
              icon: Check,
            ...
            },
            {
              ...
              icon: Database,
              status: dvConnections.length === 0 ? "pending" : activeConnections.length > 0 ? "ok" : "warn",
              detail: dvConnections.length === 0
                ? "Not configured"
            ...
            },
            {
              ...
              detail: lastSync
                ...
                : dvConnections.length > 0 ? "Sync pending" : "No connections",
            },
          ].map(item => (
            <div key={item.label} className="flex items-center gap-3 rounded-lg border border-border/40 bg-muted/10 px-3 py-2">
              <div className={cn(
                "w-2 h-2 rounded-full flex-shrink-0",
                item.status === "ok" ? "bg-emerald-400" :
                item.status === "warn" ? "bg-amber-400" : "bg-muted-foreground/30"
              )} />
              ...
              <span className={cn(
                ...
                item.status === "warn" ? "text-amber-500" : "text-muted-foreground"
              )}>
                {item.detail}
              </span>
            </div>
          ...
          {totalSynced > 0 && (
            <div className="rounded-lg border border-border/40 bg-muted/10 px-3 py-2">
              <div className="text-[10px] text-muted-foreground mb-1">Records synced from Dataverse</div>
              <div className="text-xs font-bold text-foreground">{totalSynced.toLocaleString()}</div>
            </div>
          )}
        </div>
      </div>
...
export default function AzureTenantDashboardPage() {
  ...
  const filtered = tenants.filter(t => {
    const matchSearch = !search || t.displayName.toLowerCase().includes(search.toLowerCase()) || t.azureTenantId.includes(search);
  ...
  const stats = [
    ...
    { label: "Active", value: tenants.filter(t => t.status === "active").length, icon: CheckCircle2, color: "text-emerald-500" },
    { label: "Pending Consent", value: tenants.filter(t => t.adminConsentGranted === "pending").length, icon: Clock, color: "text-amber-500" },
    { label: "Suspended", value: tenants.filter(t => t.status === "suspended").length, icon: AlertCircle, color: "text-red-400" },
  ];
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className={cn("grid gap-6 transition-all", selectedTenant ? "grid-cols-1 md:grid-cols-[1fr_380px]" : "grid-cols-1")}>
          <div className="space-y-4">
            ...
            {isLoading ? (
              ...
              <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-8 text-center">
                ...
                <div className="text-sm font-medium text-foreground mb-1">Failed to load tenants</div>
              ...
              <div className="rounded-2xl border border-dashed border-border bg-muted/10 p-12 text-center">
                ...
                <p className="text-xs text-muted-foreground mb-6">
                  {search || statusFilter !== "all" ? "Adjust your filters" : "Onboard your first enterprise Azure AD tenant to get started."}
                </p>
                ...
                {filtered.map((tenant) => (
                  <m.div
                    ...
                    className={cn(
                      ...
                      selectedTenant?.id === tenant.id && "bg-primary/5 border-l-2 border-l-primary"
                    )}
                  >
                    <div className="min-w-0">
                      <div className="text-sm font-semibold text-foreground truncate">{tenant.displayName}</div>
                    ...
                    <div className="flex items-center gap-1">
                      {[
                        { color: tenant.status === "active" ? "bg-emerald-400" : "bg-amber-400", label: "SSO" },
                        { color: tenant.adminConsentGranted === "granted" ? "bg-emerald-400" : "bg-amber-400/60", label: "Consent" },
                        { color: "bg-muted-foreground/30", label: "SCIM" },
                      ].map(h => (
                        <div key={h.label} title={h.label} className={cn("w-2 h-2 rounded-full", h.color)} />
                      ))}
                    </div>

                    <ChevronRight className={cn("w-4 h-4 text-muted-foreground/50 transition-transform", selectedTenant?.id === tenant.id && "rotate-90")} />
                  </m.div>
                ))}
              </div>
            )}
