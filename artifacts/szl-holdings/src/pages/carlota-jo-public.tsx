...
const SERVICE_QUALITIES = [
  ...
  {
    icon: Zap,
    title: "Counsel-Powered Execution",
    body: "Service workflows routed through Counsel — intake approvals, milestone confirmations, delivery authorizations — with human-in-the-loop gates and a complete audit trail behind every engagement.",
  },
];
...
const ALLOY_INHERITED = [
  {
    ...
    description: "Intake authorizations, service scope changes, and delivery confirmations require explicit human approval through Counsel's HITL gate — even in a high-trust service context.",
  },
  {
  ...
  },
  {
    ...
    description: "Counsel's role-based access infrastructure ensures client records are isolated. No cross-client data exposure. No operator sees beyond their assigned engagement scope.",
  },
  {
    capability: "Export Governance",
    description: "Deliverables exit the platform through Counsel's controlled export layer — with authorization tracking, recipient confirmation, and delivery record intact.",
  },
];
        ...
        <main id="main-content" >
              ...
              <div style={{ display: "grid", gap: "clamp(2.5rem,5vw,4rem)", alignItems: "start" }} className="lg:grid-cols-[1.2fr_0.8fr]">
                <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.05 }}>
                  ...
                  <p style={{ fontSize: "clamp(0.9375rem,1.6vw,1rem)", lineHeight: 1.72, color: "hsl(214,7%,52%)", maxWidth: "50ch", marginBottom: "2.25rem" }}>
                    Powered by Counsel for governed execution — every service action logged, attributed, and auditable. Excellence is consistent because the platform makes it structural.
                  </p>
          ...
          <section style={{ borderBottom: "1px solid var(--color-szl-border)", padding: "var(--space-section-md) 0" }}>
            <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "0 var(--space-content-x)" }}>
              <m.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45 }}>
                <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.6875rem", fontWeight: 500, letterSpacing: "0.12em", textTransform: "uppercase", color: "hsl(280,50%,65%)", marginBottom: "1rem" }}>
                  What Carlota Jo Inherits from Counsel
                </p>
                ...
                <p style={{ fontSize: "0.9375rem", lineHeight: 1.70, color: "hsl(214,7%,58%)", maxWidth: "52ch", marginBottom: "2.5rem" }}>
                  Even in a high-trust consulting context, every service action passes through Counsel's governed pipeline. The same infrastructure managing legal approvals, maritime decisions, and security remediations governs client intake, service delivery, and engagement close.
                </p>
              </m.div>
