...
const SYNERGIES = [
  {
    ...
    description: "PARAGON SOC capabilities bundled with SEXTANT maritime intelligence subscriptions. Ship operators managing high-value cargo lanes have regulatory cybersecurity obligations under IMO 2021 resolution MSC-FAL.1/Circ.3. PARAGON provides the cyber layer; SEXTANT provides the operational layer.",
    mechanism: "SEXTANT identifies vessels with cyber risk indicators (flag state, ownership structure, cargo type). PARAGON provides automated cyber posture assessment and continuous monitoring. Bundle priced at 40% premium to standalone.",
    pipeline: "38 SEXTANT enterprise clients identified as qualified for cyber bundle",
  ...
  },
  {
    ...
    description: "DOMAINE's ACRIS-linked distress signal detection feeds PRISM's matter intake pipeline. When DOMAINE identifies a property in financial distress (foreclosure filings, tax lien escalation, ownership transfer under duress), PRISM receives a referral alert for potential litigation matter.",
    mechanism: "DOMAINE automated distress classifier publishes events to shared FORGE event bus. PRISM matter intake agent evaluates eligibility and creates pre-qualified matter shells. Attorneys receive pre-populated intake with full property intelligence.",
    pipeline: "DOMAINE averages 340 distress signals monthly across NYC portfolio",
  ...
  },
  {
    ...
    description: "Carlota Jo serves principals who own or charter private vessels. SEXTANT provides the intelligence layer for their maritime assets — voyage planning, flag registry monitoring, AIS tracking of chartered vessels, and cargo movement alerts for commodity-invested clients.",
    mechanism: "Carlota Jo relationship managers identify vessel-owning clients. SEXTANT onboarded as premium concierge tier with white-glove support. Revenue split with Carlota Jo on referral basis.",
    pipeline: "14 existing Carlota Jo clients with identified vessel interests",
    conversionTarget: "SEXTANT premium tier pricing → $1.9M ARR at full conversion",
  },
  {
    id: "lyte-aegis",
    source: "KORA",
    ...
    description: "KORA's operational signal intelligence feeds directly into PARAGON threat detection. Anomalies in business process metrics (sudden SLA degradation, unusual access patterns, workflow failures) are correlated with PARAGON threat signals for early breach detection — operational data as a security sensor.",
    mechanism: "KORA signals normalized through FORGE event bus. PARAGON SIEM ingests as custom telemetry feed. Joint clients get correlated view: business anomaly + security context in single interface.",
    pipeline: "22 KORA enterprise clients are Aegis-eligible by sector profile",
    conversionTarget: "PARAGON add-on at 30% uplift → $3.1M in joint contract value",
  },
  {
    id: "terra-carlota",
    source: "DOMAINE",
    ...
    description: "Carlota Jo principals require institutional-grade real estate intelligence for acquisition decisions, portfolio monitoring, and market timing. DOMAINE provides proprietary market data, cap rate analytics, and distress signals that give UHNW clients CRE intelligence their advisors cannot match.",
    mechanism: "DOMAINE generates bespoke property intelligence reports for Carlota Jo clients. White-labeled as 'Estate Intelligence by Carlota Jo'. Delivered as premium advisory add-on.",
  ...
  },
  {
    ...
    description: "PRISM clients in regulated sectors (financial, maritime, healthcare) face increasing cybersecurity regulatory mandates. PRISM identifies compliance obligations during matter intake; PARAGON delivers the technical compliance solution. Legal identifies the need; security fills it.",
    mechanism: "PRISM compliance assessment flags cyber regulatory exposure. PARAGON receives warm referral with regulatory context pre-populated. Joint proposal developed with legal + technical remediation bundled.",
    pipeline: "45 PRISM matters identified with cyber compliance component",
    conversionTarget: "PARAGON retainer at $49K avg → $2.2M pipeline at 100% conversion",
  },
];
