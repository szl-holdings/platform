...
const WORLDLINE_PROPERTIES = [
  {
    ...
    desc: "Every signal ingestion, Twin state change, AI recommendation, operator decision, and execution event is recorded on the Worldline in chronological order. The Worldline is the single source of truth for what happened in a workflow, when, and in what sequence.",
  },
  {
    icon: Shield,
    name: "Immutable and tamper-evident",
    desc: "Worldline events are written once and cannot be modified. Each event carries a cryptographic commitment that allows any observer to verify the record has not been altered. Retroactive modification of the timeline is structurally prevented.",
  },
  {
    icon: Search,
    name: "Queryable by subject and time",
    desc: "The Worldline is queryable by subject (a specific matter, voyage, property, or threat), by event type, by actor, or by time window. Governance teams can reconstruct the exact decision context for any moment in a workflow's history.",
  },
  {
    icon: Clock,
    name: "Decision replay support",
    desc: "Any point in the Worldline can be used as a basis for reconstructing the full context that existed at that moment — including the Twin state, the signals that had arrived, and the recommendations that were pending. This supports post-hoc review without requiring a separate audit database.",
  },
  {
    icon: AlertCircle,
    name: "Exception and escalation events",
    desc: "Escalations, missed checkpoints, policy violations, and human overrides are all first-class Worldline events. They are queryable independently for exception review and pattern analysis.",
  },
  {
    icon: BarChart3,
    name: "Aggregated timeline views",
    desc: "Lyte's What Changed surface and Today dashboard are both derived from the Worldline. The Worldline is the common data layer that powers all observability surfaces — there is no separate metrics store or shadow database.",
  },
];
...
export default function DocsWorldlinePage() {
  usePageMeta({
    title: "Worldline — Docs — SZL Holdings",
    description: "Worldline system documentation: the unified, immutable chronological event and decision timeline that powers all observability and audit surfaces in Lyte + Alloy.",
      ...
      <main>
        ...
        <section className="border-b border-white/10">
          <div className="mx-auto max-w-6xl px-6 py-20 lg:px-8 lg:py-28">
            <div className="flex items-center gap-2 text-sm text-white/40">
              ...
              <span>/</span>
              <span className="text-white/60">Worldline</span>
            </div>
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-white/65">
              The Worldline is the unified, immutable chronological record of everything that happens in a Lyte
              + Alloy workflow — signals, Twin updates, AI recommendations, operator decisions, execution events,
              and escalations. It is the common data layer beneath all observability surfaces and the foundation
              of the audit trail.
            </p>
          </div>
        </section>
        ...
        <section className="border-b border-white/10">
          <div className="mx-auto max-w-6xl px-6 py-16 lg:px-8">
            ...
            <h2 className="mt-3 text-2xl font-semibold text-white">What the Worldline provides</h2>
        ...
        <section className="border-b border-white/10">
          <div className="mx-auto max-w-6xl px-6 py-16 lg:px-8">
            ...
            <h2 className="mt-3 text-2xl font-semibold text-white">First-class Worldline event types</h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-white/55">
              These are the core event types recorded on the Worldline. All are queryable by type, subject,
              actor, or time window through the control plane.
            </p>
        ...
        <section className="border-b border-white/10">
          <div className="mx-auto max-w-6xl px-6 py-16 lg:px-8">
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-white/40">Built on the Worldline</p>
            <h2 className="mt-3 text-2xl font-semibold text-white">Surfaces powered by Worldline data</h2>
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {[
                { label: "What Changed", detail: "Surfaces the most significant Worldline events since the last operator session." },
                { label: "Today dashboard", detail: "Aggregates priority signals across all active subjects into one operational view." },
                { label: "Proof chain", detail: "Derives output provenance by traversing the Worldline back from a decision event to its source signals." },
                { label: "Audit trail", detail: "Exposes the full Worldline record to governance teams through the control plane query interface." },
              ].map((s) => (
          ...
          <div className="mx-auto max-w-6xl px-6 py-16 lg:px-8">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                { label: "Control plane", href: "/docs/control-plane", detail: "GraphQL API for querying Worldline data" },
                { label: "Trust", href: "/docs/trust", detail: "How the Worldline supports governance requirements" },
                { label: "Model mesh", href: "/docs/model-mesh", detail: "AI model events in the Worldline" },
