...
function FieldInput({
  ...
  const baseClass =
    'w-full bg-muted/30 border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary';
  return (
    <div>
      ...
      <div className="flex" style={{ minHeight: 260, background: '#0d0f18' }}>
        <div
          ...
          style={{
            ...
            background: `linear-gradient(180deg, ${primary}12 0%, transparent 100%)`,
          }}
        >
            ...
            {['Tenants', 'Users', 'Revenue', 'Uptime'].map((metric, i) => (
              ...
              >
                ...
                <div
                  ...
                  style={{ color: i === 0 ? primary : 'rgba(255,255,255,0.75)' }}
                >
                  {['24', '1,240', '$48K', '99.9%'][i]}
                </div>
              </div>
    ...
    <div className="rounded-xl border border-border overflow-hidden text-xs">
      <div className="bg-muted/20 px-4 py-2 border-b border-border flex items-center gap-2">
        ...
        <span className="text-muted-foreground">
          Email Preview — From: <span className="text-foreground font-medium">{fromName}</span>
        </span>
      </div>
          ...
          <div className="flex items-center gap-2 mb-4 pb-3 border-b border-gray-100">
            {form.logoUrl ? (
              ...
              <div
                ...
                style={{
                  background: `linear-gradient(135deg, ${primary}, ${form.accentColor || '#7c3aed'})`,
                }}
              >
                {name.charAt(0)}
              </div>
...
export default function TenantBrandingPage() {
  ...
  const { data: tenant, isLoading: tenantLoading } = useStandardQuery<AzureTenant>({
    ...
    queryFn: () =>
      apiFetch<{ tenant: AzureTenant }>(`/admin/tenants/${tenantId}`).then((r) => r.tenant),
    enabled: tenantId !== null,
  });

  const { data: brandingData, isLoading: brandingLoading } = useStandardQuery<{
  ...
  const resetMutation = useStandardMutation({
    mutationFn: () => apiFetch(`/admin/tenants/${tenantId}/branding`, { method: 'DELETE' }),
    onSuccess: () => {
            ...
            <div className="p-6 max-w-2xl space-y-8">
              ...
              <section>
                ...
                <div className="space-y-4 rounded-xl border border-border p-4 bg-card">
                  ...
                  <div className="text-[10px] text-muted-foreground">
                    Provide publicly accessible HTTPS URLs. Logos display best at 2:1 aspect ratio,
                    minimum 200×100 px.
                  </div>
                </div>
              </section>
              ...
              <section>
                ...
                <div className="space-y-4 rounded-xl border border-border p-4 bg-card">
                  ...
                  />
                  <div className="text-[10px] text-muted-foreground">
                    Colors are applied to sidebar highlights, buttons, charts, and data badges. Use
                    hex format (e.g. #6366f1).
                  </div>
                </div>
              </section>
              ...
              <section>
                ...
                <div className="space-y-4 rounded-xl border border-border p-4 bg-card">
                  ...
                  />
                  <div className="text-[10px] text-muted-foreground">
                    These settings customize all notification emails sent to users from this tenant.
                    The sending address remains SZL's managed domain for deliverability.
                  </div>
                </div>
              </section>
            </div>
