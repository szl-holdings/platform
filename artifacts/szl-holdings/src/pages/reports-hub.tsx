      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span
              className={`text-xs font-medium px-2 py-0.5 rounded ${STATUS_COLORS[report.status]}`}
            >
              {report.status}
            </span>
...
function GenerateModal({
  ...
  const TEMPLATES = [
    {
      key: 'szl_quarterly_investor',
      label: 'SZL Holdings — Quarterly Investor Letter',
      domain: 'szl_holdings',
      reportType: 'quarterly_investor_letter',
    },
    {
      key: 'szl_portfolio',
      label: 'SZL Holdings — Portfolio Overview',
      domain: 'szl_holdings',
      reportType: 'portfolio_overview',
    },
    {
      key: 'carlota_engagement_summary',
      label: 'Carlota Jo — Engagement Summary',
      domain: 'carlota_jo',
      reportType: 'engagement_summary',
    },
    {
      key: 'aegis_security_assessment',
      label: 'PARAGON — Security Assessment Report',
      domain: 'aegis',
      reportType: 'security_assessment',
    },
    {
      key: 'terra_property_analysis',
      label: 'DOMAINE — Property Analysis Report',
      domain: 'terra',
      reportType: 'property_analysis',
    },
    {
      key: 'vessels_voyage',
      label: 'SEXTANT — Voyage Report',
      domain: 'vessels',
      reportType: 'voyage_report',
    },
    {
      key: 'lyte_weekly_briefing',
      label: 'KORA — Weekly Operations Briefing',
      domain: 'lyte',
...
function DistributionLog({ reportId, autoRefreshMs }: { reportId: string; autoRefreshMs?: number }) {
  ...
  const { data, isLoading } = useStandardQuery({
    ...
    refetchInterval: autoRefreshMs ?? 10_000,
  });
...
export default function ReportsHub() {
  ...
  const handleRequestApproval = async (reportId: string) => {
    try {
      await apiFetch(`/reports/${reportId}/request-approval`, {
        method: 'POST',
        body: JSON.stringify({}),
      });
  ...
  return (
    <div className="min-h-screen bg-[#0a0c10] text-zinc-200">
      ...
      <div className="border-b border-zinc-800 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-zinc-500 mt-0.5">
              Industrial report generation & document intelligence pipeline
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
              ...
              {stats.byDomain.map((d) => (
                <button
                  ...
                  onClick={() =>
                    setFilters((f) => ({
                      ...f,
                      domain: f.domain === d.domain ? undefined : d.domain,
                    }))
                  }
                  ...
                  style={{
                    borderColor:
                      filters.domain === d.domain
                        ? DOMAIN_COLORS[d.domain] || '#94a3b8'
                        : '#3f3f46',
                    backgroundColor:
                      filters.domain === d.domain
                        ? `${DOMAIN_COLORS[d.domain] || '#94a3b8'}15`
                        : 'transparent',
                  }}
                >
                  <span
                    className="text-xs font-medium"
                    style={{ color: DOMAIN_COLORS[d.domain] || '#94a3b8' }}
                  >
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          >
            ...
            {(['draft', 'review', 'approved', 'distributed', 'archived'] as ReportStatus[]).map(
              (s) => (
      ...
      {selectedReport && (
        <div className="fixed inset-0 bg-black/60 z-40" onClick={() => setSelectedReport(null)}>
          <div
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm text-zinc-300">
                    {DOMAIN_LABELS[selectedReport.domain] || selectedReport.domain}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span
                    className={`text-xs px-2 py-0.5 rounded ${STATUS_COLORS[selectedReport.status]}`}
                  >
                ...
                {selectedReport.pdfSizeBytes && (
                  <div>
                    ...
                    <p className="text-sm text-zinc-300">
                      {(selectedReport.pdfSizeBytes / 1024).toFixed(0)} KB
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="pt-4 border-t border-zinc-800 space-y-2">
                ...
                {selectedReport.status === 'draft' && (
                  <button
                    onClick={() => {
                      handleRequestApproval(selectedReport.reportId);
                ...
                {selectedReport.status === 'review' && (
                  <button
                    onClick={() => {
                      handleApprove(selectedReport.reportId);
                      setSelectedReport(null);
                    }}
                ...
                {selectedReport.status === 'approved' && (
                  <button
                    onClick={() => {
                      setDistributeReportId(selectedReport.reportId);
                      setSelectedReport(null);
                    }}
