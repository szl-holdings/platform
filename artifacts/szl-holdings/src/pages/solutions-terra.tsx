...
const CAPABILITIES = [
  { icon: AlertOctagon, title: "Distress Property Detection", body: "KORA monitors market signals, tax records, foreclosure filings, and price anomalies to surface distressed opportunities before they're widely visible." },
  ...
  { icon: Map, title: "Market Signal Monitoring", body: "DOMAINE watches the signals that matter to your acquisition thesis — cap rate compression, permit activity, absorption rates — and surfaces deviations." },
  { icon: Workflow, title: "Broker Workflow Routing", body: "FORGE routes acquisition tasks, NDA workflows, due diligence requests, and closing steps to the right person with the right context and deadline." },
...
const USE_CASES = [
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "FORGE routes renewal workflow to property management team — outreach tasks, broker instruction, and renewal term proposal assigned with deadlines" },
  ...
  },
  {
    ...
    steps: [
      ...
      { text: "DOMAINE surfaces the underlying signals: three distressed sales reset comps, new mixed-use supply under permit, absorption rate trending down" },
...
function UseCaseLane({ useCase, delay }: { useCase: typeof USE_CASES[0]; delay: number }) {
  return (
    ...
    >
      <div>
        <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.625rem", fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--color-terra)", opacity: 0.85 }}>{useCase.label}</span>
          ...
          <li key={i} style={{ display: "flex", gap: "0.875rem", alignItems: "flex-start" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
              <div style={{
                ...
                background: step.signal ? `hsla(140,50%,38%,0.15)` : step.proof ? `hsla(152,70%,50%,0.12)` : "hsla(214,12%,14%,1)",
                border: step.signal ? `1px solid hsla(140,50%,38%,0.40)` : step.proof ? `1px solid hsla(152,70%,50%,0.30)` : "1px solid hsla(0,0%,100%,0.08)",
                fontSize: "0.6rem", fontWeight: 700, color: step.signal ? "hsl(140,50%,62%)" : step.proof ? "hsl(152,70%,55%)" : "hsl(214,7%,52%)",
            ...
            <div style={{ paddingTop: "0.2rem" }}>
              <p style={{ fontSize: "0.8125rem", lineHeight: 1.62, color: step.signal ? "hsl(38,8%,88%)" : step.proof ? "hsl(152,40%,72%)" : "hsl(214,7%,62%)" }}>
                {step.signal && <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.5625rem", fontWeight: 700, letterSpacing: "0.10em", textTransform: "uppercase", color: "hsl(140,50%,58%)", marginRight: "0.4rem" }}>Signal</span>}
...
export default function SolutionsTerraPage() {
  const __pageMeta = usePageMeta({
    title: "DOMAINE — Real Estate Intelligence · SZL Holdings",
    description: "NYC distress property pipeline, ownership entity graph, deal pipeline, broker workflow. DOMAINE is the real estate intelligence domain pack — built on the same governance infrastructure as every SZL product.",
            ...
            <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "0 var(--space-content-x)" }}>
              <m.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "1.75rem" }}>
                  ...
                  <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.6875rem", fontWeight: 600, letterSpacing: "0.10em", textTransform: "uppercase", color: "var(--color-terra)", opacity: 0.9 }}>DOMAINE</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1.75rem" }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "0.625rem" }}>
                      <h1 style={{ fontSize: "clamp(2rem,4.5vw,3.25rem)", fontWeight: 600, letterSpacing: "-0.028em", lineHeight: 1.08 }}>DOMAINE</h1>
                ...
                <p style={{ fontSize: "clamp(1rem,1.8vw,1.125rem)", lineHeight: 1.72, color: "hsl(214,7%,64%)", maxWidth: "52ch", marginBottom: "2.25rem" }}>
                  A data-rich, execution-poor industry finally getting a command layer. DOMAINE
