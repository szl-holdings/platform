...
const BLOCK_PALETTE: BlockDef[] = [
  {
    ...
    label: 'Cover Page',
    icon: '□',
    description: 'Title page with branding and classification',
    defaultData: {
      title: 'Report Title',
  ...
  },
  {
    ...
    icon: '◈',
    description: 'Highlighted summary block with accent bar',
    defaultData: { text: 'Executive summary text goes here...' },
  },
  {
    type: 'section_header',
    label: 'Section Header',
    icon: '—',
    description: 'Section divider with label',
    defaultData: { text: 'Section Name' },
  },
  {
    type: 'body_text',
    label: 'Body Text',
    ...
    defaultData: { text: 'Body text goes here...' },
  },
  {
    type: 'metrics_row',
    label: 'Metrics Row',
    icon: '▦',
    description: 'Row of KPI metric cards',
    defaultData: {
      metrics: [
  ...
  },
  {
    ...
    label: 'Data Table',
    icon: '⊞',
    description: 'Structured data table with headers',
    defaultData: {
  ...
  },
  {
    ...
    label: 'Line Chart',
    icon: '∿',
    description: 'Line chart for trend data',
    defaultData: {
      title: 'Trend',
      points: [
        { label: 'Jan', value: 40 },
  ...
  },
  {
    ...
    label: 'Gauge / Score',
    icon: '◎',
    description: 'Gauge/score indicator',
    defaultData: { title: 'Score', label: 'Performance', value: 72, max: 100 },
  },
  {
    type: 'status_grid',
    label: 'Status Grid',
    ...
    defaultData: {
      items: [
        { name: 'Service A', status: 'active', detail: 'Operating normally' },
        { name: 'Service B', status: 'warning', detail: 'Elevated latency' },
      ],
    },
  },
  {
    ...
    icon: '≡',
    description: 'Label/value detail pairs',
    defaultData: {
      cols: 2,
      pairs: [
  ...
  },
  {
    ...
    defaultData: {
      events: [
        { date: 'January 2026', title: 'Event 1', status: 'completed' },
        { date: 'February 2026', title: 'Event 2', status: 'active' },
        { date: 'March 2026', title: 'Event 3', status: 'pending' },
      ],
    },
  },
  {
    ...
    icon: '⚠',
    description: 'Distress/risk score indicator',
    defaultData: { score: 65, description: 'Multi-factor risk assessment' },
  },
  {
    type: 'risk_matrix',
    label: 'Risk Matrix',
    ...
    defaultData: {
      risks: [
        { title: 'Risk 1', likelihood: 'medium', impact: 'high', description: 'Risk description' },
      ],
    },
  },
  {
    ...
    icon: '✍',
    description: 'Signature/authorization block',
    defaultData: { name: 'Name', title: 'Title' },
  },
  {
    type: 'page_break',
    label: 'Page Break',
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <span className="text-zinc-500 text-sm font-mono w-5 text-center select-none">
          {def?.icon || '□'}
        </span>
...
export default function ReportBuilder() {
  ...
  const loadDomainTemplate = async (key: string) => {
    if (!key) return;
    try {
      const res = await fetch(`${API}/reports/templates/built-in/${key}`, {
        credentials: 'include',
      });
  ...
  const handleSaveTemplate = async () => {
    if (blocks.length === 0) {
      setMessage({ type: 'error', text: 'Add blocks to save a template' });
      return;
    }
    setSaving(true);
    setMessage(null);
  ...
  return (
    <div className="min-h-screen bg-[#0a0c10] text-zinc-200 flex flex-col">
      ...
      <div className="border-b border-zinc-800 px-4 py-3 flex items-center gap-4">
        <a href={`${BASE}/reports`} className="text-zinc-500 hover:text-zinc-300 text-sm">
          ← Reports Hub
        </a>
      ...
      {message && (
        <div
          className={`px-4 py-2 text-xs ${message.type === 'success' ? 'bg-emerald-950 text-emerald-400' : 'bg-red-950 text-red-400'}`}
        >
          {message.text}
        </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Block Palette */}
        <div
          className={`border-r border-zinc-800 bg-zinc-950 transition-all ${showPalette ? 'w-56' : 'w-10'} shrink-0 overflow-y-auto`}
        >
          <div className="flex items-center justify-between p-2 border-b border-zinc-800">
            {showPalette && (
              <p className="text-xs text-zinc-500 uppercase tracking-widest px-1">Blocks</p>
            )}
            <button
          ...
          {blocks.length === 0 ? (
            <div className="border-2 border-dashed border-zinc-800 rounded-xl py-20 text-center">
              ...
              <p className="text-zinc-700 text-xs">
                Or load a domain template from the toolbar above
              </p>
            </div>
          ) : (
