...
const WORKFLOW_STEPS = [
  ...
  },
  {
    ...
    body: "Remediation steps and response actions routed to the right analyst through Counsel with SLA tracking, escalation path, and full context. No raw alert dumps. Structured, prioritized action queues.",
  },
  {
    step: "05",
    label: "Traceability",
...
const CAPABILITIES = [
  ...
  { icon: Zap, title: "Incident Response Routing", body: "When a threat signal crosses threshold, Counsel routes the incident to the right analyst with full context, playbook reference, and human-gated approval chain ready." },
...
const ALLOY_INHERITED = [
  {
    ...
    description: "Every remediation action — patching, isolation, privilege revocation — requires analyst approval through Counsel's HITL gate before execution. No autonomous remediation.",
  },
  {
  ...
  },
  {
    ...
    description: "PARAGON integrates with SIEM, EDR, SOAR, and identity platforms through Counsel's connector layer — execution happens in existing tools without disruption.",
  },
  {
...
export default function AegisPublicPage() {
  const __pageMeta = usePageMeta({
    title: "PARAGON — Threat Twin & Security Operations Command | SZL Holdings",
    description: "PARAGON gives security teams a command layer above their threat exposure. Threat Twin, CISA KEV scoring, NIST NVD context, incident routing, and compliance-ready audit trail.",
        ...
        <main id="main-content" >
          ...
          <div style={{ background: "hsla(222,60%,50%,0.07)", borderBottom: "1px solid hsla(222,60%,50%,0.18)", padding: "0.75rem var(--space-content-x)" }}>
            <div style={{ maxWidth: "1280px", margin: "0 auto", display: "flex", alignItems: "center", gap: "0.75rem", flexWrap: "wrap" }}>
              ...
              <span style={{ fontSize: "0.8125rem", color: "hsl(214,7%,60%)", lineHeight: 1.5 }}>
                PARAGON is in active development. Architecture is established and core capabilities are being built. We are not in general availability. If this fits your security operations environment, we want to have a direct conversation.
              </span>
            </div>
          </div>
              ...
              <m.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.625rem", marginBottom: "1.75rem" }}>
                  ...
                  <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.6875rem", fontWeight: 600, letterSpacing: "0.10em", textTransform: "uppercase", color: "hsl(222,60%,62%)" }}>
                    PARAGON · Threat Intelligence & Security Operations
                  </span>
                </div>
              </m.div>
              ...
              <div style={{ display: "grid", gap: "clamp(2.5rem,5vw,4rem)", alignItems: "start" }} className="lg:grid-cols-[1.2fr_0.8fr]">
                <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.05 }}>
                  ...
                  <p style={{ fontSize: "clamp(1rem,1.8vw,1.125rem)", lineHeight: 1.72, color: "hsl(214,7%,64%)", maxWidth: "50ch", marginBottom: "0.875rem" }}>
                    PARAGON gives security teams a command layer above their threat exposure. Threat Twin maps every CVE and identity signal to your actual environment. Counsel routes response through governed analyst workflows — with human approval at every consequential action.
                  </p>
              ...
              <div style={{ display: "grid", gap: "clamp(3rem,6vw,5rem)", alignItems: "start" }} className="lg:grid-cols-2">
                <m.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45 }}>
                  ...
                  <p style={{ fontSize: "0.9375rem", lineHeight: 1.70, color: "hsl(214,7%,62%)", maxWidth: "46ch", marginBottom: "1.25rem" }}>
                    Security operations is a domain drowning in signal volume and starved for structured action. The typical SIEM gives analysts raw alerts. The typical SOAR automates without sufficient human judgment. PARAGON applies Counsel's governed operating loop to the security workflow — so analysts act on context, not noise, and every action has a traceable record.
                  </p>
                  <p style={{ fontSize: "0.9375rem", lineHeight: 1.70, color: "hsl(214,7%,54%)", maxWidth: "46ch" }}>
                    The architecture mirrors Counsel's operating model: a structured threat object (Threat Twin), signal enrichment from authoritative intelligence sources, governed response routing through Counsel, and a compliance-ready audit footprint for every security decision.
                  </p>
                </m.div>
          ...
          <section style={{ borderBottom: "1px solid var(--color-szl-border)", padding: "var(--space-section-md) 0" }}>
            <div style={{ maxWidth: "1280px", margin: "0 auto", padding: "0 var(--space-content-x)" }}>
              <m.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45 }}>
                <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.6875rem", fontWeight: 500, letterSpacing: "0.12em", textTransform: "uppercase", color: "hsl(222,60%,62%)", marginBottom: "1rem" }}>
                  What PARAGON Inherits from Counsel
                </p>
                <h2 style={{ fontSize: "clamp(1.5rem,3.5vw,2.25rem)", fontWeight: 600, letterSpacing: "-0.022em", lineHeight: 1.18, maxWidth: "32ch", marginBottom: "1.25rem" }}>
                  The execution fabric comes with every lane. PARAGON builds on top of it.
                </h2>
                <p style={{ fontSize: "0.9375rem", lineHeight: 1.70, color: "hsl(214,7%,58%)", maxWidth: "52ch", marginBottom: "2.5rem" }}>
                  PARAGON does not rebuild what Counsel already provides. Every capability below is inherited from the shared platform — the same infrastructure governing Counsel, SEXTANT, DOMAINE, and Carlota Jo.
                </p>
              </m.div>
              ...
              <div style={{ display: "grid", gap: "clamp(3rem,6vw,5rem)", alignItems: "start" }} className="lg:grid-cols-2">
                <m.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45 }}>
                  ...
                  <p style={{ fontSize: "0.9375rem", lineHeight: 1.70, color: "hsl(214,7%,62%)", maxWidth: "46ch" }}>
                    PARAGON connects to CISA, NIST, Microsoft Security, and your endpoint and identity platforms — weaving authoritative threat intelligence into each environment's threat twin so analysts have context before they act.
                  </p>
                </m.div>
                <m.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45, delay: 0.08 }}>
                ...
                {[
                  {
                    ...
                    description: "Threat Twin model, signal integration schema, Counsel connector design, and compliance audit trail structure are defined.",
                  },
                  {
                    label: "Core Capabilities",
                    status: "In Development",
                    statusColor: "hsl(40,85%,55%)",
                    description: "CISA KEV integration, NIST NVD enrichment, XDR normalization, and Counsel-gated analyst workflow are in active development.",
                  },
                  {
                    label: "General Availability",
                    status: "Not Yet",
                    statusColor: "hsl(214,7%,50%)",
                    description: "PARAGON is not in general availability. We are evaluating design partner fit with a small number of security-sensitive organizations.",
                  },
                ].map((item, i) => (
                  <m.div
                    key={item.label}
              ...
              <m.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.45 }} style={{ borderRadius: "1rem", padding: "clamp(2.5rem,5vw,4rem)", gap: "2.5rem", alignItems: "center", background: "hsla(222,60%,50%,0.05)", border: "1px solid hsla(222,60%,50%,0.16)" }} className="szl-grid-cta">
                <div>
                  ...
                  <p style={{ fontSize: "0.9375rem", lineHeight: 1.70, color: "hsl(214,7%,62%)", maxWidth: "48ch" }}>
                    PARAGON is appropriate for security teams operating in high-consequence environments where compliance traceability is mandatory and analyst judgment cannot be bypassed. If that describes your environment, let's have a direct conversation.
                  </p>
                </div>
