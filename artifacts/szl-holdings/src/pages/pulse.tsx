...
import {
  Activity, Cpu, Globe, Shield,
  Zap, Network, Eye, Brain,
  Layers, Signal, GitCommit,
  BarChart3, Lock, ArrowUpRight,
} from "lucide-react";
import {
  PLATFORM_APPS, AGENT_TYPES,
  ParticleField, ConstellationView, GitTicker,
...
const MetricCard = memo(function MetricCard({ label, value, icon: Icon, color, trend, suffix, delay }: {
  ...
  return (
    ...
    >
      ...
      <div className="text-xl font-bold tabular-nums tracking-tight" style={{ color: "rgba(255,255,255,0.85)" }}>
        {value.toLocaleString()}{suffix}
      </div>
      <div className="text-[10px] font-medium mt-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>{label}</div>
      <div className="text-[9px] mt-1" style={{ color: `${color}90` }}>{trend}</div>
    </m.div>
  );
});
