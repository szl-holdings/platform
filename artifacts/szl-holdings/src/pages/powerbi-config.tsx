          ...
          <span className="ml-auto text-[10px] text-emerald-500 font-medium flex items-center gap-1">
            ...
            {loaded.config?.updatedAt && (
              <span className="text-muted-foreground ml-1">
                · {new Date(loaded.config.updatedAt).toLocaleDateString()}
              </span>
            )}
          </span>
        )}
          ...
          <>
            ...
            <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4 flex items-start gap-3">
              ...
              <div className="text-xs text-muted-foreground leading-relaxed space-y-1">
                <p>
                  <strong className="text-foreground">Prerequisites:</strong> Power BI Pro or
                  Premium Per User (PPU) license, or Premium capacity (P1+).
                </p>
                <p>
                  Create an <strong className="text-foreground">Azure App Registration</strong> with
                  Power BI service permissions (
                  <code className="text-blue-400">Dataset.Read.All</code>,{' '}
                  <code className="text-blue-400">Report.Read.All</code>). Use{' '}
                  <strong className="text-foreground">service principal</strong> (client
                  credentials) for embed token generation.
                </p>
                <a
            ...
            >
              ...
              <p className="text-xs text-muted-foreground">
                Map each SZL platform report to a Power BI Report ID. Find the Report ID in the
                Power BI Service URL:
                <code className="text-primary ml-1">
                  app.powerbi.com/groups/{'groupId'}/reports/{'{reportId}'}
                </code>
              </p>
              {REPORTS.map((report) => (
                <div
                ...
                {[
                  {
                    ...
                    value: 'Server-side (secure)',
                    desc: 'Tokens minted by API server, never exposed client-side',
                  },
                  {
                    label: 'Token Expiry',
                    value: '60 minutes (default)',
                    desc: 'Power BI default; refresh by re-requesting embed token',
                  },
                  {
                    label: 'Embed Mode',
                    value: 'App Owns Data',
                    desc: 'Service principal authentication via client credentials',
                  },
                  {
                    label: 'Row-Level Security',
                    value: 'Not configured',
                    desc: 'Configure RLS roles in your Power BI dataset',
                  },
                ].map((item) => (
                  <div
                    key={item.label}
                ...
                <div>
                  <div
                    className={cn(
                      ...
                      testResult.ok ? 'text-emerald-600' : 'text-red-400',
                    )}
                  >
