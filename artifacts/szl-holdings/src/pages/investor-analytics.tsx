...
const TOOLTIP_STYLE = {
  contentStyle: { background: "#111318", border: "1px solid #1e2230", borderRadius: 8, fontSize: 12, color: "#e8e0d0" },
  ...
  return (
    <ResponsiveContainer width="100%" height={200}>
      <AreaChart data={augmented} margin={{ top: 4, right: 4, left: -10, bottom: 0 }}>
        ...
        <YAxis tick={{ fontSize: 10, fill: MUTED }} tickLine={false} axisLine={false} tickFormatter={v => fmtCurrency(v)} />
        <Tooltip {...TOOLTIP_STYLE} formatter={(v: number, name: string) => [fmtCurrency(v), name === "mrr" ? "MRR" : "ARR"]} />
        <Area type="monotone" dataKey="mrr" stroke={GOLD} strokeWidth={2} fill="url(#mrrGrad)" dot={false} />
      </AreaChart>
    </ResponsiveContainer>
  );
}

function CustomerChart({ data }: { data: Array<{ month: string; customers: number; newCustomers: number }> }) {
  return (
    <ResponsiveContainer width="100%" height={200}>
      <BarChart data={data} margin={{ top: 4, right: 4, left: -10, bottom: 0 }}>
        ...
        <Bar dataKey="customers" name="Total Customers" fill={BLUE} radius={[2, 2, 0, 0]} opacity={0.6} />
  ...
  return (
    <ResponsiveContainer width="100%" height={180}>
      <LineChart data={data} margin={{ top: 4, right: 4, left: -10, bottom: 0 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="#1e2230" />
        <XAxis dataKey="month" tick={{ fontSize: 10, fill: MUTED }} tickLine={false} axisLine={false} />
        <YAxis tick={{ fontSize: 10, fill: MUTED }} tickLine={false} axisLine={false} tickFormatter={v => `${v}%`} />
        <Tooltip {...TOOLTIP_STYLE} formatter={(v: number) => [`${v}%`, "Churn Rate"]} />
        <ReferenceLine y={5} stroke={AMBER} strokeDasharray="4 2" label={{ value: "5% target", fontSize: 9, fill: AMBER }} />
        <Line type="monotone" dataKey="churnRate" stroke={ROSE} strokeWidth={2} dot={{ fill: ROSE, r: 3 }} />
      </LineChart>
    </ResponsiveContainer>
  );
}

function FunnelChart({ stages }: { stages: Array<{ stage: string; count: number; rate: number; dropOff?: number }> }) {
        ...
        <div key={stage.stage} className="group">
          <div className="flex items-center justify-between mb-1">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-medium" style={{ color: colors[i] }}>{stage.rate}%</span>
            </div>
          </div>
...
function CohortMatrix({ cohorts, avgRetention, periodLabels }: {
  ...
  return (
    <div className="overflow-x-auto">
      <table className="text-xs w-full border-collapse">
        <thead>
          <tr>
            <th className="text-left text-[#7a8099] py-1.5 pr-3 font-normal whitespace-nowrap">Cohort</th>
            <th className="text-right text-[#7a8099] py-1.5 px-2 font-normal">Size</th>
            {periodLabels.map(l => (
              <th key={l} className="text-center text-[#7a8099] py-1.5 px-2 font-normal whitespace-nowrap">{l}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          ...
          <tr className="border-t-2 border-[#1e2230] bg-[#0d0f14]">
            ...
            {avgRetention.map((pct, pi) => (
              <td key={pi} className="py-1.5 px-2 text-center">
                <span className="text-[10px] font-semibold" style={{ color: GOLD }}>{pct}%</span>
              </td>
            ))}
          </tr>
        </tbody>
...
function MonthlyFunnelChart({ data }: { data: Array<{ month: string; signups: number; activations: number; paid: number }> }) {
  return (
    <ResponsiveContainer width="100%" height={200}>
      <BarChart data={data} margin={{ top: 4, right: 4, left: -10, bottom: 0 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="#1e2230" />
...
export default function InvestorAnalytics() {
  ...
  const diffs: Array<{
    id: number; action: string; entityType: string; entityId: string;
    oldValues: unknown; newValues: unknown; userEmail: string | null;
    ...
    <div className="min-h-screen bg-[#0a0c10] text-zinc-200">
      ...
      <div className="border-b border-[#1e2230] px-6 py-5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-[#7a8099] mt-0.5">Unit economics, funnel conversion & cohort retention</p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-8">
            ...
            {mLoading ? (
              ...
              <>
                ...
                <div className="grid lg:grid-cols-2 gap-6">
                  <div className="bg-[#111318] border border-[#1e2230] rounded-xl p-5">
                    <SectionHeader title="MRR Trajectory" sub="Monthly recurring revenue over 12 months" />
                ...
                <div className="grid lg:grid-cols-2 gap-6">
                  <div className="bg-[#111318] border border-[#1e2230] rounded-xl p-5">
                    <SectionHeader title="Monthly Churn Rate" sub="Percentage of customers lost each month" />
                    <ChurnChart data={timeSeries} />
                  </div>
                  <div className="bg-[#111318] border border-[#1e2230] rounded-xl p-5">
                    <SectionHeader title="Plan Distribution" sub="Active subscriptions by plan tier" />
                    {metrics?.planDistribution?.length > 0 ? (
                      <ResponsiveContainer width="100%" height={180}>
                        <BarChart data={metrics.planDistribution} layout="vertical" margin={{ top: 4, right: 4, left: 40, bottom: 0 }}>
                          ...
                          <YAxis dataKey="plan" type="category" tick={{ fontSize: 10, fill: "#9ca3af" }} tickLine={false} axisLine={false} width={80} />
                          <Tooltip {...TOOLTIP_STYLE} />
                          <Bar dataKey="count" name="Subscriptions" radius={[0, 4, 4, 0]}>
                            {(metrics.planDistribution as Array<{ plan: string; count: number }>).map((_, i) => (
                ...
                <div className="bg-[#111318] border border-[#1e2230] rounded-xl p-5">
                  ...
                  <div className="grid grid-cols-3 gap-6 mt-2">
                    <div className="text-center">
                      <p className="text-3xl font-bold" style={{ color: VIOLET }}>{fmt(summary.totalUsers ?? 0)}</p>
                      <p className="text-xs text-[#7a8099] mt-1">Total Users</p>
                    </div>
                    <div className="text-center border-x border-[#1e2230]">
                      <p className="text-3xl font-bold" style={{ color: BLUE }}>{fmt(summary.activeUsers30d ?? 0)}</p>
                      <p className="text-xs text-[#7a8099] mt-1">Monthly Active (MAU)</p>
                    </div>
                    <div className="text-center">
                      <p className="text-3xl font-bold" style={{ color: EMERALD }}>{fmt(summary.newUsersThisMonth ?? 0)}</p>
          ...
          <>
            {!funnel ? (
              ...
              <>
                <div className="grid lg:grid-cols-2 gap-6">
                  ...
                  <div className="bg-[#111318] border border-[#1e2230] rounded-xl p-5">
                    ...
                    <div className="space-y-3 mt-2">
                      {(funnel.funnel?.stages ?? []).map((stage: { stage: string; count: number; rate: number; dropOff?: number }) => (
                        <div key={stage.stage} className="flex items-center justify-between border-b border-[#1e2230] pb-2">
                          <span className="text-sm text-zinc-300">{stage.stage}</span>
                          <div className="text-right">
                            <p className="text-sm font-semibold text-zinc-100">{fmt(stage.count)}</p>
                ...
                <div className="bg-[#111318] border border-[#1e2230] rounded-xl p-5">
                  <SectionHeader title="Monthly Funnel Trends" sub="Signups, activations & paid conversions by month" />
                ...
                <div className="bg-[#111318] border border-[#1e2230] rounded-xl p-5">
                  ...
                  {cohort.cohorts?.length > 0 ? (
                    ...
                    <div className="text-center py-12 text-zinc-600 text-sm">
                      Not enough user data for cohort analysis yet.<br />
                  ...
                  <div className="bg-[#111318] border border-[#1e2230] rounded-xl p-5">
                    ...
                    <ResponsiveContainer width="100%" height={200}>
                      ...
                      >
                        ...
                        <YAxis tick={{ fontSize: 10, fill: MUTED }} tickLine={false} axisLine={false} domain={[0, 100]} tickFormatter={v => `${v}%`} />
                        <Tooltip {...TOOLTIP_STYLE} formatter={(v: number) => [`${v}%`, "Retention"]} />
                        <ReferenceLine y={40} stroke={AMBER} strokeDasharray="4 2" label={{ value: "40% target", fontSize: 9, fill: AMBER }} />
                        <Area type="monotone" dataKey="retention" stroke={EMERALD} strokeWidth={2} fill="url(#retGrad)" dot={{ fill: EMERALD, r: 3 }} />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                )}
        ...
        {tab === "diffs" && (
          <>
            <div className="bg-[#111318] border border-[#1e2230] rounded-xl p-5">
              <SectionHeader title="Change Audit Report" sub="Before/after diff view for entity mutations tracked in audit events" />
              {diffsLoading ? (
                <div className="text-center py-20 text-zinc-600">Loading audit diffs...</div>
              ) : diffs.length === 0 ? (
                <div className="text-center py-12 text-zinc-600 text-sm">
                  No audit diffs found.<br />
                    ...
                    <thead>
                      <tr className="border-b border-[#1e2230]">
                        {["Timestamp", "Actor", "Action", "Entity Type", "Entity ID", "Changes"].map(h => (
                          <th key={h} className="text-left text-zinc-500 uppercase tracking-widest pb-2 pr-4 font-normal whitespace-nowrap">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {diffs.map(diff => {
                        const oldVals = diff.oldValues && typeof diff.oldValues === "object" ? diff.oldValues as Record<string, unknown> : {};
                        const newVals = diff.newValues && typeof diff.newValues === "object" ? diff.newValues as Record<string, unknown> : {};
                        const changedKeys = Array.from(new Set([...Object.keys(oldVals), ...Object.keys(newVals)])).filter(k => JSON.stringify(oldVals[k]) !== JSON.stringify(newVals[k]));
                        return (
                          <tr key={diff.id} className="border-b border-[#1a1d27] hover:bg-[#131620] transition-colors">
                            <td className="py-2 pr-4 text-zinc-500 whitespace-nowrap">{diff.createdAt ? new Date(diff.createdAt).toLocaleString() : "—"}</td>
                            <td className="py-2 pr-4 text-zinc-300">{diff.userEmail ?? diff.userName ?? "system"}</td>
                            <td className="py-2 pr-4">
                              <span className="px-1.5 py-0.5 rounded text-[10px] font-mono bg-zinc-800 text-zinc-400">{diff.action}</span>
                            </td>
                              ...
                              <div className="space-y-0.5">
                                {changedKeys.length === 0 ? (
                                  ...
                                  <div key={k} className="flex items-center gap-1 text-[10px]">
                                    ...
                                    <span className="text-red-400 line-through">{JSON.stringify(oldVals[k])?.slice(0, 20)}</span>
                                    <span className="text-zinc-600">→</span>
                                    <span className="text-emerald-400">{JSON.stringify(newVals[k])?.slice(0, 20)}</span>
                                  </div>
                                ))}
                                {changedKeys.length > 4 && <span className="text-zinc-600 text-[10px]">+{changedKeys.length - 4} more</span>}
                              </div>
                            </td>
                          </tr>
                        );
