...
const ARCH_SECTIONS = [
  ...
  },
  {
    ...
    business: "FORGE is the execution layer — but execution here means structured, approved, audited action, not automation. FORGE receives interpreted signals from KORA, generates structured action recommendations, routes them for human approval, executes on approval, and records the complete chain. The action spine connects every consequential workflow from signal to audit.",
    product: "Each FORGE action is structured: action type, responsible actor, routing path, SLA expectation, evidence package, and confidence basis. The recommendation is not a generic alert — it is a structured proposal with the full context an approver needs to decide. Execution is scoped to the approved parameters only.",
    trust: "The approval gate is built into the FORGE execution model at the infrastructure level. Bypassing the gate requires an explicit override record. There is no silent execution path. Every action that executes has an approval record, a named approver, and an attribution timestamp.",
    operational: "FORGE's connector model uses scoped permissions. A connector configured for task assignment cannot read financial records. Connector health is monitored continuously. Failed executions are retried with exponential backoff and surfaced for operator review.",
  },
  {
    id: "packs",
    icon: Package,
    ...
    product: "Each domain pack extends KORA's signal classification with pack-specific types, enriches the FORGE recommendation model with domain context, and surfaces domain-specific workflow surfaces. The pack sits above the core platform layer — it does not replace it.",
  ...
  },
  {
    ...
    product: "The governance API serves as the single interface layer between all platform surfaces (KORA, FORGE, packs) and the underlying data and connector services. Frontend surfaces, mobile clients, external integrations, and the audit layer all operate through the same typed interface — no side channels.",
...
export default function ArchitecturePage() {
  const __pageMeta = usePageMeta({
    ...
    description: "Why this architecture exists, how KORA, FORGE, packs, Proof Chain, external intelligence, the governance API, governed inference, and trust infrastructure work together — and why this is hard to copy.",
