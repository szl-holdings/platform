...
const ARCH_SECTIONS = [
  ...
  },
  {
    ...
    trust: "All signal classification includes confidence scoring and source attribution. Signals cannot be acted upon without going through the Alloy approval layer. Lyte surfaces intelligence — Alloy controls execution. The separation is architectural, not configurable away.",
  ...
  },
  {
    ...
    business: "Alloy is the execution layer — but execution here means structured, approved, audited action, not automation. Alloy receives interpreted signals from Lyte, generates structured action recommendations, routes them for human approval, executes on approval, and records the complete chain. The action spine connects every consequential workflow from signal to audit.",
    product: "Each Alloy action is structured: action type, responsible actor, routing path, SLA expectation, evidence package, and confidence basis. The recommendation is not a generic alert — it is a structured proposal with the full context an approver needs to decide. Execution is scoped to the approved parameters only.",
    trust: "The approval gate is built into the Alloy execution model at the infrastructure level. Bypassing the gate requires an explicit override record. There is no silent execution path. Every action that executes has an approval record, a named approver, and an attribution timestamp.",
    operational: "Alloy's connector model uses scoped permissions. A connector configured for task assignment cannot read financial records. Connector health is monitored continuously. Failed executions are retried with exponential backoff and surfaced for operator review.",
  },
  {
    id: "packs",
    icon: Package,
