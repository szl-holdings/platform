        ...
        return (
          <div key={step.id} className="flex items-center">
            ...
            {i < STEPS.length - 1 && (
              <div
                className={cn(
                  'h-0.5 w-10 md:w-16 transition-all',
...
function Step1OrgDetails({ onNext }: { onNext: (data: Step1Data) => void }) {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4 flex items-start gap-3">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            Register your Azure AD tenant with the SZL platform. You'll need your Azure AD Tenant ID
            (a GUID like <code className="text-blue-400">xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx</code>
            ), found in the Azure portal under Azure Active Directory → Overview.
          </p>
        </div>
        ...
        >
          {loading ? (
            ...
            <>
              Register Tenant <ArrowRight className="w-4 h-4" />
            </>
          )}
        </button>
      </form>
...
function Step2AdminConsent({
  tenantId,
  adminConsentUrl: serverConsentUrl,
  onNext,
}: {
  tenantId: string;
  ...
  const consentUrl =
    serverConsentUrl ??
    `https://login.microsoftonline.com/${tenantId || '[TENANT_ID]'}/adminconsent?client_id=[Configure_Client_ID]&redirect_uri=${encodeURIComponent(`${window.location.origin}/api/azure-ad/callback`)}&state=tenant-${tenantId}`;
  ...
  return (
    <m.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
      <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          An Azure AD Global Administrator must visit the consent URL below and click{' '}
          <strong className="text-foreground">Accept</strong> to grant the SZL platform access to
          the tenant. This grants permissions for user sign-in, profile reading, and user
          provisioning.
        </p>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="rounded-xl border border-border bg-muted/30 p-3 flex items-start gap-2">
          <code className="text-[10px] text-primary break-all flex-1 leading-relaxed">
            {consentUrl}
          </code>
          <button
            onClick={handleCopy}
...
function Step3VerifyPermissions({
  tenantDbId,
  onNext,
  onBack,
}: {
  ...
  const permissions = [
    {
      name: 'Azure AD Login (OIDC)',
      status: 'granted',
      desc: 'Multi-tenant SSO via OpenID Connect',
    },
    ...
    { name: 'Session Management', status: 'granted', desc: 'Token refresh and offline access' },
    {
      ...
      status: 'pending',
      desc: 'Automated user lifecycle management',
    },
  ...
  return (
    <m.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
      ...
      <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-3 text-xs text-muted-foreground leading-relaxed">
        ...
        SCIM and Group Sync require additional API permissions in the Azure App Registration. These
        can be configured later from the tenant settings.
      </div>
    ...
    <m.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
      <div className="space-y-3">
        {[
          ...
          <div
            key={opt.key}
            className="flex items-start justify-between gap-4 rounded-xl border border-border/50 bg-card/50 p-4"
          >
            <div>
        ...
        <div className="rounded-xl border border-border/50 bg-card/50 p-4">
          ...
          <p className="text-[10px] text-muted-foreground mb-3">
            Role assigned to new users from this tenant
          </p>
          <select
        ...
        <div className="rounded-xl border border-border/50 bg-card/50 p-4">
          ...
          <p className="text-[10px] text-muted-foreground mb-3">
            Duration before users are required to re-authenticate
          </p>
          <input
            type="number"
            min="1"
      ...
      <div className="flex gap-3">
        ...
        >
          {saving ? (
            ...
            <>
              Save & Complete <ArrowRight className="w-4 h-4" />
            </>
          )}
        </button>
      </div>
        ...
        <div className="w-full max-w-xl">
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Connect a new Azure AD tenant to the SZL platform for enterprise SSO and user
              provisioning.
            </p>
          </m.div>
          ...
          <div className="bg-card border border-border rounded-2xl p-6 shadow-sm">
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 5 && <Step5Complete key="step5" orgName={orgName} tenantId={tenantId} />}
            </AnimatePresence>
          </div>
