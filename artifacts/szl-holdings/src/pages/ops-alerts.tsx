          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <span
              style={{
                ...
                height: 8,
                borderRadius: '50%',
                background: rule.enabled ? sc : '#374151',
                display: 'inline-block',
                flexShrink: 0,
              }}
            />
            <span style={{ fontSize: '0.9375rem', fontWeight: 600, color: 'hsl(38,12%,92%)' }}>
              {rule.name}
            </span>
            <span
              style={{
            ...
            {!rule.enabled && (
              <span style={{ fontSize: 10, color: '#6b7280', fontStyle: 'italic' }}>disabled</span>
            )}
          </div>
          {rule.description && (
            <p
          ...
          >
            <span
              style={{
                ...
                padding: '2px 8px',
                borderRadius: 4,
                border: '1px solid hsla(210,55%,52%,0.2)',
              }}
            >
            ...
            {rule.notify_in_app && (
              <span style={{ fontSize: 11, color: 'hsl(210,5%,44%)' }}>📲 In-app</span>
            )}
            ...
            {rule.last_fired_at && (
              <span style={{ fontSize: 11, color: '#f59e0b' }}>
                Last fired:{' '}
                {new Date(rule.last_fired_at).toLocaleDateString('en-US', {
                  month: 'short',
                  day: 'numeric',
                  hour: '2-digit',
                  minute: '2-digit',
...
function CreateRuleModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const save = async () => {
    if (!form.name || !form.metricName) {
      setError('Name and metric name are required.');
      return;
    }
    setSaving(true);
    try {
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
          ...
          <div>
            <label style={labelSt}>Metric Name *</label>
            <input
              style={inputSt}
              value={form.metricName}
              onChange={(e) => set('metricName', e.target.value)}
              placeholder="api.error_rate"
            />
            <div style={{ fontSize: 10, color: 'hsl(210,5%,40%)', marginTop: 3 }}>
              Available: api.error_rate, api.latency_p95, queue.depth, db.pool_utilization,
              system.memory_pct, ai.failure_rate
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.75rem' }}>
            ...
            <div>
              <label style={labelSt}>Window (min)</label>
              <input
                style={inputSt}
                type="number"
          ...
          {form.notifyEmail && (
            <div>
              <label style={labelSt}>Email Recipients (comma-separated)</label>
              <input
                style={inputSt}
...
export default function OpsAlertsPage() {
  ...
  const evaluate = async () => {
    setEvaluating(true);
    try {
      const res = await fetch(`${BASE}/ops/alert-rules/evaluate`, {
        method: 'POST',
        credentials: 'include',
      });
  ...
  const deleteRule = async (id: number) => {
    ...
    const confirmed = await productionConfirm({
      ...
      description:
        'This alert rule will be permanently removed. Any active alerts from this rule will stop firing.',
      confirmText: 'DELETE',
    });
    if (!confirmed) return;
  ...
  const acknowledge = async (id: number) => {
    await fetch(`${BASE}/ops/alert-events/${id}/acknowledge`, {
      method: 'POST',
      credentials: 'include',
    });
    await load();
        ...
        >
          <div>
            ...
            <p style={{ fontSize: '0.875rem', color: 'hsl(210,5%,50%)' }}>
              Define threshold-based alert rules and receive in-app or email notifications.
            </p>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              ...
              style={{
                ...
                border: '1px solid hsla(152,50%,42%,0.25)',
                color: '#34d399',
                cursor: 'pointer',
              }}
            >
        ...
        {evalResult && (
          <div
            style={{
              ...
              borderRadius: 8,
              background: 'hsla(152,50%,42%,0.07)',
          ...
          >
            <div style={{ fontSize: 13, color: 'hsl(38,12%,82%)', marginBottom: 6 }}>
              <CheckCircle
                size={13}
                style={{ display: 'inline', color: '#10b981', marginRight: 6 }}
              />
            ...
            <div style={{ display: 'flex', gap: '1.25rem', flexWrap: 'wrap' }}>
              {Object.entries(evalResult.metrics).map(([k, v]) => (
                <span
                  key={k}
                  style={{ fontSize: 11, fontFamily: 'monospace', color: 'hsl(210,5%,52%)' }}
                >
                  ...
                  >
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        <span
                          style={{ fontSize: '0.9rem', fontWeight: 600, color: 'hsl(38,12%,90%)' }}
                        >
                          {e.rule_name}
                        </span>
                        <span
                          ...
                          style={{
                            ...
                            background:
                              e.status === 'firing'
                                ? 'hsla(0,72%,51%,0.12)'
                                : 'hsla(152,50%,42%,0.1)',
                            color: e.status === 'firing' ? '#f87171' : '#34d399',
                            border: '1px solid',
                            borderColor:
                              e.status === 'firing'
                                ? 'hsla(0,72%,51%,0.25)'
                      ...
                      <div style={{ fontSize: 12, color: 'hsl(210,5%,50%)', marginLeft: 21 }}>
                        <span style={{ fontFamily: 'monospace', color: 'hsl(210,55%,68%)' }}>
                          {e.metric_name}
                        </span>{' '}
                        = <strong style={{ color: 'hsl(38,12%,80%)' }}>{e.metric_value}</strong>
                        <span style={{ marginLeft: 6 }}>
                          (threshold: {conditionLabel(e.condition)} {e.threshold})
                        </span>
                    ...
                    {e.status === 'firing' && (
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: 6,
                          cursor: 'pointer',
                          background: 'hsla(152,50%,42%,0.08)',
                          border: '1px solid hsla(152,50%,42%,0.2)',
                          color: '#34d399',
                          flexShrink: 0,
                        }}
