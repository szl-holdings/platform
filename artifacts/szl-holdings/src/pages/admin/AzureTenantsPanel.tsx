...
import {
  Cloud, Plus, AlertCircle, Loader2, Save, Trash2,
  ChevronDown, ChevronUp, UserCheck, LinkIcon, CheckCircle2, Key, Shield,
} from "lucide-react";
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={cn("text-[10px] font-semibold px-1.5 py-0.5 rounded-full border capitalize", map[status] ?? "text-muted-foreground bg-muted border-border")}>
      {status}
    </span>
  );
}

export function AzureTenantsPanel() {
