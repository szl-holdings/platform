    ...
    <div style={{ borderBottom: '1px solid hsla(0,0%,100%,0.05)' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          {open ? (
            <ChevronDown size={13} style={{ color: 'hsl(210,5%,45%)' }} />
          ) : (
            <ChevronRight size={13} style={{ color: 'hsl(210,5%,45%)' }} />
          )}
          <span
            style={{
              fontSize: '13.5px',
            ...
            {product.capabilities.map((cap) => (
              ...
              >
                <div>
                  ...
                  <p style={{ fontSize: '11.5px', color: 'hsl(210,5%,46%)', lineHeight: 1.5 }}>
                    {cap.evidence}
                  </p>
...
const SOLUTION_BRIEFS: Array<{ slug: string; product: string; label: string; href: string }> = [
  {
    slug: 'lyte',
    product: 'Lyte (Business Observability)',
    label: 'Lyte - Decision Intelligence',
    href: '/briefs/lyte-solution-brief.pdf',
  },
  {
    slug: 'aegis',
    product: 'Aegis (Defense & Intelligence)',
    label: 'Aegis - Unified Defense',
    href: '/briefs/aegis-solution-brief.pdf',
  },
  {
    slug: 'vessels',
    product: 'Vessels (Maritime Intelligence)',
    label: 'Vessels - Maritime Intelligence',
    href: '/briefs/vessels-solution-brief.pdf',
  },
  {
    slug: 'terra',
    product: 'Terra (Real Estate Intelligence)',
    label: 'Terra - Real Estate Intelligence',
    href: '/briefs/terra-solution-brief.pdf',
  },
  {
    slug: 'carlota-jo',
    product: 'Carlota Jo (Private Advisory)',
    label: 'Carlota Jo - Private Advisory',
    href: '/briefs/carlota-jo-solution-brief.pdf',
  },
];
...
function SolutionBriefDownloads({ products }: { products: ProductSummary[] }) {
  ...
  return (
    ...
    >
      ...
      >
        Each brief is generated directly from the capability manifest: capability counts, status
        mix, top proof points, and open risks. Regenerated on every manifest change so the figures
        always match this page.
      </p>
      <div
        style={{
          display: 'grid',
...
export default function ProductReadinessPage() {
  const __pageMeta = usePageMeta({
    ...
    description:
      'Live product readiness status for every SZL Holdings capability, driven by the platform capability manifest. No hand-edited claims.',
  ...
  const platformScore =
    totals.total === 0
      ? 0
      : Math.round(
          ((totals.live * 1.0 + totals.working_demo * 0.75 + totals.partial * 0.4) / totals.total) *
            100,
        );
              ...
              >
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '1.5rem' }}>
                  {[
                    ...
                    {
                      label: 'Stub / Broken',
                      value: totals.stub + totals.broken,
                      color: '#ef4444',
                    },
                  ].map((stat) => (
                    <div
                      key={stat.label}
                      style={{ display: 'flex', flexDirection: 'column' as const, gap: '0.2rem' }}
                    >
                      <span
                        style={{
                          fontSize: '1.5rem',
                          fontWeight: 700,
                          color: stat.color ?? 'hsl(38,12%,92%)',
                        }}
                      >
                        {stat.value}
                      </span>
            ...
            >
              ...
              >
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
                    <span
                      style={{ fontSize: '2.25rem', fontWeight: 700, color: 'hsl(38,12%,92%)' }}
                    >
                      {platformScore}%
                    </span>
                    <span style={{ fontSize: '13px', color: 'hsl(210,5%,48%)' }}>
                      weighted proven
                    </span>
                  </div>
                </div>
                {flaggedClaims.length > 0 && (
                  <div
                    style={{
                      ...
                      borderRadius: '8px',
                      background: 'hsla(38,85%,50%,0.08)',
                      border: '1px solid hsla(38,85%,50%,0.18)',
                    }}
                  >
                    <p style={{ fontSize: '11px', fontWeight: 600, color: '#f59e0b' }}>
                      {flaggedClaims.length} capabilities below demo threshold
                    </p>
                    <p style={{ fontSize: '11px', color: 'hsl(210,5%,46%)', marginTop: '0.2rem' }}>
                      Stub, broken, or undocumented - not investor-presentable
                    </p>
                  </div>
                )}
              </div>
              ...
              >
                <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: '1rem' }}>
                  {(
                    ['live', 'working_demo', 'partial', 'stub', 'broken'] as CapabilityStatus[]
                  ).map((status) => {
                ...
                >
                  <BookOpen size={12} />
                  Source: capability-manifest.json
                </a>
              </div>
            </div>
          </section>
