...
export function ForgeShell({
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0e14', color: '#e7eaf0' }}>
      ...
      >
        <Link href="/forge" style={{ color: '#d4a054', fontWeight: 600, textDecoration: 'none' }}>
          ← Forge
        </Link>
        <div style={{ display: 'flex', gap: 16, fontSize: 14 }}>
          {FORGE_NAV.map((n) => (
            <Link key={n.href} href={n.href} style={{ color: '#9ca3af', textDecoration: 'none' }}>
              {n.label}
            </Link>
          ))}
        </div>
...
export function Card({
  ...
  return (
    <div
      style={{ background: '#0f1620', border: '1px solid #1f2937', borderRadius: 8, padding: 20 }}
    >
      <div
        style={{ fontSize: 12, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: 0.5 }}
      >
        {title}
      </div>
...
export function SeverityPill({ value }: { value: string }) {
  const map: Record<string, { bg: string; fg: string }> = {
    none: { bg: '#1f2937', fg: '#9ca3af' },
    low: { bg: '#1e3a5f', fg: '#93c5fd' },
    medium: { bg: '#5b3a1e', fg: '#fbbf24' },
    high: { bg: '#5b1e1e', fg: '#fca5a5' },
    critical: { bg: '#7f1d1d', fg: '#fee2e2' },
    healthy: { bg: '#14532d', fg: '#86efac' },
    drifting: { bg: '#5b3a1e', fg: '#fbbf24' },
  };
...
export function StatusPill({ value }: { value: string }) {
  const map: Record<string, { bg: string; fg: string }> = {
    requested: { bg: '#1f2937', fg: '#9ca3af' },
    validated: { bg: '#1e3a5f', fg: '#93c5fd' },
    approved: { bg: '#14532d', fg: '#86efac' },
    blocked: { bg: '#5b1e1e', fg: '#fca5a5' },
    promoted: { bg: '#14532d', fg: '#86efac' },
    success: { bg: '#14532d', fg: '#86efac' },
    failure: { bg: '#5b1e1e', fg: '#fca5a5' },
    escalated: { bg: '#5b3a1e', fg: '#fbbf24' },
