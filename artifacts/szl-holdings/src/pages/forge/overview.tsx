...
function SubmitExecutionPanel() {
  ...
  return (
    <Section title="Submit execution">
      <form
        onSubmit={onSubmit}
        data-testid="form-forge-submit"
        style={{ display: 'grid', gap: 12, gridTemplateColumns: '1fr 1fr', maxWidth: 720 }}
      >
        ...
        <div style={{ gridColumn: '1 / span 2', display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          {error && (
            <span data-testid="text-forge-submit-error" style={{ color: '#fca5a5', fontSize: 12 }}>
              {error}
            </span>
          )}
          {result && (
            <span
              data-testid="text-forge-submit-success"
...
export default function ForgeOverviewPage() {
  ...
  return (
    <ForgeShell
      ...
      subtitle="AI runtime, agent factory, and governed promotion pipeline"
    >
      {isLoading && <div style={{ color: '#9ca3af' }}>Loading…</div>}
      {error && (
        <div style={{ color: '#fca5a5' }}>Failed to load overview: {(error as Error).message}</div>
      )}
      ...
      {data && (
        <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
              {Object.entries(data.byEnv).map(([env, n]) => (
                <Card
                  ...
                  value={n}
                  accent={
                    env === 'production' ? '#fca5a5' : env === 'staging' ? '#fbbf24' : '#93c5fd'
                  }
                />
              ))}
            </div>
          ...
          <Section title="Promotion queue">
            {data.promotionQueue.length === 0 ? (
              <div style={{ color: '#6b7280' }}>No pending promotions.</div>
            ) : (
              <table style={tableStyle}>
                ...
                <tbody>
                  {data.promotionQueue.slice(0, 10).map((p) => (
                    <tr key={p.id}>
                      <td style={tdStyle}>
          ...
          <Section title="Recent failures">
            {data.recentFailures.length === 0 ? (
              <div style={{ color: '#6b7280' }}>No recent failures.</div>
            ) : (
              <table style={tableStyle}>
                <thead>
                  <tr>
                ...
                <tbody>
                  {data.recentFailures.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td style={tdStyle}>
                        <SeverityPill value={r.envTier === 'production' ? 'critical' : 'medium'} />
                      </td>
