...
interface AgentDetail {
  ...
  versions: Array<{
    ...
    modelId: string | null;
    evalsPassed: boolean;
    observabilityHookConfigured: boolean;
...
export default function ForgeAgentDetailPage() {
  ...
  const id = params?.id ?? '';
  const { data, isLoading, error } = useForgeQuery<AgentDetail>(
    `agent:${id}`,
    `/api/forge/agents/${id}`,
  );
      ...
      {data && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
            ...
            <Card
              title="Risk tier"
              value={data.agent.riskTier}
              accent={data.agent.riskTier === 'executive' ? '#fca5a5' : '#fbbf24'}
            />
            ...
            <table style={tableStyle}>
              <thead>
                <tr>
                  ...
                  <th style={thStyle}>Observability</th>
              ...
              <tbody>
                {data.versions.map((v) => (
                  <tr key={v.id}>
                    ...
                    <td style={tdStyle}>
                      <code style={{ fontSize: 11 }}>{v.modelId?.slice(0, 8) ?? '—'}</code>
                    </td>
          ...
          <Section title="Drift events">
            {data.drift.length === 0 ? (
              <div style={{ color: '#6b7280' }}>No drift recorded.</div>
            ) : (
              <table style={tableStyle}>
                <thead>
                  <tr>
                    ...
                    <th style={thStyle}>Remediation</th>
                    <th style={thStyle}>When</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <Section title="Rollbacks">
            {data.rollbacks.length === 0 ? (
              <div style={{ color: '#6b7280' }}>No rollbacks recorded.</div>
            ) : (
              <table style={tableStyle}>
                <thead>
                  <tr>
