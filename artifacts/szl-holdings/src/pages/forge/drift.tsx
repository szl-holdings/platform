...
export default function ForgeDriftPage() {
  ...
  return (
    <ForgeShell
      title="Drift"
      subtitle="Model · prompt · tool · data · config drift across every environment."
    >
      {summary.data && (
        <>
            ...
            <tbody>
              {events.data.map((e) => (
                <tr key={e.id}>
                  ...
                  <td style={tdStyle}>
                    <code style={{ fontSize: 11 }}>{e.agentId.slice(0, 8)}</code>
                  </td>
                  <td style={tdStyle}>
                    <code style={{ fontSize: 11 }}>{e.envId.slice(0, 8)}</code>
                  </td>
