...
import {
  ...
  type NuroModel,
  nuroForgeService,
  type DuelResult as ServiceDuelResult,
    ...
    <div className="min-h-screen" style={{ background: '#070a10' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px]" style={{ color: 'rgba(255,255,255,0.3)' }}>
                {totalDuels.toLocaleString()} tests completed · {models.length} models competing
              </p>
            </div>
          </div>
          <button
            ...
            >
              <div className="flex items-center justify-center gap-2 mb-4">
                ...
                <span
                  ...
                  style={{ color: '#f59e0b' }}
                >
                  {activeDuel.phase === 'running' ? 'Test in Progress' : 'Test Complete'} —{' '}
                  {activeDuel.domain}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex-1 text-center">
                  <div
                    ...
                    style={{
                      color:
                        activeDuel.phase === 'done' &&
                        activeDuel.result?.winner === activeDuel.c.name
                          ? '#10b981'
                  ...
                  <div className="text-[10px]" style={{ color: 'rgba(255,255,255,0.3)' }}>
                    {activeDuel.c.provider} · Score {activeDuel.c.elo}
                  </div>
                  {activeDuel.phase === 'done' &&
                    activeDuel.result?.winner === activeDuel.c.name && (
                      <m.div
                ...
                <div className="flex-1 text-center">
                  <div
                    ...
                    style={{
                      color:
                        activeDuel.phase === 'done' &&
                        activeDuel.result?.winner === activeDuel.d.name
                          ? '#10b981'
                          : 'rgba(255,255,255,0.7)',
                    }}
                  >
                  ...
                  <div className="text-[10px]" style={{ color: 'rgba(255,255,255,0.3)' }}>
                    {activeDuel.d.provider} · Score {activeDuel.d.elo}
                  </div>
                  {activeDuel.phase === 'done' &&
                    activeDuel.result?.winner === activeDuel.d.name && (
                      <m.div
              ...
              {activeDuel.phase === 'done' && activeDuel.result && (
                <div
                  className="mt-3 flex justify-center gap-4 text-[10px]"
                  style={{ color: 'rgba(255,255,255,0.3)' }}
                >
                  <span>
                    Score: {activeDuel.result.scoreA.toFixed(1)} vs{' '}
                    {activeDuel.result.scoreB.toFixed(1)}
                  </span>
        ...
        <div className="flex gap-2 mb-4">
          {(['leaderboard', 'history'] as const).map((v) => (
            <button
              ...
              style={{
                background: selectedView === v ? 'rgba(245,158,11,0.12)' : 'rgba(255,255,255,0.02)',
                color: selectedView === v ? '#f59e0b' : 'rgba(255,255,255,0.4)',
                border: `1px solid ${selectedView === v ? 'rgba(245,158,11,0.2)' : 'rgba(255,255,255,0.04)'}`,
              }}
            >
              {v}
            </button>
        ...
        {selectedView === 'leaderboard' ? (
          ...
          >
            <div
              className="grid grid-cols-[40px_1fr_80px_70px_70px_70px_80px_80px] gap-2 px-4 py-2"
            ...
            {sorted.map((model, i) => (
              ...
              >
                ...
                <span className="text-[12px] font-bold tabular-nums" style={{ color: model.color }}>
                  {model.elo}
                </span>
                <span className="text-[11px] tabular-nums" style={{ color: '#10b981' }}>
                  {model.wins}
                </span>
            ...
            {duels.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-[12px]" style={{ color: 'rgba(255,255,255,0.3)' }}>
                  No tests recorded yet. Click "Run Test" to start.
                </p>
              </div>
            ) : (
              duels.map((d, _i) => (
                ...
                >
                  <span
                    className="text-[11px] font-semibold"
                    style={{ color: d.winner === d.modelA ? '#10b981' : 'rgba(255,255,255,0.5)' }}
                  >
                    {d.modelA}
                  </span>
                  ...
                  <span
                    className="text-[11px] font-semibold"
                    style={{ color: d.winner === d.modelB ? '#10b981' : 'rgba(255,255,255,0.5)' }}
                  >
                    {d.modelB}
                  </span>
                  <span
                  ...
                  <span className="text-[10px] tabular-nums" style={{ color: '#f59e0b' }}>
