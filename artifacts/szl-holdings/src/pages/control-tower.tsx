    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="max-w-7xl mx-auto px-6 py-6">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1 max-w-2xl">
                Unified intelligence operating system — Sense → Decide → Act → Govern across the
                entire agent mesh
              </p>
            </div>
            <Link
        ...
        <div className="flex items-center gap-1 mb-6 bg-card border border-border rounded-xl p-1">
          {(Object.entries(LAYER_CONFIG) as [LayerTab, (typeof LAYER_CONFIG)[LayerTab]][]).map(
            ([tab, config]) => (
              <button
                key={tab}
                className={cn(
        ...
        >
          <div className="mb-3">
            <p className="text-[10px] text-muted-foreground">
              {LAYER_CONFIG[activeTab].description}
            </p>
          </div>
