...
export const caseStudies: CaseStudy[] = [
  {
    ...
    sections: {
      ...
      systemBuilt:
        "Lyte was configured to ingest approval events across procurement (Coupa), legal (Matter), and finance (NetSuite) using Alloy connectors. The signal normalization layer identified ownership gaps — approvals sitting with no acknowledged owner for >24 hours. Lyte's action routing engine escalated the stalled contract renewals to the VP Finance and flagged it in the Command Inbox with a revenue impact estimate.",
      howItWorked: [
      ...
      outcome:
        '$340K quarterly revenue leakage recovered. The approval stall was resolved 9 days before deadline. The ownership gap was corrected in the system within the same day. Lyte now monitors 14 approval workflows continuously — the weekly review is no longer the primary detection mechanism.',
      visualProof: [
      ...
      whyItMatters:
        "Most organizations rely on periodic reviews to catch operational failures. Lyte eliminates the blind spot between reviews by treating approval state as a continuous signal — not a weekly snapshot. The gap between 'something is wrong' and 'someone knows about it' is where revenue leaks. Lyte closes that gap.",
    },
  },
  {
    ...
    sections: {
      ...
      howItWorked: [
        ...
        'Vessels raised a critical exception (EXC-001) routed to fleet operators and Aegis SOC',
      ...
      outcome:
        "The vessel was formally designated 34 days after Vessels' initial alert. The fleet had already cleared the exposure window, notified P&I, and re-routed corridor vessels. Zero compliance breach. The same intelligence process now runs continuously across 40,000+ monitored vessels.",
      visualProof: [
      ...
      whyItMatters:
        'Formal sanctions lists are a trailing indicator. By the time a vessel is designated, operators in its corridor are already exposed. Vessels moves maritime compliance from reactive list-checking to predictive behavioral intelligence — giving fleet operators the lead time to act before regulators publish.',
    },
  },
  {
    ...
    subtitle:
      'How Alloy eliminated coordination overhead across procurement, legal, and operations',
    sections: {
      problem:
  ...
  },
  {
    ...
    subtitle:
      'How Aegis detected and isolated a ransomware staging environment before the destructive payload deployed',
    sections: {
      problem:
  ...
  },
  {
    ...
    subtitle:
      'How Lyte eliminated the coordination overhead that turns containable incidents into enterprise crises',
    sections: {
      problem:
      ...
      systemBuilt:
        "Lyte was configured with an Incident Response workflow template that encoded the full notification chain, authority structure, and documentation requirements for this firm's regulatory posture. When an incident was flagged (via Aegis integration), Lyte automatically assembled the incident brief, initiated the parallel notification workflows, surfaced the jurisdiction-specific disclosure deadlines, and opened the audit timeline — all within the first 4 minutes. Alloy routed each action to the correct owner with context, authority, and deadline attached.",
      howItWorked: [
        '03:18 AM: Aegis flags suspected data exfiltration — event forwarded to Lyte via webhook integration',
        '03:18 AM: Lyte assembles incident brief: affected systems, severity score, preliminary blast radius estimate',
        ...
        '03:28 AM: CISO issues initial containment instructions — Lyte logs each action to immutable audit trail',
      ...
      whyItMatters:
        'Security incidents are measured in minutes. Most organizations treat incident response coordination as a people problem — better training, clearer escalation paths, more drills. Lyte treats it as an infrastructure problem: encode the authority structure, pre-assemble the brief, run the notifications in parallel, and make the audit trail a byproduct of the workflow. The 22-minute response was possible because no human had to figure out what to do next — the system already knew.',
    },
  },
  {
    ...
    subtitle:
      'A design partner scenario: how Vessels detected a compliance gap that would have resulted in a detention order',
    sections: {
      problem:
  ...
  },
  {
    ...
    subtitle:
      "How Terra's multi-signal intelligence surfaced a $28M acquisition opportunity in the pre-foreclosure window",
    sections: {
      problem:
      ...
      systemBuilt:
        'Terra was configured with a distress signal model that combined: property tax delinquency (county records integration), utility disconnection notices (state utility registry), insurance policy lapse indicators (commercial property database), maintenance permit inactivity (building department records), and rental market price deviation (listed rent vs. market comp). The multi-signal distress score for the target property crossed the 85/100 threshold 22 days before the foreclosure filing was recorded — Terra issued a high-priority acquisition opportunity alert to the investment committee.',
      howItWorked: [
        'Day 1: Terra detects property tax delinquency on 2 consecutive quarters — distress signal score: 34/100',
