...
export const ANOMALY_PATTERNS: AnomalyPattern[] = [
   ...
   */
  {
    ...
    description: "OFAC SDN candidate match + AIS dark period + linked real estate distress uptick — consistent with sanctioned-entity capital repatriation behavior.",
      ...
      for (const threatEntity of sanctionsEntities) {
        ...
        for (const vessel of linkedVessels) {
          ...
          const linkedProperties = graph.entities.filter(
            e => e.type === "property" && reachable.has(e.id) && (e.domainData.property?.distressScore ?? 0) >= 60
          );
          ...
          const signals: AnomalySignal[] = [
            { domain: "threat", entityId: threatEntity.id, metric: "OFAC Match Confidence", value: threatEntity.domainData.threat?.ofacMatchConfidence ?? 0, threshold: 75, breached: true },
            { domain: "vessels", entityId: vessel.id, metric: "AIS Gap (hours)", value: aisGap, threshold: 12, breached: aisGap >= 12 },
            { domain: "vessels", entityId: vessel.id, metric: "Red Sea Transits (30d)", value: transitCount, threshold: 2, breached: transitCount >= 2 },
            { domain: "vessels", entityId: vessel.id, metric: "Route Risk", value: routeRisk ?? "UNKNOWN", breached: routeRisk === "RED" },
          ...
          const linkedEdgeIds = graph.edges
            .filter(e => involvedEntityIds.includes(e.sourceId) && involvedEntityIds.includes(e.targetId))
            .map(e => e.id);
   ...
   */
  {
    ...
    description: "APT/threat actor association with entity infrastructure within 72h of litigation escalation — possible adversarial cyber activity related to legal proceedings.",
    ...
    condition: (graph) => {
      ...
      const aptEntities = graph.entities.filter(
        e => (e.domainData.threat?.aptAssociations?.length ?? 0) > 0 ||
      ...
      for (const aptEntity of aptEntities) {
        ...
        for (const litigant of litigants) {
          ...
          const confidence = Math.min(82, 40 + (activeArbitrations * 10) + (aptAssociations.length * 8) + (exposure > 1000000 ? 10 : 0));
          const severity: AnomalySeverity = confidence >= 70 ? "high" : "medium";

          const signals: AnomalySignal[] = [
            { domain: "threat", entityId: aptEntity.id, metric: "APT Associations", value: aptAssociations.join(", ") || "IP overlap", breached: true },
            { domain: "threat", entityId: aptEntity.id, metric: "IP Range Overlap", value: aptEntity.domainData.threat?.ipOverlap ? "Yes" : "No", breached: !!aptEntity.domainData.threat?.ipOverlap },
            { domain: "legal", entityId: litigant.id, metric: "Active Arbitrations", value: activeArbitrations, threshold: 1, breached: activeArbitrations >= 1 },
            { domain: "legal", entityId: litigant.id, metric: "Aggregate Legal Exposure ($)", value: exposure, threshold: 1000000, breached: exposure >= 1000000 },
          ];
          ...
          matches.push({
            ...
            domains: ["Threat", "Legal", ...(litigant.domains.includes("financial") ? ["Financial"] : [])],
            involvedEntityIds,
            involvedEdgeIds: linkedEdgeIds,
            confidence,
   ...
   */
  {
    ...
    description: "Two entities from separate, independent risk networks share a co-investment with no disclosed relationship — structural anomaly requiring due diligence.",
    ...
    condition: (graph) => {
      ...
      const propertyEntities = graph.entities.filter(e => e.type === "property" || e.type === "asset");

      for (const prop of propertyEntities) {
        const inEdges = graph.edges.filter(
          e => e.targetId === prop.id && ["co_invests", "holds", "invests", "finances"].includes(e.relationship)
        );
        ...
        for (let i = 0; i < inEdges.length; i++) {
          for (let j = i + 1; j < inEdges.length; j++) {
            ...
            const directEdge = graph.edges.find(
              e => (e.sourceId === investorA && e.targetId === investorB) ||
            ...
            const sharedNeighbors = [...aReachable].filter(id => bReachable.has(id) && id !== investorA && id !== investorB);
            ...
            const confidence = Math.min(88, 30
              + (bothHighRisk ? 20 : 5)
            ...
            const severity: AnomalySeverity = bothHighRisk && distressScore >= 70 ? "high" : "medium";

            const signals: AnomalySignal[] = [
              { domain: "property", entityId: prop.id, metric: "Distress Score", value: distressScore, threshold: 70, breached: distressScore >= 70 },
              { domain: "financial", entityId: investorA, metric: `${entityA.label} Risk Score`, value: entityA.riskScore, threshold: 60, breached: aHighRisk },
              { domain: "financial", entityId: investorB, metric: `${entityB.label} Risk Score`, value: entityB.riskScore, threshold: 60, breached: bHighRisk },
              { domain: "legal", entityId: prop.id, metric: "Disclosed Co-Investor Relationship", value: directEdge ? "Yes" : "No", breached: !directEdge },
            ];

            const involvedEntityIds = [investorA, investorB, prop.id];
            const linkedEdgeIds = [inEdges[i].id, inEdges[j].id, ...(directEdge ? [directEdge.id] : [])];
            ...
            matches.push({
              ...
              domains: ["Property", "Financial", ...(involvedEntityIds.some(id => {
   ...
   */
  {
    ...
    description: "Same counterparty network under stress across 3+ verticals simultaneously — correlated risk beyond what any single-domain view would surface.",
    ...
    condition: (graph) => {
      ...
      const highRiskEntities = graph.entities.filter(e => e.riskScore >= 65 && ["person", "organization"].includes(e.type));
      ...
      for (const entity of highRiskEntities) {
        ...
        const hasLegalRisk = networkEntities.some(e => (e.domainData.legal?.aggregateExposure ?? 0) >= 2000000);
        const hasPropertyRisk = networkEntities.some(e => (e.domainData.property?.distressScore ?? 0) >= 70);
        const hasFinancialRisk = networkEntities.some(e => (e.domainData.financial?.facilityStatus ?? "").includes("litig") || (e.domainData.financial?.facilityStatus ?? "").includes("Dispute"));
        ...
        const networkRiskScore = Math.round(networkEntities.reduce((s, e) => s + e.riskScore, 0) / networkEntities.length);
        const confidence = Math.min(90, 30 + stressedDomains.length * 12 + (entity.riskScore >= 80 ? 10 : 0));
        ...
        matches.push({
          ...
          involvedEdgeIds: graph.edges.filter(e => reachable.has(e.sourceId) && reachable.has(e.targetId)).map(e => e.id),
          confidence,
