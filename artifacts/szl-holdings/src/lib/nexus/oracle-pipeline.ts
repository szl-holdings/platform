...
import { KnowledgeGraph, KNOWLEDGE_GRAPH, getEntity } from "./graph";
import { runAnomalyDetection, AnomalyMatch, AnomalyReport } from "./anomaly-engine";
 ...
 */
function deriveSeverity(entityIds: string[], anomalyMatches: AnomalyMatch[], graph: KnowledgeGraph): BriefSeverity {
...
export function generateOracleBrief(graph: KnowledgeGraph = KNOWLEDGE_GRAPH): OracleBrief {
  ...
  if (sanctionsEntities.length > 0 || ofacAnomalies.length > 0) {
    ...
    const maxRisk = entityIdArr.reduce((mx, id) => Math.max(mx, getEntity(id, graph)?.riskScore ?? 0), 0);
    const ofacConf = sanctionsEntities.reduce((mx, e) => Math.max(mx, e.domainData.threat?.ofacMatchConfidence ?? 0), 0);
    ...
    const findings: OracleFinding[] = [
      ...
      ...ofacAnomalies.flatMap(a => a.signals.filter(s => s.breached && s.domain === "property").map(s => ({
    ...
    const actions: OracleAction[] = [
      ...
      {
        priority: "immediate",
        action: "Pause any active or pending transactions with linked entities until sanctions status is formally resolved",
        owner: "Atlas / Finance",
        entityIds: entityIdArr,
      },
    ...
    sections.push({
      ...
      summary: ofacAnomalies[0]?.description ?? `OFAC SDN candidate match at ${ofacConf}% confidence detected for ${sanctionsEntities.map(e => e.label).join(", ")}. Vessel routing, AIS anomalies, and linked property distress all correlate with this exposure.`,
      findings,
  ...
  const aptEntities = graph.entities.filter(e => (e.domainData.threat?.aptAssociations?.length ?? 0) > 0 || e.domainData.threat?.ipOverlap);
  ...
  if (litCyberAnomalies.length > 0 || aptEntities.length > 0) {
    ...
    const findings: OracleFinding[] = [
      ...
      ...litCyberAnomalies.flatMap(a =>
        a.signals.filter(s => s.domain === "legal" && s.breached).map(s => ({
    ...
    sections.push({
      ...
      analystNote: "Moderate-high confidence. Threat actor attribution is probabilistic (IP range overlap only). Litigation/cyber timing coincidence warrants investigation but may not be causal. Treat as high priority pending further threat intelligence.",
      summary: topAnomaly?.description ?? `Threat indicator linked to entity infrastructure within 72h of litigation escalation. Pattern is consistent with adversarial litigation-cyber linkage.`,
      findings,
      recommendedActions: [
        { priority: "immediate", action: "Notify CISO — initiate threat hunt on any SZL systems with implicated entity API connections", owner: "Aegis SOC", entityIds: entityIdArr },
        { priority: "today", action: "Inform PRISM legal team of NEXUS-discovered cross-network co-investment — assess conflict implications", owner: "PRISM Counsel", entityIds: entityIdArr },
        { priority: "this-week", action: "Commission LP/beneficial ownership review of implicated fund — check for network overlap", owner: "Atlas / Legal", entityIds: entityIdArr },
      ],
      entityIds: entityIdArr,
  ...
  if (coInvestAnomalies.length > 0) {
    ...
    const findings: OracleFinding[] = coInvestAnomalies.flatMap(a => [
      ...
      ...a.signals.filter(s => s.breached).map(s => ({
        label: s.metric,
    ...
    sections.push({
      ...
      domainColor: "#4ade80",
      analystConfidence: topConf,
      analystNote: "Moderate confidence. Ownership link is confirmed via title records. Cross-entity principal relationship is inferred by NEXUS — no publicly disclosed association exists. Structure warrants due diligence but is not inherently improper without further evidence.",
      ...
      findings,
      recommendedActions: [
        { priority: "today", action: "Commission independent appraisal and lender covenant review on affected distressed assets", owner: "Terra Engine", entityIds: entityIdArr },
        { priority: "this-week", action: "Request PRISM title opinion on identified ownership structures — assess undisclosed conflicts", owner: "PRISM Counsel", entityIds: entityIdArr },
        { priority: "this-week", action: "Update NEXUS graph with confirmed co-investment edges · Raise combined entity risk scores", owner: "NEXUS Ops", entityIds: [] },
      ],
      entityIds: entityIdArr,
  ...
  if (stressAnomalies.length > 0) {
    const entityIdSet = new Set<string>();
    for (const a of stressAnomalies) a.involvedEntityIds.slice(0, 6).forEach(id => entityIdSet.add(id));
    const entityIdArr = [...entityIdSet];
    ...
    sections.push({
      ...
      domainColor: "#eab308",
      analystConfidence: Math.round(stressAnomalies.reduce((mx, a) => Math.max(mx, a.confidence), 75)),
      analystNote: "High confidence on trend identification. Causal linkage between verticals is probabilistic. Pattern is consistent with credit cycle stress in overlapping counterparty networks.",
      ...
      findings,
      recommendedActions: [
        { priority: "today", action: "Atlas to produce cross-domain counterparty concentration report — identify all entities appearing in 2+ verticals", owner: "Atlas", entityIds: entityIdArr },
        { priority: "this-week", action: "Convene cross-domain risk committee — review NEXUS-identified concentrations before Q2 close", owner: "SZL Executive", entityIds: [] },
      ],
      entityIds: entityIdArr,
  ...
  const legalEntities = graph.entities.filter(e => (e.domainData.legal?.matterIds?.length ?? 0) > 0);
  const totalExposure = legalEntities.reduce((s, e) => s + (e.domainData.legal?.aggregateExposure ?? 0), 0);
  const totalArbitrations = legalEntities.reduce((s, e) => s + (e.domainData.legal?.activeArbitrations ?? 0), 0);

  if (legalEntities.length > 0) {
    const entityIdArr = legalEntities.map(e => e.id);
    const highestExposure = legalEntities.reduce((prev, e) =>
      (e.domainData.legal?.aggregateExposure ?? 0) > (prev.domainData.legal?.aggregateExposure ?? 0) ? e : prev
    );
    ...
    const actions: OracleAction[] = [
      ...
      {
        priority: "today",
        action: "Brief PRISM counsel on any NEXUS-discovered conflicts affecting active matters — assess disclosure obligations",
        owner: "PRISM Counsel",
        entityIds: entityIdArr,
      },
      {
    ...
    sections.push({
      ...
      analystNote: "High confidence. All findings sourced directly from PRISM case management system with no inference required.",
  ...
  sections.push({
    ...
    findings: [
      { label: "Platform Status", detail: "Vessels, PRISM, Terra, Aegis, Alloy — all operational · Zero incidents", severity: "info", entityIds: [] },
      { label: "NEXUS Graph", detail: `${totalEntities} entities · ${totalEdges} edges (${inferredEdges} inferred) · Version ${graph.version} · Last refreshed ${graph.lastRefreshed}`, severity: "info", entityIds: [] },
      { label: "Anomaly Detection", detail: `${anomalyReport.totalCount} cross-domain anomalies detected · ${anomalyReport.bySeverity.critical} critical · ${anomalyReport.bySeverity.high} high`, severity: "info", entityIds: [] },
      { label: "ORACLE Pipeline", detail: `${sections.length + 1} sections generated · ${Date.now() - startTime}ms runtime · Graph revision ${graph.version}`, severity: "info", entityIds: [] },
    ],
    recommendedActions: [
      { priority: "this-week", action: "NEXUS analyst review of all inferred edges — validate or retract low-confidence connections", owner: "NEXUS Ops", entityIds: [] },
    ],
    entityIds: [],
    drillDownPath: "/nexus",
  });
  ...
  const immediateActions = sections.flatMap(s => s.recommendedActions).filter(a => a.priority === "immediate");
  ...
  const brief: OracleBrief = {
    ...
    executiveSummary: {
      ...
      keyDevelopment: anomalyReport.anomalies.find(a => a.patternId === "P003")?.title
        ?? anomalyReport.anomalies[0]?.title
        ?? "No major new developments",
      trendShift: anomalyReport.anomalies.find(a => a.patternId === "P004")?.description?.slice(0, 180)
