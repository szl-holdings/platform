...
export function NextActionsPanel({ scenarioId }: { scenarioId?: number }) {
  ...
  const allActions = [
    ...data.unconfirmedCitizenships.map((c) => ({
      ...
      description:
        'U.S. citizenship documentation required for WOSB/MWBE certification eligibility of controlling owner.',
      ...
      {allActions.length === 0 ? (
        <div className="flex items-center gap-3 bg-emerald-500/8 border border-emerald-500/20 rounded-xl p-4">
          ...
          <p className="text-sm text-emerald-400">
            No open action items. All readiness checks passed.
          </p>
        </div>
      ) : (
        allActions.map((action, i) => (
          <div
            key={i}
            className="flex items-start gap-3 bg-card border border-border rounded-xl p-3.5 hover:border-border/80 transition-colors"
          >
