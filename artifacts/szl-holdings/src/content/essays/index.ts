...
<p>Third: the audit record must be immutable. Not a database table that can be edited — a proof chain that shows state transitions and is structurally resistant to retroactive modification. Alloy's proof chain is not a legal requirement (though it satisfies legal requirements). It's an engineering requirement. You cannot have governed systems without tamper-evident records.</p>
...
<p>Alloy is the execution fabric. It handles workflow orchestration, AI agent coordination, connector management, signal ingestion, approval routing, and audit logging. Every other platform in the SZL portfolio runs on top of it. Lyte doesn't have its own workflow engine. Vessels doesn't have its own audit system. Terra doesn't have its own approval layer. They're all Alloy surfaces.</p>

<p>This is not a cost-cutting measure. It's a compounding bet.</p>

<p>When we improve Alloy's confidence scoring, every vertical gets better predictions. When we add a new connector to Alloy, every vertical gains that integration. When we harden Alloy's proof chain, every vertical gets stronger audit evidence. The marginal cost of adding capability to the fabric is near zero per vertical — it's already there.</p>
...
<p>This means the vertical teams must resist the temptation to build their own execution logic. Every time you build a workflow inside a vertical instead of in Alloy, you create a governance gap. The rule is simple: if it touches data, it goes through the fabric. If it makes a recommendation, it goes through Alloy. If it executes an action, the proof chain sees it.</p>
