...
<p>Terra speaks in property signals, deal stages, and ownership graphs. Vessels speaks in vessel tracks, port calls, and route anomalies. Aegis speaks in threat vectors, indicator correlations, and incident timelines. They're genuinely different domains. The discipline is building each in its native language while routing consequential actions through Continuum.</p>

<p>This means the vertical teams must resist the temptation to build their own execution logic. Every time you build a workflow inside a vertical instead of in Counsel, you create a governance gap. The rule is simple: if it touches data, it goes through the fabric. If it makes a recommendation, it goes through Continuum. If it executes an action, the proof chain sees it.</p>
