...
export function ActLayer() {
  ...
  const { data: pipelinesData, isLoading: pipelinesLoading } = useStandardQuery<{
  ...
  const pipelines =
    ((pipelinesData?.data as Record<string, unknown>)?.pipelines as unknown[]) ?? [];
  const selected = pipelines.find(
    (p: unknown) => (p as Record<string, unknown>).id === selectedPipeline,
  ...
  const removeStage = useCallback(
    (id: string) => setComposedStages((prev) => prev.filter((s) => s.id !== id)),
    [],
  );
                        ...
                        <div className="flex items-center gap-2 mb-1.5">
                          <span
                            className={cn(
                              ...
                              DOMAIN_COLORS[String(pl.domain)] ?? 'text-muted-foreground',
                            )}
                          >
                            {String(pl.domain)}
                          </span>
                  ...
                  {(selected.stages as unknown[]).map((stage: unknown, i) => {
                    ...
                    const resultStage = pipelineResult
                      ? ((
                          ...
                          (rs: unknown) =>
                            (rs as Record<string, unknown>).stageName === String(s.name),
                        ) as Record<string, unknown> | undefined)
                      : undefined;
                    ...
                    return (
                      <div key={String(s.name)} className="flex items-center gap-1.5 shrink-0">
                        <div
                          className={cn(
                            'text-center px-2 py-1.5 rounded-lg border min-w-16',
                            STAGE_COLOR_MAP[String(s.type)] ?? '',
                          )}
                        >
                          <p className="text-[9px] font-semibold uppercase">{String(s.type)}</p>
                          <p className="text-[9px] text-muted-foreground mt-0.5 max-w-16 truncate">
                            {String(s.name)}
                          </p>
                          {stageStatus && (
                            <span
            ...
            <SectionCard title="Execute Pipeline" icon={Play} color="text-amber-400">
              {!selected ? (
                <p className="text-xs text-muted-foreground text-center py-4">
                  Select a pipeline template to execute
                </p>
              ) : (
                <div className="space-y-2">
                  ...
                  <button
                    onClick={() =>
                      selectedPipeline &&
                      pipelineInput &&
                      runMutation.mutate({ pipelineId: selectedPipeline, input: pipelineInput })
                    }
              ...
              {pipelineResult && (
                <div className="mt-3 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {String(
                        (pipelineResult?.result as Record<string, unknown>)?.status ?? 'unknown',
                      )}
                    </span>
                    <span className="text-[10px] text-muted-foreground">
                      {String(
                        (pipelineResult?.result as Record<string, unknown>)?.totalDurationMs ?? 0,
                      )}
                      ms
                    </span>
                  </div>
                  <div className="p-2.5 bg-muted/10 border border-border/50 rounded-lg max-h-32 overflow-y-auto">
                    <p className="text-[10px] text-muted-foreground leading-relaxed">
                      {String(
                        (pipelineResult?.result as Record<string, unknown>)?.finalOutput ?? '',
                      ).slice(0, 600)}
                    </p>
                  </div>
                </div>
            ...
            <SectionCard title="Execute Composed Pipeline" icon={Play} color="text-amber-400">
              {composedStages.length === 0 ? (
                <p className="text-xs text-muted-foreground text-center py-3">
                  Build a pipeline on the canvas first
                </p>
              ) : (
                <div className="space-y-2">
                  <textarea
                  ...
                  <button
                    onClick={() =>
                      composerInput &&
                      composerRunMutation.mutate({ stages: composedStages, input: composerInput })
                    }
                  ...
                  >
                    <Play className="w-3 h-3" />
                    {composerRunMutation.isPending
                      ? 'Running composed pipeline…'
                      : `Run ${composedStages.length}-Stage Pipeline`}
                  </button>
                </div>
              )}
              {composerResult && (
                <div className="mt-3 space-y-2">
                  ...
                  <div className="p-2.5 bg-muted/10 border border-border/50 rounded-lg max-h-32 overflow-y-auto mt-1">
                    <p className="text-[10px] text-muted-foreground leading-relaxed">
                      {String(
                        (composerResult as Record<string, unknown>)?.finalOutput ??
                          JSON.stringify(composerResult),
                      ).slice(0, 600)}
                    </p>
                  </div>
                </div>
