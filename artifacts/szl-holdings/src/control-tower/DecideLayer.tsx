...
import { Brain, ChevronDown, Cpu, GitBranch, Play } from 'lucide-react';
import { useState } from 'react';

import { cn } from '@/lib/utils';
import { ConfidenceBadge, SectionCard, StatusDot, TimeAgo } from './components';
...
export function DecideLayer() {
  const [orchestrateForm, setOrchestrateForm] = useState<OrchestrateForm>({
    query: '',
    depth: 'standard',
  });
  ...
  } = useStandardQuery<{ data: Record<string, unknown> }>({
    queryKey: ['ct-journal'],
  ...
  const healthSummary = (agentsData?.data as Record<string, unknown>)?.healthySummary as
    | Record<string, number>
    | undefined;
  const journalEntries =
    ((journalData?.data as Record<string, unknown>)?.entries as unknown[]) ?? [];
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: 'Degraded', value: healthSummary?.degraded ?? 0, color: 'text-amber-400' },
          {
            ...
            value: (journalData?.data as Record<string, unknown>)?.total ?? 0,
            ...
            {agentsLoading
              ...
              : agents.map((agent: unknown) => {
                  ...
                  return (
                    <div
                      key={String(a.id)}
                      className="flex items-start gap-3 p-2.5 rounded-lg bg-muted/10 border border-border/30"
                    >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={cn(
                              ...
                              DOMAIN_COLORS[String(a.domain)] ?? 'text-muted-foreground',
                            )}
                          >
                            {String(a.domain)}
                          </span>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <select
                  ...
                  onChange={(e) =>
                    setOrchestrateForm((f) => ({
                      ...f,
                      depth: e.target.value as OrchestrateForm['depth'],
                    }))
                  }
                >
            ...
            {orchestrateMutation.data && (
              <div className="mt-3 p-2.5 bg-muted/10 border border-border/50 rounded-lg">
                ...
                <p className="text-xs text-foreground leading-relaxed line-clamp-4">
                  {(
                    (orchestrateMutation.data as Record<string, unknown>)?.data as Record<
                      string,
                      unknown
                    >
                  )?.result
          ...
          {journalLoading ? (
            ...
            <div className="text-center py-6">
              ...
              <p className="text-xs text-muted-foreground">
                No decisions journaled yet — run an orchestration to populate
              </p>
            </div>
          ) : (
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        ...
                        DOMAIN_COLORS[String(e.domain)] ?? 'text-muted-foreground',
                      )}
                    >
                      {String(e.domain)}
                    </span>
                    <span className="text-xs text-foreground flex-1 truncate">
                      {String(e.query ?? '').slice(0, 80)}
                    </span>
                          ...
                          <div className="space-y-0.5">
                            {(e.reasoningChain as string[]).slice(0, 3).map((r, i) => (
                              <div key={i} className="flex items-start gap-1">
                                <span className="text-[9px] text-muted-foreground/50 font-mono mt-0.5 shrink-0">
                                  {i + 1}.
                                </span>
