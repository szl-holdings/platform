...
import { FlaskConical, Server, Sparkles, TrendingUp } from 'lucide-react';
import { useState } from 'react';

import { cn } from '@/lib/utils';
import { ConfidenceBadge, SectionCard, StatusDot } from './components';
...
export function EvolveLayer() {
  ...
  const agentMetrics =
    ((metricsData?.data as Record<string, unknown>)?.agentMetrics as unknown[]) ?? [];
  const systemMetrics = (metricsData?.data as Record<string, unknown>)?.systemMetrics as
    | Record<string, unknown>
    | undefined;
  const providerHealth =
    ((metricsData?.data as Record<string, unknown>)?.providerHealth as unknown[]) ?? [];
        ...
        {[
          {
            ...
            value: String(systemMetrics?.totalInferences ?? 0),
            color: 'text-fuchsia-400',
          },
          {
            label: 'Avg Latency',
            value: `${Math.round(Number(systemMetrics?.avgLatencyMs ?? 0))}ms`,
            color: 'text-sky-400',
          },
          {
            label: 'Total Decisions',
            value: String(systemMetrics?.totalDecisions ?? 0),
            ...
            value: String(
              agentMetrics.reduce(
                (s: number, a) =>
                  s +
                  (
                    ((a as Record<string, unknown>)?.optimizationProposals as unknown[]) ?? []
                  ).filter((p: unknown) => (p as Record<string, unknown>).status === 'pending')
                    .length,
                0,
              ),
            ),
            ...
            {metricsLoading
              ...
              : agentMetrics.map((agent: unknown) => {
                  ...
                  const pendingProposals = proposals.filter(
                    (p: unknown) => (p as Record<string, unknown>).status === 'pending',
                  );
                  return (
                    <div
                      key={String(a.agentId)}
                      className="p-2.5 rounded-lg bg-muted/10 border border-border/30"
                    >
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <div>
                          ...
                          <p className="text-[10px] font-mono text-foreground">
                            {String(metrics?.totalDecisions ?? 0)}
                          </p>
                        </div>
                        <div>
                      ...
                      {proposals.slice(0, 1).map((prop: unknown) => {
                        ...
                        return (
                          <div
                            key={String(p.id)}
                            className="mt-2 p-1.5 bg-amber-500/5 border border-amber-500/20 rounded text-[9px]"
                          >
              ...
              {providerHealth.slice(0, 5).map((p: unknown) => {
                ...
                return (
                  <div key={String(ph.provider)} className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-mono text-foreground w-24 shrink-0">
                      {String(ph.provider)}
                    </span>
          ...
          <SectionCard title="Propose Optimization" icon={FlaskConical} color="text-amber-400">
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={(e) =>
                  setProposeForm((f) => ({ ...f, expectedImprovement: e.target.value }))
                }
              />
              <button
                onClick={() =>
