...
export function SearchLayer() {
  ...
  const domainsSearched =
    ((searchData?.data as Record<string, unknown>)?.domainsSearched as unknown[]) ?? [];
  const latency = (searchData?.data as Record<string, unknown>)?.searchLatencyMs as
    | number
    | undefined;
  const DOMAINS = ['aegis', 'vessels', 'terra', 'lyte', 'prism', 'continuum'];
      ...
      <div className="bg-card border border-border rounded-xl p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[10px] text-muted-foreground ml-auto">
            Queries signals, decisions, and artifacts across all domains
          </span>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            {domainsSearched.map((d: unknown) => {
              ...
              return (
                <span
                  key={String(ds.domain)}
                  className={cn(
                    'font-mono',
                    DOMAIN_COLORS[String(ds.domain)] ?? 'text-muted-foreground',
                  )}
                >
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <span
                        className={cn(
                          'text-[9px] font-mono px-1.5 py-0.5 rounded border shrink-0 mt-0.5',
                          DOMAIN_COLORS[String(result.domain)] ?? 'text-muted-foreground',
                        )}
                      >
