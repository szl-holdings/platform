...
import { Activity, Hash, Radio, RefreshCw } from 'lucide-react';
import { useState } from 'react';

import { cn } from '@/lib/utils';
import { SectionCard, TimeAgo } from './components';
import { API_BASE, DOMAIN_COLORS, SEVERITY_COLORS } from './constants';
...
export function SenseLayer() {
  ...
  const snapshot = (signalData?.data as Record<string, unknown>)?.snapshot as
    | Record<string, unknown>
    | undefined;
  const domainSummary = (snapshot?.domainSummary as Record<string, unknown>[]) ?? [];
  const simSignals =
    ((domainSnapshot?.data as Record<string, unknown>)?.signals as unknown[]) ?? [];
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: 'Total Published', value: snapshot?.totalSignals ?? 0, color: 'text-sky-400' },
          {
            ...
            value: snapshot?.activeSubscribers ?? 0,
            color: 'text-violet-400',
          },
          {
            label: 'History Buffer',
            value: snapshot?.historyWindowSize ?? 0,
          ...
          <SectionCard title="Live Signal Feed" icon={Radio} color="text-sky-400">
            <div className="flex items-center gap-2 mb-3">
              ...
              >
                ...
                {['aegis', 'vessels', 'terra', 'lyte', 'prism', 'continuum', 'orchestration'].map(
                  (d) => (
                    ...
                    >
                      <span
                        className={cn(
                          ...
                          DOMAIN_COLORS[domain] ?? 'text-muted-foreground',
                        )}
                      >
                        {domain}
                      </span>
                      ...
                      <span className="font-medium flex-1 truncate">
                        {String(s.title ?? type.replace(/_/g, ' '))}
                      </span>
                      <span
                        className={cn(
                          'shrink-0 px-1 py-0.5 rounded text-[9px] font-semibold uppercase tracking-wide border',
        ...
        <div className="space-y-4">
                    ...
                    <div key={domain} className="flex items-center gap-2">
                      ...
                      <div className="flex-1 h-1 bg-muted/30 rounded-full overflow-hidden">
                        <div
                          className={cn(
                            'h-full rounded-full',
                            DOMAIN_COLORS[domain]?.replace('text-', 'bg-') ?? 'bg-muted',
                          )}
                          style={{
                            width: `${Math.min(100, (Number(dom.count ?? 0) / 50) * 100)}%`,
                          }}
                        />
                      </div>
          ...
          <SectionCard title="Event Types" icon={Hash} color="text-sky-400">
            <div className="space-y-1 max-h-40 overflow-y-auto">
              {Object.entries((snapshot?.eventsByType as Record<string, number>) ?? {}).length ===
              0 ? (
