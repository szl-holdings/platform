...
import { ArrowUpRight, Brain, Cpu, Lock, Radio, Shield } from 'lucide-react';
...
export function StatusDot({ status }: { status: string }) {
  ...
  return (
    <span
      className={cn('inline-block w-1.5 h-1.5 rounded-full flex-shrink-0 animate-pulse', color)}
    />
  );
}
...
export function ControlTowerStatusBar() {
  ...
  const kpis = [
    {
      label: 'Total Signals',
      value: sense ? String(sense.totalSignalsPublished ?? 0) : '—',
      icon: Radio,
      color: 'text-sky-400',
    },
    {
      label: 'Agents Registered',
      value: decide ? String(decide.registeredAgents ?? 0) : '—',
      icon: Cpu,
      color: 'text-violet-400',
    },
    {
      label: 'Decisions Journaled',
      value: decide ? String(decide.totalDecisionsJournaled ?? 0) : '—',
      icon: Brain,
      color: 'text-fuchsia-400',
    },
    {
      label: 'Compliance Score',
      value: govern ? `${govern.overallComplianceScore}` : '—',
      icon: Shield,
      color: 'text-emerald-400',
    },
    {
      label: 'Audit Entries',
      value: govern ? String(govern.totalAuditEntries ?? 0) : '—',
      icon: Lock,
