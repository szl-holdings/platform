...
import { CheckCircle2, Circle, Eye, Lock, Shield, Unlock, XCircle } from 'lucide-react';
...
export function GovernLayer() {
  const { data: complianceData, isLoading: complianceLoading } = useStandardQuery<{
  ...
  const { data: certsData, isLoading: certsLoading } = useStandardQuery<{
    data: Record<string, unknown>;
  }>({
    queryKey: ['ct-certs'],
  ...
  const integrity = (auditData?.data as Record<string, unknown>)?.integrity as
        ...
        {[
          {
            ...
            value: complianceLoading ? '—' : `${score}`,
            color:
              score >= 90 ? 'text-emerald-400' : score >= 70 ? 'text-amber-400' : 'text-red-400',
          },
          {
            label: 'Risk Posture',
            value: String(compliance?.riskPosture ?? '—'),
            color:
              compliance?.riskPosture === 'low'
                ? 'text-emerald-400'
                : compliance?.riskPosture === 'medium'
                  ? 'text-amber-400'
                  : 'text-red-400',
          },
          {
            ...
            color: integrity?.valid ? 'text-emerald-400' : 'text-red-400',
          },
          {
            label: 'Total Audit Entries',
            value: String(compliance?.totalAuditEntries ?? 0),
            color: 'text-sky-400',
          },
              ...
              : policies.map((policy: unknown) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-[9px] text-muted-foreground line-clamp-2 pl-5">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span
                          className={cn(
                            'text-[9px] font-mono px-1 py-0.5 rounded border ml-auto',
                            DOMAIN_COLORS[String(c.domain)] ?? 'text-muted-foreground',
                          )}
                        >
                          {String(c.domain)}
                        </span>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span
                          className={cn(
                            'text-[9px] font-mono',
                            certificate?.maxRiskLevel === 'critical'
                              ? 'text-red-400'
                              : certificate?.maxRiskLevel === 'high'
                                ? 'text-orange-400'
                                : 'text-emerald-400',
                          )}
                        >
