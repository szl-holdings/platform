...
import { useReviews, useStoryboards, useUpdateReview } from '@/continuum/hooks/use-creative';
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {storyboards?.map((scene) => (
              <div
                key={scene.id}
                className="overflow-hidden border border-white/8 hover:border-blue-400/20 transition-colors rounded-xl bg-[#0d1117]"
              >
      ...
      <div className="flex flex-col gap-6">
        <div className="bg-[#0d1117] border border-white/8 p-6 rounded-2xl flex flex-col gap-4">
          <div className="flex items-center justify-between border-b border-white/8 pb-3">
            ...
            <span className="text-[10px] px-2 py-1 rounded border border-white/10 text-slate-400">
              Round {currentRound}
            </span>
          </div>
            ...
            {totalReviews > 0 && (
              <>
                ...
                <div
                  ...
                  style={{ width: `${(changesCount / totalReviews) * 100}%` }}
                />
              </>
            )}
          </div>
          ...
          <div className="space-y-3">
            {reviews?.map((review) => {
              ...
              return (
                <div
                  key={review.id}
                  className={`bg-black/30 rounded-xl p-4 border border-white/8 border-l-2 ${DEPARTMENT_COLORS[dept] || ''}`}
                >
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <div className="font-medium text-sm text-white">
                        {review.reviewer ?? review.reviewerName}
                      </div>
                    ...
                    <span
                      className={`text-[10px] px-2 py-1 rounded capitalize border flex items-center gap-1 ${review.status === 'approved' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : review.status === 'changes_requested' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-white/5 text-slate-400 border-white/10'}`}
                    >
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <div className="text-[10px] text-slate-500 flex items-center gap-1">
                      ...
                      {format(
                        new Date(review.date ?? review.createdAt ?? new Date()),
                        'MMM d, h:mm a',
                      )}
                    </div>
                    <span className="text-[9px] px-2 py-0.5 rounded border border-white/10 text-slate-500">
                      {review.department}
                    </span>
                  </div>
                  ...
                  {review.status === 'pending' && (
                    <div className="flex gap-2 mt-4 pt-3 border-t border-white/8">
                      <button
                        ...
                        onClick={() =>
                          updateReview.mutate({ id: review.id, campaignId, status: 'approved' })
                        }
                      >
