...
import { useAssets, useDeleteAsset } from '@/continuum/hooks/use-creative';
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        {filteredAssets?.map((asset) => {
          ...
          return (
            <div
              key={asset.id}
              className={`group overflow-hidden flex flex-col relative bg-[#0d1117] border border-white/8 hover:border-blue-400/30 transition-all hover:shadow-xl hover:-translate-y-1 border-l-2 rounded-xl ${borderColor}`}
            >
              ...
              <div className="p-3 flex flex-col flex-1">
                ...
                <div className="mt-auto flex flex-wrap gap-1">
                  {(asset.tags || []).slice(0, 3).map((tag) => (
                    <span
                      key={tag}
                      className="text-[9px] px-1.5 py-0.5 rounded border border-white/10 text-slate-500"
                    >
                      {tag}
                    </span>
                  ))}
                  {(asset.tags || []).length > 3 && (
                    <span className="text-[9px] px-1.5 py-0.5 rounded border border-white/10 text-slate-500">
                      +{(asset.tags || []).length - 3}
                    </span>
                  )}
                </div>
