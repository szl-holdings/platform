import { useContinuumSignals, useContinuumWorkflows } from '@szl-holdings/graphql-client/hooks';
import { GraphQLDataPanel } from '@szl-holdings/shared-ui/design-system';

export function ContinuumGraphQLPanel() {
  const { data: workflowsData, loading: wLoading } = useContinuumWorkflows({ limit: 3 });
  const { data: signalsData, loading: sLoading } = useContinuumSignals({ limit: 3 });
