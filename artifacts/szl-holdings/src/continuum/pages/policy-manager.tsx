...
function NewPolicyDialog({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const mutation = useStandardMutation({
    mutationFn: async () => {
      ...
      await apiFetch('/continuum/policies', {
        method: 'POST',
...
export default function PolicyManager() {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        const resp = await apiFetch<Policy[]>('/continuum/policies');
  ...
  const { data: apiIncidents } = useStandardQuery({
    ...
    queryFn: async () => {
      try {
        const resp = await apiFetch<GovernanceIncident[]>('/continuum/governance/incidents');
