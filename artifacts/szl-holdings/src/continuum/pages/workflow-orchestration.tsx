...
function useWorkflows() {
  return useStandardQuery({
    ...
    queryFn: async () => {
      const resp = await apiFetch<Workflow[] | { data: Workflow[] }>('/continuum/workflows');
...
function useUpdateWorkflow() {
  ...
  return useStandardMutation({
    mutationFn: async ({ id, ...data }: { id: number } & Partial<Workflow>) => {
      return await apiFetch<Workflow>(`/continuum/workflows/${id}`, {
        method: 'PATCH',
...
function useStartRun() {
  ...
  return useStandardMutation({
    mutationFn: async (workflowId: number) => {
      return await apiFetch<{ id: number }>('/continuum/runs', {
        method: 'POST',
...
const STATUS_CONFIG: Record<
  string,
  { color: string; label: string; bg: string; border: string; dotColor: string }
> = {
  active: {
    color: '#10b981',
    label: 'Active',
  ...
  return [
    {
      id: 1,
      orgId: 1,
      name: 'Client Onboarding — Enterprise Tier',
      description:
        'End-to-end onboarding flow: KYC verification → legal review → account provisioning → welcome sequence.',
      status: 'active',
      product: 'continuum-core',
      kind: 'onboarding',
      config: {
        steps: ['KYC Check', 'Legal Review', 'Account Setup', 'Welcome Email', 'Assign CSM'],
      },
      metadata: {
        owner: 'Sarah Chen',
        team: 'Client Success',
        sla_deadline: '48h',
        value_at_risk: 2400000,
      },
      createdAt: ago(720),
      updatedAt: ago(2),
    },
    {
      ...
      description: 'Automated renewal tracking with escalation paths for at-risk accounts.',
      status: 'active',
      product: 'continuum-core',
      kind: 'contract',
      config: {
        steps: ['90-Day Notice', 'Usage Review', 'Pricing Update', 'Legal Approval', 'Signature'],
      },
      metadata: {
        owner: 'Marcus Rivera',
        team: 'Revenue Ops',
        sla_deadline: '72h',
        value_at_risk: 8500000,
      },
      createdAt: ago(1440),
      updatedAt: ago(6),
    },
    {
      ...
      description: 'Continuous compliance evidence gathering across all control families.',
      status: 'active',
      product: 'continuum-compliance',
      kind: 'compliance',
      config: {
        steps: [
          'Access Reviews',
    ...
    },
    {
      ...
      description:
        'Multi-tier approval chain for vendor invoices > $10K with automatic escalation.',
      status: 'active',
      product: 'continuum-finance',
      kind: 'finance',
      config: {
        steps: [
          'Receipt Scan',
      ...
      metadata: { owner: 'James Okafor', team: 'Finance', sla_deadline: '5 business days' },
      createdAt: ago(960),
      updatedAt: ago(1),
    },
    {
      id: 5,
      orgId: 1,
      name: 'Quarterly Board Report Assembly',
      description:
        'Automated data pull from 12 sources, formatting, and review cycle for board materials.',
      status: 'paused',
      product: 'continuum-core',
      kind: 'automation',
      config: {
        steps: ['Data Pull', 'KPI Calc', 'Narrative Draft', 'Exec Review', 'Final Format'],
      },
      metadata: { owner: 'Lisa Thornton', team: 'Strategy', sla_deadline: 'Q2 2026' },
      createdAt: ago(480),
      updatedAt: ago(48),
    },
    {
      ...
      description: 'Automated vendor security questionnaire distribution, scoring, and tracking.',
      status: 'active',
      product: 'continuum-compliance',
      kind: 'compliance',
      config: {
    ...
    },
    {
      ...
      description:
        'Coordinated deprovisioning across IT, HR, and facilities with compliance checks.',
      status: 'active',
      product: 'continuum-hr',
      kind: 'onboarding',
      config: {
        steps: [
          'HR Notify',
      ...
      metadata: { owner: 'Kenji Watanabe', team: 'People Ops' },
      createdAt: ago(2880),
      updatedAt: ago(18),
    },
    {
      ...
      description:
        'Monitors ETL pipeline health and triggers alerts on schema drift or SLA breach.',
      status: 'error',
      product: 'continuum-data',
      kind: 'automation',
      config: {
        steps: ['Schema Validate', 'Row Count Check', 'Latency Monitor', 'Alert Dispatch'],
      },
      metadata: { owner: 'Raj Patel', team: 'Data Engineering', value_at_risk: 350000 },
      createdAt: ago(168),
      updatedAt: ago(0.5),
    },
    {
      ...
      description:
        'Creative review, legal compliance check, and budget sign-off for campaign launches.',
      status: 'draft',
      product: 'continuum-marketing',
      kind: 'approval',
      config: {
        steps: ['Creative Review', 'Brand Check', 'Legal Scan', 'Budget Approve', 'Schedule'],
      },
      metadata: { owner: 'Aisha Johnson', team: 'Marketing' },
      createdAt: ago(72),
      updatedAt: ago(72),
    },
    {
      id: 10,
      orgId: 1,
      name: 'Incident Post-Mortem Pipeline',
      description:
        'Structured post-mortem process with auto-generated timelines and action item tracking.',
      status: 'active',
      product: 'continuum-ops',
      kind: 'automation',
      config: {
        steps: ['Timeline Build', 'Root Cause', 'Action Items', 'Review Meeting', 'Publish'],
      },
      metadata: { owner: 'Tom Bradley', team: 'SRE', sla_deadline: '5 business days' },
      createdAt: ago(1080),
      updatedAt: ago(8),
    },
  ];
      ...
      {steps.slice(0, 5).map((step, i) => (
        <div key={i} className="flex items-center gap-1">
          <span
            ...
            style={{
              color: 'rgba(75,139,219,0.5)',
              background: 'rgba(75,139,219,0.05)',
          ...
          {i < steps.slice(0, 5).length - 1 && (
            <ChevronRight className="w-2 h-2" style={{ color: 'rgba(255,255,255,0.15)' }} />
          )}
        </div>
      ))}
      {steps.length > 5 && (
...
function WorkflowDrawer({
  ...
  workflow: Workflow;
  onClose: () => void;
  onUpdate: (id: number, data: Partial<Workflow>) => void;
  ...
  return (
    <div className="fixed inset-0 z-50 flex" onClick={onClose}>
      ...
      <div
        className="w-full max-w-lg bg-[#0c1420] border-l border-white/10 flex flex-col h-full overflow-y-auto"
      ...
      >
        <div className="p-5 border-b border-white/5">
          <div className="flex items-start justify-between gap-3 mb-3">
            ...
            <button
              onClick={onClose}
              className="text-slate-400 hover:text-white text-xs p-1 rounded hover:bg-white/5 transition-colors"
            >
              ✕
            </button>
          </div>
            ...
            {!!meta.owner && (
              <div className="bg-white/3 rounded-lg p-3 border border-white/5">
                ...
                <div className="text-[11px] text-white flex items-center gap-1">
                  <User className="w-3 h-3 text-blue-400/50" />
            ...
            {(meta.value_at_risk as number) > 0 && (
              <div className="bg-white/3 rounded-lg p-3 border border-amber-500/10">
                ...
                <div className="text-[11px] font-bold text-amber-400">
                  $
                  {(meta.value_at_risk as number) >= 1e6
                    ? `${((meta.value_at_risk as number) / 1e6).toFixed(1)}M`
                    : `${((meta.value_at_risk as number) / 1000).toFixed(0)}K`}
                </div>
              </div>
            )}
          </div>
        ...
        {Object.keys(config).length > 0 && (
          <div className="p-5 border-b border-white/5">
            <div
              className="flex items-center gap-1.5 text-[10px] font-medium mb-2"
              style={{ color: 'rgba(75,139,219,0.7)' }}
            >
              <Terminal className="w-3 h-3" /> Configuration
            </div>
            <pre className="text-[10px] text-slate-400 overflow-auto bg-white/3 rounded-lg p-3 border border-white/5 max-h-48">
        ...
        <div className="p-5 mt-auto">
          <div className="flex flex-wrap gap-2">
            {workflow.status !== 'active' && (
              <button
                onClick={() => {
                  onUpdate(workflow.id, { status: 'active' });
                  onClose();
                }}
                className="text-[10px] px-3 py-1.5 rounded-lg font-medium text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 hover:opacity-80 transition-all flex items-center gap-1"
              >
            ...
            {workflow.status === 'active' && (
              <button
                onClick={() => {
                  onUpdate(workflow.id, { status: 'paused' });
                  onClose();
                }}
                className="text-[10px] px-3 py-1.5 rounded-lg font-medium text-amber-400 bg-amber-500/10 border border-amber-500/20 hover:opacity-80 transition-all flex items-center gap-1"
              >
            ...
            <button
              ...
              className="text-[10px] px-3 py-1.5 rounded-lg font-medium flex items-center gap-1"
              style={{
                color: '#4B8BDB',
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>
            Manage execution workflows — step owners, SLA tracking, and reroute capabilities.
          </p>
        </div>
        <button
          onClick={() => refetch()}
          ...
          {[
            ...
            >
              <div className="flex items-center justify-center gap-1.5 mb-0.5">
                <span className="text-xl font-bold font-mono" style={{ color: c.color }}>
                  {c.value}
                </span>
                {c.pulse && c.value > 0 && (
                  <span
      ...
      {!isLoading && filtered.length === 0 && (
        ...
        >
          ...
          <p className="text-[11px] mt-1" style={{ color: 'rgba(255,255,255,0.2)' }}>
            Create a workflow via the API or import from config
          </p>
        </div>
      )}
            ...
            >
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                      <span
                        className="flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest px-1.5 py-0.5 rounded border"
                        style={{ color: s.color, background: s.bg, borderColor: s.border }}
                      >
                        {isActive && (
                          <span
                  ...
                  <div className="text-right shrink-0">
                    {valueAtRisk > 0 && (
                      <div>
                        <div className="text-sm font-bold" style={{ color: '#f59e0b' }}>
                          $
                          {valueAtRisk >= 1_000_000
                            ? `${(valueAtRisk / 1_000_000).toFixed(1)}M`
                            : `${(valueAtRisk / 1_000).toFixed(0)}K`}
                        </div>
                    ...
                    <ChevronRight
                      className="w-4 h-4 mt-2 ml-auto opacity-0 group-hover:opacity-100 transition-opacity"
              ...
              >
                ...
                <XCircle
                  ...
                  aria-label="Archive"
                  style={{ color: 'rgba(255,255,255,0.2)' }}
                  onClick={() => updateWorkflow.mutate({ id: w.id, status: 'archived' })}
                />
              </div>
            </div>
          );
