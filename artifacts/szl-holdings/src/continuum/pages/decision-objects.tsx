...
export default function DecisionObjects() {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        const r = await apiFetch<{ data: Decision[] } | Decision[]>(`/continuum/decisions?${params}`);
  ...
  const approveDecision = useStandardMutation({
    mutationFn: async (id: number) => {
      try {
        return await apiFetch(`/continuum/decisions/${id}/approve`, { method: 'POST' });
      } catch {
        return null;
      }
    },
  ...
  const rejectDecision = useStandardMutation({
    mutationFn: async (id: number) => {
      try {
        return await apiFetch(`/continuum/decisions/${id}/reject`, { method: 'POST' });
      } catch {
        return null;
      }
    },
