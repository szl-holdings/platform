...
const STEPS: OnboardingStep[] = [
  {
    ...
    fields: [
      ...
      { key: 'adminEmail', label: 'Admin Email', type: 'text', default: '' },
      {
        ...
        label: 'Plan',
        type: 'select',
        options: ['Design Partner (Pilot)', 'Enterprise Trial', 'Sandbox'],
      },
      {
        key: 'timezone',
        label: 'Timezone',
        type: 'select',
        options: ['America/New_York', 'America/Los_Angeles', 'Europe/London', 'Asia/Singapore'],
      },
    ],
  },
  {
    ...
    description:
      'Apply governance policies to your tenant. One-click templates from the policy library.',
    ...
    fields: [
      {
        ...
        type: 'select',
        options: ['SOC 2 Type II', 'HIPAA-adjacent', 'Financial Services', 'None — custom'],
      },
      ...
      { key: 'costBudget', label: 'Monthly Budget ($)', type: 'text', default: '500' },
      {
        ...
        label: 'Model Policy',
        type: 'select',
        options: ['Claude + GPT (recommended)', 'GPT only', 'Claude only', 'Permissive'],
      },
    ],
  },
  {
    ...
    description:
      'Populate your workspace with realistic sample data so you can explore immediately.',
      ...
      <div>
        ...
        >
          {field.options?.map((opt) => (
            <option key={opt} value={opt} style={{ background: '#0c1420' }}>
              {opt}
            </option>
          ))}
        </select>
...
export default function PilotOnboarding() {
  ...
  const getState = (id: number): StepState => stepStates[id] ?? { completed: false, values: {} };
    ...
    <div className="max-w-3xl mx-auto space-y-5 p-1">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>
            For accepted design partners. Provision your tenant, apply policies, seed data, and run
            your first workflow.
          </p>
        </div>
      ...
      >
        ...
        <span className="text-[11px]" style={{ color: 'rgba(245,158,11,0.8)' }}>
          <strong>Design Partner Program</strong> — Limited to 5 design partners. This wizard
          provisions a sandbox tenant configuration on the platform.
        </span>
      </div>
      ...
      {!onboardingComplete ? (
                      ...
                      {[
                        {
                          ...
                          done: runComplete || simulatingRun,
                          active: simulatingRun,
                        },
                        ...
                        <div key={i} className="flex items-center gap-2">
                          ...
                          <span
                            className="text-[11px]"
                            style={{
                              color: s.done ? 'rgba(255,255,255,0.7)' : 'rgba(255,255,255,0.3)',
                            }}
                          >
                            {s.label}
                          </span>
        ...
        >
          ...
          <p className="text-[12px] mb-4" style={{ color: 'rgba(255,255,255,0.5)' }}>
            Your pilot tenant is provisioned, policies applied, demo data seeded, and your first
