...
function SectionCard({
  ...
  return (
    <div
      ...
      style={{
        borderColor: urgent ? 'rgba(239,68,68,0.2)' : 'rgba(255,255,255,0.07)',
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {count !== undefined && count > 0 && (
            <span
              ...
              style={{
                background: urgent ? 'rgba(239,68,68,0.15)' : 'rgba(75,139,219,0.15)',
                color: urgent ? '#ef4444' : '#4B8BDB',
              }}
            >
              {count}
            </span>
        ...
        {onClick && (
          <button
            ...
            style={{ color: 'rgba(75,139,219,0.6)' }}
          >
...
export default function WorkspaceHome() {
  ...
  const { data: approvals } = useStandardQuery({
    ...
    queryFn: async () => {
      try {
        const r = await apiFetch<{ data: unknown[] }>('/continuum/approvals?status=pending&limit=5');
        return r?.data ?? [];
      } catch {
        return [];
      }
  ...
  const { data: runsData } = useStandardQuery({
    ...
    queryFn: async () => {
      try {
        ...
        }>('/continuum/runs?state=running&limit=5');
        return r?.data ?? [];
      } catch {
        return [];
      }
  ...
  const { data: signals } = useStandardQuery({
    ...
    queryFn: async () => {
      try {
        ...
        }>('/continuum/signals?severity=critical&limit=5');
        return r?.data ?? [];
      } catch {
        return [];
      }
  ...
  const { data: decisions } = useStandardQuery({
    ...
    queryFn: async () => {
      try {
        ...
        }>('/continuum/decisions?status=approval_required&limit=5');
        return r?.data ?? [];
      } catch {
        return [];
      }
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>
            Priority view — critical approvals, active agents, and unresolved signals surface first.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          value={signalCount}
          color={signalCount > 0 ? '#ef4444' : '#10b981'}
          icon={<Radio className="w-3.5 h-3.5" />}
          sub={signalCount > 0 ? 'Unresolved' : 'All clear'}
        />
        <StatCard
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <SectionCard
          ...
          onClick={() => navigate('/continuum/governance')}
        >
          {pendingApprovals.slice(0, 4).map((a: any) => (
            <div
              key={a.id}
              className="flex items-start gap-2.5 p-2.5 rounded-lg border transition-all hover:border-amber-500/20 cursor-pointer"
              style={{
            ...
            >
              <div
                ...
                style={{ background: a.urgent ? '#ef4444' : '#f59e0b' }}
              />
              <div className="flex-1 min-w-0">
                <div className="text-[11px] font-medium text-white truncate">
                  {a.label ?? `Approval #${a.id}`}
                </div>
        ...
        <SectionCard
          ...
          count={runCount}
          onClick={() => navigate('/continuum/runs')}
        >
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  <div className="text-[11px] font-medium text-white truncate flex-1">
                    {r.name ?? `Run #${r.id}`}
                  </div>
                  <span
                ...
                {r.progress !== undefined && (
                  ...
                  >
                    <div
                      ...
                      style={{ width: `${r.progress}%`, background: cfg.color }}
                    />
                  </div>
                )}
                <div
        ...
        <SectionCard
          ...
          onClick={() => navigate('/continuum/signals')}
        >
          {criticalSignals.slice(0, 4).map((s: any) => {
            ...
            return (
              <div
                key={s.id}
                className="flex items-start gap-2.5 p-2.5 rounded-lg border cursor-pointer transition-all hover:border-red-500/20"
                style={{
                  borderColor: 'rgba(255,255,255,0.05)',
                  background: 'rgba(255,255,255,0.01)',
                }}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
                    <span
                      ...
                      style={{ color: srcColor, background: `${srcColor}15` }}
                    >
                      {s.source}
                    </span>
                    <span
                      className="text-[9px] uppercase tracking-widest px-1.5 py-0.5 rounded border font-bold"
                      style={{
                        color: sevColor,
        ...
        <SectionCard
          ...
          onClick={() => navigate('/continuum/decisions')}
        >
          {pendingDecisions.slice(0, 4).map((d: any) => {
            const dStatus =
              DECISION_STATUS[d.approvalStatus ?? d.status] ?? DECISION_STATUS.propose_only;
            const conf = d.confidence ?? 78;
            return (
              <div
                key={d.id}
                className="p-2.5 rounded-lg border cursor-pointer transition-all hover:border-violet-500/20"
                style={{
                  borderColor: 'rgba(255,255,255,0.05)',
                  background: 'rgba(255,255,255,0.01)',
                }}
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  <div className="text-[11px] font-medium text-white line-clamp-2 flex-1">
                    {d.title}
                  </div>
                  <span
                    className="text-[9px] uppercase tracking-widest px-1.5 py-0.5 rounded border shrink-0 font-bold"
                    style={{
                      color: dStatus.color,
                      borderColor: `${dStatus.color}30`,
                      background: `${dStatus.color}10`,
                    }}
                  >
                    {dStatus.label}
                  </span>
      ...
      <SectionCard
        ...
        onClick={() => navigate('/continuum/documents')}
      >
          ...
          {recentArtifacts.length === 0 ? (
            ...
            recentArtifacts.map((a) => (
              ...
              >
                ...
                >
                  <span
                    className={`px-1.5 py-0.5 rounded ${a.status === 'published' ? 'text-emerald-400 bg-emerald-400/10' : 'text-amber-400 bg-amber-400/10'}`}
                  >
                    {a.status === 'published' ? 'Published' : 'Pending Review'}
                  </span>
      ...
      <div
        className="rounded-xl border p-4"
        style={{ borderColor: 'rgba(75,139,219,0.1)', background: 'rgba(75,139,219,0.03)' }}
      >
        <div
          ...
          {[
            {
              ...
              nav: '/continuum/operator',
              badge: 'New',
            },
            {
              label: 'Decision Objects',
              icon: <Brain className="w-3 h-3" />,
              nav: '/continuum/decisions',
              badge: 'New',
            },
            {
              label: 'Skill Registry',
              icon: <Layers className="w-3 h-3" />,
              nav: '/continuum/skills',
              badge: 'New',
            },
            {
              label: 'Workflow Orchestration',
              icon: <GitBranch className="w-3 h-3" />,
              nav: '/continuum/workflows',
            },
            { label: 'Signal Feed', icon: <Radio className="w-3 h-3" />, nav: '/continuum/signals' },
            {
              label: 'Governance',
              icon: <CheckCircle className="w-3 h-3" />,
              nav: '/continuum/governance',
            },
          ].map((link) => (
            <button
              key={link.nav}
