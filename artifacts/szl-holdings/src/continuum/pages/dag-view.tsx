...
import { Activity, AlertTriangle, CheckCircle, ChevronLeft, Clock, XCircle } from 'lucide-react';
...
function useWorkflow(id: number) {
  return useStandardQuery({
    ...
    queryFn: async () => {
      const resp = await apiFetch<WorkflowDetail | { data: WorkflowDetail }>(
        `/continuum/workflows/${id}`,
      );
  ...
  return (
    <div className="space-y-3">
      <div
        ...
        style={{ borderColor: 'rgba(255,255,255,0.08)', background: 'rgba(4,8,16,0.9)' }}
      >
        <svg width={svgW} height={svgH} style={{ minWidth: '100%', display: 'block' }}>
          <defs>
            ...
            return (
              ...
              >
                <rect
                  ...
                  fill={cfg.fill}
                  stroke={
                    isSelected ? cfg.stroke : isHovered ? cfg.stroke : 'rgba(255,255,255,0.1)'
                  }
                ...
                <foreignObject x={8} y={8} width={NODE_W - 16} height={NODE_H - 16}>
                  ...
                  >
                    <div
                      style={{ display: 'flex', alignItems: 'center', gap: '5px', color: cfg.text }}
                    >
                      <span
                        style={{ width: 14, height: 14, display: 'flex', alignItems: 'center' }}
                      >
                        {STATUS_ICONS[status]}
                      </span>
                      <span
                    ...
                    <div
                      style={{ display: 'flex', gap: '4px', alignItems: 'center', paddingLeft: 19 }}
                    >
                      <span
                        style={{
                          fontSize: 9,
      ...
      {selectedStep && (
        ...
        >
          ...
          <div className="text-[10px] space-y-1" style={{ color: 'rgba(255,255,255,0.4)' }}>
            <div>
              <span style={{ color: 'rgba(255,255,255,0.25)' }}>Step ID:</span>{' '}
              <span className="font-mono">{selectedStep.id}</span>
            </div>
            {selectedStep.deps.length > 0 && (
              <div>
                <span style={{ color: 'rgba(255,255,255,0.25)' }}>Dependencies:</span>{' '}
                {selectedStep.deps.join(', ')}
              </div>
            )}
            {selectedStep.durationMs && (
              <div>
                <span style={{ color: 'rgba(255,255,255,0.25)' }}>Duration:</span>{' '}
