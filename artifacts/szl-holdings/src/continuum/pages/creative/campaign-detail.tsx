...
import { CampaignBilling } from '@/continuum/components/creative/billing/campaign-billing';
import { CreativePreview } from '@/continuum/components/creative/preview/creative-preview';
import { ScriptEditor } from '@/continuum/components/creative/scripts/script-editor';
import { StoryboardBoard } from '@/continuum/components/creative/storyboards/storyboard-board';
import { AssetVault } from '@/continuum/components/creative/vault/asset-vault';
import { VoiceoverManager } from '@/continuum/components/creative/voice/voiceover-manager';
import { useCampaign } from '@/continuum/hooks/use-campaigns';
...
export function CampaignDetail() {
  const [, params] = useRoute('/continuum/creative/campaigns/:id');
  const campaignId = params?.id || '';
  const { data: campaign, isLoading } = useCampaign(campaignId);

  const [activeTab, setActiveTab] = React.useState<
    'scripts' | 'storyboards' | 'voice' | 'vault' | 'preview' | 'billing' | 'discussion'
  >('scripts');
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3 mt-2 flex-wrap">
              ...
              <span
                className={`text-xs px-2 py-0.5 rounded border capitalize ${statusColors[campaign.status] || ''}`}
              >
      ...
      <div className="flex-1 overflow-hidden relative">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeTab === 'billing' && (
              <CampaignBilling campaignId={campaignId} campaignName={campaign.name} />
            )}
