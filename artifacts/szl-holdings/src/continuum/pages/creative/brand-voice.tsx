...
import { CheckCircle, Copy, Loader2, Mic, RefreshCw, Sparkles, Star } from 'lucide-react';
...
const brandVoices: VoiceProfile[] = [
  {
    ...
    systemPrompt:
      'You are a brand voice specialist. Rewrite the following content in a confident, authoritative, data-driven tone suited for C-suite and enterprise decision makers. Be concise, factual, and trust-building. Output only the rewritten text, no explanations.',
  ...
  },
  {
    ...
    systemPrompt:
      'You are a brand voice specialist. Rewrite the following content in a warm, approachable, human-centered tone. Make it feel genuine and friendly, like a knowledgeable colleague speaking directly to the reader. Output only the rewritten text, no explanations.',
  ...
  },
  {
    ...
    systemPrompt:
      'You are a brand voice specialist. Rewrite the following content in a bold, disruptive, high-energy tone. Be direct, provocative, and action-oriented — like a challenger brand that does not accept the status quo. Output only the rewritten text, no explanations.',
  ...
  },
  {
    ...
    systemPrompt:
      'You are a brand voice specialist. Rewrite the following content with technical precision and expert-level detail. Use accurate terminology, provide specific technical depth, and write for an audience of engineers and technical architects. Output only the rewritten text, no explanations.',
    color: 'text-purple-400 bg-purple-500/10 border-purple-500/20',
  },
];

const DEFAULT_INPUT =
  'Our new feature helps teams work better together by sharing information more easily across departments.';
...
async function transformVoice(input: string, systemPrompt: string): Promise<string> {
  ...
  const content =
    ((res as Record<string, unknown>)?.content as string) ??
    (res as { data?: { content?: string } })?.data?.content;
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">
          Governed brand voice calibration — consistent tone across all generated content
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {brandVoices.map((voice) => (
          ...
          >
            <div
              className={`text-xs font-semibold mb-1.5 px-2 py-0.5 rounded-full inline-block border ${voice.color}`}
            >
              {voice.name}
            </div>
            ...
            <div className="flex flex-wrap gap-1 mt-2">
              {voice.traits.map((t) => (
                <span key={t} className="text-[10px] bg-white/5 px-1 py-0.5 rounded text-slate-500">
                  {t}
                </span>
              ))}
            </div>
            ...
            <div className="h-40 bg-white/3 rounded-lg border border-white/8 p-3 text-sm text-slate-200 overflow-y-auto">
              {loading ? (
                <div className="flex items-center gap-2 text-slate-500 h-full">
                  ...
                  {currentVoice.name.toLowerCase()} voice…
                </div>
              ) : currentOutput ? (
                currentOutput
              ) : (
                <span className="text-slate-600">
                  Click "Transform Voice" to generate content in the {currentVoice.name} style.
                </span>
              )}
            </div>
            {currentOutput && (
              <div className="flex items-center gap-2 mt-3">
                <div className="flex">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <Star
                      key={s}
                      className={`w-4 h-4 ${s <= 4 ? 'text-amber-400' : 'text-slate-700'}`}
                    />
                  ))}
                </div>
        ...
        <div className="p-4">
          <div className="grid grid-cols-3 lg:grid-cols-6 gap-4">
            {[
              ...
              { label: 'Brand Alignment', value: 91 },
              { label: 'Clarity', value: 89 },
              { label: 'Engagement', value: 82 },
              { label: 'SEO Fit', value: 78 },
            ].map(({ label, value }) => (
