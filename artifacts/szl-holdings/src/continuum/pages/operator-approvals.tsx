...
function ApprovalDetail({ approval }: { approval: ApprovalRequest }) {
  const { data: trail } = useStandardQuery({
    ...
    queryFn: async () => {
      try {
        const r = await apiRequest<ApiEnvelope<AuditEntry[]> | AuditEntry[]>(
          'GET',
          `/api/approvals/${approval.id}/audit-trail`,
        );
        return unwrap(r);
      } catch {
    ...
    >
      {payload.reason && (
        <div
          ...
          style={{ background: 'rgba(75,139,219,0.04)', border: '1px solid rgba(75,139,219,0.1)' }}
        >
          <div
          ...
          <div className="space-y-1">
            {trail.map((t) => (
              ...
              >
                ...
                <span
                  className="font-mono uppercase tracking-wider text-[9px]"
                  style={{ color: '#4B8BDB' }}
                >
                  {t.action}
                </span>
                ...
                {t.note && (
                  <span className="text-[9px] italic" style={{ color: 'rgba(255,255,255,0.45)' }}>
                    "{t.note}"
                  </span>
                )}
                <span
...
function MatchedRuleCard({ rule }: { rule: MatchedRuleDetails }) {
  return (
    ...
    >
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <span
          ...
          style={{
            ...
            border: '1px solid rgba(75,139,219,0.18)',
          }}
      ...
      <div
        className="flex items-center gap-2 flex-wrap text-[9px]"
        style={{ color: 'rgba(255,255,255,0.4)' }}
      >
        <span>priority {rule.priority}</span>
        {rule.owner && <span>· owner {rule.owner}</span>}
        {!rule.enabled && <span style={{ color: '#ef4444' }}>· disabled</span>}
        {rule.tags.length > 0 && (
    ...
    >
      ...
      >
        ...
        {tier.redactPII && (
          <span
            className="px-1.5 py-0.5 rounded font-mono"
            style={{ background: 'rgba(75,139,219,0.08)', color: '#4B8BDB' }}
          >
            PII redacted
          </span>
        )}
        {!tier.allowExternalComms && (
          <span
            className="px-1.5 py-0.5 rounded font-mono"
            style={{ background: 'rgba(139,122,200,0.08)', color: '#8b7ac8' }}
          >
            no external comms
          </span>
        )}
...
function ApprovalRow({ approval }: { approval: ApprovalRequest }) {
  ...
  const reviewMutation = useStandardMutation({
    mutationFn: async (decision: 'approved' | 'rejected' | 'revised') => {
      return apiRequest('POST', `/api/approvals/${approval.id}/review`, {
        decision,
  ...
  const escalateMutation = useStandardMutation({
    mutationFn: async () => {
      return apiRequest('POST', `/api/approvals/${approval.id}/escalate`, {
            ...
            <div className="text-[10px]" style={{ color: 'rgba(255,255,255,0.4)' }}>
              ...
              {approval.requiredApproverRole && (
                <>
                  {' '}
                  · approver:{' '}
                  <span style={{ color: 'rgba(75,139,219,0.85)' }}>
                    {approval.requiredApproverRole}
                  </span>
                </>
              )}
...
export default function OperatorApprovalsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5 p-4">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span
              ...
              style={{ color: '#4B8BDB' }}
            >
              Counsel · Operator Approval Queue
            </span>
          </div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.4)' }}>
            ...
            Counsel run ledger and unblock the agent.
          </p>
        </div>
      ...
      {escalated.length > 0 && (
        <Section
          icon={<AlertTriangle className="w-3.5 h-3.5" style={{ color: '#8b7ac8' }} />}
          label="Escalated"
        >
          {escalated.map((a) => (
            <ApprovalRow key={a.id} approval={a} />
      ...
      {resolved.length > 0 && filter === 'all' && (
        <Section
          icon={<CheckCircle className="w-3.5 h-3.5" style={{ color: '#10b981' }} />}
          label="Resolved"
        >
          {resolved.map((a) => (
            <ApprovalRow key={a.id} approval={a} />
