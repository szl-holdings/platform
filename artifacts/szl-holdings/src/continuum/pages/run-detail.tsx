...
function useRunDetail(id: number) {
  return useStandardQuery({
    ...
    queryFn: async () => {
      const resp = await apiFetch<RunDetailData | { data: RunDetailData }>(
        `/continuum/runs/${id}/steps`,
      );
...
function GanttBar({ steps, totalMs }: { steps: RunStep[]; totalMs: number | null }) {
  ...
  const baseMs = steps[0]?.startedAt
    ? new Date(steps[0].startedAt).getTime()
    : Date.now() - (totalMs ?? 10000);
      ...
      <div className="space-y-2">
        {steps.map((step) => (
          <div key={step.id} className="flex items-center gap-3">
            <div
              className="w-32 text-[10px] text-right truncate shrink-0"
              style={{ color: 'rgba(255,255,255,0.4)' }}
            >
              {step.label}
            </div>
            <div
              className="flex-1 h-6 rounded-md flex items-center px-2"
              style={{
      ...
      {steps.map((step) => {
        ...
        return (
          <div key={step.id} className="flex items-center gap-3 group">
            <div
              className="w-32 text-[10px] text-right truncate shrink-0 transition-colors"
              style={{ color: 'rgba(255,255,255,0.4)' }}
            >
              {step.label}
            </div>
            <div
              className="flex-1 h-7 relative rounded overflow-hidden"
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span
          className="text-[10px] uppercase tracking-widest px-2 py-0.5 rounded border font-semibold"
      ...
      {step.logs && step.logs.length > 0 && (
        <div>
          ...
          <div
            className="rounded-lg p-3 font-mono text-[10px] space-y-0.5"
            style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.06)' }}
          >
            {step.logs.map((log, i) => (
              <div
                key={i}
                style={{ color: log.includes('ERROR') ? '#ef4444' : 'rgba(255,255,255,0.5)' }}
              >
                {log}
              </div>
            ))}
        ...
        <div className="grid grid-cols-2 gap-3">
          {step.input && (
            <div>
              ...
              <div
                className="rounded-lg p-2 font-mono text-[9px] overflow-auto max-h-20"
                style={{
          ...
          <>
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 flex-wrap">
                    ...
                    <div>
                      ...
                      <div
                        className="text-xs"
                        style={{ color: run.retryCount > 0 ? '#f59e0b' : 'rgba(255,255,255,0.6)' }}
                      >
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                  <span
                    className="text-[10px] uppercase tracking-widest px-2 py-1 rounded-lg border font-bold"
                    style={{
                      color: cfg?.color,
            ...
            {stateHistory.length > 0 && (
              <div
                ...
                style={{ borderColor: 'rgba(255,255,255,0.08)', background: 'rgba(12,18,30,0.95)' }}
              >
                <div
                  className="text-[10px] uppercase tracking-widest font-semibold"
                  style={{ color: 'rgba(255,255,255,0.4)' }}
            ...
            >
              ...
              <div
                className="mb-3 flex items-center justify-between text-[9px]"
            ...
            {run.input && (
              ...
              >
                ...
                <div
                  className="rounded-lg p-3 font-mono text-[10px] overflow-auto max-h-32"
                  style={{
