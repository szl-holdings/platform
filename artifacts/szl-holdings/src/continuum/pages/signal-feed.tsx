...
const FALLBACK_SIGNALS: Signal[] = [
  ...
  },
  {
    ...
    severity: 'critical',
    title: 'Critical CVE-2025-1234 detected in API gateway — CVSS 9.1',
    body: 'Vulnerability detected in production infrastructure. Patch available.',
    status: 'processing',
    normalizedScore: '95.0',
    metadata: { correlationId: 'corr-a001', region: 'us-east-1' },
  ...
  },
  {
    ...
    sourceType: 'monitoring',
    severity: 'high',
    title: 'Dark vessel detected: MMSI 123456789 — 18h AIS gap in position feed',
    body: 'Vessel went dark in high-risk corridor. Sanctions screening initiated.',
    status: 'processing',
    normalizedScore: '78.5',
    metadata: { correlationId: 'corr-v001', region: 'eu-west-1' },
    receivedAt: new Date(Date.now() - 480000).toISOString(),
    processedAt: null,
  },
  {
    id: 4,
    ...
    sourceType: 'webhook',
    severity: 'high',
    title: 'Incident escalation: INC-7821 unresolved after 2h SLA breach',
    body: 'P1 incident exceeded SLA target. Escalation to VP Engineering triggered.',
    status: 'new',
    normalizedScore: '85.1',
    metadata: { correlationId: 'corr-l001', region: 'us-east-1' },
  ...
  },
  {
    ...
    sourceType: 'api',
    severity: 'high',
    title: 'Pipeline health degraded: ETL latency +340% above baseline',
    body: 'Daily ETL pipeline experiencing significant slowdown. Upstream provider rate limiting.',
    status: 'new',
    normalizedScore: '72.0',
    metadata: { correlationId: 'corr-al001', region: 'us-east-1' },
  ...
  },
  {
    ...
    sourceType: 'connector',
    severity: 'high',
    title: 'Tax lien escalation: 89-12 Jamaica Ave, Queens — 127 days delinquent',
    body: 'Property distress signal escalating. Opportunity score: 78.',
    status: 'processed',
    normalizedScore: '78.0',
    metadata: { correlationId: 'corr-t002', region: 'us-east-1' },
    receivedAt: new Date(Date.now() - 1200000).toISOString(),
    processedAt: new Date(Date.now() - 1100000).toISOString(),
  },
  {
    id: 7,
    source: 'aegis',
    sourceType: 'monitoring',
    severity: 'medium',
    title: 'Configuration drift detected in production secrets rotation policy',
    body: 'Secret rotation overdue by 14 days. Compliance gap flagged.',
    status: 'processed',
    normalizedScore: '55.2',
    metadata: { correlationId: 'corr-a002', region: 'us-east-1' },
  ...
  },
  {
    ...
    sourceType: 'monitoring',
    severity: 'medium',
    title: 'Speed anomaly: MSC Medusa exceeding 24kts in restricted zone',
    body: 'Vessel speed violation detected. Regulatory notification may be required.',
    status: 'new',
    normalizedScore: '61.4',
    metadata: { correlationId: 'corr-v002', region: 'eu-west-1' },
  ...
  },
  {
    ...
    sourceType: 'webhook',
    severity: 'medium',
    title: 'Alert storm: 847 low-severity events correlated to single root cause',
    body: 'Noise reduction applied. Root cause: upstream DNS resolution failure.',
    status: 'processed',
    normalizedScore: '48.7',
    metadata: { correlationId: 'corr-l002', region: 'us-east-1' },
    receivedAt: new Date(Date.now() - 3600000).toISOString(),
    processedAt: new Date(Date.now() - 3400000).toISOString(),
  },
  {
    id: 10,
    source: 'continuum',
    sourceType: 'api',
    severity: 'medium',
    title: 'Schema drift detected: terra_properties column type changed',
    body: 'Upstream schema change detected. Validation pipeline needs adjustment.',
    status: 'new',
    normalizedScore: '52.3',
    metadata: { correlationId: 'corr-al002', region: 'us-east-1' },
  ...
  },
  {
    ...
    sourceType: 'connector',
    severity: 'low',
    title: 'High-opportunity lead: 412 Fulton St, Brooklyn — motivated seller confirmed',
    body: 'Property opportunity score: 82. Priority lead for broker team.',
    status: 'processed',
    normalizedScore: '82.0',
    metadata: { correlationId: 'corr-t003', region: 'us-east-1' },
  ...
  },
  {
    ...
    sourceType: 'api',
    severity: 'info',
    title: 'Connector timeout: Salesforce API rate limit hit — backoff in progress',
    body: 'Automatic retry initiated. ETA: 60s.',
    status: 'processed',
    normalizedScore: '15.0',
    metadata: { correlationId: 'corr-al003', region: 'us-west-2' },
    receivedAt: new Date(Date.now() - 10800000).toISOString(),
    processedAt: new Date(Date.now() - 10700000).toISOString(),
  },
];
  ...
  return useStandardQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const resp = await apiFetch<SignalResp>(`/continuum/signals?${params}`);
...
const SOURCE_CONFIG: Record<string, { color: string; label: string }> = {
  ...
  continuum: { color: '#4B8BDB', label: 'Counsel' },
};
...
function SignalRow({ signal }: { signal: Signal }) {
  ...
  const stsCfg = STATUS_CONFIG[signal.status] ?? {
      ...
      <div className="flex items-start gap-3 p-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5 flex-wrap">
            ...
            <span className="text-[9px] uppercase tracking-widest" style={{ color: stsCfg.color }}>
              {stsCfg.label}
            </span>
            <span className="text-[9px] ml-auto" style={{ color: 'rgba(255,255,255,0.25)' }}>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              <div className="rounded-lg p-2" style={{ background: 'rgba(0,0,0,0.3)' }}>
                ...
                <div className="text-[10px] text-white">
                  {signal.normalizedScore ? parseFloat(signal.normalizedScore).toFixed(1) : '—'}
                </div>
              </div>
              <div className="rounded-lg p-2" style={{ background: 'rgba(0,0,0,0.3)' }}>
                <div
                ...
                <div className="text-[10px] text-white">
                  {new Date(signal.receivedAt).toLocaleString()}
                </div>
              </div>
              <div className="rounded-lg p-2" style={{ background: 'rgba(0,0,0,0.3)' }}>
                <div
                ...
                <div className="text-[10px] text-white">
                  {signal.processedAt ? new Date(signal.processedAt).toLocaleString() : 'Not yet'}
                </div>
              </div>
            </div>
          )}
        ...
        <ChevronRight
          className="w-3.5 h-3.5 shrink-0 transition-transform mt-0.5"
          style={{
...
export default function SignalFeed() {
  ...
  const SOURCES = ['terra', 'aegis', 'vessels', 'lyte', 'continuum'];
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-xs" style={{ color: 'rgba(255,255,255,0.35)' }}>
              Live feed of cross-platform intelligence signals — Terra, Aegis, Vessels, Lyte, Continuum.
            </p>
          </div>
          ...
          {isLoading ? (
            Array.from({ length: 8 }).map((_, i) => (
              <div
                ...
                style={{ background: 'rgba(12,18,30,0.6)' }}
              />
            ))
          ) : signals.length === 0 ? (
            <EmptyState
