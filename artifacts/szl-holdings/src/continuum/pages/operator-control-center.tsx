...
> = {
  running: {
    color: '#4B8BDB',
    bg: 'rgba(75,139,219,0.08)',
    border: 'rgba(75,139,219,0.2)',
    label: 'Running',
...
function StatCard({
  ...
  return (
    <div
      ...
      style={{
        borderColor: alert ? 'rgba(239,68,68,0.2)' : 'rgba(255,255,255,0.07)',
        background: alert ? 'rgba(239,68,68,0.02)' : 'rgba(12,18,30,0.95)',
      }}
    >
...
function AgentHealthRow({
  ...
  return (
    <div
      className="flex items-center gap-3 px-4 py-3 border-b last:border-b-0 transition-colors hover:bg-white/2"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold" style={{ color: hasAlert ? '#ef4444' : '#fff' }}>
            {agent.name}
          </span>
          {agent.violations > 0 && (
            <span
...
export default function OperatorControlCenter() {
  ...
  const { data: agentStats } = useStandardQuery({
    ...
    queryFn: async () => {
      try {
        return await apiFetch('/agent-os/agent-stats');
      } catch {
        return null;
      }
    },
  ...
  const { data: factoryFloor } = useStandardQuery({
    ...
    queryFn: async () => {
      try {
        return await apiFetch<{ globalCounts: unknown[] }>('/continuum/factory-floor');
      } catch {
        return null;
      }
    },
  ...
  const { data: violationsData } = useStandardQuery({
    ...
    queryFn: async (): Promise<{ violations: any[] } | null> => {
      try {
        return await apiFetch<{ violations: any[] }>('/continuum/policy-violations');
      } catch {
        return null;
      }
    },
  ...
  const { data: bottlenecksData } = useStandardQuery({
    ...
    queryFn: async (): Promise<{ bottlenecks: any[] } | null> => {
      try {
        return await apiFetch<{ bottlenecks: any[] }>('/continuum/approval-bottlenecks');
      } catch {
        return null;
      }
    },
  ...
  const { data: providersData } = useStandardQuery({
    ...
    queryFn: async (): Promise<{ providers: any[] } | null> => {
      try {
        return await apiFetch<{ providers: any[] }>('/continuum/provider-health');
      } catch {
        return null;
      }
    },
  ...
  const queueData = (factoryFloor as any)?.globalCounts ?? {
    running: 0,
    queued: 0,
    waitingApproval: 0,
    failed: 0,
    ...
    <div className="max-w-6xl mx-auto space-y-5">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span
              ...
              style={{ color: '#4B8BDB' }}
            >
              Counsel · Operator Control
            </span>
          </div>
          <h1 className="text-xl font-bold text-white tracking-tight">Operator Control Center</h1>
          <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>
            Real-time agent health, execution queue, policy violations, and provider status.
          </p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard
          label="Failed Agents"
          value={failedAgents}
          color={failedAgents > 0 ? '#ef4444' : '#10b981'}
          icon={<XCircle className="w-3.5 h-3.5" />}
          sub={failedAgents > 0 ? 'Needs attention' : 'All healthy'}
          alert={failedAgents > 0}
        />
        <StatCard
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          <div
            className="rounded-xl border overflow-hidden"
            style={{ borderColor: 'rgba(239,68,68,0.15)', background: 'rgba(12,18,30,0.95)' }}
          >
            <div
            ...
            <div className="p-3 space-y-2">
              {violations.map((v) => (
                <div
                  ...
                  style={{
                    borderColor:
                      v.severity === 'critical' ? 'rgba(239,68,68,0.25)' : 'rgba(245,158,11,0.2)',
                    background:
                      v.severity === 'critical' ? 'rgba(239,68,68,0.04)' : 'rgba(245,158,11,0.04)',
                  }}
                >
          ...
          <div
            className="rounded-xl border overflow-hidden"
            style={{ borderColor: 'rgba(245,158,11,0.1)', background: 'rgba(12,18,30,0.95)' }}
          >
            <div
              className="flex items-center gap-2 px-4 py-3 border-b"
              style={{ borderColor: 'rgba(255,255,255,0.05)' }}
              ...
              {bottlenecks.map((b) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      borderColor: urgent ? 'rgba(239,68,68,0.2)' : 'rgba(255,255,255,0.05)',
            ...
            {providers.map((p) => {
              ...
              return (
                <div
                  ...
                  style={{
                    borderColor:
                      p.status === 'degraded' ? 'rgba(245,158,11,0.2)' : 'rgba(255,255,255,0.05)',
                    background: 'rgba(255,255,255,0.01)',
                  }}
                >
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span
                      ...
                      style={{ color: pCfg.color, background: `${pCfg.color}15` }}
                    >
                      {pCfg.label}
                    </span>
                  </div>
                  ...
                  >
                    ...
                    <span>
                      ...
                      <span
                        ...
                        style={{
                          color:
                            p.errorRate < 1 ? '#10b981' : p.errorRate < 5 ? '#f59e0b' : '#ef4444',
                        }}
                      >
                        {p.errorRate}%
                      </span>
                    ...
                    <span>
                      Cost: <span className="font-mono">${p.cost24h}/24h</span>
                    </span>
                  </div>
                </div>
              );
          ...
          <div className="p-4 space-y-3">
            {[
              ...
              { label: 'Queued', value: queueData.queued ?? 8, color: '#f59e0b' },
              {
                ...
                value: queueData.waitingApproval ?? 3,
                color: '#8b5cf6',
              },
              ...
              <div key={item.label} className="flex items-center gap-3">
                ...
                >
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, ((item.value ?? 0) / (queueData.totalToday ?? 247)) * 100)}%`,
