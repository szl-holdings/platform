  ...
  return useStandardQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const resp = await apiFetch<RunsResp | WorkflowRun[]>(`/continuum/runs?${params}`);
        if (
          resp &&
          typeof resp === 'object' &&
          'data' in resp &&
...
function useRetryRun() {
  ...
  return useStandardMutation({
    mutationFn: async (id: number) => apiFetch(`/continuum/runs/${id}/retry`, { method: 'POST' }),
...
function useCancelRun() {
  const qc = useQueryClient();
  return useStandardMutation({
    mutationFn: async (id: number) => apiFetch(`/continuum/runs/${id}/cancel`, { method: 'POST' }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['alloyRuns'] }),
  });
}
...
export default function ExecutionHistory() {
  ...
  function exportCSV() {
    ...
    a.download = `continuum-runs-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  }
          ...
          {isLoading ? (
            ...
            runs.map((run) => (
              <RunRow
                ...
                onView={(id) => navigate(`/continuum/runs/${id}`)}
              />
            ))
          )}
        </div>
