...
function ProvisioningPanel() {
  ...
  const categories = data
    ? ['all', ...Array.from(new Set(data.adapters.map((a) => a.category))).sort()]
    : ['all'];
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            All registered service adapters — configure credentials to activate live data feeds.
          </p>
        </div>
        <button
          onClick={() => refetch()}
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-border bg-muted/30 hover:bg-muted/60 text-sm text-muted-foreground transition-colors"
        >
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select
              ...
              className="px-3 py-1.5 rounded-lg border border-border bg-background text-sm text-foreground focus:outline-none"
            >
              {categories.map((c) => (
                                  ...
                                  {adapter.requiredEnvVars.map((v) => {
                                    ...
                                    return (
                                      <span
                                        key={v}
                                        className={cn(
                                          'flex items-center gap-1.5 px-2.5 py-1 rounded-lg font-mono text-xs border',
                                          missing
                              ...
                              <div className="flex flex-wrap gap-2">
                                ...
                                {adapter.docsUrl && (
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-muted/40 border border-border text-muted-foreground text-xs hover:bg-muted/60 transition-colors"
                                  >
