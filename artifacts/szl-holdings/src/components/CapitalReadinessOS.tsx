...
import {
  ...
  Loader2, Plus, ArrowRight, Building2,
} from "lucide-react";
...
interface LenderPacket {
  id: number; title: string; lenderType: string; status: string;
  completionPct: number; targetSubmitDate?: string; notes?: string;
  deliverables?: Deliverable[];
}
interface InvestorPacket {
  id: number; title: string; investorType: string; status: string;
  completionPct: number; targetCloseDate?: string; notes?: string;
  deliverables?: Deliverable[];
}
interface Deliverable {
  id: number; deliverableKey: string; title: string; description?: string;
  status: string; version: number; content?: string;
}
interface DiligenceChecklist {
  id: number; title: string; checklistType: string; completionPct: number;
  status: string; items?: DiligenceItem[];
}
interface DiligenceItem {
  id: number; title: string; category?: string; isRequired: boolean;
  status: string; artifactUrl?: string; notes?: string;
}
interface CapDashboard {
  bankReadiness: number; angelReadiness: number;
  lenderPacketCount: number; investorPacketCount: number;
  activeLenderPacket?: LenderPacket; activeInvestorPacket?: InvestorPacket;
  milestonesTotal: number; milestonesCompleted: number; milestonesInProgress: number;
  financialModelCount: number; checklistCount: number;
}
...
function DeliverableRow({ item, endpoint, qk }: { item: Deliverable; endpoint: string; qk: (string | number)[] }) {
  ...
  const statusColors: Record<string, string> = {
    not_started: "#6b7280", drafting: "#f59e0b", draft_complete: "#3b82f6",
  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden">
      ...
      >
        ...
        <span className="text-xs font-mono px-2 py-0.5 rounded shrink-0" style={{ background: `${color}18`, color }}>
            ...
            <div className="px-4 pb-4 space-y-3 bg-muted/10 border-t border-border">
              ...
              <div>
                ...
                <textarea
                  ...
                  onBlur={() => { if (notes !== item.content) mut.mutate({ content: notes }); }}
                  rows={3}
...
function PacketBuilder({
  packet, endpoint, deliverableEndpoint, queryKey,
}: {
  ...
  const statusColors: Record<string, string> = {
    drafting: "#f59e0b", ready_for_review: "#3b82f6", submitted: "#6366f1",
    in_diligence: "#8b5cf6", approved: "#10b981", in_outreach: "#06b6d4",
    in_progress: "#06b6d4", closed: "#10b981", declined: "#ef4444", archived: "#6b7280",
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="flex items-center gap-2 mt-0.5">
            <span className="text-xs font-mono px-1.5 py-0.5 rounded" style={{ background: `${color}18`, color }}>
      ...
      {packet.notes && (
        <p className="text-xs text-muted-foreground bg-muted/30 border border-border rounded-lg px-3 py-2">{packet.notes}</p>
      )}
...
function ChecklistPanel({ checklist }: { checklist: DiligenceChecklist }) {
  ...
  const statusIcon = (s: string) => {
    ...
    if (s === "waived" || s === "na") return <Circle className="w-4 h-4 text-muted-foreground shrink-0" />;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">{checklist.completionPct}% complete</p>
        </div>
        <span className="text-xs font-mono px-2 py-0.5 rounded bg-primary/8 text-primary">{checklist.checklistType}</span>
      </div>
        ...
        <div className="space-y-4">
          {categories.map(cat => (
            <div key={cat}>
              ...
              <div className="space-y-1.5">
                {(detail?.items ?? []).filter(i => (i.category ?? "Other") === cat).map(item => (
                  <div key={item.id} className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-card border border-border">
                    <button onClick={() => {
                      const next = item.status === "not_started" ? "in_progress" : item.status === "in_progress" ? "complete" : "not_started";
                      mut.mutate({ id: item.id, status: next });
                    }}>
                      {statusIcon(item.status)}
                    </button>
                    <div className="flex-1 min-w-0">
                      <p className={cn("text-xs font-medium", item.status === "complete" ? "line-through text-muted-foreground" : "text-foreground")}>
                        {item.title}
                      </p>
                    </div>
                    {item.isRequired && (
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">Bank-ready, angel-ready, and investor-ready structure. Internal only — no legal or financial conclusions.</p>
      </div>
            ...
            <>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-card border border-border rounded-xl p-4 space-y-3">
                  <ReadinessMeter value={dashboard?.bankReadiness ?? 0} label="Bank / SBA Readiness" color="#3b82f6" />
                  <ReadinessMeter value={dashboard?.angelReadiness ?? 0} label="Angel / Investor Readiness" color="#10b981" />
                </div>
                <div className="bg-card border border-border rounded-xl p-4 grid grid-cols-2 gap-3">
                  {[
                    { label: "Lender Packets", value: dashboard?.lenderPacketCount ?? 0, color: "#3b82f6" },
                    { label: "Investor Packets", value: dashboard?.investorPacketCount ?? 0, color: "#10b981" },
                    { label: "Milestones", value: `${dashboard?.milestonesCompleted ?? 0}/${dashboard?.milestonesTotal ?? 0}`, color: "#f59e0b" },
                    { label: "Checklists", value: dashboard?.checklistCount ?? 0, color: "#8b5cf6" },
                  ].map(m => (
              ...
              {dashboard?.activeLenderPacket && (
                <div className="bg-card border border-border rounded-xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Active Lender Packet</p>
                    <button onClick={() => setActiveTab("lender")} className="text-xs text-primary flex items-center gap-1">
                      Open <ArrowRight className="w-3 h-3" />
                    </button>
                  </div>
                  <p className="text-sm font-medium text-foreground">{dashboard.activeLenderPacket.title}</p>
                  <div className="mt-2 h-1.5 rounded-full bg-border overflow-hidden">
                    <div className="h-full rounded-full bg-blue-500" style={{ width: `${dashboard.activeLenderPacket.completionPct}%` }} />
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">{dashboard.activeLenderPacket.completionPct}% complete</p>
                </div>
              )}
              ...
              {dashboard?.activeInvestorPacket && (
                <div className="bg-card border border-border rounded-xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Active Investor Packet</p>
                    <button onClick={() => setActiveTab("investor")} className="text-xs text-primary flex items-center gap-1">
                      Open <ArrowRight className="w-3 h-3" />
                    </button>
                  </div>
                  <p className="text-sm font-medium text-foreground">{dashboard.activeInvestorPacket.title}</p>
                  <div className="mt-2 h-1.5 rounded-full bg-border overflow-hidden">
                    <div className="h-full rounded-full bg-emerald-500" style={{ width: `${dashboard.activeInvestorPacket.completionPct}%` }} />
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">{dashboard.activeInvestorPacket.completionPct}% complete</p>
                </div>
              )}
              ...
              <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-4">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">This module is for readiness preparation and structure only. It does not constitute financial, legal, or investment advice. All materials should be reviewed by qualified counsel before external use.</p>
                  </div>
                </div>
              </div>
            </>
      ...
      {activeTab === "lender" && (
        <div className="space-y-4">
          {lenderPackets.length === 0 ? (
            <div className="text-center py-8 text-sm text-muted-foreground">No lender packets found.</div>
          ) : activeLenderId ? (
            <div className="space-y-4">
              <button onClick={() => setActiveLenderId(null)} className="text-xs text-muted-foreground flex items-center gap-1 hover:text-foreground transition-colors">
                ← Back to all packets
              </button>
              {(() => {
              ...
              {lenderPackets.map(p => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{p.lenderType.toUpperCase()} · {p.completionPct}% complete</p>
                  </div>
      ...
      {activeTab === "investor" && (
        <div className="space-y-4">
          {investorPackets.length === 0 ? (
            <div className="text-center py-8 text-sm text-muted-foreground">No investor packets found.</div>
          ) : activeInvestorId ? (
            <div className="space-y-4">
              <button onClick={() => setActiveInvestorId(null)} className="text-xs text-muted-foreground flex items-center gap-1 hover:text-foreground transition-colors">
                ← Back to all packets
              </button>
              {(() => {
            ...
            <div className="space-y-3">
              {investorPackets.map(p => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-foreground">{p.title}</p>
                    <p className="text-xs text-muted-foreground">{p.investorType} · {p.completionPct}% complete</p>
                  </div>
                  <ChevronRight className="w-4 h-4 text-muted-foreground shrink-0" />
                </button>
              ))}
      ...
      {activeTab === "diligence" && (
        <div className="space-y-4">
          {checklists.length === 0 ? (
            <div className="text-center py-8 text-sm text-muted-foreground">No checklists found.</div>
          ) : (
            checklists.map(c => (
...
interface CapTableEntry {
  id: number; holderName: string; holderType: string; shareClass: string;
  sharesPlaceholder?: string; ownershipPct?: string; vestingSchedule?: string; notes?: string;
}
...
function CapTablePanel() {
  ...
  const typeColors: Record<string, string> = {
    founder: "#c9a96e", employee: "#3b82f6", advisor: "#8b5cf6",
    investor: "#10b981", option_pool: "#f59e0b", other: "#6b7280",
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Structural ownership placeholder — not a legal cap table</p>
        </div>
        <button onClick={() => setShowAdd(!showAdd)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-primary text-white hover:bg-primary/90 transition-colors">
          <Plus className="w-3.5 h-3.5" /> Add Entry
        </button>
      </div>

      <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl px-4 py-3">
        <p className="text-xs text-amber-600">This is a structural placeholder only. Consult qualified legal counsel before sharing ownership information externally.</p>
      </div>
        ...
        <div className="bg-card border border-border rounded-xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select value={newEntry.holderType} onChange={e => setNewEntry(n => ({ ...n, holderType: e.target.value }))} className="w-full mt-1 text-xs bg-background border border-border rounded-lg px-3 py-2 text-foreground">
                {["founder", "employee", "advisor", "investor", "option_pool", "other"].map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={() => addMut.mutate(newEntry)} disabled={!newEntry.holderName || addMut.isPending} className="px-3 py-1.5 rounded-lg text-xs font-medium bg-primary text-white hover:bg-primary/90 disabled:opacity-50 transition-colors">
              {addMut.isPending ? "Saving..." : "Add"}
            </button>
            <button onClick={() => setShowAdd(false)} className="px-3 py-1.5 rounded-lg text-xs font-medium bg-card border border-border text-muted-foreground hover:text-foreground transition-colors">Cancel</button>
          </div>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="bg-card border border-border rounded-xl divide-y divide-border/60">
          {entries.length === 0 ? (
            <div className="py-6 text-center text-xs text-muted-foreground">No cap table entries yet. Add holders to structure placeholder ownership.</div>
          ) : entries.map(e => (
            <div key={e.id} className="flex items-center gap-3 px-4 py-3">
              <div className="w-2 h-2 rounded-full shrink-0" style={{ background: typeColors[e.holderType] ?? "#6b7280" }} />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-foreground">{e.holderName}</p>
                <p className="text-xs text-muted-foreground">{e.holderType} · {e.shareClass}</p>
              </div>
              {e.ownershipPct && (
                <span className="text-sm font-semibold" style={{ color: typeColors[e.holderType] ?? "#6b7280" }}>{e.ownershipPct}%</span>
              )}
            </div>
          ))}
        </div>
