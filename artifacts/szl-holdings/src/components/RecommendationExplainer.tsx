...
import { ChevronDown, ChevronUp, Activity, Shield, AlertTriangle, Clock, Database, Zap, TrendingUp, Info } from "lucide-react";
...
const PRIORITY_STYLE: Record<Recommendation["priority"], { color: string; bg: string; border: string }> = {
...
const CONTRIBUTION_STYLE: Record<RecommendationEvidence["contribution"], { icon: typeof Activity; color: string; label: string }> = {
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginTop: 4 }}>
          <div style={{ flex: 1, height: 3, borderRadius: 2, background: "hsla(215,30%,25%,0.4)", overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${ev.weight * 100}%`, background: style.color, borderRadius: 2 }} />
          </div>
          <span style={{ fontSize: "0.6rem", fontFamily: "var(--font-mono, monospace)", color: "#64748b" }}>
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
            <span style={{ fontSize: "0.8rem", fontWeight: 700, color: "#e2e8f0" }}>{rec.title}</span>
            <span style={{
              fontSize: "0.6rem",
              fontFamily: "var(--font-mono, monospace)",
              padding: "1px 6px",
          ...
          <p style={{ fontSize: "0.7rem", color: "#94a3b8", marginTop: 3, lineHeight: 1.5 }}>{rec.rationale}</p>
          <div style={{ display: "flex", alignItems: "center", gap: "0.875rem", marginTop: 6, flexWrap: "wrap" }}>
      ...
      {open && (
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.375rem", marginBottom: 6 }}>
            ...
            <span style={{ fontSize: "0.6rem", fontFamily: "var(--font-mono, monospace)", color: "#475569", textTransform: "uppercase", letterSpacing: "0.08em" }}>
              Why this recommendation
            </span>
            {rec.modelVersion && (
              <span style={{ marginLeft: "auto", fontSize: "0.6rem", fontFamily: "var(--font-mono, monospace)", color: "#334155" }}>
                {rec.modelVersion}
              </span>
            )}
          </div>
          {rec.evidence.map((ev, i) => <EvidenceRow key={i} ev={ev} />)}
          {rec.generatedAt && (
...
export const SAMPLE_RECOMMENDATIONS: Recommendation[] = [
  {
    ...
    rationale: "Three vessels have gone AIS-dark in a known sanction-evasion corridor. Historical pattern matches 2023 OFAC enforcement action.",
    confidence: 91,
    ...
    evidence: [
      { label: "AIS dark events", value: "3 vessels dark >6h in Bohai Strait", source: "Vessels/AIS", freshness: "4m ago", weight: 0.42, contribution: "supporting" },
      { label: "OFAC pattern match", value: "85% similarity to SDN-evade tactic 2023-11", source: "Compliance/OFAC", freshness: "1h ago", weight: 0.31, contribution: "supporting" },
      { label: "Cargo manifest", value: "Dual-use goods flagged", source: "Terra/Trade", freshness: "18m ago", weight: 0.18, contribution: "supporting" },
      { label: "Port of loading", value: "Tianjin — not currently sanctioned", source: "Alloy/KB", freshness: "2h ago", weight: 0.09, contribution: "contradicting" },
    ],
  },
  {
    id: "rec-002",
    title: "Accelerate Terra portfolio hedge — interest rate exposure",
    rationale: "Federal Reserve forward guidance signals 25bps increase. 4 Terra assets floating-rate exposure exceeds risk threshold.",
    confidence: 74,
    ...
    evidence: [
      { label: "Fed forward guidance", value: "+25bps priced at 78%", source: "Lyte/Macro", freshness: "2h ago", weight: 0.35, contribution: "supporting" },
      { label: "Floating-rate LTV", value: "4 assets >65% LTV", source: "Terra/Portfolio", freshness: "1d ago", weight: 0.38, contribution: "supporting" },
      { label: "Hedge instrument cost", value: "+0.4% vs fixed rate", source: "Finance/FX", freshness: "30m ago", weight: 0.27, contribution: "neutral" },
    ],
  },
];
