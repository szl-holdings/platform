...
const capabilities = [
  { label: "Agent orchestration", description: "Multi-agent workflows with structured approval chains" },
  { label: "Signal routing", description: "Intelligent signal dispatch across the ecosystem" },
  { label: "AI governance", description: "Explainability, audit trails, and decision accountability" },
  { label: "Ecosystem backbone", description: "Shared intelligence layer connecting all platforms" },
];
...
function FlowDiagram() {
  ...
  return (
    <div style={{ position: "relative", height: "64px", overflow: "visible" }}>
      <svg viewBox="0 0 100 20" className="w-full h-full" preserveAspectRatio="none" aria-hidden="true">
        {[0, 1, 2].map(i => (
          <m.line
            key={i}
            x1={nodes[i].x + 8} y1="10"
            x2={nodes[i + 1].x - 8} y2="10"
        ...
        {nodes.map((n, i) => (
          ...
          >
            <circle cx={n.x} cy="10" r={n.highlight ? "5" : "3.5"}
  ...
  return (
    <section style={{
      ...
      <div style={{
        position: "absolute",
        inset: 0,
        background: "radial-gradient(ellipse at 60% 50%, hsla(218,50%,58%,0.04) 0%, transparent 60%)",
        pointerEvents: "none",
      }} aria-hidden="true" />
        ...
        <div className="grid lg:grid-cols-[1fr,400px] gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <p style={{
              ...
              Alloy is the AI orchestration engine that connects the entire ecosystem. Every intelligent workflow, agent decision, and cross-platform signal flows through Alloy.
            </p>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "0.75rem" }}>
                  <div style={{ width: "6px", height: "6px", borderRadius: "50%", background: "hsl(152,50%,42%)", animation: "pulse 2s infinite" }} />
                  <span style={{ fontSize: "11px", fontWeight: "600", color: "hsl(210,5%,46%)", letterSpacing: "0.04em", textTransform: "uppercase" }}>System status</span>
                </div>
                {[
                  ...
                  <div key={item.name} style={{
                    ...
                    <span style={{ fontSize: "12.5px", color: "hsl(210,5%,56%)" }}>{item.name}</span>
                    <span style={{ fontSize: "11px", fontWeight: "500", color: item.color }}>{item.status}</span>
                  </div>
                ))}
              </div>
            </div>
