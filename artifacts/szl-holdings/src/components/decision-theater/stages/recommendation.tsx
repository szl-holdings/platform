...
import { SeverityBadge, AnimatedConfidenceBadge } from "../helpers";
...
export function LiveRecommendationStage({ recommendations }: { recommendations: LiveRecommendation[] }) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        <span className="font-semibold text-foreground">{recommendations.length}</span> active AI-generated recommendations pulled from the live Alloy recommendation engine.
      </p>
          ...
          >
            <div className="flex items-start gap-3 mb-2">
              <div className="flex-1 min-w-0">
                <p className="text-[12px] font-semibold text-foreground leading-tight">{rec.title}</p>
                <p className="text-[10px] text-muted-foreground mt-0.5">{rec.domain} · {rec.entity_type} · {rec.timeframe}</p>
              </div>
            ...
            <div className="mt-2">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[10px] font-mono text-foreground">{(rec.score * 100).toFixed(0)}</span>
              </div>
            ...
            {rec.recommended_action && (
              <p className="text-[11px] text-muted-foreground mt-2 line-clamp-2">{rec.recommended_action}</p>
            )}
          </m.div>
        ))}
      </div>
