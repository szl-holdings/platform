...
export function LiveContextStage({ metrics, recommendations }: { metrics: LiveMetrics | null; recommendations: LiveRecommendation[] }) {
  if (!metrics) return <p className="text-sm text-muted-foreground">Loading live context…</p>;

  const domainMetrics = [
    { label: "Real Estate Distress Events", value: metrics.beacon.total_distress_properties, max: Math.max(metrics.beacon.total_distress_properties, 100), color: "#10b981" },
    { label: "Security Vulnerabilities", value: metrics.firestorm.open_vulnerabilities, max: Math.max(metrics.firestorm.open_vulnerabilities, 50), color: "#ef4444" },
    { label: "AI Recommendations Generated", value: metrics.alloy.total_recommendations, max: Math.max(metrics.alloy.total_recommendations, 1000), color: "#ec4899" },
    { label: "Platform Audit Events (30d)", value: metrics.platform.audit_events_30d, max: Math.max(metrics.platform.audit_events_30d, 500), color: "#8b5cf6" },
  ];
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">Cross-domain context assembled from live platform telemetry — real signal correlation across active domain packs.</p>
      <div className="rounded-xl border border-border/40 bg-card/60 p-5">
        <h3 className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-4">Platform Signal Intensity</h3>
        <div className="space-y-3">
          {domainMetrics.map((item, i) => (
            <div key={item.label}>
              <div className="flex items-center justify-between mb-1">
                <span className="text-[11px] text-muted-foreground">{item.label}</span>
                <span className="text-[11px] font-semibold text-foreground">{item.value.toLocaleString()}</span>
              </div>
              <div className="h-2 rounded-full bg-muted/20 overflow-hidden">
                <m.div
                  ...
                  initial={{ width: 0 }}
                  animate={{ width: `${item.max > 0 ? Math.min(100, (item.value / item.max) * 100) : 0}%` }}
