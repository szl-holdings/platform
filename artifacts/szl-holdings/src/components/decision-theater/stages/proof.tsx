...
export function LiveProofStage({ auditRecords, auditTotal, metrics }: {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="rounded-xl border border-border/40 bg-card/60 p-5">
          <div className="flex items-center gap-2 mb-3">
            <h3 className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Audit Chain Summary</h3>
            {hasRealAudit && (
              <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded border bg-emerald-500/10 border-emerald-500/30 text-emerald-400">LIVE</span>
            )}
          </div>
          {hasRealAudit ? (
            <div className="space-y-1.5">
              {[
                ...
                { label: "Event types", value: [...new Set(auditRecords.map(r => r.action_type))].length.toLocaleString() },
                { label: "Entity types", value: [...new Set(auditRecords.map(r => r.entity_type))].length.toLocaleString() },
              ].map(item => (
                <div key={item.label} className="flex items-center justify-between">
                  <span className="text-[11px] text-muted-foreground">{item.label}</span>
                  <span className="text-[11px] font-semibold text-foreground font-mono">{item.value}</span>
          ...
          <div className="space-y-0">
            {hasRealAudit
              ? auditRecords.slice(0, 6).map((rec, i, arr) => (
                  ...
                  >
                    {i < arr.length - 1 && <div className="absolute left-[7px] top-5 bottom-0 w-px bg-border/30" />}
                    <div className="w-4 h-4 rounded-full bg-teal-500/20 flex items-center justify-center flex-shrink-0 z-10 mt-0.5">
                      <div className="w-1.5 h-1.5 rounded-full bg-teal-400" />
                    </div>
                    <div className="flex-1 pb-3">
                      <p className="text-[11px] font-semibold text-foreground">{rec.action_type.replace(/_/g, " ")}</p>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] text-muted-foreground font-mono">{rec.entity_type}</span>
                        <span className="text-[9px] font-mono text-muted-foreground">
              ...
              : [
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 pb-3">
                        <p className="text-[11px] font-semibold text-foreground">{entry.label.replace(/_/g, " ")}</p>
