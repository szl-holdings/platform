...
import { Activity, BarChart2, Brain, Shield, Ship, Zap } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Two independent domain signals fire within a 4-minute window, triggering cross-domain
        correlation via the Prism Event Bus.
      </p>
      ...
      <div className="rounded-lg border border-border/30 bg-muted/10 px-4 py-3 flex items-center gap-3">
        ...
        <p className="text-[11px] text-muted-foreground">
          ...
          {engine.busStats.totalPublished} events published across{' '}
          {Object.keys(engine.busStats.byType).length} event types.{' '}
...
export function LiveSignalStage({ metrics }: { metrics: LiveMetrics | null }) {
  ...
  const signalItems = [
    ...
    },
    {
      ...
      severity:
        metrics.beacon.high_opportunity_properties > 5 ? ('high' as const) : ('medium' as const),
      details: [
        {
        ...
        },
        {
          ...
          value: metrics.beacon.high_opportunity_properties.toLocaleString(),
        },
    ...
    },
    {
      ...
      severity:
        metrics.firestorm.open_vulnerabilities > 10
          ? ('critical' as const)
          : metrics.firestorm.open_vulnerabilities > 0
            ? ('high' as const)
            : ('info' as const),
      details: [
        {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Live platform signals ingested from active domain packs — real telemetry from the SZL
        Holdings platform.
      </p>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
