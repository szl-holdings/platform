...
export function AnimatedBar({ value, max, color, delay = 0 }: { value: number; max: number; color: string; delay?: number }) {
...
export function SeverityBadge({ severity }: { severity: string }) {
  ...
  return (
    <span className={cn("text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded border", colors[severity] ?? colors.info)}>
      {severity}
    </span>
  );
}

export function LiveDataBanner({ status, lastFetchedAt, onRefresh }: { status: string; lastFetchedAt: string | null; onRefresh: () => void }) {
  return (
    <m.div
    ...
    >
      ...
      <span className="text-[11px] text-muted-foreground flex-1">
        ...
        {status === "error" && <span className="text-red-400">Could not load live data — API unreachable or session expired</span>}
        {status === "success" && (
          <>
            <span className="font-semibold text-emerald-400">Live Data</span>
            {lastFetchedAt && ` · Fetched ${new Date(lastFetchedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })}`}
          </>
        )}
      </span>
      <button onClick={onRefresh} className="text-muted-foreground hover:text-foreground transition-colors" title="Refresh live data">
