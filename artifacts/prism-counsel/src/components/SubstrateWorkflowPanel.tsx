    ...
    try {
      const run = await apiFetch<Record<string, unknown>>("/control-tower/substrate/run", {
        ...
        body: JSON.stringify({
          ...
          input: { matterIds: ["MTR-2024-0108", "MTR-2024-0072"], lookAheadDays: 14, includePrivileged: false },
          mode,
        }),
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="text-xs font-semibold text-slate-100">Evidence Packaging and Deadline Escalation</p>
              <p className="text-[10px] text-violet-400/50 mt-0.5 font-mono">Substrate · prism-counsel-evidence-packaging · Phase 2</p>
            </div>
          ...
          <p className="text-[10px] text-slate-400 mt-1.5 leading-relaxed">
            Packages matter evidence bundles and escalates approaching deadlines. Attorney approval required before client notification.
          </p>

          {status !== "idle" && (
            <div className="mt-2 flex items-center gap-2">
              {status === "running" && <span className="text-[9px] font-mono text-violet-400 animate-pulse">● RUNNING</span>}
      ...
      {expanded && result && (
        <div className="border-t border-violet-500/10 pt-3 space-y-3">
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={m.label} className="rounded border border-slate-800 bg-slate-900/60 p-2">
                ...
                <p className={`text-sm font-mono font-bold truncate ${STATUS_COLOR[String(m.value)] ?? "text-violet-300"}`}>{m.value}</p>
              </div>
            ))}
          </div>
                ...
                <div key={s.stageId} className="rounded border border-slate-800 bg-slate-900/40 px-2.5 py-1.5 flex items-center justify-between">
                  ...
                  <div className="text-right ml-3 shrink-0">
                    ...
                    {s.confidence !== undefined && (
                      <p className="text-[9px] font-mono text-violet-400/60">{(s.confidence * 100).toFixed(0)}%</p>
                    )}
                  </div>
                </div>
              ))}
          ...
          <div className="flex items-center justify-between">
            <span className="text-[9px] font-mono text-slate-600">mode:{result.mode} · inbox:prism-counsel-evidence-packaging</span>
            <span className="flex items-center gap-1 text-[9px] font-mono text-emerald-400/60">
              <Shield className="w-2.5 h-2.5" />privilege-reviewed
            </span>
          </div>
          {mode === "dry-run" && (
            <div className="rounded border border-sky-500/15 bg-sky-500/5 p-2">
              <p className="text-[9px] font-mono text-sky-400">DRY-RUN — attorney notifications and external submissions suppressed.</p>
            </div>
          )}
          {status === "pending-approval" && (
            <div className="rounded border border-amber-500/20 bg-amber-500/5 p-2">
              <p className="text-[9px] font-mono text-amber-400">PENDING APPROVAL — paused at approval gate. Check the attorney approvals inbox to resume.</p>
            </div>
          )}
        </div>
      )}
