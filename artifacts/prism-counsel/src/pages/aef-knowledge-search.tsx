...
import { Search, AlertTriangle, Loader2, ChevronDown, ChevronUp, Shield, BarChart3, Route, FileText, Scale } from "lucide-react";
...
function HitCard({ hit }: { hit: SearchHit }) {
  ...
  return (
    <div className="rounded-lg border border-violet-500/15 bg-slate-950/60">
      <div
        className="flex items-start gap-3 p-4 cursor-pointer hover:bg-violet-500/3 transition-colors"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <p className="text-[10px] font-mono text-violet-400/40 mt-0.5 truncate">{hit.sourceUri ?? hit.chunkId}</p>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              ...
              {expanded ? <ChevronUp className="w-3.5 h-3.5 text-violet-400/40" /> : <ChevronDown className="w-3.5 h-3.5 text-violet-400/40" />}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            {hit.boostApplied && (
              <span className="text-[9px] font-mono px-1 py-0.5 rounded bg-violet-500/10 border border-violet-500/20 text-violet-400/60">BOOST</span>
            )}
          </div>
        </div>
      </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-violet-500/10 pt-3 space-y-3">
          <div className="rounded bg-violet-500/4 border border-violet-500/12 p-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            {hit.traceId && (
              <div className="rounded bg-violet-500/4 border border-violet-500/12 p-2">
    ...
    <div className="p-6 space-y-6 max-w-3xl mx-auto">
      <div className="flex items-center gap-3">
        <div className="w-9 h-9 rounded-lg bg-violet-500/10 border border-violet-500/20 flex items-center justify-center">
          <Scale className="w-4.5 h-4.5 text-violet-400" />
        </div>
        <div>
          <h1 className="text-sm font-semibold text-slate-100">AEF Legal Matter Search</h1>
          <p className="text-[10px] font-mono text-violet-400/40">Profile: {PROFILE_ID} · PRISM Counsel</p>
        </div>
        ...
        <div className="rounded-lg border border-violet-500/20 bg-violet-500/5 p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-violet-400/60 mt-1 leading-relaxed">
              Set <code className="font-mono text-violet-300">VITE_AEF_GATEWAY_URL</code> and <code className="font-mono text-violet-300">VITE_AEF_API_KEY</code> in your environment. See <code className="font-mono text-violet-300">docs/aef/RUNBOOK.md</code>.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            {results.processingMs !== undefined && (
              <span className="text-[9px] font-mono text-violet-400/30">{results.processingMs.toFixed(0)}ms</span>
            )}
          </div>

          {results.hits.length === 0 ? (
            <div className="rounded-lg border border-violet-500/10 p-8 text-center">
              <p className="text-xs text-violet-400/40 font-mono">No legal documents matched the query above the score threshold.</p>
            </div>
          ) : (
          ...
          <div className="rounded bg-violet-500/4 border border-violet-500/10 p-3 flex items-center gap-3">
            <Route className="w-3 h-3 text-violet-400/40 shrink-0" />
            <div className="flex gap-1.5 flex-wrap">
              {results.retrievalPath.map((stage, i) => (
                <span key={i} className="text-[9px] font-mono text-violet-400/40 after:content-['→'] after:mx-1 after:text-violet-400/20 last:after:content-['']">{stage}</span>
              ))}
            </div>
          </div>
        </div>
