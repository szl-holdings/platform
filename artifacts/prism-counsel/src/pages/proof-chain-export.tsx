...
function ProofEntry({ entry, showHash, userRole }: { entry: ProofChainEntry; showHash: boolean; userRole: string }) {
  ...
  return (
    <div className="relative flex gap-4">
      <div className="flex flex-col items-center shrink-0">
        <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${evtColor}15`, color: evtColor, border: `1px solid ${evtColor}25` }}>
          {icon}
        </div>
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[9px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded" style={{ background: `${evtColor}18`, color: evtColor }}>
                    {entry.eventType.replace("-", " ")}
                  </span>
                  <span className="text-[9px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded" style={{ background: `${privColor}18`, color: privColor }}>
                    {entry.privilegeLevel}
                  </span>
                  {entry.redacted && (
                    <span className="flex items-center gap-0.5 text-[9px] font-semibold px-1.5 py-0.5 rounded" style={{ background: "rgba(239,68,68,0.12)", color: "#ef4444" }}>
                      <EyeOff className="w-2 h-2" /> Redacted
                    </span>
                  )}
                </div>
                {isRedacted ? (
                  <p className="text-xs font-semibold text-red-400/60 italic">[Content redacted — insufficient privilege level]</p>
                ) : (
                ...
                <div className="flex items-center gap-3 mt-1.5 text-[10px] text-white/30">
                  ...
                  {entry.documentRef && <><span>·</span><span className="font-mono">{entry.documentRef}</span></>}
                </div>
              </div>
              {!isRedacted && (
              ...
              {entry.parties.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {entry.parties.map((p) => (
                      <span key={p} className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.5)" }}>{p}</span>
                    ))}
                  </div>
                </div>
              )}
              {showHash && entry.hash && (
                <div className="flex items-center gap-2 p-2 rounded-lg" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)" }}>
...
export default function ProofChainExport() {
  ...
  const handleGenerate = async () => {
    ...
    const bundle = {
      exportMeta: {
        ...
        attestation: "PRISM Counsel privilege review — all redactions applied per matter wall policy",
      },
      matter: {
        id: matter.id,
        name: matter.name,
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-1 space-y-4">
          <div className="rounded-2xl border border-white/5 p-5 space-y-4" style={{ background: "rgba(255,255,255,0.02)" }}>
            <h3 className="text-xs font-semibold text-white/60 uppercase tracking-wider">Export Configuration</h3>
            ...
            <div className="space-y-2">
              <label className="text-[10px] text-white/30 uppercase tracking-wider block">Export Scope</label>
              <select
                value={exportScope}
            ...
            <div className="space-y-2">
              <label className="text-[10px] text-white/30 uppercase tracking-wider block">Viewing As</label>
              <select
                value={userRole}
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                style={{ background: showHashes ? "rgba(167,139,250,0.4)" : "rgba(255,255,255,0.08)" }}
              >
                <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${showHashes ? "translate-x-4" : "translate-x-0.5"}`} />
              </button>
            </div>
            ...
            <div className="pt-2 border-t border-white/5">
              <div className="rounded-lg p-3 space-y-1.5" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
                ...
                <p className="text-xs font-semibold text-white/70">{visibleEntries.length} events included</p>
                <p className="text-[10px] text-white/30">Privilege levels: {allowedPriv.join(", ")}</p>
            ...
            >
              {generating ? (
                <>
                  <div className="w-4 h-4 border-2 rounded-full animate-spin" style={{ borderColor: "rgba(167,139,250,0.2)", borderTopColor: ACCENT }} />
                  Generating bundle…
                </>
              ) : (
                <>
            ...
            {generated && (
              <div className="flex items-start gap-2.5 p-3 rounded-lg" style={{ background: "rgba(34,197,94,0.06)", border: "1px solid rgba(34,197,94,0.15)" }}>
                <CheckCircle className="w-3.5 h-3.5 text-green-400 shrink-0 mt-0.5" />
                <div>
                  <p className="text-[11px] font-semibold text-green-400">Bundle ready</p>
                  <p className="text-[10px] text-green-400/60 mt-0.5">{visibleEntries.length} events · Privilege attestation attached · SHA-256 hashes included</p>
                  <p className="text-[10px] text-green-400/40 mt-1">prism-bundle-{matter.matterNumber}-{exportScope}.pdf</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="rounded-2xl border border-white/5 p-5 space-y-3" style={{ background: "rgba(255,255,255,0.02)" }}>
            <h3 className="text-xs font-semibold text-white/60 uppercase tracking-wider">Matter Summary</h3>
            <div className="space-y-2 text-[11px]">
              <div className="flex justify-between">
                <span className="text-white/30">Matter</span>
                <span className="text-white/60 truncate max-w-[160px] text-right">{matter.name.split(" — ")[0]}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white/60 truncate max-w-[160px] text-right">{matter.jurisdiction}</span>
              </div>
              ...
              <div className="flex justify-between">
                ...
                <span style={{ color: getPrivilegeColor(matter.privilegeLevel) }}>{matter.privilegeLevel}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="rounded-2xl border border-white/5 overflow-hidden" style={{ background: "rgba(255,255,255,0.02)" }}>
            <div className="px-5 py-3 border-b border-white/5 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono text-white/25">{visibleEntries.length} events</span>
              </div>
              <div className="flex items-center gap-2">
                <Shield className="w-3 h-3 text-white/20" />
                <span className="text-[10px] text-white/20">{scopeLabels[exportScope].split(" — ")[0]}</span>
              </div>
            </div>
            <div className="p-5">
              {visibleEntries.length === 0 ? (
                ...
                <div>
                  {visibleEntries.map((entry) => (
                    <ProofEntry key={entry.id} entry={entry} showHash={showHashes} userRole={userRole} />
                  ))}
                </div>
              )}
            </div>
      ...
      <div className="rounded-xl p-4 border border-white/5 text-[11px] text-white/20" style={{ background: "rgba(255,255,255,0.01)" }}>
        <div className="flex items-start gap-2.5">
          ...
          <div>
            ...
            <p className="leading-relaxed">All exports from PRISM Counsel include an attestation of privilege review. Documents marked <span style={{ color: "#f97316" }}>privileged</span> or <span style={{ color: "#ef4444" }}>restricted</span> are excluded from court-scope bundles. Each export is logged in the matter audit trail with your identity, timestamp, and bundle hash. PRISM Counsel does not constitute legal advice — all outputs require review by qualified counsel.</p>
          </div>
        </div>
      </div>
    </div>
