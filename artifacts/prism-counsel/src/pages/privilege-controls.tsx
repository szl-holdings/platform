...
export default function PrivilegeControls() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <p className="text-xs text-white/30">Matter walls · Redaction workflow · Role-aware access · Ethics screens</p>
      ...
      {wallMatters.length > 0 && (
        <div className="space-y-3">
          <h2 className="text-xs font-semibold text-white/50 uppercase tracking-wider">Active Matter Walls</h2>
          {wallMatters.map((m) => (
            <div key={m.id} className="rounded-xl p-4 border privilege-glow" style={{ background: "rgba(239,68,68,0.04)", borderColor: "rgba(239,68,68,0.2)" }}>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-[10px] mt-2">
                      <span className="text-white/30">Approved: {m.wall.approvedUsers.join(", ")}</span>
                      <span className="text-red-400/50">Blocked: {m.wall.blockedRoles.join(", ")}</span>
                    </div>
                  </div>
                </div>
                <span className="text-[9px] font-semibold px-2 py-1 rounded-full shrink-0" style={{ background: "rgba(239,68,68,0.12)", color: "#ef4444", border: "1px solid rgba(239,68,68,0.2)" }}>
                  WALL ACTIVE
                </span>
              </div>
            </div>
      ...
      <div className="space-y-3">
        <h2 className="text-xs font-semibold text-white/50 uppercase tracking-wider">Privilege Level Overview</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/5">
                ...
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-white/30 uppercase tracking-wider">Privilege Level</th>
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-white/30 uppercase tracking-wider">Wall</th>
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-white/30 uppercase tracking-wider">Lead Counsel</th>
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-white/30 uppercase tracking-wider">Parties</th>
              </tr>
            </thead>
              ...
              {SEED_MATTERS.map((m) => {
                ...
                return (
                  <tr key={m.id} className="hover:bg-white/2 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-[10px] font-semibold uppercase tracking-wider px-2 py-1 rounded-full" style={{ background: `${privColor}18`, color: privColor }}>
                        {m.privilegeLevel}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      {m.wall.enabled ? (
                        <span className="flex items-center gap-1 text-[10px] text-red-400"><Lock className="w-2.5 h-2.5" />Active</span>
                      ) : (
                        <span className="text-[10px] text-white/20">—</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-[11px] text-white/50">{m.leadCounsel}</td>
                    <td className="px-4 py-3 text-[11px] text-white/40">{m.parties.length} parties</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      <div className="rounded-2xl border border-white/5 p-5 space-y-4" style={{ background: "rgba(255,255,255,0.02)" }}>
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xs font-semibold text-white/60 uppercase tracking-wider">Live Policy Engine Check</h2>
          <span className="text-[9px] px-1.5 py-0.5 rounded font-mono" style={{ background: "rgba(167,139,250,0.1)", color: ACCENT }}>prism-counsel.matter-wall@1.0</span>
        </div>
        <p className="text-[11px] text-white/30">Submit an access request to the backend policy engine and see the real-time decision.</p>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="space-y-1">
            ...
            <select
              value={checkRole}
              onChange={(e) => { setCheckRole(e.target.value); setPolicyResult(null); }}
              className="text-xs bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white/70 focus:outline-none focus:border-purple-500/40 w-full"
            >
              {["gc", "partner", "associate", "paralegal"].map((r) => <option key={r} value={r}>{r}</option>)}
            </select>
          </div>
          <div className="space-y-1">
            <label className="text-[10px] text-white/30 uppercase tracking-wider block">Action</label>
            <select
              value={checkAction}
              onChange={(e) => { setCheckAction(e.target.value as typeof checkAction); setPolicyResult(null); }}
              className="text-xs bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white/70 focus:outline-none focus:border-purple-500/40 w-full"
            >
        ...
        {checkError && (
          <div className="p-3 rounded-xl border text-[11px]" style={{ background: "rgba(249,115,22,0.05)", borderColor: "rgba(249,115,22,0.15)", color: "rgba(249,115,22,0.8)" }}>
            <AlertTriangle className="w-3 h-3 inline mr-1.5" />{checkError}
          </div>
        )}

        {policyResult && (
          <div className="rounded-xl border p-4 space-y-3" style={{ background: policyResult.allowed ? "rgba(34,197,94,0.04)" : "rgba(239,68,68,0.04)", borderColor: policyResult.allowed ? "rgba(34,197,94,0.2)" : "rgba(239,68,68,0.2)" }}>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-xs font-semibold" style={{ color: policyResult.allowed ? "#22c55e" : "#ef4444" }}>
                  {policyResult.allowed ? "Access allowed" : "Access denied"} · Effect: <span className="font-mono">{policyResult.effect}</span>
                </p>
                {policyResult.requiresApproval && (
                  <p className="text-[11px] text-orange-400 mt-0.5">Requires approval from: {policyResult.requiredApproverRole}</p>
                )}
              </div>
              <span className="text-[9px] font-mono text-white/20">{policyResult.policyEngine}</span>
            </div>
            ...
            {policyResult.violations.length > 0 && (
              <div className="space-y-1">
                {policyResult.violations.map((v) => (
                  <div key={v.policyId} className="text-[10px] text-red-400/70 font-mono bg-red-400/5 px-2 py-1 rounded">
