...
import {
  ...
  Settings, Radio, ChevronRight, Zap, Shield,
  type LucideProps
} from "lucide-react";
...
function NavItem({ href, label, icon: Icon }: { href: string; label: string; icon: LucideIcon }) {
  ...
  const isActive = location === href || (href.endsWith("/") && location === href.slice(0, -1))
    || (label === "Today's Brief" && (location === BASE || location === `${BASE}/` || location === "/"));
...
export default function Shell({ children }: { children: ReactNode }) {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", overflow: "hidden", background: "var(--pulse-bg)" }}>
      {/* Sidebar */}
      <aside style={{
        width: 224,
        flexShrink: 0,
        ...
        <div style={{ padding: "20px 16px 16px", borderBottom: "1px solid var(--pulse-border)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
            ...
            <div>
              <div style={{ fontSize: "0.95rem", fontWeight: 700, color: "#e8edf8", letterSpacing: "-0.01em" }}>Pulse</div>
              <div style={{ fontSize: "0.6rem", color: "var(--pulse-text-muted)", letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 600 }}>AI Briefing</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: "10px 16px", borderBottom: "1px solid var(--pulse-border)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", background: "rgba(78,202,139,0.08)", borderRadius: 6, border: "1px solid rgba(78,202,139,0.15)" }}>
            <div className="live-indicator" style={{ width: 6, height: 6, borderRadius: "50%", background: "#4eca8b", flexShrink: 0 }} />
            <span style={{ fontSize: "0.67rem", color: "#4eca8b", fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase" }}>Live · Daily Brief Active</span>
          </div>
        </div>
        ...
        <div style={{ padding: "12px 14px", borderTop: "1px solid var(--pulse-border)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span style={{ fontSize: "0.6rem", color: "var(--pulse-gold-dim)", fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" }}>SZL-EXEC-RESTRICTED</span>
          </div>
          <div style={{ fontSize: "0.58rem", color: "var(--pulse-text-muted)", marginTop: 3 }}>{dateStr}</div>
        </div>
      </aside>
      ...
      <main style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column" }}>
        {/* Classification bar */}
        <div className="classification-bar" style={{ padding: "6px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
