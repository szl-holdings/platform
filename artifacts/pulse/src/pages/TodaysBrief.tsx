      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{
            ...
            background: agent?.color ?? "#c8a84b", flexShrink: 0,
          }} />
          <div>
            <div style={{ fontSize: "0.7rem", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--pulse-text-muted)", marginBottom: 3 }}>
              {section.title}
            </div>
            <div className="font-serif" style={{ fontSize: "1rem", color: "var(--pulse-text)", lineHeight: 1.4, maxWidth: 620 }}>
              {section.keyJudgment}
            </div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0, marginLeft: 16 }}>
          ...
          {expanded ? <ChevronUp size={14} color="var(--pulse-text-muted)" /> : <ChevronDown size={14} color="var(--pulse-text-muted)" />}
        </div>
      </div>
      ...
      {expanded && (
        <div style={{ padding: "20px 20px 20px 35px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {/* Key Findings */}
            <div style={{ background: "rgba(0,0,0,0.2)", borderRadius: 6, padding: 14, border: "1px solid var(--pulse-border)" }}>
              <div className="prose-brief" style={{ marginBottom: 10 }}><h3>Key Findings</h3></div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {section.keyFindings.map((f, i) => (
                  <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                    <div style={{ width: 6, height: 6, borderRadius: "50%", background: getRiskColor(f.severity), marginTop: 6, flexShrink: 0 }} />
                    <span style={{ fontSize: "0.82rem", color: "var(--pulse-text)", lineHeight: 1.5 }}>{f.finding}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              {section.assumptions.length > 0 && (
                <div style={{ background: "rgba(200,168,75,0.05)", borderRadius: 6, padding: 14, border: "1px solid rgba(200,168,75,0.15)", marginBottom: 8 }}>
                  <div className="prose-brief" style={{ marginBottom: 8 }}><h3>Key Assumptions</h3></div>
                  {section.assumptions.map((a, i) => (
                    <div key={i} style={{ fontSize: "0.78rem", color: "var(--pulse-text-dim)", lineHeight: 1.5, marginBottom: 4, paddingLeft: 10, borderLeft: "2px solid rgba(200,168,75,0.3)" }}>{a}</div>
                  ))}
                </div>
              )}
              {section.gaps.length > 0 && (
                <div style={{ background: "rgba(224,80,80,0.04)", borderRadius: 6, padding: 14, border: "1px solid rgba(224,80,80,0.12)" }}>
                  <div className="prose-brief" style={{ marginBottom: 8 }}><h3>Gaps & Unknowns</h3></div>
                  {section.gaps.map((g, i) => (
                    <div key={i} style={{ fontSize: "0.78rem", color: "var(--pulse-text-dim)", lineHeight: 1.5, marginBottom: 4, paddingLeft: 10, borderLeft: "2px solid rgba(224,80,80,0.3)" }}>{g}</div>
                  ))}
                </div>
              )}
            </div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 16, paddingTop: 12, borderTop: "1px solid var(--pulse-border)" }}>
            ...
            <button style={{
              background: "transparent", border: "1px solid var(--pulse-border-bright)",
              color: "var(--pulse-text-dim)", padding: "4px 10px", borderRadius: 4,
              fontSize: "0.7rem", cursor: "pointer", display: "flex", alignItems: "center", gap: 4,
            }}>
              File Dissent
            </button>
          </div>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
              <div className="font-mono" style={{ fontSize: "0.65rem", color: "var(--pulse-gold)", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase" }}>
                {brief.edition}
              </div>
            ...
            <p className="font-serif" style={{ fontSize: "1rem", color: "var(--pulse-text-dim)", lineHeight: 1.6, maxWidth: 640 }}>
              {brief.leadSentence}
            </p>
          </div>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {brief.domains.map(d => (
            <span key={d} style={{
              ...
              fontSize: "0.68rem", color: "var(--pulse-text-dim)", fontWeight: 500,
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {brief.recommendedActions.map((action, i) => {
              const pColor = action.priority === "P0" ? "#e05050" : action.priority === "P1" ? "#e08c40" : action.priority === "P2" ? "#c8a84b" : "#4eca8b";
              return (
                <div key={i} style={{
                  display: "flex", gap: 14, alignItems: "flex-start",
                  padding: "12px 16px", borderRadius: 6,
                  background: "var(--pulse-card)", border: `1px solid ${pColor}25`,
                  borderLeft: `3px solid ${pColor}`,
                }}>
                  <span style={{
                    fontWeight: 800, fontSize: "0.65rem", color: pColor,
                    background: `${pColor}18`, padding: "2px 6px", borderRadius: 3,
                    fontFamily: "JetBrains Mono, monospace", flexShrink: 0,
                  }}>{action.priority}</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: "0.88rem", fontWeight: 500, color: "var(--pulse-text)", marginBottom: 3 }}>{action.action}</div>
                    <div style={{ fontSize: "0.75rem", color: "var(--pulse-text-dim)", lineHeight: 1.5 }}>{action.rationale}</div>
                    <div style={{ display: "flex", gap: 12, marginTop: 6, fontSize: "0.7rem", color: "var(--pulse-text-muted)" }}>
        ...
        <div style={{
          ...
          <a
            ...
            style={{
              ...
              fontSize: "0.72rem", color: "var(--pulse-text-dim)",
              textDecoration: "none",
            }}
          >
