...
import { AutonomyTierBadge, VerifierBadge } from '../components/VerifierBadge';
...
async function generateExecBrief(domain: Domain): Promise<{ data: ExecBrief }> {
  const path =
    domain === 'consolidated' ? '/pulse/executive/generate' : `/pulse/executive/generate/${domain}`;
    ...
    >
      ...
      {expanded && (
        <div style={{ padding: '0 12px 12px' }}>
          {belief.caveats.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              {belief.caveats.map((c, i) => (
                <div
                  key={i}
                  style={{ display: 'flex', alignItems: 'flex-start', gap: 6, marginBottom: 3 }}
                >
          ...
          {relatedCitations.length > 0 && <CitationPanel citations={relatedCitations} />}
        </div>
      )}
    </div>
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      >
        ...
        >
          ...
          <span
            style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.3)', fontFamily: 'monospace' }}
          >
            {new Date(brief.generatedAt).toLocaleString()}
          </span>
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            <VerifierBadge
              status={brief.verifierStatus}
              feedback={brief.verifierFeedback}
              compact
            />
            <div
              style={{
      ...
      {brief.verifierStatus === 'revision_required' && brief.verifierFeedback && (
        <div
          style={{
            ...
            border: '1px solid rgba(232,133,91,0.25)',
          }}
        >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          {brief.whatWeBelieve.length === 0 && (
            <p style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.3)' }}>
              No belief statements generated.
            </p>
          )}
        </div>
      </div>
        ...
        >
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            ...
            {brief.entityProvenance.length > 10 && (
              <div
                style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.25)', padding: '2px 7px' }}
              >
      ...
      {brief.briefingId && (
        <div
          style={{ fontSize: '0.63rem', color: 'rgba(255,255,255,0.2)', fontFamily: 'monospace' }}
        >
          Linked brief: {brief.briefingId} · Trace IDs:{' '}
          {brief.sourceTraceIds.length > 0 ? brief.sourceTraceIds.join(', ') : 'none'}
        </div>
      )}
    </div>
  );
