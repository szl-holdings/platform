...
function readSnapshot(): MeshSnapshot {
  ...
  try {
    ...
    const pendingApprovals = (parsed.fixRequests ?? []).filter((r) => r.status === "pending").length;
    const index = parsed.resilienceOverall ?? DEFAULT_SNAPSHOT.index;
    const allResolved = Object.keys(statuses).length > 0 && openExposures === 0 && pendingApprovals === 0;
    return {
      index,
