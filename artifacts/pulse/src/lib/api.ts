...
import type { Briefing, CustomBriefRequest, DissentRecord, DomainKey, RiskLevel } from './data';
...
async function apiFetch<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  const data = (await res.json()) as { success?: boolean; error?: string } & Record<
    string,
    unknown
  >;
...
export function useBriefings(filters?: { domain?: DomainKey | 'all'; risk?: RiskLevel | 'all' }) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (isDemoMode()) {
        const data = await demoApiFetch<{ success: true; briefings: Briefing[]; total: number }>(
          '/api/pulse/demo/briefings',
        );
        if (filters?.domain && filters.domain !== 'all') {
          data.briefings = data.briefings.filter((b) =>
            b.domains.includes(filters.domain as DomainKey),
          );
        }
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (isDemoMode()) {
        ...
        const all = await demoApiFetch<{ success: true; briefings: Briefing[]; total: number }>(
          '/api/pulse/demo/briefings',
        );
...
export function useConfidenceHistory() {
  return useQuery({
    ...
    queryFn: async () => {
      if (isDemoMode()) {
        return demoApiFetch<{ success: true; history: ConfidenceHistoryEntry[] }>(
          '/api/pulse/demo/confidence',
        );
      }
      return apiFetch<{ success: true; history: ConfidenceHistoryEntry[] }>(
        '/api/pulse/confidence',
      );
    },
    select: (d) => d.history,
  });
...
export function useDissents() {
  return useQuery({
    ...
    queryFn: async () => {
      if (isDemoMode()) {
        return demoApiFetch<{ success: true; dissents: DissentRecord[] }>(
          '/api/pulse/demo/dissents',
        );
      }
    ...
    mutationFn: () => {
      if (isDemoMode()) {
        return Promise.reject(
          new Error(
            'Live AI generation is disabled in demo mode. Sign in to generate a fresh briefing.',
          ),
        );
      }
...
export function useFileDissent() {
  ...
  return useMutation({
    mutationFn: (input: FileDissentInput) => {
      if (isDemoMode()) {
        return Promise.reject(
          new Error('Write operations are not available in demo mode. Sign in to file a dissent.'),
        );
      }
...
export async function exportBriefingPdf(briefingId: string, briefingDate: string): Promise<void> {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `PDF export failed: ${res.status} ${res.statusText}${detail ? ` — ${detail}` : ''}`,
    );
  }
  ...
  return useMutation({
    ...
    onSuccess: (_data, vars) => {
      ...
      qc.invalidateQueries({
        queryKey: ['pulse', 'deployments', 'history', environment, vars.appId],
      });
    },
  });
}
  ...
  return useMutation({
    mutationFn: (input: RequestCustomBriefInput) => {
      if (isDemoMode()) {
        return Promise.reject(
          new Error(
            'Custom brief requests are not available in demo mode. Sign in to request a brief.',
          ),
        );
      }
