...
import { type Automation, type AutomationRun, automationRuns, automations } from '@/data/brokerage';
  ...
  return (
    ...
    >
      <div className="p-5">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              ...
              <button
                ...
                className={cn(
                  'flex items-center gap-1 text-[10px] font-semibold px-2 py-1 rounded-full border transition-colors flex-shrink-0',
                  enabled
                    ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'
                    : 'bg-terra-text-muted/10 text-terra-text-muted border-terra-border',
                )}
              >
        ...
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          ...
          <span
            className={cn(
              ...
              priorityConfig[automation.priority],
            )}
          >
        ...
        <div className="grid grid-cols-3 gap-3 mb-3">
          ...
          <div className="text-center rounded-lg bg-terra-bg border border-terra-border p-2">
            ...
            <p
              className={cn(
                ...
                automation.successRate >= 0.95
                  ? 'text-emerald-400'
                  : automation.successRate >= 0.85
                    ? 'text-amber-400'
                    : 'text-rose-400',
              )}
            >
          ...
          <div className="text-center rounded-lg bg-terra-bg border border-terra-border p-2">
            ...
            <p
              className={cn(
                'text-lg font-display font-bold',
                automation.failedCount > 0 ? 'text-rose-400' : 'text-terra-text',
              )}
            >
        ...
        <div className="border-t border-terra-border pt-3">
          ...
          <div className="space-y-1">
            {automation.actions.map((action, i) => (
              <div
                key={i}
                className="flex items-center gap-1.5 text-[10px] text-terra-text-secondary"
              >
        ...
        {automation.lastRun && (
          <p className="text-[10px] text-terra-text-muted mt-2">
    ...
    <>
      ...
      >
        <td className="py-3 px-4">
          <StatusIndicator
            status={
              run.status === 'success' ? 'success' : run.status === 'failed' ? 'error' : 'warning'
            }
            label={run.status}
          />
        </td>
      ...
      {failed.length > 0 && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-rose-500/30 bg-rose-500/10">
          ...
          <p className="text-sm text-rose-400 font-semibold">
            {failed.length} automation failure{failed.length > 1 ? 's' : ''} require attention
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          },
          {
            ...
            value: automations.reduce((s, a) => s + a.runCount, 0),
            alert: false,
            warn: false,
          },
          { label: 'Failed Runs', value: failed.length, alert: true, warn: false },
          {
            ...
            value: automations.reduce((s, a) => s + a.pendingRetries, 0),
            alert: false,
            warn: true,
          },
        ].map((m) => (
          ...
          <button
            ...
            className={cn(
              ...
              activeTab === tab
                ...
                : 'border-transparent text-terra-text-muted hover:text-terra-text',
            )}
          >
            {tab}
            {tab === 'failures' && failed.length > 0 && (
              <span className="ml-1.5 text-[10px] bg-rose-500/10 text-rose-400 px-1.5 py-0.5 rounded-full">
                {failed.length}
              </span>
            )}
          </button>
            ...
            <>
              <div className="flex items-center gap-2 px-4 py-3 rounded-lg bg-terra-surface border border-terra-border">
                ...
                <p className="text-xs text-terra-text-secondary">
                  Alloy retries failed automations automatically. Override or manually retry below.
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-terra-text-muted">
                          {run.affectedEntity} · {new Date(run.startedAt).toLocaleString()}
                        </p>
                        {run.errorMessage && (
