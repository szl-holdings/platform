...
const useCases = [
  { title: "Off-Market Discovery", desc: "Surface properties before they list. Distress signals, LLC ownership chains, and debt maturity flags create your sourcing edge." },
  { title: "Deal Sourcing at Scale", desc: "Run structured outreach from a distress queue — not a spreadsheet. Convert property intelligence into live pipeline." },
  { title: "Portfolio Review", desc: "Track lease expirations, occupancy gaps, and value-add opportunities across your entire hold." },
  { title: "Broker Management", desc: "Performance scorecards, response analytics, and deal attribution — for a serious brokerage operation." },
  { title: "Distressed Opportunity Tracking", desc: "Monitor pre-foreclosure through REO. Score, prioritize, and act — before the market catches up." },
  { title: "Diligence & Approvals", desc: "Structure the diligence process. Ownership verification, document collection, and deal approvals in one auditable workflow." },
];

const buyers = [
  { role: "Investors & Acquisitions", desc: "Source distressed opportunities, analyze ownership structures, model returns, and track deals from discovery to close." },
  { role: "Brokers & Agents", desc: "Manage your deal pipeline, track client relationships, monitor market movement, and hit your numbers." },
  { role: "Portfolio Teams", desc: "Monitor property health, track lease expirations, model renewals, and surface disposition opportunities." },
  { role: "Lenders & Capital", desc: "Underwrite with confidence. Cross-reference ownership, distress signals, market comps, and borrower history." },
];
  ...
  return (
    <div className="min-h-screen overflow-x-hidden" style={{ background: BG, color: "#c8ccd6", fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 h-14 flex items-center transition-all duration-400 ${scrolled ? "border-b" : ""}`}
        style={{ background: scrolled ? "rgba(10,12,16,0.96)" : "transparent", backdropFilter: scrolled ? "blur(20px)" : "none", borderColor: "rgba(255,255,255,0.05)" }}>
        <div className="max-w-[1100px] mx-auto px-6 w-full flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 rounded-lg" style={{ background: "rgba(45,106,79,0.12)", border: `1px solid rgba(45,106,79,0.22)` }}>
              <Building2 size={13} style={{ color: ACCENT_LIGHT }} />
            </div>
            <span className="font-bold text-sm tracking-tight text-white">Terra</span>
            <span className="hidden sm:inline text-[9px] tracking-[0.14em] uppercase ml-1" style={{ color: "rgba(255,255,255,0.18)", fontFamily: "monospace" }}>Property Intelligence</span>
          </div>
          <div className="hidden md:flex items-center gap-7">
            {[{ label: "Platform", href: "#platform" }, { label: "Doctrine", href: "#doctrine" }, { label: "Capabilities", href: "#capabilities" }, { label: "Who It's For", href: "#buyers" }].map(l => (
            ...
            <button onClick={onSignIn} className="text-[12px] font-semibold px-5 py-1.5 rounded-lg transition-all" style={{ background: ACCENT_LIGHT, color: "#fff" }}>
              Sign in
            </button>
          </div>
      ...
      {mobileNav && (
        <div className="fixed inset-0 z-40 flex flex-col items-center justify-center gap-7 md:hidden" style={{ background: "rgba(10,12,16,0.98)", backdropFilter: "blur(16px)" }}>
          {[{ label: "Platform", href: "#platform" }, { label: "Doctrine", href: "#doctrine" }, { label: "Capabilities", href: "#capabilities" }, { label: "Who It's For", href: "#buyers" }].map(l => (
          ...
          <button onClick={() => { onSignIn?.(); setMobileNav(false); }} className="mt-4 text-sm font-semibold px-8 py-3 rounded-xl" style={{ background: ACCENT_LIGHT, color: "#fff" }}>
            Sign in
          </button>
        </div>
      )}
      ...
      <section className="relative pt-32 sm:pt-44 pb-24 sm:pb-32 max-w-[1100px] mx-auto px-6">
        ...
        <Reveal delay={180}>
          <p className="text-[17px] sm:text-[19px] leading-[1.8] max-w-[600px] mt-7 mb-10" style={{ color: "rgba(255,255,255,0.32)" }}>
            Terra gives investors, brokers, and portfolio teams a single intelligence surface — from distressed property discovery through ownership analysis, pipeline management, and deal execution.
          </p>
        </Reveal>
      ...
      <section id="platform" className="relative py-24 sm:py-32 px-6">
        <div className="max-w-[720px] mx-auto">
          <Reveal>
            <p className="text-[10px] font-semibold tracking-[0.2em] uppercase mb-6 font-mono" style={{ color: "rgba(255,255,255,0.18)" }}>The thesis</p>
          ...
          <Reveal delay={80}>
            <div className="text-[16px] leading-[2] space-y-6" style={{ color: "rgba(255,255,255,0.3)" }}>
              <p>
                The brokers and investors who consistently win in NYC real estate
                <span style={{ color: "rgba(255,255,255,0.65)" }}> see distress signals first</span>, understand
                <span style={{ color: "rgba(255,255,255,0.65)" }}> ownership structures fastest</span>, and close deals with
                <span style={{ color: "rgba(255,255,255,0.65)" }}> more context than anyone else in the room</span>.
              </p>
              <p>
                Terra replaces the 14 browser tabs, three paid data services, and two hours of morning research that currently stand between you and your first actionable lead of the day.
              </p>
            </div>
          </Reveal>

          <Reveal delay={160}>
            <div className="mt-14 rounded-2xl p-6 sm:p-8" style={{ background: "rgba(45,106,79,0.04)", border: "1px solid rgba(45,106,79,0.10)" }}>
              <div className="flex justify-between items-center mb-6">
                <span className="text-[9px] font-bold tracking-[0.15em] uppercase font-mono" style={{ color: "rgba(255,255,255,0.25)" }}>NYC Distress Snapshot — Demo</span>
                <span className="flex items-center gap-1.5 text-[9px] font-mono" style={{ color: "rgba(64,133,106,0.6)" }}>
                  <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: ACCENT_LIGHT }} />
                  Engine Active
                </span>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                {[
                  ...
                  <div key={d.label} className="text-center">
                    <span className="text-[22px] font-extrabold font-mono block" style={{ color: d.color }}>{d.count}</span>
                    <span className="text-[10px] mt-1 block" style={{ color: "rgba(255,255,255,0.25)" }}>{d.label}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="max-w-[1100px] mx-auto">
          <Reveal>
            <div className="max-w-[560px] mb-14">
              <p className="text-[10px] font-semibold tracking-[0.2em] uppercase mb-4 font-mono" style={{ color: "rgba(255,255,255,0.18)" }}>The Terra Doctrine</p>
              ...
              <p className="text-[15px] leading-[1.85]" style={{ color: "rgba(255,255,255,0.28)" }}>
                Every serious real estate operation moves through the same sequence. Terra structures it into a repeatable, intelligence-driven workflow.
              </p>
            </div>
          </Reveal>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-5 gap-4">
            {doctrine.map((d, i) => (
              <Reveal key={d.phase} delay={i * 60}>
                <div className="p-5 rounded-xl h-full" style={{ background: BG2, border: "1px solid rgba(255,255,255,0.05)" }}>
                  <div className="flex items-center gap-2 mb-4">
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: "rgba(45,106,79,0.10)", border: "1px solid rgba(45,106,79,0.16)" }}>
                      <d.icon size={13} style={{ color: ACCENT_LIGHT }} />
                    </div>
                    <span className="text-[9px] font-bold tracking-[0.12em] uppercase font-mono" style={{ color: "rgba(255,255,255,0.25)" }}>{String(i + 1).padStart(2, "0")}</span>
                  </div>
                  <h3 className="text-[13px] font-bold text-white mb-2">{d.phase}</h3>
                  <p className="text-[12px] leading-[1.75]" style={{ color: "rgba(255,255,255,0.3)" }}>{d.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {capabilities.map((c, i) => (
              <Reveal key={c.title} delay={i * 50}>
                <div className="p-6 rounded-xl h-full" style={{ background: BG2, border: "1px solid rgba(255,255,255,0.05)" }}>
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center mb-4" style={{ background: "rgba(45,106,79,0.10)", border: "1px solid rgba(45,106,79,0.16)" }}>
                    <c.icon size={14} style={{ color: ACCENT_LIGHT }} />
                  </div>
                  <h3 className="text-[14px] font-bold text-white mb-2">{c.title}</h3>
                  <p className="text-[13px] leading-[1.8]" style={{ color: "rgba(255,255,255,0.3)" }}>{c.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
        ...
        <div className="max-w-[1100px] mx-auto">
          <Reveal>
            <div className="max-w-[520px] mb-14">
              <p className="text-[10px] font-semibold tracking-[0.2em] uppercase mb-4 font-mono" style={{ color: "rgba(255,255,255,0.18)" }}>Use Cases</p>
              <h2 className="text-[clamp(1.5rem,3.5vw,2.4rem)] font-bold leading-[1.15] tracking-tight text-white mb-4">
            ...
            {useCases.map((u, i) => (
              <Reveal key={u.title} delay={i * 50}>
                <div className="p-6 rounded-xl h-full" style={{ background: BG2, border: "1px solid rgba(255,255,255,0.05)" }}>
                  ...
                  <p className="text-[12px] leading-[1.8]" style={{ color: "rgba(255,255,255,0.3)" }}>{u.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section id="buyers" className="relative py-24 sm:py-32 px-6">
        <div className="max-w-[1100px] mx-auto">
          <Reveal>
            <div className="max-w-[520px] mb-14">
              <p className="text-[10px] font-semibold tracking-[0.2em] uppercase mb-4 font-mono" style={{ color: "rgba(255,255,255,0.18)" }}>Who It's For</p>
              <h2 className="text-[clamp(1.5rem,3.5vw,2.4rem)] font-bold leading-[1.15] tracking-tight text-white mb-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-px rounded-2xl overflow-hidden" style={{ background: "rgba(255,255,255,0.04)" }}>
            {buyers.map(b => (
              <div key={b.role} className="p-8" style={{ background: BG }}>
                ...
                <p className="text-[13px] leading-[1.85]" style={{ color: "rgba(255,255,255,0.3)" }}>{b.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative py-20 sm:py-24 px-6">
        <div className="max-w-[760px] mx-auto">
          <Reveal>
            <div className="p-8 sm:p-10 rounded-2xl" style={{ background: BG2, border: "1px solid rgba(255,255,255,0.05)" }}>
              <div className="flex items-center gap-3 mb-6">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "rgba(45,106,79,0.10)", border: "1px solid rgba(45,106,79,0.16)" }}>
                  <Shield size={14} style={{ color: ACCENT_LIGHT }} />
                </div>
                <p className="text-[10px] font-semibold tracking-[0.16em] uppercase font-mono" style={{ color: "rgba(255,255,255,0.25)" }}>Trust & Governance</p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {[
                  { label: "Data sourcing", value: "County records, court filings, public registries" },
                  { label: "Access control", value: "Role-based permissions with full audit trail" },
                  { label: "Accuracy", value: "Confidence scoring on every opportunity — no silent guesses" },
                ].map(t => (
                  <div key={t.label}>
                    <p className="text-[10px] font-semibold uppercase tracking-wider mb-1" style={{ color: "rgba(255,255,255,0.22)" }}>{t.label}</p>
                    <p className="text-[13px]" style={{ color: "rgba(255,255,255,0.55)" }}>{t.value}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="max-w-[900px] mx-auto">
          <Reveal>
            <div className="text-center mb-14">
              <p className="text-[10px] font-semibold tracking-[0.2em] uppercase mb-6 font-mono" style={{ color: "rgba(255,255,255,0.18)" }}>Ready to operate at a higher level</p>
              <h2 className="text-[clamp(2rem,5vw,3.5rem)] font-extrabold leading-[1.08] tracking-tight text-white mb-6">
                Three ways in.
              </h2>
              <p className="text-[17px] leading-[1.8] max-w-[560px] mx-auto" style={{ color: "rgba(255,255,255,0.3)" }}>
                Terra is available to qualified operators in three tiers. No free trials. No demo accounts. Real intelligence from day one.
              </p>
            </div>
          </Reveal>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-12">
            {[
              ...
              <div key={t.tier} className="p-6 rounded-2xl flex flex-col" style={{
                ...
                border: t.highlight ? `1px solid rgba(45,106,79,0.18)` : `1px solid rgba(255,255,255,0.05)`,
              }}>
          ...
          <Reveal delay={100}>
            <div className="flex items-center justify-center gap-2 text-center">
              <span className="text-[12px]" style={{ color: "rgba(255,255,255,0.2)" }}>Already have access?</span>
              <button onClick={onSignIn} className="text-[12px] font-semibold" style={{ color: ACCENT_LIGHT }}>Sign in →</button>
            </div>
          </Reveal>
        </div>
      </section>
