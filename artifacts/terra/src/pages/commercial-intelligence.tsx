          ...
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="rounded-xl border border-terra-border bg-terra-surface/40 p-5 animate-pulse">
              ...
              <div className="flex gap-2 mb-3">
                <div className="h-5 w-20 bg-terra-border/50 rounded-full" />
                <div className="h-5 w-16 bg-terra-border/40 rounded-full" />
              </div>
              <div className="h-6 bg-terra-border rounded w-24" />
            </div>
          ))}
        </div>
      ) : activeTab === "properties" ? (
