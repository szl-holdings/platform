...
const PAYLOAD_ID = 'cps-terra-market-anomaly';
...
export default function TerraCpsLaneConsole() {
  ...
  function downloadProofBundle(runId: string) {
    ...
    a.href = url;
    a.download = `risk-memo-proof-${runId.slice(0, 8)}.json`;
    a.click();
    ...
    <div className="space-y-6 animate-fade-in p-6">
      <header className="flex items-end justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-display font-bold text-slate-100">Market-Anomaly Interceptor</h1>
            <span className="text-[10px] font-mono px-2 py-0.5 rounded border border-emerald-500/30 text-emerald-400 bg-emerald-500/10">
              CPS · Real Estate Risk
            </span>
          </div>
          <p className="text-slate-400 text-sm">
            Distress clusters · Fraud indicators · Title-risk alerts — legal-gated transaction protection
          </p>
        </div>
        <div className="flex gap-3 items-center">
          ...
          <button
            ...
            disabled={executing}
            className="flex items-center gap-2 px-4 py-2 rounded text-sm font-medium bg-emerald-600/20 hover:bg-emerald-600/30 border border-emerald-500/30 text-emerald-300 transition-colors disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: 'Legal Gate Pending', value: pendingApprovals.length, color: 'text-amber-400' },
        ].map((stat) => (
      ...
      <div className="flex gap-1 border-b border-slate-700/40">
        {(['runs', 'approvals'] as const).map((tab) => (
          ...
          >
            {tab === 'approvals' ? `Legal Gate${pendingApprovals.length > 0 ? ` (${pendingApprovals.length})` : ''}` : 'Run History'}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {runs.length === 0 && (
            <div className="bg-slate-900/60 border border-slate-700/40 rounded-lg p-8 text-center text-slate-500">
              No runs yet. Click "Run Detection" to scan for market anomalies.
            </div>
          )}
          {runs.map((run) => {
              ...
              <div key={run.id} className="bg-slate-900/60 border border-slate-700/40 rounded-lg overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {run.proofBundle && <FileText className="w-3.5 h-3.5 text-slate-500" />}
                  </div>
                ...
                {expanded && (
                  <div className="border-t border-slate-700/40 p-4 space-y-4">
                    ...
                    {run.decide && (
                      <div>
                        <div className="text-[10px] font-mono text-slate-500 uppercase mb-2">Decision · Acquisition/Risk Memo</div>
                        <div className="text-xs text-slate-300">{run.decide.reasoning}</div>
                        <div className="text-xs text-slate-400 mt-1">
                          Risk level: <span className={cn('font-mono',
                      ...
                      <div className="flex gap-2">
                        ...
                        {proof && (
                          <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 transition-colors"
                          >
                            <Download className="w-3 h-3" /> Download Risk Memo
                          </button>
                        )}
                      </div>
                    )}
        ...
        <div className="space-y-3">
          {pendingApprovals.length === 0 && (
            <div className="bg-slate-900/60 border border-slate-700/40 rounded-lg p-8 text-center text-slate-500">
              No legal gate approvals pending.
            </div>
          )}
