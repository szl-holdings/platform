...
const stepStatusConfig = {
  ...
  pending: {
    color: 'bg-terra-text-muted',
    ring: 'border-terra-border',
    label: 'Pending',
    textColor: 'text-terra-text-muted',
  },
  overdue: {
    color: 'bg-rose-500',
...
function StepCard({ step, index }: { step: TransactionStep; index: number }) {
  ...
  return (
    <div className={cn('rounded-xl border overflow-hidden transition-all', cfg.ring)}>
      <div
        className="flex items-center gap-3 p-4 cursor-pointer hover:bg-terra-surface-hover"
      ...
      >
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-3 mt-0.5">
            ...
            {step.dueDate && (
              <span className="text-[10px] text-terra-text-muted">Due {step.dueDate}</span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          {step.linkedDocuments.length > 0 && (
            <span className="text-[10px] text-terra-text-muted">
              {step.linkedDocuments.length} doc{step.linkedDocuments.length > 1 ? 's' : ''}
            </span>
          )}
          {expanded ? (
          ...
          {step.blockers.length > 0 && (
            <div>
              ...
              {step.blockers.map((b, i) => (
                <div
                  key={i}
                  className="flex items-start gap-2 text-xs text-amber-400 bg-amber-500/5 border border-amber-500/20 rounded px-2 py-1.5"
                >
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {step.linkedDocuments.map((d, i) => (
                  <span
                    key={i}
                    className="text-[10px] px-2 py-0.5 rounded bg-terra-primary/10 text-terra-primary border border-terra-primary/20"
                  >
                    {d}
                  </span>
                ))}
...
function TransactionView({ tx }: { tx: Transaction }) {
  ...
  return (
    <div className="rounded-xl border border-terra-border bg-terra-surface/50 overflow-hidden">
      ...
      <div className="px-6 py-5 border-b border-terra-border">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 text-xs text-terra-text-muted mt-1">
              ...
              {tx.coordinatorName && (
                <>
                  <span>·</span>
                  <span>Coordinator: {tx.coordinatorName}</span>
                </>
              )}
            </div>
          </div>
        ...
        <div className="flex items-center gap-3 mb-3">
          <div className="flex-1 h-2 bg-terra-border rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-terra-primary to-terra-accent rounded-full transition-all"
              style={{ width: `${progress}%` }}
            />
          </div>
          <span className="text-xs font-semibold text-terra-primary flex-shrink-0">
        ...
        <div className="flex items-center gap-3 text-xs">
          {overdue > 0 && (
            <span className="text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded-full">
              {overdue} overdue
            </span>
          )}
          {blocked > 0 && (
          ...
          {tx.titleCompany && (
            <span className="text-terra-text-muted">Title: {tx.titleCompany}</span>
          )}
        </div>
        ...
        {tx.riskFlags.length > 0 && (
          <div className="mt-3 space-y-1">
            {tx.riskFlags.map((flag, i) => (
              <div
                key={i}
                className="flex items-start gap-2 text-xs text-rose-400 bg-rose-500/5 border border-rose-500/20 rounded px-2 py-1.5"
              >
                <AlertTriangle className="w-3 h-3 flex-shrink-0 mt-0.5" />
                {flag}
              </div>
      ...
      {/* Steps */}
      <div className="p-6">
        <p className="text-[10px] font-semibold text-terra-text-muted uppercase tracking-wider mb-4">
          Transaction Steps
        </p>
    ...
    <div className="p-6 space-y-6 overflow-auto">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-sm text-terra-text-secondary mt-1">
          Transaction management — accepted offer to commission payout
        </p>
      </motion.div>
        ...
        {[
          ...
          },
          {
            ...
            value: transactions.reduce(
              (s, t) => s + t.steps.filter((st) => st.status === 'complete').length,
              0,
            ),
            alert: false,
          },
          {
            label: 'Overdue Steps',
            value: transactions.reduce(
              (s, t) => s + t.steps.filter((st) => st.status === 'overdue').length,
              0,
            ),
            alert: true,
          },
          {
            label: 'Risk Flags',
            value: transactions.reduce((s, t) => s + t.riskFlags.length, 0),
            alert: true,
          },
        ].map((m) => (
          <div
