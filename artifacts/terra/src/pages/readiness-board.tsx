...
function PropertyReadinessCard({ twin }: { twin: PropertyTwin }) {
  ...
  const openTasks = twin.diligenceTasks.filter(
  ...
  return (
    <div
      className="rounded-xl border p-5 transition-all duration-200 hover:bg-white/3"
      style={{
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        {[
          ...
          <div
            ...
            style={{
              background: m.warn ? '#c08a2c08' : 'rgba(255,255,255,0.03)',
              border: `1px solid ${m.warn ? '#c08a2c20' : 'rgba(255,255,255,0.04)'}`,
            }}
          >
            <div
              className="text-lg font-bold"
              style={{ color: m.warn ? '#c08a2c' : 'rgba(255,255,255,0.7)' }}
            >
              {m.value}
            </div>
      ...
      {twin.distressSignal !== 'none' && (
        <div
          ...
          style={{ background: '#c04a2a10', border: '1px solid #c04a2a20' }}
        >
...
function UnderwritingFrictionRow({ twin }: { twin: PropertyTwin }) {
  const frictions = [
    ...
    twin.externalDataConnectors.filter((c) => c.status === 'not_connected').length > 0 &&
      'External sources not connected',
        ...
        {frictions.length === 0 ? (
          ...
          frictions.map((f) => (
            <span
              ...
              style={{ background: '#c08a2c15', color: '#c08a2c', border: '1px solid #c08a2c25' }}
            >
              {f}
            </span>
          ))
      ...
      <div className="flex items-center gap-1">
        ...
        >
          <div
            ...
            style={{
              width: `${twin.readinessScore}%`,
              background: getBand(twin.readinessScore).color,
            }}
          />
        </div>
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>
            Portfolio-wide readiness scoring — diligence progress, document status, approvals, and
            distress signals
          </p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <div
          className="rounded-xl border p-4"
          style={{ background: '#40856a08', borderColor: '#40856a25' }}
        >
          <div className="text-xs mb-1" style={{ color: '#40856a' }}>
            Ready
          </div>
          <div className="text-2xl font-bold" style={{ color: '#40856a' }}>
            {ready.length}
          </div>
        </div>
        <div
      ...
      {view === 'board' ? (
        <div className="grid grid-cols-2 gap-4">
          {propertyTwins.map((t) => (
            <PropertyReadinessCard key={t.id} twin={t} />
          ))}
        </div>
      ) : (
        <div
        ...
        >
          <h3 className="text-sm font-semibold mb-4" style={{ color: 'rgba(255,255,255,0.6)' }}>
            Underwriting Friction Analysis
          </h3>
          <div
          ...
          {propertyTwins.map((t) => (
            <UnderwritingFrictionRow key={t.id} twin={t} />
          ))}
          <div
          ...
          >
            ...
            >
              <Plug size={12} />
              <span>
                External data would further enrich friction detection. Connect sources to enable
                zoning, permit, and lien data.
              </span>
            </div>
          </div>
        </div>
