  ...
  return (
    <div
      className={cn(
        ...
        offer.brokerApprovalRequired && !offer.brokerApproved
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-xs text-terra-text-secondary mt-0.5">
              {offer.buyerName} · {offer.agentName}
            </p>
          </div>
      ...
      <div className="p-5">
        <div className="flex items-end gap-3 mb-4">
          ...
          <div className="text-xs">
            ...
            >
              ...
              {formatCurrency(offer.price - offer.listPrice)} vs list
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4 text-xs">
          ...
          <div>
            ...
            <p className="font-semibold text-terra-text capitalize">
            ...
            {offer.contingencies.map((c, i) => (
              <span
                ...
                className={cn(
                  ...
                  c.waived
                    ? 'border-emerald-500/30 text-emerald-400 bg-emerald-500/5 line-through'
                    : 'border-terra-border text-terra-text-secondary',
                )}
              >
        ...
        <div className="flex items-center gap-2 mb-4">
          <span className={cn('text-[10px] px-2 py-0.5 rounded-full', financing.cls)}>
            {financing.label}
          </span>
        ...
        {offer.brokerApprovalRequired && (
          <div className="mb-3">
            <ApprovalChip
              approved={offer.brokerApproved}
              label={offer.brokerApproved ? 'Broker Approved' : 'Awaiting Broker Approval'}
            />
          </div>
        )}
          ...
          <div
            className={cn(
              ...
              expiring
                ...
                : 'bg-terra-surface text-terra-text-muted border border-terra-border',
            )}
          >
    ...
    <div className="rounded-xl border border-terra-border bg-terra-surface/50 overflow-hidden mb-6">
      <div className="px-5 py-3.5 border-b border-terra-border">
        ...
        <p className="text-xs text-terra-text-muted mt-0.5">
          {listingOffers.length} offers received
        </p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-terra-border">
              <th className="text-left py-3 px-4 text-[10px] font-semibold text-terra-text-muted">
                Metric
              </th>
              {listingOffers.map((o) => (
                <th
                  key={o.id}
                  className="text-left py-3 px-4 text-[10px] font-semibold text-terra-text-muted"
                >
                  {o.buyerName}
                </th>
              ))}
          ...
          <tbody>
            {[
              { label: 'Offer Price', key: (o: Offer) => formatCurrency(o.price) },
              {
                label: 'vs. List',
                key: (o: Offer) => (
                  <span className={o.price >= o.listPrice ? 'text-emerald-400' : 'text-rose-400'}>
                    {o.price >= o.listPrice ? '+' : ''}
                    {formatCurrency(o.price - o.listPrice)}
                  </span>
                ),
              },
              ...
              },
              {
                ...
                key: (o: Offer) => <ConfidenceBadge value={o.closeConfidence / 100} />,
              },
              {
                ...
                key: (o: Offer) => (
                  <span
                    className={
                      o.financingStrength === 'strong'
                        ? 'text-emerald-400'
                        : o.financingStrength === 'moderate'
                          ? 'text-amber-400'
                          : 'text-rose-400'
                    }
                  >
              ...
              <tr
                key={row.label}
                className="border-b border-terra-border/50 hover:bg-terra-surface-hover"
              >
                <td className="py-2.5 px-4 text-xs font-medium text-terra-text-muted">
                  {row.label}
                </td>
                {listingOffers.map((o) => (
                  <td key={o.id} className="py-2.5 px-4 text-xs text-terra-text">
                    {row.key(o)}
                  </td>
                ))}
              </tr>
...
export default function OffersPage() {
  ...
  const listingsWithMultiple = [
    ...new Set(offers.filter((o) => o.direction === 'incoming').map((o) => o.listingAddress)),
  ].filter(
    (listing) =>
    ...
    <div className="p-6 space-y-6 overflow-auto">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-sm text-terra-text-secondary mt-1">
          Incoming and outgoing offers, comparison, expiration tracking, and approvals
        </p>
      </motion.div>
      ...
      {brokerQueue.length > 0 && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-amber-500/30 bg-amber-500/10">
          ...
          <p className="text-sm text-amber-400 font-semibold">
            {brokerQueue.length} offer{brokerQueue.length > 1 ? 's' : ''} awaiting broker approval
          </p>
        </div>
      )}
      {expiring.length > 0 && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-rose-500/30 bg-rose-500/10">
          ...
          <p className="text-sm text-rose-400 font-semibold">
            {expiring.length} offer{expiring.length > 1 ? 's' : ''} expiring within 24 hours
          </p>
        </div>
      )}
        ...
        {(() => {
          ...
          <div
            ...
            className={cn(
              ...
              m.alert && m.value > 0 ? 'border-rose-500/30' : 'border-terra-border',
            )}
          >
            ...
            <p
              className={cn(
                ...
                m.alert && m.value > 0 ? 'text-rose-400' : 'text-terra-text',
              )}
            >
              {m.value}
            </p>
      ...
      {listingsWithMultiple.map((listing) => (
        <ComparisonTable
          key={listing}
          listing={listing}
          listingOffers={offers.filter(
            (o) => o.listingAddress === listing && o.direction === 'incoming',
          )}
        />
      ))}
