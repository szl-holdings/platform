...
function StepCard({ step, index }: { step: any; index: number }) {
  ...
  const StatusIcon = step.passed ? (step.flag ? AlertTriangle : CheckCircle) : XCircle;
    ...
    <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${statusColor}25` }}>
      ...
      >
        <div className="flex flex-col items-center gap-1 flex-shrink-0 mt-0.5">
          <div
            className="text-[9px] font-mono w-5 h-5 rounded-full flex items-center justify-center font-bold"
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {step.flag && (
              <span
                ...
                style={{ background: '#c8a06020', color: '#c8a060' }}
              >
                Flag
              </span>
            )}
...
export default function UnderwritingCopilotPage() {
  ...
  const verdictCfg = result
    ? (VERDICT_CONFIG[result.verdict] ?? VERDICT_CONFIG['HOLD FOR REVIEW'])
    : null;
      ...
      <div className="flex items-start gap-3 mb-6">
        <div>
          ...
          <p className="text-sm" style={{ color: '#94a3b8' }}>
            Governed underwriting flow — planner walks each analysis step, verifier checks each
            result with citation-backed evidence.
          </p>
        </div>
      </div>
              ...
              {[
                {
                  ...
                  placeholder: 'e.g. 245 Park Ave South',
                },
                ...
                <div key={f.key}>
                  <label
                    className="block text-[10px] mb-1 font-medium uppercase tracking-wider"
                    style={{ color: 'rgba(255,255,255,0.4)' }}
                  >
                    {f.label}
                  </label>
            ...
            >
              ...
              <div className="space-y-1.5">
                {[
                  ...
                  {
                    label: 'DSCR',
                    value: `${result.metrics.dscr}x`,
                    warn: result.metrics.dscr < 1.25,
                  },
                  {
                    label: 'Cash-on-Cash',
                    value: `${result.metrics.cashOnCash.toFixed(2)}%`,
                    warn: result.metrics.cashOnCash < 7,
                  },
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{ color: m.warn ? '#c8a060' : '#e8edf8' }}
                    >
                      {m.value}
                    </span>
                  </div>
          ...
          {!result && !mutation.isPending && (
            ...
            >
              ...
              <div className="text-sm" style={{ color: 'rgba(255,255,255,0.3)' }}>
                Configure deal parameters and run the copilot
              </div>
              <div className="text-[11px] mt-1" style={{ color: 'rgba(255,255,255,0.2)' }}>
                Planner+verifier will walk each underwriting step with citations
              </div>
            </div>
          )}
          ...
          {result && verdictCfg && (
            <>
              <div
                className="rounded-xl p-4"
                style={{
                  background: `${verdictCfg.color}10`,
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-lg font-bold" style={{ color: verdictCfg.color }}>
                      {result.verdict}
                    </div>
                ...
                {result.flags?.length > 0 && (
                  <div
                    className="mt-3 pt-3"
                    style={{ borderTop: `1px solid ${verdictCfg.color}20` }}
                  >
                    <div
                    ...
                    {result.flags.map((f: any, i: number) => (
                      ...
                      >
                        <AlertTriangle className="w-3 h-3 flex-shrink-0 mt-0.5" />
                        <span>
                          <span className="font-semibold">{f.label}:</span> {f.flag}
                        </span>
                      </div>
                    ))}
                  </div>
