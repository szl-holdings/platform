...
const APPLICANTS: Applicant[] = [
  {
    ...
    flags: [
      { type: "info", text: "Business entity guaranty requested — requires personal guaranty from S. Thornton as GP" },
    ],
    notes: "Premium applicant. Strong financials, long operating history. Recommend approve with standard lease terms. Entity guaranty docs to be collected before execution.",
  },
  {
    id: "app-002",
    ...
    rentalHistory: "4 years at Westside Medical Park — 1 late payment cured, minor TI dispute resolved",
    backgroundClear: true,
    criminalFlags: [],
    radarScores: [
    ...
    flags: [
      ...
      { type: "warning", text: "Judgment (2023 medical billing dispute, $14K) — resolved but on record" },
      { type: "info", text: "Recommend additional 2 months security deposit + personal guaranty from all principals" },
    ],
    notes: "Qualified with conditions. Healthcare tenant adds value (exclusive use clause). Require 3-month security deposit, personal guarantees from all 3 principals, and quarterly financial reporting for Year 1.",
  },
  {
    id: "app-003",
    ...
    rentalHistory: "1 eviction on record (2024). Single landlord reference — mixed feedback on property upkeep.",
    ...
    flags: [
      ...
      { type: "error", text: "Income not verified — tax returns and bank statements not submitted" },
      { type: "warning", text: "Business entity formed 2025 — insufficient operating history" },
    ],
    notes: "Recommend decline. Multiple disqualifying factors: prior eviction, unverified income below threshold, active collections, and bankruptcy within 5 years. Risk exposure too high for standard lease terms.",
  },
];
  ...
  return (
    <div className="relative w-24 h-24 flex items-center justify-center">
      <svg className="absolute inset-0" viewBox="0 0 96 96">
        <circle cx="48" cy="48" r={r} fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="6" strokeDasharray={`${circ * 0.75} ${circ}`} strokeDashoffset={-circ * 0.125} strokeLinecap="round" />
        <circle cx="48" cy="48" r={r} fill="none" stroke={color} strokeWidth="6" strokeDasharray={`${dash} ${circ}`} strokeDashoffset={-circ * 0.125} strokeLinecap="round" style={{ transition: "stroke-dasharray 0.5s ease" }} />
      </svg>
      <div className="text-center">
        <p className="text-2xl font-black font-mono" style={{ color }}>{score}</p>
        <p className="text-[8px] uppercase tracking-wider" style={{ color: DS.text.muted }}>Score</p>
      </div>
    </div>
  );
}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          {applicants.map(app => {
            ...
            return (
              <motion.div
                ...
                style={{ borderColor: selectedId === app.id ? DS.accent.gold : DS.border, background: selectedId === app.id ? "rgba(184,148,60,0.04)" : DS.surface }}
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="min-w-0">
                    <p className="text-xs font-semibold truncate" style={{ color: DS.text.primary }}>{app.name}</p>
                    <p className="text-[9px] mt-0.5 truncate" style={{ color: DS.text.tertiary }}>{app.targetUnit}</p>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    <span className="text-[9px] font-bold px-1.5 py-0.5 rounded" style={{ color: r.color, background: `${r.color}12` }}>{r.label}</span>
                    <span className="text-sm font-bold font-mono" style={{ color: scoreColor }}>{app.overallScore}</span>
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    <p className="text-[8px] uppercase tracking-wider" style={{ color: DS.text.muted }}>Credit</p>
                    <p className="text-[10px] font-mono" style={{ color: app.creditScore >= 720 ? DS.accent.green : app.creditScore >= 650 ? DS.accent.gold : DS.accent.red }}>{app.creditScore}</p>
                  </div>
                  <div>
                    <p className="text-[8px] uppercase tracking-wider" style={{ color: DS.text.muted }}>Income RTI</p>
                    <p className="text-[10px] font-mono" style={{ color: app.rentToIncomeRatio <= 12 ? DS.accent.green : DS.accent.red }}>{app.rentToIncomeRatio.toFixed(1)}%</p>
                  </div>
                  <div>
                    <p className="text-[8px] uppercase tracking-wider" style={{ color: DS.text.muted }}>Evictions</p>
                    <p className="text-[10px] font-mono" style={{ color: app.priorEvictions === 0 ? DS.accent.green : DS.accent.red }}>{app.priorEvictions}</p>
                  </div>
                </div>
              </motion.div>
            );
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="rounded-xl border p-4" style={{ borderColor: DS.border, background: DS.surface }}>
            <div className="flex items-start gap-4 mb-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] mt-0.5" style={{ color: DS.text.tertiary }}>{selected.targetUnit}</p>
                <p className="text-[10px] mt-0.5" style={{ color: DS.text.muted }}>
                  ${selected.proposedRent.toLocaleString()}/mo · {selected.leaseTermMonths}-month term
                </p>
                <div className="flex items-center gap-2 mt-2">
                  <RecIcon className="w-4 h-4" style={{ color: rec.color }} />
                  <span className="text-sm font-bold" style={{ color: rec.color }}>Recommendation: {rec.label}</span>
                </div>
              </div>
              <div className="w-32 h-28 shrink-0">
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart data={selected.radarScores} cx="50%" cy="50%" outerRadius="70%">
                    ...
                    <Radar name="Score" dataKey="score" stroke={rec.color} fill={rec.color} fillOpacity={0.15} />
                  </RadarChart>
                </ResponsiveContainer>
              </div>
            </div>

            <p className="text-[10px] leading-relaxed p-3 rounded-lg" style={{ color: DS.text.secondary, background: "rgba(255,255,255,0.02)", border: `1px solid ${DS.border}` }}>
              {selected.notes}
            </p>
          </div>
          ...
          {selected.flags.length > 0 && (
            <div className="rounded-xl border overflow-hidden" style={{ borderColor: DS.border, background: DS.surface }}>
              <div className="flex items-center gap-2 px-4 py-2.5 border-b" style={{ borderColor: DS.border }}>
                ...
                <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: `${DS.accent.gold}99` }}>Flags & Review Items</span>
              </div>
              <div className="divide-y" style={{ borderColor: DS.border }}>
                {selected.flags.map((f, i) => (
                  <div key={i} className="flex items-start gap-3 px-4 py-2.5">
                    <span className="mt-0.5 text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0" style={{ color: f.type === "error" ? DS.accent.red : f.type === "warning" ? DS.accent.gold : DS.accent.blue, background: `${f.type === "error" ? DS.accent.red : f.type === "warning" ? DS.accent.gold : DS.accent.blue}12` }}>
                      {f.type.toUpperCase()}
                    </span>
                    <p className="text-[10px] leading-relaxed" style={{ color: DS.text.secondary }}>{f.text}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl border p-4" style={{ borderColor: DS.border, background: DS.surface }}>
              <p className="text-[10px] font-bold uppercase tracking-wider mb-3" style={{ color: DS.text.muted }}>Credit & Financial</p>
              {[
                { label: "Credit Score", value: selected.creditScore.toString(), color: selected.creditScore >= 720 ? DS.accent.green : selected.creditScore >= 650 ? DS.accent.gold : DS.accent.red },
                { label: "Annual Income", value: fmt(selected.annualIncome), color: DS.text.secondary },
                { label: "Income Verified", value: selected.incomeVerified ? "Yes" : "Not Verified", color: selected.incomeVerified ? DS.accent.green : DS.accent.red },
                { label: "Rent-to-Income", value: `${selected.rentToIncomeRatio.toFixed(1)}%`, color: selected.rentToIncomeRatio <= 12 ? DS.accent.green : DS.accent.red },
                { label: "Bankruptcies", value: selected.bankruptcies.toString(), color: selected.bankruptcies === 0 ? DS.accent.green : DS.accent.red },
                { label: "Judgments", value: selected.judgments.toString(), color: selected.judgments === 0 ? DS.accent.green : DS.accent.red },
              ].map(row => (
                <div key={row.label} className="flex items-center justify-between py-1.5" style={{ borderTop: `1px solid ${DS.border}` }}>
                  <span className="text-[9px] uppercase tracking-wider" style={{ color: DS.text.muted }}>{row.label}</span>
            ...
            <div className="rounded-xl border p-4" style={{ borderColor: DS.border, background: DS.surface }}>
              <p className="text-[10px] font-bold uppercase tracking-wider mb-3" style={{ color: DS.text.muted }}>Rental History & Background</p>
              {[
                { label: "Prior Evictions", value: selected.priorEvictions.toString(), color: selected.priorEvictions === 0 ? DS.accent.green : DS.accent.red },
                { label: "Landlord Score", value: `${selected.priorLandlordScore.toFixed(1)}/10`, color: selected.priorLandlordScore >= 8 ? DS.accent.green : selected.priorLandlordScore >= 6 ? DS.accent.gold : DS.accent.red },
                { label: "References", value: `${selected.priorLandlordRefs} provided`, color: DS.text.secondary },
                { label: "Background", value: selected.backgroundClear ? "Clear" : "Flags Noted", color: selected.backgroundClear ? DS.accent.green : DS.accent.red },
                { label: "Employment", value: selected.employmentStatus, color: DS.text.secondary },
                { label: "Tenure", value: `${selected.employmentYears}yr`, color: selected.employmentYears >= 3 ? DS.accent.green : DS.accent.gold },
              ].map(row => (
                <div key={row.label} className="flex items-center justify-between py-1.5" style={{ borderTop: `1px solid ${DS.border}` }}>
                  <span className="text-[9px] uppercase tracking-wider" style={{ color: DS.text.muted }}>{row.label}</span>
                  <span className="text-[10px] font-bold font-mono" style={{ color: row.color }}>{row.value}</span>
