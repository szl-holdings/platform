...
import { CheckCircle, ArrowRight, MapPin, BarChart3, Building2, X } from "lucide-react";
...
const tiers = [
  ...
  },
  {
    ...
    monthly: 899,
    annual: 749,
    description: "For acquisition teams and active operators running a systematic distress strategy.",
    icon: BarChart3,
    cta: "Start Free Trial",
    trialDays: 14,
    highlight: true,
...
export default function TerraPerricingPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: BG, color: "rgba(255,255,255,0.88)" }}>
      <ContactModal isOpen={contactOpen} onClose={closeContact} type="demo" app="terra" title="Request Enterprise Access" subtitle="For institutional investors and PE firms." />

      <div className="border-b" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <a href="/terra/" className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: `${ACCENT}30`, border: `1px solid ${ACCENT}40` }}>
          ...
          <a href="/terra/" className="text-[12px]" style={{ color: "rgba(255,255,255,0.4)" }}>← Back to Platform</a>
        </div>
      </div>

      <div className="max-w-7xl mx-auto px-6 py-20 text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[11px] font-medium mb-6" style={{ background: `${ACCENT}20`, color: ACCENT_LIGHT, border: `1px solid ${ACCENT}40` }}>
        ...
        <p className="text-[15px] max-w-xl mx-auto mb-10" style={{ color: "rgba(255,255,255,0.5)" }}>
          From individual investors to institutional capital. Distress intelligence built for operators.
        </p>
        ...
        <div className="inline-flex items-center gap-1 p-1 rounded-full mb-16" style={{ background: "rgba(255,255,255,0.06)" }}>
          ...
          >
            Annual
            <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold" style={{ background: `${ACCENT}30`, color: ACCENT_LIGHT }}>Save 17%</span>
          </button>
        </div>
          ...
          {tiers.map((tier) => {
            ...
            return (
              <div
                ...
                style={{
                  background: tier.highlight ? `linear-gradient(135deg, ${ACCENT}18, ${ACCENT}0a)` : "rgba(255,255,255,0.03)",
                  border: tier.highlight ? `1px solid ${ACCENT}40` : "1px solid rgba(255,255,255,0.07)",
                }}
              >
                {tier.highlight && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider" style={{ background: ACCENT_LIGHT, color: "#fff" }}>
                    Most Popular
                  </div>
                )}

                <div className="flex items-center gap-3 mb-4">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: tier.highlight ? `${ACCENT}30` : "rgba(255,255,255,0.06)" }}>
                    <Icon className="w-4 h-4" style={{ color: tier.highlight ? ACCENT_LIGHT : "rgba(255,255,255,0.5)" }} />
                  </div>
                ...
                <div className="space-y-2.5 flex-1">
                  {tier.features.map((f) => (
                    <div key={f} className="flex items-start gap-2.5 text-[13px]" style={{ color: "rgba(255,255,255,0.7)" }}>
                      <CheckCircle className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: ACCENT_LIGHT }} />
                      {f}
                    </div>
                  ))}
                  {tier.notIncluded.map((f) => (
                    <div key={f} className="flex items-start gap-2.5 text-[13px]" style={{ color: "rgba(255,255,255,0.22)" }}>
        ...
        <div className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-8 text-center max-w-3xl mx-auto">
          {[
            ...
            { label: "NYC-first, expanding", desc: "All 5 boroughs covered. National expansion in 2026." },
            { label: "No per-deal fees", desc: "One flat subscription. No transaction or AUM charges." },
          ].map((item) => (
            <div key={item.label}>
