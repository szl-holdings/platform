  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center justify-between mb-2">
        <p className="text-[10px] font-semibold text-terra-text-muted uppercase tracking-wider">{label}</p>
        <div className={cn("p-1.5 rounded-lg", color)}>
          <Icon className="w-3.5 h-3.5 text-white" />
        </div>
      </div>
      <p className={cn("text-2xl font-display font-bold", highlight ? "text-terra-primary" : "text-terra-text")}>{value}</p>
    ...
    <div className="p-6 space-y-6 overflow-auto">
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-6 text-center">
          <p className="text-sm text-red-400">Failed to load broker overview. Ensure the API is running.</p>
        </div>
      ) : (
        <>
          <motion.div
          ...
          >
            <MetricCard label="Distress Opportunities" value={metrics.totalDistressOpportunities ?? 0} sub="Active in engine" icon={Flame} color="bg-orange-500" highlight />
            <MetricCard label="Saved Opportunities" value={metrics.savedOpportunities ?? 0} sub="Investor queue" icon={BookmarkIcon} color="bg-blue-500" />
            <MetricCard label="Leads in CRM" value={metrics.totalLeads ?? 0} sub="Active leads" icon={Users} color="bg-violet-500" />
            <MetricCard label="Converted Leads" value={metrics.convertedLeads ?? 0} sub={`${metrics.leadConversionRate ?? 0}% conversion rate`} icon={CheckCircle} color="bg-emerald-500" />
            <MetricCard label="Active Deals" value={metrics.totalDeals ?? 0} sub="In pipeline" icon={Handshake} color="bg-amber-500" />
            <MetricCard label="Closed Deals" value={metrics.closedDeals ?? 0} sub="All time" icon={Award} color="bg-terra-primary" />
            <MetricCard label="Pipeline Conversion" value={`${metrics.leadConversionRate ?? 0}%`} sub="Lead → Converted" icon={Target} color="bg-rose-500" />
            <MetricCard label="Borough Coverage" value={topBoroughs.length} sub="Active boroughs" icon={MapPin} color="bg-cyan-500" />
          </motion.div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
              ...
              {topBoroughs.length === 0 ? (
                ...
                <div className="space-y-1">
                  {topBoroughs.map((b: any) => (
                    <div key={b.borough}>
                      <BarRow label={b.borough} value={b.count} max={maxBoroughCount} color="bg-terra-primary" />
                      <div className="ml-28 -mt-0.5 mb-1 text-[9px] text-terra-text-muted font-mono">avg score {b.avgScore}</div>
                    </div>
                  ))}
                </div>
              )}
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-display font-bold text-terra-text">Score Distribution</h2>
                ...
                <div className="space-y-3">
                  {scoreDist.map((s: any) => (
                    <div key={s.range} className="flex items-center gap-3">
                      <span className={cn("text-xs font-mono font-bold w-16 flex-shrink-0", SCORE_TEXT_COLORS[s.range] ?? "text-terra-text-muted")}>{s.range}</span>
                      <div className="flex-1 h-5 bg-terra-border rounded overflow-hidden">
                        <div
                          className={cn("h-full rounded transition-all flex items-center pl-2", SCORE_COLORS[s.range] ?? "bg-slate-500")}
                          style={{ width: `${Math.max(4, Math.round((s.count / maxScoreCount) * 100))}%` }}
                        >
          ...
          <motion.div
            ...
            className="rounded-xl border border-terra-border bg-terra-surface/50 p-5"
          >
            <h2 className="text-sm font-display font-bold text-terra-text mb-4">Conversion Funnel</h2>
            <div className="flex items-center gap-2 flex-wrap">
              {[
                { label: "Distress Properties", value: metrics.totalDistressOpportunities ?? 0, color: "bg-orange-500/20 border-orange-500/30 text-orange-400" },
                { label: "→", value: null, color: "" },
                { label: "Leads", value: metrics.totalLeads ?? 0, color: "bg-blue-500/20 border-blue-500/30 text-blue-400" },
                { label: "→", value: null, color: "" },
                { label: "Converted", value: metrics.convertedLeads ?? 0, color: "bg-violet-500/20 border-violet-500/30 text-violet-400" },
                { label: "→", value: null, color: "" },
                { label: "Deals", value: metrics.totalDeals ?? 0, color: "bg-amber-500/20 border-amber-500/30 text-amber-400" },
                { label: "→", value: null, color: "" },
                { label: "Closed", value: metrics.closedDeals ?? 0, color: "bg-emerald-500/20 border-emerald-500/30 text-emerald-400" },
              ].map((item, i) =>
                item.value === null ? (
                  <span key={i} className="text-terra-text-muted text-lg font-light">{item.label}</span>
                ) : (
                  <div key={i} className={cn("border rounded-xl px-4 py-3 flex-1 min-w-[100px] text-center", item.color)}>
