...
const DILIGENCE_CHECKLISTS: Record<
  string,
  {
    item: string;
...
> = {
  'prop-001': [
    {
      item: 'Title report reviewed',
      status: 'complete',
      assignee: 'Legal — M. Osei',
      note: 'Clear title, no liens',
    },
    {
      item: 'Phase I Environmental',
      status: 'complete',
      assignee: 'Environmental — K. Walsh',
      note: 'No RECs identified',
    },
    {
      item: 'Structural inspection',
      status: 'complete',
      assignee: 'Engineering — B. Park',
      note: 'Minor deferred maintenance only',
    },
    {
      item: 'Rent roll verification',
      status: 'in-progress',
      assignee: 'Asset Mgmt — D. Kim',
      due: '2026-04-10',
    },
    {
      item: 'Lease abstract review',
      status: 'in-progress',
      assignee: 'Legal — M. Osei',
      due: '2026-04-12',
    },
    {
      item: 'HVAC/MEP assessment',
      status: 'flagged',
      assignee: 'Engineering — B. Park',
      note: 'HVAC replacement may require $500K capex',
    },
  ...
  'prop-007': [
    ...
    },
    {
      ...
      status: 'flagged',
      assignee: 'Asset Mgmt — D. Kim',
      note: 'Delinquency cluster in units 4B–4F',
    },
    {
      item: 'Operating statements (T12)',
      status: 'in-progress',
      assignee: 'Finance — J. Okafor',
      due: '2026-04-05',
    },
    {
      item: 'Loan payoff quote',
      status: 'complete',
      assignee: 'Capital Markets — R. Torres',
      note: 'Maturity 9/14/26 — refinance urgency high',
    },
    {
      item: 'Lender approval / consent',
      status: 'pending',
      assignee: 'Legal — M. Osei',
      due: '2026-04-25',
    },
    {
      item: 'Remediation cost estimate',
      status: 'flagged',
      assignee: 'Engineering — B. Park',
      note: 'Deferred maintenance: ~$2.1M estimate',
    },
  ],
};
...
> = {
  'prop-007': [
    {
      id: 'act-001',
      issue: 'Occupancy at 68.4% — 30 units vacant',
      severity: 'critical',
      owner: 'D. Kim',
      ownerRole: 'Asset Mgmt',
      due: '2026-04-15',
      status: 'in-progress',
      action: 'Activate leasing incentive program; engage Compass multifamily team',
    },
    {
      id: 'act-002',
      issue: 'Sterling Design Studio — 45 days past due, $6,400',
      severity: 'critical',
      owner: 'T. Allen',
      ownerRole: 'Risk & Collections',
      due: '2026-04-07',
      status: 'open',
      action: 'Demand letter sent; escalate to eviction counsel if unpaid by Apr 7',
    },
    {
      id: 'act-003',
      issue: 'Loan maturity Sept 2026 — DSCR at 0.94x',
      severity: 'critical',
      owner: 'R. Torres',
      ownerRole: 'Capital Markets',
      due: '2026-05-01',
      status: 'open',
      action: 'Engage lender for maturity extension; simultaneously market for refi',
    },
    {
      id: 'act-004',
      issue: 'Deferred maintenance estimate $2.1M',
      severity: 'high',
      owner: 'B. Park',
      ownerRole: 'Engineering',
      due: '2026-04-30',
      status: 'open',
      action: 'Complete scope + bid by Apr 30; include in lender remediation plan',
    },
  ],
  'prop-005': [
    {
      id: 'act-005',
      issue: 'Retail occupancy 78.1% — 7 units vacant',
      severity: 'high',
      owner: 'D. Kim',
      ownerRole: 'Asset Mgmt',
      due: '2026-04-20',
      status: 'open',
      action: 'Tenant incentive program — 3 months free rent for 5+ year leases',
    },
    {
      id: 'act-006',
      issue: 'Luna Boutique lease expiring Jun 2026',
      severity: 'medium',
      owner: 'M. Osei',
      ownerRole: 'Legal',
      due: '2026-05-01',
      status: 'open',
      action: 'Send renewal proposal with updated market terms',
    },
  ],
  'prop-001': [
    {
      id: 'act-007',
      issue: 'HVAC Building B overdue maintenance',
      severity: 'medium',
      owner: 'R. Torres',
      ownerRole: 'Facilities',
      due: '2026-04-05',
      status: 'in-progress',
      action: 'Vendor contracted; work order #WO-2026-0847 active',
    },
    {
      id: 'act-008',
      issue: 'Horizon Tech Labs lease expires May 2026',
      severity: 'medium',
      owner: 'M. Osei',
      ownerRole: 'Legal',
      due: '2026-04-15',
      status: 'open',
      action: 'Schedule renewal conversation; assess market rate delta',
    },
  ],
};
...
const SOURCE_LABELS: Record<string, { source: string; freshness: string; confidence: string }> = {
  'prop-001': {
    source: 'Internal → Asset Management System',
...
function FreshnessTag({ label, confidence }: { label: string; confidence?: string }) {
  ...
  return (
    <span
      className="inline-flex items-center gap-1.5 text-[9px] px-2 py-0.5 rounded-full font-mono"
      style={{
...
type DetailTab = 'overview' | 'ownership' | 'diligence' | 'actions' | 'atlas' | 'why-now';
...
export default function PropertyDetailPage() {
  ...
  const loadActionItems = useCallback(async (propertyId: string) => {
    ...
    try {
      const data = await gqlFetch<{ terraActionItems: GqlTerraActionItem[] }>(GQL_ACTION_ITEMS, {
        propertyId,
      });
      if (data.terraActionItems.length === 0) {
        const seeded = await gqlFetch<{ seedTerraActionItems: GqlTerraActionItem[] }>(
          GQL_SEED_ACTIONS,
          { propertyId },
        );
  ...
  const updateActionStatus = useCallback(
    async (id: string, status: string, propertyId: string) => {
      setUpdatingActionId(id);
      try {
        await gqlFetch<{ updateTerraActionItem: GqlTerraActionItem }>(GQL_UPDATE_ACTION, {
          id,
          status,
        });
  ...
  const actionItems: GqlTerraActionItem[] =
    liveActionItems ??
    staticActionItems.map((a) => ({
      id: a.id,
      externalId: a.id,
  ...
  const tabs: { id: DetailTab; label: string; icon: LucideIcon; count?: number }[] = [
    ...
    { id: 'ownership', label: 'Ownership & Debt', icon: Shield },
    {
      ...
      label: 'Diligence',
      icon: FileText,
      count: diligence.filter((d) => d.status === 'flagged').length || undefined,
    },
    {
      id: 'actions',
      label: 'Action Routing',
      icon: Target,
      count: actionItems.filter((a) => a.status !== 'resolved').length || undefined,
    },
  ...
  return (
    <div className="space-y-4 overflow-auto">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <Link href="/dashboard">
          <span
            className="inline-flex items-center gap-1 text-sm mb-4 cursor-pointer transition-colors"
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span
                className={cn(
                  'text-[10px] font-semibold px-2.5 py-0.5 rounded-full uppercase tracking-wide border',
                  status.color,
                  status.border,
                )}
              >
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={async () => {
                setDownloading(true);
                const distressScore =
                  property.status === 'critical' ? 78 : property.status === 'watch' ? 45 : 18;
                const tenantDelinquency = propertyTenants.filter(
                  (t) => t.status === 'delinquent',
                ).length;
                ...
                try {
                  await downloadPropertyPDF(property as unknown as Record<string, unknown>, {
                    distressScore,
                    investmentThesis,
                    distressFactors,
                  });
                } catch {
                  setDownloadError('PDF generation failed.');
                } finally {
        ...
        {tabs.map((tab) => (
          ...
          >
            ...
            {tab.count != null && tab.count > 0 && (
              <span
                className="ml-0.5 text-[9px] font-bold px-1 py-0.5 rounded-full"
            ...
            >
                  ...
                  <PieChart>
                    ...
                    >
                      {occupancyData.map((_, idx) => (
                        <Cell key={idx} fill={OCCI_COLORS[idx]} />
                      ))}
                    </Pie>
                  </PieChart>
                </ResponsiveContainer>
              ...
              <div className="text-center -mt-2">
                ...
                <p className="text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.3)' }}>
                  ...
                  {Math.round(property.units * (1 - property.occupancy / 100))} vacant
                </p>
              </div>
            </motion.div>
          </div>
          ...
          >
            ...
            {propertyTenants.length > 0 ? (
                    ...
                    {propertyTenants.map((tenant, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-2" style={{ color: 'rgba(255,255,255,0.4)' }}>
                            {tenant.unit}
                          </td>
                          <td className="py-2 font-mono font-bold" style={{ color: '#b8943c' }}>
                          ...
                          <td
                            ...
                            style={{
                              color:
                                new Date(tenant.leaseEnd) < new Date(Date.now() + 365 * 86400000)
                                  ? '#f59e0b'
              ...
              <p className="text-sm text-center py-4" style={{ color: 'rgba(255,255,255,0.3)' }}>
                No lease documents abstracted — upload via Lease Abstraction module
              </p>
            )}
          </motion.div>
          ...
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {[
                ...
                {
                  href: `/why-this-property/${property.id}`,
                  icon: Flame,
                  label: 'Why This Property Now',
                  desc: 'Explainable distress score · ownership chain · financing stress · investment memo',
                  badge: 'HPD · ACRIS · ECB · DOB',
                  color: '#ef4444',
                },
              ].map((mod) => {
                const Icon = mod.icon;
              ...
              <div className="space-y-2">
                {propertyAlerts.map((alert) => (
                  <div
                    key={alert.id}
                    className={cn(
                      'p-3 rounded-lg border border-l-2',
                      alert.severity === 'high'
                        ? 'border-l-rose-500'
          ...
          {ownership ? (
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <p className="text-xs text-white/70">
                          {new Date(ownership.lastTransfer).toLocaleDateString()}
                        </p>
                      </div>
                      <div>
                        <p
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        ...
                        {
                          label: 'Maturity Date',
                          value: new Date(ownership.maturityDate).toLocaleDateString('en-US', {
                            month: 'short',
                            day: 'numeric',
                            year: 'numeric',
                          }),
                        },
                        {
                          label: 'LTV',
                          value: ownership.ltv,
                          alert: parseFloat(ownership.ltv) > 70,
                        },
                        {
                          label: 'DSCR',
                          value: ownership.dscr,
                          alert: parseFloat(ownership.dscr) < 1.0,
                        },
                      ].map((m) => (
                        <div
                          key={m.label}
                        ...
                        >
                          ...
                          <p
                            ...
                            style={{ color: m.alert ? '#ef4444' : 'white' }}
                          >
                            {m.value}
                          </p>
                        </div>
                    ...
                    {parseFloat(ownership.dscr) < 1.0 && (
                      ...
                      >
                        ...
                        <p className="text-[10px]" style={{ color: 'rgba(239,68,68,0.8)' }}>
                          DSCR below 1.0x — debt service not covered by NOI. Lender covenant breach
                          risk. Immediate capital plan required.
                        </p>
                      </div>
                    )}
                    {new Date(ownership.maturityDate).getTime() - new Date('2026-04-02').getTime() <
                      1000 * 60 * 60 * 24 * 180 && (
                      <div
                        className="flex items-start gap-2 p-2.5 rounded-lg"
                        style={{
            ...
            >
              ...
              <p className="text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>
                Ownership data not available for this asset
              </p>
              <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.2)' }}>
                Request title search to populate ownership context
              </p>
            </div>
          )}
        </motion.div>
          ...
          {diligence.length > 0 ? (
            ...
            >
              ...
              >
                ...
                >
                  <span>
                    {diligence.filter((d) => d.status === 'complete').length}/{diligence.length}{' '}
                    complete
                  </span>
              ...
              <div className="divide-y" style={{ borderColor: 'rgba(255,255,255,0.04)' }}>
                {diligence.map((item, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-4 px-5 py-3.5 hover:bg-white/[0.01] transition-colors"
                  >
            ...
            >
              ...
              />
              <p className="text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>
                No active diligence process for this asset
              </p>
              <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.2)' }}>
                Diligence checklists are created when an asset enters underwriting
              </p>
            </div>
          )}
        </motion.div>
          ...
          {actionItemsLoading ? (
            ...
            >
              {actionItems.map((item) => {
                ...
                const statusNorm = (item.status ?? 'open').replace(
                  '-',
                  '_',
                ) as OperationalEntity['status'];
                const entity: OperationalEntity & { entityType: string } = {
                  ...
                  riskLevel: (item.severity === 'critical' || item.severity === 'high'
                    ...
                    : item.severity === 'medium'
                      ? 'medium'
                      : 'low') as OperationalEntity['riskLevel'],
                        ...
                        {isUpdating ? (
                          ...
                          <>
                            {statusNorm === 'open' && (
                              <button
                                onClick={() =>
                                  updateActionStatus(String(item.id), 'in_progress', property.id)
                                }
                            ...
                            <button
                              onClick={() =>
                                updateActionStatus(String(item.id), 'resolved', property.id)
                              }
                              className="text-[10px] px-2 py-1 rounded transition-colors"
                              style={{
                                background: 'rgba(64,133,106,0.1)',
                                color: '#40856a',
                                border: '1px solid rgba(64,133,106,0.25)',
                              }}
                            >
                              Resolve
                            </button>
            ...
            >
              ...
              />
              <p className="text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>
                No open action items for this asset
              </p>
            </div>
          )}
        </motion.div>
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <Link href={`/why-this-property/${property.id}`} className="ml-auto">
                <button
            ...
            {whyNowLoading && (
              <div className="flex items-center gap-2 py-6 justify-center">
                ...
                <span className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>
                  Querying NYC public data sources…
                </span>
              </div>
            )}
            ...
            {!whyNowLoading && whyNowError === 'not-found' && (
              ...
              >
                ...
                <p className="text-xs font-medium" style={{ color: 'rgba(255,255,255,0.5)' }}>
                  Not yet in the distress pipeline
                </p>
                <p className="text-[10px] mt-1" style={{ color: 'rgba(255,255,255,0.3)' }}>
                  Add this property to a distress source to trigger live scoring.
                </p>
            ...
            {!whyNowLoading && whyNowError && whyNowError !== 'not-found' && (
              ...
              >
                ...
                <p className="text-xs" style={{ color: 'rgba(255,255,255,0.4)' }}>
                  Could not load distress data — NYC Open Data may be unavailable.
                </p>
                <button
                  onClick={() => {
                    setWhyNowError(null);
            ...
            {!whyNowLoading &&
              ...
              (() => {
                const d = whyNowData;
                const tierColor =
                  d.distressTier === 'Critical'
                    ? '#ef4444'
                    : d.distressTier === 'High'
                      ? '#f97316'
                      : d.distressTier === 'Medium'
                        ? '#eab308'
                        : '#40856a';
                  ...
                  <>
                    <div className="flex items-center gap-4 mb-4">
                      <div
                        ...
                        style={{ background: `${tierColor}18`, border: `2px solid ${tierColor}50` }}
                      >
                    ...
                    <div className="space-y-2">
                      {topFactors.map((f) => {
                        ...
                        const factorColor =
                          pct >= 70 ? '#ef4444' : pct >= 40 ? '#f97316' : '#40856a';
                        return (
                          <div key={f.name}>
                            <div className="flex items-center justify-between mb-1">
                              <span
                            ...
                            >
                              <div
                                ...
                                style={{ width: `${pct}%`, background: factorColor }}
                              />
                            </div>
                            <p
                    ...
                    >
                      <p className="text-[9px]" style={{ color: 'rgba(255,255,255,0.25)' }}>
                        Computed {new Date(d.computedAt).toLocaleString()} · HPD · ACRIS · ECB · DOB
                        · DOF
                        {d.partialOutage ? ' · ⚠ partial data' : ''}
                      </p>
