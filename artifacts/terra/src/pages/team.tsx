...
import { AgentAvatar, formatCurrency } from '@/components/brokerage-ui';
import { type Agent, agents, teams } from '@/data/brokerage';
...
function AgentCard({ agent }: { agent: Agent }) {
  const performanceScore = Math.round(
    (agent.conversionRate * 0.4 +
      (1 - agent.stalledDeals / 10) * 0.3 +
      (1 - agent.avgDaysToClose / 60) * 0.3) *
      100,
  );
  const needsCoaching =
    agent.conversionRate < 0.35 || agent.stalledDeals >= 3 || agent.avgDaysToClose > 45;
      ...
      <div className="p-5">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-terra-text-muted capitalize">
          ...
          {[
            {
              ...
              value: formatCurrency(agent.commissionMTD),
              color: 'text-terra-primary',
            },
            {
              label: 'Pipeline Value',
              value: formatCurrency(agent.pipelineValue),
        ...
        <div className="space-y-3">
          <div>
            ...
            <StatBar
              value={agent.conversionRate * 100}
              max={60}
              color={
                agent.conversionRate >= 0.4
                  ? 'bg-emerald-500'
          ...
          <div>
            <div className="flex justify-between text-[10px] mb-1">
              ...
              <span
                className={cn(
                  'font-semibold',
                  agent.avgDaysToClose <= 35
                    ? 'text-emerald-400'
                    : agent.avgDaysToClose <= 45
                      ? 'text-amber-400'
                      : 'text-rose-400',
                )}
              >
            ...
            <StatBar
              value={Math.max(0, 60 - agent.avgDaysToClose)}
              max={60}
              color={
                agent.avgDaysToClose <= 35
                  ? 'bg-emerald-500'
        ...
        <div className="flex items-center gap-3 mt-4 pt-3 border-t border-terra-border text-xs">
          <span className="text-terra-text-muted">
            Closings MTD:{' '}
            <span className="text-terra-text font-semibold">{agent.closingsThisMonth}</span>
          </span>
          <span className="text-terra-text-muted">
            Leads: <span className="text-terra-text font-semibold">{agent.leadsAssigned}</span>
          </span>
    ...
    <div className="p-6 space-y-6 overflow-auto">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-terra-text-secondary mt-1">
              Deal volume, conversion, days-to-close, coaching, and workload distribution
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          },
          {
            ...
            value: formatCurrency(agents.reduce((s, a) => s + a.commissionMTD, 0)),
            numericValue: agents.reduce((s, a) => s + a.commissionMTD, 0),
            alert: false,
          },
          {
            label: 'Total Closings MTD',
            value: agents.reduce((s, a) => s + a.closingsThisMonth, 0),
            numericValue: agents.reduce((s, a) => s + a.closingsThisMonth, 0),
            alert: false,
          },
          {
            label: 'Coaching Opportunities',
            value: agents.filter(
              (a) => a.conversionRate < 0.35 || a.stalledDeals >= 3 || a.avgDaysToClose > 45,
            ).length,
            numericValue: agents.filter(
              (a) => a.conversionRate < 0.35 || a.stalledDeals >= 3 || a.avgDaysToClose > 45,
            ).length,
            alert: true,
          },
          ...
          <div
            ...
            className={cn(
              ...
              m.alert && m.numericValue > 0 ? 'border-amber-500/30' : 'border-terra-border',
            )}
          >
            ...
            <p
              className={cn(
                ...
                m.alert && m.numericValue > 0 ? 'text-amber-400' : 'text-terra-text',
              )}
            >
              {m.value}
            </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-xl border border-terra-border bg-terra-surface/50 p-5">
          <h3 className="font-display font-bold text-terra-text mb-4">
            Commission MTD by Agent ($K)
          </h3>
          <div className="h-64">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-terra-text-secondary mt-0.5">
                      Lead: {team.leadName} · {team.region} · {team.specialization}
                    </p>
                  </div>
