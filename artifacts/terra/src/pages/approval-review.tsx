...
function ApprovalCard({ approval, onAction }: { approval: FlatApproval; onAction: (id: string, action: "approve" | "reject" | "escalate") => void }) {
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: "#4a7dc815", color: "#4a7dc8" }}>
                {ACTION_LABELS[approval.actionClass] ?? approval.actionClass}
              </span>
              <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: ps.bg, color: ps.color }}>
                {approval.priority}
              </span>
            ...
            <div className="text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.4)" }}>
              {approval.propertyName} · Requested by {approval.requestedBy} · {relTime(approval.requestedAt)}
            </div>
          </div>
        </div>

        <p className="text-sm mb-4" style={{ color: "rgba(255,255,255,0.55)" }}>{approval.description}</p>
          ...
          <div className="mb-3 space-y-2 pl-4 border-l" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
            {approval.comments.map((c, i) => (
              <div key={i} className="text-xs">
                ...
                {c.internal && <span className="ml-1 text-xs" style={{ color: "#a855f7" }}>(internal)</span>}
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm" style={{ color: "rgba(255,255,255,0.4)" }}>
            Manage approval requests across all property actions — diligence gates, transactions, and export packets
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {["all", "pending", "approved", "rejected"].map(s => {
          ...
          const ss = s === "all" ? { color: "rgba(255,255,255,0.7)", bg: "rgba(255,255,255,0.04)", border: "rgba(255,255,255,0.08)" } : {
          ...
          }[s] ?? { color: "rgba(255,255,255,0.5)", bg: "rgba(255,255,255,0.02)", border: "rgba(255,255,255,0.06)" };
          return (
            <button
              key={s}
              onClick={() => setStatusFilter(s)}
              className="rounded-xl border p-3 text-left transition-all"
              style={{ background: statusFilter === s ? ss.bg : "rgba(255,255,255,0.01)", borderColor: statusFilter === s ? ss.border : "rgba(255,255,255,0.04)" }}
            >
              <div className="text-lg font-bold" style={{ color: statusFilter === s ? ss.color : "rgba(255,255,255,0.6)" }}>{count}</div>
      ...
      <div className="space-y-4">
        {displayed.length === 0 ? (
          <EmptyState
            ...
            description={statusFilter === "pending" ? "No pending approvals at this time." : `No ${statusFilter} approvals.`}
            accentColor={ACCENT}
          />
        ) : (
          displayed.map(a => (
      ...
      <div className="mt-8 rounded-xl border p-4" style={{ background: "rgba(255,255,255,0.02)", borderColor: "rgba(255,255,255,0.06)" }}>
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="text-sm font-medium" style={{ color: "rgba(255,255,255,0.5)" }}>Export Packet / Audit Trail</h3>
        </div>
        <p className="text-xs mb-3" style={{ color: "rgba(255,255,255,0.35)" }}>
          Generate a signed export packet for any property — includes diligence summary, approval log, and full audit trace.
          Packets are immutable once generated and tied to the audit chain.
        </p>
        <button className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg transition-colors hover:bg-white/5"
