  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const blockW = 72,
        blockH = 52,
        gap = 16;
      const startX = w / 2 - (2 * blockW + 1.5 * gap),
        startY = h / 2 - blockH;
      ...
      [
        `PROPERTY_TWIN: ${propertyId}`,
        `AVM_SYNC: ${((Date.now() % 1000) / 10).toFixed(0)}ms`,
        `CAP_RATE: ${capRate}`,
        `NOI_DRIFT: ${noiDrift}`,
      ].forEach((line, i) => {
...
export default function TerraAtlasRuntimePage() {
  ...
  const { data: driftData } = useStandardQuery<{
    twins: Array<{ twinId: string; driftScore: number; status: string }>;
  }>({
    queryKey: ['terra-atlas-drift'],
    queryFn: () =>
      fetch('/api/atlas/spatial/drift?twinCategory=property')
  ...
  const { data: branchData } = useStandardQuery<{ count: number }>({
    ...
    queryFn: () =>
      fetch('/api/atlas/spatial/branches?twinCategory=property')
        .then((r) => (r.ok ? r.json() : Promise.reject(r.status)))
        .then((r) => r.data ?? r),
    staleTime: 60000,
    retry: 1,
  ...
  } = useStandardQuery<{ data: { properties: ApiProperty[] } }>({
    queryKey: ['terra-properties-list'],
    queryFn: () =>
      fetch('/api/terra/properties').then((r) => (r.ok ? r.json() : Promise.reject(r.status))),
    staleTime: 120000,
    retry: 1,
  });
  const { data: selectedPropData } = useStandardQuery<{ data: ApiProperty }>({
    ...
    queryFn: () =>
      fetch(`/api/terra/properties/${selectedId}`).then((r) =>
  ...
  const { data: marketData } = useStandardQuery<{ data: MarketData }>({
    ...
    queryFn: () =>
      fetch('/api/terra/market').then((r) => (r.ok ? r.json() : Promise.reject(r.status))),
    staleTime: 120000,
    retry: 1,
  });
  ...
  const displayValue =
    (liveProp?.kpis?.value ?? Number(liveProp?.assessedValue ?? 0)) || selected.value;
  const displayNoi = (liveProp?.kpis?.noi ?? Number(liveProp?.noi ?? 0)) || selected.noi;
  const displayCapRate =
    liveProp?.kpis?.capRate ??
    (displayNoi && displayValue
      ? (displayNoi / displayValue) * 100
      : (selected.noi / selected.value) * 100);
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs" style={{ color: 'rgba(255,255,255,0.28)' }}>
            Property + market pressure twin with spatial memory, submarket overlays, and drift
            monitoring
          </p>
        </div>
          ...
          <button
            ...
            style={
              safeMode
                ...
                : { color: 'rgba(255,255,255,0.3)', borderColor: 'rgba(45,106,79,0.1)' }
            }
          >
        ...
        <div className="flex items-start gap-2 text-[11px] px-3 py-2.5 rounded-lg border border-amber-500/30 bg-amber-500/8 text-amber-200">
          ...
          <div>
            ...
            <p className="text-amber-300/70 text-[10px] mt-0.5">
              The properties API responded successfully but returned no records. The page is showing
              illustrative demo content. It will switch to live data automatically once real records
              are available.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-2 text-[11px] px-3 py-2.5 rounded-lg border border-red-500/30 bg-red-500/8 text-red-200">
          ...
          <div>
            ...
            <p className="text-red-300/70 text-[10px] mt-0.5">
              The properties API request failed. Showing demo content while the connection is
              restored.
            </p>
          </div>
        </div>
      ...
      {(liveDriftAvg !== null || liveBranchCount !== null || isLiveProperty || liveMarket) && (
        <div
          className="flex items-center gap-3 text-[10px] px-3 py-2 rounded-lg border"
          style={{ borderColor: 'rgba(45,106,79,0.12)', background: 'rgba(45,106,79,0.03)' }}
        >
          {isLiveProperty && (
            <span className="flex items-center gap-1" style={{ color: '#2d6a4f' }}>
          ...
          {liveMarket && liveMarket.totalProperties > 0 && (
            <span style={{ color: 'rgba(255,255,255,0.4)' }}>
              {liveMarket.totalProperties} properties · avg cap rate{' '}
              {liveMarket.avgCapRate ?? 'N/A'}
            </span>
          )}
          {liveDriftAvg !== null && (
            <span style={{ color: 'rgba(255,255,255,0.4)' }}>
          ...
          {liveBranchCount !== null && (
            <>
              ...
              <span className="flex items-center gap-1" style={{ color: 'rgba(255,255,255,0.4)' }}>
                ...
                scenario branch{liveBranchCount !== 1 ? 'es' : ''}
              </span>
            </>
          )}
        </div>
      ...
      {safeMode && (
        <div
          className="flex items-center gap-2 text-[10px] px-3 py-2 rounded-lg border"
          style={{
            color: '#8b7ac8',
            borderColor: 'rgba(139,122,200,0.2)',
            background: 'rgba(139,122,200,0.05)',
          }}
        >
          <Lock className="w-3 h-3 shrink-0" /> Executive Safe Mode — only stable twin data shown;
          watch and distress signals suppressed.
        </div>
      )}
      ...
      <div className="flex items-center gap-3 overflow-x-auto pb-1">
        {TWIN_STATUS.map((s) => (
          <div
            ...
            style={
              s.status !== 'warn'
                ? { borderColor: 'rgba(45,106,79,0.12)', background: 'rgba(5,10,8,0.6)' }
                : {}
            }
          >
            <span
              className={cn(
                ...
                s.status === 'warn' ? 'bg-amber-400 animate-pulse' : '',
              )}
              style={s.status !== 'warn' ? { background: '#2d6a4f' } : {}}
            />
            <span style={{ color: 'rgba(255,255,255,0.35)' }}>{s.label}</span>
            <span
              className={cn('font-mono', s.status === 'warn' ? 'text-amber-400' : '')}
              style={s.status !== 'warn' ? { color: 'rgba(255,255,255,0.7)' } : {}}
            >
              {s.value}
            </span>
          </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {displayProperties.map((p) => (
          <button
            ...
            className={cn(
              'flex items-center gap-2 px-3 py-2 rounded-lg border text-xs whitespace-nowrap transition-all text-left',
            )}
            style={
              selected.id === p.id
                ? {
          ...
          >
            ...
            <span className={cn('text-[9px] ml-1', statusColors[p.status] ?? 'text-emerald-400')}>
              {p.status}
            </span>
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-4">
          <div
            ...
            style={{ background: 'rgba(5,10,8,0.8)', border: '1px solid rgba(45,106,79,0.12)' }}
          >
            <div
              className="px-4 py-3 border-b flex items-center justify-between"
              style={{ borderColor: 'rgba(45,106,79,0.1)' }}
            >
              <div>
                <p className="text-sm font-semibold" style={{ color: 'rgba(255,255,255,0.88)' }}>
                  {selected.address} — Property Twin
                </p>
                <p className="text-[10px]" style={{ color: 'rgba(255,255,255,0.28)' }}>
                  {selected.neighborhood} · {selected.type} · {selected.units} units ·{' '}
                  {fmt(displayValue)}
                </p>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-mono" style={{ color: 'rgba(45,106,79,0.6)' }}>
                  {isLiveProperty ? 'LIVE AVM' : 'AVM SYNC'}
                </span>
                <span
                  className="w-1.5 h-1.5 rounded-full animate-pulse"
                  style={{ background: '#2d6a4f' }}
          ...
          >
            ...
            >
              ...
              <span className="text-sm font-semibold" style={{ color: 'rgba(255,255,255,0.88)' }}>
                Market Pressure Memory — {selected.neighborhood} Submarket
              </span>
            </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b" style={{ borderColor: 'rgba(45,106,79,0.08)' }}>
                    {['Period', 'Cap Rate', 'Rent Growth', 'Vacancy', 'Submarket Pressure'].map(
                      (h) => (
                        <th
                          key={h}
                          className="text-left px-4 py-2 text-[10px] font-normal uppercase tracking-wider"
                    ...
                    return (
                      <tr
                        ...
                        style={{
                          ...
                          background: isLatest ? 'rgba(45,106,79,0.04)' : undefined,
                        }}
                      >
                        <td
                        ...
                        >
                          {row.capRate.toFixed(1)}%
                        </td>
                        <td
                          className="px-4 py-2.5 font-mono"
                          style={{ color: row.rentGrowth >= 2.5 ? '#2d6a4f' : '#c8953c' }}
                        >
                          {row.rentGrowth.toFixed(1)}%
                        </td>
                        <td
                          className="px-4 py-2.5 font-mono"
                          style={{
                            color: row.vacancyRate >= 6 ? '#c45a4a' : 'rgba(255,255,255,0.55)',
                          }}
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <div className="flex-1 h-1.5 rounded-full bg-white/5 overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${row.submktPressure}%`,
                                  background: row.submktPressure > 40 ? '#c8953c' : '#2d6a4f',
                                }}
                              />
                            </div>
                            <span
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-2">
              {overlays.map((ov) => (
                <div
                  ...
                  style={
                    ov.active
                      ? { borderColor: 'rgba(45,106,79,0.2)', background: 'rgba(45,106,79,0.06)' }
                      : {
                ...
                >
                  <div className="flex items-center gap-2">
                    <div
                      ...
                      style={{ background: ov.active ? ov.color : 'rgba(255,255,255,0.2)' }}
                    />
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div
                      ...
                      style={{
                        borderColor: ov.active ? '#2d6a4f' : 'rgba(45,106,79,0.2)',
                        background: ov.active ? 'rgba(45,106,79,0.15)' : undefined,
                      }}
                    >
                      {ov.active && (
                        <span className="text-[8px]" style={{ color: '#2d6a4f' }}>
                          ✓
                        </span>
                      )}
                    </div>
          ...
          >
            ...
            >
              ...
              Twin KPIs
              {isLiveProperty && (
                <span className="text-[9px] ml-auto" style={{ color: '#2d6a4f' }}>
                  ● live
                </span>
              )}
            </p>
            {[
              {
                ...
                val: fmt(displayValue),
                sub: isLiveProperty ? 'AVM — live sync' : 'AVM — last sync 4h',
              },
              { label: 'Annual NOI', val: fmt(displayNoi), sub: 'Stabilized' },
              {
                label: 'LTV Ratio',
                val: `${(selected.ltv * 100).toFixed(0)}%`,
                sub: selected.ltv > 0.7 ? '⚠ High' : 'OK',
              },
              ...
              <div
                ...
                style={{ borderColor: 'rgba(45,106,79,0.07)' }}
              >
                <span className="text-[10px]" style={{ color: 'rgba(255,255,255,0.35)' }}>
                  {m.label}
                </span>
                <div className="text-right">
                  <p className="text-[11px] font-mono" style={{ color: 'rgba(255,255,255,0.78)' }}>
                    {m.val}
                  </p>
          ...
          >
            ...
            {[
              ...
              >
                ...
                <span
                  className={cn('text-[10px] font-mono', d.ok ? '' : 'text-amber-400')}
                  style={d.ok ? { color: '#2d6a4f' } : {}}
                >
                  {d.val}
                </span>
              </div>
