...
const SEV_COLOR = {
  info: { bg: "rgba(255,255,255,0.04)", border: "rgba(255,255,255,0.06)", text: "rgba(255,255,255,0.65)", badge: "rgba(255,255,255,0.1)", badgeText: "rgba(255,255,255,0.4)" },
  warning: { bg: "#c08a2c08", border: "#c08a2c25", text: "rgba(255,255,255,0.75)", badge: "#c08a2c20", badgeText: "#c08a2c" },
  critical: { bg: "#c04a2a08", border: "#c04a2a30", text: "rgba(255,255,255,0.85)", badge: "#c04a2a20", badgeText: "#c04a2a" },
};

const ALL_TYPES = ["all", "distress_signal", "approval_action", "diligence_update", "document_added", "readiness_change", "market_signal"] as const;
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-0.5">
            <span className="text-xs font-medium px-1.5 py-0.5 rounded" style={{ background: s.badge, color: s.badgeText }}>
              {EVENT_LABELS[event.eventType]}
            </span>
            <span className="text-xs font-medium" style={{ color: "rgba(255,255,255,0.5)" }}>{event.propertyName}</span>
            <span className="text-xs" style={{ color: "rgba(255,255,255,0.25)" }}>· {event.source}</span>
          </div>
          <div className="text-sm font-medium" style={{ color: s.text }}>{event.summary}</div>
          {expanded && event.detail && (
            <div className="mt-2 text-xs leading-relaxed" style={{ color: "rgba(255,255,255,0.5)" }}>{event.detail}</div>
          )}
...
export default function WhatChanged() {
  ...
  const filtered = whatChangedFeed.filter(e => {
    ...
    if (search && !e.summary.toLowerCase().includes(search.toLowerCase()) && !e.propertyName.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  });
    ...
    <div className="p-6 max-w-4xl mx-auto" style={{ color: "rgba(255,255,255,0.8)" }}>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="rounded-xl border p-4" style={{ background: "rgba(255,255,255,0.02)", borderColor: "rgba(255,255,255,0.06)" }}>
          <div className="text-xs mb-1" style={{ color: "rgba(255,255,255,0.4)" }}>Total events (24h)</div>
          <div className="text-2xl font-bold" style={{ color: "rgba(255,255,255,0.9)" }}>{whatChangedFeed.length}</div>
        </div>
        <div className="rounded-xl border p-4" style={{ background: "#c04a2a08", borderColor: "#c04a2a25" }}>
          <div className="text-xs mb-1" style={{ color: "#c04a2a" }}>Critical</div>
          <div className="text-2xl font-bold" style={{ color: "#c04a2a" }}>{counts.critical}</div>
        </div>
