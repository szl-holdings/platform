...
function DocumentRow({ doc }: { doc: Document }) {
  ...
  return (
    <tr className="border-b border-terra-border/50 hover:bg-terra-surface-hover transition-colors">
      <td className="py-3 px-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-terra-text-muted capitalize">
              {typeLabels[doc.type]} · {doc.category}
            </p>
          </div>
        </div>
      </td>
      ...
      <td className="py-3 px-4">
        {doc.notes && (
          <span className="text-[10px] text-amber-400 truncate max-w-[160px] block">
            {doc.notes}
          </span>
        )}
      </td>
...
export default function DocumentsPage() {
  ...
  const readiness: 'green' | 'yellow' | 'red' =
    missing.length > 0 ? 'red' : pendingSig.length > 0 ? 'yellow' : 'green';
    ...
    <div className="p-6 space-y-6 overflow-auto">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-terra-text-secondary mt-1">
              Document checklist, missing-document detection, signer status, and compliance
              readiness scoring
            </p>
          </div>
      ...
      <div className="rounded-xl border border-terra-border bg-terra-surface/50 p-5">
        <h3 className="font-display font-bold text-terra-text mb-4">
          Compliance Readiness by Category
        </h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {(['buyer', 'seller', 'agent', 'lender', 'title', 'legal'] as const).map((cat) => {
            ...
            const level: 'green' | 'yellow' | 'red' =
              catMissing.length > 0
                ? 'red'
                : catDocs.some((d) => d.status === 'pending-signature')
                  ? 'yellow'
                  : 'green';
            return (
              <div
