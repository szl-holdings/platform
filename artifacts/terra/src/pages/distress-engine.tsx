...
const DISTRESS_TYPE_CONFIG: Record<
  DistressType,
  { label: string; color: string; bg: string; icon: typeof Flame; pinColor: string }
> = {
  'pre-foreclosure': {
...
function ScoreBadge({ score, size = 'sm' }: { score: number; size?: 'sm' | 'lg' }) {
  ...
  return (
    <div
      className={cn(
        'rounded-lg border font-bold font-mono',
        color,
        size === 'lg' ? 'px-3 py-1.5 text-xl' : 'px-2 py-0.5 text-xs',
      )}
    >
      {score}
    </div>
  ...
  return (
    ...
    >
        ...
        <div className="flex-1 min-w-0">
          <p
            className={cn(
              ...
              isSelected ? 'text-terra-primary' : 'text-terra-text',
            )}
          >
            {property.address}
          </p>
          <p className="text-[11px] text-terra-text-muted">
            {property.borough} · {property.zipCode ?? property.county}
          </p>
        </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <span className="text-[10px] text-terra-text-muted font-mono">
  ...
  return (
    <div className="relative w-full h-full bg-[#0a0f1e] overflow-hidden rounded-b-xl">
      ...
      <div className="absolute bottom-3 left-3 flex flex-col gap-1">
        {(
          Object.entries(DISTRESS_TYPE_CONFIG) as [
            DistressType,
            (typeof DISTRESS_TYPE_CONFIG)[DistressType],
          ][]
        ).map(([key, cfg]) => (
      ...
      <div className="absolute top-2 right-2 text-[9px] text-slate-500 font-mono">
        NYC DISTRESS MAP · {displayProps.length} pins
      </div>
    </div>
  );
}
    ...
    <div className="flex flex-col h-full overflow-y-auto">
      <div className="p-4 border-b border-terra-border flex items-start justify-between gap-2 sticky top-0 bg-terra-bg-secondary z-10">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-terra-text-muted mt-0.5">
            {property.borough} · {property.county} County · {property.zipCode}
          </p>
        </div>
        <button
          onClick={onClose}
          className="p-1 hover:bg-terra-surface rounded transition-colors flex-shrink-0"
        >
      ...
      <div className="p-4 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-terra-text-muted capitalize">
            {property.stage.replace(/-/g, ' ')}
          </span>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-terra-surface rounded-lg p-3 border border-terra-border">
            <p className="text-[10px] text-terra-text-muted uppercase tracking-wider mb-1">
              Est. Value
            </p>
            <p className="text-lg font-bold text-terra-text font-mono">
              {formatCurrency(property.estimatedValue)}
            </p>
          </div>
          <div className="bg-terra-surface rounded-lg p-3 border border-terra-border">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-terra-text-muted capitalize">
          ...
          {property.debtAmount != null && (
            <div className="bg-terra-surface rounded-lg p-3 border border-terra-border">
              ...
              <p className="text-lg font-bold text-red-400 font-mono">
                {formatCurrency(property.debtAmount)}
              </p>
            </div>
          )}
        ...
        <div className="bg-terra-surface border border-terra-border rounded-lg p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span
              className={cn(
                ...
                property.ownerType === 'llc'
                  ? 'bg-amber-400/10 text-amber-400'
                  : property.ownerType === 'corporate'
          ...
          <p className="text-[11px] text-terra-text-muted mt-0.5">
            Filed {property.filingDate} · {property.county} County
          </p>
        </div>
        ...
        <div className="bg-terra-surface border border-terra-border rounded-lg p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[9px] text-amber-400 bg-amber-400/10 px-1.5 py-0.5 rounded border border-amber-400/20 ml-auto">
              DISTRESS ENGINE
            </span>
          </div>
          ...
          <div className="mt-2 pt-2 border-t border-terra-border">
            <p className="text-[10px] text-terra-text-muted">
              ...
              <span
                className={cn(
                  'font-semibold',
                  property.opportunityScore >= 80 ? 'text-[#40856a]' : 'text-amber-400',
                )}
              >
                {property.opportunityScore >= 80
                  ? 'High (70–85%)'
                  : property.opportunityScore >= 65
                    ? 'Medium (40–70%)'
                    : 'Low (10–40%)'}
              </span>
            </p>
                  ...
                  <div className="flex flex-col items-center">
                    <div
                      className={cn(
                        ...
                        i === property.timeline.length - 1 ? 'bg-terra-primary' : 'bg-terra-border',
                      )}
                    />
                    {i < property.timeline.length - 1 && (
                      <div className="w-0.5 h-full bg-terra-border flex-1 mt-0.5" />
                    )}
                  </div>
                  <div className="pb-2 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-[10px] font-semibold text-terra-text">
                        {event.type}
                      </span>
                      <span className="text-[10px] text-terra-text-muted font-mono">
                        {event.date}
                    ...
                    <p className="text-[11px] text-terra-text-secondary mt-0.5">
                      {event.description}
                    </p>
                  </div>
                </div>
        ...
        {property.tags.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {property.tags.map((t) => (
              <span
                key={t}
                className="text-[10px] text-terra-text-muted bg-terra-surface border border-terra-border px-2 py-0.5 rounded"
              >
                {t}
              </span>
            ))}
        ...
        <div className="space-y-2">
          <p className="text-[10px] text-terra-text-muted uppercase tracking-wider font-semibold">
            Actions
          </p>
          <div className="grid grid-cols-2 gap-2">
            <button
              onClick={() => {
                onConvertToLead(property);
                onClose();
              }}
          ...
          <Link href={`/why-this-property/${property.id}`}>
            <button
              className="w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-white text-xs font-bold transition-colors"
...
function useDistressProperties(params: {
  borough?: string;
  distressType?: string;
  q?: string;
  limit: number;
  ...
  if (params.distressType && params.distressType !== 'all')
    searchParams.set('distressType', params.distressType);
...
export default function DistressEnginePage() {
  ...
  const auctionImminent = sorted.filter(
    (p) =>
      p.auctionDate && Math.ceil((new Date(p.auctionDate).getTime() - Date.now()) / 86400000) <= 30,
  ).length;
    ...
    <div className="flex flex-col h-full overflow-hidden">
      {conversionState.status !== 'idle' && (
        <div
          className={cn(
            ...
            conversionState.status === 'loading'
              ? 'bg-terra-bg-secondary border-terra-border text-terra-text-muted'
              : conversionState.status === 'success'
                ? 'bg-[#0f2a1e] border-[#2d6a4f]/50 text-[#40856a]'
        ...
        >
          ...
          {conversionState.status !== 'loading' && (
            <button
              onClick={() => setConversionState({ status: 'idle' })}
      ...
      <div className="flex-shrink-0 px-4 pt-4 pb-2 space-y-3 border-b border-terra-border bg-terra-bg-secondary">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-base font-display font-bold text-terra-text">
                  Distress Intelligence Engine
                </h1>
                <p className="text-[10px] text-terra-text-muted">
                  {isLoading
                    ? 'Loading...'
                    : `${sorted.length} properties · ${auctionImminent} auctions within 30 days`}
                </p>
              </div>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => refetch()}
              className="p-1.5 rounded-lg border border-terra-border text-terra-text-muted hover:text-terra-text transition-colors"
            >
            ...
            <button
              ...
              className={cn(
                ...
                showAlerts
                  ...
                  : 'border-terra-border text-terra-text-secondary hover:bg-terra-surface',
              )}
            >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <select
            value={boroughFilter}
            onChange={(e) => setBoroughFilter(e.target.value as Borough | 'all')}
          ...
          <select
            ...
            onChange={(e) => setDistressFilter(e.target.value as DistressType | 'all')}
            className="px-2 py-1.5 bg-terra-surface border border-terra-border rounded-lg text-xs text-terra-text focus:outline-none focus:border-terra-primary"
          >
            {DISTRESS_FILTERS.map((f) => (
              <option key={f.value} value={f.value}>
                {f.label}
              </option>
            ))}
          ...
          {!isLoading && (
            <span className="text-[11px] text-terra-text-muted ml-auto">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1">
          {(DISTRESS_FILTERS.slice(1) as Array<{ value: DistressType; label: string }>).map(
            ({ value: type }) => {
              const cfg = DISTRESS_TYPE_CONFIG[type];
              ...
              return (
                <button
                  key={type}
                  onClick={() => setDistressFilter(distressFilter === type ? 'all' : type)}
                  className={cn(
                    'flex items-center gap-1 px-2.5 py-1 rounded-lg border text-[10px] font-semibold whitespace-nowrap transition-colors flex-shrink-0',
                    distressFilter === type
                      ? cn(cfg.color, cfg.bg)
                      : 'border-terra-border text-terra-text-muted hover:bg-terra-surface',
                  )}
                >
                  <cfg.icon className="w-2.5 h-2.5" />
                  {cfg.label} ({count})
                    ...
                    className={cn(
                      ...
                      a.severity === 'critical'
                        ...
                        : a.severity === 'high'
                          ...
                          : 'bg-terra-surface border-terra-border',
                    )}
                  >
                    <AlertTriangle
                      className={cn(
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-terra-text-muted text-[10px] mt-0.5 font-mono">
                        {a.triggeredAt?.slice(0, 10)}
                      </p>
                    </div>
                  </div>
                ))
      ...
      <div className="flex-1 overflow-hidden">
        {isLoading && (
          <div className="p-4 space-y-2 overflow-auto">
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="rounded-xl border border-terra-border bg-terra-surface/40 p-4 flex items-start gap-3 animate-pulse"
              >
        ...
        {isError && !isLoading && (
          <div className="flex flex-col items-center justify-center h-full text-terra-text-muted">
            ...
            <button
              onClick={() => refetch()}
              className="mt-3 px-4 py-2 rounded-lg border border-terra-border text-xs hover:bg-terra-surface transition-colors"
            >
              Retry
            </button>
          </div>
        ...
        {!isLoading && !isError && viewMode === 'split' && (
          <div className="flex h-full">
            ...
            <div
              className={cn('flex-1 overflow-hidden', selectedProperty ? 'grid grid-cols-2' : '')}
            >
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-terra-surface border-b border-terra-border">
                    <th className="text-left px-4 py-3 text-[10px] font-semibold text-terra-text-muted uppercase tracking-wider">
                      Address
                    </th>
                    <th className="text-left px-3 py-3 text-[10px] font-semibold text-terra-text-muted uppercase tracking-wider">
                      Borough
                    </th>
                    <th className="text-left px-3 py-3 text-[10px] font-semibold text-terra-text-muted uppercase tracking-wider">
                      Type
                    ...
                    <th className="text-right px-3 py-3 text-[10px] font-semibold text-terra-text-muted uppercase tracking-wider">
                      Est. Value
                    </th>
                    <th className="text-right px-3 py-3 text-[10px] font-semibold text-terra-text-muted uppercase tracking-wider">
                      Score
                ...
                <tbody>
                  {sorted.map((p, i) => (
                    <motion.tr
                      ...
                      className={cn(
                        'border-b border-terra-border/50 hover:bg-terra-surface-hover transition-colors cursor-pointer',
                        selectedId === p.id && 'bg-terra-primary/5',
                      )}
                      onClick={() => {
                        setSelectedId(p.id);
                        setViewMode('split');
                    ...
                    >
                      ...
                      <td className="px-3 py-3">
                        <DistressTypePill type={p.distressType} compact />
                      </td>
                      <td className="px-3 py-3 text-xs text-terra-text-muted truncate max-w-[120px]">
                        {p.ownerName}
                      </td>
                      <td className="px-3 py-3 text-right font-mono text-xs text-terra-text">
                        {formatCurrency(p.estimatedValue)}
                      </td>
                      <td className="px-3 py-3 text-right">
                        <ScoreBadge score={p.opportunityScore} />
                      </td>
                      <td className="px-3 py-3 text-right text-[10px] text-terra-text-muted">
                        {p.daysInDistress}d
                      </td>
                      <td className="px-3 py-3 text-right text-[10px] text-terra-text-muted font-mono">
                        {p.filingDate}
                      </td>
                    </motion.tr>
                  ))}
