...
const recentReports = [
  ...
  {
    name: 'Year-End Capital Account Statement',
    type: 'Annual',
    date: 'Feb 1, 2026',
    status: 'Sent',
  },
  ...
  {
    name: 'Property Acquisition — Austin Tower',
    type: 'Transaction',
    date: 'Nov 3, 2025',
    status: 'Sent',
  },
  {
    name: 'Disposition Notice — SF Warehouse',
    type: 'Transaction',
    date: 'Dec 14, 2025',
    status: 'Sent',
  },
      ...
      <div className="p-6 flex flex-col items-center justify-center min-h-[400px] gap-4 text-center">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground max-w-sm">
            The Investor Relations module is restricted to users with the investor role. Contact
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          LP reporting, distribution waterfall calculations, and investor relationship management
        </p>
      </div>
      ...
      <div className="flex gap-1 border-b border-border">
        {(
          [
            { key: 'investors', label: 'LP Investors' },
            { key: 'reports', label: 'Reports & Letters' },
            { key: 'distributions', label: 'Distribution History' },
          ] as { key: IRTab; label: string }[]
        ).map(({ key, label }) => (
          <button
            key={key}
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-3 gap-4 mt-2 text-xs">
                      ...
                      <div>
                        ...
                        <p className="font-semibold">{lp.deployed}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Distributions</p>
                        <p className="font-semibold text-emerald-400">{lp.distributions}</p>
                      </div>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3 text-center shrink-0">
                    <div>
                      <p className="text-sm font-bold text-emerald-400">{lp.netIRR}%</p>
                      <p className="text-[10px] text-muted-foreground">Net IRR</p>
                    </div>
                    <div>
                      <p className="text-sm font-bold text-sky-400">{lp.moic}x</p>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant="outline"
                      className={`text-[10px] ${r.status === 'Sent' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' : 'text-amber-400 bg-amber-500/10 border-amber-500/20'}`}
                    >
                      {r.status}
                    </Badge>
