...
const BRANCHES: Branch[] = [
  {
    ...
    color: '#c45a4a',
    description:
      'Continued cap rate expansion (+150bps) driven by rate environment, combined with 10% NOI compression from vacancy and rent concessions.',
    ...
    risks: [
      'LTV breach on construction loan covenant',
      'NOI insufficient for DSCR at refi',
      'Portfolio-level impairment if across 3+ assets',
    ],
    mitigations: [
      'Accelerate lease-up to defend NOI',
      'Pre-negotiate rate cap extension with lender',
      'Explore preferred equity injection to reduce LTV',
    ],
  },
  {
    id: 'tenant-churn',
    ...
    color: '#c8953c',
    description:
      'Three simultaneous vacancies on unit turnover, with 6-month re-leasing timeline. Impact on NOI and occupancy stabilization.',
    ...
    risks: [
      'Extended vacancy if submarket softens',
      'Rent concession pressure from competing supply',
      'Debt service coverage at risk below 85% occupancy',
    ],
    mitigations: [
      'Pre-qualify 3 qualified tenant prospects now',
      'Build 2-month vacancy reserve fund',
      'Engage broker for proactive pre-leasing on any expiry',
    ],
  },
  {
    id: 'flood-risk',
    ...
    color: '#4a90b8',
    description:
      '50-year flood event impacts subject property. Structural damage, unit downtime, and insurance claim process over 8–18 months.',
    ...
    risks: [
      'Insurance non-renewal after major claim',
      'FEMA flood zone upgrade increases premiums permanently',
      'Tenant relocation liability during remediation',
    ],
    mitigations: [
      'Flood elevation certificate obtained',
      'Business interruption insurance in force',
      'Contractor relationships pre-qualified for rapid mobilization',
    ],
  },
  {
    ...
    color: '#8b7ac8',
    description:
      'Construction loan maturing in Q4 2025. Higher rate environment results in refinance shortfall — capital required to bridge gap.',
    ...
    risks: [
      'Lender declines refi at current LTV — forces partial paydown',
      'Personal guarantee called if DSCR < 1.0x',
      'Forced sale at depressed market conditions',
    ],
    mitigations: [
      'Engage 3 lenders 6 months pre-maturity',
      'Explore bridge-to-permanent structure',
      'Preferred equity injection to reduce LTV to 55% target',
    ],
  },
];
  ...
  return (
    <div className="space-y-4">
      <div
        ...
        style={{ background: 'rgba(5,10,8,0.8)', border: `1px solid ${branch.color}25` }}
      >
        <div className="flex items-center gap-2 mb-2">
          <Icon className="w-4 h-4" style={{ color: branch.color }} />
          <h3 className="text-sm font-bold" style={{ color: 'rgba(255,255,255,0.88)' }}>
            {branch.label}
          </h3>
          <span
        ...
        <p className="text-[10px]" style={{ color: 'rgba(255,255,255,0.25)' }}>
          <span style={{ color: 'rgba(255,255,255,0.45)' }}>Trigger:</span> {branch.trigger}
        </p>
      </div>
      ...
      >
        <p className="text-xs font-semibold mb-3" style={{ color: 'rgba(255,255,255,0.88)' }}>
          Delta vs. Baseline
        </p>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div
          className="rounded-xl p-4"
          style={{ background: 'rgba(5,10,8,0.8)', border: '1px solid rgba(196,90,74,0.12)' }}
        >
          <p className="text-[11px] font-semibold mb-2 flex items-center gap-1.5 text-red-400/80">
          ...
          <ul className="space-y-1.5">
            {branch.risks.map((r, i) => (
              <li
                key={i}
                className="text-[10px] flex gap-1.5"
        ...
        >
          <p
            ...
            style={{ color: 'rgba(107,143,113,0.9)' }}
          >
          ...
          <ul className="space-y-1.5">
            {branch.mitigations.map((m, i) => (
              <li
                ...
                style={{ color: 'rgba(255,255,255,0.4)' }}
              >
                <span className="mt-0.5" style={{ color: 'rgba(45,106,79,0.5)' }}>
                  •
                </span>
                {m}
              </li>
...
export default function TerraScenarioBranchesPage() {
  ...
  } = useStandardQuery<{ data?: { count: number } }>({
    queryKey: ['terra-atlas-scenario-branches'],
    queryFn: () =>
      fetch('/api/atlas/spatial/branches?twinCategory=property').then((r) =>
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs" style={{ color: 'rgba(255,255,255,0.28)' }}>
            Simulate diverging property worldlines — compare branch outcomes against baseline
          </p>
        </div>
        <div className="flex items-center gap-2">
          {dataMode === 'live' && (
            <div
              className="flex items-center gap-1.5 text-[10px] px-2.5 py-1 rounded-lg border"
              style={{
                color: '#2d6a4f',
                borderColor: 'rgba(45,106,79,0.3)',
                background: 'rgba(45,106,79,0.08)',
      ...
      <div
        className="rounded-xl p-4"
        style={{ background: 'rgba(5,10,8,0.8)', border: '1px solid rgba(45,106,79,0.15)' }}
      >
        <div className="flex items-center gap-2 mb-3">
          ...
          >
            Baseline Worldline — 84 Grand St, Williamsburg
          </span>
        </div>
          ...
          {BRANCHES.map((b) => {
            ...
            return (
              <div
                ...
                style={
                  isSelected
                    ? { borderColor: `${b.color}35`, background: `${b.color}08` }
                    : { borderColor: 'rgba(45,106,79,0.1)', background: 'rgba(5,10,8,0.6)' }
                }
              >
