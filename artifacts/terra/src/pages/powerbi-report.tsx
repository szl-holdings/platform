...
export default function TerraPowerBiReport() {
  ...
  const fetchEmbedToken = async () => {
    ...
    setFetchError(null);
    try {
      const data = await apiFetch<{ data: EmbedTokenResponse }>(
      ...
      >
        <div>
          ...
          <p className="text-sm text-terra-text-secondary mt-1">
            Power BI embedded report — portfolio-wide analytics, occupancy trends, NOI performance,
      ...
      {!embedConfig && (
        <>
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="rounded-xl border border-terra-border bg-terra-emerald/5 p-4 flex items-start gap-3"
          >
            ...
            <div className="text-xs text-terra-text-secondary leading-relaxed">
              <strong className="text-terra-text">Sample data shown below.</strong> Click "Load Live
              Report" to fetch a server-minted embed token. Configure Power BI credentials in SZL
              Admin → Power BI.
            </div>
          </motion.div>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span
                      className={cn(
                        ...
                        m.up ? 'text-terra-emerald' : 'text-terra-rose',
                      )}
                    >
                      {m.delta}
                    </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <motion.div
              ...
              className="rounded-2xl border border-terra-border bg-terra-surface p-5"
            >
                  ...
                  <div key={d.name}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span
                        className={cn(
                          'text-xs font-bold',
                          d.occupancy >= d.target ? 'text-terra-emerald' : 'text-terra-amber',
                        )}
                      >
                        {d.occupancy}%
                      </span>
                    ...
                    <div className="relative h-2 bg-terra-bg-tertiary rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${d.occupancy}%`,
                          background: d.occupancy >= d.target ? '#10b981' : '#f59e0b',
                        }}
                      />
                    </div>
                  </div>
            ...
            >
                ...
                {REVENUE_TREND.map((d) => (
                  <div key={d.month} className="flex-1 flex flex-col items-center gap-0.5">
                    <div className="w-full flex flex-col justify-end" style={{ height: '120px' }}>
                      <div
                        className="w-full rounded-t-sm bg-terra-emerald/30"
                        style={{ height: `${(d.noi / maxRevenue) * 100}%` }}
                      />
                      <div
                        className="w-full rounded-t-sm bg-terra-primary/60 -mt-0.5"
                        style={{ height: `${((d.revenue - d.noi) / maxRevenue) * 100}%` }}
                      />
                    </div>
              ...
              <div className="flex items-center gap-4 mt-2">
                <div className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-sm bg-terra-primary/60" />
                  <span className="text-[10px] text-terra-text-muted">Revenue</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-sm bg-terra-emerald/30" />
                  <span className="text-[10px] text-terra-text-muted">NOI</span>
                </div>
              </div>
            </motion.div>
          </div>
