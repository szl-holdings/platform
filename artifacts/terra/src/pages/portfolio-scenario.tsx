...
import {
  Sliders, BarChart3, TrendingUp, TrendingDown, DollarSign, Activity,
  Plus, Minus, MapPin, Building2, RefreshCw, Target, ArrowUpRight,
  ArrowDownRight, Info, PieChart, LayoutGrid, ChevronRight
} from "lucide-react";
...
const PORTFOLIO: PortfolioAsset[] = [
  { id: "p-1", address: "84 Grand St", neighborhood: "Williamsburg", type: "Multi-Family", value: 3_800_000, noi: 228_000, ltv: 0.58, irr: 14.2, cashOnCash: 7.4, capRate: 6.0, climateRisk: "low" },
  { id: "p-2", address: "210 Kent Ave", neighborhood: "Williamsburg", type: "Mixed-Use", value: 5_200_000, noi: 364_000, ltv: 0.62, irr: 12.8, cashOnCash: 6.2, capRate: 7.0, climateRisk: "low" },
  { id: "p-3", address: "1002 Myrtle Ave", neighborhood: "Bushwick", type: "Multi-Family", value: 2_100_000, noi: 147_000, ltv: 0.71, irr: 16.4, cashOnCash: 8.8, capRate: 7.0, climateRisk: "low" },
  { id: "p-4", address: "422 Flatbush Ave", neighborhood: "Park Slope", type: "Commercial", value: 7_400_000, noi: 444_000, ltv: 0.54, irr: 11.1, cashOnCash: 5.4, capRate: 6.0, climateRisk: "medium" },
  { id: "p-5", address: "55 Ocean Ave", neighborhood: "Crown Heights", type: "Multi-Family", value: 1_650_000, noi: 115_500, ltv: 0.68, irr: 15.6, cashOnCash: 8.1, capRate: 7.0, climateRisk: "low" },
];

const CANDIDATES: CandidateProperty[] = [
  { id: "c-1", address: "211 Liberty Ave", neighborhood: "East New York", type: "Multi-Family", askPrice: 790_000, projectedNoi: 79_000, arv: 1_100_000, rehabCost: 140_000, projectedCapRate: 10.0, projectedIrr: 22.4, projectedCashOnCash: 13.2, ltv: 0.64, climateRisk: "low" },
  { id: "c-2", address: "1847 Myrtle Ave", neighborhood: "Bushwick", type: "Multi-Family", askPrice: 1_350_000, projectedNoi: 108_000, arv: 1_850_000, rehabCost: 210_000, projectedCapRate: 8.0, projectedIrr: 18.9, projectedCashOnCash: 10.8, ltv: 0.72, climateRisk: "low" },
  { id: "c-3", address: "392 Nostrand Ave", neighborhood: "Crown Heights", type: "Mixed-Use", askPrice: 2_100_000, projectedNoi: 147_000, arv: 2_400_000, rehabCost: 85_000, projectedCapRate: 7.0, projectedIrr: 15.2, projectedCashOnCash: 8.6, ltv: 0.68, climateRisk: "low" },
  { id: "c-4", address: "78 Covert St", neighborhood: "Ridgewood", type: "Multi-Family", askPrice: 1_540_000, projectedNoi: 107_800, arv: 1_620_000, rehabCost: 45_000, projectedCapRate: 7.0, projectedIrr: 13.4, projectedCashOnCash: 7.2, ltv: 0.60, climateRisk: "low" },
];
...
function DeltaBadge({ base, projected, format = "pct", higher = "good" }: { base: number; projected: number; format?: "pct" | "currency"; higher?: "good" | "bad" }) {
  ...
  return (
    <div className={cn("flex items-center gap-0.5 text-xs font-bold", color)}>
      ...
      {format === "pct" ? `${delta > 0 ? "+" : ""}${pct}%` : `${delta > 0 ? "+" : ""}${formatCurrency(Math.abs(delta), true)}`}
    </div>
  );
}

function MetricTile({ label, base, projected, format = "pct", unit = "", higher = "good" }: {
  label: string; base: number; projected: number;
  format?: "pct" | "currency" | "number"; unit?: string; higher?: "good" | "bad";
}) {
  const delta = projected - base;
  const positive = higher === "good" ? delta >= 0 : delta <= 0;
    ...
    <div className="bg-[#0f1115] border border-white/6 rounded-xl p-4">
      ...
      <div className="flex items-end gap-2 mt-1.5">
        <p className="text-xl font-bold text-white">
          {format === "currency" ? formatCurrency(projected, true) : format === "pct" ? `${projected.toFixed(1)}%` : `${projected.toFixed(1)}${unit}`}
        </p>
        {!noChange && (
          <span className={cn("text-xs font-semibold mb-0.5", positive ? "text-emerald-400" : "text-red-400")}>
            {delta > 0 ? "+" : ""}{format === "currency" ? formatCurrency(Math.abs(delta), true) : format === "pct" ? `${delta.toFixed(1)}%` : `${delta.toFixed(1)}${unit}`}
          </span>
        )}
      </div>
      <p className="text-[10px] text-white/20 mt-1">
        Base: {format === "currency" ? formatCurrency(base, true) : format === "pct" ? `${base.toFixed(1)}%` : `${base.toFixed(1)}${unit}`}
      </p>
    </div>
  );
}
...
function GeographicConcentration({ assets }: { assets: { neighborhood: string }[] }) {
  ...
  assets.forEach(a => { counts[a.neighborhood] = (counts[a.neighborhood] || 0) + 1; });
      ...
      {sorted.map(([hood, count]) => (
        <div key={hood} className="flex items-center gap-2">
          ...
          <div className="flex-1 bg-white/5 rounded-full h-1.5">
            <div className="h-1.5 rounded-full bg-[#40856a]" style={{ width: `${(count / total) * 100}%` }} />
          </div>
          <span className="text-xs text-white/30 w-8 text-right">{Math.round((count / total) * 100)}%</span>
        </div>
      ))}
    </div>
  );
...
export default function PortfolioScenario() {
  ...
  const baseMetrics = useMemo(() => {
    ...
    return { totalValue, totalNoi, avgCapRate, avgIrr, avgCoc, avgLtv, count: PORTFOLIO.length, highRisk };
  }, []);
  ...
  const projectedMetrics = useMemo(() => {
    ...
    const adjustedIrr = candidate.projectedIrr * (1 - (rehabMultiplier - 1) * 0.3) * (exitYears <= 3 ? 0.9 : exitYears >= 7 ? 1.1 : 1.0);
    ...
    return { totalValue, totalNoi, avgCapRate, avgIrr, avgCoc, avgLtv, count: allAssets.length, highRisk };
  }, [candidate, baseMetrics, equityContribution, exitYears, rehabMultiplier]);

  const allNeighborhoods = candidate
    ? [...PORTFOLIO.map(a => ({ neighborhood: a.neighborhood })), { neighborhood: candidate.neighborhood }]
    ...
    <div className="flex h-full overflow-hidden">
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="flex-shrink-0 p-6 border-b border-white/6">
          ...
          <p className="text-xs text-white/40 mt-1">Interactive what-if analysis — see how adding a property changes returns, risk, concentration, and cash flow</p>
        </div>

        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div>
            <p className="text-xs text-white/30 uppercase tracking-wider font-medium mb-3">Select Candidate Property</p>
              ...
              {CANDIDATES.map(c => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedCandidate === c.id ? "border-[#40856a]/40 bg-[#40856a]/8" : "border-white/6 bg-white/2 hover:border-white/10"
                  )}
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="text-xs font-semibold text-white">{c.address}</p>
                      <p className="text-[10px] text-white/40 mt-0.5">{c.neighborhood} · {c.type}</p>
                    </div>
                    {selectedCandidate === c.id && <CheckIcon />}
                  </div>
                  <div className="flex gap-3 mt-2">
                    <span className="text-[10px] text-white/40">Ask: <span className="text-white/70">{formatCurrency(c.askPrice, true)}</span></span>
                    <span className="text-[10px] text-[#40856a]">IRR: {c.projectedIrr.toFixed(1)}%</span>
                    <span className="text-[10px] text-sky-400">Cap: {c.projectedCapRate.toFixed(1)}%</span>
                  </div>
                </button>
              ))}
            </div>
          ...
          {candidate && (
            <div>
              <p className="text-xs text-white/30 uppercase tracking-wider font-medium mb-3">Scenario Parameters</p>
              <div className="grid grid-cols-3 gap-4 bg-white/2 border border-white/6 rounded-xl p-4">
                <div>
                  <label className="text-[10px] text-white/40 block mb-1.5">Equity Contribution: <span className="text-white/70 font-bold">{equityContribution}%</span></label>
                  <input type="range" min={15} max={50} value={equityContribution} onChange={e => setEquityContribution(+e.target.value)}
                    className="w-full accent-[#40856a]" />
                  <div className="flex justify-between text-[9px] text-white/20 mt-0.5"><span>15%</span><span>50%</span></div>
                </div>
                <div>
                  <label className="text-[10px] text-white/40 block mb-1.5">Hold Period: <span className="text-white/70 font-bold">{exitYears} yrs</span></label>
                  <input type="range" min={2} max={10} value={exitYears} onChange={e => setExitYears(+e.target.value)}
                    className="w-full accent-[#40856a]" />
                  <div className="flex justify-between text-[9px] text-white/20 mt-0.5"><span>2 yr</span><span>10 yr</span></div>
                </div>
                <div>
                  <label className="text-[10px] text-white/40 block mb-1.5">Rehab Cost: <span className="text-white/70 font-bold">{(rehabMultiplier * 100 - 100).toFixed(0)}% adj</span></label>
                  <input type="range" min={0.7} max={1.5} step={0.05} value={rehabMultiplier} onChange={e => setRehabMultiplier(+e.target.value)}
                    className="w-full accent-[#40856a]" />
                  <div className="flex justify-between text-[9px] text-white/20 mt-0.5"><span>-30%</span><span>+50%</span></div>
                </div>
              </div>
            </div>
          )}
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <p className="text-xs text-white/30 uppercase tracking-wider font-medium">Portfolio Impact</p>
              {candidate && <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#40856a]/10 border border-[#40856a]/20 text-[#40856a]">+1 property scenario</span>}
            </div>
            <div className="grid grid-cols-3 gap-3">
              <MetricTile label="Portfolio AUM" base={baseMetrics.totalValue} projected={projectedMetrics.totalValue} format="currency" higher="good" />
              <MetricTile label="Total NOI" base={baseMetrics.totalNoi} projected={projectedMetrics.totalNoi} format="currency" higher="good" />
              <MetricTile label="Avg Cap Rate" base={baseMetrics.avgCapRate} projected={projectedMetrics.avgCapRate} format="pct" higher="good" />
              <MetricTile label="Blended IRR" base={baseMetrics.avgIrr} projected={projectedMetrics.avgIrr} format="pct" higher="good" />
              <MetricTile label="Avg Cash-on-Cash" base={baseMetrics.avgCoc} projected={projectedMetrics.avgCoc} format="pct" higher="good" />
              <MetricTile label="Avg LTV" base={baseMetrics.avgLtv * 100} projected={projectedMetrics.avgLtv * 100} format="pct" higher="bad" />
            </div>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white/2 border border-white/6 rounded-xl p-4">
              <p className="text-xs text-white/30 uppercase tracking-wider mb-3">Geographic Concentration</p>
              <GeographicConcentration assets={allNeighborhoods} />
              {candidate && (
                <p className="text-[10px] text-white/30 mt-3">
                  {(() => {
                    const counts: Record<string, number> = {};
                    allNeighborhoods.forEach(a => { counts[a.neighborhood] = (counts[a.neighborhood] || 0) + 1; });
                    const top = Object.entries(counts).sort((a, b) => b[1] - a[1])[0];
                    const pct = Math.round((top[1] / allNeighborhoods.length) * 100);
                    return pct > 35 ? `⚠ ${top[0]} at ${pct}% — concentration risk` : `Concentration within acceptable limits`;
                  })()}
                </p>
              )}
            </div>
            ...
            <div className="bg-white/2 border border-white/6 rounded-xl p-4">
              ...
              <div className="space-y-3">
                {[
                  { label: "Climate Risk (High)", base: baseMetrics.highRisk, projected: projectedMetrics.highRisk, icon: "🌊", higher: "bad" as const },
                  { label: "Avg LTV", base: +(baseMetrics.avgLtv * 100).toFixed(1), projected: +(projectedMetrics.avgLtv * 100).toFixed(1), icon: "🏦", higher: "bad" as const },
                  { label: "Properties", base: baseMetrics.count, projected: projectedMetrics.count, icon: "🏢", higher: "good" as const },
                ].map(r => (
                  <div key={r.label} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {candidate && r.base !== r.projected && (
                        <span className={cn("text-[10px] font-bold", (r.projected > r.base) === (r.higher === "good") ? "text-emerald-400" : "text-red-400")}>
                          {r.projected > r.base ? "▲" : "▼"}
                        </span>
                      )}
                    </div>
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {PORTFOLIO.map(a => (
                <div key={a.id} className="flex items-center gap-4 px-4 py-2.5 bg-white/2 border border-white/5 rounded-lg">
              ...
              {candidate && (
                <div className="flex items-center gap-4 px-4 py-2.5 bg-[#40856a]/8 border border-[#40856a]/20 rounded-lg">
                  ...
                  <span className="text-xs text-[#40856a] flex-1">{candidate.address} <span className="text-[#40856a]/60">(scenario)</span></span>
                  <span className="text-[10px] text-[#40856a]/70 w-20 text-right">{formatCurrency(candidate.arv, true)}</span>
                  <span className="text-[10px] text-[#40856a] w-16 text-right">IRR {candidate.projectedIrr}%</span>
                  <span className="text-[10px] text-sky-400 w-12 text-right">Cap {candidate.projectedCapRate}%</span>
                </div>
              )}
            </div>
          </div>
