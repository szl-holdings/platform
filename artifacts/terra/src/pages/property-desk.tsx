      ...
      {connectors.map((c) => (
        <div key={c.name} className="flex items-center justify-between text-sm">
          ...
          {c.status === 'not_connected' ? (
            <button
              className="flex items-center gap-1 text-xs px-2 py-0.5 rounded border transition-colors hover:bg-white/5"
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{
          color: task.status === 'complete' ? 'rgba(255,255,255,0.4)' : 'rgba(255,255,255,0.75)',
        }}
      >
        {task.label}
      </span>
...
export default function PropertyDesk() {
  ...
  const recentChanges = whatChangedFeed
    .filter((e) => e.propertyId === twin?.propertyId)
    .slice(0, 4);
      ...
      {twin ? (
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="text-sm" style={{ color: 'rgba(255,255,255,0.45)' }}>
                {twin.address}, {twin.city}, {twin.state} · {twin.propertyType.replace(/-/g, ' ')} ·
                Updated {relTime(twin.lastChangedAt)}
              </div>
            </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              ...
              {
                label: 'Readiness',
                value: `${twin.readinessScore}`,
                sub:
                  twin.readinessScore >= 80
                    ? 'Ready'
                    : twin.readinessScore >= 55
                      ? 'Needs attention'
                      : 'Not ready',
              },
            ].map((m) => (
            ...
            <div className="col-span-2 space-y-6">
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <DiligenceProgress
                    pct={twin.diligenceCompletionPct}
                    stage={twin.diligenceStage}
                  />
                </div>
                <div className="space-y-0.5">
                  {twin.diligenceTasks.map((t) => (
                  ...
                  {twin.approvals.map((a) => (
                    ...
                    >
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          className="text-xs px-3 py-1 rounded-md transition-colors hover:bg-white/5"
                          style={{
                      ...
                      <div key={e.id} className="flex gap-3 text-xs">
                        ...
                        <div>
                          ...
                          <span style={{ color: 'rgba(255,255,255,0.35)' }}>
                            {' '}
                            · {e.actor} ({e.actorRole}) · {relTime(e.at)}
                          </span>
                        </div>
                      </div>
                    ))}
                ...
                >
                  ...
                  <div className="space-y-2">
                    {recentChanges.map((e) => (
                      <div
                        key={e.id}
                        className="text-xs py-1.5 border-b last:border-b-0"
                        style={{ borderColor: 'rgba(255,255,255,0.04)' }}
                      >
                        <div
        ...
        <div className="flex-1 flex items-center justify-center">
          <EmptyState
            ...
            description="Choose a property from the list to view its twin."
            accentColor={ACCENT}
          />
        </div>
      )}
