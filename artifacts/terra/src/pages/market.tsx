    ...
    <div className="bg-terra-bg-tertiary border border-terra-border rounded-lg p-3 shadow-xl">
      ...
      {payload.map((item) => (
        <div key={item.name} className="flex items-center gap-2 text-xs">
          ...
          <span className="text-terra-text font-medium">
...
const comparableSales = [
  {
    address: '1800 Brickell Ave, Miami',
    price: 92000000,
    sqft: 245000,
    pricePerSqft: 376,
    type: 'Multifamily',
  ...
  {
    address: '300 Mission St, San Francisco',
    price: 145000000,
    sqft: 195000,
    pricePerSqft: 744,
    type: 'Office',
  ...
  {
    address: '2200 S Lamar Blvd, Austin',
    price: 38500000,
    sqft: 142000,
    pricePerSqft: 271,
    type: 'Mixed-Use',
  ...
  {
    address: '5500 Belt Line Rd, Dallas',
    price: 28900000,
    sqft: 320000,
    pricePerSqft: 90,
    type: 'Industrial',
  ...
  {
    address: '222 Broadway, Nashville',
    price: 16200000,
    sqft: 68000,
    pricePerSqft: 238,
    type: 'Retail',
...
export default function MarketPage() {
  ...
  const {
    ...
    refetch,
    isFetching,
  } = useStandardQuery<{ data: MarketIntelligence }>({
    ...
    <div className="p-6 space-y-6 overflow-auto">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => {
              refetch();
              if (!isFetching) toast.info('Refreshing market data…');
            }}
            disabled={isFetching}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="rounded-xl border border-terra-border bg-terra-surface/50 p-4 relative">
            <div className="flex items-center justify-between mb-1">
              <p className="text-xs text-terra-text-muted uppercase tracking-wider">
                30-Yr Fixed Rate
              </p>
            ...
            <p className="text-2xl font-display font-bold text-terra-text">
              {mortgageLoading
                ? '—'
                : mortgageRates?.rate30yr != null
                  ? `${mortgageRates.rate30yr.toFixed(2)}%`
                  : '—'}
            </p>
            {mortgageRates?.weeklyChange30yr != null && (
              <p
                className={cn(
                  ...
                  mortgageRates.weeklyChange30yr <= 0 ? 'text-terra-emerald' : 'text-terra-rose',
                )}
              >
                {mortgageRates.weeklyChange30yr <= 0 ? (
                  <TrendingDown className="w-3 h-3" aria-hidden="true" />
                ) : (
                  <TrendingUp className="w-3 h-3" aria-hidden="true" />
                )}
                {mortgageRates.weeklyChange30yr >= 0 ? '+' : ''}
                {mortgageRates.weeklyChange30yr}% WoW
              </p>
            )}
            {mortgageRates?.asOf && (
              <p className="text-[10px] text-terra-text-muted mt-1">FRED · {mortgageRates.asOf}</p>
            )}
          </div>
          ...
          <div className="rounded-xl border border-terra-border bg-terra-surface/50 p-4">
            ...
            <p className="text-2xl font-display font-bold text-terra-text">
              {mortgageLoading
                ? '—'
                : mortgageRates?.rate15yr != null
                  ? `${mortgageRates.rate15yr.toFixed(2)}%`
                  : '6.48%'}
            </p>
          ...
          <div className="rounded-xl border border-terra-border bg-terra-surface/50 p-4">
            ...
            <p className="text-2xl font-display font-bold text-terra-text">
              {blsLoading
                ? '—'
                : blsConstruction?.constructionEmployment
            ...
            {blsConstruction?.monthlyChange && (
              ...
              >
                {+blsConstruction.monthlyChange > 0 ? (
                  <TrendingUp className="w-3 h-3" aria-hidden="true" />
                ) : (
                  <TrendingDown className="w-3 h-3" aria-hidden="true" />
                )}
                {+blsConstruction.monthlyChange > 0 ? '+' : ''}
                {(+blsConstruction.monthlyChange).toLocaleString()} MoM
              </p>
            )}
            {blsConstruction?.period && (
              <p className="text-[10px] text-terra-text-muted mt-1">
                BLS · {blsConstruction.period}
              </p>
            )}
          </div>

          <div className="rounded-xl border border-terra-border bg-terra-surface/50 p-4">
            <p className="text-xs text-terra-text-muted uppercase tracking-wider mb-1">
              Avg Cap Rate Range
            </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <motion.div
          ...
          className="rounded-xl border border-terra-border bg-terra-surface/50 backdrop-blur-sm p-5"
        >
            ...
            <div className="h-72 flex flex-col gap-2 justify-end pb-2 animate-pulse">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="flex items-end gap-2">
                  <div
                    className="bg-terra-border/60 rounded-t"
                    style={{ height: `${30 + Math.round(i * 8)}px`, width: '100%' }}
                  />
                </div>
              ))}
              <div className="flex gap-2 mt-1">
                {Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="h-2 bg-terra-border/40 rounded flex-1" />
                ))}
              </div>
            </div>
          ) : (
            ...
            <div className="h-72 flex items-center justify-center animate-pulse">
              <div className="relative w-48 h-48">
                ...
                {[0, 60, 120, 180, 240, 300].map((deg, i) => (
                  <div
                    key={i}
                    className="absolute w-1.5 h-1.5 rounded-full bg-terra-border/60"
                    style={{
          ...
          <table className="w-full text-sm" >
            <thead>
              <tr className="border-b border-terra-border">
                ...
                <th className="text-right py-3 px-3 text-xs font-semibold text-terra-text-muted uppercase tracking-wider">
                  Median Price
                </th>
                <th className="text-right py-3 px-3 text-xs font-semibold text-terra-text-muted uppercase tracking-wider">
                  $/SqFt
                ...
                : displayMarketData.map((m: any, i: number) => {
                    ...
                    return (
                      <motion.tr
                        ...
                        className="border-b border-terra-border/50 hover:bg-terra-surface-hover transition-colors"
                      >
                        <td className="py-3 px-3 font-medium text-terra-text">{region}</td>
                        <td className="py-3 px-3 text-right text-terra-text">
                        ...
                        <td className="py-3 px-3 text-right">
                          <span
                            className={cn(
                              ...
                              yoyChange >= 0 ? 'text-terra-emerald' : 'text-terra-rose',
                            )}
                          >
                            {yoyChange >= 0 ? (
                              <TrendingUp className="w-3 h-3" aria-hidden="true" />
                        ...
                        <td className="py-3 px-3 text-right text-terra-text-secondary">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {comparableSales.map((sale, i) => (
            <motion.div
              ...
              className="p-4 rounded-lg border border-terra-border bg-terra-bg-secondary hover:border-terra-border-hover transition-all"
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-semibold px-2 py-0.5 rounded bg-terra-primary/10 text-terra-primary uppercase">
                  {sale.type}
                </span>
                <span className="text-xs text-terra-text-muted flex items-center gap-1">
      ...
      <motion.div
        ...
        className="rounded-xl border border-terra-border bg-terra-surface/50 backdrop-blur-sm overflow-hidden"
      >
        <div className="flex items-center justify-between p-5 border-b border-terra-border">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] px-1.5 py-0.5 rounded bg-terra-primary/10 text-terra-primary font-mono">
              {properties.length} assets
            </span>
          </div>
          <Link
            href="/property-map"
            className="text-xs text-terra-text-secondary hover:text-terra-text transition-colors flex items-center gap-1"
          >
        ...
        <div className="h-[360px] relative">
          {token ? (
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#08101e] gap-1 px-6 text-center">
              ...
              <p className="text-[10px] text-terra-text-muted/60">
