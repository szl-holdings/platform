          ...
          <div className="p-5 space-y-6">
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[9px] px-1.5 py-0.5 rounded font-bold" style={{ color: "#4B8BDB", background: "rgba(75,139,219,0.15)" }}>EXECUTE</span>
                <a href="/alloy" className="ml-auto text-[9px] flex items-center gap-1 hover:opacity-80" style={{ color: "#4B8BDB" }}><ExternalLink className="w-3 h-3" /> Open in Alloy</a>
              </div>
              <div className="space-y-2 ml-4">
                {chain.runs.map(r => (
                  <div key={r.id} className="rounded-lg p-3 flex items-center justify-between gap-4" style={{ background: "rgba(75,139,219,0.04)", border: "1px solid rgba(75,139,219,0.12)" }}>
                    <div>
                    ...
                    <span className="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded shrink-0" style={{
                      color: r.status === "completed" ? "#10b981" : r.status === "failed" ? "#ef4444" : r.status === "retried" ? "#8b5cf6" : "#4B8BDB",
