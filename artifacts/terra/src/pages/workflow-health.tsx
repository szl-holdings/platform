  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between text-[10px] mb-1">
        ...
        <span style={{ color: overSla ? '#ef4444' : '#10b981' }}>
          {formatDuration(latency)} / SLA {formatDuration(sla)}
        </span>
      </div>
        ...
        <div
          ...
          style={{
            ...
            background: overSla
              ...
              : 'linear-gradient(90deg, var(--gi-accent-blue), #10b981)',
            maxWidth: '100%',
          }}
        />
      </div>
      {overSla && (
        <div className="text-[9px] mt-0.5 font-medium" style={{ color: '#ef4444' }}>
          SLA BREACHED by {formatDuration(latency - sla)}
        </div>
      )}
    </div>
  );
...
export default function WorkflowHealth() {
  ...
  const degraded = WORKFLOWS.filter(
    (w) => w.status === 'degraded' || w.status === 'pending_approval',
  );
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div>
        <div className="flex items-center gap-2 mb-1">
          <Activity className="w-4 h-4" style={{ color: 'var(--gi-accent-blue)' }} />
          <span
        ...
        <p className="text-sm mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>
          Latency indicators, blocked steps, ownership gaps, and intervention status across all
          active workflows.
        </p>
      </div>
        ...
        {[
          {
            ...
            color: '#ef4444',
            desc: 'Require immediate intervention',
          },
          {
            label: 'Degraded / Pending',
            count: degraded.length,
            color: '#f59e0b',
            desc: 'Above SLA threshold',
          },
      ...
      <div className="space-y-4">
        {[...blocked, ...degraded, ...healthy].map((w) => (
          <div
            ...
            style={{
              borderColor:
                w.status === 'blocked' || w.status === 'escalated'
                  ? 'rgba(239,68,68,0.2)'
                  : w.status === 'degraded' || w.status === 'pending_approval'
