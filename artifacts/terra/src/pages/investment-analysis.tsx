      ...
      <div className="p-6 flex items-center justify-center min-h-[400px]">
        <div className="text-center space-y-3 max-w-xs">
          ...
          <p className="text-sm text-muted-foreground">
            Portfolio analysis is restricted to verified investors. Contact your account manager for
            access.
          </p>
        </div>
      </div>
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Portfolio performance, cap rates, and market comparables
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {investmentMetrics.map((m) => (
          <div key={m.label} className="rounded-xl border border-border bg-card p-5">
            ...
            <div
              className={`flex items-center gap-1 text-xs mt-1 ${m.trend === 'up' ? 'text-emerald-400' : 'text-red-400'}`}
            >
              {m.trend === 'up' ? (
          ...
          <tbody className="divide-y divide-border">
            ...
            <tr className="bg-muted/30 font-semibold">
              ...
              <td className="p-4 text-right text-sm font-mono text-emerald-400">
                {((totalNOI / totalValue) * 100).toFixed(1)}%
              </td>
              <td className="p-4 text-right text-sm">
                {Math.round(properties.reduce((a, p) => a + p.occupancy, 0) / properties.length)}%
              </td>
              <td className="p-4 text-right text-sm text-muted-foreground">
                {properties.reduce((a, p) => a + p.sqft, 0).toLocaleString()}
              </td>
            </tr>
          </tbody>
        </table>
            ...
            <div key={comp.area} className="rounded-lg border border-border p-4">
              ...
              <div className="space-y-1 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono">${comp.avgPrice}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Avg Rent:</span>
                  <span className="font-mono">${comp.avgRent}/sqft</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Cap Rate:</span>
                  <span className="font-mono text-emerald-400">{comp.capRate}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Vacancy:</span>
                  <span
                    className={`font-mono ${comp.vacancy > 10 ? 'text-red-400' : 'text-emerald-400'}`}
                  >
                    {comp.vacancy}%
                  </span>
                </div>
