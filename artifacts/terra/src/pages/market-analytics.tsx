...
const SUBMARKET_METRICS: SubmarketMetric[] = [
  {
    ...
    cap_rate_pct: 4.2,
    cap_rate_chg: 0.15,
    vacancy_rate_pct: 18.4,
    vacancy_rate_chg: 1.2,
    avg_rent_psf: 98,
    rent_chg: -3.2,
    absorption_units: -840,
    new_construction_units: 120,
    transaction_volume_m: 1240,
    market_score: 58,
  },
  {
    ...
    cap_rate_pct: 4.8,
    cap_rate_chg: 0.28,
    vacancy_rate_pct: 22.1,
    vacancy_rate_chg: 2.4,
    avg_rent_psf: 74,
    rent_chg: -6.1,
    absorption_units: -1240,
    new_construction_units: 0,
    transaction_volume_m: 820,
    market_score: 42,
  },
  {
    ...
    cap_rate_pct: 5.4,
    cap_rate_chg: -0.1,
    vacancy_rate_pct: 4.8,
    vacancy_rate_chg: -0.3,
    avg_rent_psf: 62,
    rent_chg: 4.8,
    absorption_units: 340,
    new_construction_units: 890,
    transaction_volume_m: 480,
    market_score: 78,
  },
  {
    submarket: 'DUMBO',
    ...
    vacancy_rate_chg: 0.4,
    avg_rent_psf: 54,
    rent_chg: 2.4,
    absorption_units: 120,
    new_construction_units: 240,
    transaction_volume_m: 320,
    market_score: 72,
  },
  {
    ...
    vacancy_rate_chg: -0.8,
    avg_rent_psf: 48,
    rent_chg: 6.2,
    absorption_units: 580,
    new_construction_units: 1240,
    transaction_volume_m: 380,
    market_score: 82,
  },
  {
    submarket: 'Astoria',
    ...
    avg_rent_psf: 42,
    rent_chg: 5.4,
    absorption_units: 240,
    new_construction_units: 380,
    transaction_volume_m: 180,
    market_score: 79,
  },
  {
    ...
    avg_rent_psf: 34,
    rent_chg: 8.4,
    absorption_units: 480,
    new_construction_units: 720,
    transaction_volume_m: 240,
    market_score: 84,
  },
  {
    submarket: 'Fordham',
    ...
    vacancy_rate_chg: -0.6,
    avg_rent_psf: 29,
    rent_chg: 7.2,
    absorption_units: 280,
    new_construction_units: 420,
    transaction_volume_m: 120,
    market_score: 77,
  },
];
...
export default function MarketAnalyticsPage() {
  ...
  const [selectedSubmarket, setSelectedSubmarket] = useState<SubmarketMetric | null>(
    SUBMARKET_METRICS[2],
  );
  ...
  const avgCapRate = (
    SUBMARKET_METRICS.reduce((s, m) => s + m.cap_rate_pct, 0) / SUBMARKET_METRICS.length
  ).toFixed(2);
  const avgVacancy = (
    SUBMARKET_METRICS.reduce((s, m) => s + m.vacancy_rate_pct, 0) / SUBMARKET_METRICS.length
  ).toFixed(1);
  const totalVolume = (
    SUBMARKET_METRICS.reduce((s, m) => s + m.transaction_volume_m, 0) / 1000
  ).toFixed(1);
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden" style={{ background: '#060c12' }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[9px]" style={{ color: 'rgba(200,160,96,0.4)' }}>
              Submarket-level cap rates · vacancy · rent trends · absorption · new construction
              pipeline
            </p>
          </div>
        </div>
      ...
      {/* KPI Strip */}
      <div
        className="px-5 py-3 border-b grid grid-cols-4 gap-3 shrink-0"
        style={{ borderColor: 'rgba(200,160,96,0.06)' }}
      >
        {[
          {
            label: 'Avg Cap Rate (NYC)',
            value: `${avgCapRate}%`,
            change: '+0.32% YoY',
            up: false,
            color: '#f97316',
          ...
          >
            <p className="text-[9px] mb-1" style={{ color: 'rgba(200,160,96,0.4)' }}>
              {label}
            </p>
            <p className="text-lg font-bold font-mono" style={{ color: '#f4e8d0' }}>
              {value}
            </p>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
          {/* Table Header */}
          <div
            className="px-5 py-2 border-b flex items-center gap-2 shrink-0"
            style={{ borderColor: 'rgba(200,160,96,0.06)' }}
          >
            <p
          ...
          <div className="flex-1 overflow-y-auto">
            {/* Column headers */}
            <div
              className="grid grid-cols-8 gap-2 px-4 py-2 text-[8px] font-medium uppercase tracking-wider border-b"
                ...
                >
                  <div className="col-span-2 min-w-0">
                    <p
                      ...
                      style={{ color: '#f4e8d0', textAlign: 'left' }}
                    >
                      {m.submarket}
                    </p>
                    <p className="text-[9px]" style={{ color: 'rgba(200,160,96,0.4)' }}>
                  ...
                  <div>
                    ...
                    <p
                      className="text-[8px]"
                      style={{ color: m.cap_rate_chg > 0 ? '#ef4444' : '#34d399' }}
                    >
                      {m.cap_rate_chg > 0 ? '+' : ''}
                      {m.cap_rate_chg}%
                    </p>
                  ...
                  <div>
                    <p
                      ...
                      style={{ color: m.vacancy_rate_pct > 10 ? '#ef4444' : '#f4e8d0' }}
                    >
                      {m.vacancy_rate_pct}%
                    </p>
                    <p
                      className="text-[8px]"
                      style={{ color: m.vacancy_rate_chg > 0 ? '#ef4444' : '#34d399' }}
                    >
                      {m.vacancy_rate_chg > 0 ? '+' : ''}
                      {m.vacancy_rate_chg}pp
                    </p>
                  </div>
                  <div>
                    <p className="text-[11px] font-mono" style={{ color: '#f4e8d0' }}>
                    ...
                    <p
                      className="text-[8px]"
                      style={{ color: m.rent_chg >= 0 ? '#34d399' : '#ef4444' }}
                    >
                      {m.rent_chg >= 0 ? '+' : ''}
                      {m.rent_chg}%
                    </p>
                  ...
                  <div>
                    <p
                      ...
                      style={{ color: m.absorption_units >= 0 ? '#34d399' : '#ef4444' }}
                    >
                      {m.absorption_units >= 0 ? '+' : ''}
                      {m.absorption_units.toLocaleString()}
                    </p>
                    <p className="text-[8px]" style={{ color: 'rgba(200,160,96,0.3)' }}>
                      units
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] font-mono" style={{ color: '#f4e8d0' }}>
                      ${m.transaction_volume_m}M
                    </p>
                  </div>
                </button>
              );
        ...
        {/* Right Panel — Charts */}
        <div
          className="w-[340px] shrink-0 border-l overflow-y-auto p-4 space-y-4"
        ...
        >
          ...
          {selectedSubmarket && (
            <div
              className="rounded-xl border p-3"
              style={{ borderColor: 'rgba(200,160,96,0.15)', background: 'rgba(200,160,96,0.05)' }}
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <div
                  ...
                  style={{
                    background: `${SCORE_COLOR(selectedSubmarket.market_score)}15`,
                    color: SCORE_COLOR(selectedSubmarket.market_score),
                  }}
                >
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  {
                    label: 'Rent Change',
                    value: `${selectedSubmarket.rent_chg >= 0 ? '+' : ''}${selectedSubmarket.rent_chg}%`,
                  },
                  {
                    label: 'New Supply',
                    value: `${selectedSubmarket.new_construction_units.toLocaleString()} units`,
                  },
                  ...
                  <div key={label}>
                    ...
                    <p className="text-[11px] font-medium" style={{ color: '#f4e8d0' }}>
                      {value}
                    </p>
                  </div>
                ))}
