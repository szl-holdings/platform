  ...
  return (
    <div className="p-6 space-y-6 overflow-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Annual Revenue", value: formatCurrency(portfolioSummary.totalMonthlyRevenue * 12), change: "+8.4%", up: true, icon: DollarSign },
          { label: "Annual NOI", value: formatCurrency(portfolioSummary.totalAnnualNOI), change: "+6.1%", up: true, icon: TrendingUp },
          { label: "Total Sq Ft", value: `${(portfolioSummary.totalSqft / 1e6).toFixed(2)}M`, change: "+0%", up: true, icon: Building2 },
          { label: "Total Units", value: portfolioSummary.totalUnits.toString(), change: "+0%", up: true, icon: Users },
        ].map((item, i) => (
          <motion.div key={item.label} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="p-4 rounded-xl border border-terra-border bg-terra-surface/50">
            <div className="flex items-center justify-between mb-2">
              <item.icon className="w-4 h-4 text-terra-text-muted" />
            ...
            <p className="text-[10px] text-terra-text-muted uppercase tracking-wider">{item.label}</p>
            <p className="text-xl font-display font-bold text-terra-text">{item.value}</p>
          </motion.div>
        ))}
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-xl border border-terra-border bg-terra-surface/50 p-5">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={revenueHistory}>
                ...
                <Line type="monotone" dataKey="occupancy" name="Occupancy" stroke="#3b82f6" strokeWidth={2.5} dot={{ r: 3, fill: "#3b82f6" }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="rounded-xl border border-terra-border bg-terra-surface/50 p-5">
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={occupancyByProperty} layout="vertical" margin={{ left: 10 }}>
                ...
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="occupancy" name="Occupancy" fill="#3b82f6" radius={[0, 4, 4, 0]} barSize={12} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
