...
import { ConfidenceBadge, formatCurrency, RiskBadge } from '@/components/brokerage-ui';
import { type Prediction, predictions } from '@/data/brokerage';
  ...
  return (
    <div className="rounded-xl border border-terra-border bg-terra-surface/50 overflow-hidden">
      <button
        ...
        className="w-full px-5 py-4 flex items-center justify-between hover:bg-terra-surface-hover transition-colors"
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-[10px] text-terra-text-muted">
              Probability · Rationale · Assumptions · Risks · Next Action
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          {open ? (
            <ChevronUp className="w-4 h-4 text-terra-text-muted" />
          ) : (
            <ChevronDown className="w-4 h-4 text-terra-text-muted" />
          )}
        </div>
      </button>
      {open && (
        <div className="px-5 pb-5 border-t border-terra-border space-y-4 pt-4">
          <div>
            <p className="text-[10px] font-semibold text-terra-text-muted uppercase tracking-wider mb-1.5">
              Rationale
            </p>
            <p className="text-xs text-terra-text-secondary leading-relaxed bg-terra-surface border border-terra-border rounded-lg p-3">
              {cl.rationale}
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <div className="rounded-lg bg-terra-primary/10 border border-terra-primary/20 p-3">
                <p className="text-xs text-terra-primary font-semibold leading-relaxed">
                  → {cl.nextAction}
                </p>
                <p className="text-[10px] text-terra-text-muted mt-1.5">
                  {cl.nextActionOwner} · Due {cl.nextActionDue}
                </p>
              </div>
            </div>
          </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-[10px] text-terra-text-muted">
              Price band · Over/under-pricing risk · Recommendation
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      {open && (
        <div className="px-5 pb-5 border-t border-terra-border space-y-4 pt-4">
          <div className="flex items-center gap-4">
            <div>
              ...
              <p className="text-sm font-semibold text-terra-text">
                {formatCurrency(pc.band.low)} – {formatCurrency(pc.band.high)}
              </p>
            </div>
            <div>
          ...
          <div>
            ...
            <p className="text-xs text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-3 font-semibold">
              {pc.recommendation}
            </p>
          </div>
          <div>
...
function StallRiskPanel({ pred }: { pred: Prediction }) {
  ...
  return (
    ...
    >
      <button
        onClick={() => setOpen(!open)}
        className="w-full px-5 py-4 flex items-center justify-between hover:bg-terra-surface-hover transition-colors bg-terra-surface/50"
      >
        <div className="flex items-center gap-3">
          <AlertTriangle
            className={cn(
          ...
          <div className="text-left">
            ...
            <p className="text-[10px] text-terra-text-muted">
              Bottleneck · Delay window · Recovery recommendation
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
        ...
        <div className="px-5 pb-5 border-t border-terra-border/50 space-y-4 pt-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-sm font-semibold text-terra-text capitalize">
                {sr.bottleneckStage}
              </p>
            </div>
            <div>
              ...
              <p className="text-sm font-semibold text-terra-text">
                {sr.delayWindowDays === 0 ? 'Already delayed' : `${sr.delayWindowDays} days`}
              </p>
            </div>
          </div>
          <div>
            ...
            <p className="text-xs text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded-lg p-3 font-semibold leading-relaxed">
              {sr.recoveryRecommendation}
            </p>
          </div>
          <div>
            <p className="text-[10px] font-semibold text-terra-text-muted uppercase tracking-wider mb-1.5">
    ...
    <div className="rounded-xl border border-terra-border bg-terra-surface/50 p-5">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex items-center gap-2 ml-auto">
          <span className={cn('text-xs font-semibold', urgencyConfig.color)}>
            {urgencyConfig.label}
          </span>
          <div className={cn('w-24 h-1.5 bg-terra-border rounded-full overflow-hidden')}>
            <div
              className={cn(
                'h-full rounded-full',
                dh.score >= 75 ? 'bg-emerald-500' : dh.score >= 50 ? 'bg-amber-500' : 'bg-rose-500',
              )}
      ...
      <div className="flex items-center gap-3 mb-2">
        <div>
          ...
          <p className="text-xs text-terra-text-muted">
            Agent: {pred.agentName} · Generated {new Date(pred.generatedAt).toLocaleDateString()}
          </p>
        </div>
      </div>
    ...
    <div className="p-6 space-y-6 overflow-auto">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-sm text-terra-text-secondary">
          ...
          output shows confidence, rationale, assumptions, and next action.
        </p>
      </motion.div>
      ...
      <div className="flex items-center gap-2 px-4 py-3 rounded-xl border border-violet-500/30 bg-violet-500/10">
        ...
        <p className="text-xs text-violet-300">
          <span className="font-semibold">Alloy Explainability Standard:</span> Every AI output
          ...
          <button
            ...
            className={cn(
              ...
              selected === i
                ...
                : 'border-terra-border bg-terra-surface/50 hover:border-terra-border-hover',
            )}
          >
            <p className="text-xs font-semibold text-terra-text truncate">
              {pred.dealAddress.split(',')[0]}
            </p>
            ...
            <div className="flex items-center gap-2 mt-1.5">
              <span
                className={cn(
                  'text-xs font-bold',
                  pred.closeLikelihood.probability >= 0.7
                    ? 'text-emerald-400'
                    : pred.closeLikelihood.probability >= 0.5
                      ? 'text-amber-400'
                      : 'text-rose-400',
                )}
              >
                {Math.round(pred.closeLikelihood.probability * 100)}% close
              </span>
