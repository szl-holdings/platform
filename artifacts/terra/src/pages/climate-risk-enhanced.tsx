...
function PropertyCard({
  ...
  const riskColor =
    property.overallRiskScore >= 80
      ? 'text-red-400'
      : property.overallRiskScore >= 65
        ? 'text-orange-400'
        : property.overallRiskScore >= 50
          ? 'text-amber-400'
          : 'text-sky-400';
  return (
    <motion.div
    ...
    >
      <div className="flex items-start gap-3">
        <div className="w-12 h-12 rounded-xl bg-white/4 border border-white/6 flex flex-col items-center justify-center flex-shrink-0">
          <span
            className={cn('text-lg font-black', GRADE_COLOR[property.overallGrade] ?? 'text-white')}
          >
            {property.overallGrade}
          </span>
          ...
          <div className="flex items-center gap-3 mt-2">
            <span className="text-[10px] text-white/30">
              ...
              <span className="text-amber-400">
                {formatCurrency(property.annualInsurance)}/yr (+{property.insuranceAdjustment}%)
              </span>
            </span>
            <span className="text-[10px] text-white/30">
              Haircut: <span className="text-red-400">−{property.valuationHaircut}%</span>
            </span>
          </div>
          <div className="flex gap-1.5 mt-2 flex-wrap">
            {property.hazards
              .filter((h) => ['Critical', 'High'].includes(h.current))
              .map((h) => {
                ...
                return (
                  <div
                    key={h.type}
                    className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-red-500/8 border border-red-500/15 text-red-400 text-[9px]"
                  >
      ...
      <div className="p-5 border-b border-white/6">
        ...
        <div className="grid grid-cols-4 gap-2 mt-4">
          {[
            {
              label: 'Risk Score',
              value: property.overallRiskScore.toString(),
              color:
                property.overallRiskScore >= 80
                  ? 'text-red-400'
                  : property.overallRiskScore >= 65
            ...
            {
              label: 'Climate Grade',
              value: property.overallGrade,
              color: GRADE_COLOR[property.overallGrade] ?? 'text-white',
            },
            {
              label: 'Value Haircut',
              value: `−${property.valuationHaircut}%`,
              color: 'text-red-400',
            },
            {
              label: '30yr Exp. Loss',
              value: formatCurrency(property.thirtyYearExpectedLoss),
          ...
          {property.hazards.map((h) => {
            ...
            return (
              <div key={h.type} className="space-y-1">
                ...
                <TimelineArrow
                  current={h.current}
                  y2030={h.projected2030}
                  y2050={h.projected2050}
                />
...
export default function ClimateRiskEnhanced() {
  ...
  const PROPERTIES: ClimateProperty[] =
    (portfolioData?.properties as ClimateProperty[] | undefined) ?? [];

  const [selected, setSelected] = useState<ClimateProperty | null>(null);
  const [gradeFilter, setGradeFilter] = useState<string>('all');

  const filtered = PROPERTIES.filter(
    (p) => gradeFilter === 'all' || p.overallGrade.startsWith(gradeFilter),
      ...
      <div className="min-h-screen p-6" style={{ background: '#0a0c10' }}>
        <div className="max-w-5xl mx-auto">
          <Link href={`/property/${propertyId}`}>
            <span
              className="inline-flex items-center gap-1 text-xs mb-5 cursor-pointer transition-colors"
          ...
          <p className="text-xs mb-6" style={{ color: 'rgba(255,255,255,0.4)' }}>
            30-year physical climate risk — flood, fire, heat, storm, sea-level &amp; seismic
            scoring for property <code className="text-orange-400">{propertyId}</code>
          </p>
                ...
                {[
                  {
                    ...
                    value: d.overallRiskScore.toString(),
                    color:
                      d.overallRiskScore >= 75
                        ? '#ef4444'
                        : d.overallRiskScore >= 55
                          ? '#f97316'
                          : '#fbbf24',
                    sub: `Grade ${d.overallGrade}`,
                  },
                  {
                    label: 'Annual Insurance',
                    value: formatCurrency(d.annualInsurance),
                    color: '#fbbf24',
                    sub: `+${d.insuranceAdjustment}% adj.`,
                  },
                  {
                    label: 'Valuation Haircut',
                    value: `−${d.valuationHaircut}%`,
                    color: '#ef4444',
                    sub: 'climate-adjusted',
                  },
                  {
                    label: '30yr Expected Loss',
                    value: formatCurrency(d.thirtyYearExpectedLoss),
                    color: '#ef4444',
                    sub: 'NPV exposure',
                  },
                ].map((m) => (
              ...
              >
                ...
                >
                  <p className="text-sm font-semibold text-white">
                    Hazard Assessment — Current &amp; Projected
                  </p>
                </div>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-white/40 mt-1">
                30-year physical climate risk assessment — flood, fire, heat, storm, sea-level per
                property with insurance and valuation impact
              </p>
            </div>
          </div>
            ...
            {[
              {
                ...
                value: portfolio.critical.toString(),
                color: 'text-red-400',
              },
              {
                label: 'High Risk Assets',
                value: portfolio.high.toString(),
                color: 'text-orange-400',
              },
              {
                label: 'Total Insurance Cost',
                value: formatCurrency(portfolio.totalInsurance),
                color: 'text-amber-400',
                sub: 'annual',
              },
              {
                label: '30yr Expected Loss',
                value: formatCurrency(portfolio.totalExpectedLoss),
                color: 'text-red-400',
                sub: 'portfolio',
              },
              {
                label: 'Adaptation Capex Needed',
                value: formatCurrency(portfolio.totalAdaptationCost),
                color: 'text-sky-400',
                sub: 'total',
              },
            ].map((m) => (
        ...
        <div className="flex-shrink-0 px-6 py-3 border-b border-white/6 flex items-center gap-2">
            ...
            <button
              ...
              className={cn(
                ...
                gradeFilter === g
                  ...
                  : 'text-white/30 border-white/8 hover:border-white/15',
              )}
            >
          ...
          <div className="ml-auto flex items-center gap-3 text-[10px] text-white/30">
            <span className="flex items-center gap-1">
              <span className="text-emerald-400">Negligible / Low</span>
            </span>
            <span className="flex items-center gap-1">
              <span className="text-amber-400">Medium</span>
            </span>
            <span className="flex items-center gap-1">
              <span className="text-red-400">High / Critical</span>
            </span>
          </div>
        </div>
