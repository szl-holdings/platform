...
const PROPERTIES: AppealProperty[] = [
  {
    ...
    notes:
      'Assessor used income capitalization at 5.5% cap — market cap rate is 7.0%+. Strong comparable evidence. Recommend immediate filing.',
    comparables: [
      {
        address: '900 Commerce St, Dallas',
        saleDate: '2025-11-15',
        salePrice: 20_200_000,
        sqft: 82000,
        pricePerSqft: 246,
        distance: '0.4 mi',
      },
      {
        address: '1400 Preston Rd, Dallas',
        saleDate: '2025-08-22',
        salePrice: 18_900_000,
        sqft: 78500,
        pricePerSqft: 241,
        distance: '0.8 mi',
      },
      {
        address: '750 N St Paul, Dallas',
        saleDate: '2025-06-10',
        salePrice: 22_400_000,
        sqft: 91000,
        pricePerSqft: 246,
        distance: '1.1 mi',
      },
      {
        address: '1801 N Lamar, Dallas',
        saleDate: '2025-04-05',
        salePrice: 16_800_000,
        sqft: 72000,
        pricePerSqft: 233,
        distance: '1.4 mi',
      },
  ...
  },
  {
    ...
    notes:
      'Petition filed 3/15/26. Hearing scheduled Q2 2026. Vacancy rates elevated — income approach supports lower value.',
    comparables: [
      {
        address: '200 Collins Ave, Miami Beach',
        saleDate: '2025-10-01',
        salePrice: 8_800_000,
        sqft: 13200,
        pricePerSqft: 667,
      ...
      {
        address: '400 Lincoln Rd, Miami Beach',
        saleDate: '2025-07-18',
      ...
      {
        address: '1120 Washington Ave, Miami Beach',
        saleDate: '2025-05-20',
        salePrice: 7_400_000,
        sqft: 11500,
        pricePerSqft: 643,
  ...
  },
  {
    ...
    appealStrength: 'weak',
    notes:
      'AVM suggests property is slightly under-assessed. No appeal warranted. Prop 13 basis lock provides protection — no action needed.',
    comparables: [
      {
        address: '2600 Pacific Ave, San Francisco',
        saleDate: '2025-09-14',
        salePrice: 14_800_000,
        sqft: 17200,
        pricePerSqft: 860,
        distance: '0.3 mi',
      },
  ...
  },
  {
    ...
    appealStrength: 'strong',
    notes:
      'Office vacancy rate at 25% negatively impacts income. Assessor appears to use pre-COVID income projections. Strong case.',
    comparables: [
      {
        address: '200 W 6th St, Austin',
        saleDate: '2025-12-01',
        salePrice: 25_600_000,
        sqft: 58000,
        pricePerSqft: 441,
        distance: '0.3 mi',
      },
      {
        address: '700 Lavaca St, Austin',
        saleDate: '2025-09-30',
        salePrice: 29_100_000,
        sqft: 66000,
        pricePerSqft: 441,
        distance: '0.5 mi',
      },
      {
        address: '301 Congress Ave, Austin',
        saleDate: '2025-07-22',
        salePrice: 22_400_000,
        sqft: 52000,
        pricePerSqft: 431,
...
export default function TaxAppealPage() {
  ...
  const seedMutation = useStandardMutation({
    mutationFn: async () => {
      for (const p of PROPERTIES) {
        await api.taxAppeals.create({
          ...
          avmValue: p.avmValue,
          taxRate: p.taxRate,
          overAssessedPct: p.overAssessedPct,
          annualTax: p.annualTax,
          potentialSavings: p.potentialSavings,
          appealDeadline: p.appealDeadline,
          appealStatus: p.appealStatus,
          juris: p.juris,
          comparables: p.comparables as unknown as Array<Record<string, unknown>>,
          appealStrength: p.appealStrength,
          notes: p.notes,
          isDemo: true,
        });
      }
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['terra-tax-appeals'] });
    },
  });

  const isLive = !isLoading && !isError && apiData && apiData.dataMode === 'live';
  const properties: AppealProperty[] = isLive
    ? (apiData.properties as unknown as AppealProperty[])
    : PROPERTIES;
  ...
  const filtered = properties.filter(
    (p) =>
      filter === 'all' ||
      p.appealStatus === filter ||
      (filter === 'eligible' && p.appealStatus === 'eligible'),
  );

  const selected = properties.find((p) => p.id === effectiveId);
  const totalSavings = properties
    .filter((p) => ['eligible', 'filed'].includes(p.appealStatus))
    .reduce((s, p) => s + p.potentialSavings, 0);
    ...
    <div className="space-y-4 max-w-[1400px]">
      <div>
        ...
        <p className="text-[10px] font-mono" style={{ color: DS.text.muted }}>
          Assessed value vs. AVM comparison · over-assessment flagging · comp evidence builder ·
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          {
            label: 'Avg Over-Assessment',
            value: `${properties.filter((p) => p.overAssessedPct > 0).length > 0 ? (properties.filter((p) => p.overAssessedPct > 0).reduce((s, p) => s + p.overAssessedPct, 0) / properties.filter((p) => p.overAssessedPct > 0).length).toFixed(1) : '0'}%`,
            color: DS.accent.red,
          },
        ].map((m) => (
          <div
      ...
      <div className="flex items-center gap-2">
        {(['all', 'eligible', 'filed'] as const).map((f) => (
          <button
            ...
            style={{
              background: filter === f ? DS.accent.gold : DS.surface,
              color: filter === f ? '#000' : DS.text.secondary,
              border: `1px solid ${filter === f ? DS.accent.gold : DS.border}`,
            }}
          >
          ...
          {filtered.map((p) => {
            ...
            return (
              <motion.div
                ...
                style={{
                  borderColor: selectedId === p.id ? DS.accent.gold : DS.border,
                  background: selectedId === p.id ? 'rgba(184,148,60,0.04)' : DS.surface,
                }}
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <span
                    ...
                    style={{ color: statusCfg.color, background: `${statusCfg.color}12` }}
                  >
                    {statusCfg.label}
                  </span>
                </div>
                ...
                <div className="flex items-start justify-between gap-3 mb-4">
                  <div>
                    ...
                    <p className="text-[10px] mt-0.5" style={{ color: DS.text.tertiary }}>
                      {selected.address} · {selected.juris}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span
                      ...
                      style={{
                        color: STRENGTH_CONFIG[selected.appealStrength].color,
                        background: `${STRENGTH_CONFIG[selected.appealStrength].color}12`,
                      }}
                    >
                    ...
                    <BarChart
                      data={[
                        ...
                        { name: 'AVM Estimate', value: selected.avmValue, fill: DS.accent.blue },
                        {
                          name: 'Comp Avg',
                          value:
                            selected.comparables.reduce((s, c) => s + c.salePrice, 0) /
                            selected.comparables.length,
                          fill: DS.accent.green,
                        },
                      ]}
                  ...
                  {selected.comparables.map((comp, i) => (
                    <div key={i} className="grid grid-cols-5 gap-2 px-4 py-2.5 items-center">
                      <div className="col-span-2">
                        <p className="text-[10px] font-medium" style={{ color: DS.text.secondary }}>
                          {comp.address}
                        </p>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setShowPacket(false)}>
                  <X className="w-5 h-5" style={{ color: DS.text.muted }} />
                </button>
              </div>
              ...
              <button
                ...
                style={{ background: DS.accent.green, color: '#fff' }}
              >
