...
function OpportunityCard({
  ...
  const cfg = DISTRESS_COLORS[property.distressType] ?? {
    color: 'text-terra-text-muted',
    bg: 'bg-terra-surface border-terra-border',
  };
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-terra-text-muted">
            {property.borough} · {property.county} · {property.zipCode}
          </p>
        ...
        <button
          ...
          className={cn(
            ...
            saved
              ...
              : 'text-terra-text-muted hover:text-terra-text hover:bg-terra-surface',
          )}
        >
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div>
          ...
          <p className="text-sm font-bold font-mono text-terra-text">
          ...
          <div>
            ...
            <p
              className={cn(
                ...
                property.equityPercent >= 25
                  ? 'text-emerald-400'
                  : property.equityPercent >= 10
                    ? 'text-amber-400'
                    : 'text-red-400',
              )}
            >
      ...
      <p className="text-[11px] text-terra-text-secondary leading-relaxed mb-3 line-clamp-2">
      ...
      <div className="flex items-center gap-1 mb-3">
        ...
        <span
          className={cn(
            'text-[9px] px-1 py-0.5 rounded ml-auto',
        ...
        <button
          ...
          className={cn(
            ...
            saved
              ...
              : 'bg-terra-surface border-terra-border text-terra-text-secondary hover:bg-terra-surface-hover',
          )}
        >
    ...
    <div className="p-6 space-y-6 overflow-auto">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-terra-text-secondary">
              Investment-grade distress opportunities — score-filtered, watchlist-ready, bulk-export
              for investor packets
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg border border-terra-border bg-terra-surface text-sm text-terra-text-secondary hover:bg-terra-surface-hover transition-colors"
            >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          {
            label: 'Total Value',
            value: summary.totalValue ? formatCurrency(summary.totalValue) : '—',
            icon: DollarSign,
      ...
      <div className="flex flex-wrap gap-3 p-4 rounded-xl border border-terra-border bg-terra-surface/30">
        <div className="flex items-center gap-2">
          <label className="text-[10px] font-semibold text-terra-text-muted uppercase tracking-wider whitespace-nowrap">
            Min Score
          </label>
          <select
            value={minScore}
        ...
        <div className="ml-auto flex items-center gap-2">
          {Object.entries(topByBorough)
            .slice(0, 5)
            .map(([b, count]) => (
              <div
                key={b}
                className="flex items-center gap-1 text-[10px] text-terra-text-muted bg-terra-surface border border-terra-border px-2 py-1 rounded"
              >
      ...
      {!isLoading && properties.length === 0 && (
        <div className="rounded-xl border border-dashed border-terra-border bg-terra-surface/20 p-12 text-center">
          ...
          <p className="text-sm font-semibold text-terra-text mb-1">
            No opportunities at this threshold
          </p>
          <p className="text-xs text-terra-text-muted">
            Try lowering the minimum score or expanding filters
          </p>
        </div>
      )}
      ...
      {!isLoading && properties.length > 0 && (
        <>
          <div className="flex items-center justify-between">
            <p className="text-xs text-terra-text-muted">
              {properties.length} investment-grade{' '}
              {properties.length === 1 ? 'property' : 'properties'} · score ≥ {minScore}
            </p>
            <button
              onClick={() => {
