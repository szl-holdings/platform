...
const DEMO_LEASES: ExtractedLease[] = [
  {
    ...
    options: ["2 × 5-year renewal options at 95% of then-market rent", "Right of first offer on contiguous space (Suite 1300)"],
    cam: 8500,
    tiAllowance: 1200000,
    ...
    confidence: 94,
    flags: [
      { field: "TI Allowance", issue: "TI allowance disbursement schedule not specified — amend to require monthly draws with lien waivers", severity: "warning" },
      { field: "Termination Option", issue: "Early termination fee of only 6 months may undercut lender hold period — review with capital markets", severity: "warning" },
    ],
  },
  {
    id: "lease-002",
    ...
    confidence: 87,
    flags: [
      { field: "Base Rent", issue: "Rent is 30% below current market ($2.85/sqft) — renewal negotiation opportunity if tenant exercises option", severity: "warning" },
      { field: "Escalation", issue: "2% annual escalation below projected CPI — effective rent will erode in real terms", severity: "info" },
      { field: "TI Allowance", issue: "No TI funded for renewal — tenant may require capital to exercise option", severity: "info" },
    ],
  },
  {
    id: "lease-003",
...
const FIELD_GROUPS: FieldGroup[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "cam", label: "CAM/Mo", format: (v) => Number(v) > 0 ? `$${Number(v).toLocaleString()}` : "—" },
      { key: "tiAllowance", label: "TI Allowance", format: (v) => Number(v) > 0 ? `$${(Number(v) / 1000).toFixed(0)}K` : "—" },
      { key: "securityDeposit", label: "Security Deposit", format: (v) => `$${Number(v).toLocaleString()}` },
    ],
  },
  {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <div className="relative w-8 h-8">
        ...
        <span className="absolute inset-0 flex items-center justify-center text-[8px] font-bold font-mono" style={{ color }}>{score}</span>
      </div>
      <span className="text-[9px] font-medium" style={{ color: DS.text.muted }}>AI Confidence</span>
    </div>
  );
}
...
function LeaseCard({ lease, selected, onClick }: { lease: ExtractedLease; selected: boolean; onClick: () => void }) {
  const daysToExpiry = Math.ceil((new Date(lease.expirationDate).getTime() - Date.now()) / 86400000);
  const yearsLeft = (daysToExpiry / 365).toFixed(1);
  const urgency = daysToExpiry < 365 ? "high" : daysToExpiry < 730 ? "medium" : "low";
  const urgencyColor = urgency === "high" ? DS.accent.red : urgency === "medium" ? DS.accent.gold : DS.accent.green;
  ...
  return (
    <motion.div
      ...
      style={{ borderColor: selected ? DS.accent.gold : DS.border, background: selected ? "rgba(184,148,60,0.04)" : DS.surface }}
    >
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="min-w-0 flex-1">
          <p className="text-xs font-semibold truncate" style={{ color: DS.text.primary }}>{lease.tenant}</p>
          <p className="text-[10px] mt-0.5 truncate" style={{ color: DS.text.tertiary }}>{lease.documentName}</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 mt-3">
        <div>
          <p className="text-[8px] uppercase tracking-wider" style={{ color: DS.text.muted }}>Base Rent</p>
          <p className="text-[11px] font-bold font-mono" style={{ color: DS.accent.gold }}>${lease.baseRent.toLocaleString()}/mo</p>
        </div>
        <div>
          <p className="text-[8px] uppercase tracking-wider" style={{ color: DS.text.muted }}>Size</p>
          <p className="text-[11px] font-bold font-mono" style={{ color: DS.text.primary }}>{lease.sqft.toLocaleString()} SF</p>
        </div>
        <div>
          <p className="text-[8px] uppercase tracking-wider" style={{ color: DS.text.muted }}>Expires</p>
          <p className="text-[11px] font-bold font-mono" style={{ color: urgencyColor }}>{yearsLeft}yr</p>
        </div>
      </div>

      {lease.flags.length > 0 && (
        <div className="flex items-center gap-1.5 mt-3 pt-2" style={{ borderTop: `1px solid ${DS.border}` }}>
          <AlertTriangle className="w-3 h-3 shrink-0" style={{ color: DS.accent.gold }} />
          <span className="text-[9px]" style={{ color: DS.text.muted }}>{lease.flags.length} review item{lease.flags.length > 1 ? "s" : ""}</span>
        </div>
      )}
    </motion.div>
  );
    ...
    <div className="space-y-3">
      <div className="rounded-xl border p-4" style={{ borderColor: DS.border, background: DS.surface }}>
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            <h3 className="text-sm font-bold" style={{ color: DS.text.primary }}>{lease.tenant}</h3>
            <p className="text-xs mt-0.5" style={{ color: DS.text.tertiary }}>{lease.premises} · {lease.propertyAddress}</p>
          </div>
          <button
            className="flex items-center gap-1.5 text-[10px] px-2.5 py-1.5 rounded-lg font-semibold"
            style={{ background: "rgba(58,122,212,0.1)", border: "1px solid rgba(58,122,212,0.2)", color: DS.accent.blue }}
          >
        ...
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            { label: "WALE", value: `${((new Date(lease.expirationDate).getTime() - Date.now()) / (365.25 * 86400000)).toFixed(1)}yr` },
            { label: "AI Conf.", value: `${lease.confidence}%` },
          ].map(m => (
            <div key={m.label} className="rounded-lg p-2.5" style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${DS.border}` }}>
              <p className="text-[8px] uppercase tracking-wider" style={{ color: DS.text.muted }}>{m.label}</p>
              <p className="text-sm font-bold font-mono mt-0.5" style={{ color: DS.accent.gold }}>{m.value}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {lease.flags.length > 0 && (
        <div className="rounded-xl border overflow-hidden" style={{ borderColor: "rgba(184,148,60,0.2)", background: "rgba(184,148,60,0.04)" }}>
          <div className="flex items-center gap-2 px-4 py-2.5 border-b" style={{ borderColor: "rgba(184,148,60,0.1)" }}>
            <AlertTriangle className="w-3.5 h-3.5" style={{ color: DS.accent.gold }} />
            <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: DS.accent.gold }}>Review Items</span>
          </div>
          <div className="divide-y" style={{ borderColor: "rgba(184,148,60,0.1)" }}>
            {lease.flags.map((f, i) => (
              <div key={i} className="px-4 py-3">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[9px] font-bold px-1.5 py-0.5 rounded" style={{ color: f.severity === "error" ? DS.accent.red : f.severity === "warning" ? DS.accent.gold : DS.accent.blue, background: `${f.severity === "error" ? DS.accent.red : f.severity === "warning" ? DS.accent.gold : DS.accent.blue}12` }}>
                    {f.field}
                  </span>
                </div>
                <p className="text-[10px] leading-relaxed" style={{ color: DS.text.secondary }}>{f.issue}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {FIELD_GROUPS.map(group => (
        <div key={group.label} className="rounded-xl border overflow-hidden" style={{ borderColor: DS.border, background: DS.surface }}>
          <button
          ...
          >
            ...
            <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: `${DS.accent.gold}99` }}>{group.label}</span>
            <ChevronDown className="w-3.5 h-3.5 ml-auto transition-transform" style={{ color: DS.text.muted, transform: expanded === group.label ? "rotate(180deg)" : "rotate(0deg)" }} />
          </button>
          <AnimatePresence>
            {expanded === group.label && (
              <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden">
                <div className="px-4 pb-4 space-y-2.5">
                  {group.fields.map(f => (
                    <div key={f.key} className="flex items-start gap-3 py-1.5" style={{ borderTop: `1px solid ${DS.border}` }}>
                      <span className="text-[9px] uppercase tracking-wider shrink-0 w-28" style={{ color: DS.text.muted }}>{f.label}</span>
                      <span className="text-[10px] flex-1" style={{ color: DS.text.secondary }}>
                        {f.format ? f.format(lease[f.key] as string | number) : String(lease[f.key] ?? "—")}
                      </span>
                    </div>
                  ))}
                  {group.label === "Escalations & Options" && lease.options.map((opt, i) => (
                    <div key={i} className="flex items-start gap-3 py-1.5" style={{ borderTop: `1px solid ${DS.border}` }}>
                      <span className="text-[9px] uppercase tracking-wider shrink-0 w-28" style={{ color: DS.text.muted }}>Option {i + 1}</span>
                      <span className="text-[10px] flex-1" style={{ color: DS.text.secondary }}>{opt}</span>
                    </div>
                  ))}
                </div>
              </motion.div>
    ...
    <div className="space-y-4 max-w-[1400px]">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {processing && (
            <div className="flex items-center gap-1.5 text-[10px]" style={{ color: DS.accent.gold }}>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: "Total Sq Footage", value: `${(totalSqft / 1000).toFixed(0)}K SF`, color: DS.accent.blue },
          { label: "Annual Rent", value: `$${(totalAnnualRent / 1000000).toFixed(1)}M`, color: DS.accent.green },
          { label: "Review Items", value: flaggedCount.toString(), color: flaggedCount > 0 ? DS.accent.gold : DS.text.muted },
        ].map(m => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="space-y-3">
          <div className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: DS.text.muted }}>Abstracted Leases</div>
          {leases.map(l => (
            <LeaseCard key={l.id} lease={l} selected={selectedId === l.id} onClick={() => setSelectedId(l.id)} />
          ))}
        </div>
