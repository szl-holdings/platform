...
const PIPELINE_PROPERTIES: DistressedProperty[] = [
  {
    ...
    units: 18,
    daysInDistress: 284,
    owner: '116th Realty LLC',
    nextAction: 'Review title search and foreclosure filing',
    actionDue: 'Apr 8',
    assignedTo: 'R. Martinez',
    notes:
      'Lis pendens filed Oct 2024. Owner unresponsive. Strong candidate for pre-foreclosure outreach.',
  },
  {
    id: 'dp-002',
    address: '854 Lincoln Ave',
    borough: 'Bronx',
    class: 'C',
    distressCategory: 'tax_lien',
    stage: 'offer',
    distressScore: 78,
    ...
    units: 12,
    daysInDistress: 142,
    owner: 'Lincoln Holdings Trust',
    nextAction: 'Counter-offer response due from owner',
    actionDue: 'Apr 7',
    assignedTo: 'K. Patel',
  ...
  {
    id: 'dp-003',
    address: '1920 Flatbush Ave',
    borough: 'Brooklyn',
    class: 'B',
    distressCategory: 'delinquent',
    stage: 'screening',
    distressScore: 67,
    ...
    units: 24,
    daysInDistress: 89,
    owner: 'Flatbush Partners LLC',
    nextAction: 'Order third-party appraisal',
    actionDue: 'Apr 12',
    assignedTo: 'R. Martinez',
  ...
  },
  {
    ...
    units: 6,
    daysInDistress: 387,
    owner: 'Estate of R. Johnson',
    nextAction: 'Locate heir/administrator contact',
    actionDue: 'Apr 15',
    assignedTo: 'S. Osei',
    notes:
      'Probate matter. Vacant 13 months. HPD violations outstanding. Significant value recovery possible.',
  },
  {
    id: 'dp-005',
    address: '412 Junction Blvd',
    borough: 'Queens',
    class: 'B',
    distressCategory: 'pre_market',
    ...
    units: 20,
    daysInDistress: 58,
    owner: 'Junction Capital Group',
    nextAction: 'Final walk-through scheduled',
    actionDue: 'Apr 6',
    assignedTo: 'K. Patel',
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-[12px] font-semibold truncate mb-0.5" style={{ color: TEXT.primary }}>
            {property.address}
          </p>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[10px]" style={{ color: TEXT.tertiary }}>
              {property.borough} · Class {property.class} · {property.units}u
            </span>
          </div>
        ...
        <div className="shrink-0 text-right">
          <p className="text-[13px] font-bold font-mono mb-0.5" style={{ color: ACCENT }}>
            {property.distressScore}
          </p>
          <p className="text-[9px]" style={{ color: TEXT.muted }}>
            distress score
          </p>
          <p className="text-[10px] font-mono mt-1" style={{ color: '#22c55e' }}>
            -{property.distressDiscount}%
          </p>
        </div>
      </div>
    </button>
      ...
      <div className="px-5 py-4" style={{ borderBottom: `1px solid ${BORDER.subtle}` }}>
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            ...
            <p className="text-[14px] font-semibold mb-0.5" style={{ color: TEXT.primary }}>
              {property.address}
            </p>
            <p className="text-[11px]" style={{ color: TEXT.tertiary }}>
              {property.borough} · Class {property.class} · {property.units} units ·{' '}
              {property.owner}
            </p>
          </div>
          ...
          {stages.slice(0, 5).map((s, i) => (
            <div key={s.label} className="flex-1 flex items-center gap-1">
              <div
                ...
                style={{
                  background: STAGE_CONFIG[property.stage].step >= s.step ? ACCENT : BORDER.muted,
                }}
              />
              {i < 4 && (
                <div
      ...
      <div className="px-5 py-4">
        <div className="grid grid-cols-2 gap-3 mb-4">
          {[
            ...
            },
            {
              ...
              value: `-${property.distressDiscount}%`,
              color: '#22c55e',
            },
            {
              label: 'Debt Balance',
              value: formatCurrency(property.debtBalance),
              color: TEXT.secondary,
            },
            {
              label: 'Occupancy',
              value: `${property.occupancy}%`,
              color: property.occupancy < 50 ? '#ef4444' : TEXT.secondary,
            },
            {
              label: 'Days in Distress',
              value: String(property.daysInDistress),
              color: property.daysInDistress > 180 ? '#ef4444' : TEXT.secondary,
            },
            ...
            >
              ...
              <p className="text-[13px] font-semibold font-mono" style={{ color: m.color }}>
                {m.value}
              </p>
            </div>
          ))}
        ...
        >
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-[12px] font-medium mb-0.5" style={{ color: TEXT.primary }}>
                {property.nextAction}
              </p>
              <p className="text-[10px]" style={{ color: TEXT.tertiary }}>
                Due: {property.actionDue} · Assigned: {property.assignedTo}
              </p>
            </div>
          </div>
        </div>
...
export default function DistressPipelinePage() {
  ...
  const filtered = PIPELINE_PROPERTIES.filter(
    (p) =>
      search === '' ||
      p.address.toLowerCase().includes(search.toLowerCase()) ||
      p.borough.toLowerCase().includes(search.toLowerCase()),
  ).sort((a, b) => b[sortBy] - a[sortBy]);

  const totalValue = PIPELINE_PROPERTIES.reduce((s, p) => s + p.estimatedValue, 0);
  const avgDiscount = Math.round(
    PIPELINE_PROPERTIES.reduce((s, p) => s + p.distressDiscount, 0) / PIPELINE_PROPERTIES.length,
  );
    ...
    <div className="flex flex-col h-full overflow-hidden" style={{ background: BG.page }}>
      ...
      <div className="grid grid-cols-4 gap-px shrink-0" style={{ background: BORDER.subtle }}>
        {[
          ...
          { label: 'Avg. discount', value: `-${avgDiscount}%`, color: '#22c55e' },
          {
            label: 'In diligence+',
            value: String(
              PIPELINE_PROPERTIES.filter((p) =>
                ['diligence', 'offer', 'under_contract'].includes(p.stage),
              ).length,
            ),
            color: '#7ba3d4',
          },
          {
            label: 'High-risk (score > 75)',
            value: String(PIPELINE_PROPERTIES.filter((p) => p.distressScore > 75).length),
            color: '#ef4444',
          },
        ].map((s) => (
      ...
      <div className="flex-1 grid grid-cols-[300px_1fr] overflow-hidden">
        <div className="overflow-y-auto" style={{ borderRight: `1px solid ${BORDER.subtle}` }}>
          ...
          >
            <p className="text-[10px]" style={{ color: TEXT.muted }}>
              {filtered.length} properties · sorted by distress score
            </p>
          </div>
          {filtered.length === 0 ? (
            <EmptyState
              ...
              headline={
                search ? 'No properties match search' : 'No distressed properties in pipeline'
              }
              description={
                search
