    ...
    <div className="flex items-center gap-2">
      ...
      <span
        className={cn(
          ...
          score >= 80 ? 'text-[#40856a]' : score >= 60 ? 'text-amber-400' : 'text-rose-400',
        )}
      >
        {score}
      </span>
    ...
    <div className="fixed inset-y-0 right-0 w-[480px] bg-terra-bg-secondary border-l border-terra-border shadow-2xl z-50 overflow-y-auto">
      <div className="p-6">
        <div className="flex items-start justify-between mb-6">
          <div>
            <h2 className="font-display font-bold text-xl text-terra-text">
              {lead.firstName} {lead.lastName}
            </h2>
            <div className="flex items-center gap-2 mt-1">
              <StageBadge stage={lead.stage as any} />
              <span className="text-xs text-terra-text-muted capitalize">
                {lead.type} · {sourceLabels[lead.source] ?? lead.source}
              </span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          {[
            { label: 'Lead Score', value: lead.score, sub: '/100' },
            {
              ...
              value: `${Math.round(lead.conversionProbability * 100)}%`,
            },
          ].map((m) => (
            <div
              key={m.label}
              className="rounded-lg border border-terra-border bg-terra-surface p-3"
            >
              <p className="text-[10px] text-terra-text-muted">{m.label}</p>
              <p className="text-lg font-display font-bold text-terra-text">
                {m.value}
                <span className="text-xs text-terra-text-muted">{m.sub}</span>
              </p>
            </div>
          ))}
        </div>
        ...
        {lead.notes && (
          <div className="mb-6">
            ...
            <p className="text-xs text-terra-text-secondary leading-relaxed bg-terra-surface rounded-lg p-3 border border-terra-border">
              {lead.notes}
            </p>
          </div>
        )}
        ...
        {lead.nextAction && (
          <div className="mb-6">
            <p className="text-[10px] font-semibold text-terra-text-muted uppercase tracking-wider mb-1">
              Next Action
            </p>
        ...
        {lead.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-6">
            {lead.tags.map((tag) => (
              <span
                key={tag}
                className="text-[10px] px-2 py-0.5 rounded-full bg-terra-primary/10 text-terra-primary border border-terra-primary/20"
              >
                #{tag}
              </span>
            ))}
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="w-full max-w-md bg-terra-bg-secondary rounded-2xl border border-terra-border shadow-2xl p-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="text-terra-text-muted hover:text-terra-text">
            <X className="w-5 h-5" />
          </button>
        </div>
  ...
  const convertToDeal = useStandardMutation({
    ...
    onMutate: async (leadId) => {
      ...
      qc.setQueryData(['terra-leads'], (old: unknown) => {
        ...
        return {
          ...asObj,
          leads: asObj.leads.map((l) => (l.id === leadId ? { ...l, stage: 'converting' } : l)),
        };
      });
      return { prev };
    },
    ...
    <div className="p-6 space-y-6 overflow-auto">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-terra-text-secondary mt-1">
              CRM-quality lead management with scoring, engagement tracking, and conversion paths
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
          ...
          <div key={stage} className="flex items-center gap-2 flex-shrink-0">
            <button
              ...
              className={cn(
                ...
                stageFilter === stage
                  ? 'bg-terra-primary border-terra-primary text-white'
                  : 'bg-terra-surface border-terra-border text-terra-text-secondary hover:text-terra-text',
              )}
            >
            ...
            {i < stageOrder.length - 1 && (
              <span className="text-terra-text-muted text-xs flex-shrink-0">→</span>
            )}
          </div>
        ))}
      </div>
      ...
      {isLoading && (
        <div className="space-y-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="rounded-xl border border-terra-border bg-terra-surface/40 p-4 flex items-center gap-4 animate-pulse"
            >
      ...
      {isError && (
        <div className="rounded-xl border border-rose-500/30 bg-rose-500/5 p-6 text-center">
          ...
          <button
            onClick={() => refetch()}
            className="mt-3 px-4 py-2 rounded-lg border border-rose-500/30 text-xs text-rose-400 hover:bg-rose-500/10 transition-colors"
          >
            Retry
          </button>
        </div>
                ...
                <tbody>
                  {leads.map((lead: ApiLead) => (
                    ...
                    >
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2">
                          <AgentAvatar
                            ...
                            avatar={`${lead.firstName[0]}${lead.lastName[0]}`}
                      ...
                      <td className="py-3 px-4 text-xs text-terra-text-secondary capitalize">
                        {lead.type}
                      </td>
                      <td className="py-3 px-4 text-xs text-terra-text-secondary">
                      ...
                      <td className="py-3 px-4">
                        <span
                          className={cn(
                            ...
                            lead.conversionProbability >= 0.7
                              ? 'text-[#40856a]'
                              : lead.conversionProbability >= 0.4
                                ? 'text-amber-400'
                                : 'text-rose-400',
                          )}
                        >
                      ...
                      <td className="py-3 px-4 text-xs text-terra-text-muted">
                        {lead.lastContact ?? '—'}
                      </td>
                      <td className="py-3 px-4">
                        <span
