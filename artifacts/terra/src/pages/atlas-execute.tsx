...
import { apiFetch, type ApiFetchOptions } from "@szl-holdings/shared-ui";
import {
  Zap, RefreshCw, CheckCircle, Clock, AlertTriangle, Shield, ChevronRight,
  Play, FileText, Activity, Layers, XCircle, Info, ChevronDown, ChevronUp, Home
} from "lucide-react";
...
const SIGNAL_TYPES = ["distress-signal", "ownership-change", "price-anomaly", "market-shift", "zoning-change", "tax-delinquency", "lien-detected"];
...
export default function TerraAtlasExecute() {
  ...
  const [newSignal, setNewSignal] = useState({ signalType: SIGNAL_TYPES[0], severity: "high" as Severity, title: "", description: "", source: "analyst", confidence: 0.85 });
  ...
  const ingestSignal = async () => {
    ...
    try {
      ...
      setNewSignal({ signalType: SIGNAL_TYPES[0], severity: "high", title: "", description: "", source: "analyst", confidence: 0.85 });
      await load();
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: "rgba(255,255,255,0.35)" }}>
            Ingest property signals, trigger deal underwriting workflows, and manage approval gates for high-value commitments.
          </p>
        </div>
      ...
      {pendingApproval && (
        <div className="rounded-xl border p-4 flex items-start gap-4" style={{ borderColor: `${ACCENT}40`, background: `${ACCENT}10` }}>
          ...
          <div className="flex-1">
            <div className="text-[11px] font-bold mb-0.5" style={{ color: ACCENT }}>Underwriting Approval Required</div>
            <div className="text-[11px] text-white mb-1">{pendingApproval.reason ?? "This deal requires underwriting review before the commitment workflow continues."}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="flex flex-col gap-3">
          <div className="rounded-xl border overflow-hidden" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
            <div className="p-3.5 border-b flex items-center gap-2" style={{ borderColor: "rgba(255,255,255,0.05)", background: "rgba(255,255,255,0.01)" }}>
              <Activity className="w-3.5 h-3.5" style={{ color: ACCENT }} />
              <span className="text-[11px] font-semibold text-white">Property Signals</span>
              <span className="ml-auto text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.3)" }}>{signals.length} ingested</span>
            </div>
            {showIngestForm ? (
              <div className="p-3.5 border-b space-y-2" style={{ borderColor: "rgba(255,255,255,0.05)", background: "rgba(255,255,255,0.01)" }}>
                <input
                ...
                <div className="grid grid-cols-2 gap-2">
                  <select className="text-[10px] bg-black/40 border rounded-lg px-2 py-1.5 text-white focus:outline-none"
                    style={{ borderColor: "rgba(255,255,255,0.12)" }}
              ...
              <div className="p-3">
                <button onClick={() => setShowIngestForm(true)}
                  ...
                  + Ingest Property Signal
                </button>
              </div>
            )}
            <div className="divide-y divide-white/[0.04] max-h-80 overflow-y-auto">
              {loading && <div className="p-4 text-center text-[11px]" style={{ color: "rgba(255,255,255,0.3)" }}>Loading…</div>}
              {!loading && signals.length === 0 && <div className="p-4 text-center text-[11px]" style={{ color: "rgba(255,255,255,0.3)" }}>No signals yet.</div>}
              {signals.map(sig => {
                const sevColor = SEV_COLOR[sig.severity] ?? "#6b7280";
                const selected = selectedSignalIds.includes(sig.id);
                return (
                  <div key={sig.id} className="p-3 cursor-pointer hover:bg-white/3 transition-colors"
                    style={selected ? { background: `${ACCENT}10` } : {}}
                    onClick={() => toggleSignal(sig.id)}>
                    <div className="flex items-start gap-2">
                      <div className="mt-0.5 w-4 h-4 rounded border flex items-center justify-center shrink-0"
                        style={selected ? { borderColor: ACCENT, background: ACCENT } : { borderColor: "rgba(255,255,255,0.15)" }}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                          ...
                          <span className="text-[9px] font-mono truncate" style={{ color: "rgba(255,255,255,0.3)" }}>{sig.signalType}</span>
                        </div>
                        <div className="text-[10px] font-medium text-white truncate">{sig.title}</div>
                        <div className="text-[9px] mt-0.5" style={{ color: "rgba(255,255,255,0.3)" }}>{sig.status} · {new Date(sig.createdAt).toLocaleTimeString()}</div>
                      </div>
                    </div>
                  </div>
                );
          ...
          <button onClick={() => executeWorkflow()} disabled={executing || selectedSignalIds.length === 0}
            ...
            {executing ? "Executing…" : isDryRun ? "Dry Run" : "Run Underwriting Workflow"}
        ...
        <div className="rounded-xl border overflow-hidden" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
          <div className="p-3.5 border-b flex items-center gap-2" style={{ borderColor: "rgba(255,255,255,0.05)", background: "rgba(255,255,255,0.01)" }}>
            ...
            <span className="ml-auto text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.3)" }}>{runs.length} total</span>
          </div>
          <div className="divide-y divide-white/[0.04] max-h-[440px] overflow-y-auto">
            {runs.length === 0 && <div className="p-6 text-center text-[11px]" style={{ color: "rgba(255,255,255,0.3)" }}>No runs yet.</div>}
            {runs.map(run => (
              <div key={run.runId}>
                <div className="p-3.5 cursor-pointer hover:bg-white/3 transition-colors"
                  style={selectedRun?.runId === run.runId ? { background: `${ACCENT}08` } : {}}
                  onClick={() => { setSelectedRun(run); setExpandedRun(run.runId); }}>
                  <div className="flex items-center gap-2 mb-1">
                    <StatusBadge status={run.status} />
                    <span className="text-[9px] font-mono truncate" style={{ color: "rgba(255,255,255,0.3)" }}>{run.runId.slice(0, 12)}…</span>
                    <button className="ml-auto p-0.5 hover:bg-white/10 rounded"
                  ...
                  {run.currentStep && <div className="flex items-center gap-1 mt-0.5 text-[9px]" style={{ color: "rgba(255,255,255,0.3)" }}><ChevronRight className="w-2.5 h-2.5" /> {run.currentStep}</div>}
                  {run.startedAt && <div className="text-[9px] mt-0.5" style={{ color: "rgba(255,255,255,0.25)" }}>Started {new Date(run.startedAt).toLocaleTimeString()}</div>}
                </div>
                {expandedRun === run.runId && run.steps && run.steps.length > 0 && (
                  <div className="border-t px-4 py-3 space-y-2" style={{ borderColor: "rgba(255,255,255,0.05)", background: "rgba(0,0,0,0.2)" }}>
                    <div className="text-[9px] font-bold uppercase tracking-widest mb-2" style={{ color: "rgba(255,255,255,0.3)" }}>Step Timeline</div>
                      ...
                      return (
                        <div key={step.id} className="flex items-center gap-2.5">
                          <div className="relative flex flex-col items-center">
                            ...
                            {i < run.steps!.length - 1 && <div className="w-px flex-1 mt-1" style={{ background: "rgba(255,255,255,0.1)", minHeight: 12 }} />}
                          </div>
                          <div className="flex-1 pb-1">
                            <div className="text-[10px] text-white">{step.name}</div>
                            <div className="text-[9px]" style={{ color: sc }}>{step.status}</div>
                          </div>
                          {step.completedAt && <span className="text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.25)" }}>{new Date(step.completedAt).toLocaleTimeString()}</span>}
                        </div>
                      );
                    })}
                  </div>
        ...
        <div className="flex flex-col gap-3">
          <div className="rounded-xl border overflow-hidden" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
            <div className="p-3.5 border-b flex items-center gap-2" style={{ borderColor: "rgba(255,255,255,0.05)", background: "rgba(255,255,255,0.01)" }}>
              ...
              {selectedRun && <span className="ml-auto text-[9px] font-mono truncate" style={{ color: "rgba(255,255,255,0.3)" }}>run/{selectedRun.runId.slice(0, 8)}</span>}
            </div>
            <div className="divide-y divide-white/[0.04] max-h-52 overflow-y-auto">
              {!selectedRun && <div className="p-4 text-center text-[11px]" style={{ color: "rgba(255,255,255,0.3)" }}>Select a run to view evidence</div>}
              {selectedRun && evidence.length === 0 && <div className="p-4 text-center text-[11px]" style={{ color: "rgba(255,255,255,0.3)" }}>No evidence captured yet</div>}
              {evidence.map(ev => (
                <div key={ev.id} className="p-3">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[9px] mt-0.5" style={{ color: "rgba(255,255,255,0.45)" }}>{ev.value}</div>
                      <div className="text-[9px] mt-0.5" style={{ color: "rgba(255,255,255,0.25)" }}>{ev.source} · {new Date(ev.capturedAt).toLocaleTimeString()}</div>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="rounded-xl border overflow-hidden flex-1" style={{ borderColor: "rgba(255,255,255,0.07)" }}>
            <div className="p-3.5 border-b flex items-center gap-2" style={{ borderColor: "rgba(255,255,255,0.05)", background: "rgba(255,255,255,0.01)" }}>
              <FileText className="w-3.5 h-3.5" style={{ color: ACCENT }} />
              <span className="text-[11px] font-semibold text-white">Outcomes</span>
              <span className="ml-auto text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.3)" }}>{outcomes.length}</span>
            </div>
            <div className="divide-y divide-white/[0.04] max-h-52 overflow-y-auto">
              {outcomes.length === 0 && <div className="p-4 text-center text-[11px]" style={{ color: "rgba(255,255,255,0.3)" }}>No outcomes recorded</div>}
              {outcomes.map(oc => {
                ...
                return (
                  <div key={oc.id} className="p-3">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className="text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.25)" }}>{new Date(oc.recordedAt).toLocaleTimeString()}</span>
                    </div>
                    <div className="text-[10px] font-medium text-white">{oc.title}</div>
                    <div className="text-[9px] mt-0.5 leading-relaxed" style={{ color: "rgba(255,255,255,0.4)" }}>{oc.summary}</div>
                    {oc.businessImpact?.financialImpactUsd != null && (
                      <div className="text-[9px] mt-1 font-mono" style={{ color: "rgba(255,255,255,0.3)" }}>
                        ${oc.businessImpact.financialImpactUsd.toLocaleString()} financial impact
                      </div>
                    )}
                  </div>
                );
          ...
          <div className="rounded-xl border p-3.5 flex items-start gap-2.5" style={{ borderColor: `${ACCENT}20`, background: `${ACCENT}06` }}>
            ...
            <div className="text-[9px] leading-relaxed" style={{ color: "rgba(255,255,255,0.35)" }}>
              Select property signals, then <strong className="text-white">Run Underwriting Workflow</strong> to trigger the deal thesis, underwriting, and legal review pipeline. Deals ≥ $5M and distressed assets require underwriting approval.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 text-[9px] font-mono" style={{ color: "rgba(255,255,255,0.2)" }}>
        ...
        <span className="ml-auto">Policy: SEC-Reg-D · Compliance: FIRPTA</span>
      </div>
    </div>
  );
}
