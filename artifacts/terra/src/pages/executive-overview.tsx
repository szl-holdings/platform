  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5">
        {up ? (
          <TrendingUp className="w-3 h-3" style={{ color: isPositive ? '#10b981' : '#ef4444' }} />
        ) : (
          <TrendingDown className="w-3 h-3" style={{ color: isPositive ? '#10b981' : '#ef4444' }} />
        )}
        <span
...
function CommandLoopPanel({ activePhase }: { activePhase: string }) {
  ...
  return (
    ...
    >
      <span
        className="text-[9px] uppercase tracking-widest font-medium mr-2"
      ...
      {phases.map((p, i) => (
        <div key={p.phase} className="flex items-center gap-2">
          {p.link ? (
            <a
              href={p.link}
              className="text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded transition-all hover:opacity-80"
              style={{
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>
            What the business is doing, what's degraded, and where value is at risk — right now.
          </p>
        </div>
              ...
              <div key={a.id} className="px-5 py-3 flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <span
                    ...
                    style={{
                      color:
                        a.age_hours > 48 ? '#ef4444' : a.age_hours > 24 ? '#f97316' : '#f59e0b',
                    }}
                  >
                    {a.age_hours}h old
                  </span>
      ...
      >
        ...
        <p className="text-xs leading-relaxed mb-4" style={{ color: 'rgba(255,255,255,0.5)' }}>
          ...
          Revenue Operations → FORGE predicted Q1 revenue miss → FORGE rerouted workflow to CFO
          backup → KORA verifying recovery.
        </p>
        <div className="flex gap-2">
          {[
            { label: 'DETECT', desc: 'KORA saw it', color: '#0ea5e9', active: true },
            ...
            >
              <div
                ...
                style={{ color: p.active ? p.color : 'rgba(255,255,255,0.2)' }}
              >
                {p.label}
              </div>
              <div
                className="text-[9px]"
                style={{ color: p.active ? 'rgba(255,255,255,0.5)' : 'rgba(255,255,255,0.2)' }}
              >
                {p.desc}
              </div>
            </div>
