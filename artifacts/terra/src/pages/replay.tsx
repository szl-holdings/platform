...
const DEMO_PROPERTY_EVENTS = [
  {
    time: 'Jan 2023',
    type: 'acquisition',
    label: 'Property acquired at $3.2M',
    detail: 'Cap rate 6.4% — below market comp avg of 7.1%',
    severity: 'info',
  },
  {
    time: 'Mar 2023',
    type: 'market',
    label: 'Interest rate environment tightened',
    detail: '10-yr Treasury crossed 4.0% — cap rate expansion pressure begins',
    severity: 'warn',
  },
  {
    time: 'Jun 2023',
    type: 'tenant',
    label: '3-unit vacancy — tenant churn',
    detail: 'Vacancy rate rose from 0% to 25%',
    severity: 'warn',
  },
  {
    time: 'Aug 2023',
    type: 'valuation',
    label: 'AVM revaluation: −$180K',
    detail: 'Cap rate expansion + NOI compression on vacant units',
    severity: 'warn',
  },
  {
    time: 'Nov 2023',
    type: 'tenant',
    label: '2 units re-leased at market rate',
    detail: 'Vacancy reduced to 8.3% — rents +6% vs prior tenants',
    severity: 'info',
  },
  {
    time: 'Jan 2024',
    type: 'market',
    label: 'Submarket pressure index: elevated',
    detail: 'Williamsburg multifamily supply pipeline 280 units',
    severity: 'warn',
  },
  {
    time: 'Apr 2024',
    type: 'filing',
    label: 'Water damage claim filed',
    detail: 'Insurance claim $22K — boiler replacement',
    severity: 'info',
  },
  {
    time: 'Jun 2024',
    type: 'valuation',
    label: 'AVM stabilized at $3.62M',
    detail: 'Occupancy 91.7% — NOI recovering toward $220K',
    severity: 'info',
  },
  {
    time: 'Sep 2024',
    type: 'market',
    label: 'Rate cut expectations — cap compression',
    detail: '50bps Fed cut — multifamily cap rates tightening',
    severity: 'info',
  },
  {
    time: 'Jan 2025',
    type: 'valuation',
    label: 'AVM: $3.80M — above acquisition',
    detail: 'IRR tracking 14.2% — on plan for 5-year hold',
    severity: 'info',
  },
  {
    time: 'Mar 2025',
    type: 'tenant',
    label: 'Lease renewal — 2 units at +8%',
    detail: 'Organic rent growth sustained',
    severity: 'info',
  },
  {
    time: 'Apr 2025',
    type: 'market',
    label: 'Lien screen clear — title clean',
    detail: 'No UCC filings, mechanic liens, or lis pendens',
    severity: 'info',
  },
  {
    time: 'Apr 2026',
    type: 'current',
    label: 'Current: Portfolio stable — IRR 14.2%',
    detail: 'Next decision: Hold / refi / portfolio sale evaluation Q3 2026',
    severity: 'info',
  },
];
...
function ValuationChart({ data, cursor }: { data: typeof VALUATION_TIMELINE; cursor: number }) {
  ...
  useEffect(() => {
    ...
    ctx.arc(
      cx,
      pad.t + chartH * (1 - (data[cursor].value - minV) / (maxV - minV)),
      4,
      0,
      Math.PI * 2,
    );
...
export default function TerraReplayPage() {
  ...
  const { data: propertiesData, isError: propertiesListError } = useStandardQuery<{
    data: { properties: Array<{ id: number; address: string }> };
  }>({
  ...
  const effectivePropertyId =
    selectedPropertyId ?? (properties[0] ? String(properties[0].id) : null);
  ...
  } = useStandardQuery<{ data: { events: ApiEvent[]; address: string } }>({
    queryKey: ['terra-property-history', effectivePropertyId],
    queryFn: () =>
      fetch(`/api/terra/properties/${effectivePropertyId}/history`).then((r) =>
  ...
  const dataMode: 'loading' | 'live' | 'demo' | 'error' =
    historyLoading || (historyFetching && !historyData)
      ? 'loading'
  ...
  const valCursor = Math.min(
    VALUATION_TIMELINE.length - 1,
    Math.floor((cursor / Math.max(total - 1, 1)) * (VALUATION_TIMELINE.length - 1)),
  );
  ...
  const selectedAddress =
    properties.find((p) => String(p.id) === effectivePropertyId)?.address ??
    historyData?.data?.address ??
    '84 Grand St';
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs" style={{ color: 'rgba(255,255,255,0.28)' }}>
            Step through the property worldline — every valuation shift, market event, and tenant
            change
          </p>
        </div>
        ...
        <div className="flex items-start gap-2 text-[11px] px-3 py-2.5 rounded-lg border border-amber-500/30 bg-amber-500/8 text-amber-200">
          ...
          <div>
            ...
            <p className="text-amber-300/70 text-[10px] mt-0.5">
              No property history was returned for {selectedAddress}. Showing illustrative demo
              events. The page will switch to live data automatically once real events are recorded.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="flex items-start gap-2 text-[11px] px-3 py-2.5 rounded-lg border border-red-500/30 bg-red-500/8 text-red-200">
          ...
          <div>
            ...
            <p className="text-red-300/70 text-[10px] mt-0.5">
              The property history API request failed. Showing demo content while the connection is
              restored.
            </p>
          </div>
        </div>
      ...
      <div
        ...
        style={{ background: 'rgba(5,10,8,0.9)', border: '1px solid rgba(139,122,200,0.2)' }}
      >
        <div
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            {current.severity === 'warn' && (
              <span className="text-[9px] text-amber-400 px-1.5 py-0.5 rounded bg-amber-500/8 border border-amber-500/20">
                warn
              </span>
            )}
          </div>
        ...
        <div className="text-right shrink-0">
          ...
          <p className="text-[11px] font-mono font-bold mt-0.5" style={{ color: '#2d6a4f' }}>
            ${(valAtCursor.value / 1000).toFixed(2)}M AVM
          </p>
        </div>
      </div>
      ...
      >
        <div className="flex items-center justify-between mb-1">
          ...
          >
            <span
              ...
              style={{ background: 'rgba(45,106,79,0.8)' }}
            />
            AVM
            <span
              className="inline-block w-3 h-0.5 rounded ml-1"
              style={{ background: 'rgba(200,149,60,0.55)' }}
            />
            NOI (K)
          </div>
        </div>
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs px-4 py-1.5 rounded-lg border transition-colors"
            style={
              playing
                ? {
          ...
          {[
            {
              label: 'Next ›',
              icon: null,
              action: () => setCursor((c) => Math.min(total - 1, c + 1)),
            },
            {
              label: 'End',
      ...
      >
        <div className="px-4 py-3 border-b" style={{ borderColor: 'rgba(45,106,79,0.1)' }}>
          <p className="text-sm font-semibold" style={{ color: 'rgba(255,255,255,0.88)' }}>
            Property History — {selectedAddress}
          </p>
          <p className="text-[10px]" style={{ color: 'rgba(255,255,255,0.28)' }}>
            Click any event to jump to that point in the worldline
          </p>
        </div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: isCurrent ? 'rgba(139,122,200,0.06)' : undefined,
                  opacity: isPast ? 0.7 : i > cursor ? 0.4 : 1,
                }}
              >
                ...
                <div className="flex-1 min-w-0 pb-1">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <p
                      ...
                      style={{
                        color: isCurrent ? 'rgba(255,255,255,0.88)' : 'rgba(255,255,255,0.6)',
                      }}
                    >
                      {ev.label}
                    </p>
