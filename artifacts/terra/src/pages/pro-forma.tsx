...
function useProForma(inputs: ProFormaInputs) {
  return useMemo(() => {
    ...
    const irr = (Math.pow(equityMultiple, 12 / projectMonths) - 1) * 100;
    ...
    <div className="space-y-4 max-w-[1400px]">
      ...
      {showInputs && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border p-4" style={{ borderColor: DS.border, background: DS.surface }}>
          <p className="text-[10px] font-bold uppercase tracking-wider mb-3" style={{ color: DS.text.muted }}>Project Inputs</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <NumInput label="Total Units" value={inputs.totalUnits} onChange={set("totalUnits")} />
            <NumInput label="Avg Unit SF" value={inputs.avgUnitSF} onChange={set("avgUnitSF")} suffix="SF" />
            <NumInput label="Land Cost" value={inputs.landCost} onChange={set("landCost")} prefix="$" step={100000} />
            <NumInput label="Hard Cost/SF" value={inputs.hardCostPerSF} onChange={set("hardCostPerSF")} prefix="$" suffix="/SF" step={5} />
            <NumInput label="Soft Cost %" value={inputs.softCostPct} onChange={set("softCostPct")} suffix="%" step={0.5} />
            <NumInput label="Contingency %" value={inputs.contingencyPct} onChange={set("contingencyPct")} suffix="%" step={0.5} />
            <NumInput label="Financing Rate" value={inputs.financingRate} onChange={set("financingRate")} suffix="%" step={0.25} />
            <NumInput label="Loan-to-Cost" value={inputs.loanToCost} onChange={set("loanToCost")} suffix="%" step={1} />
            <NumInput label="Construction Mo." value={inputs.constructionMonths} onChange={set("constructionMonths")} suffix="mo" />
            <NumInput label="Absorption Mo." value={inputs.absorptionMonths} onChange={set("absorptionMonths")} suffix="mo" />
            <NumInput label="Market Rent/SF/Mo" value={inputs.marketRentPerSF} onChange={set("marketRentPerSF")} prefix="$" step={0.05} />
            <NumInput label="Exit Cap Rate" value={inputs.exitCapRate} onChange={set("exitCapRate")} suffix="%" step={0.25} />
          </div>
        </motion.div>
      )}
        ...
        <div className="rounded-xl border p-4" style={{ borderColor: DS.border, background: DS.surface }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={r.schedule} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
              ...
              <Bar dataKey="cost" name="Cost" fill={DS.accent.gold} fillOpacity={0.8} radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
