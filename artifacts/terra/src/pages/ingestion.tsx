...
const SOURCE_LABELS: Record<string, { label: string; description: string; color: string }> = {
  nyc_open_data: {
    label: 'NYC Open Data — Core',
    description: 'ACRIS, foreclosure filings, DOF liens, HPD violations',
    color: 'text-blue-400',
  },
  nyc_open_data_extended: {
    label: 'NYC Open Data — Extended',
    description: 'Rolling Sales, Tax Lien Sale List, HPD Complaints, DOB, 311, ACRIS Parties',
    color: 'text-violet-400',
  },
  csv_upload: {
    label: 'CSV Upload',
    description: 'Manual CSV from county exports, broker sources, custom feeds',
    color: 'text-emerald-400',
  },
  manual: {
    label: 'Manual Entry',
    description: 'Individually entered distress properties',
    color: 'text-amber-400',
  },
  seed: {
    label: 'Seed Data',
    description: 'Initial dataset loaded at launch',
        ...
        >
          {[
            ...
            },
            {
              ...
              value: summary.totalRecordsInserted ?? 0,
              icon: CheckCircle,
              color: 'text-emerald-400',
            },
            {
              label: 'Alerts Generated',
              value: summary.totalAlertsGenerated ?? 0,
              icon: Zap,
              color: 'text-amber-400',
            },
          ].map((m) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-[10px] text-terra-text-muted uppercase tracking-wider">
                  {m.label}
                </p>
              </div>
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-terra-text-muted">
                County exports, broker lists, manual imports
              </p>
            </div>
          </div>
          ...
          <div
            className={cn(
              ...
              uploading
                ? 'border-terra-primary/50 bg-terra-primary/5'
                : 'border-terra-border hover:border-terra-primary/40 hover:bg-terra-primary/3',
            )}
          ...
          >
            ...
            {uploading ? (
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-[10px] text-terra-text-muted">
                  Supports address, borough/county, distress type, value, debt, filing date
                </p>
              </div>
            )}
          </div>
            ...
            {uploadResult && (
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px] text-terra-text-secondary">
                  ...
                  <span>
                    Skipped:{' '}
                    <span className="font-bold text-amber-400">{uploadResult.recordsSkipped}</span>
                  </span>
                  <span>
                    Failed:{' '}
                    <span className="font-bold text-red-400">{uploadResult.recordsFailed}</span>
                  </span>
                  <span>
                    Alerts:{' '}
                    <span className="font-bold text-blue-400">{uploadResult.alertsGenerated}</span>
                  </span>
                </div>
              </motion.div>
            )}
          ...
          <div className="mt-4 p-3 bg-terra-surface rounded-lg border border-terra-border">
            <p className="text-[10px] font-semibold text-terra-text-muted uppercase tracking-wider mb-2">
            ...
            <div className="flex flex-wrap gap-1">
              {[
                ...
                <span
                  key={c}
                  className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-terra-border text-terra-text-secondary"
                >
                  {c}
                </span>
              ))}
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-terra-text-muted">
                ACRIS, foreclosures, DOF liens, HPD violations
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2 mb-4">
            {[
              ...
              <div
                key={src.key}
                className="flex items-center justify-between p-2.5 rounded-lg border border-terra-border bg-terra-surface/50"
              >
                <div>
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              <h2 className="text-sm font-display font-bold text-terra-text">
                NYC Open Data — Extended
              </h2>
              <p className="text-[10px] text-terra-text-muted">
                Rolling sales, tax lien sales, HPD complaints, DOB, 311, ACRIS parties
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2 mb-4">
            {[
              {
                key: 'usep-8jbt',
                label: 'NYC Rolling Property Sales',
                desc: 'All borough sales transactions',
              },
              {
                key: '9rz4-mjek',
                label: 'Tax Lien Sale List',
                desc: 'Delinquent properties facing sale',
              },
              { key: 'uwyv-629c', label: 'HPD Complaints', desc: 'Tenant complaints by building' },
              {
                key: '3h2n-5cm9',
                label: 'DOB Violations',
                desc: 'Dept of Buildings stop-work & violations',
              },
              {
                key: 'erm2-nwe9',
                label: 'NYC 311 Property',
                desc: 'Building/property service complaints',
              },
              {
                key: '636b-3b5g',
                label: 'ACRIS Parties / LLC Trace',
                desc: 'Grantor/grantee entity ownership',
              },
              {
                key: '64uk-42ks',
                label: 'MapPLUTO Land Use & Zoning',
                desc: 'Parcel data, assessed values, building class, zoning',
              },
            ].map((src) => (
              <div
                key={src.key}
              ...
              >
                <div>
                  <p className="text-xs font-semibold text-terra-text">{src.label}</p>
                  <p className="text-[9px] text-terra-text-muted font-mono">
                    {src.key} — {src.desc}
                  </p>
                </div>
          ...
          {extendedResult && (
            ...
            >
              {extendedResult.error
                ? extendedResult.error
                : (extendedResult.message ?? 'Extended job enqueued')}
            </div>
          )}
        </motion.div>
      </div>
      ...
      >
        ...
        {isLoading ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-xs text-terra-text-muted mt-1">
              Upload a CSV or trigger an NYC data pull to get started
            </p>
          </div>
        ) : (
          <div className="divide-y divide-terra-border">
            {recentRuns.map((run: any, i: number) => {
              ...
              const srcCfg = SOURCE_LABELS[run.source] ?? {
                label: run.source,
                description: '',
                color: 'text-terra-text-muted',
              };
              return (
                <div key={run.id ?? i}>
                  <button
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-terra-text-muted">
                          {new Date(run.startedAt ?? run.started_at).toLocaleString()}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 mr-2">
                      <div className="text-right">
                        <p className="text-xs text-emerald-400 font-mono">
                          +{run.recordsInserted ?? run.records_inserted ?? 0}
                        </p>
                        <p className="text-[10px] text-terra-text-muted">inserted</p>
                      </div>
                      {isExpanded ? (
                        <ChevronUp className="w-3.5 h-3.5 text-terra-text-muted" />
                      ) : (
                        <ChevronDown className="w-3.5 h-3.5 text-terra-text-muted" />
                      )}
                    </div>
                  </button>
                    ...
                    <div className="px-4 pb-4 bg-terra-surface/30">
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                        {[
                          {
                            label: 'Fetched',
                            value: run.recordsFetched ?? run.records_fetched ?? 0,
                            color: 'text-terra-text',
                          },
                          {
                            label: 'Inserted',
                            value: run.recordsInserted ?? run.records_inserted ?? 0,
                            color: 'text-emerald-400',
                          },
                          {
                            label: 'Skipped',
                            value: run.recordsSkipped ?? run.records_skipped ?? 0,
                            color: 'text-amber-400',
                          },
                          {
                            label: 'Failed',
                            value: run.recordsFailed ?? run.records_failed ?? 0,
                            color: 'text-red-400',
                          },
                        ].map((m) => (
                          <div
                            key={m.label}
                            className="text-center p-2 rounded bg-terra-surface border border-terra-border"
                          >
