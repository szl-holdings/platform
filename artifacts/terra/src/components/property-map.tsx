...
function PropertyPanel({ property, onClose }: { property: Property; onClose: () => void }) {
  ...
  return (
    <div
      ...
      style={{ background: "rgba(8,16,28,0.97)", border: "1px solid rgba(200,160,96,0.15)", backdropFilter: "blur(12px)" }}
    >
      <div className="p-4 border-b flex items-center justify-between" style={{ borderColor: "rgba(200,160,96,0.1)" }}>
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-sm" style={{ background: "rgba(200,160,96,0.1)", border: "1px solid rgba(200,160,96,0.2)" }}>
            {TYPE_ICONS[property.type] || "🏢"}
          </div>
          <div>
            <p className="text-[11px] font-bold text-white/90 leading-tight">{property.name}</p>
            <p className="text-[9px] mt-0.5" style={{ color: "rgba(200,160,96,0.5)" }}>{property.type.replace("-", " ").toUpperCase()}</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        <div className="flex items-center gap-1.5">
          <span className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full font-medium" style={{ color, border: `1px solid ${color}30`, background: `${color}10` }}>
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            { label: "Monthly Rev.", value: formatCurrency(property.monthlyRevenue), icon: TrendingUp },
        ...
        <div className="rounded-lg p-2.5" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
          <p className="text-[9px] uppercase tracking-wider mb-1" style={{ color: "rgba(255,255,255,0.3)" }}>Annual NOI</p>
          <p className="text-[13px] font-mono font-bold" style={{ color: "#c8a060" }}>{formatCurrency(property.annualNOI)}</p>
        </div>
        <div className="rounded-lg p-2.5" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="flex items-center justify-between mb-1.5">
            <p className="text-[9px] uppercase tracking-wider" style={{ color: "rgba(255,255,255,0.3)" }}>Occupancy</p>
            <span className="text-[9px] font-mono" style={{ color: property.occupancy >= 90 ? "#10b981" : property.occupancy >= 75 ? "#f59e0b" : "#ef4444" }}>{property.occupancy}%</span>
          </div>
          <div className="h-1 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.08)" }}>
            <div className="h-full rounded-full transition-all" style={{ width: `${property.occupancy}%`, background: property.occupancy >= 90 ? "#10b981" : property.occupancy >= 75 ? "#f59e0b" : "#ef4444" }} />
          </div>
        </div>
        <Link href={`/property/${property.id}`}>
          <button className="w-full text-[10px] py-2 rounded-lg border transition-all hover:bg-white/5" style={{ color: "#c8a060", borderColor: "rgba(200,160,96,0.2)" }}>
...
export default function PropertyMap({ properties, token, height = "100%", showPanel = true }: PropertyMapProps) {
  ...
  const buildGeoJSON = useCallback((props: Property[]): GeoJSON.FeatureCollection<GeoJSON.Point, Property & { statusColor: string }> => ({
    ...
    import("mapbox-gl").then((mod) => {
      ...
      map.on("load", () => {
        ...
        map.addLayer({
          ...
          paint: {
            "circle-color": ["step", ["get", "point_count"], "#c8a060", 3, "#f59e0b", 6, "#ef4444"],
        ...
        map.on("mouseenter", "unclustered-point", (e) => {
          ...
          popupRef.current = new mgl.Popup({ closeButton: false, closeOnClick: false, offset: 12, className: "property-popup", maxWidth: "260px" })
      ...
      map.on("error", () => { if (!destroyed) setMapError(true); });
  ...
  useEffect(() => {
    if (!mapLoaded || !mapRef.current) return;
    try {
      mapRef.current.setFilter("unclustered-selected", ["==", ["get", "id"], selectedProperty?.id ?? ""]);
    } catch {}
  ...
  return (
    <div className="relative rounded-xl overflow-hidden" style={{ height, border: "1px solid rgba(255,255,255,0.06)" }}>
      <style>{`
      ...
      {!mapLoaded && (
        <div className="absolute inset-0 flex items-center justify-center z-10" style={{ background: "#08101c" }}>
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-[10px]" style={{ color: "rgba(200,160,96,0.5)" }}>Loading property map…</p>
          </div>
        </div>
      )}
      ...
      {mapLoaded && (
        <>
          <div className="absolute top-3 left-3 z-10 flex items-center gap-1.5">
            {FILTER_OPTIONS.map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={cn("text-[9px] px-2 py-1 rounded-lg border capitalize font-medium transition-all")}
                style={{
                  color: filter === f ? (f === "all" ? "#c8a060" : STATUS_COLORS[f] ?? "#c8a060") : "rgba(255,255,255,0.35)",
                  borderColor: filter === f ? (f === "all" ? "rgba(200,160,96,0.3)" : `${STATUS_COLORS[f] ?? "#c8a060"}30`) : "rgba(255,255,255,0.08)",
                  background: filter === f ? (f === "all" ? "rgba(200,160,96,0.08)" : `${STATUS_COLORS[f] ?? "#c8a060"}10`) : "rgba(8,16,28,0.8)",
                }}
              >
          ...
          <div className="absolute bottom-3 left-3 z-10 text-[9px] font-mono rounded-lg px-2.5 py-1.5" style={{ color: "rgba(200,160,96,0.5)", background: "rgba(8,16,28,0.8)", border: "1px solid rgba(200,160,96,0.1)" }}>
            <Building2 className="w-3 h-3 inline mr-1 opacity-60" />
            {filteredProperties.length} propert{filteredProperties.length === 1 ? "y" : "ies"} · Mapbox GL
          </div>

          <div className="absolute bottom-3 right-3 z-10 flex items-center gap-2">
            {Object.entries(STATUS_COLORS).map(([status, color]) => (
              <span key={status} className="flex items-center gap-1 text-[9px]" style={{ color: "rgba(255,255,255,0.4)" }}>
