...
import { AlertTriangle, CheckCircle, Clock, Shield, TrendingUp, XCircle, Zap } from 'lucide-react';
  ...
  return (
    <span
      className={cn(
        ...
        stageColors[stage] ?? 'bg-slate-500/10 text-slate-400',
        className,
      )}
    >
      {label}
            ...
            <div className="flex flex-col items-center">
              ...
              {!isLast && (
                <div
                  className={cn(
                    'w-0.5 flex-1 mt-1',
                    step.status === 'complete' ? 'bg-[#40856a]/30' : 'bg-terra-border',
                  )}
                />
              )}
            </div>
    ...
    <div className={cn('space-y-2', className)}>
      {items.map((item, i) => (
        <div
          key={i}
          className={cn(
            'flex items-start gap-3 p-3 rounded-lg bg-terra-surface border border-terra-border border-l-2',
        ...
        >
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-terra-text-muted mt-0.5">
              {item.entityLabel} · {item.assignee}
            </p>
          </div>
      ...
      {entries.map((entry, i) => (
        <div key={i} className="flex gap-3 text-xs">
          ...
          <div className="pb-2">
            ...
            <p className="text-[10px] text-terra-text-muted mt-0.5">
              {entry.by} · {new Date(entry.date).toLocaleDateString()}
            </p>
          </div>
        </div>
      ))}
...
export function AgentAvatar({
  ...
  return (
    <div
      className={cn(
        'w-8 h-8 rounded-full bg-gradient-to-br from-terra-primary to-terra-accent flex items-center justify-center text-white text-xs font-bold flex-shrink-0',
        className,
      )}
    >
      {avatar}
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
      {metrics.map((m, i) => (
        ...
        >
          <p className="text-[10px] text-terra-text-muted uppercase tracking-wider font-medium">
            {m.label}
          </p>
          <p
            className={cn(
...
export function PropertyDrawer({
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-96 bg-terra-bg-secondary border-l border-terra-border shadow-2xl z-50 overflow-y-auto p-6">
      <button
        onClick={onClose}
        className="absolute top-4 right-4 text-terra-text-muted hover:text-terra-text"
      >
        ×
      </button>
      <h2 className="font-display font-bold text-terra-text mb-1">{listing.address}</h2>
      <p className="text-terra-text-secondary text-sm mb-4">
        ${(listing.price / 1e6).toFixed(2)}M · {listing.dom} DOM
      </p>
      ...
      <div className="space-y-2">
        {listing.riskFlags.map((flag, i) => (
          <div
            key={i}
            className="flex items-start gap-2 text-xs text-rose-400 bg-rose-500/5 border border-rose-500/20 rounded-lg p-2"
          >
