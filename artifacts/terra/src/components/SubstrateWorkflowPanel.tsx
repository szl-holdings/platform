      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="text-xs font-semibold text-slate-100">
                Portfolio Anomaly and Event Intelligence
              </p>
              <p className="text-[10px] text-emerald-400/50 mt-0.5 font-mono">
                Substrate · terra-portfolio-anomaly · Phase 2
              </p>
            </div>
          ...
          <p className="text-[10px] text-slate-400 mt-1.5 leading-relaxed">
            Monitors the portfolio for distress signals, AVM outliers, and tenant risk events.
            Operator approval required before portfolio actions.
          </p>
        ...
        <div className="border-t border-emerald-500/10 pt-3 space-y-3">
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="rounded border border-slate-800 bg-slate-900/60 p-2">
              ...
              <p
                className={`text-sm font-mono font-bold truncate ${STATUS_COLOR[result.status] ?? 'text-emerald-300'}`}
              >
                {result.status}
              </p>
            </div>
          ...
          {result.stages.length > 0 && (
            <div className="space-y-1">
              ...
              {result.stages.map((s) => (
                <div
                  key={s.stageId}
                  className="rounded border border-slate-800 bg-slate-900/40 px-2.5 py-1.5 flex items-start justify-between"
                >
                  <div>
          ...
          <div className="flex items-center justify-between">
            <span className="text-[9px] font-mono text-slate-600">
              mode:{result.mode} · inbox:terra-portfolio-anomaly
            </span>
          ...
          {mode === 'dry-run' && (
            <div className="rounded border border-sky-500/15 bg-sky-500/5 p-2">
              <p className="text-[9px] font-mono text-sky-400">
                DRY-RUN — financial writes and notifications suppressed.
              </p>
            </div>
          )}
          {status === 'pending-approval' && (
