...
const TERRA_FORECAST_DATA: ForecastHead[] = [
  {
    headName: 'terra:distress',
    label: 'Distress Signal Score',
    intervals: [
      { horizon: '30d', point: 0.34, lower: 0.19, upper: 0.51, confidence: 0.82, unit: 'score' },
      { horizon: '60d', point: 0.43, lower: 0.27, upper: 0.61, confidence: 0.76, unit: 'score' },
      { horizon: '90d', point: 0.54, lower: 0.36, upper: 0.72, confidence: 0.70, unit: 'score' },
    ],
    provenance: { modelId: 'safe-default-terra:distress', modelVersion: '0.1.0', adapterId: 'safe-default', generatedAt: new Date().toISOString() },
    alertThreshold: 0.6,
  },
  {
    headName: 'terra:deal-likelihood',
    label: 'Deal Likelihood',
    intervals: [
      { horizon: '7d', point: 0.47, lower: 0.32, upper: 0.62, confidence: 0.85, unit: 'score' },
      { horizon: '30d', point: 0.63, lower: 0.47, upper: 0.79, confidence: 0.79, unit: 'score' },
      { horizon: '90d', point: 0.71, lower: 0.53, upper: 0.87, confidence: 0.72, unit: 'score' },
    ],
    provenance: { modelId: 'safe-default-terra:deal-likelihood', modelVersion: '0.1.0', adapterId: 'safe-default', generatedAt: new Date().toISOString() },
  },
  {
    headName: 'terra:neighborhood-momentum',
    label: 'Neighborhood Momentum Index',
    intervals: [
      { horizon: '30d', point: 0.55, lower: 0.40, upper: 0.70, confidence: 0.83, unit: 'score' },
      { horizon: '90d', point: 0.61, lower: 0.44, upper: 0.76, confidence: 0.77, unit: 'score' },
      { horizon: '180d', point: 0.67, lower: 0.48, upper: 0.84, confidence: 0.70, unit: 'score' },
    ],
    provenance: { modelId: 'safe-default-terra:neighborhood-momentum', modelVersion: '0.1.0', adapterId: 'safe-default', generatedAt: new Date().toISOString() },
  },
  {
    headName: 'terra:lead-conversion',
    label: 'Lead Conversion Rate',
    intervals: [
      { horizon: '7d', point: 0.28, lower: 0.15, upper: 0.43, confidence: 0.87, unit: 'score' },
      { horizon: '30d', point: 0.39, lower: 0.24, upper: 0.56, confidence: 0.81, unit: 'score' },
      { horizon: '90d', point: 0.51, lower: 0.34, upper: 0.68, confidence: 0.74, unit: 'score' },
    ],
    provenance: { modelId: 'safe-default-terra:lead-conversion', modelVersion: '0.1.0', adapterId: 'safe-default', generatedAt: new Date().toISOString() },
  },
];
    ...
    <div className="space-y-1">
      <div className="flex items-center justify-between text-[10px] font-mono">
        ...
        <span className={breached ? 'text-red-400' : 'text-foreground'}>
          {(iv.point * 100).toFixed(1)}% <span className="text-muted-foreground">[{(iv.lower * 100).toFixed(0)}–{(iv.upper * 100).toFixed(0)}]</span>
        </span>
      </div>
...
export function ForecastPanel() {
  const breachedCount = TERRA_FORECAST_DATA.filter((h) => h.thresholdBreached).length;
  return (
