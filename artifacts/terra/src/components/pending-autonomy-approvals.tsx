...
function impactBadge(role: string) {
  ...
  if (role.includes('acquisitions') || role.includes('fleet'))
    return {
      label: 'HIGH',
...
export function PendingAutonomyApprovalsPanel({
    ...
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: getListAlloyApprovalsQueryKey({ status: 'pending' }),
        });
        setDecidingId(null);
        setNoteText('');
        setVerdict(null);
  ...
  function submitDecide(id: string) {
    ...
    decideMutation.mutate({
      id,
      data: {
        decision: verdict === 'approve' ? 'approve' : 'reject',
    ...
    >
      ...
      {isError && !isLoading && (
        <div className="flex items-center gap-2 py-4" style={{ color: 'rgba(239,68,68,0.7)' }}>
          ...
          <span className="text-xs font-mono">
            Could not reach approvals API — check API server connectivity
          </span>
        </div>
      )}
      ...
      {!isLoading && !isError && approvals.length === 0 && (
        <div className="flex flex-col items-center justify-center py-8 gap-2">
          ...
          <span className="text-xs font-mono" style={{ color: `${accentColor}60` }}>
            No pending approvals
          </span>
          <span className="text-[0.6875rem]" style={{ color: 'rgba(255,255,255,0.2)' }}>
            All autonomy actions approved or queued
          </span>
        </div>
      )}
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <span
                    style={{ fontSize: '0.6875rem', color: accentColor, fontFamily: 'monospace' }}
                  >
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: '0.375rem',
                          background:
                            verdict === 'approve' ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.15)',
