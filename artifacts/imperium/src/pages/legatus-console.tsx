...
import {
  Crown, Globe2, Shield, Activity, TrendingUp, TrendingDown,
  AlertTriangle, ChevronRight, DollarSign, Server, Database,
  Lock, Wifi, RefreshCw
} from "lucide-react";
import { IMPERIUM_DATA, getAquilaColor, getAquilaLabel, getThreatColor, getClassificationColor, type Legion, type ThreatLevel } from "@/lib/imperium-data";
      ...
      <div className="relative w-14 h-14">
        <svg viewBox="0 0 48 48" className="w-full h-full -rotate-90">
          ...
          <circle
            ...
            strokeLinecap="round"
            style={{ filter: `drop-shadow(0 0 4px ${color}60)`, transition: "stroke-dashoffset 1s ease" }}
          />
        </svg>
...
function LegionCard({ legion }: { legion: Legion }) {
  const totalSentinels = legion.cohorts.reduce((a, c) => a + c.centuries.reduce((b, ct) => b + ct.sentinels.length, 0), 0);
  const threatColor = getThreatColor(legion.threatLevel);

  return (
    <Link href="/imperium-map">
      <a className="block imperial-card rounded-lg p-4 hover:border-gold/40 transition-all cursor-pointer group">
        <div className="flex items-start justify-between mb-3">
          <div>
            <div className="font-display text-xs tracking-[0.15em] gold-text font-semibold">{legion.name}</div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex-1 pl-4 space-y-2">
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-mono text-slate-300">${legion.costPerMonth.toLocaleString()}</span>
            </div>
          </div>
        </div>
...
function StatCard({ icon: Icon, label, value, sub, color, href }: {
  ...
  const content = (
    <div className="imperial-card rounded-lg p-4 hover:border-gold/30 transition-all">
      ...
      <div className="font-mono text-xl font-bold" style={{ color: color || "#c9a227" }}>{value}</div>
      {sub && <div className="text-[11px] text-slate-500 mt-1">{sub}</div>}
    </div>
  );
  if (href) return <Link href={href}><a className="block cursor-pointer">{content}</a></Link>;
  return content;
}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard icon={Shield} label="Sovereign Resources" value={imperium.classificationSummary.SOVEREIGN}
          sub="Praetorian-guarded" href="/praetorian" />
      </div>
      ...
      <div className="imperial-card rounded-lg p-4">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="font-display text-xs tracking-[0.15em] gold-text uppercase">Classification Posture</span>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {classEntries.map(([cls, count]) => (
            <div key={cls} className="rounded p-3 text-center border" style={{
              borderColor: `${getClassificationColor(cls)}30`,
              background: `${getClassificationColor(cls)}08`,
            }}>
              <div className="font-mono text-2xl font-bold mb-1" style={{ color: getClassificationColor(cls) }}>{count}</div>
          ...
          return (
            <Link key={action.href} href={action.href}>
              <a className="imperial-card rounded-lg p-3 hover:border-gold/40 transition-all cursor-pointer flex items-start gap-2.5">
                ...
                <div>
                  <div className="font-display text-[10px] tracking-[0.12em] gold-text font-semibold leading-tight">{action.label}</div>
                  <div className="text-[10px] text-slate-500 leading-tight mt-0.5">{action.sub}</div>
                </div>
              </a>
            </Link>
          );
