...
import { Cpu, TrendingUp, AlertTriangle, CheckCircle, ChevronDown, ChevronUp, Activity, Zap } from "lucide-react";
import { CENTURION_PROFILES, getAquilaColor, getAquilaLabel } from "@/lib/imperium-data";
  ...
  return (
    <div>
      <div className="flex justify-between mb-1">
        ...
        <span className="font-mono text-[10px]" style={{ color: autoColor }}>{value}{unit}</span>
      </div>
    ...
    <div className="flex items-center gap-2">
      <div className="relative w-10 h-10 flex-shrink-0">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="font-mono text-[9px] font-bold" style={{ color }}>{pct.toFixed(0)}%</span>
        </div>
      </div>
      <div>
        <div className="font-mono text-[10px] font-bold" style={{ color }}>{label} RISK</div>
...
function ScalingRecommendation({ rec }: { rec: { currentMin: number; currentMax: number; recommendedMin: number; recommendedMax: number } }) {
  const hasChange = rec.currentMin !== rec.recommendedMin || rec.currentMax !== rec.recommendedMax;
  return (
    <div className="rounded p-3 border" style={{ borderColor: hasChange ? "rgba(201,162,39,0.25)" : "rgba(255,255,255,0.05)", background: "rgba(10,13,26,0.5)" }}>
      <div className="flex items-center gap-2 mb-2">
        <TrendingUp className="w-3 h-3" style={{ color: "#c9a227" }} />
        <span className="text-[10px] font-mono text-slate-500 uppercase tracking-wider">Scaling Recommendation</span>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div className="text-center rounded p-2 bg-white/3">
          <div className="text-[10px] text-slate-500 mb-1">Current</div>
          <div className="font-mono text-xs text-slate-300">{rec.currentMin}–{rec.currentMax} replicas</div>
        </div>
        <div className="text-center rounded p-2" style={{ background: hasChange ? "rgba(201,162,39,0.06)" : "rgba(255,255,255,0.02)", border: hasChange ? "1px solid rgba(201,162,39,0.2)" : "none" }}>
    ...
    <div className="imperial-card rounded-lg overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="font-display text-sm tracking-[0.12em] font-bold" style={{ color }}>{profile.name}</div>
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="flex items-center gap-1.5">
            <span className="font-mono text-xs font-bold" style={{ color }}>{profile.aquilaScore}</span>
            <span className="text-[10px] text-slate-600">{getAquilaLabel(profile.aquilaScore)}</span>
          </div>
          <FailureProbabilityGauge probability={profile.failureProbability} />
        </div>
        {expanded ? <ChevronUp className="w-4 h-4 text-slate-600 flex-shrink-0 ml-1" /> : <ChevronDown className="w-4 h-4 text-slate-600 flex-shrink-0 ml-1" />}
      </button>
        ...
        <div className="px-4 pb-4 border-t border-white/5 pt-3 space-y-4">
          ...
          <div className="rounded p-3 bg-white/3 border border-white/5">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] font-mono text-slate-500 uppercase tracking-wider">Battle Readiness Report</span>
            </div>
          ...
          <div className="rounded p-3 bg-white/3 border border-white/5">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] font-mono text-slate-500 uppercase tracking-wider">Failure Prediction</span>
            </div>
          ...
          {profile.metrics && (
            <div>
              <div className="text-[10px] font-mono text-slate-500 uppercase tracking-wider mb-2">Live Metrics</div>
              <div className="grid grid-cols-1 gap-2">
                {Object.entries(profile.metrics).map(([key, val]) => {
                  const labels: Record<string, string> = {
                    cpu: "CPU Utilization", memory: "Memory Utilization", replicaCount: "Replica Count",
                    p95Latency: "P95 Latency (ms)", requestsPerSec: "Requests/sec",
                    connectionPoolPct: "Connection Pool", backupAge: "Backup Age (hrs)",
                    storagePct: "Storage Used (%)", hitRate: "Cache Hit Rate", evictedKeys: "Evicted Keys",
                    connectedClients: "Connected Clients", activeApps: "Active Apps",
                    avgLatency: "Avg Latency (ms)", cacheHitRate: "CDN Cache Hit", wafBlocks: "WAF Blocks",
                    rateLimit: "Rate Limit", activeManagedRules: "Active Rules", sslDaysRemaining: "SSL Validity"
                  };
                  const label = labels[key] || key;
                  const isPercent = ["cpu", "memory", "connectionPoolPct", "storagePct", "hitRate", "cacheHitRate"].includes(key);
                  return (
                    <MetricBar
                      key={key}
                      label={label}
...
export default function CenturionAI() {
  const avgAquila = Math.round(CENTURION_PROFILES.reduce((a, p) => a + p.aquilaScore, 0) / CENTURION_PROFILES.length);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-slate-500 ml-8">
          Autonomous AI agent profiles for each service cluster — readiness scoring, scaling recommendations, failure prediction
        </p>
      </div>
