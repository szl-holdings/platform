...
const categoryConfig: Record<InsightCategory, {
  ...
  success: { label: "Success", icon: CheckCircle, color: "text-emerald-400", bg: "bg-emerald-400/8", border: "border-emerald-400/20" },
  warning: { label: "Warning", icon: AlertTriangle, color: "text-amber-400", bg: "bg-amber-400/8", border: "border-amber-400/20" },
  trend: { label: "Trend", icon: TrendingUp, color: "text-blue-400", bg: "bg-blue-400/8", border: "border-blue-400/20" },
  discovery: { label: "Discovery", icon: Sparkles, color: "text-violet-400", bg: "bg-violet-400/8", border: "border-violet-400/20" },
};
...
function InsightCard({ insight }: { insight: typeof insights[0] }) {
  ...
  return (
    <div className={cn("border rounded-xl p-5 transition-all duration-200 hover:shadow-lg", config.bg, config.border, "bg-card/60 backdrop-blur-sm")}>
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          <Icon className={cn("w-4 h-4", config.color)} />
          <span className={cn("text-[10px] font-mono uppercase px-2 py-0.5 rounded-full", config.color, `${config.bg}`)}>
            {config.label}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className={cn("text-[10px] font-mono px-2 py-0.5 rounded-full", impactColors[insight.impact])}>
            {insight.impact}
          </span>
        </div>
      </div>

      <h3 className="text-sm font-display font-semibold text-foreground mb-2 leading-snug">{insight.title}</h3>
      <p className="text-xs text-muted-foreground leading-relaxed mb-4">{insight.description}</p>

      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
          <span className="font-mono bg-muted/30 px-2 py-0.5 rounded">{insight.sourceExperiment}</span>
  ...
  return (
    <div className="p-6 lg:p-8 space-y-5 max-w-[1200px]">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Key findings, trends, and observations from AI/ML research</p>
      </div>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  categoryFilter === cat
                    ? cat === "all" ? "bg-primary/15 text-primary" : cn(config!.bg, config!.color)
