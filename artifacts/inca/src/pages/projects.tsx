  ...
  return (
    <Link href={`/projects/${project.id}`}>
      <div className="bg-card/60 backdrop-blur-sm border border-border rounded-lg p-4 hover:border-primary/30 transition-all duration-200 cursor-pointer group">
        ...
        <h3 className="text-sm font-display font-semibold text-foreground mb-1 group-hover:text-primary transition-colors">{project.name}</h3>
        <p className="text-xs text-muted-foreground line-clamp-2 mb-3">{project.description}</p>
        <div className="flex items-center gap-4 text-[10px] text-muted-foreground">
          <span>Acc: <span className="text-foreground font-medium">{project.accuracy}%</span></span>
          <span>Loss: <span className="text-foreground font-medium">{project.loss.toFixed(3)}</span></span>
          <span>Inf: <span className="text-foreground font-medium">{project.inferenceTime}ms</span></span>
        </div>
        <div className="mt-3">
          ...
          <div className="h-1 bg-border rounded-full overflow-hidden">
            <div className="h-full bg-primary/60 rounded-full transition-all duration-500" style={{ width: `${project.progress}%` }} />
          </div>
        </div>
        <div className="mt-3 flex items-center justify-between">
          <div className="flex -space-x-1.5">
            {project.team.slice(0, 3).map((m) => (
              <div key={m.avatar} className="w-6 h-6 rounded-full bg-primary/20 border border-background flex items-center justify-center text-[8px] font-medium text-primary">
                {m.avatar}
              </div>
            ))}
  ...
  return (
    <Link href={`/projects/${project.id}`}>
      <div className="flex items-center gap-4 p-3 rounded-lg hover:bg-muted/30 transition-colors cursor-pointer group border-b border-border/50">
        <span className={cn("text-[10px] font-mono uppercase px-2 py-0.5 rounded-full border min-w-[90px] text-center", statusColors[project.status])}>
          {statusLabels[project.status]}
        </span>
        ...
        <span className="text-xs text-muted-foreground font-mono hidden md:block">{project.domain}</span>
        ...
        <div className="flex -space-x-1.5 hidden md:flex">
          {project.team.slice(0, 2).map((m) => (
            <div key={m.avatar} className="w-5 h-5 rounded-full bg-primary/20 border border-background flex items-center justify-center text-[7px] font-medium text-primary">
              {m.avatar}
            </div>
          ))}
        </div>
...
function KanbanColumn({ status, items }: { status: ProjectStatus; items: Project[] }) {
  return (
    <div className={cn("flex-1 min-w-[260px] border rounded-xl p-3", kanbanColors[status], "bg-card/30")}>
      <div className="flex items-center gap-2 mb-3 px-1">
        <div className={cn("w-2 h-2 rounded-full", kanbanDotColors[status])} />
        <h3 className="text-xs font-display font-semibold text-foreground uppercase tracking-wider">{statusLabels[status]}</h3>
  ...
  return (
    <div className="p-6 lg:p-8 space-y-5 max-w-[1600px]">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Track and manage AI/ML research projects</p>
      </div>
