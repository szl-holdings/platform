...
const alerts = [
  ...
  { id: "ALT-006", title: "Training Job Completed", severity: "info", source: "Job Scheduler", timestamp: "1.5 hr ago", status: "resolved", category: "pipeline", description: "AnomalyDetector v2.6 training completed with 96.2% validation accuracy" },
  { id: "ALT-007", title: "Feature Store Sync Delayed", severity: "low", source: "Feature Store", timestamp: "2 hr ago", status: "resolved", category: "data", description: "Feature store sync delayed by 12 minutes due to upstream API throttling" },
  { id: "ALT-008", title: "New Model Version Available", severity: "info", source: "Model Registry", timestamp: "3 hr ago", status: "resolved", category: "registry", description: "TimeSeriesNet v4.1 passed all validation gates and is ready for deployment" },
];
        ...
        {["all", "critical", "high", "medium", "low", "info"].map(s => (
          <button key={s} onClick={() => setSeverityFilter(s)}
            className={cn("px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-colors",
              severityFilter === s ? "bg-primary/15 text-primary" : "text-muted-foreground hover:bg-muted/50"
            )}>{s}</button>
        ))}
      </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1"><Zap className="w-3 h-3" /> {alert.source}</span>
