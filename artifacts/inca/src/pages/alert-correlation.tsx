...
const correlationGroups = [
  {
    ...
    alerts: [
      ...
      { id: "ALT-004", title: "Accuracy Drop - CausalInference v1.3", type: "accuracy", delay: "+3 min" },
    ...
    recommendation: "Investigate upstream data source for schema changes; revalidate feature engineering pipeline",
  },
  {
    id: "CG-002",
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-card/60 border border-border rounded-xl p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Correlation Groups</p>
          <p className="text-2xl font-bold text-foreground">{correlationGroups.length}</p>
        </div>
        <div className="bg-card/60 border border-border rounded-xl p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Total Correlated Alerts</p>
          <p className="text-2xl font-bold text-foreground">{correlationGroups.reduce((s, g) => s + g.alerts.length, 0)}</p>
        </div>
        <div className="bg-card/60 border border-border rounded-xl p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Root Causes Identified</p>
          <p className="text-2xl font-bold text-foreground">{correlationGroups.length}</p>
        </div>
      </div>
          ...
          <div key={group.id} className="bg-card/60 border border-border rounded-xl p-6">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={cn("px-2 py-0.5 rounded-full text-[10px] font-medium uppercase tracking-wider",
                    group.severity === "critical" ? "bg-red-400/10 text-red-400" :
                    group.severity === "high" ? "bg-orange-400/10 text-orange-400" :
                    "bg-amber-400/10 text-amber-400"
                  )}>{group.severity}</span>
                </div>
                <h3 className="text-lg font-semibold text-foreground">{group.title}</h3>
              </div>
              <span className="text-xs text-muted-foreground flex items-center gap-1"><Clock className="w-3 h-3" /> {group.timestamp}</span>
            </div>
              ...
              {group.alerts.map((alert, i) => (
                <div key={alert.id} className="relative pl-8 py-2">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-mono text-muted-foreground w-16">{alert.delay}</span>
                    ...
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground capitalize">{alert.type}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-red-400/5 border border-red-400/10 rounded-lg p-3">
                <p className="text-xs text-red-400 font-medium mb-1 flex items-center gap-1"><AlertTriangle className="w-3 h-3" /> Impact</p>
              ...
              <div className="bg-emerald-400/5 border border-emerald-400/10 rounded-lg p-3">
                <p className="text-xs text-emerald-400 font-medium mb-1 flex items-center gap-1"><Brain className="w-3 h-3" /> Recommendation</p>
