...
export default function SeedManagerPage() {
  ...
  const displayResults = showAllResults ? validation?.results ?? [] : failedResults;
          ...
          <div className="space-y-4">
            ...
            {failedResults.length > 0 && (
              <div className="space-y-1">
                {failedResults.map((r) => (
                  <div key={r.table} className="flex items-center gap-3 p-2 rounded-md bg-red-500/5 border border-red-500/20">
                    <XCircle className="w-4 h-4 text-red-400 shrink-0" />
                    <span className="text-sm font-mono flex-1">{r.table}</span>
                    <span className="text-xs text-muted-foreground">{r.description}</span>
                    <span className="text-xs font-mono text-red-400">{r.actual}/{r.expected}</span>
                  </div>
                ))}
              </div>
            )}
              ...
              <div>
                ...
                {showAllResults && (
                  <div className="space-y-1 mt-2">
                    {passedResults.map((r) => (
                      <div key={r.table} className="flex items-center gap-3 p-2 rounded-md bg-emerald-500/5 border border-emerald-500/10">
                        <CheckCircle className="w-4 h-4 text-emerald-400 shrink-0" />
                        <span className="text-sm font-mono flex-1">{r.table}</span>
                        <span className="text-xs text-muted-foreground">{r.description}</span>
                        <span className="text-xs font-mono text-emerald-400">{r.actual} rows</span>
                      </div>
                    ))}
                  </div>
                )}
