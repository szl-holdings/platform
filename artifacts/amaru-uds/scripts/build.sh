#!/usr/bin/env bash
# Amaru.UDS build — pure-ESM kernel, no TS compilation needed.
set -euo pipefail
...
stage_v2_shared_packages "${ARTIFACT_DIR}/build" "${REPO_ROOT}" "amaru-uds"
...
const manifest = {
  product: "amaru-uds", version: process.env.VERSION, git_sha: process.env.GIT_SHA, build_ts: process.env.BUILD_TS,
  doctrine: "V6", entries,
};
...
# Zarf package
DIST_DIR="${REPO_ROOT}/dist/amaru-uds"
mkdir -p "${DIST_DIR}"
TARBALL="${DIST_DIR}/amaru-uds-${VERSION}.tar.zst"
...
if command -v zarf >/dev/null 2>&1; then
  ...
  PRODUCED="$(ls -1t "${DIST_DIR}"/zarf-package-amaru-uds-*.tar.zst 2>/dev/null | head -n1 || true)"
...
# Sign — auto-discover cosign + default to .local/cosign/cosign.key.
...
if [[ -n "${COSIGN_BIN}" && -f "${COSIGN_KEY}" ]]; then
  ...
  if [[ -f "${COSIGN_PUB}" ]]; then
    cp -f "${COSIGN_PUB}" "${DIST_DIR}/amaru-uds-dev.pub"
    log "wrote ${DIST_DIR}/amaru-uds-dev.pub"
