...
export function Pce() {
  ...
  const linkedProof = selectedContract ? SEED_PROOF_PACKETS.find(p => p.id === selectedContract.proofPacketId) : null;
  ...
  return (
    <Layout>
      ...
      {/* Concept callout */}
      <div className="mb-6 p-4 rounded-lg border" style={{ backgroundColor: 'rgba(176,141,82,0.06)', borderColor: 'rgba(176,141,82,0.25)' }}>
        <div className="text-sm font-semibold mb-1" style={{ color: '#b08d52' }}>What is a PCE Contract?</div>
        <div className="text-xs" style={{ color: 'var(--color-a11oy-text-sub)' }}>
          A Proof-Carrying Execution (PCE) contract is the formal governance record that binds a workcell to its causal chain. It records: the originating signal (originSignalId), the full causal chain (causalChainIds), the policy evaluation result (policyEvaluationId), the human approval record (approvalRecordId), the execution trace (executionTraceId), and the cryptographic proof packet (proofPacketId). Every executed A11oy workcell has a corresponding PCE contract — no exceptions.
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <HashId id={c.id} />
                        <span className="font-mono text-xs px-1.5 py-0.5 rounded" style={{ backgroundColor: `${modeColor}18`, color: modeColor }}>
                          {c.mode}
                        </span>
                        {c.isVerified && (
                      ...
                      <div className="grid sm:grid-cols-2 gap-x-4 gap-y-0.5 text-xs">
                        <div style={{ color: 'var(--color-a11oy-text-ghost)' }}>
                          Origin: <span style={{ color: 'var(--color-a11oy-text-sub)' }}>{c.originSignalId}</span>
                        </div>
                        <div style={{ color: 'var(--color-a11oy-text-ghost)' }}>
                          Action: <span style={{ color: 'var(--color-a11oy-text-sub)' }}>{c.actionId}</span>
                        </div>
                        {wc && (
                          <div className="sm:col-span-2" style={{ color: 'var(--color-a11oy-text-ghost)' }}>
                            Workcell: <span style={{ color: 'var(--color-a11oy-blue)' }}>{wc.name}</span>
                          </div>
                        )}
                      </div>
                    </div>
                      ...
                      <div className="grid sm:grid-cols-2 gap-3 text-xs">
                        <div>
                          ...
                          {c.causalChainIds.map(id => (
                            <div key={id} className="font-mono" style={{ color: 'var(--color-a11oy-text-ghost)' }}>→ {id}</div>
                          ))}
                        </div>
                        <div>
                          <div className="font-mono mb-1" style={{ color: 'var(--color-a11oy-text-ghost)' }}>RECORD REFS</div>
                          <div style={{ color: 'var(--color-a11oy-text-ghost)' }}>Policy: {c.policyEvaluationId}</div>
                          {c.approvalRecordId && <div style={{ color: 'var(--color-a11oy-text-ghost)' }}>Approval: {c.approvalRecordId}</div>}
                          <div style={{ color: 'var(--color-a11oy-text-ghost)' }}>Trace: {c.executionTraceId}</div>
                          <div style={{ color: 'var(--color-a11oy-text-ghost)' }}>Proof: {c.proofPacketId}</div>
                        </div>
                      </div>
                    </div>
                  )}
        ...
        <div className="flex flex-col gap-6">
              ...
              <Card className="text-xs">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  ...
                  <div className="col-span-2">
                    ...
                    <div style={{ color: 'var(--color-a11oy-text-sub)' }}>{selectedContract.originSignalId}</div>
                  </div>
                  <div className="col-span-2">
                    <div className="font-mono mb-1" style={{ color: 'var(--color-a11oy-text-ghost)' }}>CAUSAL CHAIN ({selectedContract.causalChainIds.length})</div>
                ...
                {linkedWC && (
                  <div className="mt-3 pt-3 border-t" style={{ borderColor: 'var(--color-a11oy-border)' }}>
                    ...
                    <div className="font-medium" style={{ color: 'var(--color-a11oy-text)' }}>{linkedWC.name}</div>
                    <VerdictBadge verdict={linkedWC.mirrorEvalResult.verdict} />
                  </div>
                )}
                {linkedProof && (
                  <div className="mt-3 pt-3 border-t" style={{ borderColor: 'var(--color-a11oy-border)' }}>
                    <div className="font-mono mb-1" style={{ color: 'var(--color-a11oy-text-ghost)' }}>PROOF PACKET HASH</div>
                    <div className="font-mono break-all" style={{ color: '#b08d52' }}>{linkedProof.hash.slice(0, 40)}…</div>
                  </div>
                )}
              </Card>
            </div>
                ...
                return (
                  <Card key={mode} className="text-xs">
                    <div className="flex items-center justify-between">
                      <span className="font-mono" style={{ color }}>{mode}</span>
                      <span className="font-mono" style={{ color }}>{count} contracts</span>
                    </div>
                  </Card>
                );
              })}
            ...
            <Card className="text-xs">
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono" style={{ color: 'var(--color-a11oy-text-sub)' }}>{SEED_PCE_CONTRACTS.length}</span>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono" style={{ color: '#b08d52' }}>{SEED_PCE_CONTRACTS.filter(c => c.proofPacketId).length}</span>
                </div>
              </div>
            </Card>
          </div>
          ...
          <div className="p-3 rounded-lg" style={{ backgroundColor: 'rgba(176,141,82,0.08)', border: '1px solid rgba(176,141,82,0.25)' }}>
            <div className="text-xs font-semibold mb-1" style={{ color: '#b08d52' }}>Constitutional Guarantee</div>
            <div className="text-xs" style={{ color: 'var(--color-a11oy-text-sub)' }}>
              No material action executes without a valid PCE contract. This is a structural guarantee — not a configuration option.
            </div>
          </div>
        </div>
      </div>
