    ...
    <Layout>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-mono" style={{ color: GHOST }}>VERTICAL</span>
          <button
            onClick={() => setVerticalFilter('all')}
            className="text-[10px] font-mono px-2 py-1 rounded"
            style={{ backgroundColor: verticalFilter === 'all' ? `${GOLD}18` : 'transparent', color: verticalFilter === 'all' ? GOLD : GHOST, border: `1px solid ${verticalFilter === 'all' ? `${GOLD}40` : 'transparent'}` }}
          >
            ALL
          </button>
          {verticals.map(v => (
            <button
              ...
              className="text-[10px] font-mono px-2 py-1 rounded"
              style={{ backgroundColor: verticalFilter === v.id ? `${GOLD}18` : 'transparent', color: verticalFilter === v.id ? GOLD : GHOST, border: `1px solid ${verticalFilter === v.id ? `${GOLD}40` : 'transparent'}` }}
            >
        ...
        {sorted.map(twin => {
          ...
          return (
            <div
              ...
              style={{ backgroundColor: SURFACE, borderColor: drawerTwin?.id === twin.id ? GOLD : BORDER }}
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span style={{ color: v?.colorToken ?? GOLD, fontSize: '1.1rem' }}>{v?.icon}</span>
                  <span className="font-semibold" style={{ color: TEXT }}>{twin.name}</span>
                </div>
              ...
              <p className="text-xs mb-4" style={{ color: SUB }}>{twin.description}</p>
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="text-center">
                  <div className="text-lg font-bold" style={{ color: GOLD }}>{twin.healthScore}%</div>
                  <div className="text-[10px] font-mono" style={{ color: GHOST }}>Health</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold" style={{ color: TEXT }}>{twin.signalVolume}</div>
                  <div className="text-[10px] font-mono" style={{ color: GHOST }}>Signals</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold" style={{ color: twin.activeRisks > 15 ? '#f59e0b' : TEXT }}>{twin.activeRisks}</div>
                  <div className="text-[10px] font-mono" style={{ color: GHOST }}>Risks</div>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2 text-[10px] font-mono">
                <div><span style={{ color: GHOST }}>Decisions</span> <span style={{ color: TEXT }}>{twin.pendingDecisions}</span></div>
                <div><span style={{ color: GHOST }}>Approvals</span> <span style={{ color: twin.openApprovals > 4 ? '#f59e0b' : TEXT }}>{twin.openApprovals}</span></div>
                <div><span style={{ color: GHOST }}>Evidence</span> <span style={{ color: TEXT }}>{twin.evidenceCompleteness}%</span></div>
                <div><span style={{ color: GHOST }}>Proof Chain</span> <span style={{ color: TEXT }}>{twin.proofChainCoverage}%</span></div>
                <div><span style={{ color: GHOST }}>Chainlight</span> <span style={{ color: GOLD }}>{twin.chainlightConfidence}</span></div>
                <div><span style={{ color: GHOST }}>Velocity</span> <span style={{ color: TEXT }}>{twin.outcomeVelocity}</span></div>
              </div>
        ...
        <div className="fixed inset-y-0 right-0 w-full max-w-lg z-50 overflow-y-auto" style={{ backgroundColor: '#0f0f0f', borderLeft: `1px solid ${BORDER}` }}>
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <span className="font-semibold text-lg" style={{ color: TEXT }}>{drawerTwin.name}</span>
              <button onClick={() => setDrawerTwin(null)} className="text-sm font-mono px-3 py-1 rounded" style={{ color: GHOST, border: `1px solid ${BORDER}` }}>Close</button>
            </div>
            <p className="text-sm mb-4" style={{ color: SUB }}>{drawerTwin.description}</p>
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              <Card>
                <div className="text-[10px] font-mono" style={{ color: GHOST }}>Sentra</div>
                <div className="text-sm font-bold" style={{ color: GOVERNANCE_COLORS[drawerTwin.sentraGovernanceState] }}>{drawerTwin.sentraGovernanceState.toUpperCase()}</div>
              </Card>
              <Card>
                <div className="text-[10px] font-mono" style={{ color: GHOST }}>PSYCHE</div>
                <div className="text-sm font-bold" style={{ color: GOVERNANCE_COLORS[drawerTwin.psycheGovernanceState] }}>{drawerTwin.psycheGovernanceState.toUpperCase()}</div>
              </Card>
            </div>
            ...
            <div className="flex flex-wrap gap-1">
              {drawerTwin.linkedRoutes.map(r => (
                <span key={r} className="text-[10px] font-mono px-2 py-0.5 rounded" style={{ backgroundColor: `${GOLD}10`, color: GOLD }}>{r}</span>
              ))}
            </div>
          </div>
        </div>
