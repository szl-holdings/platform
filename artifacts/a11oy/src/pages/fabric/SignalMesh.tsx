...
const SIGNAL_TYPES: readonly SignalType[] = ['risk', 'opportunity', 'deadline', 'anomaly', 'drift', 'compliance', 'cost', 'vendor', 'document', 'operational', 'security', 'legal_workflow', 'executive_decision'];
...
export function FabricSignalMesh() {
  ...
  return (
    <Layout>
      ...
      <div className="text-[10px] font-mono mb-3" style={{ color: GHOST }}>{signals.length} signals matching filters</div>

      <div className="overflow-x-auto mb-8">
        <table className="w-full text-left text-xs" style={{ color: TEXT }}>
          <thead>
            <tr style={{ borderBottom: `1px solid ${BORDER}` }}>
              <th className="pb-2 font-mono text-[10px]" style={{ color: GHOST }}>SEVERITY</th>
              <th className="pb-2 font-mono text-[10px]" style={{ color: GHOST }}>SIGNAL</th>
              ...
              <th className="pb-2 font-mono text-[10px]" style={{ color: GHOST }}>CONFIDENCE</th>
              <th className="pb-2 font-mono text-[10px]" style={{ color: GHOST }}>STATUS</th>
              <th className="pb-2 font-mono text-[10px]" style={{ color: GHOST }}>SENTRA</th>
              <th className="pb-2 font-mono text-[10px]" style={{ color: GHOST }}>ACTION</th>
            </tr>
          </thead>
          <tbody>
                ...
                >
                  <td className="py-2 pr-3">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] font-mono" style={{ color: SEVERITY_COLORS[s.severity] }}>{s.severity}</span>
                    </div>
                  </td>
                  <td className="py-2 pr-3 max-w-[220px] truncate">{s.title}</td>
                  <td className="py-2 pr-3"><span className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ backgroundColor: `${GOLD}18`, color: GOLD }}>{s.verticalId}</span></td>
                  <td className="py-2 pr-3 text-[10px] font-mono" style={{ color: SUB }}>{s.signalType}</td>
                  <td className="py-2 pr-3 text-[10px] font-mono" style={{ color: GOLD }}>{s.confidence}</td>
                  <td className="py-2 pr-3">
                    <span className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ backgroundColor: isTriaged ? '#22c55e18' : 'rgba(255,255,255,0.05)', color: isTriaged ? '#22c55e' : SUB }}>
                      {isTriaged ? 'TRIAGED' : s.status}
                    </span>
                  </td>
                  <td className="py-2 pr-3">{s.sentraReviewRequired && <span className="text-[10px] font-mono" style={{ color: '#f59e0b' }}>REQUIRED</span>}</td>
                  <td className="py-2">
                    {!isTriaged && s.status === 'new' && (
                      <button
                        onClick={e => { e.stopPropagation(); markTriaged(s.id); }}
        ...
        <div className="fixed inset-y-0 right-0 w-full max-w-lg z-50 overflow-y-auto" style={{ backgroundColor: '#0f0f0f', borderLeft: `1px solid ${BORDER}` }}>
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <span className="font-semibold" style={{ color: TEXT }}>{drawerSignal.title}</span>
              <button onClick={() => setDrawerSignal(null)} className="text-sm font-mono px-3 py-1 rounded" style={{ color: GHOST, border: `1px solid ${BORDER}` }}>Close</button>
            </div>
            <p className="text-sm mb-4" style={{ color: SUB }}>{drawerSignal.description}</p>
            <div className="grid grid-cols-2 gap-3 mb-4">
              <DetailRow label="Severity" value={drawerSignal.severity} color={SEVERITY_COLORS[drawerSignal.severity]} />
              ...
              <DetailRow label="Status" value={localTriaged.has(drawerSignal.id) ? 'triaged' : drawerSignal.status} />
              <DetailRow label="Entity" value={drawerSignal.relatedEntity} />
              <DetailRow label="Sentra" value={drawerSignal.sentraReviewRequired ? 'Required' : 'Not required'} color={drawerSignal.sentraReviewRequired ? '#f59e0b' : undefined} />
            </div>
            <div className="text-xs p-3 rounded mb-4" style={{ backgroundColor: `${GOLD}08`, color: TEXT }}>
              <div className="text-[10px] font-mono mb-1" style={{ color: GOLD }}>RECOMMENDED ACTION</div>
...
function DetailRow({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="rounded p-2" style={{ backgroundColor: SURFACE }}>
      <div className="text-[10px] font-mono" style={{ color: GHOST }}>{label}</div>
      <div className="text-xs font-medium" style={{ color: color ?? TEXT }}>{value}</div>
    </div>
  );
}
