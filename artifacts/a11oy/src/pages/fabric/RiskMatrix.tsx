...
export function RiskMatrix() {
  ...
  return (
    <Layout>
        ...
        {VERTICALS.map(v => {
          ...
          return (
            <div key={v.id} className="rounded p-2 text-center cursor-pointer" onClick={() => setVerticalFilter(v.id)} style={{ backgroundColor: `${scoreColor(maxScore)}18`, border: verticalFilter === v.id ? `1px solid ${GOLD}` : `1px solid transparent` }}>
              ...
              <div className="text-[10px] font-mono" style={{ color: TEXT }}>{v.name}</div>
              <div className="text-lg font-bold" style={{ color: scoreColor(maxScore) }}>{openCount}</div>
              <div className="text-[10px] font-mono" style={{ color: GHOST }}>open</div>
            </div>
          );
        })}
      </div>
      ...
      <div className="text-[10px] font-mono mb-3" style={{ color: GHOST }}>{risks.length} risks matching filters</div>

      <div className="flex flex-col gap-2 mb-8">
        {risks.slice(0, 40).map(r => (
          <div key={r.id} className="rounded border p-3 flex items-start gap-3 cursor-pointer hover:border-[#c9b787] transition-colors" onClick={() => setDrawerRisk(r)} style={{ backgroundColor: SURFACE, borderColor: drawerRisk?.id === r.id ? GOLD : BORDER }}>
            <div className="w-10 h-10 rounded flex items-center justify-center flex-shrink-0 font-mono text-sm font-bold" style={{ backgroundColor: `${scoreColor(r.riskScore)}18`, color: scoreColor(r.riskScore) }}>
              {r.riskScore}
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-xs font-medium truncate mb-1" style={{ color: TEXT }}>{r.title}</div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ backgroundColor: `${GOLD}18`, color: GOLD }}>{r.verticalId}</span>
                <span className="text-[10px] font-mono" style={{ color: SUB }}>{r.riskCategory}</span>
                <span className="text-[10px] font-mono" style={{ color: GHOST }}>P:{r.probability} I:{r.impact} V:{r.velocity}</span>
                <span className="text-[10px] font-mono" style={{ color: GHOST }}>{r.owner}</span>
                <span className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ backgroundColor: r.status === 'open' ? '#f59e0b18' : 'rgba(255,255,255,0.05)', color: r.status === 'open' ? '#f59e0b' : SUB }}>{r.status}</span>
                {r.approvalRequired && <span className="text-[10px] font-mono" style={{ color: '#f59e0b' }}>APPROVAL</span>}
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-y-0 right-0 w-full max-w-lg z-50 overflow-y-auto" style={{ backgroundColor: '#0f0f0f', borderLeft: `1px solid ${BORDER}` }}>
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <span className="font-semibold" style={{ color: TEXT }}>{drawerRisk.title}</span>
              <button onClick={() => setDrawerRisk(null)} className="text-sm font-mono px-3 py-1 rounded" style={{ color: GHOST, border: `1px solid ${BORDER}` }}>Close</button>
            </div>
            <p className="text-sm mb-4" style={{ color: SUB }}>{drawerRisk.description}</p>
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="rounded p-3 text-center" style={{ backgroundColor: SURFACE }}>
                <div className="text-[10px] font-mono" style={{ color: GHOST }}>PROBABILITY</div>
                <div className="text-xl font-bold" style={{ color: GOLD }}>{drawerRisk.probability}</div>
              </div>
              <div className="rounded p-3 text-center" style={{ backgroundColor: SURFACE }}>
                <div className="text-[10px] font-mono" style={{ color: GHOST }}>IMPACT</div>
                <div className="text-xl font-bold" style={{ color: GOLD }}>{drawerRisk.impact}</div>
              </div>
              <div className="rounded p-3 text-center" style={{ backgroundColor: SURFACE }}>
                <div className="text-[10px] font-mono" style={{ color: GHOST }}>VELOCITY</div>
                <div className="text-xl font-bold" style={{ color: GOLD }}>{drawerRisk.velocity}</div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="rounded p-2" style={{ backgroundColor: SURFACE }}><div className="text-[10px] font-mono" style={{ color: GHOST }}>Risk Score</div><div className="text-lg font-bold" style={{ color: scoreColor(drawerRisk.riskScore) }}>{drawerRisk.riskScore}</div></div>
              <div className="rounded p-2" style={{ backgroundColor: SURFACE }}><div className="text-[10px] font-mono" style={{ color: GHOST }}>Category</div><div className="text-xs" style={{ color: TEXT }}>{drawerRisk.riskCategory}</div></div>
              <div className="rounded p-2" style={{ backgroundColor: SURFACE }}><div className="text-[10px] font-mono" style={{ color: GHOST }}>Owner</div><div className="text-xs" style={{ color: TEXT }}>{drawerRisk.owner}</div></div>
              <div className="rounded p-2" style={{ backgroundColor: SURFACE }}><div className="text-[10px] font-mono" style={{ color: GHOST }}>Status</div><div className="text-xs" style={{ color: drawerRisk.status === 'open' ? '#f59e0b' : TEXT }}>{drawerRisk.status}</div></div>
            </div>
            <div className="text-xs p-3 rounded mb-4" style={{ backgroundColor: `${GOLD}08`, color: TEXT }}>
              <div className="text-[10px] font-mono mb-1" style={{ color: GOLD }}>MITIGATION</div>
...
function FilterSelect({ label, value, onChange, options }: { label: string; value: string; onChange: (v: string) => void; options: { value: string; label: string }[] }) {
  return (
    <div className="flex items-center gap-1">
      <span className="text-[10px] font-mono mr-1" style={{ color: GHOST }}>{label}</span>
      <select value={value} onChange={e => onChange(e.target.value)} className="text-[10px] font-mono px-2 py-1 rounded bg-transparent" style={{ color: GOLD, border: `1px solid ${GOLD}40`, outline: 'none' }}>
