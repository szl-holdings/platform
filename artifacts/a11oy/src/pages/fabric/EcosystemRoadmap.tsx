    ...
    <Layout>
      ...
      <div className="flex flex-col gap-3 mb-8">
        {filtered.map(phase => {
          ...
          return (
            <div key={phase.id} className="rounded-lg border overflow-hidden" style={{ backgroundColor: SURFACE, borderColor: isExpanded ? GOLD : BORDER }}>
              <div
                className="p-4 cursor-pointer flex items-center justify-between"
                onClick={() => setExpandedPhase(isExpanded ? null : phase.id)}
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded flex items-center justify-center font-mono text-sm font-bold" style={{ backgroundColor: `${STATUS_COLORS[phase.status]}18`, color: STATUS_COLORS[phase.status] }}>
                    {phase.phase}
                  </div>
                  <div>
                    <div className="text-sm font-medium" style={{ color: TEXT }}>{phase.title}</div>
                    <div className="text-[10px] font-mono" style={{ color: GHOST }}>{phase.items.length} deliverables</div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded" style={{ backgroundColor: `${STATUS_COLORS[phase.status]}18`, color: STATUS_COLORS[phase.status] }}>
                    {phase.status.toUpperCase()}
                  </span>
                  <span className="text-sm" style={{ color: GHOST }}>{isExpanded ? '−' : '+'}</span>
                </div>
              </div>
              {isExpanded && (
                <div className="px-4 pb-4 border-t" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
                  <p className="text-xs mt-3 mb-3" style={{ color: SUB }}>{phase.description}</p>
        ...
        {FABRIC_AGENTS.map(agent => (
          <Card key={agent.id}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ backgroundColor: `${GOLD}18`, color: GOLD }}>
                {agent.verticalCoverage.length === 7 ? 'ALL VERTICALS' : agent.verticalCoverage.join(', ')}
              </span>
            </div>
            <p className="text-xs mb-3" style={{ color: SUB }}>{agent.role}</p>
            <div className="flex flex-wrap gap-1 mb-2">
              {agent.inputTypes.map(t => (
                <span key={t} className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ backgroundColor: 'rgba(255,255,255,0.05)', color: GHOST }}>{t}</span>
              ))}
            </div>
            <div className="flex flex-wrap gap-1 mb-2">
              {agent.outputTypes.map(t => (
                <span key={t} className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ backgroundColor: `${GOLD}10`, color: GOLD }}>{t}</span>
              ))}
            </div>
