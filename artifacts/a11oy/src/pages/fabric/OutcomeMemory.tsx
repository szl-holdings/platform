...
export function OutcomeMemory() {
  ...
  const avgError = Math.round(
    (FABRIC_OUTCOMES.reduce((s, o) => s + o.predictionError, 0) / FABRIC_OUTCOMES.length) * 100,
  );
  const avgReward =
    Math.round(
      (FABRIC_OUTCOMES.reduce((s, o) => s + o.rewardScore, 0) / FABRIC_OUTCOMES.length) * 100,
    ) / 100;
    ...
    <Layout>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        ...
        <KpiCard
          ...
          accent={avgError > 15 ? '#f59e0b' : GOLD}
        />
        <KpiCard label="AVG REWARD" value={avgReward} sub="reward score" accent={GOLD} />
        <KpiCard
          label="POLICY CANDIDATES"
          value={policyCandidates}
          sub="update suggested"
          accent={GOLD}
        />
      </div>
              ...
              <div className="flex items-start justify-between gap-3 mb-2">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs mb-0.5" style={{ color: SUB }}>
                    Predicted: <span style={{ color: TEXT }}>{o.predictedOutcome}</span>
                  </div>
                  <div className="text-xs" style={{ color: SUB }}>
                    Actual: <span style={{ color: TEXT }}>{o.actualOutcome}</span>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-4 text-[10px] font-mono mb-2">
                <span style={{ color: o.predictionError > 0.15 ? '#f59e0b' : GHOST }}>
                  Error: {Math.round(o.predictionError * 100)}%
                </span>
                <span style={{ color: GOLD }}>Reward: {o.rewardScore}</span>
                <span style={{ color: riskDelta > 0 ? '#22c55e' : '#ef4444' }}>
                  Risk Δ: {riskDelta > 0 ? '-' : '+'}
                  {Math.abs(riskDelta)}%
                </span>
                <span style={{ color: GHOST }}>
                  Evidence: {Math.round(o.evidenceCompleteness * 100)}%
                </span>
              </div>
              <div
                className="text-xs p-2 rounded"
                style={{ backgroundColor: `${GOLD}06`, color: SUB }}
              >
          ...
          <div className="p-6">
            ...
            <div className="mb-4">
              ...
              <p className="text-sm mb-3" style={{ color: TEXT }}>
                {drawerOutcome.predictedOutcome}
              </p>
              <div className="text-[10px] font-mono mb-1" style={{ color: GOLD }}>
                ACTUAL
              </div>
              <p className="text-sm mb-3" style={{ color: TEXT }}>
                {drawerOutcome.actualOutcome}
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="rounded p-2" style={{ backgroundColor: SURFACE }}>
                ...
                <div
                  ...
                  style={{ color: drawerOutcome.predictionError > 0.15 ? '#f59e0b' : GOLD }}
                >
                  {Math.round(drawerOutcome.predictionError * 100)}%
                </div>
              </div>
              <div className="rounded p-2" style={{ backgroundColor: SURFACE }}>
                <div className="text-[10px] font-mono" style={{ color: GHOST }}>
              ...
              <div className="rounded p-2" style={{ backgroundColor: SURFACE }}>
                ...
                <div className="text-lg font-bold" style={{ color: '#f59e0b' }}>
                  {Math.round(drawerOutcome.riskBefore * 100)}%
                </div>
              </div>
              <div className="rounded p-2" style={{ backgroundColor: SURFACE }}>
                <div className="text-[10px] font-mono" style={{ color: GHOST }}>
                ...
                <div className="text-lg font-bold" style={{ color: '#22c55e' }}>
                  {Math.round(drawerOutcome.riskAfter * 100)}%
                </div>
              </div>
            </div>
            <div
            ...
            <div className="text-xs mb-2" style={{ color: SUB }}>
              Operator: {drawerOutcome.operatorFeedback}
            </div>
            <div className="text-[10px] font-mono" style={{ color: GHOST }}>
