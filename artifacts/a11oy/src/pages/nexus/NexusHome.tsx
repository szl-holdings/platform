...
const PILLARS = [
  ...
  {
    id: 'orchestrator',
    href: `${BASE}/nexus/orchestrator`,
    icon: Workflow,
    name: 'Cross-App Orchestrator',
    tagline: 'Agent of agents',
    description:
      'Routes user intents to the right artifact — PARAGON, SEXTANT, DOMAINE, Pulse, Command, SZL Holdings, Carlota Jo, KORA, Counsel, Imperium — via the API server and stitches multi-app workflows together.',
    accent: '#c9a85c',
    bullets: [
...
export default function NexusHome() {
  ...
  async function handleRunSampleBrief() {
    ...
    try {
      ...
      setLocation(`${BASE}/nexus/orchestrator`);
    } catch (err) {
          ...
          {PILLARS.map((pillar) => {
            ...
            return (
              <Link
                key={pillar.id}
                href={pillar.href}
                className="bg-praxis-surface border border-praxis rounded-xl p-6 cursor-pointer group transition-all hover:border-opacity-50 relative overflow-hidden block no-underline"
                style={{
