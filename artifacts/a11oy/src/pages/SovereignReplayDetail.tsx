...
function buildReplayFromId(id: string): ReplayReport | null {
  ...
  const meta = VERTICAL_META[wc.vertical as string] ?? {
    tenant: 'Enterprise',
    domain: 'Operations',
  };
  ...
  const agentSteps = wc.agentSequence.map((s) => ({
    label: `${s.role}: ${s.action}`,
    actor: s.role,
  }));
  ...
  const steps: ReplayStep[] = allSteps.map((s, i) => ({
    ...
    actor: s.actor,
    ts: new Date(
      Date.parse(completedAt) - (allSteps.length - i) * Math.floor(durationMs / allSteps.length),
    ).toISOString(),
  ...
  return {
    ...
    approvedAt: wc.requiresApproval
      ? new Date(Date.parse(completedAt) - 1800000).toISOString()
      : null,
...
export function SovereignReplayDetail() {
  ...
  return (
    <Layout>
            ...
            {steps.length === 0 ? (
              ...
              <div className="flex flex-col gap-1">
                {steps.map((step, i) => {
                  ...
                  const statusDot =
                    step.outcome === 'success' ? '✓' : step.outcome === 'blocked' ? '⊗' : '⚠';
                  return (
                    <div
                      key={i}
                    ...
                    >
                      <div
                        ...
                        style={{
                          ...
                          border: isCurrent
                            ? '1px solid rgba(201,183,135,0.2)'
                      ...
                      >
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          <span
                            ...
                            style={{ color: 'var(--color-a11oy-text-ghost)', flexShrink: 0 }}
                          >
                            {step.actor}
                          </span>
                        </div>
          ...
          {replayState === 'done' && (
            ...
            >
              ...
              <div className="text-xs" style={{ color: 'var(--color-a11oy-text-sub)' }}>
                All {steps.length} steps replayed. Outcome: {data.outcome.toUpperCase()} · Proof
                chain intact.
              </div>
            </div>
          )}
            ...
            <Card className="text-xs">
              <div className="flex flex-col gap-2">
                <div>
                  ...
                  <div style={{ color: OUTCOME_COLORS[data.outcome] ?? '#5e5e5e' }}>
                ...
                <div>
                  ...
                  <div style={{ color: 'var(--color-a11oy-text-sub)' }}>
                    {data.traceSpans} spans · {data.toolCalls} tool calls
                  </div>
                </div>
                {data.proofRef && (
                  <div>
                    ...
                    <div className="font-mono" style={{ color: '#b08d52' }}>
                      ◇ {data.proofRef}
                    </div>
                  </div>
                )}
              ...
              <Card className="text-xs">
                <div className="flex items-center gap-2 mb-1">
                  <span
                    ...
                    style={{
                      color:
                        data.evalDisposition === 'pass'
                          ? '#c9b787'
                          : data.evalDisposition === 'blocked'
                            ? '#f5f5f5'
                            : '#c9b787',
                      backgroundColor:
                        data.evalDisposition === 'pass'
              ...
              {!certDecision ? (
                <>
                  <div className="mb-2" style={{ color: 'var(--color-a11oy-text-ghost)' }}>
                    Review the execution trace and certify this replay as accurate, flag it for
                    further investigation, or reject the outcome record.
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span style={{ color: '#22c55e' }}>
                      Replay certified — proof chain accepted.
                    </span>
                  </div>
                  <button
                    onClick={() => setCertDecision(null)}
      ...
      >
        <span className="w-1.5 h-1.5 rounded-full bg-[var(--color-a11oy-blue)]" /> Governed
        Environment — replay data fetched from A11oy Proof Ledger. Step-by-step audit trail is
