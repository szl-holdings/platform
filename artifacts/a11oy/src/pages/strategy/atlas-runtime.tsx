...
import { useSafeMode } from '../../lib/command/use-safe-mode';
...
const SEED_TWINS: CrossDomainTwin[] = [
  {
    ...
    pendingActions: 3,
    worldline: 'WL-BETA',
    summary: '3 active incidents in AWS VPC, K8s app tier drifted from approved baseline',
  },
  {
    id: 'tw-aeg-002',
    ...
    pendingActions: 1,
    worldline: 'WL-GAMMA',
    summary: 'PLC firmware delta awaiting CISO approval before reconciliation',
  },
  {
    id: 'tw-terra-001',
    ...
    pendingActions: 0,
    worldline: 'WL-ALPHA',
    summary: 'All property twins synced, portfolio valuation current to market',
  },
  {
    id: 'tw-vessels-001',
    ...
    pendingActions: 0,
    worldline: 'WL-ALPHA',
    summary: '14 vessels tracked, all within geofence, AIS data nominal',
  },
  {
    id: 'tw-vessels-002',
  ...
  },
  {
    ...
    pendingActions: 0,
    worldline: 'WL-ALPHA',
    summary: '247 workflows active, all executions within SLA, proof chain intact',
  },
  {
    id: 'tw-prism-001',
    ...
    pendingActions: 0,
    worldline: 'WL-ALPHA',
    summary: 'Legal entity registry current, 2 NDA reviews in flight but on track',
  },
  {
    id: 'tw-lyte-001',
    ...
    worldline: 'WL-BETA',
    summary:
      '4 SLO breaches active, autonomous NOC handling 3 incidents, 1 requires human escalation',
  },
];
...
function WorldlineDrawer({ twins, onClose }: { twins: CrossDomainTwin[]; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex" onClick={onClose}>
      ...
      <div
        className="w-full max-w-sm border-l overflow-y-auto flex flex-col"
        style={{ background: '#0c1420', borderColor: 'rgba(255,255,255,0.1)' }}
        onClick={(e) => e.stopPropagation()}
      >
        <div
          className="p-5 border-b sticky top-0"
          style={{ borderColor: 'rgba(255,255,255,0.06)', background: '#0c1420' }}
        >
          ...
          <div className="text-[10px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>
            Active worldlines across all domains
          </div>
        </div>
        <div className="p-5 space-y-4">
          {worldlines.map((wl) => {
            ...
            const color =
              degraded.length === 0
                ? '#10b981'
                : degraded.some((t) => t.state === 'awaiting_approval')
                  ? '#8b7ac8'
                  : '#f59e0b';
            return (
              <div
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span
                    ...
                    style={{
                      color: degraded.length === 0 ? '#10b981' : color,
                      background: `${color}15`,
                    }}
                  >
                ...
                {wlTwins.map((tw) => {
                  ...
                  return (
                    <div
                      key={tw.id}
                      className="flex items-center gap-2 py-1.5 border-t text-[10px]"
...
function useAtlasBranches() {
  return useStandardQuery<{
    branches: Array<{ id: string; branchName: string; status: string; twinCategory?: string }>;
    count: number;
  }>({
    queryKey: ['command-atlas-branches'],
    queryFn: () =>
      fetch('/api/atlas/spatial/branches?limit=20')
        .then((r) => {
...
function useTwinSyncStatuses(twinIds: string[]) {
  ...
  return useStandardQuery<{ statuses: TwinSyncStatus[]; count: number; generatedAt: string }>({
    ...
    queryFn: () =>
      fetch(`/api/atlas/spatial/twins/sync-status?ids=${encodeURIComponent(idsParam)}`)
        .then((r) => {
          if (!r.ok) throw new Error(`HTTP ${r.status}`);
          return r.json();
        })
...
export function AtlasRuntimePage() {
  ...
  const {
    ...
    refetch: refetchSync,
    dataUpdatedAt: syncUpdatedAt,
    isFetching: fetchingSync,
  } = useTwinSyncStatuses(seedTwinIds);
    ...
    if (merged.domain === 'aegis' && openIncidents.length > 0) {
      const extraPending =
        criticalIncidents.length > 0
          ? Math.max(0, criticalIncidents.length - merged.pendingActions)
          : 0;
      const liveState: TwinState = criticalIncidents.length >= 3 ? 'degraded' : merged.state;
      const liveSummary =
        openIncidents.length > 0 && merged.id === 'tw-aeg-001'
          ? `${openIncidents.length} open incident${openIncidents.length !== 1 ? 's' : ''} — ${criticalIncidents.length} critical — live from PARAGON`
          : merged.summary;
      merged = {
        ...merged,
        state: liveState,
        pendingActions: merged.pendingActions + extraPending,
        summary: liveSummary,
      };
    }
  ...
  const visibleTwins = safeMode
    ? TWINS.filter((t) => t.state === 'stable' && t.proofState === 'verified')
    : TWINS;
    ...
    <div className="max-w-7xl mx-auto space-y-5">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span
              ...
              style={{ color: '#8b7ac8' }}
            >
              Command · ATLAS Spatial Runtime
            </span>
          </div>
          <h1 className="text-xl font-bold text-white tracking-tight">Cross-Domain Atlas View</h1>
          <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.4)' }}>
            Real-time spatial twin health across all domains with worldline registry and governed
            action handoff.
          </p>
        </div>
      ...
      {!safeMode && (
        <div
          ...
          style={{
            borderColor: liveSyncCount > 0 ? 'rgba(16,185,129,0.15)' : 'rgba(255,255,255,0.07)',
            background: liveSyncCount > 0 ? 'rgba(16,185,129,0.03)' : 'rgba(255,255,255,0.015)',
          }}
        >
          {liveSyncCount > 0 ? (
            <>
              ...
              <span className="text-[10px]" style={{ color: 'rgba(255,255,255,0.55)' }}>
                ...
                of {seedTwinIds.length} twins reporting from ATLAS spatial runtime · auto-polling
                every 30s
              </span>
            </>
          ) : (
            <>
              ...
              <span className="text-[10px]" style={{ color: 'rgba(255,255,255,0.4)' }}>
                No live twin sync data — showing seeded baseline. Polling
                /api/atlas/spatial/twins/sync-status every 30s.
              </span>
            </>
          )}
          <span
            className="ml-auto flex items-center gap-1.5 text-[9px] font-mono"
            style={{ color: 'rgba(255,255,255,0.3)' }}
          >
            {fetchingSync && <Loader2 className="w-2.5 h-2.5 animate-spin" />}
            {syncUpdatedAt > 0
              ? `updated ${new Date(syncUpdatedAt).toLocaleTimeString()}`
              : 'awaiting first sync'}
          </span>
        </div>
      )}
        ...
        >
          ...
          <span className="text-[10px]" style={{ color: 'rgba(255,255,255,0.5)' }}>
            ...
            <span
              ...
              style={{ color: criticalIncidents.length > 0 ? '#ef4444' : 'rgba(255,255,255,0.5)' }}
            >
        ...
        >
          ...
          <div>
            ...
            <div className="text-[9px]" style={{ color: 'rgba(255,255,255,0.4)' }}>
              ...
              unreviewed twin{TWINS.length - visibleTwins.length !== 1 ? 's' : ''} hidden.
            </div>
          </div>
        </div>
      )}
        ...
        >
          ...
          <div className="flex-1">
            <div className="text-[11px] font-semibold text-white mb-0.5">
              {awaiting.length} twin{awaiting.length > 1 ? 's' : ''} awaiting approval
            </div>
            <div className="text-[10px]" style={{ color: 'rgba(255,255,255,0.4)' }}>
              These twins have state changes pending CISO or operator review before reconciliation
              can proceed.
            </div>
          </div>
          <button
      ...
      {apiBranchCount > 0 && (
        ...
        >
          ...
          <span className="text-[10px]" style={{ color: 'rgba(255,255,255,0.5)' }}>
            <span className="font-bold font-mono" style={{ color: '#8b7ac8' }}>
              {apiBranchCount}
            </span>{' '}
            live scenario branch{apiBranchCount !== 1 ? 'es' : ''} in ATLAS — open Worldline Overlay
            for details
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
        {visibleTwins.map((tw) => {
          ...
          return (
            <div
              key={tw.id}
              onClick={() => setSelectedTwin(tw === selectedTwin ? null : tw)}
              className="rounded-xl border cursor-pointer transition-all hover:border-white/15"
              style={{
                borderColor: tw.state !== 'stable' ? s.border : 'rgba(255,255,255,0.07)',
                background: tw.state !== 'stable' ? s.bg : 'rgba(255,255,255,0.01)',
              }}
            >
              <div className="p-4">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[10px] font-bold mb-0.5" style={{ color: d.color }}>
                      {d.label}
                    </div>
                ...
                <div
                  className="text-[10px] leading-snug mb-3"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  {(() => {
                    ...
                    const syncColor = isDegradedSync
                      ? '#ef4444'
                      : isStale
                        ? '#f59e0b'
                        : isLive
                    ...
                    const syncLabel = isDegradedSync
                      ? 'Sync · Degraded'
                      : isStale
                        ? 'Sync · Stale'
                        : isLive
                    ...
                    const cells = [
                      ...
                      {
                        label: 'Actions',
                        value: `${tw.pendingActions}`,
                        color: tw.pendingActions > 0 ? '#f59e0b' : '#10b981',
                        border: '1px solid rgba(255,255,255,0.05)',
                        bg: 'rgba(255,255,255,0.03)',
                      },
                    ];
                    return cells.map((s2) => (
                      <div
                        ...
                        style={{ background: s2.bg, border: s2.border }}
                      >
                        <div
                          className="text-[8px] uppercase tracking-widest mb-0.5 flex items-center justify-center gap-1"
                          style={{ color: 'rgba(255,255,255,0.3)' }}
                        >
                          {s2.label === syncLabel && isLive && !isStale && !isDegradedSync && (
                            <span
                ...
                {selectedTwin?.id === tw.id && (
                  ...
                  >
                    ...
                    {tw.pendingActions > 0 && (
                      <button
                        className="w-full text-[10px] px-3 py-1.5 rounded-lg font-medium text-center transition-all"
                        style={{
                          color: '#8b7ac8',
