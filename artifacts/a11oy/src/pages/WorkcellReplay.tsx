...
const FAILURE_CLASSES = ['evidence_insufficient', 'approval_timeout', 'eval_blocked', 'connector_denied', 'policy_violation', null];

const REPLAYS: ReplaySummary[] = SEED_WORKCELLS.map((wc, i) => {
  const meta = VERTICAL_TENANT_MAP[wc.vertical as string] ?? { tenant: 'Enterprise', domain: 'Operations' };
  const outcome = wc.status === 'error' ? 'failed' : wc.status === 'idle' && !wc.requiresApproval ? 'success' : wc.status === 'running' ? 'success' : 'success';
  ...
  return {
    ...
    domain: meta.domain,
    outcome,
    completedAt: new Date(Date.now() - (i * 3600000 + Math.random() * 1800000)).toISOString(),
    durationMs: Math.floor(Math.random() * 90000 + 8000),
    evalDisposition: wc.mirrorEvalResult.verdict === 'pass' ? 'pass' : wc.mirrorEvalResult.verdict === 'warn' ? 'pass_with_warning' : 'blocked',
  ...
  return (
    <Layout>
      ...
      <div className="mt-6 p-3 rounded-lg text-xs flex items-center gap-2" style={{ backgroundColor: 'rgba(201,183,135,0.06)', border: '1px solid rgba(201,183,135,0.15)', color: 'var(--color-a11oy-text-ghost)' }}>
        <span className="w-1.5 h-1.5 rounded-full bg-[var(--color-a11oy-blue)]" /> Governed Environment — replay data reconstructed from the immutable Proof Ledger. Every workcell is step-replayable with full audit trail.
      </div>
    </Layout>
  );
}
