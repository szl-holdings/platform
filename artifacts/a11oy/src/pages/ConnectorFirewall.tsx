...
const CONNECTORS: AgentGatewayEntry[] = [
  ...
  { id: 'crm-platform', name: 'CRM Platform', vendor: 'Salesforce', domain: 'Revenue', category: 'Customer Data', riskScore: 35, riskLevel: 'medium', status: 'approved', approvalRequired: false, dataClasses: ['account_data', 'opportunity_data', 'contact_data'], allowedTools: ['account_lookup', 'pipeline_analyze', 'churn_score'], blockedTools: ['account_delete', 'contact_email_bulk', 'deal_close'], lastCall: '2026-04-26T14:10:00Z', callsToday: 203, firewallEvents: 4, outputSanitized: true, promptInjectionScans: 203, promptInjectionBlocked: 1, trustScore: 87, consentGranted: true, schemaValidated: true, tenant: 'Lyte Revenue', note: 'PII redaction enforced.', mtlsStatus: 'active', spiffeId: 'spiffe://a11oy.szl/connectors/salesforce', certExpiry: '2027-03-15' },
...
const AGENT_FLOWS: AgentFlow[] = [
  ...
  { from: 'Terra Analyst', to: 'Pipeline Oracle', protocol: 'mTLS + A2A', status: 'idle', messagesPerMin: 0, lastMessage: 'Asset impact analysis' },
