...
import { PLUGIN_CATEGORIES, PLUGIN_TOTALS, type Plugin } from '../data/pluginHubData';
...
function KpiCard({ label, value, sub }: { label: string; value: number | string; sub?: string }) {
  return (
    ...
    >
      ...
      {sub && (
        <p style={{ fontSize: '0.625rem', color: T.textDim, margin: '0.125rem 0 0' }}>{sub}</p>
      )}
    </div>
  );
}
...
function PluginCard({ plugin }: { plugin: Plugin }) {
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
        ...
        <span
          style={{
            ...
            background:
              plugin.tier === 'core' ? 'rgba(201,183,135,0.08)' : 'rgba(255,255,255,0.03)',
            flexShrink: 0,
          }}
        >
          {plugin.tier}
      ...
      <p style={{ fontSize: '0.75rem', lineHeight: 1.55, color: T.textDim, margin: 0 }}>
        {plugin.desc}
      </p>
    ...
    >
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ marginBottom: '2rem' }}>
          ...
          >
            {PLUGIN_TOTALS.total} integrations across {PLUGIN_TOTALS.categories} categories — apps,
            skills, MCPs, and a11oy-native modules. Every plugin call is proof-chained. Every action
            is attributed. Every result feeds the Outcome Graph.
          </p>
        </div>
        ...
        {filtered.map((cat) => (
          <div key={cat.id} style={{ marginBottom: '2.5rem' }}>
            ...
            >
              {cat.plugins.map((p, i) => (
                <PluginCard key={`${cat.id}-${i}`} plugin={p} />
              ))}
            </div>
          </div>
        ))}
