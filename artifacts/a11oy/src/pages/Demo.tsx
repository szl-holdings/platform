...
export function Demo() {
  ...
  const linkedWC = scenario ? SEED_WORKCELLS.filter(w => scenario.workcellIds.includes(w.id)) : [];
  const linkedSignals = scenario ? SEED_SIGNALS.filter(s => scenario.initialSignals.includes(s.id)) : [];

  const steps = scenario ? [
    { step: 'Signal Mesh: initial signals ingested', status: 'completed', note: scenario.initialSignals.join(', ') },
    { step: 'Causal Core: evidence graph assembled', status: 'completed', note: `${linkedSignals.length} signals correlated` },
    { step: 'Context Engine: context packs built', status: 'completed', note: `${linkedWC.length} workcells provisioned` },
    ...linkedWC.slice(0, 3).map(wc => ({
      step: `Workcell: ${wc.name}`,
      status: wc.status === 'completed' ? 'completed' : wc.status === 'running' ? 'running' : 'pending',
      note: wc.objective,
    })),
    { step: 'Covenant Layer: policy gates evaluated', status: 'completed', note: `${linkedWC.filter(w => w.requiresApproval).length} approval gates triggered` },
    { step: 'MirrorEval: recommendations scored', status: 'completed', note: `Verdicts: ${linkedWC.map(w => w.mirrorEvalResult.verdict).join(', ')}` },
    { step: 'Proof Ledger: PCE contracts recorded', status: 'completed', note: 'All executions cryptographically proven' },
  ] : [];
          ...
          <div className="flex flex-col gap-3">
            {SEED_DEMO_SCENARIOS.map((s, i) => {
              ...
              return (
                ...
                >
                  <div className="font-semibold text-sm mb-1" style={{ color: 'var(--color-a11oy-text)' }}>{s.name}</div>
                  <div className="text-xs mb-2" style={{ color: 'var(--color-a11oy-text-sub)' }}>{s.description}</div>
                  <div className="flex items-center gap-2 text-xs flex-wrap">
                    <VerticalBadge vertical={VERTICAL_LABELS[primaryVertical] ?? primaryVertical} color={VERTICAL_COLORS[primaryVertical] ?? color} />
                    <span style={{ color: 'var(--color-a11oy-text-ghost)' }}>{wcs.length} workcells</span>
                    <span style={{ color: 'var(--color-a11oy-text-ghost)' }}>{s.initialSignals.length} signals</span>
                  </div>
                </button>
              );
            })}
        ...
        <div className="lg:col-span-2">
          {!scenario ? (
            <div className="h-full flex flex-col items-center justify-center py-24 text-center">
              <div className="text-3xl mb-3" style={{ color: 'var(--color-a11oy-border)' }}>▶</div>
              <div className="text-sm font-medium mb-1" style={{ color: 'var(--color-a11oy-text)' }}>Select a scenario to begin</div>
            ...
            <>
              <div className="mb-4">
                <div className="font-semibold text-lg mb-1" style={{ color: 'var(--color-a11oy-text)' }}>{scenario.name}</div>
                <p className="text-xs" style={{ color: 'var(--color-a11oy-text-sub)' }}>{scenario.description}</p>
              </div>
              ...
              <Card className="mb-4">
                <div className="flex flex-col gap-1 mb-4">
                  {steps.map((step, i) => {
                    ...
                    return (
                      <div key={i} className="transition-all" style={{ opacity: stepIdx === -1 ? 0.5 : visible ? 1 : 0.3 }}>
                        <TraceStep
                          step={step.step}
                          status={stepIdx === -1 ? 'pending' : i < stepIdx ? 'completed' : i === stepIdx ? 'running' : 'pending'}
                ...
                {/* Controls */}
                <div className="flex items-center gap-3 border-t pt-3" style={{ borderColor: 'var(--color-a11oy-border)' }}>
                  <ActionButton
                    variant="primary"
                    onClick={stepIdx >= steps.length - 1 ? resetDemo : advanceStep}
                  >
                    {stepIdx < 0 ? '▶ Start' : stepIdx >= steps.length - 1 ? '↩ Reset' : '→ Next Step'}
                  </ActionButton>
                  <ActionButton variant="ghost" onClick={resetDemo}>Reset</ActionButton>
                ...
                {linkedWC.map(wc => (
                  <Card key={wc.id} className="text-xs">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <span className="font-medium" style={{ color: 'var(--color-a11oy-text)' }}>{wc.name}</span>
              ...
              <div className="flex flex-wrap gap-2">
                {linkedSignals.map(sig => (
                  <span key={sig.id} className="text-xs font-mono px-2 py-1 rounded" style={{ backgroundColor: 'var(--color-a11oy-muted)', color: 'var(--color-a11oy-text-ghost)', border: '1px solid var(--color-a11oy-border)' }}>
                    {sig.id} — {sig.title.slice(0, 40)}
                  </span>
                ))}
                {linkedSignals.length === 0 && scenario.initialSignals.map(id => (
                  <span key={id} className="text-xs font-mono px-2 py-1 rounded" style={{ backgroundColor: 'var(--color-a11oy-muted)', color: 'var(--color-a11oy-text-ghost)', border: '1px solid var(--color-a11oy-border)' }}>
                    {id}
                  </span>
                ))}
              </div>
