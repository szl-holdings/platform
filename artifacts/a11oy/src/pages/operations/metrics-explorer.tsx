...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div
      ...
      style={{ background: '#0c1626', borderColor: 'rgba(255,255,255,0.12)', minWidth: 160 }}
    >
      <p className="font-mono text-[10px] mb-1.5" style={{ color: 'rgba(255,255,255,0.4)' }}>
        {label}
      </p>
      {payload.map((p: any) => (
        <div key={p.dataKey} className="flex items-center gap-2">
          ...
          {p.payload?.anomaly && (
            <span
              ...
              style={{ color: '#c45a4a', background: 'rgba(196,90,74,0.15)' }}
            >
              ANOMALY
            </span>
          )}
...
export default function MetricsExplorer() {
  ...
  const chartData = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      const key = t.toLocaleTimeString('en-US', {
        hour: '2-digit',
        minute: '2-digit',
        hour12: false,
      });
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>
            Select any metric and service, configure time windows, and overlay anomaly bands across
            your platform.
          </p>
        </div>
        <button
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <select
          ...
          style={{
            background: '#0c1626',
            borderColor: 'rgba(255,255,255,0.1)',
            color: metricName ? 'white' : 'rgba(255,255,255,0.35)',
          }}
        >
        ...
        {service && (
          <select
            ...
            style={{
              ...
              color: compareService ? '#60a5fa' : 'rgba(255,255,255,0.25)',
            }}
          >
        ...
        <div className="flex items-stretch">
          {[
            ...
            { label: 'Min', value: formatValue(minValue, unit), color: 'rgba(255,255,255,0.5)' },
            {
              ...
              color: anomalies.length > 0 ? '#c45a4a' : 'rgba(255,255,255,0.3)',
            },
            ...
            >
              <div className="text-lg font-bold font-mono mb-0.5" style={{ color: c.color }}>
                {c.value}
              </div>
              <div
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 gap-3">
            ...
            <p className="text-[11px] text-slate-500">
              Try a different service, metric, or time window.
            </p>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={320}>
            <LineChart data={chartData} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
              ...
              {anomalies.length > 0 && chartData.some((d) => (d as Record<string, unknown>).anomaly) && (
                <ReferenceLine
                  y={avgValue * 1.5}
                  stroke={color}
                  strokeDasharray="4 4"
              ...
              <Line
                ...
                dot={(props: any) => {
                  ...
                  return (
                    <circle key={`dot-${cx}-${cy}`} cx={cx} cy={cy} r={0} fill="transparent" />
                  );
                }}
          ...
          <div className="space-y-1.5">
            {anomalies.slice(0, 10).map((a) => (
              <div
                ...
                style={{ borderColor: 'rgba(196,90,74,0.2)', background: 'rgba(196,90,74,0.05)' }}
              >
                ...
                <span className="text-[10px] font-mono font-bold text-[#c45a4a]">
                  {formatValue(a.value, a.unit)} {a.unit}
                </span>
                <span className="text-[9px]" style={{ color: 'rgba(255,255,255,0.25)' }}>
                  {new Date(a.recordedAt).toLocaleTimeString('en-US', {
                    hour: '2-digit',
                    minute: '2-digit',
                  })}
                </span>
