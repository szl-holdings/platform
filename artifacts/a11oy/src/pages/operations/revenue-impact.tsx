...
function IncidentCard({ inc }: { inc: IncidentImpact }) {
  ...
  const trendColor =
    inc.trend === 'worsening' ? '#ef4444' : inc.trend === 'improving' ? '#6b8f71' : '#f59e0b';
  const trendLabel =
    inc.trend === 'worsening'
      ? '↑ Worsening'
      : inc.trend === 'improving'
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            <span
              ...
              style={{
                background:
                  inc.status === 'active' ? 'rgba(239,68,68,0.1)' : 'rgba(107,143,113,0.1)',
                color: inc.status === 'active' ? '#ef4444' : '#6b8f71',
              }}
            >
        ...
        <div className="text-right shrink-0">
          <div className="text-[18px] font-bold font-mono" style={{ color: sc }}>
            {fmt$(inc.totalImpact)}
          </div>
      ...
      >
        <div className="flex items-center gap-1">
          ...
          <span>
            {inc.affectedClients} clients affected — {inc.clientNames[0]}, {inc.clientNames[1]}
          </span>
        </div>
        {inc.status === 'active' && (
          <span
...
function SummaryKPI({
  ...
  return (
    ...
    >
      ...
      <div className="text-[9px] mt-1" style={{ color: DS.text.muted }}>
        {sub}
      </div>
    </div>
  );
...
export default function RevenueImpactPage() {
      ...
      setIncidents((prev) =>
        prev.map((inc) => {
          ...
          return {
            ...
            lostTransactions:
              inc.lostTransactions + (inc.lostTransactions / inc.durationMins) * additionalMins,
          };
        }),
      );
  ...
  const categoryTotals: RevenueBreakdown[] = [
    {
      ...
      amount: incidents.reduce((s, i) => s + i.lostTransactions, 0),
      percentage: 0,
      color: '#ef4444',
      icon: TrendingDown,
    },
    {
      ...
      amount: incidents.reduce((s, i) => s + i.slaExposure, 0),
      percentage: 0,
      color: '#f97316',
      icon: Shield,
    },
    {
      ...
      label: 'Productivity Cost',
      amount: incidents.reduce((s, i) => s + i.productivityCost, 0),
      percentage: 0,
      color: '#f59e0b',
      icon: Users,
    },
    {
      ...
      amount: incidents.reduce((s, i) => s + i.churnRisk, 0),
      percentage: 0,
      color: '#8b5cf6',
      icon: AlertTriangle,
    },
  ...
  categoryTotals.forEach((c) => {
    c.percentage = Math.round((c.amount / grandTotal) * 100);
  });
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px]" style={{ color: DS.text.muted }}>
            Every operational event — dollar-valued in real time. Lost transactions, SLA exposure,
            productivity cost, churn risk.
          </p>
        </div>
        <div
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <SummaryKPI
          label="Active Impact"
          value={fmt$(totalActiveImpact)}
          sub={`${activeIncs.length} active incidents`}
          color="#ef4444"
          icon={TrendingDown}
        />
        <SummaryKPI
          label="Today's Total"
          value={fmt$(totalTodayImpact)}
        ...
        <SummaryKPI
          label="SLA Exposure"
          value={fmt$(maxSlaExposure)}
          sub="Penalty risk this period"
          color="#f97316"
          icon={Shield}
        />
        <SummaryKPI
          label="Churn Risk"
          value={fmt$(totalChurnRisk)}
          sub="Customer revenue at risk"
          color="#8b5cf6"
          icon={AlertTriangle}
        />
      </div>
      ...
      >
        <div className="text-[10px] font-medium mb-3" style={{ color: DS.text.secondary }}>
          Incident Cost Over Time (Last 24h)
        </div>
        <div className="relative h-24">
          <svg viewBox="0 0 400 80" className="w-full h-full" preserveAspectRatio="none">
            ...
            <path
              d="M0,72 L20,70 L50,65 L80,68 L110,55 L140,60 L170,20 L200,35 L230,28 L260,40 L290,15 L320,25 L350,18 L380,8 L400,10 L400,80 L0,80 Z"
              fill="url(#impactGrad)"
            />
            <path
              d="M0,72 L20,70 L50,65 L80,68 L110,55 L140,60 L170,20 L200,35 L230,28 L260,40 L290,15 L320,25 L350,18 L380,8 L400,10"
              fill="none"
              stroke="#ef4444"
              strokeWidth="1.5"
            />
          ...
          <div
            className="absolute bottom-0 left-0 right-0 flex justify-between text-[8px] font-mono"
            style={{ color: DS.text.muted }}
          >
