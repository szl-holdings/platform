// @ts-nocheck
import { useRecommendations, useUpdateRecommendation } from '../../lib/operations/use-lyte';
...
export default function Recommendations() {
  ...
  const activeRecs =
    recommendations?.filter((r) => r.status === 'suggested' || r.status === 'in_progress') || [];
    ...
    <div className="max-w-6xl mx-auto">
      <div className="mb-10 flex items-end justify-between">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <p className="text-slate-400">
            Cost optimization, compliance, and reliability insights based on infrastructure
            telemetry analysis.
          </p>
        </motion.div>
        <motion.div
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {activeRecs.map((rec, i) => {
          ...
          const gradient =
            categoryGradients[rec.category as keyof typeof categoryGradients] ||
          ...
          return (
            ...
            >
              <div
                className={cn(
                  'absolute inset-0 bg-gradient-to-b pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity',
                  gradient,
                )}
              />
              ...
              <div className="flex justify-between items-start mb-6 relative z-10">
                ...
                <div
                  className={cn(
                    ...
                    impactColors[rec.impact as keyof typeof impactColors],
                  )}
                >
                  {rec.impact}
                </div>
              ...
              <div className="flex-1 relative z-10">
                ...
                <h3 className="text-xl font-bold text-white mb-3 leading-tight group-hover:text-cyan-50 transition-colors">
                  {rec.title}
                </h3>
                <p className="text-slate-400 text-sm leading-relaxed mb-4">{rec.description}</p>

                {'savings' in rec && rec.savings && (
                  <div className="flex items-center gap-2 mb-3 text-xs text-[#6b8f71] bg-[#6b8f71]/5 px-3 py-1.5 rounded-lg border border-[#6b8f71]/10">
                    ...
                    <span className="font-semibold">{String(rec.savings)}</span>
                    <span className="text-slate-500">estimated savings</span>
                  </div>
                )}
                {'compliance' in rec && rec.compliance && (
                  <div className="flex items-center gap-2 mb-3 text-xs text-violet-400 bg-violet-400/5 px-3 py-1.5 rounded-lg border border-violet-400/10">
                    ...
                    <span className="font-medium">{String(rec.compliance)}</span>
                  </div>
                )}
                ...
                <div className="mb-2">
                  ...
                  <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      animate={{
                        width:
                          rec.impact === 'high' ? '100%' : rec.impact === 'medium' ? '66%' : '33%',
                      }}
                      transition={{ delay: 0.5 + i * 0.1, duration: 0.8, ease: 'easeOut' }}
                      className={cn(
                        'h-full rounded-full',
                        impactBarColors[rec.impact as keyof typeof impactBarColors],
                      )}
                    />
                  </div>
                </div>
