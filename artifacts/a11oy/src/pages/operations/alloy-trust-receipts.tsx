import { api } from '../../lib/operations/api';
...
const RECEIPTS: TrustReceipt[] = [
  {
    ...
    policyDecisions: [
      {
        rule: 'Compliance events → audit log',
        outcome: 'approved',
        reason: 'Compliance category — audit trail appended',
      },
      {
        rule: 'CISO review for findings report',
        outcome: 'approved',
        reason: 'CISO sign-off received from Lisa Monroe',
      },
    ],
    evidence: [
  ...
  },
  {
    ...
    policyDecisions: [
      {
        rule: 'Financial actions → Finance review',
        outcome: 'approved',
        reason: 'CFO approval gate inserted — pending signature',
      },
      {
        rule: 'Compliance events → audit log',
        outcome: 'approved',
        reason: 'Compliance audit trail appended',
      },
    ],
    evidence: [
  ...
  },
  {
    ...
    policyDecisions: [
      {
        rule: 'Financial actions > $50K → Finance review',
        outcome: 'escalated',
        reason: 'Impact exceeds $50K threshold — Finance approval required',
      },
      {
        rule: 'Approval SLA: 24h',
        outcome: 'blocked',
        reason: 'No approver responded within 24h SLA window',
      },
    ],
    evidence: [
    ...
    policyDecisions: [
      {
        rule: 'Compliance events → audit log',
        outcome: 'approved',
        reason: 'Financial batch — audit trail appended',
      },
    ],
    evidence: [
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `1px solid ${receipt.status === 'failed' ? 'rgba(196,90,74,0.15)' : BORDER.subtle}`,
      }}
    >
      <button
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            {receipt.immutable && (
              <span
                className="flex items-center gap-0.5 text-[7px] font-mono px-1 py-px rounded"
                style={{ color: '#6b8f71', background: 'rgba(107,143,113,0.06)' }}
              >
          ...
          <div className="flex items-center gap-3 mt-0.5 text-[8px]" style={{ color: TEXT.muted }}>
            ...
            <span>·</span>
            {receipt.overallConfidence != null && (
              <span>{Math.round(receipt.overallConfidence * 100)}% confidence</span>
            )}
          </div>
        </div>
        <ChevronRight
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-4" style={{ borderTop: `1px solid ${BORDER.subtle}` }}>
                ...
                {Object.entries(receipt.inputs).map(([k, v]) => (
                  ...
                  >
                    <span
                      ...
                      style={{ color: TEXT.muted, minWidth: 100 }}
                    >
                      {k}
                    </span>
              ...
              {receipt.policyDecisions.map((d, i) => (
                ...
                >
                  <span
                    ...
                    style={{
                      color: POLICY_OUTCOME_CFG[d.outcome].color,
                      background: `${POLICY_OUTCOME_CFG[d.outcome].color}10`,
                    }}
                  >
                ...
                {receipt.approvals.map((a, i) => (
                  <div
                    ...
                    style={{
                      ...
                      border: `1px solid ${a.approved ? 'rgba(107,143,113,0.15)' : 'rgba(200,149,60,0.12)'}`,
                    }}
                  >
                    <Shield
                      className="w-3 h-3"
                      style={{ color: a.approved ? '#6b8f71' : '#c8953c' }}
                    />
          ...
          >
            ...
            <button
              className="flex items-center gap-1.5 px-2.5 py-1 rounded text-[8px] font-medium opacity-60 cursor-not-allowed"
              style={{
                background: 'rgba(107,143,113,0.08)',
                border: '1px solid rgba(107,143,113,0.15)',
                color: '#6b8f71',
...
function mapApiReceipt(r: Record<string, unknown>): TrustReceipt {
  ...
  return {
    ...
    outputs: {
      Status: rawStatus,
      Confidence:
        typeof r.confidenceScore === 'number' ? `${Math.round(r.confidenceScore * 100)}%` : '—',
    },
    policyDecisions: [],
    ...
    <div className="p-4 md:p-5 space-y-5" style={{ background: BG.page, minHeight: '100vh' }}>
      <div>
        ...
        <h1 className="text-lg font-bold tracking-tight" style={{ color: TEXT.primary }}>
          Run-grade Trust Receipts
        </h1>
        <p className="text-[11px] mt-0.5" style={{ color: TEXT.secondary }}>
          Every completed run produces a structured receipt capturing inputs, outputs, policy
          decisions, evidence, and confidence levels.
        </p>
      </div>

      <div
        className="rounded-md p-3 flex items-start gap-2"
        style={{ background: 'rgba(107,143,113,0.04)', border: '1px solid rgba(107,143,113,0.12)' }}
      >
        ...
        <p className="text-[9px] leading-relaxed" style={{ color: TEXT.secondary }}>
          Trust receipts are structured audit records issued by the COVENANT policy engine. They
          capture the full chain of custody for every run — what was seen, what was decided, who
          approved, and what changed. Receipts are exportable for compliance, capital review, and
          customer diligence.
        </p>
      </div>
        ...
        {[
          {
            ...
            value: receiptsLoading ? '…' : allReceipts.length,
