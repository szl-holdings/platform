  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Validate and manage platform demo data state
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => qc.invalidateQueries({ queryKey: ['admin-seed-validate'] })}
        ...
        <div
          className={cn(
            ...
            actionResult.type === 'success'
              ...
              : 'bg-[#c45a4a]/10 border-[#c45a4a]/20 text-[#c45a4a]',
          )}
        >
          {actionResult.type === 'success' ? (
      ...
      <div className="flex items-start gap-2 p-3 rounded-xl border border-[#d4a054]/20 bg-[#d4a054]/5 text-xs text-[#d4a054]/80">
        ...
        <span>
          Seeder auto-runs on startup when tables are empty. Use manual seed only to force
          re-populate, and reset only in development.
        </span>
      </div>
        ...
        {!confirmReset ? (
          ...
          <div className="flex items-center gap-2 p-2 rounded-lg border border-[#c45a4a]/30 bg-[#c45a4a]/10">
            ...
            <button
              ...
              className="px-2 py-1 rounded text-[10px] text-muted-foreground hover:text-foreground"
            >
              Cancel
            </button>
          </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border/50">
                ...
                <th className="text-right px-4 py-2.5 text-muted-foreground font-medium hidden sm:table-cell">
                  Min
                </th>
            ...
            <tbody>
              {results.map((c) => {
                ...
                return (
                  <tr
                    key={c.table}
                    className="border-b border-border/30 hover:bg-white/[0.015] transition-colors"
                  >
                    <td className="px-4 py-2.5 font-mono text-[10px] text-muted-foreground">
                      {c.table}
                    </td>
                    <td className="px-4 py-2.5 hidden sm:table-cell text-muted-foreground/70">
                      {c.description}
                    </td>
                    <td className="px-4 py-2.5 text-right font-mono font-medium text-foreground">
                      {c.actual}
                    </td>
                    <td className="px-4 py-2.5 text-right hidden sm:table-cell font-mono text-muted-foreground">
                      {c.expected}
                    </td>
