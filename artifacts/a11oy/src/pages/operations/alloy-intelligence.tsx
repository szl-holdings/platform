...
import { type AlloyAIAuditResult, type AlloyAIHealth, type AlloyAIModels, api } from '../../lib/operations/api';
    ...
    >
      <div className="flex items-center gap-2 mb-1">
        ...
        <span
          ...
          style={{ color: roleColors[slot.role] || TEXT.secondary }}
        >
          {slot.role}
        </span>
      </div>
      <div className="text-[10px] font-mono" style={{ color: TEXT.primary }}>
        {slot.model}
      </div>
      <div className="text-[8px] font-mono mt-0.5" style={{ color: TEXT.muted }}>
        {slot.provider}
      </div>
    </div>
  );
...
export default function AlloyIntelligence() {
  ...
  const { data: decisionsData, refetch: refetchDecisions } = useStandardQuery<{
    total: number;
    decisions: any[];
  }>({
    ...
    <div className="p-3 lg:p-4 space-y-3" style={{ maxWidth: 1440 }}>
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1 px-2 py-1 rounded text-[9px] font-mono hover:bg-white/[0.03]"
              ...
              <div className="p-3 space-y-2">
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    {
                      label: 'Indexed',
                      value: health?.retrieval?.totalChunks || 0,
                      icon: Database,
                    },
                  ].map((m) => (
                    <div
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-mono" style={{ color: '#c8953c' }}>
                      {String(health?.config?.executionMode || 'propose_only')}
                    </span>
            ...
            <Panel accent="#8b7ac8">
              ...
              <div className="p-3 space-y-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded text-[10px] font-medium disabled:opacity-40 hover:opacity-80 transition-all"
                  style={{
                ...
                {triageResult?.decision && (
                  <div
                    className="rounded px-2.5 py-2 space-y-2"
                    style={{ background: BG.elevated, border: `1px solid ${BORDER.subtle}` }}
                  >
                    <div className="flex items-center gap-2">
                      <span
                        ...
                        style={{
                          color: priorityColors[triageResult.decision.priority] || TEXT.primary,
                          background: `${priorityColors[triageResult.decision.priority]}14`,
                          border: `1px solid ${priorityColors[triageResult.decision.priority]}2a`,
                        }}
                      >
                      ...
                      >
                        {triageResult.decision.urgency}
                      </span>
                      <ConfidenceBand value={triageResult.decision.confidence} size="md" />
                    </div>
                    <p className="text-[10px] leading-relaxed" style={{ color: TEXT.primary }}>
                      {triageResult.decision.summary}
                    </p>
                    <div className="flex items-center gap-2">
                      <span className="text-[8px]" style={{ color: TEXT.muted }}>
                        Route to:
                      </span>
                      <span className="text-[9px] font-semibold" style={{ color: '#d4a054' }}>
                        {triageResult.decision.routeTo}
                      </span>
                    </div>
                    ...
                    >
                      ...
                      <span className="text-[7px] font-mono" style={{ color: TEXT.muted }}>
                        {triageResult.model} · {triageResult.latencyMs}ms
                      </span>
                    </div>
                  </div>
                )}
                {triageResult?.error && (
                  ...
                  >
                    <span className="text-[9px]" style={{ color: '#c45a4a' }}>
                      {triageResult.error}
                    </span>
                  </div>
                )}
          ...
          <div className="col-span-12 lg:col-span-4 space-y-3">
            <Panel accent="#6b8f71">
              <PanelHead
                ...
                accent="#6b8f71"
                right={
                  <span className="text-[8px] font-mono" style={{ color: 'rgba(107,143,113,0.5)' }}>
                    Policy-Gated
                  </span>
                }
              />
              <div className="p-3 space-y-2">
                <textarea
                  ...
                  placeholder="Describe an objective for Counsel to plan..."
                  className="w-full h-20 rounded px-2.5 py-2 text-[10px] resize-none outline-none"
                  style={{
                    background: BG.elevated,
                    border: `1px solid ${BORDER.muted}`,
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span
                        className="text-[9px] font-bold font-mono px-1.5 py-px rounded"
                        style={{
                          color: actionTypeColors[planResult.plan.actionType] || TEXT.primary,
                          background: `${actionTypeColors[planResult.plan.actionType]}14`,
                          border: `1px solid ${actionTypeColors[planResult.plan.actionType]}2a`,
                        }}
                      >
                    ...
                    <p className="text-[10px] font-medium" style={{ color: TEXT.primary }}>
                    ...
                    >
                      <Cpu className="w-2.5 h-2.5" style={{ color: TEXT.muted }} />
                      <span className="text-[7px] font-mono" style={{ color: TEXT.muted }}>
                        {planResult.model} · {planResult.latencyMs}ms
                      </span>
                    </div>
                  </div>
                )}
            ...
            <div className="divide-y" style={{ borderColor: BORDER.subtle }}>
              {Object.entries(models.routes).map(([routeClass, config]) => (
                <div key={routeClass} className="px-3 py-2 flex items-center gap-3">
                  <span
                    className="text-[10px] font-mono w-28 shrink-0"
                  ...
                  <span className="text-[8px] font-mono" style={{ color: TEXT.muted }}>
                    max:{config.maxTokens} temp:{config.temperature}
                  </span>
              ...
              <div className="grid grid-cols-3 gap-2 mb-3">
                ...
                >
                  <div className="text-lg font-bold font-mono" style={{ color: TEXT.primary }}>
                    {health?.retrieval?.totalChunks || 0}
                  </div>
                  <div
                    className="text-[8px] uppercase tracking-widest"
                    style={{ color: TEXT.muted }}
                ...
                >
                  <div className="text-lg font-bold font-mono" style={{ color: TEXT.primary }}>
                    {health?.retrieval?.withEmbeddings || 0}
                  </div>
                  <div
                    className="text-[8px] uppercase tracking-widest"
                    style={{ color: TEXT.muted }}
                ...
                <div
                  ...
                  style={{ background: BG.elevated, border: `1px solid ${BORDER.subtle}` }}
                >
                  <div className="text-lg font-bold font-mono" style={{ color: '#6b8f71' }}>
                    Hybrid
                  </div>
                  <div
                    className="text-[8px] uppercase tracking-widest"
            ...
            {auditData?.entries?.length ? (
              auditData.entries.map((entry, i) => (
                <div key={i} className="px-3 py-2">
                  ...
                  {entry.confidence != null && (
                    <ConfidenceBand value={entry.confidence as number} size="md" />
                  )}
                </div>
              ))
            ) : (
              <div className="px-3 py-4 text-center">
                <span className="text-[10px]" style={{ color: TEXT.tertiary }}>
                  No audit entries yet. Use the triage or planning engines to generate records.
                </span>
              </div>
            )}
          </div>
      ...
      {tab === 'decisions' && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-12 gap-3">
            <div
              className={`${selectedDecision ? 'col-span-12 lg:col-span-5' : 'col-span-12'} space-y-3`}
            >
              <Panel accent="#8b7ac8">
                <PanelHead
                  icon={GitBranch}
                  ...
                  right={
                    <div className="flex items-center gap-2">
                      <span className="text-[8px] font-mono" style={{ color: TEXT.muted }}>
                        {decisionsData?.total ?? 0} total
                      </span>
                      <button
                        onClick={() => refetchDecisions()}
                        className="px-1.5 py-0.5 rounded text-[8px] font-mono hover:bg-white/[0.04]"
                        style={{ border: `1px solid ${BORDER.subtle}`, color: TEXT.tertiary }}
                      >
                ...
                {!decisionsData?.decisions?.length ? (
                  <div className="p-6 text-center">
                    ...
                    <p className="text-[10px]" style={{ color: TEXT.tertiary }}>
                      No decisions yet. Submit inputs through the overview engine to generate
                      decision objects.
                    </p>
                  </div>
                ) : (
                  <div className="divide-y" style={{ borderColor: BORDER.subtle }}>
                    {decisionsData.decisions.map((d: any) => (
                      <button
                        ...
                        onClick={() =>
                          setSelectedDecision(
                            selectedDecision?.decisionId === d.decisionId ? null : d,
                          )
                        }
                        className="w-full text-left px-3 py-2.5 hover:bg-white/[0.02] transition-colors"
                        style={{
                          background:
                            selectedDecision?.decisionId === d.decisionId
                              ? BG.elevated
                              : 'transparent',
                        }}
                      >
                        <div className="flex items-start gap-2">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] truncate" style={{ color: TEXT.primary }}>
                              {d.recommendedAction}
                            </p>
                            <p
              ...
              <Panel accent="#4a90b8">
                ...
                <div className="p-3 grid grid-cols-5 gap-1.5">
                  {Object.entries({
                    P0: { role: 'Executive', sla: '15m', color: '#c45a4a' },
                    P1: { role: 'Manager', sla: '1h', color: '#c8953c' },
                    P2: { role: 'Auto', sla: '—', color: '#d4a054' },
                    P3: { role: 'Auto', sla: '—', color: '#4a90b8' },
                    P4: { role: 'Auto', sla: '—', color: '#7c85a0' },
                  }).map(([level, info]) => (
                    <div
                      key={level}
                    ...
                    right={
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="flex items-center gap-1 px-2 py-0.5 rounded text-[8px] font-mono hover:bg-white/[0.04]"
                          style={{ border: `1px solid ${BORDER.subtle}`, color: TEXT.tertiary }}
                        >
                  ...
                  <div className="p-3 space-y-3">
                    ...
                    >
                      ...
                      <p
                        ...
                        style={{ color: TEXT.secondary }}
                      >
                        {selectedDecision.rationaleSummary}
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      >
                        <div
                          className="text-[7px] uppercase tracking-widest mb-1"
                          style={{ color: TEXT.muted }}
                        >
                          Confidence
                        </div>
                      ...
                      >
                        ...
                        <span
                          ...
                          style={{
                            color: priorityColors[selectedDecision.riskLevel] || TEXT.primary,
                          }}
                        >
                      ...
                      >
                        ...
                        <span className="text-[9px] font-mono" style={{ color: '#4a90b8' }}>
                          {selectedDecision.modelRoute || '—'}
                        </span>
                      </div>
                    </div>
                    ...
                    {selectedDecision.approvalRequired && (
                      <div
                        className="rounded px-3 py-2.5 flex items-center gap-3"
                        style={{
                          background: 'rgba(200,149,60,0.04)',
                          border: '1px solid rgba(200,149,60,0.15)',
                        }}
                      >
                        <UserCheck className="w-4 h-4 flex-shrink-0" style={{ color: '#c8953c' }} />
                        <div>
                          <div className="text-[9px] font-semibold" style={{ color: '#c8953c' }}>
                            Approval Required
                          </div>
                          <div className="text-[8px] mt-0.5" style={{ color: TEXT.secondary }}>
                            {selectedDecision.riskLevel === 'P0'
                              ? 'Executive sign-off required within 15 minutes'
                              : 'Manager approval required within 1 hour'}
                          </div>
                        </div>
                        <ApprovalBadge
                          level={selectedDecision.riskLevel === 'P0' ? 'executive' : 'manager'}
                          required
                        />
                      </div>
                    )}
      ...
      {auditDrawerOpen && selectedDecision && (
        ...
        >
          <div
            ...
            style={{ background: BG.surface, borderLeft: `1px solid ${BORDER.muted}` }}
          >
            <div
              className="flex items-center justify-between px-4 py-3 sticky top-0 z-10"
              style={{ background: BG.surface, borderBottom: `1px solid ${BORDER.subtle}` }}
            >
              <div className="flex items-center gap-2">
            ...
            <div className="p-4 space-y-4">
              <div>
                ...
                <div
                  className="rounded px-3 py-2.5 overflow-x-auto"
                  style={{ background: BG.elevated, border: `1px solid ${BORDER.subtle}` }}
                >
                  <pre
                    className="text-[8px] font-mono whitespace-pre-wrap"
                    style={{ color: TEXT.secondary }}
                  >
              ...
              <div>
                ...
                {selectedDecision.evidenceRefs?.length > 0 ? (
                  ...
                  <p className="text-[9px]" style={{ color: TEXT.tertiary }}>
                    No evidence references attached to this decision.
                  </p>
                )}
              </div>
              <div>
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[9px] font-semibold" style={{ color: '#6b8f71' }}>
                      Audit Trail Immutable
                    </span>
                  </div>
                  <p className="text-[8px]" style={{ color: TEXT.secondary }}>
                    Decision objects are append-only. Once created, core fields (decisionId,
                    rationaleSummary, evidenceRefs, confidence, createdAt) cannot be modified.
