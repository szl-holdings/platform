...
function RiskBadge({ level }: { level: 'high' | 'medium' | 'low' }) {
  const cfg = {
    high: { color: '#c45a4a', bg: 'rgba(196,90,74,0.08)' },
    medium: { color: '#c8953c', bg: 'rgba(200,149,60,0.08)' },
    low: { color: '#22c55e', bg: 'rgba(34,197,94,0.08)' },
  }[level];
  return (
    <span
      className="text-[7px] font-mono px-1.5 py-0.5 rounded uppercase tracking-wider capitalize"
      style={{ color: cfg.color, background: cfg.bg }}
    >
      {level} risk
    </span>
  ...
  return (
    <div className="p-4 md:p-5 space-y-5" style={{ background: BG.page }}>
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Lock className="w-3.5 h-3.5" style={{ color: Counsel }} />
            <span
          ...
          <h1 className="text-lg font-bold tracking-tight" style={{ color: TEXT.primary }}>
            Write-Back & Export Gates
          </h1>
          <p className="text-[11px] mt-0.5" style={{ color: TEXT.secondary }}>
            Review and authorize external system writes and data exports before they execute
          </p>
        </div>
        {pending > 0 && (
          <div
          ...
          >
            <div className="text-base font-mono font-bold" style={{ color: '#d4a054' }}>
              {pending}
            </div>
            <div
              className="text-[7px] uppercase tracking-wider"
              style={{ color: 'rgba(212,160,84,0.55)' }}
            >
              Pending
            </div>
          </div>
      ...
      <div
        ...
        style={{ background: 'rgba(75,139,219,0.04)', border: `1px solid ${Counsel}20` }}
      >
        <Shield className="w-3.5 h-3.5 shrink-0 mt-0.5" style={{ color: Counsel }} />
        <p className="text-[10px] leading-relaxed" style={{ color: TEXT.secondary }}>
          ...
          leaves the platform without explicit authorization.
        </p>
      </div>
      ...
      <div className="flex gap-0.5" style={{ borderBottom: `1px solid ${BORDER.subtle}` }}>
        {[
          ...
          <button
            key={t.id}
            onClick={() => setActiveTab(t.id as 'all' | 'write_back' | 'export')}
            ...
            style={{
              ...
              borderBottom: activeTab === t.id ? `2px solid ${Counsel}` : '2px solid transparent',
              marginBottom: '-1px',
            }}
          >
            {t.label}
            ...
            >
              ...
              >
                <div
                  ...
                  style={{
                    background:
                      item.type === 'write_back' ? 'rgba(196,90,74,0.08)' : 'rgba(75,139,219,0.08)',
                  }}
                >
                  {item.type === 'write_back' ? (
                    <Database className="w-4 h-4" style={{ color: '#c45a4a' }} />
                  ) : (
                    <Download className="w-4 h-4" style={{ color: Counsel }} />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {item.irreversible && (
                      <span
                        className="flex items-center gap-0.5 text-[7px] px-1.5 py-0.5 rounded"
                        style={{ color: '#c45a4a', background: 'rgba(196,90,74,0.06)' }}
                      >
                  ...
                  <div className="flex items-center gap-2 mt-1 text-[8px]">
                    <span style={{ color: TEXT.muted }}>
                      Target: <span style={{ color: TEXT.tertiary }}>{item.target}</span>
                    </span>
                  ...
                  <div className="pt-3 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[9px]">
                      ...
                      >
                        ...
                        <p className="mt-0.5" style={{ color: TEXT.secondary }}>
                          {item.dataScope}
                        </p>
                      </div>
                      <div
                    ...
                    {item.status === 'pending' && (
                      <div className="flex items-center gap-2">
                        <button
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded text-[9px] font-medium"
                          style={{
