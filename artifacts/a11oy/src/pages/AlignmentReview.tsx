...
import { useAlignmentReviews, DoctrineLoader, type DoctrineAlignmentReview } from '../hooks/useDoctrine';
...
export function AlignmentReview() {
  const { data: reviews, loading, error } = useAlignmentReviews();
  const items = reviews ?? [];
  const inReview = items.filter((a: DoctrineAlignmentReview) => a.decision === 'in-review').length;
  const approved = items.filter((a: DoctrineAlignmentReview) => a.decision === 'approved' || a.decision === 'approved-with-conditions').length;
  const rejected = items.filter((a: DoctrineAlignmentReview) => a.decision === 'rejected').length;
  ...
  return (
    <Layout>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <KpiCard label="REPORTS" value={items.length} sub="this window" accent="#c9b787" />
      ...
      <div className="flex flex-col gap-3">
        {items.map((r: DoctrineAlignmentReview) => (
          <Card key={r.reviewId}>
            <div className="flex items-start justify-between gap-3 mb-3 flex-wrap">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span className="font-mono text-xs px-1.5 py-0.5 rounded" style={{ backgroundColor: 'rgba(201,183,135,0.1)', color: '#c9b787' }}>{r.reviewId}</span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3 text-xs">
              <div className="rounded border p-2" style={{ borderColor: 'var(--color-a11oy-border)' }}>
                ...
                <div className="font-mono" style={{ color: '#c9b787' }}>{fmtPct(Number(r.signals?.evalsCompositeMin ?? 0))}</div>
              </div>
              <div className="rounded border p-2" style={{ borderColor: 'var(--color-a11oy-border)' }}>
              ...
              <div className="rounded border p-2" style={{ borderColor: 'var(--color-a11oy-border)' }}>
                ...
                <div className="font-mono" style={{ color: (r.signals?.rewardHackingOpen ?? 0) > 0 ? '#f5f5f5' : '#c9b787' }}>{r.signals?.rewardHackingOpen}</div>
              </div>
            </div>
            ...
            {r.conditions?.length > 0 && (
              <div className="text-xs mb-2">
                ...
                <ul className="flex flex-col gap-1">
                  {(r.conditions as string[]).map((c: string, i: number) => (
