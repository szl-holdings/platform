...
import { useAlignmentReviews, DoctrineLoader } from '../hooks/useDoctrine';
...
export function AlignmentReview() {
  ...
  const items = reviews ?? [];
  const inReview = items.filter((a: any) => a.decision === 'in-review').length;
  const approved = items.filter((a: any) => a.decision === 'approved' || a.decision === 'approved-with-conditions').length;
  const rejected = items.filter((a: any) => a.decision === 'rejected').length;
        ...
        {items.map((r: any) => (
          <Card key={r.reviewId}>
            ...
            <div className="flex flex-wrap gap-1.5 pt-2 border-t" style={{ borderColor: 'var(--color-a11oy-border)' }}>
              {(r.reviewers as any[])?.map((rev: any, i: number) => (
