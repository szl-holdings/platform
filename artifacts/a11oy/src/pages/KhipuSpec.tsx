...
import { KHIPU_SPEC_KINDS, KHIPU_SPEC_VERSION } from '../data/khipuDoctrine';


const SPEC_BASE = `https://a11oy.io/spec/khipu-doctrine/${KHIPU_SPEC_VERSION}`;

export function KhipuSpec() {
  const [active, setActive] = useState(KHIPU_SPEC_KINDS[0].kind);
  const current = KHIPU_SPEC_KINDS.find(k => k.kind === active) ?? KHIPU_SPEC_KINDS[0];

  return (
    <Layout>
      <PageHeader
        label="DOCTRINE · OPEN SPEC"
        title="Khipu Doctrine Open Spec"
        subtitle={`Version ${KHIPU_SPEC_VERSION} · CC-BY-4.0 · authored and operated by A11oy. Adopt the format; A11oy is one implementation among many.`}
        status="LIVE"
      />

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <KpiCard label="VERSION" value={KHIPU_SPEC_VERSION} sub="SemVer" accent="#c9b787" />
        <KpiCard label="ARTIFACT KINDS" value={KHIPU_SPEC_KINDS.length} sub="discriminated union" accent="#c9b787" />
...
import type { Constitution, SystemCard, RiskReport } from "@khipu-doctrine/spec/types";`}</CodeBlock>
      </Card>

      <div className="grid lg:grid-cols-[260px_1fr] gap-4">
        <div className="flex flex-col gap-1">
          <div className="text-xs font-mono mb-1" style={{ color: 'var(--color-a11oy-text-ghost)' }}>ARTIFACT KINDS</div>
          {KHIPU_SPEC_KINDS.map(k => (
            <button
              key={k.kind}
