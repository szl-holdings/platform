...
const FORMULA_ROWS: FormulaRow[] = [
  ...
  { id: 'v11', version: 'v11', title: 'NPMR equator coupling coefficient (κ₁₁)', formula: 'κ₁₁ = 1 − carrierFidelity · uptakeRatio · lossCoherence;  in [0,1]', endpoint: `${API}/npmr/kappa`, endpointMethod: 'POST', codexNode: 'npmr_kappa_11', thesisAnchor: '4-1-operational-extension-kappa11-the-equator-coupling-coefficient', description: 'Operational metric over the v10 audit surface. Quantifies impedance of cross-stratum coupling at the Amaru equator. Not a new L-term; the Lutar family remains closed at v7+Ω+v10. Default advisory band [0.1, 0.6] is convention, not measurement.', newInV9: true },
  // ── AGI-stack synthesis chapters (May 2026) — see docs/research/agi-stack-synthesis-2026.md ──
  { id: 'unirec', version: 'UniRec', title: 'Unified briefing recommender (tri-tower)', formula: 'UniRec = (userAffinity^0.40 · contextFit^0.30 · itemPriority^0.30) · governance', endpoint: `${API}/a11oy/unirec/recommend`, endpointMethod: 'POST', codexNode: 'unirec_fabric', thesisAnchor: '30-unirec', description: 'Weighted geometric mean over three SZL-shaped towers, multiplied by a covenant-lift governance factor. Re-expression of XuehaiPan/UniRec in A11oy primitives. Per-operator Λ-Resonance weight bounded to [0.80, 1.20].', newInV9: true },
  { id: 'sotopia', version: 'Sotopia', title: 'Approval-calibration reward judge', formula: 'reward = mean(±1 per matched/opposed expectation) · (1 − 0.5·escalationRate);   weight ← clamp(weight ± δ, 0.80, 1.20)', endpoint: `${API}/a11oy/unirec/recommend`, endpointMethod: 'POST', codexNode: 'sotopia_judge', thesisAnchor: '31-sotopia', description: 'Deterministic social-task judge over operator approval traces. Drives the per-(operator, domain) calibration store that UniRec reads each tick. Bounded so a single operator cannot collapse global ranking.', newInV9: true },
  { id: 'memnet', version: 'Memnet', title: 'Associative recall over the Reliquary', formula: 'recall = 0.55·lexical + 0.25·temporal + 0.20·outcome;   temporal = e^(−Δt/τ),  τ ≈ 30d', endpoint: `${API}/a11oy/reliquary/recall`, endpointMethod: 'GET', codexNode: 'memnet_recall', thesisAnchor: '32-memnet', description: 'Lexical+temporal+outcome blend over the reliquary catalog. Memnet "store + recall by similarity" loop without an embedding store — every hit ships with a one-line rationale ("recent · prior success · strong topical overlap").', newInV9: true },
  { id: 'antivenom', version: 'Antivenom', title: 'Adversarial Resistance multiplier (A)', formula: 'AMI_v2 = ( … ) · e^(−0.7N − 0.5D) · G · A,   A ∈ [0.10, 1.00];   A ≤ 0.15 ⇒ gate = BLOCK', endpoint: `${API}/a11oy/formulas/ami_v2/evaluate`, endpointMethod: 'POST', codexNode: 'antivenom_fabric', thesisAnchor: '33-antivenom', description: 'Catalogue-keyed input classifier produces A: product of severity penalties, family-deduplicated. Critical match forces the AMI gate to BLOCK regardless of the other seven axes. Antidote suggestions surface in the chat envelope.', newInV9: true },
];
