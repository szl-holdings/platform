...
export function WorkcellReplayDetail() {
  ...
  const steps = wc ? [
    { step: 'Signal Mesh: signals ingested and routed', status: 'completed', note: wc.signals.slice(0, 2).join(', ') },
  ...
  return (
    <Layout>
      <div className="mb-4 flex items-center gap-3">
        <Link href={`${BASE}/workcells`} className="text-xs font-mono" style={{ color: 'var(--color-a11oy-blue)', textDecoration: 'none' }}>← All Workcells</Link>
        <span style={{ color: 'var(--color-a11oy-border)' }}>/</span>
        <Link href={`${BASE}/workcells/${wc.id}`} className="text-xs font-mono" style={{ color: 'var(--color-a11oy-blue)', textDecoration: 'none' }}>{wc.name}</Link>
        <span style={{ color: 'var(--color-a11oy-border)' }}>/</span>
        <span className="text-xs font-mono" style={{ color: 'var(--color-a11oy-text-ghost)' }}>replay</span>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          ...
          <Card>
            {steps.length === 0 ? (
              <div className="text-center py-8 text-xs" style={{ color: 'var(--color-a11oy-text-ghost)' }}>No execution trace available for this workcell.</div>
            ) : (
          ...
          <div>
            ...
            <Card className="text-xs">
              <div className="flex flex-col gap-2">
                ...
                <div>
                  <div className="font-mono mb-0.5" style={{ color: 'var(--color-a11oy-text-ghost)' }}>CREATED</div>
                  <div style={{ color: 'var(--color-a11oy-text-sub)' }}>{wc.createdAt.slice(0, 10)}</div>
                </div>
                <div>
                  <div className="font-mono mb-0.5" style={{ color: 'var(--color-a11oy-text-ghost)' }}>TRACE ID</div>
                  <div className="font-mono" style={{ color: 'var(--color-a11oy-text-ghost)' }}>{wc.executionTraceId}</div>
                </div>
              </div>
            </Card>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-1.5">
              {wc.agentSequence.map((a, i) => (
                <div key={i} className="text-xs px-2 py-1.5 rounded border" style={{ backgroundColor: 'var(--color-a11oy-card)', borderColor: 'var(--color-a11oy-border)' }}>
                  <span className="font-mono mr-2" style={{ color: 'var(--color-a11oy-text-ghost)' }}>#{i + 1}</span>
                  <span style={{ color: 'var(--color-a11oy-text)' }}>{a.role}</span>
                  <div className="text-xs mt-0.5" style={{ color: 'var(--color-a11oy-text-ghost)' }}>{a.action}</div>
                </div>
              ))}
            </div>
          </div>
