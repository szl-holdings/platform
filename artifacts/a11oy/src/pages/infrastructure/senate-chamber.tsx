...
import { getClassificationColor, SENATE_PROPOSALS } from '../../lib/infrastructure/imperium-data';
import {
  AlertTriangle,
  BookOpen,
  Check,
          ...
          return (
            <div
              ...
              style={{
                ...
                borderColor: fill !== 'rgba(255,255,255,0.08)' ? fill : undefined,
              }}
            />
          );
        })}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
            ...
            <span className="text-[10px] text-slate-600">
              {timeAgo}h ago · {proposal.proposedBy}
            </span>
          </div>
        </div>
        {expanded ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            ...
            <div className="text-center rounded p-2.5 bg-white/3">
              <div
                ...
                style={{ color: proposal.costDelta.startsWith('+') ? '#fb923c' : '#4ade80' }}
              >
                {proposal.costDelta}
              </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          {
            label: 'Approved',
            value: SENATE_PROPOSALS.filter((p) => p.status === 'APPROVED').length,
            color: '#4ade80',
          },
          {
            label: 'Vetoed',
            value: SENATE_PROPOSALS.filter((p) => p.status === 'VETOED').length,
            color: '#ef4444',
          },
      ...
      <div className="imperial-card rounded-lg p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs text-slate-400">
          <div className="rounded p-3 bg-white/3 border border-white/5">
            <div className="font-display text-[10px] tracking-[0.12em] gold-text mb-1 uppercase">
              Scaling Changes
            </div>
          ...
          <div className="rounded p-3 bg-white/3 border border-white/5">
            ...
            Any designated Approver may veto pending proposals. Veto requires written justification
            logged immutably.
          </div>
        </div>
      </div>
      ...
      {pending.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-display text-xs tracking-[0.12em] text-yellow-400 uppercase">
              Awaiting Committee Vote ({pending.length})
            </span>
          </div>
      ...
      {decided.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-display text-xs tracking-[0.12em] text-slate-500 uppercase">
              Decided — Immutable Receipts
            </span>
          </div>
          <div className="space-y-3">
            {decided.map((p) => (
