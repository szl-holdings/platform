...
const VIDS: readonly VerticalId[] = ['terra', 'vessels', 'counsel', 'carlota', 'aegis', 'lyte', 'tenax'];
const TWIN_IDS: Record<VerticalId, string> = { terra: 'twin-terra', vessels: 'twin-vessels', counsel: 'twin-counsel', carlota: 'twin-carlota', aegis: 'twin-aegis', lyte: 'twin-lyte', tenax: 'twin-tenax' };
...
const RISK_TITLES: Record<VerticalId, readonly string[]> = {
  ...
  tenax: ['Policy bypass attempt pattern', 'Audit evidence gap — 3 controls', 'Approval queue stale items', 'Cross-vertical policy contradiction', 'Rollback readiness degradation', 'Secret exposure — log scanning gap', 'Governance SLA breach trajectory', 'Policy version synchronization gap', 'Emergency override pattern concern'],
};
...
export const FABRIC_DECISIONS: readonly FabricDecision[] = VIDS.flatMap((vid) => {
  ...
  return titles.map((title, ti): FabricDecision => {
    ...
    return {
      ...
      tenaxApprovalState: pick(rng, DEC_STATS),
