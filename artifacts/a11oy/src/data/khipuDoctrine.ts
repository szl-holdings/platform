...
// Khipu Doctrine — types, constants, labels, and helpers
...
export interface PillpintuPanel {
  key: string;
  label: string;
  description: string;
}

export interface PillpintuPosture {
...
export const KHIPU_SPEC_VERSION = '0.1.0' as const;
...
export const KHIPU_SPEC_KINDS: SpecArtifactKindInfo[] = [
  {
    kind: 'Constitution',
    title: 'Constitution',
...
export type PillpintuPartnerStage =
  | 'apply' | 'verify' | 'vet' | 'onboard'
  | 'active' | 'suspended' | 'revoked';

export interface PillpintuPartner {
  id: string;
  name: string;
  legalName: string;
  homepage: string;
  appliedAt: string;
  stage: PillpintuPartnerStage;
  scope: {
