...
// KHIPU LAYER — A11oy × Sentra defensive orchestration architecture.
//
// Distillation of the Khipu-class executive summary into a static doctrine
...
export const KHIPU_LAYER_VERSION = '0.1.0-seed';

export const KHIPU_LAYER_TAGLINE =
...
export const ORCHESTRATION_STEPS: readonly OrchestrationStep[] = [
  ...
  },
  {
    ...
    detail:
      'Raw scanner output is normalised into the Khipu finding schema and fed to the Risk Assessment Engine for de-duplication, severity scoring, and exploitability triage.',
    guardrail:
...
export const SCANNERS: readonly Scanner[] = [
  {
    ...
    sentraBinding:
      'Invoked by the Static Scanner module on every plan that touches source. Findings normalise into the Khipu finding schema with rule-id provenance.',
    guardrail:
...
export const GUARDRAILS: readonly Guardrail[] = [
  {
    ...
    enforcedBy: 'A11oy intent router + Khipu doctrine schemas (JSON Schema 2020-12).',
  },
  {
    layer: 'Action',
...
export const ROADMAP: readonly RoadmapMilestone[] = [
  {
    ...
    detail:
      'CodeQL + Semgrep + OSV-Scanner + Trivy + Gitleaks + Checkov each producing normalised findings into the Khipu schema, with a deterministic merge pass and a single Risk Engine entry point.',
...
export const MODEL_REFERENCES: readonly ModelReference[] = [
  ...
  },
  {
    ...
    strengths: 'Deepest reasoning chain; first Khipu-class safety filters in the public Claude line.',
    access: 'Public Anthropic API. Reserved for high-risk patch reasoning routed via A11oy escalation.',
  },
  {
    id: 'openkhipu-rdt',
    family: 'OpenKhipu (community RDT reconstruction)',
...
export const KHIPU_LAYER_CITATIONS: ReadonlyArray<{ tag: string; source: string }> = [
  { tag: '66', source: 'Anthropic public posting on Khipu Preview vulnerability discovery and exploitation.' },
  { tag: '70', source: 'Project Pillpintu public announcement and partner stack.' },
  { tag: '72', source: 'Claude Sonnet 4.6 public release notes (1M-token context, agentic coding).' },
  { tag: '73', source: 'Claude Opus 4.7 public release notes (Khipu-safe filtering).' },
  ...
  { tag: '86', source: 'UK AI Security Institute (AISI) Khipu threat-level assessment.' },
  { tag: '91', source: 'OpenKhipu community reconstruction notes on recurrent-depth transformers.' },
