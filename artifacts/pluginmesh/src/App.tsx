...
function routeGoal(goal: string): RoutingResult {
  ...
  const keywordMap: Record<string, string[]> = {
    slack: ['slack'],
    github: ['github-copilot', 'github-actions'],
    notion: ['notion'],
    jira: ['jira'],
    linear: ['linear'],
    stripe: ['stripe'],
    search: ['perplexity-search', 'brave-search'],
    image: ['dalle', 'figma-mcp'],
    code: ['github-copilot', 'replit-agent'],
    deploy: ['replit-agent', 'github-actions'],
    test: ['vitest-runner'],
    ...
    schedule: ['google-calendar', 'zapier'],
    terraform: ['terraform'],
    figma: ['figma-mcp'],
    tailwind: ['tailwind-palette', 'shadcn-ui'],
    animation: ['framer-motion', 'lottie'],
    docker: ['docker-compose'],
    kubernetes: ['kubernetes'],
    openapi: ['openapi-spec'],
    report: ['alloy-orchestrator', 'notion'],
    workflow: ['alloy-orchestrator', 'zapier'],
    audit: ['alloy-orchestrator'],
    security: ['datadog', 'pagerduty'],
    fitness: ['strava'],
    paper: ['arxiv', 'semantic-scholar'],
  ...
  return {
    ...
    primary: primary
      ? {
          name: primary.name,
          slug: primary.slug,
          reason: `Best match for your goal`,
          credentials: primary.credentials,
        }
      : null,
    supporting: supporting.map((p) => ({
      name: p.name,
      slug: p.slug,
    ...
    nextSteps: [
      allCreds.length > 0
        ? `Set in Replit Secrets: ${[...new Set(allCreds)].join(', ')}`
        : 'No credentials required — activate immediately',
      primary?.mcpCompatible
        ? `Add "${primarySlug}" to .mcp.json to enable MCP tool access`
        : `Install ${primary?.name || primarySlug} via plugin settings`,
    ...
    >
      ...
      <div className="flex flex-wrap gap-1">
        ...
        {plugin.tags.length > 3 && (
          <span className="text-[10px] text-[var(--text-muted)]">+{plugin.tags.length - 3}</span>
        )}
      </div>
    </motion.div>
  );
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <span className="text-xs text-[var(--text-muted)]">
            {plugin.category} · {plugin.slug}
          </span>
        </div>
        <button
          onClick={onClose}
          className="p-1.5 rounded-lg text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-4)] transition-colors"
        >
          <X size={14} />
        </button>
      </div>
      ...
      {plugin.credentials.length > 0 ? (
        <div className="mb-4 p-3 rounded-lg bg-[var(--surface-3)] border border-[rgba(245,158,11,0.2)]">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="text-xs font-semibold text-[var(--brand-warning)]">
          ...
          <div className="flex flex-wrap gap-2">
            {plugin.credentials.map((cred) => (
              <code
                key={cred}
                className="px-2 py-0.5 rounded text-[11px] bg-[var(--surface-4)] text-[var(--text-secondary)] font-mono"
              >
                {cred}
              </code>
            ))}
          ...
          <p className="text-[10px] text-[var(--text-muted)] mt-2">
            Set these in Replit Secrets before activating.
          </p>
        </div>
      ) : (
      ...
      {mcpStr && (
        <div className="mb-4">
          ...
          <pre className="text-[10px] font-mono text-[var(--text-secondary)] bg-[var(--surface-3)] rounded-lg p-3 overflow-x-auto border border-[var(--border)]">
            {mcpStr}
          </pre>
        </div>
      )}
        ...
        <AnimatePresence mode="popLayout">
          {filtered.map((plugin) => (
            <PluginCard
              ...
              plugin={plugin}
              onClick={() =>
                setSelectedPlugin(selectedPlugin?.slug === plugin.slug ? null : plugin)
              }
      ...
      <div className="mb-6">
        <div className="p-4 rounded-xl bg-[rgba(99,102,241,0.05)] border border-[rgba(99,102,241,0.15)] mb-4">
          ...
          <p className="text-xs text-[var(--text-secondary)]">
            Describe your goal and get a decisive recommendation — primary plugin, supporting
            plugins, required credentials, and setup steps.
          </p>
        </div>
          ...
          >
            {result.primary && (
              <div className="p-4 rounded-xl border border-[var(--brand-primary)] bg-[var(--surface-2)] glow-indigo">
                <div className="flex items-center gap-2 mb-3">
                  <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-[var(--brand-primary)] text-white">
                    PRIMARY
                  </span>
                ...
                {result.primary.credentials.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {result.primary.credentials.map((c) => (
                      <code
                        key={c}
                        className="px-2 py-0.5 rounded text-[11px] bg-[var(--surface-4)] text-[var(--brand-warning)] font-mono"
                      >
                        {c}
                      </code>
                    ))}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {result.supporting.map((s) => (
                    <div
                      key={s.slug}
                      className="flex items-start gap-3 p-3 rounded-xl bg-[var(--surface-2)] border border-[var(--border)]"
                    >
                      <ArrowRight
                        size={12}
              ...
              <div className="space-y-2">
                {result.nextSteps.map((step, i) => (
                  <div key={i} className="flex items-start gap-2.5">
                    <span className="w-5 h-5 rounded-full bg-[var(--surface-4)] text-[10px] font-bold text-[var(--brand-primary)] flex items-center justify-center shrink-0 mt-0.5">
                      {i + 1}
                    </span>
...
function AutomationsTab() {
  ...
  const cadenceColors: Record<string, string> = {
    ...
    'event-triggered':
      'text-[var(--brand-warning)] bg-[rgba(245,158,11,0.1)] border-[rgba(245,158,11,0.3)]',
  };
        ...
        {AUTOMATIONS.map((auto) => {
          ...
          return (
            <motion.div
              key={auto.id}
              layout
              className="rounded-xl border bg-[var(--surface-2)] overflow-hidden border-[var(--border)]"
            >
              <button
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span
                    className={cn(
                      ...
                      cadenceColors[auto.cadence] || cadenceColors['on-demand'],
                    )}
                  >
                    {auto.cadence}
                  </span>
                    ...
                    <div className="px-4 pb-4 border-t border-[var(--border)] pt-3">
                      <div className="mb-3">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <code className="flex-1 text-[11px] font-mono text-[var(--brand-primary)] bg-[var(--surface-4)] px-2 py-1.5 rounded border border-[var(--border)] break-all">
                            {auto.alloyCommandPrompt}
                          </code>
  ...
  const apps = [
    {
      ...
      path: '/',
      category: 'Core',
      desc: 'Main executive dashboard and tenant management portal',
    },
    {
      name: 'Unified Command',
      path: '/command/',
      category: 'Core',
      desc: 'Single-pane-of-glass ecosystem command portal',
    },
    {
      name: 'Lyte — Decision Intelligence',
      path: '/lyte/',
      category: 'Intelligence',
      desc: 'AI-powered decision intelligence and analytics hub',
    },
    {
      name: 'Pulse — Executive Briefing',
      path: '/pulse/',
      category: 'Intelligence',
      desc: 'Automated executive briefing and signal aggregation',
    },
    {
      name: 'Vessels — Maritime Intelligence',
      path: '/vessels/',
      category: 'Domain',
      desc: 'Real-time maritime fleet tracking and weather risk',
    },
    {
      name: 'Sentra — Cyber Resilience',
      path: '/sentra/',
      category: 'Domain',
      desc: 'Security operations and threat detection',
    },
    {
      name: 'Terra — Real Estate Intelligence',
      path: '/terra/',
      category: 'Domain',
      desc: 'Real estate market intelligence and investment analytics',
    },
    {
      name: 'Counsel — Legal Matter Command',
      path: '/counsel/',
      category: 'Domain',
      desc: 'Legal matter management and document intelligence',
    },
    {
      name: 'A11oy — Brand Orchestration',
      path: '/a11oy/',
      category: 'Platform',
      desc: 'Brand asset management and design system orchestration',
    },
    {
      name: 'API Server',
      path: '/api/',
      category: 'Infrastructure',
      desc: 'Shared Express API with MCP, Alloy, and domain routes',
    },
    {
      name: 'Mobile Command',
      path: '/szl-holdings-mobile/',
      category: 'Mobile',
      desc: 'React Native mobile command center',
    },
  ];

  const categoryColors: Record<string, string> = {
    Core: 'var(--brand-primary)',
    Intelligence: 'var(--brand-accent)',
    Domain: 'var(--brand-secondary)',
    Platform: 'var(--brand-success)',
    Infrastructure: 'var(--brand-warning)',
    Mobile: '#ec4899',
  };
    ...
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
        {apps.map((app) => (
          <div
            key={app.path}
            className="p-4 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] card-hover"
          >
            <div className="flex items-start justify-between mb-2">
              ...
              <span
                className="ml-2 shrink-0 px-1.5 py-0.5 rounded text-[9px] font-bold border"
                style={{
                  color: categoryColors[app.category],
                  backgroundColor: `${categoryColors[app.category]}18`,
                  borderColor: `${categoryColors[app.category]}40`,
                }}
              >
                {app.category}
              </span>
      ...
      <div className="mt-6 p-4 rounded-xl border border-[var(--border)] bg-[var(--surface-2)]">
        ...
        <div className="space-y-2">
          {[
            {
              cmd: 'alloy run weekly-status-report',
              desc: 'Weekly executive brief across all domains',
            },
            {
              cmd: 'alloy run incident-triage --incident_id <id>',
              desc: 'Execute SZL incident triage protocol',
            },
            {
              cmd: 'alloy skill invoke secrets-audit',
              desc: 'Audit all registered secrets for expiry',
            },
            {
              cmd: 'alloy decision list --status approval_required',
              desc: 'List pending decisions awaiting approval',
            },
            {
              cmd: 'alloy research --query <topic> --domains maritime,security',
              desc: 'Multi-domain intelligence query',
            },
          ].map(({ cmd, desc }) => (
            <div
              key={cmd}
              className="flex items-center gap-3 p-2.5 rounded-lg bg-[var(--surface-3)]"
            >
              <code className="text-[11px] font-mono text-[var(--brand-primary)] flex-1">
                {cmd}
              </code>
  ...
  const mcpServers = [
    {
      ...
      tools: 12,
      noAuth: true,
      desc: 'Plugin catalog broker — zero-dependency',
    },
    {
      ...
      tools: 23,
      noAuth: false,
      creds: ['ALLOY_INTERNAL_TOKEN'],
      desc: 'SZL Holdings governed orchestration',
    },
    {
      ...
      tools: 15,
      noAuth: false,
      creds: ['GITHUB_PERSONAL_ACCESS_TOKEN'],
      desc: 'GitHub repos, issues, PRs, and Actions',
    },
    {
      name: 'Filesystem MCP',
      slug: 'filesystem',
      type: 'stdio',
      command: 'npx -y @modelcontextprotocol/server-filesystem /home/runner/workspace',
      tools: 8,
      noAuth: true,
      desc: 'Read/write workspace files',
    },
    {
      name: 'Postgres MCP',
      slug: 'postgres',
    ...
    },
    {
      ...
      noAuth: false,
      creds: ['BRAVE_API_KEY'],
      desc: 'Privacy-preserving web search',
    },
  ];
  ...
  const mcpJsonTemplate = JSON.stringify(
    {
      mcpServers: {
        ...
        alloy: {
          url: 'https://${REPLIT_DEV_DOMAIN}/api/mcp',
          headers: { Authorization: 'Bearer ${ALLOY_INTERNAL_TOKEN}' },
        },
        github: {
          command: 'npx',
  ...
  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
        {mcpServers.map((server) => (
          ...
          >
            <div className="flex items-start justify-between mb-2">
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-[var(--surface-4)] text-[var(--text-muted)] border border-[var(--border)]">
              ...
              {server.noAuth ? (
                <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-[rgba(16,185,129,0.15)] text-[var(--brand-success)] border border-[rgba(16,185,129,0.3)]">
                  No auth
                </span>
              ) : (
            ...
            <code className="text-[10px] font-mono text-[var(--text-muted)] block truncate">
      ...
      <div className="p-4 rounded-xl border border-[var(--border)] bg-[var(--surface-2)]">
        ...
        <pre className="text-[10px] font-mono text-[var(--text-secondary)] overflow-x-auto">
          {mcpJsonTemplate}
        </pre>
      </div>
      ...
      <div className="mt-4 p-4 rounded-xl border border-[rgba(99,102,241,0.2)] bg-[rgba(99,102,241,0.05)]">
        ...
        <p className="text-xs text-[var(--text-secondary)] leading-relaxed">
          MCP servers run with your workspace permissions. Only activate servers you trust. Never
          hardcode credentials — use Replit Secrets for all tokens. All Alloy tool invocations are
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[rgba(16,185,129,0.1)] border border-[rgba(16,185,129,0.2)]">
                ...
                <span className="text-[10px] font-semibold text-[var(--brand-success)]">
                  MCP Ready
                </span>
              </div>
              <span className="hidden sm:block text-xs text-[var(--text-muted)]">
                {PLUGINS.length} plugins · 7 categories
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-[var(--text-muted)]">
              Never bypasses plugin auth · Generates setup templates only
            </span>
          </div>
          <span className="text-[10px] text-[var(--text-muted)] hidden sm:block">
            MCP Protocol 2024-11-05 · SZL Holdings
          </span>
        </div>
      </footer>
    </div>
