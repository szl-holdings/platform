const CACHE_VERSION = "stephen-v1";
...
self.addEventListener("push", (event) => {
  ...
  const title = payload.title || "Stephen Lutar";
  const options = {
    body: payload.body || "",
    icon: payload.icon || "/favicon.svg",
    badge: payload.badge || "/favicon.svg",
    tag: payload.tag || "stephen-alert",
    data: payload.data || {},
...
self.addEventListener("notificationclick", (event) => {
  ...
  const actionUrl = event.notification.data?.actionUrl || "/stephen/";
  event.waitUntil(
