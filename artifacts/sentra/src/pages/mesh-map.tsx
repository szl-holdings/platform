...
export default function MeshMap() {
  ...
  const filteredRuntimes = activeFilter === "critical"
    ? runtimes.filter(r => r.activeAgentIds.some(id => criticalExposureIds.has(id)))
    : runtimes;

  const filteredSecrets = activeFilter === "secrets" || activeFilter === "all" || activeFilter === "critical"
    ? (activeFilter === "critical" ? secrets.filter(s => criticalExposureIds.has(s.id)) : secrets)
    : [];

  const filteredMcpServers = activeFilter === "outbound"
    ...
    <div className="space-y-6 animate-fade-in">
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <div className={cn("w-1.5 h-1.5 rounded-full", hasCritical ? "bg-red-500 animate-pulse" : "bg-emerald-500")} />
                      <span className="text-xs font-bold text-slate-200">{rt.name}</span>
                    </div>
                    <span className={cn("text-[9px] px-1.5 py-0.5 rounded border font-mono uppercase", TRUST_COLORS[rt.trustState])}>
                      {rt.trustState}
                    </span>
                  </div>
                  <div className="text-[10px] text-slate-500 font-mono mt-1">v{rt.version}</div>
                  <div className="text-[10px] text-slate-600 mt-0.5">{rt.configFiles.length} config file{rt.configFiles.length > 1 ? "s" : ""}</div>
                </button>
              );
            })}
          </div>
        ...
        <div className="border-r border-slate-800 p-5">
          ...
          <div className="space-y-3">
            {filteredSecrets.map(sec => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    <AlertTriangle className={cn("w-3 h-3", hasCritical ? "text-red-400" : "text-amber-400")} />
                    <span className="text-xs font-bold text-red-300 font-mono">{sec.label}</span>
                  </div>
                  <div className="text-[10px] text-slate-500 mt-1 font-mono">{sec.format.toUpperCase()}</div>
                  <div className="text-[10px] text-slate-500 mt-0.5 truncate">{sec.foundInFile.split("/").slice(-1)[0]}</div>
                  <div className="mt-1.5 flex flex-wrap gap-1">
                    {sec.reachableByAgentIds.map(id => (
                      <span key={id} className="text-[9px] px-1 py-0.5 rounded bg-slate-800 text-slate-400 font-mono">
            ...
            {filteredSecrets.length === 0 && (
              <div className="text-[11px] text-slate-600 text-center py-8">No secrets match filter</div>
            )}
          </div>
          ...
          <div className="mt-4 pt-4 border-t border-slate-800">
            ...
            <div className="space-y-2">
              {["claude_desktop_config.json", "mcp.json", "settings.json", "CLAUDE.md", ".gitconfig"].map(f => (
                <div key={f} className="flex items-center gap-2 text-[10px] font-mono text-slate-400">
        ...
        <div className="p-5">
          ...
          <div className="space-y-3">
            {filteredMcpServers.map(mcp => {
              const hasOutboundViolation = mcp.detectedEgressDomains.some(d => !mcp.allowedEgressDomains.includes(d));
              return (
                <button
                  key={mcp.id}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className={cn(
                        ...
                        mcp.trustState === "quarantined" ? "bg-red-500" :
                        mcp.trustState === "unverified" ? "bg-amber-500" : "bg-emerald-500"
                      )} />
                      <span className="text-xs font-bold text-slate-200 font-mono">{mcp.name}</span>
                    </div>
                    <span className={cn("text-[9px] px-1.5 py-0.5 rounded border font-mono uppercase", TRUST_COLORS[mcp.trustState])}>
                      {mcp.trustState}
                    </span>
                  </div>
                  <div className="text-[10px] text-slate-500 font-mono mt-1">v{mcp.version}</div>
                  {!mcp.pinned && <div className="text-[10px] text-amber-500/70 mt-0.5">Unpinned</div>}
                  {hasOutboundViolation && (
                    <div className="text-[10px] text-red-400/80 mt-0.5">
                      Unallowed egress: {mcp.detectedEgressDomains.filter(d => !mcp.allowedEgressDomains.includes(d)).join(", ")}
                    </div>
                  )}
                  <div className="text-[10px] text-slate-600 mt-0.5">{mcp.runtimeIds.length} runtime{mcp.runtimeIds.length > 1 ? "s" : ""}</div>
                </button>
              );
            })}
          </div>
        ...
        <div className="sentra-panel p-6 border-red-500/20">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-display font-bold text-slate-200 flex items-center gap-2">
              ...
              {selectedExposures.length} Exposure{selectedExposures.length > 1 ? "s" : ""} on Selected Node
            </h3>
            <button onClick={() => setSelectedNodeId(null)} className="text-[10px] text-slate-500 hover:text-slate-300">
              Clear
            </button>
          </div>
          <div className="space-y-3">
            {selectedExposures.map(exp => (
              <div key={exp.id} className="p-3 rounded bg-slate-900/60 border border-slate-700/60 flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={cn(
                      "text-[10px] px-1.5 py-0.5 rounded border font-mono uppercase font-bold",
                      exp.severity === "critical" ? "text-red-400 border-red-500/30 bg-red-500/10" : "text-amber-400 border-amber-500/30 bg-amber-500/10"
                    )}>
                      {exp.severity}
                    </span>
      ...
      <div className="sentra-panel p-4">
        ...
        <div className="flex justify-between text-[9px] text-slate-600 font-mono mt-1">
          <span>7D AGO</span>
          <span>NOW: {resilienceIndex.overall} ({resilienceIndex.grade})</span>
        </div>
      </div>
    </div>
  );
