...
const FALLBACK_TIMELINE = [
  {
    at: '14:02 UTC',
    type: 'alert',
    text: 'SIEM alert: Unusual SMB traffic from SVC-ACCNT-04 to DC-PROD-03',
    actor: 'SIEM',
  ...
  },
  {
    at: '14:08 UTC',
    type: 'action',
    text: 'Alert acknowledged and promoted to Case CASE-0041',
    actor: 'J. Chen',
  ...
  },
  {
    at: '14:11 UTC',
    type: 'evidence',
    text: 'Evidence collected: network capture PCAPs tagged #IOC-0881',
    actor: 'J. Chen',
    severity: null as string | null,
  },
  {
    at: '14:19 UTC',
    type: 'signal',
    text: 'Correlated signal: T1021.002 — RemCom execution pattern from same source',
  ...
  },
  {
    at: '14:27 UTC',
    type: 'hypothesis',
    text: 'Hypothesis drafted: APT29 TTP overlap — pass-the-hash lateral movement',
    actor: 'J. Chen',
    severity: null as string | null,
  },
  {
    at: '14:35 UTC',
    type: 'action',
    text: 'Recommended action proposed: isolate DC-PROD-03 (pending approval APR-001)',
    actor: 'J. Chen → SOC Mgr',
    severity: null as string | null,
  },
  {
    at: '14:52 UTC',
    type: 'signal',
    text: 'New signal: outbound C2 beacon detected — port 443 to 103.45.18.22',
    actor: 'Detection Engine',
    severity: 'critical' as string | null,
  },
];
...
const FALLBACK_SIGNALS = [
  {
    ...
    source: 'EDR',
    severity: 'critical',
    text: 'RemCom lateral movement — SVC-ACCNT-04 → DC-PROD-03',
    at: '14:19',
  },
  {
    id: 'SIG-082',
    ...
    source: 'FW',
    severity: 'critical',
    text: 'C2 beacon outbound — port 443 → 103.45.18.22',
    at: '14:52',
  },
];
...
const FALLBACK_EVIDENCE = [
  {
    id: 'EV-01',
    name: 'network-capture-0881.pcap',
  ...
  {
    id: 'EV-02',
    name: 'edr-lsass-dump-2024.log',
    type: 'log_artifact',
    collectedBy: 'Detection Engine',
    at: '15:05',
    trustLevel: 'verified',
    sensitivity: 'RESTRICTED',
    retentionClass: 'IR-90D',
...
const FALLBACK_HYPOTHESES = [
  {
    id: 'HYP-A',
    title: 'APT29 pass-the-hash lateral movement',
    confidence: 88,
    status: 'active',
    notes:
      'MITRE T1021.002 and T1003.001 pattern overlap with known APT29 TTPs. C2 IP matches threat intel feed.',
  },
  {
    id: 'HYP-B',
    title: 'Insider threat — privileged service account misuse',
    confidence: 22,
    status: 'considered',
    notes:
      'SVC-ACCNT-04 is shared across 3 teams. Cannot rule out misuse, but C2 beacon makes this less likely.',
  },
];
...
const FALLBACK_ACTIONS = [
  {
    id: 'RA-01',
    text: 'Isolate DC-PROD-03 from network segment',
  ...
  {
    id: 'RA-02',
    text: 'Rotate SVC-ACCNT-04 credentials and revoke active tokens',
    gate: 'approval_required',
    status: 'pending',
    severity: 'high',
  },
...
export default function InvestigationsBoard() {
  ...
  const [activeSection, setActiveSection] = useState<
    'timeline' | 'entities' | 'signals' | 'evidence' | 'hypotheses' | 'actions' | 'tradecraft'
  >('timeline');
  const [noteText, setNoteText] = useState('');
  const [selectedCaseIdx, setSelectedCaseIdx] = useState(0);

  const { data: investigationsData, isSuccess: investigationsLoaded } =
    useStandardQuery<InvestigationsPayload>({
  ...
  const displayCase = activeCaseFromLive
    ? {
        id:
          activeCaseFromLive.caseNumber ?? `CASE-${String(activeCaseFromLive.id).padStart(4, '0')}`,
        ...
        tenantLabel: investigationsData?.ztPermissionClass
          ? `CLASS:${investigationsData.ztPermissionClass.toUpperCase()}`
          : FALLBACK_CASE.tenantLabel,
        envLabel: investigationsData?.ztEnvironment ?? FALLBACK_CASE.envLabel,
        sensitivityLabel:
          investigationsData?.ztDataLabels?.sensitivityLabel ?? FALLBACK_CASE.sensitivityLabel,
      }
    : FALLBACK_CASE;
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        ...
        {activeSection === 'entities' && (
          <div className="space-y-3">
            ...
            {entities.map((ent) => (
              <div
                key={ent.id}
                className="flex items-center gap-4 bg-white/[0.025] border border-white/5 rounded-xl px-4 py-3"
              >
                <div
                ...
                <span className="text-[9px] font-mono px-1.5 py-0.5 rounded border border-white/10 text-white/40 capitalize">
                  {ent.type}
                </span>
              </div>
            ))}
        ...
        {activeSection === 'signals' && (
          <div className="space-y-3">
            ...
            {signals.map((sig) => (
              <div
                key={sig.id}
                className="bg-white/[0.025] border border-white/5 rounded-xl px-4 py-3"
              >
        ...
        {activeSection === 'hypotheses' && (
          <div className="space-y-4">
            <div className="text-[9px] font-mono text-white/25 px-1 mb-1">
              SCENARIO DATA — no live hypothesis engine
            </div>
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-[9px] font-mono px-1 py-0.5 rounded border border-white/10 text-white/40 mt-1 inline-block capitalize">
                        {hyp.status}
                      </span>
                    </div>
                  </div>
        ...
        {activeSection === 'actions' && (
          <div className="space-y-3">
            <div className="text-[9px] font-mono text-white/25 px-1 mb-1">
              RECOMMENDED — requires approval gate
            </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={cn(
                          'text-[8px] font-mono px-1.5 py-0.5 rounded border uppercase tracking-wider',
                          GATE_STYLES[ra.gate] ?? '',
                        )}
                      >
                        {ra.gate}
