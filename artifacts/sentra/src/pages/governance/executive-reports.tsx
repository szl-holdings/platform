...
const REPORTS: Report[] = [
  {
    ...
    description:
      'Current incident status, severity, containment actions taken, and immediate risk to operations. Includes evidence summary and analyst confidence.',
    cadence: 'On-demand / triggered by P1/P2',
    lastGenerated: '14m ago',
    status: 'current',
    audience: 'CISO / Executive Sponsor',
    icon: Shield,
  },
  {
    id: 'daily_digest',
    title: 'Daily Posture Digest',
    description:
      '24-hour security posture summary: new incidents, resolved findings, open approvals, alert volume, MTTD/MTTR against targets.',
    cadence: 'Daily at 07:00',
    lastGenerated: '6h ago',
    status: 'current',
    audience: 'SOC Lead / CISO',
    icon: Calendar,
  },
  {
    id: 'weekly_risk',
    ...
    description:
      '7-day risk trend analysis: escalation rates, policy blocks, retrieval quality, model trust metrics, control coverage deltas.',
    cadence: 'Weekly — Monday',
    lastGenerated: '4d ago',
    status: 'current',
  ...
  },
  {
    ...
    description:
      'Non-technical risk posture summary for board consumption. Key risk indicators, control gaps, and strategic recommendations with confidence levels.',
    cadence: 'Quarterly / on-demand',
    lastGenerated: '3w ago',
    status: 'stale',
  ...
  },
  {
    ...
    description:
      'Full chronological timeline of a specific incident: detection, triage, escalation, containment, resolution. Includes all analyst actions and approval events.',
    cadence: 'Post-incident',
    lastGenerated: '2d ago',
    status: 'current',
    audience: 'IR Team / Legal / Compliance',
    icon: Clock,
  },
  {
    id: 'after_action',
    ...
    description:
      'Structured post-incident analysis: root cause, contributing factors, gaps exposed, corrective actions assigned, and playbook update recommendations.',
    cadence: 'Post-incident (within 72h)',
    lastGenerated: '2d ago',
    status: 'current',
    audience: 'IR Lead / CISO / GRC',
    icon: FileText,
  },
  {
    id: 'drill_summary',
    title: 'Resilience Drill Summary',
    description:
      'Outcomes of tabletop or red/purple team exercise: scenario tested, gaps identified, detection coverage, response timing, and remediation assignments.',
    cadence: 'Post-exercise',
    lastGenerated: '3w ago',
    status: 'stale',
    audience: 'IR Team / CISO / Board',
    icon: Shield,
  },
];
...
const SAMPLE_INCIDENT_BRIEF = {
  ...
  confidence: 'HIGH',
  assumptions: [
    'Attacker pivoted from compromised SVC-ACCNT-04',
    'DC-PROD-03 is the primary beachhead',
  ],
  evidenceSummary: [
  ...
  actionsCompleted: [
    'Alert triage — severity confirmed CRITICAL (13:50 UTC)',
    'Perimeter block for 103.45.18.22 (13:56 UTC)',
  ],
  actionsPending: [
    'Network isolation DC-PROD-03 — awaiting approval',
    'Credential rotation SVC-ACCNT-04',
  ],
  immediateRisk:
    'Active attacker with valid credentials in PROD segment. Risk of data exfiltration or ransomware deployment if containment delayed.',
  recommendation:
    'Approve network isolation immediately. Escalate to IR Lead if approval not received within 15 minutes.',
};
  ...
  return (
    <div className="min-h-screen bg-[#07090d] text-white p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-xs text-[#8b9ab0] font-mono mb-8">
          Incident brief · Daily digest · Weekly risk · Board summary · Control-gap memo · Incident
          timeline · AAR · Drill summary
        </p>
            ...
            {REPORTS.map((r) => (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <ChevronRight
                    size={14}
                    className={`text-[#8b9ab0] mt-1 shrink-0 transition-transform ${selectedReport === r.id ? 'rotate-90' : ''}`}
                  />
                </div>
              </button>
            ))}
          ...
          <div className="lg:col-span-2">
            {!selectedReport && !generating && (
              <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-center space-y-3">
                ...
                <p className="text-sm text-[#8b9ab0] font-mono">
                  Select a report to preview or generate
                </p>
              </div>
            )}
            ...
            {selectedReport && !generating && selectedReport === 'incident_brief' && (
              <div className="bg-[#0d1117] border border-[#1e2a3a] rounded-xl p-6 space-y-5">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs text-[#8b9ab0] font-mono">
                        {SAMPLE_INCIDENT_BRIEF.incidentId}
                      </span>
                    </div>
                    <h2 className="text-base font-bold text-white">
                      {SAMPLE_INCIDENT_BRIEF.title}
                    </h2>
                    <p className="text-xs text-[#8b9ab0] font-mono mt-1">
                      Generated: {new Date(SAMPLE_INCIDENT_BRIEF.generatedAt).toLocaleString()} ·
                ...
                <div className="p-3 bg-[#c9b787]/10 border border-[#c9b787]/20 rounded-lg">
                  ...
                  <p className="text-xs text-[#8b9ab0] font-mono">
                    Assumptions: {SAMPLE_INCIDENT_BRIEF.assumptions.join('; ')}
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="bg-[#0d1117] border border-[#1e2a3a] rounded-xl p-6 space-y-5">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-[#8b9ab0] font-mono mt-1">
                      Audience: {active.audience} · Last: {active.lastGenerated}
                    </p>
                  </div>
