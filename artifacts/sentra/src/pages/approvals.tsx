import { Shield, CheckCircle2, XCircle, Clock, AlertTriangle, Bot, FileText, Terminal } from "lucide-react";
import { cn } from "@szl-holdings/shared-ui/utils";
import { useMeshState, decideFix, type FixRequest } from "@/lib/mesh-store";
    ...
    <div className="space-y-8 animate-fade-in">
      <header className="flex justify-between items-end">
        ...
        <div className="flex gap-3">
          ...
          <div className="sentra-panel px-4 py-2 text-center">
            ...
            <div className="text-2xl font-display font-bold text-emerald-400">{meshState.resilienceOverall}</div>
          </div>
        </div>
      </header>
      ...
      {resolved.length > 0 && (
        <div className="space-y-3">
          ...
          {resolved.map((item) => <ResolvedCard key={item.id} item={item} />)}
        </div>
      )}
    </div>
  );
        ...
        <div className="flex items-start gap-3 flex-1">
          ...
          <div className="flex-1">
            ...
            <div className="text-[10px] text-slate-500 font-mono">
              {item.requestedBy} · {item.resolvedAt ? new Date(item.resolvedAt).toLocaleTimeString() : ""}
            </div>
