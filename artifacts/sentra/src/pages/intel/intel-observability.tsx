import {
  ...
  type GoldenSignalsSnapshot,
  MetricTimeSeriesSimulator,
  type ServiceApmTrace,
  type SloStatus,
...
const _now = Date.now();
const _services = ['aegis-soc', 'alert-engine', 'threat-intel', 'signal-bus', 'escalation-mgr'];
...
const DEPLOY_MARKERS: DeploymentMarker[] = _sim.generateDeploymentMarkers('aegis-soc', 3, _now);
...
function ApdexGauge({ score, label }: { score: number; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <span
        className="text-[9px] font-medium text-center leading-tight w-20"
    ...
    <div className="min-h-screen p-4 space-y-3" style={{ background: BG.page }}>
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-mono" style={{ color: TEXT.muted }}>
          {new Date(_now).toISOString().slice(0, 19)}Z
        </span>
      </div>
      ...
      <Panel accent={ACCENT}>
        ...
        <div className="flex items-center justify-around p-4">
          {apdexSummary.map((a) => (
            <ApdexGauge key={a.svc} score={a.score} label={a.svc} />
          ))}
        </div>
      </Panel>
          ...
          <div className="p-2 space-y-2">
            {burnRates.map((b) => (
              <div key={b.service} className="space-y-0.5">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] truncate w-28" style={{ color: TEXT.secondary }}>
                    {b.service}
                  </span>
                  <div className="flex items-center gap-2">
                    <span
                    ...
                    <span className="text-[10px] tabular-nums" style={{ color: b.color }}>
                ...
                >
                  <div
                    ...
                    style={{ width: `${Math.min(100, 100 - b.remaining)}%`, background: b.color }}
                  />
                </div>
            ...
            {traces.map((tr, i) => {
              ...
              return (
                <div key={i} className="space-y-0.5">
                  <div className="flex items-center justify-between">
                    <span
                      className="text-[10px] font-mono truncate w-44"
                      style={{ color: TEXT.primary }}
                    >
                      {tr.method} {tr.route}
                    </span>
                    <div className="flex items-center gap-2">
                      <span className="text-[9px] tabular-nums" style={{ color: statusColor }}>
                        {tr.totalMs.toFixed(0)}ms
                      </span>
                      <span className="text-[9px]" style={{ color: TEXT.tertiary }}>
                        {tr.spans.length} spans
                      </span>
                  ...
                  >
                    <div
                      className="h-full rounded-full"
                      style={{ width: `${widthPct}%`, background: statusColor }}
                    />
                  </div>
                </div>
              );
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Panel>
          ...
          <div className="p-3 h-36">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={burnRates.map((b) => ({
                  svc: b.service.split(' ')[0],
                  errRate: parseFloat((b.burn1h * 0.3).toFixed(2)),
                  color: b.color,
                }))}
              >
                <CartesianGrid strokeDasharray="3 3" stroke={BORDER.subtle} />
