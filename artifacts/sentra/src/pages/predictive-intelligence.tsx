...
const SEED_PREDICTIONS: ThreatPrediction[] = [
  {
    ...
    currentStage: 'C2 Established',
    predictedNextStage: 'Data Staging & Exfiltration',
    timeToNextStageHours: 2.4,
    confidencePct: 89,
    severity: 'critical',
    blastRadiusTrend: [20, 28, 38, 51, 65, 78, 85, 91],
    businessImpactUsd: 340000,
    mitigationWindow: 140,
    recommendedActions: [
      'Terminate C2 channel immediately',
      'Audit data access logs for staging activity',
      'Isolate high-value data repositories',
      'Engage IR team',
    ],
  },
  {
    ...
    currentStage: 'Defense Evasion',
    predictedNextStage: 'Volume Encryption Attempt',
    timeToNextStageHours: 0.8,
    confidencePct: 94,
    severity: 'critical',
    blastRadiusTrend: [30, 45, 62, 78, 88, 93, 96, 98],
    businessImpactUsd: 890000,
    mitigationWindow: 48,
    recommendedActions: [
      'Disable vssadmin across all endpoints',
      'Enforce read-only backup volumes',
      'Activate ransomware playbook Omega',
      'Notify all clients immediately',
    ],
  },
  {
    id: 'tp3',
    threatType: 'Credential Campaign',
    currentStage: 'Reconnaissance',
    predictedNextStage: 'Exploitation Attempt',
    timeToNextStageHours: 8.5,
    confidencePct: 71,
    severity: 'high',
    blastRadiusTrend: [8, 12, 17, 23, 31, 40, 51, 63],
    businessImpactUsd: 95000,
    mitigationWindow: 510,
    recommendedActions: [
      'Enforce MFA across all tenants',
      'Block observed source IP ranges',
      'Alert identity team',
    ],
  },
];
...
const CAPACITY_RISKS: CapacityRisk[] = [
  {
    asset: 'Azure AD Identity Pool',
    assetType: 'identity',
    riskScore: 82,
    trend: [55, 62, 68, 74, 78, 80, 82],
    primaryRisk: 'MFA bypass rate exceeding threshold',
    hoursToBreach: 4.2,
  },
  {
    asset: 'Perimeter Firewall — WAN',
    assetType: 'network',
    riskScore: 61,
    trend: [40, 44, 48, 52, 56, 59, 61],
    primaryRisk: 'Connection table nearing capacity',
    hoursToBreach: 14.0,
  },
  {
    asset: 'EDR Coverage — Laptops',
    assetType: 'endpoint',
    riskScore: 73,
    trend: [60, 63, 66, 69, 71, 72, 73],
    primaryRisk: '17 endpoints missing agent update',
    hoursToBreach: 8.5,
  },
  {
    asset: 'SIEM Log Ingestion',
    assetType: 'cloud',
    riskScore: 44,
    trend: [35, 36, 38, 40, 41, 43, 44],
    primaryRisk: 'Approaching daily ingest quota',
    hoursToBreach: 22.0,
  },
];
    ...
    const t = setInterval(() => {
      setPredictions((prev) =>
        prev.map((p) => ({
          ...
          blastRadiusTrend: [
            ...p.blastRadiusTrend.slice(-7),
            Math.min(100, p.blastRadiusTrend[p.blastRadiusTrend.length - 1]! + Math.random() * 1.5),
          ],
          mitigationWindow: Math.max(0, p.mitigationWindow - 0.5),
        })),
      );
      setCapacityRisks((prev) =>
        prev.map((r) => ({
          ...r,
    ...
    <div className="max-w-7xl mx-auto space-y-5">
      <div>
        ...
        <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>
          Threat trajectory prediction with confidence intervals, capacity exhaustion forecasting,
          business impact modeling, and proactive alerts before problems materialize.
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          {
            label: 'Critical Trajectories',
            value: criticalPredictions.toString(),
            color: ACCENT,
            pulse: criticalPredictions > 0,
          },
          {
          ...
          },
          {
            ...
            value: `$${(totalImpact / 1000000).toFixed(2)}M`,
            color: '#d4a054',
          },
        ].map((c) => (
          <div
              ...
              >
                <div className="flex items-start justify-between gap-4 mb-4">
                  ...
                  <div className="text-right shrink-0">
                    <div className="text-2xl font-bold font-mono" style={{ color: sc }}>
                      {p.confidencePct}%
                    </div>
                    ...
                    {[
                      {
                        ...
                        value: `$${(p.businessImpactUsd / 1000).toFixed(0)}k`,
                        color: '#d4a054',
                      },
                      {
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-[9px] font-mono" style={{ color: DS.text.muted }}>
                      breach in {r.hoursToBreach.toFixed(1)}h
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  >
                    <div
                      ...
                      style={{ width: `${r.riskScore}%`, background: riskColor }}
                    />
                  </div>
        ...
        <div className="space-y-4">
          ...
          >
            <div className="text-xs font-bold text-white mb-4">
              3-Scenario Business Impact Model
            </div>
            <div className="space-y-4">
              {[
                {
                  label: 'Optimistic (all threats contained within 1h)',
                  impact: 45000,
                  color: '#6b8f71',
                  probability: 28,
                },
                ...
                {
                  label: 'Worst Case (threats escalate, 24h+ resolution)',
                  impact: 2100000,
                  color: ACCENT,
                  probability: 23,
                },
                ...
                >
                  ...
                  >
                    <div
                      ...
                      style={{ width: `${(s.probability / 60) * 100}%`, background: s.color }}
                    />
                  </div>
                </div>
              ))}
          ...
          >
            ...
            <div className="space-y-2 text-[10px]" style={{ color: 'rgba(255,255,255,0.5)' }}>
              <p>
                🔴 <strong className="text-white/80">Expected Value (probability-weighted)</strong>:
                ~$580k total business impact if current threats continue unmitigated
              </p>
              <p>
                ⏱ <strong className="text-white/80">Minimum mitigation window</strong>:{' '}
                {minMitigationWindow.toFixed(0)} minutes before ransomware campaign enters
                encryption phase
              </p>
              <p>
                🟢 <strong className="text-white/80">ROI on immediate containment</strong>: Every
                minute of action saves ~$12k in projected breach cost
              </p>
            </div>
          </div>
        </div>
