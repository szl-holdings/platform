...
const SEED_BRANCHES: ScenarioBranch[] = [
  {
    ...
    driftFromBaseline: 8,
    actions: [
      'Isolate WKSTN-FIN-042 via EDR',
      'Revoke krbtgt — force Kerberos reissue',
      'Enable enhanced logging on all DCs',
      'Page SOC Tier-2 analyst',
    ],
    controlGaps: [],
    recommended: true,
  },
  {
    ...
    actions: [
      ...
      'Full AD forest reset required',
      'Engage IR retainer',
      'Notify legal & comms',
    ],
    controlGaps: [
      'Alert fatigue mitigation missing',
      'SIEM correlation rules not tuned',
      'MFA gap on DC admin accounts',
    ],
    recommended: false,
  },
  {
    ...
    actions: [
      ...
      'IR + legal retainer engagement',
    ],
    controlGaps: [
      'DLP not monitoring cloud egress',
      'No UEBA baselining on privileged accounts',
      'Exfil detection rules absent',
      'Threat hunting cadence insufficient',
    ],
    recommended: false,
  },
  {
    ...
    driftFromBaseline: 2,
    actions: [
      'Hunt team identifies LDAP anomaly',
      'Proactive isolation of source host',
      'Password reset for compromised account',
      'Accelerated patch of exploited vector',
    ],
    controlGaps: [],
    recommended: false,
  },
...
function apiBranchToScenario(b: ApiBranch, idx: number): ScenarioBranch {
  ...
  return {
    ...
    actions: params.actions ??
      sim?.actions ?? ['Review twin state', 'Apply remediation playbook', 'Notify stakeholders'],
    ...
    <div
      ...
      style={{
        borderColor: selected
          ? `${oc.color}40`
          : branch.recommended
            ? 'rgba(139,122,200,0.2)'
            : 'rgba(255,255,255,0.07)',
        background: selected
          ? `${oc.color}06`
          : branch.recommended
            ? 'rgba(139,122,200,0.03)'
      ...
      <div className="p-4">
        <div className="flex items-start gap-3 mb-3">
          <div
            ...
            style={{ background: `${oc.color}15`, border: `1px solid ${oc.color}25` }}
          >
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-2 text-[9px]">
            <span className="w-20 shrink-0" style={{ color: 'rgba(255,255,255,0.3)' }}>
              Blast Radius
            </span>
            <div className="flex-1">
              <BlastRadiusBar value={branch.blastRadius} />
            </div>
          </div>
...
function ComparisonDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-stretch" onClick={onClose}>
      ...
      <div
        className="w-full max-w-2xl border-l overflow-y-auto"
          ...
          {[
            {
              ...
              valA: `${a.blastRadius}%`,
              valB: `${b.blastRadius}%`,
              better: a.blastRadius < b.blastRadius ? 'a' : 'b',
            },
            {
              label: 'Assets Affected',
              valA: `${a.assetsAffected}`,
              valB: `${b.assetsAffected}`,
              better: a.assetsAffected < b.assetsAffected ? 'a' : 'b',
            },
            { label: 'MTTR', valA: a.mttr, valB: b.mttr, better: a.mttr < b.mttr ? 'a' : 'b' },
            {
              ...
              valA: a.costImpact,
              valB: b.costImpact,
              better: a.costImpact < b.costImpact ? 'a' : 'b',
            },
            {
              label: 'Drift from Baseline',
              valA: `${a.driftFromBaseline}%`,
              valB: `${b.driftFromBaseline}%`,
              better: a.driftFromBaseline < b.driftFromBaseline ? 'a' : 'b',
            },
            {
              label: 'Probability',
              valA: `${a.probability}%`,
            ...
            <div key={row.label} className="grid grid-cols-5 gap-3 items-center">
              <div
                className="col-span-1 text-[9px] uppercase tracking-widest"
...
function AegisScenarioBranchesContent() {
  ...
  const { data: branchData, isLoading } = useStandardQuery<{
    branches: ApiBranch[];
    count: number;
  }>({
    queryKey: ['aegis-atlas-branches'],
    queryFn: () =>
      apiFetch<{ branches: ApiBranch[]; count: number }>('/atlas/spatial/branches?limit=20'),
    staleTime: 30000,
    retry: 1,
  });
  ...
  const visibleBranches = safeMode
    ? BRANCHES.filter((b) => b.outcome === 'contained' || b.outcome === 'recovering')
    : BRANCHES;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-xl font-bold text-white tracking-tight">
            Blast Radius Branch Simulation
          </h1>
          <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>
            Compare scenario branches to model blast radius, control gaps, and recommended response
            paths.
          </p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {canCompare && (
            <button
              ...
              className="text-[11px] px-3 py-1.5 rounded-lg font-medium flex items-center gap-1.5 transition-all"
              style={{
                color: '#8b7ac8',
      ...
      {safeMode && (
        ...
        >
          ...
          <div className="text-[9px]" style={{ color: 'rgba(255,255,255,0.4)' }}>
            <span className="font-semibold" style={{ color: '#8b7ac8' }}>
              Executive Safe Mode:{' '}
            </span>
            Escalation and catastrophic branches hidden. {BRANCHES.length - visibleBranches.length}{' '}
            scenario{BRANCHES.length - visibleBranches.length !== 1 ? 's' : ''} suppressed — showing
            only approved containment paths.
          </div>
        </div>
      )}
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[11px] font-semibold text-white">
              Recommended Actions — {selectedBranches[0].name}
            </span>
          </div>
          <div className="space-y-1.5">
      ...
      {comparing && canCompare && (
        <ComparisonDrawer
          a={selectedBranches[0]}
          b={selectedBranches[1]}
