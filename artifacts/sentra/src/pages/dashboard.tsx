import { ShieldAlert, Activity, Cpu, RotateCcw, ShieldCheck, BarChart3, AlertTriangle, Zap, Brain, Search } from "lucide-react";
import { useState } from "react";
import { sentraTwin } from "@/data/sentra-twin";

const BASELINE_ENTITIES = [
  { name: "api-gateway-prod", type: "Service", normalTraffic: "12.4k req/min", current: "12.1k req/min", deviation: 2, status: "normal" },
  { name: "db-replica-01", type: "Database", normalTraffic: "840 conn/min", current: "2,341 conn/min", deviation: 178, status: "anomaly" },
  { name: "auth-service", type: "Service", normalTraffic: "3.2k req/min", current: "3.4k req/min", deviation: 6, status: "normal" },
  { name: "analytics-worker-03", type: "Compute", normalTraffic: "Idle 82%", current: "Idle 12%", deviation: 85, status: "watch" },
];
    ...
    <div className="sentra-panel p-5 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[9px] font-mono text-slate-400 uppercase tracking-wider">Pattern of Life · Live</span>
        </div>
      </div>
      <div className="space-y-2">
        {BASELINE_ENTITIES.map(e => {
          const devColor = e.status === "anomaly" ? "text-red-400" : e.status === "watch" ? "text-amber-400" : "text-emerald-400";
          const barColor = e.status === "anomaly" ? "bg-red-500" : e.status === "watch" ? "bg-amber-500" : "bg-emerald-500";
          const barWidth = e.status === "anomaly" ? "100%" : e.status === "watch" ? "70%" : `${Math.min(100, 50 + e.deviation)}%`;
          return (
            <div key={e.name} className="grid grid-cols-12 gap-2 items-center py-2 border-b border-slate-800/60 last:border-0">
              ...
              <div className="col-span-3 text-[10px] text-slate-500 font-mono">{e.normalTraffic}</div>
              <div className="col-span-3 text-[10px] font-mono font-semibold" style={{ color: e.status === "anomaly" ? "#f87171" : e.status === "watch" ? "#fbbf24" : "#34d399" }}>{e.current}</div>
      ...
      <div className="text-[9px] font-mono text-slate-600">Baseline computed over rolling 30-day window · Antigena autonomous response: ENABLED</div>
    </div>
  );
}
  ...
  return (
    <div className="sentra-panel p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] font-mono uppercase tracking-widest text-slate-500">Threat Intelligence Query</span>
        <span className="text-[9px] font-mono px-1.5 py-0.5 rounded" style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.15)", color: "#f87171" }}>NL-powered</span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="sentra-panel p-5 space-y-3">
          ...
          <div className="flex items-end gap-3">
            ...
            <div className="text-xs text-slate-500 mb-1.5">estimated mean time to recover (MTTR)</div>
          </div>
          <div className="space-y-2">
            {[
          ...
          <div className="text-[9px] font-mono text-slate-600">Agent-verified · Updated on each control state change</div>
        </div>
      </div>
