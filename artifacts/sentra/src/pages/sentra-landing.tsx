...
const SIGNAL_DEMO = [
  { time: "09:42:08", severity: "critical", label: "Lateral movement detected · prod-finance-vpc", action: "Isolate host · escalate to commander" },
  { time: "09:41:51", severity: "high", label: "MFA bypass attempt · CFO identity", action: "Force re-auth · session revoked" },
  { time: "09:39:14", severity: "medium", label: "Backup drift · 2 critical datasets > RPO", action: "Trigger snapshot · notify owner" },
  { time: "09:36:02", severity: "info", label: "Control gap closed · firewall ruleset 14b", action: "Evidence captured · audit trail signed" },
];
    ...
    <div className="min-h-screen bg-[#0a0606] text-red-50 overflow-x-hidden">
      <section className="relative min-h-screen flex flex-col items-center justify-center px-6 pt-24 pb-16">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <svg className="absolute inset-0 w-full h-full opacity-[0.07]" viewBox="0 0 1200 600" aria-hidden="true">
            <defs>
            ...
            <g stroke="rgba(239,68,68,0.6)" strokeWidth="0.5" fill="none">
              {Array.from({ length: 13 }, (_, i) => (
                <line key={`h${i}`} x1={0} y1={i * 50} x2={1200} y2={i * 50} strokeDasharray="3 8" />
              ))}
        ...
        <div className="relative z-10 max-w-4xl text-center">
          <div className="flex items-center justify-center gap-2 mb-8">
            ...
            <span className="text-sm font-medium text-red-400/60">Sentra Cyber Resilience Command · Powered by Alloy</span>
          </div>
          ...
          <p className="text-lg text-red-200/60 max-w-2xl mx-auto mb-10 leading-relaxed">
            Sentra is the cyber resilience command platform for security leaders who need more than a SIEM dashboard. See the exposures that map to real assets, prove the recovery you promised, and run incidents from a single command surface.
          </p>
      ...
      <section className="px-6 py-20 bg-[#070404] border-y border-red-500/5">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-[10px] font-mono text-red-400/40 uppercase tracking-widest mb-3">What Sentra Does</p>
            <h2 className="font-display text-3xl font-bold text-red-50 mb-3">Cyber resilience command. Not another SIEM.</h2>
            <p className="text-red-300/50 mt-3 max-w-xl mx-auto text-sm">
              Most security tools stop at alert. Sentra carries every signal through to the asset it threatens, the recovery it endangers, and the decision someone has to make in the next ten minutes.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { icon: ShieldAlert, title: "Threat Overview", desc: "Live posture by asset tier, with severity, owner, and the recovery path each signal touches.", color: "text-red-400" },
              { icon: Cpu, title: "Asset Risk Graph", desc: "Every system rendered with exposure, blast radius, and downstream dependencies in one graph.", color: "text-orange-400" },
              { icon: RotateCcw, title: "Recovery Readiness", desc: "RTO, RPO, last-tested restore, and the gap between what was promised and what is rehearsed.", color: "text-amber-400" },
              { icon: Activity, title: "Incident Commander", desc: "Live incident view with attacker timeline, containment options, and the audit trail regulators expect.", color: "text-violet-400" },
              { icon: BarChart3, title: "Exposure Board", desc: "Quantified exposure by business unit — dollarized, prioritized, and tied to mitigation owners.", color: "text-red-400" },
              { icon: ShieldCheck, title: "Control Drift", desc: "Continuous diff between attested controls and live production state, with evidence on every change.", color: "text-emerald-400" },
            ].map((feature) => (
              <div key={feature.title} className="bg-[#120a0a]/80 border border-red-500/10 rounded-xl p-5 hover:border-red-500/25 transition-all">
      ...
      <section className="px-6 py-20 max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <p className="text-[10px] font-mono text-red-400/40 uppercase tracking-widest mb-3">Core Views</p>
          <h2 className="font-display text-3xl font-bold text-red-50">Four command surfaces. One resilience platform.</h2>
          <p className="text-red-300/50 mt-3 max-w-xl mx-auto text-sm">Each view is purpose-built for a specific decision security leaders actually have to make.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {CORE_VIEWS.map((view) => (
            <div key={view.title} className="bg-[#120a0a]/80 border border-red-500/10 rounded-xl p-6">
      ...
      <section className="px-6 py-20 bg-[#070404] border-y border-red-500/5">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            <p className="text-[10px] font-mono text-red-400/40 uppercase tracking-widest mb-3">Live Signal Demo</p>
            <h2 className="font-display text-3xl font-bold text-red-50 mb-3">Every signal is a decision in waiting.</h2>
            <p className="text-red-300/50 max-w-xl mx-auto text-sm">A snapshot from the Sentra command stream — every event already mapped to severity, asset, and the next action.</p>
          </div>
          ...
          <div className="bg-[#120a0a]/90 border border-red-500/15 rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-red-500/10 bg-[#1a0d0d]/60">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-mono uppercase tracking-widest text-red-300/70">Live · Decision Stream</span>
              </div>
              <span className="text-[10px] font-mono text-red-400/40">prod-finance-vpc · region us-east-1</span>
            </div>
            <div className="divide-y divide-red-500/5">
              {SIGNAL_DEMO.map((row) => (
                <div key={row.time} className="grid grid-cols-12 gap-3 items-center px-5 py-3.5">
                  ...
                  <div className="col-span-5 text-[12px] text-red-100/80 leading-snug">{row.label}</div>
                  <div className="col-span-3 text-[11px] text-red-300/55 leading-snug text-right">{row.action}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="px-6 py-20 max-w-6xl mx-auto">
        <div className="text-center mb-12">
          <p className="text-[10px] font-mono text-red-400/40 uppercase tracking-widest mb-3">What It Solves</p>
          <h2 className="font-display text-3xl font-bold text-red-50 mb-3">Eight problems. One resilience command.</h2>
        </div>
      ...
      <section className="px-6 py-20 bg-[#070404] border-y border-red-500/5">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-[10px] font-mono text-red-400/40 uppercase tracking-widest mb-3">Why It Matters</p>
            <h2 className="font-display text-3xl font-bold text-red-50 mb-4">From alert noise to resilience command.</h2>
            <p className="text-red-300/50 text-sm max-w-2xl mx-auto leading-relaxed">
              Every CISO already has alerts. Very few can prove that the controls they attested to are still in place, that the recovery they promised is actually rehearsed, or that the next incident has an owner before it happens. That is the gap Sentra closes.
            </p>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[
              { metric: "11m", label: "median containment time", trend: "vs 47m unmanaged baseline" },
              { metric: "$8.4M", label: "exposure quantified & owned", trend: "across 247 monitored assets" },
              { metric: "94%", label: "controls continuously attested", trend: "↑ vs 71% prior quarter" },
              { metric: "42%", label: "critical-tier recovery posture", trend: "tracked to RTO commitment" },
            ].map((o) => (
      ...
      <section className="px-6 py-20 bg-[#070404] border-t border-red-500/5">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-[10px] font-mono text-red-400/40 uppercase tracking-widest mb-3">Powered by Alloy · SZL Holdings</p>
          ...
          <p className="text-red-300/55 mb-8 text-sm leading-relaxed max-w-xl mx-auto">
            Sentra is built for security leaders who need to command resilience, not just monitor it. Request a pilot — we will walk through the full command center against your environment.
          </p>
          <div className="flex items-center justify-center gap-4 flex-wrap">
            <button
              onClick={() => setDemoOpen(true)}
