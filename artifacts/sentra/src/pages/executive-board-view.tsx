...
function MiniTrend({
  ...
  data: Array<{ month: string; posture: number; incidents: number; mttr: number }>;
  metric: 'posture' | 'incidents' | 'mttr';
}) {
...
export default function ExecutiveBoardView() {
  ...
  const livePostureScore =
    latestRiskScore?.score != null
      ? Math.round(Number(latestRiskScore.score))
      : FALLBACK_POSTURE_SCORE;
    ...
    return critical
      ...
      .map((f) => {
        const daysOpen = Math.max(
          1,
          Math.round((Date.now() - new Date(f.createdAt).getTime()) / (1000 * 60 * 60 * 24)),
        );
        const residual =
          f.severity === 'critical'
            ? 9.0 + Math.random() * 1
  ...
  const slaMetrics = useMemo(() => {
    ...
    const criticalResolved = criticalIncidents.filter(
      (i) => i.status === 'closed' || i.status === 'resolved',
    );
    const highResolved = highIncidents.filter(
      (i) => i.status === 'closed' || i.status === 'resolved',
    );
    ...
    const p1Sla =
      criticalResolved.length > 0
        ? Math.round((criticalWithinSla.length / criticalResolved.length) * 100)
        : 100;
    const p2Sla =
      highResolved.length > 0
        ? Math.round((highWithinSla.length / highResolved.length) * 100)
        : 100;
      ...
      <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px]" style={{ color: 'rgba(255,255,255,0.35)' }}>
            Live posture · Updated {lastUpdated}
          </p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
        <div className="grid grid-cols-12 gap-4">
          <div className="col-span-12 md:col-span-3 bg-white/[0.025] border border-white/5 rounded-xl p-5 flex flex-col items-center">
            <div className="text-[9px] font-mono uppercase tracking-[0.15em] mb-3 text-white/40 w-full">
          ...
          <div className="col-span-12 md:col-span-9 grid grid-cols-2 md:grid-cols-3 gap-4">
            <div className="col-span-2 md:col-span-1 bg-white/[0.025] border border-white/5 rounded-xl p-4">
              ...
              <div
                className="mt-3 pt-3 border-t border-white/5 text-[10px]"
                style={{ color: 'rgba(255,255,255,0.35)' }}
              >
                  ...
                  activeIncidents.map((inc) => {
                    ...
                    return (
                      <div key={inc.id} className="px-4 py-3 flex items-center gap-4">
                        ...
                        <span className="text-[9px] font-mono text-white/35 capitalize">
                          {inc.status}
                        </span>
                  ...
                  >
                    ...
                    <span>
                      <span className="text-white/40">
                        {cg.total - cg.implemented - cg.partial}
                      </span>{' '}
                      gap
                    </span>
                  </div>
