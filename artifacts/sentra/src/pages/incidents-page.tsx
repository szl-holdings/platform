    ...
    body: JSON.stringify({
      ...
      data: {
        ...
        responseActions: [
          ...
          'Incident record opened in Sentra platform with full audit trail.',
...
function buildIncidentEntity(incident: any): OperationalEntity {
  ...
  if (incident.severity) {
    evidence.push({
      ...
      confidence:
        incident.severity === 'critical' ? 0.97 : incident.severity === 'high' ? 0.88 : 0.75,
    });
  }
      ...
      incident.severity === 'critical'
        ...
        : incident.severity === 'high'
          ...
          : incident.severity === 'medium'
            ? 0.5
            : 0.2,
    owner: incident.assignedAnalyst
      ? { name: incident.assignedAnalyst, role: 'Analyst' }
      : undefined,
    nextAction:
      incident.status === 'closed'
        ? undefined
        : statusOrder[Math.min(statusOrder.indexOf(incident.status) + 1, statusOrder.length - 1)]
          ? `Advance to ${statusOrder[Math.min(statusOrder.indexOf(incident.status) + 1, statusOrder.length - 1)]}`
          : undefined,
    evidence,
...
function IncidentDetailSidePane({
  incident,
  onClose,
  onUpdate,
}: {
  ...
  const isDemoIncident =
    !incident.id || String(incident.externalId ?? incident.id).startsWith('INC-2026');
    ...
    >
      ...
      >
        <div className="px-5 py-4 border-b border-white/10 sticky top-0 bg-[#0A0D14] z-10 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-white/10 text-muted-foreground transition-colors shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
...
export default function IncidentsPage() {
  ...
  const { data: incidents = [], isLoading } = useStandardQuery({
    queryKey: ['incidents'],
  ...
  const selectedIncident =
    detailPaneIncident ??
    (selectedIncidentId !== null ? incidents.find((i: any) => i.id === selectedIncidentId) : null);
  ...
  const updateMut = useStandardMutation({
    ...
    onMutate: async ({ id, data }) => {
      ...
      qc.setQueryData(['incidents'], (old: unknown) => {
        ...
        const list = Array.isArray(asObj.incidents)
          ? asObj.incidents
          : Array.isArray(old)
            ? (old as Record<string, unknown>[])
            : [];
        const updated = list.map((i: Record<string, unknown>) =>
          i.id === id ? { ...i, ...data } : i,
        );
    ...
    <div className="p-6 space-y-6">
      ...
      {liveAlert && (
        ...
        >
          ...
          {liveAlert.title && (
            <span className="text-sm text-muted-foreground truncate">{liveAlert.title}</span>
          )}
      ...
      <div className="flex items-center justify-between animate-fade-in-up">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Triage, containment, root cause tracking, and post-incident review
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="bg-card border-border">
              <DialogHeader>
                <DialogTitle className="font-display">Create Incident</DialogTitle>
              </DialogHeader>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setForm((p) => ({ ...p, assignedAnalyst: e.target.value }))}
                ...
                <div>
                  ...
                  <Input
                    value={form.attackTechnique}
                    onChange={(e) => setForm((p) => ({ ...p, attackTechnique: e.target.value }))}
              ...
              <div className="space-y-2">
                {incidents
                  ...
                  .map((incident: any) => (
                    <Card
                      key={incident.id}
                      className={`bg-card border-border hover:border-primary/20 transition-all cursor-pointer ${incident.severity === 'critical' ? 'ring-1 ring-red-500/10' : ''}`}
                    ...
                    >
                      <CardContent className="p-3">
                        ...
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <OperationalRiskBadge
                            level={severityToRiskLevel(incident.severity ?? 'low')}
                            size="xs"
                          />
                          {incident.assignedAnalyst && (
                            <OperationalOwnerChip
                              owner={{ name: incident.assignedAnalyst, role: 'Analyst' }}
                              size="xs"
                            />
                          )}
                        </div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
                        ...
                        <span className="flex items-center gap-1">
                          ...
                          {formatAgo(incident.detectedAt ?? incident.createdAt)}
                        </span>
                      </div>
                    </div>
                    <div
      ...
      <ActivityFeed
        entityType="incident"
        title="Incident Response Team Activity"
        limit={8}
        compact
      />
    </div>
