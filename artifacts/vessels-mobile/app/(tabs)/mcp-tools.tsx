...
const QUICK_ACTIONS = [
  { id: "fleet-status", label: "Fleet Status", icon: "anchor", color: "#0ea5e9" },
  { id: "route-opt", label: "Route Optimize", icon: "map", color: "#10b981" },
  { id: "distress", label: "Distress Signals", icon: "alert-triangle", color: "#ef4444" },
  { id: "manifest", label: "Manifest Scan", icon: "file-text", color: "#f59e0b" },
];
...
export default function AICommandScreen() {
  ...
  const { cached: cachedDocs, save: saveDocs } = useOfflineCache<DocEntry[]>("vessels-recent-docs");
  const [recentDocs, setRecentDocs] = useState<DocEntry[]>([]);

  const apiBase = getApiBase();
  const { notifications, handleAction, dismissNotification } = usePushNotifications(ACCENT, apiBase, "vessels", authToken);
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.headerSub}>Vessels Maritime Intelligence</Text>
        </View>
      ...
      >
        <TouchableOpacity onPress={() => setCopilotVisible(true)} style={styles.copilotCard} activeOpacity={0.85}>
          <View style={styles.copilotInner}>
            <View style={styles.copilotLeft}>
              ...
              <View>
                <Text style={styles.copilotTitle}>Vessels AI Copilot</Text>
                <Text style={styles.copilotDesc}>Fleet intelligence • Route analysis • Distress detection</Text>
              </View>
            </View>
        ...
        <View style={styles.quickGrid}>
          {QUICK_ACTIONS.map(action => (
            <TouchableOpacity
              ...
              onPress={action.id === "manifest" ? () => setDocScanVisible(true) : () => setCopilotVisible(true)}
              activeOpacity={0.7}
            >
        ...
        <Text style={styles.sectionLabel}>Shipping Manifest Scanner</Text>
        <TouchableOpacity onPress={() => setDocScanVisible(true)} style={styles.docCard} activeOpacity={0.8}>
          <View style={styles.docLeft}>
            ...
            <View>
              <Text style={styles.docTitle}>Scan Bill of Lading</Text>
              <Text style={styles.docDesc}>AI extracts vessel, cargo, and port data instantly</Text>
            </View>
          </View>
        ...
        {recentDocs.length > 0 && (
          <>
            <Text style={styles.sectionLabel}>Recent Manifests</Text>
            {recentDocs.map(doc => (
        ...
        {[
          { label: "AIS Tracking", status: "147 Vessels", color: "#10b981" },
          { label: "Distress Monitor", status: "Active", color: "#10b981" },
          { label: "Route Optimizer", status: "Running", color: "#10b981" },
      ...
      <AICopilot
        ...
        agentId="vessels"
        accentColor={ACCENT}
        welcomeMessage="Vessels Maritime AI online. I'm tracking 147 vessels across 12 sea lanes. What would you like to know?"
        suggestions={[
          "Show vessels with distress signals",
          "Optimize route for MV Atlantic Meridian",
          "Check port availability in New York",
      ...
      <DocumentCapture
        ...
        accentColor={ACCENT}
        title="Manifest & Bill of Lading Scanner"
        apiBase={apiBase}
        authToken={authToken}
      />
    </View>
...
const styles = StyleSheet.create({
  ...
  docCard: { marginHorizontal: 14, borderRadius: 12, borderWidth: 1, borderColor: ACCENT + "30", backgroundColor: CARD, flexDirection: "row", alignItems: "center", justifyContent: "space-between", padding: 14 },
