...
export default function BriefingScreen() {
  ...
  const { context } = useContextualAwareness([
    {
      ...
      proximityTriggers: [
        { label: "Port of Los Angeles", latRange: [33.5, 33.9], lonRange: [-118.5, -118.1], relevance: 0.95, reason: "Near Port of Los Angeles — fleet data prioritized" },
        { label: "Port of New York", latRange: [40.5, 40.8], lonRange: [-74.2, -73.9], relevance: 0.95, reason: "Near Port of New York — vessel activity elevated" },
        { label: "Port of Miami", latRange: [25.7, 25.9], lonRange: [-80.3, -80.1], relevance: 0.9, reason: "Near Port of Miami — maritime intel active" },
      ],
    },
  ]);
  ...
  const { sendDomainAlert } = useAmbientBackground({ domain: "maritime", accentColor: ACCENT });
  ...
  const timeLabel = context.briefingWindow !== "none"
    ? context.briefingWindow === "morning" ? "Fleet Morning Brief" : "Fleet Evening Digest"
    ...
    <View style={[styles.container, { backgroundColor: colors.background ?? "#0a0a0a" }]}>
        ...
        <View style={styles.headerLeft}>
          ...
          <View>
            ...
            <Text style={styles.screenSubtitle}>
              ...
              {context.activeSignals.length > 0 ? ` · Near port` : ""}
            </Text>
          </View>
        </View>
      ...
      {context.activeSignals.length > 0 && (
        <View style={[styles.contextBanner, { borderColor: ACCENT + "30", backgroundColor: ACCENT + "0a" }]}>
          <Feather name="anchor" size={12} color={ACCENT} />
