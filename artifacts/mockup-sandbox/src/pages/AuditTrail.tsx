...
function generateAuditEntries(): AuditEntry[] {
  const agents = [
    ...
    { slug: "nexus", name: "NEXUS Orchestrator", actions: ["plan_agent_sequence", "stitch_outputs", "verify_completeness"] },
  ];
...
function EntryRow({ entry }: { entry: AuditEntry }) {
  ...
  const statusColor = entry.status === "success" ? "#00ff88" : entry.status === "error" ? "#ff4455" : "#8896aa";

  return (
    <div className={`border rounded-lg overflow-hidden transition-colors ${
      open ? "border-[#a3e635]/30 bg-nexus-surface" : "border-nexus bg-nexus-surface hover:border-nexus-cyan/20"
    }`}>
      <button
      ...
      >
        ...
        <div className="flex items-center gap-3 shrink-0">
          <span className="text-[9px] font-mono text-muted-foreground/40">{entry.durationMs}ms</span>
          <span className="text-[10px] font-mono px-2 py-0.5 rounded" style={{ color: statusColor, backgroundColor: `${statusColor}15` }}>
            {entry.status}
          </span>
        ...
        <div className="px-4 pb-4 pt-1 space-y-3 border-t border-nexus">
          <div className="grid grid-cols-2 gap-3 mt-2">
            ...
            <div className="bg-nexus-bg rounded-lg p-3">
              ...
              <p className="text-[10px] text-muted-foreground leading-relaxed">{entry.outputSummary}</p>
          ...
          <div className="bg-nexus-bg rounded-lg p-3">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-[10px] font-semibold text-[#ffb700]">Agent Rate Limits at Execution</span>
            </div>
          ...
          return (
            <div key={e.agentSlug} className="bg-nexus-bg rounded-lg p-3">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-5 h-5 rounded text-[8px] font-mono font-bold flex items-center justify-center shrink-0"
