...
export function AlloyKernelPanel() {
  const kernel = useMemo(() => getKernel('deck-from-brief'), []);
  const [showExample, setShowExample] = useState(false);
  const example = kernel.codex.examples[0];

  return (
    <div className="rounded-xl border border-violet-500/20 bg-[#0d0b1a]/80 p-5 mt-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-sm font-bold text-slate-100">Deck from Brief</div>
