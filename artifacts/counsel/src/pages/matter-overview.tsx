...
import { Briefcase, Clock, DollarSign, Scale, ShieldAlert } from 'lucide-react';
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          ...
          {
            label: 'In Litigation',
            value: matterTwins.filter((m) => m.type.includes('Litigation')).length,
            icon: Scale,
          },
          {
            label: 'M&A Active',
            value: matterTwins.filter((m) => m.type === 'M&A').length,
            icon: DollarSign,
          },
          {
            label: 'Avg Exposure',
            value: `$${(matterTwins.reduce((acc, m) => acc + m.exposureUsd, 0) / matterTwins.length / 1000000).toFixed(1)}M`,
            icon: ShieldAlert,
          },
        ].map((stat, i) => (
