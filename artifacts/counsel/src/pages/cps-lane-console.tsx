...
const PAYLOAD_ID = 'cps-counsel-docket-risk';
...
export default function CounselCpsLaneConsole() {
  ...
  function downloadProofBundle(runId: string) {
    ...
    a.href = url;
    a.download = `decision-log-${runId.slice(0, 8)}.json`;
    a.click();
    ...
    <div className="space-y-6 animate-fade-in p-6">
      <header className="flex items-end justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-display font-bold text-slate-100">Docket-Risk Interceptor</h1>
            <span className="text-[10px] font-mono px-2 py-0.5 rounded border border-violet-500/30 text-violet-400 bg-violet-500/10">
              CPS · Legal Risk
            </span>
          </div>
          <p className="text-slate-400 text-sm">
            Deadline collision · Adverse filings · Jurisdictional conflicts — immutable audit decision log
          </p>
        </div>
        <div className="flex gap-3 items-center">
          ...
          <button
            ...
            disabled={executing}
            className="flex items-center gap-2 px-4 py-2 rounded text-sm font-medium bg-violet-600/20 hover:bg-violet-600/30 border border-violet-500/30 text-violet-300 transition-colors disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: 'Escalation Pending', value: pendingApprovals.length, color: 'text-amber-400' },
        ].map((stat) => (
          <div key={stat.label} className="bg-[#0a0614] border border-violet-900/30 rounded-lg p-4">
      ...
      <div className="flex gap-1 border-b border-violet-900/30">
        {(['runs', 'approvals'] as const).map((tab) => (
          ...
          >
            {tab === 'approvals' ? `Escalation Queue${pendingApprovals.length > 0 ? ` (${pendingApprovals.length})` : ''}` : 'Run History'}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {runs.length === 0 && (
            <div className="bg-[#0a0614] border border-violet-900/30 rounded-lg p-8 text-center text-slate-500">
              No runs yet. Click "Run Detection" to scan dockets for risk signals.
            </div>
          )}
          {runs.map((run) => {
            ...
            return (
              <div key={run.id} className="bg-[#0a0614] border border-violet-900/30 rounded-lg overflow-hidden">
                <div
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className={cn('text-xs font-mono px-2 py-0.5 rounded border', cfg.color,
                      run.status === 'completed' ? 'bg-emerald-500/10 border-emerald-500/20'
                        ...
                        : 'bg-violet-500/10 border-violet-500/20')}>{cfg.label}</span>
                    <span className="text-xs text-slate-500 ml-auto">
                      {run.startedAt ? new Date(run.startedAt).toLocaleString() : '—'}
                    </span>
                    {run.proofBundle && <FileText className="w-3.5 h-3.5 text-slate-500" title="Decision log available" />}
                  </div>
                ...
                {expanded && (
                  <div className="border-t border-violet-900/30 p-4 space-y-4">
                    {run.detect && (
                      <div>
                        <div className="text-[10px] font-mono text-slate-500 uppercase mb-2">Detection</div>
                        <div className="text-xs text-slate-300">
                          {run.detect.triggered
                            ? `${run.detect.signals?.length ?? 0} docket risk signal(s) — confidence ${((run.detect.confidence ?? 0) * 100).toFixed(0)}%`
                            : 'No docket risks detected'}
                        </div>
                    ...
                    {run.decide && (
                      <div>
                        <div className="text-[10px] font-mono text-slate-500 uppercase mb-2">Matter Response Plan</div>
                    ...
                    {run.proofBundle && (
                      <div className="flex gap-2">
                        ...
                        >
                          <Eye className="w-3 h-3" /> View Decision Log
                        </button>
                        {proof && (
                          <button
                            onClick={() => downloadProofBundle(run.id)}
                            className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded border border-violet-500/30 text-violet-400 hover:bg-violet-500/10 transition-colors"
                          >
                            <Download className="w-3 h-3" /> Download Audit Log
                          </button>
                        )}
                      </div>
                    )}
                    {proof && (
                      <div className="bg-black/40 rounded p-3 text-xs font-mono text-slate-400 max-h-48 overflow-auto">
                        <div className="text-[10px] text-slate-600 mb-1">Immutable Decision Log · {proof.id?.slice(0, 8)}</div>
        ...
        <div className="space-y-3">
          {pendingApprovals.length === 0 && (
            <div className="bg-[#0a0614] border border-violet-900/30 rounded-lg p-8 text-center text-slate-500">
              No matter escalations pending review.
            </div>
          )}
          {pendingApprovals.map((approval) => {
            ...
            return (
              <div key={approval.id} className={cn(
                'bg-[#0a0614] border border-violet-900/30 rounded-lg p-5',
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {isUrgent && <span className="text-[10px] text-red-400 font-mono">⚠ Court deadline risk</span>}
                    </div>
