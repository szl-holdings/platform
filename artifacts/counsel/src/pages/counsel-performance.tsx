import { obligationTwins, matterTwins } from "../data/counsel-twin";
import { Network, AlertTriangle, Link2, ArrowRight } from "lucide-react";
  ...
  return (
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-violet-400/60 text-sm">Mapping matter dependencies and blocking events.</p>
      </header>
              ...
              {matterTwins.slice(0, 1).map(matter => (
                <div key={matter.id} className="space-y-8">
                  ...
                  <div className="grid grid-cols-2 gap-8">
                    {obligationTwins.filter(o => o.matterId === matter.id && o.dependencies.length === 0).map(o => (
                      <div key={o.id} className="relative">
                        <div className={cn(
                          "p-3 rounded-lg border text-[10px] space-y-1",
                          o.status === "overdue" ? "bg-red-500/10 border-red-500/40" : "bg-violet-500/5 border-violet-500/20"
                        )}>
                    ...
                    {obligationTwins.filter(o => o.matterId === matter.id && o.dependencies.length > 0).map(o => (
                      <div key={o.id} className={cn(
                        "p-3 rounded-lg border text-[10px] space-y-1",
                        o.status === "blocked" ? "bg-amber-500/10 border-amber-500/40" : "bg-violet-500/5 border-violet-500/20"
                      )}>
                        <div className="font-bold text-violet-100">{o.title}</div>
                        <div className="text-violet-400/60 uppercase">{o.status}</div>
                      </div>
              ...
              {blockedObligations.map(o => (
                <div key={o.id} className="p-3 rounded-lg bg-amber-500/5 border border-amber-500/10 space-y-1">
                  ...
                  <div className="text-[10px] text-amber-400/60">
                    Blocked by: {obligationTwins.find(dep => o.dependencies.includes(dep.id))?.title}
                  </div>
                </div>
              ))}
            </div>
