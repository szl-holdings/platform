...
import {
  CheckCircle2, XCircle, Clock, ShieldCheck, FileText, Sparkles,
  ArrowUpRight, Hash, GitBranch
} from "lucide-react";
...
export default function Approvals() {
  ...
  const matterById = useMemo(
    () => Object.fromEntries(matterTwins.map(m => [m.id, m])),
    [],
  );
    ...
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-violet-400/60 text-sm">
          Counsel OS escalation recommendations packaged as Proof Envelopes — review evidence, then approve or reject.
        </p>
      </header>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-violet-500/5 border border-violet-500/10 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-mono uppercase tracking-widest text-violet-300/70">Pending</span>
          </div>
        ...
        <div className="bg-emerald-500/5 border border-emerald-500/15 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-mono uppercase tracking-widest text-emerald-300/80">Approved</span>
          </div>
        ...
        <div className="bg-red-500/5 border border-red-500/15 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-mono uppercase tracking-widest text-red-300/80">Rejected</span>
          </div>
          ...
          {activeProof ? (
            <div className="bg-[#0a0614] border border-violet-500/15 rounded-xl p-5 space-y-5">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="text-right">
                  ...
                  <div className={cn(
                    ...
                    activeProof.confidence >= 0.9 ? "text-emerald-300"
                      : activeProof.confidence >= 0.7 ? "text-violet-200" : "text-amber-300",
                  )}>
              ...
              <div>
                ...
                <p className="text-xs text-violet-200/80 leading-relaxed">{activeProof.rationale}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg bg-emerald-500/5 border border-emerald-500/15">
                  ...
                  <p className="text-[11px] text-violet-100/80 leading-relaxed">{activeProof.riskIfApproved}</p>
                </div>
                <div className="p-3 rounded-lg bg-red-500/5 border border-red-500/15">
                  <div className="text-[9px] font-mono uppercase tracking-widest text-red-300/80 mb-1">
                    If Rejected
                  </div>
                  <p className="text-[11px] text-violet-100/80 leading-relaxed">{activeProof.riskIfRejected}</p>
                </div>
              </div>
                ...
                <div>
                  ...
                  <ul className="space-y-1">
                    {activeProof.policyRefs.map(r => (
                      <li key={r} className="text-[11px] text-violet-200/80 font-mono">· {r}</li>
                    ))}
                  </ul>
                </div>
                <div>
              ...
              <div className="flex items-center gap-3 text-[10px] font-mono text-violet-400/60 pt-3 border-t border-violet-500/10">
                <span className="flex items-center gap-1"><GitBranch className="w-3 h-3" /> {activeProof.modelVersion}</span>
                <span className="flex items-center gap-1"><Hash className="w-3 h-3" /> {activeProof.proofHash}</span>
                <span className="flex items-center gap-1"><ArrowUpRight className="w-3 h-3" /> {activeProof.proposedBy}</span>
              </div>
