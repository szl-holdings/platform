import { Lock, ShieldCheck, FileText, GitBranch, Hash, Filter } from "lucide-react";
...
const kindStyle: Record<ProvenanceKind, { bg: string; border: string; text: string; label: string; icon: typeof Lock }> = {
  recommendation: { bg: "bg-violet-500/10", border: "border-violet-500/25", text: "text-violet-300", label: "Recommendation", icon: GitBranch },
  policy: { bg: "bg-sky-500/10", border: "border-sky-500/25", text: "text-sky-300", label: "Policy", icon: ShieldCheck },
  approval: { bg: "bg-emerald-500/10", border: "border-emerald-500/25", text: "text-emerald-300", label: "Approval", icon: Lock },
  rejection: { bg: "bg-red-500/10", border: "border-red-500/25", text: "text-red-300", label: "Rejection", icon: FileText },
  drift: { bg: "bg-amber-500/10", border: "border-amber-500/25", text: "text-amber-300", label: "Drift", icon: GitBranch },
};

const statusStyle: Record<VerificationStatus, { bg: string; border: string; text: string; label: string }> = {
  verified: { bg: "bg-emerald-500/10", border: "border-emerald-500/25", text: "text-emerald-300", label: "VERIFIED" },
  pending: { bg: "bg-violet-500/10", border: "border-violet-500/25", text: "text-violet-300", label: "PENDING" },
  failed: { bg: "bg-red-500/10", border: "border-red-500/25", text: "text-red-300", label: "FAILED" },
};

const FILTERS: ("all" | ProvenanceKind)[] = ["all", "recommendation", "approval", "rejection", "policy", "drift"];
    ...
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-violet-400/60 text-sm">
          Evidence-backed log of every AI recommendation, policy decision, and human override across Counsel.
        </p>
      </header>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-violet-500/5 border border-violet-500/10 rounded-xl p-4">
          <div className="text-[10px] font-mono uppercase tracking-widest text-violet-300/70 mb-2">Entries (24h)</div>
          <div className="text-3xl font-bold text-violet-50">{entries.length}</div>
        </div>
        <div className="bg-emerald-500/5 border border-emerald-500/15 rounded-xl p-4">
          <div className="text-[10px] font-mono uppercase tracking-widest text-emerald-300/80 mb-2">Verified</div>
          <div className="text-3xl font-bold text-emerald-50">{verified}</div>
        </div>
        <div className="bg-violet-500/5 border border-violet-500/10 rounded-xl p-4">
          <div className="text-[10px] font-mono uppercase tracking-widest text-violet-300/70 mb-2">Integrity</div>
          <div className="text-3xl font-bold text-violet-50">{integrity}%</div>
        </div>
        <div className="bg-violet-500/5 border border-violet-500/10 rounded-xl p-4">
          <div className="text-[10px] font-mono uppercase tracking-widest text-violet-300/70 mb-2">Model</div>
          <div className="text-base font-bold text-violet-50 font-mono">counsel-os 0.6.2</div>
          <div className="text-[10px] text-violet-400/60 mt-1">Signed by Counsel Trust Authority</div>
        </div>
      </div>
          ...
          return (
            <div key={e.id} className="bg-[#0a0614] border border-violet-500/10 rounded-xl p-4">
              <div className="flex items-start gap-4">
                <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center shrink-0 border", k.bg, k.border)}>
                  <Icon className={cn("w-5 h-5", k.text)} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <span className={cn("px-2 py-0.5 rounded text-[10px] font-mono uppercase tracking-wider border", k.bg, k.border, k.text)}>
                      {k.label}
                    </span>
                  ...
                  <div className="text-sm font-semibold text-violet-50 leading-snug">{e.action}</div>
                  <div className="text-[11px] text-violet-300/60 mt-0.5">
                    Matter: {e.matter} · Actor: <span className="text-violet-200/80">{e.actor}</span>
                  </div>
                  <div className="mt-3 flex items-center gap-x-4 gap-y-1 flex-wrap text-[10px] font-mono text-violet-400/60">
                    <span className="flex items-center gap-1.5"><GitBranch className="w-3 h-3" /> {e.modelVersion}</span>
                    <span className="flex items-center gap-1.5"><Hash className="w-3 h-3" /> {e.proofHash}</span>
                    <span className="flex items-center gap-1.5"><FileText className="w-3 h-3" /> {e.evidenceCount} evidence refs</span>
                    <span className="flex items-center gap-1.5"><ShieldCheck className="w-3 h-3" /> {e.policyRefs.length} policy refs</span>
                  </div>
                </div>
                <span className={cn(
