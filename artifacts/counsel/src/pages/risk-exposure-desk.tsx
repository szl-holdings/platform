...
const DEADLINE_EVIDENCE: EvidenceSource[] = [
  {
    ...
    excerpt: "Discovery request from Morrison & Vance overdue by 18 days. 2 filing obligations for Meridian Compliance v.3 are blocked. Regulatory deadline in 11 days.",
  },
  {
    id: "ev-risk-002",
    ...
    excerpt: "If filing deadline missed: $4.1M exposure activates. Regulatory penalty modeled at $2.8M + $1.3M litigation exposure. Escalation reduces probability to 14%.",
  },
];
...
const BUDGET_EVIDENCE: EvidenceSource[] = [
  {
    ...
    excerpt: "Sterling & Ross YTD actual: $284K vs $408K budget. 30.4% under budget. Performance rating: 92/100 across 3 active matters. Recommend reallocation.",
  },
];
  ...
  return (
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-violet-400/60 text-sm">Financial exposure analysis and AI-driven risk mitigation.</p>
      </header>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-[#0a0614] border border-violet-500/10 rounded-xl p-5">
            <h3 className="text-sm font-semibold text-violet-100 mb-4">Financial Exposure by Type</h3>
              ...
              {[
                ...
                <div key={i} className="flex-1 flex flex-col items-center group">
                  <div className="relative w-full flex flex-col items-center">
                    <div className="absolute -top-6 text-[10px] font-bold text-violet-100">${bar.val}M</div>
                    <div className="w-full bg-violet-500/20 border border-violet-500/40 rounded-t-lg transition-all group-hover:bg-violet-500/40" 
                         style={{ height: `${(bar.val / 15) * 160}px` }} />
                  </div>
                  <span className="text-[10px] text-violet-400/60 mt-3 uppercase tracking-wider">{bar.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#0a0614] border border-violet-500/10 rounded-xl p-5">
            <h3 className="text-sm font-semibold text-violet-100 mb-4">What-If: Deadline Miss Scenarios</h3>
            <div className="space-y-3">
              {highRiskMatters.map(m => (
                <div key={m.id} className="p-4 rounded-lg bg-red-500/5 border border-red-500/10 flex items-center justify-between">
                  <div className="space-y-1">
                    <div className="text-xs font-bold text-violet-100">{m.name}</div>
                    <div className="text-[10px] text-red-400/60">Missed Deadline Risk: High Severity</div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-bold text-red-400">+${(m.exposureUsd * 0.4 / 1000000).toFixed(1)}M</div>
            ...
            >
              <div className="space-y-3">
                <p className="text-xs text-violet-200 leading-relaxed">
                  Looming deadline for <span className="text-violet-100 font-bold">Meridian Compliance v.3</span> (11 days) is blocked by 18-day overdue discovery response from Morrison & Vance.
                </p>
                <div className="space-y-2">
                  <div className="text-[10px] font-bold text-violet-400 uppercase tracking-widest">Recommended Action</div>
            ...
            >
              <div className="space-y-2">
                <p className="text-xs text-violet-200">
                  Sterling & Ross actual spend is 30% under budget while performance is &gt; 90%. Recommend shifting $50K from contingency to IP Settlement matter.
                </p>
