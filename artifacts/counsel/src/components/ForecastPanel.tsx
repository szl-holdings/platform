...
const COUNSEL_FORECAST_DATA: ForecastHead[] = [
  {
    ...
    provenance: { modelId: 'safe-default-counsel:deadline-slippage', modelVersion: '0.1.0', adapterId: 'safe-default', generatedAt: new Date().toISOString() },
    alertThreshold: 0.65,
  ...
  },
  {
    ...
    provenance: { modelId: 'safe-default-counsel:filing-defect', modelVersion: '0.1.0', adapterId: 'safe-default', generatedAt: new Date().toISOString() },
    alertThreshold: 0.5,
  },
  {
    ...
    provenance: { modelId: 'safe-default-counsel:recovery', modelVersion: '0.1.0', adapterId: 'safe-default', generatedAt: new Date().toISOString() },
  },
  {
    ...
    provenance: { modelId: 'safe-default-counsel:staffing-pressure', modelVersion: '0.1.0', adapterId: 'safe-default', generatedAt: new Date().toISOString() },
    alertThreshold: 0.7,
    thresholdBreached: true,
  },
];
...
export function ForecastPanel() {
  const breachedCount = COUNSEL_FORECAST_DATA.filter((h) => h.thresholdBreached).length;
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-mono text-muted-foreground">{COUNSEL_FORECAST_DATA.length} heads · safe-default adapter</span>
        </div>
      </div>
      <p className="text-xs text-muted-foreground">Calibrated forecast intervals for all Counsel legal matter heads. Click any card to expand all horizons and view provenance.</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {COUNSEL_FORECAST_DATA.map((head) => <HeadCard key={head.headName} head={head} />)}
      </div>
