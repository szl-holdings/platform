...
export function AlloyKernelPanel() {
  const kernel = useMemo(() => getKernel('ambient-capture-recap'), []);
  const [showExample, setShowExample] = useState(false);
  const example = kernel.codex.examples[0];

  return (
    <div className="rounded-xl border border-violet-500/15 bg-slate-950/60 p-5 mt-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-sm font-bold text-slate-100">Ambient Deposition Recap</div>
            <div className="text-[10px] text-slate-500 font-mono">
              Continuum Codex · {kernel.name} · v{kernel.version}
            </div>
          </div>
        </div>
        <span className="px-2 py-0.5 rounded-full bg-violet-500/10 border border-violet-500/20 text-[10px] text-violet-400 font-mono uppercase tracking-wider">
          {kernel.pattern}
        </span>
      </div>

      <div className="space-y-3 mb-4">
        <div className="rounded-lg bg-slate-900/60 border border-violet-500/10 p-3">
          <div className="text-[10px] text-slate-500 font-mono uppercase tracking-wider mb-1.5 flex items-center gap-1">
            <FileText className="w-3 h-3" /> Role
          </div>
      ...
      {showExample && example && (
        <div className="mt-3 rounded-lg bg-slate-950/80 border border-violet-500/10 p-3">
