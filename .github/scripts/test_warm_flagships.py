...
def attempt(number: int, status: int | None, *, error: str | None = None) -> warm.Attempt:
    return warm.Attempt(
        ...
        http_status=status,
        final_url="https://example.test/healthz" if status is not None else None,
        elapsed_ms=10,
        body_preview="ok" if status == 200 else "",
        error_class=error,
...
class ClassificationTests(unittest.TestCase):
    def test_repeated_200_with_final_200_is_healthy(self) -> None:
        self.assertEqual(
            warm.classify_attempts(
                [attempt(1, 200), attempt(2, 503), attempt(3, 200)]
            ),
            warm.HEALTHY,
        )
    ...
    def test_one_200_is_not_enough_to_close_an_incident(self) -> None:
        self.assertEqual(
            warm.classify_attempts(
                [attempt(1, None, error="URLError"), attempt(2, 200), attempt(3, None, error="TimeoutError")]
            ),
            warm.UNKNOWN,
        )
    ...
    def test_repeated_real_non_200_without_200_is_unhealthy(self) -> None:
        self.assertEqual(
            warm.classify_attempts(
                [attempt(1, 503), attempt(2, None, error="URLError"), attempt(3, 503)]
            ),
            warm.UNHEALTHY,
        )
    ...
    def test_network_only_failures_are_unknown(self) -> None:
        self.assertEqual(
            warm.classify_attempts(
                [
                    attempt(1, None, error="URLError"),
                    attempt(2, None, error="TimeoutError"),
                    attempt(3, None, error="OSError"),
                ]
            ),
            warm.UNKNOWN,
        )
...
class IssueLifecycleTests(unittest.TestCase):
    def test_create_only_for_confirmed_unhealthy_without_open_issue(self) -> None:
        self.assertEqual(
            warm.planned_issue_action(state=warm.UNHEALTHY, open_issue_number=None),
            "CREATE",
        )
        self.assertEqual(
            warm.planned_issue_action(state=warm.UNHEALTHY, open_issue_number=460),
            "NONE",
        )

    def test_close_only_for_confirmed_healthy_open_issue(self) -> None:
        self.assertEqual(
            warm.planned_issue_action(state=warm.HEALTHY, open_issue_number=460),
            "CLOSE",
        )
        self.assertEqual(
            warm.planned_issue_action(state=warm.HEALTHY, open_issue_number=None),
            "NONE",
        )

    def test_unknown_never_opens_or_closes(self) -> None:
        self.assertEqual(
            warm.planned_issue_action(state=warm.UNKNOWN, open_issue_number=None),
            "NONE",
        )
        self.assertEqual(
            warm.planned_issue_action(state=warm.UNKNOWN, open_issue_number=460),
            "NONE",
        )

    def test_issue_identity_remains_backward_compatible(self) -> None:
        self.assertEqual(
            warm.issue_title("a11oy"),
            "flagship a11oy unhealthy (warm-flagships)",
        )
        self.assertEqual(
            warm.issue_marker("a11oy"),
            "<!-- szl-flagship-health:a11oy -->",
        )
...
class ReportBoundaryTests(unittest.TestCase):
    ...
    def test_report_never_serializes_configured_credentials(self) -> None:
        ...
        with patch.object(warm, "probe_once", side_effect=responses), patch.object(
            warm.time, "sleep"
        ), patch.dict(
            os.environ,
            {
                "GITHUB_TOKEN": secret,
                "HF_TOKEN": "also-not-recorded",
                "GITHUB_SHA": "a" * 40,
    ...
    def test_workflow_separates_pr_read_only_from_issue_mutation(self) -> None:
        workflow = (ROOT / ".github/workflows/warm-flagships.yml").read_text(
            encoding="utf-8"
        )
