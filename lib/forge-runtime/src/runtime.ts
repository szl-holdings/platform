...
type ForgeTaskHandler = (execution: ForgeExecution) => Promise<{ result: unknown; costUsd?: number }>;
...
export class ForgeRuntime {
  ...
  async submit(task: ForgeTask): Promise<ForgeExecution> {
    ...
    return tracer.withSpan("alloy.workflow.start", async (span) => {
      ...
      const tenantAllowedDomains = tenantPolicy?.allowedDomains ?? [task.domain, "global"] as PrismDomain[];
      ...
      if (ac === "observe_only") {
        ...
        await prismBus.publish({
          ...
          sourceId: "forge-runtime",
          severity: "info",
          payload: { executionId, result: execution.result, note: "observe_only: no execution performed" },
          correlationId: task.correlationId,
          tenantId: task.tenantId,
          userId: task.userId,
        });
      ...
      if (ac === "propose_only") {
        ...
        await prismBus.publish({
          ...
          sourceId: "forge-runtime",
          severity: "info",
          payload: { executionId, result: execution.result, note: "propose_only: plan proposed, not executed" },
          correlationId: task.correlationId,
          tenantId: task.tenantId,
          userId: task.userId,
        });
  ...
  async approveAndRun(executionId: string, approvalId: string): Promise<ForgeExecution> {
    ...
    if (execution.status !== "awaiting_approval") {
      throw new Error(`FORGE: Execution ${executionId} is not awaiting approval (status: ${execution.status})`);
    }
    ...
    await tracer.withSpan("alloy.workflow.step.execute", async (span) => {
      ...
      if (tenantPolicy && execution.task.tenantId) {
        ...
        if (tenantActive >= tenantPolicy.maxConcurrentExecutions) {
          ...
          span.addEvent("alloy.workflow.concurrency_limit_exceeded", { executionId: execution.executionId });
          await prismBus.publish({
      ...
      try {
        ...
        if (durationViolation) {
          ...
          execution.latencyMs = execution.completedAt - execution.startedAt;
          span.setStatus("error", execution.error);
          span.addEvent("alloy.workflow.duration_exceeded", { executionId: execution.executionId, durationMs });
          return;
        }
  ...
  getStats() {
    ...
    const avgLatency =
      total > 0
        ? this.history.reduce((sum, e) => sum + (e.latencyMs ?? 0), 0) / total
        : 0;
