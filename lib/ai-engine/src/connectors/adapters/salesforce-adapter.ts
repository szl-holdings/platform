...
export class SalesforceConnectorAdapter extends BaseConnectorAdapter {
  ...
  async execute(toolName: string, input: Record<string, unknown>): Promise<unknown> {
    ...
    if (toolName === "query_soql") {
      ...
      const resp = await fetch(`${this.instanceUrl}/services/data/v57.0/query?${params}`, { headers });
    ...
    if (toolName === "create_record") {
      const resp = await fetch(`${this.instanceUrl}/services/data/v57.0/sobjects/${input.objectType}`, {
        method: "POST",
        headers,
    ...
    if (toolName === "update_record") {
      const resp = await fetch(`${this.instanceUrl}/services/data/v57.0/sobjects/${input.objectType}/${input.recordId}`, {
        method: "PATCH",
        headers,
        body: JSON.stringify(input.fields),
      });
