  ...
  tools: ConnectorToolDefinition[] = [
    {
      ...
      outputSchema: {
        type: 'object',
        properties: { id: { type: 'string' }, key: { type: 'string' }, url: { type: 'string' } },
      },
      costEstimate: 'free',
    },
    {
      ...
      outputSchema: {
        type: 'object',
        properties: { issues: { type: 'array' }, total: { type: 'number' } },
      },
      costEstimate: 'free',
    },
    {
    ...
    if (toolName === 'search_issues') {
      const resp = await fetch(`${this.baseUrl}/rest/api/3/search`, {
        ...
        body: JSON.stringify({
          ...
          fields: input.fields ?? ['summary', 'status', 'priority', 'assignee'],
        }),
      });
      return resp.json();
    }
