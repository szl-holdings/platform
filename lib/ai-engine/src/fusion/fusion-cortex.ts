...
export interface FusionPattern {
  ...
  confidence: number;
  detector: (connections: CrossDomainConnection[], entities: OntologyEntity[]) => FusionAlert | null;
}
...
const FUSION_PATTERNS: FusionPattern[] = [
  {
    ...
    detector: (connections, entities) => {
      const legalFinancial = connections.filter(c => (c.fromDomain === "prism-counsel" || c.toDomain === "prism-counsel") && (c.fromDomain === "szl-holdings" || c.toDomain === "szl-holdings" || c.fromDomain === "terra" || c.toDomain === "terra"));
      ...
      return {
        ...
        affectedEntities: [{ id: "fusion-entity-a", name: conn.entityA, domain: conn.fromDomain, type: "organization" }],
        evidenceChain: [
          { source: "PRISM Counsel", domain: "legal", description: `Active litigation: ${conn.entityA}`, timestamp: new Date().toISOString(), weight: 0.9 },
          { source: conn.toDomain, domain: conn.toDomain, description: `Financial exposure to ${conn.entityB}`, timestamp: new Date().toISOString(), weight: 0.8 },
        ],
        recommendedActions: [
  ...
  },
  {
    ...
    confidence: 0.92,
    detector: (connections, entities) => {
      const maritimeSecurity = connections.filter(c =>
      ...
      return {
        ...
        affectedEntities: [{ id: "fusion-vessel", name: conn.entityA, domain: "vessels", type: "vessel" }],
        evidenceChain: [
          { source: "Vessels Intelligence", domain: "maritime", description: `Vessel ${conn.entityA} in high-risk operational zone`, timestamp: new Date().toISOString(), weight: 0.95 },
          { source: "Aegis SOC", domain: "security", description: `Active threat intelligence correlated to vessel route`, timestamp: new Date().toISOString(), weight: 0.88 },
        ],
        recommendedActions: [
        ...
        advisoryContext: "CRITICAL CROSS-DOMAIN: Maritime and security intelligence streams have converged. This pattern matches historical APT maritime targeting. Immediate multi-domain response required.",
        generatedAt: new Date().toISOString(),
  ...
  },
  {
    ...
    confidence: 0.88,
    detector: (connections, entities) => {
      const terraLegal = connections.filter(c =>
      ...
      return {
        ...
        affectedEntities: [{ id: "fusion-property", name: conn.entityA, domain: "terra", type: "property" }],
        evidenceChain: [
          { source: "Terra Intelligence", domain: "real_estate", description: `Property ${conn.entityA} flagged with legal encumbrance`, timestamp: new Date().toISOString(), weight: 0.85 },
          { source: "PRISM Counsel", domain: "legal", description: `Active matter filed for ${conn.entityA}`, timestamp: new Date().toISOString(), weight: 0.90 },
          { source: "SZL Holdings", domain: "financial", description: `Investment position in ${conn.entityA} carries elevated risk`, timestamp: new Date().toISOString(), weight: 0.80 },
        ],
        recommendedActions: [
        ...
        advisoryContext: "Carlota Jo Advisory: Property, legal, and financial signals have converged on the same underlying asset. This warrants immediate multi-disciplinary review with client representation.",
        generatedAt: new Date().toISOString(),
        expiresAt: computeExpiryTime("high"),
        status: "active",
  ...
  },
  {
    ...
    confidence: 0.90,
    detector: (connections, entities) => {
      const multiHop = connections.filter(c => c.connectionType === "owns" || c.connectionType === "connected_to");
      ...
      return {
        ...
        affectedEntities: chain.map(c => ({ id: `fusion-${c.entityA}`, name: c.entityA, domain: c.fromDomain, type: "organization" })),
...
export class FusionCortex {
  ...
  getAlerts(options: {
    ...
    if (options.categories?.length) results = results.filter(a => options.categories!.includes(a.category));
  ...
  injectAlert(alert: Omit<FusionAlert, "id" | "generatedAt" | "expiresAt" | "status">): FusionAlert {
  ...
  getStats(): FusionCortexStats {
    ...
    const alertsBySeverity: Record<FusionAlertSeverity, number> = { low: 0, medium: 0, high: 0, critical: 0 };
    const alertsByCategory: Record<FusionAlertCategory, number> = {
      cross_domain_risk: 0, entity_correlation: 0, pattern_anomaly: 0,
      sanctions_exposure: 0, litigation_impact: 0, financial_stress: 0,
      threat_escalation: 0, opportunity_signal: 0,
    };
    ...
    const avgConfidence = this.alerts.length > 0
    ...
    const demoAlerts: Array<Omit<FusionAlert, "id" | "generatedAt" | "expiresAt" | "status">> = [
      {
        ...
        affectedEntities: [
          ...
          { id: "demo-2", name: "Meridian Capital LLC", domain: "prism-counsel", type: "organization" },
          { id: "demo-3", name: "345 Atlantic Ave, Brooklyn", domain: "terra", type: "property" },
        ],
        evidenceChain: [
          { source: "Vessels Intelligence", domain: "maritime", description: "AURORA registered to Meridian Capital LLC (BVI)", timestamp: new Date(Date.now() - 86400000).toISOString(), weight: 0.9 },
          { source: "PRISM Counsel", domain: "legal", description: "Meridian Capital LLC — new litigation filing, SDNY, $12M exposure", timestamp: new Date(Date.now() - 43200000).toISOString(), weight: 0.95 },
          { source: "Terra Intelligence", domain: "real_estate", description: "345 Atlantic Ave valuation −18% over 30 days, Meridian Capital tenant-in-common", timestamp: new Date(Date.now() - 21600000).toISOString(), weight: 0.85 },
        ],
        recommendedActions: [
      ...
      },
      {
        ...
        evidenceChain: [
          { source: "Aegis SOC", domain: "security", description: "APT41 IOCs detected on South China Sea maritime infrastructure (T1590 - Gather Victim Network Info)", timestamp: new Date(Date.now() - 7200000).toISOString(), weight: 0.92 },
          { source: "Vessels Intelligence", domain: "maritime", description: "ATLAS WIND, MERIDIAN STAR, TYPHOON PEAK transiting Strait of Malacca", timestamp: new Date(Date.now() - 3600000).toISOString(), weight: 0.88 },
        ],
        recommendedActions: [
      ...
      },
      {
        ...
        affectedEntities: [
          { id: "demo-6", name: "Pacific Ventures Holdings BVI", domain: "szl-holdings", type: "organization" },
          { id: "demo-7", name: "SDN Entity (Redacted)", domain: "prism-counsel", type: "organization" },
        ],
        evidenceChain: [
