...
export function createAlloyDecision(
  params: Partial<AlloyDecision> & {
    recommendedAction: string;
    rationaleSummary: string;
    riskLevel: RiskLevel;
    modelRoute: string;
...
export function validateAlloyDecision(raw: unknown): {
  valid: boolean;
  decision: AlloyDecision | null;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (typeof obj.recommendedAction !== 'string' || !obj.recommendedAction)
    errors.push('Missing recommendedAction');
  if (typeof obj.rationaleSummary !== 'string' || !obj.rationaleSummary)
    errors.push('Missing rationaleSummary');
  if (!['P0', 'P1', 'P2', 'P3', 'P4'].includes(obj.riskLevel as string))
    errors.push('Invalid riskLevel (must be P0-P4)');
  if (typeof obj.confidence !== 'number' || obj.confidence < 0 || obj.confidence > 1)
    errors.push('Invalid confidence (0-1)');
...
export const APPROVAL_MATRIX: Record<
  RiskLevel,
  { requiresApproval: boolean; approverRole: string; sla: string; autoExpireHours: number }
> = {
