...
export const ACTION_DECISION_SCHEMA = {
  ...
  properties: {
    ...
    actionType: {
      type: 'string',
      enum: ['approve', 'escalate', 'defer', 'route', 'close', 'investigate'],
    },
...
export function validateActionDecision(raw: unknown): {
  valid: boolean;
  ...
  if (
    !['approve', 'escalate', 'defer', 'route', 'close', 'investigate'].includes(
      obj.actionType as string,
    )
  )
