...
const DOMAIN_EXPLANATIONS: Record<string, (ctx: FeatureExplanationContext) => string> = {
  ...
  terra: (ctx) => {
    ...
    if (ctx.modelType === "distress_classifier") {
      return `The distress signal is ${ctx.topFeatures[0]?.direction === "positive" ? "elevated" : "low"} based on ${ctx.topFeatures.slice(0, 2).map(f => f.featureName).join(" and ")}. Extended time on market and price reductions are the strongest distress indicators in this model.`;
    }
  ...
  aegis: (ctx) => {
    ...
    if (ctx.modelType === "threat_anomaly_detector") {
      return `Anomaly detected based on ${ctx.topFeatures.slice(0, 3).map(f => f.featureName).join(", ")}. The primary driver is ${top?.featureName ?? "behavioral deviation"} showing ${top?.direction === "positive" ? "elevated" : "suppressed"} activity vs established baseline. Recommend immediate SOC review.`;
    }
    if (ctx.modelType === "threat_severity_scorer") {
      return `Threat severity score of ${ctx.prediction} driven by ${ctx.topFeatures.slice(0, 2).map(f => f.featureName).join(" and ")}. IOC matches and privilege escalation events carry highest weight in this assessment.`;
    }
  ...
  szl: (ctx) => {
    ...
    if (ctx.modelType === "deal_quality_scorer") {
      return `Deal quality score of ${ctx.prediction}/100 is primarily driven by ${ctx.topFeatures.slice(0, 2).map(f => f.featureName).join(" and ")}. Revenue growth trajectory and unit economics are the strongest positive signals. Burn multiple has the largest constraining effect.`;
    }
    if (ctx.modelType === "portfolio_health_scorer") {
      return `Portfolio health score driven by ${ctx.topFeatures.slice(0, 3).map(f => f.featureName).join(", ")}. NDR and gross margin are the strongest forward-looking indicators in this model.`;
    }
  ...
  lyte: (ctx) => {
    ...
    if (ctx.modelType === "capacity_demand_forecast") {
      return `Capacity demand forecast is driven by ${ctx.topFeatures.slice(0, 2).map(f => f.featureName).join(" and ")}. Recent CPU utilisation trend and deployment frequency are the strongest predictors for short-term capacity requirements.`;
    }
...
export function computeShapExplanation(
  ...
  if (typeof prediction === "number") {
    ...
    predictedNumeric = typeof pred["probability"] === "number" ? pred["probability"]
      : typeof pred["score"] === "number" ? pred["score"]
      : 0.5;
  }
  ...
  const allContributions: ShapValue[] = importanceEntries.map(([featureId, importance]) => {
    ...
    const contribution = (importance / Math.max(totalImportance, 0.001)) * (predictedNumeric - baseValue);
