...
export interface MonitoringSnapshot {
  ...
  predictionDistribution: { mean: number; std: number; min: number; max: number; p50: number; p95: number } | null;
...
function simulateKsDrift(featureId: string): FeatureDriftScore {
  ...
  return { featureId, ksDriftScore: parseFloat(ks.toFixed(4)), psiScore: parseFloat(psi.toFixed(4)), status };
}

function simulateAccuracyMetrics(model: ReturnType<typeof mlModelRegistry.getModel>, predictionCount: number): AccuracyMetrics | null {
  ...
  return {
    ...
    accuracy: baseMetrics.accuracy !== undefined ? Math.max(0, Math.min(1, baseMetrics.accuracy + degradation)) : undefined,
    f1: baseMetrics.f1 !== undefined ? Math.max(0, Math.min(1, baseMetrics.f1 + degradation)) : undefined,
    mse: baseMetrics.mse !== undefined ? Math.max(0, baseMetrics.mse + Math.abs(degradation) * 0.5) : undefined,
    r2: baseMetrics.r2 !== undefined ? Math.max(-1, Math.min(1, baseMetrics.r2 + degradation)) : undefined,
    mape: baseMetrics.mape !== undefined ? Math.max(0, baseMetrics.mape + Math.abs(degradation)) : undefined,
  };
}

function isPerformanceDegraded(baseMetrics: Record<string, number | undefined>, currentMetrics: AccuracyMetrics, threshold = 0.1): boolean {
...
export async function runMonitoringCycle(modelVersionId: string, windowHours = 24): Promise<MonitoringSnapshot> {
  ...
  const performanceDegraded = accuracyMetrics
    ? isPerformanceDegraded(model.testMetrics as Record<string, number | undefined>, accuracyMetrics)
    : false;
  ...
  const std = Math.sqrt(numericPredictions.reduce((s, v) => s + (v - mean) ** 2, 0) / predictionCount);

  let retrainingTriggered = false;
  const runIds: string[] = [];

  if (driftDetected || performanceDegraded) {
    const reason = driftDetected && performanceDegraded
      ? "Data drift and performance degradation detected"
      : driftDetected ? "Significant data drift detected"
      : "Model performance degradation detected";

    logger.warn({ modelVersionId, domain: model.domain, reason }, "Automatic retraining triggered by monitoring");
  ...
  logger.info({ snapshotId: snapshot.snapshotId, domain: model.domain, driftDetected, performanceDegraded, retrainingTriggered }, "Monitoring snapshot recorded");
  return snapshot;
}
...
export async function runMonitoringForAllProductionModels(): Promise<MonitoringSnapshot[]> {
  ...
  for (const prod of summary.productionModels) {
    try {
      ...
      logger.error({ modelVersionId: prod.modelVersionId, error: String(err) }, "Monitoring cycle failed");
    }
  }
