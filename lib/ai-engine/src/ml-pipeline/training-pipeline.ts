...
function simulateMetrics(
  algorithmFamily: string,
  dataSize: number,
): { train: TrainingMetrics; val: TrainingMetrics; test: TrainingMetrics } {
  const baseAcc = 0.78 + Math.random() * 0.15;
  const noise = () => (Math.random() - 0.5) * 0.04;

  const isRegression = ['linear_regression', 'gradient_boosted_regression', 'time_series'].includes(
    algorithmFamily,
  );
  ...
  if (isRegression) {
    ...
    return {
      train: {
        ...
        mse: 0.014 + noise(),
        mape: 0.08 + noise(),
        sampleCount: Math.floor(dataSize * 0.8),
      },
      val: {
        r2: r2 + 0.02,
        rmse: 0.14 + noise(),
        ...
        mse: 0.02 + noise(),
        mape: 0.1 + noise(),
        sampleCount: Math.floor(dataSize * 0.1),
      },
      test: {
        r2,
        rmse: 0.15 + noise(),
  ...
  return {
    train: {
      accuracy: baseAcc + 0.05,
      precision: baseAcc + 0.03 + noise(),
      recall: baseAcc - 0.02 + noise(),
      f1: baseAcc + 0.01 + noise(),
      auc: baseAcc + 0.08 + noise(),
      logLoss: 0.35 + noise(),
      sampleCount: Math.floor(dataSize * 0.8),
    },
    val: {
      accuracy: baseAcc + 0.01,
      precision: baseAcc + noise(),
      recall: baseAcc - 0.04 + noise(),
      f1: baseAcc - 0.02 + noise(),
      auc: baseAcc + 0.05 + noise(),
      logLoss: 0.39 + noise(),
      sampleCount: Math.floor(dataSize * 0.1),
    },
    test: {
      accuracy: baseAcc,
      precision: baseAcc - 0.01 + noise(),
      recall: baseAcc - 0.05 + noise(),
      f1: baseAcc - 0.03 + noise(),
      auc: baseAcc + 0.04 + noise(),
      logLoss: 0.41 + noise(),
      sampleCount: Math.floor(dataSize * 0.1),
    },
  };
...
function simulateFeatureImportance(featureIds: string[]): Record<string, number> {
  ...
  raw.forEach((r) => {
    result[r.f] = parseFloat((r.w / total).toFixed(4));
  });
  return result;
}
...
export async function startTrainingRun(config: TrainingRunConfig): Promise<TrainingRun> {
  ...
  logger.info(
    { runId, domain: config.domain, algorithm: config.algorithmFamily },
  ...
  try {
    ...
    const template = DOMAIN_MODEL_TEMPLATES[config.domain]?.find(
      (t) => t.modelType === config.modelType,
    );
    const modelName = template
      ? `${config.domain}-${config.modelType}`
      : `${config.domain}-${config.algorithmFamily}`;
...
export async function triggerDomainTraining(
  domain: string,
  triggeredBy = 'auto',
): Promise<TrainingRun[]> {
...
export function getTrainingPipelineSummary() {
  ...
  return {
    ...
    byDomain: Object.fromEntries(
      [...new Set(runs.map((r) => r.domain))].map((d) => [
        d,
        runs.filter((r) => r.domain === d).length,
      ]),
    ),
  };
}
