...
function runQualityValidation(dataset: ManagedDataset): DataQualityReport {
  ...
  if (outliers > dataset.rowCount * 0.02) issues.push(`${outliers} outliers detected (>${((outliers / dataset.rowCount) * 100).toFixed(1)}% of rows)`);

  const highCorrelations = Object.entries(correlations)
    .filter(([, v]) => v > 0.95)
    .map(([k]) => k);
  if (highCorrelations.length > 0) issues.push(`Near-perfect correlation in: ${highCorrelations.join(", ")}`);

  const qualityScore = Math.max(0, Math.min(100, Math.round(100 - missing * 200 - duplicates * 100 - (outliers / dataset.rowCount) * 50)));
...
function computeBiasMetrics(featureIds: string[]): BiasMetrics {
  ...
  for (const g of groups) {
    ...
    equalizedOdds[g] = parseFloat((0.80 + Math.random() * 0.18).toFixed(4));
    calibrationByGroup[g] = parseFloat((0.88 + Math.random() * 0.10).toFixed(4));
  }
...
export async function createDataset(input: CreateDatasetInput): Promise<ManagedDataset> {
  ...
  if (dataset.qualityReport.passed) {
    ...
    logger.info({ datasetId, qualityScore: dataset.qualityReport.dataQualityScore }, "Dataset validation passed");
  } else {
...
export function getDatasetSummary() {
  ...
  return {
    ...
    avgQualityScore: datasets.length > 0
      ? parseFloat((datasets.reduce((s, d) => s + (d.qualityReport?.dataQualityScore ?? 0), 0) / datasets.length).toFixed(1))
      : 0,
  };
}
...
export async function bootstrapDomainDatasets(): Promise<ManagedDataset[]> {
  const domainConfigs: Array<{ domain: string; labelColumn: string; featureIds: string[] }> = [
    { domain: "vessels", labelColumn: "failure_within_30d", featureIds: ["vessels.fuel_consumption_7d_avg", "vessels.speed_variance", "vessels.days_since_last_maintenance", "vessels.route_deviation_score", "vessels.cargo_utilisation"] },
    { domain: "terra", labelColumn: "sale_price_usd", featureIds: ["terra.price_per_sqft", "terra.neighborhood_cap_rate", "terra.days_on_market", "terra.walk_score", "terra.vacancy_rate_submarket"] },
    { domain: "prism", labelColumn: "case_outcome", featureIds: ["prism.case_age_days", "prism.opposing_counsel_win_rate", "prism.motion_grant_rate_judge", "prism.settlement_demand_to_claimed_ratio"] },
    { domain: "aegis", labelColumn: "is_anomaly", featureIds: ["aegis.failed_auth_rate_1h", "aegis.lateral_movement_score", "aegis.data_exfil_bytes_delta", "aegis.privilege_escalation_events", "aegis.ioc_match_count"] },
    { domain: "szl", labelColumn: "deal_quality_score", featureIds: ["szl.revenue_growth_yoy", "szl.gross_margin", "szl.burn_multiple", "szl.ndr", "szl.months_runway"] },
    { domain: "lyte", labelColumn: "sla_breach_1h", featureIds: ["lyte.p99_latency_ms", "lyte.error_rate_pct", "lyte.cpu_utilisation_avg", "lyte.incident_rate_7d", "lyte.slo_compliance_pct"] },
  ];
