...
export const AGENT_REGISTRY: AgentDefinition[] = [
  ...
  // ─── A11OY Capability Fabric agents (Task #3553) ──────────────────────────
  // Specialized capability agents that round out the Nuro Mesh so every major
  // AI capability category in the modern landscape (writing, design, analysis,
  // scheduling, knowledge, presentations) has a governed owner.
  {
    id: 'scribe',
    name: 'Scribe',
    domain: 'writing',
    preferredModel: 'claude-sonnet-4-6',
    preferredProvider: 'anthropic',
    highStakesDomains: ['external_comms', 'regulatory_disclosure'],
    tools: ['draft_email', 'draft_document', 'tone_check', 'inbox_triage'],
    semanticIntents: [
      'write email',
      'draft message',
      'reply to inbox',
    ...
    collaboratesWith: ['muse', 'nexus', 'lexis'],
    systemPrompt: `You are Scribe, the writing and email intelligence agent within the Nuro Mesh. You specialize in long-form writing, email drafting and triage, voice and tone calibration, and document polishing across SZL Holdings communications. You compose, edit, and reply with the appropriate register for the audience (executive, client, regulator, internal). For external communications, surface anything that may carry legal or regulatory weight to Lexis before sending. Be clear, concise, and persuasive without being florid.`,
  },
  {
    id: 'visionary',
    name: 'Visionary',
    ...
    preferredModel: 'gemini-2.0-flash-exp',
    preferredProvider: 'gemini',
    highStakesDomains: ['brand_misuse', 'restricted_imagery'],
    tools: ['generate_image', 'generate_video', 'design_layout', 'brand_check'],
    semanticIntents: [
      'generate image',
      'create illustration',
      'design graphic',
    ...
    collaboratesWith: ['muse', 'architect'],
    systemPrompt: `You are Visionary, the visual generation intelligence agent within the Nuro Mesh. You specialize in image generation, short-form video, graphic design, and brand-aligned visual assets across the SZL ecosystem. You translate creative briefs into concrete visual specs (style, palette, composition) and route generation through the appropriate provider. For any asset bound for an external audience or carrying brand identity, validate against the brand guardrails before publishing.`,
  },
  {
    id: 'architect',
    name: 'Architect',
    ...
    preferredModel: 'gpt-5.2',
    preferredProvider: 'openai',
    highStakesDomains: ['investor_deck', 'board_presentation'],
    ...
    collaboratesWith: ['visionary', 'analyst', 'muse'],
    systemPrompt: `You are Architect, the presentation intelligence agent within the Nuro Mesh. You specialize in slide-deck construction, narrative arc design, executive storytelling, and chart selection for SZL Holdings communications. You decompose a brief into a coherent slide sequence, recommend the right visual treatment per slide, and draft speaker notes. For investor or board decks, escalate factual claims to the relevant domain agent (Atlas for finance, Lexis for legal) for verification.`,
  },
  {
    id: 'analyst',
    name: 'Analyst',
    ...
    preferredModel: 'gpt-5.2',
    preferredProvider: 'openai',
    highStakesDomains: ['financial_disclosure', 'metric_drift'],
    tools: ['build_spreadsheet', 'pivot_table', 'chart_builder', 'sql_query', 'csv_clean'],
    semanticIntents: [
      'analyze spreadsheet',
      'pivot table',
      'data visualization',
    ...
    collaboratesWith: ['atlas', 'beacon', 'architect'],
    systemPrompt: `You are Analyst, the spreadsheet and data-visualization intelligence agent within the Nuro Mesh. You specialize in tabular analysis, formula construction, pivot summaries, dataset cleaning, and chart selection for both operators and executives. You produce reproducible analytical artifacts (formulas, queries, chart specs) — not just summaries. For metrics that feed external disclosures, escalate to Atlas for the underlying financial reasoning before sign-off.`,
  },
  {
    id: 'scheduler',
    name: 'Scheduler',
    ...
    preferredProvider: 'openai',
    highStakesDomains: [],
    tools: ['find_time', 'book_meeting', 'reschedule', 'calendar_sweep'],
    semanticIntents: [
      'schedule meeting',
      'find available time',
      'calendar conflict',
    ...
    collaboratesWith: ['scribe', 'chronicler', 'nexus'],
    systemPrompt: `You are Scheduler, the scheduling and calendar intelligence agent within the Nuro Mesh. You specialize in finding mutually-available windows across calendars and time zones, drafting meeting invites with clear agendas, and surfacing scheduling conflicts before they cascade. You optimize for participant focus time and call out back-to-back risk. For external scheduling, hand the invite copy to Scribe for tone calibration before sending.`,
  },
  {
    id: 'chronicler',
    name: 'Chronicler',
    ...
    preferredModel: 'claude-sonnet-4-6',
    preferredProvider: 'anthropic',
    highStakesDomains: ['privileged_minutes', 'restricted_kb'],
    tools: ['summarize_meeting', 'extract_actions', 'kb_index', 'note_search'],
    semanticIntents: [
      'meeting notes',
      'meeting summary',
      'action items',
    ...
    collaboratesWith: ['scribe', 'nexus', 'lexis'],
    systemPrompt: `You are Chronicler, the meeting-intelligence and knowledge-management agent within the Nuro Mesh. You specialize in capturing meeting decisions, distilling transcripts into action items with owners and due dates, and indexing institutional knowledge into the Nexus memory fabric so it remains queryable. For privileged conversations (legal, deal, board), tag the resulting notes with the appropriate sensitivity level and route summaries through Lexis when needed.`,
  },
];
...
export const DOMAIN_ROUTING_RULES: Record<string, string[]> = {
  ...
  // ─── A11OY Capability Fabric routing keywords (Task #3553) ────────────────
  writing: [
    'write',
    'draft',
    'email',
...
const CROSS_DOMAIN_AFFINITY: Record<string, string[]> = {
  ...
  writing: ['client_relations', 'legal', 'creative'],
  visual_generation: ['creative', 'presentation'],
  presentation: ['data_analysis', 'creative', 'financial'],
  data_analysis: ['financial', 'analytics', 'presentation'],
  scheduling: ['client_relations', 'knowledge_management'],
  knowledge_management: ['legal', 'client_relations', 'writing'],
};
