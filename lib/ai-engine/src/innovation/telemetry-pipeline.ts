...
export async function persistTelemetry(telemetry: OrchestrationTelemetry): Promise<void> {
  try {
    ...
    await db.insert(orchestrationTelemetryTable).values({
      ...
      routingScores: (telemetry.routingDecision.routingScores ?? null) as unknown as Record<
        string,
        unknown
      > | null,
...
export async function queryTelemetry(options: TelemetryQueryOptions = {}) {
  try {
    ...
    if (options.since) {
      conditions.push(
        gte(orchestrationTelemetryTable.timestamp, options.since) as unknown as ReturnType<
          typeof sql
        >,
      );
    }
