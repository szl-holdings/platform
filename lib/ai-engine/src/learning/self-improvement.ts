...
export async function computeAgentPerformanceSnapshot(agentId: string): Promise<AgentPerformanceSnapshot> {
  ...
  try {
    ...
    `, [agentId, period, total, accepted, rejected, overridden, avgConf, calibrationBias, accuracyScore, confidenceTrend, flaggedForReview, reviewReason]);
...
export async function runSelfReflection(agentId: string): Promise<SelfReflectionOutput> {
  ...
  if (snapshot.totalDecisions === 0) {
    ...
    observations.push(`Processed ${snapshot.totalDecisions} decisions with ${Math.round(snapshot.accuracyScore * 100)}% acceptance rate`);
    if (snapshot.confidenceTrend === "improving") observations.push("Accuracy is trending upward — reasoning strategies are effective");
    if (snapshot.confidenceTrend === "declining") observations.push("Accuracy is declining — current reasoning approach may need adjustment");
    if (Math.abs(snapshot.calibrationBias) > 0.15) {
      observations.push(`Confidence calibration is off by ${Math.round(snapshot.calibrationBias * 100)}% — overconfident in assessments`);
    }
  }

  if (snapshot.calibrationBias > 0.15) {
    confidenceAdjustment = -Math.round(snapshot.calibrationBias * 50);
    recommendations.push(`Reduce reported confidence scores by approximately ${Math.abs(confidenceAdjustment)}% to improve calibration`);
  } else if (snapshot.calibrationBias < -0.15) {
    confidenceAdjustment = Math.round(Math.abs(snapshot.calibrationBias) * 50);
    recommendations.push(`Increase confidence scores by approximately ${confidenceAdjustment}% — you are consistently underconfident`);
  }

  if (snapshot.rejectedDecisions > snapshot.acceptedDecisions * 0.3) {
    recommendations.push("High rejection rate — gather more evidence before making recommendations");
    recommendations.push("Consider requesting maker-checker validation for borderline cases");
  }

  if (snapshot.overriddenDecisions > 5) {
    recommendations.push("Significant number of overrides detected — review edge cases where reasoning diverges from operator judgment");
  }
  ...
  try {
    ...
    `, [agentId, snapshot.period, JSON.stringify(observations), JSON.stringify(recommendations), confidenceAdjustment, snapshot.flaggedForReview, snapshot.reviewReason, performanceScore]);
  } catch {}
...
export async function getAllAgentPerformanceSnapshots(): Promise<AgentPerformanceSnapshot[]> {
  ...
  try {
    ...
    return result.rows.map((r: Record<string, unknown>) => ({
      ...
      confidenceTrend: (r.confidence_trend as AgentPerformanceSnapshot["confidenceTrend"]) ?? "stable",
