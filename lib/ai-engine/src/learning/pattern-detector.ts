...
import { agentMemoryFacts } from "@szl-holdings/db";
...
const ATTACK_VECTOR_KEYWORDS: Record<string, string[]> = {
  ...
  data_exfiltration: ["exfil", "exfiltrat", "steal", "upload", "c2", "command_control", "data_loss"],
...
function extractDimensions(entry: CaseMemoryEntry): Record<string, string[]> {
  ...
  const caseText = entry.decisions.map(d => `${d.decisionType} ${d.summary} ${d.recommendedAction} ${d.gapsAndUnknowns.join(" ")}`).join(" ");
...
export async function detectCrossPatterns(
  ...
  for (const [dimension, values] of Object.entries(dimensionCounts)) {
    for (const [value, { count, cases }] of Object.entries(values)) {
      ...
      const significance: PatternCandidate["significance"] = ratio >= 0.7 ? "high" : ratio >= 0.5 ? "medium" : "low";
...
export async function runPatternDetectionAndStore(
  allCases: CaseMemoryEntry[],
): Promise<void> {
  try {
    ...
    console.log(`[pattern-detector] Stored ${patterns.length} pattern alerts (high: ${patterns.filter(p => p.significance === "high").length})`);
  } catch (err) {
