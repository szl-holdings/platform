...
import { alloyOutcomeLearning } from "@szl-holdings/db";
...
export async function recordOutcome(record: OutcomeRecord & { orgId?: number | null }): Promise<void> {
  try {
    const keywords = record.topicKeywords ?? extractKeywords(`${record.topic} ${record.originalAction}`);
...
export async function getRelevantOutcomes(
  ...
  try {
    ...
    const lines = scored.map(({ record: r }) => {
      const prefix = r.outcome === "accepted" ? "✓ ACCEPTED" :
        r.outcome === "rejected" ? "✗ REJECTED" :
        r.outcome === "overridden" ? "△ OVERRIDDEN" : "⏸ DEFERRED";
...
export async function getConfidenceCalibration(agentId: string): Promise<ConfidenceCalibration> {
  try {
    ...
    for (const row of rows) {
      total += row.count;
      if (row.outcome === "accepted") { accepted = row.count; avgReportedConfidence += row.avgConfidence * row.count; confWeightedTotal += row.count; }
    ...
    logger.warn({ err }, "getConfidenceCalibration DB read failed — returning baseline calibration");
    return {
      agentId,
      totalDecisions: 0,
      acceptedCount: 0,
