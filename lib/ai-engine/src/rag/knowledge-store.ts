...
export async function ingestBatch(
  docs: Omit<KnowledgeDocument, 'createdAt' | 'updatedAt'>[],
): Promise<void> {
...
async function vectorRetrieve(
  ...
  return result.rows.map((r: Record<string, unknown>) => {
    ...
    const snippet =
      content.slice(0, 300).replace(/\s+/g, ' ').trim() + (content.length > 300 ? '...' : '');
    return {
...
async function fallbackKeywordSearch(
  ...
  try {
    ...
    const likeConditions = queryWords
      .slice(0, 3)
      .map((_, i) => `(LOWER(content) LIKE $${i + 2} OR LOWER(title) LIKE $${i + 2})`)
      .join(' OR ');
    if (!likeConditions) return [];

    const values: unknown[] = [
      options.domain ?? null,
      ...queryWords.slice(0, 3).map((w) => `%${w}%`),
    ];
...
export async function getKnowledgeStoreStats(): Promise<{
  ...
  try {
    const [totalResult, domainResult, sourceResult, vectorResult] = await Promise.all([
      ...
      pool.query(
        'SELECT domain, COUNT(*)::int as count FROM rag_knowledge_documents GROUP BY domain',
      ),
      pool.query(
        'SELECT source_type, COUNT(*)::int as count FROM rag_knowledge_documents GROUP BY source_type',
      ),
      pool
        .query(
