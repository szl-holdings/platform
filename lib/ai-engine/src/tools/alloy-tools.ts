...
export type ExecutionMode = "observe_only" | "propose_only" | "approval_required" | "approved_execute";
...
export const ALLOY_TOOL_DEFINITIONS: HFToolDef[] = [
  {
    ...
    function: {
      ...
      parameters: { type: "object", required: ["workflowId"], properties: { workflowId: { type: "string" } } },
    },
  },
  {
    type: "function",
    function: {
      ...
      parameters: { type: "object", required: ["signalId"], properties: { signalId: { type: "string" } } },
    },
  },
  {
    type: "function",
    function: {
      name: "lookup_owner",
      description: "Look up an operator or analyst by ID to get their role, permissions, and assignment history.",
      parameters: { type: "object", required: ["ownerId"], properties: { ownerId: { type: "string" } } },
    },
  },
  {
    type: "function",
    function: {
      name: "lookup_case",
      description: "Look up an incident case by ID. Returns case details, status, assigned owner, and linked alerts.",
      parameters: { type: "object", required: ["caseId"], properties: { caseId: { type: "string" } } },
    },
  },
  {
    type: "function",
    function: {
      name: "notify_team",
      description: "Send a notification to a team or operator via the configured notification channel.",
      parameters: {
        type: "object",
  ...
  },
  {
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          caseId: { type: "string" },
          status: { type: "string", enum: ["open", "in_progress", "pending_review", "resolved", "closed"] },
          assignTo: { type: "string" },
          notes: { type: "string" },
        },
      },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Reopen a previously closed Alloy workflow when new evidence or signals require re-investigation.",
      parameters: {
        type: "object",
  ...
  },
  {
    ...
    function: {
      ...
      description: "HIGH-RISK: Execute a containment step — isolate a host, block an IP, or quarantine a user. Requires approval in approval_required mode.",
      parameters: {
        type: "object",
        required: ["containmentType", "targetId"],
        properties: {
          containmentType: { type: "string", enum: ["isolate_host", "block_ip", "quarantine_user", "disable_account", "revoke_token"] },
  ...
  },
  {
    ...
    function: {
      name: "recovery_step",
      description: "HIGH-RISK: Execute a recovery step — restore host, unblock IP, or re-enable user. Requires approval in approval_required mode.",
      parameters: {
        type: "object",
        required: ["recoveryType", "targetId"],
        properties: {
          recoveryType: { type: "string", enum: ["restore_host", "unblock_ip", "reenable_user", "restore_token"] },
          targetId: { type: "string" },
          approvalRef: { type: "string", description: "Reference to the prior approval decision ID" },
          caseId: { type: "string" },
        },
      },
    },
  },
  {
    ...
    function: {
      ...
      description: "Generate an executive-level security brief for a case or incident for C-suite review.",
      parameters: {
        type: "object",
...
export function getExecutionMode(): ExecutionMode {
  ...
  if (raw === "observe_only" || raw === "propose_only" || raw === "approval_required" || raw === "approved_execute") {
    return raw;
  }
  return "propose_only";
}
...
export function checkToolPolicy(toolName: string, _args: Record<string, unknown>): {
  allowed: boolean;
  ...
  if (mode === "observe_only") {
    ...
    return { allowed: true, requiresApproval: false, reason: "observe_only: read-only tool permitted", blockedReason: null };
  }
  ...
  if (mode === "propose_only") {
    ...
    return { allowed: true, requiresApproval: false, reason: "propose_only: safe tool permitted", blockedReason: null };
  }
  ...
  if (mode === "approval_required") {
    ...
    return { allowed: true, requiresApproval: false, reason: "approval_required: standard tool permitted", blockedReason: null };
  }

  return { allowed: true, requiresApproval: false, reason: "approved_execute: all tools permitted", blockedReason: null };
}
...
async function writeAuditLog(
  ...
  extra?: { relatedDecisionId?: string; relatedCaseId?: string; relatedIncidentId?: string; tenantId?: string },
...
export async function executeToolCall(
  ...
  context?: { tenantId?: string; relatedDecisionId?: string; relatedCaseId?: string; relatedIncidentId?: string },
  ...
  try {
    ...
    if (auditOrExecErr instanceof Error && auditOrExecErr.message.startsWith("Audit write failed")) {
      return {
        toolName,
        success: false,
        output: {
    ...
    auditEntry.error = auditOrExecErr instanceof Error ? auditOrExecErr.message : String(auditOrExecErr);
...
async function executeToolInternal(toolName: string, args: Record<string, unknown>): Promise<unknown> {
  switch (toolName) {
    ...
    case "lookup_case":
      return { id: args["caseId"], title: `Case ${args["caseId"]}`, status: "open", severity: "high", assignedTo: null };
    case "notify_team":
      return { notified: true, channel: args["channel"], message: args["message"], sentAt: new Date().toISOString() };
    case "create_case":
      return { id: `case-${Date.now()}`, created: true, ...args, status: "open" };
    case "update_case":
      return { updated: true, caseId: args["caseId"], changes: args };
    case "close_case":
      return { closed: true, caseId: args["caseId"], resolution: args["resolution"], closedAt: new Date().toISOString() };
    case "create_action_item":
      return { id: `action-${Date.now()}`, created: true, ...args, status: "open" };
    case "assign_owner":
      return { assigned: true, entityType: args["entityType"], entityId: args["entityId"], newOwner: args["newOwner"] };
    case "open_workflow":
    ...
    case "reopen_workflow":
      return { reopened: true, workflowId: args["workflowId"], reason: args["reason"], reopenedAt: new Date().toISOString() };
    case "containment_step":
      return { executed: true, containmentType: args["containmentType"], targetId: args["targetId"], executedAt: new Date().toISOString() };
    case "recovery_step":
      return { executed: true, recoveryType: args["recoveryType"], targetId: args["targetId"], executedAt: new Date().toISOString() };
    case "generate_executive_brief":
      return { generated: true, subjectId: args["subjectId"], subjectType: args["subjectType"], briefId: `brief-${Date.now()}`, generatedAt: new Date().toISOString() };
