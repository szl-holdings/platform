...
export function buildAgentCard(agentId: string): A2AAgentCard {
  const agentProfiles: Record<string, Partial<A2AAgentCard>> = {
    helmsman: {
      name: "Helmsman",
      description: "Maritime intelligence agent specializing in fleet operations, AIS tracking, maritime security, route risk assessment, and sanctions compliance.",
      skills: [
        {
          id: "maritime_risk_assessment",
          name: "Maritime Risk Assessment",
          description: "Assess route risks, weather threats, and geopolitical factors affecting shipping lanes",
          tags: ["maritime", "risk", "route", "weather"],
          examples: ["Assess risk for vessel transiting Strait of Hormuz", "Evaluate sanctions exposure for route from Dubai to Piraeus"],
    ...
    sentinel: {
      name: "Sentinel",
      description: "Cybersecurity intelligence agent specializing in threat analysis, CVE assessment, incident response, and security posture evaluation.",
      skills: [
        {
          id: "threat_analysis",
          name: "Threat Analysis",
          description: "Analyze cybersecurity threats using MITRE ATT&CK framework and CVSS scoring",
        ...
        },
        {
          ...
          description: "Guide incident response with structured containment, eradication, and recovery plans",
    ...
    inca: {
      name: "INCA",
      description: "AI research intelligence agent specializing in ML research, model evaluation, academic literature analysis, and technology trend assessment.",
      skills: [
        {
          ...
          examples: ["Find state-of-the-art models for NER", "Summarize recent advances in agentic AI"],
          inputModes: ["text"],
          outputModes: ["text", "json"],
        },
      ],
    ...
    alloy: {
      name: "Alloy",
      description: "Central orchestration agent that coordinates domain specialists across the Nuro Mesh, synthesizes multi-agent intelligence, and provides unified answers.",
      skills: [
        {
          id: "multi_agent_orchestration",
          name: "Multi-Agent Orchestration",
          description: "Route complex queries to domain specialists and synthesize unified responses",
          tags: ["orchestration", "multi-agent", "synthesis"],
          examples: ["Comprehensive risk assessment for SZL fleet operations", "Unified threat and operational status briefing"],
          inputModes: ["text", "json"],
    ...
    muse: {
      name: "Muse",
      description: "Creative intelligence agent specializing in content strategy, campaign ideation, and brand voice.",
      skills: [
        {
          id: "content_strategy",
          ...
          examples: ["Create Q3 content calendar for maritime logistics brand", "Write creative brief for safety campaign"],
          inputModes: ["text"],
          outputModes: ["text"],
        },
      ],
    ...
    beacon: {
      ...
      description: "Analytics and operations intelligence agent specializing in signal analysis, anomaly detection, and platform performance.",
      skills: [
        {
    ...
    capabilities: [
      {
        ...
        inputSchema: { type: "object", properties: { query: { type: "string" }, context: { type: "object" } }, required: ["query"] },
...
export class A2ATaskManager {
  ...
  updateTask(taskId: string, updates: Partial<Pick<A2ATask, "status" | "output" | "error" | "completedAt">>): A2ATask | null {
  ...
  handleJsonRpc(request: A2AJsonRpcRequest, agentId: string): A2AJsonRpcResponse {
    ...
    if (request.method === "task/create") {
      const params = request.params as { input?: A2ATask["input"]; callerAgentId?: string; callerPlatform?: string };
      if (!params?.input?.query) {
        return { jsonrpc: "2.0", id, error: { code: -32602, message: "Invalid params: input.query required" } };
      }
      const task = this.createTask(agentId, params.input, params.callerAgentId, params.callerPlatform);
      return { jsonrpc: "2.0", id, result: { taskId: task.taskId, status: task.status, createdAt: task.createdAt } };
    }
    ...
    return { jsonrpc: "2.0", id: id ?? null, error: { code: -32601, message: `Method '${request.method}' not found` } };
  }
}
