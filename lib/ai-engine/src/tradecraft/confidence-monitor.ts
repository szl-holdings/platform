...
export class ConfidenceMonitor {
  ...
  async evaluate(agentId: string, tenantId: string): Promise<ConfidenceAlert[]> {
    ...
    const trend = scoringEngine.detectTrend(
      agentId,
      this.config.shortWindowDays,
      this.config.longWindowDays,
    );
    ...
    if (
      ...
      const alert: ConfidenceAlert = {
        ...
        description:
          `Agent ${agentId} accuracy score has declined by ${Math.round(Math.abs(trend.delta) * 100)}% ` +
        ...
        trend: 'declining',
        recommendedAction:
          'Review recent decisions for systematic errors. Consider increasing human review triggers and reducing auto-action scope.',
        ...
        resolvedBy: null,
        metadata: {
          shortTermScore: trend.shortTermScore,
          longTermScore: trend.longTermScore,
          delta: trend.delta,
        },
      };
    ...
    if (
      ...
      const alert: ConfidenceAlert = {
        ...
        description:
          `Agent ${agentId} has an override rate of ${Math.round(profile.accuracy.overrideRate * 100)}% ` +
        ...
        trend: trend.trend,
        recommendedAction:
          'Immediate human review of agent reasoning patterns. Inspect override reasons for common themes. Consider suspending autonomous actions until root cause is resolved.',
        ...
        metadata: {
          ...
          overriddenCount: profile.accuracy.overriddenDecisions,
          total: profile.accuracy.totalDecisions,
        },
      };
    ...
    if (
      ...
      const alert: ConfidenceAlert = {
        ...
        description:
          `Agent ${agentId} acceptance rate has dropped to ${Math.round(profile.accuracy.acceptanceRate * 100)}%. ` +
        ...
        trend: trend.trend,
        recommendedAction:
          'Review agent configuration, prompt templates, and decision object schema. Evaluate if the agent is operating outside its designed competency domain.',
        requiresHumanReview: true,
        autoResolvable: false,
        createdAt: new Date().toISOString(),
        resolvedAt: null,
    ...
    if (
      ...
      const alert: ConfidenceAlert = {
        ...
        description:
          `Agent ${agentId} is significantly overconfident. Average predicted confidence is ` +
    ...
    for (const skill of decliningSkills) {
      ...
      if (!this.isOnCooldown(key)) {
        const alert: ConfidenceAlert = {
          ...
          description:
            `Skill '${skill.capability}' (${skill.skillId}) for agent ${agentId} is showing declining effectiveness. ` +
