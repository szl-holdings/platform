...
export interface ReasoningAdjustment {
  dimension: "confidence" | "evidence_weighting" | "alternative_generation" | "escalation_threshold" | "human_review_trigger";
...
function buildReasoningAdjustments(profile: AgentPerformanceProfile): ReasoningAdjustment[] {
  ...
  if (profile.calibration.calibrationVerdict === "overconfident") {
    adjustments.push({
      ...
      rationale: `Historical data shows you are overconfident by ${Math.round(Math.abs(profile.calibration.calibrationBias) * 100)}%. ` +
    ...
    adjustments.push({
      ...
      direction: "increase",
      magnitude: "moderate",
      rationale: "Overconfidence bias detected — explicitly generate and weigh more alternative hypotheses before concluding.",
    });
  }
  ...
  if (profile.calibration.calibrationVerdict === "underconfident") {
    adjustments.push({
      ...
      direction: "increase",
      magnitude: "minor",
      rationale: `Historical data shows you underestimate confidence by ${Math.round(Math.abs(profile.calibration.calibrationBias) * 100)}%. ` +
  ...
  if (profile.accuracy.overrideRate > 0.3) {
    adjustments.push({
      ...
      direction: "increase",
      magnitude: "moderate",
      rationale: `High override rate (${Math.round(profile.accuracy.overrideRate * 100)}%). ` +
    ...
    adjustments.push({
      ...
      direction: "increase",
      magnitude: "moderate",
      rationale: "Given recent override patterns, lower your threshold for flagging decisions as requiring human review.",
    });
  }
  ...
  if (profile.accuracy.acceptanceRate < 0.5 && profile.accuracy.totalDecisions >= 5) {
    adjustments.push({
      ...
      rationale: `Low acceptance rate (${Math.round(profile.accuracy.acceptanceRate * 100)}%). ` +
        `Reduce the threshold for escalating decisions to human review.`,
    });
  }

  const decliningSkills = profile.skillEffectiveness.filter(s => s.trend === "declining" && s.totalUsages >= 5);
  if (decliningSkills.length > 0) {
    adjustments.push({
      dimension: "evidence_weighting",
      direction: "increase",
      magnitude: "minor",
      rationale: `Skills showing declining effectiveness: ${decliningSkills.map(s => s.capability).join(", ")}. ` +
...
function buildContextBlock(
  ...
  if (config.includeCalibrationDetails && profile.calibration.calibrationVerdict !== "insufficient_data") {
    lines.push(
  ...
  if (config.includeSkillBreakdown && profile.skillEffectiveness.length > 0) {
    const declining = profile.skillEffectiveness.filter(s => s.trend === "declining" && s.totalUsages >= 3);
    if (declining.length > 0) {
      lines.push("", "Declining skill effectiveness — apply extra scrutiny:");
      for (const s of declining) {
        lines.push(`  - ${s.capability}: acceptance rate ${Math.round(s.acceptanceRate * 100)}%, trend ↓`);
      }
    }
  }

  lines.push("", "Apply these calibration adjustments to your reasoning before producing the decision.", "═══════════════════════════════════════════════════════════════");
...
export async function buildSelfReflectionContext(
  ...
  const urgentFlags = profile.flags.filter(f =>
...
export function injectReflectionIntoPrompt(systemPrompt: string, reflectionContext: SelfReflectionContext): string {
