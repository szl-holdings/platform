...
import { ProofBundleAdapter, exportProofBundle } from "../adapters/proof-bundle.js";
...
const baseBundle: ProofBundle = {
  ...
  citations: [
    { source: "MITRE ATT&CK T1486", excerpt: "Ransomware encryption technique", url: "https://attack.mitre.org/techniques/T1486" },
  ...
  approvalChain: [
    {
      ...
      decision: "approved",
      rationale: "Network isolation is the safest containment path given current lateral movement indicators",
    },
  ],
...
describe("Scenario Forge — ProofBundleAdapter", () => {
  ...
  it("throws on missing bundleId", () => {
    const adapter = new ProofBundleAdapter();
    expect(() =>
      adapter.validate({ ...baseBundle, bundleId: "" })
  ...
  it("throws on missing contentId", () => {
    const adapter = new ProofBundleAdapter();
    expect(() =>
      adapter.validate({ ...baseBundle, contentId: "" })
  ...
  it("throws when confidenceScore is below 0", () => {
    const adapter = new ProofBundleAdapter();
    expect(() =>
      adapter.validate({ ...baseBundle, confidenceScore: -0.1 })
    ).toThrow("confidenceScore must be a number between 0 and 1");
  });

  it("throws when confidenceScore exceeds 1", () => {
    const adapter = new ProofBundleAdapter();
    expect(() =>
      adapter.validate({ ...baseBundle, confidenceScore: 1.01 })
    ).toThrow("confidenceScore must be a number between 0 and 1");
  });
  ...
  it("throws when citations is not an array", () => {
    const adapter = new ProofBundleAdapter();
    expect(() =>
      adapter.validate({ ...baseBundle, citations: null as unknown as [] })
    ).toThrow("citations must be an array");
  });

  it("throws when approvalChain is not an array", () => {
    const adapter = new ProofBundleAdapter();
    expect(() =>
      adapter.validate({ ...baseBundle, approvalChain: null as unknown as [] })
    ).toThrow("approvalChain must be an array");
  });
  ...
  it("marks finalDecision as rejected when last step is rejected", () => {
    ...
    const output = adapter.serialize({
      ...baseBundle,
      approvalChain: [
        { ...baseBundle.approvalChain[0], decision: "rejected" as const },
      ],
    });
