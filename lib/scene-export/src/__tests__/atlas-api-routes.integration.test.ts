...
import type { SceneSnapshot, BranchPackage, ProofBundle, ExportAdapterResult } from "../types.js";
  ...
  describe("POST /api/atlas/proof-bundle/export", () => {
    ...
    it("integrity.isFullyApproved is false when any chain entry is rejected", () => {
      const bundle: ProofBundle = {
        ...
        approvalChain: [
          { approverRole: "ops_lead", approvedAt: new Date().toISOString(), decision: "approved" },
          { approverRole: "ciso", approvedAt: new Date().toISOString(), decision: "rejected", rationale: "Too risky" },
        ],
        generatedAt: new Date().toISOString(),
        correlationId: null,
      };
      ...
      const result = exportProofBundle(bundle);
      const payload = result.payload as { integrity: { isFullyApproved: boolean; finalDecision: string } };
