...
import type { SceneSnapshot, BranchPackage, ProofBundle } from "../types.js";
...
describe("ATLAS Export API Routes — Integration", () => {
  ...
  describe("POST /api/atlas/branch/export", () => {
    it("returns a branch package with comparisonSummary for a security branch", () => {
      ...
      const payload = result.payload as { comparisonSummary: { deltaKeyCount: number; projectionCount: number } };
  ...
  describe("POST /api/atlas/proof-bundle/export", () => {
    it("returns a proof bundle with full integrity summary", () => {
      const bundle: ProofBundle = {
        ...
        approvalChain: [
          { approverRole: "security_lead", approvedAt: new Date().toISOString(), decision: "approved", rationale: "Best available option" },
          { approverRole: "ciso", approvedAt: new Date().toISOString(), decision: "approved", rationale: "Concur with recommendation" },
        ],
      ...
      const payload = result.payload as {
        integrity: { citationCount: number; approvalSteps: number; isFullyApproved: boolean; finalDecision: string };
      };
