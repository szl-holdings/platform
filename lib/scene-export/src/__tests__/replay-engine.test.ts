...
import { OpenUSDManifestAdapter, buildOpenUSDManifest, exportOpenUSDManifest } from "../adapters/openusd-manifest.js";
import {
  serializeDemoScene,
...
describe("Replay Engine — OpenUSDManifestAdapter (Stub)", () => {
  ...
  it("throws on missing stage", () => {
    const adapter = new OpenUSDManifestAdapter();
    expect(() =>
      adapter.validate({ ...baseManifest, stage: "" })
  ...
  it("throws on missing domain", () => {
    const adapter = new OpenUSDManifestAdapter();
    expect(() =>
      adapter.validate({ ...baseManifest, domain: "" })
  ...
  it("throws when layers is not an array", () => {
    const adapter = new OpenUSDManifestAdapter();
    expect(() =>
      adapter.validate({ ...baseManifest, layers: null as unknown as [] })
  ...
  it("throws when prims is not an array", () => {
    const adapter = new OpenUSDManifestAdapter();
    expect(() =>
      adapter.validate({ ...baseManifest, prims: null as unknown as [] })
    ).toThrow("prims must be an array");
  });
