...
import { exportJsonSnapshot, JsonSnapshotAdapter } from '../adapters/json-snapshot.js';
...
describe('Scene Memory Router — JsonSnapshotAdapter', () => {
  ...
  it('throws on missing sceneId', () => {
    const adapter = new JsonSnapshotAdapter();
    expect(() => adapter.validate({ ...baseSnapshot, sceneId: '' })).toThrow('sceneId is required');
  });

  it('throws on missing domain', () => {
    const adapter = new JsonSnapshotAdapter();
    expect(() => adapter.validate({ ...baseSnapshot, domain: '' })).toThrow('domain is required');
  });
