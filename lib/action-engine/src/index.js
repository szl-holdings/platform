...
export function executeWorkflow(defOrOpts, context = {}, options = {}) {
  let def, initiatedBy, tenantId, recommendationId, isDryRun, isSimulation, approvedBy, policyEvaluation, metadata;
  ...
  const requiresApproval = def.requiresExplicitApproval && !approvedBy && !isDryRun && !isSimulation;
  ...
  const auditTrail = [
    { at: Date.now(), action: "workflow.initiated", detail: `Mode: ${def.executionMode ?? "semi_auto"}${isDryRun ? " (dry-run)" : ""}${isSimulation ? " (simulation)" : ""}`, actor: initiatedBy },
  ];

  if (isDryRun) {
    auditTrail.push({ at: Date.now() + 1, action: "workflow.dry_run", detail: "Simulating execution without side effects.", actor: "system" });
    auditTrail.push({ at: Date.now() + 2, action: "workflow.completed", detail: "Dry run complete — no side effects produced.", actor: "system" });
  } else if (isSimulation) {
    auditTrail.push({ at: Date.now() + 1, action: "workflow.simulated", detail: "Monte Carlo scenario applied.", actor: "system" });
  } else if (requiresApproval) {
    auditTrail.push({ at: Date.now() + 1, action: "workflow.awaiting_approval", detail: "Execution paused pending human approval.", actor: "system" });
  } else {
    auditTrail.push({ at: Date.now() + 1, action: "workflow.completed", detail: `All ${steps.length} steps completed.`, actor: "system" });
  }
  ...
  const approvalRequest = requiresApproval
    ? {
        ...
        requiredApprovers: def.steps?.filter((s) => s.requiresApproval).map((s) => s.approverRole) ?? [],
      }
    : undefined;
