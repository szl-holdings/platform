...
import { type CovenantPolicy, CovenantPolicyEngine, type CovenantRequest } from '../engine.js';
...
describe('CovenantPolicyEngine', () => {
    ...
    it('denies when attribute_match in condition does not match', () => {
      ...
      e.register(
        policy({
          id: 'attr-in',
          conditions: [
            { type: 'attribute_match', field: 'tier', operator: 'in', value: ['gold', 'platinum'] },
          ],
        }),
      );
    ...
    it('denies when attribute_match contains condition does not match', () => {
      ...
      e.register(
        policy({
          id: 'attr-contains',
          conditions: [
            { type: 'attribute_match', field: 'label', operator: 'contains', value: 'prod' },
          ],
        }),
      );
  ...
  describe('maritime-critical-response-v2 — real production policy', () => {
    ...
    it('simulate() on a stripped-role maritime request yields a DENY explanation', () => {
      const e = new CovenantPolicyEngine();
      e.register(MARITIME_RESPONSE_POLICY);
      const { decision, explanation } = e.simulate(
        maritimeReq({ subject: { roles: ['analyst'] } }),
      );
