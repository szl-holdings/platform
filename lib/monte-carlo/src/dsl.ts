...
export interface VariableExpr { type: "variable"; id: string }
export interface BinaryExpr { type: "binary"; op: "+" | "-" | "*" | "/" | "**" | "%" | "min" | "max"; left: Expr; right: Expr }
export interface UnaryExpr { type: "unary"; op: "-" | "abs" | "sqrt" | "log" | "log10" | "exp" | "floor" | "ceil" | "round"; operand: Expr }
export interface CallExpr { type: "call"; fn: "max" | "min" | "pow" | "clamp"; args: Expr[] }
export interface ConditionalExpr { type: "conditional"; condition: BoolExpr; then: Expr; else: Expr }

export type Expr = NumberExpr | VariableExpr | BinaryExpr | UnaryExpr | CallExpr | ConditionalExpr;

export interface CompareExpr { type: "compare"; op: "<" | "<=" | ">" | ">=" | "==" | "!="; left: Expr; right: Expr }
export interface LogicalExpr { type: "logical"; op: "and" | "or" | "not"; operands: BoolExpr[] }
...
export class EvalError extends Error {
  constructor(msg: string) { super(msg); this.name = "EvalError"; }
}
...
function evalBool(expr: BoolExpr, vars: Record<string, number>): boolean {
  switch (expr.type) {
    ...
    case "logical": {
      switch (expr.op) {
        case "and": return expr.operands.every((o) => evalBool(o, vars));
        case "or": return expr.operands.some((o) => evalBool(o, vars));
        case "not": return !evalBool(expr.operands[0]!, vars);
      }
    }
  }
  return false;
...
export const DSL_HELPERS = {
  ...
  clamp: (val: Expr, lo: Expr, hi: Expr): CallExpr => ({ type: "call", fn: "clamp", args: [val, lo, hi] }),
...
const VALID_DISTRIBUTION_TYPES = new Set([
  "normal", "log_normal", "uniform", "triangular", "beta", "poisson", "constant", "custom",
]);
...
const VALID_UNARY_OPS = new Set(["-", "abs", "sqrt", "log", "log10", "exp", "floor", "ceil", "round"]);
...
function validateExprNode(expr: unknown, depth = 0): boolean {
  ...
  switch (e["type"]) {
    case "number":
      return typeof e["value"] === "number" && isFinite(e["value"] as number);
    case "variable":
...
export function validateSerializableScenario(s: unknown): s is SerializableScenario {
  ...
  if (!Array.isArray(sc["inputs"]) || !Array.isArray(sc["outputs"]) || !Array.isArray(sc["outputExprs"])) return false;
  ...
  for (const out of outputs) {
    ...
    if (o["format"] !== undefined && o["format"] !== null && !VALID_OUTPUT_FORMATS.has(o["format"] as string)) return false;
  }
