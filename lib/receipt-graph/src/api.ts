...
export function getReceiptByContent(contentType: string, contentId: string): TrustReceipt[] {
...
export function getExecutiveTrustSummary(
...
export function assertExportSafe(contentType: string, contentId: string): void {
  ...
  if (!latest.exportSafe) {
    throw Object.assign(
      new Error(
        `Content ${contentType}:${contentId} is not export safe — status: ${latest.status}, policy: ${latest.policyClass}`,
      ),
