...
export function markActivationEvent(event: keyof Omit<ActivationState, 'isLoading' | 'refresh'>) {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    const cached = raw
...
async function countFromEndpoint(url: string, options: RequestInit): Promise<number> {
  try {
...
export function useActivationState(options: ActivationStateOptions = {}): ActivationState {
  ...
  const doFetch = useCallback(async () => {
    ...
    try {
      const [signalCount, workflowCount, approvedCount, profileDone] = await Promise.all([
        countFromEndpoint(`${apiBaseUrl}/lyte/signals?limit=1`, reqOpts),
        countFromEndpoint(`${apiBaseUrl}/alloy/workflows?limit=1`, reqOpts),
        countFromEndpoint(`${apiBaseUrl}/alloy/approvals?status=approved&limit=1`, reqOpts),
      ...
      const next: Omit<ActivationState, 'refresh'> = {
        ...
        teamMemberInvited: storedCache?.teamMemberInvited ?? false,
