...
export function ConfidenceBand({
  ...
  return (
    <div className="flex items-center gap-1.5">
      <div
        className={`${widths[size]} ${heights[size]} rounded-full overflow-hidden`}
      ...
      {showLabel && (
        <span className={`${fonts[size]} font-mono`} style={{ color }}>
          {pct}%
        </span>
      )}
    </div>
      ...
      {!collapsed &&
        evidence.map((e, i) => (
          ...
          >
            ...
            <p className="text-[9px] leading-relaxed line-clamp-2" style={{ color: TEXT.tertiary }}>
              {e.content}
            </p>
          </div>
        ))}
...
export function ApprovalBadge({ level, required }: { level: string; required: boolean }) {
  if (!required) {
    return (
      <span
        className="text-[7px] font-mono px-1.5 py-px rounded inline-flex items-center gap-1"
        style={{
          color: '#6b8f71',
...
export function RiskBadge({ level }: { level: string }) {
  ...
  return (
    <span
      className="text-[7px] font-mono uppercase px-1.5 py-px rounded"
      style={{
        color,
        background: `${color}12`,
...
export function EnvironmentLabel({
  ...
  return (
    <span
      className="text-[8px] font-mono font-semibold tracking-wider px-2 py-0.5 rounded"
      style={{
        color: cfg.color,
...
export function DegradedModeBanner({ message }: { message?: string }) {
  return (
    ...
    >
      <span className="text-[9px] font-mono font-semibold" style={{ color: '#c8953c' }}>
        DEGRADED MODE
      </span>
      <span className="text-[9px]" style={{ color: TEXT.secondary }}>
        {message || 'AI provider unavailable. System operating with safe fallbacks.'}
      </span>
    </div>
  );
}
...
export function SafeFallbackState({ reason }: { reason?: string }) {
  return (
    ...
    >
      <div className="text-[9px] font-mono font-semibold mb-1" style={{ color: '#c45a4a' }}>
        Safe Fallback Active
      </div>
      <p className="text-[9px]" style={{ color: TEXT.secondary }}>
        {reason ||
          'AI decision could not be produced with sufficient confidence. Manual review required.'}
      </p>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="p-3 space-y-2">
        ...
        {evidence && evidence.length > 0 && <EvidencePanel evidence={evidence} collapsible />}
        ...
        {visible.map((entry, i) => (
          <div key={i} className="px-3 py-2">
            <div className="flex items-center gap-2 mb-0.5">
              <span className="text-[9px] font-mono font-semibold" style={{ color: '#d4a054' }}>
                {entry.action}
              </span>
              {entry.model && (
                <span className="text-[8px] font-mono" style={{ color: TEXT.muted }}>
