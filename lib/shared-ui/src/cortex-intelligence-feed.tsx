...
> = {
  critical: { color: "#ef4444", bg: "#ef444415", border: "#ef444440", label: "CRITICAL", priority: 4 },
      ...
      <div style={{ padding: "10px 14px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" as const, alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
            <span style={{ fontSize: 11, color: "#ffffff50" }}>{formatRelativeTime(signal.timestamp)}</span>
            <ConfidenceMeter value={signal.confidence} />
          </div>
        </div>

        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" as const, marginTop: 6, paddingLeft: 18 }}>
          ...
          <div style={{ paddingTop: 10 }}>
            ...
            {signal.affectedEntities && signal.affectedEntities.length > 0 && (
              <div style={{ marginBottom: 10 }}>
                <p style={{ margin: "0 0 5px", fontSize: 11, fontWeight: 600, color: "#ffffff60", letterSpacing: "0.06em", textTransform: "uppercase" as const }}>
                  Affected Entities
                </p>
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                <ul style={{ margin: 0, paddingLeft: 16 }}>
                  {signal.recommendedActions.map((action, i) => (
                    <li key={i} style={{ fontSize: 12, color: "#ffffffb0", marginBottom: 2, lineHeight: 1.5 }}>
                      {action}
                    </li>
                  ))}
                </ul>
  ...
  const filtered = useMemo(() => {
    ...
    if (domainFilter !== "all") {
      result = result.filter((s) =>
        s.affectedDomains.includes(domainFilter) || (domainFilter === "firestorm" && s.affectedDomains.includes("aegis"))
      );
    }
    return result
      .sort((a, b) => {
      ...
      {stats && (
        ...
        >
          {[
            ...
            >
              <p style={{ margin: 0, fontSize: 20, fontWeight: 700, color, lineHeight: 1.2 }}>{value}</p>
              <p style={{ margin: 0, fontSize: 10, color: "#ffffff50", marginTop: 2, letterSpacing: "0.05em" }}>
                {label.toUpperCase()}
              </p>
            </div>
          ))}
      ...
      {loading ? (
        <div style={{ padding: "32px 0", textAlign: "center" as const }}>
          ...
          <p style={{ margin: 0, fontSize: 13, color: "#ffffff40" }}>
            CORTEX is scanning cross-domain signals…
          </p>
        </div>
      ) : filtered.length === 0 ? (
        <div
