            ...
            >
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-white/40 mt-0.5 line-clamp-2">{correlation.description}</p>
                </div>
                {correlation.impact && (
                  <span className={cn(
                    "text-[9px] px-1.5 py-0.5 rounded-full font-medium shrink-0",
                    correlation.impact === "high" ? "bg-red-500/20 text-red-400" :
                    correlation.impact === "medium" ? "bg-amber-500/20 text-amber-400" :
              ...
              <div className="px-3 pb-3 border-t border-white/5 pt-2 space-y-2">
                ...
                <div className="space-y-1">
                  {correlation.signals.map((signal, i) => (
                    <div key={i} className="flex items-center gap-2 text-[11px]">
                      ...
                      />
                      <span className="text-white/50">{DOMAIN_LABELS[signal.domain] ?? signal.domain}:</span>
                ...
                {correlation.suggestedActions && correlation.suggestedActions.length > 0 && (
                  <div className="bg-white/5 rounded-md p-2 mt-1">
                    <div className="text-[9px] text-white/40 uppercase tracking-wider mb-1">Suggested Actions</div>
