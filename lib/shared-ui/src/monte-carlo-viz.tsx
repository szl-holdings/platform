    ...
    <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: 8, padding: 12 }}>
      ...
      <svg width={width} height={height}>
        <g transform={`translate(${pad.left},${pad.top})`}>
          {showCI && <rect x={ciX1} y={0} width={ciX2 - ciX1} height={plotH} fill={COLORS.ci} />}
          {histogram.map((b, i) => (
            <rect
              key={i}
              x={xScale(b.lo)}
          ...
          <text x={-8} y={0} textAnchor="end" fontSize={9} fill={COLORS.subtext}>
            High
          </text>
          <text x={-8} y={plotH} textAnchor="end" fontSize={9} fill={COLORS.subtext}>
            Low
          </text>
        </g>
        <g transform={`translate(${pad.left + plotW - 120}, ${pad.top + 4})`}>
          <rect width={8} height={2} y={4} fill={COLORS.warning} />
          <text x={12} y={8} fontSize={9} fill={COLORS.subtext}>
            Mean: {fmt(stats.mean, format)}
          </text>
          <rect width={8} height={2} y={18} fill={COLORS.success} />
          <text x={12} y={22} fontSize={9} fill={COLORS.subtext}>
            Median: {fmt(stats.median, format)}
          </text>
        </g>
      </svg>
    </div>
...
export function CumulativeDistributionCurve({
  ...
  const pathD = cdf
    .map(
      (pt, i) =>
        `${i === 0 ? 'M' : 'L'}${xScale(pt.value).toFixed(1)},${yScale(pt.cumProb).toFixed(1)}`,
    )
    .join(' ');
    ...
    <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: 8, padding: 12 }}>
      ...
      <svg width={width} height={height}>
        <g transform={`translate(${pad.left},${pad.top})`}>
          ...
          {pLines.map(({ p, label }) => {
            const yPos = yScale(p);
            const closest = [...cdf].sort(
              (a, b) => Math.abs(a.cumProb - p) - Math.abs(b.cumProb - p),
            )[0];
            ...
            return (
              <g key={label}>
                ...
                <text x={xPos + 4} y={yPos - 4} fontSize={9} fill={COLORS.subtext}>
                  {label}: {fmt(closest.value, format)}
                </text>
              </g>
            );
          })}
                ...
                >
                  <span
                    style={{
                      ...
                      height: 10,
                      borderRadius: 2,
                      background: SCENARIO_PALETTE[i % SCENARIO_PALETTE.length],
                      marginRight: 6,
                    }}
                  />
                  {s.scenarioTitle}
      ...
      <svg width={width} height={height}>
        <g transform={`translate(${pad.left},${pad.top})`}>
          ...
          {[stats.p5, stats.p25, stats.p50, stats.p75, stats.p95].map((v, i) => (
            <g key={i} transform={`translate(${xScale(v)},${plotH})`}>
              <line y2={4} stroke={COLORS.subtext} />
              <text y={14} textAnchor="middle" fontSize={9} fill={COLORS.subtext}>
                {fmt(v, format)}
              </text>
            </g>
          ))}
        ...
        <div style={{ display: 'flex', gap: 16, fontSize: 11, color: COLORS.subtext }}>
          <div>
            ...
            <div style={{ color: COLORS.text, fontWeight: 600 }}>
              {fmt(stats.p10, metric.format)}
            </div>
          </div>
          <div>
            <div>P50</div>
            <div style={{ color: COLORS.text, fontWeight: 600 }}>
              {fmt(stats.p50, metric.format)}
            </div>
          </div>
          <div>
            <div>P90</div>
            <div style={{ color: COLORS.text, fontWeight: 600 }}>
              {fmt(stats.p90, metric.format)}
            </div>
          </div>
        </div>
      </div>
...
export function SimulationProgressTracker({
  ...
  return (
    <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: 8, padding: 12 }}>
      ...
      >
        <span>
          {iteration.toLocaleString()} / {totalIterations.toLocaleString()} iterations
        </span>
