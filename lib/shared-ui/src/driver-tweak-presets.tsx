...
function isIdentityTweak(t: DriverTweak | undefined): boolean {
  if (!t) return true;
  return (
    Math.abs(t.meanMultiplier - 1) < 1e-9 && Math.abs(t.spreadMultiplier - 1) < 1e-9
  );
}
...
export function DriverTweakPresets({
  ...
  const savePreset = useCallback(() => {
    ...
    const id = existingIdx >= 0 ? presets[existingIdx].id : `pst_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 6)}`;
