...
function LensScoreChip({ lens }: { lens: LensScore }) {
  ...
  return (
    <div
      className={`flex items-center gap-1.5 px-2 py-1 rounded-lg border bg-white/[0.03] ${color}`}
    >
      <span className="text-[10px] font-bold opacity-70">{icon}</span>
      <div className="min-w-0">
        <div className="text-[10px] text-white/40 capitalize leading-none mb-0.5">
          {lens.lensId}
        </div>
    ...
    <div className="rounded-xl border border-white/10 bg-white/[0.02] p-4 hover:bg-white/[0.04] hover:border-white/15 transition-all">
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <div className="text-right flex-shrink-0">
          ...
          <div className="text-[9px] text-white/30 uppercase tracking-wider leading-none mt-0.5">
            {data.postureScoreName}
          </div>
        </div>
      </div>
...
export function PostureScore({
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {label && (
        <span className={`${sc.label} text-white/30 uppercase tracking-wider text-center mt-1`}>
          {label}
        </span>
      )}
    </div>
