...
import { Bell, CreditCard, FileText, Plug, ShieldCheck, Sliders, User, Users } from 'lucide-react';
        ...
        <nav className="flex-1 py-2 overflow-y-auto">
          {sections.map((section) => {
            ...
            return (
              ...
              >
                <Icon className={cn('w-4 h-4 shrink-0', isActive ? 'opacity-100' : 'opacity-50')} />
                {section.label}
              </button>
            );
          })}
...
export function SettingsRow({ label, description, children, className }: SettingsRowProps) {
  return (
    <div
      className={cn(
        'flex items-start justify-between gap-6 py-4 border-b border-border/40 last:border-0',
        className,
      )}
    >
