      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          if (
            ...
            const liveLevel =
              (raw.severity as LiveNotification['level']) ||
              typeToLevel((raw.type as ApiNotification['type']) || 'info');
            ...
            setNotifications((prev) => {
              if (
                prev.some(
                  (n) =>
                    n.title === live.title &&
                    n.message === live.message &&
                    Math.abs(n.timestamp.getTime() - live.timestamp.getTime()) < 5000,
                )
              ) {
                return prev;
              }
