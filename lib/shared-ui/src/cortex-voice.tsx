...
async function defaultQueryHandler(query: string): Promise<CortexResult> {
  try {
    ...
    const json = await res.json() as { data?: { summary?: string; confidence?: number; domains?: string[]; actions?: Array<{ label: string; path: string }> } };
...
export function CortexVoice({
  ...
  const handleSubmit = useCallback(async (q: string) => {
    ...
    try {
      ...
      setResult({ summary: "CORTEX encountered an error processing your request. Please try again.", confidence: 0 });
      setVoiceState("result");
    }
  }, [onQuery]);
    ...
    >
      ...
      >
        ...
        >
          ...
          {voiceState === "listening" && (
            <div style={{ display: "flex", gap: "3px", alignItems: "center" }}>
              ...
              <span style={{ fontSize: "11px", color: accentColor, marginLeft: "6px" }}>Listening...</span>
            </div>
          )}
          {voiceState === "processing" && (
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              ...
              />
              <span style={{ fontSize: "11px", color: "rgba(255,255,255,0.5)" }}>Routing across domains...</span>
            </div>
          )}
          <button
            onClick={onClose}
        ...
        {voiceState === "result" && result && (
          <div style={{ padding: "20px 24px 16px", display: "flex", flexDirection: "column", gap: "14px" }}>
            <div
              style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: "8px", marginTop: "10px", flexWrap: "wrap" }}>
                    ...
                    <span
                      style={{
                        fontSize: "10px",
                        color: result.confidence > 0.85 ? "#22c55e" : result.confidence > 0.7 ? "#f59e0b" : "#ef4444",
                        background: "rgba(255,255,255,0.05)",
                        border: "1px solid rgba(255,255,255,0.1)",
                        borderRadius: "5px",
                        padding: "2px 8px",
              ...
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                {result.actions.map((action, i) => (
                  <a
                    ...
                    href={action.href}
                    onClick={(e) => {
                      if (action.action) { e.preventDefault(); action.action(); onClose(); }
                    }}
                    style={{
                      padding: "8px 14px",
                      borderRadius: "8px",
          ...
          <div style={{ padding: "16px 24px", background: "rgba(239,68,68,0.08)", borderTop: "1px solid rgba(239,68,68,0.2)" }}>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              <button
                ...
                style={{ padding: "7px 16px", borderRadius: "8px", background: "#ef4444", border: "none", color: "#fff", fontSize: "12px", fontWeight: 600, cursor: "pointer" }}
              >
                Confirm
              </button>
              <button
                onClick={() => { setPendingAction(null); setVoiceState("result"); }}
                style={{ padding: "7px 16px", borderRadius: "8px", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.6)", fontSize: "12px", cursor: "pointer" }}
              >
                Cancel
              </button>
            </div>
        ...
        {voiceState === "idle" && !query && (
          <div style={{ padding: "12px 24px 20px" }}>
            <div style={{ fontSize: "10px", color: "rgba(255,255,255,0.3)", letterSpacing: "1px", textTransform: "uppercase", marginBottom: "10px" }}>
              Suggested queries
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
              {SAMPLE_INTENTS.map((intent, i) => (
                <button
                  key={i}
                  onClick={() => { setQuery(intent); handleSubmit(intent); }}
                  style={{
                    textAlign: "left",
                  ...
                  onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = `${accentColor}12`; (e.currentTarget as HTMLButtonElement).style.color = "rgba(255,255,255,0.8)"; }}
