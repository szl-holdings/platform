...
export const DEMO_COMPANY_KPIS: CompanyKPI[] = [
  ...
  },
  {
    ...
    changeLabel: 'vs 30d ago',
    lane: 'security',
    description: 'Composite security posture index across PARAGON & PARAGON Operations',
  },
  {
    id: 'threat-d2c',
...
function KPICard({ kpi }: { kpi: CompanyKPI }) {
  ...
  const isGoodChange =
    kpi.id.includes('success') ||
    kpi.id.includes('accuracy') ||
    kpi.id.includes('adoption') ||
    kpi.id.includes('recovered') ||
    kpi.id.includes('prevented') ||
    kpi.id.includes('reliability') ||
    kpi.id.includes('readiness') ||
    kpi.id.includes('engagement') ||
    kpi.id.includes('readiness');
    ...
    >
      ...
      {kpi.change !== undefined && (
        ...
        >
          ...
          <span>
            {Math.abs(kpi.change ?? 0)}% {kpi.changeLabel}
          </span>
        </div>
      )}
    </div>
