  ...
  return {
    ...
    signals: [
      ...
      },
      {
        ...
        summary:
          'Miami Beach commercial listing at 34% below market. Austin industrial park showing 60-day DOM.',
      ...
      },
      {
        ...
        summary:
          'Maritime segment leads with 3.1% yield expansion. Real estate holdings stable. Venture positions unchanged.',
        metric: '$284.2M NAV',
        trend: 'up',
        severity: 'low',
        href: '/',
      },
      {
        ...
        summary:
          'MITRE T1190 pattern detected on perimeter. 2 identity anomalies in SOC queue. Security perimeter holding.',
        metric: '3 critical',
        trend: 'flat',
        severity: 'high',
        href: '/firestorm/',
      },
      {
        ...
        summary:
          'PRISM Counsel API p95 at 340ms (+22%). Terra data pipeline processing delay of 8min vs 3min baseline.',
        metric: 'p95: 340ms',
        trend: 'down',
        severity: 'medium',
      ...
      {(!compact || expanded) && (
        ...
        >
          {[
            {
              label: 'Portfolio NAV',
              value: `$${briefing.portfolioNav}M`,
              sub: `${briefing.navChange && briefing.navChange > 0 ? '+' : ''}${briefing.navChange}% today`,
              color: '#22c55e',
            },
            {
              label: 'Active Alerts',
              value: String(briefing.activeAlerts ?? 0),
              sub: 'across all domains',
              color: briefing.activeAlerts && briefing.activeAlerts > 20 ? '#ef4444' : '#f59e0b',
            },
            {
              label: 'Pending Deadlines',
              value: String(briefing.pendingDeadlines ?? 0),
              sub: 'require attention',
              color:
                briefing.pendingDeadlines && briefing.pendingDeadlines > 0 ? '#f97316' : '#22c55e',
            },
            {
              label: 'Domains',
              value: `${briefing.signals.length}`,
              sub: 'signals aggregated',
              color: accentColor,
            },
          ].map((kpi) => (
            <div
            ...
            return (
              <a
                ...
                style={{
                  ...
                  padding: '13px 22px',
                  borderBottom:
                    i < briefing.signals.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none',
                  textDecoration: 'none',
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div
                    style={{ fontSize: '11px', color: 'rgba(255,255,255,0.45)', lineHeight: 1.5 }}
                  >
                    {signal.summary}
                  </div>
                </div>
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  ...
                  <TrendArrow
                    {...(signal.trend !== undefined ? { trend: signal.trend } : {})}
                    {...(signal.severity !== undefined ? { severity: signal.severity } : {})}
                  />
                </div>
              </a>
            );
...
export function BriefingHistory({
  briefings = DEMO_BRIEFING_HISTORY,
  accentColor = '#8b7ac8',
