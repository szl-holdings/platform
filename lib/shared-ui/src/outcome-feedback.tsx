...
function ConfidencePip({ value }: { value: number }) {
  ...
  return <span style={{ color, fontSize: 11, fontFamily: 'monospace' }}>{pct}% conf</span>;
}
...
export function OutcomeFeedbackBar({ outcomeId, onDecision, className }: OutcomeFeedbackBarProps) {
  ...
  const handleDecision = useCallback(
    async (decision: OutcomeDecision, r?: string) => {
      ...
      {showOverride && (
        <div style={{ display: 'flex', gap: 6 }}>
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter') {
                handleDecision('overridden', reason);
...
export function OutcomeFeedbackCard({
  ...
  const handleDecision = useCallback(
    async (decision: OutcomeDecision, reason?: string) => {
      if (loading || submitted) return;
      setLoading(true);
      try {
    ...
    >
      ...
      >
        <div style={{ flex: 1 }}>
          ...
          <p
            style={{ fontSize: compact ? 12 : 13, color: TEXT.primary, lineHeight: 1.5, margin: 0 }}
          >
            {recommendationText}
          </p>
      ...
      <OutcomeFeedbackBar outcomeId={outcomeId} onDecision={handleDecision} />
    </div>
  );
}
...
export function InlineFeedbackBar({
  ...
  const [submitted, setSubmitted] = React.useState<'up' | 'down' | null>(
    () => loadFeedbackState(recommendationKey)?.vote ?? null,
  );
  const [submittedComment, setSubmittedComment] = React.useState<string | undefined>(
    () => loadFeedbackState(recommendationKey)?.comment,
  );
  ...
  return (
    <div className={className} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {error && (
          <span style={{ fontSize: 11, color: ACCENT.red }}>
...
export function OutcomeDashboard({ apiBaseUrl = '/api-server' }: OutcomeDashboardProps) {
  ...
  const [stats, setStats] = React.useState<
    Array<{ domain: string; total: string; avgConfidence: string }>
  >([]);

  React.useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch(`${apiBaseUrl}/outcome-graph/recommendations?limit=20`)
        .then((r) => r.json())
        .catch(() => ({ data: [] })),
      fetch(`${apiBaseUrl}/outcome-graph/stats`)
        .then((r) => r.json())
        .catch(() => ({ data: [] })),
    ])
      .then(([recs, statsData]) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        >
          {stats.map((s) => (
            ...
            >
              ...
              <div style={{ fontSize: 22, fontWeight: 600, color: TEXT.primary, marginTop: 2 }}>
                {s.total}
              </div>
      ...
      {loading ? (
        <div style={{ color: TEXT.secondary, fontSize: 13, textAlign: 'center', padding: 40 }}>
          Loading outcomes…
        </div>
      ) : outcomes.length === 0 ? (
        <div style={{ color: TEXT.secondary, fontSize: 13, textAlign: 'center', padding: 40 }}>
          No outcome records yet. They will appear here as recommendations are made and acted on.
        </div>
      ) : (
            ...
            >
              <span
                style={{
                  ...
                  height: 7,
                  borderRadius: '50%',
                  background: statusColors[o.status] ?? TEXT.tertiary,
                  flexShrink: 0,
                  marginTop: 5,
                }}
              />
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                  <span style={{ fontSize: 10, color: ACCENT.purple, fontFamily: 'monospace' }}>
                    {o.agentId ?? '—'}
                  </span>
                  <span style={{ fontSize: 10, color: TEXT.tertiary }}>{o.domain}</span>
                  <span style={{ fontSize: 10, color: TEXT.tertiary }}>{o.entityType}</span>
                </div>
                <p style={{ fontSize: 12, color: TEXT.primary, margin: 0, lineHeight: 1.4 }}>
                  {o.recommendationText}
                </p>
                {o.recommendationAction && (
                  <p style={{ fontSize: 11, color: TEXT.secondary, margin: '2px 0 0' }}>
                    → {o.recommendationAction}
                  </p>
                )}
              </div>
              ...
              >
                <span
                  style={{
                    fontSize: 10,
                    color: statusColors[o.status] ?? TEXT.tertiary,
                    background: `${statusColors[o.status] ?? TEXT.tertiary}15`,
                    padding: '2px 6px',
                    borderRadius: 4,
                  }}
                >
