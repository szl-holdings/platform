...
function safeWrite(domain: string, runs: SavedRiskRun[]): void {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem(storageKey(domain), JSON.stringify(runs.slice(0, MAX_RUNS_PER_DOMAIN)));
...
export function RiskEvidenceList({ domain, domainLabel, accentColor = "#7a99b8", emptyHint, className }: RiskEvidenceListProps) {
  ...
  if (runs.length === 0) {
    return (
      <div className={`rounded-xl border p-4 ${className ?? ""}`} style={{ borderColor: "rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.02)" }}>
        ...
        <p className="text-[11px]" style={{ color: "rgba(255,255,255,0.4)" }}>
          {emptyHint ?? "No simulation runs cited yet. Use \"Save run as evidence\" on the Risk Simulation page to attach percentile bands and sensitivities to a decision."}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={`rounded-xl border p-4 ${className ?? ""}`} style={{ borderColor: "rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.02)" }}>
      <div className="flex items-center gap-2 mb-3">
        <Sliders className="w-3.5 h-3.5" style={{ color: accentColor }} />
        <h4 className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: "rgba(255,255,255,0.55)" }}>
          Cited Risk Simulations{domainLabel ? ` — ${domainLabel}` : ""}
        </h4>
        <span className="text-[10px] font-mono ml-auto" style={{ color: "rgba(255,255,255,0.35)" }}>{runs.length}</span>
      </div>
      <ul className="space-y-2">
        {runs.map(run => {
          const primary = run.metrics[0];
          return (
            <li key={run.evidenceId} className="rounded-lg border p-3" style={{ borderColor: "rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.02)" }}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ background: `${accentColor}15`, color: accentColor }}>
                      {run.evidenceId}
                    </span>
                    <span className="text-[11px] font-semibold text-white truncate">{run.scenarioTitle}</span>
                  </div>
                  <div className="text-[10px] mt-1 font-mono" style={{ color: "rgba(255,255,255,0.4)" }}>
                    Scenario {run.scenarioId}{run.scenarioVersion ? `@${run.scenarioVersion}` : ""} · {run.iterations.toLocaleString()} iter · {new Date(run.savedAt).toLocaleString([], { dateStyle: "short", timeStyle: "short" })}
                  </div>
                  {primary && (
                    <div className="text-[11px] mt-1" style={{ color: "rgba(255,255,255,0.7)" }}>
                      {primary.label} · P50 {formatValue(primary.p50, primary.format)} · P5–P95 {formatValue(primary.p5, primary.format)} → {formatValue(primary.p95, primary.format)}
                    </div>
                  )}
                  {run.note && (
                    <div className="text-[11px] mt-1 italic" style={{ color: "rgba(255,255,255,0.5)" }}>“{run.note}”</div>
                  )}
                </div>
