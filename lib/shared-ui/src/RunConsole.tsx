...
import type { Run, RunEffort, RunStatus } from './os-layer';
        ...
        {results.map((r) => {
          ...
          return (
            <div key={r.skillName} className="flex flex-col gap-1 min-w-[80px]">
              <div className="text-[9px] font-mono leading-tight" style={{ color: TEXT.tertiary }}>
                {r.skillName}
              </div>
...
export function RunDetailPanel({ run, onClose, onReplay }: RunDetailProps) {
  ...
  return (
    <>
      ...
      />
      <aside
        className="fixed top-0 right-0 bottom-0 z-50 w-full max-w-lg flex flex-col overflow-y-auto"
      ...
      >
        ...
        <div
          ...
          style={{ background: BORDER.subtle, borderBottom: `1px solid ${BORDER.subtle}` }}
        >
          {[
            ...
            {
              label: 'Eval score',
              value: run.evalScore != null ? `${Math.round(run.evalScore * 100)}%` : '—',
            },
          ].map((stat) => (
            <div key={stat.label} className="px-4 py-3" style={{ background: BG.card }}>
              ...
              <div
                ...
                style={{ color: stat.color ?? TEXT.primary }}
              >
                {stat.value}
              </div>
            </div>
        ...
        {run.tokenUsage && (
          ...
          >
            ...
            <span className="text-[10px] font-mono" style={{ color: TEXT.tertiary }}>
              in {run.tokenUsage.input.toLocaleString()} · out{' '}
              {run.tokenUsage.output.toLocaleString()} · total{' '}
        ...
        <div className="px-5 py-4 flex-1">
          <div
            className="text-[9px] uppercase tracking-widest font-mono mb-3"
            style={{ color: TEXT.muted }}
          >
            ...
            {run.toolCalls.map((tc, i) => (
              ...
              >
                <span
                  ...
                  style={{ color: tc.success ? '#6b8f71' : '#c45a4a' }}
                >
                  {tc.success ? '✓' : '✕'}
                </span>
                <div className="flex-1 min-w-0">
                  ...
                  {tc.inputSummary && (
                    <div className="text-[10px] truncate" style={{ color: TEXT.tertiary }}>
                      {tc.inputSummary}
                    </div>
                  )}
                  {tc.outputSummary && (
          ...
          <div className="px-5 py-4" style={{ borderTop: `1px solid ${BORDER.subtle}` }}>
            <button
              ...
              style={{
                ...
                color: '#4a90b8',
                border: '1px solid rgba(74,144,184,0.22)',
              }}
            >
...
function RunRow({ run, onClick }: { run: Run; onClick: () => void }) {
  ...
  const effortStyle = EFFORT_STYLE[run.effort];
  const overrideColor =
    run.humanOverride === 'approved'
      ? '#6b8f71'
      : run.humanOverride === 'rejected'
        ? '#c45a4a'
        : '#c8953c';
    ...
    >
      ...
      <div className="flex items-center gap-3 shrink-0 text-right">
        <div>
          ...
          <div className="text-[10px] font-mono" style={{ color: effortStyle.color }}>
            {effortStyle.label}
          </div>
        </div>
        <div>
        ...
        {run.evalScore != null && (
          <div>
            ...
            <div
              ...
              style={{
                color:
                  run.evalScore >= 0.8 ? '#6b8f71' : run.evalScore >= 0.6 ? '#c8953c' : '#c45a4a',
              }}
            >
        ...
        {run.humanOverride && (
          <div>
            ...
            <div className="text-[10px] font-mono capitalize" style={{ color: overrideColor }}>
              {run.humanOverride}
            </div>
          </div>
        )}
