...
const EFFECT_CONFIG: Record<
  PolicyEffect,
  { label: string; color: string; icon: string; bgColor: string; borderColor: string }
> = {
  allow: {
    label: 'Permitted',
    color: '#6b8f71',
    icon: '✓',
    bgColor: 'rgba(107,143,113,0.08)',
  ...
  escalate: {
    ...
    color: '#c8953c',
    icon: '↑',
    bgColor: 'rgba(200,149,60,0.08)',
    borderColor: 'rgba(200,149,60,0.25)',
  },
  pending: {
...
export function PolicyResultBanner({
  decision,
  accentColor,
}: {
  decision: PolicyDecisionRecord;
  ...
  return (
    ...
    >
      ...
      {decision.reason && (
        <span style={{ color: TEXT.secondary, marginLeft: 4 }}>— {decision.reason}</span>
      )}
    </div>
  );
}
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          {decision.reason && (
            <div style={{ fontSize: 11, color: TEXT.secondary, marginTop: 1 }}>
              {decision.reason}
            </div>
          )}
        </div>
        ...
        <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                {decision.subject?.roles && decision.subject.roles.length > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ color: TEXT.primary, fontFamily: 'monospace', fontSize: 11 }}>
                      {decision.subject.roles.join(', ')}
                    </span>
                  </div>
                )}
                {decision.resource?.type && (
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ color: TEXT.primary }}>
                      {decision.resource.type}
                      {decision.resource.domain ? ` (${decision.resource.domain})` : ''}
                    </span>
                  </div>
                )}
                {decision.action && (
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                {decision.approvalHistory.map((h, i) => {
                  const hcfg = (APPROVAL_CONFIG[h.decision] ??
                    APPROVAL_CONFIG.pending) as NonNullable<typeof APPROVAL_CONFIG.pending>;
                  return (
                    <div
                      key={i}
                      style={{
                    ...
                    >
                      <span style={{ color: hcfg.color, fontWeight: 700, flexShrink: 0 }}>
                        {hcfg.icon}
                      </span>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <span style={{ color: TEXT.primary, fontWeight: 600 }}>{h.approver}</span>
                          <span style={{ color: TEXT.tertiary, fontSize: 10 }}>
                            {timeAgo(h.at)}
                          </span>
                        </div>
                        {h.note && (
          ...
          {decision.deniedBy && (
            <div style={{ fontSize: 11, color: TEXT.tertiary }}>
              Denied by rule:{' '}
              <span style={{ fontFamily: 'monospace', color: '#ef8a8a' }}>{decision.deniedBy}</span>
            </div>
          )}
      ...
      >
        <div
          style={{
            ...
            height: 28,
            borderRadius: '50%',
            background: effectCfg ? effectCfg.bgColor : 'rgba(255,255,255,0.04)',
            border: `1px solid ${effectCfg ? effectCfg.borderColor : BORDER.subtle}`,
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            fontSize: 12,
        ...
        <div
          style={{ width: 1, flex: 1, background: BORDER.subtle, minHeight: 16, marginTop: 4 }}
        />
      </div>
      <div style={{ flex: 1, paddingBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
          <span style={{ fontWeight: 600, color: 'rgba(255,255,255,0.88)', fontSize: 12 }}>
            {label}
          </span>
          {effectCfg && (
            <span style={{ fontSize: 10, color: effectCfg.color, fontWeight: 600 }}>
              {effectCfg.icon} {effectCfg.label}
            </span>
          )}
          {at && (
