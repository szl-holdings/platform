...
> = {
  healthy: {
    ...
    border: 'rgba(107,143,113,0.20)',
    label: 'Live',
  },
  degraded: {
    color: '#c8953c',
    bg: 'rgba(200,149,60,0.10)',
    border: 'rgba(200,149,60,0.20)',
    label: 'Degraded',
  },
  stale: {
    color: '#c45a4a',
    bg: 'rgba(196,90,74,0.10)',
    border: 'rgba(196,90,74,0.20)',
    label: 'Stale',
  },
  disconnected: {
    color: '#7c85a0',
    bg: 'rgba(124,133,160,0.10)',
    border: 'rgba(124,133,160,0.20)',
    label: 'Disconnected',
  },
};
  ...
  return (
    <div className="relative inline-flex">
      <button
        ...
        className={cn(
          'inline-flex items-center gap-1.5 rounded px-2 py-0.5 text-[10px] font-medium transition-opacity hover:opacity-80',
          className,
        )}
      ...
      >
        ...
        <span style={{ color: 'rgba(255,255,255,0.35)' }}>
          {freshnessLabel(source.freshnessSeconds)}
        </span>
      </button>
        ...
        <>
          ...
          >
            ...
            <div className="space-y-1 text-[10px]" style={{ color: 'rgba(255,255,255,0.5)' }}>
              <div>
                Connector:{' '}
                <span style={{ color: 'rgba(255,255,255,0.72)' }}>{source.connector}</span>
              </div>
              <div>
                Last seen:{' '}
              ...
              {source.latencyMs && (
                <div>
                  Latency:{' '}
                  <span style={{ color: 'rgba(255,255,255,0.72)' }}>{source.latencyMs}ms</span>
                </div>
              )}
              {source.errorMessage && (
                <div
    ...
    >
      ...
      {hasIssues && (
        <span className="ml-auto text-[9px] font-mono" style={{ color: '#c8953c' }}>
          {degraded.length} source{degraded.length !== 1 ? 's' : ''} degraded — affected widgets
          marked below
        </span>
      )}
    </div>
...
export function FreshnessPill({
  ...
  return (
    <span
      className={cn(
        'inline-flex items-center gap-1 rounded text-[9px] font-mono px-1.5 py-0.5',
        className,
      )}
      style={{ background: style.bg, color: style.color, border: `1px solid ${style.border}` }}
