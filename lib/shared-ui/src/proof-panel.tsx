...
const EXPORT_CONFIG: Record<ProofExportSafetyState, { label: string; color: string; icon: string; description: string }> = {
  safe: { label: "Export Safe", color: "#6b8f71", icon: "✓", description: "Cleared for external distribution" },
  restricted: { label: "Internal Only", color: "#c8953c", icon: "⚠", description: "Restricted — internal use only" },
  pending_review: { label: "Pending Review", color: "#4a90b8", icon: "⏳", description: "Requires human review before export" },
  blocked: { label: "Export Blocked", color: "#ef4444", icon: "🚫", description: "Blocked — flagged or retracted content" },
};
...
function ConfidenceBar({ value }: { value: number }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 700, color, minWidth: 30, textAlign: "right", fontFamily: "monospace" }}>{pct}%</span>
    </div>
  );
}
...
function Row({ label, value, mono }: { label: string; value: React.ReactNode; mono?: boolean }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, padding: "3px 0" }}>
      ...
      <span style={{ fontSize: 11, color: TEXT.primary, fontFamily: mono ? "monospace" : undefined, textAlign: "right", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 200 }}>
        {value}
      </span>
    </div>
  );
...
export function ProofPanelBadge({ proof, accentColor = "#8b5cf6" }: { proof: ProofPanelData; accentColor?: string }) {
    ...
    <div style={{ position: "relative", display: "inline-block" }}>
      <button
        ...
        style={{
          ...
          borderRadius: 5, cursor: "pointer", fontSize: 10, color: TEXT.secondary, fontFamily: "monospace",
        }}
      >
      ...
      {open && (
        <div style={{
          ...
          background: "#0f1219", border: `1px solid ${BORDER.muted}`, borderRadius: 10,
...
function ProofPanelInline({ proof, accentColor = "#8b5cf6", onReview, showActions }: { proof: ProofPanelData; accentColor?: string; onReview?: (s: ProofReviewState) => void | Promise<void>; showActions?: boolean }) {
  const review = REVIEW_CONFIG[proof.reviewState];
  const exportCfg = EXPORT_CONFIG[proof.exportSafetyState];
  ...
  return (
    <div style={{ padding: 14, fontSize: 12, display: "flex", flexDirection: "column", gap: 12 }}>
      {/* Header */}
      <div style={{ display: "flex", alignItems: "center", gap: 8, paddingBottom: 10, borderBottom: `1px solid ${BORDER.subtle}` }}>
        <span style={{ fontSize: 16 }}>{SOURCE_ICONS[proof.sourceClass]}</span>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 600, color: TEXT.primary, fontSize: 12 }}>{SOURCE_LABELS[proof.sourceClass]}</div>
          {proof.serviceAttribution && (
            <div style={{ fontSize: 10, color: TEXT.tertiary, fontFamily: "monospace" }}>{proof.serviceAttribution}</div>
          )}
        </div>
        <span style={{
          fontSize: 10, fontWeight: 600, color: review.color,
          background: `${review.color}18`, padding: "2px 7px", borderRadius: 4,
        }}>
      ...
      {(proof.actorAttribution || proof.reviewedBy) && (
        <div>
          ...
          {proof.reviewNote && (
            <div style={{ marginTop: 4, padding: "4px 8px", background: BG.elevated, borderRadius: 5, fontSize: 11, color: TEXT.secondary, fontStyle: "italic" }}>
              "{proof.reviewNote}"
            </div>
          )}
        </div>
      ...
      {/* Generated At */}
      {proof.generatedAt && (
        <Row label="Generated" value={timeAgo(proof.generatedAt)} />
      )}
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
            {proof.inputSources.map((src, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: TEXT.secondary }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: accentColor, opacity: 0.5, flexShrink: 0 }} />
      ...
      {proof.lineage && proof.lineage.length > 0 && (
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 2, paddingLeft: 8, borderLeft: `2px solid ${accentColor}40` }}>
            {proof.lineage.map((l, i) => (
              <div key={i} style={{ fontSize: 10, color: TEXT.secondary, display: "flex", alignItems: "center", gap: 5 }}>
      ...
      {proof.contradictionMarkers && proof.contradictionMarkers.length > 0 && (
        <div style={{ padding: "6px 10px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.18)", borderRadius: 6 }}>
          <SectionLabel>⚠ Contradiction Markers</SectionLabel>
          {proof.contradictionMarkers.map((m, i) => (
            <div key={i} style={{ fontSize: 11, color: "#ef8a8a" }}>• {m}</div>
          ))}
        </div>
      )}
      ...
      <div style={{
        ...
        background: `${exportCfg.color}10`, border: `1px solid ${exportCfg.color}28`,
        borderRadius: 6,
      }}>
        <span style={{ color: exportCfg.color, fontSize: 13 }}>{exportCfg.icon}</span>
        <div>
          <div style={{ fontSize: 11, fontWeight: 600, color: exportCfg.color }}>{exportCfg.label}</div>
      ...
      {showActions && onReview && proof.reviewState === "unreviewed" && (
        <div style={{ display: "flex", gap: 6, paddingTop: 4, borderTop: `1px solid ${BORDER.subtle}` }}>
          <button
            ...
            onClick={async () => { setReviewLoading(true); await onReview("approved"); setReviewLoading(false); }}
            style={{ flex: 1, padding: "6px 0", background: "#6b8f7120", border: "1px solid #6b8f7140", borderRadius: 5, color: "#6b8f71", fontSize: 11, cursor: "pointer", fontWeight: 600 }}
          >
            ✓ Approve
          </button>
          <button
            disabled={reviewLoading}
            onClick={async () => { setReviewLoading(true); await onReview("flagged"); setReviewLoading(false); }}
            style={{ flex: 1, padding: "6px 0", background: "#ef444420", border: "1px solid #ef444440", borderRadius: 5, color: "#ef4444", fontSize: 11, cursor: "pointer", fontWeight: 600 }}
          >
            ⚑ Flag
          </button>
        </div>
...
export function ProofPanel({ proof, variant = "inline", accentColor = "#8b5cf6", className, onReview, showActions }: ProofPanelProps) {
  if (variant === "badge") {
  ...
  if (variant === "drawer") {
    return (
      <div className={className} style={{ ...containerStyle, maxWidth: 320 }}>
        <div style={{ padding: "10px 14px", background: BG.header, borderBottom: `1px solid ${BORDER.subtle}`, display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: accentColor, textTransform: "uppercase", letterSpacing: "0.08em" }}>Proof Chain</span>
          <span style={{ marginLeft: "auto", fontSize: 9, color: TEXT.tertiary, fontFamily: "monospace" }}>
  ...
  return (
    <div className={className} style={containerStyle}>
      <div style={{ padding: "8px 12px", background: BG.header, borderBottom: `1px solid ${BORDER.subtle}`, display: "flex", alignItems: "center", gap: 6 }}>
        <span style={{ fontSize: 10, fontWeight: 700, color: TEXT.secondary, textTransform: "uppercase", letterSpacing: "0.08em" }}>
          🔐 Proof Chain
        </span>
        {proof.proofId && (
...
export function ProofPanelRow({ proof, accentColor = "#8b5cf6" }: { proof: ProofPanelData; accentColor?: string }) {
  const review = REVIEW_CONFIG[proof.reviewState];
  ...
  return (
    <div style={{
      ...
      background: BG.surface, border: `1px solid ${BORDER.subtle}`, borderRadius: 7, fontSize: 11,
    }}>
      <span style={{ fontSize: 13 }}>{SOURCE_ICONS[proof.sourceClass]}</span>
      <span style={{ color: TEXT.secondary, flexShrink: 0 }}>{SOURCE_LABELS[proof.sourceClass]}</span>
      {proof.modelId && <span style={{ color: TEXT.tertiary, fontFamily: "monospace", fontSize: 10 }}>{proof.modelId}</span>}
      <span style={{ marginLeft: "auto", color: confColor, fontWeight: 700, fontFamily: "monospace" }}>{pct}%</span>
      <span style={{ color: review.color, fontSize: 10, fontWeight: 600 }}>{review.icon} {review.label}</span>
