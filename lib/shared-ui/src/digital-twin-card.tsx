...
import { Activity, AlertTriangle, CheckCircle, ChevronDown, ChevronUp, Clock, Play, TrendingDown, TrendingUp, Zap } from "lucide-react";
...
function AlertBadge({ alert }: { alert: TwinAlert }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: "6px", padding: "6px 8px", background: `${color}10`, border: `1px solid ${color}30`, borderRadius: "6px", marginBottom: "4px" }}>
      <AlertTriangle size={11} color={color} style={{ flexShrink: 0, marginTop: "1px" }} />
      <span style={{ fontSize: "11px", color: "rgba(255,255,255,0.75)", lineHeight: 1.4 }}>{alert.message}</span>
    </div>
  );
}

function MetricRow({ label, value }: { label: string; value: unknown }) {
  const display = typeof value === "number" ? (value > 100 ? value.toLocaleString() : value.toFixed ? value.toFixed(2) : String(value)) : String(value);
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "3px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
      ...
      <span style={{ fontSize: "11px", fontWeight: 600, color: "rgba(255,255,255,0.88)" }}>{display}</span>
    </div>
  );
}

function formatStateKeys(state: Record<string, unknown>): Array<{ label: string; value: unknown }> {
  const formatLabel = (key: string) => key.replace(/([A-Z])/g, " $1").replace(/_/g, " ").trim().replace(/^\w/, c => c.toUpperCase());
    ...
    <div style={{
      ...
      <div style={{ padding: "14px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "4px" }}>
          ...
          <span style={{ fontSize: "13px", fontWeight: 700, color: "rgba(255,255,255,0.88)", flex: 1 }}>{entityName}</span>
          <div style={{ padding: "2px 8px", background: statusCfg.bg, borderRadius: "20px" }}>
            <span style={{ fontSize: "10px", fontWeight: 600, color: statusCfg.color }}>{statusCfg.label}</span>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.35)" }}>{TYPE_LABELS[twinType]}</span>
          <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.2)" }}>·</span>
          <Clock size={9} color="rgba(255,255,255,0.3)" />
          <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.35)" }}>{syncedAgo}</span>
          <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.2)" }}>·</span>
          <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.35)" }}>Confidence: {(confidenceScore * 100).toFixed(0)}%</span>
        </div>
      </div>

      {(criticalAlerts.length > 0 || warningAlerts.length > 0) && (
        <div style={{ padding: "10px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          {criticalAlerts.slice(0, 2).map(a => <AlertBadge key={a.id} alert={a} />)}
          {warningAlerts.slice(0, 1).map(a => <AlertBadge key={a.id} alert={a} />)}
        </div>
      )}
      ...
      <div style={{ padding: "12px 16px" }}>
        ...
        {metrics.length > 5 && (
          <button
            ...
            style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.35)", fontSize: "11px", padding: "4px 0", display: "flex", alignItems: "center", gap: "4px", marginTop: "4px" }}
          >
            {expanded ? <><ChevronUp size={11} /> Show less</> : <><ChevronDown size={11} /> +{metrics.length - 5} more metrics</>}
          </button>
        )}
      </div>
      ...
      {predictedStates.length > 0 && (
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "10px 16px" }}>
          <button
            ...
            style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.5)", fontSize: "11px", display: "flex", alignItems: "center", gap: "6px", padding: 0, width: "100%" }}
          >
          ...
          {showPredictions && (
            <div style={{ marginTop: "8px", display: "flex", flexDirection: "column", gap: "6px" }}>
              {predictedStates.slice(0, 3).map((ps, idx) => (
                <div key={idx} style={{ background: "rgba(255,255,255,0.03)", borderRadius: "6px", padding: "8px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                    <span style={{ fontSize: "11px", fontWeight: 600, color: "rgba(255,255,255,0.7)" }}>T+{ps.timeHorizon}</span>
                    <span style={{ fontSize: "10px", color: accentColor }}>{(ps.confidence * 100).toFixed(0)}% confidence</span>
                  </div>
                  {formatStateKeys(ps.state).slice(0, 3).map(m => (
                    <MetricRow key={m.label} label={m.label} value={m.value} />
                  ))}
                  {ps.drivingFactors.length > 0 && (
                    <p style={{ fontSize: "10px", color: "rgba(255,255,255,0.3)", margin: "4px 0 0" }}>Driven by: {ps.drivingFactors.slice(0, 2).join(", ")}</p>
                  )}
                </div>
              ))}
            </div>
