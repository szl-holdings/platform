...
const BORDER = { subtle: "rgba(255,255,255,0.04)", muted: "rgba(255,255,255,0.07)", strong: "rgba(255,255,255,0.10)" };
...
const ACTION_BUTTON: Record<string, { color: string; bg: string; border: string; label: string }> = {
  approve:   { color: "#6b8f71", bg: "rgba(107,143,113,0.12)", border: "rgba(107,143,113,0.28)", label: "Approve" },
  reject:    { color: "#c45a4a", bg: "rgba(196,90,74,0.12)",   border: "rgba(196,90,74,0.28)",   label: "Reject" },
  escalate:  { color: "#c8953c", bg: "rgba(200,149,60,0.12)",  border: "rgba(200,149,60,0.28)",  label: "Escalate" },
  rollback:  { color: "#7c85a0", bg: "rgba(124,133,160,0.12)", border: "rgba(124,133,160,0.28)", label: "Rollback" },
  defer:     { color: "#4a90b8", bg: "rgba(74,144,184,0.12)",  border: "rgba(74,144,184,0.28)",  label: "Defer" },
};
...
function EvidenceInlineDrawer({ rec, onClose }: { rec: Recommendation; onClose: () => void }) {
  ...
  return (
    <>
      <div className="fixed inset-0 z-40 backdrop-blur-sm" onClick={onClose} aria-hidden="true"
        style={{ background: "rgba(12,16,24,0.75)" }} />
      <aside className="fixed top-0 right-0 bottom-0 z-50 w-full max-w-md flex flex-col"
        style={{ background: BG.elevated, borderLeft: `1px solid ${BORDER.muted}` }}
        role="dialog" aria-label="Evidence" aria-modal="true">
        <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: `1px solid ${BORDER.subtle}` }}>
          <div>
            <div className="text-sm font-semibold" style={{ color: TEXT.primary }}>Evidence</div>
          ...
          <button onClick={onClose} className="rounded p-1.5 transition-colors hover:bg-white/5"
            style={{ color: TEXT.tertiary }} aria-label="Close">✕</button>
        </div>

        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          <div className="text-[10px] uppercase tracking-widest font-mono mb-3" style={{ color: TEXT.muted }}>
            Rationale
          </div>
          ...
          <div className="text-[10px] uppercase tracking-widest font-mono mb-2" style={{ color: TEXT.muted }}>
            Signal sources ({rec.evidence.length})
          </div>
          <div className="space-y-2">
            {rec.evidence.map((ev) => (
              <div key={ev.id} className="rounded-lg p-3" style={{ background: BG.card, border: `1px solid ${BORDER.subtle}` }}>
                <div className="flex items-start justify-between gap-2 mb-1.5">
                  <div>
                    <div className="text-[11px] font-medium" style={{ color: TEXT.primary }}>{ev.sourceName}</div>
                    <div className="text-[9px] font-mono mt-0.5" style={{ color: TEXT.muted }}>{ev.sourceType}</div>
                  </div>
                  <div className="text-right shrink-0">
                    ...
                    <div className="text-[9px] font-mono" style={{ color: TEXT.muted }}>
                      {ev.freshnessSeconds < 60 ? `${ev.freshnessSeconds}s` : `${Math.round(ev.freshnessSeconds / 60)}m`} ago
                    </div>
                  </div>
                </div>
                <p className="text-[11px] leading-relaxed" style={{ color: TEXT.secondary }}>{ev.content}</p>
                {ev.lineage?.length ? (
                  <div className="mt-2 flex items-center gap-1 flex-wrap">
                    {ev.lineage.map((step, i) => (
                      <React.Fragment key={i}>
                        <span className="text-[9px] font-mono rounded px-1.5 py-0.5"
                          style={{ background: "rgba(255,255,255,0.04)", color: TEXT.tertiary }}>{step}</span>
          ...
          {rec.runId && (
            <div className="mt-4 rounded-lg px-3 py-2.5 flex items-center justify-between"
              style={{ background: "rgba(74,144,184,0.07)", border: "1px solid rgba(74,144,184,0.15)" }}>
              <div>
                <div className="text-[10px] uppercase tracking-widest font-mono" style={{ color: "rgba(74,144,184,0.6)" }}>Run</div>
                <div className="text-[11px] font-mono mt-0.5" style={{ color: "rgba(74,144,184,0.9)" }}>{rec.runId}</div>
              </div>
              <span className="text-[10px]" style={{ color: "rgba(74,144,184,0.6)" }}>View full run →</span>
            </div>
          )}
        </div>
      </aside>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-md rounded-xl p-6 space-y-4"
        ...
        <div className="text-sm font-semibold" style={{ color: TEXT.primary }}>Justification Required</div>
        <p className="text-[12px] leading-relaxed" style={{ color: TEXT.secondary }}>
          Policy requires a written justification for this action. It will be logged to the audit trail.
        </p>
        <textarea
          value={text}
          ...
          style={{
            background: BG.card, border: `1px solid ${BORDER.muted}`,
            color: TEXT.primary, caretColor: cfg.color,
          }}
        />
        <div className="flex gap-2 justify-end">
          <button onClick={onCancel} className="px-3 py-1.5 rounded text-[12px]"
            style={{ background: "rgba(255,255,255,0.05)", color: TEXT.secondary, border: `1px solid ${BORDER.subtle}` }}>
            Cancel
          </button>
          <button
            disabled={!text.trim()}
...
export interface RecommendationCardProps {
  rec: Recommendation;
  onAction?: (id: string, action: RecommendationAction, justification?: string) => void | Promise<void>;
...
export function RecommendationCard({
  ...
  const [auditLog, setAuditLog] = React.useState<Array<{ action: string; justification?: string; at: string }>>([]);
  ...
  const isExecuted = rec.status === "executed" || rec.status === "rolled_back" || rec.status === "expired";
      ...
      >
        ...
        >
          ...
          <div className="shrink-0 mt-0.5 flex flex-col items-center gap-1.5">
            <span className="text-[9px] font-bold font-mono rounded px-1.5 py-0.5"
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[11px] mt-1 leading-snug line-clamp-2" style={{ color: TEXT.secondary }}>
              {rec.summary}
            </div>
            <div className="flex flex-wrap items-center gap-2 mt-2">
              <PolicyVerdictBadge verdict={rec.policyVerdict.verdict} detail={rec.policyVerdict} size="xs" />
        ...
        {expanded && (
          <div style={{ borderTop: `1px solid ${BORDER.subtle}` }}>
            ...
            <div className="px-4 pt-3 pb-2">
              <div className="text-[9px] uppercase tracking-widest font-mono mb-1.5" style={{ color: TEXT.muted }}>
                Proposed action
              </div>
              <p className="text-[12px] leading-relaxed" style={{ color: TEXT.secondary }}>
                {rec.proposedAction}
            ...
            {/* Evidence + Run links */}
            <div className="flex items-center gap-3 px-4 py-2" style={{ borderTop: `1px solid ${BORDER.subtle}` }}>
              <button
                type="button"
              ...
              >
                <span>Evidence</span>
                <span className="rounded-full px-1 py-0.5 text-[9px]"
            ...
            {onAction && (
              <div className="px-4 py-3 space-y-2" style={{ borderTop: `1px solid ${BORDER.subtle}` }}>
                {rec.status === "executed" ? (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-mono ml-auto" style={{ color: TEXT.tertiary }}>Status: {rec.status}</span>
                  </div>
                ) : rec.status === "rolled_back" || rec.status === "expired" ? (
                  <div className="text-[11px] font-mono" style={{ color: TEXT.tertiary }}>Status: {rec.status}</div>
                ) : isHardBlocked ? (
                  <div className="flex items-center gap-2">
                    <span className="rounded px-2.5 py-1 text-[10px] font-medium"
                      style={{ background: "rgba(196,90,74,0.10)", color: "rgba(196,90,74,0.75)", border: "1px solid rgba(196,90,74,0.25)" }}>
                      {verdict === "red" ? "Red verdict — mandatory admin review" : "Blocked by policy — override requires administrator approval"}
                    </span>
                  </div>
                ) : (
                  <>
                    {mandatesJustification && (
                      <div className="text-[9px] font-mono px-0.5" style={{ color: "rgba(200,149,60,0.7)" }}>
                ...
                {auditLog.length > 0 && (
                  <div className="mt-2 space-y-1 pt-2" style={{ borderTop: `1px solid ${BORDER.subtle}` }}>
                    {auditLog.map((entry, i) => (
                      <div key={i} className="flex items-start gap-2 text-[9px] font-mono" style={{ color: TEXT.tertiary }}>
                        <span style={{ color: ACTION_BUTTON[entry.action]?.color ?? TEXT.muted }}>{entry.action}</span>
...
export function DecisionCenter({
  ...
  const filtered = recommendations.filter(r => {
    ...
    if (filter === "blocked") return r.policyVerdict.verdict === "blocked" || r.policyVerdict.verdict === "red";
    if (filter === "executed") return r.status === "executed" || r.status === "approved";
    return true;
  });

  const pendingCount = recommendations.filter(r => r.status === "pending").length;
  const blockedCount = recommendations.filter(r => r.policyVerdict.verdict === "blocked" || r.policyVerdict.verdict === "red").length;
            ...
            <div className="flex items-center gap-2">
              ...
              {pendingCount > 0 && (
                <span className="rounded-full px-2 py-0.5 text-[10px] font-medium"
                  style={{ background: "rgba(200,149,60,0.12)", color: "#c8953c", border: "1px solid rgba(200,149,60,0.25)" }}>
        ...
        <div className="flex gap-1 mt-4">
          {(["all", "pending", "blocked", "executed"] as const).map(f => (
            <button
              ...
              style={{
                background: filter === f ? (accentColor ? `${accentColor}18` : "rgba(255,255,255,0.07)") : "transparent",
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        {loading && (
          <div className="flex flex-col gap-3">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-24 rounded-xl animate-pulse" style={{ background: BG.card }} />
            ))}
          </div>
        )}
