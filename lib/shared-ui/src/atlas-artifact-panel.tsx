...
export type AtlasTemplateType =
  | "deck" | "brief" | "memo" | "executive_summary" | "report"
  | "approval_packet" | "incident_packet" | "readiness_report"
  | "proposal" | "voyage_report" | "property_brief" | "threat_assessment";
...
export function AtlasArtifactCard({ artifact, onOpen, onExport, onRegenerate, onShare, className }: AtlasArtifactCardProps) {
    ...
    >
      <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 500, color: TEXT.primary, marginBottom: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {artifact.title}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 10, color: TEXT.tertiary }}>{TEMPLATE_LABELS[artifact.templateType]}</span>
            <span style={{ fontSize: 10, color: TEXT.tertiary }}>v{artifact.version}</span>
            <span style={{ fontSize: 10, color: TEXT.tertiary, textTransform: "capitalize" }}>{artifact.domain}</span>
          </div>
        </div>
        <span
          style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }} onClick={e => e.stopPropagation()}>
        {onExport && (
          <div style={{ position: "relative" }}>
            <button
              ...
              style={{
                padding: "3px 9px", borderRadius: 5, border: `1px solid ${BORDER.muted}`,
            ...
            {showExportMenu && (
              <div style={{
                ...
                background: BG.elevated, border: `1px solid ${BORDER.muted}`, borderRadius: 8, overflow: "hidden", minWidth: 120,
              }}>
                {(["pdf", "docx", "pptx", "xlsx", "web"] as AtlasExportFormat[]).map(fmt => (
                  <button
                    key={fmt}
                    onClick={() => { onExport(artifact.id, fmt); setShowExportMenu(false); }}
                    style={{
                      display: "block", width: "100%", textAlign: "left", padding: "6px 12px",
                      background: "none", border: "none", color: TEXT.secondary, fontSize: 11, cursor: "pointer",
                    }}
        ...
        {onRegenerate && (
          <button
            ...
            style={{
              padding: "3px 9px", borderRadius: 5, border: `1px solid ${ACCENT.blue}40`,
              background: `${ACCENT.blue}10`, color: ACCENT.blue, fontSize: 11, cursor: "pointer",
            }}
          >Regenerate</button>
        )}
        {onShare && (
...
export function AtlasArtifactViewer({ artifact, onExport, onRegenerate, onShare, onBack, className }: AtlasArtifactViewerProps) {
  const [activeSection, setActiveSection] = useState<string | null>(
    Array.isArray(artifact.sections) && artifact.sections.length > 0 ? artifact.sections[0]?.id ?? null : null
  );

  const sections = Array.isArray(artifact.sections) ? artifact.sections as AtlasArtifactSection[] : [];
  const currentSection = sections.find(s => s.id === activeSection);

  return (
    <div className={className} style={{ display: "flex", flexDirection: "column", height: "100%", gap: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 20px", borderBottom: `1px solid ${BORDER.muted}` }}>
        {onBack && (
          <button onClick={onBack} style={{ background: "none", border: "none", color: TEXT.secondary, cursor: "pointer", fontSize: 13, padding: 0 }}>← Back</button>
        )}
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: TEXT.tertiary }}>
            ...
            {!artifact.isLatest && <span style={{ color: ACCENT.amber, marginLeft: 8 }}>Not latest version</span>}
          </div>
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          {onRegenerate && (
            <button onClick={onRegenerate} style={{ padding: "5px 12px", borderRadius: 6, border: `1px solid ${ACCENT.blue}40`, background: `${ACCENT.blue}10`, color: ACCENT.blue, fontSize: 11, cursor: "pointer" }}>
              Regenerate
            </button>
          )}
          {onShare && (
            <button onClick={onShare} style={{ padding: "5px 12px", borderRadius: 6, border: `1px solid ${BORDER.muted}`, background: BG.elevated, color: TEXT.secondary, fontSize: 11, cursor: "pointer" }}>
              Share
            </button>
          )}
          {onExport && (
            <button onClick={() => onExport("pdf")} style={{ padding: "5px 12px", borderRadius: 6, border: `1px solid ${ACCENT.gold}40`, background: `${ACCENT.gold}10`, color: ACCENT.gold, fontSize: 11, cursor: "pointer" }}>
              Export PDF
            </button>
          )}
        </div>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        {sections.length > 0 && (
          <div style={{ width: 200, borderRight: `1px solid ${BORDER.muted}`, padding: "12px 0", overflowY: "auto", flexShrink: 0 }}>
            {sections.map(s => (
              <button
                key={s.id}
                ...
                style={{
                  ...
                  border: "none", borderLeft: activeSection === s.id ? `2px solid ${ACCENT.blue}` : "2px solid transparent",
        ...
        <div style={{ flex: 1, padding: "20px 24px", overflowY: "auto" }}>
          {currentSection ? (
            <div>
              <h2 style={{ fontSize: 15, fontWeight: 600, color: TEXT.primary, marginBottom: 12 }}>{currentSection.title}</h2>
              <div style={{ fontSize: 13, color: TEXT.secondary, lineHeight: 1.7, whiteSpace: "pre-wrap" }}>
                {currentSection.content || <span style={{ color: TEXT.tertiary, fontStyle: "italic" }}>No content in this section.</span>}
              </div>
            </div>
          ) : (
            <div style={{ color: TEXT.tertiary, textAlign: "center", paddingTop: 60 }}>
              {sections.length === 0 ? "This artifact has no sections yet." : "Select a section to view."}
            </div>
          )}
        </div>
      </div>
...
export function AtlasArtifactPanel({ apiBaseUrl = "/api-server", domain, entityType, entityId, onArtifactCreated, className }: AtlasArtifactPanelProps) {
  ...
  const handleOpen = useCallback(async (artifact: AtlasArtifactMeta) => {
    const res = await fetch(`${apiBaseUrl}/atlas/artifacts/${artifact.id}`);
    const d = await res.json();
    if (d.data) setSelectedArtifact(d.data);
  }, [apiBaseUrl]);

  const handleExport = useCallback(async (artifactId: number, format: AtlasExportFormat) => {
  ...
  const handleShare = useCallback(async (artifactId: number) => {
  ...
  return (
    <div className={className} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 11, color: TEXT.tertiary }}>{artifacts.length} artifact{artifacts.length !== 1 ? "s" : ""}</span>
      </div>
      {loading ? (
        <div style={{ color: TEXT.secondary, fontSize: 12, padding: 20, textAlign: "center" }}>Loading…</div>
      ) : artifacts.length === 0 ? (
        <div style={{ color: TEXT.tertiary, fontSize: 12, padding: 30, textAlign: "center", background: BG.card, border: `1px solid ${BORDER.subtle}`, borderRadius: 10 }}>
