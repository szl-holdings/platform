...
const AVATAR_COLORS = [
  "#8b7ac8", "#0ea5e9", "#22c55e", "#ef4444", "#f59e0b",
  "#a855f7", "#ec4899", "#06b6d4", "#f97316", "#14b8a6",
];
...
const DEMO_PARTICIPANTS: SessionParticipant[] = [
  { userId: "user-1", displayName: "Stephen L.", avatarColor: "#8b7ac8", avatarInitials: "SL", lastSeen: Date.now() - 5000 },
  { userId: "user-2", displayName: "Carlota J.", avatarColor: "#ec4899", avatarInitials: "CJ", lastSeen: Date.now() - 12000 },
  { userId: "user-3", displayName: "Marcus T.", avatarColor: "#0ea5e9", avatarInitials: "MT", lastSeen: Date.now() - 30000 },
];
    ...
    >
      ...
      {showTooltip && hovered && (
        ...
        >
          ...
          {!isActive && <span style={{ color: "rgba(255,255,255,0.35)", marginLeft: "4px" }}>(away)</span>}
        </div>
      )}
    </div>
  );
...
export function SessionPresenceBar({
  ...
  return (
    <div style={{ position: "relative", display: "inline-flex", alignItems: "center", gap: "8px", fontFamily: typography.fontFamily.body }}>
      <div
        style={{
          display: "flex",
          alignItems: "center",
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
          <div style={{ width: "6px", height: "6px", borderRadius: "50%", background: activeCount > 0 ? "#22c55e" : "rgba(255,255,255,0.2)" }} />
      ...
      {showPanel && (
        ...
        >
          <div style={{ padding: "12px 14px", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            <div style={{ fontSize: "10px", fontWeight: 700, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: "1px" }}>
            ...
            {participants.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: "12px", fontWeight: 600, color: isActive ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.4)" }}>
                      {p.displayName}
                      {p.displayName === currentUserName && (
                        <span style={{ fontSize: "9px", color: accentColor, marginLeft: "6px", background: `${accentColor}20`, borderRadius: "3px", padding: "1px 4px" }}>you</span>
                      )}
                    </div>
                    <div style={{ fontSize: "10px", color: "rgba(255,255,255,0.3)", marginTop: "1px" }}>
                      {isActive ? "Active now" : `${Math.round((Date.now() - p.lastSeen) / 60000)}m ago`}
                    </div>
                  </div>
                  <div style={{ width: "6px", height: "6px", borderRadius: "50%", background: isActive ? "#22c55e" : "rgba(255,255,255,0.15)", flexShrink: 0 }} />
                </div>
              );
            })}
          </div>
      ...
      {isOpen && (
        ...
        >
          <div style={{ padding: "10px 14px", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            <div style={{ fontSize: "10px", fontWeight: 700, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: "0.8px" }}>
              {entityLabel}
            </div>
          </div>
          ...
          {comments.length > 0 ? (
              ...
              {comments.map((c) => (
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "5px" }}>
                    ...
                    <span style={{ fontSize: "11px", fontWeight: 600, color: "rgba(255,255,255,0.7)" }}>{c.authorName}</span>
                    <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.3)", marginLeft: "auto" }}>
                      {new Date(c.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                    </span>
                    {c.resolved && (
                      <span style={{ fontSize: "9px", color: "#22c55e", background: "#22c55e20", borderRadius: "4px", padding: "1px 5px" }}>
                        resolved
                      </span>
                    )}
                  </div>
            ...
            <div style={{ padding: "20px 14px", textAlign: "center", fontSize: "12px", color: "rgba(255,255,255,0.25)" }}>
              No comments yet
            </div>
          )}
          ...
          <div style={{ padding: "10px 14px", borderTop: "1px solid rgba(255,255,255,0.07)" }}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "6px", gap: "6px" }}>
              <button
                ...
                style={{ padding: "5px 10px", borderRadius: "6px", border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: "rgba(255,255,255,0.4)", fontSize: "11px", cursor: "pointer", fontFamily: typography.fontFamily.body }}
              >
                Close
              </button>
              <button
  ...
  useEffect(() => {
    ...
    async function fetchSession() {
      try {
        ...
        if (!cancelled && Array.isArray(json.data?.participants) && (json.data?.participants?.length ?? 0) > 0) {
    ...
    void fetchSession();
    const interval = setInterval(() => { void fetchSession(); }, 15000);
