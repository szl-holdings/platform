      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-6">
        ...
        <div className="grid grid-cols-3 gap-2">
          {LENSES.map((lens) => (
            <div
              key={lens.id}
              className={`text-center p-2.5 rounded-lg bg-gradient-to-br ${lens.color} bg-opacity-10 border border-white/5`}
            >
              <span className="text-sm font-bold text-white/80">{LENS_ICONS[lens.id]}</span>
              <p className="text-[10px] text-white/60 mt-1 leading-tight font-medium">
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center gap-3 mb-5">
          ...
          <span className="text-xs text-white/30 bg-white/5 px-2 py-0.5 rounded-md">
            Every domain. Every decision.
          </span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {LENSES.map((lens) => (
            <div
              key={lens.id}
              className="group rounded-xl border border-white/10 bg-white/[0.02] p-5 hover:bg-white/[0.04] transition-all hover:border-white/20"
            >
              <div className="flex items-center gap-3 mb-3">
                <div
                  className={`w-9 h-9 rounded-lg bg-gradient-to-br ${lens.color} flex items-center justify-center shadow-sm`}
                >
              ...
              <p className="text-xs text-white/30 italic leading-relaxed line-clamp-3">
                {lens.philosophy}
              </p>
        ...
        <div className="flex gap-2">
          {philosophy.maturityModel.map((level) => {
            ...
            return (
              <div
                key={level.level}
                className={`flex-1 rounded-xl border bg-gradient-to-t ${colors[level.level - 1]} p-4`}
              >
                <div className="flex items-center gap-2 mb-1">
                  <span className={`text-xs font-bold ${labelColors[level.level - 1]}`}>
                    L{level.level}
                  </span>
                ...
                {level.level === 5 && (
                  <div className="mt-2 flex items-center gap-1">
                    <span className="text-[10px] font-semibold text-[#8b7ac8] bg-violet-400/10 px-1.5 py-0.5 rounded">
                      Lens-Native
                    </span>
                  </div>
                )}
...
export function IntelligenceMaturityBadge({ level }: { level: number }) {
  ...
  const bgs = [
    'bg-[#c45a4a]/10 border-[#c45a4a]/20',
    'bg-[#d4a054]/10 border-[#d4a054]/20',
    'bg-blue-400/10 border-[#4a90b8]/20',
    'bg-[#6b8f71]/10 border-[#6b8f71]/20',
    'bg-violet-400/10 border-[#8b7ac8]/20',
  ];

  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${colors[level - 1]} ${bgs[level - 1]}`}
    >
...
export function LensTag({ lensId }: { lensId: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-semibold bg-gradient-to-r ${lens.color} bg-opacity-10 text-white/70`}
    >
