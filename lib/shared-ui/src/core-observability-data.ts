...
export type ProductId = "beacon" | "lyte" | "nimbus" | "alloyscape";
...
export const PREDICTIONS: PredictionEntity[] = [
  {
    ...
    impact_estimate: 840000,
    driving_signals: ["48h approval aging", "Legal team capacity at 94%", "Historical Q1 close patterns"],
    linked_beacon_events: ["evt-001", "evt-002"],
    linked_lyte_states: ["apr-001", "wf-001"],
    rationale: "The Northgate contract has been in legal review for 48 hours against a 24-hour SLA. Legal capacity is at 94%. Q1 close is in 6 days. Historical data shows 73% of approvals aging past 48h miss the quarter.",
    recommended_action: "Escalate immediately to VP Legal with executive sponsor engagement. Consider parallel path: pre-execute pending final signature with risk acceptance from CFO.",
    correlation_id: GOLDEN_FLOW_CORRELATION_ID,
  ...
  },
  {
    ...
    driving_signals: ["Unowned workflow step", "Compliance queue depth +40%", "6 similar cases blocked"],
    linked_beacon_events: ["evt-003"],
    linked_lyte_states: ["wf-003", "apr-003"],
    rationale: "Apex Logistics onboarding has no assigned owner for the compliance step. Compliance queue depth increased 40% this week. 6 other vendor onboardings are behind this one.",
    recommended_action: "Assign dedicated compliance resource immediately. Escalate to CPO. Consider fast-track approval for low-risk vendors.",
    correlation_id: "corr-vendor-apex",
  ...
  },
  {
    ...
    impact_estimate: 480000,
    driving_signals: ["NPS drop -42 pts", "3 support escalations in 7 days", "Competitive renewal offer received"],
    linked_beacon_events: ["evt-004"],
    linked_lyte_states: ["wf-004", "apr-004"],
    rationale: "TechCorp's product usage dropped 35% last month. NPS score dropped 42 points. They received a competitive renewal offer at 20% below current contract. Without executive contact, historical models predict 88% churn probability.",
    recommended_action: "Immediate CEO-to-CEO outreach. Approve retention offer at 30% discount — NPV positive vs. churn cost. Assign dedicated CSM upgrade.",
    correlation_id: "corr-churn-techcorp",
  ...
  },
  {
    ...
    impact_estimate: 840000,
    driving_signals: ["Secondary approver available", "Parallel signature path validated", "Similar reroute success rate 91%"],
...
export const EXECUTION_RUNS: ExecutionRun[] = [
  {
    ...
    trigger_type: "event",
    correlation_id: GOLDEN_FLOW_CORRELATION_ID,
    exception_message: "Step 4 (Legal Review) — No approver assigned to secondary queue. Timeout after 30m.",
  ...
  },
  {
    ...
    steps_completed: 2,
    steps_failed: 0,
    triggered_by: "Beacon drift detection — NPS anomaly",
    trigger_type: "event",
    correlation_id: "corr-churn-techcorp",
  },
  {
...
export const CONNECTORS: ConnectorEntity[] = [
  { id: "conn-sf", name: "Salesforce CRM", system: "Salesforce", status: "connected", last_sync: new Date(Date.now() - 1000 * 60 * 5).toISOString(), sync_count_24h: 1847, error_count_24h: 3, data_volume_mb: 42.3 },
  { id: "conn-jira", name: "Jira Project Management", system: "Atlassian", status: "connected", last_sync: new Date(Date.now() - 1000 * 60 * 12).toISOString(), sync_count_24h: 924, error_count_24h: 0, data_volume_mb: 18.7 },
  { id: "conn-slack", name: "Slack Notifications", system: "Slack", status: "connected", last_sync: new Date(Date.now() - 1000 * 60 * 1).toISOString(), sync_count_24h: 3421, error_count_24h: 12, data_volume_mb: 8.1 },
  { id: "conn-sage", name: "Sage ERP — Finance", system: "Sage", status: "degraded", last_sync: new Date(Date.now() - 1000 * 60 * 47).toISOString(), sync_count_24h: 234, error_count_24h: 47, data_volume_mb: 91.4 },
  { id: "conn-docusign", name: "DocuSign eSignature", system: "DocuSign", status: "connected", last_sync: new Date(Date.now() - 1000 * 60 * 8).toISOString(), sync_count_24h: 127, error_count_24h: 0, data_volume_mb: 3.2 },
  { id: "conn-gh", name: "GitHub — Dev Signals", system: "GitHub", status: "connected", last_sync: new Date(Date.now() - 1000 * 60 * 3).toISOString(), sync_count_24h: 2810, error_count_24h: 1, data_volume_mb: 24.6 },
  { id: "conn-stripe", name: "Stripe — Revenue Events", system: "Stripe", status: "auth_failed", last_sync: new Date(Date.now() - 1000 * 60 * 180).toISOString(), sync_count_24h: 0, error_count_24h: 234, data_volume_mb: 0 },
  { id: "conn-zendesk", name: "Zendesk Support", system: "Zendesk", status: "connected", last_sync: new Date(Date.now() - 1000 * 60 * 6).toISOString(), sync_count_24h: 892, error_count_24h: 4, data_volume_mb: 15.9 },
];
...
export const EVENTS: ObsEvent[] = [
  {
    ...
    actor_type: "system",
    actor_name: "Beacon Drift Detection",
    severity: "critical",
    confidence: 99,
    status: "escalated",
    correlation_id: GOLDEN_FLOW_CORRELATION_ID,
  ...
  },
  {
    ...
    actor_type: "system",
    actor_name: "Beacon Workflow Monitor",
    severity: "critical",
    confidence: 100,
    status: "blocked",
    correlation_id: GOLDEN_FLOW_CORRELATION_ID,
  ...
  },
  {
    ...
    actor_type: "system",
    actor_name: "Beacon Ownership Monitor",
    severity: "high",
    confidence: 100,
    status: "blocked",
    correlation_id: "corr-vendor-apex",
  ...
  },
  {
    ...
    entity_name: "Enterprise Contract Approval — Northgate Systems",
    actor_type: "system",
    actor_name: "Beacon Verification",
    severity: "info",
    confidence: 87,
    status: "recovered",
    correlation_id: GOLDEN_FLOW_CORRELATION_ID,
...
export const WORKFLOWS_UI: WorkflowUI[] = WORKFLOWS.map(w => {
  ...
  const stateMap: Record<EntityState, EntityState> = {
    healthy: "healthy", degraded: "degraded", blocked: "blocked",
    pending_approval: "pending_approval", escalated: "escalated",
    executing: "executing", retried: "retried", completed: "completed",
    failed: "failed", recovered: "recovered",
  };

  const slaMap: Record<string, string> = {
    "wf-001": "2026-03-31", "wf-002": "2026-03-31", "wf-003": "2026-04-02",
    "wf-004": "2026-03-31", "wf-005": "2026-04-07", "wf-006": "2026-03-31",
  };
...
export const CONNECTORS_UI: ConnectorUI[] = CONNECTORS.map(c => ({
  ...
  error_rate: c.sync_count_24h > 0 ? Math.round((c.error_count_24h / c.sync_count_24h) * 100 * 10) / 10 : 100,
...
export const KPI_METRICS: KpiMetric[] = [
  { id: "arr", label: "Annual Recurring Revenue", value: 10200000, unit: "$", trend: 8.2, trend_direction: "up", status: "healthy" },
  { id: "churn", label: "Churn Rate", value: "2.1", unit: "%", trend: -0.4, trend_direction: "up", status: "warning" },
  { id: "win_rate", label: "Win Rate", value: "74", unit: "%", trend: 3.1, trend_direction: "up", status: "healthy" },
  { id: "approval_sla", label: "Approval SLA Compliance", value: "44", unit: "%", trend: -31, trend_direction: "down", status: "at_risk" },
  { id: "workflow_health", label: "Workflow Health Score", value: "61", unit: "%", trend: -14, trend_direction: "down", status: "warning" },
  { id: "time_to_action", label: "Time to Action", value: "18.7", unit: "min", trend: -8, trend_direction: "down", status: "healthy" },
];

export const KPI_FRAMEWORK = {
  global: {
    time_to_insight: { label: "Time to Insight", value: "4.2m", trend: -12, unit: "min", description: "Avg time from event to surfaced insight" },
    time_to_action: { label: "Time to Action", value: "18.7m", trend: -8, unit: "min", description: "Avg time from insight to intervention" },
    value_at_risk: { label: "Value at Risk", value: "$4.02M", trend: 23, unit: "USD", description: "Total business value in at-risk workflows" },
    value_recovered: { label: "Value Recovered", value: "$840K", trend: 100, unit: "USD", description: "Value recovered via interventions (last 30d)" },
...
export const COMMAND_LOOP_PHASES: { phase: CommandPhase; label: string; description: string; product: ProductId; color: string }[] = [
  ...
  { phase: "DECIDE", label: "Decide", description: "Model scenarios and recommend actions", product: "nimbus", color: "#8b5cf6" },
...
export const DRIFT_EVENTS = [
  ...
  },
  {
    ...
    contributing_factors: ["Procurement team reorganization", "Process gap not updated", "No escalation trigger configured"],
  ...
  },
  {
    ...
    severity: "high" as Severity,
    contributing_factors: ["3 support escalations", "Feature adoption dropped 35%", "Competitive offer received"],
