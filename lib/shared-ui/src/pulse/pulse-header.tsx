...
export function PulseHeader({ title, subtitle, accentColor = "#d4a054" }: { title: string; subtitle: string; accentColor?: string }) {
    ...
    <m.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-xl font-bold tracking-tight" style={{ color: "rgba(255,255,255,0.9)" }}>{title}</h1>
          <span className="px-2 py-0.5 rounded text-[10px] font-semibold tracking-wider"
            style={{ background: `${accentColor}20`, color: accentColor, border: `1px solid ${accentColor}30` }}>LIVE</span>
        </div>
        <p className="text-[13px]" style={{ color: "rgba(255,255,255,0.3)" }}>{subtitle}</p>
      </div>
      <div className="flex items-center gap-5">
        ...
        <div className="text-right">
          ...
          <div className="text-lg font-bold tabular-nums" style={{ color: "rgba(255,255,255,0.5)" }}>{time.toUTCString().slice(17, 25)}</div>
        </div>
      </div>
    </m.div>
  );
