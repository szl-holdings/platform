...
export function PulseTechStack({ items, title = "Platform Architecture" }: { items: TechItem[]; title?: string }) {
  return (
    <m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[10px] font-medium uppercase tracking-wider" style={{ color: "rgba(255,255,255,0.3)" }}>{title}</span>
      </div>
