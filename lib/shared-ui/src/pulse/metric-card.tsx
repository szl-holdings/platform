...
export function PulseMetricCard({ label, value, suffix, icon: Icon, color, trend, delay = 0 }: {
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      ...
      const timer = setInterval(() => {
        start += step;
        if (start >= value) { setDisplayValue(value); clearInterval(timer); } else setDisplayValue(start);
      }, 20);
