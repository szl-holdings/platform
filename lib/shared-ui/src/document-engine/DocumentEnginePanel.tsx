...
import { FileText, Plus, Search, Edit3, Eye, Trash2, Download, Send, Clock, CheckCircle, XCircle, AlertCircle, ChevronRight, X, Save, History, MessageSquare, Printer, BookOpen, Layers, Pen } from "lucide-react";
import { cn } from "../utils";
import { DocumentEditor, createEmptyDocument, type DocumentEditorContent } from "./editor";
import type { DocumentRecord, DocumentTemplate, AppSource, DocumentVersionMeta } from "./types";
...
export function DocumentEnginePanel({
  ...
  const openDocument = useCallback(async (doc: DocumentRecord) => {
    setSelectedDoc(doc);
    setEditorContent((doc.contentJson as unknown as DocumentEditorContent) || createEmptyDocument());
    setView("editor");
    try {
  ...
  const createDocument = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const templateDef = selectedTemplate ? DOCUMENT_TEMPLATES.find(t => t.slug === selectedTemplate) : null;
      ...
      const tempDoc: DocumentRecord = {
        ...
        contentJson: selectedTemplate ? (DOCUMENT_TEMPLATES.find(t => t.slug === selectedTemplate)?.content as unknown as Record<string, unknown> || {}) : {},
        status: "draft",
        ownerId: null,
        appSource,
        ...
        <div className="flex flex-col h-full space-y-4 p-6 overflow-auto">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-white/50 mt-0.5">WYSIWYG editor, e-signatures, and PDF generation</p>
            </div>
            <button
          ...
          {loading ? (
            <div className="flex items-center justify-center h-32 text-white/40 text-sm">Loading documents...</div>
          ) : filtered.length === 0 ? (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-white/40 mt-0.5 capitalize">{doc.type.replace(/_/g, " ")} · {new Date(doc.updatedAt).toLocaleDateString()}</p>
                    </div>
      ...
      {view === "editor" && selectedDoc && (
        <div className="flex flex-col h-full">
          <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10 bg-white/5 flex-shrink-0">
            <button onClick={() => setView("list")} className="text-white/50 hover:text-white text-xs flex items-center gap-1">
          ...
          <div className="flex-1 overflow-auto p-4">
            <DocumentEditor
              content={(selectedDoc.contentJson as unknown as DocumentEditorContent)?.blocks ? selectedDoc.contentJson as unknown as DocumentEditorContent : editorContent}
          ...
          {selectedDoc.signatures && selectedDoc.signatures.length > 0 && (
            <div className="border-t border-white/10 p-4 flex-shrink-0">
              <p className="text-xs font-semibold text-white/60 mb-2 flex items-center gap-1"><Pen className="w-3 h-3" /> Signature Status</p>
              <div className="flex flex-wrap gap-2">
                {selectedDoc.signatures.map(sig => {
                  const cfg = SIG_CONFIG[sig.status] || SIG_CONFIG.pending;
                  const Icon = cfg.icon;
                  return (
                    <div key={sig.id} className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-white/5 border border-white/10 text-xs">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-900 border border-white/20 rounded-2xl p-6 w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowNewDoc(false)}><X className="w-4 h-4 text-white/50" /></button>
            </div>
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs text-white/60 mb-1.5 block">Start from Template (optional)</label>
                <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto">
                  <button
                    onClick={() => setSelectedTemplate(null)}
                    className={cn("text-left p-3 rounded-lg border text-xs transition-colors", !selectedTemplate ? "border-indigo-500/50 bg-indigo-500/10 text-white" : "border-white/10 bg-white/5 text-white/60 hover:bg-white/10")}
                  >
                  ...
                  {appTemplates.map(t => (
                    <button
                      ...
                      className={cn("text-left p-3 rounded-lg border text-xs transition-colors", selectedTemplate === t.slug ? "border-indigo-500/50 bg-indigo-500/10 text-white" : "border-white/10 bg-white/5 text-white/60 hover:bg-white/10")}
                    >
            ...
            <div className="flex justify-end gap-2 mt-5">
              <button onClick={() => setShowNewDoc(false)} className="px-4 py-1.5 rounded-lg text-xs text-white/60 hover:text-white bg-white/5 hover:bg-white/10 transition-colors">Cancel</button>
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-900 border border-white/20 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowSignModal(false)}><X className="w-4 h-4 text-white/50" /></button>
            </div>
            <p className="text-xs text-white/50 mb-4">Add up to 5 signers. They'll receive a signing link.</p>
            <div className="space-y-3">
              {signerInputs.map((s, i) => (
                <div key={i} className="flex gap-2">
                  <input
                    value={s.name}
                    onChange={e => setSignerInputs(prev => prev.map((p, pi) => pi === i ? { ...p, name: e.target.value } : p))}
                  ...
                  <input
                    value={s.email}
                    onChange={e => setSignerInputs(prev => prev.map((p, pi) => pi === i ? { ...p, email: e.target.value } : p))}
                    placeholder="Email address"
                    className="flex-1 px-3 py-2 rounded-lg border border-white/20 bg-white/5 text-xs text-white placeholder:text-white/30 focus:outline-none"
                  />
                  {signerInputs.length > 1 && (
                    <button onClick={() => setSignerInputs(prev => prev.filter((_, pi) => pi !== i))} className="text-rose-400 hover:text-rose-300 p-1">
            ...
            <div className="flex justify-end gap-2 mt-5">
              <button onClick={() => setShowSignModal(false)} className="px-4 py-1.5 rounded-lg text-xs text-white/60 hover:text-white bg-white/5 hover:bg-white/10 transition-colors">Cancel</button>
              <button
...
function VersionsPanel({ doc, versions, onBack, accentColor }: { doc: DocumentRecord; versions: DocumentVersionMeta[]; onBack: () => void; accentColor: string }) {
  return (
    <div className="flex flex-col h-full p-6 space-y-4 overflow-auto">
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="text-white/50 hover:text-white text-xs flex items-center gap-1">
          <X className="w-3.5 h-3.5" /> Back
        </button>
        <h3 className="text-base font-display font-bold text-white">Version History</h3>
      </div>
      {versions.length === 0 ? (
        <div className="flex items-center justify-center h-32 text-white/40 text-sm">No versions yet.</div>
      ) : (
        <div className="space-y-2">
          {versions.map(v => (
            <div key={v.id} className="flex items-center gap-4 p-3 rounded-xl border border-white/10 bg-white/5">
              <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-xs font-bold text-white/60">{v.version}</div>
              <div className="flex-1">
                <p className="text-sm text-white/80">{v.changeNote || "Updated"}</p>
                <p className="text-[10px] text-white/40">{new Date(v.createdAt).toLocaleString()}</p>
              </div>
            </div>
          ))}
        </div>
...
function CommentsPanel({ doc, onBack, accentColor }: { doc: DocumentRecord; onBack: () => void; accentColor: string }) {
  const [comment, setComment] = useState("");
  const [comments, setComments] = useState<Array<{ id: number; authorName: string; content: string; resolved: boolean; createdAt: string }>>(doc.comments || []);

  const postComment = async () => {
    if (!comment.trim()) return;
    const newComment = { id: Date.now(), authorName: "You", content: comment, resolved: false, createdAt: new Date().toISOString() };
  ...
  return (
    <div className="flex flex-col h-full p-6 space-y-4 overflow-auto">
      ...
      {comments.length === 0 ? (
        <div className="flex items-center justify-center h-32 text-white/40 text-sm">No comments yet.</div>
      ) : (
        <div className="space-y-2">
          {comments.map(c => (
            <div key={c.id} className={cn("p-3 rounded-xl border", c.resolved ? "border-white/5 bg-white/3 opacity-60" : "border-white/10 bg-white/5")}>
...
const DEMO_DOCUMENTS: DocumentRecord[] = [
  { id: 1, title: "Q1 Deal Memo — 123 Main Street", type: "deal_memo", templateId: "terra-deal-memo", contentJson: {}, status: "signed", ownerId: 1, appSource: "terra", entityType: "property", entityId: "prop_001", mergeFieldValues: {}, isDemo: true, createdAt: "2026-01-15T10:00:00Z", updatedAt: "2026-01-20T14:30:00Z" },
  { id: 2, title: "Engagement Letter — Meridian Capital Group", type: "engagement_letter", templateId: "carlota-engagement-letter", contentJson: {}, status: "review", ownerId: 1, appSource: "carlota_jo", entityType: null, entityId: null, mergeFieldValues: {}, isDemo: true, createdAt: "2026-02-01T09:00:00Z", updatedAt: "2026-02-05T11:00:00Z" },
  { id: 3, title: "Incident Report — API Gateway Breach", type: "incident_report", templateId: "aegis-incident-report", contentJson: {}, status: "approved", ownerId: 1, appSource: "aegis", entityType: "incident", entityId: "inc_042", mergeFieldValues: {}, isDemo: true, createdAt: "2026-02-10T08:00:00Z", updatedAt: "2026-02-12T16:00:00Z" },
  { id: 4, title: "Voyage Report — MV Atlantic Pioneer", type: "voyage_report", templateId: "vessels-voyage-report", contentJson: {}, status: "draft", ownerId: 1, appSource: "vessels", entityType: "voyage", entityId: "voy_2026_001", mergeFieldValues: {}, isDemo: true, createdAt: "2026-03-01T07:00:00Z", updatedAt: "2026-03-05T09:00:00Z" },
  { id: 5, title: "NDA — Blackwood Ventures", type: "nda", templateId: "carlota-nda", contentJson: {}, status: "draft", ownerId: 1, appSource: "carlota_jo", entityType: null, entityId: null, mergeFieldValues: {}, isDemo: true, createdAt: "2026-03-10T10:00:00Z", updatedAt: "2026-03-10T10:00:00Z" },
  { id: 6, title: "Workflow Approval — CRM Integration v3", type: "workflow_approval_memo", templateId: "alloy-approval-memo", contentJson: {}, status: "review", ownerId: 1, appSource: "alloy", entityType: "workflow", entityId: "wf_crm_v3", mergeFieldValues: {}, isDemo: true, createdAt: "2026-03-15T14:00:00Z", updatedAt: "2026-03-18T11:00:00Z" },
  { id: 7, title: "Charter Party — Pacific Navigator", type: "charter_party", templateId: "vessels-charter-party", contentJson: {}, status: "signed", ownerId: 1, appSource: "vessels", entityType: "vessel", entityId: "ves_005", mergeFieldValues: {}, isDemo: true, createdAt: "2026-01-25T09:00:00Z", updatedAt: "2026-02-01T15:00:00Z" },
  { id: 8, title: "Vulnerability Assessment — Q1 2026", type: "vulnerability_assessment", templateId: "aegis-vulnerability-assessment", contentJson: {}, status: "approved", ownerId: 1, appSource: "aegis", entityType: null, entityId: null, mergeFieldValues: {}, isDemo: true, createdAt: "2026-01-05T10:00:00Z", updatedAt: "2026-01-20T16:00:00Z" },
];
