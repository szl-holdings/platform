...
import { FileDown, RefreshCw, XCircle, CheckCircle, Clock, AlertCircle, Play, Layers, ChevronDown, ChevronRight, BarChart2, Plus } from "lucide-react";
import { cn } from "../utils";
import type { PdfBatch, PdfJob, AppSource } from "./types";
...
export function BatchPdfPanel({ appSource = "general", accentColor = "#8b7ac8", className, sampleEntities }: BatchPdfPanelProps) {
  ...
  const submitBatch = async () => {
    ...
    try {
      const items = selectedEntities.map(id => {
        ...
        return { entityType: entity?.type || "entity", entityId: id, entityData: { label: entity?.label } };
      });
      const data = await apiFetch("/documents/batch-pdf", {
        method: "POST",
        body: JSON.stringify({ title: batchTitle || `PDF Batch ${new Date().toLocaleDateString()}`, templateId: selectedTemplate, appSource, items }),
      });
      ...
      setTimeout(() => {
        setBatches(prev => prev.map(b => b.batchId === demoBatch.batchId ? { ...b, status: "completed", completedJobs: selectedEntities.length } : b));
      }, 2500);
  ...
  const cancelBatch = async (batchId: string) => {
    try {
      ...
      setBatches(prev => prev.map(b => b.batchId === batchId ? { ...b, status: "cancelled" } : b));
    } catch {}
  };
  ...
  return (
    <div className={cn("flex flex-col h-full space-y-4 p-6 overflow-auto", className)}>
      ...
      {loading ? (
        <div className="flex items-center justify-center h-32 text-white/40 text-sm">Loading batches...</div>
      ) : batches.length === 0 ? (
        ...
        <div className="space-y-2">
          {batches.map(batch => {
            ...
            return (
              <div key={batch.batchId} className="rounded-xl border border-white/10 bg-white/5 overflow-hidden">
                <div
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn("flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold flex-shrink-0", cfg.bg, cfg.color)}>
                        <Icon className={cn("w-3 h-3", batch.status === "processing" ? "animate-spin" : "")} />
                        {cfg.label}
                      </span>
                    </div>
                    <p className="text-[10px] text-white/40 mt-0.5">{batch.completedJobs}/{batch.totalJobs} PDFs · Template: {batch.templateId} · {new Date(batch.createdAt).toLocaleDateString()}</p>
                    {batch.status === "processing" && (
                      <div className="mt-2">
                        <div className="w-full h-1 rounded-full bg-white/10">
                          <div className="h-1 rounded-full transition-all" style={{ width: `${progress}%`, background: accentColor }} />
                        </div>
                      </div>
                    )}
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {batch.status === "pending" && (
                      <button
                        onClick={(e) => { e.stopPropagation(); cancelBatch(batch.batchId); }}
                ...
                {isExpanded && (
                  <div className="border-t border-white/10 p-4 space-y-2">
                    <p className="text-xs text-white/50 font-semibold uppercase tracking-wider mb-2">Job Details</p>
                    {DEMO_JOBS.slice(0, Math.min(3, batch.totalJobs)).map((job, i) => {
                      const jCfg = JOB_STATUS_CONFIG[batch.status === "completed" ? "completed" : job.status] || JOB_STATUS_CONFIG.pending;
                      const JIcon = jCfg.icon;
                      return (
                        <div key={i} className="flex items-center gap-3 p-2 rounded-lg bg-white/5">
                          <JIcon className={cn("w-3.5 h-3.5 flex-shrink-0", jCfg.color)} />
                          <span className="text-xs text-white/60 flex-1">{job.entityType} #{job.entityId}</span>
                          <span className={cn("text-[10px] font-semibold", jCfg.color)}>{jCfg.label}</span>
                          {(batch.status === "completed") && (
                    ...
                    {batch.totalJobs > 3 && <p className="text-[10px] text-white/30 text-center">+ {batch.totalJobs - 3} more jobs</p>}
                  </div>
                )}
              </div>
            );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-900 border border-white/20 rounded-2xl p-6 w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowNewBatch(false)}><XCircle className="w-4 h-4 text-white/50" /></button>
            </div>
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  {appTemplates.map(t => <option key={t.slug} value={t.slug}>{t.title}</option>)}
                </select>
              </div>
              <div>
                <label className="text-xs text-white/60 mb-1.5 block">Select Items ({selectedEntities.length} selected)</label>
                <div className="max-h-40 overflow-y-auto space-y-1 border border-white/10 rounded-lg p-2">
                  {entities.map(e => (
                    <label key={e.id} className="flex items-center gap-2 p-1.5 rounded hover:bg-white/5 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={selectedEntities.includes(e.id)}
                        onChange={ev => setSelectedEntities(prev => ev.target.checked ? [...prev, e.id] : prev.filter(id => id !== e.id))}
                        className="rounded"
                      />
            ...
            <div className="flex justify-end gap-2 mt-5">
              <button onClick={() => setShowNewBatch(false)} className="px-4 py-1.5 rounded-lg text-xs text-white/60 hover:text-white bg-white/5 hover:bg-white/10 transition-colors">Cancel</button>
              <button
                onClick={submitBatch}
                ...
                style={{ background: accentColor }}
              >
                {submitting ? "Queuing..." : `Generate ${selectedEntities.length} PDF${selectedEntities.length !== 1 ? "s" : ""}`}
              </button>
            </div>
          </div>
        </div>
...
const DEMO_JOBS: PdfJob[] = [
  { id: 1, batchId: "batch_demo", templateId: "terra-deal-memo", entityType: "property", entityId: "prop_001", entityData: {}, appSource: "terra", status: "completed", outputUrl: "/api/documents/pdf-output/demo_1.pdf", outputFilename: "deal-memo-001.pdf", error: null, scheduledFor: null, startedAt: "2026-01-15T10:05:00Z", completedAt: "2026-01-15T10:05:45Z", requestedById: null, isDemo: true, createdAt: "2026-01-15T10:00:00Z" },
  { id: 2, batchId: "batch_demo", templateId: "terra-deal-memo", entityType: "property", entityId: "prop_002", entityData: {}, appSource: "terra", status: "completed", outputUrl: "/api/documents/pdf-output/demo_2.pdf", outputFilename: "deal-memo-002.pdf", error: null, scheduledFor: null, startedAt: "2026-01-15T10:05:00Z", completedAt: "2026-01-15T10:05:52Z", requestedById: null, isDemo: true, createdAt: "2026-01-15T10:00:00Z" },
];

const DEMO_BATCHES: PdfBatch[] = [
  { id: 1, batchId: "batch_q1_terra", title: "Q1 Property Reports — Manhattan", templateId: "terra-property-report", appSource: "terra", totalJobs: 12, completedJobs: 12, failedJobs: 0, status: "completed", zipUrl: null, createdAt: "2026-01-10T09:00:00Z", updatedAt: "2026-01-10T09:15:00Z" },
  { id: 2, batchId: "batch_compliance", title: "Annual Compliance Evidence — Aegis", templateId: "aegis-compliance-evidence", appSource: "aegis", totalJobs: 5, completedJobs: 5, failedJobs: 0, status: "completed", zipUrl: null, createdAt: "2026-01-20T14:00:00Z", updatedAt: "2026-01-20T14:08:00Z" },
  { id: 3, batchId: "batch_voyage", title: "Feb Voyage Reports", templateId: "vessels-voyage-report", appSource: "vessels", totalJobs: 8, completedJobs: 7, failedJobs: 1, status: "completed", zipUrl: null, createdAt: "2026-02-28T08:00:00Z", updatedAt: "2026-02-28T08:22:00Z" },
  { id: 4, batchId: "batch_engagement", title: "Client Engagement Packets", templateId: "carlota-engagement-letter", appSource: "carlota_jo", totalJobs: 4, completedJobs: 4, failedJobs: 0, status: "completed", zipUrl: null, createdAt: "2026-03-01T10:00:00Z", updatedAt: "2026-03-01T10:05:00Z" },
];
