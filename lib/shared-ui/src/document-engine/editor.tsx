...
import { type Descendant, Editor, Text, Transforms } from 'slate';
...
function slateTypeToBlock(type: string): string {
  const map: Record<string, string> = {
    ...
    'thematic-break': 'horizontal_rule',
  ...
  switch (el.type) {
    case 'heading-one':
      return (
        <h1
          {...attributes}
          style={alignStyle}
          className="text-2xl font-bold text-white px-2 py-1 outline-none"
        >
          {children}
        </h1>
      );
    case 'heading-two':
      return (
        <h2
          {...attributes}
          style={alignStyle}
          className="text-xl font-semibold text-white/90 px-2 py-1 outline-none"
        >
          {children}
        </h2>
      );
    case 'heading-three':
      return (
        <h3
          {...attributes}
          style={alignStyle}
          className="text-base font-semibold text-white/80 px-2 py-1 outline-none"
        >
          {children}
        </h3>
      );
    case 'bulleted-list':
      return (
        <div {...attributes} className="flex items-start gap-2 px-2 py-1 outline-none">
          <span contentEditable={false} className="text-white/50 mt-0.5 flex-shrink-0 select-none">
            •
          </span>
    ...
    case 'numbered-list':
      return (
        <div {...attributes} className="flex items-start gap-2 px-2 py-1 outline-none">
          <span
            contentEditable={false}
            className="text-white/50 mt-0.5 flex-shrink-0 select-none text-xs"
          >
            1.
          </span>
          <span style={alignStyle}>{children}</span>
    ...
    case 'blockquote':
      return (
        <blockquote
          {...attributes}
          style={alignStyle}
          className="border-l-4 border-indigo-500/50 pl-4 py-2 italic text-white/60 outline-none"
        >
          {children}
        </blockquote>
      );
    case 'code-block':
      return (
        <pre
          {...attributes}
          className="bg-black/40 font-mono text-xs rounded-lg p-3 text-[#6b8f71] whitespace-pre-wrap overflow-x-auto outline-none"
        >
          {children}
        </pre>
      );
          ...
          <table className="w-full text-xs">
            <tbody>
              {(el.tableData || []).map((row, ri) => (
                <tr
                  key={ri}
                  className={ri === 0 ? 'bg-white/10 font-semibold' : 'border-t border-white/5'}
                >
                  {row.map((cell, ci) => (
    ...
    default:
      return (
        <p {...attributes} style={alignStyle} className="text-white/80 px-2 py-1 outline-none">
          {children}
        </p>
      );
  }
...
function renderLeaf({
  ...
  if (t.code)
    el = <code className="bg-black/40 text-[#6b8f71] font-mono text-xs px-1 rounded">{el}</code>;
...
function EditorToolbar({
  ...
  const [versions, setVersions] = useState<
    Array<{ id: number; version: number; changeNote?: string | null }>
  >([]);
  ...
  const insertMerge = useCallback(
    (field: string) => {
      ...
      Transforms.insertNodes(
        editor as never,
        { type: 'merge-field', mergeField: field, children: [{ text: '' }] } as Descendant,
      );
    },
    [editor],
  );
  ...
  const insertLib = useCallback(
    (lib: ContentLibraryBlock) => {
      setShowLib(false);
  ...
  return (
    <div className="flex flex-wrap gap-0.5 p-2 border-b border-white/10 bg-slate-800/80 backdrop-blur-sm">
      <ToolbarBtn title="H1" onClick={() => setBlockType(editor, 'heading-one')}>
        <span className="text-xs font-bold">H1</span>
      </ToolbarBtn>
      <ToolbarBtn title="H2" onClick={() => setBlockType(editor, 'heading-two')}>
        <span className="text-xs font-bold">H2</span>
      </ToolbarBtn>
      <ToolbarBtn title="H3" onClick={() => setBlockType(editor, 'heading-three')}>
        <span className="text-xs font-bold">H3</span>
      </ToolbarBtn>
      <Sep />
      <ToolbarBtn active={bold} title="Bold (Ctrl+B)" onClick={() => toggleMark(editor, 'bold')}>
      ...
      <ToolbarBtn title="Code" onClick={() => toggleMark(editor, 'code')}>
        <Code className="w-3.5 h-3.5" />
      </ToolbarBtn>
      <Sep />
      <ToolbarBtn title="Bullet List" onClick={() => setBlockType(editor, 'bulleted-list')}>
        <List className="w-3.5 h-3.5" />
      </ToolbarBtn>
      <ToolbarBtn title="Numbered List" onClick={() => setBlockType(editor, 'numbered-list')}>
        <ListOrdered className="w-3.5 h-3.5" />
      </ToolbarBtn>
      <ToolbarBtn title="Blockquote" onClick={() => setBlockType(editor, 'blockquote')}>
        <Quote className="w-3.5 h-3.5" />
      </ToolbarBtn>
      <ToolbarBtn title="Code Block" onClick={() => setBlockType(editor, 'code-block')}>
        <Code className="w-3.5 h-3.5" />
      </ToolbarBtn>
      <Sep />
      <ToolbarBtn
        title="Align Left"
        onClick={() =>
          Transforms.setNodes(editor as never, { align: 'left' } as Partial<DocElement>)
        }
      >
      ...
      <ToolbarBtn
        title="Align Center"
        onClick={() =>
          Transforms.setNodes(editor as never, { align: 'center' } as Partial<DocElement>)
        }
      >
      ...
      <ToolbarBtn
        title="Align Right"
        onClick={() =>
          Transforms.setNodes(editor as never, { align: 'right' } as Partial<DocElement>)
        }
      >
      ...
      <ToolbarBtn
        title="Justify"
        onClick={() =>
          Transforms.setNodes(editor as never, { align: 'justify' } as Partial<DocElement>)
        }
      >
      ...
      <div className="relative">
        ...
        >
          <span className="flex items-center gap-1 text-xs">
            <Variable className="w-3.5 h-3.5" /> Field <ChevronDown className="w-2.5 h-2.5" />
          </span>
        </ToolbarBtn>
        {showMerge && (
      ...
      <div className="relative">
        ...
        {showLib && (
          <div className="absolute top-full right-0 mt-1 z-50 bg-slate-800 border border-white/20 rounded-lg shadow-xl w-72 py-1 max-h-64 overflow-y-auto">
            <div className="px-3 py-1.5 text-[10px] text-white/40 uppercase tracking-widest border-b border-white/10">
              Content Library
            </div>
            {libLoading ? (
              ...
              library.map((lib) => (
                <button
                  ...
                  className="w-full text-left px-3 py-2 text-xs hover:bg-white/10 group"
                >
                  <div className="text-white/80 font-medium group-hover:text-white">
                    {lib.title}
                  </div>
        ...
        <div className="relative">
          <ToolbarBtn
            ...
            onClick={() => {
              ...
              setShowLib(false);
              setDiff(null);
              if (!showDiff) loadVersions();
            }}
          >
            <span className="flex items-center gap-1 text-xs">
              <Diff className="w-3.5 h-3.5" /> Diff
            </span>
          </ToolbarBtn>
          {showDiff && (
            <div className="absolute top-full right-0 mt-1 z-50 bg-slate-800 border border-white/20 rounded-xl shadow-xl w-80 p-3">
              ...
              <div className="flex items-center gap-2 mb-3">
                <select
                  ...
                  className="flex-1 bg-slate-700 text-xs text-white/80 rounded px-2 py-1.5 border border-white/10"
                >
                ...
                <button
                  onClick={computeDiff}
                  className="px-2.5 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs rounded font-medium"
                >
                    ...
                    {diff.diff
                      ...
                      .map((d, i) => (
                        ...
                        >
                          ...
                          {d.op === 'change' ? (
                            <>
                              <s className="opacity-50">{d.textA?.slice(0, 50)}</s>
                              {' → '}
                              {d.textB?.slice(0, 50)}
                            </>
                          ) : (
...
export function DocumentEditor({
  ...
  const suggestedFields =
    mergeFields || MERGE_FIELD_SUGGESTIONS[appSource] || MERGE_FIELD_SUGGESTIONS.general;

  return (
    <div
      className={cn(
        'flex flex-col rounded-xl border border-white/10 overflow-hidden bg-slate-900',
        className,
      )}
    >
...
export function createEmptyDocument(version = 0): DocumentEditorContent {
  return {
    version,
    blocks: [
      { id: Math.random().toString(36).slice(2, 10), type: 'paragraph', children: [{ text: '' }] },
    ],
  };
}
