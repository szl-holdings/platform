...
export function CohortMatrix({ result, label, loading = false, maxPeriods = 8 }: CohortMatrixProps) {
  if (loading) {
    return (
  ...
  const maxPeriodCount = Math.max(
    ...result.cohorts.map(c => c.periods.length),
    1
  );
    ...
    <div className="rounded-xl border border-white/10 bg-white/5 p-4 flex flex-col gap-4">
      {label && (
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-zinc-500">
            Avg retention: <span className="text-white font-medium">{result.overallRetentionRate.toFixed(1)}%</span>
          </span>
        </div>
      )}
      ...
      <div className="overflow-x-auto">
        <table className="text-xs border-separate border-spacing-0.5">
          <thead>
            <tr>
              <th className="text-left text-zinc-500 pr-3 pb-2 font-normal whitespace-nowrap">Cohort</th>
          ...
          <tbody>
            {result.cohorts.map(cohort => (
              <tr key={cohort.cohortLabel}>
                <td className="text-zinc-400 pr-3 py-0.5 whitespace-nowrap">{cohort.cohortLabel}</td>
                  ...
                  return (
                    <td key={i} className="px-1 py-0.5">
                      {period ? (
                        ...
                        <div className="rounded text-center py-1 px-0.5 text-xs text-zinc-700 min-w-[44px]">—</div>
                      )}
                    </td>
                  );
                })}
