      ...
      {variant === "table" && (
        <div className="rounded-xl border border-border/50 overflow-hidden">
          ...
          {Array.from({ length: rows }).map((_, i) => (
            <div key={i} className="px-4 py-3 flex items-center gap-4 border-b border-border/30 last:border-0">
...
export function InlineDataSkeleton({ rows = 3, className }: InlineDataSkeletonProps) {
  return (
    <div className={cn("space-y-2", className)}>
      {Array.from({ length: rows }).map((_, i) => (
        <Skeleton key={i} className={cn("h-4", i === 0 ? "w-full" : i === rows - 1 ? "w-3/5" : "w-4/5")} />
      ))}
    </div>
  );
}
