...
export { ATLAS_EXPORT_FORMATS, ATLAS_TEMPLATE_TYPES };
...
export async function compareArtifactVersions(
  ...
  const [a, b] = await Promise.all([
    db
      .select()
      .from(atlasArtifactsTable)
      .where(eq(atlasArtifactsTable.id, idA))
      .then((r) => r[0]),
    db
      .select()
      .from(atlasArtifactsTable)
      .where(eq(atlasArtifactsTable.id, idB))
      .then((r) => r[0]),
  ]);

  if (!a || !b)
    throw Object.assign(new Error('One or both artifacts not found'), { code: 'NOT_FOUND' });
...
export async function createExportJob(params: {
  ...
  const [artifact] = await db
    .select()
    .from(atlasArtifactsTable)
    .where(eq(atlasArtifactsTable.id, params.artifactId));
  if (!artifact) {
    throw Object.assign(new Error(`Atlas artifact ${params.artifactId} not found`), {
      code: 'NOT_FOUND',
    });
  }
...
export async function listArtifacts(options: {
  ...
  if (options.templateType)
    conditions.push(eq(atlasArtifactsTable.templateType, options.templateType));
