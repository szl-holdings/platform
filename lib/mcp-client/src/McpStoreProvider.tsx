...
import type { McpConnectionState, McpServerConfig, McpTool, McpToolResult, McpDomain } from "./types";
...
interface McpStoreContextValue {
  ...
  callTool: (toolName: string, args: Record<string, unknown>, serverId?: string) => Promise<McpToolResult>;
  isEnabled: boolean;
  mcpBaseUrl: string;
}
...
export function McpStoreProvider({ children, domain }: { children: React.ReactNode; domain?: McpDomain }) {
  ...
  const nativeServer: McpServerConfig = {
    ...
    url: mcpBaseUrl,
    transport: "http",
    description: "Native SZL Alloy MCP server — built-in tools and enterprise capabilities",
    isNative: true,
    isConnected: true,
  };
  ...
  const getToolsForDomain = useCallback((d: McpDomain): McpTool[] => {
  ...
  const callTool = useCallback(async (
    ...
    try {
      ...
      if (data.pendingApproval) {
        return { toolName, success: true, output: data.output, pendingApproval: true, approvalId: data.approvalId };
      }
  ...
  return (
    <McpStoreContext.Provider value={{
      servers,
      connections,
      toolCatalog,
      addServer,
