...
const fleets = loadSeedDataSync<Array<{ id: number; name: string; region: string; status: string; description: string }>>(
  "vessels/fleets.json",
  [],
);
...
const maintenanceLogs = loadSeedDataSync<MaintenanceLog[]>(
  "vessels/maintenance-logs.json",
  [],
);
...
const shipmentRecords = loadSeedDataSync<ShipmentRecord[]>(
  "vessels/shipment-records.json",
  [],
);

const eventLogs = loadSeedDataSync<EventLog[]>("vessels/event-logs.json", []);

const aiBriefings = loadSeedDataSync<AIBriefing[]>(
  "vessels/ai-briefings.json",
  [],
);
...
const forecastModules = loadSeedDataSync<ForecastModule[]>(
  "vessels/forecast-modules.json",
  [],
);
