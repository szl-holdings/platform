...
export class JiraConnector extends ToolConnector {
  readonly id = "jira";
  readonly name = "Jira";
  readonly description = "Atlassian Jira — project tracking, sprint management, issue lifecycle, webhook ingestion, and team velocity reporting";
  ...
  readonly authConfig: AuthConfig = {
    ...
    description: "Basic auth via email + API token. Generate a token at id.atlassian.com/manage-profile/security/api-tokens",
  };
  ...
  readonly capabilities: Capability[] = [
    {
      ...
      parameters: [
        { name: "limit", type: "number", description: "Maximum projects to return (default 50)", required: false },
      ],
      requiresAuth: true,
      tags: ["read", "projects"],
    },
    {
      ...
      parameters: [
        { name: "jql", type: "string", description: "JQL query string (e.g. 'project = PROJ AND status = Open')", required: true },
        { name: "limit", type: "number", description: "Maximum results to return (default 50)", required: false },
      ],
      requiresAuth: true,
    ...
    },
    {
      ...
      parameters: [
        { name: "projectKey", type: "string", description: "Project key (e.g. PROJ)", required: true },
        { name: "summary", type: "string", description: "Issue summary/title", required: true },
        { name: "issueType", type: "string", description: "Issue type (Bug, Story, Task, Epic)", required: false },
        { name: "description", type: "string", description: "Issue description", required: false },
        { name: "priority", type: "string", description: "Priority: Highest, High, Medium, Low, Lowest", required: false },
    ...
    },
    {
      ...
      parameters: [
        { name: "boardId", type: "number", description: "Jira board ID (optional)", required: false },
      ],
      requiresAuth: true,
      tags: ["read", "sprints"],
    },
    {
      ...
      parameters: [
        { name: "sprintId", type: "number", description: "Sprint ID to analyze (optional — analyzes all if omitted)", required: false },
      ],
      requiresAuth: true,
    ...
    },
    {
      id: "ingest_signals",
      name: "Ingest Signals",
      description: "Ingest Jira issue signals (blocked issues, overdue, high-severity bugs) for platform analysis",
      parameters: [],
      requiresAuth: true,
      tags: ["read", "signals"],
    },
    {
      ...
      parameters: [
        ...
        { name: "transitionName", type: "string", description: "Transition name (e.g. 'In Progress', 'Done')", required: true },
      ],
      requiresAuth: true,
      tags: ["write", "issues"],
    },
  ...
  protected async performCapability(capabilityId: string, params: Record<string, unknown>): Promise<unknown> {
    ...
    switch (capabilityId) {
      ...
      case "search_issues":
        return adapter.searchIssues(String(params["jql"]), params["limit"] ? Number(params["limit"]) : 50);
      case "create_issue":
        return adapter.createIssue({
          ...
          priority: params["priority"] ? (String(params["priority"]) as "Highest" | "High" | "Medium" | "Low" | "Lowest") : undefined,
      ...
      case "transition_issue":
        return adapter.transitionIssue(String(params["issueKey"]), String(params["transitionName"]));
      default:
