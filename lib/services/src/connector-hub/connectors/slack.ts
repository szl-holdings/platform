...
export class SlackConnector extends ToolConnector {
  readonly id = "slack";
  readonly name = "Slack";
  readonly description = "Slack — team messaging, channel management, webhook delivery, interactive alert routing, and slash command handling";
  ...
  readonly authConfig: AuthConfig = {
    ...
    description: "Bot token (xoxb-*) from Slack App > OAuth & Permissions. Signing secret for webhook verification.",
  ...
  readonly capabilities: Capability[] = [
    {
      ...
      parameters: [
        { name: "channel", type: "string", description: "Channel ID or name (e.g. #general or C1234)", required: true },
    ...
    },
    {
      ...
      parameters: [
        { name: "channel", type: "string", description: "Target channel ID or name", required: true },
        ...
        { name: "severity", type: "string", description: "Alert severity: info, warning, critical", required: false, enum: ["info", "warning", "critical"] },
        { name: "source", type: "string", description: "System that triggered the alert", required: false },
      ],
      requiresAuth: true,
      tags: ["write", "alerts"],
    },
    ...
    },
    {
      ...
      parameters: [
        { name: "limit", type: "number", description: "Maximum channels to return (default 200)", required: false },
      ],
      requiresAuth: true,
      tags: ["read", "channels"],
    },
    {
      ...
      parameters: [
        { name: "severity", type: "string", description: "Alert severity: info, warning, critical", required: true, enum: ["info", "warning", "critical"] },
        { name: "title", type: "string", description: "Alert title", required: true },
    ...
    },
    {
      ...
      parameters: [
        ...
        { name: "channel", type: "string", description: "Channel override (if webhook supports it)", required: false },
      ],
      requiresAuth: false,
      tags: ["write", "webhook"],
    },
  ...
  protected async performCapability(capabilityId: string, params: Record<string, unknown>): Promise<unknown> {
    ...
    switch (capabilityId) {
      ...
      case "send_webhook":
        return adapter.sendWebhookMessage(String(params["text"]), params["channel"] ? { channel: String(params["channel"]) } : undefined);
      default:
