...
export class PagerDutyConnector extends ToolConnector {
  ...
  readonly description = "PagerDuty — incident alerting, on-call scheduling, escalation policy management, and real-time incident lifecycle";
  ...
  readonly authConfig: AuthConfig = {
    ...
    description: "REST API key from PagerDuty API Access Keys. Webhook secret for inbound event validation.",
  };
  ...
  readonly capabilities: Capability[] = [
    {
      ...
      parameters: [
        { name: "status", type: "string", description: "Filter by status: triggered, acknowledged, resolved", required: false },
        { name: "limit", type: "number", description: "Maximum number of incidents (default 25)", required: false },
      ],
      requiresAuth: true,
      tags: ["read", "incidents"],
    ...
    },
    {
      ...
      parameters: [
        { name: "escalationPolicyId", type: "string", description: "Filter by escalation policy ID", required: false },
      ],
      requiresAuth: true,
      tags: ["read", "oncall"],
    },
    {
      ...
      parameters: [
        ...
        { name: "serviceId", type: "string", description: "PagerDuty service ID to associate the incident with", required: true },
        { name: "urgency", type: "string", description: "Urgency: high or low (default: high)", required: false, enum: ["high", "low"] },
    ...
    },
    {
      ...
      parameters: [
        { name: "incidentId", type: "string", description: "PagerDuty incident ID", required: true },
        { name: "resolvedByEmail", type: "string", description: "Email of the user resolving the incident", required: true },
      ],
      requiresAuth: true,
      tags: ["write", "incidents"],
    },
    ...
    switch (capabilityId) {
      case "list_incidents": {
        ...
        const statusArr = rawStatus
          ? (Array.isArray(rawStatus) ? rawStatus.map(String) : [String(rawStatus)]) as Array<"triggered" | "acknowledged" | "resolved">
          : undefined;
        return adapter.listIncidents({ status: statusArr, limit: params["limit"] as number | undefined });
      }
      ...
      case "get_on_call":
        return adapter.getOnCallSchedule(params["escalationPolicyId"] ? String(params["escalationPolicyId"]) : undefined);
      case "create_incident":
        return adapter.createIncident({
          title: String(params["title"]),
          serviceId: String(params["serviceId"]),
          fromEmail: params["fromEmail"] ? String(params["fromEmail"]) : "alloy-agent@szlholdings.com",
          urgency: (params["urgency"] as "high" | "low") ?? "high",
          body: params["body"] ? String(params["body"]) : undefined,
        });
      case "resolve_incident":
        return adapter.resolveIncident(String(params["incidentId"]), String(params["resolvedByEmail"]));
      default:
