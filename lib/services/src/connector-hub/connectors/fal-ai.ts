...
export class FalAiConnector extends ToolConnector {
  ...
  readonly description = "Fal.ai — serverless GPU media generation: image synthesis (FLUX, SDXL), video generation, image editing, upscaling, and background removal";
  ...
  readonly capabilities: Capability[] = [
    {
      ...
      parameters: [
        ...
        { name: "model", type: "string", description: "Model: flux/dev, flux/schnell, stable-diffusion-xl (default: flux/schnell)", required: false },
        { name: "width", type: "number", description: "Image width in pixels (default 1024)", required: false },
        { name: "height", type: "number", description: "Image height in pixels (default 1024)", required: false },
        { name: "numImages", type: "number", description: "Number of images to generate (1-4)", required: false },
        { name: "negativePrompt", type: "string", description: "Negative prompt to exclude content", required: false },
        { name: "seed", type: "number", description: "Random seed for reproducibility", required: false },
      ],
      requiresAuth: true,
    ...
    },
    {
      ...
      parameters: [
        ...
        { name: "model", type: "string", description: "Model: stable-video-diffusion, cogvideox (default: stable-video-diffusion)", required: false },
        { name: "imageUrl", type: "string", description: "Starting image URL (for image-to-video)", required: false },
        { name: "numFrames", type: "number", description: "Number of frames (default 25)", required: false },
        { name: "fps", type: "number", description: "Frames per second (default 8)", required: false },
      ],
      requiresAuth: true,
    ...
    },
    {
      ...
      parameters: [
        { name: "imageUrl", type: "string", description: "URL of the input image", required: true },
        { name: "scale", type: "number", description: "Upscale factor: 2, 4, or 8 (default 4)", required: false },
        { name: "model", type: "string", description: "Upscaler model: esrgan, swinir (default: esrgan)", required: false },
      ],
      requiresAuth: true,
  ...
  protected async performCapability(capabilityId: string, params: Record<string, unknown>): Promise<unknown> {
    ...
    switch (capabilityId) {
      case "generate_image": {
        ...
        if (!resp.ok) throw new Error(`Fal.ai image generation error ${resp.status}: ${await resp.text()}`);
        return resp.json();
      }
      case "generate_video": {
        ...
        if (!resp.ok) throw new Error(`Fal.ai video generation error ${resp.status}: ${await resp.text()}`);
        return resp.json();
      }
      case "remove_background": {
        ...
        if (!resp.ok) throw new Error(`Fal.ai background removal error ${resp.status}: ${await resp.text()}`);
        return resp.json();
      }
      case "upscale_image": {
  ...
  protected async performHealthCheck(): Promise<void> {
    ...
    const resp = await fetch("https://fal.run/fal-ai/flux/schnell", {
      ...
      body: JSON.stringify({ prompt: "health check", num_images: 1, image_size: { width: 64, height: 64 } }),
      signal: AbortSignal.timeout(10_000),
    });
    if (!resp.ok && resp.status !== 422) throw new Error(`Fal.ai health check failed: ${resp.status}`);
  }
}
