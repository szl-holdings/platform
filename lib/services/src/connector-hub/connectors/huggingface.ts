...
export class HuggingFaceConnector extends ToolConnector {
  ...
  readonly description = "HuggingFace — open-source model hub and Inference API: text generation, summarization, classification, NER, embeddings, translation, and image generation";
  ...
  readonly capabilities: Capability[] = [
    {
      ...
      description: "Generate text using open-source LLMs (Mistral, Llama, Falcon, etc.) via HuggingFace Inference API",
      parameters: [
        { name: "prompt", type: "string", description: "Input prompt for text generation", required: true },
        { name: "model", type: "string", description: "HuggingFace model ID (optional — uses tier-based selection)", required: false },
        { name: "maxTokens", type: "number", description: "Maximum new tokens to generate (default 512)", required: false },
        { name: "temperature", type: "number", description: "Sampling temperature 0.0–2.0", required: false },
        { name: "tier", type: "string", description: "Model tier: flagship, balanced, fast (default: balanced)", required: false, enum: ["flagship", "balanced", "fast"] },
      ],
      requiresAuth: true,
    ...
    },
    {
      ...
      parameters: [
        ...
        { name: "maxLength", type: "number", description: "Maximum summary length (default 200)", required: false },
        { name: "minLength", type: "number", description: "Minimum summary length (default 30)", required: false },
      ],
      requiresAuth: true,
    ...
    },
    {
      ...
      parameters: [
        ...
        { name: "model", type: "string", description: "HuggingFace classifier model ID", required: false },
      ],
      requiresAuth: true,
    ...
    },
    {
      ...
      parameters: [
        ...
        { name: "multiLabel", type: "boolean", description: "Allow multiple labels (default false)", required: false },
      ],
      requiresAuth: true,
    ...
    },
    {
      ...
      description: "Extract named entities (people, places, organizations) from text using NER models",
      parameters: [
        { name: "text", type: "string", description: "Text to extract entities from", required: true },
        { name: "model", type: "string", description: "HuggingFace NER model ID (optional)", required: false },
      ],
      requiresAuth: true,
      tags: ["inference", "ner"],
    },
    {
      ...
      parameters: [
        { name: "texts", type: "array", description: "Array of text strings to embed", required: true },
        { name: "model", type: "string", description: "Embedding model ID (optional — default: BAAI/bge-small-en-v1.5)", required: false },
      ],
      requiresAuth: true,
    ...
    },
    {
      ...
      description: "Get HuggingFace Inference API health, model availability, and rate limit status",
      parameters: [],
      requiresAuth: true,
      tags: ["read", "health"],
    },
