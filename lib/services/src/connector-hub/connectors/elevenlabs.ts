...
export class ElevenLabsConnector extends ToolConnector {
  ...
  readonly description = "ElevenLabs — AI voice synthesis and cloning: text-to-speech, voice selection, multilingual synthesis, and audio output generation";
  ...
  readonly capabilities: Capability[] = [
    {
      ...
      parameters: [
        { name: "text", type: "string", description: "Text to synthesize (max 5000 characters)", required: true },
        { name: "voiceId", type: "string", description: "ElevenLabs voice ID (default: Rachel — 21m00Tcm4TlvDq8ikWAM)", required: false },
      ],
      requiresAuth: true,
  ...
  protected async performCapability(capabilityId: string, params: Record<string, unknown>): Promise<unknown> {
    ...
    switch (capabilityId) {
      case "text_to_speech":
        return adapter.textToSpeech(String(params["text"]), params["voiceId"] ? String(params["voiceId"]) : undefined);
      case "list_voices":
