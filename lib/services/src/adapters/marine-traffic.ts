...
const MOCK_PORT_CALLS: MarineTrafficPortCall[] = [
  {
    ...
    mmsi: '235100643',
    shipName: 'PACIFIC VOYAGER',
    arrivalDate: '2026-04-10T12:00:00Z',
    departureDate: null,
    status: 'in_port',
  },
  {
    ...
    mmsi: '566123456',
    shipName: 'ARCTIC EXPLORER',
    arrivalDate: '2026-04-08T09:00:00Z',
    departureDate: '2026-04-09T14:00:00Z',
    status: 'departed',
  },
];
...
export class MarineTrafficAdapter extends ServiceAdapter {
  ...
  async getVesselEvents(mmsi: string, eventType?: string): Promise<MarineTrafficVesselEvent[]> {
    ...
    return (data.DATA ?? []).map((e) => ({
      mmsi: e['MMSI'] ?? '',
      shipName: e['SHIPNAME'] ?? '',
      eventType: e['TYPEFLAG'] ?? '',
      eventDate: e['TIMESTAMP'] ?? '',
      portName: e['PORT_NAME'] ?? '',
      portCode: e['UN_LOCODE'] ?? '',
      flag: e['FLAG'] ?? '',
    }));
  }
  ...
  async getPortCalls(portCode: string): Promise<MarineTrafficPortCall[]> {
    ...
    const data = await this.mtRequest<{ DATA: Array<Record<string, string>> }>('getportcalls', {
      port_target_id: portCode,
    });
    return (data.DATA ?? []).map((p) => ({
      portId: p['port_target_id'] ?? '',
