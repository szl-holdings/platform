...
const MOCK_PROPERTIES: ZillowPropertyDetails[] = [
  {
    zpid: '88271895',
    address: '847 Park Ave, Queens, NY 11354',
    city: 'Queens',
    state: 'NY',
    zipCode: '11354',
    bedrooms: 3,
    ...
    zestimate: {
      ...
      zestimate: 2100000,
      zestimateLow: 1890000,
      zestimateHigh: 2310000,
      rentZestimate: 4200,
      lastUpdated: '2026-04-10',
      changePercent30Day: -2.1,
      changePercent1Year: -4.3,
    },
    latitude: 40.723,
    longitude: -73.842,
    taxAssessedValue: 1680000,
    taxAssessedYear: 2025,
    priceHistory: [
      { date: '2025-06-01', price: 2280000, event: 'Listed' },
      { date: '2025-08-15', price: 2190000, event: 'Price Change' },
      { date: '2025-11-20', price: 2100000, event: 'Price Change' },
    ],
  },
  {
    zpid: '76512340',
    address: '312 W 23rd St, New York, NY 10011',
    city: 'New York',
    state: 'NY',
    zipCode: '10011',
    bedrooms: 2,
    ...
    zestimate: {
      zpid: '76512340',
      address: '312 W 23rd St',
      zestimate: 895000,
      zestimateLow: 806000,
      zestimateHigh: 984000,
      rentZestimate: 3800,
      lastUpdated: '2026-04-12',
      changePercent30Day: 0.6,
      changePercent1Year: 2.1,
    },
    latitude: 40.745,
    longitude: -74.0,
    taxAssessedValue: 714000,
    taxAssessedYear: 2025,
...
export class ZillowAdapter extends ServiceAdapter {
  ...
  async getPropertyDetails(zpid: string): Promise<ZillowPropertyDetails | null> {
    if (this.isDemoMode)
      return MOCK_PROPERTIES.find((p) => p.zpid === zpid) ?? MOCK_PROPERTIES[0] ?? null;
    ...
    return {
      ...
      zestimate: {
        ...
        address: String(data['address'] ?? ''),
        zestimate: zest,
        zestimateLow: zest * 0.9,
        zestimateHigh: zest * 1.1,
        rentZestimate: data['rentZestimate'] ? Number(data['rentZestimate']) : null,
        lastUpdated: new Date().toISOString().split('T')[0]!,
        changePercent30Day: null,
        changePercent1Year: null,
      },
  ...
  async searchProperties(query: string, maxResults = 10): Promise<ZillowSearchResult[]> {
    if (this.isDemoMode) {
      return MOCK_PROPERTIES.map((p) => ({
        zpid: p.zpid,
        address: p.address,
        price: p.listingPrice ?? p.zestimate.zestimate,
        bedrooms: p.bedrooms,
        bathrooms: p.bathrooms,
        livingArea: p.livingArea,
        propertyType: p.propertyType,
        listingStatus: p.listingStatus,
        zestimate: p.zestimate.zestimate,
        imageUrl: null,
        daysOnMarket: null,
      }));
    }
    const data = await this.zRequest<{ props: Array<Record<string, unknown>> }>(
      '/propertyExtendedSearch',
      { location: query, status_type: 'ForSale' },
    );
    return (data.props ?? []).slice(0, maxResults).map((p) => ({
      ...
      listingStatus: String(p['listingStatus'] ?? ''),
