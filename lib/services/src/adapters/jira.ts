...
function computeSprintHealth(sprint: JiraSprint): JiraSprintHealth {
  ...
  const workProgress =
    sprint.storyPointsTotal > 0 ? sprint.storyPointsCompleted / sprint.storyPointsTotal : 0;
  ...
  const velocityEstimate =
    daysRemaining > 0
      ? sprint.storyPointsCompleted +
        Math.round((sprint.storyPointsCompleted / Math.max(1, daysElapsed)) * daysRemaining)
      : sprint.storyPointsCompleted;
  ...
  return {
    ...
    projectedCompletion: Math.round(
      (velocityEstimate / Math.max(1, sprint.storyPointsTotal)) * 100,
    ),
  };
}
...
export class JiraAdapter extends ServiceAdapter {
  ...
  private get oauthClientId(): string | undefined {
    return process.env['JIRA_OAUTH_CLIENT_ID'];
  }
  private get oauthClientSecret(): string | undefined {
    return process.env['JIRA_OAUTH_CLIENT_SECRET'];
  }
  private get oauthRefreshToken(): string | undefined {
    return process.env['JIRA_OAUTH_REFRESH_TOKEN'];
  }
  private get cloudId(): string | undefined {
  ...
  override get missingEnvVars(): string[] {
    ...
    return ['JIRA_BASE_URL', 'JIRA_API_TOKEN', 'JIRA_USER_EMAIL'].filter((v) => !process.env[v]);
  }
  ...
  private ensureOAuthInitialized(): void {
    ...
    globalTokenStore.registerRefreshCallback('jira', async () => {
      ...
      return refreshAccessToken({ clientId, clientSecret, tokenUrl: JIRA_OAUTH_TOKEN_URL }, rt);
    });
  }
    ...
    return buildAuthorizationUrl(
      {
        clientId,
        clientSecret: '',
        tokenUrl: JIRA_OAUTH_TOKEN_URL,
        authUrl: JIRA_OAUTH_AUTH_URL,
        redirectUri,
  ...
  async exchangeOAuthCode(code: string, redirectUri: string): Promise<OAuthTokenSet> {
    ...
    if (!clientId || !clientSecret)
      throw new Error('JIRA_OAUTH_CLIENT_ID and JIRA_OAUTH_CLIENT_SECRET are required');
  ...
  async ingestSignals(): Promise<JiraSignal[]> {
    ...
    try {
      ...
      const blockedIssues = await this.searchIssues(
        'status = "In Progress" AND issueType != Epic AND "Flagged" = Impediment ORDER BY priority DESC',
        20,
      );
  ...
  async createIssue(params: {
    ...
    if (!this.isLive) {
      ...
      return {
        id: `mock_${Date.now()}`,
        key: mockKey,
        url: `${this.baseUrl ?? 'https://jira.example.com'}/browse/${mockKey}`,
      };
    }
