...
const MOCK_CASES: PACERCase[] = [
  {
    caseId: "nysd-2026-cv-04821", caseNumber: "1:26-cv-04821",
    title: "Harmon Capital LLC v. Meridian Properties Inc.",
    court: "U.S. District Court for the Southern District of New York",
    courtCode: "nysd", dateOpened: "2026-01-14", dateClosed: null,
    caseType: "cv", causeCodes: ["28:1332"], natureOfSuit: "190",
    jurisdictionType: "Diversity", juryDemand: "Plaintiff", classAction: false,
    status: "open",
    parties: [
      { name: "Harmon Capital LLC", role: "plaintiff", attorney: "Sarah Chen, Latham & Watkins" },
      { name: "Meridian Properties Inc.", role: "defendant", attorney: "James Torres, Skadden Arps" },
    ],
  },
  {
    caseId: "nysd-2025-cv-18340", caseNumber: "1:25-cv-18340",
    title: "SEC v. NexGen Advisors LLC et al.",
    court: "U.S. District Court for the Southern District of New York",
    courtCode: "nysd", dateOpened: "2025-08-07", dateClosed: null,
    caseType: "cv", causeCodes: ["15:78j"], natureOfSuit: "850",
    jurisdictionType: "Federal Question", juryDemand: null, classAction: false,
    status: "open",
    parties: [
      { name: "Securities and Exchange Commission", role: "plaintiff", attorney: "U.S. Attorney SDNY" },
...
export class PACERAdapter extends ServiceAdapter {
  ...
  private get username(): string | undefined { return process.env["PACER_USERNAME"]; }
  private get password(): string | undefined { return process.env["PACER_PASSWORD"]; }
  ...
  private async pacerRequest<T>(path: string, body: Record<string, unknown> = {}): Promise<T> {
    ...
    const res = await fetch(`${this.BASE_URL}${path}`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-NEXT-GEN-CSO": token, Accept: "application/json" },
  ...
  async searchCases(params: { partyName?: string; caseTitle?: string; courtCode?: string; dateFiledStart?: string; dateFiledEnd?: string; limit?: number }): Promise<PACERSearchResult> {
    if (this.isDemoMode) return { cases: MOCK_CASES, totalCount: MOCK_CASES.length, nextCursor: null };
    const body: Record<string, unknown> = {
      pageNumber: 1, pageSize: params.limit ?? 20,
    };
    ...
    const data = await this.pacerRequest<{ content: Array<Record<string, unknown>>; totalElements: number }>("/cases/find", body);
    return {
      cases: (data.content ?? []).map(c => ({
        caseId: String(c["caseId"] ?? ""), caseNumber: String(c["caseNumberFull"] ?? ""),
        title: String(c["caseTitle"] ?? ""), court: String(c["courtName"] ?? ""),
        courtCode: String(c["courtId"] ?? ""), dateOpened: String(c["dateFiled"] ?? ""),
        dateClosed: c["dateClosed"] ? String(c["dateClosed"]) : null,
        caseType: String(c["caseType"] ?? "cv"), causeCodes: [],
        natureOfSuit: c["natureOfSuit"] ? String(c["natureOfSuit"]) : null,
        jurisdictionType: String(c["jurisdictionType"] ?? ""), juryDemand: null, classAction: false,
  ...
  async getCaseDocket(caseId: string, limit = 20): Promise<PACERDocketEntry[]> {
    if (this.isDemoMode) {
      return [
        { documentNumber: "1", dateFiled: "2026-01-14", description: "COMPLAINT against Meridian Properties Inc.", pageCount: 32, attachments: 4, documentId: "doc-001", isSealed: false },
        { documentNumber: "12", dateFiled: "2026-02-01", description: "ANSWER to Complaint", pageCount: 18, attachments: 1, documentId: "doc-012", isSealed: false },
        { documentNumber: "27", dateFiled: "2026-03-15", description: "MOTION for Summary Judgment", pageCount: 45, attachments: 12, documentId: "doc-027", isSealed: false },
      ];
    }
    const data = await this.pacerRequest<{ content: Array<Record<string, unknown>> }>(`/cases/${caseId}/dockets`, { pageSize: limit });
  ...
  getMockCases(): PACERCase[] { return MOCK_CASES; }
}
