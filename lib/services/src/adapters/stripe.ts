...
export class StripeAdapter extends ServiceAdapter {
  ...
  async listProducts(): Promise<StripeProduct[]> {
    ...
    for (const product of data.data) {
      const priceData = (await this.stripeRequest(`/prices?product=${product.id}&active=true`)) as {
        data: Array<{
          id: string;
          unit_amount: number;
          currency: string;
  ...
  async cancelSubscription(
    ...
    if (options?.cancelImmediately) {
      const data = (await this.stripeRequest(`/subscriptions/${subscriptionId}`, {
        ...
        canceled_at: number | null;
        items: {
          data: Array<{ id: string; price: { id: string; product: string }; quantity: number }>;
        };
      };
      return {
        id: data.id,
    ...
    const [allSubsData, invoiceData] = await Promise.all([
      this.stripeRequest('/subscriptions?limit=100&status=all&expand[]=data.items') as Promise<{
        data: Array<{
          ...
          created: number;
          items: {
            data: Array<{ price: { unit_amount: number; recurring?: { interval: string } } }>;
          };
        }>;
      }>,
