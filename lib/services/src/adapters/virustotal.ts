...
const MOCK_IP_REPORT: VTIPReport = {
  ip: "45.142.212.100", country: "RU", asn: 197695, asOwner: "REG.RU-AS",
  reputation: -42, positives: 8, total: 94,
  maliciousEngines: ["Forcepoint ThreatSeeker", "Webroot", "Kaspersky", "G-Data", "BitDefender", "ESET", "Avast", "Sophos"],
  lastAnalysisDate: new Date().toISOString(), whois: "inetnum: 45.142.212.0/24",
  detectedUrls: [
  ...
  detectedCommunicatingSamples: 23, network: "45.142.212.0/24",
};

const MOCK_URL_SCAN: VTUrlScan = {
  url: "https://phishing-example.com/login", positives: 6, total: 86,
  scanDate: new Date().toISOString(), permalink: "https://www.virustotal.com/gui/url/abc123",
  categories: ["phishing", "malware distribution"], malicious: true, reputation: -18,
};
...
export class VirusTotalAdapter extends ServiceAdapter {
  ...
  async scanHash(hash: string): Promise<Partial<VTFileScan>> {
    if (this.isDemoMode) {
      return { sha256: hash, positives: 0, total: 72, scanDate: new Date().toISOString(), name: "unknown", reputation: 0 };
    }
  ...
  getMockIPReport(): VTIPReport { return MOCK_IP_REPORT; }
}
