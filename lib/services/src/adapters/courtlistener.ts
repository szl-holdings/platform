...
const MOCK_CASES: CourtListenerCase[] = [
  {
    id: 67284901,
    absoluteUrl: '/docket/67284901/harmon-capital-v-meridian-properties/',
    caseName: 'Harmon Capital LLC v. Meridian Properties Inc.',
    dateFiled: '2026-01-14',
    dateTerminated: null,
    court: 'nysd',
    courtFullName: 'Southern District of New York',
    docketNumber: '1:26-cv-04821',
  ...
  {
    id: 58921034,
    absoluteUrl: '/docket/58921034/sec-v-nexgen-advisors/',
    caseName: 'Securities and Exchange Commission v. NexGen Advisors LLC',
    dateFiled: '2025-08-07',
    dateTerminated: null,
    court: 'nysd',
    courtFullName: 'Southern District of New York',
    docketNumber: '1:25-cv-18340',
    natureOfSuit: '850 Securities/Commodities/Exchanges',
    cause: '15:78j Securities Exchange Act',
    status: 'Open',
    pacer: true,
  },
];
...
const MOCK_OPINIONS: CourtListenerOpinion[] = [
  {
    id: 4928001,
    absoluteUrl: '/opinion/4928001/in-re-meridian-properties/',
...
export class CourtListenerAdapter extends ServiceAdapter {
  ...
  private get token(): string | undefined {
    return process.env['COURT_LISTENER_TOKEN'];
  }
  ...
  async searchCases(
    ...
    const data = await this.clRequest<{ results: Array<Record<string, unknown>> }>(
      '/dockets/',
      params,
    );
    return (data.results ?? []).map((c) => ({
      id: Number(c['id'] ?? 0),
      absoluteUrl: String(c['absolute_url'] ?? ''),
      caseName: String(c['case_name'] ?? ''),
      dateFiled: c['date_filed'] ? String(c['date_filed']) : null,
  ...
  async searchOpinions(query: string, court?: string, limit = 10): Promise<CourtListenerOpinion[]> {
    ...
    return (data.results ?? []).map((o) => ({
      id: Number(o['id'] ?? 0),
      absoluteUrl: String(o['absolute_url'] ?? ''),
      caseName: String(o['caseName'] ?? ''),
      dateFiled: String(o['dateFiled'] ?? ''),
      court: String(o['court_id'] ?? ''),
      type: String(o['type'] ?? ''),
      precedentialStatus: String(o['status'] ?? ''),
      citation: Array.isArray(o['citation'])
  ...
  ): Promise<{ citing: number; negative: number; positive: number }> {
    if (this.isDemoMode) return { citing: 47, negative: 3, positive: 12 };
    const data = await this.clRequest<{ count: number }>('/search/', {
      q: `"${citationString}"`,
      type: 'o',
      limit: '1',
    });
  ...
  getMockCases(): CourtListenerCase[] {
    return MOCK_CASES;
  }
}
