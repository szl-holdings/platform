...
const BUILT_IN_RULES: SiemCorrelationRule[] = [
  ...
  },
  {
    ...
    pattern: {
      minEvents: 1,
      windowMinutes: 1,
      conditions: [{ field: 'category', operator: 'contains', value: 'privilege' }],
    },
    severity: 'critical',
  ...
  },
  {
    ...
    pattern: {
      minEvents: 1,
      windowMinutes: 10,
      conditions: [{ field: 'category', operator: 'contains', value: 'exfiltration' }],
    },
    severity: 'critical',
...
function inferMitreMapping(
  event: Partial<NormalizedSiemEvent>,
): { tactic: string; technique: string } | null {
...
export class SiemAdapter extends ServiceAdapter {
  ...
  readonly description =
    'SIEM integration layer — normalized ingestion from Splunk HEC, Microsoft Sentinel, CEF/syslog, and event correlation engine with MITRE ATT&CK mapping';
    ...
    for (const raw of events) {
      try {
        ...
        const partial: Partial<NormalizedSiemEvent> = {
          ...
          destinationPort: eventData['dest_port']
            ? parseInt(String(eventData['dest_port']), 10)
            : null,
          protocol: (eventData['protocol'] as string) ?? null,
          action: (eventData['action'] as string) ?? null,
          outcome: this.mapOutcome(
            (eventData['result'] as string) ?? (eventData['outcome'] as string),
          ),
          ...
          message:
            ...
            (eventData['msg'] as string) ??
            JSON.stringify(eventData),
          category:
            (raw['sourcetype'] as string) ?? (eventData['category'] as string) ?? 'security',
          rawPayload: raw,
        };
  ...
  ingestGeneric(events: Array<Record<string, unknown>>): SiemIngestionResult {
    ...
    for (const raw of events) {
      try {
        const partial: Partial<NormalizedSiemEvent> = {
          severity: this.mapSplunkSeverity(
            (raw['severity'] as string) ?? (raw['level'] as string) ?? 'informational',
          ),
          timestamp:
            (raw['timestamp'] as string) ?? (raw['time'] as string) ?? new Date().toISOString(),
        ...
        partial.mitreTechnique =
          (raw['mitre_technique'] as string) ?? mitreMapping?.technique ?? null;
  ...
  private buildEvent(
    source: SiemSourceFormat,
    partial: Partial<NormalizedSiemEvent>,
  ): NormalizedSiemEvent {
  ...
  private runCorrelationRules(newEvents: NormalizedSiemEvent[]): SiemCorrelatedAlert[] {
    ...
    for (const rule of enabledRules) {
      ...
      const matchingEvents = windowEvents.filter((event) => {
        return rule.pattern.conditions.every((cond) => {
          const fieldValue = String(
            (event as unknown as Record<string, unknown>)[cond.field] ?? '',
          ).toLowerCase();
          switch (cond.operator) {
            case 'eq':
              return fieldValue === cond.value.toLowerCase();
            case 'contains':
      ...
      if (matchingEvents.length >= rule.pattern.minEvents) {
        const sourceIps = [
          ...new Set(matchingEvents.map((e) => e.sourceIp).filter((ip): ip is string => !!ip)),
        ];
        const firstSeen =
          matchingEvents.map((e) => e.timestamp).sort()[0] ?? new Date().toISOString();
        const lastSeen =
          matchingEvents
            .map((e) => e.timestamp)
            .sort()
  ...
  private mapOutcome(outcome?: string): NormalizedSiemEvent['outcome'] {
    ...
    if (o === 'failure' || o === 'failed' || o === 'deny' || o === 'block' || o === 'reject')
      return 'failure';
    return 'unknown';
  }
}
