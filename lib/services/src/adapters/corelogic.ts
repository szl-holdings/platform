...
const MOCK_PROPERTIES: CoreLogicPropertyRecord[] = [
  {
    apn: '4015-001-028',
    fipsCode: '36081',
    address: '1890 Adam Clayton Powell Blvd',
    city: 'New York',
    state: 'NY',
    zipCode: '10026',
    latitude: 40.805,
    longitude: -73.951,
    ownerName: 'R&B HOLDING CORP',
    ownerAddress: 'PO BOX 4521, NEW YORK NY 10037',
    landUse: 'Mixed Residential and Commercial',
    ...
    marketValue: 1600000,
    assessedYear: 2025,
    lastSaleDate: '2015-03-12',
    lastSalePrice: 950000,
    mortgageAmount: 780000,
    mortgageLender: 'CHASE BANK NA',
  ...
  },
  {
    apn: '1008-042-015',
    fipsCode: '36061',
    address: '45 Warren St',
    city: 'New York',
    state: 'NY',
    zipCode: '10007',
    latitude: 40.714,
    longitude: -74.008,
    ownerName: 'W.CAPITAL PARTNERS LLC',
    ownerAddress: '1 WORLD TRADE CTR FL 85, NY 10007',
...
export class CoreLogicAdapter extends ServiceAdapter {
  ...
  private get apiKey(): string | undefined {
    return process.env['CORELOGIC_API_KEY'];
  }
  private get clientId(): string | undefined {
    return process.env['CORELOGIC_CLIENT_ID'];
  }
  ...
  protected async performHealthCheck(): Promise<void> {
    await this.clRequest('/properties', {
      address: '123 Main St',
      city: 'New York',
      state: 'NY',
      limit: '1',
  ...
  async searchProperties(
    ...
    city: string,
    state: string,
  ): Promise<CoreLogicPropertyRecord[]> {
    if (this.isDemoMode) return MOCK_PROPERTIES;
    const data = await this.clRequest<{ properties: Array<Record<string, unknown>> }>(
      '/properties',
      { address, city, state },
    );
    return (data.properties ?? []).map((p) => ({
      apn: String(p['apn'] ?? ''),
      fipsCode: String(p['fipsCode'] ?? ''),
      ...
      ownerAddress: p['ownerAddress'] ? String(p['ownerAddress']) : null,
      landUse: String(p['landUse'] ?? ''),
  ...
  async getFloodZone(
    ...
    state: string,
    zip: string,
  ): Promise<CoreLogicFloodZone | null> {
    if (this.isDemoMode) {
      return {
        address,
        floodZone: 'X',
        floodZoneDescription: 'Area of minimal flood hazard',
        panelNumber: '36081C0306H',
        effectiveDate: '2021-09-02',
        inSpecialFloodHazardArea: false,
      };
    }
    const data = await this.clRequest<{ floodZone: Record<string, unknown> }>('/flood-zone', {
      address,
      city,
      state,
      zipCode: zip,
    ...
    return {
      address,
      floodZone: String(fz['zone'] ?? ''),
      floodZoneDescription: String(fz['description'] ?? ''),
      panelNumber: String(fz['panelNumber'] ?? ''),
      effectiveDate: String(fz['effectiveDate'] ?? ''),
