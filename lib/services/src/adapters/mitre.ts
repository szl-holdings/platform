...
export class MITREAdapter extends ServiceAdapter {
  readonly name = "mitre";
  readonly description = "MITRE ATT&CK Enterprise Matrix — Tactics, Techniques & Procedures (TTPs). Free, no key required. Hosted on GitHub.";
  ...
  protected async performHealthCheck(): Promise<void> {
    ...
    if (!Array.isArray(patterns) || patterns.length === 0) throw new Error("MITRE ATT&CK returned no techniques");
  }
}
