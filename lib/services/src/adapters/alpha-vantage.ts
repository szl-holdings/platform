...
export class AlphaVantageAdapter extends ServiceAdapter {
  ...
  async getQuote(symbol: string): Promise<AVQuote> {
    if (this.isDemoMode)
      return MOCK_QUOTES[symbol.toUpperCase()] ?? { ...MOCK_QUOTES['AAPL']!, symbol };
    const data = await this.avRequest<{ 'Global Quote': Record<string, string> }>({
  ...
  async getDailyTimeSeries(
    ...
    const data = await this.avRequest<Record<string, unknown>>({
    ...
    return {
      ...
      dataPoints: Object.entries(series ?? {}).map(([ts, v]) => ({
        ...
        adjustedClose: parseFloat(v['5. adjusted close'] ?? '0'),
      })),
    };
  }
  ...
  async getForexRate(fromCurrency: string, toCurrency: string): Promise<AVForexRate> {
    ...
    const data = await this.avRequest<Record<string, Record<string, string>>>({
      ...
      return {
        ...
        quarterlyEarnings: [
          ...
          },
          {
            ...
            estimatedEPS: '1.54',
            surprise: '0.04',
            surprisePercentage: '2.60',
          },
        ],
      };
    }
    const data = await this.avRequest<{
  ...
  getMockQuotes(): Record<string, AVQuote> {
    return MOCK_QUOTES;
  }
}
