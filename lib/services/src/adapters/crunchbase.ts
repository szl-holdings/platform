...
const MOCK_ORGS: CrunchbaseOrganization[] = [
  {
    uuid: 'a4d0f3e8-9c12-4b21-8ab1-1234567890ab',
    permalink: 'nexgen-advisors',
    name: 'NexGen Advisors',
    shortDescription: 'AI-driven investment advisory platform for institutional clients',
    description:
      'NexGen Advisors provides algorithmic trading strategies and AI-powered portfolio construction tools for hedge funds and family offices.',
    primaryRole: 'company',
    founded: '2018-03-01',
    closedOn: null,
  ...
  },
  {
    ...
    permalink: 'meridian-properties',
    name: 'Meridian Properties',
    shortDescription: 'Commercial and residential real estate development and management',
    description: null,
    primaryRole: 'company',
    founded: '2001-01-01',
    closedOn: null,
    operatingStatus: 'active',
    categories: ['Real Estate', 'Commercial Real Estate', 'Property Management'],
    categoryGroups: ['Real Estate'],
    headquartersLocation: 'New York, New York, United States',
    website: 'https://meridian-prop.com',
    linkedin: 'meridian-properties-nyc',
    twitter: null,
...
export class CrunchbaseAdapter extends ServiceAdapter {
  ...
  async getOrganization(permalink: string): Promise<CrunchbaseOrganization | null> {
    ...
    return {
      ...
      investors: Array.isArray(p['investor_identifiers'])
        ? (p['investor_identifiers'] as Array<Record<string, unknown>>).map((i) =>
            String(i['value'] ?? ''),
          )
        : [],
      ipo: null,
      acquisitions: Number(p['num_acquisitions'] ?? 0),
      acquisitions_by: 0,
  ...
  async searchOrganizations(query: string, limit = 10): Promise<CrunchbaseOrganization[]> {
    if (this.isDemoMode)
      return MOCK_ORGS.filter((o) => o.name.toLowerCase().includes(query.toLowerCase()));
    const body = {
      field_ids: [
        'identifier',
        'short_description',
        'primary_job_functions',
        'categories',
  ...
  getMockOrganizations(): CrunchbaseOrganization[] {
    return MOCK_ORGS;
  }
}
