...
export class DataverseAdapter extends ServiceAdapter {
  ...
  readonly description =
    'Dynamics 365 Online / Dataverse — CRM entity sync (Accounts, Contacts, Leads, Opportunities, Activities) via Dataverse Web API v9.2';
  ...
  async testConnection(
    ...
    try {
      ...
      return {
        connected: true,
        orgUrl: url,
        tenantId: tenantId ?? process.env['DATAVERSE_TENANT_ID'],
      };
    } catch (err) {
      return {
        connected: false,
  ...
  async getContact(
    ...
    return {
      ...
      accountId: data['_parentcustomerid_value']
        ? String(data['_parentcustomerid_value'])
        : undefined,
      statecode: Number(data.statecode ?? 0),
      createdOn: String(data.createdon ?? new Date().toISOString()),
      modifiedOn: String(data.modifiedon ?? new Date().toISOString()),
  ...
  async updateContact(
    ...
    await this.dataverseFetch(url, `/contacts(${contactId})`, tenantId, clientId, clientSecret, {
      method: 'PATCH',
  ...
  async updateLead(
    ...
    await this.dataverseFetch(url, `/leads(${leadId})`, tenantId, clientId, clientSecret, {
      method: 'PATCH',
      body: JSON.stringify(body),
    });
    return { success: true };
  ...
  async getOpportunity(
    ...
    clientId?: string,
    clientSecret?: string,
  ): Promise<DataverseOpportunity | null> {
    if (!this.isLive && !orgUrl)
      return MOCK_OPPORTUNITIES.find((o) => o.id === opportunityId) ?? null;
    const url = orgUrl ?? process.env['DATAVERSE_ORG_URL']!;
    const data = (await this.dataverseFetch(
      url,
    ...
    return {
      ...
      name: String(data.name ?? ''),
      accountId: data['_parentaccountid_value']
        ? String(data['_parentaccountid_value'])
        : undefined,
      contactId: data['_parentcontactid_value']
        ? String(data['_parentcontactid_value'])
        : undefined,
      estimatedvalue: data.estimatedvalue ? Number(data.estimatedvalue) : undefined,
      estimatedclosedate: data.estimatedclosedate ? String(data.estimatedclosedate) : undefined,
      statecode: Number(data.statecode ?? 0),
  ...
  async listActivities(
    ...
    return (data.value ?? []).map((r) => ({
      ...
      regardingObjectId: r['_regardingobjectid_value']
        ? String(r['_regardingobjectid_value'])
        : undefined,
  ...
  async getActivity(
    ...
    clientId?: string,
    clientSecret?: string,
  ): Promise<DataverseActivity | null> {
    ...
    return {
      ...
      activityType,
      regardingObjectId: data['_regardingobjectid_value']
        ? String(data['_regardingobjectid_value'])
        : undefined,
  ...
  async createContact(
    ...
    try {
      ...
      throw new Error(
        `Failed to create Dataverse contact: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  async createLead(
    ...
    try {
      ...
      throw new Error(
        `Failed to create Dataverse lead: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  async updateOpportunityStage(
    ...
    try {
      ...
      throw new Error(
        `Failed to update opportunity stage: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  async createActivity(
    ...
    const url = orgUrl ?? process.env['DATAVERSE_ORG_URL']!;
    const entityPath =
      data.activityType === 'phonecall'
        ? '/phonecalls'
        : data.activityType === 'email'
          ? '/emails'
          : data.activityType === 'appointment'
            ? '/appointments'
            : '/tasks';
    try {
      const body: Record<string, unknown> = {
      ...
      if (data.regardingObjectId && data.regardingObjectType) {
        body['regardingobjectid_opportunity@odata.bind'] =
          `/opportunities(${data.regardingObjectId})`;
      }
      ...
      throw new Error(
        `Failed to create Dataverse activity: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  async logNote(
    ...
    const url = orgUrl ?? process.env['DATAVERSE_ORG_URL']!;
    try {
      const res = (await this.dataverseFetch(
        url,
        '/annotations',
        tenantId,
        clientId,
      ...
      throw new Error(
        `Failed to log note in Dataverse: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
