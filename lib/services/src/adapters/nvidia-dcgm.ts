...
const DEMO_GPUS: DcgmGpuMetrics[] = [
  {
    ...
    eccSingleBit: 0,
    eccDoubleBit: 0,
    pcieRxBytesPerSec: 8_200_000_000,
    pcieTxBytesPerSec: 6_100_000_000,
    fanSpeedPct: 65,
    throttleReasons: [],
  },
  {
    ...
    eccSingleBit: 0,
    eccDoubleBit: 0,
    pcieRxBytesPerSec: 7_400_000_000,
    pcieTxBytesPerSec: 5_800_000_000,
    fanSpeedPct: 62,
    throttleReasons: [],
  },
  {
    gpuIndex: 2,
    uuid: 'GPU-i9j0k1l2',
    modelName: 'NVIDIA A100 80GB SXM',
    utilizationPct: 76,
    memoryUsedMb: 61_440,
    memoryTotalMb: 81_920,
    ...
    eccSingleBit: 2,
    eccDoubleBit: 0,
    pcieRxBytesPerSec: 5_900_000_000,
    pcieTxBytesPerSec: 4_200_000_000,
    fanSpeedPct: 55,
    throttleReasons: [],
  },
  {
    ...
    eccSingleBit: 0,
    eccDoubleBit: 0,
    pcieRxBytesPerSec: 3_100_000_000,
    pcieTxBytesPerSec: 2_400_000_000,
    fanSpeedPct: 45,
    throttleReasons: [],
  },
  {
    gpuIndex: 4,
    uuid: 'GPU-q7r8s9t0',
    modelName: 'NVIDIA H200 141GB HBM3e',
    utilizationPct: 97,
    memoryUsedMb: 132_096,
    memoryTotalMb: 144_384,
    ...
    eccSingleBit: 0,
    eccDoubleBit: 0,
    pcieRxBytesPerSec: 11_200_000_000,
    pcieTxBytesPerSec: 9_800_000_000,
    fanSpeedPct: 78,
    throttleReasons: ['sw_thermal'],
  },
];
...
export class NvidiaDcgmAdapter extends ServiceAdapter {
  ...
  readonly description =
    'NVIDIA DCGM — GPU cluster telemetry via DCGM Exporter Prometheus endpoints';
  ...
  async getGpuMetrics(): Promise<DcgmGpuMetrics[]> {
    ...
    for (const [key, metrics] of parsed) {
      ...
      if ((metrics.get('DCGM_FI_DEV_THERMAL_VIOLATION') ?? 0) > 0)
        throttleReasons.push('sw_thermal');
      if ((metrics.get('DCGM_FI_DEV_POWER_VIOLATION') ?? 0) > 0)
        throttleReasons.push('hw_power_brake');
      ...
      gpus.push({
        ...
        memoryTotalMb: Math.round(
          (metrics.get('DCGM_FI_DEV_FB_USED') ?? 0) + (metrics.get('DCGM_FI_DEV_FB_FREE') ?? 0),
        ),
  ...
  async getClusterSummary(): Promise<DcgmClusterSummary> {
    ...
    return {
      ...
      avgUtilization:
        gpus.length > 0 ? gpus.reduce((s, g) => s + g.utilizationPct, 0) / gpus.length : 0,
