...
const MOCK_HOST: ShodanHostInfo = {
  ...
  services: [
    ...
    },
    {
      ...
      product: 'nginx',
      version: '1.14.2',
      cpe: ['cpe:/a:nginx:nginx:1.14.2'],
      vulns: ['CVE-2019-20372'],
    },
    {
      port: 3306,
      protocol: 'tcp',
      banner: 'MySQL 5.6.49 Community Server',
      product: 'MySQL',
      version: '5.6.49',
      cpe: ['cpe:/a:mysql:mysql:5.6.49'],
      vulns: ['CVE-2020-14765'],
    },
  ],
};
...
export class ShodanAdapter extends ServiceAdapter {
  ...
  async getHostInfo(ip: string): Promise<ShodanHostInfo> {
    ...
    const services = ((data['data'] as Array<Record<string, unknown>>) ?? []).map((s) => ({
      ...
      protocol: String(s['transport'] ?? 'tcp'),
      banner: String(s['data'] ?? ''),
      product: s['product'] ? String(s['product']) : null,
  ...
  async search(query: string, limit = 20): Promise<ShodanSearchResult> {
    ...
    const data = await this.shodanRequest<{
      total: number;
      matches: Array<Record<string, unknown>>;
    }>('/shodan/host/search', { query, limit: String(limit) });
    return {
      total: data.total ?? 0,
      facets: {},
      matches: (data.matches ?? []).map((m) => ({
        ip: String(m['ip_str'] ?? ''),
        ipStr: String(m['ip_str'] ?? ''),
        org: String(m['org'] ?? ''),
        isp: String(m['isp'] ?? ''),
  ...
  async getExploits(query: string, limit = 10): Promise<ShodanExploit[]> {
    if (this.isDemoMode) {
      return [
        {
          id: 'edb-45621',
          description: `Remote Code Execution in ${query}`,
          author: 'anonymous',
          type: 'remote',
          platform: 'linux',
    ...
    const data = await this.shodanRequest<{ matches: Array<Record<string, unknown>> }>(
      '/api/exploits/search',
      { query, size: String(limit) },
    );
    return (data.matches ?? []).map((e) => ({
      ...
      datePublished: String(e['date'] ?? ''),
      cve: Array.isArray(e['cve']) ? (e['cve'] as string[]) : [],
      cwe: e['cwe'] ? String(e['cwe']) : null,
      verified: Boolean(e['verified']),
    }));
  }

  getMockHostInfo(): ShodanHostInfo {
    return MOCK_HOST;
  }
}
