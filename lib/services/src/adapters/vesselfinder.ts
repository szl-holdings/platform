...
const MOCK_POSITIONS: VesselFinderPosition[] = [
  {
    ...
    flag: 'DK',
    shipType: 79,
    typeText: 'Container Ship',
    destination: 'DEHAM',
    eta: '2026-04-20T06:00:00Z',
...
export class VesselFinderAdapter extends ServiceAdapter {
  ...
  async getVesselPosition(mmsi: string): Promise<VesselFinderPosition | null> {
    if (this.isDemoMode)
      return MOCK_POSITIONS.find((v) => v.mmsi === mmsi) ?? MOCK_POSITIONS[0] ?? null;
    const data = await this.vfRequest<{ data: Array<{ AIS: Record<string, unknown> }> }>(
      '/vessels',
      { mmsi },
    );
    ...
    return {
      ...
      imo: String(ais['IMO'] ?? ''),
      name: String(ais['NAME'] ?? ''),
      lat: Number(ais['LATITUDE'] ?? 0),
      lon: Number(ais['LONGITUDE'] ?? 0),
      speed: Number(ais['SPEED'] ?? 0),
      course: Number(ais['COURSE'] ?? 0),
      heading: Number(ais['HEADING'] ?? 511),
      status: statusNum,
      statusText: STATUS_TEXTS[statusNum] ?? 'Unknown',
      flag: String(ais['FLAG'] ?? ''),
      shipType: String(ais['TYPE'] ?? ''),
      typeText: String(ais['TYPENAME'] ?? ''),
      destination: String(ais['DESTINATION'] ?? ''),
      eta: String(ais['ETA'] ?? ''),
      received: String(ais['TIMESTAMP'] ?? ''),
    };
  }
  ...
  async getFleetPositions(mmsiList: string[]): Promise<VesselFinderPosition[]> {
    ...
    return results
      ...
      .map((r) => (r as PromiseFulfilledResult<VesselFinderPosition>).value);
  }
  ...
  async getVesselRoute(mmsi: string, hours = 24): Promise<VesselFinderRoutePoint[]> {
    ...
    const data = await this.vfRequest<{ data: Array<Record<string, unknown>> }>(
      '/vessels-pos-history',
      {
        mmsi,
        interval: '1h',
