...
const MOCK_LISTINGS: RedfinListing[] = [
  {
    ...
    estimatedMonthlyMortgage: null,
    walkScore: 99,
    transitScore: 100,
    bikeScore: 91,
    listingUrl: 'https://www.redfin.com/NY/New-York/1240-Broadway-10001',
    photoCount: 24,
  },
  {
    listingId: 'rf-1002',
    mlsId: 'REBNY-2026-9140',
    address: '312 W 23rd St Apt 4B',
    city: 'New York',
    state: 'NY',
    zipCode: '10011',
    latitude: 40.745,
    ...
    estimatedMonthlyMortgage: 4890,
    walkScore: 97,
    transitScore: 100,
    bikeScore: 85,
    listingUrl: 'https://www.redfin.com/NY/New-York/312-W-23rd-St-10011',
    photoCount: 18,
  },
];
...
const MOCK_MARKET_STATS: RedfinMarketStats = {
  regionId: '29470',
  regionName: 'Manhattan, NY',
  medianSalePrice: 1150000,
  medianPricePerSqFt: 1320,
  avgDaysOnMarket: 38,
  totalListings: 2847,
  newListings: 342,
  pendingSales: 518,
  soldLastMonth: 421,
  priceDropPercent: 22.4,
  period: '2026-03',
};
...
export class RedfinAdapter extends ServiceAdapter {
  ...
  private get apiKey(): string | undefined {
    return process.env['REDFIN_API_KEY'];
  }
  ...
  async searchListings(
    ...
    const data = await this.rfRequest<{ data: { homes: Array<Record<string, unknown>> } }>(
      '/properties/search',
      params,
    );
    return (data.data?.homes ?? []).map((h) => ({
      listingId: String(h['mlsId'] ?? ''),
      mlsId: h['mlsId'] ? String(h['mlsId']) : null,
      address: String(h['streetLine'] ?? ''),
      city: String(h['city'] ?? ''),
      state: String(h['state'] ?? ''),
      zipCode: String(h['zip'] ?? ''),
      latitude: Number(h['latitude'] ?? 0),
      longitude: Number(h['longitude'] ?? 0),
      price: Number(h['price'] ?? 0),
      pricePerSqFt: h['pricePerSqFt'] ? Number(h['pricePerSqFt']) : null,
      bedrooms: Number(h['beds'] ?? 0),
      bathrooms: Number(h['baths'] ?? 0),
      livingArea: Number(h['sqFt'] ?? 0),
      lotSize: h['lotSize'] ? Number(h['lotSize']) : null,
      yearBuilt: h['yearBuilt'] ? Number(h['yearBuilt']) : null,
      propertyType: String(h['propertyType'] ?? ''),
      listingStatus: String(h['status'] ?? ''),
      daysOnMarket: Number(h['dom'] ?? 0),
      priceDropPercent: h['priceDrop'] ? Number(h['priceDrop']) : null,
      estimatedMonthlyMortgage: null,
      walkScore: null,
      transitScore: null,
      bikeScore: null,
      listingUrl: String(h['url'] ?? ''),
      photoCount: Number(h['numPhotos'] ?? 0),
    }));
  }

  async getMarketStats(regionId: string): Promise<RedfinMarketStats> {
    if (this.isDemoMode) return MOCK_MARKET_STATS;
    const data = await this.rfRequest<{ data: Record<string, unknown> }>('/market-stats', {
      region_id: regionId,
    });
    const d = data.data;
    return {
      ...
      avgDaysOnMarket: Number(d['avgDom'] ?? 0),
      totalListings: Number(d['totalListings'] ?? 0),
      newListings: Number(d['newListings'] ?? 0),
      pendingSales: Number(d['pendingSales'] ?? 0),
      soldLastMonth: Number(d['sold'] ?? 0),
      priceDropPercent: Number(d['priceDropPercent'] ?? 0),
      period: String(d['period'] ?? ''),
    };
  }

  getMockListings(): RedfinListing[] {
    return MOCK_LISTINGS;
  }
  getMockMarketStats(): RedfinMarketStats {
