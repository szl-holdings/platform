...
export interface PagerDutyWebhookEvent {
  id: string;
  eventType: "incident.triggered" | "incident.acknowledged" | "incident.resolved" | "incident.escalated" | string;
...
const MOCK_INCIDENTS: PagerDutyIncident[] = [
  ...
  },
  {
    ...
    description: "Sentinel detected exploitation attempt against CVE-2024-3400 (PAN-OS GlobalProtect)",
    body: null,
    alertCount: 7,
...
const MOCK_ESCALATION_POLICIES: PagerDutyEscalationPolicy[] = [
  {
    ...
    numLoops: 2,
    rules: [
      { escalationDelayInMinutes: 15, targets: [{ id: "U1USR01", type: "user", name: "Dan Jones" }] },
      { escalationDelayInMinutes: 30, targets: [{ id: "S1SCH01", type: "schedule", name: "SRE Weekly Rotation" }] },
    ],
  },
  {
    id: "P1ESC02",
    ...
    numLoops: 1,
    rules: [
      { escalationDelayInMinutes: 10, targets: [{ id: "U1USR02", type: "user", name: "Bob Martinez" }] },
    ],
  },
  {
    id: "P1ESC03",
    ...
    numLoops: 3,
    rules: [
      { escalationDelayInMinutes: 5, targets: [{ id: "U1USR03", type: "user", name: "Alice Chen" }] },
      { escalationDelayInMinutes: 15, targets: [{ id: "S1SCH02", type: "schedule", name: "SOC On-Call" }] },
    ],
  },
];
...
export class PagerDutyAdapter extends ServiceAdapter {
  ...
  readonly description = "PagerDuty — incident management, on-call scheduling, and escalation policies";
  ...
  async testConnection(): Promise<PagerDutyConnectionStatus> {
    ...
    try {
      const data = await this.pdRequest<{ user: { id: string; name: string; email: string; account?: { id: string; name: string } } }>("/users/me");
      return {
        connected: true,
        userId: data.user.id,
  ...
  async handleWebhookEvent(
    ...
    if (this.webhookSecret) {
      if (!signature) {
        throw new Error("PagerDuty webhook signature required: X-PagerDuty-Signature header missing");
      }
    ...
    return {
      ...
      incident: {
        ...
        status: ((incident["status"] as string | undefined) ?? "triggered") as PagerDutyIncident["status"],
        urgency: ((incident["urgency"] as string | undefined) ?? "high") as PagerDutyIncident["urgency"],
        priority: null,
