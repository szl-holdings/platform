...
const MOCK_PULSES: OTXPulse[] = [
  {
    id: '6631abc45f8e4b0012f3d821',
    name: 'APT28 — Phishing Campaign Q1 2026',
    description:
      'New spear-phishing campaign attributed to APT28 targeting financial sector institutions with credential harvesting payloads.',
    author: 'OTX_Community',
    created: '2026-03-15T08:12:00Z',
    modified: '2026-03-28T14:20:00Z',
    tlp: 'green',
    tags: ['apt28', 'phishing', 'financial', 'credential-harvesting'],
    industries: ['Financial', 'Banking'],
    targetedCountries: ['US', 'UK', 'DE'],
    malwareFamilies: ['SOURFACE', 'EVILTOSS'],
    attackIds: [
      { id: 'T1566.002', display_name: 'Phishing: Spearphishing Link' },
      { id: 'T1078', display_name: 'Valid Accounts' },
    ],
    indicators: 47,
    references: ['https://attack.mitre.org/groups/G0007/'],
  },
  {
    id: '6619def2a1b2c0003e4f5902',
    name: 'Scattered Spider — Social Engineering Ops',
    description:
      'UNC3944 / Scattered Spider executing SIM swapping and social engineering against enterprise help desks.',
    author: 'AlienLabs',
    created: '2026-02-28T11:00:00Z',
    modified: '2026-03-10T09:45:00Z',
    tlp: 'white',
    tags: ['unc3944', 'social-engineering', 'sim-swap', 'help-desk'],
    industries: ['Technology', 'Telecommunications', 'Financial'],
    targetedCountries: ['US', 'CA'],
    malwareFamilies: ['ALPHV', 'BlackCat'],
    attackIds: [
      { id: 'T1566', display_name: 'Phishing' },
      { id: 'T1199', display_name: 'Trusted Relationship' },
    ],
    indicators: 23,
...
export class AlienVaultOTXAdapter extends ServiceAdapter {
  ...
  async lookupIP(ip: string): Promise<OTXIndicatorResult> {
    ...
    const pulses = (
      ((general['pulse_info'] as Record<string, unknown>)?.['pulses'] as Array<
    ...
    return {
      ...
      geolocation: {
        country: String(general['country_name'] ?? ''),
        city: general['city'] ? String(general['city']) : null,
      },
      malwareFamilies: ((malware['data'] as Array<Record<string, unknown>>) ?? []).map((m) =>
        String(m['family'] ?? ''),
      ),
    };
  }
  ...
  async lookupDomain(domain: string): Promise<OTXIndicatorResult> {
    ...
    const data = await this.otxRequest<Record<string, unknown>>(
      `/indicators/domain/${domain}/general`,
    );
    const pulses = (
      ((data['pulse_info'] as Record<string, unknown>)?.['pulses'] as Array<
        Record<string, unknown>
      >) ?? []
    ).slice(0, 5);
    return {
  ...
  async getSubscribedPulses(limit = 20, since?: string): Promise<OTXThreatIntelFeed> {
    if (this.isDemoMode) {
      return {
        totalPulses: MOCK_PULSES.length,
        recentPulses: MOCK_PULSES,
        industriesSummary: [
          { industry: 'Financial', count: 12 },
          { industry: 'Technology', count: 8 },
          { industry: 'Healthcare', count: 5 },
        ],
        tacticsDistribution: [
          { tactic: 'Initial Access', count: 18 },
          { tactic: 'Credential Access', count: 14 },
    ...
    const params: Record<string, string> = {
      limit: String(limit),
      modified_since: since ?? new Date(Date.now() - 7 * 86400000).toISOString(),
    };
    const data = await this.otxRequest<{ results: Array<Record<string, unknown>>; count: number }>(
      '/pulses/subscribed',
      params,
    );
    return {
      totalPulses: data.count ?? 0,
      recentPulses: (data.results ?? []).map((p) => this.mapPulse(p)),
      industriesSummary: [],
      tacticsDistribution: [],
    };
  }
  ...
  private mapPulse(p: Record<string, unknown>): OTXPulse {
    return {
      ...
      author: String(
        (p['author'] as Record<string, unknown>)?.['username'] ?? p['author_name'] ?? '',
      ),
      created: String(p['created'] ?? ''),
      modified: String(p['modified'] ?? ''),
      tlp: String(p['tlp'] ?? 'white'),
      tags: Array.isArray(p['tags']) ? (p['tags'] as string[]) : [],
      industries: Array.isArray(p['industries']) ? (p['industries'] as string[]) : [],
      targetedCountries: Array.isArray(p['targeted_countries'])
        ? (p['targeted_countries'] as string[])
        : [],
      malwareFamilies: Array.isArray(p['malware_families'])
        ? (p['malware_families'] as Array<Record<string, string>>).map(
            (m) => m['display_name'] ?? '',
          )
        : [],
      attackIds: Array.isArray(p['attack_ids'])
        ? (p['attack_ids'] as Array<{ id: string; display_name: string }>)
        : [],
