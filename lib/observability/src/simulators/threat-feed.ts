...
export type IocType = "ipv4-addr" | "domain-name" | "file:hashes.MD5" | "file:hashes.SHA-256" | "url" | "email-addr" | "autonomous-system";
export type KillChainPhase = "reconnaissance" | "weaponization" | "delivery" | "exploitation" | "installation" | "command-and-control" | "actions-on-objectives";
...
const APT_CAMPAIGNS = [
  {
    ...
    mitreAttack: ["T1566.001", "T1003.001", "T1021.002", "T1078", "T1071.001", "T1567.002", "T1105"],
    description: "Sophisticated spear-phishing campaign targeting financial infrastructure. Uses SolarWinds-style supply chain TTPs.",
  },
  {
    ...
    description: "State-sponsored group targeting IP theft in semiconductor and pharmaceutical sectors.",
  },
  {
    ...
    description: "ICS/SCADA targeting campaign with history of power grid disruption in Eastern Europe.",
  },
  {
    ...
    description: "DPRK-attributed financially motivated APT targeting crypto exchanges and SWIFT banking network.",
  },
];

const FEED_SOURCES: Omit<FeedSource, "lastIngested" | "iocCount" | "staleness" | "ingestRatePerHour" | "status">[] = [
...
const DOMAINS = [
  "update-service-win.com", "cdn-delivery-node.net", "secure-login-portal.org",
  "account-verification-api.com", "telemetry-beacon.io", "auth-service-proxy.net",
  "cloud-sync-agent.com", "mail-delivery-system.org", "ms-update-srv.com",
  "adobe-update-cdn.net", "vpn-gateway-secure.com", "antivirus-updater.net",
];
const SHA256_PREFIXES = ["a3f8c2", "b7e4d1", "9c1a5f", "4d8b3e", "f2c7a9", "1e6d4b", "8a2c5f"];
const MITRE_TECHNIQUES = [
  "T1566.001", "T1078", "T1021.002", "T1055", "T1486", "T1003.001",
  "T1071.001", "T1105", "T1059.001", "T1190", "T1027", "T1140",
  "T1548.002", "T1070.004", "T1053.005", "T1195", "T1499", "T1562.001",
];
...
export class ThreatFeedSimulator {
  ...
  generateFeedSources(nowMs = Date.now()): FeedSource[] {
    ...
    return FEED_SOURCES.map(base => {
      ...
      return {
        ...
        staleness: hoursAgo < 1 ? "fresh" : hoursAgo < 6 ? "recent" : hoursAgo < 24 ? "stale" : "expired",
  ...
  generateIocs(count = 40, nowMs = Date.now()): StixIoc[] {
    ...
    const iocTypes: IocType[] = ["ipv4-addr", "domain-name", "file:hashes.SHA-256", "url", "email-addr", "autonomous-system"];
    const killChainPhases: KillChainPhase[] = [
      "reconnaissance", "weaponization", "delivery", "exploitation",
      "installation", "command-and-control", "actions-on-objectives",
    ];
    ...
    for (let i = 0; i < count; i++) {
      ...
      const confidence = campaign ? rng.int(campaign.confidence - 15, campaign.confidence + 5) : rng.int(40, 90);

      let value: string;
      switch (iocType) {
        case "ipv4-addr": value = generateIpAddress(rng); break;
        case "domain-name": value = rng.pick(DOMAINS); break;
        case "file:hashes.SHA-256": value = generateSha256(rng); break;
        case "url": value = `http://${rng.pick(DOMAINS)}/${rng.pick(["update", "payload", "exec", "install"])}.exe`; break;
        case "email-addr": value = `${rng.pick(["admin", "noreply", "security", "support"])}@${rng.pick(DOMAINS)}`; break;
        case "autonomous-system": value = `AS${rng.int(10000, 65535)}`; break;
        default: value = generateIpAddress(rng);
      }
      ...
      const tlp: TlpLevel =
        ...
        severity === "high" ? rng.pick(["AMBER", "RED"] as TlpLevel[]) :
        severity === "medium" ? rng.pick(["GREEN", "AMBER"] as TlpLevel[]) : "WHITE";
