...
export class MetricCollector {
  ...
  private seedSimulatedData() {
    ...
    for (const metric of this.config.metrics) {
      ...
      const base = this.getBaseValueForMetric(metric);
  ...
  private generateInitialEvents(now: number): ObservabilityEvent[] {
    ...
    const eventTypes: ObservabilityEvent['type'][] = [
      'metric_threshold',
      'anomaly',
      'status_change',
      'deployment',
    ...
    const lensMessages: Record<LensId, string[]> = {
      signal: [
        domainLabels
          ? `Top signal: ${domainLabels.topSignalLabel} — ranked critical`
          : 'Priority signal detected — AI-ranked critical',
        'Signal correlation identified across 3 entities',
        'Noise filter active — 12 low-priority alerts suppressed',
      ],
      impact: [
        domainLabels
          ? `${domainLabels.impact} — exposure quantified`
          : 'Business impact quantified — revenue correlation active',
        'Cost-of-delay model updated with new data',
        'Opportunity cost projection recalculated',
      ],
      anticipation: [
        domainLabels
          ? `${domainLabels.anticipation} — forecast horizon updated`
          : 'Predictive model updated — 48h forecast horizon',
        'Anomaly trajectory converging — intervention recommended',
        'Behavioral model flagged emerging risk pattern',
      ],
      topology: [
        domainLabels
          ? `${domainLabels.topology} — dependency map refreshed`
          : 'Dependency topology refreshed — 3 new relationships mapped',
        'Graph traversal identified downstream impact path',
        'Relationship health: all critical dependencies nominal',
      ],
      posture: [
        domainLabels
          ? `${domainLabels.postureScoreName} recalculated — status: healthy`
          : 'Posture score recalculated — all dimensions nominal',
        'Continuous monitoring checkpoint passed',
        'Posture drift within acceptable bounds',
      ],
      velocity: [
        domainLabels
          ? `${domainLabels.velocity} — trend: improving`
          : 'Improvement velocity trending positive — 7-day MA up 4%',
  ...
  record(metricId: string, value: number, labels?: Record<string, string>) {
    ...
    if (def?.thresholds) {
      const exceeded =
        def.thresholds.direction === 'above'
  ...
  getSnapshot(): AppObservabilityState {
    ...
    const velocityTrend = velocityLens
      ? [velocityLens.score - 5, velocityLens.score - 3, velocityLens.score - 1, velocityLens.score]
      : [];
  ...
  private computeLensScores(snapshots: MetricSnapshot[]): LensScore[] {
    ...
    return LENS_IDS.map((lensId) => {
      ...
      const lensSnapshots = lensMetrics
        .map((m) => snapshots.find((s) => s.metricId === m.id))
      ...
      const score = Math.max(
        ...
        Math.min(
          100,
          Math.round(100 - (critCount / total) * 50 - (warnCount / total) * 20 + Math.random() * 5),
        ),
      );
