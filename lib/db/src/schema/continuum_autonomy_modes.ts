...
export const alloyAutonomyModesTable = pgTable(
  ...
  (table) => [
    uniqueIndex('continuum_autonomy_modes_tenant_domain_uniq')
      .on(table.tenantOrgId, table.domain)
      .where(sql`tenant_org_id IS NOT NULL`),
    uniqueIndex('continuum_autonomy_modes_global_domain_uniq')
      .on(table.domain)
      .where(sql`tenant_org_id IS NULL`),
    index('continuum_autonomy_modes_tenant_idx').on(table.tenantOrgId),
  ],
);
