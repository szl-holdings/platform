...
export const lyteSignalsTable = pgTable('lyte_signals', {
  ...
  sourceType: text('source_type', {
    enum: ['connector', 'webhook', 'manual', 'monitoring', 'scheduler'],
  }).notNull(),
  ...
  body: text('body'),
  status: text('status', { enum: ['new', 'acknowledged', 'resolved', 'dismissed'] })
    .notNull()
    .default('new'),
...
export const lyteCommandCardsTable = pgTable('lyte_command_cards', {
  ...
  category: text('category', {
    enum: ['operations', 'finance', 'growth', 'risk', 'compliance', 'strategy'],
  }).notNull(),
  priority: text('priority', { enum: ['critical', 'high', 'medium', 'low'] })
    .notNull()
    .default('medium'),
  status: text('status', { enum: ['pending', 'in_progress', 'completed', 'deferred'] })
    .notNull()
    .default('pending'),
...
export const lyteIncidentsTable = pgTable('lyte_incidents', {
  ...
  status: text('status', { enum: ['open', 'investigating', 'mitigating', 'resolved', 'closed'] })
    .notNull()
    .default('open'),
  assignee: text('assignee'),
...
export const lytePlaybooksTable = pgTable('lyte_playbooks', {
  ...
  description: text('description'),
  category: text('category', {
    enum: ['incident_response', 'operations', 'compliance', 'onboarding', 'escalation', 'general'],
  }).notNull(),
...
export const lyteRecommendationsTable = pgTable('lyte_recommendations', {
  ...
  effort: text('effort', { enum: ['high', 'medium', 'low'] })
    ...
    .default('medium'),
  status: text('status', {
    enum: ['suggested', 'accepted', 'in_progress', 'completed', 'dismissed'],
  })
    .notNull()
    .default('suggested'),
...
export const lyteReadinessItemsTable = pgTable('lyte_readiness_items', {
  ...
  itemType: text('item_type', {
    enum: ['launch_gate', 'blocker', 'dependency', 'milestone', 'owner_check'],
  }).notNull(),
  status: text('status', { enum: ['not_started', 'in_progress', 'blocked', 'complete', 'waived'] })
    .notNull()
    .default('not_started'),
  owner: text('owner'),
  dueAt: timestamp('due_at'),
...
export const insertLyteSavedViewSchema = createInsertSchema(lyteSavedViewsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertLyteReadinessItemSchema = createInsertSchema(lyteReadinessItemsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertLyteWorkspaceSchema = createInsertSchema(lyteWorkspacesTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertLyteSignalSchema = createInsertSchema(lyteSignalsTable).omit({
  id: true,
  createdAt: true,
});
...
export const insertLyteCommandCardSchema = createInsertSchema(lyteCommandCardsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertLyteIncidentSchema = createInsertSchema(lyteIncidentsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertLytePlaybookSchema = createInsertSchema(lytePlaybooksTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertLyteRecommendationSchema = createInsertSchema(lyteRecommendationsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const lytePrismScoresTable = pgTable(
  'lyte_prism_scores',
  {
    id: serial('id').primaryKey(),
    lens: text('lens', {
...
export const lyteMetricsTable = pgTable(
  'lyte_metrics',
  {
    id: serial('id').primaryKey(),
...
export const lyteAlertsTable = pgTable(
  'lyte_alerts',
  {
    id: serial('id').primaryKey(),
    name: text('name').notNull(),
    description: text('description'),
    alertType: text('alert_type', { enum: ['threshold', 'anomaly', 'composite'] })
      .notNull()
      .default('threshold'),
    service: text('service').notNull(),
    metricName: text('metric_name').notNull(),
    ...
    severity: text('severity', { enum: ['critical', 'high', 'medium', 'low'] })
      .notNull()
      .default('medium'),
    status: text('status', { enum: ['active', 'firing', 'resolved', 'silenced', 'draft'] })
      .notNull()
      .default('active'),
...
export const lyteAlertEventsTable = pgTable(
  'lyte_alert_events',
  {
    id: serial('id').primaryKey(),
    alertId: integer('alert_id').references(() => lyteAlertsTable.id, { onDelete: 'cascade' }),
    eventType: text('event_type', {
      enum: ['fired', 'resolved', 'silenced', 'acknowledged'],
    }).notNull(),
...
export const lyteEscalationsTable = pgTable(
  'lyte_escalations',
  {
    id: serial('id').primaryKey(),
    title: text('title').notNull(),
    ...
    severity: text('severity', { enum: ['critical', 'high', 'medium', 'low'] })
      .notNull()
      .default('high'),
    status: text('status', { enum: ['open', 'in_progress', 'escalated', 'resolved', 'closed'] })
      .notNull()
      .default('open'),
...
export const insertLytePrismScoreSchema = createInsertSchema(lytePrismScoresTable).omit({
  id: true,
  createdAt: true,
});
...
export const insertLyteMetricSchema = createInsertSchema(lyteMetricsTable).omit({
  id: true,
  createdAt: true,
});
...
export const insertLyteAlertSchema = createInsertSchema(lyteAlertsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertLyteAlertEventSchema = createInsertSchema(lyteAlertEventsTable).omit({
  id: true,
  createdAt: true,
});
...
export const insertLyteEscalationSchema = createInsertSchema(lyteEscalationsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
