...
export const portsTable = pgTable("ports", {
  ...
  portType: text("port_type", { enum: ["container", "bulk", "tanker", "multipurpose", "passenger", "fishing", "inland"] }).notNull().default("multipurpose"),
  status: text("status", { enum: ["open", "restricted", "closed", "congested"] }).notNull().default("open"),
  ...
  index("ports_org_idx").on(t.orgId),
  index("ports_country_idx").on(t.country),
]);
...
export const corridorsTable = pgTable("corridors", {
  ...
  originPortId: integer("origin_port_id").references(() => portsTable.id, { onDelete: "set null" }),
  destinationPortId: integer("destination_port_id").references(() => portsTable.id, { onDelete: "set null" }),
  distanceNm: numeric("distance_nm", { precision: 10, scale: 2 }),
  avgTransitDays: numeric("avg_transit_days", { precision: 5, scale: 2 }),
  riskLevel: text("risk_level", { enum: ["low", "moderate", "high", "critical"] }).notNull().default("low"),
...
export const maritimeVesselsTable = pgTable("maritime_vessels", {
  ...
  flag: text("flag"),
  vesselType: text("vessel_type", { enum: ["container", "tanker", "bulk", "cargo", "passenger", "ro-ro", "lpg", "lng", "chemical", "other"] }).notNull().default("container"),
  ...
  status: text("status", { enum: ["active", "in_port", "at_sea", "anchored", "maintenance", "decommissioned", "off_hire"] }).notNull().default("active"),
  latitude: numeric("latitude", { precision: 10, scale: 7 }),
  longitude: numeric("longitude", { precision: 10, scale: 7 }),
  ...
  currentPortId: integer("current_port_id").references(() => portsTable.id, { onDelete: "set null" }),
  destinationPortId: integer("destination_port_id").references(() => portsTable.id, { onDelete: "set null" }),
...
export const voyagesTable = pgTable("voyages", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull().references(() => organizationsTable.id, { onDelete: "cascade" }),
  vesselId: integer("vessel_id").notNull().references(() => maritimeVesselsTable.id, { onDelete: "cascade" }),
  ...
  status: text("status", { enum: ["planned", "loading", "departed", "at_sea", "arrived", "discharging", "completed", "cancelled", "diverted"] }).notNull().default("planned"),
  ...
  corridorId: integer("corridor_id").references(() => corridorsTable.id, { onDelete: "set null" }),
  waypoints: jsonb("waypoints"),
  metadata: jsonb("metadata"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
...
export const maritimeExceptionsTable = pgTable("maritime_exceptions", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull().references(() => organizationsTable.id, { onDelete: "cascade" }),
  voyageId: integer("voyage_id").references(() => voyagesTable.id, { onDelete: "set null" }),
  vesselId: integer("vessel_id").references(() => maritimeVesselsTable.id, { onDelete: "set null" }),
  signalId: integer("signal_id").references(() => platformSignalsTable.id, { onDelete: "set null" }),
  exceptionType: text("exception_type", { enum: ["eta_deviation", "route_deviation", "weather_delay", "port_congestion", "mechanical", "cargo_incident", "security_threat", "regulatory", "commercial", "other"] }).notNull(),
  severity: text("severity", { enum: ["critical", "high", "medium", "low"] }).notNull().default("medium"),
  title: text("title").notNull(),
  description: text("description"),
  status: text("status", { enum: ["new", "acknowledged", "assigned", "escalated", "resolved", "dismissed"] }).notNull().default("new"),
...
export const insertMaritimeVesselSchema = createInsertSchema(maritimeVesselsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertVoyageSchema = createInsertSchema(voyagesTable).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertVoyage = z.infer<typeof insertVoyageSchema>;
export type Voyage = typeof voyagesTable.$inferSelect;

export const insertMaritimeExceptionSchema = createInsertSchema(maritimeExceptionsTable).omit({ id: true, createdAt: true, updatedAt: true });
