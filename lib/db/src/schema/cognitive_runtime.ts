...
export const selfModelsTable = pgTable(
  'self_models',
  {
    ...
    performanceProfile: jsonb('performance_profile')
    ...
    freshnessLastUpdatedAt: timestamp('freshness_last_updated_at', { withTimezone: true })
      .notNull()
      .defaultNow(),
...
export const reflectionsTable = pgTable(
  'reflections',
  {
    ...
    improvements: jsonb('improvements')
      ...
      .notNull()
      .default([]),
    policyBreaches: jsonb('policy_breaches')
      .$type<
        {
          policyId: string;
          description: string;
...
export const cognitiveRollbackEventsTable = pgTable(
  'rollback_events',
  {
    ...
    targetType: text('target_type').notNull(),
    targetId: text('target_id').notNull(),
    stateBeforeRollback: jsonb('state_before_rollback')
      .$type<Record<string, unknown>>()
      .notNull()
      .default({}),
