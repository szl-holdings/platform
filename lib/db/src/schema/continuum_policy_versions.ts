...
export const continuumPolicyVersions = pgTable(
  'continuum_policy_versions',
  {
  ...
  (t) => [
    index('continuum_policy_versions_studio_idx').on(t.studioId, t.versionNumber),
    index('continuum_policy_versions_saved_idx').on(t.savedAt),
  ],
);
...
export type AlloyPolicyVersion = typeof continuumPolicyVersions.$inferSelect;
export type InsertAlloyPolicyVersion = typeof continuumPolicyVersions.$inferInsert;
