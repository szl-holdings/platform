...
export const orgBrandingTable = pgTable(
  'org_branding',
  {
  ...
  (t) => [index('org_branding_org_id_idx').on(t.orgId)],
);

// ─── Custom Domains ───────────────────────────────────────────────────────────

export const orgCustomDomainsTable = pgTable(
  'org_custom_domains',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id')
    ...
    status: text('status', {
      enum: ['pending_verification', 'verified', 'active', 'failed', 'disabled'],
    })
      .notNull()
      .default('pending_verification'),
    verificationMethod: text('verification_method', { enum: ['dns_txt', 'dns_cname', 'http_file'] })
      .notNull()
      .default('dns_txt'),
...
export const partnerAccountsTable = pgTable(
  'partner_accounts',
  {
    ...
    status: text('status', { enum: ['active', 'suspended', 'pending_approval'] })
      .notNull()
      .default('pending_approval'),
    tier: text('tier', { enum: ['referral', 'reseller', 'white_label', 'oem'] })
      .notNull()
      .default('reseller'),
...
export const partnerOrgAssignmentsTable = pgTable(
  'partner_org_assignments',
  {
    id: serial('id').primaryKey(),
    ...
    orgId: integer('org_id')
      .notNull()
      .references(() => organizationsTable.id, { onDelete: 'cascade' }),
    accessLevel: text('access_level', { enum: ['view', 'manage', 'admin'] })
      .notNull()
      .default('manage'),
    provisionedByUserId: integer('provisioned_by_user_id').references(() => usersTable.id, {
      onDelete: 'set null',
    }),
    notes: text('notes'),
    createdAt: timestamp('created_at').notNull().defaultNow(),
...
export const partnerUsersTable = pgTable(
  'partner_users',
  {
    id: serial('id').primaryKey(),
    partnerId: integer('partner_id')
...
export const insertOrgBrandingSchema = createInsertSchema(orgBrandingTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertOrgCustomDomainSchema = createInsertSchema(orgCustomDomainsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertPartnerAccountSchema = createInsertSchema(partnerAccountsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertPartnerOrgAssignmentSchema = createInsertSchema(partnerOrgAssignmentsTable).omit(
