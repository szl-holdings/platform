...
export const simulationSnapshotsTable = pgTable(
  'simulation_snapshots',
  {
    id: serial('id').primaryKey(),
    sessionId: text('session_id')
      .notNull()
      .references(() => simulationSessionsTable.sessionId, { onDelete: 'cascade' }),
    domain: text('domain').notNull(),
...
export const simulationResultsTable = pgTable(
  'simulation_results',
  {
    id: serial('id').primaryKey(),
    sessionId: text('session_id')
...
export const simulationReplayStateTable = pgTable(
  'simulation_replay_state',
  {
    id: serial('id').primaryKey(),
    sessionId: text('session_id')
      .notNull()
...
export const insertSimulationSnapshotSchema = createInsertSchema(simulationSnapshotsTable).omit({
  id: true,
});
...
export const insertSimulationResultSchema = createInsertSchema(simulationResultsTable).omit({
  id: true,
});
...
export const insertSimulationReplayStateSchema = createInsertSchema(
  simulationReplayStateTable,
).omit({ id: true });
