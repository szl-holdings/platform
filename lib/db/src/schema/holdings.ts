...
export const holdingsVenturesTable = pgTable("holdings_ventures", {
  ...
  sector: text("sector"),
  status: text("status", { enum: ["active", "growth", "stealth", "acquired", "sunset"] }).notNull().default("active"),
  stage: text("stage"),
...
export const holdingsMilestonesTable = pgTable("holdings_milestones", {
  id: serial("id").primaryKey(),
  ventureId: integer("venture_id").references(() => holdingsVenturesTable.id, { onDelete: "set null" }),
...
export const holdingsMetricsTable = pgTable("holdings_metrics", {
  id: serial("id").primaryKey(),
  ventureId: integer("venture_id").references(() => holdingsVenturesTable.id, { onDelete: "cascade" }),
...
export const insertHoldingsVentureSchema = createInsertSchema(holdingsVenturesTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertHoldingsMilestoneSchema = createInsertSchema(holdingsMilestonesTable).omit({ id: true, createdAt: true });
...
export const insertHoldingsMetricSchema = createInsertSchema(holdingsMetricsTable).omit({ id: true, createdAt: true });
...
export const insertHoldingsLeadershipSchema = createInsertSchema(holdingsLeadershipTable).omit({ id: true, createdAt: true });
...
export const insertHoldingsInquirySchema = createInsertSchema(holdingsInquiriesTable).omit({ id: true, createdAt: true, updatedAt: true });
