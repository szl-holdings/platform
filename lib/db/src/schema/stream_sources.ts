...
export const streamIngestedEventsTable = pgTable("stream_ingested_events", {
  ...
  sourceId: integer("source_id").references(() => streamDataSourcesTable.id, { onDelete: "set null" }),
  category: text("category").notNull(),
  type: text("type").notNull(),
...
export const insertStreamDataSourceSchema = createInsertSchema(streamDataSourcesTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertStreamIngestedEventSchema = createInsertSchema(streamIngestedEventsTable).omit({ id: true, normalizedAt: true });
