...
export const READINESS_SCORES_ENUM = [
  'not_started',
  'at_risk',
  'on_track',
  'ready',
...
export const szlProductsTable = pgTable(
  'szl_products',
  {
    ...
    productType: text('product_type', { enum: ['platform', 'module', 'service', 'vertical'] })
      .notNull()
      .default('platform'),
  ...
  (t) => [index('szl_products_org_idx').on(t.orgId)],
);

// ─── SIGNALS ───────────────────────────────────────────────────────────────────

export const szlSignalsTable = pgTable(
  'szl_signals',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    ...
    sourceType: text('source_type', {
      enum: ['connector', 'webhook', 'manual', 'monitoring', 'scheduler', 'ai'],
    }).notNull(),
...
export const szlActionsTable = pgTable(
  'szl_actions',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    ...
    escalationState: text('escalation_state', {
      enum: ['none', 'pending', 'escalated', 'resolved'],
    })
      .notNull()
      .default('none'),
...
export const szlWorkflowsTable = pgTable(
  'szl_workflows',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    ...
    triggerType: text('trigger_type', {
      enum: ['manual', 'scheduled', 'event', 'signal', 'webhook'],
    })
      .notNull()
      .default('manual'),
...
export const szlWorkflowRunsTable = pgTable(
  'szl_workflow_runs',
  {
    id: serial('id').primaryKey(),
    workflowId: integer('workflow_id')
      .notNull()
      .references(() => szlWorkflowsTable.id, { onDelete: 'cascade' }),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
...
export const szlApprovalsTable = pgTable(
  'szl_approvals',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    workflowRunId: integer('workflow_run_id').references(() => szlWorkflowRunsTable.id, {
      onDelete: 'cascade',
    }),
...
export const szlArtifactsTable = pgTable(
  'szl_artifacts',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    ...
    artifactType: text('artifact_type', {
      enum: ['report', 'document', 'analysis', 'recommendation', 'alert', 'data_export'],
    }).notNull(),
    title: text('title').notNull(),
...
export const szlCommentsTable = pgTable(
  'szl_comments',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    entityType: text('entity_type', {
      enum: ['signal', 'action', 'workflow_run', 'approval', 'artifact', 'exception'],
    }).notNull(),
...
export const szlEventLogTable = pgTable(
  'szl_event_log',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    actorId: integer('actor_id').references(() => usersTable.id, { onDelete: 'set null' }),
    actorType: text('actor_type', { enum: ['user', 'system', 'workflow', 'agent'] })
      .notNull()
      .default('system'),
...
export const szlFeatureFlagsTable = pgTable(
  'szl_feature_flags',
  {
    id: serial('id').primaryKey(),
  ...
  (t) => [uniqueIndex('szl_feature_flags_key_idx').on(t.key)],
);

// ─── VESSELS (canonical) ───────────────────────────────────────────────────────

export const szlVesselsTable = pgTable(
  'szl_vessels',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    ...
    maintenanceStatus: text('maintenance_status', { enum: MAINTENANCE_STATUSES })
      .notNull()
      .default('ok'),
...
export const szlPortsTable = pgTable('szl_ports', {
  ...
  portType: text('port_type', { enum: ['commercial', 'naval', 'fishing', 'ferry', 'inland'] })
    .notNull()
    .default('commercial'),
...
export const szlRoutesTable = pgTable(
  'szl_routes',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    name: text('name').notNull(),
    originPortId: integer('origin_port_id').references(() => szlPortsTable.id, {
      onDelete: 'set null',
    }),
    destinationPortId: integer('destination_port_id').references(() => szlPortsTable.id, {
      onDelete: 'set null',
    }),
  ...
  (t) => [index('szl_routes_org_idx').on(t.orgId)],
);

// ─── VOYAGES ───────────────────────────────────────────────────────────────────

export const szlVoyagesTable = pgTable(
  'szl_voyages',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    ...
    status: text('status', { enum: ['planned', 'active', 'completed', 'canceled', 'delayed'] })
      .notNull()
      .default('planned'),
...
export const szlExceptionsTable = pgTable(
  'szl_exceptions',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    entityType: text('entity_type', {
      enum: ['signal', 'voyage', 'vessel', 'workflow_run', 'action'],
    }).notNull(),
    entityId: integer('entity_id'),
    severity: text('severity', { enum: SEVERITY_LEVELS }).notNull().default('medium'),
    title: text('title').notNull(),
...
export const szlReadinessItemsTable = pgTable(
  'szl_readiness_items',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
...
export const insertSzlProductSchema = createInsertSchema(szlProductsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertSzlSignalSchema = createInsertSchema(szlSignalsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertSzlActionSchema = createInsertSchema(szlActionsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertSzlWorkflowSchema = createInsertSchema(szlWorkflowsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertSzlWorkflowRunSchema = createInsertSchema(szlWorkflowRunsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertSzlApprovalSchema = createInsertSchema(szlApprovalsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertSzlArtifactSchema = createInsertSchema(szlArtifactsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertSzlCommentSchema = createInsertSchema(szlCommentsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertSzlEventLogSchema = createInsertSchema(szlEventLogTable).omit({
  id: true,
  occurredAt: true,
});
...
export const insertSzlFeatureFlagSchema = createInsertSchema(szlFeatureFlagsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertSzlVesselSchema = createInsertSchema(szlVesselsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertSzlPortSchema = createInsertSchema(szlPortsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertSzlRouteSchema = createInsertSchema(szlRoutesTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertSzlVoyageSchema = createInsertSchema(szlVoyagesTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertSzlExceptionSchema = createInsertSchema(szlExceptionsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertSzlReadinessItemSchema = createInsertSchema(szlReadinessItemsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
