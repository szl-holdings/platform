import { pgTable, text, uuid, timestamp, integer, boolean, real, jsonb } from "drizzle-orm/pg-core";
...
export const atlasSignalsTable = pgTable("atlas_signals", {
  ...
  status: text("status", { enum: ["raw", "normalized", "processed", "acknowledged", "resolved"] }).notNull().default("raw"),
...
export const insertAtlasSignalSchema = createInsertSchema(atlasSignalsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertAtlasEvidenceSchema = createInsertSchema(atlasEvidenceTable).omit({ id: true, capturedAt: true });
...
export const insertAtlasOutcomeSchema = createInsertSchema(atlasOutcomesTable).omit({ id: true, recordedAt: true });
...
export const insertAtlasRunSchema = createInsertSchema(atlasRunsTable).omit({ id: true, snapshotAt: true });
