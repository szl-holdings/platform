import { pgTable, serial, text, integer, boolean, timestamp, jsonb, interval } from "drizzle-orm/pg-core";
...
export const dataRetentionAuditLogTable = pgTable("data_retention_audit_log", {
  id: serial("id").primaryKey(),
  policyId: integer("policy_id").references(() => dataRetentionPoliciesTable.id, { onDelete: "set null" }),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "set null" }),
  tableName: text("table_name").notNull(),
  action: text("action", {
    enum: ["policy_created", "policy_updated", "policy_deleted", "purge_started", "purge_completed", "purge_failed", "manual_trigger"],
  }).notNull(),
  actorId: integer("actor_id").references(() => usersTable.id, { onDelete: "set null" }),
  actorName: text("actor_name"),
  affectedRows: integer("affected_rows"),
  details: jsonb("details"),
  status: text("status", { enum: ["success", "failure", "partial"] }).notNull().default("success"),
