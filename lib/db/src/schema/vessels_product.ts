...
export const journeysTable = pgTable('journeys', {
  ...
  organizationId: integer('organization_id')
    .notNull()
    .references(() => organizationsTable.id, { onDelete: 'cascade' }),
  vesselAssetId: integer('vessel_asset_id').references(() => vesselsAssetsTable.id, {
    onDelete: 'set null',
  }),
...
export const insertVesselAssetSchema = createInsertSchema(vesselsAssetsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertVesselEventSchema = createInsertSchema(vesselEventsTable).omit({
  id: true,
  createdAt: true,
});
...
export const insertVesselAlertSchema2 = createInsertSchema(vesselAlertsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertVesselReportSchema = createInsertSchema(vesselReportsTable).omit({
  id: true,
  createdAt: true,
});
