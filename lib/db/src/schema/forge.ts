import { pgTable, varchar, text, timestamp, integer, boolean, jsonb, numeric, index, uniqueIndex } from "drizzle-orm/pg-core";
import { sql } from "drizzle-orm";
import { createInsertSchema, createSelectSchema } from "drizzle-zod";

const uuid = (name: string) =>
  varchar(name).notNull().default(sql`gen_random_uuid()`);
...
export const forgeModelVersionsTable = pgTable("forge_model_versions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  modelId: varchar("model_id").notNull().references(() => forgeModelsTable.id, { onDelete: "cascade" }),
...
export const forgePromptVersionsTable = pgTable("forge_prompt_versions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  promptId: varchar("prompt_id").notNull().references(() => forgePromptsTable.id, { onDelete: "cascade" }),
...
export const forgeDeploymentTargetsTable = pgTable("forge_deployment_targets", {
  ...
  computeProfile: jsonb("compute_profile").$type<Record<string, unknown>>().notNull().default(sql`'{}'::jsonb`),
  packageLock: jsonb("package_lock").$type<Record<string, unknown>>().notNull().default(sql`'{}'::jsonb`),
  ...
  allowedIntegrations: jsonb("allowed_integrations").$type<string[]>().notNull().default(sql`'[]'::jsonb`),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
...
export const forgeEnvironmentProfilesTable = pgTable("forge_environment_profiles", {
  ...
  targetId: varchar("target_id").references(() => forgeDeploymentTargetsTable.id, { onDelete: "set null" }),
  observabilityHook: varchar("observability_hook", { length: 200 }),
  requireApproval: boolean("require_approval").notNull().default(false),
  createdAt: timestamp("created_at").notNull().defaultNow(),
});

export const forgeEnvironmentSnapshotsTable = pgTable("forge_environment_snapshots", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  envId: varchar("env_id").notNull().references(() => forgeEnvironmentProfilesTable.id, { onDelete: "cascade" }),
  capturedAt: timestamp("captured_at").notNull().defaultNow(),
  agentInventory: jsonb("agent_inventory").$type<Record<string, unknown>>().notNull().default(sql`'{}'::jsonb`),
  modelInventory: jsonb("model_inventory").$type<Record<string, unknown>>().notNull().default(sql`'{}'::jsonb`),
  toolInventory: jsonb("tool_inventory").$type<Record<string, unknown>>().notNull().default(sql`'{}'::jsonb`),
...
// ─── Agents ───────────────────────────────────────────────────────────────
export const forgeAgentsTable = pgTable("forge_agents", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  slug: varchar("slug", { length: 200 }).notNull().unique(),
  name: text("name").notNull(),
  description: text("description"),
...
export const forgeAgentVersionsTable = pgTable("forge_agent_versions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  agentId: varchar("agent_id").notNull().references(() => forgeAgentsTable.id, { onDelete: "cascade" }),
  version: integer("version").notNull(),
  ...
  systemConfig: jsonb("system_config").$type<Record<string, unknown>>().notNull().default(sql`'{}'::jsonb`),
  evalsPassed: boolean("evals_passed").notNull().default(false),
...
export const forgeAgentToolPermissionsTable = pgTable("forge_agent_tool_permissions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  agentId: varchar("agent_id").notNull().references(() => forgeAgentsTable.id, { onDelete: "cascade" }),
  toolId: varchar("tool_id").notNull().references(() => forgeToolsTable.id, { onDelete: "cascade" }),
...
export const forgePolicyAssignmentsTable = pgTable("forge_policy_assignments", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  policyPackId: varchar("policy_pack_id").notNull().references(() => forgePolicyPacksTable.id, { onDelete: "cascade" }),
  agentId: varchar("agent_id").notNull().references(() => forgeAgentsTable.id, { onDelete: "cascade" }),
  envTier: varchar("env_tier", { length: 40 }).notNull(),
  createdAt: timestamp("created_at").notNull().defaultNow(),
});
...
// ─── Drift ────────────────────────────────────────────────────────────────
export const forgeDriftEventsTable = pgTable("forge_drift_events", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  agentId: varchar("agent_id").notNull().references(() => forgeAgentsTable.id, { onDelete: "cascade" }),
  envId: varchar("env_id").notNull().references(() => forgeEnvironmentProfilesTable.id, { onDelete: "cascade" }),
...
// ─── Promotions ───────────────────────────────────────────────────────────
export const forgePromotionsTable = pgTable("forge_promotions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  agentId: varchar("agent_id").notNull().references(() => forgeAgentsTable.id, { onDelete: "cascade" }),
  fromVersionId: varchar("from_version_id").references(() => forgeAgentVersionsTable.id),
  toVersionId: varchar("to_version_id").notNull().references(() => forgeAgentVersionsTable.id),
  ...
  blockers: jsonb("blockers").$type<Array<{code: string; message: string}>>().notNull().default(sql`'[]'::jsonb`),
  validationReport: jsonb("validation_report").$type<Record<string, unknown>>().notNull().default(sql`'{}'::jsonb`),
...
export const forgePromotionApprovalsTable = pgTable("forge_promotion_approvals", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  promotionId: varchar("promotion_id").notNull().references(() => forgePromotionsTable.id, { onDelete: "cascade" }),
...
// ─── Execution Runs ───────────────────────────────────────────────────────
export const forgeExecutionRunsTable = pgTable("forge_execution_runs", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  agentId: varchar("agent_id").notNull().references(() => forgeAgentsTable.id, { onDelete: "cascade" }),
  versionId: varchar("version_id").notNull().references(() => forgeAgentVersionsTable.id),
  envTier: varchar("env_tier", { length: 40 }).notNull(),
  modelId: varchar("model_id").references(() => forgeModelsTable.id),
  promptVersionId: varchar("prompt_version_id").references(() => forgePromptVersionsTable.id),
...
export const forgeExecutionArtifactsTable = pgTable("forge_execution_artifacts", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  executionId: varchar("execution_id").notNull().references(() => forgeExecutionRunsTable.id, { onDelete: "cascade" }),
...
// ─── Rollback & Audit ─────────────────────────────────────────────────────
export const forgeRollbackEventsTable = pgTable("forge_rollback_events", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  agentId: varchar("agent_id").notNull().references(() => forgeAgentsTable.id, { onDelete: "cascade" }),
  fromVersionId: varchar("from_version_id").references(() => forgeAgentVersionsTable.id),
...
export const forgeAuditEventsTable = pgTable("forge_audit_events", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
