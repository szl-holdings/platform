...
export const pcPartnerPortfolioSnapshotsTable = pgTable("pc_partner_portfolio_snapshots", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  snapshotType: text("snapshot_type", { enum: ["daily", "weekly", "manual"] }).notNull().default("daily"),
...
export const pcPartnerDigestRunsTable = pgTable("pc_partner_digest_runs", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  digestType: text("digest_type", { enum: [
    "weekly_partner", "high_pressure", "movement_opportunity",
    "bottleneck", "insurer_drag", "recovery_lien_drag"
  ] }).notNull(),
...
export const pcPartnerActionRequestsTable = pgTable("pc_partner_action_requests", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").references(() => pcMattersTable.id),
  requestType: text("request_type", { enum: [
    "reassign_review", "escalate_matter", "escalate_insurer_silence",
    "prioritize_recovery_review", "prioritize_missing_evidence",
    "request_partner_update_memo", "request_movement_memo",
    "request_high_pressure_digest", "request_readiness_improvement_plan"
  ] }).notNull(),
  ...
  urgency: text("urgency", { enum: ["critical", "high", "medium", "low"] }).notNull().default("medium"),
...
export const pcPartnerInterventionEventsTable = pgTable("pc_partner_intervention_events", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").references(() => pcMattersTable.id),
  actionRequestId: integer("action_request_id").references(() => pcPartnerActionRequestsTable.id),
  interventionType: text("intervention_type", { enum: [
    "partner_escalation", "reassignment", "insurer_escalation",
    "evidence_push", "recovery_push", "movement_push", "review_expedite"
  ] }).notNull(),
  intervenedBy: integer("intervened_by").notNull(),
  outcomeType: text("outcome_type", { enum: ["movement", "unblocked", "resolved", "pending", "no_effect"] }).notNull().default("pending"),
...
export const pcExportReadinessSnapshotsTable = pgTable("pc_export_readiness_snapshots", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  ...
  trend: text("trend", { enum: ["improving", "declining", "stable"] }).notNull().default("stable"),
...
export const pcSignoffBacklogSnapshotsTable = pgTable("pc_signoff_backlog_snapshots", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  ...
  riskLevel: text("risk_level", { enum: ["critical", "high", "moderate", "low"] }).notNull().default("low"),
  computedAt: timestamp("computed_at").notNull().defaultNow(),
  createdAt: timestamp("created_at").notNull().defaultNow(),
}, (table) => [
...
export const pcReviewBacklogSnapshotsTable = pgTable("pc_review_backlog_snapshots", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  totalPending: integer("total_pending").default(0),
  criticalCount: integer("critical_count").default(0),
...
export const pcMovementOpportunitySnapshotsTable = pgTable("pc_movement_opportunity_snapshots", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  opportunityScore: real("opportunity_score").notNull(),
  priorScore: real("prior_score"),
  opportunityType: text("opportunity_type", { enum: [
    "insurer_softening", "evidence_complete", "lien_resolved",
    "mediation_ready", "demand_ready", "follow_up_effective"
  ] }).notNull(),
...
export const pcPortfolioForecastsTable = pgTable("pc_portfolio_forecasts", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  forecastType: text("forecast_type", { enum: [
    "recovery_lien_drag_risk", "review_bottleneck_risk", "approval_lag_risk",
    "partner_intervention_leverage", "settlement_blocker_severity", "export_readiness_score"
  ] }).notNull(),
