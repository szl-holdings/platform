...
export const approvalRequestsTable = pgTable("approval_requests", {
  ...
  assignedApproverId: integer("assigned_approver_id").references(() => usersTable.id, { onDelete: "set null" }),
  ...
  escalatedToId: integer("escalated_to_id").references(() => usersTable.id, { onDelete: "set null" }),
...
export const approvalCommentsTable = pgTable("approval_comments", {
  id: serial("id").primaryKey(),
  approvalId: integer("approval_id").notNull().references(() => approvalRequestsTable.id, { onDelete: "cascade" }),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
...
export const approvalAuditTrailTable = pgTable("approval_audit_trail", {
  id: serial("id").primaryKey(),
  approvalId: integer("approval_id").notNull().references(() => approvalRequestsTable.id, { onDelete: "cascade" }),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
