...
export const insertAnalyticsEventSchema = createInsertSchema(analyticsEventsTable).omit({ receivedAt: true });
...
export const analyticsMetricDefinitionsTable = pgTable(
  "analytics_metric_definitions",
  {
    ...
    filterConditions: jsonb("filter_conditions").$type<Array<{ field: string; operator: string; value: unknown }>>().default([]),
    dimensions: jsonb("dimensions").$type<string[]>().default([]),
    unit: text("unit"),
    granularities: jsonb("granularities").$type<string[]>().default(["hour", "day", "week", "month"]),
  ...
  (t) => [
    index("analytics_metric_def_domain_idx").on(t.domain),
  ]
);

export const insertAnalyticsMetricDefinitionSchema = createInsertSchema(analyticsMetricDefinitionsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertAnalyticsMetricSnapshotSchema = createInsertSchema(analyticsMetricSnapshotsTable).omit({ computedAt: true });
...
export const insertAnalyticsAttributionTouchpointSchema = createInsertSchema(analyticsAttributionTouchpointsTable);
export type InsertAnalyticsAttributionTouchpoint = z.infer<typeof insertAnalyticsAttributionTouchpointSchema>;
export type AnalyticsAttributionTouchpoint = typeof analyticsAttributionTouchpointsTable.$inferSelect;
...
export const insertAnalyticsAttributionOutcomeSchema = createInsertSchema(analyticsAttributionOutcomesTable).omit({ id: true });
export type InsertAnalyticsAttributionOutcome = z.infer<typeof insertAnalyticsAttributionOutcomeSchema>;
...
export const analyticsCohortDefinitionsTable = pgTable(
  "analytics_cohort_definitions",
  {
    ...
    description: text("description"),
    entityType: text("entity_type").notNull(),
    entryConditions: jsonb("entry_conditions").$type<Array<{ field: string; operator: string; value: unknown }>>().notNull().default([]),
    entryEventName: text("entry_event_name"),
    analysisType: text("analysis_type", { enum: ["retention", "ltv", "engagement", "conversion"] }).notNull().default("retention"),
  ...
  (t) => [
    index("analytics_cohort_def_domain_idx").on(t.domain),
  ]
);

export const insertAnalyticsCohortDefinitionSchema = createInsertSchema(analyticsCohortDefinitionsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const analyticsFunnelDefinitionsTable = pgTable(
  ...
  (t) => [
    index("analytics_funnel_def_domain_idx").on(t.domain),
  ]
);

export const insertAnalyticsFunnelDefinitionSchema = createInsertSchema(analyticsFunnelDefinitionsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const analyticsAnomaliesTable = pgTable(
  "analytics_anomalies",
  {
    ...
    metricId: text("metric_id").notNull(),
    domain: text("domain").notNull(),
    anomalyType: text("anomaly_type", { enum: ["spike", "drop", "trend_change", "seasonal_deviation", "missing"] }).notNull(),
...
export const insertAnalyticsAnomalySchema = createInsertSchema(analyticsAnomaliesTable).omit({ id: true, detectedAt: true });
...
export const analyticsDashboardsTable = pgTable(
  "analytics_dashboards",
  {
    ...
    name: text("name").notNull(),
    description: text("description"),
    layout: jsonb("layout").$type<Array<{
      id: string;
  ...
  (t) => [
    index("analytics_dashboard_domain_idx").on(t.domain),
  ]
);

export const insertAnalyticsDashboardSchema = createInsertSchema(analyticsDashboardsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const analyticsExportJobsTable = pgTable(
  "analytics_export_jobs",
  {
    ...
    domain: text("domain").notNull(),
    exportType: text("export_type", { enum: ["events", "metric_snapshots", "funnel", "cohort", "anomalies"] }).notNull(),
    format: text("format", { enum: ["csv", "json", "parquet"] }).notNull().default("csv"),
...
export const insertAnalyticsExportJobSchema = createInsertSchema(analyticsExportJobsTable).omit({ id: true, createdAt: true });
