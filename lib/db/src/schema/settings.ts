import {
  pgTable, text, serial, timestamp, integer, jsonb, uniqueIndex, index,
  real, boolean,
...
export const tenantHealthScorecardsTable = pgTable(
  "tenant_health_scorecards",
  {
    ...
    billingStatus: text("billing_status", { enum: ["current", "overdue", "churned", "trial", "unknown"] }).notNull().default("unknown"),
    ...
    healthTier: text("health_tier", { enum: ["critical", "at_risk", "healthy", "champion"] }).notNull().default("healthy"),
