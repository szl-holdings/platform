import {
  pgTable, text, serial, timestamp, integer, numeric, jsonb, boolean, index, real
} from "drizzle-orm/pg-core";
import { pcMattersTable } from "./prism_counsel";

/* ─── Insurer Pressure Snapshots ─────────────────────────────────────────── */

export const pcInsurerPressureSnapshotsTable = pgTable("pc_insurer_pressure_snapshots", {
  ...
  direction: text("direction", { enum: ["rising", "falling", "stable", "new"] }).notNull().default("new"),
...
export const pcInsurerPressureDriversTable = pgTable("pc_insurer_pressure_drivers", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  snapshotId: integer("snapshot_id").references(() => pcInsurerPressureSnapshotsTable.id, { onDelete: "cascade" }),
  driverName: text("driver_name").notNull(),
  driverCategory: text("driver_category", { enum: [
...
export const pcSettlementFrictionSnapshotsTable = pgTable("pc_settlement_friction_snapshots", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  overallScore: real("overall_score").notNull(),
  ...
  frictionClass: text("friction_class", { enum: ["internal", "external", "mixed"] }).notNull().default("mixed"),
...
export const pcSettlementFrictionDriversTable = pgTable("pc_settlement_friction_drivers", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  snapshotId: integer("snapshot_id").references(() => pcSettlementFrictionSnapshotsTable.id, { onDelete: "cascade" }),
  driverName: text("driver_name").notNull(),
  blockerCategory: text("blocker_category", { enum: [
    "internal_process", "carrier_insurer", "evidence", "medical_damages",
    "governance_review", "venue_timing", "recovery_lien"
  ] }).notNull(),
  weight: real("weight").notNull().default(0.5),
  impact: text("impact", { enum: ["positive", "negative", "neutral"] }).notNull(),
  rawValue: text("raw_value"),
...
export const pcMovementRecommendationsTable = pgTable("pc_movement_recommendations", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  recommendationType: text("recommendation_type", { enum: [
    "reduce_friction", "counter_pressure", "unlock_movement", "improve_readiness",
    "carrier_engagement", "lien_resolution", "evidence_completion", "review_acceleration"
  ] }).notNull(),
  ...
  status: text("status", { enum: ["suggested", "accepted", "dismissed", "completed"] }).notNull().default("suggested"),
...
export const pcQuietRiskSnapshotsTable = pgTable("pc_quiet_risk_snapshots", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  ...
  index("pc_qr_snap_matter_idx").on(t.matterId),
  index("pc_qr_snap_org_idx").on(t.orgId),
]);

/* ─── Carrier Behavior Patterns ──────────────────────────────────────────── */

export const pcCarrierBehaviorPatternsTable = pgTable("pc_carrier_behavior_patterns", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  carrierName: text("carrier_name").notNull(),
  claimOffice: text("claim_office"),
  patternType: text("pattern_type", { enum: [
    "silence_delay", "verification_churn", "hardening_posture", "softening_posture",
    "reserve_increase", "denial_clustering", "adjuster_rotation", "escalation_avoidance",
    "offer_stall", "mediation_resistance", "early_settlement_tendency"
  ] }).notNull(),
...
export const pcCarrierResponseEventsTable = pgTable("pc_carrier_response_events", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  carrierName: text("carrier_name").notNull(),
  ...
  eventType: text("event_type", { enum: [
    "response_received", "silence_detected", "adjuster_changed", "escalation_triggered",
    "offer_made", "denial_issued", "verification_requested", "reserve_changed", "posture_shift"
  ] }).notNull(),
  description: text("description"),
...
export const pcCarrierSilenceWindowsTable = pgTable("pc_carrier_silence_windows", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  carrierName: text("carrier_name").notNull(),
  ...
  silenceRisk: text("silence_risk", { enum: ["none", "low", "medium", "high", "critical"] }).notNull().default("low"),
...
export const pcCarrierOfferBehaviorTable = pgTable("pc_carrier_offer_behavior", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  carrierName: text("carrier_name").notNull(),
  ...
  movementSignal: text("movement_signal", { enum: ["approaching", "stalling", "retreating", "opening", "closing", "new"] }).notNull().default("new"),
...
export const pcCarrierReserveBehaviorTable = pgTable("pc_carrier_reserve_behavior", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  carrierName: text("carrier_name").notNull(),
...
export const pcWorldlineSourceClassesTable = pgTable("pc_worldline_source_classes", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  ...
  status: text("status", { enum: ["active", "paused", "error", "disabled"] }).notNull().default("active"),
  tenantId: integer("tenant_id"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
}, (t) => [
  index("pc_wl_sc_org_idx").on(t.orgId),
  index("pc_wl_sc_class_idx").on(t.classKey),
]);

export const pcWorldlineSignalOverlaysTable = pgTable("pc_worldline_signal_overlays", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").references(() => pcMattersTable.id),
  signalId: integer("signal_id"),
  sourceClass: text("source_class").notNull(),
  overlayType: text("overlay_type", { enum: [
    "regulatory_impact", "weather_context", "venue_context", "demographic_context",
    "lien_exposure", "recovery_risk", "incident_environment"
  ] }).notNull(),
...
export const pcWorldlineWeatherEventsTable = pgTable("pc_worldline_weather_events", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
...
export const pcWorldlineCountyProfilesTable = pgTable("pc_worldline_county_profiles", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  ...
  index("pc_wl_county_org_idx").on(t.orgId),
  index("pc_wl_county_name_idx").on(t.county),
]);

export const pcWorldlineRegulatoryEventsTable = pgTable("pc_worldline_regulatory_events", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
...
export const pcWorldlineRecoveryMarkersTable = pgTable("pc_worldline_recovery_markers", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").references(() => pcMattersTable.id),
  markerType: text("marker_type", { enum: [
    "cms_msp_threshold", "medicare_conditional_payment", "medicaid_recovery",
    "erisa_lien_indicator", "workers_comp_crossover", "subrogation_risk"
  ] }).notNull(),
  description: text("description"),
...
export const pcPortfolioBenchmarkSnapshotsTable = pgTable("pc_portfolio_benchmark_snapshots", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  benchmarkType: text("benchmark_type", { enum: [
    "readiness_band", "insurer_pressure_cohort", "friction_cohort",
    "cycle_time_band", "settlement_range_band", "recovery_rate_band"
  ] }).notNull(),
...
export const pcPortfolioActionEffectivenessTable = pgTable("pc_portfolio_action_effectiveness", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
...
export const pcPortfolioTeamLagMetricsTable = pgTable("pc_portfolio_team_lag_metrics", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  metricType: text("metric_type", { enum: [
    "review_lag", "approval_lag", "demand_preparation_lag", "response_to_carrier_lag",
    "document_processing_lag", "mediation_prep_lag"
  ] }).notNull(),
...
export const pcPortfolioMatterCohortsTable = pgTable("pc_portfolio_matter_cohorts", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  cohortType: text("cohort_type", { enum: [
    "insurer_pressure", "settlement_friction", "quiet_risk",
    "movement_ready", "high_complexity", "stalled"
  ] }).notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
...
export type PcPortfolioActionEffectiveness = typeof pcPortfolioActionEffectivenessTable.$inferSelect;
