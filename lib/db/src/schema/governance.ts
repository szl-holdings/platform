...
export const alloyLegacyPoliciesTable = pgTable("alloy_policies", {
  ...
  policyType: text("policy_type", {
    enum: ["approval_matrix", "model_routing", "cost_ceiling", "agent_permission", "data_access", "compliance_template"],
  }).notNull(),
  scope: text("scope", {
    enum: ["global", "tenant", "team", "user"],
  }).notNull().default("tenant"),
...
export const modelRoutingPoliciesTable = pgTable("model_routing_policies", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
...
export const costBudgetsTable = pgTable("cost_budgets", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
  ...
  hardStopThreshold: numeric("hard_stop_threshold", { precision: 5, scale: 2 }).notNull().default("1.00"),
...
export const governanceIncidentsTable = pgTable("governance_incidents", {
  ...
  incidentType: text("incident_type", {
    enum: ["policy_violation", "budget_exceeded", "unauthorized_access", "agent_error", "manual_override", "data_breach_attempt"],
  }).notNull(),
