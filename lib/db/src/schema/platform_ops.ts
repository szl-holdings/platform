...
export const platformJobRunsTable = pgTable("platform_job_runs", {
  ...
  status: text("status", { enum: ["pending", "running", "completed", "completed_with_warnings", "failed"] }).notNull().default("pending"),
...
export const insertPlatformJobRunSchema = createInsertSchema(platformJobRunsTable).omit({ id: true, createdAt: true });
...
export const insertArtifactApprovalSchema = createInsertSchema(artifactApprovalsTable).omit({ id: true, createdAt: true });
