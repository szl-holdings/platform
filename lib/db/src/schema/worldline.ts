...
export type WorldlineSourceStatus =
  | "active"
  | "paused"
  | "degraded"
  | "inactive"
...
export const worldlineSourceRegistryTable = pgTable("worldline_source_registry", {
...
export const worldlineFetchLogsTable = pgTable("worldline_fetch_logs", {
  id: serial("id").primaryKey(),
  sourceId: integer("source_id").notNull().references(() => worldlineSourceRegistryTable.id, { onDelete: "cascade" }),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
...
export const worldlineSignalPublicationsTable = pgTable("worldline_signal_publications", {
  id: serial("id").primaryKey(),
  sourceId: integer("source_id").notNull().references(() => worldlineSourceRegistryTable.id, { onDelete: "cascade" }),
  fetchLogId: integer("fetch_log_id").references(() => worldlineFetchLogsTable.id, { onDelete: "set null" }),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
