...
export const complianceSuitabilityTable = pgTable("compliance_suitability", {
  ...
  riskTolerance: text("risk_tolerance", { enum: ["conservative", "moderate", "aggressive", "very_aggressive"] }).notNull(),
  ...
  status: text("status", { enum: ["draft", "pending_review", "approved", "rejected", "archived"] }).notNull().default("draft"),
...
export const complianceArchivalTable = pgTable("compliance_archival", {
  ...
  communicationType: text("communication_type", {
    enum: ["email", "chat", "voice_transcript", "written_correspondence", "trade_confirmation", "order_ticket", "advisory_agreement", "other"],
  }).notNull(),
...
export const complianceSupervisionQueueTable = pgTable("compliance_supervision_queue", {
  ...
  category: text("category", {
    enum: ["suitability_review", "reg_bi_violation", "concentration_risk", "best_execution", "outside_business", "communications_review", "complaint", "exception_report", "other"],
  }).notNull(),
  priority: text("priority", { enum: ["low", "medium", "high", "critical"] }).notNull().default("medium"),
  status: text("status", { enum: ["open", "in_review", "escalated", "resolved", "closed"] }).notNull().default("open"),
...
export const complianceCalendarTable = pgTable("compliance_calendar", {
  ...
  eventType: text("event_type", {
    enum: ["form_adv", "form_adv_part2", "form_crs", "annual_review", "exam_prep", "retention_review", "reg_bi_audit", "finra_exam", "sec_exam", "state_exam", "board_review", "policy_review", "other"],
  }).notNull(),
  title: text("title").notNull(),
  description: text("description"),
  dueAt: timestamp("due_at").notNull(),
  reminderAt: timestamp("reminder_at"),
  status: text("status", { enum: ["upcoming", "in_progress", "completed", "overdue", "waived"] }).notNull().default("upcoming"),
  assignedToId: text("assigned_to_id"),
  assignedToName: text("assigned_to_name"),
  ...
  notes: text("notes"),
  recurrence: text("recurrence", { enum: ["none", "annual", "quarterly", "monthly", "custom"] }).notNull().default("none"),
  metadata: jsonb("metadata"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
...
export const insertComplianceSuitabilitySchema = createInsertSchema(complianceSuitabilityTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertComplianceArchivalSchema = createInsertSchema(complianceArchivalTable).omit({ id: true, archivedAt: true });
...
export const insertComplianceSupervisionSchema = createInsertSchema(complianceSupervisionQueueTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertComplianceCalendarSchema = createInsertSchema(complianceCalendarTable).omit({ id: true, createdAt: true, updatedAt: true });
