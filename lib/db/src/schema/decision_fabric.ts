 ...
 */
export const policyVersionsTable = pgTable("decision_fabric_policy_versions", {
  ...
  authoredByUserId: integer("authored_by_user_id").references(() => usersTable.id, { onDelete: "set null" }),
 ...
 */
export const fabricSimulationSnapshotsTable = pgTable("decision_fabric_simulation_snapshots", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
 ...
 */
export const decisionRecordsTable = pgTable("decision_fabric_records", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
  domain: text("domain", { enum: FABRIC_DOMAINS }).notNull().default("general"),
  ...
  policyVersionId: integer("policy_version_id").references(() => policyVersionsTable.id, { onDelete: "set null" }),
  simulationSnapshotId: integer("simulation_snapshot_id").references(() => fabricSimulationSnapshotsTable.id, { onDelete: "set null" }),
 ...
 */
export const playbookSuggestionsTable = pgTable("decision_fabric_playbook_suggestions", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
  domain: text("domain", { enum: FABRIC_DOMAINS }).notNull().default("general"),
  title: text("title").notNull(),
 ...
 */
export const correlationLinksTable = pgTable("decision_fabric_correlation_links", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
...
export const insertPolicyVersionSchema = createInsertSchema(policyVersionsTable).omit({ id: true, capturedAt: true });
...
export const insertFabricSimulationSnapshotSchema = createInsertSchema(fabricSimulationSnapshotsTable).omit({ id: true, capturedAt: true });
...
export const insertDecisionRecordSchema = createInsertSchema(decisionRecordsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertPlaybookSuggestionSchema = createInsertSchema(playbookSuggestionsTable).omit({ id: true, createdAt: true });
...
export const insertCorrelationLinkSchema = createInsertSchema(correlationLinksTable).omit({ id: true, occurredAt: true });
