...
export const selfHealingPatternsTable = pgTable(
  'self_healing_patterns',
  {
    ...
    type: text('type', {
      enum: ['restart', 'scale', 'failover', 'clear_queue', 'rollback'],
    }).notNull(),
...
export const selfHealingRunsTable = pgTable(
  'self_healing_runs',
  {
    id: serial('id').primaryKey(),
    ...
    status: text('status', {
      enum: ['executing', 'pending_approval', 'completed', 'failed', 'queued'],
    }).notNull(),
...
export const insertSelfHealingPatternSchema = createInsertSchema(selfHealingPatternsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
export const insertSelfHealingRunSchema = createInsertSchema(selfHealingRunsTable).omit({
  id: true,
  createdAt: true,
});
