...
export const firestormScenariosTable = pgTable('firestorm_scenarios', {
  ...
  severity: text('severity', { enum: ['low', 'medium', 'high', 'critical'] })
    .notNull()
    .default('medium'),
  complexity: text('complexity', { enum: ['basic', 'intermediate', 'advanced', 'expert'] })
    .notNull()
...
export const firestormAssessmentsTable = pgTable('firestorm_assessments', {
  ...
  status: text('status', { enum: ['draft', 'scheduled', 'in_progress', 'completed', 'canceled'] })
    .notNull()
    .default('draft'),
  scope: text('scope'),
...
export const firestormSimulationRunsTable = pgTable('firestorm_simulation_runs', {
  id: serial('id').primaryKey(),
  assessmentId: integer('assessment_id').references(() => firestormAssessmentsTable.id, {
    onDelete: 'cascade',
  }),
  scenarioId: integer('scenario_id').references(() => firestormScenariosTable.id, {
    onDelete: 'set null',
  }),
  name: text('name').notNull(),
  status: text('status', { enum: ['pending', 'running', 'completed', 'failed', 'aborted'] })
    .notNull()
    .default('pending'),
  mode: text('mode', { enum: ['controlled', 'full', 'automated', 'demo'] })
    .notNull()
    .default('controlled'),
...
export const firestormFindingsTable = pgTable('firestorm_findings', {
  ...
  assessmentId: integer('assessment_id')
    .notNull()
    .references(() => firestormAssessmentsTable.id, { onDelete: 'cascade' }),
  simulationRunId: integer('simulation_run_id').references(() => firestormSimulationRunsTable.id, {
    onDelete: 'set null',
  }),
  title: text('title').notNull(),
  description: text('description'),
  severity: text('severity', { enum: ['info', 'low', 'medium', 'high', 'critical'] }).notNull(),
  status: text('status', { enum: ['open', 'confirmed', 'mitigated', 'accepted', 'false_positive'] })
    .notNull()
    .default('open'),
  category: text('category'),
...
export const firestormIncidentsTable = pgTable('firestorm_incidents', {
  ...
  severity: text('severity', { enum: ['low', 'medium', 'high', 'critical'] })
    ...
    .default('medium'),
  status: text('status', {
    enum: ['detection', 'triage', 'investigation', 'containment', 'remediation', 'closed'],
  })
    .notNull()
    .default('detection'),
...
export const firestormHardeningControlsTable = pgTable('firestorm_hardening_controls', {
  ...
  status: text('status', { enum: ['implemented', 'partial', 'not_implemented'] })
    .notNull()
  ...
  auditTrail: jsonb('audit_trail')
    .$type<Array<{ action: string; user: string; at: string }>>()
    .default([]),
...
export const insertFirestormHardeningControlSchema = createInsertSchema(
...
export const firestormComplianceControlsTable = pgTable('firestorm_compliance_controls', {
  ...
  description: text('description'),
  status: text('status', { enum: ['implemented', 'partial', 'not_implemented', 'not_applicable'] })
    .notNull()
    .default('not_implemented'),
...
export const firestormLeadsTable = pgTable('firestorm_leads', {
  id: serial('id').primaryKey(),
  campaignId: integer('campaign_id').references(() => firestormCampaignsTable.id, {
    onDelete: 'set null',
  }),
  ...
  status: text('status', { enum: ['new', 'contacted', 'engaged', 'qualified', 'disqualified'] })
    .notNull()
    .default('new'),
  metadata: jsonb('metadata'),
  createdAt: timestamp('created_at').notNull().defaultNow(),
...
export const firestormAnalyticsTable = pgTable('firestorm_analytics', {
  ...
  campaignId: integer('campaign_id')
    .notNull()
    .references(() => firestormCampaignsTable.id, { onDelete: 'cascade' }),
...
export const firestormAssetsTable = pgTable('firestorm_assets', {
  ...
  environment: text('environment', {
    enum: ['production', 'staging', 'development', 'dmz', 'internal'],
  })
    .notNull()
    .default('production'),
  exposureLevel: text('exposure_level', { enum: ['public', 'internal', 'restricted', 'critical'] })
    .notNull()
    .default('internal'),
...
export const firestormWorkflowActionsTable = pgTable('firestorm_workflow_actions', {
  ...
  entityType: text('entity_type', {
    enum: ['finding', 'incident', 'asset', 'simulation'],
  }).notNull(),
...
export const insertFirestormAssetSchema = createInsertSchema(firestormAssetsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertFirestormWorkflowActionSchema = createInsertSchema(
...
export const insertFirestormScenarioSchema = createInsertSchema(firestormScenariosTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertFirestormAssessmentSchema = createInsertSchema(firestormAssessmentsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertFirestormSimulationRunSchema = createInsertSchema(
...
export const insertFirestormFindingSchema = createInsertSchema(firestormFindingsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertFirestormRiskScoreSchema = createInsertSchema(firestormRiskScoresTable).omit({
  id: true,
});
...
export const insertFirestormIncidentSchema = createInsertSchema(firestormIncidentsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertFirestormComplianceControlSchema = createInsertSchema(
  firestormComplianceControlsTable,
).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertFirestormComplianceControl = z.infer<
  typeof insertFirestormComplianceControlSchema
>;
export type FirestormComplianceControl = typeof firestormComplianceControlsTable.$inferSelect;

export const insertFirestormAlertSchema = createInsertSchema(firestormAlertsTable).omit({
  id: true,
  createdAt: true,
});
...
export const firestormCasesTable = pgTable('firestorm_cases', {
  ...
  title: text('title').notNull(),
  description: text('description'),
  status: text('status', { enum: ['open', 'in_progress', 'pending_review', 'resolved', 'closed'] })
    .notNull()
    .default('open'),
  priority: text('priority', { enum: ['p1_critical', 'p2_high', 'p3_medium', 'p4_low'] })
    .notNull()
    .default('p3_medium'),
  assignedAnalyst: text('assigned_analyst'),
  ...
  evidence: jsonb('evidence')
    .$type<Array<{ name: string; type: string; url?: string; addedAt: string }>>()
    .default([]),
  auditTrail: jsonb('audit_trail')
    .$type<Array<{ action: string; user: string; at: string }>>()
    .default([]),
...
export const insertFirestormCaseSchema = createInsertSchema(firestormCasesTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const firestormMitreDetectionsTable = pgTable('firestorm_mitre_detections', {
  ...
  coverageStatus: text('coverage_status', { enum: ['detected', 'partial', 'not_covered'] })
    .notNull()
    .default('not_covered'),
  relatedIncidentIds: jsonb('related_incident_ids').$type<number[]>().default([]),
  relatedFindingIds: jsonb('related_finding_ids').$type<number[]>().default([]),
...
export const insertFirestormMitreDetectionSchema = createInsertSchema(
...
export const firestormTradecraftDecisionsTable = pgTable('firestorm_tradecraft_decisions', {
  ...
  evidenceQuality: text('evidence_quality', { enum: ['high', 'medium', 'low', 'insufficient'] })
    .notNull()
    .default('low'),
  ...
  confidenceLabel: text('confidence_label', { enum: ['high', 'moderate', 'low', 'insufficient'] })
    .notNull()
    .default('low'),
  confidenceStatement: text('confidence_statement'),
  gapsAndUnknowns: jsonb('gaps_and_unknowns').$type<string[]>().default([]),
  impactLevel: text('impact_level', { enum: ['critical', 'high', 'medium', 'low', 'negligible'] })
    .notNull()
    .default('medium'),
  urgency: text('urgency', { enum: ['immediate', 'urgent', 'standard', 'deferred'] })
    .notNull()
    .default('standard'),
...
export const insertFirestormTradecraftDecisionSchema = createInsertSchema(
  firestormTradecraftDecisionsTable,
).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertFirestormTradecraftDecision = z.infer<
  typeof insertFirestormTradecraftDecisionSchema
>;
...
export const insertFirestormCaseMemorySchema = createInsertSchema(firestormCaseMemoryTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertFirestormAnalystNotebookSchema = createInsertSchema(
...
export const firestormTradecraftValidationAuditTable = pgTable(
  'firestorm_tradecraft_validation_audit',
  {
    id: serial('id').primaryKey(),
...
export const insertFirestormValidationAuditSchema = createInsertSchema(
...
export const insertFirestormToolAuditLogSchema = createInsertSchema(
