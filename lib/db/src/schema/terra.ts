...
export const terraAgentsTable = pgTable(
  'terra_agents',
  {
    id: serial('id').primaryKey(),
    brokerageId: integer('brokerage_id').references(() => terraBrokeragesTable.id, {
      onDelete: 'set null',
    }),
    ...
    licenseNumber: text('license_number'),
    specialty: text('specialty', {
      enum: ['office', 'retail', 'industrial', 'multifamily', 'mixed-use', 'land', 'residential'],
    })
      .notNull()
      .default('office'),
    status: text('status', { enum: ['active', 'inactive', 'on_leave'] })
      .notNull()
      .default('active'),
    activeListings: integer('active_listings').notNull().default(0),
...
export const terraPropertiesTable = pgTable(
  'terra_properties',
  {
    id: serial('id').primaryKey(),
    ...
    ownerType: text('owner_type', {
      enum: ['individual', 'llc', 'trust', 'corporate', 'reit', 'unknown'],
    })
      .notNull()
      .default('unknown'),
...
export const terraListingsTable = pgTable(
  'terra_listings',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
    propertyId: integer('property_id').references(() => terraPropertiesTable.id, {
      onDelete: 'cascade',
    }),
    ...
    status: text('status', {
      enum: ['active', 'pending', 'under_contract', 'closed', 'expired', 'withdrawn'],
    })
      .notNull()
      .default('active'),
    ...
    priceHistory: jsonb('price_history')
      .$type<Array<{ date: string; price: number; changeType: string }>>()
      .notNull()
      .default([]),
    isDemo: boolean('is_demo').notNull().default(false),
    createdAt: timestamp('created_at').notNull().defaultNow(),
...
export const terraInquiriesTable = pgTable(
  'terra_inquiries',
  {
    id: serial('id').primaryKey(),
    listingId: integer('listing_id').references(() => terraListingsTable.id, {
      onDelete: 'set null',
    }),
    assignedAgentId: integer('assigned_agent_id').references(() => terraAgentsTable.id, {
      onDelete: 'set null',
    }),
    ...
    buyerType: text('buyer_type', {
      enum: ['investor', 'owner_occupant', 'developer', 'family_office', 'reit', 'unknown'],
    })
      .notNull()
      .default('unknown'),
    financingStatus: text('financing_status', {
      enum: ['cash', 'pre_approved', 'seeking_financing', 'unknown'],
    })
      .notNull()
      .default('unknown'),
    ...
    status: text('status', {
      ...
      .default('new'),
    source: text('source', {
      enum: ['web', 'email', 'phone', 'referral', 'portal', 'direct', 'other'],
    })
      .notNull()
      .default('other'),
...
export const terraTransactionsTable = pgTable(
  'terra_transactions',
  {
    id: serial('id').primaryKey(),
    listingId: integer('listing_id').references(() => terraListingsTable.id, {
    ...
    financingType: text('financing_type', {
      enum: ['cash', 'conventional', 'bridge', 'cmbs', 'life_co', 'agency', 'other'],
    })
      .notNull()
      .default('conventional'),
    status: text('status', { enum: ['completed', 'fallen_through', 'pending_recording'] })
      .notNull()
      .default('completed'),
    notes: text('notes'),
    isDemo: boolean('is_demo').notNull().default(false),
...
export const terraDistressPropertiesTable = pgTable(
  'terra_distress_properties',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
    address: text('address').notNull(),
    ...
    distressType: text('distress_type', {
      enum: ['pre-foreclosure', 'foreclosure', 'auction', 'reo', 'tax-lien', 'expired-listing'],
    }).notNull(),
    ...
    timeline: jsonb('timeline')
      .$type<Array<{ date: string; type: string; description: string }>>()
      .notNull()
      .default([]),
    ...
    rawData: jsonb('raw_data'),
    ingestSource: text('ingest_source', { enum: ['seed', 'csv_upload', 'nyc_open_data', 'manual'] })
      .notNull()
      .default('seed'),
...
export const terraDistressAlertsTable = pgTable(
  'terra_distress_alerts',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
    propertyId: integer('property_id').references(() => terraDistressPropertiesTable.id, {
      onDelete: 'cascade',
    }),
    propertyExternalId: text('property_external_id'),
    alertType: text('alert_type', {
      enum: ['auction', 'foreclosure', 'lien', 'reo', 'signal', 'price_drop'],
    }).notNull(),
...
export const terraIngestionRunsTable = pgTable(
  'terra_ingestion_runs',
  {
    id: serial('id').primaryKey(),
    source: text('source').notNull(),
    status: text('status', { enum: ['running', 'completed', 'failed', 'partial'] })
      .notNull()
      .default('running'),
...
export const terraLeadsTable = pgTable(
  'terra_leads',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
    ...
    email: text('email'),
    phone: text('phone'),
    type: text('type', { enum: ['buyer', 'seller', 'investor', 'both'] })
      .notNull()
      .default('buyer'),
    source: text('source', {
      enum: [
      ...
      .notNull()
      .default('manual'),
    stage: text('stage', { enum: ['new', 'engaged', 'nurtured', 'hot', 'inactive', 'converted'] })
      .notNull()
      .default('new'),
    ...
    distressPropertyId: integer('distress_property_id').references(
      () => terraDistressPropertiesTable.id,
      { onDelete: 'set null' },
    ),
    ...
    tags: jsonb('tags').$type<string[]>().notNull().default([]),
    timeline: jsonb('timeline')
      .$type<Array<{ date: string; event: string; type: string }>>()
      .notNull()
      .default([]),
...
export const terraDealsTable = pgTable(
  'terra_deals',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
    ...
    stage: text('stage', {
      ...
      .notNull()
      .default('lead'),
    type: text('type', { enum: ['acquisition', 'disposition', 'assignment', 'wholesale'] })
      .notNull()
      .default('acquisition'),
    ...
    notes: text('notes'),
    timeline: jsonb('timeline')
      .$type<Array<{ date: string; event: string; type: string; userId?: number }>>()
      .notNull()
      .default([]),
    documents: jsonb('documents')
      .$type<Array<{ name: string; status: string; url?: string }>>()
      .notNull()
      .default([]),
    isActive: boolean('is_active').notNull().default(true),
    createdAt: timestamp('created_at').notNull().defaultNow(),
...
export const terraSavedOpportunitiesTable = pgTable(
  'terra_saved_opportunities',
  {
    id: serial('id').primaryKey(),
    userId: integer('user_id'),
    distressPropertyId: integer('distress_property_id')
      .notNull()
      .references(() => terraDistressPropertiesTable.id, { onDelete: 'cascade' }),
    note: text('note'),
...
export const insertTerraDistressPropertySchema = createInsertSchema(
...
export const insertTerraDistressAlertSchema = createInsertSchema(terraDistressAlertsTable).omit({
  id: true,
});
...
export const insertTerraIngestionRunSchema = createInsertSchema(terraIngestionRunsTable).omit({
  id: true,
  startedAt: true,
});
...
export const insertTerraBrokerageSchema = createInsertSchema(terraBrokeragesTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertTerraAgentSchema = createInsertSchema(terraAgentsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertTerraPropertySchema = createInsertSchema(terraPropertiesTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertTerraListingSchema = createInsertSchema(terraListingsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertTerraInquirySchema = createInsertSchema(terraInquiriesTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertTerraTransactionSchema = createInsertSchema(terraTransactionsTable).omit({
  id: true,
  createdAt: true,
});
...
export const insertTerraLeadSchema = createInsertSchema(terraLeadsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertTerraDealSchema = createInsertSchema(terraDealsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertTerraSavedOpportunitySchema = createInsertSchema(
...
export const terraCovenantsTable = pgTable(
  'terra_covenants',
  {
    id: serial('id').primaryKey(),
    /**
    ...
    covenantType: text('covenant_type', {
      enum: ['dscr', 'ltv', 'occupancy', 'debt_yield'],
    }).notNull(),
    label: text('label'),
    thresholdValue: numeric('threshold_value', { precision: 12, scale: 4 }).notNull(),
    comparator: text('comparator', { enum: ['gte', 'lte'] })
      .notNull()
      .default('gte'),
...
export const insertTerraCovenantSchema = createInsertSchema(terraCovenantsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const terraDiligenceMattersTable = pgTable(
  'terra_diligence_matters',
  {
    ...
    propertyExternalId: text('property_external_id'),
    borough: text('borough'),
    status: text('status', { enum: ['in_progress', 'on_hold', 'completed', 'withdrawn'] })
      .notNull()
      .default('in_progress'),
    stage: text('stage', {
      enum: [
...
export const terraDiligenceEvidenceTable = pgTable(
  'terra_diligence_evidence',
  {
    id: text('id').primaryKey(),
    matterId: text('matter_id')
      .notNull()
      .references(() => terraDiligenceMattersTable.id, { onDelete: 'cascade' }),
    ...
    status: text('status', { enum: ['pending', 'in_review', 'verified', 'rejected'] })
      .notNull()
      .default('pending'),
...
export const insertTerraDiligenceMatterSchema = createInsertSchema(terraDiligenceMattersTable).omit(
...
export const insertTerraDiligenceEvidenceSchema = createInsertSchema(
...
export const terraMlsListingsTable = pgTable(
  'terra_mls_listings',
  {
    id: serial('id').primaryKey(),
    ...
    media: jsonb('media')
      .$type<Array<{ mediaUrl: string; mediaType: string; order: number }>>()
      .notNull()
      .default([]),
    ...
    rawData: jsonb('raw_data'),
    isActive: boolean('is_active').notNull().default(true),
    ingestSource: text('ingest_source', { enum: ['mls_sync', 'manual', 'demo'] })
      .notNull()
      .default('demo'),
...
export const terraCommercialPropertiesTable = pgTable(
  'terra_commercial_properties',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
...
export const terraCommercialCompsTable = pgTable(
  'terra_commercial_comps',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
    source: text('source', { enum: ['costar', 'compstak', 'manual', 'demo'] }).notNull(),
...
export const insertTerraMlsListingSchema = createInsertSchema(terraMlsListingsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertTerraCommercialPropertySchema = createInsertSchema(
...
export const insertTerraCommercialCompSchema = createInsertSchema(terraCommercialCompsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const terraActionItemsTable = pgTable(
  'terra_action_items',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
...
export const insertTerraActionItemSchema = createInsertSchema(terraActionItemsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const terraLeasesTable = pgTable(
  'terra_leases',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
    ...
    flags: jsonb('flags')
      .$type<Array<{ field: string; issue: string; severity: string }>>()
      .notNull()
      .default([]),
    rawData: jsonb('raw_data'),
...
export const insertTerraLeaseSchema = createInsertSchema(terraLeasesTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const terraProFormaProjectsTable = pgTable(
  'terra_pro_forma_projects',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
...
export const insertTerraProFormaProjectSchema = createInsertSchema(terraProFormaProjectsTable).omit(
...
export const terraExchanges1031Table = pgTable(
  'terra_exchanges_1031',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
    ...
    status: text('status', { enum: ['identification', 'exchange', 'completed', 'failed'] })
      .notNull()
    ...
    identifiedProperties: jsonb('identified_properties')
      ...
      .notNull()
      .default([]),
    complianceItems: jsonb('compliance_items')
      .$type<Array<Record<string, unknown>>>()
      .notNull()
      .default([]),
...
export const insertTerraExchange1031Schema = createInsertSchema(terraExchanges1031Table).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const terraTaxAppealsTable = pgTable(
  'terra_tax_appeals',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
    ...
    appealStatus: text('appeal_status', {
      enum: ['eligible', 'filed', 'hearing', 'won', 'lost', 'not-eligible'],
    })
      .notNull()
      .default('eligible'),
    juris: text('juris'),
    comparables: jsonb('comparables').$type<Array<Record<string, unknown>>>().notNull().default([]),
    appealStrength: text('appeal_strength', { enum: ['strong', 'moderate', 'weak'] })
      .notNull()
      .default('moderate'),
    notes: text('notes'),
    ownerUserId: integer('owner_user_id'),
...
export const insertTerraTaxAppealSchema = createInsertSchema(terraTaxAppealsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const terraWaterfallStructuresTable = pgTable(
  'terra_waterfall_structures',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
    name: text('name').notNull(),
...
export const insertTerraWaterfallStructureSchema = createInsertSchema(
...
export const terraConstructionProjectsTable = pgTable(
  'terra_construction_projects',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
    name: text('name').notNull(),
    ...
    status: text('status', { enum: ['on-track', 'behind', 'at-risk', 'complete'] })
      .notNull()
      .default('on-track'),
    gc: text('gc'),
...
export const insertTerraConstructionProjectSchema = createInsertSchema(
...
export const terraTenantApplicationsTable = pgTable(
  'terra_tenant_applications',
  {
    id: serial('id').primaryKey(),
    externalId: text('external_id').unique(),
    name: text('name').notNull(),
    type: text('type', { enum: ['individual', 'entity'] })
      .notNull()
      .default('individual'),
    ...
    status: text('status', { enum: ['pending', 'approved', 'conditional', 'declined'] })
      .notNull()
      .default('pending'),
    overallScore: integer('overall_score').notNull().default(50),
    recommendation: text('recommendation', { enum: ['approve', 'conditional', 'decline'] })
      .notNull()
      .default('conditional'),
...
export const insertTerraTenantApplicationSchema = createInsertSchema(
