import { pgTable, text, serial, timestamp, integer, numeric, jsonb, boolean, index, real } from "drizzle-orm/pg-core";
import { pcMattersTable } from "./prism_counsel";

export const pcModelLanesTable = pgTable("pc_model_lanes", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  lane: text("lane", { enum: ["embedding", "retrieval", "classification", "extraction", "reasoning", "forecast", "policy_guardrail"] }).notNull(),
  provider: text("provider").notNull(),
  modelName: text("model_name").notNull(),
  modelVersion: text("model_version"),
  endpoint: text("endpoint"),
  status: text("status", { enum: ["active", "degraded", "disabled", "failover"] }).notNull().default("active"),
...
export const pcModelRequestsTable = pgTable("pc_model_requests", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
...
export const pcHfEndpointsTable = pgTable("pc_hf_endpoints", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  name: text("name").notNull(),
  task: text("task", { enum: ["text_embedding", "classification", "reranking", "summarization", "ner", "contradiction_detection", "sentence_similarity", "zero_shot"] }).notNull(),
...
export const pcWorldlineSourcesTable = pgTable("pc_worldline_sources", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  sourceClass: text("source_class", { enum: ["regulatory_insurance", "crash_incident", "weather_environmental", "county_demographic", "court_venue", "lien_recovery", "internal_firm"] }).notNull(),
  name: text("name").notNull(),
  description: text("description"),
  endpoint: text("endpoint"),
  fetchMethod: text("fetch_method", { enum: ["api_pull", "rss", "scrape", "webhook", "internal_query", "file_import"] }).notNull(),
...
export const pcWorldlineSignalsTable = pgTable("pc_worldline_signals", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
...
export const pcWorldlineFeaturesTable = pgTable("pc_worldline_features", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
...
export const pcPressureScoresTable = pgTable("pc_pressure_scores", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  ...
  movement: text("movement", { enum: ["rising", "falling", "stable", "new"] }).notNull().default("new"),
  confidence: real("confidence"),
...
export const pcProofChainEntriesTable = pgTable("pc_proof_chain_entries", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").references(() => pcMattersTable.id),
  outputType: text("output_type", { enum: [
    "summary", "chronology", "recommendation", "forecast", "memo",
    "extraction", "classification", "alert", "export_packet", "copilot_answer"
  ] }).notNull(),
  ...
  actorType: text("actor_type", { enum: ["system", "user", "service"] }).notNull().default("system"),
  actorId: integer("actor_id"),
  reviewState: text("review_state", { enum: ["unreviewed", "pending_review", "reviewed", "approved", "rejected"] }).notNull().default("unreviewed"),
  reviewedBy: integer("reviewed_by"),
  reviewedAt: timestamp("reviewed_at"),
  approvalState: text("approval_state", { enum: ["not_required", "pending", "approved", "denied"] }).notNull().default("not_required"),
  approvedBy: integer("approved_by"),
  approvedAt: timestamp("approved_at"),
  privilegeState: text("privilege_state", { enum: ["none", "privileged", "work_product", "attorney_client"] }).notNull().default("none"),
...
export const pcCopilotSessionsTable = pgTable("pc_copilot_sessions", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  userId: integer("user_id").notNull(),
  matterId: integer("matter_id").references(() => pcMattersTable.id),
  mode: text("mode", { enum: ["matter", "communications", "document", "strategy", "ops"] }).notNull(),
  title: text("title"),
...
export const pcCopilotMessagesTable = pgTable("pc_copilot_messages", {
  id: serial("id").primaryKey(),
  sessionId: integer("session_id").notNull().references(() => pcCopilotSessionsTable.id, { onDelete: "cascade" }),
  ...
  index("pc_copilot_msgs_session_idx").on(table.sessionId),
]);

export const pcMatterTwinSnapshotsTable = pgTable("pc_matter_twin_snapshots", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  snapshotType: text("snapshot_type", { enum: ["daily", "weekly", "on_change", "manual"] }).notNull(),
...
export const pcForecastDiffsTable = pgTable("pc_forecast_diffs", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
...
export const pcDataProductScoresTable = pgTable("pc_data_product_scores", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").references(() => pcMattersTable.id),
  product: text("product", { enum: [
    "insurer_pressure_index", "venue_velocity_index", "incident_context_layer",
    "nofault_friction_score", "settlement_friction_map", "ai_defensibility_index"
  ] }).notNull(),
  score: real("score").notNull(),
  priorScore: real("prior_score"),
  movement: text("movement", { enum: ["rising", "falling", "stable", "new"] }).notNull().default("new"),
...
export const pcM365SubscriptionsTable = pgTable("pc_m365_subscriptions", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  connectorAccountId: integer("connector_account_id"),
  resourceType: text("resource_type", { enum: ["mail", "calendar", "driveItem", "chatMessage", "channel"] }).notNull(),
  ...
  status: text("status", { enum: ["active", "expiring", "expired", "error", "suspended"] }).notNull().default("active"),
...
export const pcM365DeltaCursorsTable = pgTable("pc_m365_delta_cursors", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  connectorAccountId: integer("connector_account_id"),
  ...
  syncStatus: text("sync_status", { enum: ["synced", "syncing", "error", "stale"] }).notNull().default("synced"),
...
export const pcCostTrackingTable = pgTable("pc_cost_tracking", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id"),
  workflow: text("workflow").notNull(),
  costCategory: text("cost_category", { enum: [
    "graph_sync", "extraction", "doc_intelligence", "search", "embedding",
    "hf_endpoint", "reasoning", "forecast_compute", "storage", "export_generation"
  ] }).notNull(),
