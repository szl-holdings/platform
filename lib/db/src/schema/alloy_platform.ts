...
export const alloyWorkflowsTable = pgTable("platform_workflows", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
  description: text("description"),
  trigger: text("trigger", { enum: ["manual", "signal", "schedule", "webhook", "api"] }).notNull().default("manual"),
  triggerConfig: jsonb("trigger_config"),
  steps: jsonb("steps"),
  outputType: text("output_type", { enum: ["artifact", "notification", "action", "report", "none"] }).notNull().default("artifact"),
...
export const alloySignalsTable = pgTable("platform_signals", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
  workflowId: integer("workflow_id").references(() => alloyWorkflowsTable.id, { onDelete: "set null" }),
...
export const alloyWorkflowRunsTable = pgTable("platform_workflow_runs", {
  id: serial("id").primaryKey(),
  workflowId: integer("workflow_id").notNull().references(() => alloyWorkflowsTable.id, { onDelete: "cascade" }),
  ...
  state: text("state", { enum: ["queued", "running", "waiting_approval", "completed", "failed", "canceled"] }).notNull().default("queued"),
...
export const alloyArtifactsTable = pgTable("platform_artifacts", {
  id: serial("id").primaryKey(),
  workflowRunId: integer("workflow_run_id").references(() => alloyWorkflowRunsTable.id, { onDelete: "cascade" }),
  workflowId: integer("workflow_id").references(() => alloyWorkflowsTable.id, { onDelete: "set null" }),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
  title: text("title").notNull(),
  artifactType: text("artifact_type", { enum: ["report", "recommendation", "alert", "summary", "document", "data_export"] }).notNull(),
...
export const alloyApprovalsTable = pgTable("platform_approvals", {
  id: serial("id").primaryKey(),
  workflowRunId: integer("workflow_run_id").notNull().references(() => alloyWorkflowRunsTable.id, { onDelete: "cascade" }),
  artifactId: integer("artifact_id").references(() => alloyArtifactsTable.id, { onDelete: "set null" }),
...
export const alloyAuditLogTable = pgTable("platform_audit_log", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
...
export const insertAlloySignalSchema = createInsertSchema(alloySignalsTable).omit({ id: true, createdAt: true, receivedAt: true });
...
export const insertAlloyWorkflowRunSchema = createInsertSchema(alloyWorkflowRunsTable).omit({ id: true, createdAt: true, queuedAt: true });
...
export const insertAlloyArtifactSchema = createInsertSchema(alloyArtifactsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertAlloyApprovalSchema = createInsertSchema(alloyApprovalsTable).omit({ id: true, createdAt: true });
