...
export const documentsTable = pgTable("documents", {
  ...
  type: text("type", {
    enum: [
      "deal_memo", "offer_letter", "property_report", "investor_packet",
      "incident_report", "vulnerability_assessment", "compliance_evidence", "penetration_test_summary",
      "engagement_letter", "nda", "service_agreement", "confidential_memo",
      "voyage_report", "charter_party", "port_state_inspection", "cargo_manifest",
      "workflow_approval_memo", "integration_spec", "change_request",
      "investor_one_pager", "platform_overview", "general"
    ]
...
export const documentCommentsTable = pgTable("document_comments", {
  ...
  index("doc_comments_doc_idx").on(t.documentId),
]);
...
export const documentTemplatesTable = pgTable("document_templates", {
  ...
  contentJson: jsonb("content_json").notNull().default({}),
  mergeFields: jsonb("merge_fields").$type<Array<{ key: string; label: string; description?: string; required?: boolean }>>().notNull().default([]),
...
export const contentLibraryBlocksTable = pgTable("content_library_blocks", {
  id: serial("id").primaryKey(),
  title: text("title").notNull(),
  category: text("category", {
    enum: ["legal_clause", "standard_terms", "company_boilerplate", "intro", "signature_block", "disclaimer", "custom"]
...
export const pdfJobsTable = pgTable("pdf_jobs", {
  id: serial("id").primaryKey(),
...
export const pdfBatchesTable = pgTable("pdf_batches", {
  id: serial("id").primaryKey(),
...
export const insertPdfJobSchema = createInsertSchema(pdfJobsTable).omit({ id: true, createdAt: true });
