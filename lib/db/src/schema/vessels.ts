...
export const vesselsAlertRulesTable = pgTable('vessels_alert_rules', {
  ...
  ruleType: text('rule_type', {
    enum: ['speed', 'geofence', 'weather', 'schedule', 'cargo', 'maintenance'],
  }).notNull(),
...
export const vesselsWeatherSnapshotsTable = pgTable('vessels_weather_snapshots', {
  ...
  description: text('description'),
  riskLevel: text('risk_level', { enum: ['low', 'moderate', 'high', 'severe'] })
    .notNull()
    .default('low'),
  recordedAt: timestamp('recorded_at').notNull().defaultNow(),
});
...
export const vesselsSimulationsTable = pgTable('vessels_simulations', {
  ...
  simulationType: text('simulation_type', {
    enum: ['route_risk', 'weather_impact', 'fuel_optimization', 'schedule_analysis'],
  }).notNull(),
...
export const insertVesselFleetSchema = createInsertSchema(vesselsFleetsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertVesselSchema = createInsertSchema(vesselsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertVesselPositionSchema = createInsertSchema(vesselsPositionsTable).omit({
  id: true,
});
...
export const insertVesselCargoSchema = createInsertSchema(vesselsCargoTable).omit({
  id: true,
  createdAt: true,
});
...
export const insertVesselRouteSchema = createInsertSchema(vesselsRoutesTable).omit({
  id: true,
  createdAt: true,
});
...
export const insertVesselAlertRuleSchema = createInsertSchema(vesselsAlertRulesTable).omit({
  id: true,
  createdAt: true,
});
...
export const insertVesselWeatherSnapshotSchema = createInsertSchema(
...
export const insertVesselSimulationSchema = createInsertSchema(vesselsSimulationsTable).omit({
  id: true,
  createdAt: true,
});
...
export const insertVesselsExceptionEventSchema = createInsertSchema(vesselsEventsTable).omit({
  id: true,
  createdAt: true,
});
...
export const insertVesselCommandWorkflowSchema = createInsertSchema(
