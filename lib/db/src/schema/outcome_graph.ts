...
export const outcomeGraphTable = pgTable("outcome_graph", {
...
export const outcomeGraphLearningJobsTable = pgTable("outcome_graph_learning_jobs", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
  domain: text("domain", { enum: OUTCOME_DOMAINS }).notNull().default("general"),
  jobType: text("job_type", {
    enum: ["ranking_calibration", "confidence_calibration", "escalation_threshold", "workflow_template", "owner_suggestion", "artifact_defaults"],
  }).notNull(),
  status: text("status", {
...
export const insertOutcomeGraphLearningJobSchema = createInsertSchema(outcomeGraphLearningJobsTable).omit({
  id: true,
  createdAt: true,
});
