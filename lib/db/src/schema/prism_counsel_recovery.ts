...
export const pcRecoveryItemsTable = pgTable(
  'pc_recovery_items',
  {
    ...
    amountStatus: text('amount_status', { enum: ['confirmed', 'inferred', 'pending', 'unknown'] })
      .notNull()
      .default('unknown'),
...
export const pcRecoveryPartiesTable = pgTable('pc_recovery_parties', {
  id: serial('id').primaryKey(),
  recoveryItemId: integer('recovery_item_id')
    .notNull()
    .references(() => pcRecoveryItemsTable.id, { onDelete: 'cascade' }),
  orgId: integer('org_id').notNull(),
  role: text('role', {
    enum: [
      'lien_holder',
...
export const pcRecoveryDocumentsTable = pgTable('pc_recovery_documents', {
  ...
  reviewState: text('review_state', { enum: ['unreviewed', 'reviewed', 'flagged'] }).default(
    'unreviewed',
  ),
...
export const pcRecoveryStatusHistoryTable = pgTable(
  'pc_recovery_status_history',
  {
    id: serial('id').primaryKey(),
    recoveryItemId: integer('recovery_item_id')
      .notNull()
  ...
  (table) => [index('pc_recovery_hist_item_idx').on(table.recoveryItemId)],
);
...
export const pcRecoveryAmountMarkersTable = pgTable('pc_recovery_amount_markers', {
  ...
  amountStatus: text('amount_status', { enum: ['confirmed', 'inferred', 'pending'] })
    .notNull()
    .default('pending'),
...
export const pcRecoveryFollowupsTable = pgTable(
  'pc_recovery_followups',
  {
    id: serial('id').primaryKey(),
    recoveryItemId: integer('recovery_item_id')
      .notNull()
    ...
    followupType: text('followup_type', {
      enum: [
        ...
        'request_updated_amount',
        'escalate_delay',
        'attach_support',
        'add_notes',
        'link_to_matter_event',
        'link_to_settlement_friction',
        'route_for_attorney_review',
        'route_for_managed_review',
        'mark_dependency_reviewed',
      ],
    }).notNull(),
    title: text('title').notNull(),
...
export const pcRecoveryDependencyLinksTable = pgTable(
  'pc_recovery_dependency_links',
  {
    id: serial('id').primaryKey(),
    recoveryItemId: integer('recovery_item_id')
      .notNull()
    ...
    dependencyType: text('dependency_type', {
      enum: [
        ...
        'increases_friction',
        'affects_readiness',
        'tied_to_matter_event',
        'tied_to_blocker',
      ],
    }).notNull(),
    linkedEntityType: text('linked_entity_type', {
      enum: ['matter', 'settlement_blocker', 'deadline', 'offer', 'approval'],
    }).notNull(),
  ...
  (table) => [index('pc_recovery_dep_item_idx').on(table.recoveryItemId)],
);

export const pcRecoveryRiskSnapshotsTable = pgTable(
  'pc_recovery_risk_snapshots',
  {
    id: serial('id').primaryKey(),
    matterId: integer('matter_id')
      .notNull()
    ...
    overallRisk: text('overall_risk', { enum: ['low', 'medium', 'high', 'critical'] })
      .notNull()
      .default('medium'),
  ...
  (table) => [index('pc_recovery_risk_matter_idx').on(table.matterId)],
);

export const pcSettlementBlockersTable = pgTable(
  'pc_settlement_blockers',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').notNull(),
    matterId: integer('matter_id')
    ...
    status: text('status', { enum: ['open', 'in_progress', 'resolved', 'dismissed'] })
      .notNull()
      .default('open'),
...
export const pcSettlementBlockerDriversTable = pgTable('pc_settlement_blocker_drivers', {
  ...
  blockerId: integer('blocker_id')
    .notNull()
    .references(() => pcSettlementBlockersTable.id, { onDelete: 'cascade' }),
  orgId: integer('org_id').notNull(),
...
export const pcSettlementBlockerActionsTable = pgTable(
  'pc_settlement_blocker_actions',
  {
    id: serial('id').primaryKey(),
    blockerId: integer('blocker_id')
      .notNull()
  ...
  (table) => [index('pc_blocker_actions_blocker_idx').on(table.blockerId)],
);

export const pcSettlementBlockerSnapshotsTable = pgTable(
  'pc_settlement_blocker_snapshots',
  {
    id: serial('id').primaryKey(),
    matterId: integer('matter_id')
      .notNull()
    ...
    settlementReadiness: text('settlement_readiness', { enum: ['blocked', 'partial', 'clear'] })
      .notNull()
      .default('partial'),
    exportReadiness: text('export_readiness', { enum: ['blocked', 'partial', 'clear'] })
      .notNull()
      .default('partial'),
    signals: jsonb('signals'),
    snapshotAt: timestamp('snapshot_at').notNull().defaultNow(),
  ...
  (table) => [index('pc_blocker_snap_matter_idx').on(table.matterId)],
);
