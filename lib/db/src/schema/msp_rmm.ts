...
export const mspRmmConnectorsTable = pgTable('msp_rmm_connectors', {
  ...
  mode: text('mode', { enum: ['rmm', 'psa', 'both'] })
    .notNull()
    .default('both'),
  status: text('status', { enum: ['active', 'inactive', 'error', 'pending'] })
    .notNull()
    .default('pending'),
  authType: text('auth_type', { enum: ['api_key', 'oauth2', 'basic'] })
    .notNull()
    .default('api_key'),
...
export const mspRmmDeviceMetricsTable = pgTable(
  'msp_rmm_device_metrics',
  {
    id: serial('id').primaryKey(),
    deviceId: integer('device_id').references(() => mspDevicesTable.id, { onDelete: 'cascade' }),
    deviceDbId: text('device_db_id'),
    connectorId: integer('connector_id').references(() => mspRmmConnectorsTable.id, {
      onDelete: 'set null',
    }),
    ...
    services: jsonb('services')
      .$type<Array<{ name: string; status: string; pid?: number }>>()
      .default([]),
    processes: jsonb('processes')
      .$type<Array<{ name: string; pid: number; cpu: number; memory: number }>>()
      .default([]),
...
export const mspHealingPlaybooksTable = pgTable('msp_healing_playbooks', {
  ...
  status: text('status', { enum: ['active', 'inactive', 'draft'] })
    .notNull()
    .default('active'),
  executionMode: text('execution_mode', { enum: ['full_auto', 'human_gated', 'notify_only'] })
    .notNull()
    .default('human_gated'),
...
export const mspHealingExecutionsTable = pgTable(
  'msp_healing_executions',
  {
    id: serial('id').primaryKey(),
    playbookId: integer('playbook_id').references(() => mspHealingPlaybooksTable.id, {
      onDelete: 'set null',
    }),
    ...
    status: text('status', {
      enum: ['pending_approval', 'approved', 'running', 'completed', 'failed', 'rejected'],
    })
      .notNull()
    ...
    beforeMetrics: jsonb('before_metrics').$type<{
      cpu: number;
      memory: number;
      disk: number;
      services?: Array<{ name: string; status: string }>;
    }>(),
    afterMetrics: jsonb('after_metrics').$type<{
      cpu: number;
      memory: number;
      disk: number;
      services?: Array<{ name: string; status: string }>;
    ...
    actionsExecuted: jsonb('actions_executed')
      .$type<Array<{ action: string; result: string; at: string }>>()
      .default([]),
...
export const mspRemoteActionsTable = pgTable(
  'msp_remote_actions',
  {
    id: serial('id').primaryKey(),
    deviceId: integer('device_id').references(() => mspDevicesTable.id, { onDelete: 'set null' }),
    ...
    status: text('status', {
      enum: ['pending_approval', 'approved', 'executing', 'completed', 'failed', 'cancelled'],
    })
      .notNull()
      .default('pending_approval'),
...
export const mspPsaTicketSyncTable = pgTable('msp_psa_ticket_sync', {
  id: serial('id').primaryKey(),
  internalTicketId: integer('internal_ticket_id').references(() => mspTicketsTable.id, {
    onDelete: 'cascade',
  }),
  connectorId: integer('connector_id').references(() => mspRmmConnectorsTable.id, {
    onDelete: 'set null',
  ...
  psaUrl: text('psa_url'),
  syncStatus: text('sync_status', { enum: ['synced', 'pending', 'error', 'closed'] })
    .notNull()
    .default('pending'),
  lastSyncAt: timestamp('last_sync_at'),
...
export const mspOrgSiteMappingsTable = pgTable(
  'msp_org_site_mappings',
  {
    id: serial('id').primaryKey(),
    connectorId: integer('connector_id')
      .references(() => mspRmmConnectorsTable.id, { onDelete: 'cascade' })
      .notNull(),
    ...
    internalClientId: integer('internal_client_id').references(() => mspClientsTable.id, {
      onDelete: 'set null',
    }),
