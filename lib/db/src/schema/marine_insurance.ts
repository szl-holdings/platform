import { pgTable, serial, integer, text, numeric, timestamp, pgEnum, jsonb, boolean } from "drizzle-orm/pg-core";
import { createInsertSchema } from "drizzle-zod";

export const insuranceCoverageTypeEnum = pgEnum("insurance_coverage_type", [
  "marine_cargo", "hull_machinery", "protection_indemnity", "freight_demurrage", "war_risk", "pollution_liability"
]);

export const insuranceRiskRatingEnum = pgEnum("insurance_risk_rating", ["low", "moderate", "high", "very_high", "uninsurable"]);
export const insurancePolicyStatusEnum = pgEnum("insurance_policy_status", ["quote", "bound", "active", "expired", "cancelled", "claim_in_progress"]);
export const insuranceClaimStatusEnum = pgEnum("insurance_claim_status", ["filed", "under_review", "investigation", "negotiation", "settled", "rejected", "closed"]);
...
export const insertMarineInsuranceQuoteSchema = createInsertSchema(marineInsuranceQuotesTable).omit({ id: true, createdAt: true, updatedAt: true });
export const insertMarineInsurancePolicySchema = createInsertSchema(marineInsurancePoliciesTable).omit({ id: true, createdAt: true, updatedAt: true });
export const insertMarineInsuranceClaimSchema = createInsertSchema(marineInsuranceClaimsTable).omit({ id: true, createdAt: true, updatedAt: true });
