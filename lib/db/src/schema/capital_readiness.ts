...
export const capitalArtifactsTable = pgTable("capital_artifacts", {
  id: serial("id").primaryKey(),
  slug: text("slug").notNull().unique(),
  name: text("name").notNull(),
  artifactType: text("artifact_type", {
    enum: ["lender_packet", "investor_packet", "financial_model", "diligence_checklist", "data_room", "cap_table", "other"],
...
export const lenderPacketsTable = pgTable("lender_packets", {
  ...
  lenderType: text("lender_type", { enum: ["bank", "sba", "cdfi", "credit_union", "other"] }).notNull().default("bank"),
  targetAmount: text("target_amount"),
  useOfFunds: text("use_of_funds"),
  status: text("status", { enum: ["drafting", "ready_for_review", "submitted", "in_diligence", "approved", "declined", "archived"] }).notNull().default("drafting"),
...
export const lenderPacketDeliverables = pgTable("lender_packet_deliverables", {
  id: serial("id").primaryKey(),
  packetId: integer("packet_id").notNull().references(() => lenderPacketsTable.id, { onDelete: "cascade" }),
  deliverableKey: text("deliverable_key").notNull(),
  title: text("title").notNull(),
  description: text("description"),
  status: text("status", { enum: ["not_started", "drafting", "draft_complete", "reviewed", "final"] }).notNull().default("not_started"),
  version: integer("version").notNull().default(1),
  content: text("content"),
  artifactId: integer("artifact_id").references(() => capitalArtifactsTable.id, { onDelete: "set null" }),
  sortOrder: integer("sort_order").notNull().default(0),
  completedAt: timestamp("completed_at"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
}, (t) => [
  index("lender_deliverables_packet_idx").on(t.packetId),
]);
...
export const investorPacketsTable = pgTable("investor_packets", {
  id: serial("id").primaryKey(),
  title: text("title").notNull(),
  investorType: text("investor_type", { enum: ["angel", "pre_seed", "seed", "series_a", "strategic", "other"] }).notNull().default("angel"),
  targetAmount: text("target_amount"),
  raiseStructure: text("raise_structure"),
  status: text("status", { enum: ["drafting", "ready_for_review", "in_outreach", "in_diligence", "closed", "archived"] }).notNull().default("drafting"),
  completionPct: integer("completion_pct").notNull().default(0),
...
export const investorPacketDeliverables = pgTable("investor_packet_deliverables", {
  id: serial("id").primaryKey(),
  packetId: integer("packet_id").notNull().references(() => investorPacketsTable.id, { onDelete: "cascade" }),
  deliverableKey: text("deliverable_key").notNull(),
  title: text("title").notNull(),
  description: text("description"),
  status: text("status", { enum: ["not_started", "drafting", "draft_complete", "reviewed", "final"] }).notNull().default("not_started"),
  ...
  index("investor_deliverables_packet_idx").on(t.packetId),
]);
...
export const fundraisingMilestonesTable = pgTable("fundraising_milestones", {
  ...
  title: text("title").notNull(),
  description: text("description"),
  milestoneType: text("milestone_type", { enum: ["preparation", "outreach", "diligence", "close", "post_close"] }).notNull().default("preparation"),
  status: text("status", { enum: ["pending", "in_progress", "completed", "blocked", "skipped"] }).notNull().default("pending"),
...
export const financialModelsTable = pgTable("financial_models", {
  ...
  modelType: text("model_type", {
    enum: ["12_month_operating", "24_month_operating", "cash_flow", "debt_service", "revenue_assumptions", "use_of_funds", "pro_forma", "other"],
  }).notNull().default("12_month_operating"),
  status: text("status", { enum: ["draft", "in_review", "approved", "archived"] }).notNull().default("draft"),
  version: integer("version").notNull().default(1),
...
export const diligenceChecklistsTable = pgTable("diligence_checklists", {
  id: serial("id").primaryKey(),
  title: text("title").notNull(),
  checklistType: text("checklist_type", { enum: ["lender", "investor", "data_room", "legal", "financial"] }).notNull(),
  packetType: text("packet_type", { enum: ["lender", "investor", "general"] }).notNull().default("general"),
...
export const diligenceChecklistItemsTable = pgTable("diligence_checklist_items", {
  ...
  status: text("status", { enum: ["not_started", "in_progress", "complete", "waived", "na"] }).notNull().default("not_started"),
  artifactId: integer("artifact_id").references(() => capitalArtifactsTable.id, { onDelete: "set null" }),
  ...
  index("diligence_items_checklist_idx").on(t.checklistId),
]);
...
export const capTablePlaceholdersTable = pgTable("cap_table_placeholders", {
  ...
  holderType: text("holder_type", { enum: ["founder", "employee", "advisor", "investor", "option_pool", "other"] }).notNull().default("founder"),
...
export const insertCapitalArtifactSchema = createInsertSchema(capitalArtifactsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertLenderPacketSchema = createInsertSchema(lenderPacketsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertLenderPacketDeliverableSchema = createInsertSchema(lenderPacketDeliverables).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertInvestorPacketSchema = createInsertSchema(investorPacketsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertInvestorPacketDeliverableSchema = createInsertSchema(investorPacketDeliverables).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertFundraisingMilestoneSchema = createInsertSchema(fundraisingMilestonesTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertFinancialModelSchema = createInsertSchema(financialModelsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertUseOfFundsVersionSchema = createInsertSchema(useOfFundsVersionsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertCapTablePlaceholderSchema = createInsertSchema(capTablePlaceholdersTable).omit({ id: true, createdAt: true, updatedAt: true });
