...
export const pcMatterClocksTable = pgTable("pc_matter_clocks", {
  ...
  clockType: text("clock_type", { enum: ["no_fault_notice", "no_fault_verification", "no_fault_arbitration", "disclaimer_timeliness", "eum_demand", "sol_tolling", "discovery_clock", "imc_verification", "peer_review_window"] }).notNull(),
  startedAt: timestamp("started_at").notNull(),
  deadlineAt: timestamp("deadline_at").notNull(),
  status: text("status", { enum: ["running", "breached", "met", "tolled", "waived"] }).notNull().default("running"),
...
export const pcClockEventsTable = pgTable("pc_clock_events", {
  ...
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  clockId: integer("clock_id").notNull().references(() => pcMatterClocksTable.id, { onDelete: "cascade" }),
  eventType: text("event_type", { enum: ["start", "toll", "resume", "breach", "cure", "waive", "extend", "close"] }).notNull(),
...
export const pcNyRuleProfilesTable = pgTable("pc_ny_rule_profiles", {
  ...
  category: text("category", { enum: ["no_fault", "disclaimer", "discovery", "statute_of_limitations", "court_rules", "mediation", "arbitration"] }).notNull(),
...
export const pcNoFaultClaimsTable = pgTable("pc_no_fault_claims", {
  ...
  noticeStatus: text("notice_status", { enum: ["pending", "timely", "late", "disputed"] }).notNull().default("pending"),
  billStatus: text("bill_status", { enum: ["open", "partial", "paid", "denied", "arbitration"] }).notNull().default("open"),
  ...
  arbitrationStatus: text("arbitration_status", { enum: ["not_filed", "pending", "scheduled", "heard", "awarded", "appealed"] }).notNull().default("not_filed"),
...
export const pcVerificationRequestsTable = pgTable("pc_verification_requests", {
  ...
  requestType: text("request_type", { enum: ["euo", "imc", "peer_review", "additional_verification", "examination"] }).notNull(),
  requestedBy: text("requested_by"),
  requestedAt: timestamp("requested_at").notNull(),
  dueDate: timestamp("due_date"),
  responseAt: timestamp("response_at"),
  status: text("status", { enum: ["pending", "scheduled", "completed", "failed_to_appear", "disputed", "waived"] }).notNull().default("pending"),
...
export const pcDenialsTable = pgTable("pc_denials", {
  ...
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  noFaultClaimId: integer("no_fault_claim_id").references(() => pcNoFaultClaimsTable.id),
  denialType: text("denial_type", { enum: ["no_fault_bill", "bodily_injury", "coverage", "late_notice", "policy_exclusion", "fraud"] }).notNull(),
  ...
  appealStatus: text("appeal_status", { enum: ["not_appealed", "pending", "filed", "decided", "exhausted"] }).notNull().default("not_appealed"),
...
export const pcDisclaimersTable = pgTable("pc_disclaimers", {
  ...
  challengeStatus: text("challenge_status", { enum: ["unchallenged", "challenged", "defeated", "upheld"] }).notNull().default("unchallenged"),
  notes: text("notes"),
  sourceLineage: text("source_lineage"),
  actorId: integer("actor_id"),
  isPrivileged: boolean("is_privileged").default(false),
...
export const pcCoveragePositionsTable = pgTable("pc_coverage_positions", {
  ...
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  positionType: text("position_type", { enum: ["coverage_affirmed", "coverage_denied", "reservation_of_rights", "partial_coverage", "disclaimer_issued"] }).notNull(),
  carrierName: text("carrier_name"),
...
export const pcMedicalBillCyclesTable = pgTable("pc_medical_bill_cycles", {
  ...
  status: text("status", { enum: ["submitted", "paid", "partially_paid", "denied", "arbitration", "withdrawn"] }).notNull().default("submitted"),
  denialReason: text("denial_reason"),
...
export const pcOfferMovementsTable = pgTable("pc_offer_movements", {
  ...
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  offerType: text("offer_type", { enum: ["plaintiff_demand", "insurer_offer", "counter_offer", "mediator_proposal", "nuisance_offer", "policy_limit_tender"] }).notNull(),
  ...
  movementSignal: text("movement_signal", { enum: ["approaching", "stalling", "retreating", "opening", "closing"] }),
  notes: text("notes"),
  sourceLineage: text("source_lineage"),
  actorId: integer("actor_id"),
  isPrivileged: boolean("is_privileged").default(true),
...
export const pcMediationEventsTable = pgTable("pc_mediation_events", {
  ...
  sessionType: text("session_type", { enum: ["court_ordered", "voluntary", "imc", "panel", "mini_trial"] }).notNull().default("court_ordered"),
  status: text("status", { enum: ["pending", "scheduled", "completed", "adjourned", "no_settlement", "settled"] }).notNull().default("pending"),
...
export const pcVenueProfilesTable = pgTable("pc_venue_profiles", {
  ...
  courtType: text("court_type", { enum: ["supreme", "civil", "district", "appellate", "federal"] }).notNull(),
  ...
  plaintiffFriendliness: text("plaintiff_friendliness", { enum: ["very_high", "high", "moderate", "low", "very_low"] }),
  adrAvailability: text("adr_availability", { enum: ["mandatory", "available", "limited", "none"] }),
...
export const pcInsurerProfilesTable = pgTable("pc_insurer_profiles", {
  ...
  region: text("region"),
  reservingStyle: text("reserving_style", { enum: ["aggressive", "conservative", "market", "unknown"] }),
  ...
  mediationBehavior: text("mediation_behavior", { enum: ["cooperative", "resistant", "strategic", "unpredictable"] }),
...
export const pcAdjusterProfilesTable = pgTable("pc_adjuster_profiles", {
  ...
  phone: text("phone"),
  claimOffice: text("claim_office"),
  negotiationStyle: text("negotiation_style", { enum: ["collaborative", "adversarial", "by_the_book", "delay_tactics"] }),
  averageResponseDays: integer("average_response_days"),
...
export const pcCommunicationWindowsTable = pgTable("pc_communication_windows", {
  ...
  partyRole: text("party_role", { enum: ["insurer", "adjuster", "opposing_counsel", "mediator", "court"] }).notNull(),
  lastContactAt: timestamp("last_contact_at"),
  daysSilent: integer("days_silent"),
  silenceRisk: text("silence_risk", { enum: ["none", "low", "medium", "high", "critical"] }).notNull().default("none"),
  expectedResponseDays: integer("expected_response_days"),
  outstandingItems: jsonb("outstanding_items"),
  escalationStatus: text("escalation_status", { enum: ["none", "sent", "pending_response", "escalated"] }).notNull().default("none"),
  sourceLineage: text("source_lineage"),
  actorId: integer("actor_id"),
  isPrivileged: boolean("is_privileged").default(false),
  exportFlag: boolean("export_flag").default(false),
...
export const pcDemandPacketsTable = pgTable("pc_demand_packets", {
  ...
  status: text("status", { enum: ["draft", "review", "approved", "sent", "responded"] }).notNull().default("draft"),
...
export const pcForecastRunsTable = pgTable("pc_forecast_runs", {
  ...
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  forecastType: text("forecast_type", { enum: ["deadline_breach_risk", "no_fault_evidence_lock_risk", "disclaimer_vulnerability_score", "demand_readiness_score", "offer_movement_forecast", "mediation_conversion_probability", "venue_velocity_forecast", "ai_defensibility_score"] }).notNull(),
...
export const pcForecastDriversTable = pgTable("pc_forecast_drivers", {
  id: serial("id").primaryKey(),
  forecastRunId: integer("forecast_run_id").notNull().references(() => pcForecastRunsTable.id, { onDelete: "cascade" }),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  actorId: integer("actor_id"),
...
export const pcAiReviewPacketsTable = pgTable("pc_ai_review_packets", {
  ...
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  reviewType: text("review_type", { enum: ["demand_packet", "chronology", "coverage_analysis", "negotiation_memo", "deposition_prep", "mediation_brief", "forecast_review"] }).notNull(),
  ...
  status: text("status", { enum: ["draft", "pending_review", "approved", "rejected", "revised"] }).notNull().default("draft"),
...
export const pcClockRulesTable = pgTable("pc_clock_rules", {
  ...
  appliesTo: text("applies_to", { enum: ["no_fault", "bodily_injury", "coverage", "all"] }).notNull().default("all"),
  isActive: boolean("is_active").default(true),
  sourceLineage: text("source_lineage"),
  actorId: integer("actor_id"),
  isPrivileged: boolean("is_privileged").default(false),
...
export const pcAppealsTable = pgTable("pc_appeals", {
  ...
  noFaultClaimId: integer("no_fault_claim_id").references(() => pcNoFaultClaimsTable.id),
  appealType: text("appeal_type", { enum: ["administrative", "arbitration", "court", "sup_ct_article_75", "appellate_division"] }).notNull(),
  filedAt: timestamp("filed_at"),
  deadlineAt: timestamp("deadline_at"),
  status: text("status", { enum: ["not_filed", "pending", "filed", "briefed", "argued", "decided", "withdrawn"] }).notNull().default("not_filed"),
...
export const pcExternalAppealsTable = pgTable("pc_external_appeals", {
  ...
  status: text("status", { enum: ["pending", "scheduled", "heard", "decided", "remanded", "withdrawn"] }).notNull().default("pending"),
  awardAmount: numeric("award_amount", { precision: 14, scale: 2 }),
  outcome: text("outcome"),
