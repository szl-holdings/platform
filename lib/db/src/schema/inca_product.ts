...
export const signalsTable = pgTable("signals", {
  ...
  status: text("status", { enum: ["new", "reviewed", "escalated", "closed"] }).notNull().default("new"),
...
export const findingsTable = pgTable("findings", {
  ...
  severity: text("severity", { enum: ["low", "medium", "high", "critical"] }).notNull().default("medium"),
  status: text("status", { enum: ["open", "confirmed", "disputed", "resolved"] }).notNull().default("open"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
...
export const insertInvestigationItemSchema = createInsertSchema(investigationItemsTable).omit({ id: true, createdAt: true });
...
export const insertIncaAlertSchema = createInsertSchema(incaAlertsTable).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertIncaAlert = z.infer<typeof insertIncaAlertSchema>;
export type IncaAlert = typeof incaAlertsTable.$inferSelect;

export const insertIncaReportSchema = createInsertSchema(incaReportsTable).omit({ id: true, createdAt: true });
