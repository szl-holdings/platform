...
export const insertLyteDashboardSchema = createInsertSchema(lyteDashboardsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const lyteSignalCommentsTable = pgTable(
  'lyte_signal_comments',
  {
    id: serial('id').primaryKey(),
    ...
    commentType: text('comment_type', {
      enum: ['comment', 'state_change', 'assignment', 'escalation', 'resolution'],
    })
      .notNull()
      .default('comment'),
  ...
  (table) => [index('lyte_signal_comments_signal_idx').on(table.signalId)],
);

export const lyteSignalTimelineTable = pgTable(
  'lyte_signal_timeline',
  {
    id: serial('id').primaryKey(),
    signalId: integer('signal_id')
      .notNull()
      .references(() => lyteSignalsTable.id, { onDelete: 'cascade' }),
  ...
  (table) => [index('lyte_signal_timeline_signal_idx').on(table.signalId)],
);

export const insertLyteSignalCommentSchema = createInsertSchema(lyteSignalCommentsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertLyteSignalTimelineSchema = createInsertSchema(lyteSignalTimelineTable).omit({
  id: true,
  createdAt: true,
});
