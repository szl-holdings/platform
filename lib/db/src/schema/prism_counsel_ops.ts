...
export const pcBackgroundJobsTable = pgTable("pc_background_jobs", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  jobType: text("job_type", { enum: [
    "document_ingest", "document_extract", "forecast_recompute", "deadline_evaluate",
    "notification_send", "export_generate", "connector_sync", "webhook_process",
    "manual_review", "replay_job", "clock_evaluate", "demand_packet_generate",
    "ai_review", "bulk_import", "report_generate"
  ] }).notNull(),
...
export const pcDocumentsTable = pgTable("pc_documents", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  ...
  storageContainer: text("storage_container", { enum: ["raw-ingest", "normalized-docs", "generated-artifacts", "audit-packets", "exports"] }),
  documentType: text("document_type", { enum: [
    "medical_record", "bill", "correspondence", "pleading", "discovery",
    "deposition_transcript", "expert_report", "photo", "police_report",
    "insurance_doc", "demand_letter", "settlement_agreement", "court_order",
    "motion", "affidavit", "other"
  ] }),
  ...
  reviewState: text("review_state", { enum: ["unreviewed", "reviewed", "flagged", "redacted", "quarantine"] }).default("unreviewed"),
...
export const pcExtractionJobsTable = pgTable("pc_extraction_jobs", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  documentId: integer("document_id").notNull().references(() => pcDocumentsTable.id, { onDelete: "cascade" }),
  extractionProvider: text("extraction_provider", { enum: ["azure_doc_intel", "manual", "ocr_fallback"] }).notNull().default("azure_doc_intel"),
  status: text("status", { enum: ["pending", "processing", "completed", "failed", "review_required"] }).notNull().default("pending"),
...
export const pcNotificationsTable = pgTable("pc_notifications", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  userId: integer("user_id"),
  matterId: integer("matter_id"),
  channel: text("channel", { enum: ["in_app", "email", "teams", "webhook"] }).notNull().default("in_app"),
  notificationType: text("notification_type", { enum: [
    "deadline_warning", "deadline_breach", "approval_required", "approval_resolved",
    "connector_error", "extraction_complete", "extraction_failed", "forecast_update",
    "ai_recommendation", "matter_update", "system_alert", "export_ready"
  ] }).notNull(),
  title: text("title").notNull(),
  body: text("body"),
...
export const pcWebhookSubscriptionsTable = pgTable("pc_webhook_subscriptions", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  connectorAccountId: integer("connector_account_id").references(() => pcConnectorAccountsTable.id),
  provider: text("provider", { enum: ["microsoft_graph", "docusign", "clio", "custom"] }).notNull(),
  resourceType: text("resource_type").notNull(),
  subscriptionId: text("subscription_id"),
  notificationUrl: text("notification_url"),
  expiresAt: timestamp("expires_at"),
  status: text("status", { enum: ["active", "expired", "failed", "pending"] }).notNull().default("pending"),
...
export const pcEmailsTable = pgTable("pc_emails", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
...
export const pcSourceItemsTable = pgTable("pc_source_items", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").references(() => pcMattersTable.id),
  ...
  index("pc_source_system_idx").on(table.sourceSystem, table.sourceRecordId),
]);
...
export const pcApprovalStepsTable = pgTable("pc_approval_steps", {
  ...
  status: text("status", { enum: ["pending", "approved", "rejected", "escalated", "expired"] }).notNull().default("pending"),
