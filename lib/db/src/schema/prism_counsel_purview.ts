...
export const pcPurviewCaseLinksTable = pgTable(
  'pc_purview_case_links',
  {
    ...
    linkStatus: text('link_status', { enum: ['active', 'closed', 'pending', 'error'] })
      .notNull()
      .default('active'),
...
export const pcPurviewHoldAwarenessTable = pgTable(
  'pc_purview_hold_awareness',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').notNull(),
    matterId: integer('matter_id')
    ...
    holdStatus: text('hold_status', { enum: ['active', 'released', 'pending', 'error'] })
      .notNull()
      .default('active'),
...
export const pcPurviewExportHandoffsTable = pgTable(
  'pc_purview_export_handoffs',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').notNull(),
    matterId: integer('matter_id')
    ...
    exportFormat: text('export_format', {
      enum: ['PST', 'EML', 'Relativity_RSMF', 'native', 'PDF', 'custom'],
    }).notNull(),
    exportStatus: text('export_status', {
      enum: ['pending', 'in_progress', 'ready', 'transferred', 'failed', 'expired'],
    })
      .notNull()
      .default('pending'),
    ...
    handoffMethod: text('handoff_method', {
      enum: ['secure_link', 'sftp', 'azure_blob', 'manual'],
    }),
...
export const pcPurviewScopeLinksTable = pgTable(
  'pc_purview_scope_links',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').notNull(),
    matterId: integer('matter_id')
      ...
      .references(() => pcMattersTable.id, { onDelete: 'cascade' }),
    caseLinkId: integer('case_link_id').references(() => pcPurviewCaseLinksTable.id),
    contentSourceType: text('content_source_type', {
      enum: [
        'mailbox',
        'sharepoint_site',
        'onedrive',
    ...
    reviewSetStatus: text('review_set_status', {
      enum: ['pending', 'in_review', 'complete', 'exported'],
    }),
    documentCount: integer('document_count'),
    metadata: jsonb('metadata'),
    createdAt: timestamp('created_at').notNull().defaultNow(),
...
export const pcPurviewDiagnosticsTable = pgTable(
  'pc_purview_diagnostics',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').notNull(),
