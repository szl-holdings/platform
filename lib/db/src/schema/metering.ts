import {
  pgTable, text, serial, timestamp, integer, numeric, jsonb, boolean, bigint, index, unique,
...
export const meteringEventsTable = pgTable("metering_events", {
...
export const usageAggregatesTable = pgTable("usage_aggregates", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull().references(() => organizationsTable.id, { onDelete: "cascade" }),
  featureKey: text("feature_key").notNull(),
  product: text("product").notNull().default("platform"),
  ...
  unique("usage_agg_org_feature_period_uq").on(t.orgId, t.featureKey, t.periodType, t.periodStart),
...
export const rateCardsTable = pgTable("rate_cards", {
  ...
  billingInterval: text("billing_interval", { enum: ["monthly", "annual", "usage"] }).notNull().default("monthly"),
...
export const rateCardTiersTable = pgTable("rate_card_tiers", {
  id: serial("id").primaryKey(),
  rateCardId: integer("rate_card_id").notNull().references(() => rateCardsTable.id, { onDelete: "cascade" }),
  ...
  index("rate_card_tiers_card_idx").on(t.rateCardId),
]);

export const rateCardAssignmentsTable = pgTable("rate_card_assignments", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull().references(() => organizationsTable.id, { onDelete: "cascade" }),
  rateCardId: integer("rate_card_id").notNull().references(() => rateCardsTable.id, { onDelete: "cascade" }),
  featureKey: text("feature_key").notNull(),
...
export const quotaConfigsTable = pgTable("quota_configs", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").references(() => organizationsTable.id, { onDelete: "cascade" }),
  featureKey: text("feature_key").notNull(),
  product: text("product").notNull().default("platform"),
  periodType: text("period_type", { enum: ["day", "month", "billing_cycle"] }).notNull().default("month"),
...
export const quotaViolationsTable = pgTable("quota_violations", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull().references(() => organizationsTable.id, { onDelete: "cascade" }),
  featureKey: text("feature_key").notNull(),
...
export const costAllocationsTable = pgTable("cost_allocations", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull().references(() => organizationsTable.id, { onDelete: "cascade" }),
  featureKey: text("feature_key").notNull(),
  product: text("product").notNull().default("platform"),
...
export const billingLineItemsTable = pgTable("billing_line_items", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull().references(() => organizationsTable.id, { onDelete: "cascade" }),
  ...
  status: text("status", { enum: ["draft", "finalized", "invoiced", "void"] }).notNull().default("draft"),
  metadata: jsonb("metadata"),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
}, (t) => [
...
export const insertMeteringEventSchema = createInsertSchema(meteringEventsTable).omit({ id: true, recordedAt: true });
...
export const insertUsageAggregateSchema = createInsertSchema(usageAggregatesTable).omit({ id: true, computedAt: true });
...
export const insertRateCardAssignmentSchema = createInsertSchema(rateCardAssignmentsTable).omit({ id: true, assignedAt: true });
...
export const insertQuotaConfigSchema = createInsertSchema(quotaConfigsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertQuotaViolationSchema = createInsertSchema(quotaViolationsTable).omit({ id: true, occurredAt: true });
...
export const insertCostAllocationSchema = createInsertSchema(costAllocationsTable).omit({ id: true, computedAt: true });
...
export const insertBillingLineItemSchema = createInsertSchema(billingLineItemsTable).omit({ id: true, createdAt: true });
