...
export const vesselSanctionsScreeningTable = pgTable("vessel_sanctions_screening", {
  id: serial("id").primaryKey(),
  vesselId: integer("vessel_id").notNull().references(() => vesselsTable.id, { onDelete: "cascade" }),
  screeningDate: timestamp("screening_date").notNull().defaultNow(),
  ofacStatus: text("ofac_status", { enum: ["clear", "match", "partial_match", "pending"] }).notNull().default("pending"),
  euStatus: text("eu_status", { enum: ["clear", "match", "partial_match", "pending"] }).notNull().default("pending"),
  unStatus: text("un_status", { enum: ["clear", "match", "partial_match", "pending"] }).notNull().default("pending"),
  ukStatus: text("uk_status", { enum: ["clear", "match", "partial_match", "pending"] }).notNull().default("pending"),
  ...
  pscResult: text("psc_result", { enum: ["passed", "deficiency", "detained", "no_inspection"] }).default("no_inspection"),
...
export const vesselPortCallsTable = pgTable("vessel_port_calls", {
  id: serial("id").primaryKey(),
  vesselId: integer("vessel_id").notNull().references(() => vesselsTable.id, { onDelete: "cascade" }),
  ...
  purpose: text("purpose", { enum: ["loading", "discharging", "bunkering", "crew_change", "repair", "inspection", "transit", "layup"] }).default("loading"),
...
export const vesselVoyageEconomicsTable = pgTable("vessel_voyage_economics", {
  id: serial("id").primaryKey(),
  vesselId: integer("vessel_id").notNull().references(() => vesselsTable.id, { onDelete: "cascade" }),
  ...
  charterType: text("charter_type", { enum: ["time_charter", "voyage_charter", "spot", "bareboat"] }).notNull().default("voyage_charter"),
  ...
  status: text("status", { enum: ["planned", "loading", "at_sea", "completed", "cancelled"] }).notNull().default("planned"),
...
export const insertVesselSanctionsScreeningSchema = createInsertSchema(vesselSanctionsScreeningTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertVesselPortCallSchema = createInsertSchema(vesselPortCallsTable).omit({ id: true, createdAt: true });
...
export const insertVesselVoyageEconomicsSchema = createInsertSchema(vesselVoyageEconomicsTable).omit({ id: true, createdAt: true, updatedAt: true });
