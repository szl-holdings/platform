...
export const mspClientsTable = pgTable('msp_clients', {
  ...
  status: text('status', { enum: ['active', 'inactive', 'at-risk', 'churned'] })
    .notNull()
    .default('active'),
...
export const mspTechniciansTable = pgTable('msp_technicians', {
  ...
  name: text('name').notNull(),
  email: text('email'),
  status: text('status', { enum: ['available', 'on-site', 'traveling', 'off-duty'] })
    .notNull()
    .default('available'),
...
export const mspTicketsTable = pgTable(
  'msp_tickets',
  {
    id: serial('id').primaryKey(),
    ...
    priority: text('priority', { enum: ['critical', 'high', 'medium', 'low'] })
      .notNull()
      .default('medium'),
    status: text('status', { enum: ['open', 'in-progress', 'waiting', 'resolved', 'closed'] })
      .notNull()
      .default('open'),
    assigneeId: integer('assignee_id').references(() => mspTechniciansTable.id, {
      onDelete: 'set null',
    }),
    assigneeName: text('assignee_name'),
    category: text('category'),
    slaDeadline: timestamp('sla_deadline'),
    slaStatus: text('sla_status', { enum: ['on-track', 'at-risk', 'breached'] })
      .notNull()
      .default('on-track'),
...
export const mspDevicesTable = pgTable(
  'msp_devices',
  {
    id: serial('id').primaryKey(),
    ...
    type: text('type', {
      enum: ['server', 'workstation', 'network', 'printer', 'mobile', 'firewall'],
    })
      .notNull()
      .default('workstation'),
    os: text('os'),
    ...
    site: text('site'),
    status: text('status', { enum: ['online', 'warning', 'critical', 'offline'] })
      .notNull()
      .default('online'),
...
export const mspContractsTable = pgTable(
  'msp_contracts',
  {
    id: serial('id').primaryKey(),
    name: text('name').notNull(),
    clientId: integer('client_id').references(() => mspClientsTable.id, { onDelete: 'set null' }),
    clientName: text('client_name'),
    type: text('type', { enum: ['managed-services', 'break-fix', 'project', 'security', 'cloud'] })
      .notNull()
      .default('managed-services'),
    status: text('status', { enum: ['active', 'expiring', 'expired', 'pending-renewal'] })
      .notNull()
      .default('active'),
  ...
  (t) => [index('msp_contracts_client_idx').on(t.clientId)],
);
