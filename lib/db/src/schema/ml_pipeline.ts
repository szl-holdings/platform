...
export const mlTrainingRuns = pgTable(
  ...
  (t) => [index("idx_ml_training_domain").on(t.domain), index("idx_ml_training_status").on(t.status)]
);
...
export const mlModelMonitoringSnapshots = pgTable(
  ...
  (t) => [index("idx_ml_monitor_model").on(t.modelVersionId), index("idx_ml_monitor_domain").on(t.domain)]
);
...
export const insertMlFeatureDefinitionSchema = createInsertSchema(mlFeatureDefinitions).omit({ id: true, createdAt: true, updatedAt: true });
export const insertMlFeatureValueSchema = createInsertSchema(mlFeatureValues).omit({ id: true });
export const insertMlDatasetSchema = createInsertSchema(mlDatasets).omit({ id: true, createdAt: true });
export const insertMlTrainingRunSchema = createInsertSchema(mlTrainingRuns).omit({ id: true, createdAt: true });
export const insertMlModelVersionSchema = createInsertSchema(mlModelVersions).omit({ id: true, createdAt: true, updatedAt: true });
export const insertMlPredictionSchema = createInsertSchema(mlPredictions).omit({ id: true });
export const insertMlMonitoringSnapshotSchema = createInsertSchema(mlModelMonitoringSnapshots).omit({ id: true, createdAt: true });
export const insertMlAbTestSchema = createInsertSchema(mlAbTests).omit({ id: true, createdAt: true, updatedAt: true });
