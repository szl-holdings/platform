import { pgTable, text, timestamp, integer, numeric, jsonb, boolean, primaryKey } from "drizzle-orm/pg-core";
...
export const pcGcObligationsTable = pgTable(
  ...
  {
    id: text("id").notNull(),
    matterId: text("matter_id").notNull().references(() => pcGcMattersTable.id, { onDelete: "cascade" }),
