...
export const otIcsAssetsTable = pgTable("ot_ics_assets", {
...
export const insertOtIcsAssetSchema = createInsertSchema(otIcsAssetsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const otIcsDecodedFramesTable = pgTable("ot_ics_decoded_frames", {
  id: serial("id").primaryKey(),
  ...
  fields: jsonb("fields").$type<Array<{ name: string; value: string; bytes: string; note?: string; flag?: "info" | "warn" | "anomaly" }>>().notNull().default([]),
...
export const insertOtIcsDecodedFrameSchema = createInsertSchema(otIcsDecodedFramesTable).omit({ id: true, createdAt: true });
...
export const otIcsConversationsTable = pgTable("ot_ics_conversations", {
  id: serial("id").primaryKey(),
...
export const insertOtIcsConversationSchema = createInsertSchema(otIcsConversationsTable).omit({ id: true, createdAt: true });
...
export const otIcsAnomalyScoresTable = pgTable("ot_ics_anomaly_scores", {
  id: serial("id").primaryKey(),
...
export const insertOtIcsAnomalyScoreSchema = createInsertSchema(otIcsAnomalyScoresTable).omit({ id: true, createdAt: true });
