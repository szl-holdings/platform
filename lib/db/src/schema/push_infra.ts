...
export const pushNotificationHistoryTable = pgTable('push_notification_history', {
  ...
  deliveryStatus: text('delivery_status', { enum: ['sent', 'partial', 'failed', 'pending'] })
    .notNull()
    .default('pending'),
...
export const pushReceiptsTable = pgTable('push_receipts', {
  ...
  historyId: integer('history_id').references(() => pushNotificationHistoryTable.id, {
    onDelete: 'set null',
  }),
  userId: integer('user_id').references(() => usersTable.id, { onDelete: 'set null' }),
  ...
  appId: text('app_id').notNull(),
  templateId: text('template_id'),
  status: text('status', { enum: ['pending', 'processing', 'ok', 'error'] })
    .notNull()
    .default('pending'),
...
export const scheduledNotificationsTable = pgTable('scheduled_notifications', {
  ...
  status: text('status', { enum: ['pending', 'processing', 'sent', 'failed', 'cancelled'] })
    .notNull()
    .default('pending'),
