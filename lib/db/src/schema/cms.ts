...
export const roadmapItemsTable = pgTable("roadmap_items", {
  ...
  status: text("status", { enum: ["planned", "in_progress", "completed", "delayed"] }).notNull().default("planned"),
...
export const leadStatusTable = pgTable("lead_status", {
  id: serial("id").primaryKey(),
  contactSubmissionId: integer("contact_submission_id").notNull().references(() => contactSubmissionsTable.id, { onDelete: "cascade" }),
  status: text("status", { enum: ["new", "contacted", "qualified", "closed", "lost"] }).notNull().default("new"),
...
export const insertVentureSchema = createInsertSchema(venturesTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertUseCaseSchema = createInsertSchema(useCasesTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertCtaSchema = createInsertSchema(ctasTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertNavigationItemSchema = createInsertSchema(navigationItemsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertLeadStatusSchema = createInsertSchema(leadStatusTable).omit({ id: true, updatedAt: true });
