...
export const pcMattersTable = pgTable("pc_matters", {
  ...
  matterType: text("matter_type", { enum: ["auto_injury", "premises_liability", "insurance_coverage", "medical_malpractice", "product_liability", "wrongful_death", "workers_comp", "other"] }).notNull(),
...
export const pcClaimsTable = pgTable("pc_claims", {
  ...
  coverageType: text("coverage_type", { enum: ["bodily_injury", "uninsured_motorist", "underinsured_motorist", "pip", "med_pay", "premises", "general_liability", "umbrella", "excess", "other"] }).notNull(),
...
export const pcDeadlinesTable = pgTable("pc_deadlines", {
  ...
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  title: text("title").notNull(),
  deadlineType: text("deadline_type", { enum: ["statute_of_limitations", "discovery_cutoff", "deposition", "mediation", "trial", "motion", "filing", "response", "expert_disclosure", "settlement_conference", "other"] }).notNull(),
...
export const pcForecastsTable = pgTable("pc_forecasts", {
  id: serial("id").primaryKey(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  forecastType: text("forecast_type", { enum: ["settlement_range", "cycle_time", "mediation_readiness", "trial_readiness", "evidence_sufficiency", "lien_resolution"] }).notNull(),
...
export const pcAiRecommendationsTable = pgTable("pc_ai_recommendations", {
  id: serial("id").primaryKey(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  recommendationType: text("recommendation_type", { enum: ["next_best_action", "missing_evidence", "demand_readiness", "discovery_follow_up", "deposition_prep", "mediation_prep", "privilege_warning", "inconsistency_alert", "deadline_risk"] }).notNull(),
  title: text("title").notNull(),
...
export const pcApprovalRequestsTable = pgTable("pc_approval_requests", {
  id: serial("id").primaryKey(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  requestType: text("request_type", { enum: ["demand_send", "settlement_acceptance", "external_communication", "expert_engagement", "filing", "client_disclosure", "fee_approval"] }).notNull(),
  title: text("title").notNull(),
  description: text("description"),
