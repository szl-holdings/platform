...
export const DRIFT_STATUS_VALUES = ['stable', 'watch', 'degraded', 'blocked'] as const;
...
export const spatialTwinSnapshotsTable = pgTable(
  'spatial_twin_snapshots',
  {
...
export const scenarioBranchesTable = pgTable(
  'scenario_branches',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    ...
    baselineSnapshotId: integer('baseline_snapshot_id').references(
      () => spatialTwinSnapshotsTable.id,
      { onDelete: 'set null' },
    ),
    branchSnapshotId: integer('branch_snapshot_id').references(() => spatialTwinSnapshotsTable.id, {
      onDelete: 'set null',
    }),
...
export const driftAssessmentsTable = pgTable(
  'drift_assessments',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    ...
    entityId: text('entity_id').notNull(),
    twinCategory: text('twin_category', { enum: SPATIAL_TWIN_CATEGORIES }).notNull(),
    currentSnapshotId: integer('current_snapshot_id').references(
      () => spatialTwinSnapshotsTable.id,
      { onDelete: 'set null' },
    ),
    approvedSnapshotId: integer('approved_snapshot_id').references(
      () => spatialTwinSnapshotsTable.id,
      { onDelete: 'set null' },
    ),
...
export const sceneMemoryIndexTable = pgTable(
  'scene_memory_index',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    ...
    entityId: text('entity_id').notNull(),
    twinCategory: text('twin_category', { enum: SPATIAL_TWIN_CATEGORIES }).notNull(),
    snapshotId: integer('snapshot_id').references(() => spatialTwinSnapshotsTable.id, {
      onDelete: 'cascade',
    }),
...
export const worldlineSignalOverlaysTable = pgTable(
  'worldline_signal_overlays',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id').references(() => organizationsTable.id, { onDelete: 'cascade' }),
    ...
    sourceTrustClass: text('source_trust_class', { enum: SOURCE_TRUST_CLASSES })
      .notNull()
      .default('inferred'),
...
export const insertWorldlineSignalOverlaySchema = createInsertSchema(
