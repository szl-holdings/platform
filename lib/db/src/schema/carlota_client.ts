...
export const clientAccountsTable = pgTable("client_accounts", {
  ...
  primaryContactUserId: integer("primary_contact_user_id").references(() => usersTable.id, { onDelete: "set null" }),
...
export const clientDocumentsTable = pgTable("client_documents", {
  id: serial("id").primaryKey(),
  organizationId: integer("organization_id").notNull().references(() => organizationsTable.id, { onDelete: "cascade" }),
  clientAccountId: integer("client_account_id").notNull().references(() => clientAccountsTable.id, { onDelete: "cascade" }),
  title: text("title").notNull(),
  description: text("description"),
  fileUrl: text("file_url"),
  fileType: text("file_type"),
  visibility: text("visibility", { enum: ["private", "client", "internal"] }).notNull().default("client"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
...
export const insertClientAccountSchema = createInsertSchema(clientAccountsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertClientUpdateSchema = createInsertSchema(clientUpdatesTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertClientMessageSchema = createInsertSchema(clientMessagesTable).omit({ id: true, createdAt: true });
