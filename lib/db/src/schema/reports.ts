...
export const reportDistributionsTable = pgTable("report_distributions", {
  ...
  distributedByUserId: integer("distributed_by_user_id").references(() => usersTable.id, { onDelete: "set null" }),
  createdAt: timestamp("created_at").notNull().defaultNow(),
});
...
export const insertReportGenerationSchema = createInsertSchema(reportGenerationsTable).omit({ id: true, createdAt: true, updatedAt: true });
export const insertReportApprovalSchema = createInsertSchema(reportApprovalsTable).omit({ id: true, createdAt: true });
export const insertReportDistributionSchema = createInsertSchema(reportDistributionsTable).omit({ id: true, createdAt: true });
export const insertReportScheduleSchema = createInsertSchema(reportSchedulesTable).omit({ id: true, createdAt: true, updatedAt: true });
