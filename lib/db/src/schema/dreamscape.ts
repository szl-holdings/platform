...
export const dreamscapeCampaignsTable = pgTable("dreamscape_campaigns", {
  ...
  clientName: text("client_name"),
  status: text("status", { enum: ["concept", "pre_production", "production", "post_production", "review", "published", "archived"] }).notNull().default("concept"),
  category: text("category", { enum: ["brand_story", "product_launch", "social_media", "documentary", "commercial", "internal"] }).notNull(),
...
export const dreamscapeScriptsTable = pgTable("dreamscape_scripts", {
  id: serial("id").primaryKey(),
  campaignId: integer("campaign_id").notNull().references(() => dreamscapeCampaignsTable.id, { onDelete: "cascade" }),
  title: text("title").notNull(),
  content: text("content").notNull(),
  version: integer("version").notNull().default(1),
  status: text("status", { enum: ["draft", "review", "approved", "final"] }).notNull().default("draft"),
  notes: text("notes"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
...
export const dreamscapeStoryboardsTable = pgTable("dreamscape_storyboards", {
  id: serial("id").primaryKey(),
  campaignId: integer("campaign_id").notNull().references(() => dreamscapeCampaignsTable.id, { onDelete: "cascade" }),
  scriptId: integer("script_id").references(() => dreamscapeScriptsTable.id, { onDelete: "set null" }),
  title: text("title").notNull(),
  description: text("description"),
...
export const dreamscapeVoiceAssetsTable = pgTable("dreamscape_voice_assets", {
  ...
  provider: text("provider", { enum: ["elevenlabs", "manual", "placeholder"] }).notNull().default("placeholder"),
  text: text("text"),
...
export const dreamscapeCampaignAssetsTable = pgTable("dreamscape_campaign_assets", {
  ...
  campaignId: integer("campaign_id").notNull().references(() => dreamscapeCampaignsTable.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
  type: text("type", { enum: ["image", "video", "audio", "document", "font", "template", "other"] }).notNull(),
  fileUrl: text("file_url"),
  thumbnailUrl: text("thumbnail_url"),
...
export const insertDreamscapeScriptSchema = createInsertSchema(dreamscapeScriptsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertDreamscapeStoryboardSchema = createInsertSchema(dreamscapeStoryboardsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertDreamscapeVoiceAssetSchema = createInsertSchema(dreamscapeVoiceAssetsTable).omit({ id: true, createdAt: true });
...
export const insertDreamscapeCampaignAssetSchema = createInsertSchema(dreamscapeCampaignAssetsTable).omit({ id: true, createdAt: true });
