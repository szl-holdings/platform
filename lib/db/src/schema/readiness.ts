...
export const readinessDimensionsTable = pgTable('readiness_dimensions', {
  ...
  programId: integer('program_id')
    .notNull()
    .references(() => readinessProgramsTable.id, { onDelete: 'cascade' }),
  name: text('name').notNull(),
  description: text('description'),
...
export const readinessScoreHistoryTable = pgTable('readiness_score_history', {
  ...
  dimensionId: integer('dimension_id')
    .notNull()
    .references(() => readinessDimensionsTable.id, { onDelete: 'cascade' }),
  programId: integer('program_id')
    .notNull()
    .references(() => readinessProgramsTable.id, { onDelete: 'cascade' }),
...
export const readinessMilestonesTable = pgTable('readiness_milestones', {
  ...
  description: text('description'),
  status: text('status', { enum: ['pending', 'in_progress', 'completed', 'overdue', 'canceled'] })
    .notNull()
    .default('pending'),
...
export const readinessRisksTable = pgTable('readiness_risks', {
  ...
  programId: integer('program_id')
    .notNull()
    .references(() => readinessProgramsTable.id, { onDelete: 'cascade' }),
  dimensionId: integer('dimension_id').references(() => readinessDimensionsTable.id, {
    onDelete: 'set null',
  }),
  title: text('title').notNull(),
  description: text('description').notNull(),
  severity: text('severity', { enum: ['critical', 'high', 'medium', 'low'] }).notNull(),
  likelihood: text('likelihood', { enum: ['very_likely', 'likely', 'possible', 'unlikely'] })
    .notNull()
    .default('possible'),
  status: text('status', { enum: ['open', 'mitigating', 'resolved', 'accepted'] })
    .notNull()
    .default('open'),
...
export const insertReadinessProgramSchema = createInsertSchema(readinessProgramsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertReadinessDimensionSchema = createInsertSchema(readinessDimensionsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertReadinessScoreHistorySchema = createInsertSchema(
...
export const insertReadinessMilestoneSchema = createInsertSchema(readinessMilestonesTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertReadinessRiskSchema = createInsertSchema(readinessRisksTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertReadinessAlertSchema = createInsertSchema(readinessAlertsTable).omit({
  id: true,
  createdAt: true,
});
