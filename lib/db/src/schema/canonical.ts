...
export const productsTable = pgTable('products', {
  ...
  category: text('category', {
    enum: ['platform', 'ops', 'intelligence', 'maritime', 'security', 'analytics'],
  })
    .notNull()
    .default('platform'),
...
export const platformSignalsTable = pgTable(
  'platform_signals',
  {
    id: serial('id').primaryKey(),
...
export const actionsTable = pgTable(
  'actions',
  {
    ...
    actionType: text('action_type', {
      ...
      .default('manual'),
    status: text('status', {
      enum: ['pending', 'in_progress', 'completed', 'deferred', 'cancelled', 'blocked'],
    })
      .notNull()
      .default('pending'),
...
export const workflowsTable = pgTable(
  'workflows',
  {
    ...
    triggerType: text('trigger_type', {
      enum: ['signal', 'schedule', 'manual', 'webhook', 'action'],
    })
      .notNull()
      .default('manual'),
    triggerConfig: jsonb('trigger_config'),
    steps: jsonb('steps')
      .$type<Array<{ id: string; name: string; type: string; config: Record<string, unknown> }>>()
      .default([]),
    status: text('status', { enum: ['active', 'inactive', 'draft', 'archived'] })
      .notNull()
      .default('draft'),
    ownerId: integer('owner_id').references(() => usersTable.id, { onDelete: 'set null' }),
...
export const approvalsTable = pgTable(
  'approvals',
  {
    ...
    status: text('status', { enum: ['pending', 'approved', 'rejected', 'cancelled', 'expired'] })
      .notNull()
      .default('pending'),
...
export const artifactsTable = pgTable(
  'artifacts',
  {
    ...
    artifactType: text('artifact_type', {
      enum: ['report', 'export', 'snapshot', 'evidence', 'playbook', 'template', 'other'],
    })
      .notNull()
      .default('report'),
    status: text('status', {
      enum: ['pending', 'generating', 'ready', 'approved', 'rejected', 'archived'],
    })
      .notNull()
      .default('pending'),
...
export const readinessItemsTable = pgTable(
  'readiness_items',
  {
    id: serial('id').primaryKey(),
    orgId: integer('org_id')
...
export const savedViewsTable = pgTable(
  ...
  (t) => [index('saved_views_org_idx').on(t.orgId), index('saved_views_user_idx').on(t.userId)],
);
...
export const insertPlatformSignalSchema = createInsertSchema(platformSignalsTable).omit({
  id: true,
  createdAt: true,
});
...
export const insertReadinessItemSchema = createInsertSchema(readinessItemsTable).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
