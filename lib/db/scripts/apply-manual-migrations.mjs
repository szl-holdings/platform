...
if (!fs.existsSync(MIGRATIONS_DIR)) {
  console.error(`[manual-migrations] Cannot find migrations dir at ${MIGRATIONS_DIR}`);
  process.exit(1);
}
...
if (files.length === 0) {
  console.log("[manual-migrations] No SQL files found.");
  process.exit(0);
}
  ...
  for (const filename of files) {
    ...
    if (recorded) {
      if (recorded !== checksum) {
        console.warn(
          `[manual-migrations] WARN drift: ${filename} content changed since apply ` +
            `(recorded ${recorded.slice(0, 12)}, current ${checksum.slice(0, 12)}). ` +
            `Write a new migration rather than editing applied SQL.`,
        );
        driftCount += 1;
      }
      skippedCount += 1;
    ...
    if (DRY_RUN) {
      console.log(`[manual-migrations] PENDING ${filename} (dry-run)`);
      continue;
    }

    console.log(`[manual-migrations] applying ${filename}...`);
    try {
      ...
      console.error(`[manual-migrations] FAILED ${filename}: ${err.message}`);
      process.exit(1);
    }
  }

  console.log(
    `[manual-migrations] done — applied=${appliedCount} skipped=${skippedCount} drift=${driftCount} total=${files.length}` +
      (DRY_RUN ? " (dry-run)" : ""),
  );
} finally {
  await client.end();
