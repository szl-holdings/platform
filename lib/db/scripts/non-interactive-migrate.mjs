...
console.log(`[${ts()}] [db:migrate] non-interactive wrapper starting`);
console.log(`[${ts()}] [db:migrate] timeout=${TIMEOUT_MS}ms cwd=${DB_DIR}`);
...
function relayChunk(streamName, chunk) {
  ...
  for (const marker of PROMPT_MARKERS) {
    ...
    if (idx !== -1 && idx > (streamName === "stderr" ? stderrBuf.length : stdoutBuf.length) - text.length - marker.length) {
      promptsSeen += 1;
      if (FAIL_ON_PROMPT) {
        console.error(
          `[${ts()}] [db:migrate] DB_MIGRATE_FAIL_ON_PROMPT=1 and drizzle-kit emitted prompt #${promptsSeen} (matched "${marker}") — aborting instead of auto-answering. Generate a versioned migration file with \`pnpm --filter @szl-holdings/db run generate\` and commit it, then re-run.`,
        );
        promptAbort = true;
      ...
      console.log(
        `[${ts()}] [db:migrate] auto-answered drizzle-kit prompt #${promptsSeen} (default: create new — substring match "${marker}", may overcount) — audit the lines above to confirm intent`,
      );
    }
  }
...
child.stdin.on("error", (err) => {
  ...
  console.error(`[${ts()}] [db:migrate] stdin error:`, err);
});
...
function triggerKill(reason) {
  if (closed) return;
  console.error(
    `[${ts()}] [db:migrate] sending SIGTERM to drizzle-kit (reason=${reason}) — prompts seen so far: ${promptsSeen}.`,
  );
  try {
    child.kill("SIGTERM");
  } catch {
  ...
  setTimeout(() => {
    if (!closed) {
      console.error(
        `[${ts()}] [db:migrate] drizzle-kit still alive 5s after SIGTERM — escalating to SIGKILL.`,
      );
      try {
        child.kill("SIGKILL");
      } catch {
...
const killTimer = setTimeout(() => {
  killed = true;
  console.error(
    `[${ts()}] [db:migrate] HARD TIMEOUT after ${TIMEOUT_MS}ms.`,
  );
...
child.on("error", (err) => {
  ...
  console.error(`[${ts()}] [db:migrate] failed to spawn drizzle-kit:`, err);
  process.exitCode = 1;
});
