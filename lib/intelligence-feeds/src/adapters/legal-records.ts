...
export function createLegalRecordsConfig(
...
export class LegalRecordsFeedAdapter extends BaseFeedAdapter {
  ...
  private prismMatterIndex: Map<string, { id: number; title: string; matterType: string }> =
    new Map();
  ...
  private async loadPrismMatterIndex(): Promise<void> {
    try {
      ...
      for (const m of matters) {
        if (m.caseNumber) {
          this.prismMatterIndex.set(m.caseNumber.toLowerCase(), {
            id: m.id,
            title: m.title,
  ...
  private normalizeCases(cases: CourtListenerCase[], sourceUrl: string): NormalizedFeedPayload {
    ...
    for (const cas of cases) {
      const caseName =
        cas.case_name_short ?? cas.case_name ?? `Case ${cas.docket_number ?? cas.id}`;
      ...
      entities.push({
        ...
        metadata: {
          ...
          courtListenerUrl: cas.absolute_url
            ? `https://www.courtlistener.com${cas.absolute_url}`
            : null,
          caseNature,
        ...
        for (const party of cas.parties) {
          ...
          entities.push({
            ...
            tags: [
              'legal-party',
              partyType,
              party.type ? `role:${party.type.toLowerCase()}` : 'role:unknown',
            ],
          });
  ...
  private classifyPartyType(partyType: string | undefined): 'person' | 'organization' {
    ...
    if (t.includes('corp') || t.includes('inc') || t.includes('llc') || t.includes('ltd'))
      return 'organization';
    if (t.includes('plaintiff') || t.includes('defendant') || t.includes('individual'))
      return 'person';
    return 'organization';
  }
