...
interface SDNEntry {
  ...
  programList?: string[];
  aliases?: string[];
  akaList?: Array<{ type: string; category?: string; lastName?: string; firstName?: string; wholeName?: string }>;
...
export function createSanctionsConfig(overrides: Partial<FeedAdapterConfig> = {}): FeedAdapterConfig {
  return {
    id: "sanctions-lists",
...
export class SanctionsFeedAdapter extends BaseFeedAdapter {
  ...
  private async ensureCanonicalSanctioningBodies(): Promise<void> {
    const bodies: Array<Omit<NormalizedFeedPayload["entities"][0], never> & { externalId: string }> = [
      {
        type: "organization",
  ...
  normalize(rawData: unknown): NormalizedFeedPayload {
    ...
    return {
      ...
      feedId: this.config.id, feedName: this.config.name,
      sourceUrl: "raw", fetchedAt: new Date().toISOString(), recordCount: 0,
    };
  }
  ...
  private normalizeEU(entries: EUSanctionEntry[], sourceUrl: string): NormalizedFeedPayload {
    ...
    for (const entry of entries) {
      ...
      const ontologyType: "person" | "organization" = entry.entityType === "P" ? "person" : "organization";
    ...
    return {
      entities, relationships,
      feedId: this.config.id, feedName: this.config.name,
      sourceUrl, fetchedAt, recordCount: entries.length,
    };
  }
  ...
  private normalizeUN(entries: UNSanctionEntry[], sourceUrl: string): NormalizedFeedPayload {
    ...
    for (const entry of entries) {
      ...
      const ontologyType: "person" | "organization" = (entry.type ?? "").toLowerCase().includes("individ") ? "person" : "organization";
  ...
  private parseUNXML(xmlText: string): UNSanctionEntry[] {
    ...
    while ((match = entityRegex.exec(xmlText)) !== null) {
      ...
      const dataId = this.extractXMLField(block, "DATAID") ?? `un-${Math.random().toString(36).slice(2)}`;
      const name = this.buildUNName(block, type);
      if (!name) continue;

      const aliases: string[] = [];
      const aliasBlocks = block.match(/<INDIVIDUAL_ALIAS>([\s\S]*?)<\/INDIVIDUAL_ALIAS>|<ENTITY_ALIAS>([\s\S]*?)<\/ENTITY_ALIAS>/g) ?? [];
