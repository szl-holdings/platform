...
function stixTypeToOntologyType(
  stixType: string,
): 'threat' | 'signal' | 'organization' | 'person' | 'asset' {
  switch (stixType) {
    case 'threat-actor':
      return 'threat';
    case 'campaign':
      ...
      if (this.taxiiServer) {
        const res = await fetch(`${this.taxiiServer}/taxii/`, {
          ...
          headers: {
            ...
            ...(this.otxApiKey ? { Authorization: `Bearer ${this.otxApiKey}` } : {}),
          },
    ...
    try {
      ...
      const data = (await response.json()) as {
        results?: Array<{
          ...
          description: string;
          tags: string[];
          indicators: Array<{ type: string; indicator: string; description?: string }>;
        }>;
      };
  ...
  private async pollPublicMISP(): Promise<NormalizedFeedPayload> {
    ...
    try {
      ...
      const manifest = (await response.json()) as Record<
        string,
        { url?: string; date?: string; tag?: string[] }
      >;
  ...
  private normalizeBundle(objects: STIXObject[], sourceUrl: string): NormalizedFeedPayload {
    ...
    for (const obj of mainObjects) {
      ...
      const killChainPhases = obj.kill_chain_phases
        ?.map((kc) => `${kc.kill_chain_name}:${kc.phase_name}`)
        .join(', ');
      ...
      entities.push({
        ...
        tags: ['stix', obj.type, ...(obj.labels ?? []), ...(mitreId ? [mitreId] : [])].filter(
          Boolean,
        ),
