...
export class AISFeedAdapter extends BaseFeedAdapter {
  ...
  async healthCheck(): Promise<void> {
    ...
    try {
      if (this.apiKey) {
        ...
        const res = await fetch("https://data.aishub.net/ws.php?format=1&output=json&compress=0&limit=1", {
          method: "GET",
  ...
  private async pollAISHub(): Promise<NormalizedFeedPayload> {
    ...
    try {
      ...
      const positions: AISVesselPosition[] = (Array.isArray(raw) ? raw.flat() : []).map(r => ({
  ...
  private normalizePositions(positions: AISVesselPosition[], sourceUrl: string): NormalizedFeedPayload {
  ...
  private computeVesselRiskScore(pos: AISVesselPosition): number {
    ...
    if (pos.speed !== undefined && pos.speed < 0.5 && pos.status !== 1 && pos.status !== 5) score += 0.1;
