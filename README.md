...
> **Terminology:** **Domain packs** (Sentra, Counsel, Aegis, Vessels, Terra, Carlota Jo — 6 total) each own a vertical of business data and operations. **Cross-domain surfaces** (Pulse, Command, Lyte, SZL Holdings dashboard) aggregate signals across packs and do not own an independent data domain. See `audit/source-of-truth.json` (`domain_packs.count: 6`).

| Product | Domain | Status |
|---------|--------|--------|
| **Sentra** | Cyber resilience command — exposure mapping, recovery readiness, incident command, control drift detection | Active (domain pack) |
| **Counsel** | Legal matter command — agentic matter management, obligation tracking, exposure quantification, court filing integration | Active (domain pack) |
| **Aegis** | Security and defense intelligence domain pack — SOC command, SOAR playbooks, threat intelligence. API routes active under `artifacts/api-server/src/routes/firestorm/`; defense web UI archived (Task #920). The `artifacts/aegis` web artifact is the investor pitch deck (ATLAS runtime), not the SOC UI. | API domain active; pitch deck artifact active |
| **Vessels** | Maritime fleet intelligence — AIS tracking, S&P workflow, demurrage, freight, voyage P&L | Active (domain pack) |
| **Terra** | Real estate intelligence — distress pipeline, ownership graph, deal workflow, AI analysis | Active (domain pack) |
| **Carlota Jo** | Premium advisory operations — UHNW client portal, service catalog, engagement management | Active (domain pack) |
| **Pulse** | AI executive briefing — narrative intelligence reports synthesized from live platform signals across all domain packs. Cross-domain surface (not a domain pack); does not own an independent data domain. | Active (cross-domain surface) |
