...
function isDocPathExcluded(relativePath: string): boolean {
  return EXCLUDE_DOC_PATHS.some((excluded) =>
...
function writeExclusionReport(stagingDir: string): void {
  ...
  for (const excluded of report.excludedPaths) {
    ...
    else if (/\.(env|sql\.gz|dump|pgdump|bak|backup)$/.test(excluded)) reason = "Secret or sensitive data";
...
function main(): void {
  ...
  log("  3. Report: tsx scripts/public-mirror/report-public-surface.ts " + MIRROR_DIR);
