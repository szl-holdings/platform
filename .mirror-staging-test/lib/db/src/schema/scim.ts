...
export const scimTokensTable = pgTable("scim_tokens", {
  ...
  tenantId: integer("tenant_id").notNull().references(() => azureTenantsTable.id, { onDelete: "cascade" }),
...
export const scimGroupsTable = pgTable("scim_groups", {
  ...
  uniqueIndex("scim_groups_tenant_external_idx").on(table.tenantId, table.externalId),
]);

export const scimGroupMembersTable = pgTable("scim_group_members", {
  id: serial("id").primaryKey(),
  groupId: integer("group_id").notNull().references(() => scimGroupsTable.id, { onDelete: "cascade" }),
  userId: integer("user_id").notNull().references(() => usersTable.id, { onDelete: "cascade" }),
  addedAt: timestamp("added_at").notNull().defaultNow(),
}, (table) => [
  uniqueIndex("scim_group_member_unique").on(table.groupId, table.userId),
]);

export const scimProvisionedUsersTable = pgTable("scim_provisioned_users", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => azureTenantsTable.id, { onDelete: "cascade" }),
  userId: integer("user_id").notNull().references(() => usersTable.id, { onDelete: "cascade" }),
  externalId: text("external_id"),
...
export const scimSyncLogsTable = pgTable("scim_sync_logs", {
  id: serial("id").primaryKey(),
  tenantId: integer("tenant_id").notNull().references(() => azureTenantsTable.id, { onDelete: "cascade" }),
  operation: text("operation", { enum: ["create_user", "update_user", "delete_user", "create_group", "update_group", "delete_group", "patch_user", "patch_group"] }).notNull(),
...
export const insertScimTokenSchema = createInsertSchema(scimTokensTable).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertScimToken = z.infer<typeof insertScimTokenSchema>;
export type ScimToken = typeof scimTokensTable.$inferSelect;

export const insertScimGroupSchema = createInsertSchema(scimGroupsTable).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertScimGroup = z.infer<typeof insertScimGroupSchema>;
export type ScimGroup = typeof scimGroupsTable.$inferSelect;

export const insertScimProvisionedUserSchema = createInsertSchema(scimProvisionedUsersTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertScimSyncLogSchema = createInsertSchema(scimSyncLogsTable).omit({ id: true, createdAt: true });
