...
export const incaProjectsTable = pgTable("inca_projects", {
  ...
  status: text("status", { enum: ["research", "development", "testing", "deployed", "archived"] }).notNull().default("research"),
  domain: text("domain"),
...
export const incaExperimentsTable = pgTable("inca_experiments", {
  id: serial("id").primaryKey(),
  projectId: integer("project_id").notNull().references(() => incaProjectsTable.id, { onDelete: "cascade" }),
  name: text("name").notNull(),
  hypothesis: text("hypothesis"),
  results: text("results"),
  status: text("status", { enum: ["queued", "running", "completed", "failed", "canceled"] }).notNull().default("queued"),
...
export const incaModelsTable = pgTable("inca_models", {
  ...
  status: text("status", { enum: ["training", "staging", "production", "retired"] }).notNull().default("training"),
...
export const incaDatasetsTable = pgTable("inca_datasets", {
  id: serial("id").primaryKey(),
  projectId: integer("project_id").references(() => incaProjectsTable.id, { onDelete: "set null" }),
  name: text("name").notNull(),
  description: text("description"),
  size: text("size"),
  format: text("format"),
  ...
  status: text("status", { enum: ["raw", "processed", "validated", "archived"] }).notNull().default("raw"),
  metadata: jsonb("metadata"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
}, (t) => [
...
export const insertIncaExperimentSchema = createInsertSchema(incaExperimentsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertIncaModelSchema = createInsertSchema(incaModelsTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertIncaInsightSchema = createInsertSchema(incaInsightsTable).omit({ id: true, createdAt: true });
...
export const insertIncaDatasetSchema = createInsertSchema(incaDatasetsTable).omit({ id: true, createdAt: true, updatedAt: true });
