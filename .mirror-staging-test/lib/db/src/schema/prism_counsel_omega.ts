import {
  pgTable, text, serial, timestamp, integer, numeric, jsonb, boolean, index, real, uniqueIndex
} from "drizzle-orm/pg-core";
import { pcMattersTable, pcConnectorAccountsTable } from "./prism_counsel";

/* ─── RBAC / Roles ──────────────────────────────────────────────────────── */

export const pcOrgRolesTable = pgTable("pc_org_roles", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  userId: integer("user_id").notNull(),
  role: text("role", { enum: [
    "founder_admin", "platform_admin", "org_admin", "attorney", "paralegal",
    "operator", "analyst", "client_viewer", "external_reviewer", "support_admin"
  ] }).notNull(),
...
export const pcGraphqlAuditLogsTable = pgTable("pc_graphql_audit_logs", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
...
export const pcGraphqlAuditTagsTable = pgTable("pc_graphql_audit_tags", {
  id: serial("id").primaryKey(),
  auditLogId: integer("audit_log_id").notNull().references(() => pcGraphqlAuditLogsTable.id, { onDelete: "cascade" }),
  tag: text("tag").notNull(),
  value: text("value"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
}, (t) => [
  index("pc_gql_tags_log_idx").on(t.auditLogId),
]);
...
export const pcSignalForgeRunsTable = pgTable("pc_signal_forge_runs", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  sourceId: integer("source_id"),
  stage: text("stage", { enum: [
    "ingest", "clean", "normalize", "enrich", "contradiction_detect",
    "feature_engineer", "quality_score", "complete", "failed"
  ] }).notNull().default("ingest"),
  ...
  index("pc_sf_runs_org_idx").on(t.orgId),
  index("pc_sf_runs_stage_idx").on(t.stage),
]);

export const pcSignalQualityScoresTable = pgTable("pc_signal_quality_scores", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
...
export const pcPressureGraphDimensionsTable = pgTable("pc_pressure_graph_dimensions", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  dimension: text("dimension", { enum: [
    "deadline", "insurer", "adjuster", "coverage", "venue", "medical",
    "damages", "settlement", "weather_event", "evidence", "communication", "governance"
  ] }).notNull(),
  currentScore: real("current_score").notNull(),
  priorScore: real("prior_score"),
  movementDirection: text("movement_direction", { enum: ["rising", "falling", "stable", "volatile"] }).notNull().default("stable"),
...
export const pcPressureWatchlistTable = pgTable("pc_pressure_watchlist", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  ...
  index("pc_pg_watchlist_matter_idx").on(t.matterId),
]);

/* ─── Matter Twin Domains ────────────────────────────────────────────────── */

export const pcMatterTwinSubpagesTable = pgTable("pc_matter_twin_subpages", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  subpage: text("subpage", { enum: [
    "summary", "twin", "timeline", "deadlines", "forecast", "pressure",
    "medical", "damages", "communications", "documents", "demand",
    "discovery", "depositions", "mediation", "approvals", "audit", "proof_chain"
  ] }).notNull(),
...
export const pcCopilotDraftsTable = pgTable("pc_copilot_drafts", {
  ...
  reviewState: text("review_state", { enum: ["draft", "review_pending", "reviewed", "approved", "rejected", "exported"] }).notNull().default("draft"),
...
export const pcM365CalendarEventsTable = pgTable("pc_m365_calendar_events", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").references(() => pcMattersTable.id),
  ...
  eventType: text("event_type", { enum: ["deposition", "mediation", "hearing", "settlement_conf", "client_meeting", "expert_meeting", "internal", "other"] }),
...
export const pcM365SharepointFilesTable = pgTable("pc_m365_sharepoint_files", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").references(() => pcMattersTable.id),
...
export const pcM365TeamsMessagesTable = pgTable("pc_m365_teams_messages", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").references(() => pcMattersTable.id),
  ...
  body: text("body"),
  messageType: text("message_type", { enum: ["alert_card", "approval_prompt", "deadline_warning", "connector_failure", "matter_update", "standard"] }).notNull().default("standard"),
...
export const pcSchemaRegistryTable = pgTable("pc_schema_registry", {
  id: serial("id").primaryKey(),
  ...
  index("pc_schema_reg_subgraph_idx").on(t.subgraphName),
]);

/* ─── Approval Evidence / Audit Packets ──────────────────────────────────── */

export const pcApprovalEvidenceTable = pgTable("pc_approval_evidence", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  approvalRequestId: integer("approval_request_id").notNull(),
  evidenceType: text("evidence_type", { enum: ["source_document", "proof_chain_entry", "model_output", "audit_snapshot", "user_statement", "system_log"] }).notNull(),
  ...
  index("pc_approval_evidence_req_idx").on(t.approvalRequestId),
]);

export const pcAuditPacketsTable = pgTable("pc_audit_packets", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").references(() => pcMattersTable.id),
  packetType: text("packet_type", { enum: ["review_packet", "demand_packet", "approval_packet", "export_safety_packet", "privilege_review_packet"] }).notNull(),
  title: text("title").notNull(),
...
export const pcContradictionPanelTable = pgTable("pc_contradiction_panel", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").notNull().references(() => pcMattersTable.id, { onDelete: "cascade" }),
  contradictionType: text("contradiction_type", { enum: [
    "fact_conflict", "chronology_gap", "medical_discrepancy", "testimony_conflict",
    "document_vs_testimony", "insurer_vs_claimant", "expert_conflict", "date_conflict"
  ] }).notNull(),
  ...
  status: text("status", { enum: ["open", "resolved", "dismissed", "escalated"] }).notNull().default("open"),
...
export const pcOperationalFlowRunsTable = pgTable("pc_operational_flow_runs", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  matterId: integer("matter_id").references(() => pcMattersTable.id),
  flowType: text("flow_type", { enum: [
    "outlook_to_matter_twin", "sharepoint_to_evidence", "world_to_worldline",
    "copilot_workbench_answer", "approval_to_action", "review_packet_generation"
  ] }).notNull(),
  status: text("status", { enum: ["initiated", "running", "awaiting_approval", "completed", "failed", "rolled_back"] }).notNull().default("initiated"),
...
export const pcDashboardSnapshotsTable = pgTable("pc_dashboard_snapshots", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  dashboardType: text("dashboard_type", { enum: [
    "executive_health", "connector_health", "worldline_health", "graphql_health",
    "forecast_health", "approval_health", "onboarding_health", "cost_visibility", "incident_response"
  ] }).notNull(),
...
export const pcServiceMetricsTable = pgTable("pc_service_metrics", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  service: text("service", { enum: [
  ...
  healthStatus: text("health_status", { enum: ["healthy", "degraded", "critical", "down"] }).notNull().default("healthy"),
...
export const pcTenantConfigTable = pgTable("pc_tenant_config", {
  id: serial("id").primaryKey(),
  ...
  billingPlan: text("billing_plan", { enum: ["trial", "starter", "professional", "enterprise"] }).notNull().default("trial"),
  onboardingStatus: text("onboarding_status", { enum: ["not_started", "in_progress", "connectors_setup", "data_loaded", "active"] }).notNull().default("not_started"),
  ...
  index("pc_tenant_config_org_idx").on(t.orgId),
]);

/* ─── Incident Response ──────────────────────────────────────────────────── */

export const pcIncidentsTable = pgTable("pc_incidents", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  title: text("title").notNull(),
  ...
  status: text("status", { enum: ["open", "investigating", "mitigated", "resolved", "post_mortem"] }).notNull().default("open"),
  ...
  index("pc_incidents_org_idx").on(t.orgId),
  index("pc_incidents_status_idx").on(t.status),
]);

/* ─── Onboarding ─────────────────────────────────────────────────────────── */

export const pcOnboardingChecklistTable = pgTable("pc_onboarding_checklist", {
  id: serial("id").primaryKey(),
  orgId: integer("org_id").notNull(),
  step: text("step", { enum: [
    "oidc_configured", "first_user_created", "connector_added", "first_matter_created",
    "demo_data_seeded", "m365_consent_granted", "worldline_initialized",
    "model_lanes_configured", "playbook_created", "first_export_approved"
  ] }).notNull(),
  status: text("status", { enum: ["pending", "complete", "skipped", "blocked"] }).notNull().default("pending"),
