...
export const certificationRequirementsTable = pgTable("certification_requirements", {
  id: serial("id").primaryKey(),
  programId: integer("program_id").notNull().references(() => certificationProgramsTable.id, { onDelete: "cascade" }),
  requirementKey: text("requirement_key").notNull(),
  title: text("title").notNull(),
  description: text("description"),
  category: text("category", {
    enum: ["ownership", "control", "documentation", "financials", "operational", "legal", "identity", "other"],
  }).notNull().default("other"),
  isRequired: boolean("is_required").notNull().default(true),
  requiresReview: boolean("requires_review").notNull().default(false),
  reviewType: text("review_type", { enum: ["attorney", "cpa", "both", "none"] }).notNull().default("none"),
  notes: text("notes"),
  sortOrder: integer("sort_order").notNull().default(0),
  createdAt: timestamp("created_at").notNull().defaultNow(),
}, (t) => [
  index("cert_reqs_program_idx").on(t.programId),
]);

// ─── CERTIFICATION STATUS ─────────────────────────────────────────────────────

export const certificationStatusTable = pgTable("certification_status", {
  id: serial("id").primaryKey(),
  programId: integer("program_id").notNull().references(() => certificationProgramsTable.id, { onDelete: "cascade" }),
  overallStatus: text("overall_status", {
    enum: ["not_started", "assessing", "preparing", "applied", "in_review", "approved", "denied", "renewal_due", "expired", "withdrawn"],
...
export const certificationTasksTable = pgTable("certification_tasks", {
  id: serial("id").primaryKey(),
  programId: integer("program_id").notNull().references(() => certificationProgramsTable.id, { onDelete: "cascade" }),
  requirementId: integer("requirement_id").references(() => certificationRequirementsTable.id, { onDelete: "set null" }),
  title: text("title").notNull(),
  description: text("description"),
  taskType: text("task_type", {
    enum: ["document_gather", "document_create", "review", "legal", "financial", "submit", "follow_up", "other"],
  }).notNull().default("other"),
  priority: text("priority", { enum: ["critical", "high", "medium", "low"] }).notNull().default("medium"),
  status: text("status", { enum: ["open", "in_progress", "blocked", "complete", "na"] }).notNull().default("open"),
...
export const applicationArtifactsTable = pgTable("application_artifacts", {
  ...
  artifactType: text("artifact_type", {
    enum: ["form", "supporting_doc", "identification", "financial_statement", "legal_doc", "attestation", "correspondence", "other"],
  }).notNull().default("other"),
  status: text("status", { enum: ["needed", "in_progress", "complete", "submitted", "accepted", "rejected"] }).notNull().default("needed"),
  fileUrl: text("file_url"),
  notes: text("notes"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
}, (t) => [
  index("app_artifacts_program_idx").on(t.programId),
]);

// ─── OPPORTUNITY PIPELINE ─────────────────────────────────────────────────────

export const opportunityPipelineTable = pgTable("opportunity_pipeline", {
  id: serial("id").primaryKey(),
  title: text("title").notNull(),
  opportunityType: text("opportunity_type", { enum: ["rfp", "rfq", "sole_source", "idiq", "bpa", "set_aside", "other"] }).notNull().default("other"),
  source: text("source", { enum: ["ny_state", "nyc", "federal", "other"] }).notNull().default("federal"),
  ...
  dueDate: timestamp("due_date"),
  status: text("status", { enum: ["tracking", "qualifying", "pursuing", "submitted", "awarded", "lost", "no_bid", "archived"] }).notNull().default("tracking"),
...
export const procurementContactsTable = pgTable("procurement_contacts", {
  ...
  contactType: text("contact_type", { enum: ["contracting_officer", "small_biz_liaison", "program_manager", "osd_contact", "other"] }).notNull().default("other"),
  relationshipStatus: text("relationship_status", { enum: ["cold", "introduced", "warm", "engaged", "partner"] }).notNull().default("cold"),
  notes: text("notes"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
...
export const certificationCalendarTable = pgTable("certification_calendar", {
  id: serial("id").primaryKey(),
  programId: integer("program_id").references(() => certificationProgramsTable.id, { onDelete: "cascade" }),
  title: text("title").notNull(),
  eventType: text("event_type", {
    enum: ["application_deadline", "renewal_deadline", "status_check", "document_due", "review_meeting", "submission", "follow_up", "other"],
  }).notNull().default("other"),
  eventDate: timestamp("event_date").notNull(),
  reminderDays: integer("reminder_days").notNull().default(14),
  status: text("status", { enum: ["upcoming", "overdue", "complete", "canceled"] }).notNull().default("upcoming"),
  notes: text("notes"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
}, (t) => [
...
export const insertCertificationRequirementSchema = createInsertSchema(certificationRequirementsTable).omit({ id: true, createdAt: true });
...
export const insertCertificationStatusSchema = createInsertSchema(certificationStatusTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertOpportunityPipelineSchema = createInsertSchema(opportunityPipelineTable).omit({ id: true, createdAt: true, updatedAt: true });
...
export const insertProcurementContactSchema = createInsertSchema(procurementContactsTable).omit({ id: true, createdAt: true, updatedAt: true });
