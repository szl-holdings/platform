...
export class Dynamics365Adapter extends ServiceAdapter {
  ...
  readonly description = "Microsoft Dynamics 365 CRM/ERP — Accounts, Contacts, Opportunities, Leads, Cases via Dataverse OData v4";
  ...
  private async odataCreate(entitySet: string, payload: Record<string, unknown>): Promise<Record<string, unknown>> {
    const token = await this.acquireToken();
  ...
  async listAccounts(): Promise<DynamicsAccount[]> {
    ...
    return raw.map((r) => ({
      ...
      name: String(r["name"] ?? ""),
      industry: String(r["industrycode@OData.Community.Display.V1.FormattedValue"] ?? r["industrycode"] ?? ""),
  ...
  async listLeads(): Promise<DynamicsLead[]> {
    ...
    return raw.map((r) => ({
      ...
      leadScore: null,
      status: String(r["statuscode@OData.Community.Display.V1.FormattedValue"] ?? r["statuscode"] ?? ""),
      modifiedOn: String(r["modifiedon"] ?? ""),
    }));
  }
  ...
  async listCases(): Promise<DynamicsCase[]> {
    ...
    return raw.map((r) => ({
      ...
      severity: String(r["severitycode@OData.Community.Display.V1.FormattedValue"] ?? r["severitycode"] ?? ""),
      status: String(r["statecode@OData.Community.Display.V1.FormattedValue"] ?? r["statecode"] ?? ""),
      priority: String(r["prioritycode@OData.Community.Display.V1.FormattedValue"] ?? r["prioritycode"] ?? ""),
  ...
  buildSignalFromOpportunityChange(opp: DynamicsOpportunity, previousStage?: string): DynamicsSyncSignal {
