...
export class MicrosoftGraphAdapter extends ServiceAdapter {
  ...
  readonly description = "Microsoft 365 — SharePoint, OneDrive, Outlook Calendar, Contacts, and Teams notifications via Microsoft Graph API";
  ...
  async testConnection(): Promise<GraphConnectionStatus> {
    ...
    try {
      ...
      return { connected: true, scopes: ["Sites.ReadWrite.All", "Calendars.ReadWrite", "Contacts.ReadWrite", "Files.ReadWrite.All"] };
    } catch {
      return { connected: false };
    }
  }

  async listSharePointFiles(siteId?: string, libraryPath = "/Shared Documents"): Promise<GraphFile[]> {
    ...
    try {
      ...
      return (data.value ?? []).map(item => ({
        ...
        mimeType: String((item.file as Record<string, string>)?.mimeType ?? "application/octet-stream"),
  ...
  async listCalendarEvents(userId?: string, daysAhead = 14): Promise<GraphCalendarEvent[]> {
    ...
    try {
      ...
      return (data.value ?? []).map(item => ({
        ...
        organizer: String((item.organizer as Record<string, Record<string, string>>)?.emailAddress?.address ?? "") || undefined,
  ...
  async listContacts(userId?: string): Promise<GraphContact[]> {
    ...
    try {
      ...
      return (data.value ?? []).map(item => ({
        ...
        emailAddresses: ((item.emailAddresses as Array<{ address: string }>) ?? []).map(e => e.address),
  ...
  async sendTeamsNotification(notification: GraphTeamsNotification): Promise<{ sent: boolean; error?: string }> {
    ...
    try {
      const body = {
        ...
        sections: [{ activityTitle: notification.title ?? "SZL Platform Notification", activityText: notification.text }],
      };
  ...
  async listSharePointSites(): Promise<GraphSharePointSite[]> {
    if (!this.isLive) return [
      { id: "szlholdings.sharepoint.com,demo-site-id", name: "szlholdings", displayName: "SZL Holdings", webUrl: "https://szlholdings.sharepoint.com", description: "SZL Holdings SharePoint" },
    ];
    try {
      const data = await this.graphFetch("/sites?search=") as { value: Array<Record<string, unknown>> };
  ...
  async sync(): Promise<{ synced: number; timestamp: string }> {
    ...
    return { synced: files.length + events.length + contacts.length, timestamp: new Date().toISOString() };
  }
}
