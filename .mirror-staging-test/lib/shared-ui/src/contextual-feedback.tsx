...
export function ContextualFeedback({
  ...
  const handleSubmit = useCallback(async (skipComment = false) => {
    if (!sentiment) return;
    setLoading(true);
    setError(null);
    try {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className={cn(
              ...
              sentiment === "positive" ? "bg-emerald-500/10 text-emerald-600" : "bg-red-500/10 text-red-600"
            )}>
              {sentiment === "positive"
  ...
  return (
    ...
    >
      ...
      {!compact && (
        <button
          onClick={() => { setSentiment("neutral"); setPhase("comment"); }}
