export type RouteClass = "classification" | "triage" | "reasoning" | "planning" | "tool_calling" | "vision_understanding" | "background_batch" | "extraction" | "summarization";
...
const ROUTE_DEFAULTS: Record<RouteClass, { modelFn: () => string; maxTokens: number; temperature: number }> = {
...
export function routeModel(routeClass: RouteClass, overrides?: { model?: string; maxTokens?: number; temperature?: number }): RouteResult {
...
export function getRouteConfig() {
  return {
    ...
    config: {
      ...
      requireApprovalForHighRisk: (process.env["AI_REQUIRE_APPROVAL_FOR_HIGH_RISK"] ?? "true") === "true",
