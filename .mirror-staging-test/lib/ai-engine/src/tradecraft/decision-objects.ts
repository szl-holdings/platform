...
export interface EvidenceRef {
  refId: string;
  source: string;
  sourceType: "alert" | "incident" | "playbook" | "approval" | "analyst_note" | "asset_metadata" | "user_metadata" | "control_doc" | "retention_policy" | "incident_timeline" | "prior_decision" | "retrieval";
  content: string;
...
export interface RiskDecisionObject extends BaseDecisionObject {
  ...
  mitigationSteps: Array<{ step: string; priority: number; owner: string | null; deadline: string | null }>;
...
export interface ApprovalRecommendationObject extends BaseDecisionObject {
  ...
  estimatedImpact: { financial: string | null; operational: string | null; reputational: string | null; compliance: string | null };
}
...
export interface ResponsePlanObject extends BaseDecisionObject {
  ...
  steps: Array<{ stepNumber: number; action: string; owner: string | null; deadline: string | null; status: "pending" | "in_progress" | "complete" | "skipped"; notes: string | null }>;
...
export interface ExecutiveBriefObject extends BaseDecisionObject {
  ...
  riskSummary: { overallRisk: ImpactLevel; trending: "increasing" | "stable" | "decreasing"; mitigationStatus: string };
...
function validateBase(obj: Record<string, unknown>, errors: string[]): void {
  ...
  if (typeof obj.issueStatement !== "string" || obj.issueStatement.length === 0) errors.push("Missing issueStatement");
  ...
  if (typeof obj.confidence !== "number" || obj.confidence < 0 || obj.confidence > 1) errors.push("Invalid confidence");
  if (!["high", "medium", "low", "insufficient"].includes(obj.evidenceQuality as string)) errors.push("Invalid evidenceQuality");
...
export function validateAndBuildTriageDecision(
  ...
  const errors: string[] = [];
  if (!raw || typeof raw !== "object") return { valid: false, object: null, errors: ["Not an object"], rawOutput: meta.rawOutput };
  const obj = raw as Record<string, unknown>;
  validateBase(obj, errors);
  if (!["P0", "P1", "P2", "P3", "P4"].includes(obj.priority as string)) errors.push("Invalid priority");
  if (!["escalate", "investigate", "monitor", "dismiss", "defer"].includes(obj.triageDecision as string)) errors.push("Invalid triageDecision");
  if (typeof obj.routeTo !== "string") errors.push("Missing routeTo");
  if (typeof obj.category !== "string") errors.push("Missing category");
  if (errors.length > 0) return safeFallback("TriageDecision", meta.rawOutput, meta.tenantId, "triage_decision", errors);
  ...
  const built: TriageDecisionObject = {
    ...
    confidenceStatement: (obj.confidenceStatement as string) || `Confidence: ${Math.round(confidence * 100)}%`,
...
export function validateAndBuildIncidentAssessment(
  ...
  if (!["high", "medium", "low"].includes(obj.primaryHypothesisConfidence as string)) errors.push("Invalid primaryHypothesisConfidence");
  if (typeof obj.progressionStage !== "string") errors.push("Missing progressionStage");
  if (errors.length > 0) return safeFallback("IncidentAssessment", meta.rawOutput, meta.tenantId, "case_hypothesis", errors);
  const confidence = obj.confidence as number;
  const built: IncidentAssessmentObject = {
    ...
    primaryHypothesisConfidence: obj.primaryHypothesisConfidence as IncidentAssessmentObject["primaryHypothesisConfidence"],
...
export function validateAndBuildRiskDecision(
  ...
  const obj = raw as Record<string, unknown>;
  validateBase(obj, errors);
  if (typeof obj.riskScore !== "number" || obj.riskScore < 0 || obj.riskScore > 100) errors.push("Invalid riskScore");
  if (typeof obj.riskCategory !== "string") errors.push("Missing riskCategory");
  if (errors.length > 0) return safeFallback("RiskDecision", meta.rawOutput, meta.tenantId, "risk_assessment", errors);
  const confidence = obj.confidence as number;
...
export function validateAndBuildEscalationDecision(
  ...
  if (!["none", "team_lead", "manager", "director", "executive", "external"].includes(obj.escalationLevel as string)) errors.push("Invalid escalationLevel");
  if (typeof obj.escalationReason !== "string") errors.push("Missing escalationReason");
  if (errors.length > 0) return safeFallback("EscalationDecision", meta.rawOutput, meta.tenantId, "escalation_recommendation", errors);
  const confidence = obj.confidence as number;
  const built: EscalationDecisionObject = {
    ...
    communicationChannel: (obj.communicationChannel as EscalationDecisionObject["communicationChannel"]) || "in-app",
...
export function validateAndBuildApprovalRecommendation(
  ...
  if (!["auto", "operator", "manager", "executive", "board"].includes(obj.approvalLevel as string)) errors.push("Invalid approvalLevel");
  if (errors.length > 0) return safeFallback("ApprovalRecommendation", meta.rawOutput, meta.tenantId, "approval_recommendation", errors);
  const confidence = obj.confidence as number;
  const built: ApprovalRecommendationObject = {
    ...
    issueStatement: obj.issueStatement as string,
    evidenceRefs: (obj.evidenceRefs as EvidenceRef[]) || [],
    evidenceQuality: (obj.evidenceQuality as ApprovalRecommendationObject["evidenceQuality"]) || "low",
    assumptions: (obj.assumptions as AnalyticAssumption[]) || [],
    alternatives: (obj.alternatives as AlternativeHypothesis[]) || [],
    confidence,
    confidenceLabel: labelConfidence(confidence),
    ...
    estimatedImpact: (obj.estimatedImpact as ApprovalRecommendationObject["estimatedImpact"]) || { financial: null, operational: null, reputational: null, compliance: null },
  };
  return { valid: true, object: built, errors: [], rawOutput: meta.rawOutput };
}
...
export function validateAndBuildResponsePlan(
  ...
  const obj = raw as Record<string, unknown>;
  validateBase(obj, errors);
  if (!["containment", "eradication", "recovery", "post_incident"].includes(obj.phase as string)) errors.push("Invalid phase");
  ...
  if (errors.length > 0) return safeFallback("ResponsePlan", meta.rawOutput, meta.tenantId, "response_plan", errors);
  const confidence = obj.confidence as number;
...
export function validateAndBuildExecutiveBrief(
  ...
  const obj = raw as Record<string, unknown>;
  validateBase(obj, errors);
  if (typeof obj.headline !== "string" || obj.headline.length === 0) errors.push("Missing headline");
  ...
  if (!["board", "executive", "management"].includes(obj.audienceLevel as string)) errors.push("Invalid audienceLevel");
  if (errors.length > 0) return safeFallback("ExecutiveBrief", meta.rawOutput, meta.tenantId, "executive_brief", errors);
  const confidence = obj.confidence as number;
  const built: ExecutiveBriefObject = {
    ...
    executiveRecommendations: (obj.executiveRecommendations as ExecutiveBriefObject["executiveRecommendations"]) || [],
    riskSummary: (obj.riskSummary as ExecutiveBriefObject["riskSummary"]) || { overallRisk: "medium", trending: "stable", mitigationStatus: "unknown" },
...
export function validateAndBuildControlGapFinding(
  ...
  if (!["active", "likely", "possible", "theoretical"].includes(obj.exploitability as string)) errors.push("Invalid exploitability");
  if (errors.length > 0) return safeFallback("ControlGapFinding", meta.rawOutput, meta.tenantId, "control_gap_summary", errors);
  const confidence = obj.confidence as number;
...
export function validateAndBuildDecision(
  ...
  switch (decisionType) {
    ...
    case "ApprovalRecommendation":
      return validateAndBuildApprovalRecommendation(raw, meta) as ValidationResult<AnyDecisionObject>;
    case "ResponsePlan":
    ...
    default: {
      ...
      return { valid: false, object: null, errors: [`Unknown decisionType: ${exhaustive}`], rawOutput: meta.rawOutput };
    }
  }
}
