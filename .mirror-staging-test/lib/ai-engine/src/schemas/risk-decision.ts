...
export const RISK_DECISION_SCHEMA = {
  ...
  properties: {
    ...
    evidence: { type: "array", items: { type: "object", properties: { source: { type: "string" }, content: { type: "string" }, relevanceScore: { type: "number" } } } },
    confidence: { type: "number", minimum: 0, maximum: 1 },
  },
} as const;

export function validateRiskDecision(raw: unknown): { valid: boolean; decision: RiskDecision | null; errors: string[] } {
  const errors: string[] = [];
  if (!raw || typeof raw !== "object") return { valid: false, decision: null, errors: ["Not an object"] };
  const obj = raw as Record<string, unknown>;
  if (!["critical", "high", "medium", "low", "negligible"].includes(obj.riskLevel as string)) errors.push("Invalid riskLevel");
