...
import {
  Globe, CheckCircle2, Clock, AlertTriangle, ChevronRight, Lock,
  Info, MessageSquare, Mail, Database, Ticket, HardDrive, Users, Calendar, FileText, Radio
} from "lucide-react";
...
const INTEGRATIONS: Integration[] = [
  {
    ...
    auditLog: true,
    failureSurfacing: "Alert surfaced in operator analytics if delivery fails; retry 3x with backoff",
  ...
  },
  {
    ...
    configNote: "OAuth token stored in tenant secret vault. Hook ready — requires customer tenant app registration.",
  },
  {
    id: "email",
    name: "Email / SMTP",
  ...
  },
  {
    ...
    auditLog: true,
    failureSurfacing: "Ingestion lag and parse errors shown in operator analytics; dead-letter queue for failed events",
    events: ["alert_ingest", "correlation_trigger"],
    hookPoint: "GET /api/integrations/siem/ingest",
    configNote: "Hook ready. Adapter supports Splunk HEC and generic syslog. Customer provides SIEM endpoint and API key.",
  },
  {
    id: "ticketing",
    ...
    auditLog: true,
    failureSurfacing: "API error surfaced with ticket ID and HTTP status; retry with exponential backoff",
  ...
  },
  {
    ...
    auditLog: true,
    failureSurfacing: "Auth failure surfaced to user with error code; fallback to local credentials if configured",
    events: ["user_login", "user_provisioned", "user_deprovisioned"],
    hookPoint: "GET /api/auth/sso/callback",
    configNote: "OIDC/SAML hook ready. Tested with mock IdP. Customer provides IdP metadata and client credentials.",
  },
  {
    id: "calendar",
    ...
    auditLog: true,
    failureSurfacing: "Calendar event creation failure logged; no silent retry — explicit error displayed",
    events: ["drill_scheduled", "review_scheduled", "briefing_scheduled"],
    hookPoint: "POST /api/integrations/calendar/create-event",
    configNote: "Hook ready for Exchange Online and Google Workspace. Customer provides OAuth scope approval.",
  },
  {
    id: "docs",
    ...
    auditLog: true,
    failureSurfacing: "Document creation failure surfaced with link to retry; no auto-retry to avoid duplicate docs",
    events: ["report_published", "after_action_created", "playbook_updated"],
    hookPoint: "POST /api/integrations/docs/publish",
    configNote: "Hook ready for SharePoint Online and Google Drive. Customer provides OAuth credentials and folder path.",
  },
];
...
const CATEGORIES = ["All", "Notifications", "Alert Ingestion", "Case Management", "Evidence", "Identity", "Scheduling", "Documentation"];
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-xl font-bold text-white font-mono tracking-tight">Integration Hub</h1>
          </div>
          <p className="text-xs text-[#8b9ab0] font-mono">Teams · Slack · Email · SIEM · Ticketing · Evidence store · Identity · Calendar · Docs — all tenant-owned, all auditable</p>
        </div>
        ...
        <div className="bg-amber-400/10 border border-amber-400/30 rounded-xl p-4 flex items-start gap-3">
          ...
          <p className="text-xs text-amber-200/80 font-mono leading-relaxed">
            All integrations are <strong>tenant-owned</strong>: credentials belong to and are controlled by each customer tenant. Aegis never shares credentials across tenants. Hook Ready means the adapter is built but requires per-customer configuration. Failure surfacing is explicit — no silent failures.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {CATEGORIES.map(c => (
            <button key={c} onClick={() => setSelectedCategory(c)}
              className={`px-3 py-1.5 rounded text-xs font-mono transition-colors ${selectedCategory === c ? "bg-amber-400/20 text-amber-300 border border-amber-400/40" : "text-[#8b9ab0] border border-[#1e2a3a] hover:text-white"}`}>{c}</button>
          ))}
        </div>
          ...
          {filtered.map(integration => {
            ...
            return (
              <div key={integration.id}>
                <button onClick={() => setSelectedIntegration(isSelected ? null : integration)}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="space-y-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-xs px-1.5 py-0.5 rounded font-mono font-bold ${statusStyle.bg}`}>{statusStyle.label}</span>
                      </div>
                      <p className="text-xs text-[#8b9ab0]">{integration.category} · {integration.permission}</p>
                      <p className="text-xs text-[#8b9ab0]/60 font-mono truncate">{integration.hookPoint}</p>
                    </div>
                    <div className="flex items-center gap-1.5 ml-2 shrink-0">
                      {integration.auditLog && <span className="text-xs text-green-400 font-mono">Audited</span>}
                      <ChevronRight size={14} className={`text-[#8b9ab0] transition-transform ${isSelected ? "rotate-90" : ""}`} />
                    </div>
                  </div>
                </button>
                  ...
                  <div className="mt-1 p-4 bg-[#0a0f16] border border-amber-400/20 border-t-0 rounded-b-xl space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1"><CheckCircle2 size={12} className="text-green-400" /><p className="text-xs text-green-400">Every action</p></div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {integration.events.map(e => <span key={e} className="text-xs bg-[#1e2a3a] text-[#8b9ab0] px-2 py-0.5 rounded font-mono">{e}</span>)}
                      </div>
                    </div>
                    ...
                    {integration.configNote && (
                      <div className="p-2 bg-amber-400/10 border border-amber-400/20 rounded flex items-start gap-2">
                        ...
                        <p className="text-xs text-amber-200/80 font-mono">{integration.configNote}</p>
                      </div>
                    )}
                  </div>
                )}
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: "Connected", value: INTEGRATIONS.filter(i => i.status === "connected").length, color: "#22c55e" },
            { label: "Hook Ready", value: INTEGRATIONS.filter(i => i.status === "hook_ready").length, color: "#f59e0b" },
            { label: "All Audited", value: INTEGRATIONS.filter(i => i.auditLog).length, color: "#3b82f6" },
          ].map(m => (
            <div key={m.label} className="bg-[#0d1117] border border-[#1e2a3a] rounded-xl p-4 text-center">
