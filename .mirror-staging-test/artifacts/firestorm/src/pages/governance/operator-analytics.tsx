...
interface Incident { id: number; status: string; severity: string }
interface Case { id: number; status: string }
interface Decision { id: number; objectId?: string; decisionType?: string; recommendedAction?: string; summary?: string; impactLevel?: string; urgency?: string; approvalRequired?: boolean; humanReviewRequired?: boolean; approvedAt?: string | null; rejectedAt?: string | null; caseId?: string; incidentId?: string; createdAt?: string; modelRoute?: string }

function MetricCard({ label, value, sub, trend, icon: Icon, color = "#f59e0b" }: {
...
export default function OperatorAnalyticsPage() {
  ...
  const pendingApprovals = Array.isArray(decisions)
    ? decisions.filter(d => (d.approvalRequired || d.humanReviewRequired) && !d.approvedAt && !d.rejectedAt).length
    : 0;
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <h1 className="text-xl font-bold text-white font-mono tracking-tight">Operator Analytics</h1>
            </div>
            <p className="text-xs text-[#8b9ab0] font-mono">Agent runs · Approvals · Policy blocks · Retrieval · Model usage · Latency</p>
          </div>
          <div className="flex items-center gap-2">
            {(["24h", "7d", "30d"] as const).map(r => (
              <button key={r} onClick={() => setTimeRange(r)}
                className={`px-3 py-1.5 rounded text-xs font-mono transition-colors ${timeRange === r ? "bg-amber-400/20 text-amber-300 border border-amber-400/40" : "text-[#8b9ab0] border border-[#1e2a3a] hover:text-white"}`}>{r}</button>
            ))}
          </div>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <MetricCard label="Agent Runs (24h)" value="428" sub="+12% vs yesterday" trend="up" icon={Activity} />
          <MetricCard label="Pending Approvals" value={pendingApprovals} sub={`${pendingApprovals} awaiting action`} trend={pendingApprovals > 3 ? "down" : "flat"} icon={Clock} color="#f97316" />
          <MetricCard label="Policy Blocks (7d)" value="22" sub="3 cross-tenant blocked" trend="down" icon={Lock} color="#ef4444" />
          <MetricCard label="Active Incidents" value={activeIncidents} sub={`${totalIncidents} total`} trend="flat" icon={AlertTriangle} color="#ef4444" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-[#0d1117] border border-[#1e2a3a] rounded-xl p-5">
            <h3 className="text-xs text-[#8b9ab0] uppercase tracking-widest font-mono mb-4">Agent Runs & Failures — 24h</h3>
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={AGENT_RUN_DATA}>
                ...
                <Area type="monotone" dataKey="runs" stroke="#f59e0b" fill="#f59e0b22" strokeWidth={2} />
          ...
          <div className="bg-[#0d1117] border border-[#1e2a3a] rounded-xl p-5">
            <h3 className="text-xs text-[#8b9ab0] uppercase tracking-widest font-mono mb-4">Response Latency — p50 / p95 / p99 (ms)</h3>
            <ResponsiveContainer width="100%" height={200}>
          ...
          <div className="bg-[#0d1117] border border-[#1e2a3a] rounded-xl p-5">
            <h3 className="text-xs text-[#8b9ab0] uppercase tracking-widest font-mono mb-4">Retrieval Quality — Hit / Miss Rate (%)</h3>
            <ResponsiveContainer width="100%" height={200}>
          ...
          <div className="bg-[#0d1117] border border-[#1e2a3a] rounded-xl p-5">
            <h3 className="text-xs text-[#8b9ab0] uppercase tracking-widest font-mono mb-4">Policy Blocks — 7 days</h3>
            <ResponsiveContainer width="100%" height={200}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-[#0d1117] border border-[#1e2a3a] rounded-xl p-5">
            <h3 className="text-xs text-[#8b9ab0] uppercase tracking-widest font-mono mb-4">Model Usage & Cost</h3>
              ...
              {MODEL_USAGE.map(m => (
                <div key={m.name} className="space-y-1">
                  ...
                  <div className="h-1.5 bg-[#1e2a3a] rounded-full overflow-hidden">
                    <div className="h-full bg-amber-400 rounded-full" style={{ width: `${m.pct}%` }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-[#0d1117] border border-[#1e2a3a] rounded-xl p-5">
            <h3 className="text-xs text-[#8b9ab0] uppercase tracking-widest font-mono mb-2">Pending Approvals Queue</h3>
            <p className="text-xs text-[#8b9ab0] mb-4 font-mono">Live from tradecraft decisions API</p>
            <div className="space-y-3">
              {pendingApprovals === 0 && (
                <p className="text-xs text-[#8b9ab0] font-mono">No pending approvals in queue</p>
              )}
              {Array.isArray(decisions) && decisions
                .filter(d => (d.approvalRequired || d.humanReviewRequired) && !d.approvedAt && !d.rejectedAt)
                .slice(0, 5)
                .map((a: Decision) => {
                const impact = a.impactLevel ?? "medium";
                return (
                  <div key={a.id ?? a.objectId} className="flex items-start justify-between p-3 bg-[#0a0f16] border border-[#1e2a3a] rounded-lg">
                    <div className="space-y-0.5 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className={`text-xs px-1.5 py-0.5 rounded font-mono font-bold ${impact === "critical" ? "bg-red-500/20 text-red-400" : impact === "high" ? "bg-orange-500/20 text-orange-400" : "bg-blue-500/20 text-blue-400"}`}>{String(impact).toUpperCase()}</span>
                        <span className="text-xs text-[#8b9ab0] font-mono">{a.objectId ?? String(a.id)}</span>
                      </div>
                      <p className="text-xs text-white truncate">{a.recommendedAction ?? a.summary ?? a.decisionType ?? "Decision pending review"}</p>
                      <p className="text-xs text-[#8b9ab0] font-mono">{a.modelRoute ?? "alloy"} · {a.caseId ?? a.incidentId ?? "—"}</p>
                    </div>
                    <Link href="/response-orchestration">
                      <button className="ml-3 px-3 py-1.5 bg-amber-400/20 border border-amber-400/40 text-amber-300 text-xs rounded font-mono hover:bg-amber-400/30 transition-colors whitespace-nowrap">Review</button>
                    </Link>
                  </div>
                );
              })}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <MetricCard label="Avg p50 Latency" value="148ms" sub="-12ms vs yesterday" trend="up" icon={Zap} color="#22c55e" />
          <MetricCard label="Model Cost (7d)" value="$41.82" sub="+6% vs prior week" trend="down" icon={Database} color="#f59e0b" />
          <MetricCard label="Retrieval Hit Rate" value="92.3%" sub="+1.2% vs 7d avg" trend="up" icon={Eye} color="#22c55e" />
          <MetricCard label="Failure Rate" value="1.9%" sub="-0.3% vs yesterday" trend="up" icon={AlertOctagon} color="#ef4444" />
        </div>
      </div>
    </div>
  );
