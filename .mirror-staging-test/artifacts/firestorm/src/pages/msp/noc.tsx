...
function UptimeBar({ service, uptime, incidents }: UptimeEntry) {
  ...
  return (
    <div className="flex items-center gap-4 py-2">
      ...
      <span className={cn("text-sm font-mono w-16 text-right", textColor)}>{uptime.toFixed(2)}%</span>
    ...
    <div className="glass-card rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className={`w-2 h-2 rounded-full ${metrics.device.status === "online" ? "bg-emerald-400 animate-pulse" : "bg-amber-400 animate-pulse"}`} />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="rounded-lg bg-muted/40 p-3">
          ...
          <p className={`text-2xl font-bold ${m.cpu.percent > 80 ? "text-red-400" : m.cpu.percent > 60 ? "text-amber-400" : "text-emerald-400"}`}>{m.cpu.percent}%</p>
          <p className="text-[10px] text-muted-foreground mt-1">{m.cpu.cores} cores · {m.cpu.loadAvg1m} load</p>
          <div className="mt-2 h-1.5 bg-muted rounded-full overflow-hidden">
            <div className={`h-full rounded-full ${m.cpu.percent > 80 ? "bg-red-400" : m.cpu.percent > 60 ? "bg-amber-400" : "bg-emerald-400"}`} style={{ width: `${m.cpu.percent}%` }} />
          </div>
        </div>
        <div className="rounded-lg bg-muted/40 p-3">
          <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1">Memory</p>
          <p className={`text-2xl font-bold ${m.memory.percent > 85 ? "text-red-400" : m.memory.percent > 70 ? "text-amber-400" : "text-emerald-400"}`}>{m.memory.percent}%</p>
          <p className="text-[10px] text-muted-foreground mt-1">{m.memory.usedGb}GB / {m.memory.totalGb}GB</p>
          <div className="mt-2 h-1.5 bg-muted rounded-full overflow-hidden">
            <div className={`h-full rounded-full ${m.memory.percent > 85 ? "bg-red-400" : m.memory.percent > 70 ? "bg-amber-400" : "bg-emerald-400"}`} style={{ width: `${m.memory.percent}%` }} />
          </div>
        </div>
        <div className="rounded-lg bg-muted/40 p-3">
        ...
        <div className="rounded-lg bg-muted/40 p-3">
          ...
          <p className="text-[10px] text-muted-foreground mt-1">{m.process.nodeVersion} · {Math.round(m.process.uptime / 60)}m</p>
          <div className="mt-2 h-1.5 bg-muted rounded-full overflow-hidden">
...
export default function NOCPage() {
  ...
  const serviceUptime: UptimeEntry[] = healthData?.checks
    ...
    : [
        ...
        { service: "Authentication", uptime: 100.00, incidents: 0 },
  ...
  const avgUptime = serviceUptime.length > 0
    ? Math.round(serviceUptime.reduce((s, u) => s + u.uptime, 0) / serviceUptime.length * 100) / 100
    : 99.9;
  ...
  const additionalAlerts: Alert[] = [
    { id: "a1", title: "SSL certificate expiring in 7 days — *.pinnaclehealth.org", source: "Certificate Monitor", client: "Pinnacle Health", severity: "warning", acknowledged: false, timestamp: new Date(Date.now() - 8 * 60000).toISOString() },
    { id: "a2", title: "Backup completed — 2.4TB processed successfully", source: "Backup Agent", client: "Vertex Labs", severity: "info", acknowledged: true, timestamp: new Date(Date.now() - 15 * 60000).toISOString() },
    { id: "a3", title: "Ransomware activity detected — NovaTech endpoint isolated", source: "CrowdStrike Falcon", client: "NovaTech", severity: "critical", acknowledged: false, timestamp: new Date(Date.now() - 35 * 60000).toISOString() },
    { id: "a4", title: "VPN gateway authentication failure spike — 23 events", source: "SIEM", client: "Meridian Corp", severity: "warning", acknowledged: false, timestamp: new Date(Date.now() - 45 * 60000).toISOString() },
    { id: "a5", title: "Storage array disk health warning — RAID rebuild initiated", source: "NetApp ONTAP", client: "Horizon Logistics", severity: "warning", acknowledged: true, timestamp: new Date(Date.now() - 68 * 60000).toISOString() },
  ];
  ...
  const incidentTimeline = [
    { time: new Date(Date.now() - 2 * 60000).toLocaleTimeString(), event: "API Server metrics polled — all systems nominal", severity: "info" },
    { time: new Date(Date.now() - 35 * 60000).toLocaleTimeString(), event: "NovaTech: CrowdStrike Falcon isolated WORKSTATION-NV-22 — ransomware signature match", severity: "critical" },
    { time: new Date(Date.now() - 45 * 60000).toLocaleTimeString(), event: "Meridian Corp: VPN auth failure spike detected — 23 failed attempts from 203.0.113.45", severity: "warning" },
    { time: new Date(Date.now() - 68 * 60000).toLocaleTimeString(), event: "Horizon Logistics: NetApp FAS8300 disk failure — RAID reconstruction started at 34%", severity: "warning" },
    { time: new Date(Date.now() - 90 * 60000).toLocaleTimeString(), event: "Vertex Labs: Backup job completed — 2.4TB processed, 0 errors", severity: "info" },
    { time: new Date(Date.now() - 120 * 60000).toLocaleTimeString(), event: "Pinnacle Health: SSL certificate expiration warning triggered — 7 days remaining", severity: "warning" },
    { time: new Date(Date.now() - 180 * 60000).toLocaleTimeString(), event: "Atlas Industries: Patch cycle completed — 198 endpoints updated, 0 failures", severity: "info" },
  ];
    ...
    <div className="p-6 space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Network Operations Center — infrastructure uptime, alert triage, and real-time system health</p>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[10px] text-muted-foreground font-mono">Updated {lastUpdated.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })}</span>
          <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "Avg Uptime", value: `${avgUptime}%`, color: "text-emerald-400", icon: Activity },
        ].map((stat, i) => (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 glass-card rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-border/40 flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">{allAlerts.length} total · {unacknowledged} unacknowledged</span>
          </div>
      ...
      <div className="glass-card rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className={cn("text-sm font-mono", avgUptime >= 99.5 ? "text-emerald-400" : "text-amber-400")}>Avg: {avgUptime.toFixed(2)}%</span>
        </div>
