...
export const analytics = {
  ...
  approvalDecision(actionId: string | number, decision: "approved" | "rejected", latencyMs?: number) {
  ...
  searchExecuted(query: string, resultCount?: number) {
    return postEvent("search_executed", { platform: "lyte", query: query.slice(0, 50), resultCount });
  },
