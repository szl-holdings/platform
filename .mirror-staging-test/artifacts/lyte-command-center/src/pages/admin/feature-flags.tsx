...
export default function FeatureFlags() {
  ...
  const filtered = search ? flags.filter(f => f.name.toLowerCase().includes(search.toLowerCase()) || f.key.toLowerCase().includes(search.toLowerCase())) : flags;
      ...
      {error ? (
        ...
        <>
          ...
          <div className="relative max-w-sm">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search flags..." className="w-full pl-9 pr-4 py-2 text-sm bg-muted rounded-lg border border-border focus:outline-none focus:ring-1 focus:ring-primary" />
          </div>
          ...
          <div className="rounded-xl border border-border bg-card divide-y divide-border">
            {isLoading ? (
              ...
              filtered.map((flag) => (
                <div key={flag.key} className="px-4 py-4 flex items-center justify-between gap-4 hover:bg-muted/30 transition-colors">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <span className="text-sm font-semibold">{flag.name}</span>
                      <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${categoryColors[flag.category] ?? "text-muted-foreground bg-muted"}`}>{flag.category}</span>
                    ...
                    {flag.rolloutPercentage < 100 && (
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        <span className="text-[10px] text-muted-foreground">{flag.rolloutPercentage}% rollout</span>
                      </div>
                    )}
                  </div>
                  <button
            ...
            {!isLoading && filtered.length === 0 && <div className="py-8 text-center text-sm text-muted-foreground">No flags found</div>}
          </div>
        </>
      )}
    </div>
