...
const SERVICES = [
  ...
  { id: "property-coordination", title: "Property Coordination", desc: "Operational oversight across multiple properties." },
  { id: "household-systems", title: "Household Systems", desc: "Staff management, protocols, and household infrastructure." },
  { id: "vendor-management", title: "Vendor Management", desc: "Vetted provider network and vendor accountability." },
  { id: "lifestyle-admin", title: "Lifestyle & Admin", desc: "Administrative and lifestyle support for demanding schedules." },
  { id: "special-projects", title: "Special Projects", desc: "Estate activations, relocations, and high-stakes transitions." },
];
...
export default function BookingPage() {
  ...
  const canProceed = () => {
    ...
    if (step === "details") return !!form.name.trim() && !!form.email.trim() && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email);
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setConfirmationId(data.inquiryId ? `CJ-REQ-${data.inquiryId}` : "CJ-" + Date.now().toString(36).toUpperCase());
      setSubmitted(true);
    } catch {
      setError("Something went wrong. Please try again or contact Rosa directly at inquiries@carlotajo.com.");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="min-h-screen flex flex-col" style={{ background: "var(--color-cream-warm)" }}>
        ...
        <main className="flex-1 flex items-center justify-center pt-20 pb-16">
          ...
          >
            ...
            <h1 className="font-serif text-3xl font-light mb-4" style={{ color: "var(--color-ink-900)" }}>
              Thank you, {form.name.split(" ")[0]}.
            </h1>
            <p className="text-sm font-light leading-relaxed mb-4" style={{ color: "var(--color-ink-600)" }}>
              Rosa has received your consultation request and will respond personally within two business days. Your enquiry is handled with complete confidentiality.
            </p>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={item.label} className="flex justify-between gap-4">
                    <span className="text-[12px] font-light" style={{ color: "var(--color-stone-400)" }}>{item.label}</span>
                    <span className="text-[12px] font-light text-right" style={{ color: "var(--color-ink-700)" }}>{item.value}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <main className="flex-1 pt-28 pb-16">
        <div className="max-w-2xl mx-auto px-6 lg:px-12">
          <div className="text-center mb-12">
            <p className="text-[11px] font-medium tracking-[0.4em] uppercase mb-3" style={{ color: "var(--color-gold)" }}>
              Consultation Request
            </p>
            <h1 className="font-serif text-3xl lg:text-4xl font-light mb-3" style={{ color: "var(--color-ink-900)" }}>
              Request a confidential consultation.
            </h1>
            <p className="text-sm font-light max-w-md mx-auto" style={{ color: "var(--color-ink-500)" }}>
              Rosa responds personally within two business days. There is no obligation after an initial conversation.
            </p>
          </div>
              ...
              <div key={s.id} className="flex items-center gap-1.5">
                <div className="flex flex-col items-center gap-2">
                  <div
                    ...
                    style={{
                      background: i < stepIdx ? "var(--color-gold-dim)" :
                        i === stepIdx ? "var(--color-gold)" : "transparent",
                      color: i < stepIdx ? "var(--color-gold)" :
                        i === stepIdx ? "var(--color-cream)" : "var(--color-stone-400)",
                      border: i < stepIdx ? "1px solid var(--color-gold-border)" :
                        i === stepIdx ? "none" : "1px solid var(--color-stone-200)",
                    }}
                  >
                ...
                {i < steps.length - 1 && (
                  <div
                    ...
                    style={{ background: i < stepIdx ? "var(--color-gold-border)" : "var(--color-stone-200)" }}
                  />
                )}
              </div>
            ))}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {step === "service" && (
                <div>
                  <h3 className="font-serif text-xl font-light mb-2" style={{ color: "var(--color-ink-900)" }}>
                    What are you looking for support with?
                  </h3>
                  <p className="text-[13px] font-light mb-7" style={{ color: "var(--color-ink-500)" }}>
                    Select the area closest to your needs. Rosa will clarify scope during the initial conversation.
                  </p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {SERVICES.map((svc) => (
                      <button
                        ...
                        style={{
                          border: form.service === svc.id ? "1px solid var(--color-gold)" : "1px solid var(--color-stone-200)",
                          background: form.service === svc.id ? "rgba(154,125,82,0.04)" : "white",
                        }}
                        onMouseEnter={(e) => {
                          if (form.service !== svc.id) (e.currentTarget as HTMLElement).style.borderColor = "var(--color-stone-300)";
                        }}
                        onMouseLeave={(e) => {
                          if (form.service !== svc.id) (e.currentTarget as HTMLElement).style.borderColor = "var(--color-stone-200)";
                        }}
                      >
                        <h4 className="text-[14px] font-medium mb-1" style={{ color: "var(--color-ink-900)" }}>
                          {svc.title}
                        </h4>
              ...
              {step === "schedule" && (
                <div className="space-y-7">
                  <div>
                    ...
                    <p className="text-[13px] font-light mb-7" style={{ color: "var(--color-ink-500)" }}>
                      Select a preferred date and time. All times are New York (ET). Rosa will confirm within two business days.
                    </p>
                  </div>
                  <div>
                    <p className="text-[11px] font-medium tracking-[0.18em] uppercase mb-3" style={{ color: "var(--color-stone-400)" }}>
                      Available dates
                    </p>
                    <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-7 gap-2">
                      {getNextBusinessDays().map((d) => (
                        <button
                          ...
                          style={{
                            border: form.date === d ? "1px solid var(--color-gold)" : "1px solid var(--color-stone-200)",
                    ...
                    <div className="grid grid-cols-4 sm:grid-cols-7 gap-2">
                      {TIME_SLOTS.map((t) => (
                        <button
                          ...
                          style={{
                            border: form.time === t ? "1px solid var(--color-gold)" : "1px solid var(--color-stone-200)",
              ...
              {step === "details" && (
                <div className="space-y-5">
                  <div>
                    ...
                    <p className="text-[13px] font-light mb-7" style={{ color: "var(--color-ink-500)" }}>
                      This is used solely to confirm the consultation and allow Rosa to prepare. All information is handled in confidence.
                    </p>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-[11px] font-medium tracking-[0.18em] uppercase mb-2" style={{ color: "var(--color-stone-400)" }}>
                        Full name *
                      </label>
                      <input
                        type="text"
                        ...
                        onFocus={(e) => { (e.currentTarget as HTMLElement).style.borderColor = "var(--color-gold)"; }}
                  ...
                  >
                    {[
                      ...
                      <div key={item.label}>
                        ...
                        {i < arr.length - 1 && (
                          <div className="mt-4 h-px" style={{ background: "var(--color-stone-200)" }} />
                        )}
                      </div>
                    ))}
                  </div>
                  <div className="flex items-start gap-3 text-[11px] font-light" style={{ color: "var(--color-stone-400)" }}>
                    <Shield size={14} style={{ marginTop: "0.05rem", flexShrink: 0, color: "var(--color-stone-300)" }} />
                    All details are held in strict confidence by Rosa personally. Nothing is shared.
                  </div>
                  {error && (
                    <p className="text-sm font-light" style={{ color: "var(--color-ink-500)", background: "rgba(200,80,80,0.06)", padding: "12px 16px", border: "1px solid rgba(200,80,80,0.15)" }}>
                      {error}
                    </p>
                  )}
                </div>
          ...
          <div className="flex items-center justify-between mt-12">
            ...
            {step === "review" ? (
              ...
              <button
                ...
                style={{ color: "var(--color-cream)", background: "var(--color-gold)" }}
                onMouseEnter={(e) => { if (canProceed()) (e.currentTarget as HTMLElement).style.background = "var(--color-gold-light)"; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = "var(--color-gold)"; }}
              >
                Continue
                <ArrowRight size={13} />
