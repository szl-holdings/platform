# Vessels (@vessels_maritime) — X poster

Posts the 5-item Vessels launch sequence from `post_plan.json` to X. Same
runner pattern as the `@szlholdings` parent kit.

## 1 — Get X API credentials for `@vessels_maritime`

1. **developer.x.com** → sign in as `@vessels_maritime`.
...
> If you switch between subaccounts in one workspace, override the access
> tokens per run via shell exports.

## 3 — Dry run

```bash
cd output/vessels-x-launch-kit/poster
...
python3 post_to_x.py --live --only 2   # Thu voyage P&L
